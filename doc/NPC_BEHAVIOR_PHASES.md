...
6. **P1 — make the deployment reproducible from source control.** At audit time, the five feature-package directories, deployment/configuration scripts, tests, and supporting documentation are untracked in the sibling `world-contracts` checkout, while base-package files are modified. The live chain is coherent, but commit/review the intended source, lock files, and generated-artifact policy before treating it as reproducible.
