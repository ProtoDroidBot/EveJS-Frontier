...
> **Status: In progress.** Gate, catapult, jump-drive, fuel-parity, maintenance, and route-state prerequisites exist; persistent NPC fuel provisioning, the unified NPC route executor, and the crash-safe cross-system transition journal remain to be implemented.
...
The fuel runtime is actor-neutral, but current NPCs are deliberately not given synthetic fuel. Native NPC profiles persist the opt-in `npcFuelRequirementsEnabled`; it defaults off so existing fleets keep their compatibility behavior. A later Phase 5 step must turn it on while provisioning a persistent tank/queue on eligible NPC hulls, persist fuel across restart and respawn according to loss policy, select compatible fuel, reserve a route margin, create refueling/hauling work below that margin, and suspend or request logistics when stranded. Once that explicit state exists, NPCs use the same propulsion preflight, FIFO burn, jump-drive debit, and fuel-property formulas as players. No Sui contract deployment is required for this runtime prerequisite.
