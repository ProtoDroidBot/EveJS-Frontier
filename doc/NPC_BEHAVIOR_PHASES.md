...
| 3 — assemblies and access | **In progress** | NPC construction sites, placement clearance, material fulfilment, atomic realization, network-node placement/fuel integration, local access requests/grants, Sui access contracts, chain proof verification, idempotent cross-owner custody transactions, and crash-safe multi-lane Industry execution. | After the environment rebuild, publish/upgrade `assembly_access` into the active Localnet package; consume network-node journals with durable NPC cursors, complete load-shedding policy, and add optional player assembly-access GUI controls. |
...
The status above describes repository implementation. Optional client UI exists only where a phase explicitly says so; Phase 1 includes it, while later-phase optional controls remain planned. The table does not claim that every contract is active in the currently published Localnet package. In particular, the new `assembly_access` source is compiled and tested through the efctl-managed Sui toolchain, but still requires a compatible Localnet package upgrade and deployment-manifest sync before smart-assembly grants can become live on that chain.
...
> **Status: In progress.** Construction, placement, fulfilment, Industry lanes, local access control, Sui contract source, proof verification, and cross-owner custody are implemented. Live Localnet access-contract activation is deliberately deferred until the requested environment rebuild; durable network-node signal consumption remains outstanding.
...
Completed smart assemblies keep their authoritative grant/revocation state in deterministic `world::assembly_access` shared objects on Sui. Direct issuance proves the assembly's exact `OwnerCap`; player and NPC delegation proves the parent grant plus its Character or NPC profile. The contract prevents capability amplification, excessive expiry, delegation-cycle abuse, and replay. Field and portable assemblies use the equivalent durable local policy model. A local projection may accelerate reads but cannot override the authoritative source.
