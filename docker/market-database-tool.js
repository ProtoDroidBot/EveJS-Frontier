...
function validateDatabase(databasePath) {
    ...
    try {
        ...
        if (missingTables.length > 0) {
            throw new Error(`Market database is missing required table(s): ${missingTables.join(", ")}`);
        }
        ...
        for (const requiredContentTable of [
            ...
            if (counts[requiredContentTable] < 1) {
                throw new Error(`Market database table ${requiredContentTable} is unexpectedly empty.`);
            }
        }
...
async function backupDatabase(sourcePath, backupDirectory, label) {
    ...
    const backupPath = path.join(backupDirectory, `market-${utcFileTimestamp()}-${safeLabel(label)}.sqlite`);
...
function resolveBackup(backupDirectory, selector) {
    ...
    if (!match) {
        throw new Error(`Market backup '${selector}' was not found. Run the backups command first.`);
    }
    return match;
}
...
//# sourceMappingURL=market-database-tool.js.map