...
const { getTypeAttributeValue, getModuleChargeCapacity, } = require(path.join(__dirname, "../services/fitting/liveFittingState"));
const { resolveItemByName, } = require(path.join(__dirname, "../services/inventory/itemTypeRegistry"));
...
const { FIGHTER_TUBE_FLAGS, grantItemToCharacterLocation, removeInventoryItem, updateInventoryItem, } = require(path.join(__dirname, "../services/inventory/itemStore"));
const { clearRemoteRepairShowController, clearRemoteRepairShowFighters, registerRemoteRepairShowController, tickScene: tickRemoteRepairShowScene, } = require(path.join(__dirname, "./remoteRepairShowRuntime"));
...
function buildFormationBasis(direction) {
    ...
    const upReference = Math.abs(toFiniteNumber(forward.y, 0)) >= 0.95
        ? { x: 1, y: 0, z: 0 }
        : { x: 0, y: 1, z: 0 };
    const right = normalizeVector(crossVectors(forward, upReference), { x: 0, y: 0, z: 1 });
...
function isRemoteRepairShowEntity(entity) {
    ...
    return Boolean(entity.nativeNpc === true &&
...
function buildSyntheticRepairShipSpec(ownerSession, blueprint, position, direction, entityID, skillMap) {
    ...
    for (const moduleDef of Array.isArray(blueprint.modules) ? blueprint.modules : []) {
        ...
        const moduleItem = buildSyntheticOnlineModuleItem(entityID, moduleType, flagID, slotIndex, moduleDef.family);
    ...
    for (const cargoDef of Array.isArray(blueprint.cargo) ? blueprint.cargo : []) {
        ...
        nativeCargoItems.push(buildSyntheticCargoItem(entityID, cargoType, cargoDef && cargoDef.quantity, 0));
    }
    for (const preloadDef of Array.isArray(blueprint.preloadCharges) ? blueprint.preloadCharges : []) {
        ...
        nativeCargoItems.push(buildSyntheticCargoItem(entityID, chargeType, loadedQuantity, moduleItem.itemID));
    }
    return {
        itemID: entityID,
...
function resolveRequestedFleetCount(argumentText) {
    ...
    const count = clamp(parsed, MIN_REMOTE_REPAIR_FLEET_COUNT, MAX_REMOTE_REPAIR_FLEET_COUNT);
    return {
        success: true,
        count,
...
function buildSuperBlueprint(wing, superIndex = 1) {
    ...
    return Object.freeze({
        ...
        orbitStrategy: "wingAnchor",
        orbitDistance: Math.max(SUPERCAP_ORBIT_DISTANCE_METERS, buildCapitalRingRadius(capitalSlotIndex - 1)),
        formationRadius: Math.max(SUPERCAP_ORBIT_DISTANCE_METERS, buildCapitalRingRadius(capitalSlotIndex - 1)),
...
function buildCommandShipBlueprints(count) {
    return COMMAND_SHIP_TEMPLATES.slice(0, Math.max(0, Math.min(COMMAND_SHIP_TEMPLATES.length, toInt(count, 0))));
}
...
function selectRosterBlueprints(count) {
    const resolvedCount = Math.max(MIN_REMOTE_REPAIR_FLEET_COUNT, Math.min(MAX_REMOTE_REPAIR_FLEET_COUNT, toInt(count, DEFAULT_REMOTE_REPAIR_FLEET_COUNT)));
    const commandShipCount = Math.min(resolveCommandShipCount(resolvedCount), Math.max(0, resolvedCount - 1));
...
function buildLogiSlotPosition(wingCenter, basis, wing, blueprint) {
    ...
    const radius = Math.max(LOGI_RING_BASE_RADIUS_METERS, toFiniteNumber(blueprint && blueprint.formationRadius, LOGI_RING_BASE_RADIUS_METERS));
    ...
    return addVectors(wingCenter, addVectors(scaleVector(basis.forward, forwardOffset - ringDepthOffset), scaleVector(basis.up, upOffset)));
}
function buildCommandSlotPosition(center, wingCenter, basis, blueprint) {
    ...
    if (blueprint.wing === "center") {
        return addVectors(center, addVectors(scaleVector(basis.forward, -COMMAND_SHIP_CENTER_ORBIT_DISTANCE_METERS), scaleVector(basis.up, COMMAND_SHIP_UP_OFFSET_METERS + 750)));
    }
    return addVectors(wingCenter, addVectors(scaleVector(basis.forward, blueprint.wing === "right"
        ? COMMAND_SHIP_FORWARD_OFFSET_METERS
        : -COMMAND_SHIP_FORWARD_OFFSET_METERS), scaleVector(basis.up, COMMAND_SHIP_UP_OFFSET_METERS)));
}
...
function buildFormationSlots(ownerEntity, blueprints = []) {
    ...
    const maximumWingRadius = Math.max(getWingMaxFormationRadius(blueprints, "left"), getWingMaxFormationRadius(blueprints, "right"), SUPERCAP_ORBIT_DISTANCE_METERS);
    const wingOffsetMeters = Math.max(BASE_WING_OFFSET_METERS, maximumWingRadius + WING_SEPARATION_MARGIN_METERS);
    const leftCenter = addVectors(center, scaleVector(basis.right, -wingOffsetMeters));
    const rightCenter = addVectors(center, scaleVector(basis.right, wingOffsetMeters));
    const facingCenter = (position) => normalizeVector(subtractVectors(center, position), scaleVector(basis.right, -1));
    const slots = {};
    for (const blueprint of blueprints) {
        if (!blueprint || !blueprint.key) {
            continue;
        }
        const wingCenter = blueprint.wing === "right"
            ? rightCenter
        ...
        if (blueprint.role === "anchor" || blueprint.role === "super") {
            position = buildCapitalSlotPosition(wingCenter, basis, blueprint.wing, blueprint);
        }
...
function buildRepairModulePlansForEntry(blueprint, entity, wingState) {
    ...
    const reserveCapitalKeys = peerCapitalKeys.filter((key) => (key !== primaryAnchorKey &&
        key !== primarySuperKey));
    ...
    const previousLogiKey = logiIndex > 0
        ? allLogiKeys[logiIndex - 1] || null
        : null;
    const nextLogiKey = logiIndex >= 0
        ? allLogiKeys[logiIndex + 1] || null
        : null;
    ...
    if (blueprint.role === "logi") {
        if (repairModules[0]) {
            modulePlans.push({
                ...
                candidates: buildCandidateList(buddyKey, nextLogiKey, previousLogiKey, primaryAnchorKey, primarySuperKey, reserveCapitalKeys, "player"),
            });
        }
        if (repairModules[1]) {
            modulePlans.push({
                ...
                candidates: buildCandidateList(primaryAnchorKey, previousLogiKey, buddyKey, nextLogiKey, primarySuperKey, reserveCapitalKeys, "player"),
            });
        }
        if (repairModules[2]) {
            modulePlans.push({
                ...
                candidates: buildCandidateList("player", primarySuperKey, primaryAnchorKey, reserveCapitalKeys, buddyKey, nextLogiKey, previousLogiKey),
            });
        }
        if (capModules[0]) {
            modulePlans.push({
                ...
                candidates: buildCandidateList(buddyKey, nextLogiKey, previousLogiKey, primaryAnchorKey, primarySuperKey, reserveCapitalKeys, "player", logiKeys[0]),
            });
        }
        if (capModules[1]) {
            modulePlans.push({
                ...
                candidates: buildCandidateList("player", primaryAnchorKey, primarySuperKey, reserveCapitalKeys, buddyKey, nextLogiKey, previousLogiKey, logiKeys[0]),
            });
        }
    }
    if (blueprint.role === "anchor") {
        if (repairModules[0]) {
            modulePlans.push({
                moduleID: repairModules[0].itemID,
                family: repairModules[0].rrFamily,
                candidates: buildCandidateList(peerCapitalKeys, logiKeys[0], "player", logiKeys[1], logiKeys[2]),
            });
        }
        if (repairModules[1]) {
            modulePlans.push({
                moduleID: repairModules[1].itemID,
                family: repairModules[1].rrFamily,
                candidates: buildCandidateList("player", peerCapitalKeys, logiKeys[1], logiKeys[0], logiKeys[2]),
            });
        }
        if (hullModules[0]) {
            modulePlans.push({
                ...
                candidates: buildCandidateList("player", peerCapitalKeys, logiKeys[0], logiKeys[1], logiKeys[2]),
            });
        }
        if (capModules[0]) {
            modulePlans.push({
    ...
    if (blueprint.role === "super") {
        if (repairModules[0]) {
            modulePlans.push({
                moduleID: repairModules[0].itemID,
                family: repairModules[0].rrFamily,
                candidates: buildCandidateList(primaryAnchorKey, peerCapitalKeys, "player", logiKeys[0], logiKeys[1], logiKeys[2]),
            });
        }
        if (repairModules[1]) {
            modulePlans.push({
                moduleID: repairModules[1].itemID,
                family: repairModules[1].rrFamily,
                candidates: buildCandidateList("player", primaryAnchorKey, peerCapitalKeys, logiKeys[0], logiKeys[1], logiKeys[2]),
            });
        }
        if (hullModules[0]) {
            modulePlans.push({
                moduleID: hullModules[0].itemID,
                family: hullModules[0].rrFamily,
                candidates: buildCandidateList("player", primaryAnchorKey, peerCapitalKeys, logiKeys[0], logiKeys[1]),
            });
        }
        if (capModules[0]) {
            modulePlans.push({
        ...
        if (capModules[1]) {
            modulePlans.push({
                moduleID: capModules[1].itemID,
                family: capModules[1].rrFamily,
                candidates: buildCandidateList(primaryAnchorKey, peerCapitalKeys, "player", logiKeys[1], logiKeys[0]),
            });
        }
    }
    return modulePlans;
...
function buildControllerEntries(spawnedEntries, ownerEntity = null) {
    ...
    return spawnedEntries.map((entry) => {
        ...
        const previousLogiKey = currentLogiIndex > 0
            ? wing.logiKeys[currentLogiIndex - 1] || null
            : null;
        const commandEntryPhaseIndex = entry.blueprint.role === "command"
            ? commandPhaseIndex++
            : -1;
        const wingMovementPhaseOffsetMs = entry.blueprint.wing === "right"
            ...
            : entry.blueprint.wing === "center"
                ...
                : 0;
        const roleMovementPhaseOffsetMs = entry.blueprint.role === "command"
        ...
        const isPrimaryAnchor = entry.blueprint.role === "anchor" &&
            String(entry.blueprint.key || "") === String(wing.primaryAnchorKey || "");
        const commandShipModulePlans = buildRepairModulePlansForEntry(entry.blueprint, entry.entity, wingState).map((plan, modulePlanIndex) => (plan && plan.targetless === true
            ? {
                ...plan,
                notBeforeOffsetMs: (commandEntryPhaseIndex >= 0 ? commandEntryPhaseIndex : 0) * REMOTE_REPAIR_BURST_STAGGER_STEP_MS +
                    (modulePlanIndex * REMOTE_REPAIR_BURST_MODULE_STAGGER_STEP_MS),
            }
            : plan));
        const orbitTargetCandidates = orbitStrategy === "wingAnchor"
        ...
        return {
            ...
            orbitPulseAmplitudeMeters: entry.blueprint.role === "command"
                ? 900
                : entry.blueprint.role === "super"
                    ? 800
                    : entry.blueprint.role === "anchor"
                        ? 650
                        : 450,
            orbitPulsePeriodMs: entry.blueprint.role === "command"
                ? REMOTE_REPAIR_COMMAND_ORBIT_PULSE_PERIOD_MS
                : REMOTE_REPAIR_ORBIT_PULSE_PERIOD_MS,
            orbitRetuneThresholdMeters: entry.blueprint.role === "command"
                ? 350
                : 225,
            orbitRetuneIntervalMs: entry.blueprint.role === "command"
                ? 1_500
                : 2_000,
            anchorShellDistanceMeters: isPrimaryAnchor
                ? distanceBetweenVectors(entry.entity.position, ownerEntity && ownerEntity.position)
                : 0,
...
function buildRemoteRepairFighterLaunchSpaceState(parentEntity, launchIndex = 0) {
    ...
    const launchDistance = Math.max(toFiniteNumber(parentEntity && parentEntity.radius, 0), 1) +
    ...
    const position = addVectors(addVectors(cloneVector(parentEntity && parentEntity.position), scaleVector(basis.forward, launchDistance)), addVectors(scaleVector(basis.right, lateralOffset * signedSide), scaleVector(basis.up, 75 * verticalSign)));
    return {
...
function resolveFighterTubeCountForEntity(entity) {
    const configuredTubes = Math.max(0, toInt(getTypeAttributeValue(toInt(entity && entity.typeID, 0), "fighterTubes"), 0));
...
function resolveRemoteRepairFighterLoadoutNames(entity, tubeCount) {
    ...
    const fallbackName = hullName === "Wyvern" || hullName === "Chimera"
        ? "Dragonfly II"
        : "Templar II";
...
function applyCoverFormationToController(scene, controller, ownerEntity, nowMs) {
    ...
    for (const entry of Array.isArray(controller.entries) ? controller.entries : []) {
        ...
        const wingSign = String(blueprint && blueprint.wing || "") === "right"
            ? 1
...
function deployRemoteRepairShowFighters(scene, controller, session, ownerEntity, nowMs) {
    ...
    for (const entry of Array.isArray(controller.entries) ? controller.entries : []) {
        ...
        const roleBaseOrbitProfile = entry.role === "super"
            ? {
        ...
        for (let launchIndex = 0; launchIndex < tubeCount; launchIndex += 1) {
            ...
            const createResult = grantItemToCharacterLocation(session.characterID, scene.systemID, 0, fighterType, 1, {
                singleton: 1,
                transient: true,
            ...
            fighterEntries.push({
                ...
                wing: entry.wing,
                tubeFlagID,
                orbitDistance: orbitProfile.orbitDistance +
                    roleBaseOrbitProfile.orbitDistanceBonus +
                    (launchIndex * 175),
                coverOrbitDistance: orbitProfile.coverOrbitDistance +
                    roleBaseOrbitProfile.coverOrbitDistanceBonus +
                    (launchIndex * 250),
                movementPhaseOffsetMs: toFiniteNumber(entry.movementPhaseOffsetMs, 0) +
...
function persistSyntheticNativeCargo(entity) {
    ...
    for (const cargoItem of Array.isArray(entity && entity.nativeCargoItems) ? entity.nativeCargoItems : []) {
        const writeResult = nativeNpcStore.upsertNativeCargo({
            ...
            singleton: false,
            moduleState: cargoItem && cargoItem.moduleState && typeof cargoItem.moduleState === "object"
...
function handleRemoteRepairFleetCommand(session, argumentText, options = {}) {
    ...
    for (const blueprint of selectedBlueprints) {
        ...
        const shipSpec = buildSyntheticRepairShipSpec(session, blueprint, cloneVector(slot && slot.position), cloneVector(slot && slot.direction, ownerEntity.direction), entityID, skillMap);
        if (!shipSpec) {
        ...
        Object.assign(shipSpec, buildChildEntityScopeMetadata(ownerEntity));
        const spawnResult = spaceRuntime.spawnDynamicShip(session._space.systemID, shipSpec);
        if (!spawnResult.success || !spawnResult.data || !spawnResult.data.entity) {
            clearSpawnedEntries(scene, spawnedEntries, nowMs);
            restoreOwnerBurstAffinity(ownerEntity, previousOwnerBurstAffinityGroupID);
            return {
    ...
    tickRemoteRepairShowScene(scene, nowMs);
    const armorCount = spawnedEntries.filter((entry) => (entry.blueprint.wing === "left" &&
        entry.blueprint.role !== "command")).length;
    const shieldCount = spawnedEntries.filter((entry) => (entry.blueprint.wing === "right" &&
        entry.blueprint.role !== "command")).length;
    const commandCount = spawnedEntries.filter((entry) => entry.blueprint.role === "command").length;
    const requestedSuffix = requestedCount.requestedCount !== requestedCount.count
        ? ` Requested ${requestedCount.requestedCount}, clamped to ${requestedCount.count}.`
        : "";
    const replacedSuffix = clearedResult && clearedResult.removedCount > 0
        ? ` Replaced ${clearedResult.removedCount} older support hulls in this system.`
        : "";
    const orphanedSuffix = orphanedResult && orphanedResult.removedCount > 0
...
//# sourceMappingURL=remoteRepairFleetCommands.js.map