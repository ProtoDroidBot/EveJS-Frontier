...
function getSurfaceDistance(scene, entity, targetEntity) {
    if (scene &&
        ...
        return Math.max(0, toFiniteNumber(scene.getEntitySurfaceDistance(entity, targetEntity), 0));
    }
...
function resolveControllerTargetEntity(scene, controller, entry, candidate) {
    ...
    const targetEntry = Array.isArray(controller.entries)
        ? controller.entries.find((candidateEntry) => (String(candidateEntry && candidateEntry.key || "") === normalizedCandidate))
        : null;
    if (!targetEntry) {
        return null;
    }
...
function resolveFirstTargetEntity(scene, controller, entry, candidates = []) {
    for (const candidate of uniqueCandidates(candidates)) {
        const targetEntity = resolveControllerTargetEntity(scene, controller, entry, candidate);
        if (targetEntity) {
            return targetEntity;
        }
    }
...
function ensureTargetLock(scene, sourceEntity, targetEntity, nowMs) {
    ...
    if (typeof scene.isEntityLockedTarget === "function" &&
...
function restoreOwnerBurstAffinity(scene, controller) {
    ...
    const previousAffinity = toInt(controller.previousOwnerBurstAffinityGroupID, 0);
...
function resolveOrbitDistanceForEntry(entry, nowMs) {
    const baseOrbitDistance = Math.max(0, toFiniteNumber(entry && entry.baseOrbitDistance, entry && entry.orbitDistance));
    const pulseAmplitudeMeters = Math.max(0, toFiniteNumber(entry && entry.orbitPulseAmplitudeMeters, 0));
    const pulsePeriodMs = Math.max(1, toInt(entry && entry.orbitPulsePeriodMs, 18_000));
    ...
    const normalizedPhase = (((toFiniteNumber(nowMs, 0) + phaseOffsetMs) % pulsePeriodMs) / pulsePeriodMs) *
        Math.PI *
        2;
    return Math.max(0, baseOrbitDistance + (Math.sin(normalizedPhase) * pulseAmplitudeMeters));
}
function resolveAnchorDriftTarget(scene, controller, entry, nowMs) {
    ...
    const shellDistance = Math.max(12_000, toFiniteNumber(entry.anchorShellDistanceMeters, 14_000));
    ...
    const normalizedPhase = (((toFiniteNumber(nowMs, 0) + phaseOffsetMs) % driftPeriodMs) / driftPeriodMs) *
        Math.PI *
        2;
    const forwardAmplitudeMeters = Math.max(0, toFiniteNumber(entry.anchorDriftForwardAmplitudeMeters, 3_500));
    const verticalAmplitudeMeters = Math.max(0, toFiniteNumber(entry.anchorDriftVerticalAmplitudeMeters, 1_250));
    return addVectors(cloneVector(ownerEntity.position), addVectors(scaleVector(basis.right, wingSign * shellDistance), addVectors(scaleVector(basis.forward, Math.sin(normalizedPhase) * forwardAmplitudeMeters), scaleVector(basis.up, Math.cos(normalizedPhase * 0.75) * verticalAmplitudeMeters))));
}
function syncPrimaryAnchorDrift(scene, controller, entry, entity, nowMs, pseudoSession) {
    ...
    const driftRefreshIntervalMs = Math.max(1_750, Math.trunc(Math.max(1, toInt(entry.anchorDriftPeriodMs, 20_000)) / 7));
    const shouldRefreshDriftCommand = entity.mode !== "GOTO" ||
...
function resolveCoverSlotTarget(scene, controller, entry, nowMs) {
    ...
    const normalizedPhase = (((toFiniteNumber(nowMs, 0) + phaseOffsetMs) % driftPeriodMs) / driftPeriodMs) *
        Math.PI *
        2;
    const forwardMeters = toFiniteNumber(entry.coverOffsetForwardMeters, 0) +
        (Math.sin(normalizedPhase * 0.85) * toFiniteNumber(entry.coverDriftForwardAmplitudeMeters, 0));
    const lateralMeters = toFiniteNumber(entry.coverOffsetLateralMeters, 0) +
        (Math.sin(normalizedPhase) * toFiniteNumber(entry.coverDriftLateralAmplitudeMeters, 0));
    const verticalMeters = toFiniteNumber(entry.coverOffsetVerticalMeters, 0) +
        (Math.cos(normalizedPhase * 1.15) * toFiniteNumber(entry.coverDriftVerticalAmplitudeMeters, 0));
    return addVectors(cloneVector(ownerEntity.position), addVectors(scaleVector(basis.forward, forwardMeters), addVectors(scaleVector(basis.right, lateralMeters), scaleVector(basis.up, verticalMeters))));
}
function syncCoverSlotMovement(scene, controller, entry, entity, nowMs, pseudoSession) {
    ...
    const holdRadiusMeters = Math.max(300, toFiniteNumber(entry.coverHoldRadiusMeters, 900));
    const refreshIntervalMs = Math.max(750, toInt(entry.coverRefreshIntervalMs, 1_250));
    const refreshJitterMs = Math.max(0, toInt(entry.coverRefreshJitterMs, 0));
    const minimumRetargetShiftMeters = Math.max(200, toFiniteNumber(entry.coverRetargetThresholdMeters, holdRadiusMeters * 0.4));
    const nextEligibleRefreshAtMs = toFiniteNumber(entry.lastCoverCommandAtMs, 0) +
        refreshIntervalMs +
        refreshJitterMs;
...
function syncFighterEntryMovement(scene, controller, fighterEntry, nowMs) {
    ...
    const desiredOrbitDistance = Math.max(500, toFiniteNumber(controller.formationMode === "cover"
        ? fighterEntry.coverOrbitDistance
        : fighterEntry.orbitDistance, fighterEntry.orbitDistance));
    ...
    const allowRetune = !fighterEntry.lastOrbitRetuneAtMs ||
...
function syncEntryMovement(scene, controller, entry, nowMs) {
    ...
    const targetEntity = resolveFirstTargetEntity(scene, controller, entry, entry.orbitTargetCandidates);
    ...
    const orbitReacquireDistanceMeters = orbitDistance + Math.max(5_000, orbitDistance * 0.5);
    const orbitSettleDistanceMeters = orbitDistance + Math.max(1_000, orbitDistance * 0.2);
    const orbitRetuneThresholdMeters = Math.max(50, toFiniteNumber(entry.orbitRetuneThresholdMeters, 225));
    const orbitRetuneIntervalMs = Math.max(500, toInt(entry.orbitRetuneIntervalMs, 2_000));
    const allowOrbitRetune = !entry.lastOrbitRetuneAtMs ||
    ...
    if (surfaceDistance > orbitReacquireDistanceMeters ||
...
function syncEntryModules(scene, controller, entry, nowMs) {
    ...
    for (const modulePlan of Array.isArray(entry.modulePlans) ? entry.modulePlans : []) {
        ...
        const moduleItem = entity.fittedItems.find((item) => (toInt(item && item.itemID, 0) === moduleID)) || null;
        if (!moduleItem) {
            continue;
        }
        const desiredTarget = resolveFirstTargetEntity(scene, controller, entry, modulePlan.candidates);
        ...
        scene.activateGenericModule(pseudoSession, moduleItem, modulePlan.effectName || null, targetless
            ? {}
            : {
...
function clearRemoteRepairShowController(scene, options = {}) {
    ...
    const nowMs = toFiniteNumber(options.nowMs, scene.getCurrentSimTimeMs && scene.getCurrentSimTimeMs());
    let removedCount = 0;
            ...
            ? options.entries.map((entry) => ({
                ...
                modulePlans: Array.isArray(entry && entry.modulePlans)
                    ? entry.modulePlans.map((plan) => ({
                        ...
                        notBeforeAtMs: toFiniteNumber(nowMs, 0) +
...
//# sourceMappingURL=remoteRepairShowRuntime.js.map