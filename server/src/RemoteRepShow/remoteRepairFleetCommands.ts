...
function uniqueCandidates(values: any[] = []) {
  const seen = new Set();
  const resolved: any[] = [];
...
function resolveRequestedFleetCount(argumentText): FleetCommandResult {
...
function resolveRequestedFleetCommand(argumentText): FleetCommandResult {
  const trimmed = String(argumentText || "").trim();
  if (!trimmed) {
    return {
      success: true,
...
function buildFormationSlots(ownerEntity, blueprints: any[] = []) {
...
function resolveBuddyKey(logiKeys: any[] = [], currentKey) {
...
function buildWingEntryState(spawnedEntries: any[] = []) {
  const wingState: Record<string, any> = {
    left: {
      anchorKeys: [],
      superKeys: [],
...
function deployRemoteRepairShowFighters(scene, controller, session, ownerEntity, nowMs) {
  ...
  const fighterEntries: any[] = [];
  const createdFighterItemIDs: any[] = [];
  const launchedByRole: Record<string, any> = {
    super: 0,
    anchor: 0,
  };
...
function handleRemoteRepairFleetCommand(session, argumentText, options: Record<string, any> = {}) {
