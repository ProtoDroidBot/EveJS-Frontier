...
const { getFittedModuleItems, getLoadedChargeItems, typeHasEffectName, } = require(path.join(__dirname, "../../services/fitting/liveFittingState"));
const { resolveWeaponFamily, } = require(path.join(__dirname, "../combat/weaponDogma"));
...
function resolveModuleParityFamily(moduleItem, chargeItem = null) {
    ...
    if (moduleGroupID === GROUP_SCAN_PROBE_LAUNCHER) {
        ...
        else if (weaponFamily === "laserTurret" ||
...
function getSpaceAttachModuleParityPolicy(profileID = "transition") {
    return (SPACE_ATTACH_MODULE_PARITY_POLICIES[String(profileID || "transition")] ||
...
function buildShipModuleParityManifest(characterID, shipID, options = {}) {
    ...
    const chargeByFlag = new Map(loadedCharges.map((chargeItem) => [toInt(chargeItem && chargeItem.flagID, 0), chargeItem]));
    const familyCounts = new Map();
    const familyIDsByModuleID = {};
    let requiresOnlineEffectBootstrap = false;
    for (const moduleItem of fittedModules) {
        const family = resolveModuleParityFamily(moduleItem, chargeByFlag.get(toInt(moduleItem && moduleItem.flagID, 0)) || null);
        familyIDsByModuleID[String(toInt(moduleItem && moduleItem.itemID, 0))] =
            family.familyID;
        familyCounts.set(family.familyID, (familyCounts.get(family.familyID) || 0) + 1);
...
//# sourceMappingURL=moduleClientParityAuthority.js.map