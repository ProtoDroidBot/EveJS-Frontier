...
function resolveExplicitChanceAttribute(attributeMap, attributeIDs: any[] = []) {
...
function resolveFirstAttributeValue(attributeMap, attributeIDs: any[] = [], fallback = 0) {
  for (const attributeID of attributeIDs) {
    const numericAttributeID = toInt(attributeID, 0);
...
function resolveLocalCycleState(options: Record<string, any> = {}) {
  const {
    entity,
    shipItem,
    moduleItem,
...
function consumeLoadedChargeQuantity(entity, moduleItem, chargeItem, quantity, options: Record<string, any> = {}) {
...
function scheduleLocalReload(options: Record<string, any> = {}) {
  const {
    entity,
    scene,
    session,
...
function rollRandom(callbacks: Record<string, any> = {}) {
  const rawValue =
    callbacks &&
...
function shouldDelayNpcLocalCycle(cycleState, callbacks: Record<string, any> = {}) {
...
function queueAncillaryLocalReloadOnManualDeactivate(options: Record<string, any> = {}) {
  const {
    entity,
    scene,
    session,
...
function prepareLocalCycleActivation(options: Record<string, any> = {}) {
...
function prepareLocalCycleBoundary(options: Record<string, any> = {}) {
  const {
    entity,
    moduleItem,
    effectState,
...
function executeLocalCycle(options: Record<string, any> = {}) {
  const {
    scene,
    session,
    entity,
