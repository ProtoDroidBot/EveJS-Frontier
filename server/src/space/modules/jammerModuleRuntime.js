...
const { applyActiveImplantLocationModifiersToAttributes, getActiveImplantLocationModifierSources, getActiveImplantShipModifierEntries, } = require(path.join(__dirname, "../../services/dogma/implants/activeImplantModifiers"));
...
function resolveEffectiveEcmBurstRange(moduleAttributes, moduleItem, characterID) {
    const baseBurstRange = Math.max(0, roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_ECM_BURST_RANGE], 0), 3));
    ...
    }, moduleItem, characterID);
    return Math.max(0, roundNumber(toFiniteNumber(maxRangeAlias[ATTRIBUTE_MAX_RANGE], baseBurstRange), 3));
}
...
function resolveTargetSensorProfile(targetEntity) {
    ...
    for (const sensorProfile of SENSOR_PROFILES) {
        const sensorStrength = Math.max(0, roundNumber(getEntityPassiveAttributeValue(targetEntity, sensorProfile.sensorStrengthAttributeID, 0), 6));
        if (sensorStrength <= 0) {
            continue;
        }
...
function resolveTargetEcmResistanceMultiplier(targetEntity) {
    const resistancePercent = clamp(getEntityPassiveAttributeValue(targetEntity, ATTRIBUTE_ECM_RESISTANCE, 0), 0, 100);
...
function resolveJammerRangeMultiplier(effectState, sourceEntity, targetEntity, callbacks = {}) {
    ...
    const optimalRangeMeters = Math.max(0, toFiniteNumber(effectState && effectState.jammerMaxRangeMeters, 0));
    const falloffMeters = Math.max(0, toFiniteNumber(effectState && effectState.jammerFalloffMeters, 0));
...
function resolveJammerChance(effectState, targetEntity, rangeMultiplier) {
    ...
    const strengthBySensorType = effectState &&
        effectState.jammerStrengthBySensorType &&
        typeof effectState.jammerStrengthBySensorType === "object"
        ? effectState.jammerStrengthBySensorType
        : {};
    const jammerStrength = Math.max(0, toFiniteNumber(strengthBySensorType[sensorProfile.sensorType], 0));
    if (jammerStrength <= 0) {
        return {
            sensorProfile,
            jammerStrength: 0,
    ...
    const jamChance = clamp((jammerStrength / Math.max(sensorProfile.sensorStrength, 1e-6)) *
        Math.max(rangeMultiplier, 0) *
        Math.max(resistanceMultiplier, 0), 0, 1);
    return {
        sensorProfile,
...
function buildJamRecord(sourceEntity, effectState, targetEntity, nowMs, sensorProfile, jammerStrength, jamChance) {
    return Object.freeze({
        key: buildJammedStateKey(sourceEntity && sourceEntity.itemID, effectState && effectState.moduleID, targetEntity && targetEntity.itemID),
        ...
        expiresAtMs: Math.max(0, toFiniteNumber(nowMs, Date.now()) +
            Math.max(1, toFiniteNumber(effectState && (effectState.jamDurationMs ??
                effectState.durationMs), 1000))),
        jammingType: ECM_JAMMING_TYPE,
...
function getExistingJamRecord(targetEntity, sourceEntity, effectState) {
    ...
    return jammedState.activeJams.get(buildJammedStateKey(sourceEntity && sourceEntity.itemID, effectState && effectState.moduleID, targetEntity && targetEntity.itemID)) || null;
}
...
function removeJamRecord(targetEntity, sourceEntity, effectState) {
    ...
    const key = buildJammedStateKey(sourceEntity && sourceEntity.itemID, effectState && effectState.moduleID, targetEntity && targetEntity.itemID);
...
function resolveJammerModuleActivation({ scene, entity, moduleItem, effectRecord, chargeItem = null, shipItem, skillMap = null, fittedItems = null, activeModuleContexts = null, options = {}, callbacks = {}, } = {}) {
    ...
    const rawDurationMs = toFiniteNumber(moduleAttributes[ATTRIBUTE_DURATION], moduleAttributes[ATTRIBUTE_SPEED]);
    const durationAttributeID = toFiniteNumber(moduleAttributes[ATTRIBUTE_DURATION], 0) > 0
        ? ATTRIBUTE_DURATION
        : ATTRIBUTE_SPEED;
    const jammerStrengthBySensorType = Object.freeze({
        gravimetric: roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_SCAN_GRAVIMETRIC_STRENGTH_BONUS], 0), 6),
        ladar: roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_SCAN_LADAR_STRENGTH_BONUS], 0), 6),
        magnetometric: roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_SCAN_MAGNETOMETRIC_STRENGTH_BONUS], 0), 6),
        radar: roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_SCAN_RADAR_STRENGTH_BONUS], 0), 6),
    });
    const jammerMaxStrength = Math.max(0, ...Object.values(jammerStrengthBySensorType).map((value) => toFiniteNumber(value, 0)));
    ...
    const effectiveBurstRangeMeters = definition.burst === true
    ...
    const effectStatePatch = {
        ...
        jammerMaxRangeMeters: Math.max(0, roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_MAX_RANGE], 0), 3)),
        jammerFalloffMeters: Math.max(0, roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_FALLOFF_EFFECTIVENESS], 0), 3)),
        jammerBurstRadiusMeters: Math.max(0, roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_ECM_BURST_RANGE], 0), 3)),
    ...
    const rangeResult = resolveJammerRangeMultiplier(effectStatePatch, entity, targetEntity, callbacks);
...
function listJammerBurstTargets(scene, sourceEntity, effectState, callbacks = {}) {
    ...
    const radiusMeters = Math.max(0, toFiniteNumber(effectState && effectState.jammerBurstRadiusMeters, 0));
    if (radiusMeters <= 0) {
        return [];
    }
    const getSurfaceDistance = callbacks.getEntitySurfaceDistance && typeof callbacks.getEntitySurfaceDistance === "function"
    ...
    for (const targetEntity of scene.dynamicEntities.values()) {
        ...
        const surfaceDistance = Math.max(0, toFiniteNumber(getSurfaceDistance
...
function removeJammerModuleState({ targetEntity, sourceEntity, effectState, nowMs = Date.now(), } = {}) {
...
function applyJammerModuleState({ targetEntity, sourceEntity, effectState, nowMs = Date.now(), callbacks = {}, rangeMultiplier = 1, } = {}) {
    if (!targetEntity || !sourceEntity || !effectState) {
        return {
            success: false,
            errorMsg: "UNSUPPORTED_MODULE",
    ...
    if (callbacks.recordOffensiveAggression &&
    ...
    const randomRoll = clamp(callbacks.random && typeof callbacks.random === "function"
        ? callbacks.random()
        : Math.random(), 0, 1);
    const jamApplied = chanceResult.jamChance > 0 && randomRoll < chanceResult.jamChance;
    if (jamApplied) {
        upsertJamRecord(targetEntity, buildJamRecord(sourceEntity, effectState, targetEntity, nowMs, chanceResult.sensorProfile, chanceResult.jammerStrength, chanceResult.jamChance));
    }
    const recomputeResult = recomputeEntityJammedState(targetEntity, nowMs);
    if (jamApplied &&
        callbacks.clearOutgoingTargetLocksExcept &&
        typeof callbacks.clearOutgoingTargetLocksExcept === "function") {
        callbacks.clearOutgoingTargetLocksExcept(targetEntity, recomputeResult.allowedTargetIDs, {
            notifySelf: true,
            notifyTarget: true,
            activeReason: "target",
...
function executeJammerModuleCycle({ scene, entity, effectState, nowMs, callbacks = {}, } = {}) {
    ...
    const rangeResult = resolveJammerRangeMultiplier(effectState, entity, targetEntity, callbacks);
    if (!rangeResult.success) {
        recomputeEntityJammedState(targetEntity, nowMs);
        return rangeResult;
    }
    const chanceResult = resolveJammerChance(effectState, targetEntity, rangeResult.multiplier);
    const randomRoll = clamp(callbacks.random && typeof callbacks.random === "function"
        ? callbacks.random()
        : Math.random(), 0, 1);
    const jamApplied = chanceResult.jamChance > 0 && randomRoll < chanceResult.jamChance;
    if (jamApplied) {
        upsertJamRecord(targetEntity, buildJamRecord(entity, effectState, targetEntity, nowMs, chanceResult.sensorProfile, chanceResult.jammerStrength, chanceResult.jamChance));
    }
    const recomputeResult = recomputeEntityJammedState(targetEntity, nowMs);
    if (jamApplied &&
        callbacks.clearOutgoingTargetLocksExcept &&
...
function executeJammerBurstCycle({ scene, entity, effectState, nowMs, callbacks = {}, } = {}) {
    if (!scene || !entity || !effectState) {
        return {
            success: false,
            stopReason: "module",
    ...
    for (const targetEntity of listJammerBurstTargets(scene, entity, effectState, callbacks)) {
        ...
        if (jamApplied &&
            callbacks.clearOutgoingTargetLocksExcept &&
            typeof callbacks.clearOutgoingTargetLocksExcept === "function") {
            lockClearResult = callbacks.clearOutgoingTargetLocksExcept(targetEntity, new Set(), {
                notifySelf: true,
                notifyTarget: true,
                activeReason: "target",
                pendingReason: "target",
        ...
        affectedTargets.push({
            ...
            sensorType: chanceResult.sensorProfile
                ? String(chanceResult.sensorProfile.sensorType || "")
                : "",
            clearedTargetIDs: lockClearResult && Array.isArray(lockClearResult.clearedTargetIDs)
                ? [...lockClearResult.clearedTargetIDs]
                : [],
            cancelledPendingIDs: lockClearResult && Array.isArray(lockClearResult.cancelledPendingIDs)
...
//# sourceMappingURL=jammerModuleRuntime.js.map