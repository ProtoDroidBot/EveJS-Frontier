...
const { getAttributeIDByNames, getTypeDogmaAttributes, isChargeCompatibleWithModule, } = require(path.join(__dirname, "../../services/fitting/liveFittingState"));
const { buildLiveModuleAttributeMap, } = require(path.join(__dirname, "./liveModuleAttributes"));
const { isHighSecuritySystem, } = require(path.join(__dirname, "../../services/security/crimewatchState"));
...
function buildPointDefenseDamageVector(chargeAttributes, damageMultiplier) {
    ...
    return {
        ...
        thermal: roundNumber(toFiniteNumber(chargeAttributes[ATTRIBUTE_THERMAL_DAMAGE], 0) * multiplier),
        kinetic: roundNumber(toFiniteNumber(chargeAttributes[ATTRIBUTE_KINETIC_DAMAGE], 0) * multiplier),
        explosive: roundNumber(toFiniteNumber(chargeAttributes[ATTRIBUTE_EXPLOSIVE_DAMAGE], 0) * multiplier),
    };
}
function sumDamageVector(vector) {
    return (toFiniteNumber(vector && vector.em, 0) +
        toFiniteNumber(vector && vector.thermal, 0) +
        toFiniteNumber(vector && vector.kinetic, 0) +
        toFiniteNumber(vector && vector.explosive, 0));
}
function resolvePointDefenseActivation({ scene, entity, moduleItem, effectRecord, chargeItem = null, shipItem, skillMap = null, fittedItems = null, activeModuleContexts = null, } = {}) {
    ...
    const chargeAttributes = buildLiveModuleAttributeMap(shipItem, chargeItem, null, skillMap, fittedItems, activeModuleContexts) || getTypeDogmaAttributes(chargeItem.typeID);
    ...
    const chargesPerCycle = Math.max(1, toInt(moduleAttributes[ATTRIBUTE_CHARGE_RATE], 1));
    const availableCharges = Math.max(0, toInt(chargeItem && (chargeItem.stacksize ?? chargeItem.quantity), 0));
    ...
    const rawDamage = buildPointDefenseDamageVector(chargeAttributes, toFiniteNumber(moduleAttributes[ATTRIBUTE_DAMAGE_MULTIPLIER], 1) * chargesPerCycle);
    ...
    return {
        ...
        data: {
            runtimeAttrs: {
                ...
                durationAttributeID: durationResolution.attributeID > 0
...
function executePointDefenseCycle({ scene, entity, moduleItem, effectState, nowMs, callbacks = {}, } = {}) {
    ...
    const consumeResult = callbacks.consumeLoadedCharge &&
    ...
    const damageVector = effectState.pointDefenseDamage && typeof effectState.pointDefenseDamage === "object"
        ? effectState.pointDefenseDamage
        : {};
    const candidates = callbacks.listDamageableEntitiesInRange &&
        callbacks.listDamageableEntitiesInRange(entity, rangeMeters);
    const targets = (Array.isArray(candidates) ? candidates : [])
        .filter((targetEntity) => canEntitiesInteractLocally(entity, targetEntity));
    const damageResults = [];
    ...
    for (const targetEntity of targets) {
        ...
        if (callbacks.applyPointDefenseDamage) {
            const damageResult = callbacks.applyPointDefenseDamage(entity, targetEntity, moduleItem, damageVector, nowMs);
            damageResults.push({
    ...
    return {
        ...
        data: {
            targets,
            damageResults,
            depleted: consumeResult.data &&
                consumeResult.data.depleted === true,
            stopReason: consumeResult.data &&
                consumeResult.data.depleted === true
                ? "ammo"
                : null,
        },
...
//# sourceMappingURL=pointDefenseRuntime.js.map