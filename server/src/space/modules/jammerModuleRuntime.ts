...
function resolveJammerRangeMultiplier(effectState, sourceEntity, targetEntity, callbacks: Record<string, any> = {}) {
...
function buildAggregateSignature(activeRecords, allowedTargetIDs, jammedUntilMs) {
  return JSON.stringify({
    ...
    allowedTargetIDs: [...(allowedTargetIDs instanceof Set ? allowedTargetIDs : new Set<any>())]
...
function recomputeEntityJammedState(targetEntity, nowMs = Date.now()) {
  ...
  const allowedTargetIDs = new Set<any>();
  let jammedUntilMs = 0;
...
function getActiveJammerSourceIDs(targetEntity, nowMs = Date.now()) {
  return new Set<any>(recomputeEntityJammedState(targetEntity, nowMs).allowedTargetIDs);
}
...
function listJammerBurstTargets(scene, sourceEntity, effectState, callbacks: Record<string, any> = {}) {
