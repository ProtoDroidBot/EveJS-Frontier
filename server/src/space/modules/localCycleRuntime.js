...
const { getAttributeIDByNames, buildEffectiveItemAttributeMap, } = require(path.join(__dirname, "../../services/fitting/liveFittingState"));
const { getCachedCharacterSkillMap, } = require(path.join(__dirname, "../../services/skills/skillState"));
const { updateInventoryItem, removeInventoryItem, normalizeShipConditionState, } = require(path.join(__dirname, "../../services/inventory/itemStore"));
const { getEntityMaxHealthLayers, } = require(path.join(__dirname, "../combat/damage"));
const { isNativeNpcEntity, } = require(path.join(__dirname, "../npc/npcEquipment"));
const nativeNpcStore = require(path.join(__dirname, "../npc/nativeNpcStore"));
const { queueAutomaticLocalModuleReload, resolvePendingLocalModuleReload, } = require("./localCycleReloads");
const { buildLiveModuleAttributeMap, } = require("./liveModuleAttributes");
...
const ATTRIBUTE_ENTITY_ARMOR_REPAIR_DURATION = getAttributeIDByNames("entityArmorRepairDuration") || 630;
const ATTRIBUTE_ENTITY_ARMOR_REPAIR_AMOUNT = getAttributeIDByNames("entityArmorRepairAmount") || 631;
const ATTRIBUTE_ENTITY_ARMOR_REPAIR_AMOUNT_PER_SECOND = getAttributeIDByNames("entityArmorRepairAmountPerSecond") || 1892;
const ATTRIBUTE_ENTITY_SHIELD_BOOST_DURATION = getAttributeIDByNames("entityShieldBoostDuration") || 636;
const ATTRIBUTE_ENTITY_SHIELD_BOOST_AMOUNT = getAttributeIDByNames("entityShieldBoostAmount") || 637;
const ATTRIBUTE_ENTITY_SHIELD_BOOST_AMOUNT_PER_SECOND = getAttributeIDByNames("entityShieldBoostAmountPerSecond") || 1893;
const ATTRIBUTE_ENTITY_ARMOR_REPAIR_DELAY_CHANCE = getAttributeIDByNames("entityArmorRepairDelayChance") || 638;
const ATTRIBUTE_ENTITY_SHIELD_BOOST_DELAY_CHANCE = getAttributeIDByNames("entityShieldBoostDelayChance") || 639;
const ATTRIBUTE_ENTITY_SHIELD_BOOST_DELAY_CHANCE_SMALL = getAttributeIDByNames("entityShieldBoostDelayChanceSmall") || 1006;
const ATTRIBUTE_ENTITY_SHIELD_BOOST_DELAY_CHANCE_MEDIUM = getAttributeIDByNames("entityShieldBoostDelayChanceMedium") || 1007;
const ATTRIBUTE_ENTITY_SHIELD_BOOST_DELAY_CHANCE_LARGE = getAttributeIDByNames("entityShieldBoostDelayChanceLarge") || 1008;
const ATTRIBUTE_ENTITY_ARMOR_REPAIR_DELAY_CHANCE_SMALL = getAttributeIDByNames("entityArmorRepairDelayChanceSmall") || 1009;
const ATTRIBUTE_ENTITY_ARMOR_REPAIR_DELAY_CHANCE_MEDIUM = getAttributeIDByNames("entityArmorRepairDelayChanceMedium") || 1010;
const ATTRIBUTE_ENTITY_ARMOR_REPAIR_DELAY_CHANCE_LARGE = getAttributeIDByNames("entityArmorRepairDelayChanceLarge") || 1011;
const ATTRIBUTE_CHARGED_ARMOR_DAMAGE_MULTIPLIER = getAttributeIDByNames("chargedArmorDamageMultiplier") || 1886;
...
function getChargeItemQuantity(chargeItem) {
    return Math.max(0, toInt(chargeItem && (chargeItem.stacksize ?? chargeItem.quantity), 0));
}
function normalizeEffectName(effectRecordOrName) {
    if (!effectRecordOrName) {
        return "";
    }
    const rawName = typeof effectRecordOrName === "string"
        ? effectRecordOrName
...
function resolveNpcLocalCycleDelayChance(entity, moduleItem, attributeMap, effectRecordOrName, normalizedEffectName) {
    ...
    const rawEffectName = normalizeEffectName(effectRecordOrName ||
    ...
    if (normalizedEffectName === "armorRepair") {
        const sizedAttributeID = sizedSuffix === "large"
            ...
            : sizedSuffix === "medium"
                ...
                : sizedSuffix === "small"
                    ...
                    : 0;
        return resolveExplicitChanceAttribute(attributeMap, [
            sizedAttributeID,
...
function resolveLocalRepairAmount(attributeMap, directAttributeID, entityAmountAttributeID, entityAmountPerSecondAttributeID, durationMs) {
    ...
    const amountPerSecond = toFiniteNumber(attributeMap && attributeMap[entityAmountPerSecondAttributeID], 0);
...
function resolveLocalCycleState(options = {}) {
    const { entity, shipItem, moduleItem, effectRecord, effectName, effectState, chargeItem, callbacks = {}, fallbackChargeTypeID = 0, } = options;
    const normalizedEffectName = normalizeLocalCycleEffectName(effectRecord ||
        effectName ||
    ...
    const resolveCharacterID = callbacks.resolveCharacterID ||
    ...
    const durationResolution = resolveFirstAttributeValue(moduleAttributes, resolveDurationAttributeIDs(normalizedEffectName), 1000);
    ...
    const chargeTypeID = toInt((chargeItem && chargeItem.typeID) || fallbackChargeTypeID, 0);
    const resolvedState = {
    ...
    const delayChanceResolution = resolveNpcLocalCycleDelayChance(entity, moduleItem, moduleAttributes, effectRecord ||
        effectName ||
        (moduleItem && moduleItem.npcEffectName) ||
        (effectState && effectState.effectName), normalizedEffectName);
    ...
    if (normalizedEffectName === "armorRepair") {
        resolvedState.armorRepairAmount = Math.max(0, resolveLocalRepairAmount(moduleAttributes, ATTRIBUTE_ARMOR_DAMAGE_AMOUNT, ATTRIBUTE_ENTITY_ARMOR_REPAIR_AMOUNT, ATTRIBUTE_ENTITY_ARMOR_REPAIR_AMOUNT_PER_SECOND, durationMs));
        return resolvedState;
    }
    if (normalizedEffectName === "shieldBoosting") {
        resolvedState.shieldBoostAmount = Math.max(0, resolveLocalRepairAmount(moduleAttributes, ATTRIBUTE_SHIELD_BONUS, ATTRIBUTE_ENTITY_SHIELD_BOOST_AMOUNT, ATTRIBUTE_ENTITY_SHIELD_BOOST_AMOUNT_PER_SECOND, durationMs));
        return resolvedState;
    }
    if (normalizedEffectName === "structureRepair") {
        resolvedState.structureRepairAmount = Math.max(0, toFiniteNumber(moduleAttributes[ATTRIBUTE_STRUCTURE_DAMAGE_AMOUNT], 0));
        return resolvedState;
    }
    if (normalizedEffectName === "powerBooster") {
        ...
        resolvedState.capacitorBonus = Math.max(0, toFiniteNumber(chargeAttributes[ATTRIBUTE_CAPACITOR_BONUS], 0));
        return resolvedState;
    }
    if (normalizedEffectName === "fueledShieldBoosting") {
        resolvedState.shieldBoostAmount = Math.max(0, toFiniteNumber(moduleAttributes[ATTRIBUTE_SHIELD_BONUS], 0));
        resolvedState.chargeUnitsPerCycle = chargeItem ? 1 : 0;
        return resolvedState;
    }
    if (normalizedEffectName === "fueledArmorRepair") {
        const baseArmorAmount = Math.max(0, toFiniteNumber(moduleAttributes[ATTRIBUTE_ARMOR_DAMAGE_AMOUNT], 0));
        const chargedMultiplier = Math.max(1, toFiniteNumber(moduleAttributes[ATTRIBUTE_CHARGED_ARMOR_DAMAGE_MULTIPLIER], 1));
        const chargeUnitsPerCycle = Math.max(1, toInt(moduleAttributes[ATTRIBUTE_CHARGE_SIZE], 1));
...
function consumeLoadedChargeQuantity(entity, moduleItem, chargeItem, quantity, options = {}) {
    ...
    if (isNativeNpcEntity(entity)) {
        ...
        const cargoRecord = nativeNpcStore
            ...
            .find((entry) => (toInt(entry && entry.cargoID, 0) === chargeItemID ||
            (chargeItemID <= 0 &&
                moduleID > 0 &&
                toInt(entry && entry.moduleID, 0) === moduleID))) || null;
        if (cargoRecord) {
        ...
        if (Array.isArray(entity.nativeCargoItems)) {
            entity.nativeCargoItems = entity.nativeCargoItems.flatMap((cargoItem) => {
                const matchesCharge = (toInt(cargoItem && cargoItem.itemID, 0) === chargeItemID ||
                    (chargeItemID <= 0 &&
                        moduleID > 0 &&
    ...
    if (ownerSession &&
        ...
        options.callbacks.notifyRuntimeChargeTransitionToSession(ownerSession, toInt(entity.itemID, 0), toInt(moduleItem.flagID, 0), captureChargeState(chargeItem), captureChargeState(updatedChargeItem), toInt(entity.ownerID, 0));
    }
    return {
        success: true,
        data: {
...
function applyLocalArmorRepair(entity, amount) {
    ...
    const currentArmor = maxLayers.armor * (1 - toFiniteNumber(previousState.armorDamage, 0));
...
function applyLocalStructureRepair(entity, amount) {
    ...
    const currentStructure = maxLayers.structure * (1 - toFiniteNumber(previousState.damage, 0));
...
function commitLocalHealthChange(entity, scene, ownerSession, previousConditionState, callbacks, nowMs) {
    ...
    const healthTransitionResult = callbacks.buildShipHealthTransitionResult(entity, previousConditionState);
    callbacks.persistDynamicEntity(entity);
    if (ownerSession) {
        callbacks.notifyShipHealthAttributesToSession(ownerSession, entity, healthTransitionResult, nowMs);
    }
...
function commitLocalCapacitorChange(entity, ownerSession, callbacks, previousChargeAmount, nowMs) {
    ...
    callbacks.persistDynamicEntity(entity);
    callbacks.notifyCapacitorChangeToSession(ownerSession, entity, nowMs, previousChargeAmount);
}
function scheduleLocalReload(options = {}) {
    const { entity, scene, session, moduleItem, cycleState, effectMomentMs, } = options;
...
function shouldDelayNpcLocalCycle(cycleState, callbacks = {}) {
    const delayChance = Math.max(0, Math.min(1, toFiniteNumber(cycleState && cycleState.npcLocalCycleDelayChance, 0)));
    if (delayChance <= 0) {
        return {
            delayed: false,
            delayChance: 0,
...
function queueAncillaryLocalReloadOnManualDeactivate(options = {}) {
    const { entity, scene, session, moduleItem, effectState, nowMs = 0, callbacks = {}, } = options;
    if (!effectState || effectState.localCycleEffect !== true || !moduleItem) {
        return {
            matched: false,
        };
    }
    const family = String(effectState.localCycleFamily || effectState.effectName || "");
    ...
    const loadedCharge = callbacks.getEntityRuntimeLoadedCharge &&
    ...
    return {
        ...
        data: {
            reloadState: queueResult.data && queueResult.data.reloadState
                ? queueResult.data.reloadState
                : null,
...
function buildRuntimeOverrides(cycleState, baseRuntimeAttributes = null) {
    return {
        ...
        reactivationDelayMs: baseRuntimeAttributes && Number.isFinite(Number(baseRuntimeAttributes.reactivationDelayMs))
            ? Number(baseRuntimeAttributes.reactivationDelayMs)
            : 0,
        maxGroupActive: baseRuntimeAttributes && Number.isFinite(Number(baseRuntimeAttributes.maxGroupActive))
...
function prepareLocalCycleBoundary(options = {}) {
    const { entity, moduleItem, effectState, callbacks = {}, nowMs = 0, } = options;
    ...
    if (effectState.pendingLocalReload) {
        const reloadResult = resolvePendingLocalModuleReload(entity, effectState, moduleItem, {
            nowMs,
        });
    ...
    const shipItem = callbacks.getEntityRuntimeShipItem &&
        ...
        ? callbacks.getEntityRuntimeShipItem(entity)
        : null;
    const chargeItem = callbacks.getEntityRuntimeLoadedCharge &&
        typeof callbacks.getEntityRuntimeLoadedCharge === "function"
        ? callbacks.getEntityRuntimeLoadedCharge(entity, moduleItem)
        : null;
    const cycleState = resolveLocalCycleState({
...
function executeLocalCycle(options = {}) {
    const { scene, session, entity, moduleItem, effectState, nowMs = 0, activation = false, callbacks = {}, } = options;
    if (!effectState || effectState.localCycleEffect !== true) {
        return {
            matched: false,
        };
    ...
    if (cycleState.effectName === "shieldBoosting" || cycleState.effectName === "fueledShieldBoosting") {
        ...
        if (capacitorCapacity > 0 && cycleState.capacitorBonus > 0) {
            const nextAmount = Math.min(capacitorCapacity, previousChargeAmount + cycleState.capacitorBonus);
    ...
    if (cycleState.chargeUnitsPerCycle > 0 && cycleState.chargeItem) {
        const consumeResult = consumeLoadedChargeQuantity(entity, moduleItem, cycleState.chargeItem, cycleState.chargeUnitsPerCycle, {
        ...
        const nextQuantity = toInt(consumeResult.data && consumeResult.data.nextQuantity, 0);
        if (nextQuantity <= 0) {
    ...
    if (healthChanged) {
        commitLocalHealthChange(entity, scene, ownerSession, previousConditionState, callbacks, nowMs);
    }
    if (capacitorChanged) {
        commitLocalCapacitorChange(entity, ownerSession, callbacks, previousChargeAmount, nowMs);
    }
    return {
        matched: true,
        success: true,
...
//# sourceMappingURL=localCycleRuntime.js.map