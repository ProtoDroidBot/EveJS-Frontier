...
const { captureFittedTractorPresentation, applyTractorActivationCommand, applyTractorPullCommand, applyTractorSettleCommand, applyTractorStopCommand, } = require(path.join(__dirname, "../destiny/commands/tractorBeam.js"));
const { resolveDelayedDestinyStampState, } = require(path.join(__dirname, "../destiny/delivery/deliveryPolicy"));
...
function buildTractorHoldPosition(sourceEntity, targetEntity, movementDirection, holdDistanceMeters) {
    ...
    const centerDistanceMeters = Math.max(0, toFiniteNumber(holdDistanceMeters, 0)) +
        sourceRadius +
        targetRadius;
    return addVectors(sourceEntity.position, scaleVector(movementDirection, -centerDistanceMeters));
}
...
function resolveTractorFollowRangeMeters(effectState) {
    return Math.max(0, toFiniteNumber(effectState && effectState.tractorBeamFollowRangeMeters, toFiniteNumber(effectState && effectState.tractorBeamHoldDistanceMeters, toFiniteNumber(effectState && effectState.tractorBeamVelocityMetersPerSecond, DEFAULT_TRACTOR_FOLLOW_RANGE_METERS))));
}
function resolveTractorBeamActivation({ scene, entity, moduleItem, effectRecord, chargeItem = null, shipItem, skillMap = null, fittedItems = null, activeModuleContexts = null, options = {}, callbacks = {}, } = {}) {
    ...
    const durationAttributeID = toFiniteNumber(moduleAttributes[ATTRIBUTE_DURATION], 0) > 0
        ? ATTRIBUTE_DURATION
        : ATTRIBUTE_SPEED;
    const tractorVelocityMetersPerSecond = Math.max(0, roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_MAX_TRACTOR_VELOCITY], 0), 3));
    return {
        matched: true,
        success: true,
        data: {
            ...
            effectStatePatch: {
                ...
                tractorBeamFollowRangeMeters: tractorVelocityMetersPerSecond || DEFAULT_TRACTOR_FOLLOW_RANGE_METERS,
                tractorBeamHoldDistanceMeters: tractorVelocityMetersPerSecond || DEFAULT_TRACTOR_FOLLOW_RANGE_METERS,
...
function stopTractorTarget(scene, targetEntity, nowMs, options = {}) {
    ...
    const restoreMaxVelocity = Math.max(0, toFiniteNumber(effectState && effectState.tractorBeamOriginalMaxVelocity, targetEntity.maxVelocity));
    ...
    const updates = [];
    const restoreSpeedUpdateNeeded = effectState && effectState.tractorBeamPresentationPrimed === true;
...
function broadcastTractorBeamActivationBootstrap(scene, sourceEntity, effectState, nowMs, options = {}) {
    ...
    const tractorSpeedMetersPerSecond = Math.max(0, toFiniteNumber(effectState.tractorBeamVelocityMetersPerSecond, 0));
    ...
    effectState.tractorBeamOriginalMaxVelocity = Math.max(0, toFiniteNumber(effectState.tractorBeamOriginalMaxVelocity, baseTargetMaxVelocity));
...
function executeTractorBeamCycle({ scene, entity, effectState, callbacks = {}, } = {}) {
...
function tickScene(scene, nowMs, options = {}) {
    ...
    for (const sourceEntity of scene.dynamicEntities.values()) {
        ...
        for (const effectState of sourceEntity.activeModuleEffects.values()) {
            ...
            const stamp = captureFittedTractorPresentation({ scene, nowMs });
            const lastTickAtMs = Math.max(0, toFiniteNumber(effectState.lastTractorTickAtMs, nowMs));
            ...
            const surfaceDistance = callbacks.getEntitySurfaceDistance
                ...
                : 0;
            const holdDistanceMeters = Math.max(0, resolveTractorFollowRangeMeters(effectState));
            const settleDistanceMeters = holdDistanceMeters + TRACTOR_HOLD_DISTANCE_HYSTERESIS_METERS;
            const tractorSpeedMetersPerSecond = Math.max(0, toFiniteNumber(effectState.tractorBeamVelocityMetersPerSecond, 0));
            const baseTargetMaxVelocity = Math.max(0, toFiniteNumber(targetEntity.maxVelocity, 0));
            const currentHoldDirection = normalizeVector(subtractVectors(sourceEntity.position, targetEntity.position), targetEntity.direction || sourceEntity.direction || { x: 1, y: 0, z: 0 });
            const currentHoldPosition = buildTractorHoldPosition(sourceEntity, targetEntity, currentHoldDirection, holdDistanceMeters);
            if (surfaceDistance <= settleDistanceMeters) {
                ...
                const shouldPrimePresentation = effectState.tractorBeamPresentationPrimed !== true ||
            ...
            const moveDistanceMeters = Math.min(surfaceDistance - holdDistanceMeters, Math.max(0, toFiniteNumber(effectState.tractorBeamVelocityMetersPerSecond, 0)) *
                (deltaMs / 1000));
            if (moveDistanceMeters <= 0) {
                continue;
            }
            const movementDirection = normalizeVector(subtractVectors(sourceEntity.position, targetEntity.position), targetEntity.direction || sourceEntity.direction || { x: 1, y: 0, z: 0 });
            const holdPosition = buildTractorHoldPosition(sourceEntity, targetEntity, movementDirection, holdDistanceMeters);
            ...
            if (effectState.tractorBeamPresentationPrimed !== true) {
                effectState.tractorBeamOriginalMaxVelocity = Math.max(0, baseTargetMaxVelocity);
            }
            pullCommand.finishMotionState();
            const visibilityChanged = typeof scene.hasPendingEntityVisibilityTopologyChange === "function"
            ...
            const lastVelocityBroadcastStamp = toInt(effectState.lastTractorVelocityBroadcastStamp, -1);
            const firstVelocitySeed = !effectState.lastTractorBroadcastVelocity;
            const shouldPrimePresentation = visibilityChanged ||
                effectState.tractorBeamPresentationPrimed !== true ||
            ...
            const shouldBroadcastVelocity = visibilityChanged ||
                firstVelocitySeed ||
                lastVelocityBroadcastStamp !== stamp ||
                vectorsDiffer(targetEntity.velocity, effectState.lastTractorBroadcastVelocity, TRACTOR_VELOCITY_REBROADCAST_EPSILON_METERS_PER_SECOND);
...
//# sourceMappingURL=tractorBeamRuntime.js.map