...
function resolveActivationCharacterID(entity, moduleItem, shipItem, options: Record<string, any> = {}) {
...
function getRandom(callbacks: Record<string, any> = {}) {
...
function chooseWeighted(pool, callbacks: Record<string, any> = {}) {
...
function buildSalvageRewardEntries(targetEntity, options: Record<string, any> = {}) {
...
function computeGrantVolume(grantEntries: any[] = []) {
...
function resolveCargoSnapshot(characterID, shipItem, fittedItems: any[] = [], skillMap = null) {
...
function resolveCharacterID(entity, callbacks: Record<string, any> = {}) {
...
function syncSpawnedLootContainer(scene, containerRecord, callbacks: Record<string, any> = {}) {
  if (!containerRecord) {
    return {
      success: false,
...
function replaceInventoryWreckWithLootContainer(scene, targetEntity, nowMs, callbacks: Record<string, any> = {}) {
  const wreckID = toPositiveInt(targetEntity && targetEntity.itemID, 0);
...
function replaceNativeWreckWithLootContainer(scene, targetEntity, nowMs, callbacks: Record<string, any> = {}) {
  const wreckID = toPositiveInt(targetEntity && targetEntity.itemID, 0);
...
function completeSuccessfulSalvage(scene, targetEntity, nowMs, callbacks: Record<string, any> = {}) {
...
function syncInventoryChangesToSession(session, changes: any[] = [], callbacks: Record<string, any> = {}) {
...
function resolveInventorySession(entity, callbacks: Record<string, any> = {}) {
