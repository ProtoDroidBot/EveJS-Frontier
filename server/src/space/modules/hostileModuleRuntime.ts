...
function getPreferredAttributeValue(attributeMap, preferredAttributeIDs: any[] = [], fallback = 0) {
...
function buildHostileModifierEntries(attributeMap, modifierSpecs: any[] = [], stackingPenalized = false) {
  const entries: any[] = [];
...
function resolveHostileRangeMultiplier(effectState, sourceEntity, targetEntity, callbacks: Record<string, any> = {}) {
...
function recomputeTargetAggregateState(targetEntity) {
  ...
  const weaponModuleModifierEntries: any[] = [];
  const weaponChargeModifierEntries: any[] = [];
...
function resolveNosTransferAllowed(sourceEntity, targetEntity, effectState, callbacks: Record<string, any> = {}) {
...
function executeHostileModuleCycle({
  ...
  const refreshResult: Record<string, any> = refreshHostileModuleState({
    scene,
    sourceEntity: entity,
    targetEntity,
    effectState,
