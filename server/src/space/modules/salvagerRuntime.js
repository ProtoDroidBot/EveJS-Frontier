...
const { ITEM_FLAGS, createSpaceItemForOwner, findItemById, getItemMutationVersion, grantItemToOwnerLocation, grantItemsToCharacterLocation, listContainerItems, moveItemToLocation, removeInventoryItem, } = require(path.join(__dirname, "../../services/inventory/itemStore"));
const { getAttributeIDByNames, getEffectIDByNames, getTypeAttributeMap, buildShipResourceState, } = require(path.join(__dirname, "../../services/fitting/liveFittingState"));
const { resolveItemByName, resolveItemByTypeID, } = require(path.join(__dirname, "../../services/inventory/itemTypeRegistry"));
...
const { buildChildEntityScopeMetadata, canEntitiesInteractLocally, } = require(path.join(__dirname, "../destiny/identity/interactionScope.js"));
const { getActiveImplantLocationModifierSources, } = require(path.join(__dirname, "../../services/dogma/implants/activeImplantModifiers"));
...
function buildSpaceContainerOptions(targetEntity, sourceRecord = null, nowMs = Date.now()) {
    ...
    const position = cloneVector(targetEntity && targetEntity.position ? targetEntity.position : sourceSpaceState.position);
    const direction = cloneVector(targetEntity && targetEntity.direction ? targetEntity.direction : sourceSpaceState.direction, { x: 1, y: 0, z: 0 });
    const velocity = cloneVector(targetEntity && targetEntity.velocity ? targetEntity.velocity : sourceSpaceState.velocity);
    const targetPoint = cloneVector(targetEntity && targetEntity.targetPoint ? targetEntity.targetPoint : sourceSpaceState.targetPoint, position);
    const createdAtMs = Math.max(0, toFiniteNumber(sourceRecord && sourceRecord.createdAtMs, 0) ||
        toFiniteNumber(nowMs, Date.now()));
    const sourceExpiresAtMs = toFiniteNumber((sourceRecord && sourceRecord.expiresAtMs) ||
        (targetEntity && targetEntity.expiresAtMs), 0);
...
function createLootContainerAtWreck(targetEntity, sourceRecord, nowMs) {
    ...
    const ownerID = toPositiveInt((sourceRecord && sourceRecord.ownerID) ||
        (targetEntity && targetEntity.ownerID), 0);
    const systemID = toPositiveInt((sourceRecord && (sourceRecord.systemID || sourceRecord.locationID)) ||
        (targetEntity && targetEntity.systemID), 0);
    if (!ownerID || !systemID) {
        return {
            success: false,
    ...
    const createResult = createSpaceItemForOwner(ownerID, systemID, containerType, buildSpaceContainerOptions(targetEntity, sourceRecord, nowMs));
...
function resolveEntitySize(targetEntity, itemRecord = null, nativeWreckRecord = null) {
    ...
    const typeID = toPositiveInt((itemRecord && itemRecord.typeID) ||
        (nativeWreckRecord && nativeWreckRecord.typeID) ||
        (targetEntity && targetEntity.typeID), 0);
...
function calculateSalvageChancePercent(accessBasePercent, accessBonusPercent) {
    return Math.max(0, Math.min(100, roundNumber(toFiniteNumber(accessBasePercent, 0) + toFiniteNumber(accessBonusPercent, 0), 6)));
}
function buildSalvageChanceSnapshot(targetEntity, accessBonusPercent = 0) {
    ...
    const nativeWreckRecord = targetEntity && targetEntity.nativeNpcWreck === true
        ...
        : null;
    const accessBasePercent = resolveAccessDifficultyBase(targetEntity, itemRecord, nativeWreckRecord);
    const chancePercent = calculateSalvageChancePercent(accessBasePercent, accessBonusPercent);
    return {
        accessBasePercent,
...
function resolveCargoSnapshot(characterID, shipItem, fittedItems = [], skillMap = null) {
    ...
    const usedVolume = computeUsedVolume(listContainerItems(numericCharacterID, shipID, ITEM_FLAGS.CARGO_HOLD));
    const snapshot = {
    ...
    snapshot.availableVolume = Math.max(0, roundNumber(snapshot.cargoCapacity - snapshot.usedVolume, 6));
...
function resolveCharacterID(entity, callbacks = {}) {
    ...
    return toPositiveInt(entity &&
        (entity.characterID ||
            entity.pilotCharacterID ||
            (entity.session && (entity.session.characterID || entity.session.charid))), 0);
}
function resolveSalvagerActivation({ scene, entity, moduleItem, effectRecord, chargeItem = null, shipItem, skillMap = null, fittedItems = null, activeModuleContexts = null, options = {}, callbacks = {}, } = {}) {
    ...
    const moduleAttributes = buildLiveModuleAttributeMap(shipItem, moduleItem, chargeItem, skillMap, fittedItems, activeModuleContexts, { additionalLocationModifierSources });
    if (!moduleAttributes) {
        return {
            matched: true,
            success: false,
    ...
    const maxRangeMeters = Math.max(0, roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_MAX_RANGE], 0), 3));
    ...
    const nativeWreckRecord = targetEntity.nativeNpcWreck === true
        ...
        : null;
    const accessBasePercent = resolveAccessDifficultyBase(targetEntity, itemRecord, nativeWreckRecord);
    const accessBonusPercent = toFiniteNumber(moduleAttributes[ATTRIBUTE_ACCESS_DIFFICULTY_BONUS], 0);
    const chancePercent = calculateSalvageChancePercent(accessBasePercent, accessBonusPercent);
...
function transferInventoryWreckLootToContainer(wreckID, containerID) {
    ...
    for (const item of contents) {
        const moveResult = moveItemToLocation(item.itemID, containerID, ITEM_FLAGS.HANGAR);
...
function transferNativeWreckLootToContainer(wreckRecord, containerID, fallbackOwnerID = 0) {
    ...
    for (const itemRecord of contents) {
        ...
        const ownerID = toPositiveInt((itemRecord && itemRecord.ownerID) || fallbackOwnerID, 0);
        const grantResult = grantItemToOwnerLocation(ownerID, containerID, ITEM_FLAGS.HANGAR, itemType, quantity, {
...
function replaceNativeWreckWithLootContainer(scene, targetEntity, nowMs, callbacks = {}) {
    ...
    const container = createResult.data.container;
    const changes = [...(createResult.data.changes || [])];
    const transferResult = transferNativeWreckLootToContainer(wreckRecord, container.itemID, toPositiveInt(container.ownerID, 0));
    if (!transferResult.success) {
        return {
            success: false,
            errorMsg: transferResult.errorMsg || "LOOT_TRANSFER_FAILED",
...
function executeSalvagerCycle({ scene, entity, effectState, nowMs = Date.now(), callbacks = {}, } = {}) {
    ...
    if (rewardEntries.length > 0) {
        const grantResult = grantItemsToCharacterLocation(characterID, toPositiveInt(shipItem.itemID, toPositiveInt(entity && entity.itemID, 0)), ITEM_FLAGS.CARGO_HOLD, rewardEntries);
    ...
    syncInventoryChangesToSession(resolveInventorySession(entity, callbacks), completionResult.changes || [], callbacks);
    ...
    try {
        require(path.join(__dirname, "../../services/dailyGoals/dailyGoalsState")).recordActivity(characterID, "salvage_wreck", 1);
    }
    catch (dailyGoalError) {
        log.debug(`[SalvagerRuntime] daily-goal salvage_wreck hook failed char=${characterID}: ${dailyGoalError.message}`);
    }
    return {
        success: false,
        stopReason: "target",
...
//# sourceMappingURL=salvagerRuntime.js.map