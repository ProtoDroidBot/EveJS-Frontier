...
const { getModuleChargeCapacity, } = require(path.join(__dirname, "../../services/fitting/liveFittingState"));
...
const { logNpcCombatDebug, summarizeNpcCombatCargo, summarizeNpcCombatEntity, summarizeNpcCombatModule, } = require(path.join(__dirname, "../npc/npcCombatDebug"));
...
function normalizeReloadState(reloadState, source) {
    ...
    return {
        ...
        resumeMode: String(reloadState.resumeMode || "start"),
...
function canUseVirtualNativeNpcReloadReserve(entity, moduleItem, chargeTypeID) {
    return Boolean(isNativeNpcEntity(entity) &&
        moduleItem &&
        toInt(moduleItem.itemID, 0) > 0 &&
        chargeTypeID > 0);
}
function queuePlayerModuleReload(session, moduleItem, chargeTypeID, options = {}) {
    if (!session || !moduleItem || chargeTypeID <= 0) {
        return {
            success: false,
            errorMsg: "NO_AMMO",
        };
    ...
    const reloadResult = dogma.queueAutomaticModuleReload(session, moduleItem, {
        chargeTypeID,
        shipID: toInt(options.shipID, toInt(moduleItem.locationID, 0)),
        ammoLocationID: toInt(options.ammoLocationID, toInt(moduleItem.locationID, 0)),
    ...
    return {
        ...
        data: {
            reloadState: normalizeReloadState({
                ...(reloadResult.data && reloadResult.data.reloadState),
                resumeMode: options.resumeMode || "start",
            }, "player"),
        },
    };
}
function queueNativeNpcModuleReload(entity, moduleItem, chargeTypeID, options = {}) {
    const sourceStacks = resolveNativeNpcReloadSources(entity, chargeTypeID);
    const usingVirtualReserve = sourceStacks.length === 0 &&
    ...
    logNpcCombatDebug("npc.reload.queue.request", {
        ...
        resumeMode: String(options.resumeMode || "start"),
    });
...
function queueAutomaticLocalModuleReload(options = {}) {
    ...
    if (isNativeNpcEntity(entity)) {
        return queueNativeNpcModuleReload(entity, moduleItem, chargeTypeID, options);
    }
    return queuePlayerModuleReload(options.session || null, moduleItem, chargeTypeID, options);
}
function completeNativeNpcLocalModuleReload(entity, moduleItem, reloadState) {
...
function resolvePendingLocalModuleReload(entity, effectState, moduleItem, options = {}) {
    const reloadState = effectState && effectState.pendingLocalReload
    ...
    if (reloadState.source === "nativeNpc") {
        const completionResult = completeNativeNpcLocalModuleReload(entity, moduleItem, reloadState);
    ...
    effectState.pendingLocalReload = null;
...
//# sourceMappingURL=localCycleReloads.js.map