...
const { getFuelStorageFlagsForType, } = require(path.join(__dirname, "../../services/inventory/fuelBayInventory"));
...
function buildFuelFlagPriority(fuelTypeID, priorityFlags = []) {
    return new Map(getFuelStorageFlagsForType(fuelTypeID, priorityFlags)
...
function getFuelStacksForShipStorage(entity, fuelTypeID, callbacks = {}, priorityFlags = []) {
    ...
    if (isNativeNpcEntity(entity)) {
        const storeItems = filterNativeStacksByFuelFlags(nativeNpcStore.listNativeCargoForEntity(entity.itemID), normalizedFuelTypeID, priorityFlags);
        if (storeItems.length > 0) {
            return storeItems;
        }
        return filterNativeStacksByFuelFlags(Array.isArray(entity.nativeCargoItems) ? entity.nativeCargoItems : [], normalizedFuelTypeID, priorityFlags);
    }
    const characterID = callbacks.resolveCharacterID &&
        ...
        : 0;
    return listCharacterFuelStacks(characterID, entity.itemID, normalizedFuelTypeID, priorityFlags);
}
function getFuelQuantityFromStacks(stacks = []) {
    return stacks.reduce((sum, stack) => {
        const quantity = Math.max(0, toInt(stack && (stack.quantity ?? stack.stacksize), 0));
        return sum + quantity;
    }, 0);
}
...
function consumeFuelFromShipStorage(entity, fuelTypeID, quantity, callbacks = {}, priorityFlags = []) {
    ...
    const fuelStacks = getFuelStacksForShipStorage(entity, normalizedFuelTypeID, callbacks, priorityFlags).sort((left, right) => {
    ...
    for (const stack of fuelStacks) {
        ...
        const stackQuantity = Math.max(0, toInt(stack && (stack.quantity ?? stack.stacksize), 0));
        if (stackQuantity <= 0) {
            continue;
        }
        const nextQuantity = Math.max(0, stackQuantity - Math.min(stackQuantity, remaining));
        if (isNativeNpcEntity(entity)) {
...
//# sourceMappingURL=sharedFuelRuntime.js.map