...
const { getAttributeIDByNames, getModuleChargeGroupIDs, getRequiredSkillRequirements, isChargeCompatibleWithModule, applyOtherItemModifiersToAttributes, } = require(path.join(__dirname, "../../services/fitting/liveFittingState"));
const { buildLocationModifiedAttributeMap, collectShipModifierAttributes, } = require(path.join(__dirname, "../combat/weaponDogma"));
...
function isInSameFleet(leftCharacterID, rightCharacterID) {
    const fleetHelpers = require(path.join(__dirname, "../../services/fleets/fleetHelpers"));
...
function normalizeDbuffCollectionEntry(entry, fallbackCollectionID = 0) {
    ...
    const collectionID = toInt(entry.collectionID, fallbackCollectionID);
    const operation = toInt(entry.operation, 0) > 0
    ...
    return Object.freeze({
        ...
        locationGroupModifiers: freezePlainEntries(entry.locationGroupModifiers, (candidate) => candidate.groupID > 0),
        locationCategoryModifiers: freezePlainEntries(entry.locationCategoryModifiers, (candidate) => candidate.categoryID > 0),
        locationRequiredSkillModifiers: freezePlainEntries(entry.locationRequiredSkillModifiers, (candidate) => candidate.skillID > 0),
    });
}
function buildDbuffCollectionsByID() {
    const collectionsByID = new Map();
    const result = database.read(DBUFF_COLLECTIONS_TABLE, "/");
    const rawRoot = result && result.success && result.data && typeof result.data === "object"
        ? result.data
        : null;
    const rawCollections = rawRoot &&
...
function moduleUsesExpeditionChargeGroup(moduleItem) {
    ...
    return (chargeGroups instanceof Set &&
...
function isExpeditionCommandBurst(moduleItem, chargeItem, collectionValues) {
    if (toInt(chargeItem && chargeItem.groupID, 0) ===
...
function collectDefinitionModifierEntries(destination, collectionDefinition, targetItem, outputValue) {
    ...
    for (const modifier of collectionDefinition.locationGroupModifiers) {
        if (toInt(targetItem && targetItem.groupID, 0) !== modifier.groupID ||
    ...
    for (const modifier of collectionDefinition.locationCategoryModifiers) {
        if (toInt(targetItem && targetItem.categoryID, 0) !== modifier.categoryID ||
    ...
    for (const modifier of collectionDefinition.locationRequiredSkillModifiers) {
        if (matchedSkillAttributes.has(modifier.dogmaAttributeID) ||
...
function buildClientDbuffStateEntries(entity, options = {}) {
    ...
    const buildExpiry = typeof options.buildExpiry === "function"
        ? options.buildExpiry
...
function applyTimedCommandBurstToEntity(entity, collectionValues, durationMs, nowMs) {
    ...
    for (const [collectionID, value] of collectionValues.entries()) {
        ...
        if (!existing) {
            buckets.set(collectionID, createAggregator(collectionDefinition, expiryTime, round6(value)));
            continue;
        }
...
function resolveBurstAffinityGroupID(entity) {
    ...
    return toInt(entity.remoteRepairBurstAffinityGroupID ??
        entity.commandBurstAffinityGroupID, 0);
}
...
function resolveCommandBurstRecipients(scene, sourceEntity, effectState, nowMs = null) {
    ...
    recipients.sort((left, right) => (toInt(left && left.itemID, 0) - toInt(right && right.itemID, 0)));
    return recipients;
}
function collectModifierEntriesForItem(entity, targetItem, nowMs = null) {
    ...
    for (const [collectionID, aggregator] of buckets.entries()) {
        ...
        collectDefinitionModifierEntries(modifierEntries, collectionDefinition, targetItem, outputValue);
    }
    return modifierEntries;
}
function resolveCommandBurstActivation({ characterID = 0, effectRecord, moduleItem, chargeItem = null, shipItem, skillMap = null, fittedItems = null, activeModuleContexts = null, } = {}) {
    ...
    const runtimeFamily = resolveCommandBurstRuntimeFamily(definition, moduleItem, chargeItem, collectionValues);
    return {
        matched: true,
        success: true,
        data: {
            runtimeAttrs: {
                ...
                commandBurstSnapshot: {
                    ...
                    durationMs: Math.max(1, round6(toFiniteNumber(moduleAttributes[ATTRIBUTE_DURATION], 0))),
                    buffDurationMs: Math.max(1, round6(toFiniteNumber(moduleAttributes[ATTRIBUTE_BUFF_DURATION], 0))),
            ...
            effectStatePatch: {
                ...
                commandBurstRangeMeters: Math.max(0, round6(toFiniteNumber(moduleAttributes[ATTRIBUTE_MAX_RANGE], 0))),
                commandBurstBuffDurationMs: Math.max(1, round6(toFiniteNumber(moduleAttributes[ATTRIBUTE_BUFF_DURATION], 0))),
                commandBurstReloadTimeMs: Math.max(0, round6(toFiniteNumber(moduleAttributes[ATTRIBUTE_RELOAD_TIME], 0))),
...
//# sourceMappingURL=commandBurstRuntime.js.map