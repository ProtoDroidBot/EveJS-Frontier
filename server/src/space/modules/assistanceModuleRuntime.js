...
const { getAttributeIDByNames, } = require(path.join(__dirname, "../../services/fitting/liveFittingState"));
...
const ATTRIBUTE_SCAN_GRAVIMETRIC_STRENGTH_PERCENT = getAttributeIDByNames("scanGravimetricStrengthPercent") || 1027;
const ATTRIBUTE_SCAN_LADAR_STRENGTH_PERCENT = getAttributeIDByNames("scanLadarStrengthPercent") || 1028;
const ATTRIBUTE_SCAN_MAGNETOMETRIC_STRENGTH_PERCENT = getAttributeIDByNames("scanMagnetometricStrengthPercent") || 1029;
const ATTRIBUTE_SCAN_RADAR_STRENGTH_PERCENT = getAttributeIDByNames("scanRadarStrengthPercent") || 1030;
const ATTRIBUTE_SCAN_GRAVIMETRIC_STRENGTH = getAttributeIDByNames("scanGravimetricStrength") || 211;
const ATTRIBUTE_SCAN_LADAR_STRENGTH = getAttributeIDByNames("scanLadarStrength") || 209;
const ATTRIBUTE_SCAN_MAGNETOMETRIC_STRENGTH = getAttributeIDByNames("scanMagnetometricStrength") || 210;
const ATTRIBUTE_SCAN_RADAR_STRENGTH = getAttributeIDByNames("scanRadarStrength") || 208;
const ATTRIBUTE_REMOTE_REPAIR_IMPEDANCE = getAttributeIDByNames("remoteRepairImpedance") || 2116;
...
function isStructureEntity(entity) {
    return Boolean(entity && (entity.kind === "structure" ||
...
function isAssistanceTargetEntity(effectData, entity) {
    return Boolean(entity && (entity.kind === "ship" ||
        (entity.kind === "drone" &&
            isRemoteRepairFamily(effectData && (effectData.family || effectData.assistanceFamily)))));
}
...
function isBlockedStructureRemoteRepair(effectData, targetEntity) {
    const family = effectData
        ...
        : null;
    return (isStructureEntity(targetEntity) &&
        isRemoteRepairFamily(family));
}
function getPreferredAttributeValue(moduleAttributes, attributeIDs, fallback = 0) {
    const resolvedAttributes = moduleAttributes && typeof moduleAttributes === "object"
        ? moduleAttributes
        : null;
...
function resolveRemoteAssistanceMultiplier(targetEntity, resistanceAttributeID) {
    ...
    return roundNumber(clamp(getEntityPassiveAttributeValue(targetEntity, targetResistanceAttributeID, 1), 0, 1), 6);
}
function resolveAssistanceResistanceMultiplier(effectState, targetEntity) {
    const resistanceAttributeID = toInt(effectState && effectState.assistanceRemoteResistanceAttributeID, 0);
    ...
    if (isRemoteRepairFamily(family) ||
        family === ASSISTANCE_FAMILY_REMOTE_CAPACITOR) {
...
function resolveAssistanceMultiplier(effectState, sourceEntity, targetEntity, callbacks = {}) {
    const surfaceDistance = Math.max(0, toFiniteNumber(callbacks.getEntitySurfaceDistance &&
        callbacks.getEntitySurfaceDistance(sourceEntity, targetEntity), 0));
    const optimalRangeMeters = Math.max(0, toFiniteNumber(effectState && effectState.assistanceMaxRangeMeters, 0));
    const falloffMeters = Math.max(0, toFiniteNumber(effectState && effectState.assistanceFalloffMeters, 0));
...
function ensureTargetAssistanceState(targetEntity, create = false) {
    ...
    return (targetEntity.assistanceModuleState &&
...
function buildAssistanceTargetRecord(sourceEntity, effectState, targetEntity, nowMs, rangeMultiplier) {
    return Object.freeze({
        key: buildTargetAssistanceStateKey(sourceEntity && sourceEntity.itemID, effectState && effectState.moduleID, targetEntity && targetEntity.itemID),
        ...
        expiresAtMs: Math.max(0, toFiniteNumber(nowMs, Date.now()) +
        ...
        modifierEntries: Object.freeze((Array.isArray(effectState && effectState.assistanceModifierEntries)
            ? effectState.assistanceModifierEntries
            : []).map((entry) => Object.freeze({ ...entry }))),
        weaponModuleModifierEntries: Object.freeze((Array.isArray(effectState && effectState.assistanceWeaponModuleModifierEntries)
            ? effectState.assistanceWeaponModuleModifierEntries
            : []).map((entry) => Object.freeze({ ...entry }))),
        weaponChargeModifierEntries: Object.freeze((Array.isArray(effectState && effectState.assistanceWeaponChargeModifierEntries)
...
function appendScaledModifierEntries(destination, entries, scaledMultiplier) {
    for (const entry of Array.isArray(entries) ? entries : []) {
        const value = roundNumber(toFiniteNumber(entry && entry.value, 0) * Math.max(scaledMultiplier, 0), 6);
...
function recomputeTargetAssistanceAggregateState(targetEntity) {
    ...
    for (const record of targetState.incomingEffects.values()) {
        ...
        const resistanceMultiplier = record.resistanceAttributeID > 0
            ...
            : 1;
        const scaledMultiplier = Math.max(record.rangeMultiplier, 0) *
            Math.max(resistanceMultiplier, 0);
        appendScaledModifierEntries(modifierEntries, record.modifierEntries, scaledMultiplier);
        appendScaledModifierEntries(weaponModuleModifierEntries, record.weaponModuleModifierEntries, scaledMultiplier);
        appendScaledModifierEntries(weaponChargeModifierEntries, record.weaponChargeModifierEntries, scaledMultiplier);
    }
    const nextSignature = buildAggregateSignature(modifierEntries, weaponModuleModifierEntries, weaponChargeModifierEntries);
    const changed = nextSignature !== String(targetState.aggregateSignature || "");
    targetState.modifierEntries = Object.freeze(modifierEntries.map((entry) => Object.freeze({ ...entry })));
    targetState.weaponModuleModifierEntries = Object.freeze(weaponModuleModifierEntries.map((entry) => Object.freeze({ ...entry })));
    targetState.weaponChargeModifierEntries = Object.freeze(weaponChargeModifierEntries.map((entry) => Object.freeze({ ...entry })));
...
function removeTargetAssistanceRecord(targetEntity, sourceEntity, effectState) {
    ...
    const key = buildTargetAssistanceStateKey(sourceEntity && sourceEntity.itemID, effectState && effectState.moduleID, targetEntity && targetEntity.itemID);
...
function refreshAssistanceModuleState({ scene, sourceEntity, targetEntity, effectState, nowMs, rangeMultiplier = 1, } = {}) {
    ...
    const record = buildAssistanceTargetRecord(sourceEntity, effectState, targetEntity, nowMs, rangeMultiplier);
...
function removeAssistanceModuleState({ targetEntity, sourceEntity, effectState, } = {}) {
...
function resolveSourceCharacterID(entity) {
    return toInt(entity && (entity.characterID ??
        entity.pilotCharacterID ??
        (entity.session && (entity.session.characterID || entity.session.charid))), 0);
}
function resolveAssistanceModuleActivation({ scene, entity, moduleItem, effectRecord, chargeItem = null, shipItem, skillMap = null, fittedItems = null, activeModuleContexts = null, options = {}, callbacks = {}, } = {}) {
    ...
    if (!canEntitiesInteractLocally(entity, targetEntity)) {
        return {
            matched: true,
            success: false,
            errorMsg: "TARGET_NOT_FOUND",
    ...
    if (!callbacks.isEntityLockedTarget ||
    ...
    const moduleAttributes = buildLiveModuleAttributeMap(shipItem, moduleItem, chargeItem, skillMap, fittedItems, activeModuleContexts, {
        additionalLocationModifierSources: characterID > 0
            ? getActiveImplantLocationModifierSources(characterID)
            : [],
        additionalShipModifierEntries: characterID > 0
    ...
    const preferredDurationAttributeID = (getPreferredAttributeValue(moduleAttributes, [
        effectRecord && effectRecord.durationAttributeID,
    ], 0) > 0)
        ? toInt(effectRecord && effectRecord.durationAttributeID, 0)
        : 0;
    const rawDurationMs = getPreferredAttributeValue(moduleAttributes, [
        preferredDurationAttributeID,
        ATTRIBUTE_DURATION,
        ATTRIBUTE_SPEED,
    ], 1000);
    const durationAttributeID = preferredDurationAttributeID > 0
        ? preferredDurationAttributeID
        : toFiniteNumber(moduleAttributes[ATTRIBUTE_DURATION], 0) > 0
            ? ATTRIBUTE_DURATION
            : ATTRIBUTE_SPEED;
    const modifierEntries = buildAssistanceModifierEntries(moduleAttributes, definition.moduleModifierSpecs, definition.stackingPenalized === true);
    const weaponModuleModifierEntries = buildAssistanceModifierEntries(moduleAttributes, definition.weaponModuleModifierSpecs, definition.stackingPenalized === true);
    const weaponChargeModifierEntries = buildAssistanceModifierEntries(moduleAttributes, definition.weaponChargeModifierSpecs, definition.stackingPenalized === true);
    const effectStatePatch = {
        ...
        assistanceMaxRangeMeters: Math.max(0, roundNumber(getPreferredAttributeValue(moduleAttributes, [
            effectRecord && effectRecord.rangeAttributeID,
            ATTRIBUTE_MAX_RANGE,
        ], 0), 3)),
        assistanceFalloffMeters: Math.max(0, roundNumber(getPreferredAttributeValue(moduleAttributes, [
            effectRecord && effectRecord.falloffAttributeID,
            ATTRIBUTE_FALLOFF_EFFECTIVENESS,
        ], 0), 3)),
        assistanceShieldBonusAmount: Math.max(0, roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_SHIELD_BONUS], 0), 6)),
        assistanceArmorRepairAmount: Math.max(0, roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_ARMOR_DAMAGE_AMOUNT], 0), 6)),
        assistanceHullRepairAmount: Math.max(0, roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_STRUCTURE_DAMAGE_AMOUNT], 0), 6)),
        assistancePowerTransferAmount: Math.max(0, roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_POWER_TRANSFER_AMOUNT], 0), 6)),
        assistanceRemoteResistanceAttributeID: Math.max(0, toInt(moduleAttributes[ATTRIBUTE_REMOTE_RESISTANCE_ID], 0)),
        forceFreshAcquireSpecialFxReplay: true,
        repeat: resolvePersistentRepeat(rawDurationMs),
    };
    const rangeResult = resolveAssistanceMultiplier(effectStatePatch, entity, targetEntity, callbacks);
    ...
    return {
        ...
        data: {
            targetEntity,
            runtimeAttrs: {
                capNeed: Math.max(0, roundNumber(getPreferredAttributeValue(moduleAttributes, [
                    ATTRIBUTE_CAPACITOR_NEED,
                    effectRecord && effectRecord.dischargeAttributeID,
                ], 0), 6)),
                durationMs: Math.max(1, roundNumber(rawDurationMs, 3)),
                durationAttributeID,
                reactivationDelayMs: Math.max(0, roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_REACTIVATION_DELAY], 0), 3)),
...
function commitHealthRepair({ scene, targetEntity, previousConditionState, callbacks = {}, nowMs, } = {}) {
    const healthResult = callbacks.buildShipHealthTransitionResult &&
    ...
    if (targetEntity.session && healthResult && callbacks.notifyShipHealthAttributesToSession) {
        callbacks.notifyShipHealthAttributesToSession(targetEntity.session, targetEntity, healthResult, nowMs);
    }
...
function executeAssistanceModuleCycle({ scene, session, entity, effectState, nowMs, callbacks = {}, } = {}) {
    ...
    const multiplierResult = resolveAssistanceMultiplier(effectState, entity, targetEntity, callbacks);
    if (!multiplierResult.success) {
        return multiplierResult;
    }
    const resistanceMultiplier = resolveAssistanceResistanceMultiplier(effectState, targetEntity);
    const assistanceMultiplier = roundNumber(Math.max(multiplierResult.multiplier, 0) * Math.max(resistanceMultiplier, 0), 6);
    ...
    const aggregateChanged = Boolean(refreshResult.data && refreshResult.data.aggregateChanged === true);
    if (effectState.assistanceFamily === "remoteShield") {
        const shieldCapacity = Math.max(0, toFiniteNumber(targetEntity && targetEntity.shieldCapacity, 0));
        const shieldBonusAmount = Math.max(0, toFiniteNumber(effectState.assistanceShieldBonusAmount, 0)) * Math.max(assistanceMultiplier, 0);
        const previousConditionState = buildNormalizedConditionState(targetEntity, callbacks);
        if (shieldCapacity > 0 && shieldBonusAmount > 0) {
            const currentShieldRatio = toFiniteNumber(previousConditionState && previousConditionState.shieldCharge, toFiniteNumber(targetEntity && targetEntity.capacitorChargeRatio, 0));
    ...
    if (effectState.assistanceFamily === "remoteCapacitor") {
        const capacitorCapacity = Math.max(0, toFiniteNumber(targetEntity && targetEntity.capacitorCapacity, 0));
        const capacitorBonusAmount = Math.max(0, toFiniteNumber(effectState.assistancePowerTransferAmount, 0)) * Math.max(assistanceMultiplier, 0);
        let appliedAmount = 0;
        if (capacitorCapacity > 0 && capacitorBonusAmount > 0) {
            const previousChargeAmount = callbacks.getEntityCapacitorAmount
                ...
                : 0;
            const nextChargeAmount = Math.min(capacitorCapacity, previousChargeAmount + capacitorBonusAmount);
            appliedAmount = Math.max(0, nextChargeAmount - previousChargeAmount);
            if (callbacks.setEntityCapacitorRatio) {
                callbacks.setEntityCapacitorRatio(targetEntity, nextChargeAmount / capacitorCapacity);
            }
            if (callbacks.persistDynamicEntity) {
                callbacks.persistDynamicEntity(targetEntity);
            }
            if (targetEntity.session &&
                ...
                callbacks.notifyCapacitorChangeToSession(targetEntity.session, targetEntity, nowMs, previousChargeAmount);
            }
        }
        return {
            success: true,
    ...
    if (effectState.assistanceFamily === "remoteArmor") {
        const armorHP = Math.max(0, toFiniteNumber(targetEntity && targetEntity.armorHP, 0));
        const armorRepairAmount = Math.max(0, toFiniteNumber(effectState.assistanceArmorRepairAmount, 0)) * Math.max(assistanceMultiplier, 0);
        const previousConditionState = buildNormalizedConditionState(targetEntity, callbacks);
        if (armorHP > 0 && armorRepairAmount > 0) {
            const currentArmorDamageRatio = clamp(toFiniteNumber(previousConditionState && previousConditionState.armorDamage, 0), 0, 1);
            const nextArmorDamageRatio = Math.max(0, currentArmorDamageRatio - (armorRepairAmount / armorHP));
            targetEntity.conditionState = callbacks.normalizeShipConditionState
                ? callbacks.normalizeShipConditionState({
                    ...previousConditionState,
    ...
    if (effectState.assistanceFamily === "remoteHull") {
        const structureHP = Math.max(0, toFiniteNumber(targetEntity && targetEntity.structureHP, 0));
        const hullRepairAmount = Math.max(0, toFiniteNumber(effectState.assistanceHullRepairAmount, 0)) * Math.max(assistanceMultiplier, 0);
        const previousConditionState = buildNormalizedConditionState(targetEntity, callbacks);
        if (structureHP > 0 && hullRepairAmount > 0) {
            const currentStructureDamageRatio = clamp(toFiniteNumber(previousConditionState && previousConditionState.damage, 0), 0, 1);
            const nextStructureDamageRatio = Math.max(0, currentStructureDamageRatio - (hullRepairAmount / structureHP));
            targetEntity.conditionState = callbacks.normalizeShipConditionState
                ? callbacks.normalizeShipConditionState({
                    ...previousConditionState,
...
//# sourceMappingURL=assistanceModuleRuntime.js.map