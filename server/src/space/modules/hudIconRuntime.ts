...
function buildHudIconState(input: Record<string, any> = {}) {
...
function pruneExpiredHudIconStates(entity, nowMs, options: Record<string, any> = {}) {
  const stateMap = getHudIconStateMap(entity, false);
  if (!(stateMap instanceof Map) || stateMap.size <= 0) {
    return {
      removed: [],
...
function listActiveHudIconStates(entity, nowMs, options: Record<string, any> = {}) {
