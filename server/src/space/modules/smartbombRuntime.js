...
const { sumDamageVector, } = require(path.join(__dirname, "../combat/damage"));
...
function isSmartbombEffect(effectRecord, moduleItem = null) {
    return (normalizeEffectName(effectRecord) === "empwave" ||
        toInt(moduleItem && moduleItem.groupID, 0) === GROUP_SMART_BOMB);
}
function buildSmartbombDamageVector(moduleAttributes) {
    const multiplier = Math.max(0, toFiniteNumber(moduleAttributes && moduleAttributes[ATTRIBUTE_DAMAGE_MULTIPLIER], 1));
    return {
        em: roundNumber(toFiniteNumber(moduleAttributes && moduleAttributes[ATTRIBUTE_EM_DAMAGE], 0) * multiplier),
        thermal: roundNumber(toFiniteNumber(moduleAttributes && moduleAttributes[ATTRIBUTE_THERMAL_DAMAGE], 0) * multiplier),
        kinetic: roundNumber(toFiniteNumber(moduleAttributes && moduleAttributes[ATTRIBUTE_KINETIC_DAMAGE], 0) * multiplier),
        explosive: roundNumber(toFiniteNumber(moduleAttributes && moduleAttributes[ATTRIBUTE_EXPLOSIVE_DAMAGE], 0) * multiplier),
    };
}
function resolveSmartbombActivation({ entity, moduleItem, effectRecord, shipItem, skillMap = null, fittedItems = null, activeModuleContexts = null, additionalLocationModifierSources = null, } = {}) {
    ...
    const moduleAttributes = buildLiveModuleAttributeMap(shipItem, moduleItem, null, skillMap, fittedItems, activeModuleContexts, {
...
function executeSmartbombCycle({ scene, entity, moduleItem, effectState, nowMs, callbacks = {}, } = {}) {
    ...
    const damageVector = effectState.smartbombDamage && typeof effectState.smartbombDamage === "object"
        ? effectState.smartbombDamage
        : {};
    const candidates = callbacks.listDamageableEntitiesInSmartbombRange &&
    ...
    for (const targetEntity of targets) {
        ...
        const damageResult = callbacks.applySmartbombDamage
            ? callbacks.applySmartbombDamage(entity, targetEntity, moduleItem, damageVector, nowMs)
            : null;
...
//# sourceMappingURL=smartbombRuntime.js.map