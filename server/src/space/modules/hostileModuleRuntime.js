...
const ATTRIBUTE_WARP_SCRAMBLE_STATUS = getAttributeIDByNames("warpScrambleStatus") || 104;
const ATTRIBUTE_WARP_SCRAMBLE_STRENGTH = getAttributeIDByNames("warpScrambleStrength") || 105;
...
const ATTRIBUTE_ENERGY_NEUTRALIZER_SIGNATURE_RESOLUTION = getAttributeIDByNames("energyNeutralizerSignatureResolution") || 2451;
...
const HOSTILE_EFFECT_DEFINITIONS_BY_NAME = Object.freeze({
    ...
    fighterabilitywarpdisruption: BASE_HOSTILE_DEFINITIONS.disrupt,
    fighterabilityenergyneutralizer: BASE_HOSTILE_DEFINITIONS.neut,
    fighterabilitystasiswebifier: BASE_HOSTILE_DEFINITIONS.web,
...
function isCloakedTargetEntity(entity) {
    return toInt(entity && (entity.isCloaked ??
        entity.cloakMode ??
        entity.cloaked), 0) > 0;
}
...
function resolveTargetResistanceMultiplier(targetEntity, resistanceAttributeID) {
    ...
    return roundNumber(clamp(getEntityPassiveAttributeValue(targetEntity, resistanceAttributeID, 1), 0, 1), 6);
}
function resolveHostileRangeMultiplier(effectState, sourceEntity, targetEntity, callbacks = {}) {
    ...
    const optimalRangeMeters = Math.max(0, toFiniteNumber(effectState && effectState.hostileMaxRangeMeters, 0));
    const falloffMeters = Math.max(0, toFiniteNumber(effectState && effectState.hostileFalloffMeters, 0));
...
function resolveEnergyWarfareMultiplier(effectState, targetEntity, rangeMultiplier) {
    const resistanceMultiplier = resolveTargetResistanceMultiplier(targetEntity, effectState && effectState.hostileResistanceAttributeID);
    const signatureResolution = Math.max(0, toFiniteNumber(effectState && effectState.hostileEnergySignatureResolution, 0));
    ...
    const signatureMultiplier = signatureResolution > 0 && targetSignatureRadius > 0
        ...
        : 1;
    return roundNumber(Math.max(rangeMultiplier, 0) *
        Math.max(resistanceMultiplier, 0) *
        Math.max(signatureMultiplier, 0), 6);
}
function buildHostileTargetRecord(sourceEntity, effectState, targetEntity, nowMs, rangeMultiplier) {
    return Object.freeze({
        key: buildTargetHostileStateKey(sourceEntity && sourceEntity.itemID, effectState && effectState.moduleID, targetEntity && targetEntity.itemID),
        ...
        moduleModifierEntries: Object.freeze((Array.isArray(effectState && effectState.hostileModuleModifierEntries)
            ? effectState.hostileModuleModifierEntries
            : []).map((entry) => Object.freeze({ ...entry }))),
        chargeModifierEntries: Object.freeze((Array.isArray(effectState && effectState.hostileChargeModifierEntries)
        ...
        energySignatureResolution: Math.max(0, roundNumber(toFiniteNumber(effectState && effectState.hostileEnergySignatureResolution, 0), 6)),
        scrambleStrength: roundNumber(toFiniteNumber(effectState && effectState.hostileWarpScrambleStrength, 0), 6),
...
function recomputeTargetAggregateState(targetEntity) {
    ...
    for (const record of targetState.incomingEffects.values()) {
        ...
        if (record.family === HOSTILE_FAMILY_WEB ||
            record.family === HOSTILE_FAMILY_PAINT) {
            const resistanceMultiplier = resolveTargetResistanceMultiplier(targetEntity, record.resistanceAttributeID);
            const value = roundNumber(record.modifierValue *
                Math.max(record.rangeMultiplier, 0) *
                Math.max(resistanceMultiplier, 0), 6);
        ...
        if (record.family === HOSTILE_FAMILY_SENSOR_DAMP) {
            ...
            for (const entry of record.moduleModifierEntries || []) {
                const value = roundNumber(toFiniteNumber(entry && entry.value, 0) * scaledMultiplier, 6);
                if (Math.abs(value) <= 1e-6) {
                    continue;
                }
                modifierEntries.push({
        ...
        if (record.family === HOSTILE_FAMILY_SCRAM ||
            record.family === HOSTILE_FAMILY_DISRUPT) {
            const value = roundNumber(record.scrambleStrength * Math.max(record.rangeMultiplier, 0), 6);
            if (Math.abs(value) > 1e-6) {
                modifierEntries.push({
        ...
        if (record.family === HOSTILE_FAMILY_TRACKING_DISRUPT ||
    ...
    const nextSignature = buildAggregateSignature(modifierEntries, warpScrambleStatus, microwarpdriveBlocked, microJumpDriveBlocked);
...
function removeTargetHostileRecord(targetEntity, sourceEntity, effectState) {
    ...
    const key = buildTargetHostileStateKey(sourceEntity && sourceEntity.itemID, effectState && effectState.moduleID, targetEntity && targetEntity.itemID);
...
function resolveHostileModuleActivation({ scene, entity, moduleItem, effectRecord, chargeItem = null, shipItem, skillMap = null, fittedItems = null, activeModuleContexts = null, options = {}, callbacks = {}, } = {}) {
    ...
    const targetEntity = callbacks.getEntityByID && typeof callbacks.getEntityByID === "function"
    ...
    const durationResolution = getPreferredAttributeValue(moduleAttributes, [
        effectRecord && effectRecord.durationAttributeID,
        ATTRIBUTE_DURATION,
        ATTRIBUTE_SPEED,
    ], 1000);
    const rangeResolution = getPreferredAttributeValue(moduleAttributes, [
        effectRecord && effectRecord.rangeAttributeID,
        ATTRIBUTE_MAX_RANGE,
    ], 0);
    const falloffResolution = getPreferredAttributeValue(moduleAttributes, [
        effectRecord && effectRecord.falloffAttributeID,
        ATTRIBUTE_FALLOFF_EFFECTIVENESS,
    ], 0);
    const dischargeResolution = getPreferredAttributeValue(moduleAttributes, [
        effectRecord && effectRecord.dischargeAttributeID,
        ATTRIBUTE_CAPACITOR_NEED,
    ], 0);
    const rawDurationMs = durationResolution.value;
    const durationAttributeID = durationResolution.attributeID > 0 ? durationResolution.attributeID : ATTRIBUTE_SPEED;
    const moduleModifierEntries = buildHostileModifierEntries(moduleAttributes, definition.moduleModifierSpecs, definition.stackingPenalized === true);
    const chargeModifierEntries = buildHostileModifierEntries(moduleAttributes, definition.chargeModifierSpecs, definition.stackingPenalized === true);
    const effectStatePatch = {
        ...
        hostileMaxRangeMeters: Math.max(0, roundNumber(rangeResolution.value, 3)),
        hostileFalloffMeters: Math.max(0, roundNumber(falloffResolution.value, 3)),
        hostileStrengthValue: roundNumber(toFiniteNumber(definition.strengthAttributeID > 0
        ...
        hostileWarpScrambleStrength: roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_WARP_SCRAMBLE_STRENGTH], 0), 6),
        hostileEnergySignatureResolution: Math.max(0, roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_ENERGY_NEUTRALIZER_SIGNATURE_RESOLUTION], 0), 6)),
        hostileNosOverride: toFiniteNumber(moduleAttributes[ATTRIBUTE_NOS_OVERRIDE], 0) > 0,
    ...
    const rangeResult = resolveHostileRangeMultiplier(effectStatePatch, entity, targetEntity, callbacks);
...
function applyHostileModuleState({ scene, sourceEntity, targetEntity, effectState, nowMs, } = {}) {
    ...
    const record = buildHostileTargetRecord(sourceEntity, effectState, targetEntity, nowMs, 1);
    const targetState = ensureTargetHostileState(targetEntity, true);
    const previousRecord = targetState.incomingEffects.get(record.key) || null;
    targetState.incomingEffects.set(record.key, record);
...
function refreshHostileModuleState({ scene, sourceEntity, targetEntity, effectState, nowMs, callbacks = {}, } = {}) {
    if (!scene || !sourceEntity || !targetEntity || !effectState) {
        return {
            success: false,
            errorMsg: "UNSUPPORTED_MODULE",
    ...
    const rangeResult = resolveHostileRangeMultiplier(effectState, sourceEntity, targetEntity, callbacks);
    if (!rangeResult.success) {
        return rangeResult;
    }
    const record = buildHostileTargetRecord(sourceEntity, effectState, targetEntity, nowMs, rangeResult.multiplier);
...
function removeHostileModuleState({ targetEntity, sourceEntity, effectState, } = {}) {
...
function executeHostileModuleCycle({ scene, session, entity, effectState, nowMs, callbacks = {}, } = {}) {
    ...
    if (effectState.hostileFamily === HOSTILE_FAMILY_NEUT) {
        const warfareMultiplier = resolveEnergyWarfareMultiplier(effectState, targetEntity, refreshResult.data.multiplier);
        const requestedAmount = Math.max(0, toFiniteNumber(effectState.hostileStrengthValue, 0)) * Math.max(warfareMultiplier, 0);
        const targetCapCapacity = Math.max(0, toFiniteNumber(targetEntity && targetEntity.capacitorCapacity, 0));
        ...
        if (targetCapCapacity > 0 && appliedAmount > 0 && callbacks.setEntityCapacitorRatio) {
            callbacks.setEntityCapacitorRatio(targetEntity, (previousTargetChargeAmount - appliedAmount) / targetCapCapacity);
            ...
            if (targetEntity.session && callbacks.notifyCapacitorChangeToSession) {
                callbacks.notifyCapacitorChangeToSession(targetEntity.session, targetEntity, nowMs, previousTargetChargeAmount);
            }
        }
        return {
            success: true,
    ...
    if (effectState.hostileFamily === HOSTILE_FAMILY_NOS) {
        ...
        const targetCapCapacity = Math.max(0, toFiniteNumber(targetEntity && targetEntity.capacitorCapacity, 0));
        const sourceCapCapacity = Math.max(0, toFiniteNumber(entity && entity.capacitorCapacity, 0));
        const previousTargetChargeAmount = callbacks.getEntityCapacitorAmount
            ? callbacks.getEntityCapacitorAmount(targetEntity)
            : 0;
        const previousSourceChargeAmount = callbacks.getEntityCapacitorAmount
            ...
            : 0;
        const requestedAmount = Math.max(0, toFiniteNumber(effectState.hostileStrengthValue, 0)) * Math.max(warfareMultiplier, 0);
        const transferableAmount = Math.max(0, Math.min(requestedAmount, previousTargetChargeAmount, Math.max(0, sourceCapCapacity - previousSourceChargeAmount)));
        if (transferableAmount > 0 &&
            callbacks.setEntityCapacitorRatio) {
            if (targetCapCapacity > 0) {
                callbacks.setEntityCapacitorRatio(targetEntity, (previousTargetChargeAmount - transferableAmount) / targetCapCapacity);
                if (callbacks.persistEntityCapacitorRatio) {
                    callbacks.persistEntityCapacitorRatio(targetEntity);
                }
                if (targetEntity.session && callbacks.notifyCapacitorChangeToSession) {
            ...
            if (sourceCapCapacity > 0) {
                callbacks.setEntityCapacitorRatio(entity, (previousSourceChargeAmount + transferableAmount) / sourceCapCapacity);
                if (callbacks.persistEntityCapacitorRatio) {
                    callbacks.persistEntityCapacitorRatio(entity);
                }
                if (session && callbacks.notifyCapacitorChangeToSession) {
                    callbacks.notifyCapacitorChangeToSession(session, entity, nowMs, previousSourceChargeAmount);
                }
            }
        }
        return {
...
//# sourceMappingURL=hostileModuleRuntime.js.map