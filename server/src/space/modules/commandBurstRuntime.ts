...
function freezePlainEntries(entries: any[] = [], extraPredicate = null) {
...
function normalizeDbuffCollectionEntry(entry, fallbackCollectionID: any = 0) {
...
function buildDbuffCollectionsByID() {
  ...
  for (const [collectionID, entry] of Object.entries<any>(rawCollections)) {
...
abstract class Aggregator {
  abstract _pickValue(...values: number[]): number;
  declare _continuousInput: any;
  declare _sortedInputs: any;
...
function buildCollectionValuesFromAttributes(attributes: Record<string, any> = {}) {
  const entries = [
    [
...
function collectDefinitionModifierEntries(destination, collectionDefinition, targetItem, outputValue) {
  ...
  const matchedGroupAttributes = new Set<any>();
  ...
  const matchedCategoryAttributes = new Set<any>();
  ...
  const matchedSkillAttributes = new Set<any>();
...
function buildClientDbuffStateEntriesInternal(entity, buildExpiry) {
  ...
  return [...buckets.entries()]
    .map(([collectionID, aggregator]): [number, [number, any]] | null => {
...
function buildClientDbuffStateEntries(entity, options: Record<string, any> = {}) {
