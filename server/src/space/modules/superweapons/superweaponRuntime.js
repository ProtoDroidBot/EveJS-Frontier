...
const { getAttributeIDByNames, getTypeAttributeMap, typeHasEffectName, } = require(path.join(__dirname, "../../../services/fitting/liveFittingState"));
const { resolveTitanSuperweaponProfileByModuleTypeID, } = require(path.join(__dirname, "../../../services/superweapons/superweaponCatalog"));
const { buildLiveModuleAttributeMap, } = require(path.join(__dirname, "../liveModuleAttributes"));
...
const warpDisruptFieldGeneratorRuntime = require(path.join(__dirname, "../../../services/ship/warpDisruptFieldGeneratorRuntime"));
const { getFuelStacksForShipStorage, getFuelQuantityFromStacks, consumeFuelFromShipStorage, } = require(path.join(__dirname, "../sharedFuelRuntime"));
const { hasDamageableHealth, sumDamageVector, } = require(path.join(__dirname, "../../combat/damage"));
const { applySuperweaponShowFollowMotionCommand, } = require(path.join(__dirname, "../../destiny/commands/superweapon.js"));
...
const ATTRIBUTE_DAMAGE_DELAY_DURATION = getAttributeIDByNames("damageDelayDuration") || 561;
const ATTRIBUTE_CONSUMPTION_TYPE = getAttributeIDByNames("consumptionType") || 713;
const ATTRIBUTE_CONSUMPTION_QUANTITY = getAttributeIDByNames("consumptionQuantity") || 714;
const ATTRIBUTE_DOOMSDAY_NO_JUMP_OR_CLOAK_DURATION = getAttributeIDByNames("doomsdayNoJumpOrCloakDuration") || 2142;
const ATTRIBUTE_DOOMSDAY_IMMOBILITY_DURATION = getAttributeIDByNames("doomsdayImmobilityDuration") || 2141;
const ATTRIBUTE_DOOMSDAY_WARNING_DURATION = getAttributeIDByNames("doomsdayWarningDuration") || 2143;
const ATTRIBUTE_DOOMSDAY_DAMAGE_DURATION = getAttributeIDByNames("doomsdayDamageDuration") || 2144;
const ATTRIBUTE_DOOMSDAY_DAMAGE_CYCLE_TIME = getAttributeIDByNames("doomsdayDamageCycleTime") || 2145;
const ATTRIBUTE_DOOMSDAY_DAMAGE_RADIUS = getAttributeIDByNames("doomsdayDamageRadius") || 2146;
const ATTRIBUTE_DOOMSDAY_AOE_SHAPE = getAttributeIDByNames("doomsdayAOEShape") || 2147;
const ATTRIBUTE_DOOMSDAY_AOE_RANGE = getAttributeIDByNames("doomsdayAOERange") || 2279;
const ATTRIBUTE_DOOMSDAY_AOE_DURATION = getAttributeIDByNames("doomsdayAOEDuration") || 2280;
const ATTRIBUTE_DOOMSDAY_AOE_SIGNATURE_RADIUS = getAttributeIDByNames("doomsdayAOESignatureRadius") || 2281;
const ATTRIBUTE_DOOMSDAY_RANGE_IS_FIXED = getAttributeIDByNames("doomsdayRangeIsFixed") || 2149;
const ATTRIBUTE_IS_POINT_TARGETED = getAttributeIDByNames("isPointTargeted") || 2210;
...
const ATTRIBUTE_SIGNATURE_RADIUS_BONUS = getAttributeIDByNames("signatureRadiusBonus") || 554;
const ATTRIBUTE_TARGET_PAINTER_RESISTANCE = getAttributeIDByNames("targetPainterResistance") || 2114;
const ATTRIBUTE_STASIS_WEBIFIER_RESISTANCE = getAttributeIDByNames("stasisWebifierResistance") || 2115;
const ATTRIBUTE_ENERGY_NEUTRALIZER_AMOUNT = getAttributeIDByNames("energyNeutralizerAmount") || 97;
const ATTRIBUTE_ENERGY_WARFARE_RESISTANCE = getAttributeIDByNames("energyWarfareResistance") || 2045;
const ATTRIBUTE_MAX_TARGET_RANGE_BONUS = getAttributeIDByNames("maxTargetRangeBonus") || 309;
const ATTRIBUTE_SCAN_RESOLUTION_BONUS = getAttributeIDByNames("scanResolutionBonus") || 566;
const ATTRIBUTE_SENSOR_DAMPENER_RESISTANCE = getAttributeIDByNames("sensorDampenerResistance") || 2112;
...
const ATTRIBUTE_TRACKING_SPEED_BONUS = getAttributeIDByNames("trackingSpeedBonus") || 767;
const ATTRIBUTE_MISSILE_VELOCITY_BONUS = getAttributeIDByNames("missileVelocityBonus") || 547;
const ATTRIBUTE_EXPLOSION_DELAY_BONUS = getAttributeIDByNames("explosionDelayBonus") || 596;
const ATTRIBUTE_AOE_VELOCITY_BONUS = getAttributeIDByNames("aoeVelocityBonus") || 847;
const ATTRIBUTE_AOE_CLOUD_SIZE_BONUS = getAttributeIDByNames("aoeCloudSizeBonus") || 848;
...
const ATTRIBUTE_WEAPON_DISRUPTION_RESISTANCE = getAttributeIDByNames("weaponDisruptionResistance") || 2113;
const ATTRIBUTE_SCAN_GRAVIMETRIC_STRENGTH_BONUS = getAttributeIDByNames("scanGravimetricStrengthBonus") || 238;
const ATTRIBUTE_SCAN_LADAR_STRENGTH_BONUS = getAttributeIDByNames("scanLadarStrengthBonus") || 239;
const ATTRIBUTE_SCAN_MAGNETOMETRIC_STRENGTH_BONUS = getAttributeIDByNames("scanMagnetometricStrengthBonus") || 240;
const ATTRIBUTE_SCAN_RADAR_STRENGTH_BONUS = getAttributeIDByNames("scanRadarStrengthBonus") || 241;
const ATTRIBUTE_DOOMSDAY_ENERGY_NEUT_AMOUNT = getAttributeIDByNames("doomsdayEnergyNeutAmount") || 2148;
const ATTRIBUTE_DOOMSDAY_ENERGY_NEUT_RADIUS = getAttributeIDByNames("doomsdayEnergyNeutRadius") || 2151;
const ATTRIBUTE_DOOMSDAY_ENERGY_NEUT_SIGNATURE_RADIUS = getAttributeIDByNames("doomsdayEnergyNeutSignatureRadius") || 2152;
const ATTRIBUTE_LIGHTNING_WEAPON_TARGET_AMOUNT = getAttributeIDByNames("lightningWeaponTargetAmount") || 2104;
const ATTRIBUTE_LIGHTNING_WEAPON_TARGET_RANGE = getAttributeIDByNames("lightningWeaponTargetRange") || 2105;
const ATTRIBUTE_LIGHTNING_WEAPON_DAMAGE_LOSS_TARGET = getAttributeIDByNames("lightningWeaponDamageLossTarget") || 2106;
const ATTRIBUTE_ENTITY_SUPERWEAPON_DURATION = getAttributeIDByNames("entitySuperWeaponDuration") || 2009;
const ATTRIBUTE_ENTITY_SUPERWEAPON_EM_DAMAGE = getAttributeIDByNames("entitySuperWeaponEmDamage") || 2010;
const ATTRIBUTE_ENTITY_SUPERWEAPON_KINETIC_DAMAGE = getAttributeIDByNames("entitySuperWeaponKineticDamage") || 2011;
const ATTRIBUTE_ENTITY_SUPERWEAPON_THERMAL_DAMAGE = getAttributeIDByNames("entitySuperWeaponThermalDamage") || 2012;
const ATTRIBUTE_ENTITY_SUPERWEAPON_EXPLOSIVE_DAMAGE = getAttributeIDByNames("entitySuperWeaponExplosiveDamage") || 2013;
const ATTRIBUTE_ENTITY_SUPERWEAPON_MAX_RANGE = getAttributeIDByNames("entitySuperWeaponMaxRange") || 2046;
const ATTRIBUTE_ENTITY_SUPERWEAPON_FALLOFF = getAttributeIDByNames("entitySuperWeaponFallOff") || 2047;
const ATTRIBUTE_ENTITY_SUPERWEAPON_TRACKING_SPEED = getAttributeIDByNames("entitySuperWeaponTrackingSpeed") || 2048;
const ATTRIBUTE_ENTITY_SUPERWEAPON_OPTIMAL_SIGNATURE_RADIUS = getAttributeIDByNames("entitySuperWeaponOptimalSignatureRadius") || 2049;
...
function resolveTargetResistanceMultiplier(targetEntity, resistanceAttributeID) {
    ...
    const resistancePercent = clamp(getEntityPassiveAttributeValue(targetEntity, resistanceAttributeID, 0), 0, 100);
...
function dotProduct(left, right) {
    return (toFiniteNumber(left && left.x, 0) * toFiniteNumber(right && right.x, 0) +
...
function normalizeDamageVector(rawDamage = {}) {
    const source = rawDamage && typeof rawDamage === "object"
        ? rawDamage
        : {};
    return {
...
function isStructureBurstProjector(moduleItem) {
    ...
    return (toInt(moduleItem && moduleItem.groupID, 0) === STRUCTURE_BURST_PROJECTOR_GROUP_ID &&
        STRUCTURE_BURST_PROJECTOR_EFFECT_NAMES.some((effectName) => (typeHasEffectName(typeID, effectName))));
}
function isStructureDoomsdayWeapon(moduleItem) {
    const typeID = toInt(moduleItem && moduleItem.typeID, 0);
    return (toInt(moduleItem && moduleItem.groupID, 0) === STRUCTURE_DOOMSDAY_WEAPON_GROUP_ID &&
...
function resolveSupportedSuperweapon(moduleItem) {
    if (moduleItem &&
        ...
        typeHasEffectName(toInt(moduleItem.typeID, 0), String(moduleItem.npcEffectName || "").trim() || "entitySuperWeapon")) {
    ...
    const profile = resolveTitanSuperweaponProfileByModuleTypeID(toInt(moduleItem && moduleItem.typeID, 0));
    if (!profile) {
        return null;
    }
...
function resolveSkillMap(entity, fallbackCharacterID = 0) {
    ...
    const characterID = entity && (entity.pilotCharacterID ??
        entity.characterID)
        ? toInt(entity.pilotCharacterID ??
            entity.characterID, fallbackCharacterID)
        : fallbackCharacterID;
    return characterID > 0 ? getCachedCharacterSkillMap(characterID) : new Map();
}
function buildSuperweaponDogmaState(options = {}) {
    const { entity, shipItem, moduleItem, callbacks = {}, supported = null, } = options;
    ...
    if (supported && supported.entitySuperweapon === true) {
        ...
        const durationMs = Math.max(1, toFiniteNumber(attributeMap[ATTRIBUTE_ENTITY_SUPERWEAPON_DURATION], 1));
        return {
            ...
            signatureRadius: Math.max(1, toFiniteNumber(attributeMap[ATTRIBUTE_ENTITY_SUPERWEAPON_OPTIMAL_SIGNATURE_RADIUS], 1)),
            maxRange: Math.max(0, toFiniteNumber(attributeMap[ATTRIBUTE_ENTITY_SUPERWEAPON_MAX_RANGE], 0)),
            falloff: Math.max(0, toFiniteNumber(attributeMap[ATTRIBUTE_ENTITY_SUPERWEAPON_FALLOFF], 0)),
            trackingSpeed: Math.max(0, toFiniteNumber(attributeMap[ATTRIBUTE_ENTITY_SUPERWEAPON_TRACKING_SPEED], 0)),
            damageDelayMs: Math.max(0, toFiniteNumber(attributeMap[ATTRIBUTE_DAMAGE_DELAY_DURATION], durationMs)),
            fuelTypeID: 0,
            fuelPerActivation: 0,
    ...
    const characterID = callbacks.resolveCharacterID &&
    ...
    const fittedItems = callbacks.getEntityRuntimeFittedItems &&
        ...
        : [];
    const activeModuleContexts = callbacks.getEntityRuntimeActiveModuleContexts &&
        ...
        : [];
    const attributes = buildLiveModuleAttributeMap(shipItem, moduleItem, null, skillMap, fittedItems, activeModuleContexts);
    if (!attributes) {
        return null;
    }
    if (supported && supported.structureBurstProjector === true) {
        const warningDurationMs = Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_DOOMSDAY_WARNING_DURATION], 0));
        const aoeDurationMs = Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_DOOMSDAY_AOE_DURATION], 0));
        const explicitDurationMs = Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_DURATION], 0));
        const durationMs = Math.max(1, explicitDurationMs || warningDurationMs + aoeDurationMs || 1);
        const aoeRange = Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_DOOMSDAY_AOE_RANGE], 0));
        return {
            attributes,
            ...
            signatureRadius: Math.max(1, toFiniteNumber(attributes[ATTRIBUTE_SIGNATURE_RADIUS], 1)),
            maxRange: Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_MAX_RANGE], 0)),
            damageDelayMs: Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_DAMAGE_DELAY_DURATION], 0)),
            fuelTypeID: 0,
            fuelPerActivation: 0,
            noJumpOrCloakDurationMs: 0,
            immobilityDurationMs: 0,
            ...
            aoeRange,
            aoeDurationMs,
            aoeSignatureRadius: Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_DOOMSDAY_AOE_SIGNATURE_RADIUS], 0)),
            ...
            energyNeutRadius: 0,
            energyNeutSignatureRadius: 0,
            burstProjectorHostileDefinition: resolveStructureBurstProjectorHostileDefinition(moduleItem),
            burstProjectorJammerDefinition: resolveStructureBurstProjectorJammerDefinition(moduleItem),
            burstProjectorWarpDisruptionDefinition: resolveStructureBurstProjectorWarpDisruptionDefinition(moduleItem),
        };
    }
    return {
        attributes,
        ...
        fuelPerActivation: Math.max(0, toInt(attributes[ATTRIBUTE_CONSUMPTION_QUANTITY], 0)),
        noJumpOrCloakDurationMs: Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_DOOMSDAY_NO_JUMP_OR_CLOAK_DURATION], 0)),
        immobilityDurationMs: Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_DOOMSDAY_IMMOBILITY_DURATION], 0)),
        warningDurationMs: Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_DOOMSDAY_WARNING_DURATION], 0)),
        damageDurationMs: Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_DOOMSDAY_DAMAGE_DURATION], 0)),
        damageCycleTimeMs: Math.max(1, toFiniteNumber(attributes[ATTRIBUTE_DOOMSDAY_DAMAGE_CYCLE_TIME], 1000)),
        damageRadius: Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_DOOMSDAY_DAMAGE_RADIUS], 0)),
        aoeShape: toInt(attributes[ATTRIBUTE_DOOMSDAY_AOE_SHAPE], 0),
        aoeRange: Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_DOOMSDAY_AOE_RANGE], 0)),
        aoeDurationMs: Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_DOOMSDAY_AOE_DURATION], 0)),
        aoeSignatureRadius: Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_DOOMSDAY_AOE_SIGNATURE_RADIUS], 0)),
        rangeIsFixed: toInt(attributes[ATTRIBUTE_DOOMSDAY_RANGE_IS_FIXED], 0) === 1,
        isPointTargeted: toInt(attributes[ATTRIBUTE_IS_POINT_TARGETED], 0) >= 1,
        energyNeutAmount: Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_DOOMSDAY_ENERGY_NEUT_AMOUNT], 0)),
        energyNeutRadius: Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_DOOMSDAY_ENERGY_NEUT_RADIUS], 0)),
        energyNeutSignatureRadius: Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_DOOMSDAY_ENERGY_NEUT_SIGNATURE_RADIUS], 0)),
        lightningTargetAmount: Math.max(0, toInt(attributes[ATTRIBUTE_LIGHTNING_WEAPON_TARGET_AMOUNT], 0)),
        lightningTargetRange: Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_LIGHTNING_WEAPON_TARGET_RANGE], 0)),
        lightningDamageLossTarget: Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_LIGHTNING_WEAPON_DAMAGE_LOSS_TARGET], 0)),
    };
}
...
function clampPointToFixedRange(sourceEntity, point, maxRange) {
    ...
    const normalizedDirection = normalizeVector(magnitude(offset) > 1e-9 ? offset : sourceEntity && sourceEntity.direction, { x: 1, y: 0, z: 0 });
    return addVectors(sourcePosition, scaleVector(normalizedDirection, Math.max(0, toFiniteNumber(maxRange, 0))));
}
function allocateRuntimeEntityID(callbacks = {}, preferredItemID = null) {
    if (callbacks.allocateRuntimeEntityID &&
...
function allocateSuperweaponBeaconID(scene, sourceEntity, nowMs, callbacks = {}) {
    ...
    const baseID = TRANSIENT_SUPERWEAPON_ENTITY_ID_BASE +
...
function spawnLanceBeacon(scene, sourceEntity, targetPoint, nowMs, callbacks = {}) {
    ...
    const beaconEntity = {
        ...
        direction: normalizeVector(subtractVectors(resolvedTargetPoint, sourcePosition), sourceEntity && sourceEntity.direction),
...
function resolveSuperweaponFxTargetID(effectState) {
    ...
    if (String(effectState.superweaponFamily || "").toLowerCase() === "lance") {
        return Math.max(0, toInt(effectState.superweaponFxTargetID, effectState.superweaponBeaconID));
    }
    return Math.max(0, toInt(effectState.superweaponPrimaryTargetID, effectState.targetID));
}
function resolveSuperweaponFxReplayWindowEndMs(effectState) {
    if (!effectState || effectState.superweaponEffect !== true) {
        return 0;
    }
    const activatedAtMs = Math.max(0, toFiniteNumber(effectState.superweaponActivatedAtMs, effectState.startedAtMs));
    if (activatedAtMs <= 0) {
        return 0;
    }
...
function resolveSuperweaponFxStartFileTime(scene, whenMs = Date.now()) {
    if (scene &&
        ...
        const fallback = typeof scene.getCurrentFileTime === "function"
...
function getEntitySignatureRadius(entity) {
    return Math.max(0, toFiniteNumber(entity && entity.signatureRadius, 0));
}
...
function collectPotentialBurstProjectorTargets(scene) {
    return collectPotentialCylinderTargets(scene).filter((entity) => (entity &&
...
function collectLightningChainTargets(scene, sourceEntity, primaryTargetEntity, effectState) {
    ...
    const maxTargetCount = Math.max(1, toInt(effectState && effectState.superweaponLightningTargetAmount, 1));
    const jumpRange = Math.max(0, toFiniteNumber(effectState && effectState.superweaponLightningTargetRange, 0));
...
function isEntityInsideCylinder(origin, direction, length, radius, entity) {
    ...
    const closestPoint = addVectors(sourcePosition, scaleVector(axis, clamp(along, 0, Math.max(0, toFiniteNumber(length, 0)))));
...
function hasBurstProjectorHostileDefinition(effectState) {
    const hasStrengthOnlyEffect = String(effectState && effectState.superweaponBurstProjectorHostileFamily || "") ===
        ...
        Math.abs(toFiniteNumber(effectState && effectState.superweaponBurstProjectorStrengthValue, 0)) >
            1e-6;
    const hasDirectModifier = toInt(effectState && effectState.superweaponBurstProjectorModifierAttributeID, 0) > 0 &&
        toInt(effectState && effectState.superweaponBurstProjectorModifierOperation, -1) >= 0 &&
        Math.abs(toFiniteNumber(effectState && effectState.superweaponBurstProjectorStrengthValue, 0)) >
            1e-6;
    const hasModuleModifierEntries = Array.isArray(effectState && effectState.superweaponBurstProjectorModuleModifierEntries) &&
        effectState.superweaponBurstProjectorModuleModifierEntries.length > 0;
    const hasChargeModifierEntries = Array.isArray(effectState && effectState.superweaponBurstProjectorChargeModifierEntries) &&
        effectState.superweaponBurstProjectorChargeModifierEntries.length > 0;
    return Boolean(effectState &&
...
function hasBurstProjectorDebuffDefinition(effectState) {
    return (hasBurstProjectorHostileDefinition(effectState) ||
...
function getBurstProjectorAOEWindowDurationMs(effectState) {
    const configuredDurationMs = Math.max(0, toFiniteNumber(effectState && effectState.superweaponAOEDurationMs, effectState && effectState.superweaponDamageDurationMs));
...
function configureBurstProjectorWarpDisruptionBeacon(beaconEntity, sourceEntity, effectState, moduleItem) {
    if (!beaconEntity ||
        !sourceEntity ||
        !effectState ||
    ...
    const fieldRange = Math.max(0, toFiniteNumber(effectState.superweaponBurstProjectorWarpDisruptionRange, 0));
    const windowStartMs = Math.max(0, toFiniteNumber(effectState.superweaponDamageWindowStartMs, 0));
    const windowEndMs = Math.max(0, toFiniteNumber(effectState.superweaponDamageWindowEndMs, 0));
    ...
    beaconEntity.warpDisruptionSource = String(effectState.superweaponBurstProjectorWarpDisruptionSource || "superweaponBurstProjector");
    beaconEntity.sourceShipID = toInt(sourceEntity.itemID, 0);
    beaconEntity.sourceModuleID = toInt(effectState.moduleID, 0);
    beaconEntity.ownerID = toInt(sourceEntity.ownerID, toInt(sourceEntity.corporationID, 0));
...
function buildBurstProjectorJammerStrengthBySensorType(attributeMap, definition) {
    const strengthAttributeIDsBySensorType = definition &&
        definition.strengthAttributeIDsBySensorType &&
        typeof definition.strengthAttributeIDsBySensorType === "object"
        ? definition.strengthAttributeIDsBySensorType
        : {};
    return Object.freeze({
        gravimetric: roundNumber(toFiniteNumber(attributeMap && attributeMap[strengthAttributeIDsBySensorType.gravimetric], 0), 6),
        ladar: roundNumber(toFiniteNumber(attributeMap && attributeMap[strengthAttributeIDsBySensorType.ladar], 0), 6),
        magnetometric: roundNumber(toFiniteNumber(attributeMap && attributeMap[strengthAttributeIDsBySensorType.magnetometric], 0), 6),
        radar: roundNumber(toFiniteNumber(attributeMap && attributeMap[strengthAttributeIDsBySensorType.radar], 0), 6),
    });
}
function buildBurstProjectorHostileEffectState(effectState, targetEntity, nowMs, durationMs) {
    return {
        ...
        hostileStrengthValue: roundNumber(toFiniteNumber(effectState.superweaponBurstProjectorStrengthValue, 0), 6),
        hostileModifierAttributeID: Math.max(0, toInt(effectState.superweaponBurstProjectorModifierAttributeID, 0)),
        hostileModifierOperation: toInt(effectState.superweaponBurstProjectorModifierOperation, -1),
        hostileModuleModifierEntries: Object.freeze((Array.isArray(effectState.superweaponBurstProjectorModuleModifierEntries)
            ? effectState.superweaponBurstProjectorModuleModifierEntries
            : []).map((entry) => Object.freeze({ ...entry }))),
        hostileChargeModifierEntries: Object.freeze((Array.isArray(effectState.superweaponBurstProjectorChargeModifierEntries)
            ? effectState.superweaponBurstProjectorChargeModifierEntries
            : []).map((entry) => Object.freeze({ ...entry }))),
        hostileResistanceAttributeID: Math.max(0, toInt(effectState.superweaponBurstProjectorResistanceAttributeID, 0)),
        hostileWarpScrambleStrength: 0,
        hostileEnergySignatureResolution: Math.max(0, roundNumber(toFiniteNumber(effectState.superweaponBurstProjectorEnergySignatureResolution, 0), 6)),
        hostileNosOverride: false,
        hostileStackingPenalized: effectState.superweaponBurstProjectorStackingPenalized === true,
        hostileAffectsTargetDerivedState: effectState.superweaponBurstProjectorAffectsTargetDerivedState === true,
...
function buildBurstProjectorJammerEffectState(effectState, targetEntity, nowMs, durationMs) {
    ...
    return {
        ...
        jammerMaxStrength: roundNumber(toFiniteNumber(effectState.superweaponBurstProjectorJammerMaxStrength, 0), 6),
    };
}
function buildNpcPseudoSession(entity) {
    return {
        characterID: toInt(entity && (entity.pilotCharacterID ??
...
function getFittedModuleByTypeID(entity, typeID) {
    ...
    return entity.fittedItems.find((moduleItem) => toInt(moduleItem && moduleItem.typeID, 0) === toInt(typeID, 0)) || null;
}
...
function setEntityMotionTowardTarget(entity, targetEntity, callbacks = {}) {
    const applied = applySuperweaponShowFollowMotionCommand({
        entity,
        targetEntity,
        toInt,
        cloneVector,
        subtractVectors,
        normalizeVector,
        followRange: DEFAULT_SHOW_APPROACH_RANGE,
    });
    if (!applied) {
        return false;
    }
...
function applyCapacitorDrain(entity, drainAmount, whenMs, callbacks = {}) {
    ...
    const currentCapacitor = callbacks.getEntityCapacitorAmount &&
    ...
    if (entity.session &&
        ...
        callbacks.notifyCapacitorChangeToSession(entity.session, entity, whenMs, currentCapacitor);
    }
...
function applySuperweaponDamage(scene, sourceEntity, targetEntity, damageVector, moduleItem, whenMs, callbacks = {}) {
    ...
    const weaponDamageResult = callbacks.applyWeaponDamageToTarget(scene, sourceEntity, targetEntity, damageVector, whenMs, {
    ...
    const appliedDamageAmount = callbacks.getAppliedDamageAmount &&
    ...
    if (weaponDamageResult.destroyResult &&
...
function prepareSuperweaponActivation(options = {}) {
    const { scene, entity, shipItem, moduleItem, callbacks = {}, baseRuntimeAttributes = {}, options: activationOptions = {}, } = options;
    ...
    if (supported.family === "doomsday") {
        ...
        const surfaceDistance = scene.getEntitySurfaceDistance &&
        ...
        if (dogmaState.rangeIsFixed && dogmaState.maxRange > 0) {
            targetPoint = clampPointToFixedRange(entity, targetPoint, dogmaState.maxRange);
        }
        else if (dogmaState.maxRange > 0 &&
    ...
    const fuelTypeID = Math.max(0, toInt(dogmaState.fuelTypeID, supported.fuelTypeID));
    const fuelPerActivation = Math.max(0, toInt(dogmaState.fuelPerActivation, supported.fuelPerActivation));
    if (fuelTypeID > 0 && fuelPerActivation > 0) {
        const availableFuel = getFuelQuantityFromStacks(getCargoFuelStacks(entity, fuelTypeID, callbacks));
    ...
    const burstProjectorModuleModifierEntries = burstProjectorHostileDefinition
        ? buildBurstProjectorModifierEntries(dogmaState.attributes, burstProjectorHostileDefinition.moduleModifierSpecs, burstProjectorStackingPenalized)
        : Object.freeze([]);
    const burstProjectorChargeModifierEntries = burstProjectorHostileDefinition
        ? buildBurstProjectorModifierEntries(dogmaState.attributes, burstProjectorHostileDefinition.chargeModifierSpecs, burstProjectorStackingPenalized)
        : Object.freeze([]);
    const burstProjectorJammerStrengthBySensorType = burstProjectorJammerDefinition
        ? buildBurstProjectorJammerStrengthBySensorType(dogmaState.attributes, burstProjectorJammerDefinition)
        : Object.freeze({});
    const burstProjectorJammerMaxStrength = Math.max(0, ...Object.values(burstProjectorJammerStrengthBySensorType)
        .map((value) => toFiniteNumber(value, 0)));
    const activationCapNeed = supported.structureBurstProjector === true ||
    ...
    return {
        ...
        effectStatePatch: {
            ...
            superweaponBurstProjectorHostileEffectName: burstProjectorHostileDefinition
                ? String(burstProjectorHostileDefinition.effectName || "")
                : "",
            superweaponBurstProjectorHostileFamily: burstProjectorHostileDefinition
                ? String(burstProjectorHostileDefinition.family || "")
                : "",
            superweaponBurstProjectorJammingType: burstProjectorHostileDefinition
                ...
                : burstProjectorJammerDefinition
                    ...
                    : "",
            superweaponBurstProjectorStrengthValue: burstProjectorHostileDefinition
                ? roundNumber(toFiniteNumber(dogmaState.attributes[burstProjectorHostileDefinition.strengthAttributeID], 0), 6)
                : 0,
            superweaponBurstProjectorModifierAttributeID: burstProjectorHostileDefinition
                ? Math.max(0, toInt(burstProjectorHostileDefinition.modifierAttributeID, 0))
                : 0,
            superweaponBurstProjectorModifierOperation: burstProjectorHostileDefinition
                ? toInt(burstProjectorHostileDefinition.modifierOperation, -1)
                : -1,
            superweaponBurstProjectorModuleModifierEntries: burstProjectorModuleModifierEntries,
            superweaponBurstProjectorChargeModifierEntries: burstProjectorChargeModifierEntries,
            superweaponBurstProjectorResistanceAttributeID: burstProjectorHostileDefinition
                ? Math.max(0, toInt(burstProjectorHostileDefinition.resistanceAttributeID, 0))
                : 0,
            superweaponBurstProjectorEnergySignatureResolution: burstProjectorHostileDefinition
                ? Math.max(0, roundNumber(toFiniteNumber(dogmaState.attributes[burstProjectorHostileDefinition.signatureResolutionAttributeID], 0), 6))
                : 0,
            superweaponBurstProjectorInstantPulse: burstProjectorHostileDefinition &&
                burstProjectorHostileDefinition.instantPulse === true,
            superweaponBurstProjectorStackingPenalized: burstProjectorHostileDefinition &&
                burstProjectorHostileDefinition.stackingPenalized === true,
            superweaponBurstProjectorAffectsTargetDerivedState: burstProjectorHostileDefinition &&
                burstProjectorHostileDefinition.affectsTargetDerivedState === true,
            superweaponBurstProjectorJammerEffectName: burstProjectorJammerDefinition
                ? String(burstProjectorJammerDefinition.effectName || "")
                : "",
            superweaponBurstProjectorJammerFamily: burstProjectorJammerDefinition
                ? String(burstProjectorJammerDefinition.family || "")
                : "",
            superweaponBurstProjectorJammerJammingType: burstProjectorJammerDefinition
                ? String(burstProjectorJammerDefinition.jammingType || "")
                : "",
            superweaponBurstProjectorJammerStrengthBySensorType: burstProjectorJammerStrengthBySensorType,
            superweaponBurstProjectorJammerMaxStrength: roundNumber(burstProjectorJammerMaxStrength, 6),
            superweaponBurstProjectorWarpDisruptionEffectName: burstProjectorWarpDisruptionDefinition
                ? String(burstProjectorWarpDisruptionDefinition.effectName || "")
                : "",
            superweaponBurstProjectorWarpDisruptionEffectID: burstProjectorWarpDisruptionDefinition
                ? toInt(burstProjectorWarpDisruptionDefinition.effectID, 0)
                : 0,
            superweaponBurstProjectorWarpDisruptionSource: burstProjectorWarpDisruptionDefinition
                ? String(burstProjectorWarpDisruptionDefinition.source || "")
                : "",
            superweaponBurstProjectorWarpDisruptionRange: burstProjectorWarpDisruptionDefinition
            ...
            superweaponLightningTargetAmount: Math.max(0, toInt(dogmaState.lightningTargetAmount, 0)),
            superweaponLightningTargetRange: Math.max(0, toFiniteNumber(dogmaState.lightningTargetRange, 0)),
            superweaponLightningDamageLossTarget: Math.max(0, toFiniteNumber(dogmaState.lightningDamageLossTarget, 0)),
            ...
            superweaponPrimaryTargetID: supported.family === "doomsday"
...
function inspectSuperweaponActivationContract(options = {}) {
    const { scene, entity, moduleItem, callbacks = {}, targetID = 0, targetPoint = null, } = options;
    ...
    const inspectionCallbacks = {
        ...
        resolveCharacterID(currentEntity) {
            return toInt(currentEntity &&
                (currentEntity.pilotCharacterID ??
                    currentEntity.characterID), 0);
        },
        ...callbacks,
    };
    ...
    if (!activationResult || activationResult.matched !== true || activationResult.success !== true) {
        return {
            success: false,
            errorMsg: activationResult && activationResult.matched === true
    ...
    return {
        ...
        data: {
            ...
            aoeSignatureRadius: Math.max(0, toFiniteNumber(effectStatePatch.superweaponAOESignatureRadius, 0)),
            ...
            targetPoint: effectStatePatch.superweaponTargetPoint
                ...
                : null,
            lightningTargetAmount: Math.max(0, toInt(effectStatePatch.superweaponLightningTargetAmount, 0)),
            lightningTargetRange: Math.max(0, toFiniteNumber(effectStatePatch.superweaponLightningTargetRange, 0)),
            lightningDamageLossTarget: Math.max(0, toFiniteNumber(effectStatePatch.superweaponLightningDamageLossTarget, 0)),
        },
    };
}
function broadcastSuperweaponFx(scene, sourceEntity, effectState, targetID, nowMs, options = {}) {
    ...
    scene.broadcastSpecialFx(sourceEntity.itemID, effectState.guid, {
        ...baseFxOptions,
        ...(options && typeof options === "object" ? options : {}),
    }, sourceEntity);
    return true;
}
function broadcastLanceSuperweaponFxAfterBeaconAcquire(scene, sourceEntity, effectState, beaconEntity, nowMs) {
    if (!scene ||
        !sourceEntity ||
        !effectState ||
    ...
    for (const delivery of beaconDeliveries) {
        ...
        const sourceSlimCategoryID = toInt(sourceEntity && sourceEntity.slimCategoryID, sourceCategoryID);
        const useSourceShipModuleBinding = Boolean(sourceEntity &&
        ...
        const fxResult = scene.sendSpecialFxToSession(delivery.session, sourceEntity.itemID, effectState.guid, {
            ...
            stampOverride: delivery.stamp !== null && delivery.stamp !== undefined
                ? delivery.stamp
                : undefined,
...
function executeSuperweaponActivation(options = {}) {
    const { scene, entity, moduleItem, effectState, nowMs = Date.now(), callbacks = {}, } = options;
    ...
    const consumeFuelResult = consumeSuperweaponFuel(entity, effectState.superweaponFuelTypeID, effectState.superweaponFuelPerActivation, callbacks);
    if (!consumeFuelResult.success) {
        return consumeFuelResult;
    }
    if (callbacks.stopShipEntity &&
    ...
    entity.superweaponImmobileUntilMs = Math.max(toFiniteNumber(entity.superweaponImmobileUntilMs, 0), nowMs + Math.max(0, toFiniteNumber(effectState.superweaponImmobilityDurationMs, 0)));
    entity.superweaponNoJumpOrCloakUntilMs = Math.max(toFiniteNumber(entity.superweaponNoJumpOrCloakUntilMs, 0), nowMs + Math.max(0, toFiniteNumber(effectState.superweaponNoJumpOrCloakDurationMs, 0)));
    if (callbacks.persistDynamicEntity &&
        typeof callbacks.persistDynamicEntity === "function") {
        callbacks.persistDynamicEntity(entity);
    }
    ...
    if (effectState.superweaponFamily === "doomsday") {
        ...
        effectState.superweaponDamageAtMs = nowMs + Math.max(0, toFiniteNumber(effectState.superweaponDamageDelayMs, 0));
        broadcastSuperweaponFx(scene, entity, effectState, effectState.superweaponPrimaryTargetID, nowMs);
        return {
            success: true,
            data: {
    ...
    if (effectState.superweaponFamily === "burstProjector") {
        const targetPoint = cloneVector(effectState.superweaponTargetPoint, cloneVector(entity.position));
        const sourcePosition = cloneVector(entity.position);
        const direction = normalizeVector(subtractVectors(targetPoint, sourcePosition), entity.direction);
        ...
        configureBurstProjectorWarpDisruptionBeacon(beaconEntity, entity, effectState, moduleItem);
        if (effectState.guid) {
            broadcastLanceSuperweaponFxAfterBeaconAcquire(scene, entity, effectState, beaconEntity, nowMs);
        }
    ...
    const targetPoint = cloneVector(effectState.superweaponTargetPoint, clampPointToFixedRange(entity, addVectors(entity.position, scaleVector(entity.direction, effectState.superweaponMaxRange)), effectState.superweaponMaxRange));
    const sourcePosition = cloneVector(entity.position);
    const direction = normalizeVector(subtractVectors(targetPoint, sourcePosition), entity.direction);
    const beaconEntity = spawnLanceBeacon(scene, entity, targetPoint, nowMs, callbacks);
    if (!beaconEntity) {
...
function finalizeSuperweaponDeactivation(options = {}) {
    const { scene, entity, effectState, nowMs = Date.now(), callbacks = {}, } = options;
...
function tickDoomsdayEffect(scene, sourceEntity, effectState, moduleItem, nowMs, callbacks = {}) {
    ...
    const damageLossPerJump = Math.max(0, toFiniteNumber(effectState.superweaponLightningDamageLossTarget, 0));
    const chainTargets = collectLightningChainTargets(scene, sourceEntity, targetEntity, effectState);
    for (let index = 0; index < chainTargets.length; index += 1) {
        ...
        applySuperweaponDamage(scene, sourceEntity, chainTarget, scaleDamageVector(effectState.superweaponDamageVector, damageMultiplier), moduleItem, nowMs, callbacks);
    }
}
function tickLanceEffect(scene, sourceEntity, effectState, moduleItem, nowMs, callbacks = {}) {
    ...
    const sourcePosition = cloneVector(effectState.superweaponSourcePosition, sourceEntity.position);
    const direction = normalizeVector(effectState.superweaponDirection, sourceEntity.direction);
    ...
    for (let pulseIndex = previousPulseIndex + 1; pulseIndex <= latestPulseIndex; pulseIndex += 1) {
        ...
        for (const targetEntity of collectPotentialCylinderTargets(scene)) {
            const isInsideDamageArea = aoeShape === 3
                ? isEntityInsideCone(sourcePosition, direction, length, targetEntity)
                : isEntityInsideCylinder(sourcePosition, direction, length, damageRadius, targetEntity);
            if (!targetEntity ||
            ...
            const damageApplication = resolveSignatureApplicationFactor(targetEntity, effectState.superweaponWeaponSignatureRadius);
            if (damageApplication > 0) {
                applySuperweaponDamage(scene, sourceEntity, targetEntity, scaleDamageVector(effectState.superweaponDamageVector, damageApplication), moduleItem, pulseTimeMs, callbacks);
            }
            const neutRadius = Math.max(damageRadius, toFiniteNumber(effectState.superweaponEnergyNeutRadius, 0));
            if (toFiniteNumber(effectState.superweaponEnergyNeutAmount, 0) > 0 &&
                isEntityInsideCylinder(sourcePosition, direction, length, neutRadius, targetEntity)) {
                const neutApplication = resolveSignatureApplicationFactor(targetEntity, effectState.superweaponEnergyNeutSignatureRadius);
                applyCapacitorDrain(targetEntity, toFiniteNumber(effectState.superweaponEnergyNeutAmount, 0) * neutApplication, pulseTimeMs, callbacks);
            }
        }
    }
...
function getBurstProjectorAffectedTargetIDs(effectState) {
    return new Set((Array.isArray(effectState && effectState.superweaponBurstProjectorAffectedTargetIDs)
...
function applyBurstProjectorHostileDebuff(sourceEntity, targetEntity, effectState, nowMs, durationMs, callbacks = {}) {
    if (!sourceEntity ||
        !targetEntity ||
    ...
    const targetEffectState = buildBurstProjectorHostileEffectState(effectState, targetEntity, nowMs, durationMs);
    const result = callbacks.applySuperweaponHostileModuleState(sourceEntity, targetEntity, targetEffectState, nowMs, {
        durationMs,
    });
    return Boolean(result && result.success === true);
}
function removeBurstProjectorHostileDebuff(sourceEntity, targetEntity, effectState, nowMs, callbacks = {}) {
    if (!sourceEntity ||
        !targetEntity ||
        !hasBurstProjectorHostileDefinition(effectState) ||
    ...
    const targetEffectState = buildBurstProjectorHostileEffectState(effectState, targetEntity, nowMs, 1);
    const result = callbacks.removeSuperweaponHostileModuleState(sourceEntity, targetEntity, targetEffectState, nowMs);
    return Boolean(result && result.success === true);
}
function applyBurstProjectorJammerDebuff(sourceEntity, targetEntity, effectState, nowMs, durationMs, callbacks = {}) {
    if (!sourceEntity ||
        !targetEntity ||
        !hasBurstProjectorJammerDefinition(effectState) ||
    ...
    const targetEffectState = buildBurstProjectorJammerEffectState(effectState, targetEntity, nowMs, durationMs);
    const result = callbacks.applySuperweaponJammerModuleState(sourceEntity, targetEntity, targetEffectState, nowMs, {
        durationMs,
    });
...
function removeBurstProjectorJammerDebuff(sourceEntity, targetEntity, effectState, nowMs, callbacks = {}) {
    if (!sourceEntity ||
        !targetEntity ||
        !hasBurstProjectorJammerDefinition(effectState) ||
    ...
    const targetEffectState = buildBurstProjectorJammerEffectState(effectState, targetEntity, nowMs, 1);
    const result = callbacks.removeSuperweaponJammerModuleState(sourceEntity, targetEntity, targetEffectState, nowMs);
    return Boolean(result && result.success === true);
}
function resolveBurstProjectorEnergyNeutralizationMultiplier(effectState, targetEntity) {
    const resistanceMultiplier = resolveTargetResistanceMultiplier(targetEntity, effectState && effectState.superweaponBurstProjectorResistanceAttributeID);
    const signatureResolution = Math.max(0, toFiniteNumber(effectState && effectState.superweaponBurstProjectorEnergySignatureResolution, 0));
    const targetSignatureRadius = Math.max(0, toFiniteNumber(targetEntity && targetEntity.signatureRadius, 0));
    const signatureMultiplier = signatureResolution > 0 && targetSignatureRadius > 0
        ? Math.min(1, targetSignatureRadius / signatureResolution)
        : 1;
    return roundNumber(Math.max(resistanceMultiplier, 0) * Math.max(signatureMultiplier, 0), 6);
}
function applyBurstProjectorEnergyNeutralization(targetEntity, effectState, nowMs, callbacks = {}) {
    if (!targetEntity || !hasBurstProjectorEnergyNeutralizationDefinition(effectState)) {
        return false;
    }
    const drainMultiplier = resolveBurstProjectorEnergyNeutralizationMultiplier(effectState, targetEntity);
    const drainAmount = Math.max(0, toFiniteNumber(effectState.superweaponBurstProjectorStrengthValue, 0)) *
        Math.max(0, drainMultiplier);
    return applyCapacitorDrain(targetEntity, drainAmount, nowMs, callbacks);
}
function applyBurstProjectorDebuff(sourceEntity, targetEntity, effectState, nowMs, durationMs, callbacks = {}) {
    const hostileApplied = applyBurstProjectorHostileDebuff(sourceEntity, targetEntity, effectState, nowMs, durationMs, callbacks);
    const jammerApplied = applyBurstProjectorJammerDebuff(sourceEntity, targetEntity, effectState, nowMs, durationMs, callbacks);
    return hostileApplied || jammerApplied;
}
function removeBurstProjectorDebuff(sourceEntity, targetEntity, effectState, nowMs, callbacks = {}) {
    const hostileRemoved = removeBurstProjectorHostileDebuff(sourceEntity, targetEntity, effectState, nowMs, callbacks);
    const jammerRemoved = removeBurstProjectorJammerDebuff(sourceEntity, targetEntity, effectState, nowMs, callbacks);
    return hostileRemoved || jammerRemoved;
}
function cleanupBurstProjectorHostileDebuffs(scene, sourceEntity, effectState, nowMs, callbacks = {}) {
    ...
    for (const targetID of affectedTargetIDs) {
        const targetEntity = scene && typeof scene.getEntityByID === "function"
            ...
            : null;
        if (targetEntity) {
            removeBurstProjectorDebuff(sourceEntity, targetEntity, effectState, nowMs, callbacks);
        }
    }
    setBurstProjectorAffectedTargetIDs(effectState, []);
}
function tickBurstProjectorHostileDebuffs(scene, sourceEntity, effectState, nowMs, callbacks = {}) {
    if (!scene ||
        !sourceEntity ||
    ...
    if (nowMs >= windowEnd) {
        const instantPulsePending = effectState.superweaponBurstProjectorInstantPulse === true &&
    ...
    const pulseIndex = Math.floor((toFiniteNumber(nowMs, 0) - windowStart) /
        SUPERWEAPON_BURST_PROJECTOR_AOE_PULSE_MS);
    if (pulseIndex <= toInt(effectState.superweaponLastProcessedPulse, -1)) {
        return;
    }
    effectState.superweaponLastProcessedPulse = pulseIndex;
    const center = cloneVector(effectState.superweaponTargetPoint, sourceEntity.position);
    ...
    for (const targetEntity of collectPotentialBurstProjectorTargets(scene)) {
        ...
        if (!affectedTargetIDs.has(targetID)) {
            const applied = applyBurstProjectorDebuff(sourceEntity, targetEntity, effectState, nowMs, remainingDurationMs, callbacks);
            if (!applied) {
                continue;
            }
        }
        applyBurstProjectorEnergyNeutralization(targetEntity, effectState, nowMs, callbacks);
...
function tickBurstProjectorEffect(scene, sourceEntity, effectState, moduleItem, nowMs, callbacks = {}) {
    ...
    tickBurstProjectorHostileDebuffs(scene, sourceEntity, effectState, nowMs, callbacks);
    if (toInt(effectState.superweaponBeaconID, 0) > 0 &&
        toFiniteNumber(effectState.superweaponBeaconExpireAtMs, 0) > 0 &&
        toFiniteNumber(nowMs, 0) >= toFiniteNumber(effectState.superweaponBeaconExpireAtMs, 0)) {
        removeTransientEntity(scene, effectState.superweaponBeaconID, nowMs);
...
function tickShowController(scene, controller, nowMs, callbacks = {}) {
    ...
    const pickRandom = typeof controller.random === "function"
        ? controller.random
        : Math.random;
    ...
    const fireVolley = (source, targetEntry) => {
        ...
        const activationOptions = toInt(moduleItem.typeID, 0) === toInt(profile.lanceTypeID, 0)
            ? {
                targetPoint: cloneVector(targetEntity.position),
                repeat: 1,
            }
        ...
        const activationResult = scene.activateGenericModule(pseudoSession, moduleItem, null, activationOptions);
        if (!activationResult || activationResult.success !== true) {
            return false;
        }
        const firedFamily = toInt(moduleItem.typeID, 0) === toInt(profile.lanceTypeID, 0)
            ? "lance"
            : "doomsday";
        source.nextFamily =
    ...
    const batchSize = Math.max(1, toInt(controller.volleyBatchSize, DEFAULT_SHOW_VOLLEY_BATCH_SIZE));
    const volleyStepMs = Math.max(1, toInt(controller.volleyStepMs, DEFAULT_SHOW_VOLLEY_STEP_MS));
    ...
    controller.nextVolleyAtMs = nowMs + Math.max(1, toInt(controller.refireMs, DEFAULT_SHOW_REFIRE_MS));
}
function registerSuperTitanShowController(scene, options = {}) {
    ...
    scene.superTitanShowController = {
        ...
        volleyBatchSize: Math.max(1, toInt(options.volleyBatchSize, DEFAULT_SHOW_VOLLEY_BATCH_SIZE)),
        volleyStepMs: Math.max(1, toInt(options.volleyStepMs, DEFAULT_SHOW_VOLLEY_STEP_MS)),
        pendingVolley: [],
        nextVolleyStepAtMs: 0,
        nextVolleyAtMs: nowMs + Math.max(0, toInt(options.initialDelayMs, DEFAULT_SHOW_INITIAL_DELAY_MS)),
    };
    ...
    if (scene.dynamicEntities instanceof Map) {
        for (const entity of scene.dynamicEntities.values()) {
            ...
            for (const effectState of entity.activeModuleEffects.values()) {
                ...
                const moduleItem = callbacks.getEntityRuntimeModuleItem &&
                    typeof callbacks.getEntityRuntimeModuleItem === "function"
                    ? callbacks.getEntityRuntimeModuleItem(entity, effectState.moduleID, effectState.moduleFlagID)
                    : null;
                if (!moduleItem) {
                    continue;
                }
...
//# sourceMappingURL=superweaponRuntime.js.map