...
} = require(path.join(__dirname, "../../destiny/commands/superweapon.js"));
const {
...
function normalizeDamageVector(rawDamage: Record<string, any> = {}) {
  const source =
...
function scaleDamageVector(rawDamage: Record<string, any> = {}, factor = 1) {
...
function resolveSkillMap(entity, fallbackCharacterID = 0) {
  ...
  if (entity && entity.skillMap && typeof entity.skillMap === "object") {
    return new Map<any, any>(entity.skillMap);
  }
  const characterID =
    entity && (
...
function buildSuperweaponDogmaState(options: Record<string, any> = {}) {
  const {
    entity,
    shipItem,
    moduleItem,
...
function consumeSuperweaponFuel(entity, fuelTypeID, quantity, callbacks: Record<string, any> = {}) {
...
function allocateRuntimeEntityID(callbacks: Record<string, any> = {}, preferredItemID = null) {
  if (
...
function allocateSuperweaponBeaconID(scene, sourceEntity, nowMs, callbacks: Record<string, any> = {}) {
...
function spawnLanceBeacon(scene, sourceEntity, targetPoint, nowMs, callbacks: Record<string, any> = {}) {
...
function buildBurstProjectorModifierEntries(attributeMap, modifierSpecs: any[] = [], stackingPenalized = false) {
  const entries: any[] = [];
...
function setEntityMotionTowardTarget(entity, targetEntity, callbacks: Record<string, any> = {}) {
...
function applyCapacitorDrain(entity, drainAmount, whenMs, callbacks: Record<string, any> = {}) {
...
function applySuperweaponDamage(scene, sourceEntity, targetEntity, damageVector, moduleItem, whenMs, callbacks: Record<string, any> = {}) {
  if (
    !scene ||
    !sourceEntity ||
    !targetEntity ||
...
function prepareSuperweaponActivation(options: Record<string, any> = {}) {
  const {
    scene,
    entity,
    shipItem,
  ...
  const burstProjectorHostileDefinition: Record<string, any> =
  ...
  const burstProjectorJammerMaxStrength = Math.max(
    0,
    ...Object.values<any>(burstProjectorJammerStrengthBySensorType)
...
function inspectSuperweaponActivationContract(options: Record<string, any> = {}) {
  const {
    scene,
    entity,
    moduleItem,
  ...
  const effectStatePatch: Record<string, any> = activationResult.effectStatePatch || {};
  return {
    success: true,
    data: {
...
function executeSuperweaponActivation(options: Record<string, any> = {}) {
  const {
    scene,
    entity,
    moduleItem,
...
function finalizeSuperweaponDeactivation(options: Record<string, any> = {}) {
  const {
    scene,
    entity,
    effectState,
...
function tickDoomsdayEffect(scene, sourceEntity, effectState, moduleItem, nowMs, callbacks: Record<string, any> = {}) {
...
function tickLanceEffect(scene, sourceEntity, effectState, moduleItem, nowMs, callbacks: Record<string, any> = {}) {
...
function tickBurstProjectorHostileDebuffs(
  ...
  const affectedTargetIDs = getBurstProjectorAffectedTargetIDs(effectState);
  const currentTargetIDs = new Set<any>();
...
function tickBurstProjectorEffect(scene, sourceEntity, effectState, moduleItem, nowMs, callbacks: Record<string, any> = {}) {
  void moduleItem;
...
function tickShowController(scene, controller, nowMs, callbacks: Record<string, any> = {}) {
...
function registerSuperTitanShowController(scene, options: Record<string, any> = {}) {
  if (!scene) {
    return null;
  }
  const nowMs =
...
function tickScene(scene, nowMs, callbacks: Record<string, any> = {}) {
  if (!scene) {
    return;
  }
