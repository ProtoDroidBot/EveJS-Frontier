...
function resolveMicroJumpDistanceMeters(moduleItem, moduleAttributes) {
    const explicitJumpDistance = Math.max(0, roundNumber(toFiniteNumber(moduleAttributes[ATTRIBUTE_MJD_JUMP_RANGE], 0), 3));
...
function resolveMicroJumpDriveActivation({ moduleItem, effectRecord, chargeItem = null, shipItem, skillMap = null, fittedItems = null, activeModuleContexts = null, } = {}) {
    ...
    const durationAttributeID = toFiniteNumber(moduleAttributes[ATTRIBUTE_DURATION], 0) > 0
        ? ATTRIBUTE_DURATION
        : ATTRIBUTE_SPEED;
    const jumpDistanceMeters = resolveMicroJumpDistanceMeters(moduleItem, moduleAttributes);
...
function executeMicroJumpDriveCycle({ scene, entity, effectState, nowMs, callbacks = {}, } = {}) {
    ...
    if (typeof callbacks.isMicroJumpDriveBlocked === "function" &&
    ...
    const destination = callbacks.addVectors
        ? callbacks.addVectors(entity.position, callbacks.scaleVector(direction, Math.max(0, toFiniteNumber(effectState.microJumpDistanceMeters, 0))))
        : {
    ...
    if (typeof scene.isPilotWarpLandingPendingForEntity === "function" &&
        scene.isPilotWarpLandingPendingForEntity(entity)) {
        return {
            success: false,
            stopReason: "movement",
    ...
    if (effectState.microJumpJumpFxGuid) {
        scene.broadcastSpecialFx(entity.itemID, effectState.microJumpJumpFxGuid, {
...
//# sourceMappingURL=microJumpDriveRuntime.js.map