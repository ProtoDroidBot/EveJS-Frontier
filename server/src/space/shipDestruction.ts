...
function queueCloneSelectionTransition(
  ...
  const complete = () => {
    ...
    try {
      ...
      log.warn(
        `[ShipDestruction] Clone-selection transition failed char=${characterID}: ${error.message}`,
      );
      return null;
    }
  };
...
function destroySessionClonePreservingShip(
  ...
  const transition = queueCloneSelectionTransition(session, () => {
    ...
    log.info(
      `[ShipDestruction] Opened shell-expiration clone selection ` +
  ...
  log.info(
    `[ShipDestruction] Shell-expiration transition queued char=${characterID} ` +
...
function destroySessionShipAndClone(
  ...
  const transition = queueCloneSelectionTransition(session, () => {
    ...
    log.info(
      `[ShipDestruction] Opened hull-death clone selection ` +
