...
function buildPatternTransform(siteID, source, patternIndex, options = {}) {
    const slotsPerRing = Math.max(3, Math.min(12, toInt(options.patternSlotsPerRing, DEFAULT_PATTERN_SLOTS_PER_RING)));
    const baseRadius = Math.max(50_000, toFiniteNumber(options.patternRingRadiusMeters, DEFAULT_PATTERN_RING_RADIUS_METERS));
    const ringStep = Math.max(50_000, toFiniteNumber(options.patternRingStepMeters, DEFAULT_PATTERN_RING_STEP_METERS));
    ...
    const angleJitter = ((hash32(`${seed}:angle-jitter`) / 0x100000000) - 0.5) * (Math.PI / 9);
    const angle = baseAngle +
        (slotIndex * ((Math.PI * 2) / slotsPerRing)) +
        (ringIndex * (Math.PI / slotsPerRing)) +
        angleJitter;
    const nominalRadius = baseRadius + (ringIndex * ringStep);
    const radiusJitter = ((hash32(`${seed}:radius-jitter`) / 0x100000000) - 0.5) * Math.min(40_000, nominalRadius * 0.2);
    const verticalOffset = ((hash32(`${seed}:vertical-offset`) / 0x100000000) - 0.5) * 40_000;
    return {
...
function normalizePattern(pattern) {
    ...
    const maxOccurrences = Math.max(minOccurrences, toInt(pattern && pattern.maxOccurrences, minOccurrences));
    return {
        dungeonID,
        minOccurrences,
        maxOccurrences,
...
function selectPatternOccurrences(patterns, minimum, maximum, seed) {
    ...
    for (const pattern of normalized) {
        ...
        remainingByDungeonID.set(pattern.dungeonID, pattern.maxOccurrences - pattern.minOccurrences);
    }
    ...
    const upperBound = Math.min(capacity, Math.max(lowerBound, toInt(maximum, lowerBound)));
    const targetCount = deterministicCount(seed, lowerBound, upperBound);
    while (selected.length < targetCount) {
        const candidates = normalized.filter((pattern) => (remainingByDungeonID.get(pattern.dungeonID) || 0) > 0);
        if (candidates.length === 0) {
            break;
        }
        const totalWeight = candidates.reduce((sum, pattern) => sum + (pattern.weight > 0 ? pattern.weight : 1), 0);
        let cursor = (hash32(`${seed}:pick:${selected.length}`) / 0x100000000) * totalWeight;
        ...
        remainingByDungeonID.set(selectedPattern.dungeonID, (remainingByDungeonID.get(selectedPattern.dungeonID) || 0) - 1);
    }
    return selected;
}
    ...
    for (const room of Array.isArray(dungeon && dungeon.rooms) ? dungeon.rooms : []) {
        ...
        for (const object of Array.isArray(room && room.objects) ? room.objects : []) {
            ...
            flattened.push({
                ...
                positionOffset: addVectors(cloneVector(sourceTransform.positionOffset), rotateVectorAroundY(localPosition, sourceTransform.yawDegrees)),
...
function buildLandscapeScenePlan(site, ecosystem, getDungeonByID, options = {}) {
    ...
    const natural = selectPatternOccurrences(ecosystem.naturalWorldPatterns, ecosystem.minNaturalWorldPatterns, ecosystem.maxNaturalWorldPatterns, `${siteID}:natural`).map((pattern) => ({ ...pattern, patternKind: "natural" }));
    const broken = selectPatternOccurrences(ecosystem.brokenWorldPatterns, ecosystem.minBrokenWorldPatterns, ecosystem.maxBrokenWorldPatterns, `${siteID}:broken`).map((pattern) => ({ ...pattern, patternKind: "broken" }));
    const entryDungeonID = Math.max(0, toInt(ecosystem.entryDungeonID, toInt(site && site.dungeonID, 0)));
