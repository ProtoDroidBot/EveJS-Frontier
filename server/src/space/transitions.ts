...
function syncInventoryChangesToSession(session, changes: any[] = [], options: Record<PropertyKey, any> = {}) {
...
function emitBoardedCapsuleItemsChanged(session, capsuleItem, previousState: Record<PropertyKey, any> = {}) {
...
function applyTqDockingTransitionSessionChangeShape(plan) {
  ...
  for (const [key, value] of Object.entries<any>(sessionChanges)) {
...
function scheduleUndockInvulnerabilityCancellation(session, shipID, options: Record<PropertyKey, any> = {}) {
...
function collectSessionBoundObjectIDs(session) {
  ...
  if (session && session._boundObjectIDs && typeof session._boundObjectIDs === "object") {
    for (const value of Object.values<any>(session._boundObjectIDs)) {
      addObjectID(value);
    }
  }
...
function buildLocationIdentityPatch(record, solarSystemID, extra: Record<PropertyKey, any> = {}) {
...
function cloneVector(source = null, fallback: Record<PropertyKey, any> = { x: 0, y: 0, z: 0 }) {
  const vectorSource =
...
function normalizeVector(vector, fallback: Record<PropertyKey, any> = { x: 1, y: 0, z: 0 }) {
...
function buildOffsetSpawnState(anchor, options: Record<PropertyKey, any> = {}) {
...
function queuePendingSessionEffects(session, options: Record<PropertyKey, any> = {}) {
...
function destroySessionAfterUnsafeSameSceneBoundary(
  ...
  const error: Error & Record<PropertyKey, any> = new Error(code);
  error.code = code;
...
function flushSameSceneShipSwapNotificationPlan(session, plan, options: Record<PropertyKey, any> = {}) {
  const sessionChangeOptions: Record<PropertyKey, any> = {
...
function syncDockedShipTransitionForSession(session, dockResult, options: Record<PropertyKey, any> = {}) {
...
function undockSession(session, options: Record<PropertyKey, any> = {}) {
...
function ejectSession(session, options: Record<PropertyKey, any> = {}) {
  ...
  try {
    ...
    if (!egoVisibilityCommitted || !egoDeliveryBoundaryUsable) {
      const deliveryError: Error & Record<PropertyKey, any> = new Error(
...
function ejectSessionForShipDestruction(session, options: Record<PropertyKey, any> = {}) {
...
function jumpSessionToSolarSystem(session, solarSystemID, options: Record<PropertyKey, any> = {}) {
  if (!session || !session.characterID) {
    return {
      success: false,
      errorMsg: "CHARACTER_NOT_SELECTED",
