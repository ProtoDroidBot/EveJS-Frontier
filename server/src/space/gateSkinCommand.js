...
function normalizeGraphicMaterialSet(row) {
    const materialSetID = toInt(row && row._key != null ? row._key : row && row.materialSetID, 0);
    if (materialSetID <= 0) {
        return null;
    }
    const description = String((row && (row.description || row.name || row.sofFactionName)) ||
        `Material Set ${materialSetID}`).trim();
    return {
        materialSetID,
...
function loadGraphicMaterialSets() {
    ...
    cachedGraphicMaterialSets = rows.sort((left, right) => left.materialSetID - right.materialSetID);
    return cachedGraphicMaterialSets;
}
function getGateSkinMaterialsForRaceHint(raceHint) {
    ...
    for (const material of loadGraphicMaterialSets()) {
        ...
        if (materialRaceHint === normalizedRaceHint ||
...
function buildGateSkinListMessage(argumentText = "") {
    ...
    const selectedPresets = FACTION_GATE_PRESETS.filter((preset) => !query || preset.key.startsWith(query.toLowerCase()));
...
function getTypeRecordInteractionRadius(typeRecord) {
    return (toFiniteNumber(typeRecord &&
        typeRecord.sample &&
        typeRecord.sample.interactionRadius, DEFAULT_STARGATE_INTERACTION_RADIUS_METERS) || DEFAULT_STARGATE_INTERACTION_RADIUS_METERS);
}
...
function allocateGateSkinPreviewItemID(scene) {
    let candidate = GATE_SKIN_COMMAND_ITEM_ID_BASE + nextGateSkinPreviewItemID;
...
function buildPreviewPositions(shipEntity) {
    ...
    const center = addVectors(shipPosition, scaleVector(forward, GATE_SKIN_FORWARD_OFFSET_METERS));
    const startOffset = -((FACTION_GATE_PRESETS.length - 1) * GATE_SKIN_LINE_SPACING_METERS) / 2;
    return FACTION_GATE_PRESETS.map((preset, index) => ({
        preset,
        position: addVectors(center, scaleVector(right, startOffset + (index * GATE_SKIN_LINE_SPACING_METERS))),
    }));
}
function buildDunRotationToward(position, lookAtPosition) {
    return (geo2.buildDunRotationFromDirection(subtractVectors(lookAtPosition, position)) || [0, 0, 0]);
}
function buildGateSkinPreviewEntity(options) {
    ...
    return {
        ...
        itemName: `Gate Skin Preview (${preset.label}) - ${material.description}`,
        ownerID,
...
function sendGateSkinFeedback(active, message) {
    if (active &&
        ...
        active.session &&
        message) {
        active.chatHub.sendSystemMessage(active.session, `<color=${CHAT_FEEDBACK_AQUA}>${message}</color>`, active.feedbackChannel || null);
    }
}
...
function scheduleGateSkinPreviewModelRefresh(active, entities) {
    ...
    const refreshTimer = setTimeout(() => {
        ...
        const currentActive = activeGateSkinShowcasesByCharacterID.get(active.characterID);
        if (currentActive !== active || !active.scene) {
            return;
        }
        const liveEntities = entities
            .map((entity) => typeof active.scene.getEntityByID === "function"
...
function applyGateSkinCycle(active, options = {}) {
    ...
    for (const gate of active.gates) {
        const entity = typeof active.scene.getEntityByID === "function"
    ...
    if (changedEntities.length > 0) {
        const refreshScheduled = scheduleGateSkinPreviewModelRefresh(active, changedEntities);
        if (!refreshScheduled) {
            active.scene.broadcastSlimItemChanges(changedEntities);
        }
    }
    if (options.announce !== false && labels.length > 0) {
        sendGateSkinFeedback(active, `Gate skin cycle ${active.cycleIndex + 1}: ${labels.join(" | ")}`);
    }
    return true;
}
...
function startGateSkinShowcase(session, context, callbackOptions = {}) {
    ...
    return {
        success: true,
        message: `Spawned ${addedEntities.length} gate skin preview gates 10km apart. ` +
...
function executeGateSkinCommand(session, argumentText, callbackOptions = {}) {
    ...
    if (!context.success) {
        return {
            success: false,
            message: context.errorMsg === "NOT_IN_SPACE"
...
//# sourceMappingURL=gateSkinCommand.js.map