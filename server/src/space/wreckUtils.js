...
const { resolveShipByTypeID, } = require(path.join(__dirname, "../services/chat/shipTypeRegistry"));
...
function resolveShipWreckRacePrefix(shipMeta = {}, itemMeta = {}) {
    const raceID = toPositiveInt(shipMeta.raceID !== undefined ? shipMeta.raceID : itemMeta.raceID, 0);
    return RACE_WRECK_PREFIX_BY_ID[raceID] || null;
}
function resolveShipHullClassName(shipMeta = {}, itemMeta = {}) {
    const groupName = String(shipMeta.groupName ||
        itemMeta.groupName ||
        "").trim().toLowerCase();
    if (!groupName) {
        return null;
...
function resolveNpcMissionGenericWreckType(context = {}, shipMeta = {}, itemMeta = {}) {
    const sourceGroupName = String(context.groupName ||
        shipMeta.groupName ||
        itemMeta.groupName ||
        "").trim();
    const sourceItemName = String(context.itemName ||
        shipMeta.name ||
        itemMeta.name ||
        "").trim();
...
function buildShipWreckCandidateNames(shipMeta = {}, itemMeta = {}) {
    ...
    const groupName = String(shipMeta.groupName ||
        ...
        "").trim().toLowerCase();
    const candidates = [];
    const explicitWreckName = SPECIAL_WRECK_NAME_BY_SHIP_NAME[String(shipMeta.name || itemMeta.name || "").trim()] || null;
...
function resolveSpecialShipWreckType(shipMeta = {}, itemMeta = {}) {
    const explicitShipTypeID = toPositiveInt(shipMeta.typeID !== undefined ? shipMeta.typeID : itemMeta.typeID, 0);
    ...
    for (const normalizedName of normalizedNames) {
        const directNamedWreckTypeID = toPositiveInt(SPECIAL_WRECK_TYPE_ID_BY_SHIP_NAME[normalizedName], 0);
...
function buildNpcFactionWreckCandidateNames(context = {}, shipMeta = {}, itemMeta = {}) {
    ...
    const wreckSize = resolveNpcHullWreckSize(hullClassName);
    const labels = resolveFactionWreckLabels(context.factionName, context.groupName, context.itemName, context.profileID, shipMeta.name, itemMeta.name, shipMeta.groupName, itemMeta.groupName);
    const candidates = [];
...
function resolveEntityWreckType(input = {}) {
    ...
    const nativeNpcContext = context.nativeNpc === true ||
        String(context.npcEntityType || "").trim().length > 0 ||
        String(context.profileID || "").trim().length > 0;
    if (nativeNpcContext) {
        const capitalNpcContext = context.capitalNpc === true ||
            String(context.classID || "").trim().length > 0;
        if (capitalNpcContext) {
            const { resolveCapitalNpcWreckType, } = require(path.join(__dirname, "./npc/capitals/capitalNpcWrecks"));
        ...
        const missionGenericMatch = resolveNpcMissionGenericWreckType(context, shipMeta, itemMeta);
...
//# sourceMappingURL=wreckUtils.js.map