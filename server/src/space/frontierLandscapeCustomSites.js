...
function normalizeState(value) {
    ...
    const sites = {};
    for (const rawSite of Object.values(source.sites && typeof source.sites === "object" ? source.sites : {})) {
        const site = normalizeSite(rawSite);
        if (site) {
            sites[String(site.itemID)] = site;
        }
    }
    const highestSiteID = Object.values(sites).reduce((highest, site) => Math.max(highest, site.itemID), CUSTOM_SITE_ID_BASE - 1);
    return {
        version: STATE_VERSION,
        nextSiteID: Math.max(CUSTOM_SITE_ID_BASE, highestSiteID + 1, toPositiveInt(source.nextSiteID, CUSTOM_SITE_ID_BASE)),
        sites,
    };
}
...
function createCustomLandscapeSiteStore(options = {}) {
    ...
    const resolveType = options.resolveItemByTypeID || require(path.join(__dirname, "../services/inventory/itemTypeRegistry")).resolveItemByTypeID;
    ...
    function allocateSiteID(state) {
        ...
        while (siteID < CUSTOM_SITE_ID_LIMIT &&
    ...
    function createSite(input = {}) {
        ...
        const sourceSite = (referenceData.ensureLoaded &&
            Array.isArray(referenceData.ensureLoaded().landscapeSites))
            ? referenceData.ensureLoaded().landscapeSites.find((site) => toPositiveInt(site && site.ecosystemID, 0) === ecosystemID) || null
            : null;
        ...
        const site = normalizeSite({
            ...
            categoryID: toPositiveInt(typeRecord.categoryID, toPositiveInt(sourceSite && sourceSite.categoryID, 0)),
            graphicID: toPositiveInt(typeRecord.graphicID, toPositiveInt(sourceSite && sourceSite.graphicID, 0)) || null,
            radius: Math.max(1, toFiniteNumber(typeRecord.radius, toFiniteNumber(sourceSite && sourceSite.radius, 1))),
            solarSystemID,
            position,
            featureID: siteID,
            ...feature,
...
//# sourceMappingURL=frontierLandscapeCustomSites.js.map