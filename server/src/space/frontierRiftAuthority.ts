...
function normalizeCollection(value) {
  ...
  if (value && typeof value === "object") {
    return Object.values<any>(value).filter((entry) => entry && typeof entry === "object");
  }
  return [];
}
...
function createFrontierRiftAuthority(options: Record<string, any> = {}) {
  ...
  function buildCache() {
    ...
    for (const dungeon of Array.isArray(dungeons) ? dungeons : []) {
      ...
      const sceneObjects = authoredSceneObjects.filter(
        (object) => !spawnGuardObjectIDs.has(object.objectID),
      );
      const resources = sceneObjects.filter(
        (object) => object.groupID === CRUDE_MATTER_GROUP_ID,
      );
      const resourceVariants = authoredSceneObjects.filter(
        (object) => object.groupID === CRUDE_MATTER_GROUP_ID,
      );
      ...
      templates.push({
        ...
        resourceVariants,
        resourceProfiles,
        riftTags: [...new Set<any>(resourceProfiles.flatMap((profile) => profile.tags))],
        sceneObjects,
