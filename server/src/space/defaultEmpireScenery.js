...
const { CONCORD_FACTION_ID, SENTRY_OWNER_ID, getEverMoreGateBillboardLayout, getEverMoreGateSentryLayout, isEverMoreGatePresenceSystem, } = require(path.join(__dirname, "./empireGatePresence/everMoreGatePresence"));
const { CONCORD_BILLBOARD_TYPE_ID, getGateBillboardLayout, getGateSentryLayout, } = require(path.join(__dirname, "./empireGatePresence/gateSceneryLayout"));
const { getStationSentryLayout, } = require(path.join(__dirname, "./empireGatePresence/stationSceneryLayout"));
const { DESTINY_BOOTSTRAP_DELIVERY_ADDBALLS2, applyAddBalls2StopPresentation, isAddBalls2BootstrapScope, markAddBalls2BootstrapEntity, } = require(path.join(__dirname, "./destiny/bootstrap/staticBootstrap"));
...
function dotProduct(left = ZERO_VECTOR, right = ZERO_VECTOR) {
    return ((toFiniteNumber(left.x, 0) * toFiniteNumber(right.x, 0)) +
...
function applyEligibleAddBalls2BootstrapPresentation(entity) {
    if (!entity) {
        return entity;
    }
    applyAddBalls2StopPresentation(entity);
    if (isAddBalls2BootstrapScope(entity.staticVisibilityScope)) {
        markAddBalls2BootstrapEntity(entity);
    }
    return entity;
}
function createVisualEntity({ kind, itemID, typeID, ownerID, corporationID, factionID, warFactionID = 0, position, direction, itemName = null, suppressSlimName = false, }) {
...
function buildGateBillboardEntity(system, stargate) {
    ...
    const position = addVectors(cloneVector(stargate && stargate.position), cloneVector(layout.offset));
    const facingDirection = subtractVectors(cloneVector(stargate && stargate.position), position);
    ...
    if (entity) {
        applyEligibleAddBalls2BootstrapPresentation(entity);
...
function buildGateSentryEntities(system, stargate) {
    ...
    return getGateSentryLayout(system, stargate)
        .map((slot, index) => {
        const position = addVectors(cloneVector(stargate && stargate.position), cloneVector(slot && slot.offset));
        const direction = normalizeVector(subtractVectors(position, stargate && stargate.position), DEFAULT_FORWARD);
        const entity = createVisualEntity({
            kind: "sentryGun",
            itemID: buildGeneratedItemID(GATE_SENTRY_ITEM_ID_BASE, stargate && stargate.itemID, index + 1),
            typeID: slot.typeID,
...
function buildEverMoreGateSentryEntities(system, stargate) {
    ...
    return layout
        ...
        const entity = createVisualEntity({
            kind: "sentryGun",
            itemID: buildGeneratedItemID(GATE_SENTRY_ITEM_ID_BASE, stargate && stargate.itemID, 90 + index + 1),
            typeID: slot.typeID,
...
function buildStationSentryEntities(system, station) {
    const ownerID = toPositiveInt(station && (station.corporationID || station.ownerID), toPositiveInt(system && system.factionID, CONCORD_FACTION_ID));
    return getStationSentryLayout(system, station)
        .map((slot, index) => {
        const position = addVectors(cloneVector(station && station.position), cloneVector(slot && slot.offset));
        const direction = normalizeVector(subtractVectors(position, station && station.position), DEFAULT_FORWARD);
        const entity = createVisualEntity({
            kind: "sentryGun",
            itemID: buildGeneratedItemID(STATION_SENTRY_ITEM_ID_BASE, station && station.stationID, index + 1),
            typeID: slot.typeID,
            ownerID,
            corporationID: 0,
            factionID: slot.factionID,
...
function buildSignature() {
    return JSON.stringify({
        ...
        everMoreGatePresenceSystemIDs: String(config.npcDefaultEverMoreGatePresenceSystemIDs || "").trim(),
    });
}
function rebuildCache() {
    ...
    log.info(`[SpaceScenery] Generated ${billboardCount} gate billboards and ${sentryGunCount} sentry guns across ${nextCache.size} empire-space systems in ${Date.now() - startedAt} ms`);
}
function getConfiguredStaticEntitiesForSystem(systemID) {
    rebuildCache();
    return (cachedEntitiesBySystemID.get(toPositiveInt(systemID, 0)) || []).map(cloneEntity);
}
module.exports = {
...
//# sourceMappingURL=defaultEmpireScenery.js.map