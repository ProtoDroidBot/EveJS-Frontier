...
} = require(path.join(__dirname, "./destiny/bootstrap/staticBootstrap"));
...
const DEFAULT_DIRECTION: Readonly<Vector3> = Object.freeze({ x: 1, y: 0, z: 0 });
...
function normalizeProp(rawProp, fileName, fileSystemID, propIndex = 0) {
  ...
  if (hasOwn(rawProp, "destinyCollisionTail")) {
    ...
    if (hasOwn(rawProp, "destinyCollisionTailSource")) {
      entity.destinyCollisionTailSource = normalizeText(
        rawProp.destinyCollisionTailSource,
        "",
      );
    }
  }
...
function loadAuthoredSpaceProps() {
  ...
  for (const fileName of fileNames) {
    ...
    const identityIndexByKey = new Map<any, any>(
      entities
