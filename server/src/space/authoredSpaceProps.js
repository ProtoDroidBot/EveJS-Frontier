...
const { DESTINY_BOOTSTRAP_DELIVERY_ADDBALLS2, isAddBalls2BootstrapScope, markAddBalls2BootstrapEntity, } = require(path.join(__dirname, "./destiny/bootstrap/staticBootstrap"));
...
function shouldReplaceAuthoredProp(existing, candidate) {
    ...
    return String(candidate && candidate.itemID || "").localeCompare(String(existing && existing.itemID || ""), undefined, { numeric: true }) > 0;
}
function normalizeMode(value) {
    const mode = String(value || "").trim().toUpperCase();
    return mode || "RIGID";
}
function markEligibleAuthoredBootstrapDelivery(entity, rawProp) {
    const bootstrapDelivery = hasOwn(rawProp, "destinyBootstrapDelivery")
        ? normalizeText(rawProp.destinyBootstrapDelivery, "")
        : "";
    if (bootstrapDelivery &&
        bootstrapDelivery !== DESTINY_BOOTSTRAP_DELIVERY_ADDBALLS2) {
        entity.destinyBootstrapDelivery = bootstrapDelivery;
        return entity;
    }
    if (entity.kind === "authoredSpaceProp" &&
        isAddBalls2BootstrapScope(entity.staticVisibilityScope)) {
...
function normalizeProp(rawProp, fileName, fileSystemID, propIndex = 0) {
    ...
    const categoryID = toPositiveInt(rawProp.categoryID, toPositiveInt(typeRecord && typeRecord.categoryID, 0));
    const graphicID = toPositiveInt(typeRecord && typeRecord.graphicID, 0);
    const radius = Math.max(0, toFiniteNumber(rawProp.radius, toFiniteNumber(typeRecord && typeRecord.radius, 1)));
    const entity = {
        ...
        staticVisibilityScope: normalizeVisibilityScope(rawProp.staticVisibilityScope, DEFAULT_AUTHORED_PROP_VISIBILITY_SCOPE),
    ...
    if (hasOwn(rawProp, "destinyCollisionTail")) {
        entity.destinyCollisionTail = normalizeText(rawProp.destinyCollisionTail, "");
        if (hasOwn(rawProp, "destinyCollisionTailSource")) {
...
function loadAuthoredSpaceProps() {
    ...
    for (const fileName of fileNames) {
        ...
        const identityIndexByKey = new Map(entities
...
//# sourceMappingURL=authoredSpaceProps.js.map