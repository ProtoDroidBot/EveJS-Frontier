...
const { ONE_AU_IN_METERS, findSafeWarpOriginAnchor, } = require(path.join(__dirname, "./npcWarpOrigins"));
const { applyNpcWarpCompletionWakeDeadlineCommand, } = require(path.join(__dirname, "../destiny/commands/npcWarpCompletion.js"));
...
function estimateNpcWarpWakeAtMs(scene, options = {}) {
    ...
    const tickIntervalMs = Math.max(1, toFiniteNumber(scene && scene._tickIntervalMs, 1000));
    return Math.max(now, toFiniteNumber(options.ingressCompleteAtMs, now)) + (tickIntervalMs * NPC_WARP_WAKE_GRACE_TICKS);
}
function startManagedNpcWarp(context, point, options = {}) {
    ...
    const visibilitySuppressMs = Math.max(1, toFiniteNumber(options.visibilitySuppressMs, NPC_WARP_VISIBILITY_SUPPRESS_MS));
    const warpResult = spaceRuntime.startSessionlessWarpIngress(context.systemID, context.entityID, cloneVector(point), {
        ...options,
        ...
        ingressDurationMs: Math.max(visibilitySuppressMs, toFiniteNumber(options.ingressDurationMs, visibilitySuppressMs)),
    });
    if (!warpResult.success || !warpResult.data) {
        return warpResult;
    }
    const wakeAtMs = estimateNpcWarpWakeAtMs(scene, {
        ingressCompleteAtMs: toFiniteNumber(warpResult.data.ingressCompleteAtMs, scene.getCurrentSimTimeMs()),
    });
    if (warpResult.data.entity) {
        applyNpcWarpCompletionWakeDeadlineCommand(warpResult.data.entity, wakeAtMs);
    }
    return {
        success: true,
        data: {
...
function spawnWarpBatchForSession(session, options = {}) {
    ...
    const originAnchor = findSafeWarpOriginAnchor(scene, targetEntity, {
        clearanceMeters: Math.max(ONE_AU_IN_METERS, toFiniteNumber(options.originClearanceMeters, ONE_AU_IN_METERS)),
    ...
    for (const entry of spawned) {
        const attackResult = issueAttackOrder(toPositiveInt(entry.entity && entry.entity.itemID, 0), shipID, {
            keepLock: true,
        });
    ...
    const landingRadiusMeters = Math.max(0, toFiniteNumber(options.landingRadiusMeters, NPC_COMMAND_WARP_LANDING_RADIUS_METERS));
    const warpRequests = spawned.map((entry, index) => ({
        entityID: toPositiveInt(entry.entity && entry.entity.itemID, 0),
        point: buildWarpLandingPoint(targetEntity.position, index, spawned.length, landingRadiusMeters),
        options: {
            forceImmediateStart: true,
            broadcastWarpStartToVisibleSessions: true,
            targetEntityID: shipID,
            useNativeWarpProfile: true,
            // TQ lets the mode-3 ball finish locally from the shared profile; it
            // does not fight that solver with a bare early Stop packet.
            suppressCompletionStopUpdates: true,
            ingressDurationMs: Math.max(NPC_WARP_VISIBILITY_SUPPRESS_MS, toFiniteNumber(options.ingressDurationMs, NPC_COMMAND_WARP_INGRESS_DURATION_MS)),
            visibilitySuppressMs: Math.max(1, toFiniteNumber(options.visibilitySuppressMs, NPC_WARP_VISIBILITY_SUPPRESS_MS)),
...
function stop(entityID, options = {}) {
    ...
    const ok = spaceRuntime.stopDynamicEntity(contextResult.data.systemID, contextResult.data.entityID, {
        ...options,
...
function follow(entityID, targetEntityID, range = 0, options = {}) {
    ...
    const ok = spaceRuntime.followDynamicEntity(contextResult.data.systemID, contextResult.data.entityID, toPositiveInt(targetEntityID, 0), toFiniteNumber(range, 0), options);
    if (!ok) {
        return {
            success: false,
...
function orbit(entityID, targetEntityID, distance, options = {}) {
    ...
    const ok = spaceRuntime.orbitDynamicEntity(contextResult.data.systemID, contextResult.data.entityID, toPositiveInt(targetEntityID, 0), toFiniteNumber(distance, 0), options);
    if (!ok) {
        return {
            success: false,
...
function warpToPoint(entityID, point, options = {}) {
    ...
    const warpResult = startManagedNpcWarp(contextResult.data, point, options);
    if (!warpResult.success || !warpResult.data) {
        return warpResult;
    }
    if (options.scheduleWake !== false) {
        npcService.scheduleNpcController(entityID, toFiniteNumber(warpResult.data.ingressCompleteAtMs, contextResult.data.scene.getCurrentSimTimeMs()));
    }
    return {
        success: true,
        data: {
...
function warpToEntity(entityID, targetEntityID, options = {}) {
    ...
    const targetEntity = contextResult.data.scene.getEntityByID(toPositiveInt(targetEntityID, 0));
    if (!targetEntity) {
        return {
            success: false,
...
function warpBatchToPoints(requests, options = {}) {
    ...
    for (const request of normalizedRequests) {
        ...
        const warpResult = startManagedNpcWarp(contextResult.data, request.point, requestOptions);
        if (!warpResult.success || !warpResult.data) {
            return warpResult;
        }
        const resultEntry = {
        ...
        commonWakeAtMs = Math.max(commonWakeAtMs, toFiniteNumber(warpResult.data.ingressCompleteAtMs, contextResult.data.scene.getCurrentSimTimeMs()));
    }
    if (options.scheduleWake !== false) {
        const groupedWake = options.groupWake !== false;
        for (const resultEntry of results) {
            npcService.scheduleNpcController(resultEntry.entityID, groupedWake
                ? commonWakeAtMs
                : toFiniteNumber(resultEntry.ingressCompleteAtMs, commonWakeAtMs));
        }
    }
    return {
        success: true,
