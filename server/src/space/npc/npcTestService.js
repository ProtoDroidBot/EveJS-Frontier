...
const { buildNpcDefinition, listNpcProfiles, } = require(path.join(__dirname, "./npcData"));
const npcService = require(path.join(__dirname, "./npcService"));
const { buildWeaponModuleSnapshot, } = require(path.join(__dirname, "../combat/weaponDogma"));
const { resolveItemByTypeID, } = require(path.join(__dirname, "../../services/inventory/itemTypeRegistry"));
const { getNpcCapabilityTypeID, isNpcChargeCompatibleWithModule, } = require(path.join(__dirname, "./npcCapabilityResolver"));
...
function buildWeaponSnapshotsForDefinition(definition) {
    ...
    for (const moduleEntry of modules) {
        ...
        const matchingChargeEntry = charges.find((chargeEntry) => (isNpcChargeCompatibleWithModule(moduleItem, toPositiveInt(chargeEntry && chargeEntry.typeID, 0)))) || null;
        ...
        const engagementRangeMeters = Math.max(0, toFiniteNumber(snapshot.approxRange, toFiniteNumber(snapshot.optimalRange, 0) + toFiniteNumber(snapshot.falloff, 0)));
...
function buildNpcTestCombatCatalog() {
    const definitions = listNpcProfiles()
        ...
        .filter(Boolean)
        .filter((definition) => String(definition.profile && definition.profile.entityType || "").trim().toLowerCase() === ENTITY_TYPE.NPC)
    ...
    for (const definition of definitions) {
        ...
        const primarySnapshot = snapshots.reduce((bestSnapshot, nextSnapshot) => (!bestSnapshot || nextSnapshot.engagementRangeMeters > bestSnapshot.engagementRangeMeters
            ? nextSnapshot
            : bestSnapshot), null);
        if (!primarySnapshot) {
            continue;
...
function selectNpcTestDefinitions(catalog, amount) {
    ...
    const guaranteedMissileCount = pools.missileEntries.length > 0
        ? Math.min(requestedAmount, Math.max(1, Math.round(requestedAmount * 0.2)))
        : 0;
    const selected = [
        ...sampleEntries(pools.missileEntries, guaranteedMissileCount),
        ...sampleEntries(pools.combatEntries, Math.max(0, requestedAmount - guaranteedMissileCount)),
    ];
...
function buildFilledSphereOffsets(count, spacingMeters = NPCTEST_SPHERE_SPACING_METERS) {
    ...
    while (remaining > 0) {
        ...
        const shellCapacity = Math.max(1, Math.round((4 * Math.PI * (shellRadius ** 2)) / (spacing ** 2)));
...
function buildShellSphereOffsets(count, spacingMeters = NPCTEST_SPHERE_SPACING_METERS) {
    ...
    const spacing = Math.max(1_000, toFiniteNumber(spacingMeters, NPCTEST_SPHERE_SPACING_METERS));
    const shellRadius = Math.max(spacing, Math.sqrt((requestedCount * (spacing ** 2)) / (4 * Math.PI)));
    return buildSphereShellOffsets(requestedCount, shellRadius);
}
function resolveShipForwardVector(shipEntity) {
    const forwardFromDirection = normalizeVector(shipEntity && shipEntity.direction, { x: 0, y: 0, z: 0 });
    if (magnitude(forwardFromDirection) > 0) {
        return forwardFromDirection;
    }
    return normalizeVector(shipEntity && shipEntity.velocity, { x: 1, y: 0, z: 0 });
}
function clampPlayerEngagementPosition(playerPosition, desiredPosition, engagementRangeMeters) {
    ...
    const maxDistance = Math.max(NPCTEST_MIN_PLAYER_DISTANCE_METERS, Math.min(desiredDistance, Math.max(NPCTEST_MIN_PLAYER_DISTANCE_METERS, toFiniteNumber(engagementRangeMeters, desiredDistance) * NPCTEST_ENGAGEMENT_RANGE_SCALE)));
    return addVectors(playerPosition, scaleVector(desiredDirection, maxDistance));
}
...
function buildForwardBasis(forward) {
    const worldUp = Math.abs(toFiniteNumber(forward && forward.y, 0)) >= 0.99
        ? { x: 1, y: 0, z: 0 }
        : { x: 0, y: 1, z: 0 };
    const right = normalizeVector(crossVectors(forward, worldUp), { x: 1, y: 0, z: 0 });
    const up = normalizeVector(crossVectors(right, forward), { x: 0, y: 1, z: 0 });
    return {
        right,
        up,
    };
...
function buildPlayerFacingSpawnPosition(playerPosition, forward, basis, offset, engagementRangeMeters) {
    const allowedDistance = Math.max(NPCTEST_MIN_PLAYER_DISTANCE_METERS, toFiniteNumber(engagementRangeMeters, NPCTEST_CLUSTER_DISTANCE_METERS) * NPCTEST_ENGAGEMENT_RANGE_SCALE);
    const lateral = addVectors(scaleVector(basis.right, toFiniteNumber(offset && offset.x, 0)), scaleVector(basis.up, toFiniteNumber(offset && offset.y, 0)));
    ...
    const maxForwardDistanceForAllowed = Math.sqrt(Math.max(0, (allowedDistance ** 2) - (clampedLateralMagnitude ** 2)));
    const desiredForwardDistance = Math.max(NPCTEST_MIN_PLAYER_DISTANCE_METERS, Math.min(NPCTEST_CLUSTER_DISTANCE_METERS + toFiniteNumber(offset && offset.z, 0), maxForwardDistanceForAllowed || NPCTEST_MIN_PLAYER_DISTANCE_METERS));
    return addVectors(addVectors(playerPosition, scaleVector(forward, desiredForwardDistance)), clampedLateral);
}
function buildNpcTestSpawnPlan(shipEntity, selectedEntries, mode = "player") {
    ...
    const centerPosition = addVectors(playerPosition, scaleVector(forward, NPCTEST_CLUSTER_DISTANCE_METERS));
    const orderedEntries = normalizedMode === "ffa"
        ? [...selectedEntries]
        : [...selectedEntries].sort((left, right) => (toFiniteNumber(left && left.engagementRangeMeters, 0) -
            toFiniteNumber(right && right.engagementRangeMeters, 0)));
    const offsets = normalizedMode === "ffa"
        ? buildShellSphereOffsets(orderedEntries.length, NPCTEST_SPHERE_SPACING_METERS)
        : buildFilledSphereOffsets(orderedEntries.length, NPCTEST_PLAYER_SPHERE_SPACING_METERS);
    const entries = orderedEntries.map((entry, index) => {
        const rawPosition = addVectors(centerPosition, offsets[index] || { x: 0, y: 0, z: 0 });
        const spawnPosition = normalizedMode === "ffa"
            ? rawPosition
            : buildPlayerFacingSpawnPosition(playerPosition, forward, basis, offsets[index] || { x: 0, y: 0, z: 0 }, entry && entry.engagementRangeMeters);
        const facingDirection = normalizedMode === "ffa"
            ? normalizeVector(subtractVectors(centerPosition, spawnPosition), scaleVector(forward, -1))
            : normalizeVector(subtractVectors(playerPosition, spawnPosition), scaleVector(forward, -1));
        return {
            ...entry,
            spawnState: {
...
function buildNpcTestBehaviorOverrides(mode) {
    ...
    if (normalizedMode === "ffa") {
        return {
            movementMode: "hold",
            autoAggro: true,
            autoAggroTargetClasses: [ENTITY_TYPE.NPC],
...
function clearNpcTestControllersInSystem(systemID, operatorKind) {
    ...
    const existingControllers = npcService.getNpcOperatorSummary().filter((controller) => (toPositiveInt(controller && controller.systemID, 0) === normalizedSystemID &&
        String(controller && controller.operatorKind || "").trim() === normalizedOperatorKind));
    let removedCount = 0;
    for (const controller of existingControllers) {
        const destroyResult = npcService.destroyNpcControllerByEntityID(toPositiveInt(controller && controller.entityID, 0), {
            removeContents: true,
        });
...
function spawnNpcTestForSession(session, options = {}) {
    ...
    scene.broadcastAddBalls(spawnedEntries
...
//# sourceMappingURL=npcTestService.js.map