...
function normalizeSystemGateControl(systemID, control: Record<string, any> = {}) {
...
function normalizeControlState(state) {
  ...
  const startupRuleOverrides: Record<string, any> = {};
  const characterFlags: Record<string, any> = {};
  const systemGateControls: Record<string, any> = {};

  const rawOverrides =
    source.startupRuleOverrides && typeof source.startupRuleOverrides === "object"
      ? source.startupRuleOverrides
      : {};
  for (const [ruleID, rawOverride] of Object.entries<any>(rawOverrides)) {
  ...
  const rawCharacterFlags =
    source.characterFlags && typeof source.characterFlags === "object"
      ...
      : {};
  for (const [characterID, rawFlags] of Object.entries<any>(rawCharacterFlags)) {
...
function setSystemGateControl(systemID, updates: Record<string, any> = {}) {
  const normalizedSystemID = toPositiveInt(systemID, 0);
  if (!normalizedSystemID) {
    return {
      success: false,
