...
function getSceneStaticCandidates(scene, kind) {
    ...
    return scene.staticEntities.filter((entity) => entity && String(entity.kind || "").trim().toLowerCase() === normalizedKind);
}
function buildVirtualAnchor(descriptor = {}) {
    const position = cloneVector(descriptor.position);
    const direction = normalizeVector(descriptor.direction, { x: 1, y: 0, z: 0 });
    return {
        kind: String(descriptor.kind || "coordinates"),
        itemID: toPositiveInt(descriptor.itemID || descriptor.entityID || descriptor.anchorID, 0),
...
function filterCandidatesByDescriptor(candidates, descriptor = {}) {
    const numericItemID = toPositiveInt(descriptor.itemID ||
        descriptor.entityID ||
        descriptor.stationID ||
        descriptor.stargateID ||
        descriptor.anchorID, 0);
    if (numericItemID > 0) {
    ...
    if (celestialIndex > 0) {
        filtered = filtered.filter((candidate) => toPositiveInt(candidate && candidate.celestialIndex, 0) === celestialIndex);
    }
    if (orbitIndex > 0) {
        filtered = filtered.filter((candidate) => toPositiveInt(candidate && candidate.orbitIndex, 0) === orbitIndex);
    }
    const normalizedNameQuery = normalizeQuery(descriptor.nameQuery ||
        descriptor.name ||
        descriptor.itemName ||
        descriptor.stationName ||
        descriptor.stargateName);
    if (normalizedNameQuery) {
        const exactMatches = filtered.filter((candidate) => (normalizeQuery(candidate && candidate.itemName).includes(normalizedNameQuery)));
        if (exactMatches.length > 0) {
            filtered = exactMatches;
        }
    }
    const signalTrackerSiteKind = normalizeQuery(descriptor.siteKind || descriptor.signalTrackerSiteKind);
    if (signalTrackerSiteKind) {
        filtered = filtered.filter((candidate) => (normalizeQuery(candidate && candidate.signalTrackerSiteKind) === signalTrackerSiteKind));
    }
    const signalTrackerSiteFamily = normalizeQuery(descriptor.siteFamily || descriptor.signalTrackerSiteFamily);
    if (signalTrackerSiteFamily) {
        filtered = filtered.filter((candidate) => (normalizeQuery(candidate && (candidate.signalTrackerSiteFamily ||
            candidate.signalTrackerSignatureSiteFamily)) === signalTrackerSiteFamily));
    }
    const siteTemplateIDs = [
        String(descriptor.siteTemplateID ||
    ...
    if (siteTemplateIDs.length > 0) {
        ...
        filtered = filtered.filter((candidate) => allowedTemplateIDs.has(String(candidate && candidate.signalTrackerSiteTemplateID || "").trim()));
    }
    const entryObjectTypeIDs = [...new Set([
            toPositiveInt(descriptor.entryObjectTypeID || descriptor.signalTrackerEntryObjectTypeID, 0),
    ...
    if (entryObjectTypeIDs.length > 0) {
        ...
        filtered = filtered.filter((candidate) => allowedEntryObjectTypeIDs.has(toPositiveInt(candidate && (candidate.signalTrackerEntryObjectTypeID ||
            candidate.entryObjectTypeID), 0)));
    }
    const labelIncludesAny = normalizeQueryArray(descriptor.siteLabelIncludesAny || descriptor.labelIncludesAny);
    if (labelIncludesAny.length > 0) {
        filtered = filtered.filter((candidate) => {
            const candidateLabel = normalizeQuery(candidate && (candidate.signalTrackerSiteLabel ||
                candidate.itemName ||
                candidate.slimName));
...
function resolveAnchors(systemID, descriptor = {}) {
    ...
    const numericEntityID = toPositiveInt(descriptor && (descriptor.entityID ||
        descriptor.itemID ||
        descriptor.stationID ||
        descriptor.stargateID ||
        descriptor.anchorID), 0);
    ...
    const normalizedAsteroidBeltKind = normalizedKind === "asteroidbelt" || normalizedKind === "asteroid belt";
    const candidateKind = normalizedKind === "station" ||
...
function resolveSpawnDistance(anchorEntity, definition, options = {}) {
    const explicitSpawnDistance = Math.max(0, toFiniteNumber(options.spawnDistanceMeters, 0));
    if (explicitSpawnDistance > 0) {
        return explicitSpawnDistance;
    }
    const surfaceDistance = Math.max(0, toFiniteNumber(options.distanceFromSurfaceMeters, 0));
    if (surfaceDistance > 0) {
        return Math.max(1_000, toFiniteNumber(anchorEntity && anchorEntity.radius, 0) + surfaceDistance);
    }
    return Math.max(1_000, toFiniteNumber(definition && definition.profile && definition.profile.spawnDistanceMeters, 20_000));
}
function buildSpawnStateForDefinition(anchorEntity, definition, options = {}) {
    ...
    if (explicitSpawnState &&
        ...
        return {
            ...
            direction: normalizeVector(explicitSpawnState.direction, cloneVector(anchorEntity && anchorEntity.direction, { x: 1, y: 0, z: 0 })),
...
//# sourceMappingURL=npcAnchors.js.map