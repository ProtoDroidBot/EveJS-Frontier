...
const { EVERMORE_CUSTOMS_SPAWN_GROUP_ID, EVERMORE_GATE_STARTUP_RULE_PREFIX, isEverMoreGatePresenceSystem, } = require(path.join(__dirname, "../empireGatePresence/everMoreGatePresence"));
...
function collectAuthoredCoverage(authoredRules) {
    ...
    for (const rule of Array.isArray(authoredRules) ? authoredRules : []) {
        ...
        if (String(rule && rule.entityType || "").trim().toLowerCase() !== ENTITY_TYPE.CONCORD) {
            continue;
        }
        const selectorKind = String(rule && rule.anchorSelector && rule.anchorSelector.kind || "").trim().toLowerCase();
...
function buildEverMoreGateRule(systemID, displayedSecurity) {
    return {
        ...
        description: "Config-generated TQ-style EverMore/Villore Sec Ops gate presence for Jita-style high-security stargates.",
...
function buildGateRule(systemID, displayedSecurity) {
    return {
        ...
        description: "Config-generated default CONCORD gate checkpoint coverage for this high-security system.",
        aliases: [],
        enabled: true,
        systemIDs: [systemID],
        entityType: ENTITY_TYPE.CONCORD,
...
function buildStationRule(systemID, displayedSecurity) {
    return {
        ...
        description: "Config-generated default CONCORD station police screens for high-visibility high-security systems.",
        aliases: [],
        enabled: true,
        systemIDs: [systemID],
        entityType: ENTITY_TYPE.CONCORD,
...
function buildSignature(authoredRules) {
    ...
    return JSON.stringify({
        ...
        gateAutoAggroNpcsEnabled: config.npcDefaultConcordGateAutoAggroNpcsEnabled === true,
...
function buildConfiguredConcordStartupRules(authoredRules = []) {
    ...
    const { gateCoveredSystems, stationCoveredSystems } = collectAuthoredCoverage(authoredRules);
...
//# sourceMappingURL=npcDefaultConcordRules.js.map