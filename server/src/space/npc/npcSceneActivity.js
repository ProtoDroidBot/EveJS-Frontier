...
const { isAmbientStartupControllerRecord, isAmbientVirtualizationEnabled, } = require(path.join(__dirname, "./npcAmbientMaterialization"));
const { isCombatDormancyEnabled, isCombatDormantControllerRecord, } = require(path.join(__dirname, "./npcCombatDormancy"));
...
function getSceneNpcDeadlineMs(scene) {
    ...
    const respawnDeadlines = scene._npcStartupRespawnDeadlines && typeof scene._npcStartupRespawnDeadlines === "object"
...
function getSceneActivityState(scene, wallclockNow = Date.now()) {
    ...
    const sceneNowMs = typeof scene.peekSimTimeForWallclock === "function"
...
function buildStartupPresenceSummary(systemIDs) {
    ...
    for (const systemID of normalizedSystemIDs) {
        ...
        for (const controllerRecord of nativeNpcStore.listNativeControllersForSystem(systemID)) {
            ...
            const virtualized = isAmbientStartupControllerRecord(controllerRecord) ||
        ...
        for (const entry of startupControllersByEntityID.values()) {
            ...
            const entityType = String(controller.entityType || ENTITY_TYPE.NPC).trim().toLowerCase() === ENTITY_TYPE.CONCORD
                ? ENTITY_TYPE.CONCORD
                : ENTITY_TYPE.NPC;
...
//# sourceMappingURL=npcSceneActivity.js.map