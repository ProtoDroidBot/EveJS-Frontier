...
const pilotWorkDispatch = branch("selector", "work-type",
    ...
    branch("sequence", "refit-ship",
      ...
      action("runTaskRefitTree"),
    ),
    branch("sequence", "swap-ship",
      guard("assignedShipSwapJob"),
      action("runTaskShipSwapTree"),
    ),
    branch("sequence", "fit-another-npc",
      guard("assignedNpcFittingSupportJob"),
      action("runNpcFittingSupportTree"),
    ),
...
const pilotWork = branch("sequence", "pilot-assigned-work",
  ...
  branch("selector", "task-capability",
    branch("sequence", "prepare-unready-task",
      guard("currentShipLacksAssignedTaskCapability"),
      action("runTaskPreparationTree"),
    ),
    branch("sequence", "execute-ready-task",
      guard("currentShipCanPerformAssignedTaskOrIsPreparationJob"),
      pilotWorkDispatch,
    ),
    action("suspendWorkForUnknownCapability"),
  ),
);

const pilotTaskPreparation = branch("sequence", "pilot-task-preparation",
  guard("isCategorySixPilot"),
  guard("assignedTaskNeedsCapability"),
  guard("taskPreparationWakeDue"),
  branch("selector", "task-capability-plan",
    branch("sequence", "already-capable",
      guard("currentShipCanPerformAssignedTask"),
      guard("taskCapabilityPlanSettled"),
      action("completeTaskReadiness"),
    ),
    branch("sequence", "select-authorized-plan",
      action("checkpointTaskCapabilityPlan"),
      branch("selector", "refit-or-swap",
        branch("sequence", "refit-current-ship",
          guard("selectedRefitPlan"),
          action("runTaskRefitTree"),
        ),
        branch("sequence", "swap-to-capable-ship",
          guard("selectedShipSwapPlan"),
          action("runTaskShipSwapTree"),
        ),
        action("suspendTaskForEquipmentOrHull"),
      ),
    ),
    action("suspendTaskForEquipmentOrHull"),
  ),
...
const pilotTaskRefit = branch("sequence", "pilot-task-refit",
  guard("isCategorySixPilot"),
  guard("selectedRefitPlan"),
  branch("selector", "refit-progress",
    branch("sequence", "verify-committed-refit",
      guard("taskRefitCheckpointComplete"),
      branch("selector", "committed-refit-capability",
        guard("currentShipCanPerformAssignedTask"),
        action("suspendRefitForServiceItemsOrPolicy"),
      ),
    ),
    branch("sequence", "finish-applied-refit",
      guard("taskFitChangesCommitted"),
      action("journalCompleteTaskFit"),
    ),
    branch("sequence", "use-authorized-fitting-service",
      guard("taskRefitCheckpointPending"),
      guard("authorizedFittingServiceAvailable"),
      branch("selector", "arrive-at-fitting-service",
        guard("currentlyAtAuthorizedFittingService"),
        action("advanceToFittingService"),
      ),
      guard("currentlyAtAuthorizedFittingService"),
      guard("taskFitItemsAndPolicyCurrent"),
      guard("taskFitChangePending"),
      action("journalApplyNextTaskFitChange"),
    ),
    action("suspendRefitForServiceItemsOrPolicy"),
  ),
);

const pilotTaskShipSwap = branch("sequence", "pilot-task-ship-swap",
  guard("isCategorySixPilot"),
  guard("selectedShipSwapPlan"),
  branch("selector", "swap-progress",
    branch("sequence", "verify-committed-swap",
      guard("taskShipSwapCheckpointComplete"),
      branch("selector", "committed-swap-capability",
        guard("currentShipCanPerformAssignedTask"),
        action("suspendSwapForHullAccessOrTravel"),
      ),
    ),
    branch("sequence", "board-task-capable-ship",
      guard("taskShipSwapCheckpointPending"),
      guard("replacementShipAuthorizedAndStillEligible"),
      branch("selector", "arrive-at-replacement-ship",
        guard("currentlyAtReplacementShip"),
        action("advanceToReplacementShip"),
      ),
      guard("currentlyAtReplacementShip"),
      durableStep("commit-task-swap", "taskShipSwapCheckpointComplete", "journalTaskShipSwap"),
      guard("currentShipCanPerformAssignedTask"),
    ),
    action("suspendSwapForHullAccessOrTravel"),
  ),
...
const pilotNpcFittingSupport = branch("sequence", "pilot-fits-another-npc",
  guard("isCategorySixPilot"),
  guard("assignedNpcFittingSupportJob"),
  guard("npcFittingSupportWakeDue"),
  branch("selector", "fitting-support-progress",
    branch("sequence", "verify-committed-target-fit",
      guard("npcFittingSupportCheckpointComplete"),
      branch("selector", "committed-target-capability",
        guard("targetNpcFitMeetsRequestedCapability"),
        action("suspendNpcFittingSupport"),
      ),
    ),
    branch("sequence", "finish-applied-target-fit",
      guard("targetFitChangesCommitted"),
      action("journalCompleteTargetFit"),
    ),
    branch("sequence", "authorized-target-and-service",
      guard("npcFittingSupportCheckpointPending"),
      guard("targetNpcFittingPrincipalValid"),
      guard("npcToNpcFittingAuthorityCurrent"),
      guard("authorizedSharedFittingServiceAvailable"),
      branch("selector", "bring-actor-and-target-to-service",
        guard("actorAndTargetAtFittingService"),
        action("advanceActorOrTargetToFittingService"),
      ),
      guard("actorAndTargetAtFittingService"),
      guard("targetFitItemsAndPolicyCurrent"),
      guard("targetFitChangePending"),
      action("journalApplyNextTargetFitChange"),
    ),
    action("suspendNpcFittingSupport"),
  ),
);
