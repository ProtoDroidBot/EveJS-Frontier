...
function buildPerpendicular(direction) {
    ...
    return normalizeVector(crossProduct(resolvedDirection, DEFAULT_RIGHT), DEFAULT_FORWARD);
}
...
function collectSceneAnchors(scene, options = {}) {
    const anchors = [];
    const excludedEntityIDs = new Set(Array.isArray(options.excludeEntityIDs)
...
function measureNearestSurfaceDistanceMeters(position, anchors) {
    ...
    for (const anchor of anchors) {
        const surfaceDistanceMeters = Math.sqrt(distanceSquared(position, anchor.position)) - Math.max(0, toFiniteNumber(anchor.radius, 0));
...
function findSafeWarpOriginAnchor(scene, target, options = {}) {
    const targetPosition = cloneVector(target && target.position ? target.position : target);
    const targetDirection = normalizeVector(target && target.direction ? target.direction : options.direction, DEFAULT_RIGHT);
    const clearanceMeters = Math.max(1, toFiniteNumber(options.clearanceMeters, DEFAULT_CLEARANCE_METERS));
    const minDistanceMeters = Math.max(clearanceMeters + 1, toFiniteNumber(options.minDistanceMeters, DEFAULT_MIN_DISTANCE_METERS));
    const maxDistanceMeters = Math.max(minDistanceMeters, toFiniteNumber(options.maxDistanceMeters, DEFAULT_MAX_DISTANCE_METERS));
    const stepMeters = Math.max(1, toFiniteNumber(options.stepMeters, DEFAULT_STEP_METERS));
    const anchors = collectSceneAnchors(scene, options);
    const fallbackPosition = addVectors(targetPosition, scaleVector(normalizeVector(scaleVector(targetDirection, -1), {
        x: -1,
        y: 0,
        z: 0,
    }), minDistanceMeters));
    ...
    for (const direction of buildSearchDirections(targetDirection)) {
        for (let distanceMeters = minDistanceMeters; distanceMeters <= maxDistanceMeters + 1; distanceMeters += stepMeters) {
            const candidatePosition = addVectors(targetPosition, scaleVector(direction, distanceMeters));
            const nearestSurfaceDistanceMeters = measureNearestSurfaceDistanceMeters(candidatePosition, anchors);
            if (nearestSurfaceDistanceMeters >= clearanceMeters) {
                return {
                    position: candidatePosition,
                    direction: normalizeVector(subtractVectors(targetPosition, candidatePosition), targetDirection),
                    distanceMeters,
    ...
    return bestCandidate || {
        ...
        direction: normalizeVector(subtractVectors(targetPosition, fallbackPosition), targetDirection),
...
function resolveCollisionSafeScenePosition(scene, target, options = {}) {
    const position = cloneVector(target && target.position ? target.position : target);
    const entityRadiusMeters = Math.max(0, toFiniteNumber(options.entityRadiusMeters, target && target.radius));
    const clearanceMeters = Math.max(1, entityRadiusMeters + Math.max(0, toFiniteNumber(options.clearanceMeters, 100)));
    const anchors = collectSceneAnchors(scene, options);
    ...
    const escapeDistanceMeters = Math.max(0, clearanceMeters - nearestSurfaceDistanceMeters);
    const minimumSearchDistanceMeters = Math.max(1_000, escapeDistanceMeters + clearanceMeters);
...
//# sourceMappingURL=npcWarpOrigins.js.map