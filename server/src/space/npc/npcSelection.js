...
const { buildNpcDefinition, resolveNpcProfile, resolveNpcSpawnPool, getNpcSpawnPool, resolveNpcSpawnGroup, getNpcSpawnGroup, } = require(path.join(__dirname, "./npcData"));
const { resolveWeaponFamily, isMissileWeaponFamily, } = require(path.join(__dirname, "../combat/weaponDogma"));
...
function definitionMatchesRequiredWeaponFamily(definition, requiredWeaponFamily = "") {
    ...
    const firstChargeEntry = chargeEntries.find((entry) => toPositiveInt(entry && entry.typeID, 0) > 0) || null;
...
function buildDefinitionsForPool(pool, amount, expectedEntityType = "", definitionCache = new Map(), options = {}) {
    ...
    const eligibleEntries = Array.isArray(pool && pool.entries)
        ? pool.entries.filter((entry) => {
            const definition = getCachedNpcDefinition(entry && entry.profileID, definitionCache);
            if (!definition) {
                return false;
            }
    ...
    for (let index = 0; index < requestedAmount; index += 1) {
        ...
        const definition = getCachedNpcDefinition(chosenEntry.profileID, definitionCache);
        if (!definition) {
            return {
                success: false,
...
function buildDefinitionsForProfile(profileResolution, amount, expectedEntityType = "", options = {}) {
    const definition = getCachedNpcDefinition(profileResolution &&
        profileResolution.data &&
        profileResolution.data.profileID, new Map());
    if (!definition) {
        return {
            success: false,
            errorMsg: "NPC_DEFINITION_INCOMPLETE",
    ...
    return {
        ...
        data: {
            ...
            definitions: Array.from({ length: Math.max(1, toPositiveInt(amount, 1)) }, () => cloneValue(definition)),
...
function buildDefinitionsForSpawnGroup(group, options = {}) {
    ...
    for (const entry of entries) {
        ...
        if (String(entry && entry.profileID || "").trim()) {
            const definition = getCachedNpcDefinition(entry.profileID, definitionCache);
            if (!definition) {
                return {
                    success: false,
                    errorMsg: "NPC_DEFINITION_INCOMPLETE",
        ...
        if (String(entry && entry.spawnPoolID || "").trim()) {
            ...
            const poolResult = buildDefinitionsForPool(pool, entryCount, expectedEntityType, definitionCache, {
...
function resolveNpcSpawnPlan(query, options = {}) {
    ...
    if (!trimmedQuery) {
        if (defaultPoolID) {
            ...
            if (defaultPool) {
                return buildDefinitionsForPool(defaultPool, requestedAmount, expectedEntityType, new Map(), {
                    requiredWeaponFamily,
                });
            }
        }
        if (fallbackProfileID) {
            ...
            }, requestedAmount, expectedEntityType, {
                requiredWeaponFamily,
            });
        }
    }
    ...
    if (profileSuccess && !poolSuccess) {
        return buildDefinitionsForProfile(profileResolution, requestedAmount, expectedEntityType, {
            requiredWeaponFamily,
        });
    }
    if (poolSuccess && !profileSuccess) {
        return buildDefinitionsForPool(poolResolution.data, requestedAmount, expectedEntityType, new Map(), {
            requiredWeaponFamily,
        });
    }
    if (profileSuccess && poolSuccess) {
    ...
    const errorMsg = profileResolution.errorMsg === "PROFILE_AMBIGUOUS" ||
...
//# sourceMappingURL=npcSelection.js.map