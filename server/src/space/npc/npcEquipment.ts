...
function buildNpcSyntheticHullEffectModuleItem(entity, effectRecord, slotIndex = 0, options: Record<string, any> = {}) {
  const entityID = getNpcShipID(entity);
...
function getTypeEffectRecords(typeID) {
  return [...(getTypeDogmaEffects(typeID) || new Set<any>())]
...
function getNpcEntityEwarModules(entity) {
  ...
  const coveredFamilies = new Set<any>();
  ...
  specs.forEach((spec, index) => {
    ...
    const moduleItem: Record<string, any> = buildNpcSyntheticHullEffectModuleItem(
      entity,
