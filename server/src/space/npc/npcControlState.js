...
function normalizeSystemGateControl(systemID, control = {}) {
    ...
    return {
        ...
        gateConcordSpawnGroupID: normalizeString(source.gateConcordSpawnGroupID, DEFAULT_GATE_CONCORD_SPAWN_GROUP_ID),
        gateConcordRespawnEnabled: source.gateConcordRespawnEnabled !== false,
        gateConcordRespawnDelayMs: Math.max(1_000, toFiniteNumber(source.gateConcordRespawnDelayMs, DEFAULT_GATE_CONCORD_RESPAWN_DELAY_MS)),
        gateRatEnabled: source.gateRatEnabled === true,
        gateRatSpawnGroupID: normalizeString(source.gateRatSpawnGroupID, DEFAULT_GATE_RAT_SPAWN_GROUP_ID),
        gateRatRespawnEnabled: source.gateRatRespawnEnabled !== false,
        gateRatRespawnDelayMs: Math.max(1_000, toFiniteNumber(source.gateRatRespawnDelayMs, DEFAULT_GATE_RAT_RESPAWN_DELAY_MS)),
    };
}
function normalizeControlState(state) {
    ...
    const rawOverrides = source.startupRuleOverrides && typeof source.startupRuleOverrides === "object"
    ...
    const rawCharacterFlags = source.characterFlags && typeof source.characterFlags === "object"
...
function writeNpcControlState(nextState) {
    return database.write(NPC_CONTROL_STATE_TABLE, "/", normalizeControlState(nextState));
}
...
function getCharacterFlags(characterID) {
    ...
    const state = readNpcControlState();
    return normalizeCharacterFlags(state.characterFlags[String(normalizedCharacterID)] || null);
}
...
function getSystemGateControl(systemID) {
    ...
    return normalizeSystemGateControl(normalizedSystemID, runtimeSystemGateControls.get(String(normalizedSystemID)) || null);
}
function setSystemGateControl(systemID, updates = {}) {
    ...
    const merged = normalizeSystemGateControl(normalizedSystemID, {
...
//# sourceMappingURL=npcControlState.js.map