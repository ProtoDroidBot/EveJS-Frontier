...
const { TABLE: REFERENCE_TABLE, readStaticRows, } = require(path.join(__dirname, "../../../services/_shared/referenceData"));
const trigDrifterSpawnAuthority = require(path.join(__dirname, "./trigDrifterSpawnAuthority"));
...
const POCHVEN_HOME_NAME_SET = new Set(POCHVEN_HOME_SYSTEM_NAMES.map((name) => String(name).trim().toLowerCase()));
const POCHVEN_BORDER_NAME_SET = new Set(POCHVEN_BORDER_SYSTEM_NAMES.map((name) => String(name).trim().toLowerCase()));
...
const DRIFTER_VARIANT_SPECS = Object.freeze([
    ...
    },
    {
        ...
        sdeBehaviorProfile: {
            autoAggroTargetClasses: [ENTITY_TYPE.PLAYER, ENTITY_TYPE.DRONE],
...
function buildTrigAuthorityVariant(spec) {
    return {
        ...
        extraModuleTypeIDs: Array.isArray(spec.extraModuleTypeIDs)
            ...
            : [],
        aliases: buildAliasSet(spec.label, spec.key, spec.roleLabel, spec.hullLabel, `${spec.roleLabel} ${spec.hullLabel}`, `Triglavian ${spec.hullLabel}`),
    };
}
function buildDrifterAuthorityVariant(spec) {
    return {
        ...
        aliases: buildAliasSet(spec.label, spec.key, `Drifter ${spec.key}`, spec.aliases),
    };
}
...
function buildDrifterBehaviorProfile(variant, behaviorProfileID, reinforcementDefinitions = [], options = {}) {
    const family = normalizeDrifterBehaviorFamily(options.behaviorFamily || variant && variant.behaviorFamily);
    const combatOrbitDistanceMeters = Math.max(1_000, toInt(options.orbitDistanceMeters, toInt(variant && variant.spawnDistanceMeters, 20_000)));
    const reinforcementEnabled = family === DRIFTER_BEHAVIOR_FAMILY.DEFAULT ||
        family === DRIFTER_BEHAVIOR_FAMILY.HIVE;
    const entosisPriorityEnabled = family === DRIFTER_BEHAVIOR_FAMILY.DEFAULT;
    const regroupEnabled = family === DRIFTER_BEHAVIOR_FAMILY.DEFAULT ||
        family === DRIFTER_BEHAVIOR_FAMILY.HIVE ||
        family === DRIFTER_BEHAVIOR_FAMILY.ROAMING;
    const pursuitEnabled = family === DRIFTER_BEHAVIOR_FAMILY.DEFAULT;
    const idleAnchorOrbit = family === DRIFTER_BEHAVIOR_FAMILY.COMMANDER ||
...
function buildSdeEntityNpcBehaviorProfile(variant, behaviorProfileID) {
    const sdeBehaviorProfile = variant && variant.sdeBehaviorProfile && typeof variant.sdeBehaviorProfile === "object"
...
function buildWeightedLootEntry(typeID, weight, options = {}) {
    return {
        ...
        maxQuantity: Math.max(Math.max(1, toInt(options.minQuantity, 1)), toInt(options.maxQuantity, Math.max(1, toInt(options.minQuantity, 1)))),
...
function buildTrigLootTable(variant, lootTableID) {
    ...
    return {
        ...
        guaranteedEntries: [
            buildStackedLootEntry(TRIG_LOOT_TYPE_IDS.surveyDatabase, guaranteedSurveyDatabases),
        ],
...
function buildDrifterLootTable(variant, lootTableID) {
    ...
    return {
        ...
        guaranteedEntries: [
            buildStackedLootEntry(DRIFTER_LOOT_TYPE_IDS.sleeperDataLibrary, guaranteedSleeperLibraries),
        ],
        entries: weightedEntries,
        minEntries: 0,
        maxEntries: 1,
        ...
        loadout: {
            ...
            modules: [
                ...
                ...variant.extraModuleTypeIDs.map((moduleTypeID) => ({
                    typeID: moduleTypeID,
                    quantity: 1,
                    moduleRole: moduleTypeID === TRIG_TYPE_IDS.warpScramblerII
                        ? "tackle"
                        : "hostileUtility",
                })),
            ],
...
function buildDrifterDefinition(variant, options = {}) {
    ...
    const reinforcementVariant = variant.reinforcementVariantKey
        ...
        : null;
    const reinforcementDefinitions = reinforcementVariant && toInt(variant.reinforcementAmount, 0) > 0
    ...
    const behaviorProfile = variant.behaviorAuthority === "sdeEntityNpc"
        ...
        : buildDrifterBehaviorProfile(variant, behaviorProfileID, reinforcementDefinitions, options);
    return {
        profile: {
            profileID,
            name: variant.label,
...
function buildDrifterFamilyProfileDefinition(variantKey, family) {
    ...
    return {
        profile: {
            ...
            aliases: buildAliasSet(`${familyLabel} ${variant.label}`, `${normalizedFamily} ${variant.label}`, `${normalizedFamily} ${variant.key}`, `${normalizedFamily} drifter ${variant.key}`),
        },
        behaviorProfile: {
            ...(definition.behaviorProfile || {}),
            behaviorProfileID,
...
function buildSpawnSiteRow(spawnSiteID, name, description, aliases, systemID, anchor, spawnGroupID, entityType = ENTITY_TYPE.NPC) {
    return {
        spawnSiteID,
        name,
        description,
...
function buildStartupRuleRow(startupRuleID, name, description, aliases, scope, spawnGroupID, anchorSelector, options = {}) {
    return {
        startupRuleID,
        name,
        description,
...
function buildNamedTrigPool(spawnPoolID, name, description, aliases, variantKeys) {
    ...
    return buildSpawnPoolRow(spawnPoolID, name, description, aliases, entries);
}
...
function listPochvenSystems() {
    return readStaticRows(REFERENCE_TABLE.SOLAR_SYSTEMS)
        ...
        .sort((left, right) => (toInt(left && left.solarSystemID, 0) - toInt(right && right.solarSystemID, 0)));
}
function listPochvenStargates() {
    const pochvenSystemIDs = new Set(listPochvenSystems()
...
function buildPochvenGateSpawnSites() {
    const systemNameByID = new Map(listPochvenSystems().map((row) => [
    ...
    return listPochvenStargates().map((stargate) => {
        ...
        return buildSpawnSiteRow(`parity_trig_pochven_gate_site_${gateItemID}`, `${systemName} ${gateName} Triglavian ${family[0].toUpperCase()}${family.slice(1)} Gate Patrol`, `Generated ${family} Pochven gate-side Triglavian patrol site for ${gateName} in ${systemName}.`, [
            systemName,
            gateName,
            `${family} pochven trig`,
...
function buildPochvenStationSpawnSites() {
    ...
    return listPochvenStations().map((station) => {
        ...
        return buildSpawnSiteRow(`parity_trig_pochven_station_site_${stationID}`, `${systemName} ${stationName} Triglavian ${family[0].toUpperCase()}${family.slice(1)} Station Presence`, `Generated ${family} Pochven station-side Triglavian presence site for ${stationName} in ${systemName} using repo-owned station anchors and the shared native NPC startup path.`, [
            systemName,
            stationName,
...
function buildPochvenGateStartupRules() {
    ...
    return [
    ...
    ].map((entry) => buildStartupRuleRow(entry.startupRuleID, entry.name, entry.description, [
        ...entry.aliases,
        ...
        respawnEnabled: true,
        respawnDelayMs: entry.family === POCHVEN_SYSTEM_FAMILY.HOME
            ? 20_000
...
function buildDefensiveDrifterSiteBehaviorOverrides(proximityRangeMeters) {
    ...
    return {
        ...
        proximityAggroTargetClasses: [ENTITY_TYPE.PLAYER],
        allowPodKill: true,
        returnToHomeWhenIdle: true,
        idleAnchorOrbit: true,
    };
...
function buildKnownSpaceDrifterStartupRules() {
    ...
    if (observatorySystemIDs.length > 0) {
        rules.push(buildStartupRuleRow("parity_drifter_known_space_observatory_presence_startup", "Drifter Known-Space Observatory Presence Startup", "Generated baseline Drifter observatory-side presence for known-space Jove Observatory systems using repo-owned observatory system authority and shared signature-site anchors.", [
    ...
    if (wormholeSystemIDs.length > 0) {
        rules.push(buildStartupRuleRow("parity_drifter_known_space_wormhole_presence_startup", "Drifter Known-Space Wormhole Presence Startup", "Generated baseline Drifter wormhole-side presence for known-space unidentified-wormhole systems using repo-owned observatory-derived system authority and shared signature-site anchors.", [
...
function buildDrifterSpaceHiveStartupRules() {
    ...
    return [
        buildStartupRuleRow("parity_drifter_space_hive_guard_startup", "Drifter-Space Hive Guard Startup", "Generated baseline Drifter-space hive-side guard presence for the five repo-owned Drifter systems using exact generated hive signature anchors and the shared native NPC startup path.", [
...
function buildRolePools(variants, familyPrefix) {
    ...
    for (const [roleKey, entries] of byRole.entries()) {
        pools.push(buildSpawnPoolRow(`${familyPrefix}_${roleKey}`, `Triglavian ${roleKey[0].toUpperCase()}${roleKey.slice(1)} Pool`, `Generated Triglavian ${roleKey} parity pool backed by verified native-path hull definitions.`, [`trig ${roleKey}`, roleKey, `${roleKey} triglavian`], entries));
    }
    return pools;
}
function buildTrigDrifterDataset() {
    ...
    const trigPools = [
        buildSpawnPoolRow("parity_trig_all", "Triglavian Native Parity Pool", "All generated Triglavian hull variants currently proven on the native NPC path.", ["trig", "triglavians", "triglavian", "trig all"], TRIG_VARIANTS.map((variant) => ({
            profileID: getTrigProfileID(variant),
            weight: 1,
        }))),
        ...buildRolePools(TRIG_VARIANTS, "parity_trig_role"),
        buildNamedTrigPool("parity_trig_pochven_gate_tackle", "Pochven Gate Tackle Pool", "Generated baseline tackle pool for natural Pochven gate-side Trig patrols.", ["pochven tackle", "trig gate tackle"], [
            "anchoring_damavik",
            "anchoring_kikimora",
            "tangling_damavik",
            "tangling_kikimora",
        ]),
        buildNamedTrigPool("parity_trig_pochven_gate_pressure", "Pochven Gate Pressure Pool", "Generated baseline pressure and disruption pool for natural Pochven gate-side Trig patrols.", ["pochven pressure", "trig gate pressure"], [
            "starving_damavik",
            "starving_vedmak",
            "starving_drekavac",
            "blinding_damavik",
        ...
        buildNamedTrigPool("parity_trig_pochven_gate_support", "Pochven Gate Support Pool", "Generated baseline support pool for natural Pochven gate-side Trig patrols.", ["pochven support", "trig gate support"], [
            "renewing_damavik",
            "renewing_rodiva",
            "renewing_leshak",
        ]),
        buildNamedTrigPool("parity_trig_pochven_gate_line", "Pochven Gate Line Pool", "Generated baseline line-combat pool for natural Pochven gate-side Trig patrols.", ["pochven line", "trig gate line"], [
            "liminal_damavik",
            "liminal_kikimora",
            "liminal_vedmak",
            "liminal_rodiva",
    ...
    const drifterPools = [
        buildSpawnPoolRow("parity_drifter_all", "Drifter Native Parity Pool", "All generated Drifter hull variants currently proven on the native NPC path.", ["drifter", "drifters", "drifter all"], DRIFTER_VARIANTS.map((variant) => ({
            profileID: getDrifterProfileID(variant),
            weight: variant.key === "strategos" ? 1 : 2,
        }))),
        buildSpawnPoolRow("parity_drifter_response_callers", "Drifter Response Callers", "Generated Drifter hulls that can initiate reinforcement behavior on the native path.", ["drifter lancer", "lancer", "reinforcement drifter"], DRIFTER_VARIANTS
        ...
        buildNamedDrifterPool("parity_drifter_superweapon_line", "Drifter Superweapon Line", "Generated Drifter hulls that expose the live native-path superweapon lane.", ["drifter superweapon", "drifter line", "superweapon drifter"], [
            "cruiser",
            "battleship",
            "commander",
            "polemarkos",
        ...
        buildNamedDrifterPool("parity_drifter_response_battleships", "Drifter Response Battleships", "Generated Drifter response-side battleship hulls backed by repo-owned native-path authority.", ["drifter response", "response drifter", "response battleship"], [
            "response",
            "recon",
        ]),
        buildNamedDrifterPool("parity_drifter_disruption_cruisers", "Drifter Disruption Cruisers", "Generated Drifter cruiser hulls with live native-path tackle, web, neut, or strike pressure lanes.", ["drifter disruption cruiser", "drifter cruiser screen", "drifter cruiser pressure"], [
            "strike_cruiser",
            "entanglement",
            "nullwarp",
            "nullcharge",
        ...
        buildNamedDrifterPool("parity_drifter_advanced_strike", "Drifter Advanced Strike Line", "Generated advanced Drifter line hulls backed by repo-owned native-path authority.", ["drifter advanced strike", "advanced drifter", "polemarkos raider"], [
            "polemarkos",
            "raider",
            "navarkos",
            "assault",
        ]),
        buildSpawnPoolRow("parity_drifter_roaming_family", "Drifter Roaming Family", "Generated explicit roaming-family Drifter authority for the shared native NPC path.", ["drifter roaming family", "roaming drifter family", "roaming drifter"], [{
                profileID: "parity_drifter_roaming_battleship",
                weight: 1,
            }]),
        buildSpawnPoolRow("parity_drifter_dungeon_family", "Drifter Dungeon Family", "Generated explicit dungeon/site-family Drifter authority for the shared native NPC path.", ["drifter dungeon family", "dungeon drifter family", "dungeon drifter"], [{
                profileID: "parity_drifter_dungeon_battleship",
                weight: 1,
            }]),
        buildSpawnPoolRow("parity_drifter_hive_family", "Drifter Hive Family", "Generated explicit hive-family Drifter authority for the shared native NPC path.", ["drifter hive family", "hive drifter family", "hive drifter"], [
            {
    ...
    const spawnGroups = [
        buildSpawnGroupRow("parity_drifter_lancer_response_screen", "Drifter Lancer Response Screen", "Generated grouped Drifter response authority for native-path reinforcement and future authored placement work.", [
        ...
        buildSpawnGroupRow("parity_drifter_superweapon_screen", "Drifter Superweapon Screen", "Generated grouped Drifter superweapon authority for native-path validation and future authored placement work.", [
        ...
        buildSpawnGroupRow("parity_drifter_advanced_strike_pair", "Drifter Advanced Strike Pair", "Generated grouped advanced Drifter strike authority backed by repo-owned native-path hull families.", [
        ...
        buildSpawnGroupRow("parity_drifter_hive_guard_screen", "Drifter Hive Guard Screen", "Generated grouped hive-family Drifter authority for natural Drifter-space hive entrance placement on exact generated signature anchors.", [
            "drifter hive guard",
            "drifter hive screen",
        ], [
            {
        ...
        buildSpawnGroupRow("parity_trig_pochven_border_gate_patrol", "Triglavian Pochven Border Gate Patrol", "Generated lighter mixed-role Triglavian patrol for natural Pochven border-system gate materialization on explicit stargate anchors.", [
        ...
        buildSpawnGroupRow("parity_trig_pochven_internal_gate_patrol", "Triglavian Pochven Internal Gate Patrol", "Generated mid-weight mixed-role Triglavian patrol for natural Pochven internal-system gate materialization on explicit stargate anchors.", [
        ...
        buildSpawnGroupRow("parity_trig_pochven_home_gate_patrol", "Triglavian Pochven Home Gate Patrol", "Generated heavier mixed-role Triglavian patrol for natural Pochven home-system gate materialization on explicit stargate anchors.", [
        ...
        buildSpawnGroupRow("parity_trig_pochven_border_station_presence", "Triglavian Pochven Border Station Presence", "Generated conservative station-side Triglavian presence for natural Pochven border-system station materialization on explicit station anchors.", [
        ...
        buildSpawnGroupRow("parity_trig_pochven_internal_station_presence", "Triglavian Pochven Internal Station Presence", "Generated conservative station-side Triglavian presence for natural Pochven internal-system station materialization on explicit station anchors.", [
        ...
        buildSpawnGroupRow("parity_trig_pochven_home_station_presence", "Triglavian Pochven Home Station Presence", "Generated heavier station-side Triglavian presence for natural Pochven home-system station materialization on explicit station anchors.", [
        ...
        ...TRIG_VARIANTS.map((variant) => buildSpawnGroupRow(`parity_trig_${variant.key}_solo`, `${variant.label} Singleton`, `Generated singleton group for ${variant.label}.`, [variant.label, variant.key, `${variant.hullLabel} trig`], [{ profileID: getTrigProfileID(variant), count: 1 }])),
        ...DRIFTER_VARIANTS.map((variant) => buildSpawnGroupRow(`parity_drifter_${variant.key}_solo`, `${variant.label} Singleton`, `Generated singleton group for ${variant.label}.`, [variant.label, variant.key, "drifter"], [{ profileID: getDrifterProfileID(variant), count: 1 }])),
        ...drifterFamilyDefinitions.map((definition) => buildSpawnGroupRow(`${definition.profile.profileID}_solo`, `${definition.profile.name} Singleton`, `Generated explicit singleton group for ${definition.profile.name}.`, [definition.profile.name, ...(Array.isArray(definition.profile.aliases) ? definition.profile.aliases : [])], [{ profileID: definition.profile.profileID, count: 1 }])),
    ];
    const spawnSites = [
...
//# sourceMappingURL=trigDrifterNpcCatalog.js.map