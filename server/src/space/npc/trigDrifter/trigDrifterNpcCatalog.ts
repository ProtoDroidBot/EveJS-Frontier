...
function normalizeDrifterBehaviorFamily(value) {
  const normalizedValue = String(value || "")
    .trim()
    .toLowerCase();
  if (Object.values<string>(DRIFTER_BEHAVIOR_FAMILY).includes(normalizedValue)) {
    return normalizedValue;
  }
  return DRIFTER_BEHAVIOR_FAMILY.DEFAULT;
}

function buildDrifterBehaviorProfile(variant, behaviorProfileID, reinforcementDefinitions: any[] = [], options: Record<string, any> = {}) {
...
function buildWeightedLootEntry(typeID, weight, options: Record<string, any> = {}) {
  return {
    typeID: toInt(typeID, 0),
...
function buildTrigDefinition(variant, options: Record<string, any> = {}) {
...
function buildDrifterDefinition(variant, options: Record<string, any> = {}) {
...
function listPochvenStargates() {
  const pochvenSystemIDs = new Set<any>(
    listPochvenSystems()
...
function buildPochvenGateSpawnSites() {
  const systemNameByID = new Map<any, any>(
