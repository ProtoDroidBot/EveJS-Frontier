...
const { ITEM_FLAGS, grantItemToCharacterLocation, } = require(path.join(__dirname, "../../services/inventory/itemStore"));
...
function getGenericLootPool() {
    ...
    cachedGenericLootPoolByTypeID = new Map(cachedGenericLootPool.map((entry) => [toPositiveInt(entry && entry.typeID, 0), entry]));
    return cachedGenericLootPool;
}
...
function isBlueprintType(itemType) {
    return (toPositiveInt(itemType && itemType.categoryID, 0) === 9 ||
...
function selectorAllowsSpecialGrade(selector = {}) {
    ...
    return (selector.allowSpecialGrade === true ||
...
function itemMatchesLootSelector(itemType, selector = {}) {
    ...
    if (kind === "trash" || kind === "salvage") {
        return ((RULE_TRASH_GROUP_NAMES.has(groupName) ||
...
function buildExplicitLootEntry(entrySpec = {}) {
    ...
    const minQuantity = Math.max(1, toPositiveInt(entrySpec.minQuantity, toPositiveInt(entrySpec.quantity, 1)));
    const maxQuantity = Math.max(minQuantity, toPositiveInt(entrySpec.maxQuantity, minQuantity));
    const singleton = resolveLootSingleton(itemType, typeof entrySpec.singleton === "boolean" ? entrySpec.singleton : null);
    return {
        itemType,
...
function buildRuleLootEntry(itemType, selector = {}, lootTable = {}) {
    ...
    const singleton = resolveLootSingleton(itemType, typeof selector.singleton === "boolean" ? selector.singleton : null);
    const quantity = singleton
        ? 1
        : rollQuantity(selector.minQuantity ?? selector.quantityMin ?? lootTable.stackableMinQuantity, selector.maxQuantity ?? selector.quantityMax ?? lootTable.stackableMaxQuantity);
    return {
        itemType,
        typeID: itemType.typeID,
        name: itemType.name,
...
function getBaseRuleID(lootTable = null) {
    return normalizeText(lootTable && (lootTable.baseRuleID ||
...
function rollNpcLootEntriesInternal(lootTable = null, visitedBaseRuleIDs = new Set()) {
    ...
    if (baseRuleID && !visitedBaseRuleIDs.has(baseRuleID)) {
        ...
        baseEntries.push(...rollNpcLootEntriesInternal(resolveBaseLootTable(baseRuleID), visitedBaseRuleIDs));
    }
    ...
    if (normalizeLowerText(lootTable.mode) === "rule" ||
    ...
    const hasExplicitLootEntries = Array.isArray(lootTable.entries) &&
        lootTable.entries.length > 0;
    const hasGuaranteedLootEntries = Array.isArray(lootTable.guaranteedEntries) &&
    ...
    if (lootTable.disableRandomLoot === true ||
    ...
    for (let index = 0; index < entryCount; index += 1) {
        ...
        const stackableMaxQuantity = Math.max(stackableMinQuantity, toPositiveInt(lootTable.stackableMaxQuantity, stackableMinQuantity));
...
function seedNpcShipLoot(characterID, shipID, lootTable = null, options = {}) {
    ...
    for (const lootEntry of lootEntries) {
        const grantResult = grantItemToCharacterLocation(characterID, shipID, ITEM_FLAGS.CARGO_HOLD, lootEntry.itemType, lootEntry.quantity, {
...
//# sourceMappingURL=npcLoot.js.map