...
function refreshNativeNpcPilotIdentity(characterID) {
  ...
  if (!pilot || !pilot.activeEntityID) return;
  const record = nativeNpcStore.getNativeEntity(pilot.activeEntityID);
  if (!record || record.npcCharacterID !== pilot.characterID) return;
  ...
  if (entity && entity.npcCharacterID === pilot.characterID) applyNpcPilotIdentity(entity, pilot);
}
...
function scheduleNpcIffVerdictRefresh(systemID) {
  ...
  try {
    const iffAbilityHandlers = require(path.join(
      __dirname,
      "../../services/frontier/iffAbilityHandlers",
    ));
    ...
    log.warn(
      `[NativeNpc] IFF verdict refresh failed system=${numericSystemID}: ` +
...
function isNativeAmbientRuleOptions(options: Record<string, any> = {}) {
...
function resolveNativeSpawnContextForSystem(systemID, options: Record<string, any> = {}) {
  const numericSystemID = toPositiveInt(systemID, 0);
  if (!numericSystemID) {
    return {
      success: false,
...
function buildNativeModuleRecords(entityRecord, definition, options: Record<string, any> = {}) {
...
function buildNativeCargoRecords(entityRecord, moduleRecords, definition, options: Record<string, any> = {}) {
...
function resolveNativeRuntimeKind(options: Record<string, any> = {}) {
...
function isProfiledPassiveRoamingEnabled(definitionOrController) {
  const behaviorProfile =
    definitionOrController && definitionOrController.behaviorProfile &&
...
function buildNativeControllerRecord(context, definition, entityRecord, spawnState, options: Record<string, any> = {}) {
  const runtimeKind = resolveNativeRuntimeKind(options);
  const nextThinkAtMs =
    (
      runtimeKind === "nativeAmbient" &&
      !isProfiledPassiveRoamingEnabled(definition)
...
function buildNativeRuntimeShipSpec(entityRecord, definition = null) {
  const npcFactionKey = String(
    entityRecord.npcFactionKey ||
...
function applyNativeRuntimeNpcPresentation(entity, entityRecord, definition = null) {
  ...
  return applyNativeNpcHullCombatOverrides(
    applyCapitalNpcCombatOverrides(
      applyConcordCombatOverrides(entity, entityRecord),
      entityRecord,
      definition,
    ),
    entityRecord,
...
function registerNativeRuntimeController(entityRecord, controllerRecord, definition) {
  ...
  return registerController({
    ...
    runtimeKind,
    nextThinkAtMs:
      runtimeKind === "nativeAmbient" && !passiveRoamingEnabled
        ? Number.MAX_SAFE_INTEGER
...
function materializeStoredNativeController(scene, entityID, options: Record<string, any> = {}) {
  if (!scene) {
    return {
      success: false,
...
function buildStoredEntityRecordFromRuntimeEntity(entityRecord, runtimeEntity) {
  return {
    ...
    targetEntityID: toPositiveInt(
      runtimeEntity && runtimeEntity.targetEntityID,
      toPositiveInt(entityRecord && entityRecord.targetEntityID, 0),
    ),
    followRange: Math.max(0, toFiniteNumber(
      runtimeEntity && runtimeEntity.followRange,
      entityRecord && entityRecord.followRange,
    )),
    orbitDistance: Math.max(0, toFiniteNumber(
      runtimeEntity && runtimeEntity.orbitDistance,
      entityRecord && entityRecord.orbitDistance,
    )),
    maxVelocity: Math.max(0, toFiniteNumber(
      runtimeEntity && runtimeEntity.maxVelocity,
      entityRecord && entityRecord.maxVelocity,
    )),
...
function persistNativeRuntimeEntity(runtimeEntity, options: Record<string, any> = {}) {
  ...
  const result = nativeNpcStore.upsertNativeEntity(
...
function dematerializeNativeController(controller, options: Record<string, any> = {}) {
...
function spawnNativeNpcEntityInContext(context, definition, options: Record<string, any> = {}) {
  ...
  const entityRecord: Record<string, any> = {
    ...
    npcFactionKey: String(
      options.factionStringOnlyID ||
...
function spawnNativeDefinitionsInContext(context, selectionResult, options: Record<string, any> = {}) {
  const spawned: any[] = [];
...
function spawnNativeNpcEntityInSystem(systemID, options: Record<string, any> = {}) {
...
function spawnNativeNpcGroupInSystem(systemID, options: Record<string, any> = {}) {
  const contextResult = resolveNativeSpawnContextForSystem(systemID, options);
  if (!contextResult.success || !contextResult.data) {
    return contextResult;
  }
