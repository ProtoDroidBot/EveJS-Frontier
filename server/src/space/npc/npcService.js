...
const { buildChildEntityScopeMetadata, canEntitiesInteractLocally, resolveEntityInteractionScope, } = require(path.join(__dirname, "../destiny/identity/interactionScope"));
const wormholeAuthority = require(path.join(__dirname, "../../services/exploration/wormholes/wormholeAuthority"));
const { buildNpcDefinition, listNpcProfiles, listNpcSpawnPools, listNpcSpawnGroups, listNpcSpawnSites, listNpcStartupRules, resolveNpcProfile, resolveNpcSpawnSite, } = require(path.join(__dirname, "./npcData"));
const { resolveNpcSpawnPlan, resolveNpcSpawnGroupPlan, } = require(path.join(__dirname, "./npcSelection"));
const { getControllerByEntityID, listControllers, listControllersBySystem, } = require(path.join(__dirname, "./npcRegistry"));
const { tickScene: tickBehaviorScene, issueManualOrder, setBehaviorOverrides, normalizeBehaviorOverrides, noteIncomingAggression, noteDungeonArrivalResponse, resolveNpcDungeonArrivalDisposition, } = require(path.join(__dirname, "./npcBehaviorLoop"));
const { requestNpcSupport, } = require(path.join(__dirname, "./npcSupportCoordinator"));
const { GATE_OPERATOR_KIND, getStartupRuleOverride, setStartupRuleEnabledOverride, getSystemGateControl, setSystemGateControl, toggleCharacterInvulnerability, setCharacterInvulnerability, isCharacterInvulnerable, listDynamicStartupRulesForSystem, getDynamicGateStartupRuleID, } = require(path.join(__dirname, "./npcControlState"));
const { toFiniteNumber, toPositiveInt, cloneVector, normalizeVector, resolveAnchors, resolveAnchor, } = require(path.join(__dirname, "./npcAnchors"));
const { resolveSessionCharacterID } = require(path.join(__dirname, "../../services/_shared/sessionIdentity"));
const nativeNpcService = require(path.join(__dirname, "./nativeNpcService"));
const { parseNpcCustomInfo, cleanupLegacySyntheticNpcShips, destroyLegacySyntheticNpcController, } = require(path.join(__dirname, "./legacySyntheticNpcCleanup"));
const nativeNpcStore = require(path.join(__dirname, "./nativeNpcStore"));
const { isAmbientStartupRuleVirtualizable, } = require(path.join(__dirname, "./npcAmbientMaterialization"));
const { isCombatStartupRuleDormancyEligible, } = require(path.join(__dirname, "./npcCombatDormancy"));
const { getEverMoreGateCustomsExpectedCount, getEverMoreGateCustomsLayout, } = require(path.join(__dirname, "../empireGatePresence/everMoreGatePresence"));
const { ENTITY_TYPE, } = require(path.join(__dirname, "../entityConstants"));
const { resolveNpcTargetIdentification, } = require(path.join(__dirname, "../../config/npcFactionConfig"));
...
function resolveSpawnContextForSystem(systemID, options = {}) {
    ...
    const anchorDescriptor = options.anchorDescriptor ||
        (options.position
            ? {
                kind: "coordinates",
...
function spawnNpcBatchForSession(session, options = {}) {
    ...
    const entityScopeMetadata = Object.hasOwn(spawnOptions, "entityScopeMetadata")
    ...
    const batchResult = nativeNpcService.spawnNativeDefinitionsInContext(contextResult.data, selectionResult, {
        ...
        skipInitialBehaviorTick: spawnOptions.skipInitialBehaviorTick === true
            ? true
            : spawnOptions.skipInitialBehaviorTick === false
                ? false
                : true,
        preferredTargetID: toPositiveInt(spawnOptions.preferredTargetID, toPositiveInt(session && session._space && session._space.shipID, 0)),
    ...
    if (batchResult &&
        ...
        scene &&
        spawnOptions.skipInitialBehaviorTick !== false) {
        const wakeAtMs = scene.getCurrentSimTimeMs() + Math.max(1000, toFiniteNumber(scene._tickIntervalMs, 1000));
...
function resolveNpcTypeProfile(typeID, profiles, typeName = "") {
    ...
    const matches = (Array.isArray(profiles) ? profiles : [])
        .filter((profile) => (String(profile && profile.entityType || "").toLowerCase() === ENTITY_TYPE.NPC &&
        toPositiveInt(profile && profile.shipTypeID, 0) === numericTypeID))
        .sort((left, right) => {
        const score = (profile) => ((String(profile.name || "").trim().toLowerCase() === normalizedTypeName ? 2 : 0) +
...
function spawnNpcTypeBatchForSession(session, typeID, amount = 1) {
    ...
    const profile = resolveNpcTypeProfile(numericTypeID, listNpcProfiles(), itemType && itemType.name);
    if (!profile) {
...
function spawnNpcPilotShipBatchForSession(session, typeID, amount = 1, options = {}) {
    ...
    const definition = buildNpcDefinition(`npc_${factionKey ? `${factionKey}_` : ""}pilot_ship_${numericTypeID}`);
    if (!definition) {
...
function spawnNpcSite(siteQuery, options = {}) {
    const spawnOptions = normalizeNpcSpawnOptions(options);
    const siteResolution = resolveNpcSpawnSite(siteQuery, String(spawnOptions.fallbackSpawnSiteID || ""));
    ...
    const sharedSiteSpawnOptions = {
        ...
        distanceFromSurfaceMeters: toFiniteNumber(site.anchor && site.anchor.distanceFromSurfaceMeters, 0),
        spreadMeters: toFiniteNumber(site.anchor && site.anchor.spreadMeters, 0),
        formationSpacingMeters: toFiniteNumber(site.anchor && site.anchor.formationSpacingMeters, 0),
        anchorKind: contextResult.data.anchorKind,
        anchorName: contextResult.data.anchorLabel,
        anchorID: toPositiveInt(contextResult.data.anchorEntity && contextResult.data.anchorEntity.itemID, 0),
    };
...
function spawnNpcSiteForSession(session, siteQuery, options = {}) {
    ...
    const siteResolution = resolveNpcSpawnSite(siteQuery, String(options.fallbackSpawnSiteID || ""));
    if (!siteResolution.success || !siteResolution.data) {
        return siteResolution;
    }
...
function ruleAppliesToSystem(rule, systemID) {
    ...
    const wormholeClassID = wormholeAuthority.getSystemClassID(numericSystemID, Number(system && system.security));
    if (wormholeClassID > 0 &&
...
function isGateOperatorStartupRule(rule) {
    ...
    return (operatorKind === GATE_OPERATOR_KIND.CONCORD ||
...
function listStartupRulesForSystem(systemID) {
    ...
    const numericSystemID = toPositiveInt(systemID, 0);
    const startupRules = listNpcStartupRules().filter((rule) => ruleAppliesToSystem(rule, numericSystemID) && isStartupRuleEnabled(rule));
    const dynamicRules = listDynamicStartupRulesForSystem(numericSystemID);
    const dynamicOperatorKinds = new Set(dynamicRules
        ...
        .filter(Boolean));
    const filteredStartupRules = startupRules.filter((rule) => (!dynamicOperatorKinds.has(String(rule && rule.operatorKind || "").trim())));
    const startupRuleOperatorKinds = new Set(filteredStartupRules
        .map((rule) => String(rule && rule.operatorKind || "").trim())
        .filter(Boolean));
    const filteredDynamicRules = dynamicRules.filter((rule) => (dynamicOperatorKinds.has(String(rule && rule.operatorKind || "").trim()) ||
...
function collectExistingStartupGroupSlots(scene, startupRuleID, anchorID) {
    ...
    const visitController = (controller, requireLiveEntity) => {
        if (toPositiveInt(controller && controller.systemID, 0) !== normalizedSystemID ||
        ...
        const identitySlot = String(controller.npcIdentitySlot || entityRecord && entityRecord.npcIdentitySlot || "");
...
function countMissingExactEverMoreSlots(scene, rule, anchor) {
    ...
    const existing = collectExistingStartupControllerSlots(scene, rule && rule.startupRuleID, anchorID);
...
function spawnExactEverMoreGatePresenceForAnchor(scene, rule, anchor, sharedSpawnOptions) {
    ...
    for (let index = 0; index < layout.length; index += 1) {
        ...
        const position = addVectors(cloneVector(anchor && anchor.position), cloneVector(slot && slot.offset));
        const targetEntity = resolveEverMoreOrbitTarget(scene, anchor, existing.slots, spawnedSlots, slot) ||
            anchor;
        ...
        const spawnResult = nativeNpcService.spawnNativeNpcEntityInContext(context, definition, {
            ...sharedSpawnOptions,
            broadcast: false,
            ...
            spawnStateOverride: {
                ...
                direction: normalizeVector(subtractVectors(targetPoint, position), cloneVector(anchor && anchor.direction, { x: 1, y: 0, z: 0 })),
                targetPoint,
                mode: "ORBIT",
        ...
        const spawnedTarget = spawnResult.data.entity ||
    ...
    if (scene && sharedSpawnOptions.broadcast !== false && sharedSpawnOptions.materializeRuntime !== false) {
        scene.broadcastAddBalls(spawned
            ...
            .filter(Boolean), sharedSpawnOptions.excludedSession || null, {
            freshAcquire: true,
...
function spawnStartupRuleInScene(scene, rule) {
    ...
    const shouldVirtualizeCombatSpawns = !isAmbientStartupRule &&
    ...
    for (const anchor of anchorsResult.data.anchors) {
        ...
        if (isExactEverMoreGatePresenceRule(rule)) {
            ...
            const spawnResult = spawnExactEverMoreGatePresenceForAnchor(scene, rule, anchor, sharedSpawnOptions);
            if (!spawnResult.success || !spawnResult.data) {
                return {
                    success: false,
                    errorMsg: spawnResult.errorMsg || "NPC_STARTUP_SPAWN_FAILED",
        ...
        for (let groupIndex = 0; groupIndex < groupsPerAnchor; groupIndex += 1) {
            ...
            const sharedSpawnOptions = {
                ...
                spawnDistanceMeters: toFiniteNumber(selector.spawnDistanceMeters, 0),
                distanceFromSurfaceMeters: toFiniteNumber(selector.distanceFromSurfaceMeters, 0),
                spreadMeters: toFiniteNumber(selector.spreadMeters, 0),
                formationSpacingMeters: toFiniteNumber(selector.formationSpacingMeters, 0),
...
function maintainStartupRulesInScene(scene, now) {
    ...
    for (const rule of listStartupRulesForSystem(scene.systemID)) {
        ...
        const respawnDelayMs = Math.max(1_000, toFiniteNumber(rule.respawnDelayMs, 15_000));
        const existingDeadline = toFiniteNumber(scene._npcStartupRespawnDeadlines[startupRuleID], 0);
        ...
        if (postSpawnMissingResult.success &&
    ...
    return {
        ...
        data: {
            ...
            rehydrated: Array.isArray(rehydrationResult.data)
                ...
                : [],
            applied: startupResult.success && startupResult.data && Array.isArray(startupResult.data.applied)
...
function getOperatorStartupRulesForSystem(systemID, operatorKind) {
    ...
    return listNpcStartupRules().filter((rule) => (ruleAppliesToSystem(rule, systemID) &&
...
function getGateOperatorState(systemID, operatorKind) {
    ...
    const gateControl = getSystemGateControl(normalizedSystemID);
    const dynamicRuleID = getDynamicGateStartupRuleID(normalizedSystemID, normalizedOperatorKind);
    ...
    const startupRules = getOperatorStartupRulesForSystem(normalizedSystemID, normalizedOperatorKind);
...
function wakeNpcController(entityID, whenMs = 0) {
    ...
    if (!controller) {
        ...
        if (controllerRecord && systemID > 0) {
            const scene = spaceRuntime.ensureScene(systemID);
            const materializeResult = nativeNpcService.materializeStoredNativeController(scene, normalizedEntityID, {
                broadcast: false,
            });
    ...
    controller.nextThinkAtMs =
        normalizedWhenMs > 0
            ? Math.min(toFiniteNumber(controller.nextThinkAtMs, normalizedWhenMs), normalizedWhenMs)
            : 0;
    return {
        success: true,
        data: controller,
...
function clearNpcControllersInSystem(systemID, options = {}) {
    ...
    const allowedEntityTypes = new Set(entityTypeFilter === "all"
    ...
    const startupRuleFilter = Array.isArray(options.startupRuleIDs)
        ? new Set(options.startupRuleIDs
...
function setGateOperatorEnabled(systemID, operatorKind, enabled) {
    ...
    for (const rule of startupRules) {
        const overrideResult = setStartupRuleEnabledOverride(rule.startupRuleID, false);
    ...
    const gateControlResult = setSystemGateControl(normalizedSystemID, gateControlUpdates);
    ...
    const startupRuleIDsToClear = [
        ...
        String(getDynamicGateStartupRuleID(normalizedSystemID, normalizedOperatorKind) || "").trim(),
    ].filter(Boolean);
    if (startupRuleIDsToClear.length > 0) {
        clearNpcControllersInSystem(normalizedSystemID, {
            entityType: normalizedOperatorKind === GATE_OPERATOR_KIND.CONCORD
                ? ENTITY_TYPE.CONCORD
                : ENTITY_TYPE.NPC,
...
function isDrifterController(controller, entity = null) {
    ...
    if (controller.behaviorOverrides &&
        controller.behaviorOverrides.drifterBehavior === true) {
        return true;
    }
    if (controller.behaviorProfile &&
...
function shouldPropagateDrifterAggression(sourceController, candidateController, sourceEntity, candidateEntity) {
    if (!sourceController ||
        !candidateController ||
...
function propagateDrifterAggressionToPack(targetEntity, attackerEntity, now) {
    ...
    const attackerOwnerID = toPositiveInt(attackerEntity && attackerEntity.ownerID, toPositiveInt(attackerEntity && attackerEntity.pilotCharacterID, toPositiveInt(attackerEntity && attackerEntity.characterID, 0)));
    ...
    const systemID = toPositiveInt(sourceController.systemID, toPositiveInt(targetEntity && targetEntity.systemID, 0));
    if (!systemID) {
        return [];
    }
    const scene = spaceRuntime.ensureScene(systemID);
    ...
    for (const candidateController of listControllersBySystem(systemID)) {
        const candidateEntity = scene.getEntityByID(toPositiveInt(candidateController && candidateController.entityID, 0));
        if (!shouldPropagateDrifterAggression(sourceController, candidateController, sourceEntity, candidateEntity)) {
            continue;
        }
...
function propagateAggressionToTransponderAllies(targetEntity, attackerEntity, now, options = {}) {
    const targetEntityID = toPositiveInt(targetEntity && targetEntity.itemID, 0);
    const attackerEntityID = toPositiveInt(attackerEntity && attackerEntity.itemID, 0);
    const attackerOwnerID = toPositiveInt(attackerEntity && attackerEntity.ownerID, toPositiveInt(attackerEntity && attackerEntity.pilotCharacterID, toPositiveInt(attackerEntity && attackerEntity.characterID, 0)));
    const systemID = toPositiveInt(targetEntity && targetEntity.systemID, toPositiveInt(attackerEntity && attackerEntity.systemID, 0));
    ...
    for (const candidateController of listControllersBySystem(systemID)) {
        ...
        const result = noteIncomingAggression(candidateEntityID, attackerEntityID, now, { attackerOwnerID });
...
function noteNpcIncomingAggression(targetEntity, attackerEntity, now, options = {}) {
    const targetEntityID = toPositiveInt(targetEntity && targetEntity.itemID, 0);
    const attackerEntityID = toPositiveInt(attackerEntity && attackerEntity.itemID, 0);
    const attackerOwnerID = toPositiveInt(attackerEntity && attackerEntity.ownerID, toPositiveInt(attackerEntity && attackerEntity.pilotCharacterID, toPositiveInt(attackerEntity && attackerEntity.characterID, 0)));
    if (!targetEntityID || !attackerEntityID) {
    ...
    const noteResult = noteIncomingAggression(targetEntityID, attackerEntityID, now, {
        attackerOwnerID,
    });
    ...
    const supportResult = noteResult.success && targetEntity.transient !== true &&
        ...
        ? requestNpcSupport({
            ...
            requiredRoles: Array.isArray(supportProfile.supportRequiredRoles)
                ...
                : ["combat"],
            supportGroupID: String(supportProfile.supportGroupID ||
            ...
            responseRadiusMeters: Math.max(0, toFiniteNumber(supportProfile.supportResponseRadiusMeters, 250_000)),
            ...
            allowReserveSpawn: supportProfile.supportReserveSpawnEnabled === true ||
                Array.isArray(supportProfile.reinforcementDefinitions) &&
                    supportProfile.reinforcementDefinitions.length > 0,
            maximumReserveSpawn: toPositiveInt(supportProfile.supportMaximumReserveSpawn, Array.isArray(supportProfile.reinforcementDefinitions)
            ...
            reserveSpawnDistanceMeters: Math.max(5_000, toFiniteNumber(supportProfile.supportReserveSpawnDistanceMeters, 35_000)),
            nowMs: now,
        })
        : null;
    ...
    const localTransponderResponderEntityIDs = targetEntity.transient === true || toPositiveInt(targetEntity.npcCharacterID, 0) <= 0
        ? propagateAggressionToTransponderAllies(targetEntity, attackerEntity, now, options)
        : [];
    const propagatedEntityIDs = [
        ...(noteResult.success
    ...
    return {
        ...
        data: {
            ...
            propagatedEntityIDs,
            supportIncidentID: supportResult && supportResult.success && supportResult.data && supportResult.data.incident
                ? supportResult.data.incident.incidentID
                : null,
            supportError: supportResult && supportResult.success === false
...
function notifyNpcDungeonArrival(scene, arrivingEntity, now = Date.now()) {
    ...
    const normalizedNow = Math.max(0, toFiniteNumber(now, Date.now()));
    const arrivingOwnerID = toPositiveInt(arrivingEntity.ownerID, toPositiveInt(arrivingEntity.pilotCharacterID, toPositiveInt(arrivingEntity.characterID, 0)));
    const responses = [];
    for (const controller of listControllersBySystem(scene.systemID)) {
        ...
        const disposition = resolveNpcDungeonArrivalDisposition(responderEntity, arrivingEntity, controller);
        if (disposition !== "hostile" &&
            disposition !== "friendly" &&
            disposition !== "suspicious") {
            continue;
        }
        const response = noteDungeonArrivalResponse(responderEntityID, arrivingEntityID, disposition, normalizedNow, { arrivingOwnerID });
    ...
    return {
        ...
        data: {
            ...
            hostileResponseCount: responses.filter((response) => response.disposition === "hostile").length,
            suspiciousResponseCount: responses.filter((response) => response.disposition === "suspicious").length,
            friendlyResponseCount: responses.filter((response) => response.disposition === "friendly").length,
            responses,
        },
    };
}
function getNpcOperatorSummary() {
    return listControllers().map((controller) => ({
        ...
        entityType: controller.entityType || ENTITY_TYPE.NPC,
        behaviorRole: controller.behaviorRole || "authored",
        behaviorActivity: controller.behaviorActivity || "authored",
        ...
        allowPodKill: controller.behaviorOverrides &&
        ...
        manualOrderType: controller.manualOrder && controller.manualOrder.type
...
//# sourceMappingURL=npcService.js.map