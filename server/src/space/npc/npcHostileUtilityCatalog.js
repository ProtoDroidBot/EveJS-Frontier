...
const { selectAutoFitFlagForNpcModuleType, } = require(path.join(__dirname, "./npcCapabilityResolver"));
...
function canFitAuthoredModulesOnHull(definition, modules = []) {
    const shipTypeID = toPositiveInt(definition && definition.profile && definition.profile.shipTypeID, 0);
    if (shipTypeID <= 0) {
        return false;
    }
    const shipLike = {
    ...
    for (const moduleEntry of Array.isArray(modules) ? modules : []) {
        ...
        for (let index = 0; index < quantity; index += 1) {
            const flagID = explicitFlags[index] || selectAutoFitFlagForNpcModuleType(shipLike, fittedModules.map((fittedModule) => ({
...
const COMPILED_TEMPLATES = Object.freeze(getTemplateRows()
    .map((template) => ({
    templateID: normalizeString(template && template.templateID),
    name: normalizeString(template && template.name),
    factionIDs: new Set((Array.isArray(template && template.factionIDs) ? template.factionIDs : [])
        .map((value) => toPositiveInt(value, 0))
        .filter((value) => value > 0)),
    behaviorProfileIDs: new Set((Array.isArray(template && template.behaviorProfileIDs)
        ? template.behaviorProfileIDs
        : [])
        .map((value) => normalizeString(value))
        .filter(Boolean)),
    profileIDs: new Set((Array.isArray(template && template.profileIDs) ? template.profileIDs : [])
        .map((value) => normalizeString(value))
        .filter(Boolean)),
    modules: Object.freeze((Array.isArray(template && template.modules) ? template.modules : [])
        .map((moduleEntry) => ({
...
function resolveMatchingTemplates(definition) {
    ...
    return COMPILED_TEMPLATES.filter((template) => (template.factionIDs.has(factionID) &&
...
function augmentNpcLoadoutWithHostileUtilities(definition, loadout) {
    const baseLoadout = loadout && typeof loadout === "object"
        ? loadout
        : {};
    ...
    const existingTypeIDs = new Set(existingModules
...
//# sourceMappingURL=npcHostileUtilityCatalog.js.map