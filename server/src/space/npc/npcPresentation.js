...
const { resolveItemByTypeID, resolveItemByName, } = require(path.join(__dirname, "../../services/inventory/itemTypeRegistry"));
...
function resolveNpcStandingThresholds(profile = {}) {
    const explicitHostileThreshold = Number(profile.hostileResponseThreshold ?? profile.hostile_response_threshold);
    const explicitFriendlyThreshold = Number(profile.friendlyResponseThreshold ?? profile.friendly_response_threshold);
    if (Number.isFinite(explicitHostileThreshold) &&
...
function buildNpcSlimPresentation(definition, shipLike = {}) {
    ...
    const fallbackName = String(profile.presentationName ||
        profile.name ||
    ...
    return {
        ...
        slimCategoryID: presentationType
            ...
            : fallbackCategoryID,
        slimName: String(profile.presentationName ||
            (presentationType && presentationType.name) ||
            fallbackName),
    };
}
function resolveShipPresentationRadius(entry, fallback = 0) {
    return Math.max(0, toFiniteNumber(entry && entry.radius, fallback));
}
...
function resolveNpcEntityRadius(shipType, presentationType, slimPresentation, profile = {}, shipLike = {}) {
    const directRadius = resolveShipPresentationRadius(shipType, resolveShipPresentationRadius(presentationType, toFiniteNumber(shipLike && shipLike.radius, 0)));
    if (directRadius > 1) {
        return directRadius;
    }
    const fallbackNames = [
...
function resolveNpcEntityBounty(profile, typeID) {
    ...
    try {
        const { getTypeAttributeMap } = require(path.join(__dirname, "../../services/fitting/liveFittingState"));
...
function buildNpcEntityIdentity(definition, shipLike = {}) {
    const profile = definition && definition.profile
        ? definition.profile
        : {};
    const resolvedTypeID = toPositiveInt(profile.shipTypeID, toPositiveInt(shipLike && shipLike.typeID, 0));
    ...
    return {
        typeID: toPositiveInt(profile.shipTypeID, toPositiveInt(shipType && shipType.typeID, toPositiveInt(shipLike && shipLike.typeID, 0))),
        groupID: toPositiveInt(shipType && shipType.groupID, toPositiveInt(shipLike && shipLike.groupID, 0)),
        categoryID: toPositiveInt(shipType && shipType.categoryID, toPositiveInt(shipLike && shipLike.categoryID, 0)),
        radius: resolveNpcEntityRadius(shipType, toPositiveInt(profile.presentationTypeID, 0) > 0
            ? resolveItemByTypeID(toPositiveInt(profile.presentationTypeID, 0))
            : null, slimPresentation, profile, shipLike),
        ...
        hostileResponseThreshold: toFiniteNumber(standingThresholds.hostileResponseThreshold, -11),
        friendlyResponseThreshold: toFiniteNumber(standingThresholds.friendlyResponseThreshold, -11),
    };
}
module.exports = {
    inferNpcEntityType,
...
//# sourceMappingURL=npcPresentation.js.map