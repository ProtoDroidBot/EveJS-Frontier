...
const { getFittedModuleItems, getLoadedChargeByFlag, isModuleOnline, getAttributeIDByNames, getEffectTypeRecord, getTypeDogmaEffects, getTypeAttributeMap, typeHasEffectName, } = require(path.join(__dirname, "../../services/fitting/liveFittingState"));
const { compareModuleSortOrder, isGroupableModuleItem, } = require(path.join(__dirname, "../../services/moduleGrouping/moduleGroupingRules"));
const { resolveItemByTypeID, } = require(path.join(__dirname, "../../services/inventory/itemTypeRegistry"));
const { isMissileWeaponFamily, isTurretWeaponFamily, resolveWeaponFamily, buildWeaponModuleSnapshot, } = require(path.join(__dirname, "../combat/weaponDogma"));
const { getLocationModifierSourcesForSystem, } = require(path.join(__dirname, "../../services/exploration/wormholes/wormholeEnvironmentRuntime"));
const { buildLiveModuleAttributeMap, } = require(path.join(__dirname, "../modules/liveModuleAttributes"));
const assistanceModuleRuntime = require(path.join(__dirname, "../modules/assistanceModuleRuntime"));
const hostileModuleRuntime = require(path.join(__dirname, "../modules/hostileModuleRuntime"));
const jammerModuleRuntime = require(path.join(__dirname, "../modules/jammerModuleRuntime"));
...
const { buildNpcEffectiveModuleItem, getNpcCapabilityTypeID, resolveNpcPropulsionEffectName: resolveNpcCapabilityPropulsionEffectName, NPC_ENABLE_FITTED_PROPULSION_MODULES, } = require(path.join(__dirname, "./npcCapabilityResolver"));
...
const ATTRIBUTE_MISSILE_LAUNCH_DURATION = getAttributeIDByNames("missileLaunchDuration") || 506;
const ATTRIBUTE_ENTITY_ATTACK_DELAY_MIN = getAttributeIDByNames("entityAttackDelayMin") || 475;
const ATTRIBUTE_ENTITY_ATTACK_DELAY_MAX = getAttributeIDByNames("entityAttackDelayMax") || 476;
...
const ATTRIBUTE_ENTITY_MISSILE_TYPE_ID = getAttributeIDByNames("entityMissileTypeID") || 507;
const ATTRIBUTE_MISSILE_ENTITY_VELOCITY_MULTIPLIER = getAttributeIDByNames("missileEntityVelocityMultiplier") || 645;
const ATTRIBUTE_MISSILE_ENTITY_FLIGHT_TIME_MULTIPLIER = getAttributeIDByNames("missileEntityFlightTimeMultiplier") || 646;
const ATTRIBUTE_MISSILE_ENTITY_AOE_VELOCITY_MULTIPLIER = getAttributeIDByNames("missileEntityAoeVelocityMultiplier") || 647;
const ATTRIBUTE_MISSILE_ENTITY_AOE_CLOUD_SIZE_MULTIPLIER = getAttributeIDByNames("missileEntityAoeCloudSizeMultiplier") || 648;
...
function getNpcPilotCharacterID(entity) {
    return toPositiveInt(entity && (entity.pilotCharacterID ??
...
function getNpcLoadedChargeForModule(entity, moduleItem) {
    ...
    if (isNativeNpcEntity(entity)) {
        const cargoItem = getNativeNpcCargoItems(entity).find((entry) => toPositiveInt(entry && entry.moduleID, 0) === moduleID) || null;
        if (!cargoItem) {
            return null;
        }
        return {
...
function buildNpcWeaponBankCache(entity) {
    ...
    for (const moduleItem of fittedItems) {
        const moduleID = toPositiveInt(moduleItem && moduleItem.itemID, 0);
        const chargeItem = cargoByModuleID.get(moduleID) ||
            getNpcLoadedChargeForModule(entity, moduleItem);
    ...
    for (const entries of groupsByKey.values()) {
        ...
        const sortedEntries = [...entries].sort((left, right) => (compareModuleSortOrder(left.moduleItem, right.moduleItem)));
...
function getNpcWeaponBankGroupForModule(entity, moduleItem) {
    ...
    if (moduleBankMasterID > 0 && moduleBankIDs.length > 1) {
        return {
            ...
            masterModuleItem: moduleBankMasterID === moduleID
                ? { ...moduleItem }
                : null,
...
function buildNpcSyntheticHullEffectModuleItem(entity, effectRecord, slotIndex = 0, options = {}) {
    ...
    const slotToken = Math.max(1, toPositiveInt(options.slotBase, 0) + toPositiveInt(effectRecord && effectRecord.effectID, slotIndex));
    return {
        ...
        flagID: 0,
        locationID: entityID,
        ownerID: toPositiveInt(entity && (entity.pilotCharacterID ??
            entity.characterID ??
            entity.ownerID ??
...
function getNpcWeaponModules(entity) {
    ...
    const fittedWeaponModules = getNpcFittedModuleItems(entity)
        ...
        const family = resolveWeaponFamily(moduleItem, chargeItem);
        return (isTurretWeaponFamily(family) ||
...
function typeHasNonzeroTargetAttackDamage(typeID) {
    ...
    const baseDamage = Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_EM_DAMAGE], 0)) +
...
function resolveNpcAssistanceModuleDefinition(moduleItem) {
    const effectRecord = resolveNpcModuleEffectRecord(moduleItem, (candidateEffectRecord) => Boolean(assistanceModuleRuntime.resolveAssistanceDefinition(candidateEffectRecord)));
    if (!effectRecord) {
        return null;
    }
...
function resolveNpcSelfModuleDefinition(moduleItem) {
    const effectRecord = resolveNpcModuleEffectRecord(moduleItem, (candidateEffectRecord) => Boolean(resolveNpcSelfEffectDefinition(candidateEffectRecord)));
    if (!effectRecord) {
        return null;
    }
...
function resolveNpcHostileModuleDefinition(moduleItem) {
    ...
    const cacheKey = toPositiveInt(getNpcCapabilityTypeID(effectiveModuleItem, effectiveModuleItem.typeID), 0);
    if (!cacheKey) {
        return null;
    }
    if (moduleItem &&
    ...
    const effectRecord = resolveNpcModuleEffectRecord({
        ...moduleItem,
        typeID: cacheKey,
    }, (candidateEffectRecord) => Boolean(hostileModuleRuntime.resolveHostileDefinition(candidateEffectRecord) ||
...
function getNpcSelfModules(entity) {
    const fittedItems = isNativeNpcEntity(entity) && Array.isArray(entity && entity.fittedItems)
        ? entity.fittedItems
    ...
    const fittedSelfModules = fittedItems
        ...
        .sort((left, right) => (toPositiveInt(left.moduleItem && left.moduleItem.flagID, 0) -
...
function buildNpcAssistanceModuleAttributeMap(entity, moduleItem) {
    ...
    return buildLiveModuleAttributeMap({
        itemID: shipID,
        typeID: shipTypeID,
    }, buildNpcEffectiveModuleItem(moduleItem), null, null, getNpcFittedModuleItems(entity), [], {
...
function estimateNpcAssistanceEffectiveRange(entity, moduleItem) {
    ...
    const effectRecord = resolveNpcModuleEffectRecord(moduleItem, (candidateEffectRecord) => Boolean(assistanceModuleRuntime.resolveAssistanceDefinition(candidateEffectRecord)));
    const rangeAttributeID = toPositiveInt(effectRecord && effectRecord.rangeAttributeID, ATTRIBUTE_MAX_RANGE);
    const falloffAttributeID = toPositiveInt(effectRecord && effectRecord.falloffAttributeID, ATTRIBUTE_FALLOFF_EFFECTIVENESS);
    const effectiveRange = Math.max(0, round6(toFiniteNumber(attributeMap && attributeMap[rangeAttributeID], 0) +
...
function estimateNpcHostileEffectiveRange(entity, moduleItem) {
    ...
    const effectiveRange = Math.max(0, round6(toFiniteNumber(attributeMap && attributeMap[ATTRIBUTE_MAX_RANGE], 0) +
...
function getNpcEntityEwarModules(entity) {
    ...
    specs.forEach((spec, index) => {
        const definition = hostileModuleRuntime.resolveHostileDefinition({ name: spec.effectName }) ||
        ...
        const moduleItem = buildNpcSyntheticHullEffectModuleItem(entity, { name: spec.effectName, effectID: 900000 + index }, index, { slotBase: 700 });
        if (!moduleItem) {
            return;
        }
    ...
    return isNativeNpcEntity(entity)
        ? getTypeEffectRecords(toPositiveInt(entity && entity.typeID, 0))
            ...
            const normalizedName = String(effectRecord && effectRecord.name || "")
                .trim()
                .toLowerCase();
            return (normalizedName === "entitysuperweapon" ||
                normalizedName === "entitysuperweaponlanceallraces");
        })
            .map((effectRecord, index) => (buildNpcSyntheticHullEffectModuleItem(entity, effectRecord, index, {
...
function estimateNpcWeaponEffectiveRange(entity, moduleItem) {
    ...
    return Math.max(0, round6(toFiniteNumber(snapshot.optimalRange, 0) +
...
function getNpcEntityAttackDelayWindow(entity) {
    ...
    const minMs = Math.max(0, round6(toFiniteNumber(shipAttributes && shipAttributes[ATTRIBUTE_ENTITY_ATTACK_DELAY_MIN], 0)));
    const rawMaxMs = Math.max(0, round6(toFiniteNumber(shipAttributes && shipAttributes[ATTRIBUTE_ENTITY_ATTACK_DELAY_MAX], 0)));
    return {
        minMs,
...
function getNpcEntityMissileWeaponSource(entity) {
    ...
    const shipAttributes = getTypeAttributeMap(shipTypeID);
    const missileTypeID = toPositiveInt(shipAttributes && shipAttributes[ATTRIBUTE_ENTITY_MISSILE_TYPE_ID], 0);
    if (missileTypeID <= 0) {
        return null;
    }
    const missileType = resolveItemByTypeID(missileTypeID);
    const missileAttributes = getTypeAttributeMap(missileTypeID);
    const damageMultiplier = Math.max(0, toFiniteNumber(shipAttributes[ATTRIBUTE_MISSILE_DAMAGE_MULTIPLIER], 1));
    const velocityMultiplier = Math.max(0, toFiniteNumber(shipAttributes[ATTRIBUTE_MISSILE_ENTITY_VELOCITY_MULTIPLIER], 1));
    const flightTimeMultiplier = Math.max(0, toFiniteNumber(shipAttributes[ATTRIBUTE_MISSILE_ENTITY_FLIGHT_TIME_MULTIPLIER], 1));
    const aoeVelocityMultiplier = Math.max(0, toFiniteNumber(shipAttributes[ATTRIBUTE_MISSILE_ENTITY_AOE_VELOCITY_MULTIPLIER], 1));
    const aoeCloudSizeMultiplier = Math.max(0, toFiniteNumber(shipAttributes[ATTRIBUTE_MISSILE_ENTITY_AOE_CLOUD_SIZE_MULTIPLIER], 1));
    const baseDamage = {
    ...
    const maxVelocity = Math.max(0, round6(toFiniteNumber(missileAttributes[ATTRIBUTE_MAX_VELOCITY], 0) * velocityMultiplier));
    const flightTimeMs = Math.max(1, round6(toFiniteNumber(missileAttributes[ATTRIBUTE_EXPLOSION_DELAY], 1000) * flightTimeMultiplier));
    return {
        ...
        durationMs: Math.max(1, round6(toFiniteNumber(shipAttributes[ATTRIBUTE_MISSILE_LAUNCH_DURATION], toFiniteNumber(shipAttributes[ATTRIBUTE_SPEED], 1000)))),
        damageMultiplier,
        rawShotDamage: {
        ...
        maxVelocity,
        flightTimeMs,
        explosionRadius: Math.max(1, round6(toFiniteNumber(missileAttributes[ATTRIBUTE_AOE_CLOUD_SIZE], 1) * aoeCloudSizeMultiplier)),
        explosionVelocity: Math.max(0.001, round6(toFiniteNumber(missileAttributes[ATTRIBUTE_AOE_VELOCITY], 0.001) * aoeVelocityMultiplier)),
        damageReductionFactor: Math.max(0.000001, Math.min(1, toFiniteNumber(missileAttributes[ATTRIBUTE_AOE_DAMAGE_REDUCTION_FACTOR], 1))),
        damageReductionSensitivity: Math.max(0.000001, round6(toFiniteNumber(missileAttributes[ATTRIBUTE_AOE_DAMAGE_REDUCTION_SENSITIVITY], DEFAULT_MISSILE_DAMAGE_REDUCTION_SENSITIVITY))),
...
function getNpcPropulsionModules(entity) {
    ...
    return getNpcFittedModuleItems(entity)
        ...
        .sort((left, right) => {
        const leftPriority = left.effectName === PROPULSION_EFFECT_MICROWARPDRIVE ? 0 : 1;
        const rightPriority = right.effectName === PROPULSION_EFFECT_MICROWARPDRIVE ? 0 : 1;
        if (leftPriority !== rightPriority) {
            return leftPriority - rightPriority;
        }
        return (toPositiveInt(left.moduleItem && left.moduleItem.flagID, 0) -
            toPositiveInt(right.moduleItem && right.moduleItem.flagID, 0));
    });
}
module.exports = {
...
//# sourceMappingURL=npcEquipment.js.map