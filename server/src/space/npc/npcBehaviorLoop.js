...
const { getControllerByEntityID, listControllersBySystem, unregisterController, } = require(path.join(__dirname, "./npcRegistry"));
const { normalizeTargetClassList, isCharacterInvulnerable, } = require(path.join(__dirname, "./npcControlState"));
const { getNpcWeaponModules, getNpcWeaponBankGroupForModule, getNpcHostileModules, getNpcAssistanceModules, getNpcSelfModules, getNpcSuperweaponModules, estimateNpcAssistanceEffectiveRange, estimateNpcHostileEffectiveRange, estimateNpcWeaponEffectiveRange, getNpcEntityMissileWeaponSource, getNpcEntityAttackDelayMs, getNpcPropulsionModules, PROPULSION_EFFECT_AFTERBURNER, PROPULSION_EFFECT_MICROWARPDRIVE, } = require(path.join(__dirname, "./npcEquipment"));
const { resolveSyntheticChasePropulsionTemplate, } = require(path.join(__dirname, "./npcHardwareCatalog"));
const { logNpcCombatDebug, summarizeNpcCombatEntity, summarizeNpcCombatModule, } = require(path.join(__dirname, "./npcCombatDebug"));
const { syncCapitalNpcSystems, } = require(path.join(__dirname, "./capitals/capitalNpcBehavior"));
const { syncCapitalNpcMovement, syncCapitalNpcReturnHome, } = require(path.join(__dirname, "./capitals/capitalNpcMovement"));
const { resolveCapitalMovementDirective, } = require(path.join(__dirname, "./capitals/capitalNpcDoctrine"));
const { resolveCapitalBehaviorTarget, } = require(path.join(__dirname, "./capitals/capitalNpcTargeting"));
const { resolveCapitalEngagementPolicy, } = require(path.join(__dirname, "./capitals/capitalNpcEngagement"));
...
function getNpcInitialModuleCycleDelayMs(entity, moduleItem, salt = 0) {
    ...
    const seed = ((entityID * 31) +
        (moduleID * 17) +
        (toPositiveInt(salt, 0) * 13)) >>> 0;
...
function getNpcMaintenanceJitterMs(entity, salt = 0, windowMs = 500) {
    ...
    const entityID = toPositiveInt(entity && entity.itemID, 0);
    const seed = ((entityID * 37) +
        (toPositiveInt(salt, 0) * 101)) >>> 0;
...
function buildNpcSyntheticPropulsionBroadcastOptions() {
    return {
        ...
        historyLeadPresentedMaximumFutureLead: NPC_COMBAT_PRESENTATION_PRESENTED_CLEAR_LEAD,
    };
}
function buildNpcSyntheticPropulsionFxOptions(baseOptions = {}) {
    return {
        ...
        avoidCurrentHistoryInsertion: true,
        minimumHistoryLeadFloor: NPC_COMBAT_PRESENTATION_PRESENTED_CLEAR_LEAD,
        minimumLeadFromCurrentHistory: Math.max(toPositiveInt(baseOptions.minimumLeadFromCurrentHistory, 0), NPC_COMBAT_PRESENTATION_PRESENTED_CLEAR_LEAD),
        maximumLeadFromCurrentHistory: Math.max(toPositiveInt(baseOptions.maximumLeadFromCurrentHistory, 0), NPC_COMBAT_PRESENTATION_PRESENTED_CLEAR_LEAD),
        maximumHistorySafeLeadOverride: Math.max(toPositiveInt(baseOptions.maximumHistorySafeLeadOverride, 0), NPC_COMBAT_PRESENTATION_HISTORY_LEAD),
        historyLeadUsesPresentedSessionStamp: true,
        historyLeadPresentedMaximumFutureLead: NPC_COMBAT_PRESENTATION_PRESENTED_CLEAR_LEAD,
    };
}
function buildNpcPseudoSession(entity) {
    const pilotCharacterID = toPositiveInt(entity && (entity.pilotCharacterID ??
...
function getDrifterSuperweaponCycleMs(entity) {
    return Math.max(1_000, toFiniteNumber(getTypeAttributeValue(toPositiveInt(entity && entity.typeID, 0), "entitySuperWeaponDuration"), 5_000));
}
function ensureDrifterCombatState(controller, entity, scene, nowMs) {
    ...
    if (!Number.isFinite(Number(state.baseScanResolution)) || Number(state.baseScanResolution) <= 0) {
        state.baseScanResolution = Math.max(1, toFiniteNumber(entity.scanResolution, entity.passiveDerivedState && entity.passiveDerivedState.scanResolution));
    }
    if (!Number.isFinite(Number(state.baseMaxTargetRange)) || Number(state.baseMaxTargetRange) <= 0) {
        state.baseMaxTargetRange = Math.max(0, toFiniteNumber(entity.maxTargetRange, entity.passiveDerivedState && entity.passiveDerivedState.maxTargetRange));
    }
    if (state.waitingForTurboShieldResistive !== true &&
...
function setNpcTurboShieldState(scene, entity, state, drifterState = null) {
    ...
    const resolvedState = Number.isFinite(resolvedNumericState)
        ? Math.max(TURBO_SHIELD_STATE_ACTIVE, Math.min(TURBO_SHIELD_STATE_DEPLETED, resolvedNumericState))
...
function resetDrifterCombatState(scene, entity, controller, nowMs = Date.now()) {
    ...
    setNpcScanResolutionOverride(entity, toFiniteNumber(drifterState.baseScanResolution, entity && entity.scanResolution));
...
function resolveDrifterPendingPoddingOwnerID(controller) {
    return toPositiveInt(controller &&
        controller.drifterCombatState &&
        controller.drifterCombatState.pendingPoddingOwnerID, 0);
}
...
function listDrifterPackMembers(scene, controller, entity) {
    ...
    for (const candidateController of listControllersBySystem(scene.systemID)) {
        ...
        if (!candidateEntity ||
            !isDrifterNpcEntity(candidateEntity, resolveEffectiveBehaviorProfile(candidateController)) ||
    ...
    return members.sort((left, right) => (toPositiveInt(left && left.entity && left.entity.itemID, 0) -
...
function getRecentDrifterReinforcementRequestAtMs(scene, controller, entity) {
    ...
    for (const member of buildDrifterReinforcementGroup(scene, controller, entity)) {
        latestRequestAtMs = Math.max(latestRequestAtMs, toFiniteNumber(member &&
            member.controller &&
            member.controller.drifterCombatState &&
            member.controller.drifterCombatState.lastReinforcementRequestAtMs, 0));
    }
...
function propagateDrifterReinforcementRequestState(scene, controller, entity, nowMs) {
    for (const member of buildDrifterReinforcementGroup(scene, controller, entity)) {
        ...
        const memberState = ensureDrifterCombatState(memberController, memberEntity, scene, nowMs);
        if (!memberState) {
            continue;
        }
...
function maybeRequestDrifterReinforcements(scene, entity, controller, behaviorProfile, targetEntity, nowMs) {
    ...
    const recentAggressionWindowMs = Math.max(1_000, toFiniteNumber(behaviorProfile && behaviorProfile.reinforcementAggressionWindowMs, 15_000));
    ...
    const cooldownMs = Math.max(1_000, toFiniteNumber(behaviorProfile && behaviorProfile.reinforcementCooldownMs, 60_000));
    const lastPackRequestAtMs = getRecentDrifterReinforcementRequestAtMs(scene, controller, entity);
    if (lastPackRequestAtMs > 0 &&
    ...
    const maxCalls = Math.max(1, toPositiveInt(behaviorProfile && behaviorProfile.maxReinforcementCalls, 1));
    ...
    const selectionID = String(controller.selectionID ||
        behaviorProfile && behaviorProfile.reinforcementSelectionID ||
        `drifter-reinforcement:${toPositiveInt(entity && entity.itemID, 0)}`).trim();
    const selectionName = String(controller.selectionName ||
        behaviorProfile && behaviorProfile.reinforcementSelectionName ||
        "Drifter Reinforcements").trim() || "Drifter Reinforcements";
    ...
    const spawnResult = getNativeNpcService().spawnNativeDefinitionsInContext({
        ...
        anchorKind: "ship",
        anchorLabel: String(targetEntity && (targetEntity.itemName ||
            targetEntity.slimName) || "Ship"),
    }, {
        data: {
            selectionKind,
            selectionID,
        ...
        behaviorOverrides: {
            ...
            aggressionRangeMeters: Math.max(250_000, toFiniteNumber(behaviorProfile && behaviorProfile.aggressionRangeMeters, 0)),
        ...
        spawnDistanceMeters: Math.max(5_000, toFiniteNumber(behaviorProfile && behaviorProfile.reinforcementSpawnDistanceMeters, 35_000)),
        formationSpacingMeters: Math.max(250, toFiniteNumber(behaviorProfile && behaviorProfile.reinforcementFormationSpacingMeters, 1_500)),
        selectionKind,
        selectionID,
        selectionName,
        anchorKind: "ship",
        anchorName: String(targetEntity && (targetEntity.itemName ||
            targetEntity.slimName) || "Ship"),
        anchorID: targetID,
    });
...
function publishDrifterPursuitLocation(scene, entity, controller, targetEntity, nowMs) {
    ...
    for (const member of listDrifterPackMembers(scene, controller, entity)) {
        ...
        const drifterState = ensureDrifterCombatState(memberController, memberEntity, scene, nowMs);
        if (!drifterState) {
            continue;
        }
        ...
        if (memberController.entityID !== controller.entityID &&
...
function beginNpcWarpToPoint(scene, entity, controller, destination, options = {}) {
    ...
    const stateObject = options.stateObject && typeof options.stateObject === "object"
        ? options.stateObject
        : controller;
    const cooldownField = String(options.cooldownField || "").trim();
    const cooldownMs = Math.max(0, toFiniteNumber(options.cooldownMs, 0));
    const lastWarpAtMs = Math.max(0, toFiniteNumber(cooldownField && stateObject
    ...
    if (entity.mode === "WARP" || entity.pendingWarp || entity.warpState) {
        return {
            success: true,
            handled: true,
            nextThinkAtMs: Math.max(nowMs + 1_000, toFiniteNumber(controller.nextThinkAtMs, nowMs + 1_000)),
        };
    }
    ...
    const warpResult = getNpcRuntime().warpToPoint(entity.itemID, destination, {
    ...
    return {
        success: true,
        handled: true,
        nextThinkAtMs: toFiniteNumber(warpResult &&
            warpResult.data &&
            warpResult.data.ingressCompleteAtMs, nowMs + cooldownMs),
    };
}
function beginDrifterWarp(scene, entity, controller, destination, cooldownField, nowMs) {
    ...
    const cooldownMs = cooldownField === "lastRegroupWarpAtMs"
        ? DRIFTER_REGROUP_WARP_COOLDOWN_MS
        : DRIFTER_PURSUIT_WARP_COOLDOWN_MS;
    return beginNpcWarpToPoint(scene, entity, controller, destination, {
...
function tryDrifterPursuitOrRegroup(scene, entity, controller, behaviorProfile, nowMs) {
    ...
    const lastPursuitPostedAtMs = Math.max(0, toFiniteNumber(drifterState.lastPursuitPostedAtMs, 0));
    ...
    const pursuitPosition = drifterState.lastPursuitPosition &&
    ...
    if (pursuitEnabled &&
        ...
        return beginDrifterWarp(scene, entity, controller, pursuitPosition, "lastPursuitWarpAtMs", nowMs);
    }
    ...
    if (regroupEnabled &&
        ...
        if (regroupDestination &&
            distance(entity.position, regroupDestination) > DRIFTER_WARP_MIN_DISTANCE_METERS) {
            return beginDrifterWarp(scene, entity, controller, regroupDestination, "lastRegroupWarpAtMs", nowMs);
        }
    }
    return {
        handled: false,
...
function activateNpcHostileModuleOnTarget(scene, entity, moduleEntry, targetEntity) {
    ...
    const activationResult = scene.activateGenericModule(pseudoSession, moduleItem, moduleEntry.effectName || null, {
...
function syncDrifterCombatSystems(scene, entity, controller, behaviorProfile, targetEntity, options = {}) {
    ...
    if (drifterBehaviorEnabled) {
        maybeRequestDrifterReinforcements(scene, entity, controller, behaviorProfile, targetEntity, nowMs);
    }
    ...
    const scrambleModuleID = toPositiveInt(scrambleEntry && scrambleEntry.moduleItem && scrambleEntry.moduleItem.itemID, 0);
    if (activeSuperweaponEffect) {
        ...
        return {
            ...
            nextThinkOverrideMs: Math.max(nowMs + 50, toFiniteNumber(activeSuperweaponEffect.deactivateAtMs, activeSuperweaponEffect.nextCycleAtMs)),
    ...
    if (superweaponReady) {
        ...
        setNpcScanResolutionOverride(entity, Math.max(toFiniteNumber(drifterState.baseScanResolution, 0), DRIFTER_SUPERWEAPON_SCAN_RESOLUTION));
    }
    ...
    const pendingLock = pendingTargetLocks.find((entry) => toPositiveInt(entry && entry.targetID, 0) === toPositiveInt(targetEntity.itemID, 0)) || null;
    if (!hasDesiredLock) {
        return {
            forceMaintainLock: true,
            suppressWeapons: false,
    ...
    stopNpcMovement(scene, entity);
    const activationResult = scene.activateGenericModule(buildNpcPseudoSession(entity), superweaponModule, String(superweaponModule.npcEffectName || "").trim() || null, {
        targetID: targetEntity.itemID,
    });
    ...
    return {
        ...
        nextThinkOverrideMs: refreshedEffect
            ? Math.max(nowMs + 50, toFiniteNumber(refreshedEffect.deactivateAtMs, refreshedEffect.nextCycleAtMs))
            : nowMs + cycleMs,
        preservedModuleIDs: scrambleModuleID > 0 ? [scrambleModuleID] : [],
    };
}
...
function getEntityCharacterID(entity) {
    ...
    return toPositiveInt(entity.session && entity.session.characterID
        ? entity.session.characterID
        : entity.controllerOwnerID ?? entity.ownerID ?? entity.pilotCharacterID ?? entity.characterID, 0);
}
...
function isFriendlyCombatTarget(entity, target) {
    ...
    if (sourceClass === ENTITY_TYPE.NPC && targetClass === ENTITY_TYPE.NPC) {
        const sourceController = getControllerByEntityID(toPositiveInt(entity && entity.itemID, 0));
        const targetController = getControllerByEntityID(toPositiveInt(target && target.itemID, 0));
        const sourceAllowsFriendlyNpcTargets = Boolean(sourceController &&
            sourceController.behaviorOverrides &&
            sourceController.behaviorOverrides.allowFriendlyNpcTargets === true);
        const targetAllowsFriendlyNpcTargets = Boolean(targetController &&
        ...
        if (sourceAllowsFriendlyNpcTargets &&
...
function resolveAutoAggroTargetClasses(behaviorProfile) {
    const explicitClasses = normalizeTargetClassList(behaviorProfile && behaviorProfile.autoAggroTargetClasses);
    ...
    const targetPreference = String(behaviorProfile && behaviorProfile.targetPreference || "preferredTargetThenNearestPlayer")
        .trim()
        .toLowerCase();
...
function resolveProximityAggroTargetClasses(behaviorProfile) {
    const explicitClasses = normalizeTargetClassList(behaviorProfile && behaviorProfile.proximityAggroTargetClasses);
    if (explicitClasses.length > 0 ||
        (behaviorProfile &&
...
function normalizeBehaviorOverrides(overrides) {
    ...
    if (normalized.targetSwitchChanceToNotSwitch !== undefined) {
        normalized.targetSwitchChanceToNotSwitch = normalizeChanceFraction(normalized.targetSwitchChanceToNotSwitch, 0);
    }
    if (normalized.chaseMaxDelayChance !== undefined) {
        normalized.chaseMaxDelayChance = normalizeChanceFraction(normalized.chaseMaxDelayChance, 0);
    }
    if (normalized.chaseMaxDurationChance !== undefined) {
        normalized.chaseMaxDurationChance = normalizeChanceFraction(normalized.chaseMaxDurationChance, 1);
    }
    ...
    if (overrides.autoAggroTargetClasses !== undefined) {
        normalized.autoAggroTargetClasses = normalizeTargetClassList(overrides.autoAggroTargetClasses);
    }
...
function isEntityCloakedForCombatTargeting(entity) {
    return toPositiveInt(entity && (entity.isCloaked ??
        entity.cloakMode ??
        entity.cloaked), 0) > 0;
}
...
function isValidCombatTarget(entity, target, options = {}) {
    ...
    if (targetClass === "drone") {
        const minimumSignatureRadius = Math.max(0, toFiniteNumber(options.ignoreDronesBelowSignatureRadius, 0));
        const signatureRadius = Math.max(0, toFiniteNumber(target && target.signatureRadius, toFiniteNumber(target && target.radius, 0)));
...
function findNearestCombatTarget(scene, entity, maxRangeMeters, options = {}) {
    ...
    for (const candidate of scene.dynamicEntities.values()) {
        ...
        const currentTargetID = toPositiveInt(options.currentTargetID ?? options.preferredTargetID, 0);
...
function resolveNpcTargetSwitchPolicy(behaviorProfile) {
    ...
    return {
        ...
        enabled,
        intervalMs: enabled
            ? Math.max(0, toFiniteNumber(behaviorProfile && behaviorProfile.targetSwitchIntervalMs, NPC_TARGET_SWITCH_INTERVAL_MS))
            : 0,
        chanceToNotSwitch: enabled
            ? normalizeChanceFraction(behaviorProfile && behaviorProfile.targetSwitchChanceToNotSwitch, 0)
            : 0,
    };
}
function isValidBehaviorCombatTarget(entity, target, allowedTargetClasses, maxRangeMeters, options = {}) {
    const targetClass = resolveCombatActorClass(target);
    if (allowedTargetClasses.length > 0 &&
...
function recordNpcBehaviorTargetSelection(controller, policy, nowMs, previousTargetID, nextTargetID) {
    if (!(controller && policy && policy.enabled)) {
        return;
    }
...
function resolveNpcChaseVelocityPolicy(behaviorProfile) {
    ...
    return {
        ...
        chaseMaxVelocity,
        chaseDistance,
        maxDelayMs: Math.max(0, toFiniteNumber(behaviorProfile && behaviorProfile.chaseMaxDelayMs, 0)),
        maxDelayChance: normalizeChanceFraction(behaviorProfile && behaviorProfile.chaseMaxDelayChance, 0),
        maxDurationMs: Math.max(0, toFiniteNumber(behaviorProfile && behaviorProfile.chaseMaxDurationMs, 0)),
        maxDurationChance: normalizeChanceFraction(behaviorProfile && behaviorProfile.chaseMaxDurationChance, 1),
    };
}
...
function setNpcChaseVelocityMode(entity, mode, policy) {
    ...
    const desiredMaxVelocity = normalizedMode === "chase"
...
function syncNpcChaseVelocity(scene, entity, controller, behaviorProfile, target) {
    ...
    const durationMs = policy.maxDurationMs > 0 && rollChance(policy.maxDurationChance)
...
function resolveCurrentBehaviorTargetGate(scene, controller, entity, behaviorProfile, allowedTargetClasses, maxRangeMeters, options = {}) {
    ...
    const nextSwitchAtMs = toFiniteNumber(controller && controller._nextNpcTargetSwitchAtMs, 0);
    if (nextSwitchAtMs <= 0) {
        scheduleNpcTargetSwitchCheck(controller, policy, nowMs);
        return {
            handled: true,
    ...
    const randomRoll = chanceToNotSwitch > 0 && chanceToNotSwitch < 1
        ? Math.random()
        : null;
    if (chanceToNotSwitch >= 1 ||
        (randomRoll !== null &&
...
function hasActiveEffectName(entity, effectName) {
    ...
    for (const effectState of entity.activeModuleEffects.values()) {
        const activeEffectName = String(effectState && effectState.effectName || "").trim().toLowerCase();
...
function resolveCombatOwnerID(entity) {
    ...
    return toPositiveInt(entity.ownerID, toPositiveInt(entity.pilotCharacterID, toPositiveInt(entity.characterID, 0)));
}
function scoreDrifterTarget(entity, controller, candidate, options = {}) {
    ...
    const lastAggressedAtMs = toFiniteNumber(controller && controller.lastAggressedAtMs, 0);
    const nowMs = toFiniteNumber(options.nowMs, Date.now());
    const distanceMeters = Math.max(0, toFiniteNumber(options.getSurfaceDistance
        ? options.getSurfaceDistance(entity, candidate)
        : 0, 0));
    const drifterState = options.drifterState && typeof options.drifterState === "object"
        ? options.drifterState
        : {};
    const lastSuperweaponTargetID = toPositiveInt(drifterState.lastSuperweaponTargetID, 0);
    const pendingPoddingOwnerID = toPositiveInt(drifterState.pendingPoddingOwnerID, 0);
    const guardAnchorEntity = options.guardAnchorEntity && typeof options.guardAnchorEntity === "object"
        ? options.guardAnchorEntity
        : null;
    const guardPriorityRadiusMeters = Math.max(0, toFiniteNumber(options.guardPriorityRadiusMeters, 0));
    ...
    if (guardAnchorEntity &&
        guardAnchorEntity.position &&
        candidate &&
        candidate.position &&
        guardPriorityRadiusMeters > 0) {
        const guardDistanceMeters = Math.max(0, toFiniteNumber(options.getSurfaceDistance
            ? options.getSurfaceDistance(guardAnchorEntity, candidate)
            : getSurfaceDistance(guardAnchorEntity, candidate), 0));
        if (guardDistanceMeters <= guardPriorityRadiusMeters) {
            const normalizedGuardPriority = Math.max(0, 1 - (guardDistanceMeters / guardPriorityRadiusMeters));
    ...
    if (String(options.resolveCombatActorClass
...
function selectBestScoredCombatTarget(scoredCandidates = []) {
    return [...scoredCandidates]
        ...
        return (toPositiveInt(left && left.candidate && left.candidate.itemID, 0) -
...
function resolveDrifterBehaviorTarget(scene, entity, controller, behaviorProfile, options = {}) {
    ...
    const nowMs = toFiniteNumber(options.nowMs, Date.now());
    const aggressionRangeMeters = Math.max(0, toFiniteNumber(options.aggressionRangeMeters, 0));
    const allowedTargetClasses = normalizeTargetClassList(options.allowedTargetClasses);
    if (allowedTargetClasses.length <= 0) {
        return null;
    }
    ...
    const allowPodKill = options.allowPodKill === true ||
        allowedCapsuleOwnerID > 0;
    const recentAggressionMemoryActive = toFiniteNumber(controller && controller.lastAggressedAtMs, 0) > 0 &&
    ...
    const recentPursuitForCurrentTarget = Boolean(pursuitEnabled &&
    ...
    const isEligibleTarget = (candidate) => {
        ...
        if (options.isValidCombatTarget &&
        ...
        const candidateClass = String(options.resolveCombatActorClass
            ? options.resolveCombatActorClass(candidate)
            : "").trim().toLowerCase();
        if (!allowedTargetClasses.includes(candidateClass)) {
            return false;
    ...
    const lastAggressorEntity = toPositiveInt(controller.lastAggressorID, 0) > 0
        ...
        : null;
    const preferredTargetOwnerFallbackID = preferredTargetEntity && isEligibleTarget(preferredTargetEntity)
        ? 0
        : toPositiveInt(controller.preferredTargetOwnerID, 0);
    const lastAggressorOwnerFallbackID = recentAggressionMemoryActive &&
        !(lastAggressorEntity && isEligibleTarget(lastAggressorEntity))
        ? toPositiveInt(controller.lastAggressorOwnerID, 0)
        : 0;
    const nextTargetSwitchAtMs = Math.max(0, toFiniteNumber(drifterState && drifterState.nextTargetSwitchAtMs, toFiniteNumber(drifterState && drifterState.lastTargetSwitchAtMs, 0) +
    ...
    if (recentPursuitForCurrentTarget) {
        publishDrifterPursuitLocation(scene, entity, controller, currentTarget, nowMs);
        return null;
    }
    const switchingWindowExpired = currentTarget &&
        isEligibleTarget(currentTarget) &&
        nextTargetSwitchAtMs > 0 &&
        nextTargetSwitchAtMs <= nowMs;
    const guardAnchorEntity = behaviorProfile && behaviorProfile.idleAnchorOrbit === true
        ? resolveIdleAnchorEntity(scene, controller, entity)
        : null;
    const guardPriorityRadiusMeters = guardAnchorEntity
        ? Math.max(DRIFTER_GUARD_PRIORITY_MIN_RADIUS_METERS, toFiniteNumber(behaviorProfile && behaviorProfile.idleAnchorOrbitDistanceMeters, 0) + 5_000)
        : 0;
    ...
    for (const candidate of scene.dynamicEntities.values()) {
        ...
        scoredCandidates.push(scoreDrifterTarget(entity, controller, candidate, {
            nowMs,
            drifterState,
    ...
    const shouldKeepCurrentTarget = Boolean(currentTarget &&
        isEligibleTarget(currentTarget) &&
        bestCandidate.candidate.itemID !== currentTarget.itemID);
    const entosisCandidate = switchingWindowExpired && entosisPriorityEnabled
        ? selectBestScoredCombatTarget(scoredCandidates.filter((entry) => hasActiveEffectName(entry && entry.candidate, "entosislink")))
        : null;
    const currentTargetHasEntosis = hasActiveEffectName(currentTarget, "entosislink");
    const preferredSwitchCandidate = entosisCandidate &&
    ...
    if (shouldKeepCurrentTarget) {
        ...
        if (preferredSwitchCandidate.candidate.itemID !== currentTarget.itemID &&
        ...
        else if (preferredSwitchCandidate.score <= currentScore.score + DRIFTER_TARGET_SWITCH_SCORE_MARGIN) {
            if (drifterState) {
    ...
    if (preferredTargetOwnerFallbackID > 0 &&
    ...
    if (lastAggressorOwnerFallbackID > 0 &&
...
function resolveNpcAssistanceNeedRatio(target, family) {
    ...
    const conditionState = target.conditionState && typeof target.conditionState === "object"
        ? target.conditionState
        : {};
    const normalizedFamily = String(family || "").trim().toLowerCase();
    if (normalizedFamily === "remoteshield") {
        const shieldChargeRatio = Math.min(1, Math.max(0, toFiniteNumber(conditionState.shieldCharge, 1)));
        return 1 - shieldChargeRatio;
    }
    if (normalizedFamily === "remotearmor") {
        return Math.min(1, Math.max(0, toFiniteNumber(conditionState.armorDamage, 0)));
    }
    if (normalizedFamily === "remotehull") {
        return Math.min(1, Math.max(0, toFiniteNumber(conditionState.damage, 0)));
    }
    if (normalizedFamily === "remotecapacitor") {
        const capacitorChargeRatio = Math.min(1, Math.max(0, toFiniteNumber(target.capacitorChargeRatio, target.passiveDerivedState && target.passiveDerivedState.capacitorChargeRatio)));
...
function normalizeManualOrder(order) {
    ...
    return {
        ...
        allowWeapons: order.allowWeapons === undefined ? null : order.allowWeapons === true,
        keepLock: order.keepLock === undefined ? null : order.keepLock === true,
        allowPodKill: order.allowPodKill === undefined ? null : order.allowPodKill === true,
    };
}
function resolveBehaviorTarget(scene, controller, entity, behaviorProfile) {
    ...
    if (behaviorProfile.autoAggro === false) {
        ...
        if (entity && entity.capitalNpc === true) {
            return resolveCapitalBehaviorTarget(scene, entity, controller, behaviorProfile, {
                nowMs,
                allowPodKill,
        ...
        if (isDrifterNpcEntity(entity, behaviorProfile)) {
            return resolveDrifterBehaviorTarget(scene, entity, controller, behaviorProfile, {
                nowMs,
                allowPodKill,
                aggressionRangeMeters: proximityAggroRangeMeters,
                allowedTargetClasses: proximityAggroTargetClasses,
        ...
        const gate = resolveCurrentBehaviorTargetGate(scene, controller, entity, behaviorProfile, proximityAggroTargetClasses, proximityAggroRangeMeters, {
            nowMs,
            allowPodKill,
            ignoreDronesBelowSignatureRadius: behaviorProfile.ignoreDronesBelowSignatureRadius,
        });
    ...
    const gate = resolveCurrentBehaviorTargetGate(scene, controller, entity, behaviorProfile, autoAggroTargetClasses, aggressionRangeMeters, {
        nowMs,
        allowPodKill,
        ignoreDronesBelowSignatureRadius: behaviorProfile.ignoreDronesBelowSignatureRadius,
    });
    ...
    if (!gate.currentTarget &&
        ...
        recordNpcBehaviorTargetSelection(controller, gate.policy, nowMs, 0, preferredTarget && preferredTarget.itemID);
        return preferredTarget;
    }
...
function resolveDesiredTarget(scene, controller, entity, behaviorProfile, manualOrder) {
    const allowPodKill = manualOrder && manualOrder.allowPodKill !== null
...
function deactivateNpcWeapons(scene, entity, options = {}) {
    const pseudoSession = buildNpcPseudoSession(entity);
    const excludedModuleIDs = new Set((Array.isArray(options.excludeModuleIDs) ? options.excludeModuleIDs : [options.excludeModuleIDs])
...
function syncNpcHostileModules(scene, entity, target) {
    ...
    for (const { moduleItem, definition, effectName } of modules) {
        ...
        const activationOutcome = scene.activateGenericModule(pseudoSession, moduleItem, effectName || null, {
        ...
        logNpcCombatDebug("npc.hostile.module-activate", {
            ...
            errorMsg: activationOutcome && activationOutcome.success === false
...
function syncNpcAssistanceModules(scene, entity, controller) {
    ...
    for (const { moduleItem, definition, effectName } of modules) {
        ...
        if (activeEffect) {
            ...
            const activeNeedRatio = resolveNpcAssistanceNeedRatio(activeTarget, activeEffect.assistanceFamily || definition && definition.family);
            const activeTargetStillValid = Boolean(activeTarget &&
        ...
        const supportTarget = findNearestNpcAssistanceTarget(scene, entity, controller, definition && definition.family, effectiveRange);
        if (!supportTarget) {
            continue;
        }
        maintainedAssistance = true;
        ...
        if (!hasLock) {
            const lockResult = scene.finalizeTargetLock(entity, supportTarget, {
...
function syncNpcSelfModules(scene, entity, desiredTarget, options = {}) {
    ...
    for (const { moduleItem, definition, effectName } of modules) {
        ...
        const activateWhen = String(definition && definition.activateWhen || "combat")
            .trim()
            .toLowerCase();
        const shouldActivate = activateWhen === "combat"
...
function clearNpcCombatState(scene, entity, controller, options = {}) {
    resetDrifterCombatState(scene, entity, controller, toFiniteNumber(options.nowMs, Date.now()));
...
function syncNpcMovement(scene, entity, target, movementDirective) {
    ...
    if (movementMode === "follow") {
        const followRangeMeters = Math.max(0, toFiniteNumber(movementDirective && movementDirective.followRangeMeters, 0));
        if (entity.mode !== "FOLLOW" ||
    ...
    const orbitDistanceMeters = Math.max(0, toFiniteNumber(movementDirective && movementDirective.orbitDistanceMeters, 0));
    const surfaceDistanceMeters = getSurfaceDistance(entity, target);
    const sameTarget = toPositiveInt(entity.targetEntityID, 0) === toPositiveInt(target.itemID, 0);
    const followRangeMatchesOrbit = Math.abs(toFiniteNumber(entity.followRange, 0) - orbitDistanceMeters) <= 1;
    const currentlyFollowingOrbitBand = entity.mode === "FOLLOW" &&
        sameTarget &&
        followRangeMatchesOrbit;
    const orbitReacquireDistanceMeters = orbitDistanceMeters + Math.max(5_000, orbitDistanceMeters * 0.5);
    const orbitSettleDistanceMeters = orbitDistanceMeters + Math.max(1_000, orbitDistanceMeters * 0.2);
    if (surfaceDistanceMeters > orbitReacquireDistanceMeters ||
...
function deactivateNpcPropulsion(scene, entity) {
    ...
    const pseudoSession = buildNpcPseudoSession(entity);
    const now = toFiniteNumber(scene.getCurrentSimTimeMs && scene.getCurrentSimTimeMs(), Date.now());
    for (const effectState of [...entity.activeModuleEffects.values()]) {
        if (!effectState) {
            continue;
        }
        if (effectState.effectName !== PROPULSION_EFFECT_AFTERBURNER &&
        ...
        if (effectState.npcSyntheticPropulsion === true) {
            ...
            if (effectState.guid) {
                scene.broadcastSpecialFx(entity.itemID, effectState.guid, buildNpcSyntheticPropulsionFxOptions({
                    moduleID: null,
                    moduleTypeID: null,
                    start: false,
                    active: false,
        ...
        scene.deactivatePropulsionModule(pseudoSession, toPositiveInt(effectState.moduleID, 0), {
            reason: "npc",
            deferUntilCycle: false,
        });
    }
...
function syncNpcPropulsion(scene, entity, target, behaviorProfile) {
    ...
    if (propulsionModules.length === 0) {
        const activateDistanceMeters = Math.max(0, toFiniteNumber(behaviorProfile.chasePropulsionActivateDistanceMeters, 10_000));
        const deactivateDistanceMeters = Math.max(0, toFiniteNumber(behaviorProfile.chasePropulsionDeactivateDistanceMeters, activateDistanceMeters));
        const surfaceDistanceMeters = getSurfaceDistance(entity, target);
    ...
    const preferredModuleID = toPositiveInt(preferredPropulsion && preferredPropulsion.moduleItem && preferredPropulsion.moduleItem.itemID, 0);
    const activePropulsionEffects = entity.activeModuleEffects instanceof Map
        ? [...entity.activeModuleEffects.values()].filter((effectState) => (effectState &&
            (effectState.effectName === PROPULSION_EFFECT_AFTERBURNER ||
                effectState.effectName === PROPULSION_EFFECT_MICROWARPDRIVE)))
        : [];
    const hasPreferredPropulsionActive = activePropulsionEffects.some((effectState) => (toPositiveInt(effectState && effectState.moduleID, 0) === preferredModuleID));
    if (surfaceDistanceMeters <= deactivateDistanceMeters) {
    ...
    const pseudoSession = buildNpcPseudoSession(entity);
    scene.activatePropulsionModule(pseudoSession, preferredPropulsion.moduleItem, preferredPropulsion.effectName, {
        repeat: null,
    });
}
function syncNpcWeapons(scene, entity, target) {
    ...
    logNpcCombatDebug("npc.weapons.sync.begin", () => ({
        ...
        entityMissileSource: entityMissileSource
            ? {
                ...
                nextLaunchAtMs: toFiniteNumber(entity &&
    ...
    if (entityMissileSource) {
        ...
        if (getSurfaceDistance(entity, target) <= entityMissileSource.approxRange + 1 &&
            now >= toFiniteNumber(entity.npcEntityMissileState.nextLaunchAtMs, 0)) {
            const launchResult = scene.launchMissile(entity, target.itemID, entityMissileSource, {
                launchTimeMs: now,
                chargeItem: {
    ...
    for (const rawModuleItem of modules) {
        const authoredBankModuleIDs = Array.isArray(rawModuleItem && rawModuleItem.npcWeaponBankModuleIDs)
            ? rawModuleItem.npcWeaponBankModuleIDs
                ...
                .filter((moduleID) => moduleID > 0)
            : [];
        const authoredBankMasterID = toPositiveInt(rawModuleItem && rawModuleItem.npcWeaponBankMasterID, 0);
        const bankGroup = authoredBankMasterID > 0 && authoredBankModuleIDs.length > 1
            ? {
        ...
        const bankMasterModuleID = toPositiveInt(bankGroup && bankGroup.masterModuleID, toPositiveInt(rawModuleItem && rawModuleItem.itemID, 0));
        const bankModuleIDs = bankGroup && Array.isArray(bankGroup.moduleIDs) && bankGroup.moduleIDs.length > 1
            ? [...bankGroup.moduleIDs]
        ...
        const activeEffect = entity.activeModuleEffects instanceof Map
            ? (bankModuleIDs
                ...
                .find(Boolean) ||
                [...entity.activeModuleEffects.values()].find((effectState) => (effectState &&
        ...
        const activationOutcome = scene.activateGenericModule(pseudoSession, moduleItem, String(moduleItem && moduleItem.npcEffectName || "").trim() || null, {
            targetID: target.itemID,
...
function syncNpcReturnHome(scene, entity, controller, behaviorProfile, now) {
    ...
    const arrivalMeters = Math.max(250, toFiniteNumber(behaviorProfile.homeArrivalMeters, 1500));
    ...
    const homeDirection = normalizeVector(subtractVectors(homePosition, entity.position), entity.direction || controller.homeDirection || { x: 1, y: 0, z: 0 });
    const shouldRefreshCommand = entity.mode !== "GOTO" ||
...
function resolveIdleAnchorOrbitDistance(entity, controller, anchorEntity, behaviorProfile) {
    ...
    if (homePosition && anchorEntity && anchorEntity.position) {
        const derivedSurfaceDistance = Math.max(0, distance(homePosition, anchorEntity.position) -
...
function syncNpcIdleAnchorOrbit(scene, entity, controller, behaviorProfile) {
    ...
    const orbitDistanceMeters = resolveIdleAnchorOrbitDistance(entity, controller, anchorEntity, behaviorProfile);
    ...
    if (entity.mode !== "ORBIT" ||
        ...
        Math.abs(toFiniteNumber(entity.orbitDistance, 0) - orbitDistanceMeters) > 1) {
        scene.orbit(buildNpcPseudoSession(entity), anchorEntity.itemID, orbitDistanceMeters, {
            queueHistorySafeContract: true,
            suppressFreshAcquireReplay: true,
        });
    }
...
function tryNpcIdleAnchorWarp(scene, entity, controller, behaviorProfile, nowMs) {
    ...
    if (toPositiveInt(entity.bubbleID, 0) > 0 &&
    ...
    return beginNpcWarpToPoint(scene, entity, controller, anchorDestination, {
...
function scheduleNextThink(controller, behaviorProfile, now, forcedAtMs = null, options = {}) {
    const defaultNextThinkAtMs = now + Math.max(50, toFiniteNumber(behaviorProfile.thinkIntervalMs, 250));
...
function tickController(scene, controller, now) {
    ...
    if (!desiredTarget) {
        ...
        const targetOnlyManualOrder = Boolean(manualOrder &&
            manualOrder.targetID > 0 &&
            (manualOrder.type === "attack" ||
                manualOrder.type === "orbit" ||
                manualOrder.type === "follow"));
        ...
        if (!targetOnlyManualOrder) {
            ...
            const handledIdleAnchorOrbit = syncNpcIdleAnchorOrbit(scene, entity, controller, behaviorProfile);
            if (!handledIdleAnchorOrbit) {
                const handledCapitalReturnHome = entity && entity.capitalNpc === true
                    ? syncCapitalNpcReturnHome(scene, entity, controller, behaviorProfile, { nowMs: now })
                    : false;
    ...
    const movementDirective = entity && entity.capitalNpc === true && desiredTarget
        ? resolveCapitalMovementDirective(entity, behaviorProfile, desiredTarget, baseMovementDirective)
    ...
    controller.returningHome = false;
    publishDrifterPursuitLocation(scene, entity, controller, desiredTarget, now);
    syncNpcChaseVelocity(scene, entity, controller, behaviorProfile, desiredTarget);
    const handledCapitalMovement = entity && entity.capitalNpc === true
        ? syncCapitalNpcMovement(scene, entity, controller, desiredTarget, movementDirective, { nowMs: now })
        : false;
    ...
    if (drifterEngagement &&
        ...
        nextThinkOverrideMs =
            nextThinkOverrideMs === null
                ...
                : Math.min(toFiniteNumber(nextThinkOverrideMs, null), toFiniteNumber(drifterEngagement.nextThinkOverrideMs, nextThinkOverrideMs));
    }
    const maintainCombatLock = maintainLock || drifterEngagement.forceMaintainLock === true;
    const allowCombatWeapons = allowWeapons && drifterEngagement.suppressWeapons !== true;
    ...
    const hasPendingDesiredLock = pendingTargetLocks.some((pendingLock) => toPositiveInt(pendingLock && pendingLock.targetID, 0) === desiredTarget.itemID);
    if (entity &&
    ...
    if (maintainCombatLock) {
        if (!hasDesiredLock && !hasPendingDesiredLock) {
            ...
            const pendingLock = scene.getSortedPendingTargetLocks(entity).find((entry) => toPositiveInt(entry && entry.targetID, 0) === desiredTarget.itemID);
            if (pendingLock) {
    ...
    if (nextThinkOverrideMs === null && hasPendingDesiredLock) {
        const pendingLock = pendingTargetLocks.find((entry) => toPositiveInt(entry && entry.targetID, 0) === desiredTarget.itemID);
        if (pendingLock) {
            nextThinkOverrideMs = toFiniteNumber(pendingLock.completeAtMs, null);
        }
    }
    ...
    const shouldRunAssistanceMaintenance = combatMaintenanceTargetChanged ||
        isNpcMaintenanceDue(controller, "_lastNpcAssistanceMaintenanceAtMs", now, NPC_ASSISTANCE_MAINTENANCE_MS, entity, 1);
    if (maintainCombatLock && allowCombatWeapons && shouldRunAssistanceMaintenance) {
        syncNpcAssistanceModules(scene, entity, controller);
    }
    if (maintainCombatLock && allowCombatWeapons && hasDesiredLock) {
        const shouldRunSelfMaintenance = combatMaintenanceTargetChanged ||
            isNpcMaintenanceDue(controller, "_lastNpcSelfModuleMaintenanceAtMs", now, NPC_SELF_MODULE_MAINTENANCE_MS, entity, 2);
        if (shouldRunSelfMaintenance) {
            syncNpcSelfModules(scene, entity, desiredTarget, {
                combatEnabled: true,
            });
        }
        const shouldRunWeaponMaintenance = combatMaintenanceTargetChanged ||
            isNpcMaintenanceDue(controller, "_lastNpcWeaponMaintenanceAtMs", now, NPC_WEAPON_MAINTENANCE_MS, entity, 3);
        if (shouldRunWeaponMaintenance) {
            syncNpcWeapons(scene, entity, desiredTarget);
        }
        const shouldRunHostileMaintenance = combatMaintenanceTargetChanged ||
            isNpcMaintenanceDue(controller, "_lastNpcHostileMaintenanceAtMs", now, NPC_HOSTILE_MAINTENANCE_MS, entity, 4);
        ...
        deactivateNpcWeapons(scene, entity, {
            excludeModuleIDs: drifterEngagement &&
    ...
    const stableCombatNotBeforeMs = nextThinkOverrideMs === null &&
        maintainCombatLock &&
        allowCombatWeapons &&
        hasDesiredLock
        ? now +
...
function tickScene(scene, now) {
    ...
    const startIndex = Math.max(0, toPositiveInt(scene._npcBehaviorTickCursor, 0)) %
        controllers.length;
    let processed = 0;
    let nextCursor = 0;
    let exhausted = true;
...
function tickControllersByEntityID(scene, entityIDs, now) {
    ...
    const normalizedEntityIDs = [
        ...new Set((Array.isArray(entityIDs) ? entityIDs : [entityIDs])
...
function issueManualOrder(entityID, order) {
    ...
    const manualTargetID = toPositiveInt(normalizedOrder && normalizedOrder.targetID, 0);
    if (normalizedOrder &&
        manualTargetID > 0 &&
