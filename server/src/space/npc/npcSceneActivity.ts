...
function getSceneNpcDeadlineMs(scene) {
  ...
  const respawnDeadlines =
    scene._npcStartupRespawnDeadlines && typeof scene._npcStartupRespawnDeadlines === "object"
      ? Object.values<any>(scene._npcStartupRespawnDeadlines)
      : [];
...
function buildStartupPresenceSummary(systemIDs) {
  ...
  const startupSystems = new Set<any>();
  const concordSystems = new Set<any>();
  const concordAnchors = new Set<any>();
  const concordStargateAnchors = new Set<any>();
  const npcSystems = new Set<any>();
  const npcAnchors = new Set<any>();
  const npcStargateAnchors = new Set<any>();
