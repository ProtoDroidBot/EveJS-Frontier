...
function resolveStoredEntityScopeMetadata(sources = [], options = {}) {
    ...
    for (const source of sourceList) {
        ...
        for (const key of NATIVE_SCOPE_ID_FIELDS) {
            ...
            if (firstIdentityByField.has(key) &&
    ...
    if (options.projectAirInstanceToDungeonSite === true &&
    ...
    const hasDungeonSiteContentOrID = Boolean(normalizePersistentEntityID(metadata.dungeonSiteID) !== null ||
        [...NATIVE_DUNGEON_BOOLEAN_MARKERS].some((key) => metadata[key] === true) ||
        [...NATIVE_DUNGEON_TEXT_MARKERS].some((key) => String(metadata[key] == null ? "" : metadata[key]).trim()) ||
        NATIVE_SCOPE_ARRAY_FIELDS.some((key) => Array.isArray(metadata[key])));
    const hasAirOwnerOperationOrPresentation = Boolean(normalizePersistentEntityID(metadata.airNpeOwnerCharacterID) !== null ||
    ...
    const success = invalidIdentityFields.size === 0 &&
...
function allocateID(tableName, counterKey, options = {}) {
    if (options.transient === true) {
        if (!Number.isInteger(transientCounters[tableName])) {
            const root = ensureRootShape(tableName);
            transientCounters[tableName] = Math.max(toPositiveInt(root && root[counterKey], 0), toPositiveInt(ROOT[tableName] && ROOT[tableName][counterKey], 0));
        }
    ...
    const root = ensureRootShape(tableName);
    const nextID = Math.max(toPositiveInt(root && root[counterKey], 0), toPositiveInt(ROOT[tableName] && ROOT[tableName][counterKey], 0));
    const updatedRoot = {
        ...root,
        [counterKey]: nextID + 1,
    };
...
function listNativeEntitiesForSystem(systemID) {
    ...
    return listNativeEntities().filter((entity) => toPositiveInt(entity && entity.systemID, 0) === normalizedSystemID);
}
...
function listNativeControllers() {
    ...
    controllerCache.byEntityID = new Map(allControllers.map((controller) => [String(toPositiveInt(controller && controller.entityID, 0)), controller]));
...
function listNativeControllersForSystem(systemID) {
    ...
    const systemControllers = listNativeControllers().filter((controller) => toPositiveInt(controller && controller.systemID, 0) === normalizedSystemID);
...
function listNativeWreckItemsForWreck(wreckID) {
    ...
    return listNativeWreckItems().filter((itemRecord) => toPositiveInt(itemRecord && itemRecord.wreckID, 0) === normalizedWreckID);
}
...
function buildNativeWreckContents(wreckID) {
    ...
    return listNativeWreckItemsForWreck(normalizedWreckID).map((itemRecord) => ({
        ...
        quantity: itemRecord && itemRecord.singleton === true
            ? -1
            : toPositiveInt(itemRecord && itemRecord.quantity, 0),
        stacksize: itemRecord && itemRecord.singleton === true
            ? 1
            : toPositiveInt(itemRecord && itemRecord.quantity, 0),
...
//# sourceMappingURL=nativeNpcStore.js.map