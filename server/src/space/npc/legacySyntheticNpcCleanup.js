...
const { findShipItemById, removeInventoryItem, } = require(path.join(__dirname, "../../services/inventory/itemStore"));
const { unregisterController, } = require(path.join(__dirname, "./npcRegistry"));
const { ENTITY_TYPE, } = require(path.join(__dirname, "../entityConstants"));
const { normalizeBehaviorOverrides, } = require(path.join(__dirname, "./npcBehaviorLoop"));
const { cloneVector, } = require(path.join(__dirname, "./npcAnchors"));
...
function parseNpcCustomInfo(customInfo) {
    ...
    try {
        ...
        return {
            ...
            homePosition: npc.homePosition && typeof npc.homePosition === "object"
                ? cloneVector(npc.homePosition)
                : null,
            homeDirection: npc.homeDirection && typeof npc.homeDirection === "object"
...
function cleanupNpcOwnerArtifacts(ownerCharacterID) {
    ...
    require("../../services/skills/skillState").removeSkillsRecord(normalizedOwnerCharacterID);
    require("../../services/character/characterState").removeCharacterRecord(normalizedOwnerCharacterID);
}
...
function cleanupLegacySyntheticNpcShips(scene) {
    ...
    for (const entity of [...scene.dynamicEntities.values()]) {
        ...
        const ownerCharacterID = toPositiveInt(npcMetadata.ownerCharacterID, toPositiveInt(shipItem.ownerID, 0));
        const destroyResult = destroyLegacySyntheticNpcController({
            entityID: entity.itemID,
            systemID: scene.systemID,
            ownerCharacterID,
            entityType: npcMetadata.entityType || ENTITY_TYPE.NPC,
...
//# sourceMappingURL=legacySyntheticNpcCleanup.js.map