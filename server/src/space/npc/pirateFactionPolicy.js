...
function isSpecialSpaceSystem(systemID, systemRecord = null) {
    ...
    return ((numericSystemID >= 31_000_000 && numericSystemID <= 31_999_999) ||
...
function resolvePirateFactionForSystem(systemID, options = {}) {
    ...
    if (explicitFactionKey) {
        return buildSystemFactionResolution(numericSystemID, system, explicitFactionKey, "explicit_override");
    }
    if (options.allowSpecialSpaceEmpireFallback !== true &&
    ...
    if (PIRATE_FACTION_BY_FACTION_ID[factionID]) {
        return buildSystemFactionResolution(numericSystemID, system, PIRATE_FACTION_BY_FACTION_ID[factionID], "faction_id");
    }
    if (PIRATE_FACTION_BY_REGION_ID[regionID]) {
        return buildSystemFactionResolution(numericSystemID, system, PIRATE_FACTION_BY_REGION_ID[regionID], "region");
    }
    if (EMPIRE_FALLBACK_PIRATE_BY_FACTION_ID[factionID]) {
        return buildSystemFactionResolution(numericSystemID, system, EMPIRE_FALLBACK_PIRATE_BY_FACTION_ID[factionID], "empire_fallback");
    }
...
function resolveTemplatePirateFactionKeys(template = {}) {
    ...
    const spawnQueryFactionKeys = [...new Set(listTemplateSpawnQueries(template)
...
//# sourceMappingURL=pirateFactionPolicy.js.map