...
const { getControllerByEntityID, } = require(path.join(__dirname, "./npcRegistry"));
const { tickControllersByEntityID, } = require(path.join(__dirname, "./npcBehaviorLoop"));
const { listAmbientStartupControllerRecordsForSystem, canDematerializeAmbientStartupController, } = require(path.join(__dirname, "./npcAmbientMaterialization"));
const { listCombatDormancyControllerRecordsForSystem, canDematerializeDormantCombatController, } = require(path.join(__dirname, "./npcCombatDormancy"));
...
function collectRelevantPublicGridClusterKeys(scene, options = {}) {
    ...
    for (const entity of normalizeArray(options.relevantEntities)) {
        addClusterKey(clusterKeys, scene.getPublicGridClusterKeyForEntity
            ? scene.getPublicGridClusterKeyForEntity(entity)
            : null);
    }
    for (const position of normalizeArray(options.relevantPositions)) {
        addClusterKey(clusterKeys, scene.getPublicGridClusterKeyForPosition
    ...
    if (options.includeSceneSessions !== false && scene.sessions instanceof Map) {
        for (const session of scene.sessions.values()) {
            addClusterKey(clusterKeys, scene.getPublicGridClusterKeyForSession
...
function hasStartupAnchorRelevanceContext(scene, options = {}) {
    ...
    if (normalizeArray(options.relevantClusterKeys).length > 0 ||
...
function getStartupControllerAnchorClusterKey(scene, controllerRecord) {
    ...
    const homePosition = controllerRecord.homePosition &&
...
function materializeRelevantCombatControllers(scene, relevantClusterKeys, options = {}) {
    ...
    if (materialized.length > 0 && options.catchUpBehavior !== false) {
        const nowMs = toFiniteNumber(options.nowMs, scene.getCurrentSimTimeMs ? scene.getCurrentSimTimeMs() : Date.now());
        tickControllersByEntityID(scene, materialized.map((entry) => entry.entityID), nowMs);
    }
    return {
        success: true,
        data: {
...
function dematerializeIrrelevantCombatControllers(scene, relevantClusterKeys, options = {}) {
    ...
    for (const controllerRecord of listCombatDormancyControllerRecordsForSystem(scene.systemID)) {
        ...
        const liveController = getControllerByEntityID(entityID) || controllerRecord;
        if (!canDematerializeDormantCombatController(scene, liveController, entity, {
            nowMs,
            ignoreSceneSessionCount: true,
        })) {
            skipped.push({
...
function syncRelevantStartupControllersForScene(scene, options = {}) {
    ...
    const ambientMaterializationResult = options.materializeAmbientStartup === false
    ...
    const combatMaterializationResult = options.materializeDormantCombat === false
        ? { success: true, data: { materialized: [], materializedCount: 0 } }
    ...
    const ambientDematerializationResult = options.dematerializeAmbientStartup === false
        ? {
            success: true,
    ...
    const combatDematerializationResult = options.dematerializeDormantCombat === false
        ? {
            success: true,
            data: { dematerialized: [], dematerializedCount: 0, skipped: [], skippedCount: 0 },
        }
...
//# sourceMappingURL=npcAnchorRelevance.js.map