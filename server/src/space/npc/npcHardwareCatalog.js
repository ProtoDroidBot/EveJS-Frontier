...
const { getNpcCapabilityTypeID, isNpcChargeCompatibleWithModule, resolveNpcPropulsionEffectName, } = require(path.join(__dirname, "./npcCapabilityResolver"));
...
function buildPublishedTypeIDsForGroups(groupIDs = []) {
    const allowedGroups = new Set((Array.isArray(groupIDs) ? groupIDs : [groupIDs])
    ...
    return Object.freeze(rows
        .filter((row) => (allowedGroups.has(toPositiveInt(row && row.groupID, 0)) &&
        row &&
        row.published !== false))
        .map((row) => toPositiveInt(row && row.typeID, 0))
        .filter((value) => value > 0));
}
const TRIG_PRECURSOR_WEAPON_TYPE_IDS = buildPublishedTypeIDsForGroups(PRECURSOR_WEAPON_GROUP_ID);
...
const NPC_HARDWARE_POLICY_BY_FAMILY = Object.freeze(Object.fromEntries(Object.entries(NPC_HARDWARE_CATALOG).map(([familyName, entry]) => [
...
function hasEntityMissileLaunchCapability(definition) {
    ...
    return toPositiveInt(attributes && attributes[ATTRIBUTE_ENTITY_MISSILE_TYPE_ID], 0) > 0;
}
function resolveNpcHardwareFamily(definition) {
    ...
    const miningRole = normalizeHardwareFamilyName(profile.miningRole ||
        profile.role);
    ...
    if (entityType === ENTITY_TYPE.NPC && hasEntityMissileLaunchCapability(definition)) {
        return HARDWARE_FAMILY_ENTITY_MISSILE_NPC;
    }
    if (entityType === ENTITY_TYPE.NPC &&
        toPositiveInt(profile.factionID, 0) === 500014) {
        return HARDWARE_FAMILY_MINING_OPS;
    }
    if (entityType === ENTITY_TYPE.NPC) {
        return (NPC_FACTION_HARDWARE_FAMILY[toPositiveInt(profile.factionID, 0)] ||
            null);
    }
    return null;
}
function resolveSyntheticChasePropulsionTemplate(behaviorProfile) {
    const tier = normalizeHardwareFamilyName(behaviorProfile && behaviorProfile.syntheticChasePropulsionTier);
...
function resolveModuleRole(moduleEntry, chargeTypeIDs = []) {
    ...
    if (actualGroupID === GROUP_STASIS_WEB ||
    ...
    const isWeaponLike = typeHasEffectName(capabilityTypeID, "targetAttack") ||
        typeHasEffectName(capabilityTypeID, "turretFitted") ||
        typeHasEffectName(capabilityTypeID, "launcherFitted");
    if (!isWeaponLike) {
        return "other";
    }
    if (chargeTypeIDs.some((chargeTypeID) => (isNpcChargeCompatibleWithModule(moduleItem, chargeTypeID)))) {
        return "weapon";
    }
    return "weapon";
}
function validateWeaponCharges(family, weaponModules, chargeTypeIDs) {
    ...
    const allowedChargeTypeIDs = familyPolicy && familyPolicy.chargeTypeIDs instanceof Set
    ...
    for (const weaponModule of weaponModules) {
        ...
        const compatibleChargeTypeIDs = chargeTypeIDs.filter((chargeTypeID) => (isNpcChargeCompatibleWithModule(weaponModule, chargeTypeID)));
        ...
        if (compatibleChargeTypeIDs.some((chargeTypeID) => !allowedChargeTypeIDs.has(chargeTypeID))) {
            return {
                success: false,
...
function validateNpcHardwareDefinition(definition) {
    ...
    for (const moduleEntry of modules) {
        ...
        if (role === "weapon") {
            ...
            if (!(familyPolicy.weaponTypeIDs instanceof Set) ||
        ...
        if (role === "tackle") {
            if (!(familyPolicy.tackleTypeIDs instanceof Set) ||
        ...
        if (role === "hostileutility") {
            if (!(familyPolicy.utilityTypeIDs instanceof Set) ||
...
//# sourceMappingURL=npcHardwareCatalog.js.map