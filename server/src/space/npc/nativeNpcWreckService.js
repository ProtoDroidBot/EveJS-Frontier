...
const { ITEM_FLAGS, getItemMetadata, grantItemToCharacterLocation, } = require(path.join(__dirname, "../../services/inventory/itemStore"));
...
const { getNpcLootTable, } = require(path.join(__dirname, "./npcData"));
...
const { isNpcFactionSdeTypeAllowed } = require(path.join(__dirname, "../../config/npcFactionConfig"));
...
function filterNpcGeneratedLoot(source, entries, matcher = isNpcFactionSdeTypeAllowed) {
    return (Array.isArray(entries) ? entries : []).filter(entry => matcher(source, "loot", entry.typeID));
}
...
function buildNativeWreckRuntimeEntity(wreckRecord, options = {}) {
    ...
    const wreckItemRecord = nativeNpcStore.buildNativeWreckInventoryItem(wreckRecord && wreckRecord.wreckID);
    if (!wreckItemRecord) {
        return null;
    }
    const entity = spaceRuntime.buildRuntimeSpaceEntityFromItemRecord(wreckItemRecord, toPositiveInt(wreckRecord && wreckRecord.systemID, 0), toFiniteNumber(options.nowMs, Date.now()), { entityScopeMetadata });
    if (!entity) {
        return null;
    }
    ...
    entity.conditionState = cloneValue(wreckItemRecord.conditionState || entity.conditionState || null);
    ...
    entity.isEmpty =
        nativeNpcStore.listNativeWreckItemsForWreck(toPositiveInt(wreckRecord && wreckRecord.wreckID, 0)).length === 0;
    ...
    entity.lootRightCorpID = toPositiveInt(wreckRecord && wreckRecord.lootRightCorpID, toPositiveInt(wreckRecord && wreckRecord.corporationID, 0));
    entity.dunRotation =
    ...
    if (typeof spaceRuntime.applyRuntimeEntityScopeMetadata === "function") {
        spaceRuntime.applyRuntimeEntityScopeMetadata(entity, entityScopeMetadata);
    }
...
function refreshNativeWreckRuntimeEntity(systemID, wreckID, options = {}) {
    ...
    const runtimeScopeResolution = nativeNpcStore.resolveStoredEntityScopeMetadata([scopeResolution.data.metadata, entity], { projectAirInstanceToDungeonSite: true });
    if (!runtimeScopeResolution.success) {
        return {
            success: false,
            errorMsg: runtimeScopeResolution.errorMsg,
        };
    }
    const entityScopeMetadata = runtimeScopeResolution.data.metadata;
...
function spawnNativeWreck(systemID, wreckID, options = {}) {
    ...
    const entity = buildNativeWreckRuntimeEntity(wreckRecord, {
        nowMs: typeof scene.getCurrentSimTimeMs === "function"
    ...
    const broadcastOptions = options.broadcastOptions && typeof options.broadcastOptions === "object"
...
function transferNativeWreckItemToCharacterLocation(options = {}) {
    ...
    const grantResult = grantItemToCharacterLocation(characterID, destinationLocationID, destinationFlagID, itemType, singleton ? 1 : requestedQuantity, {
        singleton,
...
function destroyNativeNpcEntityWithWreck(systemID, shipEntity, options = {}) {
    ...
    const wreckType = resolveEntityWreckType({
        ...
        capitalNpc: (nativeEntityRecord && nativeEntityRecord.capitalNpc === true) ||
    ...
    const scopeResolution = nativeNpcStore.resolveStoredEntityScopeMetadata([nativeEntityRecord, shipEntity], { projectAirInstanceToDungeonSite: true });
    if (!scopeResolution.success) {
        return {
            success: false,
            errorMsg: scopeResolution.errorMsg,
    ...
    if (scene &&
        ...
        const cleanupResult = resetNpcSupercarrierWing(scene, shipEntity, controller, {
            removeContents: options.removeContents !== false,
        });
        destroyedFighterCount = Number(cleanupResult &&
    ...
    const wreckRecord = {
        ...
        sourceNpcCharacterID: toPositiveInt(nativeEntityRecord && nativeEntityRecord.npcCharacterID, toPositiveInt(shipEntity.npcCharacterID, 0)) || null,
        ...
        sourceTypeID: toPositiveInt(shipEntity && shipEntity.slimTypeID, toPositiveInt(shipEntity && shipEntity.typeID, 0)),
        ownerID: toPositiveInt(options.ownerCharacterID ||
            options.characterID ||
            options.killerCharacterID, toPositiveInt(shipEntity.ownerID, 0)),
        corporationID: toPositiveInt(options.corporationID, toPositiveInt(shipEntity.corporationID, 0)),
        allianceID: toPositiveInt(options.allianceID, toPositiveInt(shipEntity.allianceID, 0)),
        warFactionID: toPositiveInt(options.warFactionID, toPositiveInt(shipEntity.warFactionID, 0)),
        securityStatus: toFiniteNumber(shipEntity.securityStatus, 0),
        lootRightCorpID: toPositiveInt(options.corporationID, toPositiveInt(shipEntity.corporationID, 0)),
    ...
    spatialTrace.recordWreckRecordCreated(scene, wreckRecord, {
        persistenceKind: wreckRecord.transient === true
            ? "native-transient-record"
    ...
    const rolledLootEntries = filterNpcGeneratedLoot(nativeEntityRecord || shipEntity, rollNpcLootEntries(lootTable));
    ...
    try {
        ...
        if (dungeonUniverseSiteService &&
            ...
            // Pass the entity object (not just the ID) so objective/fetch-trigger matching can
            // use its typeID/nameID/dunObjectID — required for dungeon-fetch trigger kills.
            dungeonUniverseSiteService.handleEncounterEntityDestroyed(scene, shipEntity || entityID, {
                nowMs,
            });
        }
    }
