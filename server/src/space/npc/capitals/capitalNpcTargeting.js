...
const { resolveCapitalDoctrine, } = require("./capitalNpcDoctrine");
const { getCapitalControllerState, } = require("./capitalNpcState");
...
function buildPreferredRangeDistanceScore(distanceMeters, doctrine) {
    ...
    const settleToleranceMeters = Math.max(1, toFiniteNumber(doctrine.settleToleranceMeters, 1));
...
function scoreCapitalTarget(entity, controller, candidate, options = {}) {
    ...
    const candidateDistanceMeters = Math.max(0, toFiniteNumber(options.getSurfaceDistance
...
function resolveCapitalBehaviorTarget(scene, entity, controller, behaviorProfile, options = {}) {
    ...
    const classID = String(doctrine && doctrine.classID ||
    ...
    const currentTarget = currentTargetID > 0
        ...
        : null;
    const aggressionRangeMeters = Math.max(0, toFiniteNumber(options.aggressionRangeMeters, behaviorProfile && behaviorProfile.aggressionRangeMeters));
    const isEligibleTarget = (candidate) => {
        ...
        const candidateClass = String(options.resolveCombatActorClass
            ? options.resolveCombatActorClass(candidate)
            : "").trim().toLowerCase();
        if (Array.isArray(options.allowedTargetClasses) &&
    ...
    for (const candidate of scene.dynamicEntities.values()) {
        ...
        scoredCandidates.push(scoreCapitalTarget(entity, controller, candidate, {
    ...
    if (currentTarget && isEligibleTarget(currentTarget)) {
        ...
        if (bestCandidate.candidate.itemID !== currentTarget.itemID &&
...
//# sourceMappingURL=capitalNpcTargeting.js.map