...
const { getNpcWeaponModules, estimateNpcWeaponEffectiveRange, } = require(path.join(__dirname, "../npcEquipment"));
const { getCapitalRuntimeConfig, } = require("./capitalNpcRuntimeConfig");
...
function getSurfaceDistance(left, right) {
    ...
    return Math.max(0, Math.sqrt((dx ** 2) + (dy ** 2) + (dz ** 2)) -
...
function resolveCapitalClassID(entity, behaviorProfile = {}) {
    const explicitClassID = String((entity && entity.capitalClassID) ||
...
function resolveWeaponMetrics(entity) {
    const weaponRanges = getNpcWeaponModules(entity)
        .map((moduleItem) => Math.max(0, toFiniteNumber(estimateNpcWeaponEffectiveRange(entity, moduleItem), 0)))
...
function buildDoctrineForEntity(entity, behaviorProfile = {}) {
    ...
    const explicitRange = Math.max(toFiniteNumber(behaviorProfile.followRangeMeters, 0), toFiniteNumber(behaviorProfile.orbitDistanceMeters, 0));
    const weightedWeaponRange = Math.max(weaponMetrics.medianRangeMeters, weaponMetrics.longestRangeMeters * runtimeConfig.preferredRangeFactor);
    const preferredCombatRangeMeters = clamp(Math.round(Math.max(explicitRange, weightedWeaponRange, runtimeConfig.minimumPreferredRangeMeters)), runtimeConfig.minimumPreferredRangeMeters, Math.max(runtimeConfig.minimumPreferredRangeMeters, runtimeConfig.maximumPreferredRangeMeters, explicitRange, weaponMetrics.longestRangeMeters));
    const settleToleranceMeters = Math.max(runtimeConfig.settleToleranceMinMeters, Math.round(preferredCombatRangeMeters * runtimeConfig.settleToleranceRatio));
...
function resolveCapitalDoctrine(entity, behaviorProfile = {}) {
    ...
    const signature = [
        ...
        Math.max(toFiniteNumber(behaviorProfile.followRangeMeters, 0), toFiniteNumber(behaviorProfile.orbitDistanceMeters, 0)),
...
function resolveCapitalMovementDirective(entity, behaviorProfile, target, fallbackDirective = {}) {
    ...
    const preferredCombatRangeMeters = Math.max(0, toFiniteNumber(doctrine.preferredCombatRangeMeters, 0));
    if (preferredCombatRangeMeters <= 0) {
        return fallbackDirective;
    }
    const settleToleranceMeters = Math.max(0, toFiniteNumber(doctrine.settleToleranceMeters, 0));
    const currentDistanceMeters = getSurfaceDistance(entity, target);
    if (currentDistanceMeters >= preferredCombatRangeMeters - settleToleranceMeters &&
...
//# sourceMappingURL=capitalNpcDoctrine.js.map