...
const { getCapitalControllerState, toFiniteNumber, toPositiveInt, } = require("./capitalNpcState");
...
function syncCapitalNpcMovement(scene, entity, controller, target, movementDirective, options = {}) {
    ...
    const refreshIntervalMs = Math.max(50, toPositiveInt(runtimeConfig.repositionThinkIntervalMs, 250));
    const preferredRangeMeters = Math.max(0, toFiniteNumber(doctrine.preferredCombatRangeMeters, 0));
    ...
    if (surfaceDistanceMeters > upperBoundMeters) {
        if (!hasMatchingFollowCommand(entity, targetID, preferredRangeMeters)) {
            scene.followBall(buildNpcPseudoSession(entity), targetID, preferredRangeMeters, CAPITAL_MOVEMENT_OPTIONS);
    ...
    if (surfaceDistanceMeters < lowerBoundMeters) {
        const withdrawDirection = normalizeVector(subtractVectors(entity && entity.position, target && target.position), entity && entity.direction || { x: 1, y: 0, z: 0 });
        if (shouldRefreshDirectionalCommand(entity, capitalState, "withdraw", withdrawDirection, nowMs, refreshIntervalMs)) {
            scene.gotoDirection(buildNpcPseudoSession(entity), withdrawDirection, CAPITAL_MOVEMENT_OPTIONS);
            rememberMovementState(capitalState, {
                mode: "withdraw",
                targetID,
                rangeMeters: preferredRangeMeters,
...
function syncCapitalNpcReturnHome(scene, entity, controller, behaviorProfile, options = {}) {
    ...
    const arrivalMeters = Math.max(1_000, toFiniteNumber(behaviorProfile.homeArrivalMeters, 6_000));
    ...
    const refreshIntervalMs = Math.max(50, toPositiveInt(runtimeConfig.repositionThinkIntervalMs, 250));
    const homeDirection = normalizeVector(subtractVectors(homePosition, entity && entity.position), entity && entity.direction || controller.homeDirection || { x: 1, y: 0, z: 0 });
    controller.returningHome = true;
    if (shouldRefreshDirectionalCommand(entity, capitalState, "home", homeDirection, nowMs, refreshIntervalMs)) {
        scene.gotoDirection(buildNpcPseudoSession(entity), homeDirection, CAPITAL_MOVEMENT_OPTIONS);
...
//# sourceMappingURL=capitalNpcMovement.js.map