...
const { CAPITAL_NPC_AUTHORITY, } = require("./capitalNpcAuthority");
...
function buildCapitalPools() {
    ...
    const rows = [
        buildPoolRow("capital_npc_all", "Capital NPCs", "All authored pirate and rogue-drone capital NPCs.", ["capitals", "capital", "all capitals", "all cap npcs"], allEntries),
        buildPoolRow("capital_npc_dreadnoughts", "Capital Dreadnoughts", "All authored pirate dreadnought NPCs.", ["dreads", "dreadnoughts", "capital dreads"], byClass.get("dreadnought") || []),
        buildPoolRow("capital_npc_titans", "Capital Titans", "All authored pirate titan NPCs.", ["titans", "capital titans"], byClass.get("titan") || []),
        buildPoolRow("capital_npc_supercarriers", "Capital Supercarriers", "All authored capital carrier and supercarrier NPCs.", ["supers", "supercarriers", "capital supers"], byClass.get("supercarrier") || []),
    ];
    ...
    for (const authorityEntry of CAPITAL_NPC_AUTHORITY) {
        ...
        rows.push(buildPoolRow(`capital_npc_${factionID}`, `${authorityEntry.faction.name} Capitals`, `${authorityEntry.faction.name} capital NPC pool.`, [
...
function getCapitalNpcDataset() {
    ...
    const extraProfileAliasesByID = Object.fromEntries(CAPITAL_NPC_AUTHORITY.map((entry) => [
        entry.profileID,
        [entry.shortName, entry.name, `${entry.faction.id} ${entry.classID}`],
    ]));
    datasetCache.rowsByTableName = rowsByTableName;
    datasetCache.poolTokens = buildCommandTokenIndex(rowsByTableName[NPC_TABLE.SPAWN_POOLS], "spawnPoolID", extraPoolAliasesByID);
    datasetCache.profileTokens = buildCommandTokenIndex(rowsByTableName[NPC_TABLE.PROFILES], "profileID", extraProfileAliasesByID);
    datasetCache.rowsByID = Object.freeze({
        profiles: new Map(rowsByTableName[NPC_TABLE.PROFILES].map((row) => [row.profileID, row])),
        spawnPools: new Map(rowsByTableName[NPC_TABLE.SPAWN_POOLS].map((row) => [row.spawnPoolID, row])),
        authority: new Map(CAPITAL_NPC_AUTHORITY.map((row) => [row.profileID, row])),
    });
    return datasetCache;
}
...
//# sourceMappingURL=capitalNpcCatalog.js.map