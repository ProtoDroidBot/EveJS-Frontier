...
const CAPITAL_NPC_AUTHORITY = deepFreeze(Array.isArray(authorityRoot && authorityRoot.entries)
    ? authorityRoot.entries
    : []);
const FACTION_META = deepFreeze(Object.fromEntries(CAPITAL_NPC_AUTHORITY
    .map((entry) => entry && entry.faction)
    .filter((entry) => entry && String(entry.id || "").trim().length > 0)
    .map((entry) => [entry.id, entry])));
const CAPITAL_NPC_AUTHORITY_SOURCE = deepFreeze(authorityRoot && authorityRoot.source && typeof authorityRoot.source === "object"
    ? authorityRoot.source
    : {});
const CAPITAL_NPC_MANIFESTS_BY_PROFILE_ID = deepFreeze(authorityRoot &&
...
//# sourceMappingURL=capitalNpcAuthority.js.map