...
const { CAPITAL_NPC_AUTHORITY, CAPITAL_NPC_MANIFESTS_BY_PROFILE_ID, } = require("./capitalNpcAuthority");
...
function resolveCapitalNpcWreckType(input, shipTypeID = null) {
    ...
    const normalizedShipTypeID = toPositiveInt(lookup && lookup.shipTypeID, toPositiveInt(shipTypeID, 0));
    ...
    const hullClass = (resolveCapitalHullClass(lookup && lookup.classID) ||
    ...
    const factionLabels = resolveFactionWreckLabels(lookup && lookup.factionName, lookup && lookup.groupName, lookup && lookup.itemName, itemMeta && itemMeta.groupName, itemMeta && itemMeta.name, normalizedProfileID);
...
//# sourceMappingURL=capitalNpcWrecks.js.map