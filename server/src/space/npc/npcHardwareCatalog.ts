interface NpcHardwareCatalogEntry {
  weaponTypeIDs: readonly number[];
  chargeTypeIDs: readonly number[];
  tackleTypeIDs?: readonly number[];
  utilityTypeIDs?: readonly number[];
}
...
function buildPublishedTypeIDsForGroups(groupIDs: number | readonly number[] = []) {
...
const NPC_HARDWARE_CATALOG: Readonly<Record<string, NpcHardwareCatalogEntry>> = Object.freeze({
...
function buildAllowedTypeIDSet(entries: Record<string, any> = {}) {
...
function resolveModuleRole(moduleEntry, chargeTypeIDs: any[] = []) {
