...
} = require(path.join(__dirname, "./pirateFactionPolicy"));
const {
  ENTITY_TYPE,
...
function random(options: Record<string, any> = {}) {
  return typeof options.random === "function"
    ? options.random()
    : randomProvider();
}

function chooseWeightedKey(weights: Record<string, any> = {}, options: Record<string, any> = {}) {
  const entries = Object.entries(weights)
    .map(([key, weight]): [string, number] => [key, Math.max(0, toFiniteNumber(weight, 0))])
...
function chooseArrayEntry(values: readonly any[] = [], options: Record<string, any> = {}) {
...
function resolveConcretePirateFactionKey(systemID, options: Record<string, any> = {}) {
...
function resolveConfigValue(key, options: Record<string, any> = {}) {
...
function resolveBeltRatConfig(options: Record<string, any> = {}) {
  return {
...
function getOrCreateBeltState(systemID, beltID) {
  ...
  if (!state) {
    state = {
      ...
      spawnSequence: 0,
      spawnedEntityIDs: new Set<any>(),
      npcIdentityGroups: new Map<any, any>(),
    };
...
function findNearestBeltForEntity(scene, entity, options: Record<string, any> = {}) {
...
function countActiveBeltRatGroups(scene, beltEntity, state) {
  ...
  const activeGroupKeys = new Set<any>();
...
function resolveAvailableBeltNpcIdentitySlot(scene, state) {
  ...
  if (!(state.npcIdentityGroups instanceof Map)) {
    state.npcIdentityGroups = new Map<any, any>();
  }
  for (const [ordinal, entityIDs] of state.npcIdentityGroups) {
    const hasLiveMember = [...entityIDs].some((entityID) => (
...
function resolveSpawnCount(securityProfile, options: Record<string, any> = {}) {
...
function buildProfileIDForFactionHull(factionKey, hullClass, options: Record<string, any> = {}) {
...
function chooseStandardPoolProfileIDForFactionHull(factionKey, hullClass, options: Record<string, any> = {}) {
...
function chooseWeightedPoolEntry(pool, options: Record<string, any> = {}) {
...
function rollSpecialSpawnKind(systemID, factionKey, securityProfile, resolvedConfig, options: Record<string, any> = {}) {
...
function resolveSpecialSpawnCount(kind, options: Record<string, any> = {}) {
  switch (String(kind || "").toLowerCase()) {
    case "hauler": {
...
function chooseWeightedCapitalPoolDefinition(pool, resolvedConfig, options: Record<string, any> = {}) {
...
function buildSpecialBeltRatSpawnPlan(systemID, factionKey, securityProfile, resolvedConfig, options: Record<string, any> = {}) {
  const forcedKind = String(
    options.forceSpecialSpawnKind ||
    options.specialSpawnKind ||
    "",
...
function buildBeltRatSpawnPlan(systemID, options: Record<string, any> = {}) {
  const securityProfile: Record<string, any> = resolveSecurityProfile(systemID);
...
function spawnBeltRatGroup(scene, session, beltEntity, state, plan, options: Record<string, any> = {}) {
  ...
  const spawned = Array.isArray(spawnResult.data.spawned)
    ...
    : [];
  const identityEntityIDs = new Set<any>();
...
function maybeSpawnForBeltArrival(scene, session, beltEntity, options: Record<string, any> = {}) {
  ...
  const planResult: Record<string, any> = buildBeltRatSpawnPlan(systemID, options);
...
function maybeSpawnForSessionArrival(scene, session, shipEntity, options: Record<string, any> = {}) {
...
function configureForTests(options: Record<string, any> = {}) {
