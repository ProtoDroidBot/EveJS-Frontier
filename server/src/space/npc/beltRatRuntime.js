...
const { buildNpcDefinition, resolveNpcSpawnPool, } = require(path.join(__dirname, "./npcData"));
const { PIRATE_FACTION_BY_REGION_ID, resolvePirateFactionKeyForSystem, } = require(path.join(__dirname, "./pirateFactionPolicy"));
...
function chooseArrayEntry(values = [], options = {}) {
    ...
    const index = Math.min(values.length - 1, Math.floor(random(options) * values.length));
    return values[index];
}
...
function getSecurityStatusForSystem(systemID) {
    ...
    return toFiniteNumber(system && (system.securityStatus ?? system.security), 0);
}
function resolveSecurityProfile(systemID) {
    ...
    const profile = SECURITY_PROFILES.find((entry) => securityStatus >= entry.minSecurity) || SECURITY_PROFILES[SECURITY_PROFILES.length - 1];
    return {
        ...profile,
        securityStatus,
        eligible: true,
...
function resolveBeltRatConfig(options = {}) {
    return {
        ...
        chanceHighSec: clamp(resolveConfigValue("asteroidBeltNpcRatHighSecChance", options), 0, 1),
        chanceLowSec: clamp(resolveConfigValue("asteroidBeltNpcRatLowSecChance", options), 0, 1),
        chanceNullSec: clamp(resolveConfigValue("asteroidBeltNpcRatNullSecChance", options), 0, 1),
        rollCooldownMs: Math.max(0, toInt(resolveConfigValue("asteroidBeltNpcRatRollCooldownMs", options), 120_000)),
        respawnCooldownMs: Math.max(0, toInt(resolveConfigValue("asteroidBeltNpcRatRespawnCooldownMs", options), 1_200_000)),
        maxActiveGroupsPerBelt: Math.max(1, toInt(resolveConfigValue("asteroidBeltNpcRatMaxActiveGroupsPerBelt", options), 1)),
        landingRadiusMeters: Math.max(1_000, toFiniteNumber(resolveConfigValue("asteroidBeltNpcRatLandingRadiusMeters", options), DEFAULT_LANDING_RADIUS_METERS)),
        spawnDistanceMeters: Math.max(1_000, toFiniteNumber(resolveConfigValue("asteroidBeltNpcRatSpawnDistanceMeters", options), 30_000)),
        specialsEnabled: resolveConfigValue("asteroidBeltNpcRatSpecialsEnabled", options) !== false,
        haulerChance: clamp(resolveConfigValue("asteroidBeltNpcRatHaulerChance", options), 0, 1),
        commanderChance: clamp(resolveConfigValue("asteroidBeltNpcRatCommanderChance", options), 0, 1),
        officerChance: clamp(resolveConfigValue("asteroidBeltNpcRatOfficerChance", options), 0, 1),
        officerMaxSecurity: clamp(resolveConfigValue("asteroidBeltNpcRatOfficerMaxSecurity", options), -1, 1),
        officerRequireHomeRegion: resolveConfigValue("asteroidBeltNpcRatOfficerRequireHomeRegion", options) !== false,
        capitalEnabled: resolveConfigValue("asteroidBeltNpcRatCapitalEnabled", options) !== false,
        capitalChance: clamp(resolveConfigValue("asteroidBeltNpcRatCapitalChance", options), 0, 1),
        capitalMaxSecurity: clamp(resolveConfigValue("asteroidBeltNpcRatCapitalMaxSecurity", options), -1, 1),
        capitalAllowedClasses: parseCapitalAllowedClasses(resolveConfigValue("asteroidBeltNpcRatCapitalClasses", options)),
        capitalMaxActiveGroupsPerSystem: Math.max(0, toInt(resolveConfigValue("asteroidBeltNpcRatCapitalMaxActiveGroupsPerSystem", options), 1)),
    };
}
...
function findNearestBeltForEntity(scene, entity, options = {}) {
    ...
    const maxDistanceMeters = Math.max(1_000, toFiniteNumber(options.maxDistanceMeters, resolvedConfig.landingRadiusMeters));
    let nearest = null;
...
function isSpecialSpawnKindEligible(kind, systemID, factionKey, securityProfile, resolvedConfig) {
    switch (String(kind || "").toLowerCase()) {
        case "hauler":
            return Boolean(securityProfile &&
...
function rollSpecialSpawnKind(systemID, factionKey, securityProfile, resolvedConfig, options = {}) {
    const forcedKind = String(options.forceSpecialSpawnKind ||
        options.specialSpawnKind ||
        "").trim().toLowerCase();
    if (forcedKind) {
        return isSpecialSpawnKindEligible(forcedKind, systemID, factionKey, securityProfile, resolvedConfig)
            ? forcedKind
            : null;
    }
...
function chooseWeightedCapitalPoolDefinition(pool, resolvedConfig, options = {}) {
    ...
    for (const entry of entries) {
        ...
        const definition = buildNpcDefinition(profileID);
        const capitalClassID = normalizeCapitalClassToken(definition && definition.profile && definition.profile.capitalClassID);
        if (!definition ||
...
function buildSpecialBeltRatSpawnPlan(systemID, factionKey, securityProfile, resolvedConfig, options = {}) {
    const forcedKind = String(options.forceSpecialSpawnKind ||
        ...
        "").trim().toLowerCase();
    if (forcedKind &&
        !isSpecialSpawnKindEligible(forcedKind, systemID, factionKey, securityProfile, resolvedConfig)) {
        return {
            success: false,
    ...
    const specialSpawnKind = rollSpecialSpawnKind(systemID, factionKey, securityProfile, resolvedConfig, options);
    if (!specialSpawnKind) {
        return null;
    }
...
function buildBeltRatSpawnPlan(systemID, options = {}) {
    ...
    const resolvedConfig = resolveBeltRatConfig(options);
    const specialPlan = buildSpecialBeltRatSpawnPlan(systemID, factionKey, securityProfile, resolvedConfig, options);
    if (specialPlan) {
        return specialPlan;
    }
...
function spawnBeltRatGroup(scene, session, beltEntity, state, plan, options = {}) {
    const spawnNativeDefinitionsInContext = typeof options.spawnNativeDefinitionsInContext === "function"
...
function maybeSpawnForBeltArrival(scene, session, beltEntity, options = {}) {
    ...
    if (resolvedConfig.respawnCooldownMs > 0 &&
    ...
    if (resolvedConfig.rollCooldownMs > 0 &&
    ...
    if (String(planResult.data.specialSpawnKind || "").toLowerCase() === "capital" &&
        resolvedConfig.capitalMaxActiveGroupsPerSystem > 0) {
        const activeCapitalGroups = countActiveBeltRatCapitalGroupsInSystem(scene);
        if (activeCapitalGroups >= resolvedConfig.capitalMaxActiveGroupsPerSystem &&
            options.forceSpawn !== true) {
            return {
                success: true,
                spawned: false,
    ...
    const isSpecialSpawn = String(planResult.data.specialSpawnKind || "normal").toLowerCase() !== "normal";
    ...
    const spawnResult = spawnBeltRatGroup(scene, session, beltEntity, state, planResult.data, {
        ...options,
        broadcast: options.broadcast === true,
    });
    ...
    log.debug(`[BeltRats] spawned system=${systemID} belt=${beltEntity.itemID} ` +
...
//# sourceMappingURL=beltRatRuntime.js.map