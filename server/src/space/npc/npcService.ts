...
function resolveBatchSelection(options: Record<string, any> = {}) {
...
function spawnNpcBatchForSession(session, options: Record<string, any> = {}) {
...
function spawnNpcGroupInSystem(systemID, options: Record<string, any> = {}) {
  const spawnOptions = normalizeNpcSpawnOptions(options);
...
function spawnNpcSite(siteQuery, options: Record<string, any> = {}) {
  const spawnOptions = normalizeNpcSpawnOptions(options);
  ...
  const sharedSiteSpawnOptions: Record<string, any> = {
    ...spawnOptions,
    selectionKind: "site",
...
function spawnNpcSiteForSession(session, siteQuery, options: Record<string, any> = {}) {
  if (!session || !session._space) {
    return {
      success: false,
      errorMsg: "NOT_IN_SPACE",
...
function getStartupRuleSource(rules: any[] = []) {
...
function listStartupRulesForSystem(systemID) {
  ...
  const dynamicOperatorKinds = new Set<any>(
    dynamicRules
  ...
  const startupRuleOperatorKinds = new Set<any>(
...
function addVectors(left: Record<string, any> = {}, right: Record<string, any> = {}) {
  return {
...
function subtractVectors(left: Record<string, any> = {}, right: Record<string, any> = {}) {
  return {
    x: toFiniteNumber(left.x, 0) - toFiniteNumber(right.x, 0),
    y: toFiniteNumber(left.y, 0) - toFiniteNumber(right.y, 0),
    z: toFiniteNumber(left.z, 0) - toFiniteNumber(right.z, 0),
...
function collectExistingStartupControllerSlots(scene, startupRuleID, anchorID) {
  ...
  const visitController = (controller, options: Record<string, any> = {}) => {
    if (
      toPositiveInt(controller && controller.systemID, 0) !== normalizedSystemID ||
      String(controller && controller.startupRuleID || "").trim() !== normalizedStartupRuleID ||
      toPositiveInt(controller && controller.anchorID, 0) !== normalizedAnchorID
...
function maintainStartupRulesInScene(scene, now) {
  ...
  const activeRuleIDs = new Set<any>();
...
function destroyNpcController(controller, options: Record<string, any> = {}) {
  if (!controller) {
    return {
      success: false,
      errorMsg: "NPC_NOT_FOUND",
...
function destroyNpcControllerByEntityID(entityID, options: Record<string, any> = {}) {
  const controller = getControllerByEntityID(entityID);
  if (!controller) {
    return {
      success: false,
...
function clearNpcControllersInSystem(systemID, options: Record<string, any> = {}) {
  const normalizedSystemID = toPositiveInt(systemID, 0);
  if (!normalizedSystemID) {
    return {
      success: false,
...
function clearNpcControllersForSessionRadius(session, options: Record<string, any> = {}) {
  if (!session || !session._space) {
    return {
      success: false,
      errorMsg: "NOT_IN_SPACE",
