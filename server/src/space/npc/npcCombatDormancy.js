...
const { getControllerByEntityID, listControllersBySystem, } = require(path.join(__dirname, "./npcRegistry"));
...
function resolveBehaviorProfile(controller) {
    const behaviorProfile = controller && controller.behaviorProfile && typeof controller.behaviorProfile === "object"
        ? controller.behaviorProfile
        : {};
    const behaviorOverrides = controller && controller.behaviorOverrides && typeof controller.behaviorOverrides === "object"
...
function isCombatDormantControllerRecord(controllerRecord) {
    ...
    return (runtimeKind === "nativeCombat" &&
...
function isStableDormancyHomeState(scene, entity, controller) {
    ...
    if (homePosition) {
        const homeArrivalMeters = Math.max(250, toFiniteNumber(behaviorProfile.homeArrivalMeters, 1_500));
        if (distance(entity.position, homePosition) <= homeArrivalMeters) {
            return true;
        }
    }
    return (String(entity.mode || "").trim() === "STOP" &&
...
function materializeDormantCombatControllersForScene(scene, options = {}) {
    ...
    for (const controllerRecord of listCombatDormancyControllerRecordsForSystem(scene.systemID)) {
        ...
        const materializeResult = getNativeNpcService().materializeStoredNativeController(scene, entityID, {
...
function dematerializeDormantCombatControllersForScene(scene, options = {}) {
    ...
    for (const controller of listControllersBySystem(scene.systemID)) {
        ...
        const dematerializeResult = getNativeNpcService().dematerializeNativeController(controller, {
            broadcast: options.broadcast === true,
...
//# sourceMappingURL=npcCombatDormancy.js.map