...
function collectRelevantPublicGridClusterKeys(scene, options: Record<string, any> = {}) {
  const clusterKeys = new Set<any>();
  if (!scene) {
    return clusterKeys;
  }
...
function hasStartupAnchorRelevanceContext(scene, options: Record<string, any> = {}) {
...
function materializeRelevantAmbientControllers(scene, relevantClusterKeys, options: Record<string, any> = {}) {
...
function materializeRelevantCombatControllers(scene, relevantClusterKeys, options: Record<string, any> = {}) {
  const materialized: any[] = [];
...
function dematerializeIrrelevantAmbientControllers(scene, relevantClusterKeys, options: Record<string, any> = {}) {
...
function dematerializeIrrelevantCombatControllers(scene, relevantClusterKeys, options: Record<string, any> = {}) {
  const nowMs = toFiniteNumber(
    options.nowMs,
    scene.getCurrentSimTimeMs ? scene.getCurrentSimTimeMs() : Date.now(),
  );
...
function syncRelevantStartupControllersForScene(scene, options: Record<string, any> = {}) {
  if (!scene) {
    return {
      success: false,
...
function prewarmStartupControllersForWarpDestination(scene, options: Record<string, any> = {}) {
  if (!scene) {
    return {
      success: false,
      errorMsg: "SCENE_NOT_FOUND",
