...
function buildNativeWreckRuntimeEntity(wreckRecord, options: Record<string, any> = {}) {
...
function buildNativeWreckItemRecord(wreckRecord, itemType, options: Record<string, any> = {}) {
...
function refreshNativeWreckRuntimeEntity(systemID, wreckID, options: Record<string, any> = {}) {
  ...
  const runtimeScopeResolution =
    nativeNpcStore.resolveStoredEntityScopeMetadata(
      [scopeResolution.data.metadata, entity],
      { projectAirInstanceToDungeonSite: true },
    );
...
function spawnNativeWreck(systemID, wreckID, options: Record<string, any> = {}) {
...
function destroyNativeWreck(wreckID, options: Record<string, any> = {}) {
  const wreckRecord = nativeNpcStore.getNativeWreck(wreckID);
  if (!wreckRecord) {
    return {
      success: false,
...
function transferNativeWreckItemToCharacterLocation(options: Record<string, any> = {}) {
...
function destroyNativeNpcEntityWithWreck(systemID, shipEntity, options: Record<string, any> = {}) {
  const normalizedSystemID = toPositiveInt(systemID, 0);
  ...
  const scopeResolution = nativeNpcStore.resolveStoredEntityScopeMetadata(
    [nativeEntityRecord, shipEntity],
    { projectAirInstanceToDungeonSite: true },
  );
  if (!scopeResolution.success) {
    return {
  ...
  const entityScopeMetadata = scopeResolution.data.metadata;
  const lootTableID = resolveNativeWreckLootTableID(
    wreckType.typeID,
    nativeEntityRecord && nativeEntityRecord.lootTableID,
  );
