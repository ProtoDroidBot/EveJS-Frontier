...
const { CRUDE_MATTER_QUANTITY_BY_POTENTIAL, RIFT_FORMATION_YIELD, } = require(path.join(__dirname, "./frontierRiftAuthority"));
...
function normalizeResourceProfile(entry) {
    ...
    return {
        ...
        potential,
        resourceQuantity,
        tags: [...new Set(rawTags.filter((tag) => !Object.prototype.hasOwnProperty.call(RIFT_FORMATION_YIELD, tag)))],
    };
}
function normalizeSite(site) {
    ...
    const respawnAtMs = lifecycleState === "depleted" && depletedAtMs > 0
        ? Math.max(depletedAtMs + RIFT_RESPAWN_DELAY_MS, Math.trunc(toFiniteNumber(site.respawnAtMs, 0)))
        : 0;
    return {
        ...site,
        itemID,
...
function normalizeState(value) {
    ...
    const highestSiteID = Object.values(sites).reduce((highest, site) => Math.max(highest, site.itemID), RIFT_SITE_ID_BASE - 1);
    return {
        version: STATE_VERSION,
        nextSiteID: Math.max(RIFT_SITE_ID_BASE, highestSiteID + 1, toPositiveInt(source.nextSiteID, RIFT_SITE_ID_BASE)),
        sites,
    };
}
function createFrontierRiftSiteStore(options = {}) {
    ...
    function listSites(solarSystemID = null, listOptions = {}) {
    ...
    function markDepleted(siteID, markOptions = {}) {
        const numericSiteID = toPositiveInt(siteID, 0);
        const state = readState();
        const site = state.sites[String(numericSiteID)] || null;
        if (!site) {
    ...
    function reactivateDueSites(solarSystemID = null, nowMs = Date.now()) {
        ...
        for (const [siteKey, site] of Object.entries(sites)) {
            if (site.lifecycleState !== "depleted" ||
...
//# sourceMappingURL=frontierRiftSites.js.map