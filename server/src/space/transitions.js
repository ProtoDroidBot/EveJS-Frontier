...
const { describeSessionHydrationState, flushPendingCommandSessionEffects, } = require(path.join(__dirname, "../services/chat/commandSessionEffects"));
const { queuePostSpaceAttachFittingHydration, } = require(path.join(__dirname, "./modules/spaceAttachHydration"));
const { applyCharacterToSession, clearDockedFittingBootstrap, clearDeferredDockedShipSessionChange, flushCharacterSessionNotificationPlan, getCharacterRecord, getCharacterShips, findCharacterShip, getActiveShipRecord, updateCharacterRecord, syncInventoryItemForSession, emitItemsChangedForSession, } = require(path.join(__dirname, "../services/character/characterState"));
const { CAPSULE_TYPE_ID, ITEM_FLAGS, ensureCapsuleForCharacter, findItemById, findCharacterShipByType, listContainerItems, moveShipToSpace, dockShipToLocation, dockShipToStation, normalizeShipConditionState, removeInventoryItem, setActiveShipForCharacter, updateShipItem, } = require(path.join(__dirname, "../services/inventory/itemStore"));
const { SPECIAL_SHIP_HOLD_FLAGS, } = require(path.join(__dirname, "../services/inventory/specialShipHoldRegistry"));
const structureState = require(path.join(__dirname, "../services/structure/structureState"));
const { getDockedLocationID, getDockedLocationKind, getSessionStructureID, isDockedSession, isStructureDockedSession, } = require(path.join(__dirname, "../services/structure/structureLocation"));
const { currentFileTime, } = require(path.join(__dirname, "../services/_shared/serviceHelpers"));
const { getAppliedSkinRecord, } = require(path.join(__dirname, "../services/ship/shipCosmeticsState"));
const { defersUndockBallparkStateUntilBeyonceBind, } = require(path.join(__dirname, "../services/ship/destinyCompatibility"));
const { getEnabledCosmeticsEntries, } = require(path.join(__dirname, "../services/ship/shipLogoFittingState"));
const { buildEmergencyWarpReturnSpaceState, clearEmergencyWarpReturnState, getEmergencyWarpReturnState, shipSuppressesEmergencyWarp, } = require(path.join(__dirname, "../services/_shared/emergencyWarpRuntime"));
const mapTelemetryState = require(path.join(__dirname, "../services/map/mapTelemetryState"));
const { broadcastStationGuestJoined, broadcastStationGuestLeft, broadcastStructureGuestJoined, broadcastStructureGuestLeft, } = require(path.join(__dirname, "../services/_shared/guestLists"));
...
const { buildSlimItemObject, } = require(path.join(__dirname, "./destiny/index.js"));
const { buildPlayerEgoScopeMetadata, canEntitiesInteractLocally, } = require(path.join(__dirname, "./destiny/identity/interactionScope.js"));
const hostileModuleRuntime = require(path.join(__dirname, "./modules/hostileModuleRuntime"));
const { createSameSceneDestinyPresentationAuthorityHandoff, resolveSameSceneEgoAddBallsStamp, } = require(path.join(__dirname, "./destiny/delivery/transitionStamps.js"));
const { hasDestinyStamp, normalizeDestinyStamp, } = require(path.join(__dirname, "./destiny/delivery/stamps.js"));
const { getEntityMapKey, normalizeEntityIDSet, } = require(path.join(__dirname, "./destiny/identity/entityID.js"));
const { buildSameSceneShipBoardingHandoffPlan, buildSameSceneShipEjectHandoffPlan, resolveSameSceneShipBoardingFollowingRemoval, } = require(path.join(__dirname, "./destiny/presentation/shipBoarding.js"));
const { bindSameSceneShipHandoffDeliveryScope, createSameSceneShipHandoffDeliveryScope, } = require(path.join(__dirname, "./destiny/delivery/sameSceneShipHandoff.js"));
const { clearSameSceneDynamicClassification, } = require(path.join(__dirname, "./destiny/visibility/membershipState.js"));
const { buildPostAttachVisibilityReconciliationOptions, } = require(path.join(__dirname, "./destiny/visibility/controlState.js"));
const { resolveStargatePhysicalRadius, } = require(path.join(__dirname, "./stargateRadius"));
const { getStargateSystemForwardDirection, } = require(path.join(__dirname, "./stargateOrientation"));
const { getEntityCollisionBroadphaseRadius, } = require(path.join(__dirname, "./destiny/simulation/collisions"));
...
const UNDOCK_INVULNERABILITY_DURATION_MS = Number(UNDOCK_INVULNERABILITY_DURATION_TICKS / FILETIME_TICKS_PER_MS);
...
function getContrabandInspectionRuntimeModule() {
    if (!contrabandInspectionRuntimeModule) {
        contrabandInspectionRuntimeModule = require(path.join(__dirname, "../services/security/contrabandInspectionRuntime"));
    }
...
function deactivateActiveModulesForSpaceTransition(session, reason) {
    ...
    if (!result || result.success !== true) {
        log.warn(`[SpaceTransition] Failed to fully deactivate active modules before ${reason} ` +
...
function buildEnabledCosmeticsDictForShip(shipID) {
    try {
        ...
        log.warn(`[SpaceTransition] Failed to build cosmetics dict ship=${shipID}: ${error.message}`);
...
function getAppliedShipSkinID(shipID) {
    try {
        ...
        log.warn(`[SpaceTransition] Failed to resolve current skin ship=${shipID}: ${error.message}`);
        return 0;
    }
}
...
function emitBoardedCapsuleItemsChanged(session, capsuleItem, previousState = {}) {
    ...
    return emitItemsChangedForSession(session, removedState, {
        locationID: previousState.locationID !== undefined
            ? previousState.locationID
            : capsuleItem.locationID,
        flagID: previousState.flagID !== undefined ? previousState.flagID : capsuleItem.flagID,
        quantity: previousState.quantity !== undefined
            ? previousState.quantity
            : capsuleItem.quantity,
        singleton: previousState.singleton !== undefined
            ? previousState.singleton
            : capsuleItem.singleton,
        stacksize: previousState.stacksize !== undefined
...
function submitSameSceneShipHandoffPlan(session, scene, expectedGeneration, plan, failureCode, options = {}) {
    const shouldStageFollowingRemoval = options.stageFollowingRemoval === true;
    ...
    try {
        const handoffDeliveryScope = shouldStageFollowingRemoval
            ? createSameSceneShipHandoffDeliveryScope(session, plan.sendOptions)
            : null;
        ...
        const handoffSendOptions = shouldStageFollowingRemoval
            ? bindSameSceneShipHandoffDeliveryScope(plan.sendOptions, handoffDeliveryScope, handoffSendOptionOverrides)
            : {
                ...(plan.sendOptions || {}),
                ...handoffSendOptionOverrides,
            };
        emittedStamp = scene.sendDestinyUpdates(session, plan.updates, false, handoffSendOptions);
        if (!hasDestinyStamp(emittedStamp)) {
            throw new Error(failureCode);
        }
        if (shouldStageFollowingRemoval) {
            followingRemoval = resolveSameSceneShipBoardingFollowingRemoval(plan, emittedStamp);
            if (!followingRemoval ||
            ...
            const followingRemovalSendOptions = bindSameSceneShipHandoffDeliveryScope(followingRemoval.sendOptions, handoffDeliveryScope, { deferDirectDestinyFlush: true });
            const removalDelivery = scene.sendRemoveBallsToSession(session, [followingRemoval.entityID], {
        ...
        if (typeof options.beforeDeliveryFlush === "function" &&
...
function sendEjectHandoffDestiny(session, scene, expectedGeneration, abandonedShipEntity, capsuleEntity) {
    let plan;
    try {
        ...
        destroySessionAfterUnsafeSameSceneBoundary(scene, session, "DESTINY_EGO_VISIBILITY_DELIVERY_FAILED");
        return null;
    }
    const result = submitSameSceneShipHandoffPlan(session, scene, expectedGeneration, plan, "DESTINY_EGO_VISIBILITY_DELIVERY_FAILED");
    return result && result.stamp;
}
function applyTqDockingTransitionSessionChangeShape(plan, options = {}) {
...
function scheduleUndockInvulnerabilityCancellation(session, shipID, options = {}) {
    ...
    const timer = setTimeout(() => {
        ...
        if (scene && typeof scene.syncSessionStructureTetherState === "function") {
            ...
            const tetherStructureID = tetherSync &&
                tetherSync.success &&
                tetherSync.data &&
                tetherSync.data.active
                ? tetherSync.data.structureID
                : 0;
            if (tetherStructureID > 0 &&
                typeof scene.sendTqUndockTetherPresentationToSession === "function") {
                scene.sendTqUndockTetherPresentationToSession(session, shipID, tetherStructureID);
            }
        }
        if (scene && typeof scene.flushDirectDestinyNotificationBatchIfIdle === "function") {
            scene.flushDirectDestinyNotificationBatchIfIdle();
...
function recordStargateJumpQueueEntry(destinationSolarSystemID, characterID, options = {}) {
    ...
    queue.set(normalizedCharacterID, {
        ...
        fileTime: options.fileTime === undefined || options.fileTime === null
            ? currentFileTime()
            : options.fileTime,
...
function getStargateJumpQueuePayload(destinationSolarSystemID, activeCharacterID) {
    const activeEntryQueue = recordStargateJumpQueueEntry(destinationSolarSystemID, activeCharacterID);
    const queue = activeEntryQueue || cleanupStargateJumpQueue(destinationSolarSystemID) || null;
...
function sendJumpQueueUpdate(session, destinationSolarSystemID) {
    ...
    const queuePayload = getStargateJumpQueuePayload(normalizedDestinationID, characterID);
    if (!Number.isFinite(normalizedDestinationID) ||
...
function inspectContrabandForTransition(session, shipID, solarSystemID, reason) {
    ...
    const result = inspectCharacterContraband(session.characterID, shipID, solarSystemID, { reason });
    if (!result || !result.success) {
        log.warn(`[SpaceTransition] Contraband inspection failed reason=${reason} char=${session.characterID} ship=${shipID} system=${solarSystemID}: ${(result && result.errorMsg) || "UNKNOWN_ERROR"}`);
        return null;
    }
...
function finalizeBoardedCapsuleConsumption(scene, session, expectedGeneration, capsuleEntity, options = {}) {
    if (!scene ||
        !session ||
        !expectedGeneration ||
        session._space !== expectedGeneration ||
    ...
    return {
        ...
        data: {
            ...
            changes: removeItemResult.data && Array.isArray(removeItemResult.data.changes)
...
function buildBoundResult(session) {
    ...
    const preferredBoundId = session.currentBoundObjectID ||
    ...
    const readyAtFileTime = BigInt(Math.trunc(readyAtMs)) * FILETIME_TICKS_PER_MS + FILETIME_EPOCH_OFFSET;
...
function buildLocationIdentityPatch(record, solarSystemID, extra = {}) {
    ...
    return {
        ...
        constellationID: Number((system && system.constellationID) || record.constellationID || 0) ||
            20000020,
        regionID: Number((system && system.regionID) || record.regionID || 0) ||
            10000002,
        worldSpaceID: 0,
    };
}
...
function cloneVector(source = null, fallback = { x: 0, y: 0, z: 0 }) {
    const vectorSource = source && typeof source === "object"
        ? source
        : null;
    return {
...
function getResolvedStargateForwardDirection(stargate) {
    return (getDirectionFromDunRotation(stargate && stargate.dunRotation) ||
...
function resolveShipRadiusMeters(ship) {
    const collisionRadius = Math.max(0, toFiniteNumber(getEntityCollisionBroadphaseRadius(ship), 0));
    if (collisionRadius > 0) {
        return collisionRadius;
    }
    const directRadius = Math.max(0, toFiniteNumber(ship && (ship.collisionRadius ?? ship.radius ?? ship.spaceRadius), 0));
    if (directRadius > 0) {
        return directRadius;
    }
...
function buildGateSpawnState(stargate, ship = null) {
    ...
    const offset = resolveStargatePhysicalRadius(stargate) +
        resolveShipRadiusMeters(ship) +
        STARGATE_JUMP_IN_SURFACE_CLEARANCE_METERS;
    return {
        direction,
        position: addVectors(cloneVector(stargate.position), scaleVector(direction, offset)),
    };
}
function buildOffsetSpawnState(anchor, options = {}) {
    const fallbackDirection = cloneVector(options.fallbackDirection, { x: 1, y: 0, z: 0 });
    const anchorPosition = cloneVector(anchor && anchor.position);
    const direction = normalizeVector(magnitude(anchorPosition) > 0 ? anchorPosition : fallbackDirection, fallbackDirection);
    ...
    const anchorRadius = Math.max(toFiniteNumber(options.anchorRadius, anchor && (anchor.collisionRadius ?? anchor.radius)), 0);
    const offset = Math.max(anchorRadius + shipRadius + clearance, minOffset);
...
function buildSolarSystemSpawnState(solarSystemID, ship = null) {
    ...
    const celestial = celestials.find((entry) => entry.kind !== "sun" && entry.groupID !== 6) ||
...
function buildSolarSessionChangeOptions(session, solarSystemID) {
    const sourceNodeID = Number(session && session._machoNodeID) ||
...
function queuePendingSessionEffects(session, options = {}) {
    ...
    if (options.forceInitialBallpark ||
...
function buildStoppedSpaceStateFromEntity(entity) {
    ...
    return {
        ...
        position,
        direction: normalizeVector(cloneVector(entity && entity.direction, { x: 1, y: 0, z: 0 }), { x: 1, y: 0, z: 0 }),
...
function buildCloneVatArrivalSpaceState(targetShipItem, targetEntity = null) {
    const sourceState = targetEntity && targetEntity.position
        ? {
    ...
    const targetDirection = normalizeVector(cloneVector(sourceState.direction, { x: 1, y: 0, z: 0 }), { x: 1, y: 0, z: 0 });
    const offsetMeters = Math.max(2500, toFiniteNumber(sourceState.radius, toFiniteNumber(targetShipItem && targetShipItem.spaceRadius, 0)) + 1500);
    const position = addVectors(targetPosition, scaleVector(targetDirection, -offsetMeters));
    return {
        position,
...
function captureSpaceSessionState(session) {
    return {
        ...
        initialStateSent: Boolean(session && session._space && session._space.initialStateSent),
        initialBallparkVisualsSent: Boolean(session && session._space && session._space.initialBallparkVisualsSent),
        initialBallparkClockSynced: Boolean(session && session._space && session._space.initialBallparkClockSynced),
    };
}
...
const SHIP_DESTRUCTION_EJECT_WARP_LANDING_BYPASS = Symbol("shipDestructionEjectWarpLandingBypass");
function isPilotWarpLandingPending(session) {
    return Boolean(spaceRuntime &&
...
function isSameSceneDestinyDeliveryBoundaryUsable(scene, session, expectedGeneration) {
...
function flushSameSceneDestinyDelivery(scene, session = null, expectedGeneration = null) {
    ...
    return !session || isSameSceneDestinyDeliveryBoundaryUsable(scene, session, expectedGeneration);
}
function destroySessionAfterUnsafeSameSceneBoundary(scene, session, code) {
...
function ensureSameSceneBoardTargetVisible(session, scene, targetEntity) {
    ...
    const visibleDynamicEntityIDs = normalizeEntityIDSet(session._space.visibleDynamicEntityIDs);
    ...
    return Boolean(flushSameSceneDestinyDelivery(scene, session, sourceGeneration) &&
        deliveryCommitted);
}
function refreshSameSceneSessionView(scene, session, egoEntity, additionalEntities = [], options = {}) {
    if (!scene || !session || !session._space || !egoEntity) {
        return false;
    }
    const sourceGeneration = session._space;
    const preserveExistingBallpark = options.preserveExistingBallpark === true;
    const needsFullBootstrap = session._space.initialStateSent !== true ||
        session._space.initialBallparkVisualsSent !== true ||
        session._space.initialBallparkClockSynced !== true;
    ...
    if (refreshEntities.length > 0) {
        const delivery = scene.sendAddBallsToSession(session, refreshEntities, {
            ...
            stampOverride: options.stampOverride === undefined || options.stampOverride === null
                ? undefined
    ...
    if (!isSameSceneDestinyDeliveryBoundaryUsable(scene, session, sourceGeneration)) {
        return false;
    }
...
function retargetCharacterSessionNotificationPlanShip(plan, oldShipID, shipID) {
    ...
    const previousShipID = normalizeShipID(oldShipID) ?? normalizeShipID(plan.oldShipID);
...
function resolveReusableCapsuleForCharacter(characterID, excludedShipID, currentSolarSystemID, preferredStationID) {
    const excludedItemID = Number(excludedShipID || 0) || 0;
    const ships = getCharacterShips(characterID).filter((shipItem) => Number(shipItem && shipItem.typeID) === CAPSULE_TYPE_ID &&
        Number(shipItem && shipItem.itemID) !== excludedItemID);
    const currentSystemCapsule = ships.find((shipItem) => Number(shipItem.locationID) === Number(currentSolarSystemID || 0) &&
    ...
    const storedCapsule = ships.find((shipItem) => Number(shipItem.flagID) === ITEM_FLAGS.HANGAR);
    if (storedCapsule) {
        return {
            success: true,
            created: false,
...
function completeStargateJump(session, sourceGate, destinationGate, activeShip) {
    if (!session ||
        !session.characterID ||
        !sourceGate ||
        !destinationGate ||
    ...
    const sourceSimTimeMs = session && session._space
        ? spaceRuntime.getSimulationTimeMsForSession(session, null)
        : null;
    const sourceTimeDilation = session && session._space
    ...
    const updateResult = updateCharacterRecord(session.characterID, (record) => buildLocationIdentityPatch(record, destinationGate.solarSystemID, {
        stationID: null,
        structureID: null,
    }));
    ...
    const previousLocalChannelID = Number(session.solarsystemid2 ||
    ...
    const observerArrivalFxResult = spaceRuntime.emitStargateArrivalObserverFx(session, destinationGate.itemID, moveResult.data && moveResult.data.itemID);
    if (typeof spaceRuntime.recordSessionJumpTimingTrace === "function") {
    ...
    try {
        ...
        log.warn(`[SpaceTransition] Failed to record map jump characterID=${session.characterID || 0} from=${sourceGate.solarSystemID} to=${destinationGate.solarSystemID}: ${error.message}`);
    }
    log.info(`[SpaceTransition] Stargate jump ${session.characterName || session.characterID} ship=${activeShip.itemID} from=${sourceGate.itemID} to=${destinationGate.itemID}`);
    ...
    try {
        require(path.join(__dirname, "../services/dailyGoals/dailyGoalsState")).recordActivity(session.characterID, "space_jump", 1);
    }
    catch (dailyGoalError) {
        log.debug(`[SpaceTransition] daily-goal space_jump hook failed char=${session.characterID}: ${dailyGoalError.message}`);
    }
    endTransition(session, "stargate-jump");
    return {
        success: true,
...
function syncDockedShipTransitionForSession(session, dockResult, options = {}) {
    ...
    emitItemsChangedForSession(session, dockedShip, {
    ...
    if (options.refreshActiveShip !== false) {
        syncInventoryItemForSession(session, dockedShip, {
...
function getStrategicCruiserTransitionUseError(charID, shipItem) {
    ...
    const { getStrategicCruiserUseStatus, isStrategicCruiserType, } = require(path.join(__dirname, "../services/fitting/liveFittingState"));
    ...
    return status.complete
        ...
        : {
            ...
            data: {
                missingSubsystemFlags: status.subsystemStatus && status.subsystemStatus.missingFlags || [],
...
function undockSession(session, options = {}) {
    ...
    const strategicCruiserUseError = getStrategicCruiserTransitionUseError(session.characterID, activeShip);
    ...
    try {
        const previousLocalChannelID = Number(getDockedLocationID(session) ||
            session.solarsystemid2 ||
            session.solarsystemid ||
            0) || 0;
        if (dockable.kind === "structure" &&
            structureState.hasStructureOneWayUndockRestriction(dockable.record, session.shipTypeID) &&
        ...
        const contrabandInspection = inspectContrabandForTransition(session, activeShip.itemID, dockable.solarSystemID, "undock");
        ...
        const updateResult = updateCharacterRecord(session.characterID, (record) => buildLocationIdentityPatch(record, dockable.solarSystemID, {
            homeStationID: Number(record.homeStationID || record.cloneStationID || session.homeStationID || 60003760) ||
                60003760,
            cloneStationID: Number(record.cloneStationID || record.homeStationID || session.cloneStationID || 60003760) ||
                60003760,
            stationID: null,
            structureID: null,
        }));
        ...
        spaceRuntime.attachSession(session, moveResult.data, {
            ...
            broadcast: true,
            emitSimClockRebase: false,
            deferInitialBallparkStateUntilBind: defersUndockBallparkStateUntilBeyonceBind(session && session.compatibilityProfile),
        ...
        flushCharacterSessionNotificationPlan(session, applyUndockSessionChangeShape(applyResult.notificationPlan));
        queuePendingSessionEffects(session, {
            previousLocalChannelID,
        });
        flushPendingCommandSessionEffects(session);
        ...
        log.info(`[SpaceTransition] Undocked ${session.characterName || session.characterID} ship=${moveResult.data.itemID} location=${dockedLocationID} kind=${dockable.kind} system=${dockable.solarSystemID}`);
        return {
            success: true,
            data: {
...
function dockSession(session, stationID) {
    ...
    try {
        ...
        flushCharacterSessionNotificationPlan(session, applyTqDockingTransitionSessionChangeShape(applyResult.notificationPlan));
        ...
        log.info(`[SpaceTransition] Docked ${session.characterName || session.characterID} ship=${activeShip.itemID} location=${dockable.locationID} kind=${dockable.kind}`);
        return {
            success: true,
            data: {
                station: dockable.record,
...
function consumeEmergencyWarpReturnStateForLogin(activeShip, characterID) {
    ...
    const moveResult = moveShipToSpace(activeShip.itemID, nextSpaceState.systemID || activeShip.spaceState.systemID, nextSpaceState);
    if (!moveResult.success) {
        log.warn(`[SpaceTransition] Failed to consume emergency warp return state for ` +
...
function buildLoginSpaceRepairState(activeShip, systemID) {
    const rawState = activeShip && activeShip.spaceState && typeof activeShip.spaceState === "object"
    ...
    return {
        ...
        systemID,
        position,
        direction: normalizeVector(cloneVector(rawState.direction, { x: 1, y: 0, z: 0 }), { x: 1, y: 0, z: 0 }),
        velocity: { x: 0, y: 0, z: 0 },
        speedFraction: 0,
        mode: "STOP",
...
function repairActiveShipSpaceStateForLogin(session, activeShip) {
    ...
    const stateSystemID = toInt(activeShip.spaceState && activeShip.spaceState.systemID, 0);
    const locationID = toInt(activeShip.locationID, 0);
    const flagID = toInt(activeShip.flagID, ITEM_FLAGS.HANGAR);
    if (activeShip.spaceState &&
        stateSystemID === systemID &&
        locationID === systemID &&
        flagID === 0) {
        return activeShip;
    }
    const moveResult = moveShipToSpace(activeShip.itemID, systemID, buildLoginSpaceRepairState(activeShip, systemID));
    if (!moveResult.success) {
        log.warn(`[SpaceTransition] Failed to repair active ship for space login ` +
            `char=${session.characterID} ship=${activeShip.itemID} system=${systemID}: ${moveResult.errorMsg}`);
        return activeShip;
    }
    log.info(`[SpaceTransition] Repaired active ship for space login ` +
...
function restoreSpaceSession(session) {
    ...
    activeShip = consumeEmergencyWarpReturnStateForLogin(activeShip, session.characterID);
    const attachStartedAtMs = Date.now();
    const shipEntity = spaceRuntime.attachSession(session, activeShip, {
        systemID: activeShip.spaceState.systemID ||
            session.solarsystemid ||
            session.solarsystemid2,
        pendingUndockMovement: false,
        broadcast: true,
    ...
    if (typeof spaceRuntime.recordSessionJumpTimingTrace === "function") {
        spaceRuntime.recordSessionJumpTimingTrace(session, "space-login-restore-attached", {
            ...
            systemID: Number(activeShip.spaceState && activeShip.spaceState.systemID) ||
    ...
    if (attachElapsedMs >= 250) {
        log.info(`[SpaceTransition] restoreSpaceSession attach ship=${Number(activeShip.itemID) || 0} ` +
    ...
    log.debug(`[space-login-restore] attached shipID=${Number(activeShip.itemID) || 0} ` +
        `systemID=${Number(activeShip.spaceState &&
            activeShip.spaceState.systemID) || Number(session.solarsystemid || session.solarsystemid2) || 0} ` +
...
async function restoreSpaceSessionAsync(session) {
    ...
    const activeShip = getActiveShipRecord(session.characterID);
    const systemID = toInt(activeShip && activeShip.spaceState && activeShip.spaceState.systemID, toInt(session.solarsystemid2 || session.solarsystemid, 0));
...
function ejectSession(session, options = {}) {
    ...
    if (options[SHIP_DESTRUCTION_EJECT_WARP_LANDING_BYPASS] !== true &&
    ...
    try {
        const sendAbandonedShipSlimToVictim = options.sendAbandonedShipSlimToVictim !== false;
        const refreshAbandonedShipViewForVictim = options.refreshAbandonedShipViewForVictim !== false;
        const syncAllSessionsVisibilityAfterSwap = options.syncAllSessionsVisibilityAfterSwap !== false;
        const beforeSameSceneShipSwapNotificationPlanFlush = typeof options.beforeSameSceneShipSwapNotificationPlanFlush === "function"
            ? options.beforeSameSceneShipSwapNotificationPlanFlush
            : null;
        const sendEjectSpecialFx = options.sendEjectSpecialFx !== false;
        const disconnectBoundObjectsBeforeSwap = options.disconnectBoundObjectsBeforeSwap === true;
        const hasSameSceneShipSwapSessionId = Object.prototype.hasOwnProperty.call(options, "sameSceneShipSwapSessionId");
        ...
        const preferredStationID = Number(characterRecord.homeStationID ||
            characterRecord.cloneStationID ||
            session.stationid ||
            session.stationID ||
            60003760) || 60003760;
        const preservedSpaceState = captureSpaceSessionState(session);
        const capsuleResult = resolveReusableCapsuleForCharacter(session.characterID, activeShip.itemID, currentSystemID, preferredStationID);
        ...
        if (typeof scene.prepareSameSceneSessionTetherRelease === "function") {
            const tetherReleaseResult = scene.prepareSameSceneSessionTetherRelease(session, currentEntity);
            if (!tetherReleaseResult || tetherReleaseResult.success !== true) {
                return {
                    success: false,
                    errorMsg: tetherReleaseResult && tetherReleaseResult.errorMsg ||
        ...
        const abandonedShipEntity = spaceRuntime.disembarkSession(session, {
            ...
            beforeSessionSpaceRelease: () => {
                if (!flushSameSceneDestinyDelivery(scene, session, sourceDestinyGeneration)) {
                    return destroySessionAfterUnsafeSameSceneBoundary(scene, session, "DESTINY_SAME_SCENE_FINAL_DELIVERY_FAILED");
                }
                ...
                return Boolean(destinyAuthorityHandoff) ||
                    destroySessionAfterUnsafeSameSceneBoundary(scene, session, "DESTINY_AUTHORITY_HANDOFF_FAILED");
            },
        });
        ...
        const capsuleMoveResult = moveShipToSpace(capsuleResult.data.itemID, currentSystemID, buildStoppedSpaceStateFromEntity(currentEntity));
        if (!capsuleMoveResult.success) {
            return capsuleMoveResult;
        }
        sendPodOrCorvetteAssembly(session, capsuleMoveResult.data);
        const activeShipResult = setActiveShipForCharacter(session.characterID, capsuleMoveResult.data.itemID);
        if (!activeShipResult.success) {
            return activeShipResult;
        }
        const updateResult = updateCharacterRecord(session.characterID, (record) => buildLocationIdentityPatch(record, currentSystemID, {
            stationID: null,
            structureID: null,
        }));
        if (!updateResult.success) {
        ...
        if (!isSameSceneDestinyDeliveryBoundaryUsable(scene, session, capsuleDestinyGeneration)) {
            destroySessionAfterUnsafeSameSceneBoundary(scene, session, "DESTINY_EGO_VISIBILITY_DELIVERY_FAILED");
            return {
                success: false,
        ...
        retargetCharacterSessionNotificationPlanShip(applyResult.notificationPlan, activeShip.itemID, capsuleMoveResult.data.itemID);
        flushSameSceneShipSwapNotificationPlan(session, applyResult.notificationPlan, hasSameSceneShipSwapSessionId
        ...
        if (egoDelivery && egoDelivery.delivered === true) {
            egoDeliveryBoundaryUsable = flushSameSceneDestinyDelivery(scene, session, capsuleDestinyGeneration);
        }
        if (!egoVisibilityCommitted || !egoDeliveryBoundaryUsable) {
            const deliveryError = new Error("Same-scene capsule visibility delivery was not accepted");
        ...
        if (sendEjectSpecialFx) {
            const ejectHandoffStamp = sendEjectHandoffDestiny(session, scene, capsuleDestinyGeneration, abandonedShipEntity, capsuleEntity);
        ...
        log.info(`[SpaceTransition] Ejected ${session.characterName || session.characterID} from ship=${activeShip.itemID} into capsule=${capsuleMoveResult.data.itemID} system=${currentSystemID}`);
        return {
            success: true,
            data: {
...
function boardSpaceShip(session, shipID) {
    ...
    if (visibleTargetEntity &&
        ...
        ((visibleTargetEntity.session && visibleTargetEntity.session !== session) ||
            Number(visibleTargetEntity.npcCharacterID ||
    ...
    const boardingDistance = getSurfaceDistanceBetweenEntities(currentEntity, targetEntity);
    ...
    const strategicCruiserUseError = getStrategicCruiserTransitionUseError(session.characterID, targetShip);
    if (strategicCruiserUseError) {
        return strategicCruiserUseError;
    }
    ...
    try {
        ...
        const preservedSpaceState = captureSpaceSessionState(session);
        const inheritedPlayerEgoScopeState = buildPlayerEgoScopeMetadata(currentEntity);
        const targetVisibilityReady = ensureSameSceneBoardTargetVisible(session, scene, targetEntity);
        ...
        const shouldConsumePreviousCapsule = Number(currentShip.typeID) === CAPSULE_TYPE_ID &&
        ...
        const activeShipResult = setActiveShipForCharacter(session.characterID, targetShipID);
        if (!activeShipResult.success) {
            return activeShipResult;
        }
        const updateResult = updateCharacterRecord(session.characterID, (record) => buildLocationIdentityPatch(record, currentSystemID, {
        ...
        const boardedEntity = spaceRuntime.attachSessionToExistingEntity(session, targetShip, targetEntity, {
            systemID: currentSystemID,
            pendingUndockMovement: false,
            broadcast: false,
            emitSimClockRebase: false,
        ...
        try {
            boardedDerivedStateResult = scene.refreshShipEntityDerivedState(boardedEntity, {
                session,
                broadcast: false,
        ...
        if (!boardedDerivedStateResult || boardedDerivedStateResult.success !== true) {
            destroySessionAfterUnsafeSameSceneBoundary(scene, session, "BOARD_DERIVED_STATE_REFRESH_FAILED");
            return {
                success: false,
                errorMsg: boardedDerivedStateResult && boardedDerivedStateResult.errorMsg ||
                    "BOARD_DERIVED_STATE_REFRESH_FAILED",
            };
        }
        const boardedDestinyGeneration = session._space;
        if (!isSameSceneDestinyDeliveryBoundaryUsable(scene, session, boardedDestinyGeneration)) {
            destroySessionAfterUnsafeSameSceneBoundary(scene, session, "BOARD_EGO_VISIBILITY_HANDOFF_FAILED");
            return {
                success: false,
                errorMsg: "BOARD_EGO_VISIBILITY_HANDOFF_FAILED",
            };
        }
        retargetCharacterSessionNotificationPlanShip(applyResult.notificationPlan, currentShip.itemID, targetShipID);
        ...
        const boardingHandoffResult = submitSameSceneShipHandoffPlan(session, scene, boardedDestinyGeneration, boardingHandoffPlan, "BOARD_EGO_VISIBILITY_HANDOFF_FAILED", {
            ...
            beforeDeliveryFlush: shouldConsumePreviousCapsule
                ? () => {
                    ...
                    emitBoardedCapsuleItemsChanged(session, boardedCapsuleItem, boardedCapsulePreviousState);
        ...
        if (!clearSameSceneDynamicClassification(boardedDestinyGeneration, targetShipID)) {
            destroySessionAfterUnsafeSameSceneBoundary(scene, session, "BOARD_EGO_VISIBILITY_HANDOFF_FAILED");
            return {
                success: false,
                errorMsg: "BOARD_EGO_VISIBILITY_HANDOFF_FAILED",
        ...
        if (shouldConsumePreviousCapsule) {
            ...
            try {
                capsuleConsumeResult = finalizeBoardedCapsuleConsumption(scene, session, boardedDestinyGeneration, abandonedCurrentEntity, {
                    acceptedRemovalStamp: boardingHandoffResult.followingRemovalStamp,
                });
            }
            catch (_error) {
            ...
            if (!capsuleConsumeResult || !capsuleConsumeResult.success) {
                destroySessionAfterUnsafeSameSceneBoundary(scene, session, "CAPSULE_REMOVAL_DELIVERY_FAILED");
                return {
                    success: false,
                    errorMsg: "CAPSULE_REMOVAL_DELIVERY_FAILED",
                };
        ...
        if (!previousCapsuleConsumed) {
            const recoveryAddBallsStamp = resolveSameSceneEgoAddBallsStamp(scene, session);
            ...
            refreshSameSceneSessionView(scene, session, boardedEntity, [abandonedCurrentEntity], {
                ...
                preserveExistingBallpark: true,
                sessionStampedAddBalls: true,
                stampOverride: recoveryAddBallsStamp === null
                    ? undefined
        ...
        log.info(`[SpaceTransition] Boarded ${session.characterName || session.characterID} ship=${targetShipID} from=${currentShip.itemID} system=${currentSystemID}`);
        return {
            success: true,
            data: {
                ship: targetShip,
...
function jumpSessionViaStargate(session, fromStargateID, toStargateID) {
    ...
    const destinationGate = worldData.getStargateByID(toStargateID || (sourceGate && sourceGate.destinationID));
    ...
    if (Number(sourceGate.destinationID || 0) !== Number(destinationGate.itemID || 0)) {
        endTransition(session, "stargate-jump");
        return {
            success: false,
    ...
    if (Number(sourceGate.solarSystemID || 0) !== Number(session._space.systemID || 0)) {
        endTransition(session, "stargate-jump");
        return {
            success: false,
    ...
    const contrabandInspection = inspectContrabandForTransition(session, activeShip.itemID, destinationGate.solarSystemID, "stargate-jump");
    ...
    setTimeout(() => {
        const completionResult = completeStargateJump(session, sourceGate, destinationGate, activeShip);
        if (!completionResult.success) {
            log.warn(`[SpaceTransition] Delayed stargate jump failed for ${session.characterName || session.characterID}: ${completionResult.errorMsg}`);
        }
    }, scaledDelay);
    return {
        success: true,
...
async function jumpSessionViaStargateAsync(session, fromStargateID, toStargateID) {
    if (!session || !session.characterID || !session._space) {
    ...
    const targetSolarSystemID = toInt(destinationGate && destinationGate.solarSystemID, 0);
    if (sourceGate &&
        destinationGate &&
        ...
        try {
            ...
            log.warn(`[SpaceTransition] Destination scene preparation failed for stargate jump ` +
...
function rebuildDockedSessionAtStation(session, stationID, options = {}) {
    ...
    const previousLocalChannelID = Number(session.solarsystemid2 ||
        ...
        session.stationID ||
        0) || 0;
    const preRespawnShipID = Number(session.shipID ||
        session.shipid ||
    ...
    const authoritativeHomeStationID = Number((currentRecord && (currentRecord.homeStationID ||
        ...
        session.clonestationid ||
        0) || 0;
    const updateResult = updateCharacterRecord(session.characterID, (record) => buildLocationIdentityPatch(record, station.solarSystemID, {
        homeStationID: authoritativeHomeStationID || station.stationID,
        cloneStationID: Number(record.cloneStationID || authoritativeHomeStationID || station.stationID) ||
            station.stationID,
    ...
    if (options.boardNewbieShip === true && !newbieShipResult) {
        ...
        if (typeof DogmaService.boardNewbieShipForSession === "function") {
            ...
            if (!newbieShipResult.success) {
                log.warn(`[SpaceTransition] Failed to auto-board corvette for ${session.characterName || session.characterID} station=${station.stationID} error=${newbieShipResult.errorMsg}`);
            }
            else if (newbieShipResult.data &&
                newbieShipResult.data.ship &&
                Number(newbieShipResult.data.ship.itemID) > 0) {
                retargetCharacterSessionNotificationPlanShip(applyResult.notificationPlan, preRespawnShipID, newbieShipResult.data.ship.itemID);
            }
        }
    }
    ...
    const refreshedCapsule = Number(capsuleShip && capsuleShip.itemID) > 0
    ...
    if (refreshedCapsule) {
        ...
        syncInventoryItemForSession(session, refreshedCapsule, {
    ...
    const refreshedActiveShip = getActiveShipRecord(session.characterID) ||
        refreshedCapsule ||
        capsuleShip;
    if (refreshedCapsule &&
        Number(refreshedActiveShip.itemID) !== Number(refreshedCapsule.itemID)) {
        syncInventoryItemForSession(session, refreshedActiveShip, {
    ...
    const activeShip = getActiveShipRecord(session.characterID) ||
        ...
        refreshedCapsule;
    log.info(`[SpaceTransition] Rebuilt docked session for ${session.characterName || session.characterID} station=${station.stationID} ship=${activeShip && activeShip.itemID}`);
    return {
        success: true,
        data: {
            station,
...
function jumpSessionToStation(session, stationID) {
    ...
    try {
        ...
        flushPendingCommandSessionEffects(session);
        broadcastOnCharNowInStation(session, station.stationID);
        log.info(`[SpaceTransition] Station jump ${session.characterName || session.characterID} ship=${activeShip.itemID} station=${station.stationID} system=${station.solarSystemID}`);
        return {
            success: true,
            data: {
                station,
...
function jumpSessionToSolarSystem(session, solarSystemID, options = {}) {
    ...
    try {
        const destinationSceneAlreadyLoaded = Boolean(spaceRuntime &&
        ...
        if (typeof spaceRuntime.beginSessionJumpTimingTrace === "function") {
            spaceRuntime.beginSessionJumpTimingTrace(session, "solar-jump", {
                sourceSystemID: wasInSpace && session && session._space
        ...
        const previousLocalChannelID = Number(session.solarsystemid2 ||
            ...
            getDockedLocationID(session) ||
            0) || 0;
        const spawnState = options.spawnStateOverride || buildSolarSystemSpawnState(targetSolarSystemID, activeShip);
        if (!spawnState) {
            return {
                success: false,
                errorMsg: "SOLAR_SYSTEM_NOT_FOUND",
        ...
        const updateResult = updateCharacterRecord(session.characterID, (record) => buildLocationIdentityPatch(record, targetSolarSystemID, {
            ...(sourceStationID
                ? {
                    homeStationID: Number(record.homeStationID || record.cloneStationID || sourceStationID) ||
                        sourceStationID,
                    cloneStationID: Number(record.cloneStationID || record.homeStationID || sourceStationID) ||
                        sourceStationID,
                }
                : {}),
            stationID: null,
        ...
        flushPendingCommandSessionEffects(session);
        log.info(`[SpaceTransition] Solar jump ${session.characterName || session.characterID} ship=${activeShip.itemID} system=${targetSolarSystemID} anchor=${spawnState.anchorType}:${spawnState.anchorID} warmScene=${destinationSceneAlreadyLoaded}`);
...
async function jumpSessionToSolarSystemAsync(session, solarSystemID, options = {}) {
...
function jumpSessionToShipCloneBay(session, targetShipID) {
    ...
    const targetSystemID = Number(targetShip.spaceState && targetShip.spaceState.systemID) ||
    ...
    if (Number(activeShip.typeID) !== CAPSULE_TYPE_ID) {
        ...
        const dockedLocationID = getDockedLocationID(session);
        const capsuleResult = resolveReusableCapsuleForCharacter(session.characterID, activeShip.itemID, Number(session.solarsystemid2 || session.solarsystemid || targetSystemID) ||
            targetSystemID, dockedLocationID ||
            session.homeStationID ||
            session.cloneStationID ||
            60003760);
        if (!capsuleResult.success || !capsuleResult.data) {
        ...
        const activeShipResult = setActiveShipForCharacter(session.characterID, capsuleResult.data.itemID);
        if (!activeShipResult.success) {
            return activeShipResult;
        }
    ...
    try {
        ...
        if (!targetEntity && targetShip.spaceState) {
            const spawnResult = spaceRuntime.spawnDynamicInventoryEntity(targetSystemID, targetItemID, {
                broadcastOptions: {
                    freshAcquire: false,
                },
            });
        ...
        if (typeof spaceRuntime.beginSessionJumpTimingTrace === "function") {
            spaceRuntime.beginSessionJumpTimingTrace(session, "clone-vat-jump", {
                sourceSystemID: wasInSpace && session._space
                    ? Number(session._space.systemID || 0) || null
                    : null,
        ...
        const moveResult = moveShipToSpace(activeShip.itemID, targetSystemID, spawnState);
        if (!moveResult.success) {
            return moveResult;
        }
        syncInventoryItemForSession(session, moveResult.data, {
        ...
        const updateResult = updateCharacterRecord(session.characterID, (record) => buildLocationIdentityPatch(record, targetSystemID, {
            ...(sourceStationID
                ? {
                    homeStationID: Number(record.homeStationID ||
                        record.cloneStationID ||
                        sourceStationID) || sourceStationID,
                    cloneStationID: Number(record.cloneStationID ||
        ...
        if (typeof spaceRuntime.recordSessionJumpTimingTrace === "function") {
            spaceRuntime.recordSessionJumpTimingTrace(session, "clone-vat-jump-attached", {
                destinationSystemID: targetSystemID,
                targetShipID: targetItemID,
                shipID: moveResult.data && moveResult.data.itemID,
                spawnState,
        ...
        flushCharacterSessionNotificationPlan(session, applyResult.notificationPlan, {
            sessionChangeOptions: buildSolarSessionChangeOptions(session, targetSystemID),
        });
        queuePendingSessionEffects(session, {
            awaitBeyonceBoundBallpark: true,
            previousLocalChannelID,
        ...
        flushPendingCommandSessionEffects(session);
        log.info(`[SpaceTransition] Clone-vat jump ${session.characterName || session.characterID} capsule=${moveResult.data.itemID} targetShip=${targetItemID} system=${targetSystemID}`);
        return {
            success: true,
            data: {
                ship: moveResult.data,
