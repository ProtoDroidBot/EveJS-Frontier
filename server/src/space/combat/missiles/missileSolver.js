...
function estimateMissileEffectiveRange(snapshot = {}) {
    ...
    return Math.max(0, toFiniteNumber(snapshot.maxVelocity, 0) *
        (Math.max(0, toFiniteNumber(snapshot.flightTimeMs, 0)) / 1000));
}
function estimateMissileTimeToTargetSeconds(sourcePosition, targetPosition, targetRadius, maxVelocity, options = {}) {
    const missileVelocity = Math.max(0, toFiniteNumber(maxVelocity, 0));
    if (missileVelocity <= 0.000001) {
        return 0;
    }
    const distanceToTarget = Math.max(0, distance(sourcePosition, targetPosition) - (options.toCenter === true
        ? 0
        : Math.max(0, toFiniteNumber(targetRadius, 0))));
    return distanceToTarget / missileVelocity;
}
function estimateMissileClientImpactTimeMs(sourcePosition, targetPosition, targetRadius, maxVelocity) {
    const timeToTarget = estimateMissileTimeToTargetSeconds(sourcePosition, targetPosition, targetRadius, maxVelocity);
    return Math.max(0, round6(timeToTarget * 1000));
}
const MISSILE_CLIENT_NO_SPREAD_THRESHOLD_SECONDS = 1.6;
function resolveMissileClientVisualProfile(sourcePosition, targetPosition, targetRadius, maxVelocity) {
    const surfaceTimeSeconds = estimateMissileTimeToTargetSeconds(sourcePosition, targetPosition, targetRadius, maxVelocity);
    const centerTimeSeconds = Math.max(0.5, estimateMissileTimeToTargetSeconds(sourcePosition, targetPosition, targetRadius, maxVelocity, { toCenter: true }));
    const visualTimeSeconds = surfaceTimeSeconds > 0
...
function estimateMissileClientVisualImpactTimeMs(sourcePosition, targetPosition, targetRadius, maxVelocity) {
    return resolveMissileClientVisualProfile(sourcePosition, targetPosition, targetRadius, maxVelocity).visualImpactMs;
}
function estimateMissileFlightBudgetMs(snapshot = {}, sourceRadius = 0) {
    ...
    const launchRadiusBudgetMs = maxVelocity > 0.000001
...
function buildMissileImpactState(targetEntity = null) {
    return {
        signatureRadius: Math.max(1, toFiniteNumber(targetEntity && targetEntity.signatureRadius, 1)),
        absoluteVelocity: Math.max(0, magnitude(targetEntity && targetEntity.velocity)),
    };
}
function resolveMissileDamageReductionExponent(snapshot = {}) {
    ...
    const damageReductionSensitivity = Math.max(1.000001, toFiniteNumber(snapshot.damageReductionSensitivity, 5.5));
    ...
    if (damageReductionFactor > 1.000001) {
        return round6(Math.log(damageReductionFactor) / Math.log(damageReductionSensitivity));
    }
    return round6(clamp(damageReductionFactor, 0.000001, 1));
}
function resolveMissileApplicationFactor(snapshot = {}, impactState = {}) {
    const explosionRadius = Math.max(0.000001, toFiniteNumber(snapshot.explosionRadius, 1));
    const explosionVelocity = Math.max(0.000001, toFiniteNumber(snapshot.explosionVelocity, 1));
    const targetSignatureRadius = Math.max(0, toFiniteNumber(impactState.signatureRadius, 0));
    const targetAbsoluteVelocity = Math.max(0, toFiniteNumber(impactState.absoluteVelocity, 0));
    const sigFactor = targetSignatureRadius / explosionRadius;
    const velocityFactorBase = targetAbsoluteVelocity <= 0.000001
...
//# sourceMappingURL=missileSolver.js.map