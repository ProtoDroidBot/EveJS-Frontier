...
const ATTRIBUTE_DAMAGE_MULTIPLIER_BONUS_PER_CYCLE = getAttributeIDByNames("damageMultiplierBonusPerCycle") || 2733;
const ATTRIBUTE_DAMAGE_MULTIPLIER_BONUS_MAX = getAttributeIDByNames("damageMultiplierBonusMax") || 2734;
const ATTRIBUTE_DAMAGE_MULTIPLIER_BONUS_CURRENT = getAttributeIDByNames("damageMultiplierBonusCurrent") || 5804;
...
function getPrecursorBonusPerCycle(weaponSnapshot = null) {
    return Math.max(0, round6(toFiniteNumber(weaponSnapshot &&
        weaponSnapshot.moduleAttributes &&
        weaponSnapshot.moduleAttributes[ATTRIBUTE_DAMAGE_MULTIPLIER_BONUS_PER_CYCLE], 0)));
}
function getPrecursorBonusMax(weaponSnapshot = null) {
    return Math.max(0, round6(toFiniteNumber(weaponSnapshot &&
        weaponSnapshot.moduleAttributes &&
        weaponSnapshot.moduleAttributes[ATTRIBUTE_DAMAGE_MULTIPLIER_BONUS_MAX], 0)));
}
...
function computeMaxTimestampMs(nowMs, currentBonus, bonusPerCycle, maxBonus, durationMs) {
    ...
    return Math.max(0, round6(toFiniteNumber(nowMs, 0) +
...
function syncPrecursorAttributeOverrides(effectState) {
    ...
    overrides[ATTRIBUTE_DAMAGE_MULTIPLIER_BONUS_CURRENT] = round6(Math.max(0, toFiniteNumber(effectState.precursorSpoolCurrent, 0)));
...
function initializePrecursorTurretEffectState(effectState, weaponSnapshot, nowMs) {
    ...
    effectState.precursorSpoolMaxTimestampMs = computeMaxTimestampMs(nowMs, effectState.precursorSpoolCurrent, effectState.precursorSpoolPerCycle, effectState.precursorSpoolMax, weaponSnapshot && weaponSnapshot.durationMs);
...
function synchronizePrecursorTurretEffectState(effectState, weaponSnapshot, nowMs) {
    ...
    effectState.precursorSpoolPerCycle = getPrecursorBonusPerCycle(weaponSnapshot);
    effectState.precursorSpoolMax = getPrecursorBonusMax(weaponSnapshot);
    effectState.precursorSpoolCurrent = round6(Math.min(Math.max(0, toFiniteNumber(effectState.precursorSpoolCurrent, 0)), Math.max(0, toFiniteNumber(effectState.precursorSpoolMax, 0))));
    effectState.precursorSpoolMaxTimestampMs = computeMaxTimestampMs(nowMs, effectState.precursorSpoolCurrent, effectState.precursorSpoolPerCycle, effectState.precursorSpoolMax, weaponSnapshot && weaponSnapshot.durationMs);
    syncPrecursorAttributeOverrides(effectState);
    return effectState;
}
function advancePrecursorTurretSpool(effectState, weaponSnapshot, nowMs) {
    ...
    effectState.precursorSpoolCurrent = round6(Math.min(Math.max(0, toFiniteNumber(effectState.precursorSpoolMax, 0)), Math.max(0, toFiniteNumber(effectState.precursorSpoolCurrent, 0)) +
...
function buildPrecursorTurretGraphicInfo(effectState) {
    ...
    const multiplierBonusPerCycle = round6(Math.max(0, toFiniteNumber(effectState.precursorSpoolPerCycle, 0)));
    return {
        ...
        multiplierBonusMax: round6(Math.max(0, toFiniteNumber(effectState.precursorSpoolMax, 0))),
    };
}
function applyPrecursorTurretSpoolToSnapshot(weaponSnapshot, effectState) {
    ...
    return {
        ...
        spoolBonusCurrent,
        spoolMultiplier,
        damageMultiplier: round6(Math.max(0, toFiniteNumber(weaponSnapshot.damageMultiplier, 0)) * spoolMultiplier),
        rawShotDamage: {
...
//# sourceMappingURL=precursorTurrets.js.map