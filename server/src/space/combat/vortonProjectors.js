...
const { resolveMissileAppliedDamage, } = require(path.join(__dirname, "./missiles/missileSolver"));
 ...
 */
function resolveVortonShot({ attackerEntity, targetEntity, weaponSnapshot, } = {}) {
    const rawShotDamage = normalizeDamageVector(weaponSnapshot && weaponSnapshot.rawShotDamage);
    ...
    return {
        ...
        optimalRange: round6(optimalRange),
        outOfRange: false,
        applicationFactor: round6(applied.application && applied.application.applicationFactor),
        targetSignatureRadius: round6(applied.impactState && applied.impactState.signatureRadius),
 ...
 */
function selectVortonArcTargets({ attackerEntity, primaryTargetEntity, candidateEntities = [], weaponSnapshot, } = {}) {
    const arcRange = Math.max(0, toFiniteNumber(weaponSnapshot && weaponSnapshot.arcRange, 0));
    const arcTargets = Math.max(0, Math.trunc(toFiniteNumber(weaponSnapshot && weaponSnapshot.arcTargets, 0)));
...
//# sourceMappingURL=vortonProjectors.js.map