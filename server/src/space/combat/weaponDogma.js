...
const { getAttributeIDByNames, getEffectTypeRecord, getTypeEffectRecords, getPassiveModifierEffectRecords, getTypeAttributeMap, cloneAttributeMap, typeHasEffectName, listFittedItems, listFittedItemsForLocation, getPassiveModifierSourceItems, isPassiveModifierSource, appendDirectModifierEntries, appendSelfItemModifierEntries, appendLocationModifierEntries, buildEffectiveItemAttributeMap, isStructureDogmaHost, isChargeCompatibleWithModule, resolveDogmaSkillMapForHost, applyOtherItemModifiersToAttributes, applyModifierGroups, } = require(path.join(__dirname, "../../services/fitting/liveFittingState"));
const { buildNpcEffectiveModuleItem, } = require(path.join(__dirname, "../npc/npcCapabilityResolver"));
const { getActiveImplantCharacterModifierEntries, getActiveImplantLocationModifierSources, getActiveImplantShipModifierEntries, } = require(path.join(__dirname, "../../services/dogma/implants/activeImplantModifiers"));
...
const ATTRIBUTE_MISSILE_DAMAGE_MULTIPLIER = getAttributeIDByNames("missileDamageMultiplier") || 212;
...
const ATTRIBUTE_AOE_DAMAGE_REDUCTION_FACTOR = getAttributeIDByNames("aoeDamageReductionFactor") || 1353;
const ATTRIBUTE_AOE_DAMAGE_REDUCTION_SENSITIVITY = getAttributeIDByNames("aoeDamageReductionSensitivity") || 1354;
...
function resolveSkillLevel(skillRecord) {
    return Math.max(0, toInt(skillRecord && (skillRecord.effectiveSkillLevel ??
...
function getModuleChargeGroupIDs(itemOrTypeID) {
    ...
    for (let index = 1; index <= 5; index += 1) {
        const chargeGroupID = toInt(attributeMap[getAttributeIDByNames(`chargeGroup${index}`)], 0);
        if (chargeGroupID > 0) {
...
function resolveWeaponSpecialFxGUID({ family = null, moduleItem = null, chargeItem = null, activationEffect = null, } = {}) {
    ...
    if (chargeGroupID === TORPEDO_GROUP_ID ||
...
function hasModuleDirectWeaponDamage(itemOrTypeID) {
    const attributeMap = buildEffectiveItemAttributeMap(itemOrTypeID);
    return (toFiniteNumber(attributeMap[ATTRIBUTE_EM_DAMAGE], 0) > 0 ||
...
function isChargeOptionalTurretWeapon(moduleItem, chargeItem = null) {
    ...
    return (getModuleChargeGroupIDs(effectiveModuleItem).size <= 0 &&
        hasModuleDirectWeaponDamage(effectiveModuleItem));
}
function buildLocationModifiedAttributeMap(targetItem, shipItem, skillMap, shipModifierAttributes, fittedItems, activeModuleContexts, options = {}) {
    ...
    const resolvedPassiveModifierSourceItems = getPassiveModifierSourceItems(shipItem, resolvedFittedItems);
    ...
    for (const skillRecord of resolvedSkillMap.values()) {
        appendLocationModifierEntries(modifierEntries, buildSkillEffectiveAttributes(skillRecord), getTypeEffectRecords(skillRecord.typeID), "skill", targetItem, {
            allowedDomains: locationModifierDomains,
            sourceTypeID: skillRecord.typeID,
        });
    }
    appendLocationModifierEntries(modifierEntries, shipModifierAttributes, getTypeEffectRecords(shipItem.typeID), "ship", targetItem, { allowedDomains: locationModifierDomains });
    for (const fittedItem of resolvedPassiveModifierSourceItems) {
        ...
        appendLocationModifierEntries(modifierEntries, buildEffectiveItemAttributeMap(fittedItem), getTypeEffectRecords(fittedItem.typeID), "fittedModule", targetItem, { allowedDomains: fittedModuleLocationModifierDomains });
    }
    for (const activeModuleContext of resolvedActiveModuleContexts) {
        const activeModuleItem = buildNpcEffectiveModuleItem(activeModuleContext && activeModuleContext.moduleItem);
        const activeEffectRecord = (activeModuleContext && activeModuleContext.effectRecord) ||
        ...
        if ((activeModuleContext &&
            ...
            if (toInt(activeModuleItem.itemID, 0) === toInt(targetItem && targetItem.itemID, 0)) {
                appendSelfItemModifierEntries(modifierEntries, buildEffectiveItemAttributeMap(activeModuleItem, activeModuleContext && activeModuleContext.chargeItem), [activeEffectRecord], "fittedModule");
            }
        }
        appendLocationModifierEntries(modifierEntries, buildEffectiveItemAttributeMap(activeModuleItem, activeModuleContext && activeModuleContext.chargeItem), [activeEffectRecord], "fittedModule", targetItem, { allowedDomains: locationModifierDomains });
    }
    for (const source of additionalLocationModifierSources) {
        if (!source || typeof source !== "object") {
            continue;
        }
        appendLocationModifierEntries(modifierEntries, source.sourceAttributes, source.sourceEffects, String(source.sourceKind || "system"), targetItem, { allowedDomains: locationModifierDomains });
    }
    applyModifierGroups(attributes, modifierEntries);
    return attributes;
}
function collectCharacterModifierAttributes(skillMap, fittedItems, activeModuleContexts, options = {}) {
    ...
    const resolvedActiveModuleContexts = Array.isArray(activeModuleContexts)
        ? activeModuleContexts
        : [];
    const additionalDirectModifierEntries = Array.isArray(options.additionalDirectModifierEntries)
        ? options.additionalDirectModifierEntries
        : [];
    for (const skillRecord of resolvedSkillMap.values()) {
        appendDirectModifierEntries(modifierEntries, buildSkillEffectiveAttributes(skillRecord), getTypeEffectRecords(skillRecord.typeID), "skill", CHARACTER_DIRECT_MODIFIER_OPTIONS);
    }
    for (const fittedItem of resolvedFittedItems) {
        ...
        appendDirectModifierEntries(modifierEntries, buildEffectiveItemAttributeMap(fittedItem), passiveSourceEffects, "fittedModule", CHARACTER_DIRECT_MODIFIER_OPTIONS);
    }
    for (const activeModuleContext of resolvedActiveModuleContexts) {
        const activeModuleItem = buildNpcEffectiveModuleItem(activeModuleContext && activeModuleContext.moduleItem);
        const activeEffectRecord = (activeModuleContext && activeModuleContext.effectRecord) ||
        ...
        appendDirectModifierEntries(modifierEntries, buildEffectiveItemAttributeMap(activeModuleItem, activeModuleContext && activeModuleContext.chargeItem), [activeEffectRecord], "fittedModule", CHARACTER_DIRECT_MODIFIER_OPTIONS);
    }
...
function buildSkillEffectiveAttributes(skillRecord) {
    ...
    for (const effectRecord of getTypeEffectRecords(typeID)) {
        ...
        for (const modifier of effectRecord.modifierInfo || []) {
            ...
            applyDirectModifier(attributes, modifier.modifiedAttributeID, attributes[modifier.modifyingAttributeID], modifier.operation);
        }
    }
...
function collectShipModifierAttributes(shipItem, skillMap, activeModuleContexts = null, options = {}) {
            ...
            for (const skillRecord of resolvedSkillMap.values()) {
                ...
                for (const effectRecord of getTypeEffectRecords(skillRecord.typeID)) {
                    for (const modifier of effectRecord.modifierInfo || []) {
                        ...
                        applyDirectModifier(shipAttributes, modifier.modifiedAttributeID, effectiveSkillAttributes[modifier.modifyingAttributeID], modifier.operation);
                    }
                }
            }
    ...
    const hiddenModifierSourceOptions = Array.isArray(options.hiddenModifierItems)
        ...
        : {};
    const additionalDirectModifierEntries = !isStructureHost && Array.isArray(options.additionalDirectModifierEntries)
        ? options.additionalDirectModifierEntries
        : [];
    const hiddenModifierSources = getPassiveModifierSourceItems(shipItem, [], hiddenModifierSourceOptions);
    const fittedModifierSources = isStructureHost
        ? getPassiveModifierSourceItems(shipItem, resolvedFittedItems, hiddenModifierSourceOptions)
    ...
    for (const modifierSource of fittedModifierSources) {
        ...
        appendDirectModifierEntries(modifierEntries, buildEffectiveItemAttributeMap(modifierSource), getPassiveModifierEffectRecords(modifierSource.typeID), "fittedModule", isStructureHost ? STRUCTURE_ITEM_MODIFIER_OPTIONS : SHIP_ITEM_MODIFIER_OPTIONS);
    }
    for (const activeModuleContext of resolvedActiveModuleContexts) {
        const activeModuleItem = buildNpcEffectiveModuleItem(activeModuleContext && activeModuleContext.moduleItem);
        const activeEffectRecord = (activeModuleContext && activeModuleContext.effectRecord) ||
        ...
        appendDirectModifierEntries(modifierEntries, buildEffectiveItemAttributeMap(activeModuleItem, activeModuleContext && activeModuleContext.chargeItem), [activeEffectRecord], "fittedModule", SHIP_ITEM_MODIFIER_OPTIONS);
    }
...
function resolveWeaponFamily(moduleItem, chargeItem = null) {
    const explicitFamily = String(moduleItem && (moduleItem.npcWeaponFamily ??
        moduleItem.weaponFamily) || "").trim();
    if (explicitFamily) {
    ...
    const chargeGroupID = toInt(chargeItem && chargeItem.groupID, 0);
    const isMissileLauncher = typeHasEffectName(moduleTypeID, "useMissiles") &&
    ...
    if (isMissileLauncher) {
        ...
        if (DEFENDER_MISSILE_CHARGE_GROUP_IDS.has(chargeGroupID)) {
            if (DEFENDER_MISSILE_LAUNCHER_GROUP_IDS.has(moduleGroupID) ||
    ...
    const isTurret = typeHasEffectName(moduleTypeID, "turretFitted");
    if (!isTurret) {
        return null;
    }
...
function buildMissileModuleSnapshot({ shipItem, moduleItem, chargeItem, fittedItems, skillMap, activeModuleContexts, effectiveModuleItem, family = "missileLauncher", additionalLocationModifierSources = null, directModuleModifierEntries = null, directChargeModifierEntries = null, directCharacterModifierEntries = null, directShipModifierEntries = null, hiddenModifierItems = null, } = {}) {
    ...
    const resolvedActiveModuleContexts = Array.isArray(activeModuleContexts)
        ? activeModuleContexts
        : [];
    const resolvedAdditionalLocationModifierSources = Array.isArray(additionalLocationModifierSources)
    ...
    const resolvedDirectChargeModifierEntries = Array.isArray(directChargeModifierEntries)
        ...
        : [];
    const resolvedDirectCharacterModifierEntries = Array.isArray(directCharacterModifierEntries)
    ...
    const resolvedDirectShipModifierEntries = Array.isArray(directShipModifierEntries)
        ...
        : [];
    const shipModifierAttributes = collectShipModifierAttributes(shipItem, resolvedSkillMap, resolvedActiveModuleContexts, {
    ...
    const moduleAttributes = buildLocationModifiedAttributeMap(effectiveModuleItem, shipItem, resolvedSkillMap, shipModifierAttributes, resolvedFittedItems, resolvedActiveModuleContexts, {
    ...
    const chargeAttributes = buildLocationModifiedAttributeMap(chargeItem, shipItem, resolvedSkillMap, shipModifierAttributes, resolvedFittedItems, resolvedActiveModuleContexts, {
        additionalLocationModifierSources: resolvedAdditionalLocationModifierSources,
    });
    ...
    const characterAttributes = collectCharacterModifierAttributes(resolvedSkillMap, resolvedFittedItems, resolvedActiveModuleContexts, {
    ...
    const chargeGroupID = toInt(chargeItem && chargeItem.groupID, 0);
    const moduleGroupID = toInt(effectiveModuleItem && effectiveModuleItem.groupID, 0);
    const isDefenderMissile = DEFENDER_MISSILE_CHARGE_GROUP_IDS.has(chargeGroupID) ||
        DEFENDER_MISSILE_LAUNCHER_GROUP_IDS.has(moduleGroupID);
    const missileDamageMultiplier = Math.max(0, toFiniteNumber(characterAttributes[ATTRIBUTE_MISSILE_DAMAGE_MULTIPLIER], 1));
    const baseDamage = {
    ...
    const flightTimeMs = Math.max(1, round6(toFiniteNumber(chargeAttributes[ATTRIBUTE_EXPLOSION_DELAY], 1000)));
    const maxVelocity = Math.max(0, round6(toFiniteNumber(chargeAttributes[ATTRIBUTE_MAX_VELOCITY], 0)));
    return {
        family,
        ...
        isDefenderMissile,
        chargeQuantity: Math.max(0, toInt(chargeItem && (chargeItem.stacksize ?? chargeItem.quantity), 0)),
        ...
        maxVelocity,
        flightTimeMs,
        explosionRadius: Math.max(1, round6(toFiniteNumber(chargeAttributes[ATTRIBUTE_AOE_CLOUD_SIZE], 1))),
        explosionVelocity: Math.max(0.001, round6(toFiniteNumber(chargeAttributes[ATTRIBUTE_AOE_VELOCITY], 0.001))),
        damageReductionFactor: clamp(toFiniteNumber(chargeAttributes[ATTRIBUTE_AOE_DAMAGE_REDUCTION_FACTOR], 1), 0.000001, 1),
        damageReductionSensitivity: Math.max(0.000001, round6(toFiniteNumber(chargeAttributes[ATTRIBUTE_AOE_DAMAGE_REDUCTION_SENSITIVITY], DEFAULT_MISSILE_DAMAGE_REDUCTION_SENSITIVITY))),
...
function buildWeaponModuleSnapshot({ characterID, shipItem, moduleItem, chargeItem = null, fittedItems = null, skillMap = null, activeModuleContexts = null, additionalLocationModifierSources = null, directModuleModifierEntries = null, directChargeModifierEntries = null, hiddenModifierItems = null, } = {}) {
    ...
    const implantLocationModifierSources = !isStructureHost && numericCharacterID > 0
    ...
    const implantCharacterModifierEntries = !isStructureHost && numericCharacterID > 0
        ? getActiveImplantCharacterModifierEntries(characterID)
        : [];
    const implantShipModifierEntries = !isStructureHost && numericCharacterID > 0
        ? getActiveImplantShipModifierEntries(characterID)
        : [];
    const chargeOptionalTurretWeapon = isChargeOptionalTurretWeapon(effectiveModuleItem, chargeItem);
    if (isMissileWeaponFamily(family)) {
        ...
        const resolvedSkillMap = resolveDogmaSkillMapForHost(characterID, shipItem, { skillMap });
        const resolvedActiveModuleContexts = Array.isArray(activeModuleContexts)
            ? activeModuleContexts
            : [];
    ...
    const resolvedActiveModuleContexts = Array.isArray(activeModuleContexts)
        ? activeModuleContexts
        : [];
    const resolvedAdditionalLocationModifierSources = Array.isArray(mergedAdditionalLocationModifierSources)
    ...
    const hiddenModifierSourceOptions = Array.isArray(hiddenModifierItems)
        ? { hiddenModifierItems }
        : {};
    const resolvedPassiveModifierSourceItems = getPassiveModifierSourceItems(shipItem, resolvedFittedItems, hiddenModifierSourceOptions);
    const locationModifierDomains = isStructureHost
        ? new Set(["structureID", "charID"])
        : new Set(["shipID", "charID"]);
    const shipModifierAttributes = collectShipModifierAttributes(shipItem, resolvedSkillMap, resolvedActiveModuleContexts, {
    ...
    const moduleAttributes = Boolean(moduleItem &&
    ...
    const modifierEntries = [];
    const chargeAttributes = chargeItem && typeof chargeItem === "object"
        ? buildLocationModifiedAttributeMap(chargeItem, shipItem, resolvedSkillMap, shipModifierAttributes, resolvedFittedItems, resolvedActiveModuleContexts, {
            additionalLocationModifierSources: resolvedAdditionalLocationModifierSources,
        })
        : {};
    for (const skillRecord of resolvedSkillMap.values()) {
        appendLocationModifierEntries(modifierEntries, buildSkillEffectiveAttributes(skillRecord), getTypeEffectRecords(skillRecord.typeID), "skill", effectiveModuleItem, {
            allowedDomains: locationModifierDomains,
            sourceTypeID: skillRecord.typeID,
        });
    }
    appendLocationModifierEntries(modifierEntries, shipModifierAttributes, getTypeEffectRecords(shipItem.typeID), "ship", effectiveModuleItem, { allowedDomains: locationModifierDomains });
    for (const fittedItem of resolvedPassiveModifierSourceItems) {
        if (!isPassiveModifierSource(fittedItem) ||
            toInt(fittedItem.itemID, 0) === toInt(moduleItem.itemID, 0)) {
            continue;
        }
        appendLocationModifierEntries(modifierEntries, buildEffectiveItemAttributeMap(fittedItem), getTypeEffectRecords(fittedItem.typeID), "fittedModule", effectiveModuleItem, { allowedDomains: locationModifierDomains });
    }
    for (const activeModuleContext of resolvedActiveModuleContexts) {
        const activeModuleItem = buildNpcEffectiveModuleItem(activeModuleContext && activeModuleContext.moduleItem);
        const activeEffectRecord = (activeModuleContext && activeModuleContext.effectRecord) ||
        ...
        if ((activeModuleContext &&
            ...
            activeModuleContext.effectState.isOverload === true) ||
            toInt(activeEffectRecord.effectCategoryID, 0) === 5) {
            if (toInt(activeModuleItem.itemID, 0) ===
                toInt(effectiveModuleItem.itemID || moduleItem.itemID, 0)) {
                appendSelfItemModifierEntries(modifierEntries, buildEffectiveItemAttributeMap(activeModuleItem, activeModuleContext && activeModuleContext.chargeItem), [activeEffectRecord], "fittedModule");
            }
        }
        appendLocationModifierEntries(modifierEntries, buildEffectiveItemAttributeMap(activeModuleItem, activeModuleContext && activeModuleContext.chargeItem), [activeEffectRecord], "fittedModule", effectiveModuleItem, { allowedDomains: locationModifierDomains });
    }
    for (const source of resolvedAdditionalLocationModifierSources) {
        if (!source || typeof source !== "object") {
            continue;
        }
        appendLocationModifierEntries(modifierEntries, source.sourceAttributes, source.sourceEffects, String(source.sourceKind || "system"), effectiveModuleItem, { allowedDomains: locationModifierDomains });
    }
    ...
    applyModifierGroups(moduleAttributes, resolvedDirectModuleModifierEntries);
    applyModifierGroups(chargeAttributes, resolvedDirectChargeModifierEntries);
    const damageMultiplier = Math.max(0, toFiniteNumber(moduleAttributes[ATTRIBUTE_DAMAGE_MULTIPLIER], 1));
    const activationEffect = resolveWeaponActivationEffect(effectiveModuleItem.typeID);
    const chargeMode = resolveWeaponChargeMode(family);
    const damageSourceAttributes = chargeItem || !chargeOptionalTurretWeapon
        ? chargeAttributes
        : moduleAttributes;
    const chargeDamage = {
    ...
    return {
        ...
        optimalSigRadius: Math.max(1, round6(toFiniteNumber(moduleAttributes[ATTRIBUTE_OPTIMAL_SIG_RADIUS], 40000))),
        damageMultiplier,
        ...
        ...(isVortonWeaponFamily(family)
            ? {
                explosionRadius: Math.max(0.000001, round6(toFiniteNumber(moduleAttributes[ATTRIBUTE_AOE_CLOUD_SIZE], 1))),
                explosionVelocity: Math.max(0.000001, round6(toFiniteNumber(moduleAttributes[ATTRIBUTE_AOE_VELOCITY], 1))),
                damageReductionFactor: round6(toFiniteNumber(moduleAttributes[ATTRIBUTE_AOE_DAMAGE_REDUCTION_FACTOR], 1)),
                damageReductionSensitivity: round6(toFiniteNumber(moduleAttributes[ATTRIBUTE_AOE_DAMAGE_REDUCTION_SENSITIVITY], 5.5)),
                arcRange: Math.max(0, round6(toFiniteNumber(moduleAttributes[ATTRIBUTE_VORTON_ARC_RANGE], 0))),
                arcTargets: Math.max(0, Math.trunc(toFiniteNumber(moduleAttributes[ATTRIBUTE_VORTON_ARC_TARGETS], 0))),
            }
            : {}),
        moduleAttributes,
        chargeAttributes,
...
//# sourceMappingURL=weaponDogma.js.map