...
function resolveWeaponTypeID(options: Record<string, any> = {}) {
...
function resolveAttackerIdentity(attackerEntity, options: Record<string, any> = {}) {
  if (!attackerEntity) {
    return {
      characterID: null,
      corporationID: null,
...
function awardKillmarkForFinalBlow(targetEntity, attackerEntity, finalAttacker, victimIdentity, record, options: Record<string, any> = {}) {
...
function resolveVictimIdentity(targetEntity, options: Record<string, any> = {}) {
  if (!targetEntity) {
    return {
...
function buildAttackerLedgerKey(identity: Record<string, any> = {}) {
  return [
...
function noteDamage(attackerEntity, targetEntity, appliedDamage, options: Record<string, any> = {}) {
...
function resolveKillmailItems(destroyResult: Record<string, any> = {}, lootLocationIDs: any[] = []) {
...
function sumItemLossValue(items: any[] = []) {
...
function resolveLootLocationIDs(targetEntity, destroyResult: Record<string, any> = {}) {
  if (
...
function resolveBountyPayout(targetEntity, finalAttacker: Record<string, any> = {}, options: Record<string, any> = {}) {
...
function uniqueKillRightNotificationSessions(characterIDs, candidateSessions: any[] = []) {
  const sessions: any[] = [];
...
function resolveKillmailNotificationSessions(victimCharacterID, extraSessions: any[] = []) {
...
function notifyVictimKillmailAvailable(record, targetEntity, victimIdentity, options: Record<string, any> = {}) {
...
function recordKillmailFromDestruction(targetEntity, destroyResult, options: Record<string, any> = {}) {
  ...
  if (isNpcVictimEntity(targetEntity)) {
    try {
      ...
      factionKillStanding.recordNpcFactionStandingLoss(
        targetEntity,
        toPositiveInt(finalAttacker && finalAttacker.characterID, 0),
  ...
  const killRecordInput: Record<string, any> = {
    ...
    killRightSupplied: killRightActivation
      ...
      : null,
    attackers: Object.entries<any>(ledger.attackers)
