...
const { buildKillmailItemTreeForLocation, } = require(path.join(__dirname, "../../services/killmail/killmailItemPayload"));
const { resolveItemByTypeID, } = require(path.join(__dirname, "../../services/inventory/itemTypeRegistry"));
const { JOURNAL_ENTRY_TYPE, adjustCharacterBalance, getCharacterWallet, } = require(path.join(__dirname, "../../services/account/walletState"));
...
function resolveWeaponTypeID(options = {}) {
    ...
    return (toPositiveInt(options.weaponTypeID, toPositiveInt(weaponSnapshot && weaponSnapshot.chargeTypeID, toPositiveInt(weaponSnapshot && weaponSnapshot.moduleTypeID, toPositiveInt(chargeItem && chargeItem.typeID, toPositiveInt(moduleItem && moduleItem.typeID, null))))) || null);
}
function resolveEntityTypeRecord(entity) {
    const typeID = toPositiveInt(entity && (entity.typeID || entity.slimTypeID), null);
    return typeID ? resolveItemByTypeID(typeID) || null : null;
}
function resolveEntityGroupID(entity, typeRecord = null) {
    return toPositiveInt(entity && (entity.groupID || entity.slimGroupID), toPositiveInt(typeRecord && typeRecord.groupID, 0)) || 0;
}
function resolveEntityCategoryID(entity, typeRecord = null) {
    return toPositiveInt(entity && (entity.categoryID || entity.slimCategoryID), toPositiveInt(typeRecord && typeRecord.categoryID, 0)) || 0;
}
function resolveAttackerIdentity(attackerEntity, options = {}) {
    ...
    const characterID = toPositiveInt(attackerEntity.pilotCharacterID ?? attackerEntity.characterID, null);
    const characterRecord = characterID ? resolveCharacterRecord(characterID) || {} : {};
    return {
        characterID,
        corporationID: toPositiveInt(characterRecord.corporationID, toPositiveInt(attackerEntity.corporationID, toPositiveInt(attackerEntity.ownerID, null))),
        allianceID: toPositiveInt(characterRecord.allianceID, toPositiveInt(attackerEntity.allianceID, null)),
        factionID: toPositiveInt(characterRecord.factionID, toPositiveInt(attackerEntity.warFactionID, null)),
        shipTypeID: toPositiveInt(attackerEntity.typeID, null),
        weaponTypeID: resolveWeaponTypeID(options),
        securityStatus: characterID && characterRecord
            ? toFiniteNumber(characterRecord.securityStatus ?? characterRecord.securityRating, 0)
            : null,
    };
}
...
function resolveKillmarkAwardShipID(attackerEntity) {
    ...
    return (toPositiveInt(attackerEntity.controllerID, 0) ||
    ...
    try {
        ...
        const scene = session && spaceRuntime && typeof spaceRuntime.getSceneForSession === "function"
            ...
            : spaceRuntime && spaceRuntime.scenes instanceof Map
                ? spaceRuntime.scenes.get(toPositiveInt(attackerEntity.systemID, toPositiveInt(targetEntity && targetEntity.systemID, 0))) || null
                : null;
...
function resolveSessionCharacterID(session) {
    return toPositiveInt(session && (session.characterID || session.charID || session.charid), null);
}
function resolveVictimIdentity(targetEntity, options = {}) {
    ...
    if (String(targetEntity.kind || "").toLowerCase() === "structure") {
        return {
            victimCharacterID: null,
            victimCorporationID: toPositiveInt(targetEntity.corporationID, toPositiveInt(targetEntity.ownerID, null)),
    ...
    const characterID = isNpcVictimEntity(targetEntity)
        ? null
        : toPositiveInt(targetEntity.pilotCharacterID ?? targetEntity.characterID, sessionCharacterID);
    const characterRecord = characterID ? resolveCharacterRecord(characterID) || {} : {};
    return {
        victimCharacterID: characterID,
        victimCorporationID: toPositiveInt(characterRecord.corporationID, toPositiveInt(targetEntity.corporationID, toPositiveInt(victimSession && victimSession.corporationID, toPositiveInt(targetEntity.ownerID, null)))),
        victimAllianceID: toPositiveInt(characterRecord.allianceID, toPositiveInt(targetEntity.allianceID, toPositiveInt(victimSession && victimSession.allianceID, null))),
        victimFactionID: toPositiveInt(characterRecord.factionID, toPositiveInt(targetEntity.warFactionID, toPositiveInt(victimSession && victimSession.warFactionID, null))),
        victimShipTypeID: toPositiveInt(targetEntity.typeID, null),
    };
}
...
function noteDamage(attackerEntity, targetEntity, appliedDamage, options = {}) {
    ...
    ledger.solarSystemID = toPositiveInt(targetEntity.systemID, toPositiveInt(ledger.solarSystemID, null));
    return cloneValue(currentEntry);
}
function resolveKillmailItems(destroyResult = {}, lootLocationIDs = []) {
    const lootOutcomeItems = destroyResult &&
        destroyResult.data &&
...
function sumItemLossValue(items = []) {
    return items.reduce((sum, item) => {
        const basePrice = Math.max(0, toFiniteNumber((resolveItemByTypeID(item && item.typeID) || {}).basePrice, 0));
        const quantity = Math.max(0, toInteger(item && item.qtyDropped, 0)) +
...
function resolveLootLocationIDs(targetEntity, destroyResult = {}) {
    ...
    if (String(targetEntity && targetEntity.kind || "").toLowerCase() === "structure" &&
        destroyResult &&
        destroyResult.data) {
...
function resolveBountyPayout(targetEntity, finalAttacker = {}, options = {}) {
    ...
    const creditResult = adjustCharacterBalance(hunterCharacterID, payoutResult.amount, {
        ...
        ownerID2: toPositiveInt(options.victimIdentity && options.victimIdentity.victimCharacterID, 0) ||
            ...
            0,
        referenceID: toPositiveInt(targetEntity && targetEntity.itemID, 0) ||
...
function selectKillRightActivationForKill(victimCharacterID, finalAttacker, whenMs) {
    ...
    const activations = killRightState.readActiveActivationsForTarget(targetCharacterID, whenMs);
    ...
    return activations.find((activation) => (finalCharacterID > 0 &&
        toPositiveInt(activation && activation.activatorID, 0) === finalCharacterID)) || activations[0];
}
function uniqueKillRightNotificationSessions(characterIDs, candidateSessions = []) {
    const sessions = [];
    const seen = new Set();
    const characterSet = new Set((Array.isArray(characterIDs) ? characterIDs : [])
    ...
    const addSession = (session) => {
        ...
        const characterID = toPositiveInt(session.characterID || session.charID || session.charid, 0);
...
function resolveKillmailNotificationSessions(victimCharacterID, extraSessions = []) {
    ...
    const addSession = (session) => {
        ...
        const sessionCharacterID = toPositiveInt(session.characterID || session.charID || session.charid, 0) || 0;
...
function notifyVictimKillmailAvailable(record, targetEntity, victimIdentity, options = {}) {
    const victimCharacterID = toPositiveInt(victimIdentity && victimIdentity.victimCharacterID, 0) || 0;
    ...
    const killmailState = getKillmailStateService();
    const killMailHash = killmailState && typeof killmailState.getKillmailHashValue === "function"
    ...
    const notificationResult = notificationState.createNotification(victimCharacterID, {
        ...
        data: {
            killMailHash,
            killMailID: killID,
            victimShipTypeID: toPositiveInt(record.victimShipTypeID, 0) ||
    ...
    const sessions = resolveKillmailNotificationSessions(victimCharacterID, extraSessions);
    for (const session of sessions) {
...
function consumeKillRightAfterKill(activation, targetEntity, attackerEntity, whenMs) {
    ...
    const usedBy = toPositiveInt(attackerEntity && (attackerEntity.characterID || attackerEntity.pilotCharacterID), 0) ||
...
function recordKillmailFromDestruction(targetEntity, destroyResult, options = {}) {
    ...
    const iskLost = Math.max(0, toFiniteNumber((resolveItemByTypeID(victimIdentity.victimShipTypeID) || {}).basePrice, 0)) + sumItemLossValue(items);
    ...
    if (isNpcVictimEntity(targetEntity)) {
        try {
            ...
            factionKillStanding.recordNpcFactionStandingLoss(targetEntity, toPositiveInt(finalAttacker && finalAttacker.characterID, 0), { nowMs: whenMs, systemID: toPositiveInt(targetEntity && targetEntity.systemID, 0) });
        }
        catch (error) {
            /* faction-standing loss is best-effort; the kill must still record */
        }
    }
    const killRightActivation = selectKillRightActivationForKill(victimIdentity.victimCharacterID, finalAttacker, whenMs);
    ...
    if (record && toPositiveInt(record.killID, 0) > 0) {
        ...
        awardKillmarkForFinalBlow(targetEntity, options.attackerEntity || null, finalAttacker, victimIdentity, record, { whenMs });
    }
    if (record && toPositiveInt(record.killID, 0) > 0 && killRightActivation) {
        consumeKillRightAfterKill(killRightActivation, targetEntity, options.attackerEntity || null, whenMs);
    }
    ledgersByVictim.delete(ledgerKey);
    return record;
}
...
//# sourceMappingURL=killmailTracker.js.map