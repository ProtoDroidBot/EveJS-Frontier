...
const { normalizeDamageVector, sumDamageVector, } = require(path.join(__dirname, "./damage"));
...
function getSurfaceDistance(sourceEntity, targetEntity) {
    ...
    return Math.max(0, magnitude(delta) -
...
function getAngularVelocity(sourceEntity, targetEntity) {
    const relativePosition = subtractVectors(targetEntity && targetEntity.position, sourceEntity && sourceEntity.position);
    const centerDistance = Math.max(magnitude(relativePosition), 1);
    const relativeVelocity = subtractVectors(targetEntity && targetEntity.velocity, sourceEntity && sourceEntity.velocity);
    const radialUnit = {
...
function resolveTurretShot({ attackerEntity, targetEntity, weaponSnapshot, randomValue = Math.random(), } = {}) {
    const resolvedRawDamage = normalizeDamageVector(weaponSnapshot && weaponSnapshot.rawShotDamage);
    ...
    const trackingSpeed = Math.max(0, toFiniteNumber(weaponSnapshot && weaponSnapshot.trackingSpeed, 0));
    const optimalSigRadius = Math.max(1, toFiniteNumber(weaponSnapshot && weaponSnapshot.optimalSigRadius, 40000));
    const optimalRange = Math.max(0, toFiniteNumber(weaponSnapshot && weaponSnapshot.optimalRange, 0));
    const falloff = Math.max(0, toFiniteNumber(weaponSnapshot && weaponSnapshot.falloff, 0));
    const trackingTerm = trackingSpeed > 0
...
//# sourceMappingURL=laserTurrets.js.map