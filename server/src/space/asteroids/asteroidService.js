...
const { getSolarSystemOreTypeRecords, resolveMiningVisualPresentation, } = require(path.join(__dirname, "../../services/mining/miningVisuals"));
const { classifyMiningMaterialType, } = require(path.join(__dirname, "../../services/mining/miningInventory"));
const { TABLE, readStaticTable, } = require(path.join(__dirname, "../../services/_shared/referenceData"));
const { recordAsteroidBootstrap, resetMiningStartupSummary, } = require(path.join(__dirname, "../../services/mining/miningStartupSummary"));
const { getSolarSystemByID, getCelestialsForSystem, getLandscapeDungeonTemplateByID, getLandscapeEcosystemByID, } = require(path.join(__dirname, "../../space/worldData"));
...
function buildAsteroidItemID(beltID, asteroidIndex) {
    return (STATIC_ASTEROID_ITEM_ID_BASE +
...
function signedBellJitter(rng) {
    return clamp(((rng() + rng() + rng()) / 3 - 0.5) * 2, -1, 1);
}
function normalize2D(point, fallback = { x: 1, y: 0 }) {
    const length = Math.hypot(toFiniteNumber(point && point.x, 0), toFiniteNumber(point && point.y, 0));
    if (length <= 0.000001) {
        return { ...fallback };
    }
    return {
...
function combinePlaneVectors(primary, secondary, up, offset) {
    return addVectors(addVectors(scaleVector(primary, toFiniteNumber(offset && offset.x, 0)), scaleVector(secondary, toFiniteNumber(offset && offset.y, 0))), scaleVector(up, toFiniteNumber(offset && offset.z, 0)));
}
...
function resolveBeltOrbitContext(scene, belt, systemBelts = []) {
    const parentPosition = cloneVector((getCelestialsForSystem(scene.systemID).find((candidate) => toPositiveInt(candidate && candidate.itemID, 0) ===
        toPositiveInt(belt && belt.orbitID, 0)) || belt).position);
    const radial = normalizeVector(subtractVectors(cloneVector(belt.position), parentPosition), { x: 1, y: 0, z: 0 });
    const tangent = normalizeVector({ x: -radial.z, y: 0, z: radial.x }, { x: 0, y: 0, z: 1 });
    ...
    if (siblings.length > 1) {
        const siblingsByAngle = siblings
            .map((candidate) => ({
            belt: candidate,
            angle: Math.atan2(toFiniteNumber(candidate && candidate.position && candidate.position.z, 0) -
                parentPosition.z, toFiniteNumber(candidate && candidate.position && candidate.position.x, 0) -
                parentPosition.x),
        }))
            .sort((left, right) => left.angle - right.angle);
        const siblingIndex = siblingsByAngle.findIndex((entry) => toPositiveInt(entry && entry.belt && entry.belt.itemID, 0) ===
            toPositiveInt(belt && belt.itemID, 0));
        if (siblingIndex >= 0) {
            const current = siblingsByAngle[siblingIndex];
            const previous = siblingsByAngle[(siblingIndex - 1 + siblingsByAngle.length) % siblingsByAngle.length];
            ...
            gapBias = clamp((nextGap - previousGap) / Math.max(0.000001, nextGap + previousGap), -1, 1);
        }
    }
    return {
        parentPosition,
...
function buildCurvedFieldProfile(scene, belt, style, systemBelts = []) {
    ...
    const authoredCount = clamp(Math.max(1, toPositiveInt(belt && belt.asteroidCount, 1)), 1, STATIC_ASTEROID_ITEM_ID_STRIDE - 1);
    const maximumCount = clamp(toPositiveInt(belt && belt.maxAsteroidCount, STATIC_ASTEROID_ITEM_ID_STRIDE - 1), authoredCount, STATIC_ASTEROID_ITEM_ID_STRIDE - 1);
    const fieldRadiusMeters = Math.max(12_000, toFiniteNumber(belt && belt.fieldRadiusMeters, 32_000));
    const clusterRadiusMeters = Math.max(2_500, toFiniteNumber(belt && belt.clusterRadiusMeters, 6_000));
    const verticalSpreadMeters = Math.max(900, toFiniteNumber(belt && belt.verticalSpreadMeters, 4_000));
    const seededRng = createRng(hashText(`${belt.itemID}:${belt.fieldSeed}:${belt.fieldStyleID}:curveProfile`));
    ...
    const flowBias = multiBeltOrbit && Math.abs(orbitContext.gapBias) >= 0.14
        ? orbitContext.gapBias
        : (flowSeed * 2) - 1;
    const axisAngleDegrees = multiBeltOrbit
        ? lerp(shapeProfile.axisAngleMinDegrees, shapeProfile.axisAngleMaxDegrees, orientationSeed) + (orbitContext.gapBias * 12)
        : lerp(88, 96, orientationSeed);
    const axisAngleRadians = (axisAngleDegrees * Math.PI) / 180;
    const axisDirection = normalizeVector(addVectors(scaleVector(orbitContext.tangent, Math.cos(axisAngleRadians)), scaleVector(orbitContext.radial, Math.sin(axisAngleRadians))));
    const crossDirection = normalizeVector(addVectors(scaleVector(orbitContext.radial, Math.cos(axisAngleRadians)), scaleVector(orbitContext.tangent, -Math.sin(axisAngleRadians))));
    const axisSpanMeters = fieldRadiusMeters * (multiBeltOrbit
    ...
    const ribbonWidthMeters = clusterRadiusMeters * (multiBeltOrbit
    ...
    const visualRibbonWidthMeters = clusterRadiusMeters * (multiBeltOrbit
        ? lerp(shapeProfile.multiVisualWidthFactorMin, shapeProfile.multiVisualWidthFactorMax, visualWidthSeed)
        : lerp(shapeProfile.singleVisualWidthFactorMin, shapeProfile.singleVisualWidthFactorMax, visualWidthSeed));
    const bowMeters = fieldRadiusMeters * (multiBeltOrbit
    ...
    const densityAreaPerAsteroid = multiBeltOrbit
        ? lerp(shapeProfile.multiAreaPerAsteroidMin, shapeProfile.multiAreaPerAsteroidMax, densitySeed)
        : lerp(shapeProfile.singleAreaPerAsteroidMin, shapeProfile.singleAreaPerAsteroidMax, densitySeed);
    const laneCount = pickIntegerInRange(multiBeltOrbit ? shapeProfile.multiLaneCountMin : shapeProfile.singleLaneCountMin, multiBeltOrbit ? shapeProfile.multiLaneCountMax : shapeProfile.singleLaneCountMax, seededRng);
    const anchorBias = flowBias * axisSpanMeters * (multiBeltOrbit ? 0.26 : 0.42);
    const areaCount = Math.round((axisSpanMeters * ribbonWidthMeters) / Math.max(1, densityAreaPerAsteroid));
    const count = clamp(Math.max(authoredCount, areaCount), authoredCount, maximumCount);
    const halfSpanMeters = axisSpanMeters * 0.5;
    const controlLeadMeters = anchorBias * 0.22;
    const bowSign = flowBias >= 0 ? 1 : -1;
    const sideSign = multiBeltOrbit && Math.abs(orbitContext.gapBias) >= 0.2
        ...
        : hashText(`${belt.itemID}:curveSide`) & 1
            ? -1
            : 1;
    const sideOffsetMeters = sideSign * fieldRadiusMeters * (multiBeltOrbit
        ? lerp(shapeProfile.multiSideOffsetFactorMin, shapeProfile.multiSideOffsetFactorMax, sideOffsetSeed)
        : lerp(shapeProfile.singleSideOffsetFactorMin, shapeProfile.singleSideOffsetFactorMax, sideOffsetSeed));
    const start = {
...
function buildCurvedAsteroidOffset(profile, asteroidIndex, totalCount, rng) {
    ...
    const orderedRatio = safeTotalCount <= 1 ? 0.5 : asteroidIndex / Math.max(1, safeTotalCount - 1);
    const ratio = clamp(orderedRatio + (((rng() * 2) - 1) / Math.max(8, safeTotalCount)), 0, 1);
    const laneCenterIndex = (profile.laneCount - 1) * 0.5;
    const laneIndex = profile.laneCount <= 1 ? 0 : asteroidIndex % profile.laneCount;
    const laneOffsetBase = profile.laneCount <= 1
        ? 0
        : (laneIndex - laneCenterIndex) * profile.laneSpacingMeters * 0.5;
    const laneOffset = laneOffsetBase + (signedBellJitter(rng) * profile.laneJitterMeters);
    const curvePoint = buildBezierPoint(profile.start, profile.control, profile.end, ratio);
    const tangent2D = normalize2D(buildBezierTangent(profile.start, profile.control, profile.end, ratio), { x: 1, y: 0 });
    ...
    return combinePlaneVectors(profile.axisDirection, profile.crossDirection, profile.upDirection, {
        x: local2D.x,
        y: local2D.y,
        z: ((rng() * 2) - 1) *
...
function resolveFrontierDungeonObjectAnchors(belt, options = {}) {
    ...
    let plan = null;
    try {
        plan = scenePlanBuilder(belt, ecosystem, (dungeonID) => resolveDungeon(dungeonID), { maxSceneryProps: DEFAULT_MAX_SCENERY_PROPS });
    }
    catch (_error) {
        return [];
    }
...
function buildDungeonAnchoredAsteroidOffset(belt, asteroidIndex, rng, dungeonObjectAnchors = []) {
    ...
    const minimumScatterMeters = Math.max(5_000, toFiniteNumber(belt && belt.dungeonObjectScatterMinMeters, 18_000));
    const maximumScatterMeters = Math.max(minimumScatterMeters, toFiniteNumber(belt && belt.dungeonObjectScatterMaxMeters, 48_000));
    ...
    const distance = lerp(minimumScatterMeters, maximumScatterMeters, Math.cbrt(clamp(rng(), 0, 1)));
...
function buildAsteroidOffsetForBelt(belt, fieldProfile, asteroidIndex, totalCount, rng, dungeonObjectAnchors = []) {
    if (belt && belt.frontierLandscapeSite === true && dungeonObjectAnchors.length > 0) {
        const anchoredOffset = buildDungeonAnchoredAsteroidOffset(belt, asteroidIndex, rng, dungeonObjectAnchors);
        if (anchoredOffset) {
...
function buildFrontierResourcePool(belt) {
    ...
    for (const resourceTypeID of resourceTypeIDs) {
        ...
        const outputTypeID = toPositiveInt(getTypeDogmaAttributeValue(carrierTypeID, ASTEROID_OUTPUT_TYPE_ATTRIBUTE_ID), carrierTypeID);
...
function getSecurityMetadata(systemID) {
    ...
    const securityStatus = toFiniteNumber(row.securityStatus ?? row.security ?? row.trueSec, 0);
...
function classifyFamilyBucket(familyName, securityClass) {
    const families = SECURITY_CLASS_FAMILIES[securityClass] ||
...
function buildEnrichedSystemOrePool(systemID) {
    ...
    const orePool = Array.from(mergedByTypeID.values())
        ...
        .sort((left, right) => {
        const nameDelta = String(left.familyName || left.name || "").localeCompare(String(right.familyName || right.name || ""));
        if (nameDelta !== 0) {
            return nameDelta;
        }
...
function buildBeltOreSubset(orePool, securityClass, rng) {
    ...
    for (const [bucketName, rules] of Object.entries(BELT_BUCKETS)) {
        ...
        const target = Math.min(familyNames.length, pickIntegerInRange(rules.targetMin, rules.targetMax, rng));
...
function buildSystemOreAsteroidEntity(scene, belt, asteroidIndex, totalCount, fieldProfile, rng, pool, dungeonObjectAnchors = []) {
    ...
    const asteroidOffset = buildAsteroidOffsetForBelt(belt, fieldProfile, asteroidIndex, totalCount, rng, dungeonObjectAnchors);
...
function buildBeltFieldPlan(scene, belt) {
    ...
    for (let asteroidIndex = 0; asteroidIndex < totalCount; asteroidIndex += 1) {
        const entity = beltSubset.length > 0
            ? buildSystemOreAsteroidEntity(scene, belt, asteroidIndex, totalCount, fieldProfile, rng, beltSubset, dungeonObjectAnchors)
            : buildDecorativeFallbackAsteroidEntity(belt, style, asteroidIndex, legacyClusterOffsets, rng);
        if (!entity) {
            continue;
        }
...
function finalizeSceneAsteroidFieldPlan(scene, plan, spawnedCounts = []) {
    if (!scene || !plan || toPositiveInt(plan.systemID, 0) !== toPositiveInt(scene.systemID, 0)) {
        return false;
    }
...
function listGeneratedAsteroidEntities(scene) {
    ...
    return scene.staticEntities.filter((entity) => (entity &&
...
function handleSceneCreated(scene) {
    ...
    const spawned = applySceneAsteroidFieldPlan(scene, buildSceneAsteroidFieldPlan(scene));
    return {
        success: true,
        data: {
            spawned,
...
function resetSceneAsteroidFields(scene, options = {}) {
    ...
    for (const entity of listGeneratedAsteroidEntities(scene)) {
        if (scene.removeStaticEntity(entity.itemID, {
...
//# sourceMappingURL=asteroidService.js.map