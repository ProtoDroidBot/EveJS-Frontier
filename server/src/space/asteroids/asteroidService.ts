...
function normalize2D(point, fallback: Record<string, any> = { x: 1, y: 0 }) {
...
function resolveBeltOrbitContext(scene, belt, systemBelts: any[] = []) {
...
function buildCurvedFieldProfile(scene, belt, style, systemBelts: any[] = []) {
...
function buildFrontierResourcePool(belt) {
  const resourceTypeIDs = Array.isArray(belt && belt.resourceTypeIDs)
    ...
    : [];
  const seenCarrierTypeIDs = new Set<any>();
  const pool: any[] = [];
...
function resetSceneAsteroidFields(scene, options: Record<string, any> = {}) {
  if (!scene) {
    return {
      success: false,
