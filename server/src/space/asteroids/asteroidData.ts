...
function buildCacheFromRows(
  authoredBelts: any[] = [],
  authoredFieldStyles: any[] = [],
  landscapeSites: any[] = [],
) {
  ...
  const belts = [...beltsByID.values()].sort(
    (left, right) => Number(left.itemID) - Number(right.itemID),
  );
...
function buildCache() {
  return buildCacheFromRows(
    readStaticRows(TABLE.ASTEROID_BELTS),
    readStaticRows(TABLE.ASTEROID_FIELD_STYLES),
    readStaticRows(TABLE.LANDSCAPE_SITES),
  );
}
...
function ensureLoaded() {
  if (!cache) {
    cache = buildCache();
    log.info(
      `[Asteroids] Loaded ${cache.authoredBeltCount} authored asteroid belts, ` +
