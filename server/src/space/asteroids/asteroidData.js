...
const { FRONTIER_SYNTHETIC_RESOURCE_FIELDS_ENABLED, } = require(path.join(__dirname, "./frontierResourceFields"));
let cache = null;
function buildCache() {
...
function ensureLoaded() {
    if (!cache) {
        cache = buildCache();
        log.info(`[Asteroids] Loaded ${cache.belts.length} authored asteroid belts and ` +
