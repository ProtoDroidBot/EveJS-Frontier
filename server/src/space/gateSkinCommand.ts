...
const DEFAULT_UP: Readonly<Vector3> = Object.freeze({ x: 0, y: 1, z: 0 });
const DEFAULT_FORWARD: Readonly<Vector3> = Object.freeze({ x: 0, y: 0, z: 1 });
const DEFAULT_RIGHT: Readonly<Vector3> = Object.freeze({ x: 1, y: 0, z: 0 });
...
function buildGateSkinPreviewEntries(context) {
  ...
  return buildPreviewPositions(context.shipEntity)
    .map(({ preset, position }: Record<string, any>) => {
      const materials = catalogue[preset.key] || [];
...
function scheduleGateSkinPreviewModelRefresh(active, entities) {
  ...
  if (!(active.refreshTimers instanceof Set)) {
    active.refreshTimers = new Set<any>();
  }
  active.refreshTimers.add(refreshTimer);
  return true;
}

function applyGateSkinCycle(active, options: Record<string, any> = {}) {
...
function stopGateSkinShowcaseByCharacterID(characterID, options: Record<string, any> = {}) {
...
function startGateSkinShowcase(session, context, callbackOptions: Record<string, any> = {}) {
...
function executeGateSkinCommand(session, argumentText, callbackOptions: Record<string, any> = {}) {
