...
const { tagUpdatesFreshAcquireLifecycleGroup, tagUpdatesMissileLifecycleGroup, tagUpdatesOwnerMissileLifecycleGroup, } = require("../dispatch/dispatchUtils");
const { normalizePersistentEntityID, } = require("../identity/entityID");
const { normalizeDestinyStamp, } = require("../delivery/stamps");
const { isTargetedLiveMissile, } = require("../stream/ballEncoding");
const { resolveFreshAcquireBootstrapModeUpdates, stripMissileFreshAcquireModeReplayUpdates, } = require("./freshAcquire");
...
function buildInitialBootstrapAddBallsPresentationUpdate(options = {}) {
    ...
    if (typeof options.buildAddBalls2Payload !== "function") {
        throw new TypeError("initial bootstrap AddBalls presentation requires buildAddBalls2Payload");
    }
    return {
        stamp,
        payload: options.buildAddBalls2Payload(stamp, entities, options.simFileTime ?? 0),
    };
}
function buildInitialBootstrapSetStatePresentationUpdate(options = {}) {
    ...
    if (typeof options.buildSetStatePayload !== "function") {
        throw new TypeError("initial bootstrap SetState presentation requires buildSetStatePayload");
    }
    return {
        stamp,
        payload: options.buildSetStatePayload(stamp, options.system, options.egoEntityID, entities, options.simFileTime ?? 0, Array.isArray(options.dbuffStateEntries)
...
function buildMissileLaunchActionUpdates(options = {}) {
    const entities = Array.isArray(options.entities) ? options.entities : [];
    const missileEntities = entities.filter((entity) => entity && entity.kind === "missile");
    if (missileEntities.length === 0) {
        return [];
    }
    if (typeof options.buildLaunchMissilePayload !== "function") {
        throw new TypeError("missile AddBalls presentation requires buildLaunchMissilePayload");
    }
    const stamp = normalizeDestinyStamp(options.stamp, 0);
    const updates = [];
...
function buildAddBallsPresentationUpdates(options = {}) {
    ...
    const entities = Array.isArray(options.entities) ? options.entities : [];
    if (typeof options.buildAddBalls2Payload !== "function") {
        throw new TypeError("buildAddBallsPresentationUpdates requires buildAddBalls2Payload");
    }
    if (typeof options.buildPrimeUpdatesForEntities !== "function") {
        throw new TypeError("buildAddBallsPresentationUpdates requires buildPrimeUpdatesForEntities");
    }
    if (typeof options.buildModeUpdates !== "function") {
        throw new TypeError("buildAddBallsPresentationUpdates requires buildModeUpdates");
    }
    if (options.replayActiveSpecialFx === true &&
        typeof options.buildActiveSpecialFxReplayUpdates !== "function") {
        throw new TypeError("active FX AddBalls presentation requires buildActiveSpecialFxReplayUpdates");
    }
    const buildPrimeUpdatesForEntities = options.buildPrimeUpdatesForEntities;
    const buildModeUpdates = options.buildModeUpdates;
    const buildActiveSpecialFxReplayUpdates = options.buildActiveSpecialFxReplayUpdates;
    ...
    updates.push(...requireUpdates(buildPrimeUpdatesForEntities(entities, stamp), "buildPrimeUpdatesForEntities"));
    const modeUpdates = [];
    for (const entity of entities) {
        modeUpdates.push(...requireUpdates(buildModeUpdates(entity, stamp), "buildModeUpdates"));
    }
    const bootstrapModeUpdates = resolveFreshAcquireBootstrapModeUpdates(entities, modeUpdates);
    if (bootstrapModeUpdates.length > 0) {
        updates.push(...bootstrapModeUpdates);
    }
    if (options.replayActiveSpecialFx === true) {
        const activeSpecialFxReplayUpdates = requireUpdates(buildActiveSpecialFxReplayUpdates(entities, stamp, options.presentationRawSimTimeMs, {
    ...
    const containsFreshAcquireMissiles = entities.some((entity) => entity && entity.kind === "missile");
    if (options.tagFreshAcquireLifecycle === true) {
...
function buildSessionStampedAddBallsPresentationForSession(options = {}) {
    ...
    const filterBallparkEntitiesForSession = typeof options.filterBallparkEntitiesForSession === "function"
        ? options.filterBallparkEntitiesForSession
        : (_targetSession, entities) => entities;
    const filteredEntities = filterBallparkEntitiesForSession(session, inputEntities);
    if (!Array.isArray(filteredEntities) || filteredEntities.length === 0) {
        return emptyResult;
    }
    const refreshEntitiesForSlimPayload = typeof options.refreshEntitiesForSlimPayload === "function"
    ...
    if (typeof options.buildDestinyPresentationForSession !== "function") {
        throw new TypeError("buildSessionStampedAddBallsPresentationForSession requires buildDestinyPresentationForSession");
    }
    const getCurrentSessionFileTime = typeof options.getCurrentSessionFileTime === "function"
        ? options.getCurrentSessionFileTime
        : () => 0;
    const presentation = options.buildDestinyPresentationForSession(session, refreshedEntities, stamp, { nowMs });
    const presentationEntities = presentation && Array.isArray(presentation.entities)
        ? presentation.entities
        : [];
    const presentationRawSimTimeMs = presentation && presentation.rawSimTimeMs;
    const simFileTime = getCurrentSessionFileTime(session, presentationRawSimTimeMs);
    const containsOwnerLaunchedMissiles = typeof options.containsOwnerLaunchedMissiles === "function"
    ...
    const { updates } = buildAddBallsPresentationUpdates({
        ...
        buildActiveSpecialFxReplayUpdates: options.buildActiveSpecialFxReplayUpdates,
    });
    return {
        updates,
        sendOptions: {
...
//# sourceMappingURL=addBalls.js.map