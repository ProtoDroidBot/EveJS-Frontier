...
const { advanceDestinyStamp, hasDestinyStamp, normalizeDestinyStamp, } = require("../delivery/stamps");
const { buildFollowingTeardownSendOptions, buildSameSceneShipHandoffSendOptions, } = require("../delivery/sameSceneShipHandoff");
const { DESTINY_CONTRACTS, } = require("../authority/destinyContracts");
const { requireEntityID, } = require("../identity/entityID");
const { buildOnSlimItemChangePayload, buildOnSpecialFXPayload, buildSetBallAgilityPayload, buildSetBallInteractivePayload, buildSetBallPositionPayload, buildSetMaxSpeedPayload, buildStopPayload, } = require("../stream/actions");
...
const SHIP_BOARDING_FOLLOWING_REMOVAL_PROVENANCE = Symbol("destiny.shipBoardingFollowingRemovalProvenance");
...
function getEntityMaxSpeed(entity) {
    return toFiniteNumber(entity && (entity.maxVelocity ?? entity.maxSpeed), 0);
}
function getEntityAgility(entity) {
    return toFiniteNumber(entity && (entity.inertia ?? entity.agility), 1);
}
...
function prepareHandoffInputs(options = {}) {
    ...
    if (typeof options.buildSlimItemObject !== "function") {
        throw new TypeError("same-scene ship handoff requires buildSlimItemObject");
    }
    ...
    const previousID = requireEntityID(previousEntity.itemID, "previousEntity.itemID");
    const boardedID = requireEntityID(boardedEntity.itemID, "boardedEntity.itemID");
...
function buildPlan(stamp, payloads, options = {}) {
    const includesFollowingRemoval = options.followingRemovalEntityID !== undefined;
    ...
    if (includesFollowingRemoval) {
        try {
            ...
            throw new TypeError("same-scene ship handoff actions must be cloneable wire values");
        }
    }
    ...
    if (includesFollowingRemoval) {
        ...
        followingRemoval = buildFollowingShipBoardingRemovalPlan(stamp, options.followingRemovalEntityID, sendOptions);
    ...
    if (followingRemoval) {
        ...
        Object.defineProperty(plan, SHIP_BOARDING_FOLLOWING_REMOVAL_PROVENANCE, {
            configurable: false,
            enumerable: false,
...
function buildFollowingShipBoardingRemovalPlan(handoffStamp, entityID, handoffSendOptions) {
    if (!hasDestinyStamp(handoffStamp)) {
        return null;
    }
    const exactEntityID = requireEntityID(entityID, "followingRemoval.entityID");
...
function resolveSameSceneShipBoardingFollowingRemoval(plan, acceptedHandoffStamp) {
    const provenance = plan && plan[SHIP_BOARDING_FOLLOWING_REMOVAL_PROVENANCE];
    return provenance
        ? buildFollowingShipBoardingRemovalPlan(acceptedHandoffStamp, provenance.entityID, provenance.handoffSendOptions)
        : null;
}
/**
...
function buildSameSceneShipBoardingHandoffPlan(options = {}) {
    ...
    const { boardedEntity, boardedID, boardedSlim, previousEntity, previousID, previousSlim, stamp, } = inputs;
    return buildPlan(stamp, [
        ...
        followingRemovalEntityID: options.includeFollowingRemoval === true
            ? previousID
            : undefined,
    });
}
...
function buildSameSceneShipEjectHandoffPlan(options = {}) {
    ...
    const { boardedEntity: capsuleEntity, boardedID: capsuleID, boardedSlim: capsuleSlim, previousEntity: abandonedShipEntity, previousID: abandonedShipID, previousSlim: abandonedShipSlim, stamp, } = inputs;
    const capsulePosition = capsuleEntity.position ||
    ...
    return buildPlan(stamp, [
        ...
        buildSetBallAgilityPayload(abandonedShipID, getEntityAgility(abandonedShipEntity)),
        buildSetMaxSpeedPayload(abandonedShipID, getEntityMaxSpeed(abandonedShipEntity)),
...
//# sourceMappingURL=shipBoarding.js.map