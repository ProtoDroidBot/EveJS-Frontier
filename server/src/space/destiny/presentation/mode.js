...
const { buildNonEnablingMassDemotionAction, buildOnDockingAcceptedPayload, buildOnSpecialFXPayload, buildSetBallFreePayload, buildSetBallPositionPayload, buildSetBallVelocityPayload, buildSetSpeedFractionPayload, buildStopPayload, } = require("../stream/actions");
...
function resolveModeStamp(options = {}) {
    ...
    return normalizeDestinyStamp(typeof options.getNextDestinyStamp === "function"
...
function buildOrdinaryDockingStopPresentationUpdates(entity, stampOverride = 0) {
    if (!entity) {
        return [];
    }
    const stamp = normalizeDestinyStamp(stampOverride, 0);
...
function buildStructureDockingTransitionUpdates(entity, stampOverride = 0, options = {}) {
    if (!entity) {
        return [];
    }
    const stamp = normalizeDestinyStamp(stampOverride, 0);
    const cloakEffectGuid = options.cloakEffectGuid || DEFAULT_STRUCTURE_DOCKING_CLOAK_EFFECT_GUID;
    const tetherEffectGuid = options.tetherEffectGuid || DEFAULT_STRUCTURE_DOCKING_TETHER_EFFECT_GUID;
    ...
    return [
        ...
        {
            stamp,
            payload: buildOnSpecialFXPayload(entity.itemID, tetherEffectGuid, tetherEffectOptions),
        },
        {
            stamp,
...
function buildDockingAcceptedNotificationPayload(shipPosition, stationPosition, stationID) {
    return buildOnDockingAcceptedPayload(shipPosition, stationPosition, stationID);
}
function buildSetBallFreePresentationUpdates(entityID, isFree, stampOverride = 0) {
    const stamp = normalizeDestinyStamp(stampOverride, 0);
    return [{
            stamp,
...
function buildModePresentationUpdates(options = {}) {
    ...
    const toInt = typeof options.toInt === "function"
        ? options.toInt
        : (value, fallback = 0) => {
            const numeric = Number(value);
            return Number.isFinite(numeric) ? Math.trunc(numeric) : fallback;
        };
    const magnitude = typeof options.magnitude === "function"
        ? options.magnitude
        : defaultMagnitude;
    const getCommandDirection = typeof options.getCommandDirection === "function"
    ...
    switch (entity.mode) {
        case "GOTO":
            updates.push({
                stamp: modeStamp,
                payload: destiny.buildGotoDirectionPayload(entity.itemID, getCommandDirection(entity, entity.direction)),
            });
            break;
        case "FOLLOW":
            ...
            if (hasUsableModeTarget(entity.targetEntityID)) {
                updates.push({
                    stamp: modeStamp,
                    payload: destiny.buildFollowBallPayload(entity.itemID, entity.targetEntityID, entity.followRange),
                });
            }
            break;
        case "ORBIT":
            if (hasUsableModeTarget(entity.targetEntityID)) {
                updates.push({
                    stamp: modeStamp,
                    payload: destiny.buildOrbitPayload(entity.itemID, entity.targetEntityID, entity.orbitDistance),
                });
            }
            break;
        case "WARP":
            if (entity.warpState) {
                if (entity.pendingWarp &&
                    ...
                    const prepareUpdate = options.buildWarpPrepareCommandUpdate(entity, modeStamp, entity.warpState);
                    if (prepareUpdate) {
                    ...
                    if (typeof buildWarpAcquireUpdates === "function") {
                        const warpUpdates = buildWarpAcquireUpdates(entity, entity.warpState, modeStamp);
