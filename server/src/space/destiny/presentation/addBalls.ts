...
function buildMissileLaunchActionUpdates(options: Record<string, any> = {}) {
  const entities = Array.isArray(options.entities) ? options.entities : [];
...
function buildAddBallsPresentationUpdates(options: Record<string, any> = {}) {
  const stamp = normalizeDestinyStamp(options.stamp, 0);
  const entities = Array.isArray(options.entities) ? options.entities : [];
  if (typeof options.buildAddBalls2Payload !== "function") {
    throw new TypeError(
  ...
  const addBallsUpdate: Record<string, any> = {
    stamp,
    payload: options.buildAddBalls2Payload(
      stamp,
      entities,
...
function buildSessionStampedAddBallsPresentationForSession(options: Record<string, any> = {}) {
