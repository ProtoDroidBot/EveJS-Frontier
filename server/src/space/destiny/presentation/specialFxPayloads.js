...
function buildSingleSpecialFxPresentationUpdates(options = {}) {
    return [buildPresentationUpdate(options.stamp, buildOnSpecialFXPayload(options.entityID, options.guid, options.specialFxOptions))];
}
function buildDbuffPresentationUpdates(options = {}) {
    return [buildPresentationUpdate(options.stamp, buildOnDbuffUpdatedPayload(options.entityID, options.dbuffState))];
}
...
function buildSlimItemPresentationUpdates(options = {}) {
    ...
    return [buildPresentationUpdate(options.stamp, buildOnSlimItemChangePayload(options.entityID, options.slimItem))];
}
function buildStructureLifecyclePresentationUpdates(options = {}) {
    const updates = [];
    if (options.includeRepairCompleteFx === true) {
        updates.push(buildPresentationUpdate(options.stamp, buildOnSpecialFXPayload(options.entityID, options.repairCompleteFxGuid, { active: false })), buildPresentationUpdate(options.stamp, buildOnSpecialFXPayload(options.entityID, options.ecmBurstFxGuid, { active: false })));
    }
    updates.push(buildPresentationUpdate(options.stamp, buildOnDamageStateChangePayload(options.entityID, options.damageState)));
    ...
    if (options.slimItem) {
        updates.push(buildPresentationUpdate(options.stamp, buildOnSlimItemChangePayload(options.entityID, options.slimItem)));
    }
    return updates;
}
function buildCloakBallPresentationUpdates(options = {}) {
    return [buildPresentationUpdate(options.stamp, buildCloakBallPayload(options.entityID, options.cloakMode, options.uncloakRange))];
}
function buildOwnerCloakActivationPresentationUpdates(options = {}) {
    ...
    updates.push(buildPresentationUpdate(options.stamp, buildOnSpecialFXPayload(options.entityID, options.moduleFxGuid, options.moduleFxOptions)), buildPresentationUpdate(options.stamp, buildSetMaxSpeedPayload(options.entityID, options.maxSpeed)));
    return updates;
}
function buildOwnerUncloakPresentationUpdates(options = {}) {
    ...
    if (options.includeMaxSpeed !== false) {
        updates.push(buildPresentationUpdate(options.stamp, buildSetMaxSpeedPayload(options.entityID, options.maxSpeed)));
    }
    return updates;
}
function buildCloakDeliveryPresentationUpdates(options = {}) {
    ...
    if (options.includeCloakBall !== false) {
        updates.push(buildPresentationUpdate(options.stamp, buildCloakBallPayload(options.entityID, options.cloakMode, options.uncloakRange)));
    }
...
function buildTetherPresentationUpdates(options = {}) {
    return [
        buildPresentationUpdate(options.stamp, buildOnSpecialFXPayload(options.entityID, options.tetherFxGuid, options.tetherFxOptions)),
        buildPresentationUpdate(options.stamp, buildSetBallMassPayload(options.entityID, options.mass)),
    ];
}
...
function createSpecialFxPayloadPresentation(deps = {}) {
    const { destiny, splitSpecialFxGuids, } = deps;
    const isEntityUsingNpcShipHardpointPresentation = typeof deps.isEntityUsingNpcShipHardpointPresentation === "function"
        ? deps.isEntityUsingNpcShipHardpointPresentation
        : () => false;
    const isOffensiveWeaponFamily = typeof deps.isOffensiveWeaponFamily === "function"
        ? deps.isOffensiveWeaponFamily
        : () => false;
    const shouldUseShipKeyedSpecialFxModuleBinding = typeof deps.shouldUseShipKeyedSpecialFxModuleBinding === "function"
        ? deps.shouldUseShipKeyedSpecialFxModuleBinding
        : null;
    const shouldUseFittingSlotKeyedSpecialFxModuleBinding = typeof deps.shouldUseFittingSlotKeyedSpecialFxModuleBinding === "function"
        ? deps.shouldUseFittingSlotKeyedSpecialFxModuleBinding
        : null;
    if (!destiny || typeof destiny.buildOnSpecialFXPayload !== "function") {
        throw new TypeError("special FX presentation requires buildOnSpecialFXPayload");
    }
    ...
    function usesShipKeyedSpecialFxModuleBinding(visibilityEntity, options = {}) {
        ...
        if (shouldUseShipKeyedSpecialFxModuleBinding) {
            return shouldUseShipKeyedSpecialFxModuleBinding(visibilityEntity, options) === true;
        }
    ...
    function resolveSpecialFxOptionsForEntity(shipID, options = {}, visibilityEntity = null) {
        ...
        if (usesShipKeyedSpecialFxModuleBinding(visibilityEntity, options)) {
            ...
            return {
                ...
                moduleID: normalizeEntityID(shipID) ||
        ...
        if (visibilityEntity &&
            ...
            shouldUseFittingSlotKeyedSpecialFxModuleBinding(visibilityEntity, options) === true) {
    ...
    function buildSpecialFxPayloadsForEntity(shipID, guid, options = {}, visibilityEntity = null) {
        const resolvedOptions = resolveSpecialFxOptionsForEntity(shipID, options, visibilityEntity);
        ...
        const payloads = guidEntries.map((guidEntry) => (destiny.buildOnSpecialFXPayload(shipID, guidEntry, resolvedOptions)));
        return {
            resolvedOptions,
            payloads,
        };
...
//# sourceMappingURL=specialFxPayloads.js.map