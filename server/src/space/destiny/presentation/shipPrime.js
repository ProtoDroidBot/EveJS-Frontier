...
function valuesDiffer(left, right, epsilon = 1e-9) {
    return Math.abs(toFiniteNumber(left, 0) - toFiniteNumber(right, 0)) > epsilon;
}
...
function getUndockCommandDirection(entity) {
    ...
    if (entity && entity.targetPoint && entity.position) {
        return normalizeVector({
            x: toFiniteNumber(entity.targetPoint.x, 0) -
                toFiniteNumber(entity.position.x, 0),
            y: toFiniteNumber(entity.targetPoint.y, 0) -
                toFiniteNumber(entity.position.y, 0),
            z: toFiniteNumber(entity.targetPoint.z, 0) -
                toFiniteNumber(entity.position.z, 0),
        }, fallback);
    }
    return normalizeVector(fallback);
}
function buildUndockBootstrapMovementUpdates(entity, stampOverride = 0) {
    ...
    const updates = [
        {
            stamp,
            payload: buildOnSpecialFXPayload(entity.itemID, UNDOCK_BOOTSTRAP_EFFECT_GUID, {
                start: true,
                active: false,
    ...
    updates.push({
        stamp,
        payload: buildGotoDirectionPayload(entity.itemID, getUndockCommandDirection(entity)),
    });
    return updates;
}
function buildShipPrimeUpdates(entity, stampOverride = 0) {
    ...
    if (toFiniteNumber(entity.maxAngularSpeed, 0) > 0) {
        updates.push({
            stamp,
            payload: buildSetMaxAngularSpeedPayload(entity.itemID, entity.maxAngularSpeed),
        });
    }
    if (toFiniteNumber(entity.angularAgility, 0) > 0) {
        updates.push({
            stamp,
            payload: buildSetBallAngularAgilityPayload(entity.itemID, entity.angularAgility),
        });
    }
    return updates;
}
...
function buildShipPrimeDeltaUpdates(entity, previousState = {}, stampOverride = 0) {
    if (!entity || entity.kind !== "ship") {
        return [];
    }
    const stamp = normalizeDestinyStamp(stampOverride, 0);
...
//# sourceMappingURL=shipPrime.js.map