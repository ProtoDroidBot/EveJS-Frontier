...
const { DESTINY_BOOTSTRAP_DELIVERY_ADDBALLS2, } = require("../bootstrap/staticBootstrap");
const { BALL_FLAG, } = require("../constants");
...
function resolveSiteEnvironmentPropDestinyPresentation(source = {}, typePresentation = null) {
    const safeSource = source && typeof source === "object" ? source : {};
    const safeType = typePresentation && typeof typePresentation === "object"
        ? typePresentation
        : {};
    const motionFields = {
    ...
    const hasExplicitTail = hasNonEmptyValue(safeSource, "destinyCollisionTail");
    const hasExplicitMode = hasNonEmptyValue(safeSource, "destinyBallMode") ||
        hasNonEmptyValue(safeSource, "destinyMode");
    const explicitlyFree = safeSource.destinyForceFree === true ||
        safeSource.destinyIsFree === true;
    const explicitlyNonFree = safeSource.destinyForceFree === false ||
        safeSource.destinyIsFree === false;
    const hasExplicitFreeFlag = configuredFlags !== null &&
        (configuredFlags & BALL_FLAG.IS_FREE) !== 0;
    const useFreePresentation = !hasExplicitTail && (explicitlyFree ||
        hasExplicitFreeFlag ||
        (!explicitlyNonFree &&
            !hasExplicitMode &&
    ...
    if (!useFreePresentation) {
        const presentation = {
            destinyBallMode: normalizeMode(safeSource.destinyBallMode || safeSource.destinyMode, "RIGID"),
    ...
    return appendBootstrapDelivery({
        destinyBallMode: hasExplicitFreeFlag
            ? normalizeMode(safeSource.destinyBallMode || safeSource.destinyMode, "STOP")
            : "STOP",
...
//# sourceMappingURL=siteEnvironment.js.map