...
function shouldReplayActiveSpecialFxForFreshAcquire(effectState, options = {}) {
    ...
    const isOffensiveWeaponFamily = typeof options.isOffensiveWeaponFamily === "function"
        ? options.isOffensiveWeaponFamily
        : () => false;
    const replayableStatefulSelfBuffGuids = options.replayableStatefulSelfBuffGuids instanceof Set
        ? options.replayableStatefulSelfBuffGuids
        : new Set();
    const replayableMiningFx = effectState.miningEffect === true;
    const replayableOffensiveFx = effectState.isGeneric === true &&
        isOffensiveWeaponFamily(effectState.weaponFamily);
    const replayableHostileFx = effectState.hostileModuleEffect === true &&
        typeof effectState.guid === "string" &&
        effectState.guid.trim() !== "";
    const replayableStatefulSelfBuffFx = replayableStatefulSelfBuffGuids.has(String(effectState.guid || ""));
    if (!replayableMiningFx &&
...
function buildReplayPolicyOptions(options, nowMs) {
    return {
        ...
        isSuperweaponFxReplayWindowActive: options.isSuperweaponFxReplayWindowActive,
        nowMs,
        replayableStatefulSelfBuffGuids: options.replayableStatefulSelfBuffGuids,
...
function buildFreshAcquireActiveSpecialFxReplayUpdates(options = {}) {
    ...
    if (typeof options.buildSpecialFxPayloadsForEntity !== "function") {
        throw new TypeError("active special FX replay requires buildSpecialFxPayloadsForEntity");
    }
    const stamp = normalizeDestinyStamp(options.stamp, 0);
    const nowMs = resolveNowMs(options, toFiniteNumber);
    const freshAcquireEntityIDs = new Set(entities
        .map((entity) => normalizeEntityID(entity && entity.itemID))
        .filter((entityID) => entityID !== null));
    const isOffensiveWeaponFamily = typeof options.isOffensiveWeaponFamily === "function"
        ? options.isOffensiveWeaponFamily
        : () => false;
    const resolveSpecialFxRepeatCount = typeof options.resolveSpecialFxRepeatCount === "function"
        ? options.resolveSpecialFxRepeatCount
        : () => null;
    const buildSpecialFxPayloadsForEntity = options.buildSpecialFxPayloadsForEntity;
    const updates = [];
    for (const entity of entities) {
        ...
        for (const effectState of entity.activeModuleEffects.values()) {
            if (!shouldReplayActiveSpecialFxForFreshAcquire(effectState, buildReplayPolicyOptions(options, nowMs))) {
                continue;
            }
            if (effectState.superweaponEffect === true &&
                String(effectState.superweaponFamily || "").toLowerCase() === "lance") {
                const replayTargetID = normalizeEntityID(effectState.superweaponFxTargetID ||
                    effectState.superweaponBeaconID ||
                    effectState.superweaponPrimaryTargetID ||
                    effectState.targetID);
                const targetIsFreshlyAcquired = replayTargetID !== null && freshAcquireEntityIDs.has(replayTargetID);
                if (!targetIsFreshlyAcquired) {
                    continue;
                }
            }
            const superweaponReplayOptions = effectState.superweaponEffect === true &&
                typeof options.buildSuperweaponFreshAcquireFxOptions === "function"
                ? options.buildSuperweaponFreshAcquireFxOptions(effectState, nowMs, options.scene || null)
                : null;
            const payloadResult = buildSpecialFxPayloadsForEntity(entity.itemID, effectState.guid, superweaponReplayOptions || {
...
function buildActiveSpecialFxEffectStateEntriesForSetState(options = {}) {
    ...
    if (typeof options.buildSpecialFxPayloadsForEntity !== "function") {
        throw new TypeError("SetState special FX replay requires buildSpecialFxPayloadsForEntity");
    }
    const nowMs = resolveNowMs(options, toFiniteNumber);
    const isOffensiveWeaponFamily = typeof options.isOffensiveWeaponFamily === "function"
        ? options.isOffensiveWeaponFamily
    ...
    const resolveSpecialFxRepeatCount = typeof options.resolveSpecialFxRepeatCount === "function"
        ...
        : () => null;
    const buildSpecialFxPayloadsForEntity = options.buildSpecialFxPayloadsForEntity;
    const buildGenericModuleSpecialFxGraphicInfo = typeof options.buildGenericModuleSpecialFxGraphicInfo === "function"
        ? options.buildGenericModuleSpecialFxGraphicInfo
        : () => null;
    const buildMiningSpecialFxTimingOptions = typeof options.buildMiningSpecialFxTimingOptions === "function"
    ...
    for (const entity of entities) {
        ...
        for (const effectState of entity.activeModuleEffects.values()) {
            ...
            const superweaponOptions = effectState.superweaponEffect === true &&
                typeof options.buildSuperweaponFreshAcquireFxOptions === "function"
                ? options.buildSuperweaponFreshAcquireFxOptions(effectState, nowMs, options.scene || null)
                : null;
            const payloadResult = buildSpecialFxPayloadsForEntity(entity.itemID, effectState.guid, superweaponOptions || {
                moduleID: effectState.moduleID,
                moduleTypeID: effectState.typeID,
                targetID: effectState.targetID || null,
                chargeTypeID: effectState.chargeTypeID || null,
                weaponFamily: String(effectState.weaponFamily || ""),
                isOffensive: effectState.hostileModuleEffect === true ||
                    isOffensiveWeaponFamily(effectState.weaponFamily),
                start: true,
                active: true,
                duration: effectState.durationMs,
...
//# sourceMappingURL=activeSpecialFxReplay.js.map