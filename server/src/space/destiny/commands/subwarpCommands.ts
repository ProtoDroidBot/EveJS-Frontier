...
function createMovementSubwarpCommands(deps: Record<string, any> = {}) {
  const {
    addVectors,
    armMovementTrace,
  ...
  function gotoPointEntity(runtime, entity, point, options: Record<string, any> = {}) {
  ...
  const commands = {
    gotoDirection(runtime, session, direction, options: Record<string, any> = {}) {
    ...
    gotoPoint(runtime, session, point, options: Record<string, any> = {}) {
      const entity = runtime.getShipEntityForSession(session);
      if (
        !entity ||
        entity.mode === "WARP" ||
    ...
    followShipEntity(runtime, entityOrID, targetEntityID, range = 0, options: Record<string, any> = {}) {
      const entity =
    ...
    followBall(runtime, session, targetEntityID, range = 0, options: Record<string, any> = {}) {
      const entity = runtime.getShipEntityForSession(session);
    ...
    orbitShipEntity(runtime, entityOrID, targetEntityID, distanceValue = 0, options: Record<string, any> = {}) {
      const entity =
        typeof entityOrID === "object" && entityOrID !== null
          ? entityOrID
          : runtime.getEntityByID(entityOrID);
    ...
    orbit(runtime, session, targetEntityID, distanceValue = 0, options: Record<string, any> = {}) {
      const entity = runtime.getShipEntityForSession(session);
      if (hasPendingPilotWarpLanding(session, entity)) {
        return false;
      }
