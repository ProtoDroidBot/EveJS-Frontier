...
function applyTractorStopCommand({
  ...
  return {
    applied: true,
    buildRestoreSpeedUpdates({ stamp }: Record<string, any> = {}) {
      return [
        {
          stamp,
    ...
    buildStopUpdates({ stamp, includePosition = false }: Record<string, any> = {}) {
      const updates: any[] = [];
...
function applyTractorActivationCommand({
  ...
  return {
    applied: true,
    buildPresentationUpdates({ stamp }: Record<string, any> = {}) {
      return [
        {
          stamp,
...
function applyTractorSettleCommand({
  ...
  return {
    applied: true,
    buildPresentationUpdates({ primeSpeed = false, sendGotoPoint = false }: Record<string, any> = {}) {
      const updates: any[] = [];
      if (primeSpeed === true) {
        updates.push({
          stamp,
...
function applyTractorPullCommand({
  ...
  return {
    ...
    buildRetargetUpdates({ primeSpeed = false, sendGotoPoint = false }: Record<string, any> = {}) {
      const updates: any[] = [];
      if (primeSpeed === true) {
        updates.push({
          stamp,
    ...
    buildPositionUpdates({ position = targetEntity.position }: Record<string, any> = {}) {
      return [{
        stamp,
