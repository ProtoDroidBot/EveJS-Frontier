...
const { serializeDestinyDirectionHeading, } = require("../protocol/direction");
const { resolveGotoCommandSyncState, } = require("../delivery/deliveryPolicy.js");
const { resolveDestinyAuthorityCommandTuple, snapshotDestinyAuthorityState, updateDestinyAuthorityState, } = require("../authority/destinySessionState.js");
const { advanceDestinyStamp, getDestinyStampForwardDistance, hasDestinyStamp, isDestinyStampAfter, } = require("../delivery/stamps");
function createMovementSubwarpCommands(deps = {}) {
    const { addVectors, armMovementTrace, buildDirectedMovementUpdates, buildPointMovementUpdates, buildPerpendicular, clearTrackingState, cloneVector, crossProduct, directionsNearlyMatch, getShipDockingDistanceToStation, getTargetMotionPosition, hasPendingPilotWarpLanding = () => false, logMovementDebug, normalizeVector, persistShipEntity, roundNumber, scaleVector, subtractVectors, summarizeVector, toFiniteNumber, toInt, DEFAULT_UP, OWNER_PENDING_GOTO_DUPLICATE_ALIGNMENT, } = deps;
    function gotoPointEntity(runtime, entity, point, options = {}) {
        ...
        const commandDirection = normalizeVector(subtractVectors(commandPoint, entity.position), entity.direction);
        const speedFractionChanged = entity.speedFraction <= 0 || entity.mode === "STOP";
        ...
        runtime.dispatchConfiguredSubwarpMovement(entity, (stamp) => buildPointMovementUpdates(entity, commandPoint, speedFractionChanged, stamp), now, options);
    ...
    function isCloakedFollowOrbitTarget(target) {
        return toInt(target && (target.isCloaked ??
            target.cloakMode ??
            target.cloaked), 0) > 0;
    }
    ...
    const commands = {
        gotoDirection(runtime, session, direction, options = {}) {
            ...
            const liveOwnerSessionStamp = runtime.getCurrentSessionDestinyStamp(session, now);
            ...
            const pendingOwnerMovementAnchorStamp = pendingOwnerMovementTuple.anchorStamp;
            const pendingOwnerMovementRawDispatchStamp = pendingOwnerMovementTuple.rawDispatchStamp;
            const trackedOwnerCommandDirection = pendingOwnerMovementTuple.direction;
            const recentOwnerMovementRawDispatchDelta = hasDestinyStamp(pendingOwnerMovementRawDispatchStamp)
                ? getDestinyStampForwardDistance(pendingOwnerMovementRawDispatchStamp, currentRawDispatchStamp)
                : null;
            const recentOwnerMovementStillRelevant = session &&
                session._space &&
                ...
                (pendingOwnerMovementStamp === liveOwnerSessionStamp ||
                    isDestinyStampAfter(liveOwnerSessionStamp, pendingOwnerMovementStamp) ||
                    pendingOwnerMovementStamp === advanceDestinyStamp(liveOwnerSessionStamp, -1));
            const pendingOwnerCommandDirection = isDestinyStampAfter(liveOwnerSessionStamp, pendingOwnerMovementStamp) &&
                pendingOwnerMovementAnchorStamp === liveOwnerSessionStamp &&
                trackedOwnerCommandDirection
                ? normalizeVector(trackedOwnerCommandDirection, commandDirection)
                : null;
            const recentOwnerCommandDirection = recentOwnerMovementStillRelevant
                ? normalizeVector(trackedOwnerCommandDirection, commandDirection)
                : pendingOwnerCommandDirection;
            const currentGotoDirection = entity.mode === "GOTO" &&
                entity.targetEntityID === null &&
                entity.targetPoint
                ? normalizeVector(subtractVectors(entity.targetPoint, entity.position), entity.direction)
                : null;
            const speedFractionChanged = entity.speedFraction <= 0 || entity.mode === "STOP";
            ...
            // advanced.
            const ownerDirectEchoLeadOverride = ownerLocallyPredictsHeading
                ? undefined
                : 2;
            const gotoCommandSyncState = resolveGotoCommandSyncState({
                ownerLocallyPredictsHeading,
                speedFractionChanged,
                currentGotoDirectionMatches: Boolean(currentGotoDirection) &&
                    directionsNearlyMatch(currentGotoDirection, commandDirection),
                pendingOwnerCommandDirectionMatches: Boolean(recentOwnerCommandDirection) &&
                    directionsNearlyMatch(recentOwnerCommandDirection, commandDirection, OWNER_PENDING_GOTO_DUPLICATE_ALIGNMENT),
            ...
            const suppressOwnerGotoEchoRecentDuplicate = gotoCommandSyncState.suppressOwnerGotoEchoRecentDuplicate;
            const suppressOwnerGotoEchoSameRawPendingFutureSteer = gotoCommandSyncState.suppressOwnerGotoEchoSameRawPendingFutureSteer;
            const suppressOwnerGotoEcho = suppressOwnerGotoEchoRecentDuplicate ||
                suppressOwnerGotoEchoSameRawPendingFutureSteer;
            const coalescePendingGotoRecentDuplicate = gotoCommandSyncState.isPendingGotoDuplicate &&
                suppressOwnerGotoEchoRecentDuplicate;
            if (gotoCommandSyncState.isPendingGotoDuplicate &&
            ...
            if (suppressOwnerGotoEcho) {
                const updates = buildDirectedMovementUpdates(entity, commandDirection, speedFractionChanged, runtime.getMovementStamp(now));
                if (session && session._space) {
                    ...
                    updateDestinyAuthorityState(session, {
                        ...
                        lastOwnerCommandHeadingHash: serializeDestinyDirectionHeading(commandDirection),
                    });
                }
                logMovementDebug(suppressOwnerGotoEchoSameRawPendingFutureSteer
                    ? "cmd.goto.ownerEchoSuppressedSameRawPendingFutureSteer"
                    : "cmd.goto.ownerEchoSuppressedRecentDuplicate", entity, {
                    commandDirection: summarizeVector(commandDirection),
                    currentRawDispatchStamp,
                    pendingOwnerMovementStamp,
                ...
                runtime.broadcastMovementUpdates(updates, session, options.sendOptions || {});
                runtime.scheduleWatcherMovementAnchor(entity, now, "gotoDirection");
                return true;
            }
            runtime.dispatchConfiguredSubwarpMovement(entity, (stamp) => buildDirectedMovementUpdates(entity, commandDirection, speedFractionChanged, stamp), now, {
                ...options,
        ...
        alignTo(runtime, session, targetEntityID) {
            ...
            const commandDirection = normalizeVector(subtractVectors(alignTargetPosition, entity.position), entity.direction);
            clearTrackingState(entity);
            entity.targetPoint = addVectors(cloneVector(entity.position), scaleVector(commandDirection, 1.0e16));
            ...
            const updates = buildDirectedMovementUpdates(entity, commandDirection, speedFractionChanged, movementStamp);
        ...
        followShipEntity(runtime, entityOrID, targetEntityID, range = 0, options = {}) {
            ...
            const target = runtime.getEntityByID(targetEntityID);
            if (!isFiniteFollowOrbitRange(range) ||
                hasPendingPilotWarpLanding(entity && entity.session, entity) ||
                isInvalidFollowOrbitTarget(entity, target)) {
                return false;
            }
            const now = runtime.getCurrentSimTimeMs();
            const explicitDockingTargetID = (target.kind === "station" || target.kind === "structure") &&
                Number(options.dockingTargetID || 0) === target.itemID
                ? target.itemID
                : null;
            const preservedDockingTargetID = explicitDockingTargetID === null &&
            ...
            logMovementDebug("cmd.follow", entity, {
                ...
                preservedDockingTargetID: preservedDockingTargetID || 0,
                dockPosition: (target.kind === "station" || target.kind === "structure") && target.dockPosition
                    ? summarizeVector(target.dockPosition)
                    : null,
                dockingDistance: (target.kind === "station" || target.kind === "structure")
                    ? roundNumber(getShipDockingDistanceToStation(entity, target))
                    : null,
            });
            runtime.dispatchConfiguredSubwarpMovement(entity, (stamp) => {
                const updates = [
                    {
                        stamp,
                        payload: destiny.buildFollowBallPayload(entity.itemID, target.itemID, entity.followRange),
                    },
                ];
                if (speedFractionChanged) {
                    updates.push({
        ...
        orbitShipEntity(runtime, entityOrID, targetEntityID, distanceValue = 0, options = {}) {
            const entity = typeof entityOrID === "object" && entityOrID !== null
                ...
                : runtime.getEntityByID(entityOrID);
            const target = runtime.getEntityByID(targetEntityID);
            if (!isFiniteFollowOrbitRange(distanceValue) ||
                hasPendingPilotWarpLanding(entity && entity.session, entity) ||
                isInvalidFollowOrbitTarget(entity, target)) {
                return false;
            }
            ...
            const radial = normalizeVector(subtractVectors(entity.position, target.position), buildPerpendicular(entity.direction));
            clearTrackingState(entity);
            entity.mode = "ORBIT";
            entity.targetEntityID = target.itemID;
            entity.orbitDistance = normalizedDistance;
            entity.orbitNormal = normalizeVector(crossProduct(radial, DEFAULT_UP), buildPerpendicular(radial));
            entity.orbitSign = 1;
            ...
            runtime.dispatchConfiguredSubwarpMovement(entity, (stamp) => {
                const updates = [
                    {
                        stamp,
                        payload: destiny.buildOrbitPayload(entity.itemID, target.itemID, entity.orbitDistance),
                    },
                ];
                if (speedFractionChanged) {
                    updates.push({
        ...
        orbit(runtime, session, targetEntityID, distanceValue = 0, options = {}) {
            ...
            return runtime.orbitShipEntity(entity, targetEntityID, distanceValue, options);
        },
    };
...
//# sourceMappingURL=subwarpCommands.js.map