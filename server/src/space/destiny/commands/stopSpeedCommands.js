...
const { DESTINY_STAMP_MAX_FORWARD_LEAD, hasDestinyStamp, normalizeDestinyStamp, selectFurthestDestinyStamp, } = require("../delivery/stamps");
function resolveBoundedStopStamp(anchorStamp, candidateStamps, maximumLead) {
    const candidates = (Array.isArray(candidateStamps) ? candidateStamps : [candidateStamps]).filter(hasDestinyStamp);
    const anchor = hasDestinyStamp(anchorStamp)
        ? normalizeDestinyStamp(anchorStamp)
        : candidates.length > 0
            ? normalizeDestinyStamp(candidates[0])
            : 0;
    const normalizedMaximumLead = Math.min(DESTINY_STAMP_MAX_FORWARD_LEAD, Math.max(0, Math.trunc(Number(maximumLead) || 0)));
    return selectFurthestDestinyStamp(anchor, candidates, normalizedMaximumLead);
}
function createMovementStopSpeedCommands(deps = {}) {
    const { addVectors, armMovementTrace, buildWarpCompletionUpdates, clamp, clearTrackingState, cloneVector, forgetNativeSubwarpEntity, logMovementDebug, magnitude, normalizeVector, persistShipEntity, roundNumber, scaleVector, subtractVectors, MICHELLE_HELD_FUTURE_DESTINY_LEAD, MAX_SUBWARP_SPEED_FRACTION, } = deps;
    return {
        setSpeedFraction(runtime, session, fraction) {
            ...
            if (entity.speedFraction > 0 && entity.mode === "STOP") {
                entity.mode = "GOTO";
                entity.targetPoint = addVectors(cloneVector(entity.position), scaleVector(entity.direction, 1.0e16));
            }
            ...
            runtime.broadcastPilotCommandMovementUpdates(session, [
                {
                    stamp,
        ...
        stopShipEntity(runtime, entityOrID, options = {}) {
            ...
            if (entity.mode === "WARP" && entity.warpState && !entity.pendingWarp) {
                if (options.allowSessionlessWarpAbort === true &&
                    entity.sessionlessWarpIngress) {
                    const now = runtime.getCurrentSimTimeMs();
                    const arrivalPoint = cloneVector(entity.sessionlessWarpIngress.targetPoint, entity.warpState && entity.warpState.targetPoint);
                    entity.direction = normalizeVector(subtractVectors(arrivalPoint, entity.position), entity.direction);
                    ...
                    runtime.broadcastPilotCommandMovementUpdates(entity.session || null, buildWarpCompletionUpdates(entity, stamp), now);
                    runtime.scheduleWatcherMovementAnchor(entity, now, "stopSessionlessWarpAbort");
                    return true;
                }
            ...
            const wasAlreadyStopped = entity.mode === "STOP" &&
                entity.speedFraction <= 0 &&
            ...
            const historySafeStopStamp = superweaponStop
                ? runtime.getHistorySafeSessionDestinyStamp(entity.session || null, now, MICHELLE_HELD_FUTURE_DESTINY_LEAD, MICHELLE_HELD_FUTURE_DESTINY_LEAD)
                : null;
            const currentStopStamp = entity.session &&
            ...
            const presentedStopStamp = entity.session &&
                typeof runtime.getCurrentPresentedSessionDestinyStamp === "function"
                ? runtime.getCurrentPresentedSessionDestinyStamp(entity.session, now, MICHELLE_HELD_FUTURE_DESTINY_LEAD)
                : null;
            const stamp = superweaponStop
                ? resolveBoundedStopStamp(currentStopStamp, [historySafeStopStamp, presentedStopStamp], MICHELLE_HELD_FUTURE_DESTINY_LEAD)
                : runtime.getHistorySafeSessionDestinyStamp(entity.session || null, now, 1);
            ...
            runtime.broadcastPilotCommandMovementUpdates(entity.session || null, updates, now, superweaponStop
                ? {
                    ownerDirectEchoLeadOverride: MICHELLE_HELD_FUTURE_DESTINY_LEAD,
                }
                : {});
...
//# sourceMappingURL=stopSpeedCommands.js.map