...
function createMovementWarpCommands(deps: Record<string, any> = {}) {
  const {
    activatePendingWarp,
    armMovementTrace,
  ...
  return {
    warpToEntity(runtime, session, targetEntityID, options: Record<string, any> = {}) {
    ...
    warpToPoint(runtime, session, point, options: Record<string, any> = {}) {
      const entity = runtime.getShipEntityForSession(session);
      const landingPending = Boolean(
        entity && hasPendingPilotWarpLanding(session, entity),
      );
    ...
    warpDynamicEntityToPoint(runtime, entityOrID, point, options: Record<string, any> = {}) {
      const entity =
    ...
    forceStartPendingWarp(runtime, entityOrID, options: Record<string, any> = {}) {
      const entity =
        typeof entityOrID === "object" && entityOrID !== null
          ? entityOrID
          : runtime.getEntityByID(entityOrID);
    ...
    startSessionlessWarpIngress(runtime, entityOrID, point, options: Record<string, any> = {}) {
      const entity =
        typeof entityOrID === "object" && entityOrID !== null
          ? entityOrID
          : runtime.getEntityByID(entityOrID);
