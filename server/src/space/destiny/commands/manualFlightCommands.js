...
function createMovementManualFlightCommands(deps = {}) {
    const { addVectors, armMovementTrace, clamp, clearTrackingState, cloneVector, normalizeVector, persistShipEntity, roundNumber, scaleVector, summarizeVector, toFiniteNumber, DEFAULT_RIGHT, } = deps;
    ...
    function applyManualAxis(runtime, session, axis, requestedValue) {
        ...
        const currentYawRate = clamp(toFiniteNumber(entity.manualYawRate, 0), -1, 1);
        const currentStrafingThrust = cloneVector(entity.manualStrafingThrust || { x: 0, y: 0, z: 0 });
        ...
        entity.mode = "GOTO";
        entity.targetPoint = addVectors(cloneVector(entity.position), scaleVector(normalizeVector(entity.direction, DEFAULT_RIGHT), 1.0e16));
        ...
        runtime.broadcastPilotCommandMovementUpdates(session, [{ stamp, payload }], now);
        runtime.scheduleWatcherMovementAnchor(entity, now, axis === "pitch" ? "setPitch" : "setYawRate");
        return true;
    }
    function applyStrafingThrust(runtime, session, requestedThrust) {
        ...
        const source = Array.isArray(requestedThrust)
            ...
            : requestedThrust && typeof requestedThrust === "object"
                ...
                : [];
        const agilitySeconds = Math.max(0.05, toFiniteNumber(entity.agilitySeconds, 0.05));
        const maximumStrafeAcceleration = Math.max(0, toFiniteNumber(entity.maxVelocity, 0) / agilitySeconds) * 0.2;
        const thrust = {
            x: clamp(toFiniteNumber(source[0], 0), -maximumStrafeAcceleration, maximumStrafeAcceleration),
            y: clamp(toFiniteNumber(source[1], 0), -maximumStrafeAcceleration, maximumStrafeAcceleration),
            z: 0,
        };
        const currentPitch = Number.isFinite(Number(entity.manualPitch))
            ? clamp(toFiniteNumber(entity.manualPitch, 0), -1, 1)
        ...
        runtime.scheduleWatcherMovementAnchor(entity, now, "setStrafingThrust");
        return true;
    }
    return {
...
//# sourceMappingURL=manualFlightCommands.js.map