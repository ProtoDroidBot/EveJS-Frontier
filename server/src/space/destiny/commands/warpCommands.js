...
const deadspaceWarpPolicy = require(path.join(__dirname, "../../../services/dungeon/deadspaceWarpPolicy.js"));
const { resolveDungeonSiteWarpInMinimumRange, resolveDungeonWarpStopDistance, } = require(path.join(__dirname, "../../../services/dungeon/dungeonWarpSafety.js"));
const { isFrontierProfile, } = require("../../../services/ship/destinyCompatibility.js");
const { DESTINY_CONTRACTS, } = require("../authority/destinyContracts.js");
const { hasDestinyStamp, normalizeDestinyStamp, selectFurthestDestinyStamp, } = require("../delivery/stamps");
const { entityIDsEqual, normalizePersistentEntityID, } = require("../identity/entityID");
const { isNativeWarpDistanceTooShort, } = require("../simulation/warp");
...
function getActiveWarpDestinationStaticInstanceID(entity) {
    return normalizePersistentEntityID(entity && entity.pendingWarp
...
function getActiveWarpDestinationDungeonRoomKey(entity) {
    return String(entity && entity.pendingWarp
...
function isRedundantWarpToPoint(entity, point, destinationStaticInstanceID, destinationDungeonRoomKey = null, destinationDungeonSiteID = null) {
    return (vectorsNearlyEqual(getActiveWarpRawDestination(entity), point) &&
        destinationInstanceIDsEqual(getActiveWarpDestinationStaticInstanceID(entity), destinationStaticInstanceID) &&
        getActiveWarpDestinationDungeonRoomKey(entity) === (String(destinationDungeonRoomKey || "").trim() || null) &&
        destinationInstanceIDsEqual(getActiveWarpDestinationDungeonSiteID(entity), normalizePersistentEntityID(destinationDungeonSiteID)));
}
function resolveCommandDestinationStaticInstanceID(runtime, session, options = {}) {
    const hasRequestedRoomKey = Object.prototype.hasOwnProperty.call(options, "destinationDungeonRoomKey");
    const hasRequestedSiteID = Object.prototype.hasOwnProperty.call(options, "destinationDungeonSiteID");
    if (!Object.prototype.hasOwnProperty.call(options, "destinationStaticInstanceID")) {
    ...
    if (hasContextResolver) {
        const context = runtime.resolveAuthorizedDestinationDungeonContext(session, requestedInstanceID, {
            ...(hasRequestedRoomKey
        ...
        authorizedInstanceID = normalizePersistentEntityID(context && context.instanceID);
        ...
        authorizedInstanceID = normalizePersistentEntityID(runtime.resolveAuthorizedDestinationStaticInstanceID(session, requestedInstanceID));
    }
    if (authorizedInstanceID === null ||
...
function isWarpBlockedByCloak(runtime, entity) {
    if (typeof runtime.isEntityWarpBlockedByCloak !== "function") {
        throw new Error("Scene is missing isEntityWarpBlockedByCloak, so the cloak/warp rule cannot be enforced.");
    }
    return runtime.isEntityWarpBlockedByCloak(entity);
}
function createMovementWarpCommands(deps = {}) {
    const { activatePendingWarp, armMovementTrace, buildDirectedMovementUpdates, buildOfficialWarpReferenceProfile, buildPendingWarpRequest, buildPreparingWarpState, buildSessionlessWarpIngressState, buildWarpPrepareDispatch, buildWarpStartUpdates, checkWarpCapacitorAvailability = () => ({ success: true }), checkWarpFuelAvailability = () => ({ success: true, skipped: true }), clearTrackingState, cloneVector, consumeWarpCapacitor = () => ({ success: true }), deactivateWarpUnsafeActiveModulesForWarpStart, getClientParityWarpInPoint, getStargateWarpLandingPoint, getStationWarpTargetPosition, getTargetMotionPosition, getWatcherWarpStartStamp, getWarpStopDistanceForTarget, gotoPointEntity, findActiveWarpDisruptorForEntity, hasPendingPilotWarpLanding = () => false, initializeSessionlessWarpDestination, isReadyForDestiny, logMovementDebug, logWarpDebug, materializeDungeonWarpDestination, normalizeVector, prewarmStartupControllersForWarpDestination, primePilotWarpActivationState, persistShipEntity, resolveStargateWarpTarget, subtractVectors, summarizePendingWarp, tagUpdatesRequireExistingVisibility, toFiniteNumber = (value, fallback = 0) => {
        const numeric = Number(value);
        return Number.isFinite(numeric) ? numeric : fallback;
    }, toInt, DESTINY_STAMP_INTERVAL_MS, MIN_WARP_DISTANCE_METERS, PILOT_WARP_ACTIVATION_DELAY_DESTINY_TICKS, MICHELLE_POST_HELD_FUTURE_DESTINY_LEAD, } = deps;
    const serviceMinimumWarpDistanceMeters = Math.max(0, toFiniteNumber(MIN_WARP_DISTANCE_METERS, 150_000));
    function routeNativeGotoFallback(runtime, entity, pendingWarp, now, options = {}) {
        if (!entity ||
            !pendingWarp ||
        ...
        const gotoDestination = pendingWarp.destinationDungeonSiteID !== null &&
            pendingWarp.destinationDungeonSiteID !== undefined &&
            pendingWarp.targetPoint
            ? pendingWarp.targetPoint
            : pendingWarp.rawDestination;
        const routed = gotoPointEntity(runtime, entity, gotoDestination, {
            ...options,
            nowMs: now,
    ...
    return {
        warpToEntity(runtime, session, targetEntityID, options = {}) {
            ...
            const landingPending = Boolean(entity && hasPendingPilotWarpLanding(session, entity));
            ...
            const targetInstanceID = normalizePersistentEntityID(target.dungeonSiteInstanceID ??
                target.airNpeInstanceID ??
                (target.kind === "ship" ? target.dungeonCurrentInstanceID : null));
            if (targetInstanceID !== null) {
                const destinationResolution = resolveCommandDestinationStaticInstanceID(runtime, session, options);
            ...
            if (typeof runtime.canSessionSeeDungeonScopedEntity === "function" &&
                !runtime.canSessionSeeDungeonScopedEntity(session, target, {
                    ...(targetInstanceID !== null
                        ? {
                            dedicatedSiteStaticInstanceIDOverride: targetInstanceID,
                        }
                        : {}),
                })) {
                return {
            ...
            if (target.kind === "stargate") {
                const stargateWarpTarget = typeof resolveStargateWarpTarget === "function"
                    ? resolveStargateWarpTarget(entity, target, toFiniteNumber(options.minimumRange, 0))
                    : {
                        rawDestination: getStargateWarpLandingPoint(entity, target, toFiniteNumber(options.minimumRange, 0)),
                        stopDistance: 0,
                    };
                return runtime.warpToPoint(session, stargateWarpTarget.rawDestination, {
                    ...options,
                    stopDistance: Math.max(0, toFiniteNumber(stargateWarpTarget.stopDistance, 0)),
                    targetEntityID: target.itemID,
                });
            }
            const clientParityWarpInPoint = typeof getClientParityWarpInPoint === "function"
            ...
            const stopDistance = getWarpStopDistanceForTarget(entity, target, resolveDungeonSiteWarpInMinimumRange(target, toFiniteNumber(options.minimumRange, 0)));
            const warpTargetPoint = target && (target.kind === "station" || target.kind === "structure")
        ...
        warpToPoint(runtime, session, point, options = {}) {
            ...
            const rawCommandUsesNativeGoto = isNativeWarpDistanceTooShort(getRawWarpCommandDistance(entity, point));
            if (!rawCommandUsesNativeGoto && isRedundantWarpToPoint(entity, point, destinationStaticInstanceID, destinationResolution.roomKey, destinationResolution.siteID)) {
            ...
            let warpPoint = point;
            if (config.deadspaceWarpRestrictionEnabled !== false &&
                ...
                try {
                    ...
                    if (decision && decision.action === "clamp" && decision.point) {
                        ...
                        const clampedInstanceID = normalizePersistentEntityID(decision.siteInstanceID);
                        if (clampedInstanceID !== null) {
                            const clampedResolution = resolveCommandDestinationStaticInstanceID(runtime, session, { destinationStaticInstanceID: clampedInstanceID });
                            if (clampedResolution.success !== true ||
                                clampedResolution.instanceID === null ||
                                (destinationStaticInstanceID !== null &&
                                    !entityIDsEqual(destinationStaticInstanceID, clampedResolution.instanceID))) {
                                return {
                                    success: false,
                                    errorMsg: "DUNGEON_INSTANCE_NOT_AUTHORIZED",
                                };
            ...
            const destinationDungeonSiteEntity = destinationDungeonSiteID !== null
                ...
                : null;
            const configuredDungeonMinimumRange = resolveDungeonSiteWarpInMinimumRange(destinationDungeonSiteEntity, options.stopDistance);
            const stopDistance = resolveDungeonWarpStopDistance(configuredDungeonMinimumRange, destinationDungeonSiteID, options);
            ...
            const targetEntityID = normalizePersistentEntityID(options.targetEntityID);
            const nativeDungeonSiteGoto = Boolean(rawCommandUsesNativeGoto &&
                destinationStaticInstanceID !== null &&
            ...
            if (options.ignoreWarpDisruptionField !== true &&
            ...
            if (pendingWarp.nativeWarpCommand !== "GOTO" &&
                ...
                const materialized = materializeDungeonWarpDestination(runtime, session, destinationStaticInstanceID, { nowMs: now });
            ...
            if (pendingWarp.nativeWarpCommand === "GOTO") {
                return routeNativeGotoFallback(runtime, entity, pendingWarp, now, options);
            }
            const movementStamp = runtime.getMovementStamp(now);
            const previousSpeedFraction = entity.speedFraction;
            const pilotPrepareStamp = session && isReadyForDestiny(session)
                ? runtime.getHistorySafeDestinyStamp(now, PILOT_WARP_ACTIVATION_DELAY_DESTINY_TICKS, PILOT_WARP_ACTIVATION_DELAY_DESTINY_TICKS)
                : movementStamp;
            pendingWarp.prepareStamp = pilotPrepareStamp;
            pendingWarp.prepareVisibleStamp =
                session && isReadyForDestiny(session)
                    ? runtime.getHistorySafeSessionDestinyStamp(session, now, PILOT_WARP_ACTIVATION_DELAY_DESTINY_TICKS, PILOT_WARP_ACTIVATION_DELAY_DESTINY_TICKS)
                    : pilotPrepareStamp;
            ...
            const alignDirection = normalizeVector(subtractVectors(pendingWarp.rawDestination, entity.position), entity.direction);
            ...
            logWarpDebug("warp.requested", entity, {
                officialProfile: buildOfficialWarpReferenceProfile(pendingWarp.totalDistance, pendingWarp.warpSpeedAU, entity.maxVelocity),
            });
            if (session) {
                ...
                if (!prewarmResult.success) {
                    log.warn(`[SpaceRuntime] Warp destination prewarm failed for system=${runtime.systemID} ship=${entity.itemID}: ${prewarmResult.errorMsg || "UNKNOWN_ERROR"}`);
                }
            }
            const usesFrontierNativeWarpActivation = isFrontierProfile(session && session.compatibilityProfile);
            ...
            if (session) {
                ...
                const speedFractionChanged = Math.abs(entity.speedFraction - previousSpeedFraction) > 0.000001;
                if (pendingWarp.pilotPrepareDeferred) {
                    runtime.sendDestinyUpdates(session, buildDirectedMovementUpdates(entity, alignDirection, speedFractionChanged, pilotPrepareStamp), false, {
                ...
                const observerAlignStamp = selectFurthestDestinyStamp(movementStamp, [movementStamp, pilotPrepareStamp]);
                const alignUpdates = tagUpdatesRequireExistingVisibility(buildDirectedMovementUpdates(entity, alignDirection, speedFractionChanged, observerAlignStamp));
                ...
                const prepareDispatch = buildWarpPrepareDispatch(entity, pilotPrepareStamp, entity.warpState, {
        ...
        warpDynamicEntityToPoint(runtime, entityOrID, point, options = {}) {
            ...
            const landingPending = Boolean(entity && entity.session &&
            ...
            const now = runtime.getCurrentSimTimeMs();
            const destinationResolution = resolveCommandDestinationStaticInstanceID(runtime, options.session || entity.session || null, options);
            if (destinationResolution.success !== true) {
                return destinationResolution;
            }
            ...
            if (!entity.session &&
                ...
                const initializationResult = initializeSessionlessWarpDestination(runtime, entity, pendingWarp.rawDestination, {
                    ...options,
                    nowMs: now,
                ...
                if (!initializationResult || initializationResult.success !== true) {
                    return {
                        success: false,
                        errorMsg: (initializationResult && initializationResult.errorMsg) ||
            ...
            const desiredDirection = normalizeVector(subtractVectors(pendingWarp.rawDestination, entity.position), entity.direction);
            clearTrackingState(entity);
            entity.pendingWarp = pendingWarp;
            entity.mode = "WARP";
            entity.speedFraction = 1;
            ...
            if (options.forceImmediateStart === true) {
                ...
                pendingWarp.requestedAtMs = now - Math.max(1_000, (toFiniteNumber(entity.alignTime, 0) * 1000) + 500);
            }
            entity.warpState = buildPreparingWarpState(entity, pendingWarp, {
                nowMs: now,
            });
            if (!entity.session &&
                ...
                if (!prewarmResult.success) {
                    log.warn(`[SpaceRuntime] NPC warp destination prewarm failed for system=${runtime.systemID} ship=${entity.itemID}: ${prewarmResult.errorMsg || "UNKNOWN_ERROR"}`);
                }
            }
            persistShipEntity(entity);
            armMovementTrace(entity, "warp", {
            ...
            const prepareDispatch = buildWarpPrepareDispatch(entity, movementStamp, entity.warpState);
            runtime.broadcastMovementUpdates(prepareDispatch.sharedUpdates);
            return {
                success: true,
                data: pendingWarp,
        ...
        forceStartPendingWarp(runtime, entityOrID, options = {}) {
            ...
            if (String(pendingWarp.nativeWarpCommand || "").trim().toUpperCase() ===
                "GOTO") {
                return routeNativeGotoFallback(runtime, entity, pendingWarp, now, options);
            }
            if (entity.kind !== "ship") {
            ...
            if (typeof deactivateWarpUnsafeActiveModulesForWarpStart === "function") {
                const deactivationResult = deactivateWarpUnsafeActiveModulesForWarpStart(runtime, entity, now, {
                    reason: "warpStart",
                });
                if (deactivationResult && deactivationResult.success === false) {
                    return {
                        success: false,
                        errorMsg: deactivationResult.errorMsg ||
            ...
            const warpStartStamp = entity.session && isReadyForDestiny(entity.session)
                ? selectFurthestDestinyStamp(currentStamp, [currentStamp, pendingWarp && pendingWarp.prepareStamp])
                : currentStamp;
            ...
            if (entity.session && isReadyForDestiny(entity.session)) {
                const watcherWarpStartStamp = getWatcherWarpStartStamp(warpState, pendingWarp, warpStartStamp);
                const warpStartUpdates = buildWarpStartUpdates(entity, warpState, watcherWarpStartStamp, {
                    includeEntityWarpIn: false,
                });
                if (warpStartUpdates.length > 0) {
                    runtime.broadcastMovementUpdates(warpStartUpdates, entity.session, {
                        minimumSessionStamp: hasDestinyStamp(pendingWarp && pendingWarp.prepareVisibleStamp)
        ...
        startSessionlessWarpIngress(runtime, entityOrID, point, options = {}) {
            ...
            if (warpResult.redundant === true) {
                ...
                return {
                    ...
                    data: {
                        ...
                        ingressCompleteAtMs: existingIngress &&
            ...
            const visibilitySuppressMs = Math.max(1, toFiniteNumber(options.visibilitySuppressMs, DESTINY_STAMP_INTERVAL_MS));
            entity.suppressWarpAcquireUntilNextTick = true;
            entity.visibilitySuppressedUntilMs = Math.max(toFiniteNumber(entity.visibilitySuppressedUntilMs, 0), now + visibilitySuppressMs);
            entity.sessionlessWarpIngress = buildSessionlessWarpIngressState(entity, activationResult.data && activationResult.data.warpState, {
                nowMs: now,
                durationMs: options.ingressDurationMs,
                useNativeWarpProfile: options.useNativeWarpProfile === true,
                suppressCompletionStopUpdates: options.suppressCompletionStopUpdates === true,
            });
            if (options.broadcastWarpStartToVisibleSessions === true) {
                const warpStartStamp = runtime.getNextDestinyStamp(now);
                const warpStartUpdates = buildWarpStartUpdates(entity, activationResult.data && activationResult.data.warpState, warpStartStamp, {
                    includeEntityWarpIn: false,
                });
                runtime.sendSessionlessWarpStartToVisibleSessions(entity, warpStartUpdates);
            }
            ...
            return {
                ...
                data: {
                    ...
                    ingressCompleteAtMs: entity.sessionlessWarpIngress &&
...
//# sourceMappingURL=warpCommands.js.map