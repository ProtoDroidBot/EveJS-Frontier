...
const { buildGotoPointPayload, buildSetMaxSpeedPayload, buildSetSpeedFractionPayload, } = require("../stream/actions");
function applyFighterPassiveMaxVelocityCommand({ fighterEntity, baseMaxVelocity, } = {}) {
    if (!fighterEntity) {
        return false;
    }
...
function applyFighterMobilityMaxVelocityCommand({ fighterEntity, maxVelocity, } = {}) {
    if (!fighterEntity) {
        return false;
    }
...
function createFighterOperationalMotionCommand({ fighterEntity, mass, inertia, maxVelocity, alignTime, maxAccelerationTime, agilitySeconds, } = {}) {
...
function applyFighterUtilityTeleportMotionCommand({ fighterEntity, direction, targetPoint, cloneTargetPoint, } = {}) {
    if (!fighterEntity) {
        return false;
    }
...
function applyFighterGotoPointCommand({ fighterEntity, targetPoint, direction, } = {}) {
    if (!fighterEntity) {
    ...
    return {
        ...
        buildPresentationUpdates({ stamp } = {}) {
            return [
                ...
                {
                    stamp,
                    payload: buildSetSpeedFractionPayload(fighterEntity.itemID, fighterEntity.speedFraction),
                },
            ];
        },
    };
...
function buildFighterMaxVelocityUpdates({ fighterEntity, stamp } = {}) {
    return [{
            stamp,
            payload: buildSetMaxSpeedPayload(fighterEntity.itemID, fighterEntity.maxVelocity),
        }];
}
function captureFighterMaxVelocityPresentation({ scene, fighterEntity, nowMs, } = {}) {
    const stamp = typeof scene.getMovementStamp === "function"
