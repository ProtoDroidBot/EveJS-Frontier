...
const { buildFollowBallPayload, buildGotoPointPayload, buildSetBallFreePayload, buildSetBallMassPayload, buildSetBallPositionPayload, buildSetBallVelocityPayload, buildSetMaxSpeedPayload, buildSetSpeedFractionPayload, buildStopPayload, } = require("../stream/actions");
...
function captureFittedTractorPresentation({ scene, nowMs, } = {}) {
    return scene.getMovementStamp(nowMs);
}
function applyTractorStopCommand({ targetEntity, restoreMaxVelocity, } = {}) {
    if (!targetEntity) {
        return {
            applied: false,
...
function applyTractorActivationCommand({ sourceEntity, targetEntity, tractorSpeedMetersPerSecond, followRangeMeters, } = {}) {
    ...
    return {
        ...
        buildPresentationUpdates({ stamp } = {}) {
            return [
                ...
                {
                    stamp,
                    payload: buildFollowBallPayload(targetEntity.itemID, sourceEntity.itemID, followRange),
                },
            ];
        },
    };
}
function applyTractorSettleCommand({ targetEntity, direction, holdPosition, tractorSpeedMetersPerSecond, stamp, } = {}) {
    if (!targetEntity) {
        return { applied: false, buildPresentationUpdates: () => [] };
    }
    const tractorSpeed = Math.max(0, toFiniteNumber(tractorSpeedMetersPerSecond, 0));
...
function applyTractorPullCommand({ targetEntity, movementDirection, moveDistanceMeters, holdPosition, tractorSpeedMetersPerSecond, stamp, } = {}) {
    if (!targetEntity) {
        return {
            applied: false,
    ...
    targetEntity.position = addVectors(targetEntity.position, scaleVector(movementDirection, moveDistanceMeters));
...
//# sourceMappingURL=tractorBeam.js.map