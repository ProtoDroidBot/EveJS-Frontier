...
function buildDelivery(kind, clients, state, waitForBubble, updateCount, meta: Record<string, any> = {}) {
  return {
    kind,
...
function buildParkUpdateDeliveryPlan(options: Record<string, any> = {}) {
  ...
  const clientsWaitingForBubble = new Set<any>();
  const singleBatchNarrowcasts: any[] = [];
  const dualBatchNarrowcasts: any[] = [];
  const singlecasts: any[] = [];

  for (const bubble of bubbleHistories) {
    const dualClients: any[] = [];
    const singleClients: any[] = [];
...
function orderFreshAcquireDependencies(values, options: Record<string, any> = {}) {
...
function compareParkHistoryGroups(left, right, options: Record<string, any> = {}) {
...
function appendParkHistoryGroup(collectedGroups, groupDetails: Record<string, any> = {}, options: Record<string, any> = {}) {
...
function orderParkHistoryGroups(collectedGroups, options: Record<string, any> = {}) {
...
function buildParkHistoryFlushGroups(collectedGroups, options: Record<string, any> = {}) {
  const convert = typeof options.toInt === "function" ? options.toInt : toInt;
  const orderedGroups = orderParkHistoryGroups(collectedGroups, options);
  const flushGroups: any[] = [];
  let currentStamp = null;
  ...
  let currentSpatialTraceContexts: any[] = [];
  ...
  const consumedBubbleGroupIndexes = new Set<any>();
