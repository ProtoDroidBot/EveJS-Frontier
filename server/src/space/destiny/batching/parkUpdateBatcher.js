...
const { buildPackagedActionUpdate, } = require(path.join(__dirname, "./packagedAction"));
const { getEntityMapKey, } = require(path.join(__dirname, "../identity/entityID"));
const { compareDestinyStamps, normalizeDestinyStamp, } = require(path.join(__dirname, "../delivery/stamps"));
const { getPayloadPrimaryEntityID, } = require(path.join(__dirname, "../protocol/payloadIdentity"));
const { DESTINY_NOTIFICATION_NAME, } = require(path.join(__dirname, "../delivery/michelleContract"));
...
function buildParkUpdateDeliveryPlan(options = {}) {
    ...
    const characterHistories = normalizeCharacterHistories(options.characterHistories);
    const clientsWithCharacterHistory = new Set(characterHistories.map((entry) => entry.clientID));
    ...
    for (const bubble of bubbleHistories) {
        ...
        if (singleClients.length > 0) {
            singleBatchNarrowcasts.push(buildDelivery("bubble-single-batch", singleClients, bubble.state, false, CLIENT_UPDATE_COUNT_THIS_TICK.ONE, { bubbleID: bubble.bubbleID }));
        }
        if (dualClients.length > 0) {
            dualBatchNarrowcasts.push(buildDelivery("bubble-dual-batch", dualClients, bubble.state, false, CLIENT_UPDATE_COUNT_THIS_TICK.TWO, { bubbleID: bubble.bubbleID }));
        }
    }
    for (const character of characterHistories) {
        const waitForBubble = clientsWaitingForBubble.has(character.clientID);
        singlecasts.push(buildDelivery("character", [character.clientID], character.state, waitForBubble, waitForBubble
            ? CLIENT_UPDATE_COUNT_THIS_TICK.TWO
            : CLIENT_UPDATE_COUNT_THIS_TICK.ONE, { characterID: character.characterID }));
    }
    return {
...
function mergeStateIntoHistory(state, historyList, waitForBubble = false) {
    ...
    for (let historyIndex = 0; historyIndex < historyList.length && timeListIndex < timeList.length; historyIndex += 1) {
...
function orderFreshAcquireDependencies(values, options = {}) {
    ...
    const prerequisites = Array.from({ length: values.length }, () => new Set());
    for (let index = 0; index < values.length; index += 1) {
        const update = getUpdate(values[index]);
...
function appendParkHistoryGroup(collectedGroups, groupDetails = {}, options = {}) {
    ...
    const baseOrder = Math.max(0, convert(groupDetails.order, collectedGroups.length));
    const contract = typeof groupDetails.contract === "string"
        ? groupDetails.contract
        : "";
    const rawDispatchStamp = normalizeOptionalRawDispatchStamp(groupDetails.rawDispatchStamp, convert);
    const delayedTargetEvents = normalizeDelayedTargetEvents(groupDetails.delayedTargetEvents);
    const spatialTraceContext = groupDetails.spatialTraceContext &&
        ...
        : null;
    const buildSpatialTraceContext = typeof options.buildSpatialTraceContext === "function"
    ...
    const pushSplit = () => {
        ...
        const split = {
            stamp: currentStamp,
            waitForBubble: groupDetails.waitForBubble === true && currentStamp === baseStamp,
    ...
    for (const update of rawUpdates) {
        const updateStamp = normalizeDestinyStamp(convert(update && update.stamp, baseStamp));
...
function orderParkHistoryGroups(collectedGroups, options = {}) {
    return (Array.isArray(collectedGroups) ? collectedGroups : [])
        .filter((group) => (group && Array.isArray(group.updates) && group.updates.length > 0))
...
function buildParkHistoryFlushGroups(collectedGroups, options = {}) {
    ...
    for (const group of orderedGroups) {
        ...
        if (currentUpdates.length === 0) {
            ...
            if (hasOwn(group, "delayedTargetEvents")) {
                currentDelayedTargetEvents = normalizeDelayedTargetEvents(group.delayedTargetEvents);
                currentHasDelayedTargetEvents = true;
            }
        }
        else {
            if (groupHasRawDispatchStamp) {
                currentRawDispatchStamp = selectRawDispatchStamp(currentRawDispatchStamp, groupRawDispatchStamp);
                currentHasRawDispatchStamp = true;
            }
            if (hasOwn(group, "delayedTargetEvents")) {
                currentDelayedTargetEvents = mergeDelayedTargetEvents(currentDelayedTargetEvents, group.delayedTargetEvents);
                currentHasDelayedTargetEvents = true;
            }
        }
    ...
    for (let index = 0; index < flushGroups.length; index += 1) {
        ...
        const pairedGroupIndex = flushGroups.findIndex((candidate, candidateIndex) => (candidateIndex > index &&
            ...
            sameRawDispatchLane(hasOwn(group, "rawDispatchStamp") ? group.rawDispatchStamp : null, hasOwn(candidate, "rawDispatchStamp")
...
//# sourceMappingURL=parkUpdateBatcher.js.map