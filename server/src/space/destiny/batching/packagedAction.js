...
const { marshalDecodeExact, marshalEncode } = require(path.join(__dirname, "../../../network/tcp/utils/marshal"));
...
function decodeSerializedActions(serializedActions) {
    ...
    for (const update of actions) {
        ...
        const validStamp = (typeof stamp === "number" &&
            Number.isInteger(stamp) &&
            stamp >= 0 &&
            stamp <= 0xffff_ffff) || (typeof stamp === "bigint" &&
            stamp >= 0n &&
            stamp <= 0xffffffffn);
        const validName = (typeof name === "string" && name.length > 0) || (Buffer.isBuffer(name) && name.length > 0);
...
function buildPackagedActionPayload(serializedActionsOrUpdates, options = {}) {
    const serializedActions = Array.isArray(serializedActionsOrUpdates)
        ? marshalEncode(serializedActionsOrUpdates
            .filter(Boolean)
            .map((update) => normalizePackagedUpdate(update, toStamp(options && options.stamp, 0))))
...
//# sourceMappingURL=packagedAction.js.map