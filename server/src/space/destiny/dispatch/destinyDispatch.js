...
const { createDestinyAuthority, } = require("../authority/destinyAuthority.js");
const { resolveDestinyAuthorityLaneTuple, snapshotDestinyAuthorityState, updateDestinyAuthorityState, } = require("../authority/destinySessionState.js");
const { DESTINY_CONTRACTS, } = require("../authority/destinyContracts.js");
const { recordSyncLedgerEvent, summarizeDestinyUpdates, } = require("../../../network/syncLedger");
const { extractPayloadEntityIDs, } = require("../protocol/payloadIdentity");
const { getEntityMapKey, } = require("../identity/entityID");
const { getPendingVisibilityRemovalIDs, } = require("../visibility/acquisition");
const spatialTrace = require("../../../network/spatialTrace");
const { advanceDestinyStamp, compareDestinyStamps, getDestinyStampForwardDistance, hasDestinyStamp, isDestinyStampAfter, isDestinyStampWithinForwardWindow, normalizeDestinyStamp, resolveOptionalDestinyStamp, selectFurthestDestinyStamp, selectLaterDestinyStamp, } = require("../delivery/stamps");
const { commitDestinyDeliveryTransaction, createDestinyDeliveryTransaction, isDestinyDeliveryTransactionGenerationCurrent, mergeDestinyDeliveryTransactions, registerDestinyDeliveryHooks, restoreAuthorityDeliveryStateForGeneration, retryDestinyDeliveryTransaction, rollbackDestinyDeliveryTransaction, snapshotAuthorityDeliveryState, stageDestinyDeliveryTransaction, } = require("../delivery/deliveryTransaction.js");
const { appendParkHistoryGroup, buildParkHistoryFlushGroups, orderParkHistoryGroups, } = require("../batching/parkUpdateBatcher.js");
const { appendSystemHistoryAction, takeSystemHistoryActions, } = require("../batching/systemHistory.js");
const { attemptDestinyNotificationDelivery, destroySessionAfterUnsafeDestinyDelivery, isSessionOpenForDestinyDelivery, } = require("../delivery/michelleContract.js");
...
const DESTINY_DELIVERY_PLAN_GUARDS = Symbol("destinyDeliveryPlanGuards");
const entityPresentationPlanByTransaction = new WeakMap();
const drainingTickPresentationPrefixToken = Symbol("drainingTickPresentationPrefix");
function createMovementDestinyDispatch(deps = {}) {
    const { buildMissileSessionMutation, buildMissileSessionSnapshot, clamp, destiny, getPayloadPrimaryEntityID, getNextMissileDebugTraceID, isVisibilityRemovalPresentationAuthorized, isMovementContractPayload, isReadyForDestiny, logDestinyDispatch, logMissileDebug, normalizeTraceValue, resolveDestinyLifecycleRestampState, resolveOwnerMonotonicState, resolvePreviousLastSentDestinyWasOwnerCritical, roundNumber, shouldLogMissilePayloadGroup, summarizeMissileUpdatesForLog, toInt, updatesContainMovementContractPayload, MICHELLE_DIRECT_CRITICAL_ECHO_DESTINY_LEAD, MICHELLE_HELD_FUTURE_DESTINY_LEAD, MICHELLE_POST_HELD_FUTURE_DESTINY_LEAD, PILOT_WARP_ACTIVATION_DELAY_DESTINY_TICKS, PILOT_WARP_HISTORY_SAFE_DESTINY_LEAD = PILOT_WARP_ACTIVATION_DELAY_DESTINY_TICKS, } = deps;
    ...
    const getDestinyDeliveryPlanGuard = (options = {}) => (options && typeof options.deliveryPlanGuard === "function"
        ? options.deliveryPlanGuard
        : null);
    const isIncomingDestinyDeliveryPlanCurrent = (options = {}, transaction = null) => {
    ...
    const getStaleDestinyDeliveryReason = (transaction) => (isDestinyDeliveryTransactionGenerationCurrent(transaction)
    ...
    const shouldSkipStaleDestinyDeliveryCallbacks = (transaction) => (!isDestinyDeliveryTransactionGenerationCurrent(transaction));
    const rollbackDestinyDeliveryTransactionPreservingAuthority = (transaction, details = {}) => {
        const generationCurrent = isDestinyDeliveryTransactionGenerationCurrent(transaction);
        ...
        if (currentAuthorityState &&
            transaction.session &&
            transaction.session._space === transaction.spaceGeneration) {
            restoreAuthorityDeliveryStateForGeneration(transaction.spaceGeneration, currentAuthorityState);
        }
        return transaction;
    };
    ...
    function filterPendingRemovalDependentPayloads(session, payloads, options = {}) {
        ...
        return payloads.filter((update) => {
            ...
            const targetsPendingRemoval = entityIDs.some((entityID) => pendingRemovalIDs.has(getEntityMapKey(entityID)));
            if (!targetsPendingRemoval) {
                return true;
            }
            return Boolean(typeof isVisibilityRemovalPresentationAuthorized === "function" &&
                isVisibilityRemovalPresentationAuthorized(options, session, payloadName, entityIDs));
        });
    }
    ...
    function getExistingDeliveryTransaction(runtime, session, options = {}) {
        const suppliedTransaction = resolveRootDeliveryTransaction(options && options._deliveryTransaction);
        if (suppliedTransaction) {
            return suppliedTransaction;
        }
        const tickEntry = runtime &&
        ...
        const directEntry = runtime &&
            ...
            : null;
        return resolveRootDeliveryTransaction(directEntry && directEntry.deliveryTransaction);
    }
    function rollbackUnplannedDeliveryCall(options, reason, deliveryTransaction = null) {
    ...
    function isDeliveryTransactionRetainedByRuntime(runtime, session, deliveryTransaction) {
        const transaction = resolveRootDeliveryTransaction(deliveryTransaction);
        return Boolean(transaction &&
            resolveRootDeliveryTransaction(getExistingDeliveryTransaction(runtime, session, {})) === transaction);
    }
    function rejectStaleIncomingDeliveryPlan(runtime, session, options, deliveryTransaction = null) {
        const transaction = resolveRootDeliveryTransaction(deliveryTransaction);
        ...
        if (transaction &&
            ...
            rollbackUnplannedDeliveryCall(options, "delivery-plan-replaced", transaction);
        }
        return true;
    }
    ...
    function getEntityPresentationFloor(deliveryTransaction, entityIDs) {
        ...
        return entityIDs.reduce((floor, entityID) => selectLaterDestinyStamp(floor, plan.floorByEntityID.get(entityID)), null);
    }
    function recordEntityPresentationFloor(deliveryTransaction, entityIDs, stamp) {
        const plan = getEntityPresentationPlan(deliveryTransaction);
        if (!plan ||
        ...
        for (const entityID of entityIDs) {
            plan.floorByEntityID.set(entityID, selectLaterDestinyStamp(plan.floorByEntityID.get(entityID), stamp));
        }
    }
    function updatesContainObserverPresentedMonotonicPayload(updates, ownerShipID = 0) {
    ...
    function getDestinyHistoryAnchorStampForSession(runtime, session, rawSimTimeMs = runtime.getCurrentSimTimeMs(), options = {}) {
        return destinyAuthority.getDestinyHistoryAnchorStampForSession(runtime, session, rawSimTimeMs, options);
    }
    function resolveDestinyDeliveryStampForSession(runtime, session, authoredStamp, rawSimTimeMs = runtime.getCurrentSimTimeMs(), options = {}) {
        return destinyAuthority.resolveDestinyDeliveryStampForSession(runtime, session, authoredStamp, rawSimTimeMs, options);
    }
    function prepareDestinyUpdateForSession(runtime, session, rawPayload, rawSimTimeMs = runtime.getCurrentSimTimeMs(), options = {}) {
        return destinyAuthority.prepareDestinyUpdateForSession(runtime, session, rawPayload, rawSimTimeMs, options);
    }
    function abortActiveTickDestinyPresentationBatch(runtime, reason = "tick-presentation-replaced") {
        ...
        for (const queued of tickBatch.bySession.values()) {
            const transaction = resolveRootDeliveryTransaction(queued && queued.deliveryTransaction);
            if (transaction) {
        ...
        if (directBatch && directBatch.bySession instanceof Map) {
            for (const [sessionKey, queued] of directBatch.bySession.entries()) {
                if (abortedTransactions.has(resolveRootDeliveryTransaction(queued && queued.deliveryTransaction))) {
            ...
            if (directBatch.bySession.size === 0 &&
    ...
    function hasActiveTickDestinyPresentationBatch(runtime) {
        return Boolean(runtime._tickDestinyPresentation &&
            runtime._tickDestinyPresentation.bySession instanceof Map);
    }
    function shouldDeferPilotMovementForMissilePressure(runtime, session, nowMs = runtime.getCurrentSimTimeMs()) {
        if (!session || !session._space || !isReadyForDestiny(session)) {
            return false;
        }
        const authorityState = snapshotDestinyAuthorityState(session);
        const currentSessionStamp = runtime.getCurrentSessionDestinyStamp(session, nowMs);
        const currentVisibleStamp = runtime.getCurrentVisibleSessionDestinyStamp(session, nowMs);
        ...
        const maximumTrustedMissilePressureLane = selectFurthestDestinyStamp(currentSessionStamp, [
            currentVisibleStamp,
            lastSentDestinyStamp,
            advanceDestinyStamp(currentSessionStamp, PILOT_WARP_HISTORY_SAFE_DESTINY_LEAD),
        ], PILOT_WARP_HISTORY_SAFE_DESTINY_LEAD);
        ...
        const lastMissileLifecycleRawDispatchStamp = lastMissileLifecycleTuple.rawDispatchStamp;
        const lastOwnerMissileLifecycleTuple = resolveDestinyAuthorityLaneTuple({
            authorityState,
            legacyState: session._space,
        ...
        const lastOwnerMissileLifecycleStamp = lastOwnerMissileLifecycleTuple.stamp;
        const lastOwnerMissileLifecycleRawDispatchStamp = lastOwnerMissileLifecycleTuple.rawDispatchStamp;
        const maximumTrustedMissilePressureLead = getDestinyStampForwardDistance(currentSessionStamp, maximumTrustedMissilePressureLane);
        const hasRecentVisibleMissileLifecycle = lastMissileLifecycleStamp !== currentSessionStamp &&
            hasDestinyStamp(lastMissileLifecycleRawDispatchStamp) &&
            isDestinyStampWithinForwardWindow(currentSessionStamp, lastMissileLifecycleStamp, maximumTrustedMissilePressureLead) &&
            getDestinyStampForwardDistance(lastMissileLifecycleRawDispatchStamp, currentRawDispatchStamp) <= 2;
        const hasRecentOwnerMissileLifecycle = lastOwnerMissileLifecycleStamp !== currentSessionStamp &&
            hasDestinyStamp(lastOwnerMissileLifecycleRawDispatchStamp) &&
            isDestinyStampWithinForwardWindow(currentSessionStamp, lastOwnerMissileLifecycleStamp, maximumTrustedMissilePressureLead) &&
            getDestinyStampForwardDistance(lastOwnerMissileLifecycleRawDispatchStamp, currentRawDispatchStamp) <= 2;
    ...
    function normalizeQueuedPresentationSendOptions(sendOptions) {
        ...
        const deliveryTransactionDescriptor = Object.getOwnPropertyDescriptor(sendOptions, "_deliveryTransaction");
        if (deliveryTransactionDescriptor) {
            Object.defineProperty(normalized, "_deliveryTransaction", deliveryTransactionDescriptor);
        }
    ...
    function buildQueuedPresentationSendOptionsSignature(sendOptions) {
        ...
        return JSON.stringify(Object.keys(normalized)
            .sort()
            .map((key) => [key, normalized[key]]), (_key, value) => (typeof value === "bigint"
            ? { __destinyQueuedOptionBigInt: value.toString() }
            : value));
    }
    function appendCollectedDestinyGroup(collectedGroups, groupDetails = {}, options = {}) {
    ...
    function ensureTickDestinyDeliveryEntry(runtime, session, options = {}, preferredDeliveryTransaction = null) {
        if (!runtime ||
            !session ||
            !hasActiveTickDestinyPresentationBatch(runtime)) {
            return null;
        }
        const preferredTransaction = resolveRootDeliveryTransaction(preferredDeliveryTransaction);
        const preferredTransactionAlreadyRetained = Boolean(preferredTransaction &&
            isDeliveryTransactionRetainedByRuntime(runtime, session, preferredTransaction));
        ...
        if (queued &&
            ...
            !isDestinyDeliveryPlanCurrent(resolveRootDeliveryTransaction(queued.deliveryTransaction))) {
            const staleTransaction = resolveRootDeliveryTransaction(queued.deliveryTransaction);
            rollbackDestinyDeliveryTransaction(staleTransaction, {
                reason: getStaleDestinyDeliveryReason(staleTransaction),
                skipCallbacks: shouldSkipStaleDestinyDeliveryCallbacks(staleTransaction),
            });
            ...
            if (preferredTransaction &&
                ...
                preferredTransaction.beforeState = snapshotAuthorityDeliveryState(session);
            }
        }
        if (!queued) {
            queued = {
                ...
                deliveryTransaction: preferredTransaction ||
                    createDestinyDeliveryTransaction(session, options),
            };
            batch.bySession.set(session, queued);
        }
        else {
            const queuedTransaction = resolveRootDeliveryTransaction(queued.deliveryTransaction);
            ...
            if (preferredTransaction &&
                ...
                mergeDestinyDeliveryPlanGuards(queuedTransaction, preferredTransaction);
                mergeDestinyDeliveryTransactions(queuedTransaction, preferredTransaction);
            }
            registerDestinyDeliveryHooks(queuedTransaction, options);
        }
        registerDestinyDeliveryPlanGuard(resolveRootDeliveryTransaction(queued.deliveryTransaction), options);
        return queued;
    }
    function isDeliveryTransactionOwnedByActiveTick(runtime, session, deliveryTransaction) {
        if (!hasActiveTickDestinyPresentationBatch(runtime)) {
            return false;
        }
    ...
    function hasQueuedDirectDeliveryWork(runtime, session, deliveryTransaction) {
        const batch = runtime && runtime._directDestinyNotificationBatch;
        const queued = batch && batch.bySession instanceof Map
    ...
    function discardQueuedDirectDeliveryWork(runtime, session, deliveryTransaction) {
        const batch = runtime && runtime._directDestinyNotificationBatch;
    ...
    function rollbackTickPresentationDrainFailure(runtime, batch, storedEntries, error) {
        const rolledBackTransactions = new Set();
        for (const storedQueued of Array.isArray(storedEntries) ? storedEntries : []) {
            if (!storedQueued) {
                continue;
            }
            const transaction = resolveRootDeliveryTransaction(storedQueued.deliveryTransaction);
            ...
            discardQueuedDirectDeliveryWork(runtime, storedQueued.session, transaction);
            if (transaction &&
    ...
    function canFlushInitialBallparkBootstrapGroups(session, collectedGroups) {
        ...
        return collectedGroups.every((group) => (group &&
    ...
    function validateDestinyNotificationGroup(session, groupStamp, updates, context = {}) {
        const lifecycle = ensureDestinyLifecycleLedger(session);
        const normalizedGroupStamp = normalizeDestinyStamp(groupStamp, 0);
        const updateStamps = [...new Set((Array.isArray(updates) ? updates : [])
        ...
        if (lifecycle &&
            ...
            isDestinyStampAfter(normalizedGroupStamp, lifecycle.lastNotificationStamp)) {
            violations.push({
        ...
        for (const update of Array.isArray(updates) ? updates : []) {
            ...
            if (payloadName === "RemoveBalls" && lifecycle) {
                for (const entityID of entityIDs) {
                    const lastAddStamp = resolveOptionalDestinyStamp(lifecycle.addStampByEntityID.get(entityID));
                    if (hasDestinyStamp(lastAddStamp) &&
        ...
        if (lifecycle) {
            lifecycle.lastNotificationStamp = selectLaterDestinyStamp(lifecycle.lastNotificationStamp, normalizedGroupStamp);
        }
    ...
    function rejectReentrantDestinySend(runtime, session) {
        ...
        if (!error) {
            error = new Error("Destiny delivery re-entered while a physical plan was active");
    ...
    function overrideReentrantDestinyDeliveryAttempt(runtime, session, attempt) {
        const activeFlush = runtime && runtime._activeDestinyDeliveryFlush;
        const error = activeFlush && activeFlush.unsafeSessions instanceof Map
    ...
    function flushCollectedDestinyGroups(runtime, session, collectedGroups) {
        ...
        const deliveryTransactions = new Set(collectedGroups
            .map((group) => resolveRootDeliveryTransaction(group && group.deliveryTransaction))
            .filter(Boolean));
        if ([...deliveryTransactions].some((transaction) => !isDestinyDeliveryPlanCurrent(transaction))) {
            return {
                accepted: false,
                ambiguous: false,
                deliveredCount: 0,
                plannedDeliveryCount: 0,
                highestStamp: null,
                reason: getStaleDestinyDeliveryReasonFromTransactions(deliveryTransactions),
            };
        }
        const physicalGroups = buildParkHistoryFlushGroups(collectedGroups, { toInt }).map((group) => {
            const deliveryTransaction = resolveRootDeliveryTransaction(group && group.deliveryTransaction);
            return {
                ...group,
                ...
                spatialTraceContexts: Array.isArray(group.spatialTraceContexts)
                    ...
                    : [],
                deliveryTransactions: new Set(deliveryTransaction ? [deliveryTransaction] : []),
            };
        });
        ...
        for (let groupIndex = 0; groupIndex < physicalGroups.length; groupIndex += 1) {
            ...
            const groupTransactions = group.deliveryTransactions instanceof Set
                ...
                : new Set();
            if ([...groupTransactions].some((transaction) => !isDestinyDeliveryPlanCurrent(transaction))) {
                if (deliveredCount > 0) {
                ...
                return {
                    ...
                    highestStamp,
                    reason: getStaleDestinyDeliveryReasonFromTransactions(groupTransactions),
                };
            }
            const updateSummary = summarizeDestinyUpdates(group.updates);
            const violations = validateDestinyNotificationGroup(session, group.stamp, group.updates, {
            ...
            const payloadTuple = destiny.buildDestinyUpdatePayload(group.updates, group.waitForBubble, group.delayedTargetEvents);
            ...
            let attempt = overrideReentrantDestinyDeliveryAttempt(runtime, session, attemptDestinyNotificationDelivery(session, payloadTuple));
            if (attempt.status === "rejected" && attempt.retryable === true) {
                if ([...groupTransactions].every((transaction) => isDestinyDeliveryPlanCurrent(transaction))) {
                    ...
                    if ([...groupTransactions].every((transaction) => (isDestinyDeliveryPlanCurrent(transaction)))) {
                        attempt = overrideReentrantDestinyDeliveryAttempt(runtime, session, attemptDestinyNotificationDelivery(session, payloadTuple));
                    }
                    else {
                        attempt = {
                            status: "rejected",
            ...
            deliveredCount += 1;
            highestStamp = selectLaterDestinyStamp(highestStamp, group.stamp);
            if ([...groupTransactions].some((transaction) => !isDestinyDeliveryPlanCurrent(transaction))) {
                destroySessionAfterUnsafeDestinyDelivery(session);
                return {
                    accepted: false,
    ...
    function queueCollectedDestinyGroupsForDirectFlush(runtime, session, collectedGroups) {
        if (!runtime ||
            !session ||
            !Array.isArray(collectedGroups) ||
            collectedGroups.length === 0) {
    ...
    function retireDirectDestinyNotificationEntry(batch, sessionKey, expectedTransaction = null) {
        if (!batch ||
        ...
        for (const transaction of transactions) {
            if (transaction.state !== "committed" &&
                ...
                rollbackDestinyDeliveryTransaction(transaction, {
                    ...
                    skipCallbacks: shouldSkipStaleDestinyDeliveryCallbacks(transaction),
                });
            }
        }
    ...
    function ensureDirectDestinyDeliveryTransaction(runtime, session, rawDispatchStamp = null, options = {}) {
        ...
        const sessionKey = getDirectDeliverySessionKey(session);
        const existingTransaction = batch.deliveryTransactions instanceof Map
            ? batch.deliveryTransactions.get(sessionKey)
            : null;
        if (existingTransaction &&
            !isDestinyDeliveryPlanCurrent(existingTransaction)) {
            retireDirectDestinyNotificationEntry(batch, sessionKey, existingTransaction);
        }
        if (!(batch.deliveryTransactions instanceof Map)) {
    ...
    function scheduleDirectDestinyNotificationFlush(runtime, batch) {
        ...
        scheduleFlush(() => {
            ...
            if ([...batch.bySession.values()].some((queued) => (queued &&
                isDeliveryTransactionOwnedByActiveTick(runtime, queued.session, queued.deliveryTransaction)))) {
                return;
            }
            flushDirectDestinyNotificationBatch(runtime);
        });
    }
    function queueDirectDestinyNotificationGroup(runtime, session, groupDetails = {}) {
        if (!runtime ||
            !session ||
        ...
        const groupRawDispatchStamp = resolveOptionalDestinyStamp(groupDetails.rawDispatchStamp);
        const sessionKey = getDirectDeliverySessionKey(session);
        const suppliedDeliveryTransaction = resolveRootDeliveryTransaction(groupDetails.deliveryTransaction);
        if (suppliedDeliveryTransaction &&
            (suppliedDeliveryTransaction.session !== session ||
                suppliedDeliveryTransaction.state !== "planning")) {
            return 0;
        }
        if (rejectStaleIncomingDeliveryPlan(runtime, session, groupDetails, suppliedDeliveryTransaction)) {
            return 0;
        }
        registerDestinyDeliveryPlanGuard(suppliedDeliveryTransaction, groupDetails);
        if (suppliedDeliveryTransaction &&
            !isDestinyDeliveryPlanCurrent(suppliedDeliveryTransaction)) {
            rollbackDestinyDeliveryTransaction(suppliedDeliveryTransaction, {
                ...
                skipCallbacks: shouldSkipStaleDestinyDeliveryCallbacks(suppliedDeliveryTransaction),
            });
            return 0;
        }
        let batch = runtime._directDestinyNotificationBatch;
        const mappedBatchTransaction = batch && batch.deliveryTransactions instanceof Map
            ? batch.deliveryTransactions.get(sessionKey)
            : null;
        const queuedBatchEntry = batch && batch.bySession instanceof Map
            ? batch.bySession.get(sessionKey)
            : null;
        const batchDeliveryTransaction = resolveRootDeliveryTransaction(mappedBatchTransaction ||
            (queuedBatchEntry && queuedBatchEntry.deliveryTransaction));
        const sharesActiveDeliveryTransaction = Boolean(suppliedDeliveryTransaction &&
            batchDeliveryTransaction === suppliedDeliveryTransaction);
        const suppliedTransactionAlreadyRetained = Boolean(suppliedDeliveryTransaction &&
            isDeliveryTransactionRetainedByRuntime(runtime, session, suppliedDeliveryTransaction));
        if (batchDeliveryTransaction &&
            batchDeliveryTransaction !== suppliedDeliveryTransaction &&
            !isDestinyDeliveryPlanCurrent(batchDeliveryTransaction)) {
            const retiredStaleEntry = retireDirectDestinyNotificationEntry(batch, sessionKey, batchDeliveryTransaction);
            ...
            if (suppliedDeliveryTransaction &&
                ...
                suppliedDeliveryTransaction.state === "planning" &&
                isDestinyDeliveryTransactionGenerationCurrent(suppliedDeliveryTransaction)) {
        ...
        const deliveryTransaction = suppliedDeliveryTransaction ||
            ensureDirectDestinyDeliveryTransaction(runtime, session, groupRawDispatchStamp, groupDetails);
        registerDestinyDeliveryPlanGuard(deliveryTransaction, groupDetails);
        if (runtime._directDestinyNotificationBatch &&
            runtime._directDestinyNotificationBatch !== batch) {
            batch = runtime._directDestinyNotificationBatch;
        }
        if (deliveryTransaction.state !== "planning" ||
            !isDestinyDeliveryPlanCurrent(deliveryTransaction)) {
            rollbackDestinyDeliveryTransaction(deliveryTransaction, {
                reason: deliveryTransaction.state === "planning"
                    ? getStaleDestinyDeliveryReason(deliveryTransaction)
                    : "delivery-transaction-not-planning",
                skipCallbacks: deliveryTransaction.state === "planning"
        ...
        if (queued &&
            ...
            const retiredGenerationEntry = retireDirectDestinyNotificationEntry(batch, sessionKey, queued.deliveryTransaction);
        ...
        if (!queued) {
            ...
            mergeDestinyDeliveryPlanGuards(queued.deliveryTransaction, deliveryTransaction);
            mergeDestinyDeliveryTransactions(queued.deliveryTransaction, deliveryTransaction);
        }
        ...
        appendCollectedDestinyGroup(queued.groups, {
            ...
            buildSpatialTraceContext: shouldRecordSpatialTrace
                ? (split) => spatialTrace.recordDestinyGroupEnqueued(runtime, session, {
                    stamp: split.stamp,
        ...
        recordDestinyLedger(session, "destiny.group.queued", {
            ...
            waitForBubble: groupDetails.waitForBubble === true,
            contract: typeof groupDetails.contract === "string"
        ...
        try {
            ...
            for (const queued of batch.bySession.values()) {
                if (!queued ||
                    ...
                    !isDestinyDeliveryPlanCurrent(queued.deliveryTransaction)) {
                    if (queued && queued.deliveryTransaction) {
                        rollbackDestinyDeliveryTransaction(queued.deliveryTransaction, {
                            reason: Array.isArray(queued.groups) && queued.groups.length > 0
                                ? getStaleDestinyDeliveryReason(queued.deliveryTransaction)
                                : "empty-delivery-plan",
                            skipCallbacks: shouldSkipStaleDestinyDeliveryCallbacks(queued.deliveryTransaction),
                        });
                ...
                const result = flushCollectedDestinyGroups(runtime, queued.session, queued.groups);
                const deliveryTransaction = queued.deliveryTransaction;
                const fullyAccepted = Boolean(result &&
                    result.accepted === true &&
                    result.deliveredCount === result.plannedDeliveryCount &&
                    result.plannedDeliveryCount > 0);
                if (fullyAccepted) {
                    acceptedSessionCount += 1;
                    highestFlushedStamp = selectLaterDestinyStamp(highestFlushedStamp, result.highestStamp);
                    commitDestinyDeliveryTransaction(deliveryTransaction, result);
                }
                else {
                    rejectedSessionCount += 1;
                    rollbackDestinyDeliveryTransaction(deliveryTransaction, result || { reason: "delivery-rejected" });
                }
            ...
            if (!flushContext.completed) {
                ...
                for (const queued of batch.bySession.values()) {
                    if (queued && queued.session) {
                        destroySessionAfterUnsafeDestinyDelivery(queued.session, flushContext.error);
                    }
                }
            }
    ...
    function alignQueuedEntityDependentStamps(entries) {
        ...
        const insertionOrderedEntries = (Array.isArray(entries) ? entries : [])
            .slice()
            .sort((left, right) => toInt(left && left.order, 0) - toInt(right && right.order, 0));
        return insertionOrderedEntries.map((entry) => {
            ...
            if (entry.spatialTraceContext) {
                spatialTrace.attachDestinyUpdateContext(alignedUpdate, entry.spatialTraceContext);
            }
            return {
                ...entry,
    ...
    function queueTickDestinyPresentationUpdates(runtime, session, updates, options = {}) {
        if (!session ||
            !isReadyForDestiny(session) ||
        ...
        const queuedSendOptions = options &&
            options.sendOptions &&
            typeof options.sendOptions === "object"
            ? options.sendOptions
            : null;
        const normalizedQueuedSendOptions = normalizeQueuedPresentationSendOptions(queuedSendOptions);
        const existingDeliveryTransaction = getExistingDeliveryTransaction(runtime, session, normalizedQueuedSendOptions);
        const suppliedDeliveryTransaction = resolveRootDeliveryTransaction(normalizedQueuedSendOptions &&
            normalizedQueuedSendOptions._deliveryTransaction);
        if (suppliedDeliveryTransaction &&
            suppliedDeliveryTransaction.session !== session) {
            return 0;
        }
        if (rejectStaleIncomingDeliveryPlan(runtime, session, normalizedQueuedSendOptions, existingDeliveryTransaction)) {
            return 0;
        }
        let retainedUpdates = updates.filter((update) => update && Number.isFinite(Number(update.stamp)));
        if (retainedUpdates.length !== updates.length &&
            hasDeliveryHooks(normalizedQueuedSendOptions)) {
            rollbackUnplannedDeliveryCall(normalizedQueuedSendOptions, "invalid-tick-presentation-entry", existingDeliveryTransaction);
            return 0;
        }
        ...
        const unsuppressedUpdates = filterPendingRemovalDependentPayloads(session, retainedUpdates, normalizedQueuedSendOptions);
        if (unsuppressedUpdates.length !== retainedUpdates.length &&
            hasDeliveryHooks(normalizedQueuedSendOptions)) {
            rollbackUnplannedDeliveryCall(normalizedQueuedSendOptions, "entity-removal-pending", existingDeliveryTransaction);
            return 0;
        }
        ...
        const batch = runtime._tickDestinyPresentation;
        const queued = ensureTickDestinyDeliveryEntry(runtime, session, normalizedQueuedSendOptions);
        if (!queued) {
            return 0;
        }
        const getDedupeKey = typeof options.getDedupeKey === "function"
            ? options.getDedupeKey
            : null;
        const refreshStampAtFlush = options.refreshStampAtFlush === "currentVisible"
            ? "currentVisible"
        ...
        for (const update of retainedUpdates) {
            ...
            if (dedupeKey && queued.dedupeIndexes.has(dedupeKey)) {
                ...
                if (toInt(existingIndex, -1) < 0) {
                    ...
                    if (authoredTraceContext) {
                        const supersededTraceContext = existingEntry && existingEntry.spatialTraceContext;
                        spatialTrace.recordDestinyPresentationEnqueue(runtime, session, update, {
                            dedupeKey,
                            order: queuedEntry.order,
                            simulationTimeMs: runtime.getCurrentSimTimeMs(),
                            supersededActionId: supersededTraceContext && supersededTraceContext.actionId
        ...
        updateDestinyAuthorityState(session, {
            heldQueueState: {
                active: true,
                queuedCount: queued.updates.length,
                lastQueueStamp: resolveOptionalDestinyStamp(queued.updates.reduce((latest, entry) => selectLaterDestinyStamp(latest, normalizeDestinyStamp(entry && entry.update && entry.update.stamp)), null)),
            },
        });
        return retainedUpdates.length;
    }
        ...
        for (const storedQueued of storedEntries) {
            try {
                ...
                const queued = storedQueued
                    ...
                    : null;
                const deliveryTransaction = resolveRootDeliveryTransaction(queued && queued.deliveryTransaction);
                if (deliveryTransaction &&
                    !isDestinyDeliveryPlanCurrent(deliveryTransaction)) {
                    rollbackDestinyDeliveryTransaction(deliveryTransaction, {
                        reason: getStaleDestinyDeliveryReason(deliveryTransaction),
                        skipCallbacks: shouldSkipStaleDestinyDeliveryCallbacks(deliveryTransaction),
                    });
                ...
                const hasDirectDeliveryWork = Boolean(queued &&
                    queued.session &&
                    Array.isArray(queued.updates) &&
                    queued.updates.length === 0 &&
                    deliveryTransaction &&
                    hasQueuedDirectDeliveryWork(runtime, queued.session, deliveryTransaction));
                ...
                const flushCurrentVisibleStamp = queued.updates.some((entry) => (entry && (entry.refreshStampAtFlush === "currentVisible" ||
                    entry.refreshStampAtFlush === "currentVisibleIfStale")))
                    ? runtime.getCurrentVisibleSessionDestinyStamp(queued.session, flushNowMs)
                    : null;
                const refreshedEntries = queued.updates
                    ...
                    if (!entry ||
                        ...
                        (entry.refreshStampAtFlush === "currentVisibleIfStale" &&
                            compareDestinyStamps(entry.update.stamp, flushCurrentVisibleStamp) >= 0)) {
                        return entry;
                    }
                    const refreshedUpdate = {
                        ...entry.update,
                        stamp: flushCurrentVisibleStamp,
                        payload: destiny.restampPayloadState(entry.update.payload, flushCurrentVisibleStamp),
                    };
                    if (entry.spatialTraceContext) {
                        spatialTrace.attachDestinyUpdateContext(refreshedUpdate, entry.spatialTraceContext);
                    }
                    return {
                        ...entry,
                ...
                const orderedEntries = alignQueuedEntityDependentStamps(refreshedEntries)
                    .slice()
                    .sort((left, right) => {
                    const stampOrder = compareDestinyStamps(left && left.update && left.update.stamp, right && right.update && right.update.stamp);
                    if (stampOrder !== 0) {
                        return stampOrder;
                    }
                ...
                const flushQueuedGroup = () => {
                    ...
                    Object.defineProperty(collectedSendOptions, drainingTickPresentationPrefixToken, {
                        configurable: false,
                        enumerable: false,
                        value: true,
                        writable: false,
                    });
                    runtime.sendDestinyUpdates(queued.session, currentGroupUpdates, false, collectedSendOptions);
                };
                for (const entry of orderedEntries) {
                    ...
                    const entrySendOptions = normalizeQueuedPresentationSendOptions(entry && entry.sendOptions);
                    const entrySignature = buildQueuedPresentationSendOptionsSignature(entrySendOptions);
                    if (currentGroupUpdates.length > 0 &&
                ...
                flushQueuedGroup();
                const queuedGroupCount = queueCollectedDestinyGroupsForDirectFlush(runtime, queued.session, collectedGroups);
                updateDestinyAuthorityState(queued.session, {
                    heldQueueState: {
                        active: !finalDrain,
                        queuedCount: 0,
                ...
                rollbackTickPresentationDrainFailure(runtime, batch, storedEntries, error);
                throw error;
            }
        }
        if (finalDrain) {
    ...
    function sendDestinyUpdates(runtime, session, payloads, waitForBubble = false, options = {}) {
        ...
        const existingDeliveryTransaction = getExistingDeliveryTransaction(runtime, session, options);
        const suppliedDeliveryTransaction = resolveRootDeliveryTransaction(options && options._deliveryTransaction);
        if (suppliedDeliveryTransaction &&
            suppliedDeliveryTransaction.session !== session) {
            return null;
        }
        if (rejectStaleIncomingDeliveryPlan(runtime, session, options, existingDeliveryTransaction)) {
            return null;
        }
        const retainedPayloads = options && options[drainingTickPresentationPrefixToken] === true
            ? payloads
            : filterPendingRemovalDependentPayloads(session, payloads, options);
        if (retainedPayloads.length !== payloads.length &&
            hasDeliveryHooks(options)) {
            rollbackUnplannedDeliveryCall(options, "entity-removal-pending", existingDeliveryTransaction);
            return null;
        }
        ...
        const collectNotificationGroups = Array.isArray(options && options._collectNotificationGroups)
            ? options._collectNotificationGroups
            : null;
        if (suppliedDeliveryTransaction &&
            (suppliedDeliveryTransaction.state !== "planning" ||
                !isDestinyDeliveryPlanCurrent(suppliedDeliveryTransaction))) {
            if (suppliedDeliveryTransaction.state !== "committed" &&
                suppliedDeliveryTransaction.state !== "rolled-back") {
                const rollbackStaleSuppliedTransaction = getStaleDestinyDeliveryReason(suppliedDeliveryTransaction) ===
                    "delivery-plan-replaced" &&
                    !isDeliveryTransactionRetainedByRuntime(runtime, session, suppliedDeliveryTransaction)
        ...
        const shouldUseActiveTickDelivery = Boolean(!collectNotificationGroups &&
            hasActiveTickDestinyPresentationBatch(runtime));
        const tickDeliveryEntry = shouldUseActiveTickDelivery
            ? ensureTickDestinyDeliveryEntry(runtime, session, {}, suppliedDeliveryTransaction)
            : null;
        if (shouldUseActiveTickDelivery && !tickDeliveryEntry) {
            return null;
        }
        if (shouldUseActiveTickDelivery &&
        ...
        const deliveryTransaction = (tickDeliveryEntry && tickDeliveryEntry.deliveryTransaction) ||
            suppliedDeliveryTransaction ||
            (collectNotificationGroups
                ? createDestinyDeliveryTransaction(session, {})
                : ensureDirectDestinyDeliveryTransaction(runtime, session, currentRawDispatchStamp, {}));
        if (!deliveryTransaction ||
        ...
        const rollbackAfterPlanningFailure = (reason, error = null) => {
            ...
            if (!deliveryHooksRegisteredForCall &&
                ...
                !(deliveryTransaction.rollbackCallbacks instanceof Set &&
                    deliveryTransaction.rollbackCallbacks.has(options.onDeliveryRollback))) {
                try {
                    options.onDeliveryRollback({
                        error,
                        planningOnly: true,
        ...
        const deliveryTransactionOwnedByActiveTick = isDeliveryTransactionOwnedByActiveTick(runtime, session, deliveryTransaction);
        const shouldTraceMissileDispatch = shouldLogMissilePayloadGroup(payloads) ||
            payloads.some((payload) => (payload &&
        ...
        if (shouldSplitMixedFreshAcquirePayloads(payloads, options)) {
            ...
            const shouldFlushSplitTransaction = !collectNotificationGroups &&
            ...
            try {
                for (const group of payloadGroups) {
                    ...
                    const emittedStamp = sendDestinyUpdates(runtime, session, group.updates, allowWaitForBubble, groupOptions);
                    if (!hasDestinyStamp(emittedStamp)) {
                        rollbackAfterPlanningFailure("delivery-planning-rejected");
                        return null;
                    }
                    highestEmittedGroupStamp = selectLaterDestinyStamp(highestEmittedGroupStamp, emittedStamp);
        ...
        if (shouldTraceMissileDispatch) {
            logMissileDebug("destiny.send-request", {
                ...
                rawSimTimeMs: roundNumber(rawSimTimeMs, 3),
                waitForBubble,
                sendReason: typeof options.missileDebugReason === "string"
        ...
        const collectNotificationBaseOrder = Math.max(0, toInt(options && options._collectNotificationOrder, 0));
        let emittedGroupOrder = 0;
        const authorityStateBeforeSend = snapshotDestinyAuthorityState(session);
        const delayedTargetEvents = normalizeDelayedTargetEvents(options && options.delayedTargetEvents);
        let lastFreshAcquireLifecycleStamp = resolveOptionalDestinyStamp(authorityStateBeforeSend && authorityStateBeforeSend.lastFreshAcquireLifecycleStamp, session && session._space && session._space.lastFreshAcquireLifecycleStamp);
        let lastMissileLifecycleStamp = resolveOptionalDestinyStamp(authorityStateBeforeSend && authorityStateBeforeSend.lastMissileLifecycleStamp, session && session._space && session._space.lastMissileLifecycleStamp);
        const ownerMissileLifecycleTupleBeforeSend = resolveDestinyAuthorityLaneTuple({
        ...
        let lastOwnerMissileLifecycleStamp = ownerMissileLifecycleTupleBeforeSend.stamp;
        let lastOwnerMissileLifecycleAnchorStamp = ownerMissileLifecycleTupleBeforeSend.anchorStamp;
        let lastOwnerMissileLifecycleRawDispatchStamp = ownerMissileLifecycleTupleBeforeSend.rawDispatchStamp;
        const ownerMissileFreshAcquireTupleBeforeSend = resolveDestinyAuthorityLaneTuple({
            authorityState: authorityStateBeforeSend,
            legacyState: session && session._space,
        ...
        let lastOwnerMissileFreshAcquireStamp = ownerMissileFreshAcquireTupleBeforeSend.stamp;
        let lastOwnerMissileFreshAcquireAnchorStamp = ownerMissileFreshAcquireTupleBeforeSend.anchorStamp;
        let lastOwnerMissileFreshAcquireRawDispatchStamp = ownerMissileFreshAcquireTupleBeforeSend.rawDispatchStamp;
        const ownerNonMissileCriticalTupleBeforeSend = resolveDestinyAuthorityLaneTuple({
            authorityState: authorityStateBeforeSend,
            legacyState: session && session._space,
            stampKey: "lastOwnerNonMissileCriticalStamp",
            rawDispatchKey: "lastOwnerNonMissileCriticalRawDispatchStamp",
        });
        let lastOwnerNonMissileCriticalStamp = ownerNonMissileCriticalTupleBeforeSend.stamp;
        let lastOwnerNonMissileCriticalRawDispatchStamp = ownerNonMissileCriticalTupleBeforeSend.rawDispatchStamp;
        const flushGroup = () => {
            ...
            const emitGroupedUpdates = (updatesGroup, emitOptions = {}) => {
                ...
                const minimumEntityPresentationStamp = getEntityPresentationFloor(deliveryTransaction, entityPresentationIDs);
                const authorityEmitOptions = hasDestinyStamp(minimumEntityPresentationStamp)
                    ? {
                        ...emitOptions,
                ...
                if (authorityPlan) {
                    const { authorityJourney, updates: authorityUpdates, finalStamp: authorityStamp, originalStamp: authorityOriginalStamp, traceDetails: authorityTraceDetails, currentSessionStamp: authorityCurrentSessionStamp, flags: authorityFlags, } = authorityPlan;
                    if (hasDestinyStamp(minimumEntityPresentationStamp) &&
                        isDestinyStampAfter(authorityStamp, minimumEntityPresentationStamp)) {
                        destinyAuthority.rejectGroupJourney(authorityJourney, {
                            session,
                            reason: `entity order floor ${minimumEntityPresentationStamp} exceeds trusted ceiling at ${authorityStamp}`,
                        ...
                        const conflictError = new Error("Destiny entity order floor exceeds its trusted authority ceiling");
                        conflictError.code = "DESTINY_ENTITY_ORDER_CEILING_CONFLICT";
                        throw conflictError;
                    }
                    if (spatialTrace.isEnabled()) {
                        const correlationCount = Math.min(updatesGroup.length, authorityUpdates.length);
                        for (let index = 0; index < correlationCount; index += 1) {
                            spatialTrace.propagateDestinyUpdateContext(updatesGroup[index], authorityUpdates[index], session);
                        }
                    }
                    if (isDestinyStampAfter(authorityStamp, authorityFlags.previousLastSentDestinyStamp) &&
                        ...
                        recordDestinyLedger(session, "destiny.group.rejected", {
                            ...
                            attemptedStamp: authorityStamp >>> 0,
                            previousLastSentDestinyStamp: authorityFlags.previousLastSentDestinyStamp,
                            originalStamp: authorityOriginalStamp,
                            contract: authorityJourney.contract,
                            updates: summarizeDestinyUpdates(authorityUpdates),
                        });
                    ...
                    recordEntityPresentationFloor(deliveryTransaction, entityPresentationIDs, authorityStamp);
                    ...
                    const authorityLegacyState = destinyAuthority.applyLegacySessionEmissionState(authorityJourney, {
                        session,
                    ...
                    const authoritySessionAfter = destinyAuthority.completeGroupJourney(authorityJourney, {
                        session,
                        originalStamp: authorityOriginalStamp,
                        finalStamp: authorityStamp >>> 0,
                        currentSessionStamp: authorityCurrentSessionStamp,
                        isCritical: authorityFlags.isOwnerCriticalGroup ||
                            authorityFlags.isSetStateGroup ||
                            authorityFlags.isMissileLifecycleGroup,
                        isFreshAcquireLifecycleGroup: authorityFlags.isFreshAcquireLifecycleGroup,
                        isMissileLifecycleGroup: authorityFlags.isMissileLifecycleGroup,
                        flags: authorityFlags,
                        restampSteps: authorityTraceDetails
                            ? authorityTraceDetails.restampSteps
                            : [],
                    ...
                    if (authoritySessionAfter) {
                        ...
                        lastFreshAcquireLifecycleStamp = resolveOptionalDestinyStamp(authoritySessionAfter.lastFreshAcquireLifecycleStamp, lastFreshAcquireLifecycleStamp);
                        lastMissileLifecycleStamp = resolveOptionalDestinyStamp(authoritySessionAfter.lastMissileLifecycleStamp, lastMissileLifecycleStamp);
                        const completedOwnerMissileLifecycleTuple = resolveDestinyAuthorityLaneTuple({
                        ...
                        const completedOwnerMissileFreshAcquireTuple = resolveDestinyAuthorityLaneTuple({
                            authorityState: authoritySessionAfter,
                            legacyState: legacyTrackedLaneState,
                            stampKey: "lastOwnerMissileFreshAcquireStamp",
                            rawDispatchKey: "lastOwnerMissileFreshAcquireRawDispatchStamp",
                        ...
                        const completedOwnerNonMissileCriticalTuple = resolveDestinyAuthorityLaneTuple({
                            authorityState: authoritySessionAfter,
                            legacyState: legacyTrackedLaneState,
                            stampKey: "lastOwnerNonMissileCriticalStamp",
                            rawDispatchKey: "lastOwnerNonMissileCriticalRawDispatchStamp",
                    ...
                    if (authorityTraceDetails) {
                        ...
                        authorityTraceDetails.sessionAfter = buildMissileSessionSnapshot(runtime, session, rawSimTimeMs);
                        authorityTraceDetails.authoritySessionAfter = authoritySessionAfter;
                        authorityTraceDetails.sessionMutation = buildMissileSessionMutation(authorityTraceDetails.sessionBefore, authorityTraceDetails.sessionAfter);
                    ...
                    firstGroup = false;
                    highestEmittedStamp = selectLaterDestinyStamp(highestEmittedStamp, normalizeDestinyStamp(authorityStamp));
                    return authorityStamp >>> 0;
                }
                firstGroup = false;
                return null;
            };
            const hasMixedOwnerMissileFreshAcquireAndLifecycle = (groupedUpdates.some((payload) => payload &&
                payload.freshAcquireLifecycleGroup === true &&
                payload.ownerMissileLifecycleGroup === true) &&
                groupedUpdates.some((payload) => payload &&
                    payload.ownerMissileLifecycleGroup === true &&
                    payload.freshAcquireLifecycleGroup !== true));
            if (hasMixedOwnerMissileFreshAcquireAndLifecycle) {
                ...
                const freshAcquireUpdates = groupedUpdates.filter((payload) => payload && payload.freshAcquireLifecycleGroup === true);
                const lifecycleUpdates = groupedUpdates.filter((payload) => !payload || payload.freshAcquireLifecycleGroup !== true);
        ...
        try {
            for (let rawPayloadOrder = 0; rawPayloadOrder < payloads.length; rawPayloadOrder += 1) {
                ...
                const authoredTraceContext = spatialTrace.isEnabled()
                    ...
                    : null;
                const payload = runtime.prepareDestinyUpdateForSession(session, rawPayload, rawSimTimeMs, options);
                if (authoredTraceContext) {
        ...
        if (shouldTraceMissileDispatch) {
            const sessionAfterSend = buildMissileSessionSnapshot(runtime, session, rawSimTimeMs);
            logMissileDebug("destiny.send-complete", {
                ...
                sessionMutation: buildMissileSessionMutation(sessionBeforeSend, sessionAfterSend),
                emittedGroups: emittedGroupSummaries,
            });
        }
        return highestEmittedStamp;
    }
    function sendDestinyBatch(runtime, session, payloads, waitForBubble = false, options = {}) {
        return sendDestinyUpdates(runtime, session, payloads, waitForBubble, options);
    }
    function sendDestinyUpdatesIndividually(runtime, session, payloads, waitForBubble = false, options = {}) {
        if (!session || payloads.length === 0) {
            return;
        }
        for (let index = 0; index < payloads.length; index += 1) {
            runtime.sendDestinyUpdates(session, [payloads[index]], waitForBubble && index === 0, options);
        }
    }
...
//# sourceMappingURL=destinyDispatch.js.map