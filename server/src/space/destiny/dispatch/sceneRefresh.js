...
const { projectPreviouslySentDestinyLane, } = require("../delivery/deliveryPolicy.js");
const { resolveStateRefreshStamp, } = require("../delivery/sync.js");
...
function createMovementSceneRefresh(deps = {}) {
    ...
    return {
        sendStateRefresh(runtime, session, egoEntity, stampOverride = null, options = {}) {
            ...
            const rawSimTimeMs = options.nowMs === undefined || options.nowMs === null
            ...
            const currentImmediateSessionStamp = runtime.getImmediateDestinyStampForSession(session, currentSessionStamp);
            ...
            const lastOwnerMissileLifecycleStamp = resolveOptionalDestinyStamp(ownerMissileLifecycleLane.stamp);
            const lastOwnerMissileFreshAcquireStamp = resolveOptionalDestinyStamp(authorityState && authorityState.lastOwnerMissileFreshAcquireStamp, session._space && session._space.lastOwnerMissileFreshAcquireStamp);
            const lastSentLooksOwnerMissileLifecycle = hasDestinyStamp(rawLastSentDestinyStamp) &&
            ...
            const lastSentRawDispatchDelta = hasDestinyStamp(lastSentDestinyRawDispatchStamp)
                ? getDestinyStampForwardDistance(lastSentDestinyRawDispatchStamp, currentRawDispatchStamp)
                : null;
            const projectedLastSentLane = hasDestinyStamp(lastSentDestinyStamp) &&
            ...
            const ownerMissileLifecycleRawDispatchDelta = hasDestinyStamp(lastOwnerMissileLifecycleRawDispatchStamp)
                ? getDestinyStampForwardDistance(lastOwnerMissileLifecycleRawDispatchStamp, currentRawDispatchStamp)
                : null;
            const projectedOwnerMissileLifecycleLane = hasDestinyStamp(lastOwnerMissileLifecycleStamp) &&
                ownerMissileLifecycleRawDispatchDelta !== null &&
                ownerMissileLifecycleRawDispatchDelta > 0 &&
                ownerMissileLifecycleRawDispatchDelta <= 1
                ? projectPreviouslySentDestinyLane(lastOwnerMissileLifecycleStamp, lastOwnerMissileLifecycleRawDispatchStamp, currentRawDispatchStamp)
                : null;
            const lastPilotCommandMovementStamp = resolveOptionalDestinyStamp(authorityState && authorityState.lastOwnerCommandStamp, session._space && session._space.lastPilotCommandMovementStamp);
            ...
            const { recentFutureLastSentLane, recentProjectedLastSentLane, recentOwnerMissileLifecycleLane, recentProjectedOwnerMissileLifecycleLane, recentOwnerMovementLane, monotonicRefreshFloorBase, liveStateResetFloor, stamp, } = refreshStampState;
            ...
            runtime.sendDestinyUpdates(session, [
                {
                    stamp,
                    payload: destiny.buildSetStatePayload(stamp, runtime.system, egoEntity.itemID, stateRefreshVisibleEntities, simFileTime, typeof buildDbuffStateEntriesForSession === "function"
                ...
                missileDebugReason: typeof options.reason === "string"
            ...
            if (options.skipHudIconReseed !== true &&
                ...
                notifyActiveHostileJamStatesToSession(runtime, session, egoEntity, stateRefreshVisibleEntities, rawSimTimeMs);
            }
            if (options.skipHudIconReseed !== true &&
                typeof notifyActiveCommandBurstHudStatesToSession === "function") {
                notifyActiveCommandBurstHudStatesToSession(runtime, session, egoEntity, rawSimTimeMs);
            }
        },
        scheduleWatcherMovementAnchor(runtime, entity, now = runtime.getCurrentSimTimeMs(), reason = "movement") {
            if (!entity) {
                return false;
            }
