...
const { hasDestinyStamp, isDestinyStampAfter, normalizeDestinyStamp, selectFurthestDestinyStamp, } = require("../delivery/stamps");
function createMovementWatcherCorrections(deps = {}) {
    const { destiny, toInt, ACTIVE_SUBWARP_WATCHER_CORRECTION_INTERVAL_MS, ACTIVE_SUBWARP_WATCHER_POSITION_CORRECTION_INTERVAL_MS, ENABLE_PILOT_WARP_ACTIVE_CORRECTIONS, PILOT_WARP_ACTIVATION_DELAY_DESTINY_TICKS, WATCHER_CORRECTION_INTERVAL_MS, WATCHER_POSITION_CORRECTION_INTERVAL_MS, } = deps;
    ...
    function resolveWatcherCorrectionDispatch(options = {}) {
        ...
        if (entity.mode === "WARP") {
            ...
            const warpCommandStamp = normalizeDestinyStamp(warpState && warpState.commandStamp, 0);
            const warpCorrectionStamp = selectFurthestDestinyStamp(warpCommandStamp, [warpCommandStamp, runtime.getMovementStamp(now)]);
            const inActivePilotWarpPhase = !entity.pendingWarp &&
                warpState !== null &&
                hasDestinyStamp(warpState.commandStamp);
            const shouldSendPilotWarpCorrection = ENABLE_PILOT_WARP_ACTIVE_CORRECTIONS &&
                inActivePilotWarpPhase &&
                isDestinyStampAfter(warpCommandStamp, warpCorrectionStamp) &&
                warpCorrectionStamp !==
                    toInt(entity.lastWarpPositionBroadcastStamp, -1);
            if (shouldSendPilotWarpCorrection) {
                const pilotWarpCorrectionUpdates = buildPilotWarpCorrectionUpdates(entity, warpCorrectionStamp);
...
//# sourceMappingURL=watcherCorrections.js.map