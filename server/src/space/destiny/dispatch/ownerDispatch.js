...
const { resolveGotoDirectionFromUpdates, serializeDestinyDirectionHeading, } = require("../protocol/direction");
const { resolveOwnerMovementRestampState, } = require("../delivery/deliveryPolicy.js");
const { rebuildOwnerKinematicUpdatesForProjectedStamp, } = require("../projection/entityProjection.js");
...
const { advanceDestinyStamp, getDestinyStampForwardDistance, hasDestinyStamp, isDestinyStampAfter, normalizeDestinyStamp, resolveOptionalDestinyStamp, selectLaterDestinyStamp, } = require("../delivery/stamps");
...
function shouldRefreshOwnerCommandAnchor(options = {}) {
    ...
    const previousAnchorStamp = resolveOptionalDestinyStamp(options.previousAnchorStamp);
    ...
    return (!hasDestinyStamp(previousStamp) ||
...
function resolveAtomicOwnerMovementLaneWrite(options = {}) {
    ...
    const previousStamp = resolveOptionalDestinyStamp(options.previousStamp);
    const previousRawDispatchStamp = resolveOptionalDestinyStamp(options.previousRawDispatchStamp);
    const previousAnchorStamp = resolveOptionalDestinyStamp(options.previousAnchorStamp);
    const emittedStamp = resolveOptionalDestinyStamp(options.emittedStamp);
    const currentRawDispatchStamp = resolveOptionalDestinyStamp(options.currentRawDispatchStamp);
    const nextAnchorStamp = tracksAnchor
        ? resolveOptionalDestinyStamp(options.nextAnchorStamp)
        : null;
    const candidateTupleIsComplete = hasDestinyStamp(emittedStamp) &&
        hasDestinyStamp(currentRawDispatchStamp) &&
        (!tracksAnchor || hasDestinyStamp(nextAnchorStamp));
    const matchingCandidateRawDoesNotRegress = !hasDestinyStamp(previousRawDispatchStamp) ||
        currentRawDispatchStamp === previousRawDispatchStamp ||
        isDestinyStampAfter(previousRawDispatchStamp, currentRawDispatchStamp);
    const candidateEstablishesLane = candidateTupleIsComplete &&
...
function createMovementOwnerDispatch(deps = {}) {
    const { buildMissileSessionMutation, buildMissileSessionSnapshot, cloneVector, directionsNearlyMatch, isReadyForDestiny, logMissileDebug, normalizeVector, roundNumber, sessionMatchesIdentity, summarizeMissileUpdatesForLog, summarizeVector, toFiniteNumber, toInt, advanceMovement, nativeSubwarpController, cloneDynamicEntityForDestinyPresentation, destiny, DEFAULT_RIGHT, MICHELLE_HELD_FUTURE_DESTINY_LEAD, PILOT_WARP_ACTIVATION_DELAY_DESTINY_TICKS, MICHELLE_POST_HELD_FUTURE_DESTINY_LEAD = 0, PILOT_WARP_HISTORY_SAFE_DESTINY_LEAD = PILOT_WARP_ACTIVATION_DELAY_DESTINY_TICKS, } = deps;
    return {
        broadcastPilotCommandMovementUpdates(runtime, session, updates, nowMs = runtime.getCurrentSimTimeMs(), options = {}) {
            ...
            const ownerShouldReceiveDirectUpdates = session &&
            ...
            if (ownerShouldReceiveDirectUpdates) {
                ...
                const ownerMovementUpdates = runtime.filterMovementUpdatesForSession(session, preparedUpdates);
                ...
                const liveOwnerSessionStamp = runtime.getCurrentSessionDestinyStamp(session, nowMs);
                const quietWindowActive = runtime.isSessionInPilotWarpQuietWindow(session, nowMs);
                const currentVisibleOwnerStamp = runtime.getCurrentVisibleSessionDestinyStamp(session, nowMs);
                ...
                const currentPresentedOwnerStamp = runtime.getCurrentPresentedSessionDestinyStamp(session, nowMs, presentedOwnerMaximumFutureLead);
                const quietWindowMinimumStamp = quietWindowActive
                    ? selectLaterDestinyStamp(session._space && session._space.pilotWarpQuietUntilStamp, runtime.getHistorySafeSessionDestinyStamp(session, nowMs, PILOT_WARP_HISTORY_SAFE_DESTINY_LEAD, PILOT_WARP_HISTORY_SAFE_DESTINY_LEAD))
                    : null;
                const lastFreshAcquireLifecycleStamp = resolveOptionalDestinyStamp(ownerAuthorityState && ownerAuthorityState.lastFreshAcquireLifecycleStamp, session._space && session._space.lastFreshAcquireLifecycleStamp);
                if (ownerMovementUpdates.length > 0) {
                    ...
                    const lastOwnerNonMissileCriticalStamp = ownerNonMissileCriticalLane.stamp;
                    const lastOwnerNonMissileCriticalRawDispatchStamp = ownerNonMissileCriticalLane.rawDispatchStamp;
                    ...
                    const lastOwnerMissileLifecycleStamp = ownerMissileLifecycleLane.stamp;
                    const lastOwnerMissileLifecycleRawDispatchStamp = ownerMissileLifecycleLane.rawDispatchStamp;
                    const ownerMissileFreshAcquireLane = resolveDestinyAuthorityLaneTuple({
                        authorityState: ownerAuthorityState,
                        legacyState: session._space,
                        stampKey: "lastOwnerMissileFreshAcquireStamp",
                        rawDispatchKey: "lastOwnerMissileFreshAcquireRawDispatchStamp",
                    });
                    const lastOwnerMissileFreshAcquireStamp = ownerMissileFreshAcquireLane.stamp;
                    const lastOwnerMissileFreshAcquireRawDispatchStamp = ownerMissileFreshAcquireLane.rawDispatchStamp;
                    ...
                    const ownerRestampState = resolveOwnerMovementRestampState({
                        ...
                        recentEmittedOwnerCriticalMaxLead: MICHELLE_HELD_FUTURE_DESTINY_LEAD +
                            ...
                            PILOT_WARP_HISTORY_SAFE_DESTINY_LEAD,
                        ignoreRecentOwnerCriticalFloor: options.ignoreRecentOwnerCriticalFloor === true,
                        ignoreOwnerDirectEchoHistoryFloor: options.ignoreOwnerDirectEchoHistoryFloor === true ||
                            (options.sendOptions &&
                        ...
                        getPendingHistorySafeStamp: (authoredStamp, minimumLead) => (runtime.getPendingHistorySafeSessionDestinyStamp(session, authoredStamp, nowMs, minimumLead)),
                        defaultRight: DEFAULT_RIGHT,
                    });
                    const { currentOwnerPilotCommandDirection, previousOwnerPilotCommandDirection, ownerDirectEchoMinimumStamp, repeatedOwnerPilotCommandLane, reusableHeldOwnerPilotCommandLane, nextDistinctOwnerPilotCommandLane, suppressSameRawDistinctFutureOwnerEcho, earlierTickOwnerPilotCommandMatches, recentPresentedFreshAcquireLane, recentOwnerNonMissileCriticalLane, recentOwnerMissileLifecycleLane, recentOwnerFreshAcquireLane, recentBufferedOwnerCriticalFloor, ownerVisibleStamp, ownerMinimumStamp, postFreshAcquireOwnerSteeringFloor, presentedNonCriticalOwnerEchoFloor, ownerStampFloor, ownerUpdates, } = ownerRestampState;
                    logMissileDebug("movement.owner-restamp", {
                        ...
                        ownerMovementFloor: {
                            ...
                            ownerMinimumStamp,
                            ownerDirectEchoLeadOverride: toInt(options.ownerDirectEchoLeadOverride, 0) || null,
                            ...
                            previousOwnerPilotCommandAnchorStamp,
                            previousOwnerPilotCommandRawDispatchStamp,
                            previousOwnerPilotCommandDirection: summarizeVector(previousOwnerPilotCommandDirection),
                            currentOwnerPilotCommandDirection: summarizeVector(currentOwnerPilotCommandDirection),
                    ...
                    const sessionBeforeOwnerMovementSend = buildMissileSessionSnapshot(runtime, session, nowMs);
                    const ownerEntity = typeof runtime.getShipEntityForSession === "function"
                    ...
                    const ownerUpdatesForSend = suppressSameRawDistinctFutureOwnerEcho
                        ...
                        : rebuildOwnerKinematicUpdatesForProjectedStamp({
                            ...
                            entity: ownerEntity,
                            rawNowMs: nowMs,
                            currentSimTimeMs: typeof runtime.getCurrentSessionSimTimeMs === "function"
                    ...
                    const highestQueuedOwnerMovementStamp = ownerUpdatesForSend.reduce((latestStamp, update) => selectLaterDestinyStamp(latestStamp, normalizeDestinyStamp(update && update.stamp)), null);
                    const useTickPresentationBatch = !suppressSameRawDistinctFutureOwnerEcho &&
                    ...
                    if (suppressSameRawDistinctFutureOwnerEcho) {
                        ...
                        runtime.queueTickDestinyPresentationUpdates(session, ownerUpdatesForSend, {
                            sendOptions: {
                        ...
                        emittedOwnerMovementStamp = runtime.sendDestinyUpdates(session, ownerUpdatesForSend, false, {
                            destinyAuthorityContract: DESTINY_CONTRACTS.OWNER_PILOT_COMMAND,
                    ...
                    const emittedOwnerGotoDirection = resolveGotoDirectionFromUpdates({
                        ...
                        expectedStamp: resolveOptionalDestinyStamp(emittedOwnerMovementStamp),
                    ...
                    if (session._space && hasDestinyStamp(emittedOwnerMovementStamp)) {
                        const ownerNonMissileLaneWrite = resolveAtomicOwnerMovementLaneWrite({
                            ...
                            previousRawDispatchStamp: lastOwnerNonMissileCriticalRawDispatchStamp,
                        ...
                        updateDestinyAuthorityState(session, {
                            ...
                            lastOwnerCommandHeadingHash: currentOwnerPilotCommandDirection
                                ? serializeDestinyDirectionHeading(currentOwnerPilotCommandDirection)
                                : "",
                        });
                        if (hasDestinyStamp(previousOwnerPilotCommandStamp) &&
                            currentOwnerPilotCommandDirection) {
                            session._space.lastPilotCommandDirection = cloneVector(currentOwnerPilotCommandDirection);
                        }
                    }
                    const sessionAfterOwnerMovementSend = buildMissileSessionSnapshot(runtime, session, nowMs);
                    logMissileDebug("movement.owner-send-complete", {
                        ...
                        sessionMutation: buildMissileSessionMutation(sessionBeforeOwnerMovementSend, sessionAfterOwnerMovementSend),
                    });
                }
                runtime.broadcastMovementUpdates(preparedUpdates, session, options.sendOptions || {});
                return true;
            }
            runtime.broadcastMovementUpdates(preparedUpdates, excludedSession, options.sendOptions || {});
            return true;
        },
    };
}
...
//# sourceMappingURL=ownerDispatch.js.map