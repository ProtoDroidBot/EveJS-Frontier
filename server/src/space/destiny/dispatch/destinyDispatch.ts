...
function createMovementDestinyDispatch(deps: Record<PropertyKey, any> = {}) {
  const {
  ...
  const registerDestinyDeliveryPlanGuard = (transaction, options: Record<PropertyKey, any> = {}) => {
  ...
  const getDestinyDeliveryPlanGuard = (options: Record<PropertyKey, any> = {}) => (
  ...
  function getExistingDeliveryTransaction(runtime, session, options: Record<PropertyKey, any> = {}) {
  ...
  function abortActiveTickDestinyPresentationBatch(
    ...
    const abortedTransactions = new Set<any>();
  ...
  function appendCollectedDestinyGroup(
    collectedGroups,
    groupDetails: Record<PropertyKey, any> = {},
    options: Record<PropertyKey, any> = {},
  ) {
  ...
  function rollbackTickPresentationDrainFailure(
    ...
    const rolledBackTransactions = new Set<any>();
  ...
  function recordDestinyLedger(session, event, details: Record<PropertyKey, any> = {}) {
  ...
  function flushDirectDestinyNotificationBatch(runtime) {
    ...
    try {
      ...
      const completedTransactions = new Set<any>();
  ...
  function shouldSplitMixedFreshAcquirePayloads(payloads, options: Record<PropertyKey, any> = {}) {
    return (
      options &&
  ...
  function buildNonFreshMixedPayloadSendOptions(baseOptions: Record<PropertyKey, any> = {}) {
    const nextOptions = {
  ...
  function splitContiguousFreshAcquirePayloadGroups(payloads: any[] = []) {
    const groups: any[] = [];
  ...
  function queueTickDestinyPresentationUpdates(
    ...
    const queuedSendOptions =
      ...
      typeof options.sendOptions === "object"
        ? options.sendOptions
        : null;
    const normalizedQueuedSendOptions: Record<PropertyKey, any> =
  ...
  function drainTickDestinyPresentationBatch(runtime, options: Record<PropertyKey, any> = {}) {
    if (!runtime.hasActiveTickDestinyPresentationBatch()) {
      return 0;
    }
  ...
  function sendDestinyUpdates(
    ...
    let groupedUpdates: any[] = [];
    let currentStamp = null;
    let firstGroup = true;
    let highestEmittedStamp = null;
    const emittedGroupSummaries: any[] = [];
    ...
    const flushGroup = () => {
      ...
      const emitGroupedUpdates = (updatesGroup, emitOptions: Record<PropertyKey, any> = {}) => {
        ...
        if (authorityPlan) {
          ...
          if (
            ...
            const conflictError: Error & Record<PropertyKey, any> = new Error(
