...
const { isSteeringPayloadName, } = require("../protocol/payloads.js");
const { resolveGotoDirectionFromUpdates, serializeDestinyDirectionHeading, } = require("../protocol/direction.js");
const { projectPreviouslySentDestinyLane, resolveOwnerMovementRestampState, } = require("../delivery/deliveryPolicy.js");
const { clampQueuedSubwarpUpdates, } = require("../delivery/sync.js");
const { tagUpdatesRequireExistingVisibility, } = require("./dispatchUtils.js");
const { resolveAtomicOwnerMovementLaneWrite, shouldRefreshOwnerCommandAnchor, } = require("./ownerDispatch.js");
const { DESTINY_CONTRACTS, } = require("../authority/destinyContracts.js");
const { resolveDestinyAuthorityCommandTuple, resolveDestinyAuthorityLaneTuple, snapshotDestinyAuthorityState, updateDestinyAuthorityState, } = require("../authority/destinySessionState.js");
const { advanceDestinyStamp, getDestinyStampForwardDistance, hasDestinyStamp, isDestinyStampAfter, normalizeDestinyStamp, resolveOptionalDestinyStamp, selectFurthestDestinyStamp, selectLaterDestinyStamp, } = require("../delivery/stamps");
function createMovementContractDispatch(deps = {}) {
    const { cloneVector, isReadyForDestiny, logMissileDebug, normalizeVector, roundNumber, sessionMatchesIdentity, summarizeRuntimeEntityForMissileDebug, buildMissileSessionSnapshot, directionsNearlyMatch, toInt, DEFAULT_RIGHT, MICHELLE_DIRECT_CRITICAL_ECHO_DESTINY_LEAD, MICHELLE_HELD_FUTURE_DESTINY_LEAD, PILOT_WARP_ACTIVATION_DELAY_DESTINY_TICKS, MICHELLE_POST_HELD_FUTURE_DESTINY_LEAD, } = deps;
    return {
        dispatchConfiguredSubwarpMovement(runtime, entity, buildUpdates, nowMs = runtime.getCurrentSimTimeMs(), options = {}) {
            if (!entity || typeof buildUpdates !== "function") {
                return false;
            }
            const requireExistingVisibility = options.requireExistingVisibility === true ||
            ...
            const ownerSession = entity && entity.session && isReadyForDestiny(entity.session)
                ? entity.session
                : null;
            const deferForMissilePressure = options.queueHistorySafeContract !== true &&
                ownerSession &&
            ...
            if (options.queueHistorySafeContract === true || deferForMissilePressure) {
                return runtime.queueSubwarpMovementContract(entity, prepareUpdates, {
                    ...
                    suppressedSessions: options.suppressedSessions instanceof Set
                        ? options.suppressedSessions
                        : null,
                    suppressOwnerGotoEcho: options.suppressOwnerGotoEcho === true,
                });
            }
            return runtime.broadcastPilotCommandMovementUpdates(entity.session || null, prepareUpdates(runtime.getMovementStamp(nowMs)), nowMs, {
                ...options,
        ...
        dispatchSubwarpMovementUpdates(runtime, entity, updates, options = {}) {
            ...
            const preparedUpdates = options.requireExistingVisibility === true
                ? tagUpdatesRequireExistingVisibility(updates)
                : updates;
            runtime.broadcastMovementUpdates(preparedUpdates, options.excludedSession || null, options.sendOptions || {});
            return true;
        },
        queueSubwarpMovementContract(runtime, entity, buildUpdates, options = {}) {
            ...
            runtime.pendingSubwarpMovementContracts.set(entity.itemID, {
                ...
                suppressOwnerGotoEcho: options.suppressOwnerGotoEcho === true,
                scheduledStamp: options.scheduledStamp === undefined || options.scheduledStamp === null
                    ? runtime.getHistorySafeDestinyStamp(options.nowMs === undefined || options.nowMs === null
                        ? runtime.getCurrentSimTimeMs()
                        : options.nowMs, MICHELLE_HELD_FUTURE_DESTINY_LEAD)
                    : normalizeDestinyStamp(options.scheduledStamp),
                excludedSession: options.excludedSession || null,
                suppressedSessions: options.suppressedSessions instanceof Set
                    ? new Set(options.suppressedSessions)
                    : null,
                ownerDirectEchoLeadOverride: options.ownerDirectEchoLeadOverride === undefined ||
        ...
        clearPendingSubwarpMovementContract(runtime, entityOrID) {
            const entityID = typeof entityOrID === "object" && entityOrID !== null
        ...
        flushPendingSubwarpMovementContracts(runtime, now = runtime.getCurrentSimTimeMs()) {
            ...
            const clampQueuedSubwarpUpdatesForSession = (session, queuedUpdates, options = {}) => {
                if (!session ||
                    !session._space ||
                ...
                const presentedFloorStamp = runtime.getCurrentPresentedSessionDestinyStamp(session, now, MICHELLE_HELD_FUTURE_DESTINY_LEAD);
                ...
                const lastSentDestinyRawDispatchStamp = lastPresentedLane.rawDispatchStamp;
                const lastSentDestinyWasOwnerCritical = authorityState &&
                    authorityState.lastSentWasOwnerCritical === true;
                const projectedFloorStamp = hasDestinyStamp(lastSentDestinyStamp) &&
                    hasDestinyStamp(lastSentDestinyRawDispatchStamp) &&
                    getDestinyStampForwardDistance(lastSentDestinyRawDispatchStamp, currentRawDispatchStamp) > 0 &&
                    getDestinyStampForwardDistance(lastSentDestinyRawDispatchStamp, currentRawDispatchStamp) <= 1 &&
                    !(authorityState &&
                        authorityState.lastSentOnlyStaleProjectedOwnerMissileLane === true)
                    ? projectPreviouslySentDestinyLane(lastSentDestinyStamp, lastSentDestinyRawDispatchStamp, currentRawDispatchStamp)
                    : null;
                ...
                // jolt).
                const rawVisibleStamp = runtime.getCurrentVisibleSessionDestinyStamp(session, now);
                const sessionStamp = runtime.getCurrentSessionDestinyStamp(session, now);
                const explicitHistoryFloorStamp = resolveOptionalDestinyStamp(session._space && session._space.historyFloorDestinyStamp);
                const hasFutureVisibleHistory = hasDestinyStamp(explicitHistoryFloorStamp) &&
                    isDestinyStampAfter(sessionStamp, explicitHistoryFloorStamp);
                const visibleFloorStamp = hasFutureVisibleHistory
                    ? advanceDestinyStamp(explicitHistoryFloorStamp, MICHELLE_HELD_FUTURE_DESTINY_LEAD)
                    : advanceDestinyStamp(rawVisibleStamp, MICHELLE_HELD_FUTURE_DESTINY_LEAD -
                ...
                const sameRawOwnerCriticalClearFloorStamp = options.isObserverVisibleContract === true &&
                    lastSentDestinyWasOwnerCritical &&
                    lastSentDestinyRawDispatchStamp === currentRawDispatchStamp &&
                    (lastSentDestinyStamp === selectFurthestDestinyStamp(sessionStamp, [
                        visibleFloorStamp,
                        presentedFloorStamp,
                        projectedFloorStamp,
                ...
                return clampQueuedSubwarpUpdates({
                    ...
                    visibleFloorStamp,
                    presentedFloorStamp,
                    projectedFloorStamp: selectLaterDestinyStamp(projectedFloorStamp, sameRawOwnerCriticalClearFloorStamp),
                    restampPayloadState: destiny.restampPayloadState,
                });
            };
            const minimumFutureStamp = runtime.getHistorySafeDestinyStamp(now, MICHELLE_HELD_FUTURE_DESTINY_LEAD);
            for (const pending of runtime.pendingSubwarpMovementContracts.values()) {
                const stamp = selectLaterDestinyStamp(hasDestinyStamp(pending && pending.scheduledStamp)
                    ? pending.scheduledStamp
                    : minimumFutureStamp, minimumFutureStamp);
                ...
                let delivered = false;
                const ownerSession = entity &&
                    entity.session &&
                    entity.session !== pending.excludedSession &&
                    isReadyForDestiny(entity.session)
                    ? entity.session
                    : null;
                const suppressedSessions = pending && pending.suppressedSessions instanceof Set
                ...
                if (ownerSession && !(suppressedSessions && suppressedSessions.has(ownerSession))) {
                    ...
                    const liveOwnerSessionStamp = runtime.getCurrentSessionDestinyStamp(ownerSession, now);
                    const currentRawDispatchStamp = runtime.getCurrentDestinyStamp(now);
                    let ownerUpdates = runtime.filterMovementUpdatesForSession(ownerSession, updates);
                    if (pending.suppressOwnerGotoEcho === true) {
                        ownerUpdates = ownerUpdates.filter((update) => (!Array.isArray(update && update.payload) ||
                    ...
                    const ownerHasSteeringCommandBeforeClamp = ownerMovementPayloadNamesBeforeClamp.some(isSteeringPayloadName);
                    const previousOwnerNonMissileCriticalLane = resolveDestinyAuthorityLaneTuple({
                    ...
                    const previousOwnerNonMissileCriticalStamp = previousOwnerNonMissileCriticalLane.stamp;
                    const previousOwnerNonMissileCriticalRawDispatchStamp = previousOwnerNonMissileCriticalLane.rawDispatchStamp;
                    const previousOwnerPilotCommandLane = resolveDestinyAuthorityCommandTuple({
                        authorityState: ownerAuthorityState,
                        legacyState: ownerSession._space,
                    });
                    const previousOwnerPilotCommandStamp = previousOwnerPilotCommandLane.stamp;
                    const previousOwnerPilotCommandAnchorStamp = previousOwnerPilotCommandLane.anchorStamp;
                    const previousOwnerPilotCommandRawDispatchStamp = previousOwnerPilotCommandLane.rawDispatchStamp;
                    ...
                    const previousOwnerMissileLifecycleLane = resolveDestinyAuthorityLaneTuple({
                        authorityState: ownerAuthorityState,
                        legacyState: ownerSession._space,
                        stampKey: "lastOwnerMissileLifecycleStamp",
                        rawDispatchKey: "lastOwnerMissileLifecycleRawDispatchStamp",
                    });
                    const previousOwnerMissileFreshAcquireLane = resolveDestinyAuthorityLaneTuple({
                        authorityState: ownerAuthorityState,
                        legacyState: ownerSession._space,
                    ...
                    if (ownerHasSteeringCommandBeforeClamp) {
                        const quietWindowActive = typeof runtime.isSessionInPilotWarpQuietWindow === "function" &&
                            runtime.isSessionInPilotWarpQuietWindow(ownerSession, now);
                        const currentVisibleOwnerStamp = runtime.getCurrentVisibleSessionDestinyStamp(ownerSession, now);
                        const currentPresentedOwnerStamp = runtime.getCurrentPresentedSessionDestinyStamp(ownerSession, now, quietWindowActive
                        ...
                        const quietWindowMinimumStamp = quietWindowActive
                            ? selectLaterDestinyStamp(resolveOptionalDestinyStamp(ownerSession._space &&
                                ownerSession._space.pilotWarpQuietUntilStamp), typeof runtime.getHistorySafeSessionDestinyStamp === "function"
                                ? runtime.getHistorySafeSessionDestinyStamp(ownerSession, now, PILOT_WARP_ACTIVATION_DELAY_DESTINY_TICKS, PILOT_WARP_ACTIVATION_DELAY_DESTINY_TICKS)
                                : null)
                            : null;
                        queuedOwnerRestampState = resolveOwnerMovementRestampState({
                            ...
                            previousLastSentDestinyWasOwnerCritical: ownerAuthorityState &&
                                ownerAuthorityState.lastSentWasOwnerCritical === true,
                            previousLastSentDestinyStamp: previousPresentedLane.stamp,
                            previousLastSentDestinyRawDispatchStamp: previousPresentedLane.rawDispatchStamp,
                            previousLastSentDestinyOnlyStaleProjectedOwnerMissileLane: ownerAuthorityState &&
                            ...
                            lastFreshAcquireLifecycleStamp: resolveOptionalDestinyStamp(ownerAuthorityState &&
                                ownerAuthorityState.lastFreshAcquireLifecycleStamp, ownerSession._space &&
                                ownerSession._space.lastFreshAcquireLifecycleStamp),
                            lastOwnerNonMissileCriticalStamp: previousOwnerNonMissileCriticalStamp,
                            lastOwnerNonMissileCriticalRawDispatchStamp: previousOwnerNonMissileCriticalRawDispatchStamp,
                            lastOwnerMissileLifecycleStamp: previousOwnerMissileLifecycleLane.stamp,
                            lastOwnerMissileLifecycleRawDispatchStamp: previousOwnerMissileLifecycleLane.rawDispatchStamp,
                            lastOwnerMissileFreshAcquireStamp: previousOwnerMissileFreshAcquireLane.stamp,
                            lastOwnerMissileFreshAcquireRawDispatchStamp: previousOwnerMissileFreshAcquireLane.rawDispatchStamp,
                            previousOwnerPilotCommandStamp,
                            previousOwnerPilotCommandAnchorStamp,
                            previousOwnerPilotCommandRawDispatchStamp,
                            previousOwnerPilotCommandDirectionRaw: previousOwnerPilotCommandLane.direction,
                            normalizeVector,
                            directionsNearlyMatch,
                            getPendingHistorySafeStamp: (authoredStamp, minimumLead = 0) => (typeof runtime.getPendingHistorySafeSessionDestinyStamp === "function"
                                ? runtime.getPendingHistorySafeSessionDestinyStamp(ownerSession, authoredStamp, now, minimumLead)
                                : selectLaterDestinyStamp(normalizeDestinyStamp(authoredStamp), advanceDestinyStamp(liveOwnerSessionStamp, toInt(minimumLead, 0)))),
                            defaultRight: DEFAULT_RIGHT && typeof DEFAULT_RIGHT === "object"
                                ? DEFAULT_RIGHT
                                : { x: 1, y: 0, z: 0 },
                        });
                        if (queuedOwnerRestampState &&
                            Array.isArray(queuedOwnerRestampState.ownerUpdates)) {
                            ownerUpdates = queuedOwnerRestampState.ownerUpdates;
                        }
                    }
                    ownerUpdates = clampQueuedSubwarpUpdatesForSession(ownerSession, ownerUpdates, {
                    ...
                    if (ownerUpdates.length > 0) {
                        const ownerMovementPayloadNames = ownerUpdates
                            ...
                            : null))
                            .filter((name) => Boolean(name));
                        const ownerHasSteeringCommand = ownerMovementPayloadNames.some(isSteeringPayloadName);
                        const highestQueuedOwnerMovementStamp = ownerUpdates.reduce((latestStamp, update) => selectLaterDestinyStamp(latestStamp, normalizeDestinyStamp(update && update.stamp)), null);
                        ...
                        if (runtime.hasActiveTickDestinyPresentationBatch()) {
                            ...
                            recordedOwnerMovementStamp = runtime.sendDestinyUpdates(ownerSession, ownerUpdates, false, {
                                destinyAuthorityContract: DESTINY_CONTRACTS.OWNER_PILOT_COMMAND,
                        ...
                        const emittedOwnerGotoDirection = resolveGotoDirectionFromUpdates({
                            ...
                            normalizeVector,
                            defaultDirection: DEFAULT_RIGHT && typeof DEFAULT_RIGHT === "object"
                                ? DEFAULT_RIGHT
                                : { x: 1, y: 0, z: 0 },
                            expectedStamp: resolveOptionalDestinyStamp(recordedOwnerMovementStamp),
                            normalizeStamp: resolveOptionalDestinyStamp,
                        });
                        if (ownerSession._space &&
                            ...
                            const ownerNonMissileLaneWrite = resolveAtomicOwnerMovementLaneWrite({
                                ...
                                previousRawDispatchStamp: previousOwnerNonMissileCriticalRawDispatchStamp,
                            ...
                            if (ownerHasSteeringCommand) {
                                const refreshOwnerCommandAnchor = shouldRefreshOwnerCommandAnchor({
                                ...
                                ownerCommandLaneWrite = resolveAtomicOwnerMovementLaneWrite({
                                    tracksAnchor: true,
                                    previousStamp: previousOwnerPilotCommandStamp,
                                    previousRawDispatchStamp: previousOwnerPilotCommandRawDispatchStamp,
                                    previousAnchorStamp: previousOwnerPilotCommandAnchorStamp,
                                    emittedStamp: recordedOwnerMovementStamp,
                                    currentRawDispatchStamp,
                            ...
                            updateDestinyAuthorityState(ownerSession, {
                                lastOwnerNonMissileCriticalStamp: ownerNonMissileLaneWrite.stamp,
                                lastOwnerNonMissileCriticalRawDispatchStamp: ownerNonMissileLaneWrite.rawDispatchStamp,
                                ...(ownerHasSteeringCommand
                                    ? {
                                        lastOwnerCommandStamp: ownerCommandLaneWrite.stamp,
                                        lastOwnerCommandAnchorStamp: ownerCommandLaneWrite.anchorStamp,
                                        lastOwnerCommandRawDispatchStamp: ownerCommandLaneWrite.rawDispatchStamp,
                                        lastOwnerCommandHeadingHash: ownerCommandLaneWrite.accepted
                                            ? emittedOwnerGotoDirection
                ...
                for (const session of runtime.sessions.values()) {
                    if (sessionMatchesIdentity(session, ownerSession || pending.excludedSession) ||
                        (suppressedSessions instanceof Set &&
                            suppressedSessions.has(session)) ||
                        !isReadyForDestiny(session)) {
                        continue;
                    }
                    const filteredUpdates = runtime.filterMovementUpdatesForSession(session, updates);
                    if (filteredUpdates.length === 0) {
                        continue;
                    }
                    const queuedUpdates = clampQueuedSubwarpUpdatesForSession(session, filteredUpdates, {
                        isObserverVisibleContract: toInt(session._space && session._space.shipID, 0) !==
                    ...
                    if (runtime.hasActiveTickDestinyPresentationBatch()) {
                        ...
                        runtime.sendDestinyUpdates(session, queuedUpdates, false, {
                            destinyAuthorityContract: DESTINY_CONTRACTS.CRITICAL_MOVEMENT_OR_SHIPPRIME,
                            translateStamps: false,
                        });
                    }
...
//# sourceMappingURL=contractDispatch.js.map