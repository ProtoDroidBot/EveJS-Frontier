...
function createMovementWatcherCorrections(deps: Record<string, any> = {}) {
  const {
    destiny,
    toInt,
  ...
  function resolveWatcherCorrectionDispatch(options: Record<string, any> = {}) {
    ...
    if (entity.mode === "WARP") {
      ...
      const warpCorrectionStamp = selectFurthestDestinyStamp(
        warpCommandStamp,
        [warpCommandStamp, runtime.getMovementStamp(now)],
      );
