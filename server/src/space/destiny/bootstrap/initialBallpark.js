...
function includesEntity(entities, targetEntity) {
    ...
    return targetID !== null && normalizeArray(entities).some((entity) => (getEntityID(entity) === targetID));
}
...
function buildInitialBallparkEntityPlan(options = {}) {
    ...
    const isIncrementalStaticVisibilityEntity = typeof options.isIncrementalStaticVisibilityEntity === "function"
        ? options.isIncrementalStaticVisibilityEntity
        : () => false;
    const isPostSetStateBootstrapStaticEntity = typeof options.isPostSetStateBootstrapStaticEntity === "function"
    ...
    const isBootstrapSiteStaticVisibilityEntity = typeof options.isBootstrapSiteStaticVisibilityEntity === "function"
        ? options.isBootstrapSiteStaticVisibilityEntity
        : () => false;
    let stateRefreshVisibleEntities = visibleEntities.filter((entity) => !isPostSetStateBootstrapStaticEntity(entity));
    stateRefreshVisibleEntities = appendEntityIfMissing(stateRefreshVisibleEntities, egoEntity);
    const postSetStateBootstrapStaticEntities = visibleEntities.filter((entity) => isPostSetStateBootstrapStaticEntity(entity));
    const bootstrapIncludesDedicatedSiteStatics = stateRefreshVisibleEntities.some((entity) => isBootstrapSiteStaticVisibilityEntity(entity));
...
//# sourceMappingURL=initialBallpark.js.map