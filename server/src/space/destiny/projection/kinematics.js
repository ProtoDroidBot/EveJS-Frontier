...
const { getEntityMapKey: getCanonicalEntityMapKey, } = require("../identity/entityID");
...
function resolveMaxProjectionSteps(options = {}) {
    ...
    return Math.min(ABSOLUTE_MAX_PROJECTION_STEPS, Math.max(1, Math.trunc(numeric)));
}
function resolveProjectionStepMs(deltaMs, options = {}) {
    const authoredStepMs = options.maxProjectionStepMs;
    const configuredStepMs = Math.max(0, authoredStepMs === null || authoredStepMs === undefined
...
function normalizeMovementTargetEntityIDs(entity, options = {}) {
    const resolver = typeof options.getMovementTargetEntityIDs === "function"
...
function buildNativeProjectionStepEndpoints(baseSimTimeMs, targetSimTimeMs, stepMs, maxSteps, nativeSubwarpController, nativeSubwarpFrame) {
    const boundaries = [];
    let boundaryCursor = baseSimTimeMs;
    for (let index = 0; index < maxSteps; index += 1) {
        const boundary = Number(nativeSubwarpController.getNextFrameBoundaryMs(boundaryCursor, nativeSubwarpFrame));
    ...
    const countStepsAt = (candidateStepMs) => {
        ...
        for (const segmentEnd of segmentEnds) {
            count += Math.max(1, Math.ceil((segmentEnd - segmentStart) / candidateStepMs));
    ...
    for (const segmentEnd of segmentEnds) {
        ...
        const segmentSteps = Math.max(1, Math.ceil(segmentLength / resolvedStepMs));
        for (let index = 1; index <= segmentSteps; index += 1) {
            endpoints.push(index === segmentSteps
                ? segmentEnd
                : Math.min(segmentEnd, segmentStart + (resolvedStepMs * index)));
        }
        segmentStart = segmentEnd;
    }
    return {
...
function projectEntityGraphToTime(options = {}) {
    const entities = Array.isArray(options.entities)
        ...
        : options.entity
            ? [options.entity]
            : [];
    const baseSimTimeMs = Math.max(0, toFiniteNumber(options.baseSimTimeMs, 0));
    const targetSimTimeMs = Math.max(baseSimTimeMs, toFiniteNumber(options.targetSimTimeMs, baseSimTimeMs));
    const advanceMovement = options.advanceMovement;
    const cloneEntity = options.cloneEntity;
    const getEntityByID = typeof options.getEntityByID === "function"
        ...
        : options.scene && typeof options.scene.getEntityByID === "function"
            ...
            : () => null;
    const getEntityMapKey = typeof options.getEntityMapKey === "function"
    ...
    const shouldAdvance = typeof options.shouldAdvance === "function"
        ...
        : () => true;
    const nativeSubwarpControllerRequested = Boolean(options.nativeSubwarpController);
    const nativeSubwarpController = options.nativeSubwarpController &&
    ...
    const maxDepth = Math.max(0, Math.trunc(authoredMaxDepth === null || authoredMaxDepth === undefined
        ? 3
    ...
    const expandRecord = (record, depth) => {
        ...
        for (const targetEntityID of normalizeMovementTargetEntityIDs(record.originalEntity, options)) {
    ...
    const graphComplete = identityConflictCount === 0 && unresolvedDependencyCount === 0;
    let nativeFrameComplete = !nativeSubwarpControllerRequested || Boolean(nativeSubwarpController);
    const skippedEntityCount = records.reduce((count, record) => count + (record.shouldAdvance ? 0 : 1), 0);
    const activeNativeSubwarpController = nativeSubwarpController &&
    ...
    const unaddressablePlanRecordCount = activeNativeSubwarpController
        ? records.reduce((count, record) => (count + (record.shouldAdvance &&
    ...
    const expectedNativeSubwarpSceneEntitiesOwner = options.nativeSubwarpSceneEntitiesOwner !== undefined
        ...
        : (options.scene && options.scene.dynamicEntities instanceof Map
            ...
            : undefined);
    const carriedNativeSubwarpFrameOwnerMatched = Boolean(options.nativeSubwarpFrame &&
    ...
    if (records.some((record) => record.shouldAdvance) &&
        baseSimTimeMs < targetSimTimeMs - 0.000001) {
        const totalProjectionMs = targetSimTimeMs - baseSimTimeMs;
        const maxSteps = resolveMaxProjectionSteps(options);
        projectedAtMs = baseSimTimeMs;
        const stepMs = resolveProjectionStepMs(totalProjectionMs, options);
        const nativeStepPlan = activeNativeSubwarpController
            ? buildNativeProjectionStepEndpoints(baseSimTimeMs, targetSimTimeMs, stepMs, maxSteps, activeNativeSubwarpController, carriedNativeSubwarpFrameOwnerMatched
                ? options.nativeSubwarpFrame
                : null)
            : null;
        const legacyStepCount = Math.min(maxSteps, Math.max(1, Math.ceil(totalProjectionMs / stepMs)));
        const stepEndpoints = nativeStepPlan
            ? nativeStepPlan.endpoints
            : Array.from({ length: legacyStepCount }, (_unused, index) => (index === legacyStepCount - 1
                ? targetSimTimeMs
                : Math.min(targetSimTimeMs, baseSimTimeMs + (stepMs * (index + 1)))));
        ...
        let nativeSubwarpFrame = activeNativeSubwarpController
            ? carriedNativeSubwarpFrameOwnerMatched
                ? activeNativeSubwarpController.rebindFrame(options.nativeSubwarpFrame, projectionFrameScene, {
                    expectedSourceSceneEntitiesOwner: expectedNativeSubwarpSceneEntitiesOwner,
                    originalEntitiesByID,
                })
                : options.nativeSubwarpFrame
                    ? activeNativeSubwarpController.prepareFrame(projectionFrameScene, baseSimTimeMs, {
        ...
        const carriedFrameSourceOwnerMatched = Boolean(nativeSubwarpFrame &&
            carriedNativeSubwarpFrameOwnerMatched &&
            nativeSubwarpFrame.sourceSceneOwnerMatched !== false);
        const carriedFrameCoversBase = Boolean(carriedFrameSourceOwnerMatched &&
            Number(nativeSubwarpFrame.frameStartMs) <= baseSimTimeMs + 0.000001 &&
            Number(nativeSubwarpFrame.frameEndMs) > baseSimTimeMs + 0.000001);
        const requiresCarriedFrame = Boolean(options.requireCarriedNativeSubwarpFrame === true &&
            records.some((record) => (record.shouldAdvance &&
                activeNativeSubwarpController &&
                activeNativeSubwarpController.isNativeSubwarpEntity(record.projectedEntity))));
        ...
        if (activeNativeSubwarpController) {
            ...
            if (nativeProjectionTargetMs > baseSimTimeMs + 0.000001) {
                const nativeIntervalPlan = activeNativeSubwarpController.planSceneInterval(projectionFrameScene, baseSimTimeMs, nativeProjectionTargetMs, {
                    advanceMovement,
                    cloneEntity,
                ...
                if (nativeIntervalPlan.complete === true &&
                    nativeIntervalPlan.projectedEntitiesByID instanceof Map) {
                    for (const record of records) {
                        if (!hasEntityKey(record.entityKey)) {
                            continue;
                        }
                        const plannedEntity = nativeIntervalPlan.projectedEntitiesByID.get(record.entityKey);
                        if (!plannedEntity) {
                            continue;
                        }
            ...
            for (const nextProjectedAtMs of stepEndpoints) {
                ...
                for (const record of records) {
                    if (hasEntityKey(record.entityKey)) {
                        stepSnapshotsByID.set(record.entityKey, cloneEntity(record.projectedEntity));
                    }
                }
                for (const record of records) {
                    ...
                    advanceMovement(record.projectedEntity, stepScene, currentStepMs / 1000, nextProjectedAtMs);
                }
    ...
    return {
        ...
        entities: rootRecords.map((record, index) => (record ? record.projectedEntity : entities[index])),
