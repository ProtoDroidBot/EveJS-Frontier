...
const { entityIDsEqual, getEntityMapKey, } = require("../identity/entityID");
const { projectEntityGraphToTime, projectEntityToTime, } = require("./kinematics");
const { getCurrentDestinyStamp, getDestinyStampForwardBoundaryDeltaMs, normalizeDestinyStamp, } = require("../delivery/stamps");
...
function cloneDynamicEntityForDestinyPresentation(entity) {
    ...
    if (entity.warpState && typeof entity.warpState === "object") {
        clone.warpState = {
            ...
            targetPoint: cloneVector(entity.warpState.targetPoint, entity.targetPoint || entity.position),
            direction: cloneVector(entity.warpState.direction, entity.direction || { x: 1, y: 0, z: 0 }),
            entryPosition: cloneVector(entity.warpState.entryPosition, entity.position),
        };
    }
    if (entity.pendingWarp && typeof entity.pendingWarp === "object") {
        clone.pendingWarp = {
            ...
            targetPoint: cloneVector(entity.pendingWarp.targetPoint, entity.targetPoint || entity.position),
            direction: cloneVector(entity.pendingWarp.direction, entity.direction || { x: 1, y: 0, z: 0 }),
        };
    }
    if (entity.sessionlessWarpIngress &&
        ...
        clone.sessionlessWarpIngress = {
            ...
            origin: cloneVector(entity.sessionlessWarpIngress.origin, entity.position),
            targetPoint: cloneVector(entity.sessionlessWarpIngress.targetPoint, entity.targetPoint || entity.position),
        };
    }
    return clone;
}
function buildDestinyPresentationForSession(options = {}) {
    ...
    const resolveEntityMapKey = typeof options.getEntityMapKey === "function"
        ? options.getEntityMapKey
        : getEntityMapKey;
    const cloneDynamicEntityForDestinyPresentation = options.cloneDynamicEntityForDestinyPresentation;
    ...
    const isReadyForDestiny = typeof options.isReadyForDestiny === "function"
        ...
        : () => true;
    const currentSessionSimTimeMs = typeof getCurrentSessionSimTimeMs === "function"
    ...
    const currentSessionStamp = normalizeDestinyStamp(getStampForSimTime(currentSessionSimTimeMs), 0);
    const normalizedSessionStamp = normalizeDestinyStamp(options.sessionStamp, currentSessionStamp);
    const presentationDeltaMs = getDestinyStampForwardBoundaryDeltaMs(currentSessionSimTimeMs, currentSessionStamp, normalizedSessionStamp, options.maximumForwardLead);
    const presentationSessionSimTimeMs = currentSessionSimTimeMs + presentationDeltaMs;
    ...
    const projection = projectEntityGraphToTime({
        ...
        nativeSubwarpSceneEntitiesOwner: options.nativeSubwarpSceneEntitiesOwner,
        maxProjectionDepth: 2,
        maxProjectionStepMs: options.maxProjectionStepMs,
        maxProjectionSteps: options.maxProjectionSteps,
        shouldAdvance: (projectedEntity) => (!projectedEntity.sessionlessWarpIngress &&
...
function projectEntityForDestinyStamp(options = {}) {
    ...
    const hasAuthoredCurrentSimTime = options.currentSimTimeMs !== null &&
    ...
    const currentSimTimeMs = hasAuthoredCurrentSimTime
        ...
        : rawNowMs;
    const currentStamp = normalizeDestinyStamp(options.currentStamp, getCurrentDestinyStamp(currentSimTimeMs));
    const targetStamp = normalizeDestinyStamp(options.stamp, currentStamp);
    const projectionDeltaMs = getDestinyStampForwardBoundaryDeltaMs(currentSimTimeMs, currentStamp, targetStamp, options.maximumForwardLead);
    const targetRawSimTimeMs = rawNowMs + projectionDeltaMs;
    if (targetRawSimTimeMs <= rawNowMs + 0.000001 ||
        entity.sessionlessWarpIngress ||
        entity.pendingDock) {
        return cloneDynamicEntityForDestinyPresentation(entity);
    }
    const scene = options.scene && typeof options.scene === "object"
        ? options.scene
        : {
            getEntityByID() {
                return null;
    ...
    return projectEntityToTime({
        ...
        scene,
        getEntityByID: (entityID) => (typeof scene.getEntityByID === "function"
        ...
        nativeSubwarpController: options.nativeSubwarpController,
        nativeSubwarpFrame: options.nativeSubwarpFrame,
        nativeSubwarpSceneEntitiesOwner: options.nativeSubwarpSceneEntitiesOwner !== undefined
        ...
        maxProjectionStepMs: options.maxProjectionStepMs,
        maxProjectionSteps: options.maxProjectionSteps,
        shouldAdvance: (projected) => (!projected.sessionlessWarpIngress &&
...
function rebuildKinematicUpdatesForProjectedStamp(options = {}) {
    ...
    const rewrittenUpdates = updates.map((update) => {
        const payload = update && Array.isArray(update.payload) ? update.payload : null;
        const args = payload && Array.isArray(payload[1]) ? payload[1] : null;
        const payloadEntityID = payload && payload[0] === "SetBallVelocity"
        ...
        if (!projectedEntity) {
            projectedEntity = projectEntityForDestinyStamp({
                ...
                nativeSubwarpController: options.nativeSubwarpController,
                nativeSubwarpFrame: options.nativeSubwarpFrame,
                cloneDynamicEntityForDestinyPresentation: options.cloneDynamicEntityForDestinyPresentation,
        ...
        return {
            ...update,
            payload: destiny.buildSetBallVelocityPayload(entityID, stopSeedVelocity),
        };
    });
    return changed ? rewrittenUpdates : updates;
}
const rebuildOwnerKinematicUpdatesForProjectedStamp = rebuildKinematicUpdatesForProjectedStamp;
module.exports = {
...
Object.defineProperty(module.exports, "cloneDynamicEntityForDestinyPresentation", {
    value: cloneDynamicEntityForDestinyPresentation,
    enumerable: false,
});
//# sourceMappingURL=entityProjection.js.map