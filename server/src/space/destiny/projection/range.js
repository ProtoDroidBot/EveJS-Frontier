...
const { projectEntityGraphToTime, } = require("./kinematics");
...
function getEntitySurfaceDistance(sourceEntity, targetEntity) {
    ...
    return Math.max(0, magnitude(subtractVectors(targetEntity && targetEntity.position, sourceEntity && sourceEntity.position)) -
...
function resolveSceneBaseSimTimeMs(scene, options = {}) {
    ...
    return Math.max(0, authoredBaseSimTimeMs === null || authoredBaseSimTimeMs === undefined
...
function resolveCommandSimTimeMs(scene, sceneBaseSimTimeMs, options = {}) {
    const currentSimTimeMs = typeof options.getCurrentSimTimeMs === "function"
...
function buildCommandTimeSurfaceState(options = {}) {
    ...
    const commandSimTimeMs = resolveCommandSimTimeMs(scene, sceneBaseSimTimeMs, options);
    const projectionDeltaMs = Math.max(0, commandSimTimeMs - sceneBaseSimTimeMs);
    if (projectionDeltaMs <= 0.000001) {
        const centerDistance = magnitude(subtractVectors(targetEntity && targetEntity.position, sourceEntity && sourceEntity.position));
        return {
            available: true,
            projected: false,
            sourceEntity,
    ...
    const projection = projectEntityGraphToTime({
        ...
        getEntityByID: (entityID) => getSceneEntityByID(scene, entityID, options),
        getEntityMapKey: typeof options.getEntityMapKey === "function"
        ...
        nativeSubwarpSceneEntitiesOwner: scene && scene.dynamicEntities instanceof Map
            ? scene.dynamicEntities
            : undefined,
        requireCarriedNativeSubwarpFrame: Boolean(options.nativeSubwarpController),
    ...
    const centerDistance = magnitude(subtractVectors(projectedTargetEntity && projectedTargetEntity.position, projectedSourceEntity && projectedSourceEntity.position));
    return {
        available: true,
        projected: true,
        sourceEntity: projectedSourceEntity,
        targetEntity: projectedTargetEntity,
        centerDistance,
        surfaceDistance: getEntitySurfaceDistance(projectedSourceEntity, projectedTargetEntity),
    };
}
...
function isWithinSurfaceRange(surfaceDistance, rangeMeters, graceMeters = 0) {
    return (Math.max(0, toFiniteNumber(surfaceDistance, Infinity)) <=
...
function isWithinCommandTimeSurfaceRange(options = {}) {
    return isWithinSurfaceRange(getCommandTimeEntitySurfaceDistance(options), options.rangeMeters, options.graceMeters);
}
module.exports = {
