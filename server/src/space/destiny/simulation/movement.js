...
const { DESTINY_ORBITAL_PRECESSION_PER_TICK: DEFAULT_DESTINY_ORBITAL_PRECESSION_PER_TICK, DESTINY_SPACE_FRICTION: DEFAULT_DESTINY_SPACE_FRICTION, DESTINY_STAMP_INTERVAL_MS: DEFAULT_DESTINY_STAMP_INTERVAL_MS, } = require("../constants");
const { normalizeEntityID, } = require("../identity/entityID");
const { advanceDestinyStamp: advanceDefaultDestinyStamp, getCurrentDestinyStamp: getDefaultCurrentDestinyStamp, } = require("../delivery/stamps");
const { acquireActiveWarpNativeSample: acquireDefaultActiveWarpNativeSample, } = require("./warpNativeBoundary");
const { integrateCarbonMotion: integrateDefaultCarbonMotion, } = require("./carbonIntegration");
const { resolveMassAccelerationMultiplier, resolveMassAdjustedAngularSpeed, } = require("./massDynamics");
const DEFAULT_WORLD_UP = Object.freeze({ x: 0, y: 1, z: 0 });
function createDestinyMovementSimulator(deps = {}) {
    const { addVectors, acquireActiveWarpNativeSample = acquireDefaultActiveWarpNativeSample, advanceDestinyStamp = advanceDefaultDestinyStamp, advanceSessionlessWarpIngress, buildPerpendicular, clamp, cloneVector, crossProduct, deriveAgilitySeconds, distance, dotProduct, getCurrentAlignmentDirection, getCurrentDestinyStamp = getDefaultCurrentDestinyStamp, getFollowMotionProfile, getTurnMetrics, getWarpProgress, integrateCarbonMotion = integrateDefaultCarbonMotion, log, logMissileDebug, magnitude, normalizeTraceValue, normalizeVector, refreshPreparingWarpState, roundNumber, scaleVector, serializeWarpState, subtractVectors, summarizeMissileEntity, summarizeRuntimeEntityForMissileDebug, summarizeVector, toFiniteNumber, DEFAULT_RIGHT, DESTINY_ORBITAL_PRECESSION_PER_TICK = DEFAULT_DESTINY_ORBITAL_PRECESSION_PER_TICK, DESTINY_SPACE_FRICTION = DEFAULT_DESTINY_SPACE_FRICTION, DESTINY_STAMP_INTERVAL_MS = DEFAULT_DESTINY_STAMP_INTERVAL_MS, MAX_SUBWARP_SPEED_FRACTION, TURN_ALIGNMENT_RADIANS, } = deps;
    function getCommandDirection(entity, fallback = DEFAULT_RIGHT) {
        if (entity && entity.targetPoint && entity.position) {
            return normalizeVector(subtractVectors(entity.targetPoint, entity.position), entity.direction || fallback);
        }
        return normalizeVector(entity && entity.direction, fallback);
    }
    function integrateVelocityTowardTarget(currentVelocity, desiredVelocity, responseSeconds, deltaSeconds) {
        ...
        const nextVelocity = addVectors(desiredVelocity, scaleVector(velocityOffset, decay));
        const positionDelta = addVectors(scaleVector(desiredVelocity, delta), scaleVector(velocityOffset, tau * (1 - decay)));
        return {
            nextVelocity,
            positionDelta,
            decay,
    ...
    function slerpDirection(current, target, fraction, radians) {
        ...
        if (!Number.isFinite(sinTotal) || Math.abs(sinTotal) < 0.000001) {
            return normalizeVector(addVectors(scaleVector(current, 1 - clampedFraction), scaleVector(target, clampedFraction)), target);
        }
        const leftWeight = Math.sin((1 - clampedFraction) * totalRadians) / sinTotal;
        const rightWeight = Math.sin(clampedFraction * totalRadians) / sinTotal;
        return normalizeVector(addVectors(scaleVector(current, leftWeight), scaleVector(target, rightWeight)), target);
    }
    function rotateDirectionToward(currentDirection, targetDirection, deltaSeconds, agilitySeconds, currentSpeedFraction = 0) {
    ...
    function applyDesiredVelocity(entity, desiredDirection, desiredSpeed, deltaSeconds) {
        ...
        const agilitySeconds = Math.max(toFiniteNumber(entity.agilitySeconds, 0) ||
            deriveAgilitySeconds(entity.alignTime, entity.maxAccelerationTime, entity.mass, entity.inertia), 0.05);
        const currentSpeedFraction = entity.maxVelocity > 0
            ? Math.max(0, magnitude(entity.velocity) / entity.maxVelocity)
            : 0;
        const targetSpeedFraction = entity.maxVelocity > 0
            ? Math.max(0, desiredSpeed / entity.maxVelocity)
            : 0;
        const currentAlignmentDirection = getCurrentAlignmentDirection(entity, targetDirection);
        ...
        const integration = integrateVelocityTowardTarget(previousVelocity, desiredVelocity, agilitySeconds, deltaSeconds);
        const nextSpeed = magnitude(integration.nextVelocity);
        const nextSpeedFraction = entity.maxVelocity > 0 ? Math.max(0, nextSpeed / entity.maxVelocity) : 0;
        const turnStep = rotateDirectionToward(headingSource, targetDirection, deltaSeconds, agilitySeconds, currentSpeedFraction);
        ...
        entity.lastTurnMetrics = {
            ...
            speedDeltaFraction: roundNumber(Math.abs(currentSpeedFraction - targetSpeedFraction), 3),
        ...
        return {
            changed: distance(previousPosition, entity.position) > 1 ||
    ...
    function getCarbonAgilitySeconds(entity) {
        ...
        const friction = Math.max(0.000001, toFiniteNumber(DESTINY_SPACE_FRICTION, DEFAULT_DESTINY_SPACE_FRICTION));
        const nativeAgilitySeconds = (mass * inertia) / friction;
        if (nativeAgilitySeconds > 0) {
            return nativeAgilitySeconds;
        }
        return Math.max(toFiniteNumber(entity && entity.agilitySeconds, 0) ||
            deriveAgilitySeconds(entity && entity.alignTime, entity && entity.maxAccelerationTime, entity && entity.mass, entity && entity.inertia), 0.05);
    }
    function getCarbonCruiseVelocity(entity) {
        return Math.max(0, toFiniteNumber(entity && entity.maxVelocity, 0)) *
            clamp(toFiniteNumber(entity && entity.speedFraction, 0), 0, MAX_SUBWARP_SPEED_FRACTION);
    }
    function calculateCarbonGotoThrust(entity, targetPoint, deltaSeconds, options = {}) {
        ...
        const stepDistance = cruiseVelocity * Math.max(0, toFiniteNumber(deltaSeconds, 0));
        let homingScale = 1;
        if (stepDistance > 0.000000001 &&
        ...
        homingScale = Math.max(homingScale, clamp(toFiniteNumber(options.minHomingScale, 0), 0, 1));
        const acceleration = separationDistance > 0.000000001
            ? scaleVector(separation, (maximumAcceleration * homingScale) / separationDistance)
            : { x: 0, y: 0, z: 0 };
        return {
            acceleration,
            agilitySeconds,
    ...
    function applyCarbonAcceleration(entity, acceleration, deltaSeconds, debug = null) {
        const agilitySeconds = getCarbonAgilitySeconds(entity);
        ...
        const currentSpeedFraction = entity.maxVelocity > 0
            ? Math.max(0, magnitude(entity.velocity) / entity.maxVelocity)
            : 0;
        const turnStep = rotateDirectionToward(headingSource, desiredDirection, deltaSeconds, agilitySeconds, currentSpeedFraction);
        const previousPosition = cloneVector(entity.position);
        const previousVelocity = cloneVector(entity.velocity);
        ...
        const result = applyDesiredVelocity(motionEntity, desiredDirection, desiredSpeed, deltaSeconds);
        const officialInertialMass = Math.max(0, toFiniteNumber(entity && entity.mass, 0)) *
            Math.max(0, toFiniteNumber(entity && entity.inertia, 0));
        const carbonMotion = integrateCarbonMotion(previousPosition, previousVelocity, acceleration, officialInertialMass > 0
            ? officialInertialMass
            : agilitySeconds * DESTINY_SPACE_FRICTION, DESTINY_SPACE_FRICTION, deltaSeconds);
        ...
        entity.lastMotionDebug = {
            ...
            nextSpeed: roundNumber(magnitude(entity.velocity), 3),
            positionDelta: summarizeVector(subtractVectors(entity.position, previousPosition)),
            velocityDelta: summarizeVector(subtractVectors(entity.velocity, previousVelocity)),
        };
    ...
    function calculateCarbonFollowPlan(entity, target, deltaSeconds) {
        ...
        const targetPosition = cloneVector(motionProfile && motionProfile.targetPoint, target && target.position);
        const targetToFollower = subtractVectors(entity.position, targetPosition);
        const centerDistance = magnitude(targetToFollower);
        const desiredDistance = Math.max(0, toFiniteNumber(entity.followRange, 0) +
            Math.max(0, toFiniteNumber(entity.radius, 0)) +
            Math.max(0, toFiniteNumber(motionProfile && motionProfile.rangeRadius, target && target.radius)));
        const gotoPoint = centerDistance <= 0.000000001
            ? addVectors(targetPosition, scaleVector(DEFAULT_RIGHT, desiredDistance))
            : addVectors(targetPosition, scaleVector(targetToFollower, desiredDistance / centerDistance));
        ...
        const desiredClosingVelocity = Math.max(gap * 0.5, cruiseVelocity * 0.25);
        const minHomingScale = gap > 50 && cruiseVelocity > 0.000001
            ? clamp((Math.max(0, targetRadialSpeed) + desiredClosingVelocity) /
                cruiseVelocity, 0, 1)
            : 0;
        const leadSeconds = gap > 50 && magnitude(targetVelocity) > 0.000001
            ? clamp(closingVelocity > 0.000001
                ? gap / closingVelocity
                : Math.max(1, toFiniteNumber(deltaSeconds, 0)), Math.max(1, toFiniteNumber(deltaSeconds, 0)), 5)
            : 0;
        const pursuitPoint = leadSeconds > 0
            ? addVectors(gotoPoint, scaleVector(targetVelocity, leadSeconds))
            : gotoPoint;
        const thrust = calculateCarbonGotoThrust(entity, pursuitPoint, deltaSeconds, {
            minHomingScale,
        });
        return {
            ...thrust,
    ...
    function calculateCarbonOrbitPlan(entity, target, _deltaSeconds, nowMs = 0) {
        ...
        const agilitySeconds = getCarbonAgilitySeconds(entity);
        const maximumAcceleration = cruiseVelocity / agilitySeconds;
        const desiredDistance = Math.max(0, toFiniteNumber(entity.orbitDistance, 0) +
            Math.max(0, toFiniteNumber(entity.radius, 0)) +
        ...
        const tangentDirection = normalizeCarbonOrbitVector(crossProduct(orbitNormal, toDirection));
        const tangentDistanceSquared = (rawDistance * rawDistance) - (desiredDistance * desiredDistance);
        if (tangentDistanceSquared >= 0 && rawDistance > 0.000000001) {
            const tangentComponent = (desiredDistance * Math.sqrt(tangentDistanceSquared)) / rawDistance;
            toDirection = normalizeCarbonOrbitVector(addVectors(scaleVector(toDirection, tangentDistanceSquared / rawDistance), scaleVector(tangentDirection, tangentComponent)));
        }
        const truncateSeven = (value) => Math.trunc(value * 10_000_000) / 10_000_000;
        const radialFactor = truncateSeven(Math.exp(-((desiredDistance - rawDistance) *
            (desiredDistance - rawDistance)) / 40_000));
        ...
        const acceleration = scaleVector(addVectors(scaleVector(tangentDirection, radialFactor), scaleVector(toDirection, transverseFactor)), maximumAcceleration);
        const gotoPoint = addVectors(entity.position, scaleVector(acceleration, 10 * 149_597_870_700));
        return {
            acceleration,
            cruiseVelocity,
            desiredDistance,
    ...
    function advanceGotoMovement(entity, deltaSeconds) {
        ...
        const desiredSpeed = entity.maxVelocity * clamp(entity.speedFraction, 0, MAX_SUBWARP_SPEED_FRACTION);
        const result = applyDesiredVelocity(entity, desiredDirection, desiredSpeed, deltaSeconds);
    ...
    function applyManualStrafingThrust(entity, deltaSeconds) {
        ...
        const horizontalDirection = normalizeVector(buildPerpendicular(forwardDirection), DEFAULT_RIGHT);
        const verticalDirection = normalizeVector(crossProduct(horizontalDirection, forwardDirection), DEFAULT_WORLD_UP);
        const massAccelerationMultiplier = resolveMassAccelerationMultiplier(entity.mass, entity.baseMass);
        const acceleration = scaleVector(addVectors(scaleVector(verticalDirection, thrust.x), scaleVector(horizontalDirection, thrust.y)), massAccelerationMultiplier);
        ...
        entity.lastMotionDebug = {
            ...
            manualStrafingMassMultiplier: roundNumber(massAccelerationMultiplier, 6),
    ...
    function advanceManualFlightTarget(entity, deltaSeconds) {
        ...
        const maxAngularSpeed = resolveMassAdjustedAngularSpeed(Math.max(0, toFiniteNumber(entity.maxAngularSpeed, 0.25)), entity.mass, entity.baseMass, entity.angularAgility);
        const pitchTarget = clamp(toFiniteNumber(entity.manualPitch, 0), -1, 1) * (Math.PI / 2);
        ...
        const pitchStep = clamp(pitchError, -maximumPitchStep, maximumPitchStep);
        const nextPitch = clamp(currentPitch + pitchStep, -(Math.PI / 2), Math.PI / 2);
        const currentYaw = Math.atan2(currentDirection.x, currentDirection.z);
        const yawVelocity = clamp(toFiniteNumber(entity.manualYawRate, 0), -1, 1) * maxAngularSpeed;
        ...
        entity.targetPoint = addVectors(cloneVector(entity.position), scaleVector(commandDirection, 1.0e16));
        return true;
    }
    ...
    function getMissileImpactDistance(entity, target) {
        ...
        return Math.max(0, distance(entity && entity.position, target && target.position) - Math.max(1, toFiniteNumber(target && target.radius, 0)));
    }
    function resolveMissileTargetStepStartPosition(target, deltaSeconds) {
        ...
        const motionDebug = target.lastMotionDebug &&
        ...
        return subtractVectors(currentTargetPosition, scaleVector(target.velocity || { x: 0, y: 0, z: 0 }, deltaSeconds));
    }
    ...
    function resolveMissileSweptImpact(missileStartPosition, missileVelocity, targetStartPosition, targetEndPosition, targetRadius, deltaSeconds) {
        ...
        const targetStepVelocity = stepSeconds > 0.000001
            ? scaleVector(subtractVectors(normalizedTargetEnd, normalizedTargetStart), 1 / stepSeconds)
            : { x: 0, y: 0, z: 0 };
        const relativeStart = subtractVectors(normalizedMissileStart, normalizedTargetStart);
        const relativeVelocity = subtractVectors(missileStepVelocity, targetStepVelocity);
        const relativeVelocityMagnitudeSquared = dotProduct(relativeVelocity, relativeVelocity);
        ...
        if (relativeVelocityMagnitudeSquared > 0.000001) {
            closestTimeSeconds = clamp(-dotProduct(relativeStart, relativeVelocity) /
                relativeVelocityMagnitudeSquared, 0, stepSeconds);
        }
        const relativeClosestVector = addVectors(relativeStart, scaleVector(relativeVelocity, closestTimeSeconds));
        const closestCenterDistance = magnitude(relativeClosestVector);
        const closestSurfaceDistance = Math.max(0, closestCenterDistance - impactRadius);
        const quadraticB = 2 * dotProduct(relativeStart, relativeVelocity);
        const quadraticC = dotProduct(relativeStart, relativeStart) - (impactRadius * impactRadius);
        ...
        if (quadraticC <= 0) {
            ...
            if (discriminant >= 0) {
                ...
                if (candidateTimes.length > 0) {
                    impactTimeSeconds = clamp(Math.min(...candidateTimes), 0, stepSeconds);
        ...
        const resolvedImpactTimeSeconds = impactTimeSeconds === null
            ? closestTimeSeconds
            : impactTimeSeconds;
        const missileImpactPosition = addVectors(normalizedMissileStart, scaleVector(missileStepVelocity, resolvedImpactTimeSeconds));
        const targetImpactPosition = addVectors(normalizedTargetStart, scaleVector(targetStepVelocity, resolvedImpactTimeSeconds));
        return {
            impactOccurred: impactTimeSeconds !== null,
            collisionReason,
            impactTimeSeconds: impactTimeSeconds === null
                ? null
                : roundNumber(impactTimeSeconds, 6),
            impactTimeMs: impactTimeSeconds === null
                ? null
    ...
    function advanceMissileMovement(entity, target, deltaSeconds, nowMs) {
        ...
        if (entity.pendingGeometryImpact === true) {
            const frozenPosition = cloneVector(entity.pendingGeometryImpactPosition, entity.position);
            ...
            entity.lastMissileStep = {
                ...
                pendingGeometryImpactAtMs: roundNumber(toFiniteNumber(entity.pendingGeometryImpactAtMs, 0), 3),
        ...
        const targetStepStartPosition = resolveMissileTargetStepStartPosition(target, deltaSeconds);
        const desiredDirection = normalizeVector(subtractVectors(targetPosition, previousPosition), entity.direction);
        const stepStartMs = Math.max(0, toFiniteNumber(nowMs, 0) - (Math.max(0, deltaSeconds) * 1000));
        const launchAtMs = Math.max(0, toFiniteNumber(entity.launchedAtMs, stepStartMs));
        ...
        const activeDeltaSeconds = Math.max(0, (Math.max(activeStepStartMs, toFiniteNumber(nowMs, 0)) - activeStepStartMs) /
            1000);
        const remainingFlightSeconds = toFiniteNumber(entity.expiresAtMs, 0) > 0
            ? Math.max(0, (toFiniteNumber(entity.expiresAtMs, 0) - activeStepStartMs) / 1000)
            : Math.max(0, deltaSeconds);
        const effectiveDeltaSeconds = Math.min(Math.max(0, deltaSeconds), activeDeltaSeconds, remainingFlightSeconds);
        ...
        entity.followRange = resolveMissileFollowRange(entity.radius, target && target.radius);
        ...
        entity.velocity = desiredSpeed > 0
            ? desiredVelocity
            : { x: 0, y: 0, z: 0 };
        entity.position = addVectors(previousPosition, scaleVector(entity.velocity, effectiveDeltaSeconds));
        ...
        const sweptImpact = resolveMissileSweptImpact(previousPosition, entity.velocity, targetStepStartPosition, targetPosition, Math.max(1, toFiniteNumber(target.radius, 0)), effectiveDeltaSeconds);
        const rawSurfaceDistanceAfter = surfaceDistanceAfter;
        const legacyHeuristicImpactSurface = rawSurfaceDistanceAfter <= 0.001 ||
            surfaceDistanceBefore <= (stepDistance + 0.001);
        if (sweptImpact.impactOccurred) {
            const impactPosition = cloneVector(sweptImpact.missileImpactPosition, entity.position);
            entity.pendingGeometryImpact = true;
            entity.pendingGeometryImpactReason = sweptImpact.collisionReason || "relative-sweep";
            entity.pendingGeometryImpactAtMs = roundNumber(Math.max(stepStartMs, activeStepStartMs + (toFiniteNumber(sweptImpact.impactTimeMs, 0))), 3);
            entity.pendingGeometryImpactPosition = impactPosition;
            entity.position = impactPosition;
            entity.targetPoint = cloneVector(sweptImpact.targetImpactPosition, targetPosition);
            entity.velocity = { x: 0, y: 0, z: 0 };
            entity.speedFraction = 0;
        ...
        entity.lastMissileStep = {
            ...
            reachedImpactSurface: surfaceDistanceAfter <= 0.001 ||
    ...
    function advanceFollowMovement(entity, target, deltaSeconds) {
        ...
        const desiredRange = Math.max(0, toFiniteNumber(entity.followRange, 0) +
            entity.radius +
        ...
        const targetRadialSpeed = dotProduct(target.velocity || { x: 0, y: 0, z: 0 }, radialDirection);
        const desiredDirection = gap > 50
            ? radialDirection
            : normalizeVector(target.velocity, normalizeVector(separation, entity.direction));
        const desiredSpeed = gap > 50
            ? Math.min(entity.maxVelocity, Math.max(0, targetRadialSpeed +
                Math.max(gap * 0.5, entity.maxVelocity * 0.25)))
            : Math.min(entity.maxVelocity, targetSpeed);
        entity.targetPoint = targetPoint;
        const movementResult = applyDesiredVelocity(entity, desiredDirection, desiredSpeed, deltaSeconds);
    ...
    function advanceOrbitMovement(entity, target, deltaSeconds) {
        ...
        const tangentDirection = normalizeVector(scaleVector(crossProduct(orbitNormal, radialDirection), entity.orbitSign || 1), entity.direction);
        const currentDistance = magnitude(radialVector);
        const desiredDistance = Math.max(toFiniteNumber(entity.orbitDistance, 0) + entity.radius + (target.radius || 0), entity.radius + (target.radius || 0) + 500);
        const radialError = currentDistance - desiredDistance;
        const correction = scaleVector(radialDirection, clamp(-radialError / Math.max(desiredDistance, 1), -0.75, 0.75));
        const desiredDirection = normalizeVector(addVectors(tangentDirection, correction), tangentDirection);
        const desiredSpeed = clamp(Math.max(entity.maxVelocity * 0.35, Math.abs(radialError) * 0.5), 0, entity.maxVelocity);
        entity.orbitNormal = orbitNormal;
        entity.targetPoint = addVectors(target.position, scaleVector(radialDirection, desiredDistance));
    ...
    function advanceMovement(entity, scene, deltaSeconds, now) {
        ...
        switch (entity.mode) {
            ...
            case "FOLLOW":
                return advanceFollowMovement(entity, scene.getEntityByID(entity.targetEntityID), deltaSeconds);
            case "MISSILE":
                return advanceMissileMovement(entity, scene.getEntityByID(entity.targetEntityID), deltaSeconds, now);
            case "ORBIT":
                return advanceOrbitMovement(entity, scene.getEntityByID(entity.targetEntityID), deltaSeconds);
            case "WARP": {
                ...
                const nativeSessionlessIngress = entity.sessionlessWarpIngress &&
                    entity.sessionlessWarpIngress.useNativeWarpProfile === true
                ...
                if (!nativeSample.ready) {
                    return {
                        ...
                        nativeWarpModeledAtMs: Number.isFinite(Number(entity.warpState.nativeSampleAtMs))
                ...
                let progress = getWarpProgress(entity.warpState, sampledAtMs);
                ...
                const firstUnsampledNativeTick = Math.max(1, toFiniteNumber(nativeSample.previousElapsedNativeTicks, 0) + 1);
                if (progress.complete &&
                    ...
                    while (low < high) {
                        ...
                        const middleProgress = getWarpProgress(entity.warpState, toFiniteNumber(entity.warpState.startTimeMs, now) +
                    ...
                    sampledStamp = advanceDestinyStamp(nativeSample.activationStamp, sampledNativeTick);
                    progress = getWarpProgress(entity.warpState, sampledAtMs);
                }
                const direction = normalizeVector(subtractVectors(entity.warpState.targetPoint, entity.warpState.origin), entity.direction);
                entity.direction = direction;
                const traveledDistance = Math.max(0, toFiniteNumber(progress.traveled, 0));
                entity.position = addVectors(entity.warpState.origin, scaleVector(direction, traveledDistance));
                entity.velocity = scaleVector(direction, Math.max(0, toFiniteNumber(progress.speed, 0)));
                if (progress.complete) {
                    ...
                    completedWarpState.dropoutSpeedMs = Math.max(0, toFiniteNumber(progress.dropoutSpeedMs, 0));
                    completedWarpState.remainingDistance = Math.max(0, toFiniteNumber(entity.warpState.totalDistance, 0) -
                        traveledDistance);
                    const _warpCompleteTargetEntity = entity.targetEntityID
                        ? scene.getEntityByID(entity.targetEntityID)
                        : null;
                    const _warpCompleteTargetRadius = Math.max(0, toFiniteNumber(_warpCompleteTargetEntity && _warpCompleteTargetEntity.radius, 0));
                    ...
                    const _warpCompleteDistEdge = _warpCompleteDistCenter !== null
                        ...
                        : null;
                    log.info(`[WarpComplete] itemID=${entity.itemID} typeID=${entity.typeID} targetEntityID=${entity.targetEntityID || "none"}` +
                    ...
                    const nativeStopEvolutionTicks = Math.max(1, Math.trunc(nativeSample.elapsedNativeTicks - sampledNativeTick) + 1);
                    const officialInertialMass = toFiniteNumber(entity.mass, 0) * toFiniteNumber(entity.inertia, 0);
                    const fallbackInertialMass = Math.max(deriveAgilitySeconds(entity.alignTime, entity.maxAccelerationTime, entity.mass, entity.inertia), 0.05) * 1_000_000;
                    const stopEvolution = integrateCarbonMotion(entity.position, entity.velocity, { x: 0, y: 0, z: 0 }, officialInertialMass > 0
                        ? officialInertialMass
                        : fallbackInertialMass, undefined, (nativeStopEvolutionTicks * DESTINY_STAMP_INTERVAL_MS) / 1000);
                    ...
                    return {
                        ...
                        warpCompleted: true,
                        completedWarpState,
                        suppressCompletionStopUpdates: nativeSessionlessIngress &&
