...
const { resolvePilotWarpPhasePresentation, } = require("./pilotWarpPhase");
const { DESTINY_STAMP_MAX_FORWARD_LEAD, normalizeDestinyStamp, selectFurthestDestinyStamp, } = require("../delivery/stamps");
function handleActiveWarpTickPresentation(options = {}) {
    ...
    const warpCorrectionStamp = selectFurthestDestinyStamp(warpCommandStamp, [normalizeDestinyStamp(getMovementStamp(nowMs), warpCommandStamp)], maximumForwardLead);
    ...
    if (!result.warpCompleted && session && isReadyForDestiny(session)) {
        const pilotWarpPhase = resolvePilotWarpPhasePresentation({
            ...
            maximumForwardLead,
            shouldSchedulePilotWarpCruiseBump: options.shouldSchedulePilotWarpCruiseBump,
        ...
        if (pilotWarpPhase.updates.length > 0 &&
...
//# sourceMappingURL=activeWarpTick.js.map