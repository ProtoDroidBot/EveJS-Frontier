...
const { getCurrentDestinyStamp, getDestinyStampForwardDistance, normalizeDestinyStamp, } = require("../delivery/stamps");
function toFiniteNumber(value, fallback = 0) {
    const numeric = Number(value);
    return Number.isFinite(numeric) ? numeric : fallback;
}
function getNativeBoundaryAtMs(nowMs, intervalMs = DESTINY_STAMP_INTERVAL_MS) {
...
function acquireActiveWarpNativeSample(entity, nowMs, options = {}) {
    ...
    const intervalMs = Math.max(1, toFiniteNumber(options.intervalMs, DESTINY_STAMP_INTERVAL_MS));
    const activationAtMs = toFiniteNumber(warpState.startTimeMs, getNativeBoundaryAtMs(nowMs, intervalMs));
    ...
    const elapsedNativeTicks = getDestinyStampForwardDistance(activationStamp, currentStamp);
    const previousElapsedNativeTicks = getDestinyStampForwardDistance(activationStamp, previousSampleStamp);
...
//# sourceMappingURL=warpNativeBoundary.js.map