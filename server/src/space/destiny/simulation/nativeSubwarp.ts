...
const NATIVE_SUBWARP_MODES: Record<string, any> = {};
...
function createNativeSubwarpController(deps: Record<string, any> = {}) {
  const {
  ...
  function getCadenceAnchorMs(previousFrame, overrideAnchorMs?: any) {
  ...
  function buildAccelerationControl(mode, plan, extra: Record<string, any> = {}) {
  ...
  function isSceneIntervalPlanCurrent(plan, scene, options: Record<string, any> = {}) {
    if (
      !plan ||
      !scene ||
      plan.complete !== true ||
    ...
    const seenEntityKeys = new Set<any>();
  ...
  function prepareFrame(scene, nowMs, options: Record<string, any> = {}) {
    ...
    return {
      ...
      sourceEntitiesByID,
      controlsByID,
      consumedControlIDs: new Set<any>(),
    };
  }

  function prepareHeldFrame(scene, nowMs, options: Record<string, any> = {}) {
  ...
  function rebindFrame(frame, scene, options: Record<string, any> = {}) {
    ...
    const sourceEntitiesByID = new Map();
    const controlsByID = new Map();
    const consumedControlIDs = new Set<any>();
  ...
  function buildSceneIntervalSegments(startSimTimeMs, endSimTimeMs, options: Record<string, any> = {}) {
  ...
  function recomputeRawProjectedEndpoints(options: Record<string, any> = {}) {
    const {
      advanceMovement,
      cloneEntity,
      endSimTimeMs,
  ...
  function planSceneInterval(scene, startSimTimeMs, endSimTimeMs, options: Record<string, any> = {}) {
    ...
    const interval: Record<string, any> = resynchronized
      ? {
          complete: true,
  ...
  function refreshSceneIntervalProjections(plan, scene, options: Record<string, any> = {}) {
    if (
      !plan ||
      plan.complete !== true ||
      !scene ||
  ...
  function advanceSceneInterval(scene, startSimTimeMs, endSimTimeMs, options: Record<string, any> = {}) {
    const start = Math.max(0, Number(startSimTimeMs) || 0);
    const end = Math.max(start, Number(endSimTimeMs) || start);
    const resynchronized =
