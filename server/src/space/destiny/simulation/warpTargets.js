...
function createDestinyWarpTargetPlanner(deps = {}) {
    ...
    const addVectors = typeof deps.addVectors === "function"
        ? deps.addVectors
        : (left, right) => ({
    ...
    const cloneVector = typeof deps.cloneVector === "function"
        ? deps.cloneVector
        : (source = null) => ({
    ...
    const dotProduct = typeof deps.dotProduct === "function"
        ? deps.dotProduct
        : (left, right) => (toFiniteNumber(left && left.x, 0) *
            toFiniteNumber(right && right.x, 0) +
            toFiniteNumber(left && left.y, 0) *
                toFiniteNumber(right && right.y, 0) +
            toFiniteNumber(left && left.z, 0) *
                toFiniteNumber(right && right.z, 0));
    const normalizeVector = typeof deps.normalizeVector === "function"
        ? deps.normalizeVector
        : (source, fallback) => cloneVector(source || fallback);
    const resolveStargatePhysicalRadius = typeof deps.resolveStargatePhysicalRadius === "function"
        ? deps.resolveStargatePhysicalRadius
        : () => 0;
    const scaleVector = typeof deps.scaleVector === "function"
        ? deps.scaleVector
        : (vector, scalar) => ({
    ...
    const subtractVectors = typeof deps.subtractVectors === "function"
    ...
    const MAX_STARGATE_JUMP_DISTANCE_METERS = Math.max(toFiniteNumber(deps.MAX_STARGATE_JUMP_DISTANCE_METERS, 2500), 0);
    const WARP_EXIT_VARIANCE_RADIUS_METERS = Math.max(toFiniteNumber(deps.WARP_EXIT_VARIANCE_RADIUS_METERS, 2500), 0);
    ...
    function getStargateWarpExitPoint(entity, stargate, minimumRange = 0) {
        ...
        return addVectors(gatePosition, scaleVector(fromGateToShip, requestedRange));
    }
    ...
    function getStargateWarpLandingPoint(entity, stargate, minimumRange = 0) {
        ...
        const scatterRadius = requestedClearance > MAX_STARGATE_JUMP_DISTANCE_METERS
            ? WARP_EXIT_VARIANCE_RADIUS_METERS
            : 0;
        return addVectors(getStargateWarpExitPoint(entity, stargate, minimumRange), getRandomPointInSphere(scatterRadius));
    }
    function getRandomPointInHemisphere(radius, normal) {
        ...
        return subtractVectors(point, scaleVector(hemisphereNormal, projection * 2));
    }
    function resolveStargateWarpTarget(entity, stargate, minimumRange = 0) {
        const requestedRange = Math.max(0, toFiniteNumber(minimumRange, 0));
        const nativeStopDistance = Math.max(1, Math.random() * WARP_EXIT_VARIANCE_RADIUS_METERS);
        const gatePosition = cloneVector(stargate && stargate.position);
        const fallbackDirection = normalizeVector(entity && entity.direction ? entity.direction : DEFAULT_RIGHT, DEFAULT_RIGHT);
        const fromGateToShip = normalizeVector(subtractVectors(entity && entity.position, gatePosition), fallbackDirection);
        const travelDirection = scaleVector(fromGateToShip, -1);
        const warpExitPoint = getStargateWarpExitEnvelopePoint(entity, stargate, requestedRange);
        const landingPoint = addVectors(warpExitPoint, getRandomPointInHemisphere(WARP_EXIT_VARIANCE_RADIUS_METERS, fromGateToShip));
        return {
            rawDestination: addVectors(landingPoint, scaleVector(travelDirection, nativeStopDistance)),
...
//# sourceMappingURL=warpTargets.js.map