...
function createDestinyWarpDiagnostics(deps = {}) {
    const toFiniteNumber = typeof deps.toFiniteNumber === "function"
        ? deps.toFiniteNumber
        : defaultToFiniteNumber;
    const roundNumber = typeof deps.roundNumber === "function"
        ? deps.roundNumber
        : defaultRoundNumber;
    const distance = typeof deps.distance === "function"
        ? deps.distance
        : () => 0;
    const magnitude = typeof deps.magnitude === "function"
        ? deps.magnitude
        : () => 0;
    const getCurrentDestinyStamp = typeof deps.getCurrentDestinyStamp === "function"
        ? deps.getCurrentDestinyStamp
        : () => 0;
    const getWarpProgress = typeof deps.getWarpProgress === "function"
        ? deps.getWarpProgress
        : () => ({ complete: false, speed: 0, traveled: 0 });
    const ONE_AU_IN_METERS = Math.max(toFiniteNumber(deps.ONE_AU_IN_METERS, 149597870700), 1);
    const WARP_DROPOUT_SPEED_MAX_MS = Math.max(toFiniteNumber(deps.WARP_DROPOUT_SPEED_MAX_MS, 100), 1);
    function buildOfficialWarpReferenceProfile(warpDistanceMeters, warpSpeedAU, maxSubwarpSpeedMs) {
        ...
        const resolvedSubwarpSpeedMs = Math.max(Math.min(toFiniteNumber(maxSubwarpSpeedMs, 0) / 2, WARP_DROPOUT_SPEED_MAX_MS), 1);
        ...
        const decelTimeSeconds = Math.log(Math.max(maxWarpSpeedMs / resolvedSubwarpSpeedMs, 1)) / kDecel;
        const totalTimeSeconds = accelTimeSeconds + cruiseTimeSeconds + decelTimeSeconds;
        return {
            profileType,
            ...
            cruiseDistance: roundNumber(Math.max(totalDistance - accelDistance - decelDistance, 0), 3),
            cruiseDistanceAU: roundNumber(Math.max(totalDistance - accelDistance - decelDistance, 0) /
                ONE_AU_IN_METERS, 6),
            decelDistance: roundNumber(decelDistance, 3),
            decelDistanceAU: roundNumber(decelDistance / ONE_AU_IN_METERS, 6),
            minimumDistance: roundNumber(Math.min(minimumDistance, totalDistance), 3),
            minimumDistanceAU: roundNumber(Math.min(minimumDistance, totalDistance) / ONE_AU_IN_METERS, 6),
    ...
    function buildWarpProfileDelta(warpState, officialProfile) {
        ...
        return {
            durationMs: roundNumber(toFiniteNumber(warpState.durationMs, 0) -
                toFiniteNumber(officialProfile.totalTimeMs, 0), 3),
            accelTimeMs: roundNumber(toFiniteNumber(warpState.accelTimeMs, 0) -
                toFiniteNumber(officialProfile.accelTimeMs, 0), 3),
            cruiseTimeMs: roundNumber(toFiniteNumber(warpState.cruiseTimeMs, 0) -
                toFiniteNumber(officialProfile.cruiseTimeMs, 0), 3),
            decelTimeMs: roundNumber(toFiniteNumber(warpState.decelTimeMs, 0) -
                toFiniteNumber(officialProfile.decelTimeMs, 0), 3),
            maxWarpSpeedMs: roundNumber(toFiniteNumber(warpState.maxWarpSpeedMs, 0) -
                toFiniteNumber(officialProfile.maxWarpSpeedMs, 0), 3),
            accelDistance: roundNumber(toFiniteNumber(warpState.accelDistance, 0) -
                toFiniteNumber(officialProfile.accelDistance, 0), 3),
            cruiseDistance: roundNumber(toFiniteNumber(warpState.cruiseDistance, 0) -
                toFiniteNumber(officialProfile.cruiseDistance, 0), 3),
            decelDistance: roundNumber(toFiniteNumber(warpState.decelDistance, 0) -
                toFiniteNumber(officialProfile.decelDistance, 0), 3),
        };
    }
    ...
    function buildWarpRuntimeDiagnostics(entity, now = Date.now()) {
        ...
        const elapsedMs = Math.max(0, toFiniteNumber(now, Date.now()) -
            toFiniteNumber(warpState.startTimeMs, now));
        const progress = getWarpProgress(warpState, now);
        const positionRemainingDistance = Math.max(distance(entity.position, warpState.targetPoint), 0);
        const profileRemainingDistance = Math.max(toFiniteNumber(warpState.totalDistance, 0) -
            toFiniteNumber(progress.traveled, 0), 0);
        ...
        return {
            ...
            remainingMs: roundNumber(Math.max(toFiniteNumber(warpState.durationMs, 0) - elapsedMs, 0), 3),
            progressComplete: Boolean(progress.complete),
            progressDistance: roundNumber(toFiniteNumber(progress.traveled, 0), 3),
            progressDistanceAU: roundNumber(toFiniteNumber(progress.traveled, 0) / ONE_AU_IN_METERS, 6),
            progressRemainingDistance: roundNumber(profileRemainingDistance, 3),
            progressRemainingDistanceAU: roundNumber(profileRemainingDistance / ONE_AU_IN_METERS, 6),
            progressSpeedMs: roundNumber(toFiniteNumber(progress.speed, 0), 3),
            progressSpeedAU: roundNumber(toFiniteNumber(progress.speed, 0) / ONE_AU_IN_METERS, 6),
            ...
            positionRemainingDistanceAU: roundNumber(positionRemainingDistance / ONE_AU_IN_METERS, 6),
            remainingDistanceDelta: roundNumber(positionRemainingDistance - profileRemainingDistance, 3),
        };
    }
    return {
...
//# sourceMappingURL=warpDiagnostics.js.map