...
const { PILOT_WARP_ACTIVATION_DELAY_DESTINY_TICKS, } = require("./warpContract");
const { clonePilotWarpMaxSpeedRamp, } = require("./warpRamp");
const { buildSetMaxSpeedPayload, } = require("../stream/actions");
const { DESTINY_STAMP_MAX_FORWARD_LEAD, hasDestinyStamp, isDestinyStampAfter, normalizeDestinyStamp, } = require("../delivery/stamps");
...
function resolvePilotWarpPhasePresentation(options = {}) {
    ...
    const maximumForwardLead = Math.max(0, toInt(options.maximumForwardLead, DESTINY_STAMP_MAX_FORWARD_LEAD));
    ...
    const warpEffectStamp = hasDestinyStamp(warpState.effectStamp)
        ...
        : null;
    const shouldSchedulePilotWarpCruiseBump = typeof options.shouldSchedulePilotWarpCruiseBump === "function"
        ? options.shouldSchedulePilotWarpCruiseBump
        : () => false;
    const getPilotWarpCruiseBumpAtMs = typeof options.getPilotWarpCruiseBumpAtMs === "function"
        ? options.getPilotWarpCruiseBumpAtMs
        : () => 0;
    const getPilotWarpEffectAtMs = typeof options.getPilotWarpEffectAtMs === "function"
        ? options.getPilotWarpEffectAtMs
        : () => 0;
    const warpCruiseBumpAtMs = toFiniteNumber(warpState.cruiseBumpAtMs, shouldSchedulePilotWarpCruiseBump(warpState)
        ? getPilotWarpCruiseBumpAtMs(warpState)
        : 0);
    const warpEffectAtMs = toFiniteNumber(warpState.effectAtMs, getPilotWarpEffectAtMs(warpState));
    const pilotMaxSpeedRamp = clonePilotWarpMaxSpeedRamp(warpState.pilotMaxSpeedRamp);
    let dueRampIndex = toInt(entity.lastPilotWarpMaxSpeedRampIndex, -1);
    for (let index = dueRampIndex + 1; index < pilotMaxSpeedRamp.length; index += 1) {
    ...
    const shouldSendCruiseBump = hasDestinyStamp(warpCruiseBumpStamp) &&
        isDestinyStampAfter(warpCommandStamp, warpCruiseBumpStamp, maximumForwardLead) &&
        nowMs >= warpCruiseBumpAtMs &&
        entity.lastPilotWarpCruiseBumpStamp !== warpCruiseBumpStamp;
    const shouldSendEffect = hasDestinyStamp(warpEffectStamp) &&
        isDestinyStampAfter(warpCommandStamp, warpEffectStamp, maximumForwardLead) &&
    ...
    if (foldRampIntoCruise) {
        ...
        rampDebug = {
            ...
            speedAU: roundNumber(rampEntry.speed /
                Math.max(1, toFiniteNumber(options.oneAuInMeters, 1)), 6),
        };
    }
    if (shouldSendCruiseBump &&
...
//# sourceMappingURL=pilotWarpPhase.js.map