...
function createDestinyWarpStateHelpers(deps: Record<string, any> = {}) {
  const {
    addVectors,
    clamp,
  ...
  function buildWarpProfile(entity, destination, options: Record<string, any> = {}) {
  ...
  function buildPendingWarp(rawPendingWarp, position: Record<string, any> = { x: 0, y: 0, z: 0 }) {
  ...
  function buildPendingWarpRequest(entity, destination, options: Record<string, any> = {}) {
    const rawDestination = cloneVector(destination, entity.position);
    const stopDistance = Math.max(0, toFiniteNumber(options.stopDistance, 0));
    const travelVector = subtractVectors(rawDestination, entity.position);
    const direction = normalizeVector(travelVector, entity.direction);
  ...
  function buildPreparingWarpState(entity, pendingWarp, options: Record<string, any> = {}) {
    const warpState: Record<string, any> = buildWarpProfile(entity, pendingWarp && pendingWarp.rawDestination, {
  ...
  function activatePendingWarp(entity, pendingWarp, options: Record<string, any> = {}) {
    if (
  ...
  function buildSessionlessWarpIngressState(entity, warpState, options: Record<string, any> = {}) {
  ...
  function getWarpStopDistanceForTarget(shipEntity, targetEntity, minimumRange = 0) {
    const targetRadius = Math.max(
      0,
      toFiniteNumber(getEntityCollisionBroadphaseRadius(targetEntity), 0),
    );
    const shipRadius = Math.max(
      0,
      toFiniteNumber(getEntityCollisionBroadphaseRadius(shipEntity), 0),
    );
    const desiredRange = Math.max(0, toFiniteNumber(minimumRange, 0));
    const safeSurfaceDistance =
      targetRadius + shipRadius + WARP_ARRIVAL_SURFACE_CLEARANCE_METERS;
    ...
    switch (targetEntity && targetEntity.kind) {
      ...
      case "station":
        ...
        break;
      case "stargate":
        authoredDistance = Math.max(
          Math.max(2500, targetRadius * 0.3),
          desiredRange,
        ) + (shipRadius * 2);
        break;
      case "asteroidBelt":
