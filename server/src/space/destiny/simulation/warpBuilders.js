...
const { currentFileTime, } = require("../../../services/_shared/serviceHelpers");
const { tagUpdatesRequireExistingVisibility, } = require("../dispatch/dispatchUtils.js");
const { buildSetMaxSpeedPayload, } = require("../stream/actions");
const { isNativeWarpDistanceTooShort, } = require("./warp");
const { advanceDestinyStamp, hasDestinyStamp, normalizeDestinyStamp, selectFurthestDestinyStamp, } = require("../delivery/stamps");
function createDestinyWarpUpdateBuilders(deps = {}) {
    const { clamp, cloneVector, getCurrentDestinyStamp, getNextStamp, magnitude, normalizeVector, scaleVector, subtractVectors, toFiniteNumber, toInt, DEFAULT_RIGHT, DESTINY_STAMP_INTERVAL_MS, ENABLE_PILOT_WARP_FACTOR_OPTION_A, ENABLE_PILOT_WARP_MAX_SPEED_RAMP, ENABLE_PILOT_WARP_SOLVER_ASSIST_OPTION_B, INDIVIDUAL_WARP_FACTORS, MAX_SUBWARP_SPEED_FRACTION, PILOT_WARP_FACTOR_OPTION_A_SCALE, PILOT_WARP_SOLVER_ASSIST_LEAD_MS, PILOT_WARP_SOLVER_ASSIST_SCALE, PILOT_WARP_SPEED_RAMP_FRACTIONS, PILOT_WARP_SPEED_RAMP_SCALES, WARP_NATIVE_ACTIVATION_SPEED_FRACTION, WARP_NATIVE_ACTIVATION_SPEED_MARGIN_MS, WARP_START_ACTIVATION_SEED_SCALE, } = deps;
    function getRawWarpDistance(entity, warpState) {
        ...
        return Math.hypot(toFiniteNumber(destination.x, 0) - toFiniteNumber(origin.x, 0), toFiniteNumber(destination.y, 0) - toFiniteNumber(origin.y, 0), toFiniteNumber(destination.z, 0) - toFiniteNumber(origin.z, 0));
    }
    function buildPilotPreWarpAddBallUpdate(entity, stamp) {
        return {
            ...
            payload: destiny.buildAddBallPayload(entity.itemID, {
                ...
                speedFraction: clamp(toFiniteNumber(entity.speedFraction, 1), 0, MAX_SUBWARP_SPEED_FRACTION),
            }),
        };
    }
    function buildPilotPreWarpRebaselineUpdates(entity, pendingWarp, stamp) {
        ...
        if (speed > 0.5 &&
            ...
            const warpDir = normalizeVector(subtractVectors(pendingWarp.rawDestination || pendingWarp.targetPoint, entity.position), entity.direction);
            alignedVelocity = scaleVector(warpDir, speed);
        }
        return [
            {
                stamp,
                payload: destiny.buildSetBallVelocityPayload(entity.itemID, alignedVelocity),
            },
        ];
    }
    function buildPilotWarpEgoStateRefreshUpdates(system, entity, stamp, simFileTime = currentFileTime()) {
        ...
        return [
            ...
            {
                stamp,
                payload: destiny.buildSetStatePayload(stamp, system, entity.itemID, egoEntities, simFileTime),
            },
        ];
    }
    function getNominalWarpFactor(entity, warpState) {
        const authoredWarpSpeedAU = toFiniteNumber(entity && entity.warpSpeedAU, 0);
        return Math.max(1, toInt(warpState && warpState.warpSpeed, Math.round((authoredWarpSpeedAU > 0 ? authoredWarpSpeedAU : 3) * 1000)));
    }
    function buildIndividualWarpFactorsUpdate(entity, stamp) {
        ...
        return {
            stamp,
            payload: destiny.buildSetBallWarpFactorsPayload(entity.itemID, INDIVIDUAL_WARP_FACTORS.acceleration, INDIVIDUAL_WARP_FACTORS.deceleration),
        };
    }
    function getPilotWarpFactorOptionA(entity, warpState) {
        ...
        return Math.max(nominalWarpFactor + 1, Math.round(nominalWarpFactor * PILOT_WARP_FACTOR_OPTION_A_SCALE));
    }
    function buildWarpStartCommandUpdate(entity, stamp, warpState, options = {}) {
        const frozenNativeCommand = String(warpState && warpState.nativeWarpCommand || "").trim().toUpperCase();
        if (frozenNativeCommand === "GOTO" ||
            (frozenNativeCommand !== "WARP" &&
                isNativeWarpDistanceTooShort(getRawWarpDistance(entity, warpState)))) {
            return {
                stamp,
                payload: destiny.buildGotoPointPayload(entity.itemID, (warpState && warpState.rawDestination) ||
                    (entity && entity.targetPoint) ||
                    (entity && entity.position)),
            };
        }
        const warpFactor = Math.max(1, toInt(options.warpFactor, getNominalWarpFactor(entity, warpState)));
        return {
            stamp,
            payload: destiny.buildWarpToPayload(entity.itemID, warpState.rawDestination, warpState.stopDistance, warpFactor),
        };
    }
    ...
    function getPilotWarpPeakSpeed(entity, warpState) {
        ...
        const peakWarpSpeedMs = warpState.profileType === "short"
            ? toFiniteNumber(warpState.maxWarpSpeedMs, 0)
            : toFiniteNumber(warpState.cruiseWarpSpeedMs, 0);
        return Math.max(peakWarpSpeedMs, toFiniteNumber(warpState.maxWarpSpeedMs, 0), toFiniteNumber(entity && entity.maxVelocity, 0));
    }
    function shouldSchedulePilotWarpCruiseBump(warpState) {
        return (ENABLE_PILOT_WARP_MAX_SPEED_RAMP &&
    ...
    function getPilotWarpStartupGuidanceStamp(warpStartStamp, warpState) {
        const scheduledStamp = getCurrentDestinyStamp(getPilotWarpStartupGuidanceAtMs(warpState));
        const normalizedWarpStartStamp = normalizeDestinyStamp(warpStartStamp, 0);
        return selectFurthestDestinyStamp(normalizedWarpStartStamp, [
    ...
    function getPilotWarpCruiseBumpAtMs(warpState) {
        ...
        return Math.max(accelEndAtMs, startupGuidanceAtMs + DESTINY_STAMP_INTERVAL_MS);
    }
    ...
    function getPilotWarpActivationSeedSpeed(entity) {
        return Math.max(toFiniteNumber(entity && entity.maxVelocity, 0) * WARP_START_ACTIVATION_SEED_SCALE, 0);
    }
    function getPilotWarpActivationKickoffSpeed(entity, warpState) {
        ...
        const firstRampScale = clamp(PILOT_WARP_SPEED_RAMP_SCALES[0], 0, 0.95);
        return Math.max(peakWarpSpeedMs * firstRampScale, getPilotWarpActivationSeedSpeed(entity), toFiniteNumber(entity && entity.maxVelocity, 0) + 1);
    }
    function buildPilotWarpMaxSpeedRamp(entity, warpState, warpStartStamp) {
        ...
        if (ENABLE_PILOT_WARP_SOLVER_ASSIST_OPTION_B) {
            ...
            const decelAssistAtMs = Math.max(startTimeMs, (startTimeMs + accelTimeMs + cruiseTimeMs) - PILOT_WARP_SOLVER_ASSIST_LEAD_MS);
            const resolvedStamp = selectFurthestDestinyStamp(previousStamp, [
                advanceDestinyStamp(previousStamp, 1),
                normalizeDestinyStamp(getCurrentDestinyStamp(decelAssistAtMs), previousStamp),
            ]);
            const assistSpeed = Math.max(getPilotWarpActivationSeedSpeed(entity) * PILOT_WARP_SOLVER_ASSIST_SCALE, toFiniteNumber(entity && entity.maxVelocity, 0) + 1);
            if (assistSpeed > 0) {
                ramp.push({
    ...
    function buildPilotWarpRampMaxSpeedUpdate(entity, stampOverride, speed) {
    ...
    function buildPilotWarpSeedUpdate(entity, stamp) {
        return {
            stamp,
            payload: destiny.buildSetMaxSpeedPayload(entity.itemID, getPilotWarpActivationSeedSpeed(entity)),
        };
    }
    function buildPilotWarpActivationKickoffUpdate(entity, stamp, warpState) {
        return {
            stamp,
            payload: destiny.buildSetMaxSpeedPayload(entity.itemID, getPilotWarpActivationKickoffSpeed(entity, warpState)),
        };
    }
    function buildWarpCruiseMaxSpeedUpdate(entity, stamp, warpState) {
        return {
            stamp,
            payload: destiny.buildSetMaxSpeedPayload(entity.itemID, getPilotWarpPeakSpeed(entity, warpState)),
        };
    }
    function buildEntityWarpInUpdate(entity, stamp, warpState) {
        const warpFactor = Math.max(1, toInt(getNominalWarpFactor(entity, warpState), 30));
        return {
            stamp,
            payload: destiny.buildEntityWarpInPayload(entity.itemID, warpState.targetPoint, warpFactor),
        };
    }
    function getPilotWarpNativeActivationSpeedFloor(entity) {
        const nativeMaximumSpeed = getPilotWarpActivationSeedSpeed(entity);
        return Math.max((nativeMaximumSpeed *
            WARP_NATIVE_ACTIVATION_SPEED_FRACTION) +
            WARP_NATIVE_ACTIVATION_SPEED_MARGIN_MS, WARP_NATIVE_ACTIVATION_SPEED_MARGIN_MS);
    }
    function buildWarpActivationVelocityUpdate(entity, stamp, warpState) {
        ...
        const targetPoint = cloneVector(warpState && (warpState.rawDestination || warpState.targetPoint), entity && entity.targetPoint);
        const direction = normalizeVector(subtractVectors(targetPoint, cloneVector(entity && entity.position)), cloneVector(entity && entity.direction, DEFAULT_RIGHT));
        const resolvedVelocity = scaleVector(direction, Math.max(currentSpeed, activationSpeedFloor));
    ...
    function buildWarpStartVelocityCarryoverUpdate(entity, stamp, warpState) {
        const startupGuidanceVelocity = cloneVector(warpState && warpState.startupGuidanceVelocity, { x: 0, y: 0, z: 0 });
        if (magnitude(startupGuidanceVelocity) <= 0.5) {
            return null;
        }
        return {
            stamp,
            payload: destiny.buildSetBallVelocityPayload(entity.itemID, startupGuidanceVelocity),
        };
    }
    function primePilotWarpActivationState(entity, warpState, warpStartStamp, options = {}) {
        if (!entity || !warpState) {
        ...
        warpState.cruiseBumpStamp = shouldSchedulePilotWarpCruiseBump(warpState)
            ...
            : null;
        const effectDelayDestinyTicks = Math.max(0, toInt(options.effectDelayDestinyTicks, 0));
        warpState.effectAtMs = getPilotWarpEffectAtMs(warpState) +
            (effectDelayDestinyTicks * DESTINY_STAMP_INTERVAL_MS);
        warpState.effectStamp = advanceDestinyStamp(getPilotWarpEffectStamp(resolvedStamp, warpState), effectDelayDestinyTicks);
        warpState.pilotMaxSpeedRamp = buildPilotWarpMaxSpeedRamp(entity, warpState, resolvedStamp);
        return warpState;
    }
    function getWatcherWarpStartStamp(warpState, pendingWarp, activationStamp) {
        const normalizedActivationStamp = normalizeDestinyStamp(activationStamp, getNextStamp());
        return selectFurthestDestinyStamp(normalizedActivationStamp, [
    ...
    function buildWarpPrepareDispatch(entity, stamp, warpState, options = {}) {
        ...
        if (options.includePilotActivationVelocity === true) {
            const activationVelocityUpdate = buildWarpActivationVelocityUpdate(entity, stamp, warpState);
        ...
        if (options.includePilotEffect !== false) {
            pilotPrepareUpdates.splice(pilotPrepareUpdates.length - 1, 0, buildWarpStartEffectUpdate(entity, stamp));
        }
        return {
            sharedUpdates,
    ...
    function buildWarpStartUpdates(entity, warpState, stampOverride = null, options = {}) {
        const stamp = stampOverride === null
        ...
        const warpCommandIndex = updates.findIndex((update) => (Array.isArray(update && update.payload) && update.payload[0] === "WarpTo"));
        if (options.includeEntityWarpIn !== false && warpCommandIndex >= 0) {
            updates.splice(warpCommandIndex + 1, 0, buildEntityWarpInUpdate(entity, stamp, warpState));
        }
...
//# sourceMappingURL=warpBuilders.js.map