...
function resolveWarpFactorFromWarpSpeedAU(warpSpeedAU) {
    return Math.max(toFiniteNumber(warpSpeedAU, 0) / WARP_FACTOR_TO_AU_PER_SECOND, 0);
}
function getWarpAccelRateFromWarpFactor(warpFactor) {
    return Math.max(toFiniteNumber(warpFactor, 0) * WARP_FACTOR_TO_ACCELERATION, 0.001);
}
function getWarpDecelRateFromWarpFactor(warpFactor, maxDecelRate = NATIVE_WARP_DECEL_RATE_MAX) {
    return Math.min(Math.max(toFiniteNumber(warpFactor, 0) * WARP_FACTOR_TO_DECELERATION, 0.001), toFiniteNumber(maxDecelRate, NATIVE_WARP_DECEL_RATE_MAX));
}
function getWarpAccelRateFromWarpSpeedAU(warpSpeedAU) {
    return getWarpAccelRateFromWarpFactor(resolveWarpFactorFromWarpSpeedAU(warpSpeedAU));
}
function getWarpDecelRateFromWarpSpeedAU(warpSpeedAU, maxDecelRate = NATIVE_WARP_DECEL_RATE_MAX) {
    return getWarpDecelRateFromWarpFactor(resolveWarpFactorFromWarpSpeedAU(warpSpeedAU), maxDecelRate);
}
function getWarpSpeedMetersPerSecondFromWarpFactor(warpFactor, oneAuInMeters) {
    return Math.max(toFiniteNumber(warpFactor, 0) *
        WARP_FACTOR_TO_AU_PER_SECOND *
        toFiniteNumber(oneAuInMeters, 0), 0);
}
function getNativeWarpDropoutSpeed(maxSubwarpVelocity, maximumDropoutSpeed = 100) {
    return Math.max(0, Math.min(toFiniteNumber(maxSubwarpVelocity, 0) / 2, Math.max(0, toFiniteNumber(maximumDropoutSpeed, 100))));
}
