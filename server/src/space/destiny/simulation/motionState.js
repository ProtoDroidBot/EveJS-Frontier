...
const DEFAULT_MAX_SUBWARP_SPEED_FRACTION = Number.isFinite(Number(destinyConstants.MAX_SUBWARP_SPEED_FRACTION))
...
function createDestinyMotionStateHelpers(deps = {}) {
    ...
    const clearFleetWarpCommandAssociation = typeof deps.clearFleetWarpCommandAssociation === "function"
    ...
    const MAX_SUBWARP_SPEED_FRACTION = Number.isFinite(Number(deps.MAX_SUBWARP_SPEED_FRACTION))
    ...
    function buildUndockMovement(entity, direction, speedFraction = 1) {
        ...
        entity.mode = "GOTO";
        const initialSpeed = Math.max(0, toFiniteNumber(entity.maxVelocity, 0)) *
