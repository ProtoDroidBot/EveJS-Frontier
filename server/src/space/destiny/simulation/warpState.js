...
function createDestinyWarpStateHelpers(deps = {}) {
    const { addVectors, clamp, clonePilotWarpMaxSpeedRamp, cloneVector, clearTrackingState, distance, getActualSpeedFraction, getCurrentAlignmentDirection, getTurnMetrics, magnitude, normalizeVector, scaleVector, serializeWarpState, subtractVectors, toFiniteNumber, toInt, DESTINY_STAMP_INTERVAL_MS, ONE_AU_IN_METERS, SESSIONLESS_WARP_INGRESS_DURATION_MS, WARP_COMPLETION_DISTANCE_MAX_METERS, WARP_COMPLETION_DISTANCE_MIN_METERS, WARP_COMPLETION_DISTANCE_RATIO, WARP_DECEL_RATE_MAX, WARP_DROPOUT_SPEED_MAX_MS, WARP_ENTRY_SPEED_FRACTION, } = deps;
    ...
    function getWarpDecelRate(warpSpeedAU) {
        return getWarpDecelRateFromWarpSpeedAU(warpSpeedAU, WARP_DECEL_RATE_MAX);
    }
    function getWarpDropoutSpeedMs(entity) {
        return getNativeWarpDropoutSpeed(entity && entity.maxVelocity, WARP_DROPOUT_SPEED_MAX_MS);
    }
    function getWarpCompletionDistance(warpState) {
        const stopDistance = Math.max(toFiniteNumber(warpState && warpState.stopDistance, 0), 0);
        return clamp(stopDistance * WARP_COMPLETION_DISTANCE_RATIO, WARP_COMPLETION_DISTANCE_MIN_METERS, WARP_COMPLETION_DISTANCE_MAX_METERS);
    }
    function buildWarpProfile(entity, destination, options = {}) {
        ...
        const distanceLimitedWarpSpeedMs = ((totalDistance + 1) * accelRate * decelRate) /
            Math.max(accelRate + decelRate, 0.001);
        const maxWarpSpeedMs = Math.max(0, Math.min(cruiseWarpSpeedMs, distanceLimitedWarpSpeedMs));
        ...
        const cruiseDistance = Math.max(totalDistance - accelDistance - decelDistance, 0);
        const accelTimeMs = (Math.log(Math.max(maxWarpSpeedMs / accelRate, 1)) / accelRate) * 1000;
        ...
        const decelTimeMs = (Math.log(Math.max(maxWarpSpeedMs / decelRate, 1)) / decelRate) * 1000;
        const startupGuidanceVelocity = cloneVector(options.startupGuidanceVelocity, entity.velocity);
        return {
            ...
            startupGuidanceStamp: normalizeDestinyStamp(options.startupGuidanceStamp, 0),
            ...
            effectStamp: resolveEffectDestinyStamp(options.effectStamp, options.defaultEffectStamp),
            targetEntityID: toInt(options.targetEntityID, 0),
            destinationStaticInstanceID: normalizePersistentEntityID(options.destinationStaticInstanceID),
            destinationDungeonRoomKey: String(options.destinationDungeonRoomKey || "").trim() || null,
            destinationDungeonSiteID: normalizePersistentEntityID(options.destinationDungeonSiteID),
            followID: toFiniteNumber(options.followID, 15000),
            followRangeMarker: toFiniteNumber(options.followRangeMarker, -1),
            nativeWarpCommand: resolveNativeWarpCommand(rawDistance, options.nativeWarpCommand),
            profileType,
    ...
    function buildPendingWarp(rawPendingWarp, position = { x: 0, y: 0, z: 0 }) {
        ...
        const stopDistance = Math.max(0, toFiniteNumber(rawPendingWarp.stopDistance, 0));
        const totalDistance = Math.max(0, toFiniteNumber(rawPendingWarp.totalDistance, 0));
        return {
            requestedAtMs: toInt(rawPendingWarp.requestedAtMs, 0),
            preWarpSyncStamp: resolveOptionalDestinyStamp(rawPendingWarp.preWarpSyncStamp),
            prepareStamp: resolveOptionalDestinyStamp(rawPendingWarp.prepareStamp),
            prepareVisibleStamp: resolveOptionalDestinyStamp(rawPendingWarp.prepareVisibleStamp),
            stopDistance,
            totalDistance,
            ...
            destinationStaticInstanceID: normalizePersistentEntityID(rawPendingWarp.destinationStaticInstanceID),
            destinationDungeonRoomKey: String(rawPendingWarp.destinationDungeonRoomKey || "").trim() || null,
            destinationDungeonSiteID: normalizePersistentEntityID(rawPendingWarp.destinationDungeonSiteID),
            nativeWarpCommand: resolveNativeWarpCommand(totalDistance + stopDistance, rawPendingWarp.nativeWarpCommand),
        };
    }
    function buildWarpState(rawWarpState, position, warpSpeedAU) {
        ...
        const resolvedWarpFactor = Math.max(1, Math.round(resolvedWarpSpeedAU * 1000));
        ...
        const startupGuidanceAtMs = toFiniteNumber(rawWarpState.startupGuidanceAtMs, 0);
        const cruiseBumpAtMs = toFiniteNumber(rawWarpState.cruiseBumpAtMs, startTimeMs + Math.max(accelTimeMs, 0));
        const effectAtMs = toFiniteNumber(rawWarpState.effectAtMs, startTimeMs);
        const startupGuidanceVelocity = cloneVector(rawWarpState.startupGuidanceVelocity, { x: 0, y: 0, z: 0 });
        const hydratePersistentIdentity = (value) => (!hasPersistentIdentityValue(value)
            ? null
        ...
        return {
            ...
            warpDropoutSpeedMs: toFiniteNumber(rawWarpState.warpDropoutSpeedMs, toFiniteNumber(rawWarpState.warpFloorSpeedMs, WARP_DROPOUT_SPEED_MAX_MS)),
            ...
            accelRate: Math.max(toFiniteNumber(rawWarpState.accelRate, 0) ||
                toFiniteNumber(rawWarpState.accelExponent, 0) ||
                getWarpAccelRate(resolvedWarpSpeedAU), 0.001),
            decelRate: Math.max(toFiniteNumber(rawWarpState.decelRate, 0) ||
                toFiniteNumber(rawWarpState.decelExponent, 0) ||
                getWarpDecelRate(resolvedWarpSpeedAU), 0.001),
            ...
            startupGuidanceAtMs,
            startupGuidanceStamp: resolveOptionalDestinyStamp(rawWarpState.startupGuidanceStamp),
            startupGuidanceVelocity,
            ...
            activationSpeedMs: Math.max(0, magnitude(startupGuidanceVelocity)),
            cruiseBumpAtMs,
            cruiseBumpStamp: resolveOptionalDestinyStamp(rawWarpState.cruiseBumpStamp),
            effectAtMs,
            effectStamp: resolveEffectDestinyStamp(rawWarpState.effectStamp),
            targetEntityID: toInt(rawWarpState.targetEntityID, 0),
            destinationStaticInstanceID: hydratePersistentIdentity(rawWarpState.destinationStaticInstanceID),
            destinationDungeonRoomKey: String(rawWarpState.destinationDungeonRoomKey || "").trim() || null,
            destinationDungeonSiteID: hydratePersistentIdentity(rawWarpState.destinationDungeonSiteID),
            followID: toInt(rawWarpState.followID, 0),
            followRangeMarker: toFiniteNumber(rawWarpState.followRangeMarker, rawWarpState.stopDistance),
    ...
    function buildPendingWarpRequest(entity, destination, options = {}) {
        ...
        return {
            ...
            prepareVisibleStamp: resolveOptionalDestinyStamp(options.prepareVisibleStamp),
            stopDistance,
            totalDistance,
            warpSpeedAU,
            rawDestination,
    ...
    function buildPreparingWarpState(entity, pendingWarp, options = {}) {
        const warpState = buildWarpProfile(entity, pendingWarp && pendingWarp.rawDestination, {
            ...
            destinationStaticInstanceID: pendingWarp && pendingWarp.destinationStaticInstanceID,
            destinationDungeonRoomKey: pendingWarp && pendingWarp.destinationDungeonRoomKey,
            destinationDungeonSiteID: pendingWarp && pendingWarp.destinationDungeonSiteID,
            warpSpeedAU: pendingWarp && pendingWarp.warpSpeedAU,
            nowMs: options.nowMs === undefined || options.nowMs === null
        ...
        warpState.commandStamp = normalizeDestinyStamp(pendingWarp && pendingWarp.prepareStamp, 0);
        warpState.startupGuidanceAtMs = 0;
        warpState.startupGuidanceStamp = null;
        warpState.startupGuidanceVelocity = cloneVector(entity && entity.velocity, { x: 0, y: 0, z: 0 });
        ...
        warpState.warpRequestedAtMs = toFiniteNumber(pendingWarp && pendingWarp.requestedAtMs, warpState.startTimeMs);
        warpState.pilotPrepareVisibleStamp = toInt(pendingWarp && pendingWarp.prepareVisibleStamp, toInt(pendingWarp && pendingWarp.prepareStamp, 0));
        return warpState;
    }
    ...
    function evaluatePendingWarpAlignment(entity, pendingWarp) {
        ...
        const alignmentDirection = getCurrentAlignmentDirection(entity, desiredDirection);
        const turnMetrics = getTurnMetrics(alignmentDirection, desiredDirection);
        const degrees = (turnMetrics.radians * 180) / Math.PI;
        const alignmentDot = toFiniteNumber(turnMetrics && turnMetrics.alignment, Math.cos(toFiniteNumber(turnMetrics && turnMetrics.radians, Math.PI)));
    ...
    function evaluatePendingWarp(entity, pendingWarp, now = Date.now()) {
        ...
        const elapsedMs = Math.max(0, toInt(now, Date.now()) - toInt(pendingWarp.requestedAtMs, 0));
        const elapsedTicks = Math.floor(elapsedMs / Math.max(1, DESTINY_STAMP_INTERVAL_MS));
    ...
    function getPilotWarpActivationVelocity(entity, warpState) {
        ...
        const direction = normalizeVector(subtractVectors(warpState.targetPoint, entity.position), entity.direction);
        const startupGuidanceVelocity = cloneVector(warpState && warpState.startupGuidanceVelocity, entity && entity.velocity);
    ...
    function activatePendingWarp(entity, pendingWarp, options = {}) {
        if (String(pendingWarp && pendingWarp.nativeWarpCommand || "")
            .trim()
            .toUpperCase() === "GOTO") {
            const commandPoint = cloneVector(pendingWarp && pendingWarp.rawDestination, entity && entity.position);
            const shouldResumeSubwarp = toFiniteNumber(entity && entity.speedFraction, 0) <= 0 ||
        ...
        entity.mode = "WARP";
        entity.speedFraction = 1;
        entity.direction = normalizeVector(subtractVectors(warpState.targetPoint, entity.position), entity.direction);
        ...
        entity.pendingWarp = null;
        const activationProgress = getWarpProgress(warpState, warpState.startTimeMs);
        entity.position = addVectors(warpState.origin, scaleVector(entity.direction, activationProgress.traveled));
        entity.velocity = scaleVector(entity.direction, activationProgress.speed);
    ...
    function buildSessionlessWarpIngressState(entity, warpState, options = {}) {
        ...
        const startTimeMs = useNativeWarpProfile
            ? toFiniteNumber(warpState && warpState.startTimeMs, toFiniteNumber(options.nowMs, Date.now()))
        ...
        const durationMs = useNativeWarpProfile
            ...
            : Math.max(250, toFiniteNumber(options.durationMs, SESSIONLESS_WARP_INGRESS_DURATION_MS));
        const completionHoldMs = Math.max(0, toFiniteNumber(options.completionHoldMs, useNativeWarpProfile ? 0 : DESTINY_STAMP_INTERVAL_MS));
    ...
    function advanceSessionlessWarpIngress(entity, now) {
        ...
        const travelCompleteAtMs = Math.max(startTimeMs + 1, toFiniteNumber(ingressState.travelCompleteAtMs, toFiniteNumber(ingressState.completeAtMs, now)));
        const completeAtMs = Math.max(travelCompleteAtMs, toFiniteNumber(ingressState.completeAtMs, travelCompleteAtMs));
        ...
        entity.velocity = rawProgress >= 1
            ? { x: 0, y: 0, z: 0 }
            : scaleVector(direction, distance(previousPosition, entity.position) / deltaSeconds);
        ...
        if (rawProgress >= 1) {
            const suppressCompletionStopUpdates = ingressState.suppressCompletionStopUpdates === true;
    ...
    function getWarpProgress(warpState, now) {
        ...
        const resolvedWarpSpeedAU = Math.max(toFiniteNumber(warpState.warpSpeed, 0) / 1000, toFiniteNumber(warpState.cruiseWarpSpeedMs, 0) / ONE_AU_IN_METERS, 0.001);
        const accelRate = Math.max(toFiniteNumber(warpState.accelRate, 0) ||
            toFiniteNumber(warpState.accelExponent, 0) ||
            getWarpAccelRate(resolvedWarpSpeedAU), 0.001);
        const decelRate = Math.max(toFiniteNumber(warpState.decelRate, 0) ||
            toFiniteNumber(warpState.decelExponent, 0) ||
            getWarpDecelRate(resolvedWarpSpeedAU), 0.001);
        const maxWarpSpeedMs = Math.max(toFiniteNumber(warpState.maxWarpSpeedMs, 0), 0);
        const warpDropoutSpeedMs = Math.max(toFiniteNumber(warpState.warpDropoutSpeedMs, toFiniteNumber(warpState.warpFloorSpeedMs, WARP_DROPOUT_SPEED_MAX_MS)), 0);
        ...
        if (elapsedMs < accelMs) {
            ...
            const baseSpeed = Math.min(maxWarpSpeedMs, accelRate * Math.exp(accelRate * seconds));
            const speed = Math.max(baseSpeed, toFiniteNumber(warpState.activationSpeedMs, magnitude(warpState.startupGuidanceVelocity)));
            return {
                complete: false,
                traveled: Math.min(accelDistance, Math.max(baseSpeed / accelRate, 0)),
                speed,
            };
        }
        ...
        const speed = Math.max(0, maxWarpSpeedMs * Math.exp(-decelRate * seconds));
        const progress = {
            complete: false,
            traveled: accelDistance +
                cruiseDistance +
...
//# sourceMappingURL=warpState.js.map