...
const { DESTINY_SPACE_FRICTION, } = require("../constants");
...
function integrateCarbonMotion(position, velocity, acceleration, inertialMass, friction = DESTINY_SPACE_FRICTION, seconds = 0) {
    ...
    const mass = Math.max(CARBON_INTEGRATION_EPSILON, toFiniteNumber(inertialMass, 1));
    ...
    if (lambda <= 1e-12) {
        return {
            position: addVectors(startPosition, addVectors(scaleVector(startVelocity, stepSeconds), scaleVector(forceAcceleration, 0.5 * stepSeconds * stepSeconds))),
            velocity: addVectors(startVelocity, scaleVector(forceAcceleration, stepSeconds)),
        };
    }
    ...
    return {
        position: addVectors(startPosition, addVectors(scaleVector(equilibriumVelocity, stepSeconds), scaleVector(velocityOffset, (1 - timeFactor) / lambda))),
        velocity: addVectors(equilibriumVelocity, scaleVector(velocityOffset, timeFactor)),
    };
}
function deriveCarbonIntrinsicAcceleration(startVelocity, endVelocity, inertialMass, friction = DESTINY_SPACE_FRICTION, seconds = 0) {
    const stepSeconds = Math.max(CARBON_INTEGRATION_EPSILON, toFiniteNumber(seconds, 0));
    const mass = Math.max(CARBON_INTEGRATION_EPSILON, toFiniteNumber(inertialMass, 1));
    const drag = Math.max(0, toFiniteNumber(friction, DESTINY_SPACE_FRICTION));
    const lambda = drag / mass;
    if (lambda <= 1e-12) {
        return scaleVector(subtractVectors(cloneVector(endVelocity), cloneVector(startVelocity)), 1 / stepSeconds);
    }
    const timeFactor = Math.exp(-lambda * stepSeconds);
    const denominator = Math.max(CARBON_INTEGRATION_EPSILON, 1 - timeFactor);
    return scaleVector(subtractVectors(cloneVector(endVelocity), scaleVector(cloneVector(startVelocity), timeFactor)), lambda / denominator);
}
module.exports = {
...
//# sourceMappingURL=carbonIntegration.js.map