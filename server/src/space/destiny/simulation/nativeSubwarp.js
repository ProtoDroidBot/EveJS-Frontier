...
const { DESTINY_STAMP_INTERVAL_MS, } = require("../constants");
const { getCurrentDestinyStamp, } = require("../delivery/stamps");
...
const CARBON_OLD_STYLE_STOP_Y_DAMPING = (1 - 0.07) * 0.9345794392523364485981308411215;
function createNativeSubwarpController(deps = {}) {
    const { applyCarbonAcceleration, calculateCarbonFollowPlan, calculateCarbonGotoThrust, calculateCarbonOrbitPlan, cloneVector, } = deps;
    const resolveCurrentStamp = typeof deps.getCurrentDestinyStamp === "function"
        ? deps.getCurrentDestinyStamp
        : getCurrentDestinyStamp;
    const nativeTickIntervalMs = Math.max(1, Number(deps.DESTINY_STAMP_INTERVAL_MS) || DESTINY_STAMP_INTERVAL_MS);
    ...
    function getFrameIndex(simTimeMs) {
        ...
        return Math.floor(Math.max(0, Number.isFinite(numeric) ? numeric : 0) /
    ...
    function getCadenceFrameStartMs(simTimeMs, cadenceAnchorMs) {
        ...
        return anchor + (Math.floor((numericTime - anchor) / nativeTickIntervalMs) *
            nativeTickIntervalMs);
    }
    ...
    function motionStateValuesMatch(left, right, depth = 0) {
        ...
        if (Array.isArray(left)) {
            return left.length === right.length && left.every((entry, index) => motionStateValuesMatch(entry, right[index], depth + 1));
        }
    ...
    function cloneSnapshotEntity(entity) {
        ...
        if (entity &&
            ...
            snapshot.sessionlessWarpIngress = cloneMotionStateValue(entity.sessionlessWarpIngress);
        }
        return snapshot;
    }
    function buildAccelerationControl(mode, plan, extra = {}) {
        return Object.freeze({
            ...
            targetPoint: Object.freeze(cloneVector(plan && (plan.gotoPoint || plan.targetPoint))),
            debug: Object.freeze({
                mode,
                nativeControl: true,
                ...extra,
    ...
    function buildControl(entity, resolveTarget, nowMs) {
        ...
        if (mode === "GOTO") {
            const plan = calculateCarbonGotoThrust(entity, entity.targetPoint, nativeTickSeconds);
        ...
        if (mode === "FOLLOW") {
            const plan = calculateCarbonFollowPlan(entity, target, nativeTickSeconds);
            return buildAccelerationControl(mode, plan, {
        ...
        const plan = calculateCarbonOrbitPlan(entity, target, nativeTickSeconds, nowMs);
        return Object.freeze({
    ...
    function planningSeedsMatch(entity, snapshot) {
        ...
        return (String(entity.kind || "").toLowerCase() ===
            ...
            motionStateValuesMatch(entity.sessionlessWarpIngress, snapshot.sessionlessWarpIngress) &&
            ...
            motionStateValuesMatch(entity.pendingGeometryImpactPosition, snapshot.pendingGeometryImpactPosition));
    }
    function isSceneIntervalPlanCurrent(plan, scene, options = {}) {
        ...
        for (const entity of dynamicEntities) {
            ...
            if (shouldCompare(entity) &&
                !planningSeedsMatch(entity, plan.freshnessSnapshotsByID.get(entityKey))) {
                return false;
            }
        }
        return true;
    ...
    function prepareFrame(scene, nowMs, options = {}) {
        ...
        const cadenceAnchorMs = getCadenceAnchorMs(previousFrame && previousFrame.sceneEntitiesOwner === sceneEntitiesOwner
            ? previousFrame
            : null, options.cadenceAnchorMs);
        const frameIndex = getFrameIndex(behaviorAtMs);
        const frameStartMs = getCadenceFrameStartMs(behaviorAtMs, cadenceAnchorMs);
        ...
        for (const entity of dynamicEntities) {
            ...
            controlsByID.set(entityKey, buildControl(snapshotEntity, resolveTarget, behaviorAtMs));
        }
        return {
            stamp,
            cadenceAnchorMs,
    ...
    function rebindFrame(frame, scene, options = {}) {
        ...
        const originalEntitiesByID = options.originalEntitiesByID instanceof Map
            ? options.originalEntitiesByID
            : new Map();
        const allowedDestinationByID = options.allowedDestinationByID instanceof Map
            ? options.allowedDestinationByID
            : null;
        const preserveSnapshots = options.preserveSnapshots === true;
        const expectedSourceSceneEntitiesOwner = options.expectedSourceSceneEntitiesOwner;
        const sourceSceneOwnerMatches = expectedSourceSceneEntitiesOwner === undefined ||
        ...
        for (const entity of getDynamicEntities(scene)) {
            ...
            const mayCarryControl = Boolean(sourceSceneOwnerMatches &&
                originalEntity &&
            ...
            if (preserveSnapshots &&
                ...
                snapshotByID.set(entityKey, cloneSnapshotEntity(frame.snapshotByID.get(entityKey)));
            }
            if (mayCarryControl &&
    ...
    function advancePrepared(entity, frame, deltaSeconds) {
        ...
        if (!Number.isFinite(resolvedDeltaSeconds) ||
        ...
        if (control.kind === "MISSING_TARGET") {
            const commandStillCurrent = control.mode === logicalMode &&
            ...
            result = applyCarbonAcceleration(entity, { x: 0, y: 0, z: 0 }, resolvedDeltaSeconds, { mode: control.mode, missingTarget: true });
            result = {
                ...result,
            ...
            result = applyCarbonAcceleration(entity, { x: 0, y: 0, z: 0 }, resolvedDeltaSeconds, { mode: control.mode });
        }
        else {
            ...
            result = applyCarbonAcceleration(entity, control.acceleration, resolvedDeltaSeconds, control.debug);
        }
    ...
    function forgetEntity(frame, entityOrID) {
        const entityObject = entityOrID && typeof entityOrID === "object" ? entityOrID : null;
        const entityKey = getEntityMapKey(entityObject ? entityObject.itemID : entityOrID);
    ...
    function buildSceneIntervalSegments(startSimTimeMs, endSimTimeMs, options = {}) {
        ...
        const maximumSegments = Math.max(1, Math.min(4096, Math.trunc(Number(options.maximumSegments) || 4096)));
        const segments = [];
        ...
        while (segmentStartMs < end - 0.000001) {
            ...
            const segmentEndMs = Math.min(end, getNextFrameBoundaryMs(segmentStartMs, options.previousFrame));
    ...
    function recomputeRawProjectedEndpoints(options = {}) {
        const { advanceMovement, cloneEntity, endSimTimeMs, projectedDynamicEntities, projectedEntitiesByID, projectionSeedsByID, preservedRawMovementTargetSnapshotsBySourceID, rawMovementTargetSnapshotsBySourceID, scene, shouldAdvance, sourceEntitiesByID, startSnapshotsByID, totalDeltaSeconds, } = options;
        if (typeof advanceMovement !== "function" ||
        ...
        const rawEntries = [...projectionSeedsByID.entries()].filter(([entityKey, seedEntity]) => (sourceEntitiesByID.has(entityKey) &&
        ...
        const recomputePass = (entries, endpointsByID) => {
            ...
            for (const [entityKey, seedEntity] of entries) {
                ...
                const preservedTargetSnapshotsByID = preservedRawMovementTargetSnapshotsBySourceID instanceof Map
                ...
                const passScene = {
                    getEntityByID(entityID) {
                        ...
                        const preservedTarget = targetKey !== null &&
                            targetSource &&
                        ...
                        if (targetKey !== null && !targetSnapshotsByID.has(targetKey)) {
                            targetSnapshotsByID.set(targetKey, targetView
                ...
                if (sourceEntity && shouldAdvance(sourceEntity)) {
                    advanceMovement(projectedEntity, passScene, totalDeltaSeconds, endSimTimeMs);
                }
                rawMovementTargetSnapshotsBySourceID.set(entityKey, targetSnapshotsByID);
        ...
        recomputePass(rawEntries.filter(([, entity]) => !isMissileEntity(entity)), new Map(projectedEntitiesByID));
        recomputePass(rawEntries.filter(([, entity]) => isMissileEntity(entity)), new Map(projectedEntitiesByID));
    }
    function buildEndpointProjectionContext(scene, retainedProjectedEntitiesByID, cloneEntity) {
        ...
        for (const liveEntity of getDynamicEntities(scene)) {
            ...
            if (liveEntitiesByID.has(entityKey) &&
        ...
        for (const [entityKey, liveEntity] of liveEntitiesByID) {
            const retainedProjection = retainedProjectedEntitiesByID instanceof Map
                ...
                : null;
            projectedEntitiesByID.set(entityKey, retainedProjection || cloneEntity(liveEntity) ||
    ...
    function planSceneInterval(scene, startSimTimeMs, endSimTimeMs, options = {}) {
        const requestedStartSimTimeMs = Math.max(0, Number(startSimTimeMs) || 0);
        const requestedEndSimTimeMs = Math.max(requestedStartSimTimeMs, Number(endSimTimeMs) || requestedStartSimTimeMs);
        ...
        // unbounded backlog.
        const resynchronized = Math.floor((requestedEndSimTimeMs - requestedStartSimTimeMs) /
            nativeTickIntervalMs) > 5;
        const effectiveStartSimTimeMs = resynchronized
            ? Math.max(0, requestedEndSimTimeMs - nativeTickIntervalMs)
            : requestedStartSimTimeMs;
        const sceneEntitiesOwner = getSceneEntitiesOwner(scene);
        const carriedFrameOwnedByScene = Boolean(options.previousFrame &&
        ...
        const interval = resynchronized
            ? {
                ...
                maximumSegments: 1,
                segments: [{
                        deltaSeconds: (requestedEndSimTimeMs - effectiveStartSimTimeMs) / 1000,
                        endSimTimeMs: requestedEndSimTimeMs,
            ...
            : buildSceneIntervalSegments(effectiveStartSimTimeMs, requestedEndSimTimeMs, {
                ...options,
        ...
        interval.skippedSimTimeMs = resynchronized
            ? Math.max(0, effectiveStartSimTimeMs - requestedStartSimTimeMs)
            : 0;
        ...
        const recoveryFrame = () => prepareFrame(scene, interval.targetSimTimeMs, {
        ...
        const shouldAdvance = typeof options.shouldAdvance === "function"
            ...
            : () => true;
        const shouldCarryEndpoint = typeof options.shouldCarryEndpoint === "function"
        ...
        for (const sourceEntity of dynamicEntities) {
            ...
            if (sourceEntitiesByID.get(entityKey) !== sourceEntity ||
                projectedEntitiesByID.has(entityKey)) {
                identityConflictCount += 1;
                continue;
            }
            ...
            projectionSeedsByID.set(entityKey, cloneEntity(projectedEntity) || cloneSnapshotEntity(projectedEntity));
            startSnapshotsByID.set(entityKey, cloneSnapshotEntity(sourceEntity));
            freshnessSnapshotsByID.set(entityKey, cloneSnapshotEntity(sourceEntity));
        }
        ...
        let projectedFrame = carriedFrameOwnedByScene
            ? rebindFrame(options.previousFrame, projectedScene, {
            ...
            : options.previousFrame
                ? prepareHeldFrame(projectedScene, effectiveStartSimTimeMs, {
        ...
        for (const segment of interval.segments) {
            projectedFrame = prepareFrame(projectedScene, segment.endSimTimeMs, {
            ...
            const liveFrame = rebindFrame(projectedFrame, scene, {
            ...
            for (const [entityKey, projectedEntity] of projectedEntitiesByID) {
                ...
                advancePrepared(projectedEntity, projectedFrame, segment.deltaSeconds);
            }
            recomputeRawProjectedEndpoints({
                ...
                sourceEntitiesByID,
                startSnapshotsByID,
                totalDeltaSeconds: (segment.endSimTimeMs - interval.effectiveStartSimTimeMs) / 1000,
            });
        }
        const lastProjectedFrame = projectedFrame;
        projectedFrame = prepareFrame(projectedScene, interval.targetSimTimeMs, {
            behaviorAtMs: interval.targetSimTimeMs,
            previousFrame: projectedFrame,
        });
        let endpointFrame;
        if (plannedSegments.length > 0 && projectedFrame === lastProjectedFrame) {
            ...
            endpointFrame = rebindFrame(projectedFrame, scene, {
                allowedDestinationByID: sourceEntitiesByID,
                expectedSourceSceneEntitiesOwner: projectedDynamicEntities,
                originalEntitiesByID: projectedEntitiesByID,
                preserveSnapshots: true,
            ...
            const endpointContext = buildEndpointProjectionContext(scene, projectedEntitiesByID, cloneEntity);
            ...
            projectedFrame = prepareFrame(endpointContext.projectedScene, interval.targetSimTimeMs, {
                behaviorAtMs: interval.targetSimTimeMs,
            ...
            endpointFrame = rebindFrame(projectedFrame, scene, {
                ...
                expectedSourceSceneEntitiesOwner: endpointContext.projectedEntitiesByID,
    ...
    function refreshSceneIntervalProjections(plan, scene, options = {}) {
        ...
        const advanceMovement = typeof options.advanceMovement === "function"
            ...
            : null;
        if (!advanceMovement) {
            throw new TypeError("refreshSceneIntervalProjections requires advanceMovement");
        }
        const shouldAdvance = typeof options.shouldAdvance === "function"
            ? options.shouldAdvance
            : () => true;
        ...
        const projectedDynamicEntities = new Map();
        const projectionSeedsByID = new Map();
        const preservedRawMovementTargetSnapshotsBySourceID = plan.rawMovementTargetSnapshotsBySourceID;
        const rawMovementTargetSnapshotsBySourceID = new Map();
        const liveEntitiesByID = new Map();
        ...
        for (const segment of plan.segments) {
            projectedFrame = rebindFrame(segment && segment.frame, projectedScene, {
                allowedDestinationByID: projectedEntitiesByID,
            ...
            recomputeRawProjectedEndpoints({
                ...
                totalDeltaSeconds: (segment.endSimTimeMs - plan.effectiveStartSimTimeMs) / 1000,
            });
        }
        const lastProjectedFrame = projectedFrame;
        projectedFrame = prepareFrame(projectedScene, plan.targetSimTimeMs, {
        ...
        if (projectedFrame !== lastProjectedFrame) {
            ...
            if (endpointContext.identityConflictCount > 0) {
                ...
                const endpointIsSegmentFrame = plan.segments.some((segment) => segment && segment.frame === staleEndpointFrame);
            ...
            const endpointCadenceAnchorMs = projectedFrame.cadenceAnchorMs;
            projectedFrame = prepareFrame(endpointContext.projectedScene, plan.targetSimTimeMs, {
                behaviorAtMs: plan.targetSimTimeMs,
                cadenceAnchorMs: endpointCadenceAnchorMs,
                previousFrame: lastProjectedFrame,
            });
            const refreshedEndpointFrame = rebindFrame(projectedFrame, scene, {
                allowedDestinationByID: endpointContext.liveEntitiesByID,
                expectedSourceSceneEntitiesOwner: endpointContext.projectedEntitiesByID,
                originalEntitiesByID: endpointContext.projectedEntitiesByID,
                preserveSnapshots: true,
        ...
        plan.rawMovementTargetSnapshotsBySourceID =
            rawMovementTargetSnapshotsBySourceID;
        plan.freshnessSnapshotsByID = new Map([...liveEntitiesByID].map(([entityKey, sourceEntity]) => [
            entityKey,
            cloneSnapshotEntity(sourceEntity),
        ]));
    ...
    function forgetSceneIntervalEntity(plan, entityOrID) {
        ...
        let removed = false;
        const lastSegmentFrame = Array.isArray(plan.segments) && plan.segments.length > 0
            ? plan.segments[plan.segments.length - 1].frame
            : null;
        const detachedEndpointFrame = plan.endpointFrame && plan.endpointFrame !== lastSegmentFrame
            ? plan.endpointFrame
            : null;
        ...
        if (plan.rawMovementTargetSnapshotsBySourceID instanceof Map) {
            removed = plan.rawMovementTargetSnapshotsBySourceID.delete(entityKey) ||
                removed;
            for (const targetSnapshotsByID of plan.rawMovementTargetSnapshotsBySourceID.values()) {
    ...
    function consumeSceneIntervalEntity(plan, entity) {
        ...
        let aggregate = null;
        for (const segment of plan.segments) {
            const result = advancePrepared(entity, segment.frame, segment.deltaSeconds);
            if (result === null) {
                return null;
            }
            aggregate = {
                ...
                nativeSubwarpMissingTarget: Boolean((aggregate && aggregate.nativeSubwarpMissingTarget) ||
                    result.nativeSubwarpMissingTarget),
                nativeSubwarpMissingTargetTransitionApplied: Boolean((aggregate && aggregate.nativeSubwarpMissingTargetTransitionApplied) ||
                    result.nativeSubwarpMissingTargetTransitionApplied),
                nativeSubwarpModeTransitionPending: Boolean((aggregate && aggregate.nativeSubwarpModeTransitionPending) ||
        ...
        if (aggregate && entity.lastMotionDebug) {
            entity.lastMotionDebug = {
                ...entity.lastMotionDebug,
                deltaSeconds: plan.segments.reduce((total, segment) => total + segment.deltaSeconds, 0),
                positionDelta: {
    ...
    function advanceSceneInterval(scene, startSimTimeMs, endSimTimeMs, options = {}) {
        const start = Math.max(0, Number(startSimTimeMs) || 0);
        const end = Math.max(start, Number(endSimTimeMs) || start);
        const resynchronized = Math.floor((end - start) / nativeTickIntervalMs) > 5;
        const effectiveStartSimTimeMs = resynchronized
        ...
        let frame = carriedFrameOwnedByScene
            ? options.previousFrame
            : options.previousFrame
                ? prepareHeldFrame(scene, effectiveStartSimTimeMs, {
                    behaviorAtMs: effectiveStartSimTimeMs,
                    cadenceAnchorMs: resynchronized
                        ? effectiveStartSimTimeMs
                        : 0,
        ...
        const maximumSegments = resynchronized
            ? 1
            : Math.max(1, Math.min(4096, Math.trunc(Number(options.maximumSegments) || 4096)));
        while (projectedAtMs < end - 0.000001 &&
            segmentCount < maximumSegments) {
            const segmentEndMs = Math.min(end, resynchronized
                ? end
            ...
            for (const entity of getDynamicEntities(scene)) {
                ...
                resultsByEntity.set(entity, {
                    ...(previousResult || {}),
                    ...result,
                    changed: Boolean((previousResult && previousResult.changed) || result.changed),
                    nativeSubwarpModeTransitionPending: Boolean((previousResult &&
                        previousResult.nativeSubwarpModeTransitionPending) ||
                        result.nativeSubwarpModeTransitionPending),
                    nativeSubwarpMissingTarget: Boolean((previousResult && previousResult.nativeSubwarpMissingTarget) ||
                        result.nativeSubwarpMissingTarget),
                    nativeSubwarpMissingTargetTransitionApplied: Boolean((previousResult &&
...
//# sourceMappingURL=nativeSubwarp.js.map