...
function reserveVisibilityAcquisitions(session, entities, options: Record<string, any> = {}) {
  ...
  const isCommitted = typeof options.isCommitted === "function"
    ? options.isCommitted
    : () => false;
  const reservedEntities: any[] = [];
  const reservedEntries: any[] = [];
  ...
  return {
    ...
    entries: reservedEntries.map(({ entityID, token }: Record<string, any>) => ({
      entityID,
      kind: token.kind,
    })),
...
function reserveVisibilityRemovals(session, entityIDs, options: Record<string, any> = {}) {
  const generation = session && session._space;
...
function withDeliveryHooks(sendOptions, options: Record<string, any> = {}) {
...
function buildVisibilityDeltaPresentation(options: Record<string, any> = {}) {
...
function deliverVisibilityDeltaPresentation(options: Record<string, any> = {}) {
