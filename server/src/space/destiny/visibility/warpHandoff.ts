...
function beginPilotWarpVisibilityHandoff(options: Record<string, any> = {}) {
...
function buildPilotWarpVisibilityHandoffReadiness(options: Record<string, any> = {}) {
  const entity = options.entity && typeof options.entity === "object"
    ? options.entity
    : null;
...
function buildWarpSourceRemovalPlan(options: Record<string, any> = {}) {
  ...
  const removedDynamicIDs: any[] = [];
  const nextVisibleDynamicIDs = new Set<any>(currentDynamicIDs);
  const freshDynamicIDsToDelete = new Set<any>();
  ...
  const removedStaticIDs: any[] = [];
  const nextVisibleStaticIDs = new Set<any>(currentStaticIDs);
...
function buildWarpLiveGridDynamicRefreshPlan(options: Record<string, any> = {}) {
  ...
  if (!liveDynamicDelta) {
    return {
      ...
      freshDynamicIDs: new Set<any>(),
      nextVisibleDynamicIDs: new Set<any>(),
      removedDynamicIDs: [],
    };
  }
  ...
  const nextVisibleDynamicIDs = new Set<any>();
...
function buildWarpLiveGridStaticRefreshPlan(options: Record<string, any> = {}) {
  const currentStaticIDs = normalizeSet(options.currentStaticIDs);
  ...
  const nextVisibleStaticIDs = new Set<any>(desiredStaticIDs);
  for (const entityID of currentStaticIDs) {
    if (shouldPreserveStaticID(entityID)) {
...
function buildWarpDestinationStaticAcquisitionPlan(options: Record<string, any> = {}) {
  const currentIDs = normalizeSet(options.currentStaticIDs);
  if (options.enabled !== true) {
    return {
      addedStaticEntities: [],
      desiredStaticIDs: new Set<any>(),
  ...
  const nextVisibleStaticIDs = new Set<any>(
...
function isWarpDestinationStaticPreserved(options: Record<string, any> = {}) {
...
function buildWarpDestinationAcquireIntent(options: Record<string, any> = {}) {
  const handoff = options.handoff && typeof options.handoff === "object"
    ? options.handoff
    : {};
  const destinationClusterKey = String(
...
function buildWarpDestinationAcquirePlan(options: Record<string, any> = {}) {
...
function buildWarpDestinationDynamicRefreshPlan(options: Record<string, any> = {}) {
  ...
  const nextVisibleDynamicIDs = new Set<any>(currentIDs);
  const freshDynamicIDs = new Set<any>();
