...
const { getPendingVisibilityAcquisitionIDs, getPendingVisibilityRemovalIDs, isVisibilityAcquisitionCommitted, reserveVisibilityAcquisitions, reserveVisibilityRemovals, } = require("./acquisition");
const { deleteEntityIDFromMap, entityIDsEqual, getEntityMapKey, } = require("../identity/entityID");
...
const REMOVAL_RESERVATION_BRAND = Symbol("destiny.sceneVisibilityRemovalReservation");
const REMOVAL_PRESENTATION_BRAND = Symbol("destiny.sceneVisibilityRemovalPresentation");
const DEFAULT_BOOTSTRAP_STALE_CODE = "DESTINY_BOOTSTRAP_VISIBILITY_GENERATION_REPLACED";
...
function isIncrementalStaticEntity(entity, options = {}) {
    return (typeof options.isIncrementalStaticVisibilityEntity === "function" &&
        options.isIncrementalStaticVisibilityEntity(entity) === true);
}
function getPlannedVisibleStaticEntityIDs(scene, session, committedIDs, options = {}) {
    const visibleIDs = normalizeVisibilityEntityIDSet(committedIDs);
...
function isSceneVisibilityAcquisitionCommitted(session, rawEntityID, kind = null) {
    ...
    const egoCommitted = entityIDsEqual(generation.presentedEgoEntityID, entityID);
    const dynamicCommitted = visibilitySetHasEntityID(generation.visibleDynamicEntityIDs, entityID);
    const staticCommitted = visibilitySetHasEntityID(generation.visibleBubbleScopedStaticEntityIDs, entityID);
...
function resolveVisibilityAcquisitionKind(scene, session, entity, rawEntityID, options = {}) {
    const entityID = getEntityMapKey(rawEntityID);
    if (entityID !== null &&
        session &&
        session._space &&
...
function applyStaleGenerationBehavior(intent, behavior, errorCode = "DESTINY_VISIBILITY_GENERATION_REPLACED") {
    if (behavior === "commit") {
        return Boolean(intent && typeof intent.commit === "function" && intent.commit());
    }
    if (behavior === "rollback") {
        return Boolean(intent && typeof intent.rollback === "function" && intent.rollback());
    }
    if (behavior === "throw") {
...
function reserveSceneVisibilityAcquisitions(scene, session, entities, options = {}) {
    const pendingRemovalIDs = getPendingVisibilityRemovalIDs(session);
    return reserveVisibilityAcquisitions(session, entities, {
        resolveKind: (entity, entityID) => resolveVisibilityAcquisitionKind(scene, session, entity, entityID, options),
        isCommitted: (_entity, entityID, kind) => (!visibilitySetHasEntityID(pendingRemovalIDs, entityID) &&
...
function reserveSceneVisibilityRemovals(session, entityIDs, options = {}) {
    ...
    const nativeReservation = reserveVisibilityRemovals(session, entityIDs, {
        resolveKind: (entityID) => (generation && entityIDsEqual(generation.presentedEgoEntityID, entityID)
            ? "ego"
            : requestedKind === "static" || requestedKind === "dynamic"
                ...
                : visibilitySetHasEntityID(staticIDs, entityID)
                    ? "static"
                    : "dynamic"),
        isCommitted: (entityID) => (options.allowUncommitted === true ||
    ...
    const reservedEntityIDs = normalizeVisibilityEntityIDSet(nativeReservation.entityIDs);
    const capability = (candidateSession, candidateReservation, rawEntityID) => (active === true &&
...
function isOwnedSceneVisibilityRemovalReservation(session, reservation, entityIDs) {
...
function authorizeSceneVisibilityRemovalPresentation(sendOptions, session, reservation, entityIDs, kind) {
    const normalizedEntityIDs = [...normalizeVisibilityEntityIDSet(entityIDs)];
    if (!sendOptions ||
        typeof sendOptions !== "object" ||
        kind !== "cloak" ||
        !isOwnedSceneVisibilityRemovalReservation(session, reservation, normalizedEntityIDs)) {
        return false;
    }
    const generation = session._space;
    const authorizedEntityIDs = normalizeVisibilityEntityIDSet(normalizedEntityIDs);
    const authorization = (_candidateSendOptions, candidateSession, payloadName, rawEntityIDs) => (candidateSession === session &&
        candidateSession &&
        candidateSession._space === generation &&
        payloadName === "CloakBall" &&
        Array.isArray(rawEntityIDs) &&
        rawEntityIDs.length > 0 &&
        rawEntityIDs.every((entityID) => visibilitySetHasEntityID(authorizedEntityIDs, entityID)) &&
        isOwnedSceneVisibilityRemovalReservation(candidateSession, reservation, rawEntityIDs));
...
function isSceneVisibilityRemovalPresentationAuthorized(sendOptions, session, payloadName, entityIDs) {
    const authorization = sendOptions && typeof sendOptions === "object"
        ? sendOptions[REMOVAL_PRESENTATION_BRAND]
        : null;
    const normalizedEntityIDs = [...normalizeVisibilityEntityIDSet(entityIDs)];
    return Boolean(typeof authorization === "function" &&
        normalizedEntityIDs.length > 0 &&
        authorization(sendOptions, session, payloadName, normalizedEntityIDs) === true);
}
function commitVisibilityAcquisitionMembership(scene, session, reservation, options = {}) {
    const generation = session && session._space;
    if (!generation || !reservation || reservation.generation !== generation) {
        applyStaleGenerationBehavior(reservation, options.staleGenerationBehavior || "ignore", options.staleGenerationCode);
        return false;
    }
    const dynamicIDs = normalizeVisibilityEntityIDSet(generation.visibleDynamicEntityIDs);
    const freshIDs = normalizeVisibilityEntityIDSet(generation.freshlyVisibleDynamicEntityIDs);
    const staticIDs = normalizeVisibilityEntityIDSet(generation.visibleBubbleScopedStaticEntityIDs);
...
function clearCommittedVisibilityMembership(generation, rawEntityID, options = {}) {
    ...
    if (options.dynamic !== false) {
        changed = deleteEntityIDFromSet(generation.visibleDynamicEntityIDs, entityID) || changed;
    }
    if (options.fresh !== false) {
        changed = deleteEntityIDFromSet(generation.freshlyVisibleDynamicEntityIDs, entityID) || changed;
    }
    if (options.release !== false) {
        changed = deleteEntityIDFromMap(generation.freshlyVisibleDynamicEntityReleaseStampByID, entityID) || changed;
    }
    if (options.static === true) {
        changed = deleteEntityIDFromSet(generation.visibleBubbleScopedStaticEntityIDs, entityID) || changed;
    }
    if (options.ego === true &&
...
function clearSameSceneDynamicClassification(generation, entityID) {
    const wasDynamic = visibilitySetHasEntityID(generation && generation.visibleDynamicEntityIDs, entityID);
...
function commitSceneVisibilityRemovals(session, reservation, options = {}) {
    const generation = session && session._space;
    if (!reservation || !reservation.generation || generation !== reservation.generation) {
        applyStaleGenerationBehavior(reservation, options.staleGenerationBehavior || "commit", options.staleGenerationCode);
        return false;
    }
    ...
    for (const entry of Array.isArray(reservation.entries)
        ? reservation.entries
        : []) {
        clearCommittedVisibilityMembership(reservation.generation, entry && entry.entityID, options);
    }
    return true;
}
function reserveSceneVisibilityDelta(scene, session, addedEntities, removedEntityIDs, options = {}) {
    const removalReservation = reserveSceneVisibilityRemovals(session, removedEntityIDs, options.removal);
    let acquisitionReservation;
    try {
        acquisitionReservation = reserveSceneVisibilityAcquisitions(scene, session, addedEntities, options.acquisition);
    }
    catch (error) {
...
function collectEntityIDsFromEntities(entities) {
    return normalizeVisibilityEntityIDSet((Array.isArray(entities) ? entities : []).map((entity) => entity && entity.itemID));
}
...
function getReleaseIDsToDelete(generation, desiredIDs, explicitEntityIDs, options = {}) {
    const releaseStampByID = generation.freshlyVisibleDynamicEntityReleaseStampByID;
    const deleteIDs = normalizeVisibilityEntityIDSet(explicitEntityIDs);
    if (!(releaseStampByID instanceof Map)) {
        return deleteIDs;
    }
    const isProtectionBoundaryActive = typeof options.isProtectionBoundaryActive === "function"
...
function commitDynamicVisibilityDelta(session, visibilityIntent, delta = {}, options = {}) {
    const generation = session && session._space;
    if (!generation || !visibilityIntent || visibilityIntent.generation !== generation) {
        applyStaleGenerationBehavior(visibilityIntent, options.staleGenerationBehavior || "ignore", options.staleGenerationCode);
        return false;
    }
    const desiredIDs = normalizeVisibilityEntityIDSet(delta.desiredIDs);
    const freshIDs = collectDeltaFreshEntityIDs(delta);
    const releaseIDsToDelete = getReleaseIDsToDelete(generation, desiredIDs, options.freshReleaseEntityIDsToDelete, options);
    ...
    for (const entityID of releaseIDsToDelete) {
        deleteEntityIDFromMap(generation.freshlyVisibleDynamicEntityReleaseStampByID, entityID);
    }
    return true;
}
function commitStaticVisibilityDelta(session, visibilityIntent, delta = {}, options = {}) {
    const generation = session && session._space;
    if (!generation || !visibilityIntent || visibilityIntent.generation !== generation) {
        applyStaleGenerationBehavior(visibilityIntent, options.staleGenerationBehavior || "ignore", options.staleGenerationCode);
        return false;
...
function commitCombinedVisibilityDelta(session, visibilityIntent, dynamicDelta, staticDelta, options = {}) {
    const generation = session && session._space;
    if (!generation || !visibilityIntent || visibilityIntent.generation !== generation) {
        applyStaleGenerationBehavior(visibilityIntent, options.staleGenerationBehavior || "commit", options.staleGenerationCode);
        return false;
    }
    ...
    if (dynamicDelta && typeof dynamicDelta === "object") {
        ...
        const hasPlannedFreshIDs = options.plannedFreshDynamicEntityIDs instanceof Set ||
            Array.isArray(options.plannedFreshDynamicEntityIDs);
        if (hasPlannedFreshIDs) {
            freshDynamicIDs = normalizeVisibilityEntityIDSet(options.plannedFreshDynamicEntityIDs);
        }
        else {
            ...
            const unchangedIDs = normalizeVisibilityEntityIDSet(options.unchangedIntersectionEntityIDs);
            freshDynamicIDs = new Set([...candidateFreshIDs].filter((entityID) => (visibilitySetHasEntityID(reservedAddedIDs, entityID) &&
                !visibilitySetHasEntityID(unchangedIDs, entityID))));
        }
        releaseIDsToDelete = getReleaseIDsToDelete(generation, desiredDynamicIDs, options.freshReleaseEntityIDsToDelete, options);
    }
...
function commitInitialBallparkVisibility(session, generation, visibilitySnapshot, egoEntityID, options = {}) {
    if (!session || session._space !== generation) {
        applyStaleGenerationBehavior(null, options.staleGenerationBehavior || "throw", options.staleGenerationCode || DEFAULT_BOOTSTRAP_STALE_CODE);
        return false;
    }
    const dynamicIDs = normalizeVisibilityEntityIDSet(visibilitySnapshot && visibilitySnapshot.dynamicEntityIDs);
    const staticIDs = normalizeVisibilityEntityIDSet(visibilitySnapshot && visibilitySnapshot.staticEntityIDs);
...
function installFreshVisibilityReleaseEntries(session, generation, entries, options = {}) {
    if (!session || session._space !== generation) {
        applyStaleGenerationBehavior(null, options.staleGenerationBehavior || "ignore", options.staleGenerationCode);
        return false;
    }
    ...
    const releaseStampByID = generation.freshlyVisibleDynamicEntityReleaseStampByID instanceof Map
...
function createVisibilityGenerationState(options = {}) {
    ...
    if (options.freshlyVisibleDynamicEntityReleaseStampByID instanceof Map) {
        for (const [rawEntityID, releaseStamp] of options.freshlyVisibleDynamicEntityReleaseStampByID.entries()) {
            const entityID = getEntityMapKey(rawEntityID);
            if (entityID !== null) {
                releaseStampByID.set(entityID, releaseStamp);
            }
    ...
    const maxJournalEntries = Math.min(DEFAULT_JOURNAL_MAX_ENTRIES, Math.max(1, toInteger(options.maxJournalEntries, DEFAULT_JOURNAL_MAX_ENTRIES)));
    ...
    return {
        ...
        visibleBubbleScopedStaticEntityIDs: normalizeVisibilityEntityIDSet(options.visibleBubbleScopedStaticEntityIDs),
        visibleDynamicEntityIDs: normalizeVisibilityEntityIDSet(options.visibleDynamicEntityIDs),
    };
}
...
function normalizeVisibilityJournalDetails(details, maxIDs) {
    ...
    if (Array.isArray(normalized.entities)) {
        normalized.entities = summarizeVisibilityIDs(normalized.entities.map((entity) => entity && entity.itemID), maxIDs);
    }
    return normalized;
}
function recordVisibilityJournal(session, event, details = {}, options = {}) {
    ...
    const maxEntries = Math.max(1, toInteger(options.maxEntries, DEFAULT_JOURNAL_MAX_ENTRIES));
    ...
    if (generation.visibilityJournal.length > maxEntries) {
        generation.visibilityJournal.splice(0, generation.visibilityJournal.length - maxEntries);
    }
...
function createVisibilityJournalRecorder(options = {}) {
    ...
    return (session, event, details = {}) => recordVisibilityJournal(session, event, details, boundOptions);
}
module.exports = {
...
//# sourceMappingURL=membershipState.js.map