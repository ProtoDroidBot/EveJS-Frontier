...
const { getEntityMapKey, normalizeEntityID, } = require("../identity/entityID");
...
function buildPostWarpLandingVisibilityPresentation(options = {}) {
    ...
    const completionUpdates = Array.isArray(reconcile.completionUpdates)
        ...
        : (typeof options.buildPilotWarpCompletionUpdates === "function"
            ? options.buildPilotWarpCompletionUpdates(reconcile.entity, sessionStamp)
            : []);
    const visibilityClusterKeyOverride = String(reconcile.visibilityClusterKeyOverride || "").trim() || null;
    const visibilityPositionOverride = reconcile.visibilityPositionOverride &&
    ...
    const dynamicDelta = typeof options.buildDynamicVisibilityDeltaForSession === "function"
        ...
        : null;
    if (dynamicDelta) {
        const formationRefreshEntities = typeof options.getFleetWarpFormationCompanionEntitiesForSession === "function"
            ? options.getFleetWarpFormationCompanionEntitiesForSession(session, nowMs)
            : [];
        const formationRefreshEntityIDs = new Set(normalizeArray(formationRefreshEntities)
            .map((entity) => getEntityMapKey(entity && entity.itemID))
            .filter((entityID) => entityID !== null));
        const removedIDs = normalizeArray(dynamicDelta.removedIDs).filter((entityID) => !formationRefreshEntityIDs.has(getEntityMapKey(entityID)));
        ...
        nextFreshlyVisibleDynamicEntityIDs = new Set(dynamicAddedEntities
            .map((visibleEntity) => getEntityMapKey(visibleEntity.itemID))
            .filter((entityID) => entityID !== null));
    }
    const staticDelta = typeof options.buildStaticVisibilityDeltaForSession === "function"
        ? options.buildStaticVisibilityDeltaForSession(session, nowMs, {
            dedicatedSiteStaticInstanceIDOverride: normalizeEntityID(reconcile.siteMaterializationInstanceID),
            visibilityClusterKeyOverride,
            visibilityPositionOverride,
        })
        : null;
    ...
    const unchangedIntersectionIDs = new Set([...normalizedRemovedEntityIDs].filter((entityID) => addedEntityIDs.has(entityID)));
    const effectiveAddedEntities = uniqueAddedEntities.filter((entity) => !unchangedIntersectionIDs.has(getEntityMapKey(entity && entity.itemID)));
    ...
    const uniqueRemovedEntityIDs = [...normalizedRemovedEntityIDs].filter((entityID) => !addedEntityIDs.has(entityID));
    const removeUpdates = uniqueRemovedEntityIDs.length > 0 &&
        ...
        : [];
    const addPresentation = effectiveAddedEntities.length > 0 &&
        typeof options.buildSessionStampedAddBallsUpdatesForSession === "function"
        ? options.buildSessionStampedAddBallsUpdatesForSession(session, effectiveAddedEntities, sessionStamp, {
            nowMs,
    ...
    const onDeliveryCommit = () => {
        ...
        if (nextVisibleDynamicEntityIDs instanceof Set) {
            session._space.visibleDynamicEntityIDs = new Set(nextVisibleDynamicEntityIDs);
        }
        if (nextFreshlyVisibleDynamicEntityIDs instanceof Set) {
            session._space.freshlyVisibleDynamicEntityIDs = new Set(nextFreshlyVisibleDynamicEntityIDs);
        }
        if (nextVisibleStaticEntityIDs instanceof Set) {
            session._space.visibleBubbleScopedStaticEntityIDs = new Set(nextVisibleStaticEntityIDs);
        }
...
function queueFleetWarpCompanionLandingPresentation(options = {}) {
    ...
    const getActiveFleetWarpFormationID = typeof options.getActiveFleetWarpFormationID === "function"
    ...
    const isReadyForDestiny = typeof options.isReadyForDestiny === "function"
        ...
        : () => false;
    const isEntityInSessionFleetWarpFormation = typeof options.isEntityInSessionFleetWarpFormation === "function"
        ? options.isEntityInSessionFleetWarpFormation
        : () => false;
    const getShipEntityForSession = typeof options.getShipEntityForSession === "function"
        ? options.getShipEntityForSession
        : () => null;
    const getHistorySafeSessionDestinyStamp = typeof options.getHistorySafeSessionDestinyStamp === "function"
        ? options.getHistorySafeSessionDestinyStamp
        : null;
    const buildSessionStampedAddBallsUpdatesForSession = typeof options.buildSessionStampedAddBallsUpdatesForSession === "function"
    ...
    for (const session of sessions) {
        ...
        const sessionStamp = getHistorySafeSessionDestinyStamp(session, nowMs, leadTicks, leadTicks);
        const addPresentation = buildSessionStampedAddBallsUpdatesForSession(session, [entity], sessionStamp, {
            nowMs,
            tagFreshAcquireLifecycle: true,
        });
        ...
        const commitCompanionVisibility = () => {
            ...
            const committedVisibleIDs = normalizeSet(session._space.visibleDynamicEntityIDs);
            const committedFreshIDs = normalizeSet(session._space.freshlyVisibleDynamicEntityIDs);
...
//# sourceMappingURL=warpLanding.js.map