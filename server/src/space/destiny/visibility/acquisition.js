...
function isVisibilityAcquisitionCommitted(session, rawEntityID, kind = null) {
    ...
    const dynamicCommitted = spaceState.visibleDynamicEntityIDs instanceof Set &&
        spaceState.visibleDynamicEntityIDs.has(entityID);
    const staticCommitted = spaceState.visibleBubbleScopedStaticEntityIDs instanceof Set &&
...
function visibilityDeltaRequiresDelivery(presentation) {
    ...
    return (normalizeArray(presentation.removedIDs).length > 0 ||
...
function buildVisibilityDeltaPresentation(options = {}) {
    ...
    const removeUpdates = removedIDs.length > 0 && typeof options.buildRemoveBallsUpdates === "function"
        ...
        : [];
    const addPresentation = addedEntities.length > 0 && typeof options.buildAddBallsUpdatesForSession === "function"
        ? options.buildAddBallsUpdatesForSession(options.session, addedEntities, {
            freshAcquire: true,
            nowMs: options.nowMs,
            stampOverride,
        })
...
function deliverVisibilityDeltaPresentation(options = {}) {
    ...
    const addUpdates = normalizeArray(presentation.addPresentation && presentation.addPresentation.updates);
    ...
    if (options.hasActiveTickDestinyPresentationBatch === true) {
        if (removeUpdates.length > 0 &&
            ...
            options.queueTickDestinyPresentationUpdates(options.session, removeUpdates, {
        ...
        if (addUpdates.length > 0 &&
            typeof options.queueTickDestinyPresentationUpdates === "function") {
            options.queueTickDestinyPresentationUpdates(options.session, addUpdates, {
        ...
        if (additionalUpdates.length > 0 &&
            typeof options.queueTickDestinyPresentationUpdates === "function") {
            options.queueTickDestinyPresentationUpdates(options.session, additionalUpdates, {
    ...
    if (combinedUpdates.length > 0 &&
        ...
        const emittedStamp = options.sendDestinyUpdates(options.session, combinedUpdates, options.waitForBubble === true, addSendOptions);
        return {
    ...
    if (removeUpdates.length > 0 &&
        typeof options.sendDestinyUpdates === "function") {
        const emittedStamp = options.sendDestinyUpdates(options.session, removeUpdates, options.waitForBubble === true, removeSendOptions);
        return {
            delivered: emittedStamp !== null && emittedStamp !== undefined,
            mode: emittedStamp !== null && emittedStamp !== undefined
                ? "remove-only"
