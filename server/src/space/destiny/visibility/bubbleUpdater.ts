...
function runFinalSceneTickVisibilityCoordinator(options: Record<string, any> = {}) {
  ...
  const topologyEntitySet = new Set<any>();
  ...
  const bubbleEntitySet = new Set<any>();
  ...
  const landingSessions = new Set<any>();
  ...
  const processedSessions = new Set<any>(landingSessions);
