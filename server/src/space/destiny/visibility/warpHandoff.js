...
const { entityIDsEqual, getEntityMapKey, normalizeEntityID, } = require("../identity/entityID");
...
function beginPilotWarpVisibilityHandoff(options = {}) {
    ...
    const getVisibilityPublicGridKeyForEntity = typeof options.getVisibilityPublicGridKeyForEntity === "function"
        ? options.getVisibilityPublicGridKeyForEntity
        : () => "";
    const getVisibilityPublicGridClusterKeyForEntity = typeof options.getVisibilityPublicGridClusterKeyForEntity === "function"
        ? options.getVisibilityPublicGridClusterKeyForEntity
        : () => "";
    const getPublicGridClusterKeyForPosition = typeof options.getPublicGridClusterKeyForPosition === "function"
        ? options.getPublicGridClusterKeyForPosition
        : () => "";
    const buildPublicGridKey = typeof options.buildPublicGridKey === "function"
    ...
    const destinationPoint = warpState.targetPoint || warpState.rawDestination || entity.position;
    const destinationClusterKey = getPublicGridClusterKeyForPosition(destinationPoint);
    const handoff = {
        ...
        destinationStaticInstanceID: normalizeEntityID(warpState && warpState.destinationStaticInstanceID),
        sourceRemoved: false,
...
function buildPilotWarpVisibilityHandoffReadiness(options = {}) {
    ...
    const hasWarpPhaseNowMs = options.warpPhaseNowMs !== null &&
    ...
    const warpPhaseNowMs = hasWarpPhaseNowMs
        ...
        : nowMs;
    const warpStartTimeMs = toFiniteNumber(entity.warpState && entity.warpState.startTimeMs, nowMs);
    const warpEffectAtMs = toFiniteNumber(entity.warpState && entity.warpState.effectAtMs, 0);
    const warpEffectStamp = toInt(entity.warpState && entity.warpState.effectStamp, 0);
    const warpEffectActive = warpEffectStamp >= 0 &&
        (warpEffectAtMs <= 0 ||
    ...
    const getPilotClientWarpVisualActivationAtMs = typeof options.getPilotClientWarpVisualActivationAtMs === "function"
        ? options.getPilotClientWarpVisualActivationAtMs
        : () => 0;
    const getWarpPhaseName = typeof options.getWarpPhaseName === "function"
        ? options.getWarpPhaseName
        : () => "complete";
    const getLivePublicGridKeyForEntity = typeof options.getLivePublicGridKeyForEntity === "function"
        ? options.getLivePublicGridKeyForEntity
        : () => "";
    const getLivePublicGridClusterKeyForEntity = typeof options.getLivePublicGridClusterKeyForEntity === "function"
    ...
    const clientVisualActivationAtMs = Math.max(0, toFiniteNumber(getPilotClientWarpVisualActivationAtMs(entity), 0));
    const clientWarpVisualActive = clientVisualActivationAtMs <= 0 ||
        nowMs + 0.001 >= clientVisualActivationAtMs;
    const sourceRemovalSettleMs = Math.max(0, toFiniteNumber(options.sourceRemovalSettleMs, 0));
    const clientWarpSourceRemovalSettled = clientVisualActivationAtMs <= 0 ||
        nowMs + 0.001 >=
            (clientVisualActivationAtMs + sourceRemovalSettleMs);
    const elapsedMs = Math.max(0, warpPhaseNowMs - effectiveWarpStartMs);
    ...
    const sourcePublicGridKey = String(handoff.sourcePublicGridKey || "").trim();
    const sourceClusterKey = String(handoff.sourceClusterKey || "").trim();
    const sourcePublicGridLeft = Boolean(sourcePublicGridKey &&
        String(livePublicGridKey || "").trim() !== sourcePublicGridKey);
    const sourceClusterLeft = Boolean(sourceClusterKey &&
        String(liveClusterKey || "").trim() !== sourceClusterKey);
    const sourceHandoffReady = Boolean(clientWarpSourceRemovalSettled &&
        (sourcePublicGridLeft ||
            sourceClusterLeft));
    const destinationPoint = (entity.warpState && (entity.warpState.targetPoint ||
        entity.warpState.rawDestination)) ||
        entity.position;
    const destinationPublicGridKey = String(handoff.destinationPublicGridKey ||
        buildPublicGridKey(destinationPoint || null) ||
        "").trim();
    const destinationClusterKey = String(getPublicGridClusterKeyForPosition(destinationPoint) ||
        handoff.destinationClusterKey ||
        "").trim();
    const destinationStaticInstanceID = normalizeEntityID(handoff.destinationStaticInstanceID);
    const destinationPublicGridEntered = Boolean(destinationPublicGridKey &&
        String(livePublicGridKey || "").trim() === destinationPublicGridKey);
    const destinationClusterEntered = Boolean(destinationClusterKey &&
        String(liveClusterKey || "").trim() === destinationClusterKey);
    const destinationDistanceMeters = distance(entity.position, destinationPoint);
    const staticPreloadDistanceMeters = Math.max(0, toFiniteNumber(options.destinationStaticPreloadDistanceMeters, 0));
    const destinationDedicatedStaticCompleteReady = Boolean(destinationStaticInstanceID !== null &&
        warpPhase === "complete" &&
        destinationClusterKey &&
        (destinationPublicGridEntered ||
            destinationClusterEntered ||
            destinationDistanceMeters <= staticPreloadDistanceMeters));
    const destinationStaticPreloadReady = Boolean(warpPhase === "decel" &&
        destinationDistanceMeters <= staticPreloadDistanceMeters);
    ...
    const destinationControllerPrewarmReady = Boolean(destinationClusterKey && warpPhase === "decel");
    const destinationPublicGridPreloadReady = Boolean(destinationStaticInstanceID === null &&
        destinationClusterKey &&
        warpPhase === "decel" &&
        destinationDistanceMeters <= staticPreloadDistanceMeters);
    const destinationHandoffReady = Boolean(warpPhase === "decel" &&
        (destinationPublicGridEntered ||
            destinationClusterEntered));
    const destinationStaticHandoffReady = Boolean(destinationHandoffReady ||
    ...
    const destinationDynamicHandoffReady = Boolean((destinationHandoffReady ||
        destinationPublicGridPreloadReady) &&
        (handoff.sourceRemoved === true ||
            sourceHandoffReady ||
            destinationHandoffReady));
    const liveGridClusterKey = String(liveClusterKey || "").trim();
    const lastJoinedGridClusterKey = String(handoff.currentPublicGridClusterKey ||
        handoff.sourceClusterKey ||
        "").trim();
    const liveGridChanged = Boolean(liveGridClusterKey &&
        liveGridClusterKey !== lastJoinedGridClusterKey);
    const liveGridIsDestinationCluster = Boolean(destinationClusterKey &&
        liveGridClusterKey &&
        liveGridClusterKey === destinationClusterKey);
    const liveGridVisibilityReady = Boolean(clientWarpVisualActive &&
        liveGridChanged &&
...
function buildWarpSourceRemovalPlan(options = {}) {
    ...
    const getDynamicEntityByID = typeof options.getDynamicEntityByID === "function"
        ? options.getDynamicEntityByID
        : () => null;
    const isFleetWarpFormationCompanionForSession = typeof options.isFleetWarpFormationCompanionForSession === "function"
        ? options.isFleetWarpFormationCompanionForSession
        : () => false;
    const hasStaticEntityID = typeof options.hasStaticEntityID === "function"
        ? options.hasStaticEntityID
        : () => true;
    const shouldPreserveStaticID = typeof options.shouldPreserveStaticID === "function"
        ? options.shouldPreserveStaticID
        : () => false;
    const shouldPreserveDynamicID = typeof options.shouldPreserveDynamicID === "function"
    ...
    for (const entityID of currentDynamicIDs) {
        ...
        if (!isFleetWarpFormationCompanionForSession(session, candidate, nowMs) &&
...
function buildWarpLiveGridDynamicRefreshPlan(options = {}) {
    ...
    const formationRefreshEntityIDs = new Set(normalizeArray(options.formationRefreshEntities)
    ...
    const removedDynamicIDs = normalizeArray(liveDynamicDelta.removedIDs)
        ...
        .filter((entityID) => (entityID !== null && !formationRefreshEntityIDs.has(entityID)));
    ...
    for (const entityID of normalizeSet(liveDynamicDelta.desiredIDs)) {
        if (formationRefreshEntityIDs.has(entityID) &&
    ...
    return {
        addedDynamicEntities,
        freshDynamicIDs: new Set(addedDynamicEntities
...
function buildWarpLiveGridStaticRefreshPlan(options = {}) {
    ...
    const removedStaticIDs = [...currentStaticIDs].filter((entityID) => (!desiredStaticIDs.has(entityID) &&
        !shouldPreserveStaticID(entityID)));
    const addedStaticEntities = liveStaticEntities.filter((visibleEntity) => (!currentStaticIDs.has(getEntityID(visibleEntity))));
...
function buildWarpDestinationStaticAcquisitionPlan(options = {}) {
    ...
    const shouldPreserveStaticID = typeof options.shouldPreserveStaticID === "function"
        ? options.shouldPreserveStaticID
        : () => false;
    const desiredStaticIDs = new Set(destinationStaticEntities
        .map((visibleEntity) => getEntityID(visibleEntity))
        .filter((entityID) => entityID !== null));
    ...
    const removedStaticIDs = [...currentIDs].filter((entityID) => (!desiredStaticIDs.has(entityID) &&
        !shouldPreserveStaticID(entityID)));
    const nextVisibleStaticIDs = new Set([...currentIDs].filter((entityID) => shouldPreserveStaticID(entityID)));
...
function isWarpDestinationStaticPreserved(options = {}) {
    ...
    const destinationStaticInstanceID = normalizeEntityID(options.destinationStaticInstanceID);
    if (destinationStaticInstanceID !== null &&
        entityIDsEqual(staticEntity.dungeonSiteInstanceID, destinationStaticInstanceID)) {
        return true;
    }
    const handoff = options.handoff && typeof options.handoff === "object"
        ? options.handoff
    ...
    const destinationClusterKey = String(options.destinationClusterKey || "").trim();
    const getPublicGridClusterKeyForEntity = typeof options.getPublicGridClusterKeyForEntity === "function"
        ? options.getPublicGridClusterKeyForEntity
        : () => "";
    const getPublicGridClusterKeyForPosition = typeof options.getPublicGridClusterKeyForPosition === "function"
        ? options.getPublicGridClusterKeyForPosition
        : () => "";
    const staticClusterKey = String(getPublicGridClusterKeyForEntity(staticEntity) ||
        getPublicGridClusterKeyForPosition(staticEntity.position) ||
        "").trim();
    if (destinationClusterKey &&
        staticClusterKey &&
    ...
    const distance = typeof options.distance === "function"
        ? options.distance
        : () => Infinity;
    const bubbleRadiusMeters = Math.max(0, toFiniteNumber(options.bubbleRadiusMeters, 0));
...
function buildWarpDestinationAcquireIntent(options = {}) {
    const handoff = options.handoff && typeof options.handoff === "object"
        ...
        : {};
    const destinationClusterKey = String(options.destinationClusterKey || "").trim();
    const shouldAcquireDestinationStatic = Boolean(!handoff.destinationStaticJoined &&
        destinationClusterKey &&
        options.destinationStaticHandoffReady === true);
    const shouldAcquireDestinationDynamic = Boolean(!handoff.destinationJoined &&
        destinationClusterKey &&
...
function buildWarpDestinationDynamicRefreshPlan(options = {}) {
    ...
    const deferredFormationCompanionIDs = new Set(normalizeArray(options.formationRefreshEntities)
        .map((candidate) => getEntityID(candidate))
        .filter((entityID) => entityID !== null));
    const destinationRefreshEntities = normalizeArray(destinationDelta.addedEntities)
        .filter((candidate) => !isDeferredFleetWarpCompanion(candidate, deferredFormationCompanionIDs));
...
//# sourceMappingURL=warpHandoff.js.map