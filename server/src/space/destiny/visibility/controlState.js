...
const { entityIDsEqual, getEntityMapKey, normalizePersistentEntityID, } = require("../identity/entityID");
const { DESTINY_STAMP_MAX_FORWARD_LEAD, hasDestinyStamp, isDestinyStampAfter, normalizeDestinyStamp, resolveOptionalDestinyStamp, selectFurthestDestinyStamp, } = require("../delivery/stamps");
...
function buildVisibilityControlState(options = {}) {
    ...
    return {
        pilotWarpQuietUntilStamp: resolveOptionalDestinyStamp(options.pilotWarpQuietUntilStamp),
        pilotWarpVisibilityHandoff: options.pilotWarpVisibilityHandoff &&
        ...
        postAttachVisibilityReconcileReason: typeof options.postAttachVisibilityReconcileReason === "string"
...
function buildPostAttachVisibilityReconciliationOptions(reason) {
    return {
        ...
        postAttachVisibilityReconcileReason: typeof reason === "string" ? reason : null,
    };
}
...
function beginWarpDepartureVisibilityGrace(optionsOrEntity = {}, departureBubbleID = null, positionalNowMs = 0, positionalDurationMs = 0) {
...
function updatePilotWarpHandoffLiveGrid(progress, publicGridKey, publicGridClusterKey, sourceRemoved, destinationJoined, destinationStaticJoined) {
    if (!progress || typeof progress !== "object") {
        return false;
    }
    progress.currentPublicGridKey = String(publicGridKey || "").trim();
    progress.currentPublicGridClusterKey = String(publicGridClusterKey || "").trim();
...
function capturePilotWarpVisibilityHandoff(handoff) {
    ...
    return Object.freeze({
        ...
        destinationStaticInstanceID: normalizeOptionalPersistentEntityID(handoff.destinationStaticInstanceID),
        destinationDungeonRoomKey: String(handoff.destinationDungeonRoomKey || "").trim(),
        destinationDungeonSiteID: normalizeOptionalPersistentEntityID(handoff.destinationDungeonSiteID),
...
function isPilotWarpVisibilityHandoffCaptureCurrent(handoff, capture) {
    return Boolean(handoff &&
        ...
        sameOptionalPersistentEntityID(handoff.destinationStaticInstanceID, capture.destinationStaticInstanceID) &&
        String(handoff.destinationDungeonRoomKey || "").trim() ===
            capture.destinationDungeonRoomKey &&
        sameOptionalPersistentEntityID(handoff.destinationDungeonSiteID, capture.destinationDungeonSiteID) &&
...
function resolvePilotWarpQuietWindow(options = {}) {
    ...
    const quietUntilStamp = resolveOptionalDestinyStamp(generation.pilotWarpQuietUntilStamp);
    ...
    const currentSessionStamp = normalizeDestinyStamp(options.currentSessionStamp);
    const maximumLead = toNonNegativeInteger(options.maximumLead, DESTINY_STAMP_MAX_FORWARD_LEAD);
...
function decoratePilotWarpVisibilityHandoff(options = {}) {
    ...
    handoff.destinationDungeonRoomKey = String(warpState.destinationDungeonRoomKey || "").trim() || null;
    handoff.destinationDungeonSiteID = normalizePersistentEntityID(warpState.destinationDungeonSiteID);
    return handoff;
}
...
function markPilotWarpLandingPending(options = {}) {
    ...
    if (!handoff || !entityIDsEqual(handoff.shipID, entityID)) {
        const completedWarpState = options.completedWarpState && typeof options.completedWarpState === "object"
        ...
        handoff = {
            ...
            destinationPublicGridKey: String(options.destinationPublicGridKey || "").trim(),
            destinationStaticInstanceID: normalizePersistentEntityID(completedWarpState.destinationStaticInstanceID),
            destinationDungeonRoomKey: String(completedWarpState.destinationDungeonRoomKey || "").trim() || null,
            destinationDungeonSiteID: normalizePersistentEntityID(completedWarpState.destinationDungeonSiteID),
            sourceRemoved: true,
...
function commitAcceptedPilotWarpLandingControl(options = {}) {
    ...
    const maximumLead = toNonNegativeInteger(options.maximumLead, DESTINY_STAMP_MAX_FORWARD_LEAD);
    generation.pilotWarpQuietUntilStamp = selectFurthestDestinyStamp(acceptedStamp, [generation.pilotWarpQuietUntilStamp], maximumLead);
    handoff.landingPending = false;
...
function consumeWarpAcquireUntilNextTickSuppression(options = {}) {
    ...
    const nowMs = toFiniteNumber(options.nowMs, 0);
    const durationMs = toNonNegativeInteger(options.durationMs, 0);
    entity.visibilitySuppressedUntilMs = Math.max(toFiniteNumber(entity.visibilitySuppressedUntilMs, 0), nowMs + durationMs);
...
function capturePostAttachVisibilityReconciliation(session) {
    ...
    return Object.freeze({
        generation,
        reason: typeof generation.postAttachVisibilityReconcileReason === "string"
            ? generation.postAttachVisibilityReconcileReason
            : null,
        session,
    });
...
function unregisterDeferredInitialVisibilityEntity(queue, entity, options = {}) {
    ...
    if (options.clearLaunchPresentationSnapshot !== false &&
        Object.prototype.hasOwnProperty.call(entity, "launchPresentationSnapshot")) {
        delete entity.launchPresentationSnapshot;
        changed = true;
    }
...
function collectDeferredInitialVisibilityEntities(queue, resolveEntityByID) {
...
function generationHasPendingEntityRequirement(generation, entityID) {
    return Boolean(generation &&
        (mapHasEntityID(generation.pendingNativeVisibilityAcquisitionsByID, entityID) ||
            mapHasEntityID(generation.pendingNativeVisibilityRemovalsByID, entityID)));
}
function settleDeferredInitialVisibilityAfterFlush(options = {}) {
    ...
    for (const observation of observations) {
        ...
        const carriedRequirement = Boolean(requiresAcquisition ||
        ...
        const currentlyPending = generationHasPendingEntityRequirement(observation.generation, entityID);
        if (currentlyPending) {
            summary.pendingSessionCount += 1;
            continue;
        }
        if (carriedRequirement &&
            setHasEntityID(observation.generation.visibleDynamicEntityIDs, entityID)) {
