...
function buildVisibilityControlState(options: Record<string, any> = {}) {
...
function beginWarpDepartureVisibilityGrace(
  optionsOrEntity: Record<string, any> = {},
  departureBubbleID = null,
  positionalNowMs = 0,
...
function resolvePilotWarpQuietWindow(options: Record<string, any> = {}) {
...
function decoratePilotWarpVisibilityHandoff(options: Record<string, any> = {}) {
  const session = options.session || null;
...
function markPilotWarpDestinationPrewarmed(options: Record<string, any> = {}) {
  const session = options.session || null;
  const generation = resolveExpectedGeneration(session, options.generation);
  const handoff = options.handoff && typeof options.handoff === "object"
    ? options.handoff
...
function commitPilotWarpVisibilityHandoffProgress(options: Record<string, any> = {}) {
  const session = options.session || null;
  const generation = resolveExpectedGeneration(session, options.generation);
  const handoff = options.handoff && typeof options.handoff === "object"
    ? options.handoff
  ...
  for (const [key, value] of Object.entries<any>(nextHandoff)) {
...
function markPilotWarpLandingPending(options: Record<string, any> = {}) {
  const session = options.session || null;
  const generation = resolveExpectedGeneration(session, options.generation);
  const entity = options.entity && typeof options.entity === "object"
    ? options.entity
...
function commitAcceptedPilotWarpLandingControl(options: Record<string, any> = {}) {
  const session = options.session || null;
  const generation = resolveExpectedGeneration(session, options.generation);
  const handoff = options.handoff && typeof options.handoff === "object"
    ? options.handoff
...
function consumeWarpAcquireUntilNextTickSuppression(options: Record<string, any> = {}) {
  const entity = options.entity && typeof options.entity === "object"
    ? options.entity
    : null;
...
function authorPostAttachVisibilityReconciliation(generation, options: Record<string, any> = {}) {
...
function unregisterDeferredInitialVisibilityEntity(queue, entity, options: Record<string, any> = {}) {
  const entityID = getEntityMapKey(entity && entity.itemID);
  if (!(queue instanceof Set) || !entity || entityID === null) {
    return false;
  }
...
function collectDeferredInitialVisibilityEntities(
  ...
  const entities: any[] = [];
  const seenEntities = new Set<any>();
...
function captureDeferredInitialVisibilitySessionObservation(options: Record<string, any> = {}) {
  const session = options.session || null;
...
function settleDeferredInitialVisibilityAfterFlush(options: Record<string, any> = {}) {
