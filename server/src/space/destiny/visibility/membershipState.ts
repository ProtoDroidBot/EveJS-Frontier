...
function getStaticEntity(scene, entityID, options: Record<string, any> = {}) {
...
function isIncrementalStaticEntity(entity, options: Record<string, any> = {}) {
  return (
...
function applyStaleGenerationBehavior(
  ...
  if (behavior === "throw") {
    const error: Error & Record<string, any> = new Error(errorCode);
...
function reserveSceneVisibilityRemovals(session, entityIDs, options: Record<string, any> = {}) {
  const generation = session && session._space;
...
function collectDeltaFreshEntityIDs(delta: Record<string, any> = {}) {
...
function finalizeAcceptedIntent(intent, options: Record<string, any> = {}) {
...
function commitInitialBallparkVisibility(
  ...
  generation.visibleDynamicEntityIDs = dynamicIDs;
  generation.visibleBubbleScopedStaticEntityIDs = staticIDs;
  generation.freshlyVisibleDynamicEntityIDs = new Set<any>();
...
function createVisibilityGenerationState(options: Record<string, any> = {}) {
...
function recordVisibilityJournal(session, event, details: Record<string, any> = {}, options: Record<string, any> = {}) {
...
function createVisibilityJournalRecorder(options: Record<string, any> = {}) {
  const boundOptions = { ...options };
  return (session, event, details: Record<string, any> = {}) => recordVisibilityJournal(
    session,
    event,
    details,
    boundOptions,
