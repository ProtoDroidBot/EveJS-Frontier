...
function buildPostWarpLandingVisibilityPresentation(options: Record<string, any> = {}) {
  ...
  if (staticDelta) {
    ...
    nextVisibleStaticEntityIDs = new Set<any>(normalizeSet(staticDelta.desiredIDs));
  }

  const uniqueAddedEntities: any[] = [];
  const addedEntityIDs = new Set<any>();
...
function queueFleetWarpCompanionLandingPresentation(options: Record<string, any> = {}) {
