...
function buildDynamicVisibilityDeltaPlan(options = {}) {
    ...
    const desiredIDs = new Set(desiredEntities.map((entity) => getEntityKey(entity)));
    const currentIDs = normalizeSet(options.currentIDs);
    const warpHandoff = options.warpHandoff && typeof options.warpHandoff === "object"
        ? options.warpHandoff
        : null;
    const destinationHandoffClusterKey = String(warpHandoff && warpHandoff.destinationClusterKey || "").trim();
    const preserveDestinationDynamicsThroughWarpHandoff = Boolean(destinationHandoffClusterKey &&
        warpHandoff &&
    ...
    const shouldPreserveDestinationDynamic = (entity) => {
        ...
        if (getEntityKey(entity) === egoEntityKey ||
            entity.mode === "WARP" ||
            entity.pendingWarp ||
            entity.warpState) {
            return false;
        }
        const entityClusterKey = String(getVisibilityPublicGridClusterKeyForEntity(entity) || "").trim();
    ...
    const addedEntities = desiredEntities.filter((entity) => !currentIDs.has(getEntityKey(entity)));
...
function buildStaticVisibilityDeltaPlan(options = {}) {
    const desiredEntities = normalizeArray(options.desiredEntities);
    const desiredIDs = new Set(desiredEntities
    ...
    const warpHandoff = options.warpHandoff && typeof options.warpHandoff === "object"
        ? options.warpHandoff
        : null;
    const preserveCurrentStaticsThroughWarpHandoff = Boolean(egoEntity &&
        isWarpingEntity(egoEntity) &&
        (!warpHandoff || warpHandoff.sourceRemoved !== true));
    const getStaticEntityByID = typeof options.getStaticEntityByID === "function"
        ? options.getStaticEntityByID
        : () => null;
    const isIncrementalStaticVisibilityEntity = typeof options.isIncrementalStaticVisibilityEntity === "function"
