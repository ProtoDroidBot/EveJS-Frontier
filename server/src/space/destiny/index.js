...
function buildWallclockFiletimeFromMs(value) {
    ...
    return buildFiletimeLong(BigInt(Math.trunc(numericValue)) * FILETIME_TICKS_PER_MS + FILETIME_EPOCH_OFFSET);
}
...
function buildStructureSlimDamage(entity) {
    const conditionState = entity && entity.conditionState && typeof entity.conditionState === "object"
        ...
        : {};
    const structureDamage = Math.max(0, Math.min(1, toFiniteNumber(conditionState.damage, 0)));
    const armorDamage = Math.max(0, Math.min(1, toFiniteNumber(conditionState.armorDamage, 0)));
    const shieldCharge = Math.max(0, Math.min(1, conditionState.shieldCharge === undefined || conditionState.shieldCharge === null
        ? 1
...
function appendEntityStandings(entries, entity, slimTypeID) {
    ...
    const hostileResponseThreshold = Number.isFinite(Number(entity && entity.hostileResponseThreshold))
        ? Number(entity.hostileResponseThreshold)
        : authoredStandings && authoredStandings.hostileResponseThreshold;
    const friendlyResponseThreshold = Number.isFinite(Number(entity && entity.friendlyResponseThreshold))
...
function buildSlimNameIDValue(value) {
    ...
    if (typeof value === "object") {
        ...
        return [
            label,
            buildDict(payload && typeof payload === "object" && !Array.isArray(payload)
...
function buildSlimItemDict(entity) {
    ...
    const slimTypeID = toInt32(entity && entity.slimTypeID, toInt32(entity && entity.typeID, 0));
    const slimGroupID = toInt32(entity && entity.slimGroupID, toInt32(entity && entity.groupID, 0));
    const slimCategoryID = toInt32(entity && entity.slimCategoryID, toInt32(entity && entity.categoryID, 0));
    const suppressSlimName = entity && entity.suppressSlimName === true;
    const slimName = suppressSlimName
        ? ""
        : String(entity && (hasEntityField("slimName")
            ? entity.slimName
            : entity.itemName) || "");
    const entries = [
    ...
    const slimGraphicID = toInt32(entity && entity.slimGraphicID, toInt32(entity && entity.graphicID, 0));
    ...
    const dunObjectID = toInt32(entity && (entity.dunObjectID || entity.dungeonObjectID), 0);
    if (dunObjectID > 0) {
    ...
    if (Array.isArray(entity && entity.dunRotation) &&
    ...
    if (slimGroupID === 548 ||
        ...
        const startTimeMs = entity && entity.warpDisruptionStartTimeMs !== undefined
    ...
    if (entity.kind === "ship") {
        ...
        if (Array.isArray(entity.compressionFacilityTypelists) &&
            entity.compressionFacilityTypelists.length > 0) {
            entries.push([
                "compression_facility_typelists",
                buildDict(entity.compressionFacilityTypelists
                    .map((entry) => ([
        ...
        entries.push([
            "launchModules",
            buildList(Array.isArray(entity.launchModules)
...
function buildSlimItemObject(entity, compatibilityProfile = config.clientCompatibilityProfile) {
...
function buildStargateJumps(entity) {
    const rows = entity && entity.destinationID && entity.destinationSolarSystemID
...
function buildAddBalls2Payload(stateStamp, entities, simFileTime) {
    return statePayloads.buildAddBalls2Payload(stateStamp, entities, simFileTime, {
...
function buildSetStatePayload(stateStamp, system, egoEntityID, entities, simFileTime, dbuffStateEntries = [], effectStateEntries = []) {
    return statePayloads.buildSetStatePayload(stateStamp, system, egoEntityID, entities, simFileTime, dbuffStateEntries, effectStateEntries, {
        compatibilityProfile: config.clientCompatibilityProfile,
        encodeEntityBall,
        buildSlimItemDict,
        buildSlimItemObject,
