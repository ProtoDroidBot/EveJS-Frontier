...
function createDestinyDeliveryTransaction(session, options = {}) {
    ...
    const transaction = {
        ...
        spaceGeneration: session && session._space && typeof session._space === "object"
            ? session._space
            : null,
        state: "planning",
...
function mergeDestinyDeliveryTransactions(target, source) {
    ...
    if (target.session !== source.session) {
        throw new TypeError("Destiny delivery transactions cannot merge across network sessions");
    }
    if (target.spaceGeneration !== source.spaceGeneration) {
        throw new TypeError("Destiny delivery transactions cannot merge across space generations");
    }
    if (source.creationOrder < target.creationOrder &&
...
function stageDestinyDeliveryTransaction(transaction) {
    ...
    restoreAuthorityDeliveryStateForGeneration(transaction.spaceGeneration, transaction.beforeState);
...
function commitDestinyDeliveryTransaction(transaction, details = {}) {
    ...
    restoreAuthorityDeliveryStateForGeneration(transaction.spaceGeneration, transaction.stagedState || transaction.beforeState);
...
//# sourceMappingURL=deliveryTransaction.js.map