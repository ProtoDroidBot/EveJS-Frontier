...
const DESTINY_STAMP_MAX_ORDERING_DISTANCE = (DESTINY_STAMP_MODULUS / 2) - 1;
...
function advanceDestinyStamp(stamp, lead = 1) {
    return normalizeDestinyStamp(normalizeDestinyStamp(stamp) + toInt(lead, 0));
}
...
function getDestinyStampForwardBoundaryDeltaMs(currentSimTimeMs, currentStamp, targetStamp, maximumLead = DESTINY_STAMP_MAX_FORWARD_LEAD) {
    if (!hasDestinyStamp(currentStamp) || !hasDestinyStamp(targetStamp)) {
        return 0;
    }
    const forwardDistance = getDestinyStampForwardDistance(currentStamp, targetStamp);
    const normalizedMaximumLead = clamp(toInt(maximumLead, DESTINY_STAMP_MAX_FORWARD_LEAD), 0, DESTINY_STAMP_MAX_FORWARD_LEAD);
    ...
    const currentTimeMs = Number.isFinite(normalizedCurrentSimTimeMs)
        ...
        : 0;
    const intervalRemainder = ((currentTimeMs % DESTINY_STAMP_INTERVAL_MS) +
        DESTINY_STAMP_INTERVAL_MS) % DESTINY_STAMP_INTERVAL_MS;
    return (forwardDistance * DESTINY_STAMP_INTERVAL_MS) - intervalRemainder;
}
function isDestinyStampWithinForwardWindow(currentStamp, candidateStamp, maximumLead) {
...
function isDestinyStampAfter(referenceStamp, candidateStamp, maximumLead = DESTINY_STAMP_MAX_ORDERING_DISTANCE) {
    if (!hasDestinyStamp(referenceStamp) || !hasDestinyStamp(candidateStamp)) {
        return false;
    }
    const distance = getDestinyStampForwardDistance(referenceStamp, candidateStamp);
    return distance > 0 && distance <= Math.max(0, toInt(maximumLead, 0));
}
function selectLaterDestinyStamp(previousStamp, candidateStamp, maximumLead = DESTINY_STAMP_MAX_ORDERING_DISTANCE) {
    ...
    return isDestinyStampAfter(normalizedPreviousStamp, candidateStamp, maximumLead)
        ? normalizeDestinyStamp(candidateStamp)
        : normalizedPreviousStamp;
}
function selectFurthestDestinyStamp(anchorStamp, candidateStamps, maximumLead = DESTINY_STAMP_MAX_ORDERING_DISTANCE) {
    ...
    for (const candidateStamp of Array.isArray(candidateStamps)
        ...
        const candidateDistance = getDestinyStampForwardDistance(normalizedAnchorStamp, candidateStamp);
        if (candidateDistance <= normalizedMaximumLead &&
...
function selectFurthestPresentDestinyStamp(anchorStamp, candidateStamps, maximumLead = DESTINY_STAMP_MAX_ORDERING_DISTANCE) {
    const normalizedAnchorStamp = normalizeDestinyStamp(anchorStamp, 0);
    const normalizedMaximumLead = Math.max(0, toInt(maximumLead, 0));
    const presentCandidates = (Array.isArray(candidateStamps) ? candidateStamps : [candidateStamps]).filter((candidateStamp) => (hasDestinyStamp(candidateStamp) &&
        getDestinyStampForwardDistance(normalizedAnchorStamp, candidateStamp) <= normalizedMaximumLead));
    return presentCandidates.length > 0
        ? selectFurthestDestinyStamp(normalizedAnchorStamp, presentCandidates, normalizedMaximumLead)
        : null;
}
...
function advanceNextDestinyStamp(options = {}) {
    ...
    const previousNextStamp = normalizeDestinyStamp(options.nextStamp, currentStamp);
    const previousLead = getDestinyStampForwardDistance(currentStamp, previousNextStamp);
...
function advanceHistorySafeDestinyStamp(options = {}) {
    const currentStamp = normalizeDestinyStamp(options.currentStamp, 0);
    const maximumLead = clamp(toInt(options.maximumLead, 0), 0, DESTINY_STAMP_MAX_FORWARD_LEAD);
    const normalizedLead = clamp(toInt(options.minimumLead, 1), 0, maximumLead);
    const previousNextStamp = normalizeDestinyStamp(options.nextStamp, currentStamp);
    const previousLead = getDestinyStampForwardDistance(currentStamp, previousNextStamp);
    const baseLead = previousLead <= maximumLead ? previousLead : 0;
    return advanceDestinyStamp(currentStamp, clamp(Math.max(baseLead, normalizedLead), 0, maximumLead));
}
module.exports = {
