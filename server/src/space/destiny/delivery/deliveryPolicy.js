...
const { DESTINY_STAMP_INTERVAL_MS, DESTINY_STAMP_MAX_LEAD, } = require("../constants");
const { MICHELLE_DIRECT_CRITICAL_ECHO_DESTINY_LEAD, MICHELLE_HELD_FUTURE_DESTINY_LEAD, MICHELLE_POST_HELD_FUTURE_DESTINY_LEAD, } = require("./michelleContract.js");
const { DESTINY_STAMP_MAX_FORWARD_LEAD, DESTINY_STAMP_MAX_ORDERING_DISTANCE, advanceDestinyStamp, clampDestinyStampToCeiling, getDestinyStampForwardDistance, hasDestinyStamp, isDestinyStampAfter, isDestinyStampWithinForwardWindow, normalizeDestinyStamp, resolveOptionalDestinyStamp, selectFurthestPresentDestinyStamp, } = require("./stamps");
...
function isDestinyStampWithinRelativeWindow(anchorStamp, candidateStamp, minimumLead, maximumLead) {
    ...
    return isDestinyStampWithinForwardWindow(windowStart, candidateStamp, normalizedMaximumLead - normalizedMinimumLead);
}
function getRecentDestinyStampDelta(previousStamp, currentStamp, maximumDelta) {
...
function resolveDelayedDestinyStampState(options = {}) {
    ...
    if (pendingStamp === null &&
        ...
        const delay = Math.min(DESTINY_STAMP_MAX_FORWARD_LEAD, Math.max(0, toInt(options.delay, 1)));
...
function projectPreviouslySentDestinyLane(sentLaneStamp, sentRawDispatchStamp, currentRawDispatchStamp) {
    if (!hasDestinyStamp(sentLaneStamp) ||
    ...
    const rawDispatchDelta = getDestinyStampForwardDistance(normalizedSentRawDispatchStamp, normalizedCurrentRawDispatchStamp);
    if (rawDispatchDelta <= 0 ||
...
function resolvePreviousLastSentDestinyWasOwnerCritical(options = {}) {
    ...
    const previousLastSentDestinyStamp = toDestinyStamp(options.previousLastSentDestinyStamp);
    const inferredTrackedOwnerCriticalLane = ((hasDestinyStamp(options.lastOwnerMissileFreshAcquireStamp) &&
    ...
    const inferredOwnerDamageLifecycleLane = hasDestinyStamp(options.lastOwnerMissileLifecycleStamp) &&
        previousLastSentDestinyStamp ===
...
function resolveProjectedRecentLastSentLane(options = {}) {
    ...
    const previousLastSentDestinyStamp = toDestinyStamp(options.previousLastSentDestinyStamp);
    const previousLastSentDestinyRawDispatchStamp = toDestinyStamp(options.previousLastSentDestinyRawDispatchStamp);
    const currentRawDispatchStamp = toDestinyStamp(options.currentRawDispatchStamp);
    const maximumDispatchDelta = Math.max(0, toInt(options.maximumDispatchDelta, 2));
    const rawDispatchDelta = getRecentDestinyStampDelta(previousLastSentDestinyRawDispatchStamp, currentRawDispatchStamp, maximumDispatchDelta);
    if (rawDispatchDelta === null || rawDispatchDelta <= 0) {
        return null;
    }
    return projectPreviouslySentDestinyLane(previousLastSentDestinyStamp, previousLastSentDestinyRawDispatchStamp, currentRawDispatchStamp);
}
function getRecentTrustedLane(options = {}) {
    if (!hasDestinyStamp(options.laneStamp) ||
    ...
    const maximumLead = Math.max(0, toInt(options.maximumLead, MICHELLE_HELD_FUTURE_DESTINY_LEAD));
    ...
    const maximumRawDispatchDelta = Math.max(0, toInt(options.maximumRawDispatchDelta, 2));
    if (hasDestinyStamp(laneRawDispatchStamp) &&
        hasDestinyStamp(currentRawDispatchStamp)) {
        const rawDispatchDelta = getRecentDestinyStampDelta(laneRawDispatchStamp, currentRawDispatchStamp, maximumRawDispatchDelta);
...
function getRecentProjectedLane(options = {}) {
    ...
    const currentRawDispatchStamp = toDestinyStamp(options.currentRawDispatchStamp);
    const rawDispatchDelta = getDestinyStampForwardDistance(laneRawDispatchStamp, currentRawDispatchStamp);
    if (rawDispatchDelta <= 0 ||
        rawDispatchDelta > Math.max(0, toInt(options.maximumRawDispatchDelta, 2))) {
        return null;
    }
    return getRecentTrustedLane({
        laneStamp: projectPreviouslySentDestinyLane(laneStamp, laneRawDispatchStamp, currentRawDispatchStamp),
...
function attachRecentOwnerLanePresence(state, rawDispatchStampPresent, anchorStampPresent) {
...
function resolveRecentOwnerLaneState(options = {}) {
    ...
    const laneRawDispatchStamp = hasDestinyStamp(options.laneRawDispatchStamp)
        ...
        : null;
    const requiredClocksPresent = hasRequiredDestinyStamps(options.currentSessionStamp, options.currentImmediateSessionStamp, options.currentRawDispatchStamp);
    const maximumRawDispatchDelta = Math.max(0, toInt(options.maximumRawDispatchDelta, 2));
    const maximumProjectedLead = Math.max(0, toInt(options.maximumProjectedLead, MICHELLE_HELD_FUTURE_DESTINY_LEAD +
        MICHELLE_POST_HELD_FUTURE_DESTINY_LEAD));
    const allowFarAheadReuseAfterRawAdvance = options.allowFarAheadReuseAfterRawAdvance !== false;
    ...
    const currentImmediateSessionStamp = toDestinyStamp(options.currentImmediateSessionStamp);
    const currentRawDispatchStamp = toDestinyStamp(options.currentRawDispatchStamp);
    const rawDispatchDistance = laneRawDispatchStamp !== null
        ? getDestinyStampForwardDistance(laneRawDispatchStamp, currentRawDispatchStamp)
        : null;
    const rawDispatchDelta = rawDispatchDistance !== null &&
        rawDispatchDistance <= DESTINY_STAMP_MAX_ORDERING_DISTANCE
        ? rawDispatchDistance
        : 0;
    const hasRecentRawWindow = laneRawDispatchStamp !== null &&
        rawDispatchDistance <= DESTINY_STAMP_MAX_ORDERING_DISTANCE &&
    ...
    const anchorDistance = laneAnchorStamp !== null
        ...
        : null;
    const anchorDelta = anchorDistance !== null &&
        anchorDistance <= DESTINY_STAMP_MAX_ORDERING_DISTANCE
        ? anchorDistance
        : 0;
    const progressDelta = Math.max(rawDispatchDelta, anchorDelta);
    const allowRecentLaneReuse = isDestinyStampAfter(currentImmediateSessionStamp, laneStamp) &&
        (allowFarAheadReuseAfterRawAdvance === true ||
            rawDispatchDelta <= 0 ||
            isDestinyStampWithinForwardWindow(currentImmediateSessionStamp, laneStamp, maximumProjectedLead));
    const recentLane = allowRecentLaneReuse &&
        (hasRecentRawWindow ||
        ...
        ? laneStamp
        : null;
    const projectedLane = progressDelta > 0
    ...
    const canProjectConsumedLane = projectedLane !== null &&
        (laneRawDispatchStamp === null ||
            hasRecentRawWindow);
    const projectedConsumedLane = canProjectConsumedLane &&
        isDestinyStampWithinForwardWindow(currentImmediateSessionStamp, projectedLane, maximumProjectedLead)
        ? projectedLane
        : null;
    return attachRecentOwnerLanePresence({
        laneStamp,
...
function summarizeDecisionCandidates(candidates = [], selectedValue = null) {
    ...
    const normalizedCandidates = Array.isArray(candidates)
        ? candidates.map((candidate) => {
            ...
            Object.defineProperty(normalizedCandidate, DECISION_CANDIDATE_PRESENT, { enumerable: false, value: present });
            return normalizedCandidate;
        })
        : [];
    return {
        all: normalizedCandidates,
        active: normalizedCandidates.filter((candidate) => (candidate[DECISION_CANDIDATE_PRESENT] === true)),
        winners: selectedValueIsPresent
            ? normalizedCandidates.filter((candidate) => (candidate[DECISION_CANDIDATE_PRESENT] === true &&
                candidate.value === normalizedSelectedValue))
            : [],
    };
}
function formatRecentOwnerLaneStateSummary(label, state) {
    const resolvedState = state && typeof state === "object"
        ? state
        : {};
    return `${label}: lane=${formatDiagnosticDestinyStamp(resolvedState.laneStamp)} recent=${formatDiagnosticDestinyStamp(resolvedState.recentLane)} projected=${formatDiagnosticDestinyStamp(resolvedState.projectedConsumedLane)} rawDelta=${toInt(resolvedState.rawDispatchDelta, 0)} anchorDelta=${toInt(resolvedState.anchorDelta, 0)} progress=${toInt(resolvedState.progressDelta, 0)}`;
}
function formatDecisionCandidateSummary(summary) {
    ...
    return active.map((entry) => `${winnerLabels.has(entry.label) ? "*" : ""}${entry.label}=${formatDiagnosticDestinyStamp(entry.value)}`);
}
function resolveOwnerMonotonicState(options = {}) {
    const hasOwnerShip = options.hasOwnerShip === true;
    const containsMovementContractPayload = options.containsMovementContractPayload === true;
    const isSetStateGroup = options.isSetStateGroup === true;
    const isOwnerPilotMovementGroup = options.isOwnerPilotMovementGroup === true;
    const isMissileLifecycleGroup = options.isMissileLifecycleGroup === true;
    const isOwnerMissileLifecycleGroup = options.isOwnerMissileLifecycleGroup === true;
    const isOwnerCriticalGroup = options.isOwnerCriticalGroup === true;
    const isFreshAcquireLifecycleGroup = options.isFreshAcquireLifecycleGroup === true;
    const isOwnerDamageStateGroup = options.isOwnerDamageStateGroup === true;
    const allowAdjacentRawFreshAcquireLaneReuse = options.allowAdjacentRawFreshAcquireLaneReuse === true;
    const currentSessionStamp = toDestinyStamp(options.currentSessionStamp);
    const currentImmediateSessionStamp = toDestinyStamp(options.currentImmediateSessionStamp);
    ...
    const currentPresentedOwnerCriticalStamp = readOptionalStamp("currentPresentedOwnerCriticalStamp");
    const currentRawDispatchStamp = toDestinyStamp(options.currentRawDispatchStamp);
    const recentEmittedOwnerCriticalMaxLead = Math.max(0, toInt(options.recentEmittedOwnerCriticalMaxLead, 0));
    const ownerCriticalCeilingLead = Math.max(0, toInt(options.ownerCriticalCeilingLead, MICHELLE_HELD_FUTURE_DESTINY_LEAD));
    const previousLastSentDestinyStamp = readOptionalStamp("previousLastSentDestinyStamp");
    const previousLastSentDestinyRawDispatchStamp = readOptionalStamp("previousLastSentDestinyRawDispatchStamp");
    const previousLastSentDestinyExplicitWasOwnerCritical = options.previousLastSentDestinyExplicitWasOwnerCritical === true;
    const previousLastSentDestinyWasOwnerCritical = options.previousLastSentDestinyWasOwnerCritical === true;
    const previousLastSentDestinyOnlyStaleProjectedOwnerMissileLane = options.previousLastSentDestinyOnlyStaleProjectedOwnerMissileLane === true;
    const lastOwnerPilotCommandMovementStamp = readOptionalStamp("lastOwnerPilotCommandMovementStamp");
    const lastOwnerPilotCommandMovementAnchorStamp = readOptionalStamp("lastOwnerPilotCommandMovementAnchorStamp");
    const lastOwnerPilotCommandMovementRawDispatchStamp = readOptionalStamp("lastOwnerPilotCommandMovementRawDispatchStamp");
    const lastOwnerNonMissileCriticalStamp = readOptionalStamp("lastOwnerNonMissileCriticalStamp");
    const lastOwnerMissileLifecycleStamp = readOptionalStamp("lastOwnerMissileLifecycleStamp");
    const lastOwnerMissileLifecycleAnchorStamp = readOptionalStamp("lastOwnerMissileLifecycleAnchorStamp");
    const lastOwnerMissileLifecycleRawDispatchStamp = readOptionalStamp("lastOwnerMissileLifecycleRawDispatchStamp");
    const lastOwnerMissileFreshAcquireStamp = readOptionalStamp("lastOwnerMissileFreshAcquireStamp");
    const lastOwnerMissileFreshAcquireAnchorStamp = readOptionalStamp("lastOwnerMissileFreshAcquireAnchorStamp");
    const lastOwnerMissileFreshAcquireRawDispatchStamp = readOptionalStamp("lastOwnerMissileFreshAcquireRawDispatchStamp");
    const maximumTrustedRecentEmittedOwnerCriticalStamp = hasOwnerShip
        ? advanceDestinyStamp(currentSessionStamp, recentEmittedOwnerCriticalMaxLead)
        : null;
    const maximumProjectedOwnerCriticalLead = Math.max(MICHELLE_HELD_FUTURE_DESTINY_LEAD +
        MICHELLE_POST_HELD_FUTURE_DESTINY_LEAD, recentEmittedOwnerCriticalMaxLead +
    ...
    const previousLastSentRawDispatchDelta = getRecentDestinyStampDelta(previousLastSentDestinyRawDispatchStamp, currentRawDispatchStamp, 2);
    const previousLastSentLaneIsRelevant = isDestinyStampWithinRelativeWindow(currentImmediateSessionStamp, previousLastSentDestinyStamp, -1, DESTINY_STAMP_MAX_FORWARD_LEAD) && (previousLastSentRawDispatchDelta !== null ||
        previousLastSentDestinyRawDispatchStamp === null);
    const hasProjectedRecentLastSentLane = previousLastSentRawDispatchDelta !== null &&
        previousLastSentRawDispatchDelta > 0 &&
        isDestinyStampWithinRelativeWindow(currentImmediateSessionStamp, projectedRecentLastSentLane, 0, DESTINY_STAMP_MAX_FORWARD_LEAD);
    const previousLastSentDestinyAnchorStamp = previousLastSentLaneIsRelevant &&
        ...
        : previousLastSentLaneIsRelevant &&
            ...
            : previousLastSentLaneIsRelevant &&
                ...
                : null;
    const previousLastSentDestinyMatchesOwnerMissileLane = previousLastSentLaneIsRelevant &&
        (previousLastSentDestinyStamp === lastOwnerMissileLifecycleStamp ||
            previousLastSentDestinyStamp === lastOwnerMissileFreshAcquireStamp);
    const previousLastSentDestinyWasOwnerDamageLifecycleLane = previousLastSentDestinyExplicitWasOwnerCritical !== true &&
    ...
    const recentOverallLastSentState = resolveRecentOwnerLaneState({
        ...
        allowFarAheadReuseAfterRawAdvance: !(previousLastSentDestinyOnlyStaleProjectedOwnerMissileLane === true &&
            previousLastSentDestinyMatchesOwnerMissileLane),
    });
    const recentPresentedLastSentLane = hasOwnerShip &&
        previousLastSentLaneIsRelevant &&
        previousLastSentDestinyStamp === currentPresentedOwnerCriticalStamp &&
        previousLastSentRawDispatchDelta !== null &&
        (isDestinyStampWithinForwardWindow(currentSessionStamp, previousLastSentDestinyStamp, recentEmittedOwnerCriticalMaxLead) ||
            previousLastSentDestinyRawDispatchStamp === currentRawDispatchStamp)
        ? previousLastSentDestinyStamp
        : null;
    const recentNonCriticalLastSentLane = containsMovementContractPayload &&
        !isSetStateGroup &&
        previousLastSentDestinyWasOwnerCritical !== true
    ...
    const presentedLastSentCeilingCap = hasOwnerShip
        ...
        ? selectFurthestPresentDestinyStamp(currentSessionStamp, [
            advanceDestinyStamp(currentSessionStamp, ownerCriticalCeilingLead),
            currentPresentedOwnerCriticalStamp,
        ], DESTINY_STAMP_MAX_FORWARD_LEAD)
        : null;
    const uncappedPresentedLastSentMonotonicFloor = recentPresentedLastSentLane !== null
        ...
        : null;
    const presentedLastSentMonotonicFloor = uncappedPresentedLastSentMonotonicFloor !== null &&
        presentedLastSentCeilingCap !== null
        ? clampDestinyStampToCeiling(currentSessionStamp, uncappedPresentedLastSentMonotonicFloor, presentedLastSentCeilingCap)
    ...
    const recentOverallOwnerCriticalState = previousLastSentDestinyWasOwnerCritical === true &&
        ...
        ? resolveRecentOwnerLaneState({
            ...
            currentRawDispatchStamp,
            maximumProjectedLead: maximumProjectedOwnerCriticalLead,
            allowFarAheadReuseAfterRawAdvance: !previousLastSentDestinyMatchesOwnerMissileLane,
        })
    ...
    const uncappedGenericMonotonicFloor = containsMovementContractPayload &&
        !isSetStateGroup &&
        !isOwnerMissileLifecycleGroup &&
        !isOwnerCriticalGroup &&
        isDestinyStampWithinRelativeWindow(currentSessionStamp, recentNonCriticalLastSentLane, 1, maximumProjectedOwnerCriticalLead)
        ? selectFurthestPresentDestinyStamp(currentSessionStamp, [
        ...
        : null;
    const genericMonotonicFloor = uncappedGenericMonotonicFloor !== null &&
        presentedLastSentCeilingCap !== null
        ? clampDestinyStampToCeiling(currentSessionStamp, uncappedGenericMonotonicFloor, presentedLastSentCeilingCap)
        : uncappedGenericMonotonicFloor;
    const recentOwnerCriticalLane = isOwnerCriticalGroup &&
        previousLastSentDestinyWasOwnerCritical === true
        ? (recentPresentedLastSentLane !== null
            ...
            : ((!hasOwnerShip ||
                ...
                : null))
        : null;
    const recentOwnerCriticalRawDispatchDelta = previousLastSentRawDispatchDelta === null
        ? 0
        : previousLastSentRawDispatchDelta;
    const isTrustedRecentOwnerCriticalLane = (lane, rawDispatchDelta = 0, allowBeyondBufferedCeiling = false) => (hasDestinyStamp(lane) &&
        (rawDispatchDelta === 0 ||
            allowBeyondBufferedCeiling === true ||
            !hasOwnerShip ||
            isDestinyStampWithinForwardWindow(currentSessionStamp, lane, recentEmittedOwnerCriticalMaxLead)));
    const allowFarAheadRecentOwnerMissileLifecycleLane = recentOwnerMissileLifecycleState.rawDispatchDelta === 1 &&
        previousLastSentDestinyOnlyStaleProjectedOwnerMissileLane !== true &&
    ...
    const allowFarAheadRecentOverallOwnerCriticalLane = recentOverallLastSentState.rawDispatchDelta === 1 ||
        recentOverallOwnerCriticalState.rawDispatchDelta === 1;
    const previousLastSentDestinyMatchesTrackedOwnerCriticalLane = previousLastSentLaneIsRelevant &&
        (previousLastSentDestinyStamp === lastOwnerPilotCommandMovementStamp ||
            previousLastSentDestinyStamp === lastOwnerNonMissileCriticalStamp ||
            previousLastSentDestinyStamp === lastOwnerMissileLifecycleStamp ||
            previousLastSentDestinyStamp === lastOwnerMissileFreshAcquireStamp);
    const allowUntrackedRecentOverallOwnerLaneTrust = recentOverallLastSentState.rawDispatchDelta > 0 &&
    ...
    const recentOverallLastSentTrustedLane = (previousLastSentDestinyMatchesTrackedOwnerCriticalLane === true ||
        allowUntrackedRecentOverallOwnerLaneTrust === true) &&
        isTrustedRecentOwnerCriticalLane(recentOverallLastSentState.recentLane, recentOverallLastSentState.rawDispatchDelta, allowFarAheadRecentOverallOwnerCriticalLane)
        ? recentOverallLastSentState.recentLane
        : null;
    const recentOwnerMovementTrustedLane = isTrustedRecentOwnerCriticalLane(recentOwnerMovementState.recentLane, recentOwnerMovementState.rawDispatchDelta)
        ? recentOwnerMovementState.recentLane
        : null;
    const recentOwnerMissileLifecycleTrustedLane = isTrustedRecentOwnerCriticalLane(recentOwnerMissileLifecycleState.recentLane, recentOwnerMissileLifecycleState.rawDispatchDelta, allowFarAheadRecentOwnerMissileLifecycleLane)
        ? recentOwnerMissileLifecycleState.recentLane
        : null;
    const recentOwnerFreshAcquireTrustedLane = isTrustedRecentOwnerCriticalLane(recentOwnerFreshAcquireState.recentLane, recentOwnerFreshAcquireState.rawDispatchDelta)
        ? recentOwnerFreshAcquireState.recentLane
        : null;
    const recentOverallOwnerCriticalTrustedLane = (previousLastSentDestinyMatchesTrackedOwnerCriticalLane === true ||
        allowUntrackedRecentOverallOwnerLaneTrust === true) &&
        isTrustedRecentOwnerCriticalLane(recentOverallOwnerCriticalState.recentLane, recentOverallOwnerCriticalState.rawDispatchDelta)
        ? recentOverallOwnerCriticalState.recentLane
        : null;
    const projectedOwnerMissileLifecycleLane = recentOwnerMissileLifecycleState.projectedConsumedLane !== null &&
        ...
        (!hasOwnerShip ||
            isDestinyStampWithinForwardWindow(currentSessionStamp, recentOwnerMissileLifecycleState.laneStamp, recentEmittedOwnerCriticalMaxLead) ||
        ...
        : null;
    const projectedOwnerFreshAcquireLane = recentOwnerFreshAcquireState.projectedConsumedLane !== null &&
        recentOwnerFreshAcquireState.rawDispatchDelta > 0 &&
        recentOwnerFreshAcquireState.laneStamp !== null &&
        (!hasOwnerShip ||
            isDestinyStampWithinForwardWindow(currentSessionStamp, recentOwnerFreshAcquireState.laneStamp, recentEmittedOwnerCriticalMaxLead))
        ? recentOwnerFreshAcquireState.projectedConsumedLane
        : null;
    const projectedRecentOverallOwnerCriticalLane = recentOverallLastSentTrustedLane !== null &&
        hasProjectedRecentLastSentLane &&
        isDestinyStampAfter(recentOverallLastSentTrustedLane, projectedRecentLastSentLane) &&
        !(isFreshAcquireLifecycleGroup &&
            previousLastSentDestinyMatchesOwnerMissileLane === true &&
            !isDestinyStampWithinForwardWindow(currentSessionStamp, projectedRecentLastSentLane, recentEmittedOwnerCriticalMaxLead +
                MICHELLE_DIRECT_CRITICAL_ECHO_DESTINY_LEAD +
                1))
        ? projectedRecentLastSentLane
        : null;
    const reusableRecentOwnerCriticalLane = selectFurthestPresentDestinyStamp(currentSessionStamp, [
        recentPresentedLastSentLane,
    ...
    const projectedConsumedOwnerCriticalLane = selectFurthestPresentDestinyStamp(currentSessionStamp, [
    ...
    const requiresOwnerCriticalMonotonicFloor = isOwnerCriticalGroup ||
        isOwnerDamageStateGroup ||
        isOwnerMissileLifecycleGroup ||
        (isMissileLifecycleGroup &&
            isFreshAcquireLifecycleGroup !== true);
    const sameRawReusableOwnerMissileLifecycleLane = recentOwnerMissileLifecycleTrustedLane !== null &&
        recentOwnerMissileLifecycleState.rawDispatchStampPresent === true &&
        recentOwnerMissileLifecycleState.rawDispatchDelta === 0
        ? recentOwnerMissileLifecycleTrustedLane
        : null;
    const ownerMovementRawDispatchDelta = getRecentDestinyStampDelta(lastOwnerPilotCommandMovementRawDispatchStamp, currentRawDispatchStamp, 2);
    const nearbyOwnerMovementClearFloor = isFreshAcquireLifecycleGroup &&
        previousLastSentLaneIsRelevant &&
        previousLastSentDestinyStamp === lastOwnerPilotCommandMovementStamp &&
        isDestinyStampWithinRelativeWindow(currentSessionStamp, lastOwnerPilotCommandMovementStamp, -1, MICHELLE_HELD_FUTURE_DESTINY_LEAD) &&
        ...
        : null;
    const nearbyOwnerNonMissileCriticalClearFloor = previousLastSentLaneIsRelevant &&
        previousLastSentDestinyStamp === lastOwnerNonMissileCriticalStamp &&
        previousLastSentDestinyMatchesOwnerMissileLane !== true &&
        isDestinyStampWithinRelativeWindow(currentSessionStamp, lastOwnerNonMissileCriticalStamp, -1, MICHELLE_HELD_FUTURE_DESTINY_LEAD)
        ? advanceDestinyStamp(lastOwnerNonMissileCriticalStamp, 1)
        : null;
    const sameRawFreshAcquireReusableLane = selectFurthestPresentDestinyStamp(currentSessionStamp, [
    ...
    const projectedFreshAcquireReusableLane = isFreshAcquireLifecycleGroup &&
        allowAdjacentRawFreshAcquireLaneReuse === true &&
        projectedOwnerFreshAcquireLane !== null
        ? projectedOwnerFreshAcquireLane
        : null;
    const preserveOwnerDamageLifecycleFreshAcquireFloor = isFreshAcquireLifecycleGroup &&
        previousLastSentDestinyWasOwnerDamageLifecycleLane === true;
    const freshAcquireBufferedCeilingStamp = isFreshAcquireLifecycleGroup
        ? advanceDestinyStamp(currentImmediateSessionStamp, MICHELLE_HELD_FUTURE_DESTINY_LEAD)
        : null;
    const freshAcquireSameRawFarAheadTrustCeilingStamp = isFreshAcquireLifecycleGroup
        ? advanceDestinyStamp(freshAcquireBufferedCeilingStamp, 1)
        : null;
    const freshAcquireSameRawLifecycleClearCeilingStamp = isFreshAcquireLifecycleGroup
        ? advanceDestinyStamp(freshAcquireSameRawFarAheadTrustCeilingStamp, 1)
        : null;
    const filterFarAheadOwnerLaneForFreshAcquire = (lane, rawDispatchDelta = 0, reusableLane = null, sourceMatchesMissileLane = true) => (isFreshAcquireLifecycleGroup &&
        ...
        ? null
        : lane);
    const recentOwnerFreshAcquireFreshAcquireLane = filterFarAheadOwnerLaneForFreshAcquire(recentOwnerFreshAcquireTrustedLane, recentOwnerFreshAcquireState.rawDispatchDelta, sameRawFreshAcquireReusableLane);
    const recentOwnerCriticalFreshAcquireLane = filterFarAheadOwnerLaneForFreshAcquire(recentOwnerCriticalLane, recentOwnerCriticalRawDispatchDelta, sameRawFreshAcquireReusableLane, previousLastSentDestinyMatchesOwnerMissileLane);
    const recentOwnerMissileLifecycleFreshAcquireLane = preserveOwnerDamageLifecycleFreshAcquireFloor &&
        previousLastSentDestinyStamp === lastOwnerMissileLifecycleStamp
        ? recentOwnerMissileLifecycleTrustedLane
        : filterFarAheadOwnerLaneForFreshAcquire(recentOwnerMissileLifecycleTrustedLane, recentOwnerMissileLifecycleState.rawDispatchDelta, sameRawFreshAcquireReusableLane);
    const recentOverallLastSentFreshAcquireLane = filterFarAheadOwnerLaneForFreshAcquire(recentOverallLastSentTrustedLane, recentOverallLastSentState.rawDispatchDelta, sameRawFreshAcquireReusableLane, previousLastSentDestinyMatchesOwnerMissileLane &&
        previousLastSentDestinyWasOwnerDamageLifecycleLane !== true);
    const recentOverallOwnerCriticalFreshAcquireLane = filterFarAheadOwnerLaneForFreshAcquire(recentOverallOwnerCriticalTrustedLane, recentOverallOwnerCriticalState.rawDispatchDelta, sameRawFreshAcquireReusableLane, previousLastSentDestinyMatchesOwnerMissileLane &&
        previousLastSentDestinyWasOwnerDamageLifecycleLane !== true);
    const farAheadTrackedOwnerMissileLaneBlocksFreshAcquireClear = isFreshAcquireLifecycleGroup &&
        previousLastSentDestinyWasOwnerDamageLifecycleLane !== true &&
        previousLastSentDestinyMatchesOwnerMissileLane === true &&
        recentOverallLastSentTrustedLane !== null &&
        isDestinyStampAfter(freshAcquireSameRawLifecycleClearCeilingStamp, recentOverallLastSentTrustedLane);
    const presentedOwnerCriticalLead = currentPresentedOwnerCriticalStamp !== null
        ? getDestinyStampForwardDistance(currentImmediateSessionStamp, currentPresentedOwnerCriticalStamp)
        : null;
    const nearbyPresentedNonMissileOwnerCriticalLaneClearFloor = previousLastSentLaneIsRelevant &&
        previousLastSentDestinyWasOwnerCritical === true &&
        previousLastSentDestinyMatchesOwnerMissileLane !== true &&
        presentedOwnerCriticalLead !== null &&
        presentedOwnerCriticalLead <= maximumProjectedOwnerCriticalLead &&
        isDestinyStampWithinForwardWindow(currentImmediateSessionStamp, previousLastSentDestinyStamp, presentedOwnerCriticalLead)
        ? advanceDestinyStamp(previousLastSentDestinyStamp, 1)
        : null;
    const sameRawNearOwnerLaneClearFloor = recentOverallLastSentTrustedLane !== null &&
        recentOverallLastSentState.rawDispatchStampPresent === true &&
        recentOverallLastSentState.rawDispatchDelta === 0 &&
        previousLastSentDestinyWasOwnerCritical === true &&
        ...
        (sameRawReusableOwnerMissileLifecycleLane === null ||
            previousLastSentDestinyStamp === lastOwnerMissileFreshAcquireStamp ||
            (isFreshAcquireLifecycleGroup &&
                isDestinyStampWithinForwardWindow(currentImmediateSessionStamp, recentOverallLastSentTrustedLane, getDestinyStampForwardDistance(currentImmediateSessionStamp, freshAcquireSameRawLifecycleClearCeilingStamp)))) &&
        !(isFreshAcquireLifecycleGroup &&
            previousLastSentDestinyStamp === lastOwnerMissileFreshAcquireStamp)
        ? advanceDestinyStamp(recentOverallLastSentTrustedLane, 1)
        : null;
    const sameRawOwnerDamageLifecycleClearFloor = isFreshAcquireLifecycleGroup &&
        previousLastSentDestinyWasOwnerDamageLifecycleLane === true &&
        previousLastSentDestinyRawDispatchStamp === currentRawDispatchStamp
        ? advanceDestinyStamp(previousLastSentDestinyStamp, 1)
        : null;
    const ownerDamageStateRecentOwnerCriticalClearFloor = isOwnerDamageStateGroup === true &&
        reusableRecentOwnerCriticalLane !== null
        ? advanceDestinyStamp(reusableRecentOwnerCriticalLane, 2)
        : null;
    const canReuseProjectedFreshAcquireLane = projectedFreshAcquireReusableLane !== null &&
        isDestinyStampAfter(currentPresentedOwnerCriticalStamp, projectedFreshAcquireReusableLane) &&
        isDestinyStampAfter(currentLocalStamp, projectedFreshAcquireReusableLane);
    const recentOwnerCriticalContribution = recentOwnerCriticalLane !== null
        ? (isFreshAcquireLifecycleGroup
        ...
        : null;
    const freshAcquireRecentOwnerMovementClearContribution = isFreshAcquireLifecycleGroup && recentOwnerMovementTrustedLane !== null
        ? advanceDestinyStamp(recentOwnerMovementTrustedLane, 1)
        : null;
    const freshAcquireRecentOwnerMissileLifecycleClearContribution = isFreshAcquireLifecycleGroup &&
        ...
        : null;
    const freshAcquireRecentOwnerFreshAcquireClearContribution = isFreshAcquireLifecycleGroup &&
        recentOwnerFreshAcquireFreshAcquireLane !== null &&
        recentOwnerFreshAcquireState.rawDispatchDelta > 0
        ? advanceDestinyStamp(recentOwnerFreshAcquireFreshAcquireLane, 1)
        : null;
    const freshAcquireRecentOverallLastSentClearContribution = isFreshAcquireLifecycleGroup &&
        ...
        : null;
    const freshAcquireRecentOverallOwnerCriticalClearContribution = isFreshAcquireLifecycleGroup &&
        ...
        : null;
    const freshAcquireMaxClearContribution = isFreshAcquireLifecycleGroup
        ? selectFurthestPresentDestinyStamp(currentSessionStamp, [
    ...
    const projectedConsumedOwnerCriticalContribution = projectedConsumedOwnerCriticalLane !== null
        ? (isFreshAcquireLifecycleGroup &&
            projectedFreshAcquireReusableLane !== null &&
    ...
    const recentOwnerCriticalFloorCandidates = [
        buildDecisionCandidate("ownerDamageStateRecentOwnerCriticalClearFloor", ownerDamageStateRecentOwnerCriticalClearFloor),
        buildDecisionCandidate("sameRawOwnerDamageLifecycleClearFloor", sameRawOwnerDamageLifecycleClearFloor),
        buildDecisionCandidate("sameRawFreshAcquireReusableLane", isFreshAcquireLifecycleGroup ? sameRawFreshAcquireReusableLane : null),
        buildDecisionCandidate("nearbyOwnerMovementClearFloor", nearbyOwnerMovementClearFloor),
        buildDecisionCandidate("nearbyOwnerNonMissileCriticalClearFloor", nearbyOwnerNonMissileCriticalClearFloor),
        buildDecisionCandidate("nearbyPresentedNonMissileOwnerCriticalLaneClearFloor", nearbyPresentedNonMissileOwnerCriticalLaneClearFloor),
        buildDecisionCandidate("sameRawNearOwnerLaneClearFloor", sameRawNearOwnerLaneClearFloor),
        buildDecisionCandidate("recentOwnerCriticalContribution", recentOwnerCriticalContribution),
        buildDecisionCandidate("freshAcquireRecentOwnerMovementClearContribution", freshAcquireRecentOwnerMovementClearContribution),
        buildDecisionCandidate("freshAcquireRecentOwnerMissileLifecycleClearContribution", freshAcquireRecentOwnerMissileLifecycleClearContribution),
        buildDecisionCandidate("freshAcquireRecentOwnerFreshAcquireClearContribution", freshAcquireRecentOwnerFreshAcquireClearContribution),
        buildDecisionCandidate("freshAcquireRecentOverallLastSentClearContribution", freshAcquireRecentOverallLastSentClearContribution),
        buildDecisionCandidate("freshAcquireRecentOverallOwnerCriticalClearContribution", freshAcquireRecentOverallOwnerCriticalClearContribution),
        buildDecisionCandidate(isFreshAcquireLifecycleGroup
            ? "freshAcquireMaxClearContribution"
            : "reusableRecentOwnerCriticalLane", freshAcquireMaxClearContribution),
        buildDecisionCandidate("projectedConsumedOwnerCriticalContribution", projectedConsumedOwnerCriticalContribution),
    ];
    const recentOwnerCriticalMonotonicFloor = requiresOwnerCriticalMonotonicFloor
        ? selectFurthestPresentDestinyStamp(currentSessionStamp, [
        ...
        : null;
    const ownerMissileLifecycleCeilingStamp = isOwnerMissileLifecycleGroup
        ? selectFurthestPresentDestinyStamp(currentSessionStamp, [
            presentedLastSentCeilingCap,
            advanceDestinyStamp(currentSessionStamp, MICHELLE_HELD_FUTURE_DESTINY_LEAD),
        ], DESTINY_STAMP_MAX_FORWARD_LEAD)
        : null;
    const ownerCriticalBaseCeilingStamp = isOwnerDamageStateGroup
        ? presentedLastSentCeilingCap
        : isOwnerMissileLifecycleGroup
    ...
    const ownerCriticalCeilingStamp = (isOwnerCriticalGroup ||
        isOwnerDamageStateGroup)
        ? ownerCriticalBaseCeilingStamp
        : null;
    const reusableRecentOwnerCriticalLaneCandidates = summarizeDecisionCandidates([
        buildDecisionCandidate("recentPresentedLastSentLane", recentPresentedLastSentLane),
        buildDecisionCandidate("recentOverallLastSentTrustedLane", recentOverallLastSentTrustedLane),
        buildDecisionCandidate("recentOwnerMovementTrustedLane", recentOwnerMovementTrustedLane),
        buildDecisionCandidate("recentOwnerMissileLifecycleTrustedLane", recentOwnerMissileLifecycleTrustedLane),
        buildDecisionCandidate("recentOwnerFreshAcquireTrustedLane", recentOwnerFreshAcquireTrustedLane),
        buildDecisionCandidate("recentOverallOwnerCriticalTrustedLane", recentOverallOwnerCriticalTrustedLane),
    ], reusableRecentOwnerCriticalLane);
    const projectedConsumedOwnerCriticalLaneCandidates = summarizeDecisionCandidates([
        buildDecisionCandidate("projectedRecentOverallOwnerCriticalLane", projectedRecentOverallOwnerCriticalLane),
        buildDecisionCandidate("projectedOwnerMissileLifecycleLane", projectedOwnerMissileLifecycleLane),
        buildDecisionCandidate("projectedOwnerFreshAcquireLane", projectedOwnerFreshAcquireLane),
    ], projectedConsumedOwnerCriticalLane);
    const recentOwnerCriticalMonotonicFloorCandidatesSummary = summarizeDecisionCandidates(recentOwnerCriticalFloorCandidates, recentOwnerCriticalMonotonicFloor);
    const ownerCriticalCeilingCandidates = summarizeDecisionCandidates([
        buildDecisionCandidate("ownerCriticalBaseCeilingStamp", ownerCriticalBaseCeilingStamp),
        buildDecisionCandidate("recentOwnerCriticalMonotonicFloor", recentOwnerCriticalMonotonicFloor),
    ...
    const decisionTrace = {
        ...
        candidateGroups: {
            ...
            projectedConsumedOwnerCriticalLane: projectedConsumedOwnerCriticalLaneCandidates,
            recentOwnerCriticalMonotonicFloor: recentOwnerCriticalMonotonicFloorCandidatesSummary,
    ...
    const decisionSummary = {
        ...
        recentStates: [
            formatRecentOwnerLaneStateSummary("recentOverallLastSentState", recentOverallLastSentState),
            formatRecentOwnerLaneStateSummary("recentOwnerMovementState", recentOwnerMovementState),
            formatRecentOwnerLaneStateSummary("recentOwnerMissileLifecycleState", recentOwnerMissileLifecycleState),
            formatRecentOwnerLaneStateSummary("recentOwnerFreshAcquireState", recentOwnerFreshAcquireState),
            formatRecentOwnerLaneStateSummary("recentOverallOwnerCriticalState", recentOverallOwnerCriticalState),
        ],
        resolvedLanes: {
            reusableRecentOwnerCriticalLane,
            projectedConsumedOwnerCriticalLane,
        ...
        candidateGroups: {
            reusableRecentOwnerCriticalLane: formatDecisionCandidateSummary(reusableRecentOwnerCriticalLaneCandidates),
            projectedConsumedOwnerCriticalLane: formatDecisionCandidateSummary(projectedConsumedOwnerCriticalLaneCandidates),
            recentOwnerCriticalMonotonicFloor: formatDecisionCandidateSummary(recentOwnerCriticalMonotonicFloorCandidatesSummary),
            ownerCriticalCeilingStamp: formatDecisionCandidateSummary(ownerCriticalCeilingCandidates),
        },
    };
    return {
        maximumTrustedRecentEmittedOwnerCriticalStamp,
...
function resolveGotoCommandSyncState(options = {}) {
    ...
    const ownerLocallyPredictsHeading = options.ownerLocallyPredictsHeading === true;
    if (!hasRequiredDestinyStamps(options.liveOwnerSessionStamp, options.currentRawDispatchStamp)) {
        return {
    ...
    const pendingOwnerMovementStamp = resolveOptionalDestinyStamp(options.pendingOwnerMovementStamp);
    const liveOwnerSessionStamp = toDestinyStamp(options.liveOwnerSessionStamp);
    const pendingOwnerMovementRawDispatchStamp = resolveOptionalDestinyStamp(options.pendingOwnerMovementRawDispatchStamp);
    const currentRawDispatchStamp = toDestinyStamp(options.currentRawDispatchStamp);
    const hasPendingFutureOwnerSteer = pendingOwnerMovementStamp !== null &&
        isDestinyStampAfter(liveOwnerSessionStamp, pendingOwnerMovementStamp, DESTINY_STAMP_MAX_FORWARD_LEAD);
    const sameRawPendingFutureOwnerSteer = hasPendingFutureOwnerSteer &&
        pendingOwnerMovementRawDispatchStamp !== null &&
        currentRawDispatchStamp === pendingOwnerMovementRawDispatchStamp;
    const pendingOwnerMovementRawDispatchDelta = getRecentDestinyStampDelta(pendingOwnerMovementRawDispatchStamp, currentRawDispatchStamp, 1);
    return {
        isCurrentGotoDuplicate: !speedFractionChanged && options.currentGotoDirectionMatches === true,
        isPendingGotoDuplicate: !speedFractionChanged && options.pendingOwnerCommandDirectionMatches === true,
        ...
        suppressOwnerGotoEchoRecentDuplicate: ownerLocallyPredictsHeading &&
        ...
        suppressOwnerGotoEchoSameRawPendingFutureSteer: ownerLocallyPredictsHeading &&
            !speedFractionChanged &&
...
function resolveOwnerMovementRestampState(options = {}) {
    ...
    if (!hasRequiredDestinyStamps(options.currentRawDispatchStamp, options.liveOwnerSessionStamp, options.currentVisibleOwnerStamp, options.currentPresentedOwnerStamp)) {
        return {
    ...
    const currentPresentedOwnerStamp = toDestinyStamp(options.currentPresentedOwnerStamp);
    const readOptionalStamp = (key) => resolveOptionalDestinyStamp(options[key]);
    const previousLastSentDestinyWasOwnerCritical = options.previousLastSentDestinyWasOwnerCritical === true;
    const quietWindowMinimumStamp = readOptionalStamp("quietWindowMinimumStamp");
    const lastFreshAcquireLifecycleStamp = readOptionalStamp("lastFreshAcquireLifecycleStamp");
    const lastOwnerNonMissileCriticalStamp = readOptionalStamp("lastOwnerNonMissileCriticalStamp");
    const lastOwnerNonMissileCriticalRawDispatchStamp = readOptionalStamp("lastOwnerNonMissileCriticalRawDispatchStamp");
    const lastOwnerMissileLifecycleStamp = readOptionalStamp("lastOwnerMissileLifecycleStamp");
    const lastOwnerMissileLifecycleRawDispatchStamp = readOptionalStamp("lastOwnerMissileLifecycleRawDispatchStamp");
    const lastOwnerMissileFreshAcquireStamp = readOptionalStamp("lastOwnerMissileFreshAcquireStamp");
    const lastOwnerMissileFreshAcquireRawDispatchStamp = readOptionalStamp("lastOwnerMissileFreshAcquireRawDispatchStamp");
    const previousOwnerPilotCommandStamp = readOptionalStamp("previousOwnerPilotCommandStamp");
    const previousOwnerPilotCommandAnchorStamp = readOptionalStamp("previousOwnerPilotCommandAnchorStamp");
    const previousOwnerPilotCommandRawDispatchStamp = readOptionalStamp("previousOwnerPilotCommandRawDispatchStamp");
    const previousOwnerPilotCommandDirectionRaw = options.previousOwnerPilotCommandDirectionRaw;
    const normalizeVector = typeof options.normalizeVector === "function"
        ? options.normalizeVector
        : (vector, fallback) => {
            const base = vector && typeof vector === "object" ? vector : fallback || { x: 1, y: 0, z: 0 };
    ...
    const directionsNearlyMatch = typeof options.directionsNearlyMatch === "function"
        ...
        : (left, right, minimumDot = OWNER_PENDING_GOTO_DUPLICATE_ALIGNMENT) => {
            ...
            const dot = (normalizedLeft.x * normalizedRight.x) +
                (normalizedLeft.y * normalizedRight.y) +
                (normalizedLeft.z * normalizedRight.z);
            return dot >= minimumDot;
        };
    const getPendingHistorySafeStamp = typeof options.getPendingHistorySafeStamp === "function"
        ? options.getPendingHistorySafeStamp
        : (authoredStamp) => toDestinyStamp(authoredStamp);
    const defaultRight = options.defaultRight && typeof options.defaultRight === "object"
    ...
    const ownerDirectEchoLead = Math.max(0, toInt(options.ownerDirectEchoLeadOverride, MICHELLE_DIRECT_CRITICAL_ECHO_DESTINY_LEAD));
    const ownerDirectEchoBaseFloor = advanceDestinyStamp(liveOwnerSessionStamp, ownerDirectEchoLead);
    const ownerDirectEchoMinimumStamp = ownerMovementUpdates.length > 0
        ? selectFurthestPresentDestinyStamp(liveOwnerSessionStamp, [
            ownerDirectEchoBaseFloor,
            ...ownerMovementUpdates.flatMap((update) => {
                const authoredStamp = toDestinyStamp(update && update.stamp);
                const pendingHistorySafeStamp = getPendingHistorySafeStamp(authoredStamp, ownerDirectEchoLead);
                return [
                    authoredStamp,
        ...
        : null;
    const currentOwnerPilotCommandDirection = ownerMovementUpdates.reduce((latestDirection, update) => {
        ...
        return normalizeVector({
            x: toFiniteNumber(args[1] && typeof args[1] === "object" ? args[1].value : args[1], 0),
            y: toFiniteNumber(args[2] && typeof args[2] === "object" ? args[2].value : args[2], 0),
            z: toFiniteNumber(args[3] && typeof args[3] === "object" ? args[3].value : args[3], 0),
        }, latestDirection || defaultRight);
    }, null);
    const previousOwnerPilotCommandDirection = ownerHasSteeringCommand &&
        isDestinyStampWithinRelativeWindow(liveOwnerSessionStamp, previousOwnerPilotCommandStamp, 0, DESTINY_STAMP_MAX_FORWARD_LEAD) &&
        previousOwnerPilotCommandAnchorStamp !== null &&
        previousOwnerPilotCommandDirectionRaw
        ? normalizeVector(previousOwnerPilotCommandDirectionRaw, currentOwnerPilotCommandDirection || defaultRight)
        : null;
    const repeatedOwnerPilotCommandLane = ownerHasSteeringCommand &&
        isDestinyStampAfter(liveOwnerSessionStamp, previousOwnerPilotCommandStamp, DESTINY_STAMP_MAX_FORWARD_LEAD) &&
        ...
        directionsNearlyMatch(previousOwnerPilotCommandDirection, currentOwnerPilotCommandDirection, OWNER_PENDING_GOTO_DUPLICATE_ALIGNMENT)
    ...
    const ownerMovementPlanningAnchorStamp = advanceDestinyStamp(liveOwnerSessionStamp, -1);
    const ownerPilotCommandReuseFloor = selectFurthestPresentDestinyStamp(ownerMovementPlanningAnchorStamp, [currentVisibleOwnerStamp, currentPresentedOwnerStamp], DESTINY_STAMP_MAX_FORWARD_LEAD + 1);
    const isStampAtOrAfterFloor = (candidateStamp, floorStamp, maximumLead = DESTINY_STAMP_MAX_FORWARD_LEAD, anchorStamp = liveOwnerSessionStamp) => {
        if (!hasDestinyStamp(candidateStamp) || !hasDestinyStamp(floorStamp)) {
            return false;
        }
        const candidateDistance = getDestinyStampForwardDistance(anchorStamp, candidateStamp);
    ...
    const previousOwnerPilotCommandRawDispatchDelta = getRecentDestinyStampDelta(previousOwnerPilotCommandRawDispatchStamp, currentRawDispatchStamp, 1);
    ...
    // visible lane.
    const reusableHeldOwnerPilotCommandLane = ownerHasSteeringCommand &&
        ownerDirectEchoLead > MICHELLE_DIRECT_CRITICAL_ECHO_DESTINY_LEAD &&
        isStampAtOrAfterFloor(previousOwnerPilotCommandStamp, ownerPilotCommandReuseFloor, DESTINY_STAMP_MAX_FORWARD_LEAD + 1, ownerMovementPlanningAnchorStamp) &&
        ...
        previousOwnerPilotCommandDirection &&
        currentOwnerPilotCommandDirection &&
        (isDestinyStampAfter(currentPresentedOwnerStamp, previousOwnerPilotCommandStamp, DESTINY_STAMP_MAX_FORWARD_LEAD) ||
            directionsNearlyMatch(previousOwnerPilotCommandDirection, currentOwnerPilotCommandDirection, OWNER_PENDING_GOTO_DUPLICATE_ALIGNMENT)) &&
    ...
    //   owner steer
    const nextDistinctOwnerPilotCommandLane = ownerHasSteeringCommand &&
        isDestinyStampAfter(liveOwnerSessionStamp, previousOwnerPilotCommandStamp, DESTINY_STAMP_MAX_FORWARD_LEAD) &&
        previousOwnerPilotCommandRawDispatchDelta === 1 &&
        previousOwnerPilotCommandDirection &&
        currentOwnerPilotCommandDirection &&
        ...
        ? advanceDestinyStamp(previousOwnerPilotCommandStamp, 1)
        : null;
    const suppressSameRawDistinctFutureOwnerEcho = ownerHasSteeringCommand &&
        isStampAtOrAfterFloor(previousOwnerPilotCommandStamp, ownerPilotCommandReuseFloor, DESTINY_STAMP_MAX_FORWARD_LEAD + 1, ownerMovementPlanningAnchorStamp) &&
        ...
        currentOwnerPilotCommandDirection &&
        repeatedOwnerPilotCommandLane === null;
    const earlierTickOwnerPilotCommandMatches = previousOwnerPilotCommandDirection &&
        currentOwnerPilotCommandDirection &&
        directionsNearlyMatch(previousOwnerPilotCommandDirection, currentOwnerPilotCommandDirection, OWNER_PENDING_GOTO_DUPLICATE_ALIGNMENT);
    ...
    const recentPresentedFreshAcquireLane = ownerHasSteeringCommand &&
        isDestinyStampWithinRelativeWindow(currentVisibleOwnerStamp, lastFreshAcquireLifecycleStamp, 1, MICHELLE_HELD_FUTURE_DESTINY_LEAD)
        ? lastFreshAcquireLifecycleStamp
        : null;
    const postFreshAcquireOwnerSteeringFloor = recentPresentedFreshAcquireLane !== null
        ? selectFurthestPresentDestinyStamp(liveOwnerSessionStamp, [
            ownerDirectEchoMinimumStamp,
    ...
    const recentBufferedOwnerCriticalFloor = selectFurthestPresentDestinyStamp(liveOwnerSessionStamp, [
    ...
    const presentedNonCriticalOwnerEchoFloor = ownerHasSteeringCommand &&
        ownerDirectEchoLead > MICHELLE_DIRECT_CRITICAL_ECHO_DESTINY_LEAD &&
        previousLastSentDestinyWasOwnerCritical !== true &&
        isDestinyStampAfter(liveOwnerSessionStamp, currentPresentedOwnerStamp, DESTINY_STAMP_MAX_FORWARD_LEAD)
        ? advanceDestinyStamp(selectFurthestPresentDestinyStamp(liveOwnerSessionStamp, [
            ownerDirectEchoMinimumStamp,
            clampDestinyStampToCeiling(liveOwnerSessionStamp, currentPresentedOwnerStamp, advanceDestinyStamp(liveOwnerSessionStamp, ownerDirectEchoLead)),
    ...
    const ownerMinimumStamp = ownerMovementUpdates.reduce((highestMinimumStamp, update) => {
        ...
        if (quietWindowMinimumStamp !== null) {
            return selectFurthestPresentDestinyStamp(ownerMovementPlanningAnchorStamp, [highestMinimumStamp, quietWindowMinimumStamp], DESTINY_STAMP_MAX_FORWARD_LEAD + 1);
        }
        if (!ownerHasSteeringCommand) {
            return highestMinimumStamp;
        }
        if (isStampAtOrAfterFloor(authoredStamp, ownerVisibleStamp, DESTINY_STAMP_MAX_FORWARD_LEAD + 1, ownerMovementPlanningAnchorStamp)) {
            return highestMinimumStamp;
        }
        const pendingHistorySafeStamp = getPendingHistorySafeStamp(authoredStamp, 0);
        return selectFurthestPresentDestinyStamp(ownerMovementPlanningAnchorStamp, [
            highestMinimumStamp,
            ownerVisibleStamp,
            hasDestinyStamp(pendingHistorySafeStamp)
                ? toDestinyStamp(pendingHistorySafeStamp)
    ...
    // of behind it.
    const ownerStampFloor = reusableHeldOwnerPilotCommandLane !== null
        ? selectFurthestPresentDestinyStamp(ownerMovementPlanningAnchorStamp, [
            ownerMinimumStamp,
    ...
    const ownerUpdates = ownerMovementUpdates.map((update) => ({
        ...update,
        stamp: selectFurthestPresentDestinyStamp(ownerMovementPlanningAnchorStamp, [toDestinyStamp(update && update.stamp), ownerStampFloor], DESTINY_STAMP_MAX_FORWARD_LEAD + 1),
    }));
    return {
        currentOwnerPilotCommandDirection,
        previousOwnerPilotCommandDirection,
...
function resolveDestinyLifecycleRestampState(options = {}) {
    ...
    const isMissileLifecycleGroup = options.isMissileLifecycleGroup === true;
    const isOwnerMissileLifecycleGroup = options.isOwnerMissileLifecycleGroup === true;
    const minimumPostFreshAcquireStamp = hasDestinyStamp(options.minimumPostFreshAcquireStamp)
    ...
    const localStamp = hasDestinyStamp(options.localStamp)
        ...
        : null;
    if (localStamp === null ||
        !hasRequiredDestinyStamps(options.currentSessionStamp, options.currentImmediateSessionStamp, options.currentRawDispatchStamp)) {
        return {
    ...
    const readOptionalStamp = (key) => resolveOptionalDestinyStamp(options[key]);
    const lastFreshAcquireLifecycleStamp = readOptionalStamp("lastFreshAcquireLifecycleStamp");
    const lastMissileLifecycleStamp = readOptionalStamp("lastMissileLifecycleStamp");
    const lastOwnerMissileLifecycleStamp = readOptionalStamp("lastOwnerMissileLifecycleStamp");
    const lastOwnerMissileFreshAcquireStamp = readOptionalStamp("lastOwnerMissileFreshAcquireStamp");
    const lastOwnerMissileFreshAcquireRawDispatchStamp = readOptionalStamp("lastOwnerMissileFreshAcquireRawDispatchStamp");
    const lastOwnerMissileLifecycleRawDispatchStamp = readOptionalStamp("lastOwnerMissileLifecycleRawDispatchStamp");
    ...
    const maximumTrustedOwnerMovementLane = advanceDestinyStamp(currentSessionStamp, MICHELLE_HELD_FUTURE_DESTINY_LEAD);
    const minimumTrustedOwnerMovementLane = advanceDestinyStamp(currentSessionStamp, -1);
    const ownerMovementRawDelta = getRecentDestinyStampDelta(lastOwnerPilotCommandMovementRawDispatchStamp, currentRawDispatchStamp, 2);
    const recentOwnerMovementLane = isDestinyStampWithinRelativeWindow(currentSessionStamp, lastOwnerPilotCommandMovementStamp, -1, MICHELLE_HELD_FUTURE_DESTINY_LEAD) && (ownerMovementRawDelta !== null ||
        lastOwnerPilotCommandMovementRawDispatchStamp === null)
        ? lastOwnerPilotCommandMovementStamp
        : null;
    const recentOwnerMissileLifecycleLane = getRecentTrustedLane({
    ...
    const previousLastSentDestinyMatchesKnownOwnerCriticalLane = previousLastSentDestinyStamp !== null &&
        (previousLastSentDestinyWasOwnerCritical === true ||
            previousLastSentDestinyStamp === lastOwnerPilotCommandMovementStamp ||
            previousLastSentDestinyStamp === lastOwnerMissileLifecycleStamp ||
            previousLastSentDestinyStamp === lastOwnerMissileFreshAcquireStamp);
    const recentOverallOwnerCriticalLane = previousLastSentDestinyMatchesKnownOwnerCriticalLane
        ? getRecentTrustedLane({
            laneStamp: previousLastSentDestinyStamp,
            currentStamp: currentImmediateSessionStamp,
            currentRawDispatchStamp,
        ...
        : null;
    const recentOwnerCriticalFloor = selectFurthestPresentDestinyStamp(currentImmediateSessionStamp, [
    ...
    if (isFreshAcquireLifecycleGroup) {
        const reusableFreshAcquireLane = isDestinyStampWithinRelativeWindow(currentSessionStamp, lastFreshAcquireLifecycleStamp, 1, MICHELLE_HELD_FUTURE_DESTINY_LEAD)
            ? lastFreshAcquireLifecycleStamp
            : null;
        const ownerFreshAcquireHeldCeiling = isOwnerMissileLifecycleGroup
            ? advanceDestinyStamp(currentSessionStamp, MICHELLE_HELD_FUTURE_DESTINY_LEAD)
            : null;
        ...
        const ownerSameRawFreshAcquireClearFloor = isOwnerMissileLifecycleGroup &&
            reusableFreshAcquireLane !== null &&
            previousLastSentDestinyStamp === reusableFreshAcquireLane &&
            previousLastSentDestinyRawDispatchStamp === currentRawDispatchStamp
            ? clampDestinyStampToCeiling(currentSessionStamp, advanceDestinyStamp(reusableFreshAcquireLane, 1), ownerFreshAcquireHeldCeiling)
            : null;
        const freshAcquireHistorySafeFloor = advanceDestinyStamp(isOwnerMissileLifecycleGroup
            ? currentImmediateSessionStamp
            : currentSessionStamp, MICHELLE_HELD_FUTURE_DESTINY_LEAD);
        const resolvedFreshAcquireFloor = reusableFreshAcquireLane !== null &&
    ...
    if (isMissileLifecycleGroup && !isOwnerMissileLifecycleGroup) {
        ...
        const reusableMissileLifecycleLane = isDestinyStampWithinRelativeWindow(currentSessionStamp, lastMissileLifecycleStamp, 1, MICHELLE_HELD_FUTURE_DESTINY_LEAD)
            ? lastMissileLifecycleStamp
            : null;
        const resolvedMissileLifecycleFloor = reusableMissileLifecycleLane !== null
    ...
    if (isOwnerMissileLifecycleGroup) {
        const requiredOwnerFloor = selectFurthestPresentDestinyStamp(currentImmediateSessionStamp, [
            minimumPostFreshAcquireStamp,
            advanceDestinyStamp(currentImmediateSessionStamp, MICHELLE_HELD_FUTURE_DESTINY_LEAD),
            recentOwnerCriticalFloor,
        ], DESTINY_STAMP_MAX_FORWARD_LEAD);
        const resolvedOwnerMissileLifecycleFloor = requiredOwnerFloor;
        const normalizedOwnerMissileStamp = selectFurthestPresentDestinyStamp(currentImmediateSessionStamp, [workingLocalStamp, resolvedOwnerMissileLifecycleFloor], DESTINY_STAMP_MAX_FORWARD_LEAD);
...
function resolveDamageStateDispatchStamp(options = {}) {
    ...
    const currentPresentedStamp = hasDestinyStamp(options.currentPresentedStamp)
        ...
        : null;
    const previousLastSentDestinyStamp = hasDestinyStamp(options.previousLastSentDestinyStamp) ? toDestinyStamp(options.previousLastSentDestinyStamp) : null;
    const previousLastSentDestinyRawDispatchStamp = hasDestinyStamp(options.previousLastSentDestinyRawDispatchStamp) ? toDestinyStamp(options.previousLastSentDestinyRawDispatchStamp) : null;
    const currentRawDispatchStamp = toDestinyStamp(options.currentRawDispatchStamp);
    const directCriticalEchoStamp = advanceDestinyStamp(visibleStamp, MICHELLE_DIRECT_CRITICAL_ECHO_DESTINY_LEAD);
    const maximumHeldFutureDamageStamp = advanceDestinyStamp(visibleStamp, MICHELLE_HELD_FUTURE_DESTINY_LEAD);
    ...
    // reusing `visible + 1`.
    const presentedDamageClearFloor = currentPresentedStamp !== null &&
        isDestinyStampAfter(directCriticalEchoStamp, currentPresentedStamp)
        ? clampDestinyStampToCeiling(visibleStamp, currentPresentedStamp, maximumHeldFutureDamageStamp)
        : null;
    const sameRawPresentedDamageReuseClearFloor = previousLastSentDestinyStamp !== null &&
        currentPresentedStamp !== null &&
        previousLastSentDestinyStamp === currentPresentedStamp &&
        previousLastSentDestinyRawDispatchStamp !== null &&
        previousLastSentDestinyRawDispatchStamp === currentRawDispatchStamp
        ? clampDestinyStampToCeiling(visibleStamp, advanceDestinyStamp(currentPresentedStamp, 1), maximumHeldFutureDamageStamp)
        : null;
    ...
    const projectedPresentedDamageClearFloor = previousLastSentDestinyStamp !== null &&
        currentPresentedStamp !== null &&
        previousLastSentDestinyStamp === currentPresentedStamp &&
        previousLastSentDestinyRawDispatchStamp !== null &&
        getRecentDestinyStampDelta(previousLastSentDestinyRawDispatchStamp, currentRawDispatchStamp, 1) === 1
        ? projectPreviouslySentDestinyLane(previousLastSentDestinyStamp, previousLastSentDestinyRawDispatchStamp, currentRawDispatchStamp)
        : null;
    const finalStamp = selectFurthestPresentDestinyStamp(visibleStamp, [
