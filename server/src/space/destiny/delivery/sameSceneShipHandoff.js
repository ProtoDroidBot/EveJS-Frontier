...
const { DESTINY_CONTRACTS, } = require("../authority/destinyContracts");
const { entityIDsEqual, normalizeEntityID, requireEntityID, } = require("../identity/entityID");
const { createDestinyDeliveryTransaction, isDestinyDeliveryTransactionGenerationCurrent, } = require("./deliveryTransaction");
const { buildDestructionTeardownSendOptions, } = require("./sendOptions");
const { advanceDestinyStamp, hasDestinyStamp, normalizeDestinyStamp, } = require("./stamps");
const FOLLOWING_TEARDOWN_CAPABILITY_BRAND = Symbol("destiny.followingTeardownCapability");
const FOLLOWING_TEARDOWN_HANDOFF_CAPABILITY = Symbol("destiny.followingTeardownHandoffCapability");
const SAME_SCENE_SHIP_HANDOFF_CAPABILITY = Symbol("destiny.sameSceneShipHandoffCapability");
const SAME_SCENE_SHIP_HANDOFF_CAPABILITY_BRAND = Symbol("destiny.sameSceneShipHandoffCapabilityBrand");
const SAME_SCENE_SHIP_HANDOFF_AUTHORED_ACTION_FINGERPRINT = Symbol("destiny.sameSceneShipHandoffAuthoredActionFingerprint");
const SAME_SCENE_SHIP_HANDOFF_AUTHORED_ACTION_PAYLOADS = Symbol("destiny.sameSceneShipHandoffAuthoredActionPayloads");
const SAME_SCENE_SHIP_HANDOFF_DELIVERY_SCOPE = Symbol("destiny.sameSceneShipHandoffDeliveryScope");
const SAME_SCENE_SHIP_HANDOFF_SCOPE_BINDING = Symbol("destiny.sameSceneShipHandoffScopeBinding");
const SAME_SCENE_SHIP_HANDOFF_TRANSACTION_PROOF = Symbol("destiny.sameSceneShipHandoffTransactionProof");
...
function freezeSameSceneShipHandoffAuthoredValue(value, visiting = new WeakSet(), frozen = new WeakSet()) {
    if (value === null || typeof value !== "object") {
        if (typeof value === "function" || typeof value === "symbol") {
            throw new TypeError("same-scene ship handoff actions must be wire-shaped values");
        }
        return value;
    }
    if (frozen.has(value)) {
    ...
    if (visiting.has(value)) {
        throw new TypeError("same-scene ship handoff actions must not contain cycles");
    }
    const prototype = Object.getPrototypeOf(value);
    if (!Array.isArray(value) &&
        prototype !== Object.prototype &&
        prototype !== null) {
        throw new TypeError("same-scene ship handoff actions must use plain wire objects");
    }
    if (Object.getOwnPropertySymbols(value).length > 0) {
        throw new TypeError("same-scene ship handoff actions must not contain symbol fields");
    }
    visiting.add(value);
    for (const key of Reflect.ownKeys(value)) {
        ...
        if (!descriptor ||
            ...
            throw new TypeError("same-scene ship handoff actions must use plain data fields");
        }
        if (key !== "length") {
            freezeSameSceneShipHandoffAuthoredValue(descriptor.value, visiting, frozen);
        }
    }
...
function buildSameSceneShipHandoffActionFingerprint(updates, authoredStamp) {
    ...
    try {
        const actionGroup = updates.map((update) => {
            ...
            const hasAuthoredStamp = Object.prototype.hasOwnProperty.call(update, "authoredStamp");
...
function getSameSceneShipHandoffCapability(sendOptions = {}) {
    const capability = sendOptions && typeof sendOptions === "object"
        ? sendOptions[SAME_SCENE_SHIP_HANDOFF_CAPABILITY]
        : null;
    return capability &&
        typeof capability === "object" &&
        capability[SAME_SCENE_SHIP_HANDOFF_CAPABILITY_BRAND] === true &&
        typeof capability[SAME_SCENE_SHIP_HANDOFF_AUTHORED_ACTION_FINGERPRINT] === "string" &&
        Array.isArray(capability[SAME_SCENE_SHIP_HANDOFF_AUTHORED_ACTION_PAYLOADS])
        ? capability
        : null;
}
function getFollowingTeardownCapability(sendOptions = {}) {
    ...
    let entityID;
    try {
        entityID = requireEntityID(capability.entityID, "followingTeardown.entityID");
    }
    catch (_error) {
        return null;
    }
    const handoffCapability = capability[FOLLOWING_TEARDOWN_HANDOFF_CAPABILITY];
...
function buildSameSceneShipHandoffSendOptions(baseOptions = {}, details = {}) {
    ...
    const authoredActionFingerprint = buildSameSceneShipHandoffActionFingerprint(details.updates, details.stamp);
    if (!authoredActionFingerprint) {
        throw new TypeError("same-scene ship handoff requires exact immutable authored actions");
    }
    const authoredActionPayloads = Object.freeze(details.updates.map((update) => update.payload));
    const capability = {
        authoredStamp: normalizeDestinyStamp(details.stamp),
        previousEntityID: requireEntityID(details.previousEntityID, "sameSceneShipHandoff.previousEntityID"),
        boardedEntityID: requireEntityID(details.boardedEntityID, "sameSceneShipHandoff.boardedEntityID"),
    };
...
function buildFollowingTeardownSendOptions(options = {}) {
    ...
    const handoffCapability = getSameSceneShipHandoffCapability(options.handoffSendOptions);
    if (!handoffCapability) {
        throw new TypeError("following teardown requires same-scene handoff provenance");
    }
    const entityID = requireEntityID(options.entityID, "followingTeardown.entityID");
    if (!entityIDsEqual(entityID, handoffCapability.previousEntityID)) {
        throw new TypeError("following teardown entity must match the same-scene handoff");
    }
    const capability = {
...
function isLiveSameSceneShipHandoffScopeDetails(details) {
    const rootTransaction = resolveRootDestinyDeliveryTransaction(details && details.transaction);
...
function getSameSceneShipHandoffScopeDetails(scope) {
    const details = scope && typeof scope === "object"
...
function getSameSceneShipHandoffScopeBinding(sendOptions) {
    const details = sendOptions && typeof sendOptions === "object"
...
function createSameSceneShipHandoffDeliveryScope(session, handoffSendOptions) {
    const handoffCapability = getSameSceneShipHandoffCapability(handoffSendOptions);
    if (!handoffCapability || !session || !session._space) {
        throw new TypeError("same-scene ship handoff delivery requires one live generation");
    }
...
function bindSameSceneShipHandoffDeliveryDetails(capabilitySource, enumerableOptions, details) {
    const sourceHandoffCapability = resolveSendOptionsHandoffCapability(capabilitySource);
    if (!details ||
        !sourceHandoffCapability ||
        sourceHandoffCapability !== details.handoffCapability ||
        !isLiveSameSceneShipHandoffScopeDetails(details)) {
        throw new TypeError("same-scene ship handoff delivery scope does not match send options");
    }
    const nextOptions = {
    ...
    if (getSameSceneShipHandoffCapability(capabilitySource)) {
        ...
        if (!followingCapability ||
            followingCapability.handoffCapability !== sourceHandoffCapability) {
            throw new TypeError("same-scene ship handoff teardown provenance was replaced");
        }
    }
...
function bindSameSceneShipHandoffDeliveryScope(sendOptions, scope, overrides = {}) {
    return bindSameSceneShipHandoffDeliveryDetails(sendOptions, {
        ...(sendOptions && typeof sendOptions === "object" ? sendOptions : {}),
        ...(overrides && typeof overrides === "object" ? overrides : {}),
    }, getSameSceneShipHandoffScopeDetails(scope));
}
function inheritSameSceneShipHandoffDeliveryScope(sourceOptions, targetOptions) {
    const details = getSameSceneShipHandoffScopeBinding(sourceOptions);
    if (!details) {
        if (sourceOptions &&
            typeof sourceOptions === "object" &&
            Object.prototype.hasOwnProperty.call(sourceOptions, SAME_SCENE_SHIP_HANDOFF_SCOPE_BINDING)) {
            throw new TypeError("same-scene ship handoff delivery scope is no longer live");
        }
        return targetOptions;
    }
    return bindSameSceneShipHandoffDeliveryDetails(sourceOptions, targetOptions, details);
}
...
function isExactSameSceneShipHandoffGroup(updates, capability) {
    ...
    const authoredActionPayloads = capability[SAME_SCENE_SHIP_HANDOFF_AUTHORED_ACTION_PAYLOADS];
    if (!Array.isArray(authoredActionPayloads) ||
    ...
    return Number(getPayloadArguments(updates[0])[1]) === 0 &&
        ...
        buildSameSceneShipHandoffActionFingerprint(updates, capability.authoredStamp) === capability[SAME_SCENE_SHIP_HANDOFF_AUTHORED_ACTION_FINGERPRINT];
}
...
function recordSameSceneShipHandoffTransactionProof(options = {}) {
    const { deliveryTransaction, finalStamp, originalStamp, sendOptions, updates, } = options;
    ...
    const rootDeliveryTransaction = resolveRootDestinyDeliveryTransaction(deliveryTransaction);
    const rootBindingTransaction = resolveRootDestinyDeliveryTransaction(binding && binding.transaction);
    if (!capability ||
        !binding ||
    ...
    Object.defineProperty(binding.transaction, SAME_SCENE_SHIP_HANDOFF_TRANSACTION_PROOF, {
        configurable: false,
        enumerable: false,
        value: proof,
        writable: false,
...
function authorizeFollowingTeardownTransaction(options = {}) {
    const { deliveryTransaction, originalStamp, previousLastSentDestinyStamp, sendOptions, updates, } = options;
    const capability = getFollowingTeardownCapability(sendOptions);
    const binding = getSameSceneShipHandoffScopeBinding(sendOptions);
    const rootDeliveryTransaction = resolveRootDestinyDeliveryTransaction(deliveryTransaction);
    const rootBindingTransaction = resolveRootDestinyDeliveryTransaction(binding && binding.transaction);
    const proof = binding &&
        binding.transaction[SAME_SCENE_SHIP_HANDOFF_TRANSACTION_PROOF];
    const transactionProofs = getSameSceneShipHandoffTransactionProofs(rootDeliveryTransaction);
...
//# sourceMappingURL=sameSceneShipHandoff.js.map