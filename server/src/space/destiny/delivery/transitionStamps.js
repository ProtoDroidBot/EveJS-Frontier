...
const { snapshotDestinyAuthorityState, } = require("../authority/destinySessionState");
const { restoreAuthorityDeliveryState, snapshotAuthorityDeliveryState, } = require("./deliveryTransaction");
const { advanceDestinyStamp, hasDestinyStamp, normalizeDestinyStamp, selectLaterDestinyStamp, } = require("./stamps");
...
function createSameSceneDestinyPresentationAuthorityHandoff(scene, session) {
    ...
    const visibilitySnapshot = {
        visibleDynamicEntityIDs: new Set(session._space.visibleDynamicEntityIDs instanceof Set
            ? session._space.visibleDynamicEntityIDs
            : []),
        visibleBubbleScopedStaticEntityIDs: new Set(session._space.visibleBubbleScopedStaticEntityIDs instanceof Set
            ? session._space.visibleBubbleScopedStaticEntityIDs
            : []),
        freshlyVisibleDynamicEntityIDs: new Set(session._space.freshlyVisibleDynamicEntityIDs instanceof Set
            ? session._space.freshlyVisibleDynamicEntityIDs
            : []),
        freshlyVisibleDynamicEntityReleaseStampByID: new Map(session._space.freshlyVisibleDynamicEntityReleaseStampByID instanceof Map
    ...
    return Object.freeze({
        restore(targetScene, targetSession) {
            ...
            const restored = restoreAuthorityDeliveryState(targetSession, authoritySnapshot);
            if (restored) {
                ...
                targetSession._space.visibleDynamicEntityIDs = new Set(visibilitySnapshot.visibleDynamicEntityIDs);
                targetSession._space.visibleBubbleScopedStaticEntityIDs = new Set(visibilitySnapshot.visibleBubbleScopedStaticEntityIDs);
                targetSession._space.freshlyVisibleDynamicEntityIDs = new Set(visibilitySnapshot.freshlyVisibleDynamicEntityIDs);
                targetSession._space.freshlyVisibleDynamicEntityReleaseStampByID =
                    new Map(visibilitySnapshot.freshlyVisibleDynamicEntityReleaseStampByID);
...
function resolveSameSceneEgoAddBallsStamp(scene, session, nowMs = null) {
    ...
    const resolvedNowMs = toFiniteNumber(nowMs, typeof scene.getCurrentSimTimeMs === "function"
        ? scene.getCurrentSimTimeMs()
        : 0);
    const rawCurrentPresentedStamp = typeof scene.getCurrentPresentedSessionDestinyStamp === "function"
    ...
    const rawLastSentStamp = hasDestinyStamp(authorityState && authorityState.lastPresentedStamp)
...
//# sourceMappingURL=transitionStamps.js.map