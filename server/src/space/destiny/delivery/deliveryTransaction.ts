...
function createDestinyDeliveryTransaction(session, options: Record<string, any> = {}) {
  ...
  const transaction = {
    ...
    stagedState: null,
    commitCallbacks: new Set<any>(),
    rollbackCallbacks: new Set<any>(),
    retryCallbacks: new Set<any>(),
    retryCount: 0,
    deliveryCount: 0,
    failure: null,
    mergedTransactions: new Set<any>(),
  };
...
function resolveAcceptedDeliveryCount(details: Record<string, any> = {}) {
  const rawCount = details.deliveredCount ?? details.deliveryCount ?? 0;
  const count = Number(rawCount);
  return Number.isFinite(count) ? Math.max(0, Math.trunc(count)) : 0;
}

function finalizeMergedTransactions(transaction, state, details: Record<string, any> = {}) {
...
function registerDestinyDeliveryHooks(transaction, options: Record<string, any> = {}) {
...
function commitDestinyDeliveryTransaction(transaction, details: Record<string, any> = {}) {
...
function retryDestinyDeliveryTransaction(transaction, details: Record<string, any> = {}) {
...
function rollbackDestinyDeliveryTransaction(transaction, details: Record<string, any> = {}) {
