...
function resolveDelayedDestinyStampState(options: Record<string, any> = {}) {
...
function resolvePreviousLastSentDestinyWasOwnerCritical(options: Record<string, any> = {}) {
...
function resolveProjectedRecentLastSentLane(options: Record<string, any> = {}) {
  if (
...
function getRecentTrustedLane(options: Record<string, any> = {}) {
  if (
...
function getRecentProjectedLane(options: Record<string, any> = {}) {
  if (
    !hasDestinyStamp(options.laneStamp) ||
...
function resolveRecentOwnerLaneState(options: Record<string, any> = {}) {
...
function buildDecisionCandidate(label, value, extra: Record<string, any> = {}) {
...
function summarizeDecisionCandidates(candidates: any[] = [], selectedValue = null) {
...
function formatDecisionCandidateSummary(summary) {
  ...
  const winnerLabels = new Set<any>(winners.map((entry) => entry.label));
  return active.map((entry) =>
    `${winnerLabels.has(entry.label) ? "*" : ""}${entry.label}=${formatDiagnosticDestinyStamp(entry.value)}`);
}

function resolveOwnerMonotonicState(options: Record<string, any> = {}) {
...
function resolveGotoCommandSyncState(options: Record<string, any> = {}) {
...
function resolveOwnerMovementRestampState(options: Record<string, any> = {}) {
...
function resolveDestinyLifecycleRestampState(options: Record<string, any> = {}) {
  const isFreshAcquireLifecycleGroup =
    options.isFreshAcquireLifecycleGroup === true;
  const isMissileLifecycleGroup =
    options.isMissileLifecycleGroup === true;
...
function resolveDamageStateDispatchStamp(options: Record<string, any> = {}) {
