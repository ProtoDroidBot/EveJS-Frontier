...
const { MICHELLE_DIRECT_CRITICAL_ECHO_DESTINY_LEAD, MICHELLE_HELD_FUTURE_DESTINY_LEAD, } = require("./michelleContract");
...
function buildMichelleSafeDestinySendOptions(baseOptions = {}) {
    const minimumHistoryLeadFloor = Math.max(0, toInt(baseOptions.minimumHistoryLeadFloor, MICHELLE_HELD_FUTURE_DESTINY_LEAD));
    const minimumLeadFromCurrentHistory = Math.max(toInt(baseOptions.minimumLeadFromCurrentHistory, 0), minimumHistoryLeadFloor);
    const maximumLeadFromCurrentHistory = Math.max(toInt(baseOptions.maximumLeadFromCurrentHistory, minimumLeadFromCurrentHistory), minimumLeadFromCurrentHistory);
    const historyLeadUsesImmediateSessionStamp = baseOptions.historyLeadUsesImmediateSessionStamp === true;
    const historyLeadUsesPresentedSessionStamp = baseOptions.historyLeadUsesPresentedSessionStamp === true;
    const historyLeadUsesCurrentSessionStamp = !historyLeadUsesImmediateSessionStamp &&
    ...
    return {
        ...
        maximumHistorySafeLeadOverride: Math.max(toInt(baseOptions.maximumHistorySafeLeadOverride, 0), maximumLeadFromCurrentHistory),
        historyLeadUsesCurrentSessionStamp: historyLeadUsesCurrentSessionStamp || undefined,
        historyLeadUsesImmediateSessionStamp: historyLeadUsesImmediateSessionStamp || undefined,
        historyLeadUsesPresentedSessionStamp: historyLeadUsesPresentedSessionStamp || undefined,
    };
}
function buildPresentedSessionAlignedDestinySendOptions(baseOptions = {}) {
    const minimumLeadFromCurrentHistory = Math.max(0, toInt(baseOptions.minimumLeadFromCurrentHistory, 0));
    const maximumLeadFromCurrentHistory = Math.max(toInt(baseOptions.maximumLeadFromCurrentHistory, minimumLeadFromCurrentHistory), minimumLeadFromCurrentHistory);
    return {
        ...baseOptions,
...
function buildOwnerMissileLifecycleSendOptions(baseOptions = {}) {
    return buildMichelleSafeDestinySendOptions({
        ...
        minimumLeadFromCurrentHistory: Math.max(toInt(baseOptions.minimumLeadFromCurrentHistory, 0), MICHELLE_HELD_FUTURE_DESTINY_LEAD),
        maximumLeadFromCurrentHistory: Math.max(toInt(baseOptions.maximumLeadFromCurrentHistory, 0), MICHELLE_HELD_FUTURE_DESTINY_LEAD),
        maximumHistorySafeLeadOverride: Math.max(toInt(baseOptions.maximumHistorySafeLeadOverride, 0), MICHELLE_HELD_FUTURE_DESTINY_LEAD),
        historyLeadUsesCurrentSessionStamp: false,
...
function buildObserverMissileLifecycleSendOptions(baseOptions = {}) {
    return buildMichelleSafeDestinySendOptions({
        ...
        minimumHistoryLeadFloor: MICHELLE_HELD_FUTURE_DESTINY_LEAD,
        minimumLeadFromCurrentHistory: Math.max(toInt(baseOptions.minimumLeadFromCurrentHistory, 0), MICHELLE_HELD_FUTURE_DESTINY_LEAD),
        maximumLeadFromCurrentHistory: Math.max(toInt(baseOptions.maximumLeadFromCurrentHistory, MICHELLE_HELD_FUTURE_DESTINY_LEAD), MICHELLE_HELD_FUTURE_DESTINY_LEAD),
        maximumHistorySafeLeadOverride: Math.max(toInt(baseOptions.maximumHistorySafeLeadOverride, 0), MICHELLE_HELD_FUTURE_DESTINY_LEAD),
        historyLeadUsesCurrentSessionStamp: false,
        historyLeadUsesImmediateSessionStamp: false,
        historyLeadUsesPresentedSessionStamp: true,
...
function buildOwnerShipPrimeSendOptions(broadcastOptions = {}) {
    const baseOptions = {
        translateStamps: broadcastOptions &&
            Object.prototype.hasOwnProperty.call(broadcastOptions, "translateStamps")
            ? broadcastOptions.translateStamps
            : undefined,
        minimumLeadFromCurrentHistory: broadcastOptions && broadcastOptions.minimumLeadFromCurrentHistory,
        maximumLeadFromCurrentHistory: broadcastOptions && broadcastOptions.maximumLeadFromCurrentHistory,
    ...
    return buildMichelleSafeDestinySendOptions({
        ...baseOptions,
        minimumHistoryLeadFloor: broadcastOptions && broadcastOptions.minimumHistoryLeadFloor,
        maximumHistorySafeLeadOverride: broadcastOptions && broadcastOptions.maximumHistorySafeLeadOverride,
        historyLeadUsesCurrentSessionStamp: broadcastOptions && broadcastOptions.historyLeadUsesCurrentSessionStamp,
        historyLeadUsesImmediateSessionStamp: broadcastOptions && broadcastOptions.historyLeadUsesImmediateSessionStamp,
    });
}
...
function buildOwnerMissileFreshAcquireSendOptions(baseOptions = {}) {
    return buildMichelleSafeDestinySendOptions({
        ...
        minimumLeadFromCurrentHistory: Math.max(toInt(baseOptions.minimumLeadFromCurrentHistory, 0), MICHELLE_DIRECT_CRITICAL_ECHO_DESTINY_LEAD),
        maximumLeadFromCurrentHistory: Math.max(toInt(baseOptions.maximumLeadFromCurrentHistory, 0), MICHELLE_DIRECT_CRITICAL_ECHO_DESTINY_LEAD),
        maximumHistorySafeLeadOverride: Math.max(toInt(baseOptions.maximumHistorySafeLeadOverride, 0), MICHELLE_HELD_FUTURE_DESTINY_LEAD),
        historyLeadUsesCurrentSessionStamp: false,
        historyLeadUsesImmediateSessionStamp: false,
        historyLeadUsesPresentedSessionStamp: true,
        historyLeadPresentedMaximumFutureLead: MICHELLE_DIRECT_CRITICAL_ECHO_DESTINY_LEAD,
    });
}
...
function buildObserverPropulsionShipPrimeBroadcastOptions(baseOptions = {}) {
    return {
        ...baseOptions,
        minimumHistoryLeadFloor: Math.max(toInt(baseOptions.minimumHistoryLeadFloor, 0), MICHELLE_DIRECT_CRITICAL_ECHO_DESTINY_LEAD),
        minimumLeadFromCurrentHistory: Math.max(toInt(baseOptions.minimumLeadFromCurrentHistory, 0), MICHELLE_DIRECT_CRITICAL_ECHO_DESTINY_LEAD),
        maximumLeadFromCurrentHistory: Math.max(toInt(baseOptions.maximumLeadFromCurrentHistory, 0), MICHELLE_DIRECT_CRITICAL_ECHO_DESTINY_LEAD),
        maximumHistorySafeLeadOverride: Math.max(toInt(baseOptions.maximumHistorySafeLeadOverride, 0), MICHELLE_HELD_FUTURE_DESTINY_LEAD),
        historyLeadUsesPresentedSessionStamp: true,
