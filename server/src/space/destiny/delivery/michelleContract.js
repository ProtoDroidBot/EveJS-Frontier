...
const MICHELLE_POST_HELD_FUTURE_DESTINY_LEAD = MICHELLE_HELD_FUTURE_DESTINY_LEAD + 1;
function isSessionOpenForDestinyDelivery(session) {
    ...
    if (Object.prototype.hasOwnProperty.call(session, "socket") &&
...
function attemptDestinyNotificationDelivery(session, payloadTuple) {
    ...
    try {
        const result = session.sendNotification(DESTINY_NOTIFICATION_NAME, DESTINY_NOTIFICATION_ID_TYPE, payloadTuple);
        if (result === false) {
            return {
                status: "rejected",
...
//# sourceMappingURL=michelleContract.js.map