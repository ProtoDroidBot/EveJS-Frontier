...
const { resolveDestinyAuthorityLaneTuple, snapshotDestinyAuthorityState, } = require("../authority/destinySessionState");
const { MICHELLE_HELD_FUTURE_DESTINY_LEAD, } = require("./michelleContract");
const { resolvePresentedSessionDestinyStamp, } = require("./sessionWindows");
const { DESTINY_STAMP_MAX_FORWARD_LEAD, advanceDestinyStamp, getCurrentDestinyStamp, getDestinyStampForwardBoundaryDeltaMs, hasDestinyStamp, normalizeDestinyStamp, selectFurthestDestinyStamp, } = require("./stamps");
const { PILOT_WARP_HISTORY_SAFE_DESTINY_LEAD, } = require("../simulation/warpContract");
...
function resolvePresentedDestinyLaneForSession(session) {
    ...
    return {
        ...
        onlyStaleProjectedOwnerMissileLane: classificationState[staleProjectionKey] === true,
    };
}
...
function getImmediateDestinyStampForSession(session, fallbackStamp = 0) {
    ...
    const lastVisibleStamp = hasDestinyStamp(session._space.historyFloorDestinyStamp)
        ? normalizeDestinyStamp(session._space.historyFloorDestinyStamp)
        : null;
    return selectFurthestDestinyStamp(previousStamp, [lastVisibleStamp], MICHELLE_HELD_FUTURE_DESTINY_LEAD);
}
function getStargateDepartureDestinyStampForSession(session, fallbackStamp = 0) {
    const immediateStamp = getImmediateDestinyStampForSession(session, fallbackStamp) >>> 0;
    if (!session || !session._space) {
    ...
    return selectFurthestDestinyStamp(immediateStamp, [presentedLane.stamp]);
}
function translateSimTimeForSession(session, rawSimTimeMs, fallbackRawSimTimeMs = 0) {
    ...
    return roundNumber(normalizedRawSimTimeMs + getSessionClockOffsetMs(session), 3);
}
...
function getCurrentSessionDestinyStamp(session, rawSimTimeMs, fallbackRawSimTimeMs = 0) {
    return getCurrentDestinyStamp(getCurrentSessionSimTimeMs(session, rawSimTimeMs, fallbackRawSimTimeMs));
}
function getCurrentVisibleDestinyStampForSession(session, fallbackStamp = 0) {
    const currentStamp = normalizeDestinyStamp(fallbackStamp, 0);
    const lastVisibleStamp = session && session._space &&
        hasDestinyStamp(session._space.historyFloorDestinyStamp)
        ? normalizeDestinyStamp(session._space.historyFloorDestinyStamp)
        : null;
    return selectFurthestDestinyStamp(currentStamp, [lastVisibleStamp], DESTINY_STAMP_MAX_FORWARD_LEAD);
}
function getCurrentVisibleSessionDestinyStamp(session, rawSimTimeMs, fallbackRawSimTimeMs = 0) {
    return getCurrentVisibleDestinyStampForSession(session, getCurrentSessionDestinyStamp(session, rawSimTimeMs, fallbackRawSimTimeMs));
}
function getCurrentPresentedSessionDestinyStamp(session, rawSimTimeMs, maximumFutureLead = MICHELLE_HELD_FUTURE_DESTINY_LEAD, fallbackRawSimTimeMs = 0) {
    const currentVisibleStamp = getCurrentVisibleSessionDestinyStamp(session, rawSimTimeMs, fallbackRawSimTimeMs);
    const presentedLane = resolvePresentedDestinyLaneForSession(session);
    return resolvePresentedSessionDestinyStamp({
        ...
        maximumTrustedLead: Math.max(MICHELLE_HELD_FUTURE_DESTINY_LEAD, PILOT_WARP_HISTORY_SAFE_DESTINY_LEAD),
    });
}
function getOwnerPropulsionTogglePresentationStamp(session, rawSimTimeMs, fallbackRawSimTimeMs = 0) {
    const currentSessionStamp = getCurrentSessionDestinyStamp(session, rawSimTimeMs, fallbackRawSimTimeMs) >>> 0;
    const currentPresentedStamp = getCurrentPresentedSessionDestinyStamp(session, rawSimTimeMs, PILOT_WARP_HISTORY_SAFE_DESTINY_LEAD, fallbackRawSimTimeMs) >>> 0;
    return selectFurthestDestinyStamp(currentSessionStamp, [
...
function getCurrentClampedDestinyStampForSession(session, fallbackStamp = 0) {
    return getCurrentVisibleDestinyStampForSession(session, toInt(fallbackStamp, 0) >>> 0);
}
function getCurrentVisibleSessionSimTimeMs(session, rawSimTimeMs, fallbackRawSimTimeMs = 0) {
    const currentSessionSimTimeMs = getCurrentSessionSimTimeMs(session, rawSimTimeMs, fallbackRawSimTimeMs);
    const currentVisibleStamp = getCurrentVisibleSessionDestinyStamp(session, rawSimTimeMs, fallbackRawSimTimeMs);
    const currentSessionStamp = getCurrentSessionDestinyStamp(session, rawSimTimeMs, fallbackRawSimTimeMs);
    return currentSessionSimTimeMs + getDestinyStampForwardBoundaryDeltaMs(currentSessionSimTimeMs, currentSessionStamp, currentVisibleStamp, DESTINY_STAMP_MAX_FORWARD_LEAD);
}
function getCurrentClampedSessionSimTimeMs(session, rawSimTimeMs, fallbackRawSimTimeMs = 0) {
    return getCurrentVisibleSessionSimTimeMs(session, rawSimTimeMs, fallbackRawSimTimeMs);
}
function translateDestinyStampForSession(session, rawStamp, getStamp = getCurrentDestinyStamp) {
...
//# sourceMappingURL=sessionStamps.js.map