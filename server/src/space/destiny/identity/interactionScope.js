...
const { entityIDsEqual, normalizeNonNegativeInt64, normalizePersistentEntityID, toJSONSafeEntityID, } = require("./entityID.js");
...
function hasDungeonContentMarker(entity) {
    return getScopeMetadataContainers(entity).some((container) => (DUNGEON_BOOLEAN_MARKERS.some((fieldName) => container[fieldName] === true) ||
        DUNGEON_TEXT_MARKERS.some((fieldName) => String(container[fieldName] || "").trim()) ||
...
function hasAirContentMarker(entity) {
    return getScopeMetadataContainers(entity).some((container) => (AIR_BOOLEAN_MARKERS.some((fieldName) => container[fieldName] === true) ||
...
function resolveEntityInteractionScope(entity) {
    ...
    const playerTrackerIdentities = [
        ...
        resolveExactIdentityAliases(entity, ["dungeonCurrentRoomID"], { allowZero: true }),
    ];
    const hasPlayerTrackerContext = Boolean(entity.kind === "ship" &&
    ...
    const hasDungeonScope = Boolean(dungeonMarker ||
        dungeonInstance.present ||
        dungeonSite.present ||
        hasPlayerTrackerContext);
    const hasAirScope = Boolean(airMarker ||
        airInstance.present ||
        airOwner.present ||
        airOperation.present);
    const scoped = hasDungeonScope || hasAirScope;
    const hasDungeonAuthority = dungeonInstance.value !== null;
    const hasAirAuthority = Boolean(airInstance.value !== null || airOwner.value !== null);
    const valid = Boolean(entity.entityInteractionScopeInvalid !== true &&
...
function entityMatchesOwnerScope(entity, scope, ownerCharacterID) {
    ...
    if (scope.airOwnerCharacterID !== null &&
        entityIDsEqual(scope.airOwnerCharacterID, ownerCharacterID)) {
        return true;
    }
    const actorCharacterID = resolveEntityActorCharacterID(entity, entity && entity.session);
    return Boolean(actorCharacterID !== null &&
...
function canEntitiesInteractLocally(sourceEntity, targetEntity) {
    ...
    if (sourceScope.dungeonInstanceID !== null ||
        targetScope.dungeonInstanceID !== null) {
        if (sourceScope.dungeonInstanceID === null ||
            targetScope.dungeonInstanceID === null ||
            !entityIDsEqual(sourceScope.dungeonInstanceID, targetScope.dungeonInstanceID)) {
            return false;
        }
    }
    if (sourceScope.airInstanceID !== null &&
    ...
    if (sourceScope.airInstanceID !== null &&
        targetScope.airInstanceID === null &&
        !entityMatchesOwnerScope(targetEntity, targetScope, sourceScope.airOwnerCharacterID)) {
        return false;
    }
    if (sourceScope.airInstanceID === null &&
        targetScope.airInstanceID !== null &&
        !entityMatchesOwnerScope(sourceEntity, sourceScope, targetScope.airOwnerCharacterID)) {
        return false;
    }
    if (sourceScope.airOperationID !== null &&
        targetScope.airOperationID !== null &&
        !entityIDsEqual(sourceScope.airOperationID, targetScope.airOperationID)) {
        return false;
    }
    if (sourceScope.dungeonSiteID !== null &&
        targetScope.dungeonSiteID !== null &&
        !entityIDsEqual(sourceScope.dungeonSiteID, targetScope.dungeonSiteID)) {
        return false;
    }
    if (!entityMatchesOwnerScope(targetEntity, targetScope, sourceScope.airOwnerCharacterID) ||
        !entityMatchesOwnerScope(sourceEntity, sourceScope, targetScope.airOwnerCharacterID)) {
        return false;
    }
    return true;
...
function buildPlayerEgoScopeMetadata(sourceEntity) {
    ...
    metadata.dungeonCurrentInstanceID = toJSONSafeEntityID(scope.dungeonInstanceID);
    ...
    if (hasIdentityValue(sourceEntity, "dungeonCurrentRoomID")) {
        const roomID = normalizeNonNegativeInt64(sourceEntity.dungeonCurrentRoomID);
    ...
    if (Array.isArray(sourceEntity.dungeonCurrentRoomPosition) &&
...
//# sourceMappingURL=interactionScope.js.map