...
function usesFrontierBallEncoding(options: Record<string, any> = {}) {
...
function shouldUseSessionlessNpcWarpAddBallsBootstrap(entity, options: Record<string, any> = {}) {
  if (
    !options.forAddBalls ||
    !entity ||
    entity.kind !== "ship" ||
...
function buildAddBallsBootstrapEntity(entity, options: Record<string, any> = {}) {
  return entity;
}
...
function appendFrontierCommonBallTail(chunks, entity, options: Record<string, any> = {}) {
  pushInt32(
    chunks,
...
function encodeFrontierFreeBall(entity, options: Record<string, any> = {}) {
...
function encodeFreeBall(entity, options: Record<string, any> = {}) {
  const encodedEntity = buildAddBallsBootstrapEntity(entity, options);
  const chunks: any[] = [];
  const position = buildVector(encodedEntity && encodedEntity.position);
  const velocity = buildVector(encodedEntity && encodedEntity.velocity);
...
function encodeEntityBall(entity, options: Record<string, any> = {}) {
...
function debugDescribeEntityBall(entity, options: Record<string, any> = {}) {
