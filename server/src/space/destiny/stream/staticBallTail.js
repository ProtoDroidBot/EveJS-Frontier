...
const { resolveConfiguredBallFlags, resolveConfiguredBallMode, } = require("./ballConfig");
const { buildVector, pushBigInt64, pushDouble, pushFloat, pushInt32, pushUInt8, toFiniteNumber, toInt32, } = require("./primitives");
const MINI_GEOMETRY_FLAG_MASK = (BALL_FLAG.HAS_MINIBOXES |
...
function getEntityMiniGeometryFlags(entity) {
    return ((getEntityMiniBoxes(entity).length > 0 ? BALL_FLAG.HAS_MINIBOXES : 0) |
...
function getStaticBallFlags(entity) {
    ...
    return ((configured & ~MINI_GEOMETRY_FLAG_MASK) |
        generatedMiniFlags) & 0xff;
}
...
function getMiniCapsulePoint(miniCapsule, primaryKey, fallbackKey) {
    ...
    return buildVector(miniCapsule && (miniCapsule[primaryKey] ||
...
function getMiniBoxVector(miniBox, vectorName) {
    ...
    return buildVector(miniBox && (miniBox[vectorName] ||
        {
...
function buildStaticBallHeader(entity) {
    ...
    pushBigInt64(chunks, toInt32(entity && entity.allianceID, 0) > 0 ? entity.allianceID : -1);
...
//# sourceMappingURL=staticBallTail.js.map