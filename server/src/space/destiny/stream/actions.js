...
const { buildDict, buildKeyVal, buildList, } = require("../../../services/_shared/serviceHelpers");
const { buildMarshalReal, buildMarshalRealVector, toFiniteNumber, toInt32, } = require("./primitives");
...
function buildSetBallWarpFactorsPayload(entityID, accelerationFactor, decelerationFactor) {
...
function buildAddBallPayload(entityID, { mass = 0, radius = 0, maxSpeed = 0, isFree = true, isGlobal = false, isMassive = false, isInteractive = true, isMoribund = false, position = null, velocity = null, inertia = 1, speedFraction = 0, } = {}) {
...
function buildSetBallHarmonicPayload(entityID, harmonicValue, corporationID, allianceID, isForcefield) {
    return [
        "SetBallHarmonic",
        [
            normalizeActionEntityID(entityID),
...
function buildOnSpecialFXPayload(entityID, guid, { moduleID = null, moduleTypeID = null, targetID = null, chargeTypeID = null, isOffensive = false, start = true, active = true, duration, repeat, startTime, timeFromStart, graphicInfo, } = {}) {
    const args = [
        normalizeActionEntityID(entityID),
...
function buildRemoveBallsPayload(entityIDs) {
    return ["RemoveBalls", [buildList(entityIDs.map((entityID) => normalizeActionEntityID(entityID)))]];
}
module.exports = {
    buildAddBallPayload,
