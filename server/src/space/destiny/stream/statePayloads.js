...
const { buildDict, buildKeyVal, buildList, currentFileTime, } = require("../../../services/_shared/serviceHelpers");
const { marshalEncode, } = require("../../../network/tcp/utils/marshal");
const { decodeSerializedActions, } = require("../batching/packagedAction");
const { requireEntityID, } = require("../identity/entityID");
const { DESTINY_STAMP_INTERVAL_MS, } = require("../constants");
const { encodeEntityBall: defaultEncodeEntityBall, } = require("./ballEncoding");
const { toInt32, } = require("./primitives");
const { normalizeCrDataDictionaryForProfile, usesCrDataBallMetadata, usesCrDataSetState, usesFrontierStateStreamPreamble, } = require("./statePayloadCompatibility");
...
function encodeStateHeader(packetType, stamp, deps = {}) {
    const useFrontierPreamble = usesFrontierStateStreamPreamble(deps.compatibilityProfile);
    ...
    if (useFrontierPreamble) {
        ...
        buffer.writeUInt32LE(Math.max(1, Math.min(100000, toInt32(deps.tickIntervalMs, DESTINY_STAMP_INTERVAL_MS))), 13);
...
function buildAddBallsExtraBallData(ballEntities, simFileTime, deps = {}) {
    ...
    return normalizeBallEntities(ballEntities)
        ...
        .map((entity) => {
        const slimItem = normalizeCrDataDictionaryForProfile(deps.buildSlimItemDict(entity), entity, deps.compatibilityProfile);
...
function buildAddBalls2Payload(stateStamp, entities, simFileTime = currentFileTime(), deps = {}) {
    const ballEntities = normalizeBallEntities(entities);
    const state = buildAddBallsStateBuffer(stateStamp, ballEntities, {
        ...deps,
        simFileTime,
    });
    const extraBallData = buildAddBallsExtraBallData(ballEntities, simFileTime, deps);
    return [
        "AddBalls2",
        [
            [
...
function buildSetStatePayload(stateStamp, system, egoEntityID, entities, simFileTime = currentFileTime(), dbuffStateEntries = [], effectStateEntries = [], deps = {}) {
    ...
    const damageEntries = buildDamageState
        ...
        : [];
    const slimEntities = ballEntities.filter((entity) => (entity.omitSlimItem !== true));
    const entityStateEntry = useCrData
        ? [
            "crdata",
            buildDict(slimEntities.map((entity) => [
                requireEntityID(entity.itemID, "SetState crdata entityID"),
                normalizeCrDataDictionaryForProfile(stateDeps.buildSlimItemDict(entity), entity, stateDeps.compatibilityProfile),
            ])),
        ]
        : [
            "slims",
            buildList(slimEntities.map((entity) => (stateDeps.buildSlimItemObject(entity)))),
        ];
...
function buildDestinyUpdatePayload(updates, waitForBubble = false, delayedTargetEvents = null) {
    const updateList = buildList((Array.isArray(updates) ? updates : []).map((update) => [
        update.stamp,
        update.payload,
    ]));
...
//# sourceMappingURL=statePayloads.js.map