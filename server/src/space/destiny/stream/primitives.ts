...
function normalizeVector(source = null, fallback: Record<string, any> = { x: 1, y: 0, z: 0 }) {
...
function buildMarshalRealVector(source = null, fallback: Record<string, any> = { x: 0, y: 0, z: 0 }) {
  const vector = buildVector(source, fallback);
  return {
