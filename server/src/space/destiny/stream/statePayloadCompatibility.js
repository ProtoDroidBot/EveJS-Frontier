...
const { buildDbRowset, } = require("../../../services/_shared/serviceHelpers");
...
function buildFrontierStargateJumps(entity) {
    const rows = (entity && entity.destinationID && entity.destinationSolarSystemID)
        ...
        : [];
    return buildDbRowset(FRONTIER_STARGATE_JUMP_COLUMNS, rows, FRONTIER_CROWSET_NAME);
}
function normalizeCrDataDictionaryForProfile(crData, entity, compatibilityProfile) {
    ...
    const assemblyStatusEntry = crData.entries.find((entry) => (Array.isArray(entry) && entry[0] === "assembly_status"));
    const isConstructionSite = kind === "deployable" && (Number(assemblyStatusEntry && assemblyStatusEntry[1]) ===
    ...
    return {
        ...
        entries: crData.entries
            ...
            .map((entry) => (kind === "stargate" && entry[0] === "jumps"
...
function normalizeSlimItemObjectForProfile(slimItem, compatibilityProfile) {
...
//# sourceMappingURL=statePayloadCompatibility.js.map