...
const { isEntityInActiveWarp, } = require("../simulation/warpPhase");
const { BALL_FLAG, BALL_MODE, RUNTIME_UNANCHORED_STRUCTURE_HULL_KIND, } = require("../constants");
...
const FREE_WIRE_CONFIG_FLAGS = (BALL_FLAG.IS_GLOBAL |
...
function getFollowIdentityValue(entity) {
    return entity && (entity.targetEntityID ??
...
function isFreeBallInteractive(entity) {
    ...
    if (Object.prototype.hasOwnProperty.call(entity, "destinyBallFlags") &&
        Number.isFinite(Number(entity.destinyBallFlags))) {
        return (resolveConfiguredBallFlags(entity, 0) & BALL_FLAG.IS_INTERACTIVE) !== 0;
    }
    if (entity.kind === "drone" || entity.kind === "fighter") {
        return (hasPositiveIdentity(entity.controllerID) ||
            hasPositiveIdentity(entity.ownerID));
    }
    if (isNpcShipEntity(entity)) {
        return false;
    }
    return Boolean(entity.session ||
...
function isFreeBallCollisionEnabled(entity) {
    ...
    if (entity.collisionEnabled === false ||
...
function getDestinyWarpTargetPoint(entity) {
    ...
    const preparing = Boolean(!isEntityInActiveWarp(entity) ||
        (warpState &&
...
function getEntityCloakMode(entity) {
    return toInt32(entity && (entity.isCloaked ?? entity.cloakMode ?? entity.cloaked), 0);
}
function getEntityEffectStamp(entity) {
    return toInt32(entity && (entity.effectStamp ??
        entity.destinyEffectStamp ??
        (entity.missileState && entity.missileState.effectStamp) ??
        (entity.warpState && entity.warpState.effectStamp)), 0);
}
function getEntityModeOwnerID(entity) {
    const value = entity && (entity.ownerEntityID ??
...
function getEntityFollowID(entity) {
    return resolveOptionalEntityID(entity && (entity.targetEntityID ??
        entity.followTargetID ??
        entity.followID ??
        entity.followId ??
        entity.parentEntityID), 0, "Destiny follow target");
}
function getEntityFollowRange(entity) {
    return toFiniteNumber(entity && (entity.followRange ??
        entity.orbitDistance ??
        entity.formationRange ??
        entity.missileRange), 0);
}
function getEntityMushroomSpan(entity) {
    return toFiniteNumber(entity && (entity.span ??
        entity.mushroomSpan ??
        entity.destinationSpan ??
        entity.targetSpan ??
        (entity.targetPoint && entity.targetPoint.x)), 0);
}
...
function shouldUseSessionlessNpcWarpAddBallsBootstrap(entity, options = {}) {
    ...
    const npcEntityType = String(entity.npcEntityType || "").trim().toLowerCase();
    return Boolean(entity.sessionlessWarpIngress &&
...
function appendFrontierCommonBallTail(chunks, entity, options = {}) {
    pushInt32(chunks, toInt32(entity && entity.surfaceType, -0x80000000));
    ...
    const orientation = (options.useEntityDirection === true &&
        entity &&
        entity.kind === "ship")
...
function appendFrontierModeData(chunks, entity, mode) {
    switch (mode) {
        ...
        case BALL_MODE.WARP: {
            ...
            pushBigInt64(chunks, BigInt(toInt32(warpState && warpState.effectStamp, 0)));
...
function encodeFrontierFreeBall(entity, options = {}) {
    ...
    const angularVelocity = buildVector(encodedEntity && encodedEntity.angularVelocity);
    const mode = getFreeBallMode(encodedEntity);
    pushBigInt64(chunks, requireEntityID(encodedEntity && encodedEntity.itemID, "Destiny ball itemID"));
    pushUInt8(chunks, mode);
    ...
    const fallbackMass = (encodedEntity.kind === "container" || encodedEntity.kind === "wreck") ? 10_000 : 1_000_000;
    pushDouble(chunks, resolveDestinyPhysicalMass(encodedEntity, fallbackMass));
...
function buildFreeBallDebugSummary(entity) {
    ...
    const summary = {
        ...
        mass: resolveDestinyPhysicalMass(entity, entity.kind === "container" || entity.kind === "wreck"
            ? 10_000
            : 1_000_000),
...
//# sourceMappingURL=ballEncoding.js.map