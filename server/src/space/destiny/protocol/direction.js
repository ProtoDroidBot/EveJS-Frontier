...
function extractLatestGotoDirection(updates, fallbackDirection = null) {
    ...
    try {
        for (const update of updates) {
            ...
            const direction = extractGotoDirection(payload, latestDirection || fallbackDirection);
            if (direction) {
...
function resolveGotoDirectionFromUpdates(options = {}) {
    ...
    const filterByStamp = Object.prototype.hasOwnProperty.call(resolvedOptions, "expectedStamp");
    ...
    const fallbackDirection = (defaultDirection && typeof defaultDirection === "object")
        ? defaultDirection
        : DEFAULT_DIRECTION;
    ...
    for (const update of Array.isArray(updates) ? updates : []) {
        ...
        try {
            latestDirection = customVectorNormalizer(vector, latestDirection || fallbackDirection);
        }
        catch (_error) {
