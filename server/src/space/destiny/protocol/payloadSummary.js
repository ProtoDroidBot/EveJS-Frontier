...
const { getEntityIDText, normalizeEntityID, toJSONSafeEntityID, } = require("../identity/entityID");
const { getPayloadPrimaryEntityID, } = require("./payloadIdentity");
const { canonicalizePayloadName, getCanonicalPayloadName, getPayloadName, isDestinyPayload, isKnownPayloadName, } = require("./payloads");
...
function summarizePayloadForDiagnostics(payload) {
    ...
    return {
        ...
        primaryEntityID: primaryEntityID === null ? null : toJSONSafeEntityID(primaryEntityID),
...
function createDestinyPayloadSummary(deps = {}) {
    ...
    function getPrimaryEntityID(payload, options = {}) {
        const optionNormalizer = (options &&
    ...
    function getMarshalDictEntry(value, key) {
        ...
        try {
            ...
            if (value.type === "dict" &&
                ...
                const entry = value.entries.find((candidate) => Array.isArray(candidate) && candidate[0] === key);
    ...
    function summarizeAddBalls2Args(args) {
        return getMarshalListItems(args).map((batchEntry, index) => {
            ...
            const slimEntries = getMarshalListItems(Array.isArray(batchEntry) ? batchEntry[1] : null);
            const slimIDs = slimEntries
                .map(extractSlimItemIdentity)
                .filter((entry) => entry.itemID !== null);
            return {
                batchIndex: index,
                stateStamp: Buffer.isBuffer(stateBuffer) && stateBuffer.length >= 5
                    ? (stateBuffer.readUInt32LE(1) >>> 0)
                    : 0,
                entityCount: slimIDs.length,
                entityIDs: limitTraceArray(slimIDs.map((entry) => entry.itemID), 25),
                typeIDs: limitTraceArray(slimIDs.map((entry) => entry.typeID).filter((value) => value > 0), 12),
            };
        });
    }
    function summarizeSetStateArgs(args) {
        ...
        const damageStateEntityIDs = (damageState &&
            damageState.type === "dict" &&
            Array.isArray(damageState.entries))
            ? damageState.entries
                .map((entry) => normalizeSummaryEntityID(Array.isArray(entry) ? entry[0] : null))
        ...
        return [{
                ...
                slimIDs: limitTraceArray(slimIDs.map((entry) => entry.itemID), 25),
...
//# sourceMappingURL=payloadSummary.js.map