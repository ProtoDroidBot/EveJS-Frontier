...
function createDestinyPayloadSummary(deps: Record<string, any> = {}) {
  ...
  function getPrimaryEntityID(payload, options: Record<string, any> = {}) {
...
function summarizeUpdatesForDiagnostics(updates: any[] = []) {
  ...
  const stamps: any[] = [];
  const entityIDs: any[] = [];
  const identityTexts = new Set<any>();
