...
const DESTINY_JOURNEY_LOG_PATH = path.join(__dirname, "../../../../logs/space-destiny-journey.log");
const DESTINY_RESTAMP_LOG_PATH = path.join(__dirname, "../../../../logs/space-destiny-restamp.log");
const DESTINY_ENGINE_LOG_PATH = path.join(__dirname, "../../../../logs/space-destiny-engine.log");
const DESTINY_DROP_LOG_PATH = path.join(__dirname, "../../../../logs/space-destiny-drop.log");
const MICHELLE_CONTRACT_LOG_PATH = path.join(__dirname, "../../../../logs/space-michelle-contract.log");
...
function createDestinyJourneyLog(deps = {}) {
    ...
    function normalizeStructuredLogRecord(event, details) {
        const record = buildStructuredLogRecord(event, details);
        // A caller-supplied trace normalizer may intentionally preserve exact
        // BigInt identities. Logging must remain JSON-safe regardless, so always
        // run the normalized result through the package fallback as a final guard.
        return fallbackNormalizeTraceValue(normalizeTraceValue(record));
    }
    let nextJourneyID = 1;
    ...
    function logJourney(event, details = {}) {
        appendDestinyJourneyLog(JSON.stringify(normalizeStructuredLogRecord(event, details)));
    }
    function logRestamp(event, details = {}) {
        appendNamedDebugLog(DESTINY_RESTAMP_LOG_PATH, "DestinyRestampLog", JSON.stringify(normalizeStructuredLogRecord(event, details)));
    }
    function logEngine(event, details = {}) {
        appendNamedDebugLog(DESTINY_ENGINE_LOG_PATH, "DestinyEngineLog", JSON.stringify(normalizeStructuredLogRecord(event, details)));
    }
    function logDrop(event, details = {}) {
        appendNamedDebugLog(DESTINY_DROP_LOG_PATH, "DestinyDropLog", JSON.stringify(normalizeStructuredLogRecord(event, details)));
    }
    function logMichelle(event, details = {}) {
        appendNamedDebugLog(MICHELLE_CONTRACT_LOG_PATH, "MichelleContractLog", JSON.stringify(normalizeStructuredLogRecord(event, details)));
    }
    return {
...
//# sourceMappingURL=destinyJourneyLog.js.map