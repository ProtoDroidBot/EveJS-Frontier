...
function isCriticalDestinyContract(contract) {
    return CRITICAL_DESTINY_CONTRACTS.has(normalizeDestinyContract(contract, ""));
}
function inferDestinyContract(updates = [], options = {}) {
    const explicitContract = normalizeDestinyContract(options && options.destinyAuthorityContract, "");
    if (explicitContract) {
    ...
    const hasMovementContract = payloadNames.some((name) => (name === "GotoDirection" ||
        name === "GotoPoint" ||
        name === "LaunchMissile" ||
        name === "Orbit" ||
        name === "FollowBall" ||
        ...
        name === "Stop" ||
        name === "WarpTo"));
    const hasShipPrimeSetter = payloadNames.some((name) => (name === "CloakBall" ||
        name === "UncloakBall" ||
        name === "SetBallAgility" ||
        name === "SetBallMass" ||
        name === "SetMaxSpeed" ||
    ...
    const hasFreshAcquireLifecycle = Array.isArray(updates) && updates.some((update) => update && update.freshAcquireLifecycleGroup === true);
    const hasMissileLifecycle = Array.isArray(updates) && updates.some((update) => (update &&
        (update.missileLifecycleGroup === true ||
            update.ownerMissileLifecycleGroup === true)));
    const hasOwnerMissileLifecycle = Array.isArray(updates) && updates.some((update) => update && update.ownerMissileLifecycleGroup === true);
    if (options &&
...
//# sourceMappingURL=destinyContracts.js.map