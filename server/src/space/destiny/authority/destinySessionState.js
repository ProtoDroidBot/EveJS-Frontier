...
const { MICHELLE_HELD_FUTURE_DESTINY_LEAD, } = require("../delivery/michelleContract");
const { parseDestinyDirectionHeading, serializeDestinyDirectionHeading, } = require("../protocol/direction");
const { advanceDestinyStamp, hasDestinyStamp, isDestinyStampAfter, normalizeDestinyStamp, resolveOptionalDestinyStamp, selectLaterDestinyStamp, } = require("../delivery/stamps");
...
function selectLatestPresentDestinyStamp(values, fallback = null) {
    const selected = (Array.isArray(values) ? values : [values])
        ...
        .reduce((latest, candidate) => selectLaterDestinyStamp(latest, candidate), null);
...
function resolveDestinyAuthorityLaneTuple({ authorityState, legacyState, stampKey, rawDispatchKey = "", anchorKey = "", legacyStampKey = stampKey, legacyRawDispatchKey = rawDispatchKey, legacyAnchorKey = anchorKey, } = {}) {
    const hasAuthorityLane = Boolean(authorityState &&
        stampKey &&
    ...
    return {
        stamp,
        rawDispatchStamp: hasDestinyStamp(stamp) && sourceState && sourceRawDispatchKey
            ? resolveOptionalDestinyStamp(sourceState[sourceRawDispatchKey])
            : null,
        anchorStamp: hasDestinyStamp(stamp) && sourceState && sourceAnchorKey
            ? resolveOptionalDestinyStamp(sourceState[sourceAnchorKey])
            : null,
        fromAuthority: hasAuthorityLane,
    };
}
function resolveDestinyAuthorityCommandTuple({ authorityState, legacyState, } = {}) {
    ...
    let headingHash = lane.fromAuthority
        ? typeof authorityState.lastOwnerCommandHeadingHash === "string"
            ...
            : ""
        : serializeDestinyDirectionHeading(legacyState && legacyState.lastPilotCommandDirection);
    if (lane.fromAuthority && !headingHash) {
        ...
        const exactLegacyMirror = legacyLane.stamp === lane.stamp &&
            legacyLane.rawDispatchStamp === lane.rawDispatchStamp &&
            legacyLane.anchorStamp === lane.anchorStamp;
        if (exactLegacyMirror) {
            headingHash = serializeDestinyDirectionHeading(legacyState && legacyState.lastPilotCommandDirection);
        }
    }
...
function ensureDestinyAuthorityState(session) {
    ...
    const currentState = spaceState.destinyAuthorityState &&
        typeof spaceState.destinyAuthorityState === "object"
        ? spaceState.destinyAuthorityState
        : {};
    const legacyLastSentStamp = resolveOptionalDestinyStamp(spaceState.lastSentDestinyStamp);
    const hasLegacyLastSentStamp = hasDestinyStamp(legacyLastSentStamp);
    const legacyLastSentRawDispatchStamp = resolveOptionalDestinyStamp(spaceState.lastSentDestinyRawDispatchStamp);
    const hasLegacyLastSentRawDispatchStamp = hasDestinyStamp(legacyLastSentRawDispatchStamp);
    const legacyOwnerCommandStamp = resolveOptionalDestinyStamp(spaceState.lastPilotCommandMovementStamp);
    const legacyOwnerCommandRawDispatchStamp = resolveOptionalDestinyStamp(spaceState.lastPilotCommandMovementRawDispatchStamp);
    const legacyLastSentMatchesOwnerCommand = hasLegacyLastSentStamp &&
        hasDestinyStamp(legacyOwnerCommandStamp) &&
        legacyLastSentStamp === legacyOwnerCommandStamp;
    const legacyOwnerMissileLifecycleStamp = resolveOptionalDestinyStamp(spaceState.lastOwnerMissileLifecycleStamp);
    const legacyOwnerMissileFreshAcquireStamp = resolveOptionalDestinyStamp(spaceState.lastOwnerMissileFreshAcquireStamp);
    const legacyOwnerNonMissileCriticalStamp = resolveOptionalDestinyStamp(spaceState.lastOwnerNonMissileCriticalStamp);
    const legacyLastSentMatchesOwnerMissileLifecycle = hasLegacyLastSentStamp &&
        hasDestinyStamp(legacyOwnerMissileLifecycleStamp) &&
        legacyLastSentStamp === legacyOwnerMissileLifecycleStamp;
    const legacyLastSentMatchesOwnerMissileFreshAcquire = hasLegacyLastSentStamp &&
        hasDestinyStamp(legacyOwnerMissileFreshAcquireStamp) &&
        legacyLastSentStamp === legacyOwnerMissileFreshAcquireStamp;
    const legacyLastSentMatchesOwnerNonMissileCritical = hasLegacyLastSentStamp &&
        hasDestinyStamp(legacyOwnerNonMissileCriticalStamp) &&
        legacyLastSentStamp === legacyOwnerNonMissileCriticalStamp;
    const currentAuthorityLastPresentedStamp = resolveOptionalDestinyStamp(currentState.lastPresentedStamp);
    const currentAuthorityLastRawDispatchStamp = resolveOptionalDestinyStamp(currentState.lastRawDispatchStamp);
    const legacyLastSentCriticalFlagLooksInherited = spaceState.lastSentDestinyWasOwnerCritical === true &&
    ...
    const legacyLastSentUntrustedFarAheadOwnerCommand = legacyLastSentMatchesOwnerCommand &&
        !hasDestinyStamp(legacyOwnerCommandRawDispatchStamp) &&
        hasDestinyStamp(currentState.lastPresentedStamp) &&
        isDestinyStampAfter(advanceDestinyStamp(currentAuthorityLastPresentedStamp, MICHELLE_HELD_FUTURE_DESTINY_LEAD), legacyLastSentStamp);
    const legacyLastSentUntrustedMissileActivePresentationLane = hasLegacyLastSentStamp &&
        (spaceState.lastSentDestinyWasOwnerCritical !== true ||
            legacyLastSentCriticalFlagLooksInherited) &&
        hasDestinyStamp(legacyOwnerMissileLifecycleStamp) &&
        isDestinyStampAfter(advanceDestinyStamp(legacyOwnerMissileLifecycleStamp, MICHELLE_HELD_FUTURE_DESTINY_LEAD), legacyLastSentStamp) &&
    ...
    const legacyLastSentUntrustedLane = legacyLastSentUntrustedFarAheadOwnerCommand ||
        legacyLastSentUntrustedMissileActivePresentationLane;
    const trustedLegacyLastSentStamp = legacyLastSentUntrustedLane ? null : legacyLastSentStamp;
    const trustedLegacyLastSentRawDispatchStamp = legacyLastSentUntrustedLane ? null : legacyLastSentRawDispatchStamp;
    const legacyLastSentWasOwnerCritical = typeof spaceState.lastSentDestinyWasOwnerCritical === "boolean"
        ? spaceState.lastSentDestinyWasOwnerCritical === true
        : (hasLegacyLastSentStamp &&
    ...
    const currentLastPresentedStamp = resolveOptionalDestinyStamp(currentState.lastPresentedStamp, trustedLegacyLastSentStamp);
    const legacyLastSentSupersedesAuthority = legacyLastSentUntrustedLane !== true &&
        hasLegacyLastSentStamp &&
        (!hasDestinyStamp(currentState.lastPresentedStamp) ||
            ...
            (legacyLastSentStamp === currentLastPresentedStamp &&
                hasLegacyLastSentRawDispatchStamp &&
                (!hasDestinyStamp(currentState.lastRawDispatchStamp) ||
                    isDestinyStampAfter(currentAuthorityLastRawDispatchStamp, legacyLastSentRawDispatchStamp))));
    const hasExplicitLegacyLastSentWasOwnerCritical = typeof spaceState.lastSentDestinyWasOwnerCritical === "boolean";
    const hasExplicitLegacyLastSentOnlyStaleProjectedOwnerMissileLane = typeof spaceState.lastSentDestinyOnlyStaleProjectedOwnerMissileLane ===
        "boolean";
    const legacyLastSentMatchesAuthorityTuple = legacyLastSentUntrustedLane !== true &&
        hasLegacyLastSentStamp &&
        hasDestinyStamp(currentState.lastPresentedStamp) &&
        legacyLastSentStamp === currentAuthorityLastPresentedStamp &&
        ((hasLegacyLastSentRawDispatchStamp &&
    ...
    const currentOrFallbackStamp = (key, fallback = null) => (hasDestinyStamp(currentState[key])
        ? normalizeDestinyStamp(currentState[key])
        : resolveOptionalDestinyStamp(fallback));
    const hasCurrentPresentedLane = hasDestinyStamp(currentState.lastPresentedStamp);
    ...
    const initialPresentedOnlyStaleProjectedOwnerMissileLane = hasCurrentPresentedLane
    ...
    const nextState = {
        ...
        lastCriticalStamp: currentOrFallbackStamp("lastCriticalStamp", selectLatestPresentDestinyStamp([
        ...
        lastNonCriticalStamp: currentOrFallbackStamp("lastNonCriticalStamp", legacyLastSentUntrustedLane || legacyLastSentWasOwnerCritical === true
            ? null
            : trustedLegacyLastSentStamp),
        lastSentWasOwnerCritical: initialPresentedWasOwnerCritical,
        lastSentOnlyStaleProjectedOwnerMissileLane: initialPresentedOnlyStaleProjectedOwnerMissileLane,
        lastOwnerCommandStamp: currentOrFallbackStamp("lastOwnerCommandStamp"),
        lastOwnerCommandAnchorStamp: currentOrFallbackStamp("lastOwnerCommandAnchorStamp"),
        lastOwnerCommandRawDispatchStamp: currentOrFallbackStamp("lastOwnerCommandRawDispatchStamp"),
        lastOwnerCommandHeadingHash: typeof currentState.lastOwnerCommandHeadingHash === "string"
            ...
            : "",
        lastFreshAcquireLifecycleStamp: currentOrFallbackStamp("lastFreshAcquireLifecycleStamp", spaceState.lastFreshAcquireLifecycleStamp),
        lastBootstrapStamp: currentOrFallbackStamp("lastBootstrapStamp", selectLatestPresentDestinyStamp([
            spaceState.lastFreshAcquireLifecycleStamp,
            legacyOwnerMissileFreshAcquireStamp,
        ])),
        lastMissileLifecycleStamp: currentOrFallbackStamp("lastMissileLifecycleStamp"),
        lastMissileLifecycleRawDispatchStamp: currentOrFallbackStamp("lastMissileLifecycleRawDispatchStamp"),
        lastOwnerMissileLifecycleStamp: currentOrFallbackStamp("lastOwnerMissileLifecycleStamp"),
        lastOwnerMissileLifecycleAnchorStamp: currentOrFallbackStamp("lastOwnerMissileLifecycleAnchorStamp"),
        lastOwnerMissileLifecycleRawDispatchStamp: currentOrFallbackStamp("lastOwnerMissileLifecycleRawDispatchStamp"),
        lastOwnerMissileFreshAcquireStamp: currentOrFallbackStamp("lastOwnerMissileFreshAcquireStamp"),
        lastOwnerMissileFreshAcquireAnchorStamp: currentOrFallbackStamp("lastOwnerMissileFreshAcquireAnchorStamp"),
        lastOwnerMissileFreshAcquireRawDispatchStamp: currentOrFallbackStamp("lastOwnerMissileFreshAcquireRawDispatchStamp"),
        lastOwnerNonMissileCriticalStamp: currentOrFallbackStamp("lastOwnerNonMissileCriticalStamp"),
        lastOwnerNonMissileCriticalRawDispatchStamp: currentOrFallbackStamp("lastOwnerNonMissileCriticalRawDispatchStamp"),
    ...
    const preserveCurrentTrackedLaneTuple = ({ stampKey, rawKey = "", anchorKey = "", }) => {
        ...
        if (anchorKey) {
            nextState[anchorKey] = resolveOptionalDestinyStamp(currentState[anchorKey]);
        }
    };
    ...
    const mergeLegacyTrackedLane = ({ stampKey, rawKey = "", anchorKey = "", legacyStamp, legacyRawDispatchStamp = null, legacyAnchorStamp = null, requireRawDispatch = false, }) => {
        if (!hasDestinyStamp(legacyStamp)) {
            return;
        }
        const normalizedLegacyStamp = normalizeDestinyStamp(legacyStamp);
        const hasLegacyRawDispatchStamp = hasDestinyStamp(legacyRawDispatchStamp);
        ...
        const hasCurrentRawDispatchStamp = rawKey && hasDestinyStamp(currentState[rawKey]);
        const currentRawDispatchStamp = hasCurrentRawDispatchStamp
            ? normalizeDestinyStamp(nextState[rawKey])
            : null;
        if (hasCurrentStamp) {
            const legacyMatchesSupersedingPresentedTuple = legacyLastSentSupersedesAuthority &&
            ...
            const legacyRawIsLater = rawKey &&
                hasLegacyRawDispatchStamp &&
                hasCurrentRawDispatchStamp &&
                (normalizedLegacyStamp === currentStamp ||
                    isDestinyStampAfter(currentStamp, normalizedLegacyStamp)) &&
                isDestinyStampAfter(currentRawDispatchStamp, normalizedLegacyRawDispatchStamp);
            const sameRawLegacyLaneIsLater = rawKey &&
                hasLegacyRawDispatchStamp &&
                hasCurrentRawDispatchStamp &&
                normalizedLegacyRawDispatchStamp === currentRawDispatchStamp &&
                isDestinyStampAfter(currentStamp, normalizedLegacyStamp);
            const stampOnlyLegacyLaneIsLater = !rawKey &&
                isDestinyStampAfter(currentStamp, normalizedLegacyStamp);
            if (!legacyRawIsLater &&
        ...
        const legacyMatchesCurrentSemanticStamp = hasDestinyStamp(currentStamp) &&
            normalizedLegacyStamp === currentStamp;
        ...
        if (stampKey === "lastOwnerCommandStamp" &&
            (!legacyMatchesCurrentSemanticStamp ||
                serializeDestinyDirectionHeading(spaceState.lastPilotCommandDirection))) {
    ...
    mergeLegacyTrackedLane({
        ...
        legacyRawDispatchStamp: spaceState.lastMissileLifecycleRawDispatchStamp,
    });
    const canonicalizeTrackedLane = ({ stampKey, rawKey = "", anchorKey = "", headingKey = "", }) => {
    ...
    if (legacyLastSentUntrustedLane) {
        spaceState.lastSentDestinyStamp = resolveOptionalDestinyStamp(nextState.lastPresentedStamp);
        spaceState.lastSentDestinyRawDispatchStamp = resolveOptionalDestinyStamp(nextState.lastRawDispatchStamp);
    ...
    if (legacyLastSentSupersedesAuthority ||
        ...
        if (nextState.lastSentWasOwnerCritical) {
            nextState.lastCriticalStamp = selectLaterDestinyStamp(nextState.lastCriticalStamp, legacyLastSentStamp);
        }
        else {
            nextState.lastNonCriticalStamp = selectLaterDestinyStamp(nextState.lastNonCriticalStamp, legacyLastSentStamp);
        }
    }
...
function updateDestinyAuthorityState(session, patch = {}) {
    ...
    for (const descriptor of groupedLaneDescriptors) {
        const { stampKey, rawKey = "", anchorKey = "", stringKey = "", booleanKeys = [], } = descriptor;
        const groupedKeys = [
            stampKey,
            rawKey,
            anchorKey,
        ...
        const stageCompanion = (key) => {
            ...
            return {
                accepted: !hasDestinyStamp(current) ||
                    candidate === current ||
...
//# sourceMappingURL=destinySessionState.js.map