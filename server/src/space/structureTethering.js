...
const structureTetherRestrictionState = require(path.join(__dirname, "../services/structure/structureTetherRestrictionState"));
...
function normalizeTetherRepairEffectMode(value, fallback = TETHER_REPAIR_EFFECT_MODE.GENERIC) {
    ...
    switch (normalized) {
        ...
        default:
            return normalizeTetherRepairEffectMode(fallback, TETHER_REPAIR_EFFECT_MODE.GENERIC);
    }
}
...
function hasWeaponTimer(entity, nowMs = Date.now()) {
    const characterID = toInt(entity &&
        entity.session &&
        entity.session.characterID, 0);
    if (characterID <= 0) {
        return false;
    }
...
function hasMobileAnalysisBeaconTetherBlock(entity, nowMs = Date.now()) {
    return toFiniteNumber(entity && entity.mobileAnalysisBeaconTetherBlockedUntilMs, 0) > toFiniteNumber(nowMs, Date.now());
}
function hasTetherDisallowingModuleEffect(entity) {
    if (!entity || !(entity.activeModuleEffects instanceof Map)) {
        return false;
    }
    for (const effectState of entity.activeModuleEffects.values()) {
        if (effectState &&
            ...
            effectState.disallowsTethering === true) {
            return true;
        }
    }
    return false;
...
function resolveEligibleTetherStructure(scene, entity, nowMs = Date.now(), helpers = {}) {
    ...
    if (hasTetherDisallowingModuleEffect(entity)) {
        return {
            eligible: false,
            reason: "MODULE_DISALLOWS_TETHERING",
            structure: null,
        };
    }
    const lockedTargetCount = toInt(helpers.getLockedTargetCount && helpers.getLockedTargetCount(entity), 0);
    const pendingTargetCount = toInt(helpers.getPendingTargetLockCount && helpers.getPendingTargetLockCount(entity), 0);
    const targetedByCount = toInt(helpers.getTargetedByCount && helpers.getTargetedByCount(entity), 0);
    ...
    const tetherRestriction = structureTetherRestrictionState.getCharacterStructureTetherRestriction(entity.session && entity.session.characterID, nowMs, {
    ...
    const candidates = Array.isArray(scene.staticEntities)
        ? scene.staticEntities.filter((candidate) => (candidate &&
    ...
    for (const candidate of sortedCandidates) {
        ...
        const structureRecord = structureState.getStructureByID(toInt(structureEntity && structureEntity.itemID, 0), {
            refresh: false,
        }) ||
            structureEntity;
        ...
        const dockCheck = structureState.canCharacterDockAtStructure(entity.session, structureRecord, {
            shipTypeID: toInt(entity.session.shipTypeID, 0) ||
                toInt(entity.typeID, 0),
        });
        if (!dockCheck.success) {
            continue;
        }
        const tetherRange = Math.max(0, toFiniteNumber(structureRecord && structureRecord.tetheringRange, toFiniteNumber(structureEntity && structureEntity.tetheringRange, 0)));
...
//# sourceMappingURL=structureTethering.js.map