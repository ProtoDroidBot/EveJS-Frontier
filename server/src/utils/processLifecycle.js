...
const DEFAULT_PROCESS_LIFECYCLE_LOG_PATH = path.join(__dirname, "../../logs/process-lifecycle.log");
const DEFAULT_NODE_REPORT_DIR = path.join(__dirname, "../../logs/node-reports");
function appendLifecycleLog(level, message, logPath = DEFAULT_PROCESS_LIFECYCLE_LOG_PATH) {
    try {
        rotatingLog.appendSync(logPath, `[${new Date().toISOString()}] [${level}] ${message}\n`);
    }
    catch (error) {
        try {
            console.error(`[ProcessLifecycle] failed to write ${logPath}: ${error.message}`);
        }
        catch (_) {
...
function writeDiagnosticReport(processRef, trigger, error, reportDir = DEFAULT_NODE_REPORT_DIR, logPath = DEFAULT_PROCESS_LIFECYCLE_LOG_PATH) {
    try {
        if (!processRef.report ||
        ...
        const safeTrigger = String(trigger || "process")
            .toLowerCase()
            .replace(/[^a-z0-9._-]+/g, "-");
        const reportPath = path.join(reportDir, `${safeTrigger}-${Date.now()}-${processRef.pid || "pid"}.json`);
        processRef.report.writeReport(reportPath, error);
        return reportPath;
    }
    catch (reportError) {
        appendLifecycleLog("ERR", `[ProcessLifecycle] failed to write diagnostic report for ${trigger}: ${formatValue(reportError)}`, logPath);
        if (typeof log.writeServerLog === "function") {
            log.writeServerLog("ERR", `[ProcessLifecycle] failed to write diagnostic report for ${trigger}: ${formatValue(reportError)}`);
        }
        return null;
    }
}
function emitLifecycleLog(logger, level, message, { consoleOutput = true, logPath = DEFAULT_PROCESS_LIFECYCLE_LOG_PATH, } = {}) {
...
function installProcessLifecycleLogging(options = {}) {
    ...
    const lifecycleLogPath = options.lifecycleLogPath || DEFAULT_PROCESS_LIFECYCLE_LOG_PATH;
    ...
    const handlers = {
        ...
        uncaughtExceptionMonitor: (error, origin) => {
            const reportPath = writeDiagnosticReport(processRef, "uncaught-exception", error, nodeReportDir, lifecycleLogPath);
            let message = `[ProcessLifecycle] uncaughtException app=${appName} origin=${origin || "unknown"} detail=${formatValue(error)} snapshot=${formatSnapshot(processRef)}`;
            if (reportPath) {
                message += ` report=${reportPath}`;
            }
            emitLifecycleLog(logger, "ERR", message, {
        ...
        beforeExit: (code) => {
            emitLifecycleLog(logger, "LOG", `[ProcessLifecycle] beforeExit app=${appName} code=${code} snapshot=${formatSnapshot(processRef)}`, { logPath: lifecycleLogPath });
        },
        exit: (code) => {
            emitLifecycleLog(logger, "LOG", `[ProcessLifecycle] exit app=${appName} code=${code} snapshot=${formatSnapshot(processRef)}`, { consoleOutput: false, logPath: lifecycleLogPath });
        },
        signal(signal) {
            emitLifecycleLog(logger, "WRN", `[ProcessLifecycle] signal app=${appName} signal=${signal} snapshot=${formatSnapshot(processRef)}`, { logPath: lifecycleLogPath });
        },
    };
