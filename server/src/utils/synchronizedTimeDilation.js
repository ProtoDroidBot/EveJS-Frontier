...
function sendTimeDilationNotificationToSystem(systemID, factor) {
    ...
    for (const targetSession of sessionRegistry.getSessions()) {
        const targetSystemID = Number(targetSession &&
        ...
        targetSession.sendNotification("OnSetTimeDilation", "clientID", notificationArgs);
        sentCount += 1;
    }
    return sentCount;
}
function sendTimeDilationNotificationToSession(session, factor) {
    ...
    const deliveryResult = session.sendNotification("OnSetTimeDilation", "clientID", buildTimeDilationNotificationArgs(factor));
    return deliveryResult !== false;
}
function getCurrentSystemFactor(systemID) {
    const spaceRuntime = require(path.join(__dirname, "../space/runtime"));
    return clampTimeDilationFactor(spaceRuntime.getSolarSystemTimeDilation(systemID));
}
function getSystemsNeedingTimeDilationChange(systemIDs, factor, options = {}) {
    const normalizedFactor = clampTimeDilationFactor(factor);
    const getCurrentSystemFactorFn = typeof options.getCurrentSystemFactorFn === "function"
        ? options.getCurrentSystemFactorFn
        : getCurrentSystemFactor;
    return normalizeSystemIDs(systemIDs).filter((systemID) => {
        const currentFactor = clampTimeDilationFactor(getCurrentSystemFactorFn(systemID));
...
function applyTimeDilationToSystem(systemID, factor) {
    const spaceRuntime = require(path.join(__dirname, "../space/runtime"));
    const normalizedFactor = clampTimeDilationFactor(factor);
    const currentFactor = clampTimeDilationFactor(spaceRuntime.getSolarSystemTimeDilation(systemID));
...
function normalizeSystemIDs(systemIDs) {
    return [...new Set((Array.isArray(systemIDs) ? systemIDs : [])
            .map((systemID) => Number(systemID) || 0)
            .filter((systemID) => systemID > 0))];
}
function scheduleSynchronizedTimeDilationForSystems(systemIDs, factor, options = {}) {
    const normalizedFactor = clampTimeDilationFactor(factor);
    const uniqueSystemIDs = getSystemsNeedingTimeDilationChange(systemIDs, normalizedFactor, options);
    ...
    return setTimeoutFn(() => {
        const systemsToApply = getSystemsNeedingTimeDilationChange(uniqueSystemIDs, normalizedFactor, options);
...
function scheduleAdvanceNoticeTimeDilationForSystems(systemIDs, factor, options = {}) {
    const normalizedFactor = clampTimeDilationFactor(factor);
    const uniqueSystemIDs = getSystemsNeedingTimeDilationChange(systemIDs, normalizedFactor, options);
    const delayMs = Number.isFinite(Number(options.delayMs))
        ? Number(options.delayMs)
...
//# sourceMappingURL=synchronizedTimeDilation.js.map