...
const { TIDI_ADVANCE_NOTICE_MS, scheduleAdvanceNoticeTimeDilationForSystems, } = require(path.join(__dirname, "./synchronizedTimeDilation"));
...
function intervalToFactor(actualIntervalMs, targetTickIntervalMs = DEFAULT_TARGET_TICK_INTERVAL_MS) {
    const targetMs = normalizePositiveNumber(targetTickIntervalMs, DEFAULT_TARGET_TICK_INTERVAL_MS);
    const actualMs = Math.max(targetMs, normalizePositiveNumber(actualIntervalMs, targetMs));
...
function normalizeRuntimeTickSample(sample = {}) {
    const targetTickIntervalMs = normalizePositiveNumber(sample.targetTickIntervalMs, DEFAULT_TARGET_TICK_INTERVAL_MS);
    const actualIntervalMs = Math.max(targetTickIntervalMs, normalizePositiveNumber(sample.actualIntervalMs, targetTickIntervalMs));
    const tickDurationMs = Math.max(0, normalizePositiveNumber(sample.tickDurationMs, 0));
    return {
...
function addSampleToControlWindow(windowState, sample) {
    ...
    windowState.maxActualIntervalMs = Math.max(windowState.maxActualIntervalMs, sample.actualIntervalMs);
    windowState.maxTickDurationMs = Math.max(windowState.maxTickDurationMs, sample.tickDurationMs);
    windowState.maxLatenessMs = Math.max(windowState.maxLatenessMs, sample.latenessMs);
    windowState.maxSceneCount = Math.max(windowState.maxSceneCount, sample.sceneCount);
    return windowState;
}
function finalizeControlWindow(windowState) {
    const sampleCount = Math.max(1, Number(windowState && windowState.sampleCount) || 1);
    const targetTickIntervalMs = normalizePositiveNumber(windowState && windowState.targetTickIntervalMs, DEFAULT_TARGET_TICK_INTERVAL_MS);
    const avgActualIntervalMs = (normalizePositiveNumber(windowState && windowState.actualIntervalTotalMs, targetTickIntervalMs) /
        sampleCount);
    const avgTickDurationMs = (Math.max(0, Number(windowState && windowState.tickDurationTotalMs) || 0) /
        sampleCount);
    const effectiveIntervalMs = Math.max(targetTickIntervalMs, avgActualIntervalMs, avgTickDurationMs);
    return {
        startedAtMonotonicMs: roundTo(Number(windowState && windowState.startedAtMonotonicMs), 3),
        endedAtMonotonicMs: roundTo(Number(windowState && windowState.endedAtMonotonicMs), 3),
        sampleCount,
        targetTickIntervalMs,
        avgActualIntervalMs: roundTo(avgActualIntervalMs, 3),
        avgTickDurationMs: roundTo(avgTickDurationMs, 3),
        maxActualIntervalMs: roundTo(normalizePositiveNumber(windowState && windowState.maxActualIntervalMs, targetTickIntervalMs), 3),
        maxTickDurationMs: roundTo(Math.max(0, Number(windowState && windowState.maxTickDurationMs) || 0), 3),
        maxLatenessMs: roundTo(Math.max(0, Number(windowState && windowState.maxLatenessMs) || 0), 3),
        sceneCount: Math.max(0, Math.trunc(Number(windowState && windowState.maxSceneCount) || 0)),
        windowElapsedMs: roundTo(Math.max(targetTickIntervalMs, Number(windowState && windowState.endedAtMonotonicMs) -
            Number(windowState && windowState.startedAtMonotonicMs)), 3),
...
function logTidiHold(metrics, factor, reason, transitionState = null) {
    ...
    log.flushStack();
    console.log(`${pc.dim(timestamp)} ${LABEL} ${pc.cyan("= hold")} factor ${factorStr} ` +
        `(${pc.yellow(intervalStr)} avg tick, ${pc.yellow(durationStr)} work, ${pc.yellow(lateStr)} max behind, ${solarSystemLabel}, ${reason}${suffix})`);
    appendTimeDilationLog(`HOLD factor=${Number(factor).toFixed(3)} reason=${reason} avgTickMs=${intervalStr} workMs=${durationStr} maxBehindMs=${lateStr} solarSystems=${sceneCount}` +
...
function filterAutoscaleEligibleSystemIDs(systemIDs) {
    return (Array.isArray(systemIDs) ? systemIDs : [])
        ...
        .filter((systemID, index, values) => (systemID > 0 &&
...
function getTransitionLockState(nowMonotonicMs = getCurrentMonotonicMs()) {
    ...
    const normalizedNowMonotonicMs = normalizePositiveNumber(nowMonotonicMs, getCurrentMonotonicMs());
    if (transitionLock.phase === "hold" &&
    ...
    return {
        ...transitionLock,
        remainingMs: Math.max(0, roundTo(Number(transitionLock.phase === "pending"
            ? transitionLock.applyAtMonotonicMs
            : transitionLock.holdUntilMonotonicMs) - normalizedNowMonotonicMs, 3)),
    };
}
function setManualOverride(systemID, factor) {
    ...
    appendTimeDilationLog(`MANUAL_OVERRIDE systemID=${override.systemID} factor=${override.factor.toFixed(3)}`);
    return { ...override };
}
...
function scheduleAutoscaledFactorChange(factor, options = {}) {
    ...
    const announcedAtMonotonicMs = normalizePositiveNumber(options.nowMonotonicMs, getCurrentMonotonicMs());
    ...
    transitionLock = {
        ...
        holdUntilMonotonicMs: roundTo(applyAtMonotonicMs + TRANSITION_HOLD_MS, 3),
    };
    const schedule = typeof options.scheduleChange === "function"
        ? options.scheduleChange
        : (targetSystemIDs, targetFactor) => scheduleAdvanceNoticeTimeDilationForSystems(targetSystemIDs, targetFactor, {
            ...
            onApplied: () => {
                ...
                transitionLock = {
                    ...
                    holdUntilMonotonicMs: roundTo(appliedAtMonotonicMs + TRANSITION_HOLD_MS, 3),
                };
...
function evaluateWindowMetrics(metrics, options = {}) {
    ...
    const transitionState = getTransitionLockState(metrics && metrics.endedAtMonotonicMs);
    if (transitionState) {
        resetRelaxationState();
        if (shouldLogHold && currentFactor < 1.0) {
            logTidiHold(metrics, currentFactor, transitionState.phase === "pending" ? "transition-pending" : "transition-hold", transitionState);
        }
        return {
            changed: false,
...
function observeRuntimeTickSampleInternal(sample, options = {}) {
    ...
    const elapsedMs = (Number(controlWindow.endedAtMonotonicMs) -
...
function setRuntimeEnabled(value) {
    ...
    appendTimeDilationLog(`RUNTIME_ENABLED_OVERRIDE override=${runtimeEnabledOverride === null ? "null" : runtimeEnabledOverride} effective=${target} config=${isConfigEnabled()}`);
    return {
...
function logStartupStatus() {
    ...
    const timestamp = new Date().toISOString().slice(11, 19);
    if (enabled) {
        const line = `${pc.dim(timestamp)} ${LABEL} ${pc.cyan("autoscaler")} ${pc.bold(pc.green("enabled"))} ${pc.dim(`(lateness based, ${CONTROL_WINDOW_MS}ms window)`)}`;
        console.log(line);
        appendTimeDilationLog(`START enabled=true mode=lateness-based controlWindowMs=${CONTROL_WINDOW_MS} engageLatenessMs=${OVERLOAD_ENGAGE_LATENESS_MS} transitionHoldMs=${TRANSITION_HOLD_MS} advanceNoticeMs=${TIDI_ADVANCE_NOTICE_MS}`);
    }
    else {
        const line = `${pc.dim(timestamp)} ${LABEL} ${pc.cyan("autoscaler")} ${pc.dim("disabled")}`;
        console.log(line);
...
//# sourceMappingURL=tidiAutoscaler.js.map