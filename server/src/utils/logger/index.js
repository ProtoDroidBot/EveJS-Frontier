...
function startupSection(title, rows = [], options = {}) {
    ...
    line();
    console.log(`${titleRenderer(` ${String(title || "").trim()} `)}` +
    ...
    for (const row of normalizedRows) {
        ...
        console.log(`  ${accentColor(pc.bold(label))} ${valueColor(value)}`);
    }
    line();
}
function criticalAlert(title, rows = [], options = {}) {
    ...
    const subtitle = String(options.subtitle || "").trim();
    appendServerLog("CRT", `${header}${subtitle ? ` | ${subtitle}` : ""} | ${normalizedRows
        .map((row) => {
        if (!row) {
            return "";
        }
    ...
    if (subtitle) {
        console.error(pc.bgRed(pc.white(`!!! ${subtitle}`.padEnd(border.length - 3, " ") + "!!!")));
    }
    console.error(pc.bgRed(pc.white(`!!! UTC ${isoStamp}`.padEnd(border.length - 3, " ") + "!!!")));
    console.error(pc.bgRed(pc.white(border)));
    for (const row of normalizedRows) {
        if (!row) {
            continue;
        ...
        console.error(`${pc.red("!")} ${pc.red(renderedLabel)} ${pc.yellow(value || "-")}`);
    }
    console.error(pc.bgRed(pc.white(border)));
    console.error("");
}
...
function logAsciiLogo() {
    ...
    console.log("");
    console.log(` ${captionPad}${pc.bold(pc.white("EveJS Elysian"))}${pc.dim("  local cluster bootstrap")}`);
    line();
}
