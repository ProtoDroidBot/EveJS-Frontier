...
const sessionChangeDebugPath = path.join(__dirname, "../../logs/session-change-debug.log");
...
function recordSpaceBootstrapTrace(session, event, details = {}) {
    ...
    try {
        ...
        log.debug(`[Session] Failed to record space bootstrap trace: ${error.message}`);
    }
    return false;
}
function summarizeOutgoingPacket(value, explicitMeta = null) {
    ...
    const sourceArgs = sourceAddress && Array.isArray(sourceAddress.args) ? sourceAddress.args : [];
    return {
        kind: "call-response",
        packetName,
        service: typeof sourceArgs[2] === "string" && sourceArgs[2].trim().length > 0
            ? sourceArgs[2]
            : null,
...
function appendSessionChangeDebug(entry) {
    ...
    try {
        ...
        log.warn(`[Session] Failed to write session change debug log: ${error.message}`);
    }
}
...
class ClientSession {
    ...
    constructor(handshakeData, socket, options = {}) {
        ...
        this.role = composeSessionRoleMask(this.accountRole, this.chatRole);
    ...
    sendSessionChange(changes, options = {}) {
        ...
        for (const [key, [oldVal, newVal]] of Object.entries(changes)) {
            if (!SESSION_CHANGE_ALLOWED_KEYS.has(key)) {
                appendSessionChangeDebug(`drop key=${key} old=${JSON.stringify(oldVal, (k, v) => (typeof v === "bigint" ? v.toString() : v))} new=${JSON.stringify(newVal, (k, v) => (typeof v === "bigint" ? v.toString() : v))}`);
                continue;
            }
            changeEntries.push([key, [oldVal, newVal]]);
        }
        const sessionID = options && Object.prototype.hasOwnProperty.call(options, "sessionId")
        ...
        const nodesOfInterest = normalizeSessionChangeNodesOfInterest(options, sourceNodeID);
        if (changeEntries.length === 0 && options.allowEmpty !== true) {
            appendSessionChangeDebug(`skip sid=${String(sessionID)} reason=empty payload=[]`);
            return;
        }
        appendSessionChangeDebug(`send sid=${String(sessionID)} keys=${JSON.stringify(changeEntries.map(([key]) => key))} payload=${JSON.stringify(changeEntries, (k, v) => (typeof v === "bigint" ? v.toString() : v))}`);
        ...
        log.pktOut("SessionChange", `${changeEntries.length} changes → ${changeEntries.map(([k]) => k).join(", ")}`);
        ...
        if (changeEntries.length > 0) {
            try {
                const { synchronizeSessionChatState, } = require(path.join(__dirname, "../services/chat/sessionChatSync"));
                if (typeof synchronizeSessionChatState === "function") {
                    synchronizeSessionChatState(this, Object.fromEntries(changeEntries));
                }
            }
            catch (error) {
                log.debug(`[Session] Skipped chat sync after session change: ${error.message}`);
            }
        }
    }
    /**
    ...
    sendClientCallRequest(serviceName, methodName, argsTuple = [], kwargs = null, options = {}) {
        ...
        let callID = Number(options.callID) > 0
            ...
            : 0;
        if (!callID) {
            this._nextServerCallID = Math.max(1, Math.trunc(Number(this._nextServerCallID || 610000)) + 1);
            callID = this._nextServerCallID;
        }
        const machoTimeout = Number(options.machoTimeout) > 0
    ...
    consumeClientCallResponse(packet) {
        ...
        const callID = Number(packet && packet.dest && packet.dest.callID
            ? packet.dest.callID
