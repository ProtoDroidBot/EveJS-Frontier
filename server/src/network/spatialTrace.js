...
function encodeLosslessValue(value, seen = new WeakMap(), valuePath = "$") {
    ...
    if (Array.isArray(value)) {
        return value.map((entry, index) => (encodeLosslessValue(entry, seen, `${valuePath}[${index}]`)));
    }
    ...
    if (value instanceof Set) {
        return {
            $type: "Set",
            values: [...value].map((entry, index) => (encodeLosslessValue(entry, seen, `${valuePath}.values[${index}]`))),
        };
    }
    ...
    for (const key of Object.keys(value)) {
        normalized[key] = encodeLosslessValue(value[key], seen, `${valuePath}.${key}`);
    }
    return normalized;
}
function appendRecord(record) {
    ...
    try {
        ...
        if (runtimeState.outputFd === null) {
            runtimeState.outputFd = fs.openSync(runtimeState.configuration.outputPath, "a");
        }
        const line = Buffer.from(`${JSON.stringify(encoded)}\n`, "utf8");
        let offset = 0;
        while (offset < line.length) {
            const written = fs.writeSync(runtimeState.outputFd, line, offset, line.length - offset, null);
...
function matchesFilters({ scene = null, session = null, entityIds = [], entityScoped = false, global = false, } = {}) {
    if (!isEnabled()) {
        return false;
    }
    const configuration = runtimeState.configuration;
    if (configuration.scenarioFilters.size > 0 &&
    ...
    if (configuration.systemFilters.size > 0 &&
    ...
    if (entityScoped && configuration.entityFilters.size > 0) {
        ...
        if (!candidates.some((entityId) => (runtimeState.relatedEntityIds.has(String(entityId))))) {
            return false;
        }
    }
    return true;
...
function emitEvent(event, { scene = null, session = null, entityIds = [], entityScoped = false, clock = {}, correlation = {}, data = {}, global = false, } = {}) {
...
function snapshotEntity(scene, entity) {
    ...
    const sceneMember = Boolean(scene &&
...
function recordWreckRecordCreated(scene, wreckRecord, details = {}) {
    ...
    const sourceEntityId = Number(wreckRecord.sourceEntityID !== undefined
        ? wreckRecord.sourceEntityID
        : wreckRecord.launcherID);
    const wreckEntityId = Number(wreckRecord.wreckID !== undefined
        ? wreckRecord.wreckID
        : wreckRecord.itemID);
    if (!Number.isSafeInteger(sourceEntityId) || sourceEntityId <= 0 ||
...
function recordDestinyActionAuthored(scene, session, update, details = {}) {
    ...
    const event = emitEvent("destiny.action.authored", {
        ...
        data: {
            captureBoundary: String(details.captureBoundary || "sendDestinyUpdates-pre-authority"),
...
function attachDestinyUpdateContext(update, context) {
    ...
    contexts.set(`${context.runId}:${context.sessionId || "unknown"}`, context);
    return update;
}
function propagateDestinyUpdateContext(sourceUpdate, targetUpdate, session) {
    if (!sourceUpdate || typeof sourceUpdate !== "object" ||
...
function recordDestinyGroupEnqueued(scene, session, details = {}) {
    ...
    const entityIds = uniqueEntityIds(details.updates.flatMap((update) => (extractTraceEntityIds(update && update.payload))));
...
function buildFallbackNotificationContext(session, payloadTuple) {
    ...
    const entityIds = uniqueEntityIds(updates.flatMap((update) => (extractTraceEntityIds(update.payload))));
    if (!matchesFilters({
        session,
        entityIds,
        entityScoped: runtimeState.configuration.entityFilters.size > 0,
...
function recordSerializedNotification(session, payloadTuple, buffer) {
    ...
    const event = emitEvent("destiny.notification.serialized", {
        ...
        data: {
            ...
            correlationComplete: context.actionContexts.length > 0 &&
...
//# sourceMappingURL=spatialTrace.js.map