...
function isLedgerFileEnabled() {
    return (process.env.EVEJS_SYNC_LEDGER === "1" ||
...
function getSessionSnapshot(session) {
    return {
        ...
        characterID: toInt(session && (session.characterID || session.charID || session.charid), 0) || null,
        shipID: toInt(session && session._space && session._space.shipID, toInt(session && session.shipID, 0)) || null,
        systemID: toInt(session && session._space && session._space.systemID, toInt(session && (session.solarsystemid2 || session.solarsystemid), 0)) || null,
...
function appendSessionRecord(session, record) {
    ...
    const maxEvents = Math.max(10, toInt(session._syncLedgerMaxEvents, DEFAULT_MAX_SESSION_EVENTS));
    if (session._syncLedgerEvents.length > maxEvents) {
        session._syncLedgerEvents.splice(0, session._syncLedgerEvents.length - maxEvents);
    }
}
...
function getSlimItemID(slimEntry) {
    ...
    const itemID = Number(slimItem && typeof slimItem === "object" && "itemID" in slimItem
        ? slimItem.itemID
...
//# sourceMappingURL=syncLedger.js.map