...
class BitWriter {
    ...
    writeBitsFromBuffer(buffer, bitCount = buffer.length * 8) {
        for (let bitIndex = 0; bitIndex < bitCount; bitIndex += 1) {
            ...
            const bit = (buffer[sourceByteIndex] >> sourceBitIndex) & 0x1;
            this.writeBit(bit);
        }
    }
...
function writeUnsigned32Var4(writer, value) {
    ...
    writer.writeLittleEndianNumber(normalized, groupCount >= 7 ? 32 : groupCount * 4);
}
function writeUnsigned64Var5(writer, value) {
    ...
    writer.writeLittleEndianNumber(normalized, groupCount >= 7 ? 64 : groupCount * 5);
}
...
function encodeBlueNetSingleHeader({ kind, targetClientID, masterID = BigInt(config.proxyNodeId || 0), transportID = 0, sourceStamp = 0n, sourceFlag = null, }) {
    const writer = new BitWriter();
...
function buildBlueNetFrame(headerBuffer, bodyBuffer) {
    ...
    const firstWord = ((headerBuffer.length + bodyBuffer.length + 4) |
        0x10000000) >>> 0;
...
function buildTimeDilationFrame(kind, options = {}) {
    ...
    const body = kind === TIDI_KINDS.DETACH
        ? Buffer.from([0])
...
//# sourceMappingURL=nativeTimeDilation.js.map