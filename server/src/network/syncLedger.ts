...
function normalizeDetails(value, depth = 0, seen = new Set()) {
  ...
  if (value instanceof Error) {
    return {
      ...
      code: (value as NodeJS.ErrnoException).code || undefined,
    };
  }
...
function recordSyncLedgerEvent(session, event, details: Record<string, any> = {}) {
...
function extractAddBalls2EntityIDs(args: any[] = []) {
  const entityIDs: any[] = [];
...
function summarizeDestinyUpdates(updates: any[] = []) {
  const names: any[] = [];
  const stamps: any[] = [];
  const entityIDs: any[] = [];
...
function summarizeDestinyNotificationPayload(payloadTuple: any[] = []) {
...
function summarizeNotificationPayload(notifyType, payloadTuple: any[] = []) {
