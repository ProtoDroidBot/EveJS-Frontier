  ...
  const server = net
    .createServer((socket) => {
      ...
      const queueInboundPacket = (data, rawPayload) => {
        ...
        if (queuedCodecPackets >= 64 || queuedCodecBytes + packetBytes > 32 * 1024 * 1024) {
          log.warn(
            `[TCP] closing ${socket.remoteAddress}: inbound codec queue limit exceeded`,
          );
          socket.destroy();
          return;
        }
        ...
        inboundCodecTail = inboundCodecTail
          ...
          .catch((err) => {
            ...
            logDebug(
              `[TCP] raw: ${rawPayload.toString("hex").substring(0, 80)}...`,
            );
            socket.destroy();
          })
          .finally(() => {
    ...
    .listen(config.serverPort, config.gameServerBindHost, () => {
      ...
      if (universeStatus && universeStatus.fullUpToDate === true) {
        ...
        log.info(
          "[DungeonUniverse] persistent universe site state was stale; startup reconciliation " +
