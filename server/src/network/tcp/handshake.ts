...
function buildTidiSignedFuncSource(_clientId?: unknown): string;
...
function buildMarshalDict(entries) {
  ...
  return {
    type: "dict",
    entries: Object.entries<any>(entries || {}),
  };
}
...
function encodePy2Protocol2Value(value) {
  ...
  if (typeof value === "object") {
    ...
    const chunks: any[] = [Buffer.from("}", "ascii")]; // EMPTY_DICT
...
class EVEHandshake {
  declare static _testing: Record<string, unknown>;
  ...
  _sendTransportClose(reasonCode, reasonArgs: Record<string, any> = {}, reason = reasonCode) {
