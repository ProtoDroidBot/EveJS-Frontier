...
function encodeValue(value, chunks, context: Record<string, any> = {}) {
  // null / undefined → PyNone
 ...
 */
function decodeMarshalStream(buffer, requireExactEnd, options: Record<string, any> = {}) {
  ...
  if (requireExactEnd) {
    if (state.pos !== valueEnd) {
      throw new Error(
        `Marshal stream has ${valueEnd - state.pos} unconsumed payload byte(s)`,
      );
    }
    if (
      state.saveIndicesPos !== null &&
...
function marshalDecode(buffer, options: Record<string, any> = {}) {
  return decodeMarshalStream(buffer, false, options);
}

function marshalDecodeExact(buffer, options: Record<string, any> = {}) {
