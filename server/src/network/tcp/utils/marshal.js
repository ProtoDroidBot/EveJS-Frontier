...
const { lookupIndex, lookupString, STRING_TABLE_ERROR } = require(path.join(__dirname, "../../../common/marshalStringTable"));
// ─── Opcodes ────────────────────────────────────────────────────────────────
const Op = {
    PyNone: 0x01,
    PyToken: 0x02,
...
function marshalEncode(value, options = {}) {
    ...
    const chunks = [];
    const context = {
        frontierText: String(options.compatibilityProfile || "").trim().toLowerCase() ===
            "frontier",
    };
    // Stream header
...
function validateMarshalComplexity(value, options = {}) {
    const maxDepth = normalizeMarshalLimit(options.maxDepth, DEFAULT_MAX_MARSHAL_DEPTH);
    const maxNodes = normalizeMarshalLimit(options.maxNodes, DEFAULT_MAX_MARSHAL_NODES);
    const stack = [{ value, depth: 1, exiting: false }];
...
function encodeValue(value, chunks, context = {}) {
    ...
    if (typeof value === "object" && value.type) {
        switch (value.type) {
            ...
            case "tuple":
                encodeTuple(Array.isArray(value.items) ? value.items : [], chunks, context);
                return;
            case "list":
            ...
            case "token": {
                const tokenValue = context.frontierText && value.value === "__builtin__.set"
                    ? "builtins.set"
                    : value.value;
    ...
    throw new Error(`Cannot marshal value: ${typeof value} ${JSON.stringify(value)}`);
}
...
function normalizePackedRowVirtualColumns(rowHeader) {
    const virtualColumnPayload = rowHeader &&
        rowHeader.type &&
...
function normalizePackedRowValueMap(packedRow, columns) {
    if (Array.isArray(packedRow.values)) {
        if (packedRow.values.length !== columns.length) {
            throw new Error(`Packed row values length ${packedRow.values.length} does not match column count ${columns.length}`);
        }
        return packedRow.values;
    }
    if (packedRow.fields && typeof packedRow.fields === "object") {
        return columns.map(([name]) => Object.prototype.hasOwnProperty.call(packedRow.fields, name)
...
function decompressRle(buffer, expectedLength) {
    ...
    while (inputIndex < buffer.length && outputIndex < targetLength) {
        ...
        for (let nibbleIndex = 0; nibbleIndex < 2 && outputIndex < targetLength; nibbleIndex += 1) {
            const nibble = nibbleIndex === 0 ? control & 0x0f : (control >> 4) & 0x0f;
            if (nibble < 8) {
                const literalCount = 8 - nibble;
                const available = Math.min(literalCount, buffer.length - inputIndex, targetLength - outputIndex);
                if (available > 0) {
...
function decodePackedRowFields(rowHeader, rleData, state) {
    ...
    const packedByteData = decompressRle(rleData, byteDataByteLength + bitDataByteLength);
    const bitData = packedByteData.subarray(byteDataByteLength, byteDataByteLength + bitDataByteLength);
    ...
    for (const entry of sizeMap) {
        ...
        const isNull = nullByte < bitData.length &&
...
function decodeMarshalStream(buffer, requireExactEnd, options = {}) {
    ...
    const state = {
        ...
        maxDepth: normalizeMarshalLimit(options.maxDepth, DEFAULT_MAX_MARSHAL_DEPTH),
        maxNodes: normalizeMarshalLimit(options.maxNodes, DEFAULT_MAX_MARSHAL_NODES),
        limitState: options.limitState || { nodeCount: 0 },
    };
    // Read and verify header
    const header = readUInt8(state);
    if (header !== MARSHAL_HEADER) {
        throw new Error(`Invalid marshal header: 0x${header.toString(16)} (expected 0x7E)`);
    }
    // Read save count
    const saveCount = readUInt32LE(state);
    // Initialize stored objects array
    let valueEnd = buffer.length;
    if (saveCount > 0) {
        const saveIndicesBytes = saveCount * 4;
        if (buffer.length - state.pos < saveIndicesBytes) {
            throw new Error(`Invalid marshal save table: need ${saveIndicesBytes} bytes, have ${buffer.length - state.pos}`);
        }
        ...
        valueEnd = state.saveIndicesPos;
    }
    const value = decodeValue(state);
    if (requireExactEnd) {
        if (state.pos !== valueEnd) {
            throw new Error(`Marshal stream has ${valueEnd - state.pos} unconsumed payload byte(s)`);
        }
        if (state.saveIndicesPos !== null &&
            state.saveIndicesPos !== state.saveIndicesEnd) {
            throw new Error("Marshal stream has unconsumed save-table entries");
        }
    }
    return value;
}
...
function readBytes(state, len) {
    if (!Number.isSafeInteger(len) || len < 0 || state.pos + len > state.buf.length) {
        throw new Error(`Invalid marshal byte length ${len} at position ${state.pos}`);
    }
...
function decodeValueUnchecked(state) {
    ...
    switch (op) {
        ...
        default:
            throw new Error(`Unknown marshal opcode: 0x${rawOp.toString(16)} at position ${state.pos - 1}`);
    }
...
function strVal(v) {
    ...
    if (v && typeof v === "object" && v.type === "wstring")
        return v.value;
    if (v && typeof v === "object" && v.type === "token")
        return v.value;
    if (v && typeof v === "object" && v.type === "rawstr")
        return v.value;
    return String(v);
}
/**
