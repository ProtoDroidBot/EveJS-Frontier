...
export interface SocketTuningConfig {
  socketKeepAliveEnabled?: boolean;
  socketKeepAliveInitialDelayMs?: number;
  socketNoDelayEnabled?: boolean;
  socketIdleTimeoutMs?: number;
}

export interface SocketTuningResult {
  keepAlive: boolean;
 ...
 */
function configureClientSocket(socket: Socket, config: SocketTuningConfig = {}, log: { warn(message: string): void } | null = null): SocketTuningResult {
