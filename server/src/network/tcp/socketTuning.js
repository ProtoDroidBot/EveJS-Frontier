...
function configureClientSocket(socket, config = {}, log = null) {
    ...
    const keepAliveInitialDelayMs = toNonNegativeInteger(config.socketKeepAliveInitialDelayMs, 30000);
    if (keepAliveEnabled) {
    ...
    if (idleTimeoutMs > 0 && typeof socket.setTimeout === "function") {
        socket.setTimeout(idleTimeoutMs, () => {
            if (log && typeof log.warn === "function") {
                log.warn(`[TCP] Idle timeout (${idleTimeoutMs}ms) reached for ` +
...
//# sourceMappingURL=socketTuning.js.map