...
const { marshalEncode, marshalDecode, wrapPacket, // unused
dictGet, strVal, bufVal, } = require(path.join(__dirname, "./utils/marshal"));
...
const { DEFAULT_CHAT_ROLE, buildPersistedAccountRoleRecord, roleToString, } = require(path.join(__dirname, "../../services/account/accountRoleProfiles"));
const { buildGlobalConfigDict, buildClientBootMetadataEntries, buildServerStatusDict, getRuntimeConfig, normalizeCountryCode, } = require(path.join(__dirname, "../../services/machoNet/globalConfig"));
const { reserveAccountID, } = require(path.join(__dirname, "../../services/_shared/identityAllocator"));
const { selectHandshakeSignedFunc, selectPlaceboChallengeResponseHash, selectVersionExchangeRelease, } = require(path.join(__dirname, "./handshakeCompatibility"));
...
function buildHandshakeSignedFunc(runtimeConfig, clientId) {
    return selectHandshakeSignedFunc(runtimeConfig.clientCompatibilityProfile, MARSHALED_NONE, () => buildTidiSignedFunc(clientId));
}
...
function buildGPSTransportClosedCPickle(reasonCode, reasonArgs = {}, reason = reasonCode) {
...
class EVEHandshake {
    ...
    start() {
        ...
        const serverStatusEntries = new Map(buildServerStatusDict(runtimeConfig).entries);
        const versionTuple = [
            ...
            selectVersionExchangeRelease(runtimeConfig.clientCompatibilityProfile, runtimeConfig.projectVersion, runtimeConfig.projectCodename, runtimeConfig.projectRegion),
            null, // update_info
        ];
    ...
    handlePacket(payload) {
        ...
        try {
            ...
            log.err(`[HANDSHAKE] Failed to decode packet in state ${this.state}: ${err.message}`);
            if (this.state === State.WAIT_VERSION) {
                log.debug(`[HANDSHAKE] VersionExchangeClient raw: ${decodable.toString("hex")}`);
            }
            // commented for flood prevention
            // log.debug(
        ...
        log.debug(`[HANDSHAKE] handshake state: ${this.state}`);
        // commented for flood prevention
        // log.debug(
    ...
    _handleVersion(decoded) {
        ...
        if (!Array.isArray(decoded) || decoded.length < 6) {
            log.err(`[HANDSHAKE] Invalid VersionExchangeClient: expected tuple of 6+`);
            log.debug(`[HANDSHAKE] VersionExchangeClient shape: ${this._summarize(decoded)}`);
            return { done: false };
        }
        // grab values from decoded
        const [birthday, machoVer, , versionNum, buildVer, projectVer] = decoded;
        log.debug(`[HANDSHAKE] Client version: birthday=${birthday} macho=${machoVer} ver=${versionNum} build=${buildVer} proj=${strVal(projectVer)}`);
        if (birthday !== config.eveBirthday)
            log.warn(`[HANDSHAKE] Client birthday mismatch! Expected ${config.eveBirthday}, got ${birthday}`);
        if (machoVer !== config.machoVersion)
            log.warn(`[HANDSHAKE] Client macho_version mismatch! Expected ${config.machoVersion}, got ${machoVer}`);
        // update state
    ...
    _handleCrypto(decoded, rawPayload) {
        ...
        if (keyParams && keyParams.type === "dict") {
            ...
            if (sessionKeyVal) {
                ...
                log.debug(`[HANDSHAKE] Session key: ${this.sessionKey ? this.sessionKey.length + " bytes" : "null"}`);
                if (this.sessionKey) {
                    log.debug(`[HANDSHAKE] Session key hex: ${this.sessionKey.toString("hex").substring(0, 64)}...`);
                }
            }
            if (sessionIVVal) {
                this.sessionIV = bufVal(sessionIVVal);
                log.debug(`[HANDSHAKE] Session IV: ${this.sessionIV ? this.sessionIV.length + " bytes" : "null"}`);
                if (this.sessionIV) {
                    log.debug(`[HANDSHAKE] Session IV hex: ${this.sessionIV.toString("hex").substring(0, 64)}...`);
                }
            }
            // If we got key and IV, encryption is active for subsequent packets
            if (this.sessionKey && this.sessionIV) {
                this.encrypted = true;
                log.debug(`[HANDSHAKE] Encryption mode: ACTIVE (key=${this.sessionKey.length}B, iv=${this.sessionIV.length}B)`);
                ...
                if (this.sessionKey.length > 32) {
                    log.debug(`[HANDSHAKE] Trimming session key from ${this.sessionKey.length} to 32 bytes`);
                    this.sessionKey = this.sessionKey.slice(0, 32);
                }
                if (this.sessionIV.length > 16) {
                    log.debug(`[HANDSHAKE] Trimming session IV from ${this.sessionIV.length} to 16 bytes`);
                    this.sessionIV = this.sessionIV.slice(0, 16);
                }
                log.debug(`[HANDSHAKE] Final key (${this.sessionKey.length}B): ${this.sessionKey.toString("hex")}`);
                log.debug(`[HANDSHAKE] Final IV  (${this.sessionIV.length}B): ${this.sessionIV.toString("hex")}`);
            }
        }
        // Send "OK CC"
    ...
    _handleAuthentication(decoded) {
        ...
        const userLanguageId = strVal(dictGet(loginData, "user_languageid") || "EN"); // returns a json object TODO: decode json and get language id
        /**
        ...
        // }
        log.debug(`[HANDSHAKE] Login attempt: user="${userName}" lang="${userLanguageId}"`);
        ...
        if (!rawAccount) {
            if (config.devAutoCreateAccounts) {
                const createdAccount = buildPersistedAccountRoleRecord({
                    ...
                    id: reserveAccountID(),
                    // Auto-created accounts are GM/admin by default on this dev server.
                    // buildPersistedAccountRoleRecord derives the role from isGM; drop an
                    // account to a normal player via the ConfigEditor Server Database tab.
                    isGM: true,
                    chatRole: DEV_CHAT_ROLE,
                    banned: false,
                });
                log.info(`[HANDSHAKE] Account "${userName}" not found - auto creating because devAutoCreateAccounts is enabled`);
        ...
        if (config.devSkipPasswordValidation) {
            log.debug(`[HANDSHAKE] Password accepted for "${userName}" because devSkipPasswordValidation is enabled`);
        }
        else {
            if (account.passwordhash !== passwordHash) {
                log.debug(`[HANDSHAKE] Password mismatch for "${userName}"`);
        ...
        if (loginFailureReasonCode) {
            log.debug(`[HANDSHAKE] Login attempt failed for "${userName}" with reason ${loginFailureReasonCode}`);
        ...
        this.countryCode = normalizeCountryCode(account.countryCode, config.defaultCountryCode);
            ...
            { type: "dict", entries: [] }, // context
            {
                ...
                entries: [
                    [
                        ...
                        selectPlaceboChallengeResponseHash(runtimeConfig.clientCompatibilityProfile, clientChallenge),
                    ],
    ...
    _handleFuncResult(decoded) {
        ...
        log.success(`[HANDSHAKE] Handshake complete for "${this.userName}" (id=${this.userId})`);
        return { done: true };
    }
    // ─── Encryption/Decryption ───────────────────────────────────────────────
    _decrypt(data) {
        const decipher = crypto.createDecipheriv("aes-256-cbc", this.sessionKey, this.sessionIV);
    ...
    _encrypt(data) {
        const cipher = crypto.createCipheriv("aes-256-cbc", this.sessionKey, this.sessionIV);
    ...
    _encodePacket(value) {
        return framePayload(marshalEncode(value, {
            compatibilityProfile: this.compatibilityProfile,
    ...
    _sendTransportClose(reasonCode, reasonArgs = {}, reason = reasonCode) {
        const closePacket = this._encodePacket({
            type: "cpicked",
            data: buildGPSTransportClosedCPickle(reasonCode, reasonArgs, reason),
        });
