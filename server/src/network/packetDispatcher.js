...
const { MACHONETMSG_TYPE, getTypeName } = require(path.join(__dirname, "../common/packetTypes"));
const { buildCacheOkMachoError, isCacheOkMachoError, isMachoWrappedException, } = require(path.join(__dirname, "../common/machoErrors"));
const { decodePacket, decodeCallRequest, encodePacketTuple } = require(path.join(__dirname, "../common/pyPacket"));
const { encodeAddress } = require(path.join(__dirname, "../common/machoAddress"));
const { isDeferredCallResponse, } = require(path.join(__dirname, "./callResponseControl"));
const serviceCallShapeCapture = require(path.join(__dirname, "../services/_shared/serviceCallShapeCapture"));
const { shouldReturnCacheOkForCachedMethodCall, } = require(path.join(__dirname, "../services/cache/objectCacheRuntime"));
...
function collectBoundObjectIDs(value, output, depth = 0) {
    ...
    if (Array.isArray(value)) {
        for (const entry of value) {
            collectBoundObjectIDs(entry, output, depth + 1);
        }
        return;
    }
    if (typeof value !== "object") {
    ...
    if (value.type === "dict" && Array.isArray(value.entries)) {
        for (const [entryKey] of value.entries) {
            collectBoundObjectIDs(entryKey, output, depth + 1);
        }
        return;
    }
    ...
    if (Object.prototype.hasOwnProperty.call(value, "value")) {
        collectBoundObjectIDs(value.value, output, depth + 1);
        return;
    }
...
function extractReleasedBoundObjectIDs(pkt, decodedNotification) {
    const released = new Set();
    if (decodedNotification) {
        collectBoundObjectIDs(decodedNotification.remoteObject, released);
    }
    for (const namedPayload of [pkt && pkt.namedPayload, pkt && pkt.oob]) {
        for (const [entryKey, entryValue] of extractDictEntries(namedPayload)) {
            if (String(entryKey) === "OID-") {
                collectBoundObjectIDs(entryValue, released);
            }
        }
    }
    return [...released];
...
function collectBoundObjectRegistrations(value, registrations) {
    ...
    const objectRegistration = substream && substream.type === "substream"
        ? substream.value
        : substream;
    if (Array.isArray(objectRegistration) &&
...
function recordSpaceBootstrapTrace(session, event, details = {}) {
    ...
    try {
        ...
        log.debug(`[PacketDispatcher] Failed to record space bootstrap trace: ${error.message}`);
    }
    return false;
}
...
function extractCachedObjectCompressedPart(value, seen = new Set()) {
    ...
    seen.add(value);
    if (Array.isArray(value)) {
        return value.reduce((total, entry) => total + extractCachedObjectCompressedPart(entry, seen), 0);
    }
    if (value.type === "object") {
        const objectName = normalizeObjectName(value.name);
        if (objectName === "objectCaching.CachedObject" ||
...
class PacketDispatcher {
    ...
    async dispatch(rawDecoded, session) {
        ...
        if (!pkt) {
            ...
            log.debug(`[PacketDispatcher] Raw type: ${typeof rawDecoded}, keys: ${rawDecoded && typeof rawDecoded === "object" ? Object.keys(rawDecoded).join(",") : "N/A"}`);
            return false;
        }
        log.pktIn(pkt.service || pkt.typeName, `${pkt.typeName} src=${pkt.source.type} dst=${pkt.dest.type}`);
    ...
    async _handleCallReq(pkt, session, rawDecoded = null) {
        ...
        log.pktIn(serviceName || "?", `${call.method}() callID=${callID}`);
        const lookedUpService = this.serviceManager && serviceName
            ? this.serviceManager.lookup(serviceName)
            : null;
        const resolvedServiceName = lookedUpService ? lookedUpService.name : serviceName;
        const lowerResolvedServiceName = String(resolvedServiceName || serviceName || "").toLowerCase();
        ...
        const traceSpaceCall = call.method === "Undock" ||
            ...
            lowerResolvedServiceName.includes("homestation");
        if (traceSpaceCall) {
            appendSpaceDebug(`CallReq service=${resolvedServiceName || serviceName || "?"} rawService=${serviceName || "?"} method=${call.method} callID=${callID} currentBound=${session && session.currentBoundObjectID ? session.currentBoundObjectID : "null"} args=${JSON.stringify(summarizeValue(call.args))} kwargs=${JSON.stringify(summarizeValue(call.kwargs))}`);
        }
        if (serviceName === "slash") {
            appendSlashDebug(`CallReq method=${call.method} callID=${callID} args=${JSON.stringify(summarizeValue(call.args))} kwargs=${JSON.stringify(summarizeValue(call.kwargs))} raw=${JSON.stringify(summarizeValue(call.raw))}`);
        }
        if (this.serviceManager && serviceName) {
            ...
            if (service) {
                ...
                try {
                    ...
                    const result = await service.callMethod(call.method, call.args, session, call.kwargs);
                    ...
                    if (handlerElapsedMs >= 100) {
                        log.info(`[PacketPerf] service=${service.name || resolvedServiceName || serviceName || "?"} ` +
                    ...
                    if (isDeferredCallResponse(result)) {
                        if (typeof result.start === "function") {
                            await result.start({
                                ...
                                service,
                                call,
                                responseServiceName: service.name || resolvedServiceName || serviceName || null,
                                responsePerfMeta: {
                                    kind: "call-response",
                    ...
                    if (shouldReturnCacheOkForCachedMethodCall(result, call.kwargs)) {
                        this._sendErrorResponse(pkt, buildCacheOkMachoError(), session, service.name || resolvedServiceName || serviceName || null);
                        return true;
                    }
                    ...
                    if (traceSpaceCall) {
                        appendSpaceDebug(`CallRsp service=${service.name} method=${call.method} callID=${callID} result=${JSON.stringify(summarizeValue(result))}`);
                    }
                    // Always send a response, even if result is null/undefined
                    this._sendCallResponse(pkt, result !== undefined ? result : null, session, service.name || resolvedServiceName || serviceName || null, {
                        kind: "call-response",
                        service: service.name || resolvedServiceName || serviceName || null,
                        method: call.method,
                        callID,
                    ...
                    if (typeof service.afterCallResponse === "function") {
                        try {
                            ...
                            if (afterCallElapsedMs >= 100) {
                                log.info(`[PacketPerf] afterCall service=${service.name || resolvedServiceName || serviceName || "?"} ` +
                                    `method=${call.method} callID=${callID} afterCallMs=${afterCallElapsedMs}`);
                            }
                        }
                        catch (afterCallError) {
                            log.warn(`[PacketDispatcher] afterCallResponse failed for ${service.name}.${call.method}: ${afterCallError.message}`);
                        }
                    }
                    return true;
                }
                catch (err) {
                    log.pktErr(serviceName || "?", `${call.method}() ${err.message}`);
                    if (traceSpaceCall) {
                        appendSpaceDebug(`CallErr service=${service.name} method=${call.method} callID=${callID} error=${err && err.stack ? err.stack : err.message}`);
                    }
                    if (isMachoWrappedException(err)) {
                        this._sendErrorResponse(pkt, err.machoErrorResponse, session, service.name || resolvedServiceName || serviceName || null);
                    }
                    else {
                        this._sendCallResponse(pkt, null, session, service.name || resolvedServiceName || serviceName || null, {
                            kind: "call-response",
                            service: service.name || resolvedServiceName || serviceName || null,
                            method: call.method,
                            callID,
                ...
                this._sendCallResponse(pkt, null, session, resolvedServiceName || serviceName || null);
            }
        }
        else {
    ...
    async _handleCallRsp(pkt, session) {
        const clientCallResponse = session && typeof session.consumeClientCallResponse === "function"
            ? session.consumeClientCallResponse(pkt)
            : null;
        if (clientCallResponse) {
            log.pktIn(clientCallResponse.service || "CallRsp", `${clientCallResponse.method || "client-call"}() response callID=${clientCallResponse.callID}`);
            if (typeof clientCallResponse.onResponse === "function") {
                try {
                    ...
                    log.warn(`[PacketDispatcher] client CallRsp callback failed for ${clientCallResponse.service || "?"}.${clientCallResponse.method || "?"}: ${error.message}`);
                }
            }
            return true;
        }
    ...
    _handleNotification(pkt, session) {
        const hasObjectReleaseMetadata = [pkt && pkt.namedPayload, pkt && pkt.oob]
            .some((payload) => extractDictEntries(payload)
            .some(([entryKey]) => String(entryKey) === "OID-"));
        const notification = hasObjectReleaseMetadata
            ? decodeCallRequest(pkt.payload)
            : null;
        if (notification && notification.method === "ClientHasReleasedTheseObjects") {
            const releasedObjectIDs = extractReleasedBoundObjectIDs(pkt, notification);
            let releasedCount = 0;
            for (const objectID of releasedObjectIDs) {
                if (this.serviceManager &&
                    typeof this.serviceManager.unregisterBoundObject === "function" &&
                    this.serviceManager.unregisterBoundObject(objectID, { session })) {
                    releasedCount += 1;
                }
            }
            log.pktIn("Notification", `ClientHasReleasedTheseObjects released=${releasedCount}/${releasedObjectIDs.length}`);
            return true;
        }
     ...
     */
    _sendCallResponse(pkt, result, session, responseServiceName = null, responsePerfMeta = null, responseOptions = {}) {
        if (!session || !session.sendPacket)
            return;
        const normalizedResponseOptions = responseOptions && typeof responseOptions === "object"
            ? responseOptions
            : {};
        ...
        let responseNamedPayload = mergeNamedPayloadEntries(null, Array.isArray(normalizedResponseOptions.namedPayloadEntries)
            ? normalizedResponseOptions.namedPayloadEntries
            : []);
        responseNamedPayload = mergeBoundObjectRegistrationsIntoNamedPayload(responseNamedPayload, boundObjectRegistrations);
        if (compressedPart > 0) {
            responseNamedPayload = mergeNamedPayloadEntries(responseNamedPayload, [
                ["compressedPart", compressedPart],
            ]);
        }
        const contextKey = Object.prototype.hasOwnProperty.call(normalizedResponseOptions, "contextKey")
        ...
        log.pktOut(pkt.dest.service || pkt.service || "CallRsp", `response callID=${callID}`);
    ...
    _sendErrorResponse(pkt, machoError, session, responseServiceName = null) {
        ...
        if (isCacheOk) {
            log.pktOut(pkt.dest.service || pkt.service || "ErrorRsp", `cacheOK callID=${callID}`);
        }
        else {
            log.pktErr(pkt.dest.service || pkt.service || "ErrorRsp", `wrapped exception callID=${callID}`);
        }
        session.sendPacket(responseObj);
    }
    /**
    ...
    _tryRegisterSubstruct(obj, service) {
        if (!obj || typeof obj !== "object" || obj.type !== "substruct")
            return;
        try {
            const substream = obj.value;
            const oid = substream && substream.type === "substream"
                ? substream.value
                : substream;
