...
function buildNamedPayloadDict(entries: any[] = []) {
  return {
    type: "dict",
...
function mergeNamedPayloadEntries(existingNamedPayload, extraEntries: any[] = []) {
  const existingEntries = extractDictEntries(existingNamedPayload);
  const mergedEntries: any[] = [...existingEntries];
...
function mergeBoundObjectRegistrationsIntoNamedPayload(existingNamedPayload, registrations: any[] = []) {
...
function extractCachedObjectCompressedPart(value, seen = new Set()) {
  ...
  let total = 0;
  for (const entryValue of Object.values<any>(value)) {
...
class PacketDispatcher {
  ...
  _handleNotification(pkt, session) {
    ...
    if (notification && notification.method === "ClientHasReleasedTheseObjects") {
      ...
      log.pktIn(
        "Notification",
        `ClientHasReleasedTheseObjects released=${releasedCount}/${releasedObjectIDs.length}`,
      );
      return true;
    }
  ...
  _handlePingReq(pkt, session) {
    ...
    const pingList: Record<string, any> = {
      type: "list",
      items: [
