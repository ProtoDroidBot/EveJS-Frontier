...
function normalizeSessionChangeNodesOfInterest(options: Record<string, any> = {}, sourceNodeID = 0) {
...
function buildNotificationNamedPayloadEntries(session): [string, unknown][] {
...
function summarizeAttributeChangeCounts(changes: any[] = []) {
...
function buildNotificationLogMessage(notifyType, idType, payloadTuple: any[] = []) {
...
function resolveBoundObjectRegistrationRefID(session, objectID) {
  ...
  for (const state of Object.values<any>(session._boundObjectState)) {
    if (
      state &&
      typeof state === "object" &&
...
class ClientSession {
  declare _decryptFn: any;
  declare _encryptFn: any;
  declare _nextServerCallID: any;
  declare _notificationSequenceNumber: any;
  declare _pendingBootstrapPacketPerf: any;
  declare _pendingClientCallRequests: any;
  declare _outboundWriteTail: Promise<any>;
  declare _queuedOutboundPackets: number;
  declare accountRole: any;
  declare address: any;
  declare allianceID: any;
  declare baseID: any;
  declare bloodlineID: any;
  declare characterID: any;
  declare characterName: any;
  declare characterTypeID: any;
  declare chatRole: any;
  declare clientID: any;
  declare clientId: any;
  declare cloneStationID: any;
  declare compatibilityProfile: any;
  declare connectTime: any;
  declare constellationID: any;
  declare corporationID: any;
  declare countryCode: any;
  declare empireID: any;
  declare encrypted: any;
  declare factionID: any;
  declare factionid: any;
  declare fleetid: any;
  declare fleetrole: any;
  declare genderID: any;
  declare hqID: any;
  declare languageID: any;
  declare lastActivity: any;
  declare raceID: any;
  declare regionID: any;
  declare role: any;
  declare schoolID: any;
  declare sessionID: any;
  declare sessionIV: any;
  declare sessionKey: any;
  declare shipID: any;
  declare shipName: any;
  declare sid: any;
  declare socket: any;
  declare solarsystemid: any;
  declare squadid: any;
  declare stationID: any;
  declare structureID: any;
  declare userName: any;
  declare userid: any;
  declare warFactionID: any;
  declare wingid: any;
   ...
   */
  constructor(handshakeData, socket, options: Record<string, any> = {}) {
    // Core identity
   ...
   */
  sendPacket(value, spatialTraceContext = null, codecOptions: Record<string, any> = {}) {
    ...
    const queuedWrite = this._outboundWriteTail.then(async () => {
      ...
      const innerPayloads = encodedResult && Array.isArray(encodedResult.innerPayloads)
        ? encodedResult.innerPayloads.map((entry) => (
          entry instanceof Uint8Array && !Buffer.isBuffer(entry)
            ? Buffer.from(entry)
            : entry
        ))
        : [];
      ...
      if (typeof codecOptions.onEncoded === "function") {
        ...
        if (callbackResult) activeSpatialTraceContext = callbackResult;
      }
   ...
   */
  sendRawPayload(payload, options: Record<string, any> = {}) {
    if (!this.socket || this.socket.destroyed) {
   ...
   */
  sendSessionChange(changes, options: Record<string, any> = {}) {
    if (!this.socket || this.socket.destroyed) {
      syncLedger.recordSyncLedgerEvent(this, "packet.dropped", {
        kind: "session-change",
        reason: "socket-closed",
    ...
    for (const [key, [oldVal, newVal]] of Object.entries<any>(changes)) {
   ...
   */
  sendNotification(notifyType, idType, payloadTuple: any[] = []) {
    if (!this.socket || this.socket.destroyed) {
      syncLedger.recordSyncLedgerEvent(this, "packet.dropped", {
        kind: "notification",
    ...
    const unpickledPayload: any[] = [0, [1, payloadTuple]];
    ...
    const packetPerfMeta: Record<string, any> = {
      kind: "notification",
      notifyType: notifyType || "Notification",
      idType: idType || "ownerid",
      ...syncLedger.summarizeNotificationPayload(
    ...
    this.sendPacket(packet, null, {
      innerMarshals: [{
        ...
        returnEncoded:
          (notifyType || "Notification") === "DoDestinyUpdate" && spatialTrace.isEnabled(),
      }],
   ...
   */
  sendServiceNotification(serviceName, methodName, payloadTuple: any[] = [], kwargs = null) {
    if (!this.socket || this.socket.destroyed) {
      syncLedger.recordSyncLedgerEvent(this, "packet.dropped", {
   ...
   */
  sendClientCallRequest(serviceName, methodName, argsTuple: any[] = [], kwargs = null, options: Record<string, any> = {}) {
    if (!this.socket || this.socket.destroyed) {
      syncLedger.recordSyncLedgerEvent(this, "packet.dropped", {
   ...
   */
  sendObjectNotification(objectID, methodName, payloadTuple: any[] = [], kwargs = null) {
    if (!this.socket || this.socket.destroyed) {
      syncLedger.recordSyncLedgerEvent(this, "packet.dropped", {
