...
const { buildDict, buildObjectEx1, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const VICTORY_POINT_STATE_CLASS = "factionwarfare.victoryPointState.VictoryPointState";
...
function resolveSolarSystemID(args, session) {
    return normalizeSolarSystemID((Array.isArray(args) && args.length > 0 && args[0]) ||
...
function normalizeVictoryPointState(rawState) {
    ...
    const score = Math.max(0, normalizeNumber(rawState.score ?? rawState.vpScore ?? rawState._vpScore, 0));
    const threshold = Math.max(1, normalizeNumber(rawState.threshold ?? rawState.vpThreshold ?? rawState._vpThreshold, DEFAULT_VICTORY_POINT_THRESHOLD));
    return {
        score,
        threshold,
    };
...
function getSessionStateSource(session) {
    ...
    return (session.fwVictoryPointStatesBySolarSystemID ||
...
class FwVictoryPointMgrService extends BaseService {
    ...
    Handle_GetAllVictoryPointStates(args, session) {
        ...
        log.debug(`[FwVictoryPointMgr] GetAllVictoryPointStates -> ${stateEntries.length}`);
        return buildDict(stateEntries.map(([solarSystemID, state]) => [
            solarSystemID,
    ...
    Handle_GetLocalVictoryPointState(args, session) {
        ...
        log.debug(`[FwVictoryPointMgr] GetLocalVictoryPointState(${solarSystemID}) -> ${state ? "state" : "none"}`);
        return [
            solarSystemID,
...
//# sourceMappingURL=fwVictoryPointMgrService.js.map