...
function resolveCharacterID(args, session) {
    return Number((Array.isArray(args) && args.length > 0 && args[0]) ||
        (session &&
            (session.characterID || session.charID || session.charid || session.userid))) || 0;
}
...
function resolveWarFactionID(charData = {}, session = null) {
    return normalizePositiveInteger(charData.warFactionID ??
...
function normalizeRankOverviewEntry(entry, fallbackFactionID) {
    ...
    const factionID = normalizePositiveInteger(entry.factionID ?? entry.warFactionID, fallbackFactionID);
...
function buildRankOverviewPayload(characterID, charData, session) {
    ...
    return buildList(normalizedEntries.map((entry) => buildRankInfoPayload(characterID, entry.factionID, entry.currentRank)));
}
function buildSingleRankInfo(characterID, charData, session) {
    ...
    return buildRankInfoPayload(characterID, factionID, resolveCurrentRank(charData, 0));
}
class FacWarMgrService extends BaseService {
    ...
    Handle_GetMyCharacterRankOverview(args, session) {
        ...
        log.debug(`[FacWarMgr] GetMyCharacterRankOverview(charID=${characterID}) -> ${result.items.length}`);
        return result;
    }
    Handle_GetCharacterRankOverview(args, session) {
        ...
        const charData = characterID ? getCharacterRecord(characterID) || {} : {};
        const result = buildRankOverviewPayload(characterID, charData, session);
        log.debug(`[FacWarMgr] GetCharacterRankOverview(charID=${characterID}) -> ${result.items.length}`);
        return result;
    }
    Handle_GetMyCharacterRankInfo(args, session) {
        ...
        log.debug(`[FacWarMgr] GetMyCharacterRankInfo(charID=${characterID}) -> ${result ? "rank" : "none"}`);
        return result;
    }
    Handle_GetCharacterRankInfo(args, session) {
        ...
        const charData = characterID ? getCharacterRecord(characterID) || {} : {};
        const result = buildSingleRankInfo(characterID, charData, session);
        log.debug(`[FacWarMgr] GetCharacterRankInfo(charID=${characterID}) -> ${result ? "rank" : "none"}`);
        return result;
    }
}
module.exports = FacWarMgrService;
//# sourceMappingURL=facWarMgrService.js.map