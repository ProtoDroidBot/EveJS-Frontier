...
const { grantItemsToCharacterLocation, grantItemsToOwnerLocation, ITEM_FLAGS, } = require(path.join(__dirname, "../inventory/itemStore"));
const { canSeeCorporationBlueprints, getAccessibleCorpHangarFlags, getSessionCharacterID, getSessionCorporationID, hasCorporationIndustryJobAccess, } = require(path.join(__dirname, "./industryAccess"));
const { buildManufacturingMaterials, cancelIndustryJob, deliverManufacturingJob, installManufacturingJob, listBlueprintInstancesByOwner, listJobsByOwner, markIndustryJobReady, quoteManufacturingJob, seedBlueprintForOwner, } = require(path.join(__dirname, "./industryRuntimeState"));
const { getBlueprintDefinitionByProductTypeID, getBlueprintDefinitionByTypeID, searchBlueprintDefinitions, } = require(path.join(__dirname, "./industryStaticData"));
const { DEMO_RUN_COUNT, GM_BP_PRESETS, ITEM_FLAG_CORP_DELIVERIES, ITEM_FLAG_CORP_HANGAR_1, } = require(path.join(__dirname, "./industryConstants"));
...
function seedMaterialsForBlueprint(session, definition, runs, ownerID) {
    ...
    if (ownerID === getSessionCharacterID(session)) {
        return grantItemsToCharacterLocation(ownerID, locationID, ITEM_FLAGS.HANGAR, entries);
    }
    return grantItemsToOwnerLocation(ownerID, locationID, resolveInputFlag(session, ownerID, locationID), entries);
}
...
function formatBlueprintRows(blueprints = []) {
    return blueprints.slice(0, 20).map((blueprint) => (`itemID=${blueprint.itemID} typeID=${blueprint.typeID} runs=${blueprint.runs} owner=${blueprint.ownerID} flag=${blueprint.flagID} facility=${blueprint.facilityID || 0}`));
}
function formatJobRows(jobs = []) {
    return jobs.slice(0, 20).map((job) => (`job=${job.jobID} status=${job.status} blueprint=${job.blueprintTypeID} product=${job.productTypeID} runs=${job.runs} owner=${job.ownerID}`));
}
...
function executeBlueprintAutoCommand(session, argumentText) {
    ...
    if (subcommand === "seed") {
        ...
        const runs = Math.max(1, toInt(rest[1], DEMO_RUN_COUNT[String(rest[0] || "").toLowerCase()] || 1));
    ...
    if (subcommand === "quote" || subcommand === "build") {
        ...
        if (!quoteResult.success && subcommand === "build") {
            const accountFundsOnly = Array.isArray(quoteResult.errors) &&
...
//# sourceMappingURL=industryChatCommands.js.map