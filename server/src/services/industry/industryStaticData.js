...
const { structureHasOnlineService, getStructureServiceAccessSettingID, isIndustryStructureService, isMetaStructureService, } = require(path.join(__dirname, "../structure/structureServiceAuthority"));
const { characterHasStructureService, } = require(path.join(__dirname, "../structure/structurePayloads"));
const { getFacilityTaxConfig, getFacilityTaxRate, } = require(path.join(__dirname, "./industryFacilityState"));
const { buildStructureIndustryFacilityModifiers, } = require(path.join(__dirname, "./industryFacilityModifiers"));
const { INDUSTRY_ACTIVITY, INDUSTRY_BLUEPRINT_TABLE, INDUSTRY_FACILITY_TABLE, DEFAULT_TAX_RATE, DEFAULT_SCC_TAX_MODIFIER, FACILITY_ACTIVITY_SERVICE_IDS, INDUSTRY_REFERENCE, } = require(path.join(__dirname, "./industryConstants"));
...
function searchBlueprintDefinitions(query, limit = 20) {
    ...
    return listBlueprintDefinitions()
        ...
        else if (blueprintName.startsWith(normalizedQuery) ||
            productName.startsWith(normalizedQuery)) {
            score = 0.25;
        }
        else if (blueprintName.includes(normalizedQuery) ||
        ...
        return String(left.definition.blueprintName || "").localeCompare(String(right.definition.blueprintName || ""));
    })
...
function buildDefaultGlobalModifiers() {
    return {
        ...
        system: {
            ...
            [INDUSTRY_ACTIVITY.INVENTION]: 1.0,
            [INDUSTRY_ACTIVITY.REACTION]: 1.0,
        },
    };
}
function buildActivityEntry(timeFactor = 1.0) {
    const normalizedTimeFactor = toFloat(timeFactor, 1.0);
    const timeModifiers = normalizedTimeFactor > 0 && Math.abs(normalizedTimeFactor - 1.0) > 1e-9
...
function buildNpcFacilityActivitiesPayload(profile = null) {
    ...
    if (!profile || profile.supportsFactory !== false) {
        activities[INDUSTRY_ACTIVITY.MANUFACTURING] = buildActivityEntry(profile && profile.manufacturingFactor);
    }
    if (profile && profile.supportsLaboratory) {
        ...
        activities[INDUSTRY_ACTIVITY.INVENTION] = buildActivityEntry(researchFactor);
    }
    return activities;
}
...
function buildFacilityPayloadFromNpcProfile(profile) {
    ...
    return {
        ...
        activities: applySystemCostIndexModifiers(buildNpcFacilityActivitiesPayload(profile), toInt(profile && profile.solarSystemID, 0)),
    };
}
...
function buildFacilityPayloadFromStructure(structure, session = null) {
    ...
    return {
        ...
        globalModifiers: buildDefaultGlobalModifiers(),
        activities: applySystemCostIndexModifiers(facilityModifiers.activities, toInt(structure.solarSystemID, 0)),
    };
}
    ...
    if (station) {
        return {
            ...
            activities: applySystemCostIndexModifiers({
                ...
                [INDUSTRY_ACTIVITY.INVENTION]: buildActivityEntry(1.0),
            }, toInt(station.solarSystemID, 0)),
        };
    }
    return buildFacilityPayloadFromStructure(structureState.getStructureByID(numericFacilityID, { refresh: false }), session);
}
...
//# sourceMappingURL=industryStaticData.js.map