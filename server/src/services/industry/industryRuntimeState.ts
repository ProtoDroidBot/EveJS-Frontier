...
function listJobsByOwner(ownerID, includeCompleted = false) {
  ...
  return Object.values<any>(payload.jobs || {})
...
function createIndustryJobStructureNotification(job, notificationTypeID) {
  ...
  for (const characterID of [...new Set<any>(characterIDs.map((value) => toInt(value, 0)).filter(Boolean))]) {
...
function pauseIndustryJobsForStructureServices(structureID, serviceIDs, options: Record<string, any> = {}) {
  ...
  if (numericStructureID <= 0 || serviceIDSet.size === 0) {
    return { success: true as const, data: { changedJobIDs: [] } };
  }
  ...
  for (const [jobID, job] of Object.entries<any>(jobsTable.jobs || {})) {
    const normalizedJob = normalizeIndustryJobRecord(job);
  ...
  if (!persistJobsTable(jobsTable)) {
    return { success: false as const, errorMsg: "WRITE_ERROR", data: { changedJobIDs: [] } };
  }
...
function resumeIndustryJobsForStructureServices(structureID, serviceIDs, options: Record<string, any> = {}) {
  const numericStructureID = toInt(structureID, 0);
  const serviceIDSet = normalizeServiceIDSet(serviceIDs);
  if (numericStructureID <= 0 || serviceIDSet.size === 0) {
    return { success: true as const, data: { changedJobIDs: [] } };
...
function cancelIndustryJobsMatching(predicate, options: Record<string, any> = {}) {
...
function cancelIndustryJobsForStructureServices(structureID, serviceIDs, options: Record<string, any> = {}) {
  const numericStructureID = toInt(structureID, 0);
  const serviceIDSet = normalizeServiceIDSet(serviceIDs);
  if (numericStructureID <= 0 || serviceIDSet.size === 0) {
    return { success: true as const, data: { changedJobIDs: [] } };
...
function cancelIndustryJobsForStructure(structureID, options: Record<string, any> = {}) {
  const numericStructureID = toInt(structureID, 0);
...
function listIndustryJobsOverLast24Hours(activityID = 0, nowFiletime = currentFileTime()) {
  ...
  return buildIndustryJobsOverLast24HoursRows(
    Object.values<any>(payload.jobs || {}),
    activityID,
    nowFiletime,
  );
}
...
function throwIndustryValidationError(errors: any[] = []) {
...
function resolveInventionDecryptorSelection(requestMaterials, runs) {
  const materials =
    requestMaterials && typeof requestMaterials === "object"
      ? requestMaterials
      : {};
  const selected = Object.entries<any>(materials)
    .map(([typeID, quantity]) => ({
  ...
  if (selected.length === 0) {
    return { success: true as const, typeID: 0 };
  }
  ...
  if (
    ...
    return { success: false as const, typeID: 0 };
  }
  return { success: true as const, typeID: selected[0].typeID };
}

function materialMapsMatchExactly(left, right) {
  const normalize = (source) => Object.fromEntries(
    Object.entries<any>(source && typeof source === "object" ? source : {})
      .map(([typeID, quantity]) => {
        const numericTypeID = toInt(typeID, 0);
        return [
...
function resolvePersistedInventionSuccessfulRuns(currentJob) {
  ...
  if (!storedJob) {
    return { success: false as const, errorMsg: "JOB_NOT_FOUND" };
  }
  ...
  return { success: true as const, successfulRuns };
}
...
function persistInventionDeliveryState(currentJob, update) {
  ...
  return { success: true as const, data: cloneValue(nextState) };
}
...
function deliverInventionJob(currentJob, session) {
  ...
  if (!product) {
    return { success: false as const, errorMsg: "INVALID_PRODUCT" };
  }
  ...
  if (existingItems.length > successfulRuns) {
    return { success: false as const, errorMsg: "DUPLICATE_INVENTION_OUTPUTS" };
  }

  let grantResult: { success: boolean; errorMsg?: string; data?: Record<string, any> } = {
    success: true,
    data: {
      changes: [],
      items: [],
...
function getJobCountsByInstaller(installerID) {
  const counts: Record<string, any> = {};
  for (const job of Object.values<any>(ensureJobsTable().jobs || {})) {
...
function seedBlueprintForOwner(ownerID, locationID, options: Record<string, any> = {}) {
