...
function buildIndustryErrorTuple(errorCode, args: any[] = []) {
...
function buildIndustryValidationErrors(errors: any[] = []) {
  return buildList(
...
function buildFacilityPayload(facility) {
  ...
  return buildKeyVal([
    ...
    [
      "activities",
      buildDict(
        Object.entries<any>((facility && facility.activities) || {}).map(([activityID, entry]) => [
          toInt(activityID, 0),
...
function buildFacilityTaxesPayload(taxes: Record<string, any> = {}) {
...
function buildAvailableMaterialsPayload(materials: Record<string, any> = {}) {
  return buildDict(
  ...
  return {
    ...
    materials:
      request.materials && typeof request.materials === "object"
        ? Object.fromEntries(
            Object.entries<any>(request.materials).map(([typeID, quantity]) => [
              toInt(typeID, 0),
              toInt(quantity, 0),
            ]),
          )
