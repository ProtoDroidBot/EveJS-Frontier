...
function canViewOwnerLocation(session, ownerID, locationID, flagID) {
  ...
  const queryRole: bigint = CORP_HANGAR_QUERY_ROLE_BY_FLAG[toInt(flagID, 0)];
  if (!queryRole) {
    return hasCorporationIndustryJobAccess(session, ownerID);
  }
...
function canTakeFromOwnerLocation(session, ownerID, locationID, flagID) {
  ...
  const takeRole: bigint = CORP_HANGAR_TAKE_ROLE_BY_FLAG[toInt(flagID, 0)];
  if (!takeRole) {
    return hasCorporationIndustryJobAccess(session, ownerID);
  }
  const roles = rolesAtLocation(session, locationID);
...
function getAccessibleCorpHangarFlags(session, locationID, options: Record<string, any> = {}) {
  const flags: any[] = [];
