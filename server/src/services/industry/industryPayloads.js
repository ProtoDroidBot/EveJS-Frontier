...
const { buildDict, buildFiletimeLong, buildKeyVal, buildList, buildObjectEx1, marshalObjectToObject, normalizeNumber, normalizeText, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { INDUSTRY_ERROR_NAME, } = require(path.join(__dirname, "./industryConstants"));
...
function buildMarshalValue(value) {
    ...
    if (typeof value === "object") {
        return buildDict(Object.entries(value).map(([key, entryValue]) => [
...
function buildIndustryValidationErrors(errors = []) {
    return buildList((Array.isArray(errors) ? errors : []).map((entry) => buildIndustryErrorTuple(entry.code, entry.args)));
}
...
function buildFacilityPayload(facility) {
    const taxValue = facility && Object.prototype.hasOwnProperty.call(facility, "tax")
        ? facility.tax
        : 0;
    return buildKeyVal([
        ...
        [
            "activities",
            buildDict(Object.entries((facility && facility.activities) || {}).map(([activityID, entry]) => [
                toInt(activityID, 0),
...
function buildFacilityTaxesPayload(taxes = {}) {
    const normalizeTax = (value) => (value === null || value === undefined ? null : Number(value));
    return buildKeyVal([
...
function buildAvailableMaterialsPayload(materials = {}) {
    return buildDict(Object.entries(materials || {}).map(([typeID, quantity]) => [
        toInt(typeID, 0),
        toInt(quantity, 0),
    ]));
}
...
function parseIndustryRequest(value) {
    ...
    return {
        ...
        materials: request.materials && typeof request.materials === "object"
            ? Object.fromEntries(Object.entries(request.materials).map(([typeID, quantity]) => [
                toInt(typeID, 0),
                toInt(quantity, 0),
            ]))
            : {},
...
//# sourceMappingURL=industryPayloads.js.map