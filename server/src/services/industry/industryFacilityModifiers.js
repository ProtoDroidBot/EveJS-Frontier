...
const { INDUSTRY_ACTIVITY, INDUSTRY_REFERENCE, } = require(path.join(__dirname, "./industryConstants"));
...
function buildManufacturingCategoryTargets(kind, categoryIDs) {
    return Object.freeze(categoryIDs.map((categoryID) => Object.freeze({
        kind,
...
function cloneActivityEntry(entry) {
    ...
    return [0, 1, 2, 3, 4, 5].map((index) => (Array.isArray(entry[index])
        ? entry[index].map((value) => (Array.isArray(value) ? [...value] : value))
        : []));
}
function cloneActivities(activities = {}) {
    return Object.fromEntries(Object.entries(activities || {}).map(([activityID, entry]) => [
        activityID,
...
function getTypeDogmaEntry(typeID) {
    ...
    return (payload &&
        ...
        payload.typesByTypeID[String(toInt(typeID, 0))]) || null;
}
function getEffectDogmaEntry(effectID) {
    const payload = getTypeDogmaPayload();
    return (payload &&
        payload.effectTypesByID &&
        payload.effectTypesByID[String(toInt(effectID, 0))]) || null;
}
...
function applyEngineeringHullModifiers(structure, activities) {
    ...
    if (materialAmount > 0 && Math.abs(materialAmount - 1) > 1e-9) {
        appendActivityModifier(activities, {
            kind: "material",
            activityID: INDUSTRY_ACTIVITY.MANUFACTURING,
        }, materialAmount, INDUSTRY_REFERENCE.HULL);
    }
    for (const activityID of HULL_MODIFIER_ACTIVITIES) {
        if (timeAmount > 0 && Math.abs(timeAmount - 1) > 1e-9) {
            appendActivityModifier(activities, { kind: "time", activityID }, timeAmount, INDUSTRY_REFERENCE.HULL);
        }
        if (costAmount > 0 && Math.abs(costAmount - 1) > 1e-9) {
            appendActivityModifier(activities, { kind: "cost", activityID }, costAmount, INDUSTRY_REFERENCE.HULL);
        }
    }
}
    ...
    for (const rigItem of fittedRigs) {
        ...
        for (const effectID of effects) {
            ...
            for (const modifier of modifierInfo) {
                ...
                const sourceValue = getRigSourceAttribute(attributes, modifier.modifyingAttributeID, securityBand);
                const amount = postPercentAmount(sourceValue);
                if (!(amount > 0) || Math.abs(amount - 1) <= 1e-9) {
                    continue;
                }
                for (const target of targets) {
                    if (appendActivityModifier(activities, target, amount, INDUSTRY_REFERENCE.RIG)) {
...
//# sourceMappingURL=industryFacilityModifiers.js.map