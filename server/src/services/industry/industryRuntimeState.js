...
const { getCharacterWallet, adjustCharacterBalance, } = require(path.join(__dirname, "../account/walletState"));
...
const { buildIndustryActivityMaterials, getIndustryActivity, resolveFacilityCostModifier, resolveIndustryJobBaseCost, resolveIndustryJobTimeSeconds, } = require(path.join(__dirname, "./industryParityHelpers"));
const { resolveIndustryFacilityRestriction, resolveIndustryStructureServiceID, resolveIndustrySlotContext, resolveIndustryStandingRestriction, } = require(path.join(__dirname, "./industryRestrictions"));
const { resolveSystemCostIndex, } = require(path.join(__dirname, "./industrySystemCostIndex"));
const { resolveBlueprintActivityPrice, } = require(path.join(__dirname, "./industryPricing"));
const { canSeeCorporationBlueprints, canTakeFromOwnerLocation, canUseCorporationWallet, canViewOwnerLocation, getAccessibleCorpHangarFlags, getSessionCharacterID, getSessionCorporationID, hasCorporationIndustryJobAccess, isCharacterOwner, isCorporationOwner, normalizeRoleValue, } = require(path.join(__dirname, "./industryAccess"));
...
const { BLUEPRINT_CATEGORY_ID, COST_PERCENTAGE, DEFAULT_ACCOUNT_KEY, DEFAULT_TAX_RATE, INDUSTRY_ACTIVITY, INDUSTRY_BLUEPRINT_STATE_TABLE, INDUSTRY_ERROR, INDUSTRY_INSTALLED_LOCATION_ID, INDUSTRY_JOBS_TABLE, INDUSTRY_RUNTIME_TABLE, INDUSTRY_STATUS, ITEM_FLAG_CORP_DELIVERIES, ITEM_FLAG_CORP_HANGAR_1, ITEM_FLAG_HANGAR, MAX_JOB_LENGTH_SECONDS, MAX_COPY_RUNS, MAX_INVENTION_RUNS, MAX_MATERIAL_EFFICIENCY, MAX_MANUFACTURING_RUNS, MAX_TIME_EFFICIENCY, INVENTION_MATERIAL_EFFICIENCY, INVENTION_SKILL_PROBABILITY, INVENTION_SKILL_PROBABILITY_LOWER, INVENTION_TIME_EFFICIENCY, RESEARCH_TIME_MULTIPLIERS, SCC_SURCHARGE_RATE, SCC_SURCHARGE_RESEARCH_DISCOUNT_MODIFIER, STEP_MATERIAL_EFFICIENCY, STEP_TIME_EFFICIENCY, } = require(path.join(__dirname, "./industryConstants"));
...
function getTypeDogmaAttributeValue(typeID, attributeID, fallback = 0) {
    ...
    const attributes = typeRecord && typeRecord.attributes && typeof typeRecord.attributes === "object"
...
function resolveDecryptorModifiers(typeID) {
    ...
    return {
        ...
        probabilityMultiplier: getTypeDogmaAttributeValue(numericTypeID, ATTRIBUTE_INVENTION_PROBABILITY_MULTIPLIER, 1.0),
        materialEfficiency: toInt(getTypeDogmaAttributeValue(numericTypeID, ATTRIBUTE_INVENTION_ME_MODIFIER, 0), 0),
        timeEfficiency: toInt(getTypeDogmaAttributeValue(numericTypeID, ATTRIBUTE_INVENTION_TE_MODIFIER, 0), 0),
        maxRuns: toInt(getTypeDogmaAttributeValue(numericTypeID, ATTRIBUTE_INVENTION_MAX_RUN_MODIFIER, 0), 0),
    };
}
function getCharacterSkillLevels(characterID) {
    const levels = new Map();
    for (const [typeID, skillRecord] of getCachedCharacterSkillMap(toInt(characterID, 0)).entries()) {
        levels.set(toInt(typeID, 0), Math.max(0, toInt(skillRecord &&
...
function resolveInventionProduct(definition, requestedProductTypeID = 0) {
    ...
    if (numericRequestedTypeID > 0) {
        return products.find((product) => toInt(product && product.typeID, 0) === numericRequestedTypeID) || null;
    }
    return products.length === 1 ? products[0] : null;
}
function computeInventionProbability(definition, characterID, decryptorTypeID = 0) {
    const activity = getIndustryActivity(definition, INDUSTRY_ACTIVITY.INVENTION);
    const products = Array.isArray(activity && activity.products)
        ? activity.products
        : [];
...
function canViewBlueprintInstance(session, item) {
    ...
    return (canSeeCorporationBlueprints(session, ownerID) &&
        canViewOwnerLocation(session, ownerID, accessLocationID, accessFlagID));
}
...
function updateBlueprintState(itemID, updater) {
    ...
    const nextState = typeof updater === "function" ? updater(cloneValue(current)) : updater;
...
function listBlueprintInstancesByOwner(ownerID, facilityID = null, session = null) {
    ...
    const numericFacilityID = facilityID === null || facilityID === undefined ? null : toInt(facilityID, 0);
    const counts = {};
    const blueprints = [];
    for (const item of listOwnedItems(numericOwnerID, {
        ...
        const resolvedFacilityID = instance.facilityID === null || instance.facilityID === undefined
            ? null
...
function getJobStatus(job) {
    ...
    if (toInt(job.status, 0) === INDUSTRY_STATUS.INSTALLED &&
        endDate > 0n &&
...
function normalizeServiceIDSet(serviceIDs) {
    const values = serviceIDs instanceof Set
        ? [...serviceIDs]
...
function jobRequiresStructureService(job, structureID, serviceIDSet) {
    if (toInt(job && job.facilityID, 0) !== toInt(structureID, 0) ||
        !(serviceIDSet instanceof Set) ||
        serviceIDSet.size === 0) {
        return false;
    }
    const requiredServiceID = resolveIndustryStructureServiceID(job.activityID, job.productTypeID);
...
function isCharacterOwnerID(ownerID) {
    ...
    const characters = result && result.success && result.data && typeof result.data === "object"
        ? result.data
        : {};
...
function createIndustryJobStructureNotification(job, notificationTypeID) {
    ...
    const characterIDs = isCorpOwned
        ...
        : [ownerID];
    const senderID = toInt(structure.ownerCorpID || structure.ownerID || ownerID, ownerID);
...
function pauseIndustryJobsForStructureServices(structureID, serviceIDs, options = {}) {
    ...
    for (const job of changedJobs) {
        ...
        createIndustryJobStructureNotification(job, NOTIFICATION_TYPE.STRUCTURES_JOBS_PAUSED);
    }
    return {
        success: true,
        data: {
...
function cancelIndustryJobsMatching(predicate, options = {}) {
    ...
    for (const job of changedJobs) {
        ...
        notifyIndustryJob(job);
        createIndustryJobStructureNotification(job, NOTIFICATION_TYPE.STRUCTURES_JOBS_CANCELLED);
    }
    return {
        success: true,
        data: {
...
function cancelIndustryJobsForStructureServices(structureID, serviceIDs, options = {}) {
    ...
    return cancelIndustryJobsMatching((job) => jobRequiresStructureService(job, numericStructureID, serviceIDSet), options);
}
function cancelIndustryJobsForStructure(structureID, options = {}) {
    ...
    return cancelIndustryJobsMatching((job) => toInt(job && job.facilityID, 0) === numericStructureID, options);
}
function buildIndustryJobsOverLast24HoursRows(jobs, activityID = 0, nowFiletime = currentFileTime()) {
    ...
    for (const job of Array.isArray(jobs) ? jobs : []) {
        ...
        countsBySolarSystemID.set(solarSystemID, (countsBySolarSystemID.get(solarSystemID) || 0) + 1);
    }
...
function listIndustryJobsOverLast24Hours(activityID = 0, nowFiletime = currentFileTime()) {
    const payload = ensureJobsTable();
    return buildIndustryJobsOverLast24HoursRows(Object.values(payload.jobs || {}), activityID, nowFiletime);
}
...
function resolveFacilityLocations(facilityID, ownerID, blueprint = null, session = null) {
    ...
    if (blueprint && toInt(blueprint.locationID, 0) > 0) {
        ...
        const canView = session
            ? canViewOwnerLocation(session, numericOwnerID, toInt(blueprint.locationID, 0), toInt(blueprint.flagID, 0))
            : true;
        const canTake = session
            ? canTakeFromOwnerLocation(session, numericOwnerID, toInt(blueprint.locationID, 0), toInt(blueprint.flagID, 0))
            : true;
        pushLocation({
...
function resolveAvailableMaterials(ownerID, inputLocation) {
    ...
    const materials = {};
    for (const item of listContainerItems(toInt(ownerID, 0), toInt(inputLocation.itemID, 0), toInt(inputLocation.flagID, 0))) {
        const typeID = toInt(item && item.typeID, 0);
        if (typeID <= 0 || toInt(item.categoryID, 0) === BLUEPRINT_CATEGORY_ID) {
            continue;
        }
        const quantity = toInt(item.singleton, 0) === 1 ? 1 : Math.max(0, toInt(item.stacksize || item.quantity, 0));
        materials[typeID] = (materials[typeID] || 0) + quantity;
    }
    return materials;
}
function resolveInventionDecryptorSelection(requestMaterials, runs) {
    const materials = requestMaterials && typeof requestMaterials === "object"
        ? requestMaterials
        : {};
...
function materialMapsMatchExactly(left, right) {
    const normalize = (source) => Object.fromEntries(Object.entries(source && typeof source === "object" ? source : {})
        .map(([typeID, quantity]) => {
        const numericTypeID = toInt(typeID, 0);
        return [
...
function getMissingInventionSkills(activity, characterID) {
    ...
    return (Array.isArray(activity && activity.skills) ? activity.skills : [])
        ...
        .filter((skill) => skill.typeID > 0 && skill.actualLevel < skill.requiredLevel);
}
function buildManufacturingMaterials(definition, runs, materialEfficiency = 0) {
    return buildIndustryActivityMaterials(definition, INDUSTRY_ACTIVITY.MANUFACTURING, runs, { materialEfficiency });
}
...
function resolveActivityBaseCost(definition, blueprintTypeID, activityID, runs, licensedRuns, state = null, productTypeID = 0) {
    ...
    if (numericActivityID === INDUSTRY_ACTIVITY.MANUFACTURING ||
        ...
        return Math.round(resolveIndustryJobBaseCost(definition, numericActivityID, 1) * numericRuns);
    }
    if (numericActivityID === INDUSTRY_ACTIVITY.INVENTION &&
        toInt(productTypeID, 0) > 0) {
        return Math.round(resolveBlueprintActivityPrice(productTypeID, numericActivityID) *
            COST_PERCENTAGE *
            numericRuns);
    }
    const blueprintValue = resolveIndustryJobBaseCost({
        ...
        activities: {},
    }, INDUSTRY_ACTIVITY.MANUFACTURING, COST_PERCENTAGE);
    if (!(blueprintValue > 0)) {
        return 0;
    }
    if (numericActivityID === INDUSTRY_ACTIVITY.RESEARCH_MATERIAL ||
        numericActivityID === INDUSTRY_ACTIVITY.RESEARCH_TIME) {
        const baseResearchTime = Math.max(1, toInt(getIndustryActivity(definition, numericActivityID)?.time, 0));
    ...
    if (numericActivityID === INDUSTRY_ACTIVITY.COPYING) {
        return Math.round(blueprintValue *
            Math.max(1, toInt(licensedRuns, 1)) *
            numericRuns);
    }
    return Math.round(resolveIndustryJobBaseCost(definition, numericActivityID, COST_PERCENTAGE) * numericRuns);
}
function resolveActivitySccSurcharge(cost, activityID, facility) {
    let surchargeRate = SCC_SURCHARGE_RATE * Math.max(0, toFloat(facility && facility.sccTaxModifier, 1));
    if (toInt(activityID, 0) === INDUSTRY_ACTIVITY.RESEARCH_TIME ||
...
function resolveActivityProductTypeID(definition, activityID, blueprintTypeID, requestedProductTypeID = 0) {
    const numericActivityID = toInt(activityID, 0);
    if (numericActivityID === INDUSTRY_ACTIVITY.RESEARCH_TIME ||
    ...
    if (numericActivityID === INDUSTRY_ACTIVITY.INVENTION) {
        return toInt(resolveInventionProduct(definition, requestedProductTypeID)?.typeID, 0);
    }
    const outputProductTypeID = toInt(buildProductOutput(definition, 1, numericActivityID)[0]?.typeID, 0);
...
function resolveBaseJobCost(definition) {
    return resolveIndustryJobBaseCost(definition, INDUSTRY_ACTIVITY.MANUFACTURING, COST_PERCENTAGE);
}
function resolveJobTimeSeconds(definition, runs, timeEfficiency, facility, characterID) {
    return resolveIndustryJobTimeSeconds(definition, INDUSTRY_ACTIVITY.MANUFACTURING, runs, timeEfficiency, facility, characterID, 1, toInt(definition && definition.productTypeID, 0));
}
function resolveActivityJobTimeSeconds(definition, activityID, runs, state, facility, characterID, licensedRuns = 1, productTypeID = 0) {
    const numericActivityID = toInt(activityID, 0);
    const currentEfficiency = numericActivityID === INDUSTRY_ACTIVITY.RESEARCH_MATERIAL
        ? Math.max(0, toInt(state && state.materialEfficiency, 0))
        : numericActivityID === INDUSTRY_ACTIVITY.RESEARCH_TIME
            ? Math.max(0, toInt(state && state.timeEfficiency, 0))
            : Math.max(0, toInt(state && state.timeEfficiency, 0));
    return resolveIndustryJobTimeSeconds(definition, numericActivityID, runs, currentEfficiency, facility, characterID, licensedRuns, productTypeID);
}
function resolveFacilityTaxRate(facility, activityID, productTypeID) {
    ...
    const serviceAccess = facility.serviceAccess && typeof facility.serviceAccess === "object"
...
function normalizeLocationForFacility(facility, ownerID, requestLocation, blueprint, session = null, options = {}) {
    ...
    if (requestLocation) {
        const match = locations.find((entry) => (toInt(entry.itemID, 0) === toInt(requestLocation.itemID, 0) &&
...
function quoteIndustryJob(session, requestInput) {
    const request = requestInput && requestInput.rawRequest
        ? requestInput
    ...
    const activityProductTypeID = resolveActivityProductTypeID(definition, activityID, blueprint.typeID, request.productTypeID);
    ...
    if (activityID === INDUSTRY_ACTIVITY.MANUFACTURING) {
        ...
        const perRunTime = resolveActivityJobTimeSeconds(definition, activityID, 1, state, facility, session && (session.characterID || session.charid), licensedRuns, activityProductTypeID);
        const timeDerivedMaxRuns = Math.max(1, Math.ceil(MAX_JOB_LENGTH_SECONDS / Math.max(1, perRunTime)));
        const maximumRuns = Math.min(MAX_INVENTION_RUNS, Math.max(0, toInt(state && state.runsRemaining, 0)), timeDerivedMaxRuns);
        if (runs <= 0 || runs > maximumRuns) {
            errors.push(buildIndustryValidationError(INDUSTRY_ERROR.INVALID_RUNS, maximumRuns));
        }
    }
    else {
        const remainingResearchRuns = getActivityResearchRunsRemaining(state, activityID);
        if (remainingResearchRuns <= 0) {
            errors.push(buildIndustryValidationError(INDUSTRY_ERROR.RESEARCH_LIMIT, getActivityResearchCurrentLevel(state, activityID), getActivityResearchMaxLevel(activityID)));
        }
    ...
    const slotContext = resolveIndustrySlotContext(activityID, installerID, getJobCountsByInstaller(installerID));
    if (slotContext.full) {
    ...
    if (activityID === INDUSTRY_ACTIVITY.INVENTION) {
        for (const missingSkill of getMissingInventionSkills(activity, installerID)) {
            errors.push(buildIndustryValidationError(INDUSTRY_ERROR.MISSING_SKILL, missingSkill.typeID, missingSkill.requiredLevel, missingSkill.actualLevel));
        }
    }
    const standingRestriction = resolveIndustryStandingRestriction(session, ownerID, facility, activityID);
    if (standingRestriction) {
        errors.push(buildIndustryValidationError(INDUSTRY_ERROR.STANDINGS_RESTRICTION, standingRestriction));
    }
    const facilityRestriction = resolveIndustryFacilityRestriction(activityID, activityID === INDUSTRY_ACTIVITY.MANUFACTURING ||
        activityID === INDUSTRY_ACTIVITY.REACTION
        ? activityProductTypeID
        : 0, facility, toInt(blueprint && blueprint.typeID, 0));
    if (facilityRestriction) {
        errors.push(buildIndustryValidationError(facilityRestriction.code, ...(Array.isArray(facilityRestriction.args) ? facilityRestriction.args : [])));
    }
    const inputLocation = normalizeLocationForFacility(facility, ownerID, request.inputLocation, blueprint, session, { requireTake: true });
    const outputLocation = normalizeLocationForFacility(facility, ownerID, request.outputLocation, blueprint, session, { preferDeliveries: true });
    if (!inputLocation) {
    ...
    const requestMaterials = request.materials && typeof request.materials === "object"
        ? request.materials
        : {};
    const decryptorSelection = activityID === INDUSTRY_ACTIVITY.INVENTION
        ? resolveInventionDecryptorSelection(requestMaterials, runs)
        : { success: true, typeID: 0 };
    if (!decryptorSelection.success) {
        errors.push(buildIndustryValidationError(INDUSTRY_ERROR.MISMATCH_MATERIAL, requestMaterials, {}));
    }
    const materials = buildIndustryActivityMaterials(definition, activityID, runs, activityID === INDUSTRY_ACTIVITY.MANUFACTURING
        ? {
    ...
    for (const material of materials) {
        ...
        if (available < material.quantity) {
            errors.push(buildIndustryValidationError(INDUSTRY_ERROR.MISSING_MATERIAL, material.typeID, material.quantity, available, material.quantity - available));
        }
    }
    const baseCost = resolveActivityBaseCost(definition, blueprint.typeID, activityID, runs, licensedRuns, state, activityProductTypeID);
    ...
    const systemCostIndex = resolveSystemCostIndex(facility && facility.solarSystemID, activityID);
    const cost = Math.round(baseCost * resolveFacilityCostModifier(facility, activityID, activityProductTypeID));
    const tax = Math.round(baseCost * resolveFacilityTaxRate(facility, activityID, activityProductTypeID));
    const sccSurcharge = resolveActivitySccSurcharge(baseCost, activityID, facility);
    const totalCost = cost + tax + sccSurcharge;
    const timeInSeconds = resolveActivityJobTimeSeconds(definition, activityID, runs, state, facility, session && (session.characterID || session.charid), licensedRuns, activityProductTypeID);
    const previousRunLengthSeconds = runs > 0 ? timeInSeconds - timeInSeconds / Math.max(1, toInt(runs, 1)) : timeInSeconds;
    if (previousRunLengthSeconds > MAX_JOB_LENGTH_SECONDS) {
        errors.push(buildIndustryValidationError(INDUSTRY_ERROR.RUN_LENGTH, previousRunLengthSeconds, MAX_JOB_LENGTH_SECONDS));
    }
    ...
    return {
        ...
        quote: {
            ...
            licensedRuns,
            state,
            probability: activityID === INDUSTRY_ACTIVITY.INVENTION
                ? computeInventionProbability(definition, installerID, decryptorSelection.typeID)
                : 1,
            optionalTypeID: activityID === INDUSTRY_ACTIVITY.INVENTION && decryptorSelection.typeID > 0
...
function syncInventoryChanges(session, changes = []) {
    ...
    for (const change of Array.isArray(changes) ? changes : []) {
        ...
        if (change.item) {
            syncInventoryItemForSession(session, change.item, change.previousState || change.previousData || {});
            continue;
        }
...
function moveBlueprintItemToInstalledLocation(job, session) {
    ...
    syncInventoryItemForSession(session, { ...updateResult.previousData, locationID: 6 }, updateResult.previousData);
    return updateResult;
}
...
function applyWalletCharge(ownerID, account, amount, session) {
    ...
    return adjustCorporationWalletDivisionBalance(ownerID, normalizeCorporationWalletKey(account && account[1]), -amount, {
        description: `Industry job install by ${session && (session.characterName || session.userName || session.characterID || ownerID)}`,
        ownerID1: ownerID,
...
function installIndustryJob(session, requestInput) {
    ...
    for (const material of quote.materials) {
        const takeResult = isCharacterOwner(session, quote.ownerID)
            ? takeItemTypeFromCharacterLocation(quote.ownerID, quote.inputLocation.itemID, quote.inputLocation.flagID, material.typeID, material.quantity)
            : takeItemTypeFromOwnerLocation(quote.ownerID, quote.inputLocation.itemID, quote.inputLocation.flagID, material.typeID, material.quantity);
        if (!takeResult.success) {
            throwIndustryValidationError([
                buildIndustryValidationError(INDUSTRY_ERROR.MISSING_MATERIAL, material.typeID, material.quantity, 0, material.quantity),
            ]);
        }
        itemChanges.push(...((takeResult.data && takeResult.data.changes) || []));
    }
    const walletResult = applyWalletCharge(quote.ownerID, quote.account, quote.totalCost, session);
    ...
    if (quote.activityID === INDUSTRY_ACTIVITY.MANUFACTURING) {
        try {
            require(path.join(__dirname, "../dailyGoals/dailyGoalsState")).recordActivity(toInt(session && session.characterID, 0), "install_manufacturing_job", 1);
        }
        catch (dailyGoalError) {
            log.debug(`[IndustryRuntime] daily-goal manufacture hook failed: ${dailyGoalError.message}`);
        }
    }
    return {
        success: true,
...
function finishResearchBlueprintAfterDelivery(job, session) {
    ...
    updateBlueprintState(job.blueprintID, (current) => ({
        ...current,
        materialEfficiency: toInt(job && job.activityID, 0) === INDUSTRY_ACTIVITY.RESEARCH_MATERIAL
            ? Math.min(MAX_MATERIAL_EFFICIENCY, Math.max(0, toInt(current && current.materialEfficiency, 0)) +
                STEP_MATERIAL_EFFICIENCY * Math.max(0, toInt(job && job.runs, 0)))
            : Math.max(0, toInt(current && current.materialEfficiency, 0)),
        timeEfficiency: toInt(job && job.activityID, 0) === INDUSTRY_ACTIVITY.RESEARCH_TIME
            ? Math.min(MAX_TIME_EFFICIENCY, Math.max(0, toInt(current && current.timeEfficiency, 0)) +
...
function applyBlueprintStateToGrantedCopies(grantResult, templateState) {
    const createdItems = grantResult &&
        grantResult.data &&
...
function resolvePersistedInventionSuccessfulRuns(currentJob) {
    ...
    if (currentJob &&
        ...
        return {
            success: true,
            successfulRuns: clamp(toInt(currentJob.inventionSuccessfulRuns, 0), 0, runs),
        };
    }
    const successfulRuns = rollInventionSuccessfulRuns(runs, currentJob && currentJob.probability);
    const jobsTable = ensureJobsTable();
...
function deliverInventionJob(currentJob, session) {
    ...
    const product = resolveInventionProduct(definition, currentJob.productTypeID);
    if (!product) {
    ...
    const outputRuns = Math.max(1, Math.round(toFloat(product.quantity, 0) + decryptor.maxRuns));
    const outputMarker = getInventionOutputMarker(currentJob.jobID);
    const existingItems = listOwnedItems(currentJob.ownerID)
        .filter((item) => (toInt(item && item.typeID, 0) === outputBlueprintTypeID &&
    ...
    if (missingOutputCount > 0) {
        ...
        grantResult =
            isCharacterOwner(session, currentJob.ownerID)
                ? grantItemsToCharacterLocation(currentJob.ownerID, currentJob.outputLocationID, currentJob.outputFlagID, grantEntries)
                : grantItemsToOwnerLocation(currentJob.ownerID, currentJob.outputLocationID, currentJob.outputFlagID, grantEntries);
    ...
    const stateResult = applyBlueprintStateToGrantedCopies({
        ...
        materialEfficiency: INVENTION_MATERIAL_EFFICIENCY + decryptor.materialEfficiency,
        timeEfficiency: INVENTION_TIME_EFFICIENCY + decryptor.timeEfficiency,
...
function finishInventionBlueprintAfterDelivery(job, session) {
    ...
    const alreadyApplied = toInt(state.lastCompletedJobID, 0) === toInt(job.jobID, 0);
    const remainingRuns = alreadyApplied
        ? Math.max(0, toInt(state.runsRemaining, 0))
        : Math.max(0, toInt(state.runsRemaining, 0) - toInt(job.runs, 0));
    if (!alreadyApplied) {
...
function deliverIndustryJob(session, jobID) {
    ...
    if (toInt(currentJob.activityID, 0) === INDUSTRY_ACTIVITY.MANUFACTURING ||
        ...
        const definition = getBlueprintDefinitionByTypeID(currentJob.blueprintTypeID);
        const outputs = buildProductOutput(definition, currentJob.runs, toInt(currentJob.activityID, 0));
        ...
        deliveredSuccessfulRuns = Math.max(0, toInt(grantResult.data && grantResult.data.successfulRuns, 0));
        const sourceResult = finishInventionBlueprintAfterDelivery(currentJob, session);
        ...
        const copyEntries = buildCopyOutputBlueprintEntries(currentJob, currentJob.outputLocationID, currentJob.outputFlagID);
        grantResult =
            isCharacterOwner(session, currentJob.ownerID)
                ? grantItemsToCharacterLocation(currentJob.ownerID, currentJob.outputLocationID, currentJob.outputFlagID, copyEntries.map((entry) => ({
                    itemType: entry.itemType,
                    quantity: entry.quantity,
                    options: entry.options,
                })))
                : grantItemsToOwnerLocation(currentJob.ownerID, currentJob.outputLocationID, currentJob.outputFlagID, copyEntries.map((entry) => ({
                    itemType: entry.itemType,
                    quantity: entry.quantity,
                    options: entry.options,
                })));
    ...
    if (toInt(currentJob.activityID, 0) === INDUSTRY_ACTIVITY.RESEARCH_TIME ||
...
function resolveMonitorAvailableMaterials(session, request) {
    ...
    if (facility) {
        inputLocation = normalizeLocationForFacility(facility, ownerID, request && request.inputLocation, blueprint, session, { requireTake: true });
    }
...
function seedBlueprintForOwner(ownerID, locationID, options = {}) {
    ...
    const flagID = toInt(options.flagID, corporationOwned ? ITEM_FLAG_CORP_HANGAR_1 : ITEM_FLAGS.HANGAR);
    const createResult = !corporationOwned
        ? grantItemsToCharacterLocation(ownerID, locationID, flagID, [{
                itemType: blueprintTypeID,
                quantity: 1,
                options: {
        ...
        : grantItemsToOwnerLocation(ownerID, locationID, flagID, [{
                itemType: blueprintTypeID,
                quantity: 1,
                options: {
                    singleton: isOriginal ? 1 : 2,
