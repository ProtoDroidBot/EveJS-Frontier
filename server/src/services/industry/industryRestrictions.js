...
const { getItemMetadata, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const { getIndustryServiceID, structureHasOnlineService: structureRecordHasOnlineService, } = require(path.join(__dirname, "../structure/structureServiceAuthority"));
const { INDUSTRY_ACTIVITY, INDUSTRY_ERROR, } = require(path.join(__dirname, "./industryConstants"));
...
function resolveIndustrySlotUsage(activityID, jobCounts = {}) {
    const numericCounts = jobCounts && typeof jobCounts === "object"
        ? jobCounts
        : {};
    const numericActivityID = toInt(activityID, 0);
    if (numericActivityID === INDUSTRY_ACTIVITY.MANUFACTURING) {
    ...
    if (SCIENCE_ACTIVITY_IDS.has(numericActivityID)) {
        return [
            ...
            INDUSTRY_ACTIVITY.COPYING,
            INDUSTRY_ACTIVITY.INVENTION,
        ].reduce((total, scienceActivityID) => total + Math.max(0, toInt(numericCounts[scienceActivityID], 0)), 0);
    }
    if (numericActivityID === INDUSTRY_ACTIVITY.REACTION) {
...
function isTriglavianSolarSystem(solarSystemID) {
    ...
    return (toInt(solarSystem && solarSystem.factionID, 0) === TRIGLAVIAN_FACTION_ID ||
...
function resolveStandingValueForOwner(session, ownerID, standingTargetID) {
    ...
    if (toInt(ownerID, 0) === corporationID &&
        corporationID > 0) {
        const row = standingRuntime.listCorporationStandings(corporationID).find((entry) => toInt(entry && entry.fromID, 0) === numericStandingTargetID);
...
function resolveIndustryStandingRestriction(session, ownerID, facility, activityID) {
    ...
    const currentStanding = resolveStandingValueForOwner(session, ownerID, TRIGLAVIAN_FACTION_ID);
...
function resolveManufacturingStructureServiceID(productTypeID) {
    ...
    const serviceID = getIndustryServiceID(INDUSTRY_ACTIVITY.MANUFACTURING, groupID, INDUSTRY_ACTIVITY);
    if (serviceID) {
        return serviceID;
    }
...
function resolveIndustryFacilityRestriction(activityID, productTypeID, facility, blueprintTypeID = 0) {
    ...
    if (isTriglavianSolarSystem(facility && facility.solarSystemID) &&
...
//# sourceMappingURL=industryRestrictions.js.map