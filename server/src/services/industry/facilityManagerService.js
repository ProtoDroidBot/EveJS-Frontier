...
const { buildFacilityPayload, buildLocationPayload, buildFacilityTaxesPayload, } = require(path.join(__dirname, "./industryPayloads"));
const { getFacilityPayloadByID, listFacilitiesForSession, } = require(path.join(__dirname, "./industryStaticData"));
const { getFacilityTaxConfig, setFacilityTaxConfig, } = require(path.join(__dirname, "./industryFacilityState"));
const { getBlueprintByItemID, resolveFacilityLocations, } = require(path.join(__dirname, "./industryRuntimeState"));
const { INDUSTRY_ACTIVITY, } = require(path.join(__dirname, "./industryConstants"));
const { notifyFacilitiesUpdated, } = require(path.join(__dirname, "./industryNotifications"));
...
class FacilityManagerService extends BaseService {
    ...
    Handle_GetFacilities(args, session) {
        return buildList(listFacilitiesForSession(session).map((facility) => buildFacilityPayload(facility)));
    }
    ...
    Handle_GetFacilityLocations(args, session) {
        ...
        return buildList(resolveFacilityLocations(toInt(facilityID, 0), toInt(ownerID, 0), blueprint, session).map((location) => buildLocationPayload(location)));
    }
}
module.exports = FacilityManagerService;
//# sourceMappingURL=facilityManagerService.js.map