...
const { resolveCharacterIndustryAttributes, } = require(path.join(__dirname, "../dogma/brain/providers/industryBrainProvider"));
const { resolveItemByTypeID, } = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const { INDUSTRY_ACTIVITY, MAX_MATERIAL_EFFICIENCY, MAX_TIME_EFFICIENCY, MATERIAL_ROUND_PRECISION, RESEARCH_TIME_MULTIPLIERS, STEP_MATERIAL_EFFICIENCY, STEP_TIME_EFFICIENCY, } = require(path.join(__dirname, "./industryConstants"));
...
function getCachedCharacterSkillLevels(characterID) {
    ...
    for (const [typeID, skillRecord] of skillMap.entries()) {
        levels[String(toInt(typeID, 0))] = Math.max(0, toInt(skillRecord &&
...
function getIndustryActivity(definition, activityID) {
    ...
    return (definition &&
        ...
        definition.activities[activityKey]) || null;
}
function roundMaterialQuantity(quantity, runs) {
    return Math.max(Math.trunc(Math.ceil(Math.round(quantity * (10 ** MATERIAL_ROUND_PRECISION)) /
        (10 ** MATERIAL_ROUND_PRECISION))), runs);
}
...
function buildIndustryActivityMaterials(definition, activityID, runs, options = {}) {
    ...
    const blueprintModifier = toInt(activityID, 0) === INDUSTRY_ACTIVITY.MANUFACTURING
        ? 1.0 - materialEfficiency / 100.0
        : 1.0;
    const productTypeID = toInt(options.productTypeID, toInt(definition && definition.productTypeID, 0));
    const facilityModifier = resolveFacilityMaterialModifier(options.facility, activityID, productTypeID);
    return baseMaterials
        .map((material) => {
        const quantity = Math.max(0, toInt(material && material.quantity, 0)) *
            normalizedRuns *
            blueprintModifier *
            facilityModifier;
        return {
...
function resolveFacilityActivityModifier(facility, activityID, modifierIndex, productTypeID = 0) {
    const activityEntry = facility &&
        facility.activities &&
...
function resolveCharacterIndustryTimeModifier(activityID, characterID) {
    ...
    const modifier = toFloat(attributes && attributes[attributeID], 1.0);
...
function resolveIndustryJobBaseCost(definition, activityID, costPercentage = 1) {
    ...
    return Math.max(0, resolveBlueprintActivityPrice(blueprintTypeID, activityID) * Math.max(0, toFloat(costPercentage, 0)));
}
function resolveIndustryJobTimeSeconds(definition, activityID, runs, timeEfficiency, facility, characterID, licensedRuns = 1, productTypeID = 0) {
    const activity = getIndustryActivity(definition, activityID);
    ...
    else if (toInt(activityID, 0) === INDUSTRY_ACTIVITY.RESEARCH_TIME ||
        toInt(activityID, 0) === INDUSTRY_ACTIVITY.RESEARCH_MATERIAL) {
        const stepSize = toInt(activityID, 0) === INDUSTRY_ACTIVITY.RESEARCH_TIME
            ? STEP_TIME_EFFICIENCY
            : STEP_MATERIAL_EFFICIENCY;
        const maxEfficiency = toInt(activityID, 0) === INDUSTRY_ACTIVITY.RESEARCH_TIME
            ? MAX_TIME_EFFICIENCY
    ...
    const blueprintModifier = toInt(activityID, 0) === INDUSTRY_ACTIVITY.MANUFACTURING
        ...
        : 1.0;
    const activityProductTypeID = toInt(productTypeID, toInt(definition && definition.productTypeID, 0));
    const facilityModifier = resolveFacilityTimeModifier(facility, activityID, activityProductTypeID);
    const characterModifier = resolveCharacterIndustryTimeModifier(activityID, characterID);
    const requiredSkillModifier = resolveRequiredSkillTimeModifier(activityID, characterID, activity && activity.skills);
...
//# sourceMappingURL=industryParityHelpers.js.map