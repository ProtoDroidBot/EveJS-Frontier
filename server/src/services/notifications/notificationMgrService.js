...
const { extractList, marshalObjectToObject, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { deleteAllNotifications, deleteGroupNotifications, deleteNotifications, getAllNotifications, getNotificationsByGroupID, getUnprocessedNotifications, logNotificationInteraction, markAllAsProcessed, markAsProcessed, markGroupAsProcessed, } = require(path.join(__dirname, "./notificationState"));
function toPositiveInteger(value, fallback = 0) {
    const numericValue = Math.trunc(normalizeNumber(value, fallback));
    return numericValue > 0 ? numericValue : fallback;
}
function getSessionCharacterID(session) {
    return toPositiveInteger(session &&
...
class NotificationMgrService extends BaseService {
    ...
    Handle_GetAllNotifications(args, session, kwargs) {
        ...
        const fromID = readKeywordIntArg(kwargs, ["fromID"], toPositiveInteger(Array.isArray(args) ? args[0] : 0, 0));
    ...
    Handle_GetByGroupID(args, session, kwargs) {
        ...
        const groupID = readKeywordIntArg(kwargs, ["groupID"], toPositiveInteger(Array.isArray(args) ? args[0] : 0, 0));
    ...
    Handle_LogNotificationInteraction(args, _session, kwargs) {
        const referenceID = readKeywordIntArg(kwargs, ["referenceID"], toPositiveInteger(Array.isArray(args) ? args[0] : 0, 0));
        return logNotificationInteraction(referenceID);
    }
}
module.exports = NotificationMgrService;
//# sourceMappingURL=notificationMgrService.js.map