...
const { buildDict, buildFiletimeLong, buildKeyVal, buildList, normalizeNumber, normalizeText, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getAllianceRecord, getCorporationRecord, } = require(path.join(__dirname, "../corporation/corporationState"));
const { NOTIFICATION_GROUP, NOTIFICATION_TYPE, } = require(path.join(__dirname, "./notificationConstants"));
...
function ensureNotificationShape(record, notificationID, receiverID) {
    ...
    const normalizedNotificationID = toPositiveInteger(record.notificationID || notificationID, 0);
    const normalizedReceiverID = toPositiveInteger(record.receiverID || receiverID, 0);
...
function ensureNotificationBox(state, characterID) {
    ...
    for (const notificationID of box.order) {
        ...
        const normalizedRecord = ensureNotificationShape(box.byID[String(numericNotificationID)], numericNotificationID, key);
        if (!normalizedRecord) {
    ...
    return buildDict([
        ...
        [
            ...
            buildKeyVal([
                ...
                [
                    "toCharacterIDs",
                    buildList((Array.isArray(msg.toCharacterIDs) ? msg.toCharacterIDs : [])
...
function listNotifications(characterID, options = {}) {
    ...
    const groupID = options.groupID == null ? null : toPositiveInteger(options.groupID, 0) || null;
    const processed = typeof options.processed === "boolean" ? options.processed : null;
    const results = [];
    for (const notificationID of box.order) {
...
function getCharacterSessions(characterID, options = {}) {
    ...
    const addSession = (session) => {
        ...
        const sessionCharacterID = toPositiveInteger(session.characterID || session.charID || session.charid, 0);
        if (sessionCharacterID !== numericCharacterID ||
            seen.has(session) ||
...
function updateNewMailNotificationMessageState(characterID, updatesByMessageID = {}) {
    ...
    const updates = new Map(Object.entries(updatesByMessageID)
...
function deleteNewMailNotificationsForMessages(characterID, messageIDs = []) {
    const numericCharacterID = toPositiveInteger(characterID, 0);
    const messageIDSet = new Set((Array.isArray(messageIDs) ? messageIDs : [])
...
function mutateProcessed(characterID, notificationIDs, processed) {
    ...
    const normalizedIDs = [...new Set((Array.isArray(notificationIDs) ? notificationIDs : [])
            .map((notificationID) => toPositiveInteger(notificationID, 0))
            .filter((notificationID) => notificationID > 0))];
    if (normalizedIDs.length === 0) {
...
function markGroupAsProcessed(characterID, groupID) {
    ...
    return markAsProcessed(characterID, records.map((record) => record.notificationID));
}
...
function deleteNotificationIDs(characterID, notificationIDs, options = {}) {
    ...
    box.order = box.order.filter((notificationID) => !changedNotificationIDs.includes(notificationID));
    const writeResult = persistState(state);
    if (!writeResult || !writeResult.success) {
        return { success: false, errorMsg: "WRITE_ERROR" };
    }
...
function deleteGroupNotifications(characterID, groupID, options = {}) {
    ...
    return deleteNotificationIDs(characterID, records.map((record) => record.notificationID), options);
}
...
function createNewMailNotification(characterID, data = {}, options = {}) {
    const senderName = normalizeText(data.senderName, "") || resolveMailNotificationSenderName(data);
    ...
    return createNotification(characterID, {
        ...
        data: {
            ...
            msg: {
                ...
                senderName,
                sentDate: normalizeText(messageData.sentDate, "") ||
                ...
                toCharacterIDs: Array.isArray(messageData.toCharacterIDs)
                    ...
                    : [],
                toListID: messageData.toListID == null
                    ? null
                    : toPositiveInteger(messageData.toListID, 0) || null,
                toCorpOrAllianceID: messageData.toCorpOrAllianceID == null
                    ? null
