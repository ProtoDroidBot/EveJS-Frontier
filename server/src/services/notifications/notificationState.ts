...
function ensureNotificationBox(state, characterID) {
  ...
  for (const [notificationID, record] of Object.entries<any>(box.byID)) {
...
function listNotifications(characterID, options: Record<string, any> = {}) {
...
function getAllNotifications(characterID, options: Record<string, any> = {}) {
...
function getCharacterSessions(characterID, options: Record<string, any> = {}) {
  const numericCharacterID = toPositiveInteger(characterID, 0);
...
function sendNotificationReceivedEvent(record, options: Record<string, any> = {}) {
  if (!record || typeof record !== "object") {
    return;
  }
...
function sendNotificationDeletedEvent(characterID, notificationIDs, options: Record<string, any> = {}) {
...
function updateNewMailNotificationMessageState(characterID, updatesByMessageID: Record<string, any> = {}) {
  const numericCharacterID = toPositiveInteger(characterID, 0);
  if (numericCharacterID <= 0 || !updatesByMessageID || typeof updatesByMessageID !== "object") {
    return { success: false as const, mutated: false, changedNotificationIDs: [] };
  }
  ...
  if (updates.size === 0) {
    return { success: true as const, mutated: false, changedNotificationIDs: [] };
  }

  const state = getMutableState();
  const box = ensureNotificationBox(state, numericCharacterID);
  let mutated = false;
  const changedNotificationIDs: any[] = [];

  for (const record of Object.values<any>(box.byID || {})) {
  ...
  return { success: true as const, mutated, changedNotificationIDs };
}

function deleteNewMailNotificationsForMessages(characterID, messageIDs: any[] = []) {
  const numericCharacterID = toPositiveInteger(characterID, 0);
  ...
  if (numericCharacterID <= 0 || messageIDSet.size === 0) {
    return { success: false as const, mutated: false, deletedNotificationIDs: [] };
  }

  const state = getMutableState();
  const box = ensureNotificationBox(state, numericCharacterID);
  let mutated = false;
  const deletedNotificationIDs: any[] = [];
  ...
  return { success: true as const, mutated, deletedNotificationIDs };
}

function createNotification(characterID, options: Record<string, any> = {}) {
  const numericCharacterID = toPositiveInteger(characterID, 0);
  const typeID = toPositiveInteger(options.typeID, 0);
  if (numericCharacterID <= 0 || typeID <= 0) {
    return { success: false as const, errorMsg: "INVALID_NOTIFICATION" };
  }
...
function mutateProcessed(characterID, notificationIDs, processed) {
  ...
  if (normalizedIDs.length === 0) {
    return { success: true as const, changedNotificationIDs: [] };
  }
...
function deleteNotificationIDs(characterID, notificationIDs, options: Record<string, any> = {}) {
  const numericCharacterID = toPositiveInteger(characterID, 0);
  if (numericCharacterID <= 0) {
    return { success: false as const, errorMsg: "INVALID_CHARACTER" };
  }
...
function deleteNotifications(characterID, notificationIDs, options: Record<string, any> = {}) {
  return deleteNotificationIDs(characterID, notificationIDs, options);
}

function deleteGroupNotifications(characterID, groupID, options: Record<string, any> = {}) {
...
function deleteAllNotifications(characterID, options: Record<string, any> = {}) {
...
function resolveMailNotificationSenderName(data: Record<string, any> = {}) {
...
function createNewMailNotification(characterID, data: Record<string, any> = {}, options: Record<string, any> = {}) {
  const senderName =
