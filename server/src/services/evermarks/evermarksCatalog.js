...
const { COSMETIC_TYPE_ALLIANCE_LOGO, COSMETIC_TYPE_CORPORATION_LOGO, HERALDRY_CORPORATION_ID, SLOT_GROUP_EMBLEM, } = require("./evermarksConstants");
...
function normalizeLicenseRecord(typeID, value = {}) {
    ...
    return {
        ...
        cosmeticType: cosmeticType === COSMETIC_TYPE_ALLIANCE_LOGO
...
function readRoot() {
    ...
    return {
        ...
        licensesByTypeID: result.data.licensesByTypeID && typeof result.data.licensesByTypeID === "object"
            ? cloneValue(result.data.licensesByTypeID)
            : {},
        offersByOfferID: result.data.offersByOfferID && typeof result.data.offersByOfferID === "object"
            ? cloneValue(result.data.offersByOfferID)
            : {},
        offerIDsByTypeID: result.data.offerIDsByTypeID && typeof result.data.offerIDsByTypeID === "object"
...
function buildIndexes() {
    ...
    for (const [typeID, rawLicense] of Object.entries(root.licensesByTypeID || {})) {
        ...
        licensesByShipAndCosmeticType.set(`${license.shipTypeID}:${license.cosmeticType}`, license);
    }
...
function getLicenseByTypeID(typeID) {
    const license = buildIndexes().licensesByTypeID.get(normalizePositiveInteger(typeID, 0));
    return license ? cloneValue(license) : null;
}
function getLicenseByShipAndCosmeticType(shipTypeID, cosmeticType) {
    const license = buildIndexes().licensesByShipAndCosmeticType.get(`${normalizePositiveInteger(shipTypeID, 0)}:${Number(cosmeticType || 0)}`);
    return license ? cloneValue(license) : null;
}
function getHeraldryOfferByID(offerID) {
    const offer = buildIndexes().offersByOfferID.get(normalizePositiveInteger(offerID, 0));
...
//# sourceMappingURL=evermarksCatalog.js.map