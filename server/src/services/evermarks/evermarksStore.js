...
const { EVERMARK_ISSUER_CORP_ID, getCharacterWalletLPBalance, adjustCharacterWalletLPBalance, } = require(path.join(__dirname, "../corporation/lpWalletState"));
const { HERALDRY_CORPORATION_ID, } = require("./evermarksConstants");
const { getHeraldryOfferByID, listAllHeraldryOffers, } = require("./evermarksCatalog");
const { getOwnedShipLogoEntitlementByTypeID, grantShipLogoEntitlementByTypeID, } = require("./evermarksEntitlements");
const { publishShipLogoGrantedNotice, } = require("./evermarksNotices");
const { buildLpStoreOfferKeyVal, } = require(path.join(__dirname, "../loyalty/lpStoreOfferCatalog"));
...
function listHeraldryOffersForCharacter(characterID, corpID = HERALDRY_CORPORATION_ID) {
    ...
    return listAllHeraldryOffers().map((offer) => buildOfferKeyVal(offer, numericCharacterID));
}
function debitCharacterOfferCosts(characterID, offer, options = {}) {
    ...
    const currentEvermarks = getCharacterWalletLPBalance(numericCharacterID, HERALDRY_CORPORATION_ID);
    ...
    if (lpCost > 0) {
        const lpResult = adjustCharacterWalletLPBalance(numericCharacterID, HERALDRY_CORPORATION_ID, -lpCost, {
    ...
    if (iskCost > 0) {
        const iskResult = adjustCharacterBalance(numericCharacterID, -iskCost, {
            description: options.iskDescription || "Heraldry offer purchase",
        });
        if (!iskResult.success) {
            if (lpCost > 0) {
                adjustCharacterWalletLPBalance(numericCharacterID, HERALDRY_CORPORATION_ID, lpCost, {
...
function refundCharacterOfferCosts(characterID, offer) {
    ...
    if (iskCost > 0) {
        adjustCharacterBalance(numericCharacterID, iskCost, {
...
function takeHeraldryOfferForCharacter(session, corpID, offerID, numberOfOffers = 1) {
    const numericCharacterID = normalizePositiveInteger(session && session.characterID, 0);
    const numericCorpID = normalizePositiveInteger(corpID, HERALDRY_CORPORATION_ID);
    ...
    const grantResult = grantShipLogoEntitlementByTypeID(numericCharacterID, offer.typeID, {
...
//# sourceMappingURL=evermarksStore.js.map