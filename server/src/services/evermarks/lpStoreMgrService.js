...
const { buildBoundObjectResponse, buildList, resolveBoundNodeId, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { adjustCharacterBalance, getCharacterWallet, } = require(path.join(__dirname, "../account/walletState"));
const { adjustCharacterWalletLPBalance, getCharacterWalletLPBalance, } = require(path.join(__dirname, "../corporation/lpWalletState"));
...
const { HERALDRY_CORPORATION_ID, listHeraldryOffersForCharacter, takeHeraldryOfferForCharacter, } = require("./evermarksStore");
const { hasStaticLpStoreOffers, getStaticLpStoreOfferRecord, listStaticLpStoreOffers, } = require(path.join(__dirname, "../loyalty/lpStoreOfferCatalog"));
...
function throwStoreError(errorMsg) {
    throwWrappedUserError("CustomNotify", {
        notify: ERROR_MESSAGES[String(errorMsg || "").toUpperCase()] ||
...
function ensureStructureLoyaltyStoreAccess(session) {
    ...
    if (!structure ||
        !characterHasStructureService(session, structure, STRUCTURE_SERVICE_ID.LOYALTY_STORE)) {
...
function aggregateRequirementPairs(reqItems = [], numberOfOffers = 1) {
    ...
    for (const pair of Array.isArray(reqItems) ? reqItems : []) {
        ...
        quantitiesByTypeID.set(typeID, (quantitiesByTypeID.get(typeID) || 0) + (quantity * numberOfOffers));
    }
...
function getItemStackQuantity(item) {
    ...
    return Math.max(0, Math.trunc(Number(item.stacksize ?? item.quantity) || 0));
}
...
function hasRequiredItems(characterID, locationID, requirements = []) {
    ...
    const { ITEM_FLAGS, listContainerItems, } = getItemStore();
    ...
    for (const item of listContainerItems(characterID, locationID, ITEM_FLAGS.HANGAR)) {
        ...
        availableByTypeID.set(typeID, (availableByTypeID.get(typeID) || 0) + getItemStackQuantity(item));
    }
    return requirements.every(([typeID, quantity]) => (availableByTypeID.get(typeID) || 0) >= quantity);
}
function refundRequiredItems(characterID, locationID, consumedRequirements = []) {
    const { ITEM_FLAGS, grantItemToCharacterLocation, } = getItemStore();
    for (const [typeID, quantity] of consumedRequirements) {
        grantItemToCharacterLocation(characterID, locationID, ITEM_FLAGS.HANGAR, typeID, quantity, { singleton: 0 });
    }
}
function takeRequiredItems(characterID, locationID, requirements = []) {
    const { ITEM_FLAGS, takeItemTypeFromCharacterLocation, } = getItemStore();
    const changes = [];
    const consumedRequirements = [];
    for (const [typeID, quantity] of requirements) {
        const takeResult = takeItemTypeFromCharacterLocation(characterID, locationID, ITEM_FLAGS.HANGAR, typeID, quantity);
...
function rollbackStaticLpStorePurchase({ characterID, corpID, locationID, lpCost = 0, iskCost = 0, consumedRequirements = [], }) {
    if (lpCost > 0) {
...
function resolveInventoryNotificationItem(change) {
    ...
    if (change.removed === true && change.previousData) {
        const { buildRemovedItemNotificationState, } = getItemStore();
...
function syncInventoryChangesToSession(session, changes = []) {
    ...
    for (const change of changes) {
        ...
        syncInventoryItemForSession(session, notificationItem, change.previousData || change.previousState || {}, { emitCfgLocation: true });
    }
}
...
function takeStaticOfferForCharacter(session, corpID, offerID, numberOfOffers = 1) {
    ...
    const grantedQuantity = normalizeStaticOfferQuantity(offer.qty, 1) * normalizedNumberOfOffers;
    const requirements = aggregateRequirementPairs(offer.reqItems, normalizedNumberOfOffers);
    ...
    if (lpCost > 0) {
        const lpResult = adjustCharacterWalletLPBalance(characterID, normalizedCorpID, -lpCost, { changeType: "lp_store_purchase" });
        if (!lpResult.success) {
            return lpResult;
        }
    }
    ...
    const { ITEM_FLAGS, grantItemToCharacterLocation, } = getItemStore();
    const grantResult = grantItemToCharacterLocation(characterID, locationID, ITEM_FLAGS.HANGAR, offer.typeID, grantedQuantity);
...
class LPStoreMgrService extends BaseService {
    ...
    Handle_GetAvailableOffersFromCorp(args, session) {
        ...
        if (corpID === HERALDRY_CORPORATION_ID) {
            offers = listHeraldryOffersForCharacter(session && session.characterID, corpID);
        }
    ...
    Handle_TakeOfferForCharacter(args, session) {
        ...
        const result = normalizedCorpID === HERALDRY_CORPORATION_ID
            ? takeHeraldryOfferForCharacter(session, normalizedCorpID, offerID, numberOfOffers)
            : takeStaticOfferForCharacter(session, normalizedCorpID, offerID, numberOfOffers);
        if (!result.success) {
...
//# sourceMappingURL=lpStoreMgrService.js.map