...
function buildEntitlementIdentifier(entitlement: Record<string, any> = {}) {
  return {
...
function publishNotice(noticeTypeName, messageType, payload, targetCharacterID, options: Record<string, any> = {}) {
...
function publishShipLogoGrantedNotice(entitlement, options: Record<string, any> = {}) {
...
function publishShipLogoRevokedNotice(entitlement, revokerCharacterID, options: Record<string, any> = {}) {
  const identifier = buildEntitlementIdentifier(entitlement);
  const entitlementType = normalizePositiveInteger(
    entitlement && entitlement.entitlementType,
    0,
