...
function aggregateRequirementPairs(reqItems: any[] = [], numberOfOffers = 1) {
...
function hasRequiredItems(characterID, locationID, requirements: any[] = []) {
...
function refundRequiredItems(characterID, locationID, consumedRequirements: any[] = []) {
  const {
    ITEM_FLAGS,
...
function takeRequiredItems(characterID, locationID, requirements: any[] = []) {
  const {
    ITEM_FLAGS,
    takeItemTypeFromCharacterLocation,
  } = getItemStore();
  const changes: any[] = [];
  const consumedRequirements: any[] = [];
...
function notifyStaticLpStoreInventoryChanges(session, characterID, changes: any[] = []) {
