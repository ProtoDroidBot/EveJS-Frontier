...
const { COSMETIC_TYPE_BY_ENTITLEMENT_TYPE, ENTITLEMENT_TYPE_BY_COSMETIC_TYPE, } = require("./evermarksConstants");
const { getLicenseByShipAndCosmeticType, getLicenseByTypeID, } = require("./evermarksCatalog");
...
function normalizeEntitlementRecord(value = {}) {
    ...
    const cosmeticType = Number(value.cosmeticType || 0) || COSMETIC_TYPE_BY_ENTITLEMENT_TYPE[entitlementType] || 0;
    const license = getLicenseByShipAndCosmeticType(shipTypeID, cosmeticType) ||
    ...
    return {
        ...
        cosmeticType,
        shipTypeID,
        licenseID: normalizePositiveInteger(value.licenseID, license && license.licenseID) || normalizePositiveInteger(license && license.licenseID, 0),
        fsdTypeID: normalizePositiveInteger(value.fsdTypeID, license && license.fsdTypeID) || normalizePositiveInteger(license && license.fsdTypeID, 0),
...
function readRoot() {
    ...
    return {
        meta: result.data.meta && typeof result.data.meta === "object"
            ? {
                ...DEFAULT_ROOT.meta,
        ...
        characters: result.data.characters && typeof result.data.characters === "object"
...
function writeRoot(root) {
    const nextRoot = {
        ...
        characters: root && root.characters && typeof root.characters === "object"
...
function buildIndexes() {
    ...
    for (const [characterID, rawCharacter] of Object.entries(root.characters || {})) {
        ...
        const rawEntitlements = rawCharacter && rawCharacter.shipLogosByKey && typeof rawCharacter.shipLogosByKey === "object"
        ...
        byCharacterAndTypeID.set(numericCharacterID, new Map(normalized.map((entry) => [entry.fsdTypeID, entry])));
    }
    cachedIndexes = {
        byCharacter,
...
function listOwnedShipLogoEntitlements(characterID) {
    return cloneValue(buildIndexes().byCharacter.get(normalizePositiveInteger(characterID, 0)) || []);
}
function getOwnedShipLogoEntitlement(characterID, shipTypeID, entitlementType) {
    return (listOwnedShipLogoEntitlements(characterID).find((entry) => entry.shipTypeID === normalizePositiveInteger(shipTypeID, 0) &&
        entry.entitlementType === normalizePositiveInteger(entitlementType, 0)) || null);
}
function getOwnedShipLogoEntitlementByTypeID(characterID, typeID) {
    const entry = (buildIndexes().byCharacterAndTypeID.get(normalizePositiveInteger(characterID, 0)) ||
        new Map()).get(normalizePositiveInteger(typeID, 0));
...
function grantShipLogoEntitlement(characterID, shipTypeID, entitlementType, options = {}) {
    ...
    if (!root.characters[characterKey].shipLogosByKey ||
...
function revokeShipLogoEntitlement(characterID, shipTypeID, entitlementType) {
    ...
    const characterKey = String(normalizedCharacterID);
    const entitlementKey = buildEntitlementKey(normalizedEntitlementType, normalizedShipTypeID);
    const currentRecord = root.characters &&
        root.characters[characterKey] &&
        root.characters[characterKey].shipLogosByKey
...
//# sourceMappingURL=evermarksEntitlements.js.map