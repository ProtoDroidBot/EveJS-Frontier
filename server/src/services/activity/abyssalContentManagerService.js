...
const { buildDict, buildList } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
function normalizeSpawn(entry = {}) {
    ...
    return {
        ...
        npc_fleet_spawn_tag_names: normalizeStringArray(entry.npc_fleet_spawn_tag_names ?? entry.npcFleetSpawnTagNames),
        npc_fleet_spawn_tags: normalizeIntegerArray(entry.npc_fleet_spawn_tags ?? entry.npcFleetSpawnTags),
        npcs,
    };
}
...
function normalizeReport(rawReport, difficultyTier, spawnTableID, repetitions) {
    ...
    return {
        ...
        average_spawn_points: Math.max(0, toInteger(source.average_spawn_points ?? source.averageSpawnPoints, 0)),
        average_spent_points: Math.max(0, toInteger(source.average_spent_points ?? source.averageSpentPoints, 0)),
        average_remaining_points: Math.max(0, toInteger(source.average_remaining_points ?? source.averageRemainingPoints, 0)),
        average_number_of_npcs: Math.max(0, toInteger(source.average_number_of_npcs ?? source.averageNumberOfNpcs, 0)),
        unsupported_tag_names: normalizeStringArray(source.unsupported_tag_names ?? source.unsupportedTagNames),
        unsupported_tags: normalizeIntegerArray(source.unsupported_tags ?? source.unsupportedTags),
        spawns,
    };
}
...
class AbyssalContentManagerService extends BaseService {
    ...
    Handle_generate_abyss_npc_spawn_report(args, session) {
        ...
        const repetitions = Math.min(MAX_REPETITIONS, Math.max(0, toInteger(args && args[2], 0)));
        if (typeof reportProvider === "function") {
            ...
            const report = normalizeReport(provided, difficultyTier, spawnTableID, repetitions);
        ...
        log.debug("[AbyssalContentManager] Returning empty NPC spawn report: authored Abyssal spawn tables are unavailable");
...
//# sourceMappingURL=abyssalContentManagerService.js.map