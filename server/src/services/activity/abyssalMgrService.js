...
function buildErrorList(errors) {
    return buildList((Array.isArray(errors) ? errors : [])
        .map(([code, args]) => [
...
class AbyssalMgrService extends BaseService {
    ...
    Handle_AbyssalEntranceDeployment(args, session) {
        ...
        log.debug("[AbyssalMgr] AbyssalEntranceDeployment rejected: Abyssal content generation is not available");
    ...
    Handle_AbyssalEntranceGateActivation(args, session) {
        ...
        log.debug("[AbyssalMgr] AbyssalEntranceGateActivation rejected: no runtime Abyssal entrance trace exists");
    ...
    Handle_AbyssalGateActivation(args, session) {
        ...
        log.debug("[AbyssalMgr] AbyssalGateActivation rejected: no active Abyssal pocket runtime state exists");
        throwGateRejected();
    }
    Handle_AbyssalEndGateActivation(args, session) {
        recordAuditEvent("abyssal_end_gate_activation_rejected", args, session);
        log.debug("[AbyssalMgr] AbyssalEndGateActivation rejected: no active Abyssal origin trace exists");
        throwGateRejected();
    }
...
//# sourceMappingURL=abyssalMgrService.js.map