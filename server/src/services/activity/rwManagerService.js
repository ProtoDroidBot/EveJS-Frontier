...
const { buildDict, normalizeNumber, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
function normalizeStationEntries(entries = []) {
    const sourceEntries = entries instanceof Map
        ? [...entries.entries()]
    ...
    return sourceEntries
        ...
        .filter(([corporationID, stationID]) => isResourceWarsCorporationID(corporationID) && stationID > 0);
}
...
function setClosestStationMapForTests(systemIDOrEntries, maybeEntries) {
    if (maybeEntries === undefined) {
        defaultClosestStationIDsByCorpID = new Map(normalizeStationEntries(systemIDOrEntries));
        return;
    }
    ...
    closestStationIDsBySystemID.set(solarSystemID, new Map(normalizeStationEntries(maybeEntries)));
}
...
class RWManagerService extends BaseService {
    ...
    Handle_get_closest_rw_stations(args, session) {
        ...
        const stationIDsByCorpID = closestStationIDsBySystemID.get(requestedSolarSystemID) ||
...
//# sourceMappingURL=rwManagerService.js.map