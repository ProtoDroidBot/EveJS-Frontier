...
const { buildClientGlobalConfigEntries, } = require(path.join(__dirname, "../newEdenStore/storeState"));
const { buildSuiClientGlobalConfigEntries, } = require(path.join(__dirname, "../frontier/suiGrpcClient"));
...
function normalizeCountryCode(value, fallback = getRuntimeConfig().defaultCountryCode) {
    const fallbackCountryCode = coerceCountryCode(fallback) || SAFE_DEFAULT_COUNTRY_CODE;
...
function buildGlobalConfigEntries(runtimeConfig = getRuntimeConfig()) {
    const baseEntries = [
        ...
        ...buildSuiClientGlobalConfigEntries(runtimeConfig.clientCompatibilityProfile),
...
function buildServerStatusMessage(runtimeConfig = getRuntimeConfig()) {
    ...
    if (statusLabel === "/Carbon/MachoNet/ServerStatus/StartingUp") {
        return [
            statusLabel,
            {
                progress: coerceNonNegativeInteger(runtimeConfig.serverStatusProgressSeconds, 0),
            },
        ];
    }
    if (statusLabel === "/Carbon/MachoNet/ServerStatus/ProxyFullWithLimit") {
        return [
            statusLabel,
            {
                limit: Math.max(1, coerceNonNegativeInteger(runtimeConfig.serverStatusProxyLimit, 1)),
            },
        ];
    }
    return [statusLabel, {}];
