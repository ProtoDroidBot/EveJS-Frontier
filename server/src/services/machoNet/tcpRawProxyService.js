...
const { buildDict, normalizeText } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const auditEvents = [];
let tunnelHost = "";
...
function buildPortDict() {
    return buildDict([...tunnelPortsByNodeID.entries()]
...
TcpRawProxyService._testing = {
    ...
    setTunnelsForTests(host, portsByNodeID) {
        ...
        tunnelPortsByNodeID = new Map(Object.entries(portsByNodeID || {})
            .map(([nodeID, port]) => [toInteger(nodeID, 0), toInteger(port, 0)])
            .filter(([nodeID, port]) => nodeID > 0 && port > 0));
    },
};
module.exports = TcpRawProxyService;
//# sourceMappingURL=tcpRawProxyService.js.map