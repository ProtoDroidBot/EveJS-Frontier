...
const { buildDict, buildKeyVal, extractDictEntries, normalizeNumber, normalizeText, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { buildGlobalConfigDict, buildServerStatusResponse, getRuntimeGlobalConfigOverrides, getRuntimeGlobalConfigValue, resetRuntimeGlobalConfigForTests, setRuntimeGlobalConfigValue, } = require(path.join(__dirname, "./globalConfig"));
...
function recordAuditEvent(kind, session = null, extra = {}) {
    auditEvents.push({
        kind,
        accountID: toInteger(session && (session.userid || session.userID || session.accountID), 0) || null,
        characterID: toInteger(session && (session.characterID || session.charID || session.charid), 0) || null,
        ...extra,
...
class MachoNetService extends BaseService {
    ...
    Handle_GetGlobalConfigValue(args, session, kwargs) {
        const key = toText((args && args[0]) || getKwarg(kwargs, "key", ""), "").trim();
        if (!key) {
            return ["", ""];
        }
    ...
    Handle_SetGlobalConfigValue(args, session, kwargs) {
        const key = toText(getKwarg(kwargs, "key", args && args[0]), "").trim();
        const rawValue = getKwarg(kwargs, "value", args && args[1]);
        const value = unwrapMarshalValue(rawValue);
        const isDelete = toBoolean(getKwarg(kwargs, "isDelete", args && args[2]), false);
        const isUpdate = toBoolean(getKwarg(kwargs, "isUpdate", args && args[3]), true);
        const isUpdateClients = toBoolean(getKwarg(kwargs, "isUpdateClients", args && args[4]), true);
        if (!key) {
        ...
        log.debug(`[MachoNet] SetGlobalConfigValue key=${key} deleted=${isDelete} notify=${notifiedSessions}`);
        return null;
    }
    ...
    Handle_ForwardCharacterNotification(args, session) {
        ...
        if (characterID > 0 && methodName) {
            ...
            for (const targetSession of sessions) {
                const targetCharacterID = toInteger(targetSession && (targetSession.characterID ||
                    targetSession.charID ||
                    targetSession.charid), 0);
...
//# sourceMappingURL=machoNetService.js.map