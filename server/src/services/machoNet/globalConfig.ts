...
} = require(path.join(__dirname, "../frontier/suiGrpcClient"));

const COUNTRY_CODE_PATTERN = /^[A-Z]{2}$/;
const SAFE_DEFAULT_COUNTRY_CODE = "GB";
const runtimeGlobalConfigOverrides = new Map();
const runtimeGlobalConfigDeletedKeys = new Set<any>();
...
function setRuntimeGlobalConfigValue(key, value, options: Record<string, any> = {}) {
...
function getRuntimeGlobalConfigOverrides() {
  return {
    ...
    deletedKeys: Array.from<any>(runtimeGlobalConfigDeletedKeys.values()),
  };
}
