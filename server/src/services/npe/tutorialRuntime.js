...
function isNewCharacterIntroCinematicEnabled(runtimeConfig = getRuntimeConfig()) {
...
function normalizeTutorialEntryMode(value, fallback = TUTORIAL_ENTRY_MODE.DISABLED) {
    const normalized = String(value || "").trim().toLowerCase();
    if (normalized === LEGACY_TUTORIAL_ENTRY_MODE.CINEMATIC_ENTRY_TO_SPACE ||
...
function normalizeTutorialFirstLoginHandoff(value, fallback = TUTORIAL_FIRST_LOGIN_HANDOFF.NONE) {
    const normalized = String(value || "").trim().toLowerCase();
...
function resolveNewCharacterTutorialEntryMode(runtimeConfig = getRuntimeConfig()) {
...
function buildTutorialSnapshot(record, runtimeConfig = getRuntimeConfig()) {
    const entryMode = normalizeTutorialEntryMode(record && record.tutorialEntryMode, inferLegacyTutorialEntryMode(record));
    ...
    const isCinematicEntry = entryMode ===
        ...
        isNewCharacterIntroCinematicEnabled(runtimeConfig);
    const storedAirNpeState = normalizeAirNpeState(record && record.airNpeState, AIR_NPE_STATE.COMPLETED);
    const effectiveAirNpeState = enabled
        ? storedAirNpeState
        : AIR_NPE_STATE.COMPLETED;
    const storedNesIntroState = normalizeNesIntroState(record && record.nesIntroState, NES_INTRO_STATE.UNSET);
    const effectiveNesIntroState = enabled
        ? storedNesIntroState
        : NES_INTRO_STATE.UNSET;
    const firstLoginHandoff = enabled
        ? normalizeTutorialFirstLoginHandoff(record && record.tutorialFirstLoginHandoff, inferLegacyFirstLoginHandoff(record))
        : TUTORIAL_FIRST_LOGIN_HANDOFF.NONE;
...
function getCharacterTutorialSnapshot(charId, runtimeConfig = getRuntimeConfig()) {
...
function markCharacterTutorialSkipped(charId, runtimeConfig = getRuntimeConfig()) {
    const numericCharId = Number(charId) || 0;
    if (numericCharId <= 0) {
        return buildMissingCharacterResult(runtimeConfig);
    }
    const currentSnapshot = getCharacterTutorialSnapshot(numericCharId, runtimeConfig);
    if (!currentSnapshot.exists) {
        return buildMissingCharacterResult(runtimeConfig);
    }
    const canForceSkipCompletedCinematicEntry = currentSnapshot.entryMode === TUTORIAL_ENTRY_MODE.CINEMATIC_INTRO_OVERLAY &&
        currentSnapshot.firstLoginHandoff !== TUTORIAL_FIRST_LOGIN_HANDOFF.NONE &&
        currentSnapshot.airNpeState === AIR_NPE_STATE.COMPLETED;
    if (currentSnapshot.airNpeState !== AIR_NPE_STATE.ACTIVE &&
...
function consumeTutorialFirstLoginHandoff(charId, runtimeConfig = getRuntimeConfig()) {
    const numericCharId = Number(charId) || 0;
    if (numericCharId <= 0) {
        return {
...
function consumeCompletedAirNpeRevealOnFirstLogin(charId, runtimeConfig = getRuntimeConfig()) {
    const numericCharId = Number(charId) || 0;
    if (numericCharId <= 0) {
        return buildMissingCharacterResult(runtimeConfig);
    }
...
function markCharacterNesIntroSkipped(charId, runtimeConfig = getRuntimeConfig()) {
    const numericCharId = Number(charId) || 0;
    if (numericCharId <= 0) {
        return buildMissingCharacterResult(runtimeConfig);
    }
...
//# sourceMappingURL=tutorialRuntime.js.map