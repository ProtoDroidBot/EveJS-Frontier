...
const { NEW_CHARACTER_START_OVERRIDE, } = require(path.join(__dirname, "../_shared/newCharacterStartOverride"));
const { TUTORIAL_ENTRY_MODE, normalizeTutorialEntryMode, } = require(path.join(__dirname, "./tutorialRuntime"));
...
function buildCinematicSpaceSpawnContext(options = {}) {
    ...
    const undockState = spaceRuntime.getStationUndockSpawnState(station, {
        ...
        selectionKey: Number(options.selectionKey || options.characterID || 0) || undefined,
    });
    return {
        success: true,
        data: {
...
//# sourceMappingURL=tutorialEntrySpawnRuntime.js.map