...
const { TUTORIAL_FIRST_LOGIN_HANDOFF, } = require(path.join(__dirname, "./tutorialRuntime"));
...
function resolveReturnDockLocationID(session) {
    return Number((session && (session.stationid ||
        session.stationID ||
        session.structureid ||
        session.structureID ||
        ...
        session.clonestationid)) || 0) || 0;
}
function runTutorialFirstLoginHandoff(session, handoffMode, options = {}) {
    ...
    const dockLocationID = Number(options.dockLocationID || resolveReturnDockLocationID(session)) || 0;
    if (dockLocationID <= 0) {
        return {
            success: false,
            shouldConsume: false,
...
//# sourceMappingURL=tutorialHandoffRuntime.js.map