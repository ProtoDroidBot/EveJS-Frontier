...
class ServiceManager {
  declare _boundObjectParams: any;
  declare _boundObjects: any;
  declare _services: any;
   ...
   */
  unregisterBoundObject(oidString, options: Record<string, any> = {}) {
    if (typeof oidString !== "string" || oidString === "") {
      return false;
    }
    ...
    if (session && session._boundObjectIDs && typeof session._boundObjectIDs === "object") {
      for (const [serviceName, objectID] of Object.entries<any>(session._boundObjectIDs)) {
    ...
    if (session && session._boundObjectState && typeof session._boundObjectState === "object") {
      for (const [serviceName, state] of Object.entries<any>(session._boundObjectState)) {
   ...
   */
  getServiceNames() {
    return Array.from<any>(this._services.keys());
  }
