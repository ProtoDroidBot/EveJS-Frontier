...
function getTypeDogmaByID() {
  ...
  typeDogmaCache = new Map(
    Object.entries<any>(rows).map(([typeID, row]) => [toInt(typeID, 0), row]),
  );
  return typeDogmaCache;
}
...
function getDogmaAttributeTypeByID() {
  ...
  dogmaAttributeTypeCache = new Map(
    Object.entries<any>(rows).map(([attributeID, row]) => [toInt(attributeID, 0), row]),
  );
  return dogmaAttributeTypeCache;
}

function normalizeSchematicRow(row: Record<string, any> = {}) {
  return {
