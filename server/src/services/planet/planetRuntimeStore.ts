...
function normalizeState(rawState: Record<string, any> = {}) {
...
function readState(options: Record<string, any> = {}) {
...
function getAcceptedNetworkEditBucket(state, key, options: Record<string, any> = {}) {
...
function markAcceptedNetworkEdit(state, key, editHash, details: Record<string, any> = {}) {
  if (!editHash) {
    return;
  }
...
function normalizeResourceTypeIDs(resourceTypeIDs: any[] = []) {
  return [...new Set(
...
function hashRatio(parts: any[] = []) {
  return stableHash(parts) / 0xffffffff;
}

function signedHashRatio(parts: any[] = []) {
...
function normalizeResourceLayer(layer: Record<string, any> = {}) {
...
function buildResourceRecord(planetMeta, resourceTypeIDs: any[] = []) {
  ...
  const qualitiesByTypeID: Record<string, any> = {};
  const layersByTypeID: Record<string, any> = {};
...
function getOrCreatePlanetResources(planetMeta, resourceTypeIDs: any[] = []) {
  const planetID = normalizeInteger(planetMeta && planetMeta.planetID, 0);
  if (planetID <= 0) {
...
function getResourceLayer(planetID, resourceTypeID, options: Record<string, any> = {}) {
  const normalizedPlanetID = normalizeInteger(planetID, 0);
...
function buildResourceLayerCoefficient(layer, coefficientIndex, options: Record<string, any> = {}) {
...
function buildResourceLayerBuffer(layer, numBands, options: Record<string, any> = {}) {
...
function getResourceDataForClient(planetMeta, resourceTypeID, resourceTypeIDs: any[] = [], info: Record<string, any> = {}) {
...
function evaluateResourceLayerValue(layer, latitude, longitude, options: Record<string, any> = {}) {
...
function evaluateResourceValueAt(planetID, resourceTypeID, latitude, longitude, options: Record<string, any> = {}) {
...
function pruneDepletionEvents(events: any[] = [], nowMs = Date.now()) {
...
function collectUsedIDs(state, fieldName) {
  ...
  for (const colony of Object.values<any>(state.coloniesByKey || {})) {
  ...
  if (fieldName === "launchID") {
    for (const [key, launch] of Object.entries<any>(state.launchesByID || {})) {
...
function allocateNextID(state, key, usedIDs = new Set<any>()) {
  if (!isPlainObject(state.nextIDs)) {
    state.nextIDs = cloneJson(DEFAULT_NEXT_IDS);
  }
...
function normalizeContents(contents) {
  ...
  const normalizedContents: Record<string, any> = {};
  for (const [typeID, quantity] of Object.entries<any>(contents)) {
...
function normalizeLaunchRecord(launch: Record<string, any> = {}) {
...
function summarizeContents(contents: Record<string, any> = {}) {
...
function normalizePin(pin: Record<string, any> = {}, ownerID = 0) {
  ...
  const normalizedPin: Record<string, any> = {
    ...pin,
    id: pinID,
    pinID,
...
function buildLaunchCoordinates(planetMeta: Record<string, any> = {}, launchID = 0) {
...
function normalizeColony(rawColony, context: Record<string, any> = {}) {
...
function validatePinTypeForCreate(colony, typeID, context: Record<string, any> = {}) {
  const normalizedTypeID = normalizeInteger(typeID, 0);
...
function getSortedOutputRoutes(colony, sourcePinID, commodities: Record<string, any> = {}) {
...
function moveCommodityOverRoute(colony, route, sourcePin, typeID, maxQuantity, options: Record<string, any> = {}) {
...
function routeCommodityOutput(colony, sourcePin, commodities: Record<string, any> = {}, options: Record<string, any> = {}) {
...
function repairEcuSourceRoutes(colony) {
  ...
  const routeIDsToRemove = new Set<any>();
  ...
  for (const ecuPin of ecuPins) {
    ...
    const productQuantity = normalizeInteger(
      getPinProducts(ecuPin)[String(programTypeID)],
      0,
    );
    const processRoutes: any[] = [];
    const storageRoutes: any[] = [];
    ...
    for (const route of sourceRoutes) {
      ...
      if (
        normalizeInteger(route.commodityTypeID, 0) !== programTypeID ||
    ...
    for (const route of processRoutes) {
      ...
      const quantity = Math.min(
        normalizeInteger(route.commodityQuantity, 0),
        quantityRemaining,
      );
      if (quantity <= 0) {
        routeIDsToRemove.add(routeID);
    ...
    const formerStorageQuantity = storageRoutes.reduce((total, route) => (
      total + normalizeInteger(route.commodityQuantity, 0)
    ), 0);
      ...
      if (formerStorageQuantity > 0 && quantityAvailable > 0) {
        if (isLastRoute) {
          ...
          const proportionalQuantity =
            (formerQuantity / formerStorageQuantity) * storageQuantityLimit;
...
function runColonySimulation(rawColony, targetFileTime = currentFileTimeString(), context: Record<string, any> = {}) {
  ...
  const maxEvents = Math.max(
    ...
    Math.min(
      ...
      normalizeInteger(context.maxEvents, MAX_SIMULATION_EVENTS),
    ),
  );
  ...
  const truncated = eventCount >= maxEvents && Boolean(
    getNextSimulationEvent(colony, targetTime),
  );
  if (truncated) {
    log.warn(
...
function colonySnapshotFingerprint(rawColony, context: Record<string, any> = {}) {
  return JSON.stringify(normalizeColony(rawColony, context));
}

function buildColonySimulationPlan(
  rawColony,
  targetFileTime,
...
function getLinkCpuUsage(link, colony, context: Record<string, any> = {}) {
...
function getLinkPowerUsage(link, colony, context: Record<string, any> = {}) {
  const params = planetStaticData.getUsageParametersForLinkType(link && link.typeID);
  const pinA = findPin(colony, link && link.endpoint1);
  const pinB = findPin(colony, link && link.endpoint2);
  const length = getPinDistanceMeters(pinA, pinB, context.planetRadius);
...
function getColonyCpuPowerStats(colony, context: Record<string, any> = {}) {
...
function getCommodityTotalVolume(commodities: Record<string, any> = {}) {
...
function getExpeditedTransferTimeTicks(linkBandwidth, commodities: Record<string, any> = {}) {
...
function normalizeTransferPath(path: any[] = []) {
...
function getMinimumLinkBandwidthForPath(colony, path: any[] = []) {
...
function validateRouteSemantics(colony, route, context: Record<string, any> = {}) {
  const path = Array.isArray(route && route.path) ? route.path : [];
  const typeID = normalizeInteger(route && route.commodityTypeID, 0);
  const quantity = normalizeInteger(route && route.commodityQuantity, 0);
...
function validateColonyRoutesAndLinks(colony, context: Record<string, any> = {}) {
  const seenLinkKeys = new Set<any>();
  for (const link of Array.isArray(colony.links) ? colony.links : []) {
...
function validateColonyCpuPower(colony, context: Record<string, any> = {}) {
...
function validateColonyNetwork(colony, context: Record<string, any> = {}) {
...
function normalizeHeads(heads: any[] = []) {
...
function getOwnHeadModifiers(heads, headRadius, overlapFactor) {
  const modifiers = new Map<any, any>(heads.map((head) => [normalizeInteger(head[0], 0), 1]));
...
function installECUProgram(pin, programTypeID, headRadius, context: Record<string, any> = {}) {
...
function normalizeCommandStream(serializedChanges: any[] = [], maxCommands = Infinity) {
  ...
  if (stream.length > maxCommands) {
    throw new Error(
      `PI network command limit exceeded (${stream.length}/${maxCommands})`,
    );
  }
...
function ensurePinsExist(colony, pinIDs: any[] = []) {
...
function applyUserUpdateNetwork({
  ...
  const commandStream = normalizeCommandStream(
    commands || serializedChanges,
    MAX_NETWORK_COMMANDS,
  );
  if (commandStream.length > MAX_NETWORK_COMMANDS) {
    throw new Error(
      `PI network command limit exceeded (${commandStream.length}/${MAX_NETWORK_COMMANDS})`,
    );
  }
...
function previewUserUpdateNetwork(options: Record<string, any> = {}) {
...
function prepareLaunchCommodities({
  ...
  if (normalizedPlanetID <= 0 || normalizedOwnerID <= 0 || normalizedCommandPinID <= 0) {
    return { success: false as const, errorMsg: "CannotLaunchWithoutColony" };
  }

  const commoditiesToLaunch = normalizeContents(commodities);
  if (Object.keys(commoditiesToLaunch).length < 1) {
    return { success: false as const, errorMsg: "PleaseSelectCommoditiesToLaunch" };
  }
  ...
  if (!commandPin || getPinEntityType(commandPin) !== "command") {
    return { success: false as const, errorMsg: "CanOnlyLaunchFromCommandCenters" };
  }
  ...
  if (lastLaunchTime > 0n && lastLaunchTime + COMMAND_CENTER_LAUNCH_CYCLE_TICKS > currentLaunchTime) {
    return { success: false as const, errorMsg: "CannotLaunchCommandPinNotReady" };
  }

  for (const [typeID, quantity] of Object.entries(commoditiesToLaunch)) {
    if (getContentsQuantity(commandPin, typeID) < quantity) {
      return { success: false as const, errorMsg: "CannotLaunchCommoditiesNotFound" };
    }
  }
...
function previewLaunchCommodities(options: Record<string, any> = {}) {
...
function launchCommodities(options: Record<string, any> = {}) {
  const {
    planetID,
    ownerID,
    solarSystemID = 0,
 ...
 */
function ensureColonyCaughtUpAsync(
  planetID,
  ownerID,
  context: Record<string, any> = {},
  targetFileTime = currentFileTimeString(),
) {
  const normalizedPlanetID = normalizeInteger(planetID, 0);
  const normalizedOwnerID = normalizeInteger(ownerID, 0);
  if (normalizedPlanetID <= 0 || normalizedOwnerID <= 0) {
  ...
  const catchup = (async () => {
    ...
    while (staleRetries < 4 && simulationChunks < 8) {
      ...
      const baseFingerprint = colonySnapshotFingerprint(
        rawColony,
        simulationContext,
      );
      const plan = await planetSimulationPool.buildColonySimulationPlan(
        rawColony,
        targetFileTime,
        simulationContext,
        baseFingerprint,
      ...
      if (!latestColony) return null;
      const latestFingerprint = colonySnapshotFingerprint(
        latestColony,
        simulationContext,
      );
      ...
      if (!plan.truncated) return plan.colony;
    }
    const error: Error & Record<string, any> = new Error(
      `PI catch-up limit exceeded for colony ${key}`,
    );
...
function getColonyAsync(planetID, ownerID, context: Record<string, any> = {}) {
  return ensureColonyCaughtUpAsync(planetID, ownerID, context);
}

async function listColoniesForCharacterAsync(ownerID) {
  const normalizedOwnerID = normalizeInteger(ownerID, 0);
  if (normalizedOwnerID <= 0) return [];
  const state = readState({ repair: true });
  const targets = Object.values<any>(state.coloniesByKey || {})
  ...
  for (const targetPlanetID of targets) {
    ...
    if (colony) colonies.push(colony);
  }
  return colonies;
}
...
async function listColoniesForPlanetAsync(planetID) {
  const normalizedPlanetID = normalizeInteger(planetID, 0);
  if (normalizedPlanetID <= 0) return [];
  const state = readState({ repair: true });
  const owners = Object.values<any>(state.coloniesByKey || {})
...
async function getColonyByPinAsync(ownerID, pinID) {
  const normalizedOwnerID = normalizeInteger(ownerID, 0);
  const normalizedPinID = normalizeInteger(pinID, 0);
  if (normalizedOwnerID <= 0 || normalizedPinID <= 0) return null;
  const state = readState({ repair: true });
  for (const rawColony of Object.values<any>(state.coloniesByKey || {})) {
    if (
      normalizeInteger(rawColony && rawColony.ownerID, 0) === normalizedOwnerID &&
      findPin(rawColony, normalizedPinID)
    ) {
      return ensureColonyCaughtUpAsync(
        rawColony.planetID,
        normalizedOwnerID,
      );
    }
...
function listColoniesForCharacter(ownerID) {
  ...
  const colonies: any[] = [];
  let changed = false;
  for (const [key, rawColony] of Object.entries<any>(state.coloniesByKey)) {
...
function restartExtractorsForCharacter(ownerID, options: Record<string, any> = {}) {
  const normalizedOwnerID = normalizeInteger(ownerID, 0);
  if (normalizedOwnerID <= 0) {
  ...
  for (const colony of colonies) {
    ...
    const colonySummary: Record<string, any> = {
      planetID,
...
function prepareSpaceportImportExport({
  ...
  if (normalizedPlanetID <= 0 || normalizedOwnerID <= 0 || normalizedSpaceportPinID <= 0) {
    return { success: false as const, errorMsg: "CannotImportEndpointNotFound" };
  }
  ...
  if (
    ...
    return { success: false as const, errorMsg: "PleaseSelectCommoditiesToImport" };
  }
  ...
  for (const [typeID, quantity] of Object.entries(normalizedImports)) {
    if (getAcceptableQuantity(workingPin, typeID, quantity) < quantity) {
      return { success: false as const, errorMsg: "NotEnoughCargoSpace" };
    }
...
function prepareExpeditedTransfer({
  ...
  if (normalizedPlanetID <= 0 || normalizedOwnerID <= 0) {
    return { success: false as const, errorMsg: "CannotManagePlanetWithoutCommandCenter" };
  }
  if (normalizedPath.length < 2) {
    return { success: false as const, errorMsg: "CreateRouteTooShort" };
  }
  if (Object.keys(normalizedCommodities).length < 1) {
    return { success: false as const, errorMsg: "CreateRouteWithoutCommodities" };
  }
  ...
  if (!sourcePin || !destinationPin) {
    return { success: false as const, errorMsg: "RouteFailedValidationPinDoesNotExist" };
  }
  if (!isStorageEntityType(getPinEntityType(sourcePin))) {
    return { success: false as const, errorMsg: "RouteFailedValidationExpeditedSourceNotStorage" };
  }
  ...
  for (const pinID of normalizedPath) {
    ...
    if (normalizeInteger(pin.ownerID, normalizedOwnerID) !== normalizedOwnerID) {
      return { success: false as const, errorMsg: "RouteFailedValidationPinNotYours" };
    }
  }

  for (const [typeID, quantity] of Object.entries(normalizedCommodities)) {
    if (getContentsQuantity(sourcePin, typeID) <= 0) {
      return { success: false as const, errorMsg: "RouteFailedValidationExpeditedSourceLacksCommodity" };
    }
    if (getContentsQuantity(sourcePin, typeID) < quantity) {
      return { success: false as const, errorMsg: "RouteFailedValidationExpeditedSourceLacksCommodityQty" };
    }
    if (getAcceptableQuantity(destinationPin, typeID, quantity) < 1) {
      return { success: false as const, errorMsg: "RouteFailedValidationExpeditedDestinationCannotAccept" };
    }
  }
  ...
  if (!(minBandwidth > 0)) {
    return { success: false as const, errorMsg: "RouteFailedValidationNoBandwidthAvailable" };
  }
  ...
  if (nextTransferTime > currentRunTime) {
    return { success: false as const, errorMsg: "RouteFailedValidationExpeditedSourceNotReady" };
  }
...
function previewTransferCommodities(options: Record<string, any> = {}) {
...
function transferCommodities(options: Record<string, any> = {}) {
  const result = prepareExpeditedTransfer(options);
  if (!result.success) {
    return result;
  }

  const movedCommodities: Record<string, any> = {};
...
function previewSpaceportImportExport(options: Record<string, any> = {}) {
...
function applySpaceportImportExport(options: Record<string, any> = {}) {
  const {
    planetID,
    ownerID,
  } = options;
...
function listLaunchesForCharacter(ownerID) {
  ...
  const state = readState({ repair: true });
  return Object.values<any>(state.launchesByID)
...
function cleanupExpiredLaunches(options: Record<string, any> = {}) {
  const state = readState({ repair: true });
  ...
  let scanned = 0;
  let deleted = 0;
  const deletedLaunchIDs: any[] = [];

  for (const [launchKey, rawLaunch] of Object.entries<any>(state.launchesByID || {})) {
    const launch: Record<string, any> = normalizeLaunchRecord(rawLaunch);
...
function summarizePin(pin: Record<string, any> = {}) {
  const entityType = getPinEntityType(pin);
  return {
...
function summarizeColony(colony: Record<string, any> = {}) {
  ...
  const contentsByTypeID: Record<string, any> = {};
...
function summarizeResourceRecord(record: Record<string, any> = {}) {
...
function getPlanetDiagnostics(options: Record<string, any> = {}) {
  const state = readState({ repair: true });
  const planetID = normalizeInteger(options.planetID, 0);
  const ownerID = normalizeInteger(options.ownerID, 0);
  const nowFileTime = currentFileTime();
  const colonies = Object.values<any>(state.coloniesByKey || {})
    .filter((colony) => (
  ...
  const launches = Object.values<any>(state.launchesByID || {})
    .map(normalizeLaunchRecord)
    .filter((launch) => (
...
function addCommodityToColonyPin({
  ...
  if (
    ...
    return { success: false as const, errorMsg: "INVALID_ARGUMENTS" };
  }
  ...
  const colony = state.coloniesByKey[key]
    ...
    : null;
  if (!colony) {
    return { success: false as const, errorMsg: "COLONY_NOT_FOUND" };
  }

  const pin = findPin(colony, normalizedPinID);
  if (!pin) {
    return { success: false as const, errorMsg: "PIN_NOT_FOUND" };
  }

  const added = addCommodityToPin(pin, normalizedTypeID, normalizedQuantity);
  if (added <= 0) {
    return { success: false as const, errorMsg: "PIN_CANNOT_ACCEPT_COMMODITY" };
  }
