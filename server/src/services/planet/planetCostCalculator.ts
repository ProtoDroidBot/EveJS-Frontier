...
function normalizeContents(contents) {
  ...
  for (const [typeID, quantity] of Object.entries<any>(unwrapped)) {
...
function buildPinTypeMap(colony: Record<string, any> = {}) {
...
function quoteNetworkConstructionCost(existingColony = null, serializedChanges: any[] = []) {
...
function getCommodityTaxMultiplier(typeID, options: Record<string, any> = {}) {
...
function calculateExportTax(pinTypeID, commodities, taxRate = DEFAULT_TAX_RATE, options: Record<string, any> = {}) {
  const contents = normalizeContents(commodities);
