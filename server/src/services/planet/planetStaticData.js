...
function getTypeDogmaByID() {
    ...
    const rows = payload && payload.typesByTypeID && typeof payload.typesByTypeID === "object"
        ...
        : {};
    typeDogmaCache = new Map(Object.entries(rows).map(([typeID, row]) => [toInt(typeID, 0), row]));
    return typeDogmaCache;
}
function getDogmaAttributeTypeByID() {
    ...
    const rows = payload && payload.attributeTypesByID &&
        ...
        : {};
    dogmaAttributeTypeCache = new Map(Object.entries(rows).map(([attributeID, row]) => [toInt(attributeID, 0), row]));
    return dogmaAttributeTypeCache;
}
function normalizeSchematicRow(row = {}) {
    return {
        ...
        pinTypeIDs: [...new Set((Array.isArray(row.pinTypeIDs) ? row.pinTypeIDs : row.pins || [])
...
function getPITypeInfo(typeID) {
    ...
    if (categoryID !== CATEGORY.PLANETARY_INDUSTRY &&
    ...
    return {
        ...
        planetRestrictionTypeID: toInt(getTypeAttribute(normalizedTypeID, ATTRIBUTE.PLANET_RESTRICTION, 0), 0) || null,
...
//# sourceMappingURL=planetStaticData.js.map