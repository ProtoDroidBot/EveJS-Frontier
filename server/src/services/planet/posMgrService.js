...
const { buildBoundObjectResponse, extractList, normalizeNumber, resolveBoundNodeId, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { ITEM_FLAGS, buildRemovedItemNotificationState, listContainerItems, removeInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
...
function refreshOrbitalScene(record, session = null) {
    const systemID = Number(record && record.solarSystemID ||
    ...
    if (!spawnResult || !spawnResult.success) {
        log.warn(`[PosMgr] Failed to refresh orbital ${itemID} in system ${systemID}: ${spawnResult ? spawnResult.errorMsg : "UNKNOWN"}`);
    }
}
function syncRemovedInventoryChanges(session, changes = []) {
    for (const change of Array.isArray(changes) ? changes : []) {
        ...
        syncInventoryItemForSession(session, removedState, change.previousData, { emitCfgLocation: true });
    }
}
function consumeOrbitalUpgradeHold(itemID, session = null) {
    const contents = listContainerItems(null, itemID, ITEM_FLAGS.SPECIALIZED_MATERIAL_BAY);
    let consumed = 0;
    for (const item of contents) {
        const removeResult = removeInventoryItem(item.itemID, { removeContents: true });
        if (!removeResult.success) {
            log.warn(`[PosMgr] Failed to consume upgrade material itemID=${item.itemID}: ${removeResult.errorMsg || "UNKNOWN"}`);
            continue;
        }
        consumed += 1;
        syncRemovedInventoryChanges(session, removeResult.data && removeResult.data.changes);
    }
    return consumed;
}
...
class PosMgrService extends BaseService {
    ...
    Handle_AnchorOrbital(args, session) {
        ...
        if (!result.success) {
            log.warn(`[PosMgr] AnchorOrbital failed itemID=${itemID}: ${result.errorMsg || "UNKNOWN_ERROR"}`);
            return null;
        }
    ...
    Handle_UnanchorOrbital(args, session) {
        ...
        if (!result.success) {
            log.warn(`[PosMgr] UnanchorOrbital failed itemID=${itemID}: ${result.errorMsg || "UNKNOWN_ERROR"}`);
            return null;
        }
        refreshOrbitalScene(result.data, session);
        return null;
    ...
    Handle_OnlineOrbital(args, session) {
        ...
        if (!result.success) {
            log.warn(`[PosMgr] OnlineOrbital failed itemID=${itemID}: ${result.errorMsg || "UNKNOWN_ERROR"}`);
            return null;
        }
...
//# sourceMappingURL=posMgrService.js.map