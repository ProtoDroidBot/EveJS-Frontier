...
function getCharacterColonies(characterRecord: Record<string, any> = {}) {
  const candidates = [
...
function mergeColonies(primaryColonies: any[] = [], extraColonies: any[] = []) {
...
function buildPlanetRowFields(entry: Record<string, any> = {}) {
...
function buildPlanetEntry(entry: Record<string, any> = {}) {
...
function buildPlanetRow(entry: Record<string, any> = {}) {
  const fields = buildPlanetRowFields(entry);
  if (!fields) {
    return null;
  }
...
function buildPlanetListForCharacter(characterRecord: Record<string, any> = {}) {
  return buildList(
    mergeColonies(
...
function buildPlanetRowsetForCharacter(characterRecord: Record<string, any> = {}) {
...
function buildKeyValFromObject(value: Record<string, any> = {}) {
  return buildKeyVal(
...
function buildPinRow(pin: Record<string, any> = {}) {
...
function buildLinkRow(link: Record<string, any> = {}) {
...
function buildRouteRow(route: Record<string, any> = {}) {
  return buildKeyValFromObject({
...
function buildSerializedColony(colony: Record<string, any> = {}) {
  return buildKeyVal([
...
function buildResourceDataResponse(planetMeta, info: Record<string, any> = {}) {
  const resourceTypeIDs = getResourceTypeIDsForPlanetType(planetMeta.typeID);
...
function buildLocalDistributionReport(planetMeta, surfacePoint: Record<string, any> = {}) {
  const resourceTypeIDs = getResourceTypeIDsForPlanetType(planetMeta.typeID);
...
function buildCommandPinSummary(pin: Record<string, any> = {}, ownerID = 0) {
  const pinID = toInt(pin.pinID ?? pin.id, 0);
  return buildKeyVal([
    ["pinID", pinID],
    ["id", pinID],
...
function buildExtractorSummary(pin: Record<string, any> = {}, ownerID = 0) {
  const pinID = toInt(pin.pinID ?? pin.id, 0);
  return buildKeyVal([
    ["pinID", pinID],
    ["id", pinID],
...
function buildLaunchRow(launch: Record<string, any> = {}) {
  const fields = {
...
function buildLaunchDetailsRowset(launches: any[] = []) {
  return buildDbRowset(
...
function sendPlanetNotification(session, methodName, args: any[] = []) {
...
function buildLaunchCommodityGrantEntries(contents: Record<string, any> = {}) {
...
function getPlanetLabel(planetMeta: Record<string, any> = {}) {
...
class PlanetMgrService extends BaseService {
  ...
  _resolvePlanetID(args, session, options: Record<string, any> = {}) {
  ...
  Handle_GetResourceData(args, session) {
    ...
    const entries = new Map<any, any>(response.args.entries || []);
