...
const { findItemById, getItemMetadata, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const DEFAULT_ORBITAL_TIMER_MS = Math.max(1_000, Number(process.env.EVEJS_PI_ORBITAL_TIMER_MS) || 60_000);
...
function normalizeOrbitalConditionState(rawConditionState = {}) {
...
function readState(options = {}) {
    ...
    if (!result.success) {
        log.warn(`[PlanetOrbitalState] Failed to read ${TABLE_NAME}: ${result.errorMsg || "READ_ERROR"}`);
    }
...
function flushStateToDisk() {
    ...
    if (!result.success) {
        log.warn(`[PlanetOrbitalState] Failed to flush ${TABLE_NAME}: ${result.errorMsg || "FLUSH_ERROR"}`);
    }
...
function writeState(state) {
    ...
    if (!result.success) {
        log.warn(`[PlanetOrbitalState] Failed to write ${TABLE_NAME}: ${result.errorMsg || "WRITE_ERROR"}`);
        return false;
    }
...
function getPlanetsBySystem() {
    ...
    for (const planets of planetCacheBySystem.values()) {
        planets.sort((left, right) => ((left.celestialIndex || left.planetID) - (right.celestialIndex || right.planetID)));
    }
    return planetCacheBySystem;
}
...
function buildDefaultCustomsOfficeName(planet = {}) {
    const planetName = String(planet.itemName ||
        planet.name ||
        (planet.planetID ? `Planet ${planet.planetID}` : "Planet")).trim();
...
function listDefaultCustomsOfficesForSystem(solarSystemID) {
    ...
    const occupiedPlanetIDs = new Set(listOrbitalsForSystem(numericSystemID)
...
function normalizeOrbitalRecord(rawRecord = {}, defaults = {}) {
    ...
    const corporationID = toInt(record.corporationID ?? defaults.corporationID, ownerID);
    const state = toInt(record.state ?? defaults.state, getDefaultStateForItem({
        typeID,
    ...
    return {
        ...
        conditionState: normalizeOrbitalConditionState(record.conditionState ?? defaults.conditionState),
        destroyedAtMs: toInt(record.destroyedAtMs ?? defaults.destroyedAtMs, 0) || null,
    };
}
function buildDefaultsFromItem(item = {}, options = {}) {
    const solarSystemID = toInt(options.solarSystemID, toInt(item.locationID, toInt(item.spaceState && item.spaceState.systemID, 0)));
    ...
    return {
        ...
        solarSystemID,
        planetID: toInt(options.planetID, findNearestPlanetID(solarSystemID, position)),
        state: options.state,
...
function completeOnliningOrbital(record) {
    ...
    if (!updateResult.success) {
        log.warn(`[PlanetOrbitalState] Failed to convert gantry ${record.itemID} to POCO: ${updateResult.errorMsg || "UNKNOWN"}`);
    }
    return normalizeOrbitalRecord({
        ...record,
        typeID: TYPE_CUSTOMS_OFFICE,
...
function beginOrbitalState(itemID, nextState, durationMs, session = null) {
    ...
    const record = ensureOrbitalForItem(item, {
        ...
        solarSystemID: toInt(session && session._space && session._space.systemID, toInt(item.locationID, 0)),
    });
    if (!record) {
        return {
            success: false,
...
function applyRuntimeOrbitalDamage(orbitalID, damageResult, nowMs = Date.now(), options = {}) {
...
//# sourceMappingURL=planetOrbitalState.js.map