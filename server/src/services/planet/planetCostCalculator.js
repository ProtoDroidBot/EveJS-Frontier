...
function buildNetworkEditHash({ planetID, ownerID, serializedChanges = [], } = {}) {
    const payload = {
...
function quoteNetworkConstructionCost(existingColony = null, serializedChanges = []) {
    ...
    let workingCommandCenterLevel = clamp(toInt(colony.level ?? colony.commandCenterLevel, 0), 0, 5);
    let total = 0;
    const pinCosts = [];
    const upgradeCosts = [];
    for (const command of commandStream) {
        ...
        if (command.id === COMMAND.UPGRADECOMMANDCENTER) {
            ...
            const amount = Math.max(0, planetStaticData.getCommandCenterUpgradeCost(workingCommandCenterLevel, desiredLevel));
            if (amount > 0) {
                total += amount;
                upgradeCosts.push({
...
function calculateImportTax(pinTypeID, commodities, taxRate = DEFAULT_TAX_RATE) {
    ...
    const importTaxRate = planetStaticData.getTypeAttribute(pinTypeID, planetStaticData.ATTRIBUTE.IMPORT_TAX, 0);
...
function calculateExportTax(pinTypeID, commodities, taxRate = DEFAULT_TAX_RATE, options = {}) {
    const contents = normalizeContents(commodities);
    const exportTaxRate = planetStaticData.getTypeAttribute(pinTypeID, planetStaticData.ATTRIBUTE.EXPORT_TAX, 0);
    const rate = Math.max(0, toNumber(taxRate, DEFAULT_TAX_RATE));
    let total = 0;
    for (const [typeID, quantity] of Object.entries(contents)) {
...
//# sourceMappingURL=planetCostCalculator.js.map