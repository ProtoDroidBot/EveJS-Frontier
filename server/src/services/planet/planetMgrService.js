...
const { ITEM_FLAGS, consumeInventoryItemQuantity, findItemById, getActiveShipItem, grantItemsToCharacterLocation, createSpaceItemForCharacter, removeInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
const { buildBoundObjectResponse, buildDbRowset, buildDict, buildKeyVal, buildList, currentFileTime, normalizeNumber, resolveBoundNodeId, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
function buildPlanetRowFields(entry = {}) {
    ...
    return {
        planetID,
        solarSystemID: toInt(entry.solarSystemID ?? entry.systemID, staticMeta.solarSystemID || 0),
...
function buildPlanetListForCharacter(characterRecord = {}) {
    return buildList(mergeColonies(getCharacterColonies(characterRecord), characterRecord.runtimeColonies)
        .map((entry) => buildPlanetEntry(entry))
        .filter(Boolean));
}
function buildPlanetRowsetForCharacter(characterRecord = {}) {
    return buildDbRowset(PLANETS_FOR_CHAR_COLUMNS, mergeColonies(getCharacterColonies(characterRecord), characterRecord.runtimeColonies)
        .map((entry) => buildPlanetRow(entry))
        .filter(Boolean), PLANETS_FOR_CHAR_ROWSET_NAME);
}
function marshalPlainValue(value) {
    ...
    if (value && typeof value === "object") {
        ...
        return buildDict(Object.entries(value).map(([entryKey, entryValue]) => [
            entryKey,
...
function buildKeyValFromObject(value = {}) {
    return buildKeyVal(Object.entries(value || {}).map(([entryKey, entryValue]) => [
        entryKey,
        marshalPlainValue(entryValue),
    ]));
}
...
function buildResourceInfoForPlanet(planetMeta) {
    ...
    const resourceRecord = planetRuntimeStore.getOrCreatePlanetResources(planetMeta, resourceTypeIDs);
    const qualities = resourceRecord.qualitiesByTypeID || {};
    return buildDict((resourceRecord.resourceTypeIDs || resourceTypeIDs).map((resourceTypeID) => [
...
function buildResourceDataResponse(planetMeta, info = {}) {
    ...
    const resourceData = planetRuntimeStore.getResourceDataForClient(planetMeta, resourceTypeID, resourceTypeIDs, info);
    return buildKeyVal([
...
function buildLocalDistributionReport(planetMeta, surfacePoint = {}) {
    ...
    const resources = resourceTypeIDs.map((resourceTypeID) => [
        resourceTypeID,
        planetRuntimeStore.evaluateResourceValueAt(planetMeta.planetID, resourceTypeID, surfacePoint.latitude, surfacePoint.longitude),
    ]);
    return buildKeyVal([
        ["planetID", toInt(planetMeta.planetID, 0)],
...
function buildLaunchDetailsRowset(launches = []) {
    return buildDbRowset(PI_LAUNCH_DETAILS_COLUMNS, (Array.isArray(launches) ? launches : []).map(buildLaunchRow), PI_LAUNCH_DETAILS_ROWSET_NAME);
}
function extractPlanetIDFromValue(value) {
    ...
    if (unwrapped && typeof unwrapped === "object") {
        return toInt(unwrapped.planetID ??
            unwrapped.itemID ??
            unwrapped.id ??
            unwrapped[0], 0);
    }
...
function ensurePlanetBoundObjectReuseMap(session) {
    ...
    if (!session._planetMgrBoundObjectByPlanetID ||
...
function rememberPlanetBoundObjectReuse(serviceName, planetID, boundObjectID, session) {
    ...
    reuseMap[String(planetID)] = {
        ...
        boundAtFileTime: (serviceState && serviceState.boundAtFileTime) || currentFileTime(),
    };
}
...
function getSessionCharacterID(session) {
    return toInt(session && (session.characterID ?? session.charid ?? session.charID ?? session.userid), 0);
}
...
function getSessionShipID(session, characterID = 0) {
    const sessionShipID = toInt(session && (session.shipID ??
        session.shipid ??
        session.shipId ??
...
function isCommandCenterSourceItem(item, pin, characterID, shipID) {
    ...
    return (toInt(item.ownerID, 0) === characterID &&
...
function syncInventoryChanges(session, changes = []) {
    for (const change of Array.isArray(changes) ? changes : []) {
        ...
        syncInventoryItemForSession(session, change.item, change.previousData || {}, { emitCfgLocation: true });
    }
}
...
function createPhysicalLaunchContainer(launch, session) {
    ...
    const expiresAtMs = simTimeMs + Number(planetRuntimeStore.PI_LAUNCH_ORBIT_DECAY_TICKS / 10000n);
    const createResult = createSpaceItemForCharacter(characterID, solarSystemID, PLANETARY_LAUNCH_CONTAINER_TYPE_ID, {
    ...
    if (!createResult.success || !createResult.data) {
        log.warn(`[PlanetMgr] Failed to create PI launch container launchID=${launchID}: ${createResult.errorMsg || "UNKNOWN"}`);
        return null;
    }
    const containerID = toInt(createResult.data.itemID, 0);
    const grantResult = grantItemsToCharacterLocation(characterID, containerID, ITEM_FLAGS.HANGAR, grantEntries);
    if (!grantResult.success) {
        log.warn(`[PlanetMgr] Failed to fill PI launch container launchID=${launchID} containerID=${containerID}: ${grantResult.errorMsg || "UNKNOWN"}`);
    ...
    if (!spawnResult || !spawnResult.success) {
        log.warn(`[PlanetMgr] PI launch container persisted but did not spawn launchID=${launchID} containerID=${containerID}: ${(spawnResult && spawnResult.errorMsg) || "UNKNOWN"}`);
    }
...
function debitPIWallet({ characterID, amount, entryTypeID, referenceID = 0, ownerID2 = 0, description = "Planetary Interaction charge", } = {}) {
...
function consumePlacedCommandCenterItems(colony, session) {
    ...
    for (const pin of commandPins) {
        ...
        if (!consumeResult.success) {
            log.warn(`[PlanetMgr] Failed to consume command center itemID=${pinID}: ${consumeResult.errorMsg || "UNKNOWN"}`);
            continue;
        }
...
class PlanetMgrService extends BaseService {
    ...
    async Handle_GetPlanetsForChar(args, session) {
        ...
        const runtimeColonies = await planetRuntimeStore.listColoniesForCharacterAsync(session && session.characterID);
    ...
    async Handle_GetPlanetInfo(args, session) {
        ...
        const colony = await planetRuntimeStore.getColonyAsync(planetID, session && session.characterID);
    ...
    Handle_GetPlanetResourceInfo(args, session) {
        ...
        return buildCachedMethodCallResult(buildResourceInfoForPlanet(getPlanetMeta(planetID)), {
    ...
    Handle_GetResourceData(args, session) {
        ...
        const dataBytes = data && data.type === "bytes" && Buffer.isBuffer(data.value)
            ? data.value.length
            : 0;
        log.debug(`[PlanetMgr] GetResourceData planetID=${planetID || "unknown"} resourceTypeID=${resourceTypeID || "unknown"} oldBand=${toInt(info.oldBand, 0)} newBand=${toInt(info.newBand, 0)} dataBytes=${dataBytes}`);
        return response;
    }
    Handle_GMGetCompleteResource(args, session) {
        ...
        log.debug(`[PlanetMgr] GMGetCompleteResource planetID=${planetID || "unknown"} resourceTypeID=${resourceTypeID || "unknown"} layer=${layer}`);
    ...
    Handle_GMGetLocalDistributionReport(args, session) {
        ...
        log.debug(`[PlanetMgr] GMGetLocalDistributionReport planetID=${planetID || "unknown"}`);
    ...
    async Handle_GMGetSynchedServerState(args, session) {
        ...
        log.debug(`[PlanetMgr] GMGetSynchedServerState planetID=${planetID || "unknown"} ownerID=${ownerID || "unknown"}`);
        return [duration, buildSerializedColony(colony)];
    }
    async Handle_GetFullNetworkForOwner(args, session) {
        const planetID = this._resolvePlanetID(args, session, { allowArgs: true });
        const ownerID = toInt(Array.isArray(args) && args.length > 1 ? normalizeNumber(args[1], 0) : 0, 0);
        const colony = await planetRuntimeStore.getColonyAsync(planetID, ownerID);
        if (!colony) {
    ...
    async Handle_GetCommandPinsForPlanet(args, session) {
        ...
        for (const colony of await planetRuntimeStore.listColoniesForPlanetAsync(planetID)) {
            ...
            const commandPin = (Array.isArray(colony.pins) ? colony.pins : [])
                .find((pin) => (getItemTypeGroupID(pin && pin.typeID) === planetStaticData.GROUP.COMMAND_PINS));
    ...
    async Handle_GetExtractorsForPlanet(args, session) {
        ...
        for (const colony of await planetRuntimeStore.listColoniesForPlanetAsync(planetID)) {
            ...
            for (const pin of Array.isArray(colony.pins) ? colony.pins : []) {
                if (getItemTypeGroupID(pin && pin.typeID) ===
    ...
    async Handle_UserUpdateNetwork(args, session) {
        ...
        log.debug(`[PlanetMgr] UserUpdateNetwork planetID=${planetID || "unknown"} commands=${commandCount}`);
        ...
        const constructionQuote = planetCostCalculator.quoteNetworkConstructionCost(existingColony, serializedChanges);
        try {
        ...
        try {
            ...
            refundPIWalletDebit(ownerID, constructionDebit, `Planetary construction refund on ${getPlanetLabel(planetMeta)}`);
            if (error && error.machoErrorResponse) {
                throw error;
            }
            throwWrappedUserError(error && error.message ? error.message : "PlanetUpdateFailed");
    ...
    async Handle_UserLaunchCommodities(args, session) {
        ...
        log.debug(`[PlanetMgr] UserLaunchCommodities planetID=${planetID || "unknown"} commandPinID=${commandPinID || "unknown"}`);
        const launchOptions = {
            planetID,
            ownerID,
            solarSystemID: planetMeta.solarSystemID,
        ...
        const exportTax = planetCostCalculator.calculateExportTax(preview.commandPin && preview.commandPin.typeID, preview.commoditiesToLaunch, planetCostCalculator.DEFAULT_TAX_RATE);
        ...
        if (!result.success) {
            refundPIWalletDebit(ownerID, taxDebit, `Planetary export tax refund on ${getPlanetLabel(planetMeta)}`);
    ...
    async Handle_UserTransferCommodities(args, session) {
        ...
        log.debug(`[PlanetMgr] UserTransferCommodities planetID=${planetID || "unknown"} path=${Array.isArray(pathArg) ? pathArg.length : 0}`);
        await planetRuntimeStore.ensureColonyCaughtUpAsync(planetID, ownerID, {
            solarSystemID: planetMeta.solarSystemID,
            planetTypeID: planetMeta.typeID,
            planetRadius: planetMeta.radius,
    ...
    Handle_UserAbandonPlanet(args, session) {
        ...
        const result = planetRuntimeStore.abandonColony(planetID, session && session.characterID);
    ...
    Handle_GetProgramResultInfo(args, session) {
        ...
        const headRadius = Number(unwrapMarshalValue(Array.isArray(args) && args.length > 3 ? args[3] : 0.01)) || 0.01;
        planetRuntimeStore.getOrCreatePlanetResources(planetMeta, getResourceTypeIDsForPlanetType(planetMeta.typeID));
        ...
        log.debug(`[PlanetMgr] GetProgramResultInfo planetID=${planetID || "unknown"} resourceTypeID=${resourceTypeID || "unknown"} qty=${result.qtyToDistribute}`);
    ...
    Handle_GetMyLaunchesDetails(args, session) {
        ...
        return buildLaunchDetailsRowset(planetRuntimeStore
    ...
    Handle_GMGetPlanetDiagnostics(args, session) {
        ...
        log.debug(`[PlanetMgr] GMGetPlanetDiagnostics planetID=${planetID || "all"} ownerID=${ownerID || "all"}`);
    ...
    Handle_GMCleanupExpiredLaunches(args, session) {
        const maxAgeDays = toInt(unwrapMarshalValue(Array.isArray(args) && args.length > 0 ? args[0] : 0), 0);
        const ownerID = getOptionalOwnerID(args, 1, session);
        const result = planetRuntimeStore.cleanupExpiredLaunches({
            maxAgeDays,
            ownerID,
        });
        log.debug(`[PlanetMgr] GMCleanupExpiredLaunches ownerID=${ownerID || "all"} deleted=${result.deleted}`);
        if (result.deleted > 0) {
            sendPlanetNotification(session, "OnPILaunchesChange", []);
        }
    ...
    async Handle_GMAddCommodity(args, session) {
        ...
        await planetRuntimeStore.ensureColonyCaughtUpAsync(planetID, getSessionCharacterID(session));
        ...
        log.debug(`[PlanetMgr] GMAddCommodity planetID=${planetID || "unknown"} pinID=${pinID || "unknown"} typeID=${typeID || "unknown"} quantity=${quantity}`);
        if (!result.success) {
            throwWrappedUserError("CustomNotify", {
...
//# sourceMappingURL=planetMgrService.js.map