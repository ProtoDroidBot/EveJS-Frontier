...
const { buildBoundObjectResponse, buildKeyVal, normalizeNumber, resolveBoundNodeId, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const planetOrbitalState = require("./planetOrbitalState");
const DEFAULT_CUSTOMS_OFFICE_TAX_RATE = planetOrbitalState.DEFAULT_TAX_RATES.corporation;
...
class PlanetOrbitalRegistryBrokerService extends BaseService {
    ...
    Handle_GetTaxRate(args) {
        ...
        log.debug(`[PlanetOrbitalRegistryBroker] GetTaxRate customsOfficeID=${customsOfficeID || "unknown"}`);
        return planetOrbitalState.getTaxRate(customsOfficeID);
    }
    Handle_GetSettingsInfo(args) {
        const orbitalID = extractOrbitalID(args);
        log.debug(`[PlanetOrbitalRegistryBroker] GetSettingsInfo orbitalID=${orbitalID || "unknown"}`);
    ...
    Handle_UpdateSettings(args) {
        ...
        const taxRates = unwrappedArgs[2] && typeof unwrappedArgs[2] === "object"
            ? unwrappedArgs[2]
            : {};
        log.info(`[PlanetOrbitalRegistryBroker] UpdateSettings orbitalID=${orbitalID || "unknown"}`);
        ...
        if (!result.success) {
            log.warn(`[PlanetOrbitalRegistryBroker] UpdateSettings failed orbitalID=${orbitalID}: ${result.errorMsg || "UNKNOWN_ERROR"}`);
        }
        return null;
    }
...
//# sourceMappingURL=planetOrbitalRegistryBrokerService.js.map