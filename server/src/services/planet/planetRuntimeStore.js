...
function readState(options = {}) {
    ...
    if (!result.success) {
        log.warn(`[PlanetRuntimeStore] Failed to read ${TABLE_NAME}: ${result.errorMsg || "READ_ERROR"}`);
    }
...
function flushStateToDisk() {
    ...
    if (!result.success) {
        log.warn(`[PlanetRuntimeStore] Failed to flush ${TABLE_NAME}: ${result.errorMsg || "FLUSH_ERROR"}`);
    }
...
function writeState(state) {
    ...
    if (!result.success) {
        log.warn(`[PlanetRuntimeStore] Failed to write ${TABLE_NAME}: ${result.errorMsg || "WRITE_ERROR"}`);
        return false;
    }
...
function hasAcceptedNetworkEdit({ planetID, ownerID, editHash, } = {}) {
    ...
    const bucket = getAcceptedNetworkEditBucket(state, colonyKey(normalizedPlanetID, normalizedOwnerID));
...
function normalizeResourceTypeIDs(resourceTypeIDs = []) {
    return [...new Set((Array.isArray(resourceTypeIDs) ? resourceTypeIDs : [])
...
function sphericalDistance(left, right) {
    ...
    const shortestDelta = deltaLongitude > Math.PI
        ...
        : deltaLongitude;
    const cosineDistance = (Math.cos(left.latitude) * Math.cos(right.latitude) +
...
function buildResourceLayer(planetMeta, resourceTypeID, quality, index) {
    ...
    for (let hotspotIndex = 0; hotspotIndex < hotspotCount; hotspotIndex += 1) {
        hotspots.push(buildResourceHotspot(planetMeta, resourceTypeID, quality, index, hotspotIndex));
    }
    return {
...
function normalizeResourceLayer(layer = {}) {
    ...
    normalizedLayer.depletionEvents = (Array.isArray(normalizedLayer.depletionEvents)
        ...
        headRadius: clamp(normalizeReal(event && event.headRadius, RADIUS_DRILL_AREA_MIN), RADIUS_DRILL_AREA_MIN, RADIUS_DRILL_AREA_MAX),
...
function buildResourceRecord(planetMeta, resourceTypeIDs = []) {
    ...
    normalizedTypeIDs.forEach((resourceTypeID, index) => {
        const quality = buildResourceQuality(planetMeta, resourceTypeID, index);
        qualitiesByTypeID[String(resourceTypeID)] = quality;
        layersByTypeID[String(resourceTypeID)] = buildResourceLayer(planetMeta, resourceTypeID, quality, index);
    });
    return {
...
function buildResourceLayerCoefficient(layer, coefficientIndex, options = {}) {
    ...
    for (let index = 0; index < hotspots.length; index += 1) {
        ...
        const phase = (band * Math.cos(hotspot.latitude) +
    ...
    for (const event of normalizedLayer.depletionEvents) {
        ...
        for (const head of event.heads) {
            const phase = (band * Math.cos(head[1]) +
    ...
    const coefficient = ((hotspotSignal * 13.5) +
        (noise * 2.25 * qualityScale) -
        (depletionSignal * 9.5)) * bandDecay;
    return Number.isFinite(coefficient) ? coefficient : 0;
}
function buildResourceLayerBuffer(layer, numBands, options = {}) {
    const normalizedBands = clamp(normalizeInteger(numBands, 0), 0, RESOURCE_SH_MAX_BANDS);
    ...
    for (let coefficientIndex = 0; coefficientIndex < coefficientCount; coefficientIndex += 1) {
        buffer.writeFloatLE(buildResourceLayerCoefficient(layer, coefficientIndex, options), coefficientIndex * RESOURCE_SH_COEFFICIENT_BYTES);
    }
    return buffer;
}
function getResourceDataForClient(planetMeta, resourceTypeID, resourceTypeIDs = [], info = {}) {
    ...
    const requestedBand = clamp(normalizeInteger(info.newBand, oldBand), 0, RESOURCE_SH_MAX_BANDS);
...
function normalizePin(pin = {}, ownerID = 0) {
    ...
    if (entityType === "process") {
        ...
        // ProcessPin initializes both flags to true in the client. Preserve explicit
        // simulation state, but do not turn a newly-created/restored processor into
        // a never-primed pin merely because an older record omitted the fields.
        normalizedPin.hasReceivedInputs = pin.hasReceivedInputs === undefined ||
            pin.hasReceivedInputs === null
            ? true
            : pin.hasReceivedInputs === true;
        normalizedPin.receivedInputsLastCycle = pin.receivedInputsLastCycle === undefined ||
            pin.receivedInputsLastCycle === null
            ? true
            : pin.receivedInputsLastCycle === true;
    }
    if (entityType === "ecu") {
...
function normalizeColony(rawColony, context = {}) {
    const planetID = normalizeInteger(context.planetID ?? (rawColony && rawColony.planetID), 0);
    const ownerID = normalizeInteger(context.ownerID ?? (rawColony && rawColony.ownerID), 0);
    ...
    colony.solarSystemID = normalizeInteger(context.solarSystemID ?? colony.solarSystemID, 0);
    colony.planetTypeID = normalizeInteger(context.planetTypeID ?? colony.planetTypeID ?? colony.typeID, 0);
...
function validatePinTypeForCreate(colony, typeID, context = {}) {
    ...
    const planetRestrictionTypeID = normalizeInteger(typeInfo && typeInfo.planetRestrictionTypeID, 0);
    const planetTypeID = normalizeInteger(context.planetTypeID, 0);
    if (planetRestrictionTypeID > 0 &&
        planetTypeID > 0 &&
...
function getPinProducts(pin) {
    ...
    if (entityType === "ecu") {
        ...
        const noiseFactor = Math.max(0, getEcuAttribute(pin && pin.typeID, planetStaticData.ATTRIBUTE.ECU_NOISE_FACTOR, 0));
    ...
    if (entityType === "process") {
        ...
        return Object.fromEntries((schematic && Array.isArray(schematic.outputs) ? schematic.outputs : [])
            .map((output) => [
...
function getPinConsumables(pin) {
    ...
    const schematic = getProcessSchematic(pin);
    return Object.fromEntries((schematic && Array.isArray(schematic.inputs) ? schematic.inputs : [])
        .map((input) => [
...
function assertExtractorHeadWithinArea(pin, latitude, longitude) {
    const areaOfInfluence = getEcuAttribute(pin && pin.typeID, planetStaticData.ATTRIBUTE.ECU_AREA_OF_INFLUENCE, 0);
    if (!(areaOfInfluence > 0)) {
        return;
    }
    const distance = sphericalDistance(normalizeSurfacePoint(pin.latitude, pin.longitude), normalizeSurfacePoint(latitude, longitude));
...
function getPinUsedVolume(pin) {
    ...
    return Object.entries(contents).reduce((total, [typeID, quantity]) => (total + (getTypeVolume(typeID) * quantity)), 0);
}
...
function getAcceptableQuantity(pin, typeID, desiredQuantity) {
    ...
    if (entityType === "process") {
        ...
        return Math.min(desired, Math.max(0, demand.quantity - getContentsQuantity(pin, typeID)));
    }
...
function addCommodityToPin(pin, typeID, quantity) {
    ...
    setContentsQuantity(pin, typeID, getContentsQuantity(pin, typeID) + acceptedQuantity);
    return acceptedQuantity;
}
function removeCommodityFromPin(pin, typeID, quantity) {
    const removedQuantity = Math.min(getContentsQuantity(pin, typeID), Math.max(0, normalizeInteger(quantity, 0)));
    if (removedQuantity <= 0) {
        return 0;
    }
    setContentsQuantity(pin, typeID, getContentsQuantity(pin, typeID) - removedQuantity);
...
function getEcuProgramOutput(pin, runTime) {
    ...
    const startTime = filetimeBigInt(pin && pin.installTime, filetimeBigInt(pin && pin.lastRunTime, currentTime));
    const elapsedTicks = Number(currentTime - startTime);
    const cycleNumber = Math.max(Math.floor((elapsedTicks + SECOND_TICKS) / cycleTime) - 1, 0);
    const barWidth = (cycleTime / SECOND_TICKS) / 900;
    const t = (cycleNumber + 0.5) * barWidth;
    const decayFactor = getEcuAttribute(pin && pin.typeID, planetStaticData.ATTRIBUTE.ECU_DECAY_FACTOR, 0);
    const noiseFactor = getEcuAttribute(pin && pin.typeID, planetStaticData.ATTRIBUTE.ECU_NOISE_FACTOR, 0);
    const decayValue = baseValue / (1 + (t * decayFactor));
    const phaseShift = baseValue ** 0.7;
    const noiseWave = Math.max(0, (Math.cos(phaseShift + (t / 12)) +
        Math.cos((phaseShift / 2) + (t / 5)) +
        Math.cos(t / 2)) / 3);
    return Math.max(0, Math.trunc(barWidth * decayValue * (1 + (noiseFactor * noiseWave))));
}
...
function getProcessInputBufferState(pin) {
    ...
    const filledRatio = schematic.inputs.reduce((total, input) => (total + (getContentsQuantity(pin, input.typeID) / input.quantity)), 0);
...
function getSortedOutputRoutes(colony, sourcePinID, commodities = {}) {
    ...
    for (const route of Array.isArray(colony.routes) ? colony.routes : []) {
        ...
        if (getRouteSourceID(route) !== normalizedSourcePinID ||
            quantity <= 0 ||
...
function getDestinationRoutes(colony, destinationPinID, commodityTypeID = 0) {
    ...
    return (Array.isArray(colony.routes) ? colony.routes : [])
        .filter((route) => (getRouteDestinationID(route) === normalizedDestinationPinID &&
...
function routeCommodityOutput(colony, sourcePin, commodities = {}, options = {}) {
    ...
    const routeGroups = getSortedOutputRoutes(colony, sourcePin.pinID, remainingCommodities);
    for (let groupIndex = 0; groupIndex < routeGroups.length; groupIndex += 1) {
        ...
        for (let routeIndex = 0; routeIndex < routes.length; routeIndex += 1) {
            ...
            if (groupIndex === 1) {
                maxQuantity = Math.min(maxQuantity, Math.ceil(maxQuantity / (routes.length - routeIndex)));
            }
            if (maxQuantity <= 0) {
                continue;
            }
            const { movedQuantity, destinationPin } = moveCommodityOverRoute(colony, route, sourcePin, typeID, maxQuantity, { consumeSource: sourceConsumes });
            if (movedQuantity <= 0) {
                continue;
            }
    ...
    for (const [destinationPinID, additions] of storageAdditionsByPinID.entries()) {
        ...
        routeCommodityOutput(colony, destinationPin, additions, { consumeSource: true, depth: normalizeInteger(options.depth, 0) + 1 });
    }
    return movedTotal;
}
function repairEcuSourceRoutes(colony) {
    const routes = Array.isArray(colony && colony.routes) ? colony.routes : [];
    if (routes.length < 1) {
        return false;
    }
    const routeQuantities = new Map();
    const routeIDsToRemove = new Set();
    const ecuPins = (Array.isArray(colony.pins) ? colony.pins : [])
        .filter((pin) => getPinEntityType(pin) === "ecu")
        .sort((left, right) => normalizeInteger(left.pinID, 0) - normalizeInteger(right.pinID, 0));
    for (const ecuPin of ecuPins) {
        const ecuPinID = normalizeInteger(ecuPin.pinID, 0);
        const sourceRoutes = routes
            .filter((route) => getRouteSourceID(route) === ecuPinID)
            .sort((left, right) => normalizeInteger(left.routeID, 0) - normalizeInteger(right.routeID, 0));
        if (sourceRoutes.length < 1) {
            continue;
        }
        const programTypeID = normalizeNullableInteger(ecuPin.programType);
        if (!programTypeID) {
            continue;
        }
        const productQuantity = normalizeInteger(getPinProducts(ecuPin)[String(programTypeID)], 0);
        const processRoutes = [];
        const storageRoutes = [];
        for (const route of sourceRoutes) {
            const routeID = normalizeInteger(route.routeID, 0);
            const destinationPin = findPin(colony, getRouteDestinationID(route));
            const destinationType = getPinEntityType(destinationPin);
            if (normalizeInteger(route.commodityTypeID, 0) !== programTypeID ||
                (!isConsumerEntityType(destinationType) && !isStorageEntityType(destinationType)) ||
                (isConsumerEntityType(destinationType) &&
                    normalizeInteger(getPinConsumables(destinationPin)[String(programTypeID)], 0) <= 0)) {
                routeIDsToRemove.add(routeID);
            }
            else if (isConsumerEntityType(destinationType)) {
                processRoutes.push(route);
            }
            else {
                storageRoutes.push(route);
            }
        }
        // The current client recreates processor routes first when a program is
        // installed, preserving their former quantities until output is exhausted.
        let quantityRemaining = productQuantity;
        for (const route of processRoutes) {
            const routeID = normalizeInteger(route.routeID, 0);
            const quantity = Math.min(normalizeInteger(route.commodityQuantity, 0), quantityRemaining);
            if (quantity <= 0) {
                routeIDsToRemove.add(routeID);
                continue;
            }
            routeQuantities.set(routeID, quantity);
            quantityRemaining -= quantity;
        }
        // Any remainder is shared proportionally among storage routes, mirroring
        // clientPlanet.InstallProgram's route-recreation policy.
        const formerStorageQuantity = storageRoutes.reduce((total, route) => (total + normalizeInteger(route.commodityQuantity, 0)), 0);
        const storageQuantityLimit = Math.min(quantityRemaining, formerStorageQuantity);
        let storageQuantityAssigned = 0;
        let roundingRemainder = 0;
        for (let index = 0; index < storageRoutes.length; index += 1) {
            const route = storageRoutes[index];
            const routeID = normalizeInteger(route.routeID, 0);
            const formerQuantity = normalizeInteger(route.commodityQuantity, 0);
            const isLastRoute = index === storageRoutes.length - 1;
            const quantityAvailable = storageQuantityLimit - storageQuantityAssigned;
            let quantity = 0;
            if (formerStorageQuantity > 0 && quantityAvailable > 0) {
                if (isLastRoute) {
                    quantity = quantityAvailable;
                }
                else {
                    const proportionalQuantity = (formerQuantity / formerStorageQuantity) * storageQuantityLimit;
                    quantity = Math.round(proportionalQuantity);
                    roundingRemainder += proportionalQuantity - quantity;
                    const roundingAdjustment = Math.round(roundingRemainder);
                    quantity += roundingAdjustment;
                    roundingRemainder -= roundingAdjustment;
                    quantity = Math.min(formerQuantity, quantityAvailable, Math.max(0, quantity));
                }
            }
            if (quantity <= 0) {
                routeIDsToRemove.add(routeID);
            ...
            routeQuantities.set(routeID, quantity);
            storageQuantityAssigned += quantity;
        }
    }
    let changed = false;
    colony.routes = routes.filter((route) => {
        const routeID = normalizeInteger(route.routeID, 0);
        if (routeIDsToRemove.has(routeID)) {
            changed = true;
            return false;
        }
        if (routeQuantities.has(routeID)) {
            const quantity = routeQuantities.get(routeID);
            if (normalizeInteger(route.commodityQuantity, 0) !== quantity) {
                route.commodityQuantity = quantity;
                changed = true;
            }
        }
        return true;
...
function routeCommodityInput(colony, processPin) {
    ...
    let movedTotal = 0;
    for (const input of schematic.inputs) {
        let missingQuantity = Math.max(0, input.quantity - getContentsQuantity(processPin, input.typeID));
        ...
        for (const route of getDestinationRoutes(colony, processPin.pinID, input.typeID)) {
            ...
            const { movedQuantity } = moveCommodityOverRoute(colony, route, sourcePin, input.typeID, missingQuantity, { consumeSource: true });
            movedTotal += movedQuantity;
...
function runColonySimulation(rawColony, targetFileTime = currentFileTimeString(), context = {}) {
    ...
    const repairedRoutes = repairEcuSourceRoutes(colony);
    if (targetTime <= simulationTime) {
        if (repairedRoutes) {
            colony.updatedAt = new Date().toISOString();
        }
        return { colony, changed: repairedRoutes };
    }
...
function getPinDistanceMeters(pinA, pinB, planetRadius = 0) {
    ...
    return sphericalDistance(normalizeSurfacePoint(pinA.latitude, pinA.longitude), normalizeSurfacePoint(pinB.latitude, pinB.longitude)) * radius;
}
...
function getPinCpuPower(pin) {
    ...
    return {
        cpuUsage: typeUsage.cpuUsage +
            Math.max(0, planetStaticData.getTypeAttribute(pin.typeID, planetStaticData.ATTRIBUTE.EXTRACTOR_HEAD_CPU, 0)) * headCount,
        powerUsage: typeUsage.powerUsage +
            Math.max(0, planetStaticData.getTypeAttribute(pin.typeID, planetStaticData.ATTRIBUTE.EXTRACTOR_HEAD_POWER, 0)) * headCount,
    };
}
...
function getCommodityTotalVolume(commodities = {}) {
    return Object.entries(normalizeContents(commodities)).reduce((total, [typeID, quantity]) => total + (getTypeVolume(typeID) * quantity), 0);
}
function getExpeditedTransferTimeTicks(linkBandwidth, commodities = {}) {
    ...
    return Math.ceil(Math.max(EXPEDITED_TRANSFER_MINIMUM_TICKS, (commodityVolume / bandwidth) * HOUR_TICKS));
}
...
function validateRouteSemantics(colony, route, context = {}) {
    ...
    if (isProducerEntityType(sourceType)) {
        ...
        const existingRouteQuantity = (Array.isArray(colony.routes) ? colony.routes : [])
            .filter((candidate) => (normalizeInteger(candidate.routeID, 0) !== normalizeInteger(route.routeID, 0) &&
            getRouteSourceID(candidate) === normalizeInteger(sourcePin.pinID, 0) &&
            normalizeInteger(candidate.commodityTypeID, 0) === typeID))
            .reduce((total, candidate) => (total + normalizeInteger(candidate.commodityQuantity, 0)), 0);
...
function validateColonyRoutesAndLinks(colony, context = {}) {
    ...
    for (const route of Array.isArray(colony.routes) ? colony.routes : []) {
        ...
        for (let index = 0; index < path.length - 1; index += 1) {
            ...
            bandwidthByLinkKey.set(key, (bandwidthByLinkKey.get(key) || 0) + bandwidthUsage);
        }
    }
...
function getProgramLengthFromHeadRadius(headRadius) {
    return ((clamp(headRadius, RADIUS_DRILL_AREA_MIN, RADIUS_DRILL_AREA_MAX) - RADIUS_DRILL_AREA_MIN) /
...
function getEcuMaxVolume(ecuTypeID) {
    return Math.max(0, getEcuAttribute(ecuTypeID, planetStaticData.ATTRIBUTE.ECU_MAX_VOLUME, 9.2));
}
function getCircleOverlapRatio(distance, radius) {
    ...
    const overlapArea = (2 * radiusSquared * Math.acos(0.5 * (distance / safeRadius)) -
...
function estimateProgramResult({ planetID = 0, resourceTypeID = 0, heads = [], headRadius = RADIUS_DRILL_AREA_MIN, ecuTypeID = DEFAULT_ECU_TYPE_ID, state = null, } = {}) {
    const normalizedPlanetID = normalizeInteger(planetID, 0);
    const normalizedResourceTypeID = normalizeInteger(resourceTypeID, 0);
    ...
    const clampedRadius = clamp(normalizeReal(headRadius, RADIUS_DRILL_AREA_MIN), RADIUS_DRILL_AREA_MIN, RADIUS_DRILL_AREA_MAX);
    ...
    const quality = normalizeInteger(resourceRecord.qualitiesByTypeID &&
        resourceRecord.qualitiesByTypeID[String(normalizedResourceTypeID)], 65);
    const overlapFactor = clamp(getEcuAttribute(ecuTypeID, planetStaticData.ATTRIBUTE.ECU_OVERLAP_FACTOR, 0.5), 0, 1);
    ...
    const summedHeadValue = normalizedHeads.reduce((total, head) => {
        const resourceValue = layer
            ...
            : getFallbackResourceValue(normalizedPlanetID, normalizedResourceTypeID, head, quality);
        const modifier = headModifiers.get(normalizeInteger(head[0], 0)) || 1;
        return total + (resourceValue * modifier);
    }, 0);
    const qtyToDistribute = Math.max(1, Math.trunc(maxVolume * summedHeadValue * RESOURCE_HARMONIC_VALUE_SCALE));
    return {
        qtyToDistribute,
        cycleTime,
        numCycles,
...
function recordResourceDepletionEvent({ state, planetID, resourceTypeID, heads = [], headRadius = RADIUS_DRILL_AREA_MIN, ecuTypeID = DEFAULT_ECU_TYPE_ID, result = {}, installTime = null, expiryTime = null, } = {}) {
    ...
    const normalizedHeadRadius = clamp(normalizeReal(headRadius, RADIUS_DRILL_AREA_MIN), RADIUS_DRILL_AREA_MIN, RADIUS_DRILL_AREA_MAX);
    const depletionRange = Math.max(1, getEcuAttribute(ecuTypeID, planetStaticData.ATTRIBUTE.EXTRACTOR_DEPLETION_RANGE, 5));
    const depletionRate = Math.max(0.1, getEcuAttribute(ecuTypeID, planetStaticData.ATTRIBUTE.EXTRACTOR_DEPLETION_RATE, 1));
    ...
    const depletionRadius = clamp(normalizedHeadRadius * depletionRange, normalizedHeadRadius, 0.45);
...
function assertCanInstallECUProgram(pin, ownerID, headRadius) {
    ...
    if (!Number.isFinite(normalizedHeadRadius) ||
...
function installECUProgram(pin, programTypeID, headRadius, context = {}) {
    ...
    const expiryTime = (BigInt(installTime) +
        BigInt(result.cycleTime) * BigInt(result.numCycles)).toString();
    ...
    pin.headRadius = clamp(normalizeReal(headRadius, RADIUS_DRILL_AREA_MIN), RADIUS_DRILL_AREA_MIN, RADIUS_DRILL_AREA_MAX);
    pin.state = STATE_ACTIVE;
...
function createLink(colony, endpoint1, endpoint2, level, ownerID) {
    ...
    colony.links.sort((left, right) => (left.endpoint1 === right.endpoint1
...
function applyUserUpdateNetwork({ planetID, ownerID, solarSystemID = 0, planetTypeID = 0, planetRadius = 0, serializedChanges = [], commands = null, editHash = null, constructionCost = 0, dryRun = false, } = {}) {
    const normalizedPlanetID = normalizeInteger(planetID, 0);
    const normalizedOwnerID = normalizeInteger(ownerID, 0);
    ...
    for (const command of commandStream) {
        ...
        switch (command.id) {
            case COMMAND.CREATEPIN: {
                ...
                colony.pins.push(buildPin(pinID, typeID, normalizedOwnerID, normalizeReal(args[2], 0), normalizeReal(args[3], 0)));
            ...
            case COMMAND.REMOVEPIN: {
                ...
                colony.pins = colony.pins.filter((pin) => normalizeInteger(pin.pinID, 0) !== pinID);
                colony.links = colony.links.filter((link) => (normalizeInteger(link.endpoint1, 0) !== pinID &&
                    normalizeInteger(link.endpoint2, 0) !== pinID));
                removeRoutesTouchingPin(colony, pinID);
                break;
            }
            case COMMAND.CREATELINK: {
                createLink(colony, resolveSubmittedID(args[0], idMap), resolveSubmittedID(args[1], idMap), args[2], normalizedOwnerID);
                break;
            }
            case COMMAND.REMOVELINK: {
                const endpoints = sortEndpoints(resolveSubmittedID(args[0], idMap), resolveSubmittedID(args[1], idMap));
            ...
            case COMMAND.SETLINKLEVEL: {
                setLinkLevel(colony, resolveSubmittedID(args[0], idMap), resolveSubmittedID(args[1], idMap), args[2], normalizedOwnerID);
                break;
            }
            ...
            case COMMAND.ADDEXTRACTORHEAD: {
                addExtractorHead(findPin(colony, resolveSubmittedID(args[0], idMap)), normalizedOwnerID, args[1], args[2], args[3]);
                break;
            }
            case COMMAND.KILLEXTRACTORHEAD: {
                removeExtractorHead(findPin(colony, resolveSubmittedID(args[0], idMap)), normalizedOwnerID, args[1]);
                break;
            }
            case COMMAND.MOVEEXTRACTORHEAD: {
                moveExtractorHead(findPin(colony, resolveSubmittedID(args[0], idMap)), normalizedOwnerID, args[1], args[2], args[3]);
                break;
            }
            case COMMAND.INSTALLPROGRAM: {
                ...
                // Retail clients remove and recreate ECU routes around InstallProgram.
                // Also enforce that invariant server-side so older emulator colonies and
                // non-retail callers cannot retain reservations above the new program's
                // maximum output.
                repairEcuSourceRoutes(colony);
                break;
            }
            default:
...
function prepareLaunchCommodities({ planetID, ownerID, solarSystemID = 0, planetTypeID = 0, commandPinID, commodities = {}, planetMeta = {}, } = {}) {
    const normalizedPlanetID = normalizeInteger(planetID, 0);
    const normalizedOwnerID = normalizeInteger(ownerID, 0);
...
function launchCommodities(options = {}) {
    const { planetID, ownerID, solarSystemID = 0, planetTypeID = 0, commandPinID, planetMeta = {}, } = options;
    const normalizedPlanetID = normalizeInteger(planetID, 0);
    const normalizedOwnerID = normalizeInteger(ownerID, 0);
    const normalizedCommandPinID = normalizeInteger(commandPinID, 0);
    const result = prepareLaunchCommodities(options);
    ...
    const { state, key, colony, commandPin, launchTime, commoditiesToLaunch, } = result;
    for (const [typeID, quantity] of Object.entries(commoditiesToLaunch)) {
...
function restartExtractorsForCharacter(ownerID, options = {}) {
    ...
    const colonies = listColoniesForCharacter(normalizedOwnerID).filter((colony) => planetFilter <= 0 || normalizeInteger(colony.planetID, 0) === planetFilter);
    const summaries = [];
    ...
    for (const colony of colonies) {
        ...
        if (commands.length > 0) {
            try {
                ...
                failedCount += 1;
                log.warn(`[PlanetRuntimeStore] Failed to restart extractors on planet ${planetID} ` +
...
function prepareSpaceportImportExport({ planetID, ownerID, spaceportPinID, importCommodities = {}, exportCommodities = {}, } = {}) {
    const normalizedPlanetID = normalizeInteger(planetID, 0);
    const normalizedOwnerID = normalizeInteger(ownerID, 0);
    ...
    if (Object.keys(normalizedImports).length < 1 &&
...
function prepareExpeditedTransfer({ planetID, ownerID, solarSystemID = 0, planetTypeID = 0, planetRadius = 0, path = [], commodities = {}, } = {}) {
    const normalizedPlanetID = normalizeInteger(planetID, 0);
    const normalizedOwnerID = normalizeInteger(ownerID, 0);
...
function transferCommodities(options = {}) {
    ...
    for (const [typeID, quantity] of Object.entries(result.commoditiesToTransfer)) {
        const transferQuantity = Math.min(normalizeInteger(quantity, 0), getContentsQuantity(result.sourcePin, typeID));
    ...
    const transferTicks = BigInt(getExpeditedTransferTimeTicks(result.minBandwidth, movedCommodities));
    const sourceRunTime = (filetimeBigInt(result.runTime, currentFileTime()) + transferTicks).toString();
...
function applySpaceportImportExport(options = {}) {
    const { planetID, ownerID, } = options;
    const normalizedPlanetID = normalizeInteger(planetID, 0);
    const normalizedOwnerID = normalizeInteger(ownerID, 0);
    const result = prepareSpaceportImportExport(options);
    if (!result.success) {
...
function listLaunchesForCharacter(ownerID) {
    ...
    return Object.values(state.launchesByID)
        ...
        .filter((launch) => (normalizeInteger(launch && launch.ownerID, 0) === normalizedOwnerID &&
        launch.deleted !== true));
}
...
function summarizeResourceRecord(record = {}) {
    ...
    for (const resourceTypeID of normalizeResourceTypeIDs(record.resourceTypeIDs)) {
        ...
        layers.push({
            resourceTypeID,
            quality: normalizeInteger(record.qualitiesByTypeID && record.qualitiesByTypeID[String(resourceTypeID)], 0),
...
function getPlanetDiagnostics(options = {}) {
    ...
    const colonies = Object.values(state.coloniesByKey || {})
        .filter((colony) => ((planetID <= 0 || normalizeInteger(colony && colony.planetID, 0) === planetID) &&
        (ownerID <= 0 || normalizeInteger(colony && colony.ownerID, 0) === ownerID)))
        .map(summarizeColony)
        .sort((left, right) => (left.planetID === right.planetID
        ? left.ownerID - right.ownerID
        : left.planetID - right.planetID));
    const launches = Object.values(state.launchesByID || {})
        .map(normalizeLaunchRecord)
        .filter((launch) => ((planetID <= 0 || normalizeInteger(launch.planetID, 0) === planetID) &&
        (ownerID <= 0 || normalizeInteger(launch.ownerID, 0) === ownerID)));
    const activeLaunches = launches.filter((launch) => (launch.deleted !== true && !isLaunchExpired(launch, nowFileTime)));
    const expiredLaunches = launches.filter((launch) => (launch.deleted !== true && isLaunchExpired(launch, nowFileTime)));
...
function addCommodityToColonyPin({ planetID, ownerID, pinID, typeID, quantity, } = {}) {
    const normalizedPlanetID = normalizeInteger(planetID, 0);
    const normalizedOwnerID = normalizeInteger(ownerID, 0);
    const normalizedPinID = normalizeInteger(pinID, 0);
    const normalizedTypeID = normalizeInteger(typeID, 0);
...
function getLaunch(launchID, ownerID = 0) {
    ...
    const normalizedLaunch = normalizeLaunchRecord(launch);
    const normalizedOwnerID = normalizeInteger(ownerID, 0);
    if (normalizedLaunch.deleted === true ||
        (normalizedOwnerID > 0 &&
...
function attachLaunchContainer({ launchID, ownerID = 0, itemID, } = {}) {
    const normalizedLaunchID = normalizeInteger(launchID, 0);
...
//# sourceMappingURL=planetRuntimeStore.js.map