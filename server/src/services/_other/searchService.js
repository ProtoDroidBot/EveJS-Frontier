...
const { buildDict, buildList, extractList, normalizeText, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { ensureAlliancesInitialized, ensureCorporationsInitialized, getAllianceRecord, getCorporationRecord, } = require(path.join(__dirname, "../corporation/corporationState"));
    ...
    switch (Number(exactMode) || 0) {
        ...
        default: {
            ...
            for (const entry of index.entries) {
                ...
                if (entry.rawName === rawSearch ||
                    entry.collapsedName === collapsedSearch) {
                    exactMatches.push(entry.id);
                    continue;
                }
                if (entry.rawName.startsWith(rawSearch) ||
...
function collectSearchableOwners(groupID, options = {}) {
    ...
    switch (Number(groupID) || 0) {
        ...
        case RESULT_TYPE.CHARACTER: {
            ...
            const characters = tableResult && tableResult.success && tableResult.data && typeof tableResult.data === "object"
                ? tableResult.data
                : {};
...
async function searchGroupAsync(groupID, search, exactMode, options = {}) {
    const normalizedGroupID = Number(groupID) || 0;
    if ([
        RESULT_TYPE.CONSTELLATION,
        RESULT_TYPE.SOLAR_SYSTEM,
        ...
        return await searchIndexPool.searchStaticGroup(normalizedGroupID, search, exactMode, MAX_RESULT_COUNT) || [];
    }
    const entries = collectSearchableOwners(normalizedGroupID, options)
        .map((entry) => ({ id: entry.id, name: entry.name }));
    return searchIndexPool.searchEntries(entries, search, exactMode, MAX_RESULT_COUNT);
}
