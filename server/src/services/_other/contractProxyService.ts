...
function recordAuditEvent(kind, args: any[] = [], session = null, kwargs = null) {
  auditEvents.push({
    kind,
    args: cloneArgs(args),
    kwargs: kwargs || null,
...
function buildLoginInfo(rows: Record<string, any> = {}) {
  return buildKeyVal([
...
function buildPythonObjectSet(items: any[] = []) {
...
function collectContractDetailTypeIDs(contract) {
  return [...new Set<any>(getContractDetailItems(contract)
...
function buildContractListBundle(contracts: any[] = []) {
