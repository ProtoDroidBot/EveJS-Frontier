...
const { buildDict, buildKeyVal, buildList, buildObjectEx1, buildPackedRow, buildPythonSet, buildRowset, extractList, normalizeNumber, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { findItemById, getItemMetadata, ITEM_FLAGS, listContainerItems, } = require(path.join(__dirname, "../inventory/itemStore"));
const contractRuntime = require(path.join(__dirname, "../contracts/contractRuntimeState"));
...
function buildLoginInfo(rows = {}) {
    return buildKeyVal([
        ...
        [
            "inProgress",
            buildRowset(["contractID", "startStationID", "endStationID", "expires"], rows.inProgress || []),
        ],
...
function buildContractItemList(contract) {
    return buildList((contract && Array.isArray(contract.items) ? contract.items : [])
...
function buildOutstandingContractsKeyVal(session) {
    ...
    const personalOutstanding = normalizePositiveInteger(pageInfo.numOutstandingContractsNonCorp, 0);
    const corpOutstanding = normalizePositiveInteger(pageInfo.numOutstandingContractsForCorp, 0);
    return buildKeyVal([
...
function buildContractDetailContractRow(contract) {
    ...
    const issuerWalletKey = contract.issuerWallet && contract.issuerWallet.kind === "corporation"
...
function buildContractDetailItemRow(contract, item) {
    return buildPackedRow(CONTRACT_DETAIL_ITEM_ROW_DESCRIPTOR_COLUMNS, {
        ...
        inCrate: normalizeBoolean(item && item.inCrate, false),
        parentID: normalizePositiveInteger(item && item.parentID, 0),
        productivityLevel: item && item.productivityLevel !== undefined ? normalizeNumber(item.productivityLevel, 0) : null,
        materialLevel: item && item.materialLevel !== undefined ? normalizeNumber(item.materialLevel, 0) : null,
        copy: item && item.copy !== undefined ? normalizeNumber(item.copy, 0) : null,
        licensedProductionRunsRemaining: item && item.licensedProductionRunsRemaining !== undefined
...
function buildContractDetailBundle(contract) {
    ...
    return buildKeyVal([
        ...
        [
            "items",
            buildList(getContractDetailItems(contract).map((item) => buildContractDetailItemRow(contract, item))),
        ],
        ["bids", buildList([])],
...
function listContractItemsInDockableLocation(args, session) {
    const locationID = normalizePositiveInteger(Array.isArray(args) && args.length > 0 ? args[0] : null, 0);
    const forCorp = normalizeBoolean(Array.isArray(args) && args.length > 1 ? args[1] : false, false);
    if (locationID <= 0) {
        return [];
    }
    if (forCorp) {
        ...
        return listContainerItems(corporationID, locationID, null)
            .filter((item) => CORP_CONTRACT_ITEM_FLAGS.has(normalizePositiveInteger(item && item.flagID, 0)))
...
function extractContainerItemIDs(rawValue) {
    return [...new Set(extractList(rawValue)
...
function listContractItemsInContainer(args, session) {
    const locationID = normalizePositiveInteger(Array.isArray(args) && args.length > 0 ? args[0] : null, 0);
    const containerID = normalizePositiveInteger(Array.isArray(args) && args.length > 1 ? args[1] : null, 0);
    const forCorp = normalizeBoolean(Array.isArray(args) && args.length > 2 ? args[2] : false, false);
    const flagID = normalizePositiveInteger(Array.isArray(args) && args.length > 3 ? args[3] : null, 0);
...
function countContractItemsInContainers(args, session) {
    const locationID = normalizePositiveInteger(Array.isArray(args) && args.length > 0 ? args[0] : null, 0);
    const containerIDs = extractContainerItemIDs(Array.isArray(args) && args.length > 1 ? args[1] : []);
    const forCorp = normalizeBoolean(Array.isArray(args) && args.length > 2 ? args[2] : false, false);
    const flagID = normalizePositiveInteger(Array.isArray(args) && args.length > 3 ? args[3] : null, 0);
    const ownerID = getContractOwnerID(session, forCorp);
...
class contractProxyService extends BaseService {
    ...
    Handle_GetMyCurrentContractList(args, session) {
        ...
        return buildContractListBundle(contractRuntime.filterMyCurrentContracts(session, isAccepted, forCorp));
    }
    ...
    Handle_GetContractListForStatus(args, session, kwargs) {
        ...
        const ownerID = normalizePositiveInteger(Array.isArray(args) && args.length > 1 ? unwrapMarshalValue(args[1]) : getCharacterID(session), getCharacterID(session));
    ...
    Handle_CreateContract(args, session, kwargs) {
        ...
        if (info.contractType !== contractRuntime.CONTRACT_TYPE.ITEM_EXCHANGE &&
    ...
    Handle_GetItemsInContainer(args, session) {
        ...
        return buildList(listContractItemsInContainer(args, session)
    ...
    Handle_GetItemsInDockableLocation(args, session) {
        ...
        return buildPythonObjectSet(listContractItemsInDockableLocation(args, session)
            .map(buildContractInventoryItemRow));
    }
...
//# sourceMappingURL=contractProxyService.js.map