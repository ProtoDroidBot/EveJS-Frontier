...
const { buildDict, buildList, normalizeNumber, normalizeText, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
function getSessionCharacterID(session) {
    return toPositiveInteger(session && (session.characterID || session.charid || session.clientID), 0);
}
function getSessionAccountID(session) {
    return toPositiveInteger(session && (session.userid || session.userID || session.accountID), 0);
}
...
function recordAuditEvent(kind, args = [], session = null, extra = {}) {
    ...
    log.debug(`[Petitioner] ${kind} char=${event.characterID || "?"} account=${event.accountID || "?"}`);
    return event;
}
...
//# sourceMappingURL=petitionerService.js.map