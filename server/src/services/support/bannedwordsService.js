...
const { buildDict, buildList, extractDictEntries, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
function buildRegexPayload(regexType) {
    ...
    return buildList(entries.map((entry) => buildDict([
        ["regex", entry.regex],
        ["hash", entry.hash],
    ])));
}
...
function recordWordCheck(kind, args = [], session = null) {
    ...
    log.debug(`[BannedWords] ${kind} check words=${words.length} char=${entry.characterID || "?"}`);
    return entry;
}
function assertWordsAllowed(kind, args = [], kwargs = null, session = null) {
    recordWordCheck(kind, args, session);
    const errorName = getKwargText(kwargs, "err_message", kind === "search" ? "SearchStringContainsBannedWord" : "LocationNameInvalidBannedWord");
...
module.exports._testing = {
    ...
    setRegexes(regexType, entries) {
        regexesByType.set(normalizeRegexType(regexType), (Array.isArray(entries) ? entries : [])
            .map(normalizeRegexEntry)
            .filter(Boolean));
    },
};
//# sourceMappingURL=bannedwordsService.js.map