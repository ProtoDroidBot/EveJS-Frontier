...
function pickRandomTitanSuperweaponProfile(options: Record<string, any> = {}) {
  const random =
...
function listTitanSuperweaponLoadouts(options: Record<string, any> = {}) {
...
function pickRandomTitanSuperweaponLoadout(options: Record<string, any> = {}) {
  const random =
    typeof options.random === "function"
      ? options.random
      : Math.random;
