...
const { resolveItemByTypeID, } = require("../inventory/itemTypeRegistry");
const { getTypeAttributeValue, } = require("../fitting/liveFittingState");
...
function resolveTitanSuperweaponProfileByHullTypeID(hullTypeID) {
    ...
    return listTitanSuperweaponProfiles().find((profile) => (Number(profile.hullTypeID) === normalizedHullTypeID)) || null;
}
function resolveTitanSuperweaponProfileByModuleTypeID(moduleTypeID) {
    const normalizedModuleTypeID = Number(moduleTypeID) || 0;
    return listTitanSuperweaponProfiles().find((profile) => (Number(profile.doomsdayTypeID) === normalizedModuleTypeID ||
        Number(profile.lanceTypeID) === normalizedModuleTypeID)) || null;
}
...
//# sourceMappingURL=superweaponCatalog.js.map