...
const { getActiveShipRecord, getCharacterRecord, } = require("../character/characterState");
const { ITEM_FLAGS, grantItemToCharacterLocation, grantItemsToCharacterStationHangar, listContainerItems, moveItemTypeFromCharacterLocation, } = require("../inventory/itemStore");
const { getCachedCharacterSkillMap, } = require("../skills/skillState");
const { listFittedItems, selectAutoFitFlagForType, validateFitForShip, } = require("../fitting/liveFittingState");
const spaceRuntime = require("../../space/runtime");
const { boardPreparedShipInSpace, } = require("../ship/spaceShipSwapRuntime");
const { pickRandomTitanSuperweaponLoadout, resolveTitanSuperweaponProfileByModuleTypeID, } = require("./superweaponCatalog");
const { registerSuperTitanShowController, } = require("../../space/modules/superweapons/superweaponRuntime");
...
function magnitude(vector) {
    return Math.sqrt((Number(vector && vector.x || 0) ** 2) +
...
function resolvePreferredStationID(session) {
    ...
    return Number(characterRecord.homeStationID ||
        ...
        DEFAULT_HOME_STATION_ID) || DEFAULT_HOME_STATION_ID;
}
function createShipItemInHangar(characterID, stationID, shipType) {
    const createResult = grantItemToCharacterLocation(characterID, stationID, ITEM_FLAGS.HANGAR, shipType, 1);
...
function fitModuleTypeToShip(characterID, stationID, shipItem, moduleType) {
    ...
    const nextFlagID = selectAutoFitFlagForType(shipItem, fittedItems, Number(moduleType && moduleType.typeID) || 0);
    if (!nextFlagID) {
        return {
            success: false,
    ...
    const validation = validateFitForShip(characterID, shipItem, probeItem, nextFlagID, fittedItems);
    if (!validation.success && validation.errorMsg !== "SKILL_REQUIRED") {
        return validation;
    }
    return moveItemTypeFromCharacterLocation(characterID, stationID, ITEM_FLAGS.HANGAR, shipItem.itemID, nextFlagID, moduleType.typeID, 1);
}
function computeMaxFuelUnitsForCargo(loadoutOrProfile) {
    const cargoCapacity = Math.max(0, Number(loadoutOrProfile && loadoutOrProfile.hullType && loadoutOrProfile.hullType.capacity) || 0);
    const fuelVolume = Math.max(0, Number(loadoutOrProfile && loadoutOrProfile.fuelType && loadoutOrProfile.fuelType.volume) || 0);
    const minimumFuelUnits = Math.max(1, Number(loadoutOrProfile && loadoutOrProfile.fuelPerActivation) ||
        Number(Math.max(Number(loadoutOrProfile && loadoutOrProfile.doomsdayFuelPerActivation) || 0, Number(loadoutOrProfile && loadoutOrProfile.lanceFuelPerActivation) || 0)) ||
        1);
    if (cargoCapacity <= 0 || fuelVolume <= 0) {
        return minimumFuelUnits;
    }
    return Math.max(minimumFuelUnits, Math.floor(cargoCapacity / fuelVolume));
}
function seedSuperTitanShip(characterID, stationID, loadout) {
    const createResult = createShipItemInHangar(characterID, stationID, loadout.hullType);
    ...
    const fitResult = fitModuleTypeToShip(characterID, stationID, shipItem, loadout.moduleType);
    ...
    const fuelMoveResult = moveItemTypeFromCharacterLocation(characterID, stationID, ITEM_FLAGS.HANGAR, shipItem.itemID, ITEM_FLAGS.CARGO_HOLD, loadout.fuelType.typeID, fuelUnits);
    ...
    const cargoFuelStack = listContainerItems(characterID, shipItem.itemID, ITEM_FLAGS.CARGO_HOLD).find((item) => Number(item && item.typeID) === Number(loadout.fuelType.typeID));
    return {
        success: true,
        data: {
            shipItem,
...
function buildSyntheticShowTitanShipSpec(ownerSession, loadout, formationSlot, midpoint, entityID) {
    ...
    return {
        ...
        skillMap: ownerSession && ownerSession.characterID
...
function buildFleetFormation(center, facingDirection, count) {
    ...
    return slots.map((slot) => ({
        position: addVectors(center, addVectors(scaleVector(basis.right, slot.lateral * SUPERTITAN_SHOW_LATERAL_SPACING_METERS), scaleVector(basis.up, slot.vertical * SUPERTITAN_SHOW_ROW_SPACING_METERS))),
        direction: basis.forward,
    }));
}
function resolveSceneTickIntervalMs(scene, fallback = 1000) {
    const tickIntervalMs = normalizePositiveInteger(scene && scene._tickIntervalMs, fallback);
...
function spawnShowFleetWave(scene, ownerSession, formation, fleetLabel, midpoint, config, startIndex = 0) {
    const spawned = [];
    const batchSize = Math.max(1, normalizePositiveInteger(config && config.spawnBatchSize, SUPERTITAN_SHOW_SPAWN_BATCH_SIZE));
    const endIndex = Math.min(formation.length, Math.max(0, startIndex) + batchSize);
    for (let index = Math.max(0, startIndex); index < endIndex; index += 1) {
        ...
        const spawnResult = spaceRuntime.spawnDynamicShip(scene.systemID, {
            ...buildSyntheticShowTitanShipSpec(ownerSession, loadout, slot, midpoint, entityID),
...
function scheduleShowFleetSpawns(scene, ownerSession, formationA, formationB, midpoint, config) {
    const scheduler = config.scheduleFn;
    const fleetA = [];
    const fleetB = [];
    const batchSize = Math.max(1, normalizePositiveInteger(config.spawnBatchSize, SUPERTITAN_SHOW_SPAWN_BATCH_SIZE));
    const waveIntervalMs = Math.max(1, normalizePositiveInteger(config.spawnWaveIntervalMs, resolveSceneTickIntervalMs(scene)));
    const waveCount = Math.max(Math.ceil(formationA.length / batchSize), Math.ceil(formationB.length / batchSize), 1);
    const runWave = (waveIndex) => {
        const startIndex = Math.max(0, waveIndex) * batchSize;
        fleetA.push(...spawnShowFleetWave(scene, ownerSession, formationA, "A", midpoint, config, startIndex));
        fleetB.push(...spawnShowFleetWave(scene, ownerSession, formationB, "B", midpoint, config, startIndex));
    };
    runWave(0);
    for (let waveIndex = 1; waveIndex < waveCount; waveIndex += 1) {
        schedule(() => runWave(waveIndex), waveIndex * waveIntervalMs, scheduler);
    }
    return {
        fleetA,
        fleetB,
...
function schedule(callback, delayMs, scheduleFn) {
    const run = typeof scheduleFn === "function"
        ? scheduleFn
        : setTimeout;
...
function buildSuperTitanShowConfig(scene, options = {}) {
    ...
    return {
        ...
        targetDelayMs: normalizeNonNegativeInteger(testing.targetDelayMs, SUPERTITAN_SHOW_TARGET_DELAY_MS),
...
function handleSuperTitanCommand(session, argumentText, options = {}) {
    ...
    const seededResult = seedSuperTitanShip(session.characterID, stationID, loadout);
    ...
    const spaceBoardResult = boardPreparedShipInSpace(session, seededResult.data.shipItem);
    ...
    const destroyedShipID = Number(spaceBoardResult &&
        spaceBoardResult.data &&
        spaceBoardResult.data.destroyResult &&
        spaceBoardResult.data.destroyResult.destroyedShipID) || 0;
...
function handleSuperTitanShowCommand(session, argumentText, options = {}) {
    ...
    const midpoint = addVectors(anchorEntity.position, scaleVector(basis.forward, SUPERTITAN_SHOW_MIDPOINT_DISTANCE_METERS));
    const fleetACenter = addVectors(midpoint, scaleVector(basis.forward, -SUPERTITAN_SHOW_FLEET_OFFSET_METERS));
    const fleetBCenter = addVectors(midpoint, scaleVector(basis.forward, SUPERTITAN_SHOW_FLEET_OFFSET_METERS));
    const formationA = buildFleetFormation(fleetACenter, basis.forward, perFleetCount);
    const formationB = buildFleetFormation(fleetBCenter, scaleVector(basis.forward, -1), perFleetCount);
    const stagedShow = scheduleShowFleetSpawns(scene, session, formationA, formationB, midpoint, config);
    if ((!Array.isArray(stagedShow.fleetA) || stagedShow.fleetA.length === 0) &&
    ...
    const firstVolleyDelayMs = stagedShow.spawnCompletionDelayMs + config.targetDelayMs;
    schedule(() => {
        const controllerFleetA = stagedShow.fleetA
            .map((entry) => {
            const moduleTypeID = Number(entry &&
                entry.loadout &&
                entry.loadout.moduleType &&
                entry.loadout.moduleType.typeID) || 0;
...
//# sourceMappingURL=superweaponCommands.js.map