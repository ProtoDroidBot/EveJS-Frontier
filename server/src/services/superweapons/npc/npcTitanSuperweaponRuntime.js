...
const { cloneVector, getCapitalControllerState, toFiniteNumber, toPositiveInt, } = require(path.join(__dirname, "../../../space/npc/capitals/capitalNpcState"));
...
function syncNpcTitanSuperweapon(scene, entity, controller, behaviorProfile, targetEntity, options = {}) {
    ...
    const moduleItem = (Array.isArray(entity.fittedItems) ? entity.fittedItems : []).find((entry) => (toPositiveInt(entry && entry.typeID, 0) === moduleTypeID)) || null;
    if (!moduleItem) {
        return false;
    }
    ...
    if (activeEffect) {
        if (capitalState) {
            capitalState.nextSuperweaponAttemptAtMs = nowMs + Math.max(runtimeConfig.superweaponSuccessfulRearmMs, toPositiveInt(activeEffect.durationMs, runtimeConfig.superweaponSuccessfulRearmMs));
        }
        return false;
    }
    const targetMode = String(behaviorProfile && behaviorProfile.capitalSuperweaponTargetMode || "item").trim().toLowerCase();
    const activationResult = scene.activateGenericModule(buildNpcPseudoSession(entity), moduleItem, null, targetMode === "point"
    ...
    if (capitalState) {
        const refreshedEffect = entity.activeModuleEffects instanceof Map
            ? entity.activeModuleEffects.get(toPositiveInt(moduleItem.itemID, 0)) || null
            : null;
        capitalState.nextSuperweaponAttemptAtMs = nowMs + Math.max(runtimeConfig.superweaponSuccessfulRearmMs, toPositiveInt(refreshedEffect && refreshedEffect.durationMs, runtimeConfig.superweaponSuccessfulRearmMs));
    }
    return true;
}
module.exports = {
...
//# sourceMappingURL=npcTitanSuperweaponRuntime.js.map