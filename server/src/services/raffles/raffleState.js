...
const RaffleSubscriptions = require(path.join(__dirname, "./raffleSubscriptions"));
const { normalizeInteger, } = require(path.join(__dirname, "./raffleHelpers"));
const { loadRafflesMap, writeRaffle, clearAllRaffles, } = require(path.join(__dirname, "./raffleRepository"));
const { getRaffleRuntimeState, updateRaffleRuntimeState, resetRaffleRuntimeState, } = require(path.join(__dirname, "./raffleRuntimeState"));
...
function shouldReserveItem(raffleState) {
    ...
    return (normalizeInteger(raffleState.itemId, 0) > 0 &&
