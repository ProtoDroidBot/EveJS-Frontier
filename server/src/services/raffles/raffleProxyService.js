...
const { extractKwarg, machoDictToObject, buildCreationPayload, } = require(path.join(__dirname, "./raffleHelpers"));
const { getRaffleRuntime, } = require(path.join(__dirname, "./raffleRuntimeSingleton"));
class RaffleProxyService extends BaseService {
    ...
    Handle_FilteredGrab(args, session, kwargs) {
        ...
        log.debug(`[RaffleProxy] FilteredGrab filters=${JSON.stringify(filters)} constraints=${JSON.stringify(constraints)}`);
    ...
    Handle_CreateRaffle(args, session) {
        ...
        log.debug(`[RaffleProxy] CreateRaffle ${JSON.stringify(buildCreationPayload(creationData))}`);
...
//# sourceMappingURL=raffleProxyService.js.map