...
const { normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { RAFFLE_STATUS } = require(path.join(__dirname, "./raffleConstants"));
const config = require(path.join(__dirname, "../../config"));
const { BROKERS_FEE, RAFFLE_TAX_PERCENTAGE, DEFAULT_PLEX_PRICE_OVERRIDE, TOKEN_PRICE_DIVISOR, TICKET_COUNT_POOL, MIN_TOTAL_PRICE, MAX_TOTAL_PRICE, PRIVATE_RESTRICTION_ID, BLUEPRINT_TYPE, ALLOWED_CATEGORY_IDS, } = require(path.join(__dirname, "./raffleConstants"));
...
function getSessionCharacterID(session) {
    return normalizeInteger(session && (session.characterID || session.charid), 0);
}
function getSessionClientID(session) {
    return normalizeInteger(session && (session.clientID || session.clientId), 0);
}
function getSessionStationID(session) {
    return normalizeInteger(session && (session.stationid || session.stationID), 0);
}
...
function pickSeedPolarisItem(items, preferredTypeIds = []) {
    ...
    for (const typeId of preferredTypeIds) {
        const matchingItem = items.find((item) => normalizeInteger(item && item.typeID, 0) === typeId);
        if (matchingItem) {
            return matchingItem;
        }
    }
...
function getPlexPriceOverride() {
    const configuredValue = normalizeInteger(config.hyperNetPlexPriceOverride, DEFAULT_PLEX_PRICE_OVERRIDE);
...
function tokensRequired(totalPrice) {
    ...
    return Math.max(1, Math.trunc((normalizeNumber(totalPrice, 0) / tokenPrice) * BROKERS_FEE));
}
...
function matchesBlueprintType(raffleState, blueprintType) {
    ...
    const isCopy = Boolean(raffleState &&
...
function matchesGrabFilters(raffleState, filters = {}, constraints = {}, viewerCharacterId = 0) {
    ...
    const isPrivate = normalizeInteger(raffleState && raffleState.restrictionId, 0) === PRIVATE_RESTRICTION_ID;
...
function isExpired(raffleState, nowFileTime) {
    ...
    const currentTime = typeof nowFileTime === "bigint"
        ? nowFileTime
        : BigInt(normalizeInteger(nowFileTime, 0));
    return (!isFinishedStatus(raffleState.raffleStatus) &&
...
//# sourceMappingURL=raffleHelpers.js.map