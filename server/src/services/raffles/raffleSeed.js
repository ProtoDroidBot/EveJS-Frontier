...
const { listCharacterItems, } = require(path.join(__dirname, "../inventory/itemStore"));
const itemTypeRegistry = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const { getUnpublishedShipTypes, } = require(path.join(__dirname, "../chat/shipTypeRegistry"));
const { FILETIME_TICKS_PER_MS, SEED_DURATION_MS, SEED_OWNER_ID, SEED_FALLBACK_STATION_ID, SEED_FALLBACK_SYSTEM_ID, SEED_PREFERRED_TYPE_IDS, SEED_FALLBACK_ITEM_TYPE_IDS, TICKET_COUNT_POOL, TOKEN_TYPE_ID, RAFFLE_STATUS, } = require(path.join(__dirname, "./raffleConstants"));
const { normalizeInteger, pickRandomEntries, shuffleArray, isValidTotalPrice, } = require(path.join(__dirname, "./raffleHelpers"));
const { resolveCharacterStationId, resolveStationSolarSystemId, ITEM_FLAGS, seedShipsForCharacter, seedItemsForCharacter, } = require(path.join(__dirname, "./raffleInventory"));
...
function buildSeedRaffleState(runtimeState, item, context = {}) {
    ...
    return {
        ...
        solarSystemId: normalizeInteger(context.solarSystemId, SEED_FALLBACK_SYSTEM_ID),
...
function getSeedLocationContext(ownerId = getSeedOwnerId()) {
    const locationId = resolveCharacterStationId(ownerId) || SEED_FALLBACK_STATION_ID;
    const solarSystemId = resolveStationSolarSystemId(locationId, SEED_FALLBACK_SYSTEM_ID) ||
...
function getAvailableSeedShips(runtimeState, locationId, ownerId = getSeedOwnerId()) {
    return getHangarItems(locationId, ownerId)
        .filter((item) => (normalizeInteger(item.categoryID, 0) === 6 &&
...
function buildFallbackShipPool() {
    ...
    const preferredShips = SEED_PREFERRED_TYPE_IDS
        ...
        .filter(Boolean);
    const otherShips = unpublishedShips.filter((shipType) => (!preferredShips.some((preferredShip) => preferredShip.typeID === shipType.typeID)));
...
function estimateSeedItemValue(item) {
    ...
    const stackSize = Math.max(1, rawStackSize > 0 ? rawStackSize : 1);
...
function buildSeedRaffles(runtimeState, options = {}) {
    ...
    const selectedShips = pickRandomEntries(prioritizeShips(ensureSeedShips(runtimeState, locationId, shipCount, ownerId)), shipCount);
    const selectedItems = pickRandomEntries(prioritizeItems(ensureSeedItems(runtimeState, locationId, itemCount, ownerId)), itemCount);
...
//# sourceMappingURL=raffleSeed.js.map