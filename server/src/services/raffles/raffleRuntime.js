...
const { buildList, currentFileTime, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { throwWrappedRaffleCreateError, throwWrappedRaffleError, } = require(path.join(__dirname, "../../common/machoErrors"));
const { adjustCharacterBalance, } = require(path.join(__dirname, "../account/walletState"));
const { findItemById, listCharacterItems, } = require(path.join(__dirname, "../inventory/itemStore"));
const itemTypeRegistry = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const log = require(path.join(__dirname, "../../utils/logger"));
const rotatingLog = require(path.join(__dirname, "../../utils/rotatingLog"));
const { FILETIME_TICKS_PER_MS, SEED_DURATION_MS, HISTORY_PAGE_SIZE, DEFAULT_GRAB_SIZE, DEFAULT_STARTUP_SEED_COUNT, MAX_QA_SEED_COUNT, RAFFLE_STATUS, TOKEN_TYPE_ID, } = require(path.join(__dirname, "./raffleConstants"));
const { normalizeInteger, getSessionCharacterID, dedupeTicketNumbers, shuffleArray, calculateTotalPrice, tokensRequired, matchesGrabFilters, isExpired, pickWinningTicket, buildCreationPayload, getSessionStationID, isFinishedStatus, } = require(path.join(__dirname, "./raffleHelpers"));
const { buildTicketValue, buildRaffleValue, buildCharacterStatisticsValue, buildTypeStatisticsValue, } = require(path.join(__dirname, "./raffleMarshal"));
const { notifyTicketsUpdated, notifyRaffleUpdated, notifyRaffleFinished, notifyRaffleCreated, } = require(path.join(__dirname, "./raffleNotifications"));
const { ensureCharacterTokenStack, consumeTokenStack, moveItemToRaffleEscrow, restoreEscrowedItem, deliverRaffleItemToCharacterStation, notifyInventoryChangesToCharacter, getCharacterSessions, getLargestTokenStack, ITEM_FLAGS, } = require(path.join(__dirname, "./raffleInventory"));
const RaffleState = require(path.join(__dirname, "./raffleState"));
const { buildRaffleSettlementState, } = require(path.join(__dirname, "./raffleSeed"));
const RaffleSeedManager = require(path.join(__dirname, "./raffleSeedManager"));
const { ensureSettlementState, repairPersistedRaffleState, restoreExpiredItem, refundExpiredTickets, creditSellerIfNeeded, } = require(path.join(__dirname, "./raffleSettlement"));
const { validateCreateInput, } = require(path.join(__dirname, "./raffleValidation"));
const RAFFLE_CREATE_DEBUG_PATH = path.join(__dirname, "../../../logs/raffle-create-debug.log");
...
function resolveCreateItem(ownerId, creationData, sessionStationId) {
    ...
    const locationId = normalizeInteger(creationData.location_id, sessionStationId);
    const directItemMatches = (directItem &&
...
function resolveCreateToken(ownerId, creationData, sessionStationId) {
    ...
    const locationId = normalizeInteger(creationData.token_location_id, sessionStationId);
    const directTokenMatches = (directToken &&
    ...
    const fallbackToken = getLargestTokenStack(ownerId, locationId > 0 ? locationId : null);
    if (fallbackToken) {
        return {
            token: fallbackToken,
...
class RaffleRuntime {
    ...
    grab(filters = {}, constraints = {}, size = null, session = null) {
        ...
        this._ensureCharacterReady(session);
        this._sweepExpiredRaffles();
        const maxSize = size === null || size === undefined
            ? DEFAULT_GRAB_SIZE
        ...
        const raffles = this._state.getRaffles()
            ...
            .filter((raffleState) => (matchesGrabFilters(raffleState, filters, constraints, viewerCharacterId)))
            .slice(0, maxSize)
    ...
    getHistory(session, runningId = null) {
        ...
        const page = this._state.getRaffles()
            .filter((raffleState) => (raffleState.ownerId === ownerId ||
            this._getOwnedTickets(ownerId, raffleState).length > 0))
            .filter((raffleState) => (normalizedRunningId <= 0 || raffleState.runningId < normalizedRunningId))
    ...
    getCharacterStatistics(session) {
        ...
        for (const raffleState of this._state.getRaffles()) {
            ...
            const isWinner = normalizeInteger(raffleState.winningTicket && raffleState.winningTicket.ownerId, 0) ===
                ownerId;
    ...
    getTypeStatistics(typeId) {
        ...
        const matchingRaffles = this._state.getRaffles().filter((raffleState) => raffleState.typeId === normalizedTypeId);
    ...
    createRaffle(session, rawCreationData) {
        ...
        const totalPrice = calculateTotalPrice(creationData.ticket_price, creationData.ticket_count);
        let validated = null;
        try {
        ...
        if (!tokenResult.success) {
            appendCreateDebug({
                ...
                error: tokenResult.errorMsg === "INSUFFICIENT_TOKENS"
                    ? "TokenAmountError"
                    : "TokenPaymentError",
                ownerId,
                sessionStationId,
            ...
            throwWrappedRaffleCreateError(tokenResult.errorMsg === "INSUFFICIENT_TOKENS"
                ? "TokenAmountError"
                : "TokenPaymentError");
        }
        const ids = this._state.allocateIds();
        const creationTime = currentFileTime();
        const expirationTime = creationTime + BigInt(SEED_DURATION_MS) * FILETIME_TICKS_PER_MS;
        ...
        const raffleState = {
            ...
            metaGroupId: normalizeInteger(typeMetadata.metaGroupID || typeMetadata.metaGroupId, 0),
        ...
        this._scheduleExpiration(raffleState);
        notifyInventoryChangesToCharacter(ownerId, [escrowResult.data.change, tokenResult.data.change], { includeSession: session });
        const creatorSessions = getCharacterSessions(ownerId, {
            includeSession: session,
        });
        setTimeout(() => {
            notifyRaffleCreated(creatorSessions, raffleState.raffleId, buildRaffleValue(raffleState));
        }, 0);
    ...
    buyRandomTickets(session, raffleId, ticketCount) {
        ...
        const availableTicketNumbers = shuffleArray(this._getAvailableTicketNumbers(raffleState));
        ...
        return this._buyTickets(session, raffleState, availableTicketNumbers.slice(0, requestedCount));
    }
    awardItem(session, raffleId) {
        ...
        const raffleState = this._requireRaffle(raffleId);
        const ownerId = getSessionCharacterID(session);
        const winnerId = normalizeInteger(raffleState.winningTicket && raffleState.winningTicket.ownerId, 0);
        if (raffleState.raffleStatus !== RAFFLE_STATUS.FINISHED_UNDELIVERED ||
            ownerId <= 0 ||
            winnerId !== ownerId) {
            throwWrappedRaffleError("raffles.FailureToDeliverItemError");
        }
        const deliveryResult = deliverRaffleItemToCharacterStation(raffleState.itemId, winnerId, raffleState.locationId, ITEM_FLAGS.HANGAR);
        ...
        const previousOwnerId = normalizeInteger(deliveredChange && deliveredChange.previousData && deliveredChange.previousData.ownerID, 0);
        if (deliveredChange) {
        ...
        const recipientSessions = this._state.subscriptions.getInterestedSessions(raffleState, { includeSession: session });
        notifyRaffleUpdated(recipientSessions, raffleState.raffleId, raffleData);
        return null;
    }
    qaSeedRaffles(count = DEFAULT_STARTUP_SEED_COUNT) {
        this.initialize();
        const normalizedCount = Math.max(0, Math.min(MAX_QA_SEED_COUNT, normalizeInteger(count, 0)));
    ...
    _scheduleExpiration(raffleState) {
        ...
        const remainingMs = Math.max(0, Number((raffleState.expirationTime - currentFileTime()) / FILETIME_TICKS_PER_MS));
    ...
    _expireRaffle(raffleId) {
        ...
        if (!alreadyFinished) {
            const raffleData = buildRaffleValue(raffleState);
            const recipientSessions = this._state.subscriptions.getInterestedSessions(raffleState);
            notifyRaffleUpdated(recipientSessions, raffleState.raffleId, raffleData);
    ...
    _buyTickets(session, raffleState, requestedTicketNumbers = []) {
        ...
        const requestedNumbers = dedupeTicketNumbers(requestedTicketNumbers).filter((ticketNumber) => ticketNumber < raffleState.ticketCount);
        ...
        const raffleData = buildRaffleValue(raffleState);
        const recipientSessions = this._state.subscriptions.getInterestedSessions(raffleState, { includeSession: session });
        notifyTicketsUpdated(recipientSessions, raffleState.raffleId, raffleState.soldTickets.length);
        notifyRaffleUpdated(recipientSessions, raffleState.raffleId, raffleData);
        if (justFinished) {
            notifyRaffleFinished(recipientSessions, raffleState.raffleId, raffleState.winningTicket
    ...
    _debitTicketPurchase(ownerId, raffleState, ticketCount) {
        const amount = raffleState.ticketPrice * Math.max(0, normalizeInteger(ticketCount, 0));
        if (amount <= 0) {
            return;
        }
        ...
        if (!walletResult.success) {
            throwWrappedRaffleError(walletResult.errorMsg === "INSUFFICIENT_FUNDS"
                ? "raffles.NotEnoughISKError"
                : "raffles.PurchaseError");
        }
    }
    _getOwnedTickets(ownerId, raffleState) {
        return raffleState.soldTickets.filter((ticket) => (normalizeInteger(ticket && ticket.ownerId, 0) === normalizeInteger(ownerId, 0)));
    }
    _getAvailableTicketNumbers(raffleState) {
        ...
        for (let ticketNumber = 0; ticketNumber < raffleState.ticketCount; ticketNumber += 1) {
            const alreadySold = raffleState.soldTickets.some((ticket) => (normalizeInteger(ticket && ticket.number, -1) === ticketNumber));
            if (!alreadySold) {
...
//# sourceMappingURL=raffleRuntime.js.map