...
const { adjustCharacterBalance, getCharacterWalletJournal, } = require(path.join(__dirname, "../account/walletState"));
const { findItemById, } = require(path.join(__dirname, "../inventory/itemStore"));
const log = require(path.join(__dirname, "../../utils/logger"));
const { normalizeInteger, calculateTotalPrice, calculateSellerPayout, groupTicketsByOwner, } = require(path.join(__dirname, "./raffleHelpers"));
const { restoreEscrowedItem, } = require(path.join(__dirname, "./raffleInventory"));
const { RAFFLE_STATUS, } = require(path.join(__dirname, "./raffleConstants"));
...
function hasWalletJournalEntry(characterId, raffleId, description) {
    return getCharacterWalletJournal(characterId).some((entry) => (normalizeInteger(entry && entry.referenceID, 0) === normalizeInteger(raffleId, 0) &&
...
function repairPersistedRaffleState(raffleState) {
    ...
    if (raffleState.winningTicket &&
        ...
        hasWalletJournalEntry(raffleState.ownerId, raffleState.raffleId, getPayoutDescription(raffleState.raffleId))) {
        settlement.sellerPayoutApplied = true;
        raffleState.pendingIsk = 0;
        dirty = true;
    }
    if (normalizeInteger(raffleState.raffleStatus, 0) === RAFFLE_STATUS.FINISHED_DELIVERED &&
...
function repairRefundJournalState(raffleState) {
    ...
    for (const [ownerId, tickets] of groupTicketsByOwner(raffleState.soldTickets)) {
        ...
        if (hasWalletJournalEntry(ownerId, raffleState.raffleId, getRefundDescription(raffleState.raffleId))) {
...
function restoreExpiredItem(raffleState) {
    ...
    const restoreResult = restoreEscrowedItem(raffleState.inventory.escrowItemId, raffleState.inventory.originalLocationId, raffleState.inventory.originalFlagId);
    if (!restoreResult.success) {
        log.warn(`[RaffleRuntime] Failed to restore escrowed item raffle=${raffleState.raffleId} item=${raffleState.inventory.escrowItemId}`);
        return {
            dirty: false,
            inventoryChanges: [],
        };
...
function refundExpiredTickets(raffleState) {
    ...
    for (const [ownerId, tickets] of groupTicketsByOwner(raffleState.soldTickets)) {
        ...
        if (!walletResult.success) {
            log.warn(`[RaffleRuntime] Failed to refund owner=${ownerId} raffle=${raffleState.raffleId} amount=${refundAmount}`);
            continue;
        }
        settlement.refundedOwners[String(ownerId)] = refundAmount;
        dirty = true;
...
function creditSellerIfNeeded(raffleState) {
    ...
    const sellerPayout = calculateSellerPayout(calculateTotalPrice(raffleState.ticketPrice, raffleState.ticketCount));
    ...
    if (!walletResult.success) {
        log.warn(`[RaffleRuntime] Failed to credit seller=${raffleState.ownerId} raffle=${raffleState.raffleId} amount=${normalizedAmount}`);
        return false;
    }
    settlement.sellerPayoutApplied = true;
    raffleState.pendingIsk = 0;
...
//# sourceMappingURL=raffleSettlement.js.map