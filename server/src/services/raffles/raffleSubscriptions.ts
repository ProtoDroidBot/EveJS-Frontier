...
class RaffleSubscriptions {
  declare _raffleSubscribers: any;
  declare _ticketSubscribers: any;

  constructor() {
    this._ticketSubscribers = new Set<any>();
  ...
  subscribeToRaffle(session, raffleId) {
    ...
    if (!this._raffleSubscribers.has(normalizedRaffleId)) {
      this._raffleSubscribers.set(normalizedRaffleId, new Set<any>());
    }
  ...
  getInterestedSessions(raffleState, options: Record<string, any> = {}) {
