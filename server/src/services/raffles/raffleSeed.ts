...
function buildRaffleSettlementState(overrides: Record<string, any> = {}) {
  return {
...
function buildSeedRaffleState(runtimeState, item, context: Record<string, any> = {}) {
...
function prioritizeShips(ships: any[] = []) {
  const preferredShips: any[] = [];
  const otherShips: any[] = [];
...
function prioritizeItems(items: any[] = []) {
  const withBasePrice: any[] = [];
  const withoutBasePrice: any[] = [];
...
function buildSeedRaffles(runtimeState, options: Record<string, any> = {}) {
