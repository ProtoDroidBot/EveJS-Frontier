...
const { ITEM_FLAGS, findItemById, updateInventoryItem, grantItemToCharacterLocation, grantItemsToCharacterLocation, listContainerItems, buildRemovedItemNotificationState, } = require(path.join(__dirname, "../inventory/itemStore"));
const { TOKEN_TYPE_ID, SEEDED_TOKEN_STACK_SIZE, RAFFLE_ESCROW_FLAG, SEED_FALLBACK_STATION_ID, } = require(path.join(__dirname, "./raffleConstants"));
const { normalizeInteger, } = require(path.join(__dirname, "./raffleHelpers"));
...
function getCharacterSessions(characterId, options = {}) {
    ...
    for (const session of sessionRegistry.getSessions()) {
        if (normalizeInteger(session.characterID || session.charid, 0) ===
...
function markWalletTransient(charId) {
    ...
    for (const pathKey of ["balance", "balanceChange", "walletJournal"]) {
        repo.setTransientPath("characters", `/${String(normalizedCharacterId)}/${pathKey}`, true);
    }
}
...
function resolveCharacterStationId(sessionOrCharId) {
    if (sessionOrCharId && typeof sessionOrCharId === "object") {
        const stationId = normalizeInteger(sessionOrCharId.stationid || sessionOrCharId.stationID, 0);
        if (stationId > 0) {
            return stationId;
        }
        return resolveCharacterStationId(sessionOrCharId.characterID || sessionOrCharId.charid);
    }
    const characterRecord = getCharacterRecord(normalizeInteger(sessionOrCharId, 0));
    return normalizeInteger(characterRecord &&
        (characterRecord.stationID || characterRecord.homeStationID), SEED_FALLBACK_STATION_ID);
}
function getLargestTokenStack(charId, stationId = null) {
    const normalizedCharacterId = normalizeInteger(charId, 0);
    const normalizedStationId = normalizeInteger(stationId, resolveCharacterStationId(normalizedCharacterId));
    if (normalizedCharacterId <= 0 || normalizedStationId <= 0) {
        return null;
    }
    return listContainerItems(normalizedCharacterId, normalizedStationId, ITEM_FLAGS.HANGAR)
        .filter((item) => (normalizeInteger(item && item.typeID, 0) === TOKEN_TYPE_ID &&
        normalizeInteger(item && item.stacksize, 0) > 0))
        .sort((left, right) => (normalizeInteger(right && right.stacksize, 0) -
        normalizeInteger(left && left.stacksize, 0)))[0] || null;
}
function ensureCharacterTokenStack(session, options = {}) {
    ...
    const minimumTokens = Math.max(1, normalizeInteger(options.minimumTokens, SEEDED_TOKEN_STACK_SIZE));
    ...
    const grantResult = grantItemToCharacterLocation(characterId, stationId, ITEM_FLAGS.HANGAR, TOKEN_TYPE_ID, quantityToGrant, { transient: true });
...
function consumeTokenStack(tokenItemId, tokenCount, options = {}) {
    ...
    const remaining = Math.max(0, normalizeInteger(currentToken.stacksize, 0) - normalizedTokenCount);
...
function restoreEscrowedItem(itemId, locationId, flagId = ITEM_FLAGS.HANGAR, options = {}) {
    const currentItem = findItemById(itemId);
    if (!currentItem) {
        return {
            success: false,
...
function deliverRaffleItemToCharacterStation(itemId, ownerId, stationId, flagId = ITEM_FLAGS.HANGAR, options = {}) {
    const currentItem = findItemById(itemId);
    ...
    if (normalizeInteger(currentItem.ownerID, 0) === normalizedOwnerId &&
...
function seedShipsForCharacter(charId, stationId, shipTypes = [], options = {}) {
    ...
    return grantItemsToCharacterLocation(normalizedCharacterId, normalizedStationId, ITEM_FLAGS.HANGAR, grantEntries);
}
...
//# sourceMappingURL=raffleInventory.js.map