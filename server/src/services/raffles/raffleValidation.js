...
const { throwWrappedRaffleCreateError, } = require(path.join(__dirname, "../../common/machoErrors"));
const { TOKEN_TYPE_ID, } = require(path.join(__dirname, "./raffleConstants"));
const { normalizeInteger, getSessionCharacterID, isSupportedTicketCount, isValidTotalPrice, } = require(path.join(__dirname, "./raffleHelpers"));
const { ITEM_FLAGS, resolveStationSolarSystemId, } = require(path.join(__dirname, "./raffleInventory"));
...
function isTriglavianSystem(solarSystemId) {
    const solarSystem = worldData.getSolarSystemByID(normalizeInteger(solarSystemId, 0));
    return normalizeInteger(solarSystem && solarSystem.factionID, 0) ===
        TRIGLAVIAN_FACTION_ID;
}
function validateCreateInput({ session, sessionStationId, creationData, totalPrice, item, token, raffleState, }) {
    ...
    if (normalizeInteger(token.locationID, 0) !==
    ...
    const solarSystemId = resolveStationSolarSystemId(item.locationID, creationData.solar_system_id);
    if (normalizeInteger(creationData.solar_system_id, 0) > 0 &&
        solarSystemId > 0 &&
...
//# sourceMappingURL=raffleValidation.js.map