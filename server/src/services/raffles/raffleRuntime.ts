...
class RaffleRuntime {
  declare _expirationTimers: any;
  declare _initialized: any;
  declare _seedManager: any;
  declare _state: any;
  ...
  reset(options: Record<string, any> = {}) {
  ...
  grab(filters: Record<string, any> = {}, constraints: Record<string, any> = {}, size = null, session = null) {
    this.initialize();
    this._ensureCharacterReady(session);
    this._sweepExpiredRaffles();
  ...
  _buyTickets(session, raffleState, requestedTicketNumbers: any[] = []) {
    const ownerId = getSessionCharacterID(session);
    if (ownerId <= 0) {
      throwWrappedRaffleError("raffles.TicketUnavailableError");
    }
  ...
  _getAvailableTicketNumbers(raffleState) {
    const availableTicketNumbers: any[] = [];
