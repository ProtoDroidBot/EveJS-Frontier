...
function buildChangeEntry(item, previousData: Record<string, any> = {}) {
  return {
    item,
    previousData,
  };
...
function notifyInventoryChange(session, item, previousData: Record<string, any> = {}) {
  if (
    !session ||
    !item ||
    typeof session.sendNotification !== "function"
...
function notifyInventoryChangesToCharacter(charId, changes: any[] = [], options: Record<string, any> = {}) {
...
function ensureCharacterTokenStack(session, options: Record<string, any> = {}) {
...
function consumeTokenStack(tokenItemId, tokenCount, options: Record<string, any> = {}) {
...
function moveItemToRaffleEscrow(itemId, options: Record<string, any> = {}) {
...
function seedShipsForCharacter(charId, stationId, shipTypes: any[] = [], options: Record<string, any> = {}) {
  const normalizedCharacterId = normalizeInteger(charId, 0);
  const normalizedStationId = normalizeInteger(stationId, 0);
  if (normalizedCharacterId <= 0 || normalizedStationId <= 0) {
    return {
...
function seedItemsForCharacter(charId, stationId, itemTypes: any[] = [], options: Record<string, any> = {}) {
  const normalizedCharacterId = normalizeInteger(charId, 0);
  const normalizedStationId = normalizeInteger(stationId, 0);
  if (normalizedCharacterId <= 0 || normalizedStationId <= 0) {
    return {
