...
function getAccountRecordByUserID(userID) {
  ...
  for (const [username, account] of Object.entries<any>(accounts)) {
...
function buildTrainingSlotList(trainingSlots) {
  return {
    type: "list",
    items: Object.entries<any>(trainingSlots)
      .map(([slot, expiry]) => [
        Number(slot),
...
function buildRedeemTokenRow(token) {
  ...
  return {
    tokenID: normalized.tokenID,
    massTokenID:
      normalized.massTokenID && normalized.massTokenID !== normalized.tokenID
...
function buildRedeemTokenRowset(tokens) {
  return buildDbRowset(
    REDEEM_TOKEN_COLUMNS,
    (Array.isArray(tokens) ? tokens : []).map(buildRedeemTokenRow).filter(Boolean),
...
function buildRedeemErrorPayload(message, data: Record<string, any> = {}) {
  return buildKeyVal([
    ["msg", message],
...
function syncInventoryItemChange(session, item, previousState: Record<string, any> = {}) {
...
function selectTrainingSlot(trainingSlots, requestedTrainingID) {
  ...
  return Object.entries<any>(trainingSlots)
    .map(([slot, expiry]) => ({
      slot: String(slot),
...
function buildRedeemResult(session, options: Record<string, any> = {}) {
...
class UserService extends BaseService {
  ...
  Handle_GetMultiCharactersTrainingSlots(args, session) {
    ...
    return {
      type: "dict",
      entries: Object.entries<any>(configuredSlots).map(([slot, expiry]) => [
        Number(slot),
        buildFiletimeLong(expiry),
      ]),
    };
  ...
  Handle_ClaimRedeemTokens(args, session, kwargs) {
    ...
    const consumedTokenIDs = new Set<any>();
