...
const { buildDbRowset, buildDict, buildFiletimeLong, buildKeyVal, currentFileTime, extractDictEntries, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { DEFAULT_MCT_EXPIRY_FILETIME, getCharacterRecord, } = require(path.join(__dirname, "../character/characterState"));
const { ITEM_FLAGS, grantItemToCharacterLocation, } = require(path.join(__dirname, "../inventory/itemStore"));
const { adjustCharacterBalance, } = require(path.join(__dirname, "../account/walletState"));
const { EVERMARK_ISSUER_CORP_ID, adjustCharacterWalletLPBalance, } = require(path.join(__dirname, "../corporation/lpWalletState"));
const { notifyFreeSkillPointsChanged, } = require(path.join(__dirname, "../skills/training/skillQueueNotifications"));
const { disconnectCharacterSession, } = require(path.join(__dirname, "../_shared/sessionDisconnect"));
const { computeFutureFileTime, getTrainingSlotsForAccount, readRuntimeAccount, writeRuntimeAccount, } = require(path.join(__dirname, "../newEdenStore/storeState"));
...
const REDEEM_TOKEN_COLUMNS = Object.freeze([
    ["tokenID", 0x03],
    ["massTokenID", 0x03],
    ["typeID", 0x03],
    ["quantity", 0x03],
    ["blueprintRuns", 0x03],
    ["blueprintMaterialLevel", 0x03],
    ["blueprintProductivityLevel", 0x03],
    ["label", 0x82],
    ["description", 0x82],
    ["dateTime", 0x40],
    ["expireDateTime", 0x40],
    ["availableDateTime", 0x40],
    ["stationID", 0x14],
    ["soulbound", 0x03],
]);
...
function normalizeRedeemTokenRecord(token) {
    ...
    return {
        ...
        availableDateTime: normalizeFileTimeString(source.availableDateTime, "0"),
        ...
        available: source.available === undefined || source.available === null
            ? true
...
function buildRedeemTokenRow(token) {
    const normalized = normalizeRedeemTokenRecord(token);
    if (!normalized) {
        return null;
    }
    const dateTime = toFileTimeBigInt(normalized.dateTime, currentFileTime());
    const expireDateTime = toFileTimeBigInt(normalized.expireDateTime, 0n);
    const availableDateTime = toFileTimeBigInt(normalized.availableDateTime, dateTime);
    return {
        tokenID: normalized.tokenID,
        massTokenID: normalized.massTokenID && normalized.massTokenID !== normalized.tokenID
            ? normalized.massTokenID
            : null,
        typeID: normalized.typeID,
        quantity: normalized.quantity,
        blueprintRuns: normalized.blueprintRuns,
        blueprintMaterialLevel: normalized.blueprintMaterialLevel,
        blueprintProductivityLevel: normalized.blueprintProductivityLevel,
        label: { type: "wstring", value: normalized.label },
        description: { type: "wstring", value: normalized.description },
        dateTime,
        expireDateTime: expireDateTime > 0n ? expireDateTime : null,
        availableDateTime: availableDateTime > 0n ? availableDateTime : dateTime,
        stationID: normalized.stationID || null,
        soulbound: normalized.soulbound ? 1 : 0,
    };
}
function buildRedeemTokenRowset(tokens) {
    return buildDbRowset(REDEEM_TOKEN_COLUMNS, (Array.isArray(tokens) ? tokens : []).map(buildRedeemTokenRow).filter(Boolean), "carbon.common.script.sys.crowset.CRowset");
}
...
function isAutoInjectedRedeemToken(token) {
    ...
    return Boolean((token && token.soulbound)
...
function generateRedeemTokenID(existingTokens) {
    const maxExisting = Math.max(0, ...(Array.isArray(existingTokens) ? existingTokens : [])
...
function resolveRedeemDestinationID(session, characterID, useHomeStation = false) {
    ...
    if (useHomeStation) {
        return (normalizePositiveInteger(character && character.homeStationID, 0) ||
    ...
    return (normalizePositiveInteger(session && (session.stationid || session.stationID), 0) ||
...
function grantRedeemSkillPoints(characterID, token) {
    ...
    notifyFreeSkillPointsChanged(characterID, Math.max(0, normalizeInteger((updateResult.data || {}).freeSkillPoints, 0)));
    return {
        success: true,
        data: {
            skillPoints,
...
function grantRedeemHeraldryLp(characterID, token) {
    ...
    const creditResult = adjustCharacterWalletLPBalance(characterID, EVERMARK_ISSUER_CORP_ID, lpAmount, {
        changeType: "redeem",
    });
    if (!creditResult.success) {
        return creditResult;
...
function buildRedeemResult(session, options = {}) {
    const stationID = options.stationID !== undefined
        ? options.stationID
        : Number(session && (session.stationid ||
            session.stationID ||
            session.structureid ||
...
class UserService extends BaseService {
     ...
     *
     * Called during login and whenever OnRedeemingQueueUpdated is received.
     * This client expects the same packed CRowset returned by Tranquility.
     */
    Handle_GetRedeemTokens(args, session, kwargs) {
        ...
        return buildRedeemTokenRowset(activeTokens);
    }
    ...
    Handle_ActivateMultiTraining(args, session, kwargs) {
        const trainingID = (args && args.length > 1 ? args[1] : undefined) ??
            getKwargValue(kwargs, "trainingID", null);
        log.info(`[userSvc] ActivateMultiTraining user=${getAccountIDFromSession(session)} trainingID=${trainingID || "next"}`);
        return buildTrainingSlotList(grantTrainingDays(getAccountIDFromSession(session), trainingID));
    }
    Handle_ActivateSoulboundMultiTraining(args, session, kwargs) {
        const tokens = args && Array.isArray(args[1]) ? args[1] : [];
        const trainingID = (args && args.length > 2 ? args[2] : undefined) ??
            getKwargValue(kwargs, "trainingID", null);
        const durationDays = Math.max(1, tokens.length || 1) * DEFAULT_MCT_GRANT_DAYS;
        log.info(`[userSvc] ActivateSoulboundMultiTraining user=${getAccountIDFromSession(session)} trainingID=${trainingID || "next"} days=${durationDays}`);
        const accountID = getAccountIDFromSession(session);
        const result = buildTrainingSlotList(grantTrainingDays(accountID, trainingID, durationDays));
        const requestedTokens = extractRequestedRedeemTokens(tokens);
        if (requestedTokens.length > 0) {
            const remainingTokens = getRedeemTokensForAccount(accountID).filter((token) => !requestedTokens.some((request) => tokenMatchesRequest(token, request)));
    ...
    Handle_ClaimRedeemTokens(args, session, kwargs) {
        ...
        const accountID = getAccountIDFromSession(session);
        const characterID = normalizePositiveInteger(args && args.length > 1 ? args[1] : 0, 0) ||
            getCharacterIDFromSession(session);
        ...
        for (const request of requestedTokens) {
            ...
            const grantResult = grantItemToCharacterLocation(characterID, destinationID, ITEM_FLAGS.HANGAR, token.typeID, token.quantity, {
            ...
            if (session && Number(session.characterID || session.charid || 0) === Number(characterID)) {
                for (const change of (grantResult.data && grantResult.data.changes) || []) {
                    syncInventoryItemChange(session, change.item, change.previousState || change.previousData || {});
                }
            }
        }
        if (consumedTokenIDs.size > 0) {
            writeRedeemTokensForAccount(accountID, currentTokens.filter((token) => !consumedTokenIDs.has(Number(token.tokenID))));
        }
        return {
            type: "dict",
            entries: [
    ...
    Handle_CreateRedeemTokenQA(args, session, kwargs) {
        const accountID = normalizePositiveInteger(getKwargValue(kwargs, "userID", 0), 0) ||
        ...
        log.info(`[userSvc] CreateRedeemTokenQA user=${accountID} token=${tokenID} type=${typeID} qty=${quantity}`);
    ...
    Handle_ActivateCharacterReSculpt(args, session) {
        log.info(`[userSvc] ActivateCharacterReSculpt char=${getCharacterIDFromSession(session)} item=${args && args.length ? args[0] : "?"}`);
        return true;
    }
    Handle_CastUpvote() {
    ...
    Handle_ReportBot(args, session) {
        log.info(`[userSvc] ReportBot reporter=${getCharacterIDFromSession(session)} target=${args && args.length ? args[0] : "?"}`);
        return true;
    }
    Handle_ReportISKSpammer(args, session) {
        log.info(`[userSvc] ReportISKSpammer reporter=${getCharacterIDFromSession(session)} target=${args && args.length ? args[0] : "?"}`);
        return true;
    }
    Handle_GetCreateDate(args, session) {
        const characterID = getCharacterIDFromSession(session);
        const character = characterID ? getCharacterRecord(characterID) : null;
        return buildFiletimeLong(character && character.createDateTime
    ...
    Handle_UserLogOffCharacter(args, session) {
        log.info(`[userSvc] UserLogOffCharacter user=${session ? session.userid : "?"} char=${session ? session.characterID : "?"}`);
        ...
        if (!disconnectResult.success) {
            log.warn(`[userSvc] Failed to disconnect char=${characterID}: ${disconnectResult.errorMsg}`);
        }
        return true;
    }
}
