...
const { strVal } = require(path.join(__dirname, "../../network/tcp/utils/marshal"));
...
class AlertService extends BaseService {
    ...
    Handle_GroupBeanDelivery(args, session) {
        ...
        log.debug(`[AlertService] GroupBeanDelivery entries=${entryCount} compressed=${isCompressed}`);
        return null;
    }
    ...
    Handle_SendClientStackTraceAlert(args, session) {
        ...
        log.warn(`[AlertService] SendClientStackTraceAlert char=${charID} ` +
