...
function normalizeState(rawState = {}) {
    ...
    normalized.nextContributionID = Math.max(1, toInteger(state.nextContributionID, 1));
...
function inferOwnerKind(ownerID) {
    ...
    if (isInRange(numericOwnerID, OWNER_RANGES.CHARACTER_GEN2_MIN, OWNER_RANGES.CHARACTER_GEN2_MAX) ||
        isInRange(numericOwnerID, OWNER_RANGES.CHARACTER_GEN3_MIN, OWNER_RANGES.CHARACTER_GEN3_MAX)) {
        return OWNER_KIND.CHARACTER;
    }
    if (isInRange(numericOwnerID, OWNER_RANGES.CORPORATION_MIN, OWNER_RANGES.CORPORATION_MAX)) {
        return OWNER_KIND.CORPORATION;
    }
    if (isInRange(numericOwnerID, OWNER_RANGES.ALLIANCE_MIN, OWNER_RANGES.ALLIANCE_MAX)) {
        return OWNER_KIND.ALLIANCE;
    }
    return OWNER_KIND.UNKNOWN;
}
...
function getPool(state = null, targetID = null) {
    ...
    return normalizePool(sourceState.pools[String(numericTargetID)] || {
...
function listContributionsForContributor(contributorID, options = {}) {
    ...
    return Object.values(state.contributions || {})
        .filter((contribution) => toInteger(contribution && contribution.contributorID, 0) ===
...
function listBountyHuntersByKind(ownerKind, options = {}) {
    const state = options.state || readState();
    const bucket = state.hunterStats &&
    ...
    return Object.values(bucket)
        ...
        .sort((left, right) => right.bountiesClaimed - left.bountiesClaimed ||
...
function allocatePayoutAcrossPools(pools, payoutAmount) {
    const totalBounty = normalizeMoney(pools.reduce((sum, pool) => sum + Math.max(0, normalizeMoney(pool.bounty, 0)), 0), 0);
...
function buildHunterIdentity(hunterCharacterID) {
    ...
    return {
        characterID,
        corporationID: toInteger(characterRecord && characterRecord.corporationID, 0) || null,
        allianceID: toInteger(characterRecord && characterRecord.allianceID, 0) || null,
    };
}
function incrementHunterStat(state, ownerKind, bountyHunterID, amount, affiliation = {}) {
    ...
    const currentStat = normalizeHunterStat(state.hunterStats[ownerKind][String(numericBountyHunterID)] || {
        bountyHunterID: numericBountyHunterID,
    }, ownerKind);
    state.hunterStats[ownerKind][String(numericBountyHunterID)] = {
        ...currentStat,
        bountyHunterID: numericBountyHunterID,
        corporationID: toInteger(affiliation.corporationID, 0) ||
            currentStat.corporationID ||
            null,
        allianceID: toInteger(affiliation.allianceID, 0) ||
...
function recordHunterPayout(state, hunterCharacterID, payoutAmount) {
    ...
    incrementHunterStat(state, OWNER_KIND.CHARACTER, hunter.characterID, payoutAmount, {
        corporationID: hunter.corporationID,
        allianceID: hunter.allianceID,
    });
    if (hunter.corporationID) {
        incrementHunterStat(state, OWNER_KIND.CORPORATION, hunter.corporationID, payoutAmount, {
            corporationID: hunter.corporationID,
            allianceID: hunter.allianceID,
        });
    }
    if (hunter.allianceID) {
        incrementHunterStat(state, OWNER_KIND.ALLIANCE, hunter.allianceID, payoutAmount, {
            allianceID: hunter.allianceID,
        });
    }
}
function claimBountyPayout({ victim = {}, iskLost = 0, hunterCharacterID = 0 } = {}) {
    ...
    const victimCharacterID = toInteger(victim.victimCharacterID ?? victim.characterID, 0);
    if (numericHunterCharacterID <= 0 ||
    ...
    const cappedByLoss = normalizeMoney(Math.max(0, normalizeMoney(iskLost, 0)) * PLAYER_BOUNTY_PAYOUT_LOSS_RATIO, 0);
    if (!(cappedByLoss > 0)) {
        return {
            eligible: false,
            amount: 0,
    ...
    for (const allocation of allocations) {
        ...
        const nextBounty = normalizeMoney(Math.max(0, normalizeMoney(pool.bounty, 0) - allocation.amount), 0);
    ...
    return {
        eligible: true,
        amount: normalizeMoney(allocations.reduce((sum, allocation) => sum + allocation.amount, 0), 0),
...
//# sourceMappingURL=playerBountyState.js.map