...
const { notifyKillRightUsed, } = require(path.join(__dirname, "./killRightNotifications"));
const crimewatchState = require(path.join(__dirname, "../security/crimewatchState"));
const sessionRegistry = require(path.join(__dirname, "../chat/sessionRegistry"));
const { JOURNAL_ENTRY_TYPE, buildNotEnoughMoneyUserErrorValues, getCharacterWallet, transferCharacterBalance, } = require(path.join(__dirname, "../account/walletState"));
const auditEvents = [];
...
function applyKillRightSuspectTimer(killRight, session = null, nowMs = Date.now()) {
    ...
    const result = crimewatchState.setCharacterCrimewatchDebugState(targetCharacterID, {
        suspect: true,
...
function notifyKillRightActivated(killRight, activatorID, session = null) {
    for (const targetSession of uniqueSessionsForCharacters([activatorID, killRight && killRight.fromID, killRight && killRight.toID], session)) {
...
class KillRightMgrService extends BaseService {
    ...
    Handle_ActivateKillRight(args, session) {
        ...
        const result = killRightState.activateOwnedKillRight(killRightID, characterID, {
            nowMs: Date.now(),
        });
        if (!result.success) {
            recordAuditEvent("activate_kill_right_rejected", [killRightID], session);
            log.debug(`[KillRightMgr] ActivateKillRight rejected: killRightID=${killRightID} error=${result.errorMsg}`);
        ...
        log.info(`[KillRightMgr] ActivateKillRight: killRightID=${killRightID} activator=${characterID} target=${result.data.killRight.toID}`);
        return null;
    }
    Handle_BuyKillRight(args, session) {
        ...
        const characterID = getCharacterID(session);
        const result = killRightState.buyKillRight(killRightID, characterID, buildSessionOwnerIDs(session), price, {
            nowMs: Date.now(),
            paymentCallback: (record) => transferCharacterBalance(characterID, record.fromID, record.price, {
        ...
        if (!result.success) {
            ...
            log.debug(`[KillRightMgr] BuyKillRight rejected: killRightID=${killRightID} error=${result.errorMsg}`);
        ...
        log.info(`[KillRightMgr] BuyKillRight: killRightID=${killRightID} buyer=${characterID} target=${result.data.killRight.toID} price=${price}`);
        return null;
    }
}
...
//# sourceMappingURL=killRightMgrService.js.map