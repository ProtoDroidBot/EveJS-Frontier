...
// in the MISC group.
function notifyKillRightEarned({ ownerID, targetID }: Record<string, any> = {}) {
...
function notifyKillRightUsed({ ownerID, targetID }: Record<string, any> = {}) {
  const numericOwnerID = toPositiveInteger(ownerID);
  const numericTargetID = toPositiveInteger(targetID);
  if (!numericOwnerID || !numericTargetID || numericOwnerID === numericTargetID) {
    return null;
...
function notifyKillRightAvailable({ targetID, ownerID, price, toEntityID }: Record<string, any> = {}) {
  const numericTargetID = toPositiveInteger(targetID);
  const numericOwnerID = toPositiveInteger(ownerID);
...
function notifyKillRightUnavailable({ targetID, ownerID, toEntityID }: Record<string, any> = {}) {
  const numericTargetID = toPositiveInteger(targetID);
  const numericOwnerID = toPositiveInteger(ownerID);
  if (!numericTargetID || !numericOwnerID || numericTargetID === numericOwnerID) {
    return null;
  ...
  const numericToEntityID = toPositiveInteger(toEntityID);
  const isRestricted = numericToEntityID > 0;
  const data: Record<string, any> = { charID: numericOwnerID };
  if (isRestricted) {
    data.toEntityID = numericToEntityID;
  }
  return createNotification(numericTargetID, {
