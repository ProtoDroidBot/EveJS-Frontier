...
const { JOURNAL_ENTRY_TYPE, adjustCharacterBalance, } = require(path.join(__dirname, "../account/walletState"));
...
function formatFiletime(whenMs) {
    const normalizedWhenMs = Number.isFinite(Number(whenMs))
        ...
        : nowProvider();
    return (BigInt(normalizedWhenMs) * FILETIME_TICKS_PER_MS +
        FILETIME_EPOCH_OFFSET).toString();
}
...
function resolveSecurityStatusRecoveryBaseModifier(victimEntity = {}, resolution = {}, context = {}) {
    ...
    return Math.min(SECURITY_STATUS_RECOVERY_MODIFIER_CAP, amount * SECURITY_STATUS_RECOVERY_MODIFIER_PER_ISK);
}
function resolveFastTalkSecurityStatusMultiplier(characterID) {
    ...
    const multiplier = toFiniteNumber(standingRuntime.getCharacterSocialAttribute(characterID, ATTRIBUTE_FAST_TALK_PERCENTAGE, 1), 1);
    return Math.max(0, multiplier);
}
function findSessionForCharacter(characterID, finalAttacker = {}, context = {}) {
    const directSession = context.session ||
        context.attackerSession ||
        finalAttacker.session ||
        null;
    if (directSession &&
        toPositiveInt(directSession.characterID ?? directSession.charID ?? directSession.charid, 0) === characterID) {
    ...
    return sessionRegistry
        .getSessions()
        .find((session) => toPositiveInt(session && (session.characterID ?? session.charID ?? session.charid), 0) ===
        characterID) || null;
}
function buildSecurityStatusRecoveryResult({ applied = false, reason = "NOT_APPLIED", characterID = 0, previousSecurityStatus = 0, nextSecurityStatus = previousSecurityStatus, baseModifier = 0, fastTalkMultiplier = 1, modifier = 0, } = {}) {
    return {
        applied,
        reason,
        ...
        deltaSecurityStatus: roundNumber(toFiniteNumber(nextSecurityStatus, 0) - toFiniteNumber(previousSecurityStatus, 0), SECURITY_STATUS_RECOVERY_ROUND_DIGITS),
...
function applySecurityStatusRecovery(characterID, victimEntity, finalAttacker, resolution, context = {}) {
    ...
    const nextSecurityStatus = Math.min(maxSecurityStatus, previousSecurityStatus + ((maxSecurityStatus - previousSecurityStatus) * modifier));
    if (Number(roundNumber(nextSecurityStatus, SECURITY_STATUS_RECOVERY_ROUND_DIGITS)) ===
    ...
    const updateResult = crimewatchState.setCharacterSecurityStatus(normalizedCharacterID, nextSecurityStatus, {
    ...
    return buildSecurityStatusRecoveryResult({
        ...
        reason: "APPLIED",
        characterID: normalizedCharacterID,
        previousSecurityStatus: updateResult.data && updateResult.data.previousSecurityStatus !== undefined
            ? updateResult.data.previousSecurityStatus
            : previousSecurityStatus,
        nextSecurityStatus: updateResult.data && updateResult.data.securityStatus !== undefined
...
function resolveNpcBountyAmount(victimEntity = {}, context = {}) {
    ...
    const solarSystemID = toPositiveInt(context.solarSystemID, toPositiveInt(victimEntity.systemID, 0)) || 0;
    ...
    return {
        ...
        isModified: normalizeMoney(baseAmount, 0) !== amount ||
...
function findBucketBySourceReference(referenceID) {
    ...
    return Array.from(bucketsByKey.values()).find((bucket) => (bucket &&
        Array.isArray(bucket.sourceReferences) &&
        bucket.sourceReferences.some((sourceReference) => toPositiveInt(sourceReference, null) === normalizedReferenceID))) || null;
}
...
function loadPersistedBuckets() {
    ...
    for (const [key, raw] of Object.entries(result.data)) {
        ...
        bucketsByKey.set(key, {
            ...
            npcTypes: raw.npcTypes && typeof raw.npcTypes === "object" ? { ...raw.npcTypes } : {},
...
function recordNpcBountyKill(victimEntity = {}, finalAttacker = {}, context = {}) {
    ...
    const solarSystemID = toPositiveInt(resolution.solarSystemID, toPositiveInt(victimEntity && victimEntity.systemID, 0)) || 0;
    ...
    // Advance the "Destroy N non-capsuleers" AIR daily goal on each fresh NPC
    // kill. Lazy-required and fully defensive so daily-goal bookkeeping can never
    // disrupt bounty payouts.
    try {
        require(path.join(__dirname, "../dailyGoals/dailyGoalsState")).recordActivity(characterID, "kill_npc", 1);
    }
    catch (dailyGoalError) {
        log.debug(`[BountyRuntime] daily-goal kill_npc hook failed char=${characterID}: ${dailyGoalError.message}`);
    }
    const securityStatusRecovery = applySecurityStatusRecovery(characterID, victimEntity, finalAttacker, resolution, context);
    return {
        ...resolution,
        characterID,
        solarSystemID,
...
function resolveBountyJournalReferenceID(bucket) {
    if (Math.max(0, toInt(bucket && bucket.kills, 0)) <= 1) {
        return (resolveSingleVictimTypeID(bucket) ||
            toPositiveInt(bucket && bucket.sourceReferences && bucket.sourceReferences[0], 0) ||
            -1);
    }
    return (toPositiveInt(bucket && bucket.solarSystemID, 0) ||
        toPositiveInt(bucket && bucket.sourceReferences && bucket.sourceReferences[0], 0) ||
        -1);
}
...
//# sourceMappingURL=bountyRuntime.js.map