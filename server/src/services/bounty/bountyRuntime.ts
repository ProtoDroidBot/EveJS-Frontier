...
function resolveAttackerCharacterID(finalAttacker: Record<string, any> = {}, context: Record<string, any> = {}) {
...
function resolveAuthoredMultiplier(entity: Record<string, any> = {}, context: Record<string, any> = {}) {
  const candidates = [
...
function resolveSolarSystemBountyMultiplier(_solarSystemID, _context: Record<string, any> = {}) {
  return 1;
}

function resolveEssBountySplit(_solarSystemID, amount, _context: Record<string, any> = {}) {
  return {
...
function resolveExplicitSecurityStatusRecoveryModifier(victimEntity: Record<string, any> = {}, context: Record<string, any> = {}) {
  const candidates = [
...
function resolveSecurityStatusRecoveryBaseModifier(victimEntity: Record<string, any> = {}, resolution: Record<string, any> = {}, context: Record<string, any> = {}) {
...
function findSessionForCharacter(characterID, finalAttacker: Record<string, any> = {}, context: Record<string, any> = {}) {
  const directSession =
    context.session ||
...
function applySecurityStatusRecovery(characterID, victimEntity, finalAttacker, resolution, context: Record<string, any> = {}) {
...
function resolveNpcBountyAmount(victimEntity: Record<string, any> = {}, context: Record<string, any> = {}) {
...
function addToBucket(bucket, award: Record<string, any> = {}) {
...
function loadPersistedBuckets() {
  ...
  for (const [key, raw] of Object.entries<any>(result.data)) {
...
function recordNpcBountyKill(victimEntity: Record<string, any> = {}, finalAttacker: Record<string, any> = {}, context: Record<string, any> = {}) {
  ensureLoaded();
  ...
  try {
    ...
    )).recordActivity(characterID, "kill_npc", 1);
  } catch (dailyGoalError) {
    log.debug(
      `[BountyRuntime] daily-goal kill_npc hook failed char=${characterID}: ${dailyGoalError.message}`,
    );
  }
...
function buildWalletDescription(bucket) {
  const npcTypeLines = Object.entries<any>(bucket.npcTypes || {})
    .map(([typeID, count]) => [
...
function resolveSingleVictimTypeID(bucket) {
  const entries = Object.entries<any>(bucket && bucket.npcTypes || {})
    .map(([typeID, count]) => [
      toPositiveInt(typeID, 0) || 0,
      Math.max(0, toInt(count, 0)),
    ])
