...
const bountyAuditEvents: any[] = [];

function recordAuditEvent(kind, args: any[] = [], session = null) {
...
function buildBountyPayload(targetID, bounty = 0, extraEntries: any[] = []) {
...
function buildBountyPayloadFromPool(pool: Record<string, any> = {}) {
...
function buildContributionPayload(contribution: Record<string, any> = {}) {
  return buildKeyVal([
...
function buildBountyHunterPayload(stat: Record<string, any> = {}) {
  return buildKeyVal([
...
function buildKillRightPayload(record: Record<string, any> = {}) {
  return buildKeyVal([
...
function buildKnownBountyOwnerIds(session = null) {
  ...
  for (const [characterID, characterRecord] of Object.entries<any>(characters)) {
