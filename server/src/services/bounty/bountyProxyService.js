...
const { notifyBountyPlacedOnAlliance, notifyBountyPlacedOnCharacter, notifyBountyPlacedOnCorporation, } = require(path.join(__dirname, "./bountyNotifications"));
const { notifyKillRightAvailable, notifyKillRightUnavailable, } = require(path.join(__dirname, "./killRightNotifications"));
...
const { buildFiletimeLong } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
function extractIdList(rawValue) {
    ...
    return Array.from(new Set(extractedIds.filter((value) => Number.isInteger(value) && Number.isFinite(value))));
}
...
class BountyProxyService extends BaseService {
    ...
    Handle_AddToBounty(args, session) {
        ...
        if (!(numericAmount > 0) || numericAmount < minimumAmount) {
            throwBountyValidationError(`Bounty must be at least ${minimumAmount} ISK for this target.`);
        }
        ...
        log.info(`[BountyProxy] AddToBounty placed: target=${targetID} amount=${numericAmount} by=${characterID}`);
    ...
    Handle_GetBounties(args, session) {
        ...
        log.debug(`[BountyProxy] GetBounties: ${JSON.stringify(requestedIds)}`);
    ...
    Handle_GetBountiesAndKillRights(args, session) {
        ...
        log.debug(`[BountyProxy] GetBountiesAndKillRights: ${JSON.stringify(requestedIds)}`);
        return [
            buildList(buildBountyEntries(requestedIds)),
            buildList(killRightState
                .listAvailableKillRightsOnCharacters(requestedKillRightIds, buildSessionKillRightOwnerIDs(session))
    ...
    Handle_GetMyBounties(_args, session) {
        ...
        const characterID = getSessionCharacterID(session);
        return buildList(playerBountyState
    ...
    Handle_GetMyKillRights(_args, session) {
        ...
        const characterID = getSessionCharacterID(session);
        return buildList(killRightState
    ...
    Handle_GetKillRightsOnCharacters(args, session) {
        ...
        return buildList(killRightState
            .listAvailableKillRightsOnCharacters(requestedToIDs, buildSessionKillRightOwnerIDs(session))
            .map((record) => buildKillRightPayload(record)));
    }
    Handle_SellKillRight(args, session) {
        ...
        const result = killRightState.sellKillRight(killRightID, characterID, price, restrictedTo);
        if (!result.success) {
            throwKillRightUserError(result.errorMsg);
        }
        log.info(`[BountyProxy] SellKillRight: killRightID=${killRightID} price=${price} restrictedTo=${restrictedTo}`);
    ...
    Handle_CancelSellKillRight(args, session) {
        ...
        const result = killRightState.cancelSellKillRight(killRightID, characterID, toID);
        if (!result.success) {
            log.debug(`[BountyProxy] CancelSellKillRight ignored stale/invalid right: killRightID=${killRightID} error=${result.errorMsg}`);
            return null;
        }
        log.info(`[BountyProxy] CancelSellKillRight: killRightID=${killRightID}`);
...
//# sourceMappingURL=bountyProxyService.js.map