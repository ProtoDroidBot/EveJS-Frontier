...
const { getAllianceCorporationIDs, } = require(path.join(__dirname, "../corporation/corporationState"));
const { listCorporationMembers, } = require(path.join(__dirname, "../corporation/corporationRuntimeState"));
...
function notifyBountyPlacedOnMembers(typeID, characterIDs, bountyPlacerID, amount) {
    ...
    for (const characterID of characterIDs) {
        ...
        const result = deliverBountyPlaced(numericCharacterID, typeID, numericPlacerID, numericAmount);
...
function notifyBountyPlacedOnCharacter({ targetID, bountyPlacerID, amount } = {}) {
    ...
    return deliverBountyPlaced(numericTargetID, NOTIFICATION_TYPE.BOUNTY_PLACED_CHAR, numericPlacerID, numericAmount);
}
...
function notifyBountyPlacedOnCorporation({ targetID, bountyPlacerID, amount } = {}) {
    ...
    return notifyBountyPlacedOnMembers(NOTIFICATION_TYPE.BOUNTY_PLACED_CORP, listCorporationMembers(numericTargetID).map((member) => member && member.characterID), bountyPlacerID, amount);
}
...
function notifyBountyPlacedOnAlliance({ targetID, bountyPlacerID, amount } = {}) {
    ...
    return notifyBountyPlacedOnMembers(NOTIFICATION_TYPE.BOUNTY_PLACED_ALLIANCE, characterIDs, bountyPlacerID, amount);
}
module.exports = {
...
//# sourceMappingURL=bountyNotifications.js.map