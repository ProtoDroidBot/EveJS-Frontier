...
function normalizeHunterStat(stat: Record<string, any> = {}, ownerKind: string = OWNER_KIND.UNKNOWN) {
...
function normalizeHunterStats(rawStats: Record<string, any> = {}) {
  ...
  for (const ownerKind of [
    ...
    const bucket = rawStats && rawStats[ownerKind] && typeof rawStats[ownerKind] === "object"
      ? rawStats[ownerKind]
      : {};
    for (const [bountyHunterID, stat] of Object.entries<any>(bucket)) {
...
function normalizePool(pool: Record<string, any> = {}) {
...
function listPools(options: Record<string, any> = {}) {
  const state = options.state || readState();
  return Object.values<any>(state.pools || {})
    .map((pool) => normalizePool(pool))
    .filter((pool) => pool.targetID > 0 && pool.bounty > 0);
}

function listPoolsByKind(ownerKind, options: Record<string, any> = {}) {
...
function listContributionsForContributor(contributorID, options: Record<string, any> = {}) {
  const numericContributorID = toInteger(contributorID, 0);
  if (numericContributorID <= 0) {
    return [];
  }
  const state = options.state || readState();
  return Object.values<any>(state.contributions || {})
    .filter(
      (contribution) =>
...
function listBountyHuntersByKind(ownerKind, options: Record<string, any> = {}) {
  const state = options.state || readState();
  const bucket =
    state.hunterStats &&
    ...
    typeof state.hunterStats[ownerKind] === "object"
      ? state.hunterStats[ownerKind]
      : {};
  return Object.values<any>(bucket)
...
function collectVictimBountyPools(state, victim: Record<string, any> = {}) {
  const targetIDs = [
...
function incrementHunterStat(state, ownerKind, bountyHunterID, amount, affiliation: Record<string, any> = {}) {
...
function claimBountyPayout({ victim = {}, iskLost = 0, hunterCharacterID = 0 }: Record<string, any> = {}) {
...
function placeBounty({ targetID, amount, contributorID }: Record<string, any>) {
