...
function filetimeFromMs(ms = Date.now()) {
    return (BigInt(Math.max(0, Math.trunc(Number(ms) || 0))) * FILETIME_TICKS_PER_MS +
        FILETIME_UNIX_EPOCH_OFFSET).toString();
}
...
function normalizeKillRight(record = {}, fallbackID = 0) {
    ...
    const expiryTime = String(record.expiryTime ||
...
function normalizeState(rawState = {}) {
    ...
    const sourceRights = state.rights && typeof state.rights === "object"
        ? state.rights
    ...
    for (const [killRightID, record] of Object.entries(sourceRights)) {
        ...
        if (normalizedRecord.killRightID > 0 &&
            normalizedRecord.fromID > 0 &&
            normalizedRecord.toID > 0) {
            normalized.rights[String(normalizedRecord.killRightID)] = normalizedRecord;
            normalized.nextKillRightID = Math.max(normalized.nextKillRightID, normalizedRecord.killRightID + 1);
        }
    }
    const sourceActivations = state.activations && typeof state.activations === "object"
        ? state.activations
        : {};
    for (const [activationID, record] of Object.entries(sourceActivations)) {
        ...
        if (normalizedActivation.activationID > 0 &&
            ...
            normalized.nextActivationID = Math.max(normalized.nextActivationID, normalizedActivation.activationID + 1);
        }
    }
    return normalized;
}
...
function findActiveKillRightBetween(fromID, toID, nowMs = Date.now()) {
    ...
    const record = Object.values(state.rights)
        .filter((candidate) => (candidate.fromID === numericFromID &&
        candidate.toID === numericToID &&
        isKillRightUsable(candidate, nowMs)))
        .sort((left, right) => left.killRightID - right.killRightID)[0];
    return record ? cloneValue(record) : null;
}
function createKillRight({ fromID, toID, expiryTime = null, expiryMs = DEFAULT_KILL_RIGHT_EXPIRY_MS, nowMs = Date.now(), } = {}) {
    const numericFromID = toInteger(fromID, 0);
    const numericToID = toInteger(toID, 0);
...
function createKillRightForCriminalAggression({ fromID, toID, expiryTime = null, expiryMs = DEFAULT_KILL_RIGHT_EXPIRY_MS, nowMs = Date.now(), } = {}) {
    const numericFromID = toInteger(fromID, 0);
    const numericToID = toInteger(toID, 0);
    if (numericFromID <= 0 || numericToID <= 0 || numericFromID === numericToID) {
        return {
    ...
    const existingRecord = Object.values(state.rights)
        .filter((record) => (record.fromID === numericFromID &&
...
function listMyKillRights(characterID, nowMs = Date.now()) {
    ...
    return listActiveKillRights(nowMs).filter((record) => record.fromID === numericCharacterID || record.toID === numericCharacterID);
}
function hasSaleAccess(record, validOwnerIDs = []) {
    ...
    const validSet = new Set((Array.isArray(validOwnerIDs) ? validOwnerIDs : [])
...
function listAvailableKillRightsOnCharacters(toIDs = [], validOwnerIDs = [], nowMs = Date.now()) {
    const targetIDs = new Set((Array.isArray(toIDs) ? toIDs : [toIDs])
        .map((value) => toInteger(value, 0))
        .filter((value) => value > 0));
    if (targetIDs.size === 0) {
        return [];
    }
    return listActiveKillRights(nowMs).filter((record) => targetIDs.has(record.toID) && hasSaleAccess(record, validOwnerIDs));
}
...
function buyKillRight(killRightID, buyerID, validOwnerIDs = [], expectedPrice = null, options = {}) {
    ...
    const normalizedExpectedPrice = expectedPrice === null || expectedPrice === undefined
        ? record.price
...
function readActiveActivationsForTarget(toID, nowMs = Date.now()) {
    ...
    return Object.values(state.activations)
        .filter((activation) => (activation.toID === targetID &&
...
//# sourceMappingURL=killRightState.js.map