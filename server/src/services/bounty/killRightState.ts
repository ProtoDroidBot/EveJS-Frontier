...
function normalizeKillRight(record: Record<string, any> = {}, fallbackID: number | string = 0) {
...
function normalizeActivation(record: Record<string, any> = {}, fallbackID: number | string = 0) {
...
function normalizeState(rawState: Record<string, any> = {}) {
  ...
  const sourceRights =
    state.rights && typeof state.rights === "object"
      ...
      : state.killRights && typeof state.killRights === "object"
        ? state.killRights
        : {};
  for (const [killRightID, record] of Object.entries<any>(sourceRights)) {
  ...
  const sourceActivations =
    state.activations && typeof state.activations === "object"
      ? state.activations
      : {};
  for (const [activationID, record] of Object.entries<any>(sourceActivations)) {
...
function expireRightsInState(state, nowMs = Date.now()) {
  ...
  const expired: any[] = [];
  for (const record of Object.values<any>(state && state.rights || {})) {
...
function listActiveKillRights(nowMs = Date.now()) {
  ...
  return Object.values<any>(state.rights)
...
function findActiveKillRightBetween(fromID, toID, nowMs = Date.now()) {
  ...
  const state = readState();
  const record = Object.values<any>(state.rights)
    .filter((candidate) => (
...
function createKillRightForCriminalAggression({
  ...
  const state = readState();
  const existingRecord = Object.values<any>(state.rights)
    .filter((record) => (
...
function hasSaleAccess(record, validOwnerIDs: any[] = []) {
...
function listAvailableKillRightsOnCharacters(toIDs: any[] = [], validOwnerIDs: any[] = [], nowMs = Date.now()) {
...
function sellKillRight(killRightID, ownerID, price, restrictedTo = null, options: Record<string, any> = {}) {
  const state = readState();
...
function cancelSellKillRight(killRightID, ownerID, toID = null, options: Record<string, any> = {}) {
  const state = readState();
  const numericKillRightID = toInteger(killRightID, 0);
  const numericOwnerID = toInteger(ownerID, 0);
  const numericToID = toInteger(toID, 0);
...
function appendActivation(state, record, activatorID, options: Record<string, any> = {}) {
  const nowMs = normalizeNowMs(options.nowMs);
...
function activateOwnedKillRight(killRightID, activatorID, options: Record<string, any> = {}) {
  const state = readState();
  const numericKillRightID = toInteger(killRightID, 0);
...
function buyKillRight(killRightID, buyerID, validOwnerIDs: any[] = [], expectedPrice = null, options: Record<string, any> = {}) {
  const state = readState();
  const numericKillRightID = toInteger(killRightID, 0);
...
function markKillRightUsed(killRightID, usedBy = null, options: Record<string, any> = {}) {
  const state = readState();
  const numericKillRightID = toInteger(killRightID, 0);
  const record = state.rights[String(numericKillRightID)];
  if (!record || record.usedAt || record.revokedAt) {
...
function readActiveActivationsForTarget(toID, nowMs = Date.now()) {
  ...
  return Object.values<any>(state.activations)
    .filter((activation) => (
