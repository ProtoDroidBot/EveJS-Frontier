...
function notifyBountyPlacedOnCharacter({ targetID, bountyPlacerID, amount }: Record<string, any> = {}) {
...
function notifyBountyPlacedOnCorporation({ targetID, bountyPlacerID, amount }: Record<string, any> = {}) {
  const numericTargetID = toPositiveInteger(targetID);
  if (!numericTargetID) {
    return [];
  }
...
function notifyBountyPlacedOnAlliance({ targetID, bountyPlacerID, amount }: Record<string, any> = {}) {
  const numericTargetID = toPositiveInteger(targetID);
  if (!numericTargetID) {
    return [];
  }
