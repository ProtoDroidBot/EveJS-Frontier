...
const { getCorporationMember, } = require(path.join(__dirname, "./corporationRuntimeState"));
const { getAllianceRecord, getCorporationRecord, } = require(path.join(__dirname, "./corporationState"));
const { getDaysInAlliance, } = require(path.join(__dirname, "./allianceViewState"));
    ...
    return (characterRecord &&
        (characterRecord.startDateTime ||
            (Array.isArray(characterRecord.employmentHistory)
                ? characterRecord.employmentHistory.find((entry) => Number(entry && entry.corporationID) === Number(corporationID))?.startDate
                : null) ||
...
function buildCharacterEvermarkDestinationSet(characterRecord, session) {
    const characterID = normalizePositiveInteger(session && session.characterID, normalizePositiveInteger(characterRecord && characterRecord.characterID, 0));
    const sourceCorporationID = normalizePositiveInteger(session && (session.corporationID || session.corpid), normalizePositiveInteger(characterRecord && characterRecord.corporationID, 0));
    ...
    const corpMembershipDays = getDaysSinceFiletime(getCharacterCorporationMembershipStartDate(characterID, sourceCorporationID, characterRecord));
    if (corpMembershipDays < EM_DONATION_MIN_MEMBERSHIP_DAYS) {
        return [];
    }
    const allowed = new Set([sourceCorporationID]);
    const allianceID = normalizePositiveInteger(session && (session.allianceID || session.allianceid), normalizePositiveInteger(sourceCorporation.allianceID, 0));
    if (!allianceID) {
        return [...allowed];
    }
    ...
    const executorCorpID = normalizePositiveInteger(alliance && (alliance.executorCorpID || alliance.executorCorporationID), 0);
    if (executorCorpID &&
...
function buildCorporationEvermarkDestinationSet(sourceCorporationID, session) {
    ...
    if (executorCorpID === sourceCorporationID) {
        for (const memberCorporationID of alliance.memberCorporationIDs || []) {
            ...
            if (normalizedMemberCorporationID &&
...
function readLpWalletTable() {
    ...
    return {
        _meta: result.data._meta && typeof result.data._meta === "object"
            ? {
        ...
        characterWallets: result.data.characterWallets && typeof result.data.characterWallets === "object"
            ? cloneValue(result.data.characterWallets)
            : {},
        corporationWallets: result.data.corporationWallets && typeof result.data.corporationWallets === "object"
...
function writeLpWalletTable(table) {
    const nextTable = {
        ...
        characterWallets: table && table.characterWallets && typeof table.characterWallets === "object"
            ? cloneValue(table.characterWallets)
            : {},
        corporationWallets: table && table.corporationWallets && typeof table.corporationWallets === "object"
...
function notifyCorporationLpBalanceChange(corporationID, issuerCorpID, reason = "set") {
    ...
    for (const session of sessionRegistry.getSessions()) {
        if (Number(session && (session.corporationID || session.corpid)) !== normalizedCorporationID ||
            typeof session.sendNotification !== "function") {
            continue;
        }
...
function updateWalletBalance(kind, ownerID, issuerCorpID, updater, options = {}) {
    ...
    if (kind === "corporation") {
        notifyCorporationLpBalanceChange(normalizedOwnerID, normalizedIssuerCorpID, options.reason || "set");
    }
    else {
        notifyCharacterLpBalanceChange(normalizedOwnerID, normalizedIssuerCorpID, previousAmount, nextAmount, options.changeType || "set");
    }
    return {
        success: true,
        data: {
...
function setCharacterWalletLPBalance(characterID, issuerCorpID, amount, options = {}) {
    return updateWalletBalance("character", characterID, issuerCorpID, () => normalizeLpAmount(amount, 0), { changeType: options.changeType || "set" });
}
function adjustCharacterWalletLPBalance(characterID, issuerCorpID, delta, options = {}) {
    ...
    const result = updateWalletBalance("character", characterID, issuerCorpID, (previousAmount) => Math.max(0, previousAmount + normalizedDelta), { changeType: options.changeType || "set" });
    // Advance the "Earn N LP for any corporation" AIR daily goal by LP earned.
    // Positive deltas only (ignores LP-store spends and other debits). Evermarks
    // are LP under the Evermark issuer corp and are not corporation loyalty points,
    // so they don't count (and this avoids the daily-goal EM reward self-triggering
    // the goal). Defensive so bookkeeping can never disrupt the LP wallet mutation.
    if (normalizedDelta > 0 &&
        Number(issuerCorpID) !== EVERMARK_ISSUER_CORP_ID) {
        try {
            require(path.join(__dirname, "../dailyGoals/dailyGoalsState")).recordActivity(Number(characterID) || 0, "earn_loyalty_points", normalizedDelta);
        }
        catch (dailyGoalError) {
            log.debug(`[LPWalletState] daily-goal earn_loyalty_points hook failed: ${dailyGoalError.message}`);
        }
    }
    return result;
}
function setCorporationWalletLPBalance(corporationID, issuerCorpID, amount, options = {}) {
    return updateWalletBalance("corporation", corporationID, issuerCorpID, () => normalizeLpAmount(amount, 0), { reason: options.reason || "set" });
}
function adjustCorporationWalletLPBalance(corporationID, issuerCorpID, delta, options = {}) {
    const normalizedDelta = Math.trunc(Number(delta) || 0);
    return updateWalletBalance("corporation", corporationID, issuerCorpID, (previousAmount) => Math.max(0, previousAmount + normalizedDelta), { reason: options.reason || "set" });
}
function transferCharacterWalletLPToCorporation(session, destinationCorporationID, issuerCorpID, amount) {
    ...
    const debitResult = adjustCharacterWalletLPBalance(characterID, normalizedIssuerCorpID, -normalizedAmount, { changeType: "transfer_out" });
    if (!debitResult.success) {
        return debitResult;
    }
    const creditResult = adjustCorporationWalletLPBalance(normalizedDestinationCorporationID, normalizedIssuerCorpID, normalizedAmount, { reason: "transfer_in" });
    if (!creditResult.success) {
        adjustCharacterWalletLPBalance(characterID, normalizedIssuerCorpID, normalizedAmount, { changeType: "transfer_rollback" });
        return creditResult;
    }
    return {
        success: true,
...
function transferCorporationWalletLPToCorporation(session, destinationCorporationID, issuerCorpID, amount) {
    const sourceCorporationID = normalizePositiveInteger(session && (session.corporationID || session.corpid), 0);
    const normalizedDestinationCorporationID = normalizePositiveInteger(destinationCorporationID, 0);
    const normalizedIssuerCorpID = normalizePositiveInteger(issuerCorpID, 0);
    const normalizedAmount = normalizeLpAmount(amount, 0);
    ...
    const requiredRoleMask = normalizedIssuerCorpID === EVERMARK_ISSUER_CORP_ID
    ...
    const debitResult = adjustCorporationWalletLPBalance(sourceCorporationID, normalizedIssuerCorpID, -normalizedAmount, { reason: "transfer_out" });
    if (!debitResult.success) {
        return debitResult;
    }
    const creditResult = adjustCorporationWalletLPBalance(normalizedDestinationCorporationID, normalizedIssuerCorpID, normalizedAmount, { reason: "transfer_in" });
    if (!creditResult.success) {
        adjustCorporationWalletLPBalance(sourceCorporationID, normalizedIssuerCorpID, normalizedAmount, { reason: "transfer_rollback" });
        return creditResult;
    }
    return {
        success: true,
...
//# sourceMappingURL=lpWalletState.js.map