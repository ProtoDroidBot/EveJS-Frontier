...
const { getFactionOwnerRecord, } = require(path.join(__dirname, "../faction/factionState"));
const { getCorporationWarPermitStatus, } = require(path.join(__dirname, "./warPermitState"));
const { readAggressionSettings, resolveFriendlyFireLegalAtTime, } = require(path.join(__dirname, "./aggressionSettingsState"));
...
const { CUSTOM_CORPORATION_ID_START, CUSTOM_ALLIANCE_ID_START, NPC_STARTER_CORPORATION_ID, } = require(path.join(__dirname, "./corporationIdRanges"));
...
function normalizeCorporationLogo(record = {}) {
    ...
    const hasLogoLayer = normalized.shape1 !== null ||
...
function normalizeCorporationTable(payload = {}) {
    return {
        _meta: {
            nextCustomCorporationID: normalizePositiveInteger(payload && payload._meta && payload._meta.nextCustomCorporationID, CUSTOM_CORPORATION_ID_START),
            npcSeedVersion: normalizeInteger(payload && payload._meta && payload._meta.npcSeedVersion, 0),
        },
        records: payload && payload.records && typeof payload.records === "object"
...
function normalizeAllianceTable(payload = {}) {
    return {
        _meta: {
            nextCustomAllianceID: normalizePositiveInteger(payload && payload._meta && payload._meta.nextCustomAllianceID, CUSTOM_ALLIANCE_ID_START),
        },
        records: payload && payload.records && typeof payload.records === "object"
            ? cloneValue(payload.records)
            : {},
...
function listStaticJsonlDirectoryCandidates(rootPath) {
    ...
    if (/^eve-online-static-data-\d+-jsonl$/i.test(rootName) &&
...
function loadNpcCorporations() {
    ...
    npcCorporationCache = new Map(parseJsonlFile(resolveNpcCorporationSourcePath()).map((entry) => [
...
function loadNpcCharacters() {
    ...
    npcCharacterCache = new Map(parseJsonlFile(resolveNpcCharacterSourcePath()).map((entry) => [
        normalizePositiveInteger(entry._key, 0),
        entry,
    ]));
    return npcCharacterCache;
...
function ensureCorporationMembersIndex() {
    ...
    for (const [characterID, characterRecord] of Object.entries(readCharacters())) {
        const corporationID = normalizePositiveInteger(characterRecord && characterRecord.corporationID, null);
...
function ensureAllianceCorporationsIndex() {
    ...
    const allianceRecords = allianceRecordsResult.success &&
        allianceRecordsResult.data &&
        typeof allianceRecordsResult.data === "object"
        ? allianceRecordsResult.data
        : {};
    const corporationRecords = corporationRecordsResult.success &&
    ...
    for (const [allianceID, allianceRecord] of Object.entries(allianceRecords)) {
        ...
        const members = new Set((Array.isArray(allianceRecord && allianceRecord.memberCorporationIDs)
    ...
    for (const corporationRecord of Object.values(corporationRecords)) {
        const corporationID = normalizePositiveInteger(corporationRecord && corporationRecord.corporationID, null);
        const allianceID = normalizePositiveInteger(corporationRecord && corporationRecord.allianceID, null);
...
function buildUniqueTicker(name, existingTickers = [], fallback = "CORP") {
    ...
    const used = new Set(existingTickers
...
function buildNpcCorporationRecord(rawRecord) {
    ...
    const corporationName = normalizeLocalizedText(rawRecord.name, `Corporation ${corporationID}`);
    const tickerName = String(rawRecord.tickerName || "").trim() ||
...
function ensureCorporationsInitialized() {
    ...
    const table = normalizeCorporationTable(readTable(CORPORATIONS_TABLE, {
        _meta: {
            nextCustomCorporationID: CUSTOM_CORPORATION_ID_START,
            npcSeedVersion: 0,
        },
    ...
    const shouldRefreshNpcCorporations = table._meta.npcSeedVersion !== NPC_CORPORATION_SEED_VERSION;
    ...
    for (const [corporationID, rawRecord] of loadNpcCorporations().entries()) {
        ...
        if (shouldRefreshNpcCorporations ||
...
function ensureAlliancesInitialized() {
    ...
    const table = normalizeAllianceTable(readTable(ALLIANCES_TABLE, {
        _meta: {
            nextCustomAllianceID: CUSTOM_ALLIANCE_ID_START,
        },
        records: {},
    }));
    const normalizedNextAllianceID = normalizePositiveInteger(table._meta.nextCustomAllianceID, CUSTOM_ALLIANCE_ID_START);
...
function getCharacterIDsInCorporation(corporationID) {
    ...
    return cloneValue(ensureCorporationMembersIndex().get(numericCorporationID) || []);
}
...
function getCorporationRecord(corporationID) {
    ...
    ensureCorporationsInitialized();
    const result = database.read(CORPORATIONS_TABLE, `/records/${String(numericCorporationID)}`);
    const storedRecord = result.success && result.data && typeof result.data === "object"
        ? result.data
        : null;
    if (!storedRecord) {
        return null;
...
function getAllianceRecord(allianceID) {
    ...
    ensureAlliancesInitialized();
    const result = database.read(ALLIANCES_TABLE, `/records/${String(numericAllianceID)}`);
    const storedRecord = result.success && result.data && typeof result.data === "object"
        ? result.data
        : null;
    if (!storedRecord) {
    ...
    record.memberCount = record.memberCorporationIDs.reduce((count, corporationID) => count + getCorporationMemberCount(corporationID), 0);
    return record;
}
...
function getNpcCharacterOwnerRecord(characterID) {
    ...
    return {
        ...
        ownerName: normalizeLocalizedText(rawRecord.name, `Entity ${numericCharacterID}`),
...
function getOwnerLookupRecord(ownerID) {
    return (getCorporationOwnerRecord(ownerID) ||
...
function getCorporationStationSolarSystems(corporationID) {
    ...
    for (const stationRecord of readStations()) {
        const ownerID = normalizePositiveInteger(stationRecord && (stationRecord.corporationID || stationRecord.ownerID), 0);
        if (ownerID !== numericCorporationID) {
            continue;
        }
        const solarSystemID = normalizePositiveInteger(stationRecord && stationRecord.solarSystemID, 0);
        if (!solarSystemID) {
            continue;
        }
        stationCountBySystemID.set(solarSystemID, (stationCountBySystemID.get(solarSystemID) || 0) + 1);
    }
...
function getCorporationPublicInfo(corporationID) {
    ...
    return {
        ...
        creatorID: normalizePositiveInteger(record.creatorID, normalizePositiveInteger(record.ceoID, null)),
        ...
        aggressionEnableAfter: aggressionSettings && aggressionSettings.enableAfter
            ? String(aggressionSettings.enableAfter)
            : null,
        aggressionDisableAfter: aggressionSettings && aggressionSettings.disableAfter
            ? String(aggressionSettings.disableAfter)
            : null,
        isNPC: Boolean(record.isNPC),
        factionID: normalizePositiveInteger(record.factionID, null),
        warFactionID: normalizePositiveInteger(record.warFactionID, normalizePositiveInteger(record.factionID, null)),
...
function findCorporationByName(name) {
    ...
    return (Object.values(table.records)
        ...
        .find((record) => normalizeNameKey(record && record.corporationName) === normalizedName) || null);
}
function findCorporationByTickerName(tickerName) {
    ...
    return (Object.values(table.records)
        .map((record) => getCorporationRecord(record.corporationID))
        .find((record) => String((record && record.tickerName) || "").trim().toUpperCase() ===
...
function findAllianceByName(name) {
    ...
    return (Object.values(table.records)
        ...
        .find((record) => normalizeNameKey(record && record.allianceName) === normalizedName ||
        normalizeNameKey(record && record.shortName) === normalizedName) || null);
}
function setCorporationRecord(record) {
    const corporationID = normalizePositiveInteger(record && record.corporationID, 0) || 0;
    if (!corporationID) {
        return {
            success: false,
    ...
    const table = ensureCorporationsInitialized();
    table.records[String(corporationID)] = normalizeCorporationLogo(cloneValue(record));
...
function setCorporationAlliance(corporationID, allianceID = null) {
    ...
    const normalizedAllianceID = normalizePositiveInteger(allianceID, null);
    const previousAllianceID = normalizePositiveInteger(corporationRecord.allianceID, null);
    const alliances = ensureAlliancesInitialized();
    if (previousAllianceID && alliances.records[String(previousAllianceID)]) {
        const previousAllianceRecord = cloneValue(alliances.records[String(previousAllianceID)]);
        previousAllianceRecord.memberCorporationIDs = (Array.isArray(previousAllianceRecord.memberCorporationIDs)
            ? previousAllianceRecord.memberCorporationIDs
            : []).filter((memberCorporationID) => normalizePositiveInteger(memberCorporationID, 0) !==
    ...
    if (normalizedAllianceID) {
        ...
        const memberCorporationIDs = new Set((Array.isArray(targetAllianceRecord.memberCorporationIDs)
        ...
        targetAllianceRecord.memberCorporationIDs = Array.from(memberCorporationIDs).sort((left, right) => left - right);
    ...
    const affectedCharacterIDs = getCharacterIDsInCorporation(corporationRecord.corporationID);
    for (const characterID of affectedCharacterIDs) {
        const updateResult = setCharacterAffiliation(characterID, corporationRecord.corporationID, normalizedAllianceID);
...
function createCustomCorporation(characterID, name) {
    ...
    const table = ensureCorporationsInitialized();
    const existingTickers = Object.values(table.records).map((record) => record && record.tickerName);
    ...
    table.records[String(corporationID)] = {
        ...
        allianceID: null,
        stationID: normalizePositiveInteger(characterRecord.homeStationID ||
...
function createCustomAllianceForCorporation(characterID, corporationID, name) {
    ...
    const table = ensureAlliancesInitialized();
    const existingTickers = Object.values(table.records).map((record) => record && record.shortName);
    ...
    const affiliationResult = setCorporationAlliance(corporationRecord.corporationID, allianceID);
    if (!affiliationResult.success) {
        return affiliationResult;
    }
    return {
...
function joinCorporationToAllianceByName(corporationID, allianceName) {
    ...
    if (normalizePositiveInteger(corporationRecord.allianceID, 0) ===
    ...
    const affiliationResult = setCorporationAlliance(corporationRecord.corporationID, allianceRecord.allianceID);
    if (!affiliationResult.success) {
        return affiliationResult;
    }
    return {
...
//# sourceMappingURL=corporationState.js.map