...
function buildWarPayload(war) {
  return buildKeyVal([
    ...
    [
      "allies",
      buildDict(
        Object.entries<any>((war && war.allies) || {}).map(([allyID, ally]) => [
          Number(allyID),
          buildAllyPayload({
...
class WarStatisticMgrService extends BaseService {
  ...
  Handle_GetBaseInfo(args, session) {
    ...
    return [
      buildWarPayload(war),
      buildDict(
        Object.entries<any>(stats.shipsKilled || {}).map(([ownerID, count]) => [
          Number(ownerID),
          Number(count || 0),
        ]),
      ),
      buildDict(
        Object.entries<any>(stats.iskKilled || {}).map(([ownerID, amount]) => [
          Number(ownerID),
          Number(amount || 0),
        ]),
      ),
      buildDict(
        Object.entries<any>(war.allies || {}).map(([allyID, ally]) => [
          Number(allyID),
          buildAllyPayload({
            allyID: Number(allyID),
            ...(ally || {}),
  ...
  Handle_GetKillsByGroup(args, session) {
    ...
    return buildDict(
      Object.entries<any>(stats.killsByGroup || {}).map(([groupID, groupStats]) => [
        Number(groupID),
        buildDict([
