...
const { buildFiletimeLong, buildKeyVal, buildList, buildPackedRow, currentFileTime, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getAllianceCorporationIDs, getAllianceRecord, getCorporationInfoRecord, getCorporationRecord, } = require(path.join(__dirname, "./corporationState"));
const { CORP_ROLE_DIRECTOR, getAllianceRuntime, getCorporationDivisionNames, getCorporationMember, getCorporationRuntime, listCorporationMembers, normalizePositiveInteger, toRoleMaskBigInt, } = require(path.join(__dirname, "./corporationRuntimeState"));
const { getWarPermitStatusForOwner, } = require(path.join(__dirname, "./warPermitState"));
const { getAllianceCapitalInfo: getSovereigntyAllianceCapitalInfo, getAlliancePrimeInfo: getSovereigntyAlliancePrimeInfo, } = require(path.join(__dirname, "../sovereignty/sovState"));
...
function buildAllianceSummary(allianceID) {
    ...
    const executorCorporationID = normalizePositiveInteger(allianceRecord.executorCorporationID, null);
    const executorCorporation = executorCorporationID
        ? getCorporationRecord(executorCorporationID)
        : null;
    const hasExplicitAllowWar = Object.prototype.hasOwnProperty.call(allianceRecord, "allowWar") &&
    ...
    return {
        ...
        creatorCorpID: normalizePositiveInteger(allianceRecord.creatorCorpID || allianceRecord.executorCorporationID, null),
        creatorCharID: normalizePositiveInteger(allianceRecord.creatorCharID || allianceRecord.creatorID, null),
        warFactionID: normalizePositiveInteger(allianceRecord.warFactionID, normalizePositiveInteger(executorCorporation && executorCorporation.factionID, null)),
        description: allianceRecord.description || "",
        url: allianceRecord.url || "",
        startDate: buildFiletimeLong(allianceRecord.createdAt || 0n),
        memberCount: getAllianceCorporationIDs(allianceRecord.allianceID).reduce((count, corporationID) => count + listCorporationMembers(corporationID).length, 0),
...
function buildAllianceMemberSnapshot(allianceID, corporationID) {
    ...
    const runtime = getAllianceRuntime(allianceID) || {};
    const joinedAt = runtime.memberJoinedAtByCorporation &&
    ...
    return {
        ...
        allianceID,
        chosenExecutorID: normalizePositiveInteger(runtime.executorSupportByCorporation &&
            runtime.executorSupportByCorporation[String(corporationID)], null),
...
function getSessionCharacterID(session) {
    return normalizePositiveInteger(session && (session.characterID || session.charid), null);
}
function getSessionCorporationID(session) {
    return normalizePositiveInteger(session && (session.corporationID || session.corpid), null);
}
...
function sendNotificationToCharacterIDs(characterIDs, notifyType, idType, payloadTuple = []) {
    const recipients = new Set((Array.isArray(characterIDs) ? characterIDs : [characterIDs])
...
function sendNotificationToCorporationSessions(corporationID, notifyType, idType, payloadTuple = []) {
...
function notifyCorporationRemoved(corporationID, previousSnapshot = null, options = {}) {
    ...
    sendNotificationToAllSessions("OnCorporationChanged", "corpid", payloadTuple);
    ...
    if (clientCharacterIDs.length > 0) {
        sendNotificationToCharacterIDs(clientCharacterIDs, "OnCorporationChanged", "clientID", payloadTuple);
    }
}
function notifyShareChange(idType, shareholderID, corporationID, beforeSnapshot, afterSnapshot, options = {}) {
    ...
    const change = buildChangeDict(beforeSnapshot, afterSnapshot, SHAREHOLDER_CHANGE_HEADER);
    if (change.entries.length <= 0) {
        return;
    }
    const payloadTuple = [
    ...
    if (idType === "charid") {
        sendNotificationToCharacterIDs(options.clientCharacterIDs || [numericShareholderID], "OnShareChange", idType, payloadTuple);
        return;
    }
...
function notifyCorporationLiquidationShareTransfer(corporationID, characterID, shares) {
    ...
    notifyShareChange("*corpid&corprole", numericCorporationID, numericCorporationID, {
        corporationID: numericCorporationID,
        shareholderID: numericCorporationID,
        shares: normalizedShares,
    }, null);
    notifyShareChange("charid", numericCharacterID, numericCorporationID, null, {
        corporationID: numericCorporationID,
...
function notifyCorporationMemberChanged(corporationID, characterID, previousSnapshot = null, options = {}) {
    ...
    const change = buildChangeDict(previousSnapshot, nextSnapshot, options.header || CORPORATION_MEMBER_HEADER);
    if (change.entries.length <= 0) {
        return;
    }
    const payloadTuple = [
    ...
    const idTypes = new Set(Array.isArray(options.idTypes) && options.idTypes.length > 0
        ? options.idTypes
        : ["corpid"]);
    for (const idType of idTypes) {
        if (idType === "clientID") {
            sendNotificationToCharacterIDs(options.clientCharacterIDs || [characterID], "OnCorporationMemberChanged", idType, payloadTuple);
        }
        else if (options.targeted === true && idType === "corpid") {
            sendNotificationToCorporationSessions(corporationID, "OnCorporationMemberChanged", idType, payloadTuple);
        }
        else {
            sendNotificationToAllSessions("OnCorporationMemberChanged", idType, payloadTuple);
        }
    }
...
function notifyCorporationApplicationChanged(corporationID, applicantID, applicationID, applicationRow = null) {
    sendNotificationToCharacterIDs(resolveApplicationChangeRecipientIDs(corporationID, applicantID), "OnCorporationApplicationChanged", "clientID", [
        Number(corporationID),
        Number(applicantID),
...
function notifyAllianceMemberChanged(allianceID, corporationID, previousSnapshot = null) {
    ...
    const change = buildChangeDict(previousSnapshot, nextSnapshot, ALLIANCE_MEMBER_HEADER);
    if (change.entries.length <= 0) {
        return;
    }
...
function notifyAllianceApplicationChanged(allianceID, corporationID, previousSnapshot = null) {
    ...
    const change = buildChangeDict(previousSnapshot, nextSnapshot, ALLIANCE_APPLICATION_HEADER);
    if (change.entries.length <= 0) {
        return;
    }
...
function notifyAllianceRelationshipChanged(allianceID, toID, previousSnapshot = null) {
    ...
    const change = buildChangeDict(previousSnapshot, nextSnapshot, ALLIANCE_RELATIONSHIP_HEADER);
    if (change.entries.length <= 0) {
        return;
    }
...
function notifyWarChanged(previousWar = null, nextWar = null) {
    ...
    const change = buildChangeDict(previousSnapshot, nextSnapshot, null);
    if (change.entries.length <= 0) {
        return;
    }
...
function resolveOfficeNotificationItemID(officeOrID) {
    if (officeOrID && typeof officeOrID === "object") {
        return normalizePositiveInteger(officeOrID.itemID, normalizePositiveInteger(officeOrID.officeID, 0));
    }
...
function notifyOfficeBillRefresh(corporationID) {
    sendNotificationToCorporationSessions(corporationID, "OnBillReceived", "*corpid&corprole", []);
}
...
//# sourceMappingURL=corporationNotifications.js.map