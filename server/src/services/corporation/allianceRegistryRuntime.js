...
const { buildBoundObjectResponse, buildDict, buildFiletimeLong, buildKeyVal, buildList, buildPackedRow, buildRow, buildRowset, currentFileTime, extractDictEntries, extractList, normalizeNumber, resolveBoundNodeId, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getAllianceCorporationIDs, getAllianceOwnerRecord, getAllianceRecord, getCharacterIDsInCorporation, getCorporationRecord, getOwnerLookupRecord, } = require(path.join(__dirname, "./corporationState"));
const { createAllianceWithRuntime, ensureRuntimeInitialized, getAllianceRuntime, normalizeFiletimeString, normalizeInteger, normalizePositiveInteger, normalizeText, setCorporationAlliance, updateAllianceRecord, updateCorporationRuntime, updateAllianceRuntime, } = require(path.join(__dirname, "./corporationRuntimeState"));
const { createWarRecord, handleCorporationJoinedAlliance, handleCorporationLeftAlliance, processDueWarBills, } = require(path.join(__dirname, "./warRuntimeState"));
const { getCorporationWalletBalance, } = require(path.join(__dirname, "./corpWalletState"));
const { buildAllianceApplicationsIndexRowset, buildCorporationAllianceHistoryRowset, buildAllianceMembersRowset, buildAllianceMembersIndexRowset, getAllianceMembersOlderThan, getDaysInAlliance, recordAllianceMemberJoin, recordAllianceMemberLeave, removeAllianceCorporationState, setAllianceExecutorSupportChoice, } = require(path.join(__dirname, "./allianceViewState"));
const { BILL_TYPE_WAR, createBill, listBillsForCreditor, listBillsForDebtor, payBillFromCorporation, } = require(path.join(__dirname, "../account/billRuntimeState"));
const { getWarPermitStatusForOwner, } = require(path.join(__dirname, "./warPermitState"));
...
const { cancelAllianceCapitalTransition: cancelSovereigntyCapitalTransition, getAllianceCapitalInfo: getSovereigntyAllianceCapitalInfo, getAlliancePrimeInfo: getSovereigntyAlliancePrimeInfo, getAllianceSovereigntyRows, setAllianceCapitalSystem: setSovereigntyAllianceCapitalSystem, setAlliancePrimeHour: setSovereigntyAlliancePrimeHour, } = require(path.join(__dirname, "../sovereignty/sovState"));
const { buildAllianceCapitalInfoPayload, buildAlliancePrimeInfoPayload, buildAllianceSovereigntyRowsPayload, } = require(path.join(__dirname, "../sovereignty/sovPayloads"));
const { buildAllianceApplicationSnapshot, buildAllianceMemberSnapshot, buildAllianceRelationshipSnapshot, buildCorporationSnapshot, notifyAllianceApplicationChanged, notifyAllianceChanged, notifyAllianceMemberChanged, notifyAllianceRelationshipChanged, notifyCorporationChanged, } = require(path.join(__dirname, "./corporationNotifications"));
...
function resolveAllianceIDFromArgs(args, session) {
    ...
    return normalizePositiveInteger(session && (session.allianceID || session.allianceid), null);
}
function resolveAllianceIDFromSession(session) {
    return normalizePositiveInteger(session && (session.allianceID || session.allianceid), null);
}
function buildAllianceSummary(allianceRecord) {
    const resolvedAllianceRecord = allianceRecord && typeof allianceRecord === "object"
        ? allianceRecord
    ...
    const sovCapitalInfo = getSovereigntyAllianceCapitalInfo(resolvedAllianceRecord.allianceID);
    const executorCorporationID = normalizePositiveInteger(resolvedAllianceRecord.executorCorporationID, null);
    const executorCorporation = executorCorporationID
        ? getCorporationRecord(executorCorporationID)
        : null;
    const hasExplicitAllowWar = Object.prototype.hasOwnProperty.call(resolvedAllianceRecord, "allowWar") &&
    ...
    return {
        ...
        allianceName: resolvedAllianceRecord.allianceName ||
        ...
        creatorCorpID: normalizePositiveInteger(resolvedAllianceRecord.creatorCorpID || resolvedAllianceRecord.executorCorporationID, null),
        creatorCharID: normalizePositiveInteger(resolvedAllianceRecord.creatorCharID || resolvedAllianceRecord.creatorID, null),
        warFactionID: normalizePositiveInteger(resolvedAllianceRecord.warFactionID, normalizePositiveInteger(executorCorporation && executorCorporation.factionID, null)),
        description: resolvedAllianceRecord.description || "",
        url: typeof resolvedAllianceRecord.url === "string" &&
        ...
        memberCount: getAllianceCorporationIDs(resolvedAllianceRecord.allianceID).reduce((count, corporationID) => count + getCharacterIDsInCorporation(corporationID).length, 0),
...
function buildAllianceKeyValPayload(allianceRecord) {
    ...
    return buildKeyVal(ALLIANCE_ROW_HEADER.map((fieldName) => [fieldName, summary[fieldName]]).concat([
...
class AllianceRegistryRuntimeService extends BaseService {
    ...
    Handle_GetEmploymentRecord(args, session) {
        ...
        const alliance = getAllianceRecord(resolveAllianceIDFromArgs(args, session));
        return buildRowset(["allianceID", "startDate", "deleted"], alliance
    ...
    Handle_UpdateApplication(args, session) {
        ...
        const previousAllianceMember = buildAllianceMemberSnapshot(allianceID, corporationID);
        const previousAlliance = buildAllianceSummary(allianceID);
        ...
        if (state === 2) {
            ...
            const previousAllianceID = normalizePositiveInteger(previousCorporation && previousCorporation.allianceID, null);
    ...
    Handle_GMForceAcceptApplication(args, session) {
        ...
        const application = runtime.applications && runtime.applications[String(corporationID)]
    ...
    Handle_DeclareExecutorSupport(args, session) {
        ...
        const supporterCorporationID = normalizePositiveInteger(session && (session.corporationID || session.corpid), null);
        const previousMember = buildAllianceMemberSnapshot(allianceID, supporterCorporationID);
        setAllianceExecutorSupportChoice(allianceID, supporterCorporationID, chosenExecutorCorporationID);
    ...
    Handle_GetLabels(args, session) {
        const runtime = getAllianceRuntime(resolveAllianceIDFromSession(session)) || {};
        return buildDict(Object.entries(runtime.labels || {}).map(([labelID, label]) => [
        ...
        updateAllianceRuntime(allianceID, (runtime) => {
            for (const contactID of contactIDs) {
                runtime.contacts[String(contactID)] = {
                    ...
                    labelMask: addLabelMask((runtime.contacts[String(contactID)] || {}).labelMask, labelMask),
                };
            }
            return runtime;
        });
    ...
    Handle_GetBulletins(args, session) {
        const runtime = getAllianceRuntime(resolveAllianceIDFromSession(session)) || {};
        return buildList((runtime.bulletins || [])
            .slice()
    ...
    Handle_AddBulletin(args, session) {
        ...
        updateAllianceRuntime(allianceID, (runtime, allianceRecord, table) => {
            ...
            const index = (runtime.bulletins || []).findIndex((entry) => Number(entry.bulletinID) === Number(bulletinID));
            const record = {
                bulletinID,
                ownerID: allianceID,
                createDateTime: index >= 0 ? runtime.bulletins[index].createDateTime : String(currentFileTime()),
    ...
    Handle_GetRelationships(args, session) {
        const runtime = getAllianceRuntime(resolveAllianceIDFromSession(session)) || {};
        return buildDict(Object.entries(runtime.relationships || {}).map(([ownerID, relationship]) => [
            Number(ownerID),
            relationship,
        ]));
    }
    ...
    Handle_GetPrimeTimeInfo(args, session) {
        return buildAlliancePrimeInfoPayload(getSovereigntyAlliancePrimeInfo(resolveAllianceIDFromSession(session)));
    }
    ...
    Handle_GetCapitalSystemInfo(args, session) {
        return buildAllianceCapitalInfoPayload(getSovereigntyAllianceCapitalInfo(resolveAllianceIDFromSession(session)));
    }
    Handle_GetAllianceSovereigntyStructuresInfo(args, session) {
        return buildAllianceSovereigntyRowsPayload(getAllianceSovereigntyRows(resolveAllianceIDFromSession(session)));
    }
    ...
    Handle_GetBills(args, session) {
        ...
        return buildList(listBillsForDebtor(resolveAllianceIDFromSession(session)).map((bill) => buildBillPayload(bill)));
    }
    Handle_GetBillsReceivable(args, session) {
        processDueWarBills({ session });
        return buildList(listBillsForCreditor(resolveAllianceIDFromSession(session)).map((bill) => buildBillPayload(bill)));
    }
    Handle_GetBillBalance(args, session) {
        const corporationID = normalizePositiveInteger(session && (session.corporationID || session.corpid), null);
        const accountKey = normalizePositiveInteger(session && (session.corpAccountKey || session.corpaccountkey), 1000);
    ...
    Handle_PayBill(args, session, kwargs) {
        ...
        const accountKey = normalizePositiveInteger(extractKwargValue(kwargs, "fromAccountKey") ??
            (args && args[1]) ??
            1000, 1000);
        payBillFromCorporation(billID, normalizePositiveInteger(session && (session.corporationID || session.corpid), null), accountKey);
        return null;
    }
...
//# sourceMappingURL=allianceRegistryRuntime.js.map