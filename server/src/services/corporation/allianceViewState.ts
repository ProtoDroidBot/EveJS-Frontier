...
function getRecordedCorporationAllianceHistory(corporationID) {
  ...
  const rows: any[] = [];
  for (const [allianceID, runtime] of Object.entries<any>(runtimeTable.alliances || {})) {
...
function buildAllianceApplicationsIndexRowset(allianceID) {
  ...
  return buildIndexRowset(
    ...
    Object.entries<any>(runtime.applications || {}).map(([corporationID, application]) => [
...
function buildCorporationAllianceApplicationsIndexRowset(corporationID, applications: Record<string, any> = {}) {
  return buildIndexRowset(
    ALLIANCE_APPLICATION_HEADER,
...
function setAllianceExecutorSupportChoice(
  ...
  const memberCorporationIDs = new Set<any>(getAllianceCorporationIDs(numericAllianceID));
