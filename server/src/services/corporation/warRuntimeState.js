...
const { cloneValue, ensureRuntimeInitialized, normalizeBoolean, normalizePositiveInteger, updateRuntimeState, } = require(path.join(__dirname, "./corporationRuntimeState"));
const { notifyWarChanged, } = require(path.join(__dirname, "./corporationNotifications"));
const { getAllianceCorporationIDs, getAllianceRecord, getCharacterIDsInCorporation, getCorporationRecord, } = require(path.join(__dirname, "./corporationState"));
const { BILL_TYPE_WAR, createBill, getBillRecord, listDueBills, markBillProcessed, tryAutoPayBill, } = require(path.join(__dirname, "../account/billRuntimeState"));
...
const warNotificationCenter = require(path.join(__dirname, "./warNotificationCenter"));
...
function normalizeNowFiletime(options = {}) {
    const explicitFiletime = options.nowFiletime !== undefined && options.nowFiletime !== null
        ? options.nowFiletime
        : options.nowFileTime;
...
function getStructureOwnerCorporationID(structure) {
    return normalizePositiveInteger(structure && (structure.ownerCorpID || structure.ownerID), null);
}
function isHighSecurityWarHQ(structure) {
    const worldData = getWorldDataService();
    const system = worldData.getSolarSystemByID(normalizePositiveInteger(structure && structure.solarSystemID, 0));
...
function hasPeaceTreaty(runtimeTable, warID, ownerID, otherOwnerID, reason) {
    return Object.values(runtimeTable.peaceTreaties || {}).some((treaty) => Number(treaty && treaty.warID) === Number(warID) &&
...
function createPeaceTreatyRecord(runtimeTable, { warID, ownerID, otherOwnerID, peaceReason, now, } = {}) {
    ...
    if (hasPeaceTreaty(runtimeTable, numericWarID, numericOwnerID, numericOtherOwnerID, peaceReason)) {
        return;
    }
...
function collectWarOwnerCharacterIDs(ownerID) {
    ...
    if (getAllianceRecord(numericOwnerID)) {
        return getAllianceCorporationIDs(numericOwnerID).flatMap((corporationID) => getCharacterIDsInCorporation(corporationID));
    }
...
function collectWarNotificationRecipients(war) {
    ...
    for (const ownerID of [
        ...
        ...Object.keys((war && war.allies) || {}).map((allyID) => normalizePositiveInteger(allyID, null)),
    ]) {
...
function getStructureWarHQName(structure) {
    return String((structure && (structure.itemName || structure.name)) ||
...
function createWarDeclaredNotifications(war) {
    notifyWarDefenderMembers(war, NOTIFICATION_TYPE.CORP_WAR_DECLARED, NOTIFICATION_TYPE.ALL_WAR_DECLARED, { cost: 0 });
}
...
function createWarSurrenderNotifications(war) {
    notifyWarDefenderMembers(war, NOTIFICATION_TYPE.CORP_WAR_SURRENDER, NOTIFICATION_TYPE.ALL_WAR_SURRENDER);
}
function createWarRecord({ declaredByID, againstID, warHQ = null, mutual = false, reward = 0, createdFromWarID = null, billID = null, openForAllies = false, timeDeclared = null, timeStarted = null, timeFinished = null, declaredByCharacterID = null, } = {}) {
    ...
    const writeResult = updateRuntimeState((runtimeTable) => {
        ...
        const startedAt = timeStarted !== null && timeStarted !== undefined
        ...
        runtimeTable.wars[String(nextWarID)] = {
            ...
            timeFinished: timeFinished !== null && timeFinished !== undefined
                ? String(timeFinished)
                : null,
            retracted: null,
            retractedBy: null,
...
function createWarRetractedNotifications(war) {
    notifyWarDefenderMembers(war, NOTIFICATION_TYPE.CORP_WAR_RETRACTED, NOTIFICATION_TYPE.ALL_WAR_RETRACTED);
}
...
function endWarWithReason(warID, options = {}) {
    ...
    const finishAt = options.finishAtFiletime !== undefined && options.finishAtFiletime !== null
        ? normalizeFiletimeBigInt(options.finishAtFiletime, now)
        : options.cooldown
            ? now + WAR_COOLDOWN
            : now;
    const peaceReason = options.peaceReason === null || options.peaceReason === undefined
        ? null
        : Number(options.peaceReason);
    let endReason = options.endReason === null || options.endReason === undefined
        ? null
    ...
    const writeResult = updateRuntimeState((runtimeTable) => {
        ...
        if (options.retracted) {
            ...
            war.retractedByCharacterID = normalizePositiveInteger(options.retractedByCharacterID, null);
        }
        if (peaceReason !== null) {
            ensurePeaceTreatyRuntime(runtimeTable);
    ...
    if (endReason === ENDED_WAR_HQ_SYSTEM_SECURITY_DROP) {
        warNotificationCenter.notifyWarEndedHqSecurityDrop(changed.nextWar, options.hqRemovedStructure || resolveWarHQStructure(changed.nextWar), { endDate: changed.nextWar.timeFinished });
    }
    ...
    else if (endReason === ENDED_CORP_DELETED ||
...
function processDueWarBills(options = {}) {
    ...
    for (const dueBill of dueBills) {
        ...
        if (!isAggressiveBillableWar(war) ||
...
function expireWarNegotiations(now) {
    ...
    updateRuntimeState((runtimeTable) => {
        ...
        for (const negotiation of Object.values(runtimeTable.warNegotiations)) {
            ...
            const createdDateTime = normalizeFiletimeBigInt(negotiation.createdDateTime, 0n);
    ...
    const writeResult = updateRuntimeState((runtimeTable) => {
        ...
        for (const war of Object.values(runtimeTable.wars)) {
            ...
            for (const [allyID, ally] of Object.entries(war.allies)) {
                const timeFinished = normalizeFiletimeBigInt(ally && ally.timeFinished, 0n);
...
function createLeavingCorporationPeaceTreaties(corporationID, allianceID, now) {
    ...
    updateRuntimeState((runtimeTable) => {
        ...
        for (const war of Object.values(runtimeTable.wars)) {
            ...
            createdCount += Math.max(0, Object.keys(runtimeTable.peaceTreaties || {}).length - beforeCount);
        }
        return runtimeTable;
    });
    return createdCount;
}
function handleCorporationLeftAlliance({ corporationID, allianceID, nowFiletime = currentFileTime(), } = {}) {
    ...
    for (const war of listAllWars()) {
        ...
        if (!isWarFinishedOrFinishing(war) &&
        ...
        if (createdFromWarID &&
            ...
            if (endWarWithReason(war.warID, {
                ...
                warNotificationCenter.notifyWarInherited(war, {
                    allianceID: numericAllianceID,
                    quitterID: numericCorporationID,
                    opponentID: Number(war.declaredByID) === numericCorporationID
    ...
    return {
        endedCount,
        inheritedAllyCount,
        treatyCount: createLeavingCorporationPeaceTreaties(numericCorporationID, numericAllianceID, now),
    };
}
function handleCorporationJoinedAlliance({ corporationID, allianceID, nowFiletime = currentFileTime(), } = {}) {
    const now = normalizeFiletimeBigInt(nowFiletime, currentFileTime());
    const numericCorporationID = normalizePositiveInteger(corporationID, null);
    const numericAllianceID = normalizePositiveInteger(allianceID, null);
    if (!numericCorporationID || !numericAllianceID) {
    ...
    const hasLiveAllianceWar = listWarsForOwner(numericAllianceID).some((war) => war && !isWarFinishedAt(war, now));
    let notified = false;
    ...
    for (const war of listAllWars()) {
        ...
        const principalWar = Number(war.declaredByID) === numericCorporationID ||
            Number(war.againstID) === numericCorporationID;
        const allyWar = isAllyActiveAt(war.allies && war.allies[String(numericCorporationID)], now);
...
function resolveCharacterWarEntityIDs(characterID) {
    ...
    const character = characterState && typeof characterState.getCharacterRecord === "function"
        ? characterState.getCharacterRecord(characterID)
        : null;
    const entityIDs = new Set();
    const corporationID = normalizePositiveInteger(character && character.corporationID, null);
    const allianceID = normalizePositiveInteger(character && character.allianceID, null);
    if (corporationID) {
...
function findActiveWarBetweenWarEntities(leftEntityIDs, rightEntityIDs, options = {}) {
    ...
    for (const war of listAllWars()) {
        ...
        if ((leftIsAggressor && rightIsDefender) ||
        ...
        if ((leftIsAggressor && rightIsAlly) ||
...
function findActiveWarBetweenCharacters(attackerCharacterID, targetCharacterID, options = {}) {
    ...
    return findActiveWarBetweenWarEntities(resolveCharacterWarEntityIDs(attackerID), resolveCharacterWarEntityIDs(targetID), options);
}
function areCharactersLegalWarOpponents(attackerCharacterID, targetCharacterID, options = {}) {
    return Boolean(findActiveWarBetweenCharacters(attackerCharacterID, targetCharacterID, options));
}
module.exports = {
...
//# sourceMappingURL=warRuntimeState.js.map