...
function applyCharacterRecordToSession(session, characterID, options: Record<string, any> = {}) {
...
function normalizeValueForCompare(value) {
  ...
  if (value && typeof value === "object") {
    return Object.fromEntries(
      Object.entries<any>(value).map(([key, entryValue]) => [
        key,
...
function buildAllianceMemberSnapshot(allianceID, corporationID) {
  ...
  const memberCorporationIDs = new Set<any>(getAllianceCorporationIDs(allianceID));
...
function buildWarSnapshot(war = null) {
  ...
  return {
    ...
    allies: {
      type: "dict",
      entries: Object.entries<any>(war.allies || {}).map(([allyID, ally]) => [
        Number(allyID),
        buildKeyVal([
...
function sendNotificationToAllSessions(notifyType, idType, payloadTuple: any[] = []) {
...
function resolveApplicationChangeRecipientIDs(corporationID, applicantID) {
  const recipientIDs = new Set<any>();
...
function refreshCharacterSession(characterID, options: Record<string, any> = {}) {
...
function notifyCorporationRemoved(corporationID, previousSnapshot = null, options: Record<string, any> = {}) {
  const numericCorporationID = normalizePositiveInteger(corporationID, null);
  if (!numericCorporationID) {
    return;
  }
...
function notifyShareChange(idType, shareholderID, corporationID, beforeSnapshot, afterSnapshot, options: Record<string, any> = {}) {
