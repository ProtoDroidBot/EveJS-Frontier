...
const { buildBoundObjectResponse, buildDict, buildFiletimeLong, buildKeyVal, buildList, normalizeNumber, normalizeText, resolveBoundNodeId, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getWarRecord, } = require(path.join(__dirname, "./warRuntimeState"));
const { buildKillmailPayload, buildWarDestructionStatistics, getKillmailHashValue, getKillmailRecord, listKillmailsForWar, } = require(path.join(__dirname, "../killmail/killmailState"));
...
function buildWarPayload(war) {
    return buildKeyVal([
        ...
        [
            "allies",
            buildDict(Object.entries((war && war.allies) || {}).map(([allyID, ally]) => [
                Number(allyID),
                buildAllyPayload({
...
function resolveBoundWarID(args, session) {
    return normalizeWarID((args && args[0]) || (session && session._boundWarStatisticID));
}
...
class WarStatisticMgrService extends BaseService {
    ...
    Handle_GetBaseInfo(args, session) {
        ...
        return [
            buildWarPayload(war),
            buildDict(Object.entries(stats.shipsKilled || {}).map(([ownerID, count]) => [
                Number(ownerID),
                Number(count || 0),
            ])),
            buildDict(Object.entries(stats.iskKilled || {}).map(([ownerID, amount]) => [
                Number(ownerID),
                Number(amount || 0),
            ])),
    ...
    Handle_GetKills(args, session) {
        ...
        const groupID = args && args.length > 1 && args[1] !== null && args[1] !== undefined
            ? Number(args[1])
            : null;
        return buildList(listKillmailsForWar(warID, {
            entityID,
            groupID,
    ...
    Handle_GetKillsByGroup(args, session) {
        ...
        return buildDict(Object.entries(stats.killsByGroup || {}).map(([groupID, groupStats]) => [
            Number(groupID),
            buildDict([
...
//# sourceMappingURL=warStatisticMgrService.js.map