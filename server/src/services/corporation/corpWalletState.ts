...
function buildLedgerEntry(corporationID, accountKey, amount, balance, options: Record<string, any> = {}) {
...
function normalizeMarketTransactionEntry(entry: Record<string, any> = {}, accountKey = CORPORATION_WALLET_KEY_START) {
...
function filterLedgerEntries(entries: any[] = [], options: Record<string, any> = {}) {
...
function setCorporationWalletDivisionBalance(corporationID, accountKey, nextBalance, options: Record<string, any> = {}) {
...
function adjustCorporationWalletDivisionBalance(corporationID, accountKey, delta, options: Record<string, any> = {}) {
  const normalizedCorporationID = Number(corporationID) || 0;
  const normalizedAccountKey = normalizeCorporationWalletKey(accountKey);
...
function getCorporationWalletJournal(corporationID, options: Record<string, any> = {}) {
...
function getCorporationWalletTransactions(corporationID, options: Record<string, any> = {}) {
  const division = getCorporationWalletDivision(
    corporationID,
    options.accountKey || CORPORATION_WALLET_KEY_START,
  );
...
function getCorporationMarketTransactions(corporationID, options: Record<string, any> = {}) {
  ...
  if (hasAccountKey) {
    ...
    if (wallet && wallet.divisions && typeof wallet.divisions === "object") {
      divisions.push(...Object.values<any>(wallet.divisions));
    }
  }
...
function appendCorporationMarketTransaction(corporationID, accountKey, entry: Record<string, any> = {}) {
  const normalizedCorporationID = Number(corporationID) || 0;
  const normalizedAccountKey = normalizeCorporationWalletKey(accountKey);
