...
const { buildFiletimeLong, buildKeyVal, buildList, currentFileTime, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { createWarRecord, processWarLifecycle, } = require(path.join(__dirname, "./warRuntimeState"));
const { ensureRuntimeInitialized, updateRuntimeState, } = require(path.join(__dirname, "./corporationRuntimeState"));
const { isMutualWarInviteBlocked, setMutualWarInviteBlocked, } = require(path.join(__dirname, "./warNegotiationRuntimeState"));
const { notifyMadeWarMutual, notifyMutualWarInviteAccepted, notifyMutualWarInviteRejected, notifyMutualWarInviteSent, } = require(path.join(__dirname, "./warNotificationCenter"));
...
class MutualWarInviteManagerService extends BaseService {
    ...
    Handle_GetPendingInvitesForSession(args, session) {
        ...
        return buildList(Object.values(runtime.mutualWarInvites || {})
            .filter((invite) => Number(invite.fromOwnerID) === Number(ownerID) ||
...
//# sourceMappingURL=mutualWarInviteMgrService.js.map