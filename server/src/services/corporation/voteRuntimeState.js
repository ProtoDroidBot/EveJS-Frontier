...
const { cloneValue, ensureCharacterMemberState, getCorporationMember, getCorporationRuntime, normalizeInteger, normalizePositiveInteger, normalizeText, syncMemberStateToCharacterRecord, updateCorporationRecord, updateCorporationRuntime, } = require(path.join(__dirname, "./corporationRuntimeState"));
const { NPC_STARTER_CORPORATION_ID, getCorporationRecord, setCharacterAffiliation, } = require(path.join(__dirname, "./corporationState"));
const { findItemById, } = require(path.join(__dirname, "../inventory/itemStore"));
const { buildCorporationMemberSnapshot, buildCorporationSnapshot, notifyCorporationChanged, notifyCorporationMemberChanged, notifyLockedItemChange, } = require(path.join(__dirname, "./corporationNotifications"));
...
function normalizeVoteTable(table) {
    const nextTable = table && typeof table === "object" ? cloneValue(table) : buildDefaultVoteTable();
    nextTable._meta =
...
function readTable() {
    ...
    const nextTable = result.success && result.data && typeof result.data === "object"
...
function updateVoteTable(updater) {
    ...
    const nextTable = typeof updater === "function" ? updater(table) || table : table;
...
function addFiletimeDays(filetimeValue, days) {
    return (BigInt(String(filetimeValue || currentFileTime())) +
        BigInt(normalizeDurationDays(days)) * FILETIME_TICKS_PER_DAY).toString();
}
...
function normalizeVoteCaseRow(voteCase) {
    ...
    return {
        ...
        createdByCharacterID: normalizePositiveInteger(voteCase.createdByCharacterID, 0) || 0,
    };
}
...
function getVoteCase(corporationID, voteCaseID) {
    ...
    return normalizeVoteCaseRow(state.voteCases[String(normalizePositiveInteger(voteCaseID, 0) || 0)]);
}
function updateVoteCase(corporationID, voteCaseID, updater) {
    const numericCorporationID = normalizePositiveInteger(corporationID, null) || null;
...
function listVoteCaseOptions(corporationID, voteCaseID) {
    ...
    const votesByCharacter = state.votesByVoteCaseID[String(voteCase.voteCaseID)] || {};
    ...
    return (state.optionsByVoteCaseID[String(voteCase.voteCaseID)] || [])
        ...
        votesFor: normalizeInteger(voteWeightsByOptionID.get(normalizeInteger(option && option.optionID, 0)), 0) || 0,
    }))
...
function resolveVoteWeight(session, corporationID) {
    ...
    const characterID = normalizePositiveInteger(session && (session.characterID || session.charid), 0) || 0;
    const callerCorporationID = normalizePositiveInteger(session && (session.corporationID || session.corpid), 0) || 0;
    const corporationRecord = getCorporationRecord(corporationID);
    let voteWeight = normalizeInteger(runtime.shares && runtime.shares[String(characterID)], 0);
    ...
    if (callerCorporationID && callerCorporationID !== corporationID) {
        ...
        if (callerCorporation &&
            normalizePositiveInteger(callerCorporation.ceoID, 0) === characterID) {
            voteWeight += normalizeInteger(runtime.shares && runtime.shares[String(callerCorporationID)], 0);
        }
    }
    return voteWeight;
}
...
function ensureLockedItemRecord(corporationID, locationID, itemID, typeID) {
    ...
    updateCorporationRuntime(corporationID, (runtime) => {
        ...
        const nextLocationState = currentLocationState && typeof currentLocationState === "object" && !Array.isArray(currentLocationState)
...
function removeLockedItemRecord(corporationID, locationID, itemID) {
    ...
    updateCorporationRuntime(corporationID, (runtime) => {
        ...
        if (Array.isArray(currentLocationState)) {
            ...
            runtime.lockedItemsByLocation[locationKey] = currentLocationState.filter((entry) => Number(entry && entry.itemID ? entry.itemID : entry) !== Number(itemID));
...
function createVoteCase(corporationID, creatorCharacterID, voteCaseText, description, voteType, options, durationDays) {
    ...
    updateVoteTable((table) => {
        ...
        corporationState.voteCases[String(createdVoteCaseID)] = {
            ...
            createdByCharacterID: normalizePositiveInteger(creatorCharacterID, 0) || 0,
        };
        corporationState.optionsByVoteCaseID[String(createdVoteCaseID)] = normalizeOptionRows(normalizeInteger(voteType, voteGeneral), Array.isArray(options) ? options : []);
    ...
    if (createdVoteCase &&
        ...
        ensureLockedItemRecord(corporationID, winningOption.parameter2, winningOption.parameter, winningOption.parameter1);
    }
...
function createOrUpdateActionState(corporationID, voteCaseID, updater) {
    updateVoteTable((table) => {
        const corporationState = ensureCorporationVoteState(table, corporationID);
        const currentState = corporationState.actionStatesByVoteCaseID[String(voteCaseID)] || {};
...
function applyAutomaticVoteEffects(voteCase) {
    ...
    const existingActionState = (existingVoteState.actionStatesByVoteCaseID || {})[String(voteCase.voteCaseID)] || {};
    ...
    if (voteCase.voteType === voteCEO && winningOption.parameter) {
        ...
        const previousCandidateMember = buildCorporationMemberSnapshot(voteCase.corporationID, winningOption.parameter);
        const previousCurrentCeoMember = buildCorporationMemberSnapshot(voteCase.corporationID, corporationRecord && corporationRecord.ceoID);
        if (corporationRecord &&
            ...
            ensureCharacterMemberState(voteCase.corporationID, normalizePositiveInteger(winningOption.parameter, 0) || 0);
            ensureCharacterMemberState(voteCase.corporationID, normalizePositiveInteger(corporationRecord.ceoID, 0) || 0);
            syncMemberStateToCharacterRecord(voteCase.corporationID, normalizePositiveInteger(winningOption.parameter, 0) || 0);
            syncMemberStateToCharacterRecord(voteCase.corporationID, normalizePositiveInteger(corporationRecord.ceoID, 0) || 0);
            ...
            require(path.join(__dirname, "./corpLifecycleNotifications")).notifyCorporationCeoRevoked(voteCase.corporationID, corporationRecord && corporationRecord.ceoID);
            if (previousCurrentCeoMember) {
                notifyCorporationMemberChanged(voteCase.corporationID, previousCurrentCeoMember.characterID, previousCurrentCeoMember);
            }
            notifyCorporationMemberChanged(voteCase.corporationID, normalizePositiveInteger(winningOption.parameter, 0) || 0, previousCandidateMember);
        }
    ...
    if (voteCase.voteType === voteItemLockdown && winningOption.parameter) {
        ...
        ensureLockedItemRecord(voteCase.corporationID, winningOption.parameter2, winningOption.parameter, winningOption.parameter1);
        createOrUpdateActionState(voteCase.corporationID, voteCase.voteCaseID, (actionState) => ({
            ...actionState,
            actedUpon: 1,
            inEffect: 1,
...
function buildActionState(voteCase) {
    ...
    const storedState = state.actionStatesByVoteCaseID[String(voteCase.voteCaseID)] || {};
    return {
        voteCaseID: voteCase.voteCaseID,
        voteType: voteCase.voteType,
        optionID: normalizeInteger(storedState.optionID !== undefined ? storedState.optionID : winningOption.optionID, winningOption.optionID),
        parameter: normalizePositiveInteger(storedState.parameter !== undefined ? storedState.parameter : winningOption.parameter, 0) || 0,
        parameter1: normalizePositiveInteger(storedState.parameter1 !== undefined ? storedState.parameter1 : winningOption.parameter1, 0) || 0,
        parameter2: normalizePositiveInteger(storedState.parameter2 !== undefined ? storedState.parameter2 : winningOption.parameter2, 0) || 0,
...
function activateSanctionedAction(corporationID, voteCaseID) {
    ...
    if (voteCase.voteType === voteShares && parameter > 0) {
        const runtime = getCorporationRuntime(corporationID) || {};
        const currentCorpShares = normalizeInteger(runtime.shares && runtime.shares[String(corporationID)], 0) || 0;
        const corporationRecord = getCorporationRecord(corporationID);
        ...
        moveCharacterToCorporation(parameter, corporationID, resolveFallbackCorporationID(parameter));
    }
    ...
    if (voteCase.voteType === voteItemUnlock && parameter > 0) {
        ...
        for (const otherVoteCase of allVoteCases) {
            ...
            if (otherAction &&
                ...
                removeLockedItemRecord(corporationID, normalizePositiveInteger(otherAction.parameter2, 0) || actionState.parameter2, parameter);
            }
        }
    }
    return { success: true };
...
//# sourceMappingURL=voteRuntimeState.js.map