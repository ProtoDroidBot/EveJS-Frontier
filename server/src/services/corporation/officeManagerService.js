...
const { buildBoundObjectResponse, buildDbRowset, buildFiletimeLong, buildPythonSet, currentFileTime, resolveBoundNodeId, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { CORP_ROLE_CAN_RENT_OFFICE, CORP_ROLE_DIRECTOR, getCorporationRuntime, getCorporationOffices, getOfficesAtStation, normalizePositiveInteger, toRoleMaskBigInt, updateRuntimeState, } = require(path.join(__dirname, "./corporationRuntimeState"));
const { notifyOfficeBillRefresh, notifyOfficeRentItemChange, notifyOfficeRentalChange, notifyOfficeUnrentItemChange, } = require(path.join(__dirname, "./corporationNotifications"));
const { CORPORATION_WALLET_KEY_START, adjustCorporationWalletDivisionBalance, } = require(path.join(__dirname, "./corpWalletState"));
const { getCorporationRecord, } = require(path.join(__dirname, "./corporationState"));
const { getStationRecord, } = require(path.join(__dirname, "../_shared/stationStaticData"));
const { listOwnedItems, } = require(path.join(__dirname, "../inventory/itemStore"));
const { throwWrappedUserError } = require(path.join(__dirname, "../../common/machoErrors"));
const { JOURNAL_ENTRY_TYPE, buildNotEnoughMoneyUserErrorValues, } = require(path.join(__dirname, "../account/walletState"));
...
const assetSafetyState = require(path.join(__dirname, "../structure/structureAssetSafetyState"));
const { addOfficeRentalPeriod, cancelOfficeRentalBillsForOffice, createNextOfficeRentalBill, } = require(path.join(__dirname, "./officeRentalBilling"));
...
function resolveSessionLocationID(session) {
    ...
    return normalizePositiveInteger(session.structureID ||
        session.structureid ||
        session.stationID ||
        session.stationid ||
...
function resolveBoundStationID(session, serviceManager) {
    ...
    const boundObjectID = typeof session.currentBoundObjectID === "string" ? session.currentBoundObjectID : "";
    if (!boundObjectID) {
        return null;
    }
    ...
    if (boundParams && typeof boundParams === "object") {
        return normalizePositiveInteger(boundParams.stationID ||
...
function resolveStationID(args, session, serviceManager = null) {
    return (resolveBoundStationID(session, serviceManager) ||
...
function getStructureDisplayName(structure, fallbackID) {
    return ((structure && (structure.itemName || structure.name)) ||
        `Structure ${normalizePositiveInteger(structure && structure.structureID, fallbackID)}`);
}
...
function resolveOfficeLocationContext(args, session, serviceManager = null) {
    ...
    const structure = station && station.isStructure
...
function getOfficeRentalCost(args, session, serviceManager = null) {
    ...
    return normalizeStructureOfficeRentalCost(getProfileSettingValueForStructure(structure, STRUCTURE_SETTING_ID.CORP_RENT_OFFICE, {
        session,
        defaultValue: 0,
    }));
}
...
function getStructureOwnerCorporationID(structure) {
    return normalizePositiveInteger(structure && (structure.ownerCorpID || structure.ownerID), 0);
}
function getStationOwnerCorporationID(stationID) {
    const station = getStationRecord(null, stationID);
    return normalizePositiveInteger(station && (station.corporationID || station.ownerID), 0);
}
...
function throwNotEnoughOfficeRentMoney(amount, balance) {
    throwWrappedUserError("NotEnoughMoney", buildNotEnoughMoneyUserErrorValues(amount, balance));
}
function chargeOfficeRental(corporationID, structure, stationID, rentalCost) {
    ...
    const debitResult = adjustCorporationWalletDivisionBalance(corporationID, CORPORATION_WALLET_KEY_START, -amount, {
    ...
    if (!debitResult || !debitResult.success) {
        const balance = debitResult && debitResult.data && debitResult.data.balance !== undefined
    ...
    if (ownerCorporationID && getCorporationRecord(ownerCorporationID)) {
        adjustCorporationWalletDivisionBalance(ownerCorporationID, CORPORATION_WALLET_KEY_START, amount, {
            entryTypeID: JOURNAL_ENTRY_TYPE.OFFICE_RENTAL_FEE,
            ownerID1: corporationID,
...
function resolveOfficeStartDate(office) {
    const explicitStartDate = office && (office.startDate || office.rentalStartDate || office.leaseStartDate);
...
function resolveClientOfficeID(office) {
    return normalizePositiveInteger(office && office.itemID, normalizePositiveInteger(office && office.officeID, 0));
}
function buildOfficeRowset(offices) {
    return buildDbRowset(OFFICE_ROWSET_COLUMNS, offices.map((office) => {
        const startDate = resolveOfficeStartDate(office);
        const dueDate = resolveOfficeDueDate(office, startDate);
        return {
            leaseID: normalizePositiveInteger(office && office.leaseID, normalizePositiveInteger(office && office.officeID, 0)),
            ...
            billID: normalizePositiveInteger(office && office.billID, null),
            dueDate: buildFiletimeLong(dueDate),
            officeID: resolveClientOfficeID(office),
            stationTypeID: normalizePositiveInteger(office && (office.stationTypeID || office.typeID), 0),
...
function chargeImpoundRelease(corporationID, stationID, amount) {
    ...
    const debitResult = adjustCorporationWalletDivisionBalance(corporationID, CORPORATION_WALLET_KEY_START, -releaseCost, {
...
function createOfficeRecord(runtimeTable, corporationID, stationID, options = {}) {
    ...
    const nextOfficeFolderID = normalizePositiveInteger(runtimeTable._meta.nextOfficeFolderID, 1) || 1;
    const nextOfficeItemID = normalizePositiveInteger(runtimeTable._meta.nextOfficeItemID, 1) || 1;
    ...
    return {
        ...
        stationTypeID,
        rentalCost: Number(Object.prototype.hasOwnProperty.call(options, "rentalCost")
            ? options.rentalCost
...
class OfficeManagerService extends BaseService {
    ...
    Handle_RentOffice(args, session) {
        ...
        const existingOfficeAtLocation = Object.values(corporationRuntime.offices || {}).find((office) => Number(office.stationID) === Number(stationID) && isActiveOffice(office));
        ...
        updateRuntimeState((runtimeTable) => {
            ...
            const existingOffice = Object.values(corporationRuntime.offices || {}).find((office) => Number(office.stationID) === Number(stationID) && isActiveOffice(office));
            if (!existingOffice) {
        ...
        if (changedOffice) {
            const nextBill = createNextOfficeRentalBill(corporationID, stationID, rentalCost, { baseFileTime: startDate });
            if (nextBill) {
                updateRuntimeState((runtimeTable) => {
                    const corporationRuntime = runtimeTable.corporations[String(corporationID)];
                    const office = corporationRuntime &&
        ...
        updateRuntimeState((runtimeTable) => {
            ...
            for (const [officeID, office] of Object.entries(corporationRuntime.offices)) {
                if (Number(office.stationID) === Number(stationID) && isActiveOffice(office)) {
                    ...
                    if (structure) {
                        const safetyResult = assetSafetyState.moveCorporationOfficeAssetsToSafety(corporationID, structure, office, { session });
                        if (!safetyResult.success) {
                            log.warn(`[OfficeManager] Structure office asset-safety handoff failed corporation=${corporationID} office=${officeID}: ${safetyResult.errorMsg}`);
                            continue;
                        }
                    }
    ...
    Handle_HasCorpImpoundedItems(args, session) {
        ...
        return offices.some((office) => Number(office.stationID) === Number(stationID) && Boolean(office.impounded));
    }
    Handle_GetImpoundReleasePrice(args, session) {
        const corporationID = resolveCorporationID(session);
        const stationID = resolveStationID(args, session, this.serviceManager);
        const office = getCorporationOffices(corporationID).find((entry) => Number(entry.stationID) === Number(stationID) && Boolean(entry.impounded));
        return office ? getImpoundReleasePriceForStation(stationID) : 0;
    }
    Handle_GetItemsFromImpound(args, session) {
        const corporationID = resolveCorporationID(session);
        const stationID = resolveBoundOrSessionStationID(session, this.serviceManager) ||
            normalizePositiveInteger(args && args[1], null) ||
            normalizePositiveInteger(args && args[0], null);
        const releasePrice = getImpoundReleasePriceForStation(stationID);
        const hasImpoundedOffice = getCorporationOffices(corporationID).some((office) => Number(office.stationID) === Number(stationID) && Boolean(office.impounded));
...
//# sourceMappingURL=officeManagerService.js.map