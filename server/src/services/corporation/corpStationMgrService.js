...
const { buildBoundObjectResponse, buildKeyVal, buildList, buildRowset, resolveBoundNodeId, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getStationRecord, getStationServiceIdentifiers, getStationServiceStates, getStationServiceAccessRule, getStationManagementServiceCostModifiers, getRentableItems, } = require(path.join(__dirname, "../_shared/stationStaticData"));
const { getCharacterRecord, } = require(path.join(__dirname, "../character/characterState"));
const { getCharacterEffectiveStanding, } = require(path.join(__dirname, "../character/standingRuntime"));
const { getCorporationOffices, getOfficesAtStation, normalizePositiveInteger, updateCorporationRecord, } = require(path.join(__dirname, "./corporationRuntimeState"));
...
function resolveStationID(args, session) {
    return normalizePositiveInteger((args && args[0]) ||
        (session &&
            (session.stationID ||
                session.stationid ||
                session.structureID ||
...
class CorpStationMgrService extends BaseService {
    ...
    Handle_GetStationServiceIdentifiers() {
        ...
        return buildRowset(["serviceID", "serviceName", "serviceNameID"], getStationServiceIdentifiers().map((service) => buildList([
            service.serviceID,
            service.serviceName,
    ...
    Handle_GetStationServiceAccessRule(args, session) {
        const serviceID = args && args.length > 1 ? args[1] : args && args.length > 0 ? args[0] : 0;
    ...
    Handle_GetStationManagementServiceCostModifiers() {
        ...
        ], getStationManagementServiceCostModifiers().map((row) => buildList([
            row.serviceID,
    ...
    Handle_GetRentableItems(args, session) {
        ...
        return buildRowset(["stationID", "typeID", "rentedToID", "publiclyAvailable"], getRentableItems(session).map((row) => buildList([
            row.stationID,
            row.typeID,
            row.rentedToID,
    ...
    Handle_DoStandingCheckForStationService(args, session) {
        ...
        const characterID = normalizePositiveInteger((args && args.length > 1 ? args[1] : null) ||
            (session && (session.characterID || session.charid)), 0);
        const resolvedCharacterID = characterID || 0;
        if (resolvedCharacterID) {
            const effectiveStanding = getCharacterEffectiveStanding(resolvedCharacterID, ownerID).standing;
            ...
            const securityStatus = Number(character && (character.securityStatus ?? character.securityRating ?? 0));
    ...
    Handle_GetCorporateStationOffice(args, session) {
        ...
        const stationID = resolveStationID(args, session);
        const office = getCorporationOffices(corporationID).find((entry) => Number(entry.stationID) === Number(stationID));
        return buildRowset(["corporationID", "itemID", "officeFolderID"], office
            ? [
                buildList([
    ...
    Handle_GetStationOffices(args, session) {
        ...
        return buildRowset(["corporationID", "itemID", "officeFolderID"], listStationOffices(resolveStationID(args, session)).map((office) => buildList([office.corporationID, office.itemID, office.officeFolderID])), "eve.common.script.sys.rowset.Rowset");
    }
    ...
    Handle_GetOwnerIDsOfClonesAtStation() {
        ...
        return buildRowset(["ownerID", "corporationID"], [], "eve.common.script.sys.rowset.Rowset");
    }
...
//# sourceMappingURL=corpStationMgrService.js.map