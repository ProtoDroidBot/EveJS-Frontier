...
const { buildFiletimeLong, buildKeyVal, buildList, buildRow, buildRowset, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { buildCachedMethodCallResultAsync: buildCachedMethodCallResult, } = require(path.join(__dirname, "../cache/objectCacheRuntime"));
const { NPC_STARTER_CORPORATION_ID, getCorporationInfoRecord, } = require(path.join(__dirname, "./corporationState"));
const { getCorporationRuntime, ensureRuntimeInitialized, normalizePositiveInteger, updateCorporationRuntime, } = require(path.join(__dirname, "./corporationRuntimeState"));
...
function buildCorporationInfo(session, charData) {
    ...
    const info = getCorporationInfoRecord(corpId) || {
        ...
        allianceID: charData.allianceID || (session ? session.allianceID || session.allianceid : null),
        memberCount: 1,
        shares: 1000,
        deleted: 0,
        stationID: null,
...
function buildMedalGraphicsRowset(medalGraphics = []) {
    return buildRowset([
        ...
        "graphic",
        "color",
    ], medalGraphics.map((entry) => buildList([
        Number(entry.medalID || 0),
        Number(entry.part || 0),
...
function buildMedalRecipientsRowset(recipients = []) {
    return buildRowset(["medalID", "characterID", "issuerID", "reason", "date"], recipients.map((entry) => buildList([
        Number(entry.medalID || 0),
...
function collectRuntimeMedalState() {
    ...
    for (const [corporationID, runtime] of Object.entries(runtimeTable.corporations || {})) {
        ...
        for (const [medalID, recipients] of Object.entries(medalState.recipientsByMedalID || {})) {
            recipientsByMedalID.set(Number(medalID || 0), Array.isArray(recipients) ? recipients : []);
        }
    }
...
function buildMedalDetailsPayload(medalID) {
    ...
    const payload = buildKeyVal([
        ...
        [
            "graphics",
            buildMedalGraphicsRowset((Array.isArray(medal.graphics) ? medal.graphics : []).map((graphic) => ({
                medalID: Number(medal.medalID || 0),
                part: graphic.part || 0,
...
function getReceivedMedalsForCharacter(characterID) {
    ...
    for (const [medalID, recipients] of recipientsByMedalID.entries()) {
        ...
        const matchingRecipients = recipients.filter((recipient) => Number(recipient && recipient.characterID) === numericCharacterID);
...
class CorpService extends BaseService {
    ...
    Handle_GetEmploymentRecord(args, session) {
        ...
        const history = Array.isArray(charData.employmentHistory)
            ...
            : [
                {
                    corporationID: charData.corporationID || (session ? session.corporationID : NPC_STARTER_CORPORATION_ID),
        ...
        const sortedHistory = history
            .slice()
            .sort((left, right) => String(right && right.startDate ? right.startDate : "").localeCompare(String(left && left.startDate ? left.startDate : "")));
        return buildRowset(["corporationID", "startDate", "deleted"], sortedHistory.map((entry) => buildList([
    ...
    Handle_CreateMedal(args, session) {
        const corporationID = (session && (session.corporationID || session.corpid)) || 0;
        let medalID = null;
    ...
    Handle_SetMedalStatus(args, session) {
        const corporationID = (session && (session.corporationID || session.corpid)) || 0;
        const statusMap = args && args[0] && typeof args[0] === "object" ? args[0] : {};
    ...
    Handle_GetAllCorpMedals(args) {
        ...
        return buildCachedMethodCallResult([buildCorpMedalRowset(medals), buildMedalGraphicsRowset(graphics)], {
    ...
    Handle_GetRecipientsOfMedal(args, session) {
        ...
        const runtime = getCorporationRuntime(corporationID) || {};
        return buildMedalRecipientsRowset(((runtime.medals && runtime.medals.recipientsByMedalID) || {})[String(medalID)] || []);
    }
...
//# sourceMappingURL=corpService.js.map