...
const { buildBoundObjectResponse, buildList, buildRow, currentFileTime, resolveBoundNodeId, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { ensureRuntimeInitialized, getCorporationRuntime, listCorporationMembers, normalizeInteger, normalizeText, updateCorporationRuntime, } = require(path.join(__dirname, "./corporationRuntimeState"));
const { getCorporationRecord, } = require(path.join(__dirname, "./corporationState"));
const { resolveFriendlyFireLegalAtTime, } = require(path.join(__dirname, "./aggressionSettingsState"));
const { notifyCorporationRecruitmentAdChanged, } = require(path.join(__dirname, "./corporationNotifications"));
const AD_HEADER = [
    "adID",
    "corporationID",
    "allianceID",
...
function normalizeAdvertRecord(advert, corporationRecord) {
    const corporationID = Number((advert && advert.corporationID) ||
        (corporationRecord && corporationRecord.corporationID) ||
        0);
    return {
        ...(advert || {}),
        corporationID,
        allianceID: (corporationRecord && corporationRecord.allianceID) ||
        ...
        createDateTime: String((advert && advert.createDateTime) || currentFileTime().toString()),
        expiryDateTime: String((advert && advert.expiryDateTime) || currentFileTime().toString()),
...
function matchesSearchCriteria(advert, corporationRecord, filters = {}, corporationRuntime = null) {
    const normalized = normalizeAdvertRecord(advert, corporationRecord);
    ...
    if (filters.excludeFriendlyFire &&
        resolveFriendlyFireLegalAtTime(corporationRuntime && corporationRuntime.aggressionSettings, {
...
class CorpRecruitmentProxyService extends BaseService {
    ...
    Handle_GetRecruitmentAdsForCorporation(args, session) {
        ...
        return buildList(Object.values(runtime.recruitmentAds || {}).map((advert) => buildAdvertRow(advert, corporationRecord)));
    }
...
//# sourceMappingURL=corpRecProxyService.js.map