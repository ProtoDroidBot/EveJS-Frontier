...
const { getAllianceCorporationIDs, getAllianceRecord, getCharacterIDsInCorporation, } = require(path.join(__dirname, "./corporationState"));
const { normalizePositiveInteger, } = require(path.join(__dirname, "./corporationRuntimeState"));
...
function createWarNotification(characterIDs, { typeID, senderID = 0, data = {}, created = null, emitLive = true, } = {}) {
...
function buildWarDeclarationData(war, structure = null, extraData = {}) {
    ...
    const data = buildWarOwnersData(war, {
        ...
        delayHours: startedAt > declaredAt
            ? Number((startedAt - declaredAt) / FILETIME_TICKS_PER_HOUR)
            : 0,
        ...extraData,
    });
    const structureID = normalizePositiveInteger(structure && (structure.structureID || structure.itemID), null);
    if (structureID) {
        data.warHQ = String((structure && (structure.itemName || structure.name)) ||
...
function notifyWarAdopted(war, { allianceID, isAlly = false, } = {}) {
...
function notifyWarEndedHqSecurityDrop(war, structure, options = {}) {
    const structureID = normalizePositiveInteger(structure && (structure.structureID || structure.itemID), 0) || 0;
    ...
    return createWarNotification(collectAllWarParticipantCharacterIDs(war), {
        ...
        data: buildWarOwnersData(war, {
            ...
            warHQ: String((structure && (structure.itemName || structure.name)) ||
                `Structure ${structureID}`),
            warHQ_IdType: [
                structureID,
                normalizePositiveInteger(structure && structure.typeID, 0) || 0,
...
function notifyWarInherited(war, { allianceID, quitterID, opponentID, isAlly = false, } = {}) {
    return createWarNotification(collectAllWarParticipantCharacterIDs(war), {
        typeID: isAlly
...
//# sourceMappingURL=warNotificationCenter.js.map