...
const { cloneValue, ensureRuntimeInitialized, normalizeBoolean, normalizeInteger, normalizePositiveInteger, normalizeText, updateRuntimeState, } = require(path.join(__dirname, "./corporationRuntimeState"));
const { createWarSurrenderNotifications, getWarRecord, updateWarRecord, } = require(path.join(__dirname, "./warRuntimeState"));
const { getCharacterAllyBaseCost, payConcordAllyFee, resolveWarEntityWalletOwner, settleWarEntityTransfer, } = require(path.join(__dirname, "./warCostState"));
...
function getNegotiationRecord(warNegotiationID) {
    ...
    const record = runtimeTable.warNegotiations &&
...
function listNegotiationsForOwner(ownerID) {
    ...
    return Object.values(runtimeTable.warNegotiations || {})
        .filter((entry) => Number(entry && entry.negotiationState) === WAR_NEGOTIATION_STATE_NEW &&
...
function createWarNegotiation({ warID, warNegotiationTypeID, ownerID1, ownerID2, declaredByID, againstID, iskValue = 0, description = "", ownerID1AccountKey = 1000, createdByCharacterID = null, } = {}) {
    const numericWarID = normalizePositiveInteger(warID, null);
    ...
    updateRuntimeState((runtimeTable) => {
        ...
        runtimeTable.warNegotiations[String(nextNegotiationID)] = {
            ...
            warID: numericWarID,
            warNegotiationTypeID: normalizeInteger(warNegotiationTypeID, WAR_NEGOTIATION_TYPE_ALLY_OFFER),
    ...
    if (createdNegotiation) {
        if (normalizeInteger(createdNegotiation.warNegotiationTypeID, -1) ===
        ...
        else if (normalizeInteger(createdNegotiation.warNegotiationTypeID, -1) ===
...
function resolveSessionAccountKey(session) {
    const numeric = Number(session && (session.corpAccountKey || session.corpaccountkey));
...
function acceptAllyNegotiation(warNegotiationID, session = null) {
    ...
    const now = currentFileTime();
    const payerOwnerID = normalizePositiveInteger(negotiation.ownerID2, null) ||
    ...
    const concordFee = existingAllyCount > 0
    ...
    if (getCorporationWalletBalance(payerWallet.walletCorporationID, accountKey) + 0.0001 <
        totalRequired) {
        return null;
    }
    if (concordFee > 0) {
    ...
    updateWarRecord(negotiation.warID, (war) => {
        ...
        const timeFinished = warFinish && warFinish < naturalEnd ? warFinish.toString() : naturalEnd.toString();
    ...
    warNotificationCenter.notifyAllyJoinedWar(acceptedWar, negotiation.ownerID1, {
...
function acceptSurrender(warNegotiationID, session = null) {
    ...
    if (Number(negotiation.iskValue || 0) > 0) {
        ...
        if (getCorporationWalletBalance(payerWallet.walletCorporationID, payerAccountKey) + 0.0001 <
...
function listPeaceTreatiesForOwner(ownerID) {
    ...
    const runtimeTable = ensureRuntimeRoots(ensureRuntimeInitialized());
    const now = currentFileTime();
    const allTreaties = Object.values(runtimeTable.peaceTreaties || {}).map((entry) => cloneValue(entry)).filter((entry) => {
        try {
...
//# sourceMappingURL=warNegotiationRuntimeState.js.map