...
function normalizeAggressionSettings(rawSettings = null, options: Record<string, any> = {}) {
...
function readAggressionSettings(corporationID, options: Record<string, any> = {}) {
...
function resolveFriendlyFireLegalAtTime(rawSettings = null, options: Record<string, any> = {}) {
...
function hasPendingAggressionChangeAtTime(rawSettings = null, options: Record<string, any> = {}) {
  const settings = normalizeAggressionSettings(rawSettings, options);
  const now = normalizeBigInt(options.nowFiletime, currentFileTime());
  const enableAfter = toOptionalBigInt(settings.enableAfter);
  const disableAfter = toOptionalBigInt(settings.disableAfter);
...
function buildAggressionSettingsPayload(rawSettings = null, options: Record<string, any> = {}) {
  const settings = normalizeAggressionSettings(rawSettings, options);
  return {
    type: "object",
