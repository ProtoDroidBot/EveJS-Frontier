...
function normalizeGoalRecord(goalID, value: Record<string, any> = {}) {
  ...
  const normalized = {
    ...
    contributors: Object.fromEntries(
      Object.entries<any>(contributors).map(([characterID, summary]) => [
...
function buildIndexes() {
  ...
  for (const [goalID, rawGoal] of Object.entries<any>(root.goalsByID || {})) {
...
function createGoal(characterID, payload: Record<string, any> = {}) {
...
function closeGoal(goalID, characterID) {
  ...
  if (!canCharacterManageCorporationGoals(characterID, goal.corporationID)) {
    return { success: false as const, errorMsg: "CANNOT_MANAGE_GOALS" };
  }
...
function listContributorSummariesForGoal(goalID, page) {
  ...
  const contributors = Object.values<any>(goal.contributors || {})
    .map((entry) => ({
      ...entry,
      goalID,
    }))
