...
const { buildFiletimeLong, buildIndexRowset, buildList, buildRowset, currentFileTime, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getAllianceCorporationIDs, getAllianceRecord, getCorporationRecord, } = require(path.join(__dirname, "./corporationState"));
const { ensureRuntimeInitialized, getAllianceRuntime, normalizePositiveInteger, updateAllianceRuntime, } = require(path.join(__dirname, "./corporationRuntimeState"));
...
function getAllianceMembershipStartFiletime(allianceID, corporationID) {
    ...
    const storedValue = runtime.memberJoinedAtByCorporation &&
...
function normalizeAllianceHistoryEntry(entry, fallbackAllianceID = null) {
    const explicitAllianceID = entry && Object.prototype.hasOwnProperty.call(entry, "allianceID")
        ? entry.allianceID
        : fallbackAllianceID;
    return {
...
function getRecordedCorporationAllianceHistory(corporationID) {
    ...
    for (const [allianceID, runtime] of Object.entries(runtimeTable.alliances || {})) {
        const historyByCorporation = runtime &&
...
function buildCorporationAllianceHistoryRowset(corporationID) {
    ...
    if (corporationRecord) {
        const currentAllianceID = normalizePositiveInteger(corporationRecord.allianceID, null);
        if (currentAllianceID) {
            const currentAllianceRecord = getAllianceRecord(currentAllianceID);
            rows.push({
                allianceID: currentAllianceID,
                startDate: getAllianceMembershipStartFiletime(currentAllianceID, corporationRecord.corporationID),
                deleted: currentAllianceRecord && currentAllianceRecord.deleted ? 1 : 0,
            });
        }
        const seen = new Set(rows.map((row) => `${row.allianceID || ""}:${row.startDate}`));
        for (const historyRow of getRecordedCorporationAllianceHistory(corporationRecord.corporationID)) {
    ...
    return buildRowset(["allianceID", "startDate", "deleted"], rows.map((entry) => buildList([
        entry.allianceID,
...
function buildAllianceMembersIndexRowset(allianceID) {
    const allianceRecord = getAllianceRecord(allianceID);
    const runtime = getAllianceRuntime(allianceID) || {};
    const keyedRows = (allianceRecord ? getAllianceCorporationIDs(allianceID) : []).map((corporationID) => [
        corporationID,
        [
            corporationID,
            allianceID,
            normalizePositiveInteger(runtime.executorSupportByCorporation &&
    ...
    return buildIndexRowset(ALLIANCE_MEMBER_HEADER, keyedRows, "corporationID");
}
function buildAllianceMembersRowset(allianceID) {
    const allianceRecord = getAllianceRecord(allianceID);
    const runtime = getAllianceRuntime(allianceID) || {};
    const rows = (allianceRecord ? getAllianceCorporationIDs(allianceID) : []).map((corporationID) => buildList([
        corporationID,
        allianceID,
        normalizePositiveInteger(runtime.executorSupportByCorporation &&
            runtime.executorSupportByCorporation[String(corporationID)], null),
    ...
    return buildRowset(ALLIANCE_MEMBER_HEADER, rows, "eve.common.script.sys.rowset.Rowset");
}
function buildAllianceApplicationsIndexRowset(allianceID) {
    const runtime = getAllianceRuntime(allianceID) || {};
    return buildIndexRowset(ALLIANCE_APPLICATION_HEADER, Object.entries(runtime.applications || {}).map(([corporationID, application]) => [
        ...
        [
            ...
            buildFiletimeLong(application && application.applicationDateTime
...
function buildCorporationAllianceApplicationsIndexRowset(corporationID, applications = {}) {
    return buildIndexRowset(ALLIANCE_APPLICATION_HEADER, Object.entries(applications || {}).map(([allianceID, application]) => [
        Number(allianceID),
        [
            Number(allianceID),
            Number(application && application.corporationID
...
function setAllianceExecutorSupportChoice(allianceID, supporterCorporationID, chosenExecutorID) {
    const numericAllianceID = normalizePositiveInteger(allianceID, null);
    const numericSupporterCorporationID = normalizePositiveInteger(supporterCorporationID, null);
    ...
    updateAllianceRuntime(numericAllianceID, (runtime) => {
        runtime.memberJoinedAtByCorporation =
            runtime.memberJoinedAtByCorporation &&
                ...
                ? runtime.memberJoinedAtByCorporation
                : {};
        runtime.memberJoinedAtByCorporation[String(numericCorporationID)] = String(startDate || currentFileTime());
        return runtime;
    });
}
function recordAllianceMemberLeave(allianceID, corporationID, leaveDate = null) {
    ...
    const joinDate = getAllianceMembershipStartFiletime(numericAllianceID, numericCorporationID);
    ...
    updateAllianceRuntime(numericAllianceID, (runtime) => {
        ...
        const history = Array.isArray(runtime.allianceHistoryByCorporation[corporationKey])
...
function getAllianceMembersOlderThan(allianceID, minimumDays) {
    ...
    return getAllianceCorporationIDs(allianceID).filter((corporationID) => getDaysInAlliance(allianceID, corporationID) >= requiredDays);
}
module.exports = {
...
//# sourceMappingURL=allianceViewState.js.map