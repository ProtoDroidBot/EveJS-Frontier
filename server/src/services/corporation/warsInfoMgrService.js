...
function buildWarRowset(wars = []) {
    return buildDbRowset(WAR_DBROW_COLUMNS, wars.map((war) => {
...
class WarsInfoMgrService extends BaseService {
    ...
    Handle_GetWarsByOwnerID(args) {
        ...
        return buildCachedMethodCallResult(buildWarRowset(listWarsForOwner(ownerID)), {
    ...
    Handle_GetWarsByOwners(args) {
        ...
        return buildDict(ownerIDs.map((ownerID) => [
            ownerID,
            buildDict(listWarsForOwner(ownerID).map((war) => [
                Number(war.warID),
                buildWarPayload(war),
            ])),
        ]));
    ...
    Handle_GetWarsRequiringAssistance(args, session) {
        const ownerID = (args && args.length > 0 ? Number(args[0]) || 0 : 0) ||
            ...
            0;
        return buildList(listWarsForOwner(ownerID)
            .filter((war) => Number(war.againstID) === Number(ownerID) && Number(war.openForAllies || 0) === 1)
    ...
    Handle_GetTop50(args) {
        ...
        return buildCachedMethodCallResult(buildWarRowset(listAllWarsDescending()
