...
const { buildBoundObjectResponse, buildDbIndexRowset, buildDbRowset, buildDict, buildFiletimeLong, buildKeyVal, buildList, buildPackedRow, buildPagedResultSet, buildRow, buildRowset, currentFileTime, extractList, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
const { getCharacterRecord, } = require(path.join(__dirname, "../character/characterState"));
const { resolveCharacterCreationSchoolProfile, } = require(path.join(__dirname, "../character/characterCreationData"));
...
const { getAllianceRecord, getCorporationInfoRecord, getCorporationOwnerRecord, getCorporationRecord, getOwnerLookupRecord, setCharacterAffiliation, NPC_STARTER_CORPORATION_ID, } = require(path.join(__dirname, "./corporationState"));
const { CORP_ROLE_DIRECTOR, CORPORATION_WALLET_KEY_START, DEFAULT_STRUCTURE_REINFORCE_HOUR, FULL_ADMIN_ROLE_MASK, ensureCharacterMemberState, ensureRuntimeInitialized, getCorporationDivisionNames, getCorporationMember, getCorporationRuntime, getPageForMembers, listCorporationMembers, normalizeBoolean, normalizeInteger, normalizePositiveInteger, normalizeText, recordCorporationRoleHistory, toRoleMaskBigInt, updateAllianceRuntime, updateCorporationRecord, updateCorporationRuntime, updateRuntimeState, createAllianceWithRuntime, createCorporationWithRuntime, deleteCorporationWithRuntime, setCorporationAlliance, syncMemberStateToCharacterRecord, } = require(path.join(__dirname, "./corporationRuntimeState"));
const { NO_REINFORCEMENT_WEEKDAY, } = require(path.join(__dirname, "../structure/structureConstants"));
const { createWarRecord, } = require(path.join(__dirname, "./warRuntimeState"));
const { buildCorporationMemberTrackingRowset, queryCorporationMemberIDs, } = require(path.join(__dirname, "./corpMemberQueryState"));
const { adjustCorporationWalletDivisionBalance, getCorporationWalletDivisionsInfo, getCorporationWalletBalance, normalizeCorporationWalletKey, setCorporationWalletDivisionBalance, } = require(path.join(__dirname, "./corpWalletState"));
const { buildCorporationAllianceApplicationsIndexRowset, } = require(path.join(__dirname, "./allianceViewState"));
const { executeCorporationMemberActions, } = require(path.join(__dirname, "./corpMemberActionState"));
const { BILL_TYPE_WAR, createBill, } = require(path.join(__dirname, "../account/billRuntimeState"));
const { getCharacterAllyBaseCost, } = require(path.join(__dirname, "./warCostState"));
const { listKillmailsForCorporation, } = require(path.join(__dirname, "../killmail/killmailState"));
const { sendCorporationWelcomeMailToCharacter, } = require(path.join(__dirname, "../mail/mailState"));
const { buildAggressionSettingsPayload, readAggressionSettings, resolveFriendlyFireLegalAtTime, scheduleAggressionSettingsChange, } = require(path.join(__dirname, "./aggressionSettingsState"));
const { addLabelMask, allocateNextLabelID, removeLabelMask, toLabelKey, toMarshalMaskValue, } = require(path.join(__dirname, "./contactLabelState"));
const { buildAllianceApplicationSnapshot, buildCorporationApplicationRow, buildCorporationMemberSnapshot, buildCorporationSnapshot, notifyAllianceApplicationChanged, notifyAllianceChanged, notifyAllianceMemberChanged, notifyCorporationApplicationChanged, notifyCorporationChanged, notifyCorporationLiquidationShareTransfer, notifyCorporationMemberChanged, notifyCorporationRemoved, notifyCorporationWelcomeMailChanged, refreshCharacterSession, } = require(path.join(__dirname, "./corporationNotifications"));
const { notifyCorporationApplicationAccepted, notifyCorporationApplicationAcceptedByCharacter, notifyCorporationApplicationInvited, notifyCorporationApplicationReceived, notifyCorporationApplicationRejected, notifyCorporationApplicationRejectedByCharacter, notifyCorporationApplicationWithdrawn, } = require(path.join(__dirname, "./corpApplicationNotifications"));
const { notifyCorporationDividendPaid, notifyCorporationLiquidation, notifyCorporationMemberKicked, notifyCorporationMemberLeft, notifyCorporationNewCeo, notifyCorporationTaxRateChanged, } = require(path.join(__dirname, "./corpLifecycleNotifications"));
const MEMBER_HEADER = [
    "characterID",
    "corporationID",
    "startDate",
...
function resolveCorporationID(session) {
    return normalizePositiveInteger((session && (session.corporationID || session.corpid)) || DEFAULT_NPC_CORPORATION_ID, DEFAULT_NPC_CORPORATION_ID);
}
function resolveCharacterID(session, args) {
    return normalizePositiveInteger((args && args.length > 0 && args[0]) ||
...
function buildSuggestionObjects(baseValue, fallback, fieldName) {
    return buildSuggestionVariants(baseValue, fallback).map((value) => buildKeyVal([[fieldName, value]]));
}
function buildRandomTickerSuggestion() {
    let tickerName = "";
    for (let index = 0; index < 4; index += 1) {
        tickerName += TICKER_SUGGESTION_ALPHABET[Math.floor(Math.random() * TICKER_SUGGESTION_ALPHABET.length)];
    }
    return tickerName;
}
...
function buildApplicationAcceptMemberSnapshot(corporationID, characterID) {
    ...
    const character = getCharacterRecord(characterID) || {};
    const joinDate = member.startDate ||
...
function memberHasActiveRoles(member) {
    ...
    return (toRoleMaskBigInt(member.roles, 0n) !== 0n ||
...
function resolveFallbackCorporationID(characterID) {
    ...
    const schoolProfile = resolveCharacterCreationSchoolProfile(characterRecord.schoolID, {
        raceID: characterRecord.raceID,
        corporationID: DEFAULT_NPC_CORPORATION_ID,
    });
    const schoolCorporationID = normalizePositiveInteger(schoolProfile.corporationID, null);
...
function moveCharacterToCorporation(characterID, fromCorporationID, toCorporationID, options = {}) {
    ...
    const sourceIsNpcCorporation = Boolean(sourceCorporation.isNPC) ||
        Number(fromCorporationID) === Number(DEFAULT_NPC_CORPORATION_ID);
    const previousFromMember = buildCorporationMemberSnapshot(fromCorporationID, characterID);
    const previousFromCorporation = buildCorporationSnapshot(fromCorporationID);
    const previousToMember = buildCorporationMemberSnapshot(targetCorporation.corporationID, characterID);
    ...
    const affiliationResult = setCharacterAffiliation(characterID, targetCorporation.corporationID, targetCorporation.allianceID || null);
    ...
    if (options.applicationAccept === true) {
        if (!sourceIsNpcCorporation) {
            notifyCorporationMemberChanged(fromCorporationID, characterID, previousFromMember, { refreshSession: false });
            notifyCorporationChanged(fromCorporationID, previousFromCorporation);
        }
        notifyCorporationMemberChanged(targetCorporation.corporationID, characterID, buildPreviousApplicationAcceptMemberSnapshot(fromCorporationID), {
            header: APPLICATION_ACCEPT_MEMBER_CHANGE_HEADER,
            nextSnapshot: buildApplicationAcceptMemberSnapshot(targetCorporation.corporationID, characterID),
            idTypes: ["clientID"],
        ...
        notifyCorporationMemberChanged(targetCorporation.corporationID, characterID, null, {
            header: CREATE_MEMBER_CHANGE_HEADER,
            nextSnapshot: buildCreateCorporationMemberSnapshot(targetCorporation.corporationID, characterID),
        ...
        notifyCorporationMemberChanged(fromCorporationID, characterID, previousFromMember);
        notifyCorporationChanged(fromCorporationID, previousFromCorporation);
        notifyCorporationMemberChanged(targetCorporation.corporationID, characterID, previousToMember);
    }
    notifyCorporationChanged(targetCorporation.corporationID, previousToCorporation);
    ...
    if (Number(fromCorporationID) !== Number(targetCorporation.corporationID)) {
        ...
        const welcomeMailResult = sendCorporationWelcomeMailToCharacter(characterID, targetCorporation.corporationID, {
            corporationRecord: targetCorporation,
            body: targetRuntime.welcomeMail,
        });
        if (!welcomeMailResult.success) {
            log.warn(`[CorpRegistry] Failed to send corporation welcome mail for char=${characterID} corp=${targetCorporation.corporationID}: ${welcomeMailResult.errorMsg}`);
        }
    }
    return {
        success: true,
...
function transferLiquidationSharesToCharacter(corporationID, characterID) {
    ...
    const shares = runtime.shares && typeof runtime.shares === "object"
        ? runtime.shares
        : {};
    const corporationShares = Math.max(0, normalizeInteger(shares[String(corporationID)], 0));
    ...
    notifyCorporationLiquidationShareTransfer(corporationID, characterID, corporationShares);
...
function moveResigningCeoToFallbackCorporation({ characterID, fromCorporationID, toCorporationID, }) {
    const targetCorporation = getCorporationRecord(toCorporationID);
    if (!targetCorporation) {
        return {
            success: false,
...
function liquidateCorporationForResigningCeo(corporationID, characterID) {
    ...
    const previousFromMember = buildCorporationMemberSnapshot(corporationID, characterID);
    const previousToMember = buildCorporationMemberSnapshot(targetCorporation.corporationID, characterID);
    ...
    for (const corporationID of Object.keys(runtimeTable.corporations || {})) {
        ...
        updateCorporationRuntime(corporationID, (runtime) => {
            for (const application of Object.values(runtime.applications || {})) {
                if (Number(application.characterID) === Number(characterID) &&
                    Number(application.applicationID) !== Number(exceptApplicationID)) {
                    removedApplications.push(archiveApplication(runtime, application, {
        ...
        for (const application of removedApplications) {
            notifyCorporationApplicationChanged(application.corporationID, application.characterID, application.applicationID, buildCorporationApplicationRow(application));
        }
    }
}
...
function buildMemberRow(member) {
    ...
    return buildPackedRow(MEMBER_DBROW_COLUMNS, {
        ...
        ownerName: characterRecord.characterName ||
...
function buildKillmailRows(records = []) {
    return buildDbRowset(KILLMAIL_DBROW_COLUMNS, records.map((record) => [
...
function buildCachedResult(result, { serviceName, method, args = [], versionCheck = "5 minutes", sessionInfo = null, sessionInfoValue = undefined, proxyCache = false, } = {}) {
...
function buildApplicationsByCharacter(runtime) {
    ...
    return buildDict(Object.entries(entries).map(([characterID, rows]) => [
        Number(characterID),
        buildList(rows),
    ]));
}
...
function filterApplicationHistory(applications = [], characterID = null) {
    return applications
        .filter((application) => !characterID || Number(application.characterID) === Number(characterID))
...
class CorpRegistryRuntimeService extends BaseService {
    ...
    resolveBoundCorporationID(session) {
        ...
        if (boundObjectID &&
            ...
            const boundCorporationID = normalizePositiveInteger(this.serviceManager.getBoundObjectParams(boundObjectID), 0);
    ...
    Handle_GetEveOwners(args, session) {
        ...
        const owners = listCorporationMembers(corporationID)
            ...
            .filter(Boolean)
            .map((record) => buildRow(OWNER_HEADER, [
            record.ownerID,
            record.ownerName,
            record.typeID,
            record.gender,
    ...
    Handle_GetCorporateContacts(args, session) {
        ...
        return buildDict(Object.entries(runtime.contacts || {}).map(([contactID, contact]) => [
            Number(contactID),
    ...
    Handle_GetLabels(args, session) {
        const runtime = getCorporationRuntime(resolveCorporationID(session)) || {};
        return buildDbIndexRowset(LABEL_DBROW_COLUMNS, Object.entries(runtime.labels || {}).map(([labelID, label]) => [
            toMarshalMaskValue(labelID),
    ...
    Handle_CreateLabel(args, session) {
        ...
        updateCorporationRuntime(corporationID, (runtime, corporationRecord, table) => {
            const allocation = allocateNextLabelID(runtime.labels || {}, table._meta.nextLabelID);
        ...
        updateCorporationRuntime(corporationID, (runtime) => {
            for (const contactID of contactIDs) {
                ...
                if (!runtime.contacts[key]) {
                    ...
                    runtime.contacts[key].labelMask = addLabelMask(runtime.contacts[key].labelMask, labelMask);
                }
            }
            return runtime;
        });
    ...
    Handle_RemoveLabels(args, session) {
        ...
        updateCorporationRuntime(corporationID, (runtime) => {
            for (const contactID of contactIDs) {
                if (runtime.contacts[String(contactID)]) {
                    runtime.contacts[String(contactID)].labelMask = removeLabelMask(runtime.contacts[String(contactID)].labelMask, labelMask);
                }
            }
            return runtime;
        });
    ...
    Handle_GetBulletins(args, session) {
        const corporationID = resolveCorporationID(session);
        const runtime = getCorporationRuntime(corporationID) || {};
        return buildDbRowset(BULLETIN_DBROW_COLUMNS, (runtime.bulletins || [])
            .slice()
    ...
    Handle_GetBulletin(args, session) {
        ...
        const corporationID = resolveCorporationID(session);
        const runtime = getCorporationRuntime(corporationID) || {};
        const bulletin = (runtime.bulletins || []).find((entry) => Number(entry.bulletinID) === Number(bulletinID));
    ...
    Handle_AddBulletin(args, session) {
        ...
        updateCorporationRuntime(corporationID, (runtime, corporationRecord, table) => {
            ...
            const index = (runtime.bulletins || []).findIndex((entry) => Number(entry.bulletinID) === Number(nextBulletinID));
            const record = {
                bulletinID: nextBulletinID,
                ownerID: corporationID,
                createDateTime: index >= 0
    ...
    Handle_DeleteBulletin(args, session) {
        ...
        const bulletinID = normalizePositiveInteger(args && args[0], null);
        updateCorporationRuntime(corporationID, (runtime) => {
            runtime.bulletins = (runtime.bulletins || []).filter((bulletin) => Number(bulletin.bulletinID) !== Number(bulletinID));
            return runtime;
        });
        return null;
    }
    Handle_UpdateBulletinOrder(args, session) {
        ...
        updateCorporationRuntime(corporationID, (runtime) => {
            ...
            runtime.bulletins.sort((left, right) => Number(left.sortOrder || 0) - Number(right.sortOrder || 0));
            return runtime;
        });
        return null;
    }
    ...
    Handle_InsertApplication(args, session) {
        ...
        notifyCorporationApplicationChanged(corporationID, characterID, applicationID, buildCorporationApplicationRow(createdApplication));
    ...
    Handle_UpdateApplicationOffer(args, session) {
        ...
        if (applicationFound) {
            const delayApplicationChanged = status === APP_STATUS_ACCEPTED_BY_CHARACTER;
            if (!delayApplicationChanged) {
                notifyCorporationApplicationChanged(corporationID, applicantCharacterID || characterID, applicationID, nextApplicationRow);
            }
            const notifyCharacterID = applicantCharacterID || characterID;
            if (status === APP_STATUS_ACCEPTED_BY_CORPORATION) {
                notifyCorporationApplicationAccepted(corporationID, notifyCharacterID, applicationText);
            }
            else if (status === APP_STATUS_REJECTED_BY_CORPORATION) {
                notifyCorporationApplicationRejected(corporationID, notifyCharacterID, applicationText, customMessage);
            }
            else if (status === APP_STATUS_ACCEPTED_BY_CHARACTER) {
                notifyCorporationApplicationAcceptedByCharacter(corporationID, notifyCharacterID, applicationText);
            }
            else if (status === APP_STATUS_REJECTED_BY_CHARACTER) {
                notifyCorporationApplicationRejectedByCharacter(corporationID, notifyCharacterID, applicationText);
            }
            else if (status === APP_STATUS_WITHDRAWN_BY_CHARACTER) {
                notifyCorporationApplicationWithdrawn(corporationID, notifyCharacterID, applicationText);
            }
        }
        if (joinedCharacterID) {
            const currentCorporationID = normalizePositiveInteger((getCharacterRecord(joinedCharacterID) || {}).corporationID, DEFAULT_NPC_CORPORATION_ID);
            const moveResult = moveCharacterToCorporation(joinedCharacterID, currentCorporationID, corporationID, { applicationAccept: true });
            if (moveResult.success) {
    ...
    Handle_SetCorpWelcomeMail(args, session) {
        ...
        updateCorporationRuntime(corporationID, (runtime) => {
            ...
            runtime.welcomeMailCharacterID = normalizePositiveInteger(editorCharacterID, null);
            runtime.welcomeMailChangeDate = changeDate;
            return runtime;
        });
        notifyCorporationWelcomeMailChanged(editorCharacterID, changeDate);
        return null;
    }
    Handle_InsertInvitation(args, session) {
        ...
        notifyCorporationApplicationChanged(corporationID, characterID, applicationID, buildCorporationApplicationRow(invitationApplication));
    ...
    Handle_GetMembersPaged(args, session) {
        ...
        return buildPagedResultSet(members.slice(start, end).map((member) => buildMemberRow(member)), members.length, Math.max(0, page - 1), end - start);
    }
    Handle_GetMembersByIds(args, session) {
        const corporationID = resolveCorporationID(session);
        const memberIDs = extractList(args && args[0]);
        return buildList(memberIDs
    ...
    Handle_UpdateMember(args, session) {
        ...
        updateCorporationRuntime(corporationID, (runtime) => {
            ...
            if (args && args[4] !== undefined)
                member.roles = String(toRoleMaskBigInt(args[4], 0n));
            if (args && args[5] !== undefined)
                member.grantableRoles = String(toRoleMaskBigInt(args[5], 0n));
            if (args && args[6] !== undefined)
                member.rolesAtHQ = String(toRoleMaskBigInt(args[6], 0n));
            if (args && args[7] !== undefined)
                member.grantableRolesAtHQ = String(toRoleMaskBigInt(args[7], 0n));
            if (args && args[8] !== undefined)
                member.rolesAtBase = String(toRoleMaskBigInt(args[8], 0n));
            if (args && args[9] !== undefined)
                member.grantableRolesAtBase = String(toRoleMaskBigInt(args[9], 0n));
            if (args && args[10] !== undefined)
                member.rolesAtOther = String(toRoleMaskBigInt(args[10], 0n));
            if (args && args[11] !== undefined)
                member.grantableRolesAtOther = String(toRoleMaskBigInt(args[11], 0n));
            if (args && args[12] !== undefined)
                member.baseID = normalizePositiveInteger(args[12], null);
            if (args && args[13] !== undefined)
                member.titleMask = normalizeInteger(args[13], 0);
            if (args && args[14] !== undefined)
                member.blockRoles = String(toRoleMaskBigInt(args[14], 0n));
            return runtime;
        });
        recordMemberRoleAuditRows(corporationID, memberID, previousMemberRecord, getCorporationMember(corporationID, memberID), session);
    ...
    Handle_SetAccountKey(args, session) {
        ...
        const previousSessionAccountKey = session && (session.corpAccountKey ?? session.corpaccountkey ?? null);
        const accountKey = normalizeCorporationWalletKey(args && args[0] !== undefined ? args[0] : CORPORATION_WALLET_KEY_START);
        updateCorporationRuntime(corporationID, (runtime) => {
    ...
    Handle_ExecuteActions(args, session) {
        ...
        const previousMembersByID = new Map(targetIDs
            .filter((targetID) => targetID > 0)
            .map((targetID) => [
            targetID,
            buildCorporationMemberSnapshot(corporationID, targetID),
        ]));
        const result = executeCorporationMemberActions(corporationID, args && args[0], args && args[1], session);
        if (!result.success) {
            log.warn(`[CorpRegistry] ExecuteActions failed for corp ${resolveCorporationID(session)}: ${result.errorMsg || "UNKNOWN"}`);
        }
        if (result.success) {
            for (const targetID of result.data && Array.isArray(result.data.targetIDs)
                ? result.data.targetIDs
                : []) {
                notifyCorporationMemberChanged(corporationID, targetID, previousMembersByID.get(Number(targetID)) || null);
            }
        }
        return null;
    }
    ...
    Handle_GetMemberIDsByQuery(args, session) {
        ...
        return buildList(queryCorporationMemberIDs(corporationID, args && args[0], includeImplied, searchTitles));
    }
    Handle_GetMemberTrackingInfo(args, session) {
        const corporationID = resolveCorporationID(session);
        return buildCachedResult(buildMemberTrackingRows(corporationID), {
    ...
    Handle_GetTitles(args, session) {
        const runtime = getCorporationRuntime(resolveCorporationID(session)) || {};
        return buildDbIndexRowset(TITLE_DBROW_COLUMNS, Object.values(runtime.titles || {}).map((title) => [
    ...
    Handle_GetSharesByShareholder(args, session) {
        const corporationID = resolveCorporationID(session);
        const runtime = getCorporationRuntime(corporationID) || {};
        const shareholderID = normalizeInteger(args && args[0], 0) === 1
            ? corporationID
            : resolveCharacterID(session, []);
        return buildDbRowset(SHAREHOLDER_DBROW_COLUMNS, [[shareholderID, corporationID, Number((runtime.shares || {})[String(shareholderID)] || 0)]], "carbon.common.script.sys.crowset.CRowset");
    }
    Handle_GetShareholders(args, session) {
        const corporationID = normalizePositiveInteger(args && args[0], resolveCorporationID(session));
        const runtime = getCorporationRuntime(corporationID) || {};
        return buildRowset(SHAREHOLDER_HEADER, Object.entries(runtime.shares || {}).map(([shareholderID, shares]) => buildList([Number(shareholderID), corporationID, Number(shares || 0)])), "eve.common.script.sys.rowset.Rowset");
    }
    ...
    Handle_PayoutDividend(args, session) {
        ...
        const sourceBalance = getCorporationWalletBalance(corporationID, CORPORATION_WALLET_KEY_START);
        ...
        const totalWeight = weightedRecipients.reduce((sum, recipient) => sum + Number(recipient.weight || 0), 0);
        if (!(totalWeight > 0)) {
            return null;
        }
        const allocations = [];
        ...
        const debitResult = adjustCorporationWalletDivisionBalance(corporationID, CORPORATION_WALLET_KEY_START, -payoutAmount, {
            entryTypeID: 10,
            ownerID1: corporationID,
        ...
        for (const allocation of allocations) {
            if (allocation.kind === "character") {
                ...
                notifyCorporationDividendPaid(corporationID, allocation.ownerID, allocation.amount, !payShareholders);
                continue;
            }
            adjustCorporationWalletDivisionBalance(allocation.ownerID, CORPORATION_WALLET_KEY_START, allocation.amount, {
                entryTypeID: 10,
                ownerID1: corporationID,
                ownerID2: allocation.ownerID,
                description: `Corporation dividend from ${corporationID}`,
    ...
    Handle_GetAllianceApplications(args, session) {
        const corporationID = resolveCorporationID(session);
        const runtime = getCorporationRuntime(corporationID) || {};
        return buildCorporationAllianceApplicationsIndexRowset(corporationID, runtime.allianceApplications || {});
    }
    Handle_DeleteAllianceApplication(args, session) {
        const corporationID = resolveCorporationID(session);
        const allianceID = normalizePositiveInteger(args && args[0], null);
        const previousApplication = buildAllianceApplicationSnapshot(allianceID, corporationID);
        updateCorporationRuntime(corporationID, (runtime) => {
    ...
    Handle_AddCorporation(args, session) {
        ...
        if (!creatorWallet || creatorWallet.balance < CORPORATION_FOUNDING_COST) {
            throwWrappedUserError("NotEnoughMoney", buildNotEnoughMoneyUserErrorValues(CORPORATION_FOUNDING_COST, creatorWallet ? creatorWallet.balance : 0));
        }
        const debitResult = adjustCharacterBalance(creatorCharacterID, -CORPORATION_FOUNDING_COST, {
        ...
        if (!debitResult.success) {
            throwWrappedUserError("NotEnoughMoney", buildNotEnoughMoneyUserErrorValues(CORPORATION_FOUNDING_COST, creatorWallet.balance));
        }
        const previousCorporationID = normalizePositiveInteger((session && (session.corporationID || session.corpid)) ||
            (getCharacterRecord(creatorCharacterID) || {}).corporationID, null);
        ...
        if (!result.success || !result.data || !result.data.corporationID) {
            adjustCharacterBalance(creatorCharacterID, CORPORATION_FOUNDING_COST, {
                entryTypeID: JOURNAL_ENTRY_TYPE.CORPORATION_REGISTRATION_FEE,
                ownerID1: creatorCharacterID,
                ownerID2: creatorCharacterID,
                referenceID: creatorCharacterID,
        ...
        if (result.success && result.data && result.data.corporationID) {
            if (previousCorporationID) {
                notifyCorporationMemberChanged(previousCorporationID, creatorCharacterID, previousCreatorMember, { refreshSession: false });
            ...
            notifyCorporationMemberChanged(result.data.corporationID, creatorCharacterID, null, {
                header: CREATE_MEMBER_CHANGE_HEADER,
                nextSnapshot: buildCreateCorporationMemberSnapshot(result.data.corporationID, creatorCharacterID),
                refreshSessionOptions: {
                    deferCorpAccountKey: true,
                    clearCorpAccountKey: true,
                    suppressCorpAccountKeyWithCorpChange: true,
    ...
    Handle_GetAggressionSettings(args, session) {
        const corporationID = this.resolveBoundCorporationID(session);
        const corporationRecord = getCorporationRecord(corporationID) || {};
        return buildAggressionSettingsPayload(readAggressionSettings(corporationID, {
    ...
    Handle_RegisterNewAggressionSettings(args, session) {
        ...
        updateCorporationRuntime(corporationID, (runtime) => {
            updatedSettings = scheduleAggressionSettingsChange(runtime && runtime.aggressionSettings, desiredFriendlyFireLegal, { isNpcCorporation });
    ...
    Handle_GetStructureReinforceDefault(args, session) {
        ...
        return [
            ...
            normalizeInteger(runtime.structureReinforceDefault, DEFAULT_STRUCTURE_REINFORCE_HOUR),
        ];
    }
    ...
    Handle_KickOutMember(args, session) {
        ...
        const result = moveCharacterToCorporation(characterID, corporationID, resolveFallbackCorporationID(characterID));
    ...
    Handle_GetMemberIDsWithMoreThanAvgShares(args, session) {
        ...
        const totalShares = Number((runtime.shares && runtime.shares[String(corporationID)]) || 0);
        const averageShares = totalShares / members.length;
        return buildList(members
            .filter((member) => Number((runtime.shares || {})[String(member.characterID)] || 0) >
            averageShares)
    ...
    Handle_ResignFromCEO(args, session) {
        ...
        if (newCEOID) {
            const previousCorporation = buildCorporationSnapshot(corporationID);
            const previousActingMember = buildCorporationMemberSnapshot(corporationID, actingCharacterID);
            const previousNewCeoMember = buildCorporationMemberSnapshot(corporationID, newCEOID);
            updateCorporationRecord(corporationID, {
                ceoID: newCEOID,
            });
            updateCorporationRuntime(corporationID, (runtime) => {
                if (runtime.members[String(actingCharacterID)]) {
                    ...
                    runtime.members[String(actingCharacterID)].roles = String(toRoleMaskBigInt(runtime.members[String(actingCharacterID)].roles, 0n) &
                        ~CORP_ROLE_DIRECTOR);
                }
                if (runtime.members[String(newCEOID)]) {
                    runtime.members[String(newCEOID)].isCEO = true;
                    runtime.members[String(newCEOID)].roles = String(toRoleMaskBigInt(runtime.members[String(newCEOID)].roles, 0n) |
                        CORP_ROLE_DIRECTOR);
                }
                return runtime;
            });
            ...
            notifyCorporationChanged(corporationID, previousCorporation);
            notifyCorporationMemberChanged(corporationID, actingCharacterID, previousActingMember);
            notifyCorporationMemberChanged(corporationID, newCEOID, previousNewCeoMember);
        ...
        if (members.length <= 1 &&
            members.some((member) => Number(member && member.characterID) === Number(actingCharacterID))) {
            const liquidationResult = liquidateCorporationForResigningCeo(corporationID, actingCharacterID);
            if (!liquidationResult.success) {
                log.warn(`[CorpRegistry] ResignFromCEO liquidation failed for corp=${corporationID} char=${actingCharacterID}: ${liquidationResult.errorMsg || "UNKNOWN"}`);
            }
            return null;
        }
        return buildList(members
    ...
    Handle_GetRecentKills(args, session) {
        ...
        const rowset = buildKillmailRows(listKillmailsForCorporation(corporationID, "kills", {
            limit,
            startKillID,
        }));
    ...
    Handle_GetRecentLosses(args, session) {
        ...
        const rawStartKillID = args && args.length > 1 ? args[1] : null;
        const startKillID = rawStartKillID ? Number(rawStartKillID) || 0 : null;
        const rowset = buildKillmailRows(listKillmailsForCorporation(corporationID, "losses", {
            limit,
            startKillID,
        }));
        return buildCachedResult(rowset, {
    ...
    Handle_GetInfoWindowDataForChar(args, session) {
        ...
        for (let index = 1; index <= 16; index += 1) {
            ...
            const titleRecord = runtime.titles && runtime.titles[String(titleID)]
                ? runtime.titles[String(titleID)]
                : null;
            payload.push([
                `title${index}`,
...
//# sourceMappingURL=corpRegistryRuntime.js.map