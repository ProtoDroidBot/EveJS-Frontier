...
const { buildDict, buildFiletimeLong, currentFileTime, normalizeBigInt, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
function readAggressionSettings(corporationID, options = {}) {
    const result = database.read(CORPORATION_RUNTIME_TABLE, `/corporations/${String(Number(corporationID) || 0)}/aggressionSettings`);
    return normalizeAggressionSettings(result.success && result.data && typeof result.data === "object"
        ? result.data
        : null, options);
}
...
function scheduleAggressionSettingsChange(rawSettings = null, friendlyFireLegal, options = {}) {
    ...
    if (resolveFriendlyFireLegalAtTime(currentSettings, { nowFiletime: now }) === desiredState &&
...
//# sourceMappingURL=aggressionSettingsState.js.map