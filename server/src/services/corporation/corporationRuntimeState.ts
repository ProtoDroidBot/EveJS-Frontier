...
function ensureMetaDefaults(meta: Record<string, any> = {}) {
  return {
...
function normalizeCorporationRuntime(runtime, corporationRecord, table) {
  ...
  if (base.offices && typeof base.offices === "object") {
    for (const [officeKey, office] of Object.entries<any>(base.offices)) {
  ...
  if (base.divisionNames && typeof base.divisionNames === "object") {
    for (const [key, value] of Object.entries<any>(base.divisionNames)) {
  ...
  if (
    ...
    for (const [divisionKey, division] of Object.entries<any>(base.wallet.divisions)) {
...
function listCorporationMembers(corporationID) {
  ...
  return cloneValue(Object.values<any>(members).sort(
...
function recordCorporationRoleHistory(corporationID, entry: Record<string, any> = {}) {
...
function getCorporationOffices(corporationID) {
  ...
  return cloneValue(Object.values<any>(runtime.offices).sort(
...
function rebuildOfficesByStationCache() {
  ...
  for (const runtime of Object.values<any>(runtimeTable.corporations || {})) {
    for (const office of Object.values<any>((runtime && runtime.offices) || {})) {
      const stationID = normalizePositiveInteger(office && office.stationID, null);
      if (!stationID) {
        continue;
      }
...
function getLockedItemsByLocation(corporationID, locationID) {
  ...
  if (locationState && typeof locationState === "object") {
    return cloneValue(
      Object.values<any>(locationState).sort(
        (left, right) =>
...
function updateCorporationRecord(corporationID, changes: Record<string, any> = {}) {
  const currentRecord = getCorporationRecord(corporationID);
  if (!currentRecord) {
    return { success: false as const, errorMsg: "CORPORATION_NOT_FOUND" };
  }
...
function updateAllianceRecord(allianceID, changes: Record<string, any> = {}) {
  const currentRecord = getAllianceRecord(allianceID);
  if (!currentRecord) {
    return { success: false as const, errorMsg: "ALLIANCE_NOT_FOUND" };
  }
...
function createCorporationWithRuntime(characterID, options: Record<string, any> = {}) {
...
function createAllianceWithRuntime(characterID, corporationID, options: Record<string, any> = {}) {
  const name = normalizeText(options.name, "").trim();
