...
function normalizeWalletRows(source) {
  return Object.entries<any>(source && typeof source === "object" ? source : {})
...
function updateWalletBalance(kind, ownerID, issuerCorpID, updater, options: Record<string, any> = {}) {
...
function setCharacterWalletLPBalance(characterID, issuerCorpID, amount, options: Record<string, any> = {}) {
...
function adjustCharacterWalletLPBalance(characterID, issuerCorpID, delta, options: Record<string, any> = {}) {
  const normalizedDelta = Math.trunc(Number(delta) || 0);
  const result = updateWalletBalance(
    "character",
    characterID,
    issuerCorpID,
  ...
  if (
    ...
    try {
      ...
      )).recordActivity(Number(characterID) || 0, "earn_loyalty_points", normalizedDelta);
    } catch (dailyGoalError) {
      log.debug(
        `[LPWalletState] daily-goal earn_loyalty_points hook failed: ${dailyGoalError.message}`,
      );
    }
  }
  return result;
...
function setCorporationWalletLPBalance(corporationID, issuerCorpID, amount, options: Record<string, any> = {}) {
  return updateWalletBalance(
    "corporation",
    corporationID,
    issuerCorpID,
...
function adjustCorporationWalletLPBalance(corporationID, issuerCorpID, delta, options: Record<string, any> = {}) {
  const normalizedDelta = Math.trunc(Number(delta) || 0);
  return updateWalletBalance(
    "corporation",
    corporationID,
