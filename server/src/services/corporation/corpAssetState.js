...
const { buildDbRowset, buildKeyVal, buildList, buildPackedRow, buildRowset, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getItemMetadata, getItemMutationVersion, listOwnedItems, } = require(path.join(__dirname, "../inventory/itemStore"));
const { getCorporationOffices, } = require(path.join(__dirname, "./corporationRuntimeState"));
const { getStructureByID, } = require(path.join(__dirname, "../structure/structureState"));
const { ASSET_SAFETY_WRAP_TYPE_ID, listWrapsForOwner, } = require(path.join(__dirname, "../structure/structureAssetSafetyState"));
...
function buildItemPayload(item) {
    ...
    return {
        ...
        categoryID: toInt(item && item.categoryID, toInt(metadata && metadata.categoryID, 0)),
...
function buildCorporationAssetSnapshot(corporationID) {
    ...
    const officeStationIDs = new Set(offices.map((office) => toInt(office.stationID, 0)).filter(Boolean));
    const items = listOwnedItems(numericCorporationID);
    const itemByID = new Map(items.map((item) => [toInt(item && item.itemID, 0), item]).filter(([itemID]) => itemID > 0));
    for (const office of offices) {
    ...
    for (const item of items) {
        ...
        if (root.kind === "office") {
            ...
            else if (officeStationIDs.has(toInt(root.locationInfo.locationID, 0)) &&
    ...
    const sortedLocations = Object.fromEntries(ASSET_BUCKETS.map((bucket) => [
        bucket,
        [...bucketLocations[bucket].values()].sort((left, right) => toInt(left.solarSystemID, 0) - toInt(right.solarSystemID, 0) ||
            toInt(left.locationID, 0) - toInt(right.locationID, 0)),
    ]));
    const sortedItems = Object.fromEntries(ASSET_BUCKETS.map((bucket) => [
        bucket,
        new Map([...bucketItems[bucket].entries()].map(([locationID, bucketLocationItems]) => [
            locationID,
            [...bucketLocationItems].sort((left, right) => toInt(left.flagID, 0) - toInt(right.flagID, 0) ||
...
function buildLocationList(locations = []) {
    return buildList(locations.map((location) => buildKeyVal([
...
function buildAssetLocationCrowset(locations = [], which = "offices") {
    const includeItemCount = normalizeWhich(which) === "deliveries" ||
    ...
    return buildDbRowset(columns, locations.map((location) => {
        const base = {
            locationID: toInt(location && location.locationID, 0),
            solarsystemID: location && location.solarSystemID ? toInt(location.solarSystemID, 0) : null,
            typeID: location && location.typeID ? toInt(location.typeID, 0) : null,
...
function buildAssetSearchCrowset(locations = []) {
    return buildDbRowset(ASSET_SEARCH_DBROW_COLUMNS, locations.map((location) => ({
        locationID: toInt(location && location.locationID, 0),
    })), "carbon.common.script.sys.crowset.CRowset");
}
function buildAssetItemRowset(items = []) {
    return buildRowset(ITEM_ROWSET_HEADER, items.map((item) => buildList([
...
function buildAssetItemCrowset(items = []) {
    return buildDbRowset(ASSET_ITEM_DBROW_COLUMNS, items.map((item) => buildPackedRow(ASSET_ITEM_DBROW_COLUMNS, {
        itemID: toInt(item && item.itemID, 0),
        typeID: toInt(item && item.typeID, 0),
        ownerID: toInt(item && item.ownerID, 0),
        locationID: toInt(item && item.locationID, 0),
...
function searchAssetLocations(corporationID, which, filters = {}) {
    ...
    const hasFilter = toInt(filters.categoryID, 0) > 0 ||
...
//# sourceMappingURL=corpAssetState.js.map