...
function ensureWarIndexes() {
  ...
  const warsAsc = Object.values<any>(runtimeTable.wars || {})
...
function normalizeNowFiletime(options: Record<string, any> = {}) {
...
function hasPeaceTreaty(runtimeTable, warID, ownerID, otherOwnerID, reason) {
  return Object.values<any>(runtimeTable.peaceTreaties || {}).some(
    (treaty) =>
...
function notifyWarDefenderMembers(war, corpTypeID, allianceTypeID, extraData: Record<string, any> = {}) {
  const againstID = normalizePositiveInteger(war && war.againstID, null);
  const declaredByID = normalizePositiveInteger(war && war.declaredByID, null);
...
function endWarWithReason(warID, options: Record<string, any> = {}) {
  const numericWarID = normalizePositiveInteger(warID, null);
  if (!numericWarID) {
    return null;
  }
...
function endWarsForLostWarHQ(structure, options: Record<string, any> = {}) {
  ...
  const writeResult = updateRuntimeState((runtimeTable) => {
    runtimeTable.wars =
      runtimeTable.wars && typeof runtimeTable.wars === "object"
        ...
        : {};
    ensurePeaceTreatyRuntime(runtimeTable);
    for (const war of Object.values<any>(runtimeTable.wars)) {
      if (
        !war ||
  ...
  return changedWars.map(({ nextWar }: Record<string, any>) => cloneValue(nextWar));
}
...
function processDueWarBills(options: Record<string, any> = {}) {
  const now = normalizeNowFiletime(options);
  ...
  updateRuntimeState((runtimeTable) => {
    runtimeTable.mutualWarInvites =
      ...
      typeof runtimeTable.mutualWarInvites === "object"
        ? runtimeTable.mutualWarInvites
        : {};
    for (const [key, invite] of Object.entries<any>(runtimeTable.mutualWarInvites)) {
  ...
  updateRuntimeState((runtimeTable) => {
    runtimeTable.warNegotiations =
      ...
      typeof runtimeTable.warNegotiations === "object"
        ? runtimeTable.warNegotiations
        : {};
    for (const negotiation of Object.values<any>(runtimeTable.warNegotiations)) {
      if (
        !negotiation ||
  ...
  updateRuntimeState((runtimeTable) => {
    runtimeTable.peaceTreaties =
      runtimeTable.peaceTreaties && typeof runtimeTable.peaceTreaties === "object"
        ? runtimeTable.peaceTreaties
        : {};
    for (const [key, treaty] of Object.entries<any>(runtimeTable.peaceTreaties)) {
...
function expireWarAllies(now) {
  ...
  const writeResult = updateRuntimeState((runtimeTable) => {
    ...
    for (const war of Object.values<any>(runtimeTable.wars)) {
      ...
      const previousWar = cloneValue(war);
      let changed = false;
      for (const [allyID, ally] of Object.entries<any>(war.allies)) {
...
function processWarLifecycle(options: Record<string, any> = {}) {
  const now = normalizeNowFiletime(options);
...
function findActiveWarBetweenWarEntities(leftEntityIDs, rightEntityIDs, options: Record<string, any> = {}) {
  const now = normalizeNowFiletime(options);
  const left = leftEntityIDs instanceof Set ? leftEntityIDs : new Set<any>(leftEntityIDs || []);
  const right = rightEntityIDs instanceof Set ? rightEntityIDs : new Set<any>(rightEntityIDs || []);
  ...
  for (const war of listAllWars()) {
    ...
    const activeAllyIDs = Object.entries<any>(war.allies || {})
...
function findActiveWarBetweenCharacters(attackerCharacterID, targetCharacterID, options: Record<string, any> = {}) {
...
function areCharactersLegalWarOpponents(attackerCharacterID, targetCharacterID, options: Record<string, any> = {}) {
  return Boolean(
