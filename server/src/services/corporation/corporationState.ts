...
function normalizeLocalizedText(value, fallback = "") {
  ...
  if (value && typeof value === "object") {
    ...
    for (const localizedValue of Object.values<any>(value)) {
...
type CorporationLogo = {
  ...
  color2: number | null;
  color3: number | null;
  typeface: number | null;
};

function normalizeCorporationLogo(record: Record<string, any> = {}): Record<string, any> & CorporationLogo {
  const normalized = {
    ...record,
...
function normalizeCorporationTable(payload: Record<string, any> = {}) {
  return {
    _meta: {
...
function normalizeAllianceTable(payload: Record<string, any> = {}) {
  return {
    _meta: {
...
function loadNpcCorporations() {
  ...
  npcCorporationCache = new Map<any, any>(
...
function loadNpcCharacters() {
  ...
  npcCharacterCache = new Map<any, any>(
...
function ensureCorporationMembersIndex() {
  ...
  for (const [characterID, characterRecord] of Object.entries<any>(readCharacters())) {
...
function ensureAllianceCorporationsIndex() {
  ...
  for (const [allianceID, allianceRecord] of Object.entries<any>(allianceRecords)) {
  ...
  for (const corporationRecord of Object.values<any>(corporationRecords)) {
    const corporationID = normalizePositiveInteger(
    ...
    if (!nextIndex.has(allianceID)) {
      nextIndex.set(allianceID, new Set<any>());
    }
...
function buildUniqueTicker(name, existingTickers: any[] = [], fallback = "CORP") {
...
function ensureCorporationsInitialized() {
  ...
  for (const [recordKey, storedRecord] of Object.entries<any>(table.records)) {
...
function findCorporationByName(name) {
  ...
  return (
    Object.values<any>(table.records)
...
function setCorporationAlliance(corporationID, allianceID = null) {
  ...
  if (normalizedAllianceID) {
    ...
    const memberCorporationIDs = new Set<any>(
      (
    ...
    targetAllianceRecord.memberCorporationIDs = Array.from<any>(
...
function createCustomCorporation(characterID, name) {
  ...
  const table = ensureCorporationsInitialized();
  const existingTickers = Object.values<any>(table.records).map(
