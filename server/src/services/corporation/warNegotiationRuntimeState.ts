...
function listNegotiationsForOwner(ownerID) {
  ...
  const runtimeTable = ensureRuntimeRoots(ensureRuntimeInitialized());
  return Object.values<any>(runtimeTable.warNegotiations || {})
    .filter(
      (entry) =>
...
function createPeaceTreaty({ warID, ownerID, otherOwnerID, peaceReason = PEACE_REASON_UNDEFINED }: Record<string, any> = {}) {
  const numericOwnerID = normalizePositiveInteger(ownerID, null);
...
function countExistingAllyContracts(war, currentTime) {
  return Object.values<any>((war && war.allies) || {}).filter((ally) => {
    try {
...
function listPeaceTreatiesForOwner(ownerID) {
  ...
  const runtimeTable = ensureRuntimeRoots(ensureRuntimeInitialized());
  const now = currentFileTime();
  const allTreaties = Object.values<any>(runtimeTable.peaceTreaties || {}).map((entry) =>
    cloneValue(entry),
  ).filter((entry) => {
    try {
