...
const { BILL_TYPE_RENTAL, createBill, getBillRecord, listBillsForDebtor, listDueBills, markBillProcessed, tryAutoPayBill, } = require(path.join(__dirname, "../account/billRuntimeState"));
const { getCorporationOffices, normalizePositiveInteger, updateRuntimeState, } = require(path.join(__dirname, "./corporationRuntimeState"));
const { notifyOfficeRentalChange, } = require(path.join(__dirname, "./corporationNotifications"));
const { getCharacterIDsInCorporation, } = require(path.join(__dirname, "./corporationState"));
...
function compareOfficeRentalBillsForProcessing(left, right) {
    ...
    const debtorDelta = normalizePositiveInteger(left && left.debtorID, 0) -
        normalizePositiveInteger(right && right.debtorID, 0);
    if (debtorDelta !== 0) {
        return debtorDelta;
    }
    const stationDelta = normalizePositiveInteger(left && left.externalID2, 0) -
        normalizePositiveInteger(right && right.externalID2, 0);
    if (stationDelta !== 0) {
        return stationDelta;
    }
    return (normalizePositiveInteger(left && left.billID, 0) -
...
function buildOfficeRentalBillNotificationData(bill) {
    return {
        ...
        externalID: bill && bill.externalID !== undefined && bill.externalID !== null
            ? Number(bill.externalID) || -1
            : -1,
        externalID2: bill && bill.externalID2 !== undefined && bill.externalID2 !== null
...
function buildOfficeRentalBillIssuedNotificationData(bill, options = {}) {
    return {
        ...
        currentDate: filetimeLongData(options.currentFileTime || options.baseFileTime || currentFileTime()),
...
function createOfficeRentalBillIssuedNotifications(bill, options = {}) {
    ...
    for (const characterID of [...new Set(getCharacterIDsInCorporation(debtorCorporationID))]) {
        ...
        if (!result || result.success !== true) {
            log.warn(`[OfficeRentalBilling] Failed to create live rental bill notification ` +
...
function createStructureOfficeRentalBillNotifications(bill) {
    ...
    for (const characterID of [...new Set(getCharacterIDsInCorporation(debtorCorporationID))]) {
        ...
        if (!result || result.success !== true) {
            log.warn(`[OfficeRentalBilling] Failed to create rental bill notification ` +
...
function getCurrentOfficeRentalCost(office, structure) {
    if (structure) {
        return Number(getProfileSettingValueForStructure(structure, STRUCTURE_SETTING_ID.CORP_RENT_OFFICE, {
            defaultValue: Number(office && office.rentalCost) || 0,
        }) || 0);
    }
    const station = getStationRecord(null, office && office.stationID);
    return Number((station && station.officeRentalCost) ||
        (office && office.rentalCost) ||
        0);
}
function createNextStructureOfficeRentalBill(corporationID, structure, stationID, rentalCost, options = {}) {
...
function createNextOfficeRentalBill(corporationID, stationID, rentalCost, options = {}) {
    ...
    if (station && station.isStructure) {
        ...
        return createNextStructureOfficeRentalBill(corporationID, structure, stationID, amount, options);
    }
...
function cancelOfficeRentalBillsForOffice(corporationID, stationID) {
    ...
    for (const bill of listBillsForDebtor(numericCorporationID)) {
        if (Number(bill.billTypeID) !== Number(BILL_TYPE_RENTAL) ||
...
function findCorporationOfficeForBill(bill) {
    ...
    return getCorporationOffices(corporationID).find((office) => Number(office.stationID) === stationID && !office.impounded) || null;
}
function updateOfficeAfterPaidBill(office, bill, nextBill) {
    ...
    updateRuntimeState((runtimeTable) => {
        ...
        const currentOffice = corporationRuntime &&
...
function removeStructureOfficeForUnpaidBill(office, structure, options = {}) {
    ...
    const safetyResult = assetSafetyState.moveCorporationOfficeAssetsToSafety(corporationID, structure, office, options);
    if (!safetyResult.success) {
        log.warn(`[OfficeRentalBilling] Structure office due-bill handoff failed corporation=${corporationID} office=${officeID}: ${safetyResult.errorMsg}`);
        return false;
    }
    let removed = false;
    updateRuntimeState((runtimeTable) => {
...
function processPaidOfficeRentalBill(bill, office, structure, options = {}) {
    ...
    const nextBill = structure
        ? createNextStructureOfficeRentalBill(bill.debtorID, structure, bill.externalID2, nextAmount, {
            baseFileTime: bill.dueDateTime,
            emitLive: options.emitLive,
        })
        : createNextOfficeRentalBill(bill.debtorID, bill.externalID2, nextAmount, {
            baseFileTime: bill.dueDateTime,
            emitLive: options.emitLive,
        });
...
//# sourceMappingURL=officeRentalBilling.js.map