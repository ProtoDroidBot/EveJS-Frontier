...
const { buildDbRowset, buildFiletimeLong, buildList, buildDict, buildRow, buildKeyVal, buildRowset, extractList, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
const { NPC_STARTER_CORPORATION_ID, getCorporationPublicInfo, } = require(path.join(__dirname, "./corporationState"));
const { getCorporationDivisionNames, getCorporationMember, getCorporationRuntime, toRoleMaskBigInt, } = require(path.join(__dirname, "./corporationRuntimeState"));
const { buildAggressionSettingsPayload, readAggressionSettings, } = require(path.join(__dirname, "./aggressionSettingsState"));
const { buildAssetItemCrowset, buildAssetItemRowset, buildAssetLocationCrowset, buildAssetSearchCrowset, buildLocationList, listAssetItemsForLocation, listAssetLocations, searchAssetLocations, } = require(path.join(__dirname, "./corpAssetState"));
...
function buildEmptyAuditMemberPayload() {
    return [
        buildDbRowset(AUDIT_LOG_EVENT_DBROW_COLUMNS, [], "carbon.common.script.sys.crowset.CRowset"),
        buildDbRowset(ROLE_HISTORY_DBROW_COLUMNS, [], "carbon.common.script.sys.crowset.CRowset"),
    ];
}
function sessionHasAuditAccess(session, corporationID) {
    const sessionRoles = toRoleMaskBigInt(session && (session.corprole || session.rolesAtAll || session.corpRole), 0n);
    if ((sessionRoles & CORP_ROLE_DIRECTOR) === CORP_ROLE_DIRECTOR ||
        (sessionRoles & CORP_ROLE_AUDITOR) === CORP_ROLE_AUDITOR) {
        return true;
    }
    const characterID = normalizeInteger(session && (session.characterID || session.charid), 0);
    ...
    return ((memberRoles & CORP_ROLE_DIRECTOR) === CORP_ROLE_DIRECTOR ||
...
function buildAuditEventRows(runtime, memberID, fromDate, toDate, rowsPerPage) {
    const rows = (Array.isArray(runtime && runtime.memberEventLog)
        ...
        : [])
        .filter((entry) => {
        const entryCharacterID = normalizeInteger(getFirstPresent(entry, ["charID", "characterID", "memberID"]), 0);
        const eventDateTime = getFirstPresent(entry, [
            "eventDateTime",
            "eventTime",
            "changeTime",
        ]);
        return (entryCharacterID === Number(memberID) &&
            isInAuditRange(eventDateTime, fromDate, toDate));
    })
        .sort((left, right) => normalizeAuditTime(getFirstPresent(right, ["eventDateTime", "eventTime", "changeTime"]), 0n) >
        normalizeAuditTime(getFirstPresent(left, ["eventDateTime", "eventTime", "changeTime"]), 0n)
        ? 1
        : -1)
        .slice(0, rowsPerPage)
        .map((entry) => [
    ...
    return buildDbRowset(AUDIT_LOG_EVENT_DBROW_COLUMNS, rows, "carbon.common.script.sys.crowset.CRowset");
}
function buildRoleHistoryRows(runtime, memberID, fromDate, toDate, rowsPerPage) {
    const rows = (Array.isArray(runtime && runtime.roleHistory)
        ...
        .sort((left, right) => normalizeAuditTime(getFirstPresent(right, ["changeTime", "changedAt"]), 0n) >
    ...
    return buildDbRowset(ROLE_HISTORY_DBROW_COLUMNS, rows, "carbon.common.script.sys.crowset.CRowset");
}
function resolveCorporationInfo(corpID, session) {
    const numericCorpID = Number(corpID) || 0;
    const characterID = session && (session.characterID || session.charid) ? Number(session.characterID || session.charid) : 0;
    ...
    return {
        ...
        corporationName: `Corporation ${numericCorpID}`,
        ticker: "CORP",
        tickerName: "CORP",
        ...
        allianceID: numericCorpID === Number(charData.corporationID || 0)
...
class CorpMgrService extends BaseService {
    ...
    Handle_GetAggressionSettingsForCorps(args) {
        ...
        return buildDict(corporationIDs.map((corporationID) => [
            corporationID,
    ...
    Handle_AuditMember(args, session) {
        ...
        const corporationID = normalizeInteger(session && (session.corporationID || session.corpid), 0);
        ...
        const runtime = getCorporationRuntime(corporationID) || {};
        log.debug(`[CorpMgr] AuditMember memberID=${memberID} corporationID=${corporationID} rowsPerPage=${rowsPerPage}`);
        return [
    ...
    Handle_GetAssetInventory(args, session) {
        ...
        const sessionCorporationID = (session && (session.corporationID || session.corpid)) || corporationID;
        const rowset = buildAssetLocationCrowset(listAssetLocations(corporationID, which), which);
    ...
    Handle_GetAssetInventoryForLocation(args, session) {
        ...
        const sessionCorporationID = (session && (session.corporationID || session.corpid)) || corporationID;
        return buildCachedMethodCallResult(buildAssetItemCrowset(listAssetItemsForLocation(corporationID, locationID, which)), {
            serviceName: "corpmgr",
    ...
    Handle_SearchAssets(args, session) {
        ...
        const rowset = buildAssetSearchCrowset(searchAssetLocations(corporationID, which, {
            categoryID,
            groupID,
            typeID,
            minimumQuantity,
...
//# sourceMappingURL=corpmgrService.js.map