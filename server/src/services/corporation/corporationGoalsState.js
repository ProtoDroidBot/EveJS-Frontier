...
const { createUuidString, currencyToNumber, } = require(path.join(__dirname, "../../_secondary/express/gatewayServices/gatewayServiceHelpers"));
...
function normalizeContributor(value = {}, characterID = null) {
    const numericCharacterID = normalizePositiveInteger(characterID !== null ? characterID : value.characterID, null);
    ...
    return {
        ...
        contributionsCount: Math.max(0, normalizeInteger(value.contributionsCount, 0)),
        lastContributedAtMs: Math.max(0, normalizeInteger(value.lastContributedAtMs, 0)),
    };
}
function normalizeGoalRecord(value = {}, explicitGoalID = null) {
    ...
    return {
        ...
        contributionConfigurationBase64: normalizeText(value.contributionConfigurationBase64, ""),
        rewardPools,
        contributors,
    };
}
...
function listCorporationGoals(corporationID, options = {}) {
    ...
    const stateFilter = options.state === undefined || options.state === null
        ? null
    ...
    return [...index]
        ...
        .sort((left, right) => {
        const leftFinished = Math.max(left.finishedAtMs || 0, left.updatedAtMs || 0, left.createdAtMs || 0);
        const rightFinished = Math.max(right.finishedAtMs || 0, right.updatedAtMs || 0, right.createdAtMs || 0);
...
function createGoal({ characterID, name, description, desiredProgress, contributionConfigurationBase64 = "", career = 0, dueAtMs = 0, participationLimit = null, contributionLimit = null, scalar = null, rewardPools = [], } = {}) {
    ensureCache();
...
function getContributorSummary(goalID, characterID) {
    ...
    return cloneValue(record.contributors && record.contributors[String(characterID)]
...
function listGoalsWithRewards(corporationID, characterID) {
    return listCorporationGoals(corporationID)
        .filter((record) => {
        const contributor = record.contributors && record.contributors[String(characterID)]
            ? record.contributors[String(characterID)]
            : null;
...
function recordGoalContribution(goalID, characterID, progressDelta, options = {}) {
    ...
    const contributor = normalizeContributor(record.contributors[contributorKey] || { characterID }, characterID);
    const previousProgress = record.currentProgress;
    ...
    if (record.contributionLimit !== null) {
        const remainingCoverage = Math.max(0, record.contributionLimit - contributor.earningsTotal);
    ...
    contributor.contributionsCount += Math.max(1, normalizeInteger(options.contributionsCount, 1));
    const scalar = Number.isFinite(Number(record.scalar))
        ? Number(record.scalar)
        : 1;
    const creditedQuantity = Math.max(0, Math.trunc(effectiveDelta * Math.max(0, scalar || 1)));
...
//# sourceMappingURL=corporationGoalsState.js.map