...
const { getCorporationRuntime, updateCorporationRuntime, } = require(path.join(__dirname, "./corporationRuntimeState"));
...
function normalizeMarketTransactionEntry(entry = {}, accountKey = CORPORATION_WALLET_KEY_START) {
    const normalizedAccountKey = normalizeCorporationWalletKey(entry.keyID ?? entry.accountKey ?? entry.accountID ?? accountKey);
    return {
        transactionID: normalizePositiveInteger(entry.transactionID, 0),
        transactionDate: typeof entry.transactionDate === "bigint"
...
function notifyCorporationWalletChange(corporationID, accountKey, balance) {
    ...
    for (const session of sessionRegistry.getSessions()) {
        if (Number(session && (session.corporationID || session.corpid)) !== Number(corporationID) ||
...
function setCorporationWalletDivisionBalance(corporationID, accountKey, nextBalance, options = {}) {
    ...
    updateCorporationRuntime(normalizedCorporationID, (runtime) => {
        ...
        const currentDivision = runtime.wallet.divisions[divisionKey] &&
        ...
        if (Math.abs(delta) > 0.0001) {
            ledgerEntry = buildLedgerEntry(normalizedCorporationID, normalizedAccountKey, delta, balance, options);
            appendLimited(currentDivision.journal, ledgerEntry, MAX_JOURNAL_ENTRIES);
            appendLimited(currentDivision.transactions, ledgerEntry, MAX_TRANSACTION_ENTRIES);
        }
    ...
    notifyCorporationWalletChange(normalizedCorporationID, normalizedAccountKey, result.data.balance);
    return result;
}
function adjustCorporationWalletDivisionBalance(corporationID, accountKey, delta, options = {}) {
    const normalizedCorporationID = Number(corporationID) || 0;
    const normalizedAccountKey = normalizeCorporationWalletKey(accountKey);
    const currentBalance = getCorporationWalletBalance(normalizedCorporationID, normalizedAccountKey);
    ...
    return setCorporationWalletDivisionBalance(normalizedCorporationID, normalizedAccountKey, nextBalance, options);
}
function getCorporationWalletJournal(corporationID, options = {}) {
    const division = getCorporationWalletDivision(corporationID, options.accountKey || CORPORATION_WALLET_KEY_START);
    if (!division) {
        return [];
    }
    ...
    updateCorporationRuntime(normalizedCorporationID, (runtime) => {
        ...
        runtime.wallet.divisions =
            runtime.wallet.divisions && typeof runtime.wallet.divisions === "object"
                ...
                : {};
        const divisionKey = String(normalizedAccountKey);
        const division = runtime.wallet.divisions[divisionKey] &&
            typeof runtime.wallet.divisions[divisionKey] === "object"
            ? runtime.wallet.divisions[divisionKey]
            : {
                key: normalizedAccountKey,
        ...
        division.marketTransactions = Array.isArray(division.marketTransactions)
            ? division.marketTransactions
            : [];
        appendLimited(division.marketTransactions, normalizedEntry, MAX_MARKET_TRANSACTION_ENTRIES);
...
//# sourceMappingURL=corpWalletState.js.map