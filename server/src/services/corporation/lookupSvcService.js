...
const { buildList, buildKeyVal, normalizeText, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { TABLE, readStaticRows, } = require(path.join(__dirname, "../_shared/referenceData"));
const { getAllFactionRecords, } = require(path.join(__dirname, "../faction/factionState"));
const { listAgents, } = require(path.join(__dirname, "../agent/agentAuthority"));
const { ensureAlliancesInitialized, ensureCorporationsInitialized, getAllianceRecord, getCorporationRecord, } = require(path.join(__dirname, "./corporationState"));
const { ensureRuntimeInitialized, } = require(path.join(__dirname, "./corporationRuntimeState"));
...
function filterLookupEntries(entries, search, exactMode = MATCH_BY.PARTIAL_TERMS) {
    return sortLookupEntries(entries.filter((entry) => matchesSearch(entry && entry.name, search, exactMode)), search).slice(0, MAX_LOOKUP_RESULTS);
}
...
function listCharacterEntries() {
    ...
    return Object.entries(readCharacters())
        ...
        return {
            ...
            warFactionID: normalizePositiveInteger(record.warFactionID ?? record.militiaFactionID, 0),
        };
    })
...
function listLocationEntriesForGroup(groupID) {
    switch (normalizeInteger(groupID, 0)) {
        ...
        case GROUP.REGION:
            return uniqueByID(readStaticRows(TABLE.STATIONS).map((row) => ({
...
function lookupLocations(groupID, search, exactMode = MATCH_BY.PARTIAL_TERMS) {
    const entries = listLocationEntriesForGroup(groupID)
        .filter((entry) => entry.id > 0 && entry.name);
    return buildList(filterLookupEntries(entries, search, exactMode).map((entry) => buildLocationRow(entry)));
}
class LookupService extends BaseService {
    ...
    Handle_LookupCharacters(args) {
        ...
        return buildList(filterLookupEntries(listCharacterEntries(), search, exactMode)
    ...
    Handle_LookupAgents(args) {
        const search = args && args.length > 0 ? args[0] : "";
        const exactMode = args && args.length > 1 ? args[1] : MATCH_BY.PARTIAL_TERMS;
        return buildList(filterLookupEntries(listAgentEntries(), search, exactMode)
            .map((entry) => entry.id));
    }
    ...
    Handle_LookupCorporations(args) {
        const search = args && args.length > 0 ? args[0] : "";
        const exactMode = args && args.length > 1 ? args[1] : MATCH_BY.PARTIAL_TERMS;
        return buildList(filterLookupEntries(listCorporationEntries({ includeNPC: true }), search, exactMode)
            .map((entry) => buildCorporationRow(entry)));
    }
    Handle_LookupFactions(args) {
        const search = args && args.length > 0 ? args[0] : "";
        const exactMode = args && args.length > 1 ? args[1] : MATCH_BY.PARTIAL_TERMS;
        return buildList(filterLookupEntries(listFactionEntries(), search, exactMode)
    ...
    Handle_LookupOwners(args) {
        ...
        return buildList(filterLookupEntries(owners, search, exactMode).map((entry) => buildOwnerRow(entry)));
    }
...
//# sourceMappingURL=lookupSvcService.js.map