...
function buildOfficeRentalBillIssuedNotificationData(bill, options: Record<string, any> = {}) {
  return {
    debtorID: normalizePositiveInteger(bill && bill.debtorID, 0),
    creditorID: normalizePositiveInteger(bill && bill.creditorID, 0),
    billTypeID: Number(bill && bill.billTypeID) || BILL_TYPE_RENTAL,
...
function createOfficeRentalBillIssuedNotifications(bill, options: Record<string, any> = {}) {
  ...
  for (const characterID of [...new Set<any>(getCharacterIDsInCorporation(debtorCorporationID))]) {
...
function createNextOfficeRentalBill(corporationID, stationID, rentalCost, options: Record<string, any> = {}) {
  const amount = Number(rentalCost || 0);
  if (amount < 0) {
    return null;
  }
...
function removeStructureOfficeForUnpaidBill(office, structure, options: Record<string, any> = {}) {
  const corporationID = normalizePositiveInteger(office && office.corporationID, 0);
  const officeID = normalizePositiveInteger(office && office.officeID, 0);
...
function impoundStationOfficeForUnpaidBill(office, options: Record<string, any> = {}) {
  const corporationID = normalizePositiveInteger(office && office.corporationID, 0);
  const officeID = normalizePositiveInteger(office && office.officeID, 0);
  if (!corporationID || !officeID) {
    return false;
...
function processPaidOfficeRentalBill(bill, office, structure, options: Record<string, any> = {}) {
...
function processUnpaidOfficeRentalBill(bill, office, structure, options: Record<string, any> = {}) {
...
function processDueOfficeRentalBills(options: Record<string, any> = {}) {
