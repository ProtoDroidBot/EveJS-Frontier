...
function buildMedalInfoRowset(medals: any[] = []) {
  return buildRowset(
    [
      "medalID",
      "issuerID",
...
function buildMedalGraphicsRowset(medalGraphics: any[] = []) {
  return buildRowset(
    [
      "medalID",
      "part",
...
function buildCorpMedalRowset(medals: any[] = []) {
  return buildRowset(
    [
      "medalID",
      "ownerID",
...
function buildMedalRecipientsRowset(recipients: any[] = []) {
  return buildRowset(
...
function collectRuntimeMedalState() {
  ...
  for (const [corporationID, runtime] of Object.entries<any>(runtimeTable.corporations || {})) {
    ...
    for (const medal of Object.values<any>(medalState.medals || {})) {
      medalsByID.set(Number(medal.medalID || 0), {
        ...medal,
        ownerID: Number(medal.ownerID || corporationID) || Number(corporationID) || 0,
      });
    }
    for (const [medalID, recipients] of Object.entries<any>(
...
class CorpService extends BaseService {
  ...
  Handle_SetMedalStatus(args, session) {
    ...
    updateCorporationRuntime(corporationID, (runtime) => {
      for (const [medalID, status] of Object.entries<any>(statusMap)) {
  ...
  Handle_GetAllCorpMedals(args) {
    ...
    const medals = Object.values<any>((runtime.medals && runtime.medals.medals) || {});
    const graphics: any[] = [];
