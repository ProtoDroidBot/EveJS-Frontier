...
const { peekCharacterRecord, writeCharacterRecord, } = require(path.join(__dirname, "..", "character", "characterState"));
const { CUSTOM_ALLIANCE_ID_START, CUSTOM_CORPORATION_ID_START, createCustomAllianceForCorporation, createCustomCorporation, getAllianceRecord, getCorporationRecord, setCharacterAffiliation, setCorporationAlliance, } = require(path.join(__dirname, "corporationState"));
...
function loadCanonicalCharacters() {
    ...
    const databaseCreator = require(path.join(__dirname, "..", "..", "..", "..", "tools", "DatabaseCreator", "database-creator.js"));
...
function ensureCanonicalCharacters() {
    const created = [];
    const missing = CANONICAL_CHARACTER_IDS.filter((characterID) => !peekCharacterRecord(characterID));
    if (missing.length === 0) {
        return created;
    }
...
function ensurePlayerCorporation() {
    ...
    positionCustomCounter(CORPORATIONS_TABLE, "nextCustomCorporationID", PLAYER_CORPORATION_ID);
    const result = createCustomCorporation(PLAYER_CEO_CHARACTER_ID, PLAYER_CORPORATION_NAME);
    if (!result || !result.success) {
        log.warn(`[CoreFixtures] Failed to seed player corporation ${PLAYER_CORPORATION_ID}: ${(result && result.errorMsg) || "UNKNOWN"}`);
        return false;
    }
    return true;
}
function ensurePlayerAlliance() {
    ...
    positionCustomCounter(ALLIANCES_TABLE, "nextCustomAllianceID", PLAYER_ALLIANCE_ID);
    const result = createCustomAllianceForCorporation(PLAYER_CEO_CHARACTER_ID, PLAYER_CORPORATION_ID, PLAYER_ALLIANCE_NAME);
    if (!result || !result.success) {
        log.warn(`[CoreFixtures] Failed to seed player alliance ${PLAYER_ALLIANCE_ID}: ${(result && result.errorMsg) || "UNKNOWN"}`);
        return false;
    }
    return true;
}
...
function ensureCoreFixtures() {
    ...
    if (getCorporationRecord(PLAYER_CORPORATION_ID)) {
        setCharacterAffiliation(PLAYER_CEO_CHARACTER_ID, PLAYER_CORPORATION_ID, PLAYER_ALLIANCE_ID);
    }
...
//# sourceMappingURL=coreFixtureSeeder.js.map