...
const { buildDbRowset, buildFiletimeLong, buildList, currentFileTime, extractList, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getCharacterRecord, } = require(path.join(__dirname, "../character/characterState"));
const { getOwnerLookupRecord, } = require(path.join(__dirname, "./corporationState"));
const { getCorporationRuntime, listCorporationMembers, normalizeText, toRoleMaskBigInt, } = require(path.join(__dirname, "./corporationRuntimeState"));
...
function matchesNumericOperator(leftValue, operator, rightValue) {
    ...
    const right = typeof rightValue === "bigint"
        ? rightValue
...
function matchesCriterion(member, runtime, criterion, includeImplied, searchTitles) {
    ...
    if (operator === OPERATOR_STR_CONTAINS ||
        operator === OPERATOR_STR_LIKE ||
        operator === OPERATOR_STR_STARTS_WITH ||
        operator === OPERATOR_STR_ENDS_WITH ||
        operator === OPERATOR_STR_IS) {
        return getStringCandidates(member, runtime, propertyName, searchTitles).some((candidate) => matchesStringOperator(candidate, operator, criterion.value));
    }
    const leftValue = getComparableNumericValue(member, runtime, propertyName, includeImplied);
    const rightValue = propertyName === "startDateTime"
...
function queryCorporationMemberIDs(corporationID, rawQuery, includeImplied = false, searchTitles = false) {
    ...
    return listCorporationMembers(corporationID)
        ...
        for (const criterion of criteria) {
            const nextResult = matchesCriterion(member, runtime, criterion, includeImplied, searchTitles);
...
function getLastOnlineHours(characterID, member) {
    if (characterControlRuntime.getCharacterControlSnapshot(characterID).online) {
        return -1;
    }
...
function buildCorporationMemberTrackingRowset(corporationID) {
    return buildDbRowset(MEMBER_TRACKING_DBROW_COLUMNS, listCorporationMembers(corporationID).map((member) => {
        const characterRecord = getCharacterRecord(member.characterID) || {};
        const online = characterControlRuntime.getCharacterControlSnapshot(member.characterID).online;
        return [
            member.characterID,
...
function buildCorporationMemberTrackingList(corporationID) {
    return buildList(listCorporationMembers(corporationID).map((member) => {
        const characterRecord = getCharacterRecord(member.characterID) || {};
        const online = characterControlRuntime.getCharacterControlSnapshot(member.characterID).online;
        return buildList([
            member.characterID,
...
//# sourceMappingURL=corpMemberQueryState.js.map