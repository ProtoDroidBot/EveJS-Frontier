...
function normalizeGoalRewardPool(value: Record<string, any> = {}, fallbackIndex = 0) {
...
function normalizeContributor(value: Record<string, any> = {}, characterID = null) {
...
function normalizeGoalRecord(value: Record<string, any> = {}, explicitGoalID = null) {
  ...
  if (value.contributors && typeof value.contributors === "object") {
    for (const [characterID, contributor] of Object.entries<any>(value.contributors)) {
...
function indexRecord(record) {
  if (!cache.corporationIndexes.has(record.corporationID)) {
    cache.corporationIndexes.set(record.corporationID, new Set<any>());
  }
...
function ensureCache() {
  ...
  const records = table.records && typeof table.records === "object"
    ? table.records
    : {};
  for (const [goalID, record] of Object.entries<any>(records)) {
...
function getGoalRecord(goalID, options: Record<string, any> = {}) {
  ensureCache();
...
function listCorporationGoals(corporationID, options: Record<string, any> = {}) {
  ensureCache();
...
function listContributorSummaries(goalID) {
  ...
  return Object.values<any>(record.contributors || {})
...
function recordGoalContribution(goalID, characterID, progressDelta, options: Record<string, any> = {}) {
  ensureCache();
  const record = cache.records.get(normalizeText(goalID, "").toLowerCase());
  if (!record || record.deletedAtMs > 0) {
    return {
