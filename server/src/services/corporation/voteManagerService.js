...
const { buildDbRowset, buildDict, buildFiletimeLong, buildKeyVal, buildList, currentFileTime, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { VOTECASE_STATUS_ALL, VOTECASE_STATUS_CLOSED, VOTECASE_STATUS_OPEN, activateSanctionedAction, canRunForCEO, canViewVotes, castVote, createVoteCase, getVoteCase, listSanctionedActions, listVoteCaseOptions, listVoteCasesByCorporation, listVotesByVoteCase, } = require(path.join(__dirname, "./voteRuntimeState"));
...
function resolveCorporationID(args, session) {
    return ((args && args.length > 0 && Number(args[0])) ||
        (session && (session.corporationID || session.corpid)) ||
        0);
}
function buildVoteCaseRowset(voteCases = []) {
    return buildDbRowset(VOTE_CASE_DBROW_COLUMNS, voteCases.map((voteCase) => [
...
function buildVoteOptions(options = []) {
    return buildList(options.map((option) => buildKeyVal([
...
function buildVotes(votes = []) {
    return buildList(votes.map((vote) => buildKeyVal([
...
class VoteManagerService extends BaseService {
    ...
    Handle_GetVoteCasesByCorporation(args, session) {
        ...
        const requestedStatus = args && args.length > 1 ? Number(args[1]) || VOTECASE_STATUS_ALL : VOTECASE_STATUS_ALL;
        const allVoteCases = listVoteCasesByCorporation(corporationID);
        const nowFiletime = currentFileTime();
        const openVoteCases = allVoteCases.filter((voteCase) => BigInt(String(voteCase.endDateTime || "0")) > nowFiletime);
        const closedVoteCases = allVoteCases.filter((voteCase) => !openVoteCases.some((openVoteCase) => openVoteCase.voteCaseID === voteCase.voteCaseID)).slice(0, MAX_CLOSED_VOTES_FETCHED);
    ...
    Handle_GetVotes(args, session) {
        ...
        const corporationID = resolveCorporationID([session && (session.corporationID || session.corpid)], session) || 0;
        const characterID = (session && (session.characterID || session.charid)) || 0;
        return buildVotes(listVotesByVoteCase(corporationID, voteCaseID).filter((vote) => Number(vote.characterID) === Number(characterID)));
    }
    ...
    Handle_InsertVoteCase(args, session) {
        ...
        const voteCase = createVoteCase(corporationID, characterID, voteCaseText, description, voteType, Array.isArray(options) ? options : [], duration);
    ...
    Handle_GetCorpSanctionedActions(args, session) {
        const corporationID = resolveCorporationID([], session);
        return buildDict(listSanctionedActions(corporationID).map((action) => [
...
//# sourceMappingURL=voteManagerService.js.map