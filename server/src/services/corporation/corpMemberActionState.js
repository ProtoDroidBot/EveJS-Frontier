...
const { CORPORATION_WALLET_KEY_START, adjustCorporationWalletDivisionBalance, getCorporationWalletBalance, normalizeCorporationWalletKey, } = require(path.join(__dirname, "./corpWalletState"));
const { getCorporationMember, getCorporationRuntime, normalizeInteger, normalizePositiveInteger, normalizeText, syncMemberStateToCharacterRecord, toRoleMaskBigInt, updateCorporationRuntime, } = require(path.join(__dirname, "./corporationRuntimeState"));
const CTV_ADD = 1;
const CTV_REMOVE = 2;
const CTV_SET = 3;
const CTV_GIVE = 4;
...
function resolveSourceWalletKey(session, corporationID) {
    const sessionAccountKey = normalizePositiveInteger(session && (session.corpAccountKey || session.corpaccountkey), null);
    if (sessionAccountKey) {
        return normalizeCorporationWalletKey(sessionAccountKey);
    }
    const actingCharacterID = normalizePositiveInteger(session && (session.characterID || session.charid), null);
    const actingMember = actingCharacterID
        ? getCorporationMember(corporationID, actingCharacterID)
        : null;
    return normalizeCorporationWalletKey(actingMember && actingMember.accountKey
...
function preflightActionGrants(corporationID, targetIDs, actions, walletKey) {
    ...
    if (totalSharesRequired > 0) {
        ...
        const availableShares = normalizeInteger(runtime.shares && runtime.shares[String(corporationID)], 0);
        ...
        for (const targetID of targetIDs) {
            ...
            for (const action of actions) {
                ...
                if (ROLE_MASK_PROPERTIES.has(action.property)) {
                    member[action.property] = applyMaskAction(member[action.property], action.verb, action.value).toString();
                    touchedMembers.add(targetID);
                    continue;
                }
                if (action.property === "titleMask") {
                    member.titleMask = applyIntegerMaskAction(member.titleMask, action.verb, action.value);
                    touchedMembers.add(targetID);
                    continue;
                }
...
function applyCashGrants(corporationID, targetIDs, actions, walletKey) {
    for (const action of actions) {
        ...
        for (const targetID of targetIDs) {
            const debitResult = adjustCorporationWalletDivisionBalance(corporationID, walletKey, -amount, {
                entryTypeID: 10,
...
function executeCorporationMemberActions(corporationID, rawTargetIDs, rawActions, session) {
    ...
    const preflight = preflightActionGrants(corporationID, targetIDs, actions, walletKey);
    if (!preflight.success) {
        return preflight;
    }
    applyAdministrativeMemberChanges(corporationID, targetIDs, actions);
    const cashGrantResult = applyCashGrants(corporationID, targetIDs, actions, walletKey);
...
//# sourceMappingURL=corpMemberActionState.js.map