...
const { CORP_ROLE_BRAND_MANAGER, CORP_ROLE_DIRECTOR, getCorporationMember, getCorporationRuntime, normalizeInteger, normalizePositiveInteger, toRoleMaskBigInt, updateCorporationRuntime, } = require(path.join(__dirname, "./corporationRuntimeState"));
...
function normalizeColor(value, required = false) {
    ...
    const hasAnyComponent = value.red !== undefined || value.green !== undefined || value.blue !== undefined;
    if (!hasAnyComponent) {
        return required ? null : null;
    }
    return {
...
function normalizePaletteAttributes(attributes = null) {
    ...
    return {
        mainColor,
        secondaryColor: normalizeColor(attributes.secondaryColor || attributes.secondary_color, false),
        tertiaryColor: normalizeColor(attributes.tertiaryColor || attributes.tertiary_color, false),
    };
}
function getCorporationColorPalette(corporationID) {
    ...
    return {
        ...
        lastModifierCharacterID: normalizePositiveInteger(runtime.corpColorPalette.lastModifierCharacterID, null),
...
//# sourceMappingURL=corpColorsState.js.map