...
const { normalizeAggressionSettings, } = require(path.join(__dirname, "./aggressionSettingsState"));
const { getAllianceCorporationIDs, ALLIANCES_TABLE, CORPORATIONS_TABLE, createCustomAllianceForCorporation, createCustomCorporation, ensureAlliancesInitialized, ensureCorporationsInitialized, findCorporationByName, findCorporationByTickerName, getAllianceRecord, getCharacterIDsInCorporation, getCorporationRecord, setCorporationAlliance, } = require(path.join(__dirname, "./corporationState"));
const { toStoredMaskValue, } = require(path.join(__dirname, "./contactLabelState"));
...
const CORP_ROLE_HANGAR_CAN_TAKE_ALL = 8192n + 16384n + 32768n + 65536n + 131072n + 262144n + 524288n;
const CORP_ROLE_HANGAR_CAN_QUERY_ALL = 1048576n + 2097152n + 4194304n + 8388608n + 16777216n + 33554432n + 67108864n;
...
const FULL_ADMIN_ROLE_MASK = CORP_ROLE_DIRECTOR +
...
const FULL_LOCATIONAL_ROLE_MASK = CORP_ROLE_HANGAR_CAN_TAKE_ALL + CORP_ROLE_HANGAR_CAN_QUERY_ALL;
...
function normalizeCorporationColorPalette(value) {
    ...
    return {
        ...
        lastModifierCharacterID: normalizePositiveInteger(value.lastModifierCharacterID || value.last_modifier_character_id, null),
...
function buildDefaultMemberState(characterID, corporationRecord) {
    ...
    const getCharacterRecord = characterState && typeof characterState.getCharacterRecord === "function"
    ...
    return {
        ...
        corporationID: normalizePositiveInteger(corporationRecord && corporationRecord.corporationID, 0) || 0,
        startDate: normalizeFiletimeString(characterRecord.startDateTime || characterRecord.createDateTime),
        ...
        baseID: normalizePositiveInteger(characterRecord.baseID ||
            characterRecord.homeStationID ||
            characterRecord.cloneStationID ||
            characterRecord.stationID, null),
        titleMask: 0,
        blockRoles: null,
        accountKey: CORPORATION_WALLET_KEY_START,
        isCEO,
        lastOnline: normalizeFiletimeString(currentFileTime()),
        locationID: normalizePositiveInteger(characterRecord.stationID ||
            characterRecord.structureID ||
            characterRecord.solarSystemID, null),
...
function ensureMetaDefaults(meta = {}) {
    return {
        ...
        nextRecruitmentAdID: normalizePositiveInteger(meta.nextRecruitmentAdID, 1) || 1,
...
function normalizeMemberState(member, corporationRecord, explicitCharacterID = null) {
    const defaultState = buildDefaultMemberState(explicitCharacterID !== null && explicitCharacterID !== undefined
        ? explicitCharacterID
        : member && member.characterID, corporationRecord);
    ...
    const grantableRoles = isCEO
        ...
        : normalizeRoleMaskString(member && member.grantableRoles, defaultState.grantableRoles);
    const rolesAtHQ = isCEO
        ? defaultState.rolesAtHQ
        : normalizeRoleMaskString(member && member.rolesAtHQ, defaultState.rolesAtHQ);
    const grantableRolesAtHQ = isCEO
        ? defaultState.grantableRolesAtHQ
        : normalizeRoleMaskString(member && member.grantableRolesAtHQ, defaultState.grantableRolesAtHQ);
    const rolesAtBase = isCEO
        ? defaultState.rolesAtBase
        : normalizeRoleMaskString(member && member.rolesAtBase, defaultState.rolesAtBase);
    const grantableRolesAtBase = isCEO
        ? defaultState.grantableRolesAtBase
        : normalizeRoleMaskString(member && member.grantableRolesAtBase, defaultState.grantableRolesAtBase);
    const rolesAtOther = isCEO
        ? defaultState.rolesAtOther
        : normalizeRoleMaskString(member && member.rolesAtOther, defaultState.rolesAtOther);
    const grantableRolesAtOther = isCEO
        ? defaultState.grantableRolesAtOther
        : normalizeRoleMaskString(member && member.grantableRolesAtOther, defaultState.grantableRolesAtOther);
    return {
        ...defaultState,
        ...cloneValue(member || {}),
        characterID: normalizePositiveInteger(member && member.characterID, defaultState.characterID),
        corporationID: normalizePositiveInteger(member && member.corporationID, defaultState.corporationID),
        startDate: normalizeFiletimeString(member && member.startDate, defaultState.startDate),
        title: normalizeText(member && member.title, defaultState.title),
        divisionID: normalizeInteger(member && member.divisionID, defaultState.divisionID),
        squadronID: normalizeInteger(member && member.squadronID, defaultState.squadronID),
        roles,
        grantableRoles,
        rolesAtHQ,
        grantableRolesAtHQ,
        ...
        baseID: normalizePositiveInteger(member && member.baseID, defaultState.baseID),
        titleMask: normalizeInteger(member && member.titleMask, defaultState.titleMask),
        blockRoles: member && member.blockRoles !== undefined && member.blockRoles !== null
            ? normalizeRoleMaskString(member.blockRoles, "0")
            : null,
        accountKey: normalizeInteger(member && member.accountKey, defaultState.accountKey),
        isCEO,
        lastOnline: normalizeFiletimeString(member && member.lastOnline, defaultState.lastOnline),
        locationID: normalizePositiveInteger(member && member.locationID, defaultState.locationID),
        shipTypeID: normalizePositiveInteger(member && member.shipTypeID, defaultState.shipTypeID),
    };
}
function normalizeTitleState(title, titleID) {
    return {
        ...
        grantableRolesAtHQ: normalizeRoleMaskString(title && title.grantableRolesAtHQ, "0"),
        rolesAtBase: normalizeRoleMaskString(title && title.rolesAtBase, "0"),
        grantableRolesAtBase: normalizeRoleMaskString(title && title.grantableRolesAtBase, "0"),
        rolesAtOther: normalizeRoleMaskString(title && title.rolesAtOther, "0"),
        grantableRolesAtOther: normalizeRoleMaskString(title && title.grantableRolesAtOther, "0"),
    };
}
function normalizeCorporationRuntime(runtime, corporationRecord, table) {
    ...
    for (const memberID of memberIDs) {
        members[String(memberID)] = normalizeMemberState(base.members && base.members[String(memberID)], corporationRecord, memberID);
    }
    const titles = {};
    const defaultTitles = buildDefaultTitleMap();
    for (const [titleKey, defaultTitle] of Object.entries(defaultTitles)) {
        titles[titleKey] = normalizeTitleState(base.titles && base.titles[titleKey], defaultTitle.titleID);
    }
    const offices = {};
    if (base.offices && typeof base.offices === "object") {
        for (const [officeKey, office] of Object.entries(base.offices)) {
            ...
            offices[officeKey] = {
                ...
                stationTypeID: normalizePositiveInteger(office.stationTypeID, normalizePositiveInteger(office.typeID, null)),
                typeID: normalizePositiveInteger(office.typeID, normalizePositiveInteger(office.stationTypeID, null)),
    ...
    if (!hasPersistedOffices &&
        Object.keys(offices).length === 0 &&
        normalizePositiveInteger(corporationRecord.stationID, null)) {
        const office = buildOfficeRecord(table, corporationRecord, corporationRecord.stationID);
    ...
    if (base.wallet &&
        ...
        for (const [divisionKey, division] of Object.entries(base.wallet.divisions)) {
            ...
            wallet.divisions[divisionKey] = {
                key: normalizeInteger(division && division.key, wallet.divisions[divisionKey].key),
                balance: Number((division && division.balance) || wallet.divisions[divisionKey].balance),
    ...
    return {
        ...
        welcomeMailCharacterID: normalizePositiveInteger(base.welcomeMailCharacterID, null),
        welcomeMailChangeDate: normalizeFiletimeString(base.welcomeMailChangeDate, "0"),
        applicationsEnabled: base.applicationsEnabled === undefined
            ? 1
        ...
        structureReinforceDefault: normalizeInteger(base.structureReinforceDefault, DEFAULT_STRUCTURE_REINFORCE_HOUR),
        ...
        contacts: base.contacts && typeof base.contacts === "object"
            ? cloneValue(base.contacts)
            : {},
        labels: base.labels && typeof base.labels === "object"
        ...
        roleHistory: Array.isArray(base.roleHistory)
            ...
            : [],
        applications: base.applications && typeof base.applications === "object"
            ? cloneValue(base.applications)
            : {},
        applicationHistory: Array.isArray(base.applicationHistory)
            ? cloneValue(base.applicationHistory)
            : [],
        invitations: base.invitations && typeof base.invitations === "object"
            ? cloneValue(base.invitations)
            : {},
        invitationHistory: Array.isArray(base.invitationHistory)
            ? cloneValue(base.invitationHistory)
            : [],
        shares: base.shares && typeof base.shares === "object"
            ? cloneValue(base.shares)
            : {
                [String(corporationRecord.corporationID)]: normalizeInteger(corporationRecord && corporationRecord.shares, 1000),
            },
        medals: base.medals && typeof base.medals === "object"
            ? cloneValue(base.medals)
            : { medals: {}, recipientsByMedalID: {} },
        corpColorPalette: normalizeCorporationColorPalette(base.corpColorPalette),
        offices,
        lockedItemsByLocation: base.lockedItemsByLocation && typeof base.lockedItemsByLocation === "object"
            ? cloneValue(base.lockedItemsByLocation)
            : {},
        wallet,
        recruitmentAds: base.recruitmentAds && typeof base.recruitmentAds === "object"
            ? cloneValue(base.recruitmentAds)
            : {},
        allianceApplications: base.allianceApplications && typeof base.allianceApplications === "object"
        ...
        pendingAutoKicks: Array.isArray(base.pendingAutoKicks)
            ...
            : [],
        fw: base.fw && typeof base.fw === "object"
            ? {
                allowedEnlistmentFactions: Array.isArray(base.fw.allowedEnlistmentFactions)
                    ? cloneValue(base.fw.allowedEnlistmentFactions)
                    : [],
                cooldownTimestamp: normalizeInteger(base.fw.cooldownTimestamp, 0) || 0,
                directEnlistment: base.fw.directEnlistment && typeof base.fw.directEnlistment === "object"
...
function normalizeAllianceRuntime(runtime, allianceRecord) {
    ...
    const executorSupportByCorporation = base.executorSupportByCorporation &&
        typeof base.executorSupportByCorporation === "object"
        ? cloneValue(base.executorSupportByCorporation)
        : {};
    const memberJoinedAtByCorporation = base.memberJoinedAtByCorporation &&
        typeof base.memberJoinedAtByCorporation === "object"
        ? cloneValue(base.memberJoinedAtByCorporation)
        : {};
    const allianceHistoryByCorporation = base.allianceHistoryByCorporation &&
    ...
    for (const corporationID of memberCorporationIDs) {
        if (memberJoinedAtByCorporation[String(corporationID)] === undefined ||
            memberJoinedAtByCorporation[String(corporationID)] === null) {
            memberJoinedAtByCorporation[String(corporationID)] = normalizeFiletimeString(allianceRecord.createdAt, "0");
        }
    }
    ...
    if (allianceRecord.executorCorporationID &&
    ...
    return {
        ...
        labels: base.labels && typeof base.labels === "object"
            ...
            : {},
        bulletins: Array.isArray(base.bulletins) ? cloneValue(base.bulletins) : [],
        relationships: base.relationships && typeof base.relationships === "object"
        ...
        primeInfo: {
            currentPrimeHour: normalizeInteger((base.primeInfo && base.primeInfo.currentPrimeHour) ||
                allianceRecord.currentPrimeHour, 0),
            newPrimeHour: normalizeInteger((base.primeInfo && base.primeInfo.newPrimeHour) ||
                allianceRecord.newPrimeHour ||
                allianceRecord.currentPrimeHour, 0),
            newPrimeHourValidAfter: normalizeFiletimeString(base.primeInfo && base.primeInfo.newPrimeHourValidAfter, "0"),
        },
        capitalInfo: {
            currentCapitalSystem: normalizePositiveInteger((base.capitalInfo && base.capitalInfo.currentCapitalSystem) ||
                allianceRecord.currentCapital, null),
            newCapitalSystem: normalizePositiveInteger(base.capitalInfo && base.capitalInfo.newCapitalSystem, null),
            newCapitalSystemValidAfter: normalizeFiletimeString(base.capitalInfo && base.capitalInfo.newCapitalSystemValidAfter, "0"),
        },
    };
}
function ensureRuntimeInitialized() {
    ...
    for (const corporationID of Object.keys(corporationTable.records || {})) {
        ...
        runtimeTable.corporations[String(record.corporationID)] =
            normalizeCorporationRuntime(runtimeTable.corporations[String(record.corporationID)], record, runtimeTable);
    }
    for (const allianceID of Object.keys(allianceTable.records || {})) {
        ...
        runtimeTable.alliances[String(record.allianceID)] = normalizeAllianceRuntime(runtimeTable.alliances[String(record.allianceID)], record);
    }
...
function updateRuntimeState(updater) {
    ...
    const nextTable = typeof updater === "function" ? updater(runtimeTable) || runtimeTable : runtimeTable;
...
function updateCorporationRuntime(corporationID, updater) {
    return updateRuntimeState((runtimeTable) => {
        ...
        const nextRuntime = typeof updater === "function"
    ...
    return updateRuntimeState((runtimeTable) => {
        ...
        const nextRuntime = typeof updater === "function"
            ? updater(cloneValue(currentRuntime), allianceRecord, runtimeTable) ||
                currentRuntime
            : currentRuntime;
        runtimeTable.alliances[String(allianceRecord.allianceID)] = normalizeAllianceRuntime(nextRuntime, allianceRecord);
        return runtimeTable;
    });
}
function listCorporationMembers(corporationID) {
    ...
    return cloneValue(Object.values(members).sort((left, right) => Number(left.characterID) - Number(right.characterID)));
}
...
function getCorporationOffices(corporationID) {
    ...
    return cloneValue(Object.values(runtime.offices).sort((left, right) => Number(left.stationID) - Number(right.stationID)));
}
...
function getLockedItemsByLocation(corporationID, locationID) {
    ...
    if (locationState && typeof locationState === "object") {
        return cloneValue(Object.values(locationState).sort((left, right) => normalizePositiveInteger(left && left.itemID, 0) -
...
function createCorporationWithRuntime(characterID, options = {}) {
    ...
    const characterState = getCharacterState();
    const characterRecord = characterState && typeof characterState.getCharacterRecord === "function"
        ? characterState.getCharacterRecord(characterID) || {}
        : {};
    const stationID = normalizePositiveInteger(options.stationID, result.data.corporationRecord && result.data.corporationRecord.stationID);
    ...
    const updateResult = updateCorporationRecord(corporationID, {
        ...
        description: normalizeText(options.description, `Capsuleer corporation ${name}.`),
        url: normalizeText(options.url, ""),
        stationID,
        solarSystemID: normalizePositiveInteger(options.solarSystemID || (stationRecord && stationRecord.solarSystemID), result.data.corporationRecord && result.data.corporationRecord.solarSystemID),
    ...
    updateCorporationRuntime(corporationID, (runtime) => ({
        ...runtime,
        applicationsEnabled: options.applicationsEnabled === undefined
            ? 1
        ...
        aggressionSettings: normalizeAggressionSettings(options.friendlyFireEnabled
            ? { enableAfter: "0", disableAfter: null }
            : { enableAfter: null, disableAfter: createDate }, { isNpcCorporation: false }),
    }));
    return {
        success: true,
        data: {
...
function createAllianceWithRuntime(characterID, corporationID, options = {}) {
    ...
    const updateResult = updateAllianceRecord(allianceID, {
        allianceName: name,
        shortName: normalizeText(options.shortName, result.data.allianceRecord && result.data.allianceRecord.shortName),
        description: normalizeText(options.description, `Capsuleer alliance ${name}.`),
        url: normalizeText(options.url, ""),
    });
    if (!updateResult.success) {
        return updateResult;
...
function ensureCharacterMemberState(corporationID, characterID) {
    ...
    updateCorporationRuntime(corporationID, (runtime, corporationRecord) => {
        runtime.members[String(characterID)] = buildDefaultMemberState(characterID, corporationRecord);
        return runtime;
    });
...
function getCorporationSessionRoleState(corporationID, characterID) {
    ...
    const isDirector = memberState.isCEO ||
    ...
    return {
        ...
        rolesAtOther: toRoleMaskBigInt(memberState.rolesAtOther, effectiveLocationalRoles),
        baseID: normalizePositiveInteger(memberState.baseID, null),
        accountKey: normalizeInteger(memberState.accountKey, CORPORATION_WALLET_KEY_START),
    };
}
function syncMemberStateToCharacterRecord(corporationID, characterID) {
    ...
    const characterState = getCharacterState();
    const updateCharacterRecord = characterState && typeof characterState.updateCharacterRecord === "function"
...
//# sourceMappingURL=corporationRuntimeState.js.map