...
function buildCorporationAssetSnapshot(corporationID) {
  ...
  const officeStationIDs = new Set<any>(
...
function buildLocationList(locations: any[] = []) {
  return buildList(
...
function buildAssetLocationCrowset(locations: any[] = [], which = "offices") {
...
function buildAssetSearchCrowset(locations: any[] = []) {
  return buildDbRowset(
...
function buildAssetItemRowset(items: any[] = []) {
  return buildRowset(
    ITEM_ROWSET_HEADER,
    items.map((item) =>
      buildList([
...
function buildAssetItemCrowset(items: any[] = []) {
  return buildDbRowset(
...
function matchesFilters(item, filters: Record<string, any> = {}) {
...
function searchAssetLocations(corporationID, which, filters: Record<string, any> = {}) {
  const bucket = normalizeWhich(which);
  const snapshot = buildCorporationAssetSnapshot(corporationID);
