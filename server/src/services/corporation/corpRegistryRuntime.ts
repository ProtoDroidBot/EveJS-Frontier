...
function resolveCharacterID(session, args?: any[]) {
  return normalizePositiveInteger(
...
function archiveApplication(runtime, application, overrides: Record<string, any> = {}) {
...
function archiveAllActiveApplicationsForCharacter(characterID, exceptApplicationID = null) {
  ...
  for (const corporationID of Object.keys(runtimeTable.corporations || {})) {
    const removedApplications: any[] = [];
    updateCorporationRuntime(corporationID, (runtime) => {
      for (const application of Object.values<any>(runtime.applications || {})) {
        if (
...
function buildKillmailRows(records: any[] = []) {
...
function buildCorporationWelcomeMailPayload(runtime: Record<string, any> = {}) {
  return buildKeyVal([
    [
      "characterID",
...
function filterApplicationHistory(applications: any[] = [], characterID = null) {
  return applications
    .filter(
      (application) =>
...
class CorpRegistryRuntimeService extends BaseService {
  ...
  Handle_GetCorporateContacts(args, session) {
    ...
    return buildDict(
      Object.entries<any>(runtime.contacts || {}).map(([contactID, contact]) => [
        Number(contactID),
  ...
  Handle_GetLabels(args, session) {
    ...
    return buildDbIndexRowset(
      LABEL_DBROW_COLUMNS,
      Object.entries<any>(runtime.labels || {}).map(([labelID, label]) => [
        toMarshalMaskValue(labelID),
  ...
  Handle_DeleteLabel(args, session) {
    ...
    updateCorporationRuntime(corporationID, (runtime) => {
      ...
      for (const contact of Object.values<any>(runtime.contacts || {})) {
  ...
  Handle_UpdateBulletinOrder(args, session) {
    ...
    updateCorporationRuntime(corporationID, (runtime) => {
      const orderMap = new Map<any, any>(
  ...
  Handle_GetMyApplications(args, session) {
    ...
    const runtimeTable = ensureRuntimeInitialized();
    const entries: any[] = [];
    for (const [corporationID, runtime] of Object.entries<any>(runtimeTable.corporations || {})) {
      const matching = Object.values<any>(runtime.applications || {})
  ...
  Handle_GetMyOldApplications(args, session) {
    ...
    const runtimeTable = ensureRuntimeInitialized();
    const rows: any[] = [];
    for (const runtime of Object.values<any>(runtimeTable.corporations || {})) {
  ...
  Handle_ExecuteActions(args, session) {
    ...
    const previousMembersByID = new Map<any, any>(
      targetIDs
  ...
  Handle_GetTitles(args, session) {
    ...
    return buildDbIndexRowset(
      TITLE_DBROW_COLUMNS,
      Object.values<any>(runtime.titles || {}).map((title) => [
  ...
  Handle_GetShareholders(args, session) {
    ...
    return buildRowset(
      SHAREHOLDER_HEADER,
      Object.entries<any>(runtime.shares || {}).map(([shareholderID, shares]) =>
  ...
  Handle_PayoutDividend(args, session) {
    ...
    const runtime = getCorporationRuntime(corporationID) || {};
    const weightedRecipients: any[] = [];
    if (payShareholders) {
      for (const [ownerID, shares] of Object.entries<any>(runtime.shares || {})) {
