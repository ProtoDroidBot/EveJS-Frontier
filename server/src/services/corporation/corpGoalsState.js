...
const { createUuidString, sliceWithPage, } = require(path.join(__dirname, "../../_secondary/express/gatewayServices/gatewayServiceHelpers"));
const { CORP_ROLE_DIRECTOR, CORP_ROLE_PROJECT_MANAGER, getCorporationMember, normalizePositiveInteger, toRoleMaskBigInt, } = require(path.join(__dirname, "./corporationRuntimeState"));
...
function writeRoot(root) {
    const nextRoot = {
        ...
        goalsByID: root && root.goalsByID && typeof root.goalsByID === "object"
            ? root.goalsByID
            : {},
    };
...
function normalizeOrganization(value, fallbackCharacterID = 0) {
    ...
    const corporationID = normalizePositiveInteger(organization.corporation && organization.corporation.sequential, null);
    if (corporationID) {
        return {
            corporationID,
            characterID: null,
    ...
    const characterID = normalizePositiveInteger(organization.character && organization.character.sequential, fallbackCharacterID || null);
    return {
        corporationID: null,
        characterID,
    };
...
function normalizeRewardPools(value) {
    ...
    return value
        .map((entry) => {
        const amountValue = Number(entry &&
            entry.isk &&
            entry.isk.amount &&
            entry.isk.amount.units) || 0;
...
function normalizeGoalRecord(goalID, value = {}) {
    ...
    const contributors = value.contributors && typeof value.contributors === "object"
        ? value.contributors
        : {};
    const normalized = {
        ...
        createdByCharacterID: normalizePositiveInteger(value.createdByCharacterID, 0) || 0,
        ...
        contributionConfig: value.contributionConfig && Buffer.isBuffer(value.contributionConfig)
        ...
        participationLimit: value.participationLimit === null || value.participationLimit === undefined
            ? null
            : Math.max(0, Number(value.participationLimit) || 0),
        contributionLimit: value.contributionLimit === null || value.contributionLimit === undefined
            ? null
            : Math.max(0, Number(value.contributionLimit) || 0),
        scalar: value.scalar === null || value.scalar === undefined
            ? null
            : Number(value.scalar),
        contributors: Object.fromEntries(Object.entries(contributors).map(([characterID, summary]) => [
            String(normalizePositiveInteger(characterID, 0) || 0),
            {
                characterID: normalizePositiveInteger(summary && summary.characterID, normalizePositiveInteger(characterID, 0)),
                progress: Math.max(0, Number(summary && summary.progress) || 0),
                rewardTotal: Math.max(0, Number(summary && summary.rewardTotal) || 0),
                rewardRedeemed: Math.max(0, Number(summary && summary.rewardRedeemed) || 0),
                contributedAtMs: Math.max(0, Number(summary && summary.contributedAtMs) || 0),
            },
        ])),
    };
...
function createGoal(characterID, payload = {}) {
    ...
    const goal = normalizeGoalRecord(goalID, {
        ...
        contributionConfig: payload.contribution_configuration &&
        ...
        contributionLimit: payload.contribution_limit !== undefined && payload.contribution_limit !== null
            ? Number(payload.contribution_limit) || 0
            : null,
        scalar: payload.scalar !== undefined && payload.scalar !== null
...
function setGoalCurrentProgress(goalID, currentProgress, newProgress, characterID) {
    ...
    const next = updateGoalState(goalID, (draft) => {
        ...
        if (draft.currentProgress === requestedCurrent || requestedCurrent === 0) {
            draft.currentProgress = Math.min(requestedNew, draft.desiredProgress || requestedNew);
        }
        if (draft.currentProgress >= draft.desiredProgress &&
...
function listActiveGoalIDsForCorporation(corporationID, page) {
    const activeGoals = getGoalsForCorporation(corporationID).filter((goal) => goal.state === GOAL_STATE.ACTIVE);
...
function listInactiveGoalIDsForCorporation(corporationID, timespan, page) {
    ...
    const durationMs = timespan && timespan.duration && typeof timespan.duration === "object"
        ? Math.max(0, Math.trunc(Number(timespan.duration.seconds || 0) * 1000 +
...
function buildContributorSummary(goal, contributor) {
    ...
    return {
        ...
        earnings: contributor.rewardTotal > 0
            ? [
                {
                    quantity: {
...
function getContributorSummaryForGoal(goalID, characterID) {
    ...
    return cloneValue(goal.contributors[String(normalizePositiveInteger(characterID, 0) || 0)] || null);
}
...
function listRewardGoalIDsForCharacter(characterID, page) {
    ...
    const rewards = goals.filter((goal) => {
        const contributor = goal.contributors[String(normalizePositiveInteger(characterID, 0) || 0)] || null;
        if (!contributor) {
            return false;
        }
...
function goalToPayload(goal) {
    ...
    const payload = {
        ...
        contribution_config: Buffer.from(String(numericGoal.contributionConfig || ""), "base64"),
...
//# sourceMappingURL=corpGoalsState.js.map