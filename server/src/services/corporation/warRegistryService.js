...
const { buildBoundObjectResponse, buildDict, buildFiletimeLong, buildKeyVal, buildList, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const config = require(path.join(__dirname, "../../config"));
const { endWarWithReason, listWarsForOwner, getWarRecord, processWarLifecycle, updateWarRecord, } = require(path.join(__dirname, "./warRuntimeState"));
const { WAR_NEGOTIATION_STATE_DECLINED, WAR_NEGOTIATION_STATE_RETRACTED, WAR_NEGOTIATION_TYPE_ALLY_OFFER, WAR_NEGOTIATION_TYPE_SURRENDER_OFFER, acceptAllyNegotiation, acceptSurrender, createWarNegotiation, getNegotiationRecord, listNegotiationsForOwner, updateWarNegotiation, } = require(path.join(__dirname, "./warNegotiationRuntimeState"));
...
function buildWarPayload(war) {
    return buildKeyVal([
        ...
        [
            "allies",
            buildDict(Object.entries(war.allies || {}).map(([allyID, ally]) => [
                Number(allyID),
                buildAllyPayload({
...
function buildWarNegotiationPayload(negotiation) {
    return buildKeyVal([
        ...
        [
            "createdDateTime",
            buildFiletimeLong(negotiation && negotiation.createdDateTime ? negotiation.createdDateTime : 0),
        ],
        [
            "timeAccepted",
...
class WarRegistryService extends BaseService {
    ...
    Handle_GetWars(args, session) {
        ...
        return buildDict(listWarsForOwner(ownerID).map((war) => [Number(war.warID), buildWarPayload(war)]));
    }
    Handle_GetNegotiations(args, session) {
        processWarLifecycle();
        return buildList(listNegotiationsForOwner(resolveWarEntityID(session)).map((negotiation) => buildWarNegotiationPayload(negotiation)));
    }
    ...
    Handle_CreateSurrenderNegotiation(args, session) {
        ...
        const counterpartyID = ownerID === Number(war.declaredByID || 0)
        ...
        createWarNegotiation({
            ...
            iskValue,
            description,
            ownerID1AccountKey: (session && (session.corpAccountKey || session.corpaccountkey)) || 1000,
            createdByCharacterID: resolveCharacterID(session),
        });
        return null;
    }
...
//# sourceMappingURL=warRegistryService.js.map