...
const { buildDbRowset, buildDict, buildList, buildBoundObjectResponse, resolveBoundNodeId, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getCharacterWalletLPBalances, getCorporationWalletLPBalances, getCharacterWalletLPBalance, EVERMARK_ISSUER_CORP_ID, transferCharacterWalletLPToCorporation, transferCorporationWalletLPToCorporation, } = require(path.join(__dirname, "./lpWalletState"));
...
function buildWalletRows(entries = []) {
    return buildList(entries.map((entry) => buildList([
        Number(entry && entry.issuerCorpID) || 0,
        Number(entry && entry.amount) || 0,
    ])));
}
function buildWalletBalanceRowset(entries = []) {
    return buildDbRowset(LP_BALANCE_COLUMNS, entries.map((entry) => [
        Number(entry && entry.issuerCorpID) || 0,
        Number(entry && entry.amount) || 0,
...
class LPService extends BaseService {
    ...
    Handle_GetLPsForCharacter(args, session) {
        ...
        return buildWalletRows(getCharacterWalletLPBalances(session && session.characterID));
    }
    Handle_GetAllMyCharacterWalletLPBalances(args, session) {
        log.debug("[LPSvc] GetAllMyCharacterWalletLPBalances");
        return buildWalletBalanceRowset(getCharacterWalletLPBalances(session && session.characterID));
    }
    Handle_GetAllMyCorporationWalletLPBalances(args, session) {
        log.debug("[LPSvc] GetAllMyCorporationWalletLPBalances");
        return buildWalletBalanceRowset(getCorporationWalletLPBalances(session && (session.corporationID || session.corpid)));
    }
    Handle_GetLPForCharacterCorp(args, session) {
        ...
        return getCharacterWalletLPBalance(session && session.characterID, issuerCorpID);
    }
    Handle_TransferLPFromMyWalletToOtherCorp(args, session) {
        ...
        const result = transferCharacterWalletLPToCorporation(session, receiverCorpID, issuerCorpID, lpAmount);
        if (!result.success) {
    ...
    Handle_TransferLPFromMyCorpWalletToOtherCorp(args, session) {
        ...
        const issuerCorpID = Array.isArray(args) && args.length > 1 ? args[1] : EVERMARK_ISSUER_CORP_ID;
        const lpAmount = Array.isArray(args) && args.length > 2 ? args[2] : 0;
        const result = transferCorporationWalletLPToCorporation(session, receiverCorpID, issuerCorpID, lpAmount);
        if (!result.success) {
            throwLpTransferError(result.errorMsg);
        }
        return null;
...
//# sourceMappingURL=lpService.js.map