...
const { getCorporationInfoRecord, } = require(path.join(__dirname, "./corporationState"));
const { CORP_ROLE_DIRECTOR, listCorporationMembers, normalizePositiveInteger, normalizeText, toRoleMaskBigInt, } = require(path.join(__dirname, "./corporationRuntimeState"));
...
function notifyApplicationRecruiters(corporationID, characterID, typeID, applicationText = "") {
    ...
    for (const recipientID of resolveApplicationRecipientIDs(numericCorporationID)) {
        ...
        const result = deliverCorpAppNotification(recipientID, typeID, numericCharacterID, data);
...
function notifyCorporationApplicationReceived(corporationID, characterID, applicationText = "") {
    return notifyApplicationRecruiters(corporationID, characterID, NOTIFICATION_TYPE.CORP_APP_NEW, applicationText);
}
...
function notifyCorporationApplicationWithdrawn(corporationID, characterID, applicationText = "") {
    return notifyApplicationRecruiters(corporationID, characterID, NOTIFICATION_TYPE.CHAR_APP_WITHDRAW, applicationText);
}
...
function notifyCorporationApplicationAcceptedByCharacter(corporationID, characterID, applicationText = "") {
    return notifyApplicationRecruiters(corporationID, characterID, NOTIFICATION_TYPE.CHAR_APP_ACCEPT, applicationText);
}
...
function notifyCorporationApplicationRejectedByCharacter(corporationID, characterID, applicationText = "") {
    return notifyApplicationRecruiters(corporationID, characterID, NOTIFICATION_TYPE.CHAR_APP_REJECT, applicationText);
}
...
function notifyCorporationApplicationAccepted(corporationID, characterID, applicationText = "") {
    ...
    return deliverCorpAppNotification(numericCharacterID, NOTIFICATION_TYPE.CORP_APP_ACCEPT, numericCorporationID, buildApplicationNotificationData(numericCorporationID, numericCharacterID, {
        applicationText: normalizeText(applicationText, ""),
    }));
}
...
function notifyCorporationApplicationRejected(corporationID, characterID, applicationText = "", customMessage = "") {
    const numericCorporationID = normalizePositiveInteger(corporationID, null);
    const numericCharacterID = normalizePositiveInteger(characterID, null);
    if (!numericCorporationID || !numericCharacterID) {
        return null;
    ...
    return deliverCorpAppNotification(numericCharacterID, normalizedCustomMessage
        ? NOTIFICATION_TYPE.CORP_APP_REJECT_CUSTOM
        : NOTIFICATION_TYPE.CORP_APP_REJECT, numericCorporationID, buildApplicationNotificationData(numericCorporationID, numericCharacterID, extra));
}
...
function notifyCorporationApplicationInvited(corporationID, characterID) {
    ...
    return deliverCorpAppNotification(numericCharacterID, NOTIFICATION_TYPE.CORP_APP_INVITED, numericCorporationID, buildApplicationNotificationData(numericCorporationID, numericCharacterID));
}
module.exports = {
...
//# sourceMappingURL=corpApplicationNotifications.js.map