...
function getCorporationWarPermitStatus(corporationID) {
    ...
    if (Object.prototype.hasOwnProperty.call(corporation, "allowWar") &&
...
function getWarPermitStatusForOwner(ownerID) {
    ...
    const { getAllianceRecord, getCorporationRecord, } = getCorporationState();
...
function notifyWarPermitStatusChanged(corporationID, allowWar) {
    ...
    for (const session of sessionRegistry.getSessions()) {
        if (Number(session && (session.corporationID || session.corpid)) !==
...
//# sourceMappingURL=warPermitState.js.map