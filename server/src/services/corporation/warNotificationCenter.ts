...
function collectUniqueCharacterIDs(ownerIDs: any[] = []) {
...
function createWarNotification(characterIDs, {
  ...
  const results: any[] = [];
  for (const characterID of [...new Set<any>(characterIDs || [])]) {
...
type WarOwnersData = Record<string, any> & {
  declaredByID: number;
  againstID: number;
  warHQ?: string;
  warHQ_IdType?: [number, number];
  isAlly?: boolean;
};

function buildWarOwnersData(war, extraData: Record<string, any> = {}): WarOwnersData {
  return {
...
function buildWarDeclarationData(war, structure = null, extraData: Record<string, any> = {}) {
...
function notifyModernWarDeclared(war, options: Record<string, any> = {}) {
...
function notifyWarDeclaredV2(war, options: Record<string, any> = {}) {
...
function notifyMutualWarInviteSent({ fromOwnerID, toOwnerID, sentDate = null }: Record<string, any> = {}) {
...
function notifyMutualWarInviteRejected({ fromOwnerID, toOwnerID }: Record<string, any> = {}) {
...
function notifyMutualWarInviteAccepted({ fromOwnerID, toOwnerID, time = null }: Record<string, any> = {}) {
...
function notifyMadeWarMutual({ fromOwnerID, toOwnerID, characterID = null }: Record<string, any> = {}) {
...
function notifyMutualWarExpired(war, options: Record<string, any> = {}) {
...
function notifyWarEndedHqSecurityDrop(war, structure, options: Record<string, any> = {}) {
  const structureID = normalizePositiveInteger(
    structure && (structure.structureID || structure.itemID),
    0,
  ) || 0;
...
function notifyWarRetractedByConcord(war, options: Record<string, any> = {}) {
  return createWarNotification(collectAllWarParticipantCharacterIDs(war), {
...
function notifyWarConcordInvalidates(war, options: Record<string, any> = {}) {
...
function notifyWarInvalid(war, options: Record<string, any> = {}) {
  return createWarNotification(collectAllWarParticipantCharacterIDs(war), {
...
function notifySurrenderOffered(negotiation, options: Record<string, any> = {}) {
...
function notifySurrenderDeclined(negotiation, options: Record<string, any> = {}) {
...
function notifyAcceptedSurrender(negotiation, options: Record<string, any> = {}) {
...
function notifyAllyOfferCreated(negotiation, options: Record<string, any> = {}) {
  const characterID = normalizePositiveInteger(options.characterID, 0) || 0;
...
function notifyAllyOfferRetracted(negotiation, options: Record<string, any> = {}) {
  const characterID = normalizePositiveInteger(options.characterID, 0) || 0;
  const senderID = characterID ||
    normalizePositiveInteger(negotiation && negotiation.ownerID1, 0) ||
    0;
...
function notifyAllyOfferDeclined(negotiation, options: Record<string, any> = {}) {
  return createWarNotification(collectWarOwnerCharacterIDs(negotiation && negotiation.ownerID1), {
...
function notifyAllyAccepted(negotiation, options: Record<string, any> = {}) {
...
function notifyAllyJoinedWar(war, allyID, options: Record<string, any> = {}) {
...
function notifyCorporationJoinedAllianceAtWar({ allianceID, corporationID }: Record<string, any> = {}) {
