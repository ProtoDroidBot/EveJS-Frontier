...
const { getCharacterSkills, } = require(path.join(__dirname, "../skills/skillState"));
const { adjustCorporationWalletDivisionBalance, normalizeCorporationWalletKey, } = require(path.join(__dirname, "./corpWalletState"));
...
function getDiplomaticRelationsDogmaAttributes() {
    ...
    const byTypeID = typeDogma && typeof typeDogma === "object" && typeDogma.typesByTypeID
...
function getCharacterSkillLevel(characterID, skillTypeID) {
    ...
    const skillRecord = getCharacterSkills(numericCharacterID).find((record) => Number(record && record.typeID) === numericSkillTypeID);
    if (!skillRecord) {
        return 0;
    }
    return Math.max(0, Number(skillRecord.effectiveSkillLevel ??
...
function getCharacterAllyBaseCost(characterID) {
    ...
    const baseCost = normalizeMoney(attributes[String(ATTRIBUTE_BASE_DEFENDER_ALLY_COST)], DEFAULT_ALLY_BASE_COST);
    const modifierPercentPerLevel = normalizeMoney(attributes[String(ATTRIBUTE_SKILL_ALLY_COST_MODIFIER_BONUS)], 0);
    const skillLevel = getCharacterSkillLevel(characterID, DIPLOMATIC_RELATIONS_TYPE_ID);
    const modifierMultiplier = Math.max(0, 1 + (modifierPercentPerLevel * skillLevel) / 100);
...
function debitWarEntityWallet({ ownerID, amount, accountKey = DEFAULT_RECEIVING_ACCOUNT_KEY, ownerID2 = 0, description = "War wallet transfer", } = {}) {
    ...
    return adjustCorporationWalletDivisionBalance(walletOwner.walletCorporationID, normalizeCorporationWalletKey(accountKey), -Math.abs(normalizeMoney(amount, 0)), {
        entryTypeID: 10,
...
function creditWarEntityWallet({ ownerID, amount, ownerID1 = 0, description = "War wallet transfer", } = {}) {
    const walletOwner = resolveWarEntityWalletOwner(ownerID);
    if (!walletOwner) {
        return {
            success: false,
    ...
    return adjustCorporationWalletDivisionBalance(walletOwner.walletCorporationID, DEFAULT_RECEIVING_ACCOUNT_KEY, Math.abs(normalizeMoney(amount, 0)), {
        entryTypeID: 10,
...
function settleWarEntityTransfer({ fromOwnerID, toOwnerID, amount, fromAccountKey = DEFAULT_RECEIVING_ACCOUNT_KEY, description = "War wallet transfer", } = {}) {
...
function payConcordAllyFee({ ownerID, amount, accountKey = DEFAULT_RECEIVING_ACCOUNT_KEY, description = "CONCORD ally registration fee", } = {}) {
...
//# sourceMappingURL=warCostState.js.map