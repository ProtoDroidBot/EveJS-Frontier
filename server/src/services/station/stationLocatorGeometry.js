...
const structureLocatorGeometry = require(path.join(__dirname, "../structure/structureLocatorGeometry"));
...
function squaredDistance(left, right) {
    const leftPosition = cloneVector(left);
    const rightPosition = cloneVector(right);
...
function rotateVectorByQuaternion(quaternion, vector) {
    ...
    const tx = 2 * (quaternion.y * resolvedVector.z - quaternion.z * resolvedVector.y);
    const ty = 2 * (quaternion.z * resolvedVector.x - quaternion.x * resolvedVector.z);
    const tz = 2 * (quaternion.x * resolvedVector.y - quaternion.y * resolvedVector.x);
    return {
        x: resolvedVector.x +
            quaternion.w * tx +
...
function rotateVectorByRotation(vector, rotation) {
    return rotateVectorByQuaternion(quaternionFromYawPitchRollDegrees(rotation), vector);
}
...
function normalizeLocatorProfile(profile = {}) {
    ...
    return {
        ...
        locatorCategories: [...new Set(locators
...
function loadLocatorProfiles() {
    ...
    try {
        ...
        const payload = readResult.data && typeof readResult.data === "object"
            ? readResult.data
            : null;
        ...
        if (!locatorLoadFailureLogged) {
            ...
            log.warn(`[StationLocatorGeometry] Failed to load ${STATION_GRAPHIC_LOCATORS_TABLE}: ${error.message}`);
        }
    }
...
function getLocatorCandidates(profile, shipTypeID) {
    ...
    const locators = profile.directionalLocators.filter((locator) => String(locator.category || "").startsWith("undockPoint"));
    if (locators.length === 0) {
        return [];
    }
    const requestedCategory = structureLocatorGeometry.getUndockCategoryByShipType(shipTypeID);
    const exactMatches = locators.filter((locator) => locator.category === requestedCategory);
    if (exactMatches.length > 0) {
        return exactMatches;
    }
    const genericMatches = locators.filter((locator) => locator.category === GENERIC_UNDOCK_CATEGORY);
...
function selectLocator(candidates, options = {}) {
    ...
    if (selectionStrategy === "random") {
        ...
        const randomIndex = Math.max(0, Math.min(candidates.length - 1, Math.floor(toFiniteNumber(randomValue, 0) * candidates.length)));
...
function buildStoredFallbackGeometry(station, options = {}) {
    const direction = normalizeVector(station && (station.dockOrientation || station.undockDirection), DEFAULT_FORWARD);
    const basePosition = cloneVector(station && (station.dockPosition || station.undockPosition || station.position), station && station.position);
    const extraUndockDistance = Math.max(0, toFiniteNumber(options.extraUndockDistance, 0));
    return {
        source: "stored",
        profile: getStationLocatorProfile(station && station.stationTypeID),
        shipUndockCategory: structureLocatorGeometry.getUndockCategoryByShipType(options.shipTypeID),
        locatorCategory: null,
        locatorName: null,
        dockPosition: basePosition,
        dockOrientation: direction,
        undockDirection: direction,
        undockPosition: addVectors(basePosition, scaleVector(direction, extraUndockDistance)),
...
function buildStationDockingGeometry(station, options = {}) {
    ...
    const worldDirection = normalizeVector(transform.rotate(locator.direction), station && (station.dockOrientation || station.undockDirection || DEFAULT_FORWARD));
    const worldPosition = addVectors(cloneVector(station && station.position), transform.rotate(locator.position));
    const extraUndockDistance = Math.max(0, toFiniteNumber(options.extraUndockDistance, 0));
    return {
        source: "authored",
        profile,
        ...
        undockPosition: addVectors(worldPosition, scaleVector(worldDirection, extraUndockDistance)),
...
//# sourceMappingURL=stationLocatorGeometry.js.map