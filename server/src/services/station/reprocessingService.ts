...
function buildRecoverableEntry(recoverable: Record<string, any> = {}) {
  return buildKeyVal([
...
function extractQuoteItemReferences(rawValue) {
  ...
  if (normalized && typeof normalized === "object" && !Array.isArray(normalized)) {
    return Object.entries<any>(normalized)
      .map(([itemID, row]) => ({
        itemID: normalizeNumber(
...
function bucketItemIDsByLocation(itemReferences: any[] = [], fallbackLocationID = 0) {
...
function mergeOutputByTypeID(into, outputByTypeID: Record<string, any> = {}) {
...
function mergeRecoverablesByType(recoverablesByType, recoverables: any[] = []) {
...
function buildOversizedReprocessingSplitPlan(item, context, quoteOptions: Record<string, any> = {}) {
...
function buildSplitAwareQuoteForItem(item, context, quoteOptions: Record<string, any> = {}) {
...
function buildSplitAwareReprocessingQuotesForItems(session, itemReferences: any[] = [], quoteOptions: Record<string, any> = {}) {
...
function materializeOversizedSelectionsForReprocess(session, itemIDs: any[] = [], quoteOptions: Record<string, any> = {}) {
  const contextResult = resolveReprocessingContext(session);
  if (!contextResult.success || !contextResult.data) {
    return contextResult;
  }

  const expandedItemIDs: any[] = [];
  const seen = new Set<any>();
  const splitChanges: any[] = [];
...
class ReprocessingService extends BaseService {
  ...
  Handle_GetQuotes(args, session) {
    ...
    const stationEfficiencyEntries: any[] = [];
  ...
  Handle_Reprocess(args, session) {
    ...
    const aggregatedProcessedItemIDs: any[] = [];
