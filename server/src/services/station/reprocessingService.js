...
const { findItemById, moveItemToLocation, } = require(path.join(__dirname, "../inventory/itemStore"));
const { buildDict, buildKeyVal, buildList, buildBoundObjectResponse, extractDictEntries, extractList, unwrapMarshalValue, normalizeNumber, resolveBoundNodeId, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { buildReprocessingQuoteForItem, buildReprocessingOptionsForTypes, getReprocessingProfile, getReprocessingYieldForType, getStationEfficiencyForTypeID, getStationTaxRate, reprocessItems, resolveAccessibleInventoryItem, resolveReprocessingContext, throwReprocessingError, } = require(path.join(__dirname, "../reprocessing"));
...
function extractQuoteItemReferences(rawValue) {
    ...
    if (dictEntries.length > 0) {
        return dictEntries
            ...
            return {
                itemID: normalizeNumber((row && row.itemID) ??
                    (row && row.fields && row.fields.itemID) ??
                    (normalizedRow && normalizedRow.itemID) ??
                    (normalizedRow && normalizedRow.fields && normalizedRow.fields.itemID), normalizeNumber(entryKey, 0)),
            };
        })
    ...
    if (normalized && typeof normalized === "object" && !Array.isArray(normalized)) {
        return Object.entries(normalized)
            .map(([itemID, row]) => ({
            itemID: normalizeNumber(row && (row.itemID ?? (row.fields && row.fields.itemID)), normalizeNumber(itemID, 0)),
        }))
            .filter((entry) => entry.itemID > 0);
    }
    return [];
...
function bucketItemIDsByLocation(itemReferences = [], fallbackLocationID = 0) {
    ...
    for (const itemReference of Array.isArray(itemReferences) ? itemReferences : []) {
        const itemID = normalizeNumber(itemReference && (itemReference.itemID ?? itemReference), 0);
        if (itemID <= 0) {
            continue;
        }
        const item = findItemById(itemID);
        const locationID = normalizeNumber(item && item.locationID, normalizeNumber(fallbackLocationID, 0));
...
function mergeOutputByTypeID(into, outputByTypeID = {}) {
    ...
    for (const [typeID, quantity] of Object.entries(outputByTypeID || {})) {
        ...
        into.set(numericTypeID, normalizeNumber(into.get(numericTypeID), 0) + normalizeNumber(quantity, 0));
    }
}
...
function getInventoryStackQuantity(item) {
    ...
    return Math.max(0, normalizeNumber(item.stacksize, normalizeNumber(item.quantity, 0)));
}
...
function quoteSetExceedsClientOutputDisplayLimit(quotesByItemID) {
    ...
    for (const quote of quotesByItemID.values()) {
        for (const recoverable of Array.isArray(quote && quote.recoverables) ? quote.recoverables : []) {
            ...
            const nextQuantity = normalizeNumber(outputByTypeID.get(typeID), 0) +
...
function buildOversizedReprocessingSplitPlan(item, context, quoteOptions = {}) {
    ...
    const safeChunkQuantity = findMaximumSafeReprocessingStackQuantity(item, context, quoteOptions);
    ...
    const alignedSafeRootQuantity = originalLeftovers +
        (Math.floor((safeChunkQuantity - originalLeftovers) / portionSize) * portionSize);
    const alignedSplitChunkQuantity = Math.floor(safeChunkQuantity / portionSize) * portionSize;
    if (alignedSafeRootQuantity <= 0 ||
...
function buildSplitAwareQuoteForItem(item, context, quoteOptions = {}) {
    ...
    for (const chunkQuantity of splitPlanResult.data.chunkQuantities) {
        const chunkQuote = buildReprocessingQuoteForItem(buildSyntheticQuoteItem(item, chunkQuantity), context, quoteOptions);
        if (!chunkQuote) {
            continue;
        }
    ...
    const aggregatedQuote = {
        ...
        quantityToProcess: chunkQuotes.reduce((sum, quote) => sum + normalizeNumber(quote && quote.quantityToProcess, 0), 0),
        leftOvers: chunkQuotes.reduce((sum, quote) => sum + normalizeNumber(quote && quote.leftOvers, 0), 0),
        portions: chunkQuotes.reduce((sum, quote) => sum + normalizeNumber(quote && quote.portions, 0), 0),
        numPortions: chunkQuotes.reduce((sum, quote) => sum + normalizeNumber(quote && (quote.numPortions ?? quote.portions), 0), 0),
        totalISKCost: normalizeNumber(chunkQuotes.reduce((sum, quote) => sum + normalizeNumber(quote && quote.totalISKCost, 0), 0).toFixed(2), 0),
        recoverables: [...recoverablesByType.values()].sort((left, right) => normalizeNumber(left && left.typeID, 0) - normalizeNumber(right && right.typeID, 0)),
    };
    return {
        success: true,
        data: {
...
function materializeOversizedSelectionsForReprocess(session, itemIDs = [], quoteOptions = {}) {
    ...
    for (const rawItemID of Array.isArray(itemIDs) ? itemIDs : []) {
        ...
        seen.add(itemID);
        const splitPlanResult = buildOversizedReprocessingSplitPlan(item, contextResult.data, quoteOptions);
        if (!splitPlanResult.success || !splitPlanResult.data) {
            return splitPlanResult;
        }
        ...
        for (const splitQuantity of splitPlanResult.data.chunkQuantities.slice(1)) {
            const splitResult = moveItemToLocation(itemID, normalizeNumber(item.locationID, 0), normalizeNumber(item.flagID, 0), splitQuantity);
            ...
            for (const change of changes) {
                const createdItemID = normalizeNumber(change && change.item && change.item.itemID, 0);
...
class ReprocessingService extends BaseService {
    ...
    Handle_GetOptionsForItemTypes(args) {
        ...
        return buildDict([...optionsByTypeID.entries()].map(([typeID, options]) => [
            typeID,
            buildKeyVal([
    ...
    Handle_GetReprocessingInfo(args, session) {
        ...
        const yieldValue = getReprocessingYieldForType(contextResult.data, commonTypeID);
        return buildKeyVal([
    ...
    Handle_GetQuote(args, session) {
        ...
        if (!quoteResult.success || !quoteResult.data) {
            if (!quoteResult.success && quoteResult.errorMsg) {
                throwReprocessingError(quoteResult.errorMsg, quoteResult.errorValues || quoteResult.values || {});
            }
    ...
    Handle_GetQuotes(args, session) {
        ...
        return [
            ...
            buildDict([...quoteResult.data.quotesByItemID.entries()].map(([itemID, quote]) => [
                itemID,
    ...
    Handle_Reprocess(args, session) {
        ...
        const outputLocationID = args && args.length > 3 && args[3] !== null && args[3] !== undefined
            ? normalizeNumber(args[3], 0)
            : null;
        const outputFlagID = args && args.length > 4 && args[4] !== null && args[4] !== undefined
            ? normalizeNumber(args[4], 0)
            : null;
        const splitMaterializationResult = materializeOversizedSelectionsForReprocess(session, requestedItemIDs, {
            includeRecoverablesFromRandomizedOutputs: false,
        });
        if (!splitMaterializationResult.success || !splitMaterializationResult.data) {
            if (!splitMaterializationResult.success && splitMaterializationResult.errorMsg) {
                throwReprocessingError(splitMaterializationResult.errorMsg, splitMaterializationResult.errorValues || splitMaterializationResult.values || {});
            }
        ...
        const batchRequests = fromLocationID > 0
            ? [
                {
                    itemIDs,
                    fromLocationID,
            ...
            : bucketItemIDsByLocation(itemIDs, fromLocationID).map((bucket) => ({
                ...
                ownerID: outputLocationID === null && outputFlagID === null
                    ? 0
                    : ownerID,
                outputLocationID,
                outputFlagID,
        ...
        for (const batchRequest of batchRequests) {
            ...
            if (!reprocessResult.success || !reprocessResult.data) {
                if (!reprocessResult.success && reprocessResult.errorMsg) {
                    throwReprocessingError(reprocessResult.errorMsg, reprocessResult.errorValues || reprocessResult.values || {});
                }
                continue;
            }
        ...
        return [
            ...
            buildDict([...aggregatedOutputByTypeID.entries()].map(([typeID, quantity]) => [
...
//# sourceMappingURL=reprocessingService.js.map