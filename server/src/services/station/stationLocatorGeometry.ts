...
function cloneVector(vector, fallback: { x: number; y: number; z: number } = DEFAULT_POSITION) {
  return {
...
function squaredDistance(left, right): number {
...
function normalizeVector(vector, fallback: { x: number; y: number; z: number } = DEFAULT_FORWARD) {
...
function normalizeLocator(locator: Record<string, any> = {}) {
  return {
...
function normalizeLocatorProfile(profile: Record<string, any> = {}) {
...
function selectLocator(candidates, options: Record<string, any> = {}) {
...
function buildStoredFallbackGeometry(station, options: Record<string, any> = {}) {
...
function buildStationDockingGeometry(station, options: Record<string, any> = {}) {
...
function getStationDockPosition(station, options: Record<string, any> = {}) {
...
function buildStationUndockSpawnState(station, options: Record<string, any> = {}) {
