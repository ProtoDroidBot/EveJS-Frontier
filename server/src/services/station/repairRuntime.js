...
const { getStationRecord, getStationServiceAccessRule, } = require(path.join(__dirname, "../_shared/stationStaticData"));
...
const { getFittedModuleItems, getTypeAttributeValue, } = require(path.join(__dirname, "../fitting/liveFittingState"));
...
const { findItemById, ITEM_FLAGS, normalizeShipConditionState, updateInventoryItem, updateShipItem, } = require(path.join(__dirname, "../inventory/itemStore"));
const { resolveItemByTypeID, } = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const { getAdjustedAveragePrice, } = require(path.join(__dirname, "../reprocessing/reprocessingStaticData"));
...
function getFallbackRepairBasePrice(item) {
    ...
    const categoryID = toInt(item && item.categoryID !== undefined
        ? item.categoryID
        : itemType && itemType.categoryID, 0);
    const volume = Math.max(0, toFiniteNumber(item && item.volume !== undefined ? item.volume : itemType && itemType.volume, 0));
...
function isRepairableInventoryItem(item) {
    ...
    return (REPAIRABLE_GROUP_IDS.has(groupID) ||
...
function getItemDockedAncestorInfo(item) {
    ...
    const rootLocationID = toInt(rootItem ? rootItem.locationID : leaf && leaf.locationID, 0);
    return {
        chain,
        leaf,
        rootItem,
...
function isItemDirectlyInPersonalHangar(item, context) {
    ...
    return (isRepairableInventoryItem(item) &&
...
function getItemHealthSnapshot(item) {
    ...
    const shieldCapacity = Math.max(0, toFiniteNumber(getTypeAttributeValue(item && item.typeID, "shieldCapacity"), 0));
    const armorHP = Math.max(0, toFiniteNumber(getTypeAttributeValue(item && item.typeID, "armorHP"), 0));
    const structureHP = Math.max(0, toFiniteNumber(getTypeAttributeValue(item && item.typeID, "hp", "structureHP"), 0));
...
function getStructureRepairTaxRate(context) {
    ...
    return Math.max(0, toFiniteNumber(getFacilityTaxRate(context.dockedLocationID, context.structure
        ? {
...
function getRepairCostProfile(item, context) {
    ...
    const staticBasePrice = Math.max(0, toFiniteNumber(getAdjustedAveragePrice(item.typeID), 0));
    const basePrice = staticBasePrice > 0 ? staticBasePrice : getFallbackRepairBasePrice(item);
    ...
    const unitCost = context && context.dockedKind === "structure"
...
function resolveQuotedRepairItemID(session, context, itemID) {
    ...
    const activeShipID = toInt(session && (session.shipid || session.shipID || session.activeShipID), 0);
    if (activeShipID > 0 &&
...
function resolveRepairContext(session) {
    ...
    if (dockedKind === "structure") {
        if (!characterHasStructureService(session, structure, STRUCTURE_REPAIR_SERVICE_ID)) {
            return {
                success: false,
    ...
    const standingInfo = standingRuntime.getCharacterEffectiveStanding(characterID, stationRecord && stationRecord.ownerID);
    return {
        success: true,
        data: {
            characterID,
...
function enforceRepairAccess(context) {
    ...
    const rule = getStationServiceAccessRule(REPAIR_SERVICE_ID, context.stationRecord && context.stationRecord.ownerID);
    ...
    const securityStatus = toFiniteNumber(character.securityStatus ?? character.securityRating, 0);
...
function buildRepairQuotesForSelection(session, itemReferences = []) {
    ...
    session._requestedRepairQuoteItemIDs = new Set((Array.isArray(itemReferences) ? itemReferences : [])
    ...
    for (const itemReference of Array.isArray(itemReferences) ? itemReferences : []) {
        const { requestedItemID, rows } = buildQuoteGroupForRequestedItem(session, context, itemReference);
...
function resolveRepairExecutionItem(context, itemReference) {
    const itemID = decodeDockedShipRepairQuoteItemID(itemReference && (itemReference.itemID ?? itemReference));
    if (itemID <= 0) {
        return null;
    }
    const item = findItemById(itemID);
...
function buildRepairStatePatch(item, repairAmount) {
    ...
    const usesConditionState = (item && item.conditionState && typeof item.conditionState === "object") ||
...
function syncRepairPlanToItems(repairPlan = []) {
    ...
    for (const planEntry of Array.isArray(repairPlan) ? repairPlan : []) {
        ...
        const updateResult = toInt(currentItem.categoryID, 0) === CATEGORY_SHIP
...
function creditStructureRepairTax(context, chargeAmount) {
    ...
    const ownerCorporationID = toInt(context.structure && (context.structure.ownerCorpID || context.structure.ownerID), 0);
    if (ownerCorporationID <= 0) {
        return null;
    }
    const taxCredit = round2(toFiniteNumber(chargeAmount, 0) * (structureTaxRate / (1 + structureTaxRate)));
    if (!(taxCredit > 0)) {
        return null;
    }
    return adjustCorporationWalletDivisionBalance(ownerCorporationID, 1000, taxCredit, {
...
function executeRepairPlan(context, targets, options = {}) {
    ...
    const hasModule = targets.some((target) => toInt(target && target.quote && target.quote.categoryID, 0) === CATEGORY_MODULE);
    const totalFullCost = round2(targets.reduce((sum, target) => sum + toFiniteNumber(target && target.fullCost, 0), 0));
    ...
    for (const target of targets) {
        const unitCost = Math.max(0, toFiniteNumber(target && target.quote && target.quote.costToRepairOneUnitOfDamage, 0));
        const damage = Math.max(0, toFiniteNumber(target && target.quote && target.quote.damage, 0));
        if (!(damage > 0)) {
            continue;
        }
        if (!(unitCost > 0)) {
    ...
    const chargedAmount = round2(repairPlan.reduce((sum, entry) => sum + toFiniteNumber(entry && entry.chargeAmount, 0), 0));
    ...
    if (chargedAmount > 0) {
        ...
        if (chargedAmount - currentBalance > 0.0001) {
            return {
                ...
                errorValues: buildNotEnoughMoneyUserErrorValues(chargedAmount, currentBalance),
            };
        }
        const walletResult = adjustCharacterBalance(context.characterID, -chargedAmount, {
            description: "Repair service charge",
            ownerID1: context.characterID,
            ownerID2: toInt(context.structure
                ? context.structure.ownerCorpID || context.structure.ownerID
                : context.stationRecord && context.stationRecord.ownerID, 0),
            referenceID: context.dockedLocationID,
            entryTypeID: JOURNAL_ENTRY_TYPE.TRANSACTION_TAX,
        });
...
//# sourceMappingURL=repairRuntime.js.map