...
const { buildFiletimeLong, buildKeyVal, buildRowset, currentFileTime, normalizeText, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
const { listCharacterIDs, getCharacterRecord, updateCharacterRecord, writeCharacterRecord, getActiveShipRecord, applyCharacterToSession, flushCharacterSessionNotificationPlan, syncInventoryItemForSession, } = characterState;
const { ITEM_FLAGS, dockShipToLocation, findItemById, } = require(path.join(__dirname, "../inventory/itemStore"));
const { getFittedModuleItems, getTypeAttributeValue, isEffectivelyOnlineModule, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const sessionRegistry = require(path.join(__dirname, "../chat/sessionRegistry"));
const { getCharacterWallet, adjustCharacterBalance, buildNotEnoughMoneyUserErrorValues, JOURNAL_ENTRY_TYPE, } = require(path.join(__dirname, "../account/walletState"));
const { adjustCorporationWalletDivisionBalance, CORPORATION_WALLET_KEY_START, } = require(path.join(__dirname, "../corporation/corpWalletState"));
const { createNotification, } = require(path.join(__dirname, "../notifications/notificationState"));
const { NOTIFICATION_GROUP, } = require(path.join(__dirname, "../notifications/notificationConstants"));
const { canCharacterDockAtStructure, } = require(path.join(__dirname, "../structure/structureState"));
const { STRUCTURE_SERVICE_ID, } = require(path.join(__dirname, "../structure/structureConstants"));
const { structureHasOnlineService, STRUCTURE_SETTING_ID, } = require(path.join(__dirname, "../structure/structureServiceAuthority"));
...
const { ATTRIBUTE_MAX_JUMP_CLONES, ATTRIBUTE_CLONE_JUMP_COOLDOWN, TYPE_CLONE_VAT_BAY_I, JUMP_CLONE_INSTALL_COST, REF_JUMP_CLONE_INSTALLATION_FEE, REF_JUMP_CLONE_ACTIVATION_FEE, NOTIFICATION_TYPE_JUMP_CLONE_DELETED_1, NOTIFICATION_TYPE_JUMP_CLONE_DELETED_2, EVENT_CLONE_JUMP, EVENT_CLONE_DESTRUCTION, EVENT_CLONE_INSTALLATION, EVENT_CLONE_JUMP_TIME_RESET, EVENT_CLONE_DESTROYED_WITH_LOCATION, EVENT_CLONE_IMPLANT_REMOVAL, CLONE_NAME_MAX_LENGTH, getCharacterCloneLimit, getCharacterCloneJumpCooldownHours, } = require(path.join(__dirname, "./jumpCloneRules"));
...
function toFileTimeMs(value) {
    try {
        const filetime = typeof value === "bigint"
            ? value
...
function sortImplantsBySlot(implants = []) {
    return implants
        ...
        .filter(Boolean)
        .sort((left, right) => (toPositiveInt(left.slot, 0) - toPositiveInt(right.slot, 0) ||
...
function mergeImplantsBySlot(baseImplants = [], overrideImplants = []) {
    ...
    return [
        ...[...bySlot.values()].sort((left, right) => (toPositiveInt(left.slot, 0) - toPositiveInt(right.slot, 0) ||
            toPositiveInt(left.typeID, 0) - toPositiveInt(right.typeID, 0))),
        ...unslotted,
    ];
}
...
function normalizeClone(entry = {}, fallbackID = 0, ownerID = 0) {
    const jumpCloneID = toPositiveInt(entry.jumpCloneID ?? entry.cloneID ?? entry.itemID, fallbackID);
    const locationID = toPositiveInt(entry.locationID ?? entry.stationID ?? entry.structureID, 0);
    if (!jumpCloneID || !locationID) {
        return null;
    }
    const locationKind = entry.locationKind ||
    ...
    return {
        ...
        solarSystemID: toPositiveInt(entry.solarSystemID, resolveLocationSolarSystemID(locationID, 0)),
        locationKind,
...
function getNextCloneID(record = {}, normalizedClones = []) {
    const charID = toPositiveInt(record.characterID ?? record.charID ?? record.charid, 0);
    const highestExistingID = normalizedClones.reduce((highest, clone) => Math.max(highest, toPositiveInt(clone && clone.jumpCloneID, 0)), 0);
    return Math.max(toPositiveInt(record.nextJumpCloneID, 0), highestExistingID + 1, charID * 100000 + 90000);
}
function normalizeCloneName(value, fallback = "") {
    return normalizeText(value, fallback).slice(0, CLONE_NAME_MAX_LENGTH);
}
function buildCloneRecord({ jumpCloneID, ownerID, locationID, cloneName, implants = [], locationKind = null, }) {
...
function buildCloneRows(clones = []) {
    return buildRowset(CLONE_ROW_HEADER, clones.map((clone) => [
        clone.jumpCloneID,
        clone.locationID,
        clone.cloneName || "",
    ]), CLIENT_ROWSET_NAME);
...
function buildShipCloneRows(clones = []) {
    return buildRowset(SHIP_CLONE_ROW_HEADER, clones.map((clone) => [
        clone.jumpCloneID,
        clone.ownerID,
        clone.locationID,
    ]), CLIENT_ROWSET_NAME);
...
function buildShipCloneStatePayload(session) {
    const shipID = toPositiveInt(session && (session.shipid ?? session.shipID ?? session.activeShipID), 0);
    const state = getCloneState(session && session.characterID);
    return buildShipCloneRows(state.clones.filter((clone) => (clone.locationKind === "ship" &&
...
function syncCharacterDogmaAfterImplantChange(session, characterID) {
    ...
    try {
        const { syncCharacterDogmaState, } = require(path.join(__dirname, "../dogma/brain/characterBrainRuntime"));
        return syncCharacterDogmaState(session, characterID);
    }
    catch (error) {
        log.warn(`[JumpCloneRuntime] Failed to sync implant dogma state for char=${toPositiveInt(characterID, 0)}: ${error.message}`);
        return false;
    }
}
...
function creditStructureCloneServiceOwner(sourceCharID, location, amount, options = {}) {
    ...
    if (!creditResult || creditResult.success !== true) {
        log.warn(`[JumpCloneRuntime] Failed to credit clone service fee ownerCorp=${ownerCorpID} structure=${location.locationID}: ${creditResult && creditResult.errorMsg ? creditResult.errorMsg : "UNKNOWN"}`);
    }
    return creditResult;
}
...
function getCloneServiceCostForLocation(session, location = getCurrentDockedLocation(session)) {
    ...
    return normalizeCloneServiceCost(getProfileSettingValueForStructure(structure, STRUCTURE_SETTING_ID.CLONINGBAY_TAX, {
        session,
...
function installCloneAtCurrentLocation(session) {
    ...
    debitCharacter(charID, cloneServiceCost, {
        ...
        entryTypeID: JOURNAL_ENTRY_TYPE.JUMP_CLONE_INSTALLATION || REF_JUMP_CLONE_INSTALLATION_FEE,
        ownerID1: charID,
...
function destroyInstalledClone(session, cloneID, options = {}) {
    ...
    if (options.notify !== false) {
        createJumpCloneDeletionNotification(charID, removedClone.implants.map((implant) => implant.typeID), options.destroyerID || charID);
    }
...
function removeJumpClonesAtStructure(structureID, options = {}) {
    ...
    const destroyerID = explicitDestroyerID ||
    ...
    for (const charID of listCharacterIDs()) {
        ...
        if (!result.success) {
            ...
            log.warn(`[JumpCloneRuntime] Failed to remove structure jump clones char=${charID} structure=${locationID} error=${result.errorMsg || "UNKNOWN"}`);
            continue;
        }
        if (removedForCharacter.length === 0) {
            continue;
...
function buildCloneJumpState(session, destLocationID, cloneID, confirmed = false) {
    ...
    const targetClone = state.clones.find((clone) => (clone.jumpCloneID === targetCloneID &&
    ...
    const shouldCheckTimer = targetClone.locationID !== current.locationID ||
    ...
    const suppressCooldownUpdate = current.kind === "structure" &&
        targetClone.locationKind === "structure" &&
        targetClone.locationID === current.locationID;
    const originClone = state.clones.find((clone) => (clone.locationID === current.locationID &&
    ...
    const replacementCloneID = targetClone.locationID === current.locationID
        ? targetClone.jumpCloneID
        : state.nextJumpCloneID;
    const activeFollowImplants = state.implants.filter(implantFollowsJumpClones);
    const oldBodyImplants = state.implants.filter((implant) => !implantFollowsJumpClones(implant));
    ...
    const nextClones = state.clones
        .filter((clone) => (clone.jumpCloneID !== targetClone.jumpCloneID &&
    ...
    return {
        ...
        nextJumpCloneID: replacementCloneID >= state.nextJumpCloneID
...
function buildLocationIdentityPatch(record, solarSystemID, extra = {}) {
    const targetSolarSystemID = toPositiveInt(solarSystemID, toPositiveInt(record.solarSystemID, 30000142));
    const system = worldData.getSolarSystemByID(targetSolarSystemID);
    return {
        ...record,
        ...extra,
        solarSystemID: targetSolarSystemID,
        constellationID: toPositiveInt(system && system.constellationID, toPositiveInt(record.constellationID, 20000020)),
        regionID: toPositiveInt(system && system.regionID, toPositiveInt(record.regionID, 10000002)),
        worldSpaceID: 0,
    };
}
function jumpSessionToStructure(session, structureID) {
    ...
    const updateResult = updateCharacterRecord(session.characterID, (record) => buildLocationIdentityPatch(record, structure.solarSystemID, {
        stationID: null,
    ...
    syncInventoryItemForSession(session, dockResult.data, {
...
function performCloneJump(session, destLocationID, cloneID, cost = null, confirmed = false) {
    ...
    try {
        if (jumpState.targetClone.locationID !== jumpState.originLocationID) {
            if (targetKind === "ship") {
                const { jumpSessionToShipCloneBay } = require(path.join(__dirname, "../../space/transitions"));
                const transitionResult = jumpSessionToShipCloneBay(session, jumpState.targetClone.locationID);
                if (!transitionResult.success) {
                    return transitionResult;
                }
            }
            else if (targetKind === "station") {
                const { jumpSessionToStation } = require(path.join(__dirname, "../../space/transitions"));
                const transitionResult = jumpSessionToStation(session, jumpState.targetClone.locationID);
                if (!transitionResult.success) {
                    return transitionResult;
                }
            }
            else if (targetKind === "structure") {
                const transitionResult = jumpSessionToStructure(session, jumpState.targetClone.locationID);
                if (!transitionResult.success) {
                    return transitionResult;
                }
            }
        ...
        debitCharacter(jumpState.charID, activationCost, {
            ...
            entryTypeID: JOURNAL_ENTRY_TYPE.JUMP_CLONE_ACTIVATION || REF_JUMP_CLONE_ACTIVATION_FEE,
        ...
        creditStructureCloneServiceOwner(jumpState.charID, {
            kind: jumpState.originLocationKind,
            locationID: jumpState.originLocationID,
        }, activationCost, {
...
function getShipCloneCapacity(ship) {
    ...
    return Math.max(0, Math.trunc(Number(getTypeAttributeValue(ship.typeID, "maxJumpClones") || 0) || 0));
}
function getShipSolarSystemID(ship) {
    return toPositiveInt(ship && ship.spaceState && ship.spaceState.systemID, toPositiveInt(ship && ship.locationID, 0));
}
...
function getShipSpaceRadius(ship) {
    return Math.max(0, toFiniteNumber(ship && ship.spaceRadius, 0), toFiniteNumber(ship && ship.spaceState && ship.spaceState.radius, 0));
}
function calculateSurfaceDistanceMeters(sourceShip, targetShip) {
    ...
    return Math.max(0, centerDistance - getShipSpaceRadius(sourceShip) - getShipSpaceRadius(targetShip));
}
function validateCloneVatTargetRange(sourceShip, targetShip) {
    const maxOperationalDistance = Math.max(0, toFiniteNumber(getTypeAttributeValue(sourceShip && sourceShip.typeID, "maxOperationalDistance"), 0));
...
function hasCloneReceivingBay(session) {
    ...
    const canReceive = Number(getTypeAttributeValue(activeShip.typeID, "canReceiveCloneJumps") || 0) > 0;
    if (!canReceive) {
        return false;
    }
    const fittedModules = getFittedModuleItems(charID, activeShip.itemID);
    return fittedModules.some((module) => (Number(module.typeID) === TYPE_CLONE_VAT_BAY_I &&
...
function clearActiveImplantsForPodDeath(session, options = {}) {
    const charID = toPositiveInt(options.characterID || (session && session.characterID), 0);
    if (!charID) {
        return {
            success: false,
    ...
    const result = updateCharacterRecord(charID, (record) => {
        ...
        const survivingImplants = sortImplantsBySlot(activeImplants.filter(implantIsNonDestructible));
        const destroyedImplants = activeImplants.filter((implant) => !implantIsNonDestructible(implant));
    ...
    if (removedTypeIDs.length > 0) {
        createJumpCloneDeletionNotification(charID, removedTypeIDs, options.destroyerID || charID);
        emitCloneInvalidations(session, getCurrentDockedLocation(session).locationID, charID);
        syncCharacterDogmaAfterImplantChange(session, charID);
        log.info(`[JumpCloneRuntime] Cleared ${removedTypeIDs.length} active implants for pod death char=${charID}`);
    }
    return {
        success: true,
        removedTypeIDs,
...
//# sourceMappingURL=jumpCloneRuntime.js.map