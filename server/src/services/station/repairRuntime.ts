...
function throwRepairError(errorMsg, values: Record<string, any> = {}) {
...
function buildRepairQuotesForSelection(session, itemReferences: any[] = []) {
...
function buildLegacyDamageReportsForSelection(session, itemReferences: any[] = []) {
...
function buildRepairExecutionTargets(context, itemReferences: any[] = []) {
  const targets: any[] = [];
  const seen = new Set<any>();
...
function syncRepairPlanToItems(repairPlan: any[] = []) {
  const changes: any[] = [];
...
function executeRepairPlan(context, targets, options: Record<string, any> = {}) {
...
function repairItemsInStation(session, itemReferences: any[] = [], payment = null) {
  const contextResult = resolveRepairContext(session);
  if (contextResult.success === false) {
    return contextResult;
  }
...
function repairItemsInStructure(session, itemReferences: any[] = []) {
  const contextResult = resolveRepairContext(session);
  if (contextResult.success === false) {
    return contextResult;
  }
