...
const { extractDictEntries, extractList, normalizeNumber, normalizeText, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { ITEM_FLAGS, SHIP_CATEGORY_ID, findItemById, getActiveShipItem, listContainerItems, moveItemsAndSetShipPackagingState, setItemPackagingState, setShipPackagingState, } = require(path.join(__dirname, "../inventory/itemStore"));
const { emitFittingTransactionForSession, syncInventoryItemForSession, } = require(path.join(__dirname, "../character/characterState"));
const { isShipFittingFlag, isStrategicCruiserType, } = require(path.join(__dirname, "../fitting/liveFittingState"));
...
function repackageItemsForSession(session, requests, sourceLabel = "RepackagingSvc") {
    const stationID = normalizeNumber(session && (session.stationid || session.stationID), 0);
    const charID = normalizeNumber(session && session.characterID, 0);
    const activeShip = charID > 0 ? getActiveShipItem(charID) : null;
    log.info(`[${sourceLabel}] RepackageItems station=${stationID} requests=${JSON.stringify(requests)}`);
    for (const request of requests) {
        ...
        if (normalizeNumber(item.ownerID, 0) !== charID) {
            log.debug(`[${sourceLabel}] Refusing to repackage item ${item.itemID} not owned by character ${charID}`);
            continue;
        }
        if (normalizeNumber(item.locationID, 0) !== stationID ||
            normalizeNumber(item.flagID, 0) !== ITEM_FLAGS.HANGAR) {
            log.debug(`[${sourceLabel}] Refusing to repackage item ${item.itemID} outside station hangar`);
            continue;
        }
        ...
        if (isShip) {
            if (activeShip &&
                ...
                log.debug(`[${sourceLabel}] Refusing to repackage active ship ${item.itemID}`);
                continue;
            }
            const nestedItems = listContainerItems(null, item.itemID, null);
            let fittingMoveRequests = [];
            if (nestedItems.length > 0) {
                const canAutoStrip = isStrategicCruiser &&
                    nestedItems.every((nestedItem) => (normalizeNumber(nestedItem && nestedItem.ownerID, 0) === charID &&
                        isShipFittingFlag(nestedItem && nestedItem.flagID)));
                if (!canAutoStrip) {
                    log.debug(`[${sourceLabel}] Refusing to repackage item ${item.itemID} with ${nestedItems.length} contained items`);
                    continue;
                }
                fittingMoveRequests = nestedItems
                    .sort((left, right) => normalizeNumber(left && left.itemID, 0) -
            ...
            updateResult = isStrategicCruiser
                ? moveItemsAndSetShipPackagingState(item.itemID, fittingMoveRequests, true)
                : setShipPackagingState(item.itemID, true);
            if (!updateResult.success) {
                log.warn(`[${sourceLabel}] Failed to ${isStrategicCruiser ? "atomically strip/" : ""}` +
                    `repackage ship ${item.itemID}: ` +
                    `${normalizeText(updateResult.errorMsg, "WRITE_ERROR")}`);
                continue;
            }
            if (isStrategicCruiser) {
                emitFittingTransactionForSession(session, item.itemID, updateResult.changes, {
            ...
            if (normalizeNumber(item.singleton, 0) !== 1) {
                log.debug(`[${sourceLabel}] Refusing to repackage item ${item.itemID}: already stackable`);
                continue;
            }
            updateResult = setItemPackagingState(item.itemID, true);
            if (!updateResult.success) {
                log.warn(`[${sourceLabel}] Failed to repackage item ${item.itemID}: ${normalizeText(updateResult.errorMsg, "WRITE_ERROR")}`);
                continue;
            }
        }
...
//# sourceMappingURL=repackagingSupport.js.map