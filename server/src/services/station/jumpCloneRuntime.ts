...
function normalizeImplant(entry: Record<string, any> = {}) {
...
function sortImplantsBySlot(implants: any[] = []) {
  return implants
...
function mergeImplantsBySlot(baseImplants: any[] = [], overrideImplants: any[] = []) {
...
function normalizeClone(entry: Record<string, any> = {}, fallbackID = 0, ownerID = 0) {
...
function normalizeActiveImplants(record: Record<string, any> = {}) {
  return (Array.isArray(record.implants) ? record.implants : [])
    .map(normalizeImplant)
    .filter(Boolean);
}

function normalizeJumpClones(record: Record<string, any> = {}) {
...
function getNextCloneID(record: Record<string, any> = {}, normalizedClones: any[] = []) {
  const charID = toPositiveInt(record.characterID ?? record.charID ?? record.charid, 0);
...
function buildCloneRows(clones: any[] = []) {
  return buildRowset(
    CLONE_ROW_HEADER,
    clones.map((clone) => [
      clone.jumpCloneID,
...
function buildImplantRows(clones: any[] = []) {
  const rows: any[] = [];
...
function buildShipCloneRows(clones: any[] = []) {
  return buildRowset(
...
function emitSessionNotification(session, eventName, payload: any[] = []) {
...
function appendCloneEvent(record, eventTypeID, data: Record<string, any> = {}) {
...
function debitCharacter(charID, amount, options: Record<string, any> = {}) {
  const normalizedAmount = Math.max(0, Number(amount) || 0);
  if (normalizedAmount <= 0) {
    return null;
  }
...
function creditStructureCloneServiceOwner(sourceCharID, location, amount, options: Record<string, any> = {}) {
  const normalizedAmount = Math.max(0, Number(amount) || 0);
...
function createJumpCloneDeletionNotification(charID, typeIDs: any[] = [], destroyerID = charID) {
...
function destroyInstalledClone(session, cloneID, options: Record<string, any> = {}) {
  const charID = toPositiveInt(session && session.characterID, 0);
  const targetCloneID = toPositiveInt(cloneID, 0);
  if (!charID || !targetCloneID) {
    throwWrappedUserError("CustomNotify", { notify: "Jump clone not found" });
...
function removeJumpClonesAtStructure(structureID, options: Record<string, any> = {}) {
  ...
  const affectedCharacterIDs: any[] = [];
  const removedClones: any[] = [];
  const failedCharacterIDs: any[] = [];

  for (const charID of listCharacterIDs()) {
    let removedForCharacter: any[] = [];
    const result = updateCharacterRecord(charID, (record) => {
      const clones = normalizeJumpClones(record);
      const remainingClones: any[] = [];
      removedForCharacter = [];
...
function buildLocationIdentityPatch(record, solarSystemID, extra: Record<string, any> = {}) {
...
function clearActiveImplantsForPodDeath(session, options: Record<string, any> = {}) {
