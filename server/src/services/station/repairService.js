...
const { syncDamageStateAttributesForSession, } = require(path.join(__dirname, "../character/characterState"));
...
const { buildRepairQuotesForSelection, buildLegacyDamageReportsForSelection, repairItemsInStation, repairItemsInStructure, resolveRepairContext, throwRepairError, } = require(path.join(__dirname, "./repairRuntime"));
...
function syncRepairChangesToSession(session, changes = []) {
    for (const change of Array.isArray(changes) ? changes : []) {
        ...
        syncDamageStateAttributesForSession(session, change.item, change.previousData || change.previousState || {});
    }
}
...
class RepairService extends BaseService {
    ...
    Handle_GetRepairQuotes(args, session) {
        ...
        return buildDict([...quoteResult.data.groupedQuotes.entries()].map(([itemID, rows]) => [
    ...
    Handle_GetDamageReports(args, session) {
        ...
        return buildDict([...reportResult.data.reportsByItemID.entries()].map(([itemID, report]) => [
            normalizeNumber(itemID, 0),
    ...
    Handle_RepairItemsInStation(args, session) {
        ...
        const itemReferences = extractRepairItemReferences(args && args[0]);
        const payment = args && args.length > 1 && args[1] !== null && args[1] !== undefined
        ...
        if (!repairResult.success || !repairResult.data) {
            if (!repairResult.success && repairResult.errorMsg) {
                throwRepairError(repairResult.errorMsg, repairResult.errorValues || repairResult.values || {});
            }
            return null;
        }
    ...
    Handle_UnasembleItems(args, session) {
        repackageItemsForSession(session, extractRepackageRequests(args && args[0]), "RepairSvc");
        return null;
    }
}
