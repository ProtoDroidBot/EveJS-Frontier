...
const { getStationRecord, buildStationServiceMask, } = require(path.join(__dirname, "../_shared/stationStaticData"));
...
const { getStationGuestTuples, seedObserverGuestLedger, } = require(path.join(__dirname, "../_shared/guestLists"));
const { listAllAllianceSystems, listAllianceSystems, } = require(path.join(__dirname, "../sovereignty/sovState"));
const { buildAllianceSystemListPayload, } = require(path.join(__dirname, "../sovereignty/sovPayloads"));
...
class StationService extends BaseService {
    ...
    Handle_GetGuests(args, session) {
        ...
        const stationID = (session && (session.stationid || session.stationID)) || 0;
        ...
        seedObserverGuestLedger(session, stationID, tuples.map(([characterID]) => characterID));
    ...
    Handle_GetSystemsForAlliance(args, session) {
        const allianceID = Number(args && args.length > 0 ? args[0] : session && (session.allianceID || session.allianceid)) ||
            0;
