...
function cloneValue(value, depth = 0) {
  ...
  for (const [key, entry] of Object.entries<any>(value).slice(0, MAX_AUDIT_PAYLOAD_ITEMS)) {
...
function recordClientEvent(args: any[] = [], session = null) {
  const event = {
...
function recordClientStats(args: any[] = [], session = null) {
  const event = {
