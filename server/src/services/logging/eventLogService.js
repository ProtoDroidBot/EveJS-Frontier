...
class EventLogService extends BaseService {
    ...
    Handle_LogClientEvent(args, session) {
        ...
        log.debug(`[EventLog] LogClientEvent category=${event.category} event=${event.eventName}`);
        return null;
    }
    Handle_LogClientStats(args, session) {
        const list = Array.isArray(args) ? args : [];
        const event = recordClientStats(list, session);
        log.debug(`[EventLog] LogClientStats location=${event.locationID || ""} rows=${Array.isArray(event.rows) ? event.rows.length : 0}`);
        return null;
    }
