...
const { getAttributeIDByNames, getEffectTypeRecord, getLoadedChargeByFlag, getFittedModuleItems, getTypeEffectRecords, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const { getCachedCharacterSkillMap, getSkillMutationVersion, } = require(path.join(__dirname, "../skills/skillState"));
const { getExpertSystemMutationVersion, } = require(path.join(__dirname, "../skills/expertSystems/expertSystemState"));
const { getDogmaInvalidationVersion, } = require(path.join(__dirname, "../character/dogmaInvalidationVersion"));
const { findItemById, findShipItemById, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const { getActiveImplants, getActiveBoosters, getActiveImplantLocationModifierSources, getActiveImplantSourceStates, getActiveImplantShipModifierEntries, } = require(path.join(__dirname, "../dogma/implants/activeImplantModifiers"));
...
const ATTRIBUTE_ENTITY_CHASE_MAX_DISTANCE = getAttributeIDByNames("entityChaseMaxDistance") || 613;
...
function resolveDroneDogmaItem(droneEntity) {
    ...
    const typeID = firstPositiveInt(droneEntity.typeID, storedItem && storedItem.typeID);
    if (typeID <= 0) {
        return null;
    }
    const customInfo = droneEntity.customInfo !== undefined &&
    ...
    return {
        ...
        itemID,
        typeID,
        groupID: firstPositiveInt(droneEntity.groupID, storedItem && storedItem.groupID),
        categoryID: firstPositiveInt(droneEntity.categoryID, storedItem && storedItem.categoryID),
        ownerID: firstPositiveInt(droneEntity.ownerID, storedItem && storedItem.ownerID),
        locationID: firstPositiveInt(droneEntity.locationID, storedItem && storedItem.locationID, droneEntity.systemID),
        flagID: droneEntity.flagID !== undefined && droneEntity.flagID !== null
            ? toInt(droneEntity.flagID, 0)
            : toInt(storedItem && storedItem.flagID, 0),
        singleton: droneEntity.singleton !== undefined && droneEntity.singleton !== null
            ? toInt(droneEntity.singleton, 1)
            : toInt(storedItem && storedItem.singleton, 1),
        quantity: droneEntity.quantity !== undefined && droneEntity.quantity !== null
            ? droneEntity.quantity
            : storedItem && storedItem.quantity !== undefined
                ? storedItem.quantity
                : 1,
        stacksize: droneEntity.stacksize !== undefined && droneEntity.stacksize !== null
...
function buildDroneSnapshotCacheKey(droneEntity, droneItem = null) {
    ...
    const typeID = firstPositiveInt(resolvedItem && resolvedItem.typeID, droneEntity && droneEntity.typeID);
    const itemID = firstPositiveInt(resolvedItem && resolvedItem.itemID, droneEntity && droneEntity.itemID);
...
function sumDamageVector(vector = {}) {
    return round6(Math.max(0, toFiniteNumber(vector.em, 0)) +
...
function peekCharacterRecord(charId) {
    if (!cachedPeekCharacterRecord) {
        cachedPeekCharacterRecord = require(path.join(__dirname, "../character/characterState")).peekCharacterRecord;
    }
...
function buildControllerDogmaFingerprint(controllerEntity, fittedItems = []) {
    ...
    const controllerCharacterRecord = controllerOwnerID > 0 ? peekCharacterRecord(controllerOwnerID) : null;
    const activeEffectFingerprint = controllerEntity && controllerEntity.activeModuleEffects instanceof Map
        ? [...controllerEntity.activeModuleEffects.values()]
            ...
            .join("|")
        : "";
    const activeImplantFingerprint = controllerOwnerID > 0
    ...
    const activeBoosterFingerprint = controllerOwnerID > 0
...
function buildActiveModuleContexts(controllerEntity, fittedItems = [], characterID = 0) {
    ...
    return [...controllerEntity.activeModuleEffects.values()]
        ...
        return {
            ...
            moduleItem,
            chargeItem: characterID > 0 && toInt(moduleItem && moduleItem.flagID, 0) > 0
                ? getLoadedChargeByFlag(characterID, toInt(controllerEntity && controllerEntity.itemID, 0), toInt(moduleItem && moduleItem.flagID, 0))
                : null,
        };
    })
...
function getControllerDogmaContext(controllerEntity) {
    ...
    const skillMap = controllerOwnerID > 0
    ...
    const implantShipModifierEntries = controllerOwnerID > 0 ? getActiveImplantShipModifierEntries(controllerOwnerID) : [];
...
function buildDroneOperationalAttributes(droneEntity, controllerEntity) {
    ...
    const attributes = buildLocationModifiedAttributeMap(droneItem, context.shipItem, context.skillMap, context.shipModifierAttributes, context.fittedItems, context.activeModuleContexts, {
...
function resolveDroneCombatSnapshot(droneEntity, controllerEntity) {
    ...
    const damageMultiplier = Math.max(0, round6(toFiniteNumber(attributes[ATTRIBUTE_DAMAGE_MULTIPLIER], 1)));
    const rawShotDamage = {
    ...
    if (jammerEffectRecord) {
        const durationMs = Math.max(1, round6(toFiniteNumber(attributes[jammerEffectRecord.durationAttributeID], 20_000)));
        const optimalRange = Math.max(0, round6(toFiniteNumber(attributes[jammerEffectRecord.rangeAttributeID], 0)));
        const falloff = Math.max(0, round6(toFiniteNumber(attributes[jammerEffectRecord.falloffAttributeID], 0)));
        const orbitDistanceMeters = Math.max(500, round6(toFiniteNumber(attributes[ATTRIBUTE_ENTITY_FLY_RANGE], toFiniteNumber(attributes[ATTRIBUTE_ORBIT_RANGE], 500))));
        const attackRangeMeters = Math.max(optimalRange, round6(optimalRange + falloff));
        const chaseRangeMeters = Math.max(attackRangeMeters, round6(Math.max(attackRangeMeters, toFiniteNumber(attributes[ATTRIBUTE_ENTITY_ATTACK_RANGE], 0))));
        const snapshot = {
            ...
            effectKind: "jammer",
            durationMs,
            jamDurationMs: Math.max(1, round6(toFiniteNumber(attributes[ATTRIBUTE_ECM_JAM_DURATION], 5_000))),
            optimalRange,
            falloff,
            orbitDistanceMeters,
            attackRangeMeters,
            ...
            jammerStrengthBySensorType: Object.freeze({
                gravimetric: Math.max(0, round6(toFiniteNumber(attributes[ATTRIBUTE_SCAN_GRAVIMETRIC_STRENGTH_BONUS], 0))),
                ladar: Math.max(0, round6(toFiniteNumber(attributes[ATTRIBUTE_SCAN_LADAR_STRENGTH_BONUS], 0))),
                magnetometric: Math.max(0, round6(toFiniteNumber(attributes[ATTRIBUTE_SCAN_MAGNETOMETRIC_STRENGTH_BONUS], 0))),
                radar: Math.max(0, round6(toFiniteNumber(attributes[ATTRIBUTE_SCAN_RADAR_STRENGTH_BONUS], 0))),
            }),
        };
        context.combatByTypeID.set(cacheKey, snapshot);
        return snapshot;
    }
    const durationMs = Math.max(1, round6(toFiniteNumber(attributes[ATTRIBUTE_SPEED], toFiniteNumber(attributes[ATTRIBUTE_DURATION], 1000))));
    const optimalRange = Math.max(0, round6(toFiniteNumber(attributes[ATTRIBUTE_MAX_RANGE], 0)));
    const falloff = Math.max(0, round6(toFiniteNumber(attributes[ATTRIBUTE_FALLOFF], 0)));
    const trackingSpeed = Math.max(0, round6(toFiniteNumber(attributes[ATTRIBUTE_TRACKING_SPEED], 0)));
    const optimalSigRadius = Math.max(1, round6(toFiniteNumber(attributes[ATTRIBUTE_OPTIMAL_SIG_RADIUS], toFiniteNumber(attributes[ATTRIBUTE_SIGNATURE_RADIUS], 25))));
    const orbitDistanceMeters = Math.max(0, round6(toFiniteNumber(attributes[ATTRIBUTE_ENTITY_FLY_RANGE], toFiniteNumber(attributes[ATTRIBUTE_ORBIT_RANGE], 500))));
    const attackRangeMeters = Math.max(0, round6(Math.max(toFiniteNumber(attributes[ATTRIBUTE_ENTITY_ATTACK_RANGE], 0), optimalRange)));
    const chaseRangeMeters = Math.max(attackRangeMeters, round6(Math.max(attackRangeMeters + falloff, toFiniteNumber(attributes[ATTRIBUTE_ENTITY_CHASE_MAX_DISTANCE], 0))));
    const snapshot = {
...
function resolveDroneMiningSnapshot(droneEntity, controllerEntity) {
    ...
    const attributes = operational.attributes;
    const miningAmountM3 = Math.max(0, round6(toFiniteNumber(attributes[ATTRIBUTE_MINING_AMOUNT], 0)));
    if (miningAmountM3 <= 0) {
        context.miningByTypeID.set(cacheKey, null);
        return null;
    }
    const durationMs = Math.max(1, round6(toFiniteNumber(attributes[ATTRIBUTE_DURATION], toFiniteNumber(attributes[ATTRIBUTE_SPEED], 1000))));
    const snapshot = {
        effectID: toInt(effectRecord.effectID, 0),
        effectName: String(effectRecord.name || ""),
        effectGUID: String(effectRecord.guid || ""),
        durationMs,
        miningAmountM3,
        maxRangeMeters: Math.max(0, round6(toFiniteNumber(attributes[ATTRIBUTE_MAX_RANGE], 0))),
        orbitDistanceMeters: Math.max(0, round6(toFiniteNumber(attributes[ATTRIBUTE_ORBIT_RANGE], toFiniteNumber(attributes[ATTRIBUTE_ENTITY_FLY_RANGE], 200)))),
    };
...
function resolveDroneSalvageSnapshot(droneEntity, controllerEntity) {
    ...
    const attributes = operational.attributes;
    const accessBonusPercent = Math.max(0, round6(toFiniteNumber(attributes[ATTRIBUTE_ACCESS_DIFFICULTY_BONUS], 0)));
    ...
    const snapshot = {
        ...
        accessBonusPercent,
        maxRangeMeters: Math.max(0, round6(toFiniteNumber(attributes[ATTRIBUTE_MAX_RANGE], 0))),
        orbitDistanceMeters: Math.max(0, round6(toFiniteNumber(attributes[ATTRIBUTE_ORBIT_RANGE], toFiniteNumber(attributes[ATTRIBUTE_ENTITY_FLY_RANGE], 500)))),
    };
...
function resolveDroneRepairSnapshot(droneEntity, controllerEntity) {
    ...
    const attributes = operational.attributes;
    const repairAmount = Math.max(0, round6(toFiniteNumber(attributes[definition.amountAttributeID], 0)));
    if (repairAmount <= 0) {
        context.repairByTypeID.set(cacheKey, null);
        return null;
    }
    const durationMs = Math.max(1, round6(toFiniteNumber(attributes[effectRecord && effectRecord.durationAttributeID], toFiniteNumber(attributes[ATTRIBUTE_DURATION], toFiniteNumber(attributes[ATTRIBUTE_SPEED], 1000)))));
    const maxRangeMeters = Math.max(0, round6(toFiniteNumber(attributes[effectRecord && effectRecord.rangeAttributeID], toFiniteNumber(attributes[ATTRIBUTE_MAX_RANGE], 0))));
    const orbitDistanceMeters = Math.max(0, round6(toFiniteNumber(attributes[ATTRIBUTE_ORBIT_RANGE], toFiniteNumber(attributes[ATTRIBUTE_ENTITY_FLY_RANGE], 500))));
    const attackRangeMeters = Math.max(orbitDistanceMeters, maxRangeMeters, round6(toFiniteNumber(attributes[ATTRIBUTE_ENTITY_ATTACK_RANGE], 0)));
    const chaseRangeMeters = Math.max(attackRangeMeters, round6(toFiniteNumber(attributes[ATTRIBUTE_ENTITY_CHASE_MAX_DISTANCE], attackRangeMeters)));
    const snapshot = {
        effectID: toInt(effectRecord.effectID, 0),
        effectName: String(effectRecord.name || ""),
        effectGUID: String(effectRecord.guid || ""),
        ...
        attackRangeMeters,
        chaseRangeMeters,
        shieldBonusAmount: definition.family === "remoteShield" ? repairAmount : 0,
        armorRepairAmount: definition.family === "remoteArmor" ? repairAmount : 0,
        hullRepairAmount: definition.family === "remoteHull" ? repairAmount : 0,
    };
...
//# sourceMappingURL=droneDogma.js.map