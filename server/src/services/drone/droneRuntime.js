...
const { ITEM_FLAGS, findItemById, grantItemToCharacterLocation, listContainerItems, moveItemToLocation, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
const { buildEffectiveItemAttributeMap, buildShipResourceState, getAttributeIDByNames, getTypeAttributeValue, } = require(path.join(__dirname, "../fitting/liveFittingState"));
...
const { DRONE_CATEGORY_ID, isDroneItemRecord, } = require(path.join(__dirname, "../fighter/fighterInventory"));
const { beginDogmaTick, endDogmaTick, resolveDroneOperationalAttributes, resolveDroneCombatSnapshot, resolveDroneMiningSnapshot, resolveDroneSalvageSnapshot, resolveDroneRepairSnapshot, } = require(path.join(__dirname, "./droneDogma"));
const jammerModuleRuntime = require(path.join(__dirname, "../../space/modules/jammerModuleRuntime"));
const assistanceModuleRuntime = require(path.join(__dirname, "../../space/modules/assistanceModuleRuntime"));
const salvagerRuntime = require(path.join(__dirname, "../../space/modules/salvagerRuntime"));
const { hasDamageableHealth, } = require(path.join(__dirname, "../../space/combat/damage"));
const { computeMiningResult, } = require(path.join(__dirname, "../mining/miningMath"));
const { MINING_HOLD_FLAGS, getPreferredMiningHoldFlagForType, getShipHoldCapacityByFlag, } = require(path.join(__dirname, "../mining/miningInventory"));
const { ensureSceneMiningState, getMineableState, applyMiningDelta, } = require(path.join(__dirname, "../mining/miningRuntimeState"));
...
const { createDroneOperationalMotionCommand, } = require(path.join(__dirname, "../../space/destiny/commands/drone.js"));
const STATE_IDLE = 0;
...
const ATTRIBUTE_DRONE_BANDWIDTH_USED = getAttributeIDByNames("droneBandwidthUsed", "droneBandwidthLoad", "droneBandwidth") ||
    1272;
...
const ATTRIBUTE_DRONE_BANDWIDTH = getAttributeIDByNames("droneBandwidth") || 1271;
const ATTRIBUTE_MAX_ACTIVE_DRONES = getAttributeIDByNames("maxActiveDrones") || 352;
...
const ATTRIBUTE_DRONE_IS_AGGRESSIVE = getAttributeIDByNames("droneIsAggressive", "droneIsAgressive") || 1275;
...
function canPlayerCompanionActOnTarget(scene, session, companionEntity, controllerEntity, targetEntity) {
    ...
    const controllerSession = session || resolveDroneControllerSession(companionEntity, controllerEntity);
...
function toNumber(value, fallback = 0, ..._extra) {
...
function syncInventoryItemToSessions(sessions, item, previousData, options = {}) {
    for (const session of normalizeDroneSessions(sessions)) {
        if (!session) {
            continue;
        }
        syncInventoryItemForCharacterSession(session, item, previousData, options);
    }
}
...
function ensureLaunchResponseEntry(result, itemID) {
    ...
    let existingEntry = result.entries.find((entry) => Array.isArray(entry) && toInt(entry[0], 0) === numericItemID);
    if (!existingEntry) {
...
function appendDroneError(result, droneID, message) {
    ...
    const existingEntry = result.entries.find((entry) => Array.isArray(entry) && toInt(entry[0], 0) === numericDroneID);
    if (existingEntry) {
...
function normalizeDroneIDList(rawValue) {
    return [...new Set(flattenDroneCommandValues(rawValue)
...
function getInterestedDroneSessions(entity) {
    ...
    const controllerEntity = entity && entity.systemID
...
function ensureDroneClientIdentityState(entity, shipRecord = null, sessions = null, options = {}) {
    if (!entity) {
        return false;
    }
    const targetSessions = normalizeDroneSessions(Array.isArray(sessions)
        ? sessions
    ...
    if (currentItem && !skipInventorySync) {
        const launchPreviousState = buildDroneLaunchPreviousState(entity, currentItem, shipRecord);
        ...
        syncInventoryItemToSessions(sessionsNeedingIdentity, currentItem, previousInventoryState, {
...
function buildDroneStateNotificationTuple(entity, overrides = {}) {
    ...
    const intField = (key, fallback) => (hasOverride(key) && overrides[key] === null
        ? null
...
function emitDroneDogmaPrime(entity, shipRecord, sessions = null, itemOverride = null) {
    ...
    const currentItem = itemOverride && typeof itemOverride === "object"
        ? itemOverride
    ...
    const dogmaPrimeItem = {
        ...
        typeID: toInt(currentItem && currentItem.typeID, toInt(entity.typeID, 0)),
        ownerID: toInt(currentItem && currentItem.ownerID, toInt(entity.ownerID, 0)),
        locationID: toInt(currentItem && currentItem.locationID, toInt(entity.systemID, 0)),
        flagID: toInt(currentItem && currentItem.flagID, 0),
        quantity: currentItem && currentItem.quantity !== undefined
            ? currentItem.quantity
            : null,
        stacksize: Math.max(1, toInt(currentItem && (currentItem.stacksize ?? currentItem.quantity), 1)),
        singleton: toInt(currentItem && currentItem.singleton, 1),
        groupID: toInt(currentItem && currentItem.groupID, toInt(entity.groupID, 0)),
        categoryID: toInt(currentItem && currentItem.categoryID, DRONE_CATEGORY_ID),
        customInfo: currentItem && currentItem.customInfo !== undefined && currentItem.customInfo !== null
            ? String(currentItem.customInfo)
            : "",
        moduleState: null,
        conditionState: null,
        launcherID: toInt(shipRecord.itemID, 0),
        volume: toNumber(currentItem && currentItem.volume, toNumber(entity.volume, null)),
    };
    ...
    const primeLocationID = toInt(dogmaPrimeItem.locationID, toInt(entity.systemID, 0));
    for (const session of targetSessions) {
        if (!session || typeof session.sendNotification !== "function") {
            continue;
        }
...
function isDroneCombatCapable(droneEntity, controllerEntity = null) {
    ...
    droneEntity.droneCombatCapable = Boolean(resolveDroneCombatSnapshot(droneEntity, controllerEntity) || null);
    return droneEntity.droneCombatCapable;
}
function normalizeDroneBehaviorSettings(rawSettings = null) {
    const source = rawSettings && typeof rawSettings === "object"
        ...
        : {};
    return {
        aggressive: Object.prototype.hasOwnProperty.call(source, ATTRIBUTE_DRONE_IS_AGGRESSIVE)
            ? Boolean(source[ATTRIBUTE_DRONE_IS_AGGRESSIVE])
            : DEFAULT_DRONE_IS_AGGRESSIVE,
        focusFire: Object.prototype.hasOwnProperty.call(source, ATTRIBUTE_DRONE_FOCUS_FIRE)
...
function getControllerDroneBehaviorSettings(controllerEntity = null) {
    ...
    const controllerSession = controllerEntity &&
        controllerEntity.session &&
        ...
        : characterID > 0
            ...
            : null;
    const cachedSettings = controllerSession &&
    ...
    const persistedSettings = characterRecord &&
...
function isCreatedInventoryChange(change) {
    ...
    return previousLocationID === 0 && (previousQuantity === 0 ||
        previousStacksize === 0);
}
function emitRelevantInventoryChanges(session, shipID, changes = [], options = {}) {
    ...
    for (const change of Array.isArray(changes) ? changes : []) {
        ...
        if (currentLocationID !== numericShipID &&
...
function resolveShipStorageSnapshotForDrone(controllerEntity) {
    ...
    const characterID = toInt(controllerEntity &&
        (controllerEntity.session &&
            controllerEntity.session.characterID) ||
        controllerEntity &&
            (controllerEntity.pilotCharacterID ??
                controllerEntity.characterID ??
                controllerEntity.ownerID), 0);
    ...
    const resourceFingerprint = (controllerEntity.droneDogmaCache &&
    ...
    for (const item of listContainerItems(characterID, shipID, null)) {
        ...
        const units = Math.max(0, toInt(item && (item.stacksize ?? item.quantity), 1) || 1);
        const itemVolume = Math.max(0, toNumber(item && item.volume, 0));
        usedByFlag.set(flagID, Number((toNumber(usedByFlag.get(flagID), 0) +
            (itemVolume * units)).toFixed(6)));
    }
    return {
        characterID,
        shipID,
...
function classifyDroneMiningYieldKind(droneEntity) {
    ...
    const droneName = String(typeRecord && typeRecord.name ||
...
function resolveDroneMiningDestination(controllerEntity, yieldTypeID, yieldKind = "") {
    ...
    const preferredFlag = getPreferredMiningHoldFlagForType(storageSnapshot.resourceState, yieldTypeID);
    ...
    for (const flagID of orderedFlags) {
        const availableVolume = getAvailableDroneStorageVolume(storageSnapshot, flagID);
...
function resolveDroneMiningFxTargetID(targetEntityOrID) {
    return toInt(targetEntityOrID && typeof targetEntityOrID === "object"
        ? targetEntityOrID.itemID
        : targetEntityOrID, 0);
}
function emitDroneMiningCycleFx(scene, droneEntity, targetEntityOrID, snapshot, miningState, now, options = {}) {
    ...
    const cycleStartedAtMs = toNumber(miningState && miningState.cycleStartedAtMs, toNumber(now, Date.now()));
    const fxCycleKey = `${targetID}:${Math.trunc(cycleStartedAtMs)}:${durationMs}`;
    if (active && miningState && miningState.fxCycleKey === fxCycleKey) {
        return false;
    }
    scene.broadcastSpecialFx(droneEntity.itemID, snapshot.effectGUID, {
...
function stopDroneMiningCycleFx(scene, droneEntity) {
    ...
    return emitDroneMiningCycleFx(scene, droneEntity, miningState.targetID, miningState.snapshot, miningState, Date.now(), { active: false });
}
function syncDroneInventoryChangesToSession(session, changes = []) {
    ...
    for (const change of Array.isArray(changes) ? changes : []) {
        ...
        syncInventoryItemForCharacterSession(session, change.item, change.previousData || change.previousState || {}, {
            emitCfgLocation: true,
        });
    }
}
...
function copyControllerIdentity(droneEntity, controllerEntity = null, controllerOwnerID = 0) {
    ...
    const fallbackControllerOwnerID = toInt((controllerEntity &&
        controllerEntity.session &&
        controllerEntity.session.characterID) ||
        (controllerEntity &&
            (controllerEntity.pilotCharacterID ??
                controllerEntity.characterID ??
                droneEntity.ownerID)), 0);
    const resolvedControllerOwnerID = toInt(controllerOwnerID, 0) > 0
    ...
    if (controllerEntity) {
        droneEntity.corporationID = toInt(controllerEntity.corporationID, toInt(droneEntity.corporationID, 0));
        droneEntity.allianceID = toInt(controllerEntity.allianceID, toInt(droneEntity.allianceID, 0));
        droneEntity.warFactionID = toInt(controllerEntity.warFactionID, toInt(droneEntity.warFactionID, 0));
...
function applyDroneOperationalEntityAttributes(droneEntity, controllerEntity = null) {
    ...
    const mass = Math.max(1, toNumber(attributes[ATTRIBUTE_MASS], droneEntity.mass || 1));
    const inertia = Math.max(0.05, toNumber(attributes[ATTRIBUTE_AGILITY], droneEntity.inertia || 0.1));
    ...
    const maxVelocity = Math.max(1, toNumber(attributes[ATTRIBUTE_MAX_VELOCITY], droneEntity.maxVelocity || 1));
    ...
    droneEntity.shieldCapacity = Math.max(0, toNumber(attributes[ATTRIBUTE_SHIELD_CAPACITY], droneEntity.shieldCapacity || 0));
    droneEntity.shieldRechargeRate = Math.max(0, toNumber(attributes[ATTRIBUTE_SHIELD_RECHARGE_RATE], droneEntity.shieldRechargeRate || 0));
    droneEntity.armorHP = Math.max(0, toNumber(attributes[ATTRIBUTE_ARMOR_HP], droneEntity.armorHP || 0));
    droneEntity.structureHP = Math.max(0, toNumber(attributes[ATTRIBUTE_STRUCTURE_HP], droneEntity.structureHP || 0));
...
function didDroneClientStateChange(before, entity) {
    ...
    return (before.ownerID !== toInt(entity.ownerID, 0) ||
...
function resolveDroneControllerSession(droneEntity, controllerEntity = null) {
    ...
    const controllerCharacterID = resolveDroneControllerOwnerCharacterID(controllerEntity, droneEntity);
...
function buildDroneCombatSourceEntity(droneEntity, controllerEntity = null) {
    ...
    return {
        ...
        pilotCharacterID: controllerCharacterID || toInt(droneEntity.pilotCharacterID, 0) || null,
    };
}
...
function syncDroneOrbitBehavior(scene, droneEntity, targetEntity, orbitDistanceMeters) {
    ...
    return scene.orbitShipEntity(droneEntity, targetEntity.itemID, Math.max(0, toNumber(orbitDistanceMeters, 0)), {
        broadcast: true,
    });
}
function syncDronePursuitBehavior(scene, droneEntity, targetEntity, followRangeMeters) {
    ...
    return scene.followShipEntity(droneEntity, targetEntity.itemID, Math.max(0, toNumber(followRangeMeters, 0)), {
        broadcast: true,
    });
}
function resolveDroneEngagementRanges(snapshot, fallbackRangeMeters = MIN_ORBIT_DISTANCE_METERS) {
    const orbitDistance = Math.max(MIN_ORBIT_DISTANCE_METERS, toNumber(snapshot && snapshot.orbitDistanceMeters, fallbackRangeMeters));
    const attackRange = Math.max(0, toNumber(snapshot && snapshot.attackRangeMeters, 0));
    const engagementRange = attackRange > 0 ? attackRange : orbitDistance;
    return {
        orbitDistance,
        engagementRange,
        movementRange: Math.min(orbitDistance, engagementRange),
        chaseRange: Math.max(engagementRange, toNumber(snapshot && snapshot.chaseRangeMeters, engagementRange)),
    };
}
...
function getDroneBandwidthLoad(droneItemOrTypeID) {
    const sourceRecord = droneItemOrTypeID && typeof droneItemOrTypeID === "object"
        ? droneItemOrTypeID
        : { typeID: droneItemOrTypeID };
    const itemRecord = sourceRecord &&
    ...
    return Math.max(0, toNumber(attributes[ATTRIBUTE_DRONE_BANDWIDTH_USED], getTypeAttributeValue(typeID, "droneBandwidthUsed", "droneBandwidthLoad", "droneBandwidth"), 0));
}
function resolveDroneRuntimeItemRecord(droneItemOrEntity) {
    const sourceRecord = droneItemOrEntity && typeof droneItemOrEntity === "object"
        ? droneItemOrEntity
...
function resolveDroneOrbitDistance(entity) {
    ...
    const authoredOrbitDistance = toNumber(attributes[ATTRIBUTE_ORBIT_RANGE] ?? getTypeAttributeValue(typeID, "orbitRange"), 0);
    if (authoredOrbitDistance > 0) {
        return clamp(authoredOrbitDistance, MIN_ORBIT_DISTANCE_METERS, MAX_ORBIT_DISTANCE_METERS);
    }
...
function buildDroneLaunchSpaceState(shipEntity, launchIndex = 0) {
    ...
    const launchDistance = Math.max(toNumber(shipEntity && shipEntity.radius, 0), ONE_METER) +
...
function getShipStateForSession(session) {
    ...
    const scene = shipRecord && session && session._space
...
function getSceneControlledCombatDroneIndex(scene) {
    ...
    const expectedCount = scene.droneEntityIDs instanceof Set
        ? scene.droneEntityIDs.size
        : 0;
    if (scene.droneControlledCombatIndex instanceof Map &&
        scene.droneControlledCombatIndexDirty !== true &&
...
function selectAggressiveTargetIDs(scene, controllerEntity, preferredTargetID, options = {}) {
    ...
    const recentTargets = [...controllerThreats.entries()]
        .filter(([targetID, lastAggressedAtMs]) => (toNumber(lastAggressedAtMs, 0) >= now - DRONE_AGGRESSION_THREAT_RETENTION_MS &&
...
function assignDroneCombatTask(scene, droneEntity, controllerEntity, targetEntity, options = {}) {
    if (!scene ||
        ...
        !canPlayerCompanionActOnTarget(scene, options.session || null, droneEntity, controllerEntity, targetEntity)) {
        return {
            success: false,
    ...
    const now = Math.max(0, toNumber(options.nowMs, scene && typeof scene.getCurrentSimTimeMs === "function"
    ...
    if (distanceToTarget > ranges.movementRange + 1) {
        syncDronePursuitBehavior(scene, droneEntity, targetEntity, ranges.movementRange);
        ...
        syncDroneOrbitBehavior(scene, droneEntity, targetEntity, ranges.movementRange);
...
function recallDronesToBay(scene, shipRecord, droneEntities) {
    ...
    const candidates = (Array.isArray(droneEntities) ? droneEntities : [droneEntities])
        .filter((droneEntity) => isDroneEntity(droneEntity) &&
...
function launchDronesForSession(session, rawLaunchRequests) {
    ...
    if (getPlayerCompanionSecurityScope(shipEntity, {
        ...
        for (const request of requests) {
            ...
            appendLaunchError(response, request.itemID, "Unable to launch a drone while the ship's private-site scope is invalid.");
        }
        return {
            success: true,
            response,
    ...
    const shipAttributes = fittingSnapshot && fittingSnapshot.shipAttributes
        ...
        : {};
    const maxActiveDrones = Math.max(0, toInt(shipAttributes[ATTRIBUTE_MAX_ACTIVE_DRONES], 5));
    const droneBandwidth = Math.max(0, toNumber(shipAttributes[ATTRIBUTE_DRONE_BANDWIDTH], 0));
    ...
    let usedBandwidth = activeDroneEntities.reduce((sum, entity) => sum + getDroneBandwidthLoad(entity), 0);
    let launchIndex = 0;
    for (const request of requests) {
        ensureLaunchResponseEntry(response, request.itemID);
        ...
        for (let count = 0; count < request.quantity; count += 1) {
            ...
            const moveResult = moveItemToLocation(refreshedSource.itemID, scene.systemID, 0, 1);
            ...
            const inventoryChanges = Array.isArray(moveResult.data && moveResult.data.changes)
                ? moveResult.data.changes.filter((change) => toInt(change && change.item && change.item.itemID, 0) !==
                    toInt(launchedItem.itemID, 0))
                : [];
            emitRelevantInventoryChanges(session, shipRecord.itemID, inventoryChanges);
            ...
            primeDroneBayDogmaForLaunch(updateResult.data, shipRecord, launchIdentitySessions);
            ...
            syncDroneOrbitBehavior(scene, droneEntity, shipEntity, droneEntity.droneHomeOrbitDistance);
            droneEntity.droneStateVisible = true;
            persistDroneEntityState(droneEntity);
            const splitCreatedLaunch = toInt(droneEntity.itemID, 0) !== toInt(refreshedSource.itemID, 0);
            ...
            ensureDroneClientIdentityState(droneEntity, shipRecord, launchIdentitySessions, {
...
function commandReturnDrones(session, rawDroneIDs, commandName) {
    ...
    for (const droneID of droneIDs) {
        ...
        const followDistance = commandName === DRONE_COMMAND_RETURN_BAY
...
function commandEngage(session, rawDroneIDs, rawTargetID) {
    ...
    for (const droneID of droneIDs) {
        ...
        if (!canPlayerCompanionActOnTarget(scene, session, droneEntity, shipEntity, targetEntity)) {
        ...
        const repairResult = assignDroneRepairTask(scene, droneEntity, shipEntity, targetEntity, assignOptions);
        const assignResult = repairResult && repairResult.success === true
            ? repairResult
            : assignDroneCombatTask(scene, droneEntity, shipEntity, targetEntity, assignOptions);
...
function assignDroneSalvageTask({ scene, session, droneEntity, shipEntity, shipRecord, characterID, targetEntity, } = {}) {
    ...
    if (!canPlayerCompanionActOnTarget(scene, session || null, droneEntity, shipEntity, targetEntity)) {
        return {
            success: false,
    ...
    const chanceSnapshot = salvagerRuntime.buildSalvageChanceSnapshot(targetEntity, snapshot.accessBonusPercent);
    const beforeState = captureDroneClientState(droneEntity);
    stopDroneMiningCycleFx(scene, droneEntity);
    copyControllerIdentity(droneEntity, shipEntity, characterID);
    ...
    droneEntity.droneSalvage = {
        targetID,
        nextCycleAtMs: Math.max(toNumber(scene.getCurrentSimTimeMs && scene.getCurrentSimTimeMs(), Date.now()), Date.now()) + Math.max(1, toNumber(snapshot.durationMs, 1000)),
        snapshot,
        chanceSnapshot,
    };
    droneEntity.droneMining = null;
    ...
    droneEntity.droneRepair = null;
    const distanceToTarget = getEntitySurfaceDistance(droneEntity, targetEntity);
    const orbitDistance = Math.max(MIN_ORBIT_DISTANCE_METERS, toNumber(snapshot.orbitDistanceMeters, MIN_ORBIT_DISTANCE_METERS));
    const maxRange = Math.max(orbitDistance, toNumber(snapshot.maxRangeMeters, orbitDistance));
...
function resolveAutomaticSalvageTarget(scene, droneEntity, shipEntity, characterID, assignedTargetIDs = null, session = null) {
    const entities = scene && typeof scene.getAllVisibleEntities === "function"
    ...
    for (const targetEntity of entities) {
        ...
        if (targetID <= 0 ||
            ...
            !canPlayerCompanionActOnTarget(scene, session, droneEntity, shipEntity, targetEntity)) {
            continue;
        }
...
function commandMineRepeatedly(session, rawDroneIDs, rawTargetID) {
    ...
    const targetIsMineable = Boolean(targetEntity &&
        mineableState &&
        toInt(mineableState.remainingQuantity, 0) > 0);
    const targetIsSalvageable = Boolean(targetEntity &&
        salvagerRuntime.isSalvageableTarget(targetEntity));
    if (!targetIsMineable &&
        !targetIsSalvageable) {
        for (const droneID of droneIDs) {
    ...
    for (const droneID of droneIDs) {
        ...
        if (targetIsMineable) {
            ...
            const beforeState = captureDroneClientState(droneEntity);
            const nowMs = Math.max(toNumber(scene.getCurrentSimTimeMs && scene.getCurrentSimTimeMs(), Date.now()), Date.now());
            stopDroneMiningCycleFx(scene, droneEntity);
            copyControllerIdentity(droneEntity, shipEntity, characterID);
            droneEntity.launcherID = shipRecord.itemID;
            droneEntity.controllerID = shipRecord.itemID;
            ...
            if (droneEntity.activityState === STATE_MINING) {
                emitDroneMiningCycleFx(scene, droneEntity, targetEntity, snapshot, droneEntity.droneMining, nowMs);
            }
        }
        else {
            ...
            if (!assignResult || assignResult.success !== true) {
                appendDroneError(response, droneID, assignResult && assignResult.errorMsg
...
function commandSalvage(session, rawDroneIDs, rawTargetID) {
    ...
    for (const droneID of droneIDs) {
        ...
        const targetEntity = explicitTargetEntity ||
            resolveAutomaticSalvageTarget(scene, droneEntity, shipEntity, characterID, assignedTargetIDs, session);
        if (!targetEntity) {
...
function handleControllerLost(scene, controllerEntity, options = {}) {
    ...
    const shipRecord = options.shipRecord ||
        ...
        null;
    const shouldAttemptBayRecovery = options.attemptBayRecovery === true ||
...
function commandReconnectToDrones(session, rawDroneIDs) {
    ...
    let usedBandwidth = controlledDroneEntities.reduce((sum, entity) => sum + getDroneBandwidthLoad(entity), 0);
    for (const droneID of droneIDs) {
        const droneEntity = scene.getEntityByID(droneID);
        if (!isDroneEntity(droneEntity)) {
            continue;
        ...
        if ((droneBandwidthLoad > 0 && droneBandwidth <= 0) ||
            usedBandwidth + droneBandwidthLoad > droneBandwidth) {
            return buildNotifyErrorResult("Not enough drone bandwidth to reconnect to those drones.");
        }
        droneEntity.controllerID = shipRecord.itemID;
...
function resetDroneToIdle(droneEntity, controllerEntity = null, options = {}) {
    ...
    if (!keepController) {
        ...
        copyControllerIdentity(droneEntity, controllerEntity, toInt(controllerEntity &&
    ...
    if (options.stopMovement === true && controllerEntity) {
        const orbitDistance = Math.max(MIN_ORBIT_DISTANCE_METERS, resolveDroneOrbitDistance(droneEntity));
        syncDroneOrbitBehavior(options.scene, droneEntity, controllerEntity, orbitDistance);
    }
...
function idleMiningDronesTargeting(scene, rawTargetID, options = {}) {
    ...
    for (const droneEntity of getSceneDroneEntities(scene)) {
        ...
        const miningState = droneEntity &&
            ...
            ? droneEntity.droneMining
            : null;
        const miningTargetID = toInt(miningState && miningState.targetID, toInt(droneEntity && droneEntity.targetID, 0));
...
function noteIncomingAggression(attackerEntity, targetEntity, whenMs = Date.now()) {
    ...
    const runtime = getRuntime();
    const scene = runtime && typeof runtime.ensureScene === "function"
    ...
    const controllerEntry = controlledCombatIndex.get(toInt(controllerEntity.itemID, 0)) || null;
    const idleCombatDroneIDs = controllerEntry && Array.isArray(controllerEntry.idleCombatDroneIDs)
        ? controllerEntry.idleCombatDroneIDs
        : [];
    if (idleCombatDroneIDs.length <= 0) {
        return 0;
    }
    const targetIDs = selectAggressiveTargetIDs(scene, controllerEntity, hostileEntity.itemID, {
    ...
    let engagedCount = 0;
    const assignmentCount = behaviorSettings.focusFire === true
    ...
    for (let index = 0; index < assignmentCount; index += 1) {
        ...
        const assignResult = assignDroneCombatTask(scene, droneEntity, controllerEntity, assignedTarget, {
            nowMs: whenMs,
            autoAssigned: true,
            emitActivity: true,
        });
...
function tickDroneCombat(scene, droneEntity, controllerEntity, now) {
    const combatState = droneEntity &&
        droneEntity.droneCombat &&
        typeof droneEntity.droneCombat === "object"
        ? droneEntity.droneCombat
        : null;
    const targetID = toInt(combatState && combatState.targetID, toInt(droneEntity && droneEntity.targetID, 0));
    const targetEntity = targetID > 0 ? scene.getEntityByID(targetID) : null;
    if (!combatState ||
        !controllerEntity ||
        !targetEntity ||
        !hasDamageableHealth(targetEntity) ||
        !canPlayerCompanionActOnTarget(scene, null, droneEntity, controllerEntity, targetEntity)) {
    ...
    if (surfaceDistance > ranges.movementRange + 1 ||
    ...
    if (String(snapshot && snapshot.effectKind || "") === "jammer") {
        ...
        if (cycleResult.success &&
            ...
            runtime.applyJammerCyclePresentation(scene, droneEntity, effectState, now, cycleResult);
        }
    ...
    const combatSourceEntity = buildDroneCombatSourceEntity(droneEntity, controllerEntity) || droneEntity;
    ...
    if (shotResult &&
        ...
        weaponDamageResult = droneInterop.applyWeaponDamageToTarget(scene, droneEntity, targetEntity, shotResult.shotDamage, now);
        ...
        destroyResult = weaponDamageResult && weaponDamageResult.destroyResult
            ...
            : null;
        const appliedDamageAmount = typeof droneInterop.getAppliedDamageAmount === "function"
    ...
    if (typeof droneInterop.notifyWeaponDamageMessages === "function") {
        const damageTargetEntity = weaponDamageResult && weaponDamageResult.impactTargetEntity || targetEntity;
        droneInterop.notifyWeaponDamageMessages(combatSourceEntity, damageTargetEntity, pseudoModuleItem, shotResult && shotResult.shotDamage, typeof droneInterop.getAppliedDamageAmount === "function"
            ? droneInterop.getAppliedDamageAmount(damageResult)
            : 0, typeof droneInterop.getCombatMessageHitQuality === "function"
...
function tickDroneMining(scene, droneEntity, controllerEntity, now) {
    const miningState = droneEntity &&
        ...
        ? droneEntity.droneMining
        : null;
    const targetID = toInt(miningState && miningState.targetID, toInt(droneEntity && droneEntity.targetID, 0));
    ensureSceneMiningState(scene);
    const targetEntity = targetID > 0 ? scene.getEntityByID(targetID) : null;
    const mineableState = getMineableState(scene, targetID);
    if (!miningState ||
    ...
    const snapshot = resolveDroneMiningSnapshot(droneEntity, controllerEntity) ||
    ...
    syncDroneOrbitBehavior(scene, droneEntity, targetEntity, orbitDistance);
    droneEntity.activityState = STATE_MINING;
    if (toNumber(miningState.cycleStartedAtMs, 0) <= 0 ||
        toNumber(miningState.nextCycleAtMs, 0) <= 0) {
        beginDroneMiningCycle(miningState, snapshot, now);
    }
    persistAndNotifyDroneState(droneEntity, beforeState);
    emitDroneMiningCycleFx(scene, droneEntity, targetEntity, snapshot, miningState, now);
    if (toNumber(miningState.nextCycleAtMs, 0) > now) {
        return;
    }
    const destination = resolveDroneMiningDestination(controllerEntity, mineableState.yieldTypeID, mineableState.yieldKind);
    ...
    const quantityVolumeAvailable = Math.max(0, toInt(mineableState.remainingQuantity, 0)) * unitVolume;
    const availableTransferVolume = maximumTransferredQuantity * unitVolume;
    const clampFactor = Math.min(1, quantityVolumeAvailable / miningAmountM3, availableTransferVolume / miningAmountM3);
    if (clampFactor <= 0) {
        resetDroneToIdle(droneEntity, controllerEntity, {
            scene,
            stopMovement: true,
    ...
    const grantResult = grantItemToCharacterLocation(destination.storageSnapshot.characterID, destination.storageSnapshot.shipID, destination.flagID, yieldTypeRecord, transferredQuantity);
    ...
    syncDroneInventoryChangesToSession(resolveDroneControllerSession(droneEntity, controllerEntity), grantResult.data && grantResult.data.changes);
    const pseudoModuleItem = buildDronePseudoModuleItem(droneEntity);
...
function tickDroneSalvage(scene, droneEntity, controllerEntity, now) {
    const salvageState = droneEntity &&
        droneEntity.droneSalvage &&
        typeof droneEntity.droneSalvage === "object"
        ? droneEntity.droneSalvage
        : null;
    const targetID = toInt(salvageState && salvageState.targetID, toInt(droneEntity && droneEntity.targetID, 0));
    const targetEntity = targetID > 0 ? scene.getEntityByID(targetID) : null;
    if (!salvageState ||
        !controllerEntity ||
        !targetEntity ||
    ...
    salvageState.chanceSnapshot = salvagerRuntime.buildSalvageChanceSnapshot(targetEntity, snapshot.accessBonusPercent);
    const orbitDistance = Math.max(MIN_ORBIT_DISTANCE_METERS, toNumber(snapshot.orbitDistanceMeters, MIN_ORBIT_DISTANCE_METERS));
    const maxRange = Math.max(orbitDistance, toNumber(snapshot.maxRangeMeters, orbitDistance));
    const surfaceDistance = getEntitySurfaceDistance(droneEntity, targetEntity);
    const beforeState = captureDroneClientState(droneEntity);
    ...
    const controllerSession = resolveDroneControllerSession(droneEntity, controllerEntity);
    const controllerCharacterID = toInt((controllerSession &&
        (controllerSession.characterID || controllerSession.charid)) ||
        controllerEntity.pilotCharacterID ||
        controllerEntity.characterID ||
        controllerEntity.ownerID, 0);
    const controllerShipItem = findItemById(toInt(controllerEntity && controllerEntity.itemID, 0)) || {
...
function tickDroneRepair(scene, droneEntity, controllerEntity, now) {
    const repairState = droneEntity &&
        droneEntity.droneRepair &&
        typeof droneEntity.droneRepair === "object"
        ? droneEntity.droneRepair
        : null;
    const targetID = toInt(repairState && repairState.targetID, toInt(droneEntity && droneEntity.targetID, 0));
    const targetEntity = targetID > 0 ? scene.getEntityByID(targetID) : null;
    if (!repairState ||
        !controllerEntity ||
        !targetEntity ||
...
function tickScene(scene, now) {
    ...
    try {
        for (const droneEntity of droneEntities) {
            ...
            if (droneEntity.droneCommand === DRONE_COMMAND_ENGAGE &&
                controllerEntity &&
                droneEntity.droneRepair) {
            ...
            if (droneEntity.droneCommand === DRONE_COMMAND_RETURN_HOME && controllerEntity) {
                const orbitDistance = Math.max(resolveDroneOrbitDistance(droneEntity), toNumber(droneEntity.droneHomeOrbitDistance, 0));
...
//# sourceMappingURL=droneRuntime.js.map