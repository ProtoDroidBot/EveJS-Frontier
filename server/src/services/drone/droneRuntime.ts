...
function buildDogmaPrimeEntry(item, options: Record<string, any> = {}) {
  const characterState = getCharacterStateService();
...
function toNumber(value, fallback = 0, ..._extra: any[]) {
...
function normalizeDroneSessions(sessions) {
  ...
  const seenSessions = new Set<any>();
...
function getDroneIdentityPrimeCache(entity) {
  ...
  if (!(entity.clientIdentityPrimedSessionKeys instanceof Set)) {
    entity.clientIdentityPrimedSessionKeys = new Set<any>();
  }
...
function buildMarshalList(items: any[] = []) {
  return {
    type: "list",
...
function buildMultiDroneResult(droneIDs: any[] = []) {
  return buildMarshalDict();
}
...
function flattenDroneCommandValues(rawValue, result: any[] = [], depth = 0) {
...
function buildDroneStateNotificationTuple(entity, overrides: Record<string, any> = {}) {
...
function emitDroneStateChange(entity, overrides: Record<string, any> = {}, sessions = null) {
  const targetSessions = normalizeDroneSessions(
    Array.isArray(sessions)
      ? sessions
      : getInterestedDroneSessions(entity),
...
function emitRelevantInventoryChanges(session, shipID, changes: any[] = [], options: Record<string, any> = {}) {
...
function syncDroneInventoryChangesToSession(session, changes: any[] = []) {
  if (!session || typeof session.sendNotification !== "function") {
    return;
  }
  for (const change of Array.isArray(changes) ? changes : []) {
...
function buildDroneStateRows(entities: any[] = []) {
  return entities
...
function selectAggressiveTargetIDs(scene, controllerEntity, preferredTargetID, options: Record<string, any> = {}) {
...
function assignDroneCombatTask(scene, droneEntity, controllerEntity, targetEntity, options: Record<string, any> = {}) {
  if (
    !scene ||
...
function assignDroneRepairTask(scene, droneEntity, controllerEntity, targetEntity, options: Record<string, any> = {}) {
  if (
    !scene ||
    !isDroneEntity(droneEntity) ||
    !controllerEntity ||
...
function launchDronesForSession(session, rawLaunchRequests) {
  ...
  const launchedDroneEntities: any[] = [];
...
function commandSalvage(session, rawDroneIDs, rawTargetID) {
  ...
  const assignedTargetIDs = new Set<any>();
  for (const droneID of droneIDs) {
    const droneEntity = scene.getEntityByID(droneID);
    if (!isDroneEntity(droneEntity) || toInt(droneEntity.controllerID, 0) !== toInt(shipRecord.itemID, 0)) {
      appendDroneError(response, droneID, "That drone is not currently under this ship's control.");
...
function abandonDroneInSpace(scene, droneEntity, options: Record<string, any> = {}) {
  if (!scene || !isDroneEntity(droneEntity)) {
    return false;
  }
...
function resetDroneToIdle(droneEntity, controllerEntity = null, options: Record<string, any> = {}) {
  if (!droneEntity) {
    return false;
  }
...
function idleMiningDronesTargeting(scene, rawTargetID, options: Record<string, any> = {}) {
  if (!scene) {
    return 0;
  }
  ...
  if (String(snapshot && snapshot.effectKind || "") === "jammer") {
    ...
    const cycleResult = jammerModuleRuntime.executeJammerModuleCycle({
      ...
      callbacks: {
        ...
        clearOutgoingTargetLocksExcept(externalTargetEntity, allowedTargetIDs, options: Record<string, any> = {}) {
