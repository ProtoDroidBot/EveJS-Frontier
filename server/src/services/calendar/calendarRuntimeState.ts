...
function calendarAssert(condition, errorMessage, values: Record<string, any> = {}) {
  if (!condition) {
...
function rebuildIndexes() {
  ...
  const normalizedEvents: Record<string, any> = {};
  let eventRootMutated = false;
  for (const [eventID, rawRecord] of Object.entries<any>(state.eventRoot.events || {})) {
  ...
  for (const [responseKey, rawRecord] of Object.entries<any>(state.responseRoot.responses || {})) {
...
function validateSharedEventFields(eventDateTime, title, description, importance, options: Record<string, any> = {}) {
...
function ensureSeededGlobalEvents(referenceDate = new Date()) {
  ...
  const existingSeedKeys = new Set<any>();
  for (const event of Object.values<any>((state.eventRoot && state.eventRoot.events) || {})) {
...
function editEventRecord(session, eventID, oldDateTime, nextFields: Record<string, any> = {}) {
  ensureLoaded();
...
function updateEventParticipants(session, eventID, charsToAdd: any[] = [], charsToRemove: any[] = []) {
  ensureLoaded();
  const event = requireEvent(eventID);
  calendarAssert(access.canEditOrDeleteEvent(event, session), "CustomNotify", {
  ...
  const inviteeSet = new Set<any>(event.inviteeCharacterIDs);
