...
const { CALENDAR_MAX_DESCRIPTION_SIZE, CALENDAR_MAX_INVITEES, CALENDAR_MAX_TITLE_SIZE, CALENDAR_VIEW_RANGE_IN_MONTHS, EVENT_RESPONSE_ACCEPTED, EVENT_RESPONSE_DECLINED, EVENT_RESPONSE_MAYBE, EVENT_RESPONSE_UNDECIDED, EVENT_RESPONSE_UNINVITED, OWNER_SYSTEM_ID, SOURCE_PLAYER, SOURCE_SERVER, SCOPE_ALLIANCE, SCOPE_CORPORATION, SCOPE_GLOBAL, SCOPE_PERSONAL, } = require(path.join(__dirname, "./calendarConstants"));
...
function rebuildIndexes() {
    ...
    for (const [eventID, rawRecord] of Object.entries(state.eventRoot.events || {})) {
        ...
        if (normalized.scope === SCOPE_PERSONAL) {
            addIndexEntry(state.personalMonthIndex, buildScopedMonthKey(normalized.ownerID, normalized.year, normalized.month), normalized.eventID);
            for (const inviteeCharacterID of normalized.inviteeCharacterIDs) {
                addIndexEntry(state.personalMonthIndex, buildScopedMonthKey(inviteeCharacterID, normalized.year, normalized.month), normalized.eventID);
            }
        }
        else if (normalized.scope === SCOPE_CORPORATION) {
            addIndexEntry(state.corporationMonthIndex, buildScopedMonthKey(normalized.ownerID, normalized.year, normalized.month), normalized.eventID);
        }
        else if (normalized.scope === SCOPE_ALLIANCE) {
            addIndexEntry(state.allianceMonthIndex, buildScopedMonthKey(normalized.ownerID, normalized.year, normalized.month), normalized.eventID);
        }
...
function collectVisibleMonthEventIDs(session, month, year) {
    ...
    for (const eventID of state.personalMonthIndex.get(buildScopedMonthKey(characterID, year, month)) || []) {
        ids.add(eventID);
    }
    if (corporationID > 0) {
        for (const eventID of state.corporationMonthIndex.get(buildScopedMonthKey(corporationID, year, month)) || []) {
            ids.add(eventID);
        }
    }
    if (allianceID > 0) {
        for (const eventID of state.allianceMonthIndex.get(buildScopedMonthKey(allianceID, year, month)) || []) {
            ids.add(eventID);
        }
    }
...
function getEventDetails(session, eventID, ownerID) {
    ...
    calendarAssert(ownerID == null || access.toPositiveInt(ownerID, event.ownerID) === event.ownerID, "CustomNotify", { notify: "Calendar event owner mismatch." });
...
function validateSharedEventFields(eventDateTime, title, description, importance, options = {}) {
    ...
    calendarAssert(normalizedTitle.length <= CALENDAR_MAX_TITLE_SIZE, "CalendarTitleTooLong");
    const normalizedDescription = access.sanitizeDescription(description);
    calendarAssert(normalizedDescription.length <= CALENDAR_MAX_DESCRIPTION_SIZE, "CustomNotify", { notify: "Calendar event description is too long." });
    ...
    if (!options.allowAnyDate) {
        ...
        calendarAssert(eventDate.getTime() > Date.now(), "CalendarCannotPlanThePast");
        ...
        calendarAssert(eventMonthIndex - currentMonthIndex <= CALENDAR_VIEW_RANGE_IN_MONTHS, "CalendarTooFarIntoFuture", { numMonths: CALENDAR_VIEW_RANGE_IN_MONTHS });
    }
    return {
...
function createEventRecord({ session, scope, eventDateTime, duration, title, description, importance, inviteeCharacterIDs = [], }) {
    ensureLoaded();
    const sanitized = validateSharedEventFields(eventDateTime, title, description, importance);
    const characterID = access.getSessionCharacterID(session);
    const corporationID = access.getSessionCorporationID(session);
    const allianceID = access.getSessionAllianceID(session);
    let ownerID = characterID;
    if (scope === SCOPE_CORPORATION) {
        calendarAssert(access.canCreateCorpOrAllianceEvent(session, scope), "CustomNotify", { notify: "You do not have permission to create corporation calendar events." });
        ownerID = corporationID;
    }
    else if (scope === SCOPE_ALLIANCE) {
        calendarAssert(access.canCreateCorpOrAllianceEvent(session, scope), "CustomNotify", { notify: "You do not have permission to create alliance calendar events." });
        ownerID = allianceID;
    }
    ...
    const event = store.normalizeEventRecord({
        ...
        updatedAt: access.currentFileTimeString(),
        inviteeCharacterIDs: scope === SCOPE_PERSONAL
            ? [...new Set((Array.isArray(inviteeCharacterIDs) ? inviteeCharacterIDs : [])
                    .map((entry) => access.toPositiveInt(entry, 0))
                    .filter((entry) => entry > 0 && entry !== characterID))].slice(0, CALENDAR_MAX_INVITEES)
            : [],
        seedKey: null,
        serverEditable: false,
    });
    calendarAssert(event.inviteeCharacterIDs.length <= CALENDAR_MAX_INVITEES, "CustomNotify", { notify: "Too many invitees selected for that event." });
...
function createServerEventRecord({ ownerID = OWNER_SYSTEM_ID, creatorID = OWNER_SYSTEM_ID, scope = SCOPE_GLOBAL, eventDateTime, duration, title, description, importance, autoEventType = null, seedKey = null, serverEditable = true, allowAnyDate = false, }) {
    ensureLoaded();
    const sanitized = validateSharedEventFields(eventDateTime, title, description, importance, { allowAnyDate });
    ...
    state.eventRoot.events[String(event.eventID)] = event;
    if (scope !== SCOPE_GLOBAL) {
        upsertResponse(event, access.toPositiveInt(creatorID, OWNER_SYSTEM_ID), EVENT_RESPONSE_ACCEPTED);
    }
    store.persistEventRoot(state.eventRoot);
    store.persistResponseRoot(state.responseRoot);
    rebuildIndexes();
...
function ensureSeededGlobalEvents(referenceDate = new Date()) {
    ensureLoaded();
    const base = referenceDate instanceof Date && Number.isFinite(referenceDate.getTime())
        ? referenceDate
        : new Date();
...
function editEventRecord(session, eventID, oldDateTime, nextFields = {}) {
    ...
    calendarAssert(access.toFileTimeBigInt(oldDateTime, 0n) ===
        access.toFileTimeBigInt(event.eventDateTime, 0n), "CustomNotify", { notify: "The calendar event changed before it could be edited." });
    const sanitized = validateSharedEventFields(nextFields.eventDateTime, nextFields.title, nextFields.description, nextFields.importance);
    ...
    if (previousEventDateTime !== normalizedEvent.eventDateTime) {
        for (const response of state.responsesByEvent.get(normalizedEvent.eventID)?.values() || []) {
            if (response.characterID !== normalizedEvent.creatorID &&
                [
                    EVENT_RESPONSE_ACCEPTED,
...
function updateEventParticipants(session, eventID, charsToAdd = [], charsToRemove = []) {
    ...
    const addedCharacterIDs = [...new Set((Array.isArray(charsToAdd) ? charsToAdd : [])
            .map((entry) => access.toPositiveInt(entry, 0))
            .filter((entry) => entry > 0 && entry !== event.creatorID))];
    const removedCharacterIDs = [...new Set((Array.isArray(charsToRemove) ? charsToRemove : [])
            .map((entry) => access.toPositiveInt(entry, 0))
            .filter((entry) => entry > 0 && entry !== event.creatorID))];
...
function sendEventResponse(session, eventID, ownerID = null, response) {
    ...
    if (event.scope === SCOPE_PERSONAL) {
        calendarAssert(event.ownerID === characterID || event.inviteeCharacterIDs.includes(characterID), "CustomNotify", { notify: "You are not invited to that calendar event." });
    }
    ...
    calendarAssert([
        ...
        EVENT_RESPONSE_MAYBE,
        EVENT_RESPONSE_UNDECIDED,
    ].includes(normalizedResponse), "CustomNotify", { notify: "Invalid calendar response." });
...
//# sourceMappingURL=calendarRuntimeState.js.map