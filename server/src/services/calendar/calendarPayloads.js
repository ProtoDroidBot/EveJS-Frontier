...
const { buildDict, buildFiletimeLong, buildKeyVal, buildList, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { resolveCalendarTag, toPositiveInt, } = require(path.join(__dirname, "./calendarAccess"));
function buildEventRow(event, session) {
    return buildKeyVal([
        ...
        [
            "flag",
            resolveCalendarTag(event && event.ownerID, session, event && event.autoEventType),
        ],
        [
            "autoEventType",
...
function buildEventListPayload(events, session) {
    return [
        buildList((Array.isArray(events) ? events : []).map((event) => buildEventRow(event, session))),
        null,
        null,
    ];
}
function buildResponsesForCharacterPayload(rows) {
    return buildList((Array.isArray(rows) ? rows : []).map((row) => buildCharacterResponseRow(row)));
}
function buildResponsesToEventPayload(rows) {
    return buildList((Array.isArray(rows) ? rows : []).map((row) => buildEventResponseRow(row)));
}
...
function buildServerEditorSnapshot(payload = {}) {
    return buildDict(Object.entries(payload).map(([key, value]) => [key, value]));
}
module.exports = {
    buildEventRow,
    buildEventDetails,
...
//# sourceMappingURL=calendarPayloads.js.map