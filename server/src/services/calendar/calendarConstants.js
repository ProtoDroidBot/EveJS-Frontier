...
function ensureCalendarTablesExist() {
    if (!repo.read(CALENDAR_EVENTS_TABLE, "/").success) {
        repo.write(CALENDAR_EVENTS_TABLE, "/", JSON.parse(JSON.stringify(DEFAULT_EVENT_STATE)));
    }
    if (!repo.read(CALENDAR_RESPONSES_TABLE, "/").success) {
        repo.write(CALENDAR_RESPONSES_TABLE, "/", JSON.parse(JSON.stringify(DEFAULT_RESPONSE_STATE)));
    }
}
module.exports = {
