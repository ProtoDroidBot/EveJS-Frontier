...
const { OWNER_SYSTEM_ID, SCOPE_ALLIANCE, SCOPE_CORPORATION, SCOPE_GLOBAL, SCOPE_PERSONAL, } = require(path.join(__dirname, "./calendarConstants"));
...
function buildFutureFiletimeString(daysAhead, hour = 11, minute = 11) {
    ...
    return (BigInt(target.getTime()) * 10000n + 116444736000000000n).toString();
}
function notifyCreatedEvent(event, options = {}) {
    notifications.notifyNewCalendarEvent(notifications.listOnlineViewerCharacterIDs(event, options), event, options);
}
...
function executeCalendarAutoCommand(session, rawText = "") {
    ...
    try {
        ...
        if (subcommand === "status") {
            ...
            return {
                success: true,
                message: `Events=${summary.totalEvents} responses=${summary.totalResponses} ` +
...
//# sourceMappingURL=calendarChatCommands.js.map