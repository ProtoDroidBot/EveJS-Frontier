...
function normalizeEventRecord(record, eventIDHint: number | string = 0) {
...
function getMutableEventRoot() {
  ...
  for (const [eventID, record] of Object.entries<any>(root.events)) {
...
function getMutableResponseRoot() {
  ...
  for (const [responseKey, record] of Object.entries<any>(root.responses)) {
