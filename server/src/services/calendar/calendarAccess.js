...
const { normalizeBigInt, normalizeNumber, normalizeText, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { CORP_ROLE_CHAT_MANAGER, OWNER_SYSTEM_ID, CALENDAR_TAG_PERSONAL, CALENDAR_TAG_CORP, CALENDAR_TAG_ALLIANCE, CALENDAR_TAG_CCP, CALENDAR_TAG_AUTOMATED, SCOPE_PERSONAL, SCOPE_CORPORATION, SCOPE_ALLIANCE, SCOPE_GLOBAL, } = require(path.join(__dirname, "./calendarConstants"));
...
function filetimeToDate(value) {
    ...
    const unixMilliseconds = Number((filetime - FILETIME_EPOCH_OFFSET) / FILETIME_TICKS_PER_MILLISECOND);
...
function dateToFiletimeString(dateValue) {
    ...
    return (BigInt(milliseconds) * FILETIME_TICKS_PER_MILLISECOND + FILETIME_EPOCH_OFFSET).toString();
}
...
function sessionHasChatManagerRole(session) {
    return ((getSessionCorpRole(session) & CORP_ROLE_CHAT_MANAGER) ===
...
function canEditOrDeleteEvent(event, session) {
    ...
    if (isCorporationScope(event)) {
        return (toPositiveInt(event.ownerID, 0) === getSessionCorporationID(session) &&
            sessionHasChatManagerRole(session));
    }
    if (isAllianceScope(event)) {
        return (toPositiveInt(event.ownerID, 0) === getSessionAllianceID(session) &&
            sessionHasChatManagerRole(session));
    }
    return false;
}
...
//# sourceMappingURL=calendarAccess.js.map