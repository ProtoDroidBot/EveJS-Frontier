...
const { buildExternalResponseEventKV, } = require(path.join(__dirname, "./calendarPayloads"));
const { canViewEvent, getSessionCharacterID, toPositiveInt, } = require(path.join(__dirname, "./calendarAccess"));
function getCharacterSessions(characterID, options = {}) {
    ...
    return sessionRegistry.getSessions().filter((session) => {
        ...
        return (toPositiveInt(session &&
            (session.characterID || session.charID || session.charid || 0), 0) === numericCharacterID);
    });
}
...
function listOnlineViewerCharacterIDs(event, options = {}) {
    ...
    const excludedSession = options.excludeSession || null;
    const excludedCharacterIDs = new Set((Array.isArray(options.excludeCharacterIDs)
...
function notifyNewCalendarEvent(characterIDs, event, options = {}) {
    sendToCharacterIDs(characterIDs, "OnNewCalendarEvent", [
...
function notifyEditCalendarEvent(characterIDs, event, oldEventDateTime, options = {}) {
    sendToCharacterIDs(characterIDs, "OnEditCalendarEvent", [
        toPositiveInt(event && event.eventID, 0),
        toPositiveInt(event && event.ownerID, 0),
...
function notifyRemoveCalendarEvent(characterIDs, event, isDeleted, options = {}) {
    sendToCharacterIDs(characterIDs, "OnRemoveCalendarEvent", [
        toPositiveInt(event && event.eventID, 0),
        buildFiletimeLong(event && event.eventDateTime),
        Boolean(isDeleted),
    ], options);
...
function notifyExternalEventResponse(characterIDs, event, response, options = {}) {
    sendToCharacterIDs(characterIDs, "OnEventResponseByExternal", [
        toPositiveInt(event && event.eventID, 0),
...
//# sourceMappingURL=calendarNotifications.js.map