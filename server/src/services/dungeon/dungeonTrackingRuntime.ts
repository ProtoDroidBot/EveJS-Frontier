...
const dungeonVisibilityPolicy = require(path.join(
  __dirname,
...
const {
  ...
  __dirname,
  "../../space/destiny/identity/entityID.js",
));
...
function buildDungeonValues(_instance?: Record<string, any>, _roomKey?: string, _options?: Record<string, any>): ReturnType<typeof buildDict>;
...
function buildCurrentDungeonInfo(instance, roomKey, options: Record<string, any> = {}) {
...
function markShipCurrentDungeonRoom(shipEntity, instance, roomKey, options: Record<string, any> = {}) {
  ...
  shipEntity.dungeonCurrentSiteID = normalizePersistentEntityID(
    hasExplicitSiteID
      ? options.siteID
...
function resolveTrackedDungeonForShip(shipEntity, options: Record<string, any> = {}) {
...
function inferEntryDungeonForShip(scene, shipEntity, session = null) {
  ...
  for (const entity of Array.isArray(scene.staticEntities) ? scene.staticEntities : []) {
    ...
    if (session) {
      if (!dungeonVisibilityPolicy.resolveDungeonInstanceVisibilityForSession(
        session,
        instance.instanceID,
...
function resolveAuthorizedTrackedDungeonForSession(scene, session, shipEntity) {
  ...
  const authorized = dungeonVisibilityPolicy.canSessionAccessDungeonInstance(
    session,
    tracked.instance,
  );
  return authorized
...
function resolveCurrentDungeonForSession(scene, session) {
  ...
  return (
    resolveAuthorizedTrackedDungeonForSession(scene, session, shipEntity) ||
...
function resolveTrackedDungeonForSession(scene, session) {
  ...
  const tracked = resolveAuthorizedTrackedDungeonForSession(
    scene,
    session,
    shipEntity,
  );
...
function sendEnteringDungeonRoomNotification(session, instance, roomKey, options: Record<string, any> = {}) {
...
function clearShipCurrentDungeonRoom(shipEntity, options: Record<string, any> = {}) {
  if (!shipEntity) {
    return false;
  }
...
function completeCurrentDungeonForSession(scene, session, instanceOrID, options: Record<string, any> = {}) {
...
function listSceneSessions(scene) {
  ...
  if (scene.sessions instanceof Map) {
    return Array.from<any>(scene.sessions.values());
  }
  if (Array.isArray(scene.sessions)) {
    return scene.sessions;
  }
  if (typeof scene.sessions === "object") {
    return Object.values<any>(scene.sessions);
  }
  return [];
}

function notifyDungeonCompletedForScene(scene, instanceOrID, options: Record<string, any> = {}) {
  const instance = resolveDungeonInstance(instanceOrID);
  if (!scene || !instance) {
    return 0;
  }
...
function entityBelongsToTrackedDungeon(entity, instance, shipEntity) {
  ...
  const entitySiteID = normalizePersistentEntityID(
    hasIdentityValue(entity.dungeonSiteID)
...
function shouldPreserveDungeonForWarp(scene, current, options: Record<string, any> = {}) {
  if (!current || !current.instance || !current.shipEntity) {
    return false;
  }
  if (
...
function exitCurrentDungeonForSession(scene, session, options: Record<string, any> = {}) {
  const current = resolveTrackedDungeonForSession(scene, session);
  if (!current) {
    return false;
  }
...
function enterDungeonRoomForSession(scene, session, instance, roomKey, options: Record<string, any> = {}) {
  const shipEntity = getShipEntityForSession(scene, session);
  if (!shipEntity || !instance) {
    return false;
  }
