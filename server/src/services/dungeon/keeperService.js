...
const dungeonUniverseSiteService = require(path.join(__dirname, "./dungeonUniverseSiteService"));
...
const operationSiteRuntime = require(path.join(__dirname, "./operationSiteRuntime"));
...
function getCharacterID(session) {
    return Math.max(0, toInt(session && (session.characterID ||
        session.charid ||
...
function resolveOrderedRoomKeys(instance, template) {
    ...
    const dynamicRoomKeys = Object.keys(roomStatesByKey)
        ...
        .sort((left, right) => (toInt(left.split(":").pop(), 0) - toInt(right.split(":").pop(), 0)) || left.localeCompare(right));
...
function findTemplateRoomProfile(sceneProfile, roomKey) {
    ...
    return sceneProfile.roomProfiles.find((roomProfile) => (normalizeText(roomProfile && roomProfile.roomKey, "") === normalizedRoomKey)) || null;
}
function findTemplateGateProfile(sceneProfile, gateEntity) {
    ...
    const dungeonObjectID = Math.max(0, toInt(gateEntity && (gateEntity.dungeonObjectID ||
        gateEntity.dunObjectID), 0));
    return sceneProfile.gateProfiles.find((gateProfile) => {
        ...
        const profileObjectID = Math.max(0, toInt(gateProfile.dunObjectID || gateProfile.fromObjectID, 0));
...
function resolveExactGateDestinationPoint(sitePosition, gateEntity, template, destinationRoomKey) {
    ...
    const sourceRoomKey = normalizeText(gateEntity && gateEntity.dungeonGateSourceRoomKey, normalizeText(gateProfile && gateProfile.roomKey, ""));
...
function resolveGateDestinationPoint(siteEntity, gateEntity, instance, template, destinationRoomKey) {
    const sitePosition = clonePosition((siteEntity && siteEntity.position) ||
        (gateEntity && gateEntity.position));
    const exactDestinationPoint = resolveExactGateDestinationPoint(sitePosition, gateEntity, template, destinationRoomKey);
    ...
    const radialDirection = normalizeVector(subtractVectors(gatePosition, sitePosition), { x: 1, y: 0, z: 0 });
    const baseDistance = ROOM_ENTRY_BASE_DISTANCE_METERS +
...
function resolveGateActivationRange(gateEntity) {
    return (toFiniteNumber(gateEntity && gateEntity.gateActivationRange, 0) ||
        DEFAULT_ACCELERATION_GATE_ACTIVATION_RANGE_METERS);
}
function resolveGateInteractionLimit(gateEntity, shipEntity) {
    return Math.max(DEFAULT_ACCELERATION_GATE_ACTIVATION_RANGE_METERS, toFiniteNumber(gateEntity && gateEntity.radius, 0) +
...
function resolveShipTypeContext(shipEntity) {
    const typeID = Math.max(0, toInt(shipEntity && (shipEntity.slimTypeID || shipEntity.typeID), 0));
    const typeRecord = typeID > 0 ? resolveItemByTypeID(typeID) : null;
    return {
        typeID,
        groupID: Math.max(0, toInt(shipEntity && (shipEntity.slimGroupID || shipEntity.groupID), typeRecord && typeRecord.groupID)),
        categoryID: Math.max(0, toInt(shipEntity && (shipEntity.slimCategoryID || shipEntity.categoryID), typeRecord && typeRecord.categoryID)),
        raceID: Math.max(0, toInt(shipEntity && shipEntity.raceID, typeRecord && typeRecord.raceID)),
    };
}
function resolveGateAllowedShipsList(gateEntity, gateState) {
    return Math.max(0, toInt(gateEntity && gateEntity.dungeonGateAllowedShipsList, gateState &&
        gateState.metadata &&
...
function resolveGateKeyLock(gateEntity, gateState) {
    for (const source of listGateRequirementSources(gateEntity, gateState)) {
        const keyLock = normalizeGateKeyLock(firstDefined(source.dungeonGateKeyLock, source.keyLock, source.keylock, source.keylockType, source.keyLockType, source.dungeonKeylock, source.dungeonKeyLock));
        if (keyLock !== null) {
            return keyLock;
        }
    }
...
function normalizeRequiredItemEntry(value, defaults = {}) {
    ...
    const typeID = Math.max(0, toInt(firstDefined(value.typeID, value.itemTypeID, value.requiredTypeID, value.requiredItemTypeID, value.requiredKeyTypeID, value.keyTypeID, value.keyItemTypeID, value.passcardTypeID), 0));
    if (typeID <= 0) {
        return null;
    }
    return {
        typeID,
        quantity: Math.max(1, toInt(firstDefined(value.quantity, value.qty, value.amount, value.requiredQuantity, value.keyQuantity, value.passcardQuantity, defaults.quantity), 1)),
        consume: normalizeBoolean(firstDefined(value.consume, value.consumed, value.consumeKey, value.consumeItem, defaults.consume), false),
    };
}
...
function resolveGateRequiredItems(gateEntity, gateState) {
    const requirements = [];
    for (const source of listGateRequirementSources(gateEntity, gateState)) {
        const defaultQuantity = firstDefined(source.dungeonGateRequiredItemQuantity, source.requiredItemQuantity, source.requiredQuantity, source.keyQuantity, source.passcardQuantity, 1);
        const defaultConsume = firstDefined(source.dungeonGateConsumesRequiredItems, source.consumeRequiredItems, source.consumeRequiredItem, source.consumeKey, source.consumeItem, false);
...
function formatRequiredItem(requirement) {
    ...
    const typeRecord = typeID > 0 ? resolveItemByTypeID(typeID) : null;
    const itemName = normalizeText(typeRecord && (typeRecord.typeName || typeRecord.name || typeRecord.itemName), typeID > 0 ? `type ${typeID}` : "the required key item");
...
function assertGateItemRequirements(session, shipEntity, gateEntity, gateState, options = {}) {
    ...
    const missing = requirement.requiredItems.filter((entry) => (getCargoTypeQuantity(cargoItems, entry.typeID) < Math.max(1, toInt(entry.quantity, 1))));
    if (missing.length > 0) {
...
function assertShipMayUseGate(shipEntity, gateEntity, gateState) {
    ...
    const allowedRaces = normalizeIDSet(gateState && gateState.metadata && gateState.metadata.allowedRaces);
    const hasExplicitTypeOrGroupRestriction = allowedShipTypeIDs.size > 0 ||
        allowedShipGroupIDs.size > 0;
    const hasNonDefaultRestriction = hasExplicitTypeOrGroupRestriction ||
        allowedShipsList > 0 ||
        allowedRaces.size > 0;
    if (hasExplicitTypeOrGroupRestriction &&
...
function activateAccelerationGateForSession(session, gateEntityOrID, options = {}) {
    ...
    const visibilityResolution = dungeonVisibilityPolicy.resolveDungeonInstanceVisibilityForSession(session, instanceID, { systemID: scene.systemID });
    if (!visibilityResolution ||
        Math.max(0, toInt(visibilityResolution.instance &&
            visibilityResolution.instance.metadata &&
            visibilityResolution.instance.metadata.siteID, 0)) !== siteID) {
        throwWrappedUserError("DeniedTargetAttemptFailed");
    }
    ...
    let gateState = instance.gateStatesByKey &&
    ...
    const destinationRoomKey = normalizeText(gateEntity.dungeonGateDestinationRoomKey, normalizeText(gateState.destinationRoomKey, "")) || null;
    const siteEntity = resolveSceneEntity(scene, siteID);
    const template = dungeonAuthority.getTemplateByID(instance.templateID) || null;
    const destinationPoint = resolveGateDestinationPoint(siteEntity, gateEntity, instance, template, destinationRoomKey);
    const teleportDirection = normalizeVector(subtractVectors(destinationPoint, gateEntity.position), shipEntity.direction || { x: 1, y: 0, z: 0 });
    ...
    let warpResult = null;
    let pilotWarpError = "";
    if (typeof spaceRuntime.warpToPoint === "function" &&
        typeof scene.warpToPoint === "function") {
        try {
            warpResult = spaceRuntime.warpToPoint(session, destinationPoint, {
                minimumRange: 0,
                stopDistance: 0,
                // This IS the acceleration-gate transit — exempt from the deadspace warp restriction.
                dungeonGateTransit: true,
                destinationStaticInstanceID: instanceID,
                destinationDungeonRoomKey: destinationRoomKey || "room:entry",
                destinationDungeonSiteID: siteID,
            });
        }
        catch (error) {
            pilotWarpError = normalizeText(error && error.message, "THREW");
            log.warn(`[Keeper] ActivateAccelerationGate pilot warp threw char=${getCharacterID(session)} gate=${gateKey} instance=${instanceID} ` +
    ...
    if (!warpResult || warpResult.success !== true) {
        ...
        warpResult = spaceRuntime.warpDynamicEntityToPoint(scene.systemID, shipEntity, destinationPoint, {
        ...
        if (!warpResult || warpResult.success !== true) {
            log.warn(`[Keeper] ActivateAccelerationGate movement failed char=${getCharacterID(session)} gate=${gateKey} instance=${instanceID} ` +
        ...
        if (!forceStartResult || forceStartResult.success !== true) {
            log.warn(`[Keeper] ActivateAccelerationGate force-start failed char=${getCharacterID(session)} gate=${gateKey} instance=${instanceID} ` +
    ...
    log.info(`[Keeper] ActivateAccelerationGate char=${session && session.characterID} gate=${gateKey} instance=${instanceID} destinationRoom=${destinationRoomKey || "room:entry"}`);
    return null;
}
...
KeeperService._testing = {
    ...
    resolveCurrentDungeonInfoForSession: dungeonTrackingRuntime.resolveCurrentDungeonInfoForSession,
};
module.exports = KeeperService;
//# sourceMappingURL=keeperService.js.map