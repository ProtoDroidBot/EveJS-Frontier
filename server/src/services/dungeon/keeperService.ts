...
function recordAuditEvent(kind, args: any[] = [], session = null, kwargs = null, extra: Record<string, any> = {}) {
  auditEvents.push({
    kind,
    args: cloneArgs(args),
    kwargs: kwargs || null,
...
function normalizeRequiredItemEntry(value, defaults: Record<string, any> = {}) {
...
function addRequiredItemFromValue(requirements, value, defaults: Record<string, any> = {}) {
  if (value === null || value === undefined) {
    return;
  }
  ...
  if (typeof value === "object" && !Array.isArray(value)) {
    ...
    for (const [typeID, quantity] of Object.entries<any>(value)) {
...
function getCargoItemsForGateRequirement(session, shipEntity, options: Record<string, any> = {}) {
...
function assertGateItemRequirements(session, shipEntity, gateEntity, gateState, options: Record<string, any> = {}) {
...
function activateAccelerationGateForSession(session, gateEntityOrID, options: Record<string, any> = {}) {
  const scene = spaceRuntime.getSceneForSession(session);
  if (!scene) {
  ...
  if (
    ...
    Math.max(
      0,
      toInt(
        visibilityResolution.instance &&
          visibilityResolution.instance.metadata &&
          visibilityResolution.instance.metadata.siteID,
        0,
      ),
    ) !== siteID
  ) {
