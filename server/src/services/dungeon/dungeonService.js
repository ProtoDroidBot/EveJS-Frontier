...
const { buildDict, buildKeyVal, buildList, extractDictEntries, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const auditEvents = [];
...
function normalizeFixtures(rawFixtures = {}) {
    ...
    fixtures.factions = [
        ...new Set((Array.isArray(rawFixtures.factions) ? rawFixtures.factions : [])
            .map((value) => toNullableInt(value))
            .filter((value) => value !== null)),
    ].sort((left, right) => left - right);
    fixtures.roomsByDungeonID = {};
    const roomsByDungeonID = rawFixtures.roomsByDungeonID &&
...
class DungeonService extends BaseService {
    ...
    Handle_DEGetRooms(args, session, kwargs) {
        ...
        return buildList(dungeonID && editorFixtures.roomsByDungeonID[dungeonID]
