...
function cloneObjectiveStateForComparison(value = {}) {
    const objectiveState = value && typeof value === "object" && !Array.isArray(value)
        ? cloneValue(value)
        : {};
    if (objectiveState.metadata &&
...
function emitInstanceChange(changeType, beforeInstance, afterInstance, extra = {}) {
    ...
    const payload = {
        ...
        instanceID: Math.max(0, toInt(after && after.instanceID, 0)) ||
            Math.max(0, toInt(before && before.instanceID, 0)) ||
            null,
        solarSystemID: Math.max(0, toInt(after && after.solarSystemID, 0)) ||
            Math.max(0, toInt(before && before.solarSystemID, 0)) ||
            null,
        previousSolarSystemID: Math.max(0, toInt(before && before.solarSystemID, 0)) || null,
        before,
        after,
        metadata: extra && typeof extra === "object" && !Array.isArray(extra)
            ? cloneValue(extra)
            : {},
    };
...
function collectTemplateRoomDefinitions(template) {
    ...
    const environmentTemplates = template &&
        ...
        ? template.environmentTemplates
        : null;
    const roomTemplates = environmentTemplates &&
...
function buildDefaultRoomStates(template, nowMs, options = {}) {
    const explicitRoomStates = options.roomStatesByKey && typeof options.roomStatesByKey === "object"
    ...
    for (const room of collectTemplateRoomDefinitions(template)) {
        ...
        const state = normalizeStateValue(room && (room.initialState || room.state), roomKey === "room:entry" ? "active" : "pending");
        const stage = normalizeText(room && room.stage, roomKey === "room:entry" ? "entry" : "room");
        const pocketID = Math.max(0, toInt(room && (room.pocketID || room.pocketId || room.roomID || room.roomId || room.objectID), 0)) || null;
...
function resolveDestinationRoomKey(connection, orderedRoomKeys, index) {
    const explicitDestinationRoomKey = normalizeRoomKey(connection && (connection.destinationRoomKey ||
...
function collectTemplateGateDefinitions(template) {
    ...
    const addGate = (definition, source, index) => {
        ...
        const gateKey = normalizeGateKey(raw.gateKey ||
            raw.key ||
            raw.connectionKey ||
            fromObjectID ||
            index + 1, `gate:${index + 1}`);
        if (!gateKey) {
            return;
        }
...
function buildDefaultGateStates(template, nowMs, options = {}) {
    const explicitGateStates = options.gateStatesByKey && typeof options.gateStatesByKey === "object"
...
function ensureTemplateRuntimeState(instanceID, options = {}) {
    ...
    const existingRoomStates = existing.roomStatesByKey && typeof existing.roomStatesByKey === "object"
        ? existing.roomStatesByKey
        : {};
    const existingGateStates = existing.gateStatesByKey && typeof existing.gateStatesByKey === "object"
    ...
    for (const [gateKey, gateState] of Object.entries(defaultGateStates)) {
        ...
        const needsDestinationRoomKey = !currentDestinationRoomKey &&
            !!defaultDestinationRoomKey;
        const needsStateRepair = !currentState ||
            (currentState === "locked" &&
        ...
        gatePatch[gateKey] = {
            ...
            metadata: {
                ...
                inferredDestinationRoomKey: needsDestinationRoomKey
                    ? defaultDestinationRoomKey || null
                    : normalizeText(currentGateState.metadata && currentGateState.metadata.inferredDestinationRoomKey, "") || defaultDestinationRoomKey || null,
            },
        };
    }
...
function buildDefaultObjectiveState(template, nowMs, options = {}) {
    const explicitObjectiveState = options.objectiveState && typeof options.objectiveState === "object"
        ? cloneValue(options.objectiveState)
        : null;
    if (explicitObjectiveState) {
        return explicitObjectiveState;
    }
    const clientObjectives = template &&
        template.clientObjectives &&
    ...
    const seededObjectives = Array.isArray(objectiveChain && objectiveChain.objectives)
        ...
        : [];
    const seededCurrentObjective = seededObjectives.find((objective) => (objective && (objective.startActive === 1 || objective.startActive === true))) || seededObjectives[0] || null;
    return {
        state: clientObjectives ? "seeded" : "pending",
        currentNodeID: clientObjectives && Number(clientObjectives.nodeGraphID) > 0
            ? Number(clientObjectives.nodeGraphID)
            : null,
        currentObjectiveID: clientObjectives && Number(clientObjectives.objectiveChainID) > 0
            ? Number(clientObjectives.objectiveChainID)
            : null,
        currentObjectiveKey: seededCurrentObjective && seededCurrentObjective.key
            ? String(seededCurrentObjective.key)
            : null,
        currentObjectiveTypeID: seededCurrentObjective && Number(seededCurrentObjective.objectiveType) > 0
        ...
        metadata: {
            ...
            objectiveChainID: clientObjectives && Number(clientObjectives.objectiveChainID) > 0
                ? Number(clientObjectives.objectiveChainID)
                : null,
            nodeGraphID: clientObjectives && Number(clientObjectives.nodeGraphID) > 0
                ? Number(clientObjectives.nodeGraphID)
                : null,
            blackboardParameters: clientObjectives && Array.isArray(clientObjectives.blackboardParameters)
            ...
            objectiveSummary: objectiveChain
                ...
                : null,
            objectiveTypeIDs: objectiveMetadata && Array.isArray(objectiveMetadata.objectiveTypeIDs)
                ? cloneValue(objectiveMetadata.objectiveTypeIDs)
                : [],
            nodeTypeIDs: objectiveMetadata && Array.isArray(objectiveMetadata.nodeTypeIDs)
...
function buildDefaultEnvironmentState(template, nowMs, options = {}) {
    const explicitEnvironmentState = options.environmentState && typeof options.environmentState === "object"
...
function buildInstanceRecordFromOptions(instanceID, template, options = {}, nowMs = Date.now()) {
    ...
    return {
        ...
        gateStatesByKey,
        objectiveState,
        hazardState: options.hazardState && typeof options.hazardState === "object"
...
function createInstance(options = {}) {
    ...
    if (siteKey) {
        ...
        if (existing && runtimeState.isActiveLifecycleState(existing.lifecycleState)) {
            throw dungeonError("DUNGEON_SITE_KEY_IN_USE", `Active dungeon instance already exists for site key ${siteKey}`);
        }
    }
    const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
    ...
    runtimeState.mutateState((table) => {
        ...
        table.instancesByID[String(nextInstanceID)] = buildInstanceRecordFromOptions(nextInstanceID, template, options, nowMs);
        return table;
    });
...
function reconcileUniverseSeededInstances(definitions = [], options = {}) {
    ...
    const preserveSiteKeys = new Set((Array.isArray(options.preserveSiteKeys) ? options.preserveSiteKeys : [])
...
function rotateUniversePersistentInstances(rotations = [], options = {}) {
    const normalizedRotations = (Array.isArray(rotations) ? rotations : [])
        ...
        .filter((entry) => (entry.existingInstance &&
    ...
    runtimeState.mutateState((table) => {
        ...
        for (const rotation of normalizedRotations) {
            ...
            const nextInstanceID = nextInstanceSequence;
            nextInstanceSequence += 1;
            const created = buildInstanceRecordFromOptions(nextInstanceID, template, rotation.nextDefinition, nowMs);
...
function unlockGate(instanceID, gateKey, options = {}) {
    ...
    return upsertGateState(instanceID, gateKey, {
        state: "unlocked",
        unlockedAtMs: nowMs,
        destinationRoomKey: options.destinationRoomKey != null
...
function advanceObjective(instanceID, patch = {}, options = {}) {
    ...
    if (isDeepStrictEqual(cloneObjectiveStateForComparison(existing.objectiveState), cloneObjectiveStateForComparison(nextObjectiveState))) {
        return existing;
    }
...
function upsertGateState(instanceID, gateKey, patch = {}, options = {}) {
    ...
    runtimeState.mutateState((table) => {
        ...
        target.gateStatesByKey[normalizedGateKey] = {
            ...
            lastUsedAtMs: patch && patch.lastUsedAtMs != null
                ? Math.max(0, toInt(patch.lastUsedAtMs, nowMs))
                : (target.gateStatesByKey[normalizedGateKey] &&
                    target.gateStatesByKey[normalizedGateKey].lastUsedAtMs) || 0,
        };
        target.timers = target.timers || {};
        target.timers.lastUpdatedAtMs = nowMs;
        return table;
...
//# sourceMappingURL=dungeonRuntime.js.map