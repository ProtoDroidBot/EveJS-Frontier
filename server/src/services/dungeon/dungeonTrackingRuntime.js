...
const { entityIDsEqual, normalizePersistentEntityID, } = require(path.join(__dirname, "../../space/destiny/identity/entityID.js"));
const SITE_ENTRY_INFERENCE_RANGE_METERS = 1_000_000;
const SITE_ENTRY_INFERENCE_RANGE_SQUARED = SITE_ENTRY_INFERENCE_RANGE_METERS ** 2;
const SAME_ROOM_WARP_PRESERVE_RANGE_METERS = 1_000_000;
const SAME_ROOM_WARP_PRESERVE_RANGE_SQUARED = SAME_ROOM_WARP_PRESERVE_RANGE_METERS ** 2;
...
function resolveExactPersistentIdentity(values) {
    let resolved = null;
    for (const value of Array.isArray(values) ? values : []) {
        if (!hasIdentityValue(value)) {
            continue;
        }
        const candidate = normalizePersistentEntityID(value);
        if (candidate === null) {
            return null;
        }
...
function distanceSquared(left, right) {
    ...
    return (((leftPosition.x - rightPosition.x) ** 2) +
        ((leftPosition.y - rightPosition.y) ** 2) +
        ((leftPosition.z - rightPosition.z) ** 2));
}
function isActiveDungeonInstance(instance) {
    return (instance &&
        ["seeded", "active", "paused"].includes(normalizeLowerText(instance.lifecycleState, "seeded")));
}
function isTrackedDungeonInstance(instance) {
    return (instance &&
        ["seeded", "active", "paused", "completed"].includes(normalizeLowerText(instance.lifecycleState, "seeded")));
}
...
function resolveDungeonID(instance, template = null) {
    return Math.max(0, toInt(instance && instance.sourceDungeonID, 0), toInt(instance && instance.metadata && instance.metadata.sourceDungeonID, 0), toInt(instance && instance.metadata && instance.metadata.dungeonID, 0), toInt(template && template.sourceDungeonID, 0), toInt(template && template.dungeonID, 0));
}
function resolveRoomID(roomKey, roomState = null) {
    const explicitRoomID = Math.max(0, toInt(roomState && roomState.pocketID, 0), toInt(roomState && roomState.roomID, 0));
    if (explicitRoomID > 0) {
...
function markShipCurrentDungeonRoom(shipEntity, instance, roomKey, options = {}) {
    ...
    shipEntity.dungeonCurrentInstanceID = instanceID;
    if (dungeonID > 0) {
    ...
    const hasExplicitSiteID = Object.hasOwn(options, "siteID") &&
        hasIdentityValue(options.siteID);
    shipEntity.dungeonCurrentSiteID = normalizePersistentEntityID(hasExplicitSiteID
        ? options.siteID
...
function resolveSiteEntityInstance(scene, siteEntity) {
    const instanceID = Math.max(0, toInt(siteEntity && (siteEntity.dungeonSiteInstanceID ||
        siteEntity.dungeonInstanceID ||
        siteEntity.instanceID), 0));
    if (instanceID <= 0) {
        return null;
    }
    const instance = dungeonRuntime.getInstance(instanceID);
...
function inferEntryDungeonForShip(scene, shipEntity, session = null) {
    if (!scene || !shipEntity) {
        return null;
    }
    ...
    for (const entity of Array.isArray(scene.staticEntities) ? scene.staticEntities : []) {
        ...
        if (candidateDistanceSquared > SITE_ENTRY_INFERENCE_RANGE_SQUARED ||
        ...
        if (session) {
            if (!dungeonVisibilityPolicy.resolveDungeonInstanceVisibilityForSession(session, instance.instanceID, { systemID: scene.systemID })) {
                continue;
            }
        }
        nearest = {
            instance,
            roomKey: "room:entry",
            siteID: normalizePersistentEntityID(entity.itemID),
...
function resolveAuthorizedTrackedDungeonForSession(scene, session, shipEntity) {
    ...
    const authorized = dungeonVisibilityPolicy.canSessionAccessDungeonInstance(session, tracked.instance);
    return authorized
        ? {
            ...tracked,
        }
...
function resolveCurrentDungeonForSession(scene, session) {
    ...
    return (resolveAuthorizedTrackedDungeonForSession(scene, session, shipEntity) ||
        inferEntryDungeonForShip(scene, shipEntity, session));
}
function resolveTrackedDungeonForSession(scene, session) {
    ...
    const tracked = resolveAuthorizedTrackedDungeonForSession(scene, session, shipEntity);
    return tracked
        ? {
            ...tracked,
            shipEntity,
...
function completeCurrentDungeonForSession(scene, session, instanceOrID, options = {}) {
    ...
    if (options.forceNotify !== true &&
        Math.max(0, toInt(current.shipEntity.dungeonCompletedNotifiedInstanceID, 0)) === instanceID) {
        return false;
    }
    const dungeonID = Math.max(0, toInt(current.shipEntity && current.shipEntity.dungeonCurrentDungeonID, 0), resolveDungeonID(instance), resolveDungeonID(current.instance));
...
function getSceneEntityByID(scene, entityID) {
    const normalizedEntityID = normalizePersistentEntityID(entityID);
    if (!scene || normalizedEntityID === null) {
        return null;
    }
    if (typeof scene.getEntityByID === "function") {
        return scene.getEntityByID(normalizedEntityID);
    }
    if (scene.staticEntitiesByID instanceof Map &&
        scene.staticEntitiesByID.has(normalizedEntityID)) {
        return scene.staticEntitiesByID.get(normalizedEntityID);
    }
    if (scene.dynamicEntities instanceof Map &&
        scene.dynamicEntities.has(normalizedEntityID)) {
        return scene.dynamicEntities.get(normalizedEntityID);
    }
    return null;
}
function entityBelongsToTrackedDungeon(entity, instance, shipEntity) {
    ...
    const instanceID = Math.max(0, toInt(instance.instanceID, 0));
    const siteID = resolveExactPersistentIdentity([
        shipEntity && shipEntity.dungeonCurrentSiteID,
        instance && instance.metadata && instance.metadata.siteID,
    ]);
    const entityInstanceID = Math.max(0, toInt(entity.dungeonSiteInstanceID, 0), toInt(entity.dungeonInstanceID, 0), toInt(entity.instanceID, 0));
    if (instanceID > 0 && entityInstanceID === instanceID) {
        return true;
    }
    const entitySiteID = normalizePersistentEntityID(hasIdentityValue(entity.dungeonSiteID)
        ? entity.dungeonSiteID
        : entity.itemID);
    if (siteID !== null &&
        entitySiteID !== null &&
        entityIDsEqual(entitySiteID, siteID)) {
        return true;
    }
...
function shouldPreserveDungeonForWarp(scene, current, options = {}) {
    ...
    if (options.preserveDungeonTracking === true ||
...
function exitCurrentDungeonForSession(scene, session, options = {}) {
    ...
    const dungeonID = Math.max(0, toInt(current.shipEntity && current.shipEntity.dungeonCurrentDungeonID, 0), resolveDungeonID(current.instance));
...
//# sourceMappingURL=dungeonTrackingRuntime.js.map