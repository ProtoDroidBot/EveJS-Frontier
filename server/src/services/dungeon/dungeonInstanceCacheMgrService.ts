...
const ARCHETYPE_SETS: Readonly<Record<string, Set<number>>> = Object.freeze({
...
function addVisibleWormholeSignatureCounts(countsBySystem, options: Record<string, any> = {}) {
  const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
  const snapshot = wormholeRuntimeState.getStateSnapshot();
  for (const pair of Object.values<any>(snapshot.pairsByID || {})) {
...
function buildSignatureCountDict(options: Record<string, any> = {}) {
  ensureCacheListener();
