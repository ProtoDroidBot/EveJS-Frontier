...
function resolveRuntimeSummaryFromExplicitInstance(site, solarSystemID) {
    const instanceID = Math.max(0, toInt(site && (site.instanceID ||
        site.dungeonSiteInstanceID ||
        site.dungeonMaterializedGasSiteInstanceID), 0));
    if (instanceID <= 0) {
        return null;
    }
...
function scoreTemplate(template, site, hints = {}) {
    ...
    const totalHintedResources = uniqueSortedInts(hints.oreTypeIDs).length +
...
function resolveTemplateForSite(site, hints = {}) {
    ...
    const candidates = [...candidatesByID.values()].sort((left, right) => (sortTemplateCandidates(left, right, site, hints)));
...
function enrichSiteWithDungeonRuntime(site, options = {}) {
    ...
    let runtimeSummary = resolveRuntimeSummaryFromExplicitInstance(site, solarSystemID) ||
    ...
    const enriched = {
        ...
        dungeonID: template && template.sourceDungeonID != null
        ...
        dungeonNameID: template && template.dungeonNameID != null
            ? template.dungeonNameID
            : (site && site.dungeonNameID != null ? site.dungeonNameID : null),
        factionID: template && template.factionID != null
            ? template.factionID
            : (site && site.factionID != null ? site.factionID : null),
        entryObjectTypeID: template && template.entryObjectTypeID != null
...
//# sourceMappingURL=dungeonSiteAdapter.js.map