...
function resolveDungeonShipRestrictions(dungeonID, gateID = 0) {
  ...
  let whitelist = new Set<any>(shipTypes.map((ship) => ship.typeID));
  ...
  const allowedShipTypes: any[] = [];
  const restrictedShipTypes: any[] = [];
