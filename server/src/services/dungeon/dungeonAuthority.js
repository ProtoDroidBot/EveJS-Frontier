...
function sanitizePayload(payload = {}) {
    const templatesByID = Object.fromEntries(Object.entries(payload.templatesByID || {}).filter(([templateID, template]) => !isBannedDungeonTemplateRecord(template, templateID)));
    ...
    for (const [indexName, rawIndex] of Object.entries(payload.indexes || {})) {
        ...
        for (const [key, rawTemplateIDs] of Object.entries(normalizeObject(rawIndex))) {
            if (Array.isArray(rawTemplateIDs)) {
                const templateIDs = rawTemplateIDs.filter((templateID) => !isBannedDungeonTemplateID(templateID) &&
            ...
            if (!isBannedDungeonTemplateID(rawTemplateIDs) &&
...
function buildCache() {
    ...
    for (const [templateID, template] of Object.entries(payload.templatesByID || {})) {
        ...
        const normalizedTemplate = {
            ...
            sourceDungeonID: template && template.sourceDungeonID != null
                ? toInt(template.sourceDungeonID, 0)
                : null,
            archetypeID: template && template.archetypeID != null
        ...
        const resourceComposition = normalizedTemplate &&
    ...
    const rawArchetypesByID = payload.clientData &&
    ...
    const rawDungeonsByID = payload.clientData &&
        payload.clientData.tables &&
    ...
    const rawObjectiveChainsByID = payload.clientData &&
        payload.clientData.tables &&
    ...
    const rawObjectiveTypesByID = payload.clientData &&
        payload.clientData.tables &&
    ...
    const rawSpawnProfiles = payload &&
...
function listTemplatesByDungeonNameID(dungeonNameID) {
    return clone(ensureCache().templatesByDungeonNameID.get(toInt(dungeonNameID, 0)) || []);
}
function listTemplatesByArchetypeID(archetypeID) {
    return clone(ensureCache().templatesByArchetypeID.get(toInt(archetypeID, 0)) || []);
}
function listTemplatesByResourceTypeID(resourceTypeID) {
    return clone(ensureCache().templatesByResourceTypeID.get(toInt(resourceTypeID, 0)) || []);
}
