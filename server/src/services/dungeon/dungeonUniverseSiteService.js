...
const { resolveItemByName, resolveItemByTypeID, } = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const { resolveMiningResourceIdentity, } = require(path.join(__dirname, "../mining/miningVisuals"));
const { getTypeAttributeValue, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const { grantItemsToOwnerLocation, findItemById, getAllItems, listContainerItems, ITEM_FLAGS, removeInventoryItem, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const SITE_NAME_LOOKUP_PATH = path.join(__dirname, "../../../../tools/SignalAtlas/data/siteNameLookup.json");
...
function resolveMiningRockShellTypeRecord(resourceTypeID, variantIndex = 0) {
    const profile = MINING_ROCK_SHELL_PROFILES[(Math.abs(toInt(resourceTypeID, 0)) +
        Math.abs(toInt(variantIndex, 0))) % MINING_ROCK_SHELL_PROFILES.length];
    return {
...
function resolveGateDungeonObjectID(gateProfile, gateState, gateKey) {
    const explicitObjectID = Math.max(0, toInt(gateProfile && (gateProfile.dungeonObjectID || gateProfile.dunObjectID || gateProfile.fromObjectID), 0));
    if (explicitObjectID > 0) {
        return explicitObjectID;
    }
    const metadata = normalizeObject(gateState && gateState.metadata);
    const metadataObjectID = Math.max(0, toInt(metadata.dungeonObjectID || metadata.dunObjectID || metadata.fromObjectID, 0));
...
function resolveGateActivationRangeMeters(gateProfile, typeID) {
    const explicitRange = toFiniteNumber(gateProfile && (gateProfile.gateActivationRange || gateProfile.activationRange || gateProfile.proximityRange), 0);
    if (explicitRange > 0) {
        return explicitRange;
    }
    return (toFiniteNumber(getTypeAttributeValue(typeID, "proximityRange", "Activation proximity"), 0) ||
...
function resolveGateRequirementMetadata(gateProfile, gateState) {
    ...
    return {
        keyLock: firstDefined(gateProfile && gateProfile.keyLock, gateProfile && gateProfile.keylock, gateProfile && gateProfile.keyLockType, gateState && gateState.keyLock, gateState && gateState.keylock, metadata.keyLock, metadata.keylock, metadata.keyLockType, rawConnection.keyLock, rawConnection.keylock, rawConnection.keyLockType),
        requiredItems: firstDefined(gateProfile && gateProfile.requiredItems, gateProfile && gateProfile.requiredKeyItems, gateProfile && gateProfile.requiredItemTypeIDs, gateProfile && gateProfile.requiredKeyTypeIDs, gateProfile && gateProfile.keyTypeIDs, gateProfile && gateProfile.passcardTypeIDs, gateState && gateState.requiredItems, metadata.requiredItems, metadata.requiredKeyItems, metadata.requiredItemTypeIDs, metadata.requiredKeyTypeIDs, rawConnection.requiredItems, rawConnection.requiredKeyItems, rawConnection.requiredItemTypeIDs, rawConnection.requiredKeyTypeIDs),
        requiredItemTypeID: Math.max(0, toInt(firstDefined(gateProfile && gateProfile.requiredItemTypeID, gateProfile && gateProfile.requiredKeyTypeID, gateProfile && gateProfile.keyTypeID, gateProfile && gateProfile.keyItemTypeID, gateProfile && gateProfile.passcardTypeID, gateState && gateState.requiredItemTypeID, metadata.requiredItemTypeID, metadata.requiredKeyTypeID, rawConnection.requiredItemTypeID, rawConnection.requiredKeyTypeID, rawConnection.keyTypeID, rawConnection.passcardTypeID), 0)) || null,
        requiredItemQuantity: Math.max(0, toInt(firstDefined(gateProfile && gateProfile.requiredItemQuantity, gateProfile && gateProfile.requiredQuantity, gateProfile && gateProfile.keyQuantity, gateProfile && gateProfile.passcardQuantity, gateState && gateState.requiredItemQuantity, metadata.requiredItemQuantity, metadata.requiredQuantity, rawConnection.requiredItemQuantity, rawConnection.requiredQuantity, rawConnection.keyQuantity, rawConnection.passcardQuantity), 0)) || null,
    };
}
...
function resolveTypeRecordName(typeRecord, fallback = "") {
    return normalizeText(typeRecord && (typeRecord.name || typeRecord.typeName || typeRecord.slimName), fallback);
}
function resolveLocalizedTemplateName(template) {
    ...
    const templateNamesByID = lookup && lookup.templateNamesByID && typeof lookup.templateNamesByID === "object"
        ? lookup.templateNamesByID
        : {};
    const dungeonNamesByMessageID = lookup && lookup.dungeonNamesByMessageID && typeof lookup.dungeonNamesByMessageID === "object"
...
function resolveTypeHealthState(typeID) {
    ...
    const shieldCapacity = Math.max(0, toFiniteNumber(getTypeAttributeValue(normalizedTypeID, "shieldCapacity"), 0));
    const armorHP = Math.max(0, toFiniteNumber(getTypeAttributeValue(normalizedTypeID, "armorHP"), 0));
    const structureHP = Math.max(0, toFiniteNumber(getTypeAttributeValue(normalizedTypeID, "hp", "structureHP"), 0));
    return {
        shieldCapacity,
        armorHP,
        structureHP,
...
function resolveContainerLootTags(containerEntity, populationHints) {
    const explicitTags = normalizeArray(containerEntity &&
    ...
    const profile = resolveLootProfileDefinition(populationHints, normalizeText(containerEntity &&
...
function buildLootEntryFromNames(candidateNames, seed, minimum = 1, maximum = minimum, options = {}) {
    ...
    return buildLootGrantEntry(itemType, resolveDeterministicQuantity(seed, minimum, maximum), options);
}
function buildBoosterLootGrantEntries(seed, tags, context = {}) {
    const siteLabel = normalizeText(context &&
    ...
    for (const tag of normalizeArray(tags)) {
        switch (normalizeLowerText(tag, "")) {
            case "booster_bpc":
                entries.push(buildLootEntryFromNames(orderedFamilies.flatMap((family) => [
            ...
            case "reaction_formula":
                entries.push(buildLootEntryFromNames(orderedFamilies.map((family) => `Standard ${family} Booster Reaction Formula`), `${seed}:reaction_formula`));
                break;
            case "skillbook":
    ...
    for (const tag of normalizeArray(tags)) {
        switch (normalizeLowerText(tag, "")) {
            ...
            case "data_material":
                ...
                ], `${seed}:generic_data_material`, 1, 2));
                break;
            default:
                break;
        }
        ...
        switch (normalizeLowerText(tag, "")) {
            ...
            case "relic_component":
                entries.push(buildLootEntryFromNames([
                    ...
                    "Melted Nanoribbons",
                    "Power Circuit",
                ], `${seed}:generic_relic_component`, 1, 3));
                break;
            default:
                break;
        }
...
function resolveCombatLootFlavor(context = {}) {
    const label = normalizeLowerText(context &&
        (context.template && context.template.resolvedName ||
            context.siteEntity && context.siteEntity.itemName), "");
...
function buildCombatLootGrantEntries(seed, tags, context = {}, options = {}) {
    ...
    for (const tag of normalizeArray(tags)) {
        switch (normalizeLowerText(tag, "")) {
            case "faction_module":
                entries.push(buildLootEntryFromNames(flavorModules[flavor] || flavorModules.generic, `${seed}:combat_module`));
                break;
            case "faction_ammo":
            case "pirate_tag":
                entries.push(buildLootEntryFromNames(flavorTagItems[flavor] || flavorTagItems.generic, `${seed}:combat_tag:${tag}`, 1, 2));
                break;
            case "overseer_effect":
                entries.push(buildLootEntryFromNames(overseerTierCandidates, `${seed}:overseer_effect`));
                break;
...
function buildLootGrantEntriesForContainer(containerEntity, populationHints, context = {}) {
    ...
    const seedBase = normalizeText(containerEntity && containerEntity.dungeonSiteContentKey, `${profileKey}:${normalizeText(containerEntity && containerEntity.itemName, "container")}`);
    switch (profileKey) {
...
function normalizeMissionSpawnCount(entry = null) {
    ...
    return Math.max(1, Math.min(SITE_CONTENT_MAX_ENCOUNTER_NPCS, toInt(count.max, toInt(count.min, 1))));
}
...
function resolveExactEnvironmentPropLimit(populationHints, candidateCount) {
    ...
    const limit = resolveBoundedContentLimit(caps.environmentProps ||
        caps.exactEnvironmentProps ||
        (populationHints && populationHints.maxExactEnvironmentProps), SITE_CONTENT_MAX_EXACT_ENVIRONMENT_PROPS, SITE_CONTENT_MAX_EXACT_ENVIRONMENT_PROPS);
    return Math.min(Math.max(0, toInt(candidateCount, 0)), limit);
}
function resolveEncounterSpawnEntryLimit(encounterPlan) {
    const explicitLimit = Math.max(0, toInt(encounterPlan && (encounterPlan.maxSpawnEntries ||
        encounterPlan.maxExactSpawnEntries ||
        encounterPlan.maxEncounterNpcs), 0));
    ...
    return Math.max(1, resolveBoundedContentLimit(explicitLimit, defaultLimit, SITE_CONTENT_MAX_EXACT_ENCOUNTER_NPCS));
}
function normalizeEncounterTriggerMessages(entry) {
    return normalizeMaybeArray((entry && (entry.triggerMessages || entry.triggerMessage || entry.dungeonTriggerMessages)) || [])
        .map((message) => {
        ...
        return {
            messageType: Math.max(0, toInt(message.messageType || message.type || message.notificationType, 0)),
            messageID: Math.max(0, toInt(message.messageID || message.messageId || message.id || message.labelID, 0)),
...
function normalizeEncounterTriggerAudio(entry) {
    return normalizeMaybeArray((entry && (entry.triggerAudio || entry.triggerAudios || entry.dungeonTriggerAudio)) || [])
        .map((audioEntry) => {
        ...
        return {
            dungeonID: Math.max(0, toInt(audioEntry.dungeonID || audioEntry.sourceDungeonID || audioEntry.dungeonId, 0)) || null,
...
function isLikelyNamedMissionNpcCandidate(candidate = "") {
    ...
    return (/\btrue\b/.test(normalized) ||
...
function buildFrontierDungeonMiningResourceSizing(entries, sourceDungeonID) {
    ...
    const ranked = resources
        .map((entry) => ({
        entry,
        rankKey: hashText(`${sourceDungeonID}:${toInt(entry.roomID, 0)}:${toInt(entry.objectID, 0)}:mining-radius`),
    }))
        .sort((left, right) => (left.rankKey - right.rankKey ||
...
function resolveInitialFrontierDungeonSceneObjectIDs(template, flattenedObjects, sourceDungeonID, instance = null) {
    const configuredSpawnerTypes = normalizeObject(frontierDungeonSpawns.getConfig().spawnerTypes);
    const objectByID = new Map(normalizeArray(flattenedObjects)
        .filter((entry) => entry && toInt(entry.objectID, 0) > 0)
        .map((entry) => [toInt(entry.objectID, 0), entry]));
    const authoredSpawnTargets = new Set();
    ...
    const isSceneObject = (entry) => Boolean(entry &&
    ...
    for (const entry of objectByID.values()) {
        ...
        const rawTriggerSpawn = Object.prototype.hasOwnProperty.call(guardCommand, "objectTriggerSpawn")
    ...
    const initialTriggers = triggers
        ...
        .filter(({ trigger }) => (toInt(trigger && trigger.triggerTypeID, 0) === FRONTIER_DUNGEON_INITIAL_TRIGGER_TYPE_ID));
    ...
    const instanceSeed = normalizeText(instance && (instance.instanceID ||
        instance.metadata && instance.metadata.siteID), String(sourceDungeonID));
    for (const [groupKey, group] of triggerGroups.entries()) {
        ...
        const totalUsageChance = weightedTriggers.reduce((sum, entry) => sum + entry.usageChance, 0);
        if (totalUsageChance <= 0) {
            continue;
        }
        const rollRange = Math.max(100, totalUsageChance);
        const roll = (hashText(`${sourceDungeonID}:${instanceSeed}:initial-trigger-group:${groupKey}`) / 0x1_0000_0000) * rollRange;
        ...
        const triggerID = toInt(selectedTrigger.trigger && selectedTrigger.trigger.triggerID, selectedTrigger.index);
        for (const [eventIndex, event] of normalizeArray(selectedTrigger.trigger && selectedTrigger.trigger.triggerEvents).entries()) {
            ...
            if (!frontierDungeonChancePasses(`${sourceDungeonID}:${instanceSeed}:initial-trigger:${triggerID}:event:${eventID}`, event && event.usageChance)) {
                continue;
            }
...
function buildFrontierDungeonDerivedPopulationHints(template, instance = null) {
    ...
    const entryPosition = entryObject
        ...
        : { x: 0, y: 0, z: 0 };
    const configuredSpawnerTypes = normalizeObject(frontierDungeonSpawns.getConfig().spawnerTypes);
    const initialSceneObjectIDs = resolveInitialFrontierDungeonSceneObjectIDs(template, flattenedObjects, sourceDungeonID, instance);
    ...
    const environmentProps = selected.map((entry) => {
        ...
        return {
            authoredRadius: Math.max(0, toFiniteNumber(miningSizing && miningSizing.radius, toFiniteNumber(object && object.radius, 0))) || null,
            ...
            resourceQuantity: Math.max(0, toInt(miningSizing && miningSizing.resourceQuantity, toInt(resourceProfile && resourceProfile.resourceQuantity, 0))) || null,
...
function isAuthoredFrontierExactDescriptor(entry, options = {}) {
    if (!entry || typeof entry !== "object") {
        return false;
    }
    ...
    return (Math.max(0, toInt(entry.dunObjectID, 0)) > 0 ||
...
function listAuthoredFrontierExactOffsetDescriptors(populationHints) {
    ...
    const descriptors = [];
    const append = (entry, options = {}) => {
        if (!isAuthoredFrontierExactDescriptor(entry, options) ||
...
function resolvePrimaryFrontierContentOrigin(offsetDescriptors = []) {
...
function buildMissionDerivedEncounterPlans(template) {
    ...
    const gateDestinationRoomKeys = new Set(normalizeArray(template && template.siteSceneProfile && template.siteSceneProfile.gateProfiles)
    ...
    for (let roomIndex = 0; roomIndex < rooms.length; roomIndex += 1) {
        ...
        for (const group of groupedEntries) {
            ...
            for (let entryIndex = 0; entryIndex < npcEntries.length; entryIndex += 1) {
                ...
                encounterPlans.push({
                    key: normalizeText(`${roomKey}:${normalizeText(group && group.key, "group")}:${entryIndex + 1}`, `mission_wave_${groupWaveIndex}_${entryIndex + 1}`).toLowerCase().replace(/[^a-z0-9:._-]+/g, "_"),
...
function mergeDerivedMissionPopulationHints(baseHints, derivedHints) {
    ...
    const merged = {
        ...
        source: normalizeText(baseHints && baseHints.source, normalizeText(derivedHints && derivedHints.source, "mission_runtime_derived")),
        roomCount: Math.max(toInt(baseHints && baseHints.roomCount, 0), toInt(derivedHints && derivedHints.roomCount, 0)),
        encounter: normalizeObject(baseHints && baseHints.encounter).supported !== undefined
...
function resolvePopulationHints(instance, template) {
    ...
    const baseHints = spawnHints
        ...
        : templateHints
            ...
            : null;
    const frontierHints = rebaseFrontierDungeonExactContent(mergeFrontierDungeonPopulationHints(baseHints, buildFrontierDungeonDerivedPopulationHints(template, instance)));
    if (normalizeLowerText(template && template.siteFamily, "") !== "mission") {
...
function resolveEncounterPlans(populationHints) {
    ...
    const fallbackPlan = populationHints &&
    ...
    const normalizedPlans = (explicitPlans.length > 0 ? explicitPlans : fallbackPlan)
        ...
        const spawnQuery = normalizeText(entry && entry.spawnQuery, "") ||
            normalizeText(entry && entry.baseProfileID, "") ||
            normalizeText(entry && entry.profileID, "");
        const amount = Math.max(0, toInt(entry && entry.amount, 0)) ||
            spawnEntries.length ||
        ...
        return {
            ...
            maxSpawnEntries: Math.max(0, toInt(entry && (entry.maxSpawnEntries || entry.maxExactSpawnEntries || entry.maxEncounterNpcs), 0)) || null,
            ...
            proximityRangeMeters: Math.max(0, toFiniteNumber(entry && entry.proximityRangeMeters, 0)) || null,
...
function listOrderedInstanceRoomKeys(instance) {
    const roomStatesByKey = instance &&
...
function listOrderedInstanceGateKeys(instance) {
    const gateStatesByKey = instance &&
    ...
    return Object.keys(gateStatesByKey)
        ...
        return (toInt(leftState && leftState.metadata && leftState.metadata.connectionIndex, 0) -
            toInt(rightState && rightState.metadata && rightState.metadata.connectionIndex, 0)) || left.localeCompare(right);
    });
}
function resolveEncounterRoomKey(instance, populationHints, encounterPlan) {
    ...
    const orderedPlans = resolveEncounterPlans(populationHints)
        .sort((left, right) => (Math.max(1, toInt(left && left.waveIndex, 1)) - Math.max(1, toInt(right && right.waveIndex, 1))) || normalizeText(left && left.key, "").localeCompare(normalizeText(right && right.key, "")));
    ...
    const roomIndex = Math.min(orderedRoomKeys.length - 1, Math.floor((planIndex * orderedRoomKeys.length) / Math.max(1, orderedPlans.length)));
...
function getEncounterStateByKey(instance, planKey) {
    return normalizeObject(instance &&
        instance.spawnState &&
...
function normalizeCompletionHints(populationHints) {
    ...
    return {
        ...
        objectiveMode: normalizeLowerText(completion.objectiveMode || (populationHints && populationHints.objectiveMode), "") || null,
        completeOnEncounterClear: completion.completeObjectiveOnEncounterClear === false ||
            (populationHints && populationHints.completeObjectiveOnEncounterClear === false)
            ? false
            : true,
        mode: normalizeLowerText(completion.mode || completion.completionMode, "") || null,
        encounterKeys: normalizeArray(completion.encounterKeys ||
            ...
            .filter(Boolean),
        objectiveTargets: normalizeArray(completion.objectiveTargets ||
...
function isObjectiveTargetSpawnEntry(spawnEntry, encounterPlan, completion) {
    ...
    if (spawnEntry.objective === true ||
        spawnEntry.objectiveTarget === true ||
        spawnEntry.blocksObjectiveProgress === true) {
        return true;
    }
    const completionRole = normalizeLowerText((spawnEntry && spawnEntry.completionRole) ||
...
function resolveCompletionEncounterKeys(completion, plans) {
    const explicitKeys = new Set(normalizeArray(completion && completion.encounterKeys)
        .map((entry) => normalizeText(entry, ""))
        .filter(Boolean));
...
function hasRecordedSitePlayerProgress(instance) {
    return Boolean(isSiteObjectiveExplicitlySatisfied(instance) ||
        Math.max(0, toInt(instance && instance.spawnState && instance.spawnState.lastPlayerProgressAtMs, 0)) > 0);
}
function isEncounterCompletionSatisfied(instance, plans, populationHints) {
    ...
    if (mode === "objective_target_destroyed" ||
...
function upsertEncounterState(instanceID, planKey, patch = {}, options = {}) {
    ...
    const encounterStatesByKey = normalizeObject(existing.spawnState && existing.spawnState.encounterStatesByKey);
...
function resolveEncounterPrerequisiteKeys(plans, encounterPlan) {
    ...
    const priorPlans = normalizeArray(plans)
        ...
        .sort((left, right) => (Math.max(1, toInt(left && left.waveIndex, 1)) - Math.max(1, toInt(right && right.waveIndex, 1)) ||
...
function listAliveEncounterEntityIDs(scene, encounterState, encounterPlan = null) {
    const nonBlockingEntityIDs = new Set(normalizeIDList(encounterState && encounterState.nonBlockingEntityIDs));
    const hasNpcProgressEntries = encounterPlanHasNpcProgressEntries(encounterPlan);
    const seededEntityIDs = normalizeIDList(encounterState &&
...
function rematerializeStoredDungeonEncounterEntities(scene, instance, siteEntity, options = {}) {
    const systemID = Math.max(0, toInt(scene && scene.systemID, 0));
    const instanceID = Math.max(0, toInt(instance && instance.instanceID, 0));
    const instanceSystemID = Math.max(0, toInt(instance && instance.solarSystemID, 0));
    ...
    const encounterStatesByKey = normalizeObject(instance && instance.spawnState && instance.spawnState.encounterStatesByKey);
    const storedEntityIDs = normalizeIDList(Object.values(encounterStatesByKey).flatMap((state) => [
    ...
    for (const entityID of storedEntityIDs) {
        ...
        const materializeResult = nativeNpcService.materializeStoredNativeController(scene, entityID, { broadcast: false });
    ...
    for (const wreckRecord of nativeNpcStore.listNativeWrecks()) {
        ...
        if (toInt(wreckRecord.expiresAtMs, 0) > 0 &&
        ...
        const spawnResult = nativeNpcWreckService.spawnNativeWreck(systemID, wreckRecord.wreckID, { broadcast: false });
...
function resolveObjectiveLabel(objective, objectiveType) {
    return humanizeIdentifier(objective && objective.title, humanizeIdentifier(objectiveType && objectiveType.title, humanizeIdentifier(objective && objective.key, "Objective")));
}
function resolveObjectiveTaskLabel(task, taskType) {
    return humanizeIdentifier(taskType && taskType.title, humanizeIdentifier(task && task.key, "Task"));
}
...
function buildFallbackPopulationObjectiveMarkers(populationHints, template) {
    ...
    const resources = populationHints && populationHints.resources && typeof populationHints.resources === "object"
        ? populationHints.resources
        : {};
    const markers = [];
    switch (family) {
        case "data":
            markers.push({ role: "objective", label: "Hack research caches", analyzer: "data" }, { role: "task", label: "Recover site intelligence", analyzer: "data" });
            break;
        case "relic":
            markers.push({ role: "objective", label: "Recover archaeology caches", analyzer: "relic" }, { role: "task", label: "Salvage relic materials", analyzer: "relic" });
            break;
        case "ghost":
            markers.push({ role: "objective", label: "Hack covert research caches", analyzer: "data" }, { role: "task", label: "Beat the response timer", analyzer: "data" });
            break;
        case "combat":
            markers.push({ role: "objective", label: "Eliminate hostile defenders" });
            break;
        case "combat_hacking":
            markers.push({ role: "objective", label: "Hack the facility network", analyzer: "data" }, { role: "task", label: "Defeat security reinforcements" });
            break;
        case "gas":
    ...
    if (normalizeArray(resources.oreTypeIDs).length > 0 ||
...
function resolvePopulationObjectiveMarkers(populationHints, template) {
    const explicitMarkers = normalizeObjectiveMarkerHintEntries(populationHints && populationHints.objectiveMarkers);
...
function buildContentOffset(seed, index, total, options = {}) {
    const baseDistance = Math.max(2500, toFiniteNumber(options.baseDistanceMeters, SITE_CONTENT_CONTAINER_RING_METERS));
    const jitterMeters = Math.max(0, toFiniteNumber(options.jitterMeters, SITE_CONTENT_CONTAINER_JITTER_METERS));
...
function isMissionLikeSiteInstance(instance, template = null) {
    ...
    return (runtimeFlags.missionRuntime === true ||
    ...
    for (const entityMap of [
        ...
        for (const entity of entityMap.values()) {
            ...
            occupied.push({
                ...
                radius: Math.max(0, toFiniteNumber(entity.collisionRadius ?? entity.radius, 0)),
            });
        }
    }
    return occupied;
}
function allocateSeparatedDungeonSpawnPosition(scene, desiredPosition, reservedPositions = [], options = {}) {
    const candidateRadius = Math.max(0, toFiniteNumber(options.candidateRadius, 1_000));
    const position = buildSeparatedSpawnPosition(desiredPosition, [
        ...collectOccupiedDungeonSpawnPositions(scene),
        ...(Array.isArray(reservedPositions) ? reservedPositions : []),
    ], {
        minimumSeparationMeters: Math.max(1, toFiniteNumber(options.minimumSeparationMeters, DEFAULT_DUNGEON_SPAWN_SEPARATION_METERS)),
        candidateRadius,
        seed: options.seed,
    });
...
function buildSiteEntity(instance) {
    ...
    const dungeonWarpInDistanceMeters = Math.max(10_000, toFiniteNumber(template && template.frontierDungeonWarpInDistanceMeters, frontierDungeonSpawns.getConfig().defaults.siteWarpInDistanceMeters));
...
function buildStableUniverseSiteEntitySignature(entity) {
    ...
    return JSON.stringify({
        ...
        dungeonMaterializedSiteContent: entity.dungeonMaterializedSiteContent === true,
        dungeonSiteInstanceID: Math.max(0, toInt(entity.dungeonSiteInstanceID, 0)) || null,
...
function resolveManagedUniverseSiteInstance(scene, instanceOrSite, options = {}) {
    ...
    const numericSiteID = Math.max(0, toInt(options.siteID, instanceOrSite && (instanceOrSite.siteID ||
        instanceOrSite.itemID ||
        (instanceOrSite.metadata && instanceOrSite.metadata.siteID))));
    if (numericSiteID <= 0) {
        return null;
    }
    const trackedInstanceID = Math.max(0, toInt(ensureSceneMaterializedSiteInstanceMap(scene).get(numericSiteID), 0));
    ...
    return dungeonRuntime.listActiveInstancesBySystem(systemID, {
        full: true,
    }).find((instance) => (isManagedMaterializedSiteInstance(instance) &&
        Math.max(0, toInt(instance && instance.metadata && instance.metadata.siteID, 0)) === numericSiteID)) || null;
}
function listMaterializedUniverseSiteContentEntities(scene, options = {}) {
    ...
    return [...staticEntities, ...dynamicEntities]
        ...
        .filter((entity) => ((!numericSiteID || toInt(entity && entity.dungeonSiteID, 0) === numericSiteID) &&
...
function forceResyncSiteStaticContentForSession(scene, session, instanceOrSite, options = {}) {
    ...
    instance = dungeonRuntime.ensureTemplateRuntimeState(Math.max(0, toInt(instance && instance.instanceID, 0)), {
        nowMs: options.nowMs,
    }) || instance;
    ...
    const visibleStaticIDs = session._space.visibleBubbleScopedStaticEntityIDs instanceof Set
...
function destroyMaterializedContentEntity(scene, entity, options = {}) {
    ...
    if (entity.nativeNpc === true) {
        ...
        const liveDestroyResult = typeof npcSpawnService.destroyNpcControllerByEntityID === "function"
...
function buildMiningRockEntities(instance, siteEntity, populationHints) {
    ...
    const rockSpecs = populationHints.miningRocks
        ...
        const dunRotation = Array.isArray(rock.dunRotation) && rock.dunRotation.length === 3
            ...
            : null;
        const ownerID = Math.max(0, toInt(rock.ownerID, toInt(populationHints.miningRockOwnerID ||
            populationHints.ownerID ||
            populationHints.missionOwnerID, 0))) || 500021;
...
function listMiningObjectiveRockDescriptors(instance, populationHints) {
    const siteID = Math.max(0, toInt(instance && instance.metadata && instance.metadata.siteID, instance && instance.siteID));
...
function buildEncounterRewardContainerEntity(instance, siteEntity, encounterPlan, encounterState, populationHints) {
    ...
    return {
        ...
        itemName: label,
        slimName: label,
        position: addVectors(clonePosition(siteEntity && siteEntity.position), buildContentOffset(`${siteID}:encounter_reward:${normalizeText(encounterPlan && encounterPlan.key, String(rewardIndex))}`, rewardIndex - 1, totalPlans, {
...
function materializeEncounterRewardContainers(scene, instance, siteEntity, template, populationHints, options = {}) {
    ...
    for (const plan of resolveEncounterPlans(populationHints)) {
        ...
        const rewardContainer = buildEncounterRewardContainerEntity(instance, siteEntity, plan, encounterState, populationHints);
        if (!rewardContainer) {
            continue;
        }
        const created = materializeContainerEntity(scene, instance, siteEntity, template, populationHints, rewardContainer, { nowMs });
        if (!created) {
            continue;
        }
...
function materializeFetchObjectiveContainer(scene, instance, siteEntity, template, populationHints, options = {}) {
    ...
    const siteID = Math.max(0, toInt(siteEntity && siteEntity.itemID, 0));
    const typeRecord = resolveGenericContainerTypeRecord() || resolveItemByTypeID(23) || {};
    const dropPosition = clonePosition(spawnState.fetchObjectiveContainerPosition) ||
    ...
    const created = materializeContainerEntity(scene, instance, siteEntity, template, populationHints, containerEntity, { nowMs, broadcast: true });
    if (!created) {
        return 0;
    }
...
function maybeCompleteClearedEncounterSite(instance, populationHints, options = {}) {
    ...
    const objectiveCompletedAtMs = Math.max(0, toInt(latestInstance && latestInstance.objectiveState && latestInstance.objectiveState.completedAtMs, 0));
    const completionAtMs = Math.max(latestCompletionAtMs, objectiveCompletedAtMs, Math.max(0, toInt(options.nowMs, Date.now())));
...
function isDataRelicMaterializedContainer(entity) {
    return Boolean(entity &&
        entity.dungeonMaterializedContainer === true &&
        entity.dungeonMaterializedSiteContent === true &&
        DATA_RELIC_CONTAINER_ANALYZERS.has(normalizeLowerText(entity.dungeonSiteContentAnalyzer, "")));
}
...
function resolveMaterializedContainerCompletionContext(scene, source = null, options = {}) {
    ...
    const containerID = Math.max(0, toInt(options.containerID, sourceEntity && sourceEntity.itemID));
    const liveSourceEntity = sourceEntity ||
        ...
        (containerID > 0 && scene.staticEntitiesByID instanceof Map
            ...
            : null) ||
        null;
    const instanceID = Math.max(0, toInt(options.instanceID, liveSourceEntity && liveSourceEntity.dungeonSiteInstanceID));
    const siteID = Math.max(0, toInt(options.siteID, liveSourceEntity && liveSourceEntity.dungeonSiteID));
    ...
    return {
        ...
        siteID: Math.max(0, toInt(siteID, instance && instance.metadata && instance.metadata.siteID)),
    };
}
...
function maybeCompleteMaterializedDataRelicSite(scene, source = null, options = {}) {
    ...
    const dungeonCompletionNotifiedCount = dungeonTrackingRuntime.notifyDungeonCompletedForScene(scene, updated, { nowMs });
...
function materializeContainerEntity(scene, instance, siteEntity, template, populationHints, containerEntity, options = {}) {
    ...
    const entityScopeMetadata = {
        ...
        dungeonSiteInstanceID: toInt(containerEntity && containerEntity.dungeonSiteInstanceID, 0),
        dungeonSiteContentKey: contentKey,
        dungeonSiteContentRole: normalizeLowerText(containerEntity && containerEntity.dungeonSiteContentRole, "container"),
        dungeonSiteContentAnalyzer: normalizeLowerText(containerEntity && containerEntity.dungeonSiteContentAnalyzer, "") || null,
        dungeonSiteContentBonus: containerEntity && containerEntity.dungeonSiteContentBonus === true,
        dungeonSiteContentFailureExplodes: containerEntity && containerEntity.dungeonSiteContentFailureExplodes === true,
        dungeonSiteContentPersistsAfterResponse: containerEntity && containerEntity.dungeonSiteContentPersistsAfterResponse === true,
        dungeonSiteContentTrigger: normalizeLowerText(containerEntity && containerEntity.dungeonSiteContentTrigger, "") || null,
        dungeonSiteContentLootProfile: normalizeText(containerEntity && containerEntity.dungeonSiteContentLootProfile, "") || null,
        dungeonSiteContentExplicitLoot: Array.isArray(containerEntity && containerEntity.dungeonSiteContentExplicitLoot)
            ? cloneValue(containerEntity.dungeonSiteContentExplicitLoot)
            : null,
        dungeonSiteContentLootTags: cloneValue(containerEntity && containerEntity.dungeonSiteContentLootTags) || [],
        dungeonSiteContentHackingDifficulty: normalizeLowerText(containerEntity && containerEntity.dungeonSiteContentHackingDifficulty, "") || null,
    };
    ...
    if (itemID <= 0) {
        ...
        const createResult = grantItemsToOwnerLocation(SITE_CONTENT_OWNER_ID, scene.systemID, 0, [{
    ...
    if (!lootSeeded) {
        ...
        if (lootEntries.length > 0) {
            grantItemsToOwnerLocation(SITE_CONTENT_OWNER_ID, itemID, ITEM_FLAGS.CARGO_HOLD, lootEntries);
        }
    }
    ...
    if (options.broadcast === true && typeof scene.broadcastAddBalls === "function") {
        scene.broadcastAddBalls([liveEntity], options.excludedSession || null, { freshAcquire: true });
    }
...
function resolveInculcatorBuildupSnapshot(instance, buildup, nowMs = Date.now()) {
    if (normalizeLowerText(buildup && buildup.kind, "") !== "inculcator_foundation") {
        return null;
    }
    const phases = normalizeArray(buildup && buildup.phases)
        .map((phase) => ({
        key: normalizeLowerText(phase && phase.key, ""),
        durationMs: Math.max(1_000, Math.round(toFiniteNumber(phase && phase.durationSeconds, 0) * 1_000)),
    }))
        .filter((phase) => phase.key && phase.durationMs > 0);
    if (phases.length <= 0) {
        return null;
    }
    const existing = normalizeObject(instance && instance.environmentState && instance.environmentState.inculcatorBuildup);
    const timers = normalizeObject(instance && instance.timers);
    const numericNowMs = Math.max(0, toInt(nowMs, Date.now()));
    const startedAtMs = Math.max(0, toInt(existing.startedAtMs, toInt(timers.activatedAtMs, toInt(timers.createdAtMs, numericNowMs)))) || numericNowMs;
    ...
    const lastControllingFactionKey = normalizeLowerText(existing.lastControllingFactionKey || instance && instance.dungeonFactionKey, "") || null;
    const lastControllingFactionID = Math.max(0, toInt(existing.lastControllingFactionID, toInt(instance && instance.factionID, 0))) || null;
    return {
        ...cloneValue(existing),
        ...
        completionEntityTypeID: Math.max(0, toInt(buildup && buildup.completionEntity && buildup.completionEntity.typeID, 0)) || null,
...
function buildInculcatorCompletionEntity(instance, siteEntity, buildup, snapshot) {
    ...
    const typeRecord = resolveItemByTypeID(typeID) || (typeID === 88_090
        ? {
            typeID: 88_090,
            groupID: 226,
            categoryID: 2,
    ...
    const label = normalizeText(completionEntity.name, resolveTypeRecordName(typeRecord, "Steel Forge"));
    const positionOffset = clonePosition(completionEntity.positionOffset);
    const destinyPresentation = resolveSiteEnvironmentPropDestinyPresentation({ destinyBootstrapDelivery: DESTINY_BOOTSTRAP_DELIVERY_ADDBALLS2 }, {
...
function processInculcatorSiteBuildup(scene, instance, siteEntity, populationHints, options = {}) {
    ...
    const completion = buildInculcatorCompletionEntity(instance, siteEntity, buildup, snapshot);
...
function buildHazardEntities(instance, siteEntity, populationHints) {
    ...
    const siteID = Math.max(0, toInt(siteEntity && siteEntity.itemID, 0));
    const instanceID = Math.max(0, toInt(instance && instance.instanceID, 0));
    const hazards = [...new Set(populationHints.hazards
            .map((entry) => {
            if (entry && typeof entry === "object") {
                return JSON.stringify({
    ...
    return hazards.map((hazard, index) => {
        ...
        return {
            ...
            position: addVectors(clonePosition(siteEntity && siteEntity.position), buildContentOffset(`${siteID}:${hazardKind}`, index, total, {
...
function resolveSiteSceneProfile(template) {
    return normalizeObject(template &&
...
function buildGateEntities(instance, siteEntity, template) {
    ...
    for (const gateProfile of rawGateProfiles) {
        ...
        const destinationRoomKey = normalizeText(gateProfile && gateProfile.destinationRoomKey, normalizeText(gateState && gateState.destinationRoomKey, ""));
        const label = normalizeText(gateProfile && gateProfile.label, "");
        const allowedShipsList = Math.max(0, toInt(gateProfile && gateProfile.allowedShipsList, gateState && gateState.metadata && gateState.metadata.allowedShipsList));
    ...
    return gateProfiles.map((gateProfile, index) => {
        ...
        const typeRecord = (explicitTypeID > 0
            ? resolveItemByTypeID(explicitTypeID)
            : resolveGenericTypeRecordByName(gateProfile && gateProfile.typeNameCandidates)) || resolveItemByTypeID(17_831) || {};
        const gateKey = normalizeText(gateProfile && gateProfile.gateKey, `gate:${index + 1}`);
        const gateState = normalizeObject(gateStatesByKey[gateKey]);
        const label = normalizeText(gateProfile && gateProfile.label, resolveTypeRecordName(typeRecord, "Acceleration Gate")) || "Acceleration Gate";
        ...
        const exactOffset = gateProfile && gateProfile.positionOffset && typeof gateProfile.positionOffset === "object"
            ...
            : null;
        const contentOffset = exactOffset || buildContentOffset(`${siteID}:gate:${gateKey}`, index, gateProfiles.length, {
        ...
        return {
            ...
            dungeonGateDestinationRoomKey: normalizeText(gateProfile && gateProfile.destinationRoomKey, normalizeText(gateState && gateState.destinationRoomKey, "") || null) || null,
            dungeonGateState: normalizeLowerText(gateState && gateState.state, "locked"),
            dungeonGateAllowedShipsList: Math.max(0, toInt(gateProfile && gateProfile.allowedShipsList, gateState && gateState.metadata && gateState.metadata.allowedShipsList)) || null,
...
function buildEnvironmentEntities(instance, siteEntity, template, populationHints) {
    ...
    const environmentTemplates = template &&
        ...
        ? template.environmentTemplates
        : null;
    const resolvedTemplateCatalog = environmentTemplates &&
        environmentTemplates.resolvedTemplateCatalog &&
        typeof environmentTemplates.resolvedTemplateCatalog === "object"
        ? environmentTemplates.resolvedTemplateCatalog
        : {};
    const entryObjectEnvironmentMapping = environmentTemplates &&
    ...
    const mappedTemplateRefs = [
        ...(entryObjectEnvironmentMapping && entryObjectEnvironmentMapping.baseEnvironment
            ...
            : []),
        ...Object.values(entryObjectEnvironmentMapping && entryObjectEnvironmentMapping.overridesByMaterialSetID
    ...
    const sceneProfileStructures = (hasExactHintedEnvironmentProps ? [] : normalizeArray(sceneProfile && sceneProfile.structureProfiles))
        .filter((entry) => entry && typeof entry === "object");
    if (Object.keys(resolvedTemplateCatalog).length <= 0 &&
    ...
    for (const environmentProp of hintedEnvironmentProps) {
        ...
        candidates.push({
            ...
            destinyBallFlags: environmentProp && (environmentProp.destinyBallFlags ?? environmentProp.flagsByte),
            destinyCollisionTail: environmentProp && environmentProp.destinyCollisionTail,
            destinyCollisionTailSource: environmentProp && environmentProp.destinyCollisionTailSource,
        });
    }
    const deduped = [];
    const seen = new Set();
    ...
    const entities = selected.map((candidate, index) => {
        const authoredResourceTypeID = toInt(candidate.typeRecord && candidate.typeRecord.typeID, candidate.typeID) || candidate.typeID;
        ...
        const miningResourceIdentity = frontierDungeonResource
            ...
            : null;
        const carrierTypeRecord = (miningResourceIdentity && miningResourceIdentity.carrierTypeRecord) ||
            candidate.typeRecord;
        const yieldTypeRecord = (miningResourceIdentity && miningResourceIdentity.yieldTypeRecord) ||
            candidate.typeRecord;
        const carrierTypeID = toInt(carrierTypeRecord && carrierTypeRecord.typeID, authoredResourceTypeID) ||
            authoredResourceTypeID;
        const yieldTypeID = toInt(yieldTypeRecord && yieldTypeRecord.typeID, authoredResourceTypeID) ||
        ...
        const exactOffset = candidate.exact && candidate.positionOffset
            ...
            : null;
        const contentOffset = exactOffset || buildContentOffset(`${siteID}:environment:${candidate.typeID}`, index, selected.length, {
        ...
        const hiveSpawnConfiguration = frontierDungeonSpawns.resolveHiveSpawnConfiguration(authoredResourceTypeID);
        return {
            kind: "siteEnvironmentProp",
            dungeonMaterializedSiteContent: true,
            dungeonMaterializedEnvironment: true,
...
function buildObjectiveEntities(instance, siteEntity, template, populationHints = null) {
    ...
    if (objectives.length > 0) {
        const currentObjectiveKey = normalizeText(instance &&
            instance.objectiveState &&
            instance.objectiveState.currentObjectiveKey, "");
        const selectedObjective = objectives.find((objective) => (currentObjectiveKey && normalizeText(objective && objective.key, "") === currentObjectiveKey)) ||
            objectives.find((objective) => objective && (objective.startActive === 1 || objective.startActive === true)) ||
            objectives[0] ||
            null;
        if (selectedObjective) {
            const selectedObjectiveType = normalizeObject(objectiveTypesByID[String(toInt(selectedObjective && selectedObjective.objectiveType, 0))]);
            markers.push({
                role: "objective",
            ...
            for (const task of normalizeArray(selectedObjectiveType.tasks)) {
                ...
                markers.push({
                    ...
                    analyzer: normalizeLowerText(taskType && taskType.icon, "") === "hacking"
                        ? "data"
                        : null,
                });
            }
    ...
    return markers.slice(0, total).map((marker, index) => {
        const typeRecord = marker.analyzer && genericContainerType
        ...
        return {
            ...
            position: addVectors(clonePosition(siteEntity && siteEntity.position), buildContentOffset(`${siteID}:objective:${marker.objectiveKey || index}`, index, total, {
...
function resolveTriggerAudioDungeonID(instance, audioEntry) {
    ...
    const metadataDungeonID = Math.max(0, toInt(instance && instance.metadata && (instance.metadata.sourceDungeonID ||
        instance.metadata.dungeonID ||
        instance.metadata.clientDungeonID), 0));
...
function sendEncounterTriggerNotifications(scene, instance, encounterPlan, options = {}) {
    ...
    for (const session of sessions) {
        for (const message of triggerMessages) {
            try {
                session.sendNotification("OnDungeonTriggerMessage", normalizeText(message && message.idType, "charid"), [
        ...
        for (const audioEntry of triggerAudio) {
            try {
                session.sendNotification("OnDungeonTriggerAudio", normalizeText(audioEntry && audioEntry.idType, "shipid"), [
...
function isKillableStructureSpawnEntry(entry) {
    const entityKind = normalizeLowerText(entry && (entry.entityKind || entry.kind || entry.spawnKind || entry.role), "").replace(/[^a-z0-9]+/g, "");
...
function allocateKillableStructureEntityID(scene, siteID) {
    ...
    for (let slot = 1; slot <= 99; slot += 1) {
        ...
        if (!(scene && scene.staticEntitiesByID instanceof Map && scene.staticEntitiesByID.has(candidateID)) &&
...
function buildKillableStructureEntity(instance, siteEntity, encounterPlan, spawnEntry, context = {}) {
    ...
    const anchorPosition = clonePosition(context.anchorPosition && typeof context.anchorPosition === "object"
    ...
    const exactOffset = spawnEntry && spawnEntry.positionOffset && typeof spawnEntry.positionOffset === "object"
        ...
        : null;
    const contentOffset = exactOffset || buildContentOffset(`${siteID}:killable_structure:${normalizeText(encounterPlan && encounterPlan.key, "encounter")}`, index, total, {
    ...
    const dunRotation = Array.isArray(spawnEntry && spawnEntry.dunRotation) &&
        ...
        : null;
    const healthState = resolveTypeHealthState(typeID);
    const label = normalizeText(spawnEntry && spawnEntry.label, "") ||
    ...
    return {
        ...
        dungeonSiteContentKey: normalizeText(spawnEntry && spawnEntry.key, "") ||
...
function spawnEncounterKillableStructures(scene, instance, siteEntity, encounterPlan, spawnEntries, options = {}) {
    const structures = [];
    const reservedSpawnPositions = [];
    const structureAnchorPosition = clonePosition(options.anchorPosition && typeof options.anchorPosition === "object"
    ...
    for (let index = 0; index < total; index += 1) {
        ...
        const separatedPosition = allocateSeparatedDungeonSpawnPosition(scene, entity.position, reservedSpawnPositions, {
    ...
    if (structures.length > 0 &&
        ...
        scene &&
        typeof scene.broadcastAddBalls === "function") {
        scene.broadcastAddBalls(structures.map((entry) => entry.entity), options.excludedSession || null);
    }
    return structures;
}
...
function spawnEncounterPlan(scene, instance, siteEntity, encounterPlan, options = {}) {
    ...
    const encounterAnchorPosition = clonePosition((options.roomPosition && typeof options.roomPosition === "object")
    ...
    if (identitySpawnEntries.length > 0) {
        for (let entryIndex = 0; entryIndex < identitySpawnEntries.length; entryIndex += 1) {
            ...
            const desiredEntryPosition = (entry.positionOffset && typeof entry.positionOffset === "object")
                ...
                : addVectors(encounterAnchorPosition, buildContentOffset(`${encounterKey}:entry:${entryIndex}`, entryIndex, identitySpawnEntries.length, {
                    baseDistanceMeters: SITE_CONTENT_ENCOUNTER_OFFSET_METERS,
                    jitterMeters: 8_000,
                }));
            const entryPosition = allocateSeparatedDungeonSpawnPosition(scene, desiredEntryPosition, reservedSpawnPositions, {
        ...
        const proceduralAmount = Math.max(1, Math.min(resolveEncounterSpawnEntryLimit(encounterPlan), toInt(encounterPlan.amount, 3)));
        const spawnProceduralEntries = (profileQuery, seedSuffix) => {
            let spawnedCount = 0;
            let failure = null;
            for (let entryIndex = 0; entryIndex < proceduralAmount; entryIndex += 1) {
                const desiredPosition = addVectors(encounterAnchorPosition, buildContentOffset(`${encounterKey}:${seedSuffix}`, entryIndex, proceduralAmount, {
                    baseDistanceMeters: SITE_CONTENT_ENCOUNTER_OFFSET_METERS,
                    jitterMeters: 8_000,
                }));
                const entryPosition = allocateSeparatedDungeonSpawnPosition(scene, desiredPosition, reservedSpawnPositions, {
        ...
        if (aggregatedSpawns.length <= 0 &&
            ...
            const fallbackResult = spawnProceduralEntries(fallbackSpawnQuery, "encounter:fallback");
            lastSpawnFailure = fallbackResult.failure || lastSpawnFailure;
        }
    }
    if (structureSpawnEntries.length > 0) {
        const structureSpawns = spawnEncounterKillableStructures(scene, instance, siteEntity, encounterPlan, structureSpawnEntries, {
            ...
            .map((entry) => ({
            ...entry,
            encounterProgressBlocker: shouldKillableStructureBlockEncounterProgress(entry && entry.spawnEntry, encounterPlan, {
                hasNpcEntries,
            }),
            objectiveProgressBlocker: isObjectiveTargetSpawnEntry(entry && entry.spawnEntry, encounterPlan, completion),
        }));
    ...
    const spawnedEntityIDs = normalizeIDList(spawnResult.data.spawned
        .map((entry) => toInt(entry && entry.entity && entry.entity.itemID, 0))
        .filter((entry) => entry > 0));
    const remainingEntityIDs = normalizeIDList(spawnResult.data.spawned
        .filter((entry) => !(entry && entry.encounterProgressBlocker === false))
        .map((entry) => toInt(entry && entry.entity && entry.entity.itemID, 0))
        .filter((entry) => entry > 0));
    const objectiveBlockingEntityIDs = normalizeIDList(spawnResult.data.spawned
        .filter((entry) => entry && entry.objectiveProgressBlocker === true)
        .map((entry) => toInt(entry && entry.entity && entry.entity.itemID, 0))
        .filter((entry) => entry > 0));
    const plans = resolveEncounterPlans(populationHints);
    const currentRoomKey = resolveEncounterRoomKey(dungeonRuntime.getInstance(instance.instanceID) || instance, { encounters: plans }, encounterPlan);
    try {
    ...
    const totalWaves = Math.max(1, plans.reduce((highest, plan) => Math.max(highest, toInt(plan && plan.waveIndex, 1)), 1));
    if (instance &&
        instance.objectiveState &&
        ["seeded", "in_progress"].includes(normalizeLowerText(instance.objectiveState.state, ""))) {
        const existingCounters = instance.objectiveState && instance.objectiveState.counters && typeof instance.objectiveState.counters === "object"
            ? instance.objectiveState.counters
            : {};
        const existingMetadata = instance.objectiveState && instance.objectiveState.metadata && typeof instance.objectiveState.metadata === "object"
...
function processEncounterPlansForTrigger(scene, instance, siteEntity, populationHints, trigger, options = {}) {
    ...
    const triggerRoomKey = normalizeText(options.roomKey || options.currentRoomKey || options.destinationRoomKey, "");
    const plans = resolveEncounterPlans(populationHints);
...
function maybeAdvanceEncounterDrivenProgression(instance, populationHints, options = {}) {
    ...
    if (refreshedInstance && refreshedInstance.objectiveState) {
        const existingCounters = refreshedInstance.objectiveState.counters && typeof refreshedInstance.objectiveState.counters === "object"
            ? refreshedInstance.objectiveState.counters
            : {};
        const existingMetadata = refreshedInstance.objectiveState.metadata && typeof refreshedInstance.objectiveState.metadata === "object"
        ...
        const nextPatch = {
            ...
            metadata: {
                ...
                completionEncounterKeys: resolveCompletionEncounterKeys(normalizeCompletionHints(populationHints), plans),
            },
        };
        const currentComparableObjectiveState = {
            ...
            completedAtMs: Math.max(0, toInt(refreshedInstance.objectiveState.completedAtMs, 0)),
        };
        ...
        if (completionSatisfied) {
            const existingCompletedAtMs = Math.max(0, toInt(refreshedInstance.objectiveState.completedAtMs, 0));
        ...
        if (!isDeepStrictEqual(currentComparableObjectiveState, nextComparableObjectiveState)) {
        ...
        if (completionSatisfied) {
            sendCompletionTriggerNotifications(options.scene || null, dungeonRuntime.getInstance(refreshedInstance.instanceID) || refreshedInstance, populationHints, {
                nowMs,
...
function applyTriggeredSiteEffects(scene, instance, siteEntity, populationHints, trigger, options = {}) {
    ...
    for (const entity of contentEntities) {
        ...
        const shouldTrigger = ((normalizedTrigger === "hack_failure" && entity.dungeonHazardFailureTriggersExplosion === true) ||
    ...
    if (removedContainers > 0 || triggeredHazards > 0) {
        dungeonRuntime.mergeHazardState(instance.instanceID, {
            ...
            removedContainers,
            triggeredHazards,
            responseTriggered: normalizedTrigger === "visible_countdown" || normalizedTrigger === "hack_failure",
        }, { nowMs });
    }
    return {
        removedContainers,
...
function processProximityEncounterTriggers(scene, instance, siteEntity, populationHints, options = {}) {
    ...
    for (const plan of proximityPlans) {
        ...
        const rangeMeters = Math.max(0, toFiniteNumber(plan.proximityRangeMeters, 0)) ||
...
function processMiningObjective(scene, instance, populationHints, options = {}) {
    ...
    for (const [entityID, entity] of liveRocksByID.entries()) {
        if (!descriptorsByID.has(entityID)) {
            descriptorsByID.set(entityID, {
                entityID,
                quantity: Math.max(0, toInt(entity.miningOriginalQuantity, 0), toInt(entity.originalQuantity, 0), toInt(entity.mineableQuantity, 0), toInt(entity.resourceQuantity, 0), toInt(entity.miningRemainingQuantity, 0)),
            });
        }
    }
    for (const descriptor of descriptorsByID.values()) {
        ...
        const initial = Math.max(0, toInt(state && state.originalQuantity, 0), toInt(descriptor && descriptor.quantity, 0), toInt(entity && entity.miningOriginalQuantity, 0), toInt(entity && entity.originalQuantity, 0), toInt(entity && entity.mineableQuantity, 0), toInt(entity && entity.resourceQuantity, 0), toInt(entity && entity.miningRemainingQuantity, 0));
        const remaining = Math.max(0, toInt(state && state.remainingQuantity, initial));
...
function maybeAdvanceExplicitlySatisfiedObjective(instance, options = {}) {
    ...
    const objectiveState = refreshedInstance &&
        refreshedInstance.objectiveState &&
        typeof refreshedInstance.objectiveState === "object"
        ? refreshedInstance.objectiveState
        : {};
    if (normalizeLowerText(objectiveState.state, "") === "completed" &&
        Math.max(0, toInt(objectiveState.completedAtMs, 0)) > 0) {
        return false;
    }
    const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
    const satisfiedAtMs = Math.max(0, toInt(refreshedInstance && refreshedInstance.objectiveSatisfiedAtMs, 0), toInt(refreshedInstance && refreshedInstance.metadata && refreshedInstance.metadata.objectiveSatisfiedAtMs, 0), nowMs);
    const existingMetadata = objectiveState.metadata &&
        typeof objectiveState.metadata === "object" &&
        !Array.isArray(objectiveState.metadata)
        ? objectiveState.metadata
        : {};
    const existingCounters = objectiveState.counters &&
    ...
    dungeonRuntime.advanceObjective(instanceID, {
        ...
        metadata: {
            ...
            objectiveSatisfiedReason: normalizeText(refreshedInstance && refreshedInstance.objectiveSatisfiedReason, "") ||
...
function isDungeonInstanceObjectiveCompleted(instance) {
    const objectiveState = normalizeObject(instance && instance.objectiveState);
    return (normalizeLowerText(objectiveState.state, "") === "completed" ||
        Math.max(0, toInt(objectiveState.completedAtMs, 0)) > 0);
}
...
function tickSceneSiteBehaviors(scene, options = {}) {
    ...
    for (const instance of instances) {
        ...
        const refreshedForObjective = dungeonRuntime.getInstance(instance.instanceID) || instance;
        maybeAdvanceExplicitlySatisfiedObjective(refreshedForObjective, { nowMs });
        syncAgentMissionForCompletedDungeonObjective(dungeonRuntime.getInstance(instance.instanceID) || refreshedForObjective, { nowMs });
        const siteID = Math.max(0, toInt(instance && instance.metadata && instance.metadata.siteID, 0));
        const siteEntity = (scene.staticEntitiesByID &&
            scene.staticEntitiesByID.get(siteID)) || buildSiteEntity(instance);
        if (!siteEntity) {
            continue;
        }
        ...
        processInculcatorSiteBuildup(scene, dungeonRuntime.getInstance(instance.instanceID) || instance, siteEntity, populationHints, { nowMs, broadcast: true });
        const plans = resolveEncounterPlans(populationHints);
        if (plans.length <= 0) {
            continue;
        }
        ...
        encountersSpawned += processEncounterPlansForTrigger(scene, instance, siteEntity, populationHints, "on_load", { nowMs });
        const roomStatesByKey = normalizeObject(instance && instance.roomStatesByKey);
        for (const [roomKey, roomState] of Object.entries(roomStatesByKey)) {
            if (!["active", "completed"].includes(normalizeLowerText(roomState && roomState.state, ""))) {
                continue;
            }
            encountersSpawned += processEncounterPlansForTrigger(scene, instance, siteEntity, populationHints, "on_room_active", { nowMs, roomKey });
        }
        ...
        const visibleCountdownEncounters = processEncounterPlansForTrigger(scene, instance, siteEntity, populationHints, "visible_countdown", { nowMs });
        encountersSpawned += visibleCountdownEncounters;
        if (visibleCountdownDue) {
            applyTriggeredSiteEffects(scene, instance, siteEntity, populationHints, "visible_countdown", { nowMs });
            for (const plan of resolveEncounterPlans(populationHints)) {
                if (normalizeLowerText(plan && plan.trigger, "") !== "visible_countdown") {
                    continue;
                }
        ...
        encountersSpawned += processEncounterPlansForTrigger(scene, latestInstance, siteEntity, populationHints, "wave_cleared", { nowMs });
        ...
        encountersSpawned += processEncounterPlansForTrigger(scene, afterWaveClear, siteEntity, populationHints, "battleships_destroyed", { nowMs });
        ...
        rewardContainersSpawned += materializeEncounterRewardContainers(scene, refreshedForRewards, siteEntity, template, populationHints, { nowMs });
        rewardContainersSpawned += materializeFetchObjectiveContainer(scene, dungeonRuntime.getInstance(instance.instanceID) || refreshedForRewards, siteEntity, template, populationHints, { nowMs });
        const refreshedForProgression = dungeonRuntime.getInstance(instance.instanceID) || refreshedForRewards;
        const progressionResult = maybeAdvanceEncounterDrivenProgression(refreshedForProgression, populationHints, { nowMs, scene });
        maybeCompleteClearedEncounterSite(dungeonRuntime.getInstance(instance.instanceID) || refreshedForRewards, populationHints, { nowMs, scene });
        syncAgentMissionForCompletedDungeonObjective(dungeonRuntime.getInstance(instance.instanceID) || refreshedForProgression, { nowMs });
...
function encounterStateReferencesEntityID(encounterState, entityID) {
    ...
    return (normalizeIDList(encounterState && encounterState.spawnedEntityIDs).includes(numericEntityID) ||
...
function normalizeFetchObjectiveHints(populationHints) {
    const fetchObjective = populationHints &&
...
function maybeGrantFetchObjectiveItems(matchingInstances, destroyedEntity, options = {}) {
    ...
    for (const instance of normalizeArray(matchingInstances)) {
        ...
        const hasTriggerIdentity = Math.max(0, toInt(trigger.typeID, 0)) > 0 ||
        ...
        const characterID = Math.max(0, toInt(ownership.missionOwnerCharacterID, toInt(ownership.characterID, 0)));
        if (characterID <= 0) {
            continue;
        }
        let shipItemID = 0;
        ...
        try {
            ...
            if (agentMissionRuntime &&
                ...
                agentMissionRuntime.notifyMissionInventoryChanges(characterID, (grantResult.data && grantResult.data.changes) || []);
            }
        }
        catch (_error) {
...
function handleEncounterEntityDestroyed(scene, entityOrID, options = {}) {
    const entityID = Math.max(0, toInt(entityOrID && typeof entityOrID === "object"
    ...
    const objectiveTargetsSatisfied = markDestroyedObjectiveTargets(matchingInstances, destroyedEntity || { itemID: entityID }, {
        nowMs: options.nowMs,
    });
    const fetchObjectiveGrants = maybeGrantFetchObjectiveItems(matchingInstances, destroyedEntity || { itemID: entityID }, {
        nowMs: options.nowMs,
    });
...
function triggerSiteEncounter(scene, instanceOrID, trigger, options = {}) {
    ...
    if (options.session &&
        !dungeonVisibilityPolicy.resolveDungeonInstanceVisibilityForSession(options.session, instance.instanceID, { systemID: scene.systemID })) {
        return {
            success: false,
    ...
    const siteEntity = (scene.staticEntitiesByID &&
        scene.staticEntitiesByID.get(siteID)) || buildSiteEntity(instance);
    const triggeredEffects = applyTriggeredSiteEffects(scene, instance, siteEntity, populationHints, trigger, options);
    const encountersSpawned = processEncounterPlansForTrigger(scene, instance, siteEntity, populationHints, trigger, options);
    return {
        success: true,
        data: {
            encountersSpawned,
...
function materializeSiteContents(scene, instance, siteEntity, template, options = {}) {
    ...
    const materializedSiteID = Math.max(0, toInt(siteEntity && siteEntity.itemID, instance && instance.metadata && instance.metadata.siteID));
    ...
    const detachedSourceIDs = (options.detachedDungeonPropStore || require(path.join(__dirname, "../../space/detachedDungeonProps")).getDetachedDungeonPropStore()).suppressedSourceIDs(scene.systemID, instance.instanceID, materializedSiteID);
    if (materializedSiteID > 0) {
        markSceneSiteMaterialized(scene, materializedSiteID, instance.instanceID);
    }
    let workingInstance = dungeonRuntime.ensureTemplateRuntimeState(Math.max(0, toInt(instance && instance.instanceID, 0)), {
        nowMs: options.nowMs,
    }) || instance;
    finishPhase("runtimeState");
    const populationHints = resolvePopulationHints(workingInstance, template);
    finishPhase("populationHints");
    rematerializeStoredDungeonEncounterEntities(scene, workingInstance, siteEntity, { nowMs: options.nowMs });
    ...
    for (const entity of contentEntities) {
        const created = materializeContainerEntity(scene, workingInstance, siteEntity, template, populationHints, entity, {
            nowMs: options.nowMs,
            broadcast: options.broadcast === true,
            excludedSession: options.excludedSession || null,
        });
    ...
    const buildupResult = processInculcatorSiteBuildup(scene, workingInstance, siteEntity, populationHints, {
        nowMs: options.nowMs,
        broadcast: false,
        excludedSession: options.excludedSession || null,
    });
    ...
    if (options.spawnEncounters !== false) {
        const onLoadEncountersSpawned = processEncounterPlansForTrigger(scene, workingInstance, siteEntity, populationHints, "on_load", {
            nowMs: options.nowMs,
            session: options.session || null,
            excludedSession: options.excludedSession || null,
        });
        ...
        if (onLoadEncountersSpawned <= 0) {
            const refreshedInstance = dungeonRuntime.getInstance(Math.max(0, toInt(workingInstance && workingInstance.instanceID, 0))) ||
                workingInstance;
            syncEncounterStateProgress(scene, refreshedInstance, resolveEncounterPlans(populationHints), {
                nowMs: options.nowMs,
            });
            encountersSpawned += processEncounterPlansForTrigger(scene, refreshedInstance, siteEntity, populationHints, "wave_cleared", {
                nowMs: options.nowMs,
                session: options.session || null,
                excludedSession: options.excludedSession || null,
            });
            const afterWaveClear = dungeonRuntime.getInstance(Math.max(0, toInt(workingInstance && workingInstance.instanceID, 0))) ||
                refreshedInstance;
            syncEncounterStateProgress(scene, afterWaveClear, resolveEncounterPlans(populationHints), {
                nowMs: options.nowMs,
            });
            encountersSpawned += processEncounterPlansForTrigger(scene, afterWaveClear, siteEntity, populationHints, "battleships_destroyed", {
                nowMs: options.nowMs,
                session: options.session || null,
                excludedSession: options.excludedSession || null,
            });
...
function ensureSiteContentsMaterialized(scene, instanceOrSite, options = {}) {
    ...
    if (options.session) {
        const visibilityResolution = dungeonVisibilityPolicy.resolveDungeonInstanceVisibilityForSession(options.session, instance.instanceID, { systemID: scene.systemID });
    ...
    let siteEntity = (scene.staticEntitiesByID &&
    ...
    if (!siteEntity) {
        ...
        siteEntity = (scene.staticEntitiesByID &&
            scene.staticEntitiesByID.get(siteID)) || buildSiteEntity(instance);
    }
    if (!siteEntity) {
        return {
...
function removeSceneEntity(scene, siteID, options = {}) {
    ...
    if (requestedInstanceID !== null) {
        const existingRoot = scene.staticEntitiesByID instanceof Map
            ? scene.staticEntitiesByID.get(numericSiteID) || null
            : null;
        const rootInstanceID = normalizePersistentEntityID(existingRoot && existingRoot.dungeonSiteInstanceID);
        const trackedInstanceID = normalizePersistentEntityID(ensureSceneMaterializedSiteInstanceMap(scene).get(numericSiteID));
...
function storedInventoryItemMatchesDungeonScope(item, systemID, siteID, instanceID) {
...
function storedInventoryItemBelongsToNativeController(item, controllerEntityIDs) {
    if (!item ||
    ...
    return [
        ...
        item.launcherID,
    ].some((controllerID) => (exactIdentitySetHas(controllerEntityIDs, controllerID)));
}
function getCanonicalEncounterEntityIDs(instance, options = {}) {
    ...
    const encounterStatesByKey = normalizeObject(instance.spawnState && instance.spawnState.encounterStatesByKey);
...
function listStoredDungeonNativeControllerEntityIDs(systemID, siteID, instanceID, instance, additionalEntityIDs = []) {
    const nativeNpcStore = getNativeNpcStore();
    const canonicalEncounterEntityIDs = getCanonicalEncounterEntityIDs(instance, { systemID, siteID, instanceID });
    ...
    for (const entityIDText of canonicalEncounterEntityIDs) {
        ...
        const entityRecord = numericEntityID > 0
            ...
            : null;
        const recordSiteID = normalizeCleanupIdentity(entityRecord && entityRecord.dungeonSiteID);
        const recordInstanceID = normalizeCleanupIdentity(entityRecord && entityRecord.dungeonSiteInstanceID);
        if ((recordSiteID !== null && !entityIDsEqual(recordSiteID, siteID)) ||
...
function listStoredDungeonNativeWreckIDs(systemID, siteID, instanceID, nativeControllerEntityIDs) {
    const nativeWreckIDs = new Set();
    for (const wreckRecord of getNativeNpcStore().listNativeWrecks()) {
        const wreckSystemMatches = entityIDsEqual(wreckRecord && wreckRecord.systemID, systemID);
        const wreckSiteID = normalizeCleanupIdentity(wreckRecord && wreckRecord.dungeonSiteID);
        const wreckInstanceID = normalizeCleanupIdentity(wreckRecord && wreckRecord.dungeonSiteInstanceID);
        const wreckAirInstanceID = normalizeCleanupIdentity(wreckRecord && wreckRecord.airNpeInstanceID);
        const explicitSiteMismatch = wreckSiteID !== null && !entityIDsEqual(wreckSiteID, siteID);
        const explicitInstanceMismatch = wreckInstanceID !== null &&
            !entityIDsEqual(wreckInstanceID, instanceID);
        const explicitScopeMismatch = explicitSiteMismatch || explicitInstanceMismatch;
        const exactScopeMatches = wreckSiteID !== null &&
            wreckInstanceID !== null &&
            entityIDsEqual(wreckSiteID, siteID) &&
            entityIDsEqual(wreckInstanceID, instanceID);
        const sourceMatchesCanonicalEncounter = !explicitScopeMismatch &&
            [wreckRecord && wreckRecord.sourceEntityID, wreckRecord && wreckRecord.launcherID]
                .some((sourceEntityID) => (exactIdentitySetHas(nativeControllerEntityIDs, sourceEntityID)));
        const projectedAirSourceMatchesCanonicalEncounter = explicitInstanceMismatch &&
            wreckSiteID !== null &&
            wreckInstanceID !== null &&
            ...
            exactIdentitySetHas(nativeControllerEntityIDs, wreckRecord && wreckRecord.sourceEntityID);
        if (wreckRecord &&
            wreckSystemMatches &&
            (exactScopeMatches ||
...
function listStoredDungeonInventoryItemIDs(systemID, siteID, instanceID, instance, nativeControllerEntityIDs) {
    const canonicalContentItemIDs = getCanonicalStoredContentItemIDs(instance, {
        systemID,
        siteID,
        instanceID,
    });
    ...
    for (const item of Object.values(getAllItems())) {
        ...
        const isCanonicalContentRef = exactIdentitySetHas(canonicalContentItemIDs, itemID) &&
            entityIDsEqual(item.locationID, systemID) &&
            toInt(item.flagID, -1) === 0 &&
            entityIDsEqual(item.spaceState && item.spaceState.systemID, systemID);
        const matchesInheritedDungeonScope = storedInventoryItemMatchesDungeonScope(item, systemID, siteID, instanceID) &&
            !storedInventoryItemIsPlayerOwned(item);
        if (isCanonicalContentRef ||
            matchesInheritedDungeonScope ||
            storedInventoryItemBelongsToNativeController(item, nativeControllerEntityIDs)) {
...
function removeStoredDungeonSiteContent(systemID, options = {}) {
    ...
    const nativeNpcStore = getNativeNpcStore();
    let removedCount = 0;
    const nativeControllerEntityIDs = listStoredDungeonNativeControllerEntityIDs(numericSystemID, siteID, instanceID, options.instance, options.nativeControllerEntityIDs);
    const inventoryItemIDs = listStoredDungeonInventoryItemIDs(numericSystemID, siteID, instanceID, options.instance, nativeControllerEntityIDs);
    for (const itemID of inventoryItemIDs) {
        const result = removeInventoryItem(itemID, { removeContents: true });
        if (result && result.success === true) {
            removedCount += Math.max(1, Array.isArray(result.data && result.data.removedItems)
    ...
    const nativeWreckIDs = listStoredDungeonNativeWreckIDs(normalizedSystemID, siteID, instanceID, nativeControllerEntityIDs);
...
function removeSceneSiteContent(scene, siteID, options = {}) {
    ...
    }).filter((entity) => (normalizedInstanceID === null ||
    ...
    const nativeControllerEntityIDs = listStoredDungeonNativeControllerEntityIDs(Math.max(0, toInt(scene.systemID, 0)), normalizedSiteID, normalizedInstanceID, options.instance, contentEntities
    ...
    if (scene.dynamicEntities instanceof Map) {
        for (const entity of scene.dynamicEntities.values()) {
            const liveSiteID = normalizeCleanupIdentity(entity && entity.dungeonSiteID);
            const liveInstanceID = normalizeCleanupIdentity(entity && entity.dungeonSiteInstanceID);
            const hasExplicitScopeMismatch = (liveSiteID !== null &&
                !entityIDsEqual(liveSiteID, normalizedSiteID)) ||
                (liveInstanceID !== null &&
                    !entityIDsEqual(liveInstanceID, normalizedInstanceID));
            const isCanonicalNativeNpc = entity &&
                entity.nativeNpc === true &&
                !hasExplicitScopeMismatch &&
                exactIdentitySetHas(nativeControllerEntityIDs, entity.itemID);
            if (isCanonicalNativeNpc &&
    ...
    const storedInventoryItemIDs = listStoredDungeonInventoryItemIDs(Math.max(0, toInt(scene.systemID, 0)), normalizedSiteID, normalizedInstanceID, options.instance, nativeControllerEntityIDs);
    let removedCount = 0;
    for (const entity of contentEntities) {
        const removeResult = destroyMaterializedContentEntity(scene, entity, {
            broadcast: options.broadcast === true,
    ...
    for (const itemID of storedInventoryItemIDs) {
        ...
        const removeResult = spaceRuntime.destroyDynamicInventoryEntity(scene.systemID, itemID, {
            removeContents: true,
            broadcast: options.broadcast === true,
            excludedSession: options.excludedSession || null,
            nowMs: options.nowMs,
    ...
    const trackedInstanceID = normalizeCleanupIdentity(ensureSceneMaterializedSiteInstanceMap(scene).get(numericSiteID));
    const rootInstanceID = normalizeCleanupIdentity(scene.staticEntitiesByID instanceof Map &&
    ...
    const everyMaterializedOwnerMatches = materializedOwnerInstanceIDs.length > 0 &&
        materializedOwnerInstanceIDs.every((ownerInstanceID) => (entityIDsEqual(ownerInstanceID, normalizedInstanceID)));
...
function anyPlayerShipPresentAtSite(scene, instance, siteID) {
    ...
    const position = clonePosition((probe && probe.position) || (instance && instance.position));
    if (!position || !scene || !(scene.dynamicEntities instanceof Map)) {
        return false;
    }
    const instanceIdentity = normalizeCleanupIdentity((instance && instance.instanceID) ??
        (probe && probe.dungeonSiteInstanceID));
    const rangeSquared = SITE_OCCUPANCY_GRACE_RANGE_METERS * SITE_OCCUPANCY_GRACE_RANGE_METERS;
    for (const entity of scene.dynamicEntities.values()) {
        if (!entity || entity.kind !== "ship") {
            continue;
        }
        ...
        const shipInstanceIdentity = normalizeCleanupIdentity(entity.dungeonCurrentInstanceID ?? entity.dungeonSiteInstanceID);
        if (shipInstanceIdentity !== null &&
...
function releaseSurvivorsFromSitePocket(scene, entry) {
    ...
    for (const entity of scene.dynamicEntities.values()) {
        ...
        const belongsToThisPocket = instanceIdentity !== null
            ...
            : entityIDsEqual(normalizeCleanupIdentity(entity && entity.dungeonSiteID), siteIdentity);
        ...
        if (entityID > 0) {
            if (entity.nativeNpcWreck === true) {
                ...
                if (wreckRecord) {
                    nativeNpcStore.upsertNativeWreck(clearDungeonScopeFields(wreckRecord), { transient: wreckRecord.transient === true });
                }
            }
...
function schedulePendingSiteTeardown(scene, entry) {
    ...
    ensureScenePendingSiteTeardownMap(scene).set(buildPendingSiteTeardownKey(entry.siteIdentity, entry.instanceIdentity), entry);
    return true;
}
...
function handleRuntimeChange(change) {
    ...
    const runtime = getSpaceRuntime();
    const previousSystemID = Math.max(0, toInt(change && change.previousSolarSystemID, toInt(previous && previous.solarSystemID, 0)));
    const nextSystemID = Math.max(0, toInt(change && change.solarSystemID, toInt(next && next.solarSystemID, 0)));
    ...
    const previousSiteIdentity = normalizeCleanupIdentity(previous && previous.metadata && previous.metadata.siteID);
    const nextSiteIdentity = normalizeCleanupIdentity(next && next.metadata && next.metadata.siteID);
    const previousSiteID = toSafeCleanupNumber(previousSiteIdentity);
    const nextSiteID = toSafeCleanupNumber(nextSiteIdentity);
    const previousInstanceIdentity = normalizeCleanupIdentity(previous && previous.instanceID);
    const nextInstanceIdentity = normalizeCleanupIdentity(next && next.instanceID);
    const previousIdentityChangedInSystem = Boolean(previousWasVisible &&
        nextIsVisible &&
        previousSystemID > 0 &&
    ...
    if (previousSystemID > 0 &&
        ...
        if (previousScene) {
            ...
            const wentTerminalInPlace = Boolean(!nextIsVisible &&
                !previousIdentityChangedInSystem &&
                (nextSystemID <= 0 || nextSystemID === previousSystemID));
            if (wentTerminalInPlace &&
...
function sweepStoredTerminalDungeonSiteContent() {
    ...
    const terminalInstances = ["completed", "failed", "despawned"]
        .flatMap((lifecycleState) => (dungeonRuntime.listInstancesByLifecycle(lifecycleState, { full: true })))
        .filter((instance) => isManagedMaterializedSiteInstance(instance));
    ...
    for (const instance of terminalInstances) {
        ...
        const siteID = normalizeCleanupIdentity(instance && instance.metadata && instance.metadata.siteID);
        ...
        if (scene) {
            ensureScenePendingSiteTeardownMap(scene).delete(buildPendingSiteTeardownKey(siteID, instanceID));
...
//# sourceMappingURL=dungeonUniverseSiteService.js.map