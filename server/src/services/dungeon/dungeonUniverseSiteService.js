...
function buildMiningRockEntities(instance, siteEntity, populationHints) {
    ...
    return rockSpecs.map((rock, index) => {
        ...
        const shellTypeID = MINING_ROCK_SHELL_TYPE_IDS[(Math.abs(toInt(rock.oreTypeID, 0)) + index) % MINING_ROCK_SHELL_TYPE_IDS.length];
...
function buildEnvironmentEntities(instance, siteEntity, template, populationHints) {
    ...
    return selected.map((candidate, index) => {
        const resolvedTypeID = toInt(candidate.typeRecord && candidate.typeRecord.typeID, candidate.typeID) || candidate.typeID;
...
function spawnEncounterPlan(scene, instance, siteEntity, encounterPlan, options = {}) {
    ...
    if (identitySpawnEntries.length > 0) {
        ...
        const proceduralSpawnResult = npcSpawnService.spawnNpcBatchInSystem(scene.systemID, {
            ...
            amount: Math.max(1, Math.min(resolveEncounterSpawnEntryLimit(encounterPlan), toInt(encounterPlan.amount, 3))),
            transient: true,
            position: addVectors(encounterAnchorPosition, buildContentOffset(`${encounterKey}:encounter`, Math.max(0, toInt(encounterPlan.waveIndex, 1)) - 1, Math.max(1, toInt(options.totalPlans, 1)), {
                baseDistanceMeters: SITE_CONTENT_ENCOUNTER_OFFSET_METERS,
                jitterMeters: 8_000,
            })),
        ...
        if (aggregatedSpawns.length <= 0 &&
            ...
            const fallbackSpawnResult = npcSpawnService.spawnNpcBatchInSystem(scene.systemID, {
                ...
                amount: Math.max(1, Math.min(resolveEncounterSpawnEntryLimit(encounterPlan), toInt(encounterPlan.amount, 3))),
                transient: true,
                position: addVectors(encounterAnchorPosition, buildContentOffset(`${encounterKey}:encounter:fallback`, Math.max(0, toInt(encounterPlan.waveIndex, 1)) - 1, Math.max(1, toInt(options.totalPlans, 1)), {
                    baseDistanceMeters: SITE_CONTENT_ENCOUNTER_OFFSET_METERS,
                    jitterMeters: 8_000,
                })),
                anchorName: `${normalizeText(siteEntity && siteEntity.itemName, "Site")} ${normalizeText(encounterPlan.label, "Encounter")}`,
