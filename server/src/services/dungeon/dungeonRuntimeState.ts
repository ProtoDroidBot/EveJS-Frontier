...
function isRetiredMissionInstanceRecord(instanceRecord: Record<string, any> = {}) {
...
function listContentEntityItemIDs(instanceRecord: Record<string, any> = {}) {
  const refs = instanceRecord &&
    instanceRecord.spawnState &&
    instanceRecord.spawnState.contentEntityRefsByKey;
  return [...new Set(Object.values<any>(
...
function cleanupRetiredMissionInstanceInventory(table: Record<string, any> = {}) {
  const retiredItemIDs = new Set<any>();
  const retainedItemIDs = new Set<any>();
  for (const instanceRecord of Object.values<any>(table.instancesByID || {})) {
...
function normalizeNumberMap(value) {
  ...
  for (const [key, entry] of Object.entries<any>(value)) {
...
function normalizeOwnership(value: Record<string, any> = {}) {
  return {
...
function normalizeTimers(value: Record<string, any> = {}, lifecycleState = "seeded") {
...
function normalizeRoomState(roomKey, value: Record<string, any> = {}) {
  return {
    roomKey,
...
function normalizeGateState(gateKey, value: Record<string, any> = {}) {
  return {
    gateKey,
...
function normalizeObjectiveState(value: Record<string, any> = {}) {
  return {
    state: normalizeStateName(value.state, "pending"),
...
function normalizeInstanceRecord(record: Record<string, any> = {}) {
  ...
  for (const [roomKey, roomState] of Object.entries<any>(record.roomStatesByKey || {})) {
  ...
  for (const [gateKey, gateState] of Object.entries<any>(record.gateStatesByKey || {})) {
...
function normalizeUniverseReconcileMeta(value: Record<string, any> = {}) {
  const summary =
...
function normalizeState(table: Record<string, any> = {}) {
  const instancesByID: Record<string, any> = {};
  for (const [instanceKey, instanceRecord] of Object.entries<any>(table.instancesByID || {})) {
...
function buildCache(sourceState = null) {
  ...
  const activeExpiringEntries: any[] = [];
  const universePersistentTerminalInstanceIDs: any[] = [];
  let nextActiveExpiryAtMs = 0;

  for (const instance of Object.values<any>(state.instancesByID || {})) {
    ...
    if (instance.siteKey) {
      ...
      const existingIsActive = isActiveLifecycleState(
        summariesByID.get(existing) && summariesByID.get(existing).lifecycleState,
      );
  ...
  for (const indexMap of [
    ...
    for (const [key, values] of indexMap.entries()) {
      indexMap.set(key, [...new Set<any>(values)].sort((left, right) => left - right));
    }
  }
...
function appendSortedUniqueIndexEntry(indexMap, key, instanceID) {
  ...
  indexMap.set(
    normalizedKey,
    [...existing, instanceID].sort((left, right) => left - right),
  );
}
...
function refreshSiteKeyIndex(cacheState, siteKey) {
  ...
  for (const instanceID of cacheState.instanceIDsBySiteKey.get(normalizedSiteKey) || []) {
    ...
    const selectedIsActive = isActiveLifecycleState(
      selected && selected.lifecycleState,
    );
    const candidateIsActive = isActiveLifecycleState(instance.lifecycleState);
    if (
...
function addInstanceToCache(cacheState, rawInstance) {
  ...
  appendSortedUniqueIndexEntry(
    cacheState.instanceIDsBySystem,
    instance.solarSystemID,
    instanceID,
  );
  appendSortedUniqueIndexEntry(
    cacheState.instanceIDsByTemplate,
    instance.templateID,
    instanceID,
  );
  appendSortedUniqueIndexEntry(
    cacheState.instanceIDsByFamily,
    instance.siteFamily,
    instanceID,
  );
  appendSortedUniqueIndexEntry(
    cacheState.instanceIDsByLifecycle,
    instance.lifecycleState,
    instanceID,
  );
  appendSortedUniqueIndexEntry(
    cacheState.instanceIDsBySiteKey,
    instance.siteKey,
    instanceID,
  );
  if (isActiveLifecycleState(instance.lifecycleState)) {
    appendSortedUniqueIndexEntry(
      cacheState.activeInstanceIDsBySystem,
      instance.solarSystemID,
      instanceID,
    );
    const expiresAtMs = Math.max(
      0,
      toInt(instance && instance.timers && instance.timers.expiresAtMs, 0),
    );
    if (expiresAtMs > 0) {
      insertActiveExpiryEntry(
        cacheState.activeExpiringEntries,
        { instanceID, expiresAtMs },
      );
    }
  } else if (
    instance &&
  ...
  if (instance.siteKey) {
    ...
    const existingIsActive = isActiveLifecycleState(
      existingSummary && existingSummary.lifecycleState,
    );
    const candidateIsActive = isActiveLifecycleState(instance.lifecycleState);
    if (
...
function applyInstanceChanges(changes: Record<string, any> = {}) {
  const cacheState = ensureCache();
  const removeInstanceIDs = [...new Set(
    (Array.isArray(changes.removeInstanceIDs) ? changes.removeInstanceIDs : [])
  ...
  const upsertsByID = new Map(
    upsertInstances.map((instance) => [instance.instanceID, instance]),
  );
  const changedUpserts = [...upsertsByID.values()].filter((instance) => (
    !isDeepStrictEqual(cacheState.instancesByID.get(instance.instanceID), instance)
  ));
  const changedRemovals = removeInstanceIDs.filter((instanceID) => (
    cacheState.instancesByID.has(instanceID) && !upsertsByID.has(instanceID)
  ));
  const requestedNextSequence = changes.nextInstanceSequence == null
    ? cacheState.state.nextInstanceSequence
    : Math.max(
      cacheState.state.nextInstanceSequence,
      toInt(changes.nextInstanceSequence, cacheState.state.nextInstanceSequence),
    );
  ...
  for (const instance of changedUpserts) {
    const result = repo.write(
      ...
      `/instancesByID/${instance.instanceID}`,
      cloneValue(instance),
    );
...
function filterSummaryList(instanceIDs, options: Record<string, any> = {}) {
...
function listInstanceSummariesBySystem(solarSystemID, options: Record<string, any> = {}) {
...
function listAllInstanceSummaries(options: Record<string, any> = {}) {
  return filterSummaryList([...ensureCache().summariesByID.keys()], options);
}

function listInstanceSummariesByTemplate(templateID, options: Record<string, any> = {}) {
...
function listInstanceSummariesByFamily(siteFamily, options: Record<string, any> = {}) {
...
function listInstanceSummariesByLifecycle(lifecycleState, options: Record<string, any> = {}) {
...
function listExpiredActiveInstanceSummaries(nowMs, options: Record<string, any> = {}) {
...
function writeUniverseReconcileMeta(meta: Record<string, any> = {}) {
