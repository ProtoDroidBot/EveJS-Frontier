...
function normalizeOperationSpawnpointRecord(record) {
    ...
    const characterID = Math.max(0, toInt(source.characterID, toInt(source.charID, toInt(source.charid, 0))));
    if (siteID <= 0) {
        return null;
    }
    ...
    return {
        characterID,
        siteID,
        solarSystemID: Math.max(0, toInt(source.solarSystemID, toInt(source.solarsystemid2, toInt(source.solarsystemid, 0)))) || null,
...
function registerOperationSpawnpointRecord(record) {
    ...
    operationSpawnpointRecords.set(buildOperationSpawnpointKey(normalized.characterID, normalized.siteID), normalized);
    return normalized;
}
function resolveOperationSpawnpointRecord(characterID, siteID) {
    ...
    return (operationSpawnpointRecords.get(buildOperationSpawnpointKey(numericCharacterID, numericSiteID)) ||
...
//# sourceMappingURL=operationSiteRuntime.js.map