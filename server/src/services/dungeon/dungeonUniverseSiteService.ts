...
const {
  rollNpcLootEntries,
} = require(path.join(__dirname, "../../space/npc/npcLoot"));
const {
  entityIDsEqual,
  getEntityIDText,
...
function buildLootGrantEntry(itemType, quantity = 1, options: Record<string, any> = {}) {
...
function buildLootEntryFromNames(candidateNames, seed, minimum = 1, maximum = minimum, options: Record<string, any> = {}) {
...
function buildBoosterLootGrantEntries(seed, tags, context: Record<string, any> = {}) {
...
function resolveCombatLootFlavor(context: Record<string, any> = {}) {
...
function buildCombatLootGrantEntries(seed, tags, context: Record<string, any> = {}, options: Record<string, any> = {}) {
...
function resolveConfiguredContainerLootTable(containerEntity) {
  const configuredLoot = frontierDungeonLoot.resolveCargoContainerLootTable(
    containerEntity && containerEntity.typeID,
  );
  if (!configuredLoot) {
    return null;
  }
...
function buildLootGrantEntriesForContainer(containerEntity, populationHints, context: Record<string, any> = {}) {
  ...
  if (configuredLootTable) {
    return mergeLootGrantEntries(
      rollNpcLootEntries(configuredLootTable).slice(0, SITE_CONTENT_MAX_LOOT_ENTRIES),
    );
  }
...
function resolveFrontierDungeonTemplateGroupID(template) {
  const explicitGroupID = Math.max(
    0,
    toInt(template && (template.entryObjectGroupID || template.entryTypeGroupID), 0),
  );
  if (explicitGroupID > 0) {
    return explicitGroupID;
  }
  const entryTypeID = Math.max(
    0,
    toInt(template && (template.entryObjectTypeID || template.entryTypeID), 0),
  );
...
function selectEvenlySpacedEntries(entries, maximum) {
  ...
  const selected: any[] = [];
  const selectedIndexes = new Set<any>();
...
function resolveFrontierDungeonResourceBaseRadius(entry) {
  ...
  return Math.min(
    FRONTIER_DUNGEON_MINING_MAX_RADIUS_METERS / FRONTIER_DUNGEON_MINING_MAX_RADIUS_SCALE,
    Math.max(
      FRONTIER_DUNGEON_MINING_MIN_RADIUS_METERS / FRONTIER_DUNGEON_MINING_MIN_RADIUS_SCALE,
      sourceRadius,
    ),
  );
}
...
function estimateFrontierDungeonResourceQuantity(entry, radius) {
  ...
  const estimatedVolume = Math.min(
    maximumVolume,
    Math.max(minimumVolume, maximumVolume * areaRatio),
  );
...
function buildFrontierDungeonDerivedPopulationHints(template, instance = null) {
  ...
  const sourceDungeonID = Math.max(
    0,
    toInt(template && (template.sourceDungeonID || template.dungeonID || template._key), 0),
  );
  ...
  const lootContainerObjects = initialSceneObjects.filter((entry) => (
    Boolean(frontierDungeonLoot.resolveCargoContainerLootTable(entry.typeID))
  ));
  const lootContainerObjectIDs = new Set(
    lootContainerObjects.map((entry) => entry.objectID),
  );
  const nonLootSceneObjects = initialSceneObjects.filter(
    (entry) => !lootContainerObjectIDs.has(entry.objectID),
  );
  const resourceObjects = nonLootSceneObjects.filter((entry) => entry.categoryID === 25);
  const structuralObjects = nonLootSceneObjects.filter((entry) => (
    entry.categoryID !== 25 &&
    (
      entry.categoryID === 11 ||
  ...
  const remainingCapacity = Math.max(
    0,
    SITE_CONTENT_MAX_EXACT_ENVIRONMENT_PROPS - selected.length,
  );
  selected.push(...selectEvenlySpacedEntries(
    nonLootSceneObjects.filter((entry) => !selectedObjects.has(entry)),
    remainingCapacity,
  ));
...
function mergeFrontierDungeonPopulationHints(baseHints, derivedHints) {
  ...
  const configuredEnvironmentProps = baseEnvironmentProps.filter((entry) => (
    entry && entry.frontierDungeonConfiguredEntity === true
  ));
  const hasExactBaseEnvironment = baseEnvironmentProps.some((entry) => (
    entry &&
    entry.exact === true &&
  ...
  return {
    ...
    exactContentCaps: {
      ...
      containers: Math.max(
        containers.length,
        toInt(baseCaps.containers, 0),
        toInt(derivedCaps.containers, 0),
      ),
      environmentProps: Math.max(
        environmentProps.length,
        toInt(baseCaps.environmentProps, 0),
        toInt(derivedCaps.environmentProps, 0),
      ),
    },
    containers,
    environmentProps,
    frontierDungeonScene: true,
    roomCount: Math.max(
      toInt(clonedBase && clonedBase.roomCount, 0),
      toInt(derivedHints && derivedHints.roomCount, 0),
    ),
    source: normalizeText(
      clonedBase && clonedBase.source,
...
function buildMissionDerivedObjectiveMarkers(template) {
  ...
  const markers: Array<{ role: string; label: string; analyzer?: string | null }> = objectiveHints.map((label, index) => ({
...
function resolvePopulationHints(instance, template) {
  ...
  return mergeDerivedMissionPopulationHints(
    frontierHints,
    buildDerivedMissionPopulationHints(template),
  );
}
...
function upsertEncounterState(instanceID, planKey, patch: Record<string, any> = {}, options: Record<string, any> = {}) {
...
function syncEncounterStateProgress(scene, instance, plans, options: Record<string, any> = {}) {
...
function rematerializeStoredDungeonEncounterEntities(
  ...
  const storedEntityIDs = normalizeIDList(
    Object.values<any>(encounterStatesByKey).flatMap((state) => [
...
function rehydrateMissingEncounterStates(scene, instance, populationHints, options: Record<string, any> = {}) {
  const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
...
function buildContentOffset(seed, index, total, options: Record<string, any> = {}) {
...
function resolveEntityLabel(instance, template, entryObjectTypeRecord = null) {
  ...
  return normalizeText(
    ...
    normalizeText(
      resolveTypeRecordName(entryObjectTypeRecord),
      normalizeText(
        metadata.label,
        normalizeText(
          spawnState.label,
...
function buildSiteEntity(instance) {
  ...
  const presentedTypeID = CLIENT_CR_DUNGEON_GROUP_IDS.has(
      Math.max(0, toInt(entryObjectTypeRecord && entryObjectTypeRecord.groupID, 0)),
    )
    ? entryObjectTypeID
    : fallbackTypeID;
  ...
  const dungeonFactionKey = normalizeLowerText(
    instance && instance.dungeonFactionKey ||
  ...
  const dungeonFactionTag = normalizeLowerText(
    instance && instance.dungeonFactionTag ||
  ...
  return {
    ...
    signalTrackerSiteTypeID:
      toInt(typeRecord && typeRecord.typeID, presentedTypeID) || presentedTypeID,
...
function ensureSceneMaterializedSiteSet(scene) {
  ...
  if (!(scene._dungeonUniverseMaterializedSiteIDs instanceof Set)) {
    scene._dungeonUniverseMaterializedSiteIDs = new Set<any>();
  }
...
function resolveManagedUniverseSiteInstance(scene, instanceOrSite, options: Record<string, any> = {}) {
...
function listMaterializedUniverseSiteContentEntities(scene, options: Record<string, any> = {}) {
...
function listMaterializedUniverseSiteStaticContentEntities(scene, options: Record<string, any> = {}) {
...
function forceResyncSiteStaticContentForSession(scene, session, instanceOrSite, options: Record<string, any> = {}) {
  if (
    !scene ||
    !session ||
    !session._space ||
  ...
  const visibleStaticIDs =
    session._space.visibleBubbleScopedStaticEntityIDs instanceof Set
      ? new Set<any>(session._space.visibleBubbleScopedStaticEntityIDs)
      : new Set<any>();
  let missingCount = 0;
...
function upsertContentEntityRef(instanceID, contentKey, patch: Record<string, any> = {}, options: Record<string, any> = {}) {
...
function destroyMaterializedContentEntity(scene, entity, options: Record<string, any> = {}) {
  if (!scene || !entity) {
    return {
...
function buildContainerEntities(instance, siteEntity, populationHints) {
  ...
  return containerSpecs.map((container, index) => {
    ...
    const contentKey = normalizeText(
      `container:${container.role}:${containerIdentity}:${displayName}`,
      `container:${index + 1}`,
    ).toLowerCase();
    return {
...
function buildEncounterRewardContainerEntity(instance, siteEntity, encounterPlan, encounterState, populationHints) {
  ...
  return {
    ...
    position: addVectors(
      clonePosition(siteEntity && siteEntity.position),
      buildContentOffset(
        `${siteID}:encounter_reward:${normalizeText(encounterPlan && encounterPlan.key, String(rewardIndex))}`,
        rewardIndex - 1,
        totalPlans,
        {
...
function materializeEncounterRewardContainers(scene, instance, siteEntity, template, populationHints, options: Record<string, any> = {}) {
...
function materializeFetchObjectiveContainer(scene, instance, siteEntity, template, populationHints, options: Record<string, any> = {}) {
  if (!scene || !instance || !siteEntity) {
    return 0;
  }
...
function maybeCompleteClearedEncounterSite(instance, populationHints, options: Record<string, any> = {}) {
  if (!instance) {
    return false;
  }
  if (normalizeLowerText(instance && instance.siteKind, "") !== "anomaly") {
...
function resolveMaterializedContainerCompletionContext(scene, source = null, options: Record<string, any> = {}) {
  if (!scene) {
    return null;
  }
...
function maybeSatisfyMissionHackObjective(scene, source = null, options: Record<string, any> = {}) {
  ...
  if (!context || !isMissionRuntimeInstance(context.instance)) {
    return { success: false as const, completed: false, reason: "not_mission_instance" };
  }
...
function maybeCompleteMaterializedDataRelicSite(scene, source = null, options: Record<string, any> = {}) {
  const context = resolveMaterializedContainerCompletionContext(scene, source, options);
...
function maybeCompleteMaterializedDataRelicSiteForContainerID(scene, containerID, options: Record<string, any> = {}) {
...
function materializeContainerEntity(scene, instance, siteEntity, template, populationHints, containerEntity, options: Record<string, any> = {}) {
...
function broadcastStaticSiteContentBatch(scene, entities, options: Record<string, any> = {}) {
  if (
    !scene ||
...
function buildHazardEntities(instance, siteEntity, populationHints) {
  ...
  return hazards.map((hazard, index) => {
    ...
    return {
      kind: "siteHazardBeacon" as const,
      dungeonMaterializedSiteContent: true,
...
function buildGateEntities(instance, siteEntity, template) {
  ...
  const gateStatesByKey =
    ...
    typeof instance.gateStatesByKey === "object"
      ? instance.gateStatesByKey
      : {};
  const dedupedGateProfiles: any[] = [];
  const seenEntryGateKeys = new Set<any>();
  ...
  return gateProfiles.map((gateProfile, index) => {
    ...
    return {
      kind: "siteAccelerationGate" as const,
      dungeonMaterializedSiteContent: true,
...
function buildEnvironmentEntities(instance, siteEntity, template, populationHints) {
  ...
  for (const [environmentTemplateID, definition] of Object.entries<any>(resolvedTemplateCatalog)) {
  ...
  for (const environmentProp of hintedEnvironmentProps) {
    ...
    candidates.push({
      ...
      authoredRadius: Math.max(
        0,
        toFiniteNumber(environmentProp && environmentProp.authoredRadius, 0),
      ) || null,
      dungeonObjectGroupTag:
        normalizeText(environmentProp && environmentProp.dungeonObjectGroupTag, "") || null,
      dungeonObjectLocalName:
        normalizeText(environmentProp && environmentProp.dungeonObjectLocalName, "") || null,
      frontierDungeonResource:
        environmentProp && environmentProp.frontierDungeonResource === true,
      frontierRiftFormation:
        normalizeText(environmentProp && environmentProp.frontierRiftFormation, "") || null,
      frontierRiftYieldTier:
        normalizeText(environmentProp && environmentProp.frontierRiftYieldTier, "") || null,
      miningYieldTypeID: Math.max(
        0,
        toInt(environmentProp && environmentProp.miningYieldTypeID, 0),
      ) || null,
      resourceQuantity: Math.max(
        0,
        toInt(environmentProp && environmentProp.resourceQuantity, 0),
      ) || null,
...
function buildObjectiveEntities(instance, siteEntity, template, populationHints = null) {
  ...
  return markers.slice(0, total).map((marker, index) => {
    ...
    return {
      kind: "siteObjectiveMarker" as const,
      dungeonMaterializedSiteContent: true,
...
function listEncounterNotificationSessions(scene, options: Record<string, any> = {}) {
  const sessions: any[] = [];
  const seen = new Set<any>();
  ...
  if (typeof scene.sessions === "object") {
    for (const session of Object.values<any>(scene.sessions)) {
      pushSession(session);
    }
  }
  return sessions;
...
function sendEncounterTriggerNotifications(scene, instance, encounterPlan, options: Record<string, any> = {}) {
  const planKey = normalizeText(encounterPlan && encounterPlan.key, "");
  if (!planKey) {
    return {
      messagesSent: 0,
...
function sendCompletionTriggerNotifications(scene, instance, populationHints, options: Record<string, any> = {}) {
...
function shouldKillableStructureBlockEncounterProgress(spawnEntry, encounterPlan, context: Record<string, any> = {}) {
...
function buildKillableStructureEntity(instance, siteEntity, encounterPlan, spawnEntry, context: Record<string, any> = {}) {
  ...
  const slimLabel = suppressSlimName ? "" : label;
  return {
    kind: "siteKillableStructure" as const,
...
function armDeferredEncounterPlans(instance, populationHints, options: Record<string, any> = {}) {
  const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
  let armedCount = 0;
  for (const plan of resolveEncounterPlans(populationHints)) {
...
function spawnEncounterPlan(scene, instance, siteEntity, encounterPlan, options: Record<string, any> = {}) {
  ...
  if (!(scene._dungeonUniverseEncounterKeys instanceof Set)) {
    scene._dungeonUniverseEncounterKeys = new Set<any>();
  }
  ...
  const encounterScopeMetadata = {
    ...
    dungeonFactionKey:
      normalizeLowerText(siteEntity && siteEntity.dungeonFactionKey, "") || null,
    dungeonFactionTag:
      normalizeLowerText(siteEntity && siteEntity.dungeonFactionTag, "") || null,
  };
...
function processEncounterPlansForTrigger(scene, instance, siteEntity, populationHints, trigger, options: Record<string, any> = {}) {
...
function hasDueVisibleCountdownTrigger(instance, populationHints, options: Record<string, any> = {}) {
  const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
  for (const plan of resolveEncounterPlans(populationHints)) {
...
function maybeAdvanceEncounterDrivenProgression(instance, populationHints, options: Record<string, any> = {}) {
  if (!instance) {
    return {
      roomsCompleted: 0,
      gatesUnlocked: 0,
  ...
  const settledPlansByKey: Record<string, any> = {};
  for (const plan of plans) {
  ...
  if (refreshedInstance && refreshedInstance.objectiveState) {
    ...
    const nextPatch: Record<string, any> = {
      ...
      counters: {
        ...
        rooms_completed: Object.values<any>(refreshedInstance.roomStatesByKey || {})
...
function applyTriggeredSiteEffects(scene, instance, siteEntity, populationHints, trigger, options: Record<string, any> = {}) {
  const normalizedTrigger = normalizeLowerText(trigger, "");
...
function processProximityEncounterTriggers(scene, instance, siteEntity, populationHints, options: Record<string, any> = {}) {
  const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
...
function processMiningObjective(scene, instance, populationHints, options: Record<string, any> = {}) {
...
function maybeAdvanceExplicitlySatisfiedObjective(instance, options: Record<string, any> = {}) {
...
function syncAgentMissionForCompletedDungeonObjective(instance, options: Record<string, any> = {}) {
...
function tickSceneSiteBehaviors(scene, options: Record<string, any> = {}) {
  if (!scene) {
    return {
      armedCount: 0,
      encountersSpawned: 0,
...
function instanceReferencesEncounterEntityID(instance, entityID) {
  ...
  return Object.values<any>(encounterStatesByKey)
...
function markDestroyedObjectiveTargets(matchingInstances, destroyedEntity, options: Record<string, any> = {}) {
...
function maybeGrantFetchObjectiveItems(matchingInstances, destroyedEntity, options: Record<string, any> = {}) {
  if (!destroyedEntity) {
    return 0;
  }
  const candidate = {
...
function handleEncounterEntityDestroyed(scene, entityOrID, options: Record<string, any> = {}) {
...
function triggerSiteEncounter(scene, instanceOrID, trigger, options: Record<string, any> = {}) {
  if (!scene) {
    return {
      success: false as const,
...
function materializeSiteContents(scene, instance, siteEntity, template, options: Record<string, any> = {}) {
  ...
  if (environmentEntities.some((entity) => entity && entity.frontierDungeonResource === true)) {
    try {
      frontierDungeonMiningRuntimeState = require(path.join(
        __dirname,
...
function ensureSiteContentsMaterialized(scene, instanceOrSite, options: Record<string, any> = {}) {
  if (!scene) {
    return {
      success: false as const,
...
function upsertSceneEntity(scene, instance, options: Record<string, any> = {}) {
...
function removeSceneEntity(scene, siteID, options: Record<string, any> = {}) {
  if (!scene) {
    return false;
  }
...
function getCanonicalStoredContentItemIDs(instance, options: Record<string, any> = {}) {
  ...
  for (const entry of Object.values<any>(getContentEntityRefsByKey(instance))) {
...
function getCanonicalEncounterEntityIDs(instance, options: Record<string, any> = {}) {
  const systemID = normalizeCleanupIdentity(options.systemID);
  const siteID = normalizeCleanupIdentity(options.siteID);
  const instanceID = normalizeCleanupIdentity(options.instanceID);
  if (
  ...
  for (const encounterState of Object.values<any>(encounterStatesByKey)) {
...
function listStoredDungeonNativeControllerEntityIDs(
  ...
  const nativeControllerEntityIDs = new Set<any>();
...
function listStoredDungeonNativeWreckIDs(
  ...
  const nativeWreckIDs = new Set<any>();
...
function listStoredDungeonInventoryItemIDs(
  ...
  const inventoryItemIDs = new Set<any>();
  for (const item of Object.values<any>(getAllItems())) {
...
function removeStoredDungeonSiteContent(systemID, options: Record<string, any> = {}) {
...
function removeSceneSiteContent(scene, siteID, options: Record<string, any> = {}) {
  if (!scene) {
    return 0;
  }
  ...
  const contentEntityIDs = new Set<any>();
  for (const entity of contentEntities) {
...
function notifyTrackerDelta(systemID, siteKind, options: Record<string, any> = {}) {
...
// the scanner delta.
function runSiteTeardown(scene, entry, options: Record<string, any> = {}) {
  if (!scene || !entry) {
    return 0;
  }
...
function flushPendingSiteTeardownsForSite(scene, siteID, options: Record<string, any> = {}) {
  const pending = ensureScenePendingSiteTeardownMap(scene);
  if (pending.size <= 0) {
    return 0;
  }
...
function drainPendingSiteTeardowns(scene, options: Record<string, any> = {}) {
  const pending = ensureScenePendingSiteTeardownMap(scene);
  if (pending.size <= 0) {
    return 0;
  }
