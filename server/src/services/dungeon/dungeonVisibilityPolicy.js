...
const { entityIDsEqual, normalizePersistentEntityID, } = require("../../space/destiny/identity/entityID.js");
...
function isPrivateDungeonInstance(instance) {
    ...
    return (normalizeScope(instance.instanceScope) === "private" ||
...
function isPublicDungeonInstance(instance) {
    ...
    return (["public", "shared", "system"].includes(instanceScope) &&
...
function canSessionAccessDungeonInstance(session, instance) {
    ...
    if (identityMatches(characterID, ownership.characterID) ||
    ...
    const sharedWithCharacterIDs = ownership.sharedWithCharacterIDs instanceof Set
        ...
        : Array.isArray(ownership.sharedWithCharacterIDs)
            ? ownership.sharedWithCharacterIDs
            : [];
    if (sharedWithCharacterIDs.some((sharedCharacterID) => (exactPositiveIDsEqual(characterID, sharedCharacterID)))) {
        return true;
    }
...
function resolveDungeonInstanceVisibilityForSession(session, requestedInstanceID, options = {}) {
    ...
    if (Object.hasOwn(resolverOptions, "systemID")) {
        const requestedSystemID = normalizePersistentEntityID(resolverOptions.systemID);
        if (requestedSystemID === null ||
...
function resolveAuthorizedDungeonInstanceIDForSession(session, requestedInstanceID, options = {}) {
    const resolution = resolveDungeonInstanceVisibilityForSession(session, requestedInstanceID, options);
...
//# sourceMappingURL=dungeonVisibilityPolicy.js.map