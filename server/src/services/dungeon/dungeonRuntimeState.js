...
const { isRetiredOrdinaryEncounterMissionForAgent, } = require(path.join(__dirname, "../agent/missionAuthority"));
...
function isRetiredMissionInstanceRecord(instanceRecord = {}) {
    ...
    return isRetiredOrdinaryEncounterMissionForAgent(metadata.missionAgentID, metadata.missionContentID, {
        contentID: metadata.missionContentID,
        dungeonID: instanceRecord.sourceDungeonID,
        dungeonTemplateID: metadata.missionRuntimeTemplateID || instanceRecord.templateID,
...
function listContentEntityItemIDs(instanceRecord = {}) {
    ...
    return [...new Set(Object.values(refs && typeof refs === "object" && !Array.isArray(refs) ? refs : {})
...
function cleanupRetiredMissionInstanceInventory(table = {}) {
    ...
    const { removeInventoryItem } = require(path.join(__dirname, "../inventory/itemStore"));
...
function normalizeUniverseReconcileMeta(value = {}) {
    ...
    return {
        ...
        lastGeneratedIceDowntimeRestoreAtMs: Math.max(0, toInt(value.lastGeneratedIceDowntimeRestoreAtMs, 0)),
        lastGeneratedIceDowntimeRestoreRunAtMs: Math.max(0, toInt(value.lastGeneratedIceDowntimeRestoreRunAtMs, 0)),
        summary,
    };
}
function normalizeState(table = {}) {
    ...
    for (const [instanceKey, instanceRecord] of Object.entries(table.instancesByID || {})) {
        ...
        const normalized = normalizeInstanceRecord({
            ...instanceRecord,
            instanceID: toInt(instanceRecord && instanceRecord.instanceID, toInt(instanceKey, 0)),
        });
...
function buildCache(sourceState = null) {
    ...
    activeExpiringEntries.sort((left, right) => (left.expiresAtMs - right.expiresAtMs) || (left.instanceID - right.instanceID));
...
function applyPersistedDiff(nextState) {
    ...
    const current = currentResult && currentResult.success && currentResult.data && typeof currentResult.data === "object"
        ? currentResult.data
        : {};
    const writes = [];
    const removes = [];
    ...
    const prevInstances = current.instancesByID && typeof current.instancesByID === "object" ? current.instancesByID : {};
    ...
    if (Object.prototype.hasOwnProperty.call(nextState, "instancesByID") &&
...
function listInstanceSummariesByTemplate(templateID, options = {}) {
    ...
    return filterSummaryList(ensureCache().instanceIDsByTemplate.get(normalizedTemplateID) || [], options);
}
function listInstanceSummariesByFamily(siteFamily, options = {}) {
    ...
    return filterSummaryList(ensureCache().instanceIDsByFamily.get(normalizedFamily) || [], options);
}
function listInstanceSummariesByLifecycle(lifecycleState, options = {}) {
    ...
    return filterSummaryList(ensureCache().instanceIDsByLifecycle.get(normalizedLifecycleState) || [], {
        ...options,
...
function listExpiredActiveInstanceSummaries(nowMs, options = {}) {
    ...
    if (cacheState.activeExpiringEntries.length <= 0 ||
...
//# sourceMappingURL=dungeonRuntimeState.js.map