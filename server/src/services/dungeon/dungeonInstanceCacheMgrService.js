...
const targetIdRuntime = require(path.join(__dirname, "../exploration/signatures/targetIdRuntime"));
const wormholeRuntimeState = require(path.join(__dirname, "../exploration/wormholes/wormholeRuntimeState"));
...
function doesSummaryTopologyChangeMatter(beforeSummary, afterSummary) {
    ...
    return (toInt(beforeSummary && beforeSummary.solarSystemID, 0) !==
    ...
    if (typeof dungeonRuntime.registerInstanceChangeListener === "function") {
        dungeonRuntime.registerInstanceChangeListener((change) => {
            ...
            if (changeType === "created" || changeType === "removed") {
                if (isCacheSummaryEligible(change && change.before) ||
...
function incrementSystemCount(countsBySystem, solarSystemID, incrementBy = 1) {
    ...
    countsBySystem.set(normalizedSystemID, Math.max(0, toInt(countsBySystem.get(normalizedSystemID), 0)) +
        Math.max(0, toInt(incrementBy, 0)));
}
function buildCountDictFromMap(countsBySystem) {
    return buildDict([...countsBySystem.entries()]
...
function isBulwarkMiningSummary(summary) {
    ...
    if (BUILD_3396210_BULWARK_MINING_DUNGEON_IDS.has(Math.max(0, toInt(summary && summary.sourceDungeonID, 0)))) {
        return true;
    }
    ...
    return ((family.includes("bulwark") && family.includes("mining")) ||
...
function getSummaryMetadataSiteID(summary) {
    ...
    return Math.max(0, toInt(metadata.siteID, toInt(metadata.anchorItemID, 0)));
}
...
function hydrateSummary(summary) {
    const template = dungeonAuthority.getTemplateByID(normalizeText(summary && summary.templateID, ""));
    const position = summary && summary.position && typeof summary.position === "object"
        ? {
    ...
    const siteID = generatedIce
        ...
        : instanceID;
    const targetID = generatedIce && siteID > 0 && solarSystemID > 0
    ...
    return {
        ...
        targetID,
        solarSystemID,
        dungeonID: Math.max(0, toInt(summary && summary.sourceDungeonID, 0)) ||
            Math.max(0, toInt(template && template.sourceDungeonID, 0)),
        archetypeID: Math.max(0, toInt(summary && summary.archetypeID, 0)) ||
            Math.max(0, toInt(template && template.archetypeID, 0)),
        factionID: Math.max(0, toInt(summary && summary.factionID, 0)) ||
            Math.max(0, toInt(template && template.factionID, 0)) ||
            null,
        difficulty: Math.max(0, toInt(summary && summary.difficulty, 0)) ||
            Math.max(0, toInt(template && template.difficulty, 0)) ||
            1,
        entryObjectTypeID: Math.max(0, toInt(summary && summary.entryObjectTypeID, 0)) ||
            Math.max(0, toInt(template && template.entryObjectTypeID, 0)) ||
            null,
        dungeonNameID: Math.max(0, toInt(summary && summary.dungeonNameID, 0)) ||
...
function buildGroupedBuckets() {
    const groupedEntries = new Map(Object.keys(ARCHETYPE_SETS).map((key) => [key, new Map()]));
    const groupedCounts = new Map(Object.keys(ARCHETYPE_SETS).map((key) => [key, new Map()]));
    for (const summary of listActiveSharedInstanceSummaries()) {
        ...
        for (const [bucketKey, archetypeSet] of Object.entries(ARCHETYPE_SETS)) {
            ...
            countsBySystem.set(hydrated.solarSystemID, (countsBySystem.get(hydrated.solarSystemID) || 0) + 1);
        }
    }
    for (const entriesBySystem of groupedEntries.values()) {
        for (const records of entriesBySystem.values()) {
            records.sort((left, right) => (left.instanceID - right.instanceID ||
...
function buildGroupedEntryDict(bucketKey) {
    ...
    const dict = buildDict([...buckets.entries()]
        .sort((left, right) => left[0] - right[0])
...
function buildGroupedCountDict(bucketKey) {
    ...
    const dict = buildDict([...counts.entries()]
        .sort((left, right) => left[0] - right[0])
        .map(([solarSystemID, count]) => [solarSystemID, Math.max(0, toInt(count, 0))]));
...
//# sourceMappingURL=dungeonInstanceCacheMgrService.js.map