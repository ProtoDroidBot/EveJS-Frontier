...
function dotVectors(a, b) {
    return (toFiniteNumber(a && a.x, 0) * toFiniteNumber(b && b.x, 0) +
...
function resolveInstanceAnchor(instance) {
    ...
    const metadataPosition = instance.metadata && instance.metadata.position ? pointFromVector(instance.metadata.position) : null;
...
function resolveSiteWarpInProfile(instance) {
    ...
    return {
        boundaryRadiusMeters: Math.max(1, toFiniteNumber(configured.boundaryRadiusMeters, DUNGEON_SITE_WARP_IN_BOUNDARY_METERS)),
        collisionClearanceMeters: Math.max(0, toFiniteNumber(configured.collisionClearanceMeters, DUNGEON_WARP_IN_COLLISION_CLEARANCE_METERS)),
        template,
    };
}
...
function listMaterializedSiteObstacles(instance, scene, excludedEntityID = 0) {
    ...
    return listSceneEntities(scene).flatMap((entity) => {
        const entityInstanceID = Math.max(0, toInt(entity && (entity.dungeonSiteInstanceID ||
            entity.airNpeInstanceID ||
            entity.dungeonCurrentInstanceID), 0));
        if (entityInstanceID !== instanceID ||
...
function listAuthoredSiteObstacles(instance, template) {
    ...
    const entryObjectID = Math.max(0, toInt(template.entryObjectID ||
    ...
    for (const room of rooms) {
        ...
        for (const object of Array.isArray(room && room.objects) ? room.objects : []) {
            if (entryObjectID > 0 && toInt(object && object.objectID, 0) === entryObjectID) {
                entryRoom = room;
                entryPosition = addVectors(roomPosition, pointFromVector(object && object.position) || { x: 0, y: 0, z: 0 });
                break;
            }
        }
        if (entryRoom) {
    ...
    return (Array.isArray(entryRoom && entryRoom.objects) ? entryRoom.objects : []).flatMap((object) => {
        const absolutePosition = addVectors(roomPosition, pointFromVector(object && object.position) || { x: 0, y: 0, z: 0 });
        return [{
                position: addVectors(anchor, subtractVectors(absolutePosition, entryPosition)),
                radius: Math.max(0, toFiniteNumber(object && object.radius, DEFAULT_AUTHORED_OBJECT_RADIUS_METERS)),
            }];
    });
}
function resolveCollisionSafeRadialDistance(anchor, direction, initialDistance, obstacles, clearance, shipRadius) {
    ...
    for (let pass = 0; pass < MAX_WARP_IN_COLLISION_PASSES; pass += 1) {
        ...
        for (const obstacle of obstacles) {
            ...
            const perpendicularSquared = Math.max(0, offsetLengthSquared - (alongRay * alongRay));
            const exclusionRadius = Math.max(0, toFiniteNumber(obstacle && obstacle.radius, 0)) + shipRadius + clearance;
            ...
            if (radialDistance >= nearIntersection - 1 &&
...
function evaluateDeadspaceWarp(entity, destinationPoint, options = {}, scene = null) {
...
function resolveSiteWarpInPoint(instance, shipEntity = null, scene = null) {
    ...
    const direction = normalizeVector(shipPosition ? subtractVectors(shipPosition, anchor) : null, { x: 1, y: 0, z: 0 });
    const shipRadius = Math.max(0, toFiniteNumber(shipEntity && shipEntity.radius, 0));
    const obstacles = [
        ...listMaterializedSiteObstacles(instance, scene, Math.max(0, toInt(shipEntity && shipEntity.itemID, 0))),
        ...listAuthoredSiteObstacles(instance, profile.template),
    ];
    const radialDistance = resolveCollisionSafeRadialDistance(anchor, direction, profile.boundaryRadiusMeters + profile.collisionClearanceMeters + shipRadius, obstacles, profile.collisionClearanceMeters, shipRadius);
...
//# sourceMappingURL=deadspaceWarpPolicy.js.map