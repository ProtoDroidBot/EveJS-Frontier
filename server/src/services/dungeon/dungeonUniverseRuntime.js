...
function resolveProfileAllocationMode(family, bandProfile = null) {
    ...
    const explicitMode = normalizeLowerText(bandProfile && bandProfile.allocationMode, normalizeLowerText(profile.allocationMode, ""));
...
function resolveUniverseDungeonSlotsPerSystem(family, systemID, bandProfile = null) {
    const configuredSlots = Math.max(0, toInt(bandProfile && bandProfile.slotsPerSystem, 0));
    ...
    return Math.max(configuredSlots, dungeonSpawnEligibility.resolveUniverseDungeonSiteCount(systemID));
}
...
function getSecurityBand(systemID) {
    ...
    const securityStatus = toFiniteNumber(systemRecord && (systemRecord.securityStatus ?? systemRecord.security), 0);
...
function getSpawnProfileTemplateFilters(family) {
    ...
    const filters = profile && profile.templateFilters && typeof profile.templateFilters === "object"
...
function filterSystemIDsByScopedSelector(systemIDs, selector = {}) {
    ...
    const authoritySystemIDs = normalizeIntegerArray(normalizeTextArray(selector.systemAuthorityKeys)
    ...
    const hasScopedSelectors = scopedSystemIDs.size > 0 ||
        regionIDs.size > 0 ||
        constellationIDs.size > 0 ||
        wormholeClassIDs.size > 0;
    const hasExcludeSelectors = excludeSystemIDs.size > 0 ||
...
function listEligibleSystemIDsForBandProfile(family, band, bandProfile, systemIDs = null) {
    ...
    const fullBandSystemIDs = filterSystemIDsByScopedSelector(listSystemIDsByBand(normalizedBand), buildScopedSystemSelector(family, bandProfile));
    ...
    if (!eligible) {
        ...
        if (allocationMode === "baseline") {
            ...
            eligible = fullBandSystemIDs
                .map((systemID) => ({
                systemID,
                score: hashValue((toInt(systemID, 0) * 8191) +
                    hashText(family) +
...
function pickMiningGenerationConfig() {
    ...
    for (const [key, value] of Object.entries(config || {})) {
        if (key === "miningGeneratedIceSitesEnabled" ||
...
function resolveGeneratedMiningSiteLifetimeMs() {
    return resolveSiteLifetimeMs(Math.max(1, toInt(config && config.miningGeneratedIceSiteLifetimeMinutes, GENERATED_MINING_DEFAULT_SITE_LIFETIME_MINUTES)));
}
function buildBroadUniverseDescriptor(systemIDs = null) {
    ...
    for (const family of families) {
        ...
        for (const band of ["highsec", "lowsec", "nullsec", "wormhole"]) {
            ...
            const slotsPerSystem = Math.max(0, toInt(bandProfile && bandProfile.slotsPerSystem, 0));
            const eligibleSystemIDs = listEligibleSystemIDsForBandProfile(family, band, bandProfile, systemIDs);
            const effectiveSlotCounts = eligibleSystemIDs.map((systemID) => (resolveUniverseDungeonSlotsPerSystem(family, systemID, bandProfile)));
    ...
    const descriptor = {
        ...
        authorityTemplateCount: Math.max(0, toInt(authorityPayload && authorityPayload.counts && authorityPayload.counts.templateCount, 0)),
        spawnEligibility: {
            frontierDungeonAuthorityVersion: dungeonSpawnEligibility.FRONTIER_DUNGEON_SPAWN_AUTHORITY_VERSION,
            frontierDungeonGroupIDs: [...dungeonSpawnEligibility.FRONTIER_DUNGEON_SPAWN_GROUP_IDS],
            frontierDungeonIDs: [...getFrontierDungeonSpawnAuthorityIDs()],
            frontierResourceFieldPolicyVersion: asteroidData.FRONTIER_RESOURCE_FIELD_POLICY_VERSION,
            frontierResourceFieldsEnabled: asteroidData.FRONTIER_SYNTHETIC_RESOURCE_FIELDS_ENABLED === true,
        },
        familyPolicies,
        estimatedSiteCount,
    };
...
function buildMiningUniverseDescriptor(systemIDs = null) {
    ...
    const targetSlots = {};
    let estimatedSiteCount = 0;
    const authorityRows = iceSystemAuthority.listIceSystemAuthorityRows(Array.isArray(systemIDs) && systemIDs.length > 0
    ...
    for (const band of ["highsec", "lowsec", "nullsec", "wormhole"]) {
        ...
        targetSlots[band] = bandRows.reduce((sum, row) => sum + Math.max(0, toInt(row && row.slotCount, 1)), 0);
...
function getUniverseReconcileStatus(nowMs = Date.now()) {
    ...
    return {
        ...
        meta,
        descriptor,
        broadUpToDate: normalizeText(meta && meta.broadDescriptorKey, "") === descriptor.broadDescriptorKey,
        miningUpToDate: normalizeText(meta && meta.miningDescriptorKey, "") === descriptor.miningDescriptorKey,
        fullUpToDate: normalizeText(meta && meta.descriptorKey, "") === descriptor.descriptorKey,
    };
}
...
function buildUniverseAnchorCandidates(systemID, family) {
    ...
    const systemRecord = worldData.getSolarSystemByID(systemID) || null;
    const fallbackRadius = Math.max(1_000_000_000, Math.round(toFiniteNumber(systemRecord && systemRecord.radius, 0) * 0.25));
    return [{
            itemID: systemID,
...
function buildGeneratedMiningSpawnState(definition) {
    ...
    return {
        ...
        totalOriginalQuantity: members.reduce((sum, member) => sum + Math.max(0, toInt(member && member.originalQuantity, 0)), 0),
        totalRemainingQuantity: members.reduce((sum, member) => sum + Math.max(0, toInt(member && member.remainingQuantity, 0)), 0),
        resourceTypeIDs: Array.isArray(definition && definition.resourceTypeIDs)
            ? [...definition.resourceTypeIDs]
            : [],
...
function resolveGeneratedMiningTemplate(definition) {
    ...
    const resourceHintField = family === "gas"
        ? "gasTypeIDs"
        : (family === "ice"
            ? "iceTypeIDs"
            : "oreTypeIDs");
...
function enrichGeneratedMiningDefinition(definition, nowMs, options = {}) {
    ...
    const slotIndex = Math.max(0, toInt(definition && definition.localSiteIndex, options && options.slotIndex));
    const rotationIndex = Math.max(0, toInt(options && options.rotationIndex, definition && definition.rotationIndex));
    const startedAtMs = Math.max(0, toInt(options && options.startedAtMs, nowMs));
    const lifetimeMs = Math.max(60_000, toInt(options && options.lifetimeMs, resolveGeneratedMiningSiteLifetimeMs()));
    const placement = buildUniverseSitePlacement(toInt(definition && definition.solarSystemID, 0), normalizeText(definition && definition.family, "ice").toLowerCase(), slotIndex, rotationIndex);
...
function listDesiredGeneratedMiningDefinitions(systemIDs = null, nowMs = Date.now()) {
    ...
    const activeBySiteKey = new Map(listUniverseSeededGeneratedMiningInstances(eligibleSystemIDs)
        .map((instance) => [normalizeText(instance && instance.siteKey, ""), instance])
...
function buildGeneratedMiningDefinitionFromInstance(instance) {
    ...
    const rawSiteIndex = Math.max(0, toInt(metadata.rawSiteIndex, spawnState.rawSiteIndex));
    const localSiteIndex = Math.max(0, toInt(metadata.localSiteIndex, spawnState.localSiteIndex));
    const definition = {
        ...
        localSiteIndex,
        rawSiteIndex,
        rotationIndex: Math.max(0, toInt(metadata.rotationIndex, spawnState.rotationIndex)),
        siteID: Math.max(0, toInt(metadata.siteID, spawnState.siteID)),
        sourceDungeonID: Math.max(0, toInt(metadata.sourceDungeonID, spawnState.sourceDungeonID)) || null,
        templateID: normalizeText(instance && instance.templateID, normalizeText(spawnState.templateID, "")) || null,
        authorityKey: normalizeText(metadata.authorityKey, normalizeText(spawnState.authorityKey, "")) || null,
        authorityScope: normalizeText(metadata.authorityScope, normalizeText(spawnState.authorityScope, "")) || null,
        label: normalizeText(metadata.label, normalizeText(spawnState.label, "Mining Site")),
        securityBand: normalizeText(metadata.securityBand, normalizeText(spawnState.securityBand, getSecurityBand(instance && instance.solarSystemID))),
        memberCount: Math.max(0, toInt(spawnState.memberCount, Array.isArray(spawnState.members) ? spawnState.members.length : 0)),
        activeMemberCount: Math.max(0, toInt(spawnState.activeMemberCount, Array.isArray(spawnState.members)
        ...
        members: Array.isArray(spawnState.members)
            ...
            : [],
        position: instance && instance.position && typeof instance.position === "object"
...
function buildGeneratedMiningPersistedState(member, family, nowMs) {
    return {
        ...
        yieldKind: normalizeText(member && member.yieldKind, family).toLowerCase(),
...
function reconcileGeneratedMiningRuntimeState(definitions, systemIDs, nowMs) {
    ...
    for (const definition of Array.isArray(definitions) ? definitions : []) {
        ...
        for (const member of Array.isArray(spawnState.members) ? spawnState.members : []) {
            ...
            if (entityID > 0) {
                desiredByEntityID.set(entityID, buildGeneratedMiningPersistedState(member, definition && definition.siteFamily, nowMs));
            }
        }
    }
    let createdRows = 0;
    ...
    for (const [systemID, desiredByEntityID] of desiredBySystem.entries()) {
        ...
        for (const [entityIDKey, persistedState] of Object.entries(existingByEntityID)) {
            const descriptor = miningResourceSiteService.resolveGeneratedMiningEntityDescriptor(persistedState && persistedState.entityID != null
...
function resolveUniverseSiteLabel(template, family, slotIndex) {
    ...
    const templateMarker = Math.max(0, toInt(template && template.sourceDungeonID, 0)) ||
...
function listFamilyTemplatesForBand(family, band, bandProfile) {
    ...
    const candidates = allCandidates
        ...
        .sort((left, right) => (toInt(left && left.sourceDungeonID, Number.MAX_SAFE_INTEGER) -
        toInt(right && right.sourceDungeonID, Number.MAX_SAFE_INTEGER)) || String(left && left.templateID || "").localeCompare(String(right && right.templateID || "")));
...
function getSystemSecurityStatus(systemID) {
    const systemRecord = worldData.getSolarSystemByID(systemID) || null;
    return toFiniteNumber(systemRecord && (systemRecord.securityStatus ?? systemRecord.security), 0);
}
...
function resolveThreatDetectionSecurityBracket(systemID) {
    ...
    return table.find((entry) => (security >= entry.minSecurity &&
        (security < entry.maxSecurity ||
            (entry.maxSecurity === 0 && security <= 0)))) || table[table.length - 1];
}
...
function parseThreatDetectionUpgrade(upgrade) {
    const parsed = parseNumberedUpgradeName(upgrade && upgrade.name, /\b(minor|major)\s+threat detection array\s+([123])\b/i);
...
function parseExplorationDetectorUpgrade(upgrade) {
    const parsed = parseNumberedUpgradeName(upgrade && upgrade.name, /\b(exploration detector)\s+([123])\b/i);
    if (!parsed) {
        return null;
    }
    return {
...
function listParsedOnlineUpgrades(context, parser) {
    ...
    return [...byKey.values()]
        .sort((left, right) => (normalizeText(left.kind || left.mineral, "").localeCompare(normalizeText(right.kind || right.mineral, ""))) || (left.tier - right.tier) || (toInt(left.upgrade && left.upgrade.typeID, 0) - toInt(right.upgrade && right.upgrade.typeID, 0)));
}
...
function classifyCombatAnomalyTemplate(template) {
    const name = normalizeLowerText(template && (template.resolvedName || template.name || template.templateID), "");
...
function listCombatAnomalyTemplatesForLabel(label, options = {}) {
    const normalizedLabel = normalizeLowerText(label, "");
    ...
    const spawnFamilyKey = normalizeLowerText(options.spawnFamilyKey, "sov_threat_detection");
    ...
    return Object.freeze(templates.filter((template) => {
        const evaluation = evaluateTemplateSpawnPolicy(template, spawnFamilyKey, systemID, band, policyContext, options.policyOptions || {});
...
function listProspectingOreTemplates(mineral, tier) {
    ...
    const candidates = dungeonAuthority
        ...
        return ((leftRank < 0 ? Number.MAX_SAFE_INTEGER : leftRank) -
            (rightRank < 0 ? Number.MAX_SAFE_INTEGER : rightRank)) || (toInt(left && left.sourceDungeonID, Number.MAX_SAFE_INTEGER) -
            toInt(right && right.sourceDungeonID, Number.MAX_SAFE_INTEGER)) || String(left && left.templateID || "").localeCompare(String(right && right.templateID || ""));
    });
...
function buildPolicyCandidateCacheKey(family, systemID, band, bandProfile, policyContext, policyOptions = {}) {
    ...
    return JSON.stringify({
        policyVersion: dungeonSiteSpawnPolicy.POLICY_VERSION,
        factionClassifierVersion: Math.max(0, toInt(dungeonSiteSpawnPolicy.getPolicyDescriptor().factionClassifierVersion, 0)),
        family: normalizeLowerText(family, "unknown"),
        band: normalizeSecurityBand(band),
        difficultyRange: normalizeDifficultyRange(bandProfile && bandProfile.difficultyRange),
        ...
        reviewedNeutralTemplateIDs: [...new Set((Array.isArray(policyOptions.reviewedNeutralTemplateIDs)
...
function listPolicyEligibleTemplatesForBand(family, systemID, band, bandProfile, policyContext, policyOptions = {}) {
    const cacheKey = buildPolicyCandidateCacheKey(family, systemID, band, bandProfile, policyContext, policyOptions);
    ...
    const eligible = Object.freeze(listFamilyTemplatesForBand(family, band, bandProfile)
        .map((template) => ({
        template,
        evaluation: evaluateTemplateSpawnPolicy(template, family, systemID, band, policyContext, policyOptions),
    }))
        .filter((entry) => entry.evaluation && entry.evaluation.allowed !== false));
    policyEligibleTemplateCandidatesCache.set(cacheKey, eligible);
    return eligible;
}
function pickUniverseTemplateForSlotWithPolicy(family, systemID, slotIndex, rotationIndex, band, bandProfile, options = {}) {
    const policyContext = options.policyContext ||
        buildTemplatePolicyContext(family, systemID, band);
    const policyOptions = options.policyOptions || {};
    const eligible = listPolicyEligibleTemplatesForBand(family, systemID, band, bandProfile, policyContext, policyOptions);
...
function pickRandomArrayIndex(length, rng = Math.random) {
    ...
    return Math.max(0, Math.min(size - 1, Math.floor((Number.isFinite(randomValue) ? randomValue : Math.random()) * size)));
}
...
function pickRandomUniverseTemplateForSlotWithPolicy(family, systemID, slotIndex, rotationIndex, band, bandProfile, options = {}) {
    const policyContext = options.policyContext ||
        buildTemplatePolicyContext(family, systemID, band);
    const policyOptions = options.policyOptions || {};
    const eligible = listPolicyEligibleTemplatesForBand(family, systemID, band, bandProfile, policyContext, policyOptions);
...
function pickUniverseTemplateForSlot(family, systemID, slotIndex, rotationIndex, band, bandProfile) {
    const selection = pickUniverseTemplateForSlotWithPolicy(family, systemID, slotIndex, rotationIndex, band, bandProfile);
...
function buildUniverseSiteDefinition(template, family, systemID, slotIndex, options = {}) {
    ...
    const dungeonFactionKey = normalizeLowerText(customMetadata.dungeonFactionKey ||
        customSpawnState.dungeonFactionKey ||
        (template && template.frontierFactionKey), "") || null;
    const dungeonFactionTag = normalizeLowerText(customMetadata.dungeonFactionTag ||
        customSpawnState.dungeonFactionTag ||
        (template && template.frontierFactionTag), "") || null;
    const siteOrigin = normalizeLowerText(options.siteOrigin, normalizeLowerText(dungeonAuthority.getSpawnProfile(spawnFamilyKey) &&
        dungeonAuthority.getSpawnProfile(spawnFamilyKey).siteOrigin, "universe_dungeon"));
    const spawnPolicy = dungeonSiteSpawnPolicy.buildSelectionMetadata(options.spawnPolicyEvaluation ||
        evaluateTemplateSpawnPolicy(template, spawnFamilyKey, systemID, band, options.policyContext || {}, options.policyOptions || {}));
    const metadata = {
        providerID,
...
function buildSovSpawnPolicyEvaluation(policyKey, upgradeCategory, sourceRefs, extra = {}) {
    ...
    return {
        allowed: true,
        reason: normalizeText(localPirateEvaluation && localPirateEvaluation.reason, normalizeText(extra.reason, "allowed_by_online_sov_hub_upgrade")),
        policyKey,
        ...
        localPirateFactionKey: normalizeLowerText(localPirateEvaluation && localPirateEvaluation.localPirateFactionKey, "") || null,
        localPirateFactionSource: normalizeLowerText(localPirateEvaluation && localPirateEvaluation.localPirateFactionSource, "") || null,
        localPirateFactionSpecialSpace: localPirateEvaluation && localPirateEvaluation.localPirateFactionSpecialSpace === true,
        templatePirateFactionKeys: Array.isArray(localPirateEvaluation && localPirateEvaluation.templatePirateFactionKeys)
            ? [...localPirateEvaluation.templatePirateFactionKeys]
            : [],
        factionClassifierVersion: Math.max(0, toInt(localPirateEvaluation && localPirateEvaluation.factionClassifierVersion, 0)),
        reviewedNeutralOverride: localPirateEvaluation && localPirateEvaluation.reviewedNeutralOverride === true,
        sourceRefs: [...new Set([
...
function buildSovGuaranteedDefinition(template, spawnFamilyKey, targetSystemID, slotIndex, nowMs, sovHub, options = {}) {
    ...
    return buildUniverseSiteDefinition(template, spawnFamilyKey, targetSystemID, slotIndex, {
        ...
        spawnPolicyEvaluation: buildSovSpawnPolicyEvaluation(normalizeText(options.policyKey, `${spawnFamilyKey}_guaranteed`), normalizeText(sovHub.upgradeCategory, ""), sourceRefs, {
...
function resolveDesiredSovSlotRotationIndex(options, spawnFamilyKey, targetSystemID, slotIndex) {
...
function buildThreatDetectionDefinitionsForSystem(systemID, context, nowMs, slotAllocations, options = {}) {
    ...
    for (const upgrade of upgrades) {
        ...
        for (const slot of slots) {
            ...
            const rotationIndex = resolveDesiredSovSlotRotationIndex(options, "sov_threat_detection", systemID, slotIndex);
            const template = pickDeterministicTemplate(listCombatAnomalyTemplatesForLabel(slot.label, {
                systemID,
                band: "nullsec",
            ...
            const localPirateEvaluation = evaluateTemplateSpawnPolicy(template, "sov_threat_detection", systemID, "nullsec", normalizedContext, options.policyOptions || {});
            ...
            const definition = buildSovGuaranteedDefinition(template, "sov_threat_detection", systemID, slotIndex, nowMs, sovHub, {
                rotationIndex,
...
function buildProspectingDefinitionsForSystem(systemID, context, nowMs, slotAllocations, options = {}) {
    ...
    for (const upgrade of upgrades) {
        ...
        const rotationIndex = resolveDesiredSovSlotRotationIndex(options, "sov_prospecting", systemID, slotIndex);
        const template = pickDeterministicTemplate(listProspectingOreTemplates(upgrade.mineral, upgrade.tier), `sov-prospecting:${systemID}:${upgrade.mineral}:${upgrade.tier}:${rotationIndex}`);
        if (!template) {
            continue;
        }
        const sovHub = {
        ...
        const definition = buildSovGuaranteedDefinition(template, "sov_prospecting", systemID, slotIndex, nowMs, sovHub, {
            rotationIndex,
...
function pickExplorationDetectorTarget(sourceSystemID, tier, slotOrdinal) {
    ...
    const offset = hashValue((sourceSystemID * 6151) +
        (Math.max(1, toInt(tier, 1)) * 313) +
        (Math.max(0, toInt(slotOrdinal, 0)) * 37)) % candidates.length;
    return candidates[offset] || candidates[0] || null;
}
function pickExplorationDetectorTemplate(family, sourceSystemID, targetSystemID, tier, slotOrdinal, rotationIndex, policyOptions = {}) {
    const band = getSecurityBand(targetSystemID);
    ...
    const candidates = dungeonAuthority
        ...
        .filter((template) => {
        const evaluation = evaluateTemplateSpawnPolicy(template, family, targetSystemID, band, policyContext, policyOptions);
        return evaluation && evaluation.allowed !== false;
    })
        .sort((left, right) => (toInt(left && left.sourceDungeonID, Number.MAX_SAFE_INTEGER) -
        toInt(right && right.sourceDungeonID, Number.MAX_SAFE_INTEGER)) || String(left && left.templateID || "").localeCompare(String(right && right.templateID || "")));
    return pickDeterministicTemplate(candidates, `sov-exploration:${sourceSystemID}:${targetSystemID}:${family}:${tier}:${slotOrdinal}:${rotationIndex || 0}`);
}
    ...
    for (const sourceSystemID of listSovereigntySourceSystemIDs()) {
        ...
        for (const upgrade of upgrades) {
            ...
            for (let slotOrdinal = 0; slotOrdinal < slotCount; slotOrdinal += 1) {
                ...
                const family = EXPLORATION_DETECTOR_FAMILY_SEQUENCE[slotOrdinal % EXPLORATION_DETECTOR_FAMILY_SEQUENCE.length];
                const slotIndex = allocateSovSlotIndex(slotAllocations, "sov_exploration_detector", target.systemID);
                const rotationIndex = resolveDesiredSovSlotRotationIndex(options, "sov_exploration_detector", target.systemID, slotIndex);
                const template = pickExplorationDetectorTemplate(family, sourceSystemID, target.systemID, upgrade.tier, slotOrdinal, rotationIndex, options.policyOptions || {});
                if (!template) {
                    continue;
                }
                const targetBand = getSecurityBand(target.systemID);
                const targetPolicyContext = buildTemplatePolicyContext(family, target.systemID, targetBand);
                const localPirateEvaluation = evaluateTemplateSpawnPolicy(template, family, target.systemID, targetBand, targetPolicyContext, options.policyOptions || {});
                if (!localPirateEvaluation || localPirateEvaluation.allowed === false) {
                    continue;
                }
                const sovHub = {
                ...
                const definition = buildSovGuaranteedDefinition(template, "sov_exploration_detector", target.systemID, slotIndex, nowMs, sovHub, {
                    rotationIndex,
...
function summarizeSovDefinitions(definitions, families) {
    for (const definition of definitions) {
        const family = normalizeLowerText(definition && definition.metadata && definition.metadata.spawnFamilyKey, normalizeLowerText(definition && definition.spawnState && definition.spawnState.spawnFamilyKey, ""));
        if (!family) {
            continue;
        }
        if (!families[family]) {
    ...
    for (const family of Object.keys(families)) {
        const systems = new Set(definitions
            .filter((definition) => (normalizeLowerText(definition && definition.metadata && definition.metadata.spawnFamilyKey, "") === family))
...
function listRandomAllocationCandidateSystemIDs(family, band, bandProfile) {
    return filterSystemIDsByScopedSelector(listSystemIDsByBand(band), buildScopedSystemSelector(family, bandProfile));
}
function getInstanceSpawnFamilyKey(instance) {
    return normalizeLowerText(instance && instance.metadata && instance.metadata.spawnFamilyKey, normalizeLowerText(instance && instance.spawnState && instance.spawnState.spawnFamilyKey, instance && instance.siteFamily));
}
function getInstanceSlotIndex(instance) {
    return Math.max(0, toInt(instance && instance.metadata && instance.metadata.slotIndex, instance && instance.spawnState && instance.spawnState.slotIndex));
}
function getInstanceRotationIndex(instance) {
    return Math.max(0, toInt(instance && instance.metadata && instance.metadata.rotationIndex, instance && instance.spawnState && instance.spawnState.rotationIndex));
}
function bandProfileUsesRandomAllocation(family, band, bandProfile) {
    const profile = dungeonAuthority.getSpawnProfile(family);
    if (resolveProfileAllocationMode(family, bandProfile) !== "random" ||
        !profile ||
...
function isWakeAllocatedUniverseFamily(family) {
    ...
    return Boolean(profile && ["highsec", "lowsec", "nullsec", "wormhole"].some((band) => {
        const allocationMode = resolveProfileAllocationMode(family, profile.bands && profile.bands[band]);
...
function listStableWakeAllocationCandidateSystemIDs(family, band, bandProfile) {
    const cacheKey = `${SYSTEM_WAKE_ALLOCATION_VERSION}:${buildBandProfileCacheKey(family, band, bandProfile)}`;
    if (!stableWakeCandidateRankingsByFamilyBand.has(cacheKey)) {
        const ranked = listRandomAllocationCandidateSystemIDs(family, band, bandProfile)
            .map((systemID) => ({
            systemID,
            score: hashText(`${SYSTEM_WAKE_ALLOCATION_VERSION}:${normalizeLowerText(family, "")}:` +
...
function listActiveRandomAllocatedInstances(family, band, candidateSystemIDs, activeInstances = null) {
    ...
    return sourceInstances
        .filter((instance) => (normalizeLowerText(instance && instance.siteOrigin, "") === "universe_dungeon" &&
        ...
        .sort((left, right) => (Math.max(0, toInt(left && left.instanceID, 0)) -
        Math.max(0, toInt(right && right.instanceID, 0))));
}
function buildRandomAllocatedDefinitionForSlot(family, band, bandProfile, systemID, slotIndex, nowMs, options = {}) {
    ...
    if (existing && existing.templateID) {
        ...
        if (template) {
            evaluation = evaluateTemplateSpawnPolicy(template, family, systemID, band, policyContext, options.policyOptions || {});
    ...
    if (!template) {
        const selection = pickRandomUniverseTemplateForSlotWithPolicy(family, systemID, slotIndex, rotationIndex, band, bandProfile, {
            policyContext,
            policyOptions: options.policyOptions,
            rng,
        });
...
function buildRandomAllocatedSystemPlanForBand(family, band, bandProfile, options = {}) {
    ...
    const activeInstances = listActiveRandomAllocatedInstances(family, band, candidateSystemIDs, options.activeInstances);
    ...
    const retainedSystemIDs = [...existingBySystem.keys()]
        .sort((left, right) => {
        const leftFirst = Math.min(...[...existingBySystem.get(left).values()].map((entry) => Math.max(0, toInt(entry && entry.instanceID, 0))));
        const rightFirst = Math.min(...[...existingBySystem.get(right).values()].map((entry) => Math.max(0, toInt(entry && entry.instanceID, 0))));
    ...
    const remainingCandidates = candidateSystemIDs.filter((systemID) => (!retainedSet.has(systemID) && !configuredSet.has(systemID)));
    ...
    const fallbackSystemIDs = typeof options.rng === "function"
        ...
        : listStableWakeAllocationCandidateSystemIDs(family, band, bandProfile)
            .filter((systemID) => (!retainedSet.has(systemID) && !configuredSet.has(systemID)))
...
function buildRandomAllocatedUniverseDefinitionsForBand(family, band, bandProfile, systemIDs, nowMs, options = {}) {
    ...
    for (const systemID of plan.allocatedSystemIDs) {
        ...
        for (let slotIndex = 0; slotIndex < plan.slotsPerSystem; slotIndex += 1) {
            const definition = buildRandomAllocatedDefinitionForSlot(family, band, bandProfile, systemID, slotIndex, nowMs, {
                ...options,
...
function chooseRandomRespawnSystemID(family, band, bandProfile, previousSystemID, options = {}) {
    ...
    const activeSystemIDs = new Set(listActiveRandomAllocatedInstances(family, band, candidateSystemIDs)
...
function buildExistingSovRotationBySlot(systemIDs) {
    ...
    for (const instance of listUniverseSeededPersistentSiteInstances(systemIDs)) {
        ...
        if (instanceIDsBySlot.has(slotKey) &&
    ...
    if (includeFamily("sov_threat_detection") || includeFamily("sov_prospecting")) {
        for (const systemID of targetSystemIDs) {
            ...
            if (includeFamily("sov_threat_detection")) {
                definitions.push(...buildThreatDetectionDefinitionsForSystem(systemID, context, nowMs, slotAllocations, sovOptions));
            }
            if (includeFamily("sov_prospecting")) {
                definitions.push(...buildProspectingDefinitionsForSystem(systemID, context, nowMs, slotAllocations, sovOptions));
            }
        }
    }
    if (includeFamily("sov_exploration_detector")) {
        definitions.push(...buildExplorationDetectorDefinitions(targetSystemIDs, nowMs, slotAllocations, sovOptions));
    }
...
function listDesiredUniverseDungeonSiteDefinitions(systemIDs = null, nowMs = Date.now(), options = {}) {
    ...
    for (const instance of listUniverseSeededPersistentSiteInstances(allSystemIDs)) {
        ...
        const allocationMode = normalizeLowerText(instance && instance.metadata && instance.metadata.allocationMode, instance && instance.spawnState && instance.spawnState.allocationMode);
    ...
    for (const family of families) {
        ...
        for (const band of ["highsec", "lowsec", "nullsec", "wormhole"]) {
            ...
            if (bandProfileUsesRandomAllocation(family, band, bandProfile)) {
                const randomResult = buildRandomAllocatedUniverseDefinitionsForBand(family, band, bandProfile, allSystemIDs, nowMs, options);
                definitions.push(...randomResult.definitions);
                familySummaries[family].desiredSiteCount += randomResult.definitions.length;
                familySummaries[family].systemsTouched += new Set(randomResult.definitions.map((definition) => Math.max(0, toInt(definition && definition.solarSystemID, 0)))).size;
            ...
            for (const systemID of eligibleSystemIDs) {
                ...
                for (let slotIndex = 0; slotIndex < slotsPerSystem; slotIndex += 1) {
                    ...
                    let evaluation = template
                        ? evaluateTemplateSpawnPolicy(template, family, systemID, band, policyContext, options.policyOptions || {})
                        : null;
                    if (evaluation && evaluation.allowed === false) {
                        template = null;
                        evaluation = null;
...
function buildGeneratedMiningRotationDefinitionFromInstance(instance, nowMs = Date.now()) {
    ...
    const slotIndex = Math.max(0, toInt(instance.metadata && instance.metadata.slotIndex, instance.spawnState && instance.spawnState.slotIndex));
    const currentRotationIndex = Math.max(0, toInt(instance.metadata && instance.metadata.rotationIndex, instance.spawnState && instance.spawnState.rotationIndex));
    const nextRotationIndex = currentRotationIndex + 1;
    const definition = miningResourceSiteService.buildGeneratedResourceSiteDefinition(systemID, family, slotIndex, {
...
function buildSovereigntyRotationDefinitionFromInstance(instance, nowMs = Date.now(), options = {}) {
    if (!instance) {
        return null;
    }
    const spawnFamilyKey = normalizeLowerText(instance && instance.metadata && instance.metadata.spawnFamilyKey, normalizeLowerText(instance && instance.spawnState && instance.spawnState.spawnFamilyKey, ""));
    if (!isSovereigntyGuaranteedSpawnFamily(spawnFamilyKey)) {
        return null;
    }
    const sovHub = normalizeObject(instance && instance.metadata && instance.metadata.sovHub);
    ...
    if (spawnFamilyKey === "sov_threat_detection") {
        const upgrades = listParsedOnlineUpgrades(normalizedContext, parseThreatDetectionUpgrade);
        const matchingUpgrade = upgrades.find((upgrade) => (upgrade.kind === normalizeLowerText(sovHub.upgradeKind, "") &&
            upgrade.tier === Math.max(1, toInt(sovHub.upgradeTier, 1))));
        if (!matchingUpgrade || sourceSystemID !== targetSystemID) {
            return null;
        }
        targetPolicyContext = buildTemplatePolicyContext("sov_threat_detection", targetSystemID, "nullsec");
        template = pickDeterministicTemplate(listCombatAnomalyTemplatesForLabel(sovHub.ccpSiteLabel, {
        ...
        const upgrades = listParsedOnlineUpgrades(normalizedContext, parseProspectingUpgrade);
        const matchingUpgrade = upgrades.find((upgrade) => (upgrade.mineral === normalizeLowerText(sovHub.mineral, "") &&
            upgrade.tier === Math.max(1, toInt(sovHub.upgradeTier, 1))));
        if (!matchingUpgrade || sourceSystemID !== targetSystemID) {
            return null;
        }
        template = pickDeterministicTemplate(listProspectingOreTemplates(sovHub.mineral, sovHub.upgradeTier), `sov-prospecting:${targetSystemID}:${sovHub.mineral}:${sovHub.upgradeTier}:${nextRotationIndex}`);
    }
    else if (spawnFamilyKey === "sov_exploration_detector") {
        const upgrades = listParsedOnlineUpgrades(normalizedContext, parseExplorationDetectorUpgrade);
        const matchingUpgrade = upgrades.find((upgrade) => (upgrade.tier === Math.max(1, toInt(sovHub.upgradeTier, 1))));
        ...
        template = pickExplorationDetectorTemplate(family, sourceSystemID, targetSystemID, sovHub.upgradeTier, sovHub.detectorSlotOrdinal, nextRotationIndex, options.policyOptions || {});
        targetPolicyContext = buildTemplatePolicyContext(family, targetSystemID, getSecurityBand(targetSystemID));
    }
    if (!template) {
        return null;
    }
    if (spawnFamilyKey === "sov_threat_detection" ||
        ...
        localPirateEvaluation = evaluateTemplateSpawnPolicy(template, policyFamily, targetSystemID, getSecurityBand(targetSystemID), targetPolicyContext, options.policyOptions || {});
        if (!localPirateEvaluation || localPirateEvaluation.allowed === false) {
            return null;
        }
    }
    return buildSovGuaranteedDefinition(template, spawnFamilyKey, targetSystemID, slotIndex, nowMs, sovHub, {
        rotationIndex: nextRotationIndex,
...
function buildRotationDefinitionFromInstance(instance, nowMs = Date.now(), options = {}) {
    if (!instance ||
    ...
    const spawnFamilyKey = normalizeLowerText(instance &&
        instance.metadata &&
        instance.metadata.spawnFamilyKey, normalizeLowerText(instance &&
        instance.spawnState &&
        instance.spawnState.spawnFamilyKey, family));
    const systemID = Math.max(0, toInt(instance.solarSystemID, 0));
    if (isSovereigntyGuaranteedSpawnFamily(spawnFamilyKey)) {
    ...
    if (randomAllocationBand) {
        const nextSystemID = chooseRandomRespawnSystemID(spawnFamilyKey, band, bandProfile, systemID, {
            rng: Math.random,
        });
        if (nextSystemID <= 0) {
            return null;
        }
        return buildRandomAllocatedDefinitionForSlot(spawnFamilyKey, band, bandProfile, nextSystemID, slotIndex, nowMs, {
            rotationIndex: nextRotationIndex,
            rng: Math.random,
            policyOptions: options.policyOptions,
        });
    ...
    const selection = pickUniverseTemplateForSlotWithPolicy(spawnFamilyKey, systemID, slotIndex, nextRotationIndex, band, bandProfile, {
        policyContext,
        policyOptions: options.policyOptions,
    });
...
function listProtectedGeneratedMiningSiteKeys(systemIDs, nowMs = Date.now()) {
    const targetedSystemIDs = new Set(normalizeSystemIDs(systemIDs));
    const candidates = (Array.isArray(systemIDs) &&
        systemIDs.length > 0 &&
        targetedSystemIDs.size > 0 &&
        targetedSystemIDs.size <= INDEXED_SYSTEM_LOOKUP_LIMIT)
        ? [...targetedSystemIDs].flatMap((systemID) => (dungeonRuntime.listInstancesBySystem(systemID, { full: true }))).filter((instance) => (!dungeonRuntimeState.isActiveLifecycleState(instance && instance.lifecycleState) &&
            instance &&
    ...
    return candidates
        .filter((instance) => (normalizeLowerText(instance && instance.siteOrigin, "") === "generatedmining" &&
...
function resolveDowntimeClockUtc(options = {}) {
    ...
    const hour = Math.min(23, Math.max(0, toInt(hasExplicitHour
        ? options.downtimeHourUtc
        : (explicit ? DAILY_DOWNTIME_HOUR_UTC : configClock.hour), DAILY_DOWNTIME_HOUR_UTC)));
    const minute = Math.min(59, Math.max(0, toInt(hasExplicitMinute ? options.downtimeMinuteUtc : (explicit ? 0 : configClock.minute), 0)));
    const second = Math.min(59, Math.max(0, toInt(hasExplicitSecond ? options.downtimeSecondUtc : (explicit ? 0 : configClock.second), 0)));
    return {
        hour,
        minute,
        second,
...
function resolveLastDailyDowntimeAtMs(nowMs = Date.now(), options = {}) {
    ...
    const todayDowntimeMs = Date.UTC(referenceDate.getUTCFullYear(), referenceDate.getUTCMonth(), referenceDate.getUTCDate(), clock.hour, clock.minute, clock.second, 0);
...
function resolveGeneratedIceDowntimeBoundary(options = {}, nowMs = Date.now()) {
    const explicitDowntimeAtMs = firstPositiveInt(options.downtimeAtMs, options.lastDowntimeAtMs, options.downtimeStartMs);
...
function advanceUniversePersistentSites(options = {}) {
    ...
    const emitExpiryEvents = options.emitExpiryEvents !== false &&
    ...
    const candidates = dungeonRuntime
        ...
        .filter((instance) => (!hasSystemFilter ||
    ...
    if (Math.max(0, toInt(expired && expired.expiredCount, 0)) <= 0 &&
        candidates.length <= 0) {
        return {
            expiredCount: 0,
            rotatedCount: 0,
...
function restoreGeneratedIceAfterDowntime(options = {}) {
    ...
    const meta = dungeonRuntimeState.getUniverseReconcileMeta();
    const lastAppliedDowntimeAtMs = Math.max(0, toInt(meta && meta.lastGeneratedIceDowntimeRestoreAtMs, 0));
    if (options.skipAlreadyApplied !== false &&
    ...
    for (const instance of terminalInstances) {
        ...
        const completedAtMs = Math.max(0, toInt(instance && instance.timers && instance.timers.completedAtMs, 0));
    ...
    if (affectedSystemIDs.length > 0) {
        runtimeStateSummary = reconcileGeneratedMiningRuntimeState(listActiveGeneratedMiningDefinitionsFromRuntime(affectedSystemIDs), affectedSystemIDs, nowMs);
        if (Math.max(0, toInt(rotationSummary && rotationSummary.rotatedCount, 0)) > 0) {
...
function listUniverseSeededGeneratedMiningInstances(systemIDs = null) {
    ...
    const candidates = (Array.isArray(systemIDs) &&
        ...
        targetedSystemIDs.size > 0 &&
        targetedSystemIDs.size <= INDEXED_SYSTEM_LOOKUP_LIMIT)
        ? [...targetedSystemIDs].flatMap((systemID) => (dungeonRuntime.listInstancesBySystem(systemID, { full: true, activeOnly: true })))
        : [
    ...
    return candidates.filter((instance) => (instance &&
...
function resetStartupMiningAnomalies(options = {}) {
    ...
    const existing = listStartupResetMiningAnomalyInstances(hasSystemFilter ? targetedSystemIDs : options.systemIDs);
    const affectedSystemIDs = new Set();
    ...
    const miningStateSummary = reconcileGeneratedMiningRuntimeState(listActiveGeneratedMiningDefinitionsFromRuntime([
        ...iceSystemIDs,
        ...generatedMiningSystemIDs,
    ]), [...new Set([...iceSystemIDs, ...generatedMiningSystemIDs])], nowMs);
    ...
    return {
        ...
        createdInstances: Math.max(0, toInt(miningInstanceSummary && miningInstanceSummary.createdCount, 0)) +
            Math.max(0, toInt(oreInstanceSummary && oreInstanceSummary.createdCount, 0)),
        retainedInstances: Math.max(0, toInt(miningInstanceSummary && miningInstanceSummary.retainedCount, 0)) +
            Math.max(0, toInt(oreInstanceSummary && oreInstanceSummary.retainedCount, 0)),
        replacedInstances: Math.max(0, toInt(miningInstanceSummary && miningInstanceSummary.replacedCount, 0)) +
            Math.max(0, toInt(oreInstanceSummary && oreInstanceSummary.replacedCount, 0)),
        removedInstances: Math.max(0, toInt(miningInstanceSummary && miningInstanceSummary.removedCount, 0)) +
...
function resolveInstanceSourceDungeonID(instance) {
    ...
    return Math.max(0, toInt(instance && instance.sourceDungeonID, toInt(metadata.sourceDungeonID, toInt(spawnState.sourceDungeonID, toInt(template && template.sourceDungeonID, 0)))));
}
...
function isGeneratedIceRuntimeInstance(instance) {
    ...
    return (siteOrigin === "generatedmining" ||
        providerID === "generatedmining" ||
...
function buildGeneratedIceAuthorityAuditEntry(instance) {
    ...
    const authorityKey = normalizeText(metadata.authorityKey, normalizeText(spawnState.authorityKey, ""));
    const reasons = [];
    if (!authorityRow) {
...
function cleanupInvalidGeneratedIceAuthority(options = {}) {
    ...
    const affectedSystemIDs = [...new Set(invalidEntries
    ...
    const miningState = affectedSystemIDs.length > 0
        ? reconcileGeneratedMiningRuntimeState(listActiveGeneratedMiningDefinitionsFromRuntime(affectedSystemIDs), affectedSystemIDs, nowMs)
        : {
            createdRows: 0,
            updatedRows: 0,
            removedRows: 0,
...
function reconcileUniversePersistentSites(options = {}) {
    ...
    if (logProgress) {
        log.info(`[DungeonUniverse] ${progressLabel}: starting reconcile for ${systemIDs.length} systems ` +
    ...
    const miningDefinitions = includeMining
        ...
        : [];
    if (logProgress) {
        log.info(`[DungeonUniverse] ${progressLabel}: built ${miningDefinitions.length} generated mining definitions in ${Date.now() - miningStartMs}ms`);
    }
    ...
    if (logProgress) {
        log.info(`[DungeonUniverse] ${progressLabel}: built ${broadResult.definitions.length} broad persistent definitions in ${Date.now() - broadStartMs}ms`);
    }
    ...
    if (logProgress) {
        log.info(`[DungeonUniverse] ${progressLabel}: reconciled ${allDefinitions.length} dungeon instances in ${Date.now() - instanceStartMs}ms`);
    }
    const miningStateStartMs = Date.now();
    const persistedMiningState = reconcileGeneratedMiningRuntimeState(listActiveGeneratedMiningDefinitionsFromRuntime(systemIDs), systemIDs, nowMs);
    ...
    if (logProgress) {
        log.info(`[DungeonUniverse] ${progressLabel}: reconciled mining runtime rows in ${Date.now() - miningStateStartMs}ms`);
    }
    const summary = {
    ...
    if (logProgress) {
        log.info(`[DungeonUniverse] ${progressLabel}: done in ${summary.elapsedMs}ms ` +
    ...
    if ((!Array.isArray(options.systemIDs) || options.systemIDs.length <= 0) &&
...
function scheduleNextBackgroundReconcileSlice() {
    ...
    const baseDelayMs = backgroundReconcileJob.sliceCount <= 0
        ...
        : Math.max(BACKGROUND_RECONCILE_DELAY_MS, Math.max(0, toInt(backgroundReconcileJob.retryNotBeforeMs, 0) - Date.now()));
    const delayMs = Math.max(baseDelayMs, interactiveWorkloadGate.getBackgroundDeferralDelayMs());
        ...
        setImmediate(() => {
            ...
            Promise.resolve(runBackgroundUniverseReconcileSlice()).catch((error) => {
                if (backgroundReconcileJob) {
                    backgroundReconcileJob.consecutiveFailureCount = Math.max(1, toInt(backgroundReconcileJob.consecutiveFailureCount, 0) + 1);
                    const retryDelayMs = Math.min(30_000, 1_000 * (2 ** Math.min(5, backgroundReconcileJob.consecutiveFailureCount - 1)));
                    backgroundReconcileJob.retryNotBeforeMs = Date.now() + retryDelayMs;
                }
                log.warn(`[DungeonUniverse] background reconcile slice failed: ${error.message}`);
                if (backgroundReconcileJob) {
                    scheduleNextBackgroundReconcileSlice();
                }
            });
...
function completeBackgroundUniverseReconcileJob() {
    ...
    log.info(`[DungeonUniverse] background full reconcile complete in ${summary.elapsedMs}ms ` +
...
async function buildRandomAllocatedSystemPlanForFamilyInWorker(family) {
    ...
    try {
        return await worldPlanningPool.buildRandomAllocatedSystemPlanForFamily(family, { activeInstances });
    }
    catch (error) {
        log.warn(`[DungeonUniverse] isolated worker planning failed for ${family}: ${error.message}`);
        throw error;
    }
}
async function reconcileUniverseSeededInstancesInWorker(definitions = [], options = {}) {
...
async function getBackgroundFamilyAllocationPlan(job, family) {
    if (backgroundFamilyAllocationCache &&
...
async function runBackgroundUniverseReconcileSlice() {
    ...
    if (sliceSystemIDs.length <= 0) {
        ...
        log.info(`[DungeonUniverse] background full reconcile: switching to ${job.familyQueue[job.familyIndex]} ` +
    ...
    if (family === "generatedmining") {
        ...
        const broadResult = allocatedSystemIDsByBand
            ? listDesiredUniverseDungeonSiteDefinitions(sliceSystemIDs, job.nowMs, {
                families: [family],
                allocatedSystemIDsByBand,
            })
            : listDesiredUniverseDungeonSiteDefinitions(sliceSystemIDs, job.nowMs, { families: [family] });
    ...
    else if (sliceElapsedMs < Math.max(1, Math.floor(job.targetSliceMs / 2)) &&
        job.batchSize < job.maxBatchSize) {
        job.batchSize = Math.min(job.maxBatchSize, job.batchSize + 1);
    }
    if (job.systemIndex >= job.systemIDs.length ||
        job.sliceCount === 1 ||
        (job.sliceCount % 10) === 0) {
        log.info(`[DungeonUniverse] background full reconcile: ${family} slice ${job.sliceCount} ` +
...
function scheduleBackgroundUniverseReconcile(options = {}) {
    ...
    backgroundReconcileJob = {
        ...
        familyIndex: 0,
        systemIndex: 0,
        batchSize: Math.max(1, toInt(options.batchSize, BACKGROUND_RECONCILE_INITIAL_BATCH_SIZE)),
        maxBatchSize: Math.max(Math.max(1, toInt(options.batchSize, BACKGROUND_RECONCILE_INITIAL_BATCH_SIZE)), toInt(options.maxBatchSize, BACKGROUND_RECONCILE_MAX_BATCH_SIZE)),
        targetSliceMs: Math.max(1, toInt(options.targetSliceMs, BACKGROUND_RECONCILE_TARGET_SLICE_MS)),
    ...
    log.info(`[DungeonUniverse] queued background full reconcile for ${systemIDs.length} systems ` +
...
function buildSystemUniverseReconcileFamilyQueue(options = {}) {
    ...
    if (options.includeMining !== false &&
...
function completeSystemUniverseReconcileJob(job, failure = null) {
    ...
    if (failed) {
        log.warn(`[DungeonUniverse] background system reconcile failed system=${job.systemID} ` +
            `family=${failure.family || "unknown"}: ${failure.error.message}`);
    }
    else if (job.logProgress === true || summary.elapsedMs >= 500) {
        log.info(`[DungeonUniverse] background system reconcile complete system=${job.systemID} ` +
    ...
    for (const callback of callbacks) {
        try {
            ...
            log.warn(`[DungeonUniverse] background system reconcile callback failed ` +
...
function reconcileSystemUniverseFamilySlice(job, family) {
    if (family === "generatedmining") {
        ...
        const persistedMiningState = reconcileGeneratedMiningRuntimeState(listActiveGeneratedMiningDefinitionsFromRuntime([job.systemID]), [job.systemID], job.nowMs);
        ...
        job.miningStateRowsRemoved += (Math.max(0, toInt(persistedMiningState && persistedMiningState.removedRows, 0)) +
            Math.max(0, toInt(invalidGeneratedIceCleanup && invalidGeneratedIceCleanup.miningStateRowsRemoved, 0)));
        job.invalidGeneratedIceScanned += Math.max(0, toInt(invalidGeneratedIceCleanup && invalidGeneratedIceCleanup.scannedCount, 0));
        job.invalidGeneratedIceFound += Math.max(0, toInt(invalidGeneratedIceCleanup && invalidGeneratedIceCleanup.invalidCount, 0));
        job.invalidGeneratedIceDespawned += Math.max(0, toInt(invalidGeneratedIceCleanup && invalidGeneratedIceCleanup.despawnedCount, 0));
    ...
    const broadResult = listDesiredUniverseDungeonSiteDefinitions([job.systemID], job.nowMs, definitionOptions);
...
async function runSystemUniverseReconcileSlice() {
    ...
    if (elapsedMs >= 500 || job.logProgress === true) {
        log.info(`[DungeonUniverse] background system reconcile system=${job.systemID} ` +
...
function scheduleSystemUniversePersistentSitesReconcile(systemID, options = {}) {
    ...
    const job = {
        ...
        allocatedSystemIDsByBand: options.allocatedSystemIDsByBand && typeof options.allocatedSystemIDsByBand === "object"
...
function prepareSystemForPlayerEntry(systemID, options = {}) {
    const numericSystemID = Math.max(0, toInt(systemID, 0));
    if (numericSystemID <= 0) {
        return {
            success: false,
...
function ensureSystemUniversePersistentSites(systemID, options = {}) {
    ...
    const persistedMiningState = includeMining
        ? reconcileGeneratedMiningRuntimeState(listActiveGeneratedMiningDefinitionsFromRuntime([numericSystemID]), [numericSystemID], nowMs)
        : {
            createdRows: 0,
            updatedRows: 0,
            removedRows: 0,
...
function prepareStartupUniversePersistentSites(options = {}) {
    const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
    const hasExplicitStartupSystemIDs = Array.isArray(options.startupSystemIDs) ||
    ...
    const shouldReconcileStartupSystems = options.reconcileStartupSystems !== false &&
    ...
    const startupSummary = shouldReconcileStartupSystems
                ...
                const scheduled = scheduleSystemUniversePersistentSitesReconcile(systemID, {
                    ...
                    onComplete: (result) => {
                        ...
                        try {
                            ...
                            const siteService = require(path.join(__dirname, "./dungeonUniverseSiteService"));
                            if (typeof siteService.handleSceneCreated === "function") {
                                siteService.handleSceneCreated(scene, { force: true });
                            }
                        }
                        catch (error) {
                            log.warn(`[DungeonUniverse] startup system materialization failed ` +
        ...
        : (startupSystemIDs.length > 0
            ? {
                ...
                miningStateRowsRemoved: 0,
                skipped: true,
                reason: statusBeforeStartupReconcile.fullUpToDate === true
...
function startTicker(options = {}) {
    ...
    const intervalMs = Math.max(250, toInt(options.intervalMs, UNIVERSE_SLOT_TICK_INTERVAL_MS));
    universeReconcileTicker = setInterval(() => {
        try {
            ...
            if (summary.rotatedCount > 0) {
                log.info(`[DungeonUniverse] rotated ${summary.rotatedCount} persistent site slots ` +
...
//# sourceMappingURL=dungeonUniverseRuntime.js.map