...
const DUNGEON_UNIVERSE_PLANNING_TASK = path.join(__dirname, "dungeonUniversePlanningTask.js");
...
function getLandscapeDungeonSpawnExclusionIDs() {
    if (!landscapeDungeonSpawnExclusionIDsCache) {
        ...
        landscapeDungeonSpawnExclusionIDsCache = new Set(dungeonSpawnEligibility.collectLandscapeDungeonSpawnExclusionIDs(worldSnapshot));
    }
...
function getFrontierDungeonSpawnAuthorityIDs() {
    if (!frontierDungeonSpawnAuthorityIDsCache) {
        const worldSnapshot = worldData.ensureLoaded();
        frontierDungeonSpawnAuthorityIDsCache = new Set(dungeonSpawnEligibility.collectFrontierDungeonSpawnAuthorityIDs(worldSnapshot));
    }
...
function isUniverseSpawnEligibleTemplate(template) {
    return Boolean(template && template.templateID) &&
        dungeonSpawnEligibility.isTemplateEligibleForUniverseSpawning(template, getFrontierDungeonSpawnAuthorityIDs(), getLandscapeDungeonSpawnExclusionIDs());
}
...
function buildBroadUniverseDescriptor(systemIDs = null) {
    ...
    const descriptor = {
        ...
        spawnEligibility: {
            ...
            landscapeExclusionVersion: dungeonSpawnEligibility.LANDSCAPE_SPAWN_EXCLUSION_VERSION,
