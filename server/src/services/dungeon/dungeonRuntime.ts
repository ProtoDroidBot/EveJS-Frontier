...
const instanceChangeListeners = new Set<any>();
...
function cloneObjectiveStateForComparison(value: Record<string, any> = {}) {
...
function normalizeOwnership(options: Record<string, any> = {}) {
...
function emitInstanceChange(changeType, beforeInstance, afterInstance, extra: Record<string, any> = {}) {
...
function normalizeConnections(template) {
  ...
  if (template && template.connections && typeof template.connections === "object") {
    return Object.entries<any>(template.connections)
      .filter(([, entry]) => entry && typeof entry === "object")
...
function buildDefaultRoomStates(template, nowMs, options: Record<string, any> = {}) {
  ...
  const roomStatesByKey: Record<string, any> = {};
...
function buildDefaultGateStates(template, nowMs, options: Record<string, any> = {}) {
  ...
  const gateStatesByKey: Record<string, any> = {};
...
function ensureTemplateRuntimeState(instanceID, options: Record<string, any> = {}) {
  ...
  const gatePatch: Record<string, any> = {};

  for (const [roomKey, roomState] of Object.entries<any>(defaultRoomStates)) {
  ...
  for (const [gateKey, gateState] of Object.entries<any>(defaultGateStates)) {
...
function buildDefaultObjectiveState(template, nowMs, options: Record<string, any> = {}) {
...
function buildDefaultEnvironmentState(template, nowMs, options: Record<string, any> = {}) {
...
function dungeonError(code, message = code) {
  const error: Error & Record<string, any> = new Error(String(message || code || "DUNGEON_ERROR"));
...
function buildInstanceRecordFromOptions(instanceID, template, options: Record<string, any> = {}, nowMs = Date.now()) {
  ...
  const dungeonFactionKey = normalizeText(
    options.dungeonFactionKey ||
    metadata.dungeonFactionKey ||
    spawnState.dungeonFactionKey ||
    (template && template.frontierFactionKey),
    "",
  ).toLowerCase() || null;
  const dungeonFactionTag = normalizeText(
    options.dungeonFactionTag ||
...
function normalizeTextFilterSet(values: any[] = []) {
...
function listUniverseSeededReconcileInstances(options: Record<string, any> = {}) {
  ...
  const candidateSummaries = (
    ...
    ? [...targetedSystemIDs].flatMap((systemID) => (
      runtimeState.listInstanceSummariesBySystem(systemID)
    ))
    : runtimeState.listAllInstanceSummaries().filter((summary) => (
      targetedSystemIDs.size <= 0 ||
...
function buildUniverseReconcileSnapshotFingerprint(instances: any[] = [], nextInstanceSequence = 1) {
...
function snapshotUniverseSeededReconcileState(options: Record<string, any> = {}) {
  const normalizedOptions = normalizeUniverseReconcileOptions(options);
  ...
  return {
    ...
    fingerprint: buildUniverseReconcileSnapshotFingerprint(
      instances,
      nextInstanceSequence,
    ),
  };
...
function buildUniverseSeededReconcilePlan(
  definitions: any[] = [],
  ...
  const baseNextInstanceSequence = Math.max(
    1,
    toInt(snapshot.nextInstanceSequence, 1),
  );
  let nextInstanceSequence = baseNextInstanceSequence;
  const upsertInstances: any[] = [];
  for (const definition of normalizedDefinitions) {
  ...
  return {
    baseFingerprint: snapshot.fingerprint || buildUniverseReconcileSnapshotFingerprint(
      snapshot.instances,
      baseNextInstanceSequence,
    ),
    options: normalizedOptions,
    mutation: {
...
function applyUniverseSeededReconcilePlan(plan) {
  ...
  if (currentSnapshot.fingerprint !== plan.baseFingerprint) {
    const error: Error & Record<string, any> = new Error(
      "universe reconcile plan is stale",
    );
...
function reconcileUniverseSeededInstances(definitions: any[] = [], options: Record<string, any> = {}) {
  ...
  const preserveSiteKeys = new Set(
    (Array.isArray(options.preserveSiteKeys) ? options.preserveSiteKeys : [])
...
function rotateUniversePersistentInstances(rotations: any[] = [], options: Record<string, any> = {}) {
...
function activateRoom(instanceID, roomKey, options: Record<string, any> = {}) {
  const existing = requireInstance(instanceID);
...
function completeRoom(instanceID, roomKey, options: Record<string, any> = {}) {
  const existing = requireInstance(instanceID);
  const normalizedRoomKey = normalizeText(roomKey, "");
  if (!normalizedRoomKey) {
    throw dungeonError("DUNGEON_ROOM_KEY_REQUIRED");
...
function unlockGate(instanceID, gateKey, options: Record<string, any> = {}) {
  const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
...
function recordGateUse(instanceID, gateKey, options: Record<string, any> = {}) {
  const existing = requireInstance(instanceID);
...
function advanceObjective(instanceID, patch: Record<string, any> = {}, options: Record<string, any> = {}) {
  const existing = requireInstance(instanceID);
  const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
...
function upsertRoomState(instanceID, roomKey, patch: Record<string, any> = {}, options: Record<string, any> = {}) {
  const existing = requireInstance(instanceID);
  const normalizedRoomKey = normalizeText(roomKey, "");
  if (!normalizedRoomKey) {
    throw dungeonError("DUNGEON_ROOM_KEY_REQUIRED");
...
function upsertGateState(instanceID, gateKey, patch: Record<string, any> = {}, options: Record<string, any> = {}) {
  const existing = requireInstance(instanceID);
  const normalizedGateKey = normalizeText(gateKey, "");
  if (!normalizedGateKey) {
    throw dungeonError("DUNGEON_GATE_KEY_REQUIRED");
...
function mergeObjectiveState(instanceID, patch: Record<string, any> = {}, options: Record<string, any> = {}) {
  const existing = requireInstance(instanceID);
  const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
...
function setLifecycleState(instanceID, lifecycleState, options: Record<string, any> = {}) {
  const existing = requireInstance(instanceID);
...
function markInstanceObjectiveSatisfied(instanceID, options: Record<string, any> = {}) {
  const existing = requireInstance(instanceID);
...
function purgeInstances(instanceIDs: any[] = [], options: Record<string, any> = {}) {
...
function purgeShadowProviderInstances(options: Record<string, any> = {}) {
  ...
  runtimeState.mutateState((table) => {
    const instancesByID = table.instancesByID || {};
    for (const [instanceID, rawInstance] of Object.entries<any>(instancesByID)) {
      const instance = runtimeState.normalizeInstanceRecord({
        ...rawInstance,
...
function findInstanceBySiteKey(siteKey, options: Record<string, any> = {}) {
...
function listInstancesBySystem(solarSystemID, options: Record<string, any> = {}) {
...
function listActiveInstancesBySystem(solarSystemID, options: Record<string, any> = {}) {
...
function listInstancesByTemplate(templateID, options: Record<string, any> = {}) {
...
function listInstancesByFamily(siteFamily, options: Record<string, any> = {}) {
...
function listInstancesByLifecycle(lifecycleState, options: Record<string, any> = {}) {
...
function mergeStateField(instanceID, fieldName, patch: Record<string, any> = {}, options: Record<string, any> = {}) {
  const existing = requireInstance(instanceID);
  const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
  const normalizedPatch = patch && typeof patch === "object" ? cloneValue(patch) : {};
...
function mergeHazardState(instanceID, patch: Record<string, any> = {}, options: Record<string, any> = {}) {
  return mergeStateField(instanceID, "hazardState", patch, options);
}

function mergeEnvironmentState(instanceID, patch: Record<string, any> = {}, options: Record<string, any> = {}) {
  return mergeStateField(instanceID, "environmentState", patch, options);
}

function mergeSpawnState(instanceID, patch: Record<string, any> = {}, options: Record<string, any> = {}) {
  return mergeStateField(instanceID, "spawnState", patch, options);
}

function synchronizeInstancePosition(instanceID, position, options: Record<string, any> = {}) {
  const existing = requireInstance(instanceID);
  const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
...
function tickRuntime(options: Record<string, any> = {}) {
  const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
...
function listUniversePersistentTerminalInstances(options: Record<string, any> = {}) {
