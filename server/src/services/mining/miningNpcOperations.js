...
const { ONE_AU_IN_METERS, findSafeWarpOriginAnchor, } = require(path.join(__dirname, "../../space/npc/npcWarpOrigins"));
const { resolveNpcSpawnGroup, } = require(path.join(__dirname, "../../space/npc/npcData"));
const { getEffectTypeRecord, isModuleOnline, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const { resolveItemByTypeID, } = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const { getNpcFittedModuleItems, } = require(path.join(__dirname, "../../space/npc/npcEquipment"));
const { ensureSceneMiningState, getMineableState, isMineableStaticEntity, } = require("./miningRuntimeState");
const { resolveAggressorStandingProfile, } = require("./miningNpcStandings");
const { ENTITY_TYPE, } = require(path.join(__dirname, "../../space/entityConstants"));
const { buildChildEntityScopeMetadata, canEntitiesInteractLocally, resolveEntityInteractionScope, } = require(path.join(__dirname, "../../space/destiny/identity/interactionScope"));
...
function buildFleetReferencePosition(scene, fleetRecord) {
    ...
    const preferredTarget = scene && typeof scene.getEntityByID === "function"
        ? scene.getEntityByID(normalizePositiveInteger(fleetRecord && fleetRecord.targetShipID, 0))
        : null;
    return ((preferredTarget && preferredTarget.position) ||
        (fleetRecord &&
...
function buildFleetMineableClaimCounts(scene, currentFleetRecord) {
    ...
    for (const fleetRecord of fleets) {
        ...
        const claimedTarget = resolveActiveMineableTarget(scene, fleetRecord.activeAsteroidID, resolveFleetInteractionSource(scene, fleetRecord));
        if (!claimedTarget) {
            continue;
        }
        const claimedTargetID = toInt(claimedTarget.itemID, 0);
        claimCounts.set(claimedTargetID, toInt(claimCounts.get(claimedTargetID), 0) + 1);
    }
    return claimCounts;
}
function resolveFleetMineableStaticEntities(scene, fleetRecord = null, referenceEntity = null) {
    ...
    const getByBubbleID = typeof scene.getBubbleScopedStaticEntitiesForBubbleID === "function"
        ? scene.getBubbleScopedStaticEntitiesForBubbleID.bind(scene)
        : null;
    const getByPosition = typeof scene.getBubbleScopedStaticEntitiesForPosition === "function"
    ...
    const interactionSource = resolveFleetInteractionSource(scene, fleetRecord, referenceEntity);
    const activeTarget = resolveActiveMineableTarget(scene, fleetRecord && fleetRecord.activeAsteroidID, interactionSource);
    pushBubbleID(activeTarget && activeTarget.bubbleID);
    const targetShipEntity = scene && typeof scene.getEntityByID === "function"
    ...
    for (const minerEntityID of Array.isArray(fleetRecord && fleetRecord.minerEntityIDs) ? fleetRecord.minerEntityIDs : []) {
        const minerEntity = scene && typeof scene.getEntityByID === "function"
...
function getFleetMinerAssignmentMap(fleetRecord) {
    ...
    if (!fleetRecord.assignedAsteroidIDsByMinerID ||
...
function pruneFleetMinerAssignments(scene, fleetRecord) {
    ...
    const validMinerIDs = new Set((Array.isArray(fleetRecord && fleetRecord.minerEntityIDs) ? fleetRecord.minerEntityIDs : [])
...
function buildAssignedMineableClaimCounts(scene, currentFleetRecord, excludedMinerEntityID = 0) {
    ...
    for (const fleetRecord of getMiningFleetsForSystem(scene && scene.systemID)) {
        ...
        for (const [minerEntityID, assignedAsteroidID] of Object.entries(assignmentMap)) {
            ...
            const targetEntity = resolveActiveMineableTarget(scene, assignedAsteroidID, minerEntity);
            if (!targetEntity) {
                continue;
            }
...
function getMineableClaimPenaltyMeters() {
    return Math.max(0, toFiniteNumber(config.miningNpcFleetTargetClaimPenaltyMeters, 7_500));
}
...
function buildMineableTargetSelectionSnapshots(scene, minerEntity, candidates, referencePosition, claimCounts) {
    const preferredDistanceMeters = Math.max(0, getEntityTargetLockRangeMeters(scene, minerEntity));
    return candidates.map((entry) => {
        ...
        return {
            ...
            claimCount,
            distanceMeters,
            localPreferred: preferredDistanceMeters <= 0 ||
...
function compareMineableTargetSelectionSnapshots(left, right) {
    ...
    const quantityDelta = toInt(right && right.entry && right.entry.state && right.entry.state.remainingQuantity, 0) -
...
function isMiningSnapshotCompatibleWithEntry(snapshot, entry, hooks = {}) {
    ...
    if (typeof hooks.isMiningSnapshotCompatibleWithState === "function") {
        return hooks.isMiningSnapshotCompatibleWithState(snapshot, entry.state, entry.entity || null) === true;
    }
...
function chooseMineableTargetForMiner(scene, fleetRecord, minerEntity, availableTargetEntries, claimCounts, miningSnapshot = null, hooks = {}) {
    ...
    const candidates = Array.isArray(availableTargetEntries)
        ? (miningSnapshot
            ...
            : availableTargetEntries).filter((entry) => (entry &&
            entry.entity &&
    ...
    const selectionSnapshots = buildMineableTargetSelectionSnapshots(scene, minerEntity, candidates, referencePosition, claimCounts);
    const currentAssignedTarget = resolveActiveMineableTarget(scene, assignmentMap[toInt(minerEntity.itemID, 0)], minerEntity);
    ...
    const fallbackGlobalClaimed = selectionSnapshots
        .filter((snapshot) => snapshot.claimCount > 0)
        .sort(compareMineableTargetSelectionSnapshots)[0] || null;
    const bestSelection = preferredLocalUnclaimed ||
        preferredLocalClaimed ||
        fallbackGlobalUnclaimed ||
        fallbackGlobalClaimed ||
        null;
    if (currentSelection && bestSelection) {
        const keepCurrentAssignment = currentSelection.localPreferred === true ||
        ...
        if (keepCurrentAssignment) {
            if (claimCounts instanceof Map) {
                claimCounts.set(currentSelection.targetID, toInt(claimCounts.get(currentSelection.targetID), 0) + 1);
            }
...
function getMiningEngagementRangeMeters(scene, entity, primarySnapshot, rangeBufferMeters) {
    const miningRangeMeters = Math.max(0, toFiniteNumber(primarySnapshot && primarySnapshot.maxRangeMeters, 0) - Math.max(0, toFiniteNumber(rangeBufferMeters, 0)));
    const lockRangeMeters = Math.max(0, getEntityTargetLockRangeMeters(scene, entity) - Math.max(0, toFiniteNumber(rangeBufferMeters, 0)));
...
function chooseFleetMineableTarget(scene, fleetRecord, hooks = {}) {
    ...
    const currentTarget = resolveActiveMineableTarget(scene, fleetRecord && fleetRecord.activeAsteroidID, interactionSource);
    if (currentTarget) {
    ...
    for (const assignedAsteroidID of Object.values(assignmentMap)) {
        const assignedTarget = resolveActiveMineableTarget(scene, assignedAsteroidID, interactionSource);
        if (!assignedTarget) {
            continue;
        }
        const assignedTargetID = toInt(assignedTarget.itemID, 0);
        assignmentCounts.set(assignedTargetID, toInt(assignmentCounts.get(assignedTargetID), 0) + 1);
    }
    ...
    if (assignedPrimaryTargetID && assignedPrimaryTargetID[0] > 0) {
        const assignedPrimaryTarget = resolveActiveMineableTarget(scene, assignedPrimaryTargetID[0], interactionSource);
    ...
    candidates.sort((left, right) => {
        ...
        const distanceDelta = distance(left && left.entity && left.entity.position, referencePosition) -
            distance(right && right.entity && right.entity.position, referencePosition);
        if (Math.abs(distanceDelta) > 0.000001) {
            return distanceDelta;
        }
        const quantityDelta = toInt(right && right.state && right.state.remainingQuantity, 0) -
...
function clearStaleMineableTargetLocks(scene, entity, targetEntityID, hooks = {}) {
    ...
    const getTargets = typeof hooks.getTargetsForEntity === "function"
        ? hooks.getTargetsForEntity
        : (runtimeScene, runtimeEntity) => (runtimeScene && typeof runtimeScene.getTargetsForEntity === "function"
    ...
    for (const lockedTargetID of lockedTargetIDs) {
        ...
        if (normalizedLockedTargetID <= 0 ||
            normalizedLockedTargetID === normalizedTargetID) {
            continue;
        }
        const lockedTargetEntity = typeof scene.getEntityByID === "function"
            ? scene.getEntityByID(normalizedLockedTargetID)
            : null;
        if (!isMineableStaticEntity(lockedTargetEntity) &&
            !getMineableState(scene, normalizedLockedTargetID)) {
            continue;
        }
        if (typeof scene.removeLockedTarget === "function" &&
    ...
    if (typeof scene.getSortedPendingTargetLocks === "function" &&
        ...
        for (const pendingLock of scene.getSortedPendingTargetLocks(entity) || []) {
            ...
            const pendingTargetEntity = typeof scene.getEntityByID === "function"
                ? scene.getEntityByID(pendingTargetID)
                : null;
            if (!isMineableStaticEntity(pendingTargetEntity) &&
...
function buildOffgridOriginAnchor(scene, target) {
    return findSafeWarpOriginAnchor(scene, target, {
        clearanceMeters: Math.max(ONE_AU_IN_METERS, toFiniteNumber(config.miningNpcWarpOriginClearanceMeters, ONE_AU_IN_METERS)),
        minDistanceMeters: toFiniteNumber(config.miningNpcWarpOriginMinDistanceMeters, ONE_AU_IN_METERS * 2),
        maxDistanceMeters: toFiniteNumber(config.miningNpcWarpOriginMaxDistanceMeters, ONE_AU_IN_METERS * 4),
        stepMeters: toFiniteNumber(config.miningNpcWarpOriginStepMeters, ONE_AU_IN_METERS / 2),
    });
}
...
function getSecurityBandForSystemID(systemID) {
    const systemRecord = worldData.getSolarSystemByID(normalizePositiveInteger(systemID, 0));
...
function getSecurityBandForScene(scene, fallbackSystemID = 0) {
    const systemID = normalizePositiveInteger(scene && scene.systemID, normalizePositiveInteger(fallbackSystemID, 0));
...
function resolveConfiguredBandQuery(scene, explicitQuery, options = {}) {
    ...
    const configValue = String(options.configValue ||
        "").trim();
    if (configValue) {
        return configValue;
    }
    ...
    return String(bandDefaultValues[securityBand] ||
...
function applyPassiveMiningFleetOverrides(entityID, options = {}) {
    npcService.setBehaviorOverrides(entityID, {
        ...
        orbitDistanceMeters: Math.max(100, toFiniteNumber(options.orbitDistanceMeters, 1_200)),
        followRangeMeters: Math.max(100, toFiniteNumber(options.followRangeMeters, 800)),
        idleAnchorOrbit: options.idleAnchorOrbit === true,
        idleAnchorOrbitDistanceMeters: Math.max(100, toFiniteNumber(options.idleAnchorOrbitDistanceMeters, 1_200)),
...
function getNormalizedManualOrderSignature(order) {
    ...
    return {
        ...
        allowWeapons: order.allowWeapons === undefined || order.allowWeapons === null
            ? null
            : order.allowWeapons === true,
        keepLock: order.keepLock === undefined || order.keepLock === null
            ? null
            : order.keepLock === true,
    };
}
function areManualOrdersEquivalent(leftOrder, rightOrder) {
    ...
    return (leftSignature.type === rightSignature.type &&
...
function buildMiningMovementOrder(targetEntityID, movementMode, distanceMeters) {
    ...
    return {
        ...
        orbitDistanceMeters: normalizedMovementMode === "follow"
            ? 0
            : Math.max(0, toFiniteNumber(distanceMeters, 0)),
        followRangeMeters: normalizedMovementMode === "follow"
            ? Math.max(0, toFiniteNumber(distanceMeters, 0))
            : 0,
        allowWeapons: false,
        keepLock: true,
...
function syncMiningApproachOrder(scene, entity, targetEntity, orbitDistanceMeters) {
    ...
    const sameTarget = toInt(entity.targetEntityID, 0) === toInt(targetEntity.itemID, 0);
    const followRangeMatchesOrbit = Math.abs(toFiniteNumber(entity.followRange, 0) - normalizedOrbitDistance) <= 1;
    const currentlyFollowingOrbitBand = entity.mode === "FOLLOW" &&
        sameTarget &&
        followRangeMatchesOrbit;
    const orbitReacquireDistanceMeters = normalizedOrbitDistance + Math.max(5_000, normalizedOrbitDistance * 0.5);
    const orbitSettleDistanceMeters = normalizedOrbitDistance + Math.max(1_000, normalizedOrbitDistance * 0.2);
    const desiredMovementMode = surfaceDistanceMeters > orbitReacquireDistanceMeters ||
        (currentlyFollowingOrbitBand &&
            surfaceDistanceMeters > orbitSettleDistanceMeters)
        ? "follow"
        : "orbit";
    if (desiredMovementMode === "follow") {
        return scene.followShipEntity(entity, targetEntity.itemID, normalizedOrbitDistance, {
            queueHistorySafeContract: true,
            suppressFreshAcquireReplay: true,
        });
    }
    return scene.orbitShipEntity(entity, targetEntity.itemID, normalizedOrbitDistance, {
        queueHistorySafeContract: true,
        suppressFreshAcquireReplay: true,
    });
}
...
function getNpcOreCargoSummary(entity) {
    ...
    return {
        ...
        quantity: entries.reduce((sum, entry) => sum + Math.max(0, toInt(entry && entry.quantity, 0)), 0),
    };
}
function getNpcCargoCapacityM3(entity, fallbackCapacityM3) {
    ...
    const explicitHullCapacity = Math.max(0, toFiniteNumber(MINING_NPC_CARGO_CAPACITY_M3_BY_TYPE_ID[toInt(entity && entity.typeID, 0)], 0));
...
function getFleetCargoState(scene, fleetRecord) {
    ...
    const haulerEntities = (Array.isArray(fleetRecord && fleetRecord.haulerEntityIDs) ? fleetRecord.haulerEntityIDs : [])
        .map((entityID) => scene.getEntityByID(entityID))
        .filter(Boolean);
    const minerCapacityPerHull = Math.max(0, toFiniteNumber(config.miningNpcMinerCargoCapacityM3, DEFAULT_MINING_MINER_CARGO_CAPACITY_M3));
    const haulerCapacityPerHull = Math.max(0, toFiniteNumber(config.miningNpcHaulerCargoCapacityM3, DEFAULT_MINING_HAULER_CARGO_CAPACITY_M3));
    const minerUsedVolumeM3 = minerEntities.reduce((sum, entity) => sum + getNpcOreCargoVolume(entity), 0);
    const minerCapacityM3 = minerEntities.reduce((sum, entity) => sum + getNpcCargoCapacityM3(entity, minerCapacityPerHull), 0);
    const haulerCapacityM3 = haulerEntities.reduce((sum, entity) => sum + getNpcCargoCapacityM3(entity, haulerCapacityPerHull), 0);
    return {
        minerUsedVolumeM3,
        minerCapacityM3,
        haulerCapacityM3,
        minerFillRatio: minerCapacityM3 > 0
...
function getAggressionMemoryMs() {
    return Math.max(0, toInt(config.miningNpcAggressionMemoryMs, DEFAULT_MINING_AGGRESSION_MEMORY_MS));
}
function getResponseCooldownMs() {
    return Math.max(0, toInt(config.miningNpcResponseCooldownMs, DEFAULT_MINING_RESPONSE_COOLDOWN_MS));
}
function getResponseRetreatDelayMs() {
    return Math.max(0, toInt(config.miningNpcResponseRetreatDelayMs, DEFAULT_MINING_RESPONSE_RETREAT_DELAY_MS));
}
function getHaulerInitialDelayMs() {
    return Math.max(0, toInt(config.miningNpcHaulerInitialDelayMs, DEFAULT_MINING_HAULER_INITIAL_DELAY_MS));
}
function getHaulerRepeatDelayMs() {
    return Math.max(0, toInt(config.miningNpcHaulerRepeatDelayMs, DEFAULT_MINING_HAULER_REPEAT_DELAY_MS));
}
...
function getStandingResponseConfig(standingClass) {
    switch (String(standingClass || "neutral").trim().toLowerCase()) {
        ...
        case "hostile":
            return {
                profileQuery: String(config.miningNpcHostileResponseProfileOrPool ||
                    config.miningNpcResponseProfileOrPool ||
                    DEFAULT_MINING_RESPONSE_QUERY).trim(),
                amount: Math.max(0, toInt(config.miningNpcHostileResponseCount, config.miningNpcResponseDefaultCount || DEFAULT_MINING_RESPONSE_COUNT)),
            };
        default:
            return {
                profileQuery: String(config.miningNpcNeutralResponseProfileOrPool ||
                    config.miningNpcResponseProfileOrPool ||
                    DEFAULT_MINING_RESPONSE_QUERY).trim(),
                amount: Math.max(0, toInt(config.miningNpcNeutralResponseCount, config.miningNpcResponseDefaultCount || DEFAULT_MINING_RESPONSE_COUNT)),
            };
    }
}
function resolveResponsePlan(scene, fleetRecord, aggressorEntity, options = {}) {
    ...
    const standingProfile = config.miningNpcStandingsEnabled === true && representativeNpcEntity && aggressorEntity
    ...
    const resolvedAmount = Math.max(0, amountOverride !== null ? amountOverride : configuredResponse.amount);
...
function getLatestFleetAggression(scene, fleetRecord, now = Date.now()) {
    ...
    for (const entityID of getFleetEntityIDs(fleetRecord)) {
        ...
        const lastAggressedAtMs = Math.max(0, toInt(controller.lastAggressedAtMs, 0));
...
function appendNpcMiningCargo(entity, typeID, quantity) {
    ...
    if (entity.transient !== true) {
        ...
        const ownerID = toInt(entity.ownerID ?? entity.pilotCharacterID ?? entity.characterID, 0);
        ...
        const sameTypeRecords = nativeNpcStore.listNativeCargoForEntity(entityID)
            .filter((record) => toInt(record && record.moduleID, 0) <= 0 &&
            toInt(record && record.typeID, 0) === numericTypeID);
        const legacyRecords = sameTypeRecords.filter((record) => !itemStore.findItemById(toInt(record && record.cargoID, 0)));
        const legacyQuantity = legacyRecords.reduce((sum, record) => sum + Math.max(0, toInt(record && record.quantity, 0)), 0);
        const grant = itemStore.grantItemToOwnerLocation(ownerID, entityID, itemStore.ITEM_FLAGS.CARGO_HOLD, typeRecord, numericQuantity + legacyQuantity, { singleton: 0 });
        ...
        if (!flush.success)
            return { success: false, errorMsg: "NPC_RESOURCE_CARGO_FLUSH_FAILED" };
    ...
    const existingEntry = entity.nativeCargoItems.find((entry) => (toInt(entry && entry.typeID, 0) === numericTypeID &&
        toInt(entry && entry.moduleID, 0) <= 0)) || null;
    if (existingEntry) {
...
function createMiningFleetRecord(options = {}) {
    ...
    const fleetRecord = {
        ...
        assignedAsteroidIDsByMinerID: options.assignedAsteroidIDsByMinerID &&
        ...
        haulCompleteAtMs: 0,
        haulerReturnAtMs: 0,
        haulerNextArrivalAtMs: Math.max(0, toInt(options.haulerNextArrivalAtMs, haulerEntityIDs.length > 0
...
function getConfiguredNpcActivity(controller) {
    const configuredActivity = String(controller && (controller.behaviorActivity ||
...
function isConfiguredNpcActivityAutoEnrollEnabled(controller) {
    ...
    if (behaviorProfile.mineAsteroids === false &&
        String(controller && (controller.behaviorActivity ||
            (controller.behaviorPolicy && controller.behaviorPolicy.activity)) || "").trim().toLowerCase() === "mining") {
        return false;
    }
...
function pruneMiningFleet(fleetRecord) {
    ...
    if (fleetRecord.minerEntityIDs.length === 0 &&
...
function spawnFleetWing(systemID, centerTarget, options = {}) {
    ...
    if (options.warpIn !== false && centerTarget && centerTarget.position) {
        const landingRadiusMeters = Math.max(500, toFiniteNumber(options.landingRadiusMeters, DEFAULT_MINING_WARP_LANDING_RADIUS_METERS));
        const warpRequests = spawnResult.data.spawned.map((entry, index, list) => ({
            ...
            point: buildMiningWarpLandingPoint(centerTarget.position, index, list.length, landingRadiusMeters),
            options: {
                forceImmediateStart: true,
                broadcastWarpStartToVisibleSessions: true,
                visibilitySuppressMs: 250,
                ingressDurationMs: Math.max(250, toFiniteNumber(config.miningNpcWarpIngressDurationMs, DEFAULT_MINING_WARP_INGRESS_DURATION_MS)),
            },
        }));
...
function spawnMiningFleetInternal(scene, options = {}) {
    ...
    const resolvedMinerQuery = resolveMiningFleetQuery(scene, options.minerQuery, systemID);
    const minerSpawnResult = spawnFleetWing(systemID, centerTarget, {
        amount: Math.max(1, toInt(options.minerAmount, toInt(config.miningNpcFleetDefaultCount, DEFAULT_MINING_FLEET_COUNT))),
        profileQuery: resolvedMinerQuery,
        entityScopeMetadata,
        preferredTargetID: 0,
        originAnchor,
        warpIn: true,
        landingRadiusMeters: toFiniteNumber(config.miningNpcFleetLandingRadiusMeters, DEFAULT_MINING_WARP_LANDING_RADIUS_METERS),
    });
    if (!minerSpawnResult.success || !minerSpawnResult.data) {
        return minerSpawnResult;
    }
    const haulerAmount = Math.max(0, toInt(options.haulerAmount, toInt(config.miningNpcHaulerDefaultCount, DEFAULT_MINING_HAULER_COUNT)));
    let haulerSpawnResult = null;
    if (haulerAmount > 0) {
        const resolvedHaulerQuery = resolveMiningHaulerQuery(scene, options.haulerQuery, systemID);
...
function retreatResponseWingToOrigin(fleetRecord, options = {}) {
    ...
    const ingressDurationMs = Math.max(250, toFiniteNumber(config.miningNpcWarpIngressDurationMs, DEFAULT_MINING_WARP_INGRESS_DURATION_MS));
    ...
    if (retreatedCount > 0) {
        ...
        fleetRecord.responseTargetID = 0;
        fleetRecord.responseDespawnAtMs = Math.max(0, toInt(options.nowMs, 0) + ingressDurationMs);
    }
...
function spawnResponseWingForFleet(scene, fleetRecord, aggressorEntity, responsePlan, options = {}) {
    ...
    issueResponseOrders(spawnedEntityIDs, normalizePositiveInteger(aggressorEntity.itemID, 0));
...
function triggerFleetAggression(scene, fleetRecord, options = {}) {
    ...
    const aggressorEntity = options.aggressorEntity ||
    ...
    const aggressionEventAtMs = Math.max(0, toInt(options.aggressionEventAtMs, now));
    ...
    const autoResumeDelayMs = Math.max(0, toInt(config.miningNpcFleetAutoResumeDelayMs, 0));
    ...
    const hasDeployableResponse = responsePlan.amount > 0 &&
        String(responsePlan.profileQuery || "").trim().length > 0;
    const canSpawnResponse = hasDeployableResponse &&
        (fleetRecord.lastResponseAtMs <= 0 ||
            cooldownMs <= 0 ||
            now - fleetRecord.lastResponseAtMs >= cooldownMs ||
            fleetRecord.responseEntityIDs.length <= 0);
    if (canSpawnResponse) {
        const spawnResult = spawnResponseWingForFleet(scene, fleetRecord, aggressorEntity, responsePlan, { nowMs: now });
        ...
        issueResponseOrders(fleetRecord.responseEntityIDs, normalizePositiveInteger(aggressorEntity.itemID, 0));
        fleetRecord.responseDespawnAtMs = now + getResponseRetreatDelayMs();
    }
    return {
        success: true,
        data: {
            standingClass: responsePlan.standingProfile && responsePlan.standingProfile.standingClass
                ? responsePlan.standingProfile.standingClass
                : null,
            standingValue: responsePlan.standingProfile && Number.isFinite(responsePlan.standingProfile.standing)
                ? responsePlan.standingProfile.standing
                : 0,
            retreatedCount,
...
function handleMiningFleetCommand(session, argumentText) {
    ...
    const parsedArguments = parseNpcSpawnArguments(argumentText, Math.max(1, toInt(config.miningNpcFleetDefaultCount, DEFAULT_MINING_FLEET_COUNT)));
    ...
    const fleetRecord = createMiningFleetRecord({
        ...
        spawnSelectionName: spawnResult.data.minerSpawnResult &&
            spawnResult.data.minerSpawnResult.data &&
            spawnResult.data.minerSpawnResult.data.selectionName,
        haulerSelectionName: spawnResult.data.haulerSpawnResult &&
...
function retreatFleetToOrigin(fleetRecord, options = {}) {
    ...
    if (options.scene && options.hooks) {
        deactivateMiningModulesForFleet(options.scene, fleetRecord, options.hooks, options.reason || "state");
    }
    const ingressDurationMs = Math.max(250, toFiniteNumber(config.miningNpcWarpIngressDurationMs, DEFAULT_MINING_WARP_INGRESS_DURATION_MS));
    const entityIDs = [
        ...(Array.isArray(fleetRecord.minerEntityIDs) ? fleetRecord.minerEntityIDs : []),
...
function handleMiningFleetAggroCommand(session, argumentText, options = {}) {
    ...
    const parsedArguments = parseNpcSpawnArguments(argumentText, Math.max(1, toInt(config.miningNpcResponseDefaultCount, DEFAULT_MINING_RESPONSE_COUNT)));
    if (!parsedArguments.success) {
        return {
            success: false,
    ...
    const scene = resolveSessionScene(session);
    const aggressorEntity = scene && scene.getEntityByID(normalizePositiveInteger(session._space.shipID, 0));
    ...
    for (const fleetRecord of fleets) {
        const aggressionResult = triggerFleetAggression(scene, fleetRecord, {
            ...
            responseQuery: parsedArguments.query ||
        ...
        responseSpawnedCount += Math.max(0, toInt(aggressionResult.data && aggressionResult.data.responseSpawnedCount, 0));
...
function handleMiningFleetClearCommand(session) {
    const systemID = normalizePositiveInteger(session &&
        session._space &&
        session._space.systemID, 0);
    if (!systemID) {
        return {
            success: false,
...
function formatFleetSummary(scene, fleetRecord) {
    ...
    const nextHaulerMs = Math.max(0, toInt(fleetRecord.haulerNextArrivalAtMs, 0) - Date.now());
    return [
...
function deactivateMiningModulesForEntity(scene, entity, hooks, reason = "state") {
    ...
    for (const moduleItem of getNpcFittedModuleItems(entity)) {
        const effectRecord = typeof hooks.findMiningEffectRecordForModule === "function"
...
function startHaulCycle(scene, fleetRecord, now, hooks) {
    ...
    const gatherTarget = scene.getEntityByID(toInt(fleetRecord.activeAsteroidID, 0)) ||
        scene.getEntityByID(toInt(fleetRecord.targetShipID, 0)) ||
        null;
    const gatherPoint = (gatherTarget && gatherTarget.position) ||
    ...
    for (const [index, entityID] of fleetRecord.haulerEntityIDs.entries()) {
        npcService.runtime.warpToPoint(entityID, buildMiningWarpLandingPoint(gatherPoint, index, fleetRecord.haulerEntityIDs.length, Math.max(250, toFiniteNumber(config.miningNpcHaulerLandingRadiusMeters, 750))), {
            forceImmediateStart: true,
            broadcastWarpStartToVisibleSessions: true,
            visibilitySuppressMs: 250,
            ingressDurationMs,
    ...
    fleetRecord.haulCompleteAtMs =
        now +
            ingressDurationMs +
            Math.max(500, toFiniteNumber(config.miningNpcHaulerUnloadDurationMs, DEFAULT_MINING_HAULER_UNLOAD_DURATION_MS));
...
function shouldTriggerHauling(scene, fleetRecord, noAsteroidsRemain = false, now = Date.now()) {
    ...
    if (toFiniteNumber(fleetRecord.haulerNextArrivalAtMs, 0) > 0 &&
        now < toFiniteNumber(fleetRecord.haulerNextArrivalAtMs, 0)) {
        return false;
    }
    const cargoState = getFleetCargoState(scene, fleetRecord);
    const thresholdRatio = Math.max(0.01, Math.min(1, toFiniteNumber(config.miningNpcHaulThresholdRatio, DEFAULT_MINING_HAUL_THRESHOLD_RATIO)));
...
function tickMiningFleet(scene, fleetRecord, now, hooks) {
    ...
    const thinkIntervalMs = Math.max(250, toFiniteNumber(config.miningNpcFleetThinkIntervalMs, 1_000));
    ...
    if (fleetRecord.responseEntityIDs.length > 0) {
        ...
        if (fleetRecord.responseDespawnAtMs > 0 &&
    ...
    if ((fleetRecord.state === "aggressed" || fleetRecord.state === "panic") &&
        fleetRecord.resumeAtMs > 0 &&
    ...
    const rangeBufferMeters = Math.max(0, toFiniteNumber(config.miningNpcFleetMiningRangeBufferMeters, 500));
    const orbitDistanceMeters = Math.max(500, toFiniteNumber(config.miningNpcFleetOrbitDistanceMeters, 1_000));
    for (const minerEntityID of Array.isArray(fleetRecord.minerEntityIDs) ? fleetRecord.minerEntityIDs : []) {
        ...
        const miningModules = getNpcFittedModuleItems(minerEntity)
            ...
            .map((moduleItem) => ({
            moduleItem,
            effectRecord: typeof hooks.findMiningEffectRecordForModule === "function"
                ? hooks.findMiningEffectRecordForModule(moduleItem)
                : getEffectTypeRecord(toInt(moduleItem && moduleItem.effectID, 0)),
        }))
        ...
        const primarySnapshot = hooks.buildEntityMiningSnapshot(minerEntity, miningModules[0].moduleItem, miningModules[0].effectRecord);
        if (!primarySnapshot) {
            continue;
        }
        const targetEntity = chooseMineableTargetForMiner(scene, fleetRecord, minerEntity, availableTargetEntries, claimCounts, primarySnapshot, hooks);
        if (!targetEntity) {
            delete assignmentMap[toInt(minerEntity.itemID, 0)];
            continue;
        }
        const targetEntityID = toInt(targetEntity.itemID, 0);
        targetAssignmentCounts.set(targetEntityID, toInt(targetAssignmentCounts.get(targetEntityID), 0) + 1);
        const distanceToTarget = hooks.getSurfaceDistance(minerEntity, targetEntity);
        const engagementRangeMeters = getMiningEngagementRangeMeters(scene, minerEntity, primarySnapshot, rangeBufferMeters);
        if (distanceToTarget > engagementRangeMeters) {
            syncMiningApproachOrder(scene, minerEntity, targetEntity, orbitDistanceMeters);
            continue;
        }
        const targetLockResult = syncMiningTargetLock(scene, minerEntity, targetEntity, now, hooks);
...
//# sourceMappingURL=miningNpcOperations.js.map