...
function computeMiningResult(options = {}) {
    ...
    const wasteVolumeMultiplier = Math.max(0, toFiniteNumber(options.wasteVolumeMultiplier, 0));
    const wasteProbability = clamp(options.wasteProbability, 0, 100);
    const critQuantityMultiplier = Math.max(0, toFiniteNumber(options.critQuantityMultiplier, 0));
    const critProbability = clamp(options.critProbability, 0, 1);
    const efficiency = clamp(options.efficiency === undefined ? 1 : options.efficiency, 0, 1);
    ...
    const normalQuantity = Math.min(asteroidQuantity, Math.max(0, randomizedRound((normalVolume / unitVolume) * efficiency, rngFloat)));
    const remainingAfterNormal = Math.max(0, asteroidQuantity - normalQuantity);
    const wastedQuantity = Math.max(0, Math.min(remainingAfterNormal, randomizedRound(wastedVolume / unitVolume, rngFloat)));
    const criticalHitQuantity = normalQuantity > 0 && critProbability > clamp(rngFloat(), 0, 1)
        ? Math.max(0, randomizedRound(normalQuantity * critQuantityMultiplier, rngFloat))
        : 0;
    return {
        clampFactor,
        normalVolume,
...
function getAsteroidRadiusAttributes(typeID) {
    return {
        unitSize: Math.max(0, toFiniteNumber(getTypeAttributeValue(typeID, "asteroidRadiusUnitSize"), 0)),
        growthFactor: Math.max(0, toFiniteNumber(getTypeAttributeValue(typeID, "asteroidRadiusGrowthFactor"), 0)),
        maxRadius: Math.max(0, toFiniteNumber(getTypeAttributeValue(typeID, "asteroidMaxRadius"), 0)),
    };
}
function computeAsteroidRadiusFromQuantity(typeID, quantity, options = {}) {
    ...
    const fallbackUnitVolume = Math.max(0.000001, toFiniteNumber(options.unitVolume, 1));
    const fallbackScale = Math.max(0.000001, toFiniteNumber(options.fallbackScale, ASTEROID_EXP_SCALE));
    const fallbackMinRadius = Math.max(1, toFiniteNumber(options.fallbackMinRadius, 250));
    const fallbackMaxRadius = Math.max(fallbackMinRadius, toFiniteNumber(options.fallbackMaxRadius, 25_000));
    const attributes = getAsteroidRadiusAttributes(typeID);
    if (attributes.unitSize > 0 &&
        attributes.growthFactor > 0 &&
        attributes.maxRadius > 0) {
        const rawRadius = attributes.unitSize +
...
function computeAsteroidQuantityFromRadius(typeID, radius, options = {}) {
    ...
    if (attributes.unitSize > 0 &&
        ...
        return Math.max(0, Math.round(Math.expm1(normalizedLog) / ASTEROID_EXP_SCALE));
    }
...
//# sourceMappingURL=miningMath.js.map