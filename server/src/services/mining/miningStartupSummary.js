...
function formatMiningStartupMessage(systemID, payload) {
    ...
    if (payload.beltCount > 0 || payload.asteroidCount > 0) {
        siteParts.push(`belts ${formatInteger(payload.beltCount)}`, `asteroids ${formatInteger(payload.asteroidCount)}`, `ore types ${formatInteger(payload.orePoolTypeCount)}/${formatInteger(payload.beltSubsetTypeCount)}`);
    }
    if (payload.iceSiteCount > 0 || payload.iceChunkCount > 0) {
        siteParts.push(`ice sites ${formatInteger(payload.iceSiteCount)} (${formatInteger(payload.iceChunkCount)} chunks)`);
    }
    if (payload.gasSiteCount > 0 || payload.gasCloudCount > 0) {
        siteParts.push(`gas sites ${formatInteger(payload.gasSiteCount)} (${formatInteger(payload.gasCloudCount)} clouds)`);
    }
    ...
    if (payload.oreCount > 0 || payload.oreRemainingQuantity > 0) {
        presentationParts.push(`ore ${formatInteger(payload.oreCount)} (${formatInteger(payload.oreRemainingQuantity)})`);
    }
    if (payload.iceCount > 0 || payload.iceRemainingQuantity > 0) {
        presentationParts.push(`ice ${formatInteger(payload.iceCount)} (${formatInteger(payload.iceRemainingQuantity)})`);
    }
    if (payload.gasCount > 0 || payload.gasRemainingQuantity > 0) {
        presentationParts.push(`gas ${formatInteger(payload.gasCount)} (${formatInteger(payload.gasRemainingQuantity)})`);
    }
    if (payload.otherCount > 0 || payload.otherRemainingQuantity > 0) {
        presentationParts.push(`other ${formatInteger(payload.otherCount)} (${formatInteger(payload.otherRemainingQuantity)})`);
    }
...
function flushMiningStartupSummary(scene) {
    ...
    const hasData = payload.beltCount > 0 ||
...
//# sourceMappingURL=miningStartupSummary.js.map