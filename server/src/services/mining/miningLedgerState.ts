...
function normalizeLedgerEntry(entry, overrides: Record<string, any> = {}) {
  const normalized = {
...
function normalizeObserverBucket(observerID, bucket: Record<string, any> = {}, fallbackCorporationID = 0) {
  return {
...
function filterVisibleEntries(entries: any[] = [], visibilityField, nowFiletime = currentFileTime()) {
...
function maybePruneExpiredEntries(nowFiletime = currentFileTime(), options: Record<string, any> = {}) {
  ...
  for (const [characterID, bucket] of Object.entries<any>(table.characters || {})) {
  ...
  for (const [observerID, bucket] of Object.entries<any>(table.observers || {})) {
    const entries = Array.isArray(bucket && bucket.entries) ? bucket.entries : [];
    const retainedEntries = entries.filter(
      (entry) => normalizeBigInt(entry && entry.eventDate, 0n) >= cutoffFiletime,
    );
...
function recordMiningLedgerEvent(payload: Record<string, any> = {}) {
...
function getCharacterMiningLogs(characterID, options: Record<string, any> = {}) {
...
function getObserverMiningLedger(observerItemID, corporationID = 0, options: Record<string, any> = {}) {
...
function listObserverHeadersForCorporation(corporationID, options: Record<string, any> = {}) {
  const numericCorporationID = normalizePositiveInteger(corporationID, 0);
  ...
  const table = ensureMiningLedgerTable();
  const headers = Object.entries<any>(table.observers || {})
