...
const { ITEM_FLAGS, buildInventoryItem, findItemById, getActiveShipItem, getItemMetadata, grantItemsToCharacterLocation, removeInventoryItem, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const { structureHasOnlineService, } = require(path.join(__dirname, "../structure/structureServiceAuthority"));
const { getAdjustedAveragePrice, getCompressedTypeID, getCompressionSourceTypeIDs, getTypeMaterials, hasTypeMaterials, isCompressedType, isCompressibleType, } = require("./miningStaticData");
...
function itemIsInPersonalStructureHangar(item, context) {
    return (toInt(item && item.ownerID, 0) === toInt(context && context.characterID, 0) &&
...
function itemIsInActiveShipAtStructure(item, context) {
    ...
    return (toInt(item && item.ownerID, 0) === characterID &&
...
function itemIsInDockableCorpOfficeWithTakeAccess(item, context, session) {
    ...
    const office = getCorporationOfficeByInventoryID(corporationID, toInt(item && item.locationID, 0));
...
function itemIsInStructureCompressionSource(item, context, session) {
    return (itemIsInPersonalStructureHangar(item, context) ||
...
function getSkillBonusMultiplier(skillMap, skillTypeID) {
    ...
    const percentagePerLevel = toFiniteNumber(getTypeAttributeValue(numericSkillTypeID, "refiningYieldMutator"), 0);
...
function getReprocessingSkillMultiplierForType(typeID, skillMap, implants = []) {
    if (typeHasSpecialReprocessingSkillBonuses(typeID)) {
        const specificSkillTypeID = toInt(getTypeAttributeValue(typeID, "reprocessingSkillType"), 0);
        ...
        for (const implant of Array.isArray(implants) ? implants : []) {
            ...
            const implantBonus = toFiniteNumber(getTypeAttributeValue(implantTypeID, "refiningYieldMutator"), 0);
            if (implantBonus > 0) {
...
function getGasDecompressionCharacterEfficiency(skillMap) {
    ...
    const bonusPerLevelPercent = toFiniteNumber(getTypeAttributeValue(TYPE_GAS_DECOMPRESSION_EFFICIENCY, "gasDecompressionEfficiencyBonus"), 0);
...
function isRefinableType(itemType) {
    ...
    return (toInt(itemType.categoryID, 0) === CATEGORY_ASTEROID ||
...
function getStationEfficiencyForTypeID(context, typeID) {
    ...
    if (context.dockedKind === "structure") {
        return Math.max(0, Math.min(1, toFiniteNumber(config.miningStructureReprocessingEfficiency, DEFAULT_STRUCTURE_REPROCESSING_EFFICIENCY)));
    }
    return Math.max(0, Math.min(1, toFiniteNumber(context.stationRecord && context.stationRecord.reprocessingEfficiency, DEFAULT_STATION_REPROCESSING_EFFICIENCY)));
}
function getStationTaxRate(context) {
    return Math.max(0, Math.min(1, toFiniteNumber(context &&
        context.stationRecord &&
        context.stationRecord.reprocessingStationsTake, DEFAULT_STATION_REPROCESSING_TAX)));
}
function getReprocessingYieldForType(context, typeID) {
    const stationEfficiency = getStationEfficiencyForTypeID(context, typeID);
    const characterMultiplier = getReprocessingSkillMultiplierForType(typeID, context && context.skillMap, context && context.implants);
...
function buildReprocessingQuoteForItem(item, context) {
    ...
    const recoverableResult = buildRecoverablesForItem(item, stationsTake, efficiency, portions);
    return {
...
function reprocessItems(session, options = {}) {
    ...
    const grantResult = grantEntries.length > 0
        ? grantItemsToCharacterLocation(contextResult.data.characterID, outputLocationID || contextResult.data.dockedLocationID, outputFlagID === null ? ITEM_FLAGS.HANGAR : outputFlagID, grantEntries)
        : {
            success: true,
            data: {
                changes: [],
...
function compressInventoryItem(itemID, options = {}) {
    ...
    const updateResult = updateInventoryItem(item.itemID, (currentItem) => (rebuildItemAsType(currentItem, compressedTypeID, sourceQuantity)));
...
function getStructureGasDecompressionEfficiency(context) {
    ...
    return Math.max(0, Math.min(1, toFiniteNumber(config.miningStructureGasDecompressionEfficiency, DEFAULT_STRUCTURE_GAS_DECOMPRESSION_EFFICIENCY)));
}
function decompressGasInStructure(session, itemID) {
    ...
    const updateResult = updateInventoryItem(item.itemID, (currentItem) => (rebuildItemAsType(currentItem, sourceType.typeID, outputQuantity)));
    if (!updateResult.success) {
        return updateResult;
    }
    return {
...
function resolveInSpaceCompressionContext(session, facilityBallID) {
    ...
    const facilityTypelists = typeof spaceRuntime.resolveCompressionFacilityTypelistsForEntity === "function"
    ...
    const facilityCharacterID = toInt(facilityEntity && (facilityEntity.characterID ??
        facilityEntity.pilotCharacterID), 0);
    if (toInt(facilityEntity.itemID, 0) !== shipID &&
        (characterID <= 0 ||
    ...
    const surfaceDistance = Math.max(0, centerDistance -
        Math.max(0, toFiniteNumber(shipEntity.radius, 0)) -
        Math.max(0, toFiniteNumber(facilityEntity.radius, 0)));
    const maxRangeMeters = Math.max(1, ...facilityTypelists.map((entry) => Math.max(0, toFiniteNumber(entry && entry[1], 0))));
...
//# sourceMappingURL=miningIndustry.js.map