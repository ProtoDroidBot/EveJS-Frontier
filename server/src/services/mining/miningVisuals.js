...
function extractSystemOreEntries(oreMap, systemID) {
    ...
    if (Array.isArray(oreMap)) {
        const matched = oreMap.filter((row) => {
            const rowSystemID = toPositiveInt(row && (row.solarSystemID ?? row.systemID), 0);
...
function normalizeSystemOreEntry(entry) {
    ...
    const typeID = toPositiveInt(entry.typeID ?? entry.oreTypeID ?? entry.visualTypeID ?? entry.shellTypeID, 0);
    if (typeID <= 0) {
        return null;
    }
...
function resolveMiningVisualPresentation(typeRecord, overrides = {}) {
    ...
    const visualTypeID = toPositiveInt(overrides.visualTypeID ?? overrides.typeID ?? (resolved && resolved.typeID), 0);
    const graphicID = toPositiveInt(overrides.graphicID ?? (resolved && resolved.graphicID), 0);
