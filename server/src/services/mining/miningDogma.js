...
const { getAttributeIDByNames, buildEffectiveItemAttributeMap, getTypeAttributeValue, applyOtherItemModifiersToAttributes, applyModifierGroups, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const { buildSkillEffectiveAttributes, collectShipModifierAttributes, buildLocationModifiedAttributeMap, } = require(path.join(__dirname, "../../space/combat/weaponDogma"));
const { buildNpcEffectiveModuleItem, } = require(path.join(__dirname, "../../space/npc/npcCapabilityResolver"));
...
const ATTRIBUTE_MINING_WASTE_PROBABILITY = getAttributeIDByNames("miningWasteProbability") || 2864;
const ATTRIBUTE_MINING_CRIT_CHANCE = getAttributeIDByNames("miningCritChance") || 2868;
const ATTRIBUTE_MINING_CRIT_BONUS = getAttributeIDByNames("miningCritBonusYield") || 2869;
const ATTRIBUTE_CRYSTAL_VOLATILITY_CHANCE = getAttributeIDByNames("crystalVolatilityChance") || 783;
const ATTRIBUTE_CRYSTAL_VOLATILITY_DAMAGE = getAttributeIDByNames("crystalVolatilityDamage") || 784;
const ATTRIBUTE_SPECIALIZATION_TYPE_LIST = getAttributeIDByNames("specializationAsteroidTypeList") || 3148;
const ATTRIBUTE_SPECIALIZATION_YIELD_MULTIPLIER = getAttributeIDByNames("specializationAsteroidYieldMultiplier") || 782;
...
function isMiningEffectRecord(effectRecord, moduleItem = null) {
    ...
    return toFiniteNumber(moduleAttributes && moduleAttributes[ATTRIBUTE_MINING_AMOUNT], 0) > 0;
}
function resolveMiningFamily(moduleItem, effectRecord = null) {
    ...
    const name = String((effectiveModuleItem && effectiveModuleItem.itemName) ||
...
function buildMiningModuleSnapshot({ characterID = 0, shipItem, moduleItem, effectRecord, chargeItem = null, fittedItems = null, skillMap = null, activeModuleContexts = null, additionalModifierEntries = null, additionalLocationModifierSources = null, } = {}) {
    ...
    const resolvedAdditionalLocationModifierSources = Array.isArray(additionalLocationModifierSources)
        ...
        : [];
    const implantLocationModifierSources = toInt(characterID, 0) > 0
        ? getActiveImplantLocationModifierSources(characterID)
        : [];
    const implantShipModifierEntries = toInt(characterID, 0) > 0
    ...
    const miningAmountM3 = Math.max(0, round6(toFiniteNumber(moduleAttributes[ATTRIBUTE_MINING_AMOUNT], 0)));
    if (miningAmountM3 <= 0) {
        return null;
    }
    return {
        ...
        maxRangeMeters: Math.max(0, round6(toFiniteNumber(moduleAttributes[ATTRIBUTE_MAX_RANGE], 0))),
        ...
        critQuantityMultiplier: Math.max(0, round6(toFiniteNumber(moduleAttributes[ATTRIBUTE_MINING_CRIT_BONUS], 0))),
        crystalVolatilityChance: Math.max(0, round6(toFiniteNumber(chargeAttributes[ATTRIBUTE_CRYSTAL_VOLATILITY_CHANCE], 0))),
        crystalVolatilityDamage: Math.max(0, round6(toFiniteNumber(chargeAttributes[ATTRIBUTE_CRYSTAL_VOLATILITY_DAMAGE], 0))),
        crystalTargetTypeListID: toInt(chargeAttributes[ATTRIBUTE_SPECIALIZATION_TYPE_LIST], 0),
        crystalYieldMultiplier: Math.max(0, round6(toFiniteNumber(chargeAttributes[ATTRIBUTE_SPECIALIZATION_YIELD_MULTIPLIER], 0))),
        moduleAttributes,
        chargeAttributes,
...
//# sourceMappingURL=miningDogma.js.map