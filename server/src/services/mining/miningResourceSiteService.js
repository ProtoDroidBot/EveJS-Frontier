...
const { resolveMiningVisualPresentation, } = require(path.join(__dirname, "./miningVisuals"));
const { recordGeneratedSiteBootstrap, resetMiningStartupSummary, } = require(path.join(__dirname, "./miningStartupSummary"));
...
function resolveSunExclusionRadius(scene) {
    ...
    return sceneEntities
        ...
        .reduce((maximum, entity) => Math.max(maximum, Math.max(0, toFiniteNumber(entity && entity.radius, 0)) + 250_000), 0);
}
function clampPositionOutsideSun(scene, position, minimumMarginMeters = 0) {
    const exclusionRadius = Math.max(0, resolveSunExclusionRadius(scene) + Math.max(0, toFiniteNumber(minimumMarginMeters, 0)));
    ...
    return scaleVector(normalizeVector(resolvedPosition, { x: 1, y: 0, z: 0 }), exclusionRadius);
}
...
function getAnchorCandidates(scene) {
    ...
    const isSafeAnchorEntity = (entity) => (entity &&
        entity.position &&
        entity.generatedMiningSite !== true &&
        getVectorMagnitude(entity.position) > sunExclusionRadius);
    const beltEntities = sceneEntities.filter((entity) => (entity &&
    ...
    const celestialEntities = sceneEntities.filter((entity) => (entity &&
        isSafeAnchorEntity(entity) &&
        entity.kind !== "sun" &&
        toInt(entity.groupID, 0) !== 6 &&
        entity.kind !== "station" &&
    ...
    const infrastructureEntities = sceneEntities.filter((entity) => (entity &&
        isSafeAnchorEntity(entity) &&
        (entity.kind === "station" ||
            entity.kind === "structure" ||
            entity.kind === "stargate")));
    ...
    const fallbackDistance = Math.max(1_000_000_000, Math.ceil(sunExclusionRadius + 500_000), Math.round(toFiniteNumber(systemRecord && systemRecord.radius, 0) * 0.25));
    return [{
...
function getScanMgrServiceModule() {
    if (!scanMgrServiceModule) {
        scanMgrServiceModule = require(path.join(__dirname, "../exploration/scanMgrService"));
    }
...
function getAuthorityIceTemplateTypeRecords(options = {}) {
    ...
    return getTypeRecordsByNames(iceSystemAuthority.ICE_TYPE_NAMES_BY_DUNGEON_ID[sourceDungeonID] || []);
}
function getTemplateTypeRecords(kind, securityBand, options = {}) {
    ...
    const templateNames = (TEMPLATE_NAMES[kind] &&
    ...
    for (const fallbackBand of fallbackBands) {
        ...
        records = getTypeRecordsByNames((TEMPLATE_NAMES[kind] && TEMPLATE_NAMES[kind][fallbackBand]) || []);
        if (records.length > 0) {
            return records;
        }
    }
...
function buildDungeonGasCloudItemID(siteID, childIndex) {
    return ((Math.max(0, toInt(siteID, 0)) * DUNGEON_GAS_SITE_CHILD_STRIDE) +
...
function buildFieldCenter(anchorPosition, seed) {
    ...
    const distance = Math.max(25_000, toFiniteNumber(config.miningGeneratedSiteAnchorOffsetMeters, DEFAULT_SITE_ANCHOR_OFFSET_METERS));
...
function resolveGeneratedSiteVisualPresentation(kind, templateTypeRecord, itemID) {
    ...
    const shellTypeID = normalizePositiveInt(presentation && presentation.visualTypeID, normalizePositiveInt(shellTypeRecord && shellTypeRecord.typeID, normalizePositiveInt(minedTypeRecord && minedTypeRecord.typeID, 0)));
    const resolvedShellTypeRecord = resolveItemByTypeID(shellTypeID) ||
        shellTypeRecord ||
        minedTypeRecord ||
        null;
    const graphicID = normalizePositiveInt(presentation && presentation.graphicID, normalizePositiveInt(minedTypeRecord && minedTypeRecord.graphicID, normalizePositiveInt(resolvedShellTypeRecord && resolvedShellTypeRecord.graphicID, 0)));
    return {
        shellTypeID,
...
function buildMineableQuantity(kind, seed) {
    ...
    return Math.max(1, Math.round(minimum + ((maximum - minimum) * rng())));
}
...
function buildMineableEntity(kind, systemID, siteIndex, childIndex, center, templateTypeRecord) {
    ...
    const siteRadiusMeters = Math.max(5_000, toFiniteNumber(config.miningGeneratedSiteRadiusMeters, DEFAULT_SITE_RADIUS_METERS));
    const visualPresentation = resolveGeneratedSiteVisualPresentation(kind, templateTypeRecord, itemID);
    ...
    return {
        ...
        generatedMiningSiteIndex: siteIndex,
        itemID,
        typeID: normalizePositiveInt(shellTypeRecord && shellTypeRecord.typeID, templateTypeRecord.typeID),
        groupID: normalizePositiveInt(shellTypeRecord && shellTypeRecord.groupID, templateTypeRecord.groupID),
        categoryID: normalizePositiveInt(shellTypeRecord && shellTypeRecord.categoryID, templateTypeRecord.categoryID),
        slimTypeID: templateTypeRecord.typeID,
        slimGroupID: templateTypeRecord.groupID,
        slimCategoryID: templateTypeRecord.categoryID,
        visualTypeID: normalizePositiveInt(visualPresentation.shellTypeID, normalizePositiveInt(shellTypeRecord && shellTypeRecord.typeID, templateTypeRecord.typeID)),
        ...
        staticVisibilityScope: "bubble",
        resourceQuantity: buildMineableQuantity(kind, hashValue(systemID * 8191 + siteIndex * 257 + childIndex * 13 + templateTypeRecord.typeID)),
    };
}
function buildGeneratedSiteMemberDefinition(kind, systemID, rawSiteIndex, childIndex, templateTypeRecord, rotationIndex = 0) {
    if (!templateTypeRecord) {
        return null;
    }
    const entityID = buildChildItemID(kind, systemID, rawSiteIndex, childIndex);
    const visualPresentation = resolveGeneratedSiteVisualPresentation(kind, templateTypeRecord, entityID);
    const shellTypeRecord = visualPresentation.shellTypeRecord || templateTypeRecord;
    const originalQuantity = buildMineableQuantity(kind, hashValue(systemID * 8191 +
        rawSiteIndex * 257 +
        childIndex * 13 +
...
function buildGeneratedResourceSiteDefinition(systemID, kind, localSiteIndex, options = {}) {
    ...
    const sourceDungeonID = normalizedKind === "ice"
        ? normalizePositiveInt(options && options.sourceDungeonID, normalizePositiveInt(authorityRow && authorityRow.sourceDungeonID, 0))
        : 0;
    ...
    const rawSiteIndex = resolveGeneratedSiteRawIndex(normalizedKind, normalizedLocalSiteIndex);
    const memberCount = getChildCountForKind(normalizedKind);
    const members = [];
    for (let childIndex = 0; childIndex < memberCount; childIndex += 1) {
        const templateTypeRecord = pickTemplateType(templateTypeRecords, hashValue(numericSystemID * 4099 +
            rawSiteIndex * 97 +
            childIndex * 17 +
            (rotationIndex * 131)));
        const member = buildGeneratedSiteMemberDefinition(normalizedKind, numericSystemID, rawSiteIndex, childIndex, templateTypeRecord, rotationIndex);
        if (member) {
            members.push(member);
        }
    }
    ...
    const resourceTypeIDs = [...new Set(members
            .map((member) => normalizePositiveInt(member && member.yieldTypeID, 0))
            .filter((entry) => entry > 0))].sort((left, right) => left - right);
    const resourceNames = [...new Set(members
            .map((member) => String(member && member.resourceName || "").trim())
            .filter(Boolean))].sort((left, right) => left.localeCompare(right));
    const totalOriginalQuantity = members.reduce((sum, member) => sum + Math.max(0, toInt(member && member.originalQuantity, 0)), 0);
    return {
        ...
        label: String((options && options.label) ||
            (authorityRow && authorityRow.iceTypeNames && authorityRow.iceTypeNames[0]
                ? `${authorityRow.iceTypeNames[0]} Belt`
                : resolveGeneratedSiteLabel(normalizedKind, normalizedLocalSiteIndex))).trim(),
        securityBand,
...
function buildGeneratedResourceSiteDefinitionsForSystem(systemID) {
    ...
    if (config.miningGeneratedIceSitesEnabled === true) {
        ...
        for (let localSiteIndex = 0; localSiteIndex < iceSiteCount; localSiteIndex += 1) {
            const definition = buildGeneratedResourceSiteDefinition(numericSystemID, "ice", localSiteIndex, {
                authorityRow,
    ...
    return definitions.sort((left, right) => (toInt(left && left.rawSiteIndex, 0) - toInt(right && right.rawSiteIndex, 0)));
}
...
function listSeededDungeonMiningSiteDefinitions(systemID) {
    ...
    return dungeonRuntime
        ...
        const localSiteIndex = Math.max(0, toInt(metadata.localSiteIndex, spawnState.localSiteIndex != null
        ...
        return {
            ...
            siteID: Math.max(0, toInt(metadata.siteID, spawnState.siteID)) || buildAnchorItemID(family, numericSystemID, rawSiteIndex),
            ...
            label: String(metadata.label ||
                spawnState.label ||
                resolveGeneratedSiteLabel(family, localSiteIndex)).trim(),
        ...
        .sort((left, right) => (toInt(left && left.rawSiteIndex, 0) - toInt(right && right.rawSiteIndex, 0)));
}
function buildDungeonGasSiteMemberDefinition(siteID, childIndex, typeRecord) {
    ...
    const originalQuantity = buildMineableQuantity("gas", hashValue((normalizedSiteID * 257) +
...
function listActiveDungeonGasSiteDefinitions(systemID) {
    ...
    return dungeonRuntime
        ...
        const resourceComposition = template && template.resourceComposition && typeof template.resourceComposition === "object"
            ...
            : {};
        const gasTypeRecords = [...new Map((Array.isArray(resourceComposition.gasTypeIDs) ? resourceComposition.gasTypeIDs : [])
                .map((entry) => resolveItemByTypeID(normalizePositiveInt(entry, 0)))
                .filter(Boolean)
                .map((typeRecord) => [normalizePositiveInt(typeRecord && typeRecord.typeID, 0), typeRecord])).values()];
        if (gasTypeRecords.length <= 0) {
            return null;
        }
        const desiredMemberCount = Math.max(gasTypeRecords.length, getChildCountForKind("gas"));
        const members = [];
        for (let childIndex = 0; childIndex < desiredMemberCount; childIndex += 1) {
            ...
            const member = buildDungeonGasSiteMemberDefinition(Math.max(0, toInt(metadata.siteID, toInt(instance && instance.instanceID, 0))), childIndex, typeRecord);
            if (member) {
                members.push(member);
            }
        }
        ...
        return {
            ...
            label: String(metadata.label ||
                spawnState.label ||
                `Gas Site ${Math.max(0, toInt(template && template.sourceDungeonID, 0)) || siteID}`).trim(),
...
function buildMineableEntityFromDefinitionMember(kind, systemID, rawSiteIndex, center, member, options = {}) {
    const yieldTypeRecord = resolveItemByTypeID(normalizePositiveInt(member && member.yieldTypeID, 0));
    if (!yieldTypeRecord) {
        return null;
    }
    const childIndex = Math.max(0, toInt(member && member.childIndex, 0));
    const visualTypeRecord = resolveItemByTypeID(normalizePositiveInt(member && member.visualTypeID, 0)) || yieldTypeRecord;
    const siteRadiusMeters = Math.max(5_000, toFiniteNumber(config.miningGeneratedSiteRadiusMeters, DEFAULT_SITE_RADIUS_METERS));
    return {
        kind: kind === "gas" ? "gasCloud" : "iceChunk",
        ...
        generatedMiningSiteID: options.generatedMiningSite !== false
            ? Math.max(0, toInt(options.siteID, 0)) || null
            : undefined,
        generatedMiningSiteInstanceID: options.generatedMiningSite !== false
            ? Math.max(0, toInt(options.instanceID, 0)) || null
            : undefined,
        dungeonMaterializedGasSite: options.generatedMiningSite === false && kind === "gas"
            ? true
            : undefined,
        dungeonMaterializedGasSiteID: options.generatedMiningSite === false
            ? Math.max(0, toInt(options.siteID, 0)) || null
            : undefined,
        dungeonMaterializedGasSiteInstanceID: options.generatedMiningSite === false
            ? Math.max(0, toInt(options.instanceID, 0)) || null
            : undefined,
        itemID: normalizePositiveInt(member && member.entityID, 0),
        typeID: normalizePositiveInt(visualTypeRecord && visualTypeRecord.typeID, yieldTypeRecord.typeID),
        groupID: normalizePositiveInt(visualTypeRecord && visualTypeRecord.groupID, yieldTypeRecord.groupID),
        categoryID: normalizePositiveInt(visualTypeRecord && visualTypeRecord.categoryID, yieldTypeRecord.categoryID),
        slimTypeID: yieldTypeRecord.typeID,
        slimGroupID: yieldTypeRecord.groupID,
        slimCategoryID: yieldTypeRecord.categoryID,
        visualTypeID: normalizePositiveInt(member && member.visualTypeID, normalizePositiveInt(visualTypeRecord && visualTypeRecord.typeID, yieldTypeRecord.typeID)),
        miningYieldTypeID: yieldTypeRecord.typeID,
        miningYieldKind: kind,
        graphicID: normalizePositiveInt(visualTypeRecord && visualTypeRecord.graphicID, normalizePositiveInt(yieldTypeRecord && yieldTypeRecord.graphicID, 0)),
        slimGraphicID: normalizePositiveInt(visualTypeRecord && visualTypeRecord.graphicID, normalizePositiveInt(yieldTypeRecord && yieldTypeRecord.graphicID, 0)),
        ownerID: 1,
        itemName: String(yieldTypeRecord.name ||
            member && member.resourceName ||
            `${kind} ${childIndex + 1}`),
        slimName: String(yieldTypeRecord.name ||
            member && member.resourceName ||
            `${kind} ${childIndex + 1}`),
        position: buildMineablePosition(center, siteRadiusMeters, rawSiteIndex, childIndex),
        velocity: { x: 0, y: 0, z: 0 },
        direction: { x: 1, y: 0, z: 0 },
        radius: Math.max(1, toFiniteNumber(member && member.originalRadius, kind === "gas"
            ? 1_800 + ((childIndex % 4) * 250)
            : 3_000 + ((childIndex % 5) * 450))),
        staticVisibilityScope: "bubble",
...
function resolveSiteCenter(scene, definition, anchorCandidates) {
    ...
    const siteRadiusMeters = Math.max(5_000, toFiniteNumber(config.miningGeneratedSiteRadiusMeters, DEFAULT_SITE_RADIUS_METERS));
    if (explicitPosition) {
        return clampPositionOutsideSun(scene, explicitPosition, siteRadiusMeters + 50_000);
    }
    const rawSiteIndex = Math.max(0, toInt(definition && definition.rawSiteIndex, 0));
    const family = String(definition && definition.family || "").trim().toLowerCase();
    const sourceAnchor = anchorCandidates[rawSiteIndex % anchorCandidates.length] || anchorCandidates[0];
    if (!sourceAnchor || !sourceAnchor.position) {
        return null;
    }
    return clampPositionOutsideSun(scene, buildFieldCenter(sourceAnchor.position, hashValue((toInt(scene && scene.systemID, 0) * 12289) +
        (rawSiteIndex * 257) +
...
function buildSiteEntitiesFromDefinition(scene, definition, anchorCandidates) {
    ...
    for (const member of members) {
        const entity = buildMineableEntityFromDefinitionMember(family, systemID, rawSiteIndex, center, member, {
...
function buildSiteEntities(scene, kind, siteIndex, securityBand, anchorCandidates) {
    ...
    const siteRadiusMeters = Math.max(5_000, toFiniteNumber(config.miningGeneratedSiteRadiusMeters, DEFAULT_SITE_RADIUS_METERS));
    const center = clampPositionOutsideSun(scene, buildFieldCenter(sourceAnchor.position, hashValue(systemID * 12289 + siteIndex * 257 + (kind === "gas" ? 17 : 5))), siteRadiusMeters + 50_000);
    const entities = [];
    ...
    for (let childIndex = 0; childIndex < childCount; childIndex += 1) {
        const templateType = pickTemplateType(templateTypeRecords, hashValue(systemID * 4099 + siteIndex * 97 + childIndex * 17));
        const entity = buildMineableEntity(kind, systemID, siteIndex, childIndex, center, templateType);
        if (entity) {
            entity.position = clampPositionOutsideSun(scene, entity.position, Math.max(10_000, toFiniteNumber(entity.radius, 0) + 25_000));
            mineableCount += 1;
            entities.push(entity);
...
function buildGeneratedResourceSitePlan(scene) {
    ...
    if (generatedDefinitions.length <= 0) {
        ...
        try {
            const dungeonUniverseRuntime = require(path.join(__dirname, "../dungeon/dungeonUniverseRuntime"));
...
//# sourceMappingURL=miningResourceSiteService.js.map