...
const { classifyMiningMaterialType, } = require("./miningInventory");
const { computeAsteroidRadiusFromQuantity, } = require("./miningMath");
const { resolveMiningVisualPresentation, } = require("./miningVisuals");
const { flushMiningStartupSummary, mergeMiningPresentationSummary, } = require("./miningStartupSummary");
const { applyMiningYieldRadiusCommand, } = require(path.join(__dirname, "../../space/destiny/commands/mining.js"));
...
function isWormholeSystemID(systemID) {
    ...
    return (normalizedSystemID >= WORMHOLE_SYSTEM_MIN &&
...
function normalizeStateRecord(record = {}) {
    ...
    const respawnAtMs = remainingQuantity <= 0 && depletedAtMs > 0
        ? Math.max(depletedAtMs + DEPLETED_MINEABLE_RESPAWN_DELAY_MS, toInt(record.respawnAtMs, 0))
        : 0;
    return {
...
function isPersistedStateStillValid(scene, entity, persistedState, estimatedOriginalQuantity) {
    ...
    if (estimatedOriginalQuantity >= 100 &&
...
function buildTemplateEntriesForOreDefinition(definition) {
    ...
    const baseQuantityMultiplier = Math.max(0.1, toFiniteNumber(definition && definition.quantityMultiplier, 1));
    const entries = [];
    ...
    for (const gradeVariant of ORE_GRADE_VARIANTS) {
        ...
        entries.push({
            ...
            quantityMultiplier: Math.max(0.1, baseQuantityMultiplier * gradeVariant.quantityMultiplier),
        });
    }
    return entries;
}
...
function pickWeightedTemplateEntry(entity, entries) {
    ...
    const totalWeight = normalizedEntries.reduce((sum, entry) => sum + Math.max(0, toFiniteNumber(entry.weight, 0)), 0);
    if (totalWeight <= 0) {
        return normalizedEntries[0] || null;
    }
    const seed = hashInteger(toInt(entity && entity.itemID, 0)) ^
...
function isDecorativeAsteroidType(typeRecord) {
    ...
    return (groupName.includes("decorative asteroid") ||
...
function resolveYieldTypeForEntity(scene, entity) {
    ...
    const templateEntry = pickWeightedTemplateEntry(entity, resolveTemplateSetForEntity(scene, entity));
...
function estimateOriginalQuantity(scene, entity, yieldType, templateEntry = null) {
    const explicitQuantity = Math.max(0, toInt(entity &&
        (entity.resourceQuantity ??
            entity.mineableQuantity ??
            entity.originalQuantity), 0));
    if (explicitQuantity > 0) {
        return explicitQuantity;
    }
    const unitVolume = Math.max(0.000001, toFiniteNumber(yieldType && yieldType.volume, 1));
    const quantityScale = Math.max(0.000001, toFiniteNumber(config.miningBeltQuantityScale, 0.08));
    const quantityMultiplier = Math.max(0.1, toFiniteNumber(templateEntry && templateEntry.quantityMultiplier, 1));
    const minimumVolume = Math.max(1, toFiniteNumber(config.miningBeltMinimumAsteroidVolumeM3, 15_000));
    const maximumVolume = Math.max(minimumVolume, toFiniteNumber(config.miningBeltMaximumAsteroidVolumeM3, 3_000_000));
    const estimatedVolume = clamp((toFiniteNumber(entity && entity.radius, 0) ** 2) * quantityScale * quantityMultiplier, minimumVolume, maximumVolume);
...
function recordMiningPresentationSummary(summary, entity, state) {
    ...
    if (toInt(entity && entity.graphicID, 0) > 0 ||
...
function applyYieldPresentationToEntity(entity, state, summary = null) {
    ...
    const miningPresentationTypeRecord = resolveItemByTypeID(toInt(entity.miningPresentationTypeID, 0)) ||
        yieldTypeRecord;
    const resolvedPresentation = typeof resolveMiningVisualPresentation === "function" && yieldTypeRecord
        ? (() => {
            try {
        ...
        : null;
    const preferredVisualTypeID = toInt(state.visualTypeID ||
    ...
    const resolvedGraphicID = toInt(entity.preserveMiningVisualPresentation === true
        ? entity.graphicID || entity.slimGraphicID
        : (resolvedPresentation && resolvedPresentation.graphicID) ||
            entity.graphicID ||
            entity.slimGraphicID, 0);
    ...
    if (state.remainingQuantity > 0 &&
        ...
        const minimumRadiusRatio = Math.max(0.01, toFiniteNumber(config.miningDepletedAsteroidRadiusRatio, 0.25));
...
function shouldPersistMineableState(entity) {
    ...
    return !(entity.dungeonMaterializedSiteContent === true ||
...
function shouldRespawnDepletedMineable(entity) {
    ...
    return (normalizeLowerText(entity.kind, "") === "asteroid");
}
function getDepletedMineableRespawnAtMs(entity, state) {
    ...
    const depletedAtMs = Math.max(0, toInt(state.depletedAtMs, toInt(state.updatedAtMs, 0)));
    if (depletedAtMs <= 0) {
        return 0;
    }
    return Math.max(depletedAtMs + DEPLETED_MINEABLE_RESPAWN_DELAY_MS, toInt(state.respawnAtMs, 0));
}
...
function readPersistedSystemState(systemID) {
    const result = repo.read(MINING_RUNTIME_TABLE, `/systems/${String(toInt(systemID, 0))}/entities`);
...
function areMiningStatesEquivalent(left, right) {
    ...
    return (toInt(left.entityID, 0) === toInt(right.entityID, 0) &&
...
function writePersistedState(scene, state, options = {}) {
    ...
    repo.write(MINING_RUNTIME_TABLE, `/systems/${String(toInt(scene && scene.systemID, 0))}/entities/${String(toInt(state && state.entityID, 0))}`, cloneValue(state));
    const persistedByEntityID = (options && options.persistedByEntityID && typeof options.persistedByEntityID === "object")
...
function removePersistedState(scene, entityID, options = {}) {
    const systemID = toInt(scene && scene.systemID, 0);
    const normalizedEntityID = toInt(entityID, 0);
    if (systemID <= 0 || normalizedEntityID <= 0) {
        return false;
    }
    const removeResult = repo.remove(MINING_RUNTIME_TABLE, `/systems/${String(systemID)}/entities/${String(normalizedEntityID)}`);
    const persistedByEntityID = options.persistedByEntityID && typeof options.persistedByEntityID === "object"
        ? options.persistedByEntityID
        : (scene &&
            scene._miningRuntimeState &&
            scene._miningRuntimeState.persistedByEntityID &&
...
function readPersistedSystemEntities(systemID) {
    ...
    const readResult = repo.read(MINING_RUNTIME_TABLE, `/systems/${String(normalizedSystemID)}/entities`);
    return (readResult &&
        readResult.success &&
        readResult.data &&
...
function writePersistedSystemEntity(systemID, entityID, state) {
    ...
    const writeResult = repo.write(MINING_RUNTIME_TABLE, `/systems/${String(normalizedSystemID)}/entities/${String(normalizedEntityID)}`, cloneValue(state));
...
function removePersistedSystemEntity(systemID, entityID) {
    ...
    const removeResult = repo.remove(MINING_RUNTIME_TABLE, `/systems/${String(normalizedSystemID)}/entities/${String(normalizedEntityID)}`);
    return Boolean(removeResult && removeResult.success === true);
}
function buildMineableState(scene, entity, persistedState = null) {
    ...
    const templateEntry = entity && entity.skipMiningTemplateResolution === true
        ? null
        : pickWeightedTemplateEntry(entity, resolveTemplateSetForEntity(scene, entity));
    const yieldType = resolveItemByTypeID(toInt(rawPersistedState && rawPersistedState.yieldTypeID, 0)) ||
    ...
    const estimatedOriginalQuantity = estimateOriginalQuantity(scene, entity, yieldType, templateEntry);
    ...
    const remainingQuantity = clamp(toInt(normalizedPersistedState
        ? normalizedPersistedState.remainingQuantity
        : undefined, originalQuantity), 0, originalQuantity);
    return normalizeStateRecord({
        entityID: entity.itemID,
        visualTypeID: toInt(normalizedPersistedState
            ? normalizedPersistedState.visualTypeID
            : undefined, entity.visualTypeID || entity.typeID),
...
function registerMineableEntityInCache(scene, entity, cache, options = {}) {
...
async function ensureSceneMiningStateAsync(scene, options = {}) {
    if (!scene) {
        return null;
    }
...
function registerMineableEntity(scene, entity, options = {}) {
    ...
    if (normalizedState.remainingQuantity <= 0) {
        ...
        if (typeof scene.clearAllTargetingForEntity === "function") {
            scene.clearAllTargetingForEntity(entity, {
                reason: "Exploding",
                nowMs: options.nowMs,
                miningDepletionSourceEntityID: toInt(options.sourceEntity && options.sourceEntity.itemID, 0),
                miningDepletionModuleID: toInt(options.moduleItem && options.moduleItem.itemID, 0),
            });
        }
        try {
...
function clearMineableState(scene, entityID, options = {}) {
    const cache = ensureSceneMiningState(scene);
    const normalizedEntityID = toInt(entityID, 0);
...
function respawnDepletedMineables(scene, nowMs = Date.now(), options = {}) {
    const cache = ensureSceneMiningState(scene);
...
function resolveOreMinedCharacterID(sourceEntity, session) {
    return toInt(sourceEntity && (sourceEntity.characterID ||
        sourceEntity.pilotCharacterID), toInt(session && (session.characterID ||
        session.charID ||
        session.charid ||
        session.characterId), toInt(sourceEntity && sourceEntity.ownerID, 0)));
}
function resolveOreMinedSystemID(scene, sourceEntity, session) {
    return toInt(scene && (scene.systemID || scene.solarSystemID), toInt(sourceEntity && sourceEntity.systemID, toInt(session && (session.solarsystemid2 ||
        session.solarsystemid ||
        (session._space && session._space.systemID)), 0)));
}
function notifyOreMined(scene, entity, previousState, deltaData, options = {}) {
    ...
    const moduleTypeID = toInt(options.moduleTypeID, toInt(moduleItem && moduleItem.typeID, 0));
    const moduleTypeRecord = resolveItemByTypeID(moduleTypeID) || {};
    const shipTypeID = toInt(options.shipTypeID, toInt(sourceEntity && sourceEntity.typeID, 0));
    const shipTypeRecord = resolveItemByTypeID(shipTypeID) || {};
    const quantityAdded = Math.max(0, toInt(options.quantityAdded, 0));
    const amountWasted = Math.max(0, toInt(options.amountWasted, toInt(options.wastedQuantity, 0)));
    const quantityRemoved = Math.max(0, toInt(options.quantityRemoved, toInt(deltaData && deltaData.depletedQuantity, 0)));
    ...
    const payload = buildMarshalDict([
        ...
        [
            "shipGroupID",
            toInt(options.shipGroupID, toInt(sourceEntity && sourceEntity.groupID, toInt(shipTypeRecord.groupID, 0))),
        ],
        ...
        [
            "moduleGroupID",
            toInt(options.moduleGroupID, toInt(moduleItem && moduleItem.groupID, toInt(moduleTypeRecord.groupID, 0))),
        ],
    ]);
...
function resolveGeneratedMiningInstance(scene, entity) {
    ...
    return dungeonRuntime.listActiveInstancesBySystem(systemID, { full: true })
        ...
        const instanceRawSiteIndex = toInt(metadata.rawSiteIndex, toInt(spawnState.rawSiteIndex, -2));
        return (instance &&
            normalizeLowerText(instance.siteOrigin, "") === "generatedmining" &&
            normalizeLowerText(instance.siteFamily, "ice") === "ice" &&
    ...
    for (const siteEntity of getGeneratedIceSiteEntities(scene, entity)) {
        ...
        if (typeof scene.removeStaticEntity === "function" &&
            ...
            const removeResult = scene.removeStaticEntity(entityID, {
                broadcast: options.broadcast !== false,
                nowMs,
                terminalDestructionEffectID: siteEntity.generatedMiningSiteAnchor === true
                    ? 0
...
function completeGeneratedIceSite(scene, entity, instance, members, options = {}) {
    ...
    const respawnDelayMs = Math.max(1, toInt(iceSystemAuthority && typeof iceSystemAuthority.getRespawnDelayMsForSystem === "function"
...
function syncGeneratedMiningInstanceAfterDelta(scene, entity, nextState, options = {}) {
    ...
    const members = sourceMembers.map((member) => {
        ...
        const cachedState = cache && cache.byEntityID
            ...
            : null;
        const remainingQuantity = entityID === toInt(nextState && nextState.entityID, 0)
    ...
    const totalRemainingQuantity = members.reduce((sum, member) => sum + Math.max(0, toInt(member && member.remainingQuantity, 0)), 0);
    const dungeonRuntime = getDungeonRuntimeModule();
...
function applyMiningDelta(scene, entity, minedQuantity, wastedQuantity, options = {}) {
    ...
    const depletedQuantity = Math.max(0, toInt(minedQuantity, 0) + toInt(wastedQuantity, 0));
    ...
    const generatedMiningSite = syncGeneratedMiningInstanceAfterDelta(scene, entity, nextState, options);
    ...
    if (deltaData.depleted && entity && entity.frontierRiftResource === true) {
        try {
            const frontierRiftSceneService = require(path.join(__dirname, "../../space/frontierRiftSceneService"));
            if (frontierRiftSceneService &&
                typeof frontierRiftSceneService.handleResourceDepleted === "function") {
                deltaData.frontierRiftSite = frontierRiftSceneService.handleResourceDepleted(scene, entity, {
                    broadcast: options.broadcast !== false,
...
function clearPersistedSystemState(systemID) {
    ...
    const removeResult = repo.remove(MINING_RUNTIME_TABLE, `/systems/${String(normalizedSystemID)}`);
...
function resetPersistedBeltAsteroidState() {
    ...
    for (const [systemKey, systemNode] of Object.entries(result.data)) {
        ...
        if (Object.keys(retained).length === 0 && Object.keys(systemNode).length === 1) {
            const removeResult = repo.remove(MINING_RUNTIME_TABLE, `/systems/${systemKey}`);
            if (removeResult.success || removeResult.errorMsg === "ENTRY_NOT_FOUND") {
                systemsCleared += 1;
                continue;
            }
        }
        repo.write(MINING_RUNTIME_TABLE, `/systems/${systemKey}/entities`, retained);
    }
    return {
        systemsCleared,
        systemsTouched,
...
function summarizeSceneMiningState(scene) {
    ...
    const activeStaticEntityIDs = new Set([...(scene && scene.staticEntities ? scene.staticEntities : [])]
...
function resetSceneMiningState(scene, options = {}) {
    ...
    return {
        ...
        data: {
            ...
            asteroidResetResult: asteroidResetResult && asteroidResetResult.data
                ? asteroidResetResult.data
                : null,
            generatedResourceSiteResetResult: generatedResourceSiteResetResult && generatedResourceSiteResetResult.data
...
//# sourceMappingURL=miningRuntimeState.js.map