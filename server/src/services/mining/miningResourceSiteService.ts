...
function ensureTypeRecordCache() {
  ...
  for (const name of new Set([
    "Ice Field",
    "Gas Cloud 1",
    ...Object.values<any>(iceSystemAuthority.ICE_TYPE_NAMES_BY_DUNGEON_ID).flat(),
...
function getAuthorityIceTemplateTypeRecords(options: Record<string, any> = {}) {
...
function getTemplateTypeRecords(kind, securityBand, options: Record<string, any> = {}) {
...
function buildGeneratedResourceSiteDefinition(systemID, kind, localSiteIndex, options: Record<string, any> = {}) {
  ...
  return dungeonRuntime
    ...
    .map((instance) => {
      ...
      const resourceComposition = template && template.resourceComposition && typeof template.resourceComposition === "object"
        ...
        : {};
      const gasTypeRecords = [...new Map<any, any>(
...
function buildSiteEntitiesFromDefinition(
  ...
  if (definition && definition.materializeAnchor !== false) {
    const anchorEntity: Record<string, any> = buildAnchorEntity(family, systemID, rawSiteIndex, center);
    if (anchorEntity) {
...
function resetSceneGeneratedResourceSites(scene, options: Record<string, any> = {}) {
  if (!scene) {
    return {
