...
const { getCharacterMiningLogs, } = require("./miningLedgerState");
...
class CharacterMiningLedgerService extends BaseService {
    ...
    Handle_GetCharacterLogs(args, session) {
        const characterID = normalizeNumber(session && (session.characterID || session.charid), 0);
        log.info(`[CharacterMiningLedger] GetCharacterLogs char=${characterID}`);
        return buildList(getCharacterMiningLogs(characterID).map((entry) => buildMiningLedgerEntryKeyVal(entry)));
    }
}
module.exports = CharacterMiningLedgerService;
//# sourceMappingURL=characterMiningLedgerService.js.map