...
const syntheticNullsecAuthority = require(path.join(__dirname, "./iceSystemSyntheticNullsecAuthority.json"));
...
const PENDING_COMPOSITION_SYSTEM_IDS = Object.freeze(new Set(PENDING_COMPOSITION_SYSTEMS.map((row) => row.solarSystemID)));
...
function getSecurityBandForSystem(system) {
    ...
    const security = toFiniteNumber(system && (system.securityStatus != null ? system.securityStatus : system.security), 0);
    if (security >= 0.45) {
        return "highsec";
    }
    if (security >= 0) {
...
function getSystemsByName() {
    return new Map(worldData.getSolarSystems()
...
function getIceTypeIDsByName() {
    ...
    const names = new Set([
        ...
        ...(Array.isArray(syntheticNullsecAuthority && syntheticNullsecAuthority.rows)
            ? syntheticNullsecAuthority.rows.flatMap((row) => (Array.isArray(row && row.iceTypeNames) ? row.iceTypeNames : []))
            : []),
    ]);
...
function buildAuthorityRow(options) {
    ...
    const slotEvidence = (options && options.slotEvidence) ||
...
function buildSyntheticNullsecAuthorityRow(synthetic) {
    ...
    const systemName = normalizeText(synthetic && synthetic.solarSystemName, system && system.solarSystemName);
    if (!system) {
    ...
    return Object.freeze({
        ...
        confidence: normalizeText(synthetic && synthetic.compositionConfidence, "researched_synthetic_best_fit"),
        source: normalizeText(synthetic && synthetic.source, "dotlan_region_belts_ice_synthetic_nullsec_authority"),
...
//# sourceMappingURL=iceSystemAuthority.js.map