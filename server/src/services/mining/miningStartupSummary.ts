...
function ensureMiningStartupSummary(scene) {
  ...
  scene._miningStartupSummary = {
    ...
    beltIDs: new Set<any>(),
    asteroidCount: 0,
    orePoolTypeIDs: new Set<any>(),
    beltSubsetTypeIDs: new Set<any>(),
    iceSiteKeys: new Set<any>(),
    gasSiteKeys: new Set<any>(),
    iceChunkCount: 0,
    gasCloudCount: 0,
    presentation: {
      updatedCount: 0,
...
function recordAsteroidBootstrap(scene, payload: Record<string, any> = {}) {
...
function recordGeneratedSiteBootstrap(scene, payload: Record<string, any> = {}) {
  const summary = ensureMiningStartupSummary(scene);
  if (!summary) {
    return;
  }
...
function mergeMiningPresentationSummary(scene, presentation: Record<string, any> = {}) {
  const summary = ensureMiningStartupSummary(scene);
  if (!summary) {
    return;
  }
