...
function resolveStandingValue(characterID, targetOwnerIDs = []) {
    ...
    const standingMatch = standingRuntime.resolveBestStandingValue(normalizedCharacterID, normalizedTargetOwnerIDs);
    return {
        characterID: normalizedCharacterID,
        ...
        matchedEntry: standingMatch && standingMatch.matchedEntry
            ? {
...
function hasGenericNpcThresholds(hostileResponseThreshold, friendlyResponseThreshold) {
    return (hostileResponseThreshold === 11 &&
        friendlyResponseThreshold === 11);
}
function resolveStandingThresholdsForEntity(entity) {
    const hostileResponseThreshold = toFiniteNumber(entity && entity.hostileResponseThreshold, NaN);
    const friendlyResponseThreshold = toFiniteNumber(entity && entity.friendlyResponseThreshold, NaN);
    ...
    const configuredHostileThreshold = toFiniteNumber(config.miningNpcHostileStandingThreshold, -5);
    const configuredFriendlyThreshold = Math.max(configuredHostileThreshold, toFiniteNumber(config.miningNpcFriendlyStandingThreshold, 5));
    return {
...
//# sourceMappingURL=miningNpcStandings.js.map