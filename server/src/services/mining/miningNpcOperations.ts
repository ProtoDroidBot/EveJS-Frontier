...
const startupSceneSeedSet = new Set<any>();
...
function pruneFleetMinerAssignments(scene, fleetRecord) {
  ...
  const validMinerIDs = new Set<any>(
  ...
  for (const [minerEntityID, assignedAsteroidID] of Object.entries<any>(assignmentMap)) {
...
function isMiningSnapshotCompatibleWithEntry(snapshot, entry, hooks: Record<string, any> = {}) {
...
function chooseFleetMineableTarget(scene, fleetRecord, hooks: Record<string, any> = {}) {
  ensureSceneMiningState(scene);
  ...
  for (const assignedAsteroidID of Object.values<any>(assignmentMap)) {
...
function clearStaleMineableTargetLocks(scene, entity, targetEntityID, hooks: Record<string, any> = {}) {
  if (!scene || !entity) {
    return 0;
  }
...
function syncMiningTargetLock(scene, entity, targetEntity, now, hooks: Record<string, any> = {}) {
...
function resolveConfiguredBandQuery(scene, explicitQuery, options: Record<string, any> = {}) {
...
function applyPassiveMiningFleetOverrides(entityID, options: Record<string, any> = {}) {
...
function clearFleetManualOrders(entityIDs: any[] = []) {
  let changedCount = 0;
...
function getFleetEntityIDs(fleetRecord, options: Record<string, any> = {}) {
...
function getFleetEntities(scene, fleetRecord, options: Record<string, any> = {}) {
  if (!scene) {
    return [];
  }
...
function resolveResponsePlan(scene, fleetRecord, aggressorEntity, options: Record<string, any> = {}) {
...
function createMiningFleetRecord(options: Record<string, any> = {}) {
...
function autoEnrollConfiguredMiningNpcs(scene) {
  ...
  const miners = candidates.filter((controller) => (
    getConfiguredNpcActivity(controller) === "mining"
  ));
  if (miners.length <= 0) {
    return [];
  }
  const haulers = candidates.filter((controller) => (
    getConfiguredNpcActivity(controller) === "hauling"
  ));
  ...
  for (const [groupKey, groupMiners] of minersByGroup.entries()) {
    ...
    let groupHaulers = (haulersByGroup.get(groupKey) || []).filter((controller) => (
      !trackedEntityIDs.has(normalizePositiveInteger(controller && controller.entityID, 0))
    ));
    if (groupHaulers.length <= 0) {
      const anchorID = normalizePositiveInteger(firstMiner && firstMiner.anchorID, 0);
      if (anchorID > 0) {
        groupHaulers = haulers.filter((controller) => (
          !trackedEntityIDs.has(normalizePositiveInteger(controller && controller.entityID, 0)) &&
    ...
    const fleetRecord = createMiningFleetRecord({
      ...
      spawnSelectionName: String(
        firstMiner && (firstMiner.selectionName || firstMiner.profileID) || groupKey,
      ),
    });
...
function spawnFleetWing(systemID, centerTarget, options: Record<string, any> = {}) {
...
function spawnMiningFleetInternal(scene, options: Record<string, any> = {}) {
...
function issueResponseOrders(responseEntityIDs: any[] = [], aggressorEntityID = 0) {
...
function retreatResponseWingToOrigin(fleetRecord, options: Record<string, any> = {}) {
...
function spawnResponseWingForFleet(scene, fleetRecord, aggressorEntity, responsePlan, options: Record<string, any> = {}) {
  if (
    !scene ||
    !fleetRecord ||
    !aggressorEntity ||
...
function triggerFleetAggression(scene, fleetRecord, options: Record<string, any> = {}) {
...
function retreatFleetToOrigin(fleetRecord, options: Record<string, any> = {}) {
  if (!fleetRecord || !fleetRecord.originAnchor || !fleetRecord.originAnchor.position) {
    return 0;
  }
...
function handleMiningFleetAggroCommand(session, argumentText, options: Record<string, any> = {}) {
  if (!session || !session.characterID || !session._space) {
    return {
      success: false as const,
...
function tickScene(scene, now, hooks: Record<string, any> = {}) {
