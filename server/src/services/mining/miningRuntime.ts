...
function buildEntityMiningSnapshot(entity, moduleItem, effectRecord, options: Record<string, any> = {}) {
...
function computeUsedVolume(items: any[] = []) {
...
function syncMinedOreChangesToSession(session, shipID, changes: any[] = []) {
...
function isChargeValidForYield(chargeItem, mineableState, snapshot = null) {
  ...
  const targetTypeListID = toInt(
    snapshot && snapshot.crystalTargetTypeListID,
    0,
  );
...
function resolveMiningActivation(scene, entity, moduleItem, effectRecord, options: Record<string, any> = {}) {
  ...
  if (targetID <= 0) {
    return { matched: true, success: false as const, errorMsg: "TARGET_REQUIRED" };
  }
  if (
    !targetEntity ||
    !mineableState ||
    ...
    return { matched: true, success: false as const, errorMsg: "TARGET_NOT_FOUND" };
  }
  if (!getTargetsForEntity(scene, entity).includes(targetID)) {
    return { matched: true, success: false as const, errorMsg: "TARGET_NOT_LOCKED" };
  }
  ...
  if (
    ...
    return { matched: true, success: false as const, errorMsg: "TARGET_OUT_OF_RANGE" };
  }
...
function executeMiningCycle(
  ...
  if (efficiency <= 0) {
    return { success: false as const, stopReason: "cycle" };
  }
  ...
  if (
    ...
    return { success: false as const, stopReason: "target" };
  }
  if (
    options.requireTargetLock !== false &&
  ...
  const effectRecord = getEffectTypeRecord(toInt(effectState.effectID, 0));
  const snapshot = options.snapshot || buildEntityMiningSnapshot(
    entity,
    moduleItem,
    effectRecord,
    {
  ...
  const amountMultiplier = Math.max(
    0,
    toFiniteNumber(options.amountMultiplier, 1),
  );
  ...
  if (!isNativeNpcEntity(entity)) {
    ...
    if (mineableState.yieldKind === "ore" && transferredQuantity > 0) {
      try {
        lazyRequire("../dailyGoals/dailyGoalsState").recordActivity(
          resolveEntityCharacterID(entity),
          "mine_ore",
          transferredQuantity,
        );
        ...
        log.debug(
          `[MiningRuntime] daily-goal mine_ore hook failed: ${dailyGoalError.message}`,
        );
      }
    }
  }
 ...
 */
function executeSkillShotMiningCycle(
  scene,
  entity,
  targetEntity,
  moduleItem,
  ...
  const cycleResult = executeCycle(
    scene,
    entity,
    {
      targetID,
...
function resolveSkillShotMiningAmountMultiplier(snapshot, rawRampMultiplier = 1) {
  const authoredEfficiency = Math.max(
    0,
    toFiniteNumber(snapshot && snapshot.miningEfficiencyPercent, 100) / 100,
  );
  const rampMultiplier = Math.max(
    0,
    toFiniteNumber(rawRampMultiplier, 1),
  );
  return {
    authoredEfficiency,
    rampMultiplier,
...
function chooseMineableTargetForFleet(scene, fleetRecord) {
  const interactionSource = (
    Array.isArray(fleetRecord && fleetRecord.minerEntityIDs)
