...
const { currentFileTime, normalizeBigInt, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
const LEDGER_RETENTION_TICKS = BigInt(LEDGER_RETENTION_DAYS) * 24n * 60n * 60n * 1000n * FILETIME_TICKS_PER_MS;
...
function ensureTableShape(table) {
    const nextTable = table && typeof table === "object" ? table : buildDefaultMiningLedgerTable();
    nextTable._meta =
        nextTable._meta && typeof nextTable._meta === "object"
            ? nextTable._meta
            : {};
    nextTable._meta.version = LEDGER_VERSION;
    nextTable._meta.nextEntryID = Math.max(1, normalizePositiveInteger(nextTable._meta.nextEntryID, 1));
...
function ensureMiningLedgerTable() {
    ...
    const normalizedTable = ensureTableShape(readResult && readResult.success ? readResult.data : null);
...
function normalizeLedgerEntry(entry, overrides = {}) {
    const normalized = {
        entryID: normalizePositiveInteger(overrides.entryID ?? entry.entryID, 0),
        eventDate: normalizeFiletimeString(overrides.eventDate ?? entry.eventDate),
        characterID: normalizePositiveInteger(overrides.characterID ?? entry.characterID, 0),
        corporationID: normalizePositiveInteger(overrides.corporationID ?? entry.corporationID, 0),
        solarSystemID: normalizePositiveInteger(overrides.solarSystemID ?? entry.solarSystemID, 0),
        ...
        quantityWasted: Math.max(0, toInt(overrides.quantityWasted ?? entry.quantityWasted, 0)),
        quantityCritical: Math.max(0, toInt(overrides.quantityCritical ?? entry.quantityCritical, 0)),
        shipTypeID: normalizePositiveInteger(overrides.shipTypeID ?? entry.shipTypeID, 0),
        moduleTypeID: normalizePositiveInteger(overrides.moduleTypeID ?? entry.moduleTypeID, 0),
        yieldKind: String((overrides.yieldKind ?? entry.yieldKind) || "").trim().toLowerCase(),
        observerItemID: normalizePositiveInteger(overrides.observerItemID ?? entry.observerItemID, 0),
        characterVisibleAfter: normalizeFiletimeString(overrides.characterVisibleAfter ?? entry.characterVisibleAfter, overrides.eventDate ?? entry.eventDate),
        observerVisibleAfter: normalizeFiletimeString(overrides.observerVisibleAfter ?? entry.observerVisibleAfter, overrides.eventDate ?? entry.eventDate),
    };
    return normalized;
}
function normalizeObserverBucket(observerID, bucket = {}, fallbackCorporationID = 0) {
    return {
        ...
        corporationID: normalizePositiveInteger(bucket.corporationID, fallbackCorporationID),
...
function buildObserverDescriptor(observerID, bucket = null) {
    ...
    const structure = numericObserverID
        ...
        : null;
    const normalizedBucket = normalizeObserverBucket(numericObserverID, bucket || {}, normalizePositiveInteger(structure && (structure.ownerCorpID || structure.ownerID), 0));
    return {
        itemID: numericObserverID,
        corporationID: normalizedBucket.corporationID,
        solarSystemID: normalizePositiveInteger(normalizedBucket.solarSystemID || (structure && structure.solarSystemID), 0),
        itemName: normalizedBucket.itemName ||
...
function maybePruneExpiredEntries(nowFiletime = currentFileTime(), options = {}) {
    ...
    for (const [characterID, bucket] of Object.entries(table.characters || {})) {
        ...
        const retainedEntries = entries.filter((entry) => normalizeBigInt(entry && entry.eventDate, 0n) >= cutoffFiletime);
    ...
    table._meta.lastPrunedAtFiletime = normalizeBigInt(nowFiletime, currentFileTime()).toString();
...
function ensureObserverBucket(table, observerID, descriptor) {
    ...
    const currentBucket = table.observers[key] && typeof table.observers[key] === "object"
        ? table.observers[key]
        : {};
    table.observers[key] = normalizeObserverBucket(observerID, {
        ...currentBucket,
        ...descriptor,
...
function recordMiningLedgerEvent(payload = {}) {
    ...
    const table = ensureMiningLedgerTable();
    const eventDate = normalizeFiletimeString(payload.eventDate || filetimeFromUnixMs(payload.eventDateMs));
    ...
    const entry = normalizeLedgerEntry(payload, {
        ...
        quantity,
        quantityWasted,
        characterVisibleAfter: buildVisibleAfterFiletime(eventDate, resolveCharacterLedgerDelayMs()),
        observerVisibleAfter: buildVisibleAfterFiletime(eventDate, resolveObserverLedgerDelayMs()),
    });
    const characterBucket = ensureCharacterBucket(table, characterID, corporationID);
    characterBucket.entries.push(entry);
    repo.write(MINING_LEDGER_TABLE, `/characters/${String(characterID)}`, characterBucket);
    if (entry.observerItemID > 0) {
        ...
        const observerBucket = ensureObserverBucket(table, entry.observerItemID, observerDescriptor);
        observerBucket.entries.push(entry);
        repo.write(MINING_LEDGER_TABLE, `/observers/${String(entry.observerItemID)}`, observerBucket);
    }
    bumpLedgerMutationVersion();
    return {
        success: true,
...
function getCharacterMiningLogs(characterID, options = {}) {
    ...
    const nowFiletime = normalizeBigInt(options.nowFiletime, currentFileTime());
    ...
    const table = ensureMiningLedgerTable();
    const entries = Array.isArray(table.characters &&
...
function getObserverMiningLedger(observerItemID, corporationID = 0, options = {}) {
    ...
    const table = ensureMiningLedgerTable();
    const bucket = table.observers && table.observers[String(numericObserverID)]
        ? normalizeObserverBucket(numericObserverID, table.observers[String(numericObserverID)])
        : null;
...
function listObserverHeadersForCorporation(corporationID, options = {}) {
    ...
    const headers = Object.entries(table.observers || {})
        ...
        .filter(([, bucket]) => (options.includeHidden === true ||
        ...
        .filter((entry) => entry.corporationID === numericCorporationID)
...
//# sourceMappingURL=miningLedgerState.js.map