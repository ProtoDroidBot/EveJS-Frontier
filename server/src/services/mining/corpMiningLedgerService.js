...
const { buildFiletimeLong, buildKeyVal, buildList, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { CORP_ROLE_DIRECTOR, CORP_ROLE_ACCOUNTANT, CORP_ROLE_JUNIOR_ACCOUNTANT, } = require(path.join(__dirname, "../corporation/corporationRuntimeState"));
const { getObserverMiningLedger, listObserverHeadersForCorporation, } = require("./miningLedgerState");
const LEDGER_ACCESS_MASK = CORP_ROLE_DIRECTOR +
    CORP_ROLE_ACCOUNTANT +
    CORP_ROLE_JUNIOR_ACCOUNTANT;
function hasCorpLedgerAccess(session) {
    try {
        const roleMask = typeof (session && session.corprole) === "bigint"
            ? session.corprole
...
function getSessionCorporationID(session) {
    return normalizeNumber(session && (session.corporationID || session.corpid), 0);
}
...
class CorpMiningLedgerService extends BaseService {
    ...
    Handle_GetObserversWithMiningEvents(args, session) {
        ...
        log.info(`[CorpMiningLedger] GetObserversWithMiningEvents corp=${corporationID}`);
        if (!corporationID || !hasCorpLedgerAccess(session)) {
            return buildList([]);
        }
        return buildList(listObserverHeadersForCorporation(corporationID).map((entry) => buildObserverHeaderKeyVal(entry)));
    }
    Handle_GetObserverLedger(args, session) {
        const corporationID = getSessionCorporationID(session);
        const observerItemID = normalizeNumber(args && args[0], 0);
        log.info(`[CorpMiningLedger] GetObserverLedger corp=${corporationID} observer=${observerItemID}`);
        if (!corporationID || !observerItemID || !hasCorpLedgerAccess(session)) {
            return buildList([]);
        }
        return buildList(getObserverMiningLedger(observerItemID, corporationID).map((entry) => buildObserverLedgerEntryKeyVal(entry)));
    }
}
module.exports = CorpMiningLedgerService;
//# sourceMappingURL=corpMiningLedgerService.js.map