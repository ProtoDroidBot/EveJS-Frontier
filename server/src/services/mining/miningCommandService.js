...
const { spawnShipInHangarForSession, activateShipForSession, syncInventoryItemForSession, } = require(path.join(__dirname, "../character/characterState"));
const { ITEM_FLAGS, grantItemsToCharacterStationHangar, moveItemTypeFromCharacterLocation, } = require(path.join(__dirname, "../inventory/itemStore"));
const { resolveShipByName, } = require(path.join(__dirname, "../chat/shipTypeRegistry"));
...
const { buildShipResourceState, isChargeCompatibleWithModule, listFittedItems, selectAutoFitFlagForType, validateFitForShip, } = require(path.join(__dirname, "../fitting/liveFittingState"));
...
function tryPlanNextModuleFit(charID, shipItem, itemType, fittedItems) {
    const nextFlagID = selectAutoFitFlagForType(shipItem, fittedItems, toInt(itemType && itemType.typeID, 0));
    if (!nextFlagID) {
        return {
            success: false,
    ...
    const probeItem = buildPlannedFittedModuleItem(charID, shipItem, itemType, nextFlagID, -1000 - (Array.isArray(fittedItems) ? fittedItems.length : 0));
...
function tryPlanNextModuleFitWithOptions(charID, shipItem, itemType, fittedItems, options = {}) {
    const nextFit = tryPlanNextModuleFit(charID, shipItem, itemType, fittedItems);
    if (nextFit.success || options.forceFit !== true) {
        return nextFit;
    }
    const forcedFlagID = selectAutoFitFlagForType(shipItem, fittedItems, toInt(itemType && itemType.typeID, 0));
    if (!forcedFlagID) {
        return nextFit;
    }
    const plannedItems = [
        ...(Array.isArray(fittedItems) ? fittedItems : []),
        buildPlannedFittedModuleItem(charID, shipItem, itemType, forcedFlagID, -1000 - (Array.isArray(fittedItems) ? fittedItems.length : 0)),
    ];
    return {
        success: true,
        data: {
...
function getCompatibleModuleChargeTypes(moduleTypeID) {
    ...
    const chargeTypes = dedupeItemTypes((cache.get("__allCharges") || []).filter((itemType) => isChargeCompatibleWithModule(numericModuleTypeID, itemType.typeID)));
    cache.set(numericModuleTypeID, chargeTypes);
    return chargeTypes;
}
function grantStationHangarBatchAndSyncSession(session, stationID, grantEntries) {
    const result = grantItemsToCharacterStationHangar(session.characterID, stationID, grantEntries);
    if (!result.success) {
        return result;
    }
...
function fitGrantedItemTypeToShip(session, stationID, shipItem, itemType, count, options = {}) {
    ...
    for (let index = 0; index < numericCount; index += 1) {
        ...
        const nextFit = tryPlanNextModuleFitWithOptions(session.characterID, shipItem, itemType, fittedItems, {
            forceFit,
        });
        if (!nextFit.success) {
            break;
...
function resolveMinerCommandPreset() {
    return {
        ...
        mobilityModuleName: String(config.miningCommandMobilityModuleName || DEFAULT_MINER_MOBILITY_MODULE_NAME),
        mobilityModuleCount: Math.max(0, toInt(config.miningCommandMobilityModuleCount, DEFAULT_MINER_MOBILITY_MODULE_COUNT)),
        supportModuleName: String(config.miningCommandSupportModuleName || DEFAULT_MINER_SUPPORT_MODULE_NAME),
        supportModuleCount: Math.max(0, toInt(config.miningCommandSupportModuleCount, DEFAULT_MINER_SUPPORT_MODULE_COUNT)),
        ...
        crystalsPerType: Math.max(1, toInt(config.miningCommandCrystalQuantityPerType, DEFAULT_MINER_CRYSTALS_PER_TYPE)),
    };
}
function buildMinerCommandPlan(charID, shipItem) {
    ...
    const supportModuleLookup = preset.supportModuleCount > 0
        ...
        : null;
    if (preset.supportModuleCount > 0 &&
    ...
    const mobilityModuleLookup = preset.mobilityModuleCount > 0
        ...
        : null;
    if (preset.mobilityModuleCount > 0 &&
    ...
    for (let index = 0; index < preset.moduleCount; index += 1) {
        const nextFit = tryPlanNextModuleFit(charID, shipItem, moduleLookup.match, plannedItems);
        if (!nextFit.success) {
            return {
                success: false,
    ...
    for (let index = 0; index < preset.supportModuleCount; index += 1) {
        const nextFit = tryPlanNextModuleFit(charID, shipItem, supportModuleLookup.match, plannedItems);
        if (!nextFit.success) {
            return {
                success: false,
    ...
    for (let index = 0; index < preset.rigCount; index += 1) {
        const nextFit = tryPlanNextModuleFit(charID, shipItem, rigLookup.match, plannedItems);
        if (!nextFit.success) {
            return {
                success: false,
    ...
    for (let index = 0; index < preset.mobilityModuleCount; index += 1) {
        const nextFit = tryPlanNextModuleFitWithOptions(charID, shipItem, mobilityModuleLookup.match, plannedItems, {
            forceFit: true,
        });
        if (!nextFit.success) {
            return {
    ...
    const totalCrystalVolume = chargeTypes.reduce((sum, itemType) => sum + (toFiniteNumber(itemType && itemType.volume, 0) * preset.crystalsPerType), 0);
...
function handleMinerCommand(session) {
    ...
    const fitResult = fitGrantedItemTypeToShip(session, stationID, shipItem, fitPlan.moduleType, fitPlan.preset.moduleCount);
    ...
    if (fitPlan.supportModuleType && fitPlan.preset.supportModuleCount > 0) {
        const supportFitResult = fitGrantedItemTypeToShip(session, stationID, shipItem, fitPlan.supportModuleType, fitPlan.preset.supportModuleCount);
        if (!supportFitResult.success ||
    ...
    if (fitPlan.rigType && fitPlan.preset.rigCount > 0) {
        const rigFitResult = fitGrantedItemTypeToShip(session, stationID, shipItem, fitPlan.rigType, fitPlan.preset.rigCount);
        if (!rigFitResult.success ||
            !rigFitResult.data ||
    ...
    if (fitPlan.mobilityModuleType && fitPlan.preset.mobilityModuleCount > 0) {
        const mobilityFitResult = fitGrantedItemTypeToShip(session, stationID, shipItem, fitPlan.mobilityModuleType, fitPlan.preset.mobilityModuleCount, {
            forceFit: true,
        });
        if (!mobilityFitResult.success ||
    ...
    for (const crystalType of fitPlan.chargeTypes) {
        const cargoMoveResult = moveGrantedItemTypeToShipCargo(session, stationID, shipItem, crystalType, fitPlan.preset.crystalsPerType);
...
function createMiningFleetRecord(session, spawnResult, originAnchor) {
    const spawnedEntries = spawnResult &&
        spawnResult.data &&
    ...
    const fleetRecord = {
        ...
        minerEntityIDs,
        responseEntityIDs: [],
        spawnSelectionName: spawnResult &&
            spawnResult.data &&
...
function handleMiningFleetCommand(session, argumentText) {
    ...
    const spawnResult = npcService.spawnNpcBatchInSystem(systemID, {
        entityScopeMetadata: buildChildEntityScopeMetadata(shipEntity),
    ...
    const landingRadiusMeters = Math.max(500, toFiniteNumber(config.miningNpcFleetLandingRadiusMeters, DEFAULT_MINING_WARP_LANDING_RADIUS_METERS));
    const warpRequests = spawnResult.data.spawned.map((entry, index, list) => ({
        ...
        point: buildMiningWarpLandingPoint(shipEntity.position, index, list.length, landingRadiusMeters),
        options: {
...
function handleMiningFleetStatusCommand(session) {
    ...
    const summary = fleets.map((fleetRecord) => (`fleet ${fleetRecord.fleetID}: miners=${fleetRecord.minerEntityIDs.length}, response=${fleetRecord.responseEntityIDs.length}, state=${fleetRecord.state}`));
    return {
        success: true,
...
//# sourceMappingURL=miningCommandService.js.map