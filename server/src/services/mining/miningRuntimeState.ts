...
function buildSceneCache(scene, persistedByEntityID: Record<string, any> = {}) {
  return {
    version: MINING_RUNTIME_VERSION,
    persistedByEntityID,
    byEntityID: new Map(),
...
function writePersistedState(scene, state, options: Record<string, any> = {}) {
  ...
  if (normalizedState.remainingQuantity <= 0) {
    if (entity.kind === "detachedDungeonProp") {
      ...
      if (detachedRecord) {
        ...
        if (!removedRecord.success) return removedRecord;
...
function notifyOreMined(scene, entity, previousState, deltaData, options: Record<string, any> = {}) {
...
function idleDronesTargetingMineable(scene, entityID, options: Record<string, any> = {}) {
  try {
    const droneRuntime = require(path.join(__dirname, "../drone/droneRuntime"));
    if (droneRuntime && typeof droneRuntime.idleMiningDronesTargeting === "function") {
...
function removeGeneratedIceSiteEntities(scene, entity, options: Record<string, any> = {}) {
...
function completeGeneratedIceSite(scene, entity, instance, members, options: Record<string, any> = {}) {
  const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
...
function syncGeneratedMiningInstanceAfterDelta(scene, entity, nextState, options: Record<string, any> = {}) {
  if (!isGeneratedIceMineableEntity(entity)) {
    return null;
  }
...
function applyMiningDelta(scene, entity, minedQuantity, wastedQuantity, options: Record<string, any> = {}) {
...
function resetPersistedBeltAsteroidState() {
  ...
  for (const [systemKey, systemNode] of Object.entries<any>(result.data)) {
    ...
    for (const [entityKey, state] of Object.entries<any>(entities)) {
...
function resetSceneMiningState(scene, options: Record<string, any> = {}) {
  if (!scene) {
    return {
      success: false as const,
