...
const { currentFileTime, normalizeNumber, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { ITEM_FLAGS, findItemById, getItemMetadata, grantItemToOwnerLocation, listContainerItems, moveItemToLocation, removeInventoryItem, transferItemToOwnerLocation, } = require(path.join(__dirname, "../inventory/itemStore"));
const { JOURNAL_ENTRY_TYPE, adjustCharacterBalance, getCharacterWallet, } = require(path.join(__dirname, "../account/walletState"));
const { CORPORATION_WALLET_KEY_START, adjustCorporationWalletDivisionBalance, getCorporationWalletBalance, normalizeCorporationWalletKey, } = require(path.join(__dirname, "../corporation/corpWalletState"));
...
const DAY_FILETIME_TICKS = 24n * 60n * 60n * 10000000n;
...
function getSessionCharacterID(session) {
    return toPositiveInteger(session && (session.characterID ||
        session.charid ||
        session.characterId ||
        session.charID), 0);
}
function getSessionCorporationID(session) {
    return toPositiveInteger(session && (session.corporationID ||
        session.corpid ||
        session.corpID), 0);
}
function getSessionAllianceID(session) {
    return toPositiveInteger(session && (session.allianceID ||
        session.allianceid), 0);
}
function getSessionCorpAccountKey(session) {
    return normalizeCorporationWalletKey(session && (session.corpAccountKey ||
        session.corpaccountkey ||
        session.accountKey) || CORPORATION_WALLET_KEY_START);
}
...
function addFileTimeDays(fileTimeValue, days) {
    return (toFileTimeBigInt(fileTimeValue) +
        BigInt(Math.max(0, toInteger(days, 0))) * DAY_FILETIME_TICKS).toString();
}
function addFileTimeMinutes(fileTimeValue, minutes) {
    return (toFileTimeBigInt(fileTimeValue) +
        BigInt(Math.max(0, toInteger(minutes, 0))) * 60n * 10000000n).toString();
}
...
function ensureRuntimeTable() {
    ...
    const table = result.success && result.data && typeof result.data === "object"
        ? result.data
        : {};
...
function allocateContractID() {
    ...
    const existingMax = listContractRecords()
        .reduce((maxValue, record) => Math.max(maxValue, toPositiveInteger(record.contractID, 0)), CONTRACT_ID_START - 1);
    const nextContractID = Math.max(toPositiveInteger(table._meta && table._meta.nextContractID, CONTRACT_ID_START), existingMax + 1);
...
function resolveDockableLocation(locationID, session = null) {
    ...
    if (station) {
        ...
        return {
            ...
            regionID: toPositiveInteger(station.regionID || (system && system.regionID), 0),
        };
    }
    ...
    if (structure) {
        ...
        return {
            ...
            regionID: toPositiveInteger(structure.regionID || (system && system.regionID), 0),
        };
    }
    return {
        stationID: normalizedLocationID,
        solarSystemID: toPositiveInteger(session && (session.solarSystemID ||
            session.solarsystemid2 ||
            session.solarsystemid), 0),
        regionID: toPositiveInteger(session && (session.regionID ||
            session.regionid), 0),
    };
}
...
function getWalletBalance(ref) {
    ...
    if (walletRef.kind === "corporation") {
        return normalizeMoney(getCorporationWalletBalance(walletRef.ownerID, walletRef.accountKey), 0);
    }
...
function ensureWalletFunds(ref, amount, description) {
    ...
    if (balance === null || balance + 0.0001 < normalizedAmount) {
        throwContractError(`${description} requires ${normalizedAmount.toFixed(2)} ISK, but the wallet does not have enough funds.`);
    }
}
function adjustWallet(ref, delta, description, contractID, counterpartyID = 0) {
    ...
    const result = walletRef.kind === "corporation"
        ? adjustCorporationWalletDivisionBalance(walletRef.ownerID, walletRef.accountKey, normalizedDelta, options)
...
function validateCreationItems(info, ownerID) {
    for (const [itemID, quantity] of info.itemList) {
        validateSelectedItem(itemID, quantity, ownerID, info.startStationID, info.forCorp, info.startStationDivision);
    }
}
...
function moveSelectedItemsToEscrow(info, contractID, ownerID) {
    ...
    for (const [itemID, requestedQuantity] of info.itemList) {
        const currentItem = validateSelectedItem(itemID, requestedQuantity, ownerID, info.startStationID, info.forCorp, info.startStationDivision);
        const moveQuantity = getItemContractQuantity(currentItem, requestedQuantity);
        const moveResult = transferItemToOwnerLocation(currentItem.itemID, CONTRACT_ESCROW_OWNER_ID, escrowLocationID, 0, moveQuantity);
        ...
        const escrowItem = findMovedItem(moveChanges, escrowLocationID, currentItem.itemID, currentItem.typeID);
        if (!escrowItem) {
            throwContractError(`Failed to identify escrowed item ${currentItem.itemID}.`);
        }
        rows.push(buildContractItemRowFromInventory(escrowItem, moveQuantity, true, {
            recordID,
...
function transferContainedItemsToOwner(rootItemID, destinationOwnerID) {
    ...
    for (const child of collectContainedItems(rootItemID)) {
        const transferResult = transferItemToOwnerLocation(child.itemID, destinationOwnerID, child.locationID, child.flagID, null);
...
function syncInventoryChangesToSession(session, changes = [], options = {}) {
    ...
    const { emitItemsChangedBatchForSession } = require(path.join(__dirname, "../character/characterState"));
...
function ensureSupportedCreateInfo(info) {
    if (info.contractType !== CONTRACT_TYPE.ITEM_EXCHANGE &&
...
function createPlasticWrap(ownerID, packageLocationID) {
    const grantResult = grantItemToOwnerLocation(ownerID, packageLocationID, ITEM_FLAGS.HANGAR, { typeID: PLASTIC_WRAP_TYPE_ID }, 1, { singleton: 1 });
...
function createContract(rawInfo, session, options = {}) {
    ...
    if (info.contractType === CONTRACT_TYPE.COURIER) {
        ...
        baseRecord.volume = Math.round(calculateItemRowsVolume(baseRecord.items.filter((row) => row.inCrate)) * 100) / 100;
    }
    if (info.reward > 0) {
        adjustWallet(issuerWalletRef, -info.reward, `Contract ${contractID} reward escrow`, contractID, 0);
...
function findRequestedItems(ownerID, locationID, forCorp, typeID) {
    return listContainerItems(ownerID, locationID, null)
        .filter((item) => (toPositiveInteger(item && item.typeID, 0) === toPositiveInteger(typeID, 0) &&
...
function ensureRequestedItemsAvailable(contract, acceptorOwnerID, acceptForCorp) {
    for (const requested of (contract.items || []).filter((row) => !row.inCrate)) {
        ...
        for (const item of findRequestedItems(acceptorOwnerID, contract.startStationID, acceptForCorp, requested.itemTypeID)) {
...
function transferRequestedItemsToIssuer(contract, acceptorOwnerID, acceptForCorp) {
    const changes = [];
    const issuerFlagID = getOwnerDeliveryFlag(normalizeBoolean(contract.forCorp, false), contract.startStationDivision);
    for (const requested of (contract.items || []).filter((row) => !row.inCrate)) {
        let remaining = toPositiveInteger(requested.quantity, 0);
        const candidates = findRequestedItems(acceptorOwnerID, contract.startStationID, acceptForCorp, requested.itemTypeID);
        for (const item of candidates) {
            if (remaining <= 0) {
                break;
            }
            const moveQuantity = Math.min(remaining, getItemQuantity(item));
            const transferResult = transferItemToOwnerLocation(item.itemID, contract.issuerOwnerID, contract.startStationID, issuerFlagID, toInteger(item.singleton, 0) === 1 ? null : moveQuantity);
            if (!transferResult.success) {
                throwContractError(`Failed to transfer requested item ${item.itemID}.`);
            }
            changes.push(...((transferResult.data && transferResult.data.changes) || []));
            const movedItem = findMovedItem(transferResult.data && transferResult.data.changes, contract.startStationID, item.itemID, item.typeID);
            if (movedItem) {
...
function transferEscrowRowsToOwner(contract, targetOwnerID, locationID, flagID) {
    ...
    for (const row of nextRows.filter((entry) => entry.inCrate)) {
        ...
        const transferResult = transferItemToOwnerLocation(escrowItemID, targetOwnerID, locationID, flagID, null);
        if (!transferResult.success) {
        ...
        const movedItem = findMovedItem(transferChanges, locationID, escrowItemID, row.itemTypeID);
        if (movedItem) {
...
function acceptItemExchange(contract, session, acceptForCorp) {
    ...
    const requestedChanges = transferRequestedItemsToIssuer(contract, acceptorOwnerID, acceptForCorp);
    const offeredTransfer = transferEscrowRowsToOwner(contract, acceptorOwnerID, contract.startStationID, getAcceptorDeliveryFlag(acceptForCorp));
    if (contract.price > 0) {
        adjustWallet(acceptorWalletRef, -contract.price, `Contract ${contract.contractID} price paid`, contract.contractID, contract.issuerOwnerID);
        adjustWallet(contract.issuerWallet, contract.price, `Contract ${contract.contractID} price received`, contract.contractID, acceptorOwnerID);
    }
    if (contract.rewardEscrow > 0) {
        adjustWallet(acceptorWalletRef, contract.rewardEscrow, `Contract ${contract.contractID} reward received`, contract.contractID, contract.issuerOwnerID);
    }
...
function acceptCourier(contract, session, acceptForCorp) {
    ...
    if (contract.collateral > 0) {
        adjustWallet(acceptorWalletRef, -contract.collateral, `Contract ${contract.contractID} collateral escrow`, contract.contractID, contract.issuerOwnerID);
    }
    const transferResult = transferItemToOwnerLocation(wrap.itemID, acceptorOwnerID, contract.startStationID, getAcceptorDeliveryFlag(acceptForCorp), null);
    if (!transferResult.success) {
...
function acceptContract(contractID, session, acceptForCorp = false) {
    ...
    if (contract.type === CONTRACT_TYPE.ITEM_EXCHANGE) {
        acceptedContract = acceptItemExchange(contract, session, normalizeBoolean(acceptForCorp, false));
    }
    else if (contract.type === CONTRACT_TYPE.COURIER) {
        acceptedContract = acceptCourier(contract, session, normalizeBoolean(acceptForCorp, false));
    }
    if (acceptedContract) {
...
function listPackageContentItems(contract) {
    return listContainerItems(CONTRACT_ESCROW_OWNER_ID, contract.packageLocationID || contract.escrowLocationID, null)
...
function transferPackageContents(contract, destinationOwnerID, destinationLocationID, destinationFlagID) {
    ...
    for (const item of listPackageContentItems(contract)) {
        const transferResult = destinationOwnerID === toPositiveInteger(item.ownerID, 0)
            ...
            : transferItemToOwnerLocation(item.itemID, destinationOwnerID, destinationLocationID, destinationFlagID, null);
        if (!transferResult.success) {
            throwContractError(`Failed to transfer package item ${item.itemID}.`);
        }
        const transferChanges = (transferResult.data && transferResult.data.changes) || [];
        changes.push(...transferChanges);
        const movedItem = findMovedItem(transferChanges, destinationLocationID, item.itemID, item.typeID);
...
function completeCourierContract(contract, session) {
    ...
    const contentChanges = transferPackageContents(contract, contract.issuerOwnerID, contract.endStationID, getOwnerDeliveryFlag(normalizeBoolean(contract.forCorp, false), contract.startStationDivision));
    const removeChanges = removePlasticWrap(contract);
    if (contract.collateralEscrow > 0) {
        adjustWallet(contract.acceptorWallet, contract.collateralEscrow, `Contract ${contract.contractID} collateral returned`, contract.contractID, contract.issuerOwnerID);
    }
    if (contract.rewardEscrow > 0) {
        adjustWallet(contract.acceptorWallet, contract.rewardEscrow, `Contract ${contract.contractID} reward paid`, contract.contractID, contract.issuerOwnerID);
    }
    const next = {
        ...contract,
        status: CONTRACT_STATUS.FINISHED,
...
function failCourierContract(contract, session) {
    ...
    const contentChanges = transferPackageContents(contract, acceptorOwnerID, destinationLocationID, destinationFlagID);
    const removeChanges = removePlasticWrap(contract);
    if (contract.collateralEscrow > 0) {
        adjustWallet(contract.issuerWallet, contract.collateralEscrow, `Contract ${contract.contractID} collateral paid to issuer`, contract.contractID, acceptorOwnerID);
    }
    if (contract.rewardEscrow > 0) {
        adjustWallet(contract.issuerWallet, contract.rewardEscrow, `Contract ${contract.contractID} reward escrow refunded`, contract.contractID, acceptorOwnerID);
    }
    const next = {
        ...contract,
...
function returnOutstandingEscrow(contract, session) {
    const changes = [];
    if (contract.type === CONTRACT_TYPE.ITEM_EXCHANGE) {
        const returned = transferEscrowRowsToOwner(contract, contract.issuerOwnerID, contract.startStationID, getOwnerDeliveryFlag(normalizeBoolean(contract.forCorp, false), contract.startStationDivision));
        changes.push(...returned.changes);
        contract.items = returned.rows;
    }
    else if (contract.type === CONTRACT_TYPE.COURIER) {
        changes.push(...transferPackageContents(contract, contract.issuerOwnerID, contract.startStationID, getOwnerDeliveryFlag(normalizeBoolean(contract.forCorp, false), contract.startStationDivision)));
        changes.push(...removePlasticWrap(contract));
    }
    if (contract.rewardEscrow > 0) {
        adjustWallet(contract.issuerWallet, contract.rewardEscrow, `Contract ${contract.contractID} reward escrow refunded`, contract.contractID, 0);
...
function getCourierContractFromItemID(itemID, session = null) {
    ...
    const contract = listContractRecords().find((record) => (record.type === CONTRACT_TYPE.COURIER &&
...
function contractMatchesOwner(contract, ownerID) {
    ...
    return Boolean(normalizedOwnerID &&
...
function filterContractsForOwner(ownerID, options = {}) {
    ...
    return listContractRecords()
        ...
        .filter((contract) => (includeCombinedItemSearch ||
...
function filterMyCurrentContracts(session, isAccepted = false, forCorp = false) {
    ...
    return listContractRecords()
        ...
        .filter((contract) => (contract.status === CONTRACT_STATUS.OUTSTANDING ||
...
function filterExpiredContracts(session, forCorp = false) {
    ...
    return listContractRecords()
        .filter((contract) => contractMatchesOwner(contract, ownerID))
        .filter((contract) => (isContractExpired(contract) ||
        contract.status === CONTRACT_STATUS.REJECTED ||
        contract.status === CONTRACT_STATUS.FAILED))
        .sort((left, right) => toPositiveInteger(right.contractID, 0) - toPositiveInteger(left.contractID, 0));
}
...
function getContractStationID(contract, endpoint = "start") {
    return toPositiveInteger(endpoint === "end" ? contract && contract.endStationID : contract && contract.startStationID, 0);
}
function getContractSolarSystemID(contract, endpoint = "start") {
    const direct = toPositiveInteger(endpoint === "end" ? contract && contract.endSolarSystemID : contract && contract.startSolarSystemID, 0);
    if (direct > 0) {
        return direct;
    }
    ...
    return toPositiveInteger((station && station.solarSystemID) || (structure && structure.solarSystemID), 0);
}
function getContractRegionID(contract, endpoint = "start") {
    const direct = toPositiveInteger(endpoint === "end" ? contract && contract.endRegionID : contract && contract.startRegionID, 0);
    if (direct > 0) {
        return direct;
    }
...
function getContractConstellationID(contract, endpoint = "start") {
    const direct = toPositiveInteger(endpoint === "end" ? contract && contract.endConstellationID : contract && contract.startConstellationID, 0);
    if (direct > 0) {
        return direct;
    }
    const system = worldData.getSolarSystemByID(getContractSolarSystemID(contract, endpoint));
...
function resolveLocationScope(locationID) {
    ...
    if (worldData.getStationByID(normalizedLocationID) ||
        worldData.getStructureByID(normalizedLocationID)) {
        return { kind: "station", locationID: normalizedLocationID };
    }
    if ((normalizedLocationID >= 30000000 && normalizedLocationID < 40000000) ||
...
function getContractOfferedItems(contract) {
    return getContractSearchItems(contract).filter((item) => (!Object.prototype.hasOwnProperty.call(item, "inCrate") ||
...
function contractMatchesItemFilters(contract, filters, itemTypes) {
    ...
    if (searchHint === CONTRACT_SEARCH_HINT.BPO || searchHint === CONTRACT_SEARCH_HINT.BPC) {
        ...
        return items.some((item) => (toPositiveInteger(getItemMetadata(item && item.itemTypeID).categoryID, 0) === 9 &&
...
function sortSearchContracts(contracts, filters = {}) {
    ...
    contracts.sort((left, right) => {
        let comparison = compareSearchValues(getSearchSortValue(left, sortBy), getSearchSortValue(right, sortBy));
        if (descending) {
            comparison *= -1;
        }
        if (comparison !== 0) {
...
function searchContracts(session, filters = {}) {
    ...
    let contracts = listContractRecords()
        ...
        .filter((contract) => !filters.issuerID || (contract.issuerID === toPositiveInteger(filters.issuerID, 0) ||
...
function collectMyPageInfo(session) {
    ...
    for (const contract of listContractRecords()) {
        ...
        const needsAttention = (contract.status === CONTRACT_STATUS.REJECTED ||
            contract.status === CONTRACT_STATUS.FAILED ||
...
//# sourceMappingURL=contractRuntimeState.js.map