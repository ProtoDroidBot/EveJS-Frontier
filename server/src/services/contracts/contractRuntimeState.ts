...
function throwContractUserError(message, values: Record<string, any> = {}) {
...
function buildContractItemRowFromInventory(item, quantity, inCrate, extra: Record<string, any> = {}) {
...
function syncInventoryChangesToSession(session, changes: any[] = [], options: Record<string, any> = {}) {
  if (!session || typeof session.sendNotification !== "function") {
    return;
  }
...
function createContract(rawInfo, session, options: Record<string, any> = {}) {
...
function filterContractsForOwner(ownerID, options: Record<string, any> = {}) {
...
function normalizeNumericSet(rawValue, options: Record<string, any> = {}) {
  const unwrapped = unwrapMarshalValue(rawValue);
...
function sortSearchContracts(contracts, filters: Record<string, any> = {}) {
...
function searchContracts(session, filters: Record<string, any> = {}) {
...
function collectMyPageInfo(session) {
  ...
  const outstandingContracts: any[] = [];
  let nonCorp = 0;
  let corp = 0;
  let inProgress = 0;
