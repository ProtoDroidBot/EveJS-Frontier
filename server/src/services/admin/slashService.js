...
const { AVAILABLE_SLASH_COMMANDS, executeChatCommand } = require(path.join(__dirname, "../chat/chatCommands"));
...
function extractFeedbackChannel(args, kwargs) {
    const fromKwargs = textValue(extractKwarg(kwargs, "channelID") ??
        extractKwarg(kwargs, "channelName") ??
        extractKwarg(kwargs, "roomJid")).trim();
    if (fromKwargs) {
        return fromKwargs;
    }
...
class SlashService extends BaseService {
    ...
    _throwCommandListError() {
        const quotedCommands = AVAILABLE_SLASH_COMMANDS.map((command) => `'${command}'`).join(", ");
    ...
    Handle_SlashCmd(args, session, kwargs) {
        ...
        appendSlashDebug(`SlashCmd user=${session ? session.userid : "?"} char=${session ? session.characterID : "?"} command=${JSON.stringify(command)} args=${JSON.stringify(summarizeValue(args))} kwargs=${JSON.stringify(summarizeValue(kwargs))}`);
        if (command === "/") {
        ...
        try {
            ...
            appendSlashDebug(`SlashCmd error user=${session ? session.userid : "?"} char=${session ? session.characterID : "?"} command=${JSON.stringify(command)} args=${JSON.stringify(summarizeValue(args))} kwargs=${JSON.stringify(summarizeValue(kwargs))} error=${error.stack || error.message}`);
            chatHub.sendSystemMessage(session, message, feedbackChannel);
            return message;
        }
    }
...
//# sourceMappingURL=slashService.js.map