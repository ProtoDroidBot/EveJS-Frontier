...
const { buildDict, buildList, normalizeNumber, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
function getSessionCharacterID(session) {
    return toInteger(session && (session.characterID || session.charid || session.characterId), 0);
}
function getSessionAccountID(session) {
    return toInteger(session && (session.userid || session.userID || session.accountID), 0);
}
...
//# sourceMappingURL=tourneyMgrService.js.map