...
function getSystemIDFromArgsOrSession(args: any[] = [], session = null) {
  return toPositiveInt(
...
function normalizeState(solarSystemID, source: Record<string, any> = {}) {
  const numericSolarSystemID = toPositiveInt(solarSystemID, 0);
...
function setSystemState(solarSystemID, source: Record<string, any> = {}) {
  const numericSolarSystemID = toPositiveInt(solarSystemID, 0);
  if (!numericSolarSystemID) {
    return null;
  }
...
function buildTheftHistoryPayload(entries: any[] = []) {
...
function requestMainBankUnlink(solarSystemID) {
  ...
  if (!state || !state.mainBankLink) {
    return { success: false as const, state, link: null };
  }
  const link = state.mainBankLink;
  state.mainBankLink = null;
  return { success: true as const, state, link };
}
...
function unlockReserveBank(solarSystemID, keyTypeID) {
  ...
  if (!state || !state.essID || pulses <= 0) {
    return { success: false as const, state, pulses: 0 };
  }
  ...
  return { success: true as const, state, pulses };
}
