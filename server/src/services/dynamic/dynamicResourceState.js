...
const { buildDict, buildFiletimeLong, buildList, currentFileTime, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { TABLE, readStaticRows, clearReferenceCache, } = require(path.join(__dirname, "../_shared/referenceData"));
...
function getSystemIDFromArgsOrSession(args = [], session = null) {
    return toPositiveInt(args && args.length > 0 ? args[0] : getSystemIDFromSession(session), 0);
}
function getStaticSolarSystem(solarSystemID) {
    ...
    return readStaticRows(TABLE.SOLAR_SYSTEMS).find((system) => toPositiveInt(system && system.solarSystemID, 0) === numericSolarSystemID) || null;
}
...
function normalizeState(solarSystemID, source = {}) {
    ...
    return {
        ...
        currentOutput: toNumber(source.currentOutput, DEFAULT_DYNAMIC_RESOURCE_SETTINGS.equilibriumValue),
        ...
        reserveBankPulsesRemaining: Math.max(0, toInt(source.reserveBankPulsesRemaining, 0)),
...
function buildAgencyDataPayload() {
    return buildDict(listSystemStates()
...
function deriveReserveWindowState(state = null) {
    ...
    const currentPulse = state.reserveBankPulsesTotal - state.reserveBankPulsesRemaining + 1;
    const windowSize = state.reserveBankPulsesTotal /
...
//# sourceMappingURL=dynamicResourceState.js.map