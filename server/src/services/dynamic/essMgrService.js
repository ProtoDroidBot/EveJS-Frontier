...
const dynamicResourceState = require(path.join(__dirname, "./dynamicResourceState"));
...
function getSessionShipID(session = null) {
    return toInt(session && ((session._space && session._space.shipID) ||
        session.shipID ||
        session.shipid ||
...
function getEntityPosition(entity = null) {
    const source = entity && entity.position ||
...
function validateEssLinkEligibility(session, state, bankName) {
    ...
    if (!essEntity ||
        !canEntitiesInteractLocally(shipEntity, essEntity)) {
...
class EssMgrService extends BaseService {
    ...
    Handle_GetDataForClientSolarSystem(args, session) {
        const solarSystemID = dynamicResourceState.getSystemIDFromArgsOrSession(args, session);
        log.debug(`[EssMgr] GetDataForClientSolarSystem solarsystemid=${solarSystemID}`);
        return dynamicResourceState.buildEssDataPayload(dynamicResourceState.getSystemState(solarSystemID));
    }
    Handle_IsClientLinkedToReserveBank(args, session) {
        const solarSystemID = dynamicResourceState.getSystemIDFromSession(session);
        const characterID = getCharacterID(session);
        return dynamicResourceState.isCharacterLinkedToReserveBank(solarSystemID, characterID);
    }
    Handle_AttemptLinkToMainBank(args, session) {
        ...
        if (validationError) {
            log.debug(`[EssMgr] AttemptLinkToMainBank rejected solarsystemid=${solarSystemID} characterID=${characterID} reason=${validationError}`);
            return null;
        }
        const result = dynamicResourceState.attemptMainBankLink(solarSystemID, characterID);
        log.debug(`[EssMgr] AttemptLinkToMainBank solarsystemid=${solarSystemID} characterID=${characterID} success=${result.success}`);
        if (result.success) {
    ...
    Handle_AttemptLinkToReserveBank(args, session) {
        ...
        if (validationError) {
            log.debug(`[EssMgr] AttemptLinkToReserveBank rejected solarsystemid=${solarSystemID} characterID=${characterID} reason=${validationError}`);
            return null;
        }
        const result = dynamicResourceState.attemptReserveBankLink(solarSystemID, characterID);
        log.debug(`[EssMgr] AttemptLinkToReserveBank solarsystemid=${solarSystemID} characterID=${characterID} success=${result.success}`);
        if (result.success) {
    ...
    Handle_RequestMainBankUnlink(args, session) {
        ...
        log.debug(`[EssMgr] RequestMainBankUnlink solarsystemid=${solarSystemID} success=${result.success}`);
        if (result.success) {
    ...
    Handle_RequestReserveBankUnlink(args, session) {
        const solarSystemID = dynamicResourceState.getSystemIDFromSession(session);
        const characterID = getCharacterID(session);
        const result = dynamicResourceState.requestReserveBankUnlink(solarSystemID, characterID);
        log.debug(`[EssMgr] RequestReserveBankUnlink solarsystemid=${solarSystemID} characterID=${characterID} success=${result.success}`);
        if (result.success) {
    ...
    Handle_RequestUnlockReserveBank(args, session) {
        ...
        const result = dynamicResourceState.unlockReserveBank(solarSystemID, keyTypeID);
        log.debug(`[EssMgr] RequestUnlockReserveBank solarsystemid=${solarSystemID} keyTypeID=${keyTypeID} success=${result.success}`);
        if (result.success) {
            notifyEssDataChanged(session, result.state);
        }
        return null;
    ...
    Handle_GetMainBankTheftsForClientSolarSystem(args, session) {
        const solarSystemID = dynamicResourceState.getSystemIDFromArgsOrSession(args, session);
        const state = dynamicResourceState.getSystemState(solarSystemID);
        return dynamicResourceState.buildTheftHistoryPayload(state ? state.theftHistoryMain : []);
    }
    Handle_GetReserveBankTheftsForClientSolarSystem(args, session) {
        const solarSystemID = dynamicResourceState.getSystemIDFromArgsOrSession(args, session);
        const state = dynamicResourceState.getSystemState(solarSystemID);
        return dynamicResourceState.buildTheftHistoryPayload(state ? state.theftHistoryReserve : []);
    }
...
//# sourceMappingURL=essMgrService.js.map