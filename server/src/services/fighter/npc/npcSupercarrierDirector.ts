...
function hasTextMarker(entity, keys) {
  return keys.some((key) => (
    String(entity && entity[key] == null ? "" : entity[key]).trim().length > 0
  ));
}
...
function hasValidNpcFighterControllerScope(controllerEntity) {
  const normalizedIDs: Record<string, any> = {};
  for (const key of NPC_FIGHTER_PRIVATE_SCOPE_ID_FIELDS) {
    const normalized = normalizePersistentEntityID(
      controllerEntity && controllerEntity[key],
    );
    if (
      hasPresentPrivateScopeIdentity(controllerEntity, key) &&
      normalized === null
    ) {
      return false;
    }
  ...
  const hasDungeonSiteContentOrID = Boolean(
    normalizedIDs.dungeonSiteID !== null ||
    hasTruthyMarker(
      controllerEntity,
      NPC_FIGHTER_DUNGEON_SCOPE_BOOLEAN_MARKERS,
    ) ||
    hasTextMarker(
      controllerEntity,
      NPC_FIGHTER_DUNGEON_SCOPE_TEXT_MARKERS,
    ) ||
    hasArrayMarker(
      controllerEntity,
      NPC_FIGHTER_DUNGEON_SCOPE_ARRAY_MARKERS,
    )
  );
  if (
  ...
  const hasAirOwnerOperationOrPresentation = Boolean(
    normalizedIDs.airNpeOwnerCharacterID !== null ||
    normalizedIDs.airNpeOperationID !== null ||
    hasTruthyMarker(
      controllerEntity,
      NPC_FIGHTER_AIR_SCOPE_BOOLEAN_MARKERS,
    ) ||
    String(
      controllerEntity && controllerEntity.airNpeHostileWave == null
        ? ""
        : controllerEntity.airNpeHostileWave,
    ).trim().length > 0
  );
  if (
...
function buildNpcFighterScopeMetadata(controllerEntity: Record<string, any> = {}) {
  ...
  for (const key of [
    ...
    const normalized = String(
      controllerEntity[key] == null ? "" : controllerEntity[key],
    ).trim();
    if (normalized) {
...
function launchNpcFighterWing(scene, controllerEntity, behaviorProfile, options: Record<string, any> = {}) {
...
function syncNpcSupercarrierSystems(scene, entity, controller, behaviorProfile, targetEntity, options: Record<string, any> = {}) {
...
function resetNpcSupercarrierWing(scene, controllerEntity, controller, options: Record<string, any> = {}) {
