...
const { ITEM_FLAGS, FIGHTER_TUBE_FLAGS, grantItemToOwnerLocation, moveItemToLocation, updateInventoryItem, } = require(path.join(__dirname, "../../inventory/itemStore"));
const { getFighterAbilityMetaForSlot, TARGET_MODE_ITEMTARGETED, TARGET_MODE_UNTARGETED, } = require(path.join(__dirname, "../fighterAbilities"));
const { getTypeAttributeValue, } = require(path.join(__dirname, "../../fitting/liveFittingState"));
const { getCapitalRuntimeConfig, } = require(path.join(__dirname, "../../../space/npc/capitals/capitalNpcRuntimeConfig"));
const { cloneVector, getCapitalControllerState, listControlledNpcFighters, toFiniteNumber, toPositiveInt, } = require(path.join(__dirname, "../../../space/npc/capitals/capitalNpcState"));
const { hydrateNpcFighterEntity, } = require("./npcFighterSessionAdapter");
const { buildTrackedTubeFlagSet, resetNpcSupercarrierTubeState, } = require("./npcSupercarrierTubeState");
const { normalizePersistentEntityID, toJSONSafeEntityID, } = require(path.join(__dirname, "../../../space/destiny/identity/entityID"));
const NPC_FIGHTER_PRIVATE_SCOPE_ID_FIELDS = Object.freeze([
...
const NPC_FIGHTER_DUNGEON_SCOPE_BOOLEAN_MARKERS = Object.freeze([
...
const NPC_FIGHTER_DUNGEON_SCOPE_TEXT_MARKERS = Object.freeze([
    "dungeonEncounterKey",
...
const NPC_FIGHTER_DUNGEON_SCOPE_ARRAY_MARKERS = Object.freeze([
    "dungeonSiteContentExplicitLoot",
    "dungeonSiteContentLootTags",
]);
const NPC_FIGHTER_AIR_SCOPE_BOOLEAN_MARKERS = Object.freeze([
    "airNpeBoardingShipPresentation",
    "airNpeTutorialDebris",
    "airNpeHostile",
]);
function hasPresentPrivateScopeIdentity(entity, key) {
    return Boolean(entity &&
        Object.hasOwn(entity, key) &&
        entity[key] !== null &&
        entity[key] !== undefined);
}
function hasTruthyMarker(entity, keys) {
    return keys.some((key) => entity && entity[key] === true);
}
function hasTextMarker(entity, keys) {
    return keys.some((key) => (String(entity && entity[key] == null ? "" : entity[key]).trim().length > 0));
}
function hasArrayMarker(entity, keys) {
    return keys.some((key) => entity && Array.isArray(entity[key]));
}
function hasValidNpcFighterControllerScope(controllerEntity) {
    const normalizedIDs = {};
    for (const key of NPC_FIGHTER_PRIVATE_SCOPE_ID_FIELDS) {
        const normalized = normalizePersistentEntityID(controllerEntity && controllerEntity[key]);
        if (hasPresentPrivateScopeIdentity(controllerEntity, key) &&
            normalized === null) {
            return false;
        }
        normalizedIDs[key] = normalized;
    }
    const hasDungeonSiteContentOrID = Boolean(normalizedIDs.dungeonSiteID !== null ||
        hasTruthyMarker(controllerEntity, NPC_FIGHTER_DUNGEON_SCOPE_BOOLEAN_MARKERS) ||
        hasTextMarker(controllerEntity, NPC_FIGHTER_DUNGEON_SCOPE_TEXT_MARKERS) ||
        hasArrayMarker(controllerEntity, NPC_FIGHTER_DUNGEON_SCOPE_ARRAY_MARKERS));
    if (hasDungeonSiteContentOrID &&
        normalizedIDs.dungeonSiteInstanceID === null) {
        return false;
    }
    const hasAirOwnerOperationOrPresentation = Boolean(normalizedIDs.airNpeOwnerCharacterID !== null ||
        normalizedIDs.airNpeOperationID !== null ||
        hasTruthyMarker(controllerEntity, NPC_FIGHTER_AIR_SCOPE_BOOLEAN_MARKERS) ||
        String(controllerEntity && controllerEntity.airNpeHostileWave == null
            ? ""
            : controllerEntity.airNpeHostileWave).trim().length > 0);
    if (hasAirOwnerOperationOrPresentation &&
        normalizedIDs.airNpeInstanceID === null) {
        return false;
    }
    return true;
}
...
function buildNpcFighterScopeMetadata(controllerEntity = {}) {
    const metadata = {};
    for (const key of [
        "airNpeOwnerCharacterID",
        "airNpeOperationID",
        ...
        const normalized = normalizePersistentEntityID(controllerEntity[key]);
        if (normalized !== null) {
    ...
    for (const key of [
        ...
        const normalized = String(controllerEntity[key] == null ? "" : controllerEntity[key]).trim();
        if (normalized) {
    ...
    for (const key of [
        ...
        if (Array.isArray(controllerEntity[key])) {
            metadata[key] = cloneValue(controllerEntity[key]);
        }
    }
    return metadata;
}
function resolveFighterSquadronSize(typeID) {
    return Math.max(1, toPositiveInt(getTypeAttributeValue(typeID, "fighterSquadronMaxSize"), toPositiveInt(getTypeAttributeValue(typeID, "fighterSquadronSize"), 1)));
}
function launchNpcFighterWing(scene, controllerEntity, behaviorProfile, options = {}) {
    ...
    if (!hasValidNpcFighterControllerScope(controllerEntity)) {
        return {
            success: false,
            launched: [],
            launchedTubeFlagIDs: [],
    ...
    const launched = [];
    const entityScopeMetadata = buildNpcFighterScopeMetadata(controllerEntity);
    ...
    for (const wingEntry of wing) {
        ...
        const tubeFlagID = toPositiveInt(wingEntry && wingEntry.tubeFlagID, FIGHTER_TUBE_FLAGS[launchIndex] || ITEM_FLAGS.FIGHTER_TUBE_0);
        launchIndex += 1;
        ...
        const grantResult = grantItemToOwnerLocation(toPositiveInt(controllerEntity.ownerID || controllerEntity.corporationID, 1), controllerEntity.itemID, tubeFlagID, fighterType, squadronSize, {
            singleton: false,
        ...
        const fighterEntity = hydrateNpcFighterEntity(spawnResult.data.entity, updateResult.data);
        ...
        scene.orbitShipEntity(fighterEntity, controllerEntity.itemID, 1_500, { broadcast: true });
...
function syncNpcFighterAttack(scene, controllerEntity, behaviorProfile, targetEntity, nowMs) {
    ...
    for (const fighterEntity of listControlledNpcFighters(scene, controllerEntity.itemID)) {
        ...
        if (abilityMeta.targetMode !== TARGET_MODE_ITEMTARGETED &&
            abilityMeta.targetMode !== TARGET_MODE_UNTARGETED) {
            continue;
        }
        const currentState = fighterEntity.fighterAbilityStates &&
        ...
        fighterEntity.fighterAbilityStates[slotID] = {
            ...
            targetID: abilityMeta.targetMode === TARGET_MODE_ITEMTARGETED
...
function syncNpcSupercarrierSystems(scene, entity, controller, behaviorProfile, targetEntity, options = {}) {
    ...
    let changed = false;
    const launchIntervalMs = Math.max(250, toPositiveInt(behaviorProfile.capitalFighterLaunchIntervalMs, classConfig.fighterLaunchIntervalMs));
    const launchQuota = Math.max(1, toPositiveInt(behaviorProfile.capitalFighterLaunchPerThink, classConfig.fighterLaunchPerThink));
    const trackedTubeFlagIDs = buildTrackedTubeFlagSet(scene, entity, controller);
    if (trackedTubeFlagIDs.size < behaviorProfile.capitalFighterWingTypeIDs.length &&
    ...
    if (targetEntity) {
        const abilitySyncIntervalMs = Math.max(250, toPositiveInt(behaviorProfile.capitalFighterAbilitySyncIntervalMs, classConfig.fighterAbilitySyncIntervalMs));
        if (nowMs >= toFiniteNumber(capitalState.nextFighterAbilitySyncAtMs, 0)) {
            changed = syncNpcFighterAttack(scene, entity, behaviorProfile, targetEntity, nowMs) || changed;
...
//# sourceMappingURL=npcSupercarrierDirector.js.map