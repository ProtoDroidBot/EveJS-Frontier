...
function buildTupleList(rows: any[] = []) {
...
class FighterMgrService extends BaseService {
  ...
  _listHostTubeItems(session, hostRecord, options: Record<string, any> = {}) {
    if (!hostRecord) {
      return [];
    }
  ...
  _emitInventoryChanges(session, changes: any[] = []) {
