...
const { findSessionByCharacterID, } = require(path.join(__dirname, "../chat/sessionRegistry"));
const { ITEM_FLAGS, findItemById, listContainerItems, mergeItemStacks, moveItemToLocation, normalizeShipConditionState, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
const structureState = require(path.join(__dirname, "../structure/structureState"));
const { buildEffectiveItemAttributeMap, getAttributeIDByNames, getTypeAttributeValue, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const { buildList, buildDict, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { buildUserErrorPayload, } = require(path.join(__dirname, "../../common/machoErrors"));
const { ABILITY_SLOT_IDS, TARGET_MODE_UNTARGETED, TARGET_MODE_ITEMTARGETED, TARGET_MODE_POINTTARGETED, getFighterAbilityMetaForSlot, } = require(path.join(__dirname, "./fighterAbilities"));
const { FIGHTER_CATEGORY_ID, FIGHTER_TUBE_FLAGS, buildInventorySquadronSize, canLoadFighterTypeIntoHostTube, isFighterItemRecord, isFighterTubeFlag, } = require(path.join(__dirname, "./fighterInventory"));
const { resolveFighterOperationalAttributes, resolveFighterAbilitySnapshot, } = require(path.join(__dirname, "./fighterDogma"));
const { applyDamageToEntity, normalizeDamageVector, sumDamageVector, hasDamageableHealth, } = require(path.join(__dirname, "../../space/combat/damage"));
const { resolveMissileAppliedDamage, } = require(path.join(__dirname, "../../space/combat/missiles/missileSolver"));
const crimewatchState = require(path.join(__dirname, "../security/crimewatchState"));
const jammerModuleRuntime = require(path.join(__dirname, "../../space/modules/jammerModuleRuntime"));
const hostileModuleRuntime = require(path.join(__dirname, "../../space/modules/hostileModuleRuntime"));
const { entityIDsEqual, normalizeNonNegativeInt64, normalizePersistentEntityID, toJSONSafeEntityID, } = require(path.join(__dirname, "../../space/destiny/identity/entityID"));
const { applyFighterPassiveMaxVelocityCommand, applyFighterMobilityMaxVelocityCommand, createFighterOperationalMotionCommand, applyFighterUtilityTeleportMotionCommand, applyFighterGotoPointCommand, captureFighterMaxVelocityPresentation, } = require(path.join(__dirname, "../../space/destiny/commands/fighter.js"));
...
const ATTRIBUTE_FIGHTER_SQUADRON_ORBIT_RANGE = getAttributeIDByNames("fighterSquadronOrbitRange") || 2223;
const ATTRIBUTE_SHIELD_RECHARGE_RATE = getAttributeIDByNames("shieldRechargeRate") || 479;
...
const PLAYER_COMPANION_INVALID_SCOPE_SENTINEL = "invalid-player-companion-scope";
function buildPlayerCompanionScopeMetadata(controllerEntity = null) {
    ...
    // dungeonCurrentInstanceID is the player ship's sole accepted site truth.
    // The child receives the matching presentation scope without introducing a
    // dungeonSiteInstanceID truth field on the controller.
    const dungeonSiteInstanceID = normalizePersistentEntityID(controllerEntity.dungeonCurrentInstanceID);
    if (dungeonSiteInstanceID !== null) {
        metadata.dungeonSiteInstanceID = toJSONSafeEntityID(dungeonSiteInstanceID);
    }
    if (getPlayerCompanionSecurityScope(controllerEntity, {
        controller: true,
    }).invalid === true) {
...
function getPlayerCompanionSecurityScope(entity = null, options = {}) {
    ...
    const isPlayerShipController = Boolean(isController &&
        entity &&
        entity.kind === "ship");
    ...
    if (entity && entity.kind === "ship") {
        ...
        hasPlayerDungeonTrackingContext = Boolean(hasPlayerDungeonTrackingContext ||
            String(entity.dungeonCurrentRoomKey || "").trim() ||
            Array.isArray(entity.dungeonCurrentRoomPosition));
    }
    if ((rawSiteInstanceID !== undefined &&
    ...
    const hasDungeonScopeMarker = Boolean(entity && (entity.dungeonMaterializedContainer === true ||
    ...
    const hasAirScopeMarker = Boolean(entity && (entity.airNpeBoardingShipPresentation === true ||
        entity.airNpeTutorialDebris === true ||
        entity.airNpeHostile === true ||
        String(entity.airNpeHostileWave || "").trim()));
    if ((hasDungeonScopeMarker && scope.dungeonSiteInstanceID === null) ||
        (hasAirScopeMarker &&
...
function playerCompanionTargetScopeIsCompatible(companionScope, targetScope, controllerEntity, targetEntity, session) {
    if (!companionScope ||
        !targetScope ||
    ...
    if ((companionDungeonInstanceID !== null || targetDungeonInstanceID !== null) &&
    ...
    if (companionOwnerCharacterID !== null &&
        ...
        return entityIDsEqual(companionOwnerCharacterID, targetOwnerCharacterID);
    }
    if (companionOwnerCharacterID !== null) {
        const targetActor = resolveExactCompanionActorCharacterID(targetEntity, targetEntity && targetEntity.session);
        if (targetActor.invalid) {
            return false;
        }
        if (targetActor.value !== null &&
    ...
    if (targetOwnerCharacterID !== null) {
        const controllerActor = resolveExactCompanionActorCharacterID(controllerEntity, session || (controllerEntity && controllerEntity.session));
        if (controllerActor.invalid ||
...
function hasPlayerCompanionSecurityScope(scope) {
    return Boolean(scope && (scope.invalid === true ||
        [
            "dungeonSiteInstanceID",
            ...PLAYER_COMPANION_AIR_SCOPE_ID_FIELDS,
        ].some((key) => scope[key] !== null)));
}
function canPlayerCompanionActOnTarget(scene, session, companionEntity, controllerEntity, targetEntity) {
    ...
    if (!securityScopesExactlyMatch(companionScope, controllerScope) ||
        !playerCompanionTargetScopeIsCompatible(companionScope, targetScope, controllerEntity, targetEntity, session)) {
        return false;
    }
    ...
    const isPlayerShipController = Boolean(controllerEntity.kind === "ship");
    const controllerSession = session || resolveFighterControllerSession(companionEntity, controllerEntity);
    const effectiveSession = controllerSession || (!isPlayerShipController && targetEntity.session) || null;
    if (!effectiveSession) {
        return (!isPlayerShipController &&
            !controllerScope.invalid);
    }
    try {
        return (visibilityGate.call(scene, effectiveSession, companionEntity) === true &&
...
function resolveControlledStructureRecord(session) {
    const structureID = toInt(session && (session.structureID || session.structureid), 0);
...
function syncInventoryItemForCharacterSession(session, item, previousData, options = {}) {
    ...
    return characterState.syncInventoryItemForSession(session, item, previousData, options);
}
...
function isStructureControllerRecord(controller) {
    return Boolean(controller && (controller.controlledStructure === true ||
...
function setFightersLaunchedTetherRestriction(characterID, fightersLaunched, scene) {
    ...
    try {
        ...
        const result = tetherRestrictions.setCharacterTetherRestrictionFlags(numericCharacterID, {
        ...
        log.warn(`[FighterRuntime] Failed to update launched-fighter tether restriction for char=${numericCharacterID}: ${error && error.message ? error.message : error}`);
        return false;
    }
}
function syncFightersLaunchedTetherRestriction(scene, controller, fallbackCharacterID = 0) {
    if (!scene || !controller || isStructureControllerRecord(controller)) {
        return false;
    }
    const controllerID = toInt(controller.itemID, 0);
    const characterID = resolveControllerOwnerCharacterID(controller, fallbackCharacterID);
    if (controllerID <= 0 || characterID <= 0) {
        return false;
    }
    const hasLaunchedFighters = listControlledFighterEntities(scene, controllerID)
        .some((entity) => (isFighterEntity(entity) &&
        toInt(entity.controllerID, 0) === controllerID));
    return setFightersLaunchedTetherRestriction(characterID, hasLaunchedFighters, scene);
}
...
function magnitude(vector) {
    ...
    return Math.sqrt((resolved.x * resolved.x) +
...
function surfaceDistance(leftEntity, rightEntity) {
    return Math.max(0, distance(leftEntity && leftEntity.position, rightEntity && rightEntity.position) -
...
function normalizeTubeFlagList(rawValue) {
    return listify(rawValue)
        .map((value) => toInt(value, 0))
        .filter((value, index, values) => (isFighterTubeFlag(value) &&
...
function buildPerFighterResultEntries(rawFighterIDs, errors = []) {
    ...
    for (const entry of Array.isArray(errors) ? errors : []) {
        ...
        errorsByFighterID.set(fighterID, entry.length > 1 ? entry[1] ?? null : null);
    }
...
function isMobilityAbilitySnapshot(abilityMeta) {
    return FIGHTER_MOBILITY_EFFECT_FAMILIES.has(normalizeEffectFamily(abilityMeta && (abilityMeta.normalizedEffectFamily ||
        abilityMeta.effectFamily)));
}
function isJammerAbilitySnapshot(abilityMeta) {
    return normalizeEffectFamily(abilityMeta && (abilityMeta.normalizedEffectFamily ||
        abilityMeta.effectFamily)) === "fighterabilityecm";
}
function resolveFighterRuntimeItemRecord(fighterItemOrEntity) {
    const sourceRecord = fighterItemOrEntity && typeof fighterItemOrEntity === "object"
        ? fighterItemOrEntity
...
function resolveFighterPassiveMaxVelocity(entity) {
    ...
    const resolved = Math.max(1, toNumber(attributes[ATTRIBUTE_MAX_VELOCITY] ?? getTypeAttributeValue(typeID, "maxVelocity"), entity && entity.maxVelocity));
    if (entity && typeof entity === "object") {
        applyFighterPassiveMaxVelocityCommand({
            fighterEntity: entity,
            baseMaxVelocity: resolved,
        });
    }
    return resolved;
}
function resolveFighterPassiveSignatureRadius(entity) {
    ...
    const typeID = toInt(itemRecord && itemRecord.typeID, toInt(entity && entity.typeID, 0));
    const attributes = buildEffectiveItemAttributeMap(itemRecord || typeID);
    const resolved = Math.max(1, toNumber(attributes[ATTRIBUTE_SIGNATURE_RADIUS] ?? getTypeAttributeValue(typeID, "signatureRadius"), entity && entity.signatureRadius));
    if (entity && typeof entity === "object") {
...
function emitFighterMaxVelocityUpdate(scene, fighterEntity, nowMs) {
    ...
    const presentation = captureFighterMaxVelocityPresentation({
        scene,
        fighterEntity,
        nowMs,
    });
    scene.broadcastMovementUpdates(presentation.buildUpdates());
    return true;
}
function syncFighterMobilityAbilityState(scene, fighterEntity, controllerEntity = null, nowMs = scene && typeof scene.getCurrentSimTimeMs === "function"
    ? scene.getCurrentSimTimeMs()
    : Date.now()) {
    if (!isFighterEntity(fighterEntity)) {
        return false;
    }
    const rawAbilityStates = fighterEntity && fighterEntity.fighterAbilityStates &&
    ...
    if (controllerEntity && hasAbilityState) {
        for (const slotID of ABILITY_SLOT_IDS) {
            ...
            const abilityMeta = resolveFighterAbilitySnapshot(fighterEntity, controllerEntity, slotID) || getFighterAbilityMetaForSlot(fighterEntity.typeID, slotID);
            if (!isMobilityAbilitySnapshot(abilityMeta)) {
                continue;
            }
            hasActiveMobilityEffect = true;
            const speedMultiplier = 1 + (Math.max(-99.999, toNumber(abilityMeta && abilityMeta.speedBonusPercent, 0)) / 100);
            const signatureMultiplier = 1 + (toNumber(abilityMeta && abilityMeta.signatureRadiusBonusPercent, 0) / 100);
    ...
    const maxVelocityChanged = Math.abs(toNumber(fighterEntity.maxVelocity, passiveMaxVelocity) - nextMaxVelocity) > 1e-6;
    const signatureRadiusChanged = Math.abs(toNumber(fighterEntity.signatureRadius, passiveSignatureRadius) - nextSignatureRadius) > 1e-6;
    applyFighterMobilityMaxVelocityCommand({
        fighterEntity,
        maxVelocity: nextMaxVelocity,
    });
...
function getAbilityStateChargeCount(abilityState, abilityMeta) {
    const rawExplicitCount = abilityState && Object.prototype.hasOwnProperty.call(abilityState, "remainingChargeCount")
        ? abilityState.remainingChargeCount
        : undefined;
    const explicitCount = rawExplicitCount === null || rawExplicitCount === undefined
        ? NaN
...
function buildFighterAbilitySlotStatesPayload(fighterEntity = null, session = null, scene = null, nowMs = scene && typeof scene.getCurrentSimTimeMs === "function"
    ? scene.getCurrentSimTimeMs()
    : Date.now()) {
    const chargeEntries = [];
    ...
    for (const slotID of ABILITY_SLOT_IDS) {
        const abilityState = getEntityAbilityState(fighterEntity, slotID);
        const abilityMeta = getFighterAbilityMetaForSlot(fighterEntity && fighterEntity.typeID, slotID);
        const remainingChargeCount = getAbilityStateChargeCount(abilityState, abilityMeta);
...
function buildFighterUserError(message, values = {}) {
    return buildUserErrorPayload(String(message || ""), values && typeof values === "object" ? values : {});
}
function getSceneSecurityClass(scene) {
    const security = Math.max(0, Math.min(1, toNumber(scene && scene.system && scene.system.security, 0)));
    if (security <= 0) {
        return 0;
    }
    if (security < 0.45) {
...
function getRequiredSafetyLevelForFighterAttack(scene, fighterEntity, targetEntity) {
    ...
    if (targetOwnerID > 0) {
        const nowMs = scene && typeof scene.getCurrentSimTimeMs === "function"
            ? scene.getCurrentSimTimeMs()
            : Date.now();
        const targetCrimewatch = crimewatchState &&
...
function buildAbilitySafetyRestrictionError(scene, fighterEntity, abilityMeta, targetEntity) {
    ...
    const attackerOwnerID = getCombatOwnerCharacterID(fighterEntity);
    const currentSafetyLevel = attackerOwnerID > 0 &&
        crimewatchState &&
    ...
    const requiredSafetyLevel = getRequiredSafetyLevelForFighterAttack(scene, fighterEntity, targetEntity);
...
function hydrateFighterEntityFromItem(entity, itemRecord = null) {
    ...
    const mass = Math.max(1, toNumber(attributes[ATTRIBUTE_MASS] ?? getTypeAttributeValue(typeID, "mass"), entity.mass || 1));
    const inertia = Math.max(0.05, toNumber(attributes[ATTRIBUTE_AGILITY] ?? getTypeAttributeValue(typeID, "agility"), entity.inertia || 0.1));
    const maxVelocity = Math.max(1, toNumber(attributes[ATTRIBUTE_MAX_VELOCITY] ?? getTypeAttributeValue(typeID, "maxVelocity"), entity.maxVelocity || 1));
    const signatureRadius = Math.max(1, toNumber(attributes[ATTRIBUTE_SIGNATURE_RADIUS] ?? getTypeAttributeValue(typeID, "signatureRadius"), entity.signatureRadius || 1));
    const fighterState = item && item.fighterState && typeof item.fighterState === "object"
        ? item.fighterState
        : null;
    entity.kind = "fighter";
    ...
    entity.controllerID = toInt(fighterState && fighterState.controllerID, entity.launcherID || entity.controllerID || 0) || null;
    entity.controllerOwnerID = toInt(fighterState && fighterState.controllerOwnerID, entity.ownerID);
    entity.tubeFlagID = toInt(fighterState && fighterState.tubeFlagID, entity.tubeFlagID) || null;
    entity.fighterAbilityStates = cloneAbilityStates(fighterState && fighterState.abilityStates);
    entity.squadronSize = resolveSquadronSize(item);
    entity.maxSquadronSize = Math.max(entity.squadronSize, toInt(getTypeAttributeValue(typeID, "fighterSquadronMaxSize"), 0));
...
function applyFighterOperationalEntityAttributes(fighterEntity, controllerEntity = null) {
    ...
    const previousPassiveMaxVelocity = toNumber(fighterEntity.baseMaxVelocity, previousMaxVelocity);
    const previousPassiveSignatureRadius = toNumber(fighterEntity.baseSignatureRadius, previousSignatureRadius);
    const mass = Math.max(1, toNumber(attributes[ATTRIBUTE_MASS], fighterEntity.mass || 1));
    const inertia = Math.max(0.05, toNumber(attributes[ATTRIBUTE_AGILITY], fighterEntity.inertia || 0.1));
    const maxVelocity = Math.max(1, toNumber(attributes[ATTRIBUTE_MAX_VELOCITY], fighterEntity.maxVelocity || 1));
    const signatureRadius = Math.max(1, toNumber(attributes[ATTRIBUTE_SIGNATURE_RADIUS], fighterEntity.signatureRadius || 1));
    const operationalMotionCommand = createFighterOperationalMotionCommand({
        fighterEntity,
        mass,
        inertia,
        maxVelocity,
        alignTime: inertia * Math.log(4),
        maxAccelerationTime: inertia,
        agilitySeconds: Math.max((mass * inertia) / 1000000, 0.05),
    });
    operationalMotionCommand.applyBaseState();
    fighterEntity.baseSignatureRadius = signatureRadius;
    if (fighterEntity.fighterMobilityRuntimeActive !== true) {
        operationalMotionCommand.applyLiveMaxVelocity();
        fighterEntity.signatureRadius = signatureRadius;
    }
    operationalMotionCommand.applyAgilityState();
    fighterEntity.shieldCapacity = Math.max(0, toNumber(attributes[ATTRIBUTE_SHIELD_CAPACITY], fighterEntity.shieldCapacity || 0));
    fighterEntity.shieldRechargeRate = Math.max(0, toNumber(attributes[ATTRIBUTE_SHIELD_RECHARGE_RATE], fighterEntity.shieldRechargeRate || 0));
    fighterEntity.armorHP = Math.max(0, toNumber(attributes[ATTRIBUTE_ARMOR_HP], fighterEntity.armorHP || 0));
    fighterEntity.structureHP = Math.max(0, toNumber(attributes[ATTRIBUTE_STRUCTURE_HP], fighterEntity.structureHP || 0));
    fighterEntity.passiveDerivedState = {
        ...(fighterEntity.passiveDerivedState || {}),
        attributes: { ...attributes },
    };
    return (Math.abs(previousPassiveMaxVelocity - maxVelocity) > 1e-6 ||
...
function persistFighterEntityState(entity) {
    ...
    if (!result.success) {
        log.warn(`[FighterRuntime] Failed to persist fighter ${entity.itemID}: ${result.errorMsg}`);
    }
...
function getShipStateForSession(session) {
    ...
    const shipRecord = resolveControlledStructureRecord(session) ||
        resolveActiveShipRecord(characterID);
    const systemID = toInt(session && session._space && session._space.systemID, toInt(shipRecord && shipRecord.spaceState && shipRecord.spaceState.systemID, 0));
...
function buildFightersInSpaceRowsForShip(scene, shipID) {
    return listControlledFighterEntities(scene, shipID)
        ...
        .sort((left, right) => (toInt(left && left.tubeFlagID, 0) - toInt(right && right.tubeFlagID, 0)))
        .map((entity) => [
...
function syncInventoryChangesToSession(session, changes = []) {
    ...
    for (const change of Array.isArray(changes) ? changes : []) {
        ...
        syncInventoryItemForCharacterSession(session, change.item, change.previousData || {}, { emitCfgLocation: true });
    }
}
function getShipTubeItem(shipRecord, tubeFlagID) {
    ...
    const contents = listContainerItems(toInt(shipRecord.ownerID, 0), shipRecord.itemID, tubeFlagID)
        .filter(Boolean)
...
function listShipTubeItems(shipRecord, options = {}) {
    ...
    for (const tubeFlagID of FIGHTER_TUBE_FLAGS) {
        for (const item of listContainerItems(toInt(shipRecord.ownerID, 0), shipRecord.itemID, tubeFlagID)) {
...
function notifyAbilityActivated(session, scene, fighterEntity, slotID, abilityState) {
    ...
    const payload = [
        ...
        abilityState && Number.isFinite(Number(abilityState.remainingChargeCount))
            ...
            : null,
        translateAbilityTimeForSession(scene, session, abilityState && abilityState.activeSinceMs),
...
function emitFighterAbilityActivationFx(scene, fighterEntity, slotID, abilityMeta, abilityState) {
    ...
    scene.broadcastSpecialFx(fighterEntity.itemID, abilityMeta.effectGuid, {
...
function resolveFighterControllerSession(fighterEntity, controllerEntity = null) {
    ...
    const controllerCharacterID = toInt(controllerEntity &&
        (controllerEntity.session &&
...
function buildFighterCombatSourceEntity(fighterEntity, controllerEntity = null) {
    ...
    const controllerSession = resolveFighterControllerSession(fighterEntity, controllerEntity);
    const controllerCharacterID = toInt(controllerEntity &&
        (controllerEntity.session &&
            controllerEntity.session.characterID) ||
        controllerEntity &&
    ...
    return {
        ...
        pilotCharacterID: controllerCharacterID || toInt(fighterEntity.pilotCharacterID, 0) || null,
    };
}
function getFighterOrbitDistance(fighterItemOrTypeID) {
    ...
    const attributes = buildEffectiveItemAttributeMap(itemRecord || typeID);
    return Math.max(DEFAULT_FIGHTER_ORBIT_DISTANCE_METERS, toNumber(attributes[ATTRIBUTE_FIGHTER_SQUADRON_ORBIT_RANGE] ??
        getTypeAttributeValue(typeID, "fighterSquadronOrbitRange"), DEFAULT_FIGHTER_ORBIT_DISTANCE_METERS));
}
function syncFighterOffensiveMovement(scene, fighterEntity, targetEntity, abilitySnapshot) {
    ...
    const desiredAttackRange = Math.max(preferredOrbitDistance, toNumber(abilitySnapshot.rangeMeters, 0));
    const chaseRange = Math.max(desiredAttackRange, desiredAttackRange + Math.max(0, toNumber(abilitySnapshot.falloffMeters, 0)));
    const currentSurfaceDistance = surfaceDistance(fighterEntity, targetEntity);
    if (currentSurfaceDistance > chaseRange + 1) {
        scene.followShipEntity(fighterEntity, targetEntity.itemID, desiredAttackRange, { broadcast: true });
        return false;
    }
    scene.orbitShipEntity(fighterEntity, targetEntity.itemID, preferredOrbitDistance, { broadcast: true });
...
function buildCombinedFighterDamageResult(passes, rawDamage, fighterEntity, destroyed) {
    ...
    for (const pass of passes) {
        ...
        for (const layerEntry of Array.isArray(resultData && resultData.perLayer)
            ...
            combinedPerLayer.set(layerName, combinedPerLayer.get(layerName) + toNumber(layerEntry && layerEntry.appliedEffective, 0));
        }
    }
    return {
        success: true,
        data: {
            ...
            beforeConditionState: passes.length > 0
                ? {
                    ...(passes[0].beforeConditionState || {}),
                }
                : normalizeShipConditionState(fighterEntity && fighterEntity.conditionState),
            afterConditionState: normalizeShipConditionState(fighterEntity && fighterEntity.conditionState),
...
function applyDamageToFighterSquadron(scene, fighterEntity, rawDamage, whenMs = null) {
    ...
    while (currentSquadronSize > 0 && sumDamageVector(remainingDamage) > 0) {
        const beforeConditionState = normalizeShipConditionState(fighterEntity.conditionState);
    ...
    return {
        damageResult: buildCombinedFighterDamageResult(passes, rawDamage, fighterEntity, currentSquadronSize <= 0),
        beforeSquadronSize,
        afterSquadronSize: currentSquadronSize,
...
function handleFighterDestroyed(scene, fighterEntity) {
    ...
    removeAllFighterEwarEffects(scene, fighterEntity, controllerEntity);
    ...
    if (ownerSession && toInt(fighterEntity.tubeFlagID, 0) > 0) {
        notified = notifyFighterRemoved(ownerSession, fighterEntity.itemID, fighterEntity.tubeFlagID);
    }
    const synced = controllerEntity
        ? syncFightersLaunchedTetherRestriction(scene, controllerEntity, controllerOwnerID)
        : false;
...
function executeFighterOffensiveCycle(scene, fighterEntity, controllerEntity, slotID, abilityState, nowMs) {
    const runtime = getRuntime();
    const droneInterop = runtime && runtime.droneInterop && typeof runtime.droneInterop === "object"
    ...
    const snapshot = resolveFighterAbilitySnapshot(fighterEntity, controllerEntity, slotID);
    ...
    if (!targetEntity ||
        ...
        !canPlayerCompanionActOnTarget(scene, null, fighterEntity, controllerEntity, targetEntity)) {
        return { continueActive: false, deactivate: true };
    }
    syncFighterOffensiveMovement(scene, fighterEntity, targetEntity, snapshot);
    const rangeGate = Math.max(0, toNumber(snapshot.rangeMeters, 0) + Math.max(0, toNumber(snapshot.falloffMeters, 0)));
    if (rangeGate > 0 && surfaceDistance(fighterEntity, targetEntity) > rangeGate + 1) {
        return { continueActive: true, deactivate: false };
    }
    const pseudoModuleItem = buildFighterPseudoModuleItem(fighterEntity, slotID);
    const combatSourceEntity = buildFighterCombatSourceEntity(fighterEntity, controllerEntity) || fighterEntity;
    let shotDamage = null;
    let hitQuality = 0;
    ...
    if (snapshot.effectGuid) {
        scene.broadcastSpecialFx(fighterEntity.itemID, snapshot.effectGuid, {
            moduleID: pseudoModuleItem.itemID,
            moduleTypeID: pseudoModuleItem.typeID,
    ...
    if (snapshot.offensiveKind === "turret") {
        ...
        if (shotResult && shotResult.hit === true) {
            weaponDamageResult = droneInterop.applyWeaponDamageToTarget(scene, fighterEntity, targetEntity, shotResult.shotDamage, nowMs);
        ...
        if (sumDamageVector(shotDamage) > 0) {
            weaponDamageResult = droneInterop.applyWeaponDamageToTarget(scene, fighterEntity, targetEntity, shotDamage, nowMs);
            damageResult = weaponDamageResult && weaponDamageResult.damageResult
                ? weaponDamageResult.damageResult
                : null;
            destroyResult = weaponDamageResult && weaponDamageResult.destroyResult
    ...
    const damageTargetEntity = weaponDamageResult && weaponDamageResult.impactTargetEntity || targetEntity;
    if (appliedDamageAmount > 0) {
        droneInterop.noteKillmailDamage(combatSourceEntity, damageTargetEntity, appliedDamageAmount, {
            whenMs: nowMs,
            weaponSnapshot: {
                ...snapshot,
                moduleTypeID: pseudoModuleItem.typeID,
    ...
    droneInterop.notifyWeaponDamageMessages(combatSourceEntity, damageTargetEntity, pseudoModuleItem, shotDamage, appliedDamageAmount, hitQuality, {
        suppress: Boolean(weaponDamageResult &&
...
function getFighterEwarConfig(abilityMeta) {
    return FIGHTER_EWAR_FAMILY_CONFIG[normalizeEffectFamily(abilityMeta && (abilityMeta.normalizedEffectFamily ||
        abilityMeta.effectFamily))] || null;
}
function buildFighterEwarEffectState(fighterEntity, targetEntity, slotID, snapshot, definition, strengthValue, optimalRangeMeters, falloffMeters) {
    const pseudoModuleItem = buildFighterPseudoModuleItem(fighterEntity, slotID);
    const isWarpPoint = definition.family === hostileModuleRuntime.HOSTILE_FAMILY_DISRUPT ||
        definition.family === hostileModuleRuntime.HOSTILE_FAMILY_SCRAM;
    return {
        moduleID: pseudoModuleItem.itemID,
        typeID: pseudoModuleItem.typeID,
        groupID: pseudoModuleItem.groupID,
        effectID: toInt(snapshot && snapshot.effectID, 0),
        ...
        hostileModifierOperation: definition.modifierOperation === undefined
            ? -1
            : definition.modifierOperation,
        hostileResistanceAttributeID: toInt(definition.resistanceAttributeID, 0),
        ...
        durationMs: Math.max(1, toNumber(snapshot && snapshot.durationMs, 1000)),
    };
}
function refreshFighterEwarTargetDerivedState(scene, targetEntity, aggregateChanged) {
    if (aggregateChanged === true &&
        targetEntity &&
        typeof scene.refreshShipEntityDerivedState === "function") {
        scene.refreshShipEntityDerivedState(targetEntity, {
            session: targetEntity.session || null,
            broadcast: true,
            notifyTargeting: true,
        });
    }
}
function removeFighterEwarEffect(scene, fighterEntity, controllerEntity, slotID, abilityState) {
    if (!scene || !fighterEntity || !abilityState) {
        return false;
    }
    const snapshot = resolveFighterAbilitySnapshot(fighterEntity, controllerEntity, slotID) || getFighterAbilityMetaForSlot(fighterEntity.typeID, slotID);
    const ewarConfig = getFighterEwarConfig(snapshot);
    if (!ewarConfig) {
        return false;
    }
    const targetEntity = scene.getEntityByID(toInt(abilityState.targetID, 0));
    if (!targetEntity) {
        return false;
    }
    const definition = hostileModuleRuntime.resolveHostileDefinition({
        name: normalizeEffectFamily(snapshot.effectFamily),
    });
    if (!definition) {
        return false;
    }
    const effectState = buildFighterEwarEffectState(fighterEntity, targetEntity, slotID, snapshot, definition, 0, 0, 0);
    const removeResult = hostileModuleRuntime.removeHostileModuleState({
        targetEntity,
        sourceEntity: fighterEntity,
        effectState,
    });
    refreshFighterEwarTargetDerivedState(scene, targetEntity, Boolean(removeResult &&
        removeResult.data &&
        removeResult.data.aggregateChanged));
    return Boolean(removeResult &&
        removeResult.success &&
        removeResult.data &&
        removeResult.data.removedRecord);
}
function removeAllFighterEwarEffects(scene, fighterEntity, controllerEntity) {
    let removedAny = false;
    for (const slotID of ABILITY_SLOT_IDS) {
        const abilityState = getEntityAbilityState(fighterEntity, slotID);
        removedAny = removeFighterEwarEffect(scene, fighterEntity, controllerEntity, slotID, abilityState) || removedAny;
    }
    return removedAny;
}
function executeFighterUtilityCycle(scene, fighterEntity, controllerEntity, slotID, abilityState, nowMs) {
    ...
    if (normalizeEffectFamily(snapshot.normalizedEffectFamily || snapshot.effectFamily) ===
        "fighterabilitymicrojumpdrive" &&
        abilityState &&
        abilityState.targetPoint) {
        const targetPoint = cloneVector(abilityState.targetPoint, fighterEntity.position);
        const jumpDirection = normalizeVector(subtractVectors(targetPoint, fighterEntity.position), fighterEntity.direction);
    ...
    if (normalizeEffectFamily(snapshot.normalizedEffectFamily || snapshot.effectFamily) ===
        ...
        const rangeGate = Math.max(0, toNumber(snapshot.jammerOptimalRangeMeters || snapshot.rangeMeters, 0) +
        ...
        const effectState = {
            ...
            jammerMaxRangeMeters: Math.max(0, toNumber(snapshot.jammerOptimalRangeMeters, toNumber(snapshot.rangeMeters, 0))),
            jammerFalloffMeters: Math.max(0, toNumber(snapshot.jammerFalloffMeters, toNumber(snapshot.falloffMeters, 0))),
            durationMs: Math.max(1, toNumber(snapshot.durationMs, 1000)),
            jamDurationMs: Math.max(1, toNumber(snapshot.jammerDurationMs, toNumber(snapshot.durationMs, 1000))),
            nextCycleAtMs: Math.max(0, toNumber(nowMs, Date.now())) + Math.max(1, toNumber(snapshot.durationMs, 1000)),
        };
        ...
        if (cycleResult.success &&
            ...
            runtime.applyJammerCyclePresentation(scene, fighterEntity, effectState, nowMs, cycleResult);
        }
        return {
    ...
    const effectFamily = normalizeEffectFamily(snapshot.normalizedEffectFamily || snapshot.effectFamily);
    const ewarConfig = getFighterEwarConfig(snapshot);
    if (ewarConfig) {
        const targetID = toInt(abilityState && abilityState.targetID, 0);
        const targetEntity = targetID > 0 ? scene.getEntityByID(targetID) : null;
        ...
        const optimalRangeMeters = Math.max(0, toNumber(getTypeAttributeValue(fighterTypeID, ewarConfig.optimalAttribute), toNumber(snapshot.rangeMeters, 0)));
        const falloffMeters = ewarConfig.falloffAttribute
            ...
            : 0;
        const strengthValue = toNumber(getTypeAttributeValue(fighterTypeID, ewarConfig.strengthAttribute), 0);
        const definition = hostileModuleRuntime.resolveHostileDefinition({
            name: effectFamily,
        });
        ...
        if (rangeGate > 0 &&
            surfaceDistance(fighterEntity, targetEntity) > rangeGate + 1) {
            return { continueActive: true, deactivate: false };
        }
        const pseudoModuleItem = buildFighterPseudoModuleItem(fighterEntity, slotID);
        const effectState = buildFighterEwarEffectState(fighterEntity, targetEntity, slotID, snapshot, definition, strengthValue, optimalRangeMeters, falloffMeters);
        if (snapshot.effectGuid) {
            scene.broadcastSpecialFx(fighterEntity.itemID, snapshot.effectGuid, {
                moduleID: pseudoModuleItem.itemID,
                moduleTypeID: pseudoModuleItem.typeID,
        ...
        const hostileCallbacks = {
            ...
            getEntitySurfaceDistance: (sourceEntity, otherEntity) => surfaceDistance(sourceEntity, otherEntity),
        };
        ...
        if (cycleResult && cycleResult.success) {
            refreshFighterEwarTargetDerivedState(scene, cycleResult.data && cycleResult.data.targetEntity, Boolean(cycleResult.data && cycleResult.data.aggregateChanged));
            return { continueActive: true, deactivate: false };
        }
        return { continueActive: false, deactivate: true };
    }
...
function buildFighterLaunchSpaceState(shipEntity, launchIndex = 0) {
    ...
    const launchDistance = Math.max(toNumber(shipEntity && shipEntity.radius, 0), 1) +
    ...
    const position = addVectors(addVectors(cloneVector(shipEntity && shipEntity.position), scaleVector(shipDirection, launchDistance)), scaleVector(lateralDirection, lateralOffset * signedSide));
    return {
...
function launchFightersFromTubes(session, rawTubeFlagIDs) {
    ...
    const tubeFlagIDs = normalizeTubeFlagList(rawTubeFlagIDs);
    if (getPlayerCompanionSecurityScope(shipEntity, {
        controller: true,
    }).invalid === true) {
        for (const tubeFlagID of tubeFlagIDs) {
            errors.push([
                tubeFlagID,
                buildFighterError("Unable to launch fighters while the controller's private-site scope is invalid."),
            ]);
        }
        return { success: false, errors };
    }
    ...
    for (const tubeFlagID of tubeFlagIDs) {
        ...
        if (!canLoadFighterTypeIntoHostTube(shipRecord.typeID, fighterItem.typeID, tubeFlagID, listShipTubeItems(shipRecord, { excludeItemID: fighterItem.itemID }))) {
            errors.push([tubeFlagID, buildFighterError("That fighter squadron cannot launch from that tube.")]);
            continue;
        }
        if (listControlledFighterEntities(scene, shipRecord.itemID).some((entity) => toInt(entity.tubeFlagID, 0) === tubeFlagID)) {
        ...
        const fighterEntity = hydrateFighterEntityFromItem(spawnResult.data.entity, updateResult.data);
        ...
        scene.orbitShipEntity(fighterEntity, shipRecord.itemID, DEFAULT_FIGHTER_ORBIT_DISTANCE_METERS, { broadcast: true });
        persistFighterEntityState(fighterEntity);
...
function landFighterToTube(scene, shipRecord, fighterEntity, fallbackSession = null) {
    ...
    const tubeFlagID = toInt(fighterEntity.tubeFlagID, toInt(fighterEntity.fighterRecallTubeFlagID, 0));
    ...
    removeAllFighterEwarEffects(scene, fighterEntity, scene.getEntityByID(toInt(shipRecord.itemID, 0)));
...
function recallFightersToTubes(session, rawFighterIDs) {
    ...
    for (const fighterID of fighterIDs) {
        ...
        for (const slotID of ABILITY_SLOT_IDS) {
            ...
            const abilityMeta = resolveFighterAbilitySnapshot(fighterEntity, controllerEntity, slotID) ||
                getFighterAbilityMetaForSlot(fighterEntity.typeID, slotID);
            if (isMobilityAbilitySnapshot(abilityMeta)) {
                continue;
            }
            removeFighterEwarEffect(scene, fighterEntity, controllerEntity, slotID, abilityState);
        ...
        scene.followShipEntity(fighterEntity, shipRecord.itemID, 0, { broadcast: true });
        syncFighterMobilityAbilityState(scene, fighterEntity, controllerEntity, scene.getCurrentSimTimeMs());
        persistFighterEntityState(fighterEntity);
...
function queueControlledFightersReturnToTubes(scene, controllerEntity, options = {}) {
    ...
    const ownerSession = Object.prototype.hasOwnProperty.call(options, "ownerSession")
    ...
    for (const fighterEntity of listControlledFighterEntities(scene, controllerID)) {
        ...
        scene.followShipEntity(fighterEntity, controllerID, 0, { broadcast: true });
        syncFighterMobilityAbilityState(scene, fighterEntity, controllerEntity, toNumber(options.nowMs, scene.getCurrentSimTimeMs && scene.getCurrentSimTimeMs()));
        persistFighterEntityState(fighterEntity);
        notifyTubeState(ownerSession, tubeFlagID, TUBE_STATE_RECALLING);
        queuedCount += 1;
    }
...
function validateAbilityActivation(scene, session, fighterEntity, controllerEntity, abilityMeta, rawTarget) {
    ...
    switch (abilityMeta.targetMode) {
        ...
        case TARGET_MODE_ITEMTARGETED: {
            ...
            if (!canPlayerCompanionActOnTarget(scene, session, fighterEntity, controllerEntity, targetEntity)) {
                return {
                    success: false,
                    error: buildAbilityError("That fighter cannot see the selected target."),
                };
            }
            const safetyRestrictionError = buildAbilitySafetyRestrictionError(scene, fighterEntity, abilityMeta, targetEntity);
...
function activateAbilitySlots(session, rawFighterIDs, rawAbilitySlotID, rawTarget = null) {
    ...
    for (const fighterID of fighterIDs) {
        ...
        const abilityMeta = resolveFighterAbilitySnapshot(fighterEntity, shipEntity, abilitySlotID) ||
        ...
        let existingState = refreshAbilityCharges(fighterEntity, abilitySlotID, abilityMeta, nowMs);
        ...
        const abilityChargeCount = getEffectiveAbilityChargeCount(abilityMeta);
        const remainingChargeCount = getAbilityStateChargeCount(existingState, abilityMeta);
        const cooldownIsActive = existingState &&
        ...
        const validation = validateAbilityActivation(scene, session, fighterEntity, shipEntity, abilityMeta, rawTarget);
        ...
        if (abilityChargeCount !== null) {
            nextState.remainingChargeCount = Math.max(0, (remainingChargeCount === null ? abilityChargeCount : remainingChargeCount) - 1);
            if (rearmTimeMs !== null) {
        ...
        emitFighterAbilityActivationFx(scene, fighterEntity, abilitySlotID, abilityMeta, activeAbilityState);
        if (isMobilityAbilitySnapshot(abilityMeta)) {
            syncFighterMobilityAbilityState(scene, fighterEntity, shipEntity, nowMs);
        }
        else if (getFighterEwarConfig(abilityMeta)) {
            const initialCycle = executeFighterUtilityCycle(scene, fighterEntity, shipEntity, abilitySlotID, activeAbilityState, nowMs);
            if (!initialCycle || initialCycle.deactivate === true) {
                removeFighterEwarEffect(scene, fighterEntity, shipEntity, abilitySlotID, activeAbilityState);
                clearAbilityActiveWindow(activeAbilityState);
                setEntityAbilityState(fighterEntity, abilitySlotID, activeAbilityState);
            }
        }
        else if (abilityMeta.isOffensive === true &&
            !isJammerAbilitySnapshot(abilityMeta) &&
            !usesContinuousAbilityCycle(abilityMeta)) {
            const immediateResult = executeFighterOffensiveCycle(scene, fighterEntity, shipEntity, abilitySlotID, activeAbilityState, nowMs);
        ...
        persistFighterEntityState(fighterEntity);
        queueAbilityActivated(ownerSession, scene, fighterEntity, abilitySlotID, activeAbilityState);
    }
    return {
        success: errors.length === 0,
        errors,
...
function deactivateAbilitySlots(session, rawFighterIDs, rawAbilitySlotID) {
    ...
    for (const fighterID of fighterIDs) {
        ...
        removeFighterEwarEffect(scene, fighterEntity, shipEntity, abilitySlotID, existingState);
...
function issueFighterGotoPoint(scene, fighterEntity, point) {
    ...
    const direction = normalizeVector(subtractVectors(normalizedPoint, fighterEntity.position), fighterEntity.direction);
    const now = scene.getCurrentSimTimeMs();
    const gotoPointCommand = applyFighterGotoPointCommand({
        fighterEntity,
        targetPoint: normalizedPoint,
        direction,
    });
    if (!gotoPointCommand.applied) {
        return false;
    }
    persistFighterEntityState(fighterEntity);
    scene.dispatchConfiguredSubwarpMovement(fighterEntity, (stamp) => gotoPointCommand.buildPresentationUpdates({ stamp }), now, {});
...
function executeMovementCommandOnFighters(session, rawFighterIDs, command, ...args) {
    ...
    for (const fighterID of fighterIDs) {
        ...
        const canceledRecallTubeFlagID = fighterEntity.fighterCommand === "RECALL_TUBE"
            ? toInt(fighterEntity.fighterRecallTubeFlagID, toInt(fighterEntity.tubeFlagID, 0))
            : 0;
        clearFighterCommandState(fighterEntity);
        ...
        switch (normalizedCommand) {
            case MOVEMENT_COMMAND_ORBIT: {
                ...
                const orbitDistance = Math.max(0, toNumber(args[1], DEFAULT_FIGHTER_ORBIT_DISTANCE_METERS));
...
function abandonFighterInSpace(scene, fighterEntity, options = {}) {
    ...
    const previousControllerEntity = options.controllerEntity ||
        (previousControllerID > 0 ? scene.getEntityByID(previousControllerID) : null);
    const ownerSession = Object.prototype.hasOwnProperty.call(options, "ownerSession")
        ? options.ownerSession
        : resolveFighterControllerSession(fighterEntity, previousControllerEntity);
    const tubeFlagID = toInt(fighterEntity.tubeFlagID, 0);
    ...
    removeAllFighterEwarEffects(scene, fighterEntity, previousControllerEntity);
    fighterEntity.controllerID = null;
    fighterEntity.controllerOwnerID = 0;
    fighterEntity.tubeFlagID = null;
    clearEntityAbilityStates(fighterEntity);
    clearFighterCommandState(fighterEntity);
    syncFighterMobilityAbilityState(scene, fighterEntity, null, toNumber(options.nowMs, scene.getCurrentSimTimeMs && scene.getCurrentSimTimeMs()));
    persistFighterEntityState(fighterEntity);
    ...
    if (previousControllerEntity) {
        syncFightersLaunchedTetherRestriction(scene, previousControllerEntity, previousControllerOwnerID);
    }
    return true;
}
function handleControllerLost(scene, controllerEntity, options = {}) {
    ...
    for (const fighterEntity of listControlledFighterEntities(scene, controllerID)) {
        if (options.attemptTubeRecovery === true &&
            shipRecord &&
...
function scoopAbandonedFighterFromSpace(session, rawFighterID, rawToFlagID) {
    ...
    if (toFlagID === ITEM_FLAGS.FIGHTER_BAY) {
        const mergeTarget = listContainerItems(toInt(shipRecord.ownerID, 0), shipRecord.itemID, ITEM_FLAGS.FIGHTER_BAY).find((item) => (toInt(item.itemID, 0) !== toInt(fighterEntity.itemID, 0) &&
...
function syncFighterRecallTravel(scene, fighterEntity, controllerEntity, recallDistance) {
    ...
    if (recallDistance > FIGHTER_RECALL_WARP_MINIMUM_DISTANCE_METERS &&
        typeof scene.warpDynamicEntityToPoint === "function") {
        const warpResult = scene.warpDynamicEntityToPoint(fighterEntity, cloneVector(controllerEntity.position), {
    ...
    if (mode !== "FOLLOW" ||
        toInt(fighterEntity.targetEntityID, 0) !== toInt(controllerEntity.itemID, 0)) {
        const followed = scene.followShipEntity(fighterEntity, controllerEntity.itemID, 0, { broadcast: true });
        if (!followed) {
...
function tickScene(scene, now) {
    const numericNow = toNumber(now, scene && typeof scene.getCurrentSimTimeMs === "function"
        ? scene.getCurrentSimTimeMs()
        : Date.now());
    for (const fighterEntity of getSceneFighterEntities(scene)) {
        ...
        if (controllerEntity) {
            ...
            const ownerSession = findSessionByCharacterID(toInt(fighterEntity.controllerOwnerID, 0)) ||
                resolveFighterControllerSession(fighterEntity, controllerEntity);
            syncFighterMobilityAbilityState(scene, fighterEntity, controllerEntity, numericNow);
            for (const slotID of ABILITY_SLOT_IDS) {
                ...
                const scopedTargetID = toInt(persistedAbilityState.targetID, 0);
                if (scopedTargetID > 0) {
                    const scopedTargetEntity = scene.getEntityByID(scopedTargetID);
                    if (!scopedTargetEntity ||
                        !canPlayerCompanionActOnTarget(scene, ownerSession, fighterEntity, controllerEntity, scopedTargetEntity)) {
                        removeFighterEwarEffect(scene, fighterEntity, controllerEntity, slotID, persistedAbilityState);
                        const revokedState = {
                            ...persistedAbilityState,
                        };
                        const hadActiveWindow = toNumber(revokedState.activeUntilMs, 0) > 0;
                        clearAbilityActiveWindow(revokedState);
                        setEntityAbilityState(fighterEntity, slotID, Object.keys(revokedState).length > 0 ? revokedState : null);
                        if (hadActiveWindow) {
                            notifyAbilityDeactivated(ownerSession, fighterEntity.itemID, slotID, null);
                        }
                        abilityStateChanged = true;
                ...
                let abilityState = refreshAbilityCharges(fighterEntity, slotID, abilityMeta, numericNow);
                ...
                if (toNumber(abilityState.activeUntilMs, 0) > 0) {
                    const cycleResult = abilityState.firedAtActivation === true
                        ? { continueActive: false, deactivate: true }
                        : getFighterEwarConfig(abilityMeta)
                            ? executeFighterUtilityCycle(scene, fighterEntity, controllerEntity, slotID, abilityState, numericNow)
                            : abilityMeta &&
                                abilityMeta.isOffensive &&
                                !isJammerAbilitySnapshot(abilityMeta)
                                ? executeFighterOffensiveCycle(scene, fighterEntity, controllerEntity, slotID, abilityState, numericNow)
                                : executeFighterUtilityCycle(scene, fighterEntity, controllerEntity, slotID, abilityState, numericNow);
                    const nextState = {
                        ...abilityState,
                    };
                    ...
                    setEntityAbilityState(fighterEntity, slotID, Object.keys(nextState).length > 0 ? nextState : null);
                    if (isMobilityAbilitySnapshot(abilityMeta)) {
                        syncFighterMobilityAbilityState(scene, fighterEntity, controllerEntity, numericNow);
                    }
                    abilityStateChanged = true;
        ...
        landFighterToTube(scene, shipRecord, fighterEntity, findSessionByCharacterID(toInt(shipRecord.ownerID, 0)));
    }
}
module.exports = {
    FIGHTER_CATEGORY_ID,
...
//# sourceMappingURL=fighterRuntime.js.map