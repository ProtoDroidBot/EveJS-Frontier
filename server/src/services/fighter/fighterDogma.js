...
const { getAttributeIDByNames, getEffectTypeRecord, getFittedModuleItems, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const { getCachedCharacterSkillMap, } = require(path.join(__dirname, "../skills/skillState"));
const { findItemById, findShipItemById, getItemMutationVersion, } = require(path.join(__dirname, "../inventory/itemStore"));
const { buildLocationModifiedAttributeMap, collectShipModifierAttributes, } = require(path.join(__dirname, "../../space/combat/weaponDogma"));
const { getLocationModifierSourcesForSystem, } = require(path.join(__dirname, "../exploration/wormholes/wormholeEnvironmentRuntime"));
const { getFighterAbilityMetaForSlot, } = require(path.join(__dirname, "./fighterAbilities"));
const { getActiveImplantLocationModifierSources, getActiveImplantSourceStates, getActiveImplantShipModifierEntries, } = require(path.join(__dirname, "../dogma/implants/activeImplantModifiers"));
...
const ATTRIBUTE_ATTACK_TURRET_DAMAGE_MULTIPLIER = getAttributeIDByNames("fighterAbilityAttackTurretDamageMultiplier") || 2178;
const ATTRIBUTE_ATTACK_TURRET_SIGNATURE_RESOLUTION = getAttributeIDByNames("fighterAbilityAttackTurretSignatureResolution") || 2179;
const ATTRIBUTE_ATTACK_TURRET_DAMAGE_EM = getAttributeIDByNames("fighterAbilityAttackTurretDamageEM") || 2171;
const ATTRIBUTE_ATTACK_TURRET_DAMAGE_THERMAL = getAttributeIDByNames("fighterAbilityAttackTurretDamageTherm") || 2172;
const ATTRIBUTE_ATTACK_TURRET_DAMAGE_KINETIC = getAttributeIDByNames("fighterAbilityAttackTurretDamageKin") || 2173;
const ATTRIBUTE_ATTACK_TURRET_DAMAGE_EXPLOSIVE = getAttributeIDByNames("fighterAbilityAttackTurretDamageExp") || 2174;
const ATTRIBUTE_ATTACK_MISSILE_DAMAGE_MULTIPLIER = getAttributeIDByNames("fighterAbilityAttackMissileDamageMultiplier") || 2226;
const ATTRIBUTE_ATTACK_MISSILE_EXPLOSION_RADIUS = getAttributeIDByNames("fighterAbilityAttackMissileExplosionRadius") || 2234;
const ATTRIBUTE_ATTACK_MISSILE_EXPLOSION_VELOCITY = getAttributeIDByNames("fighterAbilityAttackMissileExplosionVelocity") || 2235;
const ATTRIBUTE_ATTACK_MISSILE_REDUCTION_FACTOR = getAttributeIDByNames("fighterAbilityAttackMissileReductionFactor") || 2231;
const ATTRIBUTE_ATTACK_MISSILE_REDUCTION_SENSITIVITY = getAttributeIDByNames("fighterAbilityAttackMissileReductionSensitivity") || 2232;
const ATTRIBUTE_ATTACK_MISSILE_DAMAGE_EM = getAttributeIDByNames("fighterAbilityAttackMissileDamageEM") || 2227;
const ATTRIBUTE_ATTACK_MISSILE_DAMAGE_THERMAL = getAttributeIDByNames("fighterAbilityAttackMissileDamageTherm") || 2228;
const ATTRIBUTE_ATTACK_MISSILE_DAMAGE_KINETIC = getAttributeIDByNames("fighterAbilityAttackMissileDamageKin") || 2229;
const ATTRIBUTE_ATTACK_MISSILE_DAMAGE_EXPLOSIVE = getAttributeIDByNames("fighterAbilityAttackMissileDamageExp") || 2230;
const ATTRIBUTE_MISSILES_DAMAGE_MULTIPLIER = getAttributeIDByNames("fighterAbilityMissilesDamageMultiplier") || 2130;
const ATTRIBUTE_MISSILES_EXPLOSION_RADIUS = getAttributeIDByNames("fighterAbilityMissilesExplosionRadius") || 2125;
const ATTRIBUTE_MISSILES_EXPLOSION_VELOCITY = getAttributeIDByNames("fighterAbilityMissilesExplosionVelocity") || 2126;
const ATTRIBUTE_MISSILES_REDUCTION_FACTOR = getAttributeIDByNames("fighterAbilityMissilesDamageReductionFactor") || 2127;
const ATTRIBUTE_MISSILES_REDUCTION_SENSITIVITY = getAttributeIDByNames("fighterAbilityMissilesDamageReductionSensitivity") || 2128;
const ATTRIBUTE_MISSILES_DAMAGE_EM = getAttributeIDByNames("fighterAbilityMissilesEMDamage") || 2131;
const ATTRIBUTE_MISSILES_DAMAGE_THERMAL = getAttributeIDByNames("fighterAbilityMissilesThermDamage") || 2132;
const ATTRIBUTE_MISSILES_DAMAGE_KINETIC = getAttributeIDByNames("fighterAbilityMissilesKinDamage") || 2133;
const ATTRIBUTE_MISSILES_DAMAGE_EXPLOSIVE = getAttributeIDByNames("fighterAbilityMissilesExpDamage") || 2134;
const ATTRIBUTE_AFTERBURNER_SPEED_BONUS = getAttributeIDByNames("fighterAbilityAfterburnerSpeedBonus") || 2151;
const ATTRIBUTE_MICROWARPDRIVE_SPEED_BONUS = getAttributeIDByNames("fighterAbilityMicroWarpDriveSpeedBonus") || 2152;
const ATTRIBUTE_MICROWARPDRIVE_SIGNATURE_RADIUS_BONUS = getAttributeIDByNames("fighterAbilityMicroWarpDriveSignatureRadiusBonus") || 2153;
const ATTRIBUTE_MICROJUMPDRIVE_SIGNATURE_RADIUS_BONUS = getAttributeIDByNames("fighterAbilityMicroJumpDriveSignatureRadiusBonus") || 2156;
const ATTRIBUTE_EVASIVE_SPEED_BONUS = getAttributeIDByNames("fighterAbilityEvasiveManeuversSpeedBonus") || 2224;
const ATTRIBUTE_EVASIVE_SIGNATURE_RADIUS_BONUS = getAttributeIDByNames("fighterAbilityEvasiveManeuversSignatureRadiusBonus") || 2225;
const ATTRIBUTE_ECM_DURATION = getAttributeIDByNames("fighterAbilityECMDuration") || 2220;
const ATTRIBUTE_ECM_RANGE_OPTIMAL = getAttributeIDByNames("fighterAbilityECMRangeOptimal") || 2221;
const ATTRIBUTE_ECM_RANGE_FALLOFF = getAttributeIDByNames("fighterAbilityECMRangeFalloff") || 2222;
const ATTRIBUTE_ECM_STRENGTH_GRAVIMETRIC = getAttributeIDByNames("fighterAbilityECMStrengthGravimetric") || 2241;
const ATTRIBUTE_ECM_STRENGTH_LADAR = getAttributeIDByNames("fighterAbilityECMStrengthLadar") || 2242;
const ATTRIBUTE_ECM_STRENGTH_MAGNETOMETRIC = getAttributeIDByNames("fighterAbilityECMStrengthMagnetometric") || 2243;
const ATTRIBUTE_ECM_STRENGTH_RADAR = getAttributeIDByNames("fighterAbilityECMStrengthRadar") || 2244;
...
function resolveSquadronSize(itemOrEntity) {
    ...
    return Math.max(0, toInt(itemOrEntity.stacksize ?? itemOrEntity.quantity, 0));
}
function resolveFighterDogmaItem(fighterEntity) {
    ...
    const typeID = firstPositiveInt(fighterEntity.typeID, storedItem && storedItem.typeID);
    if (typeID <= 0) {
        return null;
    }
    const customInfo = fighterEntity.customInfo !== undefined &&
    ...
    return {
        ...
        itemID,
        typeID,
        groupID: firstPositiveInt(fighterEntity.groupID, storedItem && storedItem.groupID),
        categoryID: firstPositiveInt(fighterEntity.categoryID, storedItem && storedItem.categoryID),
        ownerID: firstPositiveInt(fighterEntity.ownerID, storedItem && storedItem.ownerID),
        locationID: firstPositiveInt(fighterEntity.locationID, storedItem && storedItem.locationID, fighterEntity.systemID),
        flagID: fighterEntity.flagID !== undefined && fighterEntity.flagID !== null
            ? toInt(fighterEntity.flagID, 0)
            : toInt(storedItem && storedItem.flagID, 0),
        singleton: fighterEntity.singleton !== undefined && fighterEntity.singleton !== null
            ? toInt(fighterEntity.singleton, 0)
            : toInt(storedItem && storedItem.singleton, 0),
        quantity: fighterEntity.quantity !== undefined && fighterEntity.quantity !== null
            ? fighterEntity.quantity
            : storedItem && storedItem.quantity !== undefined
                ? storedItem.quantity
                : squadronSize,
        stacksize: fighterEntity.stacksize !== undefined && fighterEntity.stacksize !== null
...
function buildFighterSnapshotCacheKey(fighterEntity, fighterItem = null) {
    ...
    const typeID = firstPositiveInt(resolvedItem && resolvedItem.typeID, fighterEntity && fighterEntity.typeID);
    const itemID = firstPositiveInt(resolvedItem && resolvedItem.itemID, fighterEntity && fighterEntity.itemID);
    const customInfo = String((resolvedItem && resolvedItem.customInfo) ||
...
function resolveSkillLevel(skillRecord) {
    return toInt(skillRecord && (skillRecord.effectiveSkillLevel ??
...
function buildControllerDogmaFingerprint(controllerEntity, fittedItems = [], skillMap = null) {
    ...
    const controllerOwnerID = toInt(controllerEntity &&
    ...
    const shipMutationVersion = shipID > 0 ? toInt(getItemMutationVersion(shipID), 0) : 0;
    ...
    const fittedMutationFingerprint = fittedItems
        ...
        .join("|");
    const activeEffectFingerprint = controllerEntity && controllerEntity.activeModuleEffects instanceof Map
        ? [...controllerEntity.activeModuleEffects.values()]
            .filter(Boolean)
            .map((effectState) => (`${toInt(effectState && effectState.moduleID, 0)}:` +
            `${toInt(effectState && effectState.effectID, 0)}:` +
            `${toInt(effectState && effectState.chargeTypeID, 0)}`))
            .sort()
            .join("|")
        : "";
    const activeImplantFingerprint = controllerOwnerID > 0 && !isStructureController
...
function isStructureControllerEntity(controllerEntity) {
    return Boolean(controllerEntity && (controllerEntity.kind === "structure" ||
...
function filterStructureControllerSkillMap(skillMap) {
    ...
    return new Map([...skillMap.entries()].filter(([, skillRecord]) => (STRUCTURE_CONTROLLER_SKILL_TYPE_IDS.has(toInt(skillRecord && skillRecord.typeID, 0)))));
}
function buildActiveModuleContexts(controllerEntity, fittedItems = [], characterID = 0) {
    ...
    return [...controllerEntity.activeModuleEffects.values()]
        .filter(Boolean)
        .map((effectState) => {
        const moduleItem = fittedItems.find((item) => (toInt(item && item.itemID, 0) === toInt(effectState && effectState.moduleID, 0) ||
            (toInt(effectState && effectState.moduleFlagID, 0) > 0 &&
                toInt(item && item.flagID, 0) === toInt(effectState && effectState.moduleFlagID, 0)))) || null;
        if (!moduleItem) {
            return null;
        }
        return {
...
function getControllerDogmaContext(controllerEntity) {
    ...
    const controllerOwnerID = toInt((controllerEntity &&
        ...
        (controllerEntity &&
            (controllerEntity.pilotCharacterID ??
                controllerEntity.characterID ??
                controllerEntity.ownerID)), 0);
    const shipItem = findShipItemById(controllerShipID) ||
        findItemById(controllerShipID) ||
        null;
    if (!shipItem) {
        return null;
    }
    const fittedItems = controllerOwnerID > 0
        ? getFittedModuleItems(controllerOwnerID, controllerShipID)
        : [];
    const isStructureController = isStructureControllerEntity(controllerEntity);
    const rawSkillMap = controllerOwnerID > 0
    ...
    const activeModuleContexts = buildActiveModuleContexts(controllerEntity, fittedItems, controllerOwnerID);
    const implantShipModifierEntries = controllerOwnerID > 0 && !isStructureController
        ? getActiveImplantShipModifierEntries(controllerOwnerID)
        : [];
    const shipModifierAttributes = collectShipModifierAttributes(shipItem, skillMap, activeModuleContexts, {
    ...
    const additionalLocationModifierSources = [
        ...
        ...(isStructureController
            ? []
            : getLocationModifierSourcesForSystem(controllerEntity && controllerEntity.systemID)),
    ];
    const nextCache = {
        fingerprint,
        shipItem,
...
function buildOperationalAttributes(fighterEntity, controllerEntity) {
    ...
    const attributes = buildLocationModifiedAttributeMap(fighterItem, context.shipItem, context.skillMap, context.shipModifierAttributes, context.fittedItems, context.activeModuleContexts, {
...
function sumDamageVector(damageVector) {
    return round6(Math.max(0, toFiniteNumber(damageVector.em, 0)) +
...
function classifyOffensiveKind(effectFamily) {
    ...
    if (normalizedFamily === "fighterabilityattackmissile" ||
...
function resolveFighterAbilitySnapshot(fighterEntity, controllerEntity, slotID) {
    ...
    const durationMs = abilityMeta.durationAttributeID
        ? Math.max(1, Math.round(toFiniteNumber(attributes[abilityMeta.durationAttributeID], abilityMeta.durationMs || 1)))
        : Math.max(1, toInt(abilityMeta.durationMs, 1));
    const rangeMeters = abilityMeta.rangeAttributeID
    ...
    const falloffMeters = abilityMeta.falloffAttributeID
    ...
    const trackingSpeed = abilityMeta.trackingSpeedAttributeID
    ...
    const speedBonusPercent = normalizedEffectFamily === "fighterabilityafterburner"
        ...
        : normalizedEffectFamily === "fighterabilitymicrowarpdrive"
            ...
            : normalizedEffectFamily === "fighterabilityevasivemaneuvers"
                ...
                : 0;
    const signatureRadiusBonusPercent = normalizedEffectFamily === "fighterabilitymicrowarpdrive"
        ...
        : normalizedEffectFamily === "fighterabilitymicrojumpdrive"
            ...
            : normalizedEffectFamily === "fighterabilityevasivemaneuvers"
                ...
                : 0;
    const jammerStrengthBySensorType = normalizedEffectFamily === "fighterabilityecm"
        ? Object.freeze({
    ...
    if (normalizedEffectFamily === "fighterabilityattackturret") {
        ...
        optimalSigRadius = Math.max(1, round6(toFiniteNumber(attributes[ATTRIBUTE_ATTACK_TURRET_SIGNATURE_RESOLUTION], 25)));
    }
    else if (normalizedEffectFamily === "fighterabilityattackmissile" ||
        ...
        explosionRadius = Math.max(1, round6(toFiniteNumber(attributes[ATTRIBUTE_ATTACK_MISSILE_EXPLOSION_RADIUS], 1)));
        explosionVelocity = Math.max(0.001, round6(toFiniteNumber(attributes[ATTRIBUTE_ATTACK_MISSILE_EXPLOSION_VELOCITY], 0.001)));
        damageReductionFactor = Math.max(0.000001, round6(toFiniteNumber(attributes[ATTRIBUTE_ATTACK_MISSILE_REDUCTION_FACTOR], 1)));
        damageReductionSensitivity = Math.max(0.000001, round6(toFiniteNumber(attributes[ATTRIBUTE_ATTACK_MISSILE_REDUCTION_SENSITIVITY], 5.5)));
    }
        ...
        explosionRadius = Math.max(1, round6(toFiniteNumber(attributes[ATTRIBUTE_MISSILES_EXPLOSION_RADIUS], 1)));
        explosionVelocity = Math.max(0.001, round6(toFiniteNumber(attributes[ATTRIBUTE_MISSILES_EXPLOSION_VELOCITY], 0.001)));
        damageReductionFactor = Math.max(0.000001, round6(toFiniteNumber(attributes[ATTRIBUTE_MISSILES_REDUCTION_FACTOR], 1)));
        damageReductionSensitivity = Math.max(0.000001, round6(toFiniteNumber(attributes[ATTRIBUTE_MISSILES_REDUCTION_SENSITIVITY], 5.5)));
    }
    const damageMultiplier = damageMultiplierAttributeID
        ? Math.max(0, round6(toFiniteNumber(attributes[damageMultiplierAttributeID], 1)))
        : 0;
    const perMemberDamage = damageAttributeIDs
    ...
    const snapshot = Object.freeze({
        ...
        durationMs,
        cooldownMs: abilityMeta.cooldownMs === null || abilityMeta.cooldownMs === undefined
            ? null
        ...
        chargeCount: abilityMeta.chargeCount === null || abilityMeta.chargeCount === undefined
            ? null
            : Math.max(0, toInt(abilityMeta.chargeCount, 0)),
        maxChargeCount: abilityMeta.chargeCount === null || abilityMeta.chargeCount === undefined
            ? null
            : Math.max(0, toInt(abilityMeta.chargeCount, 0)),
        rearmTimeMs: abilityMeta.rearmTimeMs === null || abilityMeta.rearmTimeMs === undefined
            ? null
        ...
        jammerDurationMs: normalizedEffectFamily === "fighterabilityecm"
            ? Math.max(1, round6(toFiniteNumber(attributes[ATTRIBUTE_ECM_DURATION], durationMs)))
            : 0,
        jammerOptimalRangeMeters: normalizedEffectFamily === "fighterabilityecm"
            ? Math.max(0, round6(toFiniteNumber(attributes[ATTRIBUTE_ECM_RANGE_OPTIMAL], rangeMeters)))
            : 0,
        jammerFalloffMeters: normalizedEffectFamily === "fighterabilityecm"
...
//# sourceMappingURL=fighterDogma.js.map