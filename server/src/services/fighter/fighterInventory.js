...
const { ITEM_FLAGS, FIGHTER_TUBE_FLAGS, } = require(path.join(__dirname, "../inventory/itemStore"));
...
function getFighterClassDefinitionForTypeID(typeID) {
    ...
    return FIGHTER_CLASS_DEFINITIONS.find((definition) => Number(getTypeAttributeValue(numericTypeID, definition.fighterAttribute)) > 0) || null;
}
...
function getFighterSquadronSizeForTypeID(typeID) {
    ...
    const maxSize = toPositiveInt(getTypeAttributeValue(numericTypeID, FIGHTER_SQUADRON_MAX_SIZE_ATTRIBUTE), 0);
    if (maxSize > 0) {
        return maxSize;
    }
    return toPositiveInt(getTypeAttributeValue(numericTypeID, FIGHTER_SQUADRON_SIZE_ATTRIBUTE), 0);
}
function getFighterSlotLimitForClass(hostTypeID, fighterClassID) {
    const definition = FIGHTER_CLASS_DEFINITIONS.find((candidate) => candidate.classID === fighterClassID);
    if (!definition) {
        return 0;
    }
...
function canLoadFighterTypeIntoHostTube(hostTypeID, fighterTypeID, tubeFlagID, occupiedTubeItems = []) {
...
function summarizePublishedTypesByCategory(categoryID) {
    ...
    const summary = [...groupsByID.values()].sort((left, right) => left.groupID - right.groupID);
...
//# sourceMappingURL=fighterInventory.js.map