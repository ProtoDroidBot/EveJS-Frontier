...
const { ITEM_FLAGS, FIGHTER_TUBE_FLAGS, findItemById, listContainerItems, moveItemToLocation, } = require(path.join(__dirname, "../inventory/itemStore"));
const { buildList, buildDict, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { canLoadFighterTypeIntoHostTube, getFighterSquadronSizeForTypeID, isFighterItemRecord, isFighterTubeFlag, buildInventorySquadronSize, } = require(path.join(__dirname, "./fighterInventory"));
const { activateAbilitySlots, buildAbilityStateEntriesForShip, buildFightersInSpaceRowsForShip, commandAbandonFighter, deactivateAbilitySlots, executeMovementCommandOnFighters, launchFightersFromTubes, recallFightersToTubes, scoopAbandonedFighterFromSpace, } = require(path.join(__dirname, "./fighterRuntime"));
...
class FighterMgrService extends BaseService {
    ...
    _getHostTubeItem(session, hostRecord, tubeFlagID) {
        ...
        const tubeContents = listContainerItems(ownerID, hostRecord.itemID, Number(tubeFlagID) || 0)
            .filter(Boolean)
            .sort((left, right) => Number(left.itemID || 0) - Number(right.itemID || 0));
        return tubeContents[0] || null;
    }
    _getShipTubeItem(session, tubeFlagID) {
        return this._getHostTubeItem(session, this._getFighterControllerHost(session), tubeFlagID);
    }
    ...
    _resolveLoadableFighterItem(session, hostRecord, fighterID) {
        ...
        if (Number(fighterItem.locationID) === Number(hostRecord.itemID) &&
    ...
    _canLoadFighterIntoTube(session, hostRecord, fighterItem, tubeFlagID) {
        ...
        return canLoadFighterTypeIntoHostTube(hostRecord.typeID, fighterItem.typeID, tubeFlagID, this._listHostTubeItems(session, hostRecord, {
    ...
    Handle_GetFightersForShip(args, session, kwargs) {
        ...
        const abilityStateEntries = scene
            ...
            : [];
        log.debug(`[FighterMgrService] GetFightersForShip ship=${hostRecord.itemID} tubes=${fightersInTubes.length} inSpace=${fightersInSpace.length}`);
        return [
    ...
    Handle_LoadFightersToTube(args, session, kwargs) {
        ...
        const hostRecord = this._getFighterControllerHost(session);
        const fighterItem = this._resolveLoadableFighterItem(session, hostRecord, fighterID);
        ...
        const moveResult = moveItemToLocation(fighterID, hostRecord.itemID, tubeFlagID, squadronSize > 0 ? squadronSize : null);
    ...
    Handle_UnloadTubeToFighterBay(args, session, kwargs) {
        ...
        const moveResult = moveItemToLocation(fighterItem.itemID, hostRecord.itemID, ITEM_FLAGS.FIGHTER_BAY);
        if (!moveResult.success) {
            return false;
        }
        this._emitInventoryChanges(session, moveResult.data && moveResult.data.changes);
    ...
    Handle_LaunchFightersFromTubes(args, session, kwargs) {
        void kwargs;
        const requestedTubeFlagIDs = Array.isArray(args) && args.length > 0 ? args[0] || [] : [];
    ...
    Handle_RecallFightersToTubes(args, session, kwargs) {
        void kwargs;
        const requestedFighterIDs = Array.isArray(args) && args.length > 0 ? args[0] || [] : [];
        const result = recallFightersToTubes(session, requestedFighterIDs);
        return buildDict(result && Array.isArray(result.entries)
            ? result.entries
    ...
    Handle_ExecuteMovementCommandOnFighters(args, session, kwargs) {
        void kwargs;
        executeMovementCommandOnFighters(session, Array.isArray(args) && args.length > 0 ? args[0] || [] : [], Array.isArray(args) && args.length > 1 ? args[1] : null, ...(Array.isArray(args) ? args.slice(2) : []));
        return null;
    }
    Handle_CmdActivateAbilitySlots(args, session, kwargs) {
        void kwargs;
        const result = activateAbilitySlots(session, Array.isArray(args) && args.length > 0 ? args[0] || [] : [], Array.isArray(args) && args.length > 1 ? args[1] : null, Array.isArray(args) && args.length > 2 ? args[2] : null);
        return buildDict(result && Array.isArray(result.entries)
            ? result.entries
            : result && Array.isArray(result.errors)
                ? result.errors
    ...
    Handle_CmdDeactivateAbilitySlots(args, session, kwargs) {
        void kwargs;
        const result = deactivateAbilitySlots(session, Array.isArray(args) && args.length > 0 ? args[0] || [] : [], Array.isArray(args) && args.length > 1 ? args[1] : null);
        return buildDict(result && Array.isArray(result.entries)
            ? result.entries
            : result && Array.isArray(result.errors)
                ? result.errors
    ...
    Handle_CmdAbandonFighter(args, session, kwargs) {
        void kwargs;
        return commandAbandonFighter(session, Array.isArray(args) && args.length > 0 ? args[0] : null);
    }
    Handle_CmdScoopAbandonedFighterFromSpace(args, session, kwargs) {
        void kwargs;
        return scoopAbandonedFighterFromSpace(session, Array.isArray(args) && args.length > 0 ? args[0] : null, Array.isArray(args) && args.length > 1 ? args[1] : ITEM_FLAGS.FIGHTER_BAY);
    }
}
module.exports = FighterMgrService;
//# sourceMappingURL=fighterMgrService.js.map