...
const { getTypeAttributeMap, getTypeEffectRecords, } = require(path.join(__dirname, "../fitting/liveFittingState"));
...
function matchEffectRecordForFamily(effectRecords, effectFamily) {
    ...
    for (const effectRecord of effectRecords) {
        ...
        if (aliases.includes(normalizedEffectName) ||
...
function buildAbilitySlotMeta(typeID, slotRecord) {
    ...
    const effectRecord = matchEffectRecordForFamily(effectRecords, abilityRecord.effectFamily);
    const durationAttributeID = toInt(effectRecord && effectRecord.durationAttributeID, toInt(abilityRecord.durationAttributeID, 0));
    const rangeAttributeID = toInt(effectRecord && effectRecord.rangeAttributeID, toInt(abilityRecord.rangeAttributeID, 0));
    const falloffAttributeID = toInt(effectRecord && effectRecord.falloffAttributeID, toInt(abilityRecord.falloffAttributeID, 0));
    const trackingSpeedAttributeID = toInt(effectRecord && effectRecord.trackingSpeedAttributeID, toInt(abilityRecord.trackingSpeedAttributeID, 0));
    ...
    const durationMs = durationAttributeID > 0
        ? Math.max(1, Math.round(toFiniteNumber(attributeMap[durationAttributeID], 1)))
        : null;
    return Object.freeze({
        slotID,
        abilityID,
        effectFamily: String(abilityRecord.effectFamily || ""),
        effectID: toInt(effectRecord && effectRecord.effectID, toInt(abilityRecord.dogmaEffectID, 0)),
        ...
        isOffensive: Boolean((effectRecord && effectRecord.isOffensive) ||
        ...
        cooldownMs: cooldownSeconds === null || !Number.isFinite(cooldownSeconds)
            ? null
            : Math.max(1, Math.round(cooldownSeconds * 1000)),
        rangeMeters: Math.max(0, rangeAttributeID > 0
            ? toFiniteNumber(attributeMap[rangeAttributeID], 0)
            : 0),
        rangeAttributeID: rangeAttributeID > 0 ? rangeAttributeID : null,
        falloffMeters: Math.max(0, falloffAttributeID > 0
            ? toFiniteNumber(attributeMap[falloffAttributeID], 0)
            : 0),
        falloffAttributeID: falloffAttributeID > 0 ? falloffAttributeID : null,
        trackingSpeed: Math.max(0, trackingSpeedAttributeID > 0
            ? toFiniteNumber(attributeMap[trackingSpeedAttributeID], 0)
            : 0),
        trackingSpeedAttributeID: trackingSpeedAttributeID > 0 ? trackingSpeedAttributeID : null,
        chargeCount,
        rearmTimeMs: rearmTimeSeconds === null || !Number.isFinite(rearmTimeSeconds)
            ? null
...
function getFighterAbilitySlots(typeID) {
    ...
    const slotMap = ABILITY_SLOT_IDS.map((slotID) => {
        const slotRecord = typeAuthority &&
            Array.isArray(typeAuthority.abilitySlots)
            ? typeAuthority.abilitySlots.find((candidate) => toInt(candidate && candidate.slotID, -1) === slotID) || null
            : null;
...
//# sourceMappingURL=fighterAbilities.js.map