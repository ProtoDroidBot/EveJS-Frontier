...
function normalizeStoreConfig(rawConfig = {}) {
    ...
    const normalized = {
        enabled: config.newEdenStoreEnabled !== undefined
            ? Boolean(config.newEdenStoreEnabled)
            : source.enabled !== false,
        fastCheckoutEnabled: config.newEdenStoreFastCheckoutEnabled !== undefined
            ? Boolean(config.newEdenStoreFastCheckoutEnabled)
            : source.fastCheckoutEnabled !== false,
        fakeCashPurchasesEnabled: config.newEdenStoreFakeCashPurchasesEnabled !== undefined
            ? Boolean(config.newEdenStoreFakeCashPurchasesEnabled)
            : source.fakeCashPurchasesEnabled !== false,
        fakeFastCheckoutResponse: typeof config.newEdenStoreFakeFastCheckoutResponse === "string" &&
            ...
            : typeof source.fakeFastCheckoutResponse === "string" &&
                ...
                ? source.fakeFastCheckoutResponse.trim()
                : "OK",
        fakeChinaFunnelEnabled: config.newEdenStoreFakeChinaFunnelEnabled !== undefined
            ? Boolean(config.newEdenStoreFakeChinaFunnelEnabled)
            : Boolean(source.fakeChinaFunnelEnabled),
        fakeBuyPlexOfferUrl: typeof config.newEdenStoreFakeBuyPlexOfferUrl === "string"
            ? config.newEdenStoreFakeBuyPlexOfferUrl
            : typeof source.fakeBuyPlexOfferUrl === "string"
                ? source.fakeBuyPlexOfferUrl
                : "",
        useShellExecuteToBuyPlexOffer: config.newEdenStoreUseShellExecuteToBuyPlexOffer !== undefined
            ? Boolean(config.newEdenStoreUseShellExecuteToBuyPlexOffer)
            : source.useShellExecuteToBuyPlexOffer !== false,
        centsPerPlex: Math.max(1, Math.trunc(toNumber(config.newEdenStoreCentsPerPlex !== undefined
            ? config.newEdenStoreCentsPerPlex
            : source.centsPerPlex, 100))),
        defaultCashTaxRatePoints: Math.max(0, Math.trunc(toNumber(config.newEdenStoreDefaultCashTaxRatePoints !== undefined
            ? config.newEdenStoreDefaultCashTaxRatePoints
            : source.defaultCashTaxRatePoints, 0))),
        editorPort: Math.max(1, Math.trunc(toNumber(config.newEdenStoreEditorPort !== undefined
            ? config.newEdenStoreEditorPort
            : source.editorPort, 26008))),
        purchaseLogLimit: Math.max(10, Math.trunc(toNumber(config.newEdenStorePurchaseLogLimit !== undefined
            ? config.newEdenStorePurchaseLogLimit
            : source.purchaseLogLimit, DEFAULT_PURCHASE_LOG_LIMIT))),
    };
    return normalized;
}
...
function normalizeRedeemTokens(rawValue) {
    return ensureArray(rawValue)
        ...
        return {
            ...
            dateTime: typeof source.dateTime === "string" && source.dateTime.trim() !== ""
                ? source.dateTime.trim()
                : "0",
            expireDateTime: typeof source.expireDateTime === "string" && source.expireDateTime.trim() !== ""
...
function normalizeRuntimeAccount(rawValue) {
    const source = ensureObject(rawValue);
    return {
        omegaExpiryFileTime: typeof source.omegaExpiryFileTime === "string" &&
            source.omegaExpiryFileTime.trim() !== ""
            ? source.omegaExpiryFileTime.trim()
            : null,
        multiCharacterTrainingSlots: normalizeTrainingSlots(source.multiCharacterTrainingSlots),
        redeemTokens: normalizeRedeemTokens(source.redeemTokens),
        dailyLoginCampaign: normalizeDailyLoginCampaign(source.dailyLoginCampaign),
        loginMilestone: normalizeLoginMilestone(source.loginMilestone),
        updatedAt: typeof source.updatedAt === "string" && source.updatedAt.trim() !== ""
            ? source.updatedAt.trim()
            : null,
    };
}
function normalizeDailyLoginCampaign(rawValue) {
    const source = ensureObject(rawValue);
    const day = Math.trunc(toNumber(source.day, 0));
    return {
        day: day >= 0 && day <= 15 ? day : 0,
        lastClaimReset: typeof source.lastClaimReset === "string" ? source.lastClaimReset : "",
    };
}
function normalizeLoginMilestone(rawValue) {
    const source = ensureObject(rawValue);
    return {
        onlineMilliseconds: Math.max(0, Math.trunc(toNumber(source.onlineMilliseconds, 0))),
...
function loadStoreState() {
    ...
    for (const [accountID, runtimeAccount] of Object.entries(runtimeAccounts)) {
        ...
        runtimeAccountsByID.set(String(normalizedAccountID), normalizeRuntimeAccount(runtimeAccount));
    }
    cachedState = {
        authority,
        runtime,
...
function findLegacyOffer(storeID, offerID) {
    const state = loadStoreState();
    const offersByID = state.indexes.legacyOffersByStore.get(String(toPositiveInteger(storeID, 4)));
    if (!offersByID) {
        return null;
    }
...
function findLegacyOfferByStoreOfferID(storeID, storeOfferID) {
    const state = loadStoreState();
    const offersByStoreOfferID = state.indexes.legacyOffersByStoreOfferID.get(String(toPositiveInteger(storeID, 4)));
    if (!offersByStoreOfferID) {
        return null;
    }
    return cloneValue(offersByStoreOfferID.get(String(storeOfferID || "").trim()) || null);
}
function findPublicOffer(storeOfferID) {
    const state = loadStoreState();
    return cloneValue(state.indexes.publicOffersByID.get(String(storeOfferID || "").trim()) || null);
}
...
function findFastCheckoutOffer(offerID) {
    const state = loadStoreState();
    return cloneValue(state.indexes.fastCheckoutOffersByID.get(String(toPositiveInteger(offerID, 0))) || null);
}
...
function findQuickPayToken(tokenID) {
    const state = loadStoreState();
    return cloneValue(state.indexes.quickPayTokensByID.get(String(toPositiveInteger(tokenID, 0))) ||
        null);
}
...
function resolveCharacterAccountID(characterID) {
    ...
    const cachedAccountID = state.indexes.characterAccountByID.get(String(normalizedCharacterID));
    if (cachedAccountID) {
        return cachedAccountID;
    }
    resetStoreCaches();
    state = loadStoreState();
    return state.indexes.characterAccountByID.get(String(normalizedCharacterID)) || 0;
}
function resolveStoreCharacterID(characterID, accountID = 0) {
    ...
    if (explicitCharacterID) {
        resetStoreCaches();
        state = loadStoreState();
        if (state.indexes.characterAccountByID.has(String(explicitCharacterID))) {
            return explicitCharacterID;
        }
    }
    const normalizedAccountID = toPositiveInteger(accountID, 0);
    ...
    const characterIDs = state.indexes.accountCharacterIDs.get(String(normalizedAccountID)) || [];
    return toPositiveInteger(characterIDs[0], 0);
}
function readRuntimeAccount(accountID) {
    const state = loadStoreState();
    return cloneValue(state.indexes.runtimeAccountsByID.get(String(toPositiveInteger(accountID, 0))) ||
...
function grantOmegaDaysToAccount(accountID, durationDays) {
    ...
    const runtimeAccount = readRuntimeAccount(numericAccountID);
    const expiryFileTime = computeFutureFileTime(durationDays, runtimeAccount.omegaExpiryFileTime);
...
function getTrainingSlotsForAccount(accountID) {
    ...
    const runtimeAccount = readRuntimeAccount(numericAccountID);
    const runtimeSlots = normalizeTrainingSlots(runtimeAccount && runtimeAccount.multiCharacterTrainingSlots);
    const nextSlots = {
        ...baseSlots,
    };
...
function getCompletedPurchase(purchaseKey) {
    const state = loadStoreState();
    return cloneValue(ensureObject(state.runtime.completedPurchases)[String(purchaseKey || "").trim()] ||
        null);
}
