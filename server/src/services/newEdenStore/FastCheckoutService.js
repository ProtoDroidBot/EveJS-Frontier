...
const { getKwarg, marshalValueToJs, toMarshalValue, } = require("./storeMarshal");
const { getFastCheckoutOffers, getStoreConfig, } = require("./storeState");
const { purchaseFastCheckoutOffer, } = require("./storeFulfillment");
...
function buildFastCheckoutOfferKeyVal(rawOffer = {}) {
    ...
    const entries = [
        ...
        [
            "tags",
            buildList(Array.isArray(rawOffer.tags)
...
class FastCheckoutService extends BaseService {
    ...
    Handle_BuyOffer(args, session, kwargs) {
        ...
        if (!result.success) {
            throwWrappedUserError(result.errorMsg || "PurchaseFailed", {});
        }
        return toMarshalValue(result.data);
    }
}
module.exports = FastCheckoutService;
//# sourceMappingURL=FastCheckoutService.js.map