...
function buildPurchaseKey(parts: any[] = []) {
  return parts
...
function grantFulfillmentToCharacter(characterID, fulfillment, quantity, options: Record<string, any> = {}) {
  const characterRecord = getCharacterRecord(characterID);
  if (!characterRecord) {
    return {
      success: false as const,
...
function finalizePurchase(purchaseKey, payload: Record<string, any> = {}) {
