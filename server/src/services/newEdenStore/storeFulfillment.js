...
const { adjustCharacterPlexBalance, getCharacterWallet, } = require(path.join(__dirname, "../account/walletState"));
const { PLEX_LOG_CATEGORY, } = require(path.join(__dirname, "../account/plexVaultLogState"));
const { getCharacterRecord, giveItemToHangarForSession, resolveHomeStationInfo, updateCharacterRecord, } = require(path.join(__dirname, "../character/characterState"));
const { grantItemToCharacterStationHangar, } = require(path.join(__dirname, "../inventory/itemStore"));
const { findFastCheckoutOffer, findLegacyOffer, findPublicOffer, getCompletedPurchase, getStoreConfig, grantMctDaysToAccount, grantOmegaDaysToAccount, markPurchaseCompleted, appendPurchaseLog, resolveCharacterAccountID, resolveStoreCharacterID, } = require("./storeState");
...
function resolveLegacyPurchaseCharacters({ payerCharacterID = 0, targetCharacterID = 0, accountID = 0, session = null, } = {}) {
    const normalizedAccountID = toPositiveInteger(accountID, toPositiveInteger(session && (session.userid || session.userID), 0));
    const sessionCharacterID = toPositiveInteger(session && (session.characterID || session.charid), 0);
    const resolvedPayerCharacterID = resolveStoreCharacterID(payerCharacterID || sessionCharacterID, normalizedAccountID);
    const resolvedTargetCharacterID = resolveStoreCharacterID(targetCharacterID || resolvedPayerCharacterID, normalizedAccountID) || resolvedPayerCharacterID;
    return {
...
function grantItemFulfillment(characterID, typeID, quantity) {
    ...
    if (liveSession) {
        const liveResult = giveItemToHangarForSession(liveSession, { typeID: toPositiveInteger(typeID, 0) }, quantity);
        if (liveResult.success) {
            return liveResult;
        }
    }
    return grantItemToCharacterStationHangar(characterID, resolveDeliveryLocationID(characterID), { typeID: toPositiveInteger(typeID, 0) }, quantity);
}
function grantSkillPointsFulfillment(characterID, points) {
    ...
    const result = updateCharacterRecord(characterID, (record) => ({
        ...record,
        freeSkillPoints: Math.max(0, Math.trunc(toNumber(record && record.freeSkillPoints, 0) + normalizedPoints)),
    }));
    if (!result.success) {
        return result;
    }
...
function buildReceiptPayload(offer, quantity, paymentMethod, paymentIdentifier) {
    ...
    const plexPriceInCents = Math.max(0, Math.trunc(toNumber(publicOffer.plexPriceInCents, 0) * itemQuantity));
    const currencyAmountInCents = Math.max(0, Math.trunc(toNumber(publicOffer.currencyAmountInCents, 0) * itemQuantity));
...
function grantFulfillmentToCharacter(characterID, fulfillment, quantity, options = {}) {
    ...
    switch (String(fulfillmentSource.kind || "")) {
        case "grant_plex": {
            const plexAmount = Math.max(0, Math.trunc(toNumber(fulfillmentSource.plexAmount, 0) * multiplier));
            const result = creditCharacterPlex(characterID, plexAmount, options.reason || `Fake cash purchase: ${plexAmount} PLEX`);
            if (!result.success) {
                return result;
            }
            return {
        ...
        case "omega": {
            ...
            const durationDays = Math.max(0, Math.trunc(toNumber(fulfillmentSource.durationDays, 0) * multiplier));
        ...
        case "mct": {
            ...
            const durationDays = Math.max(0, Math.trunc(toNumber(fulfillmentSource.durationDays, 0)));
            const slotCount = Math.max(1, Math.trunc(toNumber(fulfillmentSource.slotCount, 1) * multiplier));
        ...
        case "item": {
            ...
            const itemQuantity = Math.max(1, Math.trunc(toNumber(fulfillmentSource.quantity, 1) * multiplier));
            if (!typeID) {
                return {
                    success: false,
        ...
        case "skill_points": {
            const points = Math.max(0, Math.trunc(toNumber(fulfillmentSource.points, 0) * multiplier));
        ...
        case "bundle": {
            ...
            for (const grant of grants) {
                const grantResult = grantFulfillmentToCharacter(characterID, grant, multiplier, options);
...
function purchaseLegacyOffer({ storeID = 4, offerID, currency = "PLX", quantity = 1, characterID, targetCharacterID = 0, accountID = 0, session = null, }) {
    ...
    const pricing = Array.isArray(legacyOffer.offerPricings)
        ? legacyOffer.offerPricings.find((entry) => String(entry && entry.currency || "").trim().toUpperCase() === normalizedCurrency)
        : null;
    ...
    const totalPlexCost = Math.max(0, Math.trunc(toNumber(pricing.price, 0) * normalizedQuantity));
    ...
    const debitResult = debitCharacterPlex(payerCharacterID, totalPlexCost, `New Eden Store purchase: ${legacyOffer.name} x${normalizedQuantity}`);
    if (!debitResult.success) {
        return debitResult;
    }
    const grantResult = grantFulfillmentToCharacter(fulfillmentCharacterID, legacyOffer.fulfillment, normalizedQuantity, {
        reason: `New Eden Store purchase: ${legacyOffer.name}`,
    });
    if (!grantResult.success) {
        creditCharacterPlex(payerCharacterID, totalPlexCost, `Rollback for failed New Eden Store purchase: ${legacyOffer.name}`);
        return grantResult;
    }
    const result = {
        success: true,
...
function purchaseFastCheckoutOffer({ offerID, characterID, purchaseTraceID = "", journeyID = "", }) {
    const configState = getStoreConfig();
    ...
    const requestScope = String(purchaseTraceID || "").trim() || String(journeyID || "").trim()
    ...
    const grantResult = grantFulfillmentToCharacter(characterID, publicOffer.fulfillment, 1, {
...
function purchasePublicCashOffer({ storeOfferID, characterID, quantity = 1, paymentMethod = 11, purchaseKeySuffix = "", }) {
    const configState = getStoreConfig();
    ...
    const grantResult = grantFulfillmentToCharacter(characterID, publicOffer.fulfillment, normalizedQuantity, {
        reason: `Fake cash purchase: ${publicOffer.name}`,
    });
    if (!grantResult.success) {
        return grantResult;
    }
    const receipt = buildReceiptPayload(publicOffer, normalizedQuantity, paymentMethod, Date.now());
    const result = {
        receipt,
        fulfillment: cloneValue(grantResult.data),
    };
...
function purchasePlexVaultOffer({ storeOfferID, characterID, quantity = 1, purchaseKeySuffix = "", }) {
    const configState = getStoreConfig();
    if (!configState.enabled) {
        return {
            success: false,
    ...
    const normalizedQuantity = Math.max(1, Math.trunc(toNumber(quantity, 1)));
    const totalPlexCost = Math.max(0, Math.trunc(toNumber(publicOffer.plexPriceInCents, 0) * normalizedQuantity / configState.centsPerPlex));
    ...
    const debitResult = debitCharacterPlex(characterID, totalPlexCost, `PLEX Vault purchase: ${publicOffer.name} x${normalizedQuantity}`);
    if (!debitResult.success) {
        return debitResult;
    }
    const grantResult = grantFulfillmentToCharacter(characterID, publicOffer.fulfillment, normalizedQuantity, {
    ...
    if (!grantResult.success) {
        creditCharacterPlex(characterID, totalPlexCost, `Rollback for failed PLEX Vault purchase: ${publicOffer.name}`);
        return grantResult;
    }
    const result = {
        balance: debitResult.data.plexBalance,
...
//# sourceMappingURL=storeFulfillment.js.map