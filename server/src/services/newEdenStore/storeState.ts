...
function normalizeStoreConfig(rawConfig: Record<string, any> = {}) {
...
function normalizeTrainingSlots(rawValue) {
  ...
  for (const [slotKey, expiryValue] of Object.entries<any>(source)) {
...
function normalizeLoginMilestone(rawValue) {
  const source = ensureObject(rawValue);
  return {
    onlineMilliseconds: Math.max(
      0,
      Math.trunc(toNumber(source.onlineMilliseconds, 0)),
    ),
...
function buildFastCheckoutOffersConfigString(fastCheckoutOffers: any[] = []) {
...
function loadStoreState() {
  ...
  for (const [storeID, rawStore] of Object.entries<any>(stores)) {
  ...
  for (const [storeOfferID, offer] of Object.entries<any>(publicOffers)) {
  ...
  for (const [tokenID, token] of Object.entries<any>(quickPayTokens)) {
  ...
  for (const [characterID, character] of Object.entries<any>(characters)) {
  ...
  for (const [accountID, runtimeAccount] of Object.entries<any>(runtimeAccounts)) {
...
function resolveCharacterAccountID(characterID) {
  ...
  const cachedAccountID = state.indexes.characterAccountByID.get(
...
function getNextMctSlotKey(trainingSlots) {
  const slotEntries = Object.entries<any>(trainingSlots)
...
function appendPurchaseLog(entry: Record<string, any> = {}) {
  const state = loadStoreState();
...
function markPurchaseCompleted(purchaseKey, payload: Record<string, any> = {}) {
...
function saveEditorAuthority(nextAuthority, options: Record<string, any> = {}) {
