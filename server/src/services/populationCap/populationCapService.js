...
function resolveCharacterSolarSystemID(charId, session) {
    ...
    const characterRecord = characterID
        ...
        : null;
    return normalizePositiveInteger(characterRecord && (characterRecord.solarSystemID ||
        characterRecord.solarsystemid ||
        characterRecord.solarSystemId), normalizePositiveInteger(session && (session.solarsystemid2 || session.solarsystemid), 30000142));
}
function buildStableLoadSlotToken(charId, solarSystemID) {
    ...
    const variantByte = ((Number.parseInt(hash.slice(16, 18), 16) & 0x3f) |
        0x80).toString(16).padStart(2, "0");
    return [
        hash.slice(0, 8),
        hash.slice(8, 12),
        `4${hash.slice(13, 16)}`,
...
//# sourceMappingURL=populationCapService.js.map