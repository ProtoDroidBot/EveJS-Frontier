...
const { buildList, buildObjectEx1, normalizeNumber, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
function normalizeLandingPad(rawLandingPad = {}) {
    ...
    const solarSystemID = toInt(rawLandingPad.solarSystemID ||
        rawLandingPad.solarsystemID ||
        rawLandingPad._solarSystemID, 0);
    const factionID = normalizeFactionID(rawLandingPad.factionID || rawLandingPad._factionID);
    ...
    return {
        ...
        linkTimestamp: rawLandingPad.linkTimestamp ??
            rawLandingPad._linkTimestamp ??
            null,
        isDisrupted: normalizeBoolean(rawLandingPad.isDisrupted ?? rawLandingPad._isDisrupted, false),
    };
}
...
function listLandingPads(factionID = null) {
    ...
    return [...landingPadsByItemID.values()]
        .filter((landingPad) => !normalizedFactionID || landingPad.factionID === normalizedFactionID)
...
//# sourceMappingURL=shipcasterRuntime.js.map