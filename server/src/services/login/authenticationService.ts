...
const safeStateConfig: { underage: boolean; accruedSessions: any[][] } = {
...
function announceLogoutPresenceDeparture(session) {
  ...
  try {
    ...
    if (!result.success) {
      log.warn(
        `[AuthSvc] Failed canonical logout cleanup: ${result.errorMsg || "unknown"}`,
      );
    }
  } catch (error) {
...
function buildLoginInfo(session, request = null, loginArgs: any[] = []) {
  const userName =
...
AuthenticationService._testing = {
  ...
  setSafeStateConfig(overrides: Record<string, any> = {}) {
