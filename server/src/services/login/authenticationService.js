...
const { buildDict, extractDictEntries, normalizeNumber, normalizeText, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
// Logging out to character select keeps the socket open, so route it through
// the canonical disconnect/clear path instead of leaving a character indexed
// as a live retail controller. Lazy-required to keep service loading light.
function announceLogoutPresenceDeparture(session) {
    if (!session || getSessionCharacterID(session) <= 0) {
        return;
    }
    try {
        ...
        const result = disconnectCharacterSession(session, {
            broadcast: true,
            clearSession: true,
            lifecycleReason: "authentication_logout",
        });
        if (!result.success) {
            log.warn(`[AuthSvc] Failed canonical logout cleanup: ${result.errorMsg || "unknown"}`);
        }
    }
    catch (error) {
        log.warn(`[AuthSvc] Failed canonical logout cleanup: ${error.message}`);
    }
}
function getSessionAccountID(session) {
    return Math.max(0, toInteger(session && (session.userid ||
        session.userID ||
        session.userId ||
        session.accountID), 0));
}
function getSessionCharacterID(session) {
    return Math.max(0, toInteger(session && (session.characterID ||
        session.charID ||
        session.charid ||
        session.characterId), 0));
}
function getSessionClientID(session) {
    return Math.max(0, toInteger(session && (session.clientID || session.clientId), 0));
}
...
function buildLoginInfo(session, request = null, loginArgs = []) {
    const userName = toText(session && session.userName, "") ||
        toText(session && session.username, "") ||
        toText(loginArgs[1], "");
    const address = toText(session && session.address, "") ||
        toText(loginArgs[5], "") ||
        toText(getMappingValue(request, "address", ""), "");
    const languageID = normalizeLanguageID(session && (session.languageID ||
        session.languageId ||
        session.languageid));
...
function resolveComputerHash(session, request = null) {
    return (toText(getMappingValue(request, "computer_hash", ""), "") ||
...
class AuthenticationService extends BaseService {
    ...
    Handle_SetLanguageID(args, session) {
        ...
        log.debug(`[authentication] SetLanguageID account=${getSessionAccountID(session) || "?"} language=${languageID}`);
        return null;
    }
    Handle_Login(args, session, kwargs) {
        ...
        recordAuditEvent("login_safe_state", [], session, {
            ...
            clientVersion: toText(getMappingValue(kwargs, "clientVersion", null), toText(getMappingValue(request, "boot_build", ""), "")) || null,
        });
        return [
    ...
    Handle_Logout(args, session, kwargs) {
        ...
        // The socket stays open; canonical cleanup clears the selected character and
        // releases retail control while preserving the account-level connection.
...
AuthenticationService._testing = {
    ...
    setSafeStateConfig(overrides = {}) {
        ...
        if (Object.prototype.hasOwnProperty.call(overrides, "accruedSessions")) {
            safeStateConfig.accruedSessions = normalizeAccruedSessions(overrides.accruedSessions);
        }
    },
};
