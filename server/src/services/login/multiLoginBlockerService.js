...
const SubscriptionMgrService = require(path.join(__dirname, "../subscription/subscriptionMgrService"));
...
function getAccountID(session) {
    return toPositiveInteger(session && (session.userid || session.userID || session.accountID));
}
function getCharacterID(session) {
    return toPositiveInteger(session && (session.characterID || session.charID || session.charid));
}
...
class MultiLoginBlockerService extends BaseService {
    ...
    _resolveCloneGrade(session) {
        ...
        try {
            return normalizeCloneGrade(this.subscriptionService.Handle_GetCloneGrade([], session));
        }
        catch (error) {
            log.warn(`[multiLoginBlocker] Clone-grade lookup failed, defaulting to Omega allow-state: ${error.message}`);
    ...
    _findAlphaConflict(session, computerHash, cloneGrade) {
        ...
        for (const otherSession of activeSessions) {
            ...
            const otherComputerHash = normalizeComputerHash(otherSession.multiLoginComputerHash ||
            ...
            if (cloneGrade === CLONE_STATE_ALPHA ||
    ...
    Handle_Login(args, session) {
        ...
        if (!allowed) {
            log.info(`[multiLoginBlocker] blocked account=${getAccountID(session)} cloneGrade=${cloneGrade} conflictAccount=${conflict.accountID}`);
            return false;
        }
        log.debug(`[multiLoginBlocker] Login allow account=${getAccountID(session)} cloneGrade=${cloneGrade} reason=${reason}`);
        return true;
    }
}
...
//# sourceMappingURL=multiLoginBlockerService.js.map