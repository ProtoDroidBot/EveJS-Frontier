...
const { buildKeyVal, buildList, buildRowset, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
function buildIncursionReportEntry(rawIncursion = {}) {
    const stagingSolarSystemID = normalizePositiveInteger(rawIncursion.stagingSolarSystemID || rawIncursion.solarSystemID, 0);
    return buildKeyVal([
...
function buildIncursionGlobalReport(rawIncursions = []) {
    ...
    return rawIncursions
        .filter((entry) => normalizePositiveInteger(entry && (entry.stagingSolarSystemID || entry.solarSystemID), 0) > 0)
...
//# sourceMappingURL=incursionPayloads.js.map