...
const { buildKeyVal, buildList, buildObjectEx1, currentFileTime, extractList, isMarshalKeyValName, normalizeNumber, normalizeText, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { buildInventoryItemRow, emitItemsChangedBatchForSession, } = require(path.join(__dirname, "../character/characterState"));
const { DEFAULT_STATION, } = require(path.join(__dirname, "../_shared/stationStaticData"));
const { ITEM_FLAGS, findItemById, listContainerItems, moveItemToLocation, transferItemToOwnerLocation, } = require(path.join(__dirname, "../inventory/itemStore"));
const { isTradableInventoryItem, } = require(path.join(__dirname, "../inventory/typeListAuthority"));
const { adjustCharacterBalance, getCharacterWallet, JOURNAL_ENTRY_TYPE, } = require(path.join(__dirname, "../account/walletState"));
...
class TradeMgrService extends BaseService {
    ...
    _getTradeFromSession(session) {
        const currentBoundObjectID = session && session.currentBoundObjectID ? session.currentBoundObjectID : null;
        const sessionTrade = this._getTradeByBoundObjectID(currentBoundObjectID);
        if (sessionTrade) {
            return sessionTrade;
        }
        const cachedBoundObjectID = session &&
            session._boundObjectIDs &&
    ...
    _notifyItemChangeToTradeParticipants(trade, changedItem, previousState = {}) {
        for (const characterID of trade.traders) {
            ...
            const touchesTradeContainer = nextLocationID === trade.tradeContainerID ||
                previousLocationID === trade.tradeContainerID;
            const touchesSessionOwner = nextOwnerID === numericCharacterID ||
                previousOwnerID === numericCharacterID;
            if (!touchesTradeContainer && !touchesSessionOwner) {
                continue;
            }
            emitItemsChangedBatchForSession(session, [{ item: changedItem, previousData: previousState || {} }], { idType: "clientID", locationContext: null });
        }
    }
    _notifyChangeList(trade, changes = []) {
        for (const change of Array.isArray(changes) ? changes : []) {
            ...
            this._notifyItemChangeToTradeParticipants(trade, change.item, change.previousData || {});
        }
    }
    _notifyTradeItemPriming(trade, rootItemIDs = []) {
        const normalizedRootIDs = [...new Set((Array.isArray(rootItemIDs) ? rootItemIDs : [rootItemIDs])
                .map((value) => Number(value) || 0)
                .filter((value) => value > 0))];
    ...
    _buildCompletionLocationContext(trade, changes = []) {
        ...
        const previousLocationID = Number(firstChange && firstChange.previousData && firstChange.previousData.locationID || 0) || 0;
        const nextLocationID = Number(firstChange && firstChange.item && firstChange.item.locationID || 0) || 0;
        if (previousLocationID !== Number(trade.tradeContainerID || 0) ||
    ...
    _abortTrade(trade, options = {}) {
        ...
        for (const stagedItem of stagedItems) {
            ...
            const moveResult = moveItemToLocation(stagedItem.itemID, returnContext.locationID, returnContext.flagID, null);
    ...
    _parseManifest(args) {
        ...
        const tradeItems = new Set(extractManifestTradeItemEntries(rawTradeItems)
    ...
    _stageItemIntoTrade(trade, session, itemID, sourceLocationID, quantity = null) {
        ...
        const moveResult = moveItemToLocation(itemID, trade.tradeContainerID, ITEM_FLAGS.HANGAR, quantity);
        if (!moveResult.success) {
    ...
    _completeItemTransfers(trade) {
        ...
        for (const stagedItem of stagedItems) {
            ...
            const topLevelTransfer = transferItemToOwnerLocation(stagedItem.itemID, recipientCharacterID, trade.dockedLocationID, ITEM_FLAGS.HANGAR, null);
            if (!topLevelTransfer.success) {
                return topLevelTransfer;
            }
            changes.push(...((topLevelTransfer.data && topLevelTransfer.data.changes) || []));
            const containedTransfer = this._transferContainedItemsToOwner(trade, stagedItem.itemID, recipientCharacterID);
    ...
    Handle_InitiateTrade(args, session) {
        const targetCharacterID = normalizePositiveInteger(Array.isArray(args) ? args[0] : 0, 0);
        ...
        const solarSystemID = Number(session.solarsystemid2 ||
        ...
        log.info(`[TradeMgr] Initiated trade ${trade.tradeID} between ${initiatorCharacterID} and ${targetCharacterID} at ${trade.dockedLocationID}`);
    ...
    Handle_Add(args, session, kwargs) {
        ...
        const stageResult = this._stageItemIntoTrade(trade, session, itemID, sourceLocationID, quantity);
    ...
    Handle_MultiAdd(args, session) {
        ...
        const stagedRootItemIDs = [];
        for (const itemID of itemIDs) {
            const stageResult = this._stageItemIntoTrade(trade, session, itemID, sourceLocationID, null);
    ...
    Handle_OfferMoney(args, session) {
        const { trade, participantIndex, characterID } = this._resolveTradeContext(session, { requireOpen: true });
        if (!this._ensureAllParticipantsStillValid(trade)) {
            this._throwTradeError("Trade session is no longer available.");
        }
    ...
    Handle_MakeOffer(args, session) {
        ...
        const stagedItemIDs = new Set(this._listTradeItems(trade)
            .map((item) => Number(item.itemID) || 0)
            .filter((itemID) => itemID > 0));
        const normalizedMoney = trade.moneyOffers.map((value) => Math.round((Number(value) || 0) * 100) / 100);
        const manifestMoney = trade.moneyOffers.map((_, index) => Math.round((Number(manifest.money[index]) || 0) * 100) / 100);
        const manifestMatches = manifest.tradeContainerID === trade.tradeContainerID &&
        ...
        this._notifyTradeCompletionItemChanges(trade, itemTransferResult.data && itemTransferResult.data.changes);
        this._cleanupTradeIndex(trade);
        trade.stagedItemOrigins.clear();
    ...
    Handle_Abort(args, session) {
        ...
        const participantIndex = this._getParticipantIndex(trade, Number(session && session.characterID || 0));
        if (participantIndex < 0) {
            return true;
        }
        this._abortTrade(trade, { notifyParticipants: true });
...
//# sourceMappingURL=tradeMgrService.js.map