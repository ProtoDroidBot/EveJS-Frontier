...
function buildMarshalSet(values: any[] = []) {
  return buildObjectEx1("__builtin__.set", [
    buildList(Array.isArray(values) ? values : []),
  ]);
}

function buildTradeListRow(header: any[] = [], line: any[] = []) {
  return {
    type: "object",
...
function numericArray(values: any[] = []) {
...
class TradeMgrService extends BaseService {
  ...
  declare _tradeByParticipant: any;
  declare _tradeOrdinal: any;
  declare _trades: any;
  ...
  _getTradeByBindParams(bindParams, session = null) {
    ...
    for (const trade of this._trades.values()) {
      ...
      if (traders.length > 0 && !setsEqual(new Set<any>(trade.traders), new Set<any>(traders))) {
        continue;
      }
      return trade;
    }
  ...
  _resolveTradeContext(session, { requireOpen = false }: Record<string, any> = {}) {
  ...
  _notifyParticipants(trade, name, payload, options: Record<string, any> = {}) {
    for (const characterID of trade.traders) {
  ...
  _resetOfferState(trade, options: Record<string, any> = {}) {
  ...
  _notifyItemChangeToTradeParticipants(trade, changedItem, previousState: Record<string, any> = {}) {
    for (const characterID of trade.traders) {
      const session = this._getParticipantSession(characterID, null);
      if (!session) {
        continue;
  ...
  _notifyChangeList(trade, changes: any[] = []) {
  ...
  _notifyTradeItemPriming(trade, rootItemIDs: any[] = []) {
  ...
  _areStagedTradeTreesEligible(trade) {
    return this._listTradeItems(trade).every((rootItem) => {
      ...
      return (
        trade.traders.includes(ownerID) &&
  ...
  _buildCompletionLocationContext(trade, changes: any[] = []) {
  ...
  _groupItemChangesByPreviousContext(changes: any[] = []) {
  ...
  _notifyTradeCompletionItemChanges(trade, changes: any[] = []) {
  ...
  _abortTrade(trade, options: Record<string, any> = {}) {
    if (!trade || trade.status !== "open") {
      return {
        success: true as const,
        alreadyClosed: true,
  ...
  Handle_MakeOffer(args, session) {
    ...
    const stagedItemIDs = new Set<any>(
