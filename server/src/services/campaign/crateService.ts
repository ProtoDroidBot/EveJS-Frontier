...
function parseCustomSkillBundles(item) {
  ...
  if (typeof rawBundles === "object") {
    ...
    return Object.entries<any>(rawBundles)
...
function aggregateLootEntries(entries: any[] = []) {
...
function toGrantEntries(entries: any[] = []) {
  return entries
...
function buildLootResponseItems(items: any[] = []) {
  return items
    .filter(Boolean)
    .map((item) => [
