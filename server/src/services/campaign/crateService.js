...
const { syncInventoryItemForSession, updateCharacterRecord, } = require(path.join(__dirname, "../character/characterState"));
const { ITEM_FLAGS, consumeInventoryItemQuantity, findItemById, grantItemsToOwnerLocation, transferItemToOwnerLocation, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const { OWNER_SCOPE, findFittingByID, saveFitting, } = require(path.join(__dirname, "../../_secondary/fitting/fittingStore"));
const { grantCharacterSkillLevels, } = require(path.join(__dirname, "../skills/skillState"));
const { emitSkillSessionState, } = require(path.join(__dirname, "../skills/training/skillQueueNotifications"));
...
function normalizeSkillGrantEntry(rawEntry) {
    ...
    const level = Math.max(1, Math.min(5, toInt(source.level ?? source.toLevel ?? source.trainedSkillLevel, 1)));
    if (!typeID) {
        return null;
    }
    return {
...
function normalizeSkillBundleEntry(rawEntry, fallbackSkillBundleID = 0) {
    ...
    const skillBundleID = toPositiveInt(source.skillBundleID || source.bundleID || source.id, fallbackSkillBundleID);
    if (!skillBundleID) {
        return null;
    }
    ...
    const skills = (Array.isArray(rawSkills) ? rawSkills : [])
        ...
        .filter(Boolean);
    const freeSkillPoints = Math.max(0, toInt(source.freeSkillPoints ??
        source.skillPoints ??
...
function parseCustomLoot(item) {
    ...
    const rawLoot = (parsed && parsed.evejsCrateLoot) ||
...
function parseCustomSkillBundles(item) {
    ...
    const rawBundles = (parsed && parsed.evejsCrateSkillBundles) ||
    ...
    if (typeof rawBundles === "object") {
        ...
        return Object.entries(rawBundles)
            .map(([skillBundleID, entry]) => normalizeSkillBundleEntry(entry, toPositiveInt(skillBundleID, 0)))
            .filter(Boolean);
    }
    return [];
}
...
function applySkillBundleToCharacter(session, characterID, bundle) {
    ...
    if (bundle.freeSkillPoints > 0) {
        const updateResult = updateCharacterRecord(characterID, (record) => ({
            ...record,
            freeSkillPoints: Math.max(0, toInt(record && record.freeSkillPoints, 0)) +
...
function normalizeDestinationFromPending(itemID, destinationID, destinationFlagID) {
    ...
    if (requestedDestinationID !== pending.destinationID ||
...
function grantLootToLocation({ characterID, destinationID, destinationFlagID, lootEntries, }) {
    const grantResult = grantItemsToOwnerLocation(characterID, destinationID, destinationFlagID, toGrantEntries(lootEntries));
...
class CrateService extends BaseService {
    ...
    Handle_ClaimLoot(args, session) {
        ...
        const { destinationID, destinationFlagID, } = normalizeDestinationFromPending(itemID, args && args[1], args && args[2]);
        if (!destinationID) {
            throwNotify("That crate reward does not have a valid destination.");
        }
        const moveResult = transferItemToOwnerLocation(item.itemID, getSessionCharacterID(session), destinationID, destinationFlagID);
    ...
    Handle_ClaimLootFromCrateStack(args, session) {
        const itemID = toPositiveInt(args && args[0], 0);
        const requestedQuantity = Math.min(MAX_MULTI_OPEN, Math.max(1, toInt(args && args[1], 1)));
        const { characterID, item } = validateOwnedCrate(itemID, session);
        const destinationID = toPositiveInt(item.locationID, 0);
        const destinationFlagID = toInt(item.flagID, ITEM_FLAGS.HANGAR);
        if (!destinationID) {
        ...
        const lootEntries = aggregateLootEntries(Array.from({ length: cratesToOpen }, () => selectCrateLoot(item)).flat());
...
//# sourceMappingURL=crateService.js.map