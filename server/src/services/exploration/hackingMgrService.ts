...
function normalizeCallArgs(args: any[] = [], fallbackArgs: any[] = []) {
...
function buildEvent(eventID, eventData: Record<string, any> = {}) {
  return {
    eventID,
    eventData,
  };
...
function destroyTargetContainer(target, options: Record<string, any> = {}) {
...
function maybeCompleteAttemptSite(attempt, session, nowMs) {
  ...
  const missionResult = dungeonUniverseSiteService.maybeSatisfyMissionHackObjective(
    target.scene,
    target.entity,
    options,
  );
...
class HackingMgrService extends BaseService {
  ...
  Handle_StartNewGameInstance(args: any[] = [], session = null) {
    const [
      targetID,
      gameType,
      moduleTypeID,
  ...
  Handle_ClickedOnTile(args: any[] = [], session = null) {
  ...
  Handle_QuitHackingAttempt(args: any[] = [], session = null) {
    normalizeCallArgs(args);
    const key = getAttemptKey(session);
    const attempt = ACTIVE_ATTEMPTS.get(key);
    if (!attempt || attempt.completed) {
  ...
  Handle_UsedUtilityElement(args: any[] = [], session = null) {
    normalizeCallArgs(args);
    emitHackingTurn(session, [buildEvent(HACKING_CONSTANTS.EVENT_ACK, {})]);
    return null;
  }
  ...
  Handle_UsedUtilityElementOnVirus(args: any[] = [], session = null) {
    normalizeCallArgs(args);
    emitHackingTurn(session, [buildEvent(HACKING_CONSTANTS.EVENT_ACK, {})]);
    return null;
  }
