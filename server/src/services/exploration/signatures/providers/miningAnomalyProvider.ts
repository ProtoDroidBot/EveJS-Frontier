...
function getScene(systemID, options: Record<string, any> = {}) {
...
function listAnomalySites(systemID, options: Record<string, any> = {}) {
  ...
  return listGeneratedMiningAnchorEntities(scene).map((entity) => {
    ...
    const oreTypeIDs = [...new Set<any>(childEntities
      .map((entry) => toInt(entry && entry.miningYieldTypeID, 0))
      .filter((entry) => entry > 0))].sort((left, right) => left - right);
    const gasTypeIDs: number[] = [];
    const iceTypeIDs = [...new Set<any>(childEntities
      .map((entry) => toInt(entry && entry.miningYieldTypeID, 0))
