...
const dungeonSiteAdapter = require(path.join(__dirname, "../../../dungeon/dungeonSiteAdapter"));
const { resolveScene } = require(path.join(__dirname, "./sceneProviderRuntime"));
...
function listSignatureSites(systemID, options = {}) {
    ...
    return collectSceneEntities(scene)
        ...
        const explicitInstanceID = Math.max(0, toInt(entity && (entity.dungeonSiteInstanceID ||
            entity.dungeonInstanceID ||
            entity.instanceID), 0)) || null;
        const family = String(entity && (entity.signalTrackerSiteFamily ||
            entity.signalTrackerSignatureSiteFamily) || "unknown").trim().toLowerCase() || "unknown";
        const typeID = toInt(entity && (entity.signalTrackerSiteTypeID ??
            entity.typeID), 0) || null;
        const scanGroupID = toInt(entity && entity.signalTrackerSiteScanGroupID, toInt(explorationAuthority.getScanContracts() &&
        ...
        const enrichedSite = dungeonSiteAdapter.enrichSiteWithDungeonRuntime({
            ...
            siteKind: "signature",
            family,
            label: String(entity && (entity.signalTrackerSiteLabel ||
                entity.itemName ||
                entity.slimName) || `Signature Site ${siteID}`).trim(),
            ...
            typeID,
            groupID: toInt(entity && entity.signalTrackerSiteGroupID, toInt(signatureTypeDefinition && signatureTypeDefinition.inventoryGroupID, 0)) || null,
            scanGroupID,
            strengthAttributeID: toInt(entity && entity.signalTrackerStrengthAttributeID, explorationAuthority.getScanStrengthAttribute(family)) || null,
...
//# sourceMappingURL=sceneSignatureSiteProvider.js.map