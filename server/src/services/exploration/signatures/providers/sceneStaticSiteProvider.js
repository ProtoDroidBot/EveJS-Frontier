...
function buildTargetID(systemID, siteID) {
    return targetIdRuntime.encodeTargetID("static-site", systemID, siteID);
}
function listStaticSites(systemID, options = {}) {
    ...
    return collectSceneEntities(scene)
        ...
        toInt(entity.signalTrackerStaticSiteNameID ?? entity.dungeonNameID, 0) > 0))
        .map((entity) => {
        ...
        return dungeonSiteAdapter.enrichSiteWithDungeonRuntime({
            ...
            siteKind: "static",
            family: String(entity && (entity.signalTrackerStaticSiteFamily ||
                entity.signalTrackerSiteFamily) || "landmark").trim().toLowerCase() || "landmark",
            label: String(entity && (entity.signalTrackerStaticSiteLabel ||
                entity.itemName ||
                entity.slimName) || `Static Site ${siteID}`).trim(),
            dungeonNameID: toInt(entity && (entity.signalTrackerStaticSiteNameID ??
                entity.dungeonNameID), 0) || null,
            factionID: toInt(entity && (entity.signalTrackerStaticSiteFactionID ??
                entity.factionID), 0) || null,
...
//# sourceMappingURL=sceneStaticSiteProvider.js.map