...
function buildTargetID(systemID, siteID) {
    return targetIdRuntime.encodeTargetID("mining-anomaly", systemID, siteID);
}
function listAnomalySites(systemID, options = {}) {
    ...
    return listGeneratedMiningAnchorEntities(scene).map((entity) => {
        ...
        const childEntities = listGeneratedMiningSiteChildEntities(scene, entity && entity.generatedMiningSiteIndex, family);
        ...
        const scanStrengthAttribute = explorationAuthority.getScanStrengthAttribute("ore");
        return dungeonSiteAdapter.enrichSiteWithDungeonRuntime({
            ...
            dungeonID: null,
            archetypeID: null,
            instanceID: Math.max(0, toInt(entity && (entity.dungeonSiteInstanceID ||
                entity.dungeonInstanceID ||
                entity.instanceID), 0)) || siteID,
            dungeonNameID: null,
            factionID: null,
...
//# sourceMappingURL=miningAnomalyProvider.js.map