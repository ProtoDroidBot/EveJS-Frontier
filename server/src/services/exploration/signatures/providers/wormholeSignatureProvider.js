...
const { getStateSnapshot, } = require("../../wormholes/wormholeRuntimeState");
const AU_METERS = Number(explorationAuthority.getScanContracts().auMeters) || 149_597_870_700;
...
function isEndpointSignatureVisible(endpoint = {}) {
    const visibilityState = normalizeVisibilityState(endpoint.visibilityState, endpoint.discovered === true);
...
function buildSignaturePosition(position, signatureCode) {
    ...
    const distance = DEFAULT_SIGNATURE_DEVIATION_METERS *
...
function listWormholeSignatureCandidates(systemID, options = {}) {
    ...
    return Object.values(snapshot.pairsByID || {})
        .filter((pair) => String(pair && pair.state || "").trim().toLowerCase() === "active" &&
        ...
        .filter((entry) => toInt(entry && entry.endpoint && entry.endpoint.systemID, 0) === numericSystemID &&
        ...
        targetID: String(entry && entry.endpoint && entry.endpoint.targetID || "")
            .trim()
            .toUpperCase() || null,
...
function allocateStableSignatureCodes(systemID, candidates = []) {
    ...
    for (const candidate of Array.isArray(candidates) ? candidates : []) {
        ...
        const targetID = String(candidate && candidate.targetID || "").trim().toUpperCase() ||
...
function listSignatureSites(systemID, options = {}) {
    ...
    return candidates.map((candidate) => {
        const targetID = codesBySiteID.get(candidate.siteID) ||
            targetIdRuntime.encodeTargetID("wormhole", systemID, candidate.siteID);
        const wormholeCode = String(candidate && candidate.code || "").trim().toUpperCase() || "K162";
        return {
            ...candidate,
            siteKind: "signature",
            family: "wormhole",
...
//# sourceMappingURL=wormholeSignatureProvider.js.map