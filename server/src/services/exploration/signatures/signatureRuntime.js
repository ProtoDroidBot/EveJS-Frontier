...
const { buildDict, buildKeyVal, buildMarshalReal, buildMarshalRealVectorList, } = require(path.join(__dirname, "../../_shared/serviceHelpers"));
const miningAnomalyProvider = require(path.join(__dirname, "./providers/miningAnomalyProvider"));
const sceneAnomalySiteProvider = require(path.join(__dirname, "./providers/sceneAnomalySiteProvider"));
const sceneSignatureSiteProvider = require(path.join(__dirname, "./providers/sceneSignatureSiteProvider"));
const structureOverlayProvider = require(path.join(__dirname, "./providers/structureOverlayProvider"));
const sceneStaticSiteProvider = require(path.join(__dirname, "./providers/sceneStaticSiteProvider"));
const targetIdRuntime = require(path.join(__dirname, "./targetIdRuntime"));
const wormholeSignatureProvider = require(path.join(__dirname, "./providers/wormholeSignatureProvider"));
...
function listSystemSignatureSites(systemID, options = {}) {
    return normalizeSiteCollection(SIGNATURE_PROVIDERS.flatMap((provider) => (provider && typeof provider.listSignatureSites === "function"
...
function listSystemAnomalySites(systemID, options = {}) {
    return normalizeSiteCollection(ANOMALY_PROVIDERS.flatMap((provider) => (provider && typeof provider.listAnomalySites === "function"
...
function listSystemStaticSites(systemID, options = {}) {
    return normalizeSiteCollection(STATIC_SITE_PROVIDERS.flatMap((provider) => (provider && typeof provider.listStaticSites === "function"
...
function listSystemStructureSites(systemID, options = {}) {
    return normalizeStructureCollection(STRUCTURE_PROVIDERS.flatMap((provider) => (provider && typeof provider.listStructureSites === "function"
...
function buildSystemSignatureViews(systemID, options = {}) {
    return listSystemSignatureSites(systemID, options).map((site) => ({
        ...
        targetID: String(site && site.targetID || "").trim().toUpperCase(),
        family: String(site && site.family || "").trim().toLowerCase() || "unknown",
        label: String(site && site.label || "").trim() ||
...
function buildSignatureSiteInfo(site) {
    const defaultDeviation = Math.max(0, toFiniteNumber(site &&
        site.provider &&
        site.provider.DEFAULT_SIGNATURE_DEVIATION_METERS, wormholeSignatureProvider.DEFAULT_SIGNATURE_DEVIATION_METERS));
    return buildKeyVal([
        [
            "position",
            buildMarshalRealVectorList(Array.isArray(site && site.position) ? site.position : [0, 0, 0]),
        ],
        ...
        [
            "deviation",
            buildMarshalReal(Math.max(0, toFiniteNumber(site && site.deviation, defaultDeviation)), defaultDeviation),
        ],
    ]);
}
...
function buildSignalTrackerFullState(systemID, options = {}) {
    ...
    return [
        buildDict(anomalies.map((site) => [site.siteID, buildAnomalySiteInfo(site)])),
        buildDict(signatures.map((site) => [site.siteID, buildSignatureSiteInfo(site)])),
        buildDict(staticSites.map((site) => [site.siteID, buildStaticSiteInfo(site)])),
        buildDict(structures.map((site) => [site.siteID, buildStructureSiteInfo(site)])),
    ];
}
...
function getScanTargetID(systemID, siteID, options = {}) {
    ...
    return targetIdRuntime.encodeTargetID("site", systemID, numericSiteID);
}
...
function listWormholeSignatureSites(systemID, options = {}) {
    return normalizeSiteCollection(wormholeSignatureProvider.listSignatureSites(systemID, options).map((site) => ({
        ...site,
...
module.exports = {
    ...
    DEFAULT_SIGNATURE_DEVIATION_METERS: wormholeSignatureProvider.DEFAULT_SIGNATURE_DEVIATION_METERS,
    ...
    _testing: {
        ...
        encodeSignatureCodeFromNumber: wormholeSignatureProvider.encodeSignatureCodeFromNumber,
        ...
        listWormholeSignatureCandidates: wormholeSignatureProvider.listWormholeSignatureCandidates,
        allocateStableSignatureCodes: wormholeSignatureProvider.allocateStableSignatureCodes,
        buildSignaturePosition: wormholeSignatureProvider.buildSignaturePosition,
    },
};
//# sourceMappingURL=signatureRuntime.js.map