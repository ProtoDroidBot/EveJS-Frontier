...
function pickWeightedAnchorIndex(candidates, seed) {
  const weights = candidates.map((candidate) => Math.max(
    0,
    toFiniteNumber(candidate && candidate.selectionWeight, 0),
  ));
...
function buildAnchorRelativeSignaturePlacement(anchorCandidates: any[] = [], seed, options: Record<string, any> = {}) {
  ...
  const minimumDistanceMeters = Math.max(
    0,
    toFiniteNumber(anchor && anchor.minimumDistanceMeters, 0),
  );
  const maximumDistanceMeters = Math.max(
    minimumDistanceMeters,
    toFiniteNumber(anchor && anchor.maximumDistanceMeters, 0),
  );
  const boundedDistanceMeters = maximumDistanceMeters > 0
    ? minimumDistanceMeters + (
      (hashSeed(`${seed}:bounded-distance`) / 0x100000000) *
...
function estimateNearestAnchorDistanceMeters(position, anchorCandidates: any[] = []) {
  const candidates = (Array.isArray(anchorCandidates) ? anchorCandidates : [])
