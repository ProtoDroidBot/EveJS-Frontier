...
const { buildBoundObjectResponse, buildKeyVal, buildDict, buildPythonSet, currentFileTime, extractList, marshalObjectToObject, normalizeNumber, resolveBoundNodeId, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getAttributeIDByNames, getTypeAttributeValue, getTypeDogmaAttributes, getTypeEffectRecords, getFittedModuleItems, getLoadedChargeByFlag, getFittedModuleByFlag, getModuleChargeCapacity, isModuleOnline, isChargeCompatibleWithModule, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const { getShipFittingSnapshot, } = require(path.join(__dirname, "../../_secondary/fitting/fittingRuntime"));
const { getCharacterSkillMap, } = require(path.join(__dirname, "../skills/skillState"));
const { getImplantDogmaModifierEntries, } = require(path.join(__dirname, "../dogma/implants/activeImplantModifiers"));
const { ITEM_FLAGS, findItemById, grantItemToCharacterLocation, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const mobileScanInhibitorRuntime = require(path.join(__dirname, "../ship/mobileScanInhibitorRuntime"));
const { canEntityDiscoverTarget, } = require(path.join(__dirname, "../../space/destiny/identity/interactionScope.js"));
const SCAN_CONTRACTS = explorationAuthority.getScanContracts();
const SIGNATURE_TYPE_WORMHOLE = explorationAuthority.getSignatureTypeDefinition("wormhole") || {};
const ANOMALY_SCAN_GROUP = SCAN_CONTRACTS.probeScanGroups && SCAN_CONTRACTS.probeScanGroups.anomalies;
...
const DSCAN_COSMIC_SIGNATURE_GROUP_ID = Number(SIGNATURE_TYPE_WORMHOLE.inventoryGroupID) || 502;
const DSCAN_COSMIC_ANOMALY_GROUP_ID = Number(ANOMALY_SCAN_GROUP &&
    Array.isArray(ANOMALY_SCAN_GROUP.inventoryGroupIDs) &&
    ANOMALY_SCAN_GROUP.inventoryGroupIDs[0]) || 885;
const GROUP_SCAN_PROBE_LAUNCHER = 481;
const GROUP_SCANNER_PROBE = 479;
const ATTRIBUTE_DSCAN_IMMUNE = getAttributeIDByNames("dscanImmune") || 1958;
const ATTRIBUTE_MAX_DIRECTIONAL_SCAN_RANGE = getAttributeIDByNames("maxDirectionalScanRange") || 5796;
const ATTRIBUTE_SCAN_DURATION = getAttributeIDByNames("scanDuration") || 73;
const ATTRIBUTE_SCAN_DURATION_ALTERNATE = 66;
const ATTRIBUTE_BASE_SCAN_RANGE = getAttributeIDByNames("baseScanRange") || 1370;
const ATTRIBUTE_RANGE_FACTOR = getAttributeIDByNames("rangeFactor") || 1373;
...
const ATTRIBUTE_SCAN_STRENGTH_BONUS_MODULE = 1907;
const ATTRIBUTE_SCAN_DEVIATION_BONUS_MODULE = 1905;
const ATTRIBUTE_SCAN_DURATION_BONUS_MODULE = 1906;
const DOGMA_OP_POST_PERCENT = 6;
const SCAN_PROBE_NUMBER_OF_RANGE_STEPS = Math.max(1, Number(SCAN_CONTRACTS.rangeStepCount) || 8);
...
function resolveEntityRangeCapMeters(entity) {
    const explicitRange = Math.max(0, toFiniteNumber(entity && entity.maxDirectionalScanRange, 0));
    if (explicitRange > 0) {
        return explicitRange;
    }
    const typedRange = Math.max(0, toFiniteNumber(getTypeAttributeValue(entity && entity.typeID, "maxDirectionalScanRange"), toFiniteNumber(getTypeAttributeValueByID(entity && entity.typeID, ATTRIBUTE_MAX_DIRECTIONAL_SCAN_RANGE), 0)));
...
function isEntityDirectionalScanImmune(entity) {
    const attributeValue = toFiniteNumber(getTypeAttributeValue(entity && entity.typeID, "dscanImmune"), toFiniteNumber(getTypeAttributeValueByID(entity && entity.typeID, ATTRIBUTE_DSCAN_IMMUNE), 0));
...
function isEntityDirectionalScanCandidate(entity, egoEntity) {
    ...
    if (groupID === DSCAN_COSMIC_SIGNATURE_GROUP_ID ||
...
function isDirectionalScanBlindedByScanInhibitor(systemID, egoEntity, scene) {
    return isPositionInsideActiveScanInhibitor(systemID, egoEntity && egoEntity.position, scene, { targetEntity: egoEntity });
}
function isDirectionalScanTargetHiddenByScanInhibitor(systemID, entity, scene) {
    if (isMobileScanInhibitorEntity(entity)) {
        return false;
    }
    return isPositionInsideActiveScanInhibitor(systemID, entity && entity.position, scene, { targetEntity: entity });
}
...
function buildSignatureEntryMap(systemID, options = {}) {
    return new Map(signatureRuntime.buildSignalTrackerSignatureEntries(systemID, options)
...
function buildAnomalyEntryMap(systemID, options = {}) {
    return new Map(signatureRuntime.buildSignalTrackerAnomalyEntries(systemID, options)
        .map(([siteID, siteInfo]) => [normalizeNumber(siteID, 0), siteInfo])
        .filter(([siteID]) => siteID > 0));
}
...
function buildStructureEntryMap(systemID, options = {}) {
    return new Map(signatureRuntime.buildSignalTrackerStructureEntries(systemID, options)
        .map(([siteID, siteInfo]) => [normalizeNumber(siteID, 0), siteInfo])
        .filter(([siteID]) => siteID > 0));
}
function buildStaticSiteEntryMap(systemID, options = {}) {
    return new Map(signatureRuntime.buildSignalTrackerStaticSiteEntries(systemID, options)
        .map(([siteID, siteInfo]) => [normalizeNumber(siteID, 0), siteInfo])
        .filter(([siteID]) => siteID > 0));
}
...
function notifySignatureDeltaForSystem(systemID, options = {}) {
    ...
    for (const session of sessions) {
        session.sendNotification("OnSignalTrackerSignatureUpdate", "charid", payload);
...
function notifyAnomalyDeltaForSystem(systemID, options = {}) {
    ...
    for (const session of sessions) {
        session.sendNotification("OnSignalTrackerAnomalyUpdate", "charid", payload);
        for (const [, siteInfo] of addedEntries) {
            sendLegacyCosmicAnomalyAdded(session, siteInfo);
        }
        for (const siteID of removedSiteIDs) {
            sendLegacyCosmicAnomalyRemoved(session, siteID, removedEntries.get(siteID), numericSystemID);
        }
    }
}
...
function notifyStructureDeltaForSystem(systemID, options = {}) {
    ...
    for (const session of sessions) {
        session.sendNotification("OnSignalTrackerStructureUpdate", "solarsystemid2", payload);
    }
}
function notifyFullStateRefreshForSystem(systemID, options = {}) {
    ...
    const fullState = signatureRuntime.buildSignalTrackerFullState(numericSystemID, options);
    ...
    for (const session of sessions) {
        session.sendNotification("OnSignalTrackerFullState", "charid", payload);
    }
    return {
        success: true,
        data: {
...
function restoreRecoveredProbeCharges(session, characterID, removedProbes = [], options = {}) {
    ...
    for (const probe of removedProbes) {
        ...
        const shipID = (persistedShipID > 0 && findItemById(persistedShipID))
            ? persistedShipID
            : fallbackShipID;
    ...
    for (const entry of groupedRestores.values()) {
        ...
        const moduleItem = entry.launcherFlagID > 0
            ...
            : null;
        const canRestoreToLauncher = moduleItem &&
        ...
        const moduleCapacity = canRestoreToLauncher
            ? Math.max(0, normalizeNumber(getModuleChargeCapacity(moduleItem.typeID, entry.typeID), 0))
            : 0;
        const currentLoadedCharge = canRestoreToLauncher
            ? getLoadedChargeByFlag(numericCharacterID, entry.shipID, entry.launcherFlagID)
            : null;
        const currentLoadedQuantity = Math.max(0, normalizeNumber(currentLoadedCharge &&
            (currentLoadedCharge.stacksize ?? currentLoadedCharge.quantity), 0));
        ...
        const previousChargeState = launcherQuantity > 0
            ? dogmaHelper._captureChargeStateSnapshot(numericCharacterID, entry.shipID, entry.launcherFlagID)
            : null;
        const previousChargeItem = launcherQuantity > 0
            ? dogmaHelper._captureChargeItemSnapshot(numericCharacterID, entry.shipID, entry.launcherFlagID)
            : null;
        ...
        for (const target of restoreTargets) {
            ...
            const grantResult = grantItemToCharacterLocation(numericCharacterID, entry.shipID, target.flagID, itemType, target.quantity, { singleton: 0 });
            if (!grantResult || grantResult.success !== true) {
                log.warn(`[ScanMgr] Failed to restore ${target.quantity} recovered probe charge(s) ` +
            ...
            dogmaHelper._syncInventoryChanges(session, (grantResult.data && grantResult.data.changes) || []);
        ...
        if (launcherQuantity > 0) {
            const nextChargeState = dogmaHelper._captureChargeStateSnapshot(numericCharacterID, entry.shipID, entry.launcherFlagID);
            dogmaHelper._notifyChargeQuantityTransition(session, numericCharacterID, entry.shipID, entry.launcherFlagID, previousChargeState, nextChargeState);
            dogmaHelper._notifyWeaponModuleAttributeTransition(session, moduleItem, previousChargeItem, dogmaHelper._captureChargeItemSnapshot(numericCharacterID, entry.shipID, entry.launcherFlagID));
        }
    }
...
function emitProbeScanLifecycleNotifications(session, scanResult, options = {}) {
    ...
    const durationMs = Math.max(1, normalizeNumber(options.durationMs, normalizeNumber(scanResult.durationMs, probeScanRuntime.DEFAULT_PROBE_SCAN_DURATION_MS)));
...
function areProbePositionsEqual(left, right) {
    ...
    return (Math.abs(toFiniteNumber(leftPosition[0], 0) - toFiniteNumber(rightPosition[0], 0)) < 0.001 &&
...
function scheduleProbeMovementThenScan(session, probes = [], scanStarter, options = {}) {
    ...
    const durationMs = Math.max(1, normalizeNumber(options.moveDurationMs, DEFAULT_PROBE_MOVE_DURATION_MS));
    const scheduleFn = typeof options.scheduleFn === "function"
        ? options.scheduleFn
        : setTimeout;
    const clearFn = typeof options.clearFn === "function"
...
function getSessionShipID(session) {
    return normalizeNumber(session && (session.shipID ||
        session.shipid ||
        session.activeShipID ||
        (session._space && session._space.shipID)), 0);
}
function resolveActiveProbeLauncherForSession(session) {
    const charID = normalizeNumber(session && (session.characterID || session.charid || session.userid), 0);
...
function getSkillLevel(skillRecord) {
    return Math.max(0, normalizeNumber(skillRecord && (skillRecord.effectiveSkillLevel ??
...
function resolveCharacterProbeScanBonuses(characterID, options = {}) {
    const numericCharacterID = normalizeNumber(characterID, 0);
    ...
    for (const skillTypeID of SCANNING_SKILL_TYPE_IDS) {
        ...
        addProbeScanBonus(scanBonuses, "strength", "skills", skillLevel * toFiniteNumber(attributes[String(ATTRIBUTE_PROBE_SCAN_STRENGTH_BONUS)], 0));
        addProbeScanBonus(scanBonuses, "deviation", "skills", skillLevel * toFiniteNumber(attributes[String(ATTRIBUTE_MAX_SCAN_DEVIATION_MODIFIER)], 0));
    }
    ...
    for (const entry of getImplantDogmaModifierEntries(numericCharacterID)) {
        ...
        const targetAttributeID = normalizeNumber(entry && (entry.modifiedAttributeID ?? entry.targetAttributeID), 0);
    ...
    const shipID = normalizeNumber(options && options.shipID, 0);
    if (shipID > 0) {
        const shipItem = findItemById(shipID);
        const fittingSnapshot = shipItem
            ? getShipFittingSnapshot(numericCharacterID, shipItem)
            : null;
        const shipAttributes = fittingSnapshot && fittingSnapshot.shipAttributes
            ? fittingSnapshot.shipAttributes
            : getTypeDogmaAttributes(shipItem && shipItem.typeID);
        for (const effectRecord of getTypeEffectRecords(shipItem && shipItem.typeID)) {
            for (const modifierInfo of Array.isArray(effectRecord && effectRecord.modifierInfo)
                ? effectRecord.modifierInfo
                : []) {
                if (modifierInfo &&
                    String(modifierInfo.func || "") === "OwnerRequiredSkillModifier" &&
                    normalizeNumber(modifierInfo.modifiedAttributeID, 0) ===
                        ATTRIBUTE_BASE_SENSOR_STRENGTH &&
                    normalizeNumber(modifierInfo.operation, 0) === DOGMA_OP_POST_PERCENT) {
                    addProbeScanBonus(scanBonuses, "strength", "ship", toFiniteNumber(shipAttributes[normalizeNumber(modifierInfo.modifyingAttributeID, 0)], 0));
                }
            }
        }
        const fittedModules = getFittedModuleItems(numericCharacterID, shipID);
        const onlineProbeLaunchers = fittedModules
            .filter((moduleItem) => normalizeNumber(moduleItem && moduleItem.groupID, 0) === GROUP_SCAN_PROBE_LAUNCHER)
            .filter((moduleItem) => isModuleOnline(moduleItem));
        const activeProbeLauncher = onlineProbeLaunchers.find((moduleItem) => {
            const loadedCharge = getLoadedChargeByFlag(numericCharacterID, shipID, moduleItem.flagID);
            return normalizeNumber(loadedCharge && loadedCharge.groupID, 0) === GROUP_SCANNER_PROBE;
        }) || onlineProbeLaunchers[0] || null;
        for (const moduleItem of fittedModules) {
            if (!isModuleOnline(moduleItem)) {
                continue;
            }
            const moduleAttributes = getTypeDogmaAttributes(moduleItem && moduleItem.typeID);
            const launcherStrengthBonus = activeProbeLauncher &&
                normalizeNumber(moduleItem && moduleItem.itemID, 0) ===
                    normalizeNumber(activeProbeLauncher && activeProbeLauncher.itemID, 0)
                ? toFiniteNumber(moduleAttributes[String(ATTRIBUTE_PROBE_SCAN_STRENGTH_BONUS)], 0)
                : 0;
            addProbeScanBonus(scanBonuses, "strength", "modules", launcherStrengthBonus +
                toFiniteNumber(moduleAttributes[String(ATTRIBUTE_SCAN_STRENGTH_BONUS_MODULE)], 0));
            addProbeScanBonus(scanBonuses, "deviation", "modules", toFiniteNumber(moduleAttributes[String(ATTRIBUTE_SCAN_DEVIATION_BONUS_MODULE)], 0));
            addProbeScanBonus(scanBonuses, "duration", "modules", toFiniteNumber(moduleAttributes[String(ATTRIBUTE_SCAN_DURATION_BONUS_MODULE)], 0));
        }
    }
    return scanBonuses;
}
function resolveCharacterScanDurationBonusMultiplier(characterID, options = {}) {
    const numericCharacterID = normalizeNumber(characterID, 0);
    if (numericCharacterID <= 0) {
        return 1;
    }
    ...
    for (const skillTypeID of SCANNING_SKILL_TYPE_IDS) {
        ...
        const skillLevel = normalizeNumber(skillRecord && (skillRecord.effectiveSkillLevel ??
            skillRecord.trainedSkillLevel ??
            skillRecord.skillLevel), 0);
        if (skillLevel <= 0) {
            continue;
    ...
    let moduleMultiplier = 1;
    const shipID = normalizeNumber(options && options.shipID, 0);
    if (shipID > 0) {
        for (const moduleItem of getFittedModuleItems(numericCharacterID, shipID)) {
            if (!isModuleOnline(moduleItem)) {
                continue;
            }
            const bonus = Number(getTypeDogmaAttributes(moduleItem && moduleItem.typeID)[String(ATTRIBUTE_SCAN_DURATION_BONUS_MODULE)]);
            if (Number.isFinite(bonus) && bonus !== 0) {
                moduleMultiplier *= (1 + (bonus / 100));
            }
        }
    }
    return multiplier * implantMultiplier * moduleMultiplier;
}
function resolveProbeScanDurationMs(session, options = {}) {
    ...
    const baseDurationMs = Math.max(1, normalizeNumber(getTypeAttributeValueByID(probeLauncher && probeLauncher.typeID, ATTRIBUTE_SCAN_DURATION), probeScanRuntime.DEFAULT_PROBE_SCAN_DURATION_MS));
    const durationMultiplier = resolveCharacterScanDurationBonusMultiplier(session && (session.characterID || session.charid || session.userid), { shipID: getSessionShipID(session) });
    const resolvedDurationMs = baseDurationMs * (Number.isFinite(durationMultiplier) && durationMultiplier > 0
        ? durationMultiplier
        : 1);
...
function resolveProbeRangeStepsByTypeID(typeID) {
    ...
    const baseScanRange = toFiniteNumber(getTypeAttributeValueByID(numericTypeID, ATTRIBUTE_BASE_SCAN_RANGE), toFiniteNumber(getTypeAttributeValue(numericTypeID, "baseScanRange"), 0));
    const rangeFactor = toFiniteNumber(getTypeAttributeValueByID(numericTypeID, ATTRIBUTE_RANGE_FACTOR), toFiniteNumber(getTypeAttributeValue(numericTypeID, "rangeFactor"), 0));
...
function buildBoundObjectSubstruct(serviceName, session) {
    ...
    const objectID = existingObjectState && typeof existingObjectState.objectID === "string"
        ? existingObjectState.objectID
        : `N=${config.proxyNodeId}:${config.getNextBoundId()}`;
    const boundAtFileTime = existingObjectState && existingObjectState.boundAtFileTime
...
class ScanMgrService extends BaseService {
    ...
    _getCharacterID(session) {
        return normalizeNumber(session && (session.characterID || session.charid || session.userid), 0);
    }
    ...
    _buildFullState(session, options = {}) {
        const systemID = this._ensureSystemParity(session, normalizeNumber(options.nowMs, Date.now()));
        if (systemID <= 0) {
        ...
        if (session && typeof session.sendNotification === "function") {
            ...
            setImmediate(() => {
                if (typeof notificationSession.sendNotification === "function") {
                    notificationSession.sendNotification("OnSignalTrackerFullState", "charid", notificationPayload);
                }
            });
        }
    ...
    Handle_SetProbeDestination(args, session) {
        ...
        const updated = probeRuntimeState.synchronizeCharacterProbeGeometry(characterID, systemID, new Map([
            [probeID, { destination }],
        ]), { nowMs: Date.now() });
        if (updated.length > 0) {
    ...
    Handle_SetProbeRangeStep(args, session) {
        ...
        const clampedRangeStep = clamp(requestedRangeStep, 1, Math.max(1, rangeSteps.length || SCAN_PROBE_NUMBER_OF_RANGE_STEPS));
        const scanRange = toFiniteNumber(rangeSteps[clampedRangeStep - 1], toFiniteNumber(persistedProbe.scanRange, 0));
        probeRuntimeState.synchronizeCharacterProbeGeometry(characterID, systemID, new Map([
            [probeID, {
    ...
    Handle_GetScanTargetID(args, session, kwargs) {
        ...
        const targetSeedID = normalizeNumber(getKwargValue(kwargs, "targetSeedID"), siteID);
    ...
    Handle_ConeScan(args, session) {
        ...
        const scene = runtime && typeof runtime.getSceneForSession === "function"
            ? runtime.getSceneForSession(session)
            : null;
        if (!scene) {
            return [];
        }
        const egoEntity = scene && typeof scene.getShipEntityForSession === "function"
        ...
        const requestedRange = toFiniteNumber(args && args[1], DSCAN_FALLBACK_MAX_RANGE_METERS);
        ...
        const maxRangeMeters = Math.max(DSCAN_MIN_RANGE_METERS, resolveEntityRangeCapMeters(egoEntity));
        const effectiveRangeMeters = clamp(requestedRange, DSCAN_MIN_RANGE_METERS, maxRangeMeters);
        const egoPosition = cloneVector(egoEntity.position);
        const egoRadius = Math.max(0, toFiniteNumber(egoEntity.radius, 0));
        const fullSweep = effectiveAngle >= (DSCAN_FULL_SWEEP_RADIANS - 0.0001);
        ...
        return collectSceneEntities(scene)
            ...
            .filter((entity) => canEntityDiscoverTarget(egoEntity, entity))
            ...
            const surfaceDistance = Math.max(0, centerDistance - egoRadius - targetRadius);
    ...
    Handle_RequestScans(args, session, kwargs) {
        ...
        const synchronizedProbes = probeRuntimeState.synchronizeCharacterProbeGeometry(characterID, systemID, probePatchMap, { nowMs });
        const synchronizedProbeIDs = new Set(synchronizedProbes.map((probe) => normalizeNumber(probe && probe.probeID, 0)));
        const normalizedProbeMap = probeScanRuntime.normalizeProbeMap(args && args[0]);
        const effectiveProbeMap = new Map([...normalizedProbeMap.entries()].filter(([probeID]) => synchronizedProbeIDs.has(normalizeNumber(probeID, 0))));
        ...
        const startResolvedScan = () => {
            const scene = getSpaceRuntime() && typeof getSpaceRuntime().getSceneForSession === "function"
                ? getSpaceRuntime().getSceneForSession(session)
                : null;
            const egoEntity = scene && typeof scene.getShipEntityForSession === "function"
                ? scene.getShipEntityForSession(session)
                : null;
            const scanResult = isPositionInsideActiveScanInhibitor(systemID, egoEntity && egoEntity.position, scene)
                ? buildScanInhibitorBlindedProbeScanResult(effectiveProbeMap, resolvedDurationMs)
                : probeScanRuntime.buildSignatureScanResults(systemID, effectiveProbeMap, {
                    durationMs: resolvedDurationMs,
                    scanBonuses: resolveCharacterProbeScanBonuses(characterID, {
                        shipID: getSessionShipID(session),
                    }),
                    scene,
                    characterID,
                    shipID: getSessionShipID(session),
            ...
            // Advance the "Scan N Signatures" AIR daily goal for each cosmic signature
            // resolved to 100% signal strength. De-duplicated by signature ID so
            // re-scans of an already-resolved signature don't re-credit. Defensive so
            // daily-goal bookkeeping can never disrupt scanning.
            const perfectSignatureIDs = Array.isArray(scanResult && scanResult.perfectSignatureIDs)
                ? scanResult.perfectSignatureIDs
                : [];
            if (perfectSignatureIDs.length > 0) {
                try {
                    const dailyGoalsState = require(path.join(__dirname, "../../services/dailyGoals/dailyGoalsState"));
                    for (const signatureID of perfectSignatureIDs) {
                        dailyGoalsState.recordUniqueActivity(characterID, "scan_signature", signatureID);
                    }
                }
                catch (dailyGoalError) {
                    log.debug(`[ScanMgr] daily-goal scan_signature hook failed char=${characterID}: ${dailyGoalError.message}`);
                }
            }
        };
        scheduleProbeMovementThenScan(session, synchronizedProbes, startResolvedScan, {
            moveDurationMs: getKwargValue(kwargs, "_testingMoveDurationMs"),
            scheduleFn: getKwargValue(kwargs, "_testingSetTimeout"),
            clearFn: getKwargValue(kwargs, "_testingClearTimeout"),
            onArrive: () => {
                const arrivedProbes = probeRuntimeState.synchronizeCharacterProbeGeometry(characterID, systemID, new Map(synchronizedProbes.map((probe) => [
                    normalizeNumber(probe && probe.probeID, 0),
                    {
    ...
    Handle_ReconnectToLostProbes(args, session) {
        ...
        const probes = probeRuntimeState.getReconnectableCharacterProbes(characterID, systemID);
    ...
    Handle_RecoverProbes(args, session) {
        ...
        const characterID = this._getCharacterID(session);
        const shipID = normalizeNumber((session && (session.shipID || session.shipid || session.activeShipID)) ||
        ...
        const removedProbes = probeRuntimeState.removeCharacterProbes(characterID, probeIDs, { nowMs: Date.now() });
    ...
    Handle_QAScanSites(args, session) {
        ...
        const requestedSiteKeys = new Set((Array.isArray(args) ? args : [])
        ...
        const signatureSitesByResultID = new Map(signatureRuntime.listSystemSignatureSites(systemID).map((site) => [
...
//# sourceMappingURL=scanMgrService.js.map