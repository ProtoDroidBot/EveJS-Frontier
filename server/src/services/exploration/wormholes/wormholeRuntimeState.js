...
function normalizeEndpoint(endpoint = {}) {
    ...
    const targetID = String(endpoint.targetID || "").trim().toUpperCase() || (endpointID > 0 && systemID > 0
...
function normalizePair(pair = {}) {
    return {
        ...
        lastMassStateAtMs: Math.max(0, toInt(pair.lastMassStateAtMs, pair.createdAtMs)),
...
function normalizeState(table = {}) {
    ...
    for (const [characterKey, endpoints] of Object.entries(table.polarizationByCharacter || {})) {
        ...
        for (const [endpointKey, polarization] of Object.entries(endpoints || {})) {
            ...
            const durationSeconds = Math.max(0, toInt(polarization && polarization.durationSeconds, 0));
...
function applyPersistedDiff(nextState) {
    ...
    for (const group of WORMHOLE_GROUP_KEYS) {
        ...
        if (Object.prototype.hasOwnProperty.call(nextState, group) &&
...
//# sourceMappingURL=wormholeRuntimeState.js.map