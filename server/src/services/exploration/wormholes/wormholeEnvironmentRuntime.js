...
const { getSystemEnvironment, } = require("./wormholeAuthority");
const { resolveItemByTypeID, } = require(path.join(__dirname, "../../inventory/itemTypeRegistry"));
const { buildDict, buildPythonSet, } = require(path.join(__dirname, "../../_shared/serviceHelpers"));
const { getTypeAttributeMap, getTypeEffectRecords, appendDirectModifierEntries, } = require(path.join(__dirname, "../../fitting/liveFittingState"));
...
function buildDescriptor(systemID) {
    ...
    const secondarySunType = resolveItemByTypeID(toInt(environment.environmentTypeID, 0)) || null;
    const effectType = resolveItemByTypeID(toInt(environment.environmentEffectTypeID, 0)) || null;
    ...
    appendDirectModifierEntries(shipAttributeModifierEntries, sourceAttributes, sourceEffects, "system", {
    ...
    return Object.freeze({
        ...
        environmentEffectTypeName: String(environment.environmentEffectTypeName || "").trim() || null,
        secondarySunType,
        effectType,
        sourceAttributes,
        sourceEffects,
        shipAttributeModifierEntries: Object.freeze(shipAttributeModifierEntries.map((entry) => Object.freeze({ ...entry }))),
...
function buildSystemWideEffectsPayloadForSystem(systemID) {
    ...
    const effectIDs = [...new Set((Array.isArray(descriptor.sourceEffects) ? descriptor.sourceEffects : [])
            .map((effectRecord) => toInt(effectRecord && effectRecord.effectID, 0))
            .filter((effectID) => effectID > 0))].sort((left, right) => left - right);
    return buildDict([
        [
...
function buildSecondarySunEntity(systemID) {
    ...
    return {
        ...
        categoryID: toInt(secondarySunType && secondarySunType.categoryID, SECONDARY_SUN_CATEGORY_ID),
        itemName: String((secondarySunType && secondarySunType.name) || descriptor.environmentName || "").trim() ||
...
function buildEffectBeaconEntity(systemID) {
    ...
    return {
        ...
        categoryID: toInt(effectType && effectType.categoryID, EFFECT_BEACON_CATEGORY_ID),
        itemName: String((effectType && effectType.name) ||
...
//# sourceMappingURL=wormholeEnvironmentRuntime.js.map