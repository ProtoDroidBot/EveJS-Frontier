...
function cloneEndpoint(endpoint: Record<string, any> = {}) {
  return {
    ...endpoint,
...
function cloneRuntimeState(state: Record<string, any> = {}) {
  const pairsByID: Record<string, any> = {};
  for (const [pairKey, pair] of Object.entries<any>(state.pairsByID || {})) {
    pairsByID[pairKey] = {
      ...pair,
  ...
  const staticSlotsByKey: Record<string, any> = {};
  for (const [slotKey, slotState] of Object.entries<any>(state.staticSlotsByKey || {})) {
    staticSlotsByKey[slotKey] = {
      ...slotState,
    };
  }

  const polarizationByCharacter: Record<string, any> = {};
  for (const [characterKey, endpoints] of Object.entries<any>(state.polarizationByCharacter || {})) {
    const clonedEndpoints: Record<string, any> = {};
...
function normalizeEndpoint(endpoint: Record<string, any> = {}) {
...
function normalizePair(pair: Record<string, any> = {}) {
  return {
...
function normalizeState(table: Record<string, any> = {}) {
  ...
  const pairsByID: Record<string, any> = {};
  for (const [pairKey, pair] of Object.entries<any>(table.pairsByID || {})) {
  ...
  const staticSlotsByKey: Record<string, any> = {};
  for (const [slotKey, slotState] of Object.entries<any>(table.staticSlotsByKey || {})) {
  ...
  for (const [characterKey, endpoints] of Object.entries<any>(table.polarizationByCharacter || {})) {
    ...
    for (const [endpointKey, polarization] of Object.entries<any>(endpoints || {})) {
...
function writeState(state, options: Record<string, any> = {}) {
