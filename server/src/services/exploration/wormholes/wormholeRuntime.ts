...
const signatureStateChangeListeners = new Set<any>();
...
function buildPairIndex(table: Record<string, any> = {}) {
  const activePairsBySystemID = new Map();
  const allPairsBySystemID = new Map();
  let activePairCount = 0;
  let totalPairCount = 0;
  for (const pair of Object.values<any>(table.pairsByID || {})) {
    totalPairCount += 1;
...
function getPairIndex(table: Record<string, any> = {}) {
...
function notifySignatureStateChanged(systemIDs: any[] = []) {
...
function collectVisibleSignatureSystemIDsForPair(pair, options: Record<string, any> = {}) {
  const includeHiddenDestination = options.includeHiddenDestination === true;
  if (!pair) {
    return [];
  }
  const systems = new Set<any>();
...
function cloneVector(vector = null, fallback: Record<string, any> = { x: 0, y: 0, z: 0 }) {
  return {
...
function buildEndpointRecord(systemID, typeRecord, options: Record<string, any> = {}) {
  const authority = getSystemAuthority(systemID);
...
function selectSourceSystemIDForRandomProfile(
  ...
  const occupiedSystemIDs = occupiedSystemIDsOverride instanceof Set
    ...
    : new Set(
      Object.values<any>(table.pairsByID || {})
...
function createRandomPairFromProfile(table, profile, nowMs, options: Record<string, any> = {}) {
...
function createRandomPair(table, systemID, nowMs, options: Record<string, any> = {}) {
  const sourceSystemID = toInt(systemID, 0);
...
function prunePolarizations(table, nowMs) {
  for (const [characterKey, endpoints] of Object.entries<any>(table.polarizationByCharacter || {})) {
    for (const [endpointKey, record] of Object.entries<any>(endpoints || {})) {
...
function collectRandomProfileStats(table, nowMs) {
  ...
  for (const pair of Object.values<any>(table.pairsByID || {})) {
    ...
    if (state === "active") {
      const sourceSystemID = toInt(pair && pair.source && pair.source.systemID, 0);
      if (sourceSystemID > 0) {
        const occupiedSystemIDs = occupiedSystemIDsByProfileKey.get(profileKey) || new Set<any>();
...
function collectPairsForSystem(table, systemID, options: Record<string, any> = {}) {
  const numericSystemID = toInt(systemID, 0);
  ...
  const allPairs = Object.values<any>(table.pairsByID || {});
  if (includeCollapsed) {
    return allPairs;
  }
...
function tickScene(scene, nowMs = Date.now()) {
  ...
  const signatureChangedSystemIDs = new Set<any>();
  mutateState((table) => {
    prunePolarizations(table, nowMs);
...
function buildJumpSpawnState(pair, sourceRole, options: Record<string, any> = {}) {
...
function listPairViewsFromTable(table, options: Record<string, any> = {}) {
...
function listPairViews(options: Record<string, any> = {}) {
  return listPairViewsFromTable(getStateView(), options);
}

function buildSystemSummaryViewsFromTable(table, options: Record<string, any> = {}) {
  const systemIDFilter = toInt(options.systemID, 0);
...
function buildSystemSummaryViews(options: Record<string, any> = {}) {
  return buildSystemSummaryViewsFromTable(getStateView(), options);
}

function buildUniverseSummary(options: Record<string, any> = {}) {
  const table = getStateView();
  const entries = listPairViewsFromTable(table, {
    includeCollapsed: options.includeCollapsed === true,
    includeUndiscovered: options.includeUndiscovered !== false,
