...
const { getCodeTypeRecord, getK162TypeRecord, getSystemAuthority, getSystemClassID, listCodeTypes, listCandidateSystemIDsForClass, listSystems, listStaticSlotsForSystem, listWanderingProfiles, listWanderingProfilesForSourceClass, } = require("./wormholeAuthority");
const { buildWormholePresentationSnapshot, } = require("./wormholePresentation");
const { buildAnchorRelativeSignaturePlacement, } = require(path.join(__dirname, "../signatures/signaturePlacement"));
const { getStateView, getStateSnapshot, mutateState, } = require("./wormholeRuntimeState");
const wormholeEnvironmentRuntime = require("./wormholeEnvironmentRuntime");
const { isAddBalls2BootstrapScope, } = require(path.join(__dirname, "../../../space/destiny/bootstrap/staticBootstrap"));
const targetIdRuntime = require(path.join(__dirname, "../signatures/targetIdRuntime"));
...
function notifySignatureStateChanged(systemIDs = []) {
    const normalizedSystemIDs = [...new Set((Array.isArray(systemIDs) ? systemIDs : [systemIDs])
    ...
    for (const listener of signatureStateChangeListeners) {
        try {
            ...
            log.warn(`[Wormholes] Signature state change listener failed: ${error && error.message ? error.message : error}`);
        }
    }
}
...
function buildEndpointPose(systemID, seed) {
    ...
    const placement = buildAnchorRelativeSignaturePlacement(anchors.map((anchor) => ({
...
function buildEndpointRecord(systemID, typeRecord, options = {}) {
    ...
    const typeName = String(typeRecord && (typeRecord.typeName || typeRecord.name) || "").trim() ||
    ...
    return {
        ...
        targetID: String(options.targetID || "").trim().toUpperCase() ||
            (endpointID > 0
...
function projectRemainingMass(pair, nowMs) {
    ...
    const lastMassStateAtMs = Math.max(0, toInt(pair && pair.lastMassStateAtMs, pair && pair.createdAtMs));
    const elapsedMs = Math.max(0, toInt(nowMs, 0) - lastMassStateAtMs);
    if (elapsedMs <= 0) {
        return remainingMass;
    }
    const regenerationAmount = ((elapsedMs / WORMHOLE_MASS_REGEN_PERIOD_MS) * massRegeneration) +
...
function maybePassivelyRevealDestination(pair, nowMs) {
    ...
    const checkIndex = Math.floor(Math.max(0, toInt(nowMs, 0) - (Math.max(0, toInt(pair.createdAtMs, 0)) ||
        toInt(nowMs, 0))) / WORMHOLE_PASSIVE_K162_CHECK_INTERVAL_MS);
    const revealProgress = 1 - (remainingLifetimeMs / WORMHOLE_PASSIVE_K162_REVEAL_THRESHOLD_MS);
...
function interpolateAnchoredValue(anchors, value) {
    ...
    for (let index = 1; index < anchors.length; index += 1) {
        ...
        if (numericValue <= right.shipMass) {
            ...
            const progress = rightMass <= leftMass
                ? 0
...
function buildEntityFromPair(pair, role, nowMs = Date.now()) {
    ...
    return {
        ...
        itemName: endpoint.code === K162_CODE
            ? "Wormhole K162"
...
function createStaticPair(table, systemID, slot, nowMs) {
    ...
    const generationSeed = `${slot.slotKey}:${Math.max(0, toInt(table.staticSlotsByKey[slot.slotKey] && table.staticSlotsByKey[slot.slotKey].generation, 0)) + 1}`;
    const destinationSystemID = resolveDestinationSystemID(sourceSystemID, slot.targetClassID, generationSeed);
    ...
    const lifetimeMinutes = Math.max(1, Math.round(Math.max(1, toInt(slot.lifetimeMinutes, sourceType.lifetimeMinutes || 960)) *
...
function selectSourceSystemIDForRandomProfile(table, profile, seed, occupiedSystemIDsOverride = null) {
    ...
    const occupiedSystemIDs = occupiedSystemIDsOverride instanceof Set
        ...
        : new Set(Object.values(table.pairsByID || {})
...
function createRandomPairFromProfile(table, profile, nowMs, options = {}) {
    ...
    const profileKey = String(profile && profile.profileKey || "").trim();
    const sourceSystemID = selectSourceSystemIDForRandomProfile(table, profile, options.seed || `${profileKey}:${nowMs}:${table.nextPairSequence}`, options.occupiedSystemIDs || null);
    if (!sourceSystemID) {
        return null;
    }
    const destinationClassID = toInt(profile && profile.destinationClassID, 0) ||
        toInt(sourceType.targetClassID, 0);
    const destinationSystemID = resolveDestinationSystemID(sourceSystemID, destinationClassID, `${profileKey}:${sourceSystemID}:${nowMs}:${sourceType.code}`);
    if (!destinationSystemID) {
        return null;
    }
    const pairID = allocatePairID(table);
    const lifetimeMinutes = Math.max(1, Math.round(Math.max(1, toInt(profile && profile.lifetimeMinutes, sourceType.lifetimeMinutes || 960)) * Math.max(0.0001, toFiniteNumber(config.wormholeLifetimeScale, 1))));
    const totalMass = Math.max(0, toInt(profile && profile.maxStableMass, sourceType.maxStableMass || 0));
    const pair = {
        pairID,
        kind: "random",
        ...
        massRegeneration: Math.max(0, toInt(profile && profile.massRegeneration, sourceType.massRegeneration || 0)),
        maxJumpMass: Math.max(0, toInt(profile && profile.maxJumpMass, sourceType.maxJumpMass || 0)),
        lifetimeMinutes,
        lastMassStateAtMs: nowMs,
        massRegenRemainder: 0,
        lastPassiveRevealCheckAtMs: 0,
...
function createRandomPair(table, systemID, nowMs, options = {}) {
    ...
    const destinationSystemID = resolveDestinationSystemID(sourceSystemID, toInt(sourceType.targetClassID, targetClassID), `${sourceSystemID}:${nowMs}:${sourceType.code}`);
    const k162Type = getK162TypeRecord();
    if (!sourceType || !destinationSystemID || !k162Type) {
        return null;
    }
    const pairID = allocatePairID(table);
    const lifetimeMinutes = Math.max(1, Math.round(Math.max(1, toInt(sourceType.lifetimeMinutes, 960)) *
        Math.max(0.0001, toFiniteNumber(config.wormholeLifetimeScale, 1))));
    const pair = {
        pairID,
        kind: "random",
...
function countEffectiveRandomPairsForProfile(table, profileKey, nowMs) {
    ...
    const respawnDelayMs = Math.max(0, toInt(config.wormholeWanderingRespawnDelaySeconds, 60)) * 1000;
    let count = 0;
    for (const pair of Object.values(table.pairsByID || {})) {
...
function collectRandomProfileStats(table, nowMs) {
    ...
    for (const pair of Object.values(table.pairsByID || {})) {
        ...
        const state = String(pair && pair.state || "").toLowerCase();
        const collapseAtMs = Math.max(0, toInt(pair && pair.collapseAtMs, 0));
        const countsAsPresent = state === "active" || (respawnDelayMs > 0 &&
            collapseAtMs > 0 &&
...
function ensureRandomPairsInTable(table, nowMs) {
    ...
    const { countsByProfileKey, occupiedSystemIDsByProfileKey, } = collectRandomProfileStats(table, nowMs);
    for (const profile of listWanderingProfiles()) {
        ...
        for (let index = 0; index < deficit; index += 1) {
            const createdPair = createRandomPairFromProfile(table, profile, nowMs + createdPairs.length + index, {
                occupiedSystemIDs,
            });
            if (createdPair) {
                createdPairs.push(createdPair);
...
function seedUniverseStaticsInTable(table, nowMs) {
    ...
    for (const system of listSystems()) {
        ...
        createdPairs.push(...ensureStaticPairsInTable(table, toInt(system.solarSystemID, 0), nowMs));
    }
...
function collectVisiblePairsForSystem(table, systemID) {
    ...
    return collectPairsForSystem(table, numericSystemID)
        .filter((pair) => (toInt(pair.source && pair.source.systemID, 0) === numericSystemID &&
...
function syncSceneEntities(scene, nowMs) {
    ...
    const desiredSecondarySunEntity = wormholeEnvironmentRuntime.buildSecondarySunEntity(scene.systemID);
    if (desiredSecondarySunEntity) {
        desiredByID.set(toInt(desiredSecondarySunEntity.itemID, 0), desiredSecondarySunEntity);
    }
    const desiredEffectBeaconEntity = wormholeEnvironmentRuntime.buildEffectBeaconEntity(scene.systemID);
    if (desiredEffectBeaconEntity) {
        desiredByID.set(toInt(desiredEffectBeaconEntity.itemID, 0), desiredEffectBeaconEntity);
    }
    for (const pair of collectVisiblePairsForSystem(table, scene.systemID)) {
        if (toInt(pair.source && pair.source.systemID, 0) === toInt(scene.systemID, 0) &&
            isEndpointVisible(pair.source)) {
            desiredByID.set(pair.source.endpointID, buildEntityFromPair(pair, "source", nowMs));
        }
        if (toInt(pair.destination && pair.destination.systemID, 0) === toInt(scene.systemID, 0) &&
            isEndpointVisible(pair.destination)) {
    ...
    if (addedGlobalEntities.length > 0 &&
...
function ensureSystemStatics(systemID, nowMs = Date.now()) {
    ...
    if (toInt(currentTable.universeSeededAtMs, 0) > 0 &&
    ...
    const result = mutateState((table) => {
        ...
        if (toInt(table.universeSeededAtMs, 0) <= 0) {
            ...
            createdPairs = seededPairs
                .filter((pair) => toInt(pair && pair.source && pair.source.systemID, 0) === numericSystemID ||
    ...
    if (result.success === true && createdPairs.length > 0) {
        notifySignatureStateChanged(createdPairs.flatMap((pair) => collectVisibleSignatureSystemIDsForPair(pair)));
    }
    return {
...
function handleSceneCreated(scene) {
    ...
    if (totalElapsedMs >= 500) {
        log.info(`[Wormholes] Scene startup profile system=${toInt(scene.systemID, 0)} totalMs=${totalElapsedMs} ensureMs=${ensureElapsedMs} syncMs=${syncElapsedMs} summaryMs=${summaryElapsedMs}`);
    }
}
function tickScene(scene, nowMs = Date.now()) {
    ...
    mutateState((table) => {
        ...
        for (const pair of Object.values(table.pairsByID || {})) {
            ...
            const shouldCollapse = toInt(pair.expiresAtMs, 0) > 0 &&
            ...
            if (beforeState !== String(pair.state || "").toLowerCase() ||
...
function prepareJump(endpointID, characterID, shipMass, nowMs = Date.now()) {
    ...
    const result = mutateState((table) => {
        ...
        const existingPolarization = table.polarizationByCharacter[String(numericCharacterID)] &&
        ...
        const projectedRemainingMassAfterJump = Math.max(0, projectRemainingMass(pair, nowMs) - numericShipMass);
        prepareJumpResult = {
            success: true,
            data: {
                pairID: pair.pairID,
                ...
                willCollapseAfterJump: (projectRemainingMass(pair, nowMs) > 0 && projectedRemainingMassAfterJump <= 0) ||
...
function commitJump(endpointID, characterID, shipMass, nowMs = Date.now()) {
    ...
    const result = mutateState((table) => {
        ...
        if (toInt(pair.remainingMass, 0) <= 0 ||
...
function getPolarization(endpointID, characterID, nowMs = Date.now()) {
    const table = getStateView();
    const record = table.polarizationByCharacter[String(toInt(characterID, 0))] &&
...
function buildJumpSpawnState(pair, sourceRole, options = {}) {
    ...
    const randomSeed = options.randomSeed !== undefined && options.randomSeed !== null
        ...
        : null;
    const postJumpRemainingMass = Math.max(0, projectRemainingMass(pair, nowMs) - shipMass);
    const totalMass = Math.max(0, toFiniteNumber(pair && pair.totalMass, 0));
    const postJumpSizeRatio = totalMass > 0
        ? Math.max(0, Math.min(1, postJumpRemainingMass / totalMass))
        : 1;
    const baseSurfaceDistance = interpolateAnchoredValue(WORMHOLE_POST_JUMP_DISTANCE_ANCHORS, shipMass);
    const maxDeviation = WORMHOLE_FRESH_POST_JUMP_DEVIATION_METERS +
        ((1 - postJumpSizeRatio) *
    ...
    const surfaceDistance = Math.max(WORMHOLE_MIN_POST_JUMP_SURFACE_DISTANCE_METERS, Math.round(baseSurfaceDistance + signedDeviation));
    return {
        anchorType: "wormhole",
        anchorID: destination.endpointID,
        anchorName: destination.code === K162_CODE
            ? "Wormhole K162"
            : `Wormhole ${destination.code || destination.endpointID}`,
        direction: outwardDirection,
        position: addVectors(cloneVector(destination.position), scaleVector(outwardDirection, Math.max(toFiniteNumber(destination.radius, 3000) +
            surfaceDistance, toFiniteNumber(destination.radius, 3000) +
...
function listPairViewsFromTable(table, options = {}) {
    ...
    return pairs
        ...
        return (toInt(pair.source && pair.source.systemID, 0) === systemIDFilter ||
        ...
        return {
            ...
            destinationEnvironmentFamily: String((getSystemAuthority(pair.destination && pair.destination.systemID) || {}).environmentFamily || "").trim() || null,
            destinationEnvironmentEffectTypeID: toInt((getSystemAuthority(pair.destination && pair.destination.systemID) || {}).environmentEffectTypeID, 0),
            destinationEnvironmentEffectTypeName: String((getSystemAuthority(pair.destination && pair.destination.systemID) || {}).environmentEffectTypeName || "").trim() || null,
            ...
            visible: isEndpointVisible(pair.source) ||
                isEndpointVisible(pair.destination),
        };
    })
    ...
    for (const entry of entries) {
        ...
        for (const touchpoint of touchpoints) {
            ...
            const existing = summariesBySystemID.get(systemID) || {
                ...
                environmentEffectTypeName: String((getSystemAuthority(systemID) || {}).environmentEffectTypeName || "").trim() || null,
                codes: new Set(),
                pairIDs: new Set(),
            };
...
function logSceneSummary(scene, nowMs = Date.now()) {
    ...
    log.info(`[Wormholes] ${summary.systemName} (${summary.systemID}) loaded: ${summary.activePairCount} pair(s) | static ${summary.staticPairCount} | random ${summary.randomPairCount} | discovered ${summary.discoveredEndpointCount} | hidden ${summary.hiddenEndpointCount} | env ${summary.environmentFamily || "-"} | codes ${summary.codes.join(",") || "-"}`);
}
function warmRuntimeCache(nowMs = Date.now()) {
    ...
    if (elapsedMs >= 250) {
        log.info(`[Wormholes] Runtime cache warmed in ${elapsedMs}ms | active=${index.activePairCount} total=${index.totalPairCount} authoritySystems=${authoritySystemCount}`);
    }
    void nowMs;
    return {
        success: true,
...
//# sourceMappingURL=wormholeRuntime.js.map