...
const { buildSystemWideEffectsPayloadForSystem, buildEmptySystemWideEffectsPayload, } = require(path.join(__dirname, "./wormholeEnvironmentRuntime"));
const { buildBoundResult, jumpSessionToSolarSystem, } = require(path.join(__dirname, "../../../space/transitions"));
...
function syncSessionSystemWideEffectsForSystem(session, systemID) {
    ...
    const payload = buildSystemWideEffectsPayloadForSystem(systemID) ||
...
function completeWormholeJump({ session, sourceScene, sourceEndpointID, shipMass, prepareResult, }) {
    ...
    const destinationSystemID = toInt(prepareResult.data.destinationSystemID, 0);
    ...
    const jumpResult = jumpSessionToSolarSystem(session, destinationSystemID, {
        ...
        spawnStateOverride: wormholeRuntime.buildJumpSpawnState(prepareResult.data.pair, prepareResult.data.role, {
            shipMass,
            nowMs: Date.now(),
    ...
    wormholeRuntime.commitJump(sourceEndpointID, session.characterID, shipMass, commitNowMs);
    if (sourceScene) {
    ...
    if (destinationScene) {
        ...
        const destinationEndpointID = toInt(prepareResult.data.destinationEndpointID, 0);
        ...
        if (arrivalShipEntity && destinationWormholeEntity) {
            destinationScene.broadcastSpecialFx(destinationWormholeEntity.itemID, "effects.WormholeActivity", {
...
class WormholeMgrService extends BaseService {
    ...
    Handle_GetWormholePolarization(args, session) {
        ...
        const polarization = wormholeRuntime.getPolarization(endpointID, session.characterID, Date.now());
        if (!polarization) {
            return null;
        }
        const endFileTime = BigInt(toInt(polarization.endAtMs, 0)) * FILETIME_TICKS_PER_MS +
    ...
    Handle_WormholeJump(args, session) {
        ...
        const surfaceDistance = Math.max(0, magnitude(subtractVectors(cloneVector(shipEntity.position), cloneVector(sourceEntity.position))) - Math.max(0, toFiniteNumber(sourceEntity.radius, 0)));
        ...
        const prepareResult = wormholeRuntime.prepareJump(endpointID, session.characterID, shipMass, Date.now());
        ...
        const destinationSystemID = toInt(prepareResult.data && prepareResult.data.destinationSystemID, 0);
        if (destinationSystemID <= 0) {
            return null;
        }
        scene.broadcastSpecialFx(shipEntity.itemID, "effects.JumpOutWormhole", {
        ...
        scene.broadcastSpecialFx(sourceEntity.itemID, "effects.WormholeActivity", {
            excludedSession: session,
        }, shipEntity);
...
//# sourceMappingURL=wormholeMgrService.js.map