...
const MAX_ACTIVE_PROBES = Math.max(1, Number(explorationAuthority.getScanContracts().maxProbes) || 8);
const MAX_PROBE_DIST_FROM_SUN_SQUARED = Math.max(1, Number(explorationAuthority.getScanContracts().maxProbeDistanceFromSunSquared) ||
...
function clampVectorToProbeBounds(vector = [0, 0, 0]) {
    ...
    const distanceSquared = (normalized[0] ** 2) +
        (normalized[1] ** 2) +
        (normalized[2] ** 2);
...
function normalizeProbeRecord(record = {}) {
    ...
    const resolvedRange = resolveProbeRangeContract(typeID, record.rangeStep, record.scanRange);
    return {
        probeID,
        ...
        scopeMetadata: record.scopeMetadata && typeof record.scopeMetadata === "object"
            ? buildChildEntityScopeMetadata(record.scopeMetadata)
            : {},
    };
}
function isValidPersistedProbeRecord(record = {}) {
    return (Math.max(0, toInt(record.probeID, 0)) > 0 &&
...
function pruneState(table = {}, options = {}) {
    ...
    for (const characterState of Object.values(normalized.charactersByID || {})) {
        for (const [probeKey, probeRecord] of Object.entries(characterState.probesByID || {})) {
            if (!isValidPersistedProbeRecord(probeRecord) ||
    ...
    mutateState((table) => {
        ...
        for (const [probeID, probePatch] of probeMap.entries()) {
            ...
            const nextRecord = normalizeProbeRecord({
                ...
                destination: probePatch && probePatch.destination
                    ? probePatch.destination
                    : (probePatch && probePatch.pos
                        ? probePatch.pos
                        : existing.destination),
                scanRange: probePatch && Number.isFinite(Number(probePatch.scanRange))
                    ? probePatch.scanRange
                    : existing.scanRange,
                rangeStep: probePatch && Number.isFinite(Number(probePatch.rangeStep))
                    ? probePatch.rangeStep
                    : existing.rangeStep,
                state: probePatch && Number.isFinite(Number(probePatch.state))
                    ? probePatch.state
                    : existing.state,
                lastSeenAtMs: nowMs,
            });
...
function launchCharacterProbes(characterID, systemID, typeID, requestedCount, options = {}) {
    ...
    const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
    const nowFileTime = (BigInt(nowMs) * 10000n) + 116444736000000000n;
    ...
    const launchRingRadiusMeters = Math.max(0, toFiniteNumber(options.launchRingRadiusMeters, DEFAULT_LAUNCH_RING_RADIUS_METERS));
    ...
    mutateState((table) => {
        ...
        for (let index = 0; index < launchCount; index += 1) {
            ...
            const launchOffset = buildLaunchOffset(activeProbeCount + index, launchSlotCount, launchRingRadiusMeters);
            const probe = normalizeProbeRecord({
                ...
                rangeStep,
                state,
                expiry: (nowFileTime + (BigInt(expirySeconds) * 10000000n)).toString(),
                lastSeenAtMs: nowMs,
            });
...
function overrideCharacterProbeExpiry(characterID, durationSeconds, options = {}) {
    ...
    const expiresAt = ((BigInt(Date.now()) * 10000n) +
        116444736000000000n +
        (BigInt(clampedDurationSeconds) * 10000000n)).toString();
    const updated = [];
    mutateState((table) => {
        const characterState = ensureCharacterState(table, numericCharacterID);
        if (!characterState) {
...
//# sourceMappingURL=probeRuntimeState.js.map