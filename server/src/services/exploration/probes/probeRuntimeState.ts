...
function normalizeProbeRecord(record: Record<string, any> = {}) {
  ...
  return {
    ...
    scopeMetadata:
      record.scopeMetadata && typeof record.scopeMetadata === "object"
        ? buildChildEntityScopeMetadata(record.scopeMetadata)
        : {},
  };
}

function isValidPersistedProbeRecord(record: Record<string, any> = {}) {
  return (
...
function isProbeExpired(record: Record<string, any> = {}, nowFileTime = toFileTimeFromMs(Date.now())) {
  try {
...
function normalizeCharacterState(record: Record<string, any> = {}) {
  const characterID = Math.max(0, toInt(record.characterID, 0));
  const probesByID: Record<string, any> = {};
  for (const [probeKey, probeRecord] of Object.entries<any>(record.probesByID || {})) {
...
function normalizeState(table: Record<string, any> = {}) {
  ...
  for (const [characterKey, characterState] of Object.entries<any>(table.charactersByID || {})) {
...
function pruneState(table: Record<string, any> = {}, options: Record<string, any> = {}) {
  ...
  for (const characterState of Object.values(normalized.charactersByID || {})) {
    for (const [probeKey, probeRecord] of Object.entries<any>(characterState.probesByID || {})) {
      if (
...
function getCharacterSystemProbes(characterID, systemID) {
  ...
  return Object.values<any>(characterState.probesByID || {})
    .filter((probe) => Math.max(0, toInt(probe.systemID, 0)) === numericSystemID)
    .sort((left, right) => toInt(left.probeID, 0) - toInt(right.probeID, 0));
}

function removeInvalidCharacterProbes(characterID, options: Record<string, any> = {}) {
  const numericCharacterID = Math.max(0, toInt(characterID, 0));
...
function removeExpiredCharacterProbes(characterID, options: Record<string, any> = {}) {
  const numericCharacterID = Math.max(0, toInt(characterID, 0));
  const numericSystemID = Math.max(0, toInt(options.systemID, 0));
  const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
  if (numericCharacterID <= 0) {
...
function upsertCharacterProbes(characterID, systemID, probeMap, options: Record<string, any> = {}) {
  const numericCharacterID = Math.max(0, toInt(characterID, 0));
  const numericSystemID = Math.max(0, toInt(systemID, 0));
  const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
...
function synchronizeCharacterProbeGeometry(characterID, systemID, probeMap, options: Record<string, any> = {}) {
  const numericCharacterID = Math.max(0, toInt(characterID, 0));
  const numericSystemID = Math.max(0, toInt(systemID, 0));
  const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
  if (numericCharacterID <= 0 || numericSystemID <= 0 || !(probeMap instanceof Map)) {
...
function setCharacterProbeActivity(characterID, probeIDs, active, options: Record<string, any> = {}) {
  const numericCharacterID = Math.max(0, toInt(characterID, 0));
  const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
...
function removeCharacterProbes(characterID, probeIDs, options: Record<string, any> = {}) {
  const numericCharacterID = Math.max(0, toInt(characterID, 0));
  const nowMs = Math.max(0, toInt(options.nowMs, Date.now()));
  const normalizedProbeIDs = [...new Set((Array.isArray(probeIDs) ? probeIDs : [probeIDs])
    .map((probeID) => Math.max(0, toInt(probeID, 0)))
...
function launchCharacterProbes(characterID, systemID, typeID, requestedCount, options: Record<string, any> = {}) {
  const numericCharacterID = Math.max(0, toInt(characterID, 0));
  const numericSystemID = Math.max(0, toInt(systemID, 0));
  ...
  mutateState((table) => {
    ...
    const activeProbeCount = Object.values<any>(characterState.probesByID || {})
      .filter((probe) => Math.max(0, toInt(probe.systemID, 0)) === numericSystemID)
      .filter((probe) => Math.max(0, toInt(probe.state, 0)) > 0)
      .length;
...
function overrideCharacterProbeExpiry(characterID, durationSeconds, options: Record<string, any> = {}) {
  const numericCharacterID = Math.max(0, toInt(characterID, 0));
  const numericSystemID = Math.max(0, toInt(options.systemID, 0));
  ...
  mutateState((table) => {
    ...
    for (const probe of Object.values<any>(characterState.probesByID || {})) {
