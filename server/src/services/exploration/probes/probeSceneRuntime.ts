...
} = require(path.join(__dirname, "../../../space/destiny/commands/probe.js"));
const visibilityControlState = require(path.join(
  __dirname,
  "../../../space/destiny/visibility/controlState",
));
const {
  resolveItemByTypeID,
...
function buildVector(source = null, fallback: Record<string, any> = { x: 0, y: 0, z: 0 }) {
...
function buildProbeSceneEntity(probe: Record<string, any> = {}, options: Record<string, any> = {}) {
...
function ensureProbeEntitiesForSession(session, probes: any[] = [], options: Record<string, any> = {}) {
...
function removeProbeEntitiesForSession(session, probeIDs: any[] = [], options: Record<string, any> = {}) {
  const systemID = normalizeNumber(
    options.systemID ||
      (session && (session.solarsystemid2 || session.solarsystemid)) ||
      (session && session._space && session._space.systemID),
