...
const { buildDict, buildFiletimeLong, buildKeyVal, buildMarshalRealVectorList, buildMarshalReal, currentFileTime, normalizeBigInt, unwrapMarshalValue, } = require(path.join(__dirname, "../../_shared/serviceHelpers"));
...
const PROBE_SCAN_GROUP_SIGNATURES = Number(explorationAuthority.getScanContracts().probeScanGroups &&
    explorationAuthority.getScanContracts().probeScanGroups.signatures) || 3;
const PROBE_SCAN_GROUP_STRUCTURES = Number(explorationAuthority.getScanContracts().probeScanGroups &&
    explorationAuthority.getScanContracts().probeScanGroups.structures) || 5;
const PROBE_SCAN_GROUP_SHIPS = Number(explorationAuthority.getScanContracts().probeScanGroups &&
    explorationAuthority.getScanContracts().probeScanGroups.ships) || 4;
const PROBE_SCAN_GROUP_DRONES = Number(explorationAuthority.getScanContracts().probeScanGroups &&
    explorationAuthority.getScanContracts().probeScanGroups.drones) || 6;
const PROBE_SCAN_GROUP_NPCS = Number(explorationAuthority.getScanContracts().probeScanGroups &&
    explorationAuthority.getScanContracts().probeScanGroups.npcs) || 10;
const PROBE_SCAN_GROUP_ORBITALS = Number(explorationAuthority.getScanContracts().probeScanGroups &&
    explorationAuthority.getScanContracts().probeScanGroups.orbitals) || 11;
const PROBE_SCAN_GROUP_DEPLOYABLES = Number(explorationAuthority.getScanContracts().probeScanGroups &&
    explorationAuthority.getScanContracts().probeScanGroups.deployables) || 12;
const PROBE_SCAN_GROUP_SOVEREIGNTY = Number(explorationAuthority.getScanContracts().probeScanGroups &&
    explorationAuthority.getScanContracts().probeScanGroups.sovereignty) || 13;
const PROBE_SCAN_GROUP_ABYSSAL_TRACES = Number(explorationAuthority.getScanContracts().probeScanGroups &&
    explorationAuthority.getScanContracts().probeScanGroups.abyssalTraces) || 14;
const GROUP_COSMIC_SIGNATURE = Number(explorationAuthority.getSignatureTypeDefinition("wormhole") &&
    explorationAuthority.getSignatureTypeDefinition("wormhole").inventoryGroupID) || 502;
const ATTRIBUTE_SCAN_WORMHOLE_STRENGTH = explorationAuthority.getScanStrengthAttribute("wormhole") || 1908;
...
function normalizeScanBonuses(value) {
    const rawBonuses = value && typeof value === "object" && (value.type || value.name || value.header)
        ? unwrapMarshalValue(value)
        : value;
    const scanBonuses = {};
    for (const bonusID of PROBE_SCAN_BONUS_IDS) {
        const rawOrigins = rawBonuses &&
            typeof rawBonuses === "object" &&
            rawBonuses[bonusID] &&
            typeof rawBonuses[bonusID] === "object"
            ? rawBonuses[bonusID]
            : {};
        scanBonuses[bonusID] = Object.fromEntries(PROBE_SCAN_BONUS_ORIGINS.map((originID) => [
            originID,
    ...
    for (const value of values) {
        ...
        for (const bonusID of PROBE_SCAN_BONUS_IDS) {
            for (const originID of PROBE_SCAN_BONUS_ORIGINS) {
                output[bonusID][originID] += toFiniteNumber(scanBonuses &&
                    scanBonuses[bonusID] &&
                    scanBonuses[bonusID][originID], 0);
            }
        }
    }
    return output;
...
function hasAnyScanBonus(scanBonuses = {}) {
    ...
    return PROBE_SCAN_BONUS_IDS.some((bonusID) => PROBE_SCAN_BONUS_ORIGINS.some((originID) => Math.abs(toFiniteNumber(normalized[bonusID][originID], 0)) > 1e-9));
}
function sumScanBonus(scanBonuses = {}, bonusID) {
    const normalized = normalizeScanBonuses(scanBonuses);
    const bonuses = normalized[String(bonusID || "")] || {};
    return PROBE_SCAN_BONUS_ORIGINS.reduce((sum, originID) => sum + toFiniteNumber(bonuses[originID], 0), 0);
}
...
function buildScanBonusesDict(scanBonuses = {}) {
    return buildDict(PROBE_SCAN_BONUS_IDS.map((bonusID) => [
        bonusID,
        buildDict(PROBE_SCAN_BONUS_ORIGINS.map((originID) => [
            originID,
            buildMarshalReal(toFiniteNumber(scanBonuses &&
                scanBonuses[bonusID] &&
                scanBonuses[bonusID][originID], 0), 0),
        ])),
    ]));
}
...
function normalizeProbeRecord(rawProbeID, rawProbe = {}) {
    const normalizedProbe = rawProbe && typeof rawProbe === "object" && (rawProbe.type || rawProbe.name || rawProbe.header)
        ? unwrapMarshalValue(rawProbe)
        : rawProbe;
    const probeID = toInt(rawProbeID, toInt(normalizedProbe && normalizedProbe.probeID, 0));
    if (probeID <= 0) {
        return null;
    }
    const initialPosition = clonePosition(normalizedProbe && (normalizedProbe.pos || normalizedProbe.position));
    const destination = clonePosition(normalizedProbe && normalizedProbe.destination, initialPosition);
    ...
    const resolvedRange = probeRuntimeState.resolveProbeRangeContract(typeID, normalizedProbe && normalizedProbe.rangeStep, normalizedProbe && normalizedProbe.scanRange);
    const state = toInt(normalizedProbe && normalizedProbe.state, 1);
    const expiry = normalizedProbe && Object.prototype.hasOwnProperty.call(normalizedProbe, "expiry")
        ? normalizeBigInt(normalizedProbe.expiry, 0n).toString()
        : (currentFileTime() +
            (BigInt(DEFAULT_PROBE_EXPIRY_SECONDS) * 10000000n)).toString();
    const scanBonuses = normalizeScanBonuses(normalizedProbe && normalizedProbe.scanBonuses);
    return {
        probeID,
        typeID,
        pos,
...
function normalizeProbeMap(rawProbes) {
    return new Map(extractProbeEntries(rawProbes)
...
function normalizeProbePatchMap(rawProbes) {
    return new Map(extractProbeEntries(rawProbes)
        ...
        const normalizedProbe = rawProbe &&
        ...
        return [
            probeID,
            {
                ...
                ...(() => {
                    const resolvedRange = probeRuntimeState.resolveProbeRangeContract(toInt(normalizedProbe && normalizedProbe.typeID, 0), normalizedProbe && normalizedProbe.rangeStep, normalizedProbe && normalizedProbe.scanRange);
                    return {
                        destination: clonePosition(normalizedProbe && normalizedProbe.destination, clonePosition(normalizedProbe && (normalizedProbe.pos || normalizedProbe.position))),
                        scanRange: resolvedRange.scanRange,
                        rangeStep: resolvedRange.rangeStep,
...
function buildProbeDict(probeMap) {
    return buildDict([...probeMap.values()].map((probe) => [
        probe.probeID,
...
function isCombatScannerProbe(probeOrTypeID) {
    const typeID = toInt(probeOrTypeID && typeof probeOrTypeID === "object"
        ? probeOrTypeID.typeID
        : probeOrTypeID, 0);
    return toFiniteNumber(getProbeTypeAttributes(typeID)[String(ATTRIBUTE_PROBE_CAN_SCAN_SHIPS)], 0) > 0;
}
function resolveProbeStrengthAtRange(probe) {
    const attributes = getProbeTypeAttributes(probe && probe.typeID);
    const baseStrength = Math.max(0, toFiniteNumber(attributes[String(ATTRIBUTE_BASE_SENSOR_STRENGTH)], 0));
    const rangeFactor = Math.max(1, toFiniteNumber(attributes[String(ATTRIBUTE_RANGE_FACTOR)], 2));
    const rangeStep = Math.max(1, toInt(probe && probe.rangeStep, 1));
    const strengthAtRange = baseStrength / (rangeFactor ** (rangeStep - 1));
    return Math.max(0, strengthAtRange * resolveScanBonusMultiplier(probe && probe.scanBonuses, "strength"));
}
function resolveTargetRequiredStrength(site) {
    const explicitStrength = toFiniteNumber(site && (site.probeTargetRequiredStrength ?? site.requiredScanStrength), 0);
...
function resolveEntityScanGroup(entity, metadata = null) {
    ...
    const categoryID = toInt(entity && entity.categoryID, toInt(metadata && metadata.categoryID, 0));
...
function isEntityHiddenFromCombatProbes(entity) {
    ...
    if (entity.cloakState && typeof entity.cloakState === "object") {
        return (entity.cloakState.active === true ||
...
function resolveEntitySensorStrength(entity, attributes = {}) {
    ...
    return Math.max(explicitStrength, ...SENSOR_STRENGTH_ATTRIBUTE_IDS.map((attributeID) => toFiniteNumber(attributes[String(attributeID)], 0)));
}
...
function buildCombatTargetSite(entity) {
    ...
    const signatureRadius = Math.max(1, toFiniteNumber(entity && entity.signatureRadius, toFiniteNumber(attributes[String(ATTRIBUTE_SIGNATURE_RADIUS)], toFiniteNumber(metadata && metadata.radius, 1))));
    ...
    const deviationMeters = Math.max(10_000, Math.min(149_597_870_700, requiredScanStrength * 5_000_000));
    return {
        siteID: itemID,
        targetID: itemID,
        itemID,
...
function scoreProbeCoverage(site, probe) {
    ...
    const effectiveProbePosition = clonePosition(probe && (probe.destination || probe.pos), clonePosition(probe && probe.pos));
...
function resolveFormationGeometryQuality(coverageEntries = []) {
    ...
    for (let leftIndex = 0; leftIndex < strongestFour.length; leftIndex += 1) {
        for (let rightIndex = leftIndex + 1; rightIndex < strongestFour.length; rightIndex += 1) {
            ...
            const averageRange = Math.max(1, (toFiniteNumber(leftProbe && leftProbe.scanRange, 0) +
                toFiniteNumber(rightProbe && rightProbe.scanRange, 0)) / 2);
            normalizedSeparations.push(distanceBetween(clonePosition(leftProbe && (leftProbe.destination || leftProbe.pos)), clonePosition(rightProbe && (rightProbe.destination || rightProbe.pos))) / averageRange);
        }
    }
    if (normalizedSeparations.length <= 0) {
        return 0;
    }
    const averageSeparation = normalizedSeparations.reduce((sum, separation) => sum + separation, 0) / normalizedSeparations.length;
...
function buildResultPosition(site, certainty) {
    ...
    const blendRatio = Math.max(0, Math.min(1, (toFiniteNumber(certainty, 0) - PROBE_RESULT_UNUSABLE) / (PROBE_RESULT_PERFECT - PROBE_RESULT_UNUSABLE)));
...
function buildResultDeviation(site, certainty, resolvedPosition, scanBonuses = null) {
    ...
    const siteDeviation = Math.max(1, toFiniteNumber(site && site.deviation, 1) *
        resolveScanBonusMultiplier(scanBonuses, "deviation"));
    const positionalError = distanceBetween(actualPosition, resolvedPosition);
    return Math.max(1, Math.min(siteDeviation, positionalError > 0
        ? positionalError
...
function buildScanResultEntry(site, certainty, options = {}) {
    ...
    const deviationMeters = buildResultDeviation(site, certainty, resolvedPosition, options.scanBonuses);
...
function buildResolvedSignatureScanResults(systemID, options = {}) {
    ...
    return {
        durationMs: Math.max(1, toInt(options.durationMs, DEFAULT_PROBE_SCAN_DURATION_MS)),
        probes: new Map(),
        probeIDs: [],
...
function applyScanBonusesToProbeMap(probeMap, scanBonuses = null) {
    ...
    return new Map([...probeMap.entries()].map(([probeID, probe]) => [
        probeID,
        {
            ...probe,
...
function buildSignatureScanResults(systemID, rawProbes, options = {}) {
    const probeMap = applyScanBonusesToProbeMap(normalizeProbeMap(rawProbes), options.scanBonuses);
    ...
    const hasCombatScannerProbe = [...probeMap.values()].some((probe) => isCombatScannerProbe(probe));
...
//# sourceMappingURL=probeScanRuntime.js.map