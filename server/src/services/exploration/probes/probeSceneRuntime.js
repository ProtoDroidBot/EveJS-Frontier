...
const { applyProbePositionTargetCommand, } = require(path.join(__dirname, "../../../space/destiny/commands/probe.js"));
const visibilityControlState = require(path.join(__dirname, "../../../space/destiny/visibility/controlState"));
...
function buildProbeSceneEntity(probe = {}, options = {}) {
    ...
    return {
        ...(probe.scopeMetadata && typeof probe.scopeMetadata === "object"
            ? probe.scopeMetadata
            : {}),
        kind: "probe",
        systemID,
...
function ensureProbeEntitiesForSession(session, probes = [], options = {}) {
    const systemID = normalizeNumber(options.systemID ||
        (session && (session.solarsystemid2 || session.solarsystemid)) ||
        (session && session._space && session._space.systemID), 0);
    if (systemID <= 0) {
        return [];
    }
    ...
    const ownerID = normalizeNumber(options.ownerID ||
...
//# sourceMappingURL=probeSceneRuntime.js.map