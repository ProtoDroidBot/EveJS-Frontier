...
function normalizeScanBonuses(value) {
  const rawBonuses =
    value && typeof value === "object" && (value.type || value.name || value.header)
      ? unwrapMarshalValue(value)
      : value;
  const scanBonuses: Record<string, any> = {};
...
function hasAnyScanBonus(scanBonuses: Record<string, any> = {}) {
...
function sumScanBonus(scanBonuses: Record<string, any> = {}, bonusID) {
  const normalized = normalizeScanBonuses(scanBonuses);
...
function resolveScanBonusMultiplier(scanBonuses: Record<string, any> = {}, bonusID) {
  return Math.max(0, 1 + (sumScanBonus(scanBonuses, bonusID) / 100));
}

function buildScanBonusesDict(scanBonuses: Record<string, any> = {}) {
  return buildDict(
...
function extractProbeEntries(rawProbes) {
  ...
  if (typeof rawProbes === "object") {
    return Object.entries<any>(rawProbes);
  }

  return [];
}

function normalizeProbeRecord(rawProbeID, rawProbe: Record<string, any> = {}) {
...
function buildProbeKeyVal(probe: Record<string, any> = {}) {
  return buildKeyVal([
...
function resolveEntitySensorStrength(entity, attributes: Record<string, any> = {}) {
...
function listCombatTargetSites(options: Record<string, any> = {}) {
...
function resolveCertainty(coverageScores: any[] = []) {
...
function resolveFormationGeometryQuality(coverageEntries: any[] = []) {
  ...
  const normalizedSeparations: any[] = [];
...
function buildScanResultEntry(site, certainty, options: Record<string, any> = {}) {
  const actualPosition = [
    toFiniteNumber(site && site.actualPosition && site.actualPosition.x, 0),
    toFiniteNumber(site && site.actualPosition && site.actualPosition.y, 0),
    toFiniteNumber(site && site.actualPosition && site.actualPosition.z, 0),
...
function buildResolvedSignatureScanResults(systemID, options: Record<string, any> = {}) {
...
function buildSignatureScanResults(systemID, rawProbes, options: Record<string, any> = {}) {
  ...
  const sites = hasCombatScannerProbe
    ...
    : signatureSites;
  const results: any[] = [];
  const absentTargets: any[] = [];
  const perfectSignatureIDs: any[] = [];
