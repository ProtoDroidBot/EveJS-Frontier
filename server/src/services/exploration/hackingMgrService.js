...
function buildUtilityInventory(slotCount) {
    return Array.from({ length: Math.max(0, Math.min(8, toInt(slotCount, 0))) }, (_, index) => ({
        id: null,
...
function buildSyntheticBoard(difficulty = 1) {
    const coreHitpoints = Math.max(1, roundNumber(toFiniteNumber(difficulty, 1) * 10, 6));
    return [
        {
            id: 1,
...
function getAttemptKey(session) {
    return String(toPositiveInt(session && (session.characterID ||
        session.charid ||
        session.userid ||
        session.clientID ||
        session.clientId), 0) || "anonymous");
}
...
function getSceneEntity(scene, entityID) {
    ...
    return ((scene.dynamicEntities instanceof Map && scene.dynamicEntities.get(numericEntityID)) ||
        (scene.staticEntitiesByID instanceof Map && scene.staticEntitiesByID.get(numericEntityID)) ||
        null);
}
function getSessionShipEntity(scene, session) {
    if (!scene || !session) {
        return null;
    }
    ...
    return getSceneEntity(scene, session && ((session._space && session._space.shipID) ||
        session.shipID ||
        session.shipid ||
...
function resolveContainerTarget(targetID, session) {
    ...
    const shipEntity = getSessionShipEntity(scene, session);
    if (!scene ||
        !entity ||
        !shipEntity ||
...
function refreshAttemptTarget(attempt, session) {
    if (!attempt || toPositiveInt(attempt.targetID, 0) <= 0) {
        return true;
    }
    const target = resolveContainerTarget(attempt.targetID, session);
    if (!target) {
        return false;
    }
...
function buildStartEvents(attempt) {
    return [
        ...
        ...attempt.tiles.map((tile) => buildEvent(HACKING_CONSTANTS.EVENT_TILE_CREATED, cloneTile(tile))),
...
function destroyTargetContainer(target, options = {}) {
    ...
    return dungeonUniverseSiteService.destroyMaterializedContentEntity(target.scene, target.entity, {
...
function failContainerAttempt(attempt, session, nowMs) {
    ...
    const failureCount = Math.max(0, toInt(target.entity.dungeonSiteContentFailureCount, 0)) + 1;
    ...
    if (target.entity.dungeonSiteContentFailureExplodes === true && target.instanceID > 0) {
        return dungeonUniverseSiteService.triggerSiteEncounter(target.scene, target.instanceID, "hack_failure", {
            nowMs,
            session,
        });
    }
...
function maybeCompleteAttemptSite(attempt, session, nowMs) {
    ...
    // A hacked container in a MISSION dungeon satisfies the mission's hack objective; only fall back to
    // the exploration data/relic signature-site completion for non-mission containers.
    const missionResult = dungeonUniverseSiteService.maybeSatisfyMissionHackObjective(target.scene, target.entity, options);
    if (missionResult && missionResult.reason !== "not_mission_instance") {
        return missionResult;
    }
    return dungeonUniverseSiteService.maybeCompleteMaterializedDataRelicSite(target.scene, target.entity, options);
}
class HackingMgrService extends BaseService {
    ...
    Handle_StartNewGameInstance(args = [], session = null) {
        const [targetID, gameType, moduleTypeID, difficulty, virusInitialCoherence, virusInitialStrength, virusSlots,] = normalizeCallArgs(args);
        const nowMs = Date.now();
        const target = resolveContainerTarget(targetID, session);
        const normalizedDifficulty = Math.max(1, roundNumber(toFiniteNumber(difficulty, 1), 6));
        const attempt = {
            key: getAttemptKey(session),
            targetID: toPositiveInt(targetID, 0),
            target,
            gameType: toInt(gameType, target && String(target.entity.dungeonSiteContentAnalyzer || "") === "relic"
    ...
    StartNewGameInstance(targetID, gameType, moduleTypeID, difficulty, coherence, strength, slots, session = null) {
        return this.Handle_StartNewGameInstance([targetID, gameType, moduleTypeID, difficulty, coherence, strength, slots], session);
    }
    Handle_ClickedOnTile(args = [], session = null) {
        ...
        if (!refreshAttemptTarget(attempt, session)) {
        ...
        if (tile.type === HACKING_CONSTANTS.TYPE_CORE) {
            ...
            markContainerState(attempt.target, HACKING_CONSTANTS.HACKING_STATE_HACKED, nowMs);
            shouldEvaluateSiteCompletion = true;
            events.push(buildEvent(HACKING_CONSTANTS.EVENT_ATTACK, {
                attackerResult: buildVirusResult(attempt),
                defenderResult: cloneTile(tile),
            }), buildEvent(HACKING_CONSTANTS.EVENT_OBJECT_KILLED, cloneTile(tile)), buildEvent(HACKING_CONSTANTS.EVENT_GAME_WON, {
                targetID: attempt.targetID || null,
            }));
            ACTIVE_ATTEMPTS.delete(key);
        }
...
//# sourceMappingURL=hackingMgrService.js.map