...
const ATTRIBUTE_BASE_SCAN_RANGE =
  getAttributeIDByNames("baseScanRange") || 1370;
const ATTRIBUTE_RANGE_FACTOR =
  getAttributeIDByNames("rangeFactor") || 1373;
...
function extractProbeIDsFromValue(value) {
  ...
  if (unwrapped && typeof unwrapped === "object") {
    return Object.values<any>(unwrapped)
...
function isPositionInsideActiveScanInhibitor(systemID, position, scene, options: Record<string, any> = {}) {
  if (!position) {
    return false;
  }
...
function buildSignatureEntryMap(systemID, options: Record<string, any> = {}) {
...
function buildAnomalyEntryMap(systemID, options: Record<string, any> = {}) {
  return new Map<any, any>(
...
function ensureAnomalySnapshot(systemID, options: Record<string, any> = {}) {
...
function ensureSignatureSnapshot(systemID, options: Record<string, any> = {}) {
  const numericSystemID = normalizeNumber(systemID, 0);
  if (numericSystemID <= 0) {
    return new Map();
  }
...
function buildStructureEntryMap(systemID, options: Record<string, any> = {}) {
  return new Map<any, any>(
...
function buildStaticSiteEntryMap(systemID, options: Record<string, any> = {}) {
  return new Map<any, any>(
...
function ensureStructureSnapshot(systemID, options: Record<string, any> = {}) {
  const numericSystemID = normalizeNumber(systemID, 0);
  if (numericSystemID <= 0) {
    return new Map();
  }
...
function ensureStaticSiteSnapshot(systemID, options: Record<string, any> = {}) {
  const numericSystemID = normalizeNumber(systemID, 0);
  if (numericSystemID <= 0) {
    return new Map();
  }
...
function refreshSignalTrackerSnapshots(systemID, options: Record<string, any> = {}) {
  const numericSystemID = normalizeNumber(systemID, 0);
  if (numericSystemID <= 0) {
    return null;
  }
...
function notifySignatureDeltaForSystem(systemID, options: Record<string, any> = {}) {
  const numericSystemID = normalizeNumber(systemID, 0);
  if (numericSystemID <= 0) {
    return;
  }
  ...
  const removedSiteIDs = new Set<any>();
...
function notifyAnomalyDeltaForSystem(systemID, options: Record<string, any> = {}) {
  const numericSystemID = normalizeNumber(systemID, 0);
  if (numericSystemID <= 0) {
    return;
  }
...
function notifyStructureDeltaForSystem(systemID, options: Record<string, any> = {}) {
  const numericSystemID = normalizeNumber(systemID, 0);
  if (numericSystemID <= 0) {
    return;
  }
...
function notifyFullStateRefreshForSystem(systemID, options: Record<string, any> = {}) {
  const numericSystemID = normalizeNumber(systemID, 0);
  if (numericSystemID <= 0) {
    return {
...
function scheduleNotification(session, name, payload: any[] = []) {
  if (!session || typeof session.sendNotification !== "function") {
    return;
  }
  setImmediate(() => {
...
function scheduleProbeRemovalNotifications(session, probeIDs: any[] = []) {
...
function scheduleProbeReplayNotifications(session, probes: any[] = []) {
...
function restoreRecoveredProbeCharges(session, characterID, removedProbes: any[] = [], options: Record<string, any> = {}) {
...
function handleWormholeSignatureStateChange(systemIDs: any[] = []) {
  for (const systemID of Array.isArray(systemIDs) ? systemIDs : [systemIDs]) {
    notifySignatureDeltaForSystem(systemID);
  }
}

function emitProbeScanLifecycleNotifications(session, scanResult, options: Record<string, any> = {}) {
...
function scheduleProbeScanStopNotification(session, scanResult, options: Record<string, any> = {}) {
  if (!session || typeof session.sendNotification !== "function" || !scanResult) {
    return;
  }
...
function scheduleProbeMovementThenScan(session, probes: any[] = [], scanStarter, options: Record<string, any> = {}) {
...
function resolveCharacterProbeScanBonuses(characterID, options: Record<string, any> = {}) {
  const numericCharacterID = normalizeNumber(characterID, 0);
  ...
  if (shipID > 0) {
      ...
      for (const modifierInfo of Array.isArray(effectRecord && effectRecord.modifierInfo)
        ...
        if (
          ...
          addProbeScanBonus(
            ...
            "ship",
            toFiniteNumber(
              shipAttributes[normalizeNumber(modifierInfo.modifyingAttributeID, 0)],
              0,
            ),
          );
        }
    ...
    const onlineProbeLaunchers = fittedModules
      .filter((moduleItem) =>
        normalizeNumber(moduleItem && moduleItem.groupID, 0) === GROUP_SCAN_PROBE_LAUNCHER)
    ...
    for (const moduleItem of fittedModules) {
      ...
      addProbeScanBonus(
        ...
        "deviation",
        "modules",
        toFiniteNumber(moduleAttributes[String(ATTRIBUTE_SCAN_DEVIATION_BONUS_MODULE)], 0),
      );
      addProbeScanBonus(
        scanBonuses,
        "duration",
        "modules",
        toFiniteNumber(moduleAttributes[String(ATTRIBUTE_SCAN_DURATION_BONUS_MODULE)], 0),
      );
    }
  }
...
function resolveCharacterScanDurationBonusMultiplier(characterID, options: Record<string, any> = {}) {
  const numericCharacterID = normalizeNumber(characterID, 0);
  if (numericCharacterID <= 0) {
    return 1;
  }
  ...
  if (shipID > 0) {
    for (const moduleItem of getFittedModuleItems(numericCharacterID, shipID)) {
      ...
      const bonus = Number(
        getTypeDogmaAttributes(moduleItem && moduleItem.typeID)[
          String(ATTRIBUTE_SCAN_DURATION_BONUS_MODULE)
        ],
      );
      if (Number.isFinite(bonus) && bonus !== 0) {
...
function resolveProbeScanDurationMs(session, options: Record<string, any> = {}) {
...
function handleStructureOverlayStateChange(change: Record<string, any> = {}) {
  const systemIDs = new Set<any>();
...
class ScanMgrService extends BaseService {
  ...
  _buildFullState(session, options: Record<string, any> = {}) {
  ...
  refreshSignalTrackerForSession(session, options: Record<string, any> = {}) {
    const systemID = this._ensureSystemParity(
      session,
      normalizeNumber(options.nowMs, Date.now()),
    );
  ...
  Handle_RequestScans(args, session, kwargs) {
    ...
    const synchronizedProbeIDs = new Set<any>(
    ...
    const effectiveProbeMap = new Map<any, any>(
    ...
    const startResolvedScan = () => {
      ...
      if (perfectSignatureIDs.length > 0) {
        try {
          ...
          log.debug(
            `[ScanMgr] daily-goal scan_signature hook failed char=${characterID}: ${dailyGoalError.message}`,
          );
        }
      }
    };
  ...
  Handle_QAScanSites(args, session) {
    ...
    const signatureSitesByResultID = new Map<any, any>(
  ...
  resolveAllSystemSignaturesForSession(session, options: Record<string, any> = {}) {
    const systemID = this._ensureSystemParity(
      session,
      normalizeNumber(options.nowMs, Date.now()),
    );
