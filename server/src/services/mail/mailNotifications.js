...
const { buildFiletimeLong, buildKeyVal, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
function getCharacterSessions(characterID, options = {}) {
    ...
    return sessionRegistry.getSessions().filter((session) => {
        ...
        const sessionCharacterID = Number(session &&
...
function sendMailSentNotification(characterID, message, statusMask, options = {}) {
    ...
    sendNotificationToCharacter(characterID, "OnMailSent", [
...
function sendMailUpdatedNotification(characterID, messageIDs, isRead, newLabel, options = {}) {
    sendNotificationToCharacter(characterID, "OnMailUpdatedByExternal", [
        messageIDs,
...
function sendLabelsCreatedNotification(characterID, label, options = {}) {
    ...
    sendNotificationToCharacter(characterID, "OnLabelsCreatedByExternal", [
        buildKeyVal([
...
function sendMailingListLeaveNotification(characterID, listID, affectedCharacterID, options = {}) {
    sendNotificationToCharacter(characterID, "OnMailingListLeave", [Number(listID || 0) || 0, Number(affectedCharacterID || 0) || 0], options);
}
...
//# sourceMappingURL=mailNotifications.js.map