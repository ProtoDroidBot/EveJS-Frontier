...
function resolveWelcomeSenderID() {
  ...
  for (const [characterID, record] of Object.entries<any>(characters)) {
    if (
      record &&
...
function getMutableState() {
  ...
  for (const [listID, record] of Object.entries<any>(state.mailingLists)) {
...
function ensureMailingListShape(record, listID) {
  ...
  for (const [entityID, accessLevel] of Object.entries<any>(record.access)) {
  ...
  for (const [characterID, memberAccess] of Object.entries<any>(record.members)) {
    const numericCharacterID = toPositiveInteger(characterID, 0);
...
function normalizePositiveIDList(input, options: Record<string, any> = {}) {
...
function resolveMailingListRecipients(state, listID, senderID, saveSenderCopy) {
  ...
  if (!record || record.deleted === true) {
    return { success: false as const, errorMsg: "MAILING_LIST_NOT_FOUND" };
  }
  ...
  if (
    ...
    return { success: false as const, errorMsg: "NOT_IN_MAILING_LIST" };
  }
  ...
  if (
    ...
    return { success: false as const, errorMsg: "MAILING_LIST_MUTED" };
  }
...
function buildDeliveryRecipients(
  ...
  const recipients = new Set<any>(explicitCharacterIDs);
...
function computeStaticLabelMask(message, characterID, options: Record<string, any> = {}) {
  const numericCharacterID = toPositiveInteger(characterID, 0);
  if (numericCharacterID <= 0) {
    return 0;
  }
...
function ensureStatusEntry(state, characterID, messageID, options: Record<string, any> = {}) {
...
function pruneMessageIfOrphaned(state, messageID) {
  const key = String(messageID);
  for (const mailbox of Object.values<any>(state.mailboxes || {})) {
    if (
      mailbox &&
      mailbox.statuses &&
...
function getMailboxEntries(characterID, options: Record<string, any> = {}) {
  const numericCharacterID = toPositiveInteger(characterID, 0);
  if (numericCharacterID <= 0) {
    return [];
  }
  ...
  for (const [messageID, status] of Object.entries<any>(mailbox.statuses)) {
...
function createCharacterLabel(characterID, name, color = 0) {
  ...
  if (numericCharacterID <= 0 || !normalizedName) {
    return { success: false as const, errorMsg: "INVALID_LABEL" };
  }

  const state = getMutableState();
  const mailbox = ensureMailbox(state, numericCharacterID);
  if (Object.keys(mailbox.labels).length >= MAIL_MAX_NUM_LABELS) {
    return { success: false as const, errorMsg: "LABEL_LIMIT_REACHED" };
  }
  ...
  persistState(state);
  return { success: true as const, labelID };
}

function editCharacterLabel(characterID, labelID, nextValues: Record<string, any> = {}) {
  const numericCharacterID = toPositiveInteger(characterID, 0);
  ...
  if (!mailbox.labels[String(numericLabelID)]) {
    return { success: false as const, errorMsg: "LABEL_NOT_FOUND" };
  }
...
function deleteCharacterLabel(characterID, labelID) {
  ...
  return { success: true as const, changedMessageIDs };
}

function mutateStatuses(characterID, messageIDs, mutator) {
  const numericCharacterID = toPositiveInteger(characterID, 0);
  if (numericCharacterID <= 0) {
    return { success: false as const, errorMsg: "INVALID_CHARACTER" };
  }
  ...
  return { success: true as const, mutated, changedMessageIDs };
}
...
function sendMail(options: Record<string, any> = {}) {
  ...
  const forwardedFromMessageID = toPositiveInteger(
    options.isForwardedFrom,
    0,
  );
  ...
  if (recipientCharacterIDs.length === 0 && !saveSenderCopy) {
    return { success: false as const, errorMsg: "NO_RECIPIENTS" };
  }
  ...
  const senderNotificationUpdates: Record<string, any> = {};
  if (senderID > 0) {
    ...
    const markSourceMessage = (sourceMessageID, statusMask) => {
      ...
      sourceStatus.statusMask = Math.trunc(
        normalizeNumber(sourceStatus.statusMask, 0),
      );
...
function getJoinedMailingLists(characterID) {
  ...
  const state = getMutableState();
  const joinedLists: Record<string, any> = {};
...
function getMailingListInfo(listID, options: Record<string, any> = {}) {
  const numericListID = toPositiveInteger(listID, 0);
  if (numericListID <= 0) {
    return null;
  }
...
function createMailingList(
  ...
  if (numericOwnerID <= 0 || !nameParts.key || !nameParts.displayName) {
    return { success: false as const, errorMsg: "INVALID_MAILING_LIST" };
  }
  ...
  if (conflictingList) {
    return { success: false as const, errorMsg: "MAILING_LIST_EXISTS" };
  }
  ...
  persistState(state);
  return { success: true as const, listID };
}
...
function joinMailingList(characterID, nameOrID) {
  ...
  if (
    ...
    return { success: false as const, errorMsg: "MAILING_LIST_FULL" };
  }
  ...
  if (
    ...
    return { success: false as const, errorMsg: "MAILING_LIST_ACCESS_DENIED" };
  }
...
function getMailingListMembers(listID, options: Record<string, any> = {}) {
  ...
  return Object.fromEntries(
    Object.entries<any>(record.members || {}).map(([characterID, memberAccess]) => [
...
function getMailingListSettings(listID, options: Record<string, any> = {}) {
  const numericCharacterID = toPositiveInteger(options.characterID, 0);
  const numericListID = toPositiveInteger(listID, 0);
  if (numericCharacterID <= 0 || numericListID <= 0) {
    return null;
...
function setMailingListEntitiesAccess(characterID, listID, accessByEntityID: Record<string, any> = {}) {
  const numericCharacterID = toPositiveInteger(characterID, 0);
  const numericListID = toPositiveInteger(listID, 0);
  if (numericCharacterID <= 0 || numericListID <= 0) {
    return { success: false as const, errorMsg: "INVALID_MAILING_LIST" };
...
function updateMailingListMemberRole(characterID, listID, memberIDs, nextRole) {
  ...
  return { success: true as const, changedMembers };
}
...
function removeMailingListMembers(characterID, listID, memberIDs) {
  ...
  return { success: true as const, removedMembers };
}
...
function leaveMailingList(characterID, listID) {
  ...
  if (getMailingListMemberAccess(record, numericCharacterID) === MAILING_LIST_MEMBER_OWNER) {
    return { success: false as const, errorMsg: "OWNER_CANNOT_LEAVE" };
  }
...
function getMailingListWelcomeMail(listID, options: Record<string, any> = {}) {
  const numericCharacterID = toPositiveInteger(options.characterID, 0);
  const numericListID = toPositiveInteger(listID, 0);
  if (numericCharacterID <= 0 || numericListID <= 0) {
    return [];
...
function sendMailingListWelcomeMail(characterID, listID, title, body) {
  ...
  if (memberIDs.length === 0) {
    return { success: true as const, messageID: null };
  }
...
function sendWelcomeMailToCharacter(characterID, options: Record<string, any> = {}) {
  const numericCharacterID = toPositiveInteger(characterID, 0);
  if (numericCharacterID <= 0) {
    return { success: false as const, errorMsg: "INVALID_CHARACTER" };
  }
...
function sendCorporationWelcomeMailToCharacter(
  ...
  if (numericCharacterID <= 0 || numericCorporationID <= 0) {
    return { success: false as const, errorMsg: "INVALID_CORPORATION_WELCOME_MAIL" };
  }
...
function getCompressedBody(characterID, messageID, options: Record<string, any> = {}) {
  const numericCharacterID = toPositiveInteger(characterID, 0);
  const numericMessageID = toPositiveInteger(messageID, 0);
  if (numericCharacterID <= 0 || numericMessageID <= 0) {
    return null;
