...
function buildMailingListDict(records: Record<string, any> = {}) {
  return buildDict(
...
function buildMailingListSettingsKeyVal(settings) {
  ...
  return buildKeyVal([
    ...
    [
      "access",
      buildDict(
        Object.entries<any>(settings.access || {}).map(([entityID, accessLevel]) => [
...
function buildWelcomeMailRows(rows: any[] = []) {
  return buildList(
...
class MailingListsMgrService extends BaseService {
  ...
  Handle_GetMembers(args, session) {
    ...
    return buildDict(
      Object.entries<any>(members).map(([memberID, accessLevel]) => [
  ...
  Handle_GetWelcomeMail(args, session) {
    return buildWelcomeMailRows(
      getMailingListWelcomeMail(args && args[0], {
        characterID: resolveSessionCharacterID(session),
      }),
    );
  }
