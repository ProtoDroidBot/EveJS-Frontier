...
const { extractList, normalizeNumber, normalizeText, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getCorporationRecord, } = require(path.join(__dirname, "../corporation/corporationState"));
const { sendMailDeletedNotification, sendMailRestoredNotification, sendMailSentNotification, sendMailingListDeletedNotification, sendMailingListLeaveNotification, sendMailingListRoleClearNotification, sendMailingListRoleMutedNotification, sendMailingListRoleOperatorNotification, } = require(path.join(__dirname, "./mailNotifications"));
const { createNewMailNotification, deleteNewMailNotificationsForMessages, updateNewMailNotificationMessageState, } = require(path.join(__dirname, "../notifications/notificationState"));
...
const MAIL_SYSTEM_LABEL_MASK = MAIL_LABEL_INBOX |
    MAIL_LABEL_SENT |
...
function buildMailNotificationStatusUpdate(status) {
    const statusMask = Math.max(0, Math.trunc(normalizeNumber(status && status.statusMask, 0)));
    const labelMask = Math.max(0, Math.trunc(normalizeNumber(status && status.labelMask, 0)));
    return {
        statusMask,
        labelMask,
...
function normalizeMailingListMemberAccess(value, fallback = MAILING_LIST_MEMBER_DEFAULT) {
    const numericValue = Math.trunc(normalizeNumber(value, fallback));
...
function findExistingCharacterWelcomeMail(state, characterID, senderID) {
    ...
    for (const messageID of Object.keys(statuses)) {
        ...
        const recipientIDs = Array.isArray(message.toCharacterIDs)
            ...
            : [];
        const isRecipient = recipientIDs.some((entry) => toPositiveInteger(entry, 0) === numericCharacterID);
        if (isRecipient &&
...
function getMutableState() {
    ...
    const nextMailingListID = toPositiveInteger(state._meta.nextMailingListID, FIRST_MAILING_LIST_ID);
...
function ensureMailingListShape(record, listID) {
    ...
    const nameParts = normalizeMailingListNameComponents(record.displayName || record.name || "");
    ...
    record.defaultMemberAccess = normalizeMailingListMemberAccess(record.defaultMemberAccess);
    ...
    for (const [characterID, memberAccess] of Object.entries(record.members)) {
        ...
        delete record.members[characterID];
        record.members[String(numericCharacterID)] = normalizeMailingListMemberAccess(memberAccess);
    }
    if (record.ownerID > 0) {
...
function allocateMailingListID(state) {
    const nextMailingListID = toPositiveInteger(state &&
        state._meta &&
        state._meta.nextMailingListID, FIRST_MAILING_LIST_ID);
...
function resolveMailingListByNameOrID(state, nameOrID) {
    ...
    return (getAllMailingLists(state).find((record) => record.deleted !== true && record.name === key) || null);
}
...
function canManageMailingListTarget(record, actorCharacterID, targetCharacterID) {
    ...
    if (actorAccess !== MAILING_LIST_MEMBER_OWNER &&
    ...
    if (actorAccess === MAILING_LIST_MEMBER_OPERATOR &&
...
function ensureMailbox(state, characterID) {
    ...
    mailbox._meta.nextLabelMask = toPositiveInteger(mailbox._meta.nextLabelMask, FIRST_CUSTOM_LABEL_MASK);
    return mailbox;
}
...
function normalizePositiveIDList(input, options = {}) {
    ...
    const isCollectionInput = Array.isArray(input) ||
    ...
    const normalized = [...new Set(sourceValues
...
function resolveMailingListRecipients(state, listID, senderID, saveSenderCopy) {
    ...
    const recipients = Object.keys(record.members || {})
        ...
        .filter((characterID) => !(saveSenderCopy && senderID > 0 && characterID === senderID));
    return {
        success: true,
        recipients: [...new Set(recipients)].sort((left, right) => left - right),
    };
}
function buildDeliveryRecipients(state, explicitCharacterIDs, toCorpOrAllianceID, toListID, senderID, saveSenderCopy, overrideRecipients = null) {
    ...
    if (toPositiveInteger(toListID, 0) > 0) {
        const listRecipients = resolveMailingListRecipients(state, toListID, senderID, saveSenderCopy);
...
function getUnreadMailCount(characterID) {
    return getMailboxEntries(characterID).filter(({ status }) => (Math.trunc(normalizeNumber(status.statusMask, 0)) &
...
function allocateNextLabelMask(mailbox) {
    let candidate = toPositiveInteger(mailbox &&
        mailbox._meta &&
        mailbox._meta.nextLabelMask, FIRST_CUSTOM_LABEL_MASK);
...
function editCharacterLabel(characterID, labelID, nextValues = {}) {
    ...
    if (Object.prototype.hasOwnProperty.call(nextValues, "color")) {
        label.color = Math.max(0, Math.trunc(normalizeNumber(nextValues.color, 0)));
    }
    persistState(state);
    return { success: true };
}
function deleteCharacterLabel(characterID, labelID) {
    ...
    const numericLabelID = toPositiveInteger(labelID, 0);
    if (numericCharacterID <= 0 ||
        numericLabelID < FIRST_CUSTOM_LABEL_MASK ||
        (numericLabelID & MAIL_SYSTEM_LABEL_MASK) !== 0) {
        return { success: false, errorMsg: "INVALID_LABEL" };
    }
    const state = getMutableState();
    const mailbox = ensureMailbox(state, numericCharacterID);
    ...
    for (const [messageID, status] of Object.entries(mailbox.statuses)) {
        const previousLabelMask = Math.trunc(normalizeNumber(status.labelMask, 0));
        status.labelMask = previousLabelMask & ~numericLabelID;
        if (status.labelMask === previousLabelMask) {
            continue;
        }
        const numericMessageID = toPositiveInteger(messageID, 0);
    ...
    return { success: true, changedMessageIDs };
}
...
function emptyTrash(characterID) {
    ...
    const changedMessageIDs = [];
    for (const [messageID, status] of Object.entries(mailbox.statuses)) {
        if ((Math.trunc(normalizeNumber(status.statusMask, 0)) &
...
function sendMail(options = {}) {
    ...
    const senderName = normalizeText(options.senderName, "").trim();
    const repliedToMessageID = toPositiveInteger(options.isReplyTo, 0);
    const forwardedFromMessageID = toPositiveInteger(options.isForwardedFrom, 0);
    ...
    const initialStatusMask = Math.max(0, Math.trunc(normalizeNumber(options.statusMask, 0)));
    const state = getMutableState();
    const resolvedRecipients = buildDeliveryRecipients(state, explicitCharacterIDs, toCorpOrAllianceID, toListID, senderID, saveSenderCopy, options.deliveryRecipientCharacterIDs);
    ...
    if (saveSenderCopy && senderID > 0) {
        const shouldReadSenderCopy = toListID == null &&
    ...
    const senderNotificationUpdates = {};
    if (senderID > 0) {
        const senderMailbox = state.mailboxes[String(senderID)] || null;
        const markSourceMessage = (sourceMessageID, statusMask) => {
            if (sourceMessageID <= 0 ||
                !senderMailbox ||
                !senderMailbox.statuses ||
                !senderMailbox.statuses[String(sourceMessageID)]) {
                return;
            }
            const sourceStatus = senderMailbox.statuses[String(sourceMessageID)];
            sourceStatus.statusMask = Math.trunc(normalizeNumber(sourceStatus.statusMask, 0));
            if ((sourceStatus.statusMask & statusMask) !== 0) {
                return;
            }
            sourceStatus.statusMask |= statusMask;
            senderNotificationUpdates[String(sourceMessageID)] =
                buildMailNotificationStatusUpdate(sourceStatus);
        };
        markSourceMessage(repliedToMessageID, MAIL_STATUS_MASK_REPLIED);
        markSourceMessage(forwardedFromMessageID, MAIL_STATUS_MASK_FORWARDED);
    }
    const writeResult = persistState(state);
    if (!writeResult || !writeResult.success) {
        return { success: false, errorMsg: "WRITE_ERROR" };
    }
    if (Object.keys(senderNotificationUpdates).length > 0) {
        updateNewMailNotificationMessageState(senderID, senderNotificationUpdates);
    }
    for (const characterID of recipientCharacterIDs) {
        const recipientMailbox = state.mailboxes[String(characterID)];
        const recipientStatus = recipientMailbox &&
        ...
        createNewMailNotification(characterID, {
...
function getJoinedMailingLists(characterID) {
    ...
    for (const record of getAllMailingLists(state)) {
        ...
        joinedLists[String(record.listID)] = buildMailingListSummary(record, numericCharacterID);
    }
    return joinedLists;
}
function getMailingListInfo(listID, options = {}) {
    ...
    return buildMailingListSummary(record, toPositiveInteger(options.characterID, 0));
}
function createMailingList(ownerCharacterID, name, defaultAccess = MAILING_LIST_ACCESS_ALLOWED, defaultMemberAccess = MAILING_LIST_MEMBER_DEFAULT, cost = 0) {
    ...
    const state = getMutableState();
    const conflictingList = getAllMailingLists(state).find((record) => record.deleted !== true && record.name === nameParts.key);
    if (conflictingList) {
...
function joinMailingList(characterID, nameOrID) {
    ...
    if (Object.keys(record.members || {}).length >= MAILING_LIST_MAX_MEMBERS) {
        return { success: false, errorMsg: "MAILING_LIST_FULL" };
    }
    if (resolveMailingListJoinAccess(record, numericCharacterID) !==
        MAILING_LIST_ACCESS_ALLOWED) {
        return { success: false, errorMsg: "MAILING_LIST_ACCESS_DENIED" };
    }
    record.members[String(numericCharacterID)] = normalizeMailingListMemberAccess(record.defaultMemberAccess);
    record.updatedAt = currentFileTimeString();
    persistState(state);
    if (record.welcomeMail &&
        (record.welcomeMail.title || record.welcomeMail.body)) {
        sendMail({
            senderID: record.listID,
            senderName: record.displayName,
...
function getMailingListMembers(listID, options = {}) {
    const numericCharacterID = toPositiveInteger(options.characterID, 0);
    const numericListID = toPositiveInteger(listID, 0);
    ...
    if (!record ||
        record.deleted === true ||
        !canConfigureMailingList(record, numericCharacterID)) {
        return {};
    }
    return Object.fromEntries(Object.entries(record.members || {}).map(([characterID, memberAccess]) => [
        String(toPositiveInteger(characterID, 0)),
        normalizeMailingListMemberAccess(memberAccess),
    ]));
}
function getMailingListSettings(listID, options = {}) {
    const numericCharacterID = toPositiveInteger(options.characterID, 0);
    const numericListID = toPositiveInteger(listID, 0);
    if (numericCharacterID <= 0 || numericListID <= 0) {
        return null;
    ...
    return {
        ...
        defaultMemberAccess: normalizeMailingListMemberAccess(record.defaultMemberAccess),
...
function setMailingListMembersMuted(characterID, listID, memberIDs) {
    return updateMailingListMemberRole(characterID, listID, memberIDs, MAILING_LIST_MEMBER_MUTED);
}
function setMailingListMembersOperator(characterID, listID, memberIDs) {
    return updateMailingListMemberRole(characterID, listID, memberIDs, MAILING_LIST_MEMBER_OPERATOR);
}
function setMailingListMembersClear(characterID, listID, memberIDs) {
    return updateMailingListMemberRole(characterID, listID, memberIDs, MAILING_LIST_MEMBER_DEFAULT);
}
...
function setMailingListDefaultAccess(characterID, listID, defaultAccess, defaultMemberAccess, cost = 0) {
    const numericCharacterID = toPositiveInteger(characterID, 0);
    const numericListID = toPositiveInteger(listID, 0);
    if (numericCharacterID <= 0 || numericListID <= 0) {
        return { success: false, errorMsg: "INVALID_MAILING_LIST" };
...
function getMailingListWelcomeMail(listID, options = {}) {
    const numericCharacterID = toPositiveInteger(options.characterID, 0);
    const numericListID = toPositiveInteger(listID, 0);
    if (numericCharacterID <= 0 || numericListID <= 0) {
        return [];
...
function sendWelcomeMailToCharacter(characterID, options = {}) {
    ...
    const characterName = normalizeText(options.characterName, "") ||
        resolveCharacterName(numericCharacterID);
    const existingMail = findExistingCharacterWelcomeMail(getMutableState(), numericCharacterID, senderID);
    if (existingMail) {
        return {
            success: true,
            alreadySent: true,
...
function sendCorporationWelcomeMailToCharacter(characterID, corporationID, options = {}) {
    const numericCharacterID = toPositiveInteger(characterID, 0);
    ...
    const corporationRecord = options.corporationRecord || getCorporationRecord(numericCorporationID) || null;
    const corporationName = normalizeText(corporationRecord && corporationRecord.corporationName, `Corporation ${numericCorporationID}`) || `Corporation ${numericCorporationID}`;
    const body = normalizeBody(options.body == null ? "" : options.body);
    if (!body) {
        return {
            success: true,
            skipped: true,
    ...
    return sendMail({
        ...
        toCharacterIDs: [numericCharacterID],
        title: normalizeSubject(options.title) || `Welcome to ${corporationName}`,
        body,
        saveSenderCopy: false,
        statusMask: MAIL_STATUS_MASK_AUTOMATED,
    });
...
function moveAllToTrash(characterID) {
    return mutateAllStatuses(characterID, () => true, (status) => {
        status.statusMask = Math.trunc(normalizeNumber(status.statusMask, 0));
        status.statusMask |= MAIL_STATUS_MASK_TRASHED;
    });
}
...
function markAllAsRead(characterID) {
    return mutateAllStatuses(characterID, (status) => (Math.trunc(normalizeNumber(status.statusMask, 0)) &
        MAIL_STATUS_MASK_TRASHED) ===
        0, (status) => {
        status.statusMask = Math.trunc(normalizeNumber(status.statusMask, 0));
        status.statusMask |= MAIL_STATUS_MASK_READ;
...
function moveToTrashByLabel(characterID, labelID) {
    const numericLabelID = toPositiveInteger(labelID, 0);
    return mutateAllStatuses(characterID, (status) => (Math.trunc(normalizeNumber(status.labelMask, 0)) & numericLabelID) ===
        numericLabelID, (status) => {
        status.statusMask = Math.trunc(normalizeNumber(status.statusMask, 0));
        status.statusMask |= MAIL_STATUS_MASK_TRASHED;
    });
}
...
function moveToTrashByList(characterID, listID) {
    const numericListID = toPositiveInteger(listID, 0);
    return mutateAllStatuses(characterID, (_status, message) => toPositiveInteger(message && message.toListID, 0) === numericListID, (status) => {
        status.statusMask = Math.trunc(normalizeNumber(status.statusMask, 0));
        status.statusMask |= MAIL_STATUS_MASK_TRASHED;
    });
}
function markAsReadByList(characterID, listID) {
    const numericListID = toPositiveInteger(listID, 0);
    return mutateAllStatuses(characterID, (status, message) => (Math.trunc(normalizeNumber(status.statusMask, 0)) &
        MAIL_STATUS_MASK_TRASHED) ===
        0 &&
        toPositiveInteger(message && message.toListID, 0) === numericListID, (status) => {
        status.statusMask = Math.trunc(normalizeNumber(status.statusMask, 0));
        status.statusMask |= MAIL_STATUS_MASK_READ;
    });
}
...
function replaceLabels(characterID, messageIDs, labelMask) {
    const numericLabelMask = Math.max(0, Math.trunc(normalizeNumber(labelMask, 0)));
    return mutateStatuses(characterID, messageIDs, (status) => {
        const systemLabels = Math.trunc(normalizeNumber(status.labelMask, 0)) & MAIL_SYSTEM_LABEL_MASK;
...
//# sourceMappingURL=mailState.js.map