...
const { buildDict, buildFiletimeLong, buildKeyVal, buildList, extractList, normalizeBigInt, normalizeNumber, normalizeText, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getCorporationRecord, } = require(path.join(__dirname, "../corporation/corporationState"));
const { assignLabels, createCharacterLabel, deleteCharacterLabel, editCharacterLabel, emptyTrash, getCharacterLabels, getMessageByIDForCharacter, getCompressedBody, getMailHeaders, getSyncMailbox, markAllAsRead, markAllAsUnread, markAsRead, markAsReadByLabel, markAsReadByList, markAsUnread, markAsUnreadByLabel, markAsUnreadByList, moveAllFromTrash, moveAllToTrash, moveFromTrash, moveToTrash, moveToTrashByLabel, moveToTrashByList, pokePlayerAboutChatMsgGm, removeLabels, removeMessageStatuses, replaceLabels, sendMail, } = require(path.join(__dirname, "./mailState"));
const { sendLabelsCreatedNotification, sendMailDeletedNotification, sendMailRestoredNotification, sendMailTrashedNotification, sendMailUpdatedNotification, } = require(path.join(__dirname, "./mailNotifications"));
...
function resolveSessionCorporationID(session) {
    return Number(session &&
        (session.corporationID || session.corpID || session.corpid || 0)) || 0;
}
function resolveSessionAllianceID(session) {
    return Number(session &&
        (session.allianceID || session.allianceid || 0)) || 0;
}
function sessionHasChatManagerRole(session) {
    const rawRole = session &&
        (session.corpRole || session.corprole || session.rolesAtAll || 0);
    return ((normalizeBigInt(rawRole, 0n) & CORP_ROLE_CHAT_MANAGER) ===
        CORP_ROLE_CHAT_MANAGER);
}
function canSendCorporationOrAllianceMail(session, recipientID) {
    const numericRecipientID = normalizePositiveInteger(recipientID, 0);
    if (numericRecipientID <= 0) {
        return true;
    }
    const corporationID = resolveSessionCorporationID(session);
    if (corporationID > 0 && numericRecipientID === corporationID) {
        const corporation = getCorporationRecord(corporationID);
        return Boolean(corporation && corporation.isNPC !== true);
    }
    const allianceID = resolveSessionAllianceID(session);
    return (allianceID > 0 &&
        numericRecipientID === allianceID &&
...
class MailMgrService extends BaseService {
    ...
    Handle_SyncMail(args, session) {
        ...
        log.debug(`[MailMgr] SyncMail(charID=${characterID}, firstID=${String(firstID)}, lastID=${String(lastID)}) -> headers=${mailbox.newMail.length + mailbox.oldMail.length} statuses=${mailbox.mailStatus.length}`);
    ...
    Handle_GetBody(args, session) {
        ...
        if (compressedBody &&
            ...
            sendMailUpdatedNotification(characterID, [messageID], true, null, { excludeSession: session });
        }
    ...
    Handle_MarkAsRead(args, session) {
        ...
        if ((result.changedMessageIDs || []).length > 0) {
            sendMailUpdatedNotification(characterID, result.changedMessageIDs, true, null, { excludeSession: session });
        }
        return null;
    }
    Handle_MarkAsUnread(args, session) {
        ...
        if ((result.changedMessageIDs || []).length > 0) {
            sendMailUpdatedNotification(characterID, result.changedMessageIDs, false, null, { excludeSession: session });
        }
        return null;
    }
    ...
    Handle_CreateLabel(args, session) {
        const characterID = resolveSessionCharacterID(session);
        const result = createCharacterLabel(characterID, args && args[0], args && args[1]);
        if (result.success) {
    ...
    Handle_AssignLabels(args, session) {
        ...
        for (const [labelMask, messageIDs] of groupMessageIDsByLabelMask(characterID, result.changedMessageIDs || [])) {
    ...
    Handle_SendMail(args, session) {
        const characterID = resolveSessionCharacterID(session);
        const toCorpOrAllianceID = normalizePositiveInteger(args && args[2], 0) || null;
        if (toCorpOrAllianceID != null &&
            !canSendCorporationOrAllianceMail(session, toCorpOrAllianceID)) {
            return null;
        }
...
//# sourceMappingURL=mailMgrService.js.map