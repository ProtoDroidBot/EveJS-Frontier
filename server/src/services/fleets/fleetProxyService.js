...
const { buildAdvertMapPayload, buildAdvertPayload } = require(path.join(__dirname, "./fleetPayloads"));
...
class FleetProxyService extends BaseService {
    ...
    Handle_GetAvailableFleetAds(args, session) {
        return buildAdvertMapPayload(fleetRuntime.getAvailableFleetAds(session));
    }
    Handle_ApplyToJoinFleet(args, session) {
        return fleetRuntime.applyToJoinFleet(session, args && args[0], args && args[1]);
    }
    ...
    Handle_UpdateAdvertInfo(args, session) {
        const advert = fleetRuntime.updateAdvertInfo(session, args && args[0], args && args[1] ? args[1] : {});
        return advert ? buildAdvertPayload(advert) : null;
    }
    Handle_UpdateAdvertAllowedEntities(args, session) {
        const advert = fleetRuntime.updateAdvertAllowedEntities(session, args && args[0] ? args[0] : {});
        return advert ? buildAdvertPayload(advert) : null;
    }
    Handle_UpdateFleetAdvertWithNewLeader(args, session) {
        const advert = fleetRuntime.updateFleetAdvertWithNewLeader(session, args && args[0] ? args[0] : {});
        return advert ? buildAdvertPayload(advert) : null;
    }
}
module.exports = FleetProxyService;
//# sourceMappingURL=fleetProxyService.js.map