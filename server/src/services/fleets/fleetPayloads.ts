...
function buildMemberOptOutsPayload(optOuts: Record<string, any> = {}) {
  return buildKeyVal([
...
function buildMemberPayload(member: Record<string, any> = {}) {
  return buildKeyVal([
...
function buildJoinRequestPayload(request: Record<string, any> = {}) {
  return buildKeyVal([
...
function buildSquadPayload(squad: Record<string, any> = {}) {
  return buildKeyVal([
...
function buildWingPayload(wing: Record<string, any> = {}) {
  const squadEntries: any[] = [];
  const squads = wing.squads instanceof Map
    ? [...wing.squads.values()]
    : Object.values<any>(wing.squads || {});
...
function buildOptionsPayload(options: Record<string, any> = {}) {
  return buildKeyVal([
...
function buildFleetStatePayload(fleet: Record<string, any> = {}) {
  const memberEntries: any[] = [];
  const members = fleet.members instanceof Map
    ? [...fleet.members.values()]
    : Object.values<any>(fleet.members || {});
  ...
  const wingEntries: any[] = [];
  const wings = fleet.wings instanceof Map
    ? [...fleet.wings.values()]
    : Object.values<any>(fleet.wings || {});
...
function buildFleetStateChangePayload(fleetState: Record<string, any> = {}) {
  return buildKeyVal([
    ["targetTags", buildTargetTagsPayload(fleetState.targetTags)],
  ]);
}

function buildTuplePayload(items: any[] = []) {
  return {
    type: "tuple",
...
function buildJoinRequestsPayload(joinRequests) {
  ...
  const values = joinRequests instanceof Map
    ? [...joinRequests.values()]
    : Object.values<any>(joinRequests || {});
...
function buildLeaderPayload(leader: Record<string, any> = {}) {
  return buildKeyVal([
...
function buildAdvertPayload(advert: Record<string, any> = {}) {
  return buildDict([
...
function buildAdvertMapPayload(advertsByFleetID) {
  ...
  const values = advertsByFleetID instanceof Map
    ...
    : Object.entries<any>(advertsByFleetID || {});
...
function buildCompositionEntryPayload(entry: Record<string, any> = {}) {
  return buildKeyVal([
...
function buildCompositionPayload(entries: any[] = []) {
  return buildList(
...
function buildRespawnPointPayload(respawnPoint: Record<string, any> = {}) {
  return buildDynamicPayload(respawnPoint);
}

function buildRespawnPointsPayload(respawnPoints: any[] = []) {
  return buildList(
