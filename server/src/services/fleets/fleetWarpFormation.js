...
function isOffsetFormation(formationType) {
    return (formationType === FORMATION_SPHERE ||
    ...
    followers.forEach((follower, slotIndex) => {
        ...
        if (formationType === FORMATION_RELATIVE && anchorPosition && follower.position) {
            ...
            if (relLength > RELATIVE_FORMATION_PICKUP_RANGE_METERS) {
                relative = scaleVec(relative, RELATIVE_FORMATION_PICKUP_RANGE_METERS / relLength);
            }
            offset = relative;
        }
        else if (formationType === FORMATION_SPHERE) {
            const s = sphereOffset(slotIndex, followers.length, size || spacing || 1000);
            offset = addVec(addVec(scaleVec(right, s.ex), scaleVec(up, s.ey)), scaleVec(forward, s.ez));
        }
        else {
            ...
            if (formationType === FORMATION_WALL) {
                ...
                offset = addVec(scaleVec(right, columnOffset), scaleVec(forward, row * step));
            }
                ...
                offset = addVec(scaleVec(right, side * rank * step), scaleVec(forward, -rank * step));
            }
        }
...
//# sourceMappingURL=fleetWarpFormation.js.map