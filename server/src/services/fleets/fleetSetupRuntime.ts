...
function normalizeWingInfo(rawWing: Record<string, any> = {}) {
...
function normalizeFleetSetup(rawSetup: Record<string, any> = {}) {
  ...
  const rawWingsInfo = rawSetup.wingsInfo && typeof rawSetup.wingsInfo === "object"
    ? Object.values<any>(rawSetup.wingsInfo)
    : [];
...
function parseFleetSetupsSetting(rawValue) {
  ...
  const parsedValue = Array.isArray(jsonParsed)
    ? jsonParsed
    : (jsonParsed && typeof jsonParsed === "object" ? Object.values<any>(jsonParsed) : parseYamlLikeList(rawValue));
