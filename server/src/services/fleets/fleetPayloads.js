...
const { buildDict, buildKeyVal, buildList, buildPythonSet, currentFileTime, extractList, normalizeBigInt, normalizeNumber, normalizeText, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
function buildLootEventsPayload(lootEvents = new Map()) {
    ...
    entries.sort((left, right) => {
        ...
        return (toInteger(leftItems[0], 0) - toInteger(rightItems[0], 0) ||
...
function buildCompositionPayload(entries = []) {
    return buildList((Array.isArray(entries) ? entries : [])
...
function buildRespawnPointsPayload(respawnPoints = []) {
    return buildList((Array.isArray(respawnPoints) ? respawnPoints : []).map((entry) => (buildRespawnPointPayload(entry))));
}
module.exports = {
    toInteger,
    toOptionalInteger,
...
//# sourceMappingURL=fleetPayloads.js.map