...
const { currentFileTime, extractDictEntries, extractList, marshalObjectToObject, normalizeNumber, normalizeText, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { buildJoinRequestPayload, buildJoinRequestsPayload, buildFleetStateChangePayload, buildLootEventsPayload, buildMemberOptOutsPayload, buildMemberPayload, buildOptionsPayload, buildRespawnPointsPayload, } = require(path.join(__dirname, "./fleetPayloads"));
const { getSessions, findSessionByCharacterID } = require(path.join(__dirname, "../chat/sessionRegistry"));
const { getCharacterSetting } = require(path.join(__dirname, "../character/characterSettingsState"));
const FLEET = require(path.join(__dirname, "./fleetConstants"));
const { getFleetSetupByName } = require(path.join(__dirname, "./fleetSetupRuntime"));
...
function normalizeRespawnPoint(respawnPoint = {}) {
    ...
    if (nextRespawnPoint.extraClientState && typeof nextRespawnPoint.extraClientState === "object") {
        nextRespawnPoint.extraClientState.characterID = toOptionalInteger(nextRespawnPoint.extraClientState.characterID, null);
    }
    return nextRespawnPoint;
}
function cloneRespawnPoints(respawnPoints = []) {
    return (Array.isArray(respawnPoints) ? respawnPoints : []).map((entry) => (normalizeRespawnPoint(entry)));
}
...
function getSessionCharacterID(session) {
    return toInteger(session && (session.characterID || session.charID || session.charid || session.userid), 0);
}
function getSessionClientID(session) {
    return toOptionalInteger(session && (session.clientID || session.clientId), null);
}
function getSessionShipTypeID(session) {
    return toOptionalInteger(session && (session.shipTypeID || session.shiptypeid), null);
}
function getSessionSolarSystemID(session) {
    return toOptionalInteger(session && (session.solarsystemid2 || session.solarsystemid), null);
}
function getSessionStationID(session) {
    return toOptionalInteger(session && (session.stationid2 || session.stationid || session.stationID), null);
}
function buildMemberSnapshot(characterID, session, overrides = {}) {
    ...
    return {
        ...
        corpID: toOptionalInteger(characterRecord.corporationID ??
            characterRecord.corpID ??
            (session && (session.corpid || session.corporationID)), null),
        allianceID: toOptionalInteger(characterRecord.allianceID ??
            (session && (session.allianceid || session.allianceID)), null),
        warFactionID: toOptionalInteger(characterRecord.warFactionID ??
            (session && (session.warfactionid || session.warFactionID)), null),
        securityStatus: Number(characterRecord.securityStatus ??
...
function isMemberInCommandScope(commander, member) {
    ...
    if ((toInteger(commander.job, 0) & FLEET.FLEET_JOB_CREATOR) !== 0 ||
...
function notifyFleetMultiEvent(fleet, events, options = {}) {
    const payload = (Array.isArray(events) ? events : []).filter((event) => (Array.isArray(event) &&
...
function isMemberPlacementChanged(member, oldState) {
    return (oldState.wingID !== member.wingID ||
...
function notifyFleetMemberChanges(fleet, changes, options = {}) {
    ...
    if (normalizedChanges.length === 1) {
        ...
        notifyFleet(fleet, "OnFleetMemberChanged", buildMemberChangedArgs(fleet, change.member, change.oldState, change.isOnlyMember), {
            ...options,
            idType: options.idType || "*fleetid",
        });
    }
    else {
        notifyFleetMultiEvent(fleet, normalizedChanges.map((change) => [
            "OnFleetMemberChanged",
            buildMemberChangedArgs(fleet, change.member, change.oldState, change.isOnlyMember),
        ]), options);
    }
    for (const change of normalizedChanges) {
        const { member, oldState } = change;
...
function notifyAllLiveSessions(notificationName, payload, idType = "clientID", options = {}) {
    const excludedCharacterIDs = new Set((Array.isArray(options.excludeCharacterIDs) ? options.excludeCharacterIDs : [])
        .map((value) => toInteger(value, 0))
        .filter((value) => value > 0));
...
function applyAdvertLeaderChange(fleet, nextLeaderMember, nextLeaderSession = null) {
    ...
    fleet.advert.leader = buildMemberSnapshot(nextLeaderMember.charID, nextLeaderSession || findSessionByCharacterID(nextLeaderMember.charID), nextLeaderMember);
...
function placeMemberInFleet(fleet, member, requestedWingID, requestedSquadID, requestedRole) {
    ...
    if (role === FLEET.FLEET_ROLE_WING_COMMANDER) {
        if (otherMembers.some((candidate) => (candidate.role === FLEET.FLEET_ROLE_WING_COMMANDER &&
...
function buildFleetState(fleet) {
    return {
        ...
        wings: new Map([...fleet.wings.entries()].map(([wingID, wing]) => [
            wingID,
            {
                wingID: wing.wingID,
                name: wing.name,
...
function getActiveBeaconsForCharacter(characterID) {
    ...
    return fleet && fleet.activeModuleBeacons instanceof Map
        ? new Map([...fleet.activeModuleBeacons.entries()].map(([key, value]) => [
            key,
...
function acceptInvite(session, fleetID, shipTypeID = null) {
    ...
    if (fleet.joinRequests.has(characterID)) {
        ...
        if (bossSession) {
            notifySession(bossSession, "OnJoinRequestUpdate", [buildJoinRequestsPayload(fleet.joinRequests)], "fleetid");
        }
    }
...
function assignBossToAnyRemainingMember(fleet) {
    ...
    applyAdvertLeaderChange(fleet, replacementMember, findSessionByCharacterID(replacementMember.charID));
    return replacementMember;
}
function destroyFleet(fleet, options = {}) {
    const affectedCharacterIDs = [...fleet.members.keys()];
    const hadRegisteredAdvert = Boolean(fleet &&
        fleet.advert &&
        fleet.options &&
...
function moveMember(session, fleetID, characterID, wingID, squadID, role) {
    ...
    try {
        const oldState = updateMemberPlacement(fleet, member, wingID, squadID, role);
...
function setFleetMaxSize(session, fleetID, maxSize) {
    const fleet = ensureFleetBoss(session, fleetID);
    fleet.maxSize = Math.max(1, Math.min(FLEET.MAX_MEMBERS_IN_FLEET, toInteger(maxSize, FLEET.MAX_MEMBERS_IN_FLEET)));
...
function getSavedFleetSetup(characterID, setupName) {
    return getFleetSetupByName(getCharacterSetting(characterID, "fleetSetups", null), setupName);
}
function resolveSetupTargetSquad(fleet, defaultSquadSetting) {
    ...
    const targetWing = getSortedWings(fleet).find((wing) => (normalizeText(wing.name, "").toLowerCase() === targetWingName));
    if (!targetWing) {
        return null;
    }
...
function applyFleetSetup(session, fleetID, setupName) {
    ...
    for (let wingIndex = 0; wingIndex < desiredWings.length; wingIndex += 1) {
        ...
        for (let squadIndex = 0; squadIndex < desiredWing.squadNames.length; squadIndex += 1) {
            const desiredSquadName = normalizeText(desiredWing.squadNames[squadIndex], "").slice(0, FLEET.MAX_NAME_LENGTH);
...
function shouldReceiveBroadcast(recipient, sender, scope) {
    ...
    const isRecipientSuperiorToSender = (recipientRole < senderRole ||
    ...
    const isRecipientSubordinateToSender = (senderRole === FLEET.FLEET_ROLE_LEADER ||
...
function collectBroadcastRecipientSessions(fleet, senderSession, rangeMode) {
    ...
    if (rangeMode === FLEET.BROADCAST_BUBBLE) {
        ...
        const bubbleSessions = new Set(spaceRuntime.getSessionsInBubble(senderSystemID, bubble.id));
        return targetSessions.filter((session) => bubbleSessions.has(session));
    }
    if (rangeMode === FLEET.BROADCAST_SYSTEM) {
        return targetSessions.filter((session) => (getSessionSolarSystemID(session) === senderSystemID));
    }
...
function sendBroadcast(session, fleetID, name, scope, itemID, typeID = null, rangeMode = FLEET.BROADCAST_UNIVERSE) {
    ...
    const recipientSessions = collectBroadcastRecipientSessions(fleet, session, rangeMode);
...
function updateFleetAdvertWithNewLeader(session, allowedEntitiesInfo) {
    ...
    fleet.advert.leader = buildMemberSnapshot(getBossCharacterID(fleet), findSessionByCharacterID(getBossCharacterID(fleet)), getBossMember(fleet));
    fleet.advert.solarSystemID = getLeaderSolarSystemID(fleet);
...
function applyToJoinFleet(session, fleetID, autoAccept = false) {
    ...
    if (fleet.advert.advertJoinLimit != null &&
    ...
    if (fleet.advert.joinNeedsApproval) {
        ...
        const bossSession = findSessionByCharacterID(getBossCharacterID(fleet));
        if (bossSession) {
            notifySession(bossSession, "OnFleetJoinRequest", [buildJoinRequestPayload(request)], "fleetid");
        }
        return true;
    }
    ...
    inviteCharacter(findSessionByCharacterID(getBossCharacterID(fleet)) || session, fleet.fleetID, getSessionCharacterID(session), defaultPlacement.wingID, defaultPlacement.squadID, defaultPlacement.role, {
        autoAccept: true,
    });
    return false;
}
...
//# sourceMappingURL=fleetRuntime.js.map