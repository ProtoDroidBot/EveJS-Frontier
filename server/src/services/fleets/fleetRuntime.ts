...
function toIntegerSet(values) {
  ...
  return numericValue > 0 ? new Set([numericValue]) : new Set<any>();
}

function cloneSet(values) {
  return new Set<any>(toIntegerSet(values));
}
...
function clonePlainValue(value) {
  ...
  if (value && typeof value === "object") {
    return Object.fromEntries(
      Object.entries<any>(value).map(([entryKey, entryValue]) => [
        entryKey,
...
function normalizeRespawnPoint(respawnPoint: Record<string, any> = {}) {
...
function cloneRespawnPoints(respawnPoints: any[] = []) {
...
function buildMemberOptOuts(overrides: Record<string, any> = {}) {
  return {
...
function cloneMemberOptOuts(optOuts: Record<string, any> = {}) {
  return {
...
function buildMemberSnapshot(characterID, session, overrides: Record<string, any> = {}) {
...
function buildAdvertFromData(fleet, advertData: Record<string, any> = {}) {
...
function collectFleetSessions(fleet, options: Record<string, any> = {}) {
  const sessions: any[] = [];
  for (const member of fleet.members.values()) {
...
function buildFleetInviteNotification(inviterCharID, options: Record<string, any> = {}) {
...
function notifyFleet(fleet, notificationName, payload, options: Record<string, any> = {}) {
...
function notifyFleetMultiEvent(fleet, events, options: Record<string, any> = {}) {
...
function notifyFleetMemberChanges(fleet, changes, options: Record<string, any> = {}) {
...
function notifyAllLiveSessions(notificationName, payload, idType = "clientID", options: Record<string, any> = {}) {
...
function normalizeSessionFleetState(nextState: Record<string, any> = {}) {
  return {
...
function setPendingFleetSessionState(session, nextState: Record<string, any> = {}) {
  if (!session) {
    return;
  }
...
function applySessionFleetState(session, nextState: Record<string, any> = {}) {
...
function syncMemberStateFromSession(fleet, characterID, session, overrides: Record<string, any> = {}) {
...
function initFleet(session, fleetID, options: Record<string, any> = {}) {
...
function buildFleetState(fleet) {
  return {
    fleetID: fleet.fleetID,
    members: new Map<any, any>(fleet.members),
    wings: new Map<any, any>(
      [...fleet.wings.entries()].map(([wingID, wing]) => [
        wingID,
        {
          wingID: wing.wingID,
          name: wing.name,
          squads: new Map<any, any>(wing.squads),
        },
      ]),
    ),
...
function getJoinRequests(fleetID) {
  return new Map<any, any>(ensureFleetExists(fleetID).joinRequests);
}
...
function setRespawnPoints(fleetID, respawnPoints, options: Record<string, any> = {}) {
  const fleet = ensureFleetExists(fleetID);
...
function recordLootEventsForSession(session, lootEntries: any[] = []) {
  const characterID = getSessionCharacterID(session);
  const fleet = getFleetForCharacter(characterID);
  if (!fleet) {
    return false;
...
function createInviteRecord(fleet, inviterCharID, inviteeCharID, placement: Record<string, any> = {}, options: Record<string, any> = {}) {
  const record = {
    fleetID: fleet.fleetID,
    inviterCharID,
    inviteeCharID,
...
function inviteCharacter(session, fleetID, inviteeCharID, wingID, squadID, role, options: Record<string, any> = {}) {
  const fleet = ensureFleetMembership(session, fleetID);
...
function removeMemberFromFleet(fleet, characterID, options: Record<string, any> = {}) {
...
function destroyFleet(fleet, options: Record<string, any> = {}) {
...
function setOptions(session, fleetID, nextOptions: Record<string, any> = {}) {
  const fleet = ensureFleetBoss(session, fleetID);
...
function collectBroadcastRecipientSessions(fleet, senderSession, rangeMode) {
  ...
  if (rangeMode === FLEET.BROADCAST_BUBBLE) {
    ...
    const bubbleSessions = new Set<any>(
