...
const { entityIDsEqual, getEntityMapKey, } = require("../../space/destiny/identity/entityID.js");
...
const FLEET_WARP_COMMAND_ASSOCIATION = Symbol("fleetWarpCommandAssociation");
...
function commitFleetWarpCommandAssociationPlan(plan) {
    ...
    for (const participant of participants) {
        ...
        if (entity.session !== marker.session ||
    ...
    try {
        for (const { entity } of participants) {
            const descriptor = Object.getOwnPropertyDescriptor(entity, FLEET_WARP_COMMAND_ASSOCIATION);
        ...
        for (const { descriptor, entity } of previousDescriptors.reverse()) {
            try {
                if (descriptor) {
                    Object.defineProperty(entity, FLEET_WARP_COMMAND_ASSOCIATION, descriptor);
                }
                else {
...
function clearFleetWarpCommandAssociation(entity) {
    if (!entity || !Object.prototype.hasOwnProperty.call(entity, FLEET_WARP_COMMAND_ASSOCIATION)) {
        return false;
    }
...
function getFleetWarpCommandAssociation(entity, options = {}) {
    ...
    if (options.sceneSystemID !== undefined &&
...
function haveSameFleetWarpCommandAssociation(leftEntity, rightEntity, options = {}) {
    if (!leftEntity ||
        !rightEntity ||
...
//# sourceMappingURL=fleetWarpCommandAssociation.js.map