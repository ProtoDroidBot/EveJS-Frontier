...
function readBerthHostIDFromCustomInfo(value) {
    ...
    return toInt(parsed && parsed[BERTHING_STATE_KEY] &&
        parsed[BERTHING_STATE_KEY].hostAssemblyID, 0);
}
...
function getBerthHostIDForShipEntity(entity) {
    return toInt(entity && entity[BERTHING_HOST_ENTITY_FIELD], readBerthHostIDFromCustomInfo(entity && entity.customInfo));
}
...
function getSolarSystemID(session) {
    return toInt(session && (session.solarsystemid2 ||
        session.solarsystemid ||
        (session._space && session._space.systemID) ||
        session.locationid), 0);
}
function getShipID(session) {
    return toInt(session && (session.shipID ||
        session.shipid ||
...
function buildSmartHangarDefinitions(rows) {
    ...
    for (const row of Array.isArray(rows) ? rows : []) {
        ...
        const acceptedGroupIDs = normalizeAcceptedGroupIDs(smartHangar.acceptedGroupIDs);
        ...
        definitions.set(typeID, {
            ...
            accessRange: Math.max(0, toFiniteNumber(smartHangar.accessRange, DEFAULT_SMART_HANGAR_ACCESS_RANGE)),
...
function getSmartHangarDefinition(typeID) {
    if (!smartHangarsByTypeID) {
        smartHangarsByTypeID = buildSmartHangarDefinitions(readStaticRows(TABLE.SPACE_COMPONENTS_BY_TYPE));
    }
...
function getBerthDirection(validationData) {
    return directionFromDunRotation(validationData.hostEntity.dunRotation || validationData.hostItem.dunRotation) || normalizeVector(validationData.hostEntity.direction, validationData.shipEntity.direction);
}
function writeShipBerthMarker(activeShip, hostAssemblyID, dependencies = {}) {
    ...
    return updateShipItem(activeShip.itemID, (currentItem) => ({
        ...currentItem,
        customInfo: writeBerthHostIDToCustomInfo(currentItem.customInfo, hostAssemblyID),
    }));
}
...
function relocateShipIntoBerth(validationData, dependencies = {}) {
    ...
    const hadMarker = readBerthHostIDFromCustomInfo(validationData.activeShip.customInfo) === toInt(validationData.hostItem.itemID, 0);
    const markerResult = writeShipBerthMarker(validationData.activeShip, validationData.hostItem.itemID, dependencies);
    if (!markerResult || markerResult.success !== true) {
        return { success: false, errorMsg: "BERTHING_STATE_WRITE_FAILED" };
    }
    const previousEntityHostID = getBerthHostIDForShipEntity(validationData.shipEntity);
    setShipEntityBerthHost(validationData.shipEntity, validationData.hostItem.itemID);
    if (typeof validationData.spaceRuntime.stop === "function") {
        validationData.spaceRuntime.stop(validationData.session);
    }
    const relocation = validationData.spaceRuntime.teleportSessionShipToPoint(validationData.session, berthPosition, { direction: berthDirection, refreshOwnerSession: true });
    if (!relocation || relocation.success !== true) {
        setShipEntityBerthHost(validationData.shipEntity, previousEntityHostID);
        if (!hadMarker) {
...
function buildBerthDepartureState(contract, hostItem, hostEntity, shipEntity) {
    ...
    const approachOffset = contract && contract.preBerthPosition
        ...
        : null;
    const fallbackDirection = directionFromDunRotation(hostEntity.dunRotation || hostItem.dunRotation) || normalizeVector(hostEntity.direction);
    const direction = normalizeVector(approachOffset, fallbackDirection);
    const hostRadius = Math.max(toFiniteNumber(hostEntity.radius, 0), toInt(hostItem.typeID, 0) === REFUGE_TYPE_ID ? REFUGE_INTERIOR_RADIUS : 0);
...
function recoverPersistedContract(session, dependencies = {}) {
    ...
    return {
        ...
        berthPosition: cloneVector(activeShip.spaceState && activeShip.spaceState.position, hostItem.spaceState && hostItem.spaceState.position),
...
function beginBerth(session, hostAssemblyID, dependencies = {}) {
    return setBerthingPhase(session, hostAssemblyID, BERTHING_PHASE_APPROACHING, dependencies);
}
function berth(session, hostAssemblyID, dependencies = {}) {
    return setBerthingPhase(session, hostAssemblyID, BERTHING_PHASE_BERTHED, dependencies);
}
...
function clearContract(session, hostAssemblyID, phase, dependencies = {}) {
    ...
    const departure = buildBerthDepartureState(contract, hostItem, hostEntity, shipEntity);
    const relocation = spaceRuntime.teleportSessionShipToPoint(session, departure.position, { direction: departure.direction, refreshOwnerSession: true });
    if (!relocation || relocation.success !== true) {
...
function undockBerth(session, hostAssemblyID, dependencies = {}) {
    return clearContract(session, hostAssemblyID, BERTHING_PHASE_DEPARTING, dependencies);
}
...
function hasActiveContractForHostAssembly(hostAssemblyID) {
    ...
    return [...contractsByCharacterID.values()].some((contract) => toInt(contract && contract.hostAssemblyID, 0) === numericHostID);
}
module.exports = {
...
//# sourceMappingURL=berthingRuntime.js.map