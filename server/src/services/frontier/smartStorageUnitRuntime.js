...
const { ASSEMBLY_STATUS_OFFLINE, ASSEMBLY_STATUS_ONLINE, ASSEMBLY_STATUS_UNDER_CONSTRUCTION, buildAssemblyTransitionTransactionData, isValidAssemblyTransitionSignature, readConstructionState, } = require(path.join(__dirname, "./deploymentRuntime"));
...
function validateStorageUnit(characterID, storageUnitID, options = {}) {
    ...
    if (state.assemblyStatus !== ASSEMBLY_STATUS_OFFLINE &&
...
function validateDeposit(options) {
    ...
    const source = validateGenericContainer(options.characterID, sourceLocationID, options.access, "INVALID_SOURCE");
...
function executeStorageTransaction({ action, characterID, transactionUUID, signature, access, resolveAccess, }) {
