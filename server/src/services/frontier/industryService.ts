...
const { publishIndustryItemsChanged, publishIndustryProductionResult, publishIndustryBlueprintChanged,
  publishIndustryJobLaneChanged } = require("./industryNotifications");
...
function blueprintDict(blueprint) {
  ...
  return buildDict([
    ...
    ...["inputs", "outputs"].map(side => [side, buildDict(
      Object.entries<any>(blueprint[side]).map(([typeID, slot]) => [Number(typeID), buildDict(Object.entries(slot))]),
    )]),
  ]);
}
...
function finishProduction(session, result) {
  ...
  if (result?.data?.facility) trackIndustryProduction(result.data.facility);
  return productionDict(requireSuccess(result).production);
}

function publishTransferNotifications(session, result) {
  if (!result?.success || result.data?.replayed) return;
  ...
  for (const transfer of data.storageTransfers || []) {
    ...
    } catch (error) { log.warn(`[industry] Storage notification failed: ${error.message}`); }
  }
}

function publishIndustryTransferResult(session, result) {
  try { publishTransferNotifications(session, result); }
  catch (error) { log.warn(`[industry] Transfer notification failed: ${error.message}`); }
}

function finishTransfer(session, result) {
  if (result instanceof Promise) return result.then(current => finishTransfer(session, current));
...
class IndustryService extends BaseService {
  ...
  Handle_start_production(args, session) {
    ...
    return finishProduction(session,
      options
  ...
  Handle_get_job_lanes(args, session) {
    ...
    if (!facility || !blueprints.isIndustryFacilityType(facility.typeID) ||
        !canReadFacility(facility, session)) return null;
    requireSuccess(settleIndustryProduction(facilityID, session));
  ...
  Handle_set_job_lane_access(args, session) {
    const lane = requireSuccess(runtime.setLaneAccessPolicy(
      session,
      args?.[0],
      args?.[1],
      args?.[2],
