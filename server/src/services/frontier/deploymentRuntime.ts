...
function hasAssemblyAdminPrivileges(session) {
  const accountRole: bigint = normalizeRoleValue(
    session && session.accountRole,
    0n,
  );
  return (accountRole & (ROLEMASK_VIEW as bigint)) !== 0n;
}

function validateAssemblyAdminSession(session) {
  return hasAssemblyAdminPrivileges(session)
    ? { success: true as const }
    : { success: false as const, errorMsg: "ASSEMBLY_ADMIN_ACCESS_DENIED" };
}
...
function getDismantleContainerSpawnState(item, containerIndex) {
  ...
  const direction = normalizeDirection(
    item && item.spaceState && item.spaceState.direction,
  );
...
function resolveDeploymentPosition(clientPosition, shipPosition, options: Record<string, any> = {}) {
...
function writeConstructionState(item, state, recordIntent = true) {
  ...
  if (toInt(item?.typeID) === NETWORK_NODE_ASSEMBLY_TYPE_ID && info.evejsFrontierNetworkNodeFuel) {
    ...
    if (fuel.quantity === 0 && state.assemblyStatus === ASSEMBLY_STATUS_ONLINE &&
        !require("./networkNodeFuelRuntime").readSuiNetworkNodeFuel(item)) state = { ...state, assemblyStatus: ASSEMBLY_STATUS_OFFLINE };
  }
...
function listAssemblies(filters: Record<string, any> = {}) {
...
function getSolarSystemRecord(solarSystemID) {
  if (!solarSystemsByID) {
    solarSystemsByID = new Map<any, any>(
...
function restorePlacementMaterials(characterID, consumed) {
  ...
  for (const { locationID, flagID, typeID, quantity } of consumed) {
    const result = itemStore.grantItemsToCharacterLocation(
      ...
      locationID,
      flagID,
      [{ itemType: itemStore.getItemMetadata(typeID), quantity }],
    );
    changes.push(...((result.data && result.data.changes) || []));
    if (!result.success) {
      errorMsg = result.errorMsg || "PLACEMENT_MATERIAL_REFUND_FAILED";
      log.error(
        `[FrontierDeployment] Placement material refund failed char=${characterID} ` +
...
function consumePlacementMaterials(characterID, shipItemID, constructionCost) {
  ...
  const plan: any[] = [];
  for (const [typeID, quantity] of Object.entries<any>(constructionCost)) {
  ...
  return { success: true as const, changes, consumed };
}
...
function completeAssemblyActivation(itemID, options: Record<string, any> = {}) {
  ...
  if (state.activationCompleteAtMs > Date.now()) {
    ...
    return { success: true as const, data: { item, pending: true } };
  }
  const definition = getBuildDefinition(state.assemblyTypeID);
  ...
  const presentation = refreshAssemblyStatePresentation(
    session, updateResult.data, readConstructionState(updateResult.data),
  );
  syncChanges(session, [{ item: updateResult.data, previousData: updateResult.previousData }]);
  return { success: true as const, data: { item: updateResult.data, presentation } };
}
...
function validateOwnedConstructionItem(session, itemID) {
  ...
  return { success: true as const, item, state };
}
...
function validateOwnedChainAssembly(session, itemID) {
  ...
  if (!definition || definition.createOnChain !== true) {
    return { success: false as const, errorMsg: "ASSEMBLY_NOT_CHAIN_ANCHORED" };
  }
...
function validateSmartGate(itemID) {
  ...
  return { success: true as const, definition, item, state };
}
...
function refreshAssemblyStatePresentation(session, item, state) {
  ...
  if (entity) {
    ...
    if (typeof scene.broadcastSlimItemChanges === "function") {
      ...
      return { success: true as const, data: { broadcast: true, entity } };
    }
    return { success: true as const, data: { broadcast: false, entity } };
  }
...
function recordInitialSuiAssemblyState(assembly) {
  ...
  if (!item || Number(item.ownerID) !== assembly.ownerId || Number(item.typeID) !== assembly.typeId ||
      state?.assemblyStatus !== assembly.status) throw new Error("Assembly changed before its initial chain state was recorded");
  ...
  if (!result.success) throw new Error("Could not persist the initial assembly state request");
...
function reconcileSuiAssemblyState(assembly, targetStatus, expectedIntentID = null) {
  ...
  if (targetStatus === ASSEMBLY_STATUS_ONLINE && isAssemblyActivationPending(item)) return false;
  if ((intent?.id ?? null) !== expectedIntentID || (intent && intent.targetStatus !== targetStatus)) return false;
  if (state.assemblyStatus === targetStatus && !intent) return true;
  const result = itemStore.updateInventoryItem(item.itemID, currentItem => {
    const info = parseCustomInfo(currentItem.customInfo);
    if (Number(item.typeID) === NETWORK_NODE_ASSEMBLY_TYPE_ID && state.assemblyStatus !== targetStatus) {
      ...
      if (fuel.quantity > 0 || fuel.burnRemainderMs > 0) info.evejsFrontierNetworkNodeFuel = { ...fuel, burnUpdatedAtMs: Date.now() };
      else delete info.evejsFrontierNetworkNodeFuel;
    }
  ...
  if (!result.success || !result.data) throw new Error(`Cannot save chain assembly ${assembly.itemId}`);
  try {
    const session = findAssemblyOwnerSession(null, item.ownerID);
    refreshAssemblyStatePresentation(session, result.data, readConstructionState(result.data));
    syncChanges(session, [{ item: result.data, previousData: result.previousData }]);
...
function commitAssemblyStateTransition(
  ...
  if (!pending) {
    return { success: false as const, errorMsg: "ASSEMBLY_TRANSACTION_NOT_FOUND" };
  }
  if (
    ...
    return { success: false as const, errorMsg: "ASSEMBLY_TRANSACTION_MISMATCH" };
  }
  if (!isValidAssemblyTransitionSignature(signature)) {
    return { success: false as const, errorMsg: "INVALID_ASSEMBLY_SIGNATURE" };
  }
  ...
  if (!updateResult.success || !updateResult.data) {
    return updateResult.success
      ? { success: false as const, errorMsg: "ASSEMBLY_STATE_UPDATE_FAILED" }
      : updateResult;
  }
...
function validateGateLinkForCharacter(characterID, gateID, destinationGateID) {
  ...
  if (source.item.itemID === destination.item.itemID) {
    return { success: false as const, errorMsg: "SMART_GATE_SELF_LINK" };
  }
  if (source.state.solarSystemID === destination.state.solarSystemID) {
    return { success: false as const, errorMsg: "SMART_GATE_SAME_SYSTEM" };
  }
  if (source.state.assemblyTypeID !== destination.state.assemblyTypeID) {
    return { success: false as const, errorMsg: "SMART_GATE_TYPE_MISMATCH" };
  }
  if (
    source.state.destinationGateID > 0 ||
    destination.state.destinationGateID > 0
  ) {
    return { success: false as const, errorMsg: "SMART_GATE_ALREADY_LINKED" };
  }
  if (
    source.state.assemblyStatus !== ASSEMBLY_STATUS_OFFLINE ||
    destination.state.assemblyStatus !== ASSEMBLY_STATUS_OFFLINE
  ) {
    return { success: false as const, errorMsg: "SMART_GATE_MUST_BE_OFFLINE" };
  }
  ...
  if (!Number.isFinite(distanceLightYears)) {
    return { success: false as const, errorMsg: "SMART_GATE_SYSTEM_DATA_UNAVAILABLE" };
  }
  if (distanceLightYears > source.definition.smartGate.rangeLightYears) {
    return {
      success: false as const,
...
function validatePendingGateTransition(
  ...
  pendingAssemblyTransitions.delete(normalizedUUID);
  return { success: true as const, normalizedUUID, pending };
}
...
function updateLinkedGatePair(session, validation) {
  ...
  if (!sourceUpdate.success || !sourceUpdate.data) {
    return sourceUpdate.success
      ? { success: false as const, errorMsg: "SMART_GATE_LINK_UPDATE_FAILED" }
      : sourceUpdate;
  }
...
function validateGateJump(session, gateID) {
  ...
  if (
    ...
    return { success: false as const, errorMsg: "SMART_GATE_NOT_LINKED" };
  }
  ...
  if (
    ...
    return { success: false as const, errorMsg: "SMART_GATE_LINK_MISMATCH" };
  }
  if (destination.state.assemblyStatus !== ASSEMBLY_STATUS_ONLINE) {
    return { success: false as const, errorMsg: "SMART_GATE_DESTINATION_OFFLINE" };
  }
...
function commitGateJumpTransition(
  ...
  if (!spawnState) {
    return { success: false as const, errorMsg: "SMART_GATE_DESTINATION_UNAVAILABLE" };
  }
  const transitionResult = getSpaceTransitions().jumpSessionToSolarSystem(
    session,
...
function validateGateUnlink(session, gateID) {
  ...
  return { success: true as const, destination, source };
}
...
function placeDirectAssembly({
  ...
  if (!createResult.success || !createResult.data) {
    ...
    return createResult.success
      ? { success: false as const, errorMsg: "ASSEMBLY_CREATE_FAILED" }
      : createResult;
  }
...
function depositItems(session, itemID, inventoryID, rawQuantities) {
  ...
  if (state.assemblyStatus !== ASSEMBLY_STATUS_UNDER_CONSTRUCTION) {
    return { success: false as const, errorMsg: "CONSTRUCTION_ALREADY_COMPLETE" };
  }
  ...
  if (sourceLocationID <= 0 || Object.keys(requested).length === 0) {
    return { success: false as const, errorMsg: "INVALID_CONSTRUCTION_DEPOSIT" };
  }
  ...
  for (const [typeID, quantity] of Object.entries(requested)) {
    ...
    if (required <= 0 || quantity > outstanding) {
      return { success: false as const, errorMsg: "INVALID_CONSTRUCTION_MATERIAL" };
    }
  ...
  return { success: true as const, data: depositedAfter };
}

function completeConstruction(itemID, options: Record<string, any> = {}) {
  ...
  const spawnResult = spaceRuntime.spawnDynamicInventoryEntity(
    state.solarSystemID,
    item.itemID,
    { broadcast: true, broadcastOptions: { replaceExisting: true } },
  );
  if (!spawnResult.success) {
    log.warn(
...
function buildDismantleCargoPlan(item, state) {
  ...
  if (!containerType || toInt(containerType.typeID, 0) !== CARGO_CONTAINER_TYPE_ID) {
    return { success: false as const, errorMsg: "CARGO_CONTAINER_TYPE_NOT_FOUND" };
  }
  const capacity = Math.max(
    0,
    toFiniteNumber(containerType.capacity, CARGO_CONTAINER_CAPACITY_FALLBACK),
  ) || CARGO_CONTAINER_CAPACITY_FALLBACK;
  const entriesByOwner = new Map<any, any>();
  const addEntry = (ownerID, entry) => {
    const numericOwnerID = toInt(ownerID, 0);
    if (numericOwnerID <= 0) return false;
    if (!entriesByOwner.has(numericOwnerID)) entriesByOwner.set(numericOwnerID, []);
  ...
  const existingItems = itemStore.listContainerItems(null, item.itemID, null)
    ...
    .sort((left, right) => (
      toInt(left && left.ownerID, 0) - toInt(right && right.ownerID, 0) ||
  ...
  for (const child of existingItems) {
    ...
    if (
      ...
      return { success: false as const, errorMsg: "INVALID_DISMANTLE_CONTENTS" };
    }
  }
  ...
  if (state.assemblyStatus !== ASSEMBLY_STATUS_UNDER_CONSTRUCTION) {
    for (const [rawTypeID, rawQuantity] of Object.entries<any>(state.constructionCost)) {
      const typeID = toInt(rawTypeID, 0);
      const quantity = toInt(rawQuantity, 0);
  ...
  const bins: any[] = [];
  const binsByOwner = new Map<any, any>();
  const createBin = (ownerID) => {
    const bin = { ownerID, usedVolume: 0, moves: [], grants: [] };
    bins.push(bin);
    if (!binsByOwner.has(ownerID)) binsByOwner.set(ownerID, []);
  ...
  for (const [ownerID, entries] of [...entriesByOwner.entries()]
    ...
    for (const entry of entries) {
      ...
      while (remaining > 0) {
        let bin = (binsByOwner.get(ownerID) || []).find((candidate) => (
          entry.unitVolume <= 0 ||
          candidate.usedVolume + entry.unitVolume <= capacity + epsilon
        ));
        if (!bin) bin = createBin(ownerID);
  ...
  const totalByOwner = new Map<any, any>();
  for (const bin of bins) {
    totalByOwner.set(bin.ownerID, (totalByOwner.get(bin.ownerID) || 0) + 1);
  }
  const nextPartByOwner = new Map<any, any>();
  return {
    success: true as const,
    data: {
      capacity,
...
function syncDismantleChanges(session, changes) {
  const changesByOwner = new Map<any, any>();
  for (const change of Array.isArray(changes) ? changes : []) {
    const ownerID = toInt(
      change && change.item && change.item.ownerID ||
        change && change.previousData && change.previousData.ownerID,
      0,
    );
...
function validateDismantleAssembly(session, itemID) {
  ...
  if (state.destinationGateID > 0 || listAssemblies().some((candidate) => (
    candidate.itemID !== item.itemID && candidate.destinationGateID === item.itemID
  ))) {
  ...
  if (
    hasPersistedBerthReference(item.itemID) ||
  ...
  return { success: true as const, characterID, item, state };
}

function dismantleAssembly(session, itemID, options: Record<string, any> = {}) {
  const validation = validateDismantleAssembly(session, itemID);
  if (validation.success === false) return validation;
  const { item, state } = validation;
  if (
    options.requireUnderConstruction === true &&
    state.assemblyStatus !== ASSEMBLY_STATUS_UNDER_CONSTRUCTION
  ) {
    return { success: false as const, errorMsg: "CONSTRUCTION_ALREADY_COMPLETE" };
  }
  ...
  if (cargoPlan.success === false) return cargoPlan;
  ...
  for (const container of containerItems) {
    ...
    if (!spawnResult || spawnResult.success !== true) {
      log.warn(
        `[FrontierDeployment] Dismantle cargo presentation failed ` +
  ...
  log.info(
    `[FrontierDeployment] Dismantled item=${item.itemID} type=${state.assemblyTypeID} ` +
...
function adminSpawnAssembly(session, assemblyTypeID, rawPosition, options: Record<string, any> = {}) {
  ...
  if (rotationInput.length !== 3 || !rotationInput.every(Number.isFinite)) {
    return { success: false as const, errorMsg: "INVALID_DEPLOYMENT_ROTATION" };
  }
...
function validateAdminGateLink(sourceGateID, destinationGateID) {
  ...
  if (
    ...
    return { success: false as const, errorMsg: "ASSEMBLY_OWNER_MISMATCH" };
  }
  if (source.state.solarSystemID === destination.state.solarSystemID) {
    return { success: false as const, errorMsg: "SMART_GATE_SAME_SYSTEM" };
  }
...
function hasPersistedBerthReference(hostAssemblyID) {
  ...
  const berthingRuntime = require(path.join(__dirname, "./berthingRuntime"));
  return Object.values<any>(itemStore.getAllItems()).some((candidate) => (
...
function adminRemoveAssembly(session, itemID) {
  ...
  if (entity) {
    ...
    if (!sceneRemoval || sceneRemoval.success !== true) {
      return sceneRemoval || { success: false as const, errorMsg: "ASSEMBLY_SCENE_REMOVE_FAILED" };
    }
  }
...
function hydrateConstructionEntityFromInventoryItem(entity, item) {
  ...
  if (!isActive) scheduleAssemblyActivation(item.itemID);
  const definition = getBuildDefinition(state.assemblyTypeID);
  if (isSmartGateDefinition(definition)) {
...
function recordAssemblyInteraction(session, itemID) {
  ...
  return { success: true as const, data: { item, state } };
}
