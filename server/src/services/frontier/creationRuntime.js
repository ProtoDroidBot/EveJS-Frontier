...
const { ITEM_FLAGS, findItemById, grantItemsToCharacterLocation, moveItemToLocation, removeInventoryItem, updateInventoryItem, updateShipItem, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const { ABILITY_RELOAD, ABILITY_UNLOAD, getModuleBehaviorName, getRegisteredBehaviorAbilities, resolveCreationAbilityHandler, } = require(path.join(__dirname, "./creationAbilityRuntime"));
...
function commitCreationDraft(item, characterID, rawChanges, session) {
    ...
    for (const action of staged.inventoryActions) {
        ...
        const result = action.fitToCreation === true
            ...
            : moveItemToLocation(currentSource.itemID, action.locationID, action.flagID, null, { affectsFitting: true });
        if (!result.success) {
            rollbackInventoryActions(reversals);
            return {
                success: false,
