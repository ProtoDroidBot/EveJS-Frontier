...
const creationStateChangeListeners = new Set<any>();
...
function notifyCreationStateChanged(event: Record<string, any> = {}) {
  for (const listener of [...creationStateChangeListeners]) {
    try {
      ...
      log.warn(
        `[creation] state listener failed reason=${String(event.reason || "unknown")}: ` +
...
function getCreationModuleAbilities(typeID, options: Record<string, any> = {}) {
  ...
  const resolveTypeAbilities =
    typeof options.getRegisteredTypeAbilities === "function"
...
function buildCreationShipAttributeModifierEntries(moduleItems, options: Record<string, any> = {}) {
  ...
  const resolveEffects =
    typeof options.getTypeDogmaEffects === "function"
      ? options.getTypeDogmaEffects
      : getTypeDogmaEffects;
...
function buildCreationIntrinsicShipAttributeModifierEntries(
  ...
  for (const attributeID of [
    ...
    if (
      toFiniteNumber(attributes && attributes[attributeID], 0) <= 0 &&
      moduleModifierEntries.some(
        (entry) => toInt(entry && entry.modifiedAttributeID, 0) === attributeID,
      )
    ) {
...
function isCreationModuleOnline(item, options: Record<string, any> = {}) {
  const typeID = toInt(item && item.typeID, 0);
  const resolveEffects =
    typeof options.getTypeDogmaEffects === "function"
      ? options.getTypeDogmaEffects
...
function normalizeCreationState(value, options: Record<string, any> = {}) {
...
function filterCreationModuleInventoryItems(item, inventoryItems: any[] = []) {
...
function ensureCreationState(item, characterID) {
  ...
  if (
    ...
    return { success: false as const, errorMsg: "CREATION_ITEM_NOT_OWNED" };
  }
  const template = getCreationTemplate(canonicalItem && canonicalItem.typeID);
  if (!template) {
    return { success: false as const, errorMsg: "CREATION_TEMPLATE_NOT_FOUND" };
  }
  ...
  if (createdItems.length !== plan.length) {
    ...
    return { success: false as const, errorMsg: "CREATION_MODULE_SEED_INCOMPLETE" };
  }
...
function buildDiagnostic(code, change = null, params: Record<string, any> = {}) {
  return {
    code,
    severity: "blocker",
    moduleItemID: toInt(
...
function validateCreationModuleRemoval(item, change) {
  ...
  for (let laneID = 1; laneID <= industryRuntime.MAX_INDUSTRY_JOB_LANES; laneID += 1) {
    inputItems.push(...listContainerItems(null, toInt(item && item.itemID, 0),
      industryRuntime.industryInputFlagForLane(laneID)));
    outputItems.push(...listContainerItems(null, toInt(item && item.itemID, 0),
      industryRuntime.industryOutputFlagForLane(laneID)));
  }
...
function resolveCreationFuelCapacity(shipItem, state, characterID, options: Record<string, any> = {}) {
  ...
  return Math.max(
    0,
    Math.floor(toFiniteNumber(attributes[ATTRIBUTE_FUEL_CAPACITY], 0)),
  );
}

function resolveCreationCapacitorCapacity(
  state,
  characterID,
  options: Record<string, any> = {},
) {
  ...
  for (const module of state && Array.isArray(state.modules) ? state.modules : []) {
    ...
    if (
      !effects.has(EFFECT_CAPACITOR_CAPACITY_ADD_ONLINE) &&
...
function transitionBlackstartCapacitorCapacity(
  shipItem,
  previousCapacityState,
  nextCapacityState,
) {
  const previousBlackstartCapacity = Math.max(
    0,
    toFiniteNumber(previousCapacityState && previousCapacityState.blackstartCapacity, 0),
  );
  const nextBlackstartCapacity = Math.max(
    0,
    toFiniteNumber(nextCapacityState && nextCapacityState.blackstartCapacity, 0),
  );
  ...
  const previousCapacity = Math.max(
    0,
    toFiniteNumber(previousCapacityState && previousCapacityState.capacitorCapacity, 0),
  );
  const nextCapacity = Math.max(
    0,
    toFiniteNumber(nextCapacityState && nextCapacityState.capacitorCapacity, 0),
  );
  const previousRatio = Math.max(
    0,
    Math.min(1, toFiniteNumber(shipItem?.conditionState?.charge, 1)),
  );
...
function trimCreationFuelToCapacity(shipItem, fuelCapacity) {
  ...
  const nextFuelCharge = Math.min(
    previousFuelCharge,
    Math.max(0, toFiniteNumber(fuelCapacity, 0)),
  );
  if (nextFuelCharge >= previousFuelCharge) {
    return { item: shipItem, voidedFuel: 0 };
  }

  const fuelQueue = trimFuelQueueToQuantity(
...
function getFuelQueueQuantityLocal(fuelQueue) {
  return normalizeFuelQueue(fuelQueue).reduce(
    (total, entry) => total + Math.max(0, toFiniteNumber(entry.quantity, 0)),
    0,
  );
}

function stageFuelBlisterTransition(
  shipItem,
  creationState,
  characterID,
  moduleItem,
  ...
  if (nextOnline) {
    const storedQueue = normalizeFuelQueue(
      moduleItem.moduleState && moduleItem.moduleState.reserveFuelQueue,
  ...
  const currentCapacity = resolveCreationFuelCapacity(
    shipItem,
    creationState,
    characterID,
  );
  const fuelTank = resolveShipFuelTank(shipItem, currentCapacity);
  const totalReserveCapacity = resolveCreationReserveFuelCapacity(
    shipItem,
    fuelTank,
  );
  const partitioned = partitionFuelQueueByReserveCapacity(
    currentQueue,
    currentCapacity,
  ...
  const split = splitFuelQueueTail(
    partitioned,
    Math.min(blisterCapacity, reserveQuantity),
  );
...
function setCreationModuleOnlineState(
  ...
  if (
    ...
    return { success: false as const, errorMsg: "CREATION_MODULE_ABILITY_NOT_FOUND" };
  }
  ...
  const previousCapacitorCapacity = resolveCreationCapacitorCapacity(
    ensured.data.state,
    characterID,
    { onlineOverrides: new Map([[numericModuleItemID, previousOnline]]) },
  );
  const nextCapacitorCapacity = resolveCreationCapacitorCapacity(
    ensured.data.state,
    characterID,
    { onlineOverrides: new Map([[numericModuleItemID, nextOnline]]) },
  );
  const fuelTransition = previousOnline !== nextOnline
    ? stageFuelBlisterTransition(
        ensured.data.item,
        ensured.data.state,
        characterID,
        moduleItem,
    ...
    : { success: true as const, shipItem: ensured.data.item, moduleStatePatch: {} };
  const capacitorTransition = transitionBlackstartCapacitorCapacity(
    fuelTransition.shipItem,
    previousCapacitorCapacity,
    nextCapacitorCapacity,
  );
  ...
  if (previousOnline !== nextOnline) {
    ...
    if (toInt(moduleItem.typeID, 0) === TYPE_FUEL_BLISTER) {
      subEvents.push(buildModuleAttributeChangeEvent(
        characterID,
        numericModuleItemID,
        ATTRIBUTE_RESERVE_FUEL_CHARGE,
        toFiniteNumber(
          fuelTransition.moduleStatePatch &&
            fuelTransition.moduleStatePatch.reserveFuelCharge,
          toFiniteNumber(
            moduleItem.moduleState && moduleItem.moduleState.reserveFuelCharge,
            0,
          ),
        ),
        toFiniteNumber(
