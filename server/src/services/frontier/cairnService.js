...
function resolveSessionCorporationID(session) {
    return toInt(session && (session.corporationID || session.corpid || session.corpID), 0);
}
function resolveSessionShipID(session) {
    return toInt(session && (session.activeShipID ||
        session.shipid ||
        session.shipID ||
...
function listRuntimeCairnEntities(scene) {
    ...
    const dynamicEntities = scene.dynamicEntities instanceof Map
        ...
        : Array.isArray(scene.dynamicEntities)
            ...
            : [];
    return dynamicEntities.filter((entity) => (CAIRN_TYPE_IDS.has(toInt(entity && entity.typeID, 0))));
}
function cairnVisibleToViewer(cairn, viewer) {
    if (!cairn || !viewer) {
        return false;
    }
    if (viewer.characterID > 0 &&
    ...
    if (channel === iffRuntime.IFF_CHANNEL_TRIBE) {
        return (transponder.channel === iffRuntime.IFF_CHANNEL_TRIBE &&
            viewer.corporationID > 0 &&
            viewer.corporationID === cairn.ownerCorporationID);
    }
    if (channel === iffRuntime.IFF_CHANNEL_CODE) {
        return (transponder.channel === iffRuntime.IFF_CHANNEL_CODE &&
...
function listVisibleCairns(session, options = {}) {
    ...
    const getCharacterCorporationID = typeof options.getCharacterCorporationID === "function"
    ...
    for (const entity of listRuntimeCairnEntities(scene)) {
        ...
        const ownerID = toInt((item && item.ownerID) || (entity && entity.ownerID), 0);
        let ownerCorporationID = toInt((item && item.corporationID) || (entity && entity.corporationID), 0);
...
class CairnService extends BaseService {
    ...
    Handle_get_visible_cairns(args, session) {
        return buildList(listVisibleCairns(session, this.options).map(buildCairnRow));
    }
}
...
//# sourceMappingURL=cairnService.js.map