...
const { syncChargeGodmaPrimeForSession, syncInventoryItemForSession, } = require(path.join(__dirname, "../character/characterState"));
...
function getSessionCharacterID(session) {
    return toPositiveSafeInteger(session && (session.characterID || session.charID || session.charid));
}
function getSessionActiveShipID(session) {
    return toPositiveSafeInteger(session && session._space && session._space.shipID) || toPositiveSafeInteger(session && (session.activeShipID || session.shipID || session.shipid));
}
...
function getReloadTimeMs(moduleItem) {
    ...
    const effectiveReloadTime = Number(attributes && attributes[ATTRIBUTE_RELOAD_TIME]);
    const staticReloadTime = Number(getTypeAttributeValue(moduleItem && moduleItem.typeID, "reloadTime"));
...
function getLaunchBayReadyAtMs(moduleItem) {
    return Math.max(0, toFiniteNumber(moduleItem && moduleItem.moduleState &&
        moduleItem.moduleState.creationLaunchBayReadyAtMs, 0));
}
function getReadyFileTime(session, moduleItem, nowMs = Date.now(), readyAtMs = null) {
    ...
    return getSessionFileTime(session) + (BigInt(Math.max(0, Math.round(remainingMs))) * FILETIME_TICKS_PER_MILLISECOND);
}
...
function validateLaunchBayReady(moduleItem, nowMs) {
    if (!isLaunchBayModule(moduleItem)) {
        return { success: true, data: { readyAtMs: 0 } };
    }
    ...
    return { success: true, data: { readyAtMs } };
}
...
function buildLaunchPosition(shipEntity, payloadItem) {
    ...
    const payloadRadius = Math.max(1, toFiniteNumber(payloadItem && (payloadItem.spaceRadius || payloadItem.radius), 1));
...
function findMovedItemChange(changes, movedItemID) {
    return (Array.isArray(changes) ? changes : []).find((change) => toPositiveSafeInteger(change && change.item && change.item.itemID) === movedItemID) || null;
}
function rollbackLaunchedPayload(itemID, moduleItemID, previousPayloadState = null, previousModuleState = null) {
    ...
    const moved = itemStore.moveItemToLocation(itemID, moduleItemID, CREATION_MODULE_CHARGE_FLAG_ID, 1, { affectsFitting: true });
...
function calculateHeatTrapTransfer(shipEntity, payloadItem) {
    ...
    const maximumTransfer = Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_HEAT_TRANSFER_AMOUNT], 0));
    const shipPreviousTemperature = Math.max(0, toFiniteNumber(shipEntity && shipEntity.temperatureState && shipEntity.temperatureState.temperature, toFiniteNumber(shipEntity && shipEntity.conditionState && shipEntity.conditionState.temperature, NOMINAL_HEAT_TRAP_TEMPERATURE_K)));
    const transferredHeat = Math.max(0, Math.min(maximumTransfer, shipPreviousTemperature - NOMINAL_HEAT_TRAP_TEMPERATURE_K));
...
function getCreationModuleChargeState(characterID, moduleItemID) {
    ...
    const rows = itemStore.listContainerItems(null, numericModuleItemID, CREATION_MODULE_CHARGE_FLAG_ID);
    if (rows.length === 0) {
    ...
    if (toPositiveSafeInteger(item.ownerID) !== numericCharacterID ||
...
function resolveCreationChargeContext(context) {
    ...
    const creationID = toPositiveSafeInteger(context && context.creationItem && context.creationItem.itemID);
    const moduleItemID = toPositiveSafeInteger(context && context.moduleItemID);
    const moduleTypeID = toPositiveSafeInteger(context && context.moduleEntry && context.moduleEntry.typeID);
...
function getCreationCargoCapacity(characterID, creationItem) {
    ...
    const resourceState = buildShipResourceState(characterID, dogmaContext.data.item, {
        additionalAttributeModifierEntries: dogmaContext.data.shipAttributeModifierEntries || [],
    });
...
function getCargoUsedVolume(characterID, creationID) {
    return itemStore.listContainerItems(characterID, creationID, itemStore.ITEM_FLAGS.CARGO_HOLD).reduce((total, item) => (total + (Math.max(0, itemStore.getInventoryItemUnitVolume(item)) *
        itemQuantity(item))), 0);
}
function validateCargoCapacityAfterExchange(resolved, outgoingCargoVolume, incomingCargoVolume) {
    const capacity = getCreationCargoCapacity(resolved.characterID, resolved.creationItem);
    const used = getCargoUsedVolume(resolved.characterID, resolved.creationID);
    const nextUsed = Math.max(0, used - outgoingCargoVolume + incomingCargoVolume);
    if (nextUsed > used + CAPACITY_EPSILON &&
...
function notifyInventoryChanges(session, changes, loadedItem = null) {
    for (const change of Array.isArray(changes) ? changes : []) {
        ...
        try {
            syncInventoryItemForSession(session, change.item, change.previousData || {}, { emitCfgLocation: false });
        }
        catch (error) {
            log.warn(`[CreationCharge] Inventory notification failed item=${toPositiveSafeInteger(change.item.itemID)}: ${error && error.message ? error.message : error}`);
        }
    }
    if (loadedItem) {
        try {
            ...
            log.warn(`[CreationCharge] Dogma notification failed module=${toPositiveSafeInteger(loadedItem.locationID)}: ${error && error.message ? error.message : error}`);
        }
    }
}
function notifyCreationChanged(resolved) {
    ...
    try {
        const snapshot = buildCreationSnapshot(resolved.creationItem, resolved.characterID, resolved.creationState, resolved.creationTemplate, {
            getLoadedCharge(moduleItemID) {
                const loaded = getCreationModuleChargeState(resolved.characterID, moduleItemID);
        ...
        resolved.session.sendNotification("OnCreationChanged", "clientID", [resolved.creationID, snapshot]);
    }
    catch (error) {
...
function reloadCreationModule(context) {
    ...
    const compatibility = evaluateModuleChargeCompatibility(resolved.moduleTypeID, requestedTypeID);
    const maximumQuantity = toPositiveSafeInteger(compatibility && compatibility.maximumQuantity);
    if (!compatibility.accepted || maximumQuantity <= 0) {
        return { success: false, errorMsg: "CREATION_CHARGE_INCOMPATIBLE" };
    }
    const loadedTypeID = toPositiveSafeInteger(resolved.loadedItem && resolved.loadedItem.typeID);
    ...
    if (resolved.loadedItem &&
        ...
        return {
            ...
            data: {
                ...
                serverTime: getReadyFileTime(resolved.session, resolved.moduleItem, nowMs, getLaunchBayReadyAtMs(resolved.moduleItem)),
    ...
    const sources = resolveReloadSources(resolved, requestedTypeID, ammoItemIDs, neededQuantity);
    if (sources.success === false) {
        return sources;
    }
    const requestedCharge = itemStore.findItemById(sources.data.moveRequests[0].itemID);
    const outgoingCargoVolume = Math.max(0, itemStore.getInventoryItemUnitVolume(requestedCharge)) * neededQuantity;
    const incomingCargoVolume = resolved.loadedItem && loadedTypeID !== requestedTypeID
        ? Math.max(0, itemStore.getInventoryItemUnitVolume(resolved.loadedItem)) * resolved.loadedQuantity
        : 0;
    const capacity = validateCargoCapacityAfterExchange(resolved, outgoingCargoVolume, incomingCargoVolume);
    ...
    const launchBayReadyAtMs = isLaunchBayModule(resolved.moduleItem)
        ...
        : 0;
    const mutation = itemStore.moveItemStacksToLocation(sources.data.moveRequests, resolved.moduleItemID, CREATION_MODULE_CHARGE_FLAG_ID, {
        destinationItemID: loadedTypeID === requestedTypeID && resolved.loadedItem
            ? resolved.loadedItem.itemID
            : 0,
        moveOptions: { affectsFitting: true },
        preMoves: resolved.loadedItem && loadedTypeID !== requestedTypeID
            ? [{
        ...
        ...(isLaunchBayModule(resolved.moduleItem)
            ? {
                ...
                updateItem: (currentItem) => withLaunchBayReadyAt(currentItem, launchBayReadyAtMs, isCreationModuleOnline(resolved.moduleItem)),
            }
            : {}),
    });
    if (!mutation.success) {
    ...
    notifyInventoryChanges(resolved.session, mutation.data.changes, loadedItem);
    notifyCreationChanged(resolved);
    return {
        success: true,
        data: {
            qty: maximumQuantity,
            serverTime: getReadyFileTime(resolved.session, mutation.data.updatedItem || resolved.moduleItem, nowMs, isLaunchBayModule(resolved.moduleItem) ? launchBayReadyAtMs : null),
...
function unloadCreationModule(context) {
    ...
    const incomingCargoVolume = Math.max(0, itemStore.getInventoryItemUnitVolume(resolved.loadedItem)) * resolved.loadedQuantity;
    const capacity = validateCargoCapacityAfterExchange(resolved, 0, incomingCargoVolume);
    if (capacity.success === false) {
        return capacity;
    }
...
function deployCreationLaunchBayPayload(context) {
    ...
    if (resolved.creationState && resolved.creationState.poweredOff === true ||
        !isCreationModuleOnline(resolved.moduleItem)) {
        return { success: false, errorMsg: "CREATION_MODULE_OFFLINE" };
    }
    if (!resolved.loadedItem || resolved.loadedQuantity <= 0) {
        return { success: false, errorMsg: "CREATION_NO_AMMO" };
    }
    if (toPositiveSafeInteger(resolved.loadedItem.categoryID) !== DEPLOYABLE_CATEGORY_ID ||
        toPositiveSafeInteger(resolved.loadedItem.groupID) !== PAYLOAD_GROUP_ID) {
        return { success: false, errorMsg: "CREATION_PAYLOAD_NOT_DEPLOYABLE" };
    }
    ...
    if (systemID <= 0) {
        return { success: false, errorMsg: "CREATION_NOT_IN_SPACE" };
    }
    ...
    const scope = require(path.join(__dirname, "../../space/destiny/identity/interactionScope.js")).buildChildEntityScopeMetadata(shipEntity);
    ...
    const decayDurationMs = launchBayPayloadRuntime.getPayloadDecayDurationMs(resolved.loadedItem);
    const expiresAtMs = decayDurationMs > 0 ? nowMs + decayDurationMs : null;
    const ambientTemperature = Math.max(0, toFiniteNumber(shipEntity && shipEntity.temperatureState &&
        shipEntity.temperatureState.externalTemperature, toFiniteNumber(shipEntity && shipEntity.conditionState &&
        shipEntity.conditionState.externalTemperature, NOMINAL_HEAT_TRAP_TEMPERATURE_K)));
    ...
    const moveResult = itemStore.moveItemsToLocationsAndUpdateItem([{
            ...
            options: {
                ...
                updateMovedItem: (item) => ({
                    ...
                    conditionState: heatTrapState
                        ...
                        : item.conditionState,
                    customInfo: launchBayPayloadRuntime.buildCustomInfoWithLaunchState(item.customInfo, launchState, heatTrapState),
                    spaceState: {
                        ...scope,
                        systemID,
                        position,
        ...
        }], resolved.moduleItemID, (moduleItem) => withLaunchBayReadyAt(moduleItem, nextReadyAtMs, isCreationModuleOnline(resolved.moduleItem)), { flush: true });
    ...
    const launchedChange = findMovedItemChange(moveResult.data && moveResult.data.changes, launchedItemID);
    const launchedItem = itemStore.findItemById(launchedItemID);
    if (!launchedItemID || !launchedItem) {
        return { success: false, errorMsg: "CREATION_PAYLOAD_MOVE_INVALID" };
    }
    const heatCommit = applyHeatTrapTransfer(resolved, shipEntity, heat, nowMs);
    if (!heatCommit.success) {
        rollbackLaunchedPayload(launchedItemID, resolved.moduleItemID, launchedChange && launchedChange.previousData, moveResult.previousData);
        return heatCommit;
    }
    const spawnResult = spaceRuntime.spawnDynamicInventoryEntity(systemID, launchedItemID, { entityScopeMetadata: scope });
    ...
    notifyInventoryChanges(resolved.session, (moveResult.data && moveResult.data.changes) || []);
    notifyCreationChanged(resolved);
    return {
        success: true,
        data: {
            item_id: launchedItemID,
            serverTime: getReadyFileTime(resolved.session, resolved.moduleItem, nowMs, nextReadyAtMs),
...
//# sourceMappingURL=creationChargeRuntime.js.map