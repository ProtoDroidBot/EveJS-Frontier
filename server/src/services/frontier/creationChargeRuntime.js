...
const { CREATION_FITTING_FLAG_ID, getCreationDogmaContext, } = require(path.join(__dirname, "./creationRuntime"));
...
function getReadyFileTime(session, moduleItem) {
    return getSessionFileTime(session) + (BigInt(getReloadTimeMs(moduleItem)) * FILETIME_TICKS_PER_MILLISECOND);
}
