...
const { ABILITY_OFFLINE, ABILITY_ONLINE, registerCreationAbilityHandler, } = require(path.join(__dirname, "./creationAbilityRuntime"));
const { setCreationModuleOnlineState, } = require(path.join(__dirname, "./creationRuntime"));
...
function registerFallbackCreationAbilityHandlers() {
    ...
    const buildOnlineHandler = (online) => ({
        execute(context) {
            const result = setCreationModuleOnlineState(context.creationItem, context.characterID, context.moduleItemID, online, context.session);
            if (!result.success) {
...
//# sourceMappingURL=creationAbilityHandlers.js.map