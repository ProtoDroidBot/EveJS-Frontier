...
const { ASSEMBLY_STATUS_OFFLINE, ASSEMBLY_STATUS_ONLINE, buildAssemblyTransitionTransactionData, isValidAssemblyTransitionSignature, readConstructionState, } = require(path.join(__dirname, "./deploymentRuntime"));
 ...
 */
function validateFuelDeposit({ characterID, networkNodeID, sourceItemID, sourceFlagID, items, }) {
    ...
    for (const stack of validation.sourceStacks) {
        ...
        if (!result || result.success !== true) {
            for (const restore of consumed.reverse()) {
                itemStore.grantItemsToCharacterLocation(transaction.request.characterID, transaction.request.sourceItemID, Math.max(0, transaction.request.sourceFlagID), [{ itemType: validation.fuelTypeID, quantity: restore.quantity }]);
            }
            return {
                success: false,
