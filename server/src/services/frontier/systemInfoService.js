...
const RESOURCE_CATEGORY_ORDER = new Map(RESOURCE_CATEGORIES.map((category, index) => [category, index]));
...
function normalizeResourceComposition(value) {
    ...
    for (const entry of value) {
        ...
        if (!RESOURCE_CATEGORY_ORDER.has(category) ||
    ...
    return [...byCategory.entries()].sort((left, right) => RESOURCE_CATEGORY_ORDER.get(left[0]) - RESOURCE_CATEGORY_ORDER.get(right[0]));
}
...
function buildSystemInfoFromAuthoritativeRecord(record) {
    ...
    return buildDict([
        ...
        ["resource_composition", buildList(normalizeResourceComposition(record.resource_composition))],
        ["feature_resource_composition", buildDict(normalizeFeatureResourceComposition(record.feature_resource_composition))],
        [
            "resource_potential_bucket",
            toBoundedInteger(record.resource_potential_bucket, 1, 5),
        ],
        ["feature_resource_potential_bucket", buildDict(normalizePotentialBucketMap(record.feature_resource_potential_bucket))],
        ["site_resource_potential_bucket", buildDict(normalizePotentialBucketMap(record.site_resource_potential_bucket))],
    ]);
}
...
function resolveSystemID(args = [], session = null) {
    return toPositiveInt(Array.isArray(args) ? args[0] : args, toPositiveInt(session && session._space && session._space.systemID, toPositiveInt(session && (session.solarsystemid2 ?? session.solarsystemid ?? session.locationid), 0)));
}
