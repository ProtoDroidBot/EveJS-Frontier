...
function readEffectiveEntityAttribute(entity, attributeID, attributeName, fallback = null) {
    const attributes = entity && entity.passiveDerivedState &&
        entity.passiveDerivedState.attributes &&
        typeof entity.passiveDerivedState.attributes === "object"
        ? entity.passiveDerivedState.attributes
    ...
    const rawAuthored = getTypeAttributeValue(toInt(entity && entity.typeID, 0), attributeName);
...
function buildPassiveScannerChannel(entity, signatureType, strengthAttributeID, strengthAttributeName, resolutionAttributeID, resolutionAttributeName) {
    const strength = Math.max(0, toFiniteNumber(readEffectiveEntityAttribute(entity, strengthAttributeID, strengthAttributeName, 0), 0));
    if (!(strength > 0)) {
        return null;
    }
    const authoredResolution = toFiniteNumber(readEffectiveEntityAttribute(entity, resolutionAttributeID, resolutionAttributeName, DEFAULT_PASSIVE_SCAN_RESOLUTION), DEFAULT_PASSIVE_SCAN_RESOLUTION);
    ...
    return {
        ...
        strength,
        resolution,
        multiplier: Math.pow(10, boundedStrength - PASSIVE_SCAN_MULTIPLIER_LOG10_REFERENCE),
    };
}
/**
...
function resolvePassiveScannerProfile(entity) {
    const channels = [
        buildPassiveScannerChannel(entity, SIGNATURE_TYPE_GRAVIMETRIC, ATTRIBUTE_PASSIVE_SCAN_GRAV_STRENGTH, "passiveScanGravStrength", ATTRIBUTE_PASSIVE_SCAN_GRAV_RESOLUTION, "passiveScanGravResolution"),
        buildPassiveScannerChannel(entity, SIGNATURE_TYPE_ELECTROMAGNETIC, ATTRIBUTE_PASSIVE_SCAN_EM_STRENGTH, "passiveScanEmStrength", ATTRIBUTE_PASSIVE_SCAN_EM_RESOLUTION, "passiveScanEmResolution"),
    ].filter(Boolean);
    return {
        source: "passive",
        channels,
...
function resolveEntityEmSignatureMultiplier(entity, nowMs = Date.now()) {
    ...
    const signatureEm = readEffectiveEntityAttribute(entity, ATTRIBUTE_SIGNATURE_EM, "signatureEm", null);
    const dogmaSignatureMultiplier = signatureEm !== null &&
...
function replaceResolvedScanningContacts(session, rawEntityIDs, options = {}) {
    ...
    for (const rawEntityID of Array.isArray(rawEntityIDs) ? rawEntityIDs : []) {
        ...
        const previousSources = previousContact &&
            ...
            ? new Set(Object.keys(previousContact.sources).filter((key) => previousContact.sources[key] === true))
        ...
        next.set(entityID, {
            ...
            sources: Object.fromEntries([...previousSources].map((key) => [key, true])),
        });
    ...
    if (previous instanceof Map) {
        for (const [entityID, previousContact] of previous.entries()) {
            ...
            const combatResolved = Number.isFinite(Number(previousContact.combatResolvedAtMs));
            if (remainingSources.size > 0 || combatResolved) {
                next.set(entityID, {
                    ...previousContact,
                    sources: Object.fromEntries([...remainingSources].map((key) => [key, true])),
                });
                delayMsByEntityID.set(entityID, Math.max(0, toFiniteNumber(previousContact.resolveAtMs, nowMs) - nowMs));
            }
        }
    }
 ...
 */
function performPassiveScan({ originPosition, scannerProfile, resolutionConfig = null, candidates, previousScanIds = [], }) {
    ...
    const channels = Array.isArray(profile.channels)
        ? profile.channels.filter((channel) => (channel &&
    ...
    const bestResolution = channels.length > 0
        ? Math.min(...channels.map((channel) => Math.max(Number.EPSILON, toFiniteNumber(channel.resolution, DEFAULT_PASSIVE_SCAN_RESOLUTION))))
        : DEFAULT_PASSIVE_SCAN_RESOLUTION;
    const angularResolutionRadians = Math.min(Math.PI / 2, (bestResolution * Math.PI) / 180);
    ...
    for (const candidate of Array.isArray(candidates) ? candidates : []) {
        ...
        const signatureResults = getPresentedSignatureResults(buildSignatureResultsForTarget({
            baseSignature,
            distanceMeters,
            multipliers,
        ...
        const presentedSignatureResults = forceUnresolved
            ? capSignatureResultsBelowResolutionThreshold(signatureResults, resolvedScanningConfig.resolutionSnrThreshold)
            : signatureResults;
