...
function setTransponderBroadcastState(moduleItemID, active, configuration = null) {
  ...
  if (!moduleItem || !isIffModuleType(moduleItem.typeID)) {
    return { success: false as const, errorMsg: "IFF_TRANSPONDER_NOT_FOUND" };
  }
  ...
  if (!next.channel) {
    return { success: true as const, data: { state: current } };
  }
 ...
 * bookkeeping.
 */
function resolveActiveTransponder(characterID, shipID, options: Record<string, any> = {}) {
...
function startBeacon({
  ...
  if (existing && isBeaconLive(existing, nowMs)) {
    return { success: false as const, errorMsg: "BEACON_ALREADY_ACTIVE" };
  }
  ...
  if (
    ...
    return { success: false as const, errorMsg: "BEACON_REACTIVATION_DELAY" };
  }
  ...
  return { success: true as const, data: { beacon } };
}

function stopBeacon(beaconID, reason = "deactivated") {
  pruneExpiredBeacons();
  if (!activeBeacons.has(toInt(beaconID, 0))) {
    return { success: false as const, errorMsg: "BEACON_NOT_ACTIVE" };
  }
