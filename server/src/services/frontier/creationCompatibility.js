...
function resolveSessionCharacterID(session) {
    return normalizePositiveInteger(session && (session.characterID || session.charID || session.charid));
}
...
function buildCreationModules(state, options = {}) {
    const resolveLoadedCharge = options && typeof options.getLoadedCharge === "function"
        ? options.getLoadedCharge
        : () => null;
    return buildDict((Array.isArray(state && state.modules) ? state.modules : [])
        .map((module) => {
        const moduleItemID = normalizePositiveInteger(module && module.itemID);
        const loaded = resolveLoadedCharge(moduleItemID) || null;
        const loadedTypeID = normalizePositiveInteger(loaded && (loaded.typeID ?? loaded.type_id)) || null;
        const loadedCount = normalizePositiveInteger(loaded && (loaded.quantity ?? loaded.count ?? loaded.loaded_count));
        return [
            moduleItemID,
            buildDict([
                ["item_id", normalizePositiveInteger(module && module.itemID)],
                ["type_id", normalizePositiveInteger(module && module.typeID)],
                ["abilities", buildList((Array.isArray(module && module.abilities) ? module.abilities : [])
...
function buildInteriorPlacements(state) {
    return buildDict((Array.isArray(state && state.interiorPlacements)
...
function buildHardpoints(state) {
    return buildList((Array.isArray(state && state.hardpoints) ? state.hardpoints : [])
        .map((hardpoint) => buildDict([
        ["interior_item_id", normalizePositiveInteger(hardpoint && hardpoint.interiorItemID)],
        ...
        ["attached_item_id", normalizePositiveInteger(hardpoint && hardpoint.attachedItemID) || null],
    ])));
}
function buildCreationSnapshot(item, characterID, state = null, template = null, options = {}) {
    return buildDict([
        ["item_id", item.itemID],
        ["type_id", item.typeID],
...
function buildCreationDiagnostic(diagnostic = {}) {
    return buildDict([
        ...
        ["params", buildDict(Object.entries(diagnostic.params && typeof diagnostic.params === "object"
                ? diagnostic.params
                : {}))],
    ]);
}
...
//# sourceMappingURL=creationCompatibility.js.map