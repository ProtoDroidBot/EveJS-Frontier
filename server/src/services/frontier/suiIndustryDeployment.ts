...
export type SuiIndustryDeployment = {
  industryPackageId: string; industryTypeOrigin: string; fingerprint: string;
};
...
export function readSuiIndustryDeployment(synced: WorldIdentity, env: NodeJS.ProcessEnv = process.env): SuiIndustryDeployment {
  ...
  const configPath = explicit ? path.resolve(explicit) : worldPath ? path.join(path.dirname(path.resolve(worldPath)), "industry.json") : null;
  ...
  if (configPath) {
    ...
    if (raw !== undefined) {
      ...
      file = { schemaVersion: 1, chainId: chain(raw.chainId), worldPackageId: address(raw.worldPackageId, "world package"),
        ...
        packageId: address(raw.packageId, "package"), typeOrigin: address(raw.typeOrigin, "type origin") };
  ...
  const fingerprint = createHash("sha256").update(JSON.stringify({ world, file,
    packageOverride, originOverride, industryPackageId, industryTypeOrigin })).digest("hex");
  return { industryPackageId, industryTypeOrigin, fingerprint };
}
