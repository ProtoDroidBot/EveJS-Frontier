...
function validateBerthingRequest(session, hostAssemblyID, dependencies: Record<string, any> = {}) {
  ...
  if (
    ...
    return { success: false as const, errorMsg: "BERTHING_NOT_IN_SPACE" };
  }
  ...
  if (!hostItem) {
    return { success: false as const, errorMsg: "BERTHING_HOST_NOT_FOUND" };
  }

  const definition = getDefinition(hostItem.typeID);
  if (!definition) {
    return { success: false as const, errorMsg: "BERTHING_HOST_NOT_SMART_HANGAR" };
  }
  if (toInt(hostItem.ownerID, 0) !== characterID) {
    return { success: false as const, errorMsg: "BERTHING_HOST_NOT_OWNED" };
  }
  ...
  if (!activeShip || toInt(activeShip.itemID, 0) !== shipID) {
    return { success: false as const, errorMsg: "BERTHING_SHIP_NOT_FOUND" };
  }
  if (!smartHangarAcceptsShip(definition, activeShip)) {
    return { success: false as const, errorMsg: "BERTHING_SHIP_GROUP_NOT_ACCEPTED" };
  }
...
function writeShipBerthMarker(activeShip, hostAssemblyID, dependencies: Record<string, any> = {}) {
...
function clearShipBerthMarker(activeShip, dependencies: Record<string, any> = {}) {
  const updateShipItem = dependencies.updateShipItem || itemStore.updateShipItem;
  return updateShipItem(activeShip.itemID, (currentItem) => ({
    ...currentItem,
...
function relocateShipIntoBerth(validationData, dependencies: Record<string, any> = {}) {
...
function setBerthingPhase(session, hostAssemblyID, phase, dependencies: Record<string, any> = {}) {
  ...
  const contract: ReturnType<typeof buildContract> & {
    berthPosition?: { x: number; y: number; z: number };
    berthDirection?: { x: number; y: number; z: number };
  } = buildContract(validation.data, phase, previousContract);
...
function recoverPersistedContract(session, dependencies: Record<string, any> = {}) {
  const characterID = getCharacterID(session);
  const shipID = getShipID(session);
  const solarSystemID = getSolarSystemID(session);
  const getActiveShipItem = dependencies.getActiveShipItem || itemStore.getActiveShipItem;
...
function getContractForSession(session, dependencies: Record<string, any> = {}) {
  const characterID = getCharacterID(session);
  if (characterID <= 0) {
    return null;
  }
...
function beginBerth(session, hostAssemblyID, dependencies: Record<string, any> = {}) {
...
function berth(session, hostAssemblyID, dependencies: Record<string, any> = {}) {
  return setBerthingPhase(
    session,
    hostAssemblyID,
...
function completeBerth(session, hostAssemblyID, dependencies: Record<string, any> = {}) {
  return setBerthingPhase(
    session,
    hostAssemblyID,
    BERTHING_PHASE_BERTHED,
...
function clearContract(session, hostAssemblyID, phase, dependencies: Record<string, any> = {}) {
  const characterID = getCharacterID(session);
  ...
  if (
    ...
    return { success: false as const, errorMsg: "BERTHING_CONTRACT_NOT_FOUND" };
  }
...
function undockBerth(session, hostAssemblyID, dependencies: Record<string, any> = {}) {
...
function ejectOccupiedShip(session, hostAssemblyID, dependencies: Record<string, any> = {}) {
  return clearContract(
    session,
    hostAssemblyID,
    BERTHING_PHASE_DEPARTING,
