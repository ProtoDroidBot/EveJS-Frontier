...
const ERROR_MESSAGES = Object.freeze({
  ...
  DIRECT_ASSEMBLY_PORTABLE_ONLY: "Only Network Nodes and Portable Assemblies support direct placement. Other Smart Assemblies must be built through construction sites.",
...
function buildRequestRecord(record) {
  return buildDict(Object.entries<any>(record || {}));
}

function buildAccessValue(value) {
  if (Array.isArray(value)) return buildList(value.map(buildAccessValue));
  if (value && typeof value === "object") {
    return buildDict(Object.entries<any>(value).map(([key, entry]) => [key, buildAccessValue(entry)]));
  }
  return value;
}
...
function requireRequestResult(result) {
  if (!result || result.success !== true) throwAssemblyError(result);
  return result;
}
...
class SmartAssemblyService extends BaseService {
  ...
  Handle_get_my_assemblies(_args, session) {
    return buildList(
      getDeploymentRuntime().listMyAssemblies(session).map((record) => (
        buildDict(Object.entries<any>(record))
      )),
    );
  }
  ...
  Handle_get_construction_template_protocol() {
    ...
    return buildAccessValue({
      ...
      direct_assembly_policy: "network-node-and-portable",
      network_node_policy: "direct-assembly",
      portable_network_node_policy: "direct-assembly",
  ...
  Handle_get_construction_template(args, session) {
    const runtime = getConstructionTemplateRuntime();
    const record = runtime.getConstructionTemplate(
      runtime.playerPrincipal(session),
      args && args[0],
    );
    if (!record) throwAssemblyError({ errorMsg: "CONSTRUCTION_TEMPLATE_NOT_FOUND" });
  ...
  Handle_save_construction_template(args, session) {
    ...
    const result = runtime.saveConstructionTemplate(
      runtime.playerPrincipal(session),
      input,
      {
        ...options,
    ...
    if (!result.success && result.errorMsg) throwAssemblyError(result);
    return buildAccessValue(result);
  }

  Handle_delete_construction_template(args, session) {
    const runtime = getConstructionTemplateRuntime();
    const result = runtime.deleteConstructionTemplate(
      runtime.playerPrincipal(session),
      args && args[0],
    );
    if (!result.success) throwAssemblyError(result);
    return buildAccessValue(result.data);
  }

  Handle_preview_construction_template(args, session) {
    const runtime = getConstructionTemplateRuntime();
    const result = runtime.previewConstructionTemplate(
      session,
      args && args[0],
      requestOptions(args && args[1]),
      requestOptions(args && args[2]),
    );
  ...
  Handle_execute_construction_template(args, session) {
    const result = getConstructionTemplateRuntime().createConstructionPlanFromPreview(
      session,
      args && args[0],
    );
    if (!result.success) throwAssemblyError(result);
  ...
  Handle_get_construction_plan(args, session) {
    ...
    if (!record) throwAssemblyError({ errorMsg: "CONSTRUCTION_PLAN_NOT_FOUND" });
    return buildAccessValue(record);
  }

  Handle_resume_construction_plan(args, session) {
    const result = getConstructionTemplateRuntime().resumePlayerConstructionPlan(
      session,
      args && args[0],
    );
    if (!result.success) throwAssemblyError(result);
  ...
  Handle_pause_construction_plan(args, session) {
    const result = getConstructionTemplateRuntime().pauseConstructionPlan(
      session,
      args && args[0],
    );
    if (!result.success) throwAssemblyError(result);
  ...
  Handle_cancel_construction_plan(args, session) {
    const result = getConstructionTemplateRuntime().cancelConstructionPlan(
      session,
      args && args[0],
    );
    if (!result.success) throwAssemblyError(result);
  ...
  Handle_authorize_construction_plan_action(args, session) {
    const result = getConstructionTemplateRuntime().commitConstructionPlanAction(
      session,
      args && args[0],
      args && args[1],
      args && args[2],
  ...
  Handle_get_request_protocol() {
    ...
    return buildDict([
      ["version", 2],
      ["priorities", buildDict(Object.entries<any>(runtime.ASSEMBLY_REQUEST_PRIORITY))],
      ["priority_flags", buildDict(Object.entries<any>(runtime.ASSEMBLY_REQUEST_PRIORITY_FLAG))],
  ...
  Handle_request_assembly_access(args, session) {
    const result = getAccessRuntime().requestAccess(
      playerAccessActor(session),
      Number(args && args[0]) || 0,
      args && args[1],
      requestOptions(args && args[2]),
    );
  ...
  Handle_get_assembly_access_requests(args, session) {
    const result = getAccessRuntime().listRequests(
      playerAccessActor(session),
      Number(args && args[0]) || 0,
      requestOptions(args && args[1]),
    );
  ...
  Handle_get_assembly_access_grants(args, session) {
    const result = getAccessRuntime().listGrants(
      playerAccessActor(session),
      Number(args && args[0]) || 0,
      requestOptions(args && args[1]),
    );
  ...
  Handle_approve_assembly_access(args, session) {
    const result = getAccessRuntime().approveRequest(
      playerAccessActor(session),
      Number(args && args[0]) || 0,
      args && args[1],
      requestOptions(args && args[2]),
  ...
  Handle_deny_assembly_access(args, session) {
    const result = getAccessRuntime().denyRequest(
      playerAccessActor(session),
      Number(args && args[0]) || 0,
      args && args[1],
      args && args[2],
  ...
  Handle_cancel_assembly_access_request(args, session) {
    const result = getAccessRuntime().cancelRequest(
      playerAccessActor(session),
      Number(args && args[0]) || 0,
      args && args[1],
      args && args[2],
  ...
  Handle_share_assembly_access(args, session) {
    const result = getAccessRuntime().shareAccess(
      playerAccessActor(session),
      Number(args && args[0]) || 0,
      args && args[1],
      args && args[2],
      requestOptions(args && args[3]),
    );
    return buildAccessValue(requireRequestResult(result).data);
  }

  Handle_revoke_assembly_access(args, session) {
    const result = getAccessRuntime().revokeGrant(
      playerAccessActor(session),
      Number(args && args[0]) || 0,
      args && args[1],
      { reason: args && args[2] },
  ...
  Handle_relinquish_assembly_access(args, session) {
    const result = getAccessRuntime().relinquishGrant(
      playerAccessActor(session),
      Number(args && args[0]) || 0,
      args && args[1],
      args && args[2],
  ...
  Handle_confirm_assembly_access_chain(args, _session) {
    const result = getAccessRuntime().confirmGrantChainAuthority(
      args && args[0],
      requestOptions(args && args[1]),
    );
  ...
  Handle_get_assembly_access(args, session) {
    const result = getAccessRuntime().resolveAccess(
      playerAccessActor(session),
      Number(args && args[0]) || 0,
      args && args[1] || [],
    );
  ...
  Handle_get_assembly_access_events(args, session) {
    const result = getAccessRuntime().listEvents(
      playerAccessActor(session),
      Number(args && args[0]) || 0,
      requestOptions(args && args[1]),
    );
  ...
  Handle_open_shared_assembly_gui(args, session) {
    const result = getAccessRuntime().issueGuiSession(
      playerAccessActor(session),
      Number(args && args[0]) || 0,
      {
        ...requestOptions(args && args[1]),
  ...
  Handle_validate_shared_assembly_gui(args, session) {
    const result = getAccessRuntime().validateGuiSession(
      args && args[1],
      playerAccessActor(session),
      Number(args && args[0]) || 0,
      args && args[2],
  ...
  Handle_create_request(args, session) {
    ...
    const options = authorizedRequestOptions(
      session,
      sourceAssemblyID,
      getAccessRuntime().ASSEMBLY_ACCESS_CAPABILITY.OPERATE,
      args && args[3],
    );
    const result = getRequestRuntime().createRequest(
      sourceAssemblyID,
      Number(args && args[1]) || 0,
      args && args[2],
      options,
    );
  ...
  Handle_get_requests(args, session) {
    ...
    const result = requireRequestResult(getRequestRuntime().listRequests(
      assemblyID,
      authorizedRequestOptions(
        session,
        assemblyID,
        getAccessRuntime().ASSEMBLY_ACCESS_CAPABILITY.GUI_VIEW,
        args && args[1],
      ),
    ));
  ...
  Handle_get_request_signals(args, session) {
    const assemblyID = Number(args && args[0]) || 0;
    const result = requireRequestResult(getRequestRuntime().listSignals(
      assemblyID,
      authorizedRequestOptions(
        session,
        assemblyID,
  ...
  Handle_claim_request(args, session) {
    const assemblyID = Number(args && args[0]) || 0;
    const result = getRequestRuntime().claimRequest(
      assemblyID,
      args && args[1],
      authorizedRequestOptions(
        session,
  ...
  Handle_renew_request_claim(args, session) {
    const assemblyID = Number(args && args[0]) || 0;
    const result = requireRequestResult(getRequestRuntime().renewClaim(
      assemblyID,
      args && args[1],
      args && args[2],
      authorizedRequestOptions(
  ...
  Handle_release_request(args, session) {
    const assemblyID = Number(args && args[0]) || 0;
    const result = requireRequestResult(getRequestRuntime().releaseClaim(
      assemblyID,
      args && args[1],
      args && args[2],
      authorizedRequestOptions(
  ...
  _fulfillRequest(args, session) {
    const assemblyID = Number(args && args[0]) || 0;
    const result = getRequestRuntime().fulfillRequest(
      assemblyID,
      args && args[1],
      args && args[2],
      args && args[3],
  ...
  Handle_fail_request(args, session) {
    const assemblyID = Number(args && args[0]) || 0;
    const result = getRequestRuntime().failRequest(
      assemblyID,
      args && args[1],
      args && args[2],
      args && args[3],
  ...
  Handle_cancel_request(args, session) {
    const assemblyID = Number(args && args[0]) || 0;
    const result = getRequestRuntime().cancelRequest(
      assemblyID,
      args && args[1],
      authorizedRequestOptions(
        session,
  ...
  Handle_set_destination_system(args, session) {
    ...
    const result = getDeploymentRuntime().setCatapultDestination(
      session,
      gateID,
    ...
    if (!result || result.success !== true) {
      log.info(
        `[smartAssemblyService] Catapult destination rejected char=${session && session.characterID} ` +
  ...
  Handle_clear_destination(args, session) {
    ...
    if (!result || result.success !== true) {
      log.info(
        `[smartAssemblyService] Catapult destination clear rejected ` +
  ...
  Handle_system_jump(args, session) {
    ...
    if (!result || result.success !== true) {
      log.info(
        `[smartAssemblyService] Catapult jump rejected char=${session && session.characterID} ` +
