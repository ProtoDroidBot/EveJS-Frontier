...
function buildCreationLayout(template) {
  ...
  const entries = Object.entries<any>(parts)
    .map(([rawPartID, part]) => {
...
function buildCreationModules(state, options: Record<string, any> = {}) {
...
function buildCreationDiagnostic(diagnostic: Record<string, any> = {}) {
  return buildDict([
    ...
    ["params", buildDict(Object.entries<any>(
