...
function resolveFuelGroupID(typeID, deps: Record<string, any> = {}) {
...
function resolveShipFuelTank(
  ...
  const categoryID = toInt(
    ...
    toInt(shipItem && shipItem.categoryID, 0),
  );
  ...
  const baseCapacity = Math.max(
    ...
    toFiniteNumber(
      typeAttributes && (
        typeAttributes[ATTRIBUTE_FUEL_CAPACITY] ??
        typeAttributes[String(ATTRIBUTE_FUEL_CAPACITY)]
      ),
      0,
    ),
  );
...
function resolveInitialShipFuelCapacity(
  shipItem,
  characterID,
  deps: Record<string, any> = {},
) {
...
function initializeNewShipFuelTank(
  shipItem,
  characterID,
  deps: Record<string, any> = {},
) {
  const typeID = toInt(shipItem && shipItem.typeID, 0);
  if (!INITIAL_FULL_FUEL_SHIP_TYPE_IDS.has(typeID)) {
    return { success: true as const, skipped: true, data: { item: shipItem } };
  }
  const capacity = resolveInitialShipFuelCapacity(shipItem, characterID, deps);
  if (capacity <= 0) {
    return { success: false as const, errorMsg: "INITIAL_FUEL_CAPACITY_MISSING" };
  }
...
function normalizeFuelQueue(rawQueue) {
  ...
  for (const entry of rawEntries) {
    ...
    const fuelTypeID = toInt(
      tuple ? entry[0] : entry && (entry.fuelTypeID ?? entry.typeID),
      0,
    );
...
function getShipFuelQueue(shipItem) {
  ...
  const fuelCharge = Math.max(
    0,
    toFiniteNumber(conditionState && conditionState.fuelCharge, 0),
  );
...
function getShipFuelTypeID(shipItem) {
  ...
  const conditionState =
    getConditionState(shipItem);
  const fuelTypeID = toInt(conditionState && conditionState.fuelTypeID, 0);
  return fuelTypeID > 0 ? fuelTypeID : 0;
}

function getFuelProperties(fuelTypeID, deps: Record<string, any> = {}) {
  const resolveDogmaAttributes = typeof deps.getTypeDogmaAttributes === "function"
    ? deps.getTypeDogmaAttributes
    : getTypeDogmaAttributes;
  const attributes = resolveDogmaAttributes(toInt(fuelTypeID, 0));
  const properties: Record<string, number> = {};
  for (const [propertyName, attributeID] of Object.entries(
    FUEL_PROPERTY_ATTRIBUTE_IDS,
  )) {
    properties[propertyName] = Math.max(
      0,
      toFiniteNumber(
        attributes && (
...
function getFuelEfficiency(fuelTypeID, deps: Record<string, any> = {}) {
...
function calculateFuelQueueProperties(
  ...
  const activeProperties: Record<string, number> = activeBatch
    ? getFuelProperties(activeBatch.fuelTypeID, deps)
    : Object.fromEntries(
        Object.keys(FUEL_PROPERTY_ATTRIBUTE_IDS).map((propertyName) => [propertyName, 0]),
      );
  return {
    totalQuantity,
...
function getShipFuelProperties(shipItem, deps: Record<string, any> = {}) {
...
function calculateFueledCapacitorRecharge(
  ...
  const currentAmount = Math.min(
    capacity,
    Math.max(0, toFiniteNumber(currentCapacitorAmount, 0)),
  );
  let normalizedFuelQueue = normalizeFuelQueue(
    Array.isArray(fuelQueue) ? fuelQueue : fuelComposition,
  );
  ...
  const fuelProperties = calculateFuelQueueProperties(
    normalizedFuelQueue,
    deps,
  );
  ...
  const requestedRechargeEnergy = Math.min(
    missingEnergy,
    effectiveRechargeRate * normalizedDeltaSeconds,
  );
  ...
  const rechargedEnergy = Math.min(
    ...
    Math.max(0, suppliedEnergy - suppliedConsumerEnergy),
  );
  const rawNextFuelCharge = Math.max(
    0,
    getFuelQueueQuantity(nextFuelQueue),
  );
  const nextFuelCharge = rawNextFuelCharge <= FUEL_EPSILON
    ? 0
    : rawNextFuelCharge;
  const normalizedNextFuelQueue = trimFuelQueueToQuantity(
    nextFuelQueue,
    nextFuelCharge,
  );
  const nextFuelProperties = calculateFuelQueueProperties(
    normalizedNextFuelQueue,
    deps,
  );
  const rawNextCapacitorAmount = Math.min(
    capacity,
    currentAmount + rechargedEnergy,
  );
...
function loadFuelIntoShipTank({
  ...
  const updateShipItem = typeof deps.updateShipItem === "function"
    ? deps.updateShipItem
    : itemStore.updateShipItem;
  const isFreeStationFuelSupplyItem =
    typeof deps.isFreeStationFuelSupplyItem === "function"
  ...
  if (ownerID <= 0 || numericShipID <= 0) {
    return { success: false as const, errorMsg: "FUEL_SHIP_NOT_FOUND" };
  }
  const shipItem = findItemById(numericShipID);
  if (!shipItem || toInt(shipItem.ownerID, 0) !== ownerID) {
    return { success: false as const, errorMsg: "FUEL_SHIP_NOT_OWNED" };
  }
  const fuelTank = resolveShipFuelTank(shipItem, fuelCapacity, deps);
  if (!fuelTank.isShip) {
    return { success: false as const, errorMsg: "FUEL_SHIP_INVALID" };
  }
  if (requestedQuantity <= 0) {
    return { success: false as const, errorMsg: "FUEL_QUANTITY_INVALID" };
  }
  if (!isSupportedFuelType(numericTypeID, deps)) {
    return { success: false as const, errorMsg: "FUEL_TYPE_UNSUPPORTED" };
  }
  ...
  if (
    previousFuelCharge > FUEL_EPSILON &&
  ...
  const isFreeStationSupply = (stack) =>
    numericTypeID === UNSTABLE_FUEL_TYPE_ID &&
  ...
  sourceStacks.sort((left, right) =>
    Number(isFreeStationSupply(right)) - Number(isFreeStationSupply(left)) ||
