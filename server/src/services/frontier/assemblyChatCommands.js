...
const { resolveSpawnPosition, } = require(path.join(__dirname, "../chat/frontierLandscapeCommands"));
...
const ERROR_MESSAGES = Object.freeze({
    ...
    ASSEMBLY_NOT_EMPTY: "The assembly contains inventory. Empty it before removal.",
    ASSEMBLY_NOT_FOUND: "Smart Assembly was not found.",
    ASSEMBLY_OCCUPIED: "The assembly has an active or persisted berth. Undock first.",
    ASSEMBLY_OWNER_MISMATCH: "Both gates must have the same owner before they can be linked.",
    ...
    ASSEMBLY_UNDER_CONSTRUCTION: "The assembly is still under construction.",
    CONSTRUCTION_MATERIALS_INCOMPLETE: "The construction site does not contain all required materials.",
    ...
    SMART_GATE_DESTINATION_REQUIRED: "Link the Smart Gate before setting it online.",
    SMART_GATE_LINK_MISMATCH: "The reciprocal Smart Gate link is inconsistent.",
    SMART_GATE_LINK_NOT_SUPPORTED: "Catapults do not use reciprocal Smart Gate links.",
...
function getCharacterID(session) {
    return toPositiveInt(session && (session.characterID ?? session.charid ?? session.characterid), 0);
}
function getSolarSystemID(session) {
    return toPositiveInt(session && (session.solarsystemid2 ?? session.solarsystemid), 0);
}
...
function resolveDependencies(options = {}) {
    return {
        deploymentRuntime: options.deploymentRuntime || require(path.join(__dirname, "./deploymentRuntime")),
        spaceRuntime: options.spaceRuntime || require(path.join(__dirname, "../../space/runtime")),
    };
}
...
function resolveDefinition(definitions, query) {
    ...
    if (numericTypeID > 0) {
        const match = definitions.find((entry) => Number(entry.assemblyTypeID) === numericTypeID);
        return match
            ? { success: true, match }
    ...
    const exactMatches = definitions.filter((entry) => normalizeName(entry.name) === normalizedQuery);
    if (exactMatches.length === 1) {
        return { success: true, match: exactMatches[0] };
    }
    const partialMatches = definitions.filter((entry) => normalizeName(entry.name).includes(normalizedQuery));
...
function formatDefinitionList(definitions, scope, query) {
    ...
    return [
        ...
        ...rows.map((definition) => (`${definition.assemblyTypeID}: ${definition.name}` +
...
function findNearestAssembly(session, dependencies) {
    ...
    return dependencies.deploymentRuntime.listAssemblies({ solarSystemID: systemID })
        ...
        distance: record.position
            ? Math.hypot(record.position.x - ship.position.x, record.position.y - ship.position.y, record.position.z - ship.position.z)
            : Number.POSITIVE_INFINITY,
    }))
        .sort((left, right) => (left.distance - right.distance || left.record.itemID - right.record.itemID))[0]?.record || null;
}
...
function executeFrontierAssemblyCommand(session, argumentText, options = {}) {
    ...
    if (action === "types" || action === "catalog") {
        ...
        return {
            success: true,
            message: formatDefinitionList(dependencies.deploymentRuntime.listAssemblyDefinitions(), scope, tokens.join(" ")),
        };
    }
    if (action === "list") {
    ...
    if (action === "spawn") {
        ...
        const placement = resolveSpawnPosition(ship, placementText || `ahead=${DEFAULT_SPAWN_DISTANCE_METERS}`);
        ...
        const spawnResult = dependencies.deploymentRuntime.adminSpawnAssembly(session, resolved.match.assemblyTypeID, placement.position, {
    ...
    if (action === "state" || action === "online" || action === "offline") {
        ...
        const result = dependencies.deploymentRuntime.adminSetAssemblyState(session, itemID, stateText === "online" ? 2 : 1);
    ...
    if (action === "link") {
        ...
        const result = dependencies.deploymentRuntime.adminLinkSmartGates(session, sourceID, destinationID);
        return result.success
...
//# sourceMappingURL=assemblyChatCommands.js.map