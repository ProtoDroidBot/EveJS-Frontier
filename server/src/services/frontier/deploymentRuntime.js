...
function validateNpcConstructionMaterialPlan(actor, materialPlan) {
    ...
    if (!ship || ship.transient === true ||
        ...
        return { success: false, errorMsg: "NPC_CONSTRUCTION_ASSIGNMENT_INVALID" };
    }
    const allowedOwners = new Set([actor.ownerPrincipalID, toInt(ship.ownerID, 0)].filter(ownerID => ownerID > 0));
    const seen = new Set();
    for (const entry of materialPlan) {
        ...
        if (!allowedOwners.has(toInt(item.ownerID, 0))) {
            return { success: false, errorMsg: "NPC_CONSTRUCTION_MATERIAL_ACCESS_DENIED" };
        }
        seen.add(itemID);
    }
    return { success: true };
 ...
 */
function validateNpcAssemblyControl(actor, itemID, jobID = null) {
    const actorResult = validateNpcAssemblyActor(actor);
    if (!actorResult.success)
        return actorResult;
    const item = itemStore.findItemById(toInt(itemID, 0));
    ...
    if (!originalOperator) {
        ...
        if (!access.success) {
            return { success: false, errorMsg: access.errorMsg || "ASSEMBLY_ACCESS_DENIED" };
        }
    }
    return { success: true, actor: actorResult.data, item, state, metadata };
}
...
function registerNpcAssemblyForFaction(actor, itemID, input = {}) {
    ...
    const update = itemStore.updateInventoryItem(itemID, (currentItem) => {
        ...
        const servicePriorityFlags = [...new Set((Array.isArray(input.servicePriorityFlags) ? input.servicePriorityFlags : [])
                .map((value) => String(value || "").trim().toLowerCase())
                .filter((value) => supportedPriorityFlags.has(value)))].sort();
        return {
            ...currentItem,
            customInfo: writeNpcConstructionMetadata(currentItem, {
                ...metadata,
...
function completeConstruction(itemID, options = {}) {
    ...
    const clearance = validateFrontierAssemblyClearance(state.solarSystemID, definition, position, { ignoredItemID: item.itemID });
    if (!clearance.success)
        return clearance;
