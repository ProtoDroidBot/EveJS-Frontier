...
class DeploymentMgrService extends BaseService {
    ...
    Handle_build_deployable(args, session) {
        ...
        const result = getDeploymentRuntime().buildDeployable(session, typeID, args && args[1], args && args[2]);
        if (!result || result.success !== true) {
            ...
            const placementDetails = Number.isFinite(Number(details.deploymentDistance))
                ...
                : "";
            log.info(`[deploymentMgr] build_deployable rejected char=${session && session.characterID} ` +
...
//# sourceMappingURL=deploymentMgrService.js.map