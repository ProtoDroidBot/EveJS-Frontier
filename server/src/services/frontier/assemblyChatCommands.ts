...
function tokenizeArguments(value) {
  ...
  if (quote) {
    return { success: false as const, errorMsg: "UNTERMINATED_QUOTE", tokens: [] };
  }
  if (current) {
    tokens.push(current);
  }
...
function resolveDefinition(definitions, query) {
  ...
  if (numericTypeID > 0) {
    ...
    return match
      ? { success: true as const, match }
      : { success: false as const, errorMsg: "ASSEMBLY_TYPE_NOT_SUPPORTED", suggestions: [] };
  }
  const normalizedQuery = normalizeName(source);
  if (!normalizedQuery) {
    return { success: false as const, errorMsg: "ASSEMBLY_TYPE_REQUIRED", suggestions: [] };
  }
  const exactMatches = definitions.filter(
    (entry) => normalizeName(entry.name) === normalizedQuery,
  );
  if (exactMatches.length === 1) {
    return { success: true as const, match: exactMatches[0] };
  }
  const partialMatches = definitions.filter(
    (entry) => normalizeName(entry.name).includes(normalizedQuery),
  );
  if (partialMatches.length === 1) {
    return { success: true as const, match: partialMatches[0] };
  }
...
function executeFrontierAssemblyCommand(session, argumentText, options: Record<string, any> = {}) {
  const dependencies = resolveDependencies(options);
  const parsed = tokenizeArguments(argumentText);
  if (parsed.success === false) {
    return { success: false as const, message: "Assembly command has an unterminated quote." };
  }
  const tokens = parsed.tokens;
  const action = String(tokens.shift() || "help").toLowerCase();

  if (action === "help") {
    return { success: true as const, message: COMMAND_USAGE };
  }
  ...
  if (action === "list") {
    ...
    if (tokens.length > 0) {
      return { success: false as const, message: "Usage: /assembly list [mine|system|all]" };
    }
    let filters: Record<string, any> = {};
    if (scope === "mine") {
      const characterID = getCharacterID(session);
      if (!characterID) {
        return { success: false as const, message: "Select a character first." };
      }
      filters = { ownerID: characterID };
    } else if (scope === "system") {
      const solarSystemID = getSolarSystemID(session);
      if (!solarSystemID) {
        return { success: false as const, message: "You must be in space to list this system." };
      }
  ...
  if (action === "info" || action === "inspect") {
    ...
    if (tokens.length > 0) {
      return { success: false as const, message: "Usage: /assembly info <itemID|nearest>" };
    }
    let record = null;
    ...
    if (!record) {
      return { success: false as const, message: "Smart Assembly was not found." };
    }
    ...
    if (
      ...
      return { success: false as const, message: "Smart Assembly is not visible from this session." };
    }
    return {
      success: true as const,
      data: { record },
  ...
  if (action === "spawn") {
    ...
    const queryTokens: any[] = [];
    for (const token of tokens) {
      const normalized = token.toLowerCase();
      if (isPlacementToken(normalized)) {
        if (placementText) {
          return { success: false as const, message: "Specify only one assembly placement." };
        }
        placementText = token;
      } else if (normalized.startsWith("state=")) {
        if (stateSpecified) {
          return { success: false as const, message: "Specify only one assembly state." };
        }
        stateSpecified = true;
    ...
    if (!["default", "offline", "online"].includes(stateText)) {
      return { success: false as const, message: "Assembly state must be default, offline, or online." };
    }
    ...
    const ship = scene && typeof scene.getShipEntityForSession === "function"
      ...
      : null;
    if (!scene || !ship) {
      return { success: false as const, message: "You must be in space with an active ship." };
    }
    ...
    if (!spawnResult.success) {
      return { success: false as const, message: formatRuntimeError(spawnResult, "Assembly spawn failed") };
    }
  ...
  if (action === "state" || action === "online" || action === "offline") {
    ...
    if (
      ...
      return { success: false as const, message: "Usage: /assembly state <itemID> <offline|online>" };
    }
    ...
    return result.success
      ...
      : { success: false as const, message: formatRuntimeError(result, "Assembly state change failed") };
  }
  ...
  if (action === "link") {
    ...
    if (!sourceID || !destinationID || tokens.length > 0) {
      return { success: false as const, message: "Usage: /assembly link <sourceGateID> <destinationGateID>" };
    }
    ...
    return result.success
      ? { success: true as const, data: result.data, message: `Linked Smart Gates ${sourceID} and ${destinationID}.` }
      : { success: false as const, message: formatRuntimeError(result, "Smart Gate link failed") };
  }

  if (action === "unlink") {
    const gateID = parseItemID(tokens.shift());
    if (!gateID || tokens.length > 0) {
      return { success: false as const, message: "Usage: /assembly unlink <gateID>" };
    }
    ...
    return result.success
      ...
      : { success: false as const, message: formatRuntimeError(result, "Smart Gate unlink failed") };
  }

  if (action === "complete") {
    const itemID = parseItemID(tokens.shift());
    if (!itemID || tokens.length > 0) {
      return { success: false as const, message: "Usage: /assembly complete <constructionSiteID>" };
    }
    const result = dependencies.deploymentRuntime.adminCompleteConstruction(session, itemID);
    return result.success
      ? { success: true as const, data: result.data, message: `Completed Smart Assembly construction ${itemID}.` }
      : { success: false as const, message: formatRuntimeError(result, "Construction completion failed") };
  }
  ...
  if (action === "remove") {
    ...
    if (!itemID || confirmation !== "confirm" || tokens.length > 0) {
      return { success: false as const, message: "Usage: /assembly remove <itemID> confirm" };
    }
    const result = dependencies.deploymentRuntime.adminRemoveAssembly(session, itemID);
    return result.success
      ? { success: true as const, data: result.data, message: `Removed Smart Assembly ${itemID}.` }
      : { success: false as const, message: formatRuntimeError(result, "Assembly removal failed") };
  }

  return { success: false as const, message: COMMAND_USAGE };
}
