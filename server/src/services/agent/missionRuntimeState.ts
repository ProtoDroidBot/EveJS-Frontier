...
function isRetiredActiveMissionRuntimeRecord(record = null, fallbackAgentID: number | string = 0) {
...
function normalizeMissionStandingEvent(record, fallback: Record<string, any> = {}) {
  const source =
...
function normalizeMissionRecord(record, agentID, fallbackMissionSequence) {
  ...
  return {
    ...
    remoteCompletable:
      record.remoteCompletable === undefined || record.remoteCompletable === null
        ? null
    ...
    bookmarkIDsByRole:
      record.bookmarkIDsByRole && typeof record.bookmarkIDsByRole === "object"
        ? Object.fromEntries(
            Object.entries<any>(record.bookmarkIDsByRole)
              .map(([role, bookmarkID]) => [
                normalizeText(role, ""),
...
function normalizeEpicArcStatusRecord(record, fallbackMissionID: number | string = 0) {
...
function normalizeEpicArcCompletionRecord(record, fallbackArcID: number | string = 0) {
  const source = record && typeof record === "object" ? record : {};
...
function normalizeEpicArcStatusMap(value) {
  ...
  for (const [arcKey, missionMap] of Object.entries<any>(source)) {
    const epicArcID = toPositiveInteger(arcKey, 0);
    if (!epicArcID || !missionMap || typeof missionMap !== "object") {
      continue;
    }
    const normalizedMissionMap: Record<string, any> = {};
    for (const [missionKey, statusRecord] of Object.entries<any>(missionMap)) {
...
function normalizeEpicArcCompletionMap(value) {
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  const result: Record<string, any> = {};
  for (const [arcKey, completionRecord] of Object.entries<any>(source)) {
...
function normalizeStorylineOfferRecord(record, fallbackAgentID: number | string = 0) {
  const source = record && typeof record === "object" ? record : {};
...
function normalizeStorylineOfferMap(value, options: Record<string, any> = {}) {
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  const result: Record<string, any> = {};
  for (const [agentKey, record] of Object.entries<any>(source)) {
    if (
...
function normalizeStorylineMilestoneMap(value) {
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  const result: Record<string, any> = {};
  for (const [counterKey, record] of Object.entries<any>(source)) {
...
function normalizeStorylineProgress(progress) {
  ...
  const counters =
    ...
    !Array.isArray(source.countersByFactionAndLevel)
      ? source.countersByFactionAndLevel
      : {};
  for (const [counterKey, counter] of Object.entries<any>(counters)) {
...
function markStorylineMilestoneIssued(characterState, milestoneRecord: Record<string, any> = {}, options: Record<string, any> = {}) {
  const storylineProgress = ensureStorylineProgress(characterState);
...
function recordPendingStorylineOffer(characterState, offerRecord: Record<string, any> = {}, options: Record<string, any> = {}) {
  const storylineProgress = ensureStorylineProgress(characterState);
...
function expirePendingStorylineOffers(characterState, options: Record<string, any> = {}) {
  const storylineProgress = ensureStorylineProgress(characterState);
  if (!storylineProgress) {
    return [];
  }
  ...
  const expiredOffers: any[] = [];
  for (const [agentKey, offerRecord] of Object.entries<any>(
...
function ensureCharacterState(state, characterID) {
  ...
  const retiredMissionAgentIDs = new Set<any>();
  const retiredMissionBookmarkIDs = new Set<any>();
  const collectRetiredMissionAgentID = (record, fallbackAgentID: number | string = 0, forceRetired = false) => {
  ...
  for (const [agentKey, missionRecord] of Object.entries<any>(
    ...
    if (isRetiredActiveMission) {
      for (const bookmarkID of Object.values<any>(
        missionRecord &&
  ...
  for (const milestoneRecord of Object.values<any>(
  ...
  for (const counterRecord of Object.values<any>(
  ...
  for (const missionMap of Object.values<any>(
    missionStatusByArcID && typeof missionStatusByArcID === "object"
      ? missionStatusByArcID
      : {},
  )) {
    for (const [missionKey, statusRecord] of Object.entries<any>(
  ...
  for (const completionRecord of Object.values<any>(
  ...
  for (const [agentKey, missionRecord] of Object.entries<any>(characterState.missionsByAgentID)) {
  ...
  for (const [agentKey, replayUntilFileTime] of Object.entries<any>(
    characterState.declineTimersByAgentID,
  )) {
  ...
  for (const [agentKey, completed] of Object.entries<any>(
    characterState.completedCareerAgentIDs,
  )) {
    const normalizedAgentID = toPositiveInteger(agentKey, 0);
...
function recordEpicArcMissionStatus(characterState, statusRecord: Record<string, any> = {}, options: Record<string, any> = {}) {
  const source =
...
function recordEpicArcCompletion(characterState, completionRecord: Record<string, any> = {}, options: Record<string, any> = {}) {
  const epicArcProgress = ensureEpicArcProgress(characterState);
...
function getEpicArcReplayBlock(characterState, epicArcID, options: Record<string, any> = {}) {
  const epicArcProgress = ensureEpicArcProgress(characterState);
  const normalizedArcID = toPositiveInteger(epicArcID, 0);
