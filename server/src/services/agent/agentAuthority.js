...
const { buildPositiveIDSet, isFrontierProfile, shouldIncludeAgentRecord, } = require(path.join(__dirname, "./agentCompatibility"));
const REPO_ROOT = path.resolve(__dirname, "../../../..");
const AGENTS_IN_SPACE_JSONL_PATH = path.join(REPO_ROOT, "tools", "DataSync", "source_json", "eve-online-static-data-3396210-jsonl", "agentsInSpace.jsonl");
let cache = null;
...
function buildIndexes(agentsByID) {
    ...
    for (const index of Object.values(indexes)) {
        for (const [key, agentIDs] of index.entries()) {
            index.set(key, [...new Set(agentIDs)].sort((left, right) => left - right));
        }
    }
    return indexes;
}
function buildCache() {
    ...
    for (const [agentID, record] of Object.entries(payload.agentsByID || {})) {
        ...
        if (!shouldIncludeAgentRecord(normalizedRecord, compatibilityProfile, validStationIDs, validSolarSystemIDs)) {
            continue;
        }
        agentsByID.set(normalizedAgentID, normalizedRecord);
    }
    if (frontierProfile) {
        log.info(`[AgentAuthority] Frontier location filter retained ${agentsByID.size} of ${sourceAgentCount} agents`);
    }
    return {
        payload: {
            ...payload,
...
//# sourceMappingURL=agentAuthority.js.map