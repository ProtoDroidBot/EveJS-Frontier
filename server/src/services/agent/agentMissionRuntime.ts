...
function buildDefaultMissionLastActionInfo(overrides: Record<string, any> = {}) {
  return {
...
function buildActiveMissionLastActionInfo(overrides: Record<string, any> = {}) {
...
function buildMarshalTuple(items: any[] = []) {
  return {
    type: "tuple",
...
function buildMissionTemplateMatchCandidates(
  ...
  return Array.from<any>(candidates);
}
...
function resolveMissionTemplateForClientMission(
  ...
  const candidateTemplates: any[] = [];
  const seenTemplateIDs = new Set<any>();
...
function getClientMissionRecord(input) {
  ...
  const candidateMissionIDs: any[] = [];
  const seenMissionIDs = new Set<any>();
...
function buildEpicArcStatusPayloadFromCharacterState(characterState) {
  ...
  const statusByArcID: Record<string, any> = {};
  const progress = normalizeEpicArcProgress(characterState.epicArcProgress);
  for (const [arcKey, missionMap] of Object.entries<any>(progress.missionStatusByArcID || {})) {
    for (const statusRecord of Object.values<any>(missionMap || {})) {
  ...
  for (const missionRecord of Object.values<any>(characterState.missionsByAgentID || {})) {
...
function buildMissionConversationTargetLocationWrap(missionRecord, fallbackAgentRecord = null) {
  ...
  if (targetAgentRecord) {
    const location: Record<string, any> = buildAgentLocationWrap(targetAgentRecord);
...
function listMissionBookmarks(characterID, missionRecord) {
  const bookmarkIDsByRole =
    missionRecord && missionRecord.bookmarkIDsByRole && typeof missionRecord.bookmarkIDsByRole === "object"
      ...
      : {};
  return Object.values<any>(bookmarkIDsByRole)
...
function collectBookmarkIDsByFolder(bookmarks: any[] = []) {
...
function ensureMissionBookmarks(characterID, agentRecord, missionRecord, missionTemplate = null) {
  ...
  const bookmarkIDsByRole: Record<string, any> = {};
  ...
  const addLocationBookmark = (role, location, options: Record<string, any> = {}) => {
...
function cleanupMissionBookmarks(characterID, missionRecord) {
  const bookmarkIDs = Object.values<any>(
    missionRecord && missionRecord.bookmarkIDsByRole && typeof missionRecord.bookmarkIDsByRole === "object"
      ? missionRecord.bookmarkIDsByRole
      : {},
  )
...
function sumItemStackQuantity(items: any[] = []) {
...
function notifyMissionInventoryChanges(characterID, changes: any[] = []) {
  const normalizedCharacterID = normalizePositiveInteger(characterID, 0);
...
function syncMissionRecordState(characterID, agentID, options: Record<string, any> = {}) {
...
function syncMissionRecordForDungeonInstance(instanceOrID, options: Record<string, any> = {}) {
...
function buildMissionTrackerInfoTuple(characterID, missionRecord, options: Record<string, any> = {}) {
...
function buildMissionItemProgressSnapshot(characterID, missionRecord, itemSpec, options: Record<string, any> = {}) {
...
function resolveMissionTrackerDungeonContext(characterID, missionRecord, options: Record<string, any> = {}) {
...
function listActiveMissionEncounterEntityIDs(instance, tracked = null) {
  ...
  const encounterStates = Object.values<any>(normalizeObject(
    instance &&
      instance.spawnState &&
    ...
    .filter(({ encounterState }: Record<string, any>) => (
...
function findNextMissionGateEntityID(characterID, missionRecord, options: Record<string, any> = {}) {
  ...
  const candidateGateKeys = Object.entries<any>(gateStatesByKey)
    .filter(([, gateState]) => {
      if (
...
function buildEncounterDestroyAllObjectiveTuple(characterID, missionRecord, options: Record<string, any> = {}) {
...
function buildDungeonFetchObjectiveState(characterID, missionRecord, options: Record<string, any> = {}) {
...
function buildMiningObjectiveState(characterID, missionRecord, options: Record<string, any> = {}) {
  const itemSpec = resolvePrimaryMissionItemSpec(missionRecord) || resolvePlaceholderCargo();
  const requiredQuantity = Math.max(1, normalizeInteger(itemSpec.quantity, 1));
  const progress = buildMissionItemProgressSnapshot(characterID, missionRecord, itemSpec, {
    destinationLocationID: normalizePositiveInteger(
...
function buildEncounterObjectiveState(characterID, missionRecord, options: Record<string, any> = {}) {
...
function buildMissionDeliveryTuple(missionRecord, itemSpec, itemProgress, options: Record<string, any> = {}) {
  const normalizedTypeID = normalizePositiveInteger(itemSpec && itemSpec.typeID, 0);
  if (!normalizedTypeID) {
    return null;
  }
...
function dedupeObjectiveTuples(tuples: any[] = []) {
  const result: any[] = [];
...
function buildEncounterDestroyObjectiveTuple(characterID, missionRecord, options: Record<string, any> = {}) {
...
function resolveMissionObjectiveState(characterID, missionRecord, options: Record<string, any> = {}) {
...
function buildMissionObjectiveSequence(characterID, missionRecord, options: Record<string, any> = {}) {
...
function getMissionInfoItems(characterID, agentID, options: Record<string, any> = {}) {
...
function getAllMissionObjectives(characterID, agentID, options: Record<string, any> = {}) {
  const missionRecord = getMissionRecordForRead(characterID, agentID);
  if (!missionRecord || normalizeText(missionRecord.runtimeStatus, "offered") !== "accepted") {
    return [];
  }
...
function isMissionRemoteCompletable(missionRecord, options: Record<string, any> = {}) {
  const clientMissionRecord = getClientMissionRecord(missionRecord);
...
function buildPendingStorylineOfferJournalRows(
  ...
  const pendingOffers =
    ...
    typeof characterState.storylineProgress.pendingOffersByAgentID === "object"
      ? characterState.storylineProgress.pendingOffersByAgentID
      : {};
  return Object.values<any>(pendingOffers)
    .map((offerRecord) => {
...
function offerMission(characterID, agentRecord) {
  ...
  if (!normalizedCharacterID || !normalizedAgentID) {
    return { success: false as const, errorMsg: "INVALID_AGENT_OR_CHARACTER" };
  }
  ...
  return mutateCharacterState(normalizedCharacterID, (characterState, state) => {
    ...
    if (existingMissionRecord) {
      return { kind: "existing" as const, missionRecord: cloneValue(existingMissionRecord) };
    }
    ...
    if (forcedMissionID) {
      ...
      if (isMissionOfferAllowedForAgent(agentRecord, forcedClientMission)) {
        ...
        return { kind: "offered" as const, missionRecord: cloneValue(forcedRecord) };
      }
    }
    ...
    return { kind: "offered" as const, missionRecord: cloneValue(missionRecord) };
  });
}
...
function offerSpecificMission(characterID, agentRecord, clientMissionRecord) {
  ...
  if (!normalizedCharacterID || !normalizedAgentID || !clientMissionRecord) {
    return { success: false as const, errorMsg: "INVALID_AGENT_OR_MISSION" };
  }
...
function grantMissionInitialAgentGift(characterID, agentRecord, missionRecord) {
  ...
  if (!typeID) {
    return { success: true as const, data: { granted: false } };
  }
...
function quitMission(characterID, agentRecord) {
  ...
  if (
    ...
    return { success: false as const, errorMsg: "MISSION_NOT_ACCEPTED" };
  }
  const expectedMissionSequence = normalizePositiveInteger(
  ...
  return { success: true as const, data: { missionRecord } };
}
...
function completeMission(characterID, agentRecord) {
  ...
  if (!missionRecord) {
    return { success: false as const, errorMsg: "MISSION_NOT_FOUND" };
  }
  if (!missionRecord.objectiveCompleted) {
    return { success: false as const, errorMsg: "OBJECTIVES_NOT_COMPLETE" };
  }
  ...
  return { success: true as const, data: { missionRecord, rewardResult: rewardResult.data } };
}
...
function getJournalDetails(characterID) {
  ...
  const missionRows = Object.values<any>(characterState.missionsByAgentID || {})
    .map((missionRecord) => {
...
function getCompletedCareerAgentMap(characterID, agentIDs: any[] = []) {
  const characterState = getCharacterStateSnapshot(characterID);
  const completedByID =
    characterState &&
...
function markMissionObjectiveComplete(characterID, options: Record<string, any> = {}) {
  const normalizedCharacterID = normalizePositiveInteger(characterID, 0);
  if (!normalizedCharacterID) {
    return { success: false as const, errorMsg: "CHARACTER_NOT_FOUND" };
  }
  ...
  const missionRecords = Object.values<any>(
  ...
  return { success: true as const, data: { markedAgentIDs } };
}
...
function findReferredConversationMission(characterID, targetAgentID) {
  ...
  const characterState = getCharacterStateSnapshot(normalizedCharacterID);
  const acceptedMissions = Object.values<any>(
    (characterState && characterState.missionsByAgentID) || {},
...
function doAgentAction(characterID, agentID, actionID = null) {
  ...
  if (normalizedActionID === null) {
    ...
    return { success: true as const, data: buildIdleConversation(agentRecord, normalizedCharacterID) };
  }
