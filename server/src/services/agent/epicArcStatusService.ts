    ...
    for (const missionRecord of missionRecords) {
      ...
      if (sourceAgentID > 0) {
        ...
        if (!rootAgentArcIDs.has(sourceAgentID)) {
          rootAgentArcIDs.set(sourceAgentID, new Set<any>());
        }
...
function characterStateReferencesAgent(characterState, agentID) {
  ...
  return Object.values<any>(characterState.missionsByAgentID || {})
...
function agentHasEpicMissionsForCharacter(agentID, characterID = 0, options: Record<string, any> = {}) {
  const normalizedAgentID = normalizePositiveInteger(agentID, 0);
  if (!normalizedAgentID) {
    return false;
  }
