...
const { getAgentByID, } = require(path.join(__dirname, "./agentAuthority"));
...
const { getEpicArcReplayBlock, } = require(path.join(__dirname, "./missionRuntimeState"));
...
function getMissionByRuntimeRecord(missionRecord) {
    const missionID = normalizeMissionID(missionRecord &&
...
function buildAvailabilityCache() {
    ...
    for (const missionRecord of epicMissions) {
        const arcKey = String(normalizePositiveInteger(missionRecord.epicArcID, 0) ||
    ...
    for (const missionRecords of missionsByArc.values()) {
        ...
        for (const missionRecord of missionRecords) {
            ...
            if (sourceAgentID > 0) {
                ...
                rootAgentArcIDs.get(sourceAgentID).add(normalizePositiveInteger(missionRecord.epicArcID, 0));
            }
        }
    }
    return {
...
function getExistingCharacterStateSnapshot(characterID) {
    ...
    const result = repo.read(MISSION_RUNTIME_TABLE, `/charactersByID/${normalizedCharacterID}`);
...
function isRootEpicArcAgent(agentID) {
    return getAvailabilityCache().rootSourceAgentIDs.has(normalizePositiveInteger(agentID, 0));
}
...
function agentHasEpicMissionsForCharacter(agentID, characterID = 0, options = {}) {
    ...
    return (isRootEpicArcAgent(normalizedAgentID) &&
...
class EpicArcStatusService extends BaseService {
    ...
    Handle_AgentHasEpicMissionsForCharacter(args, session) {
        return agentHasEpicMissionsForCharacter(args && args[0], normalizePositiveInteger(session && session.characterID, 0));
    }
}
...
//# sourceMappingURL=epicArcStatusService.js.map