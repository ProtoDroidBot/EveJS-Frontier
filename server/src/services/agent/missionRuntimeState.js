...
const { isRetiredMissionTemplateIdentifier, } = require(path.join(__dirname, "../../config/productionMissionPolicy"));
const { isRetiredOrdinaryEncounterMissionForAgent, } = require(path.join(__dirname, "./missionAuthority"));
...
function isRetiredActiveMissionRuntimeRecord(record = null, fallbackAgentID = 0) {
    ...
    const missionIdentifier = record.contentID ??
        ...
        record.missionID ??
        record.missionTemplateID;
    const agentID = toPositiveInteger(record.agentID, toPositiveInteger(fallbackAgentID, 0));
    return isRetiredOrdinaryEncounterMissionForAgent(agentID, missionIdentifier, record);
}
...
function futureFileTimeString(deltaMs = 0) {
    ...
    return (BigInt(Date.now() + safeDeltaMs) * 10000n + 116444736000000000n).toString();
}
function normalizeMissionStandingEvent(record, fallback = {}) {
    ...
    return {
        ...
        eventTypeID: normalizeOptionalInteger(source.eventTypeID, normalizeOptionalInteger(fallback.eventTypeID, null)),
        applySocial: Object.prototype.hasOwnProperty.call(source, "applySocial")
        ...
        messageHeader: normalizeText(source.messageHeader, normalizeText(fallback.messageHeader, "")),
        messageBody: normalizeText(source.messageBody, normalizeText(fallback.messageBody, "")),
        int_1: Object.prototype.hasOwnProperty.call(source, "int_1")
            ? normalizeOptionalInteger(source.int_1, null)
            : normalizeOptionalInteger(fallback.int_1, null),
        int_2: Object.prototype.hasOwnProperty.call(source, "int_2")
            ? normalizeOptionalInteger(source.int_2, null)
            : normalizeOptionalInteger(fallback.int_2, null),
        int_3: Object.prototype.hasOwnProperty.call(source, "int_3")
...
function normalizeMissionRewards(record) {
    const source = record && typeof record === "object"
        ? record
        : {};
    const normalizeRewardItemList = (value) => (Array.isArray(value)
        ? value
        ...
        : []);
    const rawStandings = source.rawStandings && typeof source.rawStandings === "object"
        ? {
    ...
    const standingEvents = source.standingEvents && typeof source.standingEvents === "object"
    ...
    return {
        ...
        bonusTimeIntervalMinutes: Math.max(0, Math.round(toFiniteNumber(source.bonusTimeIntervalMinutes, 0))),
        ...
        standingEvents: {
            ...
            declined: normalizeMissionStandingEvent(standingEvents.declined || source.declinedRawStandings, {}),
            failed: normalizeMissionStandingEvent(standingEvents.failed || source.failedRawStandings, {}),
            offerExpired: normalizeMissionStandingEvent(standingEvents.offerExpired || source.offerExpiredRawStandings, {}),
            bonus: normalizeMissionStandingEvent(standingEvents.bonus || source.bonusRawStandings, {}),
        },
    };
}
...
function normalizeMissionRecord(record, agentID, fallbackMissionSequence) {
    ...
    const normalizedAgentID = toPositiveInteger(record.agentID ?? agentID, toPositiveInteger(agentID, 0));
    if (!normalizedAgentID) {
        return null;
    }
    const missionSequence = toPositiveInteger(record.missionSequence, toPositiveInteger(fallbackMissionSequence, 0));
    const contentID = normalizeMissionContentID(record.contentID ?? record.clientMissionID ?? record.missionTemplateID, null);
    const missionTemplateID = normalizeText(record.missionTemplateID ||
    ...
    return {
        ...
        missionTypeLabel: normalizeText(record.missionTypeLabel, "UI/Agents/MissionTypes/Encounter"),
        ...
        remoteCompletable: record.remoteCompletable === undefined || record.remoteCompletable === null
            ? null
            : toBoolean(record.remoteCompletable, false),
        offeredAtFileTime: normalizeText(record.offeredAtFileTime, currentFileTimeString()),
        acceptedAtFileTime: record.acceptedAtFileTime
            ? normalizeText(record.acceptedAtFileTime, currentFileTimeString())
            : null,
        expiresAtFileTime: normalizeText(record.expiresAtFileTime, futureFileTimeString(OFFER_EXPIRY_MS)),
        ...
        missionPosition: record.missionPosition && typeof record.missionPosition === "object"
            ? {
            ...
            : null,
        bookmarkIDsByRole: record.bookmarkIDsByRole && typeof record.bookmarkIDsByRole === "object"
            ? Object.fromEntries(Object.entries(record.bookmarkIDsByRole)
                .map(([role, bookmarkID]) => [
                normalizeText(role, ""),
            ...
            : {},
        cargo: record.cargo && typeof record.cargo === "object"
            ? {
            ...
            : null,
        pickupLocation: record.pickupLocation && typeof record.pickupLocation === "object"
            ? cloneValue(record.pickupLocation)
            : null,
        dropoffLocation: record.dropoffLocation && typeof record.dropoffLocation === "object"
...
function normalizeHistoryEntry(entry) {
    ...
    const contentID = normalizeMissionContentID(entry.contentID ?? entry.clientMissionID ?? entry.missionTemplateID, null);
    const missionTemplateID = normalizeText(entry.missionTemplateID ||
    ...
    return {
        ...
        completedAtFileTime: normalizeText(entry.completedAtFileTime, currentFileTimeString()),
...
function normalizeEpicArcStatusRecord(record, fallbackMissionID = 0) {
    ...
    const missionID = normalizeMissionContentID(source.missionID ?? source.contentID ?? fallbackMissionID, null);
    if (missionID === null ||
    ...
    return {
        missionID,
        acceptedDate: normalizeText(source.acceptedDate ?? source.acceptedAtFileTime, ""),
        completedDate: normalizeText(source.completedDate ?? source.completedAtFileTime, ""),
        quitDate: normalizeText(source.quitDate ?? source.declinedAtFileTime ?? source.quitAtFileTime, ""),
...
function normalizeEpicArcCompletionRecord(record, fallbackArcID = 0) {
    const source = record && typeof record === "object" ? record : {};
    const epicArcID = toPositiveInteger(source.epicArcID ?? source.arcID ?? fallbackArcID, 0);
...
function normalizeEpicArcCompletionMap(value) {
    ...
    for (const [arcKey, completionRecord] of Object.entries(source)) {
        const normalizedCompletion = normalizeEpicArcCompletionRecord(completionRecord, arcKey);
...
function normalizeEpicArcProgress(progress) {
    ...
    normalized.missionStatusByArcID = normalizeEpicArcStatusMap(source.missionStatusByArcID);
    normalized.completedArcsByID = normalizeEpicArcCompletionMap(source.completedArcsByID);
    return normalized;
}
function ensureEpicArcProgress(characterState) {
    ...
    characterState.epicArcProgress = normalizeEpicArcProgress(characterState.epicArcProgress);
...
function normalizeStorylineCounter(record, fallbackKey = "") {
    ...
    const missionLevel = toPositiveInteger(source.missionLevel ?? source.level, fallbackMissionLevel || 0);
...
function normalizeStorylineOfferMap(value, options = {}) {
    ...
    for (const [agentKey, record] of Object.entries(source)) {
        if (options.excludeRetiredActive === true &&
    ...
    for (const [counterKey, record] of Object.entries(source)) {
        ...
        if (record && typeof record === "object" && !Array.isArray(record)) {
            result[normalizedCounterKey] = {
                ...
                issuedCompletedCount: Math.max(0, toPositiveInteger(record.issuedCompletedCount, 0)),
...
function normalizeStorylineProgress(progress) {
    ...
    normalized.version = toPositiveInteger(source.version, 0) || 1;
    const counters = source.countersByFactionAndLevel &&
    ...
    for (const [counterKey, counter] of Object.entries(counters)) {
        ...
        normalized.countersByFactionAndLevel[getStorylineCounterKey(normalizedCounter.factionID, normalizedCounter.missionLevel)] = normalizedCounter;
    }
    normalized.issuedMilestonesByCounterKey = normalizeStorylineMilestoneMap(source.issuedMilestonesByCounterKey);
    normalized.pendingOffersByAgentID = normalizeStorylineOfferMap(source.pendingOffersByAgentID, { excludeRetiredActive: true });
    normalized.declinedOffersByAgentID = normalizeStorylineOfferMap(source.declinedOffersByAgentID, { excludeRetired: true });
    normalized.expiredOffersByAgentID = normalizeStorylineOfferMap(source.expiredOffersByAgentID, { excludeRetired: true });
    return normalized;
}
function ensureStorylineProgress(characterState) {
    ...
    characterState.storylineProgress = normalizeStorylineProgress(characterState.storylineProgress);
...
function isStorylineQualifyingNormalMission(agentRecord, missionRecord) {
    ...
    return (toPositiveInteger(agentRecord.agentTypeID, 0) === BASIC_AGENT_TYPE_ID &&
...
function recordStorylineQualifyingCompletion(characterState, agentRecord, missionRecord, options = {}) {
    ...
    const previousCounter = normalizeStorylineCounter(storylineProgress.countersByFactionAndLevel[counterKey], counterKey) || {
        factionID,
        missionLevel,
        completedCount: 0,
    ...
    const completedAtFileTime = normalizeText(options.completedAtFileTime, currentFileTimeString());
    ...
    const reachedMilestone = completedCount > 0 && completedCount % STORYLINE_THRESHOLD === 0;
    return {
        recorded: true,
        counterKey,
        factionID,
        ...
        milestoneNumber,
        reachedMilestone,
        nextMilestoneAt: reachedMilestone
...
function markStorylineMilestoneIssued(characterState, milestoneRecord = {}, options = {}) {
    const storylineProgress = ensureStorylineProgress(characterState);
    const counterKey = normalizeText(milestoneRecord.counterKey ||
    ...
    const issuedCompletedCount = Math.max(0, toPositiveInteger(milestoneRecord.issuedCompletedCount ?? milestoneRecord.completedCount, 0));
    if (issuedCompletedCount <= 0 ||
    ...
    const issuedRecord = {
        counterKey,
        issuedCompletedCount,
        issuedAtFileTime: normalizeText(milestoneRecord.issuedAtFileTime || options.issuedAtFileTime, currentFileTimeString()),
...
function recordPendingStorylineOffer(characterState, offerRecord = {}, options = {}) {
    ...
    const pendingOfferRecord = {
        ...offerRecord,
        status: "pending",
        offeredAtFileTime: offerRecord.offeredAtFileTime ||
            options.offeredAtFileTime ||
            currentFileTimeString(),
        expiresAtFileTime: offerRecord.expiresAtFileTime ||
    ...
    if (existingOfferRecord) {
        const existingOffer = normalizeStorylineOfferRecord(existingOfferRecord, normalizedOffer.agentID);
        return {
            recorded: false,
...
function transitionPendingStorylineOffer(characterState, agentID, status, options = {}) {
    ...
    const storylineProgress = ensureStorylineProgress(characterState);
    const pendingOfferRecord = storylineProgress &&
    ...
    const pendingOffer = normalizeStorylineOfferRecord(pendingOfferRecord, normalizedAgentID);
    delete storylineProgress.pendingOffersByAgentID[String(normalizedAgentID)];
    const closedAtFileTime = normalizeText(options.closedAtFileTime, currentFileTimeString());
    const transitionedOffer = normalizeStorylineOfferRecord({
        ...pendingOffer,
        status: normalizedStatus,
        closedAtFileTime,
        declinedAtFileTime: normalizedStatus === "declined"
            ? closedAtFileTime
            : pendingOffer.declinedAtFileTime,
        expiredAtFileTime: normalizedStatus === "expired"
            ? closedAtFileTime
            : pendingOffer.expiredAtFileTime,
        lastUpdatedAtMs: toFiniteNumber(options.nowMs, Date.now()),
    }, normalizedAgentID);
    const targetMap = normalizedStatus === "declined"
...
function expirePendingStorylineOffers(characterState, options = {}) {
    ...
    const nowFileTime = normalizeText(options.nowFileTime || options.closedAtFileTime, currentFileTimeString());
    let nowBigInt;
    try {
    ...
    const expiredOffers = [];
    for (const [agentKey, offerRecord] of Object.entries(storylineProgress.pendingOffersByAgentID)) {
        ...
        const transitionResult = transitionPendingStorylineOffer(characterState, normalizedOffer.agentID, "expired", {
...
function ensureCharacterState(state, characterID) {
    ...
    if (!state.charactersByID[String(normalizedCharacterID)] ||
    ...
    const collectRetiredMissionAgentID = (record, fallbackAgentID = 0, forceRetired = false) => {
        ...
        const agentID = toPositiveInteger(record && (record.agentID ?? record.lastAgentID), toPositiveInteger(fallbackAgentID, 0));
        if (agentID > 0) {
            retiredMissionAgentIDs.add(agentID);
        }
    };
    for (const [agentKey, missionRecord] of Object.entries(characterState.missionsByAgentID || {})) {
        const isRetiredActiveMission = isRetiredActiveMissionRuntimeRecord(missionRecord, agentKey);
        if (isRetiredActiveMission) {
            for (const bookmarkID of Object.values(missionRecord &&
    ...
    for (const mapName of [
        ...
        for (const [agentKey, offerRecord] of Object.entries(offerMap && typeof offerMap === "object" ? offerMap : {})) {
            collectRetiredMissionAgentID(offerRecord, agentKey, mapName === "pendingOffersByAgentID" &&
    ...
    for (const milestoneRecord of Object.values(issuedMilestones && typeof issuedMilestones === "object" ? issuedMilestones : {})) {
    ...
    for (const counterRecord of Object.values(storylineCounters && typeof storylineCounters === "object" ? storylineCounters : {})) {
    ...
    for (const missionMap of Object.values(missionStatusByArcID && typeof missionStatusByArcID === "object"
        ? missionStatusByArcID
        : {})) {
        for (const [missionKey, statusRecord] of Object.entries(missionMap && typeof missionMap === "object" ? missionMap : {})) {
            collectRetiredMissionAgentID(statusRecord, 0, isRetiredActiveMissionRuntimeRecord({
                ...statusRecord,
    ...
    for (const completionRecord of Object.values(completedArcsByID && typeof completedArcsByID === "object"
        ? completedArcsByID
        : {})) {
    ...
    for (const [agentKey, missionRecord] of Object.entries(characterState.missionsByAgentID)) {
        const normalizedMissionRecord = normalizeMissionRecord(missionRecord, agentKey, state.nextMissionSequence);
        ...
        state.nextMissionSequence = Math.max(toPositiveInteger(state.nextMissionSequence, 1), normalizedMissionRecord.missionSequence + 1);
    }
    ...
    for (const [agentKey, replayUntilFileTime] of Object.entries(characterState.declineTimersByAgentID)) {
    ...
    for (const [agentKey, completed] of Object.entries(characterState.completedCareerAgentIDs)) {
        const normalizedAgentID = toPositiveInteger(agentKey, 0);
...
function getMutableState() {
    ...
    let state = result.success && result.data && typeof result.data === "object"
        ? result.data
        : null;
    if (!state) {
...
function getEpicArcStatusRecord(characterState, epicArcID, missionID) {
    ...
    const statusRecord = epicArcProgress.missionStatusByArcID[String(normalizedArcID)] &&
...
function recordEpicArcMissionStatus(characterState, statusRecord = {}, options = {}) {
    const source = statusRecord && typeof statusRecord === "object" && !Array.isArray(statusRecord)
        ? statusRecord
        : {};
    const epicArcProgress = ensureEpicArcProgress(characterState);
    ...
    const existingStatus = epicArcProgress.missionStatusByArcID[arcKey] &&
...
function recordEpicArcCompletion(characterState, completionRecord = {}, options = {}) {
    ...
    const completedAtFileTime = normalizeText(completionRecord.completedAtFileTime, currentFileTimeString());
    const replayUntilFileTime = normalizeText(completionRecord.replayUntilFileTime, futureFileTimeString(EPIC_ARC_REPLAY_DELAY_MS));
...
function getEpicArcReplayBlock(characterState, epicArcID, options = {}) {
    ...
    const completion = normalizeEpicArcCompletionRecord(epicArcProgress.completedArcsByID[String(normalizedArcID)], normalizedArcID);
    ...
    try {
        ...
        nowFileTime = BigInt(normalizeText(options.nowFileTime, currentFileTimeString()));
    }
    catch (_error) {
        return null;
    }
...
//# sourceMappingURL=missionRuntimeState.js.map