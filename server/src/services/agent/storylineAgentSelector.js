...
const { listAgents, } = require(path.join(__dirname, "./agentAuthority"));
...
function getSolarSystemSecurity(system) {
    ...
    return toFiniteNumber(system.security ?? system.securityStatus, 0);
}
...
function buildAdjacency() {
    ...
    for (const systemID of adjacency.keys()) {
        ...
        adjacency.set(systemID, [...new Set(neighbors)].sort((left, right) => left - right));
    }
    return adjacency;
}
function buildStorylineSelectorIndex() {
    ...
    for (const agents of agentsBySystemID.values()) {
        agents.sort((left, right) => (left.agentTypeID - right.agentTypeID ||
...
function normalizeExcludedAgentIDs(excludedAgentIDs) {
    if (excludedAgentIDs instanceof Set) {
        return new Set([...excludedAgentIDs]
...
function findNearestStorylineAgent({ startSolarSystemID, factionID, missionLevel, requireHighSec = null, excludedAgentIDs = [], } = {}) {
    ...
    const shouldRequireHighSec = requireHighSec === null || requireHighSec === undefined
    ...
    while (queue.length > 0) {
        ...
        const candidates = (selectorIndex.agentsBySystemID.get(current.solarSystemID) || [])
            .filter((agent) => (agent.factionID === normalizedFactionID &&
...
//# sourceMappingURL=storylineAgentSelector.js.map