...
const PERMANENTLY_DISABLED_MISSION_IDS = Object.freeze(
  productionMissionPolicy.disabledMissions.map(({ missionID }: Record<string, any>) => missionID),
);
...
function normalizeLocalizedMessageMap(value) {
  ...
  for (const [messageKey, entry] of Object.entries<any>(value)) {
...
function normalizeMissionRecord(record: Record<string, any> = {}, missionID) {
  return {
    ...
    remoteCompletable:
      Object.prototype.hasOwnProperty.call(record, "remoteCompletable")
        ...
        : null,
    remoteOfferable:
      Object.prototype.hasOwnProperty.call(record, "remoteOfferable")
...
function sanitizePayload(payload: Record<string, any> = {}) {
  ...
  for (const [indexName, rawIndex] of Object.entries<any>(normalizeObject(payload.indexes))) {
    const index = normalizeObject(rawIndex);
    if (indexName === "preferredMissionIDs") {
      indexes[indexName] = Object.fromEntries(
        Object.entries<any>(index).filter(([, missionID]) => {
    ...
    indexes[indexName] = Object.fromEntries(
      Object.entries<any>(index).map(([key, missionIDs]) => [
        key,
...
function buildCache() {
  ...
  for (const [templateID, missionIDs] of Object.entries<any>(
  ...
  for (const [agentID, missionIDs] of Object.entries<any>(
  ...
  for (const [preferenceKey, missionID] of Object.entries<any>(
...
const GOLDEN_SECURITY_MISSION_BY_ID = new Map<any, any>(
...
const SUPPORTED_GOLDEN_SECURITY_DUNGEON_IDS = new Set<any>(
  GOLDEN_SECURITY_MISSIONS.map(({ dungeonID }: Record<string, any>) => dungeonID),
);
const SUPPORTED_CLIENT_DUNGEON_IDS = new Set<any>([
