...
const { buildDict, buildFiletimeLong, buildKeyVal, buildList, buildMarshalReal, currentFileTime, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { TABLE, readStaticRows, } = require(path.join(__dirname, "../_shared/referenceData"));
const { getAgentByID, listMissionTemplateIDsForAgent, } = require(path.join(__dirname, "./agentAuthority"));
const { getMissionByID, getMissionArcInfo, isMissionOfferAllowedForAgent, isOrdinarySecurityAgent, isSupportedLevelOneClientDungeonID, listMissionIDsByTemplate, listMissionIDsForAgent: listClientMissionIDsForAgent, pickMissionForAgent, } = require(path.join(__dirname, "./missionAuthority"));
const { getClientDungeonTemplate, getTemplateByID, } = require(path.join(__dirname, "../dungeon/dungeonAuthority"));
const dungeonRuntime = require(path.join(__dirname, "../dungeon/dungeonRuntime"));
const dungeonTrackingRuntime = require(path.join(__dirname, "../dungeon/dungeonTrackingRuntime"));
...
const { getCharacterRecord, syncInventoryItemForSession, } = require(path.join(__dirname, "../character/characterState"));
const { adjustCharacterBalance, } = require(path.join(__dirname, "../account/walletState"));
const { adjustCharacterWalletLPBalance, } = require(path.join(__dirname, "../corporation/lpWalletState"));
const { mutateCharacterState, getCharacterStateSnapshot, OFFER_EXPIRY_MS, REPLAY_DELAY_MS, currentFileTimeString, futureFileTimeString, normalizeEpicArcProgress, recordEpicArcCompletion, recordEpicArcMissionStatus, recordStorylineQualifyingCompletion, } = require(path.join(__dirname, "./missionRuntimeState"));
const { resolveItemByTypeID, } = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const { getActiveShipItem, grantItemsToCharacterStationHangar, listContainerItems, takeItemTypeFromCharacterLocation, ITEM_FLAGS, buildRemovedItemNotificationState, } = require(path.join(__dirname, "../inventory/itemStore"));
const { notifyInventoryChangesToCharacter, } = require(path.join(__dirname, "../raffles/raffleInventory"));
const { getDockedLocationID, } = require(path.join(__dirname, "../structure/structureLocation"));
const bookmarkRuntime = require(path.join(__dirname, "../bookmark/bookmarkRuntimeState"));
const { TYPE_SOLAR_SYSTEM, } = require(path.join(__dirname, "../bookmark/bookmarkConstants"));
...
const AGENT_DIALOGUE_ACTION_BUTTON_BY_TOKEN = Object.freeze({
    ...
    [AGENT_DIALOGUE_ACTION_TOKEN_REQUEST_AFTER_COMPLETION]: AGENT_DIALOGUE_BUTTON_REQUEST_MISSION,
});
...
const MISSION_BOOKMARK_FOLDER_DESCRIPTION = "System-managed mission bookmarks used by agent mission warp/location flows.";
...
const MISSION_PLACEHOLDER_NOTE = "Proceed to the assigned destination, complete the listed objectives, and return to your agent for debriefing.";
const MISSION_PLACEHOLDER_COMPLETE_NOTE = "Objective complete. Return to your agent for final debriefing and reward collection.";
...
function getRuntimeMissionTemplateRecord(missionRecord = null) {
    return getMissionTemplateRecord(normalizeText(missionRecord && missionRecord.missionTemplateID, ""));
}
function getDungeonMissionTemplateRecord(missionRecord = null) {
    return getMissionTemplateRecord(normalizeText(missionRecord && missionRecord.dungeonTemplateID, ""));
}
...
function extractMissionTemplateLevel(template = null) {
    ...
    const titleMatch = normalizeText(template && template.title, "").match(/\b(?:lvl|level)\s*(\d+)\b/i);
    return titleMatch ? normalizePositiveInteger(titleMatch[1], 0) : 0;
}
function buildMissionTemplateMatchCandidates(clientMissionRecord, runtimeDungeonTemplate) {
...
function scoreMissionTemplateCandidate(template, agentRecord, clientMissionRecord, runtimeDungeonTemplate, fallbackMissionTemplate = null) {
    if (!template || typeof template !== "object") {
        return Number.NEGATIVE_INFINITY;
    }
    const candidateKeys = buildMissionTemplateMatchCandidates(clientMissionRecord, runtimeDungeonTemplate);
    const templateKey = normalizeMissionTemplateMatchKey(template.title || template.resolvedName || template.templateID);
    ...
    const templateSourceDungeonID = normalizePositiveInteger(template && template.sourceDungeonID, 0);
    const runtimeSourceDungeonID = normalizePositiveInteger(runtimeDungeonTemplate &&
        (runtimeDungeonTemplate.sourceDungeonID || runtimeDungeonTemplate.dungeonID), 0);
...
function resolveMissionTemplateForClientMission(agentRecord, clientMissionRecord, fallbackMissionTemplate = null, runtimeDungeonTemplate = null) {
    const selectionCacheKey = [
        ...
        normalizePositiveInteger(clientMissionRecord &&
            clientMissionRecord.killMission &&
            clientMissionRecord.killMission.dungeonID, 0),
    ].join(":");
    ...
    if (explicitMissionTemplate) {
        cachedMissionTemplateSelections.set(selectionCacheKey, normalizeText(explicitMissionTemplate && explicitMissionTemplate.templateID, ""));
    ...
    const shouldPreferClientDungeonTemplate = killMissionDungeonID > 0 &&
        isSupportedLevelOneClientDungeonID(killMissionDungeonID) &&
        Boolean(runtimeDungeonTemplate);
    const isMiningMission = normalizeText(clientMissionRecord && clientMissionRecord.missionKind, "").toLowerCase() === "mining" &&
        normalizePositiveInteger(clientMissionRecord &&
            clientMissionRecord.killMission &&
            clientMissionRecord.killMission.objectiveTypeID, 0) > 0;
    ...
    if (shouldPreferClientDungeonTemplate) {
        cachedMissionTemplateSelections.set(selectionCacheKey, normalizeText(runtimeDungeonTemplate && runtimeDungeonTemplate.templateID, ""));
        return runtimeDungeonTemplate;
    }
    let bestTemplate = null;
    let bestScore = Number.NEGATIVE_INFINITY;
    for (const template of candidateTemplates) {
        const candidateScore = scoreMissionTemplateCandidate(template, agentRecord, clientMissionRecord, runtimeDungeonTemplate, fallbackMissionTemplate);
    ...
    if (bestTemplate && bestScore >= 150) {
        cachedMissionTemplateSelections.set(selectionCacheKey, normalizeText(bestTemplate && bestTemplate.templateID, ""));
        return bestTemplate;
    }
    if (runtimeDungeonTemplate &&
...
function getStationsByCorporationID() {
    ...
    for (const station of readStaticRows(TABLE.STATIONS)) {
        const corporationID = normalizePositiveInteger(station && (station.corporationID || station.ownerID), 0);
        if (!corporationID) {
            continue;
        }
    ...
    for (const stations of nextIndex.values()) {
        stations.sort((left, right) => normalizePositiveInteger(left && left.solarSystemID, 0) -
...
function isCourierMission(agentRecord = null, missionTemplate = null) {
    ...
    if (missionKind === MISSION_KIND_COURIER ||
        missionKind === MISSION_KIND_DISTRIBUTION) {
        return true;
    }
    const typeLabel = normalizeText(agentRecord && agentRecord.missionTypeLabel, "").toLowerCase();
    if (typeLabel.includes("courier") || typeLabel.includes("distribution")) {
        return true;
    }
    const missionTitle = normalizeText(missionTemplate && missionTemplate.title, "").toLowerCase();
...
function resolveAgentSystemSecurity(agentRecord = null) {
    const solarSystemID = normalizePositiveInteger(agentRecord && agentRecord.solarSystemID, 0);
    if (!solarSystemID) {
        return null;
    }
...
function resolveLoyaltyPointSecurityMultiplier(agentRecord = null) {
    ...
    return Math.max(LP_SECURITY_MULTIPLIER_FLOOR, LP_SECURITY_CONSTANT - Math.max(0, security));
}
...
function isLoyaltyPointExemptMission(clientMissionRecord = null) {
    ...
    return (clientMissionRecord.isStoryline === true ||
...
function isResearchAgent(agentRecord = null) {
    const missionKind = normalizeText(agentRecord && agentRecord.missionKind, "");
    const missionTypeLabel = normalizeText(agentRecord && agentRecord.missionTypeLabel, "").toLowerCase();
...
function canUseAgent(characterID, agentRecord) {
    ...
    return Boolean(normalizedCharacterID &&
        standingRuntime.canCharacterUseAgent(normalizedCharacterID, agentRecord));
}
...
function buildStorylineMissionTypeLabel(agentRecord = null, clientMissionRecord = null) {
    const contentTemplate = normalizeText(clientMissionRecord && clientMissionRecord.contentTemplate, "");
    const missionKind = normalizeText(clientMissionRecord && clientMissionRecord.missionKind, "").toLowerCase();
    ...
    if (/storylineagentinteraction/i.test(contentTemplate) ||
    ...
    const fallbackMissionKind = normalizeText(agentRecord && agentRecord.missionKind, "").toLowerCase();
...
function normalizeMissionRewardItem(rewardRecord = null) {
    const typeID = normalizePositiveInteger(rewardRecord && (rewardRecord.rewardTypeID || rewardRecord.typeID), 0);
    const quantity = Math.max(0, normalizeInteger(rewardRecord && (rewardRecord.rewardQuantity || rewardRecord.quantity), 0));
...
function buildMissionRewards(agentRecord, importantMission = false, clientMissionRecord = null, characterID = null, presentationTemplate = null, contentID = null) {
    const rewardScale = REWARD_BY_SCALE_AND_LEVEL[resolveRewardScale(agentRecord)];
    const base = rewardScale[normalizeInteger(agentRecord && agentRecord.level, 1)] || rewardScale[1];
    const importantMultiplier = importantMission ? 1.3 : 1.0;
    const missionRewards = clientMissionRecord &&
    ...
    const capturedRewardPresentation = missionRewards
        ? null
        : resolveCapturedMissionRewardPresentation(presentationTemplate, contentID === null || contentID === undefined
            ? clientMissionRecord && clientMissionRecord.missionID
            : contentID, agentRecord);
    const capturedPrimaryReward = normalizeMissionRewardItem(capturedRewardPresentation && capturedRewardPresentation.normalReward);
    const capturedBonusReward = normalizeMissionRewardItem(capturedRewardPresentation && capturedRewardPresentation.bonusReward);
    const itemRewards = [];
    ...
    const fixedLpReward = Math.max(normalizeInteger(clientMissionRecord && clientMissionRecord.fixedLpRewardOmega, 0), normalizeInteger(clientMissionRecord && clientMissionRecord.fixedLpRewardAlpha, 0));
    ...
    let bonusTimeIntervalMinutes = normalizePositiveInteger(missionRewards && missionRewards.bonusTimeInterval, Math.max(0, normalizeInteger(base.bonusTimeIntervalMinutes, 0)));
    const loyaltyPointSecurityMultiplier = resolveLoyaltyPointSecurityMultiplier(agentRecord);
    const divisionConnectionsMultiplier = normalizePositiveInteger(characterID, 0) > 0
        ? standingRuntime.getCharacterDivisionConnectionsLpMultiplier(characterID, agentRecord && agentRecord.missionKind)
        : 1;
    ...
    let loyaltyPoints = Math.max(0, fixedLpReward ||
        (isLoyaltyPointExemptMission(clientMissionRecord)
            ? 0
            : Math.round(base.baseLoyaltyPoints *
    ...
    if (capturedBonusReward) {
        ...
        bonusTimeIntervalMinutes = Math.max(0, normalizeInteger(capturedRewardPresentation &&
            capturedRewardPresentation.bonusReward &&
            capturedRewardPresentation.bonusReward.timeIntervalMinutes, 0));
    }
    if (capturedRewardPresentation &&
        Object.prototype.hasOwnProperty.call(capturedRewardPresentation, "loyaltyPoints")) {
        loyaltyPoints = Math.max(0, normalizeInteger(capturedRewardPresentation.loyaltyPoints, 0));
    }
    return {
        isk,
        bonusIsk,
...
function resolveDropoffStation(agentRecord) {
    const candidateStations = getStationsByCorporationID().get(normalizePositiveInteger(agentRecord && agentRecord.corporationID, 0)) || [];
    const sourceStationID = normalizePositiveInteger(agentRecord && agentRecord.stationID, 0);
    const alternativeStation = candidateStations.find((station) => normalizePositiveInteger(station && station.stationID, 0) !== sourceStationID);
...
function getClientMissionRecord(input) {
    const missionRecord = input && typeof input === "object"
        ? input
        : { contentID: normalizeMissionContentID(input, null) };
    const directMissionID = normalizeMissionContentID(missionRecord && missionRecord.contentID, null);
    const directMissionRecord = directMissionID === null ? null : getMissionByID(directMissionID);
    if (directMissionRecord) {
        return directMissionRecord;
    }
    const agentRecord = getAgentRecord(missionRecord && missionRecord.agentID);
    const missionContentTemplateID = normalizeText(missionRecord && missionRecord.missionContentTemplateID, "");
    const missionTemplateID = normalizeText(missionRecord && missionRecord.missionTemplateID, "");
    const desiredMissionKind = normalizeText(missionRecord && missionRecord.missionKind, "").toLowerCase();
    const objectiveMode = normalizeText(missionRecord && missionRecord.objectiveMode, "").toLowerCase();
    const desiredNameID = normalizePositiveInteger(missionRecord && missionRecord.missionNameID, 0);
    ...
    const pushCandidateMissionID = (missionID) => {
        ...
        const missionKey = normalizedMissionID === null ? "" : String(normalizedMissionID);
    ...
    const missionKindMatches = (clientMissionRecord) => {
        const clientMissionKind = normalizeText(clientMissionRecord && clientMissionRecord.missionKind, "").toLowerCase();
        ...
        return (normalizedKinds.has("courier") &&
    ...
    const objectiveModeMatches = (clientMissionRecord) => {
        ...
        if (objectiveMode === OBJECTIVE_TYPE_TRANSPORT) {
            return Boolean(clientMissionRecord &&
        ...
        if (objectiveMode === OBJECTIVE_TYPE_FETCH) {
            return normalizeText(clientMissionRecord && clientMissionRecord.missionKind, "").toLowerCase() === "trade";
        }
        if (objectiveMode === OBJECTIVE_TYPE_AGENT) {
            return (normalizeBoolean(clientMissionRecord && clientMissionRecord.isTalkToAgent, false) ||
    ...
    const scoredCandidates = candidateMissionRecords
        ...
        return normalizePositiveInteger(left.clientMissionRecord && left.clientMissionRecord.missionID, 0) - normalizePositiveInteger(right.clientMissionRecord && right.clientMissionRecord.missionID, 0);
    });
...
function resolveEpicArcMissionInfo(missionRecord) {
    ...
    return {
        ...
        nameID: normalizePositiveInteger(missionRecord && missionRecord.missionNameID, normalizePositiveInteger(clientMissionRecord.nameID, 0)) || null,
        clientMissionRecord,
    };
}
function buildEpicArcMissionStatusRecord(missionRecord, overrides = {}) {
    ...
    return {
        ...
        acceptedDate: normalizeText(overrides.acceptedDate ?? missionRecord.acceptedAtFileTime, ""),
...
function setEpicArcStatus(statusByArcID, statusRecord) {
    if (!statusRecord) {
        return;
    }
...
function buildEpicArcStatusPayloadFromCharacterState(characterState) {
    ...
    return buildDict(Object.entries(statusByArcID)
        .map(([arcKey, missionMap]) => [
        normalizePositiveInteger(arcKey, 0),
        buildDict(Object.entries(missionMap || {})
...
function resolveClientMissionContentID(missionRecord, agentRecord = null) {
    ...
    const clientMissionRecord = getClientMissionRecord({
        ...
        agentID: normalizePositiveInteger(missionRecord && missionRecord.agentID, 0) ||
            normalizePositiveInteger(agentRecord && agentRecord.agentID, 0),
    });
    return normalizeMissionContentID(clientMissionRecord && clientMissionRecord.missionID, directMissionID);
}
...
function isDungeonFetchMissionRecord(missionRecord) {
    return (normalizeText(missionRecord && missionRecord.objectiveMode, "") === OBJECTIVE_TYPE_DUNGEON &&
...
function getMissionConversationTargetAgentID(missionRecord, fallbackAgentID = 0) {
    const clientMissionRecord = getClientMissionRecord(missionRecord);
    const fallbackSourceAgentID = normalizePositiveInteger(clientMissionRecord && clientMissionRecord.sourceAgentID, normalizePositiveInteger(fallbackAgentID, 0));
    const targetAgentID = normalizePositiveInteger(clientMissionRecord && clientMissionRecord.targetAgentID, 0);
    if (targetAgentID > 0) {
...
function buildMissionConversationTargetLocationWrap(missionRecord, fallbackAgentRecord = null) {
    const targetAgentRecord = getMissionConversationTargetRecord(missionRecord, normalizePositiveInteger(fallbackAgentRecord && fallbackAgentRecord.agentID, 0));
    if (targetAgentRecord) {
        ...
        const liveEntity = getSceneEntityByID(scene, targetAgentID) ||
            listSceneEntities(scene).find((entity) => (normalizePositiveInteger(entity && (entity.agentID || entity.ownerID), 0) ===
                targetAgentID)) ||
            null;
        ...
        const position = positionSources.find((candidate) => (candidate &&
            ["x", "y", "z"].every((axis) => (candidate[axis] !== null &&
...
function resolveMissionBonusTimeRemainingTicks(missionRecord) {
    const rewards = missionRecord && missionRecord.rewards && typeof missionRecord.rewards === "object"
...
function buildClientMissionObjectiveContext(clientMissionRecord) {
    const context = {};
    const clientObjectives = normalizeObject(clientMissionRecord && clientMissionRecord.clientObjectives);
    for (const parameter of normalizeArray(clientObjectives.contextParameters)) {
        const parameterKey = normalizeText(parameter && (parameter.key || parameter.parameterKey), "");
        if (!parameterKey ||
            !parameter ||
...
function resolveClientMissionDungeonID(clientMissionRecord) {
    ...
    return (normalizePositiveInteger(objectiveContext.dungeon_id, 0) ||
        normalizePositiveInteger(clientMissionRecord &&
            clientMissionRecord.killMission &&
            clientMissionRecord.killMission.dungeonID, 0));
}
function resolveMissionExpirationDurationMs(clientMissionRecord) {
    if (clientMissionRecord &&
        ...
        const authoredExpirationMinutes = Math.max(0, normalizeInteger(clientMissionRecord.expirationTime, 0));
...
function buildMissionItemSpec(value, fallbackTypeID = 0, fallbackQuantity = 1) {
    ...
    for (const candidate of candidates) {
        ...
        const typeID = normalizePositiveInteger(source.typeID || source.type_id || source.objectiveTypeID, 0);
        const groupID = normalizePositiveInteger(source.groupID || source.group_id, 0);
        const quantity = Math.max(1, normalizeInteger(source.quantity || source.objectiveQuantity, fallbackQuantity));
...
function resolvePrimaryMissionItemSpec(missionRecord) {
    ...
    const clientMissionRecord = getClientMissionRecord(missionRecord);
    const courierMission = normalizeObject(clientMissionRecord && clientMissionRecord.courierMission);
    const killMission = normalizeObject(clientMissionRecord && clientMissionRecord.killMission);
...
function getMissionMessageID(clientMissionRecord, messageKey) {
    return normalizePositiveInteger(clientMissionRecord &&
        clientMissionRecord.messages &&
        clientMissionRecord.messages[messageKey], 0);
}
function getMissionLocalizedMessageEntry(clientMissionRecord, messageKey) {
    const entry = clientMissionRecord &&
...
function getMissionLocalizedMessageText(clientMissionRecord, messageKey) {
    ...
    return normalizeText(entry && entry.text, "");
}
...
function getMissionLocalizedNameText(clientMissionRecord) {
    return normalizeText(clientMissionRecord &&
...
function buildMissionMessageReferenceByKeys(missionRecord, clientMissionRecord, messageKeys) {
    return buildMissionMessageReference(missionRecord, getFirstMissionMessageID(clientMissionRecord, messageKeys));
}
function buildMissionProcessMessageReferenceByKeys(missionRecord, clientMissionRecord, messageKeys) {
    return buildMissionProcessMessageReference(missionRecord, getFirstMissionMessageID(clientMissionRecord, messageKeys));
}
function buildConversationAgentSays(missionRecord, clientMissionRecord, messageKeys, fallbackText) {
    ...
    const localizedText = getFirstMissionLocalizedMessageText(clientMissionRecord, keys);
    if (localizedText) {
        return [
            localizedText,
            resolveClientMissionContentID(missionRecord),
...
function buildMissionDungeonSiteKey(characterID, missionRecord) {
    return `mission:${normalizePositiveInteger(characterID, 0)}:${normalizePositiveInteger(missionRecord && missionRecord.missionSequence, 0)}`;
}
function resolveMissionAnchorCelestial(solarSystemID, seed) {
    const celestials = worldData.getCelestialsForSystem(solarSystemID);
    const candidates = celestials.filter((celestial) => (celestial &&
        celestial.position &&
...
function buildMissionSitePosition(solarSystemID, missionRecord, agentRecord) {
    ...
    const distance = MISSION_SITE_DISTANCE_METERS +
        ((hashText(`${seed}:distance`) % (MISSION_SITE_DISTANCE_JITTER_METERS * 2 + 1)) -
            MISSION_SITE_DISTANCE_JITTER_METERS);
    const vertical = ((hashText(`${seed}:vertical`) % (MISSION_SITE_VERTICAL_JITTER_METERS * 2 + 1)) -
...
function buildMissionSiteLocationWrap(agentRecord, missionRecord, missionTemplate = null) {
    const solarSystemID = normalizePositiveInteger(missionRecord && missionRecord.missionSystemID, 0) ||
...
function buildMissionBookmarkPayload(bookmarkRecord) {
    ...
    const metadata = bookmarkRecord.metadata && typeof bookmarkRecord.metadata === "object"
        ? bookmarkRecord.metadata
        : {};
    const solarSystemID = normalizePositiveInteger(metadata.solarsystemID, 0) ||
        normalizePositiveInteger(bookmarkRecord.locationID, 0);
    const role = normalizeText(metadata.role, "").toLowerCase();
    const hasSpatialCoordinates = ["x", "y", "z"].every((axis) => (bookmarkRecord[axis] !== null &&
    ...
    fields.push(["created", buildFiletimeLong(fileTimeStringToBigInt(bookmarkRecord.created))], ["flag", null], ["locationID", normalizePositiveInteger(bookmarkRecord.locationID, 0)], ["ownerID", normalizePositiveInteger(bookmarkRecord.creatorID, 0) || null], ["y", hasSpatialCoordinates ? buildMarshalReal(bookmarkRecord.y, 0) : 0], ["x", hasSpatialCoordinates ? buildMarshalReal(bookmarkRecord.x, 0) : 0], ["solarsystemID", solarSystemID], ["z", hasSpatialCoordinates ? buildMarshalReal(bookmarkRecord.z, 0) : 0]);
    return buildKeyVal(fields);
}
function listMissionBookmarks(characterID, missionRecord) {
    const bookmarkIDsByRole = missionRecord && missionRecord.bookmarkIDsByRole && typeof missionRecord.bookmarkIDsByRole === "object"
...
function ensureMissionBookmarkFolder(characterID) {
    const existingView = bookmarkRuntime.listFolderViews(characterID).find((view) => (view &&
        view.folder &&
    ...
    if (existingView && existingView.folder) {
        if (existingView.isActive === true) {
            try {
                bookmarkRuntime.updateKnownFolderState(characterID, existingView.folder.folderID, false);
            }
            catch (_error) {
    ...
    try {
        ...
        if (createdView && createdView.folder) {
            try {
                bookmarkRuntime.updateKnownFolderState(characterID, createdView.folder.folderID, false);
            }
            catch (_error) {
...
function cleanupVisibleLegacyMissionBookmarks(characterID, agentRecord, missionRecord) {
    ...
    const expectedMissionSiteID = normalizePositiveInteger(missionRecord && missionRecord.missionSiteID, 0);
    const expectedMissionInstanceID = normalizePositiveInteger(missionRecord && missionRecord.dungeonInstanceID, 0);
    const expectedMissionTitle = normalizeText(missionRecord && missionRecord.missionTitle, "").toLowerCase();
    const missionBookmarkFolderID = normalizePositiveInteger(ensureMissionBookmarkFolder(characterID).folderID, 0);
    const staleMissionBookmarks = visibleBookmarks.filter((bookmark) => {
        const metadata = bookmark && bookmark.metadata && typeof bookmark.metadata === "object"
            ? bookmark.metadata
            : {};
        const locationType = normalizeText(metadata.locationType, "");
        if (!["agenthomebase", "objective", "objective.source", "objective.destination", "dungeon"].includes(locationType)) {
            return false;
        }
        ...
        const bookmarkReferringAgentID = normalizePositiveInteger(metadata.referringAgentID, 0);
        ...
        const bookmarkTitle = normalizeText(bookmark && (bookmark.memo || metadata.hint), "").toLowerCase();
        return ((expectedAgentID > 0 &&
...
function resolveMissionDungeonBookmarkPresentation(missionRecord, missionTemplate = null) {
    ...
    const populationHints = normalizeObject(presentationTemplate && presentationTemplate.populationHints);
    const missionPresentation = normalizeObject((presentationTemplate && presentationTemplate.missionPresentation) ||
        populationHints.missionPresentation);
    const explicitDeadspaceValues = normalizeArray(populationHints.encounters)
        .filter((encounter) => (encounter && Object.prototype.hasOwnProperty.call(encounter, "deadspace")))
        .map((encounter) => encounter.deadspace === true);
    const hasAccelerationGate = normalizeArray(presentationTemplate &&
        presentationTemplate.siteSceneProfile &&
        presentationTemplate.siteSceneProfile.gateProfiles).length > 0;
    ...
    const category = normalizeText(missionPresentation.bookmarkCategory, isMiningMissionRecord(missionRecord) ? "Mining" : "Encounter");
    return {
        deadspace,
        hint: normalizeText(missionPresentation.bookmarkHint, `UI/Agents/Bookmarks/${category}/${deadspace ? "Deadspace" : category}`),
    };
}
function ensureMissionBookmarks(characterID, agentRecord, missionRecord, missionTemplate = null) {
    ...
    const addBookmark = (role, data) => {
        ...
        const bookmarkID = normalizePositiveInteger(result && result.bookmark && result.bookmark.bookmarkID, 0);
        if (bookmarkID > 0) {
    ...
    const addLocationBookmark = (role, location, options = {}) => {
        ...
        const solarSystemID = normalizePositiveInteger(location && location.solarsystemID, normalizePositiveInteger(station && station.solarSystemID, 0));
        const hasCoordinates = options.includeCoordinates !== false &&
            ["x", "y", "z"].every((axis) => (location[axis] !== null &&
                location[axis] !== undefined &&
                Number.isFinite(Number(location && location[axis]))));
        addBookmark(role, {
            itemID: normalizePositiveInteger(options.itemID, 0) ||
                normalizePositiveInteger(station && station.stationID, 0) ||
                locationID,
            typeID: normalizePositiveInteger(station && station.stationTypeID, 0) ||
    ...
    if (missionRecord.objectiveMode === OBJECTIVE_TYPE_TRANSPORT) {
    ...
    else if (isFetchMissionRecord(missionRecord) ||
        ...
        addLocationBookmark("objective", missionRecord.dropoffLocation, {
            ...
            itemID: targetAgentID > 0 &&
                !worldData.getStationByID(missionRecord &&
    ...
    if (missionRecord.objectiveMode === OBJECTIVE_TYPE_DUNGEON &&
        missionRecord.missionPosition) {
        const dungeonPresentation = resolveMissionDungeonBookmarkPresentation(missionRecord, missionTemplate);
        addBookmark("dungeon", {
...
function cleanupMissionBookmarks(characterID, missionRecord) {
    const bookmarkIDs = Object.values(missionRecord && missionRecord.bookmarkIDsByRole && typeof missionRecord.bookmarkIDsByRole === "object"
        ? missionRecord.bookmarkIDsByRole
        : {})
...
function ensureMissionSiteState(characterID, agentRecord, missionRecord, missionTemplate = null) {
    ...
    if (nextRecord.objectiveMode === OBJECTIVE_TYPE_TRANSPORT) {
        if (nextRecord.cargo &&
            ...
            const grantResult = grantItemsToCharacterStationHangar(characterID, nextRecord.pickupLocation.locationID, [{
                    itemType: nextRecord.cargo.typeID,
                    quantity: nextRecord.cargo.quantity,
                }]);
            if (grantResult && grantResult.success === true) {
                nextRecord.cargo.granted = true;
                notifyMissionInventoryChanges(characterID, (grantResult.data && grantResult.data.changes) || []);
            }
        }
        nextRecord.bookmarkIDsByRole = ensureMissionBookmarks(characterID, agentRecord, nextRecord, missionTemplate);
        return nextRecord;
    }
    ...
    const missionSystemID = normalizePositiveInteger(nextRecord.missionSystemID, 0) ||
        normalizePositiveInteger(agentRecord && agentRecord.solarSystemID, 0);
    const missionPosition = nextRecord.missionPosition && typeof nextRecord.missionPosition === "object"
        ? clonePosition(nextRecord.missionPosition)
        : buildMissionSitePosition(missionSystemID, nextRecord, agentRecord);
    const missionSiteID = normalizePositiveInteger(nextRecord.missionSiteID, 0) ||
    ...
    const runtimeInstanceTemplateID = normalizeText(runtimeInstanceTemplate && runtimeInstanceTemplate.templateID, normalizeText(nextRecord.dungeonTemplateID, ""));
    if (dungeonInstanceID) {
        const existingInstance = dungeonRuntime.getInstance(dungeonInstanceID);
        if (!existingInstance) {
            dungeonInstanceID = null;
        }
        else if (runtimeInstanceTemplateID &&
    ...
    if (!dungeonInstanceID && runtimeInstanceTemplateID) {
        const existingInstance = dungeonRuntime.findInstanceBySiteKey(buildMissionDungeonSiteKey(characterID, nextRecord), {
            activeOnly: true,
            full: true,
        });
        const existingInstanceTemplateID = normalizeText(existingInstance && existingInstance.templateID, "");
        if (existingInstance &&
            runtimeInstanceTemplateID &&
            existingInstanceTemplateID &&
            existingInstanceTemplateID !== runtimeInstanceTemplateID) {
            try {
                dungeonRuntime.purgeInstance(normalizePositiveInteger(existingInstance && existingInstance.instanceID, 0));
            }
            catch (_error) {
...
function sumItemStackQuantity(items = []) {
    return (Array.isArray(items) ? items : []).reduce((sum, item) => (sum + (normalizePositiveInteger(item && item.singleton, 0) === 1
        ? 1
        : Math.max(0, normalizeInteger(item && (item.stacksize || item.quantity), 0)))), 0);
}
function getCourierProgress(characterID, missionRecord) {
    ...
    const pickupLocationID = normalizePositiveInteger(missionRecord && missionRecord.pickupLocation && missionRecord.pickupLocation.locationID, 0);
    const dropoffLocationID = normalizePositiveInteger(missionRecord && missionRecord.dropoffLocation && missionRecord.dropoffLocation.locationID, 0);
    const activeShip = getActiveShipItem(characterID);
    return {
        sourceHangarQuantity: pickupLocationID > 0
            ? sumItemStackQuantity(listCharacterCargoStacks(characterID, pickupLocationID, ITEM_FLAGS.HANGAR, cargo.typeID))
            : 0,
        shipCargoQuantity: activeShip && normalizePositiveInteger(activeShip.itemID, 0) > 0
            ? sumItemStackQuantity(listCharacterCargoStacks(characterID, activeShip.itemID, ITEM_FLAGS.CARGO_HOLD, cargo.typeID))
            : 0,
        destinationHangarQuantity: dropoffLocationID > 0
            ? sumItemStackQuantity(listCharacterCargoStacks(characterID, dropoffLocationID, ITEM_FLAGS.HANGAR, cargo.typeID))
            : 0,
    };
}
function notifyMissionInventoryChanges(characterID, changes = []) {
    ...
    const dockedLocationID = normalizePositiveInteger(getDockedLocationID(liveSession), 0);
    for (const change of Array.isArray(changes) ? changes : []) {
        const previousState = normalizeObject(change && (change.previousData || change.previousState));
        const notificationItem = change && change.item
            ? change.item
            : (change &&
                change.removed === true &&
        ...
        const touchesDockedHangar = dockedLocationID > 0 &&
        ...
        syncInventoryItemForSession(liveSession, notificationItem, previousState, {
...
function evaluateMissionProgress(characterID, missionRecord) {
    ...
    if (nextRecord.objectiveMode === OBJECTIVE_TYPE_TRANSPORT) {
        ...
        const currentDockedLocationID = normalizePositiveInteger(getDockedLocationID(getPreferredCharacterSession(characterID)), 0);
        const dropoffLocationID = normalizePositiveInteger(nextRecord &&
            nextRecord.dropoffLocation &&
            nextRecord.dropoffLocation.locationID, 0);
        const dockedAtDropoff = currentDockedLocationID > 0 &&
            currentDockedLocationID === dropoffLocationID;
        const hasCargo = progress.shipCargoQuantity >= requiredQuantity ||
    ...
    if (isFetchMissionRecord(nextRecord)) {
        ...
        const progress = buildMissionItemProgressSnapshot(characterID, nextRecord, itemSpec, {
            destinationLocationID: normalizePositiveInteger(nextRecord &&
                nextRecord.dropoffLocation &&
                nextRecord.dropoffLocation.locationID, 0),
            currentStationID: normalizePositiveInteger(nextRecord &&
                nextRecord.dropoffLocation &&
                nextRecord.dropoffLocation.locationID, 0),
        });
        ...
        nextRecord.cargo = {
            ...itemSpec,
            hasCargo: progress.shipCargoQuantity >= requiredQuantity ||
                progress.destinationHangarQuantity >= requiredQuantity,
            granted: false,
        };
        nextRecord.objectiveCompleted =
...
function syncMissionRecordState(characterID, agentID, options = {}) {
    ...
    const completionChanged = normalizeBoolean(missionRecord && missionRecord.objectiveCompleted, false) !==
...
function syncMissionRecordForDungeonInstance(instanceOrID, options = {}) {
    ...
    const agentID = normalizePositiveInteger(options.agentID, 0) ||
...
function buildMissionTrackerInfoTuple(characterID, missionRecord, options = {}) {
    const currentTuple = resolveMissionObjectiveState(characterID, missionRecord, options).current;
    const items = getTupleItems(currentTuple);
    if (items.length <= 0) {
        return null;
    }
    return buildMarshalTuple(items.map((item, index) => (index === 0 || item === null || item === undefined ? item : String(item))));
}
...
function buildMissionItemProgressSnapshot(characterID, missionRecord, itemSpec, options = {}) {
    ...
    const activeShip = getActiveShipItem(characterID);
    const countHangarStacks = (locationID) => (locationID > 0
        ? sumItemStackQuantity(listCharacterCargoStacks(characterID, locationID, ITEM_FLAGS.HANGAR, typeID))
        : 0);
    ...
    const shipCargoQuantity = activeShip && normalizePositiveInteger(activeShip.itemID, 0) > 0
        ? sumItemStackQuantity(listCharacterCargoStacks(characterID, activeShip.itemID, ITEM_FLAGS.CARGO_HOLD, typeID))
        : 0;
    const uniqueHangarLocations = [...new Set([sourceLocationID, destinationLocationID, currentStationID].filter(Boolean))];
    const totalQuantity = shipCargoQuantity + uniqueHangarLocations.reduce((sum, locationID) => sum + countHangarStacks(locationID), 0);
    return {
...
function resolveMissionTrackerDungeonContext(characterID, missionRecord, options = {}) {
    ...
    const spaceRuntime = getSpaceRuntime();
    const scene = session && spaceRuntime && typeof spaceRuntime.getSceneForSession === "function"
        ? spaceRuntime.getSceneForSession(session)
        : null;
    const tracked = scene && session
        ? dungeonTrackingRuntime.resolveTrackedDungeonForSession(scene, session)
        : null;
    const missionInstanceID = normalizePositiveInteger(missionRecord && missionRecord.dungeonInstanceID, 0);
    return {
        session,
        scene,
        tracked: tracked &&
...
function getSceneEntityByID(scene, entityID) {
    ...
    return listSceneStaticEntities(scene).find((entity) => (normalizePositiveInteger(entity && entity.itemID, 0) === normalizedEntityID)) || null;
}
function listSceneEntities(scene) {
    const entities = [...listSceneStaticEntities(scene)];
    const seenIDs = new Set(entities
...
function listActiveMissionEncounterEntityIDs(instance, tracked = null) {
    ...
    const currentRoomKey = normalizeText((tracked && tracked.roomKey) || objectiveMetadata.currentRoomKey, "");
    const currentWave = Math.max(0, normalizeInteger(objectiveMetadata.currentWave, normalizeInteger(objectiveCounters.current_wave, 0)));
    const encounterStates = Object.values(normalizeObject(instance &&
        instance.spawnState &&
        ...
        .filter(({ encounterState }) => (normalizeArray(encounterState.remainingEntityIDs).some((entityID) => normalizePositiveInteger(entityID, 0) > 0)))
        .sort((left, right) => {
        ...
        return (rightRoomMatch - leftRoomMatch ||
...
function findNextMissionGateEntityID(characterID, missionRecord, options = {}) {
    ...
    const { scene, tracked } = resolveMissionTrackerDungeonContext(characterID, missionRecord, options);
    ...
    const candidateGateKeys = Object.entries(gateStatesByKey)
        .filter(([, gateState]) => {
        if (Math.max(0, normalizeInteger(gateState && gateState.usesCount, 0)) > 0 ||
            Math.max(0, normalizeInteger(gateState && gateState.lastUsedAtMs, 0)) > 0) {
            return false;
        }
        const sourceRoomKey = normalizeText(gateState &&
            gateState.metadata &&
...
function buildEncounterDestroyAllObjectiveTuple(characterID, missionRecord, options = {}) {
    ...
    const trackerHints = normalizeObject(populationHints.missionTracker || populationHints.tracker);
    const anchor = normalizeObject(trackerHints.destroyAllAnchor ||
    ...
    const anchorNameID = normalizePositiveInteger(anchor.nameID || anchor.dunObjectNameID, 0);
    const instanceID = normalizePositiveInteger(missionRecord && missionRecord.dungeonInstanceID, 0);
    const instance = instanceID > 0 ? dungeonRuntime.getInstance(instanceID) : null;
    const { scene, tracked } = resolveMissionTrackerDungeonContext(characterID, missionRecord, options);
    const matchesAnchor = (entity) => (entity &&
        ...
        (anchorNameID <= 0 ||
            normalizePositiveInteger(entity && (entity.nameID || entity.dunObjectNameID), 0) === anchorNameID));
    ...
    if (!anchorEntity &&
        ...
        anchorEntity = listSceneEntities(scene).find((entity) => (matchesAnchor(entity) &&
            (instanceID <= 0 ||
...
function buildDungeonFetchReturnTuple(missionRecord, itemSpec) {
    return buildList([
        ...
        normalizePositiveInteger(missionRecord &&
            missionRecord.dropoffLocation &&
            missionRecord.dropoffLocation.locationID, 0),
...
function buildDungeonFetchObjectiveState(characterID, missionRecord, options = {}) {
    ...
    const progress = buildMissionItemProgressSnapshot(characterID, missionRecord, itemSpec, {
        destinationLocationID: normalizePositiveInteger(missionRecord &&
            missionRecord.dropoffLocation &&
            missionRecord.dropoffLocation.locationID, 0),
    ...
    else if (isMissionDungeonObjectiveSatisfied(missionRecord) &&
        progress.totalQuantity >= requiredQuantity) {
        current = returnTuple;
    }
    else if (options.inActiveDungeon === true) {
        const gateEntityID = findNextMissionGateEntityID(characterID, missionRecord, options);
...
function buildAgentObjectiveState(missionRecord) {
    const targetAgentID = getMissionConversationTargetAgentID(missionRecord, normalizePositiveInteger(missionRecord && missionRecord.agentID, 0));
    ...
    const completed = normalizeBoolean(missionRecord && missionRecord.objectiveCompleted, false);
    return {
...
function buildCourierObjectiveState(characterID, missionRecord) {
    ...
    const requiredQuantity = Math.max(1, normalizeInteger(missionRecord && missionRecord.cargo && missionRecord.cargo.quantity, 1));
...
function buildMissionDeliveryTuple(missionRecord, itemSpec, itemProgress, options = {}) {
    ...
    const requiredQuantity = Math.max(1, normalizeInteger(itemSpec && itemSpec.quantity, 1));
    const destinationLocationID = normalizePositiveInteger(options.destinationLocationID, 0);
    const atDestination = destinationLocationID > 0 &&
...
function buildTransportObjectiveTuples(missionRecord) {
    ...
    const destinationLocationID = normalizePositiveInteger(missionRecord && missionRecord.dropoffLocation && missionRecord.dropoffLocation.locationID, 0);
    return {
...
function normalizeObjectiveTargetDescriptor(target) {
    ...
    const nameID = normalizePositiveInteger(target.nameID || target.name_id, normalizePositiveInteger(target.dunObjectNameID || target.dun_object_name_id, 0));
    if (!typeID) {
        return null;
    }
    return {
...
function listSpawnedMissionObjectiveTargetIDs(instance) {
    const encounterStatesByKey = normalizeObject(instance &&
        instance.spawnState &&
        instance.spawnState.encounterStatesByKey);
    const entityIDs = [];
    for (const encounterState of Object.values(encounterStatesByKey)) {
        for (const entityID of normalizeArray(encounterState && encounterState.objectiveBlockingEntityIDs)) {
            const normalizedEntityID = normalizePositiveInteger(entityID, 0);
            if (normalizedEntityID > 0 && !entityIDs.includes(normalizedEntityID)) {
                entityIDs.push(normalizedEntityID);
            }
...
function buildEncounterDestroyObjectiveTuple(characterID, missionRecord, options = {}) {
    ...
    const templateID = normalizeText(instance && instance.templateID, normalizeText(missionRecord && missionRecord.dungeonTemplateID, ""));
    const template = templateID ? getTemplateByID(templateID) : null;
    const completion = normalizeObject(template &&
    ...
    const authoredTargets = normalizeArray(options.objectiveTargets)
        ...
        .filter(Boolean);
    const targets = (authoredTargets.length > 0
        ? authoredTargets
    ...
    const { scene } = resolveMissionTrackerDungeonContext(characterID, missionRecord, options);
    ...
    for (const targetEntityID of targetEntityIDs) {
        ...
        const entityNameID = normalizePositiveInteger(entity.nameID || entity.dunObjectNameID, 0);
        const target = targets.find((candidate) => ((!candidate.itemID || candidate.itemID === targetEntityID) &&
...
function resolveMissionObjectiveState(characterID, missionRecord, options = {}) {
    ...
    if (isDungeonFetchMissionRecord(currentRecord)) {
        return buildDungeonFetchObjectiveState(characterID, currentRecord, options);
    }
    if (isMiningMissionRecord(currentRecord)) {
        return buildMiningObjectiveState(characterID, currentRecord, options);
    }
    ...
    if (currentRecord.objectiveMode === OBJECTIVE_TYPE_TRANSPORT) {
        return buildCourierObjectiveState(characterID, currentRecord);
    }
    if (isFetchMissionRecord(currentRecord)) {
        ...
        const progress = buildMissionItemProgressSnapshot(characterID, currentRecord, itemSpec, {
            destinationLocationID: normalizePositiveInteger(currentRecord &&
                currentRecord.dropoffLocation &&
                currentRecord.dropoffLocation.locationID, 0),
            currentStationID: normalizePositiveInteger(options.currentStationID, 0),
        });
        ...
        else if (progress.totalQuantity >= Math.max(1, normalizeInteger(itemSpec && itemSpec.quantity, 1))) {
...
function isSessionInActiveMissionDungeon(session, missionRecord) {
    ...
    const missionPosition = missionRecord.missionPosition && typeof missionRecord.missionPosition === "object"
    ...
    const spaceRuntime = getSpaceRuntime();
    const scene = spaceRuntime && typeof spaceRuntime.getSceneForSession === "function"
        ? spaceRuntime.getSceneForSession(session)
        : null;
    ...
    if (missionInstanceID > 0) {
        const tracked = dungeonTrackingRuntime.resolveTrackedDungeonForSession(scene, session);
...
function expireMissionIfNeeded(characterID, agentID, missionRecord) {
    ...
    const expectedRuntimeStatus = normalizeText(missionRecord.runtimeStatus, "offered").toLowerCase();
    ...
    const expectedMissionSequence = normalizePositiveInteger(missionRecord.missionSequence, 0);
    const result = mutateCharacterState(characterID, (characterState) => {
        ...
        recordEpicArcMissionStatus(characterState, buildEpicArcMissionStatusRecord(storedMission, {
    ...
    const dungeonInstanceID = normalizePositiveInteger(expiredMissionRecord && expiredMissionRecord.dungeonInstanceID, 0);
...
function buildPlaceholderBriefing(agentRecord, missionRecord, missionTemplate) {
    const missionTitle = normalizeText(missionRecord && missionRecord.missionTitle, normalizeText(missionTemplate && missionTemplate.title, "Placeholder Mission"));
    ...
    const objectiveHints = [
        ...new Set((Array.isArray(missionTemplate && missionTemplate.objectiveHints)
    ...
    const triggerHints = [
        ...new Set((Array.isArray(missionTemplate && missionTemplate.triggerHints)
    ...
    const transportHints = [
        ...new Set((Array.isArray(missionTemplate && missionTemplate.transportHints)
    ...
    const roomNotes = rawRooms
        ...
        const note = normalizeText(Array.isArray(room && room.notes) ? room.notes[0] : "", "");
        if (!label || !note) {
            return "";
        }
        return `${label}: ${note}`;
    ...
    pushSection("Objective Intel", objectiveHints.length > 0
        ? objectiveHints
        : transportHints);
...
function buildMissionKeywords(agentRecord, missionRecord, missionTemplate) {
    ...
    if (!clientMissionRecord) {
        return {
            ...
            missionName: normalizeText(missionRecord && missionRecord.missionTitle, normalizeText(missionTemplate && missionTemplate.title, "Placeholder Mission")),
    ...
    const missionSystemID = normalizePositiveInteger(missionRecord && missionRecord.missionSystemID, normalizePositiveInteger(agentRecord && agentRecord.solarSystemID, 0));
    ...
    if (missionRecord.objectiveMode === OBJECTIVE_TYPE_TRANSPORT) {
        return {
            ...
            objectiveDestinationSystemID: normalizePositiveInteger(dropoffLocation.solarsystemID, 0),
            objectiveTypeID: normalizePositiveInteger(itemSpec && itemSpec.typeID, 0),
            objectiveLocationSystemID: normalizePositiveInteger(pickupLocation.solarsystemID, 0),
            ...rewardFields,
        };
    }
    if (isDungeonFetchMissionRecord(missionRecord) ||
        isMiningMissionRecord(missionRecord) ||
        ...
        return {
            ...
            ...(missionRecord.objectiveMode === OBJECTIVE_TYPE_DUNGEON
                ? {
                    dungeonLocationID: normalizePositiveInteger(pickupLocation.locationID, 0) || missionSystemID,
                }
                : {}),
            objectiveLocationSystemID: normalizePositiveInteger(dropoffLocation.solarsystemID, 0),
            ...rewardFields,
        };
    }
    return {
...
function buildMissionImage(missionRecord) {
    const imageName = isMiningMissionRecord(missionRecord)
        ? "miningmission.png"
        : (missionRecord &&
            [OBJECTIVE_TYPE_TRANSPORT, OBJECTIVE_TYPE_FETCH, OBJECTIVE_TYPE_AGENT].includes(missionRecord.objectiveMode))
            ? "couriermission.png"
            : "killmission.png";
...
function hasMissionShipRestrictions(...missionTemplates) {
    return missionTemplates.some((missionTemplate) => {
        const gateProfiles = Array.isArray(missionTemplate &&
            missionTemplate.siteSceneProfile &&
            missionTemplate.siteSceneProfile.gateProfiles)
            ? missionTemplate.siteSceneProfile.gateProfiles
            : [];
        const connections = Array.isArray(missionTemplate && missionTemplate.connections)
            ...
            : [];
        return [...gateProfiles, ...connections].some((gateProfile) => (normalizePositiveInteger(gateProfile && gateProfile.allowedShipsList, 0) > 0));
    });
}
function resolveMissionTitleValue(missionRecord, missionTemplate) {
    const missionNameID = normalizePositiveInteger(missionRecord && missionRecord.missionNameID, 0);
    if (missionNameID > 0) {
        return missionNameID;
    }
    return normalizeText(missionRecord && missionRecord.missionTitle, normalizeText(missionTemplate && missionTemplate.title, "Placeholder Mission"));
}
function resolveMissionBriefingValue(agentRecord, missionRecord, missionTemplate, clientMissionRecord, messageKeys) {
    const messageID = getFirstMissionMessageID(clientMissionRecord, messageKeys);
    if (messageID > 0) {
        return messageID;
    }
    const localizedText = getFirstMissionLocalizedMessageText(clientMissionRecord, messageKeys);
    if (localizedText) {
        return localizedText;
    }
    return buildPlaceholderBriefing(agentRecord, missionRecord, missionTemplate);
}
function buildEncounterObjectiveBriefingMessage(agentRecord, missionRecord, presentationTemplate, dungeonTemplate, clientMissionRecord) {
    const dungeonBriefingID = normalizePositiveInteger(dungeonTemplate && dungeonTemplate.missionBriefingID, normalizePositiveInteger(presentationTemplate && presentationTemplate.missionBriefingID, 0));
    if (dungeonBriefingID > 0) {
        return buildMissionProcessMessageReference(missionRecord, dungeonBriefingID);
    }
    const authoredBriefing = buildMissionProcessMessageReferenceByKeys(missionRecord, clientMissionRecord, [
    ...
    const localizedBriefing = getFirstMissionLocalizedMessageText(clientMissionRecord, [
        "messages.mission.briefing",
        "messages.mission.extrainfo.body",
        "messages.mission.accepted.agentsays",
        "messages.root.missioninprogress.agentsays",
    ...
    const derivedBriefing = buildPlaceholderBriefing(agentRecord, missionRecord, presentationTemplate || dungeonTemplate || null);
...
function resolveCapturedMissionSetup(presentationTemplate, contentID, agentRecord) {
    const setupByMissionID = normalizeObject(presentationTemplate && presentationTemplate.missionSetupByMissionID);
    ...
    const sourceSolarSystemID = normalizePositiveInteger(missionSetup.sourceSolarSystemID, 0);
    if (sourceAgentID <= 0 ||
...
function resolveCapturedMissionSystemID(presentationTemplate, contentID, agentRecord) {
    const missionSetup = resolveCapturedMissionSetup(presentationTemplate, contentID, agentRecord);
    return normalizePositiveInteger(missionSetup && missionSetup.missionSolarSystemID, 0);
}
function resolveCapturedMissionPosition(presentationTemplate, contentID, agentRecord) {
    ...
    if (!["x", "y", "z"].every((axis) => (missionPosition[axis] !== null &&
...
function resolveCapturedMissionRewardPresentation(presentationTemplate, contentID, agentRecord) {
    const missionSetup = resolveCapturedMissionSetup(presentationTemplate, contentID, agentRecord);
    const reward = normalizeObject(missionSetup && missionSetup.reward);
    return Object.keys(reward).length > 0 ? reward : null;
}
function buildMissionRecord(state, characterState, agentRecord, missionTemplate, selectionCursor = 0, explicitClientMissionRecord = null, advanceSelectionCursor = true) {
    const agentID = normalizePositiveInteger(agentRecord && agentRecord.agentID, 0);
    ...
    const fallbackMissionTemplateID = normalizeText(missionTemplate && missionTemplate.templateID, "");
    const contentID = normalizeMissionContentID(clientMissionRecord && clientMissionRecord.missionID, fallbackMissionTemplateID);
    const clientKillMission = clientMissionRecord &&
        clientMissionRecord.killMission &&
        typeof clientMissionRecord.killMission === "object" &&
        Object.keys(clientMissionRecord.killMission).length > 0
        ? clientMissionRecord.killMission
        : null;
    const clientCourierMission = clientMissionRecord &&
        clientMissionRecord.courierMission &&
    ...
    const runtimeDungeonTemplate = clientDungeonID > 0
        ? getClientDungeonTemplate(clientDungeonID)
        : null;
    const matchedMissionTemplate = resolveMissionTemplateForClientMission(agentRecord, clientMissionRecord, missionTemplate, runtimeDungeonTemplate);
    const importantMission = normalizeBoolean(agentRecord && agentRecord.importantMission, false);
    const clientMissionKind = normalizeText(clientMissionRecord && clientMissionRecord.missionKind, "");
    const sourceStationID = normalizePositiveInteger(agentRecord && agentRecord.stationID, 0);
    ...
    const clientDungeonItemSpec = objectiveMode === OBJECTIVE_TYPE_DUNGEON &&
        clientKillMission &&
        ...
        : objectiveMode === OBJECTIVE_TYPE_DUNGEON &&
            ...
            ? clientCourierMission
            : null;
    const cargo = objectiveMode === OBJECTIVE_TYPE_TRANSPORT || objectiveMode === OBJECTIVE_TYPE_FETCH
        ? (() => {
            const typeID = normalizePositiveInteger(clientCourierMission && clientCourierMission.objectiveTypeID, PLACEHOLDER_CARGO_TYPE_ID);
            const quantity = Math.max(1, normalizeInteger(clientCourierMission && clientCourierMission.objectiveQuantity, PLACEHOLDER_CARGO_QUANTITY));
        ...
        : clientDungeonItemSpec
            ...
            : null;
    const presentationMissionTemplate = matchedMissionTemplate || runtimeDungeonTemplate || missionTemplate || null;
    const capturedMissionSystemID = resolveCapturedMissionSystemID(presentationMissionTemplate, contentID, agentRecord);
    const capturedMissionPosition = resolveCapturedMissionPosition(presentationMissionTemplate, contentID, agentRecord);
    const runtimeMissionTemplate = matchedMissionTemplate &&
    ...
    const missionTemplateID = objectiveMode === OBJECTIVE_TYPE_TRANSPORT || objectiveMode === OBJECTIVE_TYPE_FETCH || objectiveMode === OBJECTIVE_TYPE_AGENT
        ? `client-mission:${missionContentIDToText(contentID, "unknown")}`
        : normalizeText(presentationMissionTemplate && presentationMissionTemplate.templateID, fallbackMissionTemplateID || `client-mission:${missionContentIDToText(contentID, "unknown")}`);
    const clientMissionTitle = normalizeText(getMissionLocalizedNameText(clientMissionRecord), humanizeClientMissionContentTemplate(clientMissionRecord && clientMissionRecord.contentTemplate));
    const missionTitle = objectiveMode === OBJECTIVE_TYPE_TRANSPORT ||
        objectiveMode === OBJECTIVE_TYPE_FETCH ||
        objectiveMode === OBJECTIVE_TYPE_AGENT
        ? normalizeText(clientMissionTitle, missionContentIDToText(contentID, missionTemplateID))
        : normalizeText(presentationMissionTemplate && presentationMissionTemplate.title, normalizeText(clientMissionTitle, missionContentIDToText(contentID, missionTemplateID)));
    const missionSystemID = objectiveMode === OBJECTIVE_TYPE_AGENT
        ? normalizePositiveInteger(conversationTargetLocation && conversationTargetLocation.solarsystemID, normalizePositiveInteger(agentRecord && agentRecord.solarSystemID, 0))
        : objectiveMode === OBJECTIVE_TYPE_FETCH
            ? normalizePositiveInteger(agentRecord && agentRecord.solarSystemID, normalizePositiveInteger(agentRecord && agentRecord.solarSystemID, 0))
            : objectiveMode === OBJECTIVE_TYPE_TRANSPORT
                ? normalizePositiveInteger(dropoffStation && dropoffStation.solarSystemID, normalizePositiveInteger(agentRecord && agentRecord.solarSystemID, 0))
                : objectiveMode === OBJECTIVE_TYPE_DUNGEON
                    ? capturedMissionSystemID || normalizePositiveInteger(clientObjectiveContext.dungeon_location_id, normalizePositiveInteger(agentRecord && agentRecord.solarSystemID, 0))
                    : normalizePositiveInteger(agentRecord && agentRecord.solarSystemID, 0);
    const missionPosition = objectiveMode === OBJECTIVE_TYPE_TRANSPORT ||
        objectiveMode === OBJECTIVE_TYPE_FETCH ||
        objectiveMode === OBJECTIVE_TYPE_AGENT
        ? null
    ...
    if (advanceSelectionCursor) {
        characterState.missionSelectionCursorByAgentID[String(agentID)] =
            (normalizeInteger(characterState.missionSelectionCursorByAgentID[String(agentID)], 0) + 1);
    }
    return {
        missionSequence,
        agentID,
        contentID,
        missionTemplateID,
        missionContentTemplateID: normalizeText(clientMissionRecord && clientMissionRecord.contentTemplate, ""),
        missionNameID: normalizePositiveInteger(clientMissionRecord && clientMissionRecord.nameID, 0),
        missionPoolKey: normalizeText(agentRecord && agentRecord.missionPoolKey, ""),
        missionKind: normalizeText(clientMissionKind, normalizeText(agentRecord && agentRecord.missionKind, "encounter")),
        missionTypeLabel: objectiveMode === OBJECTIVE_TYPE_AGENT &&
        ...
        gmCompleted: false,
        remoteCompletable: clientMissionRecord && clientMissionRecord.remoteCompletable !== undefined &&
        ...
        expiresAtFileTime: offerExpirationMs > 0 ? futureFileTimeString(offerExpirationMs) : "0",
        lastUpdatedAtMs: Date.now(),
        dungeonTemplateID: objectiveMode === OBJECTIVE_TYPE_DUNGEON
            ? normalizeText(runtimeMissionTemplate && runtimeMissionTemplate.templateID, missionTemplateID)
            : "",
        dungeonID: objectiveMode === OBJECTIVE_TYPE_DUNGEON
            ? // The retail client's GetDungeon() only resolves real catalog dungeon ids; a synthetic/placeholder
                ...
                normalizePositiveInteger(runtimeMissionTemplate && runtimeMissionTemplate.sourceDungeonID, 0) ||
                    normalizePositiveInteger(clientDungeonID, 0) ||
                    PLACEHOLDER_DUNGEON_ID_OFFSET + missionSequence
            : null,
        dungeonInstanceID: null,
        missionSiteID: objectiveMode === OBJECTIVE_TYPE_DUNGEON
        ...
        bookmarkIDsByRole: {},
        cargo,
        pickupLocation: objectiveMode === OBJECTIVE_TYPE_TRANSPORT
        ...
        dropoffLocation: objectiveMode === OBJECTIVE_TYPE_AGENT
            ? conversationTargetLocation
            : objectiveMode === OBJECTIVE_TYPE_FETCH
                ? buildLocationWrapForStation(fetchDropoffStationID)
                : objectiveMode === OBJECTIVE_TYPE_DUNGEON
                    ? buildLocationWrapForStation(sourceStationID)
                    : buildLocationWrapForStation(normalizePositiveInteger(dropoffStation && dropoffStation.stationID, sourceStationID)),
        rewards: buildMissionRewards(agentRecord, importantMission, clientMissionRecord, characterState && characterState.characterID, presentationMissionTemplate, contentID),
    };
}
function buildPendingStorylineOfferMissionRecord(agentRecord, offerRecord) {
    ...
    const directContentID = normalizeMissionContentID(offerRecord && offerRecord.contentID, null);
    const missionTemplateID = normalizeText(offerRecord && offerRecord.missionTemplateID, "");
    if (directContentID === null && !missionTemplateID) {
        return null;
    }
    const directClientMissionRecord = directContentID === null ? null : getMissionByID(directContentID);
    ...
    const clientMissionTitle = normalizeText(getMissionLocalizedNameText(clientMissionRecord), humanizeClientMissionContentTemplate(clientMissionRecord && clientMissionRecord.contentTemplate));
    const fallbackTitle = normalizeText(offerRecord && offerRecord.missionTitle, missionContentIDToText(contentID, "Storyline Mission"));
    return {
        missionSequence: normalizePositiveInteger(offerRecord && offerRecord.missionSequence, 0),
        agentID,
        contentID,
        missionTemplateID: normalizeText(missionTemplateID, `client-mission:${missionContentIDToText(contentID, "storyline-pending")}`),
        missionContentTemplateID: normalizeText(clientMissionRecord && clientMissionRecord.contentTemplate, ""),
        missionNameID: normalizePositiveInteger(clientMissionRecord && clientMissionRecord.nameID, 0),
        missionKind: normalizeText(clientMissionKind, normalizeText(agentRecord && agentRecord.missionKind, "encounter")),
...
function buildStandingPreview(characterID, agentRecord, missionRecord) {
    return standingRuntime.buildStandingPreview(normalizePositiveInteger(characterID, 0), standingRuntime.buildMissionRewardStandingChanges(agentRecord, missionRecord));
}
...
function missionGrantsItemsOnAccept(missionRecord) {
    const clientMissionRecord = getClientMissionRecord(missionRecord);
    if (normalizePositiveInteger(clientMissionRecord && clientMissionRecord.initialAgentGiftTypeID, 0) > 0) {
        return true;
    }
    ...
    return (normalizeText(missionRecord && missionRecord.objectiveMode, "") === OBJECTIVE_TYPE_TRANSPORT);
}
...
function isCharacterAtMissionCompletionLocation(characterID, agentRecord, missionRecord) {
    ...
    const dropoffStationID = normalizePositiveInteger(missionRecord &&
        missionRecord.dropoffLocation &&
        missionRecord.dropoffLocation.locationID, 0);
    const completesAtDropoff = dropoffStationID > 0 && (missionRecord.objectiveMode === OBJECTIVE_TYPE_TRANSPORT ||
...
function isMissionRemoteCompletable(missionRecord, options = {}) {
    const clientMissionRecord = getClientMissionRecord(missionRecord);
    const explicitRemoteCompletable = normalizeBoolean(missionRecord && missionRecord.remoteCompletable, normalizeBoolean(clientMissionRecord && clientMissionRecord.remoteCompletable, false));
    if (!explicitRemoteCompletable) {
        return false;
    }
    const agentRecord = options.agentRecord || getAgentRecord(missionRecord && missionRecord.agentID);
    const characterID = normalizePositiveInteger(options.characterID, 0);
...
function buildMissionJournalRow(characterID, agentRecord, missionRecord) {
    ...
    const missionTypeLabel = normalizeText(missionRecord && missionRecord.missionTypeLabel, buildMissionTypeLabel(agentRecord));
    return [
...
function buildPendingStorylineOfferJournalRows(characterID, characterState, activeAgentIDs = new Set()) {
    const pendingOffers = characterState &&
    ...
    return Object.values(pendingOffers)
        ...
        const agentRecord = getAgentRecord(agentID);
        const projectedMissionRecord = buildPendingStorylineOfferMissionRecord(agentRecord, offerRecord);
...
function buildMissionBriefingInfo(characterID, agentRecord, missionRecord, missionTemplate) {
    const clientMissionRecord = getClientMissionRecord(missionRecord);
    const accepted = normalizeText(missionRecord && missionRecord.runtimeStatus, "offered") === "accepted";
    const hasAuthoredOfferExpiration = normalizePositiveInteger(clientMissionRecord && clientMissionRecord.expirationTime, 0) > 0;
    const expirationFileTime = fileTimeStringToBigInt(missionRecord && missionRecord.expiresAtFileTime);
    const declineUntilFileTime = fileTimeStringToBigInt(getReplayUntilFileTime(characterID, agentRecord && agentRecord.agentID));
    const declineTime = accepted
        ...
        : declineUntilFileTime > nowFileTimeBigInt()
            ...
            : -1;
    return {
        "Mission Keywords": buildMissionKeywords(agentRecord, missionRecord, missionTemplate),
        ...
        "AcceptTimestamp": accepted && missionRecord && missionRecord.acceptedAtFileTime
            ? fileTimeBigIntToLong(fileTimeStringToBigInt(missionRecord.acceptedAtFileTime))
            : null,
        "Expiration Time": expirationFileTime > 0n && (accepted || hasAuthoredOfferExpiration)
            ? fileTimeBigIntToLong(expirationFileTime)
            : null,
        "Mission Briefing ID": resolveMissionBriefingValue(agentRecord, missionRecord, missionTemplate, clientMissionRecord, [
            "messages.mission.briefing",
            "messages.mission.extrainfo.body",
...
function buildMissionObjectiveCargo(cargo) {
    ...
    const itemSpec = buildMissionItemSpec(normalizedCargo, PLACEHOLDER_CARGO_TYPE_ID, PLACEHOLDER_CARGO_QUANTITY);
    const typeID = normalizePositiveInteger(itemSpec && itemSpec.typeID, PLACEHOLDER_CARGO_TYPE_ID);
    const quantity = Math.max(1, normalizeInteger(itemSpec && itemSpec.quantity, 1));
    const itemRecord = resolveItemByTypeID(typeID);
    const explicitVolume = Number(normalizedCargo.volume);
    return {
        volume: Number.isFinite(explicitVolume) && explicitVolume > 0
            ? explicitVolume
...
function resolveMissionObjectiveLocationOwnerID(location, fallbackOwnerID = 0) {
    const station = worldData.getStationByID(location && location.locationID);
    return normalizePositiveInteger(station && station.corporationID, normalizePositiveInteger(fallbackOwnerID, 0));
}
function buildMissionObjectivePayload(agentRecord, missionRecord, missionTemplate) {
    ...
    const objectiveCompleted = normalizeBoolean(missionRecord && missionRecord.objectiveCompleted, false);
    const clientMissionRecord = getClientMissionRecord(missionRecord);
    const dungeonTemplate = getDungeonMissionTemplateRecord(missionRecord);
    ...
    const clientFacingContentID = resolveClientMissionContentID(missionRecord, agentRecord);
    const objectives = [];
    const dungeons = [];
    ...
    const pickupSystemID = normalizePositiveInteger(missionRecord && missionRecord.pickupLocation && missionRecord.pickupLocation.solarsystemID, 0);
    const dropoffSystemID = normalizePositiveInteger(missionRecord && missionRecord.dropoffLocation && missionRecord.dropoffLocation.solarsystemID, 0);
    const missionSystemID = normalizePositiveInteger(missionRecord && missionRecord.missionSystemID, 0);
    ...
    if (isCourier) {
        const pickupLocation = missionRecord.pickupLocation || buildLocationWrapForStation(agentRecord.stationID);
        const dropoffLocation = missionRecord.dropoffLocation || buildLocationWrapForStation(agentRecord.stationID);
        objectives.push(buildMarshalTuple([
            OBJECTIVE_TYPE_TRANSPORT,
            buildMarshalTuple([
                resolveMissionObjectiveLocationOwnerID(pickupLocation, agentRecord.corporationID),
                buildMissionObjectiveLocationWrap(pickupLocation),
                resolveMissionObjectiveLocationOwnerID(dropoffLocation, agentRecord.corporationID),
    ...
    if (!isCourier && (isFetch || hasDungeonItemAxis)) {
        const objectiveLocation = missionRecord.dropoffLocation || buildLocationWrapForStation(agentRecord.stationID);
        const itemSpec = resolvePrimaryMissionItemSpec(missionRecord);
        ...
        objectives.push(buildMarshalTuple([
            OBJECTIVE_TYPE_FETCH,
            buildMarshalTuple([
                resolveMissionObjectiveLocationOwnerID(objectiveLocation, agentRecord.corporationID),
    ...
    if (!isCourier && isAgentObjective) {
        const targetAgentID = getMissionConversationTargetAgentID(missionRecord, normalizePositiveInteger(agentRecord && agentRecord.agentID, 0));
        const targetLocation = buildMissionConversationTargetLocationWrap(missionRecord, agentRecord);
    ...
    if (!isCourier && !isFetch && !isAgentObjective) {
        ...
        (normalizePositiveInteger(missionRecord.dungeonID, 0) < PLACEHOLDER_DUNGEON_ID_OFFSET
            ...
            normalizePositiveInteger(clientMissionRecord && clientMissionRecord.killMission && clientMissionRecord.killMission.dungeonID, 0) ||
            PLACEHOLDER_DUNGEON_ID_OFFSET + normalizePositiveInteger(missionRecord.missionSequence, 0);
        const dungeonLocation = cloneValue(missionRecord.pickupLocation || buildMissionSiteLocationWrap(agentRecord, missionRecord, dungeonTemplate || presentationTemplate));
        const briefingMessage = buildEncounterObjectiveBriefingMessage(agentRecord, missionRecord, presentationTemplate, dungeonTemplate, clientMissionRecord);
    ...
    const bonusIntervalMinutes = Math.max(0, normalizeInteger(rewards.bonusTimeIntervalMinutes, 0));
    if (bonusIntervalMinutes <= 0) {
        ...
        normalRewards.push(...normalizeArray(rewards.bonusItemRewards)
            .map((rewardRecord) => buildMissionRewardDisplayEntry(rewardRecord))
            .filter(Boolean));
    }
    const bonusRewards = bonusIntervalMinutes > 0
        ...
        : [];
    const missionExtraHeaderID = getMissionMessageID(clientMissionRecord, "messages.mission.extrainfo.header");
    const missionExtraBodyID = getMissionMessageID(clientMissionRecord, "messages.mission.extrainfo.body");
    ...
    return {
        ...
        agentGift: normalizePositiveInteger(clientMissionRecord && clientMissionRecord.initialAgentGiftTypeID, 0) > 0
            ? [buildMarshalTuple([
                    normalizePositiveInteger(clientMissionRecord.initialAgentGiftTypeID, 0),
                    Math.max(1, normalizeInteger(clientMissionRecord.initialAgentGiftQuantity, 1)),
                    null,
                ])]
            : [],
        normalRewards,
...
function buildMissionJournalInfo(characterID, agentRecord, missionRecord, missionTemplate) {
    ...
    return {
        ...
        contentID: clientFacingContentID,
        briefingTextID: resolveMissionBriefingValue(agentRecord, missionRecord, missionTemplate, clientMissionRecord, [
            "messages.mission.briefing",
            "messages.mission.extrainfo.body",
            "messages.mission.offered.agentsays",
        ]),
        missionImage: buildMissionImage(missionRecord),
        expirationTime: fileTimeBigIntToLong(fileTimeStringToBigInt(missionRecord && missionRecord.expiresAtFileTime)),
        missionState: getMissionStateFromRecord(missionRecord),
        objectives: buildMissionObjectivePayload(agentRecord, missionRecord, missionTemplate),
        bookmarks: listMissionBookmarks(characterID, missionRecord),
        iconID: missionRecord && missionRecord.objectiveMode === OBJECTIVE_TYPE_TRANSPORT
            ? 16
            : null,
    };
}
...
function notifyMissionChange(characterID, eventName, agentID) {
    ...
    for (const session of sessionRegistry.getSessions()) {
        if (Number(session && session.characterID) !== normalizedCharacterID ||
...
function notifyStorylineOfferLifecycleChange(characterID, agentID, status = "pending") {
    ...
    const eventName = normalizedStatus === "pending" || normalizedStatus === "offered"
...
function getReplayUntilFileTime(characterID, agentID) {
    ...
    return normalizeText(characterState.declineTimersByAgentID[String(normalizePositiveInteger(agentID, 0))], "");
}
    ...
    return mutateCharacterState(normalizedCharacterID, (characterState, state) => {
        ...
        if (forcedMissionID) {
            ...
            if (isMissionOfferAllowedForAgent(agentRecord, forcedClientMission)) {
                const forcedRecord = buildMissionRecord(state, characterState, agentRecord, null, 0, forcedClientMission, false);
        ...
        const availableClientMission = pickMissionForAgent(agentRecord, normalizeInteger(characterState.missionSelectionCursorByAgentID[String(normalizedAgentID)], 0));
        if (!pool.length && !availableClientMission) {
            return { kind: "unavailable" };
        }
        const cursor = normalizeInteger(characterState.missionSelectionCursorByAgentID[String(normalizedAgentID)], 0);
        ...
        const missionTemplate = selectedMissionTemplateID
            ...
            : null;
        const missionRecord = buildMissionRecord(state, characterState, agentRecord, missionTemplate, cursor);
...
function offerSpecificMission(characterID, agentRecord, clientMissionRecord) {
    ...
    return mutateCharacterState(normalizedCharacterID, (characterState, state) => {
        ...
        const missionRecord = buildMissionRecord(state, characterState, agentRecord, null, 0, clientMissionRecord, false);
        characterState.missionsByAgentID[String(normalizedAgentID)] = missionRecord;
        return { kind: "offered", missionRecord: cloneValue(missionRecord) };
    });
}
function grantMissionInitialAgentGift(characterID, agentRecord, missionRecord) {
    const clientMissionRecord = getClientMissionRecord(missionRecord);
    const typeID = normalizePositiveInteger(clientMissionRecord && clientMissionRecord.initialAgentGiftTypeID, 0);
    if (!typeID) {
        return { success: true, data: { granted: false } };
    }
    const quantity = Math.max(1, normalizeInteger(clientMissionRecord && clientMissionRecord.initialAgentGiftQuantity, 1));
    const stationID = normalizePositiveInteger(agentRecord && agentRecord.stationID, 0);
    if (!stationID) {
        return { success: true, data: { granted: false } };
    }
...
function acceptMission(characterID, agentRecord) {
    ...
    const preparedMissionRecord = ensureMissionSiteState(characterID, agentRecord, existingMissionRecord, missionTemplate);
    ...
    try {
        ...
        log.info(`[MissionDebug] accept char=${characterID} agent=${normalizePositiveInteger(agentRecord && agentRecord.agentID, 0)} ` +
    ...
    const giftResult = grantMissionInitialAgentGift(characterID, agentRecord, preparedMissionRecord);
    ...
    const acceptResult = mutateCharacterState(characterID, (characterState) => {
        ...
        const expirationDurationMs = resolveMissionExpirationDurationMs(getClientMissionRecord(missionRecord));
...
function clearMissionWithCooldown(characterID, agentRecord, runtimeStatus = "declined") {
    return mutateCharacterState(characterID, (characterState) => {
        ...
        if (missionRecord) {
            ...
            characterState.history = characterState.history.slice(0, 128);
            const closedAtFileTime = currentFileTimeString();
            recordEpicArcMissionStatus(characterState, buildEpicArcMissionStatusRecord(missionRecord, {
                quitDate: closedAtFileTime,
            }), { nowMs: Date.now() });
        }
        return {
...
function quitMission(characterID, agentRecord) {
    ...
    const expectedMissionSequence = normalizePositiveInteger(activeMission.missionSequence, 0);
    ...
    cleanupMissionBookmarks(characterID, missionRecord);
    const dungeonInstanceID = normalizePositiveInteger(missionRecord.dungeonInstanceID, 0);
    if (dungeonInstanceID > 0) {
        try {
            dungeonRuntime.purgeInstance(dungeonInstanceID);
        }
...
function applyMissionRewards(characterID, agentRecord, missionRecord) {
    const rewards = missionRecord && missionRecord.rewards && typeof missionRecord.rewards === "object"
        ? missionRecord.rewards
        : {};
    const standingsRaw = rewards.rawStandings && typeof rewards.rawStandings === "object"
        ? rewards.rawStandings
        : {};
    const bonusAvailable = isMissionBonusRewardAvailable(missionRecord);
    const totalWalletReward = roundMoney(Number(rewards.isk || 0) +
        (bonusAvailable ? Number(rewards.bonusIsk || 0) : 0));
    const walletResult = adjustCharacterBalance(characterID, totalWalletReward, {
    ...
    ].filter((rewardRecord) => normalizePositiveInteger(rewardRecord && rewardRecord.typeID, 0) > 0 &&
        Math.max(0, normalizeInteger(rewardRecord && rewardRecord.quantity, 0)) > 0);
    if (rewardItems.length > 0) {
        const rewardStationID = normalizePositiveInteger(missionRecord &&
            missionRecord.dropoffLocation &&
            missionRecord.dropoffLocation.locationID, normalizePositiveInteger(agentRecord && agentRecord.stationID, 0));
        if (rewardStationID > 0) {
            const grantResult = grantItemsToCharacterStationHangar(characterID, rewardStationID, rewardItems.map((rewardRecord) => ({
    ...
    if (normalizeInteger(rewards.loyaltyPoints, 0) > 0) {
        const lpResult = adjustCharacterWalletLPBalance(characterID, normalizePositiveInteger(agentRecord && agentRecord.corporationID, 0), normalizeInteger(rewards.loyaltyPoints, 0), { changeType: "mission_reward" });
        if (!lpResult.success) {
            return lpResult;
        }
    }
    const standingWriteResult = standingRuntime.applyMissionStandingChanges(characterID, agentRecord, missionRecord, "completed");
...
function completeMission(characterID, agentRecord) {
    ...
    if ((missionRecord.objectiveMode === OBJECTIVE_TYPE_TRANSPORT ||
        ...
        const dropoffLocationID = normalizePositiveInteger(missionRecord.dropoffLocation && missionRecord.dropoffLocation.locationID, 0);
        if (dropoffLocationID > 0) {
            const preferredSession = getPreferredCharacterSession(characterID);
            const currentDockedLocationID = normalizePositiveInteger(getDockedLocationID(preferredSession), 0);
            let takeResult = takeItemTypeFromCharacterLocation(characterID, dropoffLocationID, ITEM_FLAGS.HANGAR, missionRecord.cargo.typeID, missionRecord.cargo.quantity);
            if ((!takeResult || takeResult.success !== true) &&
                currentDockedLocationID === dropoffLocationID) {
                const activeShip = getActiveShipItem(characterID);
                const activeShipID = normalizePositiveInteger(activeShip && activeShip.itemID, 0);
                if (activeShipID > 0) {
                    takeResult = takeItemTypeFromCharacterLocation(characterID, activeShipID, ITEM_FLAGS.CARGO_HOLD, missionRecord.cargo.typeID, missionRecord.cargo.quantity);
                }
            }
            if (!takeResult || takeResult.success !== true) {
                return takeResult || { success: false, errorMsg: "MISSION_CARGO_NOT_FOUND" };
            }
            notifyMissionInventoryChanges(characterID, (takeResult.data && takeResult.data.changes) || []);
        }
    }
    ...
    const cleanupResult = mutateCharacterState(characterID, (characterState) => {
        ...
        const epicArcStatusResult = recordEpicArcMissionStatus(characterState, completedEpicArcStatus, { nowMs: Date.now() });
        if (epicArcStatusResult.recorded &&
            isEpicArcEndMission(missionRecord)) {
            recordEpicArcCompletion(characterState, {
...
function getJournalDetails(characterID) {
    ...
    const missionRows = Object.values(characterState.missionsByAgentID || {})
        .map((missionRecord) => {
        const syncedMissionRecord = getMissionRecordForRead(characterID, missionRecord && missionRecord.agentID);
        const agentRecord = getAgentRecord(missionRecord && missionRecord.agentID);
        ...
        .filter(Boolean);
    const activeAgentIDs = new Set(missionRows.map((row) => normalizePositiveInteger(row && row[4], 0)));
    const pendingStorylineRows = buildPendingStorylineOfferJournalRows(characterID, characterState, activeAgentIDs);
    return [missionRows.concat(pendingStorylineRows), []];
}
function getMyEpicArcStatus(characterID) {
    return buildEpicArcStatusPayloadFromCharacterState(getCharacterStateSnapshot(characterID));
}
function getCompletedCareerAgentMap(characterID, agentIDs = []) {
    const characterState = getCharacterStateSnapshot(characterID);
    const completedByID = characterState &&
...
function getMissionBriefingInfo(characterID, agentID) {
    ...
    return buildMissionBriefingInfo(characterID, agentRecord, missionRecord, getRuntimeMissionTemplateRecord(missionRecord));
}
function getMissionJournalInfo(characterID, agentID) {
    ...
    return buildMissionJournalInfo(normalizePositiveInteger(characterID, 0), agentRecord, missionRecord, getRuntimeMissionTemplateRecord(missionRecord));
}
function getMissionObjectiveInfo(characterID, agentID) {
    ...
    return buildMissionObjectivePayload(agentRecord, missionRecord, getRuntimeMissionTemplateRecord(missionRecord));
}
function getMissionKeywords(characterID, agentID, contentID = null) {
    ...
    const missionRecord = getMissionRecordForRead(characterID, agentID) ||
        (contentID
            ...
            : null);
    return buildMissionKeywords(agentRecord, missionRecord, getRuntimeMissionTemplateRecord(missionRecord));
}
function getStandingGainsForMission(characterID, agentID, contentID = null) {
    ...
    const missionRecord = getMissionRecordForRead(characterID, agentID) ||
        (contentID
            ? {
                contentID: normalizeMissionContentID(contentID, null),
                rewards: buildMissionRewards(agentRecord, normalizeBoolean(agentRecord && agentRecord.importantMission, false), null, characterID),
            }
            : null);
...
function markMissionObjectiveComplete(characterID, options = {}) {
    ...
    const missionRecords = Object.values((characterState && characterState.missionsByAgentID) || {}).filter((missionRecord) => normalizeText(missionRecord && missionRecord.runtimeStatus, "offered") ===
        "accepted");
    const targets = completeAll
        ? missionRecords
        : missionRecords.filter((missionRecord) => normalizePositiveInteger(missionRecord && missionRecord.agentID, 0) ===
            normalizedAgentID);
    if (!targets.length) {
        return { success: false, errorMsg: "MISSION_NOT_FOUND" };
    }
    ...
    const result = mutateCharacterState(normalizedCharacterID, (mutableCharacterState) => {
        for (const missionRecord of targets) {
            const storedMission = mutableCharacterState.missionsByAgentID[String(missionRecord.agentID)];
            if (!storedMission) {
                continue;
            }
...
function findReferredConversationMission(characterID, targetAgentID) {
    ...
    const characterState = getCharacterStateSnapshot(normalizedCharacterID);
    const acceptedMissions = Object.values((characterState && characterState.missionsByAgentID) || {}).filter((missionRecord) => (normalizeText(missionRecord && missionRecord.runtimeStatus, "offered") === "accepted" &&
        isAgentObjectiveMissionRecord(missionRecord) &&
        normalizePositiveInteger(missionRecord && missionRecord.agentID, 0) !== normalizedTargetAgentID));
    return acceptedMissions.find((missionRecord) => (getMissionConversationTargetAgentID(missionRecord) === normalizedTargetAgentID)) || null;
}
function buildAgentInteractionActionPayload(agentRecord, missionRecord, nextMissionID, optionIndex = 0) {
    ...
    const optionMessageID = getMissionMessageID(clientMissionRecord, `messages.mission.option${optionIndex + 1}.charsays`);
    ...
    return {
        ...
        "Mission Keywords": buildMissionKeywords(agentRecord, missionRecord, getRuntimeMissionTemplateRecord(missionRecord)),
        "Mission Title ID": titleID,
        "Mission Briefing ID": getMissionMessageID(clientMissionRecord, "messages.mission.extrainfo.body") ||
...
function buildAgentInteractionActions(agentRecord, missionRecord) {
    ...
    return nextMissionIDs
        ...
        const payload = buildAgentInteractionActionPayload(agentRecord, missionRecord, nextMissionID, index);
        if (!payload) {
            return null;
        }
        return [
...
function advanceConversationMission(characterID, sourceMissionRecord, nextMissionID) {
    ...
    const markResult = setMissionObjectiveCompleted(normalizedCharacterID, sourceMissionRecord.agentID);
    ...
    notifyMissionChange(normalizedCharacterID, AGENT_MISSION_COMPLETED, normalizePositiveInteger(sourceMissionRecord.agentID, 0));
    if (normalizedNextMissionID === null) {
        return {
            success: true,
            data: buildCompletedConversation(sourceAgentRecord, completionResult.data.missionRecord, getRuntimeMissionTemplateRecord(completionResult.data.missionRecord)),
        };
    }
    const nextMissionRecord = getMissionByID(normalizedNextMissionID);
    const nextAgentID = normalizePositiveInteger(nextMissionRecord &&
        (nextMissionRecord.sourceAgentID || nextMissionRecord.targetAgentID), 0);
    ...
    const offerResult = offerSpecificMission(normalizedCharacterID, nextAgentRecord, nextMissionRecord);
    ...
    return {
        success: true,
        data: buildOfferedConversation(nextAgentRecord, offerResult.data.missionRecord, getRuntimeMissionTemplateRecord(offerResult.data.missionRecord)),
    };
}
function resolveIdleConversationContentID(agentRecord, characterID) {
    const agentID = normalizePositiveInteger(agentRecord && agentRecord.agentID, 0);
    const activeMission = getMissionRecord(characterID, agentID);
    const activeContentID = normalizeMissionContentID(activeMission && activeMission.contentID, null);
    ...
    const characterState = getCharacterStateSnapshot(characterID);
    const selectionCursor = normalizeInteger(characterState &&
        characterState.missionSelectionCursorByAgentID &&
        characterState.missionSelectionCursorByAgentID[String(agentID)], 0);
...
function buildOfferedConversation(agentRecord, missionRecord, missionTemplate) {
    const clientMissionRecord = getClientMissionRecord(missionRecord);
    return {
        agentSays: buildConversationAgentSays(missionRecord, clientMissionRecord, [
            "messages.mission.offered.agentsays",
            "messages.mission.briefing",
...
function buildAcceptedConversation(characterID, agentRecord, missionRecord, missionTemplate, options = {}) {
    const clientMissionRecord = getClientMissionRecord(missionRecord);
    const actions = [];
    ...
    const atCompletionLocation = isCharacterAtMissionCompletionLocation(characterID, agentRecord, missionRecord);
    const showCompletionActions = actionResponse || remotelyCompletable || atCompletionLocation;
    const objectiveReady = normalizeBoolean(missionRecord && missionRecord.objectiveCompleted, false);
    ...
    return {
        agentSays: normalizeBoolean(missionRecord.objectiveCompleted, false)
            ? buildConversationAgentSays(missionRecord, clientMissionRecord, [
...
function buildCompletedConversation(agentRecord, missionRecord, missionTemplate) {
    ...
    return {
        agentSays: buildConversationAgentSays(missionRecord, clientMissionRecord, [
            "messages.mission.completed.agentsays",
            "messages.mission.completed.nextmission.agentsays",
        ], `Mission complete. Rewards and standings have been applied for ${normalizeText(missionRecord && missionRecord.missionTitle, missionContentIDToText(missionRecord && missionRecord.contentID, ""))}.`),
        actions: [
            [
...
function doAgentAction(characterID, agentID, actionID = null) {
    ...
    const rawActionID = actionID === null || actionID === undefined
        ? null
    ...
    const rawMissionExpiry = fileTimeStringToBigInt(rawMission && rawMission.expiresAtFileTime);
    const isIdempotentAcceptedAction = (normalizedActionID === AGENT_DIALOGUE_BUTTON_ACCEPT ||
    ...
    if ((rawActionID === AGENT_DIALOGUE_ACTION_TOKEN_COMPLETE ||
    ...
    const referredMission = !syncedMission
        ? findReferredConversationMission(normalizedCharacterID, agentID)
        : null;
    if (normalizedActionID === null) {
        if (referredMission) {
            return advanceConversationMission(normalizedCharacterID, referredMission, getMissionNextMissionIDs(referredMission)[0] ?? null);
        }
        if (syncedMission) {
            const missionTemplate = getRuntimeMissionTemplateRecord(syncedMission);
            return {
                success: true,
                data: normalizeText(syncedMission.runtimeStatus, "offered") === "accepted"
                    ? buildAcceptedConversation(normalizedCharacterID, agentRecord, syncedMission, missionTemplate)
    ...
    if (normalizedActionID === AGENT_DIALOGUE_BUTTON_REQUEST_MISSION ||
        ...
        return {
            success: true,
            data: buildOfferedConversation(agentRecord, offeredPayload.missionRecord, getRuntimeMissionTemplateRecord(offeredPayload.missionRecord)),
        };
    }
    if (normalizedActionID === AGENT_DIALOGUE_BUTTON_DEFER) {
        ...
        return {
            success: true,
            data: normalizeText(syncedMission.runtimeStatus, "").toLowerCase() === "accepted"
                ? buildAcceptedConversation(normalizedCharacterID, agentRecord, syncedMission, missionTemplate)
                : buildOfferedConversation(agentRecord, syncedMission, missionTemplate),
        };
    }
    if (normalizedActionID === AGENT_DIALOGUE_BUTTON_ACCEPT ||
        normalizedActionID === AGENT_DIALOGUE_BUTTON_ACCEPT_REMOTELY) {
        ...
        if (normalizedActionID === AGENT_DIALOGUE_BUTTON_ACCEPT_REMOTELY &&
            syncedMission &&
            !isMissionRemoteOfferable(syncedMission)) {
            return {
                success: true,
                data: buildOfferedConversation(agentRecord, syncedMission, getRuntimeMissionTemplateRecord(syncedMission)),
            };
        }
        ...
        return {
            success: true,
            data: buildAcceptedConversation(normalizedCharacterID, agentRecord, acceptResult.data, getRuntimeMissionTemplateRecord(acceptResult.data), { actionResponse: acceptResult.acceptedNow === true }),
        };
    }
    if (normalizedActionID === AGENT_DIALOGUE_BUTTON_DECLINE) {
        if (!syncedMission ||
            ...
            return syncedMission
                ? {
                    success: true,
                    data: buildAcceptedConversation(normalizedCharacterID, agentRecord, syncedMission, getRuntimeMissionTemplateRecord(syncedMission)),
                }
        ...
        if (config.missionDeclinePenaltyEnabled === true &&
            fileTimeStringToBigInt(getReplayUntilFileTime(normalizedCharacterID, agentID)) > nowFileTimeBigInt()) {
            try {
                standingRuntime.applyMissionStandingChanges(normalizedCharacterID, agentRecord, syncedMission, "declined");
            }
            catch (error) {
                /* decline standing penalty is best-effort; the decline must still proceed */
            }
        }
        clearMissionWithCooldown(normalizedCharacterID, agentRecord, "declined");
    ...
    if (normalizedActionID === AGENT_DIALOGUE_BUTTON_QUIT) {
        ...
        if (!quitResult.success) {
            ...
            return activeMission
                ? {
                    success: true,
                    data: buildAcceptedConversation(normalizedCharacterID, agentRecord, activeMission, getRuntimeMissionTemplateRecord(activeMission)),
                }
                : { success: true, data: buildIdleConversation(agentRecord, normalizedCharacterID) };
        }
    ...
    if (syncedMission &&
        ...
        if (nextMissionID !== null) {
            return advanceConversationMission(normalizedCharacterID, syncedMission, nextMissionID);
        }
    }
    if (normalizedActionID === AGENT_DIALOGUE_BUTTON_COMPLETE ||
        normalizedActionID === AGENT_DIALOGUE_BUTTON_COMPLETE_REMOTELY) {
        if (normalizedActionID === AGENT_DIALOGUE_BUTTON_COMPLETE_REMOTELY &&
            !isMissionRemoteCompletable(syncedMission, {
                characterID: normalizedCharacterID,
                agentRecord,
            })) {
        ...
        if (normalizedActionID === AGENT_DIALOGUE_BUTTON_COMPLETE &&
            !isCharacterAtMissionCompletionLocation(normalizedCharacterID, agentRecord, syncedMission)) {
            return syncedMission
                ? {
                    success: true,
                    data: buildAcceptedConversation(normalizedCharacterID, agentRecord, syncedMission, getRuntimeMissionTemplateRecord(syncedMission)),
        ...
        return {
            success: true,
            data: buildCompletedConversation(agentRecord, completedMissionRecord, getRuntimeMissionTemplateRecord(completedMissionRecord)),
        };
    }
    return { success: true, data: buildIdleConversation(agentRecord, normalizedCharacterID) };
}
...
//# sourceMappingURL=agentMissionRuntime.js.map