...
const { listAgents, listMissionTemplateIDsForAgent, } = require(path.join(__dirname, "./agentAuthority"));
const { isDisabledMissionIdentifier, isDisabledMissionSourceURL, isDisabledMissionTemplateIdentifier, isGeneratedMissionIdentifier, isRetiredMissionTemplateIdentifier, normalizeStableMissionIdentity, productionMissionPolicy, } = require(path.join(__dirname, "../../config/productionMissionPolicy"));
let cache = null;
...
const PERMANENTLY_DISABLED_MISSION_IDS = Object.freeze(productionMissionPolicy.disabledMissions.map(({ missionID }) => missionID));
...
function normalizeMissionRewardEntry(value) {
    ...
    const hasRewardType = Object.prototype.hasOwnProperty.call(value, "rewardTypeID") ||
        Object.prototype.hasOwnProperty.call(value, "typeID");
    const hasRewardQuantity = Object.prototype.hasOwnProperty.call(value, "rewardQuantity") ||
...
function normalizeLocalizedMessageEntry(value, fallbackMessageID = null) {
    ...
    return {
        messageID,
        text,
        metadata: Object.prototype.hasOwnProperty.call(value, "metadata")
            ? cloneValue(value.metadata)
            : null,
        tokens: Object.prototype.hasOwnProperty.call(value, "tokens")
...
function normalizeMissionRecord(record = {}, missionID) {
    return {
        ...
        hasStandingRewards: Object.prototype.hasOwnProperty.call(record, "hasStandingRewards")
        ...
        expirationTime: record.expirationTime === undefined || record.expirationTime === null
            ? null
            : Math.max(0, toInt(record.expirationTime, 0)),
        ...
        remoteCompletable: Object.prototype.hasOwnProperty.call(record, "remoteCompletable")
            ? record.remoteCompletable === true
            : null,
        remoteOfferable: Object.prototype.hasOwnProperty.call(record, "remoteOfferable")
            ? record.remoteOfferable === true
            : null,
...
function isPermanentlyDisabledMissionRecord(record = null, missionID = null) {
    if (isPermanentlyDisabledMissionID(missionID) ||
...
function filterAvailableMissionIDs(values, availableMissionKeys) {
    return normalizeMissionIDList(values).filter((missionID) => availableMissionKeys.has(String(missionID)));
}
function sanitizePayload(payload = {}) {
    const missionsByID = {};
    for (const [missionID, record] of Object.entries(payload.missionsByID || {})) {
        if (isPermanentlyDisabledMissionRecord(record, missionID) ||
    ...
    for (const [indexName, rawIndex] of Object.entries(normalizeObject(payload.indexes))) {
        ...
        if (indexName === "preferredMissionIDs") {
            indexes[indexName] = Object.fromEntries(Object.entries(index).filter(([, missionID]) => {
        ...
        indexes[indexName] = Object.fromEntries(Object.entries(index).map(([key, missionIDs]) => [
            key,
            filterAvailableMissionIDs(missionIDs, availableMissionKeys),
        ]));
    }
    const normalizedMissions = Object.entries(missionsByID).map(([missionID, record]) => normalizeMissionRecord(record, normalizeMissionID(missionID, missionID)));
    ...
    return {
        ...
        counts: {
            ...
            missionTemplateCount: new Set(normalizedMissions.map((record) => record.contentTemplate).filter(Boolean)).size,
            ...
            localizedMissionMessageCount: normalizedMissions.reduce((count, record) => count + Object.keys(record.localizedMessages || {}).length, 0),
            generatedEveSurvivalMissionCount: normalizedMissions.filter((record) => record.generatedFromSource === "eve-survival-generated").length,
        },
        missionsByID,
        indexes,
    };
...
function buildCache() {
    const payload = sanitizePayload(normalizePayload(readStaticTable(TABLE.MISSION_AUTHORITY)));
    const missionsByID = new Map();
    const epicArcMessageMaps = Object.fromEntries(EPIC_ARC_MESSAGE_TYPES.map((messageType) => [messageType, {}]));
    for (const [missionID, record] of Object.entries(payload.missionsByID || {})) {
    ...
    for (const [templateID, missionIDs] of Object.entries(normalizeObject(indexPayload.missionTemplateToMissionIDs))) {
        missionTemplateToMissionIDs.set(templateID, normalizeMissionIDList(missionIDs));
    }
    for (const missionRecord of missionsByID.values()) {
        addMissionTemplateIndex(missionTemplateToMissionIDs, missionRecord.dungeonTemplateID || missionRecord.missionTemplateID, missionRecord.missionID);
    }
    for (const [agentID, missionIDs] of Object.entries(normalizeObject(indexPayload.agentIDToMissionIDs))) {
        agentIDToMissionIDs.set(String(toInt(agentID, 0)), normalizeMissionIDList(missionIDs));
    }
    for (const [preferenceKey, missionID] of Object.entries(normalizeObject(indexPayload.preferredMissionIDs))) {
        const normalizedMissionID = normalizeMissionID(missionID, null);
...
const GOLDEN_SECURITY_MISSION_BY_ID = new Map(GOLDEN_SECURITY_MISSIONS.map((mission) => [String(mission.missionID), mission]));
const SUPPORTED_LEVEL_ONE_MINING_DUNGEON_IDS = new Set([2449, 2450, 2451, 2454, 2456]);
const SUPPORTED_GOLDEN_SECURITY_DUNGEON_IDS = new Set(GOLDEN_SECURITY_MISSIONS.map(({ dungeonID }) => dungeonID));
...
function isScriptedStorylineMissionRecord(record = null) {
    return Boolean(record && (record.isStoryline === true || record.isGenericStoryline === true));
}
...
function isAgentSpecificMissionIDForAgent(agentID, missionID) {
    ...
    const missionIDs = normalizedAgentID > 0
        ...
        : null;
    return Boolean(normalizedMissionID &&
...
function isScriptedMissionRecord(record = null) {
    return Boolean(record && (record.isEpicArc === true ||
...
function isExplicitStorylineAgent(agentRecord = null) {
    return Boolean(agentRecord && agentRecord.importantMission === true ||
...
function isGoldenSecurityMissionRecord(record = null, agentLevel = 0) {
    ...
    const actualDungeonID = toInt(record && record.killMission && record.killMission.dungeonID, 0);
    return Boolean(goldenMission &&
...
function isExactConfiguredGoldenMissionRecord(record = null, runtimeRecord = null) {
    ...
    const canonicalDungeonID = toInt(record && record.killMission && record.killMission.dungeonID, 0);
    ...
    if (presentationTemplateID.toLowerCase().startsWith("client-dungeon:") &&
        presentationTemplateID.toLowerCase() !== goldenMission.templateID.toLowerCase()) {
        return false;
    }
    return true;
}
function isRetiredOrdinaryEncounterMissionForAgent(agentID, missionIdentifier, runtimeRecord = null) {
    ...
    if (isScriptedMissionRecord(missionRecord) ||
...
function isMissionOfferAllowedForAgent(agentRecord = null, missionRecord = null) {
    ...
    if (isOrdinarySecurityAgent(agentRecord) &&
        !isGoldenSecurityMissionRecord(missionRecord, toInt(agentRecord && agentRecord.level, 0))) {
        return false;
    }
    const missionKey = String(normalizeMissionID(missionRecord.missionID, null));
    return listMissionIDsForAgent(agentRecord).some((missionID) => String(missionID) === missionKey);
}
function listGoldenSecurityMissionIDsForAgent(agentRecord = null) {
    ...
    return GOLDEN_SECURITY_MISSIONS
        ...
        .filter((missionID) => isGoldenSecurityMissionRecord(getMissionByID(missionID), agentLevel));
}
...
function listPreferredMissionCandidateIDs(agentRecord = null) {
    ...
    const cachedMissionIDs = authorityCache.preferredCandidateIDsBySignature.get(preferenceSignature);
    if (cachedMissionIDs) {
    ...
    for (const preferenceKey of preferenceKeys) {
        ...
        if (missionID !== null && missionID !== undefined) {
            ...
            if (preferredTemplateMission && preferredTemplateMission.contentTemplate) {
                for (const templateMissionID of listMissionIDsByTemplate(preferredTemplateMission.contentTemplate)) {
    ...
    authorityCache.preferredCandidateIDsBySignature.set(preferenceSignature, cloneValue(candidateMissionIDs));
...
function listMissionIDsForAgent(agentRecord = null) {
    ...
    if (isExplicitStorylineAgent(agentRecord)) {
        ...
        return missionKind === "encounter"
            ? storylineCandidateIDs.filter((missionID) => isScriptedStorylineMissionRecord(getMissionByID(missionID)))
            : storylineCandidateIDs;
    }
    if (templateBoundMissionIDs.length > 0) {
        return filterUnsupportedLevelOneAgentMissions(agentRecord, uniqueMissionIDs([...templateBoundMissionIDs, ...preferredMissionIDs]));
    }
...
function pickMissionForAgent(agentRecord = null, selectionIndex = 0) {
    ...
    const poolAgentOffset = poolKey
        ? listAgents()
            ...
            .sort((left, right) => (toInt(left && left.agentID, 0) - toInt(right && right.agentID, 0)))
...
//# sourceMappingURL=missionAuthority.js.map