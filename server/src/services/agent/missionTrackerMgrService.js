...
function buildTrackerOptions(session, agentID) {
    return {
        ...
        inActiveDungeon: agentMissionRuntime.isSessionInActiveMissionDungeon(session, agentMissionRuntime.getMissionRecord(normalizePositiveInteger(session && session.characterID, 0), agentID)),
    };
}
...
class MissionTrackerMgrService extends BaseService {
    ...
    Handle_UpdateAllMissions(args, session) {
        ...
        const updates = agentIDs.map((agentID) => buildDict([
            ["info", buildMissionInfoList(agentMissionRuntime.getMissionInfoItems(characterID, agentID, buildTrackerOptions(session, agentID)))],
            ["agentID", agentID],
        ]));
    ...
    Handle_GetMissionInfoItems(args, session) {
        ...
        return buildMissionInfoList(agentMissionRuntime.getMissionInfoItems(characterID, agentID, buildTrackerOptions(session, agentID)));
    }
    Handle_GetAllMissionObjectives(args, session) {
        const characterID = normalizePositiveInteger(session && session.characterID, 0);
        const agentID = normalizePositiveInteger(args && args[0], 0);
        return buildList(agentMissionRuntime.getAllMissionObjectives(characterID, agentID, buildTrackerOptions(session, agentID)));
    }
...
//# sourceMappingURL=missionTrackerMgrService.js.map