...
const { buildDict, currentFileTime, normalizeText, resolveBoundNodeId, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { listAgents, getAgentByID, } = require(path.join(__dirname, "./agentAuthority"));
const { getEpicArcMessageMaps, listDisabledMissionIDs, } = require(path.join(__dirname, "./missionAuthority"));
const { resolveDungeonShipRestrictions, } = require(path.join(__dirname, "../dungeon/dungeonShipRestrictionResolver"));
const { buildCachedMethodCallResult, } = require(path.join(__dirname, "../cache/objectCacheRuntime"));
const { buildDeferredCallResponse, } = require(path.join(__dirname, "../../network/callResponseControl"));
const spaceRuntime = require(path.join(__dirname, "../../space/runtime"));
const worldData = require(path.join(__dirname, "../../space/worldData"));
const bookmarkRuntime = require(path.join(__dirname, "../bookmark/bookmarkRuntimeState"));
const agentMissionRuntime = require(path.join(__dirname, "./agentMissionRuntime"));
...
function buildDoActionResponsePayload(result) {
    ...
    const actions = normalizeArrayLike(payload ? payload.actions : []).map((action) => toMarshalSafe(normalizeMarshalTuple(action, [])));
...
function toMarshalSafe(value) {
    ...
    if (typeof value === "object" &&
        ["tuple", "list", "set"].includes(value.type) &&
        Array.isArray(value.items)) {
        return {
            ...value,
            items: value.items.map((entry) => toMarshalSafe(entry)),
        };
    }
    if (typeof value === "object" &&
        value.type === "dict" &&
        ...
        return {
            ...value,
            entries: value.entries.map(([entryKey, entryValue]) => [
                toMarshalSafe(entryKey),
...
function buildNumericKeyDict(source = {}) {
    return buildDict(Object.entries(source || {})
...
function buildNestedEpicArcMessageDict() {
    ...
    buildNestedEpicArcMessageDict._cache = buildDict(Object.entries(getEpicArcMessageMaps() || {}).map(([messageType, missionMap]) => [
        messageType,
...
function filterMissionJournalToAgent(journalDetails, agentID) {
    ...
    return journalDetails.map((bucket) => (Array.isArray(bucket)
        ? bucket.filter((row) => Array.isArray(row) && normalizePositiveInteger(row[4], 0) === normalizedAgentID)
        : []));
}
function buildMissionJournalDetailsPayload(journalDetails) {
    const buckets = Array.isArray(journalDetails) ? journalDetails : [[], []];
    return buildMarshalTuple(buckets.map((bucket) => ({
        type: "list",
        items: normalizeArrayLike(bucket).map((row) => buildMarshalTuple(normalizeArrayLike(row).map((field) => toMarshalSafe(field)))),
    })));
}
...
function buildNestedEpicArcMessageDictUncached() {
    return buildDict(Object.entries(getEpicArcMessageMaps() || {}).map(([messageType, missionMap]) => [
        messageType,
        buildNumericKeyDict(missionMap),
    ]));
}
...
function sendAgentDeclineConfirmation(session, agentID, whenFileTime = currentFileTime(), options = {}) {
    ...
    return session.sendClientCallRequest("agents", "YesNo", [
        [
        ...
        machoTimeout: 86400,
        onResponse: typeof options.onResponse === "function"
            ? options.onResponse
            : null,
    }) !== null;
}
...
function buildTargetFromLocationWrap(locationWrap) {
    ...
    const staticTarget = bookmarkRuntime.resolveStaticBookmarkTarget(normalizePositiveInteger(locationWrap.locationID, 0));
    if (staticTarget) {
        return {
            kind: "item",
    ...
    return buildPointTarget(locationWrap, normalizePositiveInteger(locationWrap.solarsystemID, 0) ||
...
function isCurrentMissionBookmarkTarget(bookmarkTarget, role, missionRecord, missionAgentID) {
    ...
    const metadata = bookmarkTarget.metadata && typeof bookmarkTarget.metadata === "object"
        ? bookmarkTarget.metadata
        : {};
    if (normalizeText(metadata.role, "") !== role) {
        return false;
    }
    const bookmarkAgentID = normalizePositiveInteger(metadata.referringAgentID || metadata.agentID, 0);
    ...
    if (role === "dungeon") {
        ...
        if (expectedMissionInstanceID > 0 &&
    ...
    if (role === "source") {
        return normalizePositiveInteger(bookmarkTarget.locationID, 0) === normalizePositiveInteger(missionRecord &&
            missionRecord.pickupLocation &&
            (missionRecord.pickupLocation.solarsystemID || missionRecord.pickupLocation.locationID), 0);
    }
    if (role === "destination") {
        return normalizePositiveInteger(bookmarkTarget.locationID, 0) === normalizePositiveInteger(missionRecord &&
            missionRecord.dropoffLocation &&
            (missionRecord.dropoffLocation.solarsystemID || missionRecord.dropoffLocation.locationID), 0);
    }
    if (role === "objective") {
        const objectiveLocation = missionRecord && missionRecord.dropoffLocation;
        const expectedSystemID = normalizePositiveInteger(objectiveLocation && (objectiveLocation.solarsystemID || objectiveLocation.locationID), 0);
        if (expectedSystemID <= 0 ||
            normalizePositiveInteger(bookmarkTarget.locationID, 0) !== expectedSystemID) {
            return false;
        }
        const expectedItemID = normalizePositiveInteger(objectiveLocation && objectiveLocation.locationID, 0);
        const bookmarkItemID = normalizePositiveInteger(bookmarkTarget.bookmark && bookmarkTarget.bookmark.itemID, 0);
        return (expectedItemID <= 0 ||
            expectedItemID === expectedSystemID ||
            bookmarkItemID <= 0 ||
            bookmarkItemID === expectedItemID);
    }
...
function resolveMissionLocationTarget(characterID, agentID, locationType, referringAgentID = 0) {
    ...
    if (role === "agenthomebase") {
        return buildTargetFromLocationWrap(agentMissionRuntime.getAgentLocationWrap(missionAgentID || boundAgentID));
    }
    if (!missionRecord) {
        return null;
    }
    ...
    if (role === "objective") {
        return buildTargetFromLocationWrap(missionRecord.dropoffLocation ||
            agentMissionRuntime.getAgentLocationWrap(missionAgentID || boundAgentID));
    }
    if (role === "dungeon") {
        return buildPointTarget(missionRecord.missionPosition, missionRecord.missionSystemID, {
...
class AgentMgrService extends BaseService {
    ...
    Handle_MachoBindObject(args, session) {
        ...
        const agentID = normalizePositiveInteger(Array.isArray(bindParams) ? bindParams[0] : bindParams, 0);
    ...
    Handle_GetCompletedCareerAgentIDs(args, session) {
        ...
        return buildNumericKeyDict(agentMissionRuntime.getCompletedCareerAgentMap(normalizePositiveInteger(session && session.characterID, 0), agentIDs));
    }
    ...
    Handle_GetMyJournalDetails(args, session) {
        const journalDetails = agentMissionRuntime.getJournalDetails(normalizePositiveInteger(session && session.characterID, 0));
        const boundAgentID = this._resolveBoundAgentID(session, 0);
        return buildMissionJournalDetailsPayload(boundAgentID > 0
            ? filterMissionJournalToAgent(journalDetails, boundAgentID)
            : journalDetails);
    }
    Handle_GetPlausibleMissionIDs(args) {
        return toMarshalSafe(agentMissionRuntime.getPlausibleMissionIDs(normalizePositiveInteger(args && args[0], 0)));
    }
    Handle_GetMyEpicArcStatus(args, session) {
        return agentMissionRuntime.getMyEpicArcStatus(normalizePositiveInteger(session && session.characterID, 0));
    }
    Handle_GetMessagesForEpicArcMissions() {
        return buildCachedMethodCallResult(buildNestedEpicArcMessageDict(), {
    ...
    Handle_DoAction(args, session) {
        ...
        if (agentMissionRuntime.normalizeAgentDialogueActionID(actionID) ===
            agentMissionRuntime.AGENT_DIALOGUE_BUTTON_DECLINE) {
            return buildDeferredCallResponse(async ({ dispatcher, packet, responseServiceName, responsePerfMeta, }) => {
                dispatcher._sendCallResponse(packet, buildMarshalTuple([]), session, responseServiceName, responsePerfMeta, {
                    namedPayloadEntries: [
                        [
                            "provisional",
                ...
                const sent = sendAgentDeclineConfirmation(session, agentID, currentFileTime(), {
                    onResponse: (clientResponse) => {
                        ...
                        const finalResult = confirmed
                            ? agentMissionRuntime.doAgentAction(normalizePositiveInteger(session && session.characterID, 0), agentID, actionID)
                            : null;
                        dispatcher._sendCallResponse(packet, buildDoActionResponsePayload(finalResult), session, responseServiceName, responsePerfMeta);
                    },
                });
                if (!sent) {
                    const finalResult = agentMissionRuntime.doAgentAction(normalizePositiveInteger(session && session.characterID, 0), agentID, actionID);
                    dispatcher._sendCallResponse(packet, buildDoActionResponsePayload(finalResult), session, responseServiceName, responsePerfMeta);
                }
            }, { reason: "agent-decline-confirmation" });
        }
        const result = agentMissionRuntime.doAgentAction(normalizePositiveInteger(session && session.characterID, 0), agentID, actionID);
        return buildDoActionResponsePayload(result);
    }
    Handle_GetMissionBriefingInfo(args, session) {
        return toMarshalSafe(agentMissionRuntime.getMissionBriefingInfo(normalizePositiveInteger(session && session.characterID, 0), this._resolveBoundAgentID(session)));
    }
    Handle_GetMissionJournalInfo(args, session) {
        return toMarshalSafe(agentMissionRuntime.getMissionJournalInfo(normalizePositiveInteger(session && session.characterID, 0), this._resolveBoundAgentID(session)));
    }
    Handle_GetMissionObjectiveInfo(args, session) {
        return toMarshalSafe(agentMissionRuntime.getMissionObjectiveInfo(normalizePositiveInteger(session && session.characterID, 0), this._resolveBoundAgentID(session)));
    }
    Handle_GetMissionKeywords(args, session) {
        return toMarshalSafe(agentMissionRuntime.getMissionKeywords(normalizePositiveInteger(session && session.characterID, 0), this._resolveBoundAgentID(session), args && args.length > 0 ? args[0] : null));
    }
    Handle_RemoveOfferFromJournal(args, session) {
        return agentMissionRuntime.removeOfferFromJournal(normalizePositiveInteger(session && session.characterID, 0), this._resolveBoundAgentID(session));
    }
    Handle_GetAgentLocationWrap(args, session) {
        return toMarshalSafe(agentMissionRuntime.getAgentLocationWrap(this._resolveBoundAgentID(session)));
    }
    Handle_GetInfoServiceDetails(args, session) {
        return toMarshalSafe(agentMissionRuntime.getInfoServiceDetails(normalizePositiveInteger(session && session.characterID, 0), this._resolveBoundAgentID(session)));
    }
    Handle_GetStandingGainsForMission(args, session) {
        const agentID = this._resolveBoundAgentID(session);
        return buildNumericKeyDict(agentMissionRuntime.getStandingGainsForMission(normalizePositiveInteger(session && session.characterID, 0), agentID, args && args.length > 0 ? args[0] : null));
    }
    Handle_GetDungeonShipRestrictions(args) {
        // Client: GetAgentMoniker(agentID).GetDungeonShipRestrictions(dungeonID[, gateID]). The UI reads
        // restrictions['restrictedShipTypes'] to grey out barred ships (returning null here would raise a
        // client TypeError for any restricted mission). Enforcement is separate (keeperService gate).
        const dungeonID = Array.isArray(args) && args.length > 0 ? args[0] : null;
        const gateID = Array.isArray(args) && args.length > 1 ? args[1] : null;
        return resolveDungeonShipRestrictions(dungeonID, gateID);
    }
    ...
    Handle_GotoLocation(args, session) {
        ...
        const target = resolveMissionLocationTarget(characterID, agentID, locationType, referringAgentID);
        if (!target) {
            return null;
        }
    ...
    Handle_WarpToLocation(args, session) {
        ...
        const missionInstanceID = normalizePositiveInteger(target.metadata && target.metadata.missionInstanceID, 0);
        if (missionInstanceID > 0) {
            const siteEntityID = this._materializeMissionSiteForWarp(session, characterID, locationType, missionInstanceID);
            if (siteEntityID > 0) {
                spaceRuntime.warpToEntity(session, siteEntityID, {
            ...
            log.warn(`[AgentWarp] Refusing item WarpToLocation fallback without materialized site char=${characterID} instance=${missionInstanceID}`);
            return null;
        }
        if (target.kind === "item" && normalizePositiveInteger(target.itemID, 0) > 0) {
    ...
    _materializeMissionSiteForWarp(session, characterID, locationType, missionInstanceID) {
        try {
            ...
            if (!scene) {
                log.warn(`[AgentWarp] WarpToLocation no scene for char=${characterID} instance=${missionInstanceID}`);
                return 0;
            }
            const dungeonUniverseSiteService = require(path.join(__dirname, "../dungeon/dungeonUniverseSiteService"));
            const materialized = dungeonUniverseSiteService.ensureSiteContentsMaterialized(scene, { instanceID: missionInstanceID }, { spawnEncounters: true, broadcast: true, session });
            ...
            log.info(`[AgentWarp] WarpToLocation char=${characterID} role=${normalizeText(locationType, "?")} ` +
            ...
            log.warn(`[AgentWarp] WarpToLocation materialize failed char=${characterID} instance=${missionInstanceID}: ${error.message}`);
            return 0;
        }
    }
    Handle_WarpToAgentInSpace(args, session) {
        const agentID = this._resolveBoundAgentID(session, args && args.length > 0 ? args[0] : 0);
        if (!agentID) {
            return null;
        }
        const locationWrap = agentMissionRuntime.getAgentLocationWrap(agentID);
        if (!locationWrap ||
            worldData.getStationByID(normalizePositiveInteger(locationWrap.locationID, 0))) {
            return null;
        }
        const pointTarget = buildPointTarget(locationWrap, normalizePositiveInteger(locationWrap.solarsystemID, 0) ||
            normalizePositiveInteger(locationWrap.locationID, 0));
        if (pointTarget) {
    ...
    Handle_GetReplayTimestamp(args, session) {
        return agentMissionRuntime.getReplayTimestamp(normalizePositiveInteger(session && session.characterID, 0), this._resolveBoundAgentID(session));
    }
...
//# sourceMappingURL=agentMgrService.js.map