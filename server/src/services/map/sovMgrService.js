...
const { getInfrastructureHubClaim, getSystemSovClaim, listSovStructuresForSystem, } = require("../sovereignty/sovState");
const { acquireSkyhooks, destroySkyhooks, getFuelAccessGroupID, isOnLocalFuelAccessGroup, setFuelAccessGroupID, } = require("../sovereignty/sovModernState");
const { buildSovClaimInfoPayload, buildSovHubInfoPayload, buildSovStructuresPayload, } = require("../sovereignty/sovPayloads");
class SovMgrService extends BaseService {
    constructor() {
        super("sovMgr");
    }
    Handle_GetSovStructuresInfoForLocalSolarSystem(args, session) {
        const solarSystemID = Number(session && (session.solarsystemid2 || session.solarsystemid)) || 0;
        log.debug(`[SovMgr] GetSovStructuresInfoForLocalSolarSystem called (solarsystemid=${solarSystemID})`);
    ...
    Handle_GetSovStructuresInfoForSolarSystem(args, session) {
        ...
        log.debug(`[SovMgr] GetSovStructuresInfoForSolarSystem called (solarsystemid=${solarSystemID})`);
        return buildSovStructuresPayload(listSovStructuresForSystem(solarSystemID));
    }
...
//# sourceMappingURL=sovMgrService.js.map