...
function logDataLoad(message) {
    const writer = (log && typeof log.dataLoad === "function" && log.dataLoad.bind(log)) ||
...
class MiniFlatBufferBuilder {
    ...
    prep(size, additionalBytes) {
        ...
        const alignSize = (~(this.bytes.length - this.space + additionalBytes) + 1) & (size - 1);
...
function normalizeTagRows(value) {
    ...
    return rows
        .map((row) => ({
        containerID: Math.max(0, Math.trunc(Number(row && (row.containerID || row.containerId || row.id)) || 0)),
...
function buildMapTagsFlatbuffer(payload) {
    ...
    const tags = [...new Set([...systems, ...constellations, ...regions].flatMap((row) => row.tags))].sort((left, right) => left.localeCompare(right));
    ...
    const constellationsVector = createContainerVector(builder, constellations, tagStringOffsets);
...
function ensureLoaded() {
    if (!cache) {
        cache = buildCache();
        logDataLoad(`[MapTagsAuthority] Loaded map tags systems=${cache.counts.systems} ` +
...
//# sourceMappingURL=mapTagsAuthority.js.map