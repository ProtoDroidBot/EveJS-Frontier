...
function buildWarzonesDict() {
    return buildDict(Object.entries(WARZONES).map(([warzoneID, factionIDs]) => [
        Number(warzoneID),
        buildList(factionIDs),
    ]));
}
function buildHQSystemsDict() {
    return buildDict(Object.entries(FACTION_HQ_SYSTEMS).map(([factionID, solarSystemID]) => [
        Number(factionID),
        solarSystemID,
    ]));
}
function buildEmptyOccupationStatesByWarzone() {
    return buildDict(Object.keys(WARZONES).map((warzoneID) => [
        Number(warzoneID),
        buildDict([]),
    ]));
}
...
//# sourceMappingURL=fwWarzoneSolarsystemService.js.map