...
function recordSolarSystemVisit(sessionOrCharacterID, solarSystemID, options: Record<string, any> = {}) {
...
function recordSolarSystemJump(sessionOrCharacterID, details: Record<string, any> = {}) {
  const characterID = getCharacterID(sessionOrCharacterID);
...
function listSolarSystemVisitRows(characterID) {
  ...
  return Object.values<any>(systems)
    .map((entry) => {
