...
const { currentFileTime, normalizeBigInt, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
function normalizeInterferenceState(rawState = {}) {
    ...
    let quiescentInterferenceLevel = normalizeNumber(source.quiescentInterferenceLevel ?? source._quiescentInterferenceLevel, DEFAULT_QUIESCENT_INTERFERENCE_LEVEL);
    let maxInterferenceLevel = normalizeNumber(source.maxInterferenceLevel ?? source._maxInterferenceLevel, DEFAULT_MAX_INTERFERENCE_LEVEL);
    if (maxInterferenceLevel <= quiescentInterferenceLevel) {
        maxInterferenceLevel = quiescentInterferenceLevel + 1;
    }
    const interferenceDecreasePerDecayTick = Math.max(Number.EPSILON, normalizeNumber(source.interferenceDecreasePerDecayTick ??
        source._interferenceDecreasePerDecayTick, DEFAULT_INTERFERENCE_DECREASE_PER_DECAY_TICK));
    const decayTickPeriod = Math.max(Number.EPSILON, normalizeNumber(source.decayTickPeriod ?? source._decayTickPeriod, DEFAULT_DECAY_TICK_PERIOD_SECONDS));
    const interferenceLevel = Math.max(quiescentInterferenceLevel, Math.min(maxInterferenceLevel, normalizeNumber(source.interferenceLevel ?? source._interferenceLevel, quiescentInterferenceLevel)));
    return {
        interferenceLevel,
        lastDecayTickTimestamp: normalizeTimestamp(source.lastDecayTickTimestamp ?? source._lastDecayTickTimestamp),
...
function calculateInterferenceStateAtFileTime(rawState = {}, filetime = currentFileTime()) {
    const state = normalizeInterferenceState(rawState);
    ...
    const interferenceDecrease = decayTicks * state.interferenceDecreasePerDecayTick;
    return {
        ...state,
        interferenceLevel: Math.max(state.quiescentInterferenceLevel, state.interferenceLevel - interferenceDecrease),
...
function getInterferenceBand(rawState = {}) {
    ...
    const normalisedInterferenceLevel = (state.interferenceLevel - state.quiescentInterferenceLevel) /
...
function readSolarSystemInterferenceTable() {
    ...
    if (result.success && result.data && typeof result.data === "object") {
        return cloneValue({
            ...
            systems: result.data.systems && typeof result.data.systems === "object"
                ? result.data.systems
                : {},
        });
    }
...
function getSolarSystemInterferenceState(solarSystemID) {
    ...
    const result = repo.read(SOLAR_SYSTEM_INTERFERENCE_STATE_TABLE, systemPath(normalizedSolarSystemID));
    if (!result.success || !result.data || typeof result.data !== "object") {
        return null;
    }
...
function writeSolarSystemInterferenceState(solarSystemID, rawState) {
    ...
    const writeMetaResult = repo.write(SOLAR_SYSTEM_INTERFERENCE_STATE_TABLE, "/_meta/version", SOLAR_SYSTEM_INTERFERENCE_STATE_VERSION);
    ...
    const writeResult = repo.write(SOLAR_SYSTEM_INTERFERENCE_STATE_TABLE, systemPath(normalizedSolarSystemID), normalizeStorageState(rawState));
...
//# sourceMappingURL=solarSystemInterferenceState.js.map