...
const { resolveSystemCostIndex, } = require(path.join(__dirname, "../industry/industrySystemCostIndex"));
const structureState = require(path.join(__dirname, "../structure/structureState"));
const { buildJumpBridgeAccessPayload, } = require(path.join(__dirname, "../structure/structureDirectoryService"));
const planetOrbitalState = require(path.join(__dirname, "../planet/planetOrbitalState"));
const { listCurrentSovData, listRecentSovActivity, } = require(path.join(__dirname, "../sovereignty/sovState"));
const { buildCurrentSovDataPayload, buildRecentSovActivityPayload, } = require(path.join(__dirname, "../sovereignty/sovPayloads"));
const incursionRuntime = require(path.join(__dirname, "../incursion/incursionRuntime"));
const { buildIncursionGlobalReport, buildSystemsInIncursionsRowset, } = require(path.join(__dirname, "../incursion/incursionPayloads"));
const { getAllKillmailRecords, } = require(path.join(__dirname, "../killmail/killmailState"));
...
function getSessionCorporationID(session) {
    return toPositiveInteger(session && (session.corporationID || session.corpid || session.corpID), 0);
}
function getSessionSolarSystemID(session) {
    const solarSystemID = toPositiveInteger(session &&
        (session.solarsystemid2 ||
            session.solarsystemid ||
            session.solarSystemID ||
            (session._space && (session._space.systemID || session._space.solarSystemID))), 0);
    if (solarSystemID > 0) {
        return solarSystemID;
    }
    const locationID = toPositiveInteger(session && (session.locationid || session.locationID), 0);
    return locationID >= 30000000 && locationID < 40000000 ? locationID : 0;
}
function buildCorporationMemberMapRows(requestingSession, liveSessions = sessionRegistry.getSessions()) {
...
function buildSolarSystemItemRow(entry) {
    const typeID = toPositiveInteger((entry && (entry.typeID || entry.stationTypeID)) || 0, 0);
    const typeRecord = getItemTypeByID(typeID);
    const itemID = toPositiveInteger((entry && (entry.itemID || entry.stationID || entry.structureID)) || 0, 0);
    const position = getPosition(entry);
    if (!itemID || !typeID) {
        return null;
    }
    return [
        toPositiveInteger(entry && entry.groupID, toPositiveInteger(typeRecord && typeRecord.groupID, 0)),
        typeID,
        itemID,
...
function buildSolarSystemItemRows(solarSystemID) {
    ...
    const rows = [
        ...
        ...planetOrbitalState
            ...
            .filter((orbital) => !planetOrbitalState.isOrbitalRecordDestroyed(orbital))
...
function buildIndustryCostIndexDict(activityID) {
    return buildDict(listKnownSpaceSystemIDs().map((solarSystemID) => [
        solarSystemID,
...
class MapService extends BaseService {
    ...
    Handle_GetSecurityModifiedSystems(args, session, kwargs) {
        ...
        return buildRowset(["solarSystemID"], [], "eve.common.script.sys.rowset.Rowset");
    }
    Handle_GetStarterGroups() {
        const firstStation = [...worldData.ensureLoaded().stations].sort((left, right) => Number(left.stationID) - Number(right.stationID))[0];
        const starterSystemID = Number(firstStation && firstStation.solarSystemID) ||
            FRONTIER_STARTER_SYSTEM_ID;
        const system = worldData.getSolarSystemByID(starterSystemID);
        const groupName = (system && (system.solarSystemName || system.name)) ||
            `System ${starterSystemID}`;
        log.debug(`[MapService] GetStarterGroups group=${FRONTIER_STARTER_GROUP_ID} ` +
            `system=${starterSystemID}`);
        return buildList([
            buildDict([
    ...
    Handle_GetHistory(args) {
        ...
        return buildCachedMethodCallResult(buildRowset(HISTORY_COLUMNS, buildHistoryRows(statID, hours), MAP_ROWSET), {
    ...
    Handle_GetMyExtraMapInfo(_args, session) {
        ...
        log.debug(`[MapService] GetMyExtraMapInfo corporationID=${corporationID} rows=${rows.length}`);
    ...
    Handle_GetStationInfo(args, session) {
        const stations = [...worldData.ensureLoaded().stations].sort((left, right) => Number(left.stationID) - Number(right.stationID));
    ...
    Handle_GetStationCount(args, session) {
        ...
        return {
            type: "list",
            items: [...stationCountBySystemID.entries()].sort((left, right) => left[0] - right[0]),
        };
    }
}
