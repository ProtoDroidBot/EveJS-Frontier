...
const { getDevelopmentIndicesForSystem, listAllDevelopmentIndices, } = require("../sovereignty/sovState");
const { buildAllDevelopmentIndicesPayload, buildDevelopmentIndicesForSystemPayload, } = require("../sovereignty/sovPayloads");
class DevIndexManagerService extends BaseService {
    constructor() {
        super("devIndexManager");
    }
    Handle_GetDevelopmentIndicesForSystem(args, session) {
        const solarSystemID = Number(args && args.length > 0
            ? args[0]
            : session && (session.solarsystemid2 || session.solarsystemid)) || 0;
        log.debug(`[DevIndexManager] GetDevelopmentIndicesForSystem called (solarsystemid=${solarSystemID})`);
        ...
        return buildDevelopmentIndicesForSystemPayload(getDevelopmentIndicesForSystem(solarSystemID));
    }
...
//# sourceMappingURL=devIndexManagerService.js.map