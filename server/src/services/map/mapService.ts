...
function buildDungeonBeaconMapRows(
  solarSystemID,
  session = null,
  ...
  try {
    ...
    log.warn(
      `[MapService] Failed to list dungeon beacons for solarSystemID=${numericSystemID}: ${error.message}`,
    );
    return [];
  }
  ...
  return instances
      ...
      visibilityPolicy.resolveDungeonInstanceVisibilityForSession(
        ...
        instance && instance.instanceID,
        {
          ...
          getInstance: (instanceID) => (
            instancesByID.get(toPositiveInteger(instanceID, 0)) || null
          ),
        },
      ),
    ))
    .map((instance) => {
      ...
      return buildSolarSystemItemRow({
        ...
        orbitID:
          toPositiveInteger(metadata.anchorItemID, 0) ||
