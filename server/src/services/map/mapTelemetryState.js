...
function getCharacterID(sessionOrCharacterID) {
    if (sessionOrCharacterID && typeof sessionOrCharacterID === "object") {
        return toPositiveInteger(sessionOrCharacterID.characterID ||
...
function normalizeVisitEntry(entry, solarSystemID, nowMs, nowFiletime) {
    ...
    const lastDateTime = normalizeFiletimeString(entry && entry.lastDateTime, nowFiletime);
    return {
        solarSystemID,
        visits,
        lastDateTime,
        lastDateTimeMs: toPositiveInteger(entry && entry.lastDateTimeMs, msFromFiletime(lastDateTime, nowMs)),
    };
}
function recordSolarSystemVisit(sessionOrCharacterID, solarSystemID, options = {}) {
    ...
    const existing = normalizeVisitEntry(characterState.systems[systemKey], systemID, nowMs, nowFiletime);
...
function recordSolarSystemJump(sessionOrCharacterID, details = {}) {
    ...
    const toSolarSystemID = toPositiveInteger(details.toSolarSystemID || details.solarSystemID, 0);
...
function listSolarSystemVisitRows(characterID) {
    ...
    return Object.values(systems)
        ...
        const visits = toPositiveInteger(entry && entry.visits, 0);
        const lastDateTime = normalizeFiletimeString(entry && entry.lastDateTime, currentFiletimeString());
...
function listJumpHistoryRows(hours = 1, nowMs = Date.now()) {
    ...
    for (const event of state.jumpEvents || []) {
        ...
        const solarSystemID = toPositiveInteger(event && (event.toSolarSystemID || event.solarSystemID), 0);
        if (solarSystemID <= 0) {
            continue;
        }
...
//# sourceMappingURL=mapTelemetryState.js.map