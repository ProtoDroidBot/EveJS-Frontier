...
const { buildDict, buildFiletimeLong, buildObjectEx1, currentFileTime, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const solarSystemInterferenceState = require(path.join(__dirname, "solarSystemInterferenceState"));
const STATE_CLASS_NAME = "solarsysteminterference.solarsystemInterferenceState.SolarsystemInterferenceState";
function resolveSolarSystemID(session) {
    const numericValue = Number(session &&
...
function buildInterferenceStatePayload(rawState = {}) {
    const state = solarSystemInterferenceState.calculateInterferenceStateAtFileTime(rawState, currentFileTime());
...
class SolarSystemInterferenceMgrService extends BaseService {
    ...
    Handle_GetLocalInterferenceState(args, session) {
        ...
        const state = solarSystemInterferenceState.getSolarSystemInterferenceState(solarSystemID) ||
            solarSystemInterferenceState.buildQuietInterferenceState();
        log.debug(`[SolarSystemInterferenceMgr] GetLocalInterferenceState system=${solarSystemID || "unknown"} level=${state.interferenceLevel}`);
    ...
    Handle_GetAllInterferenceBands() {
        ...
        log.debug(`[SolarSystemInterferenceMgr] GetAllInterferenceBands -> ${entries.length} active`);
...
//# sourceMappingURL=solarsystemInterferenceMgrService.js.map