...
class CorpFittingMgrService extends BaseService {
    ...
    Handle_GetFittings(args, session) {
        ...
        return buildCachedMethodCallResult(getOwnerFittingsResponse(ownerID, OWNER_SCOPE.CORPORATION), {
    ...
    Handle_SaveFitting(args, session) {
        ...
        const result = handleStoreResult(saveFitting(ownerID, args && args[1], OWNER_SCOPE.CORPORATION), (data) => data);
    ...
    Handle_UpdateFitting(args, session, kwargs) {
        ...
        assertSessionCanMutateOwner(session, ownerID, OWNER_SCOPE.CORPORATION);
        const updatedFittingID = handleStoreResult(updateFitting(ownerID, fittingID, args && args[1], OWNER_SCOPE.CORPORATION), (data) => data.fittingID);
    ...
    Handle_SaveManyFittings(args, session) {
        ...
        assertSessionCanMutateOwner(session, ownerID, OWNER_SCOPE.CORPORATION);
        const mappings = handleStoreResult(saveManyFittings(ownerID, args && args[1], OWNER_SCOPE.CORPORATION), (data) => data);
    ...
    Handle_DeleteFitting(args, session) {
        ...
        assertSessionCanMutateOwner(session, ownerID, OWNER_SCOPE.CORPORATION);
        const deletedResult = handleStoreResult(deleteFitting(ownerID, args && args[1], OWNER_SCOPE.CORPORATION), () => null);
        notifyFittingMutation(session, "OnFittingDeleted", [ownerID, Number(args && args[1]) || 0], { ownerScope: OWNER_SCOPE.CORPORATION });
    ...
    Handle_DeleteManyFittings(args, session) {
        ...
        assertSessionCanMutateOwner(session, ownerID, OWNER_SCOPE.CORPORATION);
        const deletedIDs = handleStoreResult(deleteManyFittings(ownerID, args && args[1], OWNER_SCOPE.CORPORATION), buildDeletedResult);
        notifyFittingMutation(session, "OnManyFittingsDeleted", [ownerID, Array.isArray(deletedIDs && deletedIDs.items) ? deletedIDs.items : []], { ownerScope: OWNER_SCOPE.CORPORATION });
        return deletedIDs;
    }
    Handle_UpdateNameAndDescription(args, session) {
        ...
        assertSessionCanMutateOwner(session, ownerID, OWNER_SCOPE.CORPORATION);
        const updatedResult = handleStoreResult(updateFittingNameAndDescription(fittingID, ownerID, args && args[2], args && args[3], OWNER_SCOPE.CORPORATION), () => null);
...
//# sourceMappingURL=corpFittingMgrService.js.map