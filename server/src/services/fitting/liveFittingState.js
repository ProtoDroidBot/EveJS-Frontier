...
const { ITEM_FLAGS, listContainerItems, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const STRATEGIC_CRUISER_SUBSYSTEM_FLAGS = Object.freeze([125, 126, 127, 128]);
...
function isShipFittingFlag(flagID) {
    ...
    return SHIP_FITTING_FLAG_RANGES.some(([start, end]) => numericFlagID >= start && numericFlagID <= end);
}
...
function normalizeModuleState(rawState) {
    const source = rawState && typeof rawState === "object" ? rawState : DEFAULT_MODULE_STATE;
    return {
...
function getDogmaLookups() {
    ...
    const attributeTypesByID = (typeDogmaRoot && typeDogmaRoot.attributeTypesByID) ||
        (shipDogmaRoot && shipDogmaRoot.attributeTypesByID) ||
        {};
    const effectTypesByID = (typeDogmaRoot && typeDogmaRoot.effectTypesByID) || {};
...
function getTypeDogmaRecord(typeID) {
    ...
    return ((root.typesByTypeID && root.typesByTypeID[String(numericTypeID)]) || null);
}
...
function getNormalizedTypeAttributeMap(typeID) {
    ...
    const normalized = Object.freeze(normalizeNumericAttributeMap(getTypeDogmaAttributes(numericTypeID)));
...
function isStrategicCruiserType(typeID) {
    const typeRecord = resolveItemByTypeID(toInt(typeID, 0)) || null;
    return toInt(typeRecord && typeRecord.groupID, 0) === STRATEGIC_CRUISER_GROUP_ID;
}
function getSubsystemSlotFlag(typeID) {
    if (getRequiredSlotFamily(typeID) !== "subsystem") {
        return 0;
    }
    const flagID = toInt(getTypeAttributeValue(typeID, "subSystemSlot"), 0);
    return STRATEGIC_CRUISER_SUBSYSTEM_FLAGS.includes(flagID) ? flagID : 0;
}
function isSubsystemCompatibleWithStrategicCruiser(subsystem, shipTypeID) {
    const subsystemTypeID = toInt(subsystem && subsystem.typeID, 0);
    const numericShipTypeID = toInt(shipTypeID, 0);
    if (subsystemTypeID <= 0 ||
        numericShipTypeID <= 0 ||
        getRequiredSlotFamily(subsystemTypeID) !== "subsystem") {
        return false;
    }
    const fittedShipTypeID = toInt(getTypeAttributeValue(subsystemTypeID, "fitsToShipType"), 0);
    if (fittedShipTypeID > 0) {
        return fittedShipTypeID === numericShipTypeID;
    }
    const shipType = resolveItemByTypeID(numericShipTypeID) || null;
    const subsystemType = resolveItemByTypeID(subsystemTypeID) || null;
    const shipName = String(shipType && shipType.name || "").trim().toLowerCase();
    const subsystemName = String(subsystemType && subsystemType.name || "").trim().toLowerCase();
    return Boolean(shipName && subsystemName.startsWith(`${shipName} `));
}
function getStrategicCruiserSubsystemStatus(shipItem, fittedItems = []) {
    const shipID = toInt(shipItem && shipItem.itemID, 0);
    const shipTypeID = toInt(shipItem && shipItem.typeID, 0);
    const requiredFlags = isStrategicCruiserType(shipTypeID)
        ? [...STRATEGIC_CRUISER_SUBSYSTEM_FLAGS]
        : [];
    const fittedByFlag = new Map();
    const duplicateFlags = [];
    const invalidItems = [];
    for (const item of Array.isArray(fittedItems) ? fittedItems : []) {
        if (!item || getRequiredSlotFamily(item.typeID) !== "subsystem") {
            continue;
        }
        if (!isFittedModuleItem(item)) {
            continue;
        }
        const itemLocationID = toInt(item.locationID, 0);
        const flagID = toInt(item.flagID, 0);
        if ((shipID > 0 && itemLocationID > 0 && itemLocationID !== shipID) ||
            !requiredFlags.includes(flagID) ||
            getSubsystemSlotFlag(item.typeID) !== flagID ||
            !isSubsystemCompatibleWithStrategicCruiser(item, shipTypeID)) {
            invalidItems.push(item);
            continue;
        }
        if (fittedByFlag.has(flagID)) {
            if (!duplicateFlags.includes(flagID)) {
                duplicateFlags.push(flagID);
            }
            continue;
        }
        fittedByFlag.set(flagID, item);
    }
    const missingFlags = requiredFlags.filter((flagID) => !fittedByFlag.has(flagID));
    return {
        requiredFlags,
        fittedByFlag,
        missingFlags,
        duplicateFlags,
        invalidItems,
        complete: requiredFlags.length > 0 &&
            missingFlags.length === 0 &&
            duplicateFlags.length === 0 &&
            invalidItems.length === 0,
    };
}
function getValidStrategicCruiserSubsystems(shipTypeID, fittedItems = [], shipID = 0) {
    if (!isStrategicCruiserType(shipTypeID)) {
        return [];
    }
    const status = getStrategicCruiserSubsystemStatus({
        itemID: toInt(shipID, 0),
        typeID: toInt(shipTypeID, 0),
    }, fittedItems);
    return status.requiredFlags
        .map((flagID) => status.fittedByFlag.get(flagID) || null)
        .filter(Boolean);
}
function readShipBaseAttributes(shipTypeID) {
    const numericTypeID = toInt(shipTypeID, 0);
    const shipDogmaRoot = readStaticTable(TABLE.SHIP_DOGMA_ATTRIBUTES);
    const entry = shipDogmaRoot &&
...
function getShipSlotCounts(shipTypeID, fittedItems = [], shipID = 0) {
    const attributes = readShipBaseAttributes(shipTypeID);
    ...
    const compatibleSubsystems = isStrategicCruiser
        ? getValidStrategicCruiserSubsystems(shipTypeID, fittedItems, shipID)
        : [];
    const sumSubsystemModifier = (...attributeNames) => compatibleSubsystems.reduce((sum, item) => sum + toInt(getEffectiveItemAttributeValue(item, ...attributeNames), 0), 0);
    return {
        low: (lowSlotsID ? toInt(attributes[String(lowSlotsID)], 0) : 0) +
            sumSubsystemModifier("lowSlotModifier"),
        med: (medSlotsID ? toInt(attributes[String(medSlotsID)], 0) : 0) +
            sumSubsystemModifier("medSlotModifier"),
        high: (hiSlotsID ? toInt(attributes[String(hiSlotsID)], 0) : 0) +
            sumSubsystemModifier("hiSlotModifier", "highSlotModifier"),
        rig: rigSlotsID ? toInt(attributes[String(rigSlotsID)], 0) : 0,
        subsystem: isStrategicCruiser
...
function getSlotFlagsForFamily(family, shipTypeID = null, fittedItems = [], shipID = 0) {
    ...
    if (family === "subsystem" && isStrategicCruiserType(shipTypeID)) {
        return [...STRATEGIC_CRUISER_SUBSYSTEM_FLAGS];
    }
...
function listHiddenModifierItems(ownerID, shipID, shipItem = null) {
    ...
    const hiddenItems = listContainerItems(numericOwnerID > 0 ? numericOwnerID : null, numericShipID, HIDDEN_MODIFIERS_FLAG)
        .filter((item) => item && toInt(item.typeID, 0) > 0)
        .sort(sortFittedItems);
    const shipStanceRuntime = getShipStanceRuntime();
    const stanceState = shipStanceRuntime.resolveCurrentShipStance(shipItem && typeof shipItem === "object"
        ? shipItem
        : numericShipID);
    if (stanceState.supported === true &&
        toInt(stanceState.modifierTypeID, 0) > 0 &&
        !hiddenItems.some((item) => (shipStanceRuntime.ALL_STANCE_MODIFIER_TYPE_IDS.includes(toInt(item && item.typeID, 0))))) {
        hiddenItems.push(buildVirtualHiddenModifierItem(stanceState.shipItem || shipItem || {
            itemID: numericShipID,
            ownerID: numericOwnerID,
        }, stanceState.modifierTypeID, `shipStance:${stanceState.stanceID}:default`));
    }
    return hiddenItems;
}
function getPassiveModifierSourceItems(shipItem, fittedItems = [], options = {}) {
    ...
    const hiddenSources = Array.isArray(options.hiddenModifierItems)
        ...
        : listHiddenModifierItems(toInt(shipItem && shipItem.ownerID, 0), toInt(shipItem && shipItem.itemID, 0), shipItem);
    const byKey = new Map();
...
function getFittedModuleByFlag(charID, shipID, flagID) {
    const numericFlagID = toInt(flagID, 0);
    return (getFittedModuleItems(charID, shipID).find((item) => toInt(item.flagID, 0) === numericFlagID) || null);
}
function getLoadedChargeByFlag(charID, shipID, flagID) {
    const numericFlagID = toInt(flagID, 0);
    return (getLoadedChargeItems(charID, shipID).find((item) => toInt(item.flagID, 0) === numericFlagID) || null);
}
...
function validateShipTypeOrGroupRestriction(moduleTypeID, shipItem) {
    const allowedShipTypes = getShipRestrictionValues(moduleTypeID, "canFitShipType", 12);
    const allowedShipGroups = getShipRestrictionValues(moduleTypeID, "canFitShipGroup", 20, 2);
    if (allowedShipTypes.length <= 0 && allowedShipGroups.length <= 0) {
        return { success: true };
    }
    const shipTypeID = toInt(shipItem && shipItem.typeID, 0);
    const shipGroupID = toInt(shipItem && shipItem.groupID, 0);
    const shipTypeAllowed = allowedShipTypes.length > 0 && allowedShipTypes.includes(shipTypeID);
    const shipGroupAllowed = allowedShipGroups.length > 0 && allowedShipGroups.includes(shipGroupID);
...
function filterStructureDogmaSkillMap(skillMap) {
    ...
    return new Map([...skillMap.entries()].filter(([, skillRecord]) => (STRUCTURE_DOGMA_SKILL_TYPE_IDS.has(toInt(skillRecord && skillRecord.typeID, 0)))));
}
...
const ATTRIBUTE_SPEED_BOOST_FACTOR = getAttributeIDByNames("speedBoostFactor") || 567;
...
const ATTRIBUTE_CAPACITOR_RECHARGE_RATE = getAttributeIDByNames("rechargerate", "capacitorRechargeRate") || 55;
...
const ATTRIBUTE_UPGRADE_CAPACITY = getAttributeIDByNames("upgradeCapacity") || 1132;
const ATTRIBUTE_UPGRADE_LOAD = getAttributeIDByNames("upgradeLoad") || 1154;
const ATTRIBUTE_TURRET_SLOTS_LEFT = getAttributeIDByNames("turretSlotsLeft") || 102;
const ATTRIBUTE_LAUNCHER_SLOTS_LEFT = getAttributeIDByNames("launcherSlotsLeft") || 101;
const ATTRIBUTE_LOW_SLOTS = getAttributeIDByNames("lowSlots") || 12;
const ATTRIBUTE_MED_SLOTS = getAttributeIDByNames("medSlots") || 13;
const ATTRIBUTE_HIGH_SLOTS = getAttributeIDByNames("hiSlots") || 14;
const ATTRIBUTE_DRONE_CAPACITY = getAttributeIDByNames("droneCapacity") || 283;
const ATTRIBUTE_FIGHTER_CAPACITY = getAttributeIDByNames("fighterCapacity") || 2055;
...
const ATTRIBUTE_DAMAGE_MULTIPLIER_BONUS = getAttributeIDByNames("damageMultiplierBonus") || 292;
const ATTRIBUTE_ROF_BONUS = getAttributeIDByNames("rofBonus") || 293;
const MINING_HOLD_ATTRIBUTE_IDS = Object.freeze(MINING_HOLD_DEFINITIONS.map((definition) => ({
...
const STACKING_DENOMINATORS = Object.freeze(Array.from({ length: 8 }, (_, index) => Math.exp((index / 2.67) ** 2)));
...
function getAttributeTypeRecord(attributeID) {
    ...
    const root = getTypeDogmaRoot();
    return ((root.attributeTypesByID &&
        root.attributeTypesByID[String(numericAttributeID)]) ||
        null);
}
function getShipHardpointCounts(shipTypeID, fittedItems = [], shipID = 0) {
    const numericShipTypeID = toInt(shipTypeID, 0);
    let turret = toInt(getShipBaseAttributeValue(numericShipTypeID, "turretSlotsLeft"), 0);
    let launcher = toInt(getShipBaseAttributeValue(numericShipTypeID, "launcherSlotsLeft"), 0);
    if (isStrategicCruiserType(numericShipTypeID)) {
        for (const item of getValidStrategicCruiserSubsystems(numericShipTypeID, fittedItems, shipID)) {
            turret += toInt(getEffectiveItemAttributeValue(item, "turretHardPointModifier", "turretHardpointModifier"), 0);
            launcher += toInt(getEffectiveItemAttributeValue(item, "launcherHardPointModifier", "launcherHardpointModifier"), 0);
        }
    }
    return {
        turret: Math.max(0, turret),
        launcher: Math.max(0, launcher),
    };
}
function getStrategicCruiserUseStatus(charID, shipItem, fittedItems = []) {
    const shipID = toInt(shipItem && shipItem.itemID, 0);
    const shipTypeID = toInt(shipItem && shipItem.typeID, 0);
    if (!isStrategicCruiserType(shipTypeID)) {
        return {
            ...
            invalidRegularSlotItems: [],
            turretOverload: 0,
            launcherOverload: 0,
            cpuOverload: 0,
            powerOverload: 0,
            droneBayOverload: 0,
            fighterBayOverload: 0,
        };
    }
    const subsystemStatus = getStrategicCruiserSubsystemStatus(shipItem, fittedItems);
    const invalidRegularSlotItems = [];
    const occupiedRegularFlags = new Set();
    for (const item of Array.isArray(fittedItems) ? fittedItems : []) {
        if (!item || !isFittedModuleItem(item)) {
            continue;
        }
        const family = getRequiredSlotFamily(item.typeID);
        if (!family || family === "subsystem") {
            continue;
        }
        const flagID = toInt(item.flagID, 0);
        const allowedFlags = getSlotFlagsForFamily(family, shipTypeID, fittedItems, shipID);
        if (!allowedFlags.includes(flagID) || occupiedRegularFlags.has(flagID)) {
            invalidRegularSlotItems.push(item);
            continue;
        }
        occupiedRegularFlags.add(flagID);
    }
    const hardpointCounts = getShipHardpointCounts(shipTypeID, fittedItems, shipID);
    ...
    const turretOverload = Math.max(0, usedTurrets - hardpointCounts.turret);
    const launcherOverload = Math.max(0, usedLaunchers - hardpointCounts.launcher);
    const cpuOverload = Math.max(0, round6(toFiniteNumber(resourceState.cpuLoad, 0) -
        toFiniteNumber(resourceState.cpuOutput, 0)));
    const powerOverload = Math.max(0, round6(toFiniteNumber(resourceState.powerLoad, 0) -
        toFiniteNumber(resourceState.powerOutput, 0)));
    const getBayUsedVolume = (flagID) => round6((Array.isArray(fittedItems) ? fittedItems : [])
        .filter((item) => toInt(item && item.flagID, 0) === flagID)
        .reduce((sum, item) => {
        const typeRecord = resolveItemByTypeID(toInt(item && item.typeID, 0)) || {};
        const unitVolume = Math.max(0, toFiniteNumber(item && item.volume, toFiniteNumber(typeRecord.volume, 0)));
        const quantity = toInt(item && item.singleton, 0) === 1
            ? 1
            : Math.max(0, toInt(item && (item.stacksize ?? item.quantity), 0));
        return sum + unitVolume * quantity;
    }, 0));
    const droneBayOverload = Math.max(0, round6(getBayUsedVolume(ITEM_FLAGS.DRONE_BAY) -
        toFiniteNumber(resourceState.droneCapacity, 0)));
    const fighterBayOverload = Math.max(0, round6(getBayUsedVolume(ITEM_FLAGS.FIGHTER_BAY) -
        toFiniteNumber(resourceState.fighterCapacity, 0)));
    return {
        complete: subsystemStatus.complete &&
            invalidRegularSlotItems.length === 0 &&
            turretOverload === 0 &&
            launcherOverload === 0 &&
            cpuOverload === 0 &&
            powerOverload === 0 &&
...
function getEffectTypeRecord(effectID) {
    ...
    const root = getTypeDogmaRoot();
    return ((root.effectTypesByID && root.effectTypesByID[String(numericEffectID)]) ||
        null);
}
function getTypeEffectRecords(typeID) {
    ...
    const effectRecords = Object.freeze([...getTypeDogmaEffects(numericTypeID)]
...
function cloneAttributeMap(source = {}) {
    return Object.fromEntries(Object.entries(source || {}).map(([attributeID, value]) => [
        Number(attributeID),
        Number(value),
    ]));
}
...
function getRequiredSkillTypeIDs(itemOrTypeID) {
    ...
    for (let index = 1; index <= 6; index += 1) {
        const requiredSkillTypeID = toInt(attributeMap[getAttributeIDByNames(`requiredSkill${index}`)], 0);
...
function getPassiveModifierEffectRecords(typeID) {
    return getTypeEffectRecords(typeID).filter((effectRecord) => (PASSIVE_MODIFIER_EFFECT_CATEGORIES.has(toInt(effectRecord && effectRecord.effectCategoryID, 0)) &&
...
function isPassiveModifierSource(item) {
    ...
    return (isEffectivelyOnlineModule(item) &&
...
function buildEffectiveItemAttributeMap(itemOrTypeID, otherItem = null) {
    const typeID = itemOrTypeID && typeof itemOrTypeID === "object"
        ...
        : toInt(itemOrTypeID, 0);
    const sourceTypeID = itemOrTypeID && typeof itemOrTypeID === "object"
...
function getEffectiveItemAttributeValue(itemOrTypeID, ...attributeNames) {
    ...
    const attributeMap = itemOrTypeID && typeof itemOrTypeID === "object"
...
function applyOtherItemModifiersToAttributes(attributes, otherItem) {
    ...
    for (const effectRecord of getTypeEffectRecords(otherItem.typeID)) {
        for (const modifierInfo of effectRecord.modifierInfo || []) {
            ...
            applyDogmaModifier(attributes, modifierInfo.modifiedAttributeID, modifierInfo.operation, otherAttributes[toInt(modifierInfo.modifyingAttributeID, 0)]);
        }
    }
    return attributes;
}
function appendDirectModifierEntries(destination, sourceAttributes, sourceEffects, sourceKind, options = {}) {
    ...
    for (const effectRecord of Array.isArray(sourceEffects) ? sourceEffects : []) {
        for (const modifierInfo of effectRecord.modifierInfo || []) {
            ...
            const value = toFiniteNumber(sourceAttributes && sourceAttributes[modifierInfo.modifyingAttributeID], NaN);
            ...
            destination.push({
                ...
                value,
                stackingPenalized: shouldApplyStackingPenalty(modifierInfo.modifiedAttributeID, stackingPenalized),
            });
        }
    }
}
function appendSelfItemModifierEntries(destination, sourceAttributes, sourceEffects, sourceKind) {
    appendDirectModifierEntries(destination, sourceAttributes, sourceEffects, sourceKind, {
        allowedDomains: new Set(["itemID"]),
        allowedFuncs: new Set(["ItemModifier"]),
        stackingPenalized: false,
    });
}
function appendLocationModifierEntries(destination, sourceAttributes, sourceEffects, sourceKind, moduleItem, options = {}) {
    const allowedDomains = options.allowedDomains instanceof Set
        ? options.allowedDomains
    ...
    for (const effectRecord of Array.isArray(sourceEffects) ? sourceEffects : []) {
        ...
        if (normalizedEffectName === "dronedmgbonus" &&
            ...
            const value = toFiniteNumber(sourceAttributes && sourceAttributes[ATTRIBUTE_DAMAGE_MULTIPLIER_BONUS], NaN);
            if (Number.isFinite(value)) {
                destination.push({
        ...
        const missileDamageAttributeID = DAMAGE_ATTRIBUTE_BY_SKILL_EFFECT_NAME[normalizedEffectName] || 0;
        ...
        if (normalizedEffectName === "selfrof" &&
            ...
            const value = toFiniteNumber(sourceAttributes && sourceAttributes[ATTRIBUTE_ROF_BONUS], NaN);
            if (Number.isFinite(value)) {
                destination.push({
        ...
        for (const modifierInfo of effectRecord.modifierInfo || []) {
            ...
            destination.push({
                ...
                operation: modifierInfo.operation,
                value,
                stackingPenalized: shouldApplyStackingPenalty(modifierInfo.modifiedAttributeID, sourceKind === "fittedModule"),
            });
        }
    }
}
function readShipBaseAttributeMap(shipTypeID, shipItem = null, shipMetadata = null) {
    const baseAttributes = Object.fromEntries(Object.entries(readShipBaseAttributes(shipTypeID))
        .map(([attributeID, value]) => [Number(attributeID), Number(value)])
        .filter(([attributeID, value]) => Number.isInteger(attributeID) && Number.isFinite(value)));
    const resolvedMetadata = shipMetadata || resolveItemByTypeID(toInt(shipTypeID, 0)) || null;
...
function peekSkillEffectiveAttributes(skill) {
    ...
    const level = Math.max(0, toInt(skill && (skill.effectiveSkillLevel ??
    ...
    for (const effect of getTypeDogmaEffects(skillTypeID)) {
        ...
        for (const modifierInfo of effectRecord.modifierInfo) {
            ...
            applyDogmaModifier(attributes, modifierInfo.modifiedAttributeID, modifierInfo.operation, attributes[toInt(modifierInfo.modifyingAttributeID, 0)]);
        }
    }
...
function collectSkillDrivenShipAttributeModifiers(skillMap) {
    ...
    for (const skill of skillMap.values()) {
        ...
        const skillTypeID = toInt(skill && skill.typeID, 0);
        appendDirectModifierEntries(modifierEntries, skillAttributes, getTypeEffectRecords(skillTypeID), "skill");
    }
    return modifierEntries;
}
function applySkillDrivenShipAttributeModifiers(attributes, skillMap) {
    return applyModifierGroups(attributes, collectSkillDrivenShipAttributeModifiers(skillMap));
}
...
function skillHasExplicitShipAttributeModifier(skillTypeID, targetAttributeID) {
    ...
    return getSkillExplicitShipAttributeModifierSet(numericSkillTypeID).has(numericTargetAttributeID);
}
...
function appendSkillFallbackPercentModifier(destination, skillTypeID, level, targetAttributeID, ...sourceAttributeNames) {
    if (skillHasExplicitShipAttributeModifier(skillTypeID, targetAttributeID) ||
        !skillSupportsFallbackShipAttributeModifier(skillTypeID)) {
        return;
    }
    const value = toFiniteNumber(getTypeAttributeValue(skillTypeID, ...sourceAttributeNames), 0) *
        level;
...
function collectShipTypeSelfModifiers(attributes, shipTypeID) {
    const modifierEntries = [];
    appendDirectModifierEntries(modifierEntries, attributes, getTypeEffectRecords(shipTypeID), "ship");
    return modifierEntries;
}
    ...
    for (const skillRecord of skillMap.values()) {
        ...
        for (const effectRecord of getTypeEffectRecords(skillRecord.typeID)) {
            for (const modifierInfo of effectRecord.modifierInfo || []) {
                ...
                applyDogmaModifier(attributes, modifierInfo.modifiedAttributeID, modifierInfo.operation, effectiveSkillAttributes[toInt(modifierInfo.modifyingAttributeID, 0)]);
            }
        }
    }
    return attributes;
}
function buildPassiveModuleAttributeMap(shipItem, moduleItem, skillMap = new Map(), options = {}) {
    ...
    const additionalLocationModifierSources = Array.isArray(options.additionalLocationModifierSources)
        ? options.additionalLocationModifierSources
        : [];
    const additionalShipAttributeModifierEntries = Array.isArray(options.additionalShipAttributeModifierEntries)
    ...
    for (const skillRecord of resolvedSkillMap.values()) {
        appendLocationModifierEntries(modifierEntries, peekSkillEffectiveAttributes(skillRecord), getTypeEffectRecords(skillRecord.typeID), "skill", moduleItem, {
    ...
    const shipTypeID = toInt(shipItem && shipItem.typeID, 0);
    if (shipTypeID > 0) {
        const shipLocationAttributes = collectShipLocationModifierSourceAttributes(shipItem, resolvedSkillMap);
        ...
        appendLocationModifierEntries(modifierEntries, shipLocationAttributes, getTypeEffectRecords(shipTypeID), "ship", moduleItem, { allowedDomains: locationModifierDomains });
    }
    for (const source of additionalLocationModifierSources) {
        if (!source || typeof source !== "object") {
            continue;
        }
        appendLocationModifierEntries(modifierEntries, source.sourceAttributes, source.sourceEffects, String(source.sourceKind || "system"), moduleItem, { allowedDomains: locationModifierDomains });
    }
...
function buildEffectiveFittedModuleAttributeMap(shipItem, moduleItem, skillMap = new Map(), fittedItems = [], options = {}) {
    const attributes = buildPassiveModuleAttributeMap(shipItem, moduleItem, skillMap, options);
    if (!moduleItem) {
        return attributes;
    }
    const modifierEntries = [];
    for (const sourceItem of getPassiveModifierSourceItems(shipItem, fittedItems, options)) {
        if (!itemAppliesPassiveStats(sourceItem) ||
        ...
        appendLocationModifierEntries(modifierEntries, buildPassiveModuleAttributeMap(shipItem, sourceItem, skillMap, options), passiveSourceEffects, "fittedModule", moduleItem);
    }
    if (modifierEntries.length > 0) {
        applyModifierGroups(attributes, modifierEntries);
    }
...
function getEffectiveModuleResourceLoad(shipItem, moduleItem, skillMap = new Map(), fittedItems = [], options = {}) {
    const attributes = buildEffectiveFittedModuleAttributeMap(shipItem, moduleItem, skillMap, fittedItems, options);
    return {
        cpuLoad: round6(toFiniteNumber(attributes[ATTRIBUTE_MODULE_CPU_NEED], getTypeAttributeValue(moduleItem && moduleItem.typeID, "cpuLoad", "cpu"))),
        powerLoad: round6(toFiniteNumber(attributes[ATTRIBUTE_MODULE_POWER_NEED], getTypeAttributeValue(moduleItem && moduleItem.typeID, "powerLoad", "power"))),
    };
}
function collectPassiveShipAttributeModifiers(fittedItems = [], options = {}) {
    ...
    for (const item of getPassiveModifierSourceItems(shipItem, fittedItems, options)) {
        ...
        appendDirectModifierEntries(modifiers, buildPassiveModuleAttributeMap(shipItem, item, skillMap, options), passiveSourceEffects, "fittedModule");
    }
    return modifiers;
}
function getSkillLevel(skillMap, skillTypeID) {
    ...
    return Math.max(0, toInt(skill.effectiveSkillLevel ??
        skill.trainedSkillLevel ??
        skill.skillLevel, 0));
}
function resolveAssumedActivePropulsionEffectState(moduleItem, skillMap = new Map()) {
    ...
    const accelerationControlLevel = getSkillLevel(skillMap, PROPULSION_SKILL_ACCELERATION_CONTROL);
    const moduleAttributes = buildEffectiveItemAttributeMap(moduleItem);
    const speedFactorBase = toFiniteNumber(moduleAttributes[ATTRIBUTE_SPEED_FACTOR], 0);
    const speedFactor = speedFactorBase * (1 + ((5 * accelerationControlLevel) / 100));
    const speedBoostFactor = toFiniteNumber(moduleAttributes[ATTRIBUTE_SPEED_BOOST_FACTOR], 0);
    ...
    return {
        ...
        massAddition: round6(toFiniteNumber(moduleAttributes[ATTRIBUTE_MASS_ADDITION], 0)),
        signatureRadiusBonus: round6(toFiniteNumber(moduleAttributes[ATTRIBUTE_SIGNATURE_RADIUS_BONUS], 0)),
    };
}
function applyAssumedActivePropulsionEffect(attributes, effectState) {
    ...
    attributes[ATTRIBUTE_MAX_VELOCITY] = round6(passiveMaxVelocity * Math.max(speedMultiplier, 0));
    if (effectState.effectName === PROPULSION_EFFECT_MICROWARPDRIVE) {
        const passiveSignatureRadius = toFiniteNumber(attributes[ATTRIBUTE_SIGNATURE_RADIUS], NaN);
        if (Number.isFinite(passiveSignatureRadius)) {
            attributes[ATTRIBUTE_SIGNATURE_RADIUS] = round6(passiveSignatureRadius *
                (1 + (toFiniteNumber(effectState.signatureRadiusBonus, 0) / 100)));
        }
    }
}
function applyAssumedActiveShipModuleEffects(attributes, fittedItems = [], skillMap = new Map()) {
    ...
    applyAssumedActivePropulsionEffect(attributes, resolveAssumedActivePropulsionEffectState(propulsionModule, skillMap));
}
            ...
            switch (operation) {
                ...
                case DOGMA_OP_POST_PERCENT_UNNERFED: {
                    ...
                    for (const entry of entries) {
                        ...
                        if (operation !== DOGMA_OP_POST_PERCENT_UNNERFED &&
                    ...
                    const base = Number.isFinite(currentValue)
                        ...
                        : (operation === DOGMA_OP_POST_PERCENT ||
                            ...
                            ? 0
                            : 1;
                    const directFactor = directFactors.reduce((result, factor) => result * factor, 1);
                ...
                case DOGMA_OP_MOD_ADD: {
                    ...
                    const totalAdd = entries.reduce((sum, entry) => sum + toFiniteNumber(entry.value, 0), 0);
                    attributes[attributeID] = round6(base + totalAdd);
                    break;
                }
                case DOGMA_OP_MOD_SUB: {
                    const base = Number.isFinite(currentValue) ? currentValue : 0;
                    const totalSub = entries.reduce((sum, entry) => sum + toFiniteNumber(entry.value, 0), 0);
...
function synchronizeShipResourceStateSummary(resourceState) {
    ...
    resourceState.fighterCapacity = round6(toFiniteNumber(attributes[ATTRIBUTE_FIGHTER_CAPACITY], 0));
    ...
    resourceState.maxTargetRange = round6(toFiniteNumber(attributes[ATTRIBUTE_MAX_TARGET_RANGE], 0));
    resourceState.maxLockedTargets = round6(toFiniteNumber(attributes[ATTRIBUTE_MAX_LOCKED_TARGETS], 0));
    resourceState.signatureRadius = round6(toFiniteNumber(attributes[ATTRIBUTE_SIGNATURE_RADIUS], 0));
    resourceState.cloakingTargetingDelay = round6(toFiniteNumber(attributes[ATTRIBUTE_CLOAKING_TARGETING_DELAY], 0));
    resourceState.scanResolution = round6(toFiniteNumber(attributes[ATTRIBUTE_SCAN_RESOLUTION], 0));
    resourceState.capacitorCapacity = round6(toFiniteNumber(attributes[ATTRIBUTE_CAPACITOR_CAPACITY], 0));
    resourceState.capacitorRechargeRate = round6(toFiniteNumber(attributes[ATTRIBUTE_CAPACITOR_RECHARGE_RATE], 0));
    resourceState.shieldCapacity = round6(toFiniteNumber(attributes[ATTRIBUTE_SHIELD_CAPACITY], 0));
    resourceState.shieldRechargeRate = round6(toFiniteNumber(attributes[ATTRIBUTE_SHIELD_RECHARGE_RATE], 0));
    resourceState.armorHP = round6(toFiniteNumber(attributes[ATTRIBUTE_ARMOR_HP], 0));
    resourceState.structureHP = round6(toFiniteNumber(attributes[ATTRIBUTE_STRUCTURE_HP], 0));
    resourceState.upgradeCapacity = round6(toFiniteNumber(attributes[ATTRIBUTE_UPGRADE_CAPACITY], 0));
    resourceState[FUEL_BAY_RESOURCE_KEY] = round6(toFiniteNumber(attributes[FUEL_BAY_ATTRIBUTE_ID], 0));
    for (const miningHold of MINING_HOLD_ATTRIBUTE_IDS) {
        resourceState[miningHold.resourceKey] = round6(toFiniteNumber(attributes[miningHold.attributeID], 0));
    }
    return resourceState;
}
function collectSkillFallbackShipAttributeModifiers(skillMap) {
    ...
    for (const skill of skillMap.values()) {
        ...
        appendSkillFallbackPercentModifier(modifierEntries, skillTypeID, level, ATTRIBUTE_CPU_OUTPUT, "cpuOutputBonus", "cpuOutputBonus2");
        appendSkillFallbackPercentModifier(modifierEntries, skillTypeID, level, ATTRIBUTE_POWER_OUTPUT, "powerOutputBonus", "powerEngineeringOutputBonus");
        appendSkillFallbackPercentModifier(modifierEntries, skillTypeID, level, ATTRIBUTE_MAX_VELOCITY, "maxVelocityBonus", "velocityBonus");
        ...
        appendSkillFallbackPercentModifier(modifierEntries, skillTypeID, level, ATTRIBUTE_MAX_TARGET_RANGE, "maxTargetRangeBonus");
        appendSkillFallbackPercentModifier(modifierEntries, skillTypeID, level, ATTRIBUTE_SCAN_RESOLUTION, "scanResolutionBonus");
        appendSkillFallbackPercentModifier(modifierEntries, skillTypeID, level, ATTRIBUTE_CLOAKING_TARGETING_DELAY, "cloakingTargetingDelayBonus");
    }
    return modifierEntries;
}
function applySkillFallbackAttributeBonuses(attributes, skillMap) {
    return applyModifierGroups(attributes, collectSkillFallbackShipAttributeModifiers(skillMap));
}
function buildCharacterTargetingState(charID, options = {}) {
    ...
    const skillMap = options.skillMap instanceof Map
        ...
        : numericCharID > 0
            ...
            : new Map();
    const baseAttributes = Object.fromEntries(Object.entries(getTypeDogmaAttributes(CHARACTER_TYPE_ID))
        .map(([attributeID, value]) => [Number(attributeID), Number(value)])
        .filter(([attributeID, value]) => Number.isInteger(attributeID) && Number.isFinite(value)));
    const sourceAttributes = options.characterAttributes && typeof options.characterAttributes === "object"
    ...
    for (const skill of skillMap.values()) {
        ...
        applyAdditiveModifier(baseAttributes, ATTRIBUTE_MAX_LOCKED_TARGETS, toFiniteNumber(getSkillAttributeValue(skillTypeID, "maxLockedTargetsBonus"), 0) * level);
    }
    return {
        attributes: baseAttributes,
        maxLockedTargets: Math.max(0, Math.trunc(toFiniteNumber(baseAttributes[ATTRIBUTE_MAX_LOCKED_TARGETS], 0))),
    };
}
function buildShipResourceState(charID, shipItem, options = {}) {
    ...
    const validStrategicSubsystemItems = new Set(getValidStrategicCruiserSubsystems(shipTypeID, fittedItems, numericShipID));
    const dogmaFittedItems = isStrategicCruiserType(shipTypeID)
        ? fittedItems.filter((item) => (getRequiredSlotFamily(item && item.typeID) !== "subsystem" ||
            validStrategicSubsystemItems.has(item)))
        : fittedItems;
    ...
    const derivedAttributes = readShipBaseAttributeMap(shipTypeID, shipItem, shipMetadata);
    ...
    const passiveAttributeModifierEntries = Array.isArray(options.passiveAttributeModifierEntries)
        ? options.passiveAttributeModifierEntries
        : collectPassiveShipAttributeModifiers(dogmaFittedItems, {
            shipItem,
            skillMap,
    ...
    const skillFallbackShipAttributeModifierEntries = collectSkillFallbackShipAttributeModifiers(skillMap);
    const skillDrivenShipAttributeModifierEntries = collectSkillDrivenShipAttributeModifiers(skillMap);
    ...
    applyModifierGroups(skillAdjustedShipSelfModifierAttributes, skillDrivenShipAttributeModifierEntries);
    const combinedShipAttributeModifierEntries = [
        ...
        ...collectShipTypeSelfModifiers(skillAdjustedShipSelfModifierAttributes, shipTypeID),
        ...passiveAttributeModifierEntries,
        ...additionalAttributeModifierEntries,
    ];
    applyPassiveShipAttributeModifiers(derivedAttributes, combinedShipAttributeModifierEntries);
    if (options.assumeActiveShipModules === true) {
        applyAssumedActiveShipModuleEffects(derivedAttributes, dogmaFittedItems, skillMap);
    }
    const hardpointCounts = getShipHardpointCounts(shipTypeID, fittedItems, numericShipID);
    const turretHardpoints = hardpointCounts.turret;
    const launcherHardpoints = hardpointCounts.launcher;
    if (isStrategicCruiserType(shipTypeID)) {
        const slotCounts = getShipSlotCounts(shipTypeID, fittedItems, numericShipID);
        derivedAttributes[ATTRIBUTE_LOW_SLOTS] = slotCounts.low;
        derivedAttributes[ATTRIBUTE_MED_SLOTS] = slotCounts.med;
        derivedAttributes[ATTRIBUTE_HIGH_SLOTS] = slotCounts.high;
    }
    let cpuLoad = 0;
    let powerLoad = 0;
    let upgradeLoad = 0;
    for (const item of dogmaFittedItems) {
        if (!item || !isFittedModuleItem(item)) {
            continue;
        }
        if (isEffectivelyOnlineModule(item)) {
            const effectiveResourceLoad = getEffectiveModuleResourceLoad(shipItem, item, skillMap, dogmaFittedItems, {
                additionalLocationModifierSources,
                additionalShipAttributeModifierEntries: implantShipAttributeModifierEntries,
    ...
    const usedTurrets = countMatchingFittedItems(dogmaFittedItems, itemConsumesTurretHardpoint);
    const usedLaunchers = countMatchingFittedItems(dogmaFittedItems, itemConsumesLauncherHardpoint);
    ...
    if (derivedAttributes[ATTRIBUTE_UPGRADE_CAPACITY] === undefined) {
        derivedAttributes[ATTRIBUTE_UPGRADE_CAPACITY] = round6(toFiniteNumber(getShipBaseAttributeValue(shipTypeID, "upgradeCapacity"), 0));
    }
    derivedAttributes[ATTRIBUTE_TURRET_SLOTS_LEFT] = Math.max(0, turretHardpoints - usedTurrets);
    derivedAttributes[ATTRIBUTE_LAUNCHER_SLOTS_LEFT] = Math.max(0, launcherHardpoints - usedLaunchers);
    const resourceState = {
        fittedItems,
        skillMap,
...
function validateFitForShip(charID, shipItem, item, targetFlagID, fittedItems = null) {
    ...
    const candidateFlags = getSlotFlagsForFamily(family, shipTypeID, currentFittedItems, toInt(shipItem && shipItem.itemID, 0));
    if (!candidateFlags.includes(numericTargetFlagID)) {
        return { success: false, errorMsg: "INVALID_FIT_SLOT" };
    }
    if (family === "subsystem") {
        const requiredFlagID = getSubsystemSlotFlag(item.typeID);
        if (!isStrategicCruiserType(shipTypeID) ||
            requiredFlagID <= 0 ||
            requiredFlagID !== numericTargetFlagID) {
            return { success: false, errorMsg: "INVALID_FIT_SLOT" };
        }
        if (!isSubsystemCompatibleWithStrategicCruiser(item, shipTypeID)) {
            return {
                success: false,
                errorMsg: "INVALID_SHIP_TYPE",
                data: {
                    allowedShipTypes: [
                        toInt(getTypeAttributeValue(item.typeID, "fitsToShipType"), 0),
                    ].filter((typeID) => typeID > 0),
                },
            };
        }
    }
    const conflictingItem = currentFittedItems.find((fittedItem) => fittedItem &&
    ...
    if (maxTypeFitted > 0) {
        ...
        const existingTypeCount = countMatchingFittedItems(currentFittedItems, (fittedItem) => toInt(fittedItem.typeID, 0) === moduleTypeID, toInt(item.itemID, 0));
    ...
    if (maxGroupFitted > 0) {
        ...
        const existingGroupCount = countMatchingFittedItems(currentFittedItems, (fittedItem) => toInt(fittedItem.groupID, 0) === moduleGroupID, toInt(item.itemID, 0));
    ...
    if (family === "rig") {
        ...
        const shipUpgradeCapacity = toFiniteNumber(getShipBaseAttributeValue(shipTypeID, "upgradeCapacity"), 0);
        const currentUpgradeLoad = currentFittedItems
            ...
            .reduce((sum, fittedItem) => sum + toFiniteNumber(getEffectiveItemAttributeValue(fittedItem, "upgradeCost"), 0), 0);
        const nextUpgradeLoad = currentUpgradeLoad + toFiniteNumber(getEffectiveItemAttributeValue(item, "upgradeCost"), 0);
    ...
    const hardpointCounts = getShipHardpointCounts(shipTypeID, currentFittedItems, toInt(shipItem && shipItem.itemID, 0));
    if (itemConsumesTurretHardpoint(item)) {
        const turretHardpoints = hardpointCounts.turret;
        const usedTurrets = countMatchingFittedItems(currentFittedItems, itemConsumesTurretHardpoint, toInt(item.itemID, 0));
    ...
    if (itemConsumesLauncherHardpoint(item)) {
        const launcherHardpoints = hardpointCounts.launcher;
        const usedLaunchers = countMatchingFittedItems(currentFittedItems, itemConsumesLauncherHardpoint, toInt(item.itemID, 0));
...
function resolveFitOnlineState(charID, shipItem, moduleItem, fittedItems, options = {}) {
    ...
    const baselineItems = (Array.isArray(fittedItems) ? fittedItems : []).filter((item) => toInt(item && item.itemID, 0) !== moduleID);
    ...
    const cpuOverloaded = toFiniteNumber(resourceState.cpuLoad, 0) >
    ...
    const powerOverloaded = toFiniteNumber(resourceState.powerLoad, 0) >
...
function evaluateModuleChargeCompatibility(moduleTypeID, chargeTypeID) {
    ...
    if (numericModuleTypeID <= 0 || numericChargeTypeID <= 0) {
        return {
            accepted: false,
            rejectionReason: "invalid-type-id",
            maximumQuantity: 0,
        };
    }
    const capacityAttributeID = getAttributeIDByNames("capacity");
    const moduleAttributes = getTypeDogmaAttributes(numericModuleTypeID);
    const chargeMetadata = resolveItemByTypeID(numericChargeTypeID) || null;
    const moduleMetadata = resolveItemByTypeID(numericModuleTypeID) || null;
    const rawCapacity = (capacityAttributeID &&
        toFiniteNumber(moduleAttributes[String(capacityAttributeID)], NaN)) ||
        toFiniteNumber(moduleMetadata && moduleMetadata.capacity, NaN);
    return evaluateChargeCompatibility({
        declaredChargeGroupIDs: getModuleChargeGroupIDs(numericModuleTypeID),
        chargeGroupID: chargeMetadata && chargeMetadata.groupID,
        moduleChargeSize: getTypeAttributeValue(numericModuleTypeID, "chargeSize"),
        chargeSize: getTypeAttributeValue(numericChargeTypeID, "chargeSize"),
        moduleCapacity: rawCapacity,
        chargeVolume: chargeMetadata && chargeMetadata.volume,
    });
}
function getModuleChargeCapacity(moduleTypeID, chargeTypeID) {
    return evaluateModuleChargeCompatibility(moduleTypeID, chargeTypeID).maximumQuantity;
}
...
function isChargeCompatibleWithModule(moduleTypeID, chargeTypeID) {
    return evaluateModuleChargeCompatibility(moduleTypeID, chargeTypeID).accepted;
}
function getTurretLikeModuleItems(charID, shipID) {
    return getFittedModuleItems(charID, shipID).filter((item) => typeHasEffectName(item.typeID, "turretFitted") ||
...
function selectAutoFitFlagForType(shipItem, fittedItems, typeID) {
    ...
    const shipTypeID = toInt(shipItem && shipItem.typeID, 0);
    if (family === "subsystem") {
        const requiredFlagID = getSubsystemSlotFlag(typeID);
        if (requiredFlagID <= 0 ||
            !isStrategicCruiserType(shipTypeID) ||
            !isSubsystemCompatibleWithStrategicCruiser({ typeID }, shipTypeID)) {
            return null;
        }
        const occupied = (Array.isArray(fittedItems) ? fittedItems : []).some((item) => isFittedModuleItem(item) &&
            toInt(item.flagID, 0) === requiredFlagID);
        return occupied ? null : requiredFlagID;
    }
    const candidateFlags = getSlotFlagsForFamily(family, shipTypeID, fittedItems, toInt(shipItem && shipItem.itemID, 0));
    const occupiedFlags = new Set((Array.isArray(fittedItems) ? fittedItems : [])
...
module.exports = {
    ...
    getSubsystemSlotFlag,
    isSubsystemCompatibleWithStrategicCruiser,
    getStrategicCruiserSubsystemStatus,
    getValidStrategicCruiserSubsystems,
    getRequiredSkillRequirements,
    getShipSlotCounts,
    getSlotFlagsForFamily,
    getShipHardpointCounts,
    ...
    // Exported for read-only consumers that need a fitted module's attributes
    // WITH its ship and skill context applied. `buildEffectiveItemAttributeMap`
    // ignores both, so it reports e.g. a Rifter autocannon's damageMultiplier as
    // 3.465 where the true fitted value is 6.5507 — which makes it unusable as a
    // basis for fit-level damage aggregation. Export only; no behaviour change.
    buildEffectiveFittedModuleAttributeMap,
...
//# sourceMappingURL=liveFittingState.js.map