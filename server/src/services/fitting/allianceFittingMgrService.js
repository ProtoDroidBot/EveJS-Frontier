...
const { OWNER_SCOPE, assertSessionCanAccessOwner, assertSessionCanMutateOwner, getOwnerFittingsResponse, saveFitting, saveManyFittings, updateFitting, updateFittingNameAndDescription, deleteFitting, deleteManyFittings, } = require(path.join(__dirname, "../../_secondary/fitting/fittingStore"));
...
class AllianceFittingMgrService extends BaseService {
    ...
    Handle_SaveFitting(args, session) {
        ...
        const result = handleStoreResult(saveFitting(ownerID, args && args[1], OWNER_SCOPE.ALLIANCE), (data) => data);
    ...
    Handle_UpdateFitting(args, session, kwargs) {
        ...
        assertSessionCanMutateOwner(session, ownerID, OWNER_SCOPE.ALLIANCE);
        const updatedFittingID = handleStoreResult(updateFitting(ownerID, fittingID, args && args[1], OWNER_SCOPE.ALLIANCE), (data) => data.fittingID);
    ...
    Handle_SaveManyFittings(args, session) {
        ...
        assertSessionCanMutateOwner(session, ownerID, OWNER_SCOPE.ALLIANCE);
        const mappings = handleStoreResult(saveManyFittings(ownerID, args && args[1], OWNER_SCOPE.ALLIANCE), (data) => data);
    ...
    Handle_DeleteFitting(args, session) {
        ...
        assertSessionCanMutateOwner(session, ownerID, OWNER_SCOPE.ALLIANCE);
        const deletedResult = handleStoreResult(deleteFitting(ownerID, args && args[1], OWNER_SCOPE.ALLIANCE), () => null);
        notifyFittingMutation(session, "OnFittingDeleted", [ownerID, Number(args && args[1]) || 0], { ownerScope: OWNER_SCOPE.ALLIANCE });
    ...
    Handle_DeleteManyFittings(args, session) {
        ...
        assertSessionCanMutateOwner(session, ownerID, OWNER_SCOPE.ALLIANCE);
        const deletedIDs = handleStoreResult(deleteManyFittings(ownerID, args && args[1], OWNER_SCOPE.ALLIANCE), buildDeletedResult);
        notifyFittingMutation(session, "OnManyFittingsDeleted", [ownerID, Array.isArray(deletedIDs && deletedIDs.items) ? deletedIDs.items : []], { ownerScope: OWNER_SCOPE.ALLIANCE });
        return deletedIDs;
    }
    Handle_UpdateNameAndDescription(args, session) {
        const fittingID = args && args[0];
        const ownerID = args && args[1];
        log.debug(`[AllianceFittingMgr] UpdateNameAndDescription(${ownerID}, ${fittingID})`);
        assertSessionCanMutateOwner(session, ownerID, OWNER_SCOPE.ALLIANCE);
        const updatedResult = handleStoreResult(updateFittingNameAndDescription(fittingID, ownerID, args && args[2], args && args[3], OWNER_SCOPE.ALLIANCE), () => null);
...
//# sourceMappingURL=allianceFittingMgrService.js.map