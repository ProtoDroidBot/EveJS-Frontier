...
class CharFittingMgrService extends BaseService {
    ...
    Handle_SaveFitting(args, session) {
        ...
        const result = handleStoreResult(saveFitting(ownerID, args && args[1], OWNER_SCOPE.CHARACTER), (data) => data);
    ...
    Handle_UpdateFitting(args, session, kwargs) {
        ...
        assertSessionCanMutateOwner(session, ownerID, OWNER_SCOPE.CHARACTER);
        const updatedFittingID = handleStoreResult(updateFitting(ownerID, fittingID, args && args[1], OWNER_SCOPE.CHARACTER), (data) => data.fittingID);
    ...
    Handle_SaveManyFittings(args, session) {
        ...
        assertSessionCanMutateOwner(session, ownerID, OWNER_SCOPE.CHARACTER);
        const mappings = handleStoreResult(saveManyFittings(ownerID, args && args[1], OWNER_SCOPE.CHARACTER), (data) => data);
    ...
    Handle_DeleteFitting(args, session) {
        ...
        assertSessionCanMutateOwner(session, ownerID, OWNER_SCOPE.CHARACTER);
        const deletedResult = handleStoreResult(deleteFitting(ownerID, args && args[1], OWNER_SCOPE.CHARACTER), () => null);
        notifyFittingMutation(session, "OnFittingDeleted", [ownerID, Number(args && args[1]) || 0], { ownerScope: OWNER_SCOPE.CHARACTER });
    ...
    Handle_DeleteManyFittings(args, session) {
        ...
        assertSessionCanMutateOwner(session, ownerID, OWNER_SCOPE.CHARACTER);
        const deletedIDs = handleStoreResult(deleteManyFittings(ownerID, args && args[1], OWNER_SCOPE.CHARACTER), buildDeletedResult);
        notifyFittingMutation(session, "OnManyFittingsDeleted", [ownerID, Array.isArray(deletedIDs && deletedIDs.items) ? deletedIDs.items : []], { ownerScope: OWNER_SCOPE.CHARACTER });
        return deletedIDs;
    }
    Handle_UpdateNameAndDescription(args, session) {
        ...
        assertSessionCanMutateOwner(session, ownerID, OWNER_SCOPE.CHARACTER);
        const updatedResult = handleStoreResult(updateFittingNameAndDescription(fittingID, ownerID, args && args[2], args && args[3], OWNER_SCOPE.CHARACTER), () => null);
...
//# sourceMappingURL=charFittingMgrService.js.map