...
function buildSaveManyResult(mappings: any[] = []) {
  return buildList(
...
function buildDeletedResult(fittingIDs: any[] = []) {
  return buildList(
...
function notifyFittingMutation(session, notificationName, payload: any[] = [], options: Record<string, any> = {}) {
  if (!notificationName) {
    return 0;
  }
