...
const { ITEM_FLAGS, findShipItemById, listContainerItems, moveItemToLocation, } = require(path.join(__dirname, "../inventory/itemStore"));
const { isChargeCompatibleWithModule, isFittedChargeItem, isFittedModuleItem, } = require(path.join(__dirname, "./liveFittingState"));
...
function ensureShipFittingInventoryParity(characterID, shipItemOrID, options = {}) {
    ...
    const moduleByFlag = new Map(fittedItems
    ...
    for (const chargeItem of fittedItems.filter((item) => isFittedChargeItem(item))) {
        ...
        const moveResult = moveItemToLocation(chargeItem.itemID, shipID, ITEM_FLAGS.CARGO_HOLD);
    ...
    if (issues.length > 0 && options.logFailures !== false) {
        log.warn(`[FittingIntegrity] active ship fitting parity failed char=${numericCharacterID} ` +
...
//# sourceMappingURL=fittingIntegrity.js.map