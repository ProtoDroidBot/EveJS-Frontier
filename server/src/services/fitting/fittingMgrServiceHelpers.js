...
const { buildKeyVal, buildList, extractDictEntries, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const sessionRegistry = require(path.join(__dirname, "../chat/sessionRegistry"));
const { OWNER_SCOPE, throwFittingError, } = require(path.join(__dirname, "../../_secondary/fitting/fittingStore"));
...
function buildSaveManyResult(mappings = []) {
    return buildList((Array.isArray(mappings) ? mappings : []).map((mapping) => buildKeyVal([
...
function buildDeletedResult(fittingIDs = []) {
    return buildList((Array.isArray(fittingIDs) ? fittingIDs : []).map((fittingID) => toInt(fittingID, 0)));
}
...
function notifyFittingMutation(session, notificationName, payload = [], options = {}) {
    ...
    if (ownerScope && ownerID > 0) {
        for (const candidateSession of sessionRegistry.getSessions()) {
            if (getSessionOwnerID(candidateSession, ownerScope) === ownerID &&
...
//# sourceMappingURL=fittingMgrServiceHelpers.js.map