...
function evaluateCumulativeCapacity({ capacity, used, requiredVolume, epsilon = 1e-7, }) {
    const normalizedCapacity = Math.max(0, Number(capacity) || 0);
    const normalizedUsed = Math.max(0, Number(used) || 0);
    const normalizedRequiredVolume = Math.max(0, Number(requiredVolume) || 0);
    const free = Math.max(0, normalizedCapacity - normalizedUsed);
    return {
        success: normalizedRequiredVolume <=
...
//# sourceMappingURL=cargoLootAuthority.js.map