...
class InventoryAssetHoldingMgrService extends BaseService {
    ...
    Handle_set_item_failure_enabled(args, session) {
        ...
        state.itemFailureEnabled = normalizeBoolean(Array.isArray(args) ? args[0] : undefined, state.itemFailureEnabled);
        log.debug(`[InventoryAssetHoldingMgr] Compatibility stub set item failure ` +
    ...
    Handle_set_item_validation_enabled(args, session) {
        const state = this._getState(session);
        state.itemValidationEnabled = normalizeBoolean(Array.isArray(args) ? args[0] : undefined, state.itemValidationEnabled);
        log.debug(`[InventoryAssetHoldingMgr] Compatibility stub set item validation ` +
            `char=${getSessionCharacterID(session) || "?"} enabled=${state.itemValidationEnabled}`);
        return null;
    }
}
module.exports = InventoryAssetHoldingMgrService;
//# sourceMappingURL=inventoryAssetHoldingMgrService.js.map