...
function classifyShipHullClass(groupName, name = "") {
    ...
    if (normalizedGroup.includes("frigate") ||
...
function classifyWreckHullClass(name) {
    ...
    if (normalizedName.includes("jump freighter") ||
        normalizedName.includes("freighter")) {
        return "Freighter";
    }
    if (normalizedName.includes("mining barge") ||
        normalizedName.includes("barge") ||
        normalizedName.includes("exhumer")) {
        return "Mining Barge";
    }
    if (normalizedName.includes("industrial") ||
    ...
    if (normalizedName.includes("frigate") ||
...
function dedupeAndSortRadii(indexMap) {
    for (const [key, radii] of indexMap.entries()) {
        indexMap.set(key, [...new Set(radii)]
...
function loadShipRadiusIndex() {
    ...
    for (const shipRow of shipRows) {
        ...
        const hullClass = classifyShipHullClass(shipRow && shipRow.groupName, shipRow && shipRow.name);
...
function resolveMatchingWreckageRadius(name) {
    ...
    const radius = toFiniteNumber(lookup && lookup.success && lookup.match && lookup.match.radius, 0);
...
function resolveRepresentativeShipRadius(name) {
    ...
    if (raceID > 0) {
        const raceSpecificRadius = selectMedianRadius(index.byRaceAndHullClass.get(`${raceID}:${hullClass}`));
...
function resolveGenericWreckProfileRadius(itemType) {
    ...
    return toFiniteNumber(CLIENT_GENERIC_WRECK_PROFILE_RADII_METERS[wreckGraphicProfile], 0);
}
function resolveRuntimeWreckRadius(itemType, fallback = 0) {
    const itemName = String(itemType && (itemType.itemName || itemType.name) || "").trim();
...
function resolveRuntimeWreckStructureFallbackHP(itemType, fallback = 0) {
    return Math.max(DEFAULT_RUNTIME_WRECK_RADIUS_METERS, toFiniteNumber(resolveRuntimeWreckRadius(itemType, fallback), 0));
}
module.exports = {
...
//# sourceMappingURL=wreckRadius.js.map