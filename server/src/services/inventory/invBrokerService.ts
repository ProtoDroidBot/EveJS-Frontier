...
} = require(path.join(__dirname, "./cargoLootAuthority"));
...
} = require(path.join(__dirname, "../frontier/refugeFittingRuntime"));
const {
  isShipServiceFlag,
...
const STRUCTURE_UNANCHOR_HULL_MARKER_PATTERN =
  /^evejs:structure-unanchor:hull:(\d+)$/;
const STRUCTURE_UNANCHOR_CONTAINER_MARKER_PATTERN =
  /^evejs:structure-unanchor:(?:fittings:(\d+)(?::core-complete)?|assets:(\d+):owner:\d+|office:(\d+):office:\d+)$/;

function isUnanchoredStructureHullInventoryRecord(item) {
  const markerMatch = STRUCTURE_UNANCHOR_HULL_MARKER_PATTERN.exec(
...
function isDirectSpaceUnanchoredStructureHullInventoryRecord(item) {
  ...
  return Boolean(
    isUnanchoredStructureHullInventoryRecord(item) &&
    locationID > 0 &&
...
function getStructureUnanchorContainerSourceStructureID(item) {
  const markerMatch = STRUCTURE_UNANCHOR_CONTAINER_MARKER_PATTERN.exec(
    String(item && item.customInfo || ""),
  );
  if (!markerMatch) {
    return 0;
...
class InvBrokerService extends BaseService {
  ...
  _isUnanchoredStructureHullInventoryID(inventoryID) {
    ...
    return (
      ...
      isUnanchoredStructureHullInventoryRecord(
  ...
  _throwUnanchoredStructureHullInventoryError() {
    throwWrappedUserError("CustomNotify", {
      notify:
        "An unanchored structure hull is a scoopable item, not a cargo container. " +
  ...
  _getPendingStructureUnanchorContainerAncestor(itemOrItemID) {
    ...
    while (currentItem) {
      ...
      const structureID =
        getStructureUnanchorContainerSourceStructureID(currentItem);
      if (
        structureID > 0 &&
  ...
  _isPendingStructureUnanchorContainerInventoryID(inventoryID) {
    return Boolean(
      this._getPendingStructureUnanchorContainerAncestor(inventoryID),
    );
  }
  ...
  _throwPendingStructureUnanchorContainerInventoryError() {
    throwWrappedUserError("CustomNotify", {
      notify:
        "This unanchor container is still being finalized. " +
  ...
  _describeValue(value, depth = 0) {
    ...
    if (typeof value === "object") {
      ...
      for (const [key, entryValue] of Object.entries<any>(value)) {
  ...
  _traceInventory(method, session, payload: Record<string, any> = {}) {
    const entry = {
      method,
  ...
  _buildBayDogmaBootstrapKey(items: any[] = [], attributesByItemID = new Map()) {
    ...
    return normalizedItems
      .map((item) => {
        ...
        const attributeKey = Object.entries<any>(attributes)
  ...
  _primeInSpaceBayDogmaItems(session, boundContext, flagID, items: any[] = []) {
    if (
      !session ||
      !session._space ||
  ...
  _shouldPrimeLoginShipInventoryBootstrap(session, boundContext, options: Record<string, any> = {}) {
    void session;
    void boundContext;
    void options;
    return false;
  ...
  _makeBoundSubstruct(context, session = null) {
    if (
      this._isUnanchoredStructureHullInventoryID(
        context && context.inventoryID,
      )
    ) {
    ...
    if (
      this._isPendingStructureUnanchorContainerInventoryID(
        context && context.inventoryID,
      )
    ) {
  ...
  _normalizeCommodityDict(value) {
    ...
    for (const [rawKey, rawQuantity] of Object.entries<any>(rawValue)) {
  ...
  _refundPIWalletDebits(characterID, debitResults: any[] = [], description = "PI tax refund") {
  ...
  _normalizeFitFittingItemsByType(value) {
    ...
    for (const [rawTypeID, rawItemIDs] of Object.entries<any>(source)) {
      this._appendFitFittingItemsByType(byType, rawTypeID, rawItemIDs);
    }
  ...
  _normalizeFitFittingModulesByFlag(value) {
    ...
    return Object.entries<any>(source)
  ...
  _validateMoonMaterialBayDestination(session, destination) {
    ...
    return { success: false as const, errorMsg: "OUTPUT_ONLY" };
  }
  ...
  _validateStructureCoreInstallMove(item, destination) {
    ...
    if (!structure || !this._isStructureCoreInstallMove(item, destination)) {
      return { success: false as const, errorMsg: "NoSpaceForThat" };
    }
    ...
    return { success: true as const, structure, expectedCoreTypeID };
  }
  ...
  _syncStructureServiceModuleMove(session, sourceItem, destination) {
    const touchedStructureIDs = new Set<any>();
  ...
  _emitFleetLootEvents(session, fleetLootEntries: any[] = []) {
  ...
  _validateSpaceContainerScopeTransferAccess(
    session,
    sourceContainerID,
  ...
  _throwMobileDepotFittingAccessError(errorMsg) {
    ...
    const notifyByError = {
      ...
      MOBILE_DEPOT_FITTING_SERVICE_NOT_FOUND:
        `You must be within ${MOBILE_DEPOT_FITTING_RANGE_METERS} meters of an active Mobile Depot or ${DEFAULT_REFUGE_FITTING_RANGE_METERS} meters of an online Refuge you own to refit in space.`,
      MOBILE_DEPOT_FITTING_SERVICE_NOT_AVAILABLE:
        `You must be within ${MOBILE_DEPOT_FITTING_RANGE_METERS} meters of an active Mobile Depot or ${DEFAULT_REFUGE_FITTING_RANGE_METERS} meters of an online Refuge you own to refit in space.`,
      REFUGE_FITTING_SERVICE_NOT_FOUND:
        `You must be within ${MOBILE_DEPOT_FITTING_RANGE_METERS} meters of an active Mobile Depot or ${DEFAULT_REFUGE_FITTING_RANGE_METERS} meters of an online Refuge you own to refit in space.`,
      REFUGE_FITTING_SERVICE_NOT_AVAILABLE:
        `You must be within ${MOBILE_DEPOT_FITTING_RANGE_METERS} meters of an active Mobile Depot or ${DEFAULT_REFUGE_FITTING_RANGE_METERS} meters of an online Refuge you own to refit in space.`,
    };
  ...
  _isSpaceLootMultiAddSource(session, sourceLocationID) {
    const numericSourceLocationID = this._normalizeInventoryId(
      sourceLocationID,
      0,
    );
    ...
    const metadata = getItemMetadata(
      sourceContainer.typeID,
      sourceContainer.itemName,
    );
    const sourceCategoryID = this._normalizeInventoryId(
      sourceContainer.categoryID,
      0,
    );
    const sourceGroupID = this._normalizeInventoryId(
      sourceContainer.groupID,
      0,
    );
    const metadataCategoryID = this._normalizeInventoryId(
      metadata && metadata.categoryID,
      0,
    );
    const metadataGroupID = this._normalizeInventoryId(
      metadata && metadata.groupID,
      0,
    );
    const isMobileTractorUnit =
      (sourceCategoryID === CATEGORY_DEPLOYABLE &&
  ...
  _preflightSpaceLootMultiAddCapacity(
    session,
    boundContext,
    itemIDs,
    sourceLocationID,
    ...
    if (
      !this._isSpaceLootMultiAddSource(session, sourceLocationID) ||
      !Array.isArray(itemIDs) ||
      itemIDs.length <= 0
    ) {
      return { handled: false, success: true as const };
    }
    ...
    for (const itemID of itemIDs) {
      ...
      if (
        !this._isTransferSourceLocationMatch(
          sourceItemDescriptor,
          sourceLocationID,
        )
      ) {
    ...
    if (requiredVolume <= 0 || !representativeItem) {
      return { handled: true, success: true as const };
    }
    ...
    const capacity =
      Number(
        (capacityEntries.find(([key]) => key === "capacity") || [])[1],
      ) || 0;
    const used =
      Number((capacityEntries.find(([key]) => key === "used") || [])[1]) || 0;
  ...
  _emitInventoryMoveChanges(session, changes: any[] = [], options: Record<string, any> = {}) {
  ...
  _emitStripFittingMoveChanges(session, changes: any[] = []) {
    const normalizedChanges = (Array.isArray(changes) ? changes : [])
  ...
  _emitMtuSpaceComponentLootEvents(session, sourceLocationID, destination, lootedItems: any[] = []) {
    if (!session || typeof session.sendNotification !== "function") {
      return false;
    }
    const shipID = this._normalizeInventoryId(
  ...
  _emitPlayerJetcanLootEvents(session, sourceLocationID, destination, lootedItems: any[] = []) {
    if (!session || typeof session.sendNotification !== "function") {
      return false;
    }
    const shipID = this._normalizeInventoryId(
  ...
  _emitCorpStructureHangarMoveChanges(session, changes: any[] = []) {
  ...
  _primeDroneBayDogmaForFittingChanges(session, changes: any[] = []) {
    if (!Array.isArray(changes) || changes.length === 0) {
      return false;
    }

    const shipIDs = new Set<any>();
  ...
  _emitFittingMoveChanges(session, changes: any[] = []) {
    ...
    const deduped: any[] = [];
    const seenRowDeltas = new Set<any>();
  ...
  _maybeCompleteMaterializedDungeonSitesForSources(session, sourceContainerIDs: any[] = []) {
  ...
  _refreshDockedFittingState(session, changes: any[] = []) {
    if (
      !session ||
      !isDockedSession(session) ||
  ...
  _refreshBallparkShipPresentation(session, changes: any[] = []) {
    if (!session || !session._space) {
      return;
    }
  ...
  _refreshBallparkInventoryPresentation(session, changes: any[] = []) {
    ...
    const affectedEntityIDs = new Set<any>();
  ...
  _validateStructureReactionServiceModuleFit(item, destination) {
    ...
    const structure = this._getStructureForInventoryID(destination.locationID);
    if (!structure) {
      return { success: false as const, errorMsg: "STRUCTURE_NOT_FOUND" };
    }
  ...
  _filterTopLevelTrashItemIDs(itemIDs: any[] = []) {
    const normalizedItemIDs = this._normalizeItemIdList(itemIDs);
    const selected = new Set<any>(normalizedItemIDs);
    const topLevelIDs: any[] = [];
  ...
  _buildInventoryItemOverrides(session, itemRecord) {
    ...
    return {
      ...
      customInfo:
        itemRecord.customInfo === undefined ? "" : itemRecord.customInfo,
      singleton,
      stacksize,
    };
  }
  ...
  _buildInvRow(session, overrides: Record<string, any> = {}) {
  ...
  _buildInvItem(session, overrides: Record<string, any> = {}) {
  ...
  _itemOverridesFromId(session, itemID) {
    ...
    if (shipRecord) {
      return {
        ...
        customInfo:
          shipRecord.customInfo === undefined ? "" : shipRecord.customInfo,
    ...
    if (genericItem) {
      return {
        ...
        customInfo:
          genericItem.customInfo === undefined ? "" : genericItem.customInfo,
  ...
  _buildInventoryRemoteList(itemOverrides: any[] = []) {
    return {
      type: "list",
  ...
  _buildInventoryRemoteSet(itemOverrides: any[] = []) {
  ...
  _buildInventoryPackedRow(overrides: Record<string, any> = {}) {
    const fields = {
  ...
  _buildInvKeyVal(session, overrides: Record<string, any> = {}) {
    const row = this._buildInvRow(session, overrides);
    const [
      itemID,
      typeID,
  ...
  Handle_ValidateItemListCanBeOpened(args, session) {
    ...
    if (
      itemIDs.some((itemID) =>
        this._isUnanchoredStructureHullInventoryID(itemID),
      )
    ) {
      this._throwUnanchoredStructureHullInventoryError();
    }
    if (
      itemIDs.some((itemID) =>
        this._isPendingStructureUnanchorContainerInventoryID(itemID),
      )
    ) {
      this._throwPendingStructureUnanchorContainerInventoryError();
    }
  ...
  Handle_List(args, session, kwargs) {
    ...
    const dockedShipDefaultList = Boolean(
      boundContext &&
      boundContext.kind === "shipInventory" &&
      requestedFlag === null &&
      this._getStationId(session) > 0
    );
    const controlledStructureDefaultList = Boolean(
      boundContext &&
      boundContext.kind === "controlledStructureInventory" &&
      requestedFlag === null
    );
    ...
    const shouldReturnPythonSet =
      (itemOverrides.length === 0 && requestedFlag !== null && requestedFlag !== undefined) ||
      dockedShipDefaultList;
  ...
  Handle_ListByFlags(args, session, kwargs) {
    ...
    const seenItemIds = new Set<any>();
    const itemOverrides: any[] = [];
    const dogmaPrimeItems: any[] = [];
  ...
  async Handle_ImportExportWithPlanet(args, session) {
    ...
    try {
      ...
      for (const [typeID, quantity] of Object.entries<any>(preview.importCommodities)) {
      ...
      for (const [typeID, quantity] of Object.entries<any>(preview.exportCommodities)) {
  ...
  Handle_GetItem(args, session) {
    ...
    const numericItemID = this._normalizeInventoryId(itemID);
    if (
      this._isPendingStructureUnanchorContainerInventoryID(numericItemID)
    ) {
      this._throwPendingStructureUnanchorContainerInventoryError();
    }
  ...
  Handle_TrashItems(args, session) {
    ...
    if (
      inventoryItems.some((item) =>
        this._getPendingStructureUnanchorContainerAncestor(item),
      )
    ) {
      this._throwPendingStructureUnanchorContainerInventoryError();
    }
  ...
  Handle_GetContainerContents(args, session) {
    ...
    const scopeAccessError = this._getSpaceContainerScopeAccessError(
      session,
      numericContainerID,
    );
    ...
    if (
      this._isPendingStructureUnanchorContainerInventoryID(numericContainerID)
    ) {
      this._throwPendingStructureUnanchorContainerInventoryError();
    }
    if (mobileDepotAccessError) {
    ...
    const items =
      mobileDepotAccessError || scopeAccessError
        ? []
        :
  ...
  Handle_MultiMerge(args, session, kwargs) {
    ...
    if (
      this._isPendingStructureUnanchorContainerInventoryID(sourceContainerID)
    ) {
      this._throwPendingStructureUnanchorContainerInventoryError();
    }
    for (const op of ops) {
      ...
      if (
        this._getPendingStructureUnanchorContainerAncestor(sourceItem) ||
  ...
  Handle_Add(args, session, kwargs) {
    ...
    const spaceContainerScopeAccess =
      this._validateSpaceContainerScopeTransferAccess(
        session,
        sourceLocationID,
        destination.locationID,
      );
  ...
  Handle_MultiAdd(args, session, kwargs) {
    ...
    const itemIDs = [
      ...new Set<any>(
        this._normalizeItemIdList(args && args.length > 0 ? args[0] : []),
      ),
    ];
    const sourceLocationID = this._normalizeInventoryId(
      args && args.length > 1 ? args[1] : 0,
    ...
    const lootCapacityPreflight = this._preflightSpaceLootMultiAddCapacity(
      session,
      boundContext,
      itemIDs,
      sourceLocationID,
    ...
    if (lootCapacityPreflight.handled && !lootCapacityPreflight.success) {
      log.debug(
        `[InvBroker] MultiAdd rejected loot batch source=${sourceLocationID} destination=${this._getShipId(session)}:${ITEM_FLAGS.CARGO_HOLD} error=${lootCapacityPreflight.errorMsg} required=${lootCapacityPreflight.requiredVolume} free=${lootCapacityPreflight.free}`,
      );
      if (
        lootCapacityPreflight.errorMsg === "SOURCE_ITEM_NOT_FOUND" ||
        lootCapacityPreflight.errorMsg === "SOURCE_LOCATION_MISMATCH"
      ) {
        return null;
      }
      throwWrappedUserError(
        lootCapacityPreflight.errorMsg,
        this._buildCapacityMoveUserErrorValues(
          lootCapacityPreflight,
          lootCapacityPreflight.item,
        ),
      );
    }
    ...
    for (const itemID of itemIDs) {
      ...
      if (
        ...
        isDirectSpaceUnanchoredStructureHullInventoryRecord(
          sourceItemDescriptor.item,
        )
      ) {
        this._throwUnanchoredStructureHullInventoryError();
      ...
      if (
        ...
        this._getPendingStructureUnanchorContainerAncestor(
          sourceItemDescriptor.item,
        )
      ) {
        this._throwPendingStructureUnanchorContainerInventoryError();
    ...
    for (const itemID of itemIDs) {
      ...
      if (!spaceContainerScopeAccess.success) {
        log.debug(
          `[InvBroker] MultiAdd rejected itemID=${itemID} spaceContainer=${spaceContainerScopeAccess.containerID} error=${spaceContainerScopeAccess.errorMsg}`,
        );
        if (movedCount > 0) {
          continue;
        }
  ...
  Handle_FitFitting(args, session, kwargs) {
    ...
    if (
      this._isPendingStructureUnanchorContainerInventoryID(sourceLocationID)
    ) {
      this._throwPendingStructureUnanchorContainerInventoryError();
    }
    for (const candidateItemIDs of itemsByType.values()) {
      if (
        candidateItemIDs.some((itemID) =>
          this._isPendingStructureUnanchorContainerInventoryID(itemID),
        )
      ) {
        this._throwPendingStructureUnanchorContainerInventoryError();
