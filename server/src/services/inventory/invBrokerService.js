...
const { getShipFittingSnapshot, } = require(path.join(__dirname, "../../_secondary/fitting/fittingRuntime"));
const { clearModuleFromBanksAndNotify, getMasterModuleID: getWeaponBankMasterModuleID, notifyWeaponBanksChanged, getShipWeaponBanks, } = require(path.join(__dirname, "../moduleGrouping/moduleGroupingRuntime"));
const { MINING_SHIP_BAY_FLAGS, isItemTypeAllowedInHoldFlag, getShipHoldCapacityByFlag, } = require(path.join(__dirname, "../mining/miningInventory"));
const { isFuelBayFlag, isStructureFuelBayFlag, isFuelBayCompatibleItem, isStructureFuelBayCompatibleItem, getFuelBayCapacity, ICE_PRODUCT_GROUP_ID, STRUCTURE_FUEL_BAY_FLAG, TYPE_UPWELL_AUTO_MOON_MINER, } = require(path.join(__dirname, "./fuelBayInventory"));
const { SPECIAL_SHIP_HOLD_FLAGS, isSpecialShipHoldFlag, isGenericSpecialShipHoldFlag, isSpecialShipHoldItemAllowed, getSpecialShipHoldCapacity, } = require(path.join(__dirname, "./specialShipHoldRegistry"));
const { canLoadFighterTypeIntoHostTube, isDroneItemRecord, isFighterItemRecord, isFighterTubeFlag, } = require(path.join(__dirname, "../fighter/fighterInventory"));
...
const { maybeExpireEmptySpaceContainer, } = require(path.join(__dirname, "../ship/jettisonRuntime"));
const { evaluateCumulativeCapacity, isDisposableJetcanRecord, } = require(path.join(__dirname, "./cargoLootAuthority"));
const dungeonUniverseSiteService = require(path.join(__dirname, "../dungeon/dungeonUniverseSiteService"));
const { DEFAULT_STATION, getStationRecord, } = require(path.join(__dirname, "../_shared/stationStaticData"));
const structureState = require(path.join(__dirname, "../structure/structureState"));
const { STRUCTURE_FAMILY, STRUCTURE_SERVICE_ID, STRUCTURE_STATE, } = require(path.join(__dirname, "../structure/structureConstants"));
const { isStructureServiceSlotFlag, syncStructureServiceModuleState, tryAutoOnlineFittedStructureServiceModule, } = require(path.join(__dirname, "../structure/structureServiceModules"));
const { buildCrpAccessDeniedInsufficientRolesValues, characterCanDisableStructureServiceModule, isStructureReactionServiceModuleType, STRUCTURE_SETTING_ID, } = require(path.join(__dirname, "../structure/structureServiceAuthority"));
const { canTakeFromOwnerLocation, } = require(path.join(__dirname, "../industry/industryAccess"));
const { characterHasStructureSetting, characterHasStructureService, } = require(path.join(__dirname, "../structure/structurePayloads"));
const { cancelIndustryJobsForRemovedServiceModule, } = require(path.join(__dirname, "../structure/structureIndustryJobs"));
const { getCharacterSkills, SKILL_FLAG_ID, } = require(path.join(__dirname, "../skills/skillState"));
const { describeSessionHydrationState, } = require(path.join(__dirname, "../chat/commandSessionEffects"));
const { getDockedLocationID, isDockedSession, } = require(path.join(__dirname, "../structure/structureLocation"));
const { buildDict, buildList, buildObjectEx1, buildPackedRow, buildPackedRowDescriptor, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { JOURNAL_ENTRY_TYPE, adjustCharacterBalance, buildNotEnoughMoneyUserErrorValues, getCharacterWallet, } = require(path.join(__dirname, "../account/walletState"));
const planetCostCalculator = require(path.join(__dirname, "../planet/planetCostCalculator"));
...
const STRUCTURE_UNANCHOR_HULL_MARKER_PATTERN = /^evejs:structure-unanchor:hull:(\d+)$/;
const STRUCTURE_UNANCHOR_CONTAINER_MARKER_PATTERN = /^evejs:structure-unanchor:(?:fittings:(\d+)(?::core-complete)?|assets:(\d+):owner:\d+|office:(\d+):office:\d+)$/;
function isUnanchoredStructureHullInventoryRecord(item) {
    const markerMatch = STRUCTURE_UNANCHOR_HULL_MARKER_PATTERN.exec(String(item && item.customInfo || ""));
    return Boolean(item &&
        markerMatch &&
        Number(item.typeID) > 0 &&
        Number(markerMatch[1]) > 0 &&
        Number(item.categoryID) === STRUCTURE_CATEGORY_ID &&
        Number(item.singleton) === 1 &&
        Number(item.launcherID) === Number(markerMatch[1]));
}
function isDirectSpaceUnanchoredStructureHullInventoryRecord(item) {
    const locationID = Number(item && item.locationID) || 0;
    return Boolean(isUnanchoredStructureHullInventoryRecord(item) &&
        locationID > 0 &&
        Number(item.flagID) === 0 &&
        item.spaceState &&
        Number(item.spaceState.systemID) === locationID);
}
function getStructureUnanchorContainerSourceStructureID(item) {
    const markerMatch = STRUCTURE_UNANCHOR_CONTAINER_MARKER_PATTERN.exec(String(item && item.customInfo || ""));
    if (!markerMatch) {
        return 0;
    }
    return markerMatch
...
class InvBrokerService extends BaseService {
    ...
    _isUnanchoredStructureHullInventoryID(inventoryID) {
        const numericInventoryID = this._normalizeInventoryId(inventoryID, 0);
        return (numericInventoryID > 0 &&
            isUnanchoredStructureHullInventoryRecord(findItemById(numericInventoryID)));
    }
    _throwUnanchoredStructureHullInventoryError() {
        throwWrappedUserError("CustomNotify", {
            notify: "An unanchored structure hull is a scoopable item, not a cargo container. " +
                "While it is in space, move within range and use Scoop to Cargo Hold.",
        });
    }
    _getPendingStructureUnanchorContainerAncestor(itemOrItemID) {
        let currentItem = itemOrItemID && typeof itemOrItemID === "object"
            ? itemOrItemID
        ...
        while (currentItem) {
            ...
            if (currentItemID > 0 && seen.has(currentItemID)) {
                break;
            }
            if (currentItemID > 0) {
                seen.add(currentItemID);
            }
            const structureID = getStructureUnanchorContainerSourceStructureID(currentItem);
            if (structureID > 0 &&
            ...
            if (parentID <= 0 || seen.has(parentID)) {
                break;
            }
    ...
    _isPendingStructureUnanchorContainerInventoryID(inventoryID) {
        return Boolean(this._getPendingStructureUnanchorContainerAncestor(inventoryID));
    }
    _throwPendingStructureUnanchorContainerInventoryError() {
        throwWrappedUserError("CustomNotify", {
            notify: "This unanchor container is still being finalized. " +
                "Wait for the structure unanchor to complete before opening or moving it.",
        });
    }
    ...
    _getCorporationId(session) {
        return ((session && (session.corporationID || session.corpid)) ||
            0);
    }
    _getSessionCorpRoleMask(session) {
        ...
        return [
            ...
            session.rolesAtAll,
            session.corpRole,
        ].reduce((mask, roleValue) => mask | toRoleMaskBigInt(roleValue, 0n), 0n);
    }
    ...
    _getCorporationOfficeAtLocation(session, locationID = null) {
        ...
        return getCorporationOffices(corporationID).find((office) => this._normalizeInventoryId(office && office.stationID, 0) === numericLocationID &&
            !office.impounded) || null;
    }
    _getCorporationOfficeForItemLocation(session, locationID = null) {
        ...
        return getCorporationOffices(corporationID).find((office) => {
            ...
            return [
                ...
                office.itemID,
            ].some((candidateID) => this._normalizeInventoryId(candidateID, 0) === numericLocationID);
        }) || null;
    }
    _resolveCorporationOfficeForInventory(session, officeOrLocationID = null) {
        return (this._getCorporationOffice(session, officeOrLocationID) ||
    ...
    _getGenericContainerContentsOwnerID(session, containerRecord) {
        ...
        const categoryID = this._normalizeInventoryId(containerRecord.categoryID ?? typeRecord.categoryID, 0);
    ...
    _getShipId(session) {
        ...
        return ((activeShip && activeShip.shipID) ||
    ...
    _getShipTypeId(session) {
        const charId = this._getCharacterId(session);
        const activeShip = getActiveShipRecord(charId);
        const shipTypeID = activeShip ? activeShip.shipTypeID : (session && Number.isInteger(session.shipTypeID) ? session.shipTypeID : null);
    ...
    _summarizeInventoryRowsForLog(items, limit = 12) {
        ...
        for (const item of safeItems) {
            ...
            if (summary.preview.length < limit) {
                summary.preview.push(`${itemID}:${flagID}:${typeID}:${groupID}:${categoryID}:${quantity}`);
            }
        }
        return summary;
    }
    ...
    _rememberSessionBoundContext(session, oidString, context, boundAtFileTime) {
        ...
        const inventoryID = this._normalizeInventoryId(context && context.inventoryID, 0);
        const flagID = context && context.flagID !== null && context.flagID !== undefined
            ? this._normalizeInventoryId(context.flagID, 0)
            : "all";
        const kind = context && context.kind ? context.kind : "inventory";
        session._boundObjectState[`invbroker:${kind}:${inventoryID || "unknown"}:${flagID}:${oidString}`] = {
            objectID: oidString,
            boundAtFileTime: boundAtFileTime || BigInt(Date.now()) * 10000n + 116444736000000000n,
        };
    }
    ...
    _hasLoginInventoryBootstrapPending(session) {
        ...
        return (session._loginInventoryBootstrapPending === true ||
            (session._space &&
    ...
    _resolveDroneBayDogmaAttributes(session, item, boundContext) {
        ...
        const shipID = this._normalizeInventoryId(item.locationID ||
            (boundContext && boundContext.inventoryID) ||
            (session && session._space && session._space.shipID) ||
            this._getShipId(session), 0);
        const charID = this._normalizeInventoryId(item.ownerID || this._getCharacterId(session), 0);
        if (shipID <= 0 || charID <= 0) {
            return null;
        }
        const activeShip = getActiveShipRecord(charID);
        const shipRecord = findCharacterShip(charID, shipID) ||
            (activeShip &&
        ...
        const systemID = this._normalizeInventoryId((session && (session.solarsystemid2 ||
        ...
        const controllerEntity = {
            kind: "ship",
            itemID: shipID,
            typeID: this._normalizeInventoryId(shipRecord.typeID || shipRecord.shipTypeID, 0),
            ownerID: charID,
            characterID: charID,
    ...
    _primeInSpaceBayDogmaItems(session, boundContext, flagID, items = []) {
        ...
        for (const item of normalizedItems) {
            const itemID = this._normalizeInventoryId(item && item.itemID, 0);
            const attributes = this._resolveDroneBayDogmaAttributes(session, item, boundContext);
            if (itemID > 0 && attributes && typeof attributes === "object") {
                attributesByItemID.set(itemID, attributes);
            }
        }
        const nextBootstrapKey = this._buildBayDogmaBootstrapKey(normalizedItems, attributesByItemID);
        const previousBootstrapKey = session._space.inventoryBayDogmaBootstrapKeys[numericFlagID] || "";
        ...
        for (const item of normalizedItems) {
            ...
            syncInventoryItemForSession(session, item, {
    ...
    _buildActiveShipLoadedChargeTupleRows(session, boundContext, requestedFlag) {
        if (!this._isActiveInSpaceShipInventory(session, boundContext) ||
            requestedFlag !== null) {
            return [];
        }
        const shipID = this._normalizeInventoryId(boundContext && boundContext.inventoryID, 0);
        const charID = this._getCharacterId(session);
        if (shipID <= 0 || charID <= 0) {
            return [];
        }
        return getLoadedChargeItems(charID, shipID)
            ...
            .sort((left, right) => (this._normalizeInventoryId(left && left.flagID, 0) -
            ...
            return {
                ...
                categoryID: 8,
                customInfo: item.customInfo === undefined ? "" : item.customInfo,
                singleton: Number(item.singleton) === 1 ? 1 : 0,
                stacksize: quantity,
            };
        })
            .filter((item) => (this._isChargeTupleItemID(item.itemID) &&
            item.typeID > 0 &&
            item.flagID > 0));
    }
    _mergeActiveShipLoadedChargeTupleRows(session, boundContext, requestedFlag, items = []) {
        const activeShipID = this._normalizeInventoryId(boundContext && boundContext.inventoryID, 0);
        const baseItems = (Array.isArray(items) ? items : []).filter((item) => !(activeShipID > 0 &&
            item &&
            this._normalizeInventoryId(item.locationID, 0) === activeShipID &&
            this._normalizeInventoryId(item.categoryID, 0) === 8 &&
            isShipFittingFlag(item.flagID)));
        const tupleRows = this._buildActiveShipLoadedChargeTupleRows(session, boundContext, requestedFlag);
        if (tupleRows.length === 0) {
            return baseItems;
        }
        const seenTupleKeys = new Set(baseItems
    ...
    _isActiveShipInventory(session, boundContext) {
        ...
        const activeShipID = this._normalizeInventoryId((session._space && session._space.shipID) ||
            session.activeShipID ||
            session.shipID ||
            session.shipid ||
            this._getShipId(session), 0);
        const boundInventoryID = this._normalizeInventoryId(boundContext.inventoryID, 0);
        return activeShipID > 0 && boundInventoryID === activeShipID;
    }
    _isActiveInSpaceShipInventory(session, boundContext) {
        if (!this._isActiveShipInventory(session, boundContext) ||
            !session ||
    ...
    _primeDeferredSpaceBallparkVisuals(session, boundContext = null, options = {}) {
        if (!this._isActiveInSpaceShipInventory(session, boundContext) ||
            !session ||
            !session._space) {
            return false;
        ...
        if (runtime &&
            ...
            runtime.recordSessionJumpTimingTrace(session, "invbroker-prime-deferred-ballpark", {
                primed: primed === true,
                elapsedMs,
                reason: typeof options.reason === "string" && options.reason.trim().length > 0
                    ? options.reason.trim()
                    : "unknown",
        ...
        if (primed) {
            log.debug(`[InvBroker] Primed deferred ballpark visuals source=${typeof options.reason === "string" && options.reason.trim().length > 0
                ? options.reason.trim()
                : "unknown"} ${describeSessionHydrationState(session)}`);
        }
        if (elapsedMs >= 100) {
            log.info(`[InvBroker] Deferred ballpark prime source=${typeof options.reason === "string" && options.reason.trim().length > 0
                ? options.reason.trim()
                : "unknown"} took ${elapsedMs}ms primed=${primed ? 1 : 0}`);
        }
        return primed;
    }
    _primePendingSpaceShipInventoryBootstrap(session, boundContext = null, options = {}) {
        if (!session || !session._space) {
            return;
        }
    ...
    _makeBoundSubstruct(context, session = null) {
        if (this._isUnanchoredStructureHullInventoryID(context && context.inventoryID)) {
            this._throwUnanchoredStructureHullInventoryError();
        }
        if (this._isPendingStructureUnanchorContainerInventoryID(context && context.inventoryID)) {
            this._throwPendingStructureUnanchorContainerInventoryError();
        }
    ...
    _getShipMetadata(session, shipTypeID = null, shipName = null) {
        ...
        return (resolveShipByTypeID(resolvedShipTypeID) || {
    ...
    _normalizeMergeOps(value) {
        const rawOps = Array.isArray(value)
            ? value
    ...
    _buildPIImportCommoditiesFromItems(importData, customsOfficeID, characterID) {
        ...
        for (const [rawItemID, quantity] of Object.entries(importItemQuantities)) {
            ...
            const availableQuantity = this._normalizeInventoryId(item.singleton === 1 ? 1 : item.stacksize ?? item.quantity, 0);
    ...
    _throwNotEnoughMoney(requiredAmount, currentBalance) {
        throwWrappedUserError("NotEnoughMoney", buildNotEnoughMoneyUserErrorValues(requiredAmount, currentBalance));
    }
    ...
    _debitPIWalletInLegs(characterID, amount, entryTypeID, referenceID, description, legCount = 1) {
        ...
        if (normalizedLegCount === 1) {
            return [
                this._debitPIWallet(characterID, normalizedAmount, entryTypeID, referenceID, description),
            ].filter(Boolean);
        }
        const debits = [];
        ...
        for (let index = 0; index < normalizedLegCount; index += 1) {
            ...
            const debit = this._debitPIWallet(characterID, legAmount, entryTypeID, referenceID, description);
            if (debit) {
                debits.push(debit);
            }
        }
    ...
    _normalizeFitFittingItemsByType(value) {
        ...
        const unwrappedEntryPairs = unwrapped && typeof unwrapped === "object" && Array.isArray(unwrapped.dict)
            ? unwrapped.dict
            : null;
    ...
    _normalizeFitFittingModulesByFlag(value) {
        ...
        const payload = unwrapped && typeof unwrapped === "object" && !Array.isArray(unwrapped)
            ? unwrapped
            : {};
        const source = payload.modulesByFlag &&
        ...
        return Object.entries(source)
            ...
            .sort((left, right) => {
            const leftIsSubsystem = SLOT_FAMILY_FLAGS.subsystem.includes(left.flagID);
            const rightIsSubsystem = SLOT_FAMILY_FLAGS.subsystem.includes(right.flagID);
            if (leftIsSubsystem !== rightIsSubsystem) {
                return leftIsSubsystem ? -1 : 1;
            }
    ...
    _requeueFitFittingSourceStack(candidateItemIDs, itemID, originalSourceItem) {
        ...
        if (!candidateItemIDs.some((candidateID) => this._normalizeInventoryId(candidateID, 0) === normalizedItemID)) {
    ...
    _normalizeFitFittingQuantitiesByType(value, key) {
        ...
        const pairs = Array.isArray(entryPairs)
            ? entryPairs
            : Object.entries(source && typeof source === "object" && !Array.isArray(source)
                ? source
                : {});
    ...
    _getFitFittingExistingDestinationQuantity(session, shipRecord, destinationFlagID, typeID) {
        const ownerID = this._getFitFittingDestinationOwnerID(session, shipRecord, destinationFlagID);
        ...
        return listContainerItems(ownerID, shipRecord.itemID, destinationFlagID)
            ...
            .reduce((sum, item) => sum + (Number(item && item.singleton) === 1
            ? 1
            : Math.max(1, Number(item && (item.stacksize ?? item.quantity ?? 1)) || 1)), 0);
    }
    ...
    _resolveAppliedMoveQuantity(item, destination, requestedQuantity = null) {
        const resolvedQuantity = this._resolveMoveQuantity(item, destination, requestedQuantity);
    ...
    _tryLoadChargeFromInventoryAdd(session, sourceItemDescriptor, destination, requestedQuantity = null) {
        ...
        const existingChargeTypeID = this._normalizeInventoryId(existingCharge && existingCharge.typeID, 0);
        if (existingCharge && existingChargeTypeID !== chargeTypeID) {
            return { handled: false };
        }
        const moduleCapacity = Math.max(1, getModuleChargeCapacity(moduleItem.typeID, chargeTypeID));
        ...
        const requestedLimit = requestedQuantity === null || requestedQuantity === undefined
            ? availableQuantity
            : Math.max(1, Number(requestedQuantity) || 1);
        const moveQuantity = Math.min(neededQuantity, availableQuantity, requestedLimit);
        if (moveQuantity <= 0) {
            return {
                handled: true,
                success: false,
    ...
    _isStructureDeedDestination(destination) {
        return (this._normalizeInventoryId(destination && destination.flagID, 0) ===
            ITEM_FLAGS.STRUCTURE_DEED);
    }
    _isStructureDeedBaySource(item) {
        return (this._normalizeInventoryId(item && item.flagID, 0) ===
            ITEM_FLAGS.STRUCTURE_DEED &&
    ...
    _isMoonMaterialBayFlag(flagID) {
        return (this._normalizeInventoryId(flagID, 0) === FLAG_MOON_MATERIAL_BAY);
    }
    _getStructureInventoryOwnerID(structure) {
        return this._normalizeInventoryId(structure && (structure.ownerCorpID || structure.ownerID), 0);
    }
    ...
    _isAutoMoonMinerStructure(structure) {
        return (this._normalizeInventoryId(structure && structure.typeID, 0) ===
    ...
    _isAutoMoonMinerReinforced(structure) {
        ...
        return (stateID === STRUCTURE_STATE.ARMOR_REINFORCE ||
            stateID === STRUCTURE_STATE.HULL_REINFORCE);
    }
    _getStructureDisplayName(structure, structureID) {
        return (structure &&
            (structure.itemName || structure.name || structure.structureName)) || `Structure ${this._normalizeInventoryId(structureID, 0)}`;
    }
    ...
    _getSessionCorpID(session) {
        return this._normalizeInventoryId(session && (session.corporationID || session.corpid), 0);
    }
    _normalizeCorpRoleBitfield(value) {
        ...
        return BigInt(Number.isFinite(numeric) && numeric > 0 ? Math.trunc(numeric) : 0);
    }
    _hasStationManagerRole(session) {
        const roleValue = session && (session.corprole ||
            session.corpRole ||
            session.rolesAtAll ||
            0);
        return (this._normalizeCorpRoleBitfield(roleValue) & CORP_ROLE_STATION_MANAGER) === CORP_ROLE_STATION_MANAGER;
    }
    ...
    _ensureCanAccessMoonMaterialBay(session, structure, structureID) {
        const hasAccess = this._isAutoMoonMinerStructure(structure) && (this._hasAutoMoonMinerOwnerStationManagerAccess(session, structure) ||
            characterHasStructureSetting(session, structure, STRUCTURE_SETTING_ID.AUTOMOONMINING));
        if (!this._isAutoMoonMinerStructure(structure) ||
            !hasAccess ||
    ...
    _getMoonMaterialBayCapacity(structure) {
        ...
        const capacity = Number(attributes[ATTRIBUTE_OUTPUT_MOON_MATERIAL_BAY_CAPACITY] ??
    ...
    _getStructureFighterBayCapacity(structure) {
        const capacity = Number(getShipBaseAttributeValue(structure && structure.typeID, "fighterCapacity"));
        return Number.isFinite(capacity) && capacity > 0 ? capacity : 0;
    }
    ...
    _isControlledStructureInventoryID(session, inventoryID) {
        ...
        const sessionStructureID = this._normalizeInventoryId(session && (session.structureID || session.structureid), 0);
        const sessionShipID = this._normalizeInventoryId(session && (session.shipID || session.shipid), 0);
        return (sessionStructureID === structureID &&
    ...
    _getRequiredStructureCoreTypeID(structure) {
        ...
        const explicitCoreTypeID = this._normalizeInventoryId(structure.quantumCoreItemTypeID, 0);
        if (explicitCoreTypeID > 0) {
            return explicitCoreTypeID;
        }
        const typeRecord = typeof structureState.getStructureTypeByID === "function"
            ? structureState.getStructureTypeByID(structure.typeID)
            : null;
        return this._normalizeInventoryId(typeRecord && typeRecord.defaultQuantumCoreTypeID, 0);
    }
    ...
    _validateStructureCoreInstallMove(item, destination) {
        ...
        const existingDeedItems = listContainerItems(null, structure.structureID, ITEM_FLAGS.STRUCTURE_DEED).filter((existingItem) => this._normalizeInventoryId(existingItem && existingItem.itemID, 0) !==
    ...
    _syncStructureCoreInstall(_session, item, destination) {
        ...
        const result = structureState.setStructureQuantumCoreInstalled(structure.structureID, true);
        if (!result.success) {
            log.warn(`[InvBroker] structure core install state update failed structure=${structure.structureID} error=${result.errorMsg || "UNKNOWN"}`);
            return result;
        }
        const updatedStructure = result.data || structure;
        const solarSystemID = this._normalizeInventoryId(updatedStructure.solarSystemID || structure.solarSystemID, 0);
        if (solarSystemID > 0) {
    ...
    _syncStructureServiceModuleMove(session, sourceItem, destination) {
        ...
        const removedFromSourceServiceSlots = isStructureServiceSlotFlag(sourceFlagID) &&
            sourceLocationID > 0 &&
            !(isStructureServiceSlotFlag(destinationFlagID) &&
                destinationLocationID === sourceLocationID);
        if (removedFromSourceServiceSlots) {
            const cancelResult = cancelIndustryJobsForRemovedServiceModule(sourceLocationID, sourceItem && sourceItem.typeID, {
                session,
                completedCharacterID: this._normalizeInventoryId(session && (session.characterID || session.charid), 0),
                excludedItemID: this._normalizeInventoryId(sourceItem && sourceItem.itemID, 0),
            });
            if (cancelResult && cancelResult.success === false) {
                log.warn(`[InvBroker] structure service module removal job cancel failed structure=${sourceLocationID} item=${sourceItem && sourceItem.itemID || 0} error=${cancelResult.errorMsg || "UNKNOWN"}`);
            }
            if (cancelResult &&
                Array.isArray(cancelResult.unbackedServiceIDs) &&
                cancelResult.unbackedServiceIDs.includes(STRUCTURE_SERVICE_ID.MEDICAL)) {
                const { removeJumpClonesAtStructure, } = require(path.join(__dirname, "../station/jumpCloneRuntime"));
                const cloneCleanupResult = removeJumpClonesAtStructure(sourceLocationID, {
                    reason: "structureCloneServiceRemoved",
                    destroyerID: this._normalizeInventoryId(session && (session.characterID || session.charid), 0),
                });
                if (!cloneCleanupResult || cloneCleanupResult.success !== true) {
                    log.warn(`[InvBroker] structure clone cleanup failed structure=${sourceLocationID} item=${sourceItem && sourceItem.itemID || 0} error=${cloneCleanupResult && cloneCleanupResult.errorMsg || "UNKNOWN"}`);
                }
                else if (cloneCleanupResult.removedCloneCount > 0) {
                    log.info(`[InvBroker] removed ${cloneCleanupResult.removedCloneCount} structure jump clones after cloning service removal structure=${sourceLocationID}`);
                }
            }
        }
        if (isStructureServiceSlotFlag(destinationFlagID) && destinationLocationID > 0) {
            ...
            if (movedItem &&
                ...
                const autoOnlineResult = tryAutoOnlineFittedStructureServiceModule(destinationLocationID, movedItem);
                if (autoOnlineResult &&
                ...
                else if (autoOnlineResult &&
                    autoOnlineResult.success !== true &&
                    ![
                        "NOT_ENOUGH_FUEL",
                        "STRUCTURE_DAMAGED",
                        "STRUCTURE_SERVICE_REQUIRES_SOV_UPGRADE",
                    ].includes(String(autoOnlineResult.errorMsg || ""))) {
                    log.warn(`[InvBroker] structure service module auto-online failed structure=${destinationLocationID} item=${movedItemID} error=${autoOnlineResult.errorMsg || "UNKNOWN"}`);
                }
            }
        }
        for (const structureID of touchedStructureIDs) {
            ...
            if (!result || result.success !== true) {
                log.warn(`[InvBroker] structure service module sync failed structure=${structureID} error=${result && result.errorMsg || "UNKNOWN"}`);
                continue;
            }
    ...
    _resolveShipModuleFitOnlineChanges(session, sourceItem, destination) {
        ...
        const alreadyOffline = movedItem.moduleState && movedItem.moduleState.online === false;
        if (alreadyOffline) {
            return [];
        }
        ...
        if (!updateResult || updateResult.success !== true) {
            log.warn(`[InvBroker] ship module fit auto-offline failed ship=${destinationLocationID} item=${movedItemID} reason=${decision.reason || "UNKNOWN"} error=${updateResult && updateResult.errorMsg || "UNKNOWN"}`);
            return [];
        }
        log.debug(`[InvBroker] fitted module left offline ship=${destinationLocationID} item=${movedItemID} reason=${decision.reason || "UNKNOWN"}`);
        return [
            {
    ...
    _isLootContainerEntity(session, entityID) {
        ...
        return Boolean(entity && (entity.kind === "container" || entity.kind === "wreck"));
    }
    _isFleetLootSource(session, sourceItemDescriptor, sourceLocationID = 0) {
        ...
        const itemLocationID = this._normalizeInventoryId(sourceItemDescriptor.item.locationID, 0);
        if (this._isLootContainerEntity(session, itemLocationID)) {
            return true;
        }
        const explicitSourceLocationID = this._normalizeInventoryId(sourceLocationID, 0);
        return explicitSourceLocationID > 0
            ? this._isLootContainerEntity(session, explicitSourceLocationID)
            : false;
    }
    _appendFleetLootEntry(session, fleetLootEntries, sourceItemDescriptor, sourceLocationID, destination, requestedQuantity) {
        if (!Array.isArray(fleetLootEntries) ||
        ...
        const item = sourceItemDescriptor && sourceItemDescriptor.item
            ...
            : null;
        const typeID = this._normalizeInventoryId(item && item.typeID, 0);
        const quantity = this._resolveAppliedMoveQuantity(item, destination, requestedQuantity);
        if (typeID <= 0 || quantity <= 0) {
            return;
        }
    ...
    _destinationUsesCapacity(boundContext, destination) {
        ...
        if (boundContext.kind === "controlledStructureInventory" &&
            this._isStructureOwnerBayFlag(destination.flagID)) {
            return true;
        }
        return (boundContext.kind === "shipInventory" ||
    ...
    _getSpaceContainerScopeAccessError(session, containerID) {
        const systemID = this._normalizeInventoryId(session && session._space && session._space.systemID, 0);
        const shipID = this._normalizeInventoryId(session && session._space && session._space.shipID, 0);
        const numericContainerID = this._normalizeInventoryId(containerID, 0);
        if (systemID <= 0 || shipID <= 0 || numericContainerID <= 0) {
            return null;
        }
        const scene = runtime.ensureScene(systemID);
        const targetEntity = scene && scene.getEntityByID(numericContainerID);
        if (!targetEntity ||
            (targetEntity.kind !== "container" && targetEntity.kind !== "wreck")) {
            return null;
        }
        const shipEntity = scene.getEntityByID(shipID);
        return shipEntity && canEntitiesInteractLocally(shipEntity, targetEntity)
            ? null
        ...
        for (const containerID of [sourceContainerID, destinationContainerID]) {
            ...
            if (accessError) {
                return {
                    ...
                    containerID: this._normalizeInventoryId(containerID, 0),
                };
            }
        }
        return { success: true };
    ...
    _validateMobileDepotCargoTransferAccess(session, sourceItem, destination) {
        const sourceContainerID = this._normalizeInventoryId(sourceItem && sourceItem.locationID, 0);
        const destinationContainerID = this._normalizeInventoryId(destination && destination.locationID, 0);
        const sourceAccessError = this._getMobileDepotCargoAccessError(session, sourceContainerID);
        ...
        const destinationAccessError = this._getMobileDepotCargoAccessError(session, destinationContainerID);
    ...
    _getShipServiceAccessError(session, shipID, flagID) {
        ...
        const shipRecord = findShipItemById(this._normalizeInventoryId(shipID, 0)) ||
            findItemById(this._normalizeInventoryId(shipID, 0));
        if (!shipRecord || Number(shipRecord.categoryID) !== 6) {
            return null;
        }
        const accessResult = validateShipServiceAccess(session, shipRecord, numericFlagID);
        return accessResult && accessResult.success
            ? null
    ...
    _validateShipServiceTransferAccess(session, sourceItem, destination) {
        const sourceLocationID = this._normalizeInventoryId(sourceItem && sourceItem.locationID, 0);
        const sourceFlagID = this._normalizeInventoryId(sourceItem && sourceItem.flagID, 0);
        const sourceAccessError = this._getShipServiceAccessError(session, sourceLocationID, sourceFlagID);
        if (sourceAccessError) {
            return {
                success: false,
                errorMsg: sourceAccessError,
        ...
        const destinationLocationID = this._normalizeInventoryId(destination && destination.locationID, 0);
        const destinationFlagID = this._normalizeInventoryId(destination && destination.flagID, 0);
        const destinationAccessError = this._getShipServiceAccessError(session, destinationLocationID, destinationFlagID);
        if (destinationAccessError) {
            return {
                success: false,
                errorMsg: destinationAccessError,
    ...
    _throwShipServiceAccessError(errorMsg) {
        const notifyByError = {
            SHIP_SERVICE_ACCESS_DENIED: "You do not have access to this ship service bay.",
        };
        throwWrappedUserError("CustomNotify", {
    ...
    _isActiveShipCloaked(session, shipID = 0) {
        const numericShipID = this._normalizeInventoryId(shipID || (session && session._space && session._space.shipID) || this._getShipId(session), 0);
    ...
    _isInSpaceRefitMove(session, shipRecord, item, destination) {
        ...
        const sourceIsFitted = (this._normalizeInventoryId(item.locationID, 0) === shipID &&
            isShipFittingFlag(item.flagID));
        const destinationIsFitted = (destination &&
    ...
    _getMobileDepotFittingServiceAccess(session) {
        const systemID = this._normalizeInventoryId(session && session._space && session._space.systemID, this._normalizeInventoryId(session && (session.solarsystemid2 || session.solarsystemid), 0));
        if (systemID <= 0) {
            return {
                success: false,
        ...
        const depots = listContainerItems(null, systemID, 0)
            ...
            .sort((left, right) => (this._normalizeInventoryId(left && left.itemID, 0) -
            this._normalizeInventoryId(right && right.itemID, 0)));
        ...
        for (const depot of depots) {
            ...
            const errorMsg = accessResult && accessResult.errorMsg
    ...
    _isDockedStructureRefitMove(session, shipRecord, item, destination) {
        ...
        const structureID = this._normalizeInventoryId(session.structureID || session.structureid, 0);
        if (structureID <= 0) {
            return false;
        }
        const shipID = this._normalizeInventoryId(shipRecord.itemID, 0);
    ...
    _checkCapacityForMove(session, boundContext, destination, item, requestedQuantity = null) {
        if (!item ||
            !boundContext ||
            !destination ||
        ...
        if (currentLocationID === this._normalizeInventoryId(destination.locationID, 0) &&
        ...
        const availableQuantity = Number(item.singleton) === 1
            ...
            : Math.max(1, Number(item.stacksize ?? item.quantity ?? 1) || 1);
        const resolvedQuantity = this._resolveMoveQuantity(item, destination, requestedQuantity);
        const moveQuantity = resolvedQuantity === null || resolvedQuantity === undefined
            ? availableQuantity
        ...
        const capacityInfo = this._calculateCapacity(session, boundContext, destination.flagID);
        const capacity = Number(capacityInfo &&
            capacityInfo.args &&
            capacityInfo.args.type === "dict" &&
            Array.isArray(capacityInfo.args.entries)
            ? (capacityInfo.args.entries.find(([key]) => key === "capacity") || [])[1]
            : 0) || 0;
        const used = Number(capacityInfo &&
            capacityInfo.args &&
            capacityInfo.args.type === "dict" &&
            Array.isArray(capacityInfo.args.entries)
            ? (capacityInfo.args.entries.find(([key]) => key === "used") || [])[1]
            : 0) || 0;
    ...
    _isSpaceLootMultiAddSource(session, sourceLocationID) {
        const numericSourceLocationID = this._normalizeInventoryId(sourceLocationID, 0);
        if (numericSourceLocationID <= 0) {
            return false;
        }
        if (this._getNativeWreckRecord(numericSourceLocationID)) {
            return true;
        }
        const sourceContainer = findItemById(numericSourceLocationID);
        if (!sourceContainer) {
            return false;
        }
        const metadata = getItemMetadata(sourceContainer.typeID, sourceContainer.itemName);
        const sourceCategoryID = this._normalizeInventoryId(sourceContainer.categoryID, 0);
        const sourceGroupID = this._normalizeInventoryId(sourceContainer.groupID, 0);
        ...
        const isMobileTractorUnit = (sourceCategoryID === CATEGORY_DEPLOYABLE &&
            sourceGroupID === GROUP_MOBILE_TRACTOR_UNIT) ||
            (metadataCategoryID === CATEGORY_DEPLOYABLE &&
                metadataGroupID === GROUP_MOBILE_TRACTOR_UNIT);
        return Boolean(isMobileTractorUnit ||
            isDisposableJetcanRecord(sourceContainer) ||
            this._isLootContainerEntity(session, numericSourceLocationID));
    }
    _preflightSpaceLootMultiAddCapacity(session, boundContext, itemIDs, sourceLocationID, requestedFlag, explicitFlagProvided, requestedQuantity, fittedItemsSnapshot = null) {
        if (!this._isSpaceLootMultiAddSource(session, sourceLocationID) ||
            !Array.isArray(itemIDs) ||
            itemIDs.length <= 0) {
            return { handled: false, success: true };
        }
        const activeShipID = this._normalizeInventoryId(this._getShipId(session), 0);
        if (activeShipID <= 0) {
            return { handled: false, success: true };
        }
        ...
        for (const itemID of itemIDs) {
            ...
            if (!item || !sourceItemDescriptor) {
                return {
                    handled: true,
                    success: false,
                    errorMsg: "SOURCE_ITEM_NOT_FOUND",
                };
            }
            if (!this._isTransferSourceLocationMatch(sourceItemDescriptor, sourceLocationID)) {
                return {
                    handled: true,
                    success: false,
                    errorMsg: "SOURCE_LOCATION_MISMATCH",
                    item,
                };
            }
            const destination = this._resolveDestinationForMove(session, boundContext, item, requestedFlag, explicitFlagProvided, fittedItemsSnapshot);
            if (!destination ||
                this._normalizeInventoryId(destination.locationID, 0) !==
                    activeShipID ||
                this._normalizeInventoryId(destination.flagID, 0) !==
                    ITEM_FLAGS.CARGO_HOLD) {
                return { handled: false, success: true };
            }
            const currentLocationID = this._normalizeInventoryId(item.locationID, 0);
            ...
            const appliedQuantity = this._resolveAppliedMoveQuantity(item, destination, requestedQuantity);
            requiredVolume += this._getItemMoveVolume(item, appliedQuantity);
            representativeItem = representativeItem || item;
        }
        if (requiredVolume <= 0 || !representativeItem) {
            return { handled: true, success: true };
        }
        const capacityInfo = this._calculateCapacity(session, boundContext, ITEM_FLAGS.CARGO_HOLD);
        const capacityEntries = capacityInfo &&
            capacityInfo.args &&
            capacityInfo.args.type === "dict" &&
            Array.isArray(capacityInfo.args.entries)
            ? capacityInfo.args.entries
            : [];
        const capacity = Number((capacityEntries.find(([key]) => key === "capacity") || [])[1]) || 0;
        const used = Number((capacityEntries.find(([key]) => key === "used") || [])[1]) || 0;
        const capacityCheck = evaluateCumulativeCapacity({
            capacity,
            used,
            requiredVolume,
        });
        if (capacityCheck.success) {
            return {
                handled: true,
                success: true,
                free: capacityCheck.free,
                requiredVolume: capacityCheck.requiredVolume,
    ...
    _buildCapacityMoveUserErrorValues(capacityCheck, item) {
        const available = Number(Number(capacityCheck && capacityCheck.free || 0).toFixed(6));
        const volume = Number(Number(capacityCheck && capacityCheck.requiredVolume || 0).toFixed(6));
    ...
    _getShipInventoryRecord(session, boundContext) {
        const inventoryID = this._normalizeInventoryId(boundContext && boundContext.inventoryID, 0);
        if (inventoryID <= 0) {
            return null;
        }
        const charId = this._getCharacterId(session);
        return (findCharacterShip(charId, inventoryID) ||
    ...
    _isStructureFitHostRecord(fitHostRecord) {
        return (this._normalizeInventoryId(fitHostRecord && fitHostRecord.categoryID, 0) ===
    ...
    _isAutoFitRequested(explicitFlagValue, explicitFlagProvided) {
        ...
        if (isShipFittingFlag(numericFlag) ||
            SHIP_BAY_FLAGS.has(numericFlag) ||
            this._isStructureOwnerBayFlag(numericFlag) ||
            this._isMoonMaterialBayFlag(numericFlag) ||
    ...
    _resolveDestinationForMove(session, boundContext, item, requestedFlag, explicitFlagProvided, fittedItemsOverride = null) {
        if (this._isUnanchoredStructureHullInventoryID(boundContext && boundContext.inventoryID)) {
            this._throwUnanchoredStructureHullInventoryError();
        }
        if (this._isPendingStructureUnanchorContainerInventoryID(boundContext && boundContext.inventoryID)) {
            this._throwPendingStructureUnanchorContainerInventoryError();
        }
        ...
        const numericRequestedFlag = requestedFlag === undefined || requestedFlag === null
            ? null
            : this._normalizeInventoryId(requestedFlag, 0);
        const isExplicitFitMove = numericRequestedFlag !== null && isShipFittingFlag(numericRequestedFlag);
        const isAutoFitMove = this._isAutoFitRequested(requestedFlag, explicitFlagProvided);
        if (!shipRecord && (!fitHostRecord || (!isExplicitFitMove && !isAutoFitMove))) {
            return {
                locationID: this._normalizeInventoryId(boundContext && boundContext.inventoryID, this._getStationId(session)),
                flagID: requestedFlag ?? ITEM_FLAGS.HANGAR,
            };
        }
        const currentFittedItems = Array.isArray(fittedItemsOverride) && fittedItemsOverride.length >= 0
            ? fittedItemsOverride
        ...
        if (isAutoFitMove) {
            const autoFitFlag = selectAutoFitFlagForType(fitHostRecord, currentFittedItems, item && item.typeID);
            if (autoFitFlag) {
                return {
                    locationID: fitHostRecord.itemID,
                    flagID: autoFitFlag,
        ...
        return {
            ...
            flagID: numericRequestedFlag ??
                this._normalizeInventoryId(boundContext && boundContext.flagID, ITEM_FLAGS.CARGO_HOLD),
        };
    }
    ...
    _isMtuSpaceComponentLootMove(session, sourceLocationID, destination) {
        ...
        const metadataCategoryID = this._normalizeInventoryId(metadata && metadata.categoryID, 0);
        const metadataGroupID = this._normalizeInventoryId(metadata && metadata.groupID, 0);
        const isMtuSource = (sourceCategoryID === CATEGORY_DEPLOYABLE && sourceGroupID === GROUP_MOBILE_TRACTOR_UNIT) ||
            (metadataCategoryID === CATEGORY_DEPLOYABLE && metadataGroupID === GROUP_MOBILE_TRACTOR_UNIT);
        return Boolean(sourceContainer &&
            isMtuSource &&
            destination &&
    ...
    _isPlayerJetcanLootMove(session, sourceLocationID, destination) {
        ...
        return Boolean(isDisposableJetcanRecord(sourceContainer) &&
    ...
    _buildShipCargoLocationContext(session, destination) {
        const shipID = this._normalizeInventoryId(destination && destination.locationID, this._getShipId(session));
    ...
    _emitStripFittingMoveChanges(session, changes = []) {
        ...
        for (const change of normalizedChanges) {
            emitItemsChangedBatchForSession(session, [change], {
                idType: "charid",
                locationContext: this._buildDockedHangarLocationContext(change.item && change.item.locationID),
            });
        }
    }
    _emitMtuSpaceComponentLootEvents(session, sourceLocationID, destination, lootedItems = []) {
        ...
        const shipID = this._normalizeInventoryId(destination && destination.locationID, this._getShipId(session));
        const flagID = this._normalizeInventoryId(destination && destination.flagID, ITEM_FLAGS.CARGO_HOLD);
    ...
    _classifyCorpStructureHangarMove(session, change) {
        ...
        const currentFlagID = this._normalizeInventoryId(item.flagID, 0);
        const previousOffice = this._getCorporationOfficeForItemLocation(session, previousLocationID);
        const currentOffice = this._getCorporationOfficeForItemLocation(session, currentLocationID);
        ...
        if (previousOwnerID === corporationID &&
        ...
        if (previousOwnerID === characterID &&
    ...
    _emitCorpStructureHangarMoveChanges(session, changes = []) {
        const normalizedChanges = (Array.isArray(changes) ? changes : []).filter((change) => change && change.item);
    ...
    _emitStructureCoreInstallMoveChanges(session, changes = [], destination = null, sourceItem = null) {
        const normalizedChanges = (Array.isArray(changes) ? changes : []).filter((change) => change && change.item);
        const deedChanges = normalizedChanges.filter((change) => (this._normalizeInventoryId(change.item.locationID, 0) ===
        ...
        const previousOwnerID = this._normalizeInventoryId(sourceItem && sourceItem.ownerID, this._getCharacterId(session));
        const previousLocationID = this._normalizeInventoryId(sourceItem && sourceItem.locationID, 0);
        const previousFlagID = this._normalizeInventoryId(sourceItem && sourceItem.flagID, ITEM_FLAGS.HANGAR);
        const changeEntries = [];
        ...
        this._refreshDockedFittingState(session, normalizedChanges);
        if (session && typeof session.sendNotification === "function") {
            const structureID = this._normalizeInventoryId(destination && destination.locationID, 0);
            session.sendNotification("OnClientEvent_MoveFromCargoToHangar", "clientID", [
                structureID,
                structureID,
                ITEM_FLAGS.STRUCTURE_DEED,
    ...
    _primeDroneBayDogmaForFittingChanges(session, changes = []) {
        ...
        for (const change of changes) {
            ...
            const currentFlagID = this._normalizeInventoryId(item.flagID, 0);
            const previousFlagID = this._normalizeInventoryId(previous.flagID, 0);
            if (currentFlagID !== ITEM_FLAGS.DRONE_BAY &&
                previousFlagID !== ITEM_FLAGS.DRONE_BAY) {
                continue;
            }
            const shipID = this._normalizeInventoryId(currentFlagID === ITEM_FLAGS.DRONE_BAY
                ? item.locationID
        ...
        for (const shipID of shipIDs) {
            ...
            const droneBayItems = this._resolveContainerItems(session, ITEM_FLAGS.DRONE_BAY, shipContext);
            primed = this._primeInSpaceBayDogmaItems(session, shipContext, ITEM_FLAGS.DRONE_BAY, droneBayItems) || primed;
        }
        return primed;
    }
    ...
    _emitFittingMoveChanges(session, changes = []) {
        const normalized = (Array.isArray(changes) ? changes : []).filter((change) => change && change.item);
        ...
        for (const change of normalized) {
            ...
            const hasRowDelta = signatureParts[1] !== signatureParts[7] ||
        ...
        if (fittingShipID <= 0) {
            fittingShipID = this._normalizeInventoryId((session &&
    ...
    _maybeCompleteMaterializedDungeonSitesForSources(session, sourceContainerIDs = []) {
        ...
        for (const containerID of new Set(sourceContainerIDs.map((value) => this._normalizeInventoryId(value, 0)))) {
            ...
            dungeonUniverseSiteService.maybeCompleteMaterializedDataRelicSiteForContainerID(scene, containerID, {
                broadcast: true,
    ...
    _refreshDockedFittingState(session, changes = []) {
        ...
        const activeShipID = this._normalizeInventoryId(session.activeShipID || session.shipID || session.shipid, 0);
        if (activeShipID <= 0) {
            return;
        }
        const touchesFittingState = changes.some((change) => {
            ...
            const previousLocationID = this._normalizeInventoryId(previousState.locationID, 0);
            const previousFlagID = this._normalizeInventoryId(previousState.flagID, 0);
            const nextLocationID = this._normalizeInventoryId(change.item.locationID, 0);
            const nextFlagID = this._normalizeInventoryId(change.item.flagID, 0);
    ...
    _refreshBallparkShipPresentation(session, changes = []) {
        ...
        const activeShipID = this._normalizeInventoryId(session._space.shipID || this._getShipId(session), 0);
        if (activeShipID <= 0) {
            return;
        }
        const touchesFittingState = (change) => {
            ...
            const previousLocationID = this._normalizeInventoryId(change.previousData && change.previousData.locationID, 0);
            const previousFlagID = this._normalizeInventoryId(change.previousData && change.previousData.flagID, 0);
            const nextLocationID = this._normalizeInventoryId(change.item && change.item.locationID, 0);
            const nextFlagID = this._normalizeInventoryId(change.item && change.item.flagID, 0);
            if (previousLocationID !== activeShipID &&
                nextLocationID !== activeShipID) {
                return false;
            }
    ...
    _refreshBallparkInventoryPresentation(session, changes = []) {
        ...
        for (const entityID of affectedEntityIDs) {
            ...
            if (entity && entity.nativeNpcWreck === true) {
                nativeNpcWreckService.refreshNativeWreckRuntimeEntity(session._space.systemID, entityID, { broadcast: true });
            }
            else {
                runtime.refreshInventoryBackedEntityPresentation(session._space.systemID, entityID, { broadcast: true });
            }
        }
    }
    _validateFittingMove(session, shipRecord, item, destination, fittedItemsSnapshot = null) {
        ...
        const integratedServiceModuleValidation = this._validateIntegratedStructureServiceModuleFit(item, destination);
        if (!integratedServiceModuleValidation.success) {
            return integratedServiceModuleValidation;
        }
        const securityClassValidation = this._validateStructureServiceModuleSecurityClass(item, destination);
        if (!securityClassValidation.success) {
            return securityClassValidation;
        }
        const structureReactionValidation = this._validateStructureReactionServiceModuleFit(item, destination);
        if (!structureReactionValidation.success) {
            return structureReactionValidation;
        }
        const moonDrillValidation = this._validateStructureMoonDrillServiceModuleFit(item, destination);
        if (!moonDrillValidation.success) {
            return moonDrillValidation;
        }
        return validateFitForShip(this._getCharacterId(session), shipRecord, item, destination.flagID, fittedItemsSnapshot);
    }
    _listFighterTubeItems(ownerID, locationID, excludedItemID = 0) {
        const numericOwnerID = ownerID === null || ownerID === undefined
            ? null
        ...
        for (const tubeFlagID of FIGHTER_TUBE_FLAGS) {
            for (const tubeItem of listContainerItems(numericOwnerID, numericLocationID, tubeFlagID)) {
                if (numericExcludedItemID > 0 &&
    ...
    _validateShipBayMove(session, shipRecord, item, destination) {
        ...
        if (isFighterTubeFlag(destinationFlagID)) {
            ...
            const destinationOccupants = listContainerItems(this._getCharacterId(session), shipRecord.itemID, destinationFlagID).filter((existingItem) => Number(existingItem && existingItem.itemID) !== Number(item.itemID));
            ...
            const occupiedTubeItems = this._listFighterTubeItems(this._getCharacterId(session), shipRecord.itemID, item.itemID);
            if (!canLoadFighterTypeIntoHostTube(shipRecord.typeID, item.typeID, destinationFlagID, occupiedTubeItems)) {
                return {
                    success: false,
                    errorMsg: "NotEnoughCargoSpace",
                };
        ...
        if (destinationFlagID === ITEM_FLAGS.MOBILE_DEPOT_HOLD) {
            const holdCapacity = Number(getShipBaseAttributeValue(shipRecord.typeID, MOBILE_DEPOT_HOLD_CAPACITY_ATTRIBUTE)) || 0;
        ...
        if (isGenericSpecialShipHoldFlag(destinationFlagID)) {
            const holdCapacity = getSpecialShipHoldCapacity(null, shipRecord.typeID, destinationFlagID, getShipBaseAttributeValue);
    ...
    _validateStructureOwnerBayMove(item, destination) {
        ...
        if (isFighterTubeFlag(destinationFlagID)) {
            ...
            const destinationOccupants = listContainerItems(structureOwnerID, structure.structureID, destinationFlagID).filter((existingItem) => Number(existingItem && existingItem.itemID) !== Number(item.itemID));
            if (destinationOccupants.length > 0) {
                return {
                    success: false,
                    errorMsg: "NotEnoughFighterBaySpace",
            ...
            const occupiedTubeItems = this._listFighterTubeItems(structureOwnerID, structure.structureID, item.itemID);
            if (!canLoadFighterTypeIntoHostTube(structure.typeID, item.typeID, destinationFlagID, occupiedTubeItems)) {
                return {
                    success: false,
                    errorMsg: "NotEnoughFighterBaySpace",
                };
    ...
    _validateStructureServiceModuleRemovalAccess(session, item, destination) {
        ...
        const destinationFlagID = this._normalizeInventoryId(destination && destination.flagID, 0);
        const destinationLocationID = this._normalizeInventoryId(destination && destination.locationID, 0);
        if (isStructureServiceSlotFlag(destinationFlagID) &&
            destinationLocationID === sourceLocationID) {
            return { success: true };
        }
    ...
    _validateStructureServiceModuleSecurityClass(item, destination) {
        ...
        const disallowedInEmpire = Number(getTypeAttributeValue(item.typeID, ATTRIBUTE_DISALLOW_IN_EMPIRE_SPACE)) > 0;
        ...
        const disallowedInHighSec = Number(getTypeAttributeValue(item.typeID, ATTRIBUTE_DISALLOW_IN_HIGH_SEC)) > 0;
    ...
    _validateStructureMoonDrillServiceModuleFit(item, destination) {
        ...
        const devFlags = structure.devFlags && typeof structure.devFlags === "object"
            ? structure.devFlags
            : {};
        const verifiedBeaconID = Number(devFlags.moonMiningBeaconID || 0);
        if (devFlags.moonMiningLocationVerified !== true ||
    ...
    _throwFittingMoveUserError(fitValidation, shipRecord = null, item = null) {
        ...
        const data = fitValidation && fitValidation.data && typeof fitValidation.data === "object"
            ? fitValidation.data
            : {};
        ...
        if (errorMsg === "MOON_DRILL_REQUIRES_REFINERY" ||
            errorMsg === "MOON_DRILL_REQUIRES_MOON_MINING_LOCATION" ||
            errorMsg === "STRUCTURE_SERVICE_MODULE_IS_INTEGRATED" ||
        ...
        if (errorMsg === "MAX_GROUP_FITTED") {
            ...
            throwWrappedUserError("CantFitTooManyByGroup", {
                ...
                noOfModulesFitted: Number(data.noOfModulesFitted ?? data.existingGroupCount) || 0,
            });
        }
        if (errorMsg === "MAX_TYPE_FITTED") {
            throwWrappedUserError("CantFitTooManyByType", {
                ...
                noOfModulesFitted: Number(data.noOfModulesFitted ?? data.existingTypeCount) || 0,
            });
        }
        return false;
    }
    _resolveMovedItemID(moveResult, originalItemID, destination) {
        ...
        for (const change of (moveResult && moveResult.data && moveResult.data.changes) || []) {
            ...
            if (this._normalizeInventoryId(change.item.locationID, 0) === destinationLocationID &&
                this._normalizeInventoryId(change.item.flagID, 0) === destinationFlagID) {
                return Number(change.item.itemID) || null;
            }
        }
        const originalMovedItem = findItemById(this._normalizeInventoryId(originalItemID, 0));
    ...
    _buildNativeWreckItemOverrides(_session, inventoryID) {
        return nativeNpcStore.buildNativeWreckInventoryItem(this._normalizeInventoryId(inventoryID, 0));
    }
    _findTransferSourceItem(itemID, sourceLocationID = 0) {
        ...
        if (normalizedSourceLocationID > 0 &&
    ...
    _canTakeFromCorporationHangarSource(session, sourceItemDescriptor) {
        ...
        const office = this._getCorporationOfficeForItemLocation(session, item.locationID);
        if (!office) {
            return false;
        }
        return canTakeFromOwnerLocation(session, corporationID, this._normalizeInventoryId(office.stationID, office.officeID), item.flagID);
    }
    _isInventoryItemTrashable(session, item, requestedLocationID = 0) {
        ...
        const normalizedRequestedLocationID = this._normalizeInventoryId(requestedLocationID, 0);
        if (normalizedRequestedLocationID <= 0) {
            return true;
        }
        return (this._resolveInventoryRootLocationID(item) === normalizedRequestedLocationID);
    }
    ...
    _moveSourceItemToDestination(session, sourceItemDescriptor, destination, quantity = null) {
        ...
        if (isDirectSpaceUnanchoredStructureHullInventoryRecord(sourceItem)) {
            return {
                success: false,
                errorMsg: "UNANCHORED_STRUCTURE_HULL_REQUIRES_SCOOP",
            };
        }
        if (this._getPendingStructureUnanchorContainerAncestor(sourceItem)) {
            return {
                success: false,
                errorMsg: "STRUCTURE_UNANCHOR_CONTAINER_PENDING",
            };
        }
        if (this._isPendingStructureUnanchorContainerInventoryID(destination.locationID)) {
            return {
                success: false,
                errorMsg: "STRUCTURE_UNANCHOR_CONTAINER_PENDING",
            };
        ...
        const groupingContext = {
            ...
            wasGrouped: Number(sourceItem.categoryID) === 7 &&
                isShipFittingFlag(sourceItem.flagID) &&
                getWeaponBankMasterModuleID(this._normalizeInventoryId(sourceItem.locationID, 0), this._normalizeInventoryId(sourceItem.itemID, 0)) > 0,
        };
        if (sourceItemDescriptor.sourceKind === "nativeWreck") {
            return nativeNpcWreckService.transferNativeWreckItemToCharacterLocation({
                characterID: this._getCharacterId(session),
                wreckID: this._normalizeInventoryId(sourceItemDescriptor.wreckItem && sourceItemDescriptor.wreckItem.wreckID, 0),
                wreckItemID: this._normalizeInventoryId(sourceItemDescriptor.wreckItem && sourceItemDescriptor.wreckItem.wreckItemID, 0),
        ...
        const destinationOffice = this._getCorporationOffice(session, destination.locationID);
        if (destinationOffice &&
            this._isCorporationHangarFlag(destination.flagID)) {
            const transferResult = transferItemToOwnerLocation(this._normalizeInventoryId(sourceItemDescriptor.item.itemID, 0), this._getCorporationId(session), this._normalizeInventoryId(destinationOffice.officeID, 0), destination.flagID, quantity);
            return this._applyModuleGroupingMoveCleanup(session, sourceItemDescriptor, destination, transferResult, groupingContext);
        }
        const structureDestination = this._getStructureForOwnerBayDestination(destination);
        const structureOwnerID = this._getStructureInventoryOwnerID(structureDestination);
        if (structureDestination && structureOwnerID > 0) {
            const transferResult = transferItemToOwnerLocation(this._normalizeInventoryId(sourceItemDescriptor.item.itemID, 0), structureOwnerID, this._normalizeInventoryId(structureDestination.structureID, 0), destination.flagID, quantity);
            return this._applyModuleGroupingMoveCleanup(session, sourceItemDescriptor, destination, transferResult, groupingContext);
        }
        if (this._shouldTransferInventoryItemToCharacterShipDestination(session, sourceItemDescriptor, destination)) {
            const transferResult = transferItemToOwnerLocation(this._normalizeInventoryId(sourceItemDescriptor.item.itemID, 0), this._getCharacterId(session), this._normalizeInventoryId(destination.locationID, 0), destination.flagID, quantity);
            return this._applyModuleGroupingMoveCleanup(session, sourceItemDescriptor, destination, transferResult, groupingContext);
        }
        if (this._shouldTransferInventoryItemToCharacterHangarDestination(session, sourceItemDescriptor, destination)) {
            const transferResult = transferItemToOwnerLocation(this._normalizeInventoryId(sourceItemDescriptor.item.itemID, 0), this._getCharacterId(session), this._normalizeInventoryId(destination.locationID, 0), destination.flagID, quantity);
            return this._applyModuleGroupingMoveCleanup(session, sourceItemDescriptor, destination, transferResult, groupingContext);
        }
        const moveResult = moveItemToLocation(this._normalizeInventoryId(sourceItemDescriptor.item.itemID, 0), destination.locationID, destination.flagID, quantity);
        this._moveAttachedLoadedChargeWithModule(session, sourceItemDescriptor.item, destination, moveResult);
        return this._applyModuleGroupingMoveCleanup(session, sourceItemDescriptor, destination, moveResult, groupingContext);
    }
    ...
    _moveAttachedLoadedChargeWithModule(session, sourceItem, destination, moveResult) {
        ...
        const loadedCharge = getLoadedChargeByFlag(this._getCharacterId(session), sourceShipID, sourceFlagID);
        if (!loadedCharge) {
            return;
        }
        const chargeMoveResult = moveItemToLocation(this._normalizeInventoryId(loadedCharge.itemID, 0), destinationLocationID, destinationFlagID, null);
        if (!chargeMoveResult || !chargeMoveResult.success) {
            log.warn(`[InvBroker] Failed to move attached charge itemID=${loadedCharge.itemID} ` +
        ...
        moveResult.data.changes.push(...((chargeMoveResult.data && chargeMoveResult.data.changes) || []));
    }
    _applyModuleGroupingMoveCleanup(session, sourceItemDescriptor, destination, moveResult, groupingContext = null) {
        if (!moveResult ||
            !moveResult.success ||
        ...
        const destinationLocationID = this._normalizeInventoryId(destination.locationID, 0);
        const destinationFlagID = this._normalizeInventoryId(destination.flagID, 0);
        const movedOutOfShipFitting = Number(sourceItem.categoryID) === 7 &&
        ...
        clearModuleFromBanksAndNotify(session, sourceShipID, [sourceItem.itemID], {
            characterID: this._getCharacterId(session),
        });
        if (groupingContext && groupingContext.wasGrouped) {
            notifyWeaponBanksChanged(session, groupingContext.shipID, getShipWeaponBanks(groupingContext.shipID, {
                characterID: this._getCharacterId(session),
            }));
        }
        return moveResult;
    ...
    _findCharacterSkillRecord(session, itemID) {
        ...
        return (getCharacterSkills(charId).find((skill) => skill.itemID === numericItemId) ||
            null);
    }
    ...
    _buildInventoryItemOverrides(session, itemRecord) {
        ...
        const typeID = this._normalizeInventoryId(itemRecord.typeID ?? itemRecord.shipTypeID, 0);
        ...
        const singleton = itemRecord.singleton === null || itemRecord.singleton === undefined
            ? Number(itemRecord.categoryID) === 6
                ? 1
                : 0
            : itemRecord.singleton;
        const quantity = itemRecord.quantity === null || itemRecord.quantity === undefined
            ? Number(singleton) === 1
                ? -1
                : 1
            : itemRecord.quantity;
        const stacksize = itemRecord.stacksize === null || itemRecord.stacksize === undefined
            ? Number(singleton) === 1
                ? 1
                : quantity
        ...
        return {
            ...
            ownerID: this._normalizeInventoryId(itemRecord.ownerID, this._getCharacterId(session)),
            locationID: this._normalizeInventoryId(itemRecord.locationID, this._getStationId(session)),
            ...
            customInfo: itemRecord.customInfo === undefined ? "" : itemRecord.customInfo,
            singleton,
            stacksize,
        };
    }
    _buildStationItemOverrides(session, overrideStationID = null) {
        ...
        return {
            ...
            ownerID: this._normalizeInventoryId(station.ownerID || station.corporationID, STATION_OWNER_ID),
    ...
    _buildStructureItemOverrides(session, overrideStructureID = null) {
        const structureID = this._normalizeInventoryId(overrideStructureID ?? this._getStationId(session), 0);
        if (structureID <= 0) {
            return null;
        }
        ...
        const solarSystemID = this._normalizeInventoryId(structure.solarSystemID ??
        ...
        return {
            ...
            ownerID: this._normalizeInventoryId(structure.ownerCorpID || structure.ownerID, this._getCharacterId(session)),
            locationID: this._normalizeInventoryId(solarSystemID, structureID),
            flagID: 0,
            quantity: -1,
    ...
    _getCharacterContainerItems(session, requestedFlag = null) {
        const characterID = this._getCharacterId(session);
        const numericFlag = requestedFlag === null || requestedFlag === undefined
            ? null
            : this._normalizeInventoryId(requestedFlag, 0);
    ...
    _listCorporationOfficeItems(session, office, requestedFlag = null) {
        ...
        return items.sort((left, right) => this._normalizeInventoryId(left && left.flagID, 0) -
            this._normalizeInventoryId(right && right.flagID, 0) ||
            this._normalizeInventoryId(left && left.typeID, 0) -
                this._normalizeInventoryId(right && right.typeID, 0) ||
            this._normalizeInventoryId(left && left.itemID, 0) -
    ...
    _buildCorporationOfficeItemOverrides(session, office) {
        ...
        return {
            itemID: this._normalizeInventoryId(office.itemID, this._normalizeInventoryId(office.officeID, 0)),
            typeID: OFFICE_TYPE_ID,
            ownerID: this._getCorporationId(session),
            locationID: this._normalizeInventoryId(office.stationID, this._getStationId(session)),
    ...
    _buildContainerItemOverrides(session, inventoryID) {
        ...
        const shipRecord = findCharacterShip(charId, numericInventoryID) ||
            findShipItemById(numericInventoryID);
        const genericItemRecord = shipRecord || findItemById(numericInventoryID);
        if (genericItemRecord) {
            return this._buildInventoryItemOverrides(session, genericItemRecord);
        }
        const corporationOffice = this._getCorporationOffice(session, numericInventoryID);
        ...
        if (numericInventoryID === stationId || numericInventoryID === 0) {
            if (Number(session && (session.structureID || session.structureid)) > 0) {
                return (this._buildStructureItemOverrides(session, stationId) ||
    ...
    _resolveContainerItems(session, requestedFlag, boundContext) {
        ...
        const containerID = boundContext && Number(boundContext.inventoryID)
            ...
            : stationId;
        // An unanchored hull exposes no inventory, and a container still mid-unanchor
        // must refuse rather than serve a half-migrated view.
        if (this._isUnanchoredStructureHullInventoryID(containerID)) {
            return [];
        }
        if (this._isPendingStructureUnanchorContainerInventoryID(containerID)) {
            this._throwPendingStructureUnanchorContainerInventoryError();
        }
        ...
        if (boundContext &&
            ...
            const expectedFlag = boundContext.kind === "capsuleerDeliveries"
            ...
            return listContainerItems(this._getCorporationId(session), this._normalizeInventoryId(boundContext.locationID, stationId), expectedFlag);
        }
        const corporationOffice = this._getCorporationOffice(session, containerID);
        if (corporationOffice) {
            return this._listCorporationOfficeItems(session, corporationOffice, numericFlag);
        }
        if (containerID === stationId) {
            ...
            if (this._isMoonMaterialBayFlag(numericFlag)) {
                this._ensureCanAccessMoonMaterialBay(session, structureContainer, containerID);
        ...
        if (shipContainerRecord && isShipServiceFlag(numericFlag)) {
            const accessError = this._getShipServiceAccessError(session, containerID, numericFlag);
            if (accessError) {
                log.debug(`[InvBroker] Ship service bay list denied ship=${containerID} flag=${numericFlag} error=${accessError}`);
                return [];
            }
            const ownerFilter = this._normalizeInventoryId(shipContainerRecord.ownerID, 0) === charId
                ? charId
                : null;
        ...
        if (genericContainerRecord && !findShipItemById(containerID)) {
            if (this._getSpaceContainerScopeAccessError(session, containerID)) {
                return [];
            }
            const mobileDepotAccessError = this._getMobileDepotCargoAccessError(session, containerID);
            if (mobileDepotAccessError) {
                log.debug(`[InvBroker] Mobile Depot cargo list denied container=${containerID} error=${mobileDepotAccessError}`);
                return [];
            }
            const genericContainerOwnerID = this._normalizeInventoryId(genericContainerRecord.ownerID, 0);
            const isDockedPersonalContainer = stationId > 0 &&
            ...
            return listContainerItems(genericContainerOwnerID === charId ? charId : null, containerID, numericFlag);
        }
        ...
        const containerItems = listContainerItems(this._getCharacterId(session), containerID, numericFlag);
        const shipItem = findShipItemById(containerID);
        if (!shipItem) {
            return containerItems;
        }
        const { filterCreationModuleInventoryItems, } = require(path.join(__dirname, "../frontier/creationRuntime"));
    ...
    _buildInventoryRowDescriptor(columns = INVENTORY_ROW_DESCRIPTOR_COLUMNS, virtualColumns = INVENTORY_ROW_DESCRIPTOR_VIRTUAL_COLUMNS) {
        return buildPackedRowDescriptor(columns, virtualColumns);
    }
    _calculateCapacity(session, boundContext, requestedFlag = null) {
        if (this._isUnanchoredStructureHullInventoryID(boundContext && boundContext.inventoryID)) {
            return this._buildCapacityInfo(0, 0);
        }
        if (this._isPendingStructureUnanchorContainerInventoryID(boundContext && boundContext.inventoryID)) {
            this._throwPendingStructureUnanchorContainerInventoryError();
        }
        ...
        const used = items.reduce((sum, item) => {
            ...
            const units = Number(item.singleton) === 1
                ? 1
                : Math.max(0, Number(item.stacksize ?? item.quantity ?? 0) || 0);
        ...
        if (this._isMoonMaterialBayFlag(numericFlag)) {
            const structure = this._getStructureForInventoryID(boundContext && boundContext.inventoryID);
            return this._buildCapacityInfo(this._getMoonMaterialBayCapacity(structure), used);
        }
        const structureForFighterBayCapacity = Number(numericFlag) === ITEM_FLAGS.FIGHTER_BAY
            ? this._getStructureForInventoryID(boundContext && boundContext.inventoryID)
            : null;
        if (structureForFighterBayCapacity) {
            return this._buildCapacityInfo(this._getStructureFighterBayCapacity(structureForFighterBayCapacity), used);
        }
        const shipRecord = this._getShipInventoryRecord(session, boundContext);
        if (shipRecord) {
            const requiresDerivedShipState = numericFlag === ITEM_FLAGS.CARGO_HOLD ||
                numericFlag === ITEM_FLAGS.DRONE_BAY ||
                numericFlag === ITEM_FLAGS.FIGHTER_BAY ||
            ...
            if (numericFlag === ITEM_FLAGS.CARGO_HOLD) {
                ...
                const derivedCapacity = Number(resourceState.droneCapacity);
                capacity = Number.isFinite(derivedCapacity)
                    ? derivedCapacity
                    : Number(getShipBaseAttributeValue(shipRecord.typeID, "droneCapacity")) || 0;
            }
            else if (numericFlag === ITEM_FLAGS.FIGHTER_BAY) {
                const derivedCapacity = Number(resourceState.fighterCapacity);
                capacity = Number.isFinite(derivedCapacity)
                    ? derivedCapacity
                    : Number(getShipBaseAttributeValue(shipRecord.typeID, "fighterCapacity")) || 0;
            }
            else if (numericFlag === ITEM_FLAGS.SHIP_HANGAR) {
                capacity = Number(getShipBaseAttributeValue(shipRecord.typeID, "shipMaintenanceBayCapacity")) || 0;
            }
            else if (numericFlag === ITEM_FLAGS.FLEET_HANGAR) {
                capacity = Number(getShipBaseAttributeValue(shipRecord.typeID, "fleetHangarCapacity")) || 0;
            }
            else if (numericFlag === ITEM_FLAGS.MOBILE_DEPOT_HOLD) {
                capacity = Number(getShipBaseAttributeValue(shipRecord.typeID, MOBILE_DEPOT_HOLD_CAPACITY_ATTRIBUTE)) || 0;
            }
            else if (isGenericSpecialShipHoldFlag(numericFlag)) {
                capacity = Number(getSpecialShipHoldCapacity(resourceState, shipRecord.typeID, numericFlag, getShipBaseAttributeValue)) || 0;
            }
        }
        else if (boundContext && boundContext.kind === "container") {
            const containerRecord = findItemById(this._normalizeInventoryId(boundContext.inventoryID, 0));
            const nativeWreckRecord = containerRecord ? null : this._getNativeWreckRecord(boundContext.inventoryID);
            const containerMetadata = getItemMetadata((containerRecord && containerRecord.typeID) ||
                (nativeWreckRecord && nativeWreckRecord.typeID), (containerRecord && containerRecord.itemName) ||
    ...
    _buildInvRow(session, overrides = {}) {
        const shipMetadata = this._getShipMetadata(session, overrides.typeID ?? null, overrides.shipName ?? null);
        ...
        const stacksize = overrides.stacksize ?? (singleton > 0 ? 1 : quantity);
    ...
    _itemOverridesFromId(session, itemID) {
        ...
        const shipRecord = findCharacterShip(charId, id) ||
            findShipItemById(id);
        if (shipRecord) {
            return {
                ...
                customInfo: shipRecord.customInfo === undefined ? "" : shipRecord.customInfo,
        ...
        if (genericItem) {
            return {
                ...
                customInfo: genericItem.customInfo === undefined ? "" : genericItem.customInfo,
    ...
    Handle_GetInventory(args, session) {
        ...
        const numericContainerID = containerID === null || containerID === undefined
            ? this._getStationId(session)
            : this._normalizeInventoryId(containerID);
        const stationId = this._getStationId(session);
        if (numericContainerID === CONTAINER_CORP_MARKET_ID ||
            numericContainerID === CONTAINER_CAPSULEER_DELIVERIES_ID) {
            const isCapsuleerDeliveries = numericContainerID === CONTAINER_CAPSULEER_DELIVERIES_ID;
        ...
        const isStructureDocked = Number(session && (session.structureID || session.structureid)) > 0;
        const isStationHangar = numericContainerID === stationId ||
    ...
    _buildInventoryRemoteList(itemOverrides = []) {
        return {
            type: "list",
            items: itemOverrides.map((overrides) => this._buildInventoryPackedRow(overrides)),
        };
    }
    _buildInventoryRemoteSet(itemOverrides = []) {
        return buildObjectEx1("__builtin__.set", [
            buildList(itemOverrides.map((overrides) => this._buildInventoryPackedRow(overrides))),
        ]);
    }
    _buildInventoryPackedRow(overrides = {}) {
        const fields = {
            ...
            customInfo: overrides.customInfo === undefined ? "" : overrides.customInfo,
            stacksize: overrides.stacksize,
            singleton: overrides.singleton,
        };
        if (this._isChargeTupleItemID(overrides.itemID)) {
            return buildPackedRow(CHARGE_SUBLOCATION_ROW_DESCRIPTOR_COLUMNS, fields, INVENTORY_ROW_DESCRIPTOR_VIRTUAL_COLUMNS);
        }
        return buildPackedRow(INVENTORY_ROW_DESCRIPTOR_COLUMNS, fields, INVENTORY_ROW_DESCRIPTOR_VIRTUAL_COLUMNS);
    }
    _buildInvKeyVal(session, overrides = {}) {
        const row = this._buildInvRow(session, overrides);
        const [itemID, typeID, ownerID, locationID, flagID, quantity, groupID, categoryID, customInfo, stacksize, singleton,] = row;
        return {
            type: "object",
            name: "util.KeyVal",
            args: {
    ...
    Handle_GetInventoryFromId(args, session, kwargs) {
        ...
        const boundShip = findCharacterShip(charId, numericItemId) ||
        ...
        const isControlledStructureInventory = this._isControlledStructureInventoryID(session, numericItemId);
        const isKnownInventoryTarget = Boolean(boundShip ||
            genericItem ||
            nativeWreckRecord ||
            structureInventory ||
            corporationOffice ||
            isControlledStructureInventory ||
            (charId > 0 && numericItemId === charId) ||
            (stationId > 0 && numericItemId === stationId));
        if (!isKnownInventoryTarget) {
            this._traceInventory("GetInventoryFromIdNotFound", session, {
                args,
                itemid: numericItemId,
            });
            throwWrappedUserError("FakeItemNotFound");
        }
        if (this._getSpaceContainerScopeAccessError(session, numericItemId)) {
            throwWrappedUserError("FakeItemNotFound");
        }
        const explicitLocationID = this._extractKwarg(kwargs, "locationID") ??
            (args && args.length > 2 ? args[2] : undefined);
        const normalizedExplicitLocationID = explicitLocationID === undefined || explicitLocationID === null
            ? 0
            : this._normalizeInventoryId(explicitLocationID);
        const inheritedLocationID = boundContext &&
        ...
        const shipLocationID = boundShip
            ...
            : 0;
        const resolvedLocationID = corporationOffice
        ...
        log.debug(`[InvBroker] GetInventoryFromId(itemid=${itemid}, locationID=${resolvedLocationID})`);
        const result = this._makeBoundSubstruct({
            inventoryID: corporationOffice
                ? this._normalizeInventoryId(corporationOffice.officeID, numericItemId)
                : itemid,
            locationID: resolvedLocationID,
            flagID: isControlledStructureInventory
                ? null
                :
                                ...
                                :
                                    numericItemId === stationId
                                        ...
                                        : boundShip
                                            ? ITEM_FLAGS.CARGO_HOLD
                                            : null,
            kind: isControlledStructureInventory
        ...
        if (boundShip) {
            log.debug(`[InvBroker] shipInventory bind shipID=${numericItemId} ` +
    ...
    Handle_SetLabel(args, session) {
        ...
        const itemID = this._normalizeInventoryId(unwrapMarshalValue(args && args.length > 0 ? args[0] : 0), 0);
        const item = findItemById(itemID);
        if (!item) {
            log.warn(`[InvBroker] SetLabel failed for missing item=${itemID}`);
            return null;
        }
        if (this._getPendingStructureUnanchorContainerAncestor(item)) {
            this._throwPendingStructureUnanchorContainerInventoryError();
        }
        ...
        const label = Number(item.categoryID) === 6
        ...
        if (!updateResult.success) {
            log.warn(`[InvBroker] SetLabel failed for item=${itemID}: ${updateResult.errorMsg || "UNKNOWN_ERROR"}`);
            return null;
        }
    ...
    Handle_ValidateItemListCanBeOpened(args, session) {
        ...
        if (itemIDs.some((itemID) => this._isUnanchoredStructureHullInventoryID(itemID))) {
            this._throwUnanchoredStructureHullInventoryError();
        }
        if (itemIDs.some((itemID) => this._isPendingStructureUnanchorContainerInventoryID(itemID))) {
            this._throwPendingStructureUnanchorContainerInventoryError();
        }
        return [];
    }
    Handle_List(args, session, kwargs) {
        ...
        const explicitNullFlag = (hasKwFlag && kwFlag === null) ||
            (hasArgFlag && argFlag === null);
        const initialLoginSpaceShipInventoryList = this._isInitialLoginSpaceShipInventoryList(session, boundContext);
        ...
        if (suppressInitialLoginShipList) {
            ...
            log.debug(`[InvBroker] Suppressing initial login-in-space ship List(flag=None) for ship=${boundContext && boundContext.inventoryID}`);
            return this._buildInventoryRowset([]);
        }
        const inSpaceShipInventory = boundContext?.kind === "shipInventory" &&
            !this._getStationId(session);
        const requestedFlag = boundContext?.kind === "shipInventory" &&
            (explicitNullFlag ||
        ...
        log.debug(`[InvBroker] List (inventory contents) flag=${requestedFlag} bound=${JSON.stringify(boundContext)}`);
        const itemsForContainer = this._mergeActiveShipLoadedChargeTupleRows(session, boundContext, requestedFlag, this._resolveContainerItems(session, requestedFlag, boundContext));
        this._primeInSpaceBayDogmaItems(session, boundContext, ITEM_FLAGS.DRONE_BAY, itemsForContainer);
        const dockedShipDefaultList = Boolean(boundContext &&
            boundContext.kind === "shipInventory" &&
            requestedFlag === null &&
            this._getStationId(session) > 0);
        const controlledStructureDefaultList = Boolean(boundContext &&
            boundContext.kind === "controlledStructureInventory" &&
            requestedFlag === null);
        const itemOverrides = itemsForContainer
            ...
            .filter(Boolean)
            .map((overrides) => {
            if (controlledStructureDefaultList) {
                return { ...overrides, customInfo: "" };
            }
            if (dockedShipDefaultList &&
                Number(overrides.categoryID) !== 1 &&
                overrides.customInfo === "") {
                return { ...overrides, customInfo: null };
            }
            return overrides;
        });
        ...
        if (boundContext && boundContext.kind === "shipInventory") {
            ...
            log.debug(`[InvBroker] shipInventory List summary shipID=${Number(boundContext.inventoryID) || 0} ` +
        ...
        const shouldReturnPythonSet = (itemOverrides.length === 0 && requestedFlag !== null && requestedFlag !== undefined) ||
            dockedShipDefaultList;
        const result = shouldReturnPythonSet
            ? this._buildInventoryRemoteSet(itemOverrides)
            : this._buildInventoryRemoteList(itemOverrides);
        if (this._shouldPrimeLoginShipInventoryBootstrap(session, boundContext, {
    ...
    Handle_ListByFlags(args, session, kwargs) {
        const boundContext = this._getBoundContext(session);
        const rawFlags = (args && args.length > 0 ? args[0] : null) ??
        ...
        log.debug(`[InvBroker] ListByFlags(flags=${requestedFlags.join(",")}) bound=${JSON.stringify(boundContext)}`);
        for (const requestedFlag of requestedFlags) {
            const itemsForFlag = this._resolveContainerItems(session, requestedFlag, boundContext);
            ...
            for (const item of itemsForFlag) {
                ...
                seenItemIds.add(itemID);
                const itemOverridesForRecord = this._buildInventoryItemOverrides(session, item);
                if (itemOverridesForRecord) {
                    itemOverrides.push(itemOverridesForRecord);
                }
            }
        }
        this._primeInSpaceBayDogmaItems(session, boundContext, ITEM_FLAGS.DRONE_BAY, dogmaPrimeItems);
    ...
    async Handle_ImportExportWithPlanet(args, session) {
        const boundContext = this._getBoundContext(session);
        const customsOfficeID = this._normalizeInventoryId(boundContext && (boundContext.inventoryID ?? boundContext.locationID), 0);
        const characterID = this._getCharacterId(session);
        const spaceportPinID = this._normalizeInventoryId(unwrapMarshalValue(args && args.length > 0 ? args[0] : 0), 0);
        ...
        if (!Number.isFinite(clientTaxRate) ||
            Math.abs(clientTaxRate - currentTaxRate) > 0.000001) {
            throwWrappedUserError("TaxChanged");
        }
        const colony = await planetRuntimeStore.getColonyByPinAsync(characterID, spaceportPinID);
        if (!colony) {
            throwWrappedUserError("CannotImportEndpointNotFound");
        }
        const importInfo = this._buildPIImportCommoditiesFromItems(importData, customsOfficeID, characterID);
        ...
        const importTax = planetCostCalculator.calculateImportTax(preview.spaceportPin && preview.spaceportPin.typeID, preview.importCommodities, currentTaxRate);
        const exportTax = planetCostCalculator.calculateExportTax(preview.spaceportPin && preview.spaceportPin.typeID, preview.exportCommodities, currentTaxRate);
        ...
        try {
            ...
            for (const [typeID, quantity] of Object.entries(preview.importCommodities)) {
                const takeResult = takeItemTypeFromCharacterLocation(characterID, customsOfficeID, ITEM_FLAGS.HANGAR, typeID, quantity);
            ...
            for (const [typeID, quantity] of Object.entries(preview.exportCommodities)) {
                const grantResult = grantItemToCharacterLocation(characterID, customsOfficeID, ITEM_FLAGS.HANGAR, typeID, quantity, { singleton: 0 });
                if (!grantResult.success) {
                    throwWrappedUserError("CannotExportCommodities");
                }
                inventoryChanges.push(...((grantResult.data && grantResult.data.changes) || []).map((change) => ({
                    ...change,
                    previousData: {
            ...
            debits.push(this._debitPIWallet(characterID, importTax, JOURNAL_ENTRY_TYPE.PLANETARY_IMPORT_TAX, colony.planetID, "Planetary import tax"));
            debits.push(...this._debitPIWalletInLegs(characterID, exportTax, JOURNAL_ENTRY_TYPE.PLANETARY_EXPORT_TAX, colony.planetID, "Planetary export tax", 2));
            return null;
        }
        catch (error) {
            this._refundPIWalletDebits(characterID, debits, "Planetary import/export tax refund");
            throw error;
        }
    }
    Handle_GetItem(args, session) {
        const boundContext = this._getBoundContext(session);
        const itemID = args && args.length > 0
            ? args[0]
            : boundContext && boundContext.inventoryID
        ...
        const numericItemID = this._normalizeInventoryId(itemID);
        if (this._isPendingStructureUnanchorContainerInventoryID(numericItemID)) {
            this._throwPendingStructureUnanchorContainerInventoryError();
        }
        ...
        const shipRecord = findCharacterShip(this._getCharacterId(session), numericItemID);
    ...
    Handle_GetItems(args, session) {
        ...
        if (ids.some((itemID) => this._isPendingStructureUnanchorContainerInventoryID(itemID))) {
            this._throwPendingStructureUnanchorContainerInventoryError();
        }
        const items = ids.map((id) => this._buildInvItem(session, this._itemOverridesFromId(session, id)));
    ...
    _getCrystalDamageRatioForItem(item, characterID) {
        ...
        return Math.max(0, Math.min(1, Number(item.moduleState && item.moduleState.damage) || 0));
    }
    Handle_GetDamageForCrystals(args, session) {
        ...
        return buildDict(crystalIDs
            .map((itemID) => {
            const damageRatio = this._getCrystalDamageRatioForItem(findItemById(itemID), characterID);
    ...
    Handle_GetSelfInvItem(args, session) {
        const boundContext = this._getBoundContext(session);
        const inventoryID = boundContext && boundContext.inventoryID !== null && boundContext.inventoryID !== undefined
            ? boundContext.inventoryID
            : this._getShipId(session);
        if (this._isPendingStructureUnanchorContainerInventoryID(inventoryID)) {
            this._throwPendingStructureUnanchorContainerInventoryError();
        }
        const overrides = this._getStructureForInventoryID(inventoryID)
            ? this._buildStructureItemOverrides(session, inventoryID)
            : this._buildContainerItemOverrides(session, inventoryID);
        ...
        return this._buildInventoryPackedRow(overrides);
    }
    Handle_StripFitting(args, session) {
        ...
        const firstNonChargeFittedItem = fittedItems.find((fittedItem) => (fittedItem &&
        ...
        if (firstNonChargeFittedItem) {
            const fittingServiceAccess = this._validateInSpaceFittingServiceAccess(session, shipRecord, firstNonChargeFittedItem, {
                locationID: shipRecord.locationID,
                flagID: ITEM_FLAGS.HANGAR,
            });
            if (!fittingServiceAccess.success) {
                log.debug(`[InvBroker] StripFitting rejected before moving items shipID=${shipRecord.itemID} error=${fittingServiceAccess.errorMsg}`);
        ...
        for (const fittedItem of fittedItems) {
            ...
            const fittingServiceAccess = this._validateInSpaceFittingServiceAccess(session, shipRecord, fittedItem, {
                locationID: shipRecord.locationID,
                flagID: ITEM_FLAGS.HANGAR,
            });
            if (!fittingServiceAccess.success) {
                log.debug(`[InvBroker] StripFitting rejected itemID=${fittedItem.itemID} error=${fittingServiceAccess.errorMsg}`);
                this._throwMobileDepotFittingAccessError(fittingServiceAccess.errorMsg);
            }
            const sourceItemDescriptor = this._findTransferSourceItem(fittedItem.itemID, shipRecord.itemID);
            const moveResult = this._moveSourceItemToDestination(session, sourceItemDescriptor, {
                locationID: shipRecord.locationID,
                flagID: ITEM_FLAGS.HANGAR,
            });
    ...
    Handle_DestroyFitting(args, session) {
        ...
        const boundContext = this._getBoundContext(session);
        const boundInventoryID = this._normalizeInventoryId(boundContext && boundContext.inventoryID, 0);
        ...
        if (!boundContext ||
            ...
            log.debug(`[InvBroker] DestroyFitting rejected: itemID=${itemID} locationID=${item.locationID} boundInventoryID=${boundInventoryID || 0}`);
            return null;
        }
    ...
    Handle_TrashItems(args, session) {
        ...
        const itemIDs = this._normalizeItemIdList(args && args.length > 0 ? args[0] : []);
        const requestedLocationID = this._normalizeInventoryId(args && args.length > 1 ? args[1] : this._getStationId(session), this._getStationId(session));
        if (itemIDs.length === 0) {
            return null;
        }
        const inventoryItems = itemIDs.map((itemID) => findItemById(itemID));
        if (inventoryItems.some((item) => this._getPendingStructureUnanchorContainerAncestor(item))) {
            this._throwPendingStructureUnanchorContainerInventoryError();
        }
        const hasInvalidItem = inventoryItems.some((item) => !this._isInventoryItemTrashable(session, item, requestedLocationID));
        if (hasInvalidItem) {
    ...
    _fitFittingQuantityItemsToShipBay({ session, shipRecord, sourceLocationID, itemsByType, entriesByType, destinationFlagID, itemLabel, allChanges, missingByType, isItemAllowed = null, } = {}) {
        if (!shipRecord ||
        ...
        for (const entry of entriesByType) {
            ...
            const existingQuantity = this._getFitFittingExistingDestinationQuantity(session, shipRecord, destinationFlagID, entry.typeID);
            ...
            while (remainingQuantity > 0 && candidateItemIDs.length > 0) {
                ...
                const shipServiceAccess = this._validateShipServiceTransferAccess(session, item, destination);
                if (!shipServiceAccess.success) {
                    log.debug(`[InvBroker] FitFitting rejected ${itemLabel} itemID=${itemID} shipService=${shipServiceAccess.shipID} error=${shipServiceAccess.errorMsg}`);
                ...
                if (!fittingServiceAccess.success) {
                    log.debug(`[InvBroker] FitFitting rejected ${itemLabel} itemID=${itemID} destination=${destination.locationID}:${destination.flagID} error=${fittingServiceAccess.errorMsg}`);
                    this._throwMobileDepotFittingAccessError(fittingServiceAccess.errorMsg);
                }
                const dockedStructureFittingAccess = this._validateDockedStructureFittingServiceAccess(session, shipRecord, item, destination);
                if (!dockedStructureFittingAccess.success) {
                    log.debug(`[InvBroker] FitFitting rejected ${itemLabel} itemID=${itemID} structure=${dockedStructureFittingAccess.structureID} destination=${destination.locationID}:${destination.flagID} error=${dockedStructureFittingAccess.errorMsg}`);
                    this._throwDockedStructureFittingAccessError(dockedStructureFittingAccess.errorMsg);
                }
                const shipBayValidation = this._validateShipBayMove(session, shipRecord, item, destination);
                ...
                const capacityCheck = this._checkCapacityForMove(session, { kind: "shipInventory", inventoryID: shipRecord.itemID, flagID: destinationFlagID }, destination, item, moveQuantity);
                if (!capacityCheck.success) {
                    continue;
                }
                const moveResult = this._moveSourceItemToDestination(session, sourceItemDescriptor, destination, moveQuantity);
                if (!moveResult.success) {
                    continue;
                }
                allChanges.push(...((moveResult.data && moveResult.data.changes) || []));
            ...
            if (remainingQuantity > 0) {
                missingByType.set(entry.typeID, (missingByType.get(entry.typeID) || 0) + remainingQuantity);
            }
        }
        return fittedCount;
    }
    Handle_GetContainerContents(args, session) {
        const containerID = args && args.length > 0 ? args[0] : this._getStationId(session);
        ...
        const mobileDepotAccessError = this._getMobileDepotCargoAccessError(session, numericContainerID);
        const scopeAccessError = this._getSpaceContainerScopeAccessError(session, numericContainerID);
        this._traceInventory("GetContainerContents", session, { args });
        log.debug(`[InvBroker] GetContainerContents(containerID=${numericContainerID}, locationID=${locationID})`);
        if (this._isUnanchoredStructureHullInventoryID(numericContainerID)) {
            return this._buildInventoryRowset([]);
        }
        if (this._isPendingStructureUnanchorContainerInventoryID(numericContainerID)) {
            this._throwPendingStructureUnanchorContainerInventoryError();
        }
        if (mobileDepotAccessError) {
            log.debug(`[InvBroker] Mobile Depot contents denied container=${numericContainerID} error=${mobileDepotAccessError}`);
        }
        const nativeWreckRecord = this._getNativeWreckRecord(numericContainerID);
        const items = mobileDepotAccessError || scopeAccessError
            ? []
            :
                        ...
                        corporationOffice
                            ...
                            :
                                numericContainerID === stationId
                                    ? listContainerItems(this._getCharacterId(session), stationId, ITEM_FLAGS.HANGAR)
                                    : nativeWreckRecord
                                        ? nativeNpcStore.buildNativeWreckContents(numericContainerID)
                                        : isMobileDepotItemRecord(genericContainerRecord)
                                            ? listContainerItems(null, numericContainerID, null)
                                            : listContainerItems(this._getGenericContainerContentsOwnerID(session, genericContainerRecord), numericContainerID, null);
        this._traceInventory("GetContainerContentsResult", session, {
            containerID: numericContainerID,
            count: items.length,
            firstItem: items[0] || null,
        });
        return this._buildInventoryRowset(items
            .map((item) => this._buildInventoryItemOverrides(session, item))
            .filter(Boolean)
            .map((overrides) => this._buildInvRow(session, overrides)));
    }
    Handle_GetCapacity(args, session, kwargs) {
        const boundContext = this._getBoundContext(session);
        const requestedFlag = (args && args.length > 0 ? args[0] : null) ??
        ...
        log.debug(`[InvBroker] GetCapacity(flag=${String(requestedFlag)}) bound=${JSON.stringify(boundContext)}`);
    ...
    Handle_StackAll(args, session, kwargs) {
        ...
        const requestedFlag = (args && args.length > 0 ? args[0] : null) ??
            this._extractKwarg(kwargs, "flag") ??
            (boundContext ? boundContext.flagID : null);
        const containerID = this._normalizeInventoryId(boundContext && boundContext.inventoryID, this._getStationId(session));
        const flagID = requestedFlag === null || requestedFlag === undefined
            ? boundContext && boundContext.flagID !== null && boundContext.flagID !== undefined
        ...
        const allChanges = [];
        let mergedCount = 0;
        log.debug(`[InvBroker] StackAll container=${containerID} flag=${flagID} count=${items.length}`);
    ...
    Handle_MultiMerge(args, session, kwargs) {
        ...
        const sourceContainerID = this._normalizeInventoryId(args && args.length > 1 ? args[1] : 0, 0);
        const allChanges = [];
        let mergedCount = 0;
        log.debug(`[InvBroker] MultiMerge opCount=${ops.length} sourceContainerID=${sourceContainerID}`);
        if (this._isPendingStructureUnanchorContainerInventoryID(sourceContainerID)) {
            this._throwPendingStructureUnanchorContainerInventoryError();
        }
        for (const op of ops) {
            ...
            if (this._getPendingStructureUnanchorContainerAncestor(sourceItem) ||
                this._getPendingStructureUnanchorContainerAncestor(destinationItem)) {
                this._throwPendingStructureUnanchorContainerInventoryError();
            }
        }
        for (const op of ops) {
            ...
            const mergeResult = mergeItemStacks(op.sourceItemID, op.destinationItemID, op.quantity);
            if (!mergeResult.success) {
                continue;
            }
            mergedCount += 1;
    ...
    Handle_Add(args, session, kwargs) {
        ...
        const boundContext = this._getBoundContext(session);
        const itemID = this._normalizeInventoryId(args && args.length > 0 ? args[0] : 0, 0);
        const sourceLocationID = this._normalizeInventoryId(args && args.length > 1 ? args[1] : 0, 0);
        const explicitFlagValue = this._extractKwarg(kwargs, "flag");
        const explicitFlagProvided = explicitFlagValue !== undefined;
        const requestedFlag = explicitFlagProvided
            ? this._normalizeInventoryId(explicitFlagValue, 0)
            : boundContext && boundContext.flagID !== null && boundContext.flagID !== undefined
                ? this._normalizeInventoryId(boundContext.flagID, 0)
                : null;
        const quantity = this._normalizeQuantityArg(this._extractKwarg(kwargs, "qty") ?? this._extractKwarg(kwargs, "quantity"));
        const sourceItemDescriptor = this._findTransferSourceItem(itemID, sourceLocationID);
        const item = sourceItemDescriptor && sourceItemDescriptor.item
            ? sourceItemDescriptor.item
            : null;
        const fleetLootEntries = [];
        log.debug(`[InvBroker] Add itemID=${itemID} source=${sourceLocationID} requestedFlag=${String(requestedFlag)} bound=${JSON.stringify(boundContext)}`);
        if (!boundContext || !item || !sourceItemDescriptor) {
            return null;
        }
        if (!this._isTransferSourceLocationMatch(sourceItemDescriptor, sourceLocationID)) {
            log.debug(`[InvBroker] Add rejected itemID=${itemID} source=${sourceLocationID} error=SOURCE_LOCATION_MISMATCH`);
            return null;
        }
        if (isDirectSpaceUnanchoredStructureHullInventoryRecord(item)) {
            this._throwUnanchoredStructureHullInventoryError();
        }
        if (this._getPendingStructureUnanchorContainerAncestor(item)) {
            this._throwPendingStructureUnanchorContainerInventoryError();
        ...
        if (!this._canTakeFromCorporationHangarSource(session, sourceItemDescriptor)) {
            log.debug(`[InvBroker] Add rejected itemID=${itemID} source=${sourceLocationID} error=CORP_HANGAR_TAKE_DENIED`);
        ...
        if (SLOT_FAMILY_FLAGS.rig.includes(Number(item.flagID))) {
            log.debug(`[InvBroker] Add rejected itemID=${itemID} — item is a rig in slot flagID=${item.flagID} and cannot be unfit to inventory`);
            return null;
        }
        const destination = this._resolveDestinationForMove(session, boundContext, item, requestedFlag, explicitFlagProvided);
        if (!destination) {
            log.debug(`[InvBroker] Add rejected itemID=${itemID} source=${sourceLocationID} requestedFlag=${String(requestedFlag)} error=NO_SUITABLE_FIT_SLOT`);
            throwWrappedUserError("ModuleFitFailed", {
                moduleName: Number(item.typeID) || 0,
                reason: "No suitable slot available",
            });
        }
        const moonMaterialBayDestination = this._validateMoonMaterialBayDestination(session, destination);
        if (!moonMaterialBayDestination.success) {
            log.debug(`[InvBroker] Add rejected itemID=${itemID} destination=${destination.locationID}:${destination.flagID} error=${moonMaterialBayDestination.errorMsg}`);
            return null;
        }
        const shipServiceAccess = this._validateShipServiceTransferAccess(session, item, destination);
        if (!shipServiceAccess.success) {
            log.debug(`[InvBroker] Add rejected itemID=${itemID} shipService=${shipServiceAccess.shipID} error=${shipServiceAccess.errorMsg}`);
            this._throwShipServiceAccessError(shipServiceAccess.errorMsg);
        }
        const mobileDepotAccess = this._validateMobileDepotCargoTransferAccess(session, item, destination);
        if (!mobileDepotAccess.success) {
            log.debug(`[InvBroker] Add rejected itemID=${itemID} mobileDepot=${mobileDepotAccess.containerID} error=${mobileDepotAccess.errorMsg}`);
        ...
        const fittingServiceAccess = this._validateInSpaceFittingServiceAccess(session, shipRecord, item, destination);
        if (!fittingServiceAccess.success) {
            log.debug(`[InvBroker] Add rejected itemID=${itemID} destination=${destination.locationID}:${destination.flagID} error=${fittingServiceAccess.errorMsg}`);
            this._throwMobileDepotFittingAccessError(fittingServiceAccess.errorMsg);
        }
        const dockedStructureFittingAccess = this._validateDockedStructureFittingServiceAccess(session, shipRecord, item, destination);
        if (!dockedStructureFittingAccess.success) {
            log.debug(`[InvBroker] Add rejected itemID=${itemID} structure=${dockedStructureFittingAccess.structureID} destination=${destination.locationID}:${destination.flagID} error=${dockedStructureFittingAccess.errorMsg}`);
            this._throwDockedStructureFittingAccessError(dockedStructureFittingAccess.errorMsg);
        }
        const structureDeedBaySourceValidation = this._validateStructureDeedBaySourceMove(item);
        if (!structureDeedBaySourceValidation.success) {
            log.debug(`[InvBroker] Add rejected itemID=${itemID} source=${item.locationID}:${item.flagID} error=${structureDeedBaySourceValidation.errorMsg}`);
            throwWrappedUserError(structureDeedBaySourceValidation.errorMsg, structureDeedBaySourceValidation.values || {
                type: Number(item.typeID) || 0,
            });
        }
        const chargeLoadResult = this._tryLoadChargeFromInventoryAdd(session, sourceItemDescriptor, destination, quantity);
        if (chargeLoadResult.handled) {
            if (!chargeLoadResult.success) {
                log.warn(`[InvBroker] Add charge load failed itemID=${itemID} destination=${destination.locationID}:${destination.flagID} error=${chargeLoadResult.errorMsg}`);
                return null;
            }
            const chargeChanges = chargeLoadResult.data && Array.isArray(chargeLoadResult.data.changes)
                ? chargeLoadResult.data.changes
                : [];
            if (chargeChanges.length > 0) {
                this._appendFleetLootEntry(session, fleetLootEntries, sourceItemDescriptor, sourceLocationID, destination, chargeLoadResult.data.quantity);
        ...
        const fitValidation = this._validateFittingMove(session, fitHostRecord, item, destination, fitHostRecord
            ? this._listFittedItemsForFitHost(session, fitHostRecord)
            : null);
        if (!fitValidation.success) {
            log.debug(`[InvBroker] Add rejected itemID=${itemID} destination=${destination.locationID}:${destination.flagID} error=${fitValidation.errorMsg}`);
            this._throwFittingMoveUserError(fitValidation, fitHostRecord, item);
            return null;
        }
        const structureCoreValidation = this._validateStructureCoreInstallMove(item, destination);
        if (!structureCoreValidation.success) {
            throwWrappedUserError(structureCoreValidation.errorMsg, structureCoreValidation.values || {
                type: Number(item.typeID) || 0,
            });
        }
        const shipBayValidation = this._validateShipBayMove(session, shipRecord, item, destination);
        if (!shipBayValidation.success) {
            log.debug(`[InvBroker] Add rejected itemID=${itemID} destination=${destination.locationID}:${destination.flagID} error=${shipBayValidation.errorMsg}`);
            throwWrappedUserError(shipBayValidation.errorMsg, {
                type: Number(item.typeID) || 0,
            });
        }
        const structureOwnerBayValidation = this._validateStructureOwnerBayMove(item, destination);
        if (!structureOwnerBayValidation.success) {
            throwWrappedUserError(structureOwnerBayValidation.errorMsg, {
                type: Number(item.typeID) || 0,
            });
        }
        const structureServiceModuleRemovalAccess = this._validateStructureServiceModuleRemovalAccess(session, item, destination);
        if (!structureServiceModuleRemovalAccess.success) {
            log.debug(`[InvBroker] Add rejected itemID=${itemID} structure=${structureServiceModuleRemovalAccess.structureID || 0} error=${structureServiceModuleRemovalAccess.errorMsg}`);
            throwWrappedUserError("CrpAccessDenied", buildCrpAccessDeniedInsufficientRolesValues());
        }
        const appliedQuantity = this._resolveAppliedMoveQuantity(item, destination, quantity);
        const capacityCheck = this._checkCapacityForMove(session, boundContext, destination, item, appliedQuantity);
        if (!capacityCheck.success) {
            log.debug(`[InvBroker] Add rejected itemID=${itemID} destination=${destination.locationID}:${destination.flagID} error=${capacityCheck.errorMsg}`);
            if (capacityCheck.errorMsg === "NotEnoughCargoSpace") {
                throwWrappedUserError("NotEnoughCargoSpace", this._buildCapacityMoveUserErrorValues(capacityCheck, item));
            }
            throwWrappedUserError(capacityCheck.errorMsg, this._buildCapacityMoveUserErrorValues(capacityCheck, item));
        }
        const moveResult = this._moveSourceItemToDestination(session, sourceItemDescriptor, destination, appliedQuantity);
        if (!moveResult.success) {
            log.warn(`[InvBroker] Add failed itemID=${itemID} destination=${destination.locationID}:${destination.flagID} error=${moveResult.errorMsg}`);
            return null;
        }
        this._appendFleetLootEntry(session, fleetLootEntries, sourceItemDescriptor, sourceLocationID, destination, quantity);
        const isStructureCoreInstallMove = this._isStructureCoreInstallMove(item, destination);
        ...
        const isFittingMove = isShipFittingFlag(destination.flagID) || isShipFittingFlag(item.flagID);
        const isMtuSpaceComponentLootMove = this._isMtuSpaceComponentLootMove(session, sourceLocationID, destination);
        const isPlayerJetcanLootMove = this._isPlayerJetcanLootMove(session, sourceLocationID, destination);
        if (isFittingMove) {
            ...
            const emittedCorpStructureMove = this._emitCorpStructureHangarMoveChanges(session, moveChanges);
            const emittedStructureCoreMove = !emittedCorpStructureMove &&
                isStructureCoreInstallMove &&
                this._emitStructureCoreInstallMoveChanges(session, moveChanges, destination, item);
            if (!emittedCorpStructureMove && !emittedStructureCoreMove) {
                const emitItemsChangedBatch = isMtuSpaceComponentLootMove ||
            ...
            this._emitInventoryMoveChanges(session, this._resolveShipModuleFitOnlineChanges(session, item, destination));
        }
        ...
        const sourceContainerIDs = [
            ...new Set(((moveResult.data && moveResult.data.changes) || [])
                .filter((c) => c && c.previousData && Number(c.previousData.locationID) > 0)
                .map((c) => Number(c.previousData.locationID))),
        ];
    ...
    Handle_MultiAdd(args, session, kwargs) {
        ...
        const boundContext = this._getBoundContext(session);
        const itemIDs = [
            ...new Set(this._normalizeItemIdList(args && args.length > 0 ? args[0] : [])),
        ];
        const sourceLocationID = this._normalizeInventoryId(args && args.length > 1 ? args[1] : 0, 0);
        const explicitFlagValue = this._extractKwarg(kwargs, "flag");
        const explicitFlagProvided = explicitFlagValue !== undefined;
        ...
        log.debug(`[InvBroker] MultiAdd itemCount=${itemIDs.length} source=${sourceLocationID} requestedFlag=${String(requestedFlag)} bound=${JSON.stringify(boundContext)}`);
        if (!boundContext || itemIDs.length === 0) {
            return null;
        }
        // Loot All must be capacity-atomic. MultiAdd persists each successful item
        // immediately, so an item-by-item capacity check can otherwise throw after
        // earlier rows have already moved. Preflight the cumulative active-ship
        // cargo volume before the first mutation.
        const lootCapacityPreflight = this._preflightSpaceLootMultiAddCapacity(session, boundContext, itemIDs, sourceLocationID, requestedFlag, explicitFlagProvided, quantity, fittedItemsSnapshot);
        if (lootCapacityPreflight.handled && !lootCapacityPreflight.success) {
            log.debug(`[InvBroker] MultiAdd rejected loot batch source=${sourceLocationID} destination=${this._getShipId(session)}:${ITEM_FLAGS.CARGO_HOLD} error=${lootCapacityPreflight.errorMsg} required=${lootCapacityPreflight.requiredVolume} free=${lootCapacityPreflight.free}`);
            if (lootCapacityPreflight.errorMsg === "SOURCE_ITEM_NOT_FOUND" ||
                lootCapacityPreflight.errorMsg === "SOURCE_LOCATION_MISMATCH") {
                return null;
            }
            throwWrappedUserError(lootCapacityPreflight.errorMsg, this._buildCapacityMoveUserErrorValues(lootCapacityPreflight, lootCapacityPreflight.item));
        }
        for (const itemID of itemIDs) {
            const sourceItemDescriptor = this._findTransferSourceItem(itemID, sourceLocationID);
            if (sourceItemDescriptor &&
                sourceItemDescriptor.item &&
                this._isTransferSourceLocationMatch(sourceItemDescriptor, sourceLocationID) &&
                isDirectSpaceUnanchoredStructureHullInventoryRecord(sourceItemDescriptor.item)) {
                this._throwUnanchoredStructureHullInventoryError();
            }
            if (sourceItemDescriptor &&
                sourceItemDescriptor.item &&
                this._isTransferSourceLocationMatch(sourceItemDescriptor, sourceLocationID) &&
                this._getPendingStructureUnanchorContainerAncestor(sourceItemDescriptor.item)) {
                this._throwPendingStructureUnanchorContainerInventoryError();
            }
        }
        for (const itemID of itemIDs) {
            ...
            const moonMaterialBayDestination = this._validateMoonMaterialBayDestination(session, destination);
            if (!moonMaterialBayDestination.success) {
                log.debug(`[InvBroker] MultiAdd rejected itemID=${itemID} destination=${destination.locationID}:${destination.flagID} error=${moonMaterialBayDestination.errorMsg}`);
                continue;
            }
            const shipServiceAccess = this._validateShipServiceTransferAccess(session, item, destination);
            if (!shipServiceAccess.success) {
                log.debug(`[InvBroker] MultiAdd rejected itemID=${itemID} shipService=${shipServiceAccess.shipID} error=${shipServiceAccess.errorMsg}`);
                if (movedCount > 0) {
                    continue;
                }
                this._throwShipServiceAccessError(shipServiceAccess.errorMsg);
            ...
            const mobileDepotAccess = this._validateMobileDepotCargoTransferAccess(session, item, destination);
            if (!mobileDepotAccess.success) {
                log.debug(`[InvBroker] MultiAdd rejected itemID=${itemID} mobileDepot=${mobileDepotAccess.containerID} error=${mobileDepotAccess.errorMsg}`);
                this._throwMobileDepotCargoAccessError(mobileDepotAccess.errorMsg);
            }
            const spaceContainerScopeAccess = this._validateSpaceContainerScopeTransferAccess(session, sourceLocationID, destination.locationID);
            if (!spaceContainerScopeAccess.success) {
                log.debug(`[InvBroker] MultiAdd rejected itemID=${itemID} spaceContainer=${spaceContainerScopeAccess.containerID} error=${spaceContainerScopeAccess.errorMsg}`);
                if (movedCount > 0) {
                    continue;
                }
                this._throwSpaceContainerScopeAccessError(spaceContainerScopeAccess.errorMsg);
            ...
            const fittingServiceAccess = this._validateInSpaceFittingServiceAccess(session, shipRecord, item, destination);
            if (!fittingServiceAccess.success) {
                log.debug(`[InvBroker] MultiAdd rejected itemID=${itemID} destination=${destination.locationID}:${destination.flagID} error=${fittingServiceAccess.errorMsg}`);
                this._throwMobileDepotFittingAccessError(fittingServiceAccess.errorMsg);
            }
            const dockedStructureFittingAccess = this._validateDockedStructureFittingServiceAccess(session, shipRecord, item, destination);
            if (!dockedStructureFittingAccess.success) {
                log.debug(`[InvBroker] MultiAdd rejected itemID=${itemID} structure=${dockedStructureFittingAccess.structureID} destination=${destination.locationID}:${destination.flagID} error=${dockedStructureFittingAccess.errorMsg}`);
                if (movedCount > 0) {
                    continue;
                }
                this._throwDockedStructureFittingAccessError(dockedStructureFittingAccess.errorMsg);
            ...
            if (chargeLoadResult.handled) {
                if (!chargeLoadResult.success) {
                    log.debug(`[InvBroker] MultiAdd charge load rejected itemID=${itemID} destination=${destination.locationID}:${destination.flagID} error=${chargeLoadResult.errorMsg}`);
                    continue;
                }
                const chargeChanges = chargeLoadResult.data && Array.isArray(chargeLoadResult.data.changes)
                    ? chargeLoadResult.data.changes
            ...
            const fitValidation = this._validateFittingMove(session, fitHostRecord, item, destination, fittedItemsSnapshot);
            if (!fitValidation.success) {
            ...
            const structureServiceModuleRemovalAccess = this._validateStructureServiceModuleRemovalAccess(session, item, destination);
            if (!structureServiceModuleRemovalAccess.success) {
                log.debug(`[InvBroker] MultiAdd rejected itemID=${itemID} structure=${structureServiceModuleRemovalAccess.structureID || 0} error=${structureServiceModuleRemovalAccess.errorMsg}`);
                if (movedCount > 0) {
                    continue;
                }
                throwWrappedUserError("CrpAccessDenied", buildCrpAccessDeniedInsufficientRolesValues());
            ...
            const capacityCheck = this._checkCapacityForMove(session, boundContext, destination, item, appliedQuantity);
            if (!capacityCheck.success) {
                log.debug(`[InvBroker] MultiAdd rejected itemID=${itemID} destination=${destination.locationID}:${destination.flagID} error=${capacityCheck.errorMsg}`);
                if (capacityCheck.errorMsg === "NotEnoughCargoSpace") {
                    throwWrappedUserError("NotEnoughCargoSpace", this._buildCapacityMoveUserErrorValues(capacityCheck, item));
                }
                throwWrappedUserError(capacityCheck.errorMsg, this._buildCapacityMoveUserErrorValues(capacityCheck, item));
            ...
            this._syncStructureCoreInstall(session, item, destination);
            this._syncStructureServiceModuleMove(session, item, destination);
            allChanges.push(...this._resolveShipModuleFitOnlineChanges(session, item, destination));
            if (fitHostRecord &&
                destination.locationID === fitHostRecord.itemID &&
                isShipFittingFlag(destination.flagID)) {
                const movedItemID = this._resolveMovedItemID(moveResult, itemID, destination) || itemID;
        ...
        if (movedCount <= 0) {
            this._throwFittingMoveUserError(firstFitValidationFailure && firstFitValidationFailure.fitValidation, firstFitValidationFailure && firstFitValidationFailure.fitHostRecord, firstFitValidationFailure && firstFitValidationFailure.item);
            return null;
        }
        this._emitFittingMoveChanges(session, allChanges);
        this._emitFleetLootEvents(session, fleetLootEntries);
        // If any source was a temporary space container, despawn it when now empty.
        const sourceContainerIDs = new Set(allChanges
            .filter((c) => c && c.previousData && Number(c.previousData.locationID) > 0)
            .map((c) => Number(c.previousData.locationID)));
        for (const cid of sourceContainerIDs) {
            maybeExpireEmptySpaceContainer(session, cid);
    ...
    Handle_FitFitting(args, session, kwargs) {
        ...
        const sourceLocationID = this._normalizeInventoryId(args && args.length > 3 ? args[3] : this._getStationId(session), this._getStationId(session));
        const itemsByType = this._normalizeFitFittingItemsByType(args && args.length > 2 ? args[2] : {});
        const modulesByFlag = this._normalizeFitFittingModulesByFlag(args && args.length > 4 ? args[4] : {});
        const cargoItemsByType = this._normalizeFitFittingQuantitiesByType(args && args.length > 5 ? args[5] : {}, null);
        const fitRigs = args && args.length > 6
            ? Boolean(unwrapMarshalValue(args[6]))
            : true;
        const dronesByType = this._normalizeFitFittingQuantitiesByType(args && args.length > 4 ? args[4] : {}, "dronesByType");
        const chargesByType = this._normalizeFitFittingQuantitiesByType(args && args.length > 4 ? args[4] : {}, "chargesByType");
        const fightersByType = this._normalizeFitFittingQuantitiesByType(args && args.length > 4 ? args[4] : {}, "fightersByTypeID");
        const iceByType = this._normalizeFitFittingQuantitiesByType(args && args.length > 4 ? args[4] : {}, "iceByType");
        const implantsByType = this._normalizeFitFittingQuantitiesByType(args && args.length > 4 ? args[4] : {}, "implantsByTypeID");
        const shipRecord = findCharacterShip(this._getCharacterId(session), shipID) ||
        ...
        if (this._isPendingStructureUnanchorContainerInventoryID(sourceLocationID)) {
            this._throwPendingStructureUnanchorContainerInventoryError();
        }
        for (const candidateItemIDs of itemsByType.values()) {
            if (candidateItemIDs.some((itemID) => this._isPendingStructureUnanchorContainerInventoryID(itemID))) {
                this._throwPendingStructureUnanchorContainerInventoryError();
            }
        }
        for (const entry of modulesByFlag) {
            ...
            const slotAlreadySatisfied = fittedItemsSnapshot.some((item) => Number(item && item.flagID) === Number(entry.flagID) &&
            ...
            while (candidateItemIDs.length > 0) {
                ...
                const shipServiceAccess = this._validateShipServiceTransferAccess(session, item, destination);
                if (!shipServiceAccess.success) {
                    log.debug(`[InvBroker] FitFitting rejected itemID=${itemID} shipService=${shipServiceAccess.shipID} error=${shipServiceAccess.errorMsg}`);
                    this._throwShipServiceAccessError(shipServiceAccess.errorMsg);
                }
                const fittingServiceAccess = this._validateInSpaceFittingServiceAccess(session, shipRecord, item, destination);
                if (!fittingServiceAccess.success) {
                    log.debug(`[InvBroker] FitFitting rejected itemID=${itemID} destination=${destination.locationID}:${destination.flagID} error=${fittingServiceAccess.errorMsg}`);
                    this._throwMobileDepotFittingAccessError(fittingServiceAccess.errorMsg);
                }
                const dockedStructureFittingAccess = this._validateDockedStructureFittingServiceAccess(session, shipRecord, item, destination);
                if (!dockedStructureFittingAccess.success) {
                    log.debug(`[InvBroker] FitFitting rejected itemID=${itemID} structure=${dockedStructureFittingAccess.structureID} destination=${destination.locationID}:${destination.flagID} error=${dockedStructureFittingAccess.errorMsg}`);
                    this._throwDockedStructureFittingAccessError(dockedStructureFittingAccess.errorMsg);
                }
                const fitValidation = this._validateFittingMove(session, shipRecord, item, destination, fittedItemsSnapshot);
                if (!fitValidation.success) {
                    continue;
                }
                const shipBayValidation = this._validateShipBayMove(session, shipRecord, item, destination);
                ...
                const capacityCheck = this._checkCapacityForMove(session, { kind: "shipInventory", inventoryID: shipRecord.itemID, flagID: entry.flagID }, destination, item, 1);
                if (!capacityCheck.success) {
                    continue;
                }
                const moveResult = this._moveSourceItemToDestination(session, sourceItemDescriptor, destination, this._resolveAppliedMoveQuantity(item, destination, 1));
                if (!moveResult.success) {
                    continue;
                }
                const movedItemID = this._resolveMovedItemID(moveResult, itemID, destination) || itemID;
        ...
        return buildList([...missingByType.entries()]
    ...
    _listShipInventoryFlagContents(session, flagID) {
        const boundContext = this._getBoundContext(session);
        const shipInventoryID = boundContext && boundContext.kind === "shipInventory"
        ...
        const itemsForFlag = this._resolveContainerItems(session, flagID, shipContext);
        this._primeInSpaceBayDogmaItems(session, shipContext, flagID, itemsForFlag);
    ...
    _listStructureInventoryFlagContents(session, flagID) {
        const boundContext = this._getBoundContext(session);
        const structureID = this._normalizeInventoryId(boundContext && boundContext.inventoryID, 0);
        const structure = this._getStructureForInventoryID(structureID);
        if (!structure) {
            return null;
        }
        ...
        const itemsForFlag = this._resolveContainerItems(session, flagID, structureContext);
        const itemOverrides = itemsForFlag
            .map((item) => this._buildInventoryItemOverrides(session, item))
            .filter(Boolean);
        return this._buildInventoryRemoteList(itemOverrides);
    ...
    Handle_ListFuelBay(args, session, kwargs) {
        ...
        const structureFuelBay = this._listStructureInventoryFlagContents(session, STRUCTURE_FUEL_BAY_FLAG);
        if (structureFuelBay) {
    ...
    Handle_DeliverToCorpHangar(args, session, kwargs) {
        ...
        const hasClientCallShape = rawArgs.length >= 6 ||
            (rawArgs.length >= 3 &&
                this._isMarshalListLike(rawArgs[2]) &&
                !this._isMarshalListLike(rawArgs[0]));
        const rawItemIDs = this._extractKwarg(kwargs, "itemIDs") ??
        ...
        const officeOrLocationID = this._extractKwarg(kwargs, "officeID") ??
            ...
            (hasClientCallShape
                ...
                : rawArgs.length > 1
                    ...
                    : null) ??
            (boundContext && boundContext.inventoryID);
        const corporationOffice = this._resolveCorporationOfficeForInventory(session, officeOrLocationID);
        const explicitFlag = this._extractKwarg(kwargs, "flag") ??
        ...
        const destinationFlag = this._isCorporationHangarFlag(explicitFlag)
            ...
            : 115;
        const moveQuantity = this._normalizeQuantityArg(this._extractKwarg(kwargs, "qty") ??
            this._extractKwarg(kwargs, "quantity") ??
            (hasClientCallShape ? rawArgs[3] : null));
        let sourceLocationID = this._normalizeInventoryId(this._extractKwarg(kwargs, "sourceLocationID") ??
            ...
            (hasClientCallShape
                ...
                : rawArgs.length > 1
                    ? rawArgs[1]
                    : null) ??
            this._getStationId(session), this._getStationId(session));
        const allChanges = [];
        let movedCount = 0;
        log.debug(`[InvBroker] DeliverToCorpHangar itemCount=${itemIDs.length} officeOrLocationID=${String(officeOrLocationID)} flag=${destinationFlag}`);
        ...
        if (this._isPendingStructureUnanchorContainerInventoryID(sourceLocationID) ||
            itemIDs.some((itemID) => this._isPendingStructureUnanchorContainerInventoryID(itemID))) {
            this._throwPendingStructureUnanchorContainerInventoryError();
        }
        for (const itemID of itemIDs) {
            const sourceItemDescriptor = this._findTransferSourceItem(itemID, sourceLocationID);
    ...
    Handle_GetAvailableTurretSlots(args, session) {
        ...
        log.debug(`[InvBroker] GetAvailableTurretSlots ${describeSessionHydrationState(session)}`);
        this._primeDeferredSpaceBallparkVisuals(session, this._getBoundContext(session), {
            reason: "GetAvailableTurretSlots",
        });
        const charId = Number(session && (session.characterID || session.charid || session.userid)) || 0;
        const shipID = Number(session && (session.shipID || session.shipid || session.activeShipID)) || 0;
        const shipRecord = (charId > 0 && shipID > 0 ? findCharacterShip(charId, shipID) : null) ||
    ...
    Handle_MachoBindObject(args, session, kwargs) {
        ...
        const inventoryID = Array.isArray(bindParams) && bindParams.length > 0
            ? bindParams[0]
            : bindParams;
        if (this._isUnanchoredStructureHullInventoryID(inventoryID)) {
            this._throwUnanchoredStructureHullInventoryError();
        }
        if (this._isPendingStructureUnanchorContainerInventoryID(inventoryID)) {
            this._throwPendingStructureUnanchorContainerInventoryError();
        }
        log.debug(`[InvBroker] MachoBindObject args.length=${args ? args.length : 0} bindParams=${JSON.stringify(bindParams, (k, v) => (typeof v === "bigint" ? v.toString() : v))} nestedCall=${JSON.stringify(nestedCall, (k, v) => (typeof v === "bigint" ? v.toString() : Buffer.isBuffer(v) ? v.toString("utf8") : v))} kwargs=${JSON.stringify(kwargs, (k, v) => (typeof v === "bigint" ? v.toString() : Buffer.isBuffer(v) ? v.toString("utf8") : v))}`);
        ...
        const requestedFlag = Array.isArray(bindParams) && bindParams.length > 1
        ...
        if (nestedCall && Array.isArray(nestedCall) && nestedCall.length >= 1) {
            const methodName = typeof nestedCall[0] === "string"
                ? nestedCall[0]
            ...
            try {
                ...
                callResult = this.callMethod(methodName, Array.isArray(callArgs) ? callArgs : [callArgs], session, callKwargs);
...
//# sourceMappingURL=invBrokerService.js.map