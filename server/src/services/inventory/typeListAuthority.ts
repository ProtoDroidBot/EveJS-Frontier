...
function createTypeListAuthority(payload, typeRows = []) {
  ...
  for (const row of Array.isArray(payload && payload.typeLists) ? payload.typeLists : []) {
    ...
    if (list) byID.set(list.listID, list);
  }
  ...
  for (const row of Array.isArray(typeRows) ? typeRows : []) {
    ...
    if (Number.isSafeInteger(groupID)) addToIndex(byGroup, groupID, typeID);
    if (Number.isSafeInteger(categoryID)) addToIndex(byCategory, categoryID, typeID);
    for (const tag of normalizeNumericSet(row.tags)) addToIndex(byTag, tag, typeID);
  }
...
function loadAuthority() {
  if (!cachedAuthority) {
    cachedAuthority = createTypeListAuthority(
      readStaticTable(TABLE.CLIENT_TYPE_LISTS),
...
function addIndexedTypes(target, index, identifiers) {
  for (const identifier of identifiers) {
    for (const typeID of index.get(identifier) || []) target.add(typeID);
  }
}

function removeIndexedTypes(target, index, identifiers) {
  for (const identifier of identifiers) {
    for (const typeID of index.get(identifier) || []) target.delete(typeID);
  }
}

function intersectWith(target, allowed) {
  for (const typeID of target) {
    if (!allowed.has(typeID)) target.delete(typeID);
  }
}
...
function expandTypeList(authority, listID, visiting = new Set()) {
  ...
  if (!list || visiting.has(numericListID)) return null;
  ...
  for (const typeID of list.includedTypeIDs) members.add(typeID);
  ...
  for (const childID of list.includedTypeListIDs) {
    ...
    for (const typeID of child) members.add(typeID);
  }
  ...
  for (const typeID of list.excludedTypeIDs) members.delete(typeID);
  ...
  for (const childID of list.excludedTypeListIDs) {
    ...
    for (const typeID of child) members.delete(typeID);
  }
...
function typeIDFromContext(itemOrTypeContext) {
  const typeID = Number(
    typeof itemOrTypeContext === "number"
      ? itemOrTypeContext
      : itemOrTypeContext &&
...
function matchesTypeListWithMissingFallback(itemOrTypeContext, listID, fallback, policyName) {
  ...
  const numericListID = Number(listID) || 0;
  if (!Number.isSafeInteger(numericListID) || numericListID <= 0) return false;
  ...
  if (!authority.warnedMissingFallbacks.has(warningKey)) {
    ...
    log.warn(
      `[TypeListAuthority] Missing list ${numericListID} for ${String(policyName || "unspecified")} ` +
...
function matchesAnyTypeList(itemOrTypeContext, listIDs: readonly number[] = []) {
  for (const listID of Array.isArray(listIDs) ? listIDs : [listIDs]) {
    if (matchesTypeList(itemOrTypeContext, listID)) return true;
  }
  return false;
}
...
function isTradableInventoryItemInAuthority(authority, itemOrTypeContext) {
  const typeID = typeIDFromContext(itemOrTypeContext);
  if (!typeID || !authority.typeByID.has(typeID)) return false;
  for (const listID of TRADE_RESTRICTED_TYPE_LIST_IDS) {
    const members = expandTypeList(authority, listID);
    if (!members || members.has(typeID)) return false;
  }
  return true;
}
