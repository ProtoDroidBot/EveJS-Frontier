...
const { createSpaceItemForCharacter, grantItemToCharacterLocation, } = require(path.join(__dirname, "./itemStore"));
const { resolveItemByName, } = require(path.join(__dirname, "./itemTypeRegistry"));
const { resolveRuntimeWreckRadius, } = require(path.join(__dirname, "./wreckRadius"));
const { buildChildEntityScopeMetadata, canEntitiesInteractLocally, } = require(path.join(__dirname, "../../space/destiny/identity/interactionScope"));
...
function normalizeVector(vector, fallback = { x: 1, y: 0, z: 0 }) {
    ...
    const magnitude = Math.sqrt((resolved.x ** 2) + (resolved.y ** 2) + (resolved.z ** 2));
...
function getSpaceDebrisLifetimeMs() {
    return Math.max(60_000, normalizePositiveInteger(config.spaceDebrisLifetimeMs, 2 * 60 * 60 * 1000));
}
...
function isRandomContainerCandidate(row) {
    ...
    return (groupName === "spawn container" ||
...
function getRandomWreckTypes() {
    ...
    const rows = dedupeByTypeID(readStaticRows(TABLE.ITEM_TYPES).filter(isRandomWreckCandidate));
    cachedWreckTypes =
...
function getRandomContainerTypes() {
    ...
    const rows = dedupeByTypeID(readStaticRows(TABLE.ITEM_TYPES).filter(isRandomContainerCandidate));
...
function findSpawnPositions(scene, center, direction, count, maxRadius, interactionSource = null) {
    const nearbyEntities = scene.getAllVisibleEntities()
        .filter((entity) => ((!interactionSource || canEntitiesInteractLocally(interactionSource, entity)) &&
    ...
    for (let index = 0; index < count; index += 1) {
        ...
        for (let attempt = 0; attempt < MAX_POSITION_ATTEMPTS; attempt += 1) {
            ...
            const candidateDistance = MIN_DEBRIS_DISTANCE_METERS +
                Math.random() * Math.max(0, maxRadius - MIN_DEBRIS_DISTANCE_METERS);
            const candidatePosition = addVectors(center, scaleVector(candidateDirection, candidateDistance));
            const collides = [...nearbyEntities, ...positions].some((existing) => (distance(candidatePosition, existing.position) <
                Math.max(DEBRIS_PADDING_METERS * 2, toFiniteNumber(existing.radius, 0) + DEBRIS_PADDING_METERS)));
            if (!collides) {
...
function populateDebrisLoot(characterID, itemID) {
    ...
    for (let index = 0; index < lootRolls; index += 1) {
        ...
        const quantity = lootEntry.minQuantity >= lootEntry.maxQuantity
            ...
            : lootEntry.minQuantity + Math.floor(Math.random() * ((lootEntry.maxQuantity - lootEntry.minQuantity) + 1));
        const grantResult = grantItemToCharacterLocation(characterID, itemID, LOOT_FLAG_ID, lootEntry.itemType, quantity);
...
function spawnDebrisFieldForSession(session, kind, options = {}) {
    ...
    const count = normalizePositiveInteger(options.count, kind === "wreck" ? DEFAULT_WRECK_COUNT : DEFAULT_CONTAINER_COUNT);
    const radius = Math.max(MIN_DEBRIS_DISTANCE_METERS, toFiniteNumber(options.radius, DEFAULT_DEBRIS_RADIUS_METERS));
    const positions = findSpawnPositions(scene, cloneVector(shipEntity.position), normalizeVector(shipEntity.direction, { x: 1, y: 0, z: 0 }), count, radius, shipEntity);
    const created = [];
    const changes = [];
    ...
    for (const position of positions) {
        ...
        const createResult = createSpaceItemForCharacter(session.characterID, systemID, debrisTypeLookup.match, {
...
function clearNearbyDebrisForSession(session, options = {}) {
    ...
    const radius = Math.max(1000, toFiniteNumber(options.radius, DEFAULT_DEBRIS_RADIUS_METERS));
    const removed = [];
    const changes = [];
...
//# sourceMappingURL=spaceDebrisState.js.map