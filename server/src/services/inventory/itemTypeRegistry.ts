...
function resolveItemByName(query) {
  ...
  if (/^\d+$/.test(normalizedQuery)) {
    ...
    if (exactTypeMatch) {
      return { success: true as const, match: exactTypeMatch, suggestions: [] };
    }
  }

  const exactMatches = dedupeEntries(registry.byName.get(normalizedQuery) || []);
  if (exactMatches.length === 1) {
    return { success: true as const, match: exactMatches[0], suggestions: [] };
  }
  if (exactMatches.length > 1) {
    return {
      success: false as const,
  ...
  if (deduped.length === 1) {
    return { success: true as const, match: deduped[0], suggestions: [] };
  }
