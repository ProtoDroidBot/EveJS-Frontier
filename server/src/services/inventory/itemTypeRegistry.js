...
function normalizeEntry(entry) {
    return {
        ...
        published: Object.prototype.hasOwnProperty.call(entry || {}, "published")
...
function addEntry(registry, entry) {
    ...
    if (!Number.isInteger(normalizedEntry.typeID) ||
...
function loadDbRegistry() {
    try {
        ...
        log.warn(`[ItemRegistry] Failed to load item reference data from gameStore: ${error.message}`);
        return null;
    }
}
...
function buildFuzzySuggestions(registry, normalizedQuery) {
    ...
    for (const entry of registry.byTypeID.values()) {
        ...
        if (queryWords.length > 0) {
            ...
            const tokenPrefixHits = queryWords.filter((queryWord) => nameWords.some((nameWord) => nameWord.startsWith(queryWord))).length;
    ...
    return scored
        .sort((left, right) => {
        const leftScore = left.score + left.publishedPenalty + left.blueprintPenalty + left.nameLengthPenalty;
        const rightScore = right.score + right.publishedPenalty + right.blueprintPenalty + right.nameLengthPenalty;
...
let cachedShipTypeIDs = null;
// Every ship-category (6) type, as { typeID, groupID, raceID }. Mirrors the client's
// evetypes.GetTypeIDsByCategory(categoryShip) (published + unpublished — callers filter published
// themselves, matching evedungeons util.py). Computed once and cached alongside the registry.
function listShipTypeIDs() {
    if (cachedShipTypeIDs) {
        return cachedShipTypeIDs;
    }
    const registry = loadRegistry();
    const ships = [];
    for (const entry of registry.byTypeID.values()) {
        if (Number(entry.categoryID) === CATEGORY_SHIP) {
            ships.push({
    ...
    cachedShipTypeIDs = ships.filter((ship) => ship.typeID > 0);
    return cachedShipTypeIDs;
}
function resolveItemByName(query) {
    ...
    return {
        ...
        suggestions: (deduped.length > 0 ? deduped : buildFuzzySuggestions(registry, normalizedQuery))
            .slice(0, 5)
            .map((entry) => formatSuggestionLabel(entry)),
    };
}
...
//# sourceMappingURL=itemTypeRegistry.js.map