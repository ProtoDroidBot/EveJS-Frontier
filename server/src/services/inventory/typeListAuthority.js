...
function normalizeTypeContext(itemOrTypeContext) {
    const typeID = Number(itemOrTypeContext &&
        (itemOrTypeContext.typeID ?? itemOrTypeContext.itemTypeID ?? itemOrTypeContext.id)) || 0;
...
function matchesAnyIncludedIdentifier(typeContext, typeList) {
    return (typeList.includedTypeIDs.has(typeContext.typeID) ||
...
function matchesAnyExcludedIdentifier(typeContext, typeList) {
    return (typeList.excludedTypeIDs.has(typeContext.typeID) ||
