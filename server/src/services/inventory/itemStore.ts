...
function writeItems(data, options: Record<string, any> = {}) {
...
function buildItemIndexes() {
  ...
  for (const rawItem of Object.values<any>(readItems())) {
...
function normalizeSpaceVector(rawValue, fallback: Record<string, any> = { x: 0, y: 0, z: 0 }) {
...
function normalizeDungeonTrackingState(rawValue) {
  ...
  const roomID = normalizeNonNegativeJSONInt64FailClosed(
    rawValue.dungeonCurrentRoomID,
  );
  if (roomID !== null) {
  ...
  if (
    Array.isArray(rawValue.dungeonCurrentRoomPosition) &&
  ...
  const updatedAtMs = toFiniteNumber(
    rawValue.dungeonCurrentUpdatedAtMs,
    0,
  );
  if (updatedAtMs > 0) {
      ...
      rawValue.warpState && typeof rawValue.warpState === "object"
        ? {
            ...
            targetEntityID: rawValue.warpState.targetEntityID
              ...
              : null,
            destinationStaticInstanceID: normalizePersistentJSONEntityIDFailClosed(
              rawValue.warpState.destinationStaticInstanceID,
            ),
            destinationDungeonRoomKey:
              String(
                rawValue.warpState.destinationDungeonRoomKey || "",
              ).trim() || null,
            destinationDungeonSiteID: normalizePersistentJSONEntityIDFailClosed(
              rawValue.warpState.destinationDungeonSiteID,
            ),
...
function normalizeFighterAbilityStates(rawValue) {
  ...
  const normalized: Record<string, any> = {};
  for (const [slotID, slotState] of Object.entries<any>(rawValue)) {
...
function normalizeModuleState(rawValue) {
  ...
  if (reserveFuelQueue.length > 0) {
    ...
    normalizedState.reserveFuelCharge = reserveFuelQueue.reduce(
      (total, entry) => total + entry.quantity,
      0,
    );
  } else if (Object.prototype.hasOwnProperty.call(source, "reserveFuelCharge")) {
    normalizedState.reserveFuelCharge = Math.max(
      0,
      toFiniteNumber(source.reserveFuelCharge, 0),
    );
...
function buildStackIndex(items) {
  ...
  for (const rawItem of Object.values<any>(items || {})) {
    const item = normalizeInventoryItem(rawItem);
    if (!isStackableInventoryItem(item)) {
      continue;
    }
...
function normalizeInventoryItem(rawItem, defaults: Record<string, any> = {}) {
...
function normalizeShipItem(rawItem, defaults: Record<string, any> = {}) {
...
function repairOversizedStackableItems(items, characters: Record<string, any> = {}) {
  let itemsDirty = false;
  const repairs: any[] = [];
  const stackableItems = Object.values<any>(items || {})
...
function listOwnedItems(ownerId, options: Record<string, any> = {}) {
  ensureMigrated();
...
function listCharacterItems(charId, options: Record<string, any> = {}) {
  return listOwnedItems(charId, options);
}

function listCharacterShipItems(charId, options: Record<string, any> = {}) {
...
function validateGrantEntries(entries) {
  for (const entry of entries) {
    ...
    if (
      ...
      return { success: false as const, errorMsg: "ITEM_TYPE_NOT_FOUND" };
    }
    if (!normalizePositiveSafeInteger(entry.quantity, null)) {
      return { success: false as const, errorMsg: "ITEM_QUANTITY_OUT_OF_RANGE" };
    }
  }
  return null;
}
...
function grantItemsToCharacterLocation(
  ...
  const grantedEntries: any[] = [];
  const transientCreatedItemIDs: any[] = [];
...
function grantItemsToCharacterStationHangar(charId, stationId, grantEntries: any[] = []) {
  return grantItemsToCharacterLocation(
    charId,
    stationId,
    ITEM_FLAGS.HANGAR,
...
function resolveSpawnedSpaceItemRadius(itemType, options: Record<string, any> = {}) {
...
function createSpaceItemForCharacter(charId, solarSystemId, itemType, options: Record<string, any> = {}) {
...
function createSpaceItemForOwner(ownerId, solarSystemId, itemType, options: Record<string, any> = {}) {
  const normalizedSystemId = toNumber(solarSystemId, 0);
  if (normalizedSystemId <= 0) {
    return {
      success: false as const,
...
function takeItemTypeFromCharacterLocation(
  ...
  const items = readItems();
  const matchingItems = Object.values<any>(items)
...
function removeInventoryItem(itemId, options: Record<string, any> = {}) {
  ensureMigrated();
  const numericItemId = toNumber(itemId, 0);
  if (numericItemId <= 0) {
    return {
...
function commitContainerizedInventoryRemoval(request) {
  ...
  const containerSpecs = Array.isArray(request && request.containers)
    ...
    : null;
  if (
    !Number.isSafeInteger(rootItemID) ||
    rootItemID <= 0 ||
    !containerSpecs
  ) {
    return { success: false as const, errorMsg: "INVALID_CONTAINERIZED_REMOVAL" };
  }
  ...
  if (
    request.expectedCustomInfo !== undefined &&
    JSON.stringify(rootItem.customInfo) !== JSON.stringify(request.expectedCustomInfo)
  ) {
    return { success: false as const, errorMsg: "ITEM_STATE_CHANGED" };
  }

  const normalizedSpecs: any[] = [];
  const seenMovedQuantities = new Map<any, any>();
  for (const rawSpec of containerSpecs) {
    ...
    for (const move of moves) {
      ...
      if (
        ...
        return { success: false as const, errorMsg: "INVALID_CONTAINER_MOVE" };
      }
      const availableQuantity = source.singleton === 1
        ? 1
        : normalizePositiveInteger(source.stacksize, 1);
      const requestedQuantity =
        (seenMovedQuantities.get(itemID) || 0) + quantity;
      if (requestedQuantity > availableQuantity) {
        return { success: false as const, errorMsg: "INSUFFICIENT_ITEMS" };
      }
  ...
  for (const spec of normalizedSpecs) {
    ...
    const container = buildInventoryItem({
      itemID: nextItemID(
        spec.ownerID,
        items,
        characters[String(spec.ownerID)] || null,
      ),
  ...
  for (let index = 0; index < normalizedSpecs.length; index += 1) {
    ...
    for (const entry of spec.grants) {
      ...
      if (!metadata || !quantity) {
        return { success: false as const, errorMsg: "INVALID_CONTAINER_GRANT" };
      }
  ...
  const remainingChildren = Object.values<any>(items).some((rawItem) => {
    const item = normalizeInventoryItem(rawItem);
    return item &&
  ...
  if (remainingChildren) {
    return { success: false as const, errorMsg: "CONTAINERIZED_REMOVAL_NOT_EMPTY" };
  }
...
function stageItemMoveToLocation(
  items,
  characters,
  itemId,
  ...
  if (movingWholeItem) {
    ...
    moveOptions.preserveMovedStackItemID === true &&
    !convertToSingleton
  ) {
...
function moveItemsToLocations(moveRequests) {
  ...
  if (requests.length === 0) {
    return { success: false as const, errorMsg: "INVALID_MOVE_REQUEST" };
  }
...
function consumeInventoryItems(consumptionRequests, options: Record<string, any> = {}) {
  ensureMigrated();
  const requests = Array.isArray(consumptionRequests) ? consumptionRequests : [];
  if (requests.length === 0) {
    return { success: false as const, errorMsg: "INVALID_CONSUME_REQUEST" };
  }
  ...
  for (const request of requests) {
    ...
    for (const [field, actual] of [
      ...
      if (expected[field] != null && Number(expected[field]) !== Number(actual)) {
        return { success: false as const, errorMsg: "SOURCE_ITEM_CHANGED" };
      }
    }
  ...
  return { success: true as const, data: { consumed, changes } };
}
 ...
 */
function consumeInventoryItemsAndUpdateItem(
  consumptionRequests,
  itemId,
  updater,
  options: Record<string, any> = {},
  ...
  if (!target) return { success: false as const, errorMsg: "ITEM_NOT_FOUND" };
  const seen = new Set<number>();
  const changes: any[] = [];
  const consumed: any[] = [];
  for (const request of requests) {
  ...
  if (!updatedTarget || Number(updatedTarget.itemID) !== targetItemID) {
    return { success: false as const, errorMsg: "INVALID_ITEM_STATE" };
  }
 ...
 */
function grantStackableItemsToCharacterLocationAndUpdateItem(
  charId,
  locationId,
  flagId,
  grantEntries,
  ...
  if (ownerID <= 0 || destinationLocationID <= 0 || destinationFlagID < 0 ||
    ...
    return { success: false as const, errorMsg: "INVALID_GRANT_REQUEST" };
  }
  const entryValidationError = validateGrantEntries(entries);
  if (entryValidationError) return entryValidationError;

  const characters = readCharacters();
  const characterRecord = characters[String(ownerID)];
  if (!characterRecord) return { success: false as const, errorMsg: "CHARACTER_NOT_FOUND" };
  const previousItems = readItems();
  const items = cloneValue(previousItems);
  const target = normalizeInventoryItem(items[String(targetItemID)]);
  if (!target) return { success: false as const, errorMsg: "ITEM_NOT_FOUND" };
 ...
 */
function moveItemsToLocationsAndUpdateItem(
  moveRequests,
  itemId,
  updater,
  options: Record<string, any> = {},
  ...
  for (const request of requests) {
    ...
    if (typeof movedItemUpdater === "function") {
      ...
      const updatedMovedItem = normalizeInventoryItem(
        movedItemUpdater(cloneValue(currentMovedItem)),
        currentMovedItem,
      );
      if (
        !currentMovedItem ||
      ...
      const movedChange = [...stageResult.data.changes].reverse().find(
        (change) => Number(change && change.item && change.item.itemID) === movedItemID,
      );
      if (movedChange) {
  ...
  if (options && options.flush === true) {
    ...
    if (!flushResult || flushResult.success !== true) {
      ...
      return { success: false as const, errorMsg: "PERSISTENCE_FLUSH_ERROR" };
    }
  }
...
function moveItemStacksToLocation(
  ...
  const normalizedPreliminaryRequests: any[] = [];
  const seenItemIDs = new Set<any>();
  ...
  if (destinationItemID > 0) {
    ...
    if (
      ...
      return { success: false as const, errorMsg: "INVALID_DESTINATION_STACK" };
    }
  }

  const changes: any[] = [];
  const moves: any[] = [];
  const preliminaryMoves: any[] = [];
  ...
  for (const request of normalizedPreliminaryRequests) {
    ...
    if (toNumber(stageResult.data.quantity, 0) !== request.quantity) {
      return { success: false as const, errorMsg: "MOVE_QUANTITY_MISMATCH" };
    }
  ...
  for (const request of normalizedRequests) {
    ...
    if (!sourceItem || toNumber(sourceItem.singleton, 0) !== 0) {
      return { success: false as const, errorMsg: "STACK_REQUIRED" };
    }
...
function moveItemsAndSetShipPackagingState(
  ...
  const shipSplitChanges: any[] = [];
  const packagedShipQuantity = normalizePositiveInteger(
    originalShip.stacksize ?? originalShip.quantity,
    1,
  );
  if (
    !packaged &&
    ...
    targetShipID = nextItemID(
      originalShip.ownerID,
      items,
      characters[String(originalShip.ownerID)],
    );
    prebuiltUpdatedShip = buildInventoryItem({
      ...
      singleton: 1,
      stackOriginID:
        toNumber(originalShip.stackOriginID, 0) > 0
          ? toNumber(originalShip.stackOriginID, 0)
          : originalShip.itemID,
    });
    items[String(targetShipID)] = prebuiltUpdatedShip;
    shipPreviousData = buildCreatedItemNotificationPreviousState(
      prebuiltUpdatedShip,
      originalShip.flagID,
      originalShip.ownerID,
    );
  ...
  if (!updatedShip) {
    return { success: false as const, errorMsg: "INVALID_SHIP_STATE" };
  }
...
function transferItemToOwnerLocation(
  ...
  if (hasReceipt &&
    ...
    return { success: false as const, errorMsg: "INVALID_CUSTODY_RECEIPT" };
  }
  if (hasReceipt) {
    for (const rawItem of Object.values<any>(items)) {
      ...
      return replay.operationFingerprint === operationFingerprint
        ? { success: true as const, replayed: true, data: cloneValue(replay.result) }
...
function syncCapsuleTypeForCharacter(charId, options: Record<string, any> = {}) {
  const numericCharId = toNumber(charId, 0);
  if (numericCharId <= 0) {
    return {
      success: false as const,
...
function ensureCapsuleForCharacter(charId, stationId) {
  ...
  if (existingCapsule) {
    const syncResult: Record<string, any> = syncCapsuleItemToType(
      charId,
      existingCapsule,
...
function pruneExpiredSpaceItems(now = Date.now()) {
  ...
  const items = Object.values<any>(readItems())
    .map((entry) => normalizeInventoryItem(entry))
    .filter(Boolean)
    .sort((left, right) => left.itemID - right.itemID);

  const removedTopLevelItemIDs: any[] = [];
  const removedChanges: any[] = [];
  const seen = new Set<any>();
...
function findAssemblyCustodyReceipt(operationKey) {
  ...
  for (const item of Object.values<any>(readItems())) {
    ...
    if (receipt) return cloneValue(receipt);
  }
  return null;
}
...
function writeItemCustomInfo(itemID, customInfo, options: Record<string, any> = {}) {
