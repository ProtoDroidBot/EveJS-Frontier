...
const { INDUSTRY_INSTALLED_LOCATION_ID, } = require(path.join(__dirname, "../industry/industryConstants"));
const structureAssetSafetyState = require(path.join(__dirname, "../structure/structureAssetSafetyState"));
...
function isAssetSafetyWrapLocation(locationID) {
    ...
    return Boolean(structureAssetSafetyState.getWrapByID(numericLocationID, {
        refresh: false,
    }));
}
...
function buildDockableAssetLocationMetadata(locationID, session) {
    ...
    if (structure) {
        const structureSolarSystemID = toInteger(structure.solarSystemID, getSessionSolarSystemId(session));
        ...
        return {
            ...
            constellationID: toInteger(structure.constellationID, toInteger(system && system.constellationID, getSessionConstellationId(session))),
            regionID: toInteger(structure.regionID, toInteger(system && system.regionID, getSessionRegionId(session))),
...
//# sourceMappingURL=inventoryVisibilityRules.js.map