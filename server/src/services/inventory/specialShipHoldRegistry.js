...
const definitionsByFlag = new Map(SPECIAL_SHIP_HOLD_DEFINITIONS.map((definition) => [
    definition.flagID,
    definition,
]));
const SPECIAL_SHIP_HOLD_FLAGS = Object.freeze(SPECIAL_SHIP_HOLD_DEFINITIONS.map((definition) => definition.flagID));
const GENERIC_SPECIAL_SHIP_HOLD_FLAGS = Object.freeze(SPECIAL_SHIP_HOLD_DEFINITIONS
...
function isSpecialShipHoldItemAllowed(itemOrTypeID, flagID) {
    ...
    switch (definition.accepts) {
        ...
        case "infrastructure":
            return (INFRASTRUCTURE_HOLD_ALLOWED_CATEGORY_IDS.has(categoryID(typeRecord)) ||
...
function getSpecialShipHoldCapacity(resourceState, shipTypeID, flagID, getBaseAttributeValue = null) {
    const definition = getSpecialShipHoldDefinition(flagID);
    if (!definition) {
        return 0;
    }
    ...
    if (attributes) {
        for (const attributeID of definition.attributeIDs || []) {
            const value = toFiniteNumber(attributes[attributeID] ?? attributes[String(attributeID)], NaN);
    ...
    if (typeof getBaseAttributeValue === "function") {
        return toFiniteNumber(getBaseAttributeValue(shipTypeID, ...(definition.attributeNames || [])), 0);
    }
    return 0;
}
module.exports = {
...
//# sourceMappingURL=specialShipHoldRegistry.js.map