...
const { BOOKMARKS_ADDED, BOOKMARKS_MOVED, BOOKMARKS_REMOVED, BOOKMARKS_UPDATED, FOLDER_UPDATED, SUBFOLDER_ADDED, SUBFOLDER_REMOVED, SUBFOLDER_UPDATED, } = require(path.join(__dirname, "./bookmarkConstants"));
const { buildBookmarkIDSet, buildBookmarkPayload, buildBookmarkText, buildFolderUpdateTuple, buildSubfolderPayload, } = require(path.join(__dirname, "./bookmarkPayloads"));
...
function buildBookmarksAddedUpdate(folderID, bookmarks) {
    return buildFolderUpdateTuple(folderID, BOOKMARKS_ADDED, buildList((Array.isArray(bookmarks) ? bookmarks : []).map(buildBookmarkPayload)));
}
function buildBookmarksUpdatedUpdate(folderID, bookmarks) {
    return buildFolderUpdateTuple(folderID, BOOKMARKS_UPDATED, buildList((Array.isArray(bookmarks) ? bookmarks : []).map(buildBookmarkPayload)));
}
...
function notifyBookmarksAdded(folderID, bookmarks, options = {}) {
    ...
    sendFolderUpdated([folderID], buildBookmarksAddedUpdate(folderID, bookmarks), Number(options.excludeCharacterID || 0));
}
function notifyBookmarksUpdated(folderID, bookmarks, options = {}) {
    if (!Array.isArray(bookmarks) || bookmarks.length <= 0) {
        return;
    }
    sendFolderUpdated([folderID], buildBookmarksUpdatedUpdate(folderID, bookmarks), Number(options.excludeCharacterID || 0));
}
function notifyBookmarksRemoved(folderID, bookmarkIDs, options = {}) {
    if (!Array.isArray(bookmarkIDs) || bookmarkIDs.length <= 0) {
        return;
    }
    sendFolderUpdated([folderID], buildBookmarksRemovedUpdate(folderID, bookmarkIDs), Number(options.excludeCharacterID || 0));
}
function notifyBookmarksMoved(oldFolderID, newFolderID, bookmarks, options = {}) {
    if (!Array.isArray(bookmarks) || bookmarks.length <= 0) {
        return;
    }
    sendFolderUpdated([oldFolderID, newFolderID], buildBookmarksMovedUpdate(oldFolderID, newFolderID, bookmarks), Number(options.excludeCharacterID || 0));
}
function notifyFolderUpdated(folderID, folder, options = {}) {
    sendFolderUpdated([folderID], buildFolderUpdatedUpdate(folderID, folder && folder.folderName, folder && folder.description), Number(options.excludeCharacterID || 0));
}
function notifySubfolderAdded(folderID, subfolder, options = {}) {
    sendFolderUpdated([folderID], buildSubfolderAddedUpdate(folderID, subfolder), Number(options.excludeCharacterID || 0));
}
function notifySubfolderUpdated(folderID, subfolderID, subfolderName, options = {}) {
    sendFolderUpdated([folderID], buildSubfolderUpdatedUpdate(folderID, subfolderID, subfolderName), Number(options.excludeCharacterID || 0));
}
function notifySubfolderRemoved(folderID, subfolderID, options = {}) {
    sendFolderUpdated([folderID], buildSubfolderRemovedUpdate(folderID, subfolderID), Number(options.excludeCharacterID || 0));
}
...
//# sourceMappingURL=bookmarkNotifications.js.map