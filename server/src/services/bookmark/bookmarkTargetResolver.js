...
const { TYPE_SOLAR_SYSTEM, } = require(path.join(__dirname, "./bookmarkConstants"));
...
function resolveLocationBookmarkTarget(itemID, session, scene = null) {
    ...
    if (shipEntity && numericItemID === normalizeNumber(shipEntity.itemID, 0)) {
        ...
        const perceivedWarpPosition = scene && typeof scene.getPilotPerceivedWarpPosition === "function"
            ? scene.getPilotPerceivedWarpPosition(shipEntity)
            : null;
        return buildCoordinateBookmarkTarget(shipEntity, systemID, perceivedWarpPosition);
    }
...
//# sourceMappingURL=bookmarkTargetResolver.js.map