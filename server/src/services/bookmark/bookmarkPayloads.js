...
const { buildKeyVal, buildList, buildDict, buildFiletimeLong, normalizeText, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { ACCESS_NONE, TYPE_SOLAR_SYSTEM, } = require(path.join(__dirname, "./bookmarkConstants"));
...
function buildFolderPayload(record = {}, options = {}) {
    const accessLevel = toInt(options.accessLevel, Object.prototype.hasOwnProperty.call(record, "accessLevel")
        ? record.accessLevel
        : ACCESS_NONE);
...
function buildBookmarkIDSet(bookmarkIDs = []) {
    return buildList((Array.isArray(bookmarkIDs) ? bookmarkIDs : [bookmarkIDs])
        .map((bookmarkID) => toInt(bookmarkID, 0))
        .filter((bookmarkID) => bookmarkID > 0));
}
function buildBookmarkDict(bookmarks = []) {
    return buildDict((Array.isArray(bookmarks) ? bookmarks : [])
...
function isCoordinateBookmark(record = {}) {
    return Boolean(toOptionalNumber(record.x) !== null &&
...
//# sourceMappingURL=bookmarkPayloads.js.map