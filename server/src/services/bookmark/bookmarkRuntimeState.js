...
const { currentFileTime, normalizeBigInt, normalizeText, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const worldData = require(path.join(__dirname, "../../space/worldData"));
const signatureRuntime = require(path.join(__dirname, "../exploration/signatures/signatureRuntime"));
const { MAX_BOOKMARKS_SENT_TO_CLIENT, MAX_ACTIVE_PERSONAL_BOOKMARK_FOLDERS, MAX_ACTIVE_SHARED_BOOKMARK_FOLDERS, MAX_KNOWN_BOOKMARK_FOLDERS, MAX_BOOKMARK_NAME_LENGTH, MAX_BOOKMARK_DESCRIPTION_LENGTH, MAX_FOLDER_NAME_LENGTH, MAX_FOLDER_DESCRIPTION_LENGTH, MAX_SUBFOLDERS, ACCESS_ADMIN, ACCESS_MANAGE, ACCESS_NONE, ACCESS_PERSONAL, ACCESS_USE, ACCESS_VIEW, BOOKMARK_EXPIRY_NONE, BOOKMARK_EXPIRY_4HOURS, BOOKMARK_EXPIRY_24HOURS, BOOKMARK_EXPIRY_2DAYS, TYPE_SOLAR_SYSTEM, } = require(path.join(__dirname, "./bookmarkConstants"));
...
function buildRuntimeRoot() {
    ...
    root._meta.nextBookmarkID = toPositiveInt(root._meta.nextBookmarkID, store.DEFAULT_RUNTIME._meta.nextBookmarkID);
    root._meta.nextFolderID = toPositiveInt(root._meta.nextFolderID, store.DEFAULT_RUNTIME._meta.nextFolderID);
    root._meta.nextSubfolderID = toPositiveInt(root._meta.nextSubfolderID, store.DEFAULT_RUNTIME._meta.nextSubfolderID);
    root._meta.nextGroupID = toPositiveInt(root._meta.nextGroupID, store.DEFAULT_RUNTIME._meta.nextGroupID);
...
function setKnownFolderRecord(characterID, record) {
    ...
    store.state.knownFoldersByCharacterID.get(numericCharacterID).set(normalized.folderID, normalized);
...
function finalizeChanges({ meta = false, bookmarks = false, folders = false, subfolders = false, known = false, groups = false, rebuild = false, } = {}) {
    if (rebuild) {
...
function ensureLoaded() {
    ...
    for (const [characterID, rawEntries] of Object.entries(store.state.knownRoot.recordsByCharacterID)) {
        ...
        const rawObject = rawEntries && typeof rawEntries === "object" ? rawEntries : {};
...
function migrateLegacyBookmarksForCharacter(characterID) {
    ...
    for (const legacyFolder of legacyFolders) {
        ...
        const newFolder = setFolderRecord({
            folderID: access.allocateFolderID(),
            folderName: sanitizeFolderName(legacyFolder && legacyFolder.folderName, "Personal Locations"),
    ...
    for (const legacyBookmark of legacyBookmarks) {
        ...
        const folderID = folderIDMap.get(toPositiveInt(legacyBookmark && legacyBookmark.folderID, 0)) ||
            folderIDMap.get(0) ||
            ensureDefaultPersonalFolder(numericCharacterID).folderID;
        const targetBookmarkID = bookmarkID > 0 && !store.state.bookmarksByID.has(bookmarkID)
            ? bookmarkID
...
function listActiveBookmarksForCharacter(characterID) {
    const bookmarks = [];
    const activeFolderIDs = new Set(listFolderViews(characterID)
...
function translateExpiryToFiletime(expiryMode, nowFiletime = currentFileTime()) {
    ...
    switch (normalizedExpiry) {
        case BOOKMARK_EXPIRY_4HOURS:
            return (nowFiletime + (4n * 60n * 60n * 10000000n)).toString();
        case BOOKMARK_EXPIRY_24HOURS:
            return (nowFiletime + (24n * 60n * 60n * 10000000n)).toString();
        case BOOKMARK_EXPIRY_2DAYS:
            return (nowFiletime + (48n * 60n * 60n * 10000000n)).toString();
...
function resolveStaticBookmarkTarget(itemID, session = null) {
    ...
    const sessionConstellationID = toPositiveInt(session && (session.constellationID || session.constellationid), 0);
    ...
    const sessionRegionID = toPositiveInt(session && (session.regionID || session.regionid), 0);
...
function addFolder(characterID, options = {}) {
    ...
    const folder = setFolderRecord({
        folderID: access.allocateFolderID(),
        folderName: sanitizeFolderName(options.folderName, isPersonal ? "Personal Locations" : "Shared Locations"),
    ...
    let isActive = true;
    if (access.getActiveFolderCount(numericCharacterID, folder.isPersonal) >=
...
function deleteFolder(characterID, folderID) {
    ...
    const deletedBookmarkIDs = access.listBookmarksForFolder(view.folder.folderID).map((bookmark) => bookmark.bookmarkID);
    const deletedSubfolderIDs = access.listSubfoldersForFolder(view.folder.folderID).map((subfolder) => subfolder.subfolderID);
...
function deleteSubfolder(characterID, folderID, subfolderID) {
    ...
    for (const bookmark of access.listBookmarksForFolder(view.folder.folderID)) {
        ...
        if (canManageFolder(view.accessLevel) ||
...
function updateBookmark(characterID, bookmarkID, oldFolderID, name, note, subfolderID, newFolderID, expiryCancel = false) {
    ...
    if (!canWriteFolder(oldAccessLevel) ||
...
function resolveBookmarkTarget(bookmarkID) {
    ...
    const hasCoordinates = bookmark.x !== null &&
        bookmark.x !== undefined &&
        bookmark.y !== null &&
        bookmark.y !== undefined &&
        bookmark.z !== null &&
        bookmark.z !== undefined &&
    ...
    if (hasCoordinates) {
        return {
            ...
            metadata: bookmark.metadata && typeof bookmark.metadata === "object"
...
//# sourceMappingURL=bookmarkRuntimeState.js.map