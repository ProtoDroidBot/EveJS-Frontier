...
const { ACCESS_ADMIN, ACCESS_MANAGE, ACCESS_NONE, ACCESS_PERSONAL, ACCESS_USE, ACCESS_VIEW, MAX_ACTIVE_PERSONAL_BOOKMARK_FOLDERS, MAX_ACTIVE_SHARED_BOOKMARK_FOLDERS, MAX_BOOKMARKS_IN_PERSONAL_FOLDER, MAX_BOOKMARKS_IN_SHARED_FOLDER, MAX_BOOKMARKS_SENT_TO_CLIENT, MAX_KNOWN_BOOKMARK_FOLDERS, OWNER_SYSTEM, TYPE_SOLAR_SYSTEM, } = require(path.join(__dirname, "./bookmarkConstants"));
...
function touchLegacyMaximums() {
    ...
    store.state.runtimeRoot._meta.nextBookmarkID = Math.max(store.toPositiveInt(store.state.runtimeRoot._meta.nextBookmarkID, 900000001), maxPersistedBookmarkID + 1);
    store.state.runtimeRoot._meta.nextFolderID = Math.max(store.toPositiveInt(store.state.runtimeRoot._meta.nextFolderID, 500001), maxPersistedFolderID + 1);
    store.state.runtimeRoot._meta.nextSubfolderID = Math.max(store.toPositiveInt(store.state.runtimeRoot._meta.nextSubfolderID, 700001), maxPersistedSubfolderID + 1);
    store.state.runtimeRoot._meta.nextGroupID = Math.max(store.toPositiveInt(store.state.runtimeRoot._meta.nextGroupID, 800001), maxPersistedGroupID + 1);
    store.persistMeta();
}
...
function buildDefaultGroupName(corporationID) {
    ...
    const corporationName = String((corporation && (corporation.corporationName || corporation.name)) ||
        `Corporation ${corporationID}`).trim() || `Corporation ${corporationID}`;
...
//# sourceMappingURL=bookmarkRuntimeAccess.js.map