...
function executeBookmarkAutoCommand(session, rawText = "") {
  ...
  try {
    if (subcommand === "help") {
      return { success: true as const, message: buildHelpText() };
    }
    ...
    if (subcommand === "here") {
      ...
      if (!personal) {
        return { success: false as const, message: "No personal bookmark folder available." };
      }
    ...
    if (subcommand === "shared" && arg1 === "demo") {
      ...
      if (groups.length <= 0) {
        return { success: false as const, message: "No bookmark access groups available for this character." };
      }
    ...
    if (subcommand === "subfolder" && arg1 === "demo") {
      ...
      if (!folderID) {
        return { success: false as const, message: "No folder available for subfolder demo." };
      }
    ...
    if (subcommand === "move" && arg1 === "all" && arg2 === "demo") {
      ...
      if (!folderID || !context.lastSubfolderID) {
        return { success: false as const, message: "Need a source folder and subfolder first. Run /bookmarkauto personal demo and /bookmarkauto subfolder demo." };
      }
    ...
    if (subcommand === "copy" && arg1 === "all" && arg2 === "demo") {
      ...
      if (!personal || !sharedFolderID) {
        return { success: false as const, message: "Need both a personal and shared demo folder first." };
      }
    ...
    if (subcommand === "warp" && arg1 === "last") {
      if (!context.lastBookmarkID) {
        return { success: false as const, message: "No last bookmark stored in bookmark auto context." };
      }
      const target = runtime.resolveBookmarkTarget(context.lastBookmarkID);
      if (!target) {
        return { success: false as const, message: "Last bookmark target is no longer available." };
      }
      const missionInstanceID = toInt(
      ...
      const result =
        target.kind === "item"
          ? spaceRuntime.warpToEntity(session, target.itemID, {
    ...
    return { success: false as const, message: `Unknown /bookmarkauto subcommand: ${subcommand}` };
  } catch (error) {
    return {
      success: false as const,
