...
function listBookmarksForFolder(folderID) {
  return [...(store.state.bookmarkIDsByFolderID.get(store.toPositiveInt(folderID, 0)) || new Set<any>())]
...
function listSubfoldersForFolder(folderID) {
  return [...(store.state.subfolderIDsByFolderID.get(store.toPositiveInt(folderID, 0)) || new Set<any>())]
...
function getBookmarkCountInFolder(folderID) {
  return (store.state.bookmarkIDsByFolderID.get(store.toPositiveInt(folderID, 0)) || new Set<any>()).size;
}
