...
function sanitizeBookmarkCoordinates(record: Record<string, any> = {}) {
...
function normalizeBookmarkRecord(record: Record<string, any> = {}) {
...
function normalizeFolderRecord(record: Record<string, any> = {}) {
  return {
    folderID: toPositiveInt(record.folderID, 0),
...
function normalizeSubfolderRecord(record: Record<string, any> = {}) {
  return {
...
function normalizeKnownFolderRecord(record: Record<string, any> = {}, defaultIsActive = true) {
  return {
    folderID: toPositiveInt(record.folderID, 0),
...
function normalizeGroupRecord(record: Record<string, any> = {}) {
  return {
...
function rebuildIndexes() {
  ...
  for (const folder of state.foldersByID.values()) {
    if (folder.isPersonal && folder.creatorID) {
      if (!state.folderIDsByCreatorCharacterID.has(folder.creatorID)) {
        state.folderIDsByCreatorCharacterID.set(folder.creatorID, new Set<any>());
      }
  ...
  for (const bookmark of state.bookmarksByID.values()) {
    if (!state.bookmarkIDsByFolderID.has(bookmark.folderID)) {
      state.bookmarkIDsByFolderID.set(bookmark.folderID, new Set<any>());
    }
    ...
    if (!state.bookmarkIDsByLocationID.has(bookmark.locationID)) {
      state.bookmarkIDsByLocationID.set(bookmark.locationID, new Set<any>());
    }
    ...
    if (bookmark.subfolderID) {
      if (!state.bookmarkIDsBySubfolderID.has(bookmark.subfolderID)) {
        state.bookmarkIDsBySubfolderID.set(bookmark.subfolderID, new Set<any>());
      }
  ...
  for (const subfolder of state.subfoldersByID.values()) {
    if (!state.subfolderIDsByFolderID.has(subfolder.folderID)) {
      state.subfolderIDsByFolderID.set(subfolder.folderID, new Set<any>());
    }
