...
const { buildFiletimeLong, buildKeyVal, currentFileTime, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { resolveOmegaLicenseState, } = require(path.join(__dirname, "../newEdenStore/storeState"));
...
class SubscriptionMgrService extends BaseService {
    ...
    _readCloneGradeOverride() {
        ...
        if (val === "alpha" || val === "trial")
            return CLONE_STATE_ALPHA;
        const parsed = Number.parseInt(val, 10);
        if (parsed === CLONE_STATE_ALPHA || parsed === CLONE_STATE_OMEGA) {
            return parsed;
        }
        log.warn(`[SubscriptionMgr] Unsupported EVE_CLONE_GRADE="${raw}", defaulting to config-driven state`);
        return null;
    }
    ...
    _getSubscriptionState(session) {
        ...
        const omegaState = resolveOmegaLicenseState(session && session.userid);
        const subscriptionEndTime = isSubscribed && omegaState && omegaState.expiryFileTime
        ...
        if (subscriptionEndTime !== null) {
            ...
            if (ticksRemaining > 0n) {
                subscriptionDaysRemaining = Number((ticksRemaining + FILETIME_TICKS_PER_DAY - 1n) / FILETIME_TICKS_PER_DAY);
            }
        }
        return {
            cloneGrade,
    ...
    Handle_GetSubscriptionStatus(args, session) {
        ...
        log.debug(`[SubscriptionMgr] GetSubscriptionStatus -> subscribed=${state.isSubscribed} daysRemaining=${state.subscriptionDaysRemaining}`);
        return buildKeyVal([
    ...
    Handle_GetSubscriptionInfo(args, session) {
        const state = this._getSubscriptionState(session);
        log.debug(`[SubscriptionMgr] GetSubscriptionInfo -> subscribed=${state.isSubscribed} cloneGrade=${state.cloneGrade}`);
        return buildKeyVal([
            ["isSubscribed", state.isSubscribed],
            ["subscriptionDaysRemaining", state.subscriptionDaysRemaining],
    ...
    Handle_GetSubscriptionTime(args, session) {
        const state = this._getSubscriptionState(session);
        log.debug(`[SubscriptionMgr] GetSubscriptionTime -> ${state.subscriptionEndTime !== null ? state.subscriptionEndTime.toString() : "null"}`);
...
//# sourceMappingURL=subscriptionMgrService.js.map