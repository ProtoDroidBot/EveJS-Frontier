...
const { getAllianceRecord, } = require(path.join(__dirname, "../corporation/corporationState"));
const { getAllianceRuntime, } = require(path.join(__dirname, "../corporation/corporationRuntimeState"));
const { ATTRIBUTE_DEV_INDEX_INDUSTRIAL, ATTRIBUTE_DEV_INDEX_MILITARY, ATTRIBUTE_DEV_INDEX_SOVEREIGNTY, CAPITAL_SYSTEM_CHANGE_DELAY_MS, CLAIM_DAYS_TO_SECONDS, FILETIME_EPOCH_OFFSET, FILETIME_TICKS_PER_MILLISECOND, PRIME_TIME_CHANGE_DELAY_MS, STRUCTURE_SCORE_UPDATED, TYPE_INFRASTRUCTURE_HUB, TYPE_TERRITORIAL_CLAIM_UNIT, VULNERABILITY_WINDOW_BASE_HOURS, } = require(path.join(__dirname, "./sovConstants"));
const { buildDefaultSovereigntyTable, cloneValue: cloneStoreValue, readSovereigntyTable, writeSovereigntyTable, } = require(path.join(__dirname, "./sovStore"));
const { broadcastSolarSystemDevIndexChanged, broadcastSolarSystemSovStructuresUpdated, broadcastSovereigntyAudioEvent, broadcastSovereigntyChanged, broadcastSovHubHacked, } = require(path.join(__dirname, "./sovNotifications"));
const { clearAllSovereigntyRelatedStructures, clearAllSovereigntyStructureMirrors, syncSovereigntyStructureRuntime, } = require(path.join(__dirname, "./sovSpaceInterop"));
let tableCache = null;
let indexCache = null;
...
function unixMsToFiletimeString(value, fallback = "0") {
    ...
    return (FILETIME_EPOCH_OFFSET +
        BigInt(Math.trunc(numericValue)) * FILETIME_TICKS_PER_MILLISECOND).toString();
}
function addMillisecondsToCurrentFiletime(milliseconds) {
    ...
    return (currentFileTime() +
        BigInt(numericMilliseconds) * FILETIME_TICKS_PER_MILLISECOND).toString();
}
...
function normalizeStructureRecord(value = {}, solarSystemID, fallbackAllianceID, fallbackCorporationID) {
    ...
    const corporationID = normalizePositiveInteger(value.corporationID, fallbackCorporationID);
    ...
    return {
        ...
        vulnerabilityOccupancyLevel: normalizeNumber(value.vulnerabilityOccupancyLevel, 1),
...
function normalizeDevIndices(value = {}) {
    ...
    const claimedForDays = Math.max(0, normalizeInteger(value.claimedForDays ?? value.claimedFor, 0));
...
function applyAllianceTimedTransitions(allianceState) {
    ...
    const primeValidAfter = normalizeFiletimeBigInt(allianceState.primeInfo && allianceState.primeInfo.newPrimeHourValidAfter, 0n);
    if (allianceState.primeInfo &&
        normalizePrimeHour(allianceState.primeInfo.newPrimeHour, 0) >= 0 &&
        primeValidAfter > 0n &&
        primeValidAfter <= now) {
        allianceState.primeInfo.currentPrimeHour = normalizePrimeHour(allianceState.primeInfo.newPrimeHour, allianceState.primeInfo.currentPrimeHour);
    ...
    const capitalValidAfter = normalizeFiletimeBigInt(allianceState.capitalInfo && allianceState.capitalInfo.newCapitalSystemValidAfter, 0n);
    if (allianceState.capitalInfo &&
        allianceState.capitalInfo.newCapitalSystem &&
        capitalValidAfter > 0n &&
        capitalValidAfter <= now) {
        allianceState.capitalInfo.currentCapitalSystem = normalizePositiveInteger(allianceState.capitalInfo.newCapitalSystem, allianceState.capitalInfo.currentCapitalSystem);
...
function getStaticSystemLocationInfo(solarSystemID) {
    const staticSystem = worldData.ensureLoaded().solarSystemsById.get(Number(solarSystemID)) || null;
    return {
        solarSystemID: normalizePositiveInteger(staticSystem && staticSystem.solarSystemID, normalizePositiveInteger(solarSystemID, null)),
        constellationID: normalizePositiveInteger(staticSystem && staticSystem.constellationID, null),
...
function buildDerivedCurrentDataRecord(system = null, solarSystemID = null) {
    const locationInfo = getStaticSystemLocationInfo(solarSystemID || (system && system.solarSystemID));
    ...
    return {
        ...
        claimStructureID: normalizePositiveInteger(system && system.claimStructureID, null),
        infrastructureHubID: normalizePositiveInteger(system && system.infrastructureHubID, null),
        stationID: null,
...
function normalizeCurrentDataRecord(value = {}, solarSystemID, system = null) {
    ...
    const ownerSource = Object.prototype.hasOwnProperty.call(value, "ownerID")
        ...
        : Object.prototype.hasOwnProperty.call(value, "allianceID")
            ? value.allianceID
            : fallback.ownerID;
    const ownerID = normalizeNullablePositiveInteger(ownerSource, fallback.ownerID);
    const allianceSource = Object.prototype.hasOwnProperty.call(value, "allianceID")
        ...
        : Object.prototype.hasOwnProperty.call(value, "ownerID")
            ? value.ownerID
            : ownerID;
    const allianceID = normalizeNullablePositiveInteger(allianceSource, ownerID);
    return {
        ...
        constellationID: normalizePositiveInteger(value.constellationID, fallback.constellationID),
        regionID: normalizePositiveInteger(value.regionID, fallback.regionID),
        ownerID,
        allianceID,
        corporationID: normalizePositiveInteger(value.corporationID, fallback.corporationID),
        claimStructureID: normalizePositiveInteger(value.claimStructureID, fallback.claimStructureID),
        infrastructureHubID: normalizePositiveInteger(value.infrastructureHubID, fallback.infrastructureHubID),
...
function normalizeRecentActivityRecord(value = {}, solarSystemID, system = null) {
    ...
    return {
        solarSystemID: normalizePositiveInteger(value.solarSystemID, locationInfo.solarSystemID),
        ownerID: normalizeNullablePositiveInteger(ownerSource, normalizePositiveInteger(system && system.allianceID, null)),
...
function buildDefaultAllianceState(allianceID) {
    ...
    return {
        ...
        primeInfo: {
            currentPrimeHour: normalizeInteger(runtime.primeInfo && runtime.primeInfo.currentPrimeHour, 0),
            newPrimeHour: normalizeInteger(runtime.primeInfo && runtime.primeInfo.newPrimeHour, 0),
            newPrimeHourValidAfter: normalizeFiletimeString(runtime.primeInfo && runtime.primeInfo.newPrimeHourValidAfter, "0"),
        },
        capitalInfo: {
            currentCapitalSystem: normalizePositiveInteger(runtime.capitalInfo && runtime.capitalInfo.currentCapitalSystem, null),
            newCapitalSystem: normalizePositiveInteger(runtime.capitalInfo && runtime.capitalInfo.newCapitalSystem, null),
            newCapitalSystemValidAfter: normalizeFiletimeString(runtime.capitalInfo && runtime.capitalInfo.newCapitalSystemValidAfter, "0"),
        },
    };
}
function normalizeAllianceState(value = {}, allianceID) {
    ...
    return {
        ...
        primeInfo: {
            currentPrimeHour: normalizeInteger(value.primeInfo && value.primeInfo.currentPrimeHour, fallback.primeInfo.currentPrimeHour),
            newPrimeHour: normalizeInteger(value.primeInfo && value.primeInfo.newPrimeHour, fallback.primeInfo.newPrimeHour),
            newPrimeHourValidAfter: normalizeFiletimeString(value.primeInfo && value.primeInfo.newPrimeHourValidAfter, fallback.primeInfo.newPrimeHourValidAfter),
        },
        capitalInfo: {
            currentCapitalSystem: normalizePositiveInteger(value.capitalInfo && value.capitalInfo.currentCapitalSystem, fallback.capitalInfo.currentCapitalSystem),
            newCapitalSystem: normalizePositiveInteger(value.capitalInfo && value.capitalInfo.newCapitalSystem, fallback.capitalInfo.newCapitalSystem),
            newCapitalSystemValidAfter: normalizeFiletimeString(value.capitalInfo && value.capitalInfo.newCapitalSystemValidAfter, fallback.capitalInfo.newCapitalSystemValidAfter),
        },
    };
}
function normalizeSystemState(value = {}, solarSystemID) {
    ...
    const hasCorporationIDField = Object.prototype.hasOwnProperty.call(value, "corporationID");
    const hasClaimStructureField = Object.prototype.hasOwnProperty.call(value, "claimStructureID");
    const hasInfrastructureHubField = Object.prototype.hasOwnProperty.call(value, "infrastructureHubID");
    ...
    let structures = Array.isArray(value.structures)
        ? value.structures
            .map((entry) => normalizeStructureRecord(entry, numericSolarSystemID, allianceID, corporationID))
            .filter(Boolean)
        : [];
    const tcuStructure = structures.find((entry) => entry.typeID === TYPE_TERRITORIAL_CLAIM_UNIT);
    const iHubStructure = structures.find((entry) => entry.typeID === TYPE_INFRASTRUCTURE_HUB);
    if (!hasAllianceIDField && !allianceID) {
        allianceID = normalizePositiveInteger(tcuStructure && tcuStructure.allianceID, normalizePositiveInteger(iHubStructure && iHubStructure.allianceID, null));
    }
    if (!hasCorporationIDField && !corporationID) {
        corporationID = normalizePositiveInteger(iHubStructure && iHubStructure.corporationID, normalizePositiveInteger(tcuStructure && tcuStructure.corporationID, null));
    }
    ...
    return {
        ...
        infrastructureHubID: hasInfrastructureHubField
            ...
            : normalizePositiveInteger(value.infrastructureHubID, iHubStructure && iHubStructure.itemID),
        ...
        recentActivity: Array.isArray(value.recentActivity)
            ? value.recentActivity
                .map((entry) => normalizeRecentActivityRecord(entry, numericSolarSystemID, value))
                .filter(Boolean)
            : [],
        currentData: Array.isArray(value.currentData)
            ? value.currentData
                .map((entry) => normalizeCurrentDataRecord(entry, numericSolarSystemID, value))
                .filter(Boolean)
            : [],
    };
}
...
function normalizeTable(table = {}) {
    ...
    normalized.mercenaryDens = cloneStoreValue(table && table.mercenaryDens ? table.mercenaryDens : {});
    normalized.resources = cloneStoreValue(table && table.resources ? table.resources : normalized.resources);
    for (const [allianceID, record] of Object.entries(table.alliances || {})) {
        ...
        normalized.alliances[String(numericAllianceID)] = normalizeAllianceState(record, numericAllianceID);
    }
    for (const [solarSystemID, record] of Object.entries(table.systems || {})) {
        ...
        normalized.systems[String(numericSolarSystemID)] = normalizeSystemState(record, numericSolarSystemID);
    }
    return normalized;
}
...
function buildVulnerabilityWindow(primeHour, defenseMultiplier, nowMs = Date.now()) {
    ...
    const safeMultiplier = Math.max(MIN_OCCUPANCY_BONUS, normalizeNumber(defenseMultiplier, BASE_BONUS));
    const durationMs = Math.max(HOUR_MS, Math.round((VULNERABILITY_WINDOW_BASE_HOURS * HOUR_MS) / safeMultiplier));
    const referenceDate = new Date(Math.max(0, normalizeInteger(nowMs, Date.now())));
    let centerMs = Date.UTC(referenceDate.getUTCFullYear(), referenceDate.getUTCMonth(), referenceDate.getUTCDate(), centeredPrimeHour, 0, 0, 0);
...
function buildStructureSnapshot(structure, system, capitalSystemByAllianceID, primeHourByAllianceID, nowMs = Date.now()) {
    const allianceID = normalizePositiveInteger(structure && structure.allianceID, null);
    const isCapital = normalizePositiveInteger(structure && structure.typeID, null) ===
    ...
    let vulnerableStartTime = normalizeFiletimeString(structure && structure.vulnerableStartTime, "0");
    let vulnerableEndTime = normalizeFiletimeString(structure && structure.vulnerableEndTime, "0");
    if (allianceID &&
        (vulnerableStartTime === "0" || vulnerableEndTime === "0")) {
        const vulnerabilityWindow = buildVulnerabilityWindow(getAlliancePrimeHourFromIndexes(allianceID, primeHourByAllianceID), defenseMultiplier, nowMs);
...
function buildIndexes() {
    ...
    const getAllianceStructureRowID = (system, structure, hasInfrastructureHubStructure) => {
        ...
        if (normalizePositiveInteger(structure && structure.typeID, null) !==
        ...
        return normalizePositiveInteger(system && system.infrastructureHubID, structureID);
    };
    for (const allianceState of Object.values(tableCache.alliances || {})) {
        ...
        primeHourByAllianceID.set(allianceID, normalizePrimeHour(allianceState.primeInfo && allianceState.primeInfo.currentPrimeHour, 0));
        const capitalSystemID = normalizePositiveInteger(allianceState.capitalInfo && allianceState.capitalInfo.currentCapitalSystem, null);
        if (capitalSystemID) {
            capitalSystemByAllianceID.set(allianceID, capitalSystemID);
        }
        const pendingPrimeMs = filetimeToUnixMs(allianceState.primeInfo && allianceState.primeInfo.newPrimeHourValidAfter, 0);
        if (pendingPrimeMs > nowMs) {
            nextInvalidationAtMs = Math.min(nextInvalidationAtMs, pendingPrimeMs);
        }
        const pendingCapitalMs = filetimeToUnixMs(allianceState.capitalInfo && allianceState.capitalInfo.newCapitalSystemValidAfter, 0);
        if (pendingCapitalMs > nowMs) {
            nextInvalidationAtMs = Math.min(nextInvalidationAtMs, pendingCapitalMs);
        }
    }
    for (const system of Object.values(tableCache.systems || {}).sort((left, right) => left.solarSystemID - right.solarSystemID)) {
        ...
        const structureSnapshots = (system.structures || [])
            .map((structure) => buildStructureSnapshot(structure, system, capitalSystemByAllianceID, primeHourByAllianceID, nowMs))
            .sort((left, right) => left.itemID - right.itemID);
        const hasInfrastructureHubStructure = structureSnapshots.some((structure) => structure.typeID === TYPE_INFRASTRUCTURE_HUB);
        ...
        for (const structure of structureSnapshots) {
            ...
            const allianceStructureRowID = getAllianceStructureRowID(system, structure, hasInfrastructureHubStructure);
            const baseRow = {
    ...
    for (const rows of allianceSovRowsByAllianceID.values()) {
        rows.tcuRows.sort((left, right) => left.solarSystemID - right.solarSystemID ||
            left.structureID - right.structureID);
        rows.iHubRows.sort((left, right) => left.solarSystemID - right.solarSystemID ||
            left.structureID - right.structureID);
        rows.campaignScoreRows.sort((left, right) => left.sourceItemID - right.sourceItemID ||
            left.teamID - right.teamID);
    }
    for (const staticSystem of [...worldData.ensureLoaded().solarSystems].sort((left, right) => Number(left.solarSystemID) - Number(right.solarSystemID))) {
        ...
        const rows = systemState && systemState.currentData.length > 0
    ...
    for (const system of Object.values(tableCache.systems || {})) {
        ...
        const rows = system.currentData.length > 0
            ? system.currentData
    ...
    for (const rows of currentSovDataByLocationID.values()) {
        rows.sort((left, right) => {
            const locationDelta = Number(left.locationID || 0) - Number(right.locationID || 0);
            if (locationDelta !== 0) {
                return locationDelta;
            }
            const stationDelta = Number(left.stationID || 0) - Number(right.stationID || 0);
            if (stationDelta !== 0) {
                return stationDelta;
            }
    ...
    recentSovActivity.sort((left, right) => {
        const timeDelta = compareNumericStringsDescending(left.changeTime, right.changeTime);
        if (timeDelta !== 0) {
            return timeDelta;
        }
    ...
    indexCache = {
        ...
        debugSnapshot: {
            updatedAt: tableCache && tableCache._meta ? tableCache._meta.updatedAt || null : null,
            ...
            nextInvalidationAtMs: Number.isFinite(nextInvalidationAtMs)
                ...
                : null,
            capitalSystemByAllianceID: Object.fromEntries([...capitalSystemByAllianceID.entries()].sort((left, right) => left[0] - right[0])),
        },
        generatedAtMs: nowMs,
        nextInvalidationAtMs: Number.isFinite(nextInvalidationAtMs)
            ? Math.trunc(nextInvalidationAtMs)
...
function ensureLoaded() {
    ...
    const updatedAt = rawTable && rawTable._meta ? rawTable._meta.updatedAt || null : null;
...
function setAlliancePrimeHour(allianceID, hour) {
    ...
    if (allianceState.primeInfo.currentPrimeHour === nextHour &&
    ...
    allianceState.primeInfo.newPrimeHourValidAfter = addMillisecondsToCurrentFiletime(PRIME_TIME_CHANGE_DELAY_MS);
    persist();
    return cloneValue(allianceState.primeInfo);
}
...
function setAllianceCapitalSystem(allianceID, solarSystemID) {
    const allianceState = getResolvedAllianceState(allianceID);
    const nextCapitalSystemID = normalizePositiveInteger(solarSystemID, null);
    if (!nextCapitalSystemID) {
        return cloneValue(allianceState.capitalInfo);
    }
    if (allianceState.capitalInfo.currentCapitalSystem === nextCapitalSystemID &&
        !allianceState.capitalInfo.newCapitalSystem) {
        return cloneValue(allianceState.capitalInfo);
    }
    allianceState.capitalInfo.newCapitalSystem = nextCapitalSystemID;
    allianceState.capitalInfo.newCapitalSystemValidAfter = addMillisecondsToCurrentFiletime(CAPITAL_SYSTEM_CHANGE_DELAY_MS);
    persist();
    return cloneValue(allianceState.capitalInfo);
}
...
function publishSystemStateEffects(currentValue, nextValue, options = {}) {
    const numericSolarSystemID = normalizePositiveInteger((nextValue && nextValue.solarSystemID) ||
        (currentValue && currentValue.solarSystemID), null);
    if (!numericSolarSystemID) {
        return { success: false, errorMsg: "SOLAR_SYSTEM_REQUIRED" };
    }
    const current = normalizeSystemState(currentValue || {}, numericSolarSystemID);
    const next = normalizeSystemState(nextValue || {}, numericSolarSystemID);
    const structuresChanged = !valuesEqual(current.structures, next.structures);
    let structureSyncResult = null;
    if (!normalizeBoolean(options.suppressStructureSync, false) &&
        structuresChanged) {
        structureSyncResult = syncSovereigntyStructureRuntime(next, {
    ...
    if (!normalizeBoolean(options.suppressNotifications, false)) {
        ...
        if (current.allianceID !== next.allianceID ||
            current.corporationID !== next.corporationID ||
            current.claimStructureID !== next.claimStructureID) {
            broadcastSovereigntyChanged(numericSolarSystemID, getSystemSovClaim(numericSolarSystemID), allianceIDs);
        }
        if (structuresChanged) {
            broadcastSolarSystemSovStructuresUpdated(numericSolarSystemID, structureSnapshots, options.changesByStructureID || null, { allianceIDs });
        }
        if (options.audioEventID) {
            broadcastSovereigntyAudioEvent(numericSolarSystemID, options.audioEventID, options.audioTextParams || {}, allianceIDs);
        }
        if (options.sovHubHack) {
            broadcastSovHubHacked(numericSolarSystemID, options.sovHubHack.sovHubID, options.sovHubHack.upgrades || [], allianceIDs);
        }
    }
    return {
        success: !structureSyncResult || structureSyncResult.success !== false,
        errorMsg: structureSyncResult && structureSyncResult.success === false
            ? structureSyncResult.errorMsg || "STRUCTURE_SYNC_FAILED"
            : null,
        data: {
            solarSystemID: numericSolarSystemID,
            structuresChanged,
...
function replaceSystemStateSnapshot(solarSystemID, snapshot, options = {}) {
    ensureLoaded();
    const numericSolarSystemID = normalizePositiveInteger(solarSystemID, null);
    if (!numericSolarSystemID) {
        return null;
    }
    const current = cloneValue(getSystemState(numericSolarSystemID));
    const next = snapshot == null
        ? null
        : normalizeSystemState(cloneValue(snapshot), numericSolarSystemID);
    if (next) {
        tableCache.systems[String(numericSolarSystemID)] = next;
    }
    else {
        delete tableCache.systems[String(numericSolarSystemID)];
    }
    persist();
    try {
        const { invalidateSovereigntyModernStateCache, } = require(path.join(__dirname, "./sovModernState"));
        invalidateSovereigntyModernStateCache();
    }
    catch (_error) {
        // Avoid bootstrap-time circular dependency failures.
    }
    publishSystemStateEffects(current, next, options);
    return cloneValue(next);
}
...
function mutateStructureInSystem(solarSystemID, structureID, mutator, options = {}) {
    ...
    const structureIndex = system.structures.findIndex((entry) => normalizePositiveInteger(entry && entry.itemID, null) === numericStructureID);
    if (structureIndex < 0) {
        return null;
    }
    const normalizedStructure = normalizeStructureRecord(mutator(cloneValue(system.structures[structureIndex])), system.solarSystemID, system.allianceID, system.corporationID);
    if (!normalizedStructure) {
        return null;
    }
    const nextStructures = cloneValue(system.structures);
    nextStructures[structureIndex] = normalizedStructure;
    return upsertSystemState(system.solarSystemID, {
        structures: nextStructures,
    }, options);
}
function setStructureCampaignState(solarSystemID, structureID, campaignState = {}, options = {}) {
    return mutateStructureInSystem(solarSystemID, structureID, (structure) => ({
        ...structure,
        ...
        campaignScoresByTeam: campaignState.campaignScoresByTeam !== undefined
...
function setStructureCampaignScores(solarSystemID, structureID, campaignScoresByTeam = {}, options = {}) {
    const numericStructureID = normalizePositiveInteger(structureID, null);
    return mutateStructureInSystem(solarSystemID, structureID, (structure) => ({
        ...structure,
...
function setStructureVulnerabilityState(solarSystemID, structureID, vulnerabilityState = {}, options = {}) {
    return mutateStructureInSystem(solarSystemID, structureID, (structure) => ({
        ...structure,
        vulnerableStartTime: normalizeFiletimeString(vulnerabilityState.vulnerableStartTime, "0"),
        vulnerableEndTime: normalizeFiletimeString(vulnerabilityState.vulnerableEndTime, "0"),
    }), options);
}
...
function getInfrastructureHubClaim(solarSystemID) {
    ...
    return cloneValue(indexCache.infrastructureHubClaimsBySolarSystemID.get(numericSolarSystemID) || null);
}
...
function getAllianceSovereigntyRows(allianceID) {
    ...
    return cloneValue(indexCache.allianceSovRowsByAllianceID.get(numericAllianceID) || {
        tcuRows: [],
        iHubRows: [],
        campaignScoreRows: [],
    });
...
function emitSovereigntyAudioEvent(solarSystemID, eventID, textParams = {}, allianceIDs = []) {
    broadcastSovereigntyAudioEvent(solarSystemID, eventID, textParams, allianceIDs);
}
function emitSovHubHacked(solarSystemID, sovHubID, upgrades = [], allianceIDs = []) {
...
function wipeAllSovereigntyState(options = {}) {
    ...
    if (options.preserveResources !== false) {
        nextTable.resources = cloneStoreValue(currentTable && currentTable.resources ? currentTable.resources : nextTable.resources);
    }
    if (options.preserveVersion !== false) {
        nextTable._meta.version = Math.max(1, normalizeInteger(currentTable && currentTable._meta && currentTable._meta.version, nextTable._meta.version));
    }
...
//# sourceMappingURL=sovState.js.map