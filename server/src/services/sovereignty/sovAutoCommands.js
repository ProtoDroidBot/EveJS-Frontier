...
const { buildJobSummary, listActiveJobs, resetAutomationAndState, startClaim, startLoss, startTakeover, stopAutomation, } = require(path.join(__dirname, "./sovAutoState"));
const { resolveAllianceID, resolveCorporationID, resolveSolarSystemID, } = require(path.join(__dirname, "./sovGmState"));
const { findUnclaimedSolarSystem, } = require(path.join(__dirname, "./sovAutoNavigation"));
const { isSovereigntyClaimableSolarSystem, } = require(path.join(__dirname, "./sovSystemRules"));
const { SHOWCASE_SOV_FLEX_REQUIRED_UPGRADE_TYPE_IDS, canSolarSystemSupportSovFlexShowcase, } = require(path.join(__dirname, "./sovUpgradeSupport"));
...
function executeSovAutoCommand(session, argumentText, chatHub = null) {
    ...
    if (subcommand === "claim") {
        ...
        if (!targetResult.success || !allianceID || !corporationID) {
            return {
                success: false,
                message: targetResult && targetResult.errorMsg === "SOLAR_SYSTEM_NOT_CONQUERABLE"
        ...
        const startResult = startClaim(session, targetResult.data.solarSystemID, allianceID, corporationID, { chatHub });
    ...
    if (subcommand === "takeover" || subcommand === "gain") {
        ...
        const solarSystemID = resolveSolarSystemID(session, hasExplicitSystem ? rest[0] : "here", 0);
        const allianceID = normalizePositiveInteger(hasExplicitSystem ? rest[1] : rest[0], null);
        const corporationID = normalizePositiveInteger(hasExplicitSystem ? rest[2] : rest[1], null);
        ...
        const startResult = startTakeover(session, solarSystemID, allianceID, corporationID, { chatHub });
        if (!startResult.success) {
            return {
                success: false,
    ...
    if (subcommand === "loss") {
        ...
        const startResult = startLoss(session, solarSystemID, nextAllianceID, nextCorporationID, { chatHub });
        if (!startResult.success) {
            return {
                success: false,
...
//# sourceMappingURL=sovAutoCommands.js.map