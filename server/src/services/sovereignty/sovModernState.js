...
const { getSovereigntyStaticSnapshot, } = require(path.join(__dirname, "./sovStaticData"));
const { cloneValue, mergeSovereigntyTableDefaults, normalizeSemanticVersion, readSovereigntyTable, writeSovereigntyTable, } = require(path.join(__dirname, "./sovStore"));
const { listStructures, } = require(path.join(__dirname, "../structure/structureState"));
...
function getStrategicIndexLevelForSystem(system = null) {
    return require(path.join(__dirname, "./sovState")).getStrategicIndexLevel(system &&
        system.devIndices &&
...
function normalizeFuelLedger(value = {}) {
    ...
    for (const [fuelTypeID, fuelEntry] of Object.entries(value)) {
        ...
        normalized[String(numericFuelTypeID)] = {
            amount: Math.max(0, normalizeInteger(fuelEntry && fuelEntry.amount, 0)),
        };
    }
    return normalized;
}
function normalizeHubRecord(value = {}, fallback = {}) {
    ...
    return {
        ...
        fuelLastUpdatedMs: normalizeTimestampMs(value.fuelLastUpdatedMs, fallback.fuelLastUpdatedMs || 0),
        workforceConfiguration: normalizeWorkforceConfiguration(value.workforceConfiguration, fallback.hubID),
        workforceLastUpdatedMs: normalizeTimestampMs(value.workforceLastUpdatedMs, fallback.workforceLastUpdatedMs || 0),
        upgradesLastUpdatedMs: normalizeTimestampMs(value.upgradesLastUpdatedMs, fallback.upgradesLastUpdatedMs || 0),
    };
}
function normalizeSkyhookRecord(value = {}, fallback = {}) {
    ...
    return {
        ...
        workforceAmount: normalizePositiveInteger(value.workforceAmount, fallback.workforceAmount || null),
        reagentDefinitions: rawDefinitions
            .map((entry) => ({
            reagentTypeID: normalizePositiveInteger(entry.reagentTypeID, null),
            amountPerCycle: Math.max(0, normalizeInteger(entry.amountPerCycle, 0)),
            cyclePeriodSeconds: Math.max(0, normalizeInteger(entry.cyclePeriodSeconds, 0)),
            securedPercentage: Math.max(0, normalizeInteger(entry.securedPercentage, 0)),
            securedCapacity: Math.max(0, normalizeInteger(entry.securedCapacity, 0)),
            unsecuredCapacity: Math.max(0, normalizeInteger(entry.unsecuredCapacity, 0)),
        }))
        ...
        reagentSimulations: rawSimulations
            ...
            .filter((entry) => entry.reagentTypeID),
        theftVulnerability: {
            startMs: normalizeTimestampMs(value &&
                value.theftVulnerability &&
                value.theftVulnerability.startMs, fallback &&
                fallback.theftVulnerability &&
                fallback.theftVulnerability.startMs),
            endMs: normalizeTimestampMs(value &&
                value.theftVulnerability &&
                value.theftVulnerability.endMs, fallback &&
                fallback.theftVulnerability &&
                fallback.theftVulnerability.endMs),
        },
        lastUpdatedMs: normalizeTimestampMs(value.lastUpdatedMs, fallback.lastUpdatedMs || Date.now()),
    };
}
...
function buildResourcesTableFromStatic(staticSnapshot) {
    return {
        ...
        planetDefinitionsVersion: normalizeSemanticVersion(staticSnapshot.planetDefinitionsVersion),
...
function semanticVersionEquals(left, right) {
    ...
    return (normalizedLeft.major === normalizedRight.major &&
...
function resourceRowsMatch(currentRows, targetRows, idName) {
    ...
    const targetByID = new Map(targetRows.map((row) => [normalizePositiveInteger(row && row[idName], null), row]));
...
function getSkyhookHarvestContribution(skyhook, staticSnapshot) {
    ...
    const planetDefinition = staticSnapshot.planetDefinitionsByPlanetID.get(normalizePositiveInteger(skyhook.planetID, 0));
    return {
        power: Number(planetDefinition && planetDefinition.power) || 0,
        workforce: normalizePositiveInteger(skyhook.workforceAmount, Number(planetDefinition && planetDefinition.workforce) || 0) || 0,
    };
}
function ensureStaticResources(table, staticSnapshot) {
    const needsBootstrap = !table.resources ||
    ...
    if (!semanticVersionEquals(table.resources.planetDefinitionsVersion, staticSnapshot.planetDefinitionsVersion) ||
        !resourceRowsMatch(table.resources.planetDefinitions, staticSnapshot.planetDefinitions, "planetID") ||
        !resourceRowsMatch(table.resources.starConfigurations, staticSnapshot.starConfigurations, "starID") ||
        !resourceRowsMatch(table.resources.upgradeDefinitions, staticSnapshot.upgradeDefinitions, "installationTypeID")) {
        table.resources = buildResourcesTableFromStatic(staticSnapshot);
        return true;
    }
    return false;
...
function bootstrapHubsFromSystems(table) {
    ...
    for (const [solarSystemID, system] of Object.entries(table.systems || {})) {
        ...
        const hubID = normalizePositiveInteger(system && system.infrastructureHubID, null);
        const allianceID = normalizePositiveInteger(system && system.allianceID, null);
        const corporationID = normalizePositiveInteger(system && system.corporationID, null);
        ...
        const normalized = normalizeHubRecord(table.hubs && table.hubs[key] ? table.hubs[key] : {}, fallback);
    ...
    for (const structure of listStructures({
        ...
        const corporationID = normalizePositiveInteger(structure && (structure.ownerCorpID || structure.corporationID || structure.ownerID), null);
        const allianceID = normalizePositiveInteger(structure && structure.allianceID, null);
        if (Number(structure && structure.typeID) !== 32458 ||
...
function mergeReagentSimulationsWithDefinitions(currentSimulations, definitions) {
    ...
    for (const simulation of Array.isArray(currentSimulations) ? currentSimulations : []) {
        const reagentTypeID = normalizePositiveInteger(simulation && simulation.reagentTypeID, null);
        if (reagentTypeID) {
    ...
    return (Array.isArray(definitions) ? definitions : [])
        .map((reagent) => {
        const reagentTypeID = normalizePositiveInteger(reagent && reagent.reagentTypeID, null);
        if (!reagentTypeID) {
            return null;
        }
        ...
        return {
            reagentTypeID,
            securedStock: Math.max(0, normalizeInteger(current.securedStock, reagent.securedStock || 0)),
            unsecuredStock: Math.max(0, normalizeInteger(current.unsecuredStock, reagent.unsecuredStock || 0)),
...
function bootstrapSkyhooksFromSystems(table, staticSnapshot, options = {}) {
    ...
    for (const [solarSystemID, system] of Object.entries(table.systems || {})) {
        ...
        const planetIDs = staticSnapshot.planetsBySolarSystemID.get(numericSolarSystemID) || [];
        for (const planetID of planetIDs) {
            ...
            const normalized = normalizeSkyhookRecord(table.skyhooks && table.skyhooks[key] ? table.skyhooks[key] : {}, fallback);
            ...
            if (refreshResourceDefinitions) {
                ...
                normalized.reagentSimulations = mergeReagentSimulationsWithDefinitions(normalized.reagentSimulations, fallback.reagentDefinitions);
            }
...
function getHubLocalHarvest(hubRecord, stateCache) {
    ...
    const starConfiguration = stateCache.staticSnapshot.starConfigurationsBySolarSystemID.get(solarSystemID) ||
        null;
    const harvested = stateCache.localHarvestBySolarSystemID.get(solarSystemID) || null;
    return {
        power: Number(harvested && harvested.power) ||
...
function getHubTransferDelta(hubRecord, stateCache) {
    const configuration = normalizeWorkforceConfiguration(hubRecord && hubRecord.workforceConfiguration, hubRecord && hubRecord.hubID);
...
function computeHubUpgradeSnapshot(hubRecord, stateCache) {
    ...
    let workforceAvailable = Math.max(0, workforceLocalHarvest + workforceTransferDelta);
    let powerAllocated = 0;
    let workforceAllocated = 0;
    const upgrades = [];
    for (const installation of hubRecord.installedUpgrades) {
        const definition = stateCache.staticSnapshot.upgradeDefinitionsByTypeID.get(installation.typeID);
        if (!definition) {
            continue;
        }
        let powerState = POWER_STATE.POWER_STATE_OFFLINE;
        if (installation.online) {
            const canPower = powerAllocated + definition.powerRequired <= powerAvailable;
            const canWorkforce = workforceAllocated + definition.workforceRequired <= workforceAvailable;
...
function buildWorkforceState(configuration, hubRecord, identityCache) {
    if (configuration.mode === "import") {
        const sources = configuration.sourceSystemIDs.map((sourceSystemID) => {
            const sourceHub = identityCache.hubsBySolarSystemID.get(sourceSystemID) || null;
            return {
                sourceSystemID,
    ...
    if (configuration.mode === "export") {
        const connected = configuration.destinationSystemID &&
...
function deriveSkyhookTheftVulnerability(skyhook) {
    const now = Date.now();
    const startMs = normalizeTimestampMs(skyhook &&
        skyhook.theftVulnerability &&
        skyhook.theftVulnerability.startMs, 0);
    const endMs = normalizeTimestampMs(skyhook &&
        skyhook.theftVulnerability &&
        skyhook.theftVulnerability.endMs, 0);
    return {
        vulnerable: Boolean(skyhook && skyhook.active) &&
            startMs > 0 &&
            endMs > 0 &&
            now >= startMs &&
            now <= endMs,
...
function ensureLoaded() {
    ...
    for (const [hubID, rawHub] of Object.entries(table.hubs || {})) {
        ...
        nextCache.hubsBySolarSystemID.set(normalizedHub.solarSystemID, normalizedHub);
    }
    for (const [skyhookID, rawSkyhook] of Object.entries(table.skyhooks || {})) {
        ...
        if (!normalizedSkyhook.skyhookID ||
        ...
        if (!nextCache.localHarvestBySolarSystemID.has(normalizedSkyhook.solarSystemID)) {
            const starConfiguration = staticSnapshot.starConfigurationsBySolarSystemID.get(normalizedSkyhook.solarSystemID) || null;
        ...
        const localHarvest = nextCache.localHarvestBySolarSystemID.get(normalizedSkyhook.solarSystemID);
        const contribution = getSkyhookHarvestContribution(normalizedSkyhook, staticSnapshot);
        ...
        if (vulnerability.endMs > Date.now()) {
            if (!nextCache.vulnerableSkyhooksBySolarSystemID.has(normalizedSkyhook.solarSystemID)) {
                nextCache.vulnerableSkyhooksBySolarSystemID.set(normalizedSkyhook.solarSystemID, []);
            }
    ...
    for (const hub of nextCache.hubsByID.values()) {
        nextCache.hubSnapshotsByID.set(hub.hubID, computeHubUpgradeSnapshot(hub, nextCache));
    }
    cache = nextCache;
    return cache;
}
...
function hasAllianceOrCorporationAccess(owner, identity) {
    ...
    if (normalizePositiveInteger(owner.corporationID, 0) > 0 &&
        normalizePositiveInteger(identity.corporationID, 0) ===
            normalizePositiveInteger(owner.corporationID, 0)) {
        return true;
    }
    return (normalizePositiveInteger(owner.allianceID, 0) > 0 &&
...
function requireSkyhookAccess(skyhookID, identity, options = {}) {
    const skyhook = ensureLoaded().skyhooksByID.get(normalizePositiveInteger(skyhookID, 0)) || null;
    if (!skyhook) {
        return {
            ok: false,
            statusCode: 404,
...
function buildHubUpgradePayloadState(hubID) {
    ...
    const upgradeSnapshot = stateCache.hubSnapshotsByID.get(hub.hubID) ||
...
function validateUpgradeConfiguration(installations, staticSnapshot, system = null) {
    ...
    for (const installation of installations) {
        ...
        const requiredStrategicIndex = Math.max(0, normalizeInteger(definition.requiredStrategicIndex, 0));
...
function processHubUpgradeConfiguration(hubID, newUpgradeItemIDs, configurations, identity) {
    const access = requireHubAccess(hubID, identity, true);
    if (!access.ok) {
        return access;
    }
    ...
    try {
        return mutateTable((table) => {
            ...
            const installationsByTypeID = new Map(hub.installedUpgrades.map((installation) => [installation.typeID, installation]));
            ...
            const installations = [...installationsByTypeID.values()].sort((left, right) => left.typeID - right.typeID);
            const system = table.systems[String(hub.solarSystemID)] || {
                solarSystemID: hub.solarSystemID,
            };
            const validation = validateUpgradeConfiguration(installations, staticSnapshot, system);
            if (!validation.ok) {
                throw validation;
            }
...
function setHubUpgradeInstallations(hubID, installations, identity) {
    const access = requireHubAccess(hubID, identity, true);
    if (!access.ok) {
        return access;
    }
    ...
    try {
        return mutateTable((table) => {
            ...
            const nextInstallations = [...installationsByTypeID.values()].sort((left, right) => left.typeID - right.typeID);
            const system = table.systems[String(hub.solarSystemID)] || {
                solarSystemID: hub.solarSystemID,
            };
            const validation = validateUpgradeConfiguration(nextInstallations, staticSnapshot, system);
            if (!validation.ok) {
                throw validation;
            }
...
function uninstallHubUpgrade(hubID, upgradeTypeID, identity) {
    ...
    try {
        return mutateTable((table) => {
            ...
            hub.installedUpgrades = hub.installedUpgrades.filter((installation) => installation.typeID !== numericUpgradeTypeID);
...
function getHubFuel(hubID, identity = null) {
    ...
    for (const upgrade of upgradeState.upgrades || []) {
        ...
        burnedPerHourByFuelType.set(definition.fuelTypeID, currentBurn + Number(definition.fuelConsumptionPerHour || 0));
    }
    ...
    return {
        ...
        fuels: [...fuelTypeIDs]
            ...
            .map((typeID) => ({
            fuelTypeID: typeID,
            amount: Number(hub.fuelByTypeID &&
                hub.fuelByTypeID[String(typeID)] &&
                hub.fuelByTypeID[String(typeID)].amount) || 0,
...
function seedHubFuelForInstalledUpgrades(hubID, hours = 168, identity = null) {
    ...
    for (const upgrade of payloadState.upgrades || []) {
        ...
        const amountToSeed = Math.max(0, Number(definition.fuelStartupCost || 0)) +
            (Math.max(0, Number(definition.fuelConsumptionPerHour || 0)) * seededHours);
        seededAmountsByFuelType.set(fuelTypeID, (seededAmountsByFuelType.get(fuelTypeID) || 0) + amountToSeed);
    }
    return mutateTable((table) => {
        const hub = normalizeHubRecord(table.hubs[String(access.hub.hubID)], access.hub);
        for (const [fuelTypeID, amount] of seededAmountsByFuelType.entries()) {
            const currentEntry = hub.fuelByTypeID[String(fuelTypeID)] || { amount: 0 };
            hub.fuelByTypeID[String(fuelTypeID)] = {
                amount: Math.max(normalizeInteger(currentEntry.amount, 0), Math.ceil(Math.max(0, Number(amount) || 0))),
            };
        }
        hub.fuelLastUpdatedMs = Date.now();
        table.hubs[String(hub.hubID)] = hub;
...
function getWorkforceState(hubID, identity = null) {
    ...
    return {
        hubID: hub.hubID,
        solarSystemID: hub.solarSystemID,
        state: buildWorkforceState(hub.workforceConfiguration, hub, stateCache),
        lastUpdatedMs: hub.workforceLastUpdatedMs || 0,
    };
}
function configureWorkforce(hubID, configuration, identity) {
    ...
    const normalizedConfiguration = normalizeWorkforceConfiguration(configuration, access.hub.hubID);
    ...
    return mutateTable((table) => {
        ...
        table.hubs[String(hub.hubID)] = hub;
        const refreshedState = buildWorkforceState(normalizedConfiguration, hub, ensureLoaded());
        return {
            ok: true,
            hubID: hub.hubID,
            solarSystemID: hub.solarSystemID,
...
function listNetworkableHubs(hubID, identity = null) {
    ...
    const hubs = [...stateCache.hubsByID.values()]
        .filter((candidate) => candidate.hubID !== hub.hubID &&
        ...
        state: buildWorkforceState(candidate.workforceConfiguration, candidate, stateCache),
    }));
    return {
        hubID: hub.hubID,
        solarSystemID: hub.solarSystemID,
...
function buildSkyhookGatewayState(skyhook) {
    ...
    return {
        ...
        planetResourcesVersion: cloneValue(stateCache.staticSnapshot.planetDefinitionsVersion),
    };
}
...
function listLocalSkyhooks(identity) {
    ...
    return {
        solarSystemID,
        skyhooks: cloneValue((ensureLoaded().skyhooksBySolarSystemID.get(solarSystemID) || []).map(buildSkyhookGatewayState)),
    };
}
function listSkyhooksByCorporation(corporationID, identity) {
    ...
    return {
        ...
        skyhooks: cloneValue((ensureLoaded().skyhooksByCorporationID.get(numericCorporationID) || []).map(buildSkyhookGatewayState)),
    };
}
function setSkyhookActivation(skyhookID, active, identity) {
    ...
    return mutateTable((table) => {
        const skyhook = normalizeSkyhookRecord(table.skyhooks[String(access.skyhook.skyhookID)], access.skyhook);
...
function listSolarSystemsWithTheftVulnerableSkyhooks() {
    return [...ensureLoaded().vulnerableSkyhooksBySolarSystemID.keys()].sort((left, right) => left - right);
}
function listTheftVulnerableSkyhooksInSolarSystem(solarSystemID) {
    return cloneValue(ensureLoaded().vulnerableSkyhooksBySolarSystemID.get(normalizePositiveInteger(solarSystemID, 0)) || []);
}
function modifySkyhookReagents(skyhookID, reagentStates, identity) {
    ...
    return mutateTable((table) => {
        ...
        for (const reagentState of reagentStates || []) {
            ...
            const existingIndex = skyhook.reagentSimulations.findIndex((entry) => entry.reagentTypeID === reagentTypeID);
            const updatedEntry = {
                reagentTypeID,
                securedStock: Math.max(0, normalizeInteger(reagentState.securedStock, 0)),
                unsecuredStock: Math.max(0, normalizeInteger(reagentState.unsecuredStock, 0)),
                lastCycleMs: normalizeTimestampMs(reagentState.timestampMs, Date.now()),
            };
            if (existingIndex >= 0) {
        ...
        skyhook.reagentSimulations.sort((left, right) => left.reagentTypeID - right.reagentTypeID);
        skyhook.lastUpdatedMs = Date.now();
        table.skyhooks[String(skyhook.skyhookID)] = skyhook;
        return {
            ok: true,
...
function isOnLocalFuelAccessGroup(session) {
    const solarSystemID = normalizePositiveInteger(session && (session.solarsystemid2 || session.solarsystemid), 0);
    ...
    const corporationID = normalizePositiveInteger(session && (session.corporationID || session.corpid), 0);
    const allianceID = normalizePositiveInteger(session && (session.allianceID || session.allianceid), 0);
    return (corporationID === normalizePositiveInteger(system.corporationID, 0) ||
        allianceID === normalizePositiveInteger(system.allianceID, 0));
}
function acquireSkyhooks(skyhookIDs, fuelAccessGroupID, session) {
    const systemSolarSystemID = normalizePositiveInteger(session && (session.solarsystemid2 || session.solarsystemid), 0);
    const corporationID = normalizePositiveInteger(session && (session.corporationID || session.corpid), 0);
    const allianceID = normalizePositiveInteger(session && (session.allianceID || session.allianceid), 0);
...
//# sourceMappingURL=sovModernState.js.map