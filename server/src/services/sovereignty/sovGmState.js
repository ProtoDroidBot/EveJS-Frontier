...
const { getAllianceRecord, getCorporationRecord, } = require(path.join(__dirname, "../corporation/corporationState"));
const { STRUCTURE_SERVICE_STATE, STRUCTURE_STATE_NAME_BY_ID, } = require(path.join(__dirname, "../structure/structureConstants"));
const { FILETIME_EPOCH_OFFSET, FILETIME_TICKS_PER_MILLISECOND, TYPE_INFRASTRUCTURE_HUB, TYPE_TERRITORIAL_CLAIM_UNIT, } = require(path.join(__dirname, "./sovConstants"));
const { buildAnchorLayout, } = require(path.join(__dirname, "./sovAnchorLayout"));
const { cancelAllianceCapitalTransition, clearStructureCampaignState, clearStructureVulnerabilityState, getAllianceCapitalInfo, getAlliancePrimeInfo, getOperationalIndexLevel, getStrategicIndexLevel, getSystemState, listSovStructuresForSystem, setAllianceCapitalSystem, setAlliancePrimeHour, setStructureCampaignScores, setStructureCampaignState, setStructureVulnerabilityState, upsertAllianceState, upsertSystemState, } = require(path.join(__dirname, "./sovState"));
...
function resolveAllianceID(session, token, fallback = null) {
    if (token === undefined || token === null || token === "") {
        return normalizePositiveInteger(session && (session.allianceID || session.allianceid), fallback);
    }
    return normalizePositiveInteger(token, fallback);
}
function resolveCorporationID(session, token, fallback = null) {
    if (token === undefined || token === null || token === "") {
        return normalizePositiveInteger(session && (session.corporationID || session.corpid), fallback);
    }
    return normalizePositiveInteger(token, fallback);
}
function normalizeStructureKind(token) {
    const normalized = String(token || "").trim().toLowerCase();
    if (normalized === STRUCTURE_KIND.TCU ||
        normalized === "claim" ||
        normalized === "tcu") {
        return STRUCTURE_KIND.TCU;
    }
    if (normalized === STRUCTURE_KIND.IHUB ||
        normalized === "hub" ||
        normalized === "ihub" ||
...
function nextSyntheticSovItemID() {
    const offset = Number(currentFileTime() % 1000000000n);
    const itemID = SYNTHETIC_SOV_ITEM_ID_BASE +
        (offset * 10) +
...
function unixMsToFiletimeString(unixMs) {
    ...
    return (FILETIME_EPOCH_OFFSET +
        BigInt(numericUnixMs) * FILETIME_TICKS_PER_MILLISECOND).toString();
}
function shiftFiletimeBackward(rawValue, seconds) {
    ...
    try {
        const shiftedValue = BigInt(String(rawValue)) - (BigInt(numericSeconds) * FILETIME_TICKS_PER_SECOND);
...
function anchorSovereigntyStructures(solarSystemID, kind, allianceID, corporationID, options = {}) {
    const currentSystem = getRawSystemState(solarSystemID) || {};
    ...
    const positionsByKind = options && typeof options === "object" && options.positionsByKind
        ? options.positionsByKind
        : buildAnchorLayout(solarSystemID).positionsByKind;
    const namesByKind = options && typeof options === "object" && options.namesByKind
        ? options.namesByKind
        : null;
    const itemIDsByKind = options && typeof options === "object" && options.itemIDsByKind
        ? options.itemIDsByKind
        : null;
    let ensureStructureError = null;
    function getPositionForKind(targetKind) {
        return cloneSpacePoint(positionsByKind && positionsByKind[targetKind], null);
    }
    function ensureStructure(targetKind) {
        ...
        const nextName = namesByKind && Object.prototype.hasOwnProperty.call(namesByKind, targetKind)
            ? String(namesByKind[targetKind] || "").trim()
            : "";
        if (existing) {
            if (options.failIfKindExists === true) {
                ensureStructureError = "STRUCTURE_ALREADY_EXISTS";
                return null;
            }
        ...
        const hasRequestedItemID = Boolean(itemIDsByKind &&
            Object.prototype.hasOwnProperty.call(itemIDsByKind, targetKind));
        const rawRequestedItemID = hasRequestedItemID
            ? Number(itemIDsByKind[targetKind])
            : null;
        if (hasRequestedItemID &&
            (!Number.isSafeInteger(rawRequestedItemID) || rawRequestedItemID <= 0)) {
            ensureStructureError = "INVALID_STRUCTURE_ID";
            return null;
        }
        const requestedItemID = hasRequestedItemID ? rawRequestedItemID : null;
        if (requestedItemID &&
            currentStructures.some((structure) => (Number(structure && structure.itemID) === requestedItemID))) {
            ensureStructureError = "STRUCTURE_ID_COLLISION";
            return null;
        }
        const itemID = requestedItemID || nextSyntheticSovItemID();
        const nextStructure = {
            itemID,
    ...
    const nextSystem = upsertSystemState(solarSystemID, patch, {
        suppressStructureSync: options.suppressStructureSync === true,
...
function setVulnerabilityWindowForKind(solarSystemID, kind, durationSeconds) {
    ...
    const duration = Math.max(1, normalizeInteger(durationSeconds, DEFAULT_VULNERABILITY_DURATION_SECONDS));
    const startTime = currentFileTime().toString();
    const endTime = (currentFileTime() + (BigInt(duration) * FILETIME_TICKS_PER_SECOND)).toString();
    for (const structure of matchingStructures) {
        setStructureVulnerabilityState(solarSystemID, structure.itemID, {
...
function startCampaignForKind(solarSystemID, kind, eventType = DEFAULT_CAMPAIGN_EVENT_TYPE, offsetSeconds = DEFAULT_CAMPAIGN_OFFSET_SECONDS) {
    const matchingStructures = getMatchingStructures(solarSystemID, kind);
    if (matchingStructures.length === 0) {
        return {
            success: false,
    ...
    const campaignStartTime = (currentFileTime() +
        (BigInt(Math.max(0, normalizeInteger(offsetSeconds, DEFAULT_CAMPAIGN_OFFSET_SECONDS))) *
            FILETIME_TICKS_PER_SECOND)).toString();
    for (const structure of matchingStructures) {
        setStructureCampaignState(solarSystemID, structure.itemID, {
            campaignEventType,
            campaignStartTime,
            campaignScoresByTeam: {
                1: 0,
...
function setCampaignScoresForKind(solarSystemID, kind, scoresByTeam) {
    ...
    for (const structure of matchingStructures) {
        setStructureCampaignScores(solarSystemID, structure.itemID, normalizedScoresByTeam);
    }
    return {
        success: true,
        data: {
...
function fastForwardSovereigntyTimers(solarSystemID, seconds) {
    ...
    const shiftedStructures = (Array.isArray(currentSystem.structures) ? currentSystem.structures : []).map((structure) => {
        const nextStructure = {
        ...
        if (nextStructure.campaignStartTime !== structure.campaignStartTime ||
    ...
    if (allianceID) {
        ...
        const nextPrimeValidAfter = shiftFiletimeBackward(primeInfo && primeInfo.newPrimeHourValidAfter, numericSeconds);
        const nextCapitalValidAfter = shiftFiletimeBackward(capitalInfo && capitalInfo.newCapitalSystemValidAfter, numericSeconds);
        if (nextPrimeValidAfter !== String(primeInfo && primeInfo.newPrimeHourValidAfter || "0")) {
            shiftedAllianceTimerCount += 1;
        }
        if (nextCapitalValidAfter !==
...
function captureSystemForOwner(solarSystemID, allianceID, corporationID) {
    const anchorResult = anchorSovereigntyStructures(solarSystemID, STRUCTURE_KIND.BOTH, allianceID, corporationID);
    if (!anchorResult.success) {
        return anchorResult;
    }
    const currentSystem = getRawSystemState(solarSystemID) || {};
    const claimTime = currentFileTime().toString();
    const structures = (Array.isArray(currentSystem.structures) ? currentSystem.structures : []).map((structure) => ({
        ...cloneValue(structure),
        ownerID: corporationID,
        corporationID,
        allianceID,
...
function loseSystemSovereignty(solarSystemID, nextAllianceID = null, nextCorporationID = null) {
    ...
    if (normalizedNextAllianceID || normalizedNextCorporationID) {
        ...
        return captureSystemForOwner(solarSystemID, normalizedNextAllianceID, normalizedNextCorporationID);
    }
    const nextSystem = upsertSystemState(solarSystemID, {
        allianceID: null,
        corporationID: null,
...
function setSystemDevelopmentIndexValue(solarSystemID, indexKind, value) {
...
function describeHubFuelSnapshot(hubFuel) {
    ...
    const fuelLabels = hubFuel.fuels
        ...
        const hoursLeft = burnedPerHour > 0
...
function describeSovereigntyFlexStructures(solarSystemID) {
    const { getSovereigntyFlexDefinitions, listSovereigntyFlexStructuresForSystem, } = require(path.join(__dirname, "./sovFlexStructures"));
    const flexDefinitionsByTypeID = new Map(getSovereigntyFlexDefinitions().map((definition) => [definition.typeID, definition]));
    ...
    return flexStructures
        ...
        .map((structure) => {
        const definition = flexDefinitionsByTypeID.get(normalizePositiveInteger(structure && structure.typeID, 0)) || null;
        const serviceState = definition &&
            Number(structure &&
                structure.serviceStates &&
                structure.serviceStates[String(definition.serviceID)]) === STRUCTURE_SERVICE_STATE.ONLINE
            ? "online"
            : "offline";
        return [
...
function buildSovereigntyStatusReport(solarSystemID) {
    ...
    const lines = [];
    lines.push(`${solarSystem.solarSystemName || solarSystem.itemName || `System ${solarSystemID}`}(${solarSystemID}) ${formatOwnerLabel("alliance", system.allianceID)} ${formatOwnerLabel("corporation", system.corporationID)} claim=${system.claimStructureID || "none"} ihub=${system.infrastructureHubID || "none"}`);
    if (system.allianceID) {
        ...
        lines.push(`prime current=${primeInfo.currentPrimeHour} pending=${primeInfo.newPrimeHour} validAfter=${formatFiletime(primeInfo.newPrimeHourValidAfter)}`);
        lines.push(`capital current=${capitalInfo.currentCapitalSystem || "none"} pending=${capitalInfo.newCapitalSystem || "none"} validAfter=${formatFiletime(capitalInfo.newCapitalSystemValidAfter)}`);
    }
    ...
    lines.push(`indices military=${militaryPoints}(L${getOperationalIndexLevel(militaryPoints)}) industrial=${industrialPoints}(L${getOperationalIndexLevel(industrialPoints)}) claimedForDays=${claimedForDays}(strategic=${getStrategicIndexLevel(claimedForDays)})`);
    lines.push(`claimTime=${formatFiletime(system.claimTime)}`);
    if (system.infrastructureHubID) {
        const { getHubFuel, getHubUpgrades, } = require(path.join(__dirname, "./sovModernState"));
        lines.push(describeHubUpgradeSnapshot(getHubUpgrades(system.infrastructureHubID)));
        lines.push(describeHubFuelSnapshot(getHubFuel(system.infrastructureHubID)));
    }
...
//# sourceMappingURL=sovGmState.js.map