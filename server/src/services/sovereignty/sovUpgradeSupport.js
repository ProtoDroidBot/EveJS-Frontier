...
function getUpgradeDefinition(installationTypeID) {
    ...
    return snapshot.upgradeDefinitionsByTypeID.get(normalizePositiveInteger(installationTypeID, 0)) || null;
}
...
function canSolarSystemSupportUpgrade(solarSystemID, installationTypeID) {
    ...
    return (capacity.power >= Math.max(0, Number(definition.powerRequired) || 0) &&
...
function canSolarSystemSupportUpgrades(solarSystemID, installationTypeIDs) {
    const requiredTypeIDs = Array.isArray(installationTypeIDs)
        ? [...new Set(installationTypeIDs
                .map((typeID) => normalizePositiveInteger(typeID, null))
                .filter(Boolean))]
        : [];
    return requiredTypeIDs.every((typeID) => (canSolarSystemSupportUpgrade(solarSystemID, typeID)));
}
function canSolarSystemSupportSovFlexShowcase(solarSystemID) {
    return canSolarSystemSupportUpgrades(solarSystemID, SHOWCASE_SOV_FLEX_REQUIRED_UPGRADE_TYPE_IDS);
}
module.exports = {
...
//# sourceMappingURL=sovUpgradeSupport.js.map