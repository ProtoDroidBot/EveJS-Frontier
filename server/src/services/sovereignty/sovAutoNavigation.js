...
const { canSolarSystemSupportUpgrades, canSolarSystemSupportUpgrade, } = require(path.join(__dirname, "./sovUpgradeSupport"));
const { buildAnchorLayout, cloneSpacePoint, normalizePositiveInteger, normalizeSpacePoint, } = require(path.join(__dirname, "./sovAnchorLayout"));
const { isSovereigntyClaimableSolarSystem, } = require(path.join(__dirname, "./sovSystemRules"));
const { isSovereigntyRelatedStructure, } = require(path.join(__dirname, "./sovSpaceInterop"));
const { getSystemState, listAllAllianceSystems, upsertSystemState, } = require(path.join(__dirname, "./sovState"));
...
function getSessionCharacterID(session) {
    return normalizePositiveInteger(session && (session.characterID || session.charid || session.userid), null);
}
...
function getSessionShipEntity(session) {
    const shipID = normalizePositiveInteger(session && session._space && session._space.shipID, null);
    if (!shipID) {
        return null;
    }
...
function buildSessionAnchorLayout(session, solarSystemID) {
    ...
    const origin = currentSolarSystemID === normalizePositiveInteger(solarSystemID, null)
...
function ensureSessionInSolarSystem(session, solarSystemID) {
    ...
    // Server-driven repositioning, not a jump the pilot made.
    ...
    return {
        ...
        data: {
            ...
            moved: true,
            solarSystemID: targetSolarSystemID,
            solarSystem: (jumpResult.data && jumpResult.data.solarSystem) ||
...
function getStructurePositionsForSystem(solarSystemID) {
    ...
    for (const structure of structures) {
        ...
        else if (normalizePositiveInteger(structure && structure.typeID, null) === TYPE_INFRASTRUCTURE_HUB) {
            positionsByKind.ihub = position;
        }
    }
    return {
        positionsByKind,
        primaryPoint: cloneSpacePoint(positionsByKind.tcu || positionsByKind.ihub, null),
    };
}
...
function teleportSessionToPrimarySovAnchor(session, solarSystemID) {
    ...
    const targetPoint = cloneSpacePoint(positionsResult.data && positionsResult.data.primaryPoint, null);
    if (!targetPoint) {
        return {
            success: true,
            data: {
    ...
    const direction = cloneSpacePoint(shipEntity && shipEntity.direction, { x: 1, y: 0, z: 0 }) || { x: 1, y: 0, z: 0 };
...
function findUnclaimedSolarSystem(session, options = {}) {
    const preferredCurrentSystemID = normalizePositiveInteger(options.preferredCurrentSystemID || getSessionCurrentSolarSystemID(session), null);
    const requiredUpgradeTypeID = normalizePositiveInteger(options.requiredUpgradeTypeID, null);
    const requiredUpgradeTypeIDs = Array.isArray(options.requiredUpgradeTypeIDs)
        ? [...new Set(options.requiredUpgradeTypeIDs
    ...
    const claimedSystems = new Set((listAllAllianceSystems() || [])
        .map((record) => normalizePositiveInteger(record && record.solarSystemID, null))
        .filter(Boolean));
    const fallbackCurrentSystemID = preferredCurrentSystemID && !claimedSystems.has(preferredCurrentSystemID)
...
//# sourceMappingURL=sovAutoNavigation.js.map