...
function normalizeScoresByTeam(value) {
  ...
  for (const [teamID, score] of Object.entries<any>(value)) {
...
function normalizeStructureRecord(value: Record<string, any> = {}, solarSystemID, fallbackAllianceID, fallbackCorporationID) {
...
function normalizeDevIndices(value: Record<string, any> = {}) {
...
function normalizeCurrentDataRecord(value: Record<string, any> = {}, solarSystemID, system = null) {
...
function normalizeRecentActivityRecord(value: Record<string, any> = {}, solarSystemID, system = null) {
  const locationInfo = getStaticSystemLocationInfo(
    solarSystemID || (system && system.solarSystemID),
  );
  const ownerSource = Object.prototype.hasOwnProperty.call(value, "ownerID")
...
function normalizeAllianceState(value: Record<string, any> = {}, allianceID) {
...
function normalizeSystemState(value: Record<string, any> = {}, solarSystemID) {
...
function normalizeTable(table: Record<string, any> = {}) {
  ...
  for (const [allianceID, record] of Object.entries<any>(table.alliances || {})) {
  ...
  for (const [solarSystemID, record] of Object.entries<any>(table.systems || {})) {
...
function applyTableTimedTransitions(table) {
  let changed = false;
  for (const allianceState of Object.values<any>((table && table.alliances) || {})) {
...
function buildIndexes() {
  ...
  const allAllianceSystems: any[] = [];
  const allDevelopmentIndices: any[] = [];
  const currentSovDataByLocationID = new Map();
  const recentSovActivity: any[] = [];
  ...
  const staticSystemIDs = new Set<any>();
  ...
  for (const allianceState of Object.values<any>(tableCache.alliances || {})) {
  ...
  for (const system of Object.values<any>(tableCache.systems || {}).sort(
    ...
    for (const structure of structureSnapshots) {
      ...
      for (const [teamID, score] of Object.entries<any>(structure.campaignScoresByTeam || {})) {
  ...
  for (const system of Object.values<any>(tableCache.systems || {})) {
...
function upsertAllianceState(allianceID, patch: Record<string, any> = {}) {
  ensureLoaded();
  const numericAllianceID = normalizePositiveInteger(allianceID, null);
  if (!numericAllianceID) {
    return null;
...
function collectSystemAllianceIDs(currentSystem, nextSystem, structureSnapshots: any[] = []) {
  const allianceIDs = new Set<any>();
...
function publishSystemStateEffects(currentValue, nextValue, options: Record<string, any> = {}) {
  const numericSolarSystemID = normalizePositiveInteger(
    (nextValue && nextValue.solarSystemID) ||
      (currentValue && currentValue.solarSystemID),
    null,
  );
  if (!numericSolarSystemID) {
    return { success: false as const, errorMsg: "SOLAR_SYSTEM_REQUIRED" };
  }
  ...
  if (
    !normalizeBoolean(options.suppressStructureSync, false) &&
    structuresChanged
  ) {
  ...
  return {
    ...
    errorMsg:
      structureSyncResult && structureSyncResult.success === false
...
function replaceSystemStateSnapshot(solarSystemID, snapshot, options: Record<string, any> = {}) {
  ensureLoaded();
  const numericSolarSystemID = normalizePositiveInteger(solarSystemID, null);
  if (!numericSolarSystemID) {
    return null;
...
function upsertSystemState(solarSystemID, patch: Record<string, any> = {}, options: Record<string, any> = {}) {
  ensureLoaded();
  const numericSolarSystemID = normalizePositiveInteger(solarSystemID, null);
  if (!numericSolarSystemID) {
    return null;
...
function clearStructureCampaignState(solarSystemID, structureID, options: Record<string, any> = {}) {
  return mutateStructureInSystem(
    solarSystemID,
    structureID,
    (structure) => ({
...
function setStructureCampaignScores(
  solarSystemID,
  structureID,
  campaignScoresByTeam: Record<string, any> = {},
  options: Record<string, any> = {},
) {
  const numericStructureID = normalizePositiveInteger(structureID, null);
  return mutateStructureInSystem(
...
function setStructureVulnerabilityState(
  solarSystemID,
  structureID,
  vulnerabilityState: Record<string, any> = {},
  options: Record<string, any> = {},
) {
  return mutateStructureInSystem(
    solarSystemID,
...
function clearStructureVulnerabilityState(solarSystemID, structureID, options: Record<string, any> = {}) {
  return mutateStructureInSystem(
    solarSystemID,
    structureID,
    (structure) => ({
...
function wipeAllSovereigntyState(options: Record<string, any> = {}) {
