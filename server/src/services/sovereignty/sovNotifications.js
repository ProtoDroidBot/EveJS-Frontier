...
const { buildDict, buildKeyVal, buildList, buildPythonSet, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { buildSovClaimInfoPayload, buildSovStructuresPayload, } = require(path.join(__dirname, "./sovPayloads"));
const { STRUCTURES_UPDATED, } = require(path.join(__dirname, "./sovConstants"));
...
function getTargetSessions({ solarSystemID = null, allianceIDs = [] } = {}) {
    ...
    for (const session of sessionRegistry.getSessions()) {
        const characterID = normalizePositiveInteger(session && (session.characterID || session.charid), null);
        if (characterID && seenCharacterIDs.has(characterID)) {
            continue;
        }
        const sessionSolarSystemID = normalizePositiveInteger(session && (session.solarsystemid2 || session.solarsystemid), null);
        const sessionAllianceID = normalizePositiveInteger(session && (session.allianceID || session.allianceid), null);
        const matchesSolarSystem = numericSolarSystemID && sessionSolarSystemID === numericSolarSystemID;
        const matchesAlliance = sessionAllianceID && normalizedAllianceIDs.has(sessionAllianceID);
...
function buildStructureChangesPayload(changesByStructureID = null) {
    ...
    const entries = Object.entries(changesByStructureID)
        ...
        const normalizedChanges = [...new Set((Array.isArray(changeSet) ? changeSet : [])
                .map((entry) => normalizeInteger(entry, -1))
                .filter((entry) => entry >= 0))].sort((left, right) => left - right);
        return [
            numericSourceItemID,
            buildPythonSet(normalizedChanges.length > 0
                ? normalizedChanges
                : [STRUCTURES_UPDATED]),
        ];
    })
...
function buildSovHubHackPayload(upgrades = []) {
    return buildList((Array.isArray(upgrades) ? upgrades : []).map((upgrade) => buildKeyVal([
        ["typeID", normalizePositiveInteger(upgrade && upgrade.typeID, null)],
        ["powerState", normalizeInteger(upgrade && upgrade.powerState, 0)],
    ])));
}
function broadcastSolarSystemSovStructuresUpdated(solarSystemID, structures, changesByStructureID = null, options = {}) {
    ...
    sendNotificationToSessions(sessions, "OnSolarSystemSovStructuresUpdated", "solarsystemid2", payload);
}
function broadcastSolarSystemDevIndexChanged(solarSystemID, allianceIDs = []) {
    const sessions = getTargetSessions({
        solarSystemID,
        allianceIDs,
    });
    ...
    sendNotificationToSessions(sessions, "OnSolarSystemDevIndexChanged", "solarsystemid2", [normalizePositiveInteger(solarSystemID, 0) || 0]);
}
function broadcastSovereigntyChanged(solarSystemID, claimInfo = null, allianceIDs = []) {
    const sessions = getTargetSessions({
        solarSystemID,
        allianceIDs,
    });
    ...
    sendNotificationToSessions(sessions, "OnSovereigntyChanged", "solarsystemid2", [
        normalizePositiveInteger(solarSystemID, 0) || 0,
        buildSovClaimInfoPayload(claimInfo),
    ]);
}
function broadcastSovereigntyAudioEvent(solarSystemID, eventID, textParams = {}, allianceIDs = []) {
    const sessions = getTargetSessions({
        solarSystemID,
        allianceIDs,
    });
    ...
    sendNotificationToSessions(sessions, "OnSovereigntyAudioEvent", "solarsystemid2", [
        normalizeInteger(eventID, 0),
        textParams && typeof textParams === "object" ? textParams : {},
    ]);
}
function broadcastSovHubHacked(solarSystemID, sovHubID, upgrades = [], allianceIDs = []) {
    const sessions = getTargetSessions({
        solarSystemID,
        allianceIDs,
    });
    ...
    sendNotificationToSessions(sessions, "OnSovHubHacked", "solarsystemid2", [
...
function broadcastCynoJammerChanged(solarSystemID, onlineSimTime) {
    ...
    sendNotificationToSessions(sessions, "OnCynoJammerChanged", "solarsystemid2", [
...
//# sourceMappingURL=sovNotifications.js.map