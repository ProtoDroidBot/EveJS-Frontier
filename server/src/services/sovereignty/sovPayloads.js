...
const { buildDict, buildFiletimeLong, buildKeyVal, buildList, buildObjectEx1, buildRowset, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
function buildAllianceSystemListPayload(records = []) {
    return buildList(records.map((record) => buildKeyVal([
...
function buildDevelopmentIndicesForSystemPayload(indicesByAttribute = {}) {
    return buildDict(Object.entries(indicesByAttribute).map(([attributeID, value]) => [
        Number(attributeID),
        buildKeyVal([
...
function buildCampaignStatePayload(structure = {}) {
    ...
    return [
        ...
        buildDict(Object.entries(structure.campaignScoresByTeam || {}).map(([teamID, score]) => [
            Number(teamID),
            Number(score || 0),
        ])),
    ];
...
function buildSovStructuresPayload(structures = []) {
    return buildList(structures.map((structure) => buildKeyVal([
...
function buildAllianceSovereigntyRowsPayload(rows = {}) {
    const buildRowPayload = (row = {}) => buildKeyVal(Object.entries(row).map(([key, value]) => [
        key,
...
function buildCurrentSovDataPayload(rows = []) {
    return buildRowset(CURRENT_SOV_DATA_HEADER, rows.map((row) => [
        row.locationID || null,
...
function buildRecentSovActivityPayload(rows = []) {
    return buildRowset(RECENT_SOV_ACTIVITY_HEADER, rows.map((row) => [
        row.solarSystemID || null,
        row.ownerID || null,
        row.oldOwnerID || null,
        row.stationID || null,
...
//# sourceMappingURL=sovPayloads.js.map