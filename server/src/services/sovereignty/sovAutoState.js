...
const { STRUCTURE_SERVICE_ID, STRUCTURE_SERVICE_STATE, STRUCTURE_STATE_NAME_BY_ID, } = require(path.join(__dirname, "../structure/structureConstants"));
const { getDevelopmentIndicesForSystem, getSystemState, listAllDevelopmentIndices, wipeAllSovereigntyState, } = require(path.join(__dirname, "./sovState"));
const { buildSessionAnchorLayout, ensureSessionInSolarSystem, findUnclaimedSolarSystem, getStructurePositionsForSystem, isClaimedSystemState, teleportSessionToPrimarySovAnchor, } = require(path.join(__dirname, "./sovAutoNavigation"));
const { anchorSovereigntyStructures, buildSovereigntyStatusReport, captureSystemForOwner, clearSystemSovereignty, ensureValidSolarSystem, fastForwardSovereigntyTimers, loseSystemSovereignty, setCampaignScoresForKind, setSystemDevelopmentIndexValue, setVulnerabilityWindowForKind, startCampaignForKind, } = require(path.join(__dirname, "./sovGmState"));
const { getHubIDForSolarSystem, getHubUpgrades, seedHubFuelForInstalledUpgrades, setHubUpgradeInstallations, } = require(path.join(__dirname, "./sovModernState"));
const { DEFAULT_SOV_FLEX_FUEL_HOURS, deploySovereigntyFlexStructure, getSovereigntyFlexDefinitions, seedSovereigntyFlexFuel, } = require(path.join(__dirname, "./sovFlexStructures"));
const { isSovereigntyAuxiliaryStructure, } = require(path.join(__dirname, "./sovSpaceInterop"));
const { MAX_OPERATIONAL_INDEX_POINTS, MAX_STRATEGIC_CLAIM_DAYS, TYPE_CYNO_SUPPRESSION_UPGRADE, TYPE_TENEBREX_CYNO_JAMMER, canSolarSystemSupportUpgrade, getUpgradeDefinition, getLocalSovereigntyResourceCapacity, } = require(path.join(__dirname, "./sovUpgradeSupport"));
...
const AUTO_SHOWCASE_FLEX_DEFINITIONS = Object.freeze(getSovereigntyFlexDefinitions("all").map((definition) => Object.freeze({ ...definition })));
let nextJobID = 1;
...
function cloneSessionForAutomation(session) {
    return {
        ...
        characterID: normalizePositiveInteger(session && (session.characterID || session.charid || session.userid), 0),
        charid: normalizePositiveInteger(session && (session.charid || session.characterID || session.userid), 0),
        userid: normalizePositiveInteger(session && (session.userid || session.userID || session.characterID), 0),
        corporationID: normalizePositiveInteger(session && (session.corporationID || session.corpid), 0),
        corpid: normalizePositiveInteger(session && (session.corpid || session.corporationID), 0),
        allianceID: normalizePositiveInteger(session && (session.allianceID || session.allianceid), 0),
        allianceid: normalizePositiveInteger(session && (session.allianceid || session.allianceID), 0),
...
function getCurrentSystemOwner(job) {
    ...
    return {
        allianceID: normalizePositiveInteger(system.allianceID, null) ||
            normalizePositiveInteger(job.targetAllianceID, null) ||
            normalizePositiveInteger(job.nextAllianceID, null),
        corporationID: normalizePositiveInteger(system.corporationID, null) ||
...
function getSolarSystemLabel(solarSystemID) {
    const solarSystem = worldData.getSolarSystemByID(normalizePositiveInteger(solarSystemID, 0));
...
function getFlexShowcaseDefinitionsForJob(job, options = {}) {
    ...
    const supportedDefinitions = Array.isArray(job && job.showcaseDefinitions) && job.showcaseDefinitions.length > 0
        ? job.showcaseDefinitions
        : AUTO_SHOWCASE_FLEX_DEFINITIONS.filter((definition) => (canSolarSystemSupportUpgrade(job && job.solarSystemID, definition.requiredUpgradeTypeID)));
...
function syncStructureRuntime(structureOrSystemID) {
    const solarSystemID = typeof structureOrSystemID === "object" && structureOrSystemID !== null
...
function sendJobFeedback(job, message) {
    ...
    job.chatHub.sendSystemMessage(job.liveSession, `[SOVAUTO ${job.jobID}] ${normalizedMessage}`);
}
function stopJobInternal(job, reason, status = "stopped") {
    ...
    sovLog.logAutomationEvent(job, `${status.toUpperCase()} ${reason}`, status === "failed" ? "ERR" : "INF");
    if (status === "completed") {
...
function applyAutomationStep(job, actionLabel, fn, options = {}) {
    const announceBefore = typeof options.before === "function"
        ? options.before()
        : options.before;
    if (announceBefore) {
    ...
    try {
        ...
        sovLog.logAutomationEvent(job, `${actionLabel} threw ${error.stack || error.message}`, "ERR");
    ...
    sovLog.logAutomationEvent(job, `${actionLabel} ok ${JSON.stringify(buildSovereigntyStatusReport(job.solarSystemID))}`);
    const announceAfter = typeof options.after === "function"
...
function anchorClaimForJob(job, kind) {
    ...
    const anchorResult = anchorSovereigntyStructures(job.solarSystemID, kind, job.targetAllianceID, job.targetCorporationID, {
...
function maxAndVerifyIndicesForJob(job) {
    ...
    const allDevelopmentIndices = Array.isArray(listAllDevelopmentIndices())
        ...
        : [];
    const summaryRow = allDevelopmentIndices.find((row) => normalizePositiveInteger(row && row.solarSystemID, null) === job.solarSystemID) || null;
    const militaryPoints = Number(perSystemIndices[1583] && perSystemIndices[1583].points) || 0;
    const industrialPoints = Number(perSystemIndices[1584] && perSystemIndices[1584].points) || 0;
    const claimedForSeconds = Number(perSystemIndices[1615] && perSystemIndices[1615].points) || 0;
    if (!summaryRow ||
        militaryPoints !== MAX_OPERATIONAL_INDEX_POINTS ||
        industrialPoints !== MAX_OPERATIONAL_INDEX_POINTS ||
...
function enableCynoSuppressionForJob(job) {
    ...
    const upgradeResult = setHubUpgradeInstallations(hubID, [{ typeID: TYPE_CYNO_SUPPRESSION_UPGRADE, online: true }], buildOwnershipIdentity(job));
    if (!upgradeResult.ok) {
        return {
            success: false,
    ...
    const jammerUpgrade = upgrades && Array.isArray(upgrades.upgrades)
        ? upgrades.upgrades.find((entry) => Number(entry && entry.typeID) === TYPE_CYNO_SUPPRESSION_UPGRADE) || null
        : null;
    ...
    const fuelResult = seedHubFuelForInstalledUpgrades(hubID, DEFAULT_SOV_FLEX_FUEL_HOURS, buildOwnershipIdentity(job));
    return {
        success: true,
        data: {
            hubID,
...
function buildJammerSeedPosition(job) {
    ...
    const basePoint = clonePoint(positions.positionsByKind.ihub || positions.positionsByKind.tcu, null);
    if (basePoint) {
...
function ensureJammerSeeded(job) {
    ...
    const owner = getCurrentSystemOwner(job);
    const seedResult = structureState.seedStructureForSession(job.liveSession || job.session, TYPE_TENEBREX_CYNO_JAMMER, {
        solarSystemID: job.solarSystemID,
...
function installJammerCore(job) {
    const result = structureState.setStructureQuantumCoreInstalled(job.jammerStructureID, true);
    if (!result.success) {
        return result;
    }
    syncStructureRuntime(result.data);
...
function onlineJammerService(job) {
    const serviceResult = structureState.setStructureServiceState(job.jammerStructureID, STRUCTURE_SERVICE_ID.CYNO_JAMMER, STRUCTURE_SERVICE_STATE.ONLINE, { consumeFlexOnlineFuel: false });
    if (!serviceResult.success) {
        return serviceResult;
    }
    syncStructureRuntime(serviceResult.data);
    const onlineResult = advanceStructureTimers(job.jammerStructureID, AUTO_STRUCTURE_FAST_FORWARD_SHORT_SECONDS);
    if (!onlineResult.success) {
        return onlineResult;
    }
    const fuelResult = seedSovereigntyFlexFuel(job.jammerStructureID, DEFAULT_SOV_FLEX_FUEL_HOURS);
...
function deployFlexStructureForJob(job, definition) {
    const deployResult = deploySovereigntyFlexStructure(job.liveSession || job.session, definition.kind, {
        solarSystemID: job.solarSystemID,
    ...
    const structure = deployResult.data && deployResult.data.structure
...
function focusSessionOnFlexKind(job, kind) {
    const structureID = normalizePositiveInteger(job &&
        job.flexStructureIDs &&
        job.flexStructureIDs[String(kind || "").trim().toLowerCase()], null);
    if (!structureID) {
        return {
            success: false,
...
function runPostCaptureSequence(job, baseStep, completionReason) {
    ...
    const nonJammerDefinitions = showcaseDefinitions.filter((definition) => definition.kind !== "tenebrex");
    const supportsTenebrex = showcaseDefinitions.some((definition) => definition.kind === "tenebrex");
    const postCaptureSteps = [
        ...
        },
        {
            ...
            after: (data) => (data && Array.isArray(data.removedStructureIDs) && data.removedStructureIDs.length > 0
    ...
    const result = applyAutomationStep(job, step.actionLabel, step.run, {
        before: step.before,
        after: step.after,
    });
    if (!result.success) {
    ...
    if (relativeStep === postCaptureSteps.length - 1) {
        return stopJobInternal(job, `${completionReason} ${buildFlexShowcaseSummary(job)}`, "completed");
    }
    return result;
}
function executeClaimStep(job) {
    if (job.step === 0) {
        const result = applyAutomationStep(job, `/solar ${job.solarSystemID}`, () => travelSessionForJob(job), {
    ...
    if (job.step === 1) {
        ...
        if (!claimCheck.success) {
            return stopJobInternal(job, "claim automation requires an unclaimed system; use /sovauto takeover for hostile systems", "failed");
        }
        const result = applyAutomationStep(job, `/sov anchor tcu ${job.solarSystemID} ${job.targetAllianceID} ${job.targetCorporationID}`, () => anchorClaimForJob(job, "tcu"), {
    ...
    if (job.step === 2) {
        const result = applyAutomationStep(job, `/tr me sovanchor ${job.solarSystemID}`, () => focusSessionForJob(job), {
    ...
    if (job.step === 3) {
        const result = applyAutomationStep(job, `/sov anchor ihub ${job.solarSystemID} ${job.targetAllianceID} ${job.targetCorporationID}`, () => anchorClaimForJob(job, "ihub"), {
    ...
    return runPostCaptureSequence(job, 4, `Claim automation completed in ${getSolarSystemLabel(job.solarSystemID)}.`);
}
function executeTakeoverStep(job) {
    const claimCheck = ensureClaimedSystem(job);
    if (!claimCheck.success) {
        return stopJobInternal(job, "takeover automation requires an already-claimed system; use /sovauto claim first", "failed");
    }
    ...
    if (job.step === 2) {
        const result = applyAutomationStep(job, `/sov vuln both ${AUTO_VULNERABILITY_SECONDS} ${job.solarSystemID}`, () => setVulnerabilityWindowForKind(job.solarSystemID, "both", AUTO_VULNERABILITY_SECONDS), {
    ...
    if (job.step === 3) {
        const result = applyAutomationStep(job, `/sov ff ${AUTO_VULNERABILITY_FAST_FORWARD_SECONDS} ${job.solarSystemID}`, () => fastForwardSovereigntyTimers(job.solarSystemID, AUTO_VULNERABILITY_FAST_FORWARD_SECONDS), {
    ...
    if (job.step === 4) {
        const result = applyAutomationStep(job, `/sov campaign ihub ${AUTO_CAMPAIGN_EVENT_TYPE} ${AUTO_CAMPAIGN_OFFSET_SECONDS} ${job.solarSystemID}`, () => startCampaignForKind(job.solarSystemID, "ihub", AUTO_CAMPAIGN_EVENT_TYPE, AUTO_CAMPAIGN_OFFSET_SECONDS), {
    ...
    if (job.step === 5) {
        const result = applyAutomationStep(job, `/sov ff ${AUTO_CAMPAIGN_FAST_FORWARD_SECONDS} ${job.solarSystemID}`, () => fastForwardSovereigntyTimers(job.solarSystemID, AUTO_CAMPAIGN_FAST_FORWARD_SECONDS), {
    ...
    if (job.step === 6) {
        const result = applyAutomationStep(job, `/sov score ihub 1=${AUTO_TAKEOVER_SCORES[1]} 2=${AUTO_TAKEOVER_SCORES[2]}`, () => setCampaignScoresForKind(job.solarSystemID, "ihub", AUTO_TAKEOVER_SCORES), {
    ...
    if (job.step === 7) {
        const result = applyAutomationStep(job, `/sov capture ${job.targetAllianceID} ${job.targetCorporationID} ${job.solarSystemID}`, () => captureSystemForOwner(job.solarSystemID, job.targetAllianceID, job.targetCorporationID), {
    ...
    return runPostCaptureSequence(job, 8, `Takeover automation completed in ${getSolarSystemLabel(job.solarSystemID)}.`);
}
function executeLossStep(job) {
    const claimCheck = ensureClaimedSystem(job);
    if (!claimCheck.success) {
        return stopJobInternal(job, "loss automation requires an already-claimed system; use /sovauto claim first", "failed");
    }
    if (job.step === 0) {
        const result = applyAutomationStep(job, `/solar ${job.solarSystemID}`, () => travelSessionForJob(job), {
    ...
    if (job.step === 6) {
        const result = applyAutomationStep(job, `/sov score ihub 1=${AUTO_LOSS_SCORES[1]} 2=${AUTO_LOSS_SCORES[2]}`, () => setCampaignScoresForKind(job.solarSystemID, "ihub", AUTO_LOSS_SCORES), {
    ...
    if (job.step === 7) {
        const result = applyAutomationStep(job, `/upwell remove sov flex showcase structures`, () => clearAutomationStructuresForSystem(job), {
    ...
    if (job.step === 8) {
        const result = applyAutomationStep(job, job.nextAllianceID && job.nextCorporationID
            ? `/sov lose ${job.nextAllianceID} ${job.nextCorporationID} ${job.solarSystemID}`
            : `/sov lose ${job.solarSystemID}`, () => (job.nextAllianceID && job.nextCorporationID
            ? loseSystemSovereignty(job.solarSystemID, job.nextAllianceID, job.nextCorporationID)
        ...
        if (result.success) {
            job.step += 1;
            return stopJobInternal(job, job.nextAllianceID && job.nextCorporationID
                ? `Loss automation completed and sovereignty transferred in ${getSolarSystemLabel(job.solarSystemID)}.`
                : `Loss automation completed and sovereignty was cleared in ${getSolarSystemLabel(job.solarSystemID)}.`, "completed");
        }
        return result;
    }
...
function createJob(mode, solarSystemID, session, options = {}) {
    ...
    const job = {
        ...
        flexStructureIDs: {},
        showcaseDefinitions: AUTO_SHOWCASE_FLEX_DEFINITIONS.filter((definition) => (canSolarSystemSupportUpgrade(normalizePositiveInteger(solarSystemID, 0), definition.requiredUpgradeTypeID))),
        step: 0,
        startedAt: Date.now(),
        lastActionAt: 0,
        lastAction: null,
    ...
    sendJobFeedback(job, `Started ${job.mode} automation in ${getSolarSystemLabel(job.solarSystemID)}. The next step runs now, then every 10 seconds.`);
    return job;
}
...
function startLoss(session, solarSystemID, nextAllianceID = null, nextCorporationID = null, options = {}) {
    ...
    if ((normalizePositiveInteger(nextAllianceID, null) && !normalizePositiveInteger(nextCorporationID, null)) ||
...
//# sourceMappingURL=sovAutoState.js.map