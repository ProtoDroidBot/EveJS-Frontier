...
const { buildKeyVal, buildList, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { listSkyhooksByCorporation, } = require(path.join(__dirname, "./sovModernState"));
...
function resolveSkyhookClientState(skyhook) {
    const explicitState = normalizePositiveInteger(skyhook && (skyhook.state || skyhook.skyhookState || skyhook.structureState), 0);
...
function sortSkyhookSummaries(left, right) {
    return (normalizePositiveInteger(left && left.solarSystemID, 0) -
...
//# sourceMappingURL=colonyResourcesMgrService.js.map