...
const { STRUCTURE_STATE, STRUCTURE_UPKEEP_STATE, } = require(path.join(__dirname, "../structure/structureConstants"));
...
function buildMirrorName(typeID, structureID) {
    ...
    return String(itemType && itemType.name
        ? itemType.name
...
function isSovereigntyMirrorStructure(structure) {
    ...
    return (Boolean(structure && structure.devFlags && structure.devFlags.sovereigntyMirror === true) ||
...
function isSovereigntyAuxiliaryStructure(structure) {
    ...
    const devFlags = structure && structure.devFlags && typeof structure.devFlags === "object"
        ? structure.devFlags
        : {};
    return (SOVEREIGNTY_FLEX_TYPE_IDS.has(typeID) ||
...
function isSovereigntyRelatedStructure(structure) {
    return (isSovereigntyMirrorStructure(structure) ||
...
function buildMirrorRecord(systemState, structure, nowMs = Date.now()) {
    ...
    const solarSystemID = normalizePositiveInteger(systemState && systemState.solarSystemID, null) ||
        normalizePositiveInteger(structure && structure.solarSystemID, null);
    const fallbackLayout = buildAnchorLayout(solarSystemID);
    const fallbackPosition = typeID === TYPE_TERRITORIAL_CLAIM_UNIT
    ...
    const ownerCorpID = normalizePositiveInteger(structure && structure.corporationID, null) ||
    ...
    return {
        ...
        allianceID,
        solarSystemID,
        position: normalizePosition(structure && structure.position, normalizePosition(existing && existing.position, fallbackPosition)),
        rotation: Array.isArray(existing && existing.rotation)
            ? [...existing.rotation]
            : [0, 0, 0],
        ...
        devFlags: {
            ...
            sovereigntyKind: typeID === TYPE_TERRITORIAL_CLAIM_UNIT
                ? "tcu"
                : typeID === TYPE_INFRASTRUCTURE_HUB
                    ? "ihub"
                    : "unknown",
...
function syncSovereigntyStructureRuntime(systemState, options = {}) {
    ...
    const desiredStructures = (Array.isArray(systemState && systemState.structures)
        ...
        : [])
        .filter((structure) => SOVEREIGNTY_TYPE_IDS.has(normalizePositiveInteger(structure && structure.typeID, 0)))
        .map((structure) => buildMirrorRecord(systemState, structure, nowMs));
    const desiredIDs = new Set(desiredStructures
...
//# sourceMappingURL=sovSpaceInterop.js.map