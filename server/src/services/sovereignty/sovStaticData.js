...
const { buildSovereigntyStaticSnapshotFromAuthority, } = require(path.join(__dirname, "./sovStaticAuthority"));
...
function getPlanetResourceSource(planetID) {
    return (PLANET_FAKE_OVERRIDES[planetID] ||
...
function getSovNullSystems() {
    return worldData
        ...
        .sort((left, right) => Number(left && left.solarSystemID) - Number(right && right.solarSystemID));
}
function buildPlanetDefinitionsAndIndex() {
    ...
    for (const system of getSovNullSystems()) {
        ...
        const planets = worldData
            ...
            .filter((celestial) => Number(celestial && celestial.groupID) === 7 ||
            String(celestial && celestial.kind).toLowerCase() === "planet")
            .sort((left, right) => Number(left.itemID || 0) - Number(right.itemID || 0));
        planetsBySolarSystemID.set(solarSystemID, planets.map((planet) => Number(planet.itemID || 0) || 0));
        for (const planet of planets) {
            const planetID = Number(planet.itemID || 0) || 0;
            const definition = buildPlanetDefinitionFromSource(planet, getPlanetResourceSource(planetID));
...
function buildStarConfigurations() {
    ...
    for (const system of getSovNullSystems()) {
        const star = worldData
            ...
            .find((celestial) => Number(celestial && celestial.groupID) === 6 ||
...
function deriveFallbackStarPower(star) {
    ...
    return Math.max(FALLBACK_STAR_POWER_MIN, Math.min(FALLBACK_STAR_POWER_MAX, FALLBACK_STAR_POWER_MIN + (bucket * 50)));
}
function buildUpgradeDefinitions() {
    ...
    const itemTypes = readStaticRows(TABLE.ITEM_TYPES)
        .filter((type) => Boolean(type && type.published) &&
    ...
    for (const itemType of itemTypes) {
        ...
        const fakeDefinition = UPGRADE_FAKE_BY_REMAINDER[typeID % 4] || UPGRADE_FAKE_BY_REMAINDER[0];
        const definition = {
            ...
            fuelConsumptionPerHour: Number(fakeDefinition.fuelConsumptionPerHour || 0) || 0,
            fuelStartupCost: Number(fakeDefinition.fuelStartupCost || 0) || 0,
            mutuallyExclusiveGroup: String(fakeDefinition.mutuallyExclusiveGroup || ""),
...
//# sourceMappingURL=sovStaticData.js.map