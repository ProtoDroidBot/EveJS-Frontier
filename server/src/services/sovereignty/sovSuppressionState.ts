...
function listStructuresForSuppressionSystem(solarSystemID, options: Record<string, any> = {}) {
...
function getLiveTenebrexForSolarSystem(solarSystemID, options: Record<string, any> = {}) {
...
function getLiveTenebrexStructuresForSolarSystem(solarSystemID, options: Record<string, any> = {}) {
...
function isTenebrexOperational(structure, nowMs = Date.now(), options: Record<string, any> = {}) {
  if (!structure) {
    return false;
  }
  if (
...
function collectChangedStructureSystemIDs(previousRows: any[] = [], nextRows: any[] = []) {
  const previousByID = new Map<any, any>(
    (Array.isArray(previousRows) ? previousRows : [])
      .map((entry) => [normalizePositiveInteger(entry && entry.structureID, 0), entry] as const)
  ...
  for (const structureID of new Set<any>([...previousByID.keys(), ...nextByID.keys()])) {
...
function broadcastCynoJammerChangesForStructureRows(previousRows: any[] = [], nextRows: any[] = [], options: Record<string, any> = {}) {
...
function getCynoJammerActivationTimeMs(solarSystemID, options: Record<string, any> = {}) {
  ...
  const pendingActivationTimes: any[] = [];
...
function getCynoJammerOnlineSimTime(solarSystemID, options: Record<string, any> = {}) {
...
function isSolarSystemCynoJammed(solarSystemID, options: Record<string, any> = {}) {
  const nowMs = normalizeInteger(options.nowMs, Date.now());
  const activationTimeMs = getCynoJammerActivationTimeMs(solarSystemID, options);
