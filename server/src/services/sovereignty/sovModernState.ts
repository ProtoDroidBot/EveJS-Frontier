...
function resolveSessionInteger(session, names: any[] = []) {
  if (!session) {
    return null;
  }
...
function normalizeWorkforceConfiguration(value: Record<string, any> = {}, fallbackHubID = null) {
...
function normalizeUpgradeInstallation(value: Record<string, any> = {}) {
  return {
...
function normalizeFuelLedger(value: Record<string, any> = {}) {
...
function normalizeHubRecord(value: Record<string, any> = {}, fallback: Record<string, any> = {}) {
...
function normalizeSkyhookRecord(value: Record<string, any> = {}, fallback: Record<string, any> = {}) {
...
function arrayShallowEquals(left: any[] = [], right: any[] = []) {
...
function bootstrapHubsFromSystems(table) {
  let changed = false;
  const validHubKeys = new Set<any>();
  for (const [solarSystemID, system] of Object.entries<any>(table.systems || {})) {
...
function bootstrapSkyhooksFromSystems(table, staticSnapshot, options: Record<string, any> = {}) {
  let changed = false;
  const refreshResourceDefinitions = Boolean(options.refreshResourceDefinitions);
  const validSkyhookKeys = new Set<any>();
  for (const [solarSystemID, system] of Object.entries<any>(table.systems || {})) {
    const numericSolarSystemID = normalizePositiveInteger(solarSystemID, null);
    const allianceID = normalizePositiveInteger(system && system.allianceID, null);
    const corporationID = normalizePositiveInteger(
...
function computeHubUpgradeSnapshot(hubRecord, stateCache) {
  ...
  for (const installation of hubRecord.installedUpgrades) {
    ...
    let powerState: number = POWER_STATE.POWER_STATE_OFFLINE;
...
function ensureLoaded() {
  ...
  for (const [hubID, rawHub] of Object.entries<any>(table.hubs || {})) {
  ...
  for (const [skyhookID, rawSkyhook] of Object.entries<any>(table.skyhooks || {})) {
...
function requireSkyhookAccess(skyhookID, identity, options: Record<string, any> = {}) {
  const skyhook =
...
function configureWorkforce(hubID, configuration, identity) {
  ...
  const previousState: Record<string, any> = getWorkforceState(access.hub.hubID);
  return mutateTable((table) => {
    const hub = normalizeHubRecord(table.hubs[String(access.hub.hubID)], access.hub);
