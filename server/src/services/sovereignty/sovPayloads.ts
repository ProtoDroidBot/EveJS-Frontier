...
function buildAlliancePrimeInfoPayload(primeInfo: Record<string, any> = {}) {
  return buildKeyVal([
...
function buildAllianceCapitalInfoPayload(capitalInfo: Record<string, any> = {}) {
  return buildKeyVal([
...
function buildAllianceSystemListPayload(records: any[] = []) {
  return buildList(
    records.map((record) =>
      buildKeyVal([
...
function buildDevelopmentIndicesForSystemPayload(indicesByAttribute: Record<string, any> = {}) {
  return buildDict(
...
function buildAllDevelopmentIndicesPayload(records: any[] = []) {
  return buildList(
    records.map((record) =>
      buildKeyVal([
        ["solarSystemID", Number(record.solarSystemID || 0)],
...
function buildCampaignStatePayload(structure: Record<string, any> = {}) {
  ...
  return [
    ...
    buildDict(
      Object.entries<any>(structure.campaignScoresByTeam || {}).map(([teamID, score]) => [
        Number(teamID),
        Number(score || 0),
      ]),
    ),
...
function buildVulnerabilityStatePayload(structure: Record<string, any> = {}) {
...
function buildSovStructuresPayload(structures: any[] = []) {
  return buildList(
...
function buildAllianceSovereigntyRowsPayload(rows: Record<string, any> = {}) {
...
function buildCurrentSovDataPayload(rows: any[] = []) {
  return buildRowset(
...
function buildRecentSovActivityPayload(rows: any[] = []) {
  return buildRowset(
