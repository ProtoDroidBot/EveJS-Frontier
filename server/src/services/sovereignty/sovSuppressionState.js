...
const { STRUCTURE_STATE, STRUCTURE_UPKEEP_STATE, STRUCTURE_SERVICE_ID, STRUCTURE_SERVICE_STATE, } = require(path.join(__dirname, "../structure/structureConstants"));
const { getHubIDForSolarSystem, getHubUpgrades, } = require(path.join(__dirname, "./sovModernState"));
const { TYPE_CYNO_SUPPRESSION_UPGRADE, TYPE_TENEBREX_CYNO_JAMMER, } = require(path.join(__dirname, "./sovUpgradeSupport"));
const { findServiceProximityConflict, } = require(path.join(__dirname, "../structure/structureServiceProximity"));
const { broadcastCynoJammerChanged, } = require(path.join(__dirname, "./sovNotifications"));
...
function listStructuresForSuppressionSystem(solarSystemID, options = {}) {
    ...
    if (Array.isArray(options.structureRows)) {
        return options.structureRows.filter((structure) => (normalizePositiveInteger(structure && structure.solarSystemID, 0) ===
...
function getLiveTenebrexForSolarSystem(solarSystemID, options = {}) {
    ...
    return structures.find((structure) => (normalizePositiveInteger(structure && structure.typeID, 0) === TYPE_TENEBREX_CYNO_JAMMER &&
        !normalizePositiveInteger(structure && structure.destroyedAt, null))) || null;
}
function getLiveTenebrexStructuresForSolarSystem(solarSystemID, options = {}) {
    return listStructuresForSuppressionSystem(solarSystemID, options).filter((structure) => (normalizePositiveInteger(structure && structure.typeID, 0) === TYPE_TENEBREX_CYNO_JAMMER &&
...
function isCynoSuppressionUpgradeOnline(solarSystemID) {
    ...
    const cynoSuppression = Array.isArray(upgrades && upgrades.upgrades)
        ? upgrades.upgrades.find((upgrade) => normalizePositiveInteger(upgrade && upgrade.typeID, 0) === TYPE_CYNO_SUPPRESSION_UPGRADE) || null
        : null;
...
function isTenebrexOperational(structure, nowMs = Date.now(), options = {}) {
    ...
    if (normalizeInteger(structure &&
        structure.serviceStates &&
        structure.serviceStates[String(STRUCTURE_SERVICE_ID.CYNO_JAMMER)], 0) !== STRUCTURE_SERVICE_STATE.ONLINE) {
        return false;
    }
    if (normalizeInteger(structure && structure.upkeepState, 0) !==
    ...
    if (findServiceProximityConflict(structure, STRUCTURE_SERVICE_ID.CYNO_JAMMER, listStructuresForSuppressionSystem(structure.solarSystemID, options).filter((candidate) => (!normalizePositiveInteger(candidate && candidate.destroyedAt, null))))) {
        return false;
    }
    return true;
}
...
function broadcastCynoJammerChangesForStructureRows(previousRows = [], nextRows = [], options = {}) {
    ...
    for (const systemID of systemIDs) {
        ...
        if (formatOnlineSimTimeForCompare(previousSimTime) ===
...
function getCynoJammerActivationTimeMs(solarSystemID, options = {}) {
    ...
    const flexFuelState = structure &&
        structure.devFlags &&
        structure.devFlags.flexFuelState &&
        typeof structure.devFlags.flexFuelState === "object"
        ? structure.devFlags.flexFuelState
        : null;
    const serviceActivatesAt = normalizeInteger(flexFuelState && flexFuelState.cynoJamActivatesAt, 0);
...
function isSolarSystemCynoJammed(solarSystemID, options = {}) {
    const nowMs = normalizeInteger(options.nowMs, Date.now());
    const activationTimeMs = getCynoJammerActivationTimeMs(solarSystemID, options);
    return activationTimeMs === 0 || (normalizePositiveInteger(activationTimeMs, null) !== null &&
...
//# sourceMappingURL=sovSuppressionState.js.map