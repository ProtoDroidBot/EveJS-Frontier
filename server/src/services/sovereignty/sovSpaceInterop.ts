...
function syncSovereigntyStructureRuntime(systemState, options: Record<string, any> = {}) {
...
function clearAllSovereigntyStructureMirrors(options: Record<string, any> = {}) {
  const structures = structureState.listStructures({
    includeDestroyed: true,
    refresh: false,
  }).filter((structure) => isSovereigntyMirrorStructure(structure));
  const affectedSystems = new Set<any>();
...
function clearAllSovereigntyRelatedStructures(options: Record<string, any> = {}) {
  const structures = structureState.listStructures({
    includeDestroyed: true,
    refresh: false,
