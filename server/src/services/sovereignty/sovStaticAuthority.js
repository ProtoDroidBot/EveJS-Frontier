...
function buildSovereigntyStaticSnapshotFromAuthority() {
    ...
    const planetDefinitions = normalizeDefinitionRows(authority && authority.planetDefinitions);
    const starConfigurations = normalizeDefinitionRows(authority && authority.starConfigurations);
    const upgradeDefinitions = normalizeDefinitionRows(authority && authority.upgradeDefinitions);
    if (planetDefinitions.length === 0 &&
    ...
    return {
        ...
        planetDefinitionsVersion: normalizeSemanticVersion(authority && authority.planetDefinitionsVersion) || cloneValue(FALLBACK_PLANET_DEFINITION_VERSION),
        planetsBySolarSystemID: buildPlanetsBySolarSystemID(authority && authority.planetsBySolarSystemID),
        starConfigurations,
...
//# sourceMappingURL=sovStaticAuthority.js.map