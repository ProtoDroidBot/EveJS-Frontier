...
function uniqueNumericValues(values: any[] = []) {
  return [...new Set(
...
function getTargetSessions({ solarSystemID = null, allianceIDs = [] }: Record<string, any> = {}) {
...
function sendNotificationToSessions(sessions, notifyType, idType, payloadTuple: any[] = []) {
...
function buildStructureChangesPayload(changesByStructureID = null) {
  ...
  const entries = Object.entries<any>(changesByStructureID)
...
function buildSovHubHackPayload(upgrades: any[] = []) {
  return buildList(
