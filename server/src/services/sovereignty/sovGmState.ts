...
function anchorSovereigntyStructures(
  ...
  const namesByKind =
    options && typeof options === "object" && options.namesByKind
      ...
      : null;
  const itemIDsByKind =
    options && typeof options === "object" && options.itemIDsByKind
  ...
  function getPositionForKind(targetKind) {
    ...
    function ensureStructure(targetKind) {
      ...
      const hasRequestedItemID = Boolean(
        itemIDsByKind &&
      ...
      if (
        requestedItemID &&
        currentStructures.some((structure) => (
          Number(structure && structure.itemID) === requestedItemID
        ))
      ) {
      ...
      const nextStructure: Record<string, any> = {
      itemID,
...
function setCampaignScoresForKind(solarSystemID, kind, scoresByTeam) {
  ...
  const normalizedScoresByTeam: Record<string, any> = {};
  for (const [teamID, score] of Object.entries<any>(scoresByTeam || {})) {
...
function describeCampaign(structure) {
  ...
  const scoreEntries = Object.entries<any>(structure && structure.campaignScoresByTeam || {})
...
function describeSovereigntyFlexStructures(solarSystemID) {
  ...
  const flexDefinitionsByTypeID = new Map<any, any>(
