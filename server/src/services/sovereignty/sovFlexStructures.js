...
const { STRUCTURE_SERVICE_ID, STRUCTURE_SERVICE_STATE, STRUCTURE_UPKEEP_STATE, } = require(path.join(__dirname, "../structure/structureConstants"));
const { buildAnchorLayout, cloneSpacePoint, normalizePositiveInteger, } = require(path.join(__dirname, "./sovAnchorLayout"));
const { getStructurePositionsForSystem, } = require(path.join(__dirname, "./sovAutoNavigation"));
...
const { getSystemState, } = require(path.join(__dirname, "./sovState"));
const { TYPE_ANSIBLEX_JUMP_BRIDGE, TYPE_ADVANCED_LOGISTICS_NETWORK_UPGRADE, TYPE_CYNO_NAVIGATION_UPGRADE, TYPE_CYNO_SUPPRESSION_UPGRADE, TYPE_PHAROLUX_CYNO_BEACON, TYPE_TENEBREX_CYNO_JAMMER, canSolarSystemSupportUpgrade, } = require(path.join(__dirname, "./sovUpgradeSupport"));
...
const FLEX_TYPE_IDS = new Set(FLEX_KIND_ORDER.map((kind) => FLEX_DEFINITION_BY_KIND[kind].typeID));
...
function getTypeDogmaRecord(typeID) {
    ...
    return (dogma &&
        dogma.typesByTypeID &&
        dogma.typesByTypeID[String(normalizePositiveInteger(typeID, 0))]) || null;
}
function getDogmaAttributeValue(typeID, attributeID, fallback = 0) {
    ...
    const rawValue = attributes && Object.prototype.hasOwnProperty.call(attributes, String(attributeID))
...
function listSovereigntyFlexStructuresForSystem(solarSystemID, options = {}) {
    ...
    return structureState.listStructuresForSystem(numericSolarSystemID, {
        includeDestroyed,
        refresh: false,
    }).filter((structure) => FLEX_TYPE_IDS.has(normalizePositiveInteger(structure && structure.typeID, 0)));
}
function listFlexStructuresByKind(solarSystemID, kind, options = {}) {
    ...
    return listSovereigntyFlexStructuresForSystem(solarSystemID, options)
        .filter((structure) => normalizePositiveInteger(structure && structure.typeID, 0) === definition.typeID)
        .sort((left, right) => normalizePositiveInteger(left && left.structureID, 0) -
...
function getFlexFuelProfile(definition) {
    const serviceModuleTypeID = normalizePositiveInteger(getDogmaAttributeValue(definition.typeID, 2792, 0), null);
    const hourlyAmount = Math.max(0, getDogmaAttributeValue(serviceModuleTypeID, 2109, 0));
    const onlineAmount = Math.max(0, getDogmaAttributeValue(serviceModuleTypeID, 2110, 0));
    return {
        serviceModuleTypeID,
...
function buildFlexPosition(solarSystemID, definition) {
    ...
    const basePoint = clonePoint(currentPositions.positionsByKind.ihub || currentPositions.positionsByKind.tcu, null);
    if (basePoint) {
        return addPoint(basePoint, definition.offset);
    }
    return addPoint(buildAnchorLayout(solarSystemID).primaryPoint, definition.offset);
}
...
function getJumpBridgeDestinationSolarSystemID(structure) {
    const devFlags = structure && structure.devFlags && typeof structure.devFlags === "object"
        ? structure.devFlags
        : {};
    return normalizePositiveInteger(devFlags.destinationSolarsystemID ||
        devFlags.sovereigntyJumpBridgeDestinationSolarsystemID, null);
}
...
function seedSovereigntyFlexFuel(structureID, hours = DEFAULT_SOV_FLEX_FUEL_HOURS) {
    ...
    const definition = getSovereigntyFlexDefinitions().find((entry) => entry.typeID === normalizePositiveInteger(structure.typeID, 0));
    if (!definition) {
        return {
            success: false,
    ...
    const liquidOzoneQty = Math.ceil(Math.max(0, fuelProfile.onlineAmount) +
...
function ensureFlexUpgradeOnline(solarSystemID, definition, identity, fuelHours) {
    ...
    const upgradeResult = setHubUpgradeInstallations(hubID, [{ typeID: definition.requiredUpgradeTypeID, online: true }], identity);
    if (!upgradeResult.ok) {
        return {
            success: false,
    ...
    const requiredUpgrade = upgradeSnapshot &&
        Array.isArray(upgradeSnapshot.upgrades)
        ? upgradeSnapshot.upgrades.find((entry) => Number(entry && entry.typeID) === definition.requiredUpgradeTypeID) || null
        : null;
    ...
    const seedFuelResult = seedHubFuelForInstalledUpgrades(hubID, fuelHours, identity);
    return {
        success: true,
        data: {
            hubID,
...
function deploySovereigntyFlexStructure(session, kind, options = {}) {
    ...
    const solarSystemID = normalizePositiveInteger(options.solarSystemID || (session && (session.solarsystemid2 || session.solarsystemid)), null);
    if (!solarSystemID) {
        return {
            success: false,
    ...
    const allianceID = normalizePositiveInteger(options.allianceID || (system && system.allianceID) || (session && (session.allianceID || session.allianceid)), null);
    const corporationID = normalizePositiveInteger(options.ownerCorpID || options.corporationID || (system && system.corporationID) || (session && (session.corporationID || session.corpid)), null);
    if (!system ||
    ...
    const maxPerSolarSystem = FLEX_MAX_PER_SOLAR_SYSTEM_BY_KIND[definition.kind] || 1;
    ...
    const identity = {
        characterID: normalizePositiveInteger(session && (session.characterID || session.charid || session.userid), 0) || 0,
        solarSystemID,
        allianceID,
        corporationID,
    };
    const fuelHours = Math.max(1, normalizeInteger(options.fuelHours, DEFAULT_SOV_FLEX_FUEL_HOURS));
    const upgradeResult = ensureFlexUpgradeOnline(solarSystemID, definition, identity, fuelHours);
    ...
    const seedResult = structureState.seedStructureForSession(session || {}, definition.typeID, {
        solarSystemID,
        ownerCorpID: corporationID,
        allianceID,
        position: normalizePoint(options.position, buildFlexPosition(solarSystemID, definition)),
    ...
    const fastForwardShortResult = advanceStructureTimers(structureID, FLEX_DEPLOY_FAST_FORWARD_SHORT_SECONDS);
    if (!fastForwardShortResult.success) {
        return fastForwardShortResult;
    }
    const fastForwardLongResult = advanceStructureTimers(structureID, FLEX_DEPLOY_FAST_FORWARD_LONG_SECONDS);
    ...
    const serviceResult = structureState.setStructureServiceState(structureID, definition.serviceID, STRUCTURE_SERVICE_STATE.ONLINE, { consumeFlexOnlineFuel: false });
    ...
    const serviceFastForwardResult = advanceStructureTimers(structureID, FLEX_DEPLOY_FAST_FORWARD_SHORT_SECONDS);
...
//# sourceMappingURL=sovFlexStructures.js.map