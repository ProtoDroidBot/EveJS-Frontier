...
const { normalizeText, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { ITEM_FLAGS, consumeInventoryItemQuantity, findItemById, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const { DEPLOY_DIST_MAX, validateDeploymentPlacement, } = require(path.join(__dirname, "../structure/structureDeploymentPlacement"));
const { resolveUsableProfileIDForCorporation, } = require(path.join(__dirname, "../structure/structureProfilesState"));
const { DEFAULT_REINFORCE_HOUR, DEFAULT_REINFORCE_WEEKDAY, STRUCTURE_UPKEEP_STATE, } = require(path.join(__dirname, "../structure/structureConstants"));
const { anchorSovereigntyStructures, ensureValidSolarSystem, STRUCTURE_KIND, } = require(path.join(__dirname, "./sovGmState"));
const { TYPE_INFRASTRUCTURE_HUB, TYPE_TERRITORIAL_CLAIM_UNIT, } = require(path.join(__dirname, "./sovConstants"));
const { DEFAULT_SOV_FLEX_FUEL_HOURS, deploySovereigntyFlexStructure, } = require(path.join(__dirname, "./sovFlexStructures"));
const { getSystemDefenseMultiplier, getSystemState, publishSystemStateEffects, replaceSystemStateSnapshot, upsertSystemState, } = require(path.join(__dirname, "./sovState"));
const { invalidateSovereigntyModernStateCache, } = require(path.join(__dirname, "./sovModernState"));
const { isSovereigntyClaimableSolarSystem, } = require(path.join(__dirname, "./sovSystemRules"));
const { TYPE_ANSIBLEX_JUMP_BRIDGE, TYPE_PHAROLUX_CYNO_BEACON, TYPE_TENEBREX_CYNO_JAMMER, } = require(path.join(__dirname, "./sovUpgradeSupport"));
...
function resolvePositionFromClientRequest(session, x, z) {
    ...
    const coordinateMode = absoluteDistance <= DEPLOY_DIST_MAX && relativeDistance > DEPLOY_DIST_MAX
        ? "absolute"
        : "relative";
...
function validateSovereigntyCorePlacementForSession(session, typeID, position, options = {}) {
    const numericPosition = position && typeof position === "object"
        ...
        : null;
    if (options.clientPositionValid === false ||
        !numericPosition ||
    ...
    const distance = Math.hypot(numericPosition.x - Number(shipEntity.position.x || 0), numericPosition.y - Number(shipEntity.position.y || 0), numericPosition.z - Number(shipEntity.position.z || 0));
...
function validateStructureDeploymentPlacementForSession(session, typeID, position, options = {}) {
    ...
    if (getSovereigntyKindForTypeID(typeID)) {
        return validateSovereigntyCorePlacementForSession(session, typeID, position, options);
    }
    ...
    const conflictTypeID = validation.data && validation.data.ballTypeID
...
function requireStructureManagementSession(session, structure) {
    ...
    const corporationID = normalizePositiveInteger(session && (session.corporationID || session.corpid), null);
    const ownerCorpID = normalizePositiveInteger(structure && (structure.ownerCorpID || structure.ownerID), null);
    ...
    const hasStructureRole = (roleMask & CORP_ROLE_STATION_MANAGER) !== 0n ||
...
function requireStructureDecommissionSession(session, structure) {
    ...
    const roleMask = normalizeRoleMask(session && session.corprole);
    const hasDecommissionRole = (roleMask & CORP_ROLE_DIRECTOR) !== 0n ||
        session.isCEO === true;
...
function validateHostileAdmDeployment(session, solarSystemID, typeID) {
    ...
    const ownerAllianceID = normalizePositiveInteger(systemState && systemState.allianceID, null);
    if (!ownerAllianceID) {
        return;
    }
    const deployerAllianceID = normalizePositiveInteger(session && (session.allianceID || session.allianceid), null);
...
function updateNamedSovStructure(systemID, structureID, structureName) {
    ...
    upsertSystemState(systemID, {
        structures: currentSystem.structures.map((structure) => (Number(structure && structure.itemID) === Number(structureID)
            ? {
...
function deploySovereigntyCoreFromItem(session, item, options = {}) {
    ...
    const allianceID = normalizePositiveInteger(session && (session.allianceID || session.allianceid), null);
    const corporationID = normalizePositiveInteger(session && (session.corporationID || session.corpid), null);
    ...
    const existingStructure = currentSystem && Array.isArray(currentSystem.structures)
        ? currentSystem.structures.find((structure) => (Number(structure && structure.typeID) === Number(item && item.typeID)))
        : null;
    ...
    const sourceQuantity = normalizePositiveInteger(item && (item.stacksize ?? item.quantity), 1);
    const sourceWillBeRemoved = Number(item && item.singleton) === 1 || sourceQuantity <= 1;
    ...
    const preserveSourceItemID = Boolean(sourceItemID &&
        sourceWillBeRemoved &&
        Number.isSafeInteger(sourceItemID));
    const itemIDsByKind = preserveSourceItemID
        ? { [String(kind)]: sourceItemID }
        : null;
    const anchorResult = anchorSovereigntyStructures(solarSystemID, kind, allianceID, corporationID, {
        positionsByKind,
        namesByKind,
        itemIDsByKind,
    ...
    return {
        ...
        data: {
            ...
            solarSystemID,
            kind,
            structureID: kind === STRUCTURE_KIND.TCU
...
function deploySovereigntyFlexFromItem(session, item, options = {}) {
    ...
    const deployResult = deploySovereigntyFlexStructure(session, kind, {
        solarSystemID,
        allianceID,
        corporationID,
...
function deployGenericStructureFromItem(session, item, options = {}) {
    ...
    const fallbackStructureName = normalizeStructureText(item && item.itemName, typeRecord.name) ||
        normalizeStructureText(typeRecord && typeRecord.name, `Structure ${typeRecord.typeID}`);
    const resolvedStructureName = normalizeStructureText(options.structureName, fallbackStructureName) ||
        fallbackStructureName;
    const resolvedStructureDescription = normalizeStructureText(options.bio !== undefined ? options.bio : options.description, "");
    ...
    const createResult = structureState.createStructure({
        ...
        ownerCorpID: normalizePositiveInteger(session && (session.corporationID || session.corpid), 0) || 1000009,
        allianceID: normalizePositiveInteger(session && (session.allianceID || session.allianceid), null),
        ...
        reinforceWeekday: normalizeInteger(options.reinforceWeekday, DEFAULT_REINFORCE_WEEKDAY),
        reinforceHour: normalizeInteger(options.reinforceHour, DEFAULT_REINFORCE_HOUR),
        devFlags: {
    ...
    const startResult = structureState.startAnchoring(createResult.data.structureID, Date.now(), { emitLive: false });
...
function deployStructureFromInventoryItem(session, itemID, options = {}) {
    ...
    const shipID = getSessionShipID(session);
    if (Number(item.ownerID || 0) !== Number(session.characterID || session.charid || 0) ||
        Number(item.locationID || 0) !== Number(shipID || 0) ||
        !STRUCTURE_DEPLOYMENT_SOURCE_FLAGS.has(Number(item.flagID || 0))) {
        throwWrappedUserError("TargetingAttemptCancelled");
    }
    const structureName = normalizeStructureText(options.structureName, normalizeStructureText(item && item.itemName));
    ...
    const corporationID = normalizePositiveInteger(session && (session.corporationID || session.corpid), null);
    const resolvedProfileID = resolveUsableProfileIDForCorporation(corporationID, options.profileID);
    let resolvedOptions = {
        ...options,
    ...
    const placementValidation = validateStructureDeploymentPlacementForSession(session, typeID, position, resolvedOptions);
    const moonMiningBeaconID = normalizePositiveInteger(placementValidation &&
        placementValidation.data &&
        placementValidation.data.moonMiningBeaconID, null);
    if (moonMiningBeaconID) {
        const currentDevFlags = resolvedOptions.devFlags && typeof resolvedOptions.devFlags === "object"
    ...
    if (!consumeResult.success) {
        if (deployResult &&
            ...
            replaceSystemStateSnapshot(deployResult.data.solarSystemID, previousSovereigntySystem, {
                suppressStructureSync: true,
                suppressNotifications: true,
            });
        }
    ...
    if (deployResult &&
        ...
        const publishResult = publishSystemStateEffects(previousSovereigntySystem, systemState);
...
function removeSovereigntyStructureFromSystem(solarSystemID, structureID) {
    ...
    const remainingStructures = currentSystem.structures.filter((structure) => Number(structure && structure.itemID) !== Number(structureID));
    const nextPatch = {
...
function unanchorStructureByID(session, structureID) {
    ...
    if (getSovereigntyKindForTypeID(typeID)) {
        const result = removeSovereigntyStructureFromSystem(normalizePositiveInteger(structure.solarSystemID, 0) || getSessionSolarSystemID(session), structure.structureID);
        if (!result.success) {
            throwWrappedUserError("CustomNotify", {
...
//# sourceMappingURL=sovPlayerDeployment.js.map