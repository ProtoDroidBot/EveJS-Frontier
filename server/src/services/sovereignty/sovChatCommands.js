...
const { DEFAULT_CAMPAIGN_EVENT_TYPE, DEFAULT_CAMPAIGN_OFFSET_SECONDS, DEFAULT_VULNERABILITY_DURATION_SECONDS, STRUCTURE_KIND, anchorSovereigntyStructures, buildSovereigntyStatusReport, cancelAllianceCapitalTransition, captureSystemForOwner, clearCampaignForKind, clearSystemSovereignty, clearVulnerabilityWindowForKind, ensureValidSolarSystem, fastForwardSovereigntyTimers, loseSystemSovereignty, normalizeStructureKind, resolveAllianceID, resolveCorporationID, resolveSolarSystemID, setAllianceCapitalSystem, setAlliancePrimeHour, setCampaignScoresForKind, setSystemDevelopmentIndexValue, setVulnerabilityWindowForKind, startCampaignForKind, } = require(path.join(__dirname, "./sovGmState"));
const { DEFAULT_SOV_FLEX_FUEL_HOURS, deploySovereigntyFlexStructure, getSovereigntyFlexDefinitions, normalizeSovereigntyFlexKind, } = require(path.join(__dirname, "./sovFlexStructures"));
const { getSystemState, } = require(path.join(__dirname, "./sovState"));
const { getUpgradeDefinition, } = require(path.join(__dirname, "./sovUpgradeSupport"));
const { buildSessionAnchorLayout, } = require(path.join(__dirname, "./sovAutoNavigation"));
...
function executeSovCommand(session, argumentText) {
    ...
    if (subcommand === "anchor") {
        ...
        const result = anchorSovereigntyStructures(systemResult.data, kind, allianceID, corporationID, {
            positionsByKind: buildSessionAnchorLayout(session, systemResult.data).positionsByKind,
        });
        if (!result.success) {
            return {
                success: false,
        ...
        return {
            success: true,
            message: appendStatus(`Anchored ${kind} sovereignty structure${kind === STRUCTURE_KIND.BOTH ? "s" : ""} in ${systemResult.data}.`, systemResult.data),
        };
    }
    if (subcommand === "deploy") {
        ...
        const allianceID = resolveAllianceID(session, rest[2], normalizePositiveInteger(systemState.allianceID, null));
        const corporationID = resolveCorporationID(session, rest[3], normalizePositiveInteger(systemState.corporationID, null));
        ...
        for (const definition of definitions) {
            ...
            strategicFloor = Math.max(strategicFloor, Math.max(0, normalizeInteger(upgradeDefinition && upgradeDefinition.requiredStrategicIndex, 0)));
        }
        if (strategicFloor > 0) {
            setSystemDevelopmentIndexValue(systemResult.data, "strategic", strategicFloor);
        }
        const deployedLabels = [];
        for (const definition of definitions) {
            const deployResult = deploySovereigntyFlexStructure(session, definition.kind, {
            ...
            deployedLabels.push(deployResult.data.created === false
                ? `${definition.label} refreshed`
                : `${definition.label} deployed`);
        }
        return {
            success: true,
            message: appendStatus(`${deployedLabels.join(", ")} in ${systemResult.data}. Required Sov Hub upgrades were brought online and fueled automatically.`, systemResult.data),
        };
    }
    if (subcommand === "prime") {
    ...
    if (subcommand === "capital") {
        ...
        const allianceID = resolveAllianceID(session, firstToken === "clear" || firstToken === "cancel" || firstToken === "off"
            ? rest[1]
            : rest[1], null);
        if (!allianceID) {
            return {
    ...
    if (subcommand === "index") {
        ...
        const result = setSystemDevelopmentIndexValue(systemResult.data, indexKind, value);
        if (!result.success) {
            return {
                success: false,
        ...
        return {
            success: true,
            message: appendStatus(`Updated ${indexKind} index in ${systemResult.data}.`, systemResult.data),
        };
    }
    if (subcommand === "vuln") {
        const kind = normalizeStructureKind(rest[0]);
        ...
        const result = modeToken === "clear"
            ? clearVulnerabilityWindowForKind(systemResult.data, kind)
            : setVulnerabilityWindowForKind(systemResult.data, kind, Math.max(1, normalizeInteger(modeToken, DEFAULT_VULNERABILITY_DURATION_SECONDS)));
        if (!result.success) {
            return {
                success: false,
        ...
        return {
            success: true,
            message: appendStatus(modeToken === "clear"
                ? `Cleared ${kind} vulnerability windows in ${systemResult.data}.`
                : `Opened ${kind} vulnerability windows for ${Math.max(1, normalizeInteger(modeToken, DEFAULT_VULNERABILITY_DURATION_SECONDS))} seconds in ${systemResult.data}.`, systemResult.data),
        };
    }
    if (subcommand === "campaign") {
        ...
        const result = modeToken === "clear"
            ...
            : startCampaignForKind(systemResult.data, kind, Math.max(1, normalizeInteger(modeToken, DEFAULT_CAMPAIGN_EVENT_TYPE)), Math.max(0, normalizeInteger(rest[2], DEFAULT_CAMPAIGN_OFFSET_SECONDS)));
        if (!result.success) {
            return {
                success: false,
        ...
        return {
            ...
            message: appendStatus(modeToken === "clear"
                ...
                : `Started ${kind} campaign event ${Math.max(1, normalizeInteger(modeToken, DEFAULT_CAMPAIGN_EVENT_TYPE))} in ${systemResult.data}.`, systemResult.data),
        };
    }
    if (subcommand === "score") {
        const kind = normalizeStructureKind(rest[0]);
        ...
        const result = setCampaignScoresForKind(systemResult.data, kind, scoresByTeam);
        if (!result.success) {
            return {
                success: false,
        ...
        return {
            success: true,
            message: appendStatus(`Updated ${kind} campaign scores in ${systemResult.data}.`, systemResult.data),
        };
    }
    if (subcommand === "ff") {
        ...
        return {
            success: true,
            message: appendStatus(`Fast-forwarded sovereignty timers by ${seconds} seconds in ${systemResult.data}.`, systemResult.data),
        };
    }
    if (subcommand === "capture") {
        ...
        const result = captureSystemForOwner(systemResult.data, allianceID, corporationID);
        if (!result.success) {
            return {
                success: false,
        ...
        return {
            success: true,
            message: appendStatus(`Captured solar system ${systemResult.data} for alliance ${allianceID} corporation ${corporationID}.`, systemResult.data),
        };
    }
    if (subcommand === "lose") {
        ...
        const result = loseSystemSovereignty(systemResult.data, nextAllianceID, nextCorporationID);
        if (!result.success) {
            return {
                success: false,
        ...
        return {
            success: true,
            message: appendStatus(nextAllianceID && nextCorporationID
                ? `Transferred solar system ${systemResult.data} to alliance ${nextAllianceID} corporation ${nextCorporationID}.`
                : `Cleared sovereignty ownership in solar system ${systemResult.data}.`, systemResult.data),
        };
    }
    if (subcommand === "clear") {
        const systemResult = parseSystemToken(session, rest[0] || "here");
        ...
        return {
            success: true,
            message: appendStatus(`Cleared sovereignty state in solar system ${systemResult.data}.`, systemResult.data),
        };
    }
    return {
        success: false,
...
//# sourceMappingURL=sovChatCommands.js.map