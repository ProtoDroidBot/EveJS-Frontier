...
function buildOffsetPosition(anchor, options = {}) {
    ...
    const fallbackDirection = normalizeDirection(options.fallbackDirection, { x: 1, y: 0, z: 0 });
    const direction = normalizeDirection(magnitude(anchorPosition) > 0 ? anchorPosition : fallbackDirection, fallbackDirection);
...
function buildSystemReferenceAnchor(solarSystemID) {
    ...
    const celestial = celestials.find((entry) => entry.kind !== "sun" && Number(entry.groupID) !== 6) ||
    ...
    if (celestial) {
        return buildOffsetPosition(celestial, {
            minOffset: 100000,
            clearance: celestial.kind === "sun" || Number(celestial.groupID) === 6
                ? 250000
                : 25000,
        });
    }
...
function buildDefaultAnchorOrigin(solarSystemID) {
    ...
    const scene = spaceRuntime.ensureScene(normalizePositiveInteger(solarSystemID, 0));
    if (scene && referenceAnchor && referenceAnchor.position) {
        const safeOrigin = findSafeWarpOriginAnchor(scene, {
...
function buildAnchorLayout(solarSystemID, origin = null) {
    const anchorOrigin = cloneSpacePoint(origin, buildDefaultAnchorOrigin(solarSystemID));
...
//# sourceMappingURL=sovAnchorLayout.js.map