...
function buildPositionFromClientRequest(session, x?: any, z?: any) {
...
function validateSovereigntyCorePlacementForSession(
  ...
  if (!shipEntity || !shipEntity.position) {
    ...
    return { success: true as const, data: { skipped: true } };
  }
...
function validateStructureDeploymentPlacementForSession(session, typeID, position, options: Record<string, any> = {}) {
...
function deploySovereigntyCoreFromItem(session, item, options: Record<string, any> = {}) {
  const solarSystemID = getSessionSolarSystemID(session);
  ...
  const positionsByKind: Record<string, any> = {};
  positionsByKind[String(kind)] = {
    x: Number(position.x || 0),
    y: Number(position.y || 0),
    z: Number(position.z || 0),
  };
  const namesByKind: Record<string, any> = {};
...
function deploySovereigntyFlexFromItem(session, item, options: Record<string, any> = {}) {
  const solarSystemID = getSessionSolarSystemID(session);
  validateClaimableNullsecSolarSystem(solarSystemID);
...
function deployGenericStructureFromItem(session, item, options: Record<string, any> = {}) {
  const solarSystemID = getSessionSolarSystemID(session);
...
function deployStructureFromInventoryItem(session, itemID, options: Record<string, any> = {}) {
