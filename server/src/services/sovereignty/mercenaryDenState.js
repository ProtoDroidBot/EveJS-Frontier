...
const { cloneValue, readSovereigntyTable, writeSovereigntyTable, } = require(path.join(__dirname, "./sovStore"));
const { createUuidString, } = require(path.join(__dirname, "../../_secondary/express/gatewayServices/gatewayServiceHelpers"));
const { DEFAULT_MERCENARY_DEN_ABSOLUTE_MAXIMUM, DEFAULT_MERCENARY_DEN_ACTIVITY_CAPACITY, MERCENARY_DEN_MANAGEMENT_SKILL_TYPE_ID, TYPE_MERCENARY_DEN, } = require(path.join(__dirname, "./sovConstants"));
...
function getSkillLevel(characterID, skillTypeID) {
    ...
    return Math.max(0, Math.min(5, normalizeInteger(record &&
...
function normalizeEvolutionRecord(value = {}) {
    ...
    return {
        definition: {
            development: {
                unitIncreaseTimeSeconds: Math.max(1, normalizeInteger(developmentDefinition.unitIncreaseTimeSeconds, fallbackDefinition.development.unitIncreaseTimeSeconds)),
                stages: (Array.isArray(developmentDefinition.stages)
                    ? developmentDefinition.stages
                    : fallbackDefinition.development.stages).map((entry, index) => ({
                    stage: Math.max(1, normalizeInteger(entry && entry.stage, index + 1)),
                    levelLowerBound: Math.max(0, normalizeInteger(entry && entry.levelLowerBound, index * 20)),
                    levelUpperBound: Math.max(0, normalizeInteger(entry && entry.levelUpperBound, index * 20 + 19)),
                })),
            },
            anarchy: {
                unitIncreaseTimeSeconds: Math.max(1, normalizeInteger(anarchyDefinition.unitIncreaseTimeSeconds, fallbackDefinition.anarchy.unitIncreaseTimeSeconds)),
                stages: (Array.isArray(anarchyDefinition.stages)
                    ? anarchyDefinition.stages
                    : fallbackDefinition.anarchy.stages).map((entry, index) => ({
                    stage: Math.max(1, normalizeInteger(entry && entry.stage, index + 1)),
                    levelLowerBound: Math.max(0, normalizeInteger(entry && entry.levelLowerBound, index * 20)),
                    levelUpperBound: Math.max(0, normalizeInteger(entry && entry.levelUpperBound, index * 20 + 19)),
                    workforceConsumption: Math.max(0, normalizeInteger(entry && entry.workforceConsumption, index * 5)),
                })),
            },
        },
        simulation: {
            paused: normalizeBoolean(value.simulation && value.simulation.paused, fallbackSimulation.paused),
            started: normalizeBoolean(value.simulation && value.simulation.started, fallbackSimulation.started),
            development: {
                level: Math.max(0, normalizeInteger(developmentSimulation.level, fallbackSimulation.development.level)),
                stage: Math.max(1, normalizeInteger(developmentSimulation.stage, fallbackSimulation.development.stage)),
                pausedAtMs: normalizeTimestampMs(developmentSimulation.pausedAtMs, fallbackSimulation.development.pausedAtMs),
                simulatedAtMs: normalizeTimestampMs(developmentSimulation.simulatedAtMs, fallbackSimulation.development.simulatedAtMs),
            },
            anarchy: {
                level: Math.max(0, normalizeInteger(anarchySimulation.level, fallbackSimulation.anarchy.level)),
                stage: Math.max(1, normalizeInteger(anarchySimulation.stage, fallbackSimulation.anarchy.stage)),
                pausedAtMs: normalizeTimestampMs(anarchySimulation.pausedAtMs, fallbackSimulation.anarchy.pausedAtMs),
                simulatedAtMs: normalizeTimestampMs(anarchySimulation.simulatedAtMs, fallbackSimulation.anarchy.simulatedAtMs),
            },
        },
    };
}
function normalizeInfomorphsRecord(value = {}) {
    ...
    return {
        definition: {
            infomorphTypeID: normalizePositiveInteger(definition.infomorphTypeID, fallbackDefinition.infomorphTypeID),
            generationTickSeconds: Math.max(1, normalizeInteger(definition.generationTickSeconds, fallbackDefinition.generationTickSeconds)),
            cargoCapacity: Math.max(0, normalizeInteger(definition.cargoCapacity, fallbackDefinition.cargoCapacity)),
            generationBands: (Array.isArray(definition.generationBands)
                ? definition.generationBands
                : fallbackDefinition.generationBands).map((entry, index) => ({
                stage: Math.max(1, normalizeInteger(entry && entry.stage, index + 1)),
                lowerBand: Math.max(0, normalizeInteger(entry && entry.lowerBand, index)),
                upperBand: Math.max(0, normalizeInteger(entry && entry.upperBand, index + 1)),
            })),
        },
        contents: {
            count: Math.max(0, normalizeInteger(contents.count, fallbackContents.count)),
            lastGenerationTickMs: normalizeTimestampMs(contents.lastGenerationTickMs, fallbackContents.lastGenerationTickMs),
        },
    };
}
function normalizeActivityTemplateDefinition(value = {}, fallback = {}) {
    return {
        ...
        nameMessageID: normalizePositiveInteger(value.nameMessageID, fallback.nameMessageID || 330000001),
        descriptionMessageID: normalizePositiveInteger(value.descriptionMessageID, fallback.descriptionMessageID || 330000011),
        dungeonID: normalizePositiveInteger(value.dungeonID, fallback.dungeonID || 8800001),
        developmentImpact: normalizeInteger(value.developmentImpact, fallback.developmentImpact || 0),
        anarchyImpact: normalizeInteger(value.anarchyImpact, fallback.anarchyImpact || 0),
        infomorphBonus: Math.max(0, normalizeInteger(value.infomorphBonus, fallback.infomorphBonus || 0)),
    };
}
function normalizeMercenaryActivityRecord(value = {}, fallback = {}) {
    return {
        activityID: String(value.activityID || fallback.activityID || createUuidString())
            .trim()
            .toLowerCase(),
        mercenaryDenID: normalizePositiveInteger(value.mercenaryDenID, fallback.mercenaryDenID),
        ...
        template: normalizeActivityTemplateDefinition(value.template, fallback.template || {}),
    };
}
function buildDefaultActivitiesForDen(den, now = Date.now()) {
    return DEFAULT_ACTIVITY_TEMPLATE_DEFINITIONS.slice(0, DEFAULT_MERCENARY_DEN_ACTIVITY_CAPACITY).map((templateDefinition, index) => normalizeMercenaryActivityRecord({
...
function normalizeMercenaryDenRecord(value = {}) {
    const now = Date.now();
    const mercenaryDenID = normalizePositiveInteger(value.mercenaryDenID || value.denID || value.id, null);
    return {
        mercenaryDenID,
        ...
        skyhookOwnerCorporationID: normalizePositiveInteger(value.skyhookOwnerCorporationID, null),
        evolution: normalizeEvolutionRecord(value.evolution || {}),
        infomorphs: normalizeInfomorphsRecord(value.infomorphs || {}),
        nextGenerationAtMs: normalizeTimestampMs(value.nextGenerationAtMs, now + DEFAULT_ACTIVITY_GENERATION_INTERVAL_MS),
        activities: Array.isArray(value.activities)
            ? value.activities
                .map((entry) => normalizeMercenaryActivityRecord(entry, {
                mercenaryDenID,
...
function getMercenaryDenMaximumForCharacter(characterID) {
    ...
    const skillLevel = getSkillLevel(numericCharacterID, MERCENARY_DEN_MANAGEMENT_SKILL_TYPE_ID);
    return {
...
function listOwnedMercenaryDenIDs(characterID) {
    const current = ensureLoaded();
    return cloneValue(current.denIDsByOwnerCharacterID.get(normalizePositiveInteger(characterID, 0)) || []);
}
...
function listMercenaryDenActivitiesForCharacter(characterID) {
    const current = ensureLoaded();
    return cloneValue(current.activitiesByOwnerCharacterID.get(normalizePositiveInteger(characterID, 0)) || []);
}
...
function startMercenaryDenActivity(characterID, activityID) {
    const current = ensureLoaded();
    const existingActivity = current.activityByID.get(String(activityID || "").trim().toLowerCase());
    if (!existingActivity) {
        return {
            ok: false,
            statusCode: 404,
    ...
    return mutateMercenaryDens((table) => {
        ...
        const activityIndex = den.activities.findIndex((entry) => entry.activityID === existingActivity.activityID);
        if (activityIndex < 0) {
            return {
                ok: false,
                statusCode: 404,
        ...
        const nextActivity = normalizeMercenaryActivityRecord({
            ...
            started: true,
            expiryMs: Math.max(normalizeTimestampMs(den.activities[activityIndex].expiryMs, 0), Date.now() + DEFAULT_ACTIVITY_DURATION_MS),
        }, den.activities[activityIndex]);
...
//# sourceMappingURL=mercenaryDenState.js.map