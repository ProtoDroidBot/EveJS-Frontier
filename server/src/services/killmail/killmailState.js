...
const { buildDict, buildFiletimeLong, buildKeyVal, currentFileTime, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getWarRecord, listAllWarsDescending, } = require(path.join(__dirname, "../corporation/warRuntimeState"));
...
function sortKillmailsDescending(records = []) {
    return records
        .slice()
        .sort((left, right) => normalizeInteger(right && right.killID, 0) -
        normalizeInteger(left && left.killID, 0) ||
        String(right && right.killTime ? right.killTime : "").localeCompare(String(left && left.killTime ? left.killTime : "")));
}
function filterByStartKillID(records = [], startKillID = null) {
    ...
    return records.filter((record) => normalizePositiveInteger(record && record.killID, 0) < numericStartKillID);
}
...
function collectAttackerOwnerIDs(record = {}, fieldName) {
    ...
    const attackerFieldName = fieldName.length > 0
    ...
    for (const attacker of Array.isArray(record && record.attackers) ? record.attackers : []) {
        const ownerID = normalizePositiveInteger(attacker && attacker[attackerFieldName], null);
        if (ownerID) {
...
function buildBlobItemEntry(item = {}, depth = 0) {
    const attributes = [
        ...
        .map(([name, attributeValue]) => ` ${name}="${escapeXmlAttribute(attributeValue)}"`)
        .join("");
    const contents = depth === 0 && Array.isArray(item.contents)
...
function sumKillmailItemValue(items = [], valueSelector = "all") {
    return items.reduce((sum, item) => {
        ...
        const quantity = valueSelector === "destroyed"
            ? qtyDestroyed
        ...
        return (sum +
            basePrice * quantity +
            sumKillmailItemValue(Array.isArray(item && item.contents) ? item.contents : [], valueSelector));
    }, 0);
}
...
function normalizeItemEntry(entry = {}, depth = 0) {
    return {
        ...
        contents: depth < 1 && Array.isArray(entry.contents)
...
function createKillmailRecord(record = {}) {
    ...
    const normalizedRecord = {
        ...
        iskLost: normalizeNumber(record.iskLost, getTypeBasePrice(record.victimShipTypeID) + sumKillmailItemValue(normalizedItems, "all")),
        iskDestroyed: normalizeNumber(record.iskDestroyed, getTypeBasePrice(record.victimShipTypeID) + sumKillmailItemValue(normalizedItems, "destroyed")),
    ...
    normalizedRecord.killBlob = normalizeText(record.killBlob, buildKillBlob({
        attackers: normalizedAttackers,
        items: normalizedItems,
    }));
...
function listKillmailsForCharacter(characterID, options = {}) {
    ...
    return limitRecords(filterByStartKillID(ensureKillmailIndexes().recordsByCharacter.get(numericCharacterID) || [], options.startKillID), options.limit);
}
function listKillmailsForCorporation(corporationID, relation = "all", options = {}) {
    ...
    return limitRecords(filterByStartKillID(filtered, options.startKillID), options.limit);
}
function isWarActiveAtKillTime(war, killTime) {
    ...
    const finishedAt = war.timeFinished !== undefined && war.timeFinished !== null
...
function isAllyActiveAtKillTime(ally = {}, killTime) {
    ...
    const finishedAt = ally.timeFinished !== undefined && ally.timeFinished !== null
...
function resolveKillmailWarID(record = {}) {
    for (const warEntry of listAllWarsDescending()) {
        ...
        if ((victimSide === "attacker" && attackerSides.has("defender")) ||
...
function classifyWarShipGroup(typeID) {
    ...
    if (groupName.includes("control tower") ||
    ...
    if (groupName.includes("freighter") ||
    ...
    if (groupName.includes("destroyer") ||
...
function listKillmailsForWar(warID, options = {}) {
    ...
    const groupFilter = options.groupID === null || options.groupID === undefined
        ? null
...
//# sourceMappingURL=killmailState.js.map