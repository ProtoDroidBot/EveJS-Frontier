...
function resolveStackDropCounts(snapshot, seed, options: Record<string, any> = {}) {
...
function resolveSnapshotNode(snapshot, seed, options: Record<string, any> = {}) {
  const quantity = Math.max(1, toInt(snapshot && snapshot.quantity, 1));
...
function applyResolvedNode(resolution, rootLootLocationID, parentDropped = false) {
  const movedChanges: any[] = [];
  const destroyChanges: any[] = [];
...
function resolveLocationDeathOutcome(locationID, options: Record<string, any> = {}) {
