...
const { ITEM_FLAGS, findItemById, listContainerItems, moveItemToLocation, removeInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
const { resolveInventoryItemQuantity, } = require(path.join(__dirname, "./killmailItemPayload"));
...
function isAlwaysDestroyedFlag(flagID) {
    ...
    return ((numericFlagID >= RIG_FLAG_MIN && numericFlagID <= RIG_FLAG_MAX) ||
...
function moveResolvedQuantity(itemID, destinationLocationID, quantity) {
    ...
    const moveResult = moveItemToLocation(itemID, destinationLocationID, ROOT_LOOT_FLAG_ID, quantity);
...
function applyResolvedNode(resolution, rootLootLocationID, parentDropped = false) {
    ...
    if (!isSingleton) {
        if (toInt(resolution && resolution.qtyDropped, 0) > 0) {
            movedChanges.push(...moveResolvedQuantity(currentItem.itemID, rootLootLocationID, toInt(resolution.qtyDropped, 0)));
        }
    ...
    for (const childResolution of Array.isArray(resolution && resolution.children) ? resolution.children : []) {
        const childResult = applyResolvedNode(childResolution, rootLootLocationID, itemDropped);
...
function resolveLocationDeathOutcome(locationID, options = {}) {
    ...
    const rootLootLocationID = options.forceAllDestroyed === true
        ? null
        : toPositiveInt(options.rootLootLocationID, null);
    const snapshotTree = buildSnapshotTree(locationID);
    const resolvedTree = snapshotTree.map((snapshot) => resolveSnapshotNode(snapshot, seed, options));
    const movedChanges = [];
    const destroyChanges = [];
    for (const resolution of resolvedTree) {
        const applyResult = applyResolvedNode(resolution, rootLootLocationID, false);
...
//# sourceMappingURL=deathOutcomeResolver.js.map