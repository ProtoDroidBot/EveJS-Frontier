...
function sortKillmailsDescending(records: any[] = []) {
  return records
    .slice()
    .sort(
      (left, right) =>
...
function filterByStartKillID(records: any[] = [], startKillID = null) {
...
function limitRecords(records: any[] = [], limit = null) {
...
function collectAttackerOwnerIDs(record: Record<string, any> = {}, fieldName) {
...
function ensureKillmailIndexes() {
  ...
  const records = Object.values<any>((table && table.records) || {});
...
function buildBlobAttackerEntry(attacker: Record<string, any> = {}) {
  const attributes = [
...
function buildBlobItemEntry(item: Record<string, any> = {}, depth = 0) {
  const attributes = [
...
function buildKillBlob({ attackers = [], items = [] }: Record<string, any> = {}) {
...
function sumKillmailItemValue(items: any[] = [], valueSelector = "all") {
...
function normalizeAttackerEntry(entry: Record<string, any> = {}) {
  return {
...
function normalizeItemEntry(entry: Record<string, any> = {}, depth = 0) {
  return {
...
function createKillmailRecord(record: Record<string, any> = {}) {
  const table = ensureKillmailTableInitialized();
...
function listKillmailsForCharacter(characterID, options: Record<string, any> = {}) {
...
function listKillmailsForCorporation(corporationID, relation = "all", options: Record<string, any> = {}) {
...
function isAllyActiveAtKillTime(ally: Record<string, any> = {}, killTime) {
  const killTimeValue = BigInt(normalizeFiletimeString(killTime, currentFileTime().toString()));
...
function buildWarOwnerSets(war, killTime) {
  const attackerOwners = new Set<any>();
  const defenderOwners = new Set<any>();
  ...
  for (const [allyID, ally] of Object.entries<any>((war && war.allies) || {})) {
...
function resolveKillmailWarID(record: Record<string, any> = {}) {
  for (const warEntry of listAllWarsDescending()) {
    ...
    const attackerSides = new Set<any>();
...
function listKillmailsForWar(warID, options: Record<string, any> = {}) {
...
function buildWarDestructionStatistics(warID) {
  ...
  const shipsKilled: Record<string, any> = {};
  const iskKilled: Record<string, any> = {};
  const killsByGroup: Record<string, any> = {};
  for (const bucket of [
    GROUP_CAPSULES,
    GROUP_FRIGATES,
    GROUP_DESTROYERS,
