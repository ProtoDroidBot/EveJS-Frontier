...
function buildKillmailItemTreeFromInventoryItem(item, options = {}) {
    const quantityField = options.quantityField === "qtyDestroyed" ? "qtyDestroyed" : "qtyDropped";
    ...
    const contents = currentDepth < maxDepth
        ? listContainerItems(null, toPositiveInt(item && item.itemID, 0), null).map((child) => buildKillmailItemTreeFromInventoryItem(child, nextOptions))
        : [];
...
function buildKillmailItemTreeForLocation(locationID, options = {}) {
    ...
    return listContainerItems(null, numericLocationID, null).map((item) => buildKillmailItemTreeFromInventoryItem(item, options));
}
module.exports = {
...
//# sourceMappingURL=killmailItemPayload.js.map