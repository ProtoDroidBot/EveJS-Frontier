...
function buildMethodCacheKey({ serviceName = "marketProxy", method, args = [], sessionInfoValue = undefined, }) {
    const key = [
        buildRawString(serviceName || "marketProxy"),
        buildRawString(method || ""),
    ];
    if (sessionInfoValue !== undefined && sessionInfoValue !== null) {
        key.push(typeof sessionInfoValue === "string"
            ? buildRawString(sessionInfoValue)
            : sessionInfoValue);
    }
    if (Array.isArray(args)) {
        key.push(...args.map((entry) => typeof entry === "string" ? buildRawString(entry) : entry));
    }
    return key;
}
...
function getCachedMethodRecordKey({ serviceName, method, args = [], sessionInfoValue = undefined, }) {
...
function buildCachedObjectResponse(record, compatibilityProfile = config.clientCompatibilityProfile) {
    return {
        type: "object",
...
function buildCachedMethodCallResult(result, options = {}) {
    ...
    if (proxyCache) {
        ...
        const objectVersion = [
            buildSignedLong(currentFileTime()),
            computeCrcHqx(compressedRecord.pickle, (Number(config.machoVersion) || 0) +
...
function invalidateCachedMethodCall({ serviceName, method, args = [], session = null, } = {}) {
    const details = methodCallCachingDetails.get(buildMethodDetailsKey(serviceName, method)) || null;
    let deleted = 0;
    const normalizedArgs = normalizeArgsArray(args);
...
function isCachedMethodCallResult(value) {
    ...
    return CACHED_METHOD_CALL_RESULT_NAMES.has(normalizeText(normalizeCacheIdentity(value.name), ""));
}
...
function getCachableObjectResponse(shared, objectId, objectVersion, nodeId, compatibilityProfile = config.clientCompatibilityProfile) {
...
//# sourceMappingURL=objectCacheRuntime.js.map