...
const { getCachedMethodCallVersion, getCachableObjectResponse, getCachedObject, getCachedObjectVersion, invalidateCachedMethodCall, invalidateCachedMethodCalls, invalidateCachedObjects, __testHooks, } = require(path.join(__dirname, "./objectCacheRuntime"));
...
class ObjCacheService extends BaseService {
    ...
    Handle_GetCachableObject(args, session) {
        ...
        log.debug(`[ObjCache] GetCachableObject ${__testHooks.describeObjectId(objectId)}`);
        return getCachableObjectResponse(shared, objectId, objectVersion, nodeId, session && session.compatibilityProfile);
    }
...
//# sourceMappingURL=objCacheService.js.map