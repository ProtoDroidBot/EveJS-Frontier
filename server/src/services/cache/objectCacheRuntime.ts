...
function buildCacheDetails({ versionCheck = "run", sessionInfo = null }: Record<string, any> = {}) {
...
function buildDetailsRecord({ versionCheck = "run", sessionInfo = null }: Record<string, any> = {}) {
  return {
...
function buildCachedMethodCallResult(result, options: Record<string, any> = {}) {
  const {
    serviceName = "marketProxy",
    method,
    args = [],
...
async function buildCachedMethodCallResultAsync(result, options: Record<string, any> = {}) {
  const {
    serviceName = "marketProxy",
    method,
    args = [],
...
function invalidateCachedMethodCalls(methodCalls: any[] = [], session = null) {
  let deleted = 0;
...
function invalidateCachedObjects(objectIds: any[] = []) {
  let deleted = 0;
