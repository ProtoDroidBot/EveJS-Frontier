...
function rebuildIndexes(state) {
  ...
  for (const [shipID, rawContract] of Object.entries<any>(state.contractsByShipID || {})) {
    ...
    if (!activeContractIDsByOwnerID.has(contract.ownerID)) {
      activeContractIDsByOwnerID.set(contract.ownerID, new Set<any>());
    }
...
function getActiveContractForShip(shipID, options: Record<string, any> = {}) {
  const state = ensureContractState();
...
function resolveOwnerKind(ownerID, context: Record<string, any> = {}) {
...
function resolveContractRecipientCharacterID(contract, context: Record<string, any> = {}) {
  return (
...
function insureShip(session, options: Record<string, any> = {}) {
  if (!isInsuranceServiceAvailable(session)) {
...
function notifyVoidedContract(contract, reason, options: Record<string, any> = {}) {
...
function voidInsuranceForShip(shipID, reason: string = CONTRACT_VOID_REASON.MANUAL, options: Record<string, any> = {}) {
  const state = ensureContractState();
  const contract = activeContractByShipID.get(toPositiveInt(shipID, 0)) || null;
  if (!contract) {
    return {
...
function isConcordLossContext(context: Record<string, any> = {}) {
  const reason = String(
...
function handleShipDestroyed(context: Record<string, any> = {}) {
...
function handleInventoryMutation(context: Record<string, any> = {}) {
...
function handleShipRepackaged(shipID, context: Record<string, any> = {}) {
  return voidInsuranceForShip(shipID, CONTRACT_VOID_REASON.REPACKAGED, {
    notify: true,
    ...context,
  });
