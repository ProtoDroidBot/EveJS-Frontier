...
const { ITEM_FLAGS, SHIP_CATEGORY_ID, findShipItemById, getCharacterHangarShipItems, listContainerItems, listOwnedItems, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const { getCorporationOffices, getCorporationOfficeByInventoryID, } = require(path.join(__dirname, "../corporation/corporationRuntimeState"));
...
const { buildFiletimeLong, buildKeyVal, currentFileTime, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { BASE_INSURANCE_FRACTION, DEFAULT_INSURANCE_FRACTION, centsToIsk, computePayoutCents, computePremiumCents, getFullInsurancePrice, getFullInsurancePriceCents, getInsurancePrices, getPackageByFraction, resolvePackageFromPremium, } = require(path.join(__dirname, "./insurancePriceAuthority"));
const { INSURANCE_INVALID_REASON, PEND_INSURANCE_CORPORATION_ID, notifyInsuranceExpiration, notifyInsuranceInvalidated, notifyInsuranceIssued, notifyInsurancePayout, } = require(path.join(__dirname, "./insuranceNotifications"));
...
function normalizeContract(rawContract) {
    ...
    const fullInsurancePriceCents = Math.max(0, Math.round(Number(rawContract.fullInsurancePriceCents || 0)));
    ...
    const normalized = {
        ...
        typeID,
        ownerID,
        ownerKind: rawContract.ownerKind === OWNER_KIND.CORPORATION
        ...
        fraction,
        packageName: rawContract.packageName ||
            (packageInfo ? packageInfo.name : null),
        premiumCents: Math.max(0, Math.round(Number(rawContract.premiumCents || 0))),
        payoutCents: Math.max(0, Math.round(Number(rawContract.payoutCents || 0))) ||
        ...
        status: rawContract.status === CONTRACT_STATUS.PAID ||
...
function ensureContractState() {
    ...
    const state = result.success && result.data && typeof result.data === "object"
        ? result.data
        : createDefaultState();
    let changed = false;
...
function hasCorpInsuranceRole(session) {
    ...
    return ((roleMask & CORP_ROLE_ACCOUNTANT) !== 0n ||
        (roleMask & CORP_ROLE_JUNIOR_ACCOUNTANT) !== 0n);
}
function getSessionDockContext(session) {
    const stationID = toPositiveInt(session && (session.stationid || session.stationID), 0);
    const structureID = toPositiveInt(session && (session.structureid || session.structureID), 0);
    const dockableLocationID = stationID || structureID;
    const solarSystemID = toPositiveInt(session && (session.solarsystemid2 || session.solarsystemid || session.systemID), 0);
    return {
        stationID,
        structureID,
        dockableLocationID,
...
function isInsuranceServiceAvailable(session) {
    ...
    if (dockContext.structureID) {
        ...
        return characterHasStructureService(session, structure, STRUCTURE_SERVICE_ID.INSURANCE);
    }
    const { buildStationServiceMask, } = require(path.join(__dirname, "../_shared/stationStaticData"));
    return ((buildStationServiceMask(session, dockContext.stationID) &
...
function getCorpOfficeLocationIDsAtDockable(corporationID, dockableLocationID) {
    return new Set(getCorporationOffices(corporationID)
...
function listItemsToInsure(session) {
    ...
    if (corporationID && hasCorpInsuranceRole(session)) {
        const officeLocationIDs = getCorpOfficeLocationIDsAtDockable(corporationID, dockContext.dockableLocationID);
...
function canSessionSeeContract(session, contract) {
    ...
    if (contract.ownerKind === OWNER_KIND.CORPORATION) {
        return (contract.ownerID === getSessionCorporationID(session) &&
...
function creditInsurancePayout(ownerKind, ownerID, accountKey, amountCents, typeID) {
    ...
    if (ownerKind === OWNER_KIND.CORPORATION) {
        return adjustCorporationWalletDivisionBalance(ownerID, normalizeCorporationWalletKey(accountKey), amount, {
            description,
            entryTypeID: JOURNAL_ENTRY_TYPE.INSURANCE,
...
function resolveContractRecipientCharacterID(contract, context = {}) {
    return (toPositiveInt(contract && contract.insuredByCharacterID, 0) ||
...
function insureShip(session, options = {}) {
    ...
    const contract = {
        ...
        corpAccountKey: normalizeCorporationWalletKey(options.corpAccountKey ||
    ...
    if (!writeResult || !writeResult.success) {
        creditInsurancePayout(contract.ownerKind, contract.ownerID, contract.corpAccountKey, contract.premiumCents, contract.typeID);
        throwWrappedUserError("InsCouldNotFindItem");
    }
...
function notifyVoidedContract(contract, reason, options = {}) {
    ...
    const notificationReason = reason === CONTRACT_VOID_REASON.NO_VALUE ||
...
function isConcordLossContext(context = {}) {
    const reason = String(context.insuranceSuppressedReason ||
    ...
    const attackerNpcType = String(attackerEntity && (attackerEntity.npcEntityType || attackerEntity.entityType) || "").trim().toLowerCase();
    return attackerNpcType === ENTITY_TYPE.CONCORD;
}
...
function handleShipDestroyed(context = {}) {
    ...
    const accountKey = activeContract && activeContract.ownerKind === OWNER_KIND.CORPORATION
        ? activeContract.corpAccountKey
        : ACCOUNT_KEY.CASH;
    const creditResult = creditInsurancePayout(ownerResolution.ownerKind, ownerResolution.ownerID, accountKey, payoutCents, typeID);
    ...
    if (recipientCharacterID) {
        notifyInsurancePayout(recipientCharacterID, centsToIsk(payoutCents), {
            itemID: shipID,
            isDefaultPayout: !activeContract,
        });
    }
    return {
        success: true,
...
function handleInventoryMutation(context = {}) {
    ...
    const shipID = toPositiveInt(previousItem && previousItem.itemID || nextItem && nextItem.itemID, 0);
    if (!shipID) {
        return {
            success: true,
            changed: false,
    ...
    if (contract.ownerKind === OWNER_KIND.CHARACTER &&
        nextItem &&
...
//# sourceMappingURL=insuranceRuntime.js.map