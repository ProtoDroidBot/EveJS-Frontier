...
function notifyInsuranceIssued(characterID, contract, options = {}) {
    const itemID = toPositiveInt((contract && (contract.shipID || contract.itemID)) ||
        options.itemID ||
        options.shipID, 0);
    return createInsuranceNotification(characterID, NOTIFICATION_TYPE.INSURANCE_ISSUED, {
...
function notifyInsurancePayout(characterID, payoutAmount, options = {}) {
    ...
    const amount = Number.isFinite(numericPayoutAmount)
        ? Math.max(0, numericPayoutAmount)
        : 0;
    const isDefaultPayout = options.isDefaultPayout === true;
    return createInsuranceNotification(characterID, NOTIFICATION_TYPE.INSURANCE_PAYOUT, {
        ...(itemID > 0 ? { itemID } : {}),
        amount,
        payout: isDefaultPayout,
    }, options);
}
function notifyInsuranceInvalidated(characterID, reason, data = {}, options = {}) {
    return createInsuranceNotification(characterID, NOTIFICATION_TYPE.INSURANCE_INVALIDATED, {
        ...data,
...
function notifyInsuranceExpiration(characterID, data = {}, options = {}) {
    return createInsuranceNotification(characterID, NOTIFICATION_TYPE.INSURANCE_EXPIRATION, data, options);
}
module.exports = {
...
//# sourceMappingURL=insuranceNotifications.js.map