...
function computePremiumCents(fullInsurancePriceCents, fraction) {
    ...
    const increment = (normalizedFraction - BASE_INSURANCE_FRACTION) * INSURANCE_FRACTION_INCREMENT;
...
function normalizePriceRow(row = {}) {
    ...
    const fullInsurancePriceCents = Math.max(0, Math.round(toNumber(row.fullInsurancePriceCents, 0)));
...
function getPackageByFraction(fraction) {
    const normalizedFraction = roundFraction(fraction);
    return (PACKAGE_ORDER.find((packageInfo) => packageInfo.fraction === normalizedFraction) ||
        null);
}
function getPackageByName(name) {
    const normalizedName = String(name || "").trim().toLowerCase();
    return (PACKAGE_ORDER.find((packageInfo) => packageInfo.name.toLowerCase() === normalizedName) || null);
}
function resolvePackageFromPremium(fullInsurancePriceCents, quotedPremium) {
    const premiumCents = iskToCents(quotedPremium);
    return (PACKAGE_ORDER.find((packageInfo) => (Math.abs(computePremiumCents(fullInsurancePriceCents, packageInfo.fraction) -
        premiumCents) <= 1)) || null);
}
module.exports = {
...
//# sourceMappingURL=insurancePriceAuthority.js.map