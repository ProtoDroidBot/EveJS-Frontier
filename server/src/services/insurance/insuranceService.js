...
const { buildBoundObjectResponse, buildDict, buildList, extractList, marshalObjectToObject, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
class InsuranceService extends BaseService {
    ...
    Handle_GetItemsToInsure(args, session) {
        ...
        return buildList(insuranceRuntime
    ...
    Handle_InsureShip(args, session, kwargs) {
        ...
        log.debug(`[InsuranceSvc] InsureShip ship=${itemID} premium=${quotedPremium} corp=${isCorpItem ? 1 : 0} voidOld=${kw.voidOld ? 1 : 0}`);
