...
function normalizeRequirementPair(entry) {
    ...
    const typeID = normalizePositiveInteger(firstDefined(entry.typeID, entry.type_id), 0);
    const quantity = normalizeNonNegativeInteger(firstDefined(entry.quantity, entry.qty), 0);
    return typeID > 0 && quantity > 0 ? [typeID, quantity] : null;
}
...
function buildRequirementPairs(requirements = []) {
    return buildList(normalizeRequirementPairs(requirements).map(([typeID, quantity]) => buildList([typeID, quantity])));
}
function normalizeOfferRecord(offer, fallbackCorpID = 0) {
    const safeOffer = offer && typeof offer === "object" ? offer : {};
    const corpID = normalizePositiveInteger(firstDefined(safeOffer.corpID, safeOffer.corporation_id, fallbackCorpID), fallbackCorpID);
    const requiredStandings = firstDefined(safeOffer.requiredStandings, safeOffer.required_standings, null);
    return {
        typeID: normalizePositiveInteger(firstDefined(safeOffer.typeID, safeOffer.type_id), 0),
        iskCost: normalizeNonNegativeInteger(firstDefined(safeOffer.iskCost, safeOffer.isk_cost), 0),
        akCost: normalizeNonNegativeInteger(firstDefined(safeOffer.akCost, safeOffer.ak_cost), 0),
        reqItems: normalizeRequirementPairs(firstDefined(safeOffer.reqItems, safeOffer.required_items, [])),
        offerID: normalizePositiveInteger(firstDefined(safeOffer.offerID, safeOffer.offer_id), 0),
        qty: normalizePositiveInteger(firstDefined(safeOffer.qty, safeOffer.quantity), 1),
        requiredStandings,
        corpID,
        lootItems: normalizeRequirementPairs(firstDefined(safeOffer.lootItems, safeOffer.loot_items, [])),
        lpCost: normalizeNonNegativeInteger(firstDefined(safeOffer.lpCost, safeOffer.lp_cost), 0),
    };
}
...
function listStaticLpStoreOffers(corpID) {
    const normalizedCorpID = normalizePositiveInteger(corpID, 0);
    return getStaticOfferRows(normalizedCorpID).map((offer) => buildLpStoreOfferKeyVal(offer, normalizedCorpID));
}
function getStaticLpStoreOfferRecord(corpID, offerID) {
    ...
    const offer = getStaticOfferRows(normalizedCorpID).find((candidate) => normalizePositiveInteger(firstDefined(candidate.offerID, candidate.offer_id), 0) === normalizedOfferID);
...
//# sourceMappingURL=lpStoreOfferCatalog.js.map