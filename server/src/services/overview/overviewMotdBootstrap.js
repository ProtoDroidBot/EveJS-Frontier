...
const REPO_DEFAULT_OVERVIEW_PROFILE_PATH = path.join(__dirname, "../../../assets/overview/iridium_overview_20260410-v381_carbon.yaml");
const DEFAULT_OVERVIEW_PROFILE_PATH = REPO_DEFAULT_OVERVIEW_PROFILE_PATH;
const DEFAULT_OVERVIEW_CHANNEL = process.env.EVEJS_SHARED_OVERVIEW_CHANNEL ||
    "player_900001";
const DEFAULT_OVERVIEW_LINK_LABEL = process.env.EVEJS_SHARED_OVERVIEW_LABEL ||
    "Install Iridium Overview v381 Carbon";
const DEFAULT_OVERVIEW_SECTION_LABEL = process.env.EVEJS_SHARED_OVERVIEW_SECTION_LABEL ||
    "Shared Overview";
...
function stripManagedOverviewSection(motd) {
    return String(motd || "")
        .replace(/(?:<br>\s*){0,2}<b>[^<]*:?<\/b>\s*<a href="overviewPreset:[^"]+">[^<]*<\/a>/gi, "")
        .trim();
}
...
function installSharedOverviewMotd(serviceManager, options = {}) {
    const overviewService = serviceManager && typeof serviceManager.lookup === "function"
    ...
    const profilePath = String(options.profilePath || DEFAULT_OVERVIEW_PROFILE_PATH).trim();
    if (!profilePath || !fs.existsSync(profilePath)) {
        log.debug(`[OverviewPresetMgr] Shared overview MOTD bootstrap skipped; file not found at ${profilePath || "<empty>"}`);
        return null;
    }
    const channelRoomName = String(options.channelRoomName || DEFAULT_OVERVIEW_CHANNEL).trim();
    if (!channelRoomName) {
    ...
    if (!rawYaml.trim()) {
        log.warn(`[OverviewPresetMgr] Shared overview MOTD bootstrap skipped; file is empty at ${profilePath}`);
        return null;
    }
    const linkLabel = String(options.linkLabel || DEFAULT_OVERVIEW_LINK_LABEL).trim() || DEFAULT_OVERVIEW_LINK_LABEL;
    const sectionLabel = String(options.sectionLabel || DEFAULT_OVERVIEW_SECTION_LABEL).trim() || DEFAULT_OVERVIEW_SECTION_LABEL;
    const channelRecord = chatRuntime.ensureChannel(channelRoomName);
    if (!channelRecord) {
        log.warn(`[OverviewPresetMgr] Shared overview MOTD bootstrap skipped; channel ${channelRoomName} could not be ensured`);
        return null;
    }
    ...
    const motd = buildOverviewMotd(channelRecord.motd, linkUrl, linkLabel, sectionLabel);
    ...
    log.info(`[OverviewPresetMgr] Shared overview MOTD ready for ${channelRoomName} using ${profilePath}`);
    return {
        channelRoomName,
...
//# sourceMappingURL=overviewMotdBootstrap.js.map