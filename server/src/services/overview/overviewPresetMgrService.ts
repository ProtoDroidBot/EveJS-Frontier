...
function buildDeterministicValueFromYaml(value, activeObjects = new Set<any>()) {
...
function ensureStateShape(state) {
  ...
  for (const [hashvalue, sqIDs] of Object.entries<any>(nextState.hashIndex)) {
  ...
  const repairedEntries: Record<string, any> = {};
  let repairedEntryCount = 0;
  let highestSqID = 0;
  for (const rawEntry of Object.values<any>(nextState.entries || {})) {
...
class OverviewPresetMgrService extends BaseService {
  ...
  _findExistingEntryByPayloadString(payloadString) {
    const state = this._getState();
    for (const entry of Object.values<any>(state.entries || {})) {
      if (entry && entry.payload === payloadString) {
        return entry;
      }
    }
  ...
  _storePayloadString(payloadString, options: Record<string, any> = {}) {
    const normalizedPayloadString = normalizeText(payloadString, "");
    if (!normalizedPayloadString.trim()) {
      return null;
    }
  ...
  storeOverviewPayload(payload, options: Record<string, any> = {}) {
  ...
  storeRawPresetString(payloadString, options: Record<string, any> = {}) {
  ...
  Handle_StoreLinkAndGetID(args: any[] = [], session) {
  ...
  Handle_GetStoredPreset(args: any[] = []) {
    const state = this._getState();
