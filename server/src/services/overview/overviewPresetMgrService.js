...
const { buildKeyVal, normalizeNumber, normalizeText, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
function repairLegacyPayloadString(payloadString) {
    ...
    const parsedObject = Object.fromEntries(parsedPayload.filter((entry) => Array.isArray(entry) && entry.length >= 2));
    ...
    const repairedPayloadString = buildPayloadStringFromOverviewPayload(repairedPayload);
...
function repairStoredPayloadString(payloadString) {
    ...
    return (repairLegacyPayloadString(normalizedPayloadString) ||
...
function ensureStateShape(state) {
    ...
    for (const rawEntry of Object.values(nextState.entries || {})) {
        ...
        const repairedPayloadString = repairStoredPayloadString(originalPayloadString) || originalPayloadString;
        const repairedHashvalue = normalizeText(rawEntry.hashvalue, "") || buildHashValue(repairedPayloadString);
...
class OverviewPresetMgrService extends BaseService {
    ...
    _storePayloadString(payloadString, options = {}) {
        ...
        const existingEntry = this._findExistingEntry(hashvalue, normalizedPayloadString) ||
        ...
        if (existingEntry) {
            ...
            const nextSourcePath = normalizeText(options.sourcePath, existingEntry.sourcePath);
            if (nextSource !== existingEntry.source ||
        ...
        this._persistState();
        log.info(`[OverviewPresetMgr] Stored shared overview profile hash=${hashvalue} sqID=${sqID}${entry.source ? ` source=${entry.source}` : ""}`);
        return entry;
    }
    ...
    storeRawPresetString(payloadString, options = {}) {
        return this._storePayloadString(repairExportedOverviewYamlPayloadString(payloadString) || payloadString, options);
    }
...
//# sourceMappingURL=overviewPresetMgrService.js.map