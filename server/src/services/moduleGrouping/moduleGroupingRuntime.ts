...
function getShipGroupableModules(shipID, options: Record<string, any> = {}) {
...
function getMasterModuleID(shipID, moduleID, banks = null) {
  ...
  for (const [masterID, slaveIDs] of Object.entries<any>(currentBanks || {})) {
...
function collectBankModuleIDs(shipID, moduleIDs: any[] = [], banks = null) {
  const currentBanks = banks || getShipWeaponBanks(shipID);
  const collectedIDs = new Set<any>();
...
function validateOnlineModules(shipID, moduleIDs: any[] = [], banks = null) {
...
function notifyWeaponBanksChanged(session, shipID, banks = null, options: Record<string, any> = {}) {
...
function buildOperationResult(result, fallbackBanks: Record<string, any> = {}) {
...
function linkWeapons(shipID, masterModuleID, slaveModuleID, options: Record<string, any> = {}) {
...
function mergeModuleGroups(shipID, targetMasterID, sourceMasterID, options: Record<string, any> = {}) {
  const numericShipID = toNumber(shipID, 0);
  const currentBanks = getShipWeaponBanks(numericShipID, options);
...
function peelAndLink(shipID, targetMasterID, sourceMasterID, options: Record<string, any> = {}) {
  const numericShipID = toNumber(shipID, 0);
  const currentBanks = getShipWeaponBanks(numericShipID, options);
  const resolvedTargetMasterID =
    getMasterModuleID(numericShipID, targetMasterID, currentBanks) ||
...
function unlinkModuleFromBank(shipID, masterModuleID, options: Record<string, any> = {}) {
  const numericShipID = toNumber(shipID, 0);
  const currentBanks = getShipWeaponBanks(numericShipID, options);
  const resolvedMasterID =
    getMasterModuleID(numericShipID, masterModuleID, currentBanks) ||
...
function destroyWeaponBank(shipID, masterModuleID, options: Record<string, any> = {}) {
  const numericShipID = toNumber(shipID, 0);
  const currentBanks = getShipWeaponBanks(numericShipID, options);
  const resolvedMasterID =
    getMasterModuleID(numericShipID, masterModuleID, currentBanks) ||
...
function linkAllWeapons(shipID, options: Record<string, any> = {}) {
  const numericShipID = toNumber(shipID, 0);
  const currentBanks = getShipWeaponBanks(numericShipID, options);
  ...
  const nextBanks: Record<string, any> = {};
...
function unlinkAllWeaponBanks(shipID, options: Record<string, any> = {}) {
