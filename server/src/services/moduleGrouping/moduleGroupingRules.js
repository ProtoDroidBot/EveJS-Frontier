...
const { isShipFittingFlag, isModuleOnline, getLoadedChargeByFlag, } = require(path.join(__dirname, "../fitting/liveFittingState"));
...
function compareModuleSortOrder(leftItem, rightItem, leftFallbackItemID = 0, rightFallbackItemID = 0) {
    const leftFlag = toNumber(leftItem && leftItem.flagID, 0);
    const rightFlag = toNumber(rightItem && rightItem.flagID, 0);
    if (leftFlag !== rightFlag) {
        return leftFlag - rightFlag;
    }
    const leftItemID = toNumber(leftItem && leftItem.itemID, leftFallbackItemID);
    const rightItemID = toNumber(rightItem && rightItem.itemID, rightFallbackItemID);
...
function getModuleLoadedChargeSignature(shipID, moduleItem) {
    ...
    const loadedCharge = getLoadedChargeByFlag(numericOwnerID, numericShipID, numericFlagID);
    const quantity = Math.max(0, toNumber(loadedCharge && (loadedCharge.stacksize ?? loadedCharge.quantity), 0));
    return {
...
function canModulesGroupTogether(shipID, masterItem, slaveItem, options = {}) {
    ...
    if (toNumber(masterItem.locationID, 0) !== numericShipID ||
    ...
    const masterChargeSignature = getModuleLoadedChargeSignature(numericShipID, masterItem);
    const slaveChargeSignature = getModuleLoadedChargeSignature(numericShipID, slaveItem);
    if (masterChargeSignature.chargeTypeID !== slaveChargeSignature.chargeTypeID ||
...
//# sourceMappingURL=moduleGroupingRules.js.map