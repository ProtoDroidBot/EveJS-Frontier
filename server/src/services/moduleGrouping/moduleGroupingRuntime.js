...
const { findItemById, listContainerItems, } = require(path.join(__dirname, "../inventory/itemStore"));
const { compareModuleSortOrder, isGroupableModuleItem, canModulesGroupTogether, buildModuleGroupingCompatibilityKey, } = require(path.join(__dirname, "./moduleGroupingRules"));
const { getShipWeaponBanks, getCharacterWeaponBanks, setShipWeaponBanks, clearModuleFromBanks, } = require(path.join(__dirname, "./moduleGroupingState"));
const { buildWeaponBankStateDict, } = require(path.join(__dirname, "./moduleGroupingBootstrap"));
...
function sortModuleIDs(shipID, moduleIDs) {
    ...
    return [...new Set((Array.isArray(moduleIDs) ? moduleIDs : [])
            ...
            .filter((moduleID) => moduleID > 0))].sort((leftID, rightID) => compareModuleSortOrder(moduleIndex.get(leftID), moduleIndex.get(rightID), leftID, rightID));
}
...
function linkWeapons(shipID, masterModuleID, slaveModuleID, options = {}) {
    ...
    const resolvedMasterID = getMasterModuleID(numericShipID, masterModuleID, currentBanks) ||
    ...
    const onlineValidation = validateOnlineModules(numericShipID, [resolvedMasterID, numericSlaveID], currentBanks);
    if (!onlineValidation.success) {
        return buildOperationResult(onlineValidation, currentBanks);
    }
    const validation = canModulesGroupTogether(numericShipID, masterItem, slaveItem, { requireOnline: true });
    if (!validation.success) {
        return buildOperationResult({ success: false, errorMsg: validation.errorMsg }, currentBanks);
    }
...
function mergeModuleGroups(shipID, targetMasterID, sourceMasterID, options = {}) {
    const numericShipID = toNumber(shipID, 0);
    const currentBanks = getShipWeaponBanks(numericShipID, options);
    const resolvedTargetMasterID = getMasterModuleID(numericShipID, targetMasterID, currentBanks) ||
        toNumber(targetMasterID, 0);
    const resolvedSourceMasterID = getMasterModuleID(numericShipID, sourceMasterID, currentBanks) ||
        toNumber(sourceMasterID, 0);
    if (resolvedTargetMasterID <= 0 ||
    ...
    const onlineValidation = validateOnlineModules(numericShipID, [resolvedTargetMasterID, resolvedSourceMasterID], currentBanks);
    if (!onlineValidation.success) {
        return buildOperationResult(onlineValidation, currentBanks);
    }
    const validation = canModulesGroupTogether(numericShipID, targetItem, sourceMasterItem, { requireOnline: true });
    if (!validation.success) {
        return buildOperationResult({ success: false, errorMsg: validation.errorMsg }, currentBanks);
    }
    const nextBanks = cloneValue(currentBanks);
...
function peelAndLink(shipID, targetMasterID, sourceMasterID, options = {}) {
    ...
    const targetItem = findItemById(resolvedTargetMasterID);
    const peeledModuleID = sortModuleIDs(numericShipID, sourceSlaveIDs)[0];
    const peeledItem = findItemById(peeledModuleID);
    const onlineValidation = validateOnlineModules(numericShipID, [resolvedTargetMasterID, resolvedSourceMasterID], currentBanks);
    if (!onlineValidation.success) {
        return buildOperationResult(onlineValidation, currentBanks);
    }
    const validation = canModulesGroupTogether(numericShipID, targetItem, peeledItem, { requireOnline: true });
    if (!validation.success) {
        return buildOperationResult({ success: false, errorMsg: validation.errorMsg }, currentBanks);
    }
    const nextBanks = cloneValue(currentBanks);
    ...
    const remainingSourceSlaveIDs = getBankEntry(currentBanks, resolvedSourceMasterID).filter((moduleID) => moduleID !== peeledModuleID);
...
function unlinkModuleFromBank(shipID, masterModuleID, options = {}) {
    ...
    const resolvedMasterID = getMasterModuleID(numericShipID, masterModuleID, currentBanks) ||
        toNumber(masterModuleID, 0);
    const sourceSlaveIDs = sortModuleIDs(numericShipID, getBankEntry(currentBanks, resolvedMasterID));
...
function linkAllWeapons(shipID, options = {}) {
    ...
    for (const moduleItem of groupableModules) {
        const compatibilityKey = buildModuleGroupingCompatibilityKey(numericShipID, moduleItem);
        if (!compatibilityKey) {
            continue;
        }
    ...
    for (const modules of groupedByCompatibilityKey.values()) {
        ...
        const existingMaster = modules.find((moduleItem) => (Array.isArray(currentBanks[String(toNumber(moduleItem && moduleItem.itemID, 0))])));
...
function clearModuleFromBanksAndNotify(session, shipID, moduleIDs, options = {}) {
...
function destroyWeaponBankAndNotify(session, shipID, masterModuleID, options = {}) {
...
//# sourceMappingURL=moduleGroupingRuntime.js.map