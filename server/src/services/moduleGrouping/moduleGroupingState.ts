...
function normalizeBankMap(rawValue) {
  ...
  for (const [masterID, rawSlaveIDs] of Object.entries<any>(rawValue)) {
...
function sortModuleIDsByShip(moduleIndex, moduleIDs: any[] = []) {
  return [...new Set(
...
function areBanksEqual(leftBanks: Record<string, any> = {}, rightBanks: Record<string, any> = {}) {
  return JSON.stringify(leftBanks) === JSON.stringify(rightBanks);
}

function sanitizeShipWeaponBanks(shipID, rawBanks: Record<string, any> = {}) {
  const numericShipID = toNumber(shipID, 0);
  if (numericShipID <= 0) {
    return {};
  }
  ...
  const consumedModuleIDs = new Set<any>();
...
function getShipWeaponBanks(shipID, options: Record<string, any> = {}) {
  const numericShipID = toNumber(shipID, 0);
  if (numericShipID <= 0) {
    return {};
  }
...
function setShipWeaponBanks(shipID, nextBanks, options: Record<string, any> = {}) {
  const numericShipID = toNumber(shipID, 0);
  if (numericShipID <= 0) {
    return {
...
function mutateShipWeaponBanks(shipID, mutator, options: Record<string, any> = {}) {
...
function clearModuleFromBanks(shipID, moduleIDs, options: Record<string, any> = {}) {
