...
const { getItemMutationVersion, listContainerItems, } = require(path.join(__dirname, "../inventory/itemStore"));
const { isShipFittingFlag, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const { compareModuleSortOrder, isGroupableModuleItem, canModulesGroupTogether, } = require(path.join(__dirname, "./moduleGroupingRules"));
...
function normalizeRoot(rawValue) {
    return {
        ...
        ships: rawValue && rawValue.ships && typeof rawValue.ships === "object"
...
function ensureShipEntry(root, shipID) {
    ...
    if (!root.ships[shipKey].banksByMasterID ||
...
function sanitizeShipWeaponBanks(shipID, rawBanks = {}) {
    ...
    const sortedMasterIDs = sortModuleIDsByShip(byItemID, Object.keys(normalizedBanks).map((masterID) => toNumber(masterID, 0)));
    for (const masterID of sortedMasterIDs) {
        ...
        const nextSlaveIDs = [];
        const candidateSlaveIDs = sortModuleIDsByShip(byItemID, normalizedBanks[String(masterID)]);
        for (const slaveID of candidateSlaveIDs) {
            ...
            const validation = canModulesGroupTogether(numericShipID, masterItem, slaveItem, {});
...
function getCharacterWeaponBanks(characterID, shipID = null) {
    ...
    const resolvedShipID = toNumber(shipID, 0) ||
...
function mutateShipWeaponBanks(shipID, mutator, options = {}) {
    ...
    const nextBanks = normalizeBankMap(typeof mutator === "function" ? mutator(cloneValue(currentBanks)) : mutator);
    return setShipWeaponBanks(shipID, nextBanks, options);
}
function clearModuleFromBanks(shipID, moduleIDs, options = {}) {
    const targets = new Set((Array.isArray(moduleIDs) ? moduleIDs : [moduleIDs])
        .map((moduleID) => toNumber(moduleID, 0))
        .filter((moduleID) => moduleID > 0));
    return mutateShipWeaponBanks(shipID, (currentBanks) => {
        const nextBanks = {};
...
//# sourceMappingURL=moduleGroupingState.js.map