...
const { findShipItemById } = require(path.join(__dirname, "../inventory/itemStore"));
const { TABLE, readStaticRows, } = require(path.join(__dirname, "../_shared/referenceData"));
const { buildKeyVal, buildList } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getStationRecord } = require(path.join(__dirname, "../_shared/stationStaticData"));
const { getStaticOwnerRecord } = require(path.join(__dirname, "../_shared/stationStaticData"));
const { getAllianceShortNameRecord, getCorporationRecord, getCorporationStationSolarSystems, getOwnerLookupRecord, } = require(path.join(__dirname, "../corporation/corporationState"));
const structureState = require(path.join(__dirname, "../structure/structureState"));
const { getAgentByID, } = require(path.join(__dirname, "../agent/agentAuthority"));
/**
...
function buildAveragePriceEntry(price, adjustedPrice = null) {
    ...
    const normalizedAdjusted = adjustedPrice == null ? normalizedAverage : Number(adjustedPrice) || 0;
    return buildKeyVal([
...
function resolveSessionSolarSystemID(session, fallback = null) {
    return normalizeSolarSystemID(session &&
        (session.solarSystemID ??
            session.solarsystemid ??
            session.solarSystemID2 ??
            session.solarsystemid2), fallback);
}
function buildLocationRow(locationID, locationName, solarSystemID = null, position = null, fallbackName = null) {
    return [
        locationID,
        toClientSafeDisplayName(locationName, fallbackName || `Location ${locationID}`),
...
function getStaticLocationRowsById() {
    ...
    const addLocation = (locationID, locationName, solarSystemID = null, position = null) => {
        const numericId = Number(locationID) || 0;
        if (numericId <= 0 || !locationName) {
            return;
        }
        rowsById.set(numericId, buildLocationRow(numericId, locationName, solarSystemID, position));
    };
    for (const system of readStaticRows(TABLE.SOLAR_SYSTEMS)) {
        addLocation(system.solarSystemID, system.solarSystemName, system.solarSystemID, system.position);
    }
    for (const station of readStaticRows(TABLE.STATIONS)) {
        addLocation(station.stationID, station.stationName, station.solarSystemID, station.position);
    }
    for (const celestial of readStaticRows(TABLE.CELESTIALS)) {
        addLocation(celestial.itemID, celestial.itemName, celestial.solarSystemID, celestial.position);
    }
    for (const asteroidBelt of readStaticRows(TABLE.ASTEROID_BELTS)) {
        addLocation(asteroidBelt.itemID, asteroidBelt.itemName, asteroidBelt.solarSystemID, asteroidBelt.position);
    }
    for (const stargate of readStaticRows(TABLE.STARGATES)) {
        addLocation(stargate.itemID, stargate.itemName, stargate.solarSystemID, stargate.position);
    }
...
function buildShipItemOwnerRow(itemID) {
    ...
    return [
        ...
        toClientSafeDisplayName(shipItem.shipName || shipItem.itemName || `Ship ${itemID}`, `Ship ${itemID}`),
...
function buildAgentOwnerRow(ownerID) {
    ...
    return [
        ...
        toClientSafeDisplayName(agentRecord.ownerName || `Agent ${ownerID}`, `Agent ${ownerID}`),
...
class ConfigService extends BaseService {
    ...
    Handle_GetMultiOwnersEx(args, session) {
        ...
        log.debug(`[ConfigService] GetMultiOwnersEx: ${JSON.stringify(requestedIds)}`);
    ...
    Handle_GetMultiLocationsEx(args, session) {
        const rawArg = args && args.length > 0 ? args[0] : [];
        const requestedIds = extractList(rawArg);
        log.debug(`[ConfigService] GetMultiLocationsEx: ${JSON.stringify(requestedIds)}`);
        const characterResult = database.read("characters", "/");
        const characters = characterResult.success ? characterResult.data : {};
        const station = getStationRecord(session);
        const sessionSolarSystemID = resolveSessionSolarSystemID(session, normalizeSolarSystemID(station && station.solarSystemID, null));
        const locationRowsById = new Map();
        const staticRowsById = getStaticLocationRowsById();
        if (station) {
            locationRowsById.set(station.stationID, buildLocationRow(station.stationID, station.stationName, station.solarSystemID, station.position));
            locationRowsById.set(station.orbitID, buildLocationRow(station.orbitID, station.orbitName || station.stationName, station.solarSystemID));
            locationRowsById.set(station.solarSystemID, buildLocationRow(station.solarSystemID, station.solarSystemName || `System ${station.solarSystemID}`, station.solarSystemID));
            locationRowsById.set(station.constellationID, buildLocationRow(station.constellationID, station.constellationName || `Constellation ${station.constellationID}`, null));
            locationRowsById.set(station.regionID, buildLocationRow(station.regionID, station.regionName || `Region ${station.regionID}`, null));
        }
        ...
        for (const id of requestedIds) {
            ...
            if (locationRowsById.has(numericId)) {
                ...
                if (structure) {
                    rows.push(buildLocationRow(structure.structureID, structure.itemName || structure.name || `Structure ${structure.structureID}`, structure.solarSystemID, structure.position, `Structure ${structure.structureID}`));
                    continue;
                }
            }
            if (shipNameById.has(numericId)) {
                rows.push(buildLocationRow(numericId, shipNameById.get(numericId), sessionSolarSystemID, null, `Ship ${numericId}`));
            }
            else if (charNameById.has(numericId)) {
                rows.push(buildLocationRow(numericId, charNameById.get(numericId), sessionSolarSystemID, null, `Character ${numericId}`));
            }
            else if (numericId >= 60000000 && numericId < 64000000) {
                rows.push(buildLocationRow(numericId, `Station ${numericId}`, sessionSolarSystemID, null, `Station ${numericId}`));
            }
            else if (numericId >= 30000000 && numericId < 40000000) {
                rows.push(buildLocationRow(numericId, `System ${numericId}`, numericId, null, `System ${numericId}`));
            }
            else {
                rows.push(buildLocationRow(numericId, `Location ${numericId}`, sessionSolarSystemID, null, `Location ${numericId}`));
            }
        }
        if (rows.length === 0) {
            return [];
    ...
    Handle_GetMultiAllianceShortNamesEx(args, session) {
        ...
        log.debug(`[ConfigService] GetMultiAllianceShortNamesEx: ${JSON.stringify(allianceIDs)}`);
    ...
    Handle_GetMultiCorpTickerNamesEx(args, session) {
        const rawArg = args && args.length > 0 ? args[0] : [];
        const requestedIds = extractList(rawArg);
        log.debug(`[ConfigService] GetMultiCorpTickerNamesEx: ${JSON.stringify(requestedIds)}`);
        const rows = [];
        for (const id of requestedIds) {
    ...
    Handle_GetStationSolarSystemsByOwner(args, session) {
        ...
        return buildList(getCorporationStationSolarSystems(ownerID).map((solarSystemRecord) => buildKeyVal([
