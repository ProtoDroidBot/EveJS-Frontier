...
function normalizePackedInstanceRowForProfile(packedInstanceRow, compatibilityProfile) {
    if (!usesTimestampedInfoAttributes(compatibilityProfile) ||
        !packedInstanceRow ||
    ...
    const entries = [];
    for (const [field, attributeID] of Object.entries(INSTANCE_ATTRIBUTE_ID_BY_FIELD)) {
...
function normalizeInstanceCacheForProfile(instanceCache, compatibilityProfile) {
    if (!usesTimestampedInfoAttributes(compatibilityProfile) ||
        !instanceCache ||
    ...
    return {
        ...
        entries: instanceCache.entries.map(([itemID, instanceAttributes]) => [
            itemID,
            normalizePackedInstanceRowForProfile(instanceAttributes, compatibilityProfile),
        ]),
    };
}
function normalizeActivationStateForProfile(activationState, compatibilityProfile) {
    if (usesTimestampedInfoAttributes(compatibilityProfile) &&
        Array.isArray(activationState)) {
        const normalizedState = activationState.slice(0, 3);
        normalizedState[0] = normalizeInstanceCacheForProfile(normalizedState[0], compatibilityProfile);
        return normalizedState;
    }
    return activationState;
}
function normalizeInfoAttributesForProfile(attributes, timestamp, compatibilityProfile) {
    if (!usesTimestampedInfoAttributes(compatibilityProfile) ||
        !attributes ||
...
//# sourceMappingURL=dogmaInfoCompatibility.js.map