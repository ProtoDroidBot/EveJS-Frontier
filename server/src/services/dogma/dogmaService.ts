...
function recordSpaceBootstrapTrace(session, event, details: Record<string, any> = {}) {
  if (
    !session ||
...
const ATTRIBUTE_EXPLOSION_DELAY =
  getAttributeIDByNames("explosionDelay") || 281;
...
const ATTRIBUTE_STRUCTURE_FULL_POWER_HP_MULTIPLIER =
  getAttributeIDByNames("structureFullPowerStateHitpointMultiplier") || 2743;
const ATTRIBUTE_HIDDEN_ARMOR_HP_MULTIPLIER =
  getAttributeIDByNames("hiddenArmorHPMultiplier") || 2751;
const ATTRIBUTE_STRUCTURE_POWER_STATE_ARMOR_PLATING_MULTIPLIER =
  getAttributeIDByNames("structurePowerStateArmorPlatingMultiplier") || 2805;
...
function getMissingRequiredSkill(characterID, typeID) {
  const skillLevels = new Map<any, any>(
    getCharacterSkills(Number(characterID) || 0).map((skillRecord) => [
  ...
  return getRequiredSkillRequirements(Number(typeID) || 0).find((requirement) => {
    ...
    const requiredLevel = Math.max(
      1,
      Number(requirement && (requirement.requiredLevel ?? requirement.level)) || 1,
    );
...
function boardNewbieShipForSession(session, options: Record<string, any> = {}) {
...
function summarizeAmmoLoadRequests(requests: any[] = []) {
...
function normalizeReloadSourceItemIDs(rawItemIDs: any[] = []) {
  return [...new Set(
...
function resolveMaxGroupOnlineLimit(moduleItem, fittedItems: any[] = []) {
...
class DogmaService extends BaseService {
  declare _onlineEffectStartTimes: any;
  ...
  _getDockedItemInfoCache(session, options: Record<string, any> = {}) {
  ...
  _resolveActiveShipLocationID(session, shipMetadata: Record<string, any> = {}, fallback = 0) {
  ...
  _resolveActiveShipFlagID(session, shipMetadata: Record<string, any> = {}, fallback = 4) {
    const spaceSystemID = this._getSessionSpaceSystemID(session);
    if (spaceSystemID > 0) {
      return 0;
    }
    ...
    return {
      ...
      dogmaAttributeOverrides: {
        [ATTRIBUTE_STRUCTURE_FULL_POWER_HP_MULTIPLIER]:
          Number(fullPowerDogma.hitpointMultiplier) || 1,
        [ATTRIBUTE_HIDDEN_ARMOR_HP_MULTIPLIER]:
          Number(fullPowerDogma.hiddenArmorHPMultiplier) || 1,
        [ATTRIBUTE_STRUCTURE_POWER_STATE_ARMOR_PLATING_MULTIPLIER]:
          Number(fullPowerDogma.armorPlatingMultiplier) || 0,
      },
      conditionState:
  ...
  _normalizeControlledStructureFittingOwners(shipContext) {
    ...
    const ownerID = Number(
      shipContext.shipMetadata && shipContext.shipMetadata.ownerID,
    ) || 0;
    ...
    for (const item of listFittedItemsForLocation(shipID)) {
      ...
      if (updateResult.success) {
        ...
        log.warn(
          `[DogmaIM] Failed to normalize controlled structure fitting owner ` +
    ...
    if (normalizedCount > 0) {
      log.info(
        `[DogmaIM] Normalized controlled structure fitting owners ` +
  ...
  _persistDroneSettingChanges(session, droneSettingChanges: Record<string, any> = {}) {
  ...
  _buildCharacterAttributes(charData: Record<string, any> = {}, characterID = null) {
  ...
  _buildCharacterBaseAttributes(charData: Record<string, any> = {}) {
    ...
    const attributes = Object.fromEntries(
      Object.entries<any>(getTypeDogmaAttributes(typeID))
    ...
    for (const [attributeID, value] of Object.entries<any>(source)) {
  ...
  _buildShipModifiedCharacterAttributes(
    ...
    if (activeShip) {
      ...
      if (fittingSnapshot) {
        ...
        for (const [attributeID, value] of Object.entries<any>(
  ...
  _buildCharacterAttributeDict(charData: Record<string, any> = {}, characterID = null) {
  ...
  _getShipRuntimeAttributeOverrides(session, shipData: Record<string, any> = {}) {
  ...
  _getGenericModuleAttributeOverrides(item, session, options: Record<string, any> = {}) {
    if (!item || !session || Number(item.flagID) === ITEM_FLAGS.DRONE_BAY) {
      return null;
    }
    const charID = this._getCharID(session);
  ...
  _resolveLoadedChargeItem(item, session = null, options: Record<string, any> = {}) {
  ...
  _getWeaponDogmaAttributeOverrides(item, session = null, options: Record<string, any> = {}) {
    if (
      !item ||
  ...
  _buildWeaponModuleAttributeMap(moduleItem, chargeItem = null, session = null) {
    ...
    return Object.fromEntries(
      Object.entries<any>(moduleAttributes)
        .map(([attributeID, value]) => [Number(attributeID), Number(value)])
        .filter(
          ([attributeID, value]) =>
            Number.isInteger(attributeID) && Number.isFinite(value),
  ...
  _getFittedModuleResourceAttributeOverrides(item, session = null, options: Record<string, any> = {}) {
    if (
      !item ||
  ...
  _buildShipAttributes(charData: Record<string, any> = {}, shipData: Record<string, any> = {}, session = null, options: Record<string, any> = {}) {
    const securityStatus = Number(
    ...
    if (
      ...
      for (const [attributeID, value] of Object.entries<any>(runtimeAttributeOverrides.attributes)) {
        const numericAttributeID = Number(attributeID);
        const numericValue = Number(value);
        if (!Number.isInteger(numericAttributeID) || !Number.isFinite(numericValue)) {
          continue;
  ...
  _buildShipAttributeDict(charData: Record<string, any> = {}, shipData: Record<string, any> = {}, session = null, options: Record<string, any> = {}) {
  ...
  _buildAttributeValueDict(attributes: Record<string, any> = {}) {
    return {
      type: "dict",
  ...
  _buildInventoryItemAttributes(item, session = null, options: Record<string, any> = {}) {
    ...
    const attributes = Object.fromEntries(
      Object.entries<any>(typeAttributes || {})
        .map(([attributeID, value]) => [Number(attributeID), Number(value)])
        .filter(
          ([attributeID, value]) =>
            Number.isInteger(attributeID) && Number.isFinite(value),
    ...
    for (const [attributeID, value] of [
      ...
      const numericValue = Number(value);
      if (Number.isFinite(numericValue)) {
        attributes[attributeID] = marshalDogmaAttributeValue(
          attributeID,
          numericValue,
        );
      }
    ...
    if (
      ...
      for (const [attributeID, value] of Object.entries<any>(item.dogmaAttributeOverrides)) {
        const numericAttributeID = Number(attributeID);
        const numericValue = Number(value);
        if (
    ...
    if (
      ...
      for (const [attributeID, value] of Object.entries<any>(droneBayAttributeOverrides)) {
        const numericAttributeID = Number(attributeID);
        const numericValue = Number(value);
        if (
          !Number.isInteger(numericAttributeID) ||
    ...
    if (structureServiceAttributes) {
      for (const [attributeID, value] of Object.entries<any>(structureServiceAttributes)) {
        const numericAttributeID = Number(attributeID);
        const numericValue = Number(value);
        if (
          !Number.isInteger(numericAttributeID) ||
    ...
    if (resourceAttributeOverrides) {
      for (const [attributeID, value] of Object.entries<any>(resourceAttributeOverrides)) {
        const numericAttributeID = Number(attributeID);
        const numericValue = Number(value);
        if (
          Number.isInteger(numericAttributeID) &&
    ...
    if (overrideAttributes && typeof overrideAttributes === "object") {
      for (const [attributeID, value] of Object.entries<any>(overrideAttributes)) {
        const numericAttributeID = Number(attributeID);
        const numericValue = Number(value);
        if (
          !Number.isInteger(numericAttributeID) ||
    ...
    if (propulsionRuntimeAttributes) {
      ...
      if (genericRuntimeAttributes) {
        ...
        if (genericAttributeOverrides) {
          for (const [attributeID, value] of Object.entries<any>(genericAttributeOverrides)) {
            const numericAttributeID = Number(attributeID);
            const numericValue = Number(value);
            if (
              !Number.isInteger(numericAttributeID) ||
  ...
  _notifyChargeBeingLoadedToModule(session, moduleIDs: any[] = [], chargeTypeID, reloadTimeMs) {
  ...
  _buildInventoryItemAttributeDict(item, session = null, options: Record<string, any> = {}) {
  ...
  _buildActiveEffectEntry(item, effectID, options: Record<string, any> = {}, session = null) {
    if (!item || effectID <= 0) {
      return null;
    }
  ...
  _buildInventoryItemActiveEffects(item, session = null, options: Record<string, any> = {}) {
    if (!item) {
  ...
  _listLoadedChargeSublocationItems(charID, shipID, options: Record<string, any> = {}) {
  ...
  _buildChargeSublocationData(charID, shipID, options: Record<string, any> = {}) {
    const numericShipID = Number(shipID) || 0;
        ...
        for (const item of fittedItems) {
          ...
          const entry =
            ...
            this._buildCommonGetInfoEntry({
              ...
              activeEffects: this._buildInventoryItemActiveEffects(
                item,
                session,
                {
              ...
              attributes: this._buildInventoryItemAttributeDict(item, session, {
                ...
                includeQuantityAttribute:
                  options.includeFittedItemQuantityAttribute !== false,
              }),
              session,
            });
  ...
  _buildChargeStateDict(charID, shipID, options: Record<string, any> = {}) {
  ...
  _findInventoryItemContext(requestedItemID, session, options: Record<string, any> = {}) {
  ...
  _notifyModuleAttributeChanges(session, changes: any[] = []) {
    if (
      !session ||
  ...
  _dispatchPostResponseAttributeChanges(
    session,
    changes: any[] = [],
    options: Record<string, any> = {},
  ) {
  ...
  _refreshDockedFittingState(session, changes: any[] = [], options: Record<string, any> = {}) {
    if (
      !session ||
      !isDockedSession(session) ||
      !Array.isArray(changes) ||
  ...
  _syncInventoryChanges(session, changes: any[] = [], options: Record<string, any> = {}) {
  ...
  _resolveShipInventoryChangeContext(session, change: Record<string, any> = {}) {
  ...
  _syncShipInventoryItemsChangedForSession(session, change: Record<string, any> = {}) {
  ...
  _launchProbesFromContext(session, probeContext = null) {
    ...
    const probeTypeAttributes = getTypeDogmaAttributes(
      Number(loadedCharge && loadedCharge.typeID) || 0,
    );
    const expiryDelayMs = Math.max(
      0,
      Number(probeTypeAttributes[String(ATTRIBUTE_EXPLOSION_DELAY)]) || 0,
    );
  ...
  _buildRemovedInventoryNotificationState(item: Record<string, any> = {}) {
    return {
      ...item,
  ...
  _filterInventoryChangesForClient(session, changes: any[] = []) {
  ...
  _normalizeInventoryChanges(changes: any[] = []) {
    if (!Array.isArray(changes)) {
      return [];
    }
    return changes
  ...
  _buildShipBaseAttributes(shipData: Record<string, any> = {}) {
    ...
    const attributes = staticAttributes
      ? Object.fromEntries(
          Object.entries<any>(staticAttributes)
            .map(([attributeID, value]) => [Number(attributeID), Number(value)])
            .filter(
              ([attributeID, value]) =>
                Number.isInteger(attributeID) && Number.isFinite(value),
  ...
  _buildFittingHydrationContext(charID, shipID, shipItem = null, options: Record<string, any> = {}) {
    const numericShipID =
  ...
  _isFittingContextMatch(context, charID, shipID, options: Record<string, any> = {}) {
    if (!context) {
      return false;
    }
    const numericShipID = Number(shipID) || 0;
  ...
  _getFittingContextSnapshotOptions(context, charID, shipID, options: Record<string, any> = {}) {
  ...
  _getFittingContextModuleByFlag(context, charID, shipID, flagID, options: Record<string, any> = {}) {
    if (!this._isFittingContextMatch(context, charID, shipID, options)) {
      return null;
    }
    return context.moduleByFlag.get(Number(flagID) || 0) || null;
  }
  _getFittingContextChargeByFlag(context, charID, shipID, flagID, options: Record<string, any> = {}) {
    if (!this._isFittingContextMatch(context, charID, shipID, options)) {
      return null;
    }
    return context.chargeByFlag.get(Number(flagID) || 0) || null;
  }
  _buildActivationState(charID, shipID, shipRecord = null, options: Record<string, any> = {}) {
  ...
  _getCharacterItemLocationID(session, options: Record<string, any> = {}) {
  ...
  _buildStructureInfoDict(structure, session = null, options: Record<string, any> = {}) {
    if (!structure) {
      return this._buildEmptyDict();
    }
    ...
    const structureAttributes = this._buildInventoryItemAttributes(
      structureItem,
      session,
      {
  ...
  _shouldPrimeLoginShipInfoChargeSublocations(session, options: Record<string, any> = {}) {
    if (!session || !session._space) {
      return false;
    }
  ...
  _buildLoadedChargeTupleQuantityChanges(session, charID, shipID, options: Record<string, any> = {}) {
    if (!ATTRIBUTE_QUANTITY) {
      return [];
    }
    const numericCharID = Number(charID) || this._getCharID(session);
  ...
  _queuePostGetAllInfoChargeQuantityRefresh(session, charID, shipID, options: Record<string, any> = {}) {
    if (!session || !session._space) {
      return 0;
    }
  ...
  _queuePostGetAllInfoCreationAttributeRefresh(
    ...
    charID,
    shipID,
    shipAttributes: Record<string, any> = {},
    ...
    const attributeIDs = new Set<any>(
      modifierEntries
    ...
    const changes = [...attributeIDs]
      ...
      .filter(({ value }: Record<string, any>) => Number.isFinite(value))
      .map(({ attributeID, value }: Record<string, any>) => [
        "OnModuleAttributeChange",
        numericCharID,
        numericShipID,
        attributeID,
  ...
  _flushPostGetAllInfoCreationAttributeRefresh(session) {
    ...
    this._dispatchPostResponseAttributeChanges(session, changes);
    const fuelChargeChanges = changes.filter(
      (change) => Number(change && change[3]) === ATTRIBUTE_FUEL_CHARGE,
    );
    if (isDockedSession(session) && fuelChargeChanges.length > 0) {
      const configuredReplayDelay = Number(
        session && session._postCreationFuelRefreshReplayDelayMs,
      );
      this._dispatchPostResponseAttributeChanges(session, fuelChargeChanges, {
        delayMs:
          Number.isFinite(configuredReplayDelay) && configuredReplayDelay >= 0
  ...
  _buildShipState(charID, shipID, shipRecord = null, options: Record<string, any> = {}) {
  ...
  _buildTargetIDList(targetIDs: any[] = []) {
    return {
      type: "list",
  ...
  _resolveAnalyzerHackingContext(session, moduleItem, targetID) {
    ...
    if (normalizedTargetID <= 0) {
      return { success: false as const, errorMsg: "TARGET_REQUIRED" };
    }
    const scene =
      typeof spaceRuntime.getSceneForSession === "function"
        ? spaceRuntime.getSceneForSession(session)
    ...
    if (
      ...
      return { success: false as const, errorMsg: "TARGET_NOT_FOUND" };
    }
    if (!this._isLockedTarget(sourceEntity, normalizedTargetID)) {
      return { success: false as const, errorMsg: "TARGET_NOT_LOCKED" };
    }
    ...
    if (!isHackableContainer) {
      return { success: false as const, errorMsg: "ANALYZER_TARGET_INVALID" };
    }
    if (Number(targetEntity.dungeonSiteContentHackingState) === HACKING_STATE_HACKED) {
      return { success: false as const, errorMsg: "ANALYZER_TARGET_ALREADY_HACKED" };
    }
    if (Number(targetEntity.dungeonSiteContentHackingState) === HACKING_STATE_BEING_HACKED) {
      return { success: false as const, errorMsg: "ANALYZER_TARGET_BUSY" };
    }
    ...
    if (
      ...
      return { success: false as const, errorMsg: "TARGET_OUT_OF_RANGE" };
    }
    ...
    if (!this._isAnalyzerCompatibleWithTarget(moduleKind, targetKind)) {
      return { success: false as const, errorMsg: "ANALYZER_TYPE_MISMATCH" };
    }
  ...
  _buildModuleTargetOutOfRangeNotify(context: Record<string, any> = {}) {
  ...
  _buildModuleTargetOutOfRangeUserErrorValues(context: Record<string, any> = {}) {
    const session = context.session || null;
    const moduleItem = context.moduleItem || null;
    const targetID = Number(context.targetID) || 0;
    const scene =
  ...
  _buildModuleTargetTooFarUserErrorValues(context: Record<string, any> = {}) {
    const session = context.session || null;
    const moduleItem = context.moduleItem || null;
    const targetID = Number(context.targetID) || 0;
    const scene =
  ...
  _throwModuleActivationUserError(errorMsg = "", context: Record<string, any> = {}) {
    ...
    switch (normalizedErrorMsg) {
      ...
      case "CLOAK_WARP_BLOCKED":
        this._throwCustomNotifyUserError(
          "You cannot activate that cloaking device while warping.",
        );
        break;
      case "MODULE_CLOAKED":
  ...
  _startModuleRepair(session, moduleID, options: Record<string, any> = {}) {
  ...
  _collectWeaponBankTouchedModuleIDs(
    ...
    const numericShipID = Number(shipID) || 0;
    const touchedModuleIDs = new Set<any>();
    if (options.includeAllBanks === true) {
      ...
      for (const [masterID, slaveIDs] of Object.entries<any>(banks || {})) {
  ...
  _repairWeaponBankModulePresentation(session, shipID, moduleIDs: any[] = []) {
  ...
  Handle_LinkAllWeapons(args, session) {
    ...
    if (result && result.data && result.data.changed) {
      ...
      this._repairWeaponBankModulePresentation(
        session,
        shipID,
        [...new Set<any>([...touchedModuleIDs, ...nextTouchedModuleIDs])],
      );
    }
    return buildWeaponBankStateDict(shipID, {
      banks:
  ...
  _setModuleOnlineState(shipID, moduleID, online, session) {
    ...
    const numericModuleID = Number(moduleID) || 0;
    const moduleItem = findItemById(numericModuleID);
    const structureShipRecord: Record<string, any> = this._getStructureShipMetadataByID(numericShipID);
    ...
    let fuelConsumptionChanges: any[] = [];
    if (
      nextOnline &&
      !previousOnline &&
      isStructureHost &&
    ...
    if (!nextOnline && inSpace) {
      const overloadEffect = spaceRuntime.getOverloadModuleEffect(
        session,
        numericModuleID,
      );
      if (overloadEffect) {
        spaceRuntime.stopOverloadModule(
          session,
          moduleItem,
          overloadEffect.effectID,
    ...
    const onlineToggleSubEvents: any[] = [];
    const isOnlineAttributeID = getAttributeIDByNames("isOnline");
  ...
  _resolveAmmoSourceStacks(charID, ammoLocationID, sourceFlagID, chargeTypeID, chargeRequests: any[] = []) {
  ...
  _resolveRequestedAmmoTypeID(charID, ammoLocationID, sourceFlagID, chargeRequests: any[] = []) {
  ...
  _queuePendingModuleReload(session, moduleItem, options: Record<string, any> = {}) {
    const numericModuleID = Number(moduleItem && moduleItem.itemID) || 0;
    if (numericModuleID <= 0) {
      return {
        success: false as const,
  ...
  queueAutomaticModuleReload(session, moduleItem, options: Record<string, any> = {}) {
  ...
  Handle_Activate(args, session) {
    ...
    if (isAnalyzerActivation && analyzerHackingContext && analyzerHackingContext.success === true) {
      const contextData: Record<string, any> = analyzerHackingContext.data || {};
      try {
  ...
  Handle_InitiateModuleRepair(args, session) {
    const itemID = args && args.length > 0 ? Number(args[0]) || 0 : 0;
    const result: Record<string, any> = this._startModuleRepair(session, itemID, {
      source: "single",
    });
    if (result.success) {
      log.debug(
  ...
  Handle_LoadFuel(args, session) {
    ...
    this._syncSpaceEntityFuelCharge(
      session,
      shipID,
      nextFuelCharge,
      loadedFuelTypeID,
  ...
  afterCallResponse(methodName, session, context: Record<string, any> = {}) {
