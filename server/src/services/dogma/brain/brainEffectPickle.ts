...
function encodeList(values: any[] = []) {
  const parts: Buffer[] = [Buffer.from([0x5d])];
...
function encodeTuple(values: any[] = []) {
  if (!Array.isArray(values) || values.length === 0) {
    return Buffer.from([0x29]);
  }
  const parts: Buffer[] = [Buffer.from([0x28])];
  for (const value of values) {
    parts.push(encodeInt(value));
  }
...
function buildBrainEffectPickle(effect) {
  ...
  const stateEntries: [string, Buffer][] = [
