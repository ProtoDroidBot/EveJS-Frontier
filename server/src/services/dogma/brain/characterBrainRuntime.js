...
const { buildKeyVal, buildObjectEx1, buildList, } = require(path.join(__dirname, "../../_shared/serviceHelpers"));
const { encodeCharacterBrainEffectLists, } = require(path.join(__dirname, "./characterBrainCompatibility"));
const { FITTING_BRAIN_PROVIDER, } = require(path.join(__dirname, "./providers/fittingBrainProvider"));
const { INDUSTRY_BRAIN_PROVIDER, } = require(path.join(__dirname, "./providers/industryBrainProvider"));
const { IMPLANT_BRAIN_PROVIDER, } = require(path.join(__dirname, "./providers/implantBrainProvider"));
...
function buildBrainEffectObject(targetItemID, effectDefinition) {
    ...
    return buildObjectEx1("eve.common.script.dogma.effect.BrainEffect", [
...
function buildCharacterBrainDefinitionSet(characterID) {
    ...
    return {
        characterEffects: collectProviderDefinitions(numericCharacterID, "buildCharacterEffects"),
        shipEffects: collectProviderDefinitions(numericCharacterID, "buildShipEffects"),
        structureEffects: collectProviderDefinitions(numericCharacterID, "buildStructureEffects"),
    };
}
    ...
    if (shipID <= 0 || structureID <= 0) {
        try {
            ...
            if (structureID <= 0) {
                ...
                structureID = toInt(characterRecord.structureID ??
                    characterRecord.structureid ??
                    characterRecord.structureId, 0);
            }
        }
        catch (error) {
...
function buildBootstrapCharacterBrain(characterID, version = 0, options = {}) {
    ...
    return [
        toInt(version, 0),
        definitions.characterEffects.map((effectDefinition) => buildBrainEffectObject(targets.characterID, effectDefinition)),
        definitions.shipEffects.map((effectDefinition) => buildBrainEffectObject(targets.shipID, effectDefinition)),
        definitions.structureEffects.map((effectDefinition) => buildBrainEffectObject(targets.structureID, effectDefinition)),
    ];
}
function getNextCharacterBrainVersion(characterID) {
    const numericCharacterID = toInt(characterID, 0);
    const nextVersion = (CHARACTER_BRAIN_VERSION_BY_CHARACTER_ID.get(numericCharacterID) || 0) + 1;
...
function buildCharacterBrainGrayMatter(characterID, options = {}) {
    const targets = resolveCharacterBrainTargetIDs(characterID, options);
    const definitions = buildCharacterBrainDefinitionSet(targets.characterID);
    const charEffects = definitions.characterEffects.map((effectDefinition) => buildBrainEffectObject(targets.characterID, effectDefinition));
    const shipEffects = definitions.shipEffects.map((effectDefinition) => buildBrainEffectObject(targets.shipID, effectDefinition));
    const structureEffects = definitions.structureEffects.map((effectDefinition) => buildBrainEffectObject(targets.structureID, effectDefinition));
    return encodeCharacterBrainEffectLists(charEffects, shipEffects, structureEffects, options.compatibilityProfile);
}
...
function syncCharacterDogmaState(session, characterID = null, options = {}) {
    ...
    const syncedBrain = options.syncBrain === false
        ? false
...
//# sourceMappingURL=characterBrainRuntime.js.map