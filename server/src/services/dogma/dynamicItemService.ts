...
function normalizeAttributeDefinition(entry) {
  ...
  const result: Record<string, any> = { attributeID, min, max };
...
function normalizeMapping(entry) {
  ...
  return {
    resultingType,
    applicableTypes,
    applicableTypeSet: new Set<any>(applicableTypes),
  };
}
...
function normalizeMutator(entry) {
  ...
  return {
    mutatorTypeID,
    attributes,
    attributesByID: new Map<any, any>(
...
function loadAuthority() {
  ...
  const dynamicResultTypes = new Set<any>();
...
function normalizeNumericObject(source: Record<string, any> = {}) {
...
function buildNumericDict(source: Record<string, any> = {}) {
  return buildDict(
...
function buildDynamicItemPayload(item, metadata?: any) {
...
function chooseModifier({ mutatorItemID, sourceItemID, attributeID, randomValue = null }: Record<string, any>) {
...
class DynamicItemService extends BaseService {
  ...
  Handle_GetDynamicItemInfo(args: any[] = []) {
  ...
  Handle_CreateDynamicItem(args: any[] = [], session = null) {
