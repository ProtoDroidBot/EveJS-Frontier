...
const { buildDict, buildKeyVal, normalizeNumber, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { findItemById, consumeInventoryItemQuantity, grantItemToCharacterLocation, ITEM_FLAGS, } = require(path.join(__dirname, "../inventory/itemStore"));
...
function getSessionCharacterID(session) {
    return toPositiveInt(session && (session.charid || session.characterID || session.userid), 0);
}
...
function normalizeMutator(entry) {
    ...
    return {
        mutatorTypeID,
        attributes,
        attributesByID: new Map(attributes.map((attribute) => [attribute.attributeID, attribute])),
        mappings,
    };
}
...
function resolveMutationMapping(mutator, sourceTypeID) {
    ...
    return (mutator.mappings.find((mapping) => mapping.applicableTypeSet.has(numericSourceTypeID)) || null);
}
...
function buildMutationMetadata({ mutator, mapping, mutatorItem, sourceItem, resultItemID = 0, characterID = 0, now = Date.now(), rollProvider = null, }) {
...
function validateCraftInputs(mutatorItem, sourceItem, session) {
    ...
    if (toPositiveInt(mutatorItem.ownerID, 0) !== characterID ||
...
class DynamicItemService extends BaseService {
    ...
    Handle_CreateDynamicItem(args = [], session = null) {
        ...
        const { mutator, mapping, characterID } = validateCraftInputs(mutatorItem, sourceItem, session);
        ...
        const mutatorConsumeResult = consumeInventoryItemQuantity(mutatorItem.itemID, MUTATION_QUANTITY, { removeContents: true });
        if (!mutatorConsumeResult || mutatorConsumeResult.success !== true) {
            throwNotify("The selected mutaplasmid could not be consumed.");
        }
        const sourceConsumeResult = consumeInventoryItemQuantity(sourceItem.itemID, MUTATION_QUANTITY, { removeContents: true });
        if (!sourceConsumeResult || sourceConsumeResult.success !== true) {
            log.error(`[DynamicItemService] Source consumption failed after mutator consumption mutator=${mutatorItem.itemID} source=${sourceItem.itemID}: ${sourceConsumeResult && sourceConsumeResult.errorMsg}`);
            throwNotify("The selected source item could not be consumed.");
        }
        const resultGrant = grantItemToCharacterLocation(characterID, mutatorItem.locationID || sourceItem.locationID || characterID, mutatorItem.flagID || sourceItem.flagID || ITEM_FLAGS.HANGAR, mapping.resultingType, MUTATION_QUANTITY, {
            singleton: 1,
            customInfo: stringifyCustomInfo(metadata),
        });
        if (!resultGrant || resultGrant.success !== true) {
            log.error(`[DynamicItemService] Result grant failed after consuming inputs mutator=${mutatorItem.itemID} source=${sourceItem.itemID}: ${resultGrant && resultGrant.errorMsg}`);
        ...
        if (resultItemID > 0) {
            ...
            const updateResult = require("../inventory/itemStore").writeItemCustomInfo(resultItemID, updatedCustomInfo, { force: true });
            if (!updateResult || updateResult.success !== true) {
                log.warn(`[DynamicItemService] Unable to backfill result itemID into customInfo for ${resultItemID}`);
            }
            if (resultItem) {
...
//# sourceMappingURL=dynamicItemService.js.map