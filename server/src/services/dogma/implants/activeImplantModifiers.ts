...
function cloneAttributeMap(attributes: Record<string, any> = {}) {
...
function targetMatchesLocationModifier(target, modifierInfo, options: Record<string, any> = {}) {
...
function applyDogmaModifierGroups(attributes, modifierEntries: any[] = []) {
...
function getEffectRecordsForBooster(booster, typeEntry) {
  const sideEffectIDs = new Set<any>(
...
function appendDogmaModifierEntries(entries, source, options: Record<string, any> = {}) {
  const allowedDomains = options.allowedDomains instanceof Set
    ? options.allowedDomains
    : null;
...
function getImplantDogmaModifierEntries(characterOrID, options: Record<string, any> = {}) {
  const entries: any[] = [];
...
function getActiveImplantDirectModifierEntries(characterOrID, options: Record<string, any> = {}) {
  const allowedDomains = options.allowedDomains instanceof Set
    ? options.allowedDomains
    : new Set(["shipID"]);
  const allowedFuncs = options.allowedFuncs instanceof Set
...
function getActiveImplantLocationModifierSources(characterOrID, options: Record<string, any> = {}) {
  const sources: any[] = [];
  for (const implant of getActiveImplantSourceStates(characterOrID)) {
