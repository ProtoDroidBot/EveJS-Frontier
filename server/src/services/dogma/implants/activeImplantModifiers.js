...
const { BOOSTER_GROUP_ID, applyBoosterSkillModifiersToAttributes, } = require(path.join(__dirname, "../../skills/boosters/boosterSkillRuntime"));
...
function typeRequiresSkillType(typeEntry, skillTypeID) {
    ...
    return REQUIRED_SKILL_ATTRIBUTE_IDS.some((attributeID) => toInt(attributes[String(attributeID)] ?? attributes[attributeID], 0) === numericSkillTypeID);
}
...
function appendLocationModifierEntriesForTarget(destination, sourceAttributes, sourceEffects, sourceKind, target, options = {}) {
    for (const effectRecord of Array.isArray(sourceEffects) ? sourceEffects : []) {
        for (const modifierInfo of effectRecord.modifierInfo || []) {
            ...
            const value = toFiniteNumber(sourceAttributes && (sourceAttributes[String(sourceAttributeID)] ??
                sourceAttributes[sourceAttributeID]), NaN);
        ...
        for (const [key, entries] of groups.entries()) {
            ...
            switch (operation) {
                ...
                case DOGMA_OP_POST_MUL: {
                    ...
                    const factor = entries.reduce((product, entry) => product * toFiniteNumber(entry.value, 1), 1);
                    attributes[attributeID] = round6(base * factor);
                    break;
                }
                case DOGMA_OP_MOD_ADD: {
                    const base = Number.isFinite(currentValue) ? currentValue : 0;
                    attributes[attributeID] = round6(base + entries.reduce((sum, entry) => sum + toFiniteNumber(entry.value, 0), 0));
                    break;
                }
                ...
                case DOGMA_OP_POST_PERCENT_UNNERFED: {
                    const base = Number.isFinite(currentValue) ? currentValue : 0;
                    const factor = entries.reduce((product, entry) => product * (1 + (toFiniteNumber(entry.value, 0) / 100)), 1);
                    attributes[attributeID] = round6(base * factor);
                    break;
                }
                case 5: {
...
function getTypeDogmaEntry(typeID) {
    ...
    const typesByTypeID = payload && payload.typesByTypeID && typeof payload.typesByTypeID === "object"
...
function getEffectTypeDogmaEntry(effectID) {
    const payload = getTypeDogmaPayload();
    const effectTypesByID = payload && payload.effectTypesByID && typeof payload.effectTypesByID === "object"
...
function getAttributeDefaultValue(attributeID) {
    const payload = getTypeDogmaPayload();
    const attributeTypesByID = payload && payload.attributeTypesByID && typeof payload.attributeTypesByID === "object"
        ? payload.attributeTypesByID
        : {};
    const attributeRecord = attributeTypesByID[String(attributeID)] || attributeTypesByID[attributeID] || null;
...
function getAttributeMapValue(attributes, attributeID) {
    ...
    return toFiniteNumber(attributes[String(numericAttributeID)] ??
        attributes[numericAttributeID], getAttributeDefaultValue(numericAttributeID));
}
...
function getEffectRecordsForBooster(booster, typeEntry) {
    const sideEffectIDs = new Set((Array.isArray(booster && booster.sideEffectIDs)
    ...
    return getEffectRecordsForTypeEntry(typeEntry).filter((effectEntry) => {
        const chanceAttributeID = toInt(effectEntry && effectEntry.fittingUsageChanceAttributeID, 0);
...
function getEffectiveBoosterSourceAttributes(booster, characterOrID, typeEntry = null) {
    ...
    const skillAttributes = applyBoosterSkillModifiersToAttributes(resolvedTypeEntry && resolvedTypeEntry.attributes, characterOrID);
    return applyActiveImplantLocationModifiersToAttributes(skillAttributes, {
        typeID: booster.typeID,
        groupID: BOOSTER_GROUP_ID,
    }, characterOrID, { includeBoosters: false });
}
...
function resolveImplantTypeID(implant) {
    return toInt(implant && (implant.typeID ??
        implant.implantTypeID ??
        implant.itemTypeID), 0);
}
function resolveImplantSlot(implant, typeEntry = null) {
    return toInt(implant && (implant.slot ??
        implant.implantSlot ??
        implant.implantness), toInt(typeEntry &&
        typeEntry.attributes &&
        (typeEntry.attributes[String(ATTRIBUTE_IMPLANT_SLOT)] ??
            typeEntry.attributes[ATTRIBUTE_IMPLANT_SLOT]), 0));
}
function resolveBoosterTypeID(booster) {
    return toInt(booster && (booster.boosterTypeID ??
        booster.typeID ??
        booster.itemTypeID), 0);
}
function resolveBoosterID(booster) {
    return toInt(booster && (booster.boosterID ??
        booster.itemID ??
        booster.sourceItemID), 0);
}
function resolveBoosterSlot(booster, typeEntry = null) {
    return toInt(booster && (booster.slot ??
        booster.boosterness), toInt(typeEntry &&
        typeEntry.attributes &&
        (typeEntry.attributes[String(ATTRIBUTE_BOOSTER_SLOT)] ??
            typeEntry.attributes[ATTRIBUTE_BOOSTER_SLOT]), 0));
}
...
function getImplantDogmaModifierEntries(characterOrID, options = {}) {
    ...
    for (const booster of getActiveBoosters(characterOrID)) {
        ...
        const sourceAttributes = getEffectiveBoosterSourceAttributes(booster, characterOrID, typeEntry);
        appendDogmaModifierEntries(entries, {
            sourceKind: "booster",
            typeID: booster.typeID,
            slot: booster.slot,
...
function compareImplantBrainEffectDefinitions(left, right) {
    return (toInt(left && left.skillTypeID, 0) - toInt(right && right.skillTypeID, 0) ||
        String(left && left.domain || "").localeCompare(String(right && right.domain || "")) ||
        String(left && left.modifierType || "").localeCompare(String(right && right.modifierType || "")) ||
        ...
        JSON.stringify(Array.isArray(left && left.extras) ? left.extras : []).localeCompare(JSON.stringify(Array.isArray(right && right.extras) ? right.extras : [])) ||
...
function getActiveImplantSourceStates(characterOrID) {
    ...
    for (const target of sources) {
        ...
        for (const source of sources) {
            appendLocationModifierEntriesForTarget(modifierEntries, source.sourceAttributes, source.sourceEffects, "implant", target, { allowedDomains: new Set(["charID"]) });
        }
...
function getActiveImplantLocationModifierSources(characterOrID, options = {}) {
    ...
    for (const implant of getActiveImplantSourceStates(characterOrID)) {
        ...
        const sourceEffects = implant.sourceEffects.filter((effectEntry) => (effectEntry.modifierInfo || []).some((modifierInfo) => {
            const domain = String(modifierInfo && modifierInfo.domain || "");
            const func = String(modifierInfo && modifierInfo.func || "");
            return ((domain === "shipID" || domain === "charID") &&
    ...
    for (const booster of getActiveBoosters(characterOrID)) {
        ...
        const sourceEffects = activeEffects.filter((effectEntry) => (effectEntry.modifierInfo || []).some((modifierInfo) => {
            const domain = String(modifierInfo && modifierInfo.domain || "");
            const func = String(modifierInfo && modifierInfo.func || "");
            return ((domain === "shipID" || domain === "charID") &&
                LOCATION_MODIFIER_FUNCS.has(func));
...
function applyActiveImplantLocationModifiersToAttributes(attributes, targetItem, characterOrID, options = {}) {
    ...
    for (const source of getActiveImplantLocationModifierSources(characterOrID, options)) {
        appendLocationModifierEntriesForTarget(modifierEntries, source.sourceAttributes, source.sourceEffects, source.sourceKind || "implant", target);
    }
...
function applyActiveImplantAttributeBonuses(attributes, characterOrID) {
    ...
    for (const entry of getImplantModifierEntries(characterOrID)) {
        ...
        if (!CHARACTER_PRIMARY_ATTRIBUTE_IDS.includes(targetAttributeID) ||
...
function buildImplantAttributeChangePayloads(session, characterID = null) {
    const numericCharacterID = toInt(characterID ?? session?.characterID ?? session?.charid, 0);
    ...
    const baseAttributes = {
        [ATTRIBUTE_CHARISMA]: toFiniteNumber(source[ATTRIBUTE_CHARISMA] ?? source.charisma, 20),
        [ATTRIBUTE_INTELLIGENCE]: toFiniteNumber(source[ATTRIBUTE_INTELLIGENCE] ?? source.intelligence, 20),
        [ATTRIBUTE_MEMORY]: toFiniteNumber(source[ATTRIBUTE_MEMORY] ?? source.memory, 20),
        [ATTRIBUTE_PERCEPTION]: toFiniteNumber(source[ATTRIBUTE_PERCEPTION] ?? source.perception, 20),
        [ATTRIBUTE_WILLPOWER]: toFiniteNumber(source[ATTRIBUTE_WILLPOWER] ?? source.willpower, 20),
    };
    ...
    const attributes = applyClientTrainingSpeedScale(applyActiveImplantAttributeBonuses(baseAttributes, record.characterID ? record : numericCharacterID));
    const changes = [];
...
//# sourceMappingURL=activeImplantModifiers.js.map