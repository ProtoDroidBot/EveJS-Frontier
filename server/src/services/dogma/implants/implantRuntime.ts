...
function buildImplantRecord(item, typeInfo, options: Record<string, any> = {}) {
...
function validateImplantItem(characterID, itemID, options: Record<string, any> = {}) {
  ...
  if (!typeInfo) {
    return { success: false as const, errorMsg: "NOT_AN_IMPLANT" };
  }
  ...
  if (occupiedSlot) {
    return { success: false as const, errorMsg: "IMPLANT_SLOT_OCCUPIED" };
  }
...
function installImplantItem(characterID, itemID, options: Record<string, any> = {}) {
