...
const { consumeInventoryItemQuantity, findItemById, ITEM_FLAGS, } = require(path.join(__dirname, "../../inventory/itemStore"));
...
const { getAttributeIDByNames, getRequiredSkillRequirements, getTypeDogmaRecord, } = require(path.join(__dirname, "../../fitting/liveFittingState"));
const { EVENT_CLONE_IMPLANT_INSTALLATION, EVENT_CLONE_IMPLANT_REMOVAL, } = require(path.join(__dirname, "../../station/jumpCloneRules"));
const { currentFileTime, } = require(path.join(__dirname, "../../_shared/serviceHelpers"));
const { getActiveImplants, } = require(path.join(__dirname, "./activeImplantModifiers"));
...
function getImplantTypeInfo(typeID) {
    ...
    const implantness = toInt(getTypeAttribute(dogmaRecord.attributes, ATTRIBUTE_IMPLANTNESS, 0), 0);
    if (implantness <= 0) {
        return null;
    }
    return {
...
function getSkillLevel(skillRecord) {
    return Math.max(0, Math.min(5, toInt(skillRecord && (skillRecord.effectiveSkillLevel ??
...
function isItemInPilotLocation(item, session) {
    ...
    for (let depth = 0; depth < 8 && current; depth += 1) {
        ...
        if (pilotLocationIDs.has(locationID)) {
            return !(locationID === activeShipID &&
...
function buildImplantRecord(item, typeInfo, options = {}) {
    const installedAt = String(typeof options.nowFileTime === "bigint"
        ? options.nowFileTime
        : currentFileTime());
    return {
...
function resolveImplantItemID(implant) {
    return toInt(implant && (implant.itemID ??
        implant.implantID ??
        implant.sourceItemID), 0);
}
function validateImplantItem(characterID, itemID, options = {}) {
    ...
    const occupiedSlot = getActiveImplants(characterRecord).find((implant) => (toInt(implant && implant.slot, 0) === typeInfo.slot));
    if (occupiedSlot) {
...
function findActiveImplantByItemID(characterID, itemID) {
    ...
    const implant = normalizeExistingImplants(characterRecord).find((entry) => (resolveImplantItemID(entry) === numericItemID));
    if (!implant) {
        return {
            success: false,
            errorMsg: "IMPLANT_NOT_FOUND",
...
function installImplantItem(characterID, itemID, options = {}) {
    ...
    const nowFileTime = typeof options.nowFileTime === "bigint" ? options.nowFileTime : currentFileTime();
    ...
    const updateResult = updateCharacterRecord(characterID, (record) => {
        ...
        if (removedImplant) {
            appendCloneEvent(record, EVENT_CLONE_IMPLANT_REMOVAL, {
                ...
                slot: toInt(removedImplant.slot ??
                    removedImplant.implantness ??
                    removedImplant.implantSlot, 0),
            });
        }
        return record;
    });
...
//# sourceMappingURL=implantRuntime.js.map