...
const { normalizeActivationStateForProfile, normalizeInfoAttributesForProfile, } = require(path.join(__dirname, "./dogmaInfoCompatibility"));
...
const { getCharacterRecord, getActiveShipRecord, findCharacterShip, activateShipForSession, buildItemChangePayload, buildChargeSublocationItem, syncInventoryItemForSession, syncChargeGodmaPrimeForSession, syncChargeSublocationTransitionForSession, syncShipFittingStateForSession, buildChargeDogmaPrimeEntry, syncDamageStateAttributesForSession, } = require(path.join(__dirname, "../character/characterState"));
const { getShipConditionState, normalizeShipConditionState, ITEM_FLAGS, SHIP_CATEGORY_ID, getItemMutationVersion, ensureFreeStationFuelSupply, findCharacterShipByType, findItemById, grantItemToCharacterLocation, grantItemToOwnerLocation, listContainerItems, moveItemToLocation, removeInventoryItem, syncCapsuleTypeForCharacter, updateInventoryItem, updateShipItem, mergeItemStacks, consumeInventoryItemQuantity, } = require(path.join(__dirname, "../inventory/itemStore"));
const { resolveItemByTypeID, } = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const { getCharacterSkills, getCharacterSkillPointTotal, getSkillMutationVersion, SKILL_FLAG_ID, } = require(path.join(__dirname, "../skills/skillState"));
const { injectSkillbookItems, } = require(path.join(__dirname, "../skills/skillbooks/skillbookRuntime"));
const { useBoosterItem, } = require(path.join(__dirname, "../skills/boosters/boosterRuntime"));
const { getLocationModifierSourcesForSystem, buildSystemWideEffectsPayloadForSystem, buildEmptySystemWideEffectsPayload, } = require(path.join(__dirname, "../exploration/wormholes/wormholeEnvironmentRuntime"));
const HackingMgrService = require(path.join(__dirname, "../exploration/hackingMgrService"));
const { getAttributeIDByNames, getFittedModuleItems, getFittedModuleByFlag, getItemModuleState, getLoadedChargeByFlag, getLoadedChargeItems, getModuleChargeCapacity, getEffectIDByNames, isFittedChargeItem, isFittedModuleItem, isModuleOnline, isEffectivelyOnlineModule, isChargeCompatibleWithModule, buildChargeTupleItemID, buildModuleStatusSnapshot, buildCharacterTargetingState, buildEffectiveItemAttributeMap, buildSkillEffectiveAttributes, getTypeDogmaAttributes, getTypeAttributeValue, getRequiredSkillRequirements, isShipFittingFlag, listHiddenModifierItems, listFittedItemsForLocation, applyModifierGroups, typeHasEffectName, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const { getShipFittingSnapshot, refreshShipFittingSnapshot, invalidateShipFittingSnapshot, listShipFittingAttributeChanges, } = require(path.join(__dirname, "../../_secondary/fitting/fittingRuntime"));
const { buildModuleAttributeChangeEvent, buildGodmaShipEffectEvent, sendOnMultiEvent, } = require(path.join(__dirname, "../_shared/godmaMultiEvent"));
const { ensureShipFittingInventoryParity, } = require(path.join(__dirname, "../fitting/fittingIntegrity"));
const { resolveCharacterIndustryAttributes, } = require(path.join(__dirname, "./brain/providers/industryBrainProvider"));
const { applyActiveImplantLocationModifiersToAttributes, applyActiveImplantAttributeBonuses, } = require(path.join(__dirname, "./implants/activeImplantModifiers"));
const { applyClientTrainingSpeedScale, } = require(path.join(__dirname, "../skills/training/skillTrainingSpeed"));
const { destroyImplantItem, installImplantItem, } = require(path.join(__dirname, "./implants/implantRuntime"));
const { ATTRIBUTE_MAX_JUMP_CLONES, ATTRIBUTE_CLONE_JUMP_COOLDOWN, getCharacterCloneLimit, getCharacterCloneJumpCooldownHours, } = require(path.join(__dirname, "../station/jumpCloneRules"));
const probeRuntimeState = require(path.join(__dirname, "../exploration/probes/probeRuntimeState"));
const probeScanRuntime = require(path.join(__dirname, "../exploration/probes/probeScanRuntime"));
const probeSceneRuntime = require(path.join(__dirname, "../exploration/probes/probeSceneRuntime"));
const interdictionProbeRuntime = require(path.join(__dirname, "../ship/interdictionProbeRuntime"));
const warpDisruptFieldGeneratorRuntime = require(path.join(__dirname, "../ship/warpDisruptFieldGeneratorRuntime"));
const { buildBootstrapCharacterBrain, buildCharacterBrainDefinitionSet, syncCharacterDogmaState, } = require(path.join(__dirname, "./brain/characterBrainRuntime"));
const { buildWeaponBankStateDict, getShipWeaponBanks, getMasterModuleID: getWeaponBankMasterModuleID, getModulesInBank, linkWeapons: linkWeaponBanks, mergeModuleGroups, peelAndLink, unlinkModuleFromBank, linkAllWeapons: linkAllWeaponBanks, unlinkAllWeaponBanks, destroyWeaponBank, destroyWeaponBankAndNotify, } = require(path.join(__dirname, "../moduleGrouping/moduleGroupingRuntime"));
const { buildActivationHeatStateDict, } = require(path.join(__dirname, "./activationHeatState"));
const { buildWeaponDogmaAttributeOverrides, collectCharacterModifierAttributes, } = require(path.join(__dirname, "../../space/combat/weaponDogma"));
const { extractDictEntries, extractList, normalizeNumber, currentFileTime, buildList, buildKeyVal, buildPackedRow, buildMarshalReal, buildDict, buildBoundObjectResponse, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getDockedLocationID, isDockedSession, } = require(path.join(__dirname, "../structure/structureLocation"));
const { consumeStructureServiceModuleOnlineFuel, buildStructureServiceModuleEffectiveAttributeMap, isStructureDamagedForServiceOnline, isStructureServiceModuleItem, syncStructureServiceModuleState, } = require(path.join(__dirname, "../structure/structureServiceModules"));
...
function recordSpaceBootstrapTrace(session, event, details = {}) {
    ...
    return (spaceRuntime.recordSessionJumpTimingTrace(session, event, details) === true);
}
...
const ATTRIBUTE_MAX_GROUP_ONLINE = getAttributeIDByNames("maxGroupOnline") || 978;
...
const ATTRIBUTE_CLOAKING_TARGETING_DELAY = getAttributeIDByNames("cloakingTargetingDelay") || 560;
...
const ATTRIBUTE_NEXT_ACTIVATION_TIME = getAttributeIDByNames("nextActivationTime") || 1796;
const ATTRIBUTE_DAMAGE_MULTIPLIER_BONUS_MAX_TIMESTAMP = getAttributeIDByNames("damageMultiplierBonusMaxTimestamp") || 5818;
const ATTRIBUTE_DRONE_IS_AGGRESSIVE = getAttributeIDByNames("droneIsAggressive") || 1275;
const ATTRIBUTE_DRONE_FOCUS_FIRE = getAttributeIDByNames("droneFocusFire") || 1297;
const ATTRIBUTE_MODULE_REPAIR_RATE = getAttributeIDByNames("moduleRepairRate") || 1267;
const ATTRIBUTE_SHIP_BROKEN_MODULE_REPAIR_COST_MULTIPLIER = getAttributeIDByNames("shipBrokenModuleRepairCostMultiplier") || 1277;
const ATTRIBUTE_SHIP_BROKEN_REPAIR_COST_MULTIPLIER_BONUS = getAttributeIDByNames("shipBrokenRepairCostMultiplierBonus") || 1294;
const ATTRIBUTE_MODULE_REPAIR_RATE_BONUS = getAttributeIDByNames("moduleRepairRateBonus") || 1295;
...
const GROUP_INTERDICTION_SPHERE_LAUNCHER = interdictionProbeRuntime.GROUP_INTERDICTION_SPHERE_LAUNCHER;
const GROUP_WARP_DISRUPT_FIELD_GENERATOR = warpDisruptFieldGeneratorRuntime.GROUP_WARP_DISRUPT_FIELD_GENERATOR;
...
const ATTRIBUTE_VIRUS_COHERENCE = getAttributeIDByNames("virusCoherence") || 1909;
const ATTRIBUTE_VIRUS_STRENGTH = getAttributeIDByNames("virusStrength") || 1910;
const ATTRIBUTE_VIRUS_ELEMENT_SLOTS = getAttributeIDByNames("virusElementSlots", "virusUtilityElementSlots") || 1911;
const ATTRIBUTE_ACCESS_DIFFICULTY_BONUS = getAttributeIDByNames("accessDifficultyBonus") || 902;
...
const ATTRIBUTE_STRUCTURE_FULL_POWER_HP_MULTIPLIER = getAttributeIDByNames("structureFullPowerStateHitpointMultiplier") || 2743;
const ATTRIBUTE_HIDDEN_ARMOR_HP_MULTIPLIER = getAttributeIDByNames("hiddenArmorHPMultiplier") || 2751;
const ATTRIBUTE_STRUCTURE_POWER_STATE_ARMOR_PLATING_MULTIPLIER = getAttributeIDByNames("structurePowerStateArmorPlatingMultiplier") || 2805;
const STRUCTURE_ACTIVATION_ATTRIBUTE_VALUES = Object.freeze({
    2: 0,
    276: 0,
    1168: 0,
    1175: 0,
...
const MODULE_ATTRIBUTE_CAPACITOR_NEED = getAttributeIDByNames("capacitorNeed") || 6;
...
const ATTRIBUTE_MODULE_REACTIVATION_DELAY = getAttributeIDByNames("moduleReactivationDelay", "reactivationDelay") || 669;
...
function getTrainedSkillLevel(skillRecord) {
    return Math.max(0, Number(skillRecord &&
        (skillRecord.effectiveSkillLevel ??
            skillRecord.trainedSkillLevel ??
            skillRecord.skillLevel)) || 0);
}
function getMissingRequiredSkill(characterID, typeID) {
    const skillLevels = new Map(getCharacterSkills(Number(characterID) || 0).map((skillRecord) => [
        Number(skillRecord && skillRecord.typeID) || 0,
        getTrainedSkillLevel(skillRecord),
    ]));
    return getRequiredSkillRequirements(Number(typeID) || 0).find((requirement) => {
        const skillTypeID = Number(requirement && requirement.skillTypeID) || 0;
        const requiredLevel = Math.max(1, Number(requirement && (requirement.requiredLevel ?? requirement.level)) || 1);
        return skillTypeID > 0 && (skillLevels.get(skillTypeID) || 0) < requiredLevel;
    }) || null;
}
...
function resolveNewbieShipTypeID(session, characterRecord = null) {
    return resolveRookieShipTypeID(session, characterRecord || getCharacterRecord(session && session.characterID) || {});
}
function boardNewbieShipForSession(session, options = {}) {
    ...
    if (boardResult &&
        ...
        log.info(`[DogmaIM] ${String(options.logLabel || "BoardNewbieShip")} boarded char=${Number(session && session.characterID) || 0} ship=${boardResult.data.ship.itemID} typeID=${boardResult.data.corvetteTypeID} reusedExisting=${boardResult.data.reusedExistingShip === true}`);
    }
    return boardResult;
}
...
function isModuleTimingAttribute(attributeID) {
    ...
    return (numericAttributeID === MODULE_ATTRIBUTE_DURATION ||
        numericAttributeID === MODULE_ATTRIBUTE_SPEED);
}
function isMarshalRealDogmaAttribute(attributeID) {
    const numericAttributeID = Number(attributeID) || 0;
    return (isModuleTimingAttribute(numericAttributeID) ||
...
function summarizeDogmaLogValue(value) {
    ...
    if (value && typeof value === "object") {
        if (Array.isArray(value.entries) ||
            ...
            return JSON.parse(JSON.stringify(value, (key, entryValue) => (typeof entryValue === "bigint"
                ? entryValue.toString()
                : entryValue)));
        }
...
function buildAmmoLoadRequest(value) {
    ...
    if (value && typeof value === "object") {
        ...
        if (Array.isArray(source.itemID)) {
            ...
            itemID = Math.trunc(normalizeNumber(source.itemID ??
                source.chargeItemID ??
                source.chargeID, 0));
            typeID = Math.trunc(normalizeNumber(source.typeID ??
                source.chargeTypeID ??
                source.ammoTypeID, 0));
            quantity = Math.trunc(normalizeNumber(source.quantity ??
                source.qty ??
                source.chargeQty ??
                source.stacksize, 0)) || null;
        }
...
function summarizeAmmoLoadRequests(requests = []) {
    return requests.map((request) => (request.itemID > 0
        ? `item:${request.itemID}`
...
function normalizeReloadSourceItemIDs(rawItemIDs = []) {
    return [...new Set((Array.isArray(rawItemIDs) ? rawItemIDs : [rawItemIDs])
...
function typeHasDogmaAttribute(typeID, attributeID) {
    ...
    return Object.prototype.hasOwnProperty.call(attributes || {}, String(attributeID));
}
function resolveMaxGroupOnlineLimit(moduleItem, fittedItems = []) {
    ...
    const onlineGroupCount = (Array.isArray(fittedItems) ? fittedItems : [])
        .filter((fittedItem) => Number(fittedItem && fittedItem.groupID) === groupID &&
        isEffectivelyOnlineModule(fittedItem)).length;
    return {
        groupID,
        maxGroupOnline,
        onlineGroupCount,
...
function resolveFrontierCreationDogmaContext(shipMetadata, charID, session) {
    if (String(session && session.compatibilityProfile || "").trim().toLowerCase() !==
        "frontier" ||
        !shipMetadata ||
    ...
    const { getCreationDogmaContext } = require(path.join(__dirname, "../frontier/creationRuntime"));
    const result = getCreationDogmaContext(shipMetadata, charID);
    if (!result || result.success !== true) {
        log.warn(`[DogmaIM] Creation hydration failed ship=${Number(shipMetadata.itemID) || 0} ` +
...
function setFrontierCreationModuleOnlineStateIfHandled(shipMetadata, charID, moduleID, online, session, dependencies = {}) {
    if (String(session && session.compatibilityProfile || "").trim().toLowerCase() !==
        "frontier" ||
        !shipMetadata ||
        Number(shipMetadata.itemID) <= 0 ||
    ...
    const getCreationTemplate = typeof dependencies.getCreationTemplate === "function"
        ? dependencies.getCreationTemplate
        : require(path.join(__dirname, "../frontier/creationStaticData")).getCreationTemplate;
    if (!getCreationTemplate(shipMetadata.typeID)) {
        return { handled: false, result: null };
    }
    const setCreationModuleOnlineState = typeof dependencies.setCreationModuleOnlineState === "function"
        ? dependencies.setCreationModuleOnlineState
        : require(path.join(__dirname, "../frontier/creationRuntime")).setCreationModuleOnlineState;
    const result = setCreationModuleOnlineState(shipMetadata, charID, moduleID, online, session);
    return {
        handled: true,
        result: result || {
            success: false,
...
class DogmaService extends BaseService {
    constructor() {
        ...
        this._onlineEffectStartTimes = new WeakMap();
    }
    _getDockedItemInfoCache(session, options = {}) {
        ...
        const cached = session._dockedDogmaItemInfoCache &&
    ...
    _buildDockedItemInfoCacheKey(requestedItemID, item = null) {
        if (Array.isArray(requestedItemID)) {
            return `tuple:${JSON.stringify(requestedItemID.map((value) => Number(value) || 0))}`;
        }
        const numericRequestedItemID = Number.parseInt(String(requestedItemID), 10) || 0;
    ...
    _cacheDockedItemInfoEntry(session, requestedItemID, item, entry) {
        ...
        const itemKey = this._buildDockedItemInfoCacheKey(Number(item && item.itemID) || 0, item);
    ...
    _resolveActiveShipLocationID(session, shipMetadata = {}, fallback = 0) {
        const metadataLocationID = Math.trunc(normalizeNumber(shipMetadata && shipMetadata.locationID, 0));
    ...
    _buildRequiredSkillLevelDict(typeID) {
        ...
        for (const requirement of getRequiredSkillRequirements(typeID)) {
            ...
            const requiredLevel = Math.max(1, this._normalizeTypeID(requirement && (requirement.requiredLevel || requirement.level)));
            if (skillTypeID <= 0) {
                continue;
            }
            levelsBySkillID.set(skillTypeID, Math.max(levelsBySkillID.get(skillTypeID) || 0, requiredLevel));
        }
        return buildDict([...levelsBySkillID.entries()].sort((left, right) => left[0] - right[0]));
    }
    _getCharID(session) {
        return resolveSessionCharacterID(session);
    }
    _isControllingStructureSession(session) {
        const structureID = Number(session && (session.structureID || session.structureid)) || 0;
    ...
    _getDogmaInventoryOwnerID(session, locationID) {
        const structureOwnerID = this._getControlledStructureInventoryOwnerID(session, locationID);
        return structureOwnerID > 0 ? structureOwnerID : this._getCharID(session);
    }
    _getShipID(session) {
        if (this._isControllingStructureSession(session)) {
            return (session &&
                (session.shipID || session.shipid)) || 140000101;
        }
        return (session &&
            (session.activeShipID || session.shipID || session.shipid)) || 140000101;
    }
    ...
    _getDogmaAttributeDefaultValue(attributeID, fallback = 0) {
        ...
        const attributeTypesByID = typeDogma && typeDogma.attributeTypesByID &&
    ...
    _getCharacterSkillEffectiveAttributes(charID, skillTypeID) {
        ...
        const skillRecord = numericCharID > 0
            ? getCharacterSkills(numericCharID).find((skill) => Number(skill && skill.typeID) === numericSkillTypeID)
            : null;
    ...
    _resolveCharacterNaniteRepairAttributes(charID) {
        const baseRepairRate = this._getDogmaAttributeDefaultValue(ATTRIBUTE_MODULE_REPAIR_RATE, 10);
        const baseRepairCostMultiplier = this._getDogmaAttributeDefaultValue(ATTRIBUTE_SHIP_BROKEN_MODULE_REPAIR_COST_MULTIPLIER, 0.5);
        const operationAttributes = this._getCharacterSkillEffectiveAttributes(charID, SKILL_NANITE_OPERATION);
        const interfacingAttributes = this._getCharacterSkillEffectiveAttributes(charID, SKILL_NANITE_INTERFACING);
        const repairCostBonus = Number(operationAttributes[ATTRIBUTE_SHIP_BROKEN_REPAIR_COST_MULTIPLIER_BONUS]) || 0;
        const repairRateBonus = Number(interfacingAttributes[ATTRIBUTE_MODULE_REPAIR_RATE_BONUS]) || 0;
        return {
            moduleRepairRate: Math.max(0.01, baseRepairRate * (1 + repairRateBonus / 100)),
            repairCostMultiplier: Math.max(0.01, baseRepairCostMultiplier * (1 + repairCostBonus / 100)),
        };
    }
    _buildStructureControlShipMetadata(structure = null) {
        ...
        const fullPowerDogma = effectiveHitpoints.fullPowerDogma || {};
        return {
            itemID: Number(structure.structureID) || 0,
            typeID,
            ownerID: Number(structure.ownerCorpID || structure.ownerID) || 0,
            // TQ keeps the controlled structure's canonical inventory identity. It
            // becomes the active shipID without becoming its own inventory location.
            locationID: Number(structure.solarSystemID || structure.locationID) || 0,
            flagID: 0,
            quantity: -1,
            singleton: 1,
            stacksize: 1,
            ...
            customInfo: "",
            mass: Number(structure.mass ?? itemType.mass) || 0,
            volume: Number(structure.volume ?? itemType.volume) || 0,
            capacity: Number(structure.capacity ?? itemType.capacity) || 0,
            radius: Number(structure.radius ?? itemType.radius) || 0,
            ...
            dogmaAttributeOverrides: {
                [ATTRIBUTE_STRUCTURE_FULL_POWER_HP_MULTIPLIER]: Number(fullPowerDogma.hitpointMultiplier) || 1,
                [ATTRIBUTE_HIDDEN_ARMOR_HP_MULTIPLIER]: Number(fullPowerDogma.hiddenArmorHPMultiplier) || 1,
                [ATTRIBUTE_STRUCTURE_POWER_STATE_ARMOR_PLATING_MULTIPLIER]: Number(fullPowerDogma.armorPlatingMultiplier) || 0,
            },
            conditionState: structure && structure.conditionState && typeof structure.conditionState === "object"
    ...
    _getControlledStructureShipMetadata(session) {
        ...
        return this._getStructureShipMetadataByID(session && (session.structureID || session.structureid));
    }
    _getStructureShipMetadataByID(structureID) {
        ...
        return this._buildStructureControlShipMetadata(worldData.getStructureByID(numericStructureID) || null);
    }
    _normalizeControlledStructureFittingOwners(shipContext) {
        if (!shipContext || shipContext.controllingStructure !== true) {
            return 0;
        }
        const shipID = Number(shipContext.shipID) || 0;
        const ownerID = Number(shipContext.shipMetadata && shipContext.shipMetadata.ownerID) || 0;
        if (shipID <= 0 || ownerID <= 0) {
            return 0;
        }
        let normalizedCount = 0;
        for (const item of listFittedItemsForLocation(shipID)) {
            if (!item || Number(item.ownerID) === ownerID) {
                continue;
            }
            ...
            if (updateResult.success) {
                ...
                log.warn(`[DogmaIM] Failed to normalize controlled structure fitting owner ` +
                    `structure=${shipID} item=${Number(item.itemID) || 0} ` +
                    `error=${updateResult.errorMsg || "UNKNOWN_ERROR"}`);
            }
        }
        if (normalizedCount > 0) {
            log.info(`[DogmaIM] Normalized controlled structure fitting owners ` +
                `structure=${shipID} owner=${ownerID} items=${normalizedCount}`);
        }
    ...
    _getShipMetadata(session) {
        ...
        return (resolveShipByTypeID(shipTypeID) || {
            typeID: shipTypeID,
    ...
    _getPersistedDroneSettingAttributes(session) {
        ...
        const storedSettings = characterRecord.droneSettings &&
    ...
    _persistDroneSettingChanges(session, droneSettingChanges = {}) {
        ...
        const writeResult = require("../character/characterState").writeCharacterRecord(characterID, nextCharacterRecord, { silent: true });
        if (!writeResult.success) {
            log.warn(`[DogmaService] Failed to persist drone settings for char=${characterID}: ${writeResult.errorMsg || "WRITE_ERROR"}`);
            return this._getPersistedDroneSettingAttributes(session);
        }
        return nextDroneSettings;
    }
    _buildDroneSettingAttributesPayload(session) {
        return buildDict(Object.entries(this._getPersistedDroneSettingAttributes(session)).map(([attributeID, value]) => [
    ...
    _getLocationID(session) {
        return ((getDockedLocationID(session) || (session && (session.locationid || session.solarsystemid2 || session.solarsystemid))) ||
            60003760);
    }
    _nowFileTime() {
    ...
    _toFileTime(value, fallback = null) {
        const fallbackValue = typeof fallback === "bigint" ? fallback : this._nowFileTime();
        const numericValue = Number(value);
        if (!Number.isFinite(numericValue) || numericValue <= 0) {
    ...
    _buildInvRow({ itemID, typeID, ownerID, locationID, flagID, groupID, categoryID, quantity = -1, singleton = 1, stacksize = 1, customInfo = "", preserveNullCustomInfo = false, }) {
        const normalizedQuantity = Number.isFinite(Number(quantity))
            ? quantity
            : -1;
        const normalizedSingleton = singleton === null || singleton === undefined
            ? (normalizedQuantity === -1 ? 1 : 0)
            : singleton;
        const normalizedStacksize = stacksize === null || stacksize === undefined
            ? (normalizedSingleton === 1
                ? 1
                : (normalizedQuantity === -1 ? 0 : normalizedQuantity))
            : stacksize;
        const normalizedCustomInfo = preserveNullCustomInfo && customInfo == null
            ? null
            : customInfo === null || customInfo === undefined
                ? ""
                : customInfo;
        return buildPackedRow(INVENTORY_ROW_DESCRIPTOR_COLUMNS, {
            itemID,
            typeID,
            ownerID,
            locationID,
    ...
    _buildCommonGetInfoEntry({ itemID, typeID, ownerID, locationID, flagID, groupID, categoryID, quantity = -1, singleton = 1, stacksize = 1, customInfo = "", preserveNullCustomInfo = false, attributes = null, activeEffects = null, invItem, session = null, }) {
        const resolvedInvItem = invItem === undefined
            ? this._buildInvRow({
                ...
                stacksize,
                customInfo,
                preserveNullCustomInfo,
            })
            : invItem;
        // TQ sends the simulation and wall clocks independently. They are close
        // while docked, but are not the same field and can diverge under TiDi.
        const time = session ? this._sessionFileTime(session) : this._nowFileTime();
        let wallclockTime = this._nowFileTime();
        if (typeof time === "bigint" && wallclockTime === time) {
            // Date.now() only has millisecond precision; retain distinct clock
            // semantics at the 100ns FILETIME precision used by the wire payload.
            wallclockTime = time - 1n;
        }
        const resolvedAttributes = normalizeInfoAttributesForProfile(attributes || { type: "dict", entries: [] }, time, session && session.compatibilityProfile);
        const entries = [
            ["itemID", itemID],
    ...
    _buildStatusRow({ itemID, online = false, damage = 0.0, charge = 0.0, skillPoints = 0, armorDamage = 0.0, shieldCharge = 0.0, incapacitated = false, }) {
        return {
            type: "object",
            name: "util.Row",
            args: {
    ...
    _buildPackedInstanceRow({ itemID, online = false, damage = 0.0, charge = 0.0, skillPoints = 0, armorDamage = 0.0, shieldCharge = 0.0, incapacitated = false, }) {
        return {
            type: "packedrow",
    ...
    _buildCharacterAttributes(charData = {}, characterID = null) {
        ...
        const charID = Number(characterID ?? charData.characterID ?? charData.charID ?? charData.charid ?? 0) || 0;
        const securityStatus = Number(charData.securityStatus ?? charData.securityRating ?? source.securityStatus ?? 0);
        const characterTargetingState = buildCharacterTargetingState(charID, {
            characterAttributes: source,
        });
        const industryAttributes = resolveCharacterIndustryAttributes(charID);
        const naniteRepairAttributes = this._resolveCharacterNaniteRepairAttributes(charID);
        ...
        return applyClientTrainingSpeedScale(applyActiveImplantAttributeBonuses({
            ...
            [ATTRIBUTE_INTELLIGENCE]: Number(source[ATTRIBUTE_INTELLIGENCE] ?? source.intelligence ?? 20),
            [ATTRIBUTE_MEMORY]: Number(source[ATTRIBUTE_MEMORY] ?? source.memory ?? 20),
            [ATTRIBUTE_PERCEPTION]: Number(source[ATTRIBUTE_PERCEPTION] ?? source.perception ?? 20),
            [ATTRIBUTE_WILLPOWER]: Number(source[ATTRIBUTE_WILLPOWER] ?? source.willpower ?? 20),
            [ATTRIBUTE_MAX_LOCKED_TARGETS]: Number(characterTargetingState.maxLockedTargets ?? source[ATTRIBUTE_MAX_LOCKED_TARGETS] ?? 0),
            [ATTRIBUTE_MAX_JUMP_CLONES]: Number(source[ATTRIBUTE_MAX_JUMP_CLONES] ?? getCharacterCloneLimit(charID)),
            [ATTRIBUTE_CLONE_JUMP_COOLDOWN]: Number(source[ATTRIBUTE_CLONE_JUMP_COOLDOWN] ?? getCharacterCloneJumpCooldownHours(charID)),
            [ATTRIBUTE_MANUFACTURE_SLOT_LIMIT]: Number(source[ATTRIBUTE_MANUFACTURE_SLOT_LIMIT] ??
                industryAttributes[ATTRIBUTE_MANUFACTURE_SLOT_LIMIT]),
            [ATTRIBUTE_MANUFACTURE_TIME_MULTIPLIER]: Number(source[ATTRIBUTE_MANUFACTURE_TIME_MULTIPLIER] ??
                industryAttributes[ATTRIBUTE_MANUFACTURE_TIME_MULTIPLIER]),
            [ATTRIBUTE_MANUFACTURING_TIME_RESEARCH_SPEED]: Number(source[ATTRIBUTE_MANUFACTURING_TIME_RESEARCH_SPEED] ??
                industryAttributes[ATTRIBUTE_MANUFACTURING_TIME_RESEARCH_SPEED]),
            [ATTRIBUTE_COPY_SPEED_PERCENT]: Number(source[ATTRIBUTE_COPY_SPEED_PERCENT] ??
                industryAttributes[ATTRIBUTE_COPY_SPEED_PERCENT]),
            [ATTRIBUTE_MINERAL_NEED_RESEARCH_SPEED]: Number(source[ATTRIBUTE_MINERAL_NEED_RESEARCH_SPEED] ??
                industryAttributes[ATTRIBUTE_MINERAL_NEED_RESEARCH_SPEED]),
            [ATTRIBUTE_MAX_LABORATORY_SLOTS]: Number(source[ATTRIBUTE_MAX_LABORATORY_SLOTS] ??
                industryAttributes[ATTRIBUTE_MAX_LABORATORY_SLOTS]),
            [ATTRIBUTE_INVENTION_RESEARCH_SPEED]: Number(source[ATTRIBUTE_INVENTION_RESEARCH_SPEED] ??
                industryAttributes[ATTRIBUTE_INVENTION_RESEARCH_SPEED]),
            [ATTRIBUTE_REACTION_TIME_MULTIPLIER]: Number(source[ATTRIBUTE_REACTION_TIME_MULTIPLIER] ??
                industryAttributes[ATTRIBUTE_REACTION_TIME_MULTIPLIER]),
            [ATTRIBUTE_REACTION_SLOT_LIMIT]: Number(source[ATTRIBUTE_REACTION_SLOT_LIMIT] ??
                industryAttributes[ATTRIBUTE_REACTION_SLOT_LIMIT]),
            [ATTRIBUTE_MODULE_REPAIR_RATE]: Number(source[ATTRIBUTE_MODULE_REPAIR_RATE] ??
                naniteRepairAttributes.moduleRepairRate),
            [ATTRIBUTE_SHIP_BROKEN_MODULE_REPAIR_COST_MULTIPLIER]: Number(source[ATTRIBUTE_SHIP_BROKEN_MODULE_REPAIR_COST_MULTIPLIER] ??
    ...
    _buildCharacterBaseAttributes(charData = {}) {
        ...
        const source = charData.characterAttributes && typeof charData.characterAttributes === "object"
            ? charData.characterAttributes
            : {};
        const attributes = Object.fromEntries(Object.entries(getTypeDogmaAttributes(typeID))
    ...
    _buildShipModifiedCharacterAttributes(charData = {}, characterID = null, session = null, options = {}) {
        const charID = Number(characterID ?? charData.characterID ?? charData.charID ?? charData.charid ?? 0) || 0;
        const source = charData.characterAttributes && typeof charData.characterAttributes === "object"
            ? charData.characterAttributes
            : {};
        ...
        const directCharacterModifierEntries = buildCharacterBrainDefinitionSet(charID)
            .characterEffects
            .filter((effectDefinition) => String(effectDefinition && effectDefinition.modifierType || "M") === "M")
            ...
            .filter((modifierEntry) => modifierEntry.modifiedAttributeID > 0 &&
        ...
        const sessionShipID = Number(session && (session.activeShipID ?? session.shipID ?? session.shipid)) || 0;
        const activeShip = (sessionShipID > 0 && findCharacterShip(charID, sessionShipID)) ||
        ...
        if (activeShip) {
            const fittingSnapshot = getShipFittingSnapshot(charID, activeShip.itemID, {
                ...
                ...this._getFittingContextSnapshotOptions(options.fittingContext, charID, activeShip.itemID),
            });
            if (fittingSnapshot) {
                const ownerModifierAttributes = collectCharacterModifierAttributes(fittingSnapshot.skillMap, fittingSnapshot.fittedItems, fittingSnapshot.assumedActiveModuleContexts);
                for (const [attributeID, value] of Object.entries(ownerModifierAttributes || {})) {
                    const numericAttributeID = Number(attributeID);
                    const numericValue = Number(value);
                    if (!Number.isInteger(numericAttributeID) || !Number.isFinite(numericValue)) {
                        continue;
        ...
        const securityStatus = Number(charData.securityStatus ?? charData.securityRating ?? source.securityStatus ?? 0);
        const naniteRepairAttributes = this._resolveCharacterNaniteRepairAttributes(charID);
        attributes[ATTRIBUTE_MAX_LOCKED_TARGETS] = Number(characterTargetingState.maxLockedTargets ?? attributes[ATTRIBUTE_MAX_LOCKED_TARGETS] ?? 0);
        attributes[ATTRIBUTE_MAX_JUMP_CLONES] = Number(source[ATTRIBUTE_MAX_JUMP_CLONES] ??
            attributes[ATTRIBUTE_MAX_JUMP_CLONES] ??
            getCharacterCloneLimit(charID));
        attributes[ATTRIBUTE_CLONE_JUMP_COOLDOWN] = Number(source[ATTRIBUTE_CLONE_JUMP_COOLDOWN] ??
            attributes[ATTRIBUTE_CLONE_JUMP_COOLDOWN] ??
            getCharacterCloneJumpCooldownHours(charID));
        attributes[ATTRIBUTE_MANUFACTURE_SLOT_LIMIT] = Number(source[ATTRIBUTE_MANUFACTURE_SLOT_LIMIT] ??
            industryAttributes[ATTRIBUTE_MANUFACTURE_SLOT_LIMIT] ??
            attributes[ATTRIBUTE_MANUFACTURE_SLOT_LIMIT] ??
            0);
        attributes[ATTRIBUTE_MANUFACTURE_TIME_MULTIPLIER] = Number(source[ATTRIBUTE_MANUFACTURE_TIME_MULTIPLIER] ??
            industryAttributes[ATTRIBUTE_MANUFACTURE_TIME_MULTIPLIER] ??
            attributes[ATTRIBUTE_MANUFACTURE_TIME_MULTIPLIER] ??
            0);
        attributes[ATTRIBUTE_MANUFACTURING_TIME_RESEARCH_SPEED] = Number(source[ATTRIBUTE_MANUFACTURING_TIME_RESEARCH_SPEED] ??
            industryAttributes[ATTRIBUTE_MANUFACTURING_TIME_RESEARCH_SPEED] ??
            attributes[ATTRIBUTE_MANUFACTURING_TIME_RESEARCH_SPEED] ??
            0);
        attributes[ATTRIBUTE_COPY_SPEED_PERCENT] = Number(source[ATTRIBUTE_COPY_SPEED_PERCENT] ??
            industryAttributes[ATTRIBUTE_COPY_SPEED_PERCENT] ??
            attributes[ATTRIBUTE_COPY_SPEED_PERCENT] ??
            0);
        attributes[ATTRIBUTE_MINERAL_NEED_RESEARCH_SPEED] = Number(source[ATTRIBUTE_MINERAL_NEED_RESEARCH_SPEED] ??
            industryAttributes[ATTRIBUTE_MINERAL_NEED_RESEARCH_SPEED] ??
            attributes[ATTRIBUTE_MINERAL_NEED_RESEARCH_SPEED] ??
            0);
        attributes[ATTRIBUTE_MAX_LABORATORY_SLOTS] = Number(source[ATTRIBUTE_MAX_LABORATORY_SLOTS] ??
            industryAttributes[ATTRIBUTE_MAX_LABORATORY_SLOTS] ??
            attributes[ATTRIBUTE_MAX_LABORATORY_SLOTS] ??
            0);
        attributes[ATTRIBUTE_INVENTION_RESEARCH_SPEED] = Number(source[ATTRIBUTE_INVENTION_RESEARCH_SPEED] ??
            industryAttributes[ATTRIBUTE_INVENTION_RESEARCH_SPEED] ??
            attributes[ATTRIBUTE_INVENTION_RESEARCH_SPEED] ??
            0);
        attributes[ATTRIBUTE_REACTION_TIME_MULTIPLIER] = Number(source[ATTRIBUTE_REACTION_TIME_MULTIPLIER] ??
            industryAttributes[ATTRIBUTE_REACTION_TIME_MULTIPLIER] ??
            attributes[ATTRIBUTE_REACTION_TIME_MULTIPLIER] ??
            0);
        attributes[ATTRIBUTE_REACTION_SLOT_LIMIT] = Number(source[ATTRIBUTE_REACTION_SLOT_LIMIT] ??
            industryAttributes[ATTRIBUTE_REACTION_SLOT_LIMIT] ??
            attributes[ATTRIBUTE_REACTION_SLOT_LIMIT] ??
            0);
        attributes[ATTRIBUTE_MODULE_REPAIR_RATE] = Number(source[ATTRIBUTE_MODULE_REPAIR_RATE] ??
            attributes[ATTRIBUTE_MODULE_REPAIR_RATE] ??
            naniteRepairAttributes.moduleRepairRate);
        attributes[ATTRIBUTE_SHIP_BROKEN_MODULE_REPAIR_COST_MULTIPLIER] = Number(source[ATTRIBUTE_SHIP_BROKEN_MODULE_REPAIR_COST_MULTIPLIER] ??
    ...
    _buildShipModifiedCharacterAttributeDict(charData = {}, characterID = null, session = null, options = {}) {
        const attributes = this._buildShipModifiedCharacterAttributes(charData, characterID, session, options);
        return {
            type: "dict",
    ...
    _getShipRuntimeAttributeOverrides(session, shipData = {}) {
        ...
        const activeShipID = Number(shipData.itemID ??
            shipData.shipID ??
            this._getShipID(session)) || 0;
    ...
    _getPropulsionModuleAttributeOverrides(item, session) {
        ...
        const runtimeAttributes = spaceRuntime.getPropulsionModuleRuntimeAttributes(this._getCharID(session), item, {
            activeModuleContexts: spaceRuntime &&
    ...
    _getGenericModuleAttributeOverrides(item, session, options = {}) {
        ...
        const chargeItem = this._getFittingContextChargeByFlag(options.fittingContext, charID, shipItem.itemID, item.flagID) ||
            getLoadedChargeByFlag(charID, shipItem.itemID, item.flagID);
        const activeModuleContexts = spaceRuntime &&
            typeof spaceRuntime.getActiveModuleContextsForSession === "function"
            ? spaceRuntime.getActiveModuleContextsForSession(session, {
                excludeModuleID: Number(item && item.itemID) || 0,
                includeOverloadModuleID: Number(item && item.itemID) || 0,
            ...
            : [];
        const additionalLocationModifierSources = getLocationModifierSourcesForSystem(session &&
            (session.solarsystemid2 ||
                session.solarsystemid ||
                (session._space && session._space.systemID) ||
                0));
        const fittingSnapshotOptions = this._getFittingContextSnapshotOptions(options.fittingContext, charID, shipItem.itemID);
        const runtimeAttributes = spaceRuntime.getGenericModuleRuntimeAttributes(charID, shipItem, item, chargeItem, null, {
    ...
    _buildScannerProbeLauncherRuntimeAttributeMap(item, session) {
        ...
        if (Number.isFinite(Number(runtimeAttributes.capNeed))) {
            attributes[MODULE_ATTRIBUTE_CAPACITOR_NEED] = Number(runtimeAttributes.capNeed);
        }
        const durationAttributeID = Number(runtimeAttributes.durationAttributeID) || MODULE_ATTRIBUTE_DURATION;
        if (Number.isFinite(Number(runtimeAttributes.durationMs))) {
            attributes[durationAttributeID] = Number(runtimeAttributes.durationMs);
        }
        const attributeOverrides = runtimeAttributes.attributeOverrides &&
            typeof runtimeAttributes.attributeOverrides === "object"
            ? runtimeAttributes.attributeOverrides
            : null;
        if (attributeOverrides &&
            Number.isFinite(Number(attributeOverrides[MODULE_ATTRIBUTE_SPEED]))) {
            attributes[MODULE_ATTRIBUTE_SPEED] = Number(attributeOverrides[MODULE_ATTRIBUTE_SPEED]);
        }
        const reloadRuntimeAttributes = this._getModuleReloadAttributeOverrides(item, session);
    ...
    _syncScannerProbeLauncherRuntimeAttributes(session, moduleItem, options = {}) {
        if (!session || typeof session.sendNotification !== "function" || !moduleItem) {
            return 0;
        }
        const runtimeAttributes = this._buildScannerProbeLauncherRuntimeAttributeMap(moduleItem, session);
        if (!runtimeAttributes) {
            return 0;
        }
    ...
    _refreshScannerProbeLauncherClientState(session, shipID, moduleItem, options = {}) {
        if (!session ||
            !session._space ||
            !moduleItem ||
    ...
    _resolveLoadedChargeItem(item, session = null, options = {}) {
        ...
        const contextCharge = this._getFittingContextChargeByFlag(options.fittingContext, charID, shipID, flagID);
        if (contextCharge) {
    ...
    _getWeaponDogmaAttributeOverrides(item, session = null, options = {}) {
        ...
        const shipItem = findCharacterShip(characterID, shipID) ||
            (activeShipRecord &&
        ...
        const moduleItem = isChargeItem
            ? (this._getFittingContextModuleByFlag(options.fittingContext, characterID, shipID, item.flagID) ||
        ...
        return this._getWeaponDogmaAttributeOverridesForCharge(moduleItem, chargeItem, session, options);
    }
    _getWeaponDogmaAttributeOverridesForCharge(moduleItem, chargeItem = null, session = null, options = {}) {
        ...
        return buildWeaponDogmaAttributeOverrides({
            ...
            moduleItem,
            chargeItem,
            ...this._getFittingContextSnapshotOptions(options.fittingContext, characterID, shipID),
        });
    }
    _buildWeaponModuleAttributeMap(moduleItem, chargeItem = null, session = null) {
        const weaponDogmaAttributes = this._getWeaponDogmaAttributeOverridesForCharge(moduleItem, chargeItem, session);
        const moduleAttributes = weaponDogmaAttributes &&
        ...
        return Object.fromEntries(Object.entries(moduleAttributes)
            .map(([attributeID, value]) => [Number(attributeID), Number(value)])
            .filter(([attributeID, value]) => Number.isInteger(attributeID) && Number.isFinite(value)));
    }
    ...
    _buildShipAttributes(charData = {}, shipData = {}, session = null, options = {}) {
        const securityStatus = Number(charData.securityStatus ??
        ...
        const fittingSnapshot = getShipFittingSnapshot(numericCharID, shipData && shipData.itemID, {
            shipItem: shipData,
            reason: "dogma.ship-attrs",
            ...this._getFittingContextSnapshotOptions(options.fittingContext, numericCharID, shipData && shipData.itemID),
        });
        const attributes = fittingSnapshot
            ? { ...fittingSnapshot.shipAttributes }
            : {};
        const creationDogmaContext = Object.prototype.hasOwnProperty.call(options, "creationDogmaContext")
            ? options.creationDogmaContext
            : resolveFrontierCreationDogmaContext(shipData, numericCharID, session);
        const creationShipAttributeModifierEntries = creationDogmaContext &&
            Array.isArray(creationDogmaContext.shipAttributeModifierEntries)
            ? creationDogmaContext.shipAttributeModifierEntries
            : [];
        const runtimeAttributeOverrides = this._getShipRuntimeAttributeOverrides(session, shipData);
        const shipTypeID = Number(shipData.typeID);
        const shipMetadata = Number.isInteger(shipTypeID) && shipTypeID > 0
        ...
        if (runtimeAttributeOverrides) {
            ...
            attributes[ATTRIBUTE_MAX_VELOCITY] = Number(runtimeAttributeOverrides.maxVelocity);
            attributes[ATTRIBUTE_MAX_TARGET_RANGE] = Number(runtimeAttributeOverrides.maxTargetRange);
            attributes[ATTRIBUTE_MAX_LOCKED_TARGETS] = Number(runtimeAttributeOverrides.maxLockedTargets);
            attributes[ATTRIBUTE_SIGNATURE_RADIUS] = Number(runtimeAttributeOverrides.signatureRadius);
            attributes[ATTRIBUTE_CLOAKING_TARGETING_DELAY] = Number(runtimeAttributeOverrides.cloakingTargetingDelay);
            attributes[ATTRIBUTE_SCAN_RESOLUTION] = Number(runtimeAttributeOverrides.scanResolution);
        }
        ...
        const runtimeAlreadyIncludesCreationModifiers = Boolean(runtimeAttributeOverrides &&
        ...
        if (Number(attributes[ATTRIBUTE_HEAT_CAPACITY]) > 0 &&
            Number(attributes[ATTRIBUTE_HEAT_CONDUCTANCE]) > 0) {
            const externalTemperature = Number(shipCondition.externalTemperature ??
                attributes[ATTRIBUTE_EXTERNAL_TEMPERATURE]);
            const temperature = Number(shipCondition.temperature ?? attributes[ATTRIBUTE_TEMPERATURE]);
            attributes[ATTRIBUTE_EXTERNAL_TEMPERATURE] =
        ...
        if (Number.isFinite(shieldCapacity) &&
            shieldCapacity >= 0 &&
            Number.isFinite(shipCondition.shieldCharge)) {
            attributes[ATTRIBUTE_SHIELD_CHARGE_HELPER] = Number((shieldCapacity * shipCondition.shieldCharge).toFixed(6));
        }
        const armorHP = Number(attributes[ATTRIBUTE_ARMOR_HP]);
        if (Number.isFinite(armorHP) &&
            armorHP >= 0 &&
            Number.isFinite(shipCondition.armorDamage)) {
            attributes[ATTRIBUTE_ARMOR_DAMAGE] = Number((armorHP * shipCondition.armorDamage).toFixed(6));
        }
        const structureHP = Number(attributes[ATTRIBUTE_STRUCTURE_HP]);
        if (Number.isFinite(structureHP) &&
            structureHP >= 0 &&
            ...
            attributes[ATTRIBUTE_ITEM_DAMAGE] = Number((structureHP * shipCondition.damage).toFixed(6));
        }
        ...
        if (Number.isFinite(capacitorCapacity) &&
            capacitorCapacity > 0 &&
            Number.isFinite(shipCondition.charge)) {
            attributes[ATTRIBUTE_CHARGE] = Number((capacitorCapacity * shipCondition.charge).toFixed(6));
        }
    ...
    _buildShipAttributeDict(charData = {}, shipData = {}, session = null, options = {}) {
        const attributes = this._buildShipAttributes(charData, shipData, session, options);
    ...
    _getDroneBayItemAttributeOverrides(item, session = null) {
        ...
        const scene = spaceRuntime &&
            typeof spaceRuntime.getSceneForSession === "function"
            ? spaceRuntime.getSceneForSession(session)
            : null;
        const runtimeShipEntity = scene && typeof scene.getShipEntityForSession === "function"
            ? scene.getShipEntityForSession(session)
            : null;
        const characterRecord = this._getCharacterRecord(session) || {};
        const systemID = Number((session && (session.solarsystemid2 ||
            session.solarsystemid ||
            (session._space && session._space.systemID))) ||
        ...
        const controllerEntity = {
            ...
            session,
            systemID,
            activeModuleEffects: runtimeShipEntity &&
        ...
        const { resolveDroneOperationalAttributes, } = require(path.join(__dirname, "../drone/droneDogma"));
    ...
    _buildInventoryItemAttributes(item, session = null, options = {}) {
        ...
        const attributes = Object.fromEntries(Object.entries(typeAttributes || {})
            .map(([attributeID, value]) => [Number(attributeID), Number(value)])
            .filter(([attributeID, value]) => Number.isInteger(attributeID) && Number.isFinite(value))
        ...
        // Inventory metadata and runtime structure state are authoritative over
        // base type dogma for these per-instance fields. In particular, Upwell
        // full-power hitpoint multipliers are already resolved onto the canonical
        // structure item before it reaches this builder.
        for (const [attributeID, value] of [
            [ATTRIBUTE_MASS, item && item.mass],
            [ATTRIBUTE_CAPACITY, item && item.capacity],
            [ATTRIBUTE_VOLUME, item && item.volume],
            [ATTRIBUTE_RADIUS, item && item.radius],
            [ATTRIBUTE_SHIELD_CAPACITY, item && item.shieldCapacity],
            [ATTRIBUTE_ARMOR_HP, item && item.armorHP],
            [ATTRIBUTE_STRUCTURE_HP, item && (item.structureHP ?? item.hullHP)],
        ]) {
            const numericValue = Number(value);
            if (Number.isFinite(numericValue)) {
                attributes[attributeID] = marshalDogmaAttributeValue(attributeID, numericValue);
            }
        }
        if (item &&
            item.dogmaAttributeOverrides &&
            typeof item.dogmaAttributeOverrides === "object") {
            for (const [attributeID, value] of Object.entries(item.dogmaAttributeOverrides)) {
                const numericAttributeID = Number(attributeID);
                const numericValue = Number(value);
                if (!Number.isInteger(numericAttributeID) ||
                    !Number.isFinite(numericValue)) {
                    continue;
                }
                attributes[numericAttributeID] = marshalDogmaAttributeValue(numericAttributeID, numericValue);
            }
        }
        const droneBayAttributeOverrides = this._getDroneBayItemAttributeOverrides(item, session);
        if (droneBayAttributeOverrides &&
        ...
        const structureServiceAttributes = this._getStructureServiceModuleAttributeOverrides(item);
        ...
        const resourceAttributeOverrides = this._getFittedModuleResourceAttributeOverrides(item, session, options);
        ...
        const weaponDogmaAttributes = this._getWeaponDogmaAttributeOverrides(item, session, options);
        const overrideAttributes = Number(item && item.categoryID) === 8
        ...
        if (quantityAttributeID && options.includeQuantityAttribute !== false) {
            attributes[quantityAttributeID] = Number(item && (item.stacksize ?? item.quantity ?? 0)) || 0;
        }
        ...
        if (item && item.conditionState && typeof item.conditionState === "object") {
            ...
            const shieldCapacity = Number(item.shieldCapacity ??
                this._resolveDogmaAttributeNumber(attributes, ATTRIBUTE_SHIELD_CAPACITY, NaN));
            if (Number.isFinite(shieldCapacity) &&
                shieldCapacity >= 0 &&
                Number.isFinite(conditionState.shieldCharge)) {
                attributes[ATTRIBUTE_SHIELD_CHARGE_HELPER] = Number((shieldCapacity * conditionState.shieldCharge).toFixed(6));
            }
            const armorHP = Number(item.armorHP ??
                this._resolveDogmaAttributeNumber(attributes, ATTRIBUTE_ARMOR_HP, NaN));
            if (Number.isFinite(armorHP) &&
                armorHP >= 0 &&
                Number.isFinite(conditionState.armorDamage)) {
                attributes[ATTRIBUTE_ARMOR_DAMAGE] = Number((armorHP * conditionState.armorDamage).toFixed(6));
            }
            const structureHP = Number(item.structureHP ??
                item.hullHP ??
                this._resolveDogmaAttributeNumber(attributes, ATTRIBUTE_STRUCTURE_HP, NaN));
            if (Number.isFinite(structureHP) &&
                structureHP >= 0 &&
                Number.isFinite(conditionState.damage)) {
                attributes[ATTRIBUTE_ITEM_DAMAGE] = Number((structureHP * conditionState.damage).toFixed(6));
            }
        }
        const propulsionRuntimeAttributes = this._getPropulsionModuleAttributeOverrides(item, session);
        if (propulsionRuntimeAttributes) {
            attributes[MODULE_ATTRIBUTE_CAPACITOR_NEED] = Number(propulsionRuntimeAttributes.capNeed);
            attributes[MODULE_ATTRIBUTE_SPEED_FACTOR] = Number(propulsionRuntimeAttributes.speedFactor);
            attributes[MODULE_ATTRIBUTE_DURATION] = marshalDogmaAttributeValue(MODULE_ATTRIBUTE_DURATION, Number(propulsionRuntimeAttributes.durationMs));
        }
        else {
            const genericRuntimeAttributes = this._getGenericModuleAttributeOverrides(item, session, options);
            if (genericRuntimeAttributes) {
                const genericAttributeOverrides = genericRuntimeAttributes.attributeOverrides &&
                ...
                attributes[MODULE_ATTRIBUTE_CAPACITOR_NEED] = Number(genericRuntimeAttributes.capNeed);
                const durationAttributeID = Number(genericRuntimeAttributes.durationAttributeID) || MODULE_ATTRIBUTE_DURATION;
                attributes[durationAttributeID] = marshalDogmaAttributeValue(durationAttributeID, Number(genericRuntimeAttributes.durationMs));
                if (durationAttributeID !== MODULE_ATTRIBUTE_DURATION &&
                    MODULE_ATTRIBUTE_DURATION in attributes) {
                    delete attributes[MODULE_ATTRIBUTE_DURATION];
                }
                if (durationAttributeID !== MODULE_ATTRIBUTE_SPEED &&
    ...
    _getModuleReloadTimeMs(moduleItem) {
        const effectiveAttributes = moduleItem
            ...
            : null;
        const reloadTimeMs = Number(effectiveAttributes && effectiveAttributes[ATTRIBUTE_RELOAD_TIME] !== undefined
            ? effectiveAttributes[ATTRIBUTE_RELOAD_TIME]
            : getTypeAttributeValue(Number(moduleItem && moduleItem.typeID) || 0, "reloadTime"));
    ...
    _notifyChargeBeingLoadedToModule(session, moduleIDs = [], chargeTypeID, reloadTimeMs) {
        ...
        log.debug(`[DogmaIM] OnChargeBeingLoadedToModule modules=${JSON.stringify(numericModuleIDs)} chargeTypeID=${Number(chargeTypeID) || 0} ` +
            `reloadTimeMs=${Math.max(0, Math.round(Number(reloadTimeMs) || 0))}`);
    }
    _notifyModuleNextActivationTime(session, moduleID, nextActivationTime = 0n, previousActivationTime = 0n) {
        if (!ATTRIBUTE_NEXT_ACTIVATION_TIME) {
            return;
        }
        const numericModuleID = Number(moduleID) || 0;
        ...
        log.debug(`[DogmaIM] NextActivationTime moduleID=${numericModuleID} ` +
    ...
    _buildInventoryItemAttributeDict(item, session = null, options = {}) {
        return this._buildAttributeValueDict(this._buildInventoryItemAttributes(item, session, options));
    }
    ...
    _getOnlineEffectStartTime(item, session = null) {
        const itemID = Number(item && item.itemID) || 0;
        let cache = null;
        if (session && typeof session === "object") {
            cache = this._onlineEffectStartTimes.get(session) || null;
            if (!cache) {
                cache = new Map();
                this._onlineEffectStartTimes.set(session, cache);
            }
        }
        const moduleState = item && item.moduleState && typeof item.moduleState === "object"
            ? item.moduleState
            : {};
        const persistedStartMs = [
            moduleState.onlineEffectStartedAtMs,
            moduleState.onlineStartedAtMs,
            moduleState.serviceFuelOnlineAt,
            item && item.onlineEffectStartedAtMs,
            item && item.onlineStartedAtMs,
            item && item.createdAtMs,
        ].map(Number).find((value) => Number.isFinite(value) && value > 0);
        if (persistedStartMs) {
            const persistedStart = this._toFileTime(persistedStartMs);
            if (cache && itemID > 0) {
                cache.set(itemID, persistedStart);
            }
            return persistedStart;
        }
        if (cache && itemID > 0 && typeof cache.get(itemID) === "bigint") {
            return cache.get(itemID);
        }
        const startTime = session ? this._sessionFileTime(session) : this._nowFileTime();
        if (cache && itemID > 0) {
            cache.set(itemID, startTime);
        }
        return startTime;
    }
    _buildActiveEffectEntry(item, effectID, options = {}, session = null) {
        ...
        const timestamp = typeof options.startTime === "bigint"
            ? options.startTime
            : this._toFileTime(options.startedAt, now);
    ...
    _buildInventoryItemActiveEffects(item, session = null, options = {}) {
        if (!item) {
        ...
        if (options.forceOnline === true || isEffectivelyOnlineModule(item)) {
            ...
            const onlineEntry = this._buildActiveEffectEntry(item, onlineEffectID, {
                ownerID: options.controlledStructure === true
                    ? null
                    : Number(item.ownerID) || 0,
                startTime: this._getOnlineEffectStartTime(item, session),
                duration: -1,
                repeat: 1,
            }, session);
            if (onlineEntry) {
        ...
        if (session && session._space && options.controlledStructure !== true) {
            const activeEffect = spaceRuntime.getActiveModuleEffect(session, item.itemID);
            if (activeEffect) {
                const activeEffectID = Number(activeEffect.effectID) > 0
                    ? Number(activeEffect.effectID)
                    : this._getPropulsionEffectID(activeEffect.effectName);
                const activeEntry = this._buildActiveEffectEntry(item, activeEffectID, {
            ...
            const overloadEffect = typeof spaceRuntime.getOverloadModuleEffect === "function"
                ? spaceRuntime.getOverloadModuleEffect(session, item.itemID)
                : null;
            if (overloadEffect && Number(overloadEffect.effectID) > 0) {
                const overloadEntry = this._buildActiveEffectEntry(item, Number(overloadEffect.effectID), {
    ...
    _listLoadedChargeSublocationItems(charID, shipID, options = {}) {
        ...
        if (this._isFittingContextMatch(options.fittingContext, charID, numericShipID, options)) {
    ...
    _buildShipInventoryInfoEntries(charID, shipID, ownerID, locationID, session = null, options = {}) {
        const inventoryEntries = [];
        const creationModuleItems = Array.isArray(options.creationModuleItems)
            ? options.creationModuleItems.filter(Boolean)
            : [];
        const creationModuleItemIDs = new Set(creationModuleItems
            .map((item) => Number(item && item.itemID) || 0)
            .filter((itemID) => itemID > 0));
        const listFittedRows = (chargeRows = false) => {
            if (this._isFittingContextMatch(options.fittingContext, charID, shipID, options)) {
                return chargeRows
        ...
        if (options.includeFittedItems !== false) {
            ...
            if (Array.isArray(fittedItems)) {
                for (const item of fittedItems) {
                    ...
                    const isCreationModuleItem = creationModuleItemIDs.has(Number(item && item.itemID) || 0);
                    const cachedEntry = hasDynamicStructureServiceAttributes || isCreationModuleItem
                        ? null
                        : this._getCachedDockedItemInfoEntry(session, item.itemID, item);
                    const entry = cachedEntry ||
                        this._buildCommonGetInfoEntry({
                            ...
                            customInfo: options.controlledStructure === true
                                ? (item.customInfo || "")
                                : (item.customInfo || null),
                            preserveNullCustomInfo: options.controlledStructure !== true,
                            ...
                            activeEffects: this._buildInventoryItemActiveEffects(item, session, {
                                ...
                                forceOnline: isCreationModuleItem &&
                                    item.moduleState &&
                            ...
                            attributes: this._buildInventoryItemAttributeDict(item, session, {
                                ...
                                includeQuantityAttribute: options.includeFittedItemQuantityAttribute !== false,
                            }),
                            session,
                        });
        ...
        if (creationModuleItems.length > 0) {
            const { getCreationModuleChargeState } = require(path.join(__dirname, "../frontier/creationChargeRuntime"));
        ...
        if (options.includeLoadedCharges === true) {
            ...
            if (Array.isArray(loadedCharges)) {
                for (const item of loadedCharges) {
                    const cachedEntry = this._getCachedDockedItemInfoEntry(session, item.itemID, item);
                    const entry = cachedEntry ||
                        this._buildCommonGetInfoEntry({
                            itemID: item.itemID,
                            typeID: item.typeID,
        ...
        if (options.includeChargeSublocations !== false) {
            const tupleChargeEntries = this._buildChargeSublocationData(charID, shipID, options)
                .map((entry) => {
                const loadedCharge = entry.sourceItem || getLoadedChargeByFlag(charID, shipID, entry.flagID);
                if (!loadedCharge) {
                    return null;
                }
                const quantity = Math.max(0, Number(loadedCharge.stacksize ?? loadedCharge.quantity ?? 0) || 0);
                ...
                return [
                    ...
                    this._buildCommonGetInfoEntry({
                        ...
                        attributes: this._buildInventoryItemAttributeDict(tupleChargeItem, session, {
                            fittingContext: options.fittingContext,
                            includeQuantityAttribute: true,
                        }),
                        invItem: null,
                        session,
                    }),
    ...
    _buildChargeSublocationRow({ locationID, flagID, typeID, quantity, }) {
        return {
            type: "object",
            name: "util.Row",
            args: {
    ...
    _dispatchPostResponseAttributeChanges(session, changes = [], options = {}) {
        ...
        const timer = setTimeout(() => {
            try {
                ...
                log.debug(`[DogmaIM] delayed attribute refresh skipped: ${error && error.message ? error.message : error}`);
            }
        }, delayMs);
    ...
    _queuePostLoadFuelAttributeChanges(session, changes = []) {
    ...
    _notifyShipFittingResourceAttributeChanges(session, shipID, previousSnapshot, nextSnapshot) {
        ...
        const changes = listShipFittingAttributeChanges(previousSnapshot, nextSnapshot).map((change) => [
            "OnModuleAttributeChanges",
            charID,
            numericShipID,
    ...
    _refreshDockedFittingState(session, changes = [], options = {}) {
        ...
        const activeShipID = Number(session.activeShipID || session.shipID || session.shipid || 0) || 0;
        if (activeShipID <= 0) {
            return;
        }
        const touchesFittingState = changes.some((change) => {
            ...
            if (previousLocationID !== activeShipID &&
                nextLocationID !== activeShipID) {
                return false;
            }
            return (isShipFittingFlag(previousFlagID) ||
        ...
        syncShipFittingStateForSession(session, activeShipID, {
            ...
            ...this._getFittingContextSnapshotOptions(options.fittingContext, this._getCharID(session), activeShipID),
        });
    }
    _captureChargeStateSnapshot(charID, shipID, flagID) {
        ...
        return {
            ...
            quantity: Math.max(0, Number(chargeItem.stacksize ?? chargeItem.quantity ?? 0) || 0),
        };
    }
    ...
    _buildChargeTransitionPrimeItem(charID, shipID, flagID, chargeState = null, chargeItem = null) {
        const typeID = Number(chargeState && chargeState.typeID) ||
    ...
    _primeChargeTupleForQuantityTransition(session, charID, shipID, flagID, chargeState = null, chargeItem = null, options = {}) {
        if (options.primeNewChargeTuple === false) {
            return false;
        }
        const primeItem = this._buildChargeTransitionPrimeItem(charID, shipID, flagID, chargeState, chargeItem);
        if (!primeItem) {
            return false;
        }
        const weaponAttributeItem = chargeItem &&
            ...
            : primeItem;
        const weaponDogmaAttributes = this._getWeaponDogmaAttributeOverrides(weaponAttributeItem, session, options);
        syncChargeGodmaPrimeForSession(session, shipID, primeItem, {
            description: "charge",
            now: options.when != null ? options.when : this._sessionFileTime(session),
            attributeOverrides: weaponDogmaAttributes && weaponDogmaAttributes.chargeAttributes,
        });
        return true;
    }
    _notifyWeaponModuleAttributeTransition(session, moduleItem, previousChargeItem = null, nextChargeItem = null) {
        if (!session || typeof session.sendNotification !== "function" || !moduleItem) {
            return;
        }
        const numericModuleID = Number(moduleItem.itemID) || 0;
        ...
        const previousAttributes = this._buildWeaponModuleAttributeMap(moduleItem, previousChargeItem, session) || {};
        const nextAttributes = this._buildWeaponModuleAttributeMap(moduleItem, nextChargeItem, session) || {};
        ...
        for (const rawAttributeID of changedAttributeIDs) {
            ...
            const previousValue = Object.prototype.hasOwnProperty.call(previousAttributes, attributeID)
                ? Number(previousAttributes[attributeID])
                : 0;
            const nextValue = Object.prototype.hasOwnProperty.call(nextAttributes, attributeID)
    ...
    _notifyChargeQuantityTransition(session, charID, shipID, flagID, previousState = null, nextState = null, options = {}) {
        ...
        if (isNewChargeTuple) {
            ...
            this._primeChargeTupleForQuantityTransition(session, numericCharID, numericShipID, numericFlagID, nextState, options.nextChargeItem, {
                ...options,
                when,
            });
        }
        ...
        let notified = false;
        if (previousTypeID > 0) {
            notified = sendQuantityChange(buildQuantityChange(previousTypeID, previousTypeID === nextTypeID ? nextQuantity : 0, previousQuantity)) || notified;
        }
        if (isNewChargeTuple) {
            notified = sendQuantityChange(buildQuantityChange(nextTypeID, nextQuantity, 0)) || notified;
        }
        return notified;
    }
    _syncInventoryChanges(session, changes = [], options = {}) {
        ...
        const clientFacingChanges = this._filterInventoryChangesForClient(session, normalizedChanges);
    ...
    _getSkillQueueRuntime() {
        return require(path.join(__dirname, "../skills/training/skillQueueRuntime"));
    }
    _captureQueueSnapshotForAttributeChange(charID, reason = "attribute") {
        try {
            ...
            log.warn(`[DogmaIM] Failed to capture skill queue before ${reason} change char=${charID}: ${error && error.message ? error.message : error}`);
            return null;
        }
    }
    _prepareQueueForAttributeChange(charID, reason = "attribute", snapshot = null) {
        try {
            ...
            log.warn(`[DogmaIM] Failed to prepare skill queue for ${reason} change char=${charID}: ${error && error.message ? error.message : error}`);
            return null;
        }
    }
    _syncQueueAfterAttributeChange(charID, reason = "attribute") {
        try {
            ...
            log.warn(`[DogmaIM] Failed to sync skill queue after ${reason} change char=${charID}: ${error && error.message ? error.message : error}`);
            return null;
        }
    }
    _syncCapsuleTypeForCharacter(session, charID) {
        ...
        if (!result.success) {
            if (result.errorMsg !== "CAPSULE_NOT_FOUND") {
                log.warn(`[DogmaIM] Failed to sync capsule type after implant change char=${charID}: ${result.errorMsg || "UNKNOWN"}`);
            }
            return result;
        }
        if (session && result.changed && result.data) {
            const activeShipID = Number(session.shipID ||
                session.shipid ||
    ...
    _resolveShipInventoryChangeContext(session, change = {}) {
        ...
        const activeShipID = Number((session._space && session._space.shipID) ||
            session.activeShipID ||
            session.shipID ||
            session.shipid ||
            0) || 0;
        ...
        if ((currentLocationID === activeShipID &&
    ...
    _syncShipInventoryItemsChangedForSession(session, change = {}) {
        ...
        const payload = buildItemChangePayload(change.item, change.previousData || change.previousState || {});
    ...
    _resolveValidatedProbeLaunchContext(session, moduleID, requestedCount = 1) {
        ...
        if (removedGhostProbes.length > 0) {
            log.debug(`[DogmaIM] Purged ${removedGhostProbes.length} invalid persisted probe record(s) ` +
        ...
        if (removedExpiredProbes.length > 0) {
            log.debug(`[DogmaIM] Purged ${removedExpiredProbes.length} expired persisted probe record(s) ` +
                `before launch for charID=${charID} systemID=${systemID}`);
        }
        ...
        if (Number(loadedCharge.categoryID) !== SCANNER_PROBE_CATEGORY_ID ||
            Number(loadedCharge.groupID) !== GROUP_SCANNER_PROBE) {
            this._throwProbeLaunchUserError("INVALID_CHARGE", moduleItem);
        }
        if (!isChargeCompatibleWithModule(moduleItem.typeID, loadedCharge.typeID)) {
            this._throwProbeLaunchUserError("INCOMPATIBLE_CHARGE", moduleItem);
        }
        // NOTE: the community patch also gated launch on the probe type's required
        // skills here. Fitting/loading already enforces the same requirements via
        // validateFitForShip (liveFittingState), so this launch-time repeat only
        // rejected flows that inject charges directly. Left out pending a decision.
        const loadedChargeQuantity = Math.max(0, Number(loadedCharge.stacksize ?? loadedCharge.quantity ?? 0) || 0);
    ...
    _throwProbeLaunchUserError(errorMsg = "", moduleItem = null) {
        switch (String(errorMsg || "").trim()) {
            ...
            case "INVALID_CHARGE":
                ...
                break;
            case "INCOMPATIBLE_CHARGE":
                this._throwCustomNotifyUserError("That scanner probe does not fit in this probe launcher.");
                break;
            case "MISSING_REQUIRED_SKILL":
                this._throwCustomNotifyUserError("You do not meet the skill requirements for that scanner probe.");
                break;
            default:
    ...
    _resolveValidatedInterdictionProbeLaunchContext(session, moduleID) {
        ...
        const validation = interdictionProbeRuntime.validateInterdictionProbeLaunchContext(session, moduleItem, loadedCharge);
        if (!validation || validation.success !== true) {
            this._throwInterdictionProbeLaunchUserError(validation && validation.errorMsg, moduleItem);
        }
        ...
        return {
            ...
            shipID,
            charID,
            systemID: contextData.context && Number(contextData.context.systemID) > 0
                ? Number(contextData.context.systemID)
                : Number((session && session.solarsystemid2) ||
                    (session && session.solarsystemid) ||
                    (session && session._space && session._space.systemID) ||
                    0) || 0,
        };
    ...
    _launchInterdictionProbeFromContext(session, probeContext = null) {
        ...
        const systemID = Number(probeContext && probeContext.systemID) || Number((session && session.solarsystemid2) ||
            (session && session.solarsystemid) ||
            (session && session._space && session._space.systemID) ||
            0) || 0;
        const launchResult = interdictionProbeRuntime.launchInterdictionProbeFromModule(session, moduleItem, loadedCharge);
        if (!launchResult || launchResult.success !== true) {
            this._throwInterdictionProbeLaunchUserError(launchResult && launchResult.errorMsg, moduleItem);
        }
        const launchedItemID = Number(launchResult.data && launchResult.data.itemID) || 0;
        const consumeResult = this._consumeLoadedProbeCharge(session, charID, shipID, moduleItem, 1);
        if (!consumeResult.success) {
            if (launchedItemID > 0) {
                interdictionProbeRuntime.removeInterdictionProbe(launchedItemID, systemID, "charge-consume-failed");
            }
            this._throwInterdictionProbeLaunchUserError(consumeResult.errorMsg, moduleItem);
        }
        const remainingQuantity = Math.max(0, Number(consumeResult && consumeResult.data && consumeResult.data.remainingQuantity) || 0);
        return {
    ...
    _resolveValidatedWarpDisruptFieldGeneratorContext(session, moduleItem) {
        ...
        const restriction = warpDisruptFieldGeneratorRuntime.getActivationRestriction(systemID, moduleItem, loadedCharge);
        if (restriction) {
    ...
    _launchProbesFromContext(session, probeContext = null) {
        ...
        const loadedCharge = probeContext && probeContext.loadedCharge
            ? probeContext.loadedCharge
            : null;
        const requestedCount = Math.max(1, Number(probeContext && probeContext.requestedCount) || 1);
        const shipID = Number(probeContext && probeContext.shipID) || this._getShipID(session);
        const charID = Number(probeContext && probeContext.charID) || this._getCharID(session);
        const systemID = Number(probeContext && probeContext.systemID) || Number((session && session.solarsystemid2) ||
            (session && session.solarsystemid) ||
            (session && session._space && session._space.systemID) ||
            0) || 0;
        const consumeResult = this._consumeLoadedProbeCharge(session, charID, shipID, moduleItem, requestedCount);
        if (!consumeResult.success) {
        ...
        const launchShipEntity = spaceRuntime.getEntity(session, shipID);
        const probeTypeAttributes = getTypeDogmaAttributes(Number(loadedCharge && loadedCharge.typeID) || 0);
        const expiryDelayMs = Math.max(0, Number(probeTypeAttributes[String(ATTRIBUTE_EXPLOSION_DELAY)]) || 0);
        const launchedProbes = probeRuntimeState.launchCharacterProbes(charID, systemID, Number(loadedCharge && loadedCharge.typeID) || 0, requestedCount, {
            nowMs: Date.now(),
            position: launchPosition,
            shipID,
            launcherItemID: Number(moduleItem && moduleItem.itemID) || 0,
            launcherFlagID: Number(moduleItem && moduleItem.flagID) || 0,
            scopeMetadata: buildChildEntityScopeMetadata(launchShipEntity),
            // Probe expiry is authored in milliseconds in attribute 281.  Keep the
            // runtime's one-hour fallback only when the type has no authored value.
            expirySeconds: expiryDelayMs > 0
                ? Math.max(1, Math.round(expiryDelayMs / 1000))
                : probeScanRuntime.DEFAULT_PROBE_EXPIRY_SECONDS,
        });
        if (launchedProbes.length !== requestedCount) {
            probeSceneRuntime.removeProbeEntitiesForSession(session, launchedProbes.map((probe) => Number(probe && probe.probeID) || 0));
            probeRuntimeState.removeCharacterProbes(charID, launchedProbes.map((probe) => Number(probe && probe.probeID) || 0), { nowMs: Date.now() });
            this._throwProbeLaunchUserError("TOO_MANY_ACTIVE_PROBES", moduleItem);
        }
        ...
        return {
            launchedProbes,
            chargeTypeID: Number(loadedCharge && loadedCharge.typeID) || 0,
            remainingQuantity: Math.max(0, Number(consumeResult && consumeResult.data && consumeResult.data.remainingQuantity) || 0),
            autoReloadRecommended: Math.max(0, Number(consumeResult && consumeResult.data && consumeResult.data.remainingQuantity) || 0) <= 0,
        };
    }
    _consumeLoadedProbeCharge(session, charID, shipID, moduleItem, quantity = 1) {
        ...
        const chargeItem = getLoadedChargeByFlag(numericCharID, numericShipID, Number(moduleItem.flagID) || 0);
        if (!chargeItem) {
            return {
                success: false,
                errorMsg: "NO_CHARGES",
        ...
        const availableQuantity = Math.max(0, Number(chargeItem.stacksize ?? chargeItem.quantity ?? 0) || 0);
        ...
        const previousChargeState = this._captureChargeStateSnapshot(numericCharID, numericShipID, moduleItem.flagID);
        const previousChargeItem = this._captureChargeItemSnapshot(numericCharID, numericShipID, moduleItem.flagID);
        ...
        const nextChargeState = this._captureChargeStateSnapshot(numericCharID, numericShipID, moduleItem.flagID);
        const nextChargeItem = this._captureChargeItemSnapshot(numericCharID, numericShipID, moduleItem.flagID);
        this._notifyChargeQuantityTransition(session, numericCharID, numericShipID, moduleItem.flagID, previousChargeState, nextChargeState, {
            previousChargeItem,
            nextChargeItem,
        });
        this._notifyWeaponModuleAttributeTransition(session, moduleItem, previousChargeItem, nextChargeItem);
        return {
            success: true,
            data: {
                chargeTypeID: Number(chargeItem.typeID) || 0,
                remainingQuantity: Math.max(0, Number(nextChargeState && nextChargeState.quantity) || 0),
            },
        };
    }
    _buildRemovedInventoryNotificationState(item = {}) {
        return {
            ...
            quantity: Number(item.singleton) === 1
                ? -1
                : Number(item.stacksize ?? item.quantity ?? 0) || 0,
            stacksize: Number(item.singleton) === 1
                ? 1
                : Number(item.stacksize ?? item.quantity ?? 0) || 0,
        };
    }
    _filterInventoryChangesForClient(session, changes = []) {
        ...
        return changes.flatMap((change) => {
            ...
            const previousLocationID = Number(previousItem && previousItem.locationID) || 0;
            ...
            const currentFitted = currentLocationID === activeShipID &&
                isShipFittingFlag(currentFlagID);
            const previousFitted = previousLocationID === activeShipID &&
                isShipFittingFlag(previousFlagID);
            if (!currentFitted && !previousFitted) {
                return [change];
            }
            const movedWholeCargoStackIntoSlot = previousItem &&
                currentFitted &&
                !previousFitted &&
                previousLocationID === activeShipID;
    ...
    _moveLoadedChargeToDestination(chargeItem, destinationLocationID, destinationFlagID, quantity = null) {
        ...
        const sourceQuantity = Math.max(0, Number(chargeItem && (chargeItem.stacksize ?? chargeItem.quantity ?? 0)) || 0);
        ...
        const requestedQuantity = quantity === null || quantity === undefined
            ? sourceQuantity
        ...
        const sourceIsLoadedCharge = Number(chargeItem.categoryID) === 8 &&
            isShipFittingFlag(sourceFlagID) &&
            !isShipFittingFlag(numericDestinationFlagID);
        if (!sourceIsLoadedCharge) {
            return moveItemToLocation(sourceItemID, numericDestinationLocationID, numericDestinationFlagID, requestedQuantity);
        }
        const matchingDestinationCandidates = listContainerItems(ownerID, numericDestinationLocationID, numericDestinationFlagID)
            .filter((item) => item &&
        ...
        const matchingDestinationStack = (preferredOriginStackID > 0
            ? matchingDestinationCandidates.find((item) => item &&
        ...
        if (matchingDestinationStack) {
            return mergeItemStacks(sourceItemID, matchingDestinationStack.itemID, requestedQuantity);
        }
        if (requestedQuantity < sourceQuantity) {
            return moveItemToLocation(sourceItemID, numericDestinationLocationID, numericDestinationFlagID, requestedQuantity);
        }
        const grantResult = grantItemToOwnerLocation(ownerID, numericDestinationLocationID, numericDestinationFlagID, Number(chargeItem.typeID) || 0, requestedQuantity, {
    ...
    _buildShipBaseAttributes(shipData = {}) {
        ...
        const shipTypeID = Number(shipData.typeID);
        const staticEntry = Number.isInteger(shipTypeID) &&
            payload &&
            ...
            : null;
        const staticAttributes = staticEntry && staticEntry.attributes && typeof staticEntry.attributes === "object"
            ? staticEntry.attributes
            : null;
        const attributes = staticAttributes
            ? Object.fromEntries(Object.entries(staticAttributes)
                .map(([attributeID, value]) => [Number(attributeID), Number(value)])
                .filter(([attributeID, value]) => Number.isInteger(attributeID) && Number.isFinite(value)))
            : {};
        const shipMetadata = Number.isInteger(shipTypeID) && shipTypeID > 0
        ...
        const resolvedCapacity = Number(shipData.capacity ?? (shipMetadata && shipMetadata.capacity));
    ...
    _resolveNewbieShipTypeID(session) {
        return resolveNewbieShipTypeID(session, this._getCharacterRecord(session) || {});
    }
    ...
    _resolveItemAttributeContext(requestedItemID, session) {
        ...
        const numericItemID = Number.parseInt(String(tupleItemID), 10) || this._getShipID(session);
        const skillRecord = getCharacterSkills(charID).find((skill) => skill.itemID === numericItemID ||
            skill.itemID === Number.parseInt(String(requestedItemID), 10)) || null;
        ...
        if (controlledStructureShip &&
            ...
            const attributes = this._buildInventoryItemAttributes(controlledStructureShip, session);
            return {
    ...
    _compareFittingRows(left, right) {
        const flagDiff = (Number(left && left.flagID) || 0) - (Number(right && right.flagID) || 0);
        if (flagDiff !== 0) {
            return flagDiff;
        }
    ...
    _buildFittingHydrationContext(charID, shipID, shipItem = null, options = {}) {
        const numericShipID = Number(shipID) || Number(shipItem && shipItem.itemID) || 0;
        if (numericShipID <= 0) {
            return null;
        }
        ...
        const hiddenModifierItems = listHiddenModifierItems(ownerID, numericShipID, shipItem);
        return {
    ...
    _buildCharacterInfoDict(charID, charData, locationID, session = null) {
        return {
            type: "dict",
            entries: this._buildCharacterInfoEntries(charID, charData, locationID, session),
        };
    }
    ...
    _buildShipModifiedCharacterAttributeInfo(charID, charData, locationID, session = null, options = {}) {
        return this._buildCommonGetInfoEntry({
            ...
            stacksize: 1,
            description: "character",
            attributes: this._buildShipModifiedCharacterAttributeDict(charData, charID, session, options),
            session,
        });
    }
    _buildCharacterBrain(charID, session = null) {
        return buildBootstrapCharacterBrain(charID, 0, {
            shipID: session && (session.activeShipID ??
                session.shipID ??
                session.shipid),
            structureID: session && (session.structureid ??
                session.structureID ??
                session.structureId),
        });
    }
    ...
    _buildStructureInfoDict(structure, session = null, options = {}) {
        if (!structure) {
            return this._buildEmptyDict();
        }
        const structureItem = this._buildStructureControlShipMetadata(structure);
        if (!structureItem) {
            return this._buildEmptyDict();
        }
        const structureAttributes = this._buildInventoryItemAttributes(structureItem, session, {
            includeQuantityAttribute: false,
        });
        if (options.includeActivationAttributes === true) {
            Object.assign(structureAttributes, STRUCTURE_ACTIVATION_ATTRIBUTE_VALUES);
        }
        return {
            type: "dict",
            entries: [[
                    ...
                    this._buildCommonGetInfoEntry({
                        ...
                        attributes: this._buildAttributeValueDict(structureAttributes),
                        session,
                    }),
                ]],
        };
    ...
    _shouldPrimeLoginShipInfoChargeSublocations(session, options = {}) {
        ...
        return LIVE_SPACE_TUPLE_CHARGE_PROFILES.has(String(session._space.loginChargeHydrationProfile || ""));
    }
    ...
    _queuePostGetAllInfoChargeQuantityRefresh(session, charID, shipID, options = {}) {
        ...
        const changes = this._buildLoadedChargeTupleQuantityChanges(session, charID, shipID, options);
        if (changes.length <= 0) {
    ...
    _flushPostGetAllInfoChargeQuantityRefresh(session) {
        ...
        const changes = Array.isArray(session._space.pendingDogmaGetAllInfoChargeQuantityRefresh)
    ...
    _queuePostGetAllInfoCreationAttributeRefresh(session, charID, shipID, shipAttributes = {}, creationDogmaContext = null) {
        if (!session || !session._space) {
            return 0;
        }
        const numericCharID = Number(charID) || this._getCharID(session);
        const numericShipID = Number(shipID) || this._getShipID(session);
        const modifierEntries = creationDogmaContext && Array.isArray(creationDogmaContext.shipAttributeModifierEntries)
            ? creationDogmaContext.shipAttributeModifierEntries
            : [];
        ...
        const attributeIDs = new Set(modifierEntries
    ...
    _flushPostGetAllInfoCreationAttributeRefresh(session) {
        ...
        const changes = Array.isArray(session._space.pendingDogmaGetAllInfoCreationAttributeRefresh)
    ...
    _buildShipState(charID, shipID, shipRecord = null, options = {}) {
        ...
        const standardFittedItems = options.includeFittedItems === false
            ? []
            : this._isFittingContextMatch(options.fittingContext, charID, shipID, options)
                ? options.fittingContext.moduleItems
            ...
            entries: [
                ...
                ...fittedItems.map((item) => {
                    ...
                    if (creationModuleItemIDs.has(Number(item && item.itemID) || 0)) {
                        snapshot.online = Boolean(item && item.moduleState && item.moduleState.online === true);
                    }
                    return [
                        item.itemID,
    ...
    Handle_GetCharacterAttributes(args, session) {
        ...
        return this._buildCharacterAttributeDict(this._getCharacterRecord(session) || {}, this._getCharID(session));
    }
    Handle_ChangeDroneSettings(args, session) {
        const rawDroneSettingChanges = args && args.length > 0 ? args[0] : null;
        const droneSettingChanges = this._normalizeDroneSettingChanges(rawDroneSettingChanges);
        const nextDroneSettings = this._persistDroneSettingChanges(session, droneSettingChanges);
        if (session && typeof session === "object") {
            session.droneSettings = {
                ...nextDroneSettings,
            };
        }
        log.debug(`[DogmaService] ChangeDroneSettings char=${this._getCharID(session)} keys=${Object.keys(droneSettingChanges).join(",")}`);
        return true;
    }
    ...
    _buildItemLayerDamageValues(item) {
        ...
        const rawConditionState = item.conditionState && typeof item.conditionState === "object"
            ? item.conditionState
            : null;
        const conditionState = rawConditionState
            ...
            : (categoryID === SHIP_CATEGORY_ID || categoryID === DRONE_CATEGORY_ID
                ...
                : {
                    ...
                    armorDamage: clampRatio(item && item.moduleState && item.moduleState.armorDamage, 0),
                    shieldCharge: clampRatio(item && item.moduleState && item.moduleState.shieldCharge, 1),
                    incapacitated: Boolean(item && item.moduleState && item.moduleState.incapacitated),
                });
        const shieldCapacity = Number(item.shieldCapacity ?? getTypeAttributeValue(item.typeID, "shieldCapacity")) || 0;
        const shieldRechargeRate = Number(item.shieldRechargeRate ?? getTypeAttributeValue(item.typeID, "shieldRechargeRate")) || 0;
        const armorHP = Number(item.armorHP ?? getTypeAttributeValue(item.typeID, "armorHP")) || 0;
        const structureHP = Number(item.structureHP ?? item.hullHP ?? getTypeAttributeValue(item.typeID, "hp", "structureHP")) || 0;
        const shieldRatio = shieldCapacity > 0 ? clampRatio(conditionState.shieldCharge, 1) : 0;
        const armorRatio = armorHP > 0 ? clampRatio(1 - clampRatio(conditionState.armorDamage, 0), 1) : 0;
        const hullRatio = structureHP > 0 ? clampRatio(1 - clampRatio(conditionState.damage, 0), 1) : 0;
        const currentShield = shieldCapacity > 0 ? shieldCapacity * shieldRatio : 0;
        const armorDamageAmount = armorHP > 0 ? armorHP * clampRatio(conditionState.armorDamage, 0) : 0;
        const hullDamageAmount = structureHP > 0 ? structureHP * clampRatio(conditionState.damage, 0) : 0;
    ...
    _buildTargetingAttemptFailedUserErrorValues(data = null) {
        ...
        const targetTypeID = Number(errorData.targetTypeID ??
        ...
        switch (String(errorMsg || "").trim()) {
            ...
            case "TARGET_LOCK_LIMIT_REACHED":
                throwWrappedUserError("TargetingSystemsAlreadyFullyUtilized", {
                    limit: Math.max(0, Number(errorData.limit ?? errorData.effectiveMaxLockedTargets) || 0),
                });
                break;
            case "TARGET_JAMMED":
                this._throwCustomNotifyUserError("You cannot lock that target while jammed except against the ships currently jamming you.");
                break;
            default:
                throwWrappedUserError("DeniedTargetingAttemptFailed", this._buildTargetingAttemptFailedUserErrorValues(errorData));
                break;
        }
    }
    ...
    _resolveModuleDisplayName(moduleItem, fallback = "module") {
        ...
        const rawName = (moduleItem && moduleItem.itemName) ||
    ...
    _resolveEntityDisplayName(entity, fallback = "That target") {
        ...
        const rawName = (entity && (entity.itemName || entity.name)) ||
            (typeRecord && (typeRecord.name || typeRecord.typeName)) ||
            fallback;
        const normalizedName = String(rawName || fallback).trim();
        return normalizedName || fallback;
    ...
    _resolveModuleActivationRangeMeters(session, moduleItem) {
        ...
        const weaponAttributes = this._buildWeaponModuleAttributeMap(moduleItem, loadedChargeItem, session);
        const moduleAttributes = weaponAttributes ||
            buildEffectiveItemAttributeMap(moduleItem, loadedChargeItem);
        const maxRangeMeters = Math.max(0, Number(moduleAttributes && moduleAttributes[ATTRIBUTE_MAX_RANGE]) || 0);
        const falloffAttributeID = FALLOFF_EFFECTIVENESS_MODULE_GROUPS.has(Number(moduleItem && moduleItem.groupID) || 0)
            ? ATTRIBUTE_FALLOFF_EFFECTIVENESS
            : ATTRIBUTE_FALLOFF;
        const falloffMeters = Math.max(0, Number(moduleAttributes && moduleAttributes[falloffAttributeID]) || 0);
    ...
    _isAnalyzerModuleActivation(moduleItem, effectName) {
        ...
        return (normalizedEffectName === ANALYZER_EFFECT_NAME &&
            typeID > 0 &&
    ...
    _getEntityRadiusMeters(entity) {
        return Math.max(0, normalizeNumber(entity && (entity.radius ?? entity.spaceRadius ?? entity.interactionRadius), 0));
    }
    _getEntitySurfaceDistanceMeters(sourceEntity, targetEntity) {
        ...
        return Math.max(0, centerDistance -
    ...
    _resolveAnalyzerModuleKind(moduleItem) {
        ...
        const typeName = String((moduleItem && moduleItem.itemName) ||
            (typeRecord && (typeRecord.name || typeRecord.typeName)) ||
            "").toLowerCase();
        ...
        if ((requiresHacking && requiresArchaeology) ||
    ...
    _resolveAnalyzerTargetKind(targetEntity) {
        const normalized = String(targetEntity &&
    ...
    _resolveAnalyzerDifficulty(targetEntity) {
        const rawDifficulty = targetEntity &&
    ...
    _resolveAnalyzerAccessDifficultyBonusPercent(session, moduleItem) {
        const characterID = this._getCharID(session);
        const effectiveAttributes = applyActiveImplantLocationModifiersToAttributes(buildEffectiveItemAttributeMap(moduleItem), moduleItem, characterID);
        const rawBonus = normalizeNumber(effectiveAttributes[ATTRIBUTE_ACCESS_DIFFICULTY_BONUS], 0);
    ...
    _resolveAnalyzerEffectiveDifficulty(session, moduleItem, targetEntity) {
        ...
        const accessDifficultyBonusPercent = this._resolveAnalyzerAccessDifficultyBonusPercent(session, moduleItem);
        const effectiveDifficulty = Math.max(1, baseDifficulty -
    ...
    _getCharacterSkillLevelForAnalyzer(session, skillTypeID) {
        ...
        const skill = getCharacterSkills(characterID).find((record) => Number(record && record.typeID) === normalizedSkillTypeID);
        const skillLevel = normalizeNumber(skill && (skill.trainedSkillLevel ?? skill.skillLevel ?? skill.effectiveSkillLevel), 0);
    ...
    _resolveAnalyzerVirusStats(session, moduleItem, moduleKind) {
        ...
        const baseCoherence = Math.max(1, Math.round(Number.isFinite(rawCoherence) ? rawCoherence : 40));
        const baseStrength = Math.max(1, Math.round(Number.isFinite(rawStrength) ? rawStrength : 20));
        const slots = Math.max(0, Math.round(Number(effectiveAttributes[ATTRIBUTE_VIRUS_ELEMENT_SLOTS]) || 0));
        const skillTypeIDs = moduleKind === "integrated"
        ...
        const skillCoherence = skillTypeIDs.reduce((sum, skillTypeID) => sum + (this._getCharacterSkillLevelForAnalyzer(session, skillTypeID) * 10), 0);
        return {
    ...
    _resolveAnalyzerHackingContext(session, moduleItem, targetID) {
        ...
        const targetEntity = typeof scene.getEntityByID === "function"
        ...
        const isHackableContainer = Boolean(targetEntity.dungeonMaterializedContainer === true ||
            targetEntity.dungeonSiteContentAnalyzer ||
            targetEntity.hackingAnalyzer ||
            targetEntity.analyzer);
        ...
        const difficultyStats = this._resolveAnalyzerEffectiveDifficulty(session, moduleItem, targetEntity);
        return {
            success: true,
            data: {
    ...
    _buildModuleTargetOutOfRangeNotify(context = {}) {
        ...
        const scene = session && typeof spaceRuntime.getSceneForSession === "function"
            ? spaceRuntime.getSceneForSession(session)
            : null;
        const targetEntity = scene && targetID > 0 && typeof scene.getEntityByID === "function"
    ...
    _buildModuleTargetOutOfRangeUserErrorValues(context = {}) {
        ...
        const scene = session && typeof spaceRuntime.getSceneForSession === "function"
            ? spaceRuntime.getSceneForSession(session)
            : null;
        const sourceEntity = scene && typeof scene.getShipEntityForSession === "function"
            ? scene.getShipEntityForSession(session)
            : null;
        const targetEntity = scene && targetID > 0 && typeof scene.getEntityByID === "function"
            ? scene.getEntityByID(targetID)
            : null;
        const desiredRange = Math.max(0, Number(this._resolveModuleActivationRangeMeters(session, moduleItem)) || 0);
    ...
    _buildModuleTargetTooFarUserErrorValues(context = {}) {
        ...
        const targetEntity = scene && targetID > 0 && typeof scene.getEntityByID === "function"
            ? scene.getEntityByID(targetID)
            : null;
        const distance = Math.max(0, Number(this._resolveModuleActivationRangeMeters(session, moduleItem)) || 0);
        const targetname = this._buildUserErrorTypeValue(targetEntity && targetEntity.typeID);
        const module = Math.max(0, Math.trunc(Number(moduleItem && moduleItem.typeID) || 0));
        return {
            distance,
            targetname,
            module,
    ...
    _buildModuleReactivationUserErrorValues(session, moduleItem) {
        ...
        const nowMs = scene && typeof scene.getCurrentSimTimeMs === "function"
            ? Number(scene.getCurrentSimTimeMs()) || Date.now()
            : Date.now();
        const lockUntilMs = shipEntity &&
            shipEntity.moduleReactivationLocks instanceof Map
            ? Number(shipEntity.moduleReactivationLocks.get(numericModuleID)) || 0
            : 0;
        const moduleAttributes = moduleItem && numericTypeID > 0
            ? buildEffectiveItemAttributeMap(moduleItem)
            : null;
        const fullDelayMs = Math.max(0, Number(moduleAttributes && moduleAttributes[ATTRIBUTE_MODULE_REACTIVATION_DELAY] !== undefined
            ? moduleAttributes[ATTRIBUTE_MODULE_REACTIVATION_DELAY]
            : getTypeAttributeValue(numericTypeID, "moduleReactivationDelay")) || 0);
    ...
    _buildEffectCrowdedOutValues(session, moduleItem) {
        ...
        const scene = spaceRuntime.getSceneForSession(session);
        const shipEntity = scene ? scene.getShipEntityForSession(session) : null;
        const count = shipEntity &&
            shipEntity.activeModuleEffects instanceof Map
            ? [...shipEntity.activeModuleEffects.values()].filter((effectState) => Number(effectState && effectState.groupID) === numericGroupID).length
            : 0;
        return {
        ...
        switch (String(errorMsg || "").trim()) {
            ...
            case "MAX_GROUP_ONLINE":
                throwWrappedUserError("CannotOnlineReachedMaxGroupOnline", {
                    ...
                    group: [
                        USER_ERROR_GROUP_ID,
                        Number((data && data.groupID) ||
                            (moduleItem && moduleItem.groupID)) || 0,
                    ],
                    maxGroupOnline: Math.max(0, Math.trunc(Number(data && data.maxGroupOnline) || 0)),
                });
                break;
            case "STRUCTURE_DAMAGED":
                this._throwCustomNotifyUserError("Service modules cannot be onlined while the structure is damaged.");
                break;
            case "STRUCTURE_SERVICE_TOO_CLOSE":
                this._throwCustomNotifyUserError("That navigation service cannot be onlined because another Upwell structure is too close.");
                break;
            case "STRUCTURE_SERVICE_REQUIRES_SOV_UPGRADE":
                this._throwCustomNotifyUserError(this._formatStructureServiceRequiresSovUpgradeError(data));
                break;
            case "CRP_ACCESS_DENIED":
                throwWrappedUserError("CrpAccessDenied", buildCrpAccessDeniedInsufficientRolesValues());
                break;
            default:
                this._throwCustomNotifyUserError(`Failed to change the online state for ${this._resolveModuleDisplayName(moduleItem)}.`);
                break;
        }
    }
    _formatStructureServiceRequiresSovUpgradeError(data = null) {
        const requiredUpgradeTypeID = Math.max(0, Math.trunc(Number(data && data.requiredUpgradeTypeID) || 0));
        ...
        switch (normalizedErrorMsg) {
            ...
            case "TARGET_OUT_OF_RANGE":
                if (String(context.effectName || "").trim() === "miningLaser") {
                    throwWrappedUserError("TargetTooFar", this._buildModuleTargetTooFarUserErrorValues(context));
                }
                throwWrappedUserError("TargetNotWithinRangeGeneric", this._buildModuleTargetOutOfRangeUserErrorValues(context));
                break;
            ...
            case "MODULE_REACTIVATING":
                throwWrappedUserError("ModuleReactivationDelayed2", this._buildModuleReactivationUserErrorValues(session, moduleItem));
                break;
            case "NO_AMMO":
                throwWrappedUserError("NoCharges", {
                    launcher: this._buildUserErrorTypeValue(moduleItem && moduleItem.typeID),
            ...
            case "MAX_GROUP_ACTIVE":
                throwWrappedUserError("EffectCrowdedOut", this._buildEffectCrowdedOutValues(session, moduleItem));
                break;
            ...
            case "CLOAK_PROXIMITY_BLOCKED":
                this._throwCustomNotifyUserError("You are too close to another object to activate your cloaking device.");
                break;
            case "CLOAK_TARGET_LOCKED":
                this._throwCustomNotifyUserError("Your ship is already target locked and cannot activate a cloaking device.");
                break;
            case "CLOAK_WARP_BLOCKED":
                this._throwCustomNotifyUserError("You cannot activate that cloaking device while warping.");
                break;
            case "MODULE_CLOAKED":
            ...
            case "ACTIVE_INDUSTRIAL_CORE_REQUIRED":
                this._throwCustomNotifyUserError("An active industrial core is required to activate that module.");
                break;
            case "WARP_SCRAMBLED":
                this._throwCustomNotifyUserError("You cannot warp because you are warp scrambled.");
                break;
            case "MICROWARPDRIVE_BLOCKED":
                this._throwCustomNotifyUserError("That module cannot be activated while you are warp scrambled.");
                break;
            case "MICRO_JUMP_DRIVE_BLOCKED":
                this._throwCustomNotifyUserError("That module cannot be activated while you are warp scrambled.");
                break;
            case "SHIP_IMMOBILE":
                this._throwCustomNotifyUserError("That module cannot be activated while your ship is immobilized.");
                break;
            ...
            case "MODULE_RESTRICTED_IN_LOWSEC":
                this._throwCustomNotifyUserError("That module cannot be activated in the current security band.");
                break;
            case "TARGET_POINT_REQUIRED":
                this._throwCustomNotifyUserError("You must choose a point in space for that module.");
                break;
            default:
                this._throwCustomNotifyUserError(`Failed to activate ${moduleName}: ${normalizedErrorMsg || "unknown error"}.`);
                break;
        }
    }
    _throwModuleDeactivationUserError(errorMsg = "", moduleItem = null) {
        switch (String(errorMsg || "").trim()) {
            ...
            case "MODULE_NOT_ACTIVE":
                ...
                break;
            default:
                this._throwCustomNotifyUserError(`Failed to deactivate ${this._resolveModuleDisplayName(moduleItem)}.`);
                break;
        }
    }
    _throwModuleOverloadUserError(errorMsg = "", moduleItem = null, data = null) {
        ...
        switch (String(errorMsg || "").trim()) {
            ...
            case "THERMODYNAMICS_SKILL_REQUIRED":
                ...
                break;
            default:
                this._throwCustomNotifyUserError(`Failed to change overload state for ${moduleName}.`);
                break;
        }
    }
    ...
    _listNaniteRepairPasteStacks(charID, shipID) {
        return listContainerItems(charID, shipID, ITEM_FLAGS.CARGO_HOLD)
            .filter((item) => Number(item && item.typeID) === TYPE_NANITE_REPAIR_PASTE &&
    ...
    _consumeNaniteRepairPaste(session, charID, shipID, requiredUnits) {
        ...
        const availableUnits = stacks.reduce((total, stack) => total + this._getNaniteRepairPasteQuantity(stack), 0);
        ...
        for (const stack of stacks) {
            ...
            const consumeUnits = Math.min(remainingUnits, this._getNaniteRepairPasteQuantity(stack));
    ...
    _resolveModuleRepairContext(session, moduleID) {
        ...
        const damageRatio = clampRatio(moduleItem.moduleState && moduleItem.moduleState.damage, 0);
        ...
        const naniteRepairAttributes = this._resolveCharacterNaniteRepairAttributes(charID);
        const rawTypeRepairCostMultiplier = getTypeAttributeValue(moduleItem.typeID, "repairCostMultiplier");
        const rawTypeRepairCostPercent = getTypeAttributeValue(moduleItem.typeID, "repairCostPercent");
        ...
        const pasteUnits = Math.max(1, Math.ceil(damageHP *
            naniteRepairAttributes.repairCostMultiplier *
    ...
    _startModuleRepair(session, moduleID, options = {}) {
        ...
        const consumeResult = this._consumeNaniteRepairPaste(session, data.charID, data.shipID, data.pasteUnits);
        if (!consumeResult.success) {
    ...
    _finishModuleRepair(session, moduleID) {
        const numericModuleID = Number(moduleID) || 0;
        const repairMap = this._getModuleRepairRuntimeMap(session);
        const repairState = repairMap && numericModuleID > 0
        ...
        const hp = this._getItemStructureHitpoints(moduleItem.typeID);
        const currentDamageRatio = clampRatio(moduleItem.moduleState && moduleItem.moduleState.damage, 0);
        const currentDamageHP = hp * currentDamageRatio;
        const elapsedMs = Math.max(0, Date.now() - (Number(repairState.startedAtMs) || Date.now()));
        const repairHP = Math.min(currentDamageHP, (elapsedMs / 60000) * Math.max(0, Number(repairState.moduleRepairRate) || 0));
        ...
        const updateResult = updateInventoryItem(numericModuleID, (currentItem) => ({
            ...
            moduleState: {
                ...
                incapacitated: nextDamageRatio >= 1
                    ? true
        ...
        syncDamageStateAttributesForSession(session, updateResult.data, updateResult.previousData || {});
        return updateResult;
    }
    Handle_AddTarget(args, session) {
        ...
        if (!result || !result.success) {
            const errorData = result && result.data && typeof result.data === "object"
                ? { ...result.data }
                : {};
        ...
        const shipID = Number(session && session._space && session._space.shipID ||
            session && (session.activeShipID || session.shipID || session.shipid)) || 0;
        ...
        return [
            ...
            this._buildTargetIDList((result.data && result.data.targets) || spaceRuntime.getTargets(session)),
        ];
    }
    ...
    _expandGroupedModuleIDs(shipID, rawModuleIDs) {
        ...
        const requestedModuleIDs = normalizedModuleIDs.length > 0
            ? normalizedModuleIDs
        ...
        for (const requestedModuleID of requestedModuleIDs) {
            ...
            const nextModuleIDs = Array.isArray(bankModuleIDs) && bankModuleIDs.length > 0
                ? bankModuleIDs
                : [numericModuleID];
    ...
    _buildGroupedUnloadTargets(charID, shipID, rawModuleIDs, quantity = null) {
        const normalizedQuantity = quantity === null || quantity === undefined
            ? null
        ...
        const requestedModuleIDs = normalizedRequestedModuleIDs.length > 0
        ...
        for (const moduleID of expandedModuleIDs) {
            ...
            const availableQuantity = Number(chargeItem && (chargeItem.stacksize || chargeItem.quantity)) || 0;
            if (availableQuantity <= 0) {
                continue;
            }
    ...
    _collectWeaponBankTouchedModuleIDs(shipID, moduleIDs = [], options = {}) {
        const numericShipID = Number(shipID) || 0;
    ...
    _repairWeaponBankModulePresentation(session, shipID, moduleIDs = []) {
        ...
        if (spaceRuntime &&
            ...
            const reconcileResult = spaceRuntime.reconcileActiveTurretWeaponBankEffects(session, numericShipID, moduleIDs);
            if (reconcileResult && reconcileResult.changed) {
                log.debug(`[DogmaIM] Reconciled active turret bank effects shipID=${numericShipID} ` +
    ...
    _throwWeaponBankMutationUserError(errorMsg = "") {
        switch (String(errorMsg || "").trim()) {
            case "MODULES_MUST_BE_ONLINE":
                this._throwCustomNotifyUserError("All weapons in the bank must be online before grouping them.");
                break;
            case "MODULE_CHARGE_MISMATCH":
                this._throwCustomNotifyUserError("All weapons in the bank must have the same loaded charge, or all be empty.");
                break;
            case "BANK_NOT_FOUND":
                this._throwCustomNotifyUserError("That weapon bank no longer exists.");
                break;
            default:
                this._throwCustomNotifyUserError("Failed to change the current weapon bank configuration.");
                break;
        }
    }
    Handle_LinkWeapons(args, session) {
        ...
        const touchedModuleIDs = this._collectWeaponBankTouchedModuleIDs(shipID, [masterModuleID, slaveModuleID]);
        ...
        return buildWeaponBankStateDict(shipID, {
            banks: result && result.data && result.data.banks
                ? result.data.banks
                : null,
            characterID: this._getCharID(session),
        });
    ...
    Handle_MergeModuleGroups(args, session) {
        ...
        const touchedModuleIDs = this._collectWeaponBankTouchedModuleIDs(shipID, [targetMasterID, sourceMasterID]);
    ...
    Handle_UnlinkModule(args, session) {
        const shipID = args && args.length > 0 ? Number(args[0]) || this._getShipID(session) : this._getShipID(session);
        const masterModuleID = args && args.length > 1 ? Number(args[1]) || 0 : 0;
        const touchedModuleIDs = this._collectWeaponBankTouchedModuleIDs(shipID, [masterModuleID]);
    ...
    Handle_LinkAllWeapons(args, session) {
        const shipID = args && args.length > 0 ? Number(args[0]) || this._getShipID(session) : this._getShipID(session);
        const touchedModuleIDs = this._collectWeaponBankTouchedModuleIDs(shipID, [], { includeAllBanks: true, session });
        ...
        if (result && result.data && result.data.changed) {
            const nextTouchedModuleIDs = this._collectWeaponBankTouchedModuleIDs(shipID, [], { includeAllBanks: true, session });
            this._repairWeaponBankModulePresentation(session, shipID, [...new Set([...touchedModuleIDs, ...nextTouchedModuleIDs])]);
        }
        return buildWeaponBankStateDict(shipID, {
            banks: result && result.data && result.data.banks
                ? result.data.banks
    ...
    _setModuleOnlineState(shipID, moduleID, online, session) {
        ...
        const structureOwnerID = Number(structureShipRecord &&
            (structureShipRecord.ownerCorpID || structureShipRecord.ownerID)) || 0;
        const moduleOwnedForHost = moduleOwnerID === charID ||
        ...
        const shipRecord = findCharacterShip(charID, numericShipID) ||
            structureShipRecord ||
            this._getActiveShipRecord(session) ||
            null;
        const creationToggle = setFrontierCreationModuleOnlineStateIfHandled(shipRecord, charID, numericModuleID, Boolean(online), session);
        if (creationToggle.handled) {
            const creationResult = creationToggle.result;
            log.debug(`[DogmaIM] SetModuleOnlineState Creation bridge shipID=${numericShipID} ` +
        ...
        if (!nextOnline) {
            const bankMasterID = getWeaponBankMasterModuleID(numericShipID, numericModuleID);
            if (bankMasterID > 0) {
        ...
        if (!nextOnline &&
            ...
            !characterCanDisableStructureServiceModule(session, moduleItem, structureShipRecord)) {
            return {
                success: false,
        ...
        if (nextOnline && !previousOnline) {
            const maxGroupOnlineLimit = resolveMaxGroupOnlineLimit(moduleItem, previousFittingSnapshot && previousFittingSnapshot.fittedItems);
            if (maxGroupOnlineLimit &&
            ...
            const onlineCandidate = previousFittingSnapshot &&
                previousFittingSnapshot.buildOnlineCandidateResourceState(moduleItem);
            const resourceState = onlineCandidate && onlineCandidate.baselineResourceState;
            const moduleResourceLoad = onlineCandidate && onlineCandidate.moduleResourceLoad;
        ...
        if (nextOnline &&
            ...
            const fuelResult = consumeStructureServiceModuleOnlineFuel(numericShipID, moduleItem);
            if (!fuelResult || fuelResult.success !== true) {
                return {
                    success: false,
                    errorMsg: fuelResult && fuelResult.errorMsg
                        ? fuelResult.errorMsg
                        : "NOT_ENOUGH_FUEL",
        ...
        if (!nextOnline && inSpace) {
            const overloadEffect = spaceRuntime.getOverloadModuleEffect(session, numericModuleID);
            if (overloadEffect) {
                spaceRuntime.stopOverloadModule(session, moduleItem, overloadEffect.effectID, { refreshDerivedState: false });
            }
        ...
        if (isStructureHost && isStructureServiceModuleItem(updateResult.data)) {
            ...
            if (!syncResult || syncResult.success !== true) {
                log.warn(`[DogmaIM] structure service module sync failed structure=${numericShipID} module=${numericModuleID} error=${syncResult && syncResult.errorMsg || "UNKNOWN"}`);
            }
            else {
                if (Array.isArray(syncResult.fuelCycleChanges) &&
        ...
        const refreshedShipStateSource = findCharacterShip(charID, numericShipID) ||
        ...
        if (isOnlineAttributeID && previousOnline !== nextOnline) {
            onlineToggleSubEvents.push(buildModuleAttributeChangeEvent(charID, numericModuleID, isOnlineAttributeID, nextOnline ? 1 : 0, previousOnline ? 1 : 0, onlineToggleTime));
        }
        if (previousFittingSnapshot && nextFittingSnapshot) {
            for (const change of listShipFittingAttributeChanges(previousFittingSnapshot, nextFittingSnapshot)) {
                onlineToggleSubEvents.push(buildModuleAttributeChangeEvent(charID, numericShipID, change.attributeID, change.nextValue, change.previousValue, onlineToggleTime));
            }
        }
        onlineToggleSubEvents.push(buildGodmaShipEffectEvent(numericModuleID, charID, numericShipID, EFFECT_ONLINE, onlineToggleTime, { isStart: nextOnline ? 1 : 0, shouldStart: nextOnline ? 1 : 0 }));
        sendOnMultiEvent(session, onlineToggleSubEvents, onlineToggleTime);
        log.debug(`[DogmaIM] SetModuleOnlineState applied shipID=${numericShipID} ` +
        ...
        if (inSpace) {
            if (nextOnline && !previousOnline) {
                spaceRuntime.setShipCapacitorRatio(session, ONLINE_CAPACITOR_REMAINDER_RATIO / 100);
            }
            spaceRuntime.refreshShipDerivedState(session, {
                broadcast: true,
            });
    ...
    _resolveAmmoSourceStacks(charID, ammoLocationID, sourceFlagID, chargeTypeID, chargeRequests = []) {
        const explicitItemIDs = new Set(chargeRequests
            .map((request) => Number(request && request.itemID) || 0)
            .filter((itemID) => itemID > 0));
        const requestedTypeIDs = new Set(chargeRequests
        ...
        return locationItems.sort((left, right) => (Number(left.itemID) || 0) - (Number(right.itemID) || 0));
    }
    _resolveRequestedAmmoTypeID(charID, ammoLocationID, sourceFlagID, chargeRequests = []) {
        for (const request of chargeRequests) {
            ...
            if (candidate &&
                ...
                if (Number(candidate.locationID) === ammoLocationID &&
    ...
    _resolvePendingReloadSourceStacks(charID, ammoLocationID, sourceFlagID, chargeTypeID, sourceItemIDs = []) {
    ...
    _queuePendingModuleReload(session, moduleItem, options = {}) {
        ...
        const reloadTimeMs = Math.max(0, Math.round(Number(options.reloadTimeMs) || this._getModuleReloadTimeMs(moduleItem)));
        if (reloadTimeMs <= 0) {
            return {
                success: false,
        ...
        const reloadState = {
            ...
            inventoryOwnerID: Number(options.inventoryOwnerID) ||
                this._getDogmaInventoryOwnerID(session, Number(options.shipID) || Number(moduleItem.locationID) || 0),
            sourceOwnerID: Number(options.sourceOwnerID) ||
                this._getDogmaInventoryOwnerID(session, Number(options.ammoLocationID) || Number(moduleItem.locationID) || 0),
            ...
            quantity: options.quantity === undefined || options.quantity === null
                ? null
        ...
        if (reloadState.chargeTypeID > 0) {
            this._notifyChargeBeingLoadedToModule(session, [numericModuleID], reloadState.chargeTypeID, reloadTimeMs);
        }
        return {
            success: true,
            data: {
    ...
    queueAutomaticModuleReload(session, moduleItem, options = {}) {
        ...
        const shipID = Number(options.shipID) ||
        ...
        const sourceStacks = this._resolvePendingReloadSourceStacks(sourceOwnerID, ammoLocationID, sourceFlagID, chargeTypeID, options.sourceItemIDs);
        ...
        const requestedQuantity = options.quantity === undefined || options.quantity === null
        ...
        return this._queuePendingModuleReload(session, normalizedModuleItem, {
            ...
            reloadTimeMs: Number(options.reloadTimeMs) || this._getModuleReloadTimeMs(normalizedModuleItem),
            quantity: requestedQuantity,
        });
    }
    _completePendingModuleReload(reloadState, nowMs = getReloadStateCurrentTimeMs(reloadState, Date.now())) {
        if (!reloadState) {
            return {
                success: false,
        ...
        schedulePendingModuleReloadPump();
        const session = reloadState.session &&
        ...
        const inventoryOwnerID = Number(reloadState.inventoryOwnerID) ||
            this._getDogmaInventoryOwnerID(session, reloadState.shipID) ||
            charID;
        const sourceOwnerID = Number(reloadState.sourceOwnerID) ||
        ...
        const previousNextActivationTime = toFileTimeFromMs(Number(reloadState.completeAtMs) || nowMs, 0n);
        if (!moduleItem ||
        ...
        const previousChargeState = this._captureChargeStateSnapshot(inventoryOwnerID, shipID, moduleFlagID);
        const previousChargeItem = this._captureChargeItemSnapshot(inventoryOwnerID, shipID, moduleFlagID);
        try {
            if (reloadState.action === "load") {
                ...
                if (chargeTypeID > 0 &&
                    ...
                    const sourceStacks = this._resolvePendingReloadSourceStacks(sourceOwnerID, reloadState.ammoLocationID, reloadState.sourceFlagID, chargeTypeID, reloadState.sourceItemIDs);
                    if (sourceStacks.length > 0 ||
                        (existingCharge && activeChargeTypeID === chargeTypeID)) {
                        if (existingCharge && activeChargeTypeID !== chargeTypeID) {
                            const unloadResult = this._moveLoadedChargeToDestination(existingCharge, reloadState.ammoLocationID, reloadState.sourceFlagID);
                        ...
                        for (const sourceCharge of sourceStacks) {
                            ...
                            const moveResult = existingCharge && activeChargeTypeID === chargeTypeID
                                ? mergeItemStacks(chargeItem.itemID, existingCharge.itemID, moveQuantity)
                                : moveItemToLocation(chargeItem.itemID, shipID, moduleFlagID, moveQuantity);
                            ...
                            if (existingCharge && activeChargeTypeID === chargeTypeID) {
                                ...
                                existingCharge = getLoadedChargeByFlag(inventoryOwnerID, shipID, moduleFlagID);
            ...
            const nextChargeState = this._captureChargeStateSnapshot(inventoryOwnerID, shipID, moduleFlagID);
            const nextChargeItem = this._captureChargeItemSnapshot(inventoryOwnerID, shipID, moduleFlagID);
            this._notifyChargeQuantityTransition(session, charID, shipID, moduleFlagID, previousChargeState, nextChargeState, {
                previousChargeItem,
                nextChargeItem,
            });
            this._notifyWeaponModuleAttributeTransition(session, moduleItem, previousChargeItem, nextChargeItem);
            this._refreshScannerProbeLauncherClientState(session, shipID, moduleItem, {
    ...
    Handle_Activate(args, session) {
        ...
        const effectName = this._normalizeActivationEffectName(args && args.length > 1 ? args[1] : "");
        ...
        const groupedMasterModuleID = requestedItem &&
            Number(requestedItem.categoryID) === 7
            ? getWeaponBankMasterModuleID(Number(requestedItem.locationID) || this._getShipID(session), requestedItemID)
            : 0;
        const itemID = groupedMasterModuleID || requestedItemID;
        const item = groupedMasterModuleID > 0
            ? findItemById(itemID)
            : requestedItem;
        log.debug(`[DogmaIM] Activate(itemID=${itemID}, effect=${effectName}, target=${String(targetID)}, repeat=${String(repeat)}) ` +
        ...
        if (effectName === "online") {
            ...
            if (!result.success) {
                log.warn(`[DogmaIM] Activate online rejected itemID=${itemID} shipID=${shipID} error=${result.errorMsg}`);
                this._throwModuleOnlineUserError(result.errorMsg, item, result.data);
            }
            return 1;
        }
        if (!item || !isEffectivelyOnlineModule(item)) {
            log.warn(`[DogmaIM] Activate rejected itemID=${itemID} effect=${effectName} error=MODULE_NOT_ONLINE`);
        ...
        const isPropulsion = effectName === "moduleBonusAfterburner" ||
            effectName === "moduleBonusMicrowarpdrive";
        const isProbeLauncherActivation = effectName === "useMissiles" &&
            item &&
            Number(item.groupID) === GROUP_SCAN_PROBE_LAUNCHER;
        const isInterdictionProbeLauncherActivation = effectName === "useMissiles" &&
            item &&
            Number(item.groupID) === GROUP_INTERDICTION_SPHERE_LAUNCHER;
        const isWarpDisruptFieldGeneratorActivation = item &&
            Number(item.groupID) === GROUP_WARP_DISRUPT_FIELD_GENERATOR &&
            warpDisruptFieldGeneratorRuntime.isWarpDisruptFieldGeneratorActivation(item, effectName);
        ...
        if (analyzerHackingContext && analyzerHackingContext.success !== true) {
            log.debug(`[DogmaIM] Activate analyzer rejected itemID=${itemID} target=${String(targetID)} ` +
        ...
        if (isProbeLauncherActivation) {
            this._refreshScannerProbeLauncherClientState(session, Number(item && item.locationID) || this._getShipID(session), item, {
                forceRuntimeSync: true,
            });
        }
        const activationRepeat = isProbeLauncherActivation ||
        ...
        if (!result.success) {
            log.warn(`[DogmaIM] Activate rejected itemID=${itemID} effect=${effectName} error=${result.errorMsg}`);
        ...
        if (isProbeLauncherActivation) {
            if (result && result.data && result.data.effectState) {
                ...
                result.data.effectState.consequenceDelivery = "managed";
            ...
            try {
                ...
                if (result &&
                    ...
                    result.data.effectState.autoReloadOnCycleEnd = {
                        ...
                        ammoLocationID: Number(item && item.locationID) || this._getShipID(session),
                    };
                }
                this._refreshScannerProbeLauncherClientState(session, Number(item && item.locationID) || this._getShipID(session), item, {
                    forceRuntimeSync: true,
        ...
        if (isInterdictionProbeLauncherActivation) {
            ...
            try {
                const launchResult = this._launchInterdictionProbeFromContext(session, interdictionProbeLaunchContext);
                if (result &&
                    result.data &&
                    result.data.effectState &&
                    launchResult &&
        ...
        log.debug(`[DogmaIM] Activate accepted itemID=${itemID} effect=${effectName} ` +
            `module=${JSON.stringify(summarizeModuleItemForLog(item))} ` +
            `runtime=${JSON.stringify(summarizeRuntimeEffectForLog(session && session._space
    ...
    Handle_Deactivate(args, session) {
        ...
        if (effectName === "online") {
            ...
            if (!result.success) {
                log.warn(`[DogmaIM] Deactivate online rejected itemID=${itemID} shipID=${shipID} error=${result.errorMsg}`);
        ...
        if (!result.success) {
            log.warn(`[DogmaIM] Deactivate rejected itemID=${itemID} effect=${effectName} error=${result.errorMsg}`);
            this._throwModuleDeactivationUserError(result.errorMsg, item || requestedItem);
        }
        log.debug(`[DogmaIM] Deactivate accepted itemID=${itemID} effect=${effectName} ` +
            `module=${JSON.stringify(summarizeModuleItemForLog(item))} ` +
            `runtime=${JSON.stringify(summarizeRuntimeEffectForLog(session && session._space
                ? spaceRuntime.getActiveModuleEffect(session, itemID)
                : null))}`);
    ...
    Handle_Overload(args, session) {
        ...
        const item = findItemById(itemID);
        log.debug(`[DogmaIM] Overload(itemID=${itemID}, effectID=${effectID}) ` +
            `module=${JSON.stringify(summarizeModuleItemForLog(item))} inSpace=${Boolean(session && session._space)}`);
        const result = spaceRuntime.overloadModule(session, item, effectID);
        if (!result.success) {
            log.warn(`[DogmaIM] Overload rejected itemID=${itemID} effectID=${effectID} error=${result.errorMsg}`);
    ...
    Handle_OverloadRack(args, session) {
        const itemID = args && args.length > 0 ? Number(args[0]) || 0 : 0;
        const item = findItemById(itemID);
        log.debug(`[DogmaIM] OverloadRack(itemID=${itemID}) ` +
            `module=${JSON.stringify(summarizeModuleItemForLog(item))} inSpace=${Boolean(session && session._space)}`);
    ...
    Handle_StopOverload(args, session) {
        ...
        const effectID = args && args.length > 1 ? Number(args[1]) || 0 : 0;
        const item = findItemById(itemID);
        log.debug(`[DogmaIM] StopOverload(itemID=${itemID}, effectID=${effectID}) ` +
            `module=${JSON.stringify(summarizeModuleItemForLog(item))} inSpace=${Boolean(session && session._space)}`);
        const result = spaceRuntime.stopOverloadModule(session, item, effectID);
        if (!result.success) {
            log.warn(`[DogmaIM] StopOverload rejected itemID=${itemID} effectID=${effectID} error=${result.errorMsg}`);
            this._throwModuleOverloadUserError(result.errorMsg, item, result.data);
        }
        return itemID;
    }
    Handle_StopOverloadRack(args, session) {
        const itemID = args && args.length > 0 ? Number(args[0]) || 0 : 0;
        const item = findItemById(itemID);
        log.debug(`[DogmaIM] StopOverloadRack(itemID=${itemID}) ` +
            `module=${JSON.stringify(summarizeModuleItemForLog(item))} inSpace=${Boolean(session && session._space)}`);
    ...
    Handle_InitiateModuleRepair(args, session) {
        ...
        if (result.success) {
            log.debug(`[DogmaIM] InitiateModuleRepair accepted itemID=${itemID} ` +
                `pasteUnits=${result.data && result.data.pasteUnits || 0}`);
            return true;
        }
        log.debug(`[DogmaIM] InitiateModuleRepair rejected itemID=${itemID} error=${result.errorMsg}`);
    ...
    Handle_InitiateModuleRepairMany(args, session) {
        ...
        log.debug(`[DogmaIM] InitiateModuleRepairMany success=${success.length} ` +
    ...
    Handle_StopModuleRepair(args, session) {
        ...
        if (!result.success) {
            log.debug(`[DogmaIM] StopModuleRepair rejected itemID=${itemID} error=${result.errorMsg}`);
            return false;
        }
    ...
    Handle_CreateNewbieShip(args, session) {
        const requestedShipID = args && args.length > 0 ? Number(args[0]) || 0 : this._getShipID(session);
        const requestedLocationID = args && args.length > 1 ? Number(args[1]) || 0 : this._getLocationID(session);
        const stationID = getDockedLocationID(session) || 0;
        log.info(`[DogmaIM] CreateNewbieShip(shipID=${requestedShipID}, locationID=${requestedLocationID})`);
    ...
    Handle_LaunchProbes(args, session) {
        const moduleID = args && args.length > 0
            ? Number(args[0]) || 0
            : 0;
        const requestedCount = Math.max(1, Number(args && args.length > 1 ? args[1] : 1) || 1);
        const shipID = this._getShipID(session);
        const charID = this._getCharID(session);
        const systemID = Number((session && session.solarsystemid2) ||
            (session && session.solarsystemid) ||
            (session && session._space && session._space.systemID) ||
            0) || 0;
        log.info(`[DogmaIM] LaunchProbes(moduleID=${moduleID}, requestedCount=${requestedCount}, shipID=${shipID}, charID=${charID}, systemID=${systemID})`);
        const probeLaunchContext = this._resolveValidatedProbeLaunchContext(session, moduleID, requestedCount);
    ...
    Handle_LoadAmmo(args, session) {
        ...
        log.info(`[DogmaIM] LoadAmmo(shipID=${shipID}, modules=[${moduleIDs}], charges=[${summarizeAmmoLoadRequests(chargeRequests)}], ammoLocationID=${ammoLocationID})`);
        ...
        for (const moduleID of moduleIDs.map((value) => Number(value) || 0).filter((value) => value > 0)) {
            ...
            if (!moduleItem ||
                Number(moduleItem.ownerID) !== inventoryOwnerID ||
                Number(moduleItem.locationID) !== shipID) {
                log.warn(`[DogmaIM] LoadAmmo: module ${moduleID} not found or not owned (owner=${moduleItem && moduleItem.ownerID}, loc=${moduleItem && moduleItem.locationID}, inventoryOwnerID=${inventoryOwnerID}, charID=${charID}, shipID=${shipID})`);
                continue;
            }
            const previousChargeState = this._captureChargeStateSnapshot(inventoryOwnerID, shipID, moduleItem.flagID);
            const previousChargeItem = this._captureChargeItemSnapshot(inventoryOwnerID, shipID, moduleItem.flagID);
            try {
                let existingCharge = getLoadedChargeByFlag(inventoryOwnerID, shipID, moduleItem.flagID);
                let activeChargeTypeID = existingCharge ? Number(existingCharge.typeID) || 0 : 0;
                let requestedChargeTypeID = this._resolveRequestedAmmoTypeID(sourceOwnerID, ammoLocationID, sourceFlagID, chargeRequests);
                if (requestedChargeTypeID <= 0 && activeChargeTypeID > 0) {
                    requestedChargeTypeID = activeChargeTypeID;
                }
                if (requestedChargeTypeID <= 0) {
                    log.warn(`[DogmaIM] LoadAmmo: no valid charge found for module ${moduleID} (flag=${moduleItem.flagID}) in location ${ammoLocationID} requests=[${summarizeAmmoLoadRequests(chargeRequests)}]`);
                    continue;
                }
                const chargeTypeID = requestedChargeTypeID;
                if (!isChargeCompatibleWithModule(moduleItem.typeID, chargeTypeID)) {
                    log.warn(`[DogmaIM] LoadAmmo: incompatible charge typeID=${chargeTypeID} for module ${moduleID} typeID=${moduleItem.typeID}`);
                    continue;
                }
                const moduleCapacity = getModuleChargeCapacity(moduleItem.typeID, chargeTypeID);
                const existingQuantity = existingCharge
                    ? Number(existingCharge.stacksize || existingCharge.quantity || 0) || 0
                    : 0;
                const resolvedChargeSources = this._resolveAmmoSourceStacks(sourceOwnerID, ammoLocationID, sourceFlagID, chargeTypeID, chargeRequests);
                if (session &&
                    session._space &&
                    ...
                    if (resolvedChargeSources.length === 0 &&
                        !(existingCharge && activeChargeTypeID === chargeTypeID)) {
                        log.warn(`[DogmaIM] LoadAmmo: no source stacks resolved for reload module ${moduleID} typeID=${chargeTypeID} in location ${ammoLocationID}`);
                        continue;
                    }
                ...
                if (existingCharge && activeChargeTypeID !== chargeTypeID) {
                    const unloadResult = this._moveLoadedChargeToDestination(existingCharge, ammoLocationID, sourceFlagID);
                    if (unloadResult.success) {
                        this._syncInventoryChanges(session, unloadResult.data.changes);
                    }
                    existingCharge = null;
                ...
                if (resolvedChargeSources.length === 0) {
                    log.warn(`[DogmaIM] LoadAmmo: no source stacks resolved for module ${moduleID} typeID=${chargeTypeID} in location ${ammoLocationID}`);
                    continue;
                }
                for (const sourceCharge of resolvedChargeSources) {
                    ...
                    const moveResult = existingCharge && activeChargeTypeID === chargeTypeID
                        ? mergeItemStacks(chargeItem.itemID, existingCharge.itemID, moveQuantity)
                        : moveItemToLocation(chargeItem.itemID, shipID, moduleItem.flagID, moveQuantity);
                    if (!moveResult.success) {
                        log.warn(`[DogmaIM] LoadAmmo: move failed for charge ${chargeItem.itemID} -> module flag ${moduleItem.flagID}: ${moveResult.errorMsg}`);
                        continue;
                    }
                    log.info(`[DogmaIM] LoadAmmo: loaded ${moveQuantity}x typeID=${chargeTypeID} into module ${moduleID} (flag=${moduleItem.flagID})`);
                    neededQuantity -= moveQuantity;
                    this._syncInventoryChanges(session, moveResult.data.changes);
                    if (existingCharge && activeChargeTypeID === chargeTypeID) {
                        existingCharge = findItemById(existingCharge.itemID) || existingCharge;
                        ...
                        existingCharge = getLoadedChargeByFlag(inventoryOwnerID, shipID, moduleItem.flagID);
                        activeChargeTypeID = existingCharge ? Number(existingCharge.typeID) || 0 : 0;
                    }
                }
            }
            finally {
                const nextChargeState = this._captureChargeStateSnapshot(inventoryOwnerID, shipID, moduleItem.flagID);
                const nextChargeItem = this._captureChargeItemSnapshot(inventoryOwnerID, shipID, moduleItem.flagID);
                this._notifyChargeQuantityTransition(session, charID, shipID, moduleItem.flagID, previousChargeState, nextChargeState, {
                    previousChargeItem,
                    nextChargeItem,
                });
                this._notifyWeaponModuleAttributeTransition(session, moduleItem, previousChargeItem, nextChargeItem);
                const shouldForceScannerProbeRuntimeSync = session &&
                    session._space &&
    ...
    Handle_UnloadAmmo(args, session) {
        ...
        const charID = this._getCharID(session);
        const inventoryOwnerID = this._getDogmaInventoryOwnerID(session, shipID);
        const unloadTargets = this._buildGroupedUnloadTargets(inventoryOwnerID, shipID, rawModuleIDs, quantity);
        const resolvedDestination = this._resolveUnloadDestination(destination, session, shipID);
        log.debug(`[DogmaIM] UnloadAmmo(shipID=${shipID}, moduleCount=${unloadTargets.length}, destination=${JSON.stringify(resolvedDestination)})`);
        for (const unloadTarget of unloadTargets) {
            ...
            if (typeHasEffectName(moduleItem.typeID, "rigSlot")) {
                log.debug(`[DogmaIM] UnloadAmmo skipping rig module moduleID=${moduleID} typeID=${moduleItem.typeID} — rigs cannot be unloaded`);
                continue;
            }
            const chargeItem = getLoadedChargeByFlag(inventoryOwnerID, shipID, moduleItem.flagID);
            if (!chargeItem) {
                continue;
            }
            const previousChargeState = this._captureChargeStateSnapshot(inventoryOwnerID, shipID, moduleItem.flagID);
            const previousChargeItem = this._captureChargeItemSnapshot(inventoryOwnerID, shipID, moduleItem.flagID);
            try {
                const unloadResult = this._moveLoadedChargeToDestination(chargeItem, resolvedDestination.locationID, resolvedDestination.flagID, unloadTarget.quantity);
                if (!unloadResult.success) {
                    continue;
                }
                this._syncInventoryChanges(session, unloadResult.data.changes);
    ...
    Handle_LoadFuel(args, session) {
        ...
        const requestKey = [
            ...
            fuelTypeID,
            quantity,
            JSON.stringify(fuelItems, (key, value) => typeof value === "bigint" ? value.toString() : value),
            sourceLocationID,
        ].join("|");
        ...
        if (lastRequest &&
            ...
            log.info(`[DogmaIM] LoadFuel duplicate request suppressed ship=${shipID} ` +
                `type=${fuelTypeID} qty=${quantity} ageMs=${nowMs - lastRequest.atMs}`);
            return null;
        }
        const charData = this._getCharacterRecord(session) || {};
        const creationDogmaContext = resolveFrontierCreationDogmaContext(shipContext.shipMetadata, charID, session);
        const shipAttributes = this._buildShipAttributes(charData, shipContext.shipMetadata, session, { creationDogmaContext });
        const fuelCapacity = Number(shipAttributes && shipAttributes[ATTRIBUTE_FUEL_CAPACITY]) || 0;
        ...
        if (dockedLocationID > 0 && fuelTypeID === 77818) {
            ...
            const previousFuelCharge = Math.min(Math.max(0, getShipFuelCharge(shipContext.shipMetadata)), Math.max(0, fuelCapacity));
            if (quantity <= 0) {
                ...
                if (quantity <= 0) {
                    ...
                    log.info(`[DogmaIM] LoadFuel zero-unit station request refreshed full tank ` +
                        `ship=${shipID} fuelCharge=${previousFuelCharge}/${fuelCapacity}`);
                    return null;
                }
                log.info(`[DogmaIM] LoadFuel inferred station fill ship=${shipID} ` +
        ...
        if (!loadResult.success) {
            log.info(`[DogmaIM] LoadFuel rejected ship=${shipID} type=${fuelTypeID} ` +
                `qty=${quantity} error=${loadResult.errorMsg}`);
            this._throwCustomNotifyUserError(this._formatLoadFuelUserError(loadResult, fuelTypeID));
        }
        if (session) {
            session._lastLoadFuelRequest = { key: requestKey, atMs: nowMs };
        }
    ...
    Handle_GetAllInfo(args, session) {
        ...
        this._normalizeControlledStructureFittingOwners(shipContext);
        if (shipContext.controllingStructure) {
            const syncResult = syncStructureServiceModuleState(shipID);
            if (!syncResult || syncResult.success !== true) {
                log.warn(`[DogmaIM] GetAllInfo structure service sync failed structure=${shipID} error=${syncResult && syncResult.errorMsg || "UNKNOWN"}`);
            }
            else if (Array.isArray(syncResult.fuelCycleChanges) &&
                syncResult.fuelCycleChanges.length > 0) {
                this._syncInventoryChanges(session, syncResult.fuelCycleChanges);
            }
        }
        ...
        const getStructureInfo = this._toBoolArg(args && args[2], Boolean(session && (session.structureid || session.structureID)));
        const creationDogmaContext = (getShipInfo && !shipContext.controllingStructure) ? resolveFrontierCreationDogmaContext(shipMetadata, charID, session) : null;
        ...
        const getFittingContext = () => {
            ...
            if (!fittingContext) {
                fittingContext = this._buildFittingHydrationContext(charID, shipID, shipMetadata, {
        ...
        if (getShipInfo && !shipContext.controllingStructure) {
            const fittingParityResult = ensureShipFittingInventoryParity(charID, shipContext.shipRecord || shipID, {
                logFailures: true,
            });
            if (fittingParityResult &&
        ...
        fittingContext = getFittingContext();
        const includeDockedShipInfoBootstrap = getShipInfo &&
            !getCharInfo &&
            isDockedSession(session);
        const includeOptionalDogmaBootstrap = getShipInfo && (getCharInfo ||
            getStructureInfo ||
        ...
        const deferLoginShipFittingBootstrap = getShipInfo && this._shouldDeferLoginShipFittingBootstrap(session);
        const primeLoginShipInfoChargeSublocations = getShipInfo && (shipContext.controllingStructure ||
            this._shouldPrimeLoginShipInfoChargeSublocations(session, {
                controllingStructure: false,
            }));
        const includeLoginShipInfoLoadedCharges = getShipInfo &&
            !shipContext.controllingStructure &&
            this._shouldIncludeLoginShipInfoLoadedCharges(session);
        const includeShipModifiedCharAttribs = getShipInfo && getCharInfo && !primeLoginShipInfoChargeSublocations;
        const characterLocationID = this._getCharacterItemLocationID(session, {
            allowShipLocation: getShipInfo,
        });
        const resolvedShipLocationID = this._resolveActiveShipLocationID(session, shipMetadata, locationID);
        const resolvedShipFlagID = this._resolveActiveShipFlagID(session, shipMetadata, 4);
        const locationInfo = this._buildEmptyDict();
        const getAllInfoLocationInfo = getCharInfo && getShipInfo && !primeLoginShipInfoChargeSublocations
            ? locationInfo
            : null;
        ...
        const shipInfoEntry = getShipInfo
            ? (this._getCachedDockedItemInfoEntry(session, shipID, shipMetadata) ||
                this._buildCommonGetInfoEntry({
                    ...
                    quantity: shipMetadata.quantity === undefined ||
                        shipMetadata.quantity === null
                        ? -1
                        : shipMetadata.quantity,
                    singleton: shipMetadata.singleton === undefined ||
                        shipMetadata.singleton === null
                        ? 1
                        : shipMetadata.singleton,
                    stacksize: shipMetadata.stacksize === undefined ||
                        shipMetadata.stacksize === null
                        ? 1
                        : shipMetadata.stacksize,
                    customInfo: shipContext.controllingStructure
                        ? ""
                        : (shipMetadata.customInfo || null),
                    preserveNullCustomInfo: !shipContext.controllingStructure,
        ...
        const shipInventoryInfoEntries = getShipInfo
            ? this._buildShipInventoryInfoEntries(charID, shipID, shipMetadata.ownerID || ownerID, resolvedShipLocationID, session, {
            ...
            : [];
        const shipInfoTupleChargeEntries = shipInventoryInfoEntries.filter((entry) => Array.isArray(Array.isArray(entry) ? entry[0] : null)).length;
        if (!shipContext.controllingStructure &&
            primeLoginShipInfoChargeSublocations &&
            shipInfoTupleChargeEntries > 0) {
        ...
        if (getShipInfo && !shipContext.controllingStructure) {
            this._queuePostGetAllInfoCreationAttributeRefresh(session, charID, shipID, shipAttributeValues, creationDogmaContext);
        }
        else if (session && session._space) {
            delete session._space.pendingDogmaGetAllInfoCreationAttributeRefresh;
        }
        log.debug(`[DogmaIM] GetAllInfo shipInfo entries=${shipInventoryInfoEntries.length} ` +
            ...
            [
                ...
                includeOptionalDogmaBootstrap
                    ? this._buildActivationState(charID, shipID, shipContext.shipRecord, {
                        ...
                        fittingContext,
                        creationModuleItems,
                        includeCharges: shipContext.controllingStructure
                            ? true
                            : !isDockedSession(session),
            ...
            args: {
                ...
                entries: [
                    [
                        ...
                        buildSystemWideEffectsPayloadForSystem(Number(session && (session.solarsystemid2 || session.solarsystemid)) || 0) || buildEmptySystemWideEffectsPayload(),
                    ],
                    [
                        "shipModifiedCharAttribs",
                        includeShipModifiedCharAttribs
                            ? this._buildShipModifiedCharacterAttributeInfo(charID, charData, characterLocationID, session, {
                                fittingContext,
                            })
                            : null,
                    ],
                    [
                        ...
                        dockedStructureRecord
                            ? this._buildStructureInfoDict(dockedStructureRecord, session, {
                                includeActivationAttributes: !shipContext.controllingStructure,
                            })
                            : this._buildEmptyDict(),
                    ],
                    ...
                    // Keep the first five fields in the order emitted by the controlled-structure
                    // golden capture. These compatibility fields remain present after them because the
                    // live V24.01 client raises AttributeError when activeShipID/shipState/charInfo are
                    // absent during undock processing.
                    ...optionalDogmaBootstrapEntries,
                    [
                        "charInfo",
                        includeCharInfo
                            ? [
                                this._buildCharacterInfoDict(charID, charData, characterLocationID, session),
        ...
        const shipState = getShipInfo
            ...
            : null;
        const shipStateEntries = shipState && Array.isArray(shipState) && shipState[0] && shipState[0].type === "dict"
            ? shipState[0].entries.length
            : 0;
        const chargeStateEntries = shipState && Array.isArray(shipState) && shipState[1] && shipState[1].type === "dict"
        ...
        if (elapsedMs >= 100) {
            log.info(`[DogmaIM] GetAllInfo took ${elapsedMs}ms ship=${shipID} ` +
    ...
    Handle_ShipGetInfo(args, session) {
        ...
        const locationID = this._resolveActiveShipLocationID(session, shipMetadata, this._getLocationID(session));
        const entry = this._buildCommonGetInfoEntry({
            ...
            groupID: shipMetadata.groupID,
            categoryID: shipMetadata.categoryID,
            quantity: shipMetadata.quantity === undefined || shipMetadata.quantity === null
                ? -1
                : shipMetadata.quantity,
            singleton: shipMetadata.singleton === undefined || shipMetadata.singleton === null
                ? 1
                : shipMetadata.singleton,
            stacksize: shipMetadata.stacksize === undefined || shipMetadata.stacksize === null
                ? 1
                : shipMetadata.stacksize,
            customInfo: shipContext.controllingStructure
                ? ""
            ...
            attributes: shipContext.controllingStructure
                ...
                : this._buildShipAttributeDict(this._getCharacterRecord(session) || {}, shipMetadata, session),
            session,
        });
    ...
    Handle_CharGetInfo(args, session) {
        ...
        const charData = this._getCharacterRecord(session) || {};
        const characterLocationID = this._getCharacterItemLocationID(session);
        return this._buildCharacterInfoDict(charID, charData, characterLocationID, session);
    }
    Handle_ItemGetInfo(args, session) {
        ...
        const charID = this._getCharID(session);
        const charData = this._getCharacterRecord(session) || {};
        const skillRecord = getCharacterSkills(charID).find((skill) => skill.itemID === requestedItemID || skill.itemID === Number.parseInt(String(requestedItemID), 10)) || null;
        ...
        if (inventoryContext && inventoryContext.item) {
            ...
            const cachedEntry = this._getCachedDockedItemInfoEntry(session, requestedItemID, item);
            if (cachedEntry) {
                return cachedEntry;
            }
            const entry = this._buildCommonGetInfoEntry({
        ...
        if (shipContext.controllingStructure &&
            ...
            const cachedEntry = this._getCachedDockedItemInfoEntry(session, requestedItemID, shipContext.shipMetadata);
            if (cachedEntry) {
                return cachedEntry;
            }
            const entry = this._buildCommonGetInfoEntry({
                ...
                locationID: this._resolveActiveShipLocationID(session, shipContext.shipMetadata, this._getLocationID(session)),
                flagID: this._resolveActiveShipFlagID(session, shipContext.shipMetadata, 0),
                groupID: shipContext.shipMetadata.groupID,
                categoryID: shipContext.shipMetadata.categoryID,
                quantity: shipContext.shipMetadata.quantity === undefined ||
                    shipContext.shipMetadata.quantity === null
                    ? -1
                    : shipContext.shipMetadata.quantity,
                singleton: shipContext.shipMetadata.singleton === undefined ||
                    shipContext.shipMetadata.singleton === null
                    ? 1
                    : shipContext.shipMetadata.singleton,
                stacksize: shipContext.shipMetadata.stacksize === undefined ||
                    shipContext.shipMetadata.stacksize === null
                    ? 1
                    : shipContext.shipMetadata.stacksize,
                customInfo: shipContext.shipMetadata.customInfo || "",
                description: "item",
                attributes: this._buildInventoryItemAttributeDict(shipContext.shipMetadata, session),
                session,
            });
            this._cacheDockedItemInfoEntry(session, requestedItemID, shipContext.shipMetadata, entry);
            return entry;
        }
    ...
    Handle_QueryAttributeValue(args, session) {
        ...
        log.debug(`[DogmaIM] QueryAttributeValue(itemID=${requestedItemID}, attributeID=${attributeID})`);
        if (!Number.isInteger(attributeID)) {
            return null;
        }
        const context = this._resolveItemAttributeContext(requestedItemID, session);
    ...
    Handle_FullyDescribeAttribute(args, session) {
        ...
        log.debug(`[DogmaIM] FullyDescribeAttribute(itemID=${requestedItemID}, attributeID=${attributeID})`);
        const context = this._resolveItemAttributeContext(requestedItemID, session);
    ...
    Handle_InjectImplant(args, session) {
        ...
        if (!result.success) {
            log.warn(`[DogmaIM] InjectImplant rejected itemID=${itemID} error=${result.errorMsg || "UNKNOWN"}`);
    ...
    Handle_DestroyImplant(args, session) {
        ...
        if (!result.success) {
            log.warn(`[DogmaIM] DestroyImplant rejected itemID=${itemID} error=${result.errorMsg || "UNKNOWN"}`);
            this._throwCustomNotifyUserError(this._formatImplantUserError(result.errorMsg));
        }
        if (queueSnapshot) {
            this._prepareQueueForAttributeChange(charID, "implant", queueSnapshot);
    ...
    Handle_UseBooster(args, session) {
        ...
        const result = useBoosterItem(charID, itemID, locationID > 0 ? locationID : null);
        if (!result.success) {
            log.warn(`[DogmaIM] UseBooster rejected itemID=${itemID} error=${result.errorMsg || "UNKNOWN"}`);
    ...
    _sendPostUndockDogmaMultiEvent(session) {
        ...
        const shipRecord = shipContext.shipRecord ||
            this._getActiveShipRecord(session) ||
            shipContext.shipMetadata ||
            {};
        const fittingContext = this._buildFittingHydrationContext(charID, shipID, shipContext.shipMetadata || shipRecord);
        const shipAttributes = this._buildShipAttributes(this._getCharacterRecord(session) || {}, shipRecord, session, { fittingContext });
        ...
        for (const attributeID of shipAttributeIDs) {
            ...
            subEvents.push(buildModuleAttributeChangeEvent(charID, shipID, attributeID, value, value, time));
        }
        ...
        for (const moduleItem of getFittedModuleItems(charID, shipID)) {
            ...
            subEvents.push(buildModuleAttributeChangeEvent(charID, moduleItem.itemID, isOnlineAttributeID, 1, 1, time));
        }
    ...
    Handle_MachoBindObject(args, session, kwargs) {
        ...
        log.debug(`[DogmaIM] MachoBindObject args.length=${args ? args.length : 0} bindParams=${JSON.stringify(bindParams, (k, v) => (typeof v === "bigint" ? v.toString() : v))} nestedCall=${JSON.stringify(nestedCall, (k, v) => (typeof v === "bigint" ? v.toString() : Buffer.isBuffer(v) ? v.toString("utf8") : v))}`);
    ...
    afterCallResponse(methodName, session, context = {}) {
        ...
        if (this._isControllingStructureSession(session)) {
            // The controlled fitting is already authoritative in GetAllInfo. TQ does
            // not replay each structure module as a synthetic inventory transition
            // after the response; its later batched recalculation concerns the prior
            // ship/character activation state.
            return;
        }
 ...
 */
DogmaService.flushPendingModuleReloads = function flushPendingModuleReloads(nowMs = Date.now()) {
