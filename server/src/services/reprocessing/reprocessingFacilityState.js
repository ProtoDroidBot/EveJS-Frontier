...
const { REPROCESSING_FACILITY_STATE_TABLE, } = require("./reprocessingConstants");
const { getReprocessingRigProfile, } = require("./reprocessingStaticData");
...
function normalizeRigTypeIDs(rigTypeIDs = []) {
    return [...new Set((Array.isArray(rigTypeIDs) ? rigTypeIDs : [])
            .map((value) => toInt(value, 0))
            .filter((typeID) => typeID > 0 && getReprocessingRigProfile(typeID)))].sort((left, right) => left - right);
}
...
function writeTable(payload) {
    ...
    return Boolean(database.write(REPROCESSING_FACILITY_STATE_TABLE, "/", nextPayload).success);
}
function ensureCache() {
    ...
    const facilities = payload.facilities && typeof payload.facilities === "object"
        ? payload.facilities
        : {};
    for (const [facilityID, record] of Object.entries(facilities)) {
        ...
        facilitiesByID.set(numericFacilityID, buildFacilityRecord(numericFacilityID, record));
    }
    cachedState = {
        payload,
        facilitiesByID,
...
function getReprocessingFacilityConfig(facilityID) {
    ...
    return (ensureCache().facilitiesByID.get(numericFacilityID) ||
...
function getReprocessingFacilityRigTypeIDs(facilityID) {
    ...
    const configuredRigTypeIDs = getReprocessingFacilityConfig(numericFacilityID).rigTypeIDs || [];
...
//# sourceMappingURL=reprocessingFacilityState.js.map