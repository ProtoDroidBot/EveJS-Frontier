...
const { getAccessibleCorpHangarFlags, getSessionCharacterID, getSessionCorporationID, } = require(path.join(__dirname, "../industry/industryAccess"));
const { ITEM_FLAGS, grantItemsToOwnerLocation, listOwnedItems, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const { buildReprocessingQuoteForItem, getReprocessingFacilityRigTypeIDs, getReprocessingProfile, getReprocessingRigProfile, getStructureReprocessingProfile, listReprocessingRigProfilesBySize, reprocessItems, resolveReprocessingContext, setReprocessingFacilityRigTypeIDs, } = require("./index");
...
function resolvePresetRigTypeID(preset, rigSize) {
    ...
    const wantCoverage = normalizedPreset.startsWith("ore") ? "ore"
...
function executeReprocessingSmokeCommand(session, argumentText = "") {
    ...
    if (subcommand === "seed") {
        ...
        const grantResult = grantItemsToOwnerLocation(characterID, locationID, ITEM_FLAGS.HANGAR, seedPlan);
    ...
    if (subcommand === "status") {
        const structureProfile = getStructureReprocessingProfile(contextResult.data.structure && contextResult.data.structure.typeID);
    ...
    if (subcommand === "rig") {
        ...
        const saveResult = setReprocessingFacilityRigTypeIDs(locationID, rigTypeID > 0 ? [rigTypeID] : []);
    ...
    if (subcommand === "quote") {
        ...
        if (quote.errorMsg) {
            return {
                ok: false,
                message: quote.errorMsg === "REPROCESSING_SPLIT_REQUIRED"
...
//# sourceMappingURL=reprocessingChatCommands.js.map