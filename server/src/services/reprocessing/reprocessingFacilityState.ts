...
function normalizeRigTypeIDs(rigTypeIDs: any[] = []) {
  return [...new Set(
...
function buildFacilityRecord(facilityID, record: Record<string, any> = {}) {
  return {
...
function ensureCache() {
  ...
  const facilities =
    payload.facilities && typeof payload.facilities === "object"
      ? payload.facilities
      : {};
  for (const [facilityID, record] of Object.entries<any>(facilities)) {
...
function setReprocessingFacilityRigTypeIDs(facilityID, rigTypeIDs: any[] = []) {
  const numericFacilityID = toInt(facilityID, 0);
  if (numericFacilityID <= 0) {
    return {
