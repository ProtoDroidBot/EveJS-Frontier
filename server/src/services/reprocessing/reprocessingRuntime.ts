...
function getLegacyReprocessingImplantMultiplier(implants: any[] = []) {
  let multiplier = 1;
...
function getReprocessingSkillMultiplierForType(typeID, skillMap, implants: any[] = []) {
...
function buildPreviewRandomizedRecoverables(item, stationsTake, portions, efficiency, includeRecoverables) {
  ...
  const expectedMarketValuePerBatch = Object.entries<any>(
...
function buildReprocessingQuoteForItem(item, context, options: Record<string, any> = {}) {
...
function buildReprocessingOptionsForTypes(typeIDs: any[] = []) {
...
function buildReprocessingQuotesForItems(session, itemReferences: any[] = [], options: Record<string, any> = {}) {
  ...
  for (const itemReference of Array.isArray(itemReferences) ? itemReferences : []) {
    ...
    const quote: Record<string, any> = buildReprocessingQuoteForItem(item, contextResult.data, options);
...
function buildExecutedRecoverables(item, quote) {
  ...
  return pickedMaterials.map(({ material, numOutputPortions }: Record<string, any>) => {
    let quantity = 0;
...
function resolveReprocessingOutputTarget(session, context, sampleItem, options: Record<string, any> = {}) {
...
function reprocessItems(session, options: Record<string, any> = {}) {
  ...
  const processedItemIDs: any[] = [];
  const inputChanges: any[] = [];
  const outputByTypeID = new Map();
  const reprocessedEvents: any[] = [];
  const quotes: any[] = [];
  let totalISKCost = 0;

  for (const item of items) {
    const quote: Record<string, any> = buildReprocessingQuoteForItem(item, contextResult.data, {
...
function throwReprocessingError(errorMsg, values: Record<string, any> = {}) {
