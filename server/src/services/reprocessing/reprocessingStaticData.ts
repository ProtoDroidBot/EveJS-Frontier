...
function freezeFixedMaterial(entry: Record<string, any> = {}) {
...
function freezeRandomizedMaterial(entry: Record<string, any> = {}) {
...
function freezeStructureReprocessingProfile(entry: Record<string, any> = {}) {
  return Object.freeze({
...
function freezeReprocessingRigProfile(entry: Record<string, any> = {}) {
...
function buildAverageRandomizedOutputs(randomizedMaterials: any[] = []) {
...
function buildCacheFromPayload(payload: Record<string, any> = {}) {
  ...
  const reprocessableTypeIDs = new Set<any>();
  const randomizedTypeIDs = new Set<any>();
  ...
  const compressedTypeBySourceTypeID = new Map<any, any>(
    Object.entries<any>(
      ...
      .map(([sourceTypeID, compressedTypeID]): [number, number] => [
  ...
  const sourceTypesByCompressedTypeID = new Map<any, any>(
    Object.entries<any>(
      ...
      .map(([compressedTypeID, sourceTypeIDs]): [number, readonly number[]] => [
        toInt(compressedTypeID, 0),
        Object.freeze(
          [...new Set(
