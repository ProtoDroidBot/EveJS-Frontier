...
const { getCharacterRecord, getActiveShipRecord, } = require(path.join(__dirname, "../character/characterState"));
...
const { ITEM_FLAGS, CLIENT_INVENTORY_STACK_LIMIT, findItemById, getItemMetadata, grantItemsToOwnerLocation, removeInventoryItem, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
const { resolveItemByTypeID, } = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const { getAttributeIDByNames, getTypeAttributeValue, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const { applyDogmaModifierGroups, getActiveImplantCharacterModifierEntries, } = require(path.join(__dirname, "../dogma/implants/activeImplantModifiers"));
...
const { getStructureByID, } = require(path.join(__dirname, "../structure/structureState"));
...
const { getFacilityTaxRate, } = require(path.join(__dirname, "../industry/industryFacilityState"));
const { canTakeFromOwnerLocation, canViewOwnerLocation, getSessionCharacterID, getSessionCorporationID, } = require(path.join(__dirname, "../industry/industryAccess"));
const { getCorporationOfficeByInventoryID, } = require(path.join(__dirname, "../corporation/corporationRuntimeState"));
const { adjustCharacterBalance, buildNotEnoughMoneyUserErrorValues, getCharacterWallet, JOURNAL_ENTRY_TYPE, } = require(path.join(__dirname, "../account/walletState"));
const { adjustCorporationWalletDivisionBalance, } = require(path.join(__dirname, "../corporation/corpWalletState"));
const { ATTRIBUTE_GAS_DECOMPRESSION_EFFICIENCY_BONUS, ATTRIBUTE_REFINING_YIELD_MUTATOR, DEFAULT_IN_SPACE_COMPRESSION_RANGE_METERS, DEFAULT_STATION_REPROCESSING_EFFICIENCY, DEFAULT_STATION_REPROCESSING_TAX, NPC_STANDING_TAX_ZERO_POINT, STRUCTURE_DEFAULT_GAS_DECOMPRESSION_EFFICIENCY, STRUCTURE_DEFAULT_REPROCESSING_EFFICIENCY, TYPE_GAS_DECOMPRESSION_EFFICIENCY, TYPE_REPROCESSING, TYPE_REPROCESSING_EFFICIENCY, TYPE_SCRAPMETAL_PROCESSING, } = require("./reprocessingConstants");
const { getAdjustedAveragePrice, getCompressionSourceTypeIDs, getCompressedTypeID, getReprocessingRigProfile, getReprocessingProfile, getStructureReprocessingProfile, getTypeMaterials, getTypeRandomizedMaterials, hasTypeMaterials, isCompressedType, isCompressibleType, pickRandomizedMaterialTypesByWeight, typeHasRandomizedMaterials, getAverageRandomizedMaterialsPerBatch, } = require("./reprocessingStaticData");
const { getReprocessingFacilityRigTypeIDs, } = require("./reprocessingFacilityState");
const { publishReprocessedNotice, } = require("./reprocessingNotices");
const ATTRIBUTE_REFINING_YIELD_PERCENTAGE_CHAR = getAttributeIDByNames("refiningYieldPercentageChar") || 378;
...
function getInventoryQuantity(item) {
    ...
    return toInt(item.singleton === 1 ? 1 : item.stacksize ?? item.quantity, item.singleton === 1 ? 1 : 0);
}
function getSkillLevel(skillMap, skillTypeID) {
    ...
    return Math.max(0, toInt(record.effectiveSkillLevel ??
...
function getSkillBonusMultiplier(skillMap, skillTypeID) {
    ...
    const percentagePerLevel = toFiniteNumber(getTypeAttributeValue(numericSkillTypeID, ATTRIBUTE_REFINING_YIELD_MUTATOR), 0);
...
function getActiveReprocessingImplantMultiplier(characterID) {
    ...
    const entries = getActiveImplantCharacterModifierEntries(numericCharacterID)
        .filter((entry) => (toInt(entry && entry.modifiedAttributeID, 0) ===
    ...
    return Math.max(0, toFiniteNumber(characterAttributes[ATTRIBUTE_REFINING_YIELD_PERCENTAGE_CHAR], 100) / 100);
}
function getLegacyReprocessingImplantMultiplier(implants = []) {
    ...
    for (const implant of Array.isArray(implants) ? implants : []) {
        ...
        const implantBonus = toFiniteNumber(getTypeAttributeValue(implantTypeID, ATTRIBUTE_REFINING_YIELD_MUTATOR), 0);
        if (implantBonus > 0) {
...
function getReprocessingSkillMultiplierForType(typeID, skillMap, implants = []) {
    if (typeHasSpecialReprocessingSkillBonuses(typeID)) {
        const profile = getReprocessingProfile(typeID);
        let multiplier = getSkillBonusMultiplier(skillMap, TYPE_REPROCESSING) *
...
function getGasDecompressionCharacterEfficiency(skillMap) {
    ...
    const bonusPerLevelPercent = toFiniteNumber(getTypeAttributeValue(TYPE_GAS_DECOMPRESSION_EFFICIENCY, ATTRIBUTE_GAS_DECOMPRESSION_EFFICIENCY_BONUS), 0);
    return Math.max(0, (level * bonusPerLevelPercent) / 100);
}
function getInSpaceCompressionRangeMeters(skillMap = null) {
    const baseRangeMeters = Math.max(1, toFiniteNumber(config.miningInSpaceCompressionRangeMeters, DEFAULT_IN_SPACE_COMPRESSION_RANGE_METERS));
    const rangeBonusPerLevel = Math.max(0, toFiniteNumber(getTypeAttributeValue(62453, "fleetCompressionLogisticsRangeBonus"), 0));
    if (rangeBonusPerLevel <= 0) {
        return Math.round(baseRangeMeters);
    }
    const skillLevel = getSkillLevel(skillMap, 62453);
    return Math.max(1, Math.round(baseRangeMeters * (1 + ((rangeBonusPerLevel * skillLevel) / 100))));
}
function resolveReprocessingContext(session) {
    ...
    const structure = dockedKind === "structure"
        ? getStructureByID(dockedLocationID, { refresh: false })
        : null;
    if (dockedKind === "structure") {
        if (!characterHasStructureService(session, structure, STRUCTURE_SERVICE_ID.REPROCESSING)) {
            return {
                success: false,
    ...
    return {
        ...
        data: {
            ...
            standing: standingRuntime.resolveBestStandingValue(characterID, stationRecord && stationRecord.ownerID).standing,
        },
    };
}
function resolveStructureSecurityBand(context) {
    ...
    const solarSystemID = toInt(context.structure && context.structure.solarSystemID, context.stationRecord && context.stationRecord.solarSystemID);
    const solarSystem = worldData.getSolarSystemByID(solarSystemID);
    const security = toFiniteNumber(solarSystem && solarSystem.security, toFiniteNumber(context.stationRecord && context.stationRecord.security, 0));
    if (security >= 0.45) {
        return "high";
    }
    if (security > 0) {
...
function resolveStructureReprocessingProfile(context) {
    ...
    const structureTypeID = toInt(context.structure && context.structure.typeID, context.stationRecord && context.stationRecord.stationTypeID);
...
function resolveStructureRigProfiles(context) {
    ...
    const rigSize = Math.max(0, toInt(structureProfile && structureProfile.rigSize, 0));
...
function resolveStructureRigEfficiency(context, reprocessingProfile) {
    ...
    for (const rigProfile of resolveStructureRigProfiles(context)) {
        ...
        const securityMultiplier = toFiniteNumber(rigProfile.securityMultipliers && rigProfile.securityMultipliers[securityBand], 1);
    ...
    if (context.dockedKind === "structure") {
        ...
        if (reprocessingProfile) {
            ...
            if (structureProfile && (reprocessingProfile.reprocessingFamily === "ore" ||
                ...
                efficiency *= 1 + (toFiniteNumber(structureProfile.reprocessingYieldBonusPercent, 0) / 100);
            }
        }
        return Math.max(0, Math.min(1, efficiency));
    }
    const profile = getReprocessingProfile(typeID);
    return Math.max(0, Math.min(1, toFiniteNumber(context.stationRecord && context.stationRecord.reprocessingEfficiency, profile ? DEFAULT_STATION_REPROCESSING_EFFICIENCY : DEFAULT_STATION_REPROCESSING_EFFICIENCY)));
}
function getStationTaxRate(context) {
    ...
    if (context.dockedKind === "structure") {
        return Math.max(0, Math.min(1, toFiniteNumber(getFacilityTaxRate(context.dockedLocationID, context.structure
            ? {
    ...
    const baseTax = Math.max(0, Math.min(1, toFiniteNumber(context.stationRecord && context.stationRecord.reprocessingStationsTake, DEFAULT_STATION_REPROCESSING_TAX)));
    const standing = Math.max(0, toFiniteNumber(context.standing, 0));
    if (standing <= 0) {
        return baseTax;
    }
    const reductionScale = Math.max(0, 1 - Math.min(standing, NPC_STANDING_TAX_ZERO_POINT) / NPC_STANDING_TAX_ZERO_POINT);
...
function getReprocessingYieldForType(context, typeID) {
    ...
    const activeImplantMultiplier = getActiveReprocessingImplantMultiplier(context && context.characterID);
    let characterMultiplier = getReprocessingSkillMultiplierForType(typeID, context && context.skillMap, activeImplantMultiplier !== null ? [] : context && context.implants);
...
function getStructureGasDecompressionEfficiency(context) {
    ...
    const structureProfile = resolveStructureReprocessingProfile(context);
    return Math.max(0, Math.min(1, toFiniteNumber(structureProfile && structureProfile.gasDecompressionEfficiencyBase, STRUCTURE_DEFAULT_GAS_DECOMPRESSION_EFFICIENCY) +
        toFiniteNumber(structureProfile && structureProfile.gasDecompressionEfficiencyBonusAdd, 0)));
}
...
function buildFixedRecoverables(item, stationsTake, portions, efficiency) {
    ...
    for (const material of getTypeMaterials(item.typeID)) {
        ...
        if (!isClientSafeRecoverableQuantity(clientQuantity) ||
...
function buildPreviewRandomizedRecoverables(item, stationsTake, portions, efficiency, includeRecoverables) {
    ...
    const expectedMarketValuePerBatch = Object.entries(getAverageRandomizedMaterialsPerBatch(item.typeID)).reduce((sum, [materialTypeID, averageQuantity]) => sum + getAdjustedAveragePrice(materialTypeID) * toFiniteNumber(averageQuantity, 0), 0);
    ...
    for (const material of getTypeRandomizedMaterials(item.typeID)) {
        ...
        if (!isClientSafeRecoverableQuantity(maxClientQuantity) ||
...
function buildReprocessingQuoteForItem(item, context, options = {}) {
    ...
    const includeRecoverablesFromRandomizedOutputs = options.includeRecoverablesFromRandomizedOutputs === true;
    let recoverableResult = buildFixedRecoverables(item, stationsTake, portions, efficiency);
    ...
    if (recoverableResult.recoverables.length <= 0 && typeHasRandomizedMaterials(profile.typeID)) {
        recoverableResult = buildPreviewRandomizedRecoverables(item, stationsTake, portions, efficiency, includeRecoverablesFromRandomizedOutputs);
    }
    return {
...
function resolveReprocessingOutputTarget(session, context, sampleItem, options = {}) {
    const explicitOutputLocationID = options.outputLocationID === null || options.outputLocationID === undefined
        ? null
        : toInt(options.outputLocationID, 0);
    const explicitOutputFlagID = options.outputFlagID === null || options.outputFlagID === undefined
        ? null
        : toInt(options.outputFlagID, ITEM_FLAGS.HANGAR);
    const explicitOwnerID = options.ownerID === null || options.ownerID === undefined
        ? null
...
function reprocessItems(session, options = {}) {
    ...
    const targetResult = resolveReprocessingOutputTarget(session, contextResult.data, items[0], options);
    ...
    if (round2(totalISKCost) > 0) {
        ...
        if (normalizedTotalISKCost - currentBalance > 0.0001) {
            return {
                ...
                errorValues: buildNotEnoughMoneyUserErrorValues(normalizedTotalISKCost, currentBalance),
            };
        }
        const debitResult = adjustCharacterBalance(contextResult.data.characterID, -normalizedTotalISKCost, {
        ...
        if (contextResult.data.dockedKind === "structure") {
            const ownerCorporationID = toInt(contextResult.data.structure &&
                (contextResult.data.structure.ownerCorpID || contextResult.data.structure.ownerID), 0);
            if (ownerCorporationID > 0) {
                adjustCorporationWalletDivisionBalance(ownerCorporationID, 1000, normalizedTotalISKCost, {
    ...
    const grantResult = grantEntries.length > 0
        ? grantItemsToOwnerLocation(targetResult.data.ownerID, targetResult.data.locationID, targetResult.data.flagID, grantEntries)
        : {
            success: true,
            data: {
                changes: [],
...
function throwReprocessingError(errorMsg, values = {}) {
    switch (String(errorMsg || "")) {
        ...
        case "INSUFFICIENT_FUNDS":
            throwWrappedUserError("NotEnoughMoney", buildNotEnoughMoneyUserErrorValues(values.amount, values.balance));
            break;
...
//# sourceMappingURL=reprocessingRuntime.js.map