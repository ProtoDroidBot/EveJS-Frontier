...
const { REPROCESSING_CLIENT_RANDOMIZED_TABLE, REPROCESSING_STATIC_TABLE, } = require("./reprocessingConstants");
...
function freezeReprocessingRigProfile(entry = {}) {
    const securityMultipliers = entry.securityMultipliers && typeof entry.securityMultipliers === "object"
    ...
    return Object.freeze({
        ...
        yieldClasses: Object.freeze([...new Set((Array.isArray(entry.yieldClasses) ? entry.yieldClasses : [])
...
function buildAverageRandomizedOutputs(randomizedMaterials = []) {
    const materials = Array.isArray(randomizedMaterials)
        ? randomizedMaterials.filter((entry) => entry &&
    ...
    const explicitWeightSum = materials.reduce((sum, entry) => sum + Math.max(0, toFloat(entry.weight, 0)), 0);
    const defaultWeight = explicitWeightSum > 0 ? 0 : 1 / materials.length;
    return Object.freeze(materials
        .map((entry) => {
        const probability = explicitWeightSum > 0
            ? Math.max(0, toFloat(entry.weight, 0)) / explicitWeightSum
            : defaultWeight;
        const averageQuantity = ((Math.max(0, toInt(entry.quantityMin, 0)) +
...
function readClientRandomizedProfiles() {
    ...
    return source
        ...
        return {
            ...
            reprocessingFamily: String(entry.reprocessingFamily || "randomized-client-materials"),
            portionSize: Math.max(1, toInt(typeRecord.portionSize || entry.portionSize, 1)),
            basePrice: Math.max(0, toFloat(typeRecord.basePrice || entry.basePrice, 0)),
...
function buildCacheFromPayload(payload = {}) {
    ...
    for (const randomizedProfile of readClientRandomizedProfiles()) {
        ...
        profileByTypeID.set(randomizedProfile.typeID, {
            ...randomizedProfile,
            ...existing,
            randomizedMaterials: Array.isArray(existing.randomizedMaterials) &&
                existing.randomizedMaterials.length > 0
                ? existing.randomizedMaterials
                : randomizedProfile.randomizedMaterials,
            averageRandomizedOutputs: Array.isArray(existing.averageRandomizedOutputs) &&
    ...
    for (const rawProfile of profiles) {
        ...
        const materials = Object.freeze((Array.isArray(rawProfile && rawProfile.materials) ? rawProfile.materials : [])
            .map((entry) => freezeFixedMaterial(entry))
            .filter((entry) => entry.materialTypeID > 0 && entry.quantity > 0));
        const randomizedMaterials = Object.freeze((Array.isArray(rawProfile && rawProfile.randomizedMaterials)
            ? rawProfile.randomizedMaterials
            : [])
            .map((entry) => freezeRandomizedMaterial(entry))
            .filter((entry) => entry.materialTypeID > 0 &&
            entry.quantityMin >= 0 &&
            entry.quantityMax > 0));
        const averageRandomizedOutputs = buildAverageRandomizedOutputs(Array.isArray(rawProfile && rawProfile.averageRandomizedOutputs)
        ...
        const profile = Object.freeze({
            ...
            reprocessingSkillType: Math.max(0, toInt(rawProfile && rawProfile.reprocessingSkillType, 0)),
    ...
    const compressedTypeBySourceTypeID = new Map(Object.entries(payload.compressedTypeBySourceTypeID &&
    ...
    const sourceTypesByCompressedTypeID = new Map(Object.entries(payload.sourceTypesByCompressedTypeID &&
        ...
        toInt(compressedTypeID, 0),
        Object.freeze([...new Set((Array.isArray(sourceTypeIDs) ? sourceTypeIDs : [])
...
function getAverageRandomizedMaterialsPerBatch(typeID) {
    const entries = ensureCache().averageRandomizedOutputsByTypeID.get(toInt(typeID, 0)) ||
        Object.freeze([]);
    return Object.fromEntries(entries.map((entry) => [entry.materialTypeID, entry.quantityAverage]));
}
...
//# sourceMappingURL=reprocessingStaticData.js.map