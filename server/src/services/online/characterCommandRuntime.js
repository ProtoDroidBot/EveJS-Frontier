...
function makeCommandError(code) {
    switch (code) {
        case ERROR_CODES.INVALID:
            return new CharacterCommandError(code, "Character command envelope is invalid.", 400);
        case ERROR_CODES.ID_REUSED:
            return new CharacterCommandError(code, "Character command ID was reused with different command data.", 409);
        case ERROR_CODES.STATE_VERSION_MISMATCH:
            return new CharacterCommandError(code, "Character state version does not match the authoritative state.", 409);
        default:
            return new CharacterCommandError(ERROR_CODES.UNAVAILABLE, "Authoritative character command execution is unavailable.", 503);
    }
}
...
function normalizeOpaqueString(value, maxLength = MAX_OPAQUE_VALUE_LENGTH, minLength = 1) {
...
function serializeHandlerError(error, definition, secrets) {
    if (definition && typeof definition.mapPublicError === "function") {
        try {
            const mapped = serializeMappedDomainError(definition.mapPublicError(error), secrets);
            if (mapped) {
                return mapped;
            }
        }
...
function createCharacterCommandRuntime(options = {}) {
    ...
    for (const [type, definition] of Object.entries(rawDefinitions)) {
        if (!normalizeOpaqueString(type, MAX_COMMAND_TYPE_LENGTH) ||
            !definition ||
        ...
        if (definition.authorizationPolicy === AUTHORIZATION_POLICIES.BROWSER_PILOT &&
    ...
    const completedReceiptLimit = normalizePositiveInteger(options.completedReceiptLimit, DEFAULT_COMPLETED_RECEIPT_LIMIT);
    ...
    const unsubscribeControl = controlRuntime.subscribe((transition) => {
        ...
        const previousState = String(transition && transition.previous && transition.previous.controlState || "");
        const currentState = String(transition && transition.current && transition.current.controlState || "");
    ...
    function authorizeInsideLane(characterID, envelope, authorization) {
        ...
        const credentials = authorization && typeof authorization === "object"
            ? authorization
            : {};
        controlRuntime.validateBrowserControl(characterID, envelope.controllerID, credentials.leaseID, credentials.leaseSecret);
    }
    function executeInsideLane(characterID, envelope, authorization) {
        ...
        return Promise.resolve().then(async () => {
            ...
            try {
                ...
                const cached = cacheReceipt(characterID, envelope.commandID, envelope.fingerprint, outcome);
                if (cached) {
                ...
                const cached = cacheReceipt(characterID, envelope.commandID, envelope.fingerprint, outcome);
                if (cached) {
                    publishCachedReceipt(characterID, envelope, outcome, handlerStarted ? "admitted" : "rejected");
                }
    ...
    function submitCommand(characterIDValue, envelopeValue, submissionOptions = {}) {
        ...
        const promise = enqueue(characterID, () => executeInsideLane(characterID, envelope, submissionOptions.authorization));
        const entry = Object.freeze({
            fingerprint: envelope.fingerprint,
            promise,
        });
        setInFlight(characterID, envelope.commandID, entry);
        promise.then(() => deleteInFlight(characterID, envelope.commandID, entry), () => deleteInFlight(characterID, envelope.commandID, entry));
        return promise;
    }
    function shutdown() {
        if (stopped) {
...
//# sourceMappingURL=characterCommandRuntime.js.map