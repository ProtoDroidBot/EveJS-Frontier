...
function makeSnapshot(characterID, controlState, options: Record<string, any> = {}) {
...
function createCharacterControlRuntime(options: Record<string, any> = {}) {
  ...
  const transitionListeners = new Set<any>();
  ...
  function notifyTransition(previous, current, metadata: Record<string, any> = {}) {
  ...
  function claimBrowserControl(characterIDValue, controllerIDValue, claimOptions: Record<string, any> = {}) {
    const characterID = normalizeCharacterID(characterIDValue);
  ...
  function renewBrowserControl(
    ...
    leaseIDValue,
    leaseSecretValue,
    renewOptions: Record<string, any> = {},
  ) {
  ...
  function recordRetailSessionStarted(characterIDValue, options: Record<string, any> = {}) {
    const characterID = normalizeCharacterID(characterIDValue);
    if (!characterID) {
  ...
  function recordRetailSessionEnded(characterIDValue, options: Record<string, any> = {}) {
    const characterID = normalizeCharacterID(characterIDValue);
    if (!characterID) {
      return offlineSnapshot(0);
    }
...
const defaultRuntime = createCharacterControlRuntime({
  findRetailSession(characterID, options: Record<string, any> = {}) {
