...
const characterControlRuntime = require(path.join(__dirname, "./characterControlRuntime"));
...
function getSessionCharacterID(session) {
    return normalizeCharacterID(session &&
        (session.characterID ||
            session.charID ||
            session.charid));
}
function getPersonalContact(record, contactID) {
    ...
    const contacts = record.personalContacts && typeof record.personalContacts === "object"
...
function isCharacterOnline(targetID, observerID = null) {
    ...
    return characterControlRuntime.getCharacterControlSnapshot(normalizedTargetID).online;
}
function listInitialOnlineStatusRows(observerID) {
    const normalizedObserverID = normalizeCharacterID(observerID);
    const observerRecord = getCharacterRecord(normalizedObserverID) || {};
    const contacts = observerRecord.personalContacts &&
    ...
    for (const [rawContactID, contact] of Object.entries(contacts)) {
        const contactID = normalizeCharacterID(contact && contact.contactID !== undefined ? contact.contactID : rawContactID);
...
function notifyCharacterOnlineState(characterID, online, options = {}) {
    ...
    for (const session of sessionRegistry.getSessions()) {
        ...
        try {
            ...
            // One broken observer must not prevent the transition reaching others.
        }
    }
    return sent;
}
characterControlRuntime.subscribe((transition) => {
    if (transition.previous.online === transition.current.online) {
        return;
    }
    notifyCharacterOnlineState(transition.characterID, transition.current.online, { excludeSession: transition.excludeSession || null });
});
module.exports = {
...
//# sourceMappingURL=onlineStatusRuntime.js.map