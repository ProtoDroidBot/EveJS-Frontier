...
function makeControlError(controlState) {
  ...
  const error: Error & Record<string, any> = new Error(details.message);
  error.name = "CharacterControlError";
  ...
  function visit(entry) {
    ...
    if (entry && typeof entry === "object") {
      return Object.fromEntries(
        Object.entries<any>(entry).map(([key, child]) => [
...
function serializeMappedDomainError(mapped, secrets: any[] = []) {
...
function deserializeError(serialized) {
  const error: Error & Record<string, any> = new Error(serialized.message);
...
function createCharacterCommandRuntime(options: Record<string, any> = {}) {
  ...
  for (const [type, definition] of Object.entries<any>(rawDefinitions)) {
    if (
  ...
  function submitCommand(characterIDValue, envelopeValue, submissionOptions: Record<string, any> = {}) {
    if (stopped) {
