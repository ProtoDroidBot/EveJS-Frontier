...
const { buildRowset, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getSessionCharacterID, isCharacterOnline, listInitialOnlineStatusRows, } = require(path.join(__dirname, "./onlineStatusRuntime"));
...
class OnlineStatusService extends BaseService {
    ...
    Handle_GetOnlineStatus(args, session) {
        ...
        log.debug(`[OnlineStatus] GetOnlineStatus observer=${observerID || "none"} target=${targetID || "none"} -> ${online}`);
        return online;
    }
...
//# sourceMappingURL=onlineStatusService.js.map