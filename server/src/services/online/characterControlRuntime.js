...
function makeSnapshot(characterID, controlState, options = {}) {
    ...
    return Object.freeze({
        ...
        controlState,
        transport: controlState === CONTROL_STATES.RETAIL_CLIENT
...
function makeControlError(code) {
    switch (code) {
        case ERROR_CODES.RETAIL_CLIENT:
            return new CharacterControlError(code, "Character is controlled by a retail client.", 409);
        case ERROR_CODES.BROWSER_PILOT:
            return new CharacterControlError(code, "Character already has an active browser-pilot lease.", 409);
        case ERROR_CODES.LEASE_EXPIRED:
            return new CharacterControlError(code, "Browser-pilot lease has expired.", 409);
        case ERROR_CODES.LEASE_INVALID:
            return new CharacterControlError(code, "Browser-pilot lease credentials are invalid.", 403);
        default:
            return new CharacterControlError(ERROR_CODES.UNAVAILABLE, "Authoritative character control is unavailable.", 503);
    }
}
function createCharacterControlRuntime(options = {}) {
    ...
    function rememberExpiredLease(characterID, lease, expiredAtMs) {
        ...
        expiredLeases.set(characterID, history.slice(-MAX_EXPIRED_LEASES_PER_CHARACTER));
    }
    ...
    function expiredCredentialsMatch(characterID, controllerID, leaseID, leaseSecret) {
        return pruneExpiredHistory(characterID).some((entry) => credentialsMatch(entry, controllerID, leaseID, leaseSecret));
    }
    function validateLeaseCredentials(characterIDValue, controllerIDValue, leaseIDValue, leaseSecretValue) {
        const characterID = normalizeCharacterID(characterIDValue);
        const controllerID = normalizeControllerID(controllerIDValue);
    ...
    function renewBrowserControl(characterIDValue, controllerIDValue, leaseIDValue, leaseSecretValue, renewOptions = {}) {
        const { characterID, lease } = validateLeaseCredentials(characterIDValue, controllerIDValue, leaseIDValue, leaseSecretValue);
    ...
    function validateBrowserControl(characterIDValue, controllerIDValue, leaseIDValue, leaseSecretValue) {
        const { characterID, lease } = validateLeaseCredentials(characterIDValue, controllerIDValue, leaseIDValue, leaseSecretValue);
        return Object.freeze({ control: browserSnapshot(characterID, lease) });
    }
    function releaseBrowserControl(characterIDValue, controllerIDValue, leaseIDValue, leaseSecretValue) {
        const { characterID, lease } = validateLeaseCredentials(characterIDValue, controllerIDValue, leaseIDValue, leaseSecretValue);
        if (lease.timerHandle !== null && lease.timerHandle !== undefined) {
            clearTimer(lease.timerHandle);
        }
...
//# sourceMappingURL=characterControlRuntime.js.map