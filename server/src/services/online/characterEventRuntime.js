...
function createCharacterEventRuntime(options = {}) {
    ...
    if (!controlRuntime ||
        ...
        throw new TypeError("character event runtime requires authoritative control and state versions");
    }
    ...
    const historyLimit = normalizePositiveInteger(options.historyLimit, DEFAULT_EVENT_HISTORY_LIMIT);
    ...
    const commandOutcomeLimit = Math.max(boundedHistoryLimit, Math.min(normalizePositiveInteger(options.commandOutcomeLimit, DEFAULT_COMMAND_OUTCOME_LIMIT), MAX_COMMAND_OUTCOME_LIMIT));
    ...
    function streamHasRetainedCommandID(stream, commandID) {
        if (stream.commandOutcomes.some((outcome) => outcome.commandID === commandID)) {
            return true;
        }
        return stream.history.some((frame) => frame &&
            frame.event &&
    ...
    function hasRetainedCommandID(characterIDValue, commandIDValue) {
        ...
        return Boolean(stream && commandID && streamHasRetainedCommandID(stream, commandID));
    }
    function publishCommandSettlement(settlementValue) {
        if (stopped) {
            return null;
        }
        const characterID = normalizeCharacterID(settlementValue && settlementValue.characterID);
        if (!characterID) {
            return null;
        }
        try {
            ...
            if (stream.commandOutcomes.length > commandOutcomeLimit) {
                stream.commandOutcomes.splice(0, stream.commandOutcomes.length - commandOutcomeLimit);
            }
    ...
    function buildSnapshotFrame(characterID, stream) {
        ...
        const control = sanitizeControlSnapshot(controlRuntime.getCharacterControlSnapshot(characterID));
    ...
    function subscribe(characterIDValue, cursorValue, handlers = {}) {
        ...
        if (canReplay(stream, cursor)) {
            initialFrames = stream.history.filter((frame) => frame.cursor.sequence > cursor.sequence);
        }
        else {
...
//# sourceMappingURL=characterEventRuntime.js.map