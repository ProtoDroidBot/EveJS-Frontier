...
function isCharacterOnline(targetID, observerID = null) {
  ...
  return characterControlRuntime.getCharacterControlSnapshot(
    normalizedTargetID,
  ).online;
}
...
function listInitialOnlineStatusRows(observerID) {
  ...
  for (const [rawContactID, contact] of Object.entries<any>(contacts)) {
...
function notifyCharacterOnlineState(characterID, online, options: Record<string, any> = {}) {
...
characterControlRuntime.subscribe((transition) => {
  ...
  notifyCharacterOnlineState(
    ...
    transition.current.online,
    { excludeSession: transition.excludeSession || null },
  );
});
