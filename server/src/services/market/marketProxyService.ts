...
function getNumericSessionValue(session, keys: any[] = [], fallback = 0) {
...
function buildSummaryDict(summaryRows: any[] = [], valueKind = "tuple") {
  const entries: any[] = [];
...
function buildOrderRowset(rows: any[] = [], options: Record<string, any> = {}) {
...
function buildHistoryRowset(historyRows: any[] = []) {
...
function splitHistoryRows(historyRows: any[] = []) {
...
function buildHistoryPair(historyRows: any[] = []) {
...
function buildOwnerOrdersRowset(ownerOrders: any[] = []) {
  const lines: any[] = [];
...
function buildMarketTransactionEntry(entry: Record<string, any> = {}) {
  const fields = [
...
function buildMarketTransactionList(entries: any[] = []) {
  return buildList(
    ensureArray(entries).map((entry) => buildMarketTransactionEntry(entry)),
  );
}

function filterMarketTransactionsFromDate(entries: any[] = [], fromDate = null) {
...
function unwrapMarshalValue(rawValue, depth = 0) {
  ...
  if (rawValue && typeof rawValue === "object") {
    ...
    return Object.fromEntries(
      Object.entries<any>(rawValue).map(([key, value]) => [
        key,
        unwrapMarshalValue(value, depth + 1),
      ]),
    );
...
function buildOrderNotificationEntry(orderLike, overrides: Record<string, any> = {}) {
...
function notifyOwnOrdersChanged(characterID, orders: any[] = [], reason = ORDER_REASON_PARTIAL, isCorp = false) {
...
function transformVisibleBuyerChanges(changes: any[] = [], ownerID) {
...
function recordCharacterMarketTransaction(characterID, entry: Record<string, any> = {}) {
...
function upsertBuyEscrowRecord(orderLike, overrides: Record<string, any> = {}) {
...
function upsertSellEscrowRecord(record: Record<string, any> = {}) {
  return putEscrowRecord({
    ...record,
    escrowType: "sell",
  });
...
function ensureOpenOrderLimit(context, openOrders: any[] = []) {
...
async function requestExpirySweepIfDue(force = false, daemonCallOptions: Record<string, any> = {}) {
  const now = Date.now();
...
async function cancelStructureMarketOrdersForServiceLoss(structure, options: Record<string, any> = {}) {
...
function syncOpenBuyEscrowRecords(ownerOrders: any[] = []) {
...
function updateMovedItemsOwner(changes: any[] = [], ownerID) {
...
function notifyBuyerDelivery(characterID, changes: any[] = []) {
...
async function matchBuyOrdersForSeller({
  ...
  for (const buyOrder of filterMarketRowsByAccessibleStructureMarket(
    ...
    const sellerInventoryChanges: any[] = [];
    let destinationChanges: any[] = [];
