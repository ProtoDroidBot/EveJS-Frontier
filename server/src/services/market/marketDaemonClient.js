...
class MarketDaemonClient {
    constructor(options = {}) {
        ...
        this.port = normalizePositiveInteger(options.port || config.marketDaemonPort, 40111);
        this.connectTimeoutMs = normalizePositiveInteger(options.connectTimeoutMs || config.marketDaemonConnectTimeoutMs, 1500);
        this.requestTimeoutMs = normalizePositiveInteger(options.requestTimeoutMs || config.marketDaemonRequestTimeoutMs, 15000);
        this.retryDelayMs = normalizePositiveInteger(options.retryDelayMs || config.marketDaemonRetryDelayMs, 2000);
        this._socket = null;
        this._buffer = "";
    ...
    async ensureConnected(options = {}) {
        ...
        this._connectingPromise = new Promise((resolve, reject) => {
            ...
            const handleConnect = () => {
                ...
                log.info(`[MarketDaemonClient] Connected to market daemon RPC at ${this.host}:${this.port}`);
                resolve();
            };
            ...
            const handleTimeout = () => {
                finishReject(new Error(`market daemon RPC connect timeout after ${this.connectTimeoutMs} ms`));
            };
    ...
    _attachSocket(socket) {
        ...
        socket.on("close", () => {
            ...
            this._connected = false;
            this._buffer = "";
            this._failPendingRequests(new Error("market daemon RPC connection closed"));
            if (wasConnected) {
    ...
    _handleResponseLine(line) {
        ...
        try {
            ...
            log.warn(`[MarketDaemonClient] Failed to parse RPC response line: ${error.message}`);
            return;
        }
        const requestId = String(response && Object.prototype.hasOwnProperty.call(response, "id")
            ? response.id
            : "");
        ...
        if (response.ok === false) {
            pendingRequest.reject(new Error(response.error || "market daemon RPC request failed"));
            return;
        }
        pendingRequest.resolve(Object.prototype.hasOwnProperty.call(response, "result")
            ? response.result
            : null);
    }
    _sendRequest(method, params) {
        ...
        return new Promise((resolve, reject) => {
            const timeoutHandle = setTimeout(() => {
                this._pendingRequests.delete(requestId);
                reject(new Error(`market daemon RPC request timed out after ${this.requestTimeoutMs} ms`));
    ...
    _logConnectFailure(error, options = {}) {
        ...
        log.warn(`[MarketDaemonClient] Unable to reach market daemon RPC at ${this.host}:${this.port}: ${error.message}`);
    }
}
...
//# sourceMappingURL=marketDaemonClient.js.map