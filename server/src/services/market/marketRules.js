...
const { RANGE_STATION, RANGE_SOLAR_SYSTEM, RANGE_REGION, getOrderJumpDistance, getStationSolarSystemID, } = require(path.join(__dirname, "./marketTopology"));
...
function getMarketContext({ characterID, stationID, session } = {}) {
    ...
    const warFactionID = normalizePositiveInteger(session && (session.warfactionid || session.warFactionID), 0) ||
        normalizePositiveInteger(characterRecord.warFactionID, 0) ||
        normalizePositiveInteger(characterRecord.factionID, 0);
    transactionTaxRate *=
        1 -
            getLocationDiscount(warFactionID, station && station.solarSystemID);
    return {
...
function getStationDistanceFromSession(session, stationID) {
    const currentStationID = normalizePositiveInteger(session && (session.stationid || session.stationID || session.locationid), 0);
    const currentSolarSystemID = normalizePositiveInteger(session &&
        (session.solarsystemid2 || session.solarsystemid || session.solarSystemID), 0);
...
//# sourceMappingURL=marketRules.js.map