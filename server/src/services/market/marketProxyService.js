...
const { throwWrappedUserError, isMachoWrappedException } = require(path.join(__dirname, "../../common/machoErrors"));
const { buildDict, buildKeyVal, buildList, buildRowset, buildFiletimeLong, normalizeBigInt, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { marketDaemonClient } = require(path.join(__dirname, "./marketDaemonClient"));
const { buildCachedMethodCallResult } = require(path.join(__dirname, "../cache/objectCacheRuntime"));
const { ACCOUNT_KEY, JOURNAL_ENTRY_TYPE, adjustCharacterBalance, appendCharacterMarketTransaction, getCharacterMarketTransactions, getCharacterWallet, } = require(path.join(__dirname, "../account/walletState"));
const { adjustCorporationWalletDivisionBalance, CORPORATION_WALLET_KEY_START, getCorporationMarketTransactions, } = require(path.join(__dirname, "../corporation/corpWalletState"));
const { ITEM_FLAGS, findItemById, grantItemToCharacterLocation, listContainerItems, moveItemToLocation, removeInventoryItem, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const { getEscrowRecord, listEscrowRecords, putEscrowRecord, removeEscrowRecord, } = require(path.join(__dirname, "./marketEscrowState"));
const { RANGE_STATION, RANGE_REGION, getOrderJumpDistance, getStationConstellationID, getStationSolarSystemID, isBidOrderInRange, isSellOrderInRange, } = require(path.join(__dirname, "./marketTopology"));
const { MARKET_MAX_ORDER_PRICE, computeBrokerFeeInfo, computeSccSurchargeInfo, computeSalesTaxAmount, getMarketContext, getStationDistanceFromSession, isAllowedBuyRange, isAllowedDuration, } = require(path.join(__dirname, "./marketRules"));
const { getMarketRuntimeState, updateMarketRuntimeState, } = require(path.join(__dirname, "./marketRuntimeState"));
const { getStationRecord } = require(path.join(__dirname, "../_shared/stationStaticData"));
const structureState = require(path.join(__dirname, "../structure/structureState"));
const { STRUCTURE_SERVICE_ID, } = require(path.join(__dirname, "../structure/structureConstants"));
const { STRUCTURE_SETTING_ID, } = require(path.join(__dirname, "../structure/structureServiceAuthority"));
const { getProfileSettingValueForStructure, } = require(path.join(__dirname, "../structure/structureProfilesState"));
const { characterHasStructureService, } = require(path.join(__dirname, "../structure/structurePayloads"));
...
function mapOwnerOrderRow(ownerOrder) {
    ...
    const lastStateChangeDate = isoTimestampToFileTimeBigInt(ownerOrder && ownerOrder.last_state_change_at);
...
function buildMarketTransactionList(entries = []) {
    return buildList(ensureArray(entries).map((entry) => buildMarketTransactionEntry(entry)));
}
function filterMarketTransactionsFromDate(entries = [], fromDate = null) {
    ...
    return ensureArray(entries).filter((entry) => normalizeBigInt(entry && entry.transactionDate, 0n) >= threshold);
}
function collectRequestedTypeIds(rawValue, out, depth = 0) {
    ...
    if (rawValue && typeof rawValue === "object") {
        if ((rawValue.type === "objectex1" || rawValue.type === "objectex2")) {
        ...
        if ((rawValue.type === "list" || rawValue.type === "set") &&
...
function extractRequestedTypeIds(rawValue) {
    ...
    return Array.from(new Set(requestedTypeIds.filter((typeID) => Number.isInteger(typeID) && Number.isFinite(typeID) && typeID > 0)));
}
...
function isMarshalKeyValObjectName(rawName, depth = 0) {
    ...
    return (normalizedName === "util.KeyVal" ||
...
function marshalObjectToPlainObject(rawValue) {
    ...
    if (rawValue &&
        ...
        return Object.fromEntries(rawValue.header[2].entries.map(([key, value]) => [
    ...
    if (unwrapped && typeof unwrapped === "object" && !Array.isArray(unwrapped)) {
        if (Array.isArray(unwrapped.header) &&
...
function getCharacterSessions(characterID) {
    return sessionRegistry
        .getSessions()
        .filter((session) => normalizePositiveInteger(session && session.characterID, 0) ===
        normalizePositiveInteger(characterID, 0));
}
function buildOrderNotificationEntry(orderLike, overrides = {}) {
    const issueDate = isoTimestampToFileTimeBigInt(overrides.issued_at ??
        overrides.issuedAt ??
        (orderLike && orderLike.issued_at) ??
        (orderLike && orderLike.issuedAt));
    const orderID = overrides.order_id ??
        overrides.orderId ??
    ...
    return buildKeyVal([
        ...
        ["charID", normalizePositiveInteger(overrides.char_id ??
                overrides.charID ??
                orderLike.owner_id ??
                orderLike.ownerId ??
                0, 0)],
...
function notifyOwnOrdersChanged(characterID, orders = [], reason = ORDER_REASON_PARTIAL, isCorp = false) {
    const payload = buildList(ensureArray(orders).map((order) => buildOrderNotificationEntry(order)));
...
function buildImmediateMarketRefreshOrder({ characterID, stationID, regionID, solarSystemID, constellationID, typeID, price, quantity, bid = false, } = {}) {
...
function getMarketCounterpartyOwnerID(stationID) {
    ...
    if (structure) {
        return normalizePositiveInteger(structure.ownerCorpID || structure.ownerID, 0);
    }
    const stationRecord = getStationRecord(null, stationID);
    return normalizePositiveInteger(stationRecord && (stationRecord.corporationID || stationRecord.ownerID), 0);
}
...
function buildCharacterMarketContext(session, characterID, stationID) {
    ...
    const configuredPercent = getProfileSettingValueForStructure(structure, STRUCTURE_SETTING_ID.MARKET_TAX, { session });
...
function creditStructureMarketBrokerFeeOwner(characterID, stationID, amount, description) {
    const normalizedAmount = roundIsk(amount);
    if (!(normalizedAmount > 0)) {
        return null;
    }
    const structure = getStructureMarketLocation(stationID);
    const ownerCorpID = normalizePositiveInteger(structure && (structure.ownerCorpID || structure.ownerID), 0);
    if (!ownerCorpID) {
        return null;
    }
    const creditResult = adjustCorporationWalletDivisionBalance(ownerCorpID, CORPORATION_WALLET_KEY_START, normalizedAmount, {
        description,
    ...
    if (!creditResult || creditResult.success !== true) {
        log.warn(`[MarketProxy] Failed to credit structure broker fee ownerCorp=${ownerCorpID} structure=${stationID}: ${creditResult && creditResult.errorMsg ? creditResult.errorMsg : "UNKNOWN"}`);
    }
    return creditResult;
}
...
function upsertBuyEscrowRecord(orderLike, overrides = {}) {
    ...
    const ownerId = normalizePositiveInteger(overrides.owner_id ?? orderLike.owner_id ?? orderLike.ownerId, 0);
    const stationId = normalizePositiveInteger(overrides.station_id ?? orderRow.station_id ?? orderRow.stationID, 0);
    const typeId = normalizePositiveInteger(overrides.type_id ?? orderRow.type_id ?? orderRow.typeID, 0);
    const price = roundIsk(overrides.price ?? orderRow.price);
    const remainingQuantity = normalizePositiveInteger(overrides.vol_remaining ?? orderRow.vol_remaining ?? orderRow.volRemaining, 0);
...
function ensureValidBuyRange(rangeValue, limits) {
    ...
    if (normalizedRange === RANGE_REGION &&
...
function ensureOpenOrderLimit(context, openOrders = []) {
    ...
    const maxOrderCount = normalizePositiveInteger(context && context.limits && context.limits.cnt, 0);
...
function validateExpectedBrokerFeePercentage(expectedBrokerFee, marketContext) {
    if (expectedBrokerFee === null ||
        expectedBrokerFee === undefined ||
        expectedBrokerFee === "") {
        return;
    }
    const actualBrokerFee = Number(marketContext && marketContext.brokerCommissionRate);
...
function debitCharacterWallet(characterID, amount, description, ownerID2 = 0, options = {}) {
...
function creditCharacterWallet(characterID, amount, description, ownerID2 = 0, options = {}) {
    const normalizedAmount = roundIsk(Math.abs(amount));
    if (!(normalizedAmount > 0)) {
        return null;
    }
...
function applySignedCharacterWalletDelta(characterID, amount, description, ownerID2 = 0, options = {}) {
    const normalizedAmount = roundIsk(amount);
    if (Math.abs(normalizedAmount) <= 0) {
        return null;
    }
    if (normalizedAmount > 0) {
        return creditCharacterWallet(characterID, normalizedAmount, description, ownerID2, options);
    }
    return debitCharacterWallet(characterID, Math.abs(normalizedAmount), description, ownerID2, options);
}
...
function ensureStructureMarketServiceAccess(session, stationID) {
    ...
    if (!characterHasStructureService(session, structure, STRUCTURE_SERVICE_ID.MARKET)) {
        throwWrappedUserError("StructureMarketDenied", {
            structureName: structure.itemName ||
                structure.name ||
...
function filterMarketRowsByAccessibleStructureMarket(session, rows = [], stationKeys = ["station_id", "stationID"]) {
    return ensureArray(rows).filter((row) => {
        const stationID = normalizePositiveInteger(stationKeys
            .map((key) => row && row[key])
            .find((value) => normalizePositiveInteger(value, 0) > 0), 0);
...
function listEscrowItemsForOrder(orderId, ownerId) {
    ...
    return listContainerItems(normalizePositiveInteger(ownerId, 0), normalizePositiveInteger(escrowRecord.escrowLocationID, 0), ITEM_FLAGS.HANGAR);
}
...
function isMarketDaemonUnavailableError(error) {
    const message = String(error && error.message ? error.message : error || "")
        .toLowerCase();
    return (message.includes("market daemon rpc is disabled") ||
...
async function applyExpiredOrderEvent(event) {
    ...
    if (normalizeBoolean(orderRow && orderRow.bid)) {
        ...
        if (escrowRecord && getEscrowType(escrowRecord) === "buy") {
            ...
            creditCharacterWallet(ownerID, roundIsk(escrowRecord.escrowAmount), `Market escrow refund for expired buy order ${orderID}`, stationID, {
        ...
        const escrowRecord = getEscrowRecord(orderID);
        if (escrowRecord && getEscrowType(escrowRecord) === "sell") {
            moveEscrowItemsBackToSeller(orderID, ownerID, normalizePositiveInteger(orderRow && orderRow.station_id, 0));
            removeEscrowRecord(orderID);
        }
    }
...
function applyCancelledStructureMarketOrder(order) {
    ...
    if (normalizeBoolean(orderRow && orderRow.bid)) {
        ...
        creditCharacterWallet(ownerID, refundAmount, `Market escrow refund for cancelled buy order ${orderID}`, stationID, {
            entryTypeID: JOURNAL_ENTRY_TYPE.MARKET_ESCROW,
            ownerID1: getMarketCounterpartyOwnerID(stationID) || stationID,
            ownerID2: ownerID,
            referenceID: stationID,
...
async function cancelStructureMarketOrdersForServiceLoss(structure, options = {}) {
    ...
    return {
        success: true,
        cancelledCount: normalizePositiveInteger(response && response.cancelled_count, orders.length),
        appliedCount,
        orders,
    };
}
async function processPendingExpiryEvents({ forceSweep = false, daemonCallOptions = {}, } = {}) {
...
function consumeInventoryItemQuantity(itemID, quantity) {
    ...
    const availableQuantity = normalizeInteger(currentItem.singleton, 0) === 1
        ? 1
...
function moveEscrowItemsBackToSeller(orderId, ownerId, stationId) {
    ...
    for (const escrowItem of escrowItems) {
        const returnResult = moveItemToLocation(escrowItem.itemID, stationId, ITEM_FLAGS.HANGAR);
...
function deliverSeedItemToCharacter(characterID, stationID, typeID, quantity) {
    ...
    const grantOptions = Number(typeRecord && typeRecord.groupID) === 963
        ...
        : {};
    const grantResult = grantItemToCharacterLocation(characterID, stationID, ITEM_FLAGS.HANGAR, { typeID: normalizePositiveInteger(typeID, 0) }, normalizePositiveInteger(quantity, 0), grantOptions);
...
async function matchSellOrdersForBuyer({ session, buyerCharacterID, regionID, typeID, maxPrice, requestedQuantity, orderRange, currentStationID, currentSolarSystemID, chargeBuyerWallet = true, } = {}) {
    ...
    let totalSpent = 0;
    let totalBought = 0;
    for (const sellOrder of filterMarketRowsByAccessibleStructureMarket(session, book && book.sells)) {
        ...
        const availableQuantity = normalizePositiveInteger(sellOrder && sellOrder.vol_remaining, 0);
        if (availableQuantity <= 0) {
            continue;
        }
        if (!isSellOrderInRange(sellOrder, currentStationID, currentSolarSystemID, orderRange)) {
            continue;
        }
        const tradedQuantity = Math.min(remainingQuantity, availableQuantity);
        const destinationStationID = normalizePositiveInteger(sellOrder && sellOrder.station_id, currentStationID);
        const grossCost = roundIsk(orderPrice * tradedQuantity);
        const isSeedSellOrder = String(sellOrder && sellOrder.source || "").toLowerCase() === "seed";
        const stationMarketOwnerID = getMarketCounterpartyOwnerID(destinationStationID);
        const marketCounterpartyOwnerID = isSeedSellOrder ? stationMarketOwnerID : 0;
        let purchaseCounterpartyID = marketCounterpartyOwnerID || destinationStationID;
        let buyerWalletResult = null;
        try {
            if (isSeedSellOrder) {
                if (chargeBuyerWallet) {
                    buyerWalletResult = debitCharacterWallet(buyerCharacterID, grossCost, `Market purchase of ${typeLabel}`, destinationStationID, {
                ...
                deliverSeedItemToCharacter(buyerCharacterID, destinationStationID, typeID, tradedQuantity);
            }
            else {
                const playerSellOrder = await fetchOrderById(sellOrder && sellOrder.order_id);
                const sellerCharacterID = normalizePositiveInteger(playerSellOrder && playerSellOrder.owner_id, 0);
                ...
                const escrowItems = listEscrowItemsForOrder(sellOrder && sellOrder.order_id, sellerCharacterID);
                ...
                for (const escrowItem of escrowItems) {
                    ...
                    const itemQuantity = normalizeInteger(escrowItem && escrowItem.singleton, 0) === 1
                        ? 1
                        : normalizePositiveInteger(escrowItem && escrowItem.stacksize, 0);
                    const moveQuantity = Math.min(escrowRemaining, itemQuantity);
                    const moveResult = moveItemToLocation(escrowItem.itemID, destinationStationID, ITEM_FLAGS.HANGAR, moveQuantity);
                    if (!moveResult.success) {
                        throwWrappedUserError("CustomInfo", {
                            info: `Failed to move escrowed market item ${escrowItem.itemID} to the buyer.`,
                        });
                    }
                    const destinationChanges = ensureArray(moveResult.data && moveResult.data.changes).filter((change) => change &&
                        change.item &&
                ...
                const sellerMarketContext = buildCharacterMarketContext(null, sellerCharacterID, destinationStationID);
                const salesTax = computeSalesTaxAmount(sellerMarketContext, grossAmount);
                const sellerWalletResult = creditCharacterWallet(sellerCharacterID, grossAmount, `Market sale proceeds for ${typeLabel}`, buyerCharacterID, {
                    entryTypeID: JOURNAL_ENTRY_TYPE.MARKET_TRANSACTION,
                    ownerID1: buyerCharacterID,
                ...
                if (salesTax > 0) {
                    debitCharacterWallet(sellerCharacterID, salesTax, `Transaction tax for sale of ${typeLabel}`, destinationStationID, {
                ...
                recordCharacterMarketTransaction(buyerCharacterID, {
                    ...
                    journalRefID: buyerWalletResult && buyerWalletResult.journalEntry
                ...
                recordCharacterMarketTransaction(sellerCharacterID, {
                    ...
                    journalRefID: sellerWalletResult && sellerWalletResult.journalEntry
                        ? sellerWalletResult.journalEntry.transactionID
                        : -1,
                });
                notifyOwnOrdersChanged(sellerCharacterID, [{
                        ...playerSellOrder,
                        row: {
            ...
            if (buyerWalletResult) {
                creditCharacterWallet(buyerCharacterID, grossCost, `Market purchase refund for ${typeLabel}`, destinationStationID, {
                    entryTypeID: JOURNAL_ENTRY_TYPE.MARKET_TRANSACTION,
                    ownerID1: buyerCharacterID,
                    ownerID2: purchaseCounterpartyID,
                    referenceID: destinationStationID,
...
async function matchBuyOrdersForSeller({ session, sellerCharacterID, regionID, stationID, solarSystemID, typeID, minimumPrice, requestedQuantity, sourceItemID, sourceOrderId, } = {}) {
    const book = await marketDaemonClient.call("GetOrders", {
        region_id: regionID,
        type_id: typeID,
    });
    ...
    let totalSold = 0;
    let totalGross = 0;
    const sellerMarketContext = buildCharacterMarketContext(null, sellerCharacterID, stationID);
    const typeLabel = getMarketTypeLabel(typeID);
    for (const buyOrder of filterMarketRowsByAccessibleStructureMarket(session, book && book.buys)) {
        if (remainingQuantity <= 0) {
            break;
        }
        ...
        const availableQuantity = normalizePositiveInteger(buyOrder && buyOrder.vol_remaining, 0);
        if (availableQuantity <= 0) {
            continue;
        }
        ...
        const meetsMinVolume = fillableQuantity >= minVolume ||
        ...
        if (!isSeedBuyOrder) {
            ...
            buyerCharacterID = normalizePositiveInteger(playerBuyOrder && playerBuyOrder.owner_id, 0);
            if (!buyerCharacterID) {
                continue;
            }
        }
        const destinationStationID = normalizePositiveInteger(buyOrder && buyOrder.station_id, stationID);
        ...
        if (normalizePositiveInteger(sourceOrderId, 0) > 0) {
            ...
            for (const escrowItem of escrowItems) {
                ...
                const transferResult = isSeedBuyOrder
                    ...
                    : moveItemToLocation(escrowItem.itemID, destinationStationID, ITEM_FLAGS.HANGAR, moveQuantity);
                ...
                if (!isSeedBuyOrder) {
                    destinationChanges.push(...changeSet.filter((change) => change &&
                        change.item &&
                        normalizePositiveInteger(change.item.typeID, 0) === typeID &&
                        normalizePositiveInteger(change.item.locationID, 0) === destinationStationID));
                }
            ...
            const transferResult = isSeedBuyOrder
                ...
                : moveItemToLocation(sourceItemID, destinationStationID, ITEM_FLAGS.HANGAR, fillableQuantity);
            if (!transferResult.success) {
                throwWrappedUserError("CustomInfo", {
                    info: isSeedBuyOrder
            ...
            sellerInventoryChanges.push(...changeSet);
            if (!isSeedBuyOrder) {
                destinationChanges = changeSet.filter((change) => change &&
                    change.item &&
                    normalizePositiveInteger(change.item.typeID, 0) === typeID &&
                    normalizePositiveInteger(change.item.locationID, 0) === destinationStationID);
            }
        ...
        if (sellerInventoryChanges.length > 0) {
            notifyInventoryChangesToCharacter(sellerCharacterID, sellerInventoryChanges);
        }
        if (!isSeedBuyOrder) {
            updateMovedItemsOwner(destinationChanges, buyerCharacterID);
        ...
        const stationMarketOwnerID = getMarketCounterpartyOwnerID(destinationStationID);
        const salesTax = computeSalesTaxAmount(sellerMarketContext, grossAmount);
        const sellerWalletResult = creditCharacterWallet(sellerCharacterID, grossAmount, `Market sale proceeds for ${typeLabel}`, isSeedBuyOrder ? destinationStationID : buyerCharacterID, {
            entryTypeID: JOURNAL_ENTRY_TYPE.MARKET_TRANSACTION,
        ...
        if (!isSeedBuyOrder) {
            ...
            notifyOwnOrdersChanged(buyerCharacterID, [{
                    ...playerBuyOrder,
                    row: {
...
async function executeBuyRequest({ session, characterID, stationID, regionID, typeID, maxPrice, requestedQuantity, orderRange, minVolume, durationDays, expectedBrokerFee, } = {}) {
    ...
    const normalizedMinVolume = Math.max(1, normalizePositiveInteger(minVolume, 1));
    const typeLabel = getMarketTypeLabel(typeID);
    ...
    const sccSurchargeUpperBound = normalizedDuration > 0
        ...
        : 0;
    ensureCharacterHasFunds(characterID, reserveUpperBound + brokerFeeUpperBound + sccSurchargeUpperBound, `Placing a buy order for ${typeLabel}`);
    ...
    if (normalizedDuration > 0 && matchResult.remainingQuantity > 0) {
        ...
        const brokerFeeInfo = computeBrokerFeeInfo(marketContext, null, remainingReserve);
        const sccSurchargeInfo = computeSccSurchargeInfo(marketContext, null, remainingReserve);
        const stationMarketOwnerID = getMarketCounterpartyOwnerID(stationID);
        debitCharacterWallet(characterID, remainingReserve, `Market escrow for buy order ${typeLabel}`, stationID, {
            entryTypeID: JOURNAL_ENTRY_TYPE.MARKET_ESCROW,
            ownerID1: stationMarketOwnerID || stationID,
            ownerID2: characterID,
            referenceID: stationID,
        });
        debitCharacterWallet(characterID, brokerFeeInfo.amount, `Broker fee for buy order ${typeLabel}`, stationID, {
            entryTypeID: JOURNAL_ENTRY_TYPE.BROKERS_FEE,
            ownerID1: stationMarketOwnerID || stationID,
            ownerID2: characterID,
            referenceID: stationID,
        });
        debitCharacterWallet(characterID, sccSurchargeInfo.amount, `SCC surcharge for buy order ${typeLabel}`, stationID, {
        ...
        try {
            ...
            creditStructureMarketBrokerFeeOwner(characterID, stationID, brokerFeeInfo.amount, `Structure broker fee for buy order ${typeLabel}`);
            notifyOwnOrdersChanged(characterID, [createdOrder], ORDER_REASON_CREATED);
        }
        catch (error) {
            creditCharacterWallet(characterID, remainingReserve, `Market escrow refund for failed buy order ${typeLabel}`, stationID, {
                entryTypeID: JOURNAL_ENTRY_TYPE.MARKET_ESCROW,
                ownerID1: stationMarketOwnerID || stationID,
                ownerID2: characterID,
                referenceID: stationID,
            });
            creditCharacterWallet(characterID, brokerFeeInfo.amount, `Broker fee refund for failed buy order ${typeLabel}`, stationID, {
                entryTypeID: JOURNAL_ENTRY_TYPE.BROKERS_FEE,
                ownerID1: stationMarketOwnerID || stationID,
                ownerID2: characterID,
                referenceID: stationID,
            });
            creditCharacterWallet(characterID, sccSurchargeInfo.amount, `SCC surcharge refund for failed buy order ${typeLabel}`, stationID, {
                entryTypeID: JOURNAL_ENTRY_TYPE.MARKET_PROVIDER_TAX,
                ownerID1: stationMarketOwnerID || stationID,
                ownerID2: characterID,
                referenceID: stationID,
    ...
    if (matchResult.boughtQuantity > 0 && !createdOrder) {
        notifyOwnOrdersChanged(characterID, [buildImmediateMarketRefreshOrder({
                characterID,
                stationID,
                regionID,
...
async function executeSellEntry({ session, entry, durationDays, expectedBrokerFee, } = {}) {
    const characterID = getNumericSessionValue(session, ["charid", "characterID"]);
    ...
    if (normalizePositiveInteger(item.locationID, 0) !== stationID ||
    ...
    if (normalizedDuration > 0 && sellResult.remainingQuantity > 0) {
        ...
        const brokerFeeInfo = computeBrokerFeeInfo(marketContext, null, openOrderValue);
        const sccSurchargeInfo = computeSccSurchargeInfo(marketContext, null, openOrderValue);
        const stationMarketOwnerID = getMarketCounterpartyOwnerID(stationID);
        debitCharacterWallet(characterID, brokerFeeInfo.amount, `Broker fee for sell order ${typeLabel}`, stationID, {
            entryTypeID: JOURNAL_ENTRY_TYPE.BROKERS_FEE,
            ownerID1: stationMarketOwnerID || stationID,
            ownerID2: characterID,
            referenceID: stationID,
        });
        debitCharacterWallet(characterID, sccSurchargeInfo.amount, `SCC surcharge for sell order ${typeLabel}`, stationID, {
            entryTypeID: JOURNAL_ENTRY_TYPE.MARKET_PROVIDER_TAX,
            ownerID1: stationMarketOwnerID || stationID,
            ownerID2: characterID,
            referenceID: stationID,
        ...
        const escrowLocationID = MARKET_ESCROW_LOCATION_BASE + normalizePositiveInteger(placedOrder.order_id, 0);
        try {
            const moveResult = moveItemToLocation(itemID, escrowLocationID, ITEM_FLAGS.HANGAR, sellResult.remainingQuantity);
            if (!moveResult.success) {
                throwWrappedUserError("CustomInfo", {
                    info: `Failed to escrow inventory item ${itemID} for market order ${normalizeOrderId(placedOrder.order_id)}.`,
                });
            }
            notifyInventoryChangesToCharacter(characterID, ensureArray(moveResult.data && moveResult.data.changes));
            ...
            creditStructureMarketBrokerFeeOwner(characterID, stationID, brokerFeeInfo.amount, `Structure broker fee for sell order ${typeLabel}`);
            createdOrder = {
                order_id: placedOrder.order_id,
                owner_id: characterID,
                is_corp: false,
            ...
            const strandedEscrowItems = listContainerItems(characterID, escrowLocationID, ITEM_FLAGS.HANGAR);
            for (const strandedItem of strandedEscrowItems) {
                moveItemToLocation(strandedItem.itemID, stationID, ITEM_FLAGS.HANGAR);
            }
            creditCharacterWallet(characterID, brokerFeeInfo.amount, `Broker fee refund for failed sell order ${typeLabel}`, stationID, {
                entryTypeID: JOURNAL_ENTRY_TYPE.BROKERS_FEE,
                ownerID1: stationMarketOwnerID || stationID,
                ownerID2: characterID,
                referenceID: stationID,
            });
            creditCharacterWallet(characterID, sccSurchargeInfo.amount, `SCC surcharge refund for failed sell order ${typeLabel}`, stationID, {
                entryTypeID: JOURNAL_ENTRY_TYPE.MARKET_PROVIDER_TAX,
                ownerID1: stationMarketOwnerID || stationID,
                ownerID2: characterID,
                referenceID: stationID,
...
async function applyBuyOrderCrossingAfterModify(session, order, newPrice) {
    ...
    const reservedForMatchedVolume = computeOrderValue(roundIsk(newPrice), matchResult.boughtQuantity);
    const priceImprovementRefund = roundIsk(reservedForMatchedVolume - roundIsk(matchResult.totalSpent));
    if (priceImprovementRefund > 0) {
        creditCharacterWallet(ownerID, priceImprovementRefund, `Market price improvement refund for ${typeLabel} order ${normalizeOrderId(order && order.order_id)}`, stationID, {
            entryTypeID: JOURNAL_ENTRY_TYPE.MARKET_ESCROW,
            ownerID1: getMarketCounterpartyOwnerID(stationID) || stationID,
            ownerID2: ownerID,
            referenceID: stationID,
...
class MarketProxyService extends BaseService {
    ...
    async Handle_GetStationAsks(args, session) {
        ...
        try {
            ...
            return buildCachedMethodCallResult(buildSummaryDict(filterMarketRowsByAccessibleStructureMarket(session, result, ["best_ask_station_id", "bestAskStationID"]), "tuple"), {
    ...
    async Handle_GetRegionBest(args, session) {
        ...
        try {
            ...
            return buildSummaryDict(filterMarketRowsByAccessibleStructureMarket(session, result, ["best_ask_station_id", "bestAskStationID"]), "bestByOrder");
        }
        catch (error) {
    ...
    async Handle_GetOrders(args, session) {
        ...
        if (!typeID || !regionID) {
            return buildCachedMethodCallResult([EMPTY_ORDER_ROWSET, EMPTY_ORDER_ROWSET], {
                method: "GetOrders",
                args: [typeID],
        ...
        try {
            ...
            const sells = filterMarketRowsByAccessibleStructureMarket(session, result && result.sells);
            const buys = filterMarketRowsByAccessibleStructureMarket(session, result && result.buys);
    ...
    async Handle_GetHistoryForManyTypeIDs(args) {
        ...
        try {
            ...
            try {
                ...
                historyResponses = await Promise.all(typeIDs.map((typeID) => marketDaemonClient.call("GetHistory", {
                    type_id: typeID,
                })));
            }
    ...
    async Handle_GetCharOrders(args, session) {
        ...
        try {
            ...
            const openOrders = (Array.isArray(result) ? result : []).filter((order) => String(order && order.state ? order.state : "open").toLowerCase() === "open");
            syncOpenBuyEscrowRecords(openOrders);
            return buildCachedMethodCallResult(buildOwnerOrdersRowset(openOrders), {
    ...
    async Handle_GetMarketOrderHistory(args, session) {
        ...
        log.debug(`[MarketProxy] GetMarketOrderHistory char=${characterID} corp=${corporationID}`);
        ...
        try {
            ...
            const historyOrders = [...(Array.isArray(charOrders) ? charOrders : []), ...(Array.isArray(corpOrders) ? corpOrders : [])]
                .filter((order) => String(order && order.state ? order.state : "open").toLowerCase() !== "open");
            return buildCachedMethodCallResult(buildOwnerOrdersRowset(historyOrders), {
    ...
    Handle_CharGetTransactions(args, session) {
        ...
        log.debug(`[MarketProxy] CharGetTransactions char=${characterID} fromDate=${normalizeBigInt(fromDate, 0n).toString()}`);
        if (!characterID) {
            return buildList([]);
        }
        return buildMarketTransactionList(filterMarketTransactionsFromDate(getCharacterMarketTransactions(characterID), fromDate));
    }
    Handle_CorpGetTransactions(args, session) {
        ...
        const accountKey = args && args.length > 1 && args[1] !== null
            ...
            : null;
        log.debug(`[MarketProxy] CorpGetTransactions corp=${corporationID} accountKey=${accountKey === null ? "any" : accountKey} fromDate=${normalizeBigInt(fromDate, 0n).toString()}`);
        const result = corporationID
            ? buildMarketTransactionList(filterMarketTransactionsFromDate(getCorporationMarketTransactions(corporationID, { accountKey }), fromDate))
            : buildList([]);
    ...
    async Handle_GetCharEscrow(args, session) {
        ...
        try {
            ...
            const escrowRecords = listEscrowRecords()
                .filter((record) => normalizePositiveInteger(record && record.ownerId, 0) === characterID &&
                !normalizeBoolean(record && record.isCorp));
            const iskEscrow = escrowRecords
                .filter((record) => getEscrowType(record) === "buy")
                .reduce((total, record) => total + roundIsk(record && record.escrowAmount), 0);
            const itemsEscrow = escrowRecords
                .filter((record) => getEscrowType(record) === "sell")
                .reduce((total, record) => total + normalizePositiveInteger(record && record.remainingQuantity, 0), 0);
            return buildKeyVal([
    ...
    async Handle_GetPlexBest(args, session) {
        ...
        const regionID = getNumericSessionValue(session, ["regionid", "regionID"]);
        if (!regionID) {
            return buildCachedMethodCallResult(buildDict([]), {
                method: "GetPlexBest",
                sessionInfo: "regionid",
                sessionInfoValue: regionID,
            });
        ...
        try {
            ...
            const plexRows = (Array.isArray(result) ? result : []).filter((row) => Number(row && row.type_id) === PLEX_TYPE_ID);
            return buildCachedMethodCallResult(buildSummaryDict(filterMarketRowsByAccessibleStructureMarket(session, plexRows, ["best_ask_station_id", "bestAskStationID"]), "bestByOrder"), {
                method: "GetPlexBest",
                sessionInfo: "regionid",
                sessionInfoValue: regionID,
            });
        ...
        try {
            ...
            if (normalizeBoolean(orderRow && orderRow.bid) && cancelResponse && cancelResponse.invalidated) {
                ...
                const refundAmount = escrowRecord && getEscrowType(escrowRecord) === "buy"
                    ? roundIsk(escrowRecord.escrowAmount)
                    : computeOrderValue(orderRow && orderRow.price, orderRow && orderRow.vol_remaining);
                creditCharacterWallet(normalizePositiveInteger(order && order.owner_id, 0), refundAmount, `Market escrow refund for cancelled buy order ${orderID}`, stationID, {
                    entryTypeID: JOURNAL_ENTRY_TYPE.MARKET_ESCROW,
                    ownerID1: getMarketCounterpartyOwnerID(stationID) || stationID,
            ...
            if (!normalizeBoolean(orderRow && orderRow.bid)) {
                moveEscrowItemsBackToSeller(orderID, normalizePositiveInteger(order && order.owner_id, 0), normalizePositiveInteger(orderRow && orderRow.station_id, 0));
                removeEscrowRecord(orderID);
            }
            notifyOwnOrdersChanged(normalizePositiveInteger(order && order.owner_id, 0), [{
                    ...order,
    ...
    async Handle_PlaceBuyOrder(args, session) {
        ...
        const characterID = getNumericSessionValue(session, ["charid", "characterID"]);
        const regionID = getNumericSessionValue(session, ["regionid", "regionID"]);
        log.debug(`[MarketProxy] PlaceBuyOrder char=${characterID} station=${stationID} type=${typeID} quantity=${quantity} duration=${durationDays}`);
    ...
    async Handle_BuyMultipleItems(args, session) {
        ...
        const characterID = getNumericSessionValue(session, ["charid", "characterID"]);
        const regionID = getNumericSessionValue(session, ["regionid", "regionID"]);
        log.debug(`[MarketProxy] BuyMultipleItems char=${characterID} station=${stationID} count=${itemList.length}`);
        assertPersonalMarketOnly(useCorp);
    ...
    async Handle_PlaceMultiSellOrder(args, session) {
        ...
        const characterID = getNumericSessionValue(session, ["charid", "characterID"]);
        log.debug(`[MarketProxy] PlaceMultiSellOrder char=${characterID} count=${itemList.length} duration=${durationDays}`);
        assertPersonalMarketOnly(useCorp);
    ...
    async Handle_PlacePlexSellOrder(args, session) {
        ...
        const durationDays = normalizeInteger(args && args[2], 0);
        const expectedBrokerFee = args && args.length > 3 ? args[3] : null;
        log.debug(`[MarketProxy] PlacePlexSellOrder item=${normalizePositiveInteger(entry && entry.itemID, 0)} duration=${durationDays}`);
        assertPersonalMarketOnly(useCorp);
    ...
    async Handle_ModifyCharOrder(args, session) {
        ...
        try {
            ...
            const brokerFeeInfo = computeBrokerFeeInfo(marketContext, oldOrderValue, newOrderValue);
            const sccSurchargeInfo = computeSccSurchargeInfo(marketContext, oldOrderValue, newOrderValue);
            const reserveDelta = normalizeBoolean(orderRow && orderRow.bid)
                ? roundIsk(newOrderValue - oldOrderValue)
                : 0;
            const stationMarketOwnerID = getMarketCounterpartyOwnerID(stationID);
            applySignedCharacterWalletDelta(ownerID, -reserveDelta, reserveDelta > 0
                ? `Market escrow increase for modified ${typeLabel} order ${orderID}`
                : `Market escrow refund for modified ${typeLabel} order ${orderID}`, stationID, {
                entryTypeID: JOURNAL_ENTRY_TYPE.MARKET_ESCROW,
                ownerID1: stationMarketOwnerID || stationID,
                ownerID2: ownerID,
                referenceID: stationID,
            });
            applySignedCharacterWalletDelta(ownerID, -brokerFeeInfo.amount, brokerFeeInfo.amount > 0
                ? `Broker fee for modified ${typeLabel} order ${orderID}`
                : `Broker fee refund for modified ${typeLabel} order ${orderID}`, stationID, {
                entryTypeID: JOURNAL_ENTRY_TYPE.BROKERS_FEE,
                ownerID1: stationMarketOwnerID || stationID,
                ownerID2: ownerID,
                referenceID: stationID,
            });
            applySignedCharacterWalletDelta(ownerID, -sccSurchargeInfo.amount, sccSurchargeInfo.amount > 0
                ? `SCC surcharge for modified ${typeLabel} order ${orderID}`
                : `SCC surcharge refund for modified ${typeLabel} order ${orderID}`, stationID, {
                entryTypeID: JOURNAL_ENTRY_TYPE.MARKET_PROVIDER_TAX,
                ownerID1: stationMarketOwnerID || stationID,
                ownerID2: ownerID,
                referenceID: stationID,
            ...
            try {
                ...
                finalRemainingVolume = normalizePositiveInteger(modifyResponse && modifyResponse.vol_remaining, remainingVolume);
                if (normalizeBoolean(orderRow && orderRow.bid)) {
                    ...
                    finalRemainingVolume = normalizePositiveInteger(fillResponse && fillResponse.vol_remaining, finalRemainingVolume);
                }
                else {
                ...
                applySignedCharacterWalletDelta(ownerID, reserveDelta, reserveDelta > 0
                    ...
                    : `Market escrow reversal for failed modified ${typeLabel} order ${orderID}`, stationID, {
                    entryTypeID: JOURNAL_ENTRY_TYPE.MARKET_ESCROW,
                    ownerID1: stationMarketOwnerID || stationID,
                    ownerID2: ownerID,
                    referenceID: stationID,
                ...
                applySignedCharacterWalletDelta(ownerID, brokerFeeInfo.amount, brokerFeeInfo.amount > 0
                    ...
                    : `Broker fee reversal for failed modified ${typeLabel} order ${orderID}`, stationID, {
                    entryTypeID: JOURNAL_ENTRY_TYPE.BROKERS_FEE,
                    ownerID1: stationMarketOwnerID || stationID,
                    ownerID2: ownerID,
                    referenceID: stationID,
                ...
                applySignedCharacterWalletDelta(ownerID, sccSurchargeInfo.amount, sccSurchargeInfo.amount > 0
                    ...
                    : `SCC surcharge reversal for failed modified ${typeLabel} order ${orderID}`, stationID, {
                    entryTypeID: JOURNAL_ENTRY_TYPE.MARKET_PROVIDER_TAX,
                    ownerID1: stationMarketOwnerID || stationID,
                    ownerID2: ownerID,
                    referenceID: stationID,
            ...
            creditStructureMarketBrokerFeeOwner(ownerID, stationID, brokerFeeInfo.amount, `Structure broker fee for modified ${typeLabel} order ${orderID}`);
            if (normalizeBoolean(orderRow && orderRow.bid)) {
...
//# sourceMappingURL=marketProxyService.js.map