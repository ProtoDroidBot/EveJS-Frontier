...
function ensureAdjacency() {
    ...
    for (const solarSystem of worldData.getSolarSystems()) {
        const sourceSystemID = normalizePositiveInteger(solarSystem && solarSystem.solarSystemID);
        if (!sourceSystemID) {
            continue;
        }
        for (const stargate of worldData.getStargatesForSystem(sourceSystemID)) {
            const destinationSystemID = normalizePositiveInteger(stargate && stargate.destinationSolarSystemID);
...
function getStationSolarSystemID(stationID) {
    return normalizePositiveInteger(getStation(stationID) && getStation(stationID).solarSystemID, 0);
}
function getStationConstellationID(stationID) {
    return normalizePositiveInteger(getStation(stationID) && getStation(stationID).constellationID, 0);
}
function getStationRegionID(stationID) {
    return normalizePositiveInteger(getStation(stationID) && getStation(stationID).regionID, 0);
}
function getSolarSystemConstellationID(solarSystemID) {
    return normalizePositiveInteger(getSolarSystem(solarSystemID) && getSolarSystem(solarSystemID).constellationID, 0);
}
function getSolarSystemRegionID(solarSystemID) {
    return normalizePositiveInteger(getSolarSystem(solarSystemID) && getSolarSystem(solarSystemID).regionID, 0);
}
...
function getOrderJumpDistance({ currentStationID = 0, currentSolarSystemID = 0, orderStationID = 0, orderSolarSystemID = 0, } = {}) {
...
function isBidOrderInRange(orderRow, sellerStationID, sellerSolarSystemID) {
    ...
    const bidStationID = normalizePositiveInteger(orderRow && (orderRow.station_id ?? orderRow.stationID), 0);
    const bidSolarSystemID = normalizePositiveInteger(orderRow && (orderRow.solar_system_id ?? orderRow.solarSystemID), 0);
    if (orderRange === RANGE_REGION) {
        const bidRegionID = normalizePositiveInteger(orderRow && (orderRow.region_id ?? orderRow.regionID), getSolarSystemRegionID(bidSolarSystemID));
...
function isSellOrderInRange(orderRow, buyerStationID, buyerSolarSystemID, buyRange) {
    ...
    const askStationID = normalizePositiveInteger(orderRow && (orderRow.station_id ?? orderRow.stationID), 0);
    const askSolarSystemID = normalizePositiveInteger(orderRow && (orderRow.solar_system_id ?? orderRow.solarSystemID), getStationSolarSystemID(askStationID));
    const normalizedBuyerStationID = normalizePositiveInteger(buyerStationID, 0);
    const normalizedBuyerSolarSystemID = normalizePositiveInteger(buyerSolarSystemID, getStationSolarSystemID(normalizedBuyerStationID));
    if (orderRange === RANGE_REGION) {
        const askRegionID = normalizePositiveInteger(orderRow && (orderRow.region_id ?? orderRow.regionID), getStationRegionID(askStationID));
...
//# sourceMappingURL=marketTopology.js.map