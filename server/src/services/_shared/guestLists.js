...
const { getSessionStationID, getSessionStructureID, } = require(path.join(__dirname, "../structure/structureLocation"));
...
function buildGuestIdentityTuple(session) {
    return [
        normalizePositiveInt(session && (session.characterID || session.charid), 0),
        ...
        normalizePositiveInt(session && (session.warFactionID || session.warfactionid), 0),
    ];
}
...
function seedObserverGuestLedger(observerSession, locationID, characterIDs = []) {
    ...
    observerGuestLedger.set(observerSession, {
        ...
        guests: new Set((Array.isArray(characterIDs) ? characterIDs : [])
...
function getStationGuestTuples(stationID) {
    ...
    return collectPreferredSessionsByCharacter(sessionRegistry
        .getSessions()
...
function broadcastStructureGuestJoined(session, structureID) {
    ...
    for (const guestSession of sessionRegistry.getSessions()) {
        ...
        ledger.guests.add(guestCharacterID);
        guestSession.sendNotification("OnCharacterEnteredStructure", "clientID", guestTuple);
    }
}
...
//# sourceMappingURL=guestLists.js.map