...
const { applyDogmaModifierGroups, getActiveImplantShipModifierEntries, } = require(path.join(__dirname, "../dogma/implants/activeImplantModifiers"));
...
function buildEmergencyWarpDirection(liveSpaceState, options = {}) {
    const seed = toInt(options.characterID, 0) * 31 +
...
function buildEmergencyWarpLogoffState(liveSpaceState, options = {}) {
    ...
    const emergencyPosition = addVectors(origin, scaleVector(emergencyDirection, EMERGENCY_WARP_DISTANCE_METERS));
    return {
        ...liveSpaceState,
        systemID,
...
function shipSuppressesEmergencyWarp(characterOrID) {
    ...
    applyDogmaModifierGroups(attributes, getActiveImplantShipModifierEntries(characterID));
...
//# sourceMappingURL=emergencyWarpRuntime.js.map