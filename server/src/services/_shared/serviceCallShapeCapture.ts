...
function mapEntries(entries: any[] = []) {
  return new Map<any, any>(Array.isArray(entries) ? entries : []);
}
...
function summarizeDbRowDescriptor(value) {
  ...
  return {
    kind: "dbrow-descriptor" as const,
    name: descriptorName,
    columns,
...
function summarizeObjectEx(value, depth) {
  ...
  return {
    kind: "marshal-objectex" as const,
...
function summarizeDict(value, depth) {
  ...
  const keyKinds = [...new Set<any>(entries.map(([key]) => summarizeValue(key, depth + 1).kind))]
    .sort();
...
function summarizeValue(value, depth = 0) {
  if (depth > MAX_DEPTH) {
    return { kind: "max-depth" as const };
  }
  if (value === null) {
  ...
  if (typeof value === "bigint") {
    return { kind: "bigint" as const };
  }
  if (typeof value === "boolean") {
    return { kind: "boolean" as const };
  }
  if (Buffer.isBuffer(value)) {
    return { kind: "buffer" as const, byteLength: value.length };
  }
  if (Array.isArray(value)) {
