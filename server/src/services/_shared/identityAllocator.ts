...
function readTableRoot(tableName, fallback: Record<string, any> = {}) {
...
function collectAccountIDConflicts(accountID) {
  ...
  for (const record of Object.values<any>(accounts)) {
  ...
  for (const record of Object.values<any>(characters)) {
...
function collectCharacterIDConflicts(characterID) {
  ...
  for (const record of Object.values<any>(mail.messages || {})) {
  ...
  for (const [ownerID, ownerRecord] of Object.entries<any>(savedFittings.owners || {})) {
    if (
  ...
  for (const folder of Object.values<any>(bookmarkFolders.records || {})) {
  ...
  for (const subfolder of Object.values<any>(bookmarkSubfolders.records || {})) {
  ...
  for (const bookmark of Object.values<any>(bookmarks.records || {})) {
  ...
  for (const group of Object.values<any>(bookmarkGroups.records || {})) {
  ...
  for (const event of Object.values<any>(calendarEvents.events || {})) {
  ...
  for (const response of Object.values<any>(calendarResponses.responses || {})) {
...
function getItemReferenceHighWaterMark() {
  ...
  for (const record of Object.values<any>(items)) {
    maxValue = maxFromNumericValues(
      maxValue,
...
function reserveItemIDs(count = 1, options: Record<string, any> = {}) {
