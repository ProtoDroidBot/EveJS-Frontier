...
function localName(value, fallback = "") {
  ...
  if (value && typeof value === "object") {
    ...
    const first = Object.values<any>(value).find((candidate) => typeof candidate === "string");
    if (first) {
      return first;
    }
  }
...
// The current SDE models the Warp Core Stabilizer benefit as a one-shot active
// effect, but the legacy client used by this shard only exposes the module as
// passive. Keep the override name-scoped: the effect GUID is shared by modules
// such as Assault Damage Controls that must remain activatable.
const EFFECT_CATEGORY_OVERRIDES_BY_NAME = new Map([
  ["warpcorestabilizeractive", 4],
]);
...
function normalizeDogmaEffectType(row) {
  ...
  return {
    ...
    description: localName(row.description),
    guid: row.guid || "",
    effectCategoryID: resolveEffectCategoryID(row),
...
function normalizeTypeDogmaRecord(rawRecord, fallbackTypeID) {
  ...
  if (rawRecord.attributes && typeof rawRecord.attributes === "object") {
    for (const [attributeID, value] of Object.entries<any>(rawRecord.attributes)) {
...
function normalizeTypeDogmaPayload(payload) {
  const normalizedAttributeTypes: Record<string, any> = {};
  for (const [attributeID, row] of Object.entries<any>(payload.attributeTypesByID || {})) {
  ...
  const normalizedEffectTypes: Record<string, any> = {};
  for (const [effectID, row] of Object.entries<any>(payload.effectTypesByID || {})) {
  ...
  let totalEffects = 0;
  for (const [typeID, row] of Object.entries<any>(payload.typesByTypeID || {})) {
...
function readStaticRows(tableName) {
  ...
  if (rows && typeof rows === "object") {
    return Object.entries<any>(rows).map(([key, value]) => (
