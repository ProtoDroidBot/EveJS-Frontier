...
function buildDbRowset(columns = [], rows = [], name = "eve.common.script.sys.rowset.Rowset", virtualColumns = []) {
    ...
    if (name === "carbon.common.script.sys.crowset.CRowset") {
        return {
            ...
            list: normalizedRows.map((row) => buildPackedRowFromRowsetLine(columns, descriptor, row)),
            dict: [],
        };
    }
    return {
...
function findRowsetColumnIndex(columns = [], keyColumn = null) {
    ...
    return (Array.isArray(columns) ? columns : []).findIndex(([columnName]) => columnName === keyColumn);
}
...
function buildDbIndexRowset(columns = [], keyedRows = [], keyColumn = null, name = "carbon.common.script.sys.crowset.CIndexedRowset", virtualColumns = []) {
    const descriptor = buildPackedRowDescriptor(columns, virtualColumns);
    ...
    return buildIndexRowset(columns.map(([columnName]) => columnName), normalizedRows, keyColumn, name);
}
function buildIndexRowset(header = [], keyedRows = [], keyColumn = null, name = "eve.common.script.sys.rowset.IndexRowset") {
    return {
        type: "object",
        name,
        args: buildDict([
            ...
            [
                "items",
                buildDict(keyedRows.map(([key, line]) => [key, buildList(Array.isArray(line) ? line : [])])),
            ],
        ]),
    };
}
function buildPagedResultSet(collection = [], totalCount = 0, page = 0, perPage = 50) {
...
function extractList(value) {
    ...
    if (value &&
        ...
        const headerName = value.header[0] && typeof value.header[0] === "object"
            ? normalizeText(value.header[0].value, "")
            : "";
        const headerArgs = Array.isArray(value.header[1]) ? value.header[1] : [];
        if ((headerName === "__builtin__.set" || headerName === "builtins.set") &&
            headerArgs.length > 0) {
...
function mapMarshalEntriesToObject(entries = [], depth = 0) {
    return Object.fromEntries((Array.isArray(entries) ? entries : []).map(([entryKey, entryValue]) => [
...
function unwrapMarshalValue(value, depth = 0) {
    ...
    if (value instanceof Map) {
        return Object.fromEntries([...value.entries()].map(([entryKey, entryValue]) => [
            unwrapMarshalValue(entryKey, depth + 1),
            unwrapMarshalValue(entryValue, depth + 1),
        ]));
    }
    ...
    switch (value.type) {
        ...
        case "objectex1":
        case "objectex2": {
            const headerName = Array.isArray(value.header) && value.header.length > 0
                ? normalizeText(value.header[0], "")
                : "";
            const headerArgs = Array.isArray(value.header) && value.header.length > 1 && Array.isArray(value.header[1])
                ? value.header[1]
                : [];
...
function buildBoundObjectResponse(service, args, session, kwargs) {
    ...
    const serviceName = service && typeof service.name === "string" && service.name.trim() !== ""
        ? service.name
        : null;
    const existingObjectID = session &&
        session._boundObjectIDs &&
        serviceName &&
        ...
        : null;
    const existingObjectState = boundObjectState && serviceName && boundObjectState[serviceName]
        ? boundObjectState[serviceName]
        : null;
    const shouldReuseBoundObject = Boolean(service &&
        service.reuseBoundObjectForSession === true &&
        existingObjectID);
    const previousBoundAtFileTime = existingObjectState && typeof existingObjectState.boundAtFileTime === "bigint"
        ? existingObjectState.boundAtFileTime
        : null;
    const freshBoundAtFileTime = nextBoundObjectReferenceFileTime(previousBoundAtFileTime);
    ...
    if (nestedCall && Array.isArray(nestedCall) && nestedCall.length >= 1) {
        ...
        try {
            callResult = service.callMethod(methodName, Array.isArray(callArgs) ? callArgs : [callArgs], session, callKwargs);
        }
        finally {
            if (session) {
