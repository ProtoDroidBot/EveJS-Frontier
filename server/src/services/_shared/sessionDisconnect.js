...
function resolveDockedStateFromShip(activeShip, fallbackSystemID = null) {
    ...
    if (station) {
        const solarSystemID = Number((station && station.solarSystemID) || fallbackSystemID || 0) || null;
        if (!solarSystemID) {
            return null;
        }
        return {
    ...
    if (structure) {
        const solarSystemID = Number((structure && structure.solarSystemID) || fallbackSystemID || 0) ||
            null;
        if (!solarSystemID) {
            return null;
        }
...
function persistCharacterLogoffState(session) {
    ...
    const sessionStructureID = hasLocationID(session.structureID || session.structureid)
        ...
        : null;
    const sessionSpaceSystemID = hasLocationID(session._space && session._space.systemID)
    ...
    const dockedSolarSystemID = sessionStationID
        ? Number((worldData.getStationByID(sessionStationID) || {}).solarSystemID || sessionSolarSystemID || 0) || null
        : sessionStructureID
            ? Number((structureState.getStructureByID(sessionStructureID, { refresh: false }) || {}).solarSystemID || sessionSolarSystemID || 0) || null
            : null;
    const nextSolarSystemID = (sessionStationID || sessionStructureID)
        ? dockedSolarSystemID
    ...
    if (!persistedStationID && !persistedStructureID && activeShip) {
        ...
        if (liveSpaceState) {
            ...
            if (!moveResult.success) {
                ...
                persistedSolarSystemID = Number(persistedSpaceState.systemID || persistedSolarSystemID);
            }
        }
        else {
            const dockedFallback = resolveDockedStateFromShip(activeShip, persistedSolarSystemID);
            if (dockedFallback) {
                ...
                log.debug(persistedStructureID
                ...
                log.debug(`[SessionDisconnect] Missing live space state for char=${characterID} ship=${activeShip.itemID}; leaving persisted ship state unchanged to avoid inventing coordinates.`);
            }
        }
    }
    ...
    if (!updateResult.success) {
        log.warn(`[SessionDisconnect] Failed to persist logoff state for char=${characterID}: ${updateResult.errorMsg}`);
        return updateResult;
    }
    return updateResult;
}
function disconnectCharacterSession(session, options = {}) {
    ...
    const wasRetailControlSession = Boolean(characterID > 0 &&
    ...
    const attemptCleanup = (label, callback) => {
        try {
            ...
            log.warn(`[SessionDisconnect] ${label} failed for char=${characterID}: ${error.message}`);
            return null;
        }
    };
    try {
        attemptCleanup("safe-logoff timer cleanup", () => {
            const { cancelSafeLogoff } = require(path.join(__dirname, "../ship/safeLogoffRuntime"));
        ...
        attemptCleanup("login milestone checkpoint", () => {
            const MilestoneMgrService = require(path.join(__dirname, "../character/milestoneMgrService"));
...
//# sourceMappingURL=sessionDisconnect.js.map