...
const SERVER_DATA_ROOT = path.resolve(process.cwd(), "server/src/gameStore/data");
...
function resolveOptions() {
    if (overrideOptions) {
        return {
            ...
            filePath: overrideOptions.filePath === null
                ? null
                : normalizeText(overrideOptions.filePath, DEFAULT_CAPTURE_PATH),
            memory: overrideOptions.memory !== false,
            memoryLimit: Math.max(0, Number.isFinite(Number(overrideOptions.memoryLimit))
...
function summarizeRowsetObject(value, depth) {
    ...
    const lineCount = lines && lines.type === "list" && Array.isArray(lines.items)
        ? lines.items.length
        : 0;
    return {
        kind: "rowset",
...
function buildEntry({ service, method, handlerName = null, args = [], kwargs = null, session = null, result = undefined, error = null, elapsedMs = 0, unhandled = false, } = {}) {
    return {
...
function buildNotificationEntry({ lane = "broadcast", notifyType = null, idType = null, service = null, method = null, objectID = null, payload = [], kwargs = null, session = null, innerBytes = null, innerMarshalMs = null, } = {}) {
    return {
...
function buildPacketEntry({ direction = "unknown", rawDecoded = null, packet = null, context = null, session = null, encodedBytes = null, encrypted = null, } = {}) {
    ...
    const dest = packet && packet.dest
        ...
        : rawTuple && rawTuple.length > 2
            ...
            : null;
    const packetType = firstFiniteNumber(packet && packet.type, rawTuple && rawTuple[0]);
    const packetTypeName = normalizeText(packet && packet.typeName, "") ||
        (packetType !== null ? getTypeName(packetType) : null);
    const service = safeServiceName(context && context.service !== undefined
        ? context.service
        : packet && packet.service
            ? packet.service
            : dest && dest.service);
    const target = summarizeTextIdentifier(context && context.service !== undefined
        ? context.service
        : packet && packet.service
            ? packet.service
            : dest && dest.service);
    const callID = firstFiniteNumber(context && context.callID, source && source.callID, dest && dest.callID);
    return {
        captureKind: "packet",
        timestamp: new Date().toISOString(),
        ...
        sessionShape: summarizeSession(session),
        payloadShape: summarizeValue(packet && packet.payload !== undefined
            ? packet.payload
            : rawTuple && rawTuple.length > 4
                ? rawTuple[4]
                : null, 0),
        namedPayloadShape: summarizeValue(packet && packet.namedPayload !== undefined
            ? packet.namedPayload
            : rawTuple && rawTuple.length > 5
                ? rawTuple[5]
                : null, 0),
        oobShape: summarizeValue(packet && packet.oob !== undefined
            ? packet.oob
...
//# sourceMappingURL=serviceCallShapeCapture.js.map