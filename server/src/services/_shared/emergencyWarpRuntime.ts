...
function cloneVector(value, fallback: { x: number; y: number; z: number } = ZERO_VECTOR) {
...
function normalizeVector(value, fallback: { x: number; y: number; z: number } = DEFAULT_DIRECTION) {
...
function buildEmergencyWarpDirection(liveSpaceState, options: Record<string, any> = {}) {
  const seed =
...
function buildEmergencyWarpLogoffState(liveSpaceState, options: Record<string, any> = {}) {
