...
function getActiveJumpTimers(characterID, options: Record<string, any> = {}) {
...
function buildClientJumpTimerTuple(characterID, options: Record<string, any> = {}) {
...
function hasActiveJumpActivation(characterID, options: Record<string, any> = {}) {
  const timers = getActiveJumpTimers(characterID, options);
  return Boolean(timers.jumpActivation);
}

function calculateJumpTimers(options: Record<string, any> = {}) {
  const nowMs = Math.max(0, Math.trunc(toFiniteNumber(options.nowMs, Date.now())));
...
function applyJumpTimersForCharacter(characterID, options: Record<string, any> = {}) {
  const normalizedCharacterID = normalizePositiveInt(characterID, 0);
  if (normalizedCharacterID <= 0) {
    return {
...
function applyJumpTimersForSession(session, options: Record<string, any> = {}) {
...
function applyJumpTimersBestEffort(session, options: Record<string, any> = {}) {
