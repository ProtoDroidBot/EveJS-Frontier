...
function buildDefaultDeps() {
    return {
        ...
        publishLocalMembershipListForSession: (session) => chatRuntime.publishLocalMembershipListForSession(session),
...
function reconcileOnce(depsOverride = {}) {
    ...
    const sessions = (deps.getSessions() || []).filter((session) => resolveCharacterID(session) > 0);
    let chatSnapshots = 0;
    let xmppReasserts = 0;
    if (deps.chatEnabled) {
        for (const session of sessions) {
            try {
                ...
                log.debug(`[PresenceReconcile] chat snapshot failed char=${resolveCharacterID(session)}: ${error.message}`);
            }
        }
    }
    if (deps.xmppEnabled) {
        ...
        if (typeof reassert === "function") {
            for (const session of sessions) {
                try {
                    ...
                    log.debug(`[PresenceReconcile] xmpp reassert failed char=${resolveCharacterID(session)}: ${error.message}`);
                }
            }
        }
    }
...
function start(options = {}) {
    ...
    const intervalMs = Math.max(0, Number(options.intervalMs !== undefined
        ? options.intervalMs
        : config.presenceReconcileIntervalMs) || 0);
    if (intervalMs <= 0) {
    ...
    log.info(`[PresenceReconcile] started interval=${intervalMs}ms ` +
...
//# sourceMappingURL=presenceReconciler.js.map