...
function resolveEffectCategoryID(row) {
    const stockCategoryID = toInt(row && (row.effectCategoryID ?? row.effectCategory), 0);
    const override = EFFECT_CATEGORY_OVERRIDES_BY_NAME.get(String(row && (row.name || row.effectName) || "").trim().toLowerCase());
...
function readStaticTable(tableName) {
    ...
    if (typeof database.tableExists === "function" &&
        ...
        log.info(`[ReferenceData] Static table ${tableName} not available; using empty fallback`);
        const fallback = {};
    ...
    if (!result.success) {
        log.warn(`[ReferenceData] Failed to load table ${tableName}: ${result.errorMsg || "READ_ERROR"}`);
        const fallback = {};
        cache.set(tableName, fallback);
        return fallback;
    }
...
function readStaticRows(tableName) {
    ...
    if (rows && typeof rows === "object") {
        return Object.entries(rows).map(([key, value]) => (value && typeof value === "object"
            ? { _key: key, ...value }
            : { _key: key, value }));
    }
    return [];
