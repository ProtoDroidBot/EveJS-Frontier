...
const { buildFiletimeLong, } = require(path.join(__dirname, "./serviceHelpers"));
...
function msToFiletimeString(ms) {
    ...
    return (BigInt(normalizedMs) * FILETIME_TICKS_PER_MS +
...
function resolveJumpFatigueMultiplier(resourceState) {
    const attributes = resourceState &&
...
function getStoredJumpTimers(characterID) {
    ...
    const timers = record.jumpTimers &&
...
function calculateJumpTimers(options = {}) {
    ...
    const jumpFatigueMultiplier = Math.max(0, toFiniteNumber(options.jumpFatigueMultiplier, 1)) || 1;
    const previousFatigueMs = filetimeToMs(options.previousJumpFatigue);
    const remainingFatigueSeconds = previousFatigueMs === null
        ? 0
    ...
    const fatigueSeconds = clamp(remainingFatigueSeconds * distanceFactor, fatigueMinSeconds, FATIGUE_MAX_SECONDS);
    const activationSeconds = clamp(Math.max(ADDITIONAL_DISTANCE * ACTIVATION_SECONDS_PER_DISTANCE, distanceFactor * ACTIVATION_SECONDS_PER_DISTANCE, remainingFatigueSeconds * FATIGUE_RATIO), ADDITIONAL_DISTANCE * ACTIVATION_SECONDS_PER_DISTANCE, ACTIVATION_MAX_SECONDS);
...
function applyJumpTimersBestEffort(session, options = {}) {
    ...
    if (!result.success) {
        log.warn(`[JumpTimers] failed to apply jump timers char=${session && session.characterID} ` +
...
//# sourceMappingURL=jumpTimerRuntime.js.map