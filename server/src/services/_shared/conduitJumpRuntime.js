...
const { buildShipResourceState, getTypeDogmaAttributes, isEffectivelyOnlineModule, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const sessionRegistry = require(path.join(__dirname, "../chat/sessionRegistry"));
const { resolveItemByTypeID, } = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const { applyJumpTimersBestEffort, hasActiveJumpActivation, resolveJumpFatigueMultiplier, } = require(path.join(__dirname, "./jumpTimerRuntime"));
...
function getSessionSolarSystemID(session) {
    return normalizePositiveInt(session && (session.solarsystemid2 || session.solarsystemid), 0);
}
...
function resolveFittedModuleRequiredPassengerAttributeID(resourceState) {
    ...
    for (const item of fittedItems) {
        ...
        const requiredAttributeID = normalizePositiveInt(moduleAttributes[ATTRIBUTE_JUMP_CONDUIT_PASSENGER_REQUIRED_ATTRIBUTE_ID], 0);
...
function resolveRequiredPassengerAttributeID(resourceState, attributes) {
    const directRequiredAttributeID = normalizePositiveInt(attributes[ATTRIBUTE_JUMP_CONDUIT_PASSENGER_REQUIRED_ATTRIBUTE_ID], 0);
...
function getEntitySurfaceDistanceMeters(leftEntity, rightEntity) {
    ...
    const centerDistance = vectorDistance(cloneVector(leftEntity.position), cloneVector(rightEntity.position));
...
function isCarrierConduitPassengerShip(shipItem) {
    ...
    const categoryID = normalizePositiveInt((shipItem && shipItem.categoryID) ?? (typeRecord && typeRecord.categoryID), 0);
    const groupID = normalizePositiveInt((shipItem && shipItem.groupID) ?? (typeRecord && typeRecord.groupID), 0);
    return (categoryID === CATEGORY_SHIP &&
        CARRIER_CONDUIT_PASSENGER_GROUP_IDS.has(groupID));
}
function hasRequiredPassengerAttribute(passengerAttributes, requiredPassengerAttributeID, passengerShip) {
    if (toFiniteNumber(passengerAttributes[requiredPassengerAttributeID], 0) > 0) {
        return true;
    }
...
function resolveConduitJumpActivation(resourceState, distanceLy) {
    ...
    const passengerLimit = Math.max(0, normalizeInt(attributes[ATTRIBUTE_CONDUIT_JUMP_PASSENGER_COUNT], 0));
    const fuelAmountPerLy = Math.max(0, toFiniteNumber(attributes[ATTRIBUTE_GROUP_JUMP_DRIVE_CONSUMPTION_AMOUNT], 0));
    const requiredPassengerAttributeID = resolveRequiredPassengerAttributeID(resourceState, attributes);
    if (requiredPassengerAttributeID <= 0 ||
        passengerLimit <= 0 ||
        fuelAmountPerLy <= 0) {
        return {
            success: false,
    ...
    return {
        ...
        passengerLimit,
        fuelAmountPerLy,
        fuelQuantity: Math.max(0, Math.trunc(Math.max(0, toFiniteNumber(distanceLy, 0)) * fuelAmountPerLy)),
    };
}
function buildConduitPassengerPlans(options = {}) {
    const { fleet = null, anchorSession = null, anchorEntity = null, destinationSolarSystemID = 0, requiredPassengerAttributeID = 0, passengerLimit = 0, buildSpawnState = null, } = options;
    const anchorCharacterID = normalizePositiveInt(anchorSession && anchorSession.characterID, 0);
    ...
    for (const member of fleet.members.values()) {
        ...
        const passengerEntity = spaceRuntime.getEntity(passengerSession, passengerShipID);
        if (!passengerEntity) {
        ...
        if (!canEntitiesInteractLocally(anchorEntity, passengerEntity)) {
            skipped.push({ characterID, reason: "scope" });
            continue;
        }
        const rangeMeters = getEntitySurfaceDistanceMeters(passengerEntity, anchorEntity);
        if (rangeMeters > CONDUIT_JUMP_RANGE_METERS) {
            skipped.push({ characterID, reason: "range", rangeMeters });
            continue;
        }
        const passengerResourceState = buildShipResourceState(characterID, passengerShip);
        const passengerAttributes = getResourceAttributes(passengerResourceState);
        if (!hasRequiredPassengerAttribute(passengerAttributes, requiredPassengerAttributeID, passengerShip)) {
            skipped.push({
                characterID,
        ...
        const spawnStateOverride = typeof buildSpawnState === "function"
            ? buildSpawnState({
                passengerEntity,
                passengerSession,
                passengerShip,
...
function notifyGroupJumpedPassenger(session, anchorCharacterID, destinationSolarSystemID) {
...
function executeConduitPassengerJumps(options = {}) {
    const { plans = [], anchorCharacterID = 0, destinationSolarSystemID = 0, distanceLy = 0, logPrefix = "ConduitJump", } = options;
    const results = [];
    for (const plan of Array.isArray(plans) ? plans : []) {
        const passengerSession = plan && plan.passengerSession;
        const characterID = normalizePositiveInt(plan && plan.characterID, 0);
        const jumpResult = jumpSessionToSolarSystem(passengerSession, destinationSolarSystemID, {
            spawnStateOverride: plan.spawnStateOverride || null,
        });
        if (!jumpResult.success) {
            log.warn(`[${logPrefix}] conduit passenger left behind ` +
        ...
        notifyGroupJumpedPassenger(passengerSession, anchorCharacterID, destinationSolarSystemID);
        results.push({
            characterID,
            success: true,
            errorMsg: null,
...
//# sourceMappingURL=conduitJumpRuntime.js.map