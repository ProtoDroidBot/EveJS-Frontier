...
function readIdentityState() {
    ...
    const nextAccountID = toPositiveInt(state.nextAccountID, DEFAULT_IDENTITY_STATE.nextAccountID);
    if (nextAccountID !== state.nextAccountID) {
        state.nextAccountID = nextAccountID;
        mutated = true;
    }
    const nextCharacterID = toPositiveInt(state.nextCharacterID, DEFAULT_IDENTITY_STATE.nextCharacterID);
    if (nextCharacterID !== state.nextCharacterID) {
        state.nextCharacterID = nextCharacterID;
        mutated = true;
    }
    const nextItemID = toPositiveInt(state.nextItemID, DEFAULT_IDENTITY_STATE.nextItemID);
...
function collectCharacterIDConflicts(characterID) {
    ...
    for (const record of Object.values(mail.messages || {})) {
        ...
        if (Array.isArray(record && record.toCharacterIDs) &&
            record.toCharacterIDs.some((entry) => toPositiveInt(entry, 0) === normalizedCharacterID)) {
            conflicts.push("mail.messages.toCharacterIDs");
            break;
        }
    }
    const notifications = readTableRoot("notifications", {});
    if (notifications.boxes &&
    ...
    if (lpWallets.characterWallets &&
        Object.prototype.hasOwnProperty.call(lpWallets.characterWallets, String(normalizedCharacterID))) {
        conflicts.push("lpWallets.characterWallets");
    }
    const characterNotes = readTableRoot("characterNotes", {});
    if (characterNotes.characters &&
        Object.prototype.hasOwnProperty.call(characterNotes.characters, String(normalizedCharacterID))) {
    ...
    if (Object.prototype.hasOwnProperty.call(skillTradingState, String(normalizedCharacterID))) {
        conflicts.push("skillTradingState");
    }
    const characterExpertSystems = readTableRoot("characterExpertSystems", {});
    if (Object.prototype.hasOwnProperty.call(characterExpertSystems, String(normalizedCharacterID))) {
        conflicts.push("characterExpertSystems");
    }
    const bookmarkKnownFolders = readTableRoot("bookmarkKnownFolders", {});
    if (bookmarkKnownFolders.recordsByCharacterID &&
        Object.prototype.hasOwnProperty.call(bookmarkKnownFolders.recordsByCharacterID, String(normalizedCharacterID))) {
    ...
    for (const group of Object.values(bookmarkGroups.records || {})) {
        ...
        if (Array.isArray(group && group.admins) &&
    ...
    for (const event of Object.values(calendarEvents.events || {})) {
        ...
        if (Array.isArray(event && event.inviteeCharacterIDs) &&
            event.inviteeCharacterIDs.some((entry) => toPositiveInt(entry, 0) === normalizedCharacterID)) {
...
function getAccountReferenceHighWaterMark() {
    ...
    const characters = readTableRoot("characters", {});
    for (const record of Object.values(characters)) {
        maxValue = maxFromNumericValues(maxValue, record && record.accountId, ACCOUNT_ID_FLOOR);
    }
    return Math.max(maxValue, ACCOUNT_ID_FLOOR - 1);
}
function getCharacterReferenceHighWaterMark() {
    const identityState = readIdentityState();
    let maxValue = toPositiveInt(identityState.nextCharacterID, CHARACTER_ID_FLOOR) - 1;
    const characters = readTableRoot("characters", {});
    maxValue = maxFromNumericKeys(maxValue, characters, CHARACTER_ID_FLOOR);
    const mail = readTableRoot("mail", {});
    maxValue = maxFromNumericKeys(maxValue, mail.mailboxes || {}, CHARACTER_ID_FLOOR);
    for (const record of Object.values(mail.messages || {})) {
        maxValue = maxFromNumericValues(maxValue, record && record.senderID, CHARACTER_ID_FLOOR);
        maxValue = maxFromNumericValues(maxValue, record && record.toCharacterIDs, CHARACTER_ID_FLOOR);
    }
    const notifications = readTableRoot("notifications", {});
    maxValue = maxFromNumericKeys(maxValue, notifications.boxes || {}, CHARACTER_ID_FLOOR);
    const lpWallets = readTableRoot("lpWallets", {});
    maxValue = maxFromNumericKeys(maxValue, lpWallets.characterWallets || {}, CHARACTER_ID_FLOOR);
    const characterNotes = readTableRoot("characterNotes", {});
    maxValue = maxFromNumericKeys(maxValue, characterNotes.characters || {}, CHARACTER_ID_FLOOR);
    maxValue = maxFromNumericKeys(maxValue, readTableRoot("skillPlans", {}), CHARACTER_ID_FLOOR);
    maxValue = maxFromNumericKeys(maxValue, readTableRoot("skillTradingState", {}), CHARACTER_ID_FLOOR);
    maxValue = maxFromNumericKeys(maxValue, readTableRoot("characterExpertSystems", {}), CHARACTER_ID_FLOOR);
    const bookmarkKnownFolders = readTableRoot("bookmarkKnownFolders", {});
    maxValue = maxFromNumericKeys(maxValue, bookmarkKnownFolders.recordsByCharacterID || {}, CHARACTER_ID_FLOOR);
    const savedFittings = readTableRoot("savedFittings", {});
    for (const [ownerID, ownerRecord] of Object.entries(savedFittings.owners || {})) {
        if (String(ownerRecord && ownerRecord.scope || "").toLowerCase() === "character") {
            maxValue = maxFromNumericValues(maxValue, ownerID, CHARACTER_ID_FLOOR);
            maxValue = maxFromNumericValues(maxValue, ownerRecord && ownerRecord.ownerID, CHARACTER_ID_FLOOR);
        }
    }
    const bookmarkFolders = readTableRoot("bookmarkFolders", {});
    for (const folder of Object.values(bookmarkFolders.records || {})) {
        maxValue = maxFromNumericValues(maxValue, folder && folder.creatorID, CHARACTER_ID_FLOOR);
    }
    const bookmarkSubfolders = readTableRoot("bookmarkSubfolders", {});
    for (const subfolder of Object.values(bookmarkSubfolders.records || {})) {
        maxValue = maxFromNumericValues(maxValue, subfolder && subfolder.creatorID, CHARACTER_ID_FLOOR);
    }
    const bookmarks = readTableRoot("bookmarks", {});
    for (const bookmark of Object.values(bookmarks.records || {})) {
        maxValue = maxFromNumericValues(maxValue, bookmark && bookmark.creatorID, CHARACTER_ID_FLOOR);
    }
    const bookmarkGroups = readTableRoot("bookmarkGroups", {});
    for (const group of Object.values(bookmarkGroups.records || {})) {
        maxValue = maxFromNumericValues(maxValue, group && group.creatorID, CHARACTER_ID_FLOOR);
        maxValue = maxFromNumericValues(maxValue, group && group.admins, CHARACTER_ID_FLOOR);
        maxValue = maxFromNumericValues(maxValue, group && group.members, CHARACTER_ID_FLOOR);
    }
    const calendarEvents = readTableRoot("calendarEvents", {});
    for (const event of Object.values(calendarEvents.events || {})) {
        maxValue = maxFromNumericValues(maxValue, event && event.creatorID, CHARACTER_ID_FLOOR);
        maxValue = maxFromNumericValues(maxValue, event && event.ownerID, CHARACTER_ID_FLOOR);
        maxValue = maxFromNumericValues(maxValue, event && event.inviteeCharacterIDs, CHARACTER_ID_FLOOR);
    }
    const calendarResponses = readTableRoot("calendarResponses", {});
    for (const response of Object.values(calendarResponses.responses || {})) {
        maxValue = maxFromNumericValues(maxValue, response && response.characterID, CHARACTER_ID_FLOOR);
    }
...
function getItemReferenceHighWaterMark() {
    ...
    for (const record of Object.values(items)) {
        maxValue = maxFromNumericValues(maxValue, record && record.itemID, ITEM_ID_FLOOR);
    }
    const characters = readTableRoot("characters", {});
    for (const record of Object.values(characters)) {
        maxValue = maxFromNumericValues(maxValue, record && record.shipID, ITEM_ID_FLOOR);
        if (Array.isArray(record && record.storedShips)) {
            for (const ship of record.storedShips) {
                maxValue = maxFromNumericValues(maxValue, ship && ship.itemID, ITEM_ID_FLOOR);
            }
        }
    }
...
function reserveAccountID() {
    ...
    let candidate = Math.max(toPositiveInt(state.nextAccountID, ACCOUNT_ID_FLOOR), getAccountReferenceHighWaterMark() + 1);
    while (true) {
        const conflicts = collectAccountIDConflicts(candidate);
        if (conflicts.length === 0) {
            break;
        }
        log.warn(`[IdentityAllocator] Skipping reused accountID ${candidate}; still referenced by ${conflicts.join(", ")}`);
        candidate += 1;
    }
...
function reserveCharacterID() {
    ...
    while (true) {
        ...
        log.warn(`[IdentityAllocator] Skipping reused characterID ${candidate}; still referenced by ${conflicts.join(", ")}`);
        candidate += 1;
    }
...
//# sourceMappingURL=identityAllocator.js.map