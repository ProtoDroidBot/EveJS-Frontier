...
const { getSessionStructureID, } = require(path.join(__dirname, "../structure/structureLocation"));
const { getOwnerLookupRecord, } = require(path.join(__dirname, "../corporation/corporationState"));
const { getFactionRecord, getFactionRecordByCorporationID, } = require(path.join(__dirname, "../faction/factionState"));
const { TABLE, readStaticTable, } = require(path.join(__dirname, "./referenceData"));
...
function normalizeRestrictionsPayload(payload = {}) {
    return {
        restrictionsByOwnerID: payload && typeof payload.restrictionsByOwnerID === "object"
...
function getStandingsRestrictions() {
    ...
    const payload = normalizeRestrictionsPayload(readStaticTable(TABLE.STATION_STANDINGS_RESTRICTIONS));
...
function getStationRecord(session = null, overrideStationID = null) {
    const stationID = overrideStationID ||
        (session && (session.stationid || session.stationID || session.locationid)) ||
        DEFAULT_STATION.stationID;
    const structureID = overrideStationID ||
        getSessionStructureID(session) ||
        null;
    const structure = structureID && !worldData.getStationByID(structureID)
        ? structureState.getStructureByID(structureID)
        : null;
    if (structure) {
        const dockingGeometry = structureLocatorGeometry.buildStructureDockingGeometry(structure, {
            selectionStrategy: "first",
        });
        const ownerRecord = getOwnerLookupRecord(structure.ownerCorpID) ||
            NPC_OWNER_OVERRIDES[structure.ownerCorpID] ||
            null;
        const factionIdentity = resolveFactionIdentity(structure.ownerCorpID, DEFAULT_STATION.factionID);
        ...
        return {
            ...
            solarSystemName: (solarSystem && solarSystem.solarSystemName) ||
    ...
    if (station) {
        const ownerRecord = getOwnerLookupRecord(station.corporationID) ||
            getOwnerLookupRecord(station.ownerID) ||
            NPC_OWNER_OVERRIDES[station.corporationID] ||
            NPC_OWNER_OVERRIDES[station.ownerID] ||
            null;
        const factionIdentity = resolveFactionIdentity(station.corporationID || station.ownerID, DEFAULT_STATION.factionID);
    ...
    return {
        ...DEFAULT_STATION,
        stationID,
        solarSystemID: (session && (session.solarsystemid2 || session.solarsystemid)) ||
            DEFAULT_STATION.solarSystemID,
        constellationID: (session && session.constellationID) || DEFAULT_STATION.constellationID,
...
function getStaticOwnerRecord(ownerID, session = null) {
    ...
    if (numericOwnerID === station.ownerID ||
...
function buildStationServiceMask(session = null, overrideStationID = null) {
    const station = getStationRecord(session, overrideStationID);
    return STATION_SERVICES.reduce((mask, service) => {
...
function stationServiceIsEnabledForStationRecord(station, stationServiceID) {
    ...
    const structureServiceID = STATION_SERVICE_TO_STRUCTURE_SERVICE_ID[Number(stationServiceID) || 0];
    if (!structureServiceID) {
        return false;
    }
    return (Number(station.serviceStates &&
        station.serviceStates[String(structureServiceID)]) === STRUCTURE_SERVICE_STATE.ONLINE);
}
module.exports = {
    DEFAULT_STATION,
    NPC_OWNER_OVERRIDES,
...
//# sourceMappingURL=stationStaticData.js.map