...
function freezeThresholds(row) {
    return Object.freeze({
        hostileResponseThreshold: normalizeThreshold(row && row.hostileResponseThreshold, -11),
        friendlyResponseThreshold: normalizeThreshold(row && row.friendlyResponseThreshold, 11),
    });
}
function buildCache() {
...
function ensureLoaded() {
    if (!cache) {
        cache = buildCache();
        logDataLoad(`[ClientEntityStandings] Loaded ${cache.typeCount} type thresholds.`);
    }
    return cache;
}
...
//# sourceMappingURL=clientEntityStandings.js.map