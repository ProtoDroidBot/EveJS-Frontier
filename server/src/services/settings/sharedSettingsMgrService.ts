...
class SharedSettingsMgrService extends BaseService {
  ...
  _storePayloadString(settingTypeID, payloadString, options: Record<string, any> = {}) {
  ...
  Handle_StoreSettingLinkAndGetID(args: any[] = [], session) {
  ...
  Handle_GetStoredSharedSetting(args: any[] = []) {
    const state = this._getState();
