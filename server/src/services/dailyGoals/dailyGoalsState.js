...
const { CATEGORY, DAILY_ROTATION_SIZE, DAILY_TEMPLATE_BY_KEY, BONUS_TEMPLATE, BONUS_TARGET_DAILY_COMPLETIONS, EVERMARK_ISSUER_CORP_ID, EM_PAID_COMPLETION_COST_FIRST, EM_PAID_COMPLETION_COST_SECOND, MAX_PAID_COMPLETIONS_PER_DAY, grantableRewardsForTemplate, pickDailyTemplateKeys, MONTHLY_MILESTONE_TARGETS, MONTHLY_MILESTONES, MONTHLY_MILESTONE_BY_KEY, grantableRewardForMilestone, } = require("./dailyGoalsCatalog");
...
function publishNotice(characterID, event) {
    ...
    try {
        ...
        log.debug(`[DailyGoals] notice publish failed char=${characterID} kind=${event && event.kind}: ${error.message}`);
    }
}
...
function currentPeriod(nowMs = Date.now()) {
    ...
    const boundary = Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), DOWNTIME_HOUR_UTC, 0, 0, 0);
    let activeAfterMs;
    let activeUntilMs;
    if (nowMs >= boundary) {
...
function findGoalObject(entry, uuid) {
    ...
    return (allGoalObjects(entry).find((goal) => String(goal.uuid).toLowerCase() === normalized) || null);
}
...
function getSkillQueueNotifications() {
    return require(path.join(__dirname, "../skills/training/skillQueueNotifications"));
}
...
function getEvermarkBalance(characterID) {
    try {
        return (getLpWalletState().getCharacterWalletLPBalance(toInt(characterID), EVERMARK_ISSUER_CORP_ID) || 0);
    }
    catch (error) {
        return 0;
    }
...
function grantEvermarks(characterID, amount) {
    ...
    try {
        getLpWalletState().adjustCharacterWalletLPBalance(toInt(characterID), EVERMARK_ISSUER_CORP_ID, amount, { changeType: "daily_goal_reward" });
    }
    catch (error) {
        log.warn(`[DailyGoals] Evermark grant failed char=${characterID} amount=${amount}: ${error.message}`);
    }
}
...
function spendEvermarks(characterID, amount) {
    ...
    try {
        const result = getLpWalletState().adjustCharacterWalletLPBalance(toInt(characterID), EVERMARK_ISSUER_CORP_ID, -amount, { changeType: "daily_goal_paid_completion" });
        return Boolean(result && result.success);
    }
    catch (error) {
        log.warn(`[DailyGoals] Evermark spend failed char=${characterID} amount=${amount}: ${error.message}`);
        return false;
    }
}
function grantIsk(characterID, amount) {
    ...
    try {
        ...
        log.warn(`[DailyGoals] ISK grant failed char=${characterID} amount=${amount}: ${error.message}`);
    }
}
function grantSkillPoints(characterID, amount) {
    ...
    try {
        const result = getCharacterState().updateCharacterRecord(characterID, (record) => ({
            ...record,
        ...
        log.warn(`[DailyGoals] SP grant failed char=${characterID} amount=${amount}: ${error.message}`);
    }
}
...
function grantAndRedeem(characterID, entry, goal) {
    ...
    goal.redeemed = true;
    log.info(`[DailyGoals] reward granted char=${characterID} goal=${template.key} ` +
...
function grantMilestoneReward(characterID, milestone) {
    ...
    grantIsk(characterID, grant.isk);
    grantSkillPoints(characterID, grant.skillPoints);
    log.info(`[DailyGoals] monthly milestone granted char=${characterID} milestone=${milestone.key} ` +
...
function autoClaimDueMilestones(characterID, state, nowMs, force = false) {
    ...
    for (const milestone of MONTHLY_MILESTONES) {
        ...
        const due = force || nowMs - toInt(milestoneState.earnedAtMs) >= MONTHLY_AUTO_CLAIM_AFTER_MS;
        if (!due) {
            continue;
        }
...
function creditMonthlyBonusDay(root, characterID, entry, nowMs) {
    ...
    log.info(`[DailyGoals] monthly bonus day char=${characterID} month=${state.monthKey} ` +
...
function payForCompletion(characterID, uuid, nowMs = Date.now()) {
    ...
    updateBonusProgress(numericCharacterID, root, entry, nowMs);
    writeRoot(root);
    log.info(`[DailyGoals] paid completion char=${numericCharacterID} goal=${goal.templateKey} cost=${cost}EM`);
...
//# sourceMappingURL=dailyGoalsState.js.map