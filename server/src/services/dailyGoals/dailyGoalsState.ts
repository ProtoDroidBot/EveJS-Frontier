...
function redeemGoal(characterID, uuid, nowMs = Date.now()) {
  ...
  const goal = findGoalObject(entry, uuid);
  if (!goal) {
    return { success: false as const, errorMsg: "GOAL_NOT_FOUND" };
  }
  if (!goal.completed) {
    return { success: false as const, errorMsg: "GOAL_NOT_COMPLETED" };
  }
  ...
  return { success: true as const, granted };
}
...
function payForCompletion(characterID, uuid, nowMs = Date.now()) {
  ...
  if (goal.completed) {
    return { success: false as const, errorMsg: "ALREADY_COMPLETE" };
  }
  const cost = evermarkCostForNextPaidCompletion(entry);
  if (cost === null) {
    return { success: false as const, errorMsg: "MAX_PAID_COMPLETIONS" };
  }
  const balance = getEvermarkBalance(numericCharacterID);
  if (balance < cost) {
    return { success: false as const, errorMsg: "INSUFFICIENT_EVERMARKS", cost, balance };
  }
...
function claimMonthlyMilestone(characterID, uuid, nowMs = Date.now()) {
  ...
  const found = findMilestoneByUuid(state, uuid);
  if (!found) {
    return { success: false as const, errorMsg: "MILESTONE_NOT_FOUND" };
  }
  const { milestone, milestoneState } = found;
  if (!milestoneState.earnedAtMs) {
    return { success: false as const, errorMsg: "NOT_EARNED" };
  }
  ...
  if (milestone.omega && !characterHasOmega(numericCharacterID)) {
    return { success: false as const, errorMsg: "OMEGA_REQUIRED" };
  }
  ...
  writeRoot(root);
  return { success: true as const, granted: true };
}
