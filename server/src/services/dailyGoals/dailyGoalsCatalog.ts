...
type RewardUnit = { isk?: { amount: { units: number } }; sp?: { amount: number }; lp?: { amount: { amount: number; associated_corporation: { sequential: number } } } };

function grantableRewardsForTemplate(template) {
  const specs: { period: number; unit: RewardUnit }[] = template.isBonus
