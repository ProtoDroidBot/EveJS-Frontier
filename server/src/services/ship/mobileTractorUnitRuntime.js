...
const { ITEM_FLAGS, createSpaceItemForOwner, findItemById, getItemMetadata, listContainerItems, listSystemSpaceItems, moveItemToLocation, removeInventoryItem, transferItemToOwnerLocation, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
const { buildShipResourceState, getTypeAttributeValue, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const { resolveItemByName, } = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const worldData = require(path.join(__dirname, "../../space/worldData"));
const { captureMobileTractorUnitPresentation, applyMobileTractorUnitTractorStartCommand, } = require(path.join(__dirname, "../../space/destiny/commands/mobileTractorUnit.js"));
const nativeNpcStore = require(path.join(__dirname, "../../space/npc/nativeNpcStore"));
const { buildGodmaShipEffectEvent, buildModuleAttributeChangeEvent, sendOnMultiEventPairs, } = require(path.join(__dirname, "../_shared/godmaMultiEvent"));
...
function syncInventoryChange(session, item, previousData, options = {}) {
    ...
    emitItemsChangedForSession(session, item, previousData || {}, {
...
function listInterestedSessions(scene, characterIDs = []) {
    ...
    const interestedCharacterIDs = new Set(characterIDs.map((characterID) => toInt(characterID, 0)).filter((characterID) => characterID > 0));
    if (interestedCharacterIDs.size === 0) {
        return [];
    }
    return [...scene.sessions.values()].filter((session) => (session && interestedCharacterIDs.has(toInt(session.characterID || session.charid, 0))));
}
function syncInventoryChangesToInterestedSessions(scene, characterIDs = [], changes = [], options = {}) {
    ...
    for (const session of listInterestedSessions(scene, characterIDs)) {
        for (const change of changes) {
            ...
            syncInventoryChange(session, change.item, change.previousData || change.previousState || {}, options);
        }
    }
}
...
function isMobileTractorUnitType(typeIDOrItem) {
    if (typeIDOrItem && typeof typeIDOrItem === "object") {
        if (toInt(typeIDOrItem.categoryID, 0) === CATEGORY_DEPLOYABLE &&
    ...
    return (toInt(metadata.categoryID, 0) === CATEGORY_DEPLOYABLE &&
...
function getStateFromCustomInfo(customInfo) {
    ...
    return {
        ...
        maxTractorVelocity: toReal(state.maxTractorVelocity, MOBILE_TRACTOR_UNIT_DEFAULT_MAX_TRACTOR_VELOCITY),
...
function validateMobileTractorUnitDestinationCargo(item, context) {
    ...
    const used = getShipFlagUsedVolume(context.characterID, context.shipID, ITEM_FLAGS.CARGO_HOLD, item && item.itemID);
...
function validateMobileTractorUnitSpaceAccess(session, item, state, context, options = {}) {
    const rangeMeters = Math.max(0, toReal(options.rangeMeters, MOBILE_TRACTOR_UNIT_SCOOP_RANGE_METERS));
    ...
    if (toInt(item.locationID, 0) !== context.systemID ||
        toInt(item.flagID, -1) !== 0 ||
        !item.spaceState) {
        return "MOBILE_TRACTOR_UNIT_NOT_IN_SPACE";
    }
    if (toInt(item.ownerID, 0) !== context.characterID &&
    ...
    const shipPosition = (shipEntity && shipEntity.position) ||
        (shipItem && shipItem.spaceState && shipItem.spaceState.position);
    const tractorPosition = (tractorEntity && tractorEntity.position) ||
        (item.spaceState && item.spaceState.position);
    if (!canEntitiesInteractLocally(shipEntity || shipItem, tractorEntity || item)) {
...
function activateMobileTractorUnit(itemID) {
    ...
    log.info(`[MobileTractorUnit] Activated tractorUnit=${itemID} system=${state.solarSystemID}`);
    return true;
}
...
function scheduleMobileTractorUnitTimers(itemOrID) {
    ...
    if (state.expiresAtMs > 0) {
        nextTimers.expiryTimer = setUnrefTimeout(() => expireMobileTractorUnit(itemID, "expired"), state.expiresAtMs - now);
    }
    if (state.active || state.activateAtMs <= now) {
        if (!state.active) {
            nextTimers.activationTimer = setUnrefTimeout(() => activateMobileTractorUnit(itemID), 0);
        }
    }
    else {
        nextTimers.activationTimer = setUnrefTimeout(() => activateMobileTractorUnit(itemID), state.activateAtMs - now);
    }
...
function hydrateMobileTractorUnitEntityFromInventoryItem(entity, itemRecord) {
    ...
    entity.activate_comp_durationSeconds = Math.trunc(MOBILE_TRACTOR_UNIT_ACTIVATION_DELAY_MS / 1000);
...
function launchMobileTractorUnitFromShip(session, itemID, options = {}) {
    ...
    if (placementConflict) {
        log.info(`[MobileTractorUnit] Launch rejected char=${context.characterID} item=${item.itemID} system=${context.systemID} reason=${placementConflict.errorMsg}`);
        return {
            success: false,
    ...
    syncInventoryChange(session, updatedResult.data, launchedChange.previousData || updatedResult.previousData || {}, { idType: "charid" });
    ...
    if (!spawnResult || !spawnResult.success) {
        log.warn(`[MobileTractorUnit] Launched tractor unit ${launchedItemID} but space spawn failed: ${spawnResult ? spawnResult.errorMsg : "UNKNOWN"}`);
    }
    registerMobileTractorUnitItem(updatedResult.data);
    log.info(`[MobileTractorUnit] char=${context.characterID} launched tractorUnit itemID=${launchedItemID} typeID=${sourceTypeID} system=${context.systemID}`);
    return {
        success: true,
        data: {
...
function buildNearbyEjectedContainerSpawnState(tractorItem, tractorEntity, shipEntity) {
    const position = getObjectPosition(tractorEntity) ||
        getObjectPosition(tractorItem) ||
        getObjectPosition(shipEntity);
    if (!position) {
        return null;
    }
    const direction = normalizeDirection((tractorEntity && tractorEntity.direction) ||
        (tractorItem && tractorItem.spaceState && tractorItem.spaceState.direction) ||
        (shipEntity && shipEntity.direction), { x: 1, y: 0, z: 0 });
    const containerPosition = addVectors(position, scaleVector(direction, MOBILE_TRACTOR_UNIT_EJECTED_CONTAINER_DISTANCE_METERS));
    return {
        ...buildChildEntityScopeMetadata({
            ...(tractorItem && tractorItem.spaceState
                ? tractorItem.spaceState
                : {}),
            ...(tractorEntity || shipEntity || {}),
        }),
...
function ejectMobileTractorUnitContentsToContainer(session, item, state, context) {
    ...
    for (const contentItem of contents) {
        const moveResult = moveItemToLocation(contentItem.itemID, containerItem.itemID, ITEM_FLAGS.HANGAR);
        if (!moveResult.success) {
            log.warn(`[MobileTractorUnit] Failed to eject content item=${contentItem.itemID} from tractorUnit=${item.itemID} to container=${containerItem.itemID}: ${moveResult.errorMsg}`);
            return {
                success: false,
    ...
    if (!spawnResult || !spawnResult.success) {
        log.warn(`[MobileTractorUnit] Ejected container ${containerItem.itemID} was created but space spawn failed: ${spawnResult ? spawnResult.errorMsg : "UNKNOWN"}`);
    }
    return {
        success: true,
        data: {
...
function scoopMobileTractorUnitToCargo(session, itemID) {
    ...
    const ejectionResult = ejectMobileTractorUnitContentsToContainer(session, item, state, context);
    ...
    syncInventoryChange(session, updateResult.data, updateResult.previousData || item, {
        idType: "charid",
    ...
    log.info(`[MobileTractorUnit] char=${context.characterID} scooped tractorUnit itemID=${item.itemID} system=${context.systemID} ship=${context.shipID} ejectedContainer=${ejectionResult.data.containerID || 0} movedItems=${ejectionResult.data.movedItemCount || 0}`);
    return {
        success: true,
        response: true,
        data: {
...
function getItemMoveVolume(item) {
    ...
    return Math.max(0, firstFiniteReal([item && item.volume, metadata.volume], 0));
}
...
function getContainerUsedVolume(locationID) {
    return listContainerItems(null, locationID, MOBILE_TRACTOR_UNIT_CARGO_FLAG_ID).reduce((sum, item) => (sum + getItemMoveVolume(item) * getItemMoveQuantity(item)), 0);
}
...
function findNearestLootTarget(scene, tractorEntity, state) {
    ...
    for (const entity of candidates) {
        if (!isTractorableTarget(entity) ||
            toInt(entity.itemID, 0) === toInt(tractorEntity.itemID, 0) ||
            !canEntitiesInteractLocally(tractorEntity, entity) ||
...
function transferInventoryTargetLootToMtu(targetEntity, tractorUnitID, ownerID, holdCapacity) {
    ...
    for (const item of contents) {
        ...
        const moveResult = transferItemToOwnerLocation(item.itemID, ownerID, tractorUnitID, MOBILE_TRACTOR_UNIT_CARGO_FLAG_ID, moveQuantity);
        if (!moveResult.success) {
            return {
                success: false,
...
function broadcastMobileTractorUnitTractorStart(scene, entity, targetEntity, state, nowMs) {
    ...
    const stamp = captureMobileTractorUnitPresentation({ scene, nowMs });
    const startTime = getSceneFileTime(scene, nowMs);
    const tractorVelocity = Math.max(0, toReal(state && state.maxTractorVelocity, MOBILE_TRACTOR_UNIT_DEFAULT_MAX_TRACTOR_VELOCITY));
    const tractorStartCommand = applyMobileTractorUnitTractorStartCommand({
        sourceEntity: entity,
        targetEntity,
        tractorSpeedMetersPerSecond: tractorVelocity,
        followRangeMeters: TRACTOR_BEAM_FOLLOW_RANGE_METERS,
        effectGuid: TRACTOR_BEAM_EFFECT_GUID,
        effectDurationMs: TRACTOR_BEAM_EFFECT_DURATION_MS,
        effectRepeat: TRACTOR_BEAM_EFFECT_REPEAT,
        effectStartTime: startTime,
    });
    if (!tractorStartCommand.applied) {
        return null;
    }
    if (typeof scene.broadcastMovementUpdates === "function") {
        scene.broadcastMovementUpdates(tractorStartCommand.buildPresentationUpdates({ stamp }));
    }
    if (typeof scene.persistDynamicEntity === "function") {
        scene.persistDynamicEntity(targetEntity);
    }
    const godmaEffect = buildGodmaShipEffectEvent(entity.itemID, state && state.ownerID, entity.itemID, TRACTOR_BEAM_GODMA_EFFECT_ID, startTime, {
        isStart: true,
        shouldStart: true,
    ...
    const velocityChange = buildModuleAttributeChangeEvent(state && state.ownerID, targetEntity.itemID, TRACTOR_TARGET_MAX_SPEED_ATTRIBUTE_ID, tractorVelocity, tractorVelocity, startTime);
...
function processMobileTractorUnitCycle(scene, entity, item, state, nowMs) {
    ...
    const tractorPresentation = targetEntity
    ...
    if (targetEntity) {
        transferResult = targetEntity.nativeNpcWreck === true
            ? transferNativeTargetLootToMtu(targetEntity, state.tractorUnitID, state.ownerID, state.holdCapacity)
            : transferInventoryTargetLootToMtu(targetEntity, state.tractorUnitID, state.ownerID, state.holdCapacity);
        if (!transferResult.success) {
            log.warn(`[MobileTractorUnit] Loot cycle failed tractorUnit=${state.tractorUnitID} target=${targetEntity.itemID} error=${transferResult.errorMsg}`);
        }
    }
    ...
    updateMobileTractorUnitState(state.tractorUnitID, (currentState) => ({
        ...currentState,
        lastCycleAtMs: nowMs,
        lastLootMoveAtMs: transferResult.movedItemCount > 0 ? nowMs : toInt(currentState.lastLootMoveAtMs, 0),
        lastTractorTargetID: targetEntity ? toInt(targetEntity.itemID, 0) : 0,
        movedItemCount: toInt(currentState.movedItemCount, 0) + toInt(transferResult.movedItemCount, 0),
        nextCycleAtMs: nowMs + cycleMs,
    }));
    if (transferResult.changes.length > 0) {
        syncInventoryChangesToInterestedSessions(scene, [state.ownerID, state.deployerCharacterID], transferResult.changes, { idType: "shipid" });
        ...
        if (targetEntity && targetEntity.nativeNpcWreck === true) {
            getNativeNpcWreckService().refreshNativeWreckRuntimeEntity(state.solarSystemID, targetEntity.itemID);
        }
        if (targetEntity && targetEntity.nativeNpcWreck !== true) {
...
//# sourceMappingURL=mobileTractorUnitRuntime.js.map