...
function normalizeShipCounterRecord(rawRecord: Record<string, any> = {}, itemID = 0) {
...
function incrementItemKillCountPlayer(itemID, options: Record<string, any> = {}) {
  const numericItemID = toPositiveInt(itemID, 0);
  if (!numericItemID) {
    return {
...
function setShipKillCounter(itemID, counts: Record<string, any> = {}) {
  const numericItemID = toPositiveInt(itemID, 0);
  if (!numericItemID) {
    return null;
  }
