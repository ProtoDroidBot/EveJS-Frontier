...
const { getOwnedShipLogoEntitlement, } = require(path.join(__dirname, "../evermarks/evermarksEntitlements"));
const { COSMETIC_TYPE_ALLIANCE_LOGO, COSMETIC_TYPE_CORPORATION_LOGO, SHIP_LOGO_ENTITLEMENT_ALLIANCE, SHIP_LOGO_ENTITLEMENT_CORPORATION, } = require(path.join(__dirname, "../evermarks/evermarksConstants"));
...
function normalizeEntry(shipID, backendSlot, value = {}) {
    ...
    return {
        ...
        updatedAtMs: Math.max(0, Math.trunc(Number(value.updatedAtMs || Date.now()) || Date.now())),
    };
}
function readRoot() {
    ...
    return {
        meta: result.data.meta && typeof result.data.meta === "object"
            ...
            : cloneValue(DEFAULT_ROOT.meta),
        ships: result.data.ships && typeof result.data.ships === "object"
...
function writeRoot(root) {
    const nextRoot = {
        ...
        ships: root && root.ships && typeof root.ships === "object"
...
function ensureShipEntry(root, shipID) {
    ...
    if (!root.ships[shipKey].enabledByBackendSlot ||
...
function getEnabledCosmeticsEntries(shipID) {
    const slotMap = buildIndex().byShipID.get(normalizePositiveInteger(shipID, 0)) || new Map();
...
function validateDisplayRequest(shipID, backendSlot, cosmeticType, characterID) {
    ...
    const entitlement = getOwnedShipLogoEntitlement(normalizedCharacterID, Number(shipItem.typeID || 0) || 0, entitlementType);
    if (!entitlement) {
        return {
            success: false,
...
function setDisplayedLogo(shipID, backendSlot, cosmeticType, options = {}) {
    const validation = validateDisplayRequest(shipID, backendSlot, cosmeticType, options.characterID);
...
function clearDisplayedLogo(shipID, backendSlot) {
    ...
    const shipEntry = root.ships && root.ships[shipKey] && typeof root.ships[shipKey] === "object"
        ? root.ships[shipKey]
        : null;
    const currentEntry = shipEntry &&
...
//# sourceMappingURL=shipLogoFittingState.js.map