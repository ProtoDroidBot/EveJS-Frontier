...
function isMobileObservatoryType(typeIDOrItem) {
    if (typeIDOrItem && typeof typeIDOrItem === "object") {
        return (toInt(typeIDOrItem.categoryID, 0) === CATEGORY_DEPLOYABLE &&
            toInt(typeIDOrItem.groupID, 0) === GROUP_MOBILE_OBSERVATORY) || isMobileObservatoryType(toInt(typeIDOrItem.typeID, 0));
    }
...
function collectPlacementBlockers(session, context, options = {}) {
    ...
    const append = (source, kind = null) => appendPlacementBlocker(blockers, seenKeys, {
        ...source,
...
function activateMobileObservatory(itemID) {
    ...
    log.info(`[MobileObservatory] Activated observatory=${itemID} system=${state.solarSystemID}`);
    return true;
}
function emitMobileObservatoryPing(itemID) {
    ...
    const observatoryEntity = scene && typeof scene.getEntityByID === "function"
        ...
        : null;
    const pingResult = scene && typeof scene.processRemoteDecloakPing === "function"
    ...
    const nextPingAtMs = now + MOBILE_OBSERVATORY_PING_INTERVAL_MS < state.expiresAtMs
    ...
    scheduleMobileObservatoryTimers(itemID);
    log.info(`[MobileObservatory] Ping observatory=${itemID} system=${state.solarSystemID} decloaked=${(pingResult.decloakedEntityIDs || []).length}`);
    return true;
}
...
function scheduleMobileObservatoryTimers(itemOrID) {
    ...
    if (state.expiresAtMs > 0) {
        nextTimers.expiryTimer = setUnrefTimeout(() => expireMobileObservatory(itemID, "expired"), state.expiresAtMs - now);
    }
    if (state.active || state.activateAtMs <= now) {
        if (!state.active) {
            nextTimers.activationTimer = setUnrefTimeout(() => activateMobileObservatory(itemID), 0);
        }
        else if (state.nextPingAtMs > now) {
            nextTimers.pingTimer = setUnrefTimeout(() => emitMobileObservatoryPing(itemID), state.nextPingAtMs - now);
        }
        else if (state.nextPingAtMs > 0) {
            nextTimers.pingTimer = setUnrefTimeout(() => emitMobileObservatoryPing(itemID), 0);
        }
    }
    else {
        nextTimers.activationTimer = setUnrefTimeout(() => activateMobileObservatory(itemID), state.activateAtMs - now);
    }
...
function hydrateMobileObservatoryEntityFromInventoryItem(entity, itemRecord) {
    ...
    entity.activate_comp_durationSeconds = Math.trunc(MOBILE_OBSERVATORY_ACTIVATION_DELAY_MS / 1000);
    entity.component_decloakemitter_nextPing =
        active && state.nextPingAtMs > 0 ? state.nextPingAtMs : null;
    entity.decloakEmitterIntervalSeconds = Math.trunc(MOBILE_OBSERVATORY_PING_INTERVAL_MS / 1000);
...
function launchMobileObservatoryFromShip(session, itemID, options = {}) {
    ...
    if (placementConflict) {
        log.info(`[MobileObservatory] Launch rejected char=${context.characterID} item=${item.itemID} system=${context.systemID} reason=${placementConflict.errorMsg}`);
        return {
            success: false,
    ...
    if (!spawnResult || !spawnResult.success) {
        log.warn(`[MobileObservatory] Launched observatory ${launchedItemID} but space spawn failed: ${spawnResult ? spawnResult.errorMsg : "UNKNOWN"}`);
    }
    registerMobileObservatoryItem(updatedResult.data);
    log.info(`[MobileObservatory] char=${context.characterID} launched observatory itemID=${launchedItemID} system=${context.systemID}`);
    return {
        success: true,
        data: {
...
//# sourceMappingURL=mobileObservatoryRuntime.js.map