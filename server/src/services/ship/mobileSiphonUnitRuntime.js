...
const { ITEM_FLAGS, findItemById, getItemMetadata, listContainerItems, removeInventoryItem, transferItemToOwnerLocation, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
...
function getSiphonTypeConfig(typeIDOrItem) {
    const typeID = toInt(typeIDOrItem && typeof typeIDOrItem === "object" ? typeIDOrItem.typeID : typeIDOrItem, 0);
...
function isMobileSiphonUnitType(typeIDOrItem) {
    if (typeIDOrItem && typeof typeIDOrItem === "object") {
        return (toInt(typeIDOrItem.categoryID, 0) === CATEGORY_DEPLOYABLE &&
            toInt(typeIDOrItem.groupID, 0) === GROUP_MOBILE_SIPHON_UNIT) || isMobileSiphonUnitType(toInt(typeIDOrItem.typeID, 0));
    }
...
function scheduleMobileSiphonUnitTimers(itemOrID) {
    ...
    if (state.expiresAtMs > 0) {
        ...
        timersBySiphonID.set(itemID, {
            expiryTimer: setUnrefTimeout(() => scheduleMobileSiphonUnitTimers(itemID), state.expiresAtMs - now),
        });
        return true;
    }
    return false;
...
function launchMobileSiphonUnitFromShip(session, itemID, options = {}) {
    ...
    if (!spawnResult || !spawnResult.success) {
        log.warn(`[SiphonUnit] Launched siphon ${launchedItemID} but space spawn failed: ${spawnResult ? spawnResult.errorMsg : "UNKNOWN"}`);
    }
    registerMobileSiphonUnitItem(updatedResult.data);
    log.info(`[SiphonUnit] char=${context.characterID} launched siphon itemID=${launchedItemID} system=${context.systemID}`);
    return {
        success: true,
        data: {
...
//# sourceMappingURL=mobileSiphonUnitRuntime.js.map