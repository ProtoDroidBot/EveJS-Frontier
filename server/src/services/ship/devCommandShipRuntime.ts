...
function fitGrantedItemTypeToShip(session, locationID, shipItem, itemType, count, options: Record<string, any> = {}) {
...
function moveGrantedItemTypeToShipCargo(session, locationID, shipItem, itemType, quantity, options: Record<string, any> = {}) {
...
function moveGrantedItemTypeToShipDroneBay(session, locationID, shipItem, itemType, quantity, options: Record<string, any> = {}) {
  return moveGrantedItemTypeToShipFlag(
    session,
    locationID,
    shipItem,
...
function stagePresetShipForLocation(session, preset, locationID, options: Record<string, any> = {}) {
...
function buildDockedResultMessage(preset, shipItem, fitSummaries: any[] = []) {
...
function buildSpaceResultMessage(preset, shipItem, fitSummaries: any[] = [], destroyResult = null) {
  const fitText = fitSummaries
    .map((entry) => {
      const forcedText = entry && entry.forcedCount > 0
        ? ` (${entry.forcedCount} forced)`
