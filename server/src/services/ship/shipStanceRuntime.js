...
const { ITEM_FLAGS, findShipItemById, grantItemToCharacterLocation, listContainerItems, removeInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const ALL_STANCE_MODIFIER_TYPE_IDS = Object.freeze([...new Set(Object.values(SHIP_STANCE_MODIFIERS_BY_TYPE_ID)
        .flatMap((mapping) => Object.values(mapping).map((typeID) => toInt(typeID, 0))))].sort((left, right) => left - right));
...
function getSessionCharacterID(session) {
    return toInt(session && (session.characterID ||
        session.charid ||
        session.charID), 0);
}
...
function listShipStanceModifierItems(ownerID, shipID) {
    return listContainerItems(toInt(ownerID, 0) > 0 ? toInt(ownerID, 0) : null, toInt(shipID, 0), ITEM_FLAGS.HIDDEN_MODIFIERS)
...
function resolveCurrentShipStance(shipOrID) {
    const shipItem = shipOrID && typeof shipOrID === "object"
        ? shipOrID
...
function syncInventoryChangesToSession(session, changes = []) {
    ...
    for (const change of changes) {
        ...
        syncInventoryItemForSession(session, item, change.previousState || change.previousData || {}, { emitCfgLocation: false });
    }
}
...
function getTransitionFileTime(spaceRuntime, session) {
    if (spaceRuntime &&
        ...
        return normalizeBigInt(spaceRuntime.getSimulationFileTimeForSession(session, currentFileTime()), currentFileTime());
    }
...
function replaceShipStanceModifierItem(shipItem, targetStanceID) {
    ...
    const createResult = grantItemToCharacterLocation(shipItem.ownerID, shipItem.itemID, ITEM_FLAGS.HIDDEN_MODIFIERS, { typeID: targetModifierTypeID }, 1, {
        singleton: 1,
...
function setShipStance(shipID, stanceID, session, options = {}) {
    ...
    const targetModifierTypeID = getModeTypeIDForStance(shipItem.typeID, targetStanceID);
    const hasExactlyTargetModifier = currentState.modifierItems.length === 1 &&
    ...
    const slimBroadcasted = broadcastShipStanceSlimChange(spaceRuntime, session, numericShipID, slimTuple);
    return {
        success: true,
        changed: true,
        shipID: numericShipID,
...
//# sourceMappingURL=shipStanceRuntime.js.map