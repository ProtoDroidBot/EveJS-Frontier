...
const { ITEM_FLAGS, grantItemToCharacterLocation, moveItemToLocation, } = require(path.join(__dirname, "../inventory/itemStore"));
const { resolveItemByTypeID, } = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const { getFittedModuleItems, getSlotFlagsForFamily, selectAutoFitFlagForType, validateFitForShip, } = require(path.join(__dirname, "../fitting/liveFittingState"));
...
function normalizeStarterShipFitting(entry, shipTypeID) {
    ...
    const normalizedShipTypeID = toInt(entry.shipTypeID, toInt(shipTypeID, 0));
...
function getStarterShipFitting(shipTypeID, options = {}) {
    ...
    if (typeof database.tableExists === "function" &&
        !database.tableExists(STARTER_SHIP_FITTINGS_TABLE)) {
        return null;
    }
    const result = database.read(STARTER_SHIP_FITTINGS_TABLE, `/${String(numericShipTypeID)}`);
    const normalizedFitting = result && result.success && result.data
...
function tryResolveTargetFlag(characterID, shipItem, fittedItems, itemType, entry) {
    ...
    if (explicitFlagID > 0) {
        const explicitValidation = validateFitForShip(numericCharacterID, shipItem, {
            itemID: -1,
            typeID: itemType.typeID,
            groupID: itemType.groupID,
            categoryID: itemType.categoryID,
            flagID: explicitFlagID,
        }, explicitFlagID, fittedItems);
    ...
    if (slotFamily) {
        const candidateFlags = getSlotFlagsForFamily(slotFamily, toInt(shipItem && shipItem.typeID, 0));
        for (const candidateFlagID of candidateFlags) {
            const validation = validateFitForShip(numericCharacterID, shipItem, {
                itemID: -1,
                typeID: itemType.typeID,
                groupID: itemType.groupID,
                categoryID: itemType.categoryID,
                flagID: candidateFlagID,
            }, candidateFlagID, fittedItems);
    ...
    const autoFlagID = toInt(selectAutoFitFlagForType(shipItem, fittedItems, itemType.typeID), 0);
    if (autoFlagID <= 0) {
        return {
            success: false,
    ...
    const autoValidation = validateFitForShip(numericCharacterID, shipItem, {
        itemID: -1,
        typeID: itemType.typeID,
        groupID: itemType.groupID,
        categoryID: itemType.categoryID,
        flagID: autoFlagID,
    }, autoFlagID, fittedItems);
...
function ensureStarterShipDefaultFit(characterID, dockedLocationID, shipItem, options = {}) {
    const numericCharacterID = toInt(characterID, 0);
    ...
    if (toInt(resolvedShipItem.flagID, ITEM_FLAGS.HANGAR) !== ITEM_FLAGS.HANGAR ||
    ...
    for (const entry of fitting.modules) {
        ...
        let existingCount = getFittedModuleItems(numericCharacterID, resolvedShipItem.itemID).filter((item) => toInt(item && item.typeID, 0) === typeID).length;
        while (existingCount < requiredQuantity) {
            const fittedItems = getFittedModuleItems(numericCharacterID, resolvedShipItem.itemID);
            const targetFlagResult = tryResolveTargetFlag(numericCharacterID, resolvedShipItem, fittedItems, itemType, entry);
            ...
            const grantResult = grantItemToCharacterLocation(numericCharacterID, numericDockedLocationID, ITEM_FLAGS.HANGAR, itemType, 1);
            ...
            const grantedItem = grantResult.data &&
            ...
            const moveResult = moveItemToLocation(grantedItem.itemID, resolvedShipItem.itemID, targetFlagResult.flagID);
    ...
    if (!success && options.logFailures !== false) {
        log.warn(`[StarterShipFit] Incomplete default fit for char=${numericCharacterID} ship=${toInt(resolvedShipItem.itemID, 0)} typeID=${toInt(resolvedShipItem.typeID, 0)} issues=${JSON.stringify(issues)}`);
    }
    return {
        success,
...
//# sourceMappingURL=starterShipFittingState.js.map