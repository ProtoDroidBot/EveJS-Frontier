...
const { buildConduitPassengerPlans, executeConduitPassengerJumps, resolveConduitJumpActivation, } = require(path.join(__dirname, "../_shared/conduitJumpRuntime"));
const { applyJumpTimersBestEffort, hasActiveJumpActivation, resolveJumpFatigueMultiplier, } = require(path.join(__dirname, "../_shared/jumpTimerRuntime"));
const sessionRegistry = require(path.join(__dirname, "../chat/sessionRegistry"));
const { flushPendingCommandSessionEffects, } = require(path.join(__dirname, "../chat/commandSessionEffects"));
const { getCynoJammerOnlineSimTime, isSolarSystemCynoJammed, } = require(path.join(__dirname, "../sovereignty/sovSuppressionState"));
const { TYPE_PHAROLUX_CYNO_BEACON, } = require(path.join(__dirname, "../sovereignty/sovUpgradeSupport"));
const fleetRuntime = require(path.join(__dirname, "../fleets/fleetRuntime"));
const { resolveFormationSettings, computeFleetWarpFormationPoints, } = require(path.join(__dirname, "../fleets/fleetWarpFormation.js"));
const structureState = require(path.join(__dirname, "../structure/structureState"));
const { STRUCTURE_SERVICE_ID, STRUCTURE_SERVICE_STATE, } = require(path.join(__dirname, "../structure/structureConstants"));
...
const { buildBookmarkReplyTuple, } = require(path.join(__dirname, "../bookmark/bookmarkPayloads"));
const { buildDestinyConfigurationSettings, } = require(path.join(__dirname, "destinyCompatibility"));
const { resolveLocationBookmarkTarget, } = require(path.join(__dirname, "../bookmark/bookmarkTargetResolver"));
...
const { normalizeDungeonInstanceID, } = require(path.join(__dirname, "../dungeon/dungeonVisibilityPolicy"));
const operationSiteRuntime = require(path.join(__dirname, "../dungeon/operationSiteRuntime"));
...
const SHIP_IMMOBILE_WARP_BLOCKED_MESSAGE = "Your ship cannot warp while an active module is preventing movement.";
...
function buildDockingApproachUserErrorValues(dockingDebug = null) {
    const distance = Math.max(0, Math.round(Number(dockingDebug && dockingDebug.dockingDistance) || 0));
    return { distance };
}
...
function extractDungeonInstanceID(rawTarget) {
    const numericTarget = normalizeDungeonInstanceID(rawTarget);
    ...
    for (const candidate of candidates) {
        const numericCandidate = normalizeDungeonInstanceID(candidate);
...
function resolveBookmarkAnyStore(session, bookmarkID) {
    ...
    if (isBookmarkDelayed(bookmark, charID)) {
        log.info(`[Beyonce] Bookmark ${bookmarkID} is still in the shared delay window for char=${charID}`);
        return null;
    }
    return bookmark;
}
function hasBeyonceViewRole(session) {
    ...
    const roleMask = normalizeRoleValue(session.role ?? session.accountRole ?? session.accountRoles, 0n);
...
function resolveBookmarkTargetForSession(session, bookmarkID, options = {}) {
    const bookmarkInfo = bookmarkRuntime.getBookmarkForCharacter(session && session.characterID, bookmarkID, options);
    if (!bookmarkInfo) {
        return null;
    }
...
function getSessionSolarSystemID(session) {
    return normalizeNumber(session &&
...
function vectorMagnitude(vector) {
    return Math.sqrt((toFiniteNumber(vector && vector.x, 0) ** 2) +
...
function throwIfPilotWarpLandingPending(session, message) {
    if (typeof spaceRuntime.isPilotWarpLandingPending === "function" &&
        spaceRuntime.isPilotWarpLandingPending(session)) {
        throwCynoNotify(message);
    }
}
function throwIfJumpActivationActive(session) {
    if (hasActiveJumpActivation(session && session.characterID)) {
        throwCynoNotify("You cannot use a jump drive, jump bridge, or jump portal while your jump activation cooldown is active.");
    }
}
function getSolarSystemDistanceLy(sourceSolarSystemID, destinationSolarSystemID) {
    ...
    const distanceMeters = vectorMagnitude(subtractVectors(sourceSystem.position, destinationSystem.position));
...
function getEntitySurfaceDistanceMeters(leftEntity, rightEntity) {
    ...
    const centerDistance = vectorMagnitude(subtractVectors(cloneVector(leftEntity.position), cloneVector(rightEntity.position)));
...
function isDeployableCynoAnchorType(typeID) {
    const normalizedTypeID = normalizePositiveInt(typeID, 0);
    return (normalizedTypeID === TYPE_MOBILE_CYNOSURAL_FIELD ||
        isMobileCynoBeaconDeployableType(normalizedTypeID));
}
function resolveJumpPortalMassFactor(moduleItem, moduleAttributes) {
    const dogmaValue = toFiniteNumber(moduleAttributes && moduleAttributes[ATTRIBUTE_JUMP_PORTAL_CONSUMPTION_MASS_FACTOR], 0);
    if (dogmaValue > 0) {
        return dogmaValue;
    }
...
function resolveBridgePortalModule(bridgeCharacterID, bridgeShip, fieldTypeID) {
    ...
    const requiredPassengerAttributeID = getRequiredPortalPassengerAttributeForField(fieldTypeID);
    ...
    for (const fittedItem of fittedItems) {
        ...
        const modulePassengerAttributeID = normalizePositiveInt(moduleAttributes[ATTRIBUTE_JUMP_PORTAL_PASSENGER_REQUIRED_ATTRIBUTE_ID], 0);
        if (modulePassengerAttributeID !== requiredPassengerAttributeID) {
            continue;
        }
        const portalMassFactor = resolveJumpPortalMassFactor(fittedItem, moduleAttributes);
        if (portalMassFactor <= 0) {
            throwCynoNotify("That jump portal generator is missing its portal fuel mass factor.");
        }
        return {
            bridgeResourceState,
            moduleAttributes,
            ...
            additionalConsumption: Math.max(0, toFiniteNumber(moduleAttributes[ATTRIBUTE_JUMP_PORTAL_ADDITIONAL_CONSUMPTION], 0)),
        };
    }
    throwCynoNotify("Your ship is not fitted with an online jump portal generator for that cynosural field.");
}
...
function resolveShipRadiusMeters(shipItem, shipEntity = null) {
    const directRadius = Math.max(0, toFiniteNumber(shipEntity && shipEntity.radius, toFiniteNumber(shipItem && (shipItem.radius || shipItem.spaceRadius), 0)));
    if (directRadius > 0) {
        return directRadius;
    }
    const movement = worldData.getMovementAttributesForType(normalizePositiveInt(shipItem && (shipItem.typeID || shipItem.shipTypeID), 0));
...
function buildFleetCynoArrivalSpawnState(fieldEntity, shipItem, shipEntity = null, options = {}) {
    ...
    const direction = normalizeVector(vectorMagnitude(anchorPosition) > 0 ? anchorPosition : { x: 1, y: 0, z: 0 }, { x: 1, y: 0, z: 0 });
    const offset = Math.max(0, toFiniteNumber(fieldEntity && fieldEntity.radius, 0)) +
    ...
    return {
        ...
        anchorID: normalizePositiveInt(fieldEntity && (fieldEntity.itemID || fieldEntity.structureID), 0),
        anchorName: String((fieldEntity && (fieldEntity.itemName || fieldEntity.name)) ||
...
function resolveActiveShipAndJumpDriveForCyno(session, destinationSolarSystemID, fieldTypeID) {
    ...
    const attributes = resourceState && resourceState.attributes && typeof resourceState.attributes === "object"
        ? resourceState.attributes
        : {};
    const jumpRangeLy = toFiniteNumber(attributes[ATTRIBUTE_JUMP_DRIVE_RANGE], 0);
    const fuelTypeID = normalizePositiveInt(attributes[ATTRIBUTE_JUMP_DRIVE_CONSUMPTION_TYPE], 0);
    const fuelAmountPerLy = toFiniteNumber(attributes[ATTRIBUTE_JUMP_DRIVE_CONSUMPTION_AMOUNT], 0);
    if (jumpRangeLy <= 0 || fuelTypeID <= 0 || fuelAmountPerLy <= 0) {
        throwCynoNotify("Your ship is not capable of jumping to a cynosural beacon.");
    }
    if (fieldTypeRequiresCovertCompatibility(fieldTypeID) &&
        toFiniteNumber(attributes[ATTRIBUTE_USES_COVERT_CYNO], 0) <= 0) {
        throwCynoNotify("Your ship cannot jump to covert cynosural fields.");
    }
    if (fieldTypeRequiresIndustrialCompatibility(fieldTypeID) &&
...
function consumeJumpFuel(session, activeShip, shipEntity, fuelTypeID, fuelQuantity) {
    ...
    const fuelResult = consumeFuelFromShipStorage(fuelEntity, fuelTypeID, quantity, {
...
function calculateBridgeFuelQuantity({ distanceLy, passengerMass, portalMassFactor, fuelAmountPerLy, additionalConsumption, }) {
    const rawQuantity = Math.max(0, toFiniteNumber(distanceLy, 0)) *
...
function isStructureCynoBeaconServiceOnline(structure) {
    return normalizeInt(structure &&
        structure.serviceStates &&
        structure.serviceStates[String(STRUCTURE_SERVICE_ID.CYNO_BEACON)], STRUCTURE_SERVICE_STATE.OFFLINE) === STRUCTURE_SERVICE_STATE.ONLINE;
}
function buildStructureCynoAnchorEntity(structure, fallbackEntity = null) {
    return {
        ...
        itemID: normalizePositiveInt(structure && (structure.structureID || structure.itemID), normalizePositiveInt(fallbackEntity && fallbackEntity.itemID, 0)),
        structureID: normalizePositiveInt(structure && (structure.structureID || structure.itemID), normalizePositiveInt(fallbackEntity && fallbackEntity.structureID, 0)),
        typeID: normalizePositiveInt(structure && structure.typeID, normalizePositiveInt(fallbackEntity && fallbackEntity.typeID, TYPE_PHAROLUX_CYNO_BEACON)),
        ...
        name: (structure && (structure.name || structure.itemName)) ||
        ...
        radius: Math.max(0, toFiniteNumber(structure && structure.radius, toFiniteNumber(fallbackEntity && fallbackEntity.radius, 0))),
    };
}
function resolveFleetBridgeDestination(destinationSolarSystemID, beaconID) {
    ...
    if (isPharoluxStructure(structure)) {
        ...
        if (structureSolarSystemID !== normalizePositiveInt(destinationSolarSystemID, 0)) {
...
function resolveFleetDeployableBeacon(args, session) {
    ...
    const fieldTypeID = resolveDeployableBeaconFieldTypeID(deployableEntity, fieldEntity || deployableEntity);
    if (isSolarSystemCynoJammed(destinationSolarSystemID) &&
        fieldTypeID !== TYPE_COVERT_CYNOSURAL_FIELD) {
        throwCynoNotify("That solar system is cynosural jammed.");
    }
...
function resolveFleetBridgePassengerJump(args, session) {
    ...
    const bridge = fleetRuntime.getActiveBridgeForCharacter(session && session.characterID, otherShipID);
    const destinationSolarSystemID = normalizePositiveInt(Array.isArray(bridge) ? bridge[0] : null, 0);
    ...
    const bridgeDestination = resolveFleetBridgeDestination(destinationSolarSystemID, beaconID);
    ...
    const bridgeOwnerID = normalizePositiveInt(bridgeEntity.ownerID || bridgeEntity.charID || otherCharID, otherCharID);
    ...
    const bridgeDistanceMeters = getEntitySurfaceDistanceMeters(passengerEntity, bridgeEntity);
    ...
    const passengerResourceState = buildShipResourceState(session && session.characterID, passengerShip);
    ...
    const fuelTypeID = normalizePositiveInt(bridgeAttributes[ATTRIBUTE_JUMP_DRIVE_CONSUMPTION_TYPE], 0);
    const fuelAmountPerLy = toFiniteNumber(bridgeAttributes[ATTRIBUTE_JUMP_DRIVE_CONSUMPTION_AMOUNT], 0);
    if (fuelTypeID <= 0 || fuelAmountPerLy <= 0) {
        throwCynoNotify("The bridge ship does not have valid jump portal fuel data.");
    }
    const passengerMass = Math.max(0, toFiniteNumber(passengerAttributes[ATTRIBUTE_MASS], 0), toFiniteNumber(passengerEntity.mass, 0), toFiniteNumber(passengerShip.mass, 0));
    if (passengerMass <= 0) {
...
function resolveOperationDungeonWarpPoint(session, siteID) {
    ...
    const record = operationSiteRuntime.resolveOperationSpawnpointRecord(getSessionCharacterID(session), numericSiteID);
...
function ensureUniverseSiteContentsMaterialized(scene, siteOrEntity, options = {}) {
    ...
    try {
        ...
        log.warn(`[Beyonce] Failed to materialize universe site contents in system=${normalizeNumber(scene && scene.systemID, 0)}: ${error.message}`);
        return null;
    }
}
function maybeMaterializeMissionBookmarkTarget(session, bookmarkTarget) {
    const metadata = bookmarkTarget &&
...
function maybeMaterializeUniverseSiteInstanceTarget(session, instanceID) {
    const numericInstanceID = normalizeDungeonInstanceID(instanceID);
    if (numericInstanceID === null) {
        return null;
    }
    const scene = spaceRuntime.getSceneForSession(session);
    if (!scene) {
    ...
    const siteID = normalizeNumber(materialized.data && materialized.data.siteID, normalizeNumber(instance && instance.metadata && instance.metadata.siteID, 0));
    if (siteID <= 0) {
        return null;
    }
...
function resolveScanWarpTarget(session, targetID) {
    ...
    const entityID = normalizeNumber(site.itemID || site.endpointID || site.siteID, 0);
    if (entityID > 0) {
        const entity = scene && typeof scene.getEntityByID === "function"
        ...
        if (entity) {
            return {
                kind: "entity",
                entityID,
                instanceID: normalizeDungeonInstanceID(site.instanceID || site.dungeonSiteInstanceID),
                site,
            };
        }
    }
...
function resolveFleetMemberWarpTarget(session, targetCharacterID) {
    const sourceCharacterID = normalizeNumber(session && (session.characterID || session.charID || session.charid), 0);
    ...
    const targetSession = sessionRegistry.findSessionByCharacterID(numericTargetCharacterID);
    if (!targetSession) {
        return null;
    }
    const sourceSystemID = normalizeNumber(session && (session.solarsystemid2 || session.solarsystemid), 0);
    const targetSystemID = normalizeNumber(targetSession.solarsystemid2 || targetSession.solarsystemid, 0);
    if (sourceSystemID > 0 &&
        targetSystemID > 0 &&
    ...
    const targetShipEntity = targetScene && typeof targetScene.getShipEntityForSession === "function"
        ? targetScene.getShipEntityForSession(targetSession)
        : null;
    const targetShipID = normalizeNumber(targetShipEntity && targetShipEntity.itemID, normalizeNumber(targetSession.shipID || targetSession.shipid, 0));
    const destinationStaticInstanceID = normalizeDungeonInstanceID(targetShipEntity && targetShipEntity.dungeonCurrentInstanceID);
    const destinationDungeonRoomKey = String(targetShipEntity && targetShipEntity.dungeonCurrentRoomKey || "").trim() || null;
    const destinationDungeonSiteID = normalizePersistentEntityID(targetShipEntity && targetShipEntity.dungeonCurrentSiteID);
    if (targetShipID > 0) {
        return {
...
class BeyonceService extends BaseService {
    ...
    _throwThrottleGeneric(remainingMs) {
        throwWrappedUserError("ThrottleGeneric", {
            remainingTime: Math.max(1, Math.ceil(Math.max(0, Number(remainingMs) || 0) * EVE_TIME_TICKS_PER_MS)),
        });
    }
    ...
    _throwWarpFailureUserError(errorMsg = "") {
        switch (String(errorMsg || "").trim()) {
            ...
            case "WARP_SCRAMBLED":
                ...
                break;
            case "WARP_BLOCKED_BY_CLOAK":
                throwWrappedUserError("CustomNotify", {
                    notify: "You cannot warp while cloaked. Deactivate your cloaking device first.",
                });
                break;
            case "SHIP_IMMOBILE":
                throwWrappedUserError("CustomNotify", {
    ...
    Handle_GetDestinyConfigurationSettings(args, session) {
        void args;
        return buildDestinyConfigurationSettings(session && session.compatibilityProfile);
    }
    ...
    Handle_AdminGetBubbleID(args, session) {
        ...
        if (!hasBeyonceViewRole(session)) {
            log.warn(`[Beyonce] AdminGetBubbleID denied for non-view session userId=${session && session.userid}`);
            return null;
        }
        const bubbleID = spaceRuntime.getBubbleIDForEntity(session, itemID);
        log.debug(`[Beyonce] AdminGetBubbleID item=${itemID} -> ${bubbleID == null ? "null" : bubbleID}`);
        return bubbleID;
    }
    ...
    _jumpToFleetModuleBeacon(args, session, options = {}) {
        throwIfPilotWarpLandingPending(session, "The fleet cynosural beacon session change failed.");
        const { beaconID, beaconOwnerCharID, destinationSolarSystemID, fieldEntity, fieldTypeID, } = resolveFleetModuleBeacon(args, session);
        const { activeShip, distanceLy, fuelTypeID, fuelQuantity, resourceState, } = resolveActiveShipAndJumpDriveForCyno(session, destinationSolarSystemID, fieldTypeID);
        ...
        if (options.groupAnchor === true) {
            conduitActivation = resolveConduitJumpActivation(resourceState, distanceLy);
            ...
            conduitPlan = buildConduitPassengerPlans({
                ...
                destinationSolarSystemID,
                requiredPassengerAttributeID: conduitActivation.requiredPassengerAttributeID,
                passengerLimit: conduitActivation.passengerLimit,
                buildSpawnState: ({ passengerEntity, passengerShip }) => buildFleetCynoArrivalSpawnState(fieldEntity, passengerShip, passengerEntity),
            });
            if (!conduitPlan.success) {
                throwConduitAnchorFailure(conduitPlan);
            }
        }
        const fuelResult = consumeJumpFuel(session, activeShip, shipEntity, fuelTypeID, consumedFuelQuantity);
        const jumpResult = jumpSessionToSolarSystem(session, destinationSolarSystemID, {
            spawnStateOverride: buildFleetCynoArrivalSpawnState(fieldEntity, activeShip, shipEntity),
        });
        if (!jumpResult.success) {
            log.warn(`[Beyonce] fleet module cyno jump failed char=${session && session.characterID} ` +
        ...
        if (options.groupAnchor === true) {
            ...
            notifyGroupJumpAnchor(session, destinationSolarSystemID, conduitPassengerResult.successCount);
        }
        log.info(`[Beyonce] char=${session && session.characterID} jumped fleetModuleCyno ` +
    ...
    _jumpToFleetDeployableBeacon(args, session, options = {}) {
        throwIfPilotWarpLandingPending(session, "The fleet deployable cynosural beacon session change failed.");
        const { beaconID, deployableID, destinationSolarSystemID, fieldEntity, fieldTypeID, fleet, ownerID, } = resolveFleetDeployableBeacon(args, session);
        const { activeShip, distanceLy, fuelTypeID, fuelQuantity, resourceState, } = resolveActiveShipAndJumpDriveForCyno(session, destinationSolarSystemID, fieldTypeID);
        const shipEntity = spaceRuntime.getEntity(session, activeShip.itemID);
        throwIfJumpActivationActive(session);
        let consumedFuelQuantity = fuelQuantity;
        ...
        if (options.groupAnchor === true) {
            ...
            conduitPlan = buildConduitPassengerPlans({
                ...
                requiredPassengerAttributeID: conduitActivation.requiredPassengerAttributeID,
                passengerLimit: conduitActivation.passengerLimit,
                buildSpawnState: ({ passengerEntity, passengerShip }) => buildFleetCynoArrivalSpawnState(fieldEntity, passengerShip, passengerEntity, spawnOptions),
            });
            if (!conduitPlan.success) {
                throwConduitAnchorFailure(conduitPlan);
            }
        ...
        const fuelResult = consumeJumpFuel(session, activeShip, shipEntity, fuelTypeID, consumedFuelQuantity);
        const jumpResult = jumpSessionToSolarSystem(session, destinationSolarSystemID, {
            spawnStateOverride: buildFleetCynoArrivalSpawnState(fieldEntity, activeShip, shipEntity, spawnOptions),
        });
        if (!jumpResult.success) {
            log.warn(`[Beyonce] fleet deployable cyno jump failed char=${session && session.characterID} ` +
        ...
        log.info(`[Beyonce] char=${session && session.characterID} jumped fleetDeployableCyno ` +
    ...
    Handle_CmdBridgeToFleetModuleBeacon(args, session) {
        const { beaconID, beaconOwnerCharID, destinationSolarSystemID, fieldTypeID, } = resolveFleetModuleBeacon(args, session);
        const { activeShip, distanceLy, fuelTypeID, fuelQuantity, } = resolveActiveShipAndJumpDriveForCyno(session, destinationSolarSystemID, fieldTypeID);
        const portal = resolveBridgePortalModule(session && session.characterID, activeShip, fieldTypeID);
        const activationResult = spaceRuntime.activateGenericModule(session, portal.portalModuleItem, getBridgePortalEffectName(portal.portalModuleItem), {
            repeat: 1,
        });
        ...
        const effectState = activationResult.data && activationResult.data.effectState
            ? activationResult.data.effectState
            : null;
        if (effectState) {
            effectState.autoDeactivateAtCycleEnd = true;
            effectState.consequenceDelivery = "managed";
        ...
        fleetRuntime.setBridgeMode(session.fleetid, activeShip.itemID, destinationSolarSystemID, beaconID, true);
        notifyGroupJumpAnchor(session, destinationSolarSystemID, 0);
        log.info(`[Beyonce] char=${session && session.characterID} opened fleet module bridge ` +
    ...
    Handle_CmdJumpThroughFleet(args, session) {
        throwIfPilotWarpLandingPending(session, "The fleet bridge session change failed.");
        const bridgeJump = resolveFleetBridgePassengerJump(args, session);
        throwIfJumpActivationActive(session);
        const fuelResult = consumeBridgeFuel(bridgeJump.bridgeOwnerCharID, bridgeJump.bridgeShip, bridgeJump.bridgeEntity, bridgeJump.fuelTypeID, bridgeJump.fuelQuantity);
        const jumpResult = jumpSessionToSolarSystem(session, bridgeJump.destinationSolarSystemID, {
            spawnStateOverride: buildFleetCynoArrivalSpawnState(bridgeJump.fieldEntity, bridgeJump.passengerShip, bridgeJump.passengerEntity, {
                anchorType: bridgeJump.bridgeDestination.anchorType,
                fallbackName: bridgeJump.bridgeDestination.anchorFallbackName,
            }),
        });
        if (!jumpResult.success) {
            log.warn(`[Beyonce] fleet bridge passenger jump failed char=${session && session.characterID} ` +
        ...
        log.info(`[Beyonce] char=${session && session.characterID} jumped through fleet bridge ` +
    ...
    Handle_CmdBridgeToFleetDeployableBeacon(args, session) {
        const { beaconID, deployableEntity, deployableID, destinationSolarSystemID, fieldTypeID, ownerID, } = resolveFleetDeployableBeacon(args, session);
        const { activeShip, distanceLy, fuelTypeID, fuelQuantity, } = resolveActiveShipAndJumpDriveForCyno(session, destinationSolarSystemID, fieldTypeID);
        const portal = resolveBridgePortalModule(session && session.characterID, activeShip, fieldTypeID);
        const activationResult = spaceRuntime.activateGenericModule(session, portal.portalModuleItem, getBridgePortalEffectName(portal.portalModuleItem), {
            repeat: 1,
        ...
        fleetRuntime.setBridgeMode(session.fleetid, activeShip.itemID, destinationSolarSystemID, bridgeItemID, true);
        notifyGroupJumpAnchor(session, destinationSolarSystemID, 0);
        log.info(`[Beyonce] char=${session && session.characterID} opened fleet deployable bridge ` +
    ...
    Handle_CallComponentFromClient(args, session, kwargs) {
        ...
        if (componentName === "assemblyConstruction") {
            if (methodName === "get_deposited_items_by_type") {
                const result = frontierDeploymentRuntime.getDepositedItemsByType(session, itemID);
                if (!result || result.success !== true) {
                    throwWrappedUserError("CustomNotify", {
                        notify: `Construction inventory request failed: ${result && result.errorMsg || "UNKNOWN"}`,
                    });
                }
                return buildDict(Object.entries(result.data || {}).map(([typeID, quantity]) => ([
                    normalizeNumber(typeID, 0),
                    normalizeNumber(quantity, 0),
                ])));
            }
            if (methodName === "deposit_items") {
                const inventoryID = normalizeNumber(getKwargValue(componentKwargs, "inventory_id") ?? componentArgs[0], 0);
                const quantities = getKwargValue(componentKwargs, "qty_by_type_id") ?? componentArgs[1];
                const result = frontierDeploymentRuntime.depositItems(session, itemID, inventoryID, quantities);
                if (!result || result.success !== true) {
                    throwWrappedUserError("CustomNotify", {
        ...
        if (componentName === "microJumpDriver" &&
            methodName === "StartMicroJumpDriveForShip") {
            const result = mobileMicroJumpUnitRuntime.startMobileMicroJumpDriveForShip(session, itemID);
            if (!result || result.success !== true) {
                log.info(`[Beyonce] CallComponentFromClient rejected component=${componentName} method=${methodName} item=${itemID} char=${session && session.characterID} reason=${result ? result.errorMsg : "UNKNOWN"}`);
            }
            return null;
        }
        if (componentName === "linkWithShip" &&
            (methodName === "InitiateLink" || methodName === "InitiateLinkQuick")) {
            const result = mobileAnalysisBeaconRuntime.initiateMobileAnalysisBeaconLink(session, itemID, { quick: methodName === "InitiateLinkQuick" });
            if (!result || result.success !== true) {
                log.info(`[Beyonce] CallComponentFromClient rejected component=${componentName} method=${methodName} item=${itemID} char=${session && session.characterID} reason=${result ? result.errorMsg : "UNKNOWN"}`);
            }
            return null;
        }
        log.info(`[Beyonce] Unsupported CallComponentFromClient component=${componentName} method=${methodName} item=${itemID} char=${session && session.characterID}`);
        return null;
    }
    Handle_CmdGotoDirection(args, session) {
        ...
        log.info(`[Beyonce] CmdGotoDirection char=${session && session.characterID} dir=(${x}, ${y}, ${z})`);
    ...
    Handle_CmdSteerDirection(args, session) {
        ...
        log.info(`[Beyonce] CmdSteerDirection char=${session && session.characterID} dir=(${x}, ${y}, ${z})`);
        spaceRuntime.gotoDirection(session, { x, y, z }, {
    ...
    Handle_CmdGotoPoint(args, session) {
        ...
        log.info(`[Beyonce] CmdGotoPoint char=${session && session.characterID} point=(${x}, ${y}, ${z})`);
    ...
    Handle_CmdGotoBookmark(args, session) {
        ...
        const bookmark = resolveBookmarkAnyStore(session, bookmarkID);
        log.info(`[Beyonce] CmdGotoBookmark char=${session && session.characterID} bookmark=${bookmarkID}`);
        if (!bookmark) {
    ...
    Handle_CmdAlignTo(args, session, kwargs) {
        ...
        const targetID = positionalTargetID ||
            kwargTargetID ||
            resolveBookmarkAlignTarget(session, bookmarkID);
        log.info(`[Beyonce] CmdAlignTo char=${session && session.characterID} target=${targetID} bookmark=${bookmarkID}`);
        if (targetID > 0) {
    ...
    Handle_CmdOrbit(args, session) {
        ...
        log.info(`[Beyonce] CmdOrbit char=${session && session.characterID} target=${targetID} range=${range}`);
    ...
    Handle_CmdSetSpeedFraction(args, session) {
        ...
        log.info(`[Beyonce] CmdSetSpeedFraction char=${session && session.characterID} fraction=${fraction}`);
        spaceRuntime.setSpeedFraction(session, fraction);
        return null;
    }
    Handle_CmdSetPitch(args, session) {
        const pitch = normalizeNumber(args && args[0], 0);
        log.info(`[Beyonce] CmdSetPitch char=${session && session.characterID} pitch=${pitch}`);
    ...
    Handle_CmdSetStrafingThrust(args, session) {
        ...
        const thrust = Array.isArray(requested)
            ...
            : [0, 0, 0];
        log.info(`[Beyonce] CmdSetStrafingThrust char=${session && session.characterID} thrust=(${thrust.join(", ")}) enabled=${config.strafeEnabled !== false}`);
    ...
    Handle_CmdSetYawRate(args, session) {
        ...
        log.info(`[Beyonce] CmdSetYawRate char=${session && session.characterID} yawRate=${yawRate}`);
    ...
    Handle_CmdAbandonLoot(args, session) {
        const itemIDs = args && Array.isArray(args[0])
            ? args[0]
            : args && args[0] !== null && args[0] !== undefined
                ? [args[0]]
                : [];
        log.info(`[Beyonce] CmdAbandonLoot char=${session && session.characterID} count=${itemIDs.length}`);
    ...
    Handle_CmdFleetTagTarget(args, session) {
        ...
        log.info(`[Beyonce] CmdFleetTagTarget char=${session && session.characterID} item=${itemID} tag=${tag === null || tag === undefined ? "<clear>" : String(tag)}`);
    ...
    Handle_CmdWarpToStuff(args, session, kwargs) {
        ...
        log.info(`[Beyonce] CmdWarpToStuff char=${session && session.characterID} type=${warpType} target=${numericTarget || rawTarget} minRange=${minimumRange}${fleetWarpRequested ? " fleet=1" : ""}`);
        let result = null;
        if (warpType === "scan" && rawTarget !== null && rawTarget !== undefined) {
            ...
            if (scanTarget && scanTarget.kind === "entity") {
                result = spaceRuntime.warpToEntity(session, scanTarget.entityID, {
                    minimumRange,
                    ...(scanTarget.instanceID !== null
                        ? { destinationStaticInstanceID: scanTarget.instanceID }
                        : {}),
                });
            }
            ...
            const launch = planetRuntimeStore.getLaunch(numericTarget, session && session.characterID);
            if (launch) {
            ...
            const materializedInstanceTarget = maybeMaterializeUniverseSiteInstanceTarget(session, dungeonInstanceID);
            if (materializedInstanceTarget) {
                result = spaceRuntime.warpToEntity(session, materializedInstanceTarget.entityID, {
                    minimumRange,
                    destinationStaticInstanceID: materializedInstanceTarget.instanceID,
                });
            }
            else {
                result = {
            ...
            if (fleetMemberTarget && fleetMemberTarget.kind === "entity") {
                result = spaceRuntime.warpToEntity(session, fleetMemberTarget.entityID, {
                    minimumRange,
                    ...(fleetMemberTarget.destinationStaticInstanceID !== null
                        ? {
                            ...(fleetMemberTarget.destinationDungeonRoomKey
                                ? {
                                    destinationDungeonRoomKey: fleetMemberTarget.destinationDungeonRoomKey,
                                }
                                : {}),
                            ...(fleetMemberTarget.destinationDungeonSiteID !== null
                                ? {
                                    destinationDungeonSiteID: fleetMemberTarget.destinationDungeonSiteID,
                                }
                                : {}),
                            destinationStaticInstanceID: fleetMemberTarget.destinationStaticInstanceID,
                        }
                        : {}),
                });
            }
        ...
        else if ((warpType === "item" || warpType === "deployable" || !warpType) &&
            numericTarget > 0) {
            const scene = spaceRuntime.getSceneForSession(session);
            const entity = scene && typeof scene.getEntityByID === "function"
            ...
            const destinationStaticInstanceID = normalizeDungeonInstanceID(materializedInstanceTarget
                ? materializedInstanceTarget.instanceID
                : entity && (entity.dungeonSiteInstanceID ||
                    entity.airNpeInstanceID ||
                    (entity.kind === "ship"
                        ? entity.dungeonCurrentInstanceID
                        : null)));
            const destinationDungeonRoomKey = destinationStaticInstanceID !== null &&
                entity && entity.kind === "ship"
                ? String(entity.dungeonCurrentRoomKey || "").trim() || null
                : null;
            const destinationDungeonSiteID = destinationStaticInstanceID !== null
                ? normalizePersistentEntityID(entity && entity.kind === "ship"
                    ? entity.dungeonCurrentSiteID
                    : entity && entity.dungeonSiteID)
                : null;
            result = spaceRuntime.warpToEntity(session, materializedInstanceTarget
            ...
            const bookmarkMetadata = bookmark.metadata && typeof bookmark.metadata === "object"
                ? bookmark.metadata
                : {};
            const missionInstanceID = normalizeNumber(bookmarkMetadata.missionInstanceID, 0);
            const missionMaterialization = missionInstanceID > 0
                ? maybeMaterializeMissionBookmarkTarget(session, bookmark)
                : null;
            const scopedBookmarkFailed = Boolean(missionInstanceID > 0 &&
                (!missionMaterialization || missionMaterialization.success !== true));
            const destinationDungeonOptions = missionInstanceID > 0
                ? { destinationStaticInstanceID: missionInstanceID }
                : {};
            const materializedSiteID = normalizeNumber(missionMaterialization &&
                missionMaterialization.data &&
                missionMaterialization.data.siteID, 0);
            const bookmarkItemID = materializedSiteID ||
                normalizeNumber(bookmark.itemID, 0);
            if (scopedBookmarkFailed) {
                result = {
                    success: false,
            ...
            if (bookmarkItemID > 0) {
                if (!scopedBookmarkFailed) {
                    result = spaceRuntime.warpToEntity(session, bookmarkItemID, {
                        ...destinationDungeonOptions,
                        minimumRange,
                    });
                }
                if ((!result || !result.success) &&
                    !scopedBookmarkFailed &&
        ...
        if (!result || !result.success) {
            log.warn(`[Beyonce] CmdWarpToStuff failed for char=${session && session.characterID}: ${(result && result.errorMsg) || "UNKNOWN_ERROR"}`);
    ...
    _warpFleetFollowers(commanderSession, leaderResult, options = {}) {
        const commanderCharID = normalizeNumber(commanderSession &&
            (commanderSession.characterID || commanderSession.charID), 0);
        if (commanderCharID <= 0) {
            return 0;
        }
        const pendingWarp = leaderResult && leaderResult.data && typeof leaderResult.data === "object"
            ? leaderResult.data
            : null;
        const targetEntityID = pendingWarp
            ? normalizeNumber(pendingWarp.targetEntityID, 0)
            : 0;
        const rawDestination = pendingWarp &&
        ...
        const stopDistance = pendingWarp
            ...
            : 0;
        const destinationDungeonOptions = pendingWarp
            ? {
                ...(pendingWarp.destinationStaticInstanceID !== null &&
                    pendingWarp.destinationStaticInstanceID !== undefined
                    ? {
                        destinationStaticInstanceID: pendingWarp.destinationStaticInstanceID,
                    }
                    : {}),
                ...(String(pendingWarp.destinationDungeonRoomKey || "").trim()
                    ? {
                        destinationDungeonRoomKey: String(pendingWarp.destinationDungeonRoomKey).trim(),
                    }
                    : {}),
        ...
        const commanderSystemID = normalizeNumber(commanderSession &&
            (commanderSession.solarsystemid2 || commanderSession.solarsystemid), 0);
        let followers = [];
        try {
            followers = fleetRuntime.collectFleetWarpFollowers(commanderCharID) || [];
        }
        catch (error) {
            log.warn(`[Beyonce] Fleet warp follower lookup failed for char=${commanderCharID}: ${error.message}`);
            return 0;
        }
        ...
        const eligibleFollowers = followers.filter((follower) => {
            ...
            const followerSystemID = normalizeNumber(followerSession.solarsystemid2 || followerSession.solarsystemid, 0);
            return !(commanderSystemID > 0 &&
                followerSystemID > 0 &&
                followerSystemID !== commanderSystemID);
        });
        const formationPoints = this._resolveFleetWarpFormationPoints(commanderSession, eligibleFollowers, { pendingWarp, formationSettings: options.formationSettings });
        const commanderGeneration = commanderSession && commanderSession._space;
        const associationToken = leaderResult &&
        ...
        for (const follower of eligibleFollowers) {
            ...
            const followerGeneration = followerSession && followerSession._space;
            const formationPoint = formationPoints.get(normalizeNumber(follower.characterID, 0));
            ...
            if (formationPoint) {
                ...
                followerResult = spaceRuntime.warpToEntity(followerSession, targetEntityID, { ...destinationDungeonOptions, minimumRange });
            }
            ...
            if (followerResult && followerResult.success) {
                warpedCount += 1;
                if (associationToken && followerResult.redundant !== true) {
                    try {
                        spaceRuntime.associateSuccessfulFleetWarpFollower(commanderSession, followerSession, associationToken, {
                            leaderGeneration: commanderGeneration,
                            leaderResult,
                            followerGeneration,
                            followerResult,
                        });
                        ...
                        log.warn(`[Beyonce] Fleet warp association failed for char=${normalizeNumber(follower && follower.characterID, 0)}: ${error.message}`);
                    }
                }
        ...
        if (warpedCount > 0) {
            log.info(`[Beyonce] Fleet warp pulled ${warpedCount} member(s) for commander char=${commanderCharID}` +
    ...
    _resolveFleetWarpFormationPoints(commanderSession, followers, context = {}) {
        ...
        const getShipEntity = scene && typeof scene.getShipEntityForSession === "function"
    ...
    _notifyFleetWarpRemoteMessage(session, celestialID) {
        ...
        const warpInfoDict = {
            type: "dict",
            entries: normalizeNumber(celestialID, 0) > 0
        ...
        try {
            ...
            log.warn(`[Beyonce] Fleet warp remote message failed for char=${session && session.characterID}: ${error.message}`);
        }
    }
    _notifyShipInWarpRemoteMessage(session) {
        ...
        try {
            ...
            log.warn(`[Beyonce] ShipInWarp remote message failed for char=${session && session.characterID}: ${error.message}`);
            return false;
        }
    }
    ...
    Handle_CmdWarpToStuffAutopilot(args, session) {
        const targetID = normalizeNumber(args && args[0], 0);
        log.info(`[Beyonce] CmdWarpToStuffAutopilot char=${session && session.characterID} target=${targetID}`);
        const result = spaceRuntime.warpToEntity(session, targetID, { minimumRange: 10000 });
        if (!result || !result.success) {
            log.warn(`[Beyonce] CmdWarpToStuffAutopilot failed for char=${session && session.characterID}: ${(result && result.errorMsg) || "UNKNOWN_ERROR"}`);
            this._throwWarpFailureUserError(result && result.errorMsg);
        }
        return null;
    }
    Handle_CmdDock(args, session) {
        ...
        log.info(`[Beyonce] CmdDock char=${session && session.characterID} station=${stationID}`);
        ...
        if (spaceRuntime.isDockingDisallowedByModules(session)) {
            log.info(`[Beyonce] CmdDock denied by active module for char=${session && session.characterID} station=${stationID}`);
            throwWrappedUserError("DockingRequestDenied", {
                reason: "You cannot dock while a module that prevents docking is active.",
            });
        }
        if (!spaceRuntime.canDockAtStation(session, stationID)) {
            log.info(`[Beyonce] CmdDock converting to docking approach for char=${session && session.characterID} station=${stationID}`);
            const followed = spaceRuntime.followBall(session, stationID, 2500, {
                dockingTargetID: stationID,
            });
            if (!followed) {
                log.info(`[Beyonce] CmdDock keeping existing docking approach for char=${session && session.characterID} station=${stationID}`);
            }
            throwWrappedUserError("DockingApproach", buildDockingApproachUserErrorValues(dockingDebug));
        }
        const result = spaceRuntime.acceptDocking(session, stationID);
        if (!result.success) {
            log.warn(`[Beyonce] CmdDock failed for char=${session && session.characterID}: ${result.errorMsg}`);
            if (result.errorMsg === "DOCKING_DISALLOWED_BY_MODULE") {
                throwWrappedUserError("DockingRequestDenied", {
                    reason: "You cannot dock while a module that prevents docking is active.",
                });
            }
    ...
    async Handle_CmdStargateJump(args, session) {
        ...
        const activeShipID = normalizeNumber(session && (session.shipID ||
            session.shipid ||
            session.activeShipID ||
            (session._space && session._space.shipID)), 0);
        log.info(`[Beyonce] CmdStargateJump char=${session && session.characterID} from=${fromStargateID} to=${toStargateID} ship=${requestedShipID || activeShipID || 0}`);
    ...
    Handle_MachoBindObject(args, session, kwargs) {
        ...
        const bootstrapResult = bootstrapSessionBallpark(session, {
            ...
            reset: Boolean(dockedStructureObserverSession &&
                session &&
        ...
        if (responseBuiltMs >= 100 || bootstrapElapsedMs >= 100) {
            log.info(`[Beyonce] MachoBindObject responseBuiltMs=${responseBuiltMs} ` +
...
//# sourceMappingURL=beyonceService.js.map