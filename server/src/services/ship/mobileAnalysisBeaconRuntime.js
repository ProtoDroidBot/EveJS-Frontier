...
const { ITEM_FLAGS, findItemById, findShipItemById, getItemMetadata, listSystemSpaceItems, removeInventoryItem, transferItemToOwnerLocation, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
const characterEnergyMgrService = require(path.join(__dirname, "../character/characterEnergyMgrService"));
...
function isMobileAnalysisBeaconType(typeIDOrItem) {
    if (typeIDOrItem && typeof typeIDOrItem === "object") {
        return (toInt(typeIDOrItem.categoryID, 0) === CATEGORY_DEPLOYABLE &&
            toInt(typeIDOrItem.groupID, 0) === GROUP_MOBILE_ANALYSIS_BEACON) || isMobileAnalysisBeaconType(toInt(typeIDOrItem.typeID, 0));
    }
    const typeID = toInt(typeIDOrItem, 0);
    if (typeID === TYPE_CONCORD_ROGUE_ANALYSIS_BEACON ||
...
function collectPlacementBlockers(session, context, options = {}) {
    ...
    const append = (source, kind = null) => {
        ...
        appendPlacementBlocker(blockers, seenKeys, {
            ...source,
...
function buildDeployableSpawnState(session, context) {
    const shipEntity = session && session._space && session._space.shipID
...
function activateMobileAnalysisBeacon(itemID) {
    ...
    log.info(`[MobileAnalysisBeacon] Activated beacon=${itemID} system=${state.solarSystemID}`);
    return true;
}
...
function scheduleMobileAnalysisBeaconTimers(itemOrID) {
    ...
    if (state.expiresAtMs > 0) {
        nextTimers.expiryTimer = setUnrefTimeout(() => expireMobileAnalysisBeacon(itemID, "expired"), state.expiresAtMs - now);
    }
    if (!state.active) {
        nextTimers.activationTimer = setUnrefTimeout(() => activateMobileAnalysisBeacon(itemID), state.activateAtMs - now);
    }
    if (toInt(state.linkState, LINKSTATE_IDLE) === LINKSTATE_RUNNING &&
        state.linkCompleteAtMs > 0) {
        nextTimers.linkTimer = setUnrefTimeout(() => completeMobileAnalysisBeaconLink(itemID), state.linkCompleteAtMs - now);
    }
...
function hydrateMobileAnalysisBeaconEntityFromInventoryItem(entity, itemRecord) {
    ...
    entity.activate_comp_durationSeconds = Math.trunc(MOBILE_ANALYSIS_BEACON_ACTIVATION_DELAY_MS / 1000);
...
function resolveShipTypeContext(shipEntity, shipItem) {
    ...
    return {
        typeID,
        groupID: toInt(shipEntity && shipEntity.groupID, toInt(shipItem && shipItem.groupID, toInt(metadata.groupID, 0))),
        categoryID: toInt(shipEntity && shipEntity.categoryID, toInt(shipItem && shipItem.categoryID, toInt(metadata.categoryID, 0))),
        groupName: String((shipEntity && shipEntity.groupName) ||
...
function validateLinkingShip(beaconItem, shipEntity, shipItem) {
    ...
    if (!canEntitiesInteractLocally(shipEntity, beaconItem)) {
        return "MOBILE_ANALYSIS_BEACON_NOT_FOUND";
    }
...
function applyLinkRestrictionsToShip(session, shipEntity, beaconID, linkCompleteAtMs) {
    ...
    const statusClearAtMs = linkCompleteAtMs + MOBILE_ANALYSIS_BEACON_LINK_STATUS_CLEAR_DELAY_MS;
    shipEntity.mobileAnalysisBeaconLinkedBeaconID = toInt(beaconID, 0);
    shipEntity.mobileAnalysisBeaconImmobileUntilMs = Math.max(toReal(shipEntity.mobileAnalysisBeaconImmobileUntilMs, 0), linkCompleteAtMs);
    shipEntity.mobileAnalysisBeaconWarpDisabledUntilMs = Math.max(toReal(shipEntity.mobileAnalysisBeaconWarpDisabledUntilMs, 0), statusClearAtMs);
    shipEntity.mobileAnalysisBeaconCloakBlockedUntilMs = Math.max(toReal(shipEntity.mobileAnalysisBeaconCloakBlockedUntilMs, 0), statusClearAtMs);
    shipEntity.mobileAnalysisBeaconTetherBlockedUntilMs = Math.max(toReal(shipEntity.mobileAnalysisBeaconTetherBlockedUntilMs, 0), statusClearAtMs);
...
function launchMobileAnalysisBeaconFromShip(session, itemID, options = {}) {
    ...
    if (placementConflict) {
        log.info(`[MobileAnalysisBeacon] Launch rejected char=${context.characterID} item=${item.itemID} system=${context.systemID} reason=${placementConflict.errorMsg}`);
        return {
            success: false,
    ...
    if (!spawnResult || !spawnResult.success) {
        log.warn(`[MobileAnalysisBeacon] Launched beacon ${launchedItemID} but space spawn failed: ${spawnResult ? spawnResult.errorMsg : "UNKNOWN"}`);
    }
    registerMobileAnalysisBeaconItem(updatedResult.data);
    log.info(`[MobileAnalysisBeacon] char=${context.characterID} launched beacon itemID=${launchedItemID} typeID=${sourceTypeID} system=${context.systemID}`);
    return {
        success: true,
        data: {
...
function initiateMobileAnalysisBeaconLink(session, beaconID, options = {}) {
    ...
    if (toInt(state.linkState, LINKSTATE_IDLE) !== LINKSTATE_IDLE ||
    ...
    syncInventoryChange(session, updatedResult.data, updatedResult.previousData || {});
    ...
    log.info(`[MobileAnalysisBeacon] Link started char=${characterID} ship=${context.shipID} beacon=${beaconID} method=${options.quick ? "InitiateLinkQuick" : "InitiateLink"}`);
    return {
        success: true,
        data: {
