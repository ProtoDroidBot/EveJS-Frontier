...
const { createSpaceItemForOwner, getItemMetadata, removeInventoryItem, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
...
function isInterdictionSphereLauncherType(typeIDOrItem) {
    if (typeIDOrItem && typeof typeIDOrItem === "object") {
        return (toInt(typeIDOrItem.categoryID, 0) === CATEGORY_MODULE &&
            toInt(typeIDOrItem.groupID, 0) === GROUP_INTERDICTION_SPHERE_LAUNCHER) || isInterdictionSphereLauncherType(toInt(typeIDOrItem.typeID, 0));
    }
    ...
    const metadata = getItemMetadata(typeID) || {};
    return (toInt(metadata.categoryID, 0) === CATEGORY_MODULE &&
...
function getProbeLifetimeMs(typeID) {
    return Math.max(0, toInt(CURRENT_SDE_PROBE_LIFETIME_MS_BY_TYPE_ID[toInt(typeID, 0)], 0));
}
function isWarpDisruptionProbeType(typeIDOrItem) {
    if (typeIDOrItem && typeof typeIDOrItem === "object") {
        const typeID = toInt(typeIDOrItem.typeID, 0);
        return (toInt(typeIDOrItem.categoryID, 0) === CATEGORY_CHARGE &&
    ...
    const metadata = getItemMetadata(typeID) || {};
    return (toInt(metadata.categoryID, 0) === CATEGORY_CHARGE &&
...
function getActivationRestriction(systemID, moduleTypeID, chargeTypeID) {
    ...
    const disallowInEmpire = checkedTypeIDs.some((typeID) => getTypeDogmaAttributeValue(typeID, ATTRIBUTE_DISALLOW_IN_EMPIRE_SPACE, 0) > 0);
    if (disallowInEmpire) {
    ...
    const disallowInHazard = checkedTypeIDs.some((typeID) => getTypeDogmaAttributeValue(typeID, ATTRIBUTE_DISALLOW_IN_HAZARD_SYSTEM, 0) > 0);
...
function validateInterdictionProbeLaunchContext(session, moduleItem, loadedCharge) {
    ...
    const availableQuantity = Math.max(0, toInt(loadedCharge.stacksize ?? loadedCharge.quantity, 0));
    ...
    const activationRestriction = getActivationRestriction(context.systemID, moduleTypeID, chargeTypeID);
...
function hydrateInterdictionProbeEntityFromInventoryItem(entity, itemRecord) {
    ...
    const now = Date.now();
    const rangeMeters = Math.max(0, toReal(state.rangeMeters, getWarpDisruptionRangeMeters(itemRecord.typeID)));
...
function getInterdictionProbeStateFromSource(source) {
    ...
    if (source.interdictionProbeState && typeof source.interdictionProbeState === "object") {
        return {
            ...
            rangeMeters: toReal(source.interdictionProbeState.rangeMeters, toReal(source.warpDisruptionRangeMeters, getWarpDisruptionRangeMeters(source.typeID))),
        };
    }
...
function buildActiveWarpDisruptorCandidate(source, options = {}) {
    ...
    const rangeMeters = Math.max(0, toReal(state.rangeMeters, getWarpDisruptionRangeMeters(source.typeID)));
...
function removeInterdictionProbe(itemID, systemID = 0, reason = "removed") {
    ...
    if (normalizedSystemID > 0) {
        const destroyResult = getSpaceRuntime().destroyDynamicInventoryEntity(normalizedSystemID, normalizedItemID);
    ...
    if (!removeResult.success) {
        log.warn(`[InterdictionProbe] Failed to remove probe itemID=${normalizedItemID} reason=${reason} error=${removeResult.errorMsg}`);
    }
...
function launchInterdictionProbeFromModule(session, moduleItem, loadedCharge, options = {}) {
    ...
    const createResult = createSpaceItemForOwner(context.ownerID, context.systemID, chargeTypeID, {
    ...
    log.info(`[InterdictionProbe] char=${context.characterID} launched probe itemID=${probeID} ` +
...
//# sourceMappingURL=interdictionProbeRuntime.js.map