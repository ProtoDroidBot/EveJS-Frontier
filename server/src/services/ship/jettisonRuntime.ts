...
} = require(path.join(__dirname, "../inventory/cargoLootAuthority"));
...
function normalizeSpaceVector(v, fallback: Record<string, any> = { x: 1, y: 0, z: 0 }) {
...
function buildJetcanLootCustomInfo(session) {
  ...
  const lootInfo: Record<string, any> = {};
...
function syncChangesToSession(session, changes: any[] = [], options: Record<string, any> = {}) {
...
function jettisonItemsForSession(session, itemIDs) {
  ...
  if (!Array.isArray(itemIDs) || itemIDs.length === 0) {
    return { success: false as const, errorMsg: "NO_ITEMS" };
  }
  ...
  if (jettisonedToCanIDs.length === 0) {
    ...
    return { success: false as const, errorMsg: "MOVE_FAILED" };
  }
