...
function appendMobileDepotPlacementBlocker(blockers, seenKeys, source, options: Record<string, any> = {}) {
...
function collectMobileDepotPlacementBlockers(session, context, options: Record<string, any> = {}) {
  const blockers: any[] = [];
  ...
  const append = (source, kind = null) => {
    ...
    appendMobileDepotPlacementBlocker(
      blockers,
      seenKeys,
      source,
      {
...
function validateMobileDepotCargoAccess(session, itemOrID, options: Record<string, any> = {}) {
...
function validateMobileDepotFittingAccess(session, itemOrID, options: Record<string, any> = {}) {
  const item = itemOrID && typeof itemOrID === "object"
    ? itemOrID
    : findItemById(itemOrID);
  const state = getMobileDepotStateFromItem(item);
...
function enterMobileDepotReinforcementFromDamage(scene, entity, damageResult, options: Record<string, any> = {}) {
...
function launchMobileDepotFromShip(session, itemID, options: Record<string, any> = {}) {
  const context = getSessionContext(session, options);
  const item = findItemById(itemID);
