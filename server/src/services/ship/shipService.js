...
const { activateShipForSession, findCharacterShip, getActiveShipRecord, buildInventoryItemRow, emitFittingTransactionForSession, syncInventoryItemForSession, } = require(path.join(__dirname, "../character/characterState"));
const { ensureCapsuleForCharacter, ITEM_FLAGS, findItemById, getShipConditionState, listContainerItems, moveItemsAndSetShipPackagingState, setShipPackagingState, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
const { normalizeShipConfiguration, } = require(path.join(__dirname, "./shipServiceAccess"));
const { getCharacterSkillPointTotal, } = require(path.join(__dirname, "../skills/skillState"));
const { normalizeActivationStateForProfile, } = require(path.join(__dirname, "../dogma/dogmaInfoCompatibility"));
const { undockSession, ejectSession, boardSpaceShip, } = require(path.join(__dirname, "../../space/transitions"));
const { getDockedLocationID, isDockedSession, } = require(path.join(__dirname, "../structure/structureLocation"));
const { listFittedItems, getFittedModuleItems, getTurretLikeModuleItems, buildModuleStatusSnapshot, buildChargeSublocationData, SLOT_FAMILY_FLAGS, isStrategicCruiserType, getSubsystemSlotFlag, isSubsystemCompatibleWithStrategicCruiser, getStrategicCruiserSubsystemStatus, getStrategicCruiserUseStatus, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const { buildWeaponBankStateDict, } = require(path.join(__dirname, "../moduleGrouping/moduleGroupingRuntime"));
const { buildActivationHeatStateDict, } = require(path.join(__dirname, "../dogma/activationHeatState"));
const { findFittingByID, } = require(path.join(__dirname, "../../_secondary/fitting/fittingStore"));
const { refreshShipFittingSnapshot, } = require(path.join(__dirname, "../../_secondary/fitting/fittingRuntime"));
const { buildList, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { resolveItemByTypeID, } = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const { launchDronesForSession, scoopDrone, } = require(path.join(__dirname, "../drone/droneRuntime"));
const { jettisonItemsForSession, } = require(path.join(__dirname, "./jettisonRuntime"));
const { boardStoredShipFromMaintenanceContainer, launchShipFromMaintenanceContainer, storeActiveShipInMaintenanceContainer, } = require(path.join(__dirname, "./ejectRuntime"));
const { launchOrbitalsFromShip, } = require(path.join(__dirname, "./orbitalLaunchRuntime"));
...
function extractKwarg(kwargs, key) {
    ...
    if (kwargs.type === "dict" && Array.isArray(kwargs.entries)) {
        // Kwarg keys can arrive as plain strings, Buffers, or marshalled string
        // objects depending on how the client encoded them. Compare on the
        // normalized string form so we don't miss (for example) a Buffer-encoded
        // "subSystems" key on strategic-cruiser AssembleShip calls, which would
        // otherwise silently drop the subsystem selection (=> MISSING_SUBSYSTEMS).
        const match = kwargs.entries.find((entry) => normalizeText(entry[0], "") === key);
...
class ShipService extends BaseService {
    ...
    _getShipID(session) {
        const activeShip = session && session.characterID
            ? getActiveShipRecord(session.characterID)
            : null;
        return ((activeShip && (activeShip.itemID || activeShip.shipID)) ||
    ...
    _getInvBrokerForFitting() {
        const InvBrokerService = require(path.join(__dirname, "../inventory/invBrokerService"));
    ...
    _addRequiredFitType(requiredByType, typeID, quantity) {
        ...
        requiredByType.set(normalizedTypeID, (requiredByType.get(normalizedTypeID) || 0) + normalizedQuantity);
    }
    ...
    _buildMultifitItemsToFit(characterID, itemLocationID, itemContainerID, requiredByType) {
        ...
        for (const item of this._listMultifitSourceItems(characterID, itemLocationID, itemContainerID)) {
        ...
        return Object.fromEntries([...itemsByType.entries()].map(([typeID, itemIDs]) => [String(typeID), itemIDs]));
    }
    ...
    _setMultifitShipName(shipID, name, session = null) {
        ...
        if (!updateResult.success) {
            log.warn(`[Ship] FitShips failed to label ship=${shipID}: ${updateResult.errorMsg || "UNKNOWN_ERROR"}`);
            return;
        }
        const previousName = String((updateResult.previousData && updateResult.previousData.itemName) || "");
    ...
    _syncRequestedOnlineModulesForUndock(session, shipID, kwargs) {
        const requestedOnlineModules = collectMachoDictValues(extractKwarg(kwargs, "onlineModules"))
        ...
        const fittedModules = getFittedModuleItems(charID, shipID).filter((item) => requestedSet.has(Number(item && item.itemID)));
        let updatedCount = 0;
        ...
        if (updatedCount > 0) {
            log.info(`[Ship] Promoted ${updatedCount} requested online module(s) before undock for ship=${shipID}`);
        }
        return updatedCount;
    }
    ...
    _buildActivationResponse(activeShip, session) {
        ...
        const shipID = (activeShip && (activeShip.itemID || activeShip.shipID)) ||
    ...
    _getShipConfiguration(shipID) {
        ...
        if (!this._shipConfiguration.has(numericShipID)) {
            ...
            this._shipConfiguration.set(numericShipID, this._normalizeShipConfiguration(shipItem && shipItem.shipConfiguration));
        }
    ...
    _persistShipConfiguration(shipID, configuration) {
        ...
        if (!updateResult.success) {
            log.warn(`[Ship] Failed to persist ship configuration shipID=${numericShipID}: ${updateResult.errorMsg || "UNKNOWN_ERROR"}`);
        }
        return updateResult;
    }
    _getStrategicCruiserUseStatus(shipItem) {
        if (!shipItem || !isStrategicCruiserType(shipItem.typeID)) {
            return null;
        }
        const fittedItems = listContainerItems(null, shipItem.itemID, null);
        return getStrategicCruiserUseStatus(shipItem.ownerID, shipItem, fittedItems);
    }
    _assertStrategicCruiserSubsystems(shipItem, sourceLabel) {
        const status = this._getStrategicCruiserUseStatus(shipItem);
        if (!status || status.complete) {
            return;
        }
        const subsystemStatus = status.subsystemStatus || {
            missingFlags: [],
            duplicateFlags: [],
            invalidItems: [],
        };
        const familyNameByFlag = new Map([
            [125, "Core"],
            [126, "Defensive"],
            [127, "Offensive"],
            [128, "Propulsion"],
        ]);
        const missingFamilies = subsystemStatus.missingFlags
            .map((flagID) => familyNameByFlag.get(flagID) || `slot ${flagID}`);
        let detail;
        if (missingFamilies.length > 0) {
            detail = ` Missing: ${missingFamilies.join(", ")}.`;
        }
        else if (subsystemStatus.duplicateFlags.length > 0 ||
            subsystemStatus.invalidItems.length > 0) {
            detail = " Remove duplicate or incompatible subsystems before boarding.";
        }
        else {
            detail =
                " Remove modules or bay contents that exceed the selected subsystems' " +
                    "slots, hardpoints, CPU, powergrid, drone bay, or fighter bay before " +
                    "boarding.";
        }
        log.warn(`[Ship] ${sourceLabel} rejected incomplete strategic cruiser ` +
            `ship=${Number(shipItem.itemID) || 0} ` +
            `missing=${JSON.stringify(subsystemStatus.missingFlags)} ` +
            `duplicates=${JSON.stringify(subsystemStatus.duplicateFlags)} ` +
            `invalidSubsystems=${subsystemStatus.invalidItems.length} ` +
            `invalidSlots=${status.invalidRegularSlotItems.length} ` +
            `turretOverload=${status.turretOverload} ` +
            `launcherOverload=${status.launcherOverload} ` +
            `cpuOverload=${status.cpuOverload} powerOverload=${status.powerOverload} ` +
            `droneBayOverload=${status.droneBayOverload} ` +
            `fighterBayOverload=${status.fighterBayOverload}`);
        throwWrappedUserError("CustomNotify", {
            notify: "A strategic cruiser requires one Core, Defensive, Offensive, and " +
                `Propulsion subsystem before it can be used.${detail}`,
        });
    }
    _extractSubsystemSelection(rawValue) {
    ...
    _buildStrategicCruiserAssemblyPlan(charID, stationID, shipItem, requestedSubsystemIDs = []) {
        if (!isStrategicCruiserType(shipItem && shipItem.typeID)) {
            return { success: true, plan: [] };
        }
        const fittedItems = listFittedItems(charID, shipItem.itemID);
        const currentStatus = getStrategicCruiserSubsystemStatus(shipItem, fittedItems);
        if (currentStatus.duplicateFlags.length > 0 ||
            currentStatus.invalidItems.length > 0) {
            return { success: false, errorMsg: "INVALID_FITTED_SUBSYSTEMS" };
        }
        const numericRequestedIDs = (Array.isArray(requestedSubsystemIDs)
            ? requestedSubsystemIDs
            : [])
            .map((itemID) => this._extractShipId(itemID))
            .filter((itemID) => itemID > 0);
        if (new Set(numericRequestedIDs).size !== numericRequestedIDs.length) {
            return { success: false, errorMsg: "DUPLICATE_SUBSYSTEM_SELECTION" };
        }
        const selectedByFlag = new Map();
        for (const itemID of numericRequestedIDs) {
            const item = findItemById(itemID);
            const flagID = getSubsystemSlotFlag(item && item.typeID);
            if (!item ||
                Number(item.ownerID) !== Number(charID) ||
                Number(item.locationID) !== Number(stationID) ||
                Number(item.flagID) !== ITEM_FLAGS.HANGAR ||
                flagID <= 0 ||
                !isSubsystemCompatibleWithStrategicCruiser(item, shipItem.typeID)) {
                return { success: false, errorMsg: "INVALID_SUBSYSTEM_SELECTION" };
            }
            if (!currentStatus.missingFlags.includes(flagID) ||
                selectedByFlag.has(flagID)) {
                return { success: false, errorMsg: "DUPLICATE_SUBSYSTEM_FAMILY" };
            }
            selectedByFlag.set(flagID, item);
        }
        const plan = [];
        const prospectiveFittedItems = [...fittedItems];
        for (const flagID of currentStatus.missingFlags) {
            const selectedItem = selectedByFlag.get(flagID);
            if (!selectedItem) {
                return {
                    success: false,
                    errorMsg: "MISSING_SUBSYSTEMS",
                    flagID,
                };
            }
            plan.push({
            ...
            prospectiveFittedItems.push({
                ...selectedItem,
                locationID: shipItem.itemID,
                flagID,
            });
        }
        if (selectedByFlag.size !== currentStatus.missingFlags.length) {
            return { success: false, errorMsg: "UNEXPECTED_SUBSYSTEM_SELECTION" };
        }
        const prospectiveStatus = getStrategicCruiserSubsystemStatus(shipItem, prospectiveFittedItems);
        return prospectiveStatus.complete
            ? { success: true, plan }
            : { success: false, errorMsg: "INCOMPLETE_SUBSYSTEM_PLAN" };
    }
    _activateShipById(shipID, session, sourceLabel) {
        ...
        if (requestedShip) {
            this._assertStrategicCruiserSubsystems(requestedShip, sourceLabel);
        }
        log.info(`[Ship] ${sourceLabel} shipID=${numericShipID} current=${currentShip ? (currentShip.itemID || currentShip.shipID) : "none"} requested=${requestedShip ? `${requestedShip.shipName}(${requestedShip.shipTypeID})` : "unknown"}`);
        const activationResult = activateShipForSession(session, numericShipID, {
            emitNotifications: true,
            logSelection: true,
        });
        if (!activationResult.success) {
            log.warn(`[Ship] ${sourceLabel} failed for shipID=${numericShipID}: ${activationResult.errorMsg}`);
            return null;
        }
    ...
    _leaveShip(session, shipID, sourceLabel) {
        ...
        const capsuleWasCreated = Boolean(capsuleResult.created === true ||
            (capsuleResult.data &&
        ...
        if (capsuleWasCreated) {
            ...
            syncInventoryItemForSession(session, capsuleResult.data, {
                locationID: 0,
                flagID: 0,
                quantity: 0,
                singleton: 0,
        ...
        const activationResult = activateShipForSession(session, capsuleResult.data.itemID, {
            emitNotifications: true,
            logSelection: true,
        });
        if (!activationResult.success) {
            log.warn(`[Ship] ${sourceLabel} failed to activate capsule: ${activationResult.errorMsg}`);
            return null;
        }
    ...
    Handle_GetDirtTimestamp(args, session, kwargs) {
        ...
        log.debug(`[Ship] GetDirtTimestamp(shipID=${shipID}) -> ${String(dirtTimestamp)}`);
    ...
    Handle_SetDirtTimestamp(args, session, kwargs) {
        ...
        log.debug(`[Ship] SetDirtTimestamp(shipID=${shipID}, ts=${String(ts)}, stored=${String(dirtTimestamp)})`);
        return null;
    }
    Handle_ResetDirtTimestamp(args, session, kwargs) {
        const shipID = args && args.length > 0 ? args[0] : null;
        const numericShipID = this._extractShipId(shipID);
        const characterID = session && (session.characterID || session.charID || session.charid);
        if (numericShipID <= 0 || !characterID || !findCharacterShip(characterID, numericShipID)) {
            log.warn(`[Ship] ResetDirtTimestamp denied for shipID=${shipID}, characterID=${characterID || 0}`);
            return 0n;
        }
        ...
        log.debug(`[Ship] ResetDirtTimestamp(shipID=${shipID}) -> ${String(dirtTimestamp || 0n)}`);
    ...
    Handle_GetModules(args, session, kwargs) {
        ...
        return {
            type: "list",
            items: getFittedModuleItems(charID, shipID).map((item) => buildInventoryItemRow(item)),
        };
    }
    Handle_LaunchDrones(args, session, kwargs) {
        void kwargs;
        const launchResult = launchDronesForSession(session, args && args.length > 0 ? args[0] : []);
        if (!launchResult || launchResult.success !== true) {
            log.warn(`[Ship] LaunchDrones failed for char=${session ? session.characterID : "?"}: ${launchResult ? launchResult.errorMsg : "UNKNOWN_ERROR"}`);
            return { type: "dict", entries: [] };
        }
    ...
    Handle_ScoopDrone(args, session, kwargs) {
        void kwargs;
        return scoopDrone(session, args && args.length > 0 ? args[0] : []);
    }
    ...
    Handle_ScoopToMobileDepotHold(args, session, kwargs) {
        void kwargs;
        const objectID = this._extractShipId(args && args.length > 0 ? args[0] : null);
        return this._handleDeployableScoopResult(session, objectID, scoopMobileDepotToMobileDepotHold(session, objectID));
    }
    _handleDeployableScoopResult(session, objectID, result) {
        if (!result || !result.success) {
            ...
            writeLog(`[Ship] Scoop failed for char=${session ? session.characterID : "?"} objectID=${objectID}: ${errorMsg}`);
    ...
    Handle_Jettison(args, session, kwargs) {
        ...
        if (!result || !result.success) {
            log.warn(`[Ship] Jettison failed for char=${session ? session.characterID : "?"}: ${result ? result.errorMsg : "UNKNOWN_ERROR"}`);
            return [[], []];
        }
    ...
    Handle_LaunchFromShip(args, session, kwargs) {
        ...
        if (!result || !result.success) {
            log.warn(`[Ship] LaunchFromShip failed for char=${session ? session.characterID : "?"}: ${result ? result.errorMsg : "UNKNOWN_ERROR"}`);
    ...
    Handle_LaunchFromContainer(args, session, kwargs) {
        ...
        const result = launchShipFromMaintenanceContainer(session, sourceLocationID, itemID);
        if (!result || !result.success) {
            const errorMsg = result ? result.errorMsg : "UNKNOWN_ERROR";
            log.warn(`[Ship] LaunchFromContainer failed source=${String(sourceLocationID)} item=${String(itemID)}: ${errorMsg}`);
    ...
    Handle_Drop(args, session, kwargs) {
        ...
        log.info(`[Ship] Drop launchRequests=${JSON.stringify(rawLaunchRequests, (k, v) => (typeof v === "bigint" ? v.toString() : v))} whoseBehalfID=${whoseBehalfID || "default"} ignoreWarning=${ignoreWarning}`);
        const result = launchOrbitalsFromShip(session, rawLaunchRequests, {
            ownerID: whoseBehalfID,
            ignoreWarning,
        });
        if (!result || !result.success) {
            log.warn(`[Ship] Drop failed for char=${session ? session.characterID : "?"}: ${result ? result.errorMsg : "UNKNOWN_ERROR"}`);
        }
    ...
    Handle_ActivateShip(args, session, kwargs) {
        ...
        log.info(`[Ship] ActivateShip(shipID=${String(shipID)}, oldShipID=${String(oldShipID)})`);
    ...
    Handle_AssembleShip(args, session, kwargs) {
        ...
        const charID = session && session.characterID ? session.characterID : 0;
        const rows = [];
        const rawRequestedName = unwrapMarshalValue(extractKwarg(kwargs, "name") ?? (args && args.length > 1 ? args[1] : ""));
        const requestedName = shipIds.length === 1
            ? normalizeShipNameLabel(rawRequestedName)
            : "";
        const requestedSubsystemIDs = this._extractSubsystemSelection(extractKwarg(kwargs, "subSystems") ??
            (args && args.length > 2 ? args[2] : null));
        log.info(`[Ship] AssembleShip station=${stationID} shipIDs=${JSON.stringify(shipIds)}`);
        for (const shipID of shipIds) {
            const shipItem = findCharacterShip(charID, shipID);
            if (!shipItem) {
                continue;
            }
            if (shipItem.locationID !== stationID ||
                shipItem.flagID !== ITEM_FLAGS.HANGAR ||
                shipItem.singleton === 1) {
                continue;
            }
            const subsystemPlanResult = this._buildStrategicCruiserAssemblyPlan(charID, stationID, shipItem, requestedSubsystemIDs);
            if (!subsystemPlanResult.success) {
                log.warn(`[Ship] AssembleShip refused strategic cruiser ship=${shipID}: ` +
                    `${subsystemPlanResult.errorMsg}`);
                continue;
            }
            const isStrategicCruiser = isStrategicCruiserType(shipItem.typeID);
            const previousFittingSnapshot = isStrategicCruiser
                ? refreshShipFittingSnapshot(charID, shipItem.itemID, {
                    shipItem,
                    reason: "strategic-cruiser-assembly.before",
                })
                : null;
            const assemblyResult = isStrategicCruiser
                ? moveItemsAndSetShipPackagingState(shipItem.itemID, subsystemPlanResult.plan.map((planEntry) => ({
                    itemID: planEntry.item.itemID,
                    destinationLocationID: shipItem.itemID,
                    destinationFlagID: planEntry.flagID,
                    quantity: 1,
                })), false)
                : setShipPackagingState(shipItem.itemID, false);
            if (!assemblyResult.success) {
                log.warn(`[Ship] AssembleShip failed${isStrategicCruiser ? " atomically" : ""} ` +
                    `for ${shipID}: ` +
                    `${assemblyResult.errorMsg || "UNKNOWN_ERROR"}`);
                continue;
            }
            let finalItem = assemblyResult.data;
            let transactionChanges = isStrategicCruiser
                ? assemblyResult.changes
                : null;
            const assembledShipID = Number(finalItem && finalItem.itemID) || shipItem.itemID;
            if (requestedName) {
                const labelResult = updateInventoryItem(assembledShipID, (currentItem) => ({
                    ...currentItem,
                ...
                if (labelResult.success) {
                    ...
                    if (transactionChanges) {
                        transactionChanges = transactionChanges.map((change) => (Number(change && change.item && change.item.itemID) === assembledShipID
                            ? { ...change, item: finalItem }
                            : change));
                    }
                }
                else {
                    log.warn(`[Ship] AssembleShip failed to label ship=${shipID}: ${labelResult.errorMsg || "UNKNOWN_ERROR"}`);
                }
            }
            if (transactionChanges) {
                emitFittingTransactionForSession(session, assembledShipID, transactionChanges, {
                    shipItem: finalItem,
                    previousSnapshot: previousFittingSnapshot,
                });
            }
            else {
                syncInventoryItemForSession(session, finalItem, {
                    locationID: assemblyResult.previousData.locationID,
                    flagID: assemblyResult.previousData.flagID,
                    quantity: assemblyResult.previousData.quantity,
                    singleton: assemblyResult.previousData.singleton,
                    stacksize: assemblyResult.previousData.stacksize,
    ...
    Handle_FitShips(args, session, kwargs) {
        const charID = session && session.characterID ? session.characterID : 0;
        const shipTypeID = normalizeInteger(extractKwarg(kwargs, "shipTypeID") ?? (args && args.length > 0 ? args[0] : 0), 0);
        const fitInfo = extractKwarg(kwargs, "fitInfo") ?? (args && args.length > 1 ? args[1] : {});
        const itemLocationID = normalizeInteger(extractKwarg(kwargs, "itemLocationID") ??
            (args && args.length > 2 ? args[2] : getDockedLocationID(session)), getDockedLocationID(session) || 0);
        const cargoItemsByType = extractKwarg(kwargs, "cargoItemsByType") ??
            (args && args.length > 3 ? args[3] : {});
        const fitRigs = normalizeBoolean(extractKwarg(kwargs, "fitRigs") ?? (args && args.length > 4 ? args[4] : undefined), true);
        const fittingName = normalizeText(extractKwarg(kwargs, "name") ?? (args && args.length > 5 ? args[5] : ""), "");
        const numToFit = Math.max(0, normalizeInteger(extractKwarg(kwargs, "numToFit") ?? (args && args.length > 6 ? args[6] : 0), 0));
        const itemContainerID = normalizeInteger(extractKwarg(kwargs, "itemContainerID") ??
            (args && args.length > 7 ? args[7] : 0), 0);
        if (charID <= 0 || shipTypeID <= 0 || itemLocationID <= 0 || numToFit <= 0) {
            log.warn(`[Ship] FitShips invalid request char=${charID} shipType=${shipTypeID} location=${itemLocationID} numToFit=${numToFit}`);
            return null;
        }
        const invBroker = this._getInvBrokerForFitting();
        const requiredByType = this._buildMultifitRequiredTypes(invBroker, fitInfo, cargoItemsByType, fitRigs);
        const shipsToFit = this._listPackagedShipsForMultifit(charID, itemLocationID, shipTypeID, numToFit);
        if (shipsToFit.length < numToFit) {
            return buildFitShipsFailedInfo([[shipTypeID, numToFit - shipsToFit.length]], null);
        }
        ...
        for (const ship of shipsToFit) {
            ...
            if (!assembleResult.success) {
                log.warn(`[Ship] FitShips failed to assemble ship=${ship.itemID}: ${assembleResult.errorMsg || "UNKNOWN_ERROR"}`);
                return buildFitShipsFailedInfo([], ship.itemID);
            }
            syncInventoryItemForSession(session, assembleResult.data, {
            ...
            const itemsToFit = this._buildMultifitItemsToFit(charID, itemLocationID, itemContainerID, requiredByType);
    ...
    Handle_LeaveShip(args, session, kwargs) {
        ...
        if (session && !isDockedSession(session)) {
            ...
            if (!transitionResult.success) {
                log.warn(`[Ship] LeaveShip failed in space for char=${session && session.characterID}: ${transitionResult.errorMsg}`);
                return null;
            }
            return null;
        }
    ...
    Handle_BoardStoredShip(args, session, kwargs) {
        ...
        log.info(`[Ship] BoardStoredShip(structureID=${String(structureID)}, shipID=${String(shipID)})`);
        if (session && !isDockedSession(session)) {
            const transitionResult = boardStoredShipFromMaintenanceContainer(session, structureID, shipID);
            if (!transitionResult.success) {
                const errorMsg = transitionResult.errorMsg || "UNKNOWN_ERROR";
                log.warn(`[Ship] BoardStoredShip failed source=${String(structureID)} ship=${String(shipID)}: ${errorMsg}`);
                if (errorMsg === "STRATEGIC_CRUISER_FIT_INCOMPLETE") {
                    throwWrappedUserError("CustomNotify", {
                        notify: "A strategic cruiser requires a valid Core, Defensive, Offensive, " +
                            "and Propulsion subsystem fit before it can be boarded.",
                    });
                }
    ...
    Handle_StoreVessel(args, session, kwargs) {
        ...
        const result = storeActiveShipInMaintenanceContainer(session, sourceLocationID);
        if (!result.success) {
            const errorMsg = result.errorMsg || "UNKNOWN_ERROR";
            log.warn(`[Ship] StoreVessel failed source=${String(sourceLocationID)}: ${errorMsg}`);
            this._throwMaintenanceBayUserError(errorMsg);
        }
    ...
    Handle_Board(args, session, kwargs) {
        const shipID = args && args.length > 0 ? args[0] : null;
        const oldShipID = args && args.length > 1 ? args[1] : null;
        log.info(`[Ship] Board(shipID=${String(shipID)}, oldShipID=${String(oldShipID)})`);
        if (session && !isDockedSession(session)) {
            const transitionResult = boardSpaceShip(session, shipID);
            if (!transitionResult.success) {
                log.warn(`[Ship] Board failed in space for shipID=${String(shipID)}: ${transitionResult.errorMsg}`);
                if (transitionResult.errorMsg === "STRATEGIC_CRUISER_FIT_INCOMPLETE") {
                    throwWrappedUserError("CustomNotify", {
                        notify: "A strategic cruiser requires a valid Core, Defensive, Offensive, " +
                            "and Propulsion subsystem fit before it can be boarded.",
                    });
            ...
            const activeShip = (transitionResult.data && transitionResult.data.ship) ||
                getActiveShipRecord(session.characterID);
            return this._buildActivationResponse(activeShip, session);
        }
    ...
    Handle_Undock(args, session, kwargs) {
        ...
        log.info(`[Ship] Undock(shipID=${String(shipID)}, ignoreContraband=${ignoreContraband})`);
        const undockingShip = getActiveShipRecord(session && session.characterID);
        if (undockingShip) {
            this._assertStrategicCruiserSubsystems(undockingShip, "Undock");
        }
        this._syncRequestedOnlineModulesForUndock(session, shipID, kwargs);
        const result = undockSession(session);
        if (!result.success) {
            log.warn(`[Ship] Undock failed for char=${session && session.characterID}: ${result.errorMsg}`);
            return null;
        }
    ...
    Handle_Eject(args, session, kwargs) {
        ...
        if (session && !isDockedSession(session)) {
            const transitionResult = ejectSession(session);
            if (!transitionResult.success) {
                log.warn(`[Ship] Eject failed in space for char=${session && session.characterID}: ${transitionResult.errorMsg}`);
                return null;
            }
            return null;
        }
    ...
    Handle_ConfigureShip(args, session, kwargs) {
        const configPayload = args && args.length > 0 && args[0] && typeof args[0] === "object"
            ? args[0]
            : null;
        const shipID = this._getShipID(session);
        const configuration = this._getShipConfiguration(shipID);
        if (configPayload && configPayload.type === "dict" && Array.isArray(configPayload.entries)) {
            ...
            if (Object.prototype.hasOwnProperty.call(configPayload, "allowFleetSMBUsage") ||
                Object.prototype.hasOwnProperty.call(configPayload, "SMB_AllowFleetAccess")) {
                const normalizedValue = Boolean(Object.prototype.hasOwnProperty.call(configPayload, "SMB_AllowFleetAccess")
                    ? configPayload.SMB_AllowFleetAccess
                    : configPayload.allowFleetSMBUsage);
                configuration.allowFleetSMBUsage = normalizedValue;
                configuration.SMB_AllowFleetAccess = normalizedValue;
            }
            if (Object.prototype.hasOwnProperty.call(configPayload, "allowCorpSMBUsage") ||
                Object.prototype.hasOwnProperty.call(configPayload, "SMB_AllowCorpAccess")) {
                const normalizedValue = Boolean(Object.prototype.hasOwnProperty.call(configPayload, "SMB_AllowCorpAccess")
    ...
    Handle_MachoBindObject(args, session, kwargs) {
        ...
        log.debug(`[Ship] MachoBindObject args.length=${args ? args.length : 0} bindParams=${JSON.stringify(bindParams, (k, v) => (typeof v === "bigint" ? v.toString() : v))} nestedCall=${JSON.stringify(nestedCall, (k, v) => (typeof v === "bigint" ? v.toString() : Buffer.isBuffer(v) ? v.toString("utf8") : v))}`);
    ...
    afterCallResponse(methodName, session, context = {}) {
        ...
        const failedConditions = this._extractSafeLogoffFailedConditions(methodName, context);
        ...
        if (!startResult.success) {
            log.info(`[Ship] SafeLogoff countdown not started char=${session ? session.characterID : "?"} via=${methodName} reason=${startResult.errorMsg || "UNKNOWN"}`);
        }
    }
