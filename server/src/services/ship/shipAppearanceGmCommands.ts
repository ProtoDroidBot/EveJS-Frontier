...
function calculateDirtStatus(rawFiletime) {
  const dirtTime: bigint | null = normalizeFiletime(rawFiletime, null);
  if (dirtTime === null) {
    return null;
  }
...
function extractExplicitShipID(tokens: any[] = []) {
...
function describeShip(shipID, options: Record<string, any> = {}) {
...
function resolveTargetShip(session, tokens: any[] = []) {
...
function broadcastShipSlimChange(session, shipID, options: Record<string, any> = {}) {
...
function handleShipDirtCommand(session, argumentText = "") {
  ...
  if (directRatio !== null) {
    const target: Record<string, any> = resolveTargetShip(session, tokens.slice(1));
    if (!target.success) {
      return {
        success: false as const,
