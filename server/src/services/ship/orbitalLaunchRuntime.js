...
const { buildChildEntityScopeMetadata, } = require(path.join(__dirname, "../../space/destiny/identity/interactionScope.js"));
const log = require(path.join(__dirname, "../../utils/logger"));
const { ITEM_FLAGS, findItemById, transferItemToOwnerLocation, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
const { buildDict, buildList, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
const { deployStructureFromInventoryItem, getSovereigntyFlexKindForTypeID, getSovereigntyKindForTypeID, } = require(path.join(__dirname, "../sovereignty/sovPlayerDeployment"));
...
function ensureLaunchResponseEntry(response, itemID) {
    ...
    let entry = response.entries.find((candidate) => Array.isArray(candidate) && toInt(candidate[0], 0) === numericItemID);
    if (!entry) {
...
function buildOrbitalSpawnState(session, systemID) {
    const shipEntity = session && session._space && session._space.shipID
        ...
        : null;
    const position = normalizeVector(shipEntity && shipEntity.position, { x: 0, y: 0, z: 0 });
    const direction = normalizeDirection(shipEntity && shipEntity.direction, { x: 1, y: 0, z: 0 });
    const launchedPosition = addVectors(position, scaleVector(direction, ORBITAL_LAUNCH_DISTANCE_METERS));
    return {
...
function launchOrbitalItem(session, itemID, options = {}) {
    ...
    if (!spawnResult || !spawnResult.success) {
        log.warn(`[OrbitalLaunch] Launched orbital ${launchedItemID} but space spawn failed: ${spawnResult ? spawnResult.errorMsg : "UNKNOWN"}`);
    }
    log.info(`[OrbitalLaunch] char=${context.characterID} launched orbital itemID=${launchedItemID} typeID=${sourceTypeID} owner=${context.ownerID} system=${context.systemID} planet=${orbitalRecord ? orbitalRecord.planetID : 0}`);
    return {
        success: true,
        data: {
...
function isSovereigntyDeploymentType(item) {
    ...
    return Boolean(getSovereigntyKindForTypeID(typeID) ||
...
function launchOrbitalsFromShip(session, rawItemIDs, options = {}) {
    ...
    for (const request of requests) {
        ...
        for (let index = 0; index < request.quantity; index += 1) {
            ...
            const result = sourceItem && mobileAnalysisBeaconRuntime.isMobileAnalysisBeaconType(sourceItem)
                ? mobileAnalysisBeaconRuntime.launchMobileAnalysisBeaconFromShip(session, request.itemID, options)
                : sourceItem && mobileCynoInhibitorRuntime.isMobileCynoInhibitorType(sourceItem)
                    ? mobileCynoInhibitorRuntime.launchMobileCynoInhibitorFromShip(session, request.itemID, options)
                    : sourceItem && mobileScanInhibitorRuntime.isMobileScanInhibitorType(sourceItem)
                        ? mobileScanInhibitorRuntime.launchMobileScanInhibitorFromShip(session, request.itemID, options)
                        : sourceItem && mobileSiphonUnitRuntime.isMobileSiphonUnitType(sourceItem)
                            ? mobileSiphonUnitRuntime.launchMobileSiphonUnitFromShip(session, request.itemID, options)
                            : sourceItem && mobileMicroJumpUnitRuntime.isMobileMicroJumpUnitType(sourceItem)
                                ? mobileMicroJumpUnitRuntime.launchMobileMicroJumpUnitFromShip(session, request.itemID, options)
                                : sourceItem && mobileObservatoryRuntime.isMobileObservatoryType(sourceItem)
                                    ? mobileObservatoryRuntime.launchMobileObservatoryFromShip(session, request.itemID, options)
                                    : sourceItem && mobilePhaseAnchorRuntime.isMobilePhaseAnchorType(sourceItem)
                                        ? mobilePhaseAnchorRuntime.launchMobilePhaseAnchorFromShip(session, request.itemID, options)
                                        : sourceItem && mobileWarpDisruptorRuntime.isMobileWarpDisruptorType(sourceItem)
                                            ? mobileWarpDisruptorRuntime.launchMobileWarpDisruptorFromShip(session, request.itemID, options)
                                            : sourceItem && deployableCynoRuntime.isMobileCynoDeployableType(sourceItem.typeID)
                                                ? deployableCynoRuntime.launchMobileCynoDeployableFromShip(session, request.itemID, options)
                                                : sourceItem && mobileDepotRuntime.isMobileDepotDeployableType(sourceItem)
                                                    ? mobileDepotRuntime.launchMobileDepotFromShip(session, request.itemID, options)
                                                    : sourceItem && mobileTractorUnitRuntime.isMobileTractorUnitType(sourceItem)
                                                        ? mobileTractorUnitRuntime.launchMobileTractorUnitFromShip(session, request.itemID, options)
                                                        : sourceItem && isSovereigntyDeploymentType(sourceItem)
                                                            ? launchSovereigntyStructureFromShip(session, request.itemID, options)
            ...
            errors.push(error);
            appendLaunchResponseValue(response, request.itemID, buildNotifyErrorTuple(error.errorMsg));
            log.warn(`[OrbitalLaunch] LaunchFromShip failed itemID=${request.itemID}: ${error.errorMsg}`);
            break;
        }
    }
    return {
...
//# sourceMappingURL=orbitalLaunchRuntime.js.map