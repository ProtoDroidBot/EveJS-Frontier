...
const { ITEM_FLAGS, findItemById, listContainerItems, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const { getShipBaseAttributeValue, } = require(path.join(__dirname, "../fitting/liveFittingState"));
...
const { boardSpaceShip, ejectSession, getStrategicCruiserTransitionUseError, } = require(path.join(__dirname, "../../space/transitions"));
const { maybeExpireEmptySpaceContainer, } = require(path.join(__dirname, "./jettisonRuntime"));
const { validateShipServiceAccess, } = require(path.join(__dirname, "./shipServiceAccess"));
...
function distanceBetween(left, right) {
    const resolvedLeft = cloneVector(left);
    const resolvedRight = cloneVector(right);
    return Math.sqrt(((resolvedLeft.x - resolvedRight.x) ** 2) +
        ((resolvedLeft.y - resolvedRight.y) ** 2) +
        ((resolvedLeft.z - resolvedRight.z) ** 2));
}
function surfaceDistanceBetweenEntities(left, right) {
    return Math.max(0, distanceBetween(left && left.position, right && right.position) -
...
function buildLaunchStateNearSource(sourceEntity, offsetMeters = DEFAULT_LAUNCH_OFFSET_METERS) {
    const direction = normalizeVector(sourceEntity && sourceEntity.direction, { x: 1, y: 0, z: 0 });
    const sourceRadius = Math.max(0, toFiniteNumber(sourceEntity && sourceEntity.radius, 0));
    const offset = Math.max(50, sourceRadius + Math.max(50, toFiniteNumber(offsetMeters, DEFAULT_LAUNCH_OFFSET_METERS)));
    const position = addVectors(cloneVector(sourceEntity && sourceEntity.position), scaleVector(direction, offset));
    return {
        ...buildChildEntityScopeMetadata(sourceEntity),
...
function isShipMaintenanceBaySource(item) {
    ...
    const hasMaintenanceBay = toFiniteNumber(getShipBaseAttributeValue(item.typeID, "hasShipMaintenanceBay"), 0);
    const bayCapacity = toFiniteNumber(getShipBaseAttributeValue(item.typeID, "shipMaintenanceBayCapacity"), 0);
    return hasMaintenanceBay > 0 || bayCapacity > 0;
}
function getShipMaintenanceBayCapacity(sourceItem) {
    return Math.max(0, toFiniteNumber(getShipBaseAttributeValue(sourceItem && sourceItem.typeID, "shipMaintenanceBayCapacity"), 0));
}
function getMaintenanceSourceCapacity(sourceItem, sourceIsShipBay) {
    ...
    return Math.max(0, toFiniteNumber(sourceItem.capacity, toFiniteNumber(metadata.capacity, 0)));
}
function getItemVolume(item) {
    ...
    const volume = Math.max(0, toFiniteNumber(item.volume, toFiniteNumber(metadata.volume, 0)));
    const stackSize = toInt(item.singleton, 0) === 1
        ? 1
...
function canStoreShipInMaintenanceSource(context, shipItem, excludedItemID = 0) {
    const capacity = getMaintenanceSourceCapacity(context && context.sourceItem, context && context.sourceIsShipBay);
    if (capacity <= 0) {
        return true;
    }
    const used = getMaintenanceSourceUsedVolume(context.sourceID, excludedItemID);
    return used + getItemVolume(shipItem) <= capacity + 0.0001;
}
function isMaintenanceSourceInCurrentSystem(sourceItem, systemID) {
    return Boolean(sourceItem &&
...
function buildFallbackSourceEntity(sourceItem, systemID) {
    ...
    const metadata = resolveItemByTypeID(sourceItem.typeID) || {};
    return {
        ...buildChildEntityScopeMetadata(sourceItem),
        ...
        radius: Math.max(0, toFiniteNumber(sourceItem.radius, toFiniteNumber(metadata.radius, 0))),
    };
}
function resolveMaintenanceSourceContext(session, sourceLocationID, options = {}) {
    ...
    if (options.rejectActiveSource === true &&
        currentShip &&
    ...
    if (sourceIsShipBay) {
        const serviceAccess = validateShipServiceAccess(session, sourceItem, ITEM_FLAGS.SHIP_HANGAR);
    ...
    const activeEntity = currentShip && scene
        ? scene.getEntityByID(toInt(currentShip.itemID, 0))
        : null;
    if (activeEntity && !canEntitiesInteractLocally(activeEntity, sourceEntity)) {
        return {
            success: false,
            errorMsg: "SOURCE_MAINTENANCE_CONTAINER_NOT_VISIBLE",
        };
...
function isLaunchableContainerType(item, metadata) {
    const groupName = String(metadata && metadata.groupName || item && item.groupName || "").trim().toLowerCase();
    return (groupName.includes("container") ||
...
function launchShipOrContainerFromWreckOrContainer(session, sourceLocationID, itemID) {
    ...
    const scene = spaceRuntime.ensureScene(systemID);
    const sourceEntity = scene && scene.getEntityByID(sourceID);
    const activeEntity = scene && scene.getShipEntityForSession(session);
    if (!scene ||
        !activeEntity ||
        !isSourceSpaceContainerEntity(sourceEntity) ||
        !canEntitiesInteractLocally(activeEntity, sourceEntity)) {
        return {
            success: false,
    ...
    const updateResult = updateInventoryItem(targetItemID, (currentItem) => ({
        ...
        locationID: systemID,
        flagID: 0,
        singleton: toInt(currentItem.categoryID, 0) === CATEGORY_SHIP
            ? 1
    ...
    if (!spawnResult || spawnResult.success !== true) {
        log.warn(`[EjectRuntime] Launched item=${targetItemID} from source=${sourceID} but dynamic spawn failed: ${spawnResult ? spawnResult.errorMsg : "UNKNOWN_ERROR"}`);
        return {
            success: false,
...
function launchShipFromMaintenanceContainer(session, sourceLocationID, itemID, options = {}) {
    const contextResult = resolveMaintenanceSourceContext(session, sourceLocationID, options);
    ...
    if (!spawnResult || spawnResult.success !== true) {
        log.warn(`[EjectRuntime] Launched maintenance ship=${targetItemID} from source=${context.sourceID} but dynamic spawn failed: ${spawnResult ? spawnResult.errorMsg : "UNKNOWN_ERROR"}`);
        return {
            success: false,
            errorMsg: spawnResult && spawnResult.errorMsg || "SPACE_SPAWN_FAILED",
        };
...
function storeShipRecordInMaintenanceSource(session, shipItem, context, options = {}) {
    ...
    if (options.removeEntity !== false) {
        const removeResult = spaceRuntime.removeDynamicEntity(context.systemID, shipID, {
            forceVisibleSessions: session ? [session] : [],
        });
        if (removeResult &&
            removeResult.success === false &&
            removeResult.errorMsg !== "DYNAMIC_ENTITY_NOT_FOUND") {
            log.warn(`[EjectRuntime] Stored maintenance ship=${shipID} in source=${context.sourceID} but dynamic removal failed: ${removeResult.errorMsg}`);
        }
    }
    syncUpdatedItem(session, updateResult);
    return {
...
function boardStoredShipFromMaintenanceContainer(session, sourceLocationID, itemID) {
    if (spaceRuntime.isPilotWarpLandingPending(session)) {
        return {
            success: false,
    ...
    const strategicCruiserUseError = getStrategicCruiserTransitionUseError(session && session.characterID, targetResult.data);
    ...
    const launchResult = launchShipFromMaintenanceContainer(session, sourceLocationID, targetShipID, {
    ...
    if (currentShip && toInt(currentShip.typeID, 0) !== CAPSULE_TYPE_ID) {
        storeResult = storeShipRecordInMaintenanceSource(session, currentShip, context, { removeEntity: true });
        if (!storeResult.success) {
            log.warn(`[EjectRuntime] BoardStoredShip boarded ship=${targetShipID} but failed to store previous ship=${currentShip.itemID}: ${storeResult.errorMsg}`);
        }
    }
    return {
        success: true,
...
function storeActiveShipInMaintenanceContainer(session, sourceLocationID) {
    ...
    const storeResult = storeShipRecordInMaintenanceSource(session, currentShip, context, { removeEntity: true });
    if (!storeResult.success) {
        return storeResult;
    }
    return {
        ...
        data: {
            ...
            capsule: ejectResult.data && ejectResult.data.capsule
...
//# sourceMappingURL=ejectRuntime.js.map