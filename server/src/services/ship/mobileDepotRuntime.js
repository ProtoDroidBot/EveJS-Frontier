...
const { ITEM_FLAGS, findItemById, getItemMetadata, listContainerItems, listSystemSpaceItems, removeInventoryItem, transferItemToOwnerLocation, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
const { buildShipResourceState, getShipBaseAttributeValue, } = require(path.join(__dirname, "../fitting/liveFittingState"));
...
function normalizeConditionState(conditionState) {
    const source = conditionState && typeof conditionState === "object"
        ? conditionState
        : {};
    return {
        ...source,
...
function getMobileDepotStateFromItem(item) {
    ...
    const state = normalizeMobileDepotState(item.mobileDepotState) ||
...
function updateMobileDepotState(itemID, updater) {
    ...
    return updateInventoryItem(itemID, (currentItem) => ({
        ...
        clientCustomInfo: buildClientCustomInfoForShip(normalizedNextState && normalizedNextState.sourceShipID) ??
...
function isMobileDepotReinforced(itemOrState, nowMs = Date.now()) {
    const state = itemOrState && typeof itemOrState === "object" && itemOrState.depotID
        ? itemOrState
...
function adjustDamageResultForReinforcement(damageResult, nextConditionState) {
    ...
    const afterShield = round6(Math.max(0, toReal(maxLayers.shield, 0)) * MOBILE_DEPOT_REINFORCE_SHIELD_THRESHOLD);
...
function shouldEnterReinforcementFromDamage(damageResult) {
    ...
    return (beforeShield + 1e-6 >= MOBILE_DEPOT_REINFORCE_SHIELD_THRESHOLD &&
...
function getItemMoveVolume(item) {
    ...
    return Math.max(0, toReal(item && item.volume, toReal(metadata.volume, 0)));
}
function getShipMobileDepotHoldCapacity(shipItem) {
    return Math.max(0, toReal(getShipBaseAttributeValue(shipItem && shipItem.typeID, MOBILE_DEPOT_HOLD_CAPACITY_ATTRIBUTE), 0));
}
...
function validateMobileDepotDestinationFlag(item, context, destinationFlagID) {
    ...
    if (destinationFlagID === ITEM_FLAGS.CARGO_HOLD) {
        ...
        const used = getShipFlagUsedVolume(context.characterID, context.shipID, destinationFlagID, item && item.itemID);
...
function collectMobileDepotPlacementBlockers(session, context, options = {}) {
    ...
    const append = (source, kind = null) => {
        ...
        appendMobileDepotPlacementBlocker(blockers, seenKeys, source, {
            kind,
...
function validateMobileDepotSpaceAccess(session, item, state, context, options = {}) {
    const rangeMeters = Math.max(0, toReal(options.rangeMeters, DEPLOY_DISTANCE_METERS));
    ...
    const depotPosition = (depotEntity && depotEntity.position) ||
        (item.spaceState && item.spaceState.position);
    if (!canEntitiesInteractLocally(shipEntity || shipItem, depotEntity || item)) {
...
function activateMobileDepot(itemID) {
    ...
    log.info(`[MobileDepot] Activated depot=${itemID} system=${state.solarSystemID}`);
    return true;
}
function enterMobileDepotReinforcementFromDamage(scene, entity, damageResult, options = {}) {
    ...
    const updateResult = updateInventoryItem(item.itemID, (currentItem) => ({
        ...
        clientCustomInfo: buildClientCustomInfoForShip(nextState.sourceShipID) ??
            currentItem.clientCustomInfo,
        customInfo: buildCustomInfoWithoutState(currentItem.customInfo),
    }));
    if (!updateResult.success) {
    ...
    log.info(`[MobileDepot] Depot=${item.itemID} entered reinforcement until=${nextState.reinforcedUntilMs}`);
    return {
        success: true,
        entered: true,
        data: {
...
function exitMobileDepotReinforcement(itemID, reason = "reinforcement-expired") {
    ...
    if (!updateResult.success) {
        log.warn(`[MobileDepot] Failed to exit reinforcement depot=${item.itemID}: ${updateResult.errorMsg}`);
        return false;
    }
...
function scheduleMobileDepotTimers(itemOrID) {
    ...
    if (state.expiresAtMs > 0) {
        nextTimers.expiryTimer = setUnrefTimeout(() => expireMobileDepot(itemID, "expired"), state.expiresAtMs - now);
    }
    if (!state.active) {
        nextTimers.activationTimer = setUnrefTimeout(() => activateMobileDepot(itemID), state.activateAtMs - now);
    }
    if (state.reinforced === true && state.reinforcedUntilMs > 0) {
        ...
        nextTimers.reinforcementTimer = setUnrefTimeout(() => exitMobileDepotReinforcement(itemID, "reinforcement-expired"), state.reinforcedUntilMs - now);
    }
...
function launchMobileDepotFromShip(session, itemID, options = {}) {
    ...
    const placementConflict = findMobileDepotPlacementConflict(session, item, context, spawnState);
    if (placementConflict) {
        log.info(`[MobileDepot] Launch rejected char=${context.characterID} item=${item.itemID} system=${context.systemID} reason=${placementConflict.errorMsg}`);
        return {
            success: false,
    ...
    if (toInt(item.singleton, 0) !== 1 && toInt(item.stacksize ?? item.quantity, 0) <= 1) {
        ...
        syncInventoryChange(session, assembledResult.data, assembledResult.previousData || item, {
            locationContext: ["Ship", context.shipID, "ShipCargo"],
        });
    }
    const transferResult = transferItemToOwnerLocation(launchSourceItemID, context.ownerID, context.systemID, 0, 1);
    ...
    if (!spawnResult || !spawnResult.success) {
        log.warn(`[MobileDepot] Launched depot ${launchedItemID} but space spawn failed: ${spawnResult ? spawnResult.errorMsg : "UNKNOWN"}`);
    }
    registerMobileDepotItem(updatedResult.data);
    log.info(`[MobileDepot] char=${context.characterID} launched depot itemID=${launchedItemID} typeID=${sourceTypeID} system=${context.systemID}`);
    return {
        success: true,
        data: {
...
function scoopMobileDepotToShipFlag(session, itemID, destinationFlagID) {
    ...
    const destinationValidationError = validateMobileDepotDestinationFlag(item, context, normalizedDestinationFlagID);
    ...
    const updateResult = updateInventoryItem(item.itemID, (currentItem) => ({
        ...
        clientCustomInfo: buildClientCustomInfoForShip(state && state.sourceShipID) ??
    ...
    log.info(`[MobileDepot] char=${context.characterID} scooped depot itemID=${item.itemID} system=${context.systemID} ship=${context.shipID} flag=${normalizedDestinationFlagID}`);
    return {
        success: true,
        data: {
            itemID: item.itemID,
...
//# sourceMappingURL=mobileDepotRuntime.js.map