...
function getCachedRookieShipProfileForRace(raceID, options: Record<string, any> = {}) {
  const numericRaceID = toInt(raceID, 0);
  if (numericRaceID <= 0) {
    return null;
  }
...
function resolveRookieShipProfile(options: Record<string, any> = {}) {
...
function syncInventoryChangesForSession(session, changes: any[] = []) {
...
function boardRookieShipForSession(session, options: Record<string, any> = {}) {
