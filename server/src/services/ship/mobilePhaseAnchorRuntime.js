...
const { ITEM_FLAGS, findItemById, getItemMetadata, transferItemToOwnerLocation, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
...
function isMobilePhaseAnchorType(typeIDOrItem) {
    if (typeIDOrItem && typeof typeIDOrItem === "object") {
        return (toInt(typeIDOrItem.categoryID, 0) === CATEGORY_DEPLOYABLE &&
            toInt(typeIDOrItem.groupID, 0) === GROUP_MOBILE_PHASE_ANCHOR) || isMobilePhaseAnchorType(toInt(typeIDOrItem.typeID, 0));
    }
...
function buildCustomInfoWithState(customInfo, state) {
    ...
    parsed[CUSTOM_INFO_KEY] = {
        ...
        shipEnergy: toInt(state.shipEnergy, 0),
        phasingInCompletionAtMs: toInt(state.phasingInCompletionAtMs, 0),
        chargesRemaining: Math.max(0, toInt(state.chargesRemaining, MOBILE_PHASE_ANCHOR_NUM_CHARGES)),
        nearestPhaseableID: toInt(state.nearestPhaseableID, 0),
        phaseableIsAvailable: state.phaseableIsAvailable === true,
        phaseableIsPhasedIn: state.phaseableIsPhasedIn === true,
        deactivatedAtMs: toInt(state.deactivatedAtMs, 0),
...
function buildPhaseStabilizerSlimValue(state) {
    return [
        ...
        toInt(state && state.phasingInCompletionAtMs, 0) > 0
            ...
            : null,
        Math.max(0, toInt(state && state.chargesRemaining, MOBILE_PHASE_ANCHOR_NUM_CHARGES)),
...
function launchMobilePhaseAnchorFromShip(session, itemID, options = {}) {
    ...
    if (!spawnResult || !spawnResult.success) {
        log.warn(`[MobilePhaseAnchor] Launched anchor ${launchedItemID} but space spawn failed: ${spawnResult ? spawnResult.errorMsg : "UNKNOWN"}`);
    }
    registerMobilePhaseAnchorItem(updatedResult.data);
    log.info(`[MobilePhaseAnchor] char=${context.characterID} launched anchor itemID=${launchedItemID} system=${context.systemID}`);
    return {
        success: true,
        data: {
...
//# sourceMappingURL=mobilePhaseAnchorRuntime.js.map