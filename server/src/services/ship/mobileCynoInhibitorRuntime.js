...
const { ITEM_FLAGS, findItemById, getItemMetadata, listSystemSpaceItems, removeInventoryItem, transferItemToOwnerLocation, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const GROUP_CYNO_INHIBITOR = 1249;
const TYPE_MOBILE_CYNOSURAL_INHIBITOR = 33476;
const TYPE_ANCHORING = 11584;
const TYPE_COVERT_CYNOSURAL_FIELD = 28650;
const DEPLOY_DISTANCE_METERS = 2_500;
const MOBILE_CYNO_INHIBITOR_ACTIVATION_DELAY_MS = 60 * 1000;
const MOBILE_CYNO_INHIBITOR_LIFETIME_MS = 60 * 60 * 1000;
const MOBILE_CYNO_INHIBITOR_RANGE_METERS = 100_000;
const MOBILE_CYNO_INHIBITOR_HIGHSEC_CUTOFF = 0.45;
const MOBILE_CYNO_INHIBITOR_MIN_DISTANCE_FROM_OWN_GROUP_METERS = 200_000;
const MOBILE_CYNO_INHIBITOR_MIN_DISTANCE_FROM_STATION_STARGATE_UPWELL_METERS = 75_000;
const MOBILE_CYNO_INHIBITOR_MIN_DISTANCE_FROM_CONTROL_TOWER_METERS = 40_000;
const CUSTOM_INFO_KEY = "evejsMobileCynoInhibitor";
...
function isMobileCynoInhibitorType(typeIDOrItem) {
    if (typeIDOrItem && typeof typeIDOrItem === "object") {
        return (toInt(typeIDOrItem.categoryID, 0) === CATEGORY_DEPLOYABLE &&
            toInt(typeIDOrItem.groupID, 0) === GROUP_CYNO_INHIBITOR) || toInt(typeIDOrItem.typeID, 0) === TYPE_MOBILE_CYNOSURAL_INHIBITOR;
    }
    return toInt(typeIDOrItem, 0) === TYPE_MOBILE_CYNOSURAL_INHIBITOR;
}
...
function getMobileCynoInhibitorStateFromItem(item) {
    if (!item || !isMobileCynoInhibitorType(item)) {
        return null;
    }
...
function syncInventoryChange(session, item, previousData) {
    ...
    syncInventoryItemForSession(session, item, previousData || {}, { emitCfgLocation: true });
}
...
function getSessionContext(session, options = {}) {
    ...
    const corporationID = toInt(session && (session.corporationID || session.corpid), characterID);
    const behalfOwnerID = toInt(options.ownerID, 0);
    const shipID = toInt(session && session._space && session._space.shipID, toInt(session && (session.shipID || session.shipid || session.activeShipID), 0));
    const systemID = toInt(session && session._space && session._space.systemID, toInt(session && (session.solarsystemid2 || session.solarsystemid), 0));
    return {
        characterID,
        corporationID,
...
function getSkillLevel(characterID, skillTypeID) {
    ...
    return Math.max(0, Math.min(5, toInt(record &&
...
function getHighSecurityDeploymentRestriction(systemID) {
    const system = worldData.getSolarSystemByID(systemID);
    if (!system) {
        return "Solar system data is unavailable for this deployment.";
    }
    if (getSolarSystemSecurity(system) >= MOBILE_CYNO_INHIBITOR_HIGHSEC_CUTOFF) {
        return "Mobile Cynosural Inhibitors cannot be activated in high security space.";
    }
    return null;
}
...
function getObjectItemID(source) {
    return toInt(source && (source.itemID ||
        source.stationID ||
        source.structureID ||
        source.entityID ||
        source.id), 0);
...
function getObjectTypeID(source) {
    return toInt(source && (source.typeID ||
        source.stationTypeID ||
        source.entityTypeID), 0);
}
...
function resolvePlacementBlockerKind(source, explicitKind = null) {
    ...
    if (isMobileCynoInhibitorType(source)) {
        return "cynoInhibitor";
    }
...
function getPlacementBlockerMinimumDistance(kind) {
    if (kind === "cynoInhibitor") {
        return MOBILE_CYNO_INHIBITOR_MIN_DISTANCE_FROM_OWN_GROUP_METERS;
    }
    if (kind === "stationStargateUpwell") {
        return MOBILE_CYNO_INHIBITOR_MIN_DISTANCE_FROM_STATION_STARGATE_UPWELL_METERS;
    }
    if (kind === "controlTower") {
        return MOBILE_CYNO_INHIBITOR_MIN_DISTANCE_FROM_CONTROL_TOWER_METERS;
    }
    return 0;
}
function getPlacementConflictError(kind) {
    if (kind === "cynoInhibitor") {
        return "MOBILE_CYNO_INHIBITOR_TOO_CLOSE_TO_INHIBITOR";
    }
    if (kind === "stationStargateUpwell") {
        return "MOBILE_CYNO_INHIBITOR_TOO_CLOSE_TO_STATION_STARGATE_OR_UPWELL";
    }
    if (kind === "controlTower") {
        return "MOBILE_CYNO_INHIBITOR_TOO_CLOSE_TO_CONTROL_TOWER";
    }
    return "MOBILE_CYNO_INHIBITOR_DEPLOYMENT_BLOCKED";
}
...
function collectPlacementBlockers(session, context, options = {}) {
    ...
    const interactionSource = session && context && context.shipID
        ? getSpaceRuntime().getEntity(session, context.shipID) || findItemById(context.shipID)
        : null;
    const append = (source, kind = null) => {
        if (interactionSource &&
            !canEntitiesInteractLocally(interactionSource, source)) {
            return;
        }
        appendPlacementBlocker(blockers, seenKeys, source, {
            kind,
...
function findPlacementConflict(session, item, context, spawnState) {
    ...
    if (!deployPosition) {
        return {
            errorMsg: "MOBILE_CYNO_INHIBITOR_DEPLOY_POSITION_UNAVAILABLE",
        };
    }
...
function buildDeployableSpawnState(session, context) {
    const shipEntity = session && context.shipID
    ...
    const direction = normalizeDirection(shipEntity && shipEntity.direction, { x: 1, y: 0, z: 0 });
    const deployedPosition = addVectors(position, scaleVector(direction, DEPLOY_DISTANCE_METERS));
    return {
...
function updateMobileCynoInhibitorState(itemID, updater) {
    ...
    if (!currentState) {
        return {
            success: false,
            errorMsg: "MOBILE_CYNO_INHIBITOR_STATE_NOT_FOUND",
        };
    }
    const nextState = typeof updater === "function" ? updater({ ...currentState }, item) : updater;
...
function clearMobileCynoInhibitor(itemOrID, reason = "cleared") {
    ...
    const state = getMobileCynoInhibitorStateFromItem(item);
    if (!state) {
        return false;
    }
    if (findItemById(itemID)) {
        updateMobileCynoInhibitorState(itemID, (currentState) => ({
            ...currentState,
            active: false,
            deactivatedAtMs: Date.now(),
        }));
    }
    log.info(`[CynoInhibitor] Cleared inhibitor=${itemID} reason=${reason}`);
    return true;
}
function activateMobileCynoInhibitor(itemID) {
    const item = findItemById(itemID);
    const state = getMobileCynoInhibitorStateFromItem(item);
    if (!state) {
        clearTimers(itemID);
    ...
    if (state.expiresAtMs > 0 && state.expiresAtMs <= now) {
        expireMobileCynoInhibitor(itemID, "expired-before-activation");
        return false;
    }
    if (toInt(item.locationID, 0) !== state.solarSystemID ||
        toInt(item.flagID, 0) !== 0 ||
        !item.spaceState) {
        clearMobileCynoInhibitor(item, "removed-before-activation");
        return false;
    }
    const highSecurityRestriction = getHighSecurityDeploymentRestriction(state.solarSystemID);
    if (highSecurityRestriction) {
        updateMobileCynoInhibitorState(itemID, (currentState) => ({
            ...currentState,
            active: false,
            activationBlockedReason: "HIGH_SECURITY_SPACE",
            activationBlockedAtMs: now,
        }));
        getSpaceRuntime().refreshInventoryBackedEntityPresentation(state.solarSystemID, itemID);
        scheduleMobileCynoInhibitorTimers(itemID);
        log.info(`[CynoInhibitor] Activation blocked inhibitor=${itemID} system=${state.solarSystemID} reason=highsec`);
        return false;
    }
    updateMobileCynoInhibitorState(itemID, (currentState) => ({
        ...currentState,
    ...
    getSpaceRuntime().refreshInventoryBackedEntityPresentation(state.solarSystemID, itemID);
    scheduleMobileCynoInhibitorTimers(itemID);
    log.info(`[CynoInhibitor] Activated inhibitor=${itemID} system=${state.solarSystemID}`);
    return true;
}
function expireMobileCynoInhibitor(itemID, reason = "expired") {
    const item = findItemById(itemID);
    const state = getMobileCynoInhibitorStateFromItem(item);
    ...
    if (systemID > 0) {
        ...
        if (destroyResult && destroyResult.success) {
            log.info(`[CynoInhibitor] Removed inhibitor=${itemID} reason=${reason}`);
            return true;
        }
    }
...
function scheduleMobileCynoInhibitorTimers(itemOrID) {
    const itemID = toInt(itemOrID && itemOrID.itemID ? itemOrID.itemID : itemOrID, 0);
    const item = itemOrID && itemOrID.itemID ? itemOrID : findItemById(itemID);
    const state = getMobileCynoInhibitorStateFromItem(item);
    if (!state) {
    ...
    if (state.expiresAtMs > 0) {
        nextTimers.expiryTimer = setUnrefTimeout(() => expireMobileCynoInhibitor(itemID, "expired"), state.expiresAtMs - now);
    }
    ...
    if (state.active || state.activateAtMs <= now) {
        if (!state.active) {
            nextTimers.activationTimer = setUnrefTimeout(() => activateMobileCynoInhibitor(itemID), 0);
        }
    }
    else {
        nextTimers.activationTimer = setUnrefTimeout(() => activateMobileCynoInhibitor(itemID), state.activateAtMs - now);
    }
    timersByInhibitorID.set(itemID, nextTimers);
    return true;
}
function registerMobileCynoInhibitorItem(itemOrID) {
    return scheduleMobileCynoInhibitorTimers(itemOrID);
}
function isActiveInhibitorState(state, nowMs = Date.now()) {
    if (!state || state.deactivatedAtMs > 0 || state.activationBlockedReason) {
        return false;
    }
...
function hydrateMobileCynoInhibitorEntityFromInventoryItem(entity, itemRecord) {
    if (!entity || !isMobileCynoInhibitorType(itemRecord)) {
        return entity;
    }
    const state = getMobileCynoInhibitorStateFromItem(itemRecord);
    if (!state) {
        return entity;
    }
    const active = isActiveInhibitorState(state, Date.now());
    entity.component_activate = [active, active ? null : state.activateAtMs];
    entity.activate_comp_durationSeconds = Math.trunc(MOBILE_CYNO_INHIBITOR_ACTIVATION_DELAY_MS / 1000);
    entity.cynoInhibitorRangeMeters = MOBILE_CYNO_INHIBITOR_RANGE_METERS;
    return entity;
}
...
function validateMobileCynoInhibitorLaunch(item, context) {
    ...
    if (!isMobileCynoInhibitorType(item)) {
        return "ITEM_NOT_MOBILE_CYNO_INHIBITOR";
    }
    if (![context.characterID, context.corporationID, context.ownerID].includes(toInt(item.ownerID, 0)) ||
    ...
    if (getSkillLevel(context.characterID, TYPE_ANCHORING) < 3) {
        return "Mobile Cynosural Inhibitor deployment requires Anchoring III.";
    }
    const highSecurityRestriction = getHighSecurityDeploymentRestriction(context.systemID);
    if (highSecurityRestriction) {
        return highSecurityRestriction;
    }
    return null;
}
function launchMobileCynoInhibitorFromShip(session, itemID, options = {}) {
    const context = getSessionContext(session, options);
    const item = findItemById(itemID);
    const validationError = validateMobileCynoInhibitorLaunch(item, context);
    if (validationError) {
        return {
            success: false,
    ...
    if (placementConflict) {
        log.info(`[CynoInhibitor] Launch rejected char=${context.characterID} item=${item.itemID} system=${context.systemID} reason=${placementConflict.errorMsg}`);
        return {
            success: false,
    ...
    const transferResult = transferItemToOwnerLocation(item.itemID, context.ownerID, context.systemID, 0, 1);
    ...
    const transferChanges = transferResult.data && transferResult.data.changes
        ...
        : [];
    const launchedChange = findLaunchedChange(transferChanges, context.systemID, context.ownerID, sourceTypeID);
    ...
    const state = {
        ...
        deployedAtMs: now,
        activateAtMs: now + MOBILE_CYNO_INHIBITOR_ACTIVATION_DELAY_MS,
        activatedAtMs: 0,
        expiresAtMs: now + MOBILE_CYNO_INHIBITOR_LIFETIME_MS,
        active: false,
        deactivatedAtMs: 0,
    };
    ...
    syncInventoryChange(session, updatedResult.data, launchedChange.previousData || updatedResult.previousData || {});
    ...
    if (!spawnResult || !spawnResult.success) {
        log.warn(`[CynoInhibitor] Launched inhibitor ${launchedItemID} but space spawn failed: ${spawnResult ? spawnResult.errorMsg : "UNKNOWN"}`);
    }
    registerMobileCynoInhibitorItem(updatedResult.data);
    log.info(`[CynoInhibitor] char=${context.characterID} launched inhibitor itemID=${launchedItemID} system=${context.systemID}`);
    return {
        success: true,
        data: {
...
function findActiveInhibitorConflict(systemID, position, options = {}) {
    if (toInt(options.fieldTypeID, 0) === TYPE_COVERT_CYNOSURAL_FIELD) {
        return null;
    }
    const normalizedSystemID = toInt(systemID, 0);
    const targetPosition = getObjectPosition({ position });
    if (normalizedSystemID <= 0 || !targetPosition) {
        return null;
    }
    ...
    const appendCandidate = (source) => {
        if (options.targetEntity &&
            !canEntitiesInteractLocally(options.targetEntity, source)) {
            return;
        }
        const itemID = getObjectItemID(source);
    ...
    for (const candidate of candidates) {
        const itemID = getObjectItemID(candidate);
        const item = itemID > 0 ? (findItemById(itemID) || candidate) : candidate;
        const state = getMobileCynoInhibitorStateFromItem(item);
        if (!isActiveInhibitorState(state, now)) {
            continue;
        }
        const inhibitorPosition = getObjectPosition(item) || getObjectPosition(candidate);
        if (!inhibitorPosition) {
            continue;
        }
        const distance = getVectorDistance(targetPosition, inhibitorPosition);
        if (distance <= MOBILE_CYNO_INHIBITOR_RANGE_METERS) {
            return {
                inhibitorID: itemID,
                distance,
                rangeMeters: MOBILE_CYNO_INHIBITOR_RANGE_METERS,
            };
        }
    }
    return null;
...
module.exports = {
    CATEGORY_DEPLOYABLE,
    GROUP_CYNO_INHIBITOR,
    TYPE_MOBILE_CYNOSURAL_INHIBITOR,
    MOBILE_CYNO_INHIBITOR_ACTIVATION_DELAY_MS,
    MOBILE_CYNO_INHIBITOR_LIFETIME_MS,
    MOBILE_CYNO_INHIBITOR_RANGE_METERS,
    isMobileCynoInhibitorType,
    launchMobileCynoInhibitorFromShip,
    registerMobileCynoInhibitorItem,
    clearMobileCynoInhibitor,
    getMobileCynoInhibitorStateFromItem,
    hydrateMobileCynoInhibitorEntityFromInventoryItem,
    findActiveInhibitorConflict,
    _testing: {
        buildDeployableSpawnState,
        buildCustomInfoWithState,
        getStateFromCustomInfo,
        findPlacementConflict,
        getHighSecurityDeploymentRestriction,
        getSkillLevel,
    },
};
//# sourceMappingURL=mobileCynoInhibitorRuntime.js.map