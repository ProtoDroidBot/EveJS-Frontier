...
const warpDisruptFieldGeneratorRuntime = require(path.join(__dirname, "./warpDisruptFieldGeneratorRuntime"));
...
function isMobileWarpDisruptorType(typeIDOrItem) {
    if (typeIDOrItem && typeof typeIDOrItem === "object") {
        return (toInt(typeIDOrItem.categoryID, 0) === CATEGORY_DEPLOYABLE &&
            toInt(typeIDOrItem.groupID, 0) === GROUP_MOBILE_WARP_DISRUPTOR) || isMobileWarpDisruptorType(toInt(typeIDOrItem.typeID, 0));
    }
...
function getAnchoringDelayMs(typeID) {
    return Math.max(0, toInt(getTypeDogmaAttributeValue(typeID, ATTRIBUTE_ANCHORING_DELAY, 0), 0));
}
...
function getEmpireSpaceDeploymentRestriction(systemID, typeID) {
    if (getTypeDogmaAttributeValue(typeID, ATTRIBUTE_DISALLOW_IN_EMPIRE_SPACE, 0) <= 0) {
        return null;
    }
...
function activateMobileWarpDisruptor(itemID) {
    ...
    log.info(`[MobileWarpDisruptor] Activated disruptor=${itemID} system=${state.solarSystemID} range=${state.rangeMeters}`);
    return true;
}
...
function scheduleMobileWarpDisruptorTimers(itemOrID) {
    ...
    if (state.expiresAtMs > 0) {
        nextTimers.expiryTimer = setUnrefTimeout(() => expireMobileWarpDisruptor(itemID, "expired"), state.expiresAtMs - now);
    }
    if (state.active || state.activateAtMs <= now) {
        if (!state.active) {
            nextTimers.activationTimer = setUnrefTimeout(() => activateMobileWarpDisruptor(itemID), 0);
        }
    }
    else {
        nextTimers.activationTimer = setUnrefTimeout(() => activateMobileWarpDisruptor(itemID), state.activateAtMs - now);
    }
...
function hydrateMobileWarpDisruptorEntityFromInventoryItem(entity, itemRecord) {
    ...
    entity.activate_comp_durationSeconds = Math.trunc(Math.max(0, state.anchoringDelayMs) / 1000);
    entity.warpDisruptionRangeMeters = Math.max(0, toReal(state.rangeMeters, getWarpDisruptionRangeMeters(itemRecord.typeID)));
    return entity;
}
...
function launchMobileWarpDisruptorFromShip(session, itemID, options = {}) {
    ...
    if (!spawnResult || !spawnResult.success) {
        log.warn(`[MobileWarpDisruptor] Launched disruptor ${launchedItemID} but space spawn failed: ${spawnResult ? spawnResult.errorMsg : "UNKNOWN"}`);
    }
    registerMobileWarpDisruptorItem(updatedResult.data);
    log.info(`[MobileWarpDisruptor] char=${context.characterID} launched disruptor itemID=${launchedItemID} system=${context.systemID} range=${state.rangeMeters} activateDelayMs=${anchoringDelayMs}`);
    return {
        success: true,
        data: {
...
function collectActiveWarpDisruptors(systemID, options = {}) {
    ...
    const appendCandidate = (source) => {
        ...
        const interdictionProbeCandidate = interdictionProbeRuntime.buildActiveWarpDisruptorCandidate(source, {
            nowMs: now,
        });
        if (pushCandidate(itemID, interdictionProbeCandidate)) {
            return;
        }
        const fieldGeneratorCandidate = warpDisruptFieldGeneratorRuntime.buildActiveWarpDisruptorCandidate(source, {
            nowMs: now,
        });
        ...
        const rangeMeters = toReal(state && state.rangeMeters, getWarpDisruptionRangeMeters(getObjectTypeID(item) || getObjectTypeID(source)));
...
function findActiveWarpDisruptorForEntity(systemID, entity, options = {}) {
    ...
    return findActiveWarpDisruptorForPosition(systemID, position, {
        ...options,
        targetEntity: entity,
    });
}
...
//# sourceMappingURL=mobileWarpDisruptorRuntime.js.map