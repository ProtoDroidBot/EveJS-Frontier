...
function giveSkin(charId, skinID, options: Record<string, any> = {}) {
  const catalog = readCatalog();
  const skinEntry = getSkinCatalogEntry(skinID, catalog);
  if (!skinEntry) {
    return {
...
function grantAllSkinsToCharacter(charId, options: Record<string, any> = {}) {
  const numericCharID = Number(charId || 0) || 0;
  if (!numericCharID) {
    return {
      success: false as const,
  ...
  const runtimeRoot = ensureRuntimeRoot();
  const characterEntry = getCharacterRuntimeEntry(runtimeRoot, numericCharID);
  const skinEntries = Object.values<any>(catalog.skinsBySkinID || {})
...
function applySkinToShip(shipID, skinID, options: Record<string, any> = {}) {
...
function getAppliedSkinRecordsForOwner(ownerID) {
  ...
  const runtimeRoot = ensureRuntimeRoot();
  return Object.values<any>(runtimeRoot.ships)
    .filter(
      (record) =>
