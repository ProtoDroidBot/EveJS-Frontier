...
function normalizeTable(rawTable: Record<string, any> = {}) {
  return {
    _meta: {
      schemaVersion: 1,
...
function normalizeShipDirtRecord(rawRecord: Record<string, any> = {}, itemID = 0) {
...
function getShipDirtRecord(itemID, options: Record<string, any> = {}) {
...
function getShipDirtTimestamp(itemID, options: Record<string, any> = {}) {
  const record = getShipDirtRecord(itemID, options);
  return record ? normalizeFiletime(record.dirtTime, 0n) || 0n : 0n;
}

function resetShipDirtTimestamp(itemID, rawTimestamp = null, options: Record<string, any> = {}) {
  const numericItemID = toPositiveInt(itemID, 0);
  if (!numericItemID) {
    return {
