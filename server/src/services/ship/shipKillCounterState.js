...
function normalizeTable(rawTable = {}) {
    return {
        ...
        ships: rawTable && rawTable.ships && typeof rawTable.ships === "object"
            ? rawTable.ships
            : {},
    };
}
function normalizeShipCounterRecord(rawRecord = {}, itemID = 0) {
    const numericItemID = toPositiveInt(rawRecord && rawRecord.itemID, toPositiveInt(itemID, 0));
    return {
        ...
        lastAward: rawRecord && rawRecord.lastAward && typeof rawRecord.lastAward === "object"
...
function writeShipCounterRecord(record) {
    ...
    const writeResult = database.write(SHIP_KILL_COUNTER_TABLE, `/ships/${String(record.itemID)}`, record);
...
function readShipCounterRecord(itemID) {
    ...
    const table = ensureShipKillCounterTable();
    return normalizeShipCounterRecord(table.ships[String(numericItemID)] || {}, numericItemID);
}
...
function incrementItemKillCountPlayer(itemID, options = {}) {
    ...
    const record = normalizeShipCounterRecord({
        ...
        updatedAt: now,
        lastAward: options.lastAward && typeof options.lastAward === "object"
            ? options.lastAward
            : {
                awardedAt: now,
...
function clearShipKillCounter(itemID, reason = "clear") {
    ...
    const removeResult = database.remove(SHIP_KILL_COUNTER_TABLE, `/ships/${String(numericItemID)}`);
    return {
...
//# sourceMappingURL=shipKillCounterState.js.map