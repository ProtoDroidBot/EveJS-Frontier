...
const {
  ...
  __dirname,
  "../../space/destiny/commands/mobileTractorUnit.js",
));
...
function syncInventoryChange(session, item, previousData, options: Record<string, any> = {}) {
...
function listInterestedSessions(scene, characterIDs: any[] = []) {
...
function syncInventoryChangesToInterestedSessions(scene, characterIDs: any[] = [], changes: any[] = [], options: Record<string, any> = {}) {
...
function validateMobileTractorUnitSpaceAccess(session, item, state, context, options: Record<string, any> = {}) {
...
function launchMobileTractorUnitFromShip(session, itemID, options: Record<string, any> = {}) {
...
function broadcastMobileTractorUnitTractorStart(scene, entity, targetEntity, state, nowMs) {
  ...
  if (typeof scene.broadcastMovementUpdates === "function") {
    scene.broadcastMovementUpdates(
      tractorStartCommand.buildPresentationUpdates({ stamp }),
    );
  }
...
function processMobileTractorUnitCycle(scene, entity, item, state, nowMs) {
  ...
  const tractorPresentation =
    targetEntity
      ...
      : null;
  let transferResult: Record<string, any> = {
    success: true,
    changes: [],
    movedItemCount: 0,
  };
