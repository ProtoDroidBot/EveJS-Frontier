...
function isShipServiceFlag(flagID) {
    ...
    return (numericFlagID === ITEM_FLAGS.SHIP_HANGAR ||
...
function getSessionCorporationID(session) {
    return toInt(session && (session.corporationID ??
        session.corpid), 0);
}
function getOwnerCorporationID(ownerID) {
    ...
    const ownerCharacter = characterResult &&
        characterResult.success &&
        characterResult.data &&
        typeof characterResult.data === "object"
        ? characterResult.data
        : null;
    return toInt(ownerCharacter && (ownerCharacter.corporationID ??
        ownerCharacter.corpid), numericOwnerID);
}
...
function canUseShipServiceFlag(session, shipItem, serviceFlag) {
    ...
    if (session && session._space) {
        ...
        const activeShipEntity = spaceRuntime.getEntity(session, session._space.shipID);
        const serviceShipEntity = spaceRuntime.getEntity(session, shipItem.itemID);
        if (!activeShipEntity ||
            !serviceShipEntity ||
            !canEntitiesInteractLocally(activeShipEntity, serviceShipEntity)) {
            return false;
        }
    }
    const characterID = getSessionCharacterID(session);
    ...
    const inSameCorp = callerCorporationID > 0 &&
    ...
    if (numericFlagID === ITEM_FLAGS.FLEET_HANGAR) {
        return ((config.FleetHangar_AllowCorpAccess && inSameCorp) ||
            (config.FleetHangar_AllowFleetAccess && inSameFleet));
    }
    if (numericFlagID === ITEM_FLAGS.SHIP_HANGAR) {
        return ((config.SMB_AllowCorpAccess && inSameCorp) ||
...
//# sourceMappingURL=shipServiceAccess.js.map