...
function isMobileMicroJumpUnitType(typeIDOrItem) {
    if (typeIDOrItem && typeof typeIDOrItem === "object") {
        return (toInt(typeIDOrItem.categoryID, 0) === CATEGORY_DEPLOYABLE &&
            toInt(typeIDOrItem.groupID, 0) === GROUP_MOBILE_MICRO_JUMP_UNIT) || toInt(typeIDOrItem.typeID, 0) === TYPE_MOBILE_MICRO_JUMP_UNIT;
    }
...
function getStateFromCustomInfo(customInfo) {
    ...
    return {
        ...
        lastJumpStartedAtFileTime: state.lastJumpStartedAtFileTime === undefined ||
...
function activateMobileMicroJumpUnit(itemID) {
    ...
    log.info(`[MobileMicroJumpUnit] Activated unit=${itemID} system=${state.solarSystemID}`);
    return true;
}
...
function scheduleMobileMicroJumpUnitTimers(itemOrID) {
    ...
    if (state.expiresAtMs > 0) {
        nextTimers.expiryTimer = setUnrefTimeout(() => expireMobileMicroJumpUnit(itemID, "expired"), state.expiresAtMs - now);
    }
    if (state.active || state.activateAtMs <= now) {
        if (!state.active) {
            nextTimers.activationTimer = setUnrefTimeout(() => activateMobileMicroJumpUnit(itemID), 0);
        }
    }
    else {
        nextTimers.activationTimer = setUnrefTimeout(() => activateMobileMicroJumpUnit(itemID), state.activateAtMs - now);
    }
...
function hydrateMobileMicroJumpUnitEntityFromInventoryItem(entity, itemRecord) {
    ...
    entity.activate_comp_durationSeconds = Math.trunc(MOBILE_MICRO_JUMP_UNIT_ACTIVATION_DELAY_MS / 1000);
...
function launchMobileMicroJumpUnitFromShip(session, itemID, options = {}) {
    ...
    if (placementConflict) {
        log.info(`[MobileMicroJumpUnit] Launch rejected char=${context.characterID} item=${item.itemID} system=${context.systemID} reason=${placementConflict.errorMsg}`);
        return {
            success: false,
    ...
    if (!spawnResult || !spawnResult.success) {
        log.warn(`[MobileMicroJumpUnit] Launched unit ${launchedItemID} but space spawn failed: ${spawnResult ? spawnResult.errorMsg : "UNKNOWN"}`);
    }
    registerMobileMicroJumpUnitItem(updatedResult.data);
    log.info(`[MobileMicroJumpUnit] char=${context.characterID} launched unit itemID=${launchedItemID} system=${context.systemID}`);
    return {
        success: true,
        data: {
...
function resolveShipMassKg(shipEntity, shipItem = null) {
    ...
    return Math.max(0, toReal(shipEntity && shipEntity.mass, 0), toReal(shipItem && shipItem.mass, 0), toReal(metadata && metadata.mass, 0));
}
...
function validateShipCanUseUnit(session, unitEntity, scene) {
    ...
    if (!canEntitiesInteractLocally(shipEntity, unitEntity)) {
        return {
            success: false,
            errorMsg: "MOBILE_MICRO_JUMP_UNIT_NOT_FOUND",
        };
...
function completeMobileMicroJumpSpool(session, shipID, unitID) {
    ...
    if (!unitResult.success) {
        log.info(`[MobileMicroJumpUnit] Spool cancelled ship=${shipID} unit=${unitID} reason=${unitResult.errorMsg}`);
        return false;
    }
    ...
    if (!shipResult.success && shipResult.errorMsg !== "SHIP_TOO_FAR_FROM_MOBILE_MICRO_JUMP_UNIT") {
        log.info(`[MobileMicroJumpUnit] Spool cancelled ship=${shipID} unit=${unitID} reason=${shipResult.errorMsg}`);
        return false;
    }
    const shipEntity = scene.getShipEntityForSession(session);
    if (!shipEntity || toInt(shipEntity.itemID, 0) !== normalizedShipID) {
        log.info(`[MobileMicroJumpUnit] Spool cancelled ship=${shipID} unit=${unitID} reason=SHIP_CHANGED`);
        return false;
    }
    if (isShipCloaked(shipEntity) || hostileModuleRuntime.isMicroJumpDriveBlocked(shipEntity)) {
        log.info(`[MobileMicroJumpUnit] Spool cancelled ship=${shipID} unit=${unitID} reason=SHIP_BLOCKED`);
        return false;
    }
    ...
    if (!jumpResult || !jumpResult.success) {
        log.warn(`[MobileMicroJumpUnit] Jump failed ship=${shipID} unit=${unitID}: ${jumpResult ? jumpResult.errorMsg : "UNKNOWN"}`);
        return false;
    }
...
function startMobileMicroJumpDriveForShip(session, unitID) {
    const unitResult = resolveActiveUnit(session, unitID);
    if (!unitResult.success) {
        log.info(`[MobileMicroJumpUnit] Activation rejected char=${session && session.characterID} unit=${unitID} reason=${unitResult.errorMsg}`);
        return {
            success: false,
    ...
    if (!shipResult.success) {
        log.info(`[MobileMicroJumpUnit] Activation rejected char=${session && session.characterID} unit=${unitID} reason=${shipResult.errorMsg}`);
        return shipResult;
    }
    ...
    const timer = setUnrefTimeout(() => completeMobileMicroJumpSpool(session, shipID, unitID), MOBILE_MICRO_JUMP_UNIT_SPOOLUP_MS);
    ...
    log.info(`[MobileMicroJumpUnit] Started spool ship=${shipID} unit=${unitID} char=${session && session.characterID}`);
    return {
        success: true,
        data: {
            shipID,
...
//# sourceMappingURL=mobileMicroJumpUnitRuntime.js.map