...
const { isCapsuleTypeID, moveShipToSpace, } = require(path.join(__dirname, "../inventory/itemStore"));
const { destroyShipEntityWithWreck, } = require(path.join(__dirname, "../../space/shipDestruction"));
const { boardSpaceShip, } = require(path.join(__dirname, "../../space/transitions"));
...
function buildStoppedSpawnStateNearEntity(entity, offsetMeters = DEFAULT_SPACE_SWAP_OFFSET_METERS) {
    ...
    const position = addVectors(cloneVector(entity && entity.position), scaleVector(direction, offset));
    return {
        ...buildChildEntityScopeMetadata(entity),
        position,
...
function destroyAbandonedShipForSwap(session, shipRecord) {
    ...
    const systemID = toInt(session._space.systemID, toInt(shipRecord.locationID, 0));
...
function boardPreparedShipInSpace(session, shipItem) {
    ...
    const moveResult = moveShipToSpace(shipItem.itemID, session._space.systemID, buildStoppedSpawnStateNearEntity(anchorEntity));
    if (!moveResult.success) {
        return moveResult;
    }
    const spawnResult = spaceRuntime.spawnDynamicInventoryEntity(session._space.systemID, shipItem.itemID, {
...
//# sourceMappingURL=spaceShipSwapRuntime.js.map