...
const { buildDict, buildKeyVal, buildList, buildFiletimeLong, extractList, extractDictEntries, normalizeNumber, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getAllLicensedSkinRecords, getLicensedSkinRecordsForType, getEffectiveLicenseRecord, getAppliedSkinRecord, getLicenseCatalogEntry, getSkinMaterialSetIDForSkin, giveSkin, removeSkin, expireSkin, applySkinToShip, } = require(path.join(__dirname, "./shipCosmeticsState"));
const { consumeInventoryItemQuantity, findItemById, } = require(path.join(__dirname, "../inventory/itemStore"));
const { getEnabledCosmeticsEntries, } = require(path.join(__dirname, "./shipLogoFittingState"));
const { publishShipStateSetNotice, } = require(path.join(__dirname, "../../_secondary/express/publicGatewayLocal"));
...
function resolveOwnedShipItem(shipID, characterID, options = {}) {
    ...
    const resolveItem = typeof options.findItemById === "function"
...
function resolveAppliedSkinLicenseRecord(licenseeID, itemID, hullTypeID, options = {}) {
    const numericItemID = Number(itemID || 0) || 0;
    if (numericItemID <= 0) {
        return null;
    }
    const resolveAppliedRecord = typeof options.getAppliedSkinRecord === "function"
        ? options.getAppliedSkinRecord
        : getAppliedSkinRecord;
    const resolveItem = typeof options.findItemById === "function"
        ? options.findItemById
        : findItemById;
    const resolveLicense = typeof options.getEffectiveLicenseRecord === "function"
    ...
    if ((storedTypeID > 0 && itemTypeID > 0 && storedTypeID !== itemTypeID) ||
        (requestedTypeID > 0 &&
    ...
    if (requestedLicenseeID > 0 &&
        appliedOwnerIDs.size > 0 &&
        !appliedOwnerIDs.has(requestedLicenseeID)) {
        return null;
    }
    const effectiveLicenseeID = requestedLicenseeID || appliedOwnerIDs.values().next().value || 0;
...
function normalizeItemIDs(rawValue) {
    const directList = extractList(rawValue)
        ...
        return normalizeNumber((value && value.itemID) ??
    ...
    if (dictEntries.length > 0) {
        const itemIDs = dictEntries
            ...
            return normalizeNumber((row && row.itemID) ??
    ...
    if (normalizedValue && typeof normalizedValue === "object" && !Array.isArray(normalizedValue)) {
        const objectValues = Object.entries(normalizedValue)
            .map(([entryKey, row]) => normalizeNumber(row && (row.itemID ?? (row.fields && row.fields.itemID)), normalizeNumber(entryKey, 0)))
            .filter((value) => value > 0);
...
class ShipCosmeticsMgrService extends BaseService {
    ...
    Handle_GetEnabledCosmetics(args, session, kwargs) {
        ...
        return buildDict(getEnabledCosmeticsEntries(shipID).map((entry) => [
            entry.backendSlot,
            entry.cosmeticType,
        ]));
    }
    Handle_GetLicencedSkins(args, session, ..._extra) {
        ...
        log.debug(`[ShipCosmeticsMgr] GetLicencedSkins(charID=${charId}) -> ${licensed.length}`);
    ...
    Handle_GetLicencedSkinsForShipType(args, session, ..._extra) {
        const argCount = Array.isArray(args) ? args.length : 0;
        const sessionCharacterID = Number(session && session.characterID) || 0;
        const charId = argCount >= 2
            ? Number(args[0] || 0) || 0
            : sessionCharacterID;
        const shipTypeID = argCount >= 2
            ? Number(args[1] || 0) || 0
            : Number(argCount >= 1 ? args[0] : 0) || 0;
        const licensed = getLicensedSkinRecordsForType(charId, shipTypeID).map(buildLicensedSkinKeyVal);
        log.debug(`[ShipCosmeticsMgr] GetLicencedSkinsForShipType(charID=${charId}, shipTypeID=${shipTypeID}) -> ${licensed.length}`);
        return buildList(licensed);
    }
    ...
    Handle_GetFirstPartySkinData(args) {
        ...
        log.debug(`[ShipCosmeticsMgr] GetFirstPartySkinData(licenseeID=${licenseeID}, skinID=${skinID}) -> ${record ? "hit" : "miss"}`);
    ...
    Handle_GetAppliedSkin(args) {
        ...
        const record = resolveAppliedSkinLicenseRecord(licenseeID, itemID, hullTypeID);
        log.debug(`[${this.name}] GetAppliedSkin(licenseeID=${licenseeID}, itemID=${itemID}, ` +
    ...
    Handle_ApplySkinToShip(args, session) {
        ...
        const skinID = args && args.length > 1 && args[1] !== undefined && args[1] !== null
            ? Number(args[1] || 0) || 0
            : null;
        ...
        if (finalResult.success) {
            publishShipStateSetNotice(shipID, activeCharacterID ||
        ...
        log.info(`[ShipCosmeticsMgr] ApplySkinToShip(shipID=${shipID}, skinID=${skinID || 0}) -> ${finalResult.success ? "ok" : finalResult.errorMsg}`);
        if (!finalResult.success) {
    ...
    Handle_GiveSkin(args, session, kwargs) {
        ...
        log.info(`[ShipCosmeticsMgr] GiveSkin(charID=${charId}, skinID=${skinID}, durationDays=${durationDays}, isSingleUse=${isSingleUse}) -> ${result.success ? "ok" : result.errorMsg}`);
        return null;
    }
    Handle_RemoveSkin(args) {
        ...
        log.info(`[ShipCosmeticsMgr] RemoveSkin(licenseeID=${licenseeID}, skinID=${skinID}) -> ${result.success ? "ok" : result.errorMsg}`);
        return null;
    }
    Handle_GMExpireSkinLicense(args, session) {
        ...
        log.info(`[ShipCosmeticsMgr] GMExpireSkinLicense(charID=${charId}, skinID=${skinID}) -> ${result.success ? "ok" : result.errorMsg}`);
        return null;
    }
    Handle_ActivateSkinLicense(args, session) {
        ...
        if (itemIDs.length === 0) {
            log.warn(`[ShipCosmeticsMgr] ActivateSkinLicense(charID=${characterID}) -> no itemIDs resolved from payloadType=${rawItemsArg && rawItemsArg.type ? rawItemsArg.type : typeof rawItemsArg}`);
            return null;
        }
        log.info(`[ShipCosmeticsMgr] ActivateSkinLicense(charID=${characterID}) resolving itemIDs=${itemIDs.join(",")}`);
        ...
        for (const entry of activationPlan) {
            ...
            if (!grantResult.success) {
                log.warn(`[ShipCosmeticsMgr] ActivateSkinLicense(charID=${characterID}, itemID=${entry.item.itemID}, skinID=${entry.skinID}) grant failed=${grantResult.errorMsg}`);
            ...
            if (!consumeResult.success) {
                log.warn(`[ShipCosmeticsMgr] ActivateSkinLicense(charID=${characterID}, itemID=${entry.item.itemID}, skinID=${entry.skinID}) consume failed=${consumeResult.errorMsg}`);
        ...
        log.info(`[ShipCosmeticsMgr] ActivateSkinLicense(charID=${characterID}, itemCount=${activationPlan.length}) -> ok`);
        return null;
    }
}
...
//# sourceMappingURL=shipCosmeticsMgrService.js.map