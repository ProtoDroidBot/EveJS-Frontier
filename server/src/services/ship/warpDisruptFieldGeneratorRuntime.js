...
function isWarpDisruptFieldGeneratorType(typeIDOrItem) {
    if (typeIDOrItem && typeof typeIDOrItem === "object") {
        return (toInt(typeIDOrItem.categoryID, 0) === CATEGORY_MODULE &&
            toInt(typeIDOrItem.groupID, 0) === GROUP_WARP_DISRUPT_FIELD_GENERATOR) || isWarpDisruptFieldGeneratorType(toInt(typeIDOrItem.typeID, 0));
    }
...
function isWarpDisruptionScriptType(typeIDOrItem) {
    if (typeIDOrItem && typeof typeIDOrItem === "object") {
        return (toInt(typeIDOrItem.categoryID, 0) === CATEGORY_CHARGE &&
            toInt(typeIDOrItem.groupID, 0) === GROUP_WARP_DISRUPTION_SCRIPT) || isWarpDisruptionScriptType(toInt(typeIDOrItem.typeID, 0));
    }
    const typeID = toInt(typeIDOrItem, 0);
    if (typeID <= 0) {
        return false;
...
function getFocusedWarpDisruptionScriptMode(typeIDOrItem) {
    const typeID = toInt(typeIDOrItem && typeof typeIDOrItem === "object"
        ? typeIDOrItem.typeID
        : typeIDOrItem, 0);
    switch (typeID) {
...
function getWarpDisruptionRangeMeters(typeID, effectState = null) {
    const overrides = effectState &&
        effectState.genericAttributeOverrides &&
        typeof effectState.genericAttributeOverrides === "object"
        ? effectState.genericAttributeOverrides
        : null;
    const overriddenRange = overrides
        ? toReal(overrides[String(ATTRIBUTE_WARP_SCRAMBLE_RANGE)] ??
            overrides[ATTRIBUTE_WARP_SCRAMBLE_RANGE], 0)
        : 0;
...
function getActivationRestriction(systemID, moduleItem, loadedCharge = null) {
    ...
    if (getTypeDogmaAttributeValue(moduleTypeID, ATTRIBUTE_DISALLOW_IN_EMPIRE_SPACE, 0) > 0) {
    ...
    if (toInt(systemID, 0) === ZARZAKH_SOLAR_SYSTEM_ID &&
        getTypeDogmaAttributeValue(moduleTypeID, ATTRIBUTE_DISALLOW_IN_HAZARD_SYSTEM, 0) > 0) {
...
function buildActiveWarpDisruptorCandidate(entity, options = {}) {
    ...
    return {
        ...
        ownerID: toInt(bestEffect.ownerID, toInt(entity.ownerID, toInt(entity.characterID || entity.pilotCharacterID, 0))),
        sourceShipID: toInt(bestEffect.sourceShipID, toInt(entity.sourceShipID, toInt(entity.itemID, 0))),
...
//# sourceMappingURL=warpDisruptFieldGeneratorRuntime.js.map