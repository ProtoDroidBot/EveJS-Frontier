...
const { ITEM_FLAGS, findItemById, createSpaceItemForCharacter, moveItemToLocation, moveItemsToLocations, removeInventoryItem, listContainerItems, } = require(path.join(__dirname, "../inventory/itemStore"));
const { resolveItemByName, } = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const { isDisposableJetcanRecord, } = require(path.join(__dirname, "../inventory/cargoLootAuthority"));
...
function getJetcanSessionLootContext(session) {
    ...
    return {
        corporationID: Number(session && (session.corporationID ||
            session.corpid ||
            session.corpID ||
            session._character && session._character.corporationID)) || 0,
        allianceID: Number(session && (session.allianceID ||
            session.allianceid ||
            session._character && session._character.allianceID)) || 0,
        warFactionID: Number(session && (session.warFactionID ||
            session.warfactionid ||
            session._character && session._character.warFactionID)) || 0,
        fleetID,
    };
}
function resolveJetcanSessionFleetID(session) {
    const characterRecord = session && session._character ? session._character : {};
    const directFleetID = Number(session
        ? session.fleetid ?? session.fleetID ?? characterRecord.fleetid ?? characterRecord.fleetID
        : 0) || 0;
    if (directFleetID > 0) {
        return directFleetID;
    }
    const characterID = Number(session
    ...
    try {
        ...
        const mappedFleetID = Number(fleetRuntime &&
...
function broadcastJetcanPresentation(session, systemID, containerID) {
    ...
    const entity = spawnResult.data && spawnResult.data.entity
...
function createJetcanForSession(session, context = {}) {
    const characterID = Number(context.characterID || session && (session.characterID || session.charid)) || 0;
    ...
    if (!presentationResult || !presentationResult.success) {
        log.warn(`[Jettison] Container ${containerID} presentation failed: ` +
 ...
 *
 * Only acts on actual temporary Cargo Container records (typeID 23). Other
 * expiring space inventory hosts, such as Mobile Tractor Units and wrecks,
 * must remain authoritative after their last contained item is removed.
 *
...
function maybeExpireEmptySpaceContainer(session, containerID) {
    ...
    // Expiry metadata is shared by several space entities. Type identity is the
    // authority boundary: only the disposable jetcan type may be removed here.
    if (!isDisposableJetcanRecord(container)) {
        return;
    }
    ...
    log.info(`[Jettison] Container ${numericID} is now empty — despawning early`);
    const removeResult = removeInventoryItem(numericID, { removeContents: false });
    if (!removeResult.success) {
        log.warn(`[Jettison] Failed to remove empty container ${numericID}: ${removeResult.errorMsg}`);
        return;
    }
    ...
    for (const change of (removeResult.data && removeResult.data.changes) || []) {
        ...
        emitItemsChangedForSession(session, removedState, change.previousData, { idType: "charid" });
    }
...
function jettisonItemsForSession(session, itemIDs) {
    ...
    for (const rawID of itemIDs) {
        ...
        if (Number(item.locationID) !== shipID || !isJettisonableShipFlag(item.flagID)) {
            log.warn(`[Jettison] Item ${itemID} not in a jettisonable ship hold (locationID=${item.locationID}, flagID=${item.flagID})`);
            continue;
        }
    ...
    for (const item of validItems) {
        const moveResult = moveItemToLocation(item.itemID, containerID, JETCAN_CONTENT_FLAG_ID);
        if (!moveResult.success) {
            log.warn(`[Jettison] Failed to move item ${item.itemID} into container ${containerID}: ${moveResult.errorMsg}`);
            continue;
        }
    ...
    log.info(`[Jettison] char=${characterID} jettisoned ${jettisonedToCanIDs.length} item(s) into container ${containerID} (expires in 2h)`);
    return {
        success: true,
        jettisonedToCanIDs,
        containerID,
 ...
 */
function jettisonItemQuantitiesFromSourceForSession(session, rawMoves, options = {}) {
    ...
    const expireEmptyContainer = typeof dependencies.maybeExpireEmptySpaceContainer === "function"
    ...
    const broadcastLootRights = typeof dependencies.broadcastJetcanLootRightsSlimUpdate === "function"
    ...
    const allowedSourceFlags = new Set((Array.isArray(options.allowedSourceFlagIDs) ? options.allowedSourceFlagIDs : [])
        .map((value) => Number(value))
        .filter((value) => Number.isSafeInteger(value) && value >= 0));
    const requests = Array.isArray(rawMoves) ? rawMoves : [];
    if (!characterID || !shipID || !systemID || sourceLocationID <= 0 ||
        allowedSourceFlags.size === 0 || requests.length === 0) {
        return { success: false, errorMsg: "INVALID_SOURCE" };
    }
    ...
    for (const request of requests) {
        ...
        if (!item || seenItemIDs.has(itemID) || Number(item.ownerID) !== characterID ||
    ...
    try {
        ...
        log.warn(`[Jettison] Post-commit inventory notification failed for container ${containerID}: ` +
    ...
    try {
        ...
        log.warn(`[Jettison] Post-commit loot-rights presentation failed for container ${containerID}: ` +
            `${error && error.message ? error.message : error}`);
    }
    const jettisonedToCanIDs = moveResult.data.moves
        .map((move) => Number(move && move.movedItemID) || 0)
        .filter((itemID) => itemID > 0);
    log.info(`[Jettison] char=${characterID} moved ${jettisonedToCanIDs.length} ` +
...
//# sourceMappingURL=jettisonRuntime.js.map