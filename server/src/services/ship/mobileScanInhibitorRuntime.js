...
function isMobileScanInhibitorType(typeIDOrItem) {
    if (typeIDOrItem && typeof typeIDOrItem === "object") {
        return (toInt(typeIDOrItem.categoryID, 0) === CATEGORY_DEPLOYABLE &&
            toInt(typeIDOrItem.groupID, 0) === GROUP_MOBILE_SCAN_INHIBITOR) || toInt(typeIDOrItem.typeID, 0) === TYPE_MOBILE_SCAN_INHIBITOR;
    }
...
function activateMobileScanInhibitor(itemID) {
    ...
    log.info(`[ScanInhibitor] Activated inhibitor=${itemID} system=${state.solarSystemID}`);
    return true;
}
...
function scheduleMobileScanInhibitorTimers(itemOrID) {
    ...
    if (state.expiresAtMs > 0) {
        nextTimers.expiryTimer = setUnrefTimeout(() => expireMobileScanInhibitor(itemID, "expired"), state.expiresAtMs - now);
    }
    if (state.active || state.activateAtMs <= now) {
        if (!state.active) {
            nextTimers.activationTimer = setUnrefTimeout(() => activateMobileScanInhibitor(itemID), 0);
        }
    }
    else {
        nextTimers.activationTimer = setUnrefTimeout(() => activateMobileScanInhibitor(itemID), state.activateAtMs - now);
    }
...
function hydrateMobileScanInhibitorEntityFromInventoryItem(entity, itemRecord) {
    ...
    entity.activate_comp_durationSeconds = Math.trunc(MOBILE_SCAN_INHIBITOR_ACTIVATION_DELAY_MS / 1000);
...
function launchMobileScanInhibitorFromShip(session, itemID, options = {}) {
    ...
    if (placementConflict) {
        log.info(`[ScanInhibitor] Launch rejected char=${context.characterID} item=${item.itemID} system=${context.systemID} reason=${placementConflict.errorMsg}`);
        return {
            success: false,
    ...
    if (!spawnResult || !spawnResult.success) {
        log.warn(`[ScanInhibitor] Launched inhibitor ${launchedItemID} but space spawn failed: ${spawnResult ? spawnResult.errorMsg : "UNKNOWN"}`);
    }
    registerMobileScanInhibitorItem(updatedResult.data);
    log.info(`[ScanInhibitor] char=${context.characterID} launched inhibitor itemID=${launchedItemID} system=${context.systemID}`);
    return {
        success: true,
        data: {
...
//# sourceMappingURL=mobileScanInhibitorRuntime.js.map