...
const { getCharacterCreationRace, } = require(path.join(__dirname, "../character/characterCreationData"));
const { ITEM_FLAGS, findCharacterShipByType, findCharacterShipItem, grantItemToCharacterLocation, setActiveShipForCharacter, updateInventoryItem, updateShipItem, } = require(path.join(__dirname, "../inventory/itemStore"));
const { resolveItemByTypeID, } = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const { listFittedItems, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const { getDockedLocationID, isDockedSession, } = require(path.join(__dirname, "../structure/structureLocation"));
const { ensureStarterShipDefaultFit, getStarterShipFitting, } = require(path.join(__dirname, "./starterShipFittingState"));
...
function readCharacterRecord(characterID) {
    ...
    const result = database.read(CHARACTERS_TABLE, `/${String(numericCharacterID)}`);
...
function getCachedRookieShipProfileForRace(raceID, options = {}) {
    ...
    const normalizedProfile = {
        ...
        shipTypeID,
        shipName: String(raceProfile.shipName || "").trim() ||
...
function resolveRookieShipProfile(options = {}) {
    const characterRecord = options.characterRecord && typeof options.characterRecord === "object"
        ? options.characterRecord
        : readCharacterRecord(options.characterID);
    const raceID = toInt(options.raceID ??
        (characterRecord && characterRecord.raceID) ??
        (options.session && (options.session.raceID ?? options.session.raceid)), 0);
    ...
    if (options.preferExplicitShipType === true && explicitShipTypeID > 0) {
        ...
        return {
            ...
            shipName: String(options.shipName ??
                (characterRecord && characterRecord.shipName) ??
                (explicitShipType && explicitShipType.name) ??
                DEFAULT_ROOKIE_SHIP_NAME).trim() || DEFAULT_ROOKIE_SHIP_NAME,
        };
    }
    ...
    const fallbackShipTypeID = toInt(explicitShipTypeID ||
        (characterRecord && characterRecord.shipTypeID), DEFAULT_ROOKIE_SHIP_TYPE_ID);
...
function isRookieShipItem(item) {
    ...
    return (toInt(item.groupID, 0) === CORVETTE_GROUP_ID ||
        toInt(shipType && shipType.groupID, 0) === CORVETTE_GROUP_ID);
}
function getCurrentActiveShip(characterID, characterRecord = null) {
    const resolvedCharacterRecord = characterRecord && typeof characterRecord === "object"
        ? characterRecord
...
function syncInventoryChangesForSession(session, changes = []) {
    ...
    for (const change of Array.isArray(changes) ? changes : []) {
        ...
        characterState.syncInventoryItemForSession(session, change.item, change.previousData || change.previousState || {}, {
            emitCfgLocation: true,
        });
    }
}
function refreshShip(characterID, shipItem) {
    return (findCharacterShipItem(toInt(characterID, 0), toInt(shipItem && shipItem.itemID, 0)) ||
        shipItem);
}
function repairShipAndFittedItemsForSession(session, shipItem) {
    ...
    if (shipUpdateResult.success) {
        characterState.syncInventoryItemForSession(session, shipUpdateResult.data, shipUpdateResult.previousData || {}, {
            emitCfgLocation: true,
        });
        changes.push({
    ...
    const fittedItems = listFittedItems(toInt(session.characterID, 0), shipItem.itemID);
    for (const fittedItem of fittedItems) {
        const moduleUpdateResult = updateInventoryItem(fittedItem.itemID, (currentItem) => ({
            ...currentItem,
            moduleState: {
        ...
        characterState.syncInventoryItemForSession(session, moduleUpdateResult.data, moduleUpdateResult.previousData || {}, {
...
function createRookieShipInHangar(characterID, dockedLocationID, profile) {
    ...
    const grantResult = grantItemToCharacterLocation(characterID, dockedLocationID, ITEM_FLAGS.HANGAR, shipType, 1, {
        singleton: 1,
...
function spawnRookieShipForCharacter(characterID, dockedLocationID, options = {}) {
    const numericCharacterID = toInt(characterID, 0);
    ...
    let rookieShip = options.reuseExistingShip === false
        ? null
        : findCharacterShipByType(numericCharacterID, profile.shipTypeID, numericDockedLocationID);
    let createdShip = false;
    const reusedExistingShip = Boolean(rookieShip);
    if (!rookieShip) {
        const createResult = createRookieShipInHangar(numericCharacterID, numericDockedLocationID, profile);
        if (!createResult.success || !createResult.data || !createResult.data.ship) {
            log.warn(`[RookieShipRuntime] ${String(options.logLabel || "SpawnRookieShip")} failed to create rookie ship for char=${numericCharacterID} typeID=${profile.shipTypeID} error=${createResult.errorMsg}`);
            return {
                success: false,
    ...
    const fitResult = ensureStarterShipDefaultFit(numericCharacterID, numericDockedLocationID, activeRookieShip, {
        logFailures: true,
    });
    if (options.emitNotifications !== false &&
    ...
    if (!fitResult.success) {
        log.warn(`[RookieShipRuntime] ${String(options.logLabel || "SpawnRookieShip")} starter fit incomplete for char=${numericCharacterID} ship=${activeRookieShip.itemID} typeID=${activeRookieShip.typeID} error=${fitResult.errorMsg}`);
    }
    let activationResult = null;
    if (options.autoBoard === true) {
        const characterState = getCharacterStateService();
        activationResult = characterState.activateShipForSession(session, activeRookieShip.itemID, {
            emitNotifications: options.emitNotifications !== false,
            logSelection: options.logSelection !== false,
        });
        if (!activationResult.success) {
            log.warn(`[RookieShipRuntime] ${String(options.logLabel || "SpawnRookieShip")} failed to activate rookie ship=${activeRookieShip.itemID} char=${numericCharacterID} error=${activationResult.errorMsg}`);
            return {
                success: false,
        ...
        activeRookieShip = refreshShip(numericCharacterID, activationResult.activeShip || activeRookieShip);
    }
    else if (options.setActiveShip !== false) {
        const setActiveResult = setActiveShipForCharacter(numericCharacterID, activeRookieShip.itemID);
    ...
    return {
        ...
        data: {
            ...
            activationChanged: activationResult && typeof activationResult.changed === "boolean"
...
function boardRookieShipForSession(session, options = {}) {
    ...
    return spawnRookieShipForCharacter(toInt(session.characterID, 0), getDockedLocationID(session), {
        ...options,
        session,
        autoBoard: true,
    });
...
//# sourceMappingURL=rookieShipRuntime.js.map