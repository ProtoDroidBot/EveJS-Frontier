...
function setFleetDeployableBeaconState(state, active) {
    try {
        return fleetRuntime.setJumpBeaconDeployableState(state.fleetID, state.deployableID, state.solarSystemID, state.beaconID || state.deployableID, state.ownerID || state.deployerCharacterID, active === true);
    }
    catch (error) {
        log.warn(`[DeployableCyno] Failed to ${active ? "activate" : "clear"} fleet beacon deployable=${state.deployableID} fleet=${state.fleetID}: ${error.message}`);
        return false;
    }
}
...
function activateMobileCynoDeployable(itemID) {
    ...
    log.info(`[DeployableCyno] Activated mobile cyno deployable=${itemID} fleet=${state.fleetID} system=${state.solarSystemID}`);
    return true;
}
function scheduleMobileCynoTimers(itemOrID) {
    ...
    if (state.expiresAtMs > 0) {
        nextTimers.expiryTimer = setUnrefTimeout(() => clearMobileCynoDeployable(itemID, "expired"), state.expiresAtMs - now);
    }
    if (state.active || state.activateAtMs <= now) {
        if (!state.active) {
            nextTimers.activationTimer = setUnrefTimeout(() => activateMobileCynoDeployable(itemID), 0);
        }
        else {
            setFleetDeployableBeaconState(state, true);
        }
        ...
        nextTimers.activationTimer = setUnrefTimeout(() => activateMobileCynoDeployable(itemID), state.activateAtMs - now);
    }
...
function validateDeployableCynoLaunch(session, item, context) {
    ...
    if (toInt(item.categoryID, 0) !== CATEGORY_DEPLOYABLE ||
...
function launchMobileCynoDeployableFromShip(session, itemID, options = {}) {
    ...
    if (!spawnResult || !spawnResult.success) {
        log.warn(`[DeployableCyno] Launched deployable ${launchedItemID} but space spawn failed: ${spawnResult ? spawnResult.errorMsg : "UNKNOWN"}`);
    }
    registerMobileCynoDeployableItem(updatedResult.data);
    log.info(`[DeployableCyno] char=${context.characterID} launched mobile cyno itemID=${launchedItemID} typeID=${sourceTypeID} fleet=${context.fleetID} system=${context.systemID}`);
    return {
        success: true,
        data: {
...
//# sourceMappingURL=deployableCynoRuntime.js.map