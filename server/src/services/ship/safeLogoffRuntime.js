...
const SAFE_LOGOFF_CONDITION_LABELS = Object.freeze({
    ...
    SHIP_MOVING: "UI/Inflight/SafeLogoff/ConditionShipOrbitingOrKeepingAtRange",
...
const ABORT_LABEL_BY_CONDITION = Object.freeze({
    [SAFE_LOGOFF_CONDITION_LABELS.SHIP_IN_WARP]: SAFE_LOGOFF_ABORT_LABELS.SHIP_WARPING,
    [SAFE_LOGOFF_CONDITION_LABELS.SHIP_MOVING]: SAFE_LOGOFF_ABORT_LABELS.SHIP_MOVEMENT,
    [SAFE_LOGOFF_CONDITION_LABELS.PVP_TIMER]: SAFE_LOGOFF_ABORT_LABELS.PVP_TIMER,
    [SAFE_LOGOFF_CONDITION_LABELS.PVE_TIMER]: SAFE_LOGOFF_ABORT_LABELS.NPC_TIMER,
    [SAFE_LOGOFF_CONDITION_LABELS.WEAPONS_TIMER]: SAFE_LOGOFF_ABORT_LABELS.PVP_TIMER,
    [SAFE_LOGOFF_CONDITION_LABELS.TARGET_LOCK]: SAFE_LOGOFF_ABORT_LABELS.SHIP_TARGETING,
    [SAFE_LOGOFF_CONDITION_LABELS.TARGET_LOCKED]: SAFE_LOGOFF_ABORT_LABELS.SHIP_TARGETED,
    [SAFE_LOGOFF_CONDITION_LABELS.DRONES_CONNECTED]: SAFE_LOGOFF_ABORT_LABELS.DRONES_CONNECTED,
    [SAFE_LOGOFF_CONDITION_LABELS.ACTIVE_MODULES]: SAFE_LOGOFF_ABORT_LABELS.MODULE_ACTIVATED,
    [SAFE_LOGOFF_CONDITION_LABELS.IN_FLEET]: SAFE_LOGOFF_ABORT_LABELS.IN_FLEET,
    [SAFE_LOGOFF_CONDITION_LABELS.DOCKING]: SAFE_LOGOFF_ABORT_LABELS.SHIP_MOVEMENT,
    [SAFE_LOGOFF_CONDITION_LABELS.SELF_DESTRUCT]: SAFE_LOGOFF_ABORT_LABELS.SELF_DESTRUCT,
    [SAFE_LOGOFF_CONDITION_LABELS.FIGHTERS_IN_SPACE]: SAFE_LOGOFF_ABORT_LABELS.FIGHTERS_IN_SPACE,
});
...
function clearSafeLogoffCompletion(session) {
    ...
    const hadCompletion = Boolean(session._safeLogoffCompletion || session._safeLogoffCompletedAtMs);
...
function getSafeLogoffCompletion(session) {
    ...
    const completedAtMs = Math.max(0, toFiniteNumber(completion.completedAtMs, 0));
    if (!characterID || !shipID || !completedAtMs) {
        return null;
    }
...
function consumeSafeLogoffCompletion(session, expected = {}) {
    ...
    const nowMs = Math.max(0, toFiniteNumber(expected.nowMs, typeof expected.now === "function" ? expected.now() : Date.now()));
...
function resolveSpaceContext(session, options = {}) {
    ...
    const entity = scene && typeof scene.getShipEntityForSession === "function"
        ...
        : scene && typeof scene.getEntityByID === "function"
            ? scene.getEntityByID(toPositiveInt(session && session._space && session._space.shipID, 0))
            : null;
    return { scene, entity };
}
function getCrimewatchState(characterID, nowMs, options = {}) {
    const crimewatchState = options.crimewatchState || require(path.join(__dirname, "../security/crimewatchState"));
...
function isCharacterInFleet(session, characterID, options = {}) {
    ...
    const fleetRuntime = options.fleetRuntime || require(path.join(__dirname, "../fleets/fleetRuntime"));
    return Boolean(fleetRuntime &&
...
function isCharacterInvulnerable(characterID, options = {}) {
    const npcControlState = options.npcControlState || require(path.join(__dirname, "../../space/npc/npcControlState"));
    return Boolean(npcControlState &&
...
function entityBelongsToPilot(entity, characterID, shipID) {
    ...
    const controllerID = toPositiveInt(entity.controllerID || entity.launcherID, 0);
    ...
    return ((shipID > 0 && controllerID === shipID) ||
...
function hasActiveSelfDestruct(entity, nowMs) {
    const value = entity && (entity.selfDestructAtMs ||
...
function evaluateSafeLogoffConditions(session, options = {}) {
    ...
    const nowMs = Math.max(0, toFiniteNumber(options.nowMs, typeof options.now === "function" ? options.now() : Date.now()));
    ...
    if (collectionSize(entity.lockedTargets) > 0 ||
...
function cancelSafeLogoff(session, options = {}) {
    ...
    if (options.notify === true) {
        notifySession(session, "OnSafeLogoffAborted", [options.reason || SAFE_LOGOFF_ABORT_LABELS.SHIP_MOVEMENT]);
    }
    return true;
}
...
function beginSafeLogoff(session, options = {}) {
    ...
    const countdownSeconds = Math.min(300, Math.max(1, Math.trunc(toFiniteNumber(options.countdownSeconds, runtimeConfig.safeLogoffCountdownSeconds || DEFAULT_COUNTDOWN_SECONDS))));
    ...
    const recheckIntervalMs = Math.max(50, Math.trunc(toFiniteNumber(options.recheckIntervalMs, DEFAULT_RECHECK_INTERVAL_MS)));
    const state = {
    ...
    const armNextCheck = () => {
        ...
        const delayMs = Math.max(0, Math.min(recheckIntervalMs, deadlineMs - currentMs));
    ...
    const runCheck = () => {
        ...
        try {
            ...
            log.warn(`[SafeLogoff] condition recheck failed char=${state.characterID}: ${error.message}`);
        ...
        if (currentCharacterID !== state.characterID ||
        ...
        if (currentMs >= deadlineMs) {
            ...
            if (!completion) {
                ...
                log.warn(`[SafeLogoff] completion identity missing char=${state.characterID} ship=${shipID}`);
                return;
            }
            notifySession(session, "OnSafeLogoffActivated", []);
            const disconnectCharacterSession = typeof options.disconnectCharacterSession === "function"
            ...
            try {
                ...
                log.warn(`[SafeLogoff] canonical cleanup threw char=${state.characterID}: ${error.message}`);
            }
            if (!disconnectResult || disconnectResult.success !== true) {
                log.warn(`[SafeLogoff] canonical cleanup failed char=${state.characterID} ` +
                    `reason=${disconnectResult && disconnectResult.errorMsg || "UNKNOWN"}`);
                return;
            }
            log.info(`[SafeLogoff] countdown completed and session cleared char=${state.characterID} ship=${shipID}`);
            return;
        }
        armNextCheck();
    };
    ...
    armNextCheck();
    log.info(`[SafeLogoff] countdown started char=${state.characterID} seconds=${countdownSeconds}`);
    return {
        success: true,
        deadlineMs,
        deadlineFiletime,
...
//# sourceMappingURL=safeLogoffRuntime.js.map