...
function normalizeVector(value, fallback: Record<string, any> = { x: 0, y: 0, z: 0 }) {
...
function normalizeDirection(value, fallback: Record<string, any> = { x: 1, y: 0, z: 0 }) {
...
function isMobileCynoInhibitorType(typeIDOrItem) {
  if (typeIDOrItem && typeof typeIDOrItem === "object") {
    return (
      ...
      toInt(typeIDOrItem.groupID, 0) === GROUP_CYNO_INHIBITOR
    ) || toInt(typeIDOrItem.typeID, 0) === TYPE_MOBILE_CYNOSURAL_INHIBITOR;
  }
...
function syncChangesExceptItem(session, changes: any[] = [], excludedItemID = 0) {
...
function getSessionContext(session, options: Record<string, any> = {}) {
...
function appendPlacementBlocker(blockers, seenKeys, source, options: Record<string, any> = {}) {
...
function collectPlacementBlockers(session, context, options: Record<string, any> = {}) {
  const blockers: any[] = [];
...
function activateMobileCynoInhibitor(itemID) {
  ...
  if (highSecurityRestriction) {
    ...
    log.info(
      `[CynoInhibitor] Activation blocked inhibitor=${itemID} system=${state.solarSystemID} reason=highsec`,
    );
    return false;
  }
  ...
  scheduleMobileCynoInhibitorTimers(itemID);
  log.info(
    `[CynoInhibitor] Activated inhibitor=${itemID} system=${state.solarSystemID}`,
  );
  return true;
}
...
function scheduleMobileCynoInhibitorTimers(itemOrID) {
  const itemID = toInt(itemOrID && itemOrID.itemID ? itemOrID.itemID : itemOrID, 0);
  const item = itemOrID && itemOrID.itemID ? itemOrID : findItemById(itemID);
  const state: Record<string, any> = getMobileCynoInhibitorStateFromItem(item);
  if (!state) {
    clearTimers(itemID);
    return false;
  }
  ...
  if (state.expiresAtMs > 0) {
    nextTimers.expiryTimer = setUnrefTimeout(
      () => expireMobileCynoInhibitor(itemID, "expired"),
  ...
  if (state.active || state.activateAtMs <= now) {
    if (!state.active) {
      nextTimers.activationTimer = setUnrefTimeout(
        () => activateMobileCynoInhibitor(itemID),
        0,
      );
    }
  } else {
...
function hydrateMobileCynoInhibitorEntityFromInventoryItem(entity, itemRecord) {
  ...
  entity.activate_comp_durationSeconds = Math.trunc(
    MOBILE_CYNO_INHIBITOR_ACTIVATION_DELAY_MS / 1000,
  );
  entity.cynoInhibitorRangeMeters = MOBILE_CYNO_INHIBITOR_RANGE_METERS;
  return entity;
}

function findLaunchedChange(changes: any[] = [], systemID, ownerID, sourceTypeID) {
...
function launchMobileCynoInhibitorFromShip(session, itemID, options: Record<string, any> = {}) {
  ...
  if (placementConflict) {
    log.info(
      `[CynoInhibitor] Launch rejected char=${context.characterID} item=${item.itemID} system=${context.systemID} reason=${placementConflict.errorMsg}`,
    );
    return {
      success: false as const,
  ...
  if (!spawnResult || !spawnResult.success) {
    log.warn(
      `[CynoInhibitor] Launched inhibitor ${launchedItemID} but space spawn failed: ${spawnResult ? spawnResult.errorMsg : "UNKNOWN"}`,
    );
  }

  registerMobileCynoInhibitorItem(updatedResult.data);
  log.info(
    `[CynoInhibitor] char=${context.characterID} launched inhibitor itemID=${launchedItemID} system=${context.systemID}`,
  );
...
function findActiveInhibitorConflict(systemID, position, options: Record<string, any> = {}) {
