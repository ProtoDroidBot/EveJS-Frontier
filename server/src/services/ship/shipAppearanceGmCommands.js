...
const { clearShipDirtTimestamp, getShipDirtRecord, normalizeFiletime, resetShipDirtTimestamp, } = require(path.join(__dirname, "./shipDirtState"));
const { MAX_DISPLAYED_KILLMARKS, clearShipKillCounter, getItemKillCountPlayer, readShipCounterRecord, setShipKillCounter, } = require(path.join(__dirname, "./shipKillCounterState"));
...
function calculateDirtLevelFromWeeks(weeks) {
    ...
    return DIRT_LEVEL_MAX - 1.0 / (Math.pow(dirtTimeDiffInWeeks, DIRT_CURVE_POWER) + DIRT_CURVE_BASE);
}
...
function convertDirtLevelToRatio(rawLevel) {
    const dirtLevel = clampNumber(Number(rawLevel) || 0, DIRT_LEVEL_MIN, MAX_RUNTIME_DIRT_LEVEL);
...
function buildFiletimeForDirtRatio(rawRatio) {
    const targetLevel = Math.min(remapDirtRatioToLevel(rawRatio), MAX_RUNTIME_DIRT_LEVEL);
    const ageWeeks = Math.min(calculateAgeWeeksForTargetDirtLevel(targetLevel), MAX_DIRT_AGE_WEEKS);
...
function describeShip(shipID, options = {}) {
    ...
    const sessionShipName = resolveSessionShipID(session) === Number(shipID)
        ? String(session && session.shipName || "").trim()
        : "";
    const itemName = String(item && item.itemName || "").trim();
    const activeShipName = Number(activeShip && activeShip.itemID) === Number(shipID)
...
function resolveTargetShip(session, tokens = []) {
    ...
    const shipID = explicitTarget.shipID ||
...
//# sourceMappingURL=shipAppearanceGmCommands.js.map