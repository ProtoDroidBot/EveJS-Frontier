...
function buildActiveWarpDisruptorCandidate(source, options: Record<string, any> = {}) {
  if (!source || !isWarpDisruptionProbeType(source)) {
    return null;
  }
...
function launchInterdictionProbeFromModule(session, moduleItem, loadedCharge, options: Record<string, any> = {}) {
