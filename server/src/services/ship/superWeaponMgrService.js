...
const { isModuleOnline, getTypeEffectRecords, } = require(path.join(__dirname, "../fitting/liveFittingState"));
...
class SuperWeaponMgrService extends BaseService {
    ...
    Handle_ActivateSinglePointTargetedModule(args, session, kwargs) {
        ...
        let effectName = null;
        if (effectID > 0) {
            const effectRecord = getTypeEffectRecords(moduleItem.typeID).find((candidate) => Number(candidate && candidate.effectID) === effectID);
            if (!effectRecord) {
                return null;
            }
...
//# sourceMappingURL=superWeaponMgrService.js.map