...
function ensureRuntimeRoot() {
    ...
    const next = {
        meta: current.meta && typeof current.meta === "object"
            ? current.meta
            : {
                description: "Runtime ship cosmetic ownership and applied-skin state.",
                createdBy: "Codex",
                createdAt: "2026-03-11",
            },
        characters: current.characters && typeof current.characters === "object"
            ? current.characters
            : {},
        ships: current.ships && typeof current.ships === "object" ? current.ships : {},
    };
...
function writeRuntimeRoot(runtimeRoot) {
    const nextRoot = {
        ...runtimeRoot,
        meta: runtimeRoot.meta && typeof runtimeRoot.meta === "object"
            ? runtimeRoot.meta
            : {
                description: "Runtime ship cosmetic ownership and applied-skin state.",
                createdBy: "Codex",
...
function readCatalog() {
    ...
    cachedCatalog = {
        meta: root.meta || {},
        counts: root.counts || {},
        skinsBySkinID: root.skinsBySkinID && typeof root.skinsBySkinID === "object"
            ? root.skinsBySkinID
            : {},
        shipTypesByTypeID: root.shipTypesByTypeID && typeof root.shipTypesByTypeID === "object"
            ? root.shipTypesByTypeID
            : {},
        materialsByMaterialID: root.materialsByMaterialID && typeof root.materialsByMaterialID === "object"
            ? root.materialsByMaterialID
            : {},
        licenseTypesByTypeID: root.licenseTypesByTypeID && typeof root.licenseTypesByTypeID === "object"
...
function getSkinMaterialSetIDForSkin(skinID, catalog = readCatalog()) {
    ...
    const directMaterialSetID = Number((skinEntry.material && skinEntry.material.materialSetID) ||
    ...
    const materialEntry = catalog.materialsByMaterialID[String(skinMaterialID)] || null;
...
function pickDefaultLicenseTypeID(skinEntry) {
    ...
    const permanent = skinEntry.licenseTypes.find((entry) => Number(entry.duration) === -1);
    if (permanent) {
...
function getCharacterRuntimeEntry(runtimeRoot, charId) {
    ...
    if (!runtimeRoot.characters[key].skinOverridesBySkinID ||
...
function getCharacterSkinOverride(charId, skinID, runtimeRoot = ensureRuntimeRoot()) {
    const key = String(Number(charId || 0) || 0);
    const characterEntry = runtimeRoot.characters && typeof runtimeRoot.characters === "object"
        ? runtimeRoot.characters[key]
        : null;
...
function getEffectiveLicenseRecord(charId, skinID, options = {}) {
    ...
    return {
        ...
        skinMaterialID: Number(override.skinMaterialID || 0) ||
            Number(skinEntry.skinMaterialID || 0) ||
            null,
        licenseTypeID: Number(override.licenseTypeID || 0) ||
...
function getAllLicensedSkinRecords(charId) {
    ...
    const runtimeRoot = ensureRuntimeRoot();
    const characterEntry = runtimeRoot.characters &&
        runtimeRoot.characters[String(Number(charId || 0) || 0)];
    const explicitSkinIDs = characterEntry &&
    ...
    return explicitSkinIDs
        .map((skinID) => getEffectiveLicenseRecord(charId, Number(skinID), { catalog, runtimeRoot }))
        .filter(Boolean);
}
function getLicensedSkinRecordsForType(charId, typeID) {
    ...
    return shipTypeEntry.skinIDs
        .map((skinID) => getEffectiveLicenseRecord(charId, skinID, { catalog, runtimeRoot }))
        .filter(Boolean);
}
function giveSkin(charId, skinID, options = {}) {
    ...
    const durationDays = options.durationDays !== undefined && options.durationDays !== null
        ? Number(options.durationDays)
        : licenseEntry
            ? Number(licenseEntry.duration)
            : 0;
    const expiresAtFileTime = durationDays && durationDays > 0
        ? futureFileTimeString(durationDays)
        : null;
    const success = setCharacterSkinOverride(charId, skinID, {
        licenseTypeID: Number(options.licenseTypeID || 0) ||
            (licenseEntry ? Number(licenseEntry.licenseTypeID || 0) : 0) ||
            pickDefaultLicenseTypeID(skinEntry),
        skinMaterialID: Number(options.skinMaterialID ||
            (licenseEntry && licenseEntry.skinMaterialID) ||
            skinEntry.skinMaterialID ||
            0) || null,
        expiresAtFileTime,
        isSingleUse: options.isSingleUse !== undefined
...
function applySkinToShip(shipID, skinID, options = {}) {
    ...
    const effectiveCharacterID = requestedCharacterID || Number(shipItem.ownerID || 0) || 0;
    if (numericSkinID) {
    ...
    log.info(`[ShipCosmeticsState] Applied skin shipID=${numericShipID} typeID=${shipItem.typeID} skinID=${numericSkinID || 0}`);
    return {
        success: true,
...
function getAppliedSkinRecordsForOwner(ownerID) {
    ...
    return Object.values(runtimeRoot.ships)
        .filter((record) => Number(record && record.characterID ? record.characterID : 0) === numericOwnerID ||
...
//# sourceMappingURL=shipCosmeticsState.js.map