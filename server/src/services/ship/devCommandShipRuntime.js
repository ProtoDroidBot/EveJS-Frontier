...
const { getActiveShipRecord, getCharacterRecord, activateShipForSession, syncInventoryItemForSession, } = require(path.join(__dirname, "../character/characterState"));
const { ITEM_FLAGS, grantItemToCharacterLocation, grantItemsToCharacterStationHangar, moveItemTypeFromCharacterLocation, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const { buildShipResourceState, listFittedItems, selectAutoFitFlagForType, validateFitForShip, getModuleChargeCapacity, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const { getDockedLocationID, isDockedSession, } = require(path.join(__dirname, "../structure/structureLocation"));
const { buildStoppedSpawnStateNearEntity, boardPreparedShipInSpace, } = require(path.join(__dirname, "./spaceShipSwapRuntime"));
...
function createTrigPreset({ shipName, weaponName, primaryChargeName, alternateChargeNames = [], propulsionName = null, } = {}) {
    const modules = [
...
function resolvePresetPreloadCharges(preset) {
    ...
    for (const rawEntry of Array.isArray(preset && preset.preloadCharges) ? preset.preloadCharges : []) {
        ...
        resolvedEntries.push({
            ...
            quantity: rawEntry && rawEntry.fullClip === true
                ? null
...
function resolvePreferredStagingLocationID(session) {
    ...
    return toInt(characterRecord.homeStationID ||
...
function tryPlanNextModuleFit(charID, shipItem, itemType, fittedItems) {
    ...
    if (!resourceCheck.success) {
        return {
            success: false,
            errorMsg: resourceCheck.resourceState.upgradeLoad >
...
function fitGrantedItemTypeToShip(session, locationID, shipItem, itemType, count, options = {}) {
    ...
    for (let index = 0; index < numericCount; index += 1) {
        ...
        let destinationFlagID = nextFit && nextFit.success && nextFit.data
            ? toInt(nextFit.data.flagID, 0)
            : 0;
        if (destinationFlagID <= 0 && forceFit) {
            destinationFlagID = toInt(selectAutoFitFlagForType(shipItem, fittedItems, toInt(itemType && itemType.typeID, 0)), 0);
        }
        ...
        const moveResult = moveItemTypeFromCharacterLocation(session.characterID, locationID, ITEM_FLAGS.HANGAR, shipItem.itemID, destinationFlagID, itemType.typeID, 1);
...
function moveGrantedItemTypeToShipFlag(session, locationID, shipItem, destinationFlagID, itemType, quantity, options = {}) {
    const moveResult = moveItemTypeFromCharacterLocation(session.characterID, locationID, ITEM_FLAGS.HANGAR, shipItem.itemID, destinationFlagID, itemType.typeID, quantity);
...
function moveGrantedItemTypeToShipCargo(session, locationID, shipItem, itemType, quantity, options = {}) {
    return moveGrantedItemTypeToShipFlag(session, locationID, shipItem, ITEM_FLAGS.CARGO_HOLD, itemType, quantity, options);
}
function moveGrantedItemTypeToShipDroneBay(session, locationID, shipItem, itemType, quantity, options = {}) {
    return moveGrantedItemTypeToShipFlag(session, locationID, shipItem, ITEM_FLAGS.DRONE_BAY, itemType, quantity, options);
}
function loadGrantedChargeTypeIntoFirstMatchingModule(session, locationID, shipItem, moduleType, chargeType, quantity, options = {}) {
    ...
    const moduleCapacity = Math.max(1, toInt(getModuleChargeCapacity(toInt(targetModule.typeID, 0), toInt(chargeType && chargeType.typeID, 0)), 1));
    const requestedQuantity = Math.max(1, toInt(quantity, 1));
    const loadQuantity = Math.min(moduleCapacity, requestedQuantity);
    const moveResult = moveItemTypeFromCharacterLocation(session.characterID, locationID, ITEM_FLAGS.HANGAR, shipItem.itemID, targetModule.flagID, chargeType.typeID, loadQuantity);
    if (moveResult.success && options.syncToSession !== false) {
        syncInventoryChangesToSession(session, moveResult.data && moveResult.data.changes);
    }
    return moveResult;
...
function resolvePreloadChargeQuantity(preloadEntry) {
    ...
    if (preloadEntry.fullClip === true) {
        return Math.max(1, toInt(getModuleChargeCapacity(toInt(preloadEntry.moduleType && preloadEntry.moduleType.typeID, 0), toInt(preloadEntry.chargeType && preloadEntry.chargeType.typeID, 0)), 1));
    }
...
function stagePresetShipForLocation(session, preset, locationID, options = {}) {
    ...
    const createResult = grantItemToCharacterLocation(session.characterID, locationID, ITEM_FLAGS.HANGAR, shipTypeResult.data, 1);
    ...
    for (const cargoEntry of cargoResult.data) {
        ...
        cargoGrantQuantityByTypeID.set(cargoEntry.itemType.typeID, currentQuantity + cargoEntry.quantity);
    }
    for (const preloadEntry of preloadResult.data) {
        ...
        cargoGrantQuantityByTypeID.set(preloadEntry.chargeType.typeID, currentQuantity + resolvePreloadChargeQuantity(preloadEntry));
    }
    ...
    const grantResult = grantItemsToCharacterStationHangar(session.characterID, locationID, grantEntries);
    ...
    for (const moduleEntry of moduleResult.data) {
        const fitResult = fitGrantedItemTypeToShip(session, locationID, shipItem, moduleEntry.itemType, moduleEntry.quantity, {
            syncToSession,
    ...
    for (const cargoEntry of cargoResult.data) {
        const moveResult = moveGrantedItemTypeToShipCargo(session, locationID, shipItem, cargoEntry.itemType, cargoEntry.quantity, {
            syncToSession,
        });
        if (!moveResult.success) {
            return {
    ...
    for (const droneEntry of droneBayResult.data) {
        const moveResult = moveGrantedItemTypeToShipDroneBay(session, locationID, shipItem, droneEntry.itemType, droneEntry.quantity, {
            syncToSession,
        });
        if (!moveResult.success) {
            return {
    ...
    for (const preloadEntry of preloadResult.data) {
        const loadResult = loadGrantedChargeTypeIntoFirstMatchingModule(session, locationID, shipItem, preloadEntry.moduleType, preloadEntry.chargeType, resolvePreloadChargeQuantity(preloadEntry), {
            syncToSession,
        });
...
function buildSpaceResultMessage(preset, shipItem, fitSummaries = [], destroyResult = null) {
    const fitText = fitSummaries
        ...
        .join(", ");
    const actionText = destroyResult && destroyResult.destroyedShipID
...
function handleDevCommandShip(session, presetKey) {
    ...
    const stageResult = stagePresetShipForLocation(session, preset, stagingLocationID, {
    ...
    if (docked) {
        ...
        return {
            success: true,
            message: buildDockedResultMessage(preset, stageResult.data.shipItem, stageResult.data.fitSummaries),
        };
    }
    ...
    return {
        success: true,
        message: buildSpaceResultMessage(preset, stageResult.data.shipItem, stageResult.data.fitSummaries, spaceBoardResult.data && spaceBoardResult.data.destroyResult),
    };
}
...
//# sourceMappingURL=devCommandShipRuntime.js.map