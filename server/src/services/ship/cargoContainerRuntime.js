...
function isCargoContainerType(item) {
    ...
    return (toInt(item.categoryID, 0) === CATEGORY_CELESTIAL &&
...
function validateCargoDestination(item, context) {
    ...
    const used = getShipCargoUsedVolume(context.characterID, context.shipID, item && item.itemID);
...
function validateCargoContainerSpaceAccess(session, item, context) {
    ...
    if (toInt(item.locationID, 0) !== context.systemID ||
        ...
        !item.spaceState ||
        toInt(item.spaceState.systemID, 0) !== context.systemID ||
        !isFiniteVector(item.spaceState.position)) {
    ...
    if (!scene || !shipEntity || shipEntity.kind !== "ship") {
        return "SHIP_NOT_VISIBLE";
    }
    const containerEntity = scene && scene.getEntityByID(item.itemID);
    // An unanchored structure hull sits in space as its own runtime kind rather
    // than an ordinary jettison container, so accept either depending on the item.
    const expectedEntityKind = isUnanchoredStructureHullItem(item)
        ? RUNTIME_UNANCHORED_STRUCTURE_HULL_KIND
        : "container";
    if (!shipEntity ||
        !containerEntity ||
        containerEntity.kind !== expectedEntityKind ||
        !canEntitiesInteractLocally(shipEntity, containerEntity)) {
        return "CONTAINER_NOT_VISIBLE";
    }
    const shipPosition = shipEntity && shipEntity.position;
    const containerPosition = containerEntity && containerEntity.position;
    if (!isFiniteVector(shipPosition) || !isFiniteVector(containerPosition)) {
        return "INVALID_SPACE_POSITION";
    }
    const centerDistance = getVectorDistance(shipPosition, containerPosition);
    const transferDistance = isUnanchoredStructureHullItem(item)
        ? (typeof scene.getCommandTimeEntitySurfaceDistance === "function"
            ? scene.getCommandTimeEntitySurfaceDistance(shipEntity, containerEntity)
            : Math.max(0, centerDistance -
                Math.max(0, toReal(shipEntity && shipEntity.radius, 0)) -
                Math.max(0, toReal(containerEntity && containerEntity.radius, 0))))
        : centerDistance;
    if (!Number.isFinite(transferDistance)) {
        return "INVALID_SPACE_POSITION";
    }
    if (transferDistance > MAX_CARGO_CONTAINER_TRANSFER_DISTANCE_METERS) {
...
function transferContainedItemsToOwner(containerID, ownerID, seen = new Set()) {
    ...
    for (const child of listContainerItems(null, normalizedContainerID, null)) {
        ...
        if (!updateResult.success) {
            log.warn(`[CargoContainer] Failed to transfer contained item=${child.itemID} owner=${normalizedOwnerID}: ${updateResult.errorMsg}`);
            continue;
        }
...
function restoreSpaceEntityAfterFailedScoop(context, originalItem) {
    const itemID = toInt(originalItem && originalItem.itemID, 0);
    let restoreResult = null;
    try {
        restoreResult = updateInventoryItem(originalItem.itemID, () => ({ ...originalItem }));
    }
    catch (error) {
        restoreResult = {
            success: false,
            errorMsg: error && error.message || "ROLLBACK_UPDATE_FAILED",
        };
    }
    let restoredItem = null;
    if (restoreResult && restoreResult.success === true) {
        try {
            restoredItem = findItemById(itemID);
        }
        catch (error) {
            restoreResult = {
                success: false,
                errorMsg: error && error.message || "ROLLBACK_VERIFY_FAILED",
            };
        }
    }
    const inventoryRestored = Boolean(restoredItem &&
        toInt(restoredItem.locationID, 0) === toInt(context.systemID, 0) &&
        toInt(restoredItem.flagID, -1) === 0 &&
        restoredItem.spaceState &&
        toInt(restoredItem.spaceState.systemID, 0) === toInt(context.systemID, 0) &&
        isFiniteVector(restoredItem.spaceState.position));
    if (!inventoryRestored) {
        log.error(`[CargoContainer] Failed scoop rollback item=${itemID} ` +
            `inventory=${restoreResult && restoreResult.errorMsg || "state-invalid"} ` +
            "space=not-respawned");
        return {
    ...
    try {
        spawnResult = spaceRuntime.spawnDynamicInventoryEntity(context.systemID, itemID);
    }
    catch (error) {
        spawnResult = {
            success: false,
            errorMsg: error && error.message || "ROLLBACK_SPAWN_FAILED",
        };
    }
    if (!spawnResult || !spawnResult.success) {
        log.error(`[CargoContainer] Failed scoop rollback item=${itemID} ` +
            "inventory=ok " +
            `space=${spawnResult && spawnResult.errorMsg || "unknown"}`);
    }
    return {
        inventoryRestored: true,
        entityRestored: Boolean(spawnResult && spawnResult.success === true),
    };
}
function scoopCargoContainerToCargo(session, itemID, password = "") {
    ...
    const removeResult = spaceRuntime.removeDynamicEntity(context.systemID, item.itemID);
    ...
    try {
        updateResult = updateInventoryItem(item.itemID, (currentItem) => ({
            ...currentItem,
        ...
        updateResult = {
            success: false,
            errorMsg: error && error.message || "SCOOP_UPDATE_FAILED",
        };
    }
    if (!updateResult || !updateResult.success || !updateResult.data) {
        return {
            ...(updateResult || {
                success: false,
                errorMsg: "SCOOP_UPDATE_FAILED",
            }),
            rollback: restoreSpaceEntityAfterFailedScoop(context, item),
        };
    }
    ...
    log.info(`[CargoContainer] char=${context.characterID} scooped container=${item.itemID} system=${context.systemID} ship=${context.shipID}`);
    return {
        success: true,
        data: {
            itemID: item.itemID,
...
module.exports = {
    ...
    _testing: {
        ...
        restoreSpaceEntityAfterFailedScoop,
    },
};
//# sourceMappingURL=cargoContainerRuntime.js.map