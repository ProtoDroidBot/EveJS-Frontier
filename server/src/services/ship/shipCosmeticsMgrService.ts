...
function resolveOwnedShipItem(shipID, characterID, options: Record<string, any> = {}) {
...
function normalizeItemIDs(rawValue) {
  ...
  if (normalizedValue && typeof normalizedValue === "object" && !Array.isArray(normalizedValue)) {
    const objectValues = Object.entries<any>(normalizedValue)
      .map(([entryKey, row]) =>
        normalizeNumber(
...
class ShipCosmeticsMgrService extends BaseService {
  ...
  Handle_GetLicencedSkins(args, session, ..._extra: any[]) {
  ...
  Handle_GetLicencedSkinsForShipType(args, session, ..._extra: any[]) {
  ...
  Handle_ActivateSkinLicense(args, session) {
    ...
    const activationPlan: any[] = [];
    const plannedSkinIDs = new Set<any>();
