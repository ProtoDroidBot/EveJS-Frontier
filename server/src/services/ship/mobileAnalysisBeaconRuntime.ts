...
const CARRIER_CRAB_LINKABLE_SHIP_TYPE_IDS = new Set<any>();
...
function normalizeHiveLinkState(rawState, defaults: Record<string, any> = {}) {
  ...
  return {
    ...
    linkState: Math.max(
      LINKSTATE_IDLE,
      Math.min(LINKSTATE_COMPLETED, toInt(state.linkState, LINKSTATE_IDLE)),
    ),
    linkCompleteAtMs: toInt(state.linkCompleteAtMs, 0),
    linkedShipID: toInt(state.linkedShipID, 0),
    hiveSpawnedAtMs: toInt(state.hiveSpawnedAtMs, 0),
...
function spawnConfiguredHiveNpcs(anchorEntity, state, options: Record<string, any> = {}) {
  const configuration = frontierDungeonSpawns.resolveHiveSpawnConfiguration(
    anchorEntity && anchorEntity.typeID,
  );
  if (!configuration) {
    return {
  ...
  const systemID = toInt(
    ...
    toInt(anchorEntity && anchorEntity.systemID, toInt(anchorEntity && anchorEntity.locationID, 0)),
  );
  ...
  const npcService = options.npcService || require(path.join(
    __dirname,
    "../../space/npc/npcService",
  ));
  const spawned: any[] = [];
  const failures: any[] = [];
  ...
  configuration.spawnEntries.forEach((entry, index) => {
    const spawnResult = npcService.spawnNpcBatchInSystem(systemID, {
      ...
      spawnDistanceMeters: Math.max(
        1_000,
        6_000 + (index * Math.max(500, toInt(
          frontierDungeonSpawns.getConfig().defaults.formationSpacingMeters,
          1_800,
        ))),
      ),
...
function completeConfiguredHiveEntityLink(scene, entity) {
  ...
  if (spawnResult && !spawnResult.success) {
    log.warn(
      `[MobileAnalysisBeacon] Hive spawn partially failed beacon=${entity.itemID} ` +
      `failures=${spawnResult.data.failures.length}`,
    );
  }
  log.info(
    `[MobileAnalysisBeacon] Hive link completed beacon=${entity.itemID} ` +
...
function completeMobileAnalysisBeaconLink(itemID) {
  ...
  if (spawnResult && !spawnResult.success) {
    log.warn(
      `[MobileAnalysisBeacon] Hive spawn partially failed beacon=${itemID} ` +
      `failures=${spawnResult.data.failures.length}`,
    );
  }
  log.info(
...
function launchMobileAnalysisBeaconFromShip(session, itemID, options: Record<string, any> = {}) {
...
function initiateConfiguredHiveEntityLink(
  session,
  beaconID,
  context,
  ...
  if (
    state.linkState !== LINKSTATE_IDLE ||
  ...
  timersByBeaconID.set(toInt(beaconID, 0), {
    activationTimer: null,
    expiryTimer: null,
    linkTimer: setUnrefTimeout(
      () => completeConfiguredHiveEntityLink(scene, hiveEntity),
      linkCompleteAtMs - now,
    ),
  });
  ...
  log.info(
    `[MobileAnalysisBeacon] Hive link started char=${context.characterID} ` +
...
function initiateMobileAnalysisBeaconLink(session, beaconID, options: Record<string, any> = {}) {
