...
const { buildList, buildBoundObjectResponse, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
class CrimewatchService extends BaseService {
    ...
    Handle_MachoBindObject(args, session, kwargs) {
        log.debug(`[CrimewatchService] MachoBindObject args=${args ? args.length : 0}`);
    ...
    Handle_SetSafetyLevel(args, session) {
        ...
        const requestedSafetyLevel = Number.isFinite(rawSafetyLevel)
            ? rawSafetyLevel
            : crimewatchState.SAFETY_LEVEL_FULL;
        const result = crimewatchState.setSafetyLevel(characterID, requestedSafetyLevel);
        return result.success ? result.data.safetyLevel : crimewatchState.SAFETY_LEVEL_FULL;
    }
    Handle_GetMySecurityStatus(args, session) {
        const charID = (session && (session.characterID || session.charid || session.userid)) || 0;
        ...
        log.debug(`[CrimewatchService] GetMySecurityStatus(charID=${charID}) -> ${normalizedStatus}`);
    ...
    Handle_GetCharacterSecurityStatus(args, session) {
        ...
        const securityStatus = Number(charData.securityStatus ?? charData.securityRating ?? 0);
        const normalizedStatus = Number.isFinite(securityStatus) ? securityStatus : 0;
        log.debug(`[CrimewatchService] GetCharacterSecurityStatus(charID=${charID}) -> ${normalizedStatus}`);
        return normalizedStatus;
    }
...
//# sourceMappingURL=crimewatchService.js.map