...
function triggerHighSecCriminalOffense(scene, attackerEntity, options: Record<string, any> = {}) {
  const now = Math.max(
    0,
  ...
  const securityStatusPenalty: Record<string, any> = securityPenaltyTargetEntity
...
function setCharacterCrimewatchDebugState(characterID, updates: Record<string, any> = {}, options: Record<string, any> = {}) {
  const state = ensureCharacterState(characterID);
  if (!state) {
    return {
      success: false as const,
...
function retaskResponders(scene, offenderEntityID, responderEntityIDs, options: Record<string, any> = {}) {
  if (
    !scene ||
    !offenderEntityID ||
    !Array.isArray(responderEntityIDs) ||
