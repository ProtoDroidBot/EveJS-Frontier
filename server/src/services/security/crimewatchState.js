...
const { buildDict, buildFiletimeLong, buildList, buildPythonSet, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { normalizeCombatTimersForProfile, } = require(path.join(__dirname, "crimewatchCompatibility"));
const sessionRegistry = require(path.join(__dirname, "../chat/sessionRegistry"));
const npcRuntime = require(path.join(__dirname, "../../space/npc/npcRuntime"));
const { findSafeWarpOriginAnchor, } = require(path.join(__dirname, "../../space/npc/npcWarpOrigins"));
const { ENTITY_TYPE, } = require(path.join(__dirname, "../../space/entityConstants"));
const { buildChildEntityScopeMetadata, } = require(path.join(__dirname, "../../space/destiny/identity/interactionScope"));
...
function findLegalWarAggression(attackerCharacterID, targetCharacterID, nowMs) {
    ...
    return warRuntime.findActiveWarBetweenCharacters(attackerCharacterID, targetCharacterID, { nowMs });
}
...
function clamp(value, minimum, maximum) {
    return Math.min(Math.max(toFiniteNumber(value, minimum), toFiniteNumber(minimum, value)), toFiniteNumber(maximum, value));
}
...
function buildConcordResponseOriginAnchor(scene, offenderEntity) {
    ...
    const fallbackDirection = normalizeVector(offenderEntity && offenderEntity.direction, { x: 1, y: 0, z: 0 });
    const fallbackOffsetDirection = normalizeVector(scaleVector(fallbackDirection, -1), { x: -1, y: 0, z: 0 });
    const fallbackPosition = addVectors(offenderPosition, scaleVector(fallbackOffsetDirection, CONCORD_RESPONSE_WARP_ORIGIN_MIN_DISTANCE_METERS));
    if (!scene || !offenderEntity) {
        return {
            position: fallbackPosition,
            direction: normalizeVector(subtractVectors(offenderPosition, fallbackPosition), fallbackDirection),
        };
    }
    ...
    return {
        ...
        direction: normalizeVector(subtractVectors(offenderPosition, cloneVector(originAnchor && originAnchor.position, fallbackPosition)), fallbackDirection),
    };
}
...
function ensureCharacterState(characterID) {
    ...
    if (!characterCrimewatchState.has(normalizedCharacterID)) {
        characterCrimewatchState.set(normalizedCharacterID, buildDefaultCharacterState(normalizedCharacterID));
    }
...
function pruneCharacterState(characterID, now = Date.now()) {
    ...
    const shouldKeep = state.safetyLevel !== SAFETY_LEVEL_FULL ||
...
function normalizeSafetyLevel(safetyLevel) {
    ...
    return Math.max(SAFETY_LEVEL_NONE, Math.min(SAFETY_LEVEL_FULL, numeric));
}
...
function getEntityActorKind(entity) {
    ...
    const npcEntityType = String(entity.npcEntityType ||
        entity.entityType ||
        "").trim().toLowerCase();
    ...
    const sessionCharacterID = toPositiveInt(entity.session && (entity.session.characterID || entity.session.charID), 0);
...
function getEntityCharacterID(entity) {
    ...
    return toPositiveInt(entity.session && (entity.session.characterID || entity.session.charID), toPositiveInt(entity.characterID, 0));
}
function roundSecurityStatus(value) {
    return Number(toFiniteNumber(value, 0).toFixed(SECURITY_STATUS_ROUND_DIGITS));
}
function clampSecurityStatus(value) {
    return roundSecurityStatus(Math.max(SECURITY_STATUS_MIN, Math.min(SECURITY_STATUS_MAX, toFiniteNumber(value, 0))));
}
...
function getCharacterSecurityStatus(characterID, fallback = 0) {
    ...
    return clampSecurityStatus(record && (record.securityStatus ?? record.securityRating) !== undefined
...
function getEntitySecurityStatus(entity, fallback = 0) {
    ...
    return clampSecurityStatus(entity && (entity.securityStatus ?? entity.securityRating) !== undefined
        ? entity.securityStatus ?? entity.securityRating
        : fallback);
}
function setCharacterSecurityStatus(characterID, nextSecurityStatus, options = {}) {
    const normalizedCharacterID = toPositiveInt(characterID, 0);
    if (!normalizedCharacterID) {
        return {
            success: false,
    ...
    const syncResult = synchronizeLiveSecurityStatus(options.scene || null, options.entity || null, previousSecurityStatus, clampedSecurityStatus, options.now);
    if (!options.entity && options.session) {
        notifyOwnSecurityStatusUpdate(options.session, clampedSecurityStatus, previousSecurityStatus);
    }
    return {
        success: true,
        data: {
...
function rememberRecentSecurityPenaltyTarget(characterID, targetKey, expiresAtMs) {
    ...
    tracker.set(targetKey, Math.max(0, Math.trunc(toFiniteNumber(expiresAtMs, Date.now()))));
    return true;
}
...
function resolveSessionSystemID(session) {
    return toPositiveInt(session && ((session._space && session._space.systemID) ||
...
function buildSecurityStatusPenaltyResult({ applied = false, reason = "NOT_APPLIED", characterID = 0, systemSecurity = 0, targetKind = "ship", previousSecurityStatus = 0, nextSecurityStatus = previousSecurityStatus, targetSecurityStatus = 0, modification = 0, } = {}) {
    return {
        applied,
        reason,
        characterID: toPositiveInt(characterID, 0),
        ...
        deltaSecurityStatus: roundSecurityStatus(clampSecurityStatus(nextSecurityStatus) -
...
function notifySecurityStatusAttributeChange(session, characterID, itemID, nextValue, previousValue, whenMs) {
    if (!session ||
    ...
    session.sendNotification("OnModuleAttributeChanges", "clientID", [{
            type: "list",
            items: [
                buildSecurityStatusAttributeChange(characterID, itemID, nextValue, previousValue, whenMs),
            ],
        }]);
    return true;
}
...
function synchronizeLiveSecurityStatus(scene, entity, previousSecurityStatus, nextSecurityStatus, now = Date.now()) {
    if (!scene ||
        !entity ||
    ...
    const characterID = getEntityCharacterID(entity);
    const attributeNotified = notifySecurityStatusAttributeChange(entity.session, characterID, entity.itemID, nextSecurityStatus, previousSecurityStatus, Math.max(0, Math.trunc(toFiniteNumber(now, Date.now()))));
    const selfStatusNotified = notifyOwnSecurityStatusUpdate(entity.session, nextSecurityStatus, previousSecurityStatus);
    return {
        slimRecipients,
        attributeNotified,
        selfStatusNotified,
...
function applySecurityStatusPenaltyForAggression(scene, attackerEntity, targetEntity, now = Date.now()) {
    ...
    const modification = modificationBase *
        systemSecurity *
        (1 + ((targetSecurityStatus - previousSecurityStatus) / 100));
    const nextSecurityStatus = clampSecurityStatus(previousSecurityStatus + ((10 - previousSecurityStatus) * modification));
    if (targetKey) {
        rememberRecentSecurityPenaltyTarget(attackerCharacterID, targetKey, now + SECURITY_STATUS_DUPLICATE_WINDOW_MS);
    }
    ...
    const syncResult = synchronizeLiveSecurityStatus(scene, attackerEntity, previousSecurityStatus, nextSecurityStatus, now);
    return {
        success: true,
        data: {
...
function buildWeaponTimerTuple(state, now = Date.now()) {
    return buildTimerTuple(WEAPONS_TIMER_STATE_IDLE, state && state.weaponTimerExpiresAtMs, now);
}
function buildPvpTimerTuple(state, now = Date.now()) {
    return buildTimerTuple(PVP_TIMER_STATE_IDLE, state && state.pvpTimerExpiresAtMs, now);
}
function buildNpcTimerTuple(state, now = Date.now()) {
    return buildTimerTuple(NPC_TIMER_STATE_IDLE, state && state.npcTimerExpiresAtMs, now);
}
function buildCriminalTimerTuple(state, now = Date.now()) {
    const normalizedState = state || buildDefaultCharacterState(0);
    const normalizedExpiry = Math.max(0, Math.trunc(toFiniteNumber(normalizedState.criminalTimerExpiresAtMs, 0)));
    if (normalizedExpiry <= now ||
...
function buildDisapprovalTimerTuple(state, now = Date.now()) {
    return buildTimerTuple(DISAPPROVAL_TIMER_STATE_IDLE, state && state.disapprovalTimerExpiresAtMs, now);
}
...
function buildFlaggedCharacterSnapshot(systemID, now = Date.now()) {
    ...
    for (const state of characterCrimewatchState.values()) {
        ...
        if (state.criminal === true &&
            toFiniteNumber(state.criminalTimerExpiresAtMs, 0) > now) {
            criminals.push(state.characterID);
        }
        else if (state.suspect === true &&
            toFiniteNumber(state.criminalTimerExpiresAtMs, 0) > now) {
...
function buildFlaggedCharactersForSession(session, now = Date.now()) {
    const snapshot = buildFlaggedCharacterSnapshot(resolveSessionSystemID(session), now);
    return [
        buildPythonSet(snapshot.criminals),
        buildPythonSet(snapshot.suspects),
    ];
}
function buildClientStatesForSession(session, now = Date.now()) {
    ...
    return [
        normalizeCombatTimersForProfile(combatTimers, session && session.compatibilityProfile),
        buildDict([]),
...
function sendTimerNotificationIfChanged(session, notificationName, previousTuple, nextTuple) {
...
function synchronizeSessionTimerNotifications(scene, session, now = Date.now()) {
    ...
    sendTimerNotificationIfChanged(session, "OnWeaponsTimerUpdate", previous.weapons, snapshot.weapons);
    sendTimerNotificationIfChanged(session, "OnPvpTimerUpdate", previous.pvp, snapshot.pvp);
    sendTimerNotificationIfChanged(session, "OnNpcTimerUpdate", previous.npc, snapshot.npc);
    sendTimerNotificationIfChanged(session, "OnCriminalTimerUpdate", previous.criminal, snapshot.criminal);
    sendTimerNotificationIfChanged(session, "OnDisapprovalTimerUpdate", previous.disapproval, snapshot.disapproval);
...
function ensureConcordResponseRecord(scene, attackerEntity, targetEntity, now) {
    ...
    if (!responseMap.has(offenderEntityID)) {
        responseMap.set(offenderEntityID, {
            ...
            victimEntityID: toPositiveInt((targetEntity && targetEntity.itemID) ||
                (attackerEntity && attackerEntity.itemID), 0),
    ...
    response.victimEntityID = toPositiveInt((targetEntity && targetEntity.itemID) ||
        (attackerEntity && attackerEntity.itemID), response.victimEntityID);
    response.lastOffenseAtMs = now;
    response.completionClearAtMs = Math.max(0, Math.trunc(toFiniteNumber(response.completionClearAtMs, 0)));
    response.ingressCompleteAtMs = Math.max(0, Math.trunc(toFiniteNumber(response.ingressCompleteAtMs, 0)));
    response.capsuleTargeted = response.capsuleTargeted === true;
    if (toFiniteNumber(response.respondedAtMs, 0) <= 0) {
        response.dueAtMs = Math.min(Math.max(now, toFiniteNumber(response.dueAtMs, now)), now + getConcordResponseDelayMsForSystem(scene && scene.system));
    }
    return response;
}
function scheduleConcordResponse(scene, offenderEntity, now = Date.now(), targetEntity = null) {
    ...
    return ensureConcordResponseRecord(scene, offenderEntity, targetEntity || offenderEntity, now);
}
function triggerHighSecCriminalOffense(scene, attackerEntity, options = {}) {
    ...
    const securityPenaltyTargetEntity = options.securityPenaltyTargetEntity || targetEntity || null;
    ...
    state.weaponTimerExpiresAtMs = Math.max(toFiniteNumber(state.weaponTimerExpiresAtMs, 0), now + WEAPON_TIMER_DURATION_MS);
    state.pvpTimerExpiresAtMs = Math.max(toFiniteNumber(state.pvpTimerExpiresAtMs, 0), now + PVP_TIMER_DURATION_MS);
    state.lastKnownSystemID = toPositiveInt(scene.systemID, 0);
    const legalWar = findLegalWarAggression(attackerCharacterID, targetCharacterID, now);
    if (legalWar) {
        return {
            success: true,
            data: {
    ...
    if (!isHighSecuritySystem(scene.system)) {
        ...
        state.criminalTimerExpiresAtMs = Math.max(toFiniteNumber(state.criminalTimerExpiresAtMs, 0), now + CRIMINAL_TIMER_DURATION_MS);
        state.criminal = true;
        state.suspect = false;
        state.lastCriminalAtMs = now;
        const securityStatusPenalty = applySecurityStatusPenaltyForAggression(scene, attackerEntity, targetEntity, now);
        const killRight = createKillRightForCriminalAggression(scene, attackerEntity, targetEntity, now);
        return {
            success: true,
            data: {
                applied: true,
                ...
                securityStatusPenalty: securityStatusPenalty && securityStatusPenalty.data
                    ? securityStatusPenalty.data
                    : buildSecurityStatusPenaltyResult({
                        applied: false,
                        reason: securityStatusPenalty && securityStatusPenalty.errorMsg
    ...
    state.suspect = false;
    state.lastCriminalAtMs = now;
    const response = scheduleConcordResponse(scene, attackerEntity, now, targetEntity);
    const securityStatusPenalty = securityPenaltyTargetEntity
        ? applySecurityStatusPenaltyForAggression(scene, attackerEntity, securityPenaltyTargetEntity, now)
        : {
            success: true,
            data: buildSecurityStatusPenaltyResult({
                applied: false,
...
function setCharacterCrimewatchDebugState(characterID, updates = {}, options = {}) {
    ...
    const now = Math.max(0, Math.trunc(toFiniteNumber(options.now, Date.now())));
    const numericSystemID = toPositiveInt(options.systemID, toPositiveInt(state.lastKnownSystemID, 0));
    ...
    if (updates.refreshConcord === true &&
        state.criminal === true &&
        toFiniteNumber(state.criminalTimerExpiresAtMs, 0) > now &&
        options.scene &&
        options.offenderEntity) {
        scheduleConcordResponse(options.scene, options.offenderEntity, now, options.targetEntity || null);
    }
    return {
        success: true,
    ...
    if (!isHighSecuritySystem(scene.system)) {
        if (isLowSecuritySystem(scene.system)) {
            if (isCapsuleEntity(targetEntity)) {
                return triggerHighSecCriminalOffense(scene, attackerEntity, {
                    now,
                    reason: "LOWSEC_CAPSULE_AGGRESSION",
                    targetEntity,
...
function clearConcordResponders(scene, responderEntityIDs) {
    ...
    for (const responderEntityID of responderEntityIDs) {
        const destroyResult = npcRuntime.despawn(responderEntityID, {
            removeContents: true,
        });
...
function stopResponders(scene, responderEntityIDs) {
    ...
    for (const responderEntityID of responderEntityIDs) {
        ...
        const needsAuthoritativeParkRefresh = responderEntity.mode !== "STOP" ||
        ...
        if (needsAuthoritativeParkRefresh) {
            ...
            const parkedPosition = responderEntity.mode === "WARP" &&
                responderEntity.warpState &&
                !responderEntity.pendingWarp
                ? cloneVector(responderEntity.warpState.targetPoint, responderEntity.position)
                : cloneVector(responderEntity.position);
            scene.teleportDynamicEntityToPoint(responderEntity, parkedPosition, {
...
function spawnConcordResponders(scene, offenderEntity, response) {
    ...
    const responseOriginAnchor = buildConcordResponseOriginAnchor(scene, offenderEntity);
    const spawnResult = npcRuntime.spawnGroupInSystem(scene.systemID, {
        entityScopeMetadata: buildChildEntityScopeMetadata(offenderEntity),
    ...
    spawnResult.data.spawned.forEach((entry, index) => {
        ...
        const arrivalPoint = buildConcordArrivalPoint(offenderEntity, index, totalSpawned);
        warpRequests.push({
            entityID,
            point: arrivalPoint,
            options: {
    ...
    const groupedWakeAtMs = Math.max(0, Math.trunc(toFiniteNumber(warpBatchResult.data.commonWakeAtMs, 0)));
    for (const resultEntry of warpBatchResult.data.results) {
        ...
        npcRuntime.issueAttackOrder(responderEntityID, offenderEntity.itemID, {
            allowWeapons: true,
            keepLock: true,
            allowPodKill: isCrimewatchConcordPodKillEnabled(),
        });
...
function retaskResponders(scene, offenderEntityID, responderEntityIDs, options = {}) {
    ...
    for (const responderEntityID of responderEntityIDs) {
        ...
        const orderResult = npcRuntime.issueAttackOrder(responderEntityID, offenderEntityID, {
            allowWeapons: true,
            keepLock: true,
            allowPodKill,
        });
...
function resolveCurrentOffenderEntity(scene, offenderCharacterID, fallbackEntityID = 0) {
    ...
    const liveSession = offenderCharacterID > 0
    ...
    if (liveSession &&
        ...
        const sessionEntity = scene.getEntityByID(toPositiveInt(liveSession._space.shipID, 0));
        if (sessionEntity) {
    ...
    if (offenderCharacterID > 0) {
        for (const entity of scene.dynamicEntities.values()) {
            if (toPositiveInt(entity && entity.pilotCharacterID, 0) === offenderCharacterID ||
...
function tickScene(scene, now = Date.now()) {
    ...
    for (const [offenderEntityID, response] of responseMap.entries()) {
        ...
        const offenderEntity = resolveCurrentOffenderEntity(scene, offenderCharacterID, offenderEntityID);
        ...
        if (response.responderEntityIDs.length === 0) {
            const spawnResult = spawnConcordResponders(scene, offenderEntity, response);
            response.responderEntityIDs = Array.isArray(spawnResult && spawnResult.responderEntityIDs)
                ? spawnResult.responderEntityIDs
                : [];
            response.ingressCompleteAtMs = Math.max(0, Math.trunc(toFiniteNumber(spawnResult && spawnResult.ingressCompleteAtMs, 0)));
            if (response.responderEntityIDs.length === 0) {
            ...
            if (!(response.capsuleTargeted && !allowConcordPodKill)) {
                ...
                const wakeAtMs = Math.max(now, toFiniteNumber(response.ingressCompleteAtMs, now));
            ...
            response.nextRetaskAtMs = Math.max(now, toFiniteNumber(response.ingressCompleteAtMs, now));
            continue;
        }
...
//# sourceMappingURL=crimewatchState.js.map