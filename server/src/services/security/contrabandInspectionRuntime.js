...
const { ITEM_FLAGS, listContainerItems, removeInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const { EVENT_STANDING_CONTRABAND_TRAFFICKING, applyStandingChanges, } = require(path.join(__dirname, "../character/standingRuntime"));
...
function resolveCharacterSmugglingChance(characterID) {
    ...
    applyDogmaModifierGroups(attributes, getActiveImplantCharacterModifierEntries(characterID).filter((entry) => toInt(entry && entry.modifiedAttributeID, 0) === ATTRIBUTE_SMUGGLING_MODIFIER));
...
function getSystemInspectionAuthority(solarSystemID) {
    ...
    const security = toNumber(system && (system.security ??
        system.securityStatus), NaN);
...
function listCargoItems(characterID, shipID) {
    const ownedCargo = listContainerItems(characterID, shipID, ITEM_FLAGS.CARGO_HOLD);
...
function getInspectionRoll(options = {}) {
    const explicitRoll = toNumber(options.roll ??
        options.detectionRoll, NaN);
...
function applyContrabandPenalties(characterID, authority, entries, options = {}) {
    ...
    const walletResult = chargedFine > 0
        ...
        : null;
    const standingLoss = entries.reduce((sum, entry) => sum + Math.max(0, toNumber(entry.standingLoss, 0)), 0);
    const standingResult = standingLoss > 0
        ? applyStandingChanges(characterID, [{
...
function inspectCharacterContraband(characterID, shipID, solarSystemID, options = {}) {
    ...
    const contraband = buildContrabandEntries(normalizedCharacterID, normalizedShipID, authority);
    ...
    if (options.applyPenalties !== false) {
        penalties = applyContrabandPenalties(normalizedCharacterID, authority, contraband, options);
    }
    log.info(`[Contraband] char=${normalizedCharacterID} ship=${normalizedShipID} system=${authority.solarSystemID} faction=${authority.factionID} items=${contraband.length} fine=${totalFine} standingLoss=${standingLoss.toFixed(3)}`);
    return {
        success: true,
        data: {
            inspected: true,
...
//# sourceMappingURL=contrabandInspectionRuntime.js.map