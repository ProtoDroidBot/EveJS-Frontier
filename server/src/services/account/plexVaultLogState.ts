...
function normalizeCategoryMessageID(
  ...
  return (Object.values(PLEX_LOG_CATEGORY) as number[]).includes(categoryId)
    ? categoryId
    : fallback;
}

function normalizePlexTransactionEntry(entry: Record<string, any> = {}) {
...
function sortTransactionsNewestFirst(entries: any[] = []) {
  return [...entries].sort(
    (left, right) =>
