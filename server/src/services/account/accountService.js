...
function transferCharacterBalanceToCorporation(fromCharId, toCorporationID, toAccountKey, amount, options = {}) {
    ...
    const description = resolveExplicitDescription(options, `Transfer to corporation ${Number(toCorporationID || 0)}`);
    ...
    const creditResult = adjustCorporationWalletDivisionBalance(toCorporationID, normalizedAccountKey, normalizedAmount, {
        description,
        ownerID1: fromCharId,
        ownerID2: toCorporationID,
...
function transferCorporationWalletBalance(session, toID, fromAccountKey, toAccountKey, amount, options = {}) {
    const normalizedAmount = normalizeNumber(amount, 0);
    if (!(normalizedAmount > 0)) {
        return {
            success: false,
    ...
    const normalizedFromAccountKey = normalizeCorporationWalletKey(fromAccountKey ?? session.corpAccountKey ?? session.corpaccountkey);
    const destinationCorporation = getCorporationRecord(normalizedToID);
    const description = resolveExplicitDescription(options, `Corporation wallet transfer to ${normalizedToID}`);
    if (destinationCorporation) {
        const normalizedToAccountKey = normalizeCorporationWalletKey(toAccountKey);
        if (Number(fromCorporationID) === Number(normalizedToID) &&
            ...
            return {
                ...
                from: {
                    ...
                    balance: getCorporationWalletBalance(fromCorporationID, normalizedFromAccountKey),
                },
                to: {
                    corporationID: normalizedToID,
                    accountKey: normalizedToAccountKey,
                    balance: getCorporationWalletBalance(normalizedToID, normalizedToAccountKey),
                },
                amount: normalizedAmount,
            };
        }
        const debitResult = adjustCorporationWalletDivisionBalance(fromCorporationID, normalizedFromAccountKey, -normalizedAmount, {
            description,
        ...
        const creditResult = adjustCorporationWalletDivisionBalance(normalizedToID, normalizedToAccountKey, normalizedAmount, {
            description,
            ownerID1: fromCorporationID,
            ownerID2: normalizedToID,
        ...
        if (!creditResult.success) {
            adjustCorporationWalletDivisionBalance(fromCorporationID, normalizedFromAccountKey, normalizedAmount, {
                description: `Rollback: ${description}`,
...
function buildJournalRowset(entries) {
    const rows = entries.map((entry) => buildList([
...
class AccountService extends BaseService {
    ...
    Handle_GetCashBalance(args, session, kwargs) {
        const isCorpWallet = normalizeNumber(extractKwarg(kwargs, "isCorpWallet"), NaN) ||
            normalizeNumber(args && args[0], 0);
        const walletKey = resolveAccountKey(extractKwarg(kwargs, "accountKey") ?? (args && args[1]));
        if (isCorpWallet) {
            return getCorporationWalletBalance(session && (session.corporationID || session.corpid), walletKey.id);
        }
    ...
    Handle_GetEntryTypes(args = []) {
        const entryTypes = buildList([...ENTRY_TYPE_METADATA_BY_ID.entries()].map(([entryTypeID, [entryTypeName, entryTypeNameID, entryJournalMessageID]]) => buildKeyVal([
    ...
    Handle_GetWalletDivisionsInfo(args, session) {
        ...
        if (corporationID) {
            return buildList(getCorporationWalletDivisionsInfo(corporationID).map((division) => buildKeyVal([
    ...
    Handle_GetJournal(args, session) {
        ...
        if (isCorpWallet) {
            return buildJournalRowset(getCorporationWalletJournal(session && (session.corporationID || session.corpid), {
                accountKey: args && args[0],
                year: args && args[1],
                month: args && args[2],
            }));
        }
        return buildJournalRowset(getCharacterWalletJournal(session && session.characterID));
    }
    Handle_GetJournalForAccounts(args, session) {
        ...
        if (session && (session.corporationID || session.corpid)) {
            return buildJournalRowset(getCorporationWalletJournal(session.corporationID || session.corpid, {
                accountKey,
            }));
        }
        return buildJournalRowset(getCharacterWalletJournal(session && session.characterID));
    ...
    Handle_GetTransactions(args, session, kwargs) {
        ...
        const walletKey = resolveAccountKey(extractKwarg(kwargs, "accountKey") ?? (args && args[0]));
        const year = normalizeNumber(args && args[1], NaN);
        const month = normalizeNumber(args && args[2], NaN);
        if (isCorpWallet) {
            return buildTransactionList(getCorporationWalletTransactions(session && (session.corporationID || session.corpid), {
                accountKey: walletKey.id,
                year,
                month,
            }));
        }
        return buildTransactionList(getCharacterWalletTransactions(session && session.characterID, {
            accountKey: walletKey.id,
            year,
            month,
        }));
    ...
    Handle_GiveCash(args, session, kwargs) {
        ...
        const reason = normalizeText(extractKwarg(kwargs, "reason") ?? (args && args[2]), `Player donation to ${toID}`);
        ...
        const result = destinationCorporation
            ? transferCharacterBalanceToCorporation(session.characterID, toID, extractKwarg(kwargs, "toAccountKey"), amount, {
                description: reason,
            })
        ...
        if (!result.success) {
            log.warn(`[AccountService] GiveCash failed: ${result.errorMsg} from=${session.characterID} to=${toID} amount=${amount}`);
            return null;
        }
        if (destinationCorporation) {
            return [
    ...
    Handle_GiveCashFromCorpAccount(args, session, kwargs) {
        ...
        const reason = normalizeText(extractKwarg(kwargs, "reason") ?? (args && args[3]), `Corporation wallet transfer to ${toID}`);
        if (!session || !(toID > 0) || !(amount > 0)) {
            return null;
        }
        const result = transferCorporationWalletBalance(session, toID, fromAccountKey, extractKwarg(kwargs, "toAccountKey"), amount, {
            description: reason,
        });
        if (!result.success) {
            log.warn(`[AccountService] GiveCashFromCorpAccount failed: ${result.errorMsg} corp=${resolveSessionCorporationID(session)} to=${toID} amount=${amount}`);
        }
        return null;
    }
}
