...
const { getKwarg, toMarshalValue, } = require(path.join(__dirname, "../newEdenStore/storeMarshal"));
const { getLegacyCatalog, getStoreConfig, } = require(path.join(__dirname, "../newEdenStore/storeState"));
const { purchaseLegacyOffer, } = require(path.join(__dirname, "../newEdenStore/storeFulfillment"));
...
class StoreManagerService extends BaseService {
    ...
    Handle_get_offers(args, ..._extra) {
        const storeID = toPositiveInteger(args && args[0], STORE_ID_INGAME);
        const catalog = getLegacyCatalog(storeID);
        log.info(`[StoreManager] get_offers store_id=${storeID} count=${catalog.offers.length}`);
        return toMarshalValue(catalog.offers);
    }
    Handle_get_categories(args, ..._extra) {
        const storeID = toPositiveInteger(args && args[0], STORE_ID_INGAME);
        const catalog = getLegacyCatalog(storeID);
        log.info(`[StoreManager] get_categories store_id=${storeID} count=${catalog.categories.length}`);
        return toMarshalValue(catalog.categories);
    }
    Handle_get_products(args, ..._extra) {
        const storeID = toPositiveInteger(args && args[0], STORE_ID_INGAME);
        const catalog = getLegacyCatalog(storeID);
        log.info(`[StoreManager] get_products store_id=${storeID} count=${catalog.products.length}`);
    ...
    Handle_buy_offer(args, session, kwargs) {
        ...
        const payerCharacterID = toPositiveInteger(getKwarg(kwargs, "from_character_id"), toPositiveInteger(session && (session.characterID || session.charid), 0));
        const requestedTargetCharacterID = toPositiveInteger(getKwarg(kwargs, "to_character_id"), 0);
        const targetCharacterID = requestedTargetCharacterID || payerCharacterID;
        const accountID = toPositiveInteger(session && (session.userid || session.userID), 0);
        ...
        if (!result.success) {
            log.warn(`[StoreManager] buy_offer rejected offer_id=${offerID} store_id=${storeID} ` +
                `currency=${currency} quantity=${quantity} payer=${payerCharacterID} ` +
                `target=${targetCharacterID} requestedTarget=${requestedTargetCharacterID} ` +
                `account=${accountID} error=${result.errorMsg}`);
            return null;
        }
        ...
        log.info(`[StoreManager] buy_offer payer=${responsePayload.payer_character_id} ` +
    ...
    Handle_can_purchase_offer(args, session, kwargs) {
        ...
        log.debug(`[StoreManager] can_purchase_offer requested=${String(requestedOfferID || "")} ` +
