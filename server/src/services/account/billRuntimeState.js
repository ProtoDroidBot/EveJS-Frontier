...
const { adjustCharacterBalance, JOURNAL_ENTRY_TYPE, } = require(path.join(__dirname, "./walletState"));
const { adjustCorporationWalletDivisionBalance, normalizeCorporationWalletKey, } = require(path.join(__dirname, "../corporation/corpWalletState"));
...
function getAutomaticPaySettingsForOwner(ownerID) {
    const table = ensureTable();
    return normalizeAutomaticPaySettings(table.automaticPaySettingsByOwner[String(ownerID)] || {});
}
...
function normalizeBillRecord(bill) {
    return {
        ...
        dueDateTime: String(bill && bill.dueDateTime ? bill.dueDateTime : currentFileTime()),
        ...
        externalID2: bill && bill.externalID2 !== undefined && bill.externalID2 !== null
            ...
            : -1,
        processingStatus: bill && typeof bill.processingStatus === "string"
            ? bill.processingStatus
            : null,
        processedDateTime: bill && bill.processedDateTime ? String(bill.processedDateTime) : null,
...
function notifyBillReceived(ownerID, billID) {
    ...
    for (const session of sessionRegistry.getSessions()) {
        if (Number(session && (session.corporationID || session.corpid)) !== numericOwnerID &&
...
function createBill({ billTypeID, amount, debtorID, creditorID, interest = 0, dueDateTime = null, externalID = -1, externalID2 = -1, emitBillReceived = true, } = {}) {
...
function markBillProcessed(billID, processingStatus, options = {}) {
    const numericBillID = Number(billID) || 0;
    const normalizedStatus = typeof processingStatus === "string" && processingStatus.trim() !== ""
...
function getBillPaymentEntryTypeID(bill) {
    if (Number(bill && bill.billTypeID) === BILL_TYPE_RENTAL &&
...
function creditCorporationBillCreditor(bill, paidByOwnerID) {
    ...
    adjustCorporationWalletDivisionBalance(creditorID, DEFAULT_DIVISION_ID, normalizeMoney(bill.amount, 0), {
...
function payBillFromCorporation(billID, corporationID, accountKey) {
    ...
    const debitResult = adjustCorporationWalletDivisionBalance(numericCorporationID, normalizeCorporationWalletKey(accountKey), -bill.amount, {
        entryTypeID: getBillPaymentEntryTypeID(bill),
...
function tryAutoPayBill(billID) {
    ...
    if (getCorporationRecord(bill.debtorID)) {
        return payBillFromCorporation(bill.billID, bill.debtorID, debtorSettings.divisionID || DEFAULT_DIVISION_ID);
    }
    const alliance = getAllianceRecord(bill.debtorID);
    if (!alliance) {
        return null;
    ...
    return payBillFromCorporation(bill.billID, executorCorporationID, executorSettings.divisionID || DEFAULT_DIVISION_ID);
}
...
function listDueBills({ nowFileTime = currentFileTime(), billTypeID = null, externalID = null, } = {}) {
    const now = normalizeFileTimeBoundary(nowFileTime);
    const normalizedBillTypeID = billTypeID === null || billTypeID === undefined
        ? null
        : normalizeBillTypeID(billTypeID);
    const normalizedExternalID = externalID === null || externalID === undefined
        ? null
        : Number(externalID) || -1;
    const table = ensureTable();
    return Object.values(table.bills || {})
...
//# sourceMappingURL=billRuntimeState.js.map