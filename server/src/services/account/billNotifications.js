...
const { getAllianceRecord, getAllianceCorporationIDs, getCharacterIDsInCorporation, getCorporationRecord, } = require(path.join(__dirname, "../corporation/corporationState"));
...
// a character debtor gets type 12; corporation and alliance members get type 13.
...
function notifyBillPaid(bill) {
    ...
    const isAllianceDebtor = Boolean(getAllianceRecord(debtorID));
    let recipients = [debtorID];
    if (isCorporationDebtor) {
        recipients = getCharacterIDsInCorporation(debtorID);
    }
    else if (isAllianceDebtor) {
        recipients = getAllianceCorporationIDs(debtorID).flatMap((corporationID) => getCharacterIDsInCorporation(corporationID));
    }
    recipients = [...new Set(recipients)];
    const typeID = isCorporationDebtor || isAllianceDebtor
...
//# sourceMappingURL=billNotifications.js.map