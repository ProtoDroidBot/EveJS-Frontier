...
class VaultManagerService extends BaseService {
    ...
    Handle_get_account_balance(args, session, ..._extra) {
        const currency = normalizeText(args && args[0], "").toUpperCase();
        const wallet = getCharacterWallet(session && session.characterID);
        const balance = currency === PLEX_CURRENCY && wallet ? wallet.plexBalance : 0;
        log.info(`[VaultManager] get_account_balance currency=${currency || "<none>"} ` +
...
//# sourceMappingURL=vaultManagerService.js.map