...
const { buildDict, buildKeyVal, buildList, buildRowset, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getCharacterCreationSchools, } = require(path.join(__dirname, "../character/characterCreationData"));
...
class TutorialSvcService extends BaseService {
    ...
    Handle_GetCriterias() {
        ...
        return buildRowset(["criteriaID", "messageTextID", "criteriaTypeID", "pageCriteriaID"], []);
    }
    Handle_GetCategories() {
        log.debug("[TutorialSvc] GetCategories");
        return buildRowset(["categoryID", "categoryNameID", "descriptionID"], []);
    }
    Handle_GetActions() {
...
//# sourceMappingURL=tutorialSvcService.js.map