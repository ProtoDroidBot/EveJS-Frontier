...
const { PLEX_LOG_CATEGORY, getFileTimeNowString, getTransactionID, appendCharacterPlexTransaction, fileTimeStringToDate, } = require(path.join(__dirname, "./plexVaultLogState"));
const ACCOUNT_KEY = {
    CASH: 1000,
    AURUM: 1200,
};
...
function publishPlexBalanceChangedNotice(characterID, balanceInCents, deltaInCents) {
    const gateway = require(path.join(__dirname, "../../_secondary/express/publicGatewayLocal"));
    return gateway && typeof gateway.publishPlexBalanceChangedNotice === "function"
        ? gateway.publishPlexBalanceChangedNotice(characterID, balanceInCents, deltaInCents)
        : null;
}
...
function getCharacterWallet(charId) {
    ...
    return {
        ...
        balanceChange: normalizeMoney(record.balanceChange, DEFAULT_WALLET.balanceChange),
    };
}
...
function normalizeMarketTransactionEntry(entry = {}) {
    ...
    return {
        ...
        locationID: Math.max(0, Math.trunc(Number(entry.locationID ?? entry.stationID) || 0)),
        ...
        accountID: Math.max(0, Math.trunc(Number(entry.accountID || ACCOUNT_KEY.CASH) || ACCOUNT_KEY.CASH)),
        buyerAccountID: Math.max(0, Math.trunc(Number(entry.buyerAccountID || ACCOUNT_KEY.CASH) || ACCOUNT_KEY.CASH)),
        sellerAccountID: Math.max(0, Math.trunc(Number(entry.sellerAccountID || ACCOUNT_KEY.CASH) || ACCOUNT_KEY.CASH)),
...
function appendCharacterMarketTransaction(charId, entry) {
    ...
    const writeResult = getCharacterState().updateCharacterRecord(charId, (record) => {
        appendLimitedRecordEntry(record, "marketTransactions", normalizedEntry, MAX_MARKET_TRANSACTION_ENTRIES);
        return record;
    });
    return {
        ...writeResult,
...
function getCharacterWalletTransactions(charId, options = {}) {
    ...
    return getCharacterWalletJournal(charId).filter((entry) => {
        if ((Number(entry && entry.accountKey) || ACCOUNT_KEY.CASH) !== accountKey) {
            return false;
        }
        if (hasMonthFilter) {
...
function emitAccountChangeToSession(session, options = {}) {
    ...
    const ownerID = Number(options.ownerID || session.characterID || session.userid || 0) || 0;
...
function notifyCharacterWalletChange(charId, wallet, options = {}) {
    const sessions = sessionRegistry
        .getSessions()
        .filter((session) => Number(session.characterID || 0) === Number(charId || 0));
    for (const session of sessions) {
        ...
        emitAccountChangeToSession(session, {
            ...
            ownerID: charId,
            balance: options.balance !== undefined && options.balance !== null
                ? options.balance
                : wallet.balance,
        });
    }
...
function setCharacterBalance(charId, nextBalance, options = {}) {
    ...
    const journalEntry = {
        ...
        entryTypeID: Number(options.entryTypeID || JOURNAL_ENTRY_TYPE.ADMIN_ADJUSTMENT) || 0,
        ...
        currency: Number(options.currency || JOURNAL_CURRENCY.ISK) || JOURNAL_CURRENCY.ISK,
        sortValue: 1,
    };
    const writeResult = getCharacterState().updateCharacterRecord(charId, (record) => {
...
function adjustCharacterBalance(charId, amount, options = {}) {
    ...
    return setCharacterBalance(charId, currentWallet.balance + delta, {
        ...options,
        description: resolveLedgerReason(options, delta >= 0 ? "Wallet credit" : "Wallet debit"),
    });
}
function transferCharacterBalance(fromCharId, toCharId, amount, options = {}) {
    ...
    const description = resolveLedgerReason(options, `Transfer to ${Number(toCharId || 0)}`);
...
function setCharacterPlexBalance(charId, nextBalance, options = {}) {
    ...
    const normalizedBalance = normalizePlex(nextBalance, currentWallet.plexBalance);
    const writeResult = getCharacterState().updateCharacterRecord(charId, (record) => {
    ...
    if (delta !== 0 && options.recordTransaction !== false) {
        ...
        const categoryMessageID = Number(options.categoryMessageID || 0) || PLEX_LOG_CATEGORY.CCP;
        const logResult = appendCharacterPlexTransaction(charId, {
            ...
            balance: normalizedBalance,
            categoryMessageID,
            summaryMessageID: Number(options.summaryMessageID || 0) || categoryMessageID,
    ...
    publishPlexBalanceChangedNotice(charId, updatedWallet.plexBalance, delta);
    return {
        success: true,
        data: updatedWallet,
...
function adjustCharacterPlexBalance(charId, amount, options = {}) {
    ...
    return setCharacterPlexBalance(charId, currentWallet.plexBalance + normalizePlexDelta(amount, 0), options);
}
module.exports = {
    ACCOUNT_KEY,
    ACCOUNT_KEY_NAME,
