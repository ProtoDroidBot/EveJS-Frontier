...
class StoreManagerService extends BaseService {
  ...
  Handle_get_offers(args, ..._extra: any[]) {
  ...
  Handle_get_categories(args, ..._extra: any[]) {
    const storeID = toPositiveInteger(args && args[0], STORE_ID_INGAME);
    const catalog = getLegacyCatalog(storeID);
    log.info(
  ...
  Handle_get_products(args, ..._extra: any[]) {
    const storeID = toPositiveInteger(args && args[0], STORE_ID_INGAME);
    const catalog = getLegacyCatalog(storeID);
    log.info(
  ...
  Handle_buy_offer(args, session, kwargs) {
    ...
    const requestedTargetCharacterID = toPositiveInteger(
      getKwarg(kwargs, "to_character_id"),
      0,
    );
