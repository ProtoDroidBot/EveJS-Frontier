...
const { buildDict, buildFiletimeLong, buildKeyVal, buildList, extractDictEntries, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { buildNotEnoughMoneyUserErrorValues, } = require(path.join(__dirname, "./walletState"));
const { buildAutomaticPaySettingsSnapshot, getBillRecord, listBillsForCreditor, listBillsForDebtor, payBillFromCharacter, payBillFromCorporation, setAutomaticPaySettingsForOwner, } = require(path.join(__dirname, "./billRuntimeState"));
const { processDueOfficeRentalBills, } = require(path.join(__dirname, "../corporation/officeRentalBilling"));
const { processDueWarBills, } = require(path.join(__dirname, "../corporation/warRuntimeState"));
...
function throwBillPaymentFailure(result, bill = null) {
    ...
    if (errorMsg === "INSUFFICIENT_FUNDS") {
        const balance = result && result.data && result.data.balance !== undefined
            ? result.data.balance
            : 0;
        throwWrappedUserError("NotEnoughMoney", buildNotEnoughMoneyUserErrorValues(Number(bill && bill.amount) || 0, balance));
    }
...
class BillManagerService extends BaseService {
    ...
    Handle_GetAutomaticPaySettings(args, session) {
        ...
        return buildDict(Object.entries(buildAutomaticPaySettingsSnapshot(ownerIDs)).map(([ownerID, settings]) => [
            Number(ownerID),
            buildDict(Object.entries(settings).map(([key, value]) => [key === "divisionID" ? key : Number(key), value])),
        ]));
    }
    ...
    Handle_GetCorporationBills(args, session) {
        ...
        return buildList(listBillsForDebtor(resolveCorporationID(session)).map((bill) => buildBillPayload(bill)));
    }
    Handle_GetCorporationBillsReceivable(args, session) {
        processDueOfficeRentalBills({ session });
        processDueWarBills({ session });
        return buildList(listBillsForCreditor(resolveCorporationID(session)).map((bill) => buildBillPayload(bill)));
    }
    Handle_PayCorporationBill(args, session, kwargs) {
        const billID = Number(args && args[0]) || 0;
        const fromAccountKey = Number(extractKwargValue(kwargs, "fromAccountKey") ??
            (args && args[1]) ??
            1000) || 1000;
        const bill = getBillRecord(billID);
        const result = payBillFromCorporation(billID, resolveCorporationID(session), fromAccountKey);
...
//# sourceMappingURL=billMgrService.js.map