...
function normalizeCategoryMessageID(value, fallback = PLEX_LOG_CATEGORY.UNCATEGORIZED) {
...
function normalizePlexTransactionEntry(entry = {}) {
    const categoryMessageID = normalizeCategoryMessageID(entry.categoryMessageID, PLEX_LOG_CATEGORY.UNCATEGORIZED);
    ...
    return {
        ...
        summaryMessageID: rawSummaryMessageID > 0 ? rawSummaryMessageID : categoryMessageID,
        summaryText,
        reason,
    };
}
function sortTransactionsNewestFirst(entries = []) {
    return [...entries].sort((left, right) => normalizeInteger(right && right.transactionID, 0) -
...
function getCharacterPlexTransactions(charId) {
    ...
    return sortTransactionsNewestFirst(record.plexVaultTransactions.map((entry) => normalizePlexTransactionEntry(cloneValue(entry))));
}
function appendCharacterPlexTransaction(charId, entry) {
    ...
    const writeResult = updateCharacterRecord(charId, (record) => {
        const nextEntries = Array.isArray(record.plexVaultTransactions)
            ? record.plexVaultTransactions.map((candidate) => normalizePlexTransactionEntry(cloneValue(candidate)))
            : [];
        nextEntries.unshift(normalizedEntry);
        record.plexVaultTransactions = sortTransactionsNewestFirst(nextEntries).slice(0, MAX_PLEX_TRANSACTION_ENTRIES);
        return record;
    });
    return {
        ...writeResult,
...
function getCharacterPlexTransaction(charId, transactionId) {
    ...
    return (getCharacterPlexTransactions(charId).find((entry) => normalizeInteger(entry && entry.transactionID, 0) === targetId) || null);
}
function getCharacterPlexTransactionStatistics(charId) {
    const transactions = getCharacterPlexTransactions(charId).slice(0, MAX_PLEX_TRANSACTION_ENTRIES);
    const groupedEntries = new Map();
    for (const transaction of transactions) {
        const categoryMessageID = normalizeCategoryMessageID(transaction && transaction.categoryMessageID, PLEX_LOG_CATEGORY.UNCATEGORIZED);
...
//# sourceMappingURL=plexVaultLogState.js.map