...
function normalizeAutomaticPaySettings(settings: Record<string, any> = {}) {
...
function buildAutomaticPaySettingsSnapshot(ownerIDs: any[] = []) {
...
function listBillsForDebtor(ownerID) {
  ...
  const table = ensureTable();
  return Object.values<any>(table.bills || {})
...
function markBillProcessed(billID, processingStatus, options: Record<string, any> = {}) {
  const numericBillID = Number(billID) || 0;
...
function payBillFromCharacter(billID, characterID) {
  ...
  if (!bill || bill.paid || bill.debtorID !== numericCharacterID) {
    return { success: false as const, errorMsg: "BILL_NOT_PAYABLE" };
  }
  ...
  return { success: true as const, data: getBillRecord(bill.billID) };
}
...
function payBillFromCorporation(billID, corporationID, accountKey) {
  ...
  if (bill.debtorID === numericCorporationID) {
    ...
    if (Number(alliance.executorCorporationID) !== numericCorporationID) {
      return { success: false as const, errorMsg: "ONLY_EXECUTOR_CAN_PAY" };
    }
  }
