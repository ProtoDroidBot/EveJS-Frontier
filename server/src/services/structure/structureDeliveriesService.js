...
const { ITEM_FLAGS, findItemById, transferItemToOwnerLocation, } = require(path.join(__dirname, "../inventory/itemStore"));
...
function createStructureItemsDeliveredNotification(structure, recipientCharacterID, senderCharacterID, deliveredItems) {
...
class StructureDeliveriesService extends BaseService {
    ...
    Handle_Deliver(args, session) {
        ...
        const recipientCharacterID = toPositiveInt(Array.isArray(args) ? unwrapMarshalValue(args[1]) : null, 0);
        ...
        for (const itemID of itemIDs) {
            ...
            const moveResult = transferItemToOwnerLocation(item.itemID, recipientCharacterID, structureID, ITEM_FLAGS.DELIVERIES);
        ...
        createStructureItemsDeliveredNotification(structure, recipientCharacterID, senderCharacterID, deliveredItems);
        return null;
    }
}
...
//# sourceMappingURL=structureDeliveriesService.js.map