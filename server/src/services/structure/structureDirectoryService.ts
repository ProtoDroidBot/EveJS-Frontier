...
function canReadCorporationStructureDirectory(session) {
  ...
  const allowedRoles = (CORP_ROLE_STATION_MANAGER as bigint) | (CORP_ROLE_BRAND_MANAGER as bigint);
...
function listRequestedStructures(args, options: Record<string, any> = {}) {
...
function buildJumpBridgeAccessPayload(session) {
  ...
  const seenPairKeys = new Set<any>();
  const pairs: any[] = [];
  const hasAccessTo: any[] = [];
  const hasNoAccessTo: any[] = [];
