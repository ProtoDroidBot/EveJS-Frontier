...
const { getActiveShipItem, moveShipToSpace, } = require(path.join(__dirname, "../inventory/itemStore"));
const { buildStructureEmergencySpaceState, } = require(path.join(__dirname, "./structureSpaceInterop"));
const { queuePostSpaceAttachFittingHydration, } = require(path.join(__dirname, "../../space/modules/spaceAttachHydration"));
...
function evacuateDockedCharactersFromStructure(structure, options = {}) {
    ...
    for (const [index, occupant] of occupants.entries()) {
        ...
        if (activeShip) {
            const moveShipResult = moveShipToSpace(activeShip.itemID, solarSystemID, emergencySpaceState);
            if (!moveShipResult.success) {
                log.warn(`[StructureDockedRecovery] Failed to move docked ship ${activeShip.itemID} for character ${occupant.characterID} out of destroyed structure ${structureID}: ${moveShipResult.errorMsg}`);
            }
            else {
        ...
        const nextRecord = {
            ...
            structureID: null,
            solarSystemID,
            constellationID: toPositiveInt(system.constellationID, toPositiveInt(occupant.record && occupant.record.constellationID, 0)) || null,
            regionID: toPositiveInt(system.regionID, toPositiveInt(occupant.record && occupant.record.regionID, 0)) || null,
        };
        const updateResult = writeCharacterRecord(occupant.characterID, nextRecord);
        if (!updateResult.success) {
            log.warn(`[StructureDockedRecovery] Failed to update character ${occupant.characterID} after structure ${structureID} destruction: ${updateResult.errorMsg}`);
            continue;
        }
        ...
        if (session) {
            try {
                ...
                restoredLive = Boolean(activeShip &&
                ...
                log.warn(`[StructureDockedRecovery] Live session rebuild failed for character ${occupant.characterID} after structure ${structureID} destruction: ${error.message}`);
            }
        }
        recovered.push({
            characterID: occupant.characterID,
...
//# sourceMappingURL=structureDockedRecoveryState.js.map