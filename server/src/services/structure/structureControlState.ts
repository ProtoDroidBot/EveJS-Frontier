...
function getSessionSolarSystemID(session, options: Record<string, any> = {}) {
  return normalizePositiveInt(
...
function getStructurePilotSession(structureID, options: Record<string, any> = {}) {
...
function getStructurePilotCharacterID(structureID, options: Record<string, any> = {}) {
...
function queueStructureFightersForControlRelease(session, structureID, options: Record<string, any> = {}) {
  const targetStructureID = normalizePositiveInt(structureID, 0);
...
function relinquishStructureControl(session, options: Record<string, any> = {}) {
  const structureID = getSessionStructureID(session);
...
function boardStructureControlFromSpace(session, structureID, options: Record<string, any> = {}) {
  const targetStructureID = normalizePositiveInt(structureID, 0);
  if (!targetStructureID) {
    return {
      success: false as const,
...
function ejectFromStructureControl(session, options: Record<string, any> = {}) {
  const structureID = getSessionStructureID(session);
  if (!structureID || !isControllingStructureSession(session, structureID)) {
    return {
      success: true as const,
...
function assumeStructureControl(session, structureID, options: Record<string, any> = {}) {
  const targetStructureID = normalizePositiveInt(structureID, 0);
  if (!targetStructureID) {
    return {
      success: false as const,
