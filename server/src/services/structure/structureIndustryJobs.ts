...
function listServiceStateTransitions(beforeServiceStates: Record<string, any> = {}, afterServiceStates: Record<string, any> = {}) {
  const serviceIDs = new Set([
    ...Object.keys(beforeServiceStates || {}),
    ...Object.keys(afterServiceStates || {}),
  ]);
  const onlineServiceIDs: any[] = [];
...
function syncIndustryJobsForServiceStateTransition(
  structureID,
  beforeServiceStates: Record<string, any> = {},
  afterServiceStates: Record<string, any> = {},
) {
...
function cancelIndustryJobsForRemovedServiceModule(
  ...
  if (removedServiceIDs.length === 0) {
    return { success: true as const, data: { changedJobIDs: [] }, unbackedServiceIDs: [] };
  }
...
function cancelIndustryJobsForStructureLifecycle(structureID, options: Record<string, any> = {}) {
  const industryRuntime = require(path.join(__dirname, "../industry/industryRuntimeState"));
