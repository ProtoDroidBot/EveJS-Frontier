...
const { buildDict, buildFiletimeLong, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
const typeListAuthority = require(path.join(__dirname, "../inventory/typeListAuthority"));
const structureState = require(path.join(__dirname, "./structureState"));
const { STRUCTURE_STATE, } = require(path.join(__dirname, "./structureConstants"));
const { STRUCTURE_SETTING_ID, } = require(path.join(__dirname, "./structureServiceAuthority"));
const { characterHasStructureSetting, } = require(path.join(__dirname, "./structurePayloads"));
const MoonExtractionsService = require(path.join(__dirname, "./moonExtractionsService"));
...
function getItemQuantity(item) {
    ...
    return Math.max(0, normalizeNonNegativeInteger(item.stacksize, 0), normalizeNonNegativeInteger(item.quantity, 0));
}
function getTypeContext(itemOrTypeID) {
    const typeID = normalizePositiveInteger(typeof itemOrTypeID === "object"
    ...
    return {
        ...
        categoryID: normalizePositiveInteger(source.categoryID, normalizePositiveInteger(type.categoryID, 0)),
...
function buildNumericDictFromMap(amountsByTypeID) {
    return buildDict(sortNumericDictEntries([...amountsByTypeID.entries()]
...
function getOutputBayCapacityForStructure(structure) {
    const structureTypeID = normalizePositiveInteger(structure && structure.typeID, TYPE_UPWELL_AUTO_MOON_MINER);
    return normalizeNonNegativeNumber(getTypeDogmaAttribute(structureTypeID, ATTRIBUTE_OUTPUT_MOON_MATERIAL_BAY_CAPACITY, DEFAULT_OUTPUT_MOON_MATERIAL_BAY_CAPACITY), DEFAULT_OUTPUT_MOON_MATERIAL_BAY_CAPACITY);
}
function isFuelOrReagent(typeContext) {
    return (typeContext.groupID === GROUP_FUEL_BLOCK ||
...
function isAutoMoonMinerOutputMaterial(typeContext) {
    ...
    return typeListAuthority.matchesTypeListWithMissingFallback(typeContext, OUTPUT_MATERIAL_TYPE_LIST_ID, () => typeContext.groupID === GROUP_MOON_MATERIAL, "auto-moon-miner-output");
}
function sumItemsByType(items = [], predicate = (..._extra) => true) {
...
function resolveNextHarvestTime(profile) {
    ...
    const explicitNextHarvestMs = normalizeNonNegativeInteger(profile.nextHarvestTimeMs || profile.nextHarvestTime, 0);
    if (explicitNextHarvestMs > 0) {
        return fileTimeLongFromMs(explicitNextHarvestMs);
    }
    const lastHarvestMs = normalizeNonNegativeInteger(profile.lastHarvestTimeMs || profile.lastHarvestTime, 0);
    const cycleSeconds = normalizeNonNegativeInteger(profile.miningCycleTimeSeconds || profile.miningCycleTime, 0);
...
function extractStructureID(args = [], session = {}) {
    return normalizePositiveInteger(Array.isArray(args) && args.length > 1
        ? args[1]
        : session.structureid || session.structureID || session.shipid, 0);
}
...
function hasStationManagerRole(session) {
    const roleValue = session && (session.corprole ||
        ...
        session.rolesAtAll ||
        0);
    return (normalizeRoleBitfield(roleValue) & CORP_ROLE_STATION_MANAGER) === CORP_ROLE_STATION_MANAGER;
}
...
function getStructureDisplayName(structure, structureID) {
    return (structure &&
        (structure.itemName || structure.name || structure.structureName)) || `Structure ${normalizePositiveInteger(structureID, 0)}`;
}
...
function ensureCanAccessAutoMoonMiner(session, structure, structureID) {
    const hasAccess = Boolean(structure) && (hasAutoMoonMinerOwnerStationManagerAccess(session, structure) ||
...
class AutoMoonMiningService extends BaseService {
    constructor(options = {}) {
        ...
        this._resourceProfileProvider =
            options.resourceProfileProvider ||
                ((structureID) => MoonExtractionsService.readResourceProfileForStructure(structureID));
        this._structureProvider =
            options.structureProvider ||
                ((structureID) => structureState.getStructureByID(structureID, { refresh: false }));
        this._containerItemProvider =
            options.containerItemProvider ||
                ((structureID, flagID) => itemStore.listContainerItems(null, structureID, flagID));
    }
    _getResourceProfile(structureID) {
        try {
            ...
            log.warn(`[AutoMoonMining] Failed to resolve resource profile for structure ${structureID}: ${error.message}`);
            return null;
        }
    }
    _getStructure(structureID) {
        try {
            ...
            log.warn(`[AutoMoonMining] Failed to resolve structure ${structureID}: ${error.message}`);
            return null;
        }
    }
    _getContainerItems(structureID, flagID) {
        try {
            ...
            log.warn(`[AutoMoonMining] Failed to list structure ${structureID} flag ${flagID}: ${error.message}`);
            return [];
        }
    }
    ...
    _buildAvailableFuelPayload(structureID) {
        return buildNumericDictFromMap(sumItemsByType(this._getContainerItems(structureID, itemStore.ITEM_FLAGS.STRUCTURE_FUEL_BAY), (typeContext) => isFuelOrReagent(typeContext)));
    }
    _buildOutputMaterialsPayload(structureID) {
        return buildNumericDictFromMap(sumItemsByType(this._getContainerItems(structureID, FLAG_MOON_MATERIAL_BAY), (typeContext) => isAutoMoonMinerOutputMaterial(typeContext)));
    }
    _getRemainingOutputBayCapacity(structure, structureID) {
        const capacity = getOutputBayCapacityForStructure(structure);
        const used = getOutputBayUsedVolume(this._getContainerItems(structureID, FLAG_MOON_MATERIAL_BAY));
...
//# sourceMappingURL=autoMoonMiningService.js.map