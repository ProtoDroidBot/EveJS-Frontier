...
const { STRUCTURE_SERVICE_ID, STRUCTURE_SERVICE_STATE, STRUCTURE_STATE, STRUCTURE_UPKEEP_STATE, } = require(path.join(__dirname, "./structureConstants"));
...
function getAutoMoonFuelProfile(structureTypeID) {
    ...
    const serviceModuleTypeID = toPositiveInt(getDogmaAttribute(TYPE_UPWELL_AUTO_MOON_MINER, ATTRIBUTE_BUILT_IN_SERVICE_MODULE, TYPE_STANDUP_METENOX_MOON_DRILL), TYPE_STANDUP_METENOX_MOON_DRILL);
    const hourlyFuelBlocks = Math.max(0, toInt(getDogmaAttribute(serviceModuleTypeID, ATTRIBUTE_SERVICE_MODULE_FUEL_AMOUNT, 5), 5));
    const onlineFuelBlocks = Math.max(0, toInt(getDogmaAttribute(serviceModuleTypeID, ATTRIBUTE_SERVICE_MODULE_ONLINE_FUEL_AMOUNT, 1000), 1000));
...
function getItemGroupID(item) {
    ...
    return toPositiveInt(item && item.groupID, toPositiveInt(typeRecord && typeRecord.groupID, 0));
}
...
function listMetenoxFuelBlockStacks(structureID, profile) {
    ...
    return listStructureFuelStacks(structureID, (item) => getItemGroupID(item) === fuelGroupID);
}
function listMetenoxMagmaticGasStacks(structureID) {
    return listStructureFuelStacks(structureID, (item) => toPositiveInt(item && item.typeID, 0) === TYPE_COLONY_REAGENT_LAVA);
}
...
function consumeMetenoxFuel(structureID, profile, fuelBlockQuantity, magmaticGasQuantity) {
    ...
    if (availableFuelBlocks < requiredFuelBlocks ||
...
function estimateFuelExpiresAt(structureID, profile, nextCycleAt) {
    ...
    if (toInt(profile.hourlyFuelBlocks, 0) > 0) {
        cycleCounts.push(Math.floor(sumStackQuantity(listMetenoxFuelBlockStacks(structureID, profile)) /
            Math.max(1, toInt(profile.hourlyFuelBlocks, 0))));
    }
    if (toInt(profile.hourlyMagmaticGas, 0) > 0) {
        cycleCounts.push(Math.floor(sumStackQuantity(listMetenoxMagmaticGasStacks(structureID)) /
...
function isStructureDamagedForAutoMoonServiceOnline(structure) {
    const conditionState = structure &&
...
function buildOnlineAutoMoonFuelPatch(structure, profile, nowMs) {
    ...
    return {
        ...
        fuelExpiresAt: estimateFuelExpiresAt(structure.structureID, profile, nextCycleAt),
        devFlags: {
            ...(structure.devFlags || {}),
...
function prepareAutoMoonMinerServiceStateTransition(structure, serviceID, serviceState, options = {}) {
    ...
    const nextOnline = toInt(serviceState, STRUCTURE_SERVICE_STATE.OFFLINE) ===
    ...
    const fuelResult = consumeMetenoxFuel(structure.structureID, profile, profile.onlineFuelBlocks, 0);
...
function applyAutoMoonMinerFuelCycle(structure, nowMs = Date.now()) {
    ...
    if (toInt(nowMs, Date.now()) < nextCycleAt) {
        const fuelExpiresAt = estimateFuelExpiresAt(structure.structureID, profile, nextCycleAt);
    ...
    const fuelResult = consumeMetenoxFuel(structure.structureID, profile, profile.hourlyFuelBlocks * cycleCount, profile.hourlyMagmaticGas * cycleCount);
    if (!fuelResult || fuelResult.success !== true) {
        return offlineAutoMoonService(structure, profile, nowMs, {
    ...
    return {
        structure: {
            ...structure,
            fuelExpiresAt: estimateFuelExpiresAt(structure.structureID, profile, nextCycle),
            devFlags: {
                ...(structure.devFlags || {}),
...
//# sourceMappingURL=structureAutoMoonFuelRuntime.js.map