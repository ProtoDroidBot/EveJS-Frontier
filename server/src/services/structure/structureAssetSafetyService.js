...
function getSessionCharacterID(session) {
    return normalizePositiveInt(session && (session.characterID || session.charID || session.charid || session.userid), 0);
}
...
function getNotificationTargets(ownerKind, ownerID, fallbackSession = null) {
    ...
    return [...targets].filter((session) => session && typeof session.sendNotification === "function");
}
...
class StructureAssetSafetyService extends BaseService {
    ...
    Handle_GetItemsInSafetyForCharacter(args, session) {
        const wraps = assetSafetyState.listWrapsForOwner("char", session && (session.characterID || session.charid || session.userid));
    ...
    Handle_GetItemsInSafetyForCorp(args, session) {
        const wraps = assetSafetyState.listWrapsForOwner("corp", session && (session.corporationID || session.corpid));
        log.debug(`[StructureAssetSafety] GetItemsInSafetyForCorp count=${wraps.length}`);
        return buildCachedMethodCallResult(buildList(wraps.map((wrap) => buildWrapPayload(wrap))), {
    ...
    Handle_GetWrapNames(args) {
        ...
        return buildDict(wrapIDs.map((wrapID) => [
    ...
    Handle_GetStructuresICanDeliverTo(args, session) {
        ...
        const activeWraps = [
            ...assetSafetyState.listWrapsForOwner("char", session && (session.characterID || session.charid || session.userid), { refresh: false }),
            ...assetSafetyState.listWrapsForOwner("corp", session && (session.corporationID || session.corpid), { refresh: false }),
        ].filter((wrap) => Number(wrap.solarSystemID) === Number(solarSystemID));
        log.debug(`[StructureAssetSafety] GetStructuresICanDeliverTo solarSystem=${String(solarSystemID)}`);
        return [
            activeWraps.length > 0
                ? buildList(targets.structures.map((structure) => buildKeyVal([
    ...
    Handle_MovePersonalAssetsToSafety(args, session) {
        ...
        log.debug(`[StructureAssetSafety] MovePersonalAssetsToSafety solarSystem=${String(solarSystemID)} structure=${String(structureID)}`);
    ...
    Handle_MoveCorpAssetsToSafety(args, session) {
        const solarSystemID = Array.isArray(args) && args.length > 0 ? args[0] : null;
        const structureID = Array.isArray(args) && args.length > 1 ? args[1] : null;
        log.debug(`[StructureAssetSafety] MoveCorpAssetsToSafety solarSystem=${String(solarSystemID)} structure=${String(structureID)}`);
    ...
    Handle_MoveSafetyWrapToStructure(args, session, kwargs) {
        ...
        log.debug(`[StructureAssetSafety] MoveSafetyWrapToStructure wrap=${String(assetWrapID)} solarSystem=${String(solarSystemID)} destination=${String(destinationID)}`);
    ...
    Handle_MoveEjectTimeGM(args) {
        ...
        log.debug(`[StructureAssetSafety] MoveEjectTimeGM wrap=${String(assetWrapID)} days=${String(days)}`);
        assetSafetyState.shiftWrapEjectTimeGM(assetWrapID, days);
        return null;
    }
}
module.exports = StructureAssetSafetyService;
//# sourceMappingURL=structureAssetSafetyService.js.map