...
const { createSpaceItemForCharacter, } = require(path.join(__dirname, "../inventory/itemStore"));
const { getStructureSpaceDirection, } = require(path.join(__dirname, "./structureSpaceInterop"));
...
function isStructureWreckSourceCandidate(row) {
    ...
    return (groupName.includes("structure") ||
...
function buildStructureWreckNameCandidates(row) {
    ...
    if (groupID === 1924 || groupName.includes("stronghold")) {
        const factionName = stripPirateFactionQualifier(strippedName.replace(/\bStronghold\b/gi, "").trim());
...
function resolveStructureWreckType(structureOrTypeID) {
    const structureTypeID = typeof structureOrTypeID === "object" && structureOrTypeID !== null
...
function createStructureWreck(structure, ownerCharacterID, options = {}) {
    const { getSpaceDebrisLifetimeMs } = require(path.join(__dirname, "../inventory/spaceDebrisState"));
    ...
    const createResult = createSpaceItemForCharacter(numericOwnerCharacterID, solarSystemID, wreckType, {
...
//# sourceMappingURL=structureWreckState.js.map