...
function normalizeLicenseRecord(licenseID, value: Record<string, any> = {}) {
  return {
...
function hasAnyOnlineServiceModule(structure) {
  const serviceStates =
    structure && structure.serviceStates && typeof structure.serviceStates === "object"
      ...
      : {};
  return Object.values<any>(serviceStates).some(
    (stateID) =>
...
function buildIndexes() {
  ...
  for (const [licenseID, rawLicense] of Object.entries<any>(root.licensesByID || {})) {
...
function removeLicenseRecord(licenseID) {
  ...
  if (root.structureAssignments && typeof root.structureAssignments === "object") {
    for (const [structureID, assignedLicenseID] of Object.entries<any>(root.structureAssignments)) {
...
function createStructurePaintPurchasedNotifications(
  ...
  for (const targetCharacterID of [...new Set<any>(getCharacterIDsInCorporation(numericCorporationID))]) {
...
function normalizeIssueOptions(options: Record<string, any> = {}) {
  return {
...
function resolvePerStructureIssuePrice(structure, durationSeconds, options: Record<string, any> = {}) {
...
function issueLicenses(characterID, paintwork, durationSeconds, structureIDs: any[] = [], options: Record<string, any> = {}) {
  const normalizedOptions = normalizeIssueOptions(options);
  ...
  if (normalizedOptions.adminRequest && !hasAdminPrivileges) {
    return { success: false as const, errorMsg: "INSUFFICIENT_ROLES" };
  }
  ...
  if (
    ...
    return { success: false as const, errorMsg: "INVALID_DATA" };
  }
  ...
  const ownedStructureIDs = new Set<any>(
  ...
  nextRoot.structureAssignments = Object.fromEntries(
    Object.values<any>(nextRoot.licensesByID || {}).map((license) => [
  ...
  for (const entry of issues) {
    ...
    const existing = Object.values<any>(nextRoot.licensesByID || {}).find(
...
function revokeLicense(characterID, licenseID, options: Record<string, any> = {}) {
  const normalizedOptions = normalizeIssueOptions(options);
  const license = getLicense(licenseID);
  if (!license) {
    return { success: false as const, errorMsg: "NOT_FOUND" };
  }
  const hasAdminPrivileges =
    normalizedOptions.adminRequest && hasStructurePaintworkAdminPrivileges(characterID);
  if (normalizedOptions.adminRequest && !hasAdminPrivileges) {
