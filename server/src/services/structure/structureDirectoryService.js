...
const { extractList, buildList, normalizeText, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
const { TYPE_PHAROLUX_CYNO_BEACON, TYPE_ANSIBLEX_JUMP_BRIDGE, } = require(path.join(__dirname, "../sovereignty/sovUpgradeSupport"));
const { isSolarSystemCynoJammed, } = require(path.join(__dirname, "../sovereignty/sovSuppressionState"));
const { characterHasStructureService, buildIDList, buildBasicStructureInfoPayload, buildStructureInfoDict, buildStructureInfoPayload, buildStructureMapList, } = require(path.join(__dirname, "./structurePayloads"));
const { STRUCTURE_SERVICE_ID, STRUCTURE_SERVICE_STATE, } = require(path.join(__dirname, "./structureConstants"));
const { CORP_ROLE_BRAND_MANAGER, } = require(path.join(__dirname, "../corporation/corporationRuntimeState"));
const { findServiceProximityConflict, } = require(path.join(__dirname, "./structureServiceProximity"));
const { CORP_ROLE_STATION_MANAGER, buildCrpAccessDeniedInsufficientRolesValues, } = require(path.join(__dirname, "./structureServiceAuthority"));
...
function buildWarHQPayload(structure) {
    return buildKeyVal([
        ...
        [
            "itemName",
            String((structure && (structure.itemName || structure.name)) ||
...
function getSessionCorporationID(session) {
    return normalizePositiveInt(session && (session.corpid || session.corporationID || session.corpID), 0);
}
function canManageStructure(session, structure) {
    const sessionCorpID = getSessionCorporationID(session);
    const structureOwnerCorpID = normalizePositiveInt(structure && (structure.ownerCorpID || structure.ownerID), 0);
    if (!sessionCorpID || sessionCorpID !== structureOwnerCorpID) {
        return false;
    }
    const roleMask = normalizeRoleMask(session && (session.corprole ?? session.corpRole ?? session.rolesAtAll));
...
function isJumpBridgeServiceOnline(structure) {
    return Number(structure &&
        structure.serviceStates &&
        structure.serviceStates[String(STRUCTURE_SERVICE_ID.JUMP_BRIDGE)]) === STRUCTURE_SERVICE_STATE.ONLINE;
}
function hasJumpBridgeServiceProximityConflict(structure, structures) {
    return Boolean(findServiceProximityConflict(structure, STRUCTURE_SERVICE_ID.JUMP_BRIDGE, structures));
}
function isJumpBridgeServiceAvailable(structure, structures) {
    return (isJumpBridgeServiceOnline(structure) &&
        !hasJumpBridgeServiceProximityConflict(structure, structures));
}
function isHighSecurityStructure(structure) {
    const system = worldData.getSolarSystemByID(normalizePositiveInt(structure && structure.solarSystemID, 0));
...
function structureBelongsToOwnerID(structure, ownerID) {
    ...
    return (normalizePositiveInt(structure.ownerCorpID || structure.ownerID, 0) ===
        normalizedOwnerID ||
...
function canRequestWarHQsForOwner(session, ownerID) {
    ...
    return (normalizedOwnerID === getSessionCorporationID(session) ||
        normalizedOwnerID ===
...
function buildJumpBridgeMapStructurePayload(structure) {
    ...
    const name = String((structure && (structure.itemName || structure.name)) ||
...
function findReciprocalJumpBridge(sourceStructure, structures) {
    ...
    return structures.find((candidate) => (isAnsiblexStructure(candidate) &&
        ...
        getJumpBridgeDestinationSolarSystemID(candidate) === sourceSolarSystemID)) || null;
}
function buildJumpBridgeAccessPayload(session) {
    ...
    const sortedStructures = [...ansiblexStructures].sort((left, right) => normalizePositiveInt(left && left.structureID, 0) -
...
class StructureDirectoryService extends BaseService {
    ...
    callMethod(method, args, session, kwargs) {
        ...
        if (typeof method === "string" && method.startsWith("Get")) {
            log.debug(`[StructureDirectoryService] Fallback empty result for ${method}`);
    ...
    Handle_GetMyCorporationStructures(args, session, kwargs) {
        const corpID = normalizePositiveInt(session && (session.corporationID || session.corpid), 0);
    ...
    Handle_GetMyDockableStructures(args, session, kwargs) {
        const solarSystemID = normalizePositiveInt(Array.isArray(args) && args.length > 0 ? args[0] : getCurrentSolarSystemID(session), getCurrentSolarSystemID(session));
        const structures = structureState.listDockableStructuresForCharacter(session, {
            solarSystemID,
        });
    ...
    Handle_GetStructureDescription(args, session, kwargs) {
        const structureID = normalizePositiveInt(Array.isArray(args) && args.length > 0 ? args[0] : null, 0);
        const structure = structureState.getStructureByID(structureID);
    ...
    Handle_SetStructureDescription(args, session, kwargs) {
        const structureID = normalizePositiveInt(Array.isArray(args) && args.length > 0 ? args[0] : null, 0);
        const description = normalizeStructureDescription(Array.isArray(args) && args.length > 1 ? args[1] : "");
        ...
        if (!updateResult.success) {
            log.warn(`[StructureDirectoryService] SetStructureDescription failed structure=${structureID} error=${updateResult.errorMsg || "UNKNOWN"}`);
        }
        else {
    ...
    Handle_GetMyAccessibleOnlineCynoBeaconStructures(args, session, kwargs) {
        const structures = structureState.listStructures()
            .filter((structure) => (isPharoluxStructure(structure) &&
            Number(structure.serviceStates &&
                structure.serviceStates[String(STRUCTURE_SERVICE_ID.CYNO_BEACON)]) === STRUCTURE_SERVICE_STATE.ONLINE &&
    ...
    Handle_GetValidWarHQs(args, session, kwargs) {
        ...
        const structures = structureState.listStructures()
            .filter((structure) => (structureBelongsToOwnerID(structure, ownerID) &&
    ...
    Handle_GetNearbyJumpBridges(args, session, kwargs) {
        ...
        const structures = structureState.listStructures({
            includeDestroyed: false,
            refresh: false,
        }).filter((structure) => (Number(structure && structure.typeID) === TYPE_ANSIBLEX_JUMP_BRIDGE &&
            !structure.destroyedAt));
        return {
            type: "list",
            items: structures.map((structure) => {
                const devFlags = structure && structure.devFlags && typeof structure.devFlags === "object"
                    ? structure.devFlags
                    : {};
                const destinationSolarsystemID = normalizePositiveInt(devFlags.destinationSolarsystemID ||
                    devFlags.sovereigntyJumpBridgeDestinationSolarsystemID, 0);
                return buildKeyVal([
...
//# sourceMappingURL=structureDirectoryService.js.map