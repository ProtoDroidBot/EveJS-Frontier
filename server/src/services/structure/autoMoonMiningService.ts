...
function sortNumericDictEntries(entries: any[] = []) {
...
function sumItemsByType(items: any[] = [], predicate = (..._extra: any[]) => true) {
...
function getOutputBayUsedVolume(items: any[] = []) {
...
function extractStructureID(args: any[] = [], session: Record<string, any> = {}) {
...
class AutoMoonMiningService extends BaseService {
  declare _containerItemProvider: any;
  declare _resourceProfileProvider: any;
  declare _structureProvider: any;
  ...
  Handle_RequestMiningDetails(args: any[] = [], session) {
  ...
  Handle_RequestMiningCycleOutput(args: any[] = [], session) {
    const structureID = extractStructureID(args, session);
    if (!structureID) {
      return buildDict([]);
    }
