...
const { buildFiletimeLong, buildMarshalReal, buildObjectEx2 } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const structureState = require(path.join(__dirname, "./structureState"));
const { STRUCTURE_STATE, STRUCTURE_STATE_NAME_BY_ID, STRUCTURE_UPKEEP_STATE, STRUCTURE_UPKEEP_NAME_BY_ID, } = require(path.join(__dirname, "./structureConstants"));
...
const STRUCTURE_HANGAR_VIEW_STATE_CLASS = "structures.structureHangarViewState.StructureHangarViewState";
...
function buildStructureHangarDamageState(structure = null) {
    ...
    const defaultDamageState = STRUCTURE_DAMAGE_STATE_BY_OPERATING_STATE[operatingState] || [1, 1, 1];
    const conditionState = structure && structure.conditionState && typeof structure.conditionState === "object"
        ? structure.conditionState
        : {};
    return [
        clamp01(conditionState.shieldCharge, defaultDamageState[0]),
        clamp01(conditionState.armorDamage === undefined
            ? defaultDamageState[1]
            : 1 - Number(conditionState.armorDamage), defaultDamageState[1]),
        clamp01(conditionState.damage === undefined
            ? defaultDamageState[2]
            : 1 - Number(conditionState.damage), defaultDamageState[2]),
    ];
}
...
function buildStructureHangarTimerState(structure = null) {
    ...
    const endAtMs = Number(structure && structure.stateEndsAt);
    const pauseAtMs = Number(structure && structure.timerPausedAt);
    const currentAtMs = Number.isFinite(pauseAtMs) && pauseAtMs > 0 ? pauseAtMs : Date.now();
...
function buildStructureHangarViewState(structure = null) {
    ...
    if (timerState.timerStartAt !== null ||
        timerState.timerEndAt !== null ||
        timerState.timerPauseAt !== null) {
        stateEntries.push(["_timerStartAt", timerState.timerStartAt], ["_timerEndAt", timerState.timerEndAt], ["_timerPauseAt", timerState.timerPauseAt]);
    }
...
//# sourceMappingURL=structureHangarViewState.js.map