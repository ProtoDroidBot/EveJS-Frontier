...
const { STRUCTURE_GROUP_ID, STRUCTURE_STATE, } = require(path.join(__dirname, "./structureConstants"));
...
function buildStructureDeathPositions(anchorEntity, count, radiusMeters) {
    ...
    const resolvedRadius = Math.max(MIN_STRUCTURE_DEATH_TEST_RADIUS_METERS, toFiniteNumber(radiusMeters, DEFAULT_STRUCTURE_DEATH_TEST_RADIUS_METERS));
    const positions = [];
    const ringCount = Math.max(1, Math.ceil(resolvedCount / 6));
    for (let index = 0; index < resolvedCount; index += 1) {
        const ringIndex = Math.floor(index / 6);
        const spokeIndex = index % 6;
        const ringRadius = Math.min(resolvedRadius, MIN_STRUCTURE_DEATH_TEST_RADIUS_METERS + (ringIndex *
    ...
    if (/^\d+$/.test(tokens[0] || "")) {
        return {
            ...
            count: tokens.length >= 2
                ...
                : null,
            delaySeconds: tokens.length >= 3 && /^\d+(?:\.\d+)?$/.test(tokens[2])
...
function resolveStructureDeathTestType(typeToken) {
    ...
    if (STRUCTURE_DEATH_TEST_ALIASES[alias]) {
        return buildStructureTypeResolution(resolveStructureTypeByID(STRUCTURE_DEATH_TEST_ALIASES[alias]), "alias");
    }
    if (/^\d+$/.test(trimmed)) {
    ...
    return buildStructureTypeResolution(resolveStructureTypeByID(lookup.match.typeID), "itemName");
}
    ...
    for (const [pendingID, pending] of [...pendingStructureDeathTests.entries()]) {
        ...
        for (const structureID of pending.structureIDs) {
            ...
            if (!destroyResult.success) {
                log.warn(`[StructureDeathTest] Failed to destroy test structure=${structureID}: ${destroyResult.errorMsg}`);
                continue;
            }
...
function queuePendingStructureDeathTest({ systemID, typeRecord, wreckType, structureIDs, spawnedCount, delayMs, session, }) {
...
function spawnStructureDeathTestField(session, options = {}) {
    ...
    const count = Math.max(1, Math.min(MAX_STRUCTURE_DEATH_TEST_COUNT, toPositiveInt(options.count, DEFAULT_STRUCTURE_DEATH_TEST_COUNT)));
    const radiusMeters = Math.max(MIN_STRUCTURE_DEATH_TEST_RADIUS_METERS, toFiniteNumber(options.radiusMeters, DEFAULT_STRUCTURE_DEATH_TEST_RADIUS_METERS));
    const delayMs = Math.max(0, Math.min(MAX_STRUCTURE_DEATH_TEST_DELAY_MS, toFiniteNumber(options.delayMs, DEFAULT_STRUCTURE_DEATH_TEST_DELAY_MS)));
    ...
    const ownerCorpID = toPositiveInt(session.corporationID || session.corpid, 1000009);
    for (let index = 0; index < positions.length; index += 1) {
        const createResult = structureState.seedStructureForSession(session, String(typeRecord.typeID), {
        ...
        if (!createResult.success || !createResult.data) {
            log.warn(`[StructureDeathTest] Failed to seed ${typeRecord.typeID}: ${createResult.errorMsg}`);
            continue;
        }
        const stateResult = structureState.setStructureState(createResult.data.structureID, stateID, {
            clearTimer: true,
        });
...
//# sourceMappingURL=structureDeathTestRuntime.js.map