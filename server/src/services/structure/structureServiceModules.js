...
const { listContainerItems, consumeInventoryItemQuantity, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
const { isFuelBayCompatibleItem, STRUCTURE_FUEL_BAY_FLAG, } = require(path.join(__dirname, "../inventory/fuelBayInventory"));
const { applyModifierGroups, getAttributeIDByNames, getTypeAttributeMap, getTypeAttributeValue, getTypeEffectRecords, isEffectivelyOnlineModule, isShipFittingFlag, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const structureState = require(path.join(__dirname, "./structureState"));
const { STRUCTURE_SERVICE_ID, STRUCTURE_SERVICE_STATE, STRUCTURE_UPKEEP_STATE, } = require(path.join(__dirname, "./structureConstants"));
const { syncIndustryJobsForServiceStateTransition, } = require(path.join(__dirname, "./structureIndustryJobs"));
const { MANAGED_SERVICE_IDS, SERVICE_IDS_BY_MODULE_TYPE_ID, getStructureServiceIDsForModuleType, isStructureServiceModuleType, } = require(path.join(__dirname, "./structureServiceAuthority"));
const { validateServiceModuleOnlineRequirements, } = require(path.join(__dirname, "./structureServiceOnlineRequirements"));
const { findServiceProximityConflict, } = require(path.join(__dirname, "./structureServiceProximity"));
...
const ATTRIBUTE_SERVICE_MODULE_FUEL_AMOUNT = getAttributeIDByNames("serviceModuleFuelAmount", "Service Module Cycle Fuel Need") || 2109;
const ATTRIBUTE_SERVICE_MODULE_ONLINE_FUEL_AMOUNT = getAttributeIDByNames("serviceModuleFuelOnlineAmount", "Service Module Online Fuel Need") || 2110;
...
function isStructureDamagedForServiceOnline(structureRecord) {
    const conditionState = structureRecord && structureRecord.conditionState && typeof structureRecord.conditionState === "object"
    ...
    return ((Number.isFinite(shieldCharge) && shieldCharge < STRUCTURE_ONLINE_MAX_SHIELD_DAMAGE_THRESHOLD) ||
...
function getStructureServiceModuleProximityConflict(structure, moduleItem) {
    ...
    const candidates = structureState.listStructuresForSystem(structure.solarSystemID, { includeDestroyed: true, refresh: false });
...
function validateStructureServiceModuleOnlineRequirements(structure, moduleItem) {
    return validateServiceModuleOnlineRequirements(structure, moduleItem && moduleItem.typeID);
}
...
function listFittedStructureServiceModules(structureID) {
    ...
    return listContainerItems(null, numericStructureID, null)
        ...
        .sort((left, right) => (toInt(left.flagID, 0) - toInt(right.flagID, 0) ||
...
function getStructureServiceModuleCycleFuelNeed(typeID) {
    return Math.max(0, toInt(getTypeAttributeValue(typeID, "serviceModuleFuelAmount", "Service Module Cycle Fuel Need"), 0));
}
function getStructureServiceModuleFuelGroupID(typeID) {
    return Math.max(0, toInt(getTypeAttributeValue(typeID, "serviceModuleFuelConsumptionGroup", "Service Module Fuel Need"), 0));
}
function getStructureServiceModuleOnlineFuelNeed(typeID) {
    const explicitOnlineFuel = toInt(getTypeAttributeValue(typeID, "serviceModuleFuelOnlineAmount", "Service Module Online Fuel Need"), 0);
...
function appendStructureLocationFuelModifierEntries(destination, sourceItem, moduleItem, options = {}) {
    ...
    for (const effectRecord of sourceEffects) {
        for (const modifierInfo of effectRecord.modifierInfo || []) {
            ...
            const value = toFiniteNumber(sourceAttributes[toInt(modifierInfo.modifyingAttributeID, 0)], NaN);
...
function buildStructureServiceModuleEffectiveAttributeMap(structureID, moduleItem) {
    ...
    if (structure && toPositiveInt(structure.typeID, 0) > 0) {
        appendStructureLocationFuelModifierEntries(modifierEntries, { typeID: toPositiveInt(structure.typeID, 0) }, moduleItem, { sourceKind: "structureHull" });
    }
    for (const sourceItem of listContainerItems(null, structureID, null)) {
        ...
        appendStructureLocationFuelModifierEntries(modifierEntries, sourceItem, moduleItem, { sourceKind: "fittedModule" });
    }
...
function getEffectiveStructureServiceModuleOnlineFuelNeed(structureID, moduleItem) {
    const attributes = buildStructureServiceModuleEffectiveAttributeMap(structureID, moduleItem);
    const explicitOnlineFuel = toFiniteNumber(attributes[ATTRIBUTE_SERVICE_MODULE_ONLINE_FUEL_AMOUNT], 0);
    if (explicitOnlineFuel > 0) {
...
function resolveWholeServiceFuelCharge(exactQuantity, existingRemainder = 0) {
    const total = Math.max(0, toFiniteNumber(exactQuantity, 0) + toFiniteNumber(existingRemainder, 0));
...
function consumeStructureServiceFuelQuantity(structureID, moduleItem, requiredQuantity, options = {}) {
    ...
    const availableQuantity = fuelItems.reduce((total, item) => total + getStackQuantity(item), 0);
...
function consumeStructureServiceModuleOnlineFuel(structureID, moduleItem, options = {}) {
    const structure = structureState.getStructureByID(structureID, { refresh: false });
    const requirementResult = validateStructureServiceModuleOnlineRequirements(structure, moduleItem);
    ...
    const requiredQuantity = resolveWholeOnlineFuelCharge(getEffectiveStructureServiceModuleOnlineFuelNeed(structureID, moduleItem));
    const fuelResult = consumeStructureServiceFuelQuantity(structureID, moduleItem, requiredQuantity, options);
...
function tryAutoOnlineFittedStructureServiceModule(structureID, moduleItem, options = {}) {
    ...
    const fuelResult = consumeStructureServiceModuleOnlineFuel(numericStructureID, moduleItem, options);
    if (!fuelResult || fuelResult.success !== true) {
        return {
            ...(fuelResult || {}),
            success: false,
    ...
    const moduleStampChanges = fuelResult.moduleUpdateResult && fuelResult.moduleUpdateResult.success
        ? [{
...
function ensureStructureServiceFuelCycleStamp(moduleItem, nowMs) {
    const nextCycleAt = toInt(moduleItem && moduleItem.moduleState && moduleItem.moduleState.serviceFuelNextCycleAt, 0);
    if (nextCycleAt > 0) {
        return {
            success: true,
            changed: false,
    ...
    const updateResult = updateInventoryItem(moduleItem.itemID, (currentItem) => ({
        ...
        moduleState: {
            ...(currentItem.moduleState || {}),
            serviceFuelOnlineAt: toInt(currentItem.moduleState && currentItem.moduleState.serviceFuelOnlineAt, nowMs),
            serviceFuelNextCycleAt: nowMs + SERVICE_FUEL_CYCLE_MS,
            serviceFuelRemainder: toFiniteNumber(currentItem.moduleState && currentItem.moduleState.serviceFuelRemainder, 0),
        },
    }));
    return {
...
function resolveServiceModuleLastFuelConsumptionAt(moduleItem, nextCycleAt, nowMs) {
    const moduleState = moduleItem && moduleItem.moduleState && typeof moduleItem.moduleState === "object"
...
function consumeStructureServiceModuleCycleFuel(structureID, moduleItem, options = {}) {
    ...
    const requirementResult = validateStructureServiceModuleOnlineRequirements(structure, moduleItem);
    if (!requirementResult.success) {
        return offlineServiceModuleForMissingRequirement(moduleItem, requirementResult.errorMsg);
    }
    const proximityConflict = getStructureServiceModuleProximityConflict(structure, moduleItem);
    if (proximityConflict) {
    ...
    const nextCycleAt = toInt(stampedModule && stampedModule.moduleState && stampedModule.moduleState.serviceFuelNextCycleAt, 0);
    ...
    const cycleFuelNeed = getEffectiveStructureServiceModuleCycleFuelNeed(structureID, stampedModule);
    if (cycleFuelNeed <= 0) {
        return {
            success: true,
            moduleItem: stampedModule,
    ...
    const fuelCharge = resolveWholeServiceFuelCharge(cycleFuelNeed * cycleCount, stampedModule && stampedModule.moduleState && stampedModule.moduleState.serviceFuelRemainder);
    const fuelResult = consumeStructureServiceFuelQuantity(structureID, stampedModule, fuelCharge.wholeQuantity, options);
    if (!fuelResult || fuelResult.success !== true) {
        const abandonAnchorAt = resolveServiceModuleLastFuelConsumptionAt(stampedModule, nextCycleAt, nowMs);
...
function syncStructureServiceModuleState(structureID, options = {}) {
    ...
    for (const moduleItem of rawOnlineModules) {
        ...
        const fuelCycleResult = consumeStructureServiceModuleCycleFuel(numericStructureID, moduleItem, options);
        ...
        if (fuelCycleResult && fuelCycleResult.success === true) {
            ...
            for (const entry of (fuelCycleResult && fuelCycleResult.fuelAlertTypesAndQty) || []) {
                ...
                if (quantity > 0 && typeID > 0) {
                    fuelAlertTypesByTypeID.set(typeID, Math.max(quantity, fuelAlertTypesByTypeID.get(typeID) || 0));
                }
            }
            const abandonAnchorAt = toPositiveInt(fuelCycleResult && fuelCycleResult.abandonAnchorAt, 0);
    ...
    const updateResult = structureState.updateStructureRecord(numericStructureID, (current) => {
        ...
        const upkeepState = resolveStructureUpkeepAfterServiceSync(current.upkeepState, hasOnlineServiceModule);
        ...
        if (hasOnlineServiceModule) {
            ...
            abandonAt = structureState.resolveStructureAbandonAtFromFuelAnchor(Math.max(...fuelStarvationAbandonAnchors));
            abandonmentWarningAbandonAt = null;
            abandonmentWarningRecipients = [];
        }
        return {
    ...
    if (updateResult && updateResult.success === true && fuelAlertTypesByTypeID.size > 0) {
        structureState.createStructureFuelAlertNotifications(updateResult.data, [...fuelAlertTypesByTypeID.entries()]
            .sort(([leftTypeID], [rightTypeID]) => leftTypeID - rightTypeID)
            .map(([typeID, quantity]) => [quantity, typeID]));
    }
    return {
        ...
        industryJobSync: updateResult && updateResult.success
            ? syncIndustryJobsForServiceStateTransition(numericStructureID, beforeStructure && beforeStructure.serviceStates, updateResult.data && updateResult.data.serviceStates)
            : null,
    };
}
module.exports = {
...
//# sourceMappingURL=structureServiceModules.js.map