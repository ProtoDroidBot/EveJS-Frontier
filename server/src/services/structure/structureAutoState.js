...
const { STRUCTURE_SERVICE_ID, STRUCTURE_SERVICE_STATE, STRUCTURE_STATE, STRUCTURE_STATE_NAME_BY_ID, STRUCTURE_UPKEEP_STATE, STRUCTURE_UPKEEP_NAME_BY_ID, } = require(path.join(__dirname, "./structureConstants"));
...
function getSeedPosition(session) {
    const entity = spaceRuntime.getEntity(session, session && session._space ? session._space.shipID : null);
...
function syncStructureRuntime(structureOrSystemID) {
    const systemID = typeof structureOrSystemID === "object" && structureOrSystemID !== null
...
function stopJobInternal(job, reason, status = "stopped") {
    ...
    structureLog.logAutomationEvent(job, `${status.toUpperCase()} ${reason}`, status === "failed" ? "ERR" : "INF");
    return {
        success: true,
        data: {
            ...job,
...
function applyStructureAction(job, actionLabel, fn) {
    ...
    try {
        ...
        structureLog.logAutomationEvent(job, `${actionLabel} threw ${error.stack || error.message}`, "ERR");
    ...
    structureLog.logAutomationEvent(job, `${actionLabel} ok ${formatStructureSummary(nextStructure || result.data && result.data.structure || result.data)}`);
    return {
        success: true,
...
function executeOnlineStep(job, structure) {
    ...
    const dockingState = normalizeInt(structure.serviceStates && structure.serviceStates[String(STRUCTURE_SERVICE_ID.DOCKING)], STRUCTURE_SERVICE_STATE.OFFLINE);
    ...
    if (normalizeInt(structure.state, 0) === STRUCTURE_STATE.UNANCHORED) {
        return applyStructureAction(job, `/upwell anchor ${structure.structureID}`, () => (structureState.startAnchoring(structure.structureID)));
    }
    if (normalizeInt(structure.state, 0) === STRUCTURE_STATE.ANCHOR_VULNERABLE) {
        return applyStructureAction(job, `/upwell ff ${structure.structureID} ${ONLINE_FAST_FORWARD_SHORT_SECONDS}`, () => (structureState.fastForwardStructure(structure.structureID, ONLINE_FAST_FORWARD_SHORT_SECONDS)));
    }
    if (normalizeInt(structure.state, 0) === STRUCTURE_STATE.ANCHORING) {
        if (structure.hasQuantumCore !== true && hasActiveTimer(structure)) {
            return applyStructureAction(job, `/upwell ff ${structure.structureID} ${ONLINE_FAST_FORWARD_LONG_SECONDS}`, () => (structureState.fastForwardStructure(structure.structureID, ONLINE_FAST_FORWARD_LONG_SECONDS)));
        }
        if (structure.hasQuantumCore !== true) {
            return applyStructureAction(job, `/upwell core ${structure.structureID} on`, () => (structureState.setStructureQuantumCoreInstalled(structure.structureID, true)));
        }
    }
    if (normalizeInt(structure.state, 0) === STRUCTURE_STATE.ONLINING_VULNERABLE) {
        ...
        if (dockingState !== STRUCTURE_SERVICE_STATE.ONLINE) {
            const serviceResult = applyStructureAction(job, `/upwell service ${structure.structureID} docking online`, () => structureState.setStructureServiceState(structure.structureID, STRUCTURE_SERVICE_ID.DOCKING, STRUCTURE_SERVICE_STATE.ONLINE));
    ...
    if (normalizeInt(structure.state, 0) === STRUCTURE_STATE.SHIELD_VULNERABLE) {
        return applyStructureAction(job, `/upwell service ${structure.structureID} docking online`, () => (structureState.setStructureServiceState(structure.structureID, STRUCTURE_SERVICE_ID.DOCKING, STRUCTURE_SERVICE_STATE.ONLINE)));
    }
    return stopJobInternal(job, `online automation does not know how to advance state=${STRUCTURE_STATE_NAME_BY_ID[normalizeInt(structure.state, 0)] || structure.state}`, "failed");
}
function executeDestroyStep(job, structure) {
    ...
    if (stateID === STRUCTURE_STATE.UNANCHORED ||
    ...
    if (job.prepared !== true) {
        if (normalizeInt(structure.upkeepState, 0) !== STRUCTURE_UPKEEP_STATE.FULL_POWER) {
            return applyStructureAction(job, `/upwell upkeep ${structure.structureID} full_power`, () => (structureState.setStructureUpkeepState(structure.structureID, STRUCTURE_UPKEEP_STATE.FULL_POWER)));
        }
        job.prepared = true;
        return applyStructureAction(job, `/upwell repair ${structure.structureID}`, () => (structureState.repairStructure(structure.structureID)));
    }
    if (stateID === STRUCTURE_STATE.SHIELD_VULNERABLE) {
        return applyStructureAction(job, `/upwell damage ${structure.structureID} shield 1`, () => (structureState.applyAdminStructureDamage(structure.structureID, "shield", 1, { session: job.session })));
    }
    if (stateID === STRUCTURE_STATE.ARMOR_REINFORCE || stateID === STRUCTURE_STATE.HULL_REINFORCE) {
        return applyStructureAction(job, `/upwell ff ${structure.structureID} ${REINFORCE_FAST_FORWARD_SECONDS}`, () => (structureState.fastForwardStructure(structure.structureID, REINFORCE_FAST_FORWARD_SECONDS)));
    }
    if (stateID === STRUCTURE_STATE.ARMOR_VULNERABLE) {
        return applyStructureAction(job, `/upwell damage ${structure.structureID} armor 1`, () => (structureState.applyAdminStructureDamage(structure.structureID, "armor", 1, { session: job.session })));
    }
    if (stateID === STRUCTURE_STATE.HULL_VULNERABLE) {
        return applyStructureAction(job, `/upwell damage ${structure.structureID} hull 1`, () => (structureState.applyAdminStructureDamage(structure.structureID, "hull", 1, { session: job.session })));
    }
    return stopJobInternal(job, `destroy automation does not know how to advance state=${STRUCTURE_STATE_NAME_BY_ID[stateID] || structure.state}`, "failed");
}
function runJobNow(jobID) {
    ...
    if (typeof job.executeStep === "function") {
        ...
        try {
            ...
            structureLog.logAutomationEvent(job, `custom step threw ${error.stack || error.message}`, "ERR");
        ...
        if (result.completed === true) {
            return stopJobInternal(job, result.completionReason || "custom automation completed", "completed");
        }
        return {
            success: true,
...
//# sourceMappingURL=structureAutoState.js.map