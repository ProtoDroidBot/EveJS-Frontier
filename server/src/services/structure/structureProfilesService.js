...
const { buildDict, buildKeyVal, buildList, extractList, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const sessionRegistry = require(path.join(__dirname, "../chat/sessionRegistry"));
const { assignProfileToStructuresForCorporation, createProfileForCorporation, deleteProfileForCorporation, duplicateProfileForCorporation, ensureCorporationDefaultProfile, getCorporationIDForSession, getProfileForCorporation, listProfilesForCorporation, saveProfileSettingsForCorporation, setDefaultProfileForCorporation, updateProfileForCorporation, } = require(path.join(__dirname, "./structureProfilesState"));
...
function buildProfileSettingsPayload(profile) {
    const settingsBySettingID = profile && profile.settingsBySettingID && typeof profile.settingsBySettingID === "object"
        ...
        : {};
    return buildDict(Object.entries(settingsBySettingID)
        ...
        return [
            settingID,
            buildList((Array.isArray(groups) ? groups : []).map((group) => buildKeyVal([
...
function notifyCorporationSessions(corporationID, notifyType, payload = []) {
    ...
    for (const session of sessionRegistry.getSessions()) {
        if (getCorporationIDForSession(session) !== numericCorporationID ||
...
function notifyProfileSettingsChanged(corporationID, profileIDs) {
    ...
    return notifyCorporationSessions(corporationID, "OnProfileSettingsChange", [buildList(normalizedProfileIDs)]);
}
function notifyCorporationStructuresUpdated(corporationID) {
    return notifyCorporationSessions(corporationID, "OnCorporationStructuresUpdated", []);
}
class StructureProfilesService extends BaseService {
    ...
    Handle_GetProfiles(args, session) {
        ...
        return buildList(listProfilesForCorporation(corporationID).map((profile) => buildProfilePayload(profile)));
    }
    Handle_CreateProfile(args, session) {
        const corporationID = getCorporationIDForSession(session);
        requireStructureProfileManager(session);
        const normalizedArgs = normalizeArgs(args);
        const createdProfile = createProfileForCorporation(corporationID, normalizedArgs[0], normalizedArgs[1]);
        if (createdProfile) {
    ...
    Handle_SaveProfileSettings(args, session) {
        ...
        requireStructureProfileManager(session);
        const normalizedArgs = normalizeArgs(args);
        const savedProfile = saveProfileSettingsForCorporation(corporationID, normalizedArgs[0], extractList(normalizedArgs[1]));
        if (savedProfile) {
    ...
    Handle_SetDefaultProfile(args, session) {
        ...
        if (updatedProfile) {
            notifyProfileSettingsChanged(corporationID, getCorporationProfileIDs(corporationID, [updatedProfile.profileID]));
        }
        return null;
    }
    Handle_ChangeProfiles(args, session) {
        ...
        requireStructureProfileManager(session);
        const normalizedArgs = normalizeArgs(args);
        const result = assignProfileToStructuresForCorporation(corporationID, normalizedArgs[1], extractList(normalizedArgs[0]));
    ...
    Handle_DeleteProfile(args, session) {
        ...
        if (didDelete) {
            notifyProfileSettingsChanged(corporationID, getCorporationProfileIDs(corporationID, [deletedProfileID]));
        }
        return null;
    }
    Handle_DuplicateProfile(args, session) {
        ...
        requireStructureProfileManager(session);
        const normalizedArgs = normalizeArgs(args);
        const duplicatedProfile = duplicateProfileForCorporation(corporationID, normalizedArgs[0]);
...
//# sourceMappingURL=structureProfilesService.js.map