...
function getStructureProximityCandidates(structure, options: Record<string, any> = {}) {
...
function getFlexServiceProximityConflict(structure, profile, options: Record<string, any> = {}) {
  if (!profile) {
    return null;
  }
...
function isFlexCynoBeaconBlockedBySystemJam(structure, profile, options: Record<string, any> = {}) {
  if (
    !profile ||
...
function findOnlineTenebrexCynoJammerConflict(structure, profile, options: Record<string, any> = {}) {
  if (
    !profile ||
    toPositiveInt(profile.serviceID, 0) !== STRUCTURE_SERVICE_ID.CYNO_JAMMER
  ) {
...
function findLowerOnlineTenebrexCynoJammerConflict(structure, profile, options: Record<string, any> = {}) {
  const structureID = toPositiveInt(structure && structure.structureID, 0);
...
function buildOnlineFlexFuelPatch(structure, profile, nowMs) {
  ...
  const onlineState: Record<string, any> = {
...
function prepareFlexServiceStateTransition(structure, serviceID, serviceState, options: Record<string, any> = {}) {
...
function buildFlexFuelState(profile, nextCycleAt, extra: Record<string, any> = {}) {
  return {
...
function applyFlexServiceFuelCycle(structure, nowMs = Date.now(), options: Record<string, any> = {}) {
  const profile = getFlexFuelProfile(structure && structure.typeID);
  if (!profile) {
    return {
      structure,
