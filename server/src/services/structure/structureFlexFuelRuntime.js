...
const { listContainerItems, consumeInventoryItemQuantity, } = require(path.join(__dirname, "../inventory/itemStore"));
const { resolveItemByTypeID } = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const { STRUCTURE_FUEL_BAY_FLAG } = require(path.join(__dirname, "../inventory/fuelBayInventory"));
const { STRUCTURE_SERVICE_ID, STRUCTURE_SERVICE_STATE, STRUCTURE_UPKEEP_STATE, } = require(path.join(__dirname, "./structureConstants"));
const { findServiceProximityConflict, getServiceProximityCandidatesForSystem, } = require(path.join(__dirname, "./structureServiceProximity"));
...
function getFlexFuelProfile(structureTypeID) {
    ...
    const serviceModuleTypeID = toPositiveInt(getDogmaAttribute(definition.typeID, ATTRIBUTE_BUILT_IN_SERVICE_MODULE, definition.fallbackServiceModuleTypeID), definition.fallbackServiceModuleTypeID);
    const hourlyAmount = Math.max(0, toInt(getDogmaAttribute(serviceModuleTypeID, ATTRIBUTE_SERVICE_MODULE_FUEL_AMOUNT, definition.fallbackHourlyAmount), definition.fallbackHourlyAmount));
    const onlineAmount = Math.max(0, toInt(getDogmaAttribute(serviceModuleTypeID, ATTRIBUTE_SERVICE_MODULE_ONLINE_FUEL_AMOUNT, definition.fallbackOnlineAmount), definition.fallbackOnlineAmount));
    const fuelGroupID = Math.max(0, toInt(getDogmaAttribute(serviceModuleTypeID, ATTRIBUTE_SERVICE_MODULE_FUEL_GROUP, GROUP_FUEL_BLOCK), GROUP_FUEL_BLOCK));
    const cynoJammerActivationDelayMs = definition.serviceID === STRUCTURE_SERVICE_ID.CYNO_JAMMER
        ? Math.max(0, toInt(getDogmaAttribute(serviceModuleTypeID, ATTRIBUTE_CYNO_JAMMER_ACTIVATION_DELAY, definition.fallbackCynoJammerActivationDelayMs), definition.fallbackCynoJammerActivationDelayMs))
        : null;
    return {
        ...definition,
        serviceModuleTypeID,
...
function isFlexServiceOnline(structure, profile) {
    return toInt(structure &&
        structure.serviceStates &&
        structure.serviceStates[String(profile.serviceID)], STRUCTURE_SERVICE_STATE.OFFLINE) === STRUCTURE_SERVICE_STATE.ONLINE;
}
function isStructureDamagedForFlexServiceOnline(structureRecord) {
    const conditionState = structureRecord &&
    ...
    return ((Number.isFinite(shieldCharge) && shieldCharge < 0.995) ||
...
function getStructureProximityCandidates(structure, options = {}) {
    const explicitCandidates = Array.isArray(options.structureProximityCandidates)
...
function getFlexServiceProximityConflict(structure, profile, options = {}) {
    ...
    return findServiceProximityConflict(structure, profile.serviceID, getStructureProximityCandidates(structure, options));
}
...
function isOnlineTenebrexCynoJammer(candidate, profile) {
    return Boolean(profile &&
        ...
        toInt(candidate.serviceStates &&
            candidate.serviceStates[String(STRUCTURE_SERVICE_ID.CYNO_JAMMER)], STRUCTURE_SERVICE_STATE.OFFLINE) === STRUCTURE_SERVICE_STATE.ONLINE);
}
function findOnlineTenebrexCynoJammerConflict(structure, profile, options = {}) {
    ...
    return candidates
        .filter((candidate) => (toPositiveInt(candidate && candidate.structureID, 0) !== structureID &&
        toPositiveInt(candidate && candidate.solarSystemID, 0) === solarSystemID &&
        isOnlineTenebrexCynoJammer(candidate, profile)))
        .sort((left, right) => (toPositiveInt(left && left.structureID, 0) -
        toPositiveInt(right && right.structureID, 0)))[0] || null;
}
...
function buildOnlineFlexFuelPatch(structure, profile, nowMs) {
    ...
    if (toPositiveInt(profile.serviceID, 0) === STRUCTURE_SERVICE_ID.CYNO_JAMMER) {
        onlineState.cynoJamActivatesAt =
            normalizedNowMs +
                Math.max(0, toInt(profile.cynoJammerActivationDelayMs, CYNO_JAMMER_ACTIVATION_DELAY_MS));
    }
    return {
        upkeepState: STRUCTURE_UPKEEP_STATE.FULL_POWER,
        fuelExpiresAt: estimateFuelExpiresAt(structure.structureID, profile.hourlyAmount, nextCycleAt),
        devFlags: {
            ...(structure.devFlags || {}),
...
function prepareFlexServiceStateTransition(structure, serviceID, serviceState, options = {}) {
    ...
    const nextOnline = toInt(serviceState, STRUCTURE_SERVICE_STATE.OFFLINE) === STRUCTURE_SERVICE_STATE.ONLINE;
    ...
    const requirementResult = validateServiceModuleOnlineRequirements(structure, profile.serviceModuleTypeID);
...
function applyFlexServiceFuelCycle(structure, nowMs = Date.now(), options = {}) {
    ...
    const requirementResult = validateServiceModuleOnlineRequirements(structure, profile.serviceModuleTypeID);
    if (!requirementResult.success) {
        return offlineFlexServiceForMissingRequirement(structure, profile, nowMs, requirementResult);
    }
    if (isFlexCynoBeaconBlockedBySystemJam(structure, profile, { nowMs })) {
        return offlineFlexServiceForCynoJam(structure, profile, nowMs);
    }
    const onlineJammerConflict = findLowerOnlineTenebrexCynoJammerConflict(structure, profile, options);
    if (onlineJammerConflict) {
        return offlineFlexServiceForOnlineJammerConflict(structure, profile, nowMs, onlineJammerConflict);
    }
    const proximityConflict = getFlexServiceProximityConflict(structure, profile, options);
    ...
    const currentState = structure.devFlags &&
        structure.devFlags.flexFuelState &&
        typeof structure.devFlags.flexFuelState === "object"
        ? structure.devFlags.flexFuelState
        : {};
    ...
    if (toInt(nowMs, Date.now()) < nextCycleAt) {
        const fuelExpiresAt = estimateFuelExpiresAt(structure.structureID, profile.hourlyAmount, nextCycleAt);
    ...
    return {
        structure: {
            ...structure,
            fuelExpiresAt: estimateFuelExpiresAt(structure.structureID, profile.hourlyAmount, nextCycle),
            devFlags: {
                ...(structure.devFlags || {}),
...
//# sourceMappingURL=structureFlexFuelRuntime.js.map