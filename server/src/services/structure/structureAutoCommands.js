...
const { getAllShipTypes, } = require(path.join(__dirname, "../chat/shipTypeRegistry"));
...
const { STRUCTURE_STATE_NAME_BY_ID, } = require(path.join(__dirname, "./structureConstants"));
...
function getUndockBatchSize(count) {
    ...
    return clamp(Math.ceil(requestedCount / TARGET_UNDOCK_BATCH_COUNT), MIN_UNDOCK_BATCH_SIZE, MAX_UNDOCK_BATCH_SIZE);
}
function getCompatibleUndockShipTypes(structure, hullMode = UNDOCK_HULL_MODE.MIXED) {
    return getAllShipTypes({ includeUnpublished: true }).filter((shipType) => {
        if (hullMode === UNDOCK_HULL_MODE.UNPUBLISHED &&
            shipType.published !== false) {
            return false;
        }
        if (hullMode === UNDOCK_HULL_MODE.PUBLISHED &&
            shipType.published === false) {
            return false;
        }
...
function buildUndockDummyShipSpec(session, structure, shipType, itemID) {
    ...
    const ownerID = normalizePositiveInt(session && (session.characterID || session.charid || session.userid), normalizePositiveInt(structure && (structure.ownerID || structure.ownerCorpID), 1)) || 1;
    const corporationID = normalizePositiveInt(session && (session.corporationID || session.corpid), normalizePositiveInt(structure && structure.ownerCorpID, 0));
    return {
        itemID,
...
function spawnUndockWaveBatch(session, structure, shipTypes = []) {
    ...
    for (const shipType of candidates) {
        ...
        const entity = spaceRuntime._testing &&
...
function startUndockWave(session, structure, count = DEFAULT_UNDOCK_DUMMY_COUNT, options = {}) {
    ...
    const job = structureAutoState.createCustomJob("undock", structure.structureID, session, (jobState, currentStructure) => {
        if (!currentStructure) {
            return {
                success: false,
...
function executeUpwellAutoCommand(session, argumentText) {
    ...
    if (normalizedFirstToken === "undock") {
        ...
        if (!spawnResult.success) {
            const failureText = spawnResult.errorMsg === "NO_COMPATIBLE_SHIP_TYPES"
        ...
        const currentSystemID = normalizePositiveInt(session && (session._space && session._space.systemID
            ? session._space.systemID
            : (session.solarsystemid2 || session.solarsystemid)), 0);
        const visibilityHint = currentSystemID > 0 && currentSystemID !== structure.solarSystemID
...
//# sourceMappingURL=structureAutoCommands.js.map