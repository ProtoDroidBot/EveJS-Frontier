...
const { STRUCTURE_SERVICE_STATE, } = require(path.join(__dirname, "./structureConstants"));
const { getStructureServiceIDsForModuleType, isStructureServiceModuleType, } = require(path.join(__dirname, "./structureServiceAuthority"));
...
function readServiceState(serviceStates, serviceID) {
    return toInt(serviceStates && serviceStates[String(serviceID)], STRUCTURE_SERVICE_STATE.OFFLINE);
}
...
function listServiceStateTransitions(beforeServiceStates = {}, afterServiceStates = {}) {
    ...
    for (const serviceIDText of serviceIDs) {
        ...
        if (beforeState === STRUCTURE_SERVICE_STATE.ONLINE &&
            afterState !== STRUCTURE_SERVICE_STATE.ONLINE) {
            offlineServiceIDs.push(serviceID);
        }
        else if (beforeState !== STRUCTURE_SERVICE_STATE.ONLINE &&
            afterState === STRUCTURE_SERVICE_STATE.ONLINE) {
...
function syncIndustryJobsForServiceStateTransition(structureID, beforeServiceStates = {}, afterServiceStates = {}) {
    ...
    const pauseResult = transitions.offlineServiceIDs.length > 0
        ? industryRuntime.pauseIndustryJobsForStructureServices(structureID, transitions.offlineServiceIDs)
        : { success: true, data: { changedJobIDs: [] } };
    const resumeResult = transitions.onlineServiceIDs.length > 0
        ? industryRuntime.resumeIndustryJobsForStructureServices(structureID, transitions.onlineServiceIDs)
        : { success: true, data: { changedJobIDs: [] } };
    return {
...
function cancelIndustryJobsForRemovedServiceModule(structureID, moduleTypeID, options = {}) {
    ...
    const backedServiceIDs = listBackedServiceIDsForStructure(structureID, options.excludedItemID);
    ...
    const industryRuntime = require(path.join(__dirname, "../industry/industryRuntimeState"));
    const result = industryRuntime.cancelIndustryJobsForStructureServices(structureID, unbackedServiceIDs, options);
    return {
        ...result,
        unbackedServiceIDs,
    };
...
//# sourceMappingURL=structureIndustryJobs.js.map