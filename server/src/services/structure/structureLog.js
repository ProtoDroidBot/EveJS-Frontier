...
function normalizeSessionContext(session) {
    ...
    const solarSystemID = (session._space && session._space.systemID) ||
...
function logCommand(session, commandText, result) {
    ...
    const normalizedMessage = result && result.message
        ...
        : "";
    info(`${normalizeSessionContext(session)} command=${JSON.stringify(normalizedCommand)} status=${status}${normalizedMessage ? ` message=${JSON.stringify(normalizedMessage)}` : ""}`);
}
...
//# sourceMappingURL=structureLog.js.map