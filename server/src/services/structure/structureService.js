...
const { buildBoundObjectResponse, extractList, resolveBoundNodeId, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { syncInventoryItemForSession, } = require(path.join(__dirname, "../character/characterState"));
const { ITEM_FLAGS, createSpaceItemForOwner, findItemById, moveItemToLocation, } = require(path.join(__dirname, "../inventory/itemStore"));
const { resolveItemByName, resolveItemByTypeID, } = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const { FUEL_BLOCK_GROUP_ID, TYPE_LIQUID_OZONE, } = require(path.join(__dirname, "../inventory/fuelBayInventory"));
const { buildCrpAccessDeniedInsufficientRolesValues, } = require(path.join(__dirname, "./structureServiceAuthority"));
const structureState = require(path.join(__dirname, "./structureState"));
const spaceRuntime = require(path.join(__dirname, "../../space/runtime"));
const { JETCAN_LIFETIME_MS, } = require(path.join(__dirname, "../ship/jettisonRuntime"));
...
function getSessionCorporationID(session) {
    return toPositiveInt(session && (session.corpid ||
        session.corporationID ||
        session.corpID), 0);
}
function getSessionRoleMask(session) {
    return toRoleMask(session && (session.corprole ||
        session.corpRole ||
        session.rolesAtAll));
}
...
function isStructureFuelJettisonItem(item) {
    ...
    return toPositiveInt((item && item.groupID) || (typeRecord && typeRecord.groupID), 0) === FUEL_BLOCK_GROUP_ID;
}
function getStructureSpacePosition(session, structure) {
    ...
    return normalizeVector((entity && entity.position) ||
...
function getStructureSpaceDirection(session, structure) {
    const structureID = toPositiveInt(structure && structure.structureID, 0);
    const entity = structureID > 0 ? spaceRuntime.getEntity(session, structureID) : null;
    return normalizeDirection((entity && entity.direction) ||
...
class StructureService extends BaseService {
    ...
    Handle_JettisonStructureFuel(args, session) {
        ...
        const createResult = createSpaceItemForOwner(ownerID, solarSystemID, containerType.match, {
        ...
        if (!createResult.success || !createResult.data) {
            log.warn(`[Structure] JettisonStructureFuel container creation failed: ${createResult.errorMsg || "UNKNOWN_ERROR"}`);
            return null;
        }
        ...
        for (const item of validItems) {
            ...
            if (!moveResult.success) {
                log.warn(`[Structure] JettisonStructureFuel failed item=${item.itemID}: ${moveResult.errorMsg || "UNKNOWN_ERROR"}`);
                continue;
            }
        ...
        if (!spawnResult || !spawnResult.success) {
            log.warn(`[Structure] JettisonStructureFuel moved fuel but failed to spawn container=${containerID}`);
        }
        return null;
    }
}
module.exports = StructureService;
//# sourceMappingURL=structureService.js.map