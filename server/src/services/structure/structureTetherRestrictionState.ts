...
function normalizeRestrictionState(value, characterID: number | string = 0) {
...
function updateCharacterTetherRestrictionState(characterID, updater, options: Record<string, any> = {}) {
  const numericCharacterID = toPositiveInt(characterID, 0);
...
function setCharacterTetherRestrictionFlags(characterID, flags: Record<string, any> = {}, options: Record<string, any> = {}) {
...
function setCharacterTetherDelay(characterID, delayMs, options: Record<string, any> = {}) {
...
function clearCharacterTetherRestrictions(characterID, options: Record<string, any> = {}) {
  return updateCharacterTetherRestrictionState(
    characterID,
    (current) => ({
      ...current,
...
function pruneExpiredCharacterTetherRestrictions(nowMs = Date.now()) {
  ...
  for (const [characterID, rawState] of Object.entries<any>(payload.restrictions || {})) {
...
function getCharacterStructureTetherRestriction(characterID, nowMs = Date.now(), options: Record<string, any> = {}) {
...
function getCharacterStructureDockingRestriction(characterID, nowMs = Date.now(), options: Record<string, any> = {}) {
  if (options.session && hasTetherRestrictionBypass(options.session)) {
    return {
      restricted: false,
      reason: null,
