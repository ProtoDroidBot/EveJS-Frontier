...
const { STRUCTURE_SERVICE_ID, STRUCTURE_SERVICE_STATE, } = require("./structureConstants");
...
const INSUFFICIENT_ROLES_LABEL = "UI/Corporations/AccessRestrictions/InsufficientRoles";
...
function buildServicesFromAccessSettings() {
    ...
    return Object.freeze(Object.fromEntries(Object.entries(bySettingID).map(([settingID, serviceIDs]) => [
        settingID,
...
function getStructureServiceAccessSettingID(serviceID) {
    return toInt(SERVICE_ACCESS_SETTING_BY_ID[toPositiveInt(serviceID, 0)], STRUCTURE_SETTING_ID.NONE);
}
function getStructureSettingAccessErrorLabel(settingID) {
    return (SETTING_ACCESS_ERROR_BY_ID[toInt(settingID, 0)] ||
        "StructureGenericSettingDenied");
}
function getStructureSettingAccessKind(settingID) {
    return (STRUCTURE_SETTING_ACCESS_KIND_BY_ID[toInt(settingID, 0)] ||
...
function characterCanDisableStructureServiceModule(session, moduleItem, structure = null) {
    const sessionCorpID = toPositiveInt(session && (session.corpid ??
        session.corporationID ??
        session.corpID), 0);
    const moduleOwnerID = toPositiveInt(moduleItem && moduleItem.ownerID, 0);
    const structureOwnerID = toPositiveInt(structure && (structure.ownerCorpID ??
        structure.ownerID), 0);
    if (!sessionCorpID) {
        return false;
    }
    ...
    const roleMask = toBigIntRoleMask(session && (session.corprole ??
        session.corpRole ??
        session.rolesAtAll));
...
function getConcreteServiceIDsForMetaService(serviceID) {
    ...
    return serviceIDs.filter((candidateServiceID) => (!META_SERVICE_SET.has(toPositiveInt(candidateServiceID, 0))));
}
...
function structureHasOnlineService(structure, serviceID) {
    ...
    const stateID = toInt(structure.serviceStates && structure.serviceStates[String(normalizedServiceID)], STRUCTURE_SERVICE_STATE.OFFLINE);
...
//# sourceMappingURL=structureServiceAuthority.js.map