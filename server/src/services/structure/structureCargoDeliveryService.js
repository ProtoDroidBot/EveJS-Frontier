...
function getSessionShipID(session) {
    return toPositiveInt(session && ((session._space && session._space.shipID) ||
        session.shipID ||
        session.shipid ||
...
function incrementFlexLiquidOzone(structureID, quantity) {
    ...
    const updateResult = structureState.updateStructureRecord(structureID, (current) => ({
        ...current,
        liquidOzoneQty: Math.max(0, toInt(current && current.liquidOzoneQty, 0)) +
            normalizedQuantity,
    }));
...
function validateCargoDepositAccess(session, shipID, structure) {
    ...
    if (toPositiveInt(structure.solarSystemID, 0) !== systemID) {
    ...
    if (!structureEntity ||
        ...
        throwNotify("That structure is not reachable from your current location.");
    }
...
class StructureCargoDeliveryService extends BaseService {
    ...
    Handle_DropOffItems(args, session) {
        ...
        for (const itemID of itemIDs) {
            ...
            const destinationOwnerID = structure.dockable === true
                ? access.characterID
                : toPositiveInt(structure.ownerCorpID || structure.ownerID, access.characterID);
            const destinationFlagID = structure.dockable === true
                ? ITEM_FLAGS.DELIVERIES
                : ITEM_FLAGS.STRUCTURE_FUEL_BAY;
            const movedQuantity = getStackQuantity(item);
            const moveResult = transferItemToOwnerLocation(item.itemID, destinationOwnerID, structure.structureID, destinationFlagID);
...
//# sourceMappingURL=structureCargoDeliveryService.js.map