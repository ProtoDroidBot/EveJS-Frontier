...
function getRequiredSovHubUpgradeTypeIDForServiceModule(moduleTypeID) {
    return toPositiveInt(getDogmaAttributes(moduleTypeID)[String(ATTRIBUTE_REQUIRES_SOV_HUB_UPGRADE)], 0);
}
function hasOnlineRequiredSovHubUpgrade(structure, requiredUpgradeTypeID) {
    ...
    const { getHubUpgrades } = require(path.join(__dirname, "../sovereignty/sovModernState"));
    ...
    const upgrades = Array.isArray(upgradeSnapshot && upgradeSnapshot.upgrades)
        ...
        : [];
    return upgrades.some((upgrade) => (toPositiveInt(upgrade && (upgrade.typeID || upgrade.installationTypeID), 0) ===
        upgradeTypeID &&
...
function validateServiceModuleOnlineRequirements(structure, moduleTypeID) {
    const requiredUpgradeTypeID = getRequiredSovHubUpgradeTypeIDForServiceModule(moduleTypeID);
...
//# sourceMappingURL=structureServiceOnlineRequirements.js.map