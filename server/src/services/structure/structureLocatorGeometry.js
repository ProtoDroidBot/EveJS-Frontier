...
function loadLocatorProfiles() {
    ...
    try {
        ...
        if (!locatorLoadFailureLogged) {
            ...
            log.warn(`[StructureLocatorGeometry] Failed to load ${STRUCTURE_GRAPHIC_LOCATORS_TABLE}: ${error.message}`);
        }
    }
...
function getLocatorCandidates(profile, shipTypeID) {
    ...
    const categoryMatches = locators.filter((locator) => locator.category === requestedCategory);
...
function getStructureFallbackDirection(structure) {
    const rotated = rotateVectorByRotation(DEFAULT_FORWARD, structure && structure.rotation);
...
function buildFallbackGeometry(structure, options = {}) {
    ...
    const dockOffset = Math.max(radius + DEFAULT_FALLBACK_DOCK_PADDING, DEFAULT_FALLBACK_DOCK_OFFSET);
    const extraUndockDistance = Math.max(0, toFiniteNumber(options.extraUndockDistance, 0));
    const dockPosition = addVectors(cloneVector(structure && structure.position), scaleVector(direction, dockOffset));
    return {
        source: "fallback",
        ...
        undockPosition: addVectors(dockPosition, scaleVector(direction, DEFAULT_FALLBACK_UNDOCK_PADDING + extraUndockDistance)),
...
function buildStructureDockingGeometry(structure, options = {}) {
    ...
    const worldDirection = normalizeVector(rotateVectorByRotation(locator.direction, rotation), getStructureFallbackDirection(structure));
    const worldPosition = addVectors(cloneVector(structure && structure.position), rotateVectorByRotation(locator.position, rotation));
    const extraUndockDistance = Math.max(0, toFiniteNumber(options.extraUndockDistance, 0));
    return {
        source: "authored",
        profile,
...
//# sourceMappingURL=structureLocatorGeometry.js.map