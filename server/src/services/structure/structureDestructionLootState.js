...
const { ITEM_FLAGS, createSpaceItemForCharacter, findItemById, grantItemToCharacterLocation, listContainerItems, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
const { resolveItemByTypeID, } = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const { resolveLocationDeathOutcome, } = require(path.join(__dirname, "../killmail/deathOutcomeResolver"));
...
function addQuantumCoreToDropLocation(structure, dropLocationID) {
    ...
    const ownerID = resolveLootOwnerCharacterID(structure);
    const grantResult = grantItemToCharacterLocation(ownerID, dropLocationID, ITEM_FLAGS.HANGAR, coreType, 1, {
...
function handleStructureDestroyedLoot(structure, options = {}) {
    ...
    const topLevelContents = includeContents
        ...
        : [];
    const killmailItems = [];
    const needsDropLocation = Boolean(resolvedWreckType) || topLevelContents.length > 0 || (includeQuantumCore &&
        structure &&
    ...
    const ownerID = resolveLootOwnerCharacterID(structure);
    let dropLocationResult = structureWreckState.createStructureWreck(structure, ownerID, {
        nowMs: options.nowMs,
    });
    let dropKind = "wreck";
    ...
    if (!quantumCoreResult.success) {
        log.warn(`[StructureDestructionLoot] Failed to create quantum-core drop for structure ${structureID}: ${quantumCoreResult.errorMsg}`);
    }
    if (quantumCoreResult.success &&
    ...
    if (dropKind === "wreck") {
        wreck = {
            ...
            movedItemIDs,
            quantumCore: quantumCoreResult.success && quantumCoreResult.data
                ? quantumCoreResult.data
                : {
                    dropped: false,
                },
...
//# sourceMappingURL=structureDestructionLootState.js.map