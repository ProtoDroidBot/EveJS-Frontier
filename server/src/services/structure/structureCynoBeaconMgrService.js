...
const { jumpSessionToSolarSystem, } = require(path.join(__dirname, "../../space/transitions"));
const { consumeFuelFromShipStorage, } = require(path.join(__dirname, "../../space/modules/sharedFuelRuntime"));
const { getActiveShipRecord, syncInventoryItemForSession, } = require(path.join(__dirname, "../character/characterState"));
const { buildEffectiveItemAttributeMap, buildShipResourceState, isEffectivelyOnlineModule, } = require(path.join(__dirname, "../fitting/liveFittingState"));
...
const { isSolarSystemCynoJammed, } = require(path.join(__dirname, "../sovereignty/sovSuppressionState"));
...
const structureTetherRestrictionState = require(path.join(__dirname, "./structureTetherRestrictionState"));
...
function throwIfPilotWarpLandingPending(session) {
...
function throwIfJumpActivationActive(session) {
    if (hasActiveJumpActivation(session && session.characterID)) {
        throwNotify("You cannot use a jump drive, jump bridge, or jump portal while your jump activation cooldown is active.");
    }
}
function applyStructureCynoTetherDelay(session, nowMs = Date.now()) {
    ...
    const current = structureTetherRestrictionState.getCharacterTetherRestrictionState(characterID, now);
    if (current &&
    ...
    return structureTetherRestrictionState.setCharacterTetherDelay(characterID, STRUCTURE_CYNO_TETHER_DELAY_MS, {
        nowMs: now,
    });
}
function applyStructureCynoTetherDelayToSuccessfulPassengers(conduitPlan, conduitPassengerResult, nowMs = Date.now()) {
    const successfulCharacterIDs = new Set((Array.isArray(conduitPassengerResult && conduitPassengerResult.results)
...
function getCurrentSolarSystemID(session) {
    return normalizePositiveInt(session &&
...
function getSolarSystemDistanceLy(sourceSolarSystemID, destinationSolarSystemID) {
    ...
    const distanceMeters = magnitude(subtractVectors(sourceSystem.position, destinationSystem.position));
...
function buildCynoBeaconArrivalSpawnState(destinationStructure, shipItem, shipEntity = null) {
    ...
    const direction = normalizeVector(magnitude(anchorPosition) > 0 ? anchorPosition : { x: 1, y: 0, z: 0 }, { x: 1, y: 0, z: 0 });
    const offset = Math.max(0, toFiniteNumber(destinationStructure && destinationStructure.radius, 0)) +
    ...
    return {
        ...
        anchorName: String((destinationStructure && (destinationStructure.itemName || destinationStructure.name)) ||
...
function resolveStandardBridgePortalModule(characterID, activeShip) {
    ...
    for (const fittedItem of fittedItems) {
        ...
        if (resolveJumpPortalMassFactor(fittedItem, moduleAttributes) <= 0) {
            throwNotify("That jump portal generator is missing its portal fuel mass factor.");
        }
        return {
            moduleAttributes,
    ...
    throwNotify("Your ship is not fitted with an online jump portal generator for that cynosural beacon.");
}
...
class StructureCynoBeaconMgrService extends BaseService {
    ...
    _jumpToBeacon(args, session, options = {}) {
        throwIfPilotWarpLandingPending(session);
        const { destinationSolarSystemID, structure, structureID, } = resolveUsableBeacon(args, session);
        const { activeShip, distanceLy, fuelTypeID, fuelQuantity, resourceState, } = resolveActiveShipAndJumpDrive(session, destinationSolarSystemID);
        ...
        if (options.groupAnchor === true) {
            ...
            conduitPlan = buildConduitPassengerPlans({
                ...
                buildSpawnState: ({ passengerEntity, passengerShip }) => buildCynoBeaconArrivalSpawnState(structure, passengerShip, passengerEntity),
            });
        ...
        const jumpResult = jumpSessionToSolarSystem(session, destinationSolarSystemID, {
            spawnStateOverride: buildCynoBeaconArrivalSpawnState(structure, activeShip, shipEntity),
        });
        if (!jumpResult.success) {
            log.warn(`[StructureCynoBeaconMgr] jump failed char=${session.characterID} structure=${structureID}: ${jumpResult.errorMsg}`);
            throwNotify("The cynosural beacon session change failed.");
        }
        ...
        if (options.groupAnchor === true) {
            ...
            applyStructureCynoTetherDelayToSuccessfulPassengers(conduitPlan, conduitPassengerResult, tetherDelayNowMs);
        }
        log.info(`[StructureCynoBeaconMgr] char=${session.characterID} jumped structure=${structureID} ` +
    ...
    Handle_CmdBridgeToStructureBeacon(args, session) {
        ensureFleetMembership(session);
        const { destinationSolarSystemID, structure, structureID, } = resolveUsableBeacon(args, session);
        const { activeShip, distanceLy, fuelTypeID, fuelQuantity, } = resolveActiveShipAndJumpDrive(session, destinationSolarSystemID);
        const portalModule = resolveStandardBridgePortalModule(session && session.characterID, activeShip);
        const activationResult = spaceRuntime.activateGenericModule(session, portalModule.portalModuleItem, JUMP_PORTAL_EFFECT_STANDARD, {
            repeat: 1,
        });
        ...
        fleetRuntime.setBridgeMode(session.fleetid, activeShip.itemID, destinationSolarSystemID, structureID, true);
        notifyGroupJumpAnchor(session, destinationSolarSystemID, 0);
        log.info(`[StructureCynoBeaconMgr] char=${session.characterID} opened bridge ` +
...
//# sourceMappingURL=structureCynoBeaconMgrService.js.map