...
const { applyModifierGroups, getTypeAttributeValue, isEffectivelyOnlineModule, isShipFittingFlag, typeHasEffectName, } = require(path.join(__dirname, "../fitting/liveFittingState"));
const { STRUCTURE_UPKEEP_STATE, } = require(path.join(__dirname, "./structureConstants"));
const { isStructureServiceModuleType, } = require(path.join(__dirname, "./structureServiceAuthority"));
const ATTRIBUTE_STRUCTURE_FULL_POWER_HP_MULTIPLIER = "structureFullPowerStateHitpointMultiplier";
const ATTRIBUTE_SERVICE_MODULE_FULL_POWER_HP_MULTIPLIER = "serviceModuleFullPowerStateHitpointMultiplier";
const ATTRIBUTE_SERVICE_MODULE_FULL_POWER_ARMOR_PLATING_MULTIPLIER = "serviceModuleFullPowerStateArmorPlatingMultiplier";
...
function getBaseStructureHitpoints(structure = {}) {
    ...
    return {
        shieldCapacity: Math.max(0, toFiniteNumber(structure.shieldCapacity, getTypeAttributeValue(typeID, "shieldCapacity"))),
        armorHP: Math.max(0, toFiniteNumber(structure.armorHP, getTypeAttributeValue(typeID, "armorHP"))),
        structureHP: Math.max(0, toFiniteNumber(structure.hullHP ?? structure.structureHP, getTypeAttributeValue(typeID, "hp", "structureHP"))),
    };
}
...
function resolveStructureFullPowerDogma(structure = {}, options = {}) {
    const structureID = toPositiveInt(structure.structureID || structure.itemID || options.structureID, 0);
    const typeID = toPositiveInt(structure.typeID, 0);
    ...
    const baseHitpointMultiplier = Math.max(0, toFiniteNumber(getTypeAttributeValue(typeID, ATTRIBUTE_STRUCTURE_FULL_POWER_HP_MULTIPLIER), 1));
    const hitpointMultiplier = hasOnlineServiceModule
        ? resolveAssignedMultiplier(onlineServiceModules, ATTRIBUTE_SERVICE_MODULE_FULL_POWER_HP_MULTIPLIER, baseHitpointMultiplier || 1)
        : (baseHitpointMultiplier || 1);
    const armorPlatingMultiplier = hasOnlineServiceModule
        ? resolveAssignedMultiplier(onlineServiceModules, ATTRIBUTE_SERVICE_MODULE_FULL_POWER_ARMOR_PLATING_MULTIPLIER, 0)
        : 0;
    return {
...
function resolveStructureEffectiveHitpoints(structure = {}, options = {}) {
    ...
    const armorMultiplierAttributes = {
        2751: Math.max(0, toFiniteNumber(getTypeAttributeValue(typeID, ATTRIBUTE_HIDDEN_ARMOR_HP_MULTIPLIER), 1)),
    };
    applyModifierGroups(armorMultiplierAttributes, onlineArmorPlatingModules.map((item) => ({
        modifiedAttributeID: 2751,
        operation: DOGMA_OP_POST_PERCENT,
        value: toFiniteNumber(getTypeAttributeValue(item.typeID, ATTRIBUTE_ARMOR_HP_BONUS), 0) *
    ...
    const hiddenArmorHPMultiplier = Math.max(0, toFiniteNumber(armorMultiplierAttributes[2751], 1));
    const resolvedDogma = {
        ...fullPowerDogma,
    ...
    return {
        ...
        effectiveArmorHP: round6(base.armorHP * fullPowerDogma.hitpointMultiplier * hiddenArmorHPMultiplier),
...
//# sourceMappingURL=structureFullPowerDogma.js.map