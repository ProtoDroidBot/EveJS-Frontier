...
function marshalValue(value) {
  ...
  if (typeof value === "object") {
    return buildDict(
      Object.entries<any>(value).map(([key, entry]) => [
...
function buildKeyValFromRecord(record: Record<string, any> = {}) {
  return buildKeyVal(
...
function buildAccessibleStructureServices(structure, session) {
  const entries = Object.entries<any>(structure && structure.serviceStates ? structure.serviceStates : {})
...
function buildStructureInfoPayload(structure, session, options: Record<string, any> = {}) {
  const info = structureState.buildStructureDirectoryInfo(structure);
...
function buildStructureInfoDict(structures: any[] = [], session, options: Record<string, any> = {}) {
  return buildDict(
...
function buildStructureMapList(structures: any[] = []) {
...
function buildIDList(values: any[] = []) {
  return buildList(
