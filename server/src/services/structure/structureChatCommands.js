...
const { STRUCTURE_SERVICE_NAME_BY_ID, STRUCTURE_SERVICE_STATE, STRUCTURE_STATE_NAME_BY_ID, STRUCTURE_UPKEEP_NAME_BY_ID, } = require(path.join(__dirname, "./structureConstants"));
const { getSessionStructureID, } = require(path.join(__dirname, "./structureLocation"));
...
function resolveServiceID(token) {
    ...
    const match = Object.entries(STRUCTURE_SERVICE_NAME_BY_ID).find(([, name]) => (normalized === name || normalized === String(name).replace(/^service_/, "")));
...
function executeStructureGmCommand(session, argumentText) {
    ...
    if (subcommand === "timer") {
        ...
        const result = structureState.setStructureStateTimerRemaining(structure.structureID, seconds);
        if (!result.success) {
            return {
                success: false,
    ...
    if (subcommand === "deploytimer") {
        ...
        const result = structureState.setStructureDeployTimerRemaining(structure.structureID, seconds);
        if (!result.success) {
            return {
                success: false,
    ...
    if (subcommand === "abandontimer") {
        ...
        const result = structureState.setStructureAbandonTimerRemaining(structure.structureID, seconds);
        if (!result.success) {
            return {
                success: false,
...
function formatTetherRestrictionState(characterID, nowMs = Date.now()) {
    const state = structureTetherRestrictionState.describeCharacterTetherRestrictions(characterID, nowMs);
    return [
...
function executeUpwellCommand(session, argumentText) {
    ...
    if (subcommand === "wraps") {
        ...
        return {
            success: true,
            message: wraps.map((wrap) => (`${wrap.wrapName}(${wrap.assetWrapID}) | owner=${wrap.ownerKind}:${wrap.ownerID} | source=${wrap.sourceStructureID} | delivered=${wrap.deliveredAt ? "yes" : "no"}`)).join("\n"),
        };
    }
    if (subcommand === "deliver") {
        ...
        const deliverResult = structureAssetSafetyState.deliverWrapToDestination(wrapID, destinationID, {
            session,
        });
    ...
    if (subcommand === "tether") {
        const characterID = normalizePositiveInt(session && (session.characterID || session.charid || session.userid), 0);
        ...
        if (tetherAction === "clear" || tetherAction === "reset") {
            const clearResult = structureTetherRestrictionState.clearCharacterTetherRestrictions(characterID, {
                nowMs,
            });
            return {
        ...
        if (["scram", "cyno", "fighters", "fw"].includes(tetherAction)) {
            ...
            const result = structureTetherRestrictionState.setCharacterTetherRestrictionFlags(characterID, flagMap[tetherAction], {
                nowMs,
            });
            return {
        ...
        if (tetherAction === "delay") {
            ...
            const result = structureTetherRestrictionState.setCharacterTetherDelay(characterID, Math.round(seconds * 1000), {
                nowMs,
            });
            return {
                success: result.success,
    ...
    if (subcommand === "core") {
        ...
        const result = structureState.setStructureQuantumCoreInstalled(structure.structureID, ["on", "true", "1"].includes(installed));
        if (!result.success) {
            return {
                success: false,
    ...
    if (subcommand === "service") {
        ...
        const result = structureState.setStructureServiceState(structure.structureID, serviceID, stateToken === "online" ? STRUCTURE_SERVICE_STATE.ONLINE : STRUCTURE_SERVICE_STATE.OFFLINE);
        if (!result.success) {
            return {
                success: false,
    ...
    if (subcommand === "damage") {
        ...
        const result = structureState.applyAdminStructureDamage(structure.structureID, layerToken, amount, {
            session,
        });
        if (!result.success) {
            return {
    ...
    if (subcommand === "safety") {
        ...
        const result = ownerKind === "corp"
            ? structureAssetSafetyState.moveCorporationAssetsToSafety(session, structure.solarSystemID, structure.structureID, {})
            : structureAssetSafetyState.movePersonalAssetsToSafety(session, structure.solarSystemID, structure.structureID, {});
        if (!result.success) {
            return {
                success: false,
...
//# sourceMappingURL=structureChatCommands.js.map