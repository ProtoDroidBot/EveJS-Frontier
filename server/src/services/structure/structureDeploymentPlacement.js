...
const { getClientParityWarpInPoint } = require(path.join(__dirname, "../../space/destiny/simulation/warpInPointParity"));
...
function hasGroupStargateRestriction(groupID) {
    return Object.prototype.hasOwnProperty.call(GROUP_MIN_DISTANCE_TO_STARGATE, groupID);
}
...
function findMoonBeaconInRange(solarSystemID, location, maxRange = MAXIMUM_MINING_BEACON_DISTANCE, balls = null) {
    const systemID = toPositiveInt(solarSystemID, 0);
    if (!systemID) {
        return null;
    }
...
function findDeploymentConflict(solarSystemID, typeID, location, balls) {
    ...
    if (isAutoMoonMiner(deployedTypeID) &&
        !findMoonBeaconInRange(solarSystemID, location, MAXIMUM_MINING_BEACON_DISTANCE, balls)) {
        return {
    ...
    for (const ball of Array.isArray(balls) ? balls : []) {
        ...
        const blockerPointType = toPositiveInt(ball.pointType, DEPLOYMENT_POINT_TYPE.REAL_BALL);
        if (isBallMoon(blockerTypeID, blockerPointType) &&
            distance < MINIMUM_MOON_DISTANCE + deployedRadius + blockerRadius) {
            const canDeployNearMoon = drillingPlatform &&
                findMoonBeaconInRange(solarSystemID, location, MAXIMUM_MINING_BEACON_DISTANCE, balls);
...
function validateDeploymentPlacement({ solarSystemID, typeID, position, offset, scene }) {
    ...
    if (offset && typeof offset === "object") {
        ...
        const offsetDistance = Math.sqrt((clientOffset.x * clientOffset.x) +
    ...
    const moonMiningBeaconID = (isAutoMoonMiner(typeID) ||
        isDrillingPlatform(typeID))
        ? findMoonBeaconInRange(solarSystemID, deploymentPosition, MAXIMUM_MINING_BEACON_DISTANCE, balls)
        : null;
    const conflict = findDeploymentConflict(solarSystemID, typeID, deploymentPosition, balls);
    if (conflict) {
        return {
            success: false,
...
//# sourceMappingURL=structureDeploymentPlacement.js.map