...
const { buildDict, buildFiletimeLong, buildKeyVal, buildList, normalizeNumber, normalizeText, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const structureState = require(path.join(__dirname, "./structureState"));
const { STRUCTURE_SERVICE_ID, STRUCTURE_SERVICE_STATE, } = require(path.join(__dirname, "./structureConstants"));
const { characterHasStructureService, } = require(path.join(__dirname, "./structurePayloads"));
const { isControllingStructureSession, } = require(path.join(__dirname, "./structureControlState"));
const { listOnlineStructureServiceModules, } = require(path.join(__dirname, "./structureServiceModules"));
...
function fileTimeLongFromMs(valueMs) {
    ...
    return buildFiletimeLong(BigInt(numericMs) * FILETIME_TICKS_PER_MS + FILETIME_EPOCH_OFFSET);
}
function notificationFileTimeLongFromMs(valueMs) {
    const numericMs = normalizeNonNegativeInteger(valueMs, currentFileTimeMs());
    return {
        type: "long",
        value: String(BigInt(numericMs) * FILETIME_TICKS_PER_MS + FILETIME_EPOCH_OFFSET),
    };
}
function clampDurationSeconds(value) {
    ...
    return Math.max(MINIMUM_EXTRACTION_DURATION_SECONDS, Math.min(MAXIMUM_EXTRACTION_DURATION_SECONDS, Math.trunc(duration)));
}
function getSessionCorpID(session) {
    return normalizePositiveInteger(session && (session.corpid || session.corporationID || session.corpID), 0);
}
function getSessionStructureID(session) {
    return normalizePositiveInteger(session && (session.structureid || session.structureID || session.shipid), 0);
}
function getSessionCharacterID(session) {
    return normalizePositiveInteger(session && (session.characterID || session.charid || session.charID), 0);
}
...
function hasOnlineMoonDrillModule(structureID) {
    return listOnlineStructureServiceModules(structureID)
        .some((moduleItem) => normalizePositiveInteger(moduleItem && moduleItem.typeID, 0) ===
        TYPE_STANDUP_MOON_DRILL);
}
function isMoonMiningServiceOnline(structure) {
    return normalizePositiveInteger(structure &&
        structure.serviceStates &&
        structure.serviceStates[String(STRUCTURE_SERVICE_ID.MOON_MINING)], STRUCTURE_SERVICE_STATE.OFFLINE) === STRUCTURE_SERVICE_STATE.ONLINE;
}
function isExtractionPullDisrupted(extraction, currentTimeMs) {
    const structureID = normalizePositiveInteger(extraction && extraction.structureID, 0);
    if (!structureID) {
        return false;
    }
...
function validateMoonExtractionMutationAccess(session, structureID, options = {}) {
    ...
    if (!characterHasStructureService(session, structure, STRUCTURE_SERVICE_ID.MOON_MINING)) {
        return { success: false, errorMsg: "STRUCTURE_CONTROL_DENIED" };
    }
...
function normalizeResourceEntry(rawEntry = {}) {
    ...
    return {
        ...
        yieldMultiplier: Math.max(0, normalizeFiniteNumber(source.yieldMultiplier, DEFAULT_YIELD_MULTIPLIER)) || DEFAULT_YIELD_MULTIPLIER,
        autoFractureDelaySeconds: Math.max(0, normalizeFiniteNumber(source.autoFractureDelaySeconds, DEFAULT_AUTO_FRACTURE_DELAY_SECONDS)),
        resources,
        oreVolumeByType: normalizeResourceVolumes(source.oreVolumeByType ||
            source.oreVolumeByTypeID ||
            source.oreVolumes ||
            source.moonOreVolumeByType),
        autoMoonMiningCycleOutput: normalizeResourceAmounts(source.autoMoonMiningCycleOutput || source.autoMoonMinerCycleOutput || source.cycleOutput),
        nextHarvestTimeMs: normalizeNonNegativeInteger(source.nextHarvestTimeMs || source.nextHarvestTime, 0),
        lastHarvestTimeMs: normalizeNonNegativeInteger(source.lastHarvestTimeMs || source.lastHarvestTime, 0),
        miningCycleTimeSeconds: normalizeNonNegativeInteger(source.miningCycleTimeSeconds || source.miningCycleTime, 0),
    };
}
...
function normalizeExtraction(rawEntry = {}) {
    ...
    const chunkAvailableTimeMs = normalizePositiveInteger(source.chunkAvailableTimeMs, 0);
    ...
    const naturalDecayTimeMs = normalizePositiveInteger(source.naturalDecayTimeMs, chunkAvailableTimeMs + DEFAULT_AUTO_FRACTURE_DELAY_SECONDS * 1000);
    return {
        extractionID,
        structureID,
        moonID: normalizePositiveInteger(source.moonID, 0),
        ...
        readyNotificationSentAtMs: normalizeNonNegativeInteger(source.readyNotificationSentAtMs || source.readyNotifiedAtMs, 0),
        oreVolumeByType: normalizeResourceVolumes(source.oreVolumeByType ||
            source.oreVolumeByTypeID ||
            source.oreVolumes ||
            source.moonOreVolumeByType),
...
function ensureStateShape(state) {
    const nextState = state && typeof state === "object"
        ? cloneValue(state)
        : buildInitialState();
    nextState.nextExtractionID = Math.max(1, normalizePositiveInteger(nextState.nextExtractionID, 1));
    nextState.nextEventID = Math.max(1, normalizePositiveInteger(nextState.nextEventID, 1));
    ...
    nextState.nextExtractionID = Math.max(nextState.nextExtractionID, highestExtractionID + 1);
...
function getActiveExtractionForStructure(state, structureID) {
    ...
    return Object.values(state.extractions)
        ...
        ACTIVE_EXTRACTION_STATES.has(entry.state)) || null;
}
...
function buildResourceCompositionPayload(profile) {
    ...
    return buildDict(Object.entries(profile.resources)
...
function resolveStructureContext(structureID, profile, session) {
    const structure = structureState.getStructureByID(structureID, { refresh: false });
    return {
        moonID: normalizePositiveInteger(profile && profile.moonID, normalizePositiveInteger(structure && structure.moonID, 0)),
        solarSystemID: normalizePositiveInteger(profile && profile.solarSystemID, normalizePositiveInteger(structure && structure.solarSystemID, normalizePositiveInteger(session && (session.solarsystemid2 || session.solarsystemid), 0))),
        ownerID: normalizePositiveInteger(profile && profile.ownerID, normalizePositiveInteger(structure && (structure.ownerCorpID || structure.ownerID), getSessionCorpID(session))),
        yieldMultiplier: Math.max(0, normalizeFiniteNumber(profile && profile.yieldMultiplier, DEFAULT_YIELD_MULTIPLIER)) || DEFAULT_YIELD_MULTIPLIER,
        autoFractureDelaySeconds: Math.max(0, normalizeFiniteNumber(profile && profile.autoFractureDelaySeconds, DEFAULT_AUTO_FRACTURE_DELAY_SECONDS)),
    };
}
...
function shouldSendExtractionUpdateToSession(session, extraction) {
    ...
    return ((ownerID > 0 && getSessionCorpID(session) === ownerID) ||
...
function readCharacterRecord(characterID) {
    ...
    const characters = result.success && result.data && typeof result.data === "object"
        ? result.data
        : {};
...
function buildCharacterShowInfoLink(characterID) {
    ...
    const characterTypeID = normalizePositiveInteger(record && (record.typeID || record.characterTypeID), TYPE_CHARACTER);
    const characterName = String((record && (record.characterName || record.name)) ||
...
function buildMoonMiningNotificationBaseData(extraction) {
    ...
    const structure = structureState.getStructureByID(structureID, { refresh: false });
    const structureTypeID = normalizePositiveInteger(structure && structure.typeID, 0);
    const solarSystemID = normalizePositiveInteger(extraction.solarSystemID, normalizePositiveInteger(structure && structure.solarSystemID, 0));
    const moonID = normalizePositiveInteger(extraction.moonID, normalizePositiveInteger(structure && structure.moonID, 0));
    if (!structureTypeID || !solarSystemID || !moonID) {
        log.warn(`[MoonExtractions] Skipping moon-mining notification for ` +
            `structure=${structureID}: missing structureTypeID/solarSystemID/moonID`);
        return null;
    }
    const structureName = String((structure && (structure.itemName || structure.name)) ||
...
function createMoonMiningOwnerNotifications(extraction, notificationTypeID, options = {}) {
    ...
    for (const characterID of [...new Set(getCharacterIDsInCorporation(ownerID))]) {
        ...
        if (!result || result.success !== true) {
            log.warn(`[MoonExtractions] Failed to create moon-mining notification ` +
...
function completeExpiredExtractionsInState(state, nowMs = Date.now()) {
    ...
    for (const [rawKey, rawEntry] of Object.entries(state.extractions || {})) {
        ...
        const fractureDue = extraction.state === "fracturing" &&
            extraction.fractureCompleteAtMs > 0 &&
            extraction.fractureCompleteAtMs <= currentTimeMs;
        const decayDue = extraction.state === "active" &&
...
class MoonExtractionsService extends BaseService {
    ...
    _completeExtractionForStructure(structureID, reason, session) {
        ...
        if (reason === COMPLETED_REASON_FRACTURED) {
            ...
            this._persistState();
            createMoonMiningOwnerNotifications(extraction, NOTIFICATION_TYPE.MOONMINING_LASER_FIRED, { firedBy: getSessionCharacterID(session) });
            return null;
        }
        ...
        if (reason === COMPLETED_REASON_CANCELLED) {
            createMoonMiningOwnerNotifications(extraction, NOTIFICATION_TYPE.MOONMINING_EXTRACTION_CANCELLED, { cancelledBy: getSessionCharacterID(session) });
        }
        sendMoonExtractionUpdate(session, null);
        return null;
    }
    Handle_GetExtractionAndEventIDFromStructureID(args = [], session) {
        const structureID = normalizePositiveInteger(Array.isArray(args) && args.length > 0 ? args[0] : getSessionStructureID(session), getSessionStructureID(session));
        const extraction = structureID
            ? getActiveExtractionForStructure(this._getState(), structureID)
            : null;
        return [
            buildExtractionPayload(extraction),
            extraction ? extraction.calendarEventID || 0 : null,
        ];
    }
    Handle_GetExtractionsForCorp(args = [], session) {
        const corpID = normalizePositiveInteger(Array.isArray(args) && args.length > 0 ? args[0] : getSessionCorpID(session), getSessionCorpID(session));
    ...
    Handle_StartNewExtraction(args = [], session) {
        ...
        if (!profile || Object.keys(profile.resources || {}).length === 0) {
            log.warn(`[MoonExtractions] Refusing to start extraction without seeded moon resources for structure ${structureID}`);
            return null;
        }
        ...
        const extraction = {
            ...
            chunkAvailableTimeMs,
            naturalDecayTimeMs: chunkAvailableTimeMs + structureContext.autoFractureDelaySeconds * 1000,
            durationSeconds,
        ...
        this._persistState();
        createMoonMiningOwnerNotifications(extraction, NOTIFICATION_TYPE.MOONMINING_EXTRACTION_STARTED, { startedBy: getSessionCharacterID(session) });
    ...
    Handle_FractureChunkWithStructure(args = [], session) {
        return this._completeExtractionForStructure(getSessionStructureID(session), COMPLETED_REASON_FRACTURED, session);
    }
    Handle_CancelExtraction(args = [], session) {
        return this._completeExtractionForStructure(getSessionStructureID(session), COMPLETED_REASON_CANCELLED, session);
    }
}
MoonExtractionsService.readResourceProfileForStructure = function readResourceProfileForStructure(structureID, options = {}) {
    const targetStructureID = normalizePositiveInteger(structureID, 0);
...
MoonExtractionsService.tickExtractions = function tickExtractions(nowMs = Date.now(), options = {}) {
    ...
    if (readyResult.changed || result.changed) {
        ...
        for (const extraction of readyResult.readyExtractions) {
            notifiedCharacters += createMoonMiningOwnerNotifications(extraction, NOTIFICATION_TYPE.MOONMINING_EXTRACTION_FINISHED);
        }
        for (const extraction of result.completedExtractions) {
            if (extraction.completedReason === COMPLETED_REASON_DECAYED) {
                notifiedCharacters += createMoonMiningOwnerNotifications(extraction, NOTIFICATION_TYPE.MOONMINING_AUTOMATIC_FRACTURE);
            }
...
//# sourceMappingURL=moonExtractionsService.js.map