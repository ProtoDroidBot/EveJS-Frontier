...
const { extractDictEntries, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
function throwDockingDenied(errorMsg = "", context = {}) {
    switch (String(errorMsg || "").trim()) {
        case "DOCKING_APPROACH_REQUIRED":
            throwWrappedUserError("DockingApproach", buildDockingApproachUserErrorValues(context && context.dockingDebug));
            break;
        ...
        case "STRUCTURE_DOCKING_DENIED":
            ...
            break;
        case "DOCKING_DISALLOWED_BY_MODULE":
            throwWrappedUserError("DockingRequestDenied", {
...
class StructureDockingService extends BaseService {
    ...
    Handle_Dock(args, session) {
        ...
        log.info(`[StructureDocking] Dock char=${session && session.characterID} structure=${structureID} ship=${requestedShipID || activeShipID || 0}`);
        ...
        if (spaceRuntime.isDockingDisallowedByModules(session)) {
            throwDockingDenied("DOCKING_DISALLOWED_BY_MODULE");
        }
    ...
    Handle_Undock(args, session, kwargs) {
        ...
        const requestedShipID = normalizePositiveInt(args && args[1], 0);
        const activeShipID = getSessionShipID(session);
        const ignoreContraband = Boolean(normalizeNumber(getKwargValue(kwargs, "ignoreContraband"), 0));
        log.info(`[StructureDocking] Undock char=${session && session.characterID} structure=${structureID} ship=${requestedShipID || activeShipID || 0}`);
        if (structureID > 0 &&
        ...
        if (!result || !result.success) {
            log.warn(`[StructureDocking] Undock failed for char=${session && session.characterID}: ${(result && result.errorMsg) || "UNKNOWN_ERROR"}`);
            if (result && result.errorMsg === "STRATEGIC_CRUISER_FIT_INCOMPLETE") {
                throwWrappedUserError("CustomNotify", {
                    notify: "A strategic cruiser requires a valid Core, Defensive, Offensive, " +
                        "and Propulsion subsystem fit before it can undock.",
                });
            }
            return null;
        }
...
//# sourceMappingURL=structureDockingService.js.map