...
const { buildDict, buildFiletimeLong, buildKeyVal, buildList, buildPackedRow, buildPackedRowDescriptor, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
const { SERVICE_ACCESS_SETTING_BY_ID, STRUCTURE_SETTING_ACCESS_KIND, STRUCTURE_SETTING_ID, getConcreteServiceIDsForMetaService, getStructureServiceAccessSettingID, getStructureSettingAccessKind, isStationOnlyStructureService, serviceOfflinesIfStructureTooClose, } = require(path.join(__dirname, "./structureServiceAuthority"));
const { findServiceProximityConflict, } = require(path.join(__dirname, "./structureServiceProximity"));
const { getProfileSettingValueForStructure, structureProfileSettingAllowsSession, } = require(path.join(__dirname, "./structureProfilesState"));
...
function buildKeyValFromRecord(record = {}) {
    return buildKeyVal(Object.entries(record || {}).map(([key, value]) => [key, marshalValue(value)]));
}
...
function getSessionAllianceID(session) {
    return normalizePositiveInt(session && (session.allianceID || session.allianceid), 0);
}
...
function characterHasStructureSetting(session, structure, settingID) {
    ...
    return accessKindAllows && structureProfileSettingAllowsSession(structure, normalizedSettingID, { session });
}
function characterHasStructureService(session, structure, serviceID) {
    ...
    const stateID = normalizeInt(structure.serviceStates && structure.serviceStates[String(normalizedServiceID)], STRUCTURE_SERVICE_STATE.OFFLINE);
    if (concreteServiceIDs.length > 0 && stateID !== STRUCTURE_SERVICE_STATE.ONLINE) {
        return concreteServiceIDs.some((concreteServiceID) => (characterHasStructureService(session, structure, concreteServiceID)));
    }
    if (stateID !== STRUCTURE_SERVICE_STATE.ONLINE) {
        return false;
    }
    if (CORE_REQUIRED_SERVICE_IDS.has(normalizedServiceID) &&
...
function buildAccessibleStructureServiceValue(structure, session, serviceID) {
    ...
    return normalizeStructureServiceValue(getProfileSettingValueForStructure(structure, settingID, {
        session,
        defaultValue: 0.0,
    }));
}
function buildAccessibleStructureServices(structure, session) {
    const entries = Object.entries(structure && structure.serviceStates ? structure.serviceStates : {})
        ...
        .filter(([serviceID, stateID]) => serviceID > 0 &&
...
function buildStructureInfoDict(structures = [], session, options = {}) {
    return buildDict((Array.isArray(structures) ? structures : [])
...
function buildStructureMapPayload(structure) {
    const [groupID, typeID, itemID, itemName, locationID, orbitID, connector, x, y, z, celestialIndex, orbitIndex,] = structureState.buildStructureMapEntry(structure);
...
function buildIDList(values = []) {
    return buildList([...new Set((Array.isArray(values) ? values : []).map((value) => normalizePositiveInt(value, 0)).filter(Boolean))]
...
//# sourceMappingURL=structurePayloads.js.map