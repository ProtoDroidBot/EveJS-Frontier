...
function createJob(mode, structureID, session, options: Record<string, any> = {}) {
...
function createCustomJob(mode, structureID, session, executeStep, options: Record<string, any> = {}) {
  const existingJob = getJobByStructureID(structureID);
  if (existingJob) {
    stopJobInternal(existingJob, "replaced by a new automation request", "replaced");
  }
...
function startAutoOnline(session, typeToken, options: Record<string, any> = {}) {
