...
function normalizeWrapRecord(entry: Record<string, any> = {}) {
...
function ensureWrapCache() {
  ...
  wrapCache = {
    ...
    wraps,
    byWrapID: new Map<any, any>(wraps.map((wrap) => [wrap.assetWrapID, wrap])),
  };
  return wrapCache;
}

function persistWraps(wraps, metaOverrides: Record<string, any> = {}) {
...
function listWraps(options: Record<string, any> = {}) {
...
function getWrapByID(assetWrapID, options: Record<string, any> = {}) {
  if (options.refresh !== false) {
    tickAssetSafetyWraps(options.nowMs);
  }
...
function getWrapNames(wrapIDs: any[] = []) {
...
function listWrapsForOwner(ownerKind, ownerID, options: Record<string, any> = {}) {
...
function listTopLevelStructureItems(ownerID, structureID, options: Record<string, any> = {}) {
...
function listTopLevelStructureHangarItems(ownerID, structureID, options: Record<string, any> = {}) {
...
function normalizeAllInventoryItems() {
  ...
  return (Array.isArray(items) ? items : Object.values<any>(items || {}))
    .filter(Boolean)
    .sort(
      (left, right) =>
...
function collectAssetSafetyNotificationRecipients(wrap) {
  ...
  if (wrap.ownerKind === "corp") {
    return [...new Set<any>(getCharacterIDsInCorporation(ownerID))];
  }
  return [ownerID];
}
...
function collectStructureOwnerNotificationRecipients(ownerID) {
  ...
  if (getOwnerKindForAssetSafety(numericOwnerID) === "corp") {
    return [...new Set<any>(getCharacterIDsInCorporation(numericOwnerID))];
  }
...
function ejectStructureAssetsToSpace(ownerID, structure, items, index, options: Record<string, any> = {}) {
  const numericOwnerID = normalizePositiveInt(ownerID, 0);
  const solarSystemID = normalizePositiveInt(structure && structure.solarSystemID, 0);
...
function handOffStructureAttachedItemsForUnanchor(structure, options: Record<string, any> = {}) {
  const structureID = normalizePositiveInt(structure && structure.structureID, 0);
  const solarSystemID = normalizePositiveInt(structure && structure.solarSystemID, 0);
  ...
  const fittingsContainerIDs = new Set<any>(
...
function ejectCorporationOfficeAssetsToSpace(corporationID, structure, office, items, options: Record<string, any> = {}) {
...
function createWrapFromItems(ownerKind, ownerID, structure, items: any[] = [], options: Record<string, any> = {}) {
  ...
  const existingWrap = lifecycleKey
    ...
    : null;
  const wrapCreateResult: Record<string, any> = existingWrap
    ? {
      success: true,
      data: existingWrap,
    }
  ...
  const recordedItemIDs = new Set<any>(
  ...
  for (const item of topLevelItems) {
    ...
    if (!moveResult.success) {
      ...
      const refreshResult: Record<string, any> = updateWrap(wrapID, (current) => ({
        ...current,
...
function deliverWrapToDestination(assetWrapID, destinationID, options: Record<string, any> = {}) {
...
function movePersonalAssetsToSafety(session, solarSystemID, structureID, options: Record<string, any> = {}) {
  const structureState = require(path.join(__dirname, "./structureState"));
...
function moveCorporationAssetsToSafety(session, solarSystemID, structureID, options: Record<string, any> = {}) {
  const structureState = require(path.join(__dirname, "./structureState"));
...
function moveCorporationOfficeAssetsToSafety(corporationID, structure, office, options: Record<string, any> = {}) {
  const corpID = normalizePositiveInt(corporationID, 0);
  const structureID = normalizePositiveInt(structure && structure.structureID, 0);
...
function removeCorporationOfficesAfterStructureUnanchor(
  ...
  const removedOfficeIDs: any[] = [];
  const removedOfficeKeys = new Set<any>();
  const updateResult = getCorporationRuntimeState().updateRuntimeState(
    (runtimeTable) => {
      const nextRuntimeTable = cloneValue(runtimeTable);
      for (const [corporationKey, corporationRuntime] of Object.entries<any>(
        ...
        for (const [officeKey, office] of Object.entries<any>(
          corporationRuntime.offices,
        )) {
          const officeID = normalizePositiveInt(
            office && (
  ...
  try {
    ...
    const affectedCorporationIDs = new Set<any>();
    for (const target of targetOffices) {
      if (
...
function handleStructureDestroyed(structure, options: Record<string, any> = {}) {
...
function handleStructureUnanchored(structure, options: Record<string, any> = {}) {
  const structureID = normalizePositiveInt(
    structure && structure.structureID,
    0,
  );
