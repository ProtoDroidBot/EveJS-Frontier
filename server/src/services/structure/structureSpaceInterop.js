...
const structureLocatorGeometry = require(path.join(__dirname, "./structureLocatorGeometry"));
...
function buildStructureUndockSpawnState(structure, options = {}) {
    return structureLocatorGeometry.buildStructureUndockSpawnState(structure, {
        ...
        selectionKey: options.selectionKey ??
            ...
            options.characterID ??
            null,
        extraUndockDistance: Math.max(0, toFiniteNumber(options.extraDistance, 0)),
...
function buildStructureScatterPosition(structure, index = 0, options = {}) {
    ...
    const radius = Math.max(Math.max(0, toFiniteNumber(structure && structure.radius, 0)) + 6000, Math.max(0, toFiniteNumber(options.scatterRadius, 0)) || 12000);
...
//# sourceMappingURL=structureSpaceInterop.js.map