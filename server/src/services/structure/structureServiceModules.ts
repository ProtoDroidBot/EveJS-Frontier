...
function appendStructureLocationFuelModifierEntries(destination, sourceItem, moduleItem, options: Record<string, any> = {}) {
...
function consumeStructureServiceFuelQuantity(structureID, moduleItem, requiredQuantity, options: Record<string, any> = {}) {
...
function consumeStructureServiceModuleOnlineFuel(structureID, moduleItem, options: Record<string, any> = {}) {
  const structure = structureState.getStructureByID(structureID, { refresh: false });
...
function tryAutoOnlineFittedStructureServiceModule(structureID, moduleItem, options: Record<string, any> = {}) {
  const numericStructureID = toPositiveInt(structureID, 0);
...
function consumeStructureServiceModuleCycleFuel(structureID, moduleItem, options: Record<string, any> = {}) {
...
function syncStructureServiceModuleState(structureID, options: Record<string, any> = {}) {
  const numericStructureID = toPositiveInt(structureID, 0);
  if (!numericStructureID) {
    return {
      success: false as const,
  ...
  const onlineModules: any[] = [];
  const fuelCycleChanges: any[] = [];
  const offlinedModuleIDs: any[] = [];
  const fuelStarvationAbandonAnchors: any[] = [];
  ...
  for (const moduleItem of rawOnlineModules) {
    ...
    const fuelCycleResult: Record<string, any> = consumeStructureServiceModuleCycleFuel(
      numericStructureID,
      moduleItem,
      options,
    );
  ...
  const onlineServiceIDs = new Set<any>();
