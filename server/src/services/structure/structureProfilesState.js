...
const { getCharacterRecord, listCharacterIDs, } = require(path.join(__dirname, "../character/characterState"));
const { listContainerItems, } = require(path.join(__dirname, "../inventory/itemStore"));
...
function normalizeRoot(rawValue) {
    ...
    next.nextProfileID = Math.max(toPositiveInt(source.nextProfileID, 1), highestProfileID + 1, 1);
    return next;
}
function readRoot() {
...
function findProfileSettingGroup(settingGroups, groupID) {
    ...
    return settingGroups.find((entry) => (entry && toInt(entry.groupID, 0) === numericGroupID));
}
...
function getAccessGroupMatchRank(groupID, session) {
    ...
    const directMember = members.find((entry) => (entry && toInt(entry.memberID, -1) === numericCharacterID));
    if (directMember) {
        const membershipType = toInt(directMember.membershipType, accessGroupStore.MEMBERSHIP_TYPE_NONE);
    ...
    for (const candidate of ownerCandidates) {
        const member = members.find((entry) => (entry && toInt(entry.memberID, -1) === candidate.memberID));
        if (!member) {
            continue;
        }
        const membershipType = toInt(member.membershipType, accessGroupStore.MEMBERSHIP_TYPE_NONE);
        if (membershipType === accessGroupStore.MEMBERSHIP_TYPE_EXCLUDED) {
            return 0;
        }
        const membershipRank = getMembershipRank(membershipType);
...
function resolveProfileSettingGroup(settingGroups, options = {}) {
    ...
    if (!hasGroupedLookup) {
        return findProfileSettingGroup(settingGroups, options && Object.prototype.hasOwnProperty.call(options, "groupID")
            ? options.groupID
            : 0);
    }
...
function profileHasAnySettingRows(profile) {
    const settingsBySettingID = profile && profile.settingsBySettingID && typeof profile.settingsBySettingID === "object"
...
function buildUniqueProfileName(root, corporationID, requestedName, ignoreProfileID = 0) {
    ...
    const takenNames = new Set(listProfileRecordsForCorporation(root, corporationID)
...
function createProfileRecord(root, corporationID, options = {}) {
    ...
    const profile = {
        ...
        name: buildUniqueProfileName(root, numericCorporationID, options.name || DEFAULT_PROFILE_NAME),
...
function resolveUsableProfileIDForCorporation(corporationID, requestedProfileID = null) {
    ...
    if (numericRequestedProfileID > 0) {
        const requestedProfile = getProfileForCorporation(numericCorporationID, numericRequestedProfileID);
        if (requestedProfile) {
...
function updateProfileForCorporation(corporationID, profileID, updates = {}) {
    ...
    profile.name = buildUniqueProfileName(root, numericCorporationID, updates.name || profile.name, profile.profileID);
    profile.description = normalizeProfileDescription(updates.description, profile.description);
...
function assignProfileToStructuresForCorporation(corporationID, profileID, structureIDs = []) {
    const numericCorporationID = toPositiveInt(corporationID, 0);
    const resolvedProfileID = resolveUsableProfileIDForCorporation(numericCorporationID, profileID);
    const normalizedStructureIDs = [...new Set((Array.isArray(structureIDs) ? structureIDs : [])
...
function buildCharacterStructureAccessSession(characterID) {
    ...
    const corporationID = toPositiveInt(character.corporationID || character.corpid || character.corpID, 0);
    return {
...
function characterHasDockingAccessForProfile(structure, session) {
    ...
    return structureProfileSettingAllowsSession(structure, STRUCTURE_SETTING_ID.HOUSING_CAN_DOCK, { session });
}
...
function isDefaultOwnerCorporationFallback(context) {
    ...
    return (corporationProfiles.length === 1 &&
...
//# sourceMappingURL=structureProfilesState.js.map