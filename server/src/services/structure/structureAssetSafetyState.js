...
const { ITEM_FLAGS, createSpaceItemForOwner, findItemById, getAllItems, grantItemToOwnerLocation, listContainerItems, moveItemToLocation, removeInventoryItem, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const { NEXT_ASSET_WRAP_ID_START, STRUCTURE_UPKEEP_STATE, } = require(path.join(__dirname, "./structureConstants"));
const { getCharacterIDsInCorporation, } = require(path.join(__dirname, "../corporation/corporationState"));
...
const { buildStructureScatterPosition, getStructureSpaceDirection, } = require(path.join(__dirname, "./structureSpaceInterop"));
...
function getCorporationRuntimeState() {
    return require(path.join(__dirname, "../corporation/corporationRuntimeState"));
}
function getOfficeRentalBilling() {
    return require(path.join(__dirname, "../corporation/officeRentalBilling"));
}
function getCorporationNotifications() {
    return require(path.join(__dirname, "../corporation/corporationNotifications"));
}
...
function getStaticStations() {
    ...
    stationCache = readStaticRows(TABLE.STATIONS)
        ...
        itemName: String(station && (station.stationName || station.itemName || `Station ${station.stationID}`)),
    }))
...
function normalizeWrapRecord(entry = {}) {
    ...
    const ejectTimeMs = normalizeTimestampMs(entry.ejectTimeMs, null) ||
    ...
    return {
        ...
        lifecycleKey: String(entry.lifecycleKey || "").trim() || null,
...
function ensureWrapCache() {
    ...
    wrapCache = {
        meta: {
            nextWrapID: Math.max(NEXT_ASSET_WRAP_ID_START, normalizePositiveInt(payload._meta && payload._meta.nextWrapID, NEXT_ASSET_WRAP_ID_START)),
...
function persistWraps(wraps, metaOverrides = {}) {
    ...
    const nextWrapID = Math.max(NEXT_ASSET_WRAP_ID_START, ...normalizedWraps.map((wrap) => normalizePositiveInt(wrap.assetWrapID, 0) + 1), NEXT_ASSET_WRAP_ID_START);
...
function updateWrap(assetWrapID, updater) {
    ...
    const writeResult = persistWraps(cache.wraps.map((wrap) => (wrap.assetWrapID === targetID ? next : wrap)));
...
function getWrapNames(wrapIDs = []) {
    return Object.fromEntries((Array.isArray(wrapIDs) ? wrapIDs : [wrapIDs])
...
function listWrapsForOwner(ownerKind, ownerID, options = {}) {
    ...
    return listWraps(options).filter((wrap) => wrap.ownerKind === normalizedOwnerKind &&
        normalizePositiveInt(wrap.ownerID, 0) === normalizedOwnerID);
}
function getSessionCharacterID(session) {
    return normalizePositiveInt(session && (session.characterID || session.charid || session.userid), 0);
}
function getSessionCorporationID(session) {
    return normalizePositiveInt(session && (session.corporationID || session.corpid), 0);
}
function sessionCanManageWrap(session, wrap) {
    ...
    return ((wrap.ownerKind === "char" && wrap.ownerID === getSessionCharacterID(session)) ||
...
function getFallbackNpcStationInfo(solarSystemID) {
    ...
    if (systemRecord) {
        const sameConstellation = stations.find((station) => normalizePositiveInt(station.constellationID, 0) > 0 &&
        ...
        const sameRegion = stations.find((station) => normalizePositiveInt(station.regionID, 0) > 0 &&
...
function isStructureHangarAssetItem(item) {
    ...
    return (STRUCTURE_RECOVERABLE_ASSET_FLAGS.has(flagID) ||
...
function buildOfficeEjectContainerName(structure) {
    const structureName = String(structure && (structure.itemName || structure.name || `Structure ${structure.structureID}`));
...
function buildStructureUnanchorAssetsMarker(structureID, ownerID) {
    return (`${STRUCTURE_UNANCHOR_ARTIFACT_PREFIX}:assets:${structureID}:owner:${ownerID}`);
}
function buildStructureUnanchorOfficeMarker(structureID, officeID) {
    return (`${STRUCTURE_UNANCHOR_ARTIFACT_PREFIX}:office:${structureID}:office:${officeID}`);
}
...
function isCanonicalStructureUnanchorSpaceArtifact(item, structure, ownerID) {
...
function findStructureUnanchorHull(structure, allItems, hullMarker) {
    ...
    return allItems.find((item) => normalizePositiveInt(item && item.typeID, 0) === structureTypeID &&
        ...
        String(item && item.customInfo || "") === hullMarker) || null;
}
function findStructureUnanchorFittingsContainers(structure, ownerID, allItems, fittingsMarker, coreCompleteMarker) {
    const structureID = normalizePositiveInt(structure && structure.structureID, 0);
    const solarSystemID = normalizePositiveInt(structure && structure.solarSystemID, 0);
    const legacyName = buildStructureDecommissionContainerName(structure);
    return allItems
        .filter((item) => {
        if (normalizePositiveInt(item && item.typeID, 0) !== OFFICE_EJECT_CONTAINER_TYPE_ID ||
            normalizePositiveInt(item && item.launcherID, 0) !== structureID ||
            normalizeInt(item && item.singleton, 0) !== 1) {
            return false;
        }
        ...
        return (normalizePositiveInt(item && item.ownerID, 0) === ownerID &&
        ...
        return (normalizePositiveInt(left && left.itemID, 0) -
            normalizePositiveInt(right && right.itemID, 0));
    });
}
function findStructureUnanchorEjectContainer(structure, ownerID, allItems, marker) {
    const structureID = normalizePositiveInt(structure && structure.structureID, 0);
    const markedItems = allItems.filter((item) => String(item && item.customInfo || "") === marker);
    const matchingItems = markedItems.filter((item) => normalizePositiveInt(item && item.typeID, 0) ===
        OFFICE_EJECT_CONTAINER_TYPE_ID &&
        normalizePositiveInt(item && item.launcherID, 0) === structureID &&
        normalizeInt(item && item.singleton, 0) === 1 &&
        isCanonicalStructureUnanchorSpaceArtifact(item, structure, ownerID));
...
function buildAssetSafetyMovedNotificationData(structure, wrap) {
    ...
    return {
        ...
        newStationID: normalizePositiveInt(wrap && wrap.nearestNPCStationInfo && wrap.nearestNPCStationInfo.itemID, 0) || null,
    };
}
...
function createAssetSafetyMovedNotifications(structure, wrap) {
    ...
    const senderID = normalizePositiveInt(structure && (structure.ownerCorpID || structure.ownerID), 0);
    ...
    for (const characterID of recipients) {
        ...
        if (!result || result.success !== true) {
            log.warn(`[StructureAssetSafety] Failed to create asset-safety notification ` +
...
function createStructureItemsNeedAttentionNotifications(structure, ownerID) {
    ...
    for (const characterID of recipients) {
        ...
        if (!result || result.success !== true) {
            log.warn(`[StructureAssetSafety] Failed to create items-need-attention notification ` +
...
function ejectStructureAssetsToSpace(ownerID, structure, items, index, options = {}) {
    ...
    const structureID = normalizePositiveInt(structure && structure.structureID, 0);
    const marker = buildStructureUnanchorAssetsMarker(structureID, numericOwnerID);
    const existingContainerResult = findStructureUnanchorEjectContainer(structure, numericOwnerID, normalizeAllInventoryItems(), marker);
    ...
    if (!container) {
        ...
        const createContainerResult = createSpaceItemForOwner(numericOwnerID, solarSystemID, containerType, {
    ...
    for (const item of structureItems) {
        ...
        if (!moveResult.success) {
            log.warn(`[StructureAssetSafety] Failed to eject structure item ${itemID} into container ${container.itemID}: ${moveResult.errorMsg}`);
...
function handOffStructureAttachedItemsForUnanchor(structure, options = {}) {
    const structureID = normalizePositiveInt(structure && structure.structureID, 0);
    const solarSystemID = normalizePositiveInt(structure && structure.solarSystemID, 0);
    const ownerID = normalizePositiveInt(structure && (structure.ownerCorpID || structure.ownerID), 0);
    ...
    if (normalizePositiveInt(structureType.categoryID, 0) !== CATEGORY_STRUCTURE) {
        return {
            success: false,
    ...
    const attachedItems = topLevelItems.filter((item) => !isStructureRigSlotItem(item) && isStructureAttachedItemForUnanchor(item));
    const quantumCoreItemTypeID = normalizePositiveInt(structure && structure.quantumCoreItemTypeID, 0);
    const existingTopLevelCoreItem = attachedItems.find((item) => normalizePositiveInt(item && item.flagID, 0) === ITEM_FLAGS.STRUCTURE_DEED &&
        normalizeInt(item && item.singleton, 0) === 1 &&
    ...
    const markedHullArtifacts = allItems.filter((item) => String(item && item.customInfo || "") === hullMarker);
    const markedFittingsArtifacts = allItems.filter((item) => [fittingsMarker, coreCompleteMarker].includes(String(item && item.customInfo || "")));
    const validMarkedFittingsArtifacts = markedFittingsArtifacts.filter((item) => normalizePositiveInt(item && item.typeID, 0) ===
        OFFICE_EJECT_CONTAINER_TYPE_ID &&
        normalizePositiveInt(item && item.launcherID, 0) === structureID &&
        normalizeInt(item && item.singleton, 0) === 1);
    let unanchoredHull = findStructureUnanchorHull(structure, allItems, hullMarker);
    if (markedHullArtifacts.length > 1 ||
    ...
    if (markedFittingsArtifacts.length > 1 ||
    ...
    const existingFittingsContainers = findStructureUnanchorFittingsContainers(structure, ownerID, allItems, fittingsMarker, coreCompleteMarker);
    ...
    const fittingsContainerIDs = new Set(existingFittingsContainers.map((item) => normalizePositiveInt(item && item.itemID, 0)));
    const existingContainedCoreItem = quantumCoreItemTypeID > 0
        ? allItems.find((item) => fittingsContainerIDs.has(normalizePositiveInt(item && item.locationID, 0)) &&
            normalizePositiveInt(item && item.flagID, 0) === ITEM_FLAGS.HANGAR &&
            normalizeInt(item && item.singleton, 0) === 1 &&
            normalizePositiveInt(item && item.typeID, 0) === quantumCoreItemTypeID)
        : null;
    const coreCompletionRecorded = existingFittingsContainers.some((item) => String(item && item.customInfo || "") === coreCompleteMarker);
    const requiresQuantumCore = structure &&
        structure.hasQuantumCore === true &&
        quantumCoreItemTypeID > 0;
    const shouldCreateCore = requiresQuantumCore &&
    ...
    if (trackedStructure &&
        unanchoredHull &&
        !isCanonicalStructureUnanchorSpaceArtifact(unanchoredHull, structure, ownerID)) {
        return {
            success: false,
            errorMsg: "INVALID_STRUCTURE_UNANCHOR_HULL_ARTIFACT",
        };
    ...
    if (trackedStructure &&
        ...
        !isCanonicalStructureUnanchorSpaceArtifact(decommissionContainer, structure, ownerID)) {
        return {
            success: false,
            errorMsg: "INVALID_STRUCTURE_UNANCHOR_FITTINGS_ARTIFACT",
        };
    ...
    if (decommissionContainer &&
        ![fittingsMarker, coreCompleteMarker].includes(String(decommissionContainer.customInfo || ""))) {
        const adoptLegacyContainerResult = updateInventoryItem(decommissionContainer.itemID, (current) => ({
            ...current,
    ...
    if (!unanchoredHull) {
        ...
        const createHullResult = createSpaceItemForOwner(ownerID, solarSystemID, structureType, {
    ...
    if (!decommissionContainer && needsFittingsContainer) {
        ...
        const createContainerResult = createSpaceItemForOwner(ownerID, solarSystemID, containerType, {
    ...
    for (const item of attachedItems) {
        ...
        const moveResult = moveItemToLocation(itemID, decommissionContainer.itemID, ITEM_FLAGS.HANGAR);
        if (!moveResult.success) {
            return moveResult;
        }
        movedItemIDs.push(itemID);
    ...
    if (shouldCreateCore) {
        const coreGrantResult = grantItemToOwnerLocation(ownerID, decommissionContainer.itemID, ITEM_FLAGS.HANGAR, coreType, 1, {
        ...
        const coreItem = coreGrantResult.data &&
    ...
    const completedCoreItemID = createdCoreItemID ||
        ...
        null;
    const quantumCoreHandoffComplete = requiresQuantumCore &&
    ...
    if (decommissionContainer &&
        ...
        const markCoreCompleteResult = updateInventoryItem(decommissionContainer.itemID, (current) => ({
            ...current,
...
function ejectCorporationOfficeAssetsToSpace(corporationID, structure, office, items, options = {}) {
    ...
    const structureID = normalizePositiveInt(structure && structure.structureID, 0);
    const officeID = normalizePositiveInt(office && (office.officeID || office.officeFolderID || office.itemID), 0);
    if (!officeID) {
        return {
            success: false,
    ...
    const existingContainerResult = structureUnanchor
        ? findStructureUnanchorEjectContainer(structure, corpID, normalizeAllInventoryItems(), marker)
        : {
            success: true,
            data: null,
        };
    ...
    let container = existingContainerResult.data;
    if (!container) {
        const position = buildStructureScatterPosition(structure, normalizeInt(options.scatterIndex, 0), options);
        const createContainerResult = createSpaceItemForOwner(corpID, solarSystemID, containerType, {
    ...
    for (const item of officeItems) {
        ...
        const moveResult = moveItemToLocation(itemID, container.itemID, ITEM_FLAGS.HANGAR);
        if (!moveResult.success) {
            log.warn(`[StructureAssetSafety] Failed to eject office item ${itemID} into container ${container.itemID}: ${moveResult.errorMsg}`);
            if (movedItemIDs.length > 0) {
...
function createWrapFromItems(ownerKind, ownerID, structure, items = [], options = {}) {
    ...
    const existingWrap = lifecycleKey
        ? ensureWrapCache().wraps.find((wrap) => !wrap.deliveredAt &&
            ...
            String(wrap.lifecycleKey || "") === lifecycleKey) || null
        : null;
    const wrapCreateResult = existingWrap
        ...
        : createWrap({
            ...
            nearestNPCStationInfo: normalizeStationInfo(options.nearestNPCStationInfo) ||
    ...
    const wrapID = normalizePositiveInt(wrapCreateResult.data && wrapCreateResult.data.assetWrapID, 0);
    const recordedItemIDs = new Set(listContainerItems(numericOwnerID, wrapID, ASSET_SAFETY_FLAG_ID)
        .map((item) => normalizePositiveInt(item && item.itemID, 0))
        .filter(Boolean));
    const movedItemIDs = [];
    for (const item of topLevelItems) {
        const moveResult = moveItemToLocation(item.itemID, wrapID, ASSET_SAFETY_FLAG_ID);
        if (!moveResult.success) {
            log.warn(`[StructureAssetSafety] Failed to move item ${item.itemID} into wrap ${wrapID}: ${moveResult.errorMsg}`);
    ...
    if (!refreshResult.success) {
        if (movedItemIDs.length > 0) {
            createAssetSafetyMovedNotifications(structure, wrapCreateResult.data);
        }
        return refreshResult;
    }
    if (movedItemIDs.length > 0) {
...
function destroyStructureRigsAfterUnanchorHandoffs(rigItemIDs) {
    ...
    for (const itemID of [...new Set((Array.isArray(rigItemIDs) ? rigItemIDs : [])
...
function listItemsInsideWrap(wrap) {
    ...
    return listContainerItems(normalizePositiveInt(wrap.ownerID, 0), normalizePositiveInt(wrap.assetWrapID, 0), ASSET_SAFETY_FLAG_ID);
}
function deliverWrapToDestination(assetWrapID, destinationID, options = {}) {
    ...
    const structureState = require(path.join(__dirname, "./structureState"));
    const bypass = Boolean(options.ignoreTimer === true || structureState.hasStructureGmBypass(session));
    ...
    const numericDestinationID = normalizePositiveInt(destinationID, 0) ||
        normalizePositiveInt(wrap.nearestNPCStationInfo && wrap.nearestNPCStationInfo.itemID, 0);
    ...
    if (destinationStructure) {
        ...
        if (destinationStructure.destroyedAt ||
        ...
        const accessResult = structureState.canCharacterDockAtStructure(session, destinationStructure, {
        ...
        if (normalizePositiveInt(destinationStation.solarSystemID, 0) !== wrap.solarSystemID &&
            numericDestinationID !== normalizePositiveInt(wrap.nearestNPCStationInfo && wrap.nearestNPCStationInfo.itemID, 0)) {
            return {
                success: false,
...
function tickAssetSafetyWraps(nowMs = Date.now()) {
    ...
    for (const wrap of wraps) {
        ...
        const deliverResult = deliverWrapToDestination(wrap.assetWrapID, wrap.nearestNPCStationInfo.itemID, {
            session: null,
        ...
        if (!deliverResult.success) {
            log.warn(`[StructureAssetSafety] Auto-move failed for wrap ${wrap.assetWrapID}: ${deliverResult.errorMsg}`);
            continue;
        }
        changed = true;
    }
...
function movePersonalAssetsToSafety(session, solarSystemID, structureID, options = {}) {
    ...
    if (normalizePositiveInt(solarSystemID, structure.solarSystemID) !==
    ...
    const assetSafetyDisabled = isAssetSafetyDisabledSolarSystem(structure.solarSystemID) &&
    ...
    const activeShipID = normalizePositiveInt(options.excludeActiveShipID ||
        (session && session.structureID === structure.structureID && (session.activeShipID || session.shipID || session.shipid)), 0);
    return createWrapFromItems("char", charID, structure, listTopLevelStructureItems(charID, structure.structureID, {
...
function moveCorporationAssetsToSafety(session, solarSystemID, structureID, options = {}) {
    ...
    return createWrapFromItems("corp", corpID, structure, listTopLevelStructureItems(corpID, structure.structureID), options);
}
function moveCorporationOfficeAssetsToSafety(corporationID, structure, office, options = {}) {
    ...
    if (assetSafetyDisabled) {
        return ejectCorporationOfficeAssetsToSpace(corpID, structure, office, officeItems, options);
    }
    const structureName = String(structure && (structure.itemName || structure.name || `Structure ${structureID}`));
    const officeID = normalizePositiveInt(office && (office.officeID || office.officeFolderID || office.itemID), 0);
    return createWrapFromItems("corp", corpID, structure, officeItems, {
        ...options,
        lifecycleKey: options.lifecycleKey ||
...
function removeCorporationOfficesAfterStructureUnanchor(structureID, offices) {
    const numericStructureID = normalizePositiveInt(structureID, 0);
    const officeKeys = new Set((Array.isArray(offices) ? offices : [])
        .map((office) => {
        const corporationID = normalizePositiveInt(office && office.corporationID, 0);
        const officeID = normalizePositiveInt(office && (office.officeID || office.officeFolderID || office.itemID), 0);
    ...
    try {
        ...
        for (const target of targetOffices) {
            const cancelledBills = cancelOfficeRentalBillsForOffice(target.corporationID, numericStructureID);
            if (cancelledBills.some((bill) => String(bill && bill.processingStatus || "") !==
                "cancelled")) {
                return {
                    success: false,
            ...
            for (const cancelledBill of cancelledBills) {
                const billID = normalizePositiveInt(cancelledBill && cancelledBill.billID, 0);
                if (billID) {
    ...
    const updateResult = getCorporationRuntimeState().updateRuntimeState((runtimeTable) => {
        const nextRuntimeTable = cloneValue(runtimeTable);
        for (const [corporationKey, corporationRuntime] of Object.entries(nextRuntimeTable.corporations || {})) {
            ...
            for (const [officeKey, office] of Object.entries(corporationRuntime.offices)) {
                const officeID = normalizePositiveInt(office && (office.officeID ||
                    office.officeFolderID ||
                    office.itemID), normalizePositiveInt(officeKey, 0));
                const officeIdentity = `${corporationID}:${officeID}`;
                if (normalizePositiveInt(office && office.stationID, 0) !==
                    numericStructureID ||
    ...
    try {
        const { notifyOfficeBillRefresh, notifyOfficeRentalChange, } = getCorporationNotifications();
        const affectedCorporationIDs = new Set();
        for (const target of targetOffices) {
            if (removedOfficeKeys.has(`${target.corporationID}:${target.officeID}`)) {
        ...
        log.warn(`[StructureAssetSafety] Failed to notify corporation office removal ` +
...
function handleStructureDestroyed(structure, options = {}) {
    ...
    const assetSafetyDisabled = (isAssetSafetyDisabledSolarSystem(structure.solarSystemID) ||
        Number(structure.upkeepState || 0) === STRUCTURE_UPKEEP_STATE.ABANDONED) && !bypass;
    if (assetSafetyDisabled) {
        log.warn(`[StructureAssetSafety] Asset safety is disabled for destroyed structure ${structure.structureID}; structure contents must be handled by the destruction-loot path instead.`);
        return {
            success: false,
            errorMsg: "ASSET_SAFETY_DISABLED",
        };
    ...
    for (const ownerID of ownerIDs) {
        ...
        const wrapResult = createWrapFromItems(ownerKind, ownerID, structure, listTopLevelStructureItems(ownerID, structure.structureID), {
            nowMs: options.nowMs,
        });
        if (!wrapResult.success) {
            log.warn(`[StructureAssetSafety] Failed to create ${ownerKind} wrap for owner ${ownerID} on structure ${structure.structureID}: ${wrapResult.errorMsg}`);
            return {
                success: false,
                errorMsg: "ASSET_SAFETY_DISABLED",
                data: {
                    fallbackReason: wrapResult.errorMsg || "ASSET_SAFETY_HANDOFF_FAILED",
                    partialWrap: wrapResult.data && wrapResult.data.createdWrap || null,
                    movedItemIDs: wrapResult.data &&
...
function handleStructureUnanchored(structure, options = {}) {
    ...
    const structureState = require(path.join(__dirname, "./structureState"));
    const bypass = structureState.hasStructureGmBypass(options.session);
    const assetSafetyDisabled = isAssetSafetyDisabledSolarSystem(structure.solarSystemID) && !bypass;
    const structureOffices = getCorporationRuntimeState()
        .getOfficesAtStation(structureID)
        .filter((office) => normalizePositiveInt(office && office.stationID, 0) === structureID);
    const ownerIDs = new Set();
    for (const item of listContainerItems(null, structure.structureID, null)) {
    ...
    const movedItemIDs = [];
    const attachedHandoffResult = handOffStructureAttachedItemsForUnanchor(structure, options);
    if (!attachedHandoffResult.success) {
        return attachedHandoffResult;
    }
    let ownerIndex = attachedHandoffResult.data && attachedHandoffResult.data.decommissionContainer ? 1 : 0;
    for (const ownerID of ownerIDs) {
        ...
        if (assetSafetyDisabled) {
            const ejectResult = ejectStructureAssetsToSpace(ownerID, structure, ownerItems, ownerIndex, options);
            ownerIndex += 1;
        ...
        const ownerKind = getOwnerKindForAssetSafety(ownerID);
        const wrapResult = createWrapFromItems(ownerKind, ownerID, structure, ownerItems, {
            lifecycleKey: buildStructureUnanchorAssetsMarker(structureID, ownerID),
            nowMs: options.nowMs,
        });
        if (!wrapResult.success) {
            return wrapResult;
    ...
    for (const office of structureOffices) {
        ...
        const officeResult = moveCorporationOfficeAssetsToSafety(corporationID, structure, office, {
            ...options,
    ...
    const officeRemovalResult = removeCorporationOfficesAfterStructureUnanchor(structureID, structureOffices);
    if (!officeRemovalResult.success) {
        return officeRemovalResult;
    }
    const rigDestructionResult = destroyStructureRigsAfterUnanchorHandoffs(attachedHandoffResult.data &&
    ...
    const attachedHandoff = {
        ...
        destroyedRigItemIDs: rigDestructionResult.data &&
    ...
    return {
        ...
        data: {
            ...
            ejectedContainers,
            movedItemIDs,
            removedOfficeIDs: officeRemovalResult.data &&
...
//# sourceMappingURL=structureAssetSafetyState.js.map