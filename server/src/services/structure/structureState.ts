...
const MANAGED_SERVICE_MODULE_TYPE_IDS_BY_SERVICE_ID = Object.freeze(
  Object.entries<any>(SERVICE_IDS_BY_MODULE_TYPE_ID).reduce((accumulator, [typeID, serviceIDs]) => {
...
let solarCache = null;
const structureChangeListeners = new Set<any>();
const pendingMarketOrderCancellationTasks = new Set<any>();
...
function normalizePosition(value, fallback: Record<string, any> = { x: 0, y: 0, z: 0 }) {
...
function normalizeStructureDescription(entry: Record<string, any> = {}) {
  const source =
...
function listTopLevelItemsInStructure(structureID) {
  ...
  return Object.values<any>(itemsResult.data)
...
function normalizeServiceStates(value, typeID, family) {
  ...
  const source = value && typeof value === "object" ? value : {};
  for (const [serviceID, stateID] of Object.entries<any>(source)) {
...
function countCharactersDockedInStructure(structureID) {
  ...
  return Object.values<any>(readCharacterRecords())
    .filter((record) => (
      record &&
...
function maybeCreateImpendingAbandonNotifications(structure, nowMs) {
  ...
  const notifiedKeys = notifiedForCurrentTimer && Array.isArray(structure.abandonmentWarningRecipients)
    ? new Set<any>(structure.abandonmentWarningRecipients.map((entry) => String(entry)))
    : new Set<any>();
...
function normalizeStructureTypeRecord(entry: Record<string, any> = {}) {
...
function ensureStructureTypes() {
  ...
  const typeIDs = new Set<any>(rows.map((entry) => entry.typeID));
  ...
  typeCache = {
    rows,
    byTypeID: new Map<any, any>(rows.map((entry) => [entry.typeID, entry])),
  };
  return typeCache;
}
...
function normalizeStructureRecord(entry: Record<string, any> = {}) {
...
function ensureStructureCache() {
  ...
  structureCache = {
    ...
    rows,
    byStructureID: new Map<any, any>(rows.map((entry) => [entry.structureID, entry])),
  };
  return structureCache;
}

function persistStructures(rows, metaOverrides: Record<string, any> = {}, options: Record<string, any> = {}) {
  ...
  structureCache = {
    ...
    rows: cachedRows,
    byStructureID: new Map<any, any>(cachedRows.map((entry) => [entry.structureID, entry])),
  };
...
function buildStructureChangeSystemIDs(previousRows: any[] = [], nextRows: any[] = []) {
  const previousByID = new Map<any, any>(
    (Array.isArray(previousRows) ? previousRows : [])
      .map((entry): [number, any] => [toPositiveInt(entry && entry.structureID, 0), entry])
  ...
  const changedSystemIDs = new Set<any>();
...
function notifyStructureChangeListeners(previousRows: any[] = [], nextRows: any[] = []) {
...
function maybeBroadcastCynoJammerStateChanges(previousRows: any[] = [], nextRows: any[] = []) {
  const systemIDs = buildStructureChangeSystemIDs(previousRows, nextRows);
  if (systemIDs.length <= 0) {
    return 0;
  }
...
function endWarsForLostWarHQLazily(structure, options: Record<string, any> = {}) {
  try {
    const {
      endWarsForLostWarHQ,
...
function collectStructureOwnerNotificationTargets(structure) {
  ...
  return [...new Set<any>(getCharacterIDsInCorporation(ownerCorpID))];
}
...
function collectStructureCorporationNotificationTargets(corporationIDs: any[] = []) {
...
function createStructureOwnerNotifications(structure, typeID, dataOverrides: Record<string, any> = {}) {
...
function normalizeStructureAttackContext(options: Record<string, any> = {}) {
  const source =
...
function shouldNotifyStructureUnderAttack(previousStructure, nextStructure, damageResult, nowMs, options: Record<string, any> = {}) {
...
function resolveOfflineServiceModuleTypeIDs(structure, offlineServiceIDs) {
  ...
  const structureID = toPositiveInt(structure && structure.structureID, 0);
  const fittedTypeIDs = new Set<any>();
  if (structureID > 0) {
    for (const item of listContainerItems(null, structureID, null)) {
  ...
  const fallbackTypeIDs = new Set<any>();
  for (const serviceID of serviceIDSet) {
    const mappedTypeIDs = [
      ...new Set<any>(MANAGED_SERVICE_MODULE_TYPE_IDS_BY_SERVICE_ID[String(serviceID)] || []),
...
function maybeCreateStructureTransitionNotifications(previousRows: any[] = [], nextRows: any[] = []) {
  const previousByID = new Map<any, any>(
    (Array.isArray(previousRows) ? previousRows : [])
      .map((entry): [number, any] => [toPositiveInt(entry && entry.structureID, 0), entry])
      .filter(([structureID]) => structureID > 0),
...
function getStructureByID(structureID, options: Record<string, any> = {}) {
...
function listStructures(options: Record<string, any> = {}) {
  if (options.refresh !== false) {
    tickStructures(Date.now());
  }
...
function listStructuresForSystem(solarSystemID, options: Record<string, any> = {}) {
...
function listOwnedStructures(ownerCorpID, options: Record<string, any> = {}) {
...
function updateStructureRecord(structureID, updater, options: Record<string, any> = {}) {
  const targetID = toPositiveInt(structureID, 0);
...
function createStructure(record, options: Record<string, any> = {}) {
...
function setStructureState(structureID, stateNameOrID, options: Record<string, any> = {}) {
  const numericState =
...
function setStructureStateTimerRemaining(structureID, seconds, options: Record<string, any> = {}) {
...
function setStructureDeployTimerRemaining(structureID, seconds, options: Record<string, any> = {}) {
  const remainingMs = Math.max(0, Math.round(Math.max(0, toFloat(seconds, 0)) * 1000));
  const nowMs = toInt(options.nowMs, Date.now());
  return updateStructureRecord(structureID, (current) => ({
    ...current,
...
function setStructureUnanchoringRemaining(structureID, seconds, options: Record<string, any> = {}) {
...
function setStructureAbandonTimerRemaining(structureID, seconds, options: Record<string, any> = {}) {
  const current = getStructureByID(structureID, { refresh: false });
  if (!current) {
    return {
      success: false as const,
...
function hasStructureGmBypass(session) {
  ...
  const role: bigint = normalizeRoleValue(session && session.role, 0n);
...
function canCharacterDockAtStructure(session, structure, options: Record<string, any> = {}) {
  if (!structure) {
    return {
      success: false as const,
      errorMsg: "STRUCTURE_NOT_FOUND",
  ...
  if (hasStructureGmBypass(session) || options.ignoreRestrictions === true) {
    return { success: true as const, data: { bypassed: true } };
  }
...
function isStructureRepairLayerDamageApplied(structure, beforeLayers: Record<string, any> = {}, afterLayers: Record<string, any> = {}) {
  const state = toInt(structure && structure.state, 0);
  if (state === STRUCTURE_STATE.SHIELD_VULNERABLE) {
...
function tickStructures(nowMs = Date.now()) {
  ...
  const cache = ensureStructureCache();
  const nextRows: any[] = [];
  const pendingFuelAlerts: any[] = [];
  const pendingReagentAlerts: any[] = [];
  const pendingWarHQLossStructures: any[] = [];
  let changed = false;
...
function seedStructureForSession(session, typeToken, options: Record<string, any> = {}) {
  const typeMap = {
    astrahus: 35832,
    fortizar: 35833,
    keepstar: 35834,
...
function startAnchoring(structureID, nowMs = Date.now(), options: Record<string, any> = {}) {
  return updateStructureRecord(structureID, (current) => ({
    ...current,
    state: STRUCTURE_STATE.ANCHOR_VULNERABLE,
    stateStartedAt: nowMs,
...
function startStructureUnanchoring(structureID, nowMs = Date.now()) {
  ...
  const updateResult: Record<string, any> = updateStructureRecord(structureID, (record) => ({
    ...record,
...
function cancelStructureUnanchoring(structureID, options: Record<string, any> = {}) {
  const nowMs = toInt(options.nowMs, Date.now());
  const current = getStructureByID(structureID, { refresh: false });
  if (!current) {
    return {
...
function removeJumpClonesForStructureLifecycle(structure, reason, options: Record<string, any> = {}) {
  const structureID = toPositiveInt(structure && structure.structureID, 0);
  if (!structureID) {
    return null;
  }
...
function setStructureServiceState(structureID, serviceID, serviceState, options: Record<string, any> = {}) {
  const numericServiceID = toPositiveInt(serviceID, 0);
  if (!numericServiceID) {
    return {
      success: false as const,
  ...
  const updateResult: Record<string, any> = updateStructureRecord(structureID, (current) => ({
    ...current,
...
function removeStructure(structureID, options: Record<string, any> = {}) {
  const targetID = toPositiveInt(structureID, 0);
  if (!targetID) {
    return {
      success: false as const,
...
function destroyStructure(structureID, options: Record<string, any> = {}) {
  const nowMs = toInt(options.nowMs, Date.now());
  const current = getStructureByID(structureID, { refresh: false });
  if (!current) {
    return {
  ...
  const updateResult: Record<string, any> = updateStructureRecord(structureID, (currentStructure) => ({
    ...currentStructure,
    destroyedAt: nowMs,
    unanchoring: null,
    stateEndsAt: null,
...
function applyRuntimeStructureDamage(structureID, damageResult, nowMs = Date.now(), options: Record<string, any> = {}) {
  ...
  const updateResult: Record<string, any> = updateStructureRecord(
    structureID,
    () => transition.structure,
    {
      ...options,
...
function applyAdminStructureDamage(structureID, layerToken, amount = null, options: Record<string, any> = {}) {
  const structure = getStructureByID(structureID, { refresh: false });
  if (!structure) {
    return {
      success: false as const,
  ...
  const result: Record<string, any> = applyRuntimeStructureDamage(
    structureID,
    damageResult,
    toInt(options.nowMs, Date.now()),
    options,
...
function getStructureServices(structure) {
  return Object.fromEntries(
    Object.entries<any>(structure && structure.serviceStates || {})
...
function listDockableStructuresForCharacter(session, options: Record<string, any> = {}) {
  const solarSystemID =
