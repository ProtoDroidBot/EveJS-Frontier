...
const { getDeploymentDistance, } = require(path.join(__dirname, "./structureDeploymentPlacement"));
const { serviceOfflinesIfStructureTooClose, } = require(path.join(__dirname, "./structureServiceAuthority"));
...
function getStructureRadius(structure) {
    return Math.max(0, toFiniteNumber(structure && structure.radius, getTypeRadius(structure && structure.typeID)));
}
...
function getMinimumCategoryStructureDistanceMeters(structure, otherStructure) {
    ...
    const minimumDistance = Math.min(Math.max(0, toFiniteNumber(ownDeploymentDistance, 0)), Math.max(0, toFiniteNumber(otherDeploymentDistance, 0)));
...
//# sourceMappingURL=structureServiceProximity.js.map