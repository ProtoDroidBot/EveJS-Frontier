...
const { ITEM_FLAGS, listContainerItems, updateInventoryItem, removeInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
const { ACCOUNT_KEY, JOURNAL_ENTRY_TYPE, adjustCharacterBalance, buildNotEnoughMoneyUserErrorValues, getCharacterWallet, } = require(path.join(__dirname, "../account/walletState"));
const { CORPORATION_WALLET_KEY_START, adjustCorporationWalletDivisionBalance, } = require(path.join(__dirname, "../corporation/corpWalletState"));
...
const { getProfileSettingValueForStructure, } = require(path.join(__dirname, "./structureProfilesState"));
const { STRUCTURE_SETTING_ID, } = require(path.join(__dirname, "./structureServiceAuthority"));
const { findServiceProximityConflict, } = require(path.join(__dirname, "./structureServiceProximity"));
const { getCharacterTetherRestrictionState, } = require(path.join(__dirname, "./structureTetherRestrictionState"));
const { TYPE_ANSIBLEX_JUMP_BRIDGE, } = require(path.join(__dirname, "../sovereignty/sovUpgradeSupport"));
...
function centerDistance(left, right) {
    return magnitude(subtractVectors(cloneVector(left && left.position), cloneVector(right && right.position)));
}
function surfaceDistance(left, right) {
    return Math.max(0, centerDistance(left, right) -
...
function isJumpBridgeServiceOnline(structure) {
    return normalizeInt(structure &&
        ...
        structure.serviceStates[String(STRUCTURE_SERVICE_ID.JUMP_BRIDGE)], STRUCTURE_SERVICE_STATE.OFFLINE) === STRUCTURE_SERVICE_STATE.ONLINE;
}
function hasJumpBridgeServiceProximityConflict(structure) {
    if (!structure) {
        return false;
    }
    return Boolean(findServiceProximityConflict(structure, STRUCTURE_SERVICE_ID.JUMP_BRIDGE, structureState.listStructuresForSystem(normalizePositiveInt(structure.solarSystemID, 0), { includeDestroyed: true, refresh: false })));
}
function getStructureDestinationSolarSystemID(structure) {
    const devFlags = structure && structure.devFlags && typeof structure.devFlags === "object"
        ? structure.devFlags
        : {};
    return normalizePositiveInt(devFlags.destinationSolarsystemID ||
        devFlags.sovereigntyJumpBridgeDestinationSolarsystemID, 0);
}
...
function findReciprocalLinkedStructure(sourceStructure) {
    ...
    return structureState.listStructures({
        ...
        refresh: false,
    }).find((candidate) => (isAnsiblexStructure(candidate) &&
        normalizePositiveInt(candidate.solarSystemID, 0) === destinationSolarSystemID &&
        getStructureDestinationSolarSystemID(candidate) ===
            normalizePositiveInt(sourceStructure.solarSystemID, 0))) || null;
}
function resolveShipMassKg(shipItem, shipEntity = null) {
    const directMass = toFiniteNumber(shipEntity && shipEntity.mass, toFiniteNumber(shipItem && shipItem.mass, 0));
    if (directMass > 0) {
        return directMass;
    }
...
function getJumpBridgeIskPerLiquidOzone(structure, session) {
    return normalizeJumpBridgeIskPerLiquidOzone(getProfileSettingValueForStructure(structure, STRUCTURE_SETTING_ID.JUMP_BRIDGE_ACTIVATION, {
        session,
        defaultValue: 0,
    }));
}
...
function chargeJumpBridgeActivationFee(session, structure, amount) {
    ...
    if (ownerCorpID > 0) {
        creditResult = adjustCorporationWalletDivisionBalance(ownerCorpID, CORPORATION_WALLET_KEY_START, normalizedAmount, {
            description,
            entryTypeID: JOURNAL_ENTRY_TYPE.STRUCTURE_GATE_JUMP,
            ownerID1: characterID,
            ownerID2: ownerCorpID,
        ...
        if (!creditResult || creditResult.success !== true) {
            log.warn(`[StructureJumpBridgeMgr] failed to credit activation fee ownerCorp=${ownerCorpID} structure=${structureID}: ${creditResult && creditResult.errorMsg ? creditResult.errorMsg : "UNKNOWN"}`);
        }
    }
    return {
...
function listStructureLiquidOzoneStacks(structure) {
    ...
    return listContainerItems(null, structureID, ITEM_FLAGS.STRUCTURE_FUEL_BAY)
        .filter((item) => (normalizePositiveInt(item && item.typeID, 0) === TYPE_LIQUID_OZONE &&
...
function isCharacterWarpDisrupted(session, shipEntity = null) {
    ...
    const restrictionState = getCharacterTetherRestrictionState(session && session.characterID);
...
function findWarpDisruptionFieldForShip(systemID, shipEntity = null) {
    ...
    const scene = spaceRuntime &&
        ...
        : null;
    return mobileWarpDisruptorRuntime.findActiveWarpDisruptorForPosition(systemID, shipEntity.position, { scene, targetEntity: shipEntity });
}
class StructureJumpBridgeMgrService extends BaseService {
    ...
    Handle_CmdJumpThroughStructureStargate(args, session) {
        ...
        if (normalizePositiveInt(sourceStructure.solarSystemID, 0) !==
        ...
        if (!shipEntity ||
            ...
            throwNotify("That jump bridge is not reachable from your current location.");
        }
        ...
        const liquidOzoneRequired = calculateLiquidOzoneRequired(shipMassKg, distanceLy === null ? 0 : distanceLy);
        const activationFee = calculateJumpBridgeActivationFee(sourceStructure, session, liquidOzoneRequired);
        ...
        if (!jumpResult.success) {
            log.warn(`[StructureJumpBridgeMgr] jump failed char=${session.characterID} structure=${structureID}: ${jumpResult.errorMsg}`);
            throwNotify("The jump bridge session change failed.");
        }
        log.info(`[StructureJumpBridgeMgr] char=${session.characterID} jumped source=${sourceStructure.structureID} destination=${destinationStructure.structureID} ozone=${liquidOzoneRequired} fee=${activationFee}`);
...
//# sourceMappingURL=structureJumpBridgeMgrService.js.map