...
function normalizeProfileSettingsBySettingID(value) {
  ...
  for (const [rawSettingID, rawEntries] of Object.entries<any>(value)) {
...
function normalizeProfileRecord(profile: Record<string, any> = {}) {
...
function normalizeRoot(rawValue) {
  ...
  if (source.profilesByID && typeof source.profilesByID === "object") {
    for (const rawProfile of Object.values<any>(source.profilesByID)) {
...
function normalizeGroupIDList(values: any[] = []) {
...
function resolveProfileSettingGroup(settingGroups, options: Record<string, any> = {}) {
...
function listProfileRecordsForCorporation(root, corporationID) {
  ...
  return Object.values<any>(root.profilesByID)
...
function profileHasAnySettingRows(profile) {
  const settingsBySettingID =
    profile && profile.settingsBySettingID && typeof profile.settingsBySettingID === "object"
      ...
      : {};
  return Object.values<any>(settingsBySettingID)
...
function clearDefaultFlagForCorporation(root, corporationID) {
  for (const profile of Object.values<any>(root.profilesByID)) {
...
function createProfileRecord(root, corporationID, options: Record<string, any> = {}) {
  const numericCorporationID = toPositiveInt(corporationID, 0);
...
function updateProfileForCorporation(corporationID, profileID, updates: Record<string, any> = {}) {
  const numericCorporationID = toPositiveInt(corporationID, 0);
  const numericProfileID = toPositiveInt(profileID, 0);
  if (!numericCorporationID || !numericProfileID) {
    return null;
...
function saveProfileSettingsForCorporation(corporationID, profileID, settings: any[] = []) {
  const numericCorporationID = toPositiveInt(corporationID, 0);
  const numericProfileID = toPositiveInt(profileID, 0);
  if (!numericCorporationID || !numericProfileID) {
    return null;
...
function assignProfileToStructuresForCorporation(corporationID, profileID, structureIDs: any[] = []) {
  const numericCorporationID = toPositiveInt(corporationID, 0);
...
function collectStructureAssetCharacterIDs(structure) {
  ...
  const characterIDs = new Set<any>();
...
function getProfileSettingResolutionForStructure(structure, settingID, options: Record<string, any> = {}) {
...
function structureProfileSettingAllowsSession(structure, settingID, options: Record<string, any> = {}) {
...
function getProfileSettingValueForStructure(structure, settingID, options: Record<string, any> = {}) {
  const resolution = getProfileSettingResolutionForStructure(structure, settingID, options);
