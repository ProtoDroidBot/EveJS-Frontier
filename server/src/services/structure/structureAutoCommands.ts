...
function cloneVector(vector, fallback: Record<string, any> = { x: 0, y: 0, z: 1 }) {
  return {
...
function getCompatibleUndockShipTypes(structure, hullMode: string = UNDOCK_HULL_MODE.MIXED) {
...
function spawnUndockWaveBatch(session, structure, shipTypes: any[] = []) {
  const candidates = Array.isArray(shipTypes) ? shipTypes.filter(Boolean) : [];
...
function parseUndockCommandOptions(tokens: any[] = []) {
