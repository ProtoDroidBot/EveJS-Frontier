...
const { createUuidString, } = require(path.join(__dirname, "../../_secondary/express/gatewayServices/gatewayServiceHelpers"));
const { normalizeRoleValue, } = require(path.join(__dirname, "../account/accountRoleProfiles"));
const sessionRegistry = require(path.join(__dirname, "../chat/sessionRegistry"));
const { adjustCorporationWalletLPBalance, EVERMARK_ISSUER_CORP_ID, getCorporationWalletLPBalance, } = require(path.join(__dirname, "../corporation/lpWalletState"));
const { getCharacterIDsInCorporation, } = require(path.join(__dirname, "../corporation/corporationState"));
const { CORP_ROLE_BRAND_MANAGER, CORP_ROLE_DIRECTOR, getCorporationMember, normalizePositiveInteger, toRoleMaskBigInt, } = require(path.join(__dirname, "../corporation/corporationRuntimeState"));
...
const { getStructureByID, getStructureTypeByID, listOwnedStructures, toFileTimeLongFromMs, } = require(path.join(__dirname, "./structureState"));
const { STRUCTURE_OFFLINE_STATES, STRUCTURE_SERVICE_STATE, STRUCTURE_SIZE, STRUCTURE_UPKEEP_STATE, } = require(path.join(__dirname, "./structureConstants"));
...
const STRUCTURE_PAINTWORK_ADMIN_ROLE_MASK = ACCOUNT_ROLE_PROGRAMMER | ACCOUNT_ROLE_QA | ACCOUNT_ROLE_GML;
...
function writeRoot(root) {
    const next = {
        ...
        catalogueByTypeID: root && root.catalogueByTypeID && typeof root.catalogueByTypeID === "object"
            ? root.catalogueByTypeID
            : {},
        licensesByID: root && root.licensesByID && typeof root.licensesByID === "object"
            ? root.licensesByID
            : {},
        structureAssignments: root && root.structureAssignments && typeof root.structureAssignments === "object"
...
function canCharacterManageStructurePaintwork(characterID, corporationID = null) {
    ...
    return ((roles & CORP_ROLE_DIRECTOR) === CORP_ROLE_DIRECTOR ||
...
function normalizeLicenseRecord(licenseID, value = {}) {
    return {
        ...
        activatorCharacterID: normalizePositiveInteger(value.activatorCharacterID, 0) || 0,
        issuedAtMs: Math.max(0, Number(value.issuedAtMs || 0) || 0),
        durationSeconds: Math.max(0, Math.trunc(Number(value.durationSeconds || DEFAULT_DURATION_SECONDS) || DEFAULT_DURATION_SECONDS)),
        ...
        priceAmount: Math.max(0, Math.trunc(Number(value.priceAmount || 0) || 0)),
        associatedCorporationID: normalizePositiveInteger(value.associatedCorporationID, HERALDRY_CORPORATION_ID) || HERALDRY_CORPORATION_ID,
        paintwork: normalizePaintwork(value.paintwork),
    };
}
function normalizeCatalogueEntry(entry, fallbackDurationSeconds) {
    return {
        durationSeconds: Math.max(0, Math.trunc(Number(entry && entry.durationSeconds !== undefined
            ? entry.durationSeconds
            : fallbackDurationSeconds) || fallbackDurationSeconds)),
        priceAmount: Math.max(0, Math.trunc(Number(entry && entry.priceAmount) || DEFAULT_PRICE_AMOUNT)),
        associatedCorporationID: normalizePositiveInteger(entry && entry.associatedCorporationID, HERALDRY_CORPORATION_ID) || HERALDRY_CORPORATION_ID,
    };
}
function getBootstrapCataloguePrice(structureTypeID, durationSeconds) {
    const typeRecord = getStructureTypeByID(structureTypeID) || {};
    const sizeKey = String(typeRecord.structureSize || STRUCTURE_SIZE.UNDEFINED) || STRUCTURE_SIZE.UNDEFINED;
    const priceTable = DEFAULT_CATALOGUE_PRICE_BY_SIZE[sizeKey] ||
        DEFAULT_CATALOGUE_PRICE_BY_SIZE[STRUCTURE_SIZE.UNDEFINED];
    return Math.max(0, Math.trunc(Number(priceTable[durationSeconds]) || DEFAULT_PRICE_AMOUNT));
}
...
function normalizeCatalogueEntriesForType(structureTypeID, rawEntries) {
    ...
    for (const rawEntry of Array.isArray(rawEntries) ? rawEntries : []) {
        ...
        if (!STRUCTURE_PAINTWORK_DURATION_SECONDS.includes(normalized.durationSeconds) ||
...
function getCatalogueItems() {
    ...
    return [...PAINT_ELIGIBLE_STRUCTURE_TYPE_IDS]
        ...
        .flatMap((typeID) => (Array.isArray(catalogueByTypeID[String(typeID)])
...
function getCatalogueItemForStructureTypeAndDuration(structureTypeID, durationSeconds) {
    ...
    const normalizedDurationSeconds = Math.max(0, Math.trunc(Number(durationSeconds || 0) || 0));
    const entries = ensureCatalogueDefaults()[String(normalizedStructureTypeID)];
    if (!Array.isArray(entries)) {
        return null;
    }
    const match = entries.find((entry) => Math.trunc(Number(entry && entry.durationSeconds) || 0) ===
...
function hasAnyOnlineServiceModule(structure) {
    const serviceStates = structure && structure.serviceStates && typeof structure.serviceStates === "object"
        ? structure.serviceStates
        : {};
    return Object.values(serviceStates).some((stateID) => Number(stateID || 0) === STRUCTURE_SERVICE_STATE.ONLINE);
}
...
function buildIndexes() {
    ...
    const sanitizedLicensesByID = Object.fromEntries(sanitizedLicenses.map((license) => [license.licenseID, cloneValue(license)]));
    const sanitizedStructureAssignments = Object.fromEntries(sanitizedLicenses.map((license) => [String(license.structureID), license.licenseID]));
    if (!areJsonEqual(root.licensesByID || {}, sanitizedLicensesByID) ||
...
function getLicensesForCorporation(corporationID) {
    return cloneValue(buildIndexes().licensesByCorporation.get(Number(corporationID) || 0) || []);
}
function getPaintworksForSolarSystem(solarSystemID) {
    return cloneValue(buildIndexes().paintworksBySolarSystem.get(Number(solarSystemID) || 0) || []);
}
...
function buildWritableLicenseMapFromIndexes() {
    return Object.fromEntries([...buildIndexes().licensesByID.values()].map((license) => [
        license.licenseID,
        cloneValue(license),
    ]));
}
...
function createStructurePaintPurchasedNotifications(characterID, corporationID, issuedLicenses, lpAmount) {
    ...
    const durationSeconds = normalizePositiveInteger(licenses[0] && licenses[0].durationSeconds, 0);
    const data = {
    ...
    for (const targetCharacterID of [...new Set(getCharacterIDsInCorporation(numericCorporationID))]) {
        ...
        if (!result || result.success !== true) {
            log.warn(`[StructurePaintwork] Failed to create paint purchase notification ` +
...
function normalizeIssueOptions(options = {}) {
    return {
        ...
        priceAmount: options.priceAmount === undefined || options.priceAmount === null
            ? null
...
function resolvePerStructureIssuePrice(structure, durationSeconds, options = {}) {
    ...
    if (!normalizedOptions.adminRequest || normalizedOptions.useCatalogue) {
        const catalogueItem = getCatalogueItemForStructureTypeAndDuration(structure && structure.typeID, durationSeconds);
        if (!catalogueItem) {
            return {
                success: false,
...
function issueLicenses(characterID, paintwork, durationSeconds, structureIDs = [], options = {}) {
    ...
    const hasAdminPrivileges = normalizedOptions.adminRequest && hasStructurePaintworkAdminPrivileges(characterID);
    ...
    const requestedDurationSeconds = Math.max(0, Math.trunc(Number(durationSeconds || 0) || 0));
    ...
    const ownedStructureIDs = new Set(listOwnedStructures(corporationID).map((structure) => Number(structure.structureID || 0) || 0));
    ...
    for (const structureID of requestedStructureIDs) {
        ...
        const priceResult = resolvePerStructureIssuePrice(structure, requestedDurationSeconds, normalizedOptions);
    ...
    const totalPriceAmount = issues.reduce((total, entry) => total + Math.max(0, Number(entry.priceAmount || 0) || 0), 0);
    if (getCorporationWalletLPBalance(corporationID, HERALDRY_CORPORATION_ID) <
        totalPriceAmount) {
        return {
            success: false,
    ...
    if (totalPriceAmount > 0) {
        const debitResult = adjustCorporationWalletLPBalance(corporationID, HERALDRY_CORPORATION_ID, -totalPriceAmount, { reason: "structure_paintwork_issue" });
    ...
    nextRoot.structureAssignments = Object.fromEntries(Object.values(nextRoot.licensesByID || {}).map((license) => [
    ...
    for (const entry of issues) {
        ...
        const existing = Object.values(nextRoot.licensesByID || {}).find((license) => Number(license && license.structureID) === Number(structure.structureID));
        if (existing) {
    ...
    if (!writeRoot(nextRoot)) {
        if (debitApplied) {
            adjustCorporationWalletLPBalance(corporationID, HERALDRY_CORPORATION_ID, totalPriceAmount, { reason: "structure_paintwork_issue_rollback" });
        }
        return {
            success: false,
    ...
    if (!normalizedOptions.adminRequest) {
        createStructurePaintPurchasedNotifications(characterID, corporationID, issued, totalPriceAmount);
    }
    return {
        success: true,
        data: issued,
...
//# sourceMappingURL=structurePaintworkState.js.map