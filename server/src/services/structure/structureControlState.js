...
function getSessionStructureID(session) {
    return normalizePositiveInt(session && (session.structureID || session.structureid), 0);
}
function getSessionShipID(session) {
    return normalizePositiveInt(session && (session.shipID || session.shipid), 0);
}
...
function getSessionLocationID(session) {
    return normalizePositiveInt(session && (session.locationID || session.locationid), 0);
}
function getSessionSolarSystemID(session, options = {}) {
    return normalizePositiveInt(options.solarSystemID ||
...
function getStructurePilotSession(structureID, options = {}) {
    ...
    return sessionRegistry
        .getSessions()
        .find((session) => (session !== excludedSession &&
        isControllingStructureSession(session, targetStructureID))) || null;
}
function getStructurePilotCharacterID(structureID, options = {}) {
    ...
    return normalizePositiveInt(controllerSession && (controllerSession.characterID || controllerSession.charid), 0) || null;
}
function getRestorableShipID(session) {
    const storedShipID = normalizePositiveInt(session && session._structureControlPreviousShipID, 0);
    if (storedShipID) {
        return storedShipID;
    }
...
function sendStructureControlShipCosmeticsChanged(session, shipID) {
    const normalizedShipID = normalizePositiveInt(shipID, 0);
    if (!normalizedShipID ||
        !session ||
        typeof session.sendNotification !== "function") {
        return false;
    }
    session.sendNotification("OnShipCosmeticsChanged", "charid", [
        normalizedShipID,
        { type: "dict", entries: [] },
    ]);
    return true;
...
function queueStructureFightersForControlRelease(session, structureID, options = {}) {
    ...
    try {
        ...
        const scene = typeof spaceRuntime.ensureScene === "function"
        ...
        if (typeof scene.syncStructureEntitiesFromState === "function" &&
        ...
        return fighterRuntime.queueControlledFightersReturnToTubes(scene, controllerEntity, {
...
function relinquishStructureControl(session, options = {}) {
    ...
    const restoredLocationID = getSessionLocationID(session) || null;
    const restoredShipID = normalizePositiveInt(options.restoreShipID, 0) ||
    ...
    sendStructureControlShipCosmeticsChanged(session, restoredShipID);
    return {
        success: true,
        data: {
            changed: true,
...
function boardStructureControlFromSpace(session, structureID, options = {}) {
    ...
    const controlLocationID = structureSolarSystemID ||
        currentSolarSystemID ||
        oldLocationID ||
        null;
    const previousShipID = normalizePositiveInt(options.previousShipID, 0) ||
    ...
    const previousControllerCharacterID = normalizePositiveInt(previousController && (previousController.characterID ||
        previousController.charid), 0) || null;
...
function ejectFromStructureControl(session, options = {}) {
    ...
    const restoredShipID = normalizePositiveInt(options.restoreShipID, 0) ||
        getRestorableShipID(session) ||
        null;
    const restoredLocationID = getSessionSolarSystemID(session) ||
...
function assumeStructureControl(session, structureID, options = {}) {
    ...
    if (isControllingStructureSession(session, targetStructureID)) {
        return {
            ...
            data: {
                changed: false,
                structureID: targetStructureID,
                previousControllerCharacterID: getStructurePilotCharacterID(targetStructureID, { excludeSession: session }),
            },
        };
    }
    const currentShipID = getSessionShipID(session);
    const previousShipID = currentShipID && currentShipID !== targetStructureID
        ? currentShipID
        : getRestorableShipID(session);
    if (previousShipID) {
        session._structureControlPreviousShipID = previousShipID;
    ...
    const controlLocationID = currentLocationID || null;
    delete session._structureControlPreviousLocationID;
    const previousController = getStructurePilotSession(targetStructureID, {
        excludeSession: session,
    });
    ...
    applySessionShipID(session, targetStructureID);
    // TQ changes only shipid here. A docked Upwell session already has the
    // solar system as locationid, and taking control must preserve it so the
    // dogma loader continues to resolve the structure in that solar system.
    const changes = {};
    if ((currentShipID || null) !== targetStructureID) {
        changes.shipid = [currentShipID || null, targetStructureID];
    }
    sendStructureControlSessionChange(session, changes);
    sendStructureControlShipCosmeticsChanged(session, previousShipID);
    return {
        success: true,
        data: {
            changed: true,
...
//# sourceMappingURL=structureControlState.js.map