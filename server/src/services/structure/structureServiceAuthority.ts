...
function buildServicesFromAccessSettings() {
  const bySettingID: Record<string, any> = {};
...
function getConcreteServiceIDsForMetaService(serviceID) {
  ...
  let serviceIDs: readonly number[] = [];
...
function getActivityIDForStructureService(serviceID, industryActivity: Record<string, any> = {}) {
  const normalizedServiceID = toPositiveInt(serviceID, 0);
...
function getIndustryActivityServiceIDs(activityID, industryActivity: Record<string, any> = {}) {
...
function getIndustryServiceID(activityID, productGroupID, industryActivity: Record<string, any> = {}) {
  const normalizedActivityID = toPositiveInt(activityID, 0);
