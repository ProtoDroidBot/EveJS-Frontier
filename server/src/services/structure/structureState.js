...
const { listContainerItems, removeInventoryItem, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
...
const { normalizeRoleValue } = require(path.join(__dirname, "../account/accountRoleProfiles"));
const structureAssetSafetyState = require(path.join(__dirname, "./structureAssetSafetyState"));
const structureDockedRecoveryState = require(path.join(__dirname, "./structureDockedRecoveryState"));
const structureDestructionLootState = require(path.join(__dirname, "./structureDestructionLootState"));
const structureFlexFuelRuntime = require(path.join(__dirname, "./structureFlexFuelRuntime"));
const structureAutoMoonFuelRuntime = require(path.join(__dirname, "./structureAutoMoonFuelRuntime"));
const { getAllianceOwnerRecord, getCharacterIDsInCorporation, getCorporationOwnerRecord, } = require(path.join(__dirname, "../corporation/corporationState"));
const { NOTIFICATION_GROUP, NOTIFICATION_TYPE, } = require(path.join(__dirname, "../notifications/notificationConstants"));
const { createNotification, } = require(path.join(__dirname, "../notifications/notificationState"));
const { resolveStructureEffectiveHitpoints, } = require(path.join(__dirname, "./structureFullPowerDogma"));
const { cancelIndustryJobsForStructureLifecycle, syncIndustryJobsForServiceStateTransition, } = require(path.join(__dirname, "./structureIndustryJobs"));
const { MANAGED_SERVICE_IDS, SERVICE_IDS_BY_MODULE_TYPE_ID, isStructureServiceModuleType, } = require(path.join(__dirname, "./structureServiceAuthority"));
const { STRUCTURE_STATE, STRUCTURE_STATE_NAME_BY_ID, STRUCTURE_STATE_ID_BY_NAME, STRUCTURE_DISABLED_STATES, STRUCTURE_TETHER_ENABLED_STATES, STRUCTURE_VULNERABLE_STATES, STRUCTURE_UPKEEP_STATE, STRUCTURE_UPKEEP_NAME_BY_ID, STRUCTURE_UPKEEP_ID_BY_NAME, STRUCTURE_SERVICE_ID, STRUCTURE_SERVICE_STATE, STRUCTURE_SIZE, STRUCTURE_FAMILY, STRUCTURE_GROUP_ID, STRUCTURE_TIMER_SECONDS, STRUCTURE_UNANCHOR_CANCEL_STATES, STRUCTURE_REPAIR_SECONDS_BY_STATE, DEFAULT_REINFORCE_WEEKDAY, DEFAULT_REINFORCE_HOUR, DEFAULT_STRUCTURE_RADIUS, DEFAULT_STRUCTURE_TETHER_RANGE, NEXT_STRUCTURE_ID_START, NEXT_ASSET_WRAP_ID_START, STRUCTURE_TYPE_PRESETS, TATARA_EXCLUDED_DOCK_GROUP_NAMES, ONE_WAY_UNDOCK_TYPE_IDS, getAllowedServicesForStructureType, } = require(path.join(__dirname, "./structureConstants"));
...
const MANAGED_SERVICE_MODULE_TYPE_IDS_BY_SERVICE_ID = Object.freeze(Object.entries(SERVICE_IDS_BY_MODULE_TYPE_ID).reduce((accumulator, [typeID, serviceIDs]) => {
...
function normalizeConditionState(value) {
    ...
    const conditionFloat = (raw, fallback) => raw === undefined || raw === null ? fallback : toFloat(raw, fallback);
...
function normalizeStructureDescription(entry = {}) {
    const source = entry.description !== undefined && entry.description !== null
        ? entry.description
...
function getSolarSystemsByID() {
    ...
    solarCache = new Map(readStaticRows(TABLE.SOLAR_SYSTEMS)
...
function buildImpendingAbandonNotificationData(structure, isCorpOwned, daysUntilAbandon) {
    ...
    const structureName = String((structure && (structure.itemName || structure.name)) || `<Structure ${structureID}>`);
    return {
        structureID,
...
function maybeCreateImpendingAbandonNotifications(structure, nowMs) {
    ...
    const notifiedForCurrentTimer = toPositiveInt(structure.abandonmentWarningAbandonAt, 0) === abandonAt;
    const notifiedKeys = notifiedForCurrentTimer && Array.isArray(structure.abandonmentWarningRecipients)
        ...
        : new Set();
    const daysUntilAbandon = Math.max(1, Math.ceil(remainingMs / ABANDONED_WARNING_DAY_MS));
    ...
    for (const target of collectImpendingAbandonNotificationTargets(structure)) {
        ...
        const result = createNotification(target.characterID, {
            ...
            processed: false,
            data: buildImpendingAbandonNotificationData(structure, target.isCorpOwned, daysUntilAbandon),
            emitLive: false,
        });
...
function normalizeStructureTypeRecord(entry = {}) {
    ...
    const defaultQuantumCoreTypeID = toPositiveInt(entry.defaultQuantumCoreTypeID, (preset && preset.defaultQuantumCoreTypeID) || 0) || null;
    return {
        typeID,
        ...
        structureSize: size,
        radius: Math.max(DEFAULT_STRUCTURE_RADIUS, toFloat(entry.radius, getTypeAttributeValue(typeID, "radius")) || DEFAULT_STRUCTURE_RADIUS),
        ...
        capacitorCapacity: Math.max(0, toFloat(entry.capacitorCapacity, getTypeAttributeValue(typeID, "capacitorCapacity"))),
        ...
        tetheringRange: Math.max(DEFAULT_STRUCTURE_TETHER_RANGE, toFloat(entry.tetheringRange, getTypeAttributeValue(typeID, "tetheringRange")) || DEFAULT_STRUCTURE_TETHER_RANGE),
        ...
        tetherRepairEffectMode: entry.tetherRepairEffectMode === undefined ||
...
function ensureStructureTypes() {
    ...
    if (rows.length === 0) {
        ...
        if (!writeResult.success) {
            log.warn(`[StructureState] Failed to persist structureTypes bootstrap: ${writeResult.errorMsg}`);
        }
    }
    ...
    if (rowsChanged) {
        ...
        if (!writeResult.success) {
            log.warn(`[StructureState] Failed to persist required sovereignty structure types: ${writeResult.errorMsg}`);
        }
    }
    typeCache = {
        rows,
...
function normalizeStructureRecord(entry = {}) {
    ...
    return {
        ...
        quantumCoreItemTypeID: toPositiveInt(entry.quantumCoreItemTypeID, typeRecord.defaultQuantumCoreTypeID || 0) || null,
        reinforceWeekday,
        reinforceHour,
        ...
        fuelExpiresAt: entry.fuelExpiresAt === undefined || entry.fuelExpiresAt === null
            ? null
        ...
        unanchoring: entry.unanchoring === undefined || entry.unanchoring === null
            ? null
            : toInt(entry.unanchoring, 0),
        abandonAt: entry.abandonAt === undefined || entry.abandonAt === null
            ? null
            : toInt(entry.abandonAt, 0),
        abandonmentWarningAbandonAt: entry.abandonmentWarningAbandonAt === undefined ||
...
function ensureStructureCache() {
    ...
    structureCache = {
        meta: {
            nextStructureID: Math.max(NEXT_STRUCTURE_ID_START, toPositiveInt(payload._meta && payload._meta.nextStructureID, NEXT_STRUCTURE_ID_START)),
...
function persistStructures(rows, metaOverrides = {}, options = {}) {
    ...
    const nextStructureID = Math.max(NEXT_STRUCTURE_ID_START, ...normalizedRows.map((entry) => toPositiveInt(entry && entry.structureID, 0) + 1), NEXT_STRUCTURE_ID_START);
    const nextMeta = {
...
function buildStructureChangeSystemIDs(previousRows = [], nextRows = []) {
    const previousByID = new Map((Array.isArray(previousRows) ? previousRows : [])
        .map((entry) => [toPositiveInt(entry && entry.structureID, 0), entry])
        .filter(([structureID]) => structureID > 0));
    const nextByID = new Map((Array.isArray(nextRows) ? nextRows : [])
        .map((entry) => [toPositiveInt(entry && entry.structureID, 0), entry])
        .filter(([structureID]) => structureID > 0));
...
function maybeBroadcastCynoJammerStateChanges(previousRows = [], nextRows = []) {
    ...
    try {
        const { broadcastCynoJammerChangesForStructureRows, } = lazyRequire("../sovereignty/sovSuppressionState");
...
function endWarsForLostWarHQLazily(structure, options = {}) {
    try {
        const { endWarsForLostWarHQ, } = lazyRequire("../corporation/warRuntimeState");
        ...
        log.warn(`[StructureState] Failed to end War HQ wars for ` +
...
function getStructureOwnerCorpID(structure) {
    return toPositiveInt(structure && (structure.ownerCorpID || structure.ownerID), 0);
}
...
function createStructureOwnerNotifications(structure, typeID, dataOverrides = {}) {
    ...
    for (const characterID of collectStructureOwnerNotificationTargets(structure)) {
        ...
        if (!result || result.success !== true) {
            log.warn(`[StructureState] Failed to create owner notification ` +
...
function ownershipTransferMetadataMatchesOwners(source, oldOwnerCorpID, newOwnerCorpID) {
    return (ownershipTransferMetadataMatchesCorpID(source, oldOwnerCorpID, [
        "oldOwnerCorpID",
        "fromOwnerCorpID",
        "previousOwnerCorpID",
        "ownerTransferOldOwnerCorpID",
    ]) &&
        ownershipTransferMetadataMatchesCorpID(source, newOwnerCorpID, [
            "newOwnerCorpID",
            "toOwnerCorpID",
            "nextOwnerCorpID",
...
function resolveStructureOwnershipTransferActorID(next, oldOwnerCorpID, newOwnerCorpID) {
    ...
    for (const source of sources) {
        ...
        if (characterID &&
            ownershipTransferMetadataMatchesOwners(source, oldOwnerCorpID, newOwnerCorpID)) {
            return characterID;
        }
    }
    return 0;
...
function createStructureOwnershipTransferredNotifications(previous, next) {
    ...
    const charID = resolveStructureOwnershipTransferActorID(next, oldOwnerCorpID, newOwnerCorpID);
    if (!charID) {
        return;
    }
    ...
    const data = {
        ...
        oldOwnerCorpID,
        charID,
        structureName: String((next && (next.name || next.itemName)) ||
    ...
    for (const characterID of collectStructureCorporationNotificationTargets([
        ...
        if (!result || result.success !== true) {
            log.warn(`[StructureState] Failed to create ownership-transfer notification ` +
...
function createStructureAnchoringNotifications(structure) {
    const ownerData = resolveStructureOwnerCorpNotificationData(structure, "anchoring");
    if (!ownerData) {
        return;
    }
    ...
    createStructureOwnerNotifications(structure, NOTIFICATION_TYPE.STRUCTURE_ANCHORING, {
        ...ownerData,
        timeLeft: durationMsToLong(Math.max(0, stateEndsAt - stateStartedAt)),
        vulnerableTime: durationMsToLong(scaledTimerMs(STRUCTURE_REPAIR_SECONDS_BY_STATE[STRUCTURE_STATE.ONLINING_VULNERABLE], structure)),
    });
}
function createStructureUnanchoringNotifications(structure, nowMs = Date.now()) {
    const ownerData = resolveStructureOwnerCorpNotificationData(structure, "unanchoring");
    if (!ownerData) {
        return;
    }
    ...
    createStructureOwnerNotifications(structure, NOTIFICATION_TYPE.STRUCTURE_UNANCHORING, {
        ...ownerData,
...
function createStructureOnlineNotifications(structure, requiresDeedTypeID = null) {
    ...
    createStructureOwnerNotifications(structure, NOTIFICATION_TYPE.STRUCTURE_ONLINE, deedTypeID ? { requiresDeedTypeID: deedTypeID } : {});
}
...
function createStructureReinforcementLossNotifications(structure, typeID) {
    ...
    createStructureOwnerNotifications(structure, typeID, {
        ...
        vulnerableTime: durationMsToLong(scaledTimerMs(STRUCTURE_REPAIR_SECONDS_BY_STATE[vulnerableState], structure)),
    });
}
function createStructureFuelAlertNotifications(structure, listOfTypesAndQty) {
    ...
    createStructureOwnerNotifications(structure, NOTIFICATION_TYPE.STRUCTURE_FUEL_ALERT, { listOfTypesAndQty: fuelList });
}
function normalizeStructureAttackContext(options = {}) {
    const source = options && typeof options === "object"
        ...
        : {};
    const aggressorID = toPositiveInt(source && (source.aggressorID ||
        source.characterID ||
        source.charid ||
        source.charID ||
        source.pilotCharacterID), 0);
    const aggressorCorpID = toPositiveInt(source && (source.aggressorCorpID ||
        source.corporationID ||
        source.corpid ||
        source.corpID ||
        source.ownerID), 0);
    const aggressorAllianceID = toPositiveInt(source && (source.aggressorAllianceID ||
        source.allianceID ||
        source.allianceid), 0) || null;
...
function shouldNotifyStructureUnderAttack(previousStructure, nextStructure, damageResult, nowMs, options = {}) {
    ...
    const previousNotificationActiveUntil = toPositiveInt(previousStructure &&
        previousStructure.devFlags &&
        previousStructure.devFlags.underAttackNotificationActiveUntilMs, 0);
...
function buildStructureUnderAttackNotificationData(structure, attackContext) {
    ...
    const aggressorAllianceID = toPositiveInt(attackContext && attackContext.aggressorAllianceID, 0);
...
function createStructureUnderAttackNotifications(structure, attackContext) {
    ...
    if (!data) {
        log.warn(`[StructureState] Skipping under-attack notification for ` +
            `structure=${structure && structure.structureID} aggressorCorp=${attackContext.aggressorCorpID}: ` +
            `owner record missing`);
        return;
    }
    createStructureOwnerNotifications(structure, NOTIFICATION_TYPE.STRUCTURE_UNDER_ATTACK, data);
}
function createStructureReagentAlertNotifications(structure, typeID) {
    const notificationTypeID = toPositiveInt(typeID, 0);
    if (notificationTypeID !== NOTIFICATION_TYPE.STRUCTURE_LOW_REAGENTS_ALERT &&
...
function resolveAutoMoonReagentAlertType(fuelResult) {
    const requiredMagmaticGas = toPositiveInt(fuelResult && fuelResult.requiredMagmaticGas, 0);
    if (!requiredMagmaticGas) {
        return null;
    }
    const availableMagmaticGas = toInt(fuelResult && fuelResult.availableMagmaticGas, 0);
...
function resolveAutoMoonFuelAlertTypesAndQty(fuelResult) {
    if (toPositiveInt(fuelResult && fuelResult.requiredFuelBlocks, 0) <=
        toInt(fuelResult && fuelResult.availableFuelBlocks, 0)) {
        return [];
    }
    const reagentTypeID = toPositiveInt(structureAutoMoonFuelRuntime.TYPE_COLONY_REAGENT_LAVA, 0);
...
function createStructureDestroyedNotifications(structure) {
    ...
    const ownerCorpName = ownerRecord && ownerRecord.ownerName
        ? String(ownerRecord.ownerName)
        : `Corporation ${ownerCorpID}`;
    createStructureOwnerNotifications(structure, NOTIFICATION_TYPE.STRUCTURE_DESTROYED, {
        ownerCorpName,
        ownerCorpLinkData: [
            "showinfo",
            toPositiveInt(ownerRecord && ownerRecord.typeID, TYPE_CORPORATION) ||
        ...
        isAbandoned: toInt(structure && structure.upkeepState, 0) ===
            STRUCTURE_UPKEEP_STATE.ABANDONED,
    });
}
function serviceStateIsOnline(serviceStates, serviceID) {
    return (toInt(serviceStates && serviceStates[String(serviceID)], STRUCTURE_SERVICE_STATE.OFFLINE) === STRUCTURE_SERVICE_STATE.ONLINE);
}
function resolveOfflineServiceModuleTypeIDs(structure, offlineServiceIDs) {
    const serviceIDSet = new Set((Array.isArray(offlineServiceIDs) ? offlineServiceIDs : [])
    ...
    if (structureID > 0) {
        for (const item of listContainerItems(null, structureID, null)) {
            ...
            if (isStructureServiceSlotFlag(item && item.flagID) &&
                typeID > 0 &&
...
function collectOfflineServiceIDs(previous, next) {
    ...
    for (const serviceID of MANAGED_SERVICE_IDS) {
        if (serviceStateIsOnline(previous && previous.serviceStates, serviceID) &&
...
function createStructureServicesOfflineNotifications(previous, next) {
    ...
    createStructureOwnerNotifications(next, NOTIFICATION_TYPE.STRUCTURE_SERVICES_OFFLINE, { listOfServiceModuleIDs });
}
function cancelStructureMarketOrdersLazily(structure, reason) {
    ...
    const task = Promise.resolve()
        .then(async () => {
        const { cancelStructureMarketOrdersForServiceLoss, } = lazyRequire("../market/marketProxyService");
        ...
        if (result && result.errorMsg === "MARKET_DAEMON_UNAVAILABLE") {
            log.debug(`[StructureState] Deferred market order cancellation for ` +
                `structure=${structureID} reason=${reason || "serviceLoss"}: ` +
                "market daemon unavailable");
            return;
        }
        const cancelledCount = toPositiveInt(result && result.cancelledCount, 0);
        if (cancelledCount > 0) {
            createStructureOwnerNotifications(structure, NOTIFICATION_TYPE.STRUCTURE_MARKET_ORDERS_CANCELLED);
        }
    })
        .catch((error) => {
        log.warn(`[StructureState] Failed to cancel market orders for ` +
            `structure=${structureID} reason=${reason || "serviceLoss"}: ` +
            `${error.message}`);
    });
...
function createStructureLifecycleTransitionNotifications(previous, next) {
    ...
    if (previousState === STRUCTURE_STATE.ANCHORING &&
    ...
    if (previousState === STRUCTURE_STATE.ONLINING_VULNERABLE &&
...
function maybeCreateStructureTransitionNotifications(previousRows = [], nextRows = []) {
    ...
    for (const next of Array.isArray(nextRows) ? nextRows : []) {
        ...
        if (previousUpkeepState !== nextUpkeepState) {
            if (previousUpkeepState === STRUCTURE_UPKEEP_STATE.FULL_POWER &&
                nextUpkeepState === STRUCTURE_UPKEEP_STATE.LOW_POWER) {
                createStructurePowerStateNotifications(next, NOTIFICATION_TYPE.STRUCTURE_WENT_LOW_POWER);
            }
            else if (previousUpkeepState !== STRUCTURE_UPKEEP_STATE.FULL_POWER &&
                nextUpkeepState === STRUCTURE_UPKEEP_STATE.FULL_POWER) {
                createStructurePowerStateNotifications(next, NOTIFICATION_TYPE.STRUCTURE_WENT_HIGH_POWER);
            }
        }
...
function updateStructureRecord(structureID, updater, options = {}) {
    ...
    if (shouldCancelUnanchoringForState(next, toInt(options.nowMs, Date.now()))) {
        next.unanchoring = null;
    }
    const writeResult = persistStructures(rows.map((entry) => (entry.structureID === targetID ? next : entry)), {}, options);
    if (!writeResult.success) {
        return writeResult;
    }
    return {
...
function upsertStructureRecord(record) {
    ...
    const writeResult = persistStructures(rows, {
        nextStructureID: Math.max(NEXT_STRUCTURE_ID_START, cache.meta && cache.meta.nextStructureID ? cache.meta.nextStructureID : NEXT_STRUCTURE_ID_START, structureID + 1),
    });
    if (!writeResult.success) {
        return writeResult;
    }
...
function setStructureState(structureID, stateNameOrID, options = {}) {
    const numericState = typeof stateNameOrID === "string"
    ...
    return updateStructureRecord(structureID, (current) => ({
        ...
        stateEndsAt: options.clearTimer === true
            ? null
...
function isDockingServiceOnline(structure) {
    ...
    return toInt(structure.serviceStates &&
        structure.serviceStates[String(STRUCTURE_SERVICE_ID.DOCKING)], STRUCTURE_SERVICE_STATE.OFFLINE) === STRUCTURE_SERVICE_STATE.ONLINE;
}
...
function getShipDockClass(shipTypeID) {
    const metadata = resolveShipByTypeID(toPositiveInt(shipTypeID, 0)) ||
...
function canShipTypeDockAtStructure(shipTypeID, structure) {
    ...
    if (typeRecord.structureSize === STRUCTURE_SIZE.MEDIUM &&
...
function resolveStructureUnanchoringSeconds(structure) {
    ...
    const groupID = toPositiveInt(typeRecord && typeRecord.groupID, toPositiveInt(structure && structure.groupID, 0));
    const family = String((typeRecord && typeRecord.structureFamily) ||
...
function isStructureUnanchoringCompletionDue(structure, nowMs = Date.now()) {
    const unanchoringAt = Number(structure && structure.unanchoring);
    return Boolean(Number.isFinite(unanchoringAt) &&
        unanchoringAt > 0 &&
        toInt(nowMs, Date.now()) >= unanchoringAt);
}
function shouldCancelUnanchoringForState(structure, nowMs = Date.now()) {
    return Boolean(structure &&
...
function isStructureRepairLayerDamaged(structure) {
    ...
    if (state === STRUCTURE_STATE.HULL_VULNERABLE ||
...
function applyStructureRepairTimerForDamage(structure, beforeLayers, afterLayers, nowMs) {
    ...
    const repairSeconds = toFloat(STRUCTURE_REPAIR_SECONDS_BY_STATE[toInt(structure && structure.state, 0)], 0);
    if (repairSeconds <= 0) {
        return structure;
    }
...
function buildOnliningVulnerableStructure(structure, nowMs) {
    ...
    return {
        ...
        stateStartedAt: nowMs,
        stateEndsAt: hasCore
            ? nowMs + scaledTimerMs(STRUCTURE_REPAIR_SECONDS_BY_STATE[STRUCTURE_STATE.ONLINING_VULNERABLE], structure)
            : null,
    };
}
function maybeAdvanceStructureState(structure, nowMs = Date.now()) {
    ...
    if (structure.state === STRUCTURE_STATE.ARMOR_REINFORCE || structure.state === STRUCTURE_STATE.HULL_REINFORCE) {
        const nextState = structure.state === STRUCTURE_STATE.ARMOR_REINFORCE
        ...
        return {
            structure: {
                ...
                state: nextState,
                stateStartedAt: nowMs,
                stateEndsAt: nowMs + scaledTimerMs(STRUCTURE_REPAIR_SECONDS_BY_STATE[nextState], structure),
            },
            changed: true,
        };
    }
...
function tickStructures(nowMs = Date.now()) {
    ...
    for (const structure of cache.rows) {
        ...
        const flexFuelTick = structureFlexFuelRuntime.applyFlexServiceFuelCycle(current, nowMs, { structureProximityCandidates: cache.rows });
        ...
        const autoMoonFuelTick = structureAutoMoonFuelRuntime.applyAutoMoonMinerFuelCycle(current, nowMs);
        current = autoMoonFuelTick.structure;
        if (autoMoonFuelTick &&
            autoMoonFuelTick.fuelResult &&
            autoMoonFuelTick.fuelResult.success !== true) {
            const reagentAlertType = resolveAutoMoonReagentAlertType(autoMoonFuelTick.fuelResult);
            if (reagentAlertType) {
            ...
            const fuelAlertTypesAndQty = resolveAutoMoonFuelAlertTypesAndQty(autoMoonFuelTick.fuelResult);
        ...
        if (hasAbandonTimer) {
            if (currentUpkeepState !== STRUCTURE_UPKEEP_STATE.LOW_POWER ||
        ...
        else if (currentUpkeepState === STRUCTURE_UPKEEP_STATE.LOW_POWER &&
            structureAllowsAbandonment(current)) {
        ...
        if (!current.destroyedAt &&
            ...
            const recoveryResult = structureDockedRecoveryState.evacuateDockedCharactersFromStructure(current, { nowMs: toInt(nowMs, Date.now()) });
            if (!recoveryResult.success) {
                nextRows.push(current);
                log.warn(`[StructureState] Failed to finalize unanchoring for ${current.structureID}: ${recoveryResult.errorMsg}`);
                continue;
            }
            const assetHandoffResult = structureAssetSafetyState.handleStructureUnanchored(current, { nowMs: toInt(nowMs, Date.now()), session: null });
            if (!assetHandoffResult.success) {
                nextRows.push(current);
                log.warn(`[StructureState] Failed to hand off assets while finalizing unanchoring for ${current.structureID}: ${assetHandoffResult.errorMsg}`);
                continue;
            }
            const remainingTopLevelItems = listTopLevelItemsInStructure(current.structureID);
            if (remainingTopLevelItems.length > 0) {
                nextRows.push(current);
                log.warn(`[StructureState] Delaying unanchoring completion for ${current.structureID}: structure still has ${remainingTopLevelItems.length} top-level item(s)`);
                continue;
            }
    ...
    if (changed) {
        ...
        if (!writeResult.success) {
            ...
            for (const alert of pendingFuelAlerts) {
                createStructureFuelAlertNotifications(alert.structure, alert.listOfTypesAndQty);
            }
...
function startAnchoring(structureID, nowMs = Date.now(), options = {}) {
    return updateStructureRecord(structureID, (current) => ({
        ...
        state: STRUCTURE_STATE.ANCHOR_VULNERABLE,
        stateStartedAt: nowMs,
        stateEndsAt: nowMs + scaledTimerMs(STRUCTURE_REPAIR_SECONDS_BY_STATE[STRUCTURE_STATE.ANCHOR_VULNERABLE], current),
        destroyedAt: null,
        conditionState: normalizeConditionState({
            damage: 0,
            armorDamage: 0,
...
function startStructureUnanchoring(structureID, nowMs = Date.now()) {
    ...
    const updateResult = updateStructureRecord(structureID, (record) => ({
        ...record,
        unanchoring: nowMs + scaledTimerMs(resolveStructureUnanchoringSeconds(record), record),
    }));
    if (updateResult && updateResult.success === true) {
...
function removeJumpClonesForStructureLifecycle(structure, reason, options = {}) {
    ...
    try {
        const { removeJumpClonesAtStructure, } = lazyRequire("../station/jumpCloneRuntime");
        const result = removeJumpClonesAtStructure(structureID, {
            reason,
            destroyerID: toPositiveInt(options.destroyerID, 0) ||
                toPositiveInt(options.session && (options.session.characterID ||
                    options.session.charid), 0) ||
                toPositiveInt(structure.ownerID || structure.ownerCorpID, 0),
        });
        if (result && result.success === false) {
            log.warn(`[StructureState] Jump clone cleanup failed structure=${structureID} reason=${reason} error=${result.errorMsg || "UNKNOWN"}`);
        }
        return result;
    }
    catch (error) {
        log.warn(`[StructureState] Jump clone cleanup threw structure=${structureID} reason=${reason} error=${error.message}`);
        return {
            success: false,
...
function setStructureQuantumCoreInstalled(structureID, installed, nowMs = Date.now()) {
    const hasActiveTimer = (current) => (current &&
    ...
    return updateStructureRecord(structureID, (current) => {
        const shouldStartOnliningRepair = installed === true &&
        ...
        return {
            ...
            stateEndsAt: shouldStartOnliningRepair
                ? nowMs + scaledTimerMs(STRUCTURE_REPAIR_SECONDS_BY_STATE[STRUCTURE_STATE.ONLINING_VULNERABLE], current)
...
function setStructureUpkeepState(structureID, upkeepState) {
    const numericUpkeepState = typeof upkeepState === "string"
...
function setStructureServiceState(structureID, serviceID, serviceState, options = {}) {
    ...
    const autoMoonServiceTransition = structureAutoMoonFuelRuntime
        .prepareAutoMoonMinerServiceStateTransition(beforeStructure, numericServiceID, serviceState, options);
    ...
    const flexServiceTransition = structureFlexFuelRuntime.prepareFlexServiceStateTransition(beforeStructure, numericServiceID, serviceState, {
        ...options,
        structureProximityCandidates: listStructuresForSystem(beforeStructure.solarSystemID, { includeDestroyed: true, refresh: false }),
    });
    ...
    const updateResult = updateStructureRecord(structureID, (current) => ({
        ...
        serviceStates: {
            ...
            [String(numericServiceID)]: toInt(serviceState, STRUCTURE_SERVICE_STATE.OFFLINE) === STRUCTURE_SERVICE_STATE.ONLINE
                ? STRUCTURE_SERVICE_STATE.ONLINE
                : STRUCTURE_SERVICE_STATE.OFFLINE,
        },
    }));
    ...
    return {
        ...updateResult,
        fuelResult: autoMoonServiceTransition.fuelResult ||
            flexServiceTransition.fuelResult ||
            null,
        industryJobSync: syncIndustryJobsForServiceStateTransition(structureID, beforeStructure && beforeStructure.serviceStates, updateResult.data && updateResult.data.serviceStates),
    };
}
    ...
    return updateStructureRecord(structureID, (current) => ({
        ...
        devFlags: (current &&
            ...
            ? {
                ...
                flexFuelState: {
                    ...current.devFlags.flexFuelState,
                    cynoJamActivatesAt: toInt(current.devFlags.flexFuelState.cynoJamActivatesAt, 0) - deltaMs,
                },
            }
            : current.devFlags,
    }));
...
function removeStructure(structureID, options = {}) {
    ...
    const recoveryResult = structureDockedRecoveryState.evacuateDockedCharactersFromStructure(existing, {
    ...
    const writeResult = persistStructures(rows.filter((entry) => entry.structureID !== targetID));
    if (!writeResult.success) {
        return writeResult;
    }
...
function destroyStructure(structureID, options = {}) {
    ...
    if (!current.destroyedAt) {
        recoveryResult = structureDockedRecoveryState.evacuateDockedCharactersFromStructure(current, {
            nowMs,
        });
        industryJobCancelResult = cancelIndustryJobsForStructureLifecycle(current.structureID, {
        ...
        if (industryJobCancelResult && industryJobCancelResult.success === false) {
            log.warn(`[StructureState] Failed to cancel industry jobs for destroyed structure ${current.structureID}: ${industryJobCancelResult.errorMsg || "UNKNOWN"}`);
            return industryJobCancelResult;
        }
    }
    let assetSafetyDisabled = false;
    if (options.skipAssetSafety !== true && !current.destroyedAt) {
        assetSafetyResult = structureAssetSafetyState.handleStructureDestroyed(current, {
            nowMs,
            session: options.session,
        });
        if (assetSafetyResult && !assetSafetyResult.success) {
            ...
            log.warn(`[StructureState] Asset safety handoff failed for structure ${current.structureID}: ${assetSafetyResult.errorMsg}`);
        }
    }
    if (!current.destroyedAt) {
        lootResult = structureDestructionLootState.handleStructureDestroyedLoot(current, {
            nowMs,
            includeStructureContents: assetSafetyDisabled,
            includeQuantumCore: true,
        });
        jumpCloneCleanupResult = removeJumpClonesForStructureLifecycle(current, "structureDestroyed", options);
    }
    const updateResult = updateStructureRecord(structureID, (currentStructure) => ({
        ...
        stateEndsAt: null,
        hasQuantumCore: false,
        serviceStates: Object.fromEntries(Object.keys(currentStructure.serviceStates || {}).map((serviceID) => [
            String(serviceID),
            STRUCTURE_SERVICE_STATE.OFFLINE,
        ])),
    }));
    ...
    return {
        ...
        data: {
            ...updateResult.data,
            assetSafety: assetSafetyResult && assetSafetyResult.success && assetSafetyResult.data
                ? assetSafetyResult.data
                : null,
            dockedRecovery: recoveryResult && recoveryResult.success && recoveryResult.data
                ? recoveryResult.data
                : null,
            loot: lootResult && lootResult.success && lootResult.data
                ? lootResult.data
                : null,
            jumpCloneCleanup: jumpCloneCleanupResult && jumpCloneCleanupResult.success !== false
                ? jumpCloneCleanupResult
                : null,
            industryJobCancel: industryJobCancelResult && industryJobCancelResult.success !== false
...
function applyStructureDamageTransition(structure, damageResult, nowMs = Date.now()) {
    ...
    if (next.state === STRUCTURE_STATE.SHIELD_VULNERABLE && shieldBroke) {
        const reinforceState = next.structureSize === STRUCTURE_SIZE.MEDIUM ||
        ...
        const reinforcedStructure = {
            ...
            stateStartedAt: nowMs,
            stateEndsAt: nowMs + scaledTimerMs(reinforceState === STRUCTURE_STATE.ARMOR_REINFORCE
                ...
                : resolveSecurityBand(next) === "high"
                    ...
                    : STRUCTURE_TIMER_SECONDS.HULL_REINFORCE_NULL_LOW, next),
            conditionState: normalizeConditionState(reinforceState === STRUCTURE_STATE.ARMOR_REINFORCE
                ? { damage: 0, armorDamage: 0, shieldCharge: 0 }
                : { damage: 0, armorDamage: 1, shieldCharge: 0 }),
        };
        return {
            structure: reinforceState === STRUCTURE_STATE.HULL_REINFORCE
    ...
    if (next.state === STRUCTURE_STATE.ARMOR_VULNERABLE && armorBroke) {
        if ((next.structureSize === STRUCTURE_SIZE.LARGE || next.structureSize === STRUCTURE_SIZE.EXTRA_LARGE) &&
            next.upkeepState === STRUCTURE_UPKEEP_STATE.FULL_POWER) {
            const reinforcedStructure = {
                ...next,
                state: STRUCTURE_STATE.HULL_REINFORCE,
                stateStartedAt: nowMs,
                stateEndsAt: nowMs + scaledTimerMs(resolveSecurityBand(next) === "high"
                    ? STRUCTURE_TIMER_SECONDS.HULL_REINFORCE_HIGH
                    : STRUCTURE_TIMER_SECONDS.HULL_REINFORCE_NULL_LOW, next),
        ...
        return {
            structure: {
                ...
                stateStartedAt: nowMs,
                stateEndsAt: nowMs + scaledTimerMs(STRUCTURE_REPAIR_SECONDS_BY_STATE[STRUCTURE_STATE.HULL_VULNERABLE], next),
            },
    ...
    const repairTimedStructure = applyStructureRepairTimerForDamage(next, before, after, nowMs);
    return {
...
function applyRuntimeStructureDamage(structureID, damageResult, nowMs = Date.now(), options = {}) {
    ...
    const lifecycleNowMs = toInt(options.lifecycleNowMs, Date.now());
    const transition = applyStructureDamageTransition(structure, damageResult, nowMs);
    const underAttackNotification = shouldNotifyStructureUnderAttack(structure, transition && transition.structure, damageResult, nowMs, options);
    ...
    const enteringHullReinforce = transition &&
    ...
    if (previousState === STRUCTURE_STATE.SHIELD_VULNERABLE &&
        (nextState === STRUCTURE_STATE.ARMOR_REINFORCE ||
            nextState === STRUCTURE_STATE.HULL_REINFORCE)) {
        createStructureReinforcementLossNotifications(updateResult.data, NOTIFICATION_TYPE.STRUCTURE_LOST_SHIELDS);
    }
    else if (previousState === STRUCTURE_STATE.ARMOR_VULNERABLE &&
        nextState === STRUCTURE_STATE.HULL_REINFORCE) {
        createStructureReinforcementLossNotifications(updateResult.data, NOTIFICATION_TYPE.STRUCTURE_LOST_ARMOR);
    }
    ...
    const industryJobSync = syncIndustryJobsForServiceStateTransition(structureID, structure.serviceStates, updateResult.data && updateResult.data.serviceStates);
    return {
        success: true,
        data: {
...
function applyAdminStructureDamage(structureID, layerToken, amount = null, options = {}) {
    ...
    const result = applyRuntimeStructureDamage(structureID, damageResult, toInt(options.nowMs, Date.now()), options);
    if (!result.success) {
        return result;
    }
    if (result.data &&
...
function getStructureServices(structure) {
    return Object.fromEntries(Object.entries(structure && structure.serviceStates || {})
...
function listDockableStructuresForCharacter(session, options = {}) {
    const solarSystemID = options.solarSystemID === undefined || options.solarSystemID === null
        ? null
...
//# sourceMappingURL=structureState.js.map