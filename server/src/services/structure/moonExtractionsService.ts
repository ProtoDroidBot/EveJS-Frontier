...
function validateMoonExtractionMutationAccess(session, structureID, options: Record<string, any> = {}) {
  ...
  if (!targetStructureID || !structure) {
    return { success: false as const, errorMsg: "STRUCTURE_CONTROL_DENIED" };
  }
  if (!isControllingStructureSession(session, targetStructureID)) {
    return { success: false as const, errorMsg: "NOT_CONTROLLING_STRUCTURE" };
  }
  ...
  if (!hasOnlineMoonDrillModule(targetStructureID)) {
    return { success: false as const, errorMsg: "NO_ONLINE_MOON_DRILL" };
  }
  if (options.requireStationManager === true && !hasStationManagerRole(session)) {
    return { success: false as const, errorMsg: "STATION_MANAGER_REQUIRED" };
  }
...
function normalizeResourceVolumes(rawVolumes: Record<string, any> = {}) {
  ...
  for (const [rawTypeID, rawVolume] of Object.entries<any>(source)) {
...
function normalizeResourceEntry(rawEntry: Record<string, any> = {}) {
  ...
  for (const [rawTypeID, rawAbundance] of Object.entries<any>(rawResources)) {
    const typeID = normalizePositiveInteger(rawTypeID, 0);
...
function normalizeResourceAmounts(rawAmounts: Record<string, any> = {}) {
  ...
  for (const [rawTypeID, rawAmount] of Object.entries<any>(source)) {
    const typeID = normalizePositiveInteger(rawTypeID, 0);
...
function normalizeExtraction(rawEntry: Record<string, any> = {}) {
  const source = unwrapMarshalValue(rawEntry);
  if (!isPlainObject(source)) {
    return null;
  }
...
function ensureStateShape(state) {
  ...
  const repairedExtractions: Record<string, any> = {};
  for (const rawEntry of Object.values<any>(nextState.extractions)) {
  ...
  const repairedResources: Record<string, any> = {};
  for (const [rawStructureID, rawEntry] of Object.entries<any>(nextState.resourcesByStructureID)) {
...
function getActiveExtractionForStructure(state, structureID) {
  const targetStructureID = normalizePositiveInteger(structureID, 0);
  return Object.values<any>(state.extractions)
...
function buildResourceCompositionPayload(profile) {
  ...
  return buildDict(
    Object.entries<any>(profile.resources)
...
function sendMoonExtractionClearedToSessions(extraction, options: Record<string, any> = {}) {
  const sessions = Array.isArray(options.sessions)
    ? options.sessions
    : sessionRegistry.getSessions();
  const notifiedSessions = new Set<any>();
  let sentCount = 0;
...
function buildMoonMiningNotificationData(extraction, notificationTypeID, options: Record<string, any> = {}) {
...
function createMoonMiningOwnerNotifications(extraction, notificationTypeID, options: Record<string, any> = {}) {
  ...
  let createdCount = 0;
  for (const characterID of [...new Set<any>(getCharacterIDsInCorporation(ownerID))]) {
...
function markReadyMoonExtractionsInState(state, nowMs = Date.now()) {
  ...
  const readyExtractions: any[] = [];
  for (const [rawKey, rawEntry] of Object.entries<any>(state.extractions || {})) {
    const extraction = normalizeExtraction(rawEntry);
    if (
      !extraction ||
      extraction.state !== "active" ||
...
function completeExpiredExtractionsInState(state, nowMs = Date.now()) {
  const currentTimeMs = normalizeNonNegativeInteger(nowMs, Date.now());
  const completedExtractions: any[] = [];
  for (const [rawKey, rawEntry] of Object.entries<any>(state.extractions || {})) {
    const extraction = normalizeExtraction(rawEntry);
...
class MoonExtractionsService extends BaseService {
  ...
  Handle_GetExtractionAndEventIDFromStructureID(args: any[] = [], session) {
    const structureID = normalizePositiveInteger(
  ...
  Handle_GetMoonResourcesForStructure(args: any[] = [], session) {
    const structureID = normalizePositiveInteger(
      Array.isArray(args) && args.length > 0 ? args[0] : getSessionStructureID(session),
      getSessionStructureID(session),
    );
  ...
  Handle_StartNewExtraction(args: any[] = [], session) {
  ...
  Handle_FractureChunkWithStructure(args: any[] = [], session) {
  ...
  Handle_CancelExtraction(args: any[] = [], session) {
    return this._completeExtractionForStructure(
      getSessionStructureID(session),
