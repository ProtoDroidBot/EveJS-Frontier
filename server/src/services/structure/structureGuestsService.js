...
const { getStructureGuestEntries, normalizePositiveInt, seedObserverGuestLedger, } = require(path.join(__dirname, "../_shared/guestLists"));
...
class StructureGuestsService extends BaseService {
    ...
    Handle_GetGuests(args, session) {
        ...
        seedObserverGuestLedger(session, structureID, entries.map(([characterID]) => characterID));
        return buildDict(entries);
    }
}
module.exports = StructureGuestsService;
//# sourceMappingURL=structureGuestsService.js.map