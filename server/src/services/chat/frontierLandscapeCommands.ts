...
function resolveSpawnPosition(shipEntity, placementText = "") {
  ...
  if (!shipPosition) {
    return { success: false as const, errorMsg: "SHIP_POSITION_NOT_FOUND" };
  }
  ...
  if (!placement || placement.startsWith("ahead=")) {
    ...
    if (!Number.isFinite(distance) || distance < 0) {
      return { success: false as const, errorMsg: "INVALID_AHEAD_DISTANCE" };
    }
  ...
  if (placement === "here") {
    return { success: true as const, mode: "here", position: { ...shipPosition } };
  }
  if (placement.startsWith("offset=")) {
    const offset = parseCoordinateVector(placement.slice("offset=".length));
    if (!offset) {
      return { success: false as const, errorMsg: "INVALID_OFFSET" };
    }
    return {
      success: true as const,
      mode: "offset",
  ...
  return { success: false as const, errorMsg: "INVALID_PLACEMENT" };
}
...
function getActiveSceneAndShip(session, runtime) {
  ...
  return { success: true as const, scene, ship };
}
...
function formatEcosystemInspection(ecosystemID, dependencies) {
  ...
  if (!ecosystem) {
    return { success: false as const, errorMsg: "LANDSCAPE_ECOSYSTEM_NOT_FOUND" };
  }
...
function executeFrontierLandscapeCommand(session, argumentText, options: Record<string, any> = {}) {
  ...
  if (action === "inspect") {
    ...
    if (!ecosystemID) {
      return { success: false as const, message: "Usage: /landscape inspect <ecosystemID>" };
    }
  ...
  if (action === "spawn") {
    ...
    if (!ecosystemID) {
      return { success: false as const, message: "Usage: /landscape spawn <ecosystemID> [placement]" };
    }
  ...
  if (action === "remove") {
    ...
    if (!site) {
      return { success: false as const, message: "Custom landscape site was not found." };
    }
