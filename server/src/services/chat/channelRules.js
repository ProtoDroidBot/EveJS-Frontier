...
const { escapeRegExp, getXmppConferenceDomain, } = require("./xmppConfig");
...
function getLocalChatRoomNamePattern() {
    return new RegExp(`^(local|wormhole|triglavian|nullsec|nolocal)_(\\d+)(?:@${escapeRegExp(getXmppConferenceDomain())})?$`, "i");
}
...
function getCurrentSolarSystemID(session) {
    const directSolarSystemID = normalizePositiveInt(session &&
    ...
    const stationID = normalizePositiveInt(session &&
        (session.stationid ??
            session.stationID ??
            session.stationId), 0);
    if (stationID) {
        const station = worldData.getStationByID(stationID);
        const stationSolarSystemID = normalizePositiveInt(station && station.solarSystemID, 0);
        if (stationSolarSystemID) {
            return stationSolarSystemID;
        }
    }
    const structureID = normalizePositiveInt(session &&
        (session.structureid ??
            session.structureID ??
            session.structureId), 0);
    if (structureID) {
        const structure = worldData.getStructureByID(structureID);
        const structureSolarSystemID = normalizePositiveInt(structure && structure.solarSystemID, 0);
...
function isWormholeSolarSystemID(solarSystemID) {
    ...
    return (normalizedSolarSystemID >= WORMHOLE_SYSTEM_MIN &&
...
function isDelayedLocalSolarSystemID(solarSystemID) {
    return (isWormholeSolarSystemID(solarSystemID) ||
...
function getLocalChatCategoryForSolarSystemID(solarSystemID) {
    const normalizedSolarSystemID = normalizePositiveInt(solarSystemID, 0) || DEFAULT_SOLAR_SYSTEM_ID;
...
function isDelayedLocalChatRoomName(value) {
    ...
    return (parsed.category === LOCAL_CHAT_CATEGORY_WORMHOLE ||
...
//# sourceMappingURL=channelRules.js.map