...
function resolveDependencies(options = {}) {
    return {
        itemTypes: options.itemTypes || require(path.join(__dirname, "../inventory/itemTypeRegistry")),
        siteStore: options.siteStore || require(path.join(__dirname, "../../space/frontierLandscapeCustomSites")),
        spaceRuntime: options.spaceRuntime || require(path.join(__dirname, "../../space/runtime")),
        worldData: options.worldData || require(path.join(__dirname, "../../space/worldData")),
    };
}
function formatEcosystemList(referenceData, query = "") {
    ...
    const ecosystems = listEcosystems(referenceData)
        ...
        return (String(ecosystem.ecosystemID ?? ecosystem._key) === normalizedQuery ||
            String(ecosystem.name || "").toLowerCase().includes(normalizedQuery));
    })
        .sort((left, right) => (toPositiveInt(left.ecosystemID ?? left._key, 0) -
    ...
    return [
        ...
        ...ecosystems.map((ecosystem) => (`${toPositiveInt(ecosystem.ecosystemID ?? ecosystem._key, 0)}: ${ecosystem.name}`)),
    ].join("\n");
}
...
function executeFrontierLandscapeCommand(session, argumentText, options = {}) {
        ...
        return {
            ...
            message: sites.length > 0
                ? [
                    ...
                    ...sites.map((site) => (`${site.itemID}: ecosystem ${site.ecosystemID} ${site.itemName} at ${formatPosition(site.position)}`)),
                ].join("\n")
    ...
    if (action === "spawn") {
        ...
        const liveResult = dependencies.spaceRuntime.addCustomLandscapeSite(createResult.data, { broadcast: true });
    ...
    if (action === "remove") {
        ...
        const liveResult = dependencies.spaceRuntime.removeCustomLandscapeSite(site.itemID, site.solarSystemID, { broadcast: true });
        if (!liveResult || liveResult.success !== true) {
            return {
                success: false,
...
//# sourceMappingURL=frontierLandscapeCommands.js.map