...
function addEntry(registry, entry) {
    ...
    if (!Number.isInteger(normalizedEntry.solarSystemID) ||
        normalizedEntry.solarSystemID <= 0 ||
        !normalizedEntry.solarSystemName) {
        return;
    }
    const normalizedName = normalizeSolarSystemName(normalizedEntry.solarSystemName);
    if (!normalizedName) {
        return;
    }
    registry.bySolarSystemID.set(normalizedEntry.solarSystemID, normalizedEntry);
    if (!registry.byName.has(normalizedName)) {
        registry.byName.set(normalizedName, []);
    }
    const entries = registry.byName.get(normalizedName);
    if (!entries.some((candidate) => candidate.solarSystemID === normalizedEntry.solarSystemID)) {
...
function resolveSolarSystemByName(query) {
    ...
    for (const entry of registry.bySolarSystemID.values()) {
        if (normalizeSolarSystemName(entry.solarSystemName).includes(normalizedQuery)) {
...
//# sourceMappingURL=solarSystemRegistry.js.map