...
function handledResultWithExtras(chatHub, session, options, message, extras: Record<string, any> = {}) {
...
function resolveCharacterTargetDescriptor(requestSession, targetToken) {
  ...
  const matchEntry = Object.entries<any>(characters).find(([, record]) => (
...
function resolveTrailingCharacterTargetDescriptor(requestSession, tokens: any[] = [], ownerStartIndex = 0) {
...
function filterChangedSkillRecords(skillRecords: any[] = [], previousSkillMap = null) {
...
function skillMutationLowersExistingLevels(skillRecords: any[] = [], previousSkillMap = null) {
  if (!(previousSkillMap instanceof Map)) {
    return false;
  }
...
function buildSkillNotificationDict(skillRecords: any[] = []) {
  return {
    type: "dict",
...
function normalizePositiveInteger(value: unknown, _unusedFallback?: number): number | null;
...
function formatShipLogoGrantTypeList(entitlementTypes: any[] = []) {
...
function parseNpcSpawnArguments(argumentText, options: Record<string, any> = {}) {
...
function getAccountRecordForSession(session) {
  ...
  const matchedEntry = Object.entries<any>(accounts).find(
...
function getChatColorLabel(roleValue) {
  ...
  const match = Object.entries<any>(CHAT_ROLE_PROFILES).find(
...
function syncSpaceRootInventoryChangesToSession(session, changes: any[] = []) {
...
function normalizeSpaceVector(vector, fallback: Record<string, any> = { x: 1, y: 0, z: 0 }) {
  const resolved = {
...
function cloneSpaceVector(vector, fallback: Record<string, any> = { x: 0, y: 0, z: 0 }) {
  return {
    x: Number.isFinite(Number(vector && vector.x)) ? Number(vector.x) : fallback.x,
    y: Number.isFinite(Number(vector && vector.y)) ? Number(vector.y) : fallback.y,
    z: Number.isFinite(Number(vector && vector.z)) ? Number(vector.z) : fallback.z,
...
function syncStationHangarChangesToSession(session, stationID, changes: any[] = []) {
...
function grantStationHangarBatchAndSyncSession(session, stationID, entries: any[] = []) {
...
function removeStationHangarBatchAndSyncSession(session, stationID, itemIDs: any[] = []) {
  const changes: any[] = [];
...
function boardStoredShipLikeHangarBoard(session, stationID, shipItem, options: Record<string, any> = {}) {
...
function handleLesmisCommand(session, chatHub, options) {
  ...
  if (
    launcherFitResult.success === false ||
...
function handleSpawnNpcTypeCommand(session, argumentText, chatHub, options) {
  ...
  if (!Number.isSafeInteger(typeID) || typeID <= 0 || !Number.isSafeInteger(count) || count < 1 || count > MAX_NPC_COMMAND_SPAWN_COUNT) {
    return handledResult(
      ...
      `/spawn requires a positive NPC typeID and a count from 1 to ${MAX_NPC_COMMAND_SPAWN_COUNT}.`,
    );
  }
  ...
  return handledResult(
    chatHub, session, options,
    `typeID ${typeID}: ${formatNpcSpawnSummary(result, "/spawn")} Spawn positions are spread around your ship.`,
  );
}
...
function parseExpertSystemTail(tailTokens: any[] = []) {
...
function handleLoadSystemCommand(session, chatHub, options) {
  ...
  const destinationSystemIDs = [...new Set<any>(
    stargates
...
function executeChatCommand(session, rawMessage, chatHub, options: Record<string, any> = {}) {
