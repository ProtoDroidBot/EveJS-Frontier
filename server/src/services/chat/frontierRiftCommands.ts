...
function executeFrontierRiftCommand(session, argumentText, options: Record<string, any> = {}) {
  ...
  if (action === "spawn") {
    ...
    if (!template) {
      return { success: false as const, message: `Rift template ${JSON.stringify(templateQuery)} was not found.` };
    }
  ...
  if (action === "remove") {
    ...
    if (!site || site.solarSystemID !== systemID) {
      return { success: false as const, message: "Custom Rift was not found in this system." };
    }
