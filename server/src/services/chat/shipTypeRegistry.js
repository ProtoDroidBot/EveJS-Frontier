...
function loadDbRegistry() {
    try {
        ...
        log.warn(`[ShipRegistry] Failed to load ship reference data from gameStore: ${error.message}`);
        return null;
    }
}
    ...
    if (exactMatches.length > 1) {
        return {
            ...
            suggestions: exactMatches
                .slice(0, 5)
                .map((entry) => `${entry.name} (${entry.typeID}${entry.published === false ? ", unpublished" : ""})`),
        };
    }
...
//# sourceMappingURL=shipTypeRegistry.js.map