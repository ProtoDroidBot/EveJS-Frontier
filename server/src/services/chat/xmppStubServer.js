...
const chatStore = require(path.join(__dirname, "../../_secondary/chat/chatStore"));
const { getLocalChatRoomNameForSession, getLocalChatRoomNameForSolarSystemID, isDelayedLocalChatRoomName, isLocalChatRoomName, parseLocalChatRoomName, } = require(path.join(__dirname, "./channelRules"));
const { getCharacterRecord, toBigInt } = require(path.join(__dirname, "../character/characterState"));
const { executeChatCommand, DEFAULT_MOTD_MESSAGE } = require(path.join(__dirname, "./chatCommands"));
const { buildXmppConferenceJid, buildXmppUserJid, escapeRegExp, getXmppConferenceDomain, getXmppDomain, } = require(path.join(__dirname, "./xmppConfig"));
const { configureClientSocket } = require(path.join(__dirname, "../../network/tcp/socketTuning"));
let server = null;
...
function readTlsCredentials() {
    if (!fs.existsSync(certPath) || !fs.existsSync(keyPath)) {
        ...
        try {
            const { ensureXmppTlsCertificate, } = require("../../_secondary/express/localTlsCertificate");
            ...
            log.info(`[XMPP] Generated a self-signed TLS certificate at ${certDir} (first run).`);
        }
        catch (error) {
            throw new Error(`Missing XMPP TLS certificate files at ${certDir} and auto-generation failed: ${error.message}`);
        }
    }
    return {
...
function buildBoundJid(client) {
    ...
    return buildXmppUserJid((client && client.userName) || "capsuleer", requestedResource || "eveclient");
}
...
function noteRoomMessageDelivered(client, roomJid, createdAtMs = Date.now()) {
    ...
    const previousCreatedAtMs = Math.max(0, Number(cursorMap.get(roomJid)) || 0);
...
function getCorpRoomNameForSession(session) {
    const corpID = Number((session && (session.corporationID || session.corpid)) || 0);
...
function getAllianceRoomNameForSession(session) {
    const allianceID = Number((session && (session.allianceID || session.allianceid)) || 0);
    return allianceID > 0 ? `alliance_${allianceID}` : "";
}
function getFactionRoomNameForSession(session) {
    const factionID = Number((session && (session.warFactionID || session.warfactionid)) || 0);
...
function isConferenceServiceJid(value) {
    ...
    return (normalizedValue === conferenceDomain ||
...
function normalizeRoomJid(roomJid, client = null) {
    ...
    if (rawRoomJid === "local" || rawRoomJid === `local@${conferenceDomain}`) {
        return buildConferenceRoomJid(client ? getLocalRoomNameForClient(client) : getLocalRoomNameForSession(null));
    }
    ...
    const legacyLocalMatch = new RegExp(`^solarsystemid2?_(\\d+)@${escapeRegExp(getXmppConferenceDomain())}$`, "i").exec(rawRoomJid);
    if (legacyLocalMatch) {
        return buildConferenceRoomJid(getLocalChatRoomNameForSolarSystemID(legacyLocalMatch[1]));
    }
    const legacyCorpMatch = new RegExp(`^corpid_(\\d+)@${escapeRegExp(getXmppConferenceDomain())}$`, "i").exec(rawRoomJid);
    if (legacyCorpMatch) {
...
function findSessionForClient(client) {
    ...
    if (Number.isInteger(expectedCharId) &&
        expectedCharId > 0 &&
    ...
    return (sessionRegistry.getSessions().find((session) => {
        return (String(session.userName || "").trim().toLowerCase() === expectedUser ||
...
function getUserDataForCharacterId(charId) {
    ...
    const session = sessionRegistry.getSessions().find((candidate) => Number(candidate.characterID || 0) === Number(charId)) || null;
    return {
        charId,
...
function getConnectedClientsForLocalRoom(roomName, { excludeClient = null, roomJid = "" } = {}) {
    const normalizedRoomName = normalizeString(roomName, "").trim();
    if (!normalizedRoomName) {
        return [];
    }
    const visibleCharacterIDs = new Set(chatRuntime
...
function getDescriptorRecord(descriptor, session = null) {
    ...
    if (record.type === "system" &&
        ...
        record = chatRuntime.updateChannel(record.roomName, (currentRecord) => ({
            ...
            adminCharacterIDs: addUniqueCharacterID(currentRecord.adminCharacterIDs, ownerCharacterID),
            operatorCharacterIDs: addUniqueCharacterID(currentRecord.operatorCharacterIDs, ownerCharacterID),
            metadata: {
                ...(currentRecord.metadata || {}),
                joinLink: currentRecord.roomName
...
function getCharacterIDForSession(session) {
    return Number(session && (session.characterID || session.charid || session.userid) || 0) || 0;
}
...
function deliverRoomBacklogToClient(client, roomJid, roomName, options = {}) {
    ...
    const normalizedRoomJid = normalizeRoomJid(roomJid, client);
    const cursorMap = getClientRoomBacklogCursorMap(client);
    const afterCreatedAtMs = Math.max(0, Number(cursorMap instanceof Map ? cursorMap.get(normalizedRoomJid) : 0) || 0);
    ...
    for (const entry of backlogEntries) {
        ...
        sendXml(client, buildRoomMessageXml(normalizedRoomJid, senderID, entry.message || "", client));
        noteRoomMessageDelivered(client, normalizedRoomJid, Number(entry && entry.createdAtMs) || Date.now());
        deliveredCount += 1;
    }
...
function extractRequestedAdminAffiliation(xml) {
    ...
    const requestedAffiliation = normalizeString(adminItems[0] && adminItems[0].affiliation, "").trim().toLowerCase();
    return (requestedAffiliation === "owner" ||
...
function buildRoomConfigResultXml(client, roomJid, requestId, record) {
    ...
    const passwordProtected = Boolean(effectiveRecord && effectiveRecord.passwordRequired);
...
function sendRoomInvite(roomName, inviterCharacterID, targetCharacterID, reason = "") {
    ...
    for (const client of connectedClients) {
        ...
        sendXml(client, buildRoomInviteXml(roomJid, inviterCharacterID, client, reason));
        sent = true;
    }
    return sent;
}
function inviteCharacterToRoom(roomName, inviterCharacterID, targetCharacterID, reason = "") {
    const normalizedRoomName = normalizeString(roomName, "").trim();
    ...
    try {
        chatRuntime.inviteCharacterToChannel(normalizedRoomName, normalizedTargetCharacterID);
    }
    catch (error) {
        log.debug(`[XMPP] Failed to persist room invite room=${normalizedRoomName} char=${normalizedTargetCharacterID}: ${error.message}`);
    }
    return sendRoomInvite(normalizedRoomName, inviterCharacterID, normalizedTargetCharacterID, reason);
}
...
function removeClientFromRoom(roomJid, client, options = {}) {
    ...
    if (session && descriptor.roomName) {
        try {
            ...
            log.debug(`[XMPP] Ignored room leave runtime error room=${descriptor.roomName} char=${charId}: ${error.message}`);
        }
    }
    ...
    if (options.notifySelf === true) {
        ...
        sendXml(client, buildSelfUnavailablePresenceXml(normalizedRoomJid, client, nick, options.requestId || nextMessageId(), userData));
    }
    ...
    for (const recipient of recipients) {
        sendXml(recipient, buildRoomPresenceXml(normalizedRoomJid, charId, recipient, { available: false }));
    }
}
...
function sendSessionSystemMessage(session, message, roomJid = null) {
    ...
    for (const client of connectedClients) {
        ...
        sendSystemMessageToClient(client, roomJid || client.lastRoomJid || normalizeRoomJid("", client), message);
    }
}
...
function buildRoomInfoResultXml(client, roomJid, requestId) {
    ...
    const roomLabel = sanitizeRoomLabel(record && record.displayName, descriptor.kind === "corp"
        ? "Corp"
        : descriptor.kind === "local"
            ? "Local"
...
function extractDiscoInfoNode(xml) {
    const match = /<query\b[^>]*\bxmlns=['"]http:\/\/jabber\.org\/protocol\/disco#info['"][^>]*\bnode=['"]([^'"]+)['"]/i.exec(xml);
    return match ? decodeXml(match[1]).trim() : "";
}
function extractDiscoItemsNode(xml) {
    const match = /<query\b[^>]*\bxmlns=['"]http:\/\/jabber\.org\/protocol\/disco#items['"][^>]*\bnode=['"]([^'"]+)['"]/i.exec(xml);
    return match ? decodeXml(match[1]).trim() : "";
}
function extractExpiringRecordSearchRoom(xml) {
    const match = /<query\b[^>]*\bxmlns=['"]urn:xmpp:expiring_record#search['"][^>]*\broom=['"]([^'"]+)['"]/i.exec(xml);
    return match ? decodeXml(match[1]).trim() : "";
}
function extractExpiringRecordSearchCategory(xml) {
    const match = /<query\b[^>]*\bxmlns=['"]urn:xmpp:expiring_record#search['"][^>]*\bcategory=['"]([^'"]+)['"]/i.exec(xml);
...
function getRoomDiscoIdentityType(record, descriptor) {
    const browserCategoryName = normalizeConferenceIdentityType(record && record.metadata && record.metadata.browserCategoryName);
...
function handleConferenceDiscoItemsIq(client, xml) {
    ...
    if (node === "forme") {
        ...
        records = chatRuntime.findDiscoverableConferenceChannelsByDisplayName(session, displayName);
    }
    sendXml(client, buildConferenceDiscoItemsResultXml(client, requestId, node, records));
}
function handleConferenceDiscoInfoIq(client, xml) {
    ...
    if (isConferenceServiceJid(rawTo) && node) {
        sendXml(client, buildConferenceNodeInfoResultXml(client, requestId, node, session));
        return;
    }
    ...
    sendXml(client, buildRoomInfoResultXml(client, rawTo, requestId));
}
function buildRoomAdminResultXml(client, roomJid, requestId, requestedAffiliation = "") {
    ...
    const items = [];
    if (record) {
        if (record.ownerCharacterID &&
            (!normalizedAffiliation || normalizedAffiliation === "owner")) {
            items.push(`<item affiliation='owner' jid='${escapeXml(buildXmppUserJid(record.ownerCharacterID))}' nick='${escapeXml(record.ownerCharacterID)}'/>`);
        }
        if (!normalizedAffiliation || normalizedAffiliation === "admin") {
            for (const adminCharacterID of dedupePositiveCharacterIDs([
                ...
                items.push(`<item affiliation='admin' jid='${escapeXml(buildXmppUserJid(adminCharacterID))}' nick='${escapeXml(adminCharacterID)}'/>`);
            }
        }
        if (!normalizedAffiliation || normalizedAffiliation === "member") {
            for (const memberCharacterID of dedupePositiveCharacterIDs([
                ...
                items.push(`<item affiliation='member' jid='${escapeXml(buildXmppUserJid(memberCharacterID))}' nick='${escapeXml(memberCharacterID)}'/>`);
            }
        }
        if (!normalizedAffiliation || normalizedAffiliation === "outcast") {
            for (const characterID of dedupePositiveCharacterIDs(Object.keys(record.bannedCharacters || {}))) {
                items.push(`<item affiliation='outcast' jid='${escapeXml(buildXmppUserJid(characterID))}' nick='${escapeXml(characterID)}'/>`);
            }
        }
    }
    return [
...
function syncSessionScopedRoomMembership(session, options = {}) {
    ...
    const autoJoinKinds = new Set((Array.isArray(options.autoJoinKinds) ? options.autoJoinKinds : [])
...
function buildExpiringRecordSearchResultXml(client, requestId, roomJid, category, entries = []) {
    return [
        `<iq type='result' id='${escapeXml(requestId)}'`,
...
function handleExpiringRecordSearchIq(client, xml) {
    ...
    const entries = chatRuntime.listChannelTemporaryRestrictions(descriptor.roomName, category);
    sendXml(client, buildExpiringRecordSearchResultXml(client, requestId, descriptor.normalizedRoomJid, category, entries));
}
function refreshSessionChatRolePresence(session) {
    ...
    const relevantClients = [...connectedClients].filter((client) => getClientCharacterId(client) === charId);
...
function reassertSessionRoomPresence(session) {
    ...
    const currentRoomJid = currentRoomName && isCharacterVisibleInLocalRoom(currentRoomName, charId)
...
function buildSyntheticRoomJoinPresenceXml(roomJid, charId) {
    return (`<presence to='${escapeXml(roomJid)}/${escapeXml(charId)}'` +
...
function joinClientToRoom(client, session, charId, roomName) {
    ...
    handleJoinPresence(client, buildSyntheticRoomJoinPresenceXml(roomJid, charId));
...
function moveSessionToCurrentLocalRoom(session) {
    ...
    for (const client of connectedClients) {
        ...
        const localRooms = [...client.rooms].filter((roomJid) => isLocalChatRoomName(roomJid));
...
function handleJoinPresence(client, xml) {
    ...
    if (!to) {
        ...
        sendXml(client, `<presence from='${escapeXml(client.boundJid)}' to='${escapeXml(client.boundJid)}'/>`);
        return;
    }
    ...
    if (session && descriptor.roomName) {
        try {
            ...
            sendSystemMessageToClient(client, descriptor.normalizedRoomJid, formatChannelAccessError(error, "join"));
            return;
        }
    }
    client.lastRoomJid =
    ...
    if (!descriptor.suppressPresenceBroadcast && charId) {
        ...
        for (const member of existingRecipients) {
            ...
            sendXml(client, buildRoomPresenceXml(descriptor.normalizedRoomJid, memberCharacterID, client, {
                affiliation: getAffiliationForCharacter(roomRecord, memberCharacterID),
            }));
        }
        ...
        for (const recipient of existingRecipients) {
            sendXml(recipient, buildRoomPresenceXml(descriptor.normalizedRoomJid, charId, recipient, {
                affiliation: getAffiliationForCharacter(roomRecord, charId),
            }));
        }
    }
    if (roomRecord && roomRecord.motd) {
        sendXml(client, buildRoomSubjectXml(descriptor.normalizedRoomJid, roomRecord.motd, client));
    }
    if (roomRecord && descriptor.kind !== "local") {
        deliverRoomBacklogToClient(client, descriptor.normalizedRoomJid, descriptor.roomName, {
            historySeconds,
            limit: Math.max(0, Number(roomRecord.backlogLimit) || 25),
        });
    }
    if (!client.localWelcomeSent &&
        descriptor.normalizedRoomJid === normalizeRoomJid("", client)) {
        client.localWelcomeSent = true;
        setTimeout(() => {
            sendSystemMessageToClient(client, descriptor.normalizedRoomJid, buildLocalWelcomeMessage());
        }, 100);
    }
}
function handleGroupMessage(client, xml) {
    ...
    if (messageType === "chat" &&
        ...
        if (targetCharacterId) {
            const roomRecord = chatRuntime.ensurePrivateChannelForInvite(session, targetCharacterId, {
                metadata: {
            ...
            if (roomRecord) {
                try {
                    ...
                    sendSystemMessageToClient(client, buildConferenceRoomJid(roomRecord.roomName), formatChannelAccessError(error, "join"));
                    return;
                }
                ...
                sendRoomInvite(roomRecord.roomName, getCharacterIDForSession(session), targetCharacterId, inviteReason || body || "");
            }
        }
        return;
    }
    const roomJid = normalizeRoomJid(getRoomJid(rawTo), client) ||
        client.lastRoomJid;
    if (!roomJid) {
        return;
    }
    ...
    if (subject && !body) {
        if (!session || !roomRecord || !chatRuntime.isChannelAdmin(session, roomRecord)) {
            sendSystemMessageToClient(client, roomJid, "You are not allowed to update the channel MOTD.");
            return;
        }
    ...
    if (!session) {
        sendSystemMessageToClient(client, roomJid, "Message unavailable: no active game session matched this chat connection.");
        return;
    }
    let sendResult = null;
    try {
        ...
        sendSystemMessageToClient(client, roomJid, formatChannelAccessError(error, "speak"));
        return;
    }
    const senderCharacterId = getClientCharacterId(client, session) ||
    ...
    deliverRoomMessage(roomJid, senderCharacterId, body, null, {
        messageId: requestId,
        createdAtMs: Number(sendResult && sendResult.entry && sendResult.entry.createdAtMs) || Date.now(),
    });
}
...
function handleRoomAdminIq(client, xml) {
    ...
    for (const item of extractAdminItems(xml)) {
        const targetCharacterID = parseCharacterId(String(item.jid || "").split("@")[0]) ||
        ...
        if (item.affiliation === "owner") {
            ...
            chatRuntime.banChannelCharacter(record.roomName, targetCharacterID, 0, item.reason || "", actorCharacterID);
            ...
            chatRuntime.updateChannel(record.roomName, (currentRecord) => {
                ...
                return {
                    ...currentRecord,
                    adminCharacterIDs: (currentRecord.adminCharacterIDs || []).filter((candidate) => candidate !== targetCharacterID),
                    operatorCharacterIDs: (currentRecord.operatorCharacterIDs || []).filter((candidate) => candidate !== targetCharacterID),
                    invitedCharacters: (currentRecord.invitedCharacters || []).filter((candidate) => candidate !== targetCharacterID),
                    allowCharacterIDs: (currentRecord.allowCharacterIDs || []).filter((candidate) => candidate !== targetCharacterID),
                    allowedParticipantCharacterIDs: (currentRecord.allowedParticipantCharacterIDs || []).filter((candidate) => candidate !== targetCharacterID),
                    bannedCharacters,
                };
            });
        }
    ...
    sendXml(client, buildRoomAdminResultXml(client, roomJid, requestId, requestedAffiliation));
}
function handleRoomOwnerIq(client, xml) {
    ...
    const passwordProtected = parseBooleanFieldValue(fieldMap, "muc#roomconfig_passwordprotectedroom", Boolean(record.passwordRequired));
    const nextPassword = passwordProtected
        ? getFirstFieldValue(fieldMap, "muc#roomconfig_roomsecret", String(record.password || ""))
        : "";
    const updatedRecord = chatRuntime.updateChannel(record.roomName, (currentRecord) => ({
        ...currentRecord,
        displayName: getFirstFieldValue(fieldMap, "muc#roomconfig_roomname", currentRecord.displayName),
        topic: getFirstFieldValue(fieldMap, "muc#roomconfig_roomdesc", currentRecord.topic || ""),
        inviteOnly: parseBooleanFieldValue(fieldMap, "muc#roomconfig_membersonly", Boolean(currentRecord.inviteOnly)),
...
function handleReadyIq(client, xml) {
    ...
    if (xml.includes("http://jabber.org/protocol/muc#admin")) {
        ...
        sendXml(client, buildRoomAdminResultXml(client, extractAttr(xml, "to"), extractId(xml), extractRequestedAdminAffiliation(xml)));
        return;
    }
...
//   * chatRuntime.runtimeEmitter local-join/leave/message            -> Local
//   * webChatGatewayService.corpChatEmitter corp-join/leave/message  -> Corp
//
...
// Corp membership is session-derived (retail corp chat has no getVisibleLocal-
// Sessions analogue): enumerate the live session registry by corporationID,
// collapsed to one preferred session per character.
function getCorpRoomCharacterSessions(corporationID) {
    ...
    for (const session of sessionRegistry.getSessions()) {
        const sessionCorpID = Number((session && (session.corporationID || session.corpid)) || 0);
...
// is enumerated from the session registry by corporationID. Characters that
// already hold an XMPP client are excluded — they are advertised by the existing
// occupant-presence broadcast, never twice.
function getSocketlessRoomMemberCharacterIDs(descriptor) {
    ...
    if (descriptor.kind === "local" && descriptor.roomName) {
        candidateSessions = chatRuntime.getVisibleLocalSessions(descriptor.roomName);
    }
    else if (descriptor.kind === "corp" && descriptor.roomID) {
        candidateSessions = getCorpRoomCharacterSessions(descriptor.roomID);
    }
    else {
        return [];
    }
...
function bridgeSocketlessLocalJoin(event) {
    try {
        ...
        const characterID = Number((event && event.summary && event.summary.characterID) || 0);
        ...
        for (const recipient of getLocalRoomBridgeRecipients(roomName, roomJid)) {
            ...
            sendXml(recipient, buildRoomPresenceXml(roomJid, characterID, recipient, { available: true }));
        }
    }
    catch (error) {
...
function bridgeSocketlessLocalLeave(event) {
    try {
        ...
        for (const recipient of recipients) {
            sendXml(recipient, buildRoomPresenceXml(roomJid, characterID, recipient, { available: false }));
        }
    }
    catch (error) {
...
function bridgeSocketlessCorpPresence(corporationID, characterID, available) {
    ...
    for (const recipient of getCorpRoomOccupants(corpID)) {
        ...
        sendXml(recipient, buildRoomPresenceXml(roomJid, normalizedCharacterID, recipient, {
...
function bridgeSocketlessCorpJoin(event) {
    try {
        bridgeSocketlessCorpPresence(event && event.corporationID, event && event.characterID, true);
    }
    catch (error) {
...
function bridgeSocketlessCorpLeave(event) {
    try {
        bridgeSocketlessCorpPresence(event && event.corporationID, event && event.characterID, false);
    }
    catch (error) {
...
function bridgeSocketlessCorpDeparture(session) {
    try {
        ...
        bridgeSocketlessCorpPresence(Number((session.corporationID || session.corpid) || 0), characterID, false);
    }
    catch (error) {
...
// Reconcile the web gateway's private corpChatEmitter onto this server. The
// emitter's owner (webChatGatewayService) safely requires xmppStubServer and
// calls this at load, so there is no require cycle (xmppStubServer never
// requires webChatGatewayService). Idempotent per emitter.
    ...
    socket.on("data", (chunk) => {
        ...
        try {
            if (client.stage === "stream1" && client.buffer.includes("<stream:stream")) {
                sendXml(client, `<?xml version='1.0'?><stream:stream from='${escapeXml(getXmppDomain())}' id='evejs' version='1.0' xmlns='jabber:client' xmlns:stream='http://etherx.jabber.org/streams'><stream:features><mechanisms xmlns='urn:ietf:params:xml:ns:xmpp-sasl'><mechanism>PLAIN</mechanism></mechanisms></stream:features>`);
                client.buffer = "";
                client.stage = "auth";
                return;
            }
            if (client.stage === "auth" && client.buffer.includes("<auth")) {
                ...
                sendXml(client, "<success xmlns='urn:ietf:params:xml:ns:xmpp-sasl'/>");
                client.buffer = "";
                client.stage = "stream2";
                return;
            }
            if (client.stage === "stream2" &&
                client.buffer.includes("<stream:stream")) {
                sendXml(client, `<?xml version='1.0'?><stream:stream from='${escapeXml(getXmppDomain())}' id='evejs2' version='1.0' xmlns='jabber:client' xmlns:stream='http://etherx.jabber.org/streams'><stream:features><bind xmlns='urn:ietf:params:xml:ns:xmpp-bind'/><session xmlns='urn:ietf:params:xml:ns:xmpp-session'/></stream:features>`);
                client.buffer = "";
                client.stage = "bind";
                return;
            }
            if (client.stage === "bind" && client.buffer.includes("<bind")) {
                ...
                client.boundJid = buildBoundJid(client);
                sendXml(client, `<iq type='result' id='${escapeXml(id)}'><bind xmlns='urn:ietf:params:xml:ns:xmpp-bind'><jid>${escapeXml(client.boundJid)}</jid></bind></iq>`);
                client.buffer = "";
...
function startXmppStub() {
    ...
    server.listen(config.xmppServerPort, config.xmppServerBindHost, () => {
        log.success(`[XMPP] stub chat listener running on ${config.xmppServerBindHost}:${config.xmppServerPort}`);
    });
    server.on("error", (error) => {
        log.err(`[XMPP] stub server error: ${error.message}`);
    });
    server.on("secureConnection", (socket) => {
        writeTranscript("TLSOK", `${socket.remoteAddress || "unknown"}:${socket.remotePort || 0}`);
    });
    server.on("tlsClientError", (error, socket) => {
        writeTranscript("TLSERR", `${socket.remoteAddress || "unknown"}:${socket.remotePort || 0} ${error.message}`);
...
//# sourceMappingURL=xmppStubServer.js.map