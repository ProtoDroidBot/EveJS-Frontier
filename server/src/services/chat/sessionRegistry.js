...
function isLiveSession(session) {
    return Boolean(isTrackedControlSession(session) &&
...
function selectPreferredFrom(candidates, targetCharacterID, excludedSession, options = {}) {
...
function findSessionByCharacterID(characterID, options = {}) {
    ...
    if (bucket && bucket.size > 0) {
        const indexed = selectPreferredFrom(bucket, targetCharacterID, excludedSession, options);
        if (indexed) {
            return indexed;
        }
    }
    ...
    const fallbackSessions = options.includeClosing === true
        ? Array.from(sessions)
        : getSessions();
    return selectPreferredFrom(fallbackSessions, targetCharacterID, excludedSession, options);
}
module.exports = {
    register,
    unregister,
