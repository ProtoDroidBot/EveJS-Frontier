...
function resolveSessionToken(session) {
    ...
    return (normalizeText(session.chatAuthenticationToken, null) ||
        normalizeText(session.xmppAuthenticationToken, null) ||
        normalizeText(resolveMapValue(session.chatAuthenticationTokensByCharacterID, characterID), null) ||
        normalizeText(resolveMapValue(session.xmppAuthenticationTokensByCharacterID, characterID), null) ||
        null);
}
class ChatAuthenticationService extends BaseService {
    ...
    getAuthenticationToken(session) {
        const characterID = resolveCharacterID(session);
        const injectedToken = normalizeText(resolveMapValue(this._tokenByCharacterID, characterID), null);
        if (injectedToken) {
    ...
    Handle_GetAuthenticationToken(args, session) {
        ...
        log.debug(`[ChatAuthenticationService] GetAuthenticationToken char=${characterID || "unknown"} -> ${token ? "provided" : "local-fallback"}`);
        return token || null;
    }
}
...
//# sourceMappingURL=chatAuthenticationServiceService.js.map