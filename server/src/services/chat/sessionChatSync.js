...
function getChangeKeys(changes) {
    ...
    if (Array.isArray(changes)) {
        return changes
            .map((entry) => (Array.isArray(entry) && entry.length > 0 ? String(entry[0]) : ""))
            .filter(Boolean);
    }
...
function synchronizeSessionChatState(session, changes) {
    ...
    const shouldRefreshPresence = hasMatchingKey(changeKeys, CHAT_PRESENCE_SESSION_KEYS);
    ...
    try {
        if (shouldSyncLocal) {
            ...
            if (typeof chatHub.moveLocalSession === "function") {
                localSynced = Boolean(chatHub.moveLocalSession(session, getLocalPreviousSolarSystemID(changes)));
            }
        }
        if (shouldSyncScoped) {
            const { syncSessionScopedRoomMembership } = require(path.join(__dirname, "./xmppStubServer"));
            if (typeof syncSessionScopedRoomMembership === "function") {
                scopedSynced = Boolean(syncSessionScopedRoomMembership(session, {
        ...
        if (shouldRefreshPresence) {
            ...
            if (typeof chatHub.refreshSessionChatRolePresence === "function") {
                presenceRefreshed = Boolean(chatHub.refreshSessionChatRolePresence(session));
            }
        }
    }
    catch (error) {
        log.debug(`[ChatSync] Skipped post-session-change chat sync: ${error.message}`);
    }
    return {
        localSynced,
        scopedSynced,
...
//# sourceMappingURL=sessionChatSync.js.map