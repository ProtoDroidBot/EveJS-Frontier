...
function getSessionScopedRoomNames(session) {
  const roomNames = new Set<any>(chatRuntime.getChannelsForStaticAccess(session));
...
function addUniqueCharacterID(values: any[] = [], characterID) {
...
function addRoomMember(roomJid, client) {
  ...
  if (!roomMembers.has(roomJid)) {
    roomMembers.set(roomJid, new Set<any>());
  }
...
function getDistinctRoomCharacterIds(roomJid) {
  ...
  const seenCharacterIDs = new Set<any>();
...
function getConnectedClientsForLocalRoom(
  roomName,
  { excludeClient = null, roomJid = "" }: Record<string, any> = {},
) {
  ...
  const visibleCharacterIDs = new Set<any>(
    chatRuntime
...
function removeClientFromStaleLocalRooms(client, expectedRoomJid = "", options: Record<string, any> = {}) {
...
function deliverRoomBacklogToClient(client, roomJid, roomName, options: Record<string, any> = {}) {
  if (!client || !roomName) {
    return 0;
  }
...
function dedupePositiveCharacterIDs(values: any[] = []) {
  return [...new Set(
...
function removeClientFromRoom(roomJid, client, options: Record<string, any> = {}) {
  const descriptor = getRoomDescriptor(roomJid, client);
...
function removeClientFromRooms(client, options: Record<string, any> = {}) {
  for (const roomJid of [...client.rooms]) {
...
function buildRoomMessageXml(roomJid, sender, message, recipient, options: Record<string, any> = {}) {
...
function deliverRoomMessage(roomJid, sender, message, recipients = null, options: Record<string, any> = {}) {
  const members = recipients || roomMembers.get(roomJid);
  if (!members || message === "") {
    return;
  }
...
function sendLocalAdminLeave(roomJid, characterID, recipients: any[] = []) {
  const normalizedCharacterID = Number(characterID || 0);
...
function buildRoomPresenceXml(roomJid, charId, recipient, options: Record<string, any> = {}) {
...
function buildConferenceDiscoItemsResultXml(client, requestId, node, records: any[] = []) {
  const seenRoomNames = new Set<any>();
  const items: any[] = [];
...
function syncSessionScopedRoomMembership(session, options: Record<string, any> = {}) {
  if (!session) {
    return false;
  }
...
function joinClientToRoom(client, session, charId, roomName) {
  ...
  const roomsBefore = new Set<any>(client.rooms);
  handleJoinPresence(
    client,
  ...
  if (body.startsWith("/") || body.startsWith(".")) {
    ...
    Promise.resolve(commandResult).then((result) => {
      ...
      sendSystemMessageToClient(
        client,
        roomJid,
        "Command failed before it could be completed.",
      );
    });
    return;
  }
