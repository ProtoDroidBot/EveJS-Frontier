...
const { spawnShipInHangarForSession, giveItemToActiveShipCargoForSession, giveItemToHangarForSession, getActiveShipRecord, getCharacterRecord, applyCharacterToSession, activateShipForSession, syncInventoryItemForSession, syncShipFittingStateForSession, } = require("../character/characterState");
const sessionRegistry = require("./sessionRegistry");
const { isLocalChatRoomName } = require("./channelRules");
const { getAllItems, getCharacterHangarShipItems, ITEM_FLAGS, CLIENT_INVENTORY_STACK_LIMIT, listContainerItems, listOwnedItems, createSpaceItemForCharacter, grantItemsToCharacterStationHangar, moveItemTypeFromCharacterLocation, normalizeShipConditionState, removeInventoryItem, updateInventoryItem, updateShipItem, } = require("../inventory/itemStore");
const { resolveDebrisType, listAvailableDebrisTypes, spawnDebrisFieldForSession, clearNearbyDebrisForSession, clearSystemDebrisForSession, getSpaceDebrisLifetimeMs, } = require("../inventory/spaceDebrisState");
const { getCharacterWallet, setCharacterBalance, adjustCharacterBalance, emitPlexBalanceChangeToSession, setCharacterPlexBalance, adjustCharacterPlexBalance, } = require("../account/walletState");
const { EVERMARK_ISSUER_CORP_ID, adjustCharacterWalletLPBalance, adjustCorporationWalletLPBalance, getCharacterWalletLPBalance, getCharacterWalletLPBalances, getCorporationWalletLPBalance, setCharacterWalletLPBalance, setCorporationWalletLPBalance, } = require("../corporation/lpWalletState");
const { COSMETIC_TYPE_ALLIANCE_LOGO, COSMETIC_TYPE_CORPORATION_LOGO, SHIP_LOGO_ENTITLEMENT_ALLIANCE, SHIP_LOGO_ENTITLEMENT_CORPORATION, } = require("../evermarks/evermarksConstants");
const { getLicenseByShipAndCosmeticType, } = require("../evermarks/evermarksCatalog");
const { grantShipLogoEntitlement, } = require("../evermarks/evermarksEntitlements");
const { publishShipLogoGrantedNotice, } = require("../evermarks/evermarksNotices");
const { PLEX_LOG_CATEGORY, } = require("../account/plexVaultLogState");
const { getUnpublishedShipTypes, resolveShipByName, resolveShipByTypeID, } = require("./shipTypeRegistry");
const { resolveItemByName, resolveItemByTypeID, } = require("../inventory/itemTypeRegistry");
const { resolveRuntimeWreckRadius, } = require("../inventory/wreckRadius");
const { resolveSolarSystemByName } = require("./solarSystemRegistry");
const { buildShipResourceState, getTypeAttributeValue, isChargeCompatibleWithModule, isFittedModuleItem, listFittedItems, normalizeModuleState, selectAutoFitFlagForType, typeHasEffectName, validateFitForShip, } = require("../fitting/liveFittingState");
const { clearCharacterSkills, ensureCharacterPublishedSkills, ensureCharacterUnpublishedSkills, grantCharacterSkillLevels, getCharacterSkillMap, getPublishedSkillTypes, getSkillTypes, getUnpublishedSkillTypes, removeCharacterSkillTypes, } = require("../skills/skillState");
const { reconcileQueueForSkillMutation, resetCharacterSkillsToStarterProfile, } = require("../skills/skillAdminRuntime");
const { emitSkillSessionState, } = require("../skills/training/skillQueueNotifications");
const { getQueueSnapshot, } = require("../skills/training/skillQueueRuntime");
const { getExpertSystemByTypeID, listExpertSystems, resolveExpertSystemQuery: resolveExpertSystemCatalogQuery, } = require("../skills/expertSystems/expertSystemCatalog");
...
const { TIDI_ADVANCE_NOTICE_MS, scheduleAdvanceNoticeTimeDilationForSystems, } = require("../../utils/synchronizedTimeDilation");
const tidiAutoscaler = require("../../utils/tidiAutoscaler");
const database = require("../../gameStore");
const { buildEffectListText, playPlayableEffect, stopAllPlayableEffects, } = require("./specialFxRegistry");
...
const { TRIG_DRIFTER_CHAT_COMMANDS, TRIG_DRIFTER_HELP_LINES, executeTrigDrifterCommand, } = require("./trigDrifter");
const { GATE_SKIN_CHAT_COMMANDS, GATE_SKIN_HELP_LINES, executeGateSkinCommand, } = require("../../space/gateSkinCommand");
const { handleShipDirtCommand, handleShipKillmarksCommand, } = require("../ship/shipAppearanceGmCommands");
const { NPCTEST_DEFAULT_AMOUNT, spawnNpcTestForSession, } = require("../../space/npc/npcTestService");
...
const { executeUpwellAutoCommand, } = require(path.join(__dirname, "../structure/structureAutoCommands"));
const structureLog = require(path.join(__dirname, "../structure/structureLog"));
const { executeSovCommand, } = require(path.join(__dirname, "../sovereignty/sovChatCommands"));
const { executeSovAutoCommand, } = require(path.join(__dirname, "../sovereignty/sovAutoCommands"));
const sovLog = require(path.join(__dirname, "../sovereignty/sovLog"));
const { handleSuperTitanCommand: executeSuperTitanCommand, handleSuperTitanShowCommand: executeSuperTitanShowCommand, } = require(path.join(__dirname, "../superweapons/superweaponCommands"));
const { handleMinerCommand: executeMinerCommand, handleMiningFleetCommand: executeMiningFleetCommand, handleMiningFleetAggroCommand: executeMiningFleetAggroCommand, handleMiningFleetClearCommand: executeMiningFleetClearCommand, handleMiningFleetStatusCommand: executeMiningFleetStatusCommand, handleMiningFleetRetreatCommand: executeMiningFleetRetreatCommand, handleMiningFleetResumeCommand: executeMiningFleetResumeCommand, handleMiningFleetHaulCommand: executeMiningFleetHaulCommand, handleMiningStateStatusCommand: executeMiningStateStatusCommand, handleMiningStateResetCommand: executeMiningStateResetCommand, } = require(path.join(__dirname, "../mining/miningCommandService"));
const { handleOrcaCommand: executeOrcaCommand, handleProbeCommand: executeProbeCommand, handleProbe2Command: executeProbe2Command, handleCburstCommand: executeCburstCommand, handleGuardianCommand: executeGuardianCommand, handleBasiliskCommand: executeBasiliskCommand, handleEwarCommand: executeEwarCommand, handleTrigCommand: executeTrigCommand, } = require(path.join(__dirname, "../ship/devCommandShipRuntime"));
const ScanMgrService = require(path.join(__dirname, "../exploration/scanMgrService"));
const signatureRuntime = require(path.join(__dirname, "../exploration/signatures/signatureRuntime"));
const { markMissionObjectiveComplete, } = require(path.join(__dirname, "../agent/agentMissionRuntime"));
const { listAgents, } = require(path.join(__dirname, "../agent/agentAuthority"));
const standingRuntime = require(path.join(__dirname, "../character/standingRuntime"));
const { handleRemoteRepairFleetCommand: executeRemoteRepairFleetCommand, } = require(path.join(__dirname, "../../RemoteRepShow/remoteRepairFleetCommands"));
const { executeBlueprintAutoCommand, executeBlueprintCommand, } = require(path.join(__dirname, "../industry/industryChatCommands"));
const { executeBookmarkAutoCommand, } = require(path.join(__dirname, "../bookmark/bookmarkChatCommands"));
const { executeCalendarAutoCommand, } = require(path.join(__dirname, "../calendar/calendarChatCommands"));
const { executeReprocessingSmokeCommand, } = require(path.join(__dirname, "../reprocessing/reprocessingChatCommands"));
const { resolveWelcomeSenderID, sendMail, } = require(path.join(__dirname, "../mail/mailState"));
const { grantAllSkinsToCharacter, } = require(path.join(__dirname, "../ship/shipCosmeticsState"));
...
const DEER_HUNTER_MESSAGE = "Thank you, Deer_Hunter on Discord, for helping make EveJS Elysian possible with your contribution to rising AI development costs.";
...
function getLoyaltyPointSummary(session, issuerText = "") {
    ...
    if (normalizedIssuerText) {
        ...
        if (issuerResult.success === false) {
            return formatLoyaltyPointIssuerError(issuerResult, normalizedIssuerText);
        }
        const amount = getCharacterWalletLPBalance(session.characterID, issuerResult.data.corporationID);
...
function getFeedbackChannel(options) {
    ...
    const candidate = options.feedbackChannel || options.channelID || options.channelName || null;
...
function resolveCharacterTargetDescriptor(requestSession, targetToken) {
    ...
    if (normalizeCommandName(normalizedToken) === "me") {
        const characterID = normalizePositiveInteger(requestSession && requestSession.characterID);
        if (!characterID) {
            return {
                success: false,
        ...
        return {
            ...
            data: {
                characterID,
                record,
                session: sessionRegistry.findSessionByCharacterID(characterID) || requestSession || null,
            },
        };
    }
    ...
    const matchEntry = Object.entries(characters).find(([, record]) => (normalizeCommandName(record && record.characterName) === normalizedName));
    if (!matchEntry) {
        return {
            success: false,
            errorMsg: "TARGET_NOT_FOUND",
...
function formatCharacterTargetLabel(targetDescriptor) {
    ...
    const name = (targetDescriptor.record && targetDescriptor.record.characterName) ||
...
function resolveTrailingCharacterTargetDescriptor(requestSession, tokens = [], ownerStartIndex = 0) {
    ...
    if (fallbackTarget.success === false) {
...
function resolveStandingOwnerDescriptor(ownerToken) {
    ...
    const agentRecord = listAgents().find((entry) => normalizeCommandName(entry && entry.ownerName) === normalizedName);
    if (agentRecord) {
        return {
            success: true,
            data: {
...
function formatStandingOwnerLabel(ownerDescriptor) {
    ...
    const ownerName = (ownerDescriptor.ownerRecord && ownerDescriptor.ownerRecord.ownerName) ||
        null;
...
function suggestSkillNames(skillTypes, query) {
    ...
    return (Array.isArray(skillTypes) ? skillTypes : [])
        ...
        return String(left.skillType.name || "").localeCompare(String(right.skillType.name || ""));
    })
        .slice(0, 5)
...
function resolveSkillGrantDescriptor(skillToken) {
    ...
    if (numericTypeID) {
        const match = skillTypes.find((skillType) => Number(skillType && skillType.typeID) === numericTypeID);
        if (!match) {
            return {
                success: false,
    ...
    const exactMatches = skillTypes.filter((skillType) => (normalizeCommandName(skillType && skillType.name) === normalizedQuery));
...
function skillMutationLowersExistingLevels(skillRecords = [], previousSkillMap = null) {
    ...
    return dedupeSkillRecords(skillRecords).some((skillRecord) => {
        ...
        const previousPoints = Number(previousSkillRecord.trainedSkillPoints ?? previousSkillRecord.skillPoints ?? 0) || 0;
        const nextPoints = Number(skillRecord.trainedSkillPoints ?? skillRecord.skillPoints ?? 0) || 0;
        if (nextPoints < previousPoints) {
            return true;
        }
        const previousLevel = Number(previousSkillRecord.trainedSkillLevel ?? previousSkillRecord.skillLevel ?? 0) || 0;
        const nextLevel = Number(skillRecord.trainedSkillLevel ?? skillRecord.skillLevel ?? 0) || 0;
...
function queueSnapshotChanged(previousSnapshot = null, nextSnapshot = null) {
    ...
    const previousEntries = previousSnapshot && Array.isArray(previousSnapshot.queueEntries)
        ? previousSnapshot.queueEntries
        : [];
    const nextEntries = nextSnapshot && Array.isArray(nextSnapshot.queueEntries)
    ...
    for (let index = 0; index < previousEntries.length; index += 1) {
        ...
        if (normalizePositiveInteger(previousEntry && previousEntry.trainingTypeID) !==
...
function refreshLiveCharacterSkillSession(targetSession, targetDescriptor, changedSkillRecords = [], previousSkillMap = null, options = {}) {
    const characterID = normalizePositiveInteger(targetDescriptor.characterID);
    if (!characterID) {
        return;
    }
    ...
    const queueSnapshot = options && options.queueSnapshot && typeof options.queueSnapshot === "object"
        ? options.queueSnapshot
        : null;
    const hasQueueState = queueSnapshot &&
        Array.isArray(queueSnapshot.queueEntries);
    const hasFreeSkillPoints = options &&
    ...
    emitSkillSessionState(targetSession, characterID, changedSkills, {
        ...
        previousSkillMap,
        emitSkillLevelsTrained: options && Object.prototype.hasOwnProperty.call(options, "emitSkillLevelsTrained")
        ...
        emitQueuePaused: Boolean(hasQueueState &&
...
function applySkillGrantToCharacter(targetDescriptor, skillDescriptor, skillLevel, previousSkillMap = null) {
    const characterID = normalizePositiveInteger(targetDescriptor && targetDescriptor.characterID);
    if (!characterID) {
        return {
            success: false,
            errorMsg: "TARGET_NOT_FOUND",
    ...
    if (skillDescriptor.selector === "all") {
        ...
        return {
            success: true,
            data: {
                changedSkills: filterChangedSkillRecords(grantCharacterSkillLevels(characterID, publishedSkillTypes.map((skillType) => ({
    ...
    if (skillDescriptor.selector === "super") {
        ...
        return {
            success: true,
            data: {
                changedSkills: filterChangedSkillRecords(grantCharacterSkillLevels(characterID, targetSkillLevels), previousSkillMap),
            },
        };
    }
    if (skillDescriptor.selector === "single" && skillDescriptor.skillType) {
        return {
            success: true,
            data: {
                changedSkills: filterChangedSkillRecords(grantCharacterSkillLevels(characterID, [
                    {
...
function resolveShipLogoGrantTarget(session, lookupText) {
    ...
    if (!normalizedLookup || ["active", "current", "me"].includes(normalizeCommandName(normalizedLookup))) {
        ...
        return {
            ...
            data: {
                shipTypeID,
                shipLabel: (shipType && shipType.name) ||
...
function handleGrantShipLogoCommand(command, session, argumentText, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before granting ship emblem licenses.");
    }
    const usage = command === "grantshipemblem"
    ...
    const targetResult = resolveShipLogoGrantTarget(session, modeResult.remainingTokens.join(" "));
    if (targetResult.success === false) {
        if (targetResult.errorMsg === "ACTIVE_SHIP_REQUIRED") {
            return handledResult(chatHub, session, options, "No active ship was found. Pass a ship name or typeID, or board a ship first.");
        }
        if (targetResult.errorMsg === "SHIP_NOT_FOUND") {
            return handledResult(chatHub, session, options, `Ship type not found: ${targetResult.lookupText}.${formatSuggestions(targetResult.suggestions)}`.trim());
        }
        if (targetResult.errorMsg === "AMBIGUOUS_SHIP_NAME") {
            return handledResult(chatHub, session, options, `Ship type is ambiguous: ${targetResult.lookupText}.${formatSuggestions(targetResult.suggestions)}`.trim());
        }
    ...
    const licenseEntries = requestedEntitlementTypes.map((entitlementType) => ({
        entitlementType,
        license: getLicenseByShipAndCosmeticType(shipTypeID, entitlementType === SHIP_LOGO_ENTITLEMENT_ALLIANCE
    ...
    if (missingLicenseEntry) {
        return handledResult(chatHub, session, options, `No cached ${getShipLogoGrantTypeLabel(missingLicenseEntry.entitlementType)} emblem license exists for ${shipLabel} (shipTypeID=${shipTypeID}).`);
    }
    const grantedEntries = [];
    const alreadyOwnedEntries = [];
    for (const entry of licenseEntries) {
        const grantResult = grantShipLogoEntitlement(session.characterID, shipTypeID, entry.entitlementType, {
            source: "admin_grant",
        });
    ...
    const renderLicenseIDList = (entries) => entries.map((entry) => entry.license.fsdTypeID).join(", ");
    ...
    if (grantedEntries.length === 0) {
        ...
        return handledResult(chatHub, session, options, `${requestedLabel.charAt(0).toUpperCase()}${requestedLabel.slice(1)} emblem license${requestedEntitlementTypes.length === 1 ? "" : "s"} for ${shipLabel} ${requestedPlural} already owned.`);
    }
...
function parseNpcSpawnArguments(argumentText, options = {}) {
    const defaultAmount = Math.max(1, Number.parseInt(options.defaultAmount, 10) || 1);
    const trimmed = String(argumentText || "").trim();
    if (!trimmed) {
        return {
            success: true,
...
function getAccountRecordForSession(session) {
    ...
    const matchedEntry = Object.entries(accounts).find(([, account]) => Number(account && account.id) === Number(session.userid || 0));
    if (!matchedEntry) {
        return null;
    }
    return {
...
function getChatColorLabel(roleValue) {
    ...
    const match = Object.entries(CHAT_ROLE_PROFILES).find(([, profileRole]) => normalizeRoleValue(profileRole, 0n) === normalizedRole);
...
function updateSessionRole(session, nextRole) {
    ...
    const previousRole = composeSessionRoleMask(session.accountRole ?? session.role, session.chatRole ?? session.role);
    const nextSessionRole = composeSessionRoleMask(session.accountRole ?? session.role, normalizedNextRole);
...
function handleChatColorCommand(session, colorName, chatHub, options) {
    ...
    if (!nextRole) {
        return handledResult(chatHub, session, options, `Unknown chat color: ${colorName}. Use /blue, /red, /teal, or /yellow.`);
    }
    const persisted = persistSessionChatRole(session, nextRole);
    if (persisted.success === false) {
        return handledResult(chatHub, session, options, "Chat color change failed: account record could not be updated.");
    }
    updateSessionRole(session, nextRole);
    return handledResultWithExtras(chatHub, session, options, `Chat color set to ${getChatColorLabel(nextRole)}.`, {
...
function handleMailMeCommand(session, argumentText, chatHub, options) {
    const characterID = normalizePositiveInteger(session && (session.characterID || session.charID || session.charid), 0);
    if (!characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /mailme.");
    }
    ...
    if (!sendResult.success) {
        return handledResult(chatHub, session, options, `Test mail failed: ${sendResult.errorMsg || "unknown error"}.`);
    }
    return handledResult(chatHub, session, options, `Live Eve Mail sent to your mailbox: "${subject}".`);
}
...
function syncSpaceRootInventoryChangesToSession(session, changes = []) {
    const numericSystemID = normalizePositiveInteger(session &&
        session._space &&
    ...
    const filteredChanges = (Array.isArray(changes) ? changes : []).filter((change) => {
        ...
        return ((nextLocationID === numericSystemID && Number(item && item.flagID) === 0) ||
...
function resolveSessionSolarSystemID(session) {
    return normalizePositiveInteger(session &&
...
function healDockedShipForSession(session) {
    ...
    syncInventoryItemForSession(session, updateResult.data, updateResult.previousData || {}, { emitCfgLocation: true });
    return updateResult;
}
...
function damageDockedShipForSession(session, preset) {
    ...
    const fittedModules = listFittedItems(session.characterID, activeShip.itemID).filter((item) => isFittedModuleItem(item));
    for (const fittedModule of fittedModules) {
        const moduleUpdateResult = updateInventoryItem(fittedModule.itemID, (currentItem) => ({
            ...currentItem,
...
function resolveTransportDungeonIdentity(value) {
    ...
    const instanceID = normalizePositiveInteger(value.destinationStaticInstanceID) ||
    ...
    return {
        instanceID,
        roomKey: String(value.destinationDungeonRoomKey ||
            value.dungeonCurrentRoomKey ||
            value.dungeonRoomKey ||
            "room:entry").trim() || "room:entry",
        siteID: normalizePositiveInteger(value.destinationDungeonSiteID) ||
...
function initializeTransportDestinationDungeon(scene, destination, session = null, options = {}) {
    ...
    const landscapeSceneService = dependencies.landscapeSceneService || require(path.join(__dirname, "../../space/frontierLandscapeSceneService"));
    const identity = findTransportDestinationDungeonIdentity(scene, destination);
    let universeResult = null;
    if (identity) {
        const dungeonService = dependencies.dungeonService || require(path.join(__dirname, "../dungeon/dungeonUniverseSiteService"));
        ...
        universeResult = dungeonService.ensureSiteContentsMaterialized(scene, {
    ...
    const materializeLandscape = landscapeSceneService && (typeof landscapeSceneService.materializeNearbyLandscapeSites === "function"
    ...
    if (typeof materializeLandscape === "function") {
        landscapeResult = materializeLandscape(scene, { position: cloneSpaceVector(destination.point), session: session || null }, {
            broadcast: true,
            excludedSession: session || null,
            nowMs: options.nowMs,
            session: session || null,
...
function getSessionTransportEntity(session) {
    ...
    return spaceRuntime.getEntity(session, session._space && session._space.shipID);
}
function buildTransportPointAnchor(entity, fallbackSystemID = null) {
    if (!entity) {
        return null;
    }
    const systemID = normalizePositiveInteger(entity.systemID || entity.solarSystemID) ||
...
function getSessionTransportAnchor(session) {
    ...
    if (shipEntity) {
        const shipAnchor = buildTransportPointAnchor(shipEntity, getSessionCurrentSolarSystemID(session));
        return shipAnchor
            ? {
                ...shipAnchor,
    ...
    return {
        kind: "solarSystem",
        solarSystemID,
        label: (solarSystem && solarSystem.solarSystemName) ||
...
function findStaticTransportAnchorByID(entityID) {
    ...
    for (const solarSystem of worldData.getSolarSystems()) {
        const match = worldData.getStaticSceneForSystem(solarSystem.solarSystemID).find((candidate) => Number(candidate && candidate.itemID) === numericEntityID);
        if (match) {
            if (match.stationID) {
                return {
                    kind: "station",
                    stationID: numericEntityID,
                    label: match.stationName ||
...
function resolveTransportPointContext(session, targetDescriptor) {
    ...
    if (targetDescriptor && targetDescriptor.kind === "entity") {
        return buildTransportPointAnchor(targetDescriptor.entity, targetDescriptor.systemID);
    }
    return null;
}
function resolveTransportLocationToken(session, targetDescriptor, token) {
    ...
    if (solarSystem) {
        return {
            ...
            label: solarSystem.solarSystemName ||
...
function formatTransportTargetLabel(targetDescriptor) {
    ...
    if (targetDescriptor.kind === "session") {
        ...
        return ((targetSession && targetSession.characterName) ||
...
async function executeSessionTransportTarget(requestSession, targetDescriptor, destination, chatHub, options) {
    ...
    if (!targetSession || !targetSession.characterID) {
        return handledResult(chatHub, requestSession, options, "Transport target session is not available.");
    }
    ...
    if (destination.kind === "solarSystem") {
        ...
        const result = jumpSessionToStation(targetSession, destination.stationID);
        if (!result.success) {
            return handledResult(chatHub, requestSession, options, formatTransportTransitionError(result, `Failed to transport ${targetLabel} to ${destinationLabel}.`));
        }
        crossedLocationBoundary = true;
        ...
        if (!destinationSystemID || !destination.point) {
            return handledResult(chatHub, requestSession, options, "Point transport is missing a valid solar-system location.");
        }
        ...
        if (!teleportResult.success) {
            return handledResult(chatHub, requestSession, options, teleportResult.errorMsg === "NOT_IN_SPACE"
                ? `Failed to transport ${targetLabel}: target is not in space.`
                : `Failed to teleport ${targetLabel} in space.`);
        }
        const destinationDungeonIdentity = dungeonInitialization.data && dungeonInitialization.data.identity;
        if (destinationDungeonIdentity &&
            destinationScene &&
            typeof destinationScene.commitAcceptedPilotWarpDungeonContext === "function") {
            try {
                destinationScene.commitAcceptedPilotWarpDungeonContext(teleportResult.data && teleportResult.data.entity, {
                ...
                log.warn(`[ChatCommands] Teleport dungeon context commit failed for character=${Number(targetSession.characterID) || 0} instance=${destinationDungeonIdentity.instanceID}: ${error.message}`);
            }
        }
    }
    else {
        return handledResult(chatHub, requestSession, options, `Unsupported /tr destination: ${destination.kind}.`);
    }
    if (targetSession === requestSession && crossedLocationBoundary) {
        if (destination.kind !== "station") {
            const destinationSystemID = destination.kind === "solarSystem"
                ? destination.solarSystemID
                : destination.kind === "point"
                    ? destination.systemID
                    : normalizePositiveInteger((worldData.getStationByID(destination.stationID) || {}).solarSystemID);
        ...
        return handledResult(chatHub, requestSession, getPostLocalMoveFeedbackOptions(options), `Transported ${targetLabel} to ${destinationLabel}.`);
    }
    return handledResult(chatHub, requestSession, options, `Transported ${targetLabel} to ${destinationLabel}.`);
}
function buildNearbySpaceSpawnState(shipEntity, distanceMeters = 250) {
    ...
    const direction = normalizeSpaceVector(shipEntity && shipEntity.direction, { x: 1, y: 0, z: 0 });
    return {
...
function buildOffsetSpaceSpawnState(shipEntity, distanceMeters = 20_000) {
    ...
    const position = addVectors(origin, scaleVector(offsetDirection, Math.max(1_000, Number(distanceMeters) || 20_000)));
    return {
        position,
        velocity: { x: 0, y: 0, z: 0 },
        direction: normalizeSpaceVector(buildRandomUnitVector(), shipEntity && shipEntity.direction),
...
function buildPalatineKeepstarVisualEntity(session, anchorEntity) {
    ...
    const spawnDistance = Math.max(KEEPSTAR_DEFAULT_SPAWN_DISTANCE_METERS, (Number(keepstarType.radius) || KEEPSTAR_DEFAULT_RADIUS_METERS) + 200000);
...
function buildFormationBasis(direction) {
    ...
    const upReference = Math.abs(Number(forward.y) || 0) >= 0.95
        ? { x: 1, y: 0, z: 0 }
        : { x: 0, y: 1, z: 0 };
    const right = normalizeSpaceVector(crossVectors(forward, upReference), { x: 0, y: 0, z: 1 });
    const up = normalizeSpaceVector(crossVectors(right, forward), upReference);
...
function buildFire2FleetShipList(shipTypes, fleetSize) {
    ...
    for (let index = 0; index < normalizedSize; index += 1) {
        const bucketIndex = Math.min(shipTypes.length - 1, Math.floor((index * shipTypes.length) / normalizedSize));
...
function buildFire2FleetFormation(anchorEntity, shipTypes, fleetSize) {
    ...
    const formationOrigin = addVectors(anchorPosition, scaleVector(basis.forward, FIRE2_BASE_DISTANCE_METERS));
    const facingDirection = normalizeSpaceVector(subtractVectors(anchorPosition, formationOrigin), scaleVector(basis.forward, -1));
    const layout = [];
    let shipIndex = 0;
    ...
    for (const rowCount of rowCounts) {
        ...
        const rowMaxRadius = rowShips.reduce((largest, shipType) => Math.max(largest, getShipRadiusMeters(shipType)), 0);
        if (layout.length > 0) {
            rowDistanceMeters += Math.max(FIRE2_ROW_SPACING_METERS, previousRowMaxRadius + rowMaxRadius + FIRE2_OVERLAP_PADDING_METERS);
        }
        const lateralSpacingMeters = Math.max(FIRE2_LATERAL_SPACING_METERS, (rowMaxRadius * 2) + FIRE2_OVERLAP_PADDING_METERS);
        const verticalSpacingMeters = Math.max(2_500, rowMaxRadius + (FIRE2_OVERLAP_PADDING_METERS * 0.75));
        const wingSweepBackMeters = Math.max(1_250, Math.min(3_500, (rowMaxRadius * 0.45) + (FIRE2_OVERLAP_PADDING_METERS * 0.25)));
        ...
        for (let slotIndex = 0; slotIndex < rowShips.length; slotIndex += 1) {
            ...
            const verticalOffsetMeters = lane === 0
                ...
                : verticalDirection
                    ...
                    * 0.55;
            const position = addVectors(addVectors(formationOrigin, scaleVector(basis.forward, rowDistanceMeters - wingPullbackMeters + centerAdvanceMeters)), addVectors(scaleVector(basis.right, lateralOffsetMeters), scaleVector(basis.up, verticalOffsetMeters)));
            layout.push({
...
function isNonTechOneVariant(row) {
    ...
    const metaGroupID = getNumericTypeAttributeValue(typeID, "metaGroupID", Number(row && row.metaGroupID) || 0);
    return (techLevel >= 2 ||
        metaGroupID > 1 ||
...
function isCombatWeaponType(row) {
    ...
    return (typeHasEffectName(typeID, "turretFitted") ||
...
function getMineralsSeedPlan() {
    ...
    const mineralTypes = dedupeItemTypes(rows
        .filter(isPublishedMineralRow)
        .map((row) => resolveItemByTypeID(row.typeID))
        .filter(Boolean)).sort(sortItemTypesByName);
    const oreTypes = dedupeItemTypes(rows
...
function getGmWeaponsSeedPlan() {
    ...
    const rows = readStaticRows(TABLE.ITEM_TYPES) || [];
    const weaponTypes = dedupeItemTypes(rows
    ...
    const ammoTypes = dedupeItemTypes(rows
...
function syncStationHangarChangesToSession(session, stationID, changes = []) {
    ...
    for (const change of Array.isArray(changes) ? changes : []) {
        ...
        syncInventoryItemForSession(session, change.item, change.previousState || {
            locationID: 0,
...
function grantStationHangarBatchAndSyncSession(session, stationID, entries = []) {
    const result = grantItemsToCharacterStationHangar(Number(session && session.characterID) || 0, stationID, entries);
    if (result.success) {
        syncStationHangarChangesToSession(session, stationID, result.data && result.data.changes);
    }
    return result;
}
...
function continueGmWeaponsSeedJob(job, chatHub) {
    ...
    try {
        const nextEntries = job.entries.slice(job.nextIndex, job.nextIndex + GM_WEAPONS_BATCH_SIZE);
        if (nextEntries.length === 0) {
            activeGmWeaponsJobs.delete(job.characterID);
            chatHub.sendSystemMessage(job.session, [
            ...
            ].filter(Boolean).join(" "), job.feedbackChannel);
            return;
        }
        const grantResult = grantStationHangarBatchAndSyncSession(job.session, job.stationID, nextEntries);
        if (!grantResult.success) {
        ...
        activeGmWeaponsJobs.delete(job.characterID);
        chatHub.sendSystemMessage(job.session, ` /gmweapons failed after ${job.nextIndex}/${job.entries.length} grants: ${error.message}`.trim(), job.feedbackChannel);
    }
}
function getPropulsionCommandItemTypes() {
    ...
    cachedPropulsionCommandTypes = rows
        ...
        return (isTechTwoPropulsionName(name) ||
        ...
        .filter((itemType, index, list) => list.findIndex((candidate) => Number(candidate.typeID) === Number(itemType.typeID)) === index)
...
function canPlannedModulesStayOnline(charID, shipItem, plannedModules) {
    ...
    return {
        resourceState,
        success: resourceState.cpuLoad <= resourceState.cpuOutput + 1e-6 &&
            resourceState.powerLoad <= resourceState.powerOutput + 1e-6,
    };
}
function tryPlanNextModuleFit(charID, shipItem, itemType, fittedItems) {
    const nextFlagID = selectAutoFitFlagForType(shipItem, fittedItems, Number(itemType && itemType.typeID) || 0);
    if (!nextFlagID) {
        return {
            success: false,
    ...
    const probeItem = buildPlannedFittedModuleItem(charID, shipItem, itemType, nextFlagID, -1000 - fittedItems.length);
    const validation = validateFitForShip(charID, shipItem, probeItem, nextFlagID, fittedItems);
    ...
    if (resourceCheck.success === false) {
        return {
            success: false,
            errorMsg: resourceCheck.resourceState.cpuLoad > resourceCheck.resourceState.cpuOutput + 1e-6
                ? "NOT_ENOUGH_CPU"
                : "NOT_ENOUGH_POWER",
            data: {
...
function getCompatibleModuleChargeTypes(moduleTypeID, filterFn = null) {
    ...
    return dedupeItemTypes(rows
        ...
        .filter((itemType) => typeof filterFn !== "function" || filterFn(itemType))).sort(sortItemTypesByName);
}
...
function getCompatibleLesmisHeavyMissileTypes(moduleTypeID) {
    ...
    cachedLesmisHeavyMissileTypes = getCompatibleModuleChargeTypes(moduleTypeID, (itemType) => /^(Inferno|Mjolnir|Nova|Scourge)(?: (Fury|Precision))? Heavy Missile$/i.test(String(itemType && itemType.name || "").trim()));
    return cachedLesmisHeavyMissileTypes;
}
...
function buildPresetSingleRackCommandPlan(charID, shipItem, preset) {
    const propulsionType = resolveItemByName(String(preset && preset.propulsionName || LASER_COMMAND_MWD_NAME));
    ...
    for (let index = 0; index < moduleCount; index += 1) {
        const nextFit = tryPlanNextModuleFit(charID, shipItem, moduleType.match, plannedItems);
        if (!nextFit.success) {
            return {
                success: false,
    ...
    const totalChargeVolume = chargeTypes.reduce((sum, itemType) => sum +
        ((Number(itemType && itemType.volume) || 0) * (Number(preset && preset.chargeQuantity) || 0)), 0);
...
function buildLesmisCommandPlan(charID, shipItem) {
    ...
    for (let index = 0; index < LESMIS_COMMAND_LAUNCHER_COUNT; index += 1) {
        const nextFit = tryPlanNextModuleFit(charID, shipItem, launcherType.match, plannedItems);
        if (!nextFit.success) {
            return {
                success: false,
    ...
    for (let index = 0; index < LESMIS_COMMAND_TURRET_COUNT; index += 1) {
        const nextFit = tryPlanNextModuleFit(charID, shipItem, turretType.match, plannedItems);
        if (!nextFit.success) {
            return {
                success: false,
    ...
    const crystalVolume = crystalTypes.reduce((sum, itemType) => sum +
        ((Number(itemType && itemType.volume) || 0) * LASER_COMMAND_MIN_CRYSTALS_PER_TYPE), 0);
    const missileVolume = missileTypes.reduce((sum, itemType) => sum +
        ((Number(itemType && itemType.volume) || 0) * LESMIS_COMMAND_MISSILES_PER_TYPE), 0);
...
function fitGrantedItemTypeToShip(session, stationID, shipItem, itemType, count, chatHub, options) {
    ...
    for (let index = 0; index < numericCount; index += 1) {
        ...
        const nextFit = tryPlanNextModuleFit(session.characterID, shipItem, itemType, fittedItems);
        if (!nextFit.success) {
            break;
        }
        const moveResult = moveItemTypeFromCharacterLocation(session.characterID, stationID, ITEM_FLAGS.HANGAR, shipItem.itemID, nextFit.data.flagID, itemType.typeID, 1);
...
function moveGrantedItemTypeToShipCargo(session, stationID, shipItem, itemType, quantity) {
    const moveResult = moveItemTypeFromCharacterLocation(session.characterID, stationID, ITEM_FLAGS.HANGAR, shipItem.itemID, ITEM_FLAGS.CARGO_HOLD, itemType.typeID, quantity);
    if (!moveResult.success) {
        return moveResult;
    }
    syncInventoryChangesToSession(session, moveResult.data && moveResult.data.changes);
...
function boardStoredShipLikeHangarBoard(session, stationID, shipItem, options = {}) {
    ...
    const shipService = serviceManager && typeof serviceManager.lookup === "function"
        ? serviceManager.lookup("ship")
        : null;
    if (shipService && typeof shipService.callMethod === "function") {
        shipService.callMethod("BoardStoredShip", [stationID, shipItem.itemID], session, null);
        const activeShip = getActiveShipRecord(session.characterID);
        return {
            success: Boolean(activeShip && Number(activeShip.itemID) === Number(shipItem.itemID)),
            activeShip,
            errorMsg: activeShip && Number(activeShip.itemID) === Number(shipItem.itemID)
                ? null
...
function handlePresetSingleRackCommand(session, chatHub, options, preset, commandName) {
    ...
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, `Select a character before using ${slashCommand}.`);
    }
    const stationID = Number(getDockedLocationID(session) || 0);
    if (stationID <= 0) {
        return handledResult(chatHub, session, options, `You must be docked before using ${slashCommand}.`);
    }
    const shipType = resolveShipByName(String(preset && preset.shipName || ""));
    if (!shipType || !shipType.success || !shipType.match) {
        return handledResult(chatHub, session, options, `Ship type not found for ${slashCommand}: ${String(preset && preset.shipName || "")}.`);
    }
    const spawnResult = spawnShipInHangarForSession(session, shipType.match);
    if (!spawnResult.success || !spawnResult.ship) {
        return handledResult(chatHub, session, options, `Failed to spawn the ${slashCommand} hull in your station hangar.`);
    }
    ...
    if (planResult.success === false) {
        return handledResult(chatHub, session, options, `Unable to build the ${slashCommand} fit: ${planResult.errorMsg}.`);
    }
    ...
    const grantResult = grantStationHangarBatchAndSyncSession(session, stationID, grantEntries);
    if (!grantResult.success) {
        return handledResult(chatHub, session, options, `Unable to seed the ${slashCommand} modules and ammo: ${grantResult.errorMsg || "WRITE_ERROR"}.`);
    }
    const propulsionFitResult = fitGrantedItemTypeToShip(session, stationID, shipItem, fitPlan.propulsionType, 1, chatHub, options);
    if (propulsionFitResult.success === false || propulsionFitResult.data.fittedCount !== 1) {
        return handledResult(chatHub, session, options, `The ${slashCommand} hull spawned, but the MWD fit failed: ${propulsionFitResult.errorMsg || "FIT_FAILED"}.`);
    }
    const moduleFitResult = fitGrantedItemTypeToShip(session, stationID, shipItem, fitPlan.moduleType, fitPlan.moduleCount, chatHub, options);
    if (moduleFitResult.success === false || moduleFitResult.data.fittedCount !== fitPlan.moduleCount) {
        return handledResult(chatHub, session, options, `The ${slashCommand} hull spawned, but the ${String(preset && preset.moduleFailureLabel || "module")} fit failed: ${moduleFitResult.errorMsg || "FIT_FAILED"}.`);
    }
    for (const chargeType of fitPlan.chargeTypes) {
        const cargoMoveResult = moveGrantedItemTypeToShipCargo(session, stationID, shipItem, chargeType, Number(preset && preset.chargeQuantity) || 0);
        if (!cargoMoveResult.success) {
            return handledResult(chatHub, session, options, `The ${slashCommand} hull spawned, but cargo seeding failed on ${chargeType.name}: ${cargoMoveResult.errorMsg || "MOVE_FAILED"}.`);
        }
    }
...
function handleLesmisCommand(session, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /lesmis.");
    }
    const stationID = Number(getDockedLocationID(session) || 0);
    if (stationID <= 0) {
        return handledResult(chatHub, session, options, "You must be docked before using /lesmis.");
    }
    const shipType = resolveShipByName(LESMIS_COMMAND_SHIP_NAME);
    if (!shipType || !shipType.success || !shipType.match) {
        return handledResult(chatHub, session, options, `Ship type not found for /lesmis: ${LESMIS_COMMAND_SHIP_NAME}.`);
    }
    const spawnResult = spawnShipInHangarForSession(session, shipType.match);
    if (!spawnResult.success || !spawnResult.ship) {
        return handledResult(chatHub, session, options, "Failed to spawn the /lesmis hull in your station hangar.");
    }
    const shipItem = spawnResult.ship;
    const planResult = buildLesmisCommandPlan(session.characterID, shipItem);
    if (planResult.success === false) {
        return handledResult(chatHub, session, options, `Unable to build the /lesmis fit: ${planResult.errorMsg}.`);
    }
    const fitPlan = planResult.data;
    const grantEntries = [
        {
    ...
    const grantResult = grantStationHangarBatchAndSyncSession(session, stationID, grantEntries);
    if (!grantResult.success) {
        return handledResult(chatHub, session, options, `Unable to seed the /lesmis fit and ammo: ${grantResult.errorMsg || "WRITE_ERROR"}.`);
    }
    const propulsionFitResult = fitGrantedItemTypeToShip(session, stationID, shipItem, fitPlan.propulsionType, 1, chatHub, options);
    if (propulsionFitResult.success === false || propulsionFitResult.data.fittedCount !== 1) {
        return handledResult(chatHub, session, options, `The /lesmis hull spawned, but the MWD fit failed: ${propulsionFitResult.errorMsg || "FIT_FAILED"}.`);
    }
    const launcherFitResult = fitGrantedItemTypeToShip(session, stationID, shipItem, fitPlan.launcherType, fitPlan.launcherCount, chatHub, options);
    if (launcherFitResult.success === false ||
        launcherFitResult.data.fittedCount !== fitPlan.launcherCount) {
        return handledResult(chatHub, session, options, `The /lesmis hull spawned, but the launcher fit failed: ${launcherFitResult.errorMsg || "FIT_FAILED"}.`);
    }
    const turretFitResult = fitGrantedItemTypeToShip(session, stationID, shipItem, fitPlan.turretType, fitPlan.turretCount, chatHub, options);
    if (turretFitResult.success === false || turretFitResult.data.fittedCount !== fitPlan.turretCount) {
        return handledResult(chatHub, session, options, `The /lesmis hull spawned, but the turret fit failed: ${turretFitResult.errorMsg || "FIT_FAILED"}.`);
    }
    for (const crystalType of fitPlan.crystalTypes) {
        const cargoMoveResult = moveGrantedItemTypeToShipCargo(session, stationID, shipItem, crystalType, LASER_COMMAND_MIN_CRYSTALS_PER_TYPE);
        if (!cargoMoveResult.success) {
            return handledResult(chatHub, session, options, `The /lesmis hull spawned, but laser crystal cargo seeding failed on ${crystalType.name}: ${cargoMoveResult.errorMsg || "MOVE_FAILED"}.`);
        }
    }
    for (const missileType of fitPlan.missileTypes) {
        const cargoMoveResult = moveGrantedItemTypeToShipCargo(session, stationID, shipItem, missileType, LESMIS_COMMAND_MISSILES_PER_TYPE);
        if (!cargoMoveResult.success) {
            return handledResult(chatHub, session, options, `The /lesmis hull spawned, but missile cargo seeding failed on ${missileType.name}: ${cargoMoveResult.errorMsg || "MOVE_FAILED"}.`);
        }
    }
    const activationResult = boardStoredShipLikeHangarBoard(session, stationID, shipItem, options);
    if (activationResult.success === false) {
        return handledResult(chatHub, session, options, `The /lesmis ship was spawned and fitted, but boarding it failed: ${activationResult.errorMsg || "BOARD_FAILED"}.`);
    }
    const finalResourceState = buildShipResourceState(session.characterID, shipItem);
    return handledResult(chatHub, session, options, [
        `${shipType.match.name} was added to your ship hangar as ship ${shipItem.itemID}.`,
...
function refreshAffiliationSessions(characterIDs) {
    const targetCharacterIDs = new Set((Array.isArray(characterIDs) ? characterIDs : [])
    ...
    for (const targetSession of sessionRegistry.getSessions()) {
        const characterID = normalizePositiveInteger(targetSession && (targetSession.characterID || targetSession.charid));
...
function reconcileSolarTargetSessionIdentity(session, solarSystem) {
    ...
    const targetSolarSystemID = normalizePositiveInteger(solarSystem.solarSystemID) || null;
    const targetConstellationID = normalizePositiveInteger(solarSystem.constellationID) || null;
    const targetRegionID = normalizePositiveInteger(solarSystem.regionID) || null;
    if (!targetSolarSystemID) {
        return false;
    }
    const sessionChanges = {};
    const applyChange = (key, nextValue, aliases) => {
        const previousValue = normalizePositiveInteger(aliases.map((alias) => session[alias]).find((value) => value !== undefined));
...
function getWalletSummary(session) {
    ...
    const deltaText = wallet.balanceChange === 0
        ? "0.00 ISK"
        : `${wallet.balanceChange > 0 ? "+" : ""}${formatIsk(wallet.balanceChange)}`;
    const evermarks = getCharacterWalletLPBalance(session.characterID, EVERMARK_ISSUER_CORP_ID);
...
function formatConnectedCharacterLocation(session) {
    ...
    const systemLabel = (currentSystem && currentSystem.solarSystemName) ||
    ...
    const dockedLocationLabel = (station && station.stationName) ||
...
function handleShipSpawn(commandLabel, session, argumentText, chatHub, options) {
    if (!argumentText) {
        return handledResult(chatHub, session, options, `Usage: /${commandLabel} <ship name|typeID>`);
    }
    const shipLookup = resolveShipByName(argumentText);
    if (!shipLookup.success) {
        const message = shipLookup.errorMsg === "SHIP_NOT_FOUND"
    ...
    return handledResult(chatHub, session, options, `${shipLookup.match.name}${shipLookup.match.published === false ? " [unpublished]" : ""} was added to your ship hangar. /${commandLabel} only spawns the hull for now; board it manually from the hangar.`);
}
function handleGiveItemCommand(session, argumentText, chatHub, options) {
    const trimmedArgument = String(argumentText || "").trim();
    if (!trimmedArgument) {
        return handledResult(chatHub, session, options, "Usage: /giveitem <item name|typeID> [amount]");
    }
    ...
    if (!normalizedAmount) {
        return handledResult(chatHub, session, options, "Item amount must be a positive whole number.");
    }
    if (!Number.isSafeInteger(normalizedAmount)) {
        return handledResult(chatHub, session, options, "Item amount is too large. Use a positive whole number up to 9,007,199,254,740,991.");
    }
    if (!itemLookup.success) {
        const message = itemLookup.errorMsg === "ITEM_NOT_FOUND"
    ...
    const giveResult = giveItemToHangarForSession(session, itemLookup.match, normalizedAmount);
    ...
    const stackMode = changedItems.length === 1 && Number(changedItems[0].singleton || 0) === 0;
    ...
    const splitNote = giveResult.data &&
        ...
        : "";
    return handledResult(chatHub, session, options, `${summary}${itemLookup.match.published === false ? " [unpublished]" : ""} was added to your station hangar.${splitNote}`);
}
function handleCreateItemCommand(session, argumentText, chatHub, options, commandName = "createitem", behavior = {}) {
    const slashCommand = `/${String(commandName || "createitem").trim() || "createitem"}`;
    const trimmedArgument = String(argumentText || "").trim();
    if (!trimmedArgument) {
        return handledResult(chatHub, session, options, `Usage: ${slashCommand} <item name|typeID> [amount]`);
    }
    let itemLookup = resolveItemByName(trimmedArgument);
    let normalizedAmount = 1;
    if (!itemLookup.success) {
    ...
    const giveResult = createInShipCargo
        ? giveItemToActiveShipCargoForSession(session, itemLookup.match, normalizedAmount, behavior.shipGrantOptions || {})
        : giveItemToHangarForSession(session, itemLookup.match, normalizedAmount);
    if (!giveResult.success) {
        ...
        else if (giveResult.errorMsg === "ACTIVE_SHIP_NOT_FOUND" ||
...
function handleFrontierLoadCommand(session, argumentText, chatHub, options) {
    ...
    if (!match) {
        return handledResult(chatHub, session, options, "Usage: /load me <item name|typeID> [amount]");
    }
    return handleCreateItemCommand(session, match[1], chatHub, options, "load", {
        forceShipCargo: true,
...
function handleMineralsCommand(session, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /minerals.");
    }
    const stationID = Number(getDockedLocationID(session) || 0) || 0;
    if (stationID <= 0) {
        return handledResult(chatHub, session, options, "You must be docked before using /minerals.");
    }
    const plan = getMineralsSeedPlan();
    if (!plan.entries.length) {
        return handledResult(chatHub, session, options, "No published mineral or ore types matched the /minerals filter in local reference data.");
    }
    ...
    const clearResult = removeStationHangarBatchAndSyncSession(session, stationID, existingItemIDs);
    if (!clearResult.success) {
        return handledResult(chatHub, session, options, "Mineral and ore reset failed while clearing existing test stocks.");
    }
    const grantResult = grantStationHangarBatchAndSyncSession(session, stationID, plan.entries);
    if (!grantResult.success) {
        return handledResult(chatHub, session, options, "Mineral and ore grant failed.");
    }
    return handledResult(chatHub, session, options, [
...
function handleSpawnContainerCommand(session, argumentText, chatHub, options) {
    if (!isSpaceSessionReady(session)) {
        return handledResult(chatHub, session, options, "You must be in space before using /spawncontainer.");
    }
    const containerLookup = resolveSpaceItemType(argumentText, DEFAULT_SPACE_CONTAINER_NAME, isContainerType, "CONTAINER");
    if (!containerLookup.success) {
        const message = containerLookup.errorMsg === "ITEM_NOT_FOUND"
    ...
    const createResult = createSpaceItemForCharacter(session.characterID, session._space.systemID, containerLookup.match, buildNearbySpaceSpawnState(shipEntity));
    ...
    const spawnResult = spaceRuntime.spawnDynamicInventoryEntity(session._space.systemID, createResult.data.itemID);
    if (!spawnResult.success) {
        return handledResult(chatHub, session, options, "Container item was created, but the space ball failed to spawn.");
    }
    return handledResult(chatHub, session, options, `Spawned ${containerLookup.match.name} (${createResult.data.itemID}) in space.`);
}
function handleSpawnWreckCommand(session, argumentText, chatHub, options) {
    if (!isSpaceSessionReady(session)) {
        return handledResult(chatHub, session, options, "You must be in space before using /spawnwreck.");
    }
    const wreckLookup = resolveSpaceItemType(argumentText, DEFAULT_SPACE_WRECK_NAME, isWreckType, "WRECK");
    if (!wreckLookup.success) {
        const message = wreckLookup.errorMsg === "ITEM_NOT_FOUND"
    ...
    const shipEntity = spaceRuntime.getEntity(session, session._space.shipID);
    const createResult = createSpaceItemForCharacter(session.characterID, session._space.systemID, wreckLookup.match, {
    ...
    const spawnResult = spaceRuntime.spawnDynamicInventoryEntity(session._space.systemID, createResult.data.itemID);
    if (!spawnResult.success) {
        return handledResult(chatHub, session, options, "Wreck item was created, but the space ball failed to spawn.");
    }
    return handledResult(chatHub, session, options, `Spawned ${wreckLookup.match.name} (${createResult.data.itemID}) in space.`);
}
function handleJetcanCommand(session, argumentText, chatHub, options) {
    if (!isSpaceSessionReady(session)) {
        return handledResult(chatHub, session, options, "You must be in space before using /jetcan.");
    }
    const trimmedArgument = String(argumentText || "").trim();
    if (!trimmedArgument) {
        return handledResult(chatHub, session, options, "Usage: /jetcan <item name|typeID> [amount]");
    }
    let itemLookup = resolveItemByName(trimmedArgument);
    let normalizedAmount = 1;
    if (!itemLookup.success) {
    ...
    if (!normalizedAmount) {
        return handledResult(chatHub, session, options, "Jetcan amount must be a positive whole number.");
    }
    if (!itemLookup.success) {
        const message = itemLookup.errorMsg === "ITEM_NOT_FOUND"
            ? `Item not found: ${trimmedArgument}.${formatSuggestions(itemLookup.suggestions)}`
    ...
    const cargoItems = listContainerItems(session.characterID, shipID, ITEM_FLAGS.CARGO_HOLD).filter((item) => Number(item.typeID) === Number(itemLookup.match.typeID));
    const availableQuantity = cargoItems.reduce((sum, item) => (sum + (Number(item.singleton) === 1 ? 1 : Math.max(0, Number(item.stacksize || item.quantity || 0)))), 0);
    if (availableQuantity < normalizedAmount) {
        return handledResult(chatHub, session, options, `Not enough ${itemLookup.match.name} in cargo. Available: ${availableQuantity.toLocaleString("en-US")}.`);
    }
    const containerLookup = resolveSpaceItemType(DEFAULT_SPACE_CONTAINER_NAME, DEFAULT_SPACE_CONTAINER_NAME, isContainerType, "CONTAINER");
    if (!containerLookup.success) {
        return handledResult(chatHub, session, options, "Default jetcan container type could not be resolved.");
    }
    ...
    const createResult = createSpaceItemForCharacter(session.characterID, session._space.systemID, containerLookup.match, {
    ...
    const moveResult = moveItemTypeFromCharacterLocation(session.characterID, shipID, ITEM_FLAGS.CARGO_HOLD, createResult.data.itemID, ITEM_FLAGS.HANGAR, itemLookup.match.typeID, normalizedAmount);
    if (!moveResult.success) {
    ...
    const spawnResult = spaceRuntime.spawnDynamicInventoryEntity(session._space.systemID, createResult.data.itemID);
    if (!spawnResult.success) {
        return handledResult(chatHub, session, options, "Jetcan contents moved, but the space ball failed to spawn.");
    }
    return handledResult(chatHub, session, options, `Jettisoned ${normalizedAmount.toLocaleString("en-US")}x ${itemLookup.match.name} into ${containerLookup.match.name} (${createResult.data.itemID}). Expires in 2 hours.`);
}
function handleDebrisFieldCommand(session, argumentText, chatHub, options, kind) {
    ...
    if (String(argumentText || "").trim().toLowerCase() === "list") {
        ...
        return handledResult(chatHub, session, options, `Available ${label.toLowerCase()} types (${types.length}):\n${lines.join("\n")}\n\nUsage: /${kind} <name|typeID> [count]`);
    }
    if (!isSpaceSessionReady(session)) {
        return handledResult(chatHub, session, options, `You must be in space before using /${kind}.`);
    }
    ...
    if (argumentText &&
        ...
        !parsed.count) {
        return handledResult(chatHub, session, options, "Count must be a positive whole number.");
    }
    if (parsed.typeName) {
        const typeLookup = resolveDebrisType(kind, parsed.typeName);
        if (!typeLookup.success) {
            const label = kind === "wreck" ? "Wreck" : "Container";
            const message = typeLookup.errorMsg === "ITEM_NOT_FOUND"
    ...
    if (!spawnResult.success) {
        const message = spawnResult.errorMsg === "SPACE_REQUIRED"
            ? `You must be in space before using /${kind}.`
            : "Debris spawn failed.";
        return handledResult(chatHub, session, options, message);
    }
    syncSpaceRootInventoryChangesToSession(session, spawnResult.data && spawnResult.data.changes);
...
function handleTestClearCommand(session, chatHub, options) {
    if (!isSpaceSessionReady(session)) {
        return handledResult(chatHub, session, options, "You must be in space before using /testclear.");
    }
    const clearResult = clearNearbyDebrisForSession(session);
    if (!clearResult.success) {
        return handledResult(chatHub, session, options, "Clearing nearby debris failed.");
    }
    syncSpaceRootInventoryChangesToSession(session, clearResult.data && clearResult.data.changes);
    return handledResult(chatHub, session, options, `Cleared ${Number((clearResult.data && clearResult.data.removed || []).length) || 0} wrecks/containers within 20 km.`);
}
function handleSystemJunkClearCommand(session, chatHub, options) {
    if (!isSpaceSessionReady(session)) {
        return handledResult(chatHub, session, options, "You must be in space before using /sysjunkclear.");
    }
    const clearResult = clearSystemDebrisForSession(session);
    if (!clearResult.success) {
        return handledResult(chatHub, session, options, "Clearing system debris failed.");
    }
    syncSpaceRootInventoryChangesToSession(session, clearResult.data && clearResult.data.changes);
    return handledResult(chatHub, session, options, `Cleared ${Number((clearResult.data && clearResult.data.removed || []).length) || 0} wrecks/containers across the current solar system.`);
}
function handleSuicideCommand(session, chatHub, options) {
    if (!isSpaceSessionReady(session)) {
        return handledResult(chatHub, session, options, "You must be in space before using /suicide.");
    }
    ...
    if (!destroyResult.success || !destroyResult.data) {
        return handledResult(chatHub, session, options, destroyResult.errorMsg === "ALREADY_IN_CAPSULE"
...
function handleDeathTestCommand(session, argumentText, chatHub, options) {
    if (!isSpaceSessionReady(session)) {
        return handledResult(chatHub, session, options, "You must be in space before using /deathtest.");
    }
    const parsed = parseOptionalTypeAndCount(argumentText);
    let shipType = null;
    if (parsed.typeName) {
        ...
        if (!lookup.success || !lookup.match) {
            const message = lookup.errorMsg === "SHIP_NOT_FOUND"
    ...
    if (!spawnResult.success || !spawnResult.data) {
        return handledResult(chatHub, session, options, "Death-test hull spawning failed.");
    }
    if (spawnResult.data.completionPromise && chatHub) {
        ...
        spawnResult.data.completionPromise
            ...
            chatHub.sendSystemMessage(session, `Detonated ${result.destroyed.length}/${result.spawnedCount} ${result.shipType.name} hulls into wrecks.`, feedbackChannel);
        })
            .catch((error) => {
            log.warn(`[ChatCommands] /deathtest completion failed: ${error.message}`);
        });
    }
    return handledResult(chatHub, session, options, `Spawned ${spawnResult.data.spawned.length} ${spawnResult.data.shipType.name} hulls across 20 km. Detonation in ${(spawnResult.data.delayMs / 1000).toFixed(1)}s sim time.`);
}
...
function handleStructureDeathTestCommand(session, argumentText, chatHub, options) {
    if (!isSpaceSessionReady(session)) {
        return handledResult(chatHub, session, options, "You must be in space before using /deathstructure.");
    }
    ...
    const spawnResult = spawnStructureDeathTestField(session, {
        ...
        count: parsed.count,
        delayMs: parsed.delaySeconds === null || parsed.delaySeconds === undefined
            ? undefined
            : parsed.delaySeconds * 1000,
    });
    if (!spawnResult.success || !spawnResult.data) {
        return handledResult(chatHub, session, options, formatStructureDeathTestError(spawnResult.errorMsg, {
            ...parsed,
    ...
    if (spawnResult.data.completionPromise && chatHub) {
        ...
        spawnResult.data.completionPromise
            ...
            const wreckSummary = result.wreckType
                ...
                : "";
            chatHub.sendSystemMessage(session, `Detonated ${result.destroyed.length}/${result.spawnedCount} ${result.typeRecord.name} structure${result.spawnedCount === 1 ? "" : "s"}.${wreckSummary}`, feedbackChannel);
        })
            .catch((error) => {
    ...
    return handledResult(chatHub, session, options, `Spawned ${spawnResult.data.spawned.length} ${spawnResult.data.typeRecord.name} structure${spawnResult.data.spawned.length === 1 ? "" : "s"} across ${(spawnResult.data.radiusMeters / 1000).toFixed(0)} km. Detonation in ${(spawnResult.data.delayMs / 1000).toFixed(1)}s sim time.${wreckSummary}`);
}
...
function applyWarpCommandChaseOverride(result) {
    ...
    for (const entry of spawned) {
        ...
        const existingOverrides = controller &&
        ...
        const overrideResult = npcService.setBehaviorOverrides(entityID, {
            ...
            syntheticChasePropulsionTier: resolveWarpCommandSyntheticChaseTier(entry && entry.entity),
...
function handleNpcCommand(session, argumentText, chatHub, options) {
    ...
    if (parsedArguments.success === false) {
        return handledResult(chatHub, session, options, "Usage: /npc [amount] [faction|profile|pool]");
    }
    if (parsedArguments.amount > MAX_NPC_COMMAND_SPAWN_COUNT) {
        return handledResult(chatHub, session, options, `NPC spawn count must be between 1 and ${MAX_NPC_COMMAND_SPAWN_COUNT}.`);
    }
...
function handleSpawnNpcPilotShipCommand(session, argumentText, chatHub, options, command = "spawnpilot") {
    ...
    if (!Number.isSafeInteger(typeID) || typeID <= 0 || !Number.isSafeInteger(count) ||
        ...
        return handledResult(chatHub, session, options, `${commandLabel} requires a positive Ship typeID and a count from 1 to ${MAX_NPC_COMMAND_SPAWN_COUNT}.`);
    }
    ...
    return handledResult(chatHub, session, options, `typeID ${typeID}: ${formatNpcSpawnSummary(result, commandLabel)} Spawn positions are spread around your ship.`);
}
function handleMissileNpcCommand(session, argumentText, chatHub, options) {
    ...
    if (parsedArguments.success === false) {
        return handledResult(chatHub, session, options, "Usage: /mnpc [amount] [faction|profile|pool]");
    }
    if (parsedArguments.amount > MAX_NPC_COMMAND_SPAWN_COUNT) {
        return handledResult(chatHub, session, options, `NPC spawn count must be between 1 and ${MAX_NPC_COMMAND_SPAWN_COUNT}.`);
    }
    ...
    return handledResult(chatHub, session, options, formatNpcSpawnSummary(result, "/mnpc"));
}
function handleCapitalNpcCommand(session, argumentText, chatHub, options, commandName = "capnpc") {
    ...
    return handledResult(chatHub, session, options, result && result.message ? result.message : `${commandName} completed.`);
}
function handleNpcTestCommand(session, argumentText, chatHub, options, mode = "player") {
    ...
    if (parsedArguments.success === false) {
        return handledResult(chatHub, session, options, `Usage: ${commandLabel} [amount]`);
    }
    if (parsedArguments.amount > MAX_NPC_COMMAND_SPAWN_COUNT) {
        return handledResult(chatHub, session, options, `${commandLabel} spawn count must be between 1 and ${MAX_NPC_COMMAND_SPAWN_COUNT}.`);
    }
    if (normalizedMode === "ffa" && parsedArguments.amount < 2) {
        return handledResult(chatHub, session, options, "/npctest2 needs at least 2 NPCs.");
    }
...
function handleNpcWarpCommand(session, argumentText, chatHub, options, commandName = "npcw") {
    ...
    const parsedArguments = parseNpcSpawnArguments(argumentText);
    if (parsedArguments.success === false) {
        return handledResult(chatHub, session, options, `Usage: ${commandLabel} [amount] [profile|pool]`);
    }
    if (parsedArguments.amount > MAX_NPC_COMMAND_SPAWN_COUNT) {
        return handledResult(chatHub, session, options, `NPC warp spawn count must be between 1 and ${MAX_NPC_WARP_COMMAND_SPAWN_COUNT}.`);
    }
    ...
    const chaseOverrideResult = normalizedCommandName === "wnpc"
    ...
    return handledResult(chatHub, session, options, summary);
}
function handleDeadwarpCommand(session, argumentText, chatHub, options) {
    if (String(argumentText || "").trim()) {
        return handledResult(chatHub, session, options, "Usage: /deadwarp");
    }
    if (!isSpaceSessionReady(session)) {
        return handledResult(chatHub, session, options, "You must be in space before using /deadwarp.");
    }
    ...
    if (!warpRequestResult.success) {
        return handledResult(chatHub, session, options, `Deadwarp failed: ${warpRequestResult.errorMsg || "WARP_REQUEST_FAILED"}.`);
    }
    const activationResult = scene.forceStartPendingWarp(shipEntity, {
        nowMs: scene.getCurrentSimTimeMs(),
    });
    if (!activationResult.success) {
        return handledResult(chatHub, session, options, `Deadwarp failed: ${activationResult.errorMsg || "WARP_ACTIVATION_FAILED"}.`);
    }
    return handledResult(chatHub, session, options, `Deadwarp started to a safe point ${formatDistanceAu(originAnchor.distanceMeters)} AU away.`);
}
function handleConcordCommand(session, argumentText, chatHub, options) {
    const parsedArguments = parseNpcSpawnArguments(argumentText);
    if (parsedArguments.success === false) {
        return handledResult(chatHub, session, options, "Usage: /concord [amount] [profile|pool]");
    }
    if (parsedArguments.amount > MAX_CONCORD_COMMAND_SPAWN_COUNT) {
        return handledResult(chatHub, session, options, `CONCORD spawn count must be between 1 and ${MAX_CONCORD_COMMAND_SPAWN_COUNT}.`);
    }
    ...
    return handledResult(chatHub, session, options, formatNpcSpawnSummary(result, "/concord"));
}
function handleNpcClearCommand(session, argumentText, chatHub, options) {
    const parsed = parseNpcClearArguments(argumentText);
    if (parsed.success === false || !parsed.entityType) {
        return handledResult(chatHub, session, options, "Usage: /npcclear <system [npc|concord|all]|radius <meters> [npc|concord|all]>");
    }
    const systemID = resolveSessionSolarSystemID(session);
    if (!systemID) {
        return handledResult(chatHub, session, options, "Current solar system could not be resolved for /npcclear.");
    }
    let result = null;
    if (parsed.scope === "radius") {
        if (!isSpaceSessionReady(session) || !resolveSessionShipEntity(session)) {
            return handledResult(chatHub, session, options, "You must be in space before using /npcclear radius.");
        }
    ...
    if (!result || !result.success) {
        return handledResult(chatHub, session, options, `NPC clear failed: ${(result && result.errorMsg) || "UNKNOWN_ERROR"}.`);
    }
    ...
    const entityLabel = parsed.entityType === "all"
        ...
        : parsed.entityType === ENTITY_TYPE.CONCORD
            ? "CONCORD"
            : "NPC";
    return handledResult(chatHub, session, options, `Cleared ${result.data.destroyedCount} ${entityLabel} controller${result.data.destroyedCount === 1 ? "" : "s"} ${scopeText}.`);
}
...
function handleGateOperatorCommand(session, argumentText, chatHub, options, operatorKind) {
    const systemID = resolveSessionSolarSystemID(session);
    if (!systemID) {
        return handledResult(chatHub, session, options, "Current solar system could not be resolved for gate operator controls.");
    }
    ...
    if (!result || !result.success || !result.data) {
        return handledResult(chatHub, session, options, `${label} command failed: ${(result && result.errorMsg) || "UNKNOWN_ERROR"}.`);
    }
    return handledResult(chatHub, session, options, formatGateOperatorStatus(label, result.data));
}
function handleInvuCommand(session, argumentText, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /invu.");
    }
    const trimmed = String(argumentText || "").trim();
    let result = null;
    if (!trimmed) {
        ...
        if (parsed.success === false) {
            return handledResult(chatHub, session, options, "Usage: /invu [on|off]");
        }
        if (parsed.mode === "status") {
            const enabled = npcService.isCharacterInvulnerable(session.characterID);
            return handledResult(chatHub, session, options, `Invulnerability is ${enabled ? "ON" : "OFF"}. Rats and CONCORD ${enabled ? "will" : "will not"} ignore you.`);
        }
        result = npcService.setCharacterNpcInvulnerability(session.characterID, parsed.mode === "on");
    }
    if (!result || !result.success || !result.data) {
        return handledResult(chatHub, session, options, `Invulnerability update failed: ${(result && result.errorMsg) || "UNKNOWN_ERROR"}.`);
    }
    if (isSpaceSessionReady(session)) {
    ...
    return handledResult(chatHub, session, options, `Invulnerability is now ${result.data.invulnerable ? "ON" : "OFF"}. Rats and CONCORD ${result.data.invulnerable ? "will" : "will not"} ignore you.`);
}
function handleHealCommand(session, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /heal.");
    }
    if (isSpaceSessionReady(session)) {
        ...
        if (!result || !result.success) {
            const message = result && result.errorMsg === "SCENE_NOT_FOUND"
                ? "Your ship is not loaded in space yet."
                : "Active ship not found in space.";
            return handledResult(chatHub, session, options, message);
        }
        return handledResult(chatHub, session, options, "Restored full shields, armor, hull, and capacitor on your active ship.");
    }
    const updateResult = healDockedShipForSession(session);
    if (!updateResult || !updateResult.success) {
        return handledResult(chatHub, session, options, "Active ship not found.");
    }
    return handledResult(chatHub, session, options, "Restored full shields, armor, hull, and capacitor on your active ship.");
}
function handleDamageCommand(session, argumentText, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /dmg.");
    }
    const presetResult = resolveDamageTestPreset(argumentText);
    if (presetResult.success === false || !presetResult.preset) {
        return handledResult(chatHub, session, options, "Usage: /dmg [light|medium|heavy]");
    }
    ...
    if (!damageResult || !damageResult.success || !damageResult.data) {
        const message = damageResult && damageResult.errorMsg === "NOT_DOCKED"
    ...
    const moduleText = moduleCount > 0
        ...
        : "";
    return handledResult(chatHub, session, options, `Applied ${presetLabel} test damage to your active ship${moduleText}.`);
}
function buildCrimewatchStatusMessage(session) {
    ...
    const remainingWeaponMs = Math.max(0, Number(effectiveState.weaponTimerExpiresAtMs || 0) - now);
    const remainingPvpMs = Math.max(0, Number(effectiveState.pvpTimerExpiresAtMs || 0) - now);
    const remainingNpcMs = Math.max(0, Number(effectiveState.npcTimerExpiresAtMs || 0) - now);
    const remainingPenaltyMs = Math.max(0, Number(effectiveState.criminalTimerExpiresAtMs || 0) - now);
    const remainingDisapprovalMs = Math.max(0, Number(effectiveState.disapprovalTimerExpiresAtMs || 0) - now);
...
function synchronizeCrimewatchSessionState(session, scene, now) {
    const activeScene = scene || (isSpaceSessionReady(session)
...
function handleCrimewatchCommand(session, argumentText, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /cwatch.");
    }
    const usage = "Usage: /cwatch [status|clear|safety <full|partial|none>|weapon <off|seconds>|pvp <off|seconds>|npc <off|seconds>|criminal <off|seconds>|suspect <off|seconds>|disapproval <off|seconds>]";
    const trimmed = String(argumentText || "").trim();
    ...
    if (!trimmed || ["status", "state"].includes(trimmed.toLowerCase())) {
        return handledResult(chatHub, session, options, buildCrimewatchStatusMessage(session));
    }
    if (["clear", "reset"].includes(trimmed.toLowerCase())) {
        const result = crimewatchState.setCharacterCrimewatchDebugState(session.characterID, { clearAll: true }, {
            now,
        ...
        if (!result.success) {
            return handledResult(chatHub, session, options, `Crimewatch update failed: ${result.errorMsg || "UNKNOWN_ERROR"}.`);
        }
        synchronizeCrimewatchSessionState(session, scene, now);
        return handledResult(chatHub, session, options, `Crimewatch timers cleared. ${buildCrimewatchStatusMessage(session)}`);
    }
    ...
    if (subcommand === "safety") {
        ...
        const defaultDurationMs = subcommand === "weapon"
    ...
    const result = crimewatchState.setCharacterCrimewatchDebugState(session.characterID, updates, {
        now,
        systemID: resolveSessionSolarSystemID(session),
        scene,
        offenderEntity,
    ...
    synchronizeCrimewatchSessionState(session, scene, now);
    return handledResult(chatHub, session, options, `Crimewatch updated. ${buildCrimewatchStatusMessage(session)}`);
}
function handleNaughtyCommand(session, argumentText, chatHub, options) {
    if (!isSpaceSessionReady(session)) {
        return handledResult(chatHub, session, options, "You must be in space before using /naughty.");
    }
    if (String(argumentText || "").trim()) {
        return handledResult(chatHub, session, options, "Usage: /naughty");
    }
    const scene = spaceRuntime.ensureScene(session._space.systemID);
    const offenderEntity = resolveSessionShipEntity(session);
    if (!scene || !offenderEntity) {
        return handledResult(chatHub, session, options, "Active ship not found in space.");
    }
    const now = scene.getCurrentSimTimeMs();
    const result = crimewatchState.triggerHighSecCriminalOffense(scene, offenderEntity, {
        now,
    ...
    if (!result.data || result.data.applied !== true) {
        ...
        const message = reason === "NOT_HIGHSEC"
        ...
        return handledResult(chatHub, session, options, `${message} ${buildCrimewatchStatusMessage(session)}`);
    }
    const responseDueMs = Math.max(0, Number(result.data.concordResponseDueAtMs || 0) - now);
    const securityPenalty = result.data.securityStatusPenalty || null;
    const securitySuffix = securityPenalty && securityPenalty.applied === true
        ? ` Security status is now ${Number(securityPenalty.nextSecurityStatus || 0).toFixed(2)}.`
        : "";
    return handledResult(chatHub, session, options, `Crimewatch offense simulated. CONCORD ${responseDueMs > 0 ? `will respond in about ${formatDurationBriefMs(responseDueMs)}` : "response is active now"}. ${buildCrimewatchStatusMessage(session)}${securitySuffix}`);
}
function handleSecurityStatusCommand(session, argumentText, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /secstatus.");
    }
    const trimmed = String(argumentText || "").trim();
    const currentSecurityStatus = crimewatchState.getCharacterSecurityStatus(session.characterID, 0);
    if (!trimmed) {
        return handledResult(chatHub, session, options, `Security status is ${currentSecurityStatus.toFixed(2)}. Use /secstatus <value> to set it (${crimewatchState.SECURITY_STATUS_MIN.toFixed(0)} to ${crimewatchState.SECURITY_STATUS_MAX.toFixed(0)}).`);
    }
    const requestedSecurityStatus = Number(trimmed);
    if (!Number.isFinite(requestedSecurityStatus)) {
        return handledResult(chatHub, session, options, "Usage: /secstatus [status]");
    }
    const scene = isSpaceSessionReady(session)
        ? spaceRuntime.ensureScene(session._space.systemID)
        : null;
    ...
    const result = crimewatchState.setCharacterSecurityStatus(session.characterID, requestedSecurityStatus, {
        now,
        scene,
        entity,
        session,
    ...
    if (!result.success) {
        return handledResult(chatHub, session, options, `Security status update failed: ${result.errorMsg || "UNKNOWN_ERROR"}.`);
    }
    return handledResult(chatHub, session, options, `Security status set to ${result.data.securityStatus.toFixed(2)} (requested ${requestedSecurityStatus.toFixed(2)}; clamped to ${crimewatchState.SECURITY_STATUS_MIN.toFixed(0)} to ${crimewatchState.SECURITY_STATUS_MAX.toFixed(0)}).`);
}
function handleFireCommand(session, argumentText, chatHub, options) {
    if (!isSpaceSessionReady(session)) {
        return handledResult(chatHub, session, options, "You must be in space before using /fire.");
    }
    ...
    return handledResult(chatHub, session, options, `Spawned ${shipLookup.match.name} dummy hull ${spawnResult.data.entity.itemID} roughly 20 km away.`);
}
function handleKeepstarCommand(session, chatHub, options) {
    if (!isSpaceSessionReady(session)) {
        return handledResult(chatHub, session, options, "You must be in space before using /keepstar.");
    }
    const anchorEntity = spaceRuntime.getEntity(session, session._space.shipID);
    if (!anchorEntity) {
        return handledResult(chatHub, session, options, "Active ship was not found in space.");
    ...
    if (!keepstarEntity) {
        return handledResult(chatHub, session, options, "Palatine Keepstar type data was not found in the local item registry.");
    }
    const scene = spaceRuntime.getSceneForSession(session);
    if (!scene) {
    ...
    return handledResult(chatHub, session, options, `Spawned runtime-only Palatine Keepstar ${spawnResult.data.entity.itemID} roughly ${(KEEPSTAR_DEFAULT_SPAWN_DISTANCE_METERS / 1000).toFixed(0)} km away. It is visual-only and clears on restart.`);
}
function handleFire2Command(session, argumentText, chatHub, options) {
    if (!isSpaceSessionReady(session)) {
        return handledResult(chatHub, session, options, "You must be in space before using /fire2.");
    }
    const anchorEntity = spaceRuntime.getEntity(session, session._space.shipID);
    if (!anchorEntity) {
        return handledResult(chatHub, session, options, "Active ship was not found in space.");
    ...
    if (fleetLookup.success === false) {
        return handledResult(chatHub, session, options, `The default /fire2 fleet could not be assembled from local ship data (${fleetLookup.availableCount || 0}/10 available).`);
    }
    const formation = buildFire2FleetFormation(anchorEntity, fleetLookup.ships, requestedFleetSize);
    const spawned = [];
...
function handleGmSkillsCommand(session, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /gmskills.");
    }
    const unpublishedSkillTypes = getUnpublishedSkillTypes({ refresh: true });
    if (unpublishedSkillTypes.length === 0) {
        return handledResult(chatHub, session, options, "No unpublished skill types are available in local reference data.");
    }
    ...
    reconcileQueueForSkillMutation(session.characterID, "prune_satisfied");
    ...
    refreshLiveCharacterSkillSession(targetDescriptor.session, targetDescriptor, grantedSkills, previousSkillMap, {
...
function handleAllSkillsCommand(session, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /allskills.");
    }
    const publishedSkillTypes = getPublishedSkillTypes({ refresh: true });
    if (publishedSkillTypes.length === 0) {
        return handledResult(chatHub, session, options, "No published skill types are available in local reference data.");
    }
    const previousQueueSnapshot = getQueueSnapshot(session.characterID);
    const previousSkillMap = getCharacterSkillMap(session.characterID);
...
function handleAllSkinsCommand(session, argumentText, chatHub, options) {
    ...
    if (targetResult.success === false) {
        const message = targetResult.errorMsg === "CHARACTER_NOT_SELECTED"
    ...
    if (!grantResult.success) {
        return handledResult(chatHub, session, options, "Failed to unlock ship SKIN licenses.");
    }
...
function handleBackInTimeCommand(session, argumentText, chatHub, options) {
    ...
    if (!parsed.success) {
        return handledResult(chatHub, session, options, "Usage: /backintime [me|characterID|character name]");
    }
    const targetResult = resolveCharacterTargetDescriptor(session, parsed.data.targetToken);
    if (targetResult.success === false) {
        const message = targetResult.errorMsg === "CHARACTER_NOT_SELECTED"
    ...
    const targetDescriptor = targetResult.data;
    const resetResult = resetCharacterSkillsToStarterProfile(targetDescriptor.characterID);
    if (!resetResult.success) {
        return handledResult(chatHub, session, options, resetResult.errorMsg === "STARTER_PROFILE_NOT_FOUND"
    ...
    refreshLiveCharacterSkillSession(targetDescriptor.session, targetDescriptor, starterReset.changedSkillRecords, starterReset.previousSkillMap, {
    ...
    const queueCleared = starterReset.queueSnapshot &&
...
function handleExpertSystemCommand(session, argumentText, chatHub, options) {
    ...
    if (subcommand === "help") {
        return handledResult(chatHub, session, options, "Usage: /expertsystem list | inspect <type|name> | status [me|characterID|name] | add <type|name> [target] [days=N] | remove <type|name|all> [target] | clear [target] | giveitem <type|name> [qty] | consume <itemID>");
    }
    if (subcommand === "list") {
        ...
        const sample = expertSystems
            ...
            .join(", ");
        return handledResult(chatHub, session, options, `Expert Systems authority contains ${expertSystems.length} systems (${visibleCount} normally activatable). ${sample ? `First entries: ${sample}.` : ""}`);
    }
    if (subcommand === "status") {
        ...
        if (activeEntries.length === 0) {
            return handledResult(chatHub, session, options, `${targetLabel} has no active Expert Systems. Catalog count: ${status.catalogCount}.`);
        }
        return handledResult(chatHub, session, options, `${targetLabel} has ${activeEntries.length} active Expert Systems: ${activeEntries.map(formatExpertSystemStatusLine).join("; ")}.`);
    }
    if (subcommand === "inspect" || subcommand === "info") {
        const expertResult = resolveExpertSystemFromTokens(tokens, 1);
        if (!expertResult.success) {
            return handledResult(chatHub, session, options, `Expert System not found.${formatSuggestions(expertResult.suggestions)}`);
        }
    ...
    if (subcommand === "add" || subcommand === "install") {
        ...
        const expertSystem = expertResult.data.expertSystem;
        const targetDescriptor = targetResult.data;
        const installResult = installExpertSystemForCharacter(targetDescriptor.characterID, expertSystem.typeID, {
        ...
        return handledResult(chatHub, session, options, `Installed ${formatExpertSystemSummary(expertSystem)} for ${formatCharacterTargetLabel(targetDescriptor)}${installResult.data.isTopUp ? " as a top-up" : ""}.`);
    }
    if (subcommand === "remove" || subcommand === "uninstall") {
        if (normalizeCommandName(tokens[1] || "") === "all") {
            ...
            return handledResult(chatHub, session, options, `Removed ${Array.isArray(clearResult.data) ? clearResult.data.length : 0} Expert Systems from ${formatCharacterTargetLabel(targetResult.data)}.`);
        }
        const expertResult = resolveExpertSystemFromTokens(tokens, 1);
        if (!expertResult.success) {
            return handledResult(chatHub, session, options, `Expert System not found.${formatSuggestions(expertResult.suggestions)}`);
        ...
        const expertSystem = expertResult.data.expertSystem;
        const removeResult = removeExpertSystemFromCharacter(targetResult.data.characterID, expertSystem.typeID, { session: targetResult.data.session });
        if (!removeResult.success) {
            return handledResult(chatHub, session, options, removeResult.message || "Expert System removal failed.");
        }
        return handledResult(chatHub, session, options, removeResult.removed
    ...
    if (subcommand === "clear") {
        ...
        return handledResult(chatHub, session, options, `Cleared ${Array.isArray(clearResult.data) ? clearResult.data.length : 0} Expert Systems from ${formatCharacterTargetLabel(targetResult.data)}.`);
    }
    if (subcommand === "giveitem") {
        ...
        const giveResult = giveItemToHangarForSession(session, itemType, quantity);
        if (!giveResult.success) {
            const message = giveResult.errorMsg === "DOCK_REQUIRED"
        ...
        return handledResult(chatHub, session, options, `Placed ${quantity}x ${formatExpertSystemSummary(expertSystem)} item in your hangar.`);
    }
    if (subcommand === "consume") {
        ...
        return handledResult(chatHub, session, options, `Consumed ${formatExpertSystemSummary(consumeResult.data.expertSystem)} and installed it on your character.`);
    }
    return handledResult(chatHub, session, options, "Usage: /expertsystem list | inspect <type|name> | status [target] | add <type|name> [target] [days=N] | remove <type|name|all> [target] | clear [target] | giveitem <type|name> [qty] | consume <itemID>");
}
function handleGiveSkillCommand(session, argumentText, chatHub, options) {
    const parsed = parseGiveSkillArguments(argumentText);
    if (parsed.success === false) {
        return handledResult(chatHub, session, options, "Usage: /giveskill <target> <skill name|typeID|all|super> [level]");
    }
    const targetResult = resolveCharacterTargetDescriptor(session, parsed.data.targetToken);
    if (targetResult.success === false) {
        const message = targetResult.errorMsg === "CHARACTER_NOT_SELECTED"
    ...
    if (skillResult.success === false) {
        ...
        const label = skillResult.errorMsg === "SKILL_AMBIGUOUS"
    ...
    const grantResult = applySkillGrantToCharacter(targetDescriptor, skillDescriptor, parsed.data.level, previousSkillMap);
    if (grantResult.success === false) {
        return handledResult(chatHub, session, options, "Failed to update character skills.");
    }
    const changedSkills = dedupeSkillRecords(grantResult.data.changedSkills);
    const clearQueueForMutation = skillMutationLowersExistingLevels(changedSkills, previousSkillMap);
    const queueResult = reconcileQueueForSkillMutation(targetDescriptor.characterID, clearQueueForMutation ? "clear" : "prune_satisfied");
    const nextQueueSnapshot = queueResult.snapshot || getQueueSnapshot(targetDescriptor.characterID);
    refreshLiveCharacterSkillSession(targetDescriptor.session, targetDescriptor, changedSkills, previousSkillMap, {
    ...
    const levelClampNotice = parsed.data.requestedLevel !== null && parsed.data.requestedLevel !== parsed.data.level
...
function handleRemoveSkillCommand(session, argumentText, chatHub, options) {
    ...
    if (parsed.success === false) {
        return handledResult(chatHub, session, options, "Usage: /removeskill <target> <skill name|typeID|all>");
    }
    const targetResult = resolveCharacterTargetDescriptor(session, parsed.data.targetToken);
    if (targetResult.success === false) {
        const message = targetResult.errorMsg === "CHARACTER_NOT_SELECTED"
    ...
    if (removalResult.success === false) {
        return handledResult(chatHub, session, options, "Failed to update character skills.");
    }
    const changedSkills = dedupeSkillRecords(removalResult.data.changedSkills);
    const queueResult = reconcileQueueForSkillMutation(targetDescriptor.characterID, "clear");
    const nextQueueSnapshot = queueResult.snapshot || getQueueSnapshot(targetDescriptor.characterID);
    refreshLiveCharacterSkillSession(targetDescriptor.session, targetDescriptor, [], previousSkillMap, {
        removed: true,
    ...
    const targetLabel = formatCharacterTargetLabel(targetDescriptor);
    if (skillDescriptor.selector === "all") {
        return handledResult(chatHub, session, options, changedSkills.length > 0
...
function handleGmShipsCommand(session, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /gmships.");
    }
    if (!isDockedSession(session)) {
        return handledResult(chatHub, session, options, "You must be docked before using /gmships.");
    }
    const unpublishedShips = getUnpublishedShipTypes();
    if (unpublishedShips.length === 0) {
        return handledResult(chatHub, session, options, "No unpublished ship types are available in local reference data.");
    }
    const createdShips = [];
...
function handlePropCommand(session, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /prop.");
    }
    if (!isDockedSession(session)) {
        return handledResult(chatHub, session, options, "You must be docked before using /prop.");
    }
    const propulsionTypes = getPropulsionCommandItemTypes();
    if (propulsionTypes.length === 0) {
        return handledResult(chatHub, session, options, "No propulsion module types matched the /prop filter in local reference data.");
    }
    const createdItems = [];
...
function handleGmWeaponsCommand(session, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /gmweapons.");
    }
    const stationID = Number(getDockedLocationID(session) || 0) || 0;
    if (stationID <= 0) {
        return handledResult(chatHub, session, options, "You must be docked before using /gmweapons.");
    }
    if (activeGmWeaponsJobs.has(Number(session.characterID))) {
        ...
        return handledResult(chatHub, session, options, `A /gmweapons seed job is already running (${existingJob.nextIndex}/${existingJob.entries.length} grants queued).`);
    }
    const plan = getGmWeaponsSeedPlan();
    if (!plan.entries.length) {
        return handledResult(chatHub, session, options, "No non-T1 weapon or ammo types matched the /gmweapons filter in local reference data.");
    }
...
async function handleSolarTeleport(session, argumentText, chatHub, options) {
    ...
    if (!argumentText) {
        return handledResult(chatHub, session, options, "Usage: /solar <system name>");
    }
    const lookup = resolveSolarSystemByName(argumentText);
    if (!lookup.success) {
        const message = lookup.errorMsg === "SOLAR_SYSTEM_NOT_FOUND"
    ...
    const targetSolarSystem = (result.data && result.data.solarSystem) ||
    ...
    return handledResult(chatHub, session, getPostLocalMoveFeedbackOptions(options), `Teleported to ${lookup.match.solarSystemName} (${lookup.match.solarSystemID})${anchorText}.`);
}
function handleTransportCommand(session, argumentText, chatHub, options) {
    ...
    if (directPos) {
        ...
        if (!pointContext || !pointContext.systemID) {
            return handledResult(chatHub, session, options, "Could not resolve a solar-system context for /tr pos=...");
        }
        destination = {
            kind: "point",
            systemID: pointContext.systemID,
            point: directPos,
            direction: cloneSpaceVector(pointContext.direction, { x: 1, y: 0, z: 0 }),
        };
    }
        ...
        const anchor = targetDescriptor.kind === "session"
            ? getSessionTransportAnchor(targetDescriptor.session)
            : buildTransportPointAnchor(targetDescriptor.entity, targetDescriptor.systemID);
        destination = withTransportOffset(anchor, directOffset);
        if (!destination) {
            return handledResult(chatHub, session, options, "Could not apply /tr offset=... because the target has no in-space anchor.");
        }
    }
    else {
        ...
        if (coordinateTriplet) {
            const pointContext = resolveTransportPointContext(session, targetDescriptor);
            if (!pointContext || !pointContext.systemID) {
                return handledResult(chatHub, session, options, "Could not resolve a solar-system context for raw /tr coordinates.");
            }
            destination = {
                kind: "point",
                systemID: pointContext.systemID,
            ...
            const offsetToken = parseTransportVectorTag(destinationTokens[destinationTokens.length - 1], "offset");
            ...
            const baseDestination = resolveTransportLocationToken(session, targetDescriptor, baseTokens[0]);
            if (!baseDestination) {
                return handledResult(chatHub, session, options, `Transport destination not found: ${baseTokens[0]}.`);
            }
            if (offsetToken) {
                destination = withTransportOffset(baseDestination, offsetToken);
                if (!destination) {
                    return handledResult(chatHub, session, options, "Could not apply /tr offset=... to that destination.");
                }
            }
            else {
    ...
    const destinationLabel = formatTransportDestinationLabel(destination);
    if (targetDescriptor.kind === "session") {
        return executeSessionTransportTarget(session, targetDescriptor, destination, chatHub, options);
    }
    if (destination.kind !== "point" || !destination.point) {
        return handledResult(chatHub, session, options, "Runtime entity /tr currently supports only in-space point moves.");
    }
    if (normalizePositiveInteger(destination.systemID) !==
        normalizePositiveInteger(targetDescriptor.systemID)) {
        return handledResult(chatHub, session, options, "Runtime entities can only be moved within their current solar system.");
    }
    const destinationScene = spaceRuntime.ensureScene(targetDescriptor.systemID);
    const dungeonInitialization = initializeTransportDestinationDungeon(destinationScene, destination, null, {
        dependencies: options && options.transportDungeonInitializationDependencies,
    });
    if (!dungeonInitialization.success) {
        return handledResult(chatHub, session, options, `Failed to initialize the dungeon at ${destinationLabel}: ${dungeonInitialization.errorMsg || "unknown error"}.`);
    }
    const entityMoveResult = spaceRuntime.teleportDynamicEntityToPoint(targetDescriptor.systemID, targetDescriptor.entity.itemID, destination.point, {
        direction: destination.direction,
        refreshOwnerSession: false,
    });
    if (!entityMoveResult.success) {
        return handledResult(chatHub, session, options, `Failed to transport ${targetLabel}.`);
    }
    return handledResult(chatHub, session, options, `Transported ${targetLabel} to ${destinationLabel}.`);
}
function handleTeleCommand(session, argumentText, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /tele.");
    }
    const targetToken = String(argumentText || "").trim();
    if (!targetToken) {
        return handledResult(chatHub, session, options, "Usage: /tele <character name|characterID>");
    }
    const targetResult = resolveCharacterTargetDescriptor(session, targetToken);
    if (targetResult.success === false || !targetResult.data) {
        return handledResult(chatHub, session, options, targetResult.errorMsg === "CHARACTER_NOT_SELECTED"
    ...
    if (!targetDescriptor.session || !targetDescriptor.session.characterID) {
        return handledResult(chatHub, session, options, `${formatCharacterTargetLabel(targetDescriptor)} is not currently connected.`);
    }
    const destination = buildTransportDestinationFromSession(targetDescriptor.session);
    if (!destination) {
        return handledResult(chatHub, session, options, `Could not resolve a live location for ${formatCharacterTargetLabel(targetDescriptor)}.`);
    }
    return executeSessionTransportTarget(session, {
        kind: "session",
        session,
        label: "me",
    }, destination, chatHub, options);
}
function handleSetStandingCommand(session, argumentText, chatHub, options) {
    ...
    if (!Number.isFinite(standingValue) || standingValue < -10 || standingValue > 10) {
        return handledResult(chatHub, session, options, "Standing value must be a number between -10.0 and 10.0.");
    }
    const targetResolution = resolveTrailingCharacterTargetDescriptor(session, tokens, 1);
    if (targetResolution.success === false) {
        return handledResult(chatHub, session, options, targetResolution.errorMsg === "CHARACTER_NOT_SELECTED"
    ...
    if (ownerResolution.success === false) {
        return handledResult(chatHub, session, options, `Could not resolve a standing owner from "${ownerToken}".`);
    }
    const ownerDescriptor = ownerResolution.data;
    const writeResult = standingRuntime.setCharacterStanding(targetDescriptor.characterID, ownerDescriptor.ownerID, standingValue, {
        eventTypeID: standingRuntime.EVENT_STANDING_SLASH_SET,
        message: `GM /setstanding ${standingValue} ${ownerToken}`,
    });
    if (!writeResult.success) {
        return handledResult(chatHub, session, options, "Could not update the requested standing.");
    }
    return handledResult(chatHub, session, options, `Set standing with ${formatStandingOwnerLabel(ownerDescriptor)} to ${Number(standingValue).toFixed(2)} for ${formatCharacterTargetLabel(targetDescriptor)}.`);
}
function handleMaxAgentStandingsCommand(session, argumentText, chatHub, options) {
    ...
    if (entries.length <= 0) {
        return handledResult(chatHub, session, options, "No agent corporations or factions were available to update.");
    }
    const writeResult = standingRuntime.setCharacterStandings(targetDescriptor.characterID, entries, {
        eventTypeID: standingRuntime.EVENT_STANDING_SLASH_SET,
        message: "GM /maxagentstandings",
    });
    if (!writeResult.success) {
        return handledResult(chatHub, session, options, "Could not max the agent standings set.");
    }
    return handledResult(chatHub, session, options, `Set ${corporationIDs.length} agent corporation standings and ${factionIDs.length} agent faction standings to 10.00 for ${formatCharacterTargetLabel(targetDescriptor)}.`);
}
function handleSigscanCommand(session, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /sigscan.");
    }
    ...
    if (!result.success) {
        return handledResult(chatHub, session, options, "Could not resolve signatures for the current system.");
    }
    ...
    const system = worldData.getSolarSystemByID(systemID);
    const systemLabel = (system && system.solarSystemName) ||
        (systemID > 0 ? `system ${systemID}` : "the current system");
    return handledResult(chatHub, session, options, signatureCount > 0
...
function handleSigsCommand(session, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /sigs.");
    }
    const systemID = Number((session._space && session._space.systemID) ||
        session.solarsystemid2 ||
        session.solarsystemid ||
        0) || 0;
    if (systemID <= 0) {
        return handledResult(chatHub, session, options, "You must be in space to inspect live signatures.");
    }
    ...
    const systemLabel = (system && system.solarSystemName) ||
        (systemID > 0 ? `system ${systemID}` : "the current system");
    if (sites.length <= 0) {
        return handledResult(chatHub, session, options, `No scannable sites are currently present in ${systemLabel}.`);
    }
    const lines = sites.map((entry) => (`${entry.targetID} | ${entry.siteKind} | ${entry.family} | ${entry.label}` +
...
function handleMissionCompleteCommand(session, argumentText, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /missioncomplete.");
    }
    const targetToken = String(argumentText || "").trim();
    ...
    if (!result.success) {
        return handledResult(chatHub, session, options, result.errorMsg === "MISSION_NOT_FOUND"
    ...
    if (missionCount <= 0) {
        return handledResult(chatHub, session, options, "No accepted placeholder mission objectives were found to mark complete.");
    }
    const suffix = missionCount === 1 ? "" : "s";
    const agentSuffix = missionCount === 1 ? "" : "s";
    const details = missionCount === 1
        ? ` Agent ${markedAgentIDs[0]} is now ready to hand in.`
        : ` Agents: ${markedAgentIDs.join(", ")}.`;
    return handledResult(chatHub, session, options, `Marked ${missionCount} placeholder mission objective${suffix} complete.${details} Talk to the agent${agentSuffix} and click Complete Mission.`);
}
function handleOverlayRefreshCommand(session, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /overlayrefresh.");
    }
    const systemID = Number((session._space && session._space.systemID) ||
        session.solarsystemid2 ||
        session.solarsystemid ||
        0) || 0;
    if (systemID <= 0) {
        return handledResult(chatHub, session, options, "You must be in space to refresh the sensor overlay.");
    }
    const scanMgr = new ScanMgrService();
    ...
    if (!refreshResult.success) {
        return handledResult(chatHub, session, options, "Could not refresh the sensor overlay for the current system.");
    }
    const system = worldData.getSolarSystemByID(systemID);
    const systemLabel = (system && system.solarSystemName) ||
        (systemID > 0 ? `system ${systemID}` : "the current system");
    return handledResult(chatHub, session, options, `Refreshed the sensor overlay for ${systemLabel}.`);
}
function handleHomeDock(session, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /dock.");
    }
    const homeStationID = Number(session.homeStationID ||
        session.homestationid ||
        session.cloneStationID ||
        session.clonestationid ||
        0) || 0;
    if (!homeStationID) {
        return handledResult(chatHub, session, options, "Home station is not set for this character.");
    }
    if (Number(session.stationid || session.stationID || 0) === homeStationID) {
        return handledResult(chatHub, session, options, `Already docked at home station ${homeStationID}.`);
    }
    ...
    flushPendingLocalChannelSync(chatHub, session);
    return handledResult(chatHub, session, getPostLocalMoveFeedbackOptions(options), `Docked at ${station ? station.stationName : `station ${homeStationID}`}.`);
}
function handleEffectCommand(session, argumentText, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before using /effect.");
    }
    if (!session._space || isDockedSession(session)) {
        return handledResult(chatHub, session, options, "You must be in space to use /effect.");
    }
    const trimmed = String(argumentText || "").trim();
    ...
    if (stop && !effectName) {
        ...
        if (!stopResult.success) {
            return handledResult(chatHub, session, options, "Effect stop failed.");
        }
        return handledResult(chatHub, session, options, "Stopped all known self FX on your ship.");
    }
    if (!effectName) {
        return handledResult(chatHub, session, options, "Usage: /effect <name>, /effect stop, or /effect stop <name>");
    }
    const result = playPlayableEffect(session, effectName, { stop });
    if (!result.success) {
        if (result.errorMsg === "EFFECT_NOT_FOUND") {
            return handledResult(chatHub, session, options, `Unknown effect: ${effectName}. ${buildEffectListText()}`);
        }
        if (result.errorMsg === "DESTINY_NOT_READY") {
            return handledResult(chatHub, session, options, "Space scene is not ready for FX yet. Try again in a moment.");
        }
        if (result.errorMsg === "DEBUG_TEST_TARGET_NO_STATION") {
            return handledResult(chatHub, session, options, "That debug/test effect needs a nearby station target, but there is no station entity available in the current scene.");
        }
        if (result.errorMsg === "DEBUG_TEST_TARGET_OUT_OF_RANGE") {
            const maxRangeText = formatDistanceMeters(result.data && result.data.maxRangeMeters);
            const nearestDistanceText = formatDistanceMeters(result.data && result.data.nearestDistanceMeters);
            const targetName = (result.data && result.data.targetName) || "the nearest station";
            return handledResult(chatHub, session, options, `That debug/test effect needs a nearby station target within ${maxRangeText}. The nearest station is ${targetName} at ${nearestDistanceText}.`);
        }
        return handledResult(chatHub, session, options, "Effect playback failed.");
    }
    const effect = result.data.effect;
    const autoTarget = result.data.autoTarget;
    if (effect.debugOnly && autoTarget) {
        return handledResult(chatHub, session, options, `${stop ? "Stopped" : "Played"} debug/test ${effect.key} (${effect.guid}) on your ship using nearby station ${autoTarget.targetName} (${autoTarget.targetID}) at ${formatDistanceMeters(autoTarget.distanceMeters)}.`);
    }
    if (effect.debugOnly) {
        return handledResult(chatHub, session, options, `${stop ? "Stopped" : "Played"} debug/test ${effect.key} (${effect.guid}) on your ship.`);
    }
    return handledResult(chatHub, session, options, `${stop ? "Stopped" : "Played"} ${effect.key} (${effect.guid}) on your ship.`);
}
function handleLoadSystemCommand(session, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before loading stargate destination systems.");
    }
    const currentSystemID = getActiveSolarSystemID(session);
    if (!currentSystemID) {
        return handledResult(chatHub, session, options, "Current solar system could not be resolved.");
    }
    const stargates = worldData.getStargatesForSystem(currentSystemID);
    if (stargates.length === 0) {
        return handledResult(chatHub, session, options, `No stargates found in ${formatSolarSystemLabel(currentSystemID)}.`);
    }
    const destinationSystemIDs = [...new Set(stargates
    ...
    if (destinationSystemIDs.length === 0) {
        return handledResult(chatHub, session, options, `No valid stargate destination systems found in ${formatSolarSystemLabel(currentSystemID)}.`);
    }
    const alreadyLoaded = destinationSystemIDs.filter((systemID) => spaceRuntime.isSolarSystemSceneLoaded(systemID));
    ...
    const loadedNow = destinationSystemIDs.filter((systemID) => spaceRuntime.isSolarSystemSceneLoaded(systemID));
    const newlyLoaded = loadedNow.filter((systemID) => !alreadyLoaded.includes(systemID));
    const failed = destinationSystemIDs.filter((systemID) => !loadedNow.includes(systemID));
    return handledResult(chatHub, session, options, [
...
function handleLoadAllSystemsCommand(session, chatHub, options) {
    if (!session || !session.characterID) {
        return handledResult(chatHub, session, options, "Select a character before loading all solar systems.");
    }
    ...
    if (solarSystemIDs.length === 0) {
        return handledResult(chatHub, session, options, "No solar systems are available to preload.");
    }
    const alreadyLoaded = solarSystemIDs.filter((systemID) => spaceRuntime.isSolarSystemSceneLoaded(systemID));
    const activationChanges = spaceRuntime.preloadSolarSystems(solarSystemIDs, {
        broadcast: true,
    });
    const loadedNow = solarSystemIDs.filter((systemID) => spaceRuntime.isSolarSystemSceneLoaded(systemID));
    const newlyLoaded = loadedNow.filter((systemID) => !alreadyLoaded.includes(systemID));
    const failed = solarSystemIDs.filter((systemID) => !loadedNow.includes(systemID));
    return handledResult(chatHub, session, options, [
        "/loadallsys:",
...
function handleTimeDilationCommand(session, argumentText, chatHub, options) {
    ...
    if (!trimmedArgument) {
        ...
        if (!snapshot) {
            return handledResult(chatHub, session, options, `/tidi ${systemLabel}: scene not available.`);
        }
        return handledResult(chatHub, session, options, [
            `/tidi ${systemLabel}:`,
    ...
    if (trimmedArgument.toLowerCase() === "auto" ||
        ...
        scheduleAdvanceNoticeTimeDilationForSystems([currentSystemID], resumeFactor, { delayMs: TIDI_ADVANCE_NOTICE_MS });
        return handledResult(chatHub, session, options, [
            `/tidi ${systemLabel}:`,
            cleared
    ...
    if (!Number.isFinite(requestedFactor)) {
        return handledResult(chatHub, session, options, "Usage: /tidi <0.1-1.0|auto>");
    }
    ...
    scheduleAdvanceNoticeTimeDilationForSystems([currentSystemID], normalizedFactor, { delayMs: TIDI_ADVANCE_NOTICE_MS });
    const clampedMessage = normalizedFactor !== requestedFactor
...
function sendTimeDilationNotificationToSystem(systemID, factor) {
    ...
    for (const targetSession of sessionRegistry.getSessions()) {
        ...
        targetSession.sendNotification("OnSetTimeDilation", "clientID", [maxDil, minDil, threshold]);
        sentCount += 1;
    }
    return sentCount;
}
...
function sendTimeDilationNotificationToSession(session, factor) {
    ...
    const minDil = isDisabling ? 1.0 : factor;
    const threshold = isDisabling ? 100000000 : 0;
    session.sendNotification("OnSetTimeDilation", "clientID", [maxDil, minDil, threshold]);
    return true;
}
function executeChatCommand(session, rawMessage, chatHub, options = {}) {
    ...
    if (!commandLine) {
        return handledResult(chatHub, session, options, "No command supplied. Use /help.");
    }
    ...
    if (command === "assembly" || command === "assemblies") {
        const result = executeFrontierAssemblyCommand(session, argumentText, options.assemblyCommandOptions || {});
        return handledResult(chatHub, session, options, result.message);
    }
    if (command === "npc") {
    ...
    if (command === "gateconcord") {
        return handleGateOperatorCommand(session, argumentText, chatHub, options, npcService.GATE_OPERATOR_KIND.CONCORD);
    }
    if (command === "gaterats") {
        return handleGateOperatorCommand(session, argumentText, chatHub, options, npcService.GATE_OPERATOR_KIND.RATS);
    }
    if (command === "invu") {
    ...
    if (command === "supertitanshow" || command === "titansupershow") {
        const result = executeSuperTitanShowCommand(session, argumentText, options);
        return handledResult(chatHub, session, options, result.message);
    }
    if (command === "rr") {
        const result = executeRemoteRepairFleetCommand(session, argumentText, options);
        return handledResult(chatHub, session, options, result.message);
    }
    ...
    if (command === "who") {
        return handledResult(chatHub, session, options, getConnectedCharacterSummary());
    }
    if (command === "wallet" || command === "isk") {
        const summary = getWalletSummary(session);
        return handledResult(chatHub, session, options, summary || "Select a character before checking wallet balance.");
    }
    if (command === "lp") {
        return handledResult(chatHub, session, options, getLoyaltyPointSummary(session, argumentText));
    }
    if (command === "evermarks") {
        if (!session || !session.characterID) {
            return handledResult(chatHub, session, options, "Select a character before checking EverMarks.");
        }
        const evermarks = getCharacterWalletLPBalance(session.characterID, EVERMARK_ISSUER_CORP_ID);
        return handledResult(chatHub, session, options, `EverMarks: ${formatEvermarks(evermarks)}.`);
    }
    if (command === "corpevermarks") {
        if (!session || !(session.corporationID || session.corpid)) {
            return handledResult(chatHub, session, options, "Select a character in a corporation before checking corporation EverMarks.");
        }
        const corporationID = Number(session.corporationID || session.corpid) || 0;
        const evermarks = getCorporationWalletLPBalance(corporationID, EVERMARK_ISSUER_CORP_ID);
        return handledResult(chatHub, session, options, `Corporation EverMarks: ${formatEvermarks(evermarks)}.`);
    }
    if (command === "grantshipemblem" ||
        command === "grantcorplogo" ||
        command === "grantalliancelogo") {
        return handleGrantShipLogoCommand(command, session, argumentText, chatHub, options);
    }
    if (command === "corpcreate") {
        if (!session || !session.characterID) {
            return handledResult(chatHub, session, options, "Select a character before creating a corporation.");
        }
        if (!argumentText) {
            return handledResult(chatHub, session, options, "Usage: /corpcreate <corporation name>");
        }
        const result = createCustomCorporation(session.characterID, argumentText);
        if (!result.success) {
            const message = result.errorMsg === "CORPORATION_NAME_TAKEN"
        ...
        return handledResult(chatHub, session, options, `Created corporation ${result.data.corporationRecord.corporationName} [${result.data.corporationRecord.tickerName}] and moved your character into it.`);
    }
    if (command === "setalliance") {
        if (!session || !session.characterID) {
            return handledResult(chatHub, session, options, "Select a character before creating an alliance.");
        }
        if (!argumentText) {
            return handledResult(chatHub, session, options, "Usage: /setalliance <alliance name>");
        }
        const corporationRecord = getCorporationRecord(session.corporationID);
        if (!corporationRecord) {
            return handledResult(chatHub, session, options, "Current corporation could not be resolved.");
        }
        const result = createCustomAllianceForCorporation(session.characterID, corporationRecord.corporationID, argumentText);
        if (!result.success) {
        ...
        refreshAffiliationSessions(result.data.affectedCharacterIDs);
        return handledResult(chatHub, session, options, `Created alliance ${result.data.allianceRecord.allianceName} [${result.data.allianceRecord.shortName}] and set your corporation into it.`);
    }
    if (command === "joinalliance") {
        if (!session || !session.characterID) {
            return handledResult(chatHub, session, options, "Select a character before joining an alliance.");
        }
        if (!argumentText) {
            return handledResult(chatHub, session, options, "Usage: /joinalliance <alliance name>");
        }
        const corporationRecord = getCorporationRecord(session.corporationID);
        if (!corporationRecord) {
            return handledResult(chatHub, session, options, "Current corporation could not be resolved.");
        }
        const result = joinCorporationToAllianceByName(corporationRecord.corporationID, argumentText);
        if (!result.success) {
        ...
        refreshAffiliationSessions(result.data.affectedCharacterIDs);
        return handledResult(chatHub, session, options, `Joined alliance ${result.data.allianceRecord.allianceName} [${result.data.allianceRecord.shortName}].`);
    }
    if (command === "solar") {
    ...
    if (command === "maxagentstandings" || command === "fullstandings") {
        return handleMaxAgentStandingsCommand(session, argumentText, chatHub, options);
    }
    if (command === "sigscan") {
    ...
    if (command === "missioncomplete") {
        return handleMissionCompleteCommand(session, argumentText, chatHub, options);
    }
    ...
    if (command === "addisk") {
        if (!session || !session.characterID) {
            return handledResult(chatHub, session, options, "Select a character before changing wallet balance.");
        }
        const amount = parseAmount(argumentText);
        if (amount === null) {
            return handledResult(chatHub, session, options, "Usage: /addisk <amount>");
        }
        ...
        if (!result.success) {
            return handledResult(chatHub, session, options, result.errorMsg === "INSUFFICIENT_FUNDS"
                ? "Wallet change failed: insufficient funds."
                : "Wallet change failed.");
        }
        return handledResult(chatHub, session, options, `Adjusted wallet by ${formatIsk(amount)}. New balance: ${formatIsk(result.data.balance)}.`);
    }
    if (command === "addlp") {
        if (!session || !session.characterID) {
            return handledResult(chatHub, session, options, "Select a character before changing LP.");
        }
        const parsed = parseLoyaltyPointMutationArgs(argumentText);
        if (parsed.amount === null || !parsed.issuerText) {
            return handledResult(chatHub, session, options, "Usage: /addlp <amount> <npc corp name|corpID>");
        }
        const issuerResult = resolveLoyaltyPointIssuerCorporation(parsed.issuerText);
        if (issuerResult.success === false) {
            return handledResult(chatHub, session, options, formatLoyaltyPointIssuerError(issuerResult, parsed.issuerText));
        }
        const result = adjustCharacterWalletLPBalance(session.characterID, issuerResult.data.corporationID, parsed.amount, { changeType: "admin_adjust" });
        if (!result.success) {
            return handledResult(chatHub, session, options, "LP change failed.");
        }
        return handledResult(chatHub, session, options, `Adjusted ${formatCorporationLabel(issuerResult.data)} LP by ${formatSignedLoyaltyPoints(parsed.amount)}. New balance: ${formatLoyaltyPoints(result.data.amount)}.`);
    }
    if (command === "addevermarks") {
        if (!session || !session.characterID) {
            return handledResult(chatHub, session, options, "Select a character before changing EverMarks.");
        }
        const amount = parseAmount(argumentText);
        if (amount === null) {
            return handledResult(chatHub, session, options, "Usage: /addevermarks <amount>");
        }
        const result = adjustCharacterWalletLPBalance(session.characterID, EVERMARK_ISSUER_CORP_ID, amount, { changeType: "admin_adjust" });
        if (!result.success) {
            return handledResult(chatHub, session, options, "EverMarks change failed.");
        }
        return handledResult(chatHub, session, options, `Adjusted EverMarks by ${formatSignedEvermarks(amount)}. New balance: ${formatEvermarks(result.data.amount)}.`);
    }
    if (command === "addcorpevermarks") {
        if (!session || !(session.corporationID || session.corpid)) {
            return handledResult(chatHub, session, options, "Select a character in a corporation before changing corporation EverMarks.");
        }
        const amount = parseAmount(argumentText);
        if (amount === null) {
            return handledResult(chatHub, session, options, "Usage: /addcorpevermarks <amount>");
        }
        const corporationID = Number(session.corporationID || session.corpid) || 0;
        const result = adjustCorporationWalletLPBalance(corporationID, EVERMARK_ISSUER_CORP_ID, amount, { reason: "admin_adjust" });
        if (!result.success) {
            return handledResult(chatHub, session, options, "Corporation EverMarks change failed.");
        }
        return handledResult(chatHub, session, options, `Adjusted corporation EverMarks by ${formatSignedEvermarks(amount)}. New balance: ${formatEvermarks(result.data.amount)}.`);
    }
    if (command === "addplex") {
        if (!session || !session.characterID) {
            return handledResult(chatHub, session, options, "Select a character before changing PLEX balance.");
        }
        const amount = parseAmount(argumentText);
        if (amount === null) {
            return handledResult(chatHub, session, options, "Usage: /addplex <amount>");
        }
        ...
        if (!result.success) {
            return handledResult(chatHub, session, options, "PLEX balance change failed.");
        }
        emitPlexBalanceChangeToSession(session, result.data.plexBalance);
        return handledResult(chatHub, session, options, `Adjusted PLEX by ${formatSignedPlex(amount)}. New balance: ${formatPlex(result.data.plexBalance)}.`);
    }
    if (command === "setisk") {
        if (!session || !session.characterID) {
            return handledResult(chatHub, session, options, "Select a character before changing wallet balance.");
        ...
        const amount = parseAmount(argumentText);
        if (amount === null) {
            return handledResult(chatHub, session, options, "Usage: /setisk <amount>");
        }
        ...
        return handledResult(chatHub, session, options, `Wallet balance set to ${formatIsk(result.data.balance)}.`);
    }
    if (command === "setlp") {
        if (!session || !session.characterID) {
            return handledResult(chatHub, session, options, "Select a character before changing LP.");
        ...
        const parsed = parseLoyaltyPointMutationArgs(argumentText);
        if (parsed.amount === null || !parsed.issuerText) {
            return handledResult(chatHub, session, options, "Usage: /setlp <amount> <npc corp name|corpID>");
        }
        const issuerResult = resolveLoyaltyPointIssuerCorporation(parsed.issuerText);
        if (issuerResult.success === false) {
            return handledResult(chatHub, session, options, formatLoyaltyPointIssuerError(issuerResult, parsed.issuerText));
        }
        const result = setCharacterWalletLPBalance(session.characterID, issuerResult.data.corporationID, parsed.amount, { changeType: "admin_set" });
        if (!result.success) {
            return handledResult(chatHub, session, options, "LP change failed.");
        }
        return handledResult(chatHub, session, options, `${formatCorporationLabel(issuerResult.data)} LP set to ${formatLoyaltyPoints(result.data.amount)}.`);
    }
    if (command === "setevermarks") {
        ...
        const amount = parseAmount(argumentText);
        if (amount === null) {
            return handledResult(chatHub, session, options, "Usage: /setevermarks <amount>");
        }
        const result = setCharacterWalletLPBalance(session.characterID, EVERMARK_ISSUER_CORP_ID, amount, { changeType: "admin_set" });
        if (!result.success) {
            return handledResult(chatHub, session, options, "EverMarks change failed.");
        }
        return handledResult(chatHub, session, options, `EverMarks set to ${formatEvermarks(result.data.amount)}.`);
    }
    if (command === "setcorpevermarks") {
        ...
        const amount = parseAmount(argumentText);
        if (amount === null) {
            return handledResult(chatHub, session, options, "Usage: /setcorpevermarks <amount>");
        }
        const corporationID = Number(session.corporationID || session.corpid) || 0;
        const result = setCorporationWalletLPBalance(corporationID, EVERMARK_ISSUER_CORP_ID, amount, { reason: "admin_set" });
        if (!result.success) {
            return handledResult(chatHub, session, options, "Corporation EverMarks change failed.");
        }
        return handledResult(chatHub, session, options, `Corporation EverMarks set to ${formatEvermarks(result.data.amount)}.`);
    }
    if (command === "setplex") {
        if (!session || !session.characterID) {
            return handledResult(chatHub, session, options, "Select a character before changing PLEX balance.");
        ...
        const amount = parseAmount(argumentText);
        if (amount === null) {
            return handledResult(chatHub, session, options, "Usage: /setplex <amount>");
        }
        ...
        emitPlexBalanceChangeToSession(session, result.data.plexBalance);
        return handledResult(chatHub, session, options, `PLEX balance set to ${formatPlex(result.data.plexBalance)}.`);
    }
    ...
    if (rackCommandPreset) {
        return handlePresetSingleRackCommand(session, chatHub, options, rackCommandPreset, command);
    }
    if (command === "lesmis") {
    ...
    if (command === "typeinfo") {
        if (!argumentText) {
            return handledResult(chatHub, session, options, "Usage: /typeinfo <ship name|typeID>");
        }
        ...
        return handledResult(chatHub, session, options, `${lookup.match.name}: typeID=${lookup.match.typeID}, groupID=${lookup.match.groupID}, categoryID=${lookup.match.categoryID}, published=${lookup.match.published === false ? "false" : "true"}.`);
    }
    if (command === "announce") {
        if (!argumentText) {
            return handledResult(chatHub, session, options, "Usage: /announce <message>");
        }
        sendAnnouncement(chatHub, session, argumentText);
        return handledResult(chatHub, session, options, `Announcement sent: ${argumentText}`);
    }
    return handledResult(chatHub, session, options, `Unknown command: /${command}. Use /help.${formatSuggestions(suggestCommands(command))}`.trim());
}
module.exports = {
