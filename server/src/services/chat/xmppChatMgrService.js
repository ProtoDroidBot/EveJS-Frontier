...
const { getHelpChannelContract, getRookieHelpChannelContract, normalizeLanguageCode, } = require("../../_secondary/chat/staticChannelContracts");
const { getXmppConnectHost, } = require("./xmppConfig");
const { normalizeRoleValue, } = require("../account/accountRoleProfiles");
...
class XmppChatMgrService extends BaseService {
    ...
    Handle_ResyncSystemChannelAccess(args, session) {
        ...
        log.debug(`[XmppChatMgr] ResyncSystemChannelAccess -> ${JSON.stringify(channels)}`);
        return channels;
    }
    ...
    Handle_GMMute(args, session) {
        ...
        chatRuntime.muteChannelCharacter(channelName, characterID, durationSeconds * 1000, reason, getCharacterID(session));
        log.info(`[XmppChatMgr] GMMute channel=${channelName} characterID=${characterID} duration_seconds=${durationSeconds}`);
        return true;
    }
    Handle_GMUnmute(args, session) {
        ...
        log.info(`[XmppChatMgr] GMUnmute channel=${channelName} characterID=${characterID}`);
        return true;
    }
    Handle_EnsureResourceWarsChannelExists(args, session) {
        ...
        log.info(`[XmppChatMgr] EnsureResourceWarsChannelExists instanceID=${instanceID} room=${record ? record.roomName : ""}`);
    ...
    Handle_CreatePlayerOwnedChannel(args, session) {
        ...
        log.info(`[XmppChatMgr] CreatePlayerOwnedChannel displayName=${displayName} channelID=${created ? created.channelID : 0} room=${created && created.roomName ? created.roomName : ""}`);
    ...
    Handle_CreatePrivateChannel(args, session) {
        ...
        if (!targetCharacterID ||
            ...
            log.debug(`[XmppChatMgr] CreatePrivateChannel rejected caller=${callerCharacterID || 0} target=${targetCharacterID || 0}`);
            return null;
        }
        const record = chatRuntime.ensurePrivateChannelForCharacters(callerCharacterID, targetCharacterID);
        const roomName = record ? record.roomName : null;
        log.info(`[XmppChatMgr] CreatePrivateChannel caller=${callerCharacterID} target=${targetCharacterID} room=${roomName || ""}`);
        return roomName;
    }
    Handle_AddCharacterToChannel(args, session) {
        ...
        inviteCharacterToRoom(channelName, inviterCharacterID, targetCharacterID, reason);
        log.info(`[XmppChatMgr] AddCharacterToChannel channel=${channelName} target=${targetCharacterID} inviter=${inviterCharacterID}`);
        return true;
    }
    Handle_GrantChannelOwnership(args, session) {
        ...
        if (!channelName || !characterID || !hasChannelOwnershipGrantRole(session)) {
            log.debug(`[XmppChatMgr] GrantChannelOwnership denied channel=${channelName || "invalid"} characterID=${characterID || 0}`);
            return false;
        }
        ...
        const updatedRecord = chatRuntime.updateChannel(channelName, (record) => {
            ...
            return {
                ...record,
                denyCharacterIDs: (record.denyCharacterIDs || []).filter((candidate) => candidate !== characterID),
                bannedCharacters,
            };
        });
        log.info(`[XmppChatMgr] GrantChannelOwnership channel=${channelName} characterID=${characterID}`);
...
//# sourceMappingURL=xmppChatMgrService.js.map