...
const { resolveSpawnPosition, } = require(path.join(__dirname, "./frontierLandscapeCommands"));
...
function resolveDependencies(options = {}) {
    return {
        authority: options.authority || require(path.join(__dirname, "../../space/frontierRiftAuthority")),
        siteStore: options.siteStore || require(path.join(__dirname, "../../space/frontierRiftSites")),
...
function formatTemplateList(authority, query = "") {
    ...
    const templates = authority.listTemplates().filter((template) => (!normalizedQuery ||
    ...
    return [
        ...
        ...templates.map((template) => (`${template.dungeonID}: ${template.itemName} (type ${template.entryTypeID}, ` +
...
function formatTemplateInspection(authority, query) {
    ...
    return [
        ...
        `Resources: ${template.resources.map((resource) => (`${resource.itemName} (${resource.typeID})`)).join(", ") || "none"}.`,
...
function executeFrontierRiftCommand(session, argumentText, options = {}) {
        ...
        return {
            ...
            message: sites.length > 0
                ? [
                    ...
                    ...sites.map((site) => (`${site.itemID}: ${site.itemName} (dungeon ${site.dungeonID}) at ${formatPosition(site.position)}`)),
                ].join("\n")
    ...
    if (action === "spawn") {
        ...
        const liveResult = dependencies.spaceRuntime.addCustomRiftSite(createResult.data, { broadcast: true });
    ...
    if (action === "remove") {
        ...
        if (selector === "nearest" || selector === "here") {
            site = sites
                ...
                .sort((left, right) => (distanceSquared(left.position, active.ship.position) -
                distanceSquared(right.position, active.ship.position)))[0] || null;
        }
        else {
        ...
        const liveResult = dependencies.spaceRuntime.removeCustomRiftSite(site.itemID, site.solarSystemID, { broadcast: true });
        if (!liveResult || liveResult.success !== true) {
            return {
                success: false,
...
//# sourceMappingURL=frontierRiftCommands.js.map