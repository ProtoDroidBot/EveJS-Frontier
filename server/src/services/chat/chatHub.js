...
const { getCurrentSolarSystemID, getLocalChatRoomNameForSolarSystemID, isDelayedLocalChatRoomName, parseLocalChatRoomName, } = require("./channelRules");
const { sendSessionSystemMessage, moveSessionToCurrentLocalRoom, unregisterCharacterSession: unregisterXmppCharacterSession, refreshSessionChatRolePresence: refreshXmppSessionChatRolePresence, } = require("./xmppStubServer");
const { getXmppConferenceDomain, } = require("./xmppConfig");
...
function isDelayedLocalChannel(channel) {
    return isDelayedLocalChatRoomName(channel && (channel.comparisonKey || getLocalChannelName(channel.id)));
}
...
function getEstimatedMemberCount(channel) {
    ...
    return chatRuntime.getEstimatedMemberCount(channel.comparisonKey || getLocalChannelName(channel.id));
}
...
function buildChannelChars(channel) {
    const lines = getLocalChannelMembers(channel).map((session) => buildList([
        session.characterID || session.userid || 0,
...
function resolveSystemMessageTarget(session, specificChannel = null) {
    if (!specificChannel) {
        ...
        return {
            channel,
            roomJid: buildRoomJid(channel.comparisonKey || getLocalChannelName(channel.id)),
        };
    }
