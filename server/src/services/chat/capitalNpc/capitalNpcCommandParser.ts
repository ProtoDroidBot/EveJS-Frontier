...
function parseAmountAndQuery(argumentText, options: Record<string, any> = {}) {
...
function parseActionAndQuery(argumentText, options: Record<string, any> = {}) {
...
function parseQueryAndTarget(argumentText, options: Record<string, any> = {}) {
...
function parseActionQueryAndTarget(argumentText, options: Record<string, any> = {}) {
  const allowedActions = new Set(
    (Array.isArray(options.allowedActions) ? options.allowedActions : [])
      .map((entry) => String(entry || "").trim().toLowerCase())
      .filter(Boolean),
...
function parseQueryAndOptionalTarget(argumentText, options: Record<string, any> = {}) {
  const defaultQuery = String(options.defaultQuery || "").trim();
  const tokens = tokenizeArgumentText(argumentText);
  if (tokens.length <= 0) {
    return {
