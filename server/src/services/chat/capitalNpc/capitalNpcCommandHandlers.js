...
const { resolveFuelTypeID, resolveTitanSuperweaponProfileByModuleTypeID, } = require(path.join(__dirname, "../../superweapons/superweaponCatalog"));
const { inspectSuperweaponActivationContract, } = require(path.join(__dirname, "../../../space/modules/superweapons/superweaponRuntime"));
const { getCapitalControllerState, listControlledNpcFighters, toFiniteNumber, toPositiveInt, } = require(path.join(__dirname, "../../../space/npc/capitals/capitalNpcState"));
const { resolveCapitalDoctrine, } = require(path.join(__dirname, "../../../space/npc/capitals/capitalNpcDoctrine"));
const { getCapitalRuntimeConfig, } = require(path.join(__dirname, "../../../space/npc/capitals/capitalNpcRuntimeConfig"));
const { resetNpcSupercarrierWing, } = require(path.join(__dirname, "../../fighter/npc/npcSupercarrierDirector"));
const { buildTrackedTubeFlagSet, resetNpcSupercarrierTubeState, } = require(path.join(__dirname, "../../fighter/npc/npcSupercarrierTubeState"));
const { parseAmountAndQuery, parseActionAndQuery, parseQueryAndTarget, parseActionQueryAndTarget, parseQueryAndOptionalTarget, } = require("./capitalNpcCommandParser");
const { resolveCapitalSelection, getSystemCapitalSummaries, resolveTargetEntity, resolveSessionSystemID, } = require("./capitalNpcCommandResolver");
const { formatSpawnSummary, formatSelectionInfo, formatLiveStatus, formatManualOrderResult, formatClearResult, formatFighterStatus, formatFighterReset, formatSuperStatus, formatPerfSummary, formatSignoffSummary, } = require("./capitalNpcCommandFormatter");
...
function getEntityCargoQuantityByType(entity, typeID) {
    ...
    return (Array.isArray(entity.nativeCargoItems) ? entity.nativeCargoItems : [])
        ...
        .reduce((sum, item) => sum + (Number(item && item.quantity) ||
...
function summarizeWingTypes(wingEntries = []) {
    ...
    for (const wingEntry of wingEntries) {
        ...
        const fighterName = String((fighterType && (fighterType.typeName || fighterType.name)) ||
            `type ${fighterTypeID}`).trim();
...
function resolveTitanReadiness(scene, entity, behaviorProfile, targetEntity) {
    ...
    const moduleItem = (Array.isArray(entity && entity.fittedItems) ? entity.fittedItems : []).find((item) => (toPositiveInt(item && item.typeID, 0) === moduleTypeID)) || null;
    if (!moduleItem) {
        return {
            ready: false,
            moduleTypeID,
...
function handleCapnpc(session, argumentText) {
    ...
    if (!capitalResolution.success) {
        return {
            handled: true,
            message: buildSelectionErrorMessage(parsedArguments.query, capitalResolution.errorMsg, capitalResolution.suggestions),
        };
    }
...
function handleCapnpcstatus(session, argumentText) {
    ...
    return {
        handled: true,
        message: formatLiveStatus(liveResult.data.selection.label, liveResult.data.summaries, liveResult.data.scene),
    };
}
...
function handleCapnpctarget(session, argumentText) {
    ...
    if (!targetResult.success) {
        return {
            handled: true,
            message: targetResult.errorMsg === "SHIP_NOT_FOUND"
    ...
    let updatedCount = 0;
    for (const summary of liveResult.data.summaries) {
        const orderResult = issueCapitalAttackOrder(summary.entityID, targetResult.data.itemID, { wakeController: false });
    ...
    return {
        handled: true,
        message: formatManualOrderResult("Set attack order for", liveResult.data.selection.label, updatedCount, `${targetResult.data.itemName || targetResult.data.itemID}`),
    };
}
function handleCapnpchome(session, argumentText) {
    ...
    return {
        handled: true,
        message: formatManualOrderResult("Queued return-home for", liveResult.data.selection.label, updatedCount),
    };
}
    ...
    if (parsed.action === "status") {
        const entries = fighterControllers.map((summary) => {
            ...
            return {
                ...
                nextLaunchMs: Math.max(0, toFiniteNumber(capitalState && capitalState.nextFighterLaunchAtMs, 0) -
...
function handleCapnpcsuper(session, argumentText) {
    ...
    if (parsed.action === "status") {
        const entries = titanSummaries.map((summary) => {
            ...
            const behaviorProfile = controller && controller.behaviorProfile;
            const moduleTypeID = toPositiveInt(behaviorProfile && behaviorProfile.capitalSuperweaponModuleTypeID, 0);
            const moduleItem = (Array.isArray(entity && entity.fittedItems) ? entity.fittedItems : []).find((item) => (toPositiveInt(item && item.typeID, 0) === moduleTypeID));
            ...
            return {
                ...
                nextAttemptMs: Math.max(0, toFiniteNumber(capitalState && capitalState.nextSuperweaponAttemptAtMs, 0) -
                    liveResult.data.scene.getCurrentSimTimeMs()),
            };
        });
        return {
    ...
    return {
        handled: true,
        message: formatManualOrderResult("Armed superweapons for", liveResult.data.selection.label, armedCount, `${targetResult.data.itemName || targetResult.data.itemID}`),
    };
}
function handleCapnpcsignoff(session, argumentText) {
    ...
    if (Array.isArray(behaviorProfile.capitalFighterWingTypeIDs)) {
        ...
        const launchQuota = Math.max(1, toPositiveInt(behaviorProfile.capitalFighterLaunchPerThink, runtimeConfig.fighterLaunchPerThink));
        const launchIntervalMs = Math.max(250, toPositiveInt(behaviorProfile.capitalFighterLaunchIntervalMs, runtimeConfig.fighterLaunchIntervalMs));
        const abilitySyncIntervalMs = Math.max(250, toPositiveInt(behaviorProfile.capitalFighterAbilitySyncIntervalMs, runtimeConfig.fighterAbilitySyncIntervalMs));
        const fullWingMs = Math.max(0, (Math.max(0, Math.ceil(wingEntries.length / launchQuota) - 1) * launchIntervalMs));
...
function handleCapnpcperf(session, argumentText) {
    ...
    const capitalSummaries = npcService.getNpcOperatorSummary().filter((summary) => (summary &&
    ...
    const fighterCount = capitalSummaries.reduce((sum, summary) => sum + listControlledNpcFighters(scene, summary.entityID).length, 0);
    return {
        handled: true,
...
//# sourceMappingURL=capitalNpcCommandHandlers.js.map