...
function formatFighterStatus(selectionLabel, entries) {
    ...
    const parts = entries.slice(0, 5).map((entry) => (`${entry.profileID}#${entry.entityID} fighters=${entry.fighterCount} tracked=${entry.trackedTubeCount} nextLaunch=${formatDurationMs(entry.nextLaunchMs)}`));
...
function formatSuperStatus(selectionLabel, entries) {
    ...
    const parts = entries.slice(0, 5).map((entry) => (`${entry.profileID}#${entry.entityID} module=${entry.moduleTypeID || "none"} fuel=${entry.fuelQuantity} active=${entry.active ? "yes" : "no"} nextAttempt=${formatDurationMs(entry.nextAttemptMs)}`));
...
//# sourceMappingURL=capitalNpcCommandFormatter.js.map