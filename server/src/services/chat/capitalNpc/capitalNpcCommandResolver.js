...
const { resolveCapitalNpcCommandQuery, getCapitalNpcProfileRow, getCapitalNpcSpawnPoolRow, getCapitalNpcAuthorityEntry, } = require(path.join(__dirname, "../../../space/npc/capitals/capitalNpcCatalog"));
...
function resolveSessionSystemID(session) {
    return toPositiveInt(session &&
        session._space &&
        session._space.systemID, toPositiveInt(session && session.solarsystemid2, toPositiveInt(session && session.solarsystemid, 0)));
}
function resolveSessionShipID(session) {
    return toPositiveInt(session &&
        session._space &&
        session._space.shipID, toPositiveInt(session && session.shipItem && session.shipItem.itemID, 0));
}
...
function getSystemCapitalSummaries(npcService, session, query = "") {
    ...
    const summaries = npcService.getNpcOperatorSummary().filter((summary) => (summary &&
...
//# sourceMappingURL=capitalNpcCommandResolver.js.map