...
const nativeNpcService = require(path.join(__dirname, "../../../space/npc/nativeNpcService"));
const { ENTITY_TYPE, } = require(path.join(__dirname, "../../../space/entityConstants"));
const { DRIFTER_VARIANTS: AUTHORITY_DRIFTER_VARIANTS, DRIFTER_BEHAVIOR_FAMILY, buildDrifterBehaviorProfile, } = require(path.join(__dirname, "../../../space/npc/trigDrifter/trigDrifterNpcCatalog"));
const { resolveItemByName, resolveItemByTypeID, } = require(path.join(__dirname, "../../inventory/itemTypeRegistry"));
const { resolveTrigCommandPresetKey, } = require(path.join(__dirname, "../../ship/devCommandShipRuntime"));
...
function buildTrigVariant(options) {
    return Object.freeze({
        ...
        extraModuleTypeIDs: Object.freeze((Array.isArray(options.extraModuleTypeIDs) ? options.extraModuleTypeIDs : [])
...
const TRIG_VARIANTS_BY_ALIAS = Object.freeze(Object.fromEntries(TRIG_VARIANTS.flatMap((variant) => (variant.aliases.map((alias) => [
...
const DRIFTER_VARIANTS = Object.freeze(Object.fromEntries(AUTHORITY_DRIFTER_VARIANTS.map((variant) => [variant.key, Object.freeze({ ...variant })])));
const DRIFTER_VARIANTS_BY_ALIAS = Object.freeze(Object.fromEntries(AUTHORITY_DRIFTER_VARIANTS.flatMap((variant) => ((Array.isArray(variant.aliases) ? variant.aliases : [])
...
function resolveDrifterVariant(query) {
    ...
    return (DRIFTER_VARIANTS[normalized] ||
...
function clearOperatorKindInSystem(systemID, operatorKind) {
    ...
    for (const summary of npcService.getNpcOperatorSummary()) {
        if (toInt(summary && summary.systemID, 0) !== targetSystemID ||
            String(summary && summary.operatorKind || "").trim() !== targetOperatorKind) {
            continue;
        }
        const destroyResult = npcService.destroyNpcControllerByEntityID(toInt(summary && summary.entityID, 0), { removeContents: true });
...
function getOperatorSummariesInSystem(systemID, operatorKind) {
    ...
    return npcService.getNpcOperatorSummary()
        .filter((summary) => (toInt(summary && summary.systemID, 0) === targetSystemID &&
        String(summary && summary.operatorKind || "").trim() === targetOperatorKind));
}
function buildOperatorStatusMessage(context, operatorKind, label) {
    const summaries = getOperatorSummariesInSystem(context && context.systemID, operatorKind);
    ...
    for (const summary of summaries) {
        const entity = context && context.scene
            ...
            : null;
        const name = String(entity && (entity.itemName ||
            entity.slimName) ||
...
function buildTrigDefinition(variant, index) {
    ...
    const combatOrbitDistanceMeters = Math.max(1_000, toInt(variant && variant.spawnDistanceMeters, 12_000));
    return {
        profile: {
...
function buildDrifterDefinition(variant, index, options = {}) {
    ...
    const reinforcementVariantQuery = String(variant && (variant.reinforcementVariantKey ||
        variant.reinforcementVariantQuery) || "").trim();
    const reinforcementVariant = reinforcementVariantQuery
        ? resolveDrifterVariant(reinforcementVariantQuery)
        : null;
    const reinforcementAmount = Math.max(0, toInt(variant && variant.reinforcementAmount, 0));
    const reinforcementDefinitions = reinforcementVariant && reinforcementAmount > 0
        ? Array.from({ length: reinforcementAmount }, (_, reinforcementIndex) => (buildDrifterDefinition(reinforcementVariant, reinforcementIndex, {
            behaviorFamily,
        }))).map((definition) => ({
            ...definition,
            behaviorProfile: {
    ...
    return {
        ...
        behaviorProfile: {
            ...buildDrifterBehaviorProfile(variant, `gm_drifter_${slug}${familySuffix}_${suffix}`, reinforcementDefinitions, {
                behaviorFamily,
            }),
        },
        lootTable: null,
...
function spawnDefinitionPackForSession(session, definitions, options = {}) {
    ...
    const spawnResult = nativeNpcService.spawnNativeDefinitionsInContext(context, selectionResult, {
        transient: true,
...
function executeTrigSpawnCommand(session, argumentText = "") {
    ...
    if (normalizedQuery === "status" || normalizedQuery === "state") {
        ...
        return {
            success: true,
            message: buildOperatorStatusMessage(contextResult.data, TRIG_OPERATOR_KIND, "/trigspawn"),
        };
    }
    if (normalizedQuery === "clear" || normalizedQuery === "cleanup" || normalizedQuery === "reset") {
        ...
        const clearedCount = clearOperatorKindInSystem(contextResult.data.systemID, TRIG_OPERATOR_KIND);
        return {
            success: true,
...
function executeDrifterCommand(session, argumentText = "") {
    ...
    if (normalizedQuery === "status" || normalizedQuery === "state") {
        ...
        return {
            success: true,
            message: buildOperatorStatusMessage(contextResult.data, DRIFTER_OPERATOR_KIND, "/drifter"),
        };
    }
    if (normalizedQuery === "clear" || normalizedQuery === "cleanup" || normalizedQuery === "reset") {
        const contextResult = buildSpawnContextForSession(session);
        ...
        const clearedCount = clearOperatorKindInSystem(contextResult.data.systemID, DRIFTER_OPERATOR_KIND);
        return {
            success: true,
            message: clearedCount > 0
    ...
    if (randomSelection && randomSelection.random === true) {
        ...
        const familyLabel = randomSelection.behaviorFamily &&
        ...
        const result = spawnDefinitionPackForSession(session, definitions, {
            ...
            spawnDistanceMeters: Math.max(1_000, ...variants.map((variant) => toInt(variant && variant.spawnDistanceMeters, 20_000))),
    ...
    const definitions = Array.from({ length: parsed.amount }, (_, index) => buildDrifterDefinition(variant, index, {
