...
function cloneValue(value) {
  ...
  if (typeof value === "object") {
    return Object.fromEntries(
      Object.entries<any>(value).map(([key, entryValue]) => [key, cloneValue(entryValue)]),
    );
  }
  return value;
}
...
function getAlphaCapsByTypeID() {
  ...
  for (const [typeID, maxLevel] of Object.entries<any>(authorityCaps)) {
...
function getMaxTrainableLevelForClone(skillTypeID, options: Record<string, any> = {}) {
  const cloneGrade =
...
function isSkillLevelRestrictedForClone(skillTypeID, level, options: Record<string, any> = {}) {
  return getMaxTrainableLevelForClone(skillTypeID, options) < toInt(level, 0);
}

function buildCurrentPointsBySkillType(skillRecords: any[] = []) {
...
function getAlphaTrainingCapBreakdown(characterID, queueEntries: any[] = [], options: Record<string, any> = {}) {
  const cloneGrade =
    options.cloneGrade !== undefined && options.cloneGrade !== null
      ? toInt(options.cloneGrade, CLONE_STATE_ALPHA)
...
function getCharacterCloneRestrictionSummary(characterID, options: Record<string, any> = {}) {
