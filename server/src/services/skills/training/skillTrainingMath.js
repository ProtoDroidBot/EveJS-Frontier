...
const { applyActiveImplantAttributeBonuses, } = require(path.join(__dirname, "../../dogma/implants/activeImplantModifiers"));
...
function getSkillPointsForLevel(skillRank, level) {
    ...
    return Math.ceil(normalizedRank * 250 * 2 ** (2.5 * (normalizedLevel - 1)));
}
...
function normalizeCharacterAttributes(characterRecord = {}) {
    const source = characterRecord && typeof characterRecord === "object"
...
function getEffectiveSkillPointsPerMinute(characterRecord, skillTypeID, accountID = 0) {
    return (getBaseSkillPointsPerMinute(characterRecord, skillTypeID, accountID) *
...
function getTrainingDurationFiletimeTicks(remainingSkillPoints, skillPointsPerMinute) {
    ...
    const durationTicks = Math.floor((normalizedRemainingSkillPoints / normalizedSpPerMinute) *
        Number(FILETIME_TICKS_PER_MINUTE));
    return BigInt(Math.max(0, durationTicks));
}
function getEstimatedSkillPointsAtTime(startSkillPoints, destinationSkillPoints, startFileTime, sampleFileTime, skillPointsPerMinute) {
    ...
    const gainedPoints = Math.floor((Number(elapsedTicks) / Number(FILETIME_TICKS_PER_MINUTE)) * normalizedSpPerMinute);
    return Math.max(fromSkillPoints, Math.min(toSkillPoints, fromSkillPoints + Math.max(0, gainedPoints)));
}
...
//# sourceMappingURL=skillTrainingMath.js.map