interface FreeSkillPointRequest {
  entries?: any[];
  pointsBySkillTypeID?: Map<number, number> | Record<string, number>;
}
...
function buildDict(entries: any[] = []) {
  return {
    type: "dict",
    entries,
  };
...
function getAvailableTrainingSlotCount(accountID) {
  ...
  for (const expiryValue of Object.values<any>(extraSlots || {})) {
...
function updateFinishedSkills(characterID, finishedSkills: any[] = []) {
  updateCharacterRecord(characterID, (record) => ({
    ...record,
...
function getSkillRecordForProjection(characterID, skillMap, typeID, options: Record<string, any> = {}) {
...
function settleCharacterTraining(characterID) {
  ...
  const finishedSkills: any[] = [];
  const changedSkills: any[] = [];
  const recentSkillPointChanges: any[] = [];
...
function getQueueSnapshot(characterID, options: Record<string, any> = {}) {
  const numericCharacterID = toInt(characterID, 0);
  if (numericCharacterID <= 0) {
    return {
      characterID: 0,
  ...
  if (
    ...
    const projectedSkillMap = new Map<any, any>(
...
function rebaseActiveTrainingProgress(characterID, options: Record<string, any> = {}) {
  const numericCharacterID = toInt(characterID, 0);
  if (numericCharacterID <= 0) {
    return {
...
function prepareQueueForAttributeChange(characterID, options: Record<string, any> = {}) {
  const numericCharacterID = toInt(characterID, 0);
  if (numericCharacterID <= 0) {
    return {
      success: false as const,
...
function syncQueueAfterAttributeChange(characterID, options: Record<string, any> = {}) {
  const numericCharacterID = toInt(characterID, 0);
  if (numericCharacterID <= 0) {
...
function scheduleQueueCompletion(characterID) {
  ...
  const now = getNowFileTime();
  const endTime: bigint = normalizeBigInt(snapshot.currentEntry.trainingEndTime, 0n);
...
function saveQueue(characterID, queueEntries, options: Record<string, any> = {}) {
  const numericCharacterID = toInt(characterID, 0);
  const characterRecord = getCharacterRecord(numericCharacterID);
  if (!characterRecord) {
    throwWrappedUserError("QueueSkillNotUploaded");
...
function abortTraining(characterID, options: Record<string, any> = {}) {
  const numericCharacterID = toInt(characterID, 0);
  const queueState = getCharacterQueueState(numericCharacterID);
  if (!queueState.active || queueState.queue.length === 0) {
    return getQueueSnapshot(numericCharacterID);
...
function previewFreeSkillPointsApplication(characterID, entries: any[] = []) {
...
function applyFreeSkillPointsInternal(characterID, entries: any[] | FreeSkillPointRequest = []) {
  let requestedPointsBySkillTypeID = null;
  let requestedEntries: any[] = Array.isArray(entries) ? entries : [];
  if (entries && !Array.isArray(entries) && typeof entries === "object") {
    ...
    if (entries.pointsBySkillTypeID instanceof Map) {
      ...
      requestedPointsBySkillTypeID = new Map(
        Object.entries<any>(entries.pointsBySkillTypeID).map(([typeID, points]) => [
          toInt(typeID, 0),
          toInt(points, 0),
        ]),
      );
...
function primeQueueSchedulers() {
  for (const [characterID, rawState] of Object.entries<any>(getAllQueueStates())) {
