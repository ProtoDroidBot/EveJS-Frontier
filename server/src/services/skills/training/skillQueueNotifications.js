...
const { applyCharacterToSession, syncShipFittingStateForSession, } = require(path.join(__dirname, "../../character/characterState"));
const { buildFiletimeLong, currentFileTime, } = require(path.join(__dirname, "../../_shared/serviceHelpers"));
const { buildModuleAttributeChangeEvent, sendOnMultiEventPairs, } = require(path.join(__dirname, "../../_shared/godmaMultiEvent"));
const { buildImplantAttributeChangePayloads, } = require(path.join(__dirname, "../../dogma/implants/activeImplantModifiers"));
const { buildIndustryAttributeChangePayloads, } = require(path.join(__dirname, "../../dogma/brain/providers/industryBrainProvider"));
const { syncCharacterDogmaBrain, syncCharacterDogmaState, } = require(path.join(__dirname, "../../dogma/brain/characterBrainRuntime"));
const { recordRecentSkillPointChangesFromDiff, } = require(path.join(__dirname, "../certificates/skillChangeTracker"));
...
function buildSkillQueuePayload(entries = []) {
    return {
        type: "list",
        items: entries.map((entry) => buildKeyVal([
...
function normalizeTrainedLevelPair(value, skillRecordsByTypeID = new Map()) {
    ...
    if (value && typeof value === "object") {
        ...
        const level = toInt(value.trainedSkillLevel ?? value.skillLevel ?? value.level ?? value.toLevel, 0);
        return typeID > 0 && level > 0 ? [typeID, level] : null;
    }
    const typeID = toInt(value, 0);
    const skillRecord = skillRecordsByTypeID.get(typeID);
    const level = toInt(skillRecord && (skillRecord.trainedSkillLevel ?? skillRecord.skillLevel), 0);
    return typeID > 0 && level > 0 ? [typeID, level] : null;
}
function buildTrainedLevelPairs(changedSkills = [], options = {}) {
    const skillRecordsByTypeID = new Map(changedSkills.map((record) => [toInt(record.typeID, 0), record]));
    ...
    for (const record of changedSkills) {
        ...
        if (previousRecord) {
            const previousLevel = toInt(previousRecord.trainedSkillLevel ?? previousRecord.skillLevel, 0);
...
function buildMultiEventPairFromModuleAttributeChange(change) {
    ...
    return {
        event: buildModuleAttributeChangeEvent(change[1], change[2], change[3], change[5], change[6], time),
        time,
    };
}
...
function notifySkillDogmaAttributesChangedAsMultiEvent(characterID, options = {}) {
    ...
    const send = () => sendOnMultiEventPairs(session, buildSkillMutationAttributeMultiEventPairs(session, characterID));
...
function emitSkillSessionState(session, characterID, changedSkillRecords = [], options = {}) {
    if (options.skipRecentSkillTracking !== true) {
        recordRecentSkillPointChangesFromDiff(characterID, changedSkillRecords, options.previousSkillMap);
    }
    if (!session || typeof session.sendNotification !== "function") {
        return;
    }
    ...
    if (removedSkills.length > 0) {
        session.sendNotification("OnServerSkillsRemoved", "clientID", [
            buildSkillNotificationDict(removedSkills
                .map((skillRecord) => buildRemovedSkillServerRecord(skillRecord))
                .filter(Boolean), skillTransportOptions),
            buildFiletimeLong(timeStamp),
        ]);
    }
    ...
    if (hasSkillMutation && options.syncDogmaState !== false) {
        const activeShipID = toInt(session.activeShipID || session.shipID || session.shipid, 0);
...
//# sourceMappingURL=skillQueueNotifications.js.map