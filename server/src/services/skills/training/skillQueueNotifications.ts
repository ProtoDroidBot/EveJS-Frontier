...
function buildSkillQueuePayload(entries: any[] = []) {
  return {
    type: "list",
...
function buildSkillNotificationInfo(skillRecord, options: Record<string, any> = {}) {
...
function buildSkillNotificationDict(skillRecords: any[] = [], options: Record<string, any> = {}) {
...
function buildTrainedLevelPairs(changedSkills: any[] = [], options: Record<string, any> = {}) {
...
function notifySkillDogmaBrainChanged(characterID, options: Record<string, any> = {}) {
...
function notifySkillDogmaAttributesChangedAsMultiEvent(characterID, options: Record<string, any> = {}) {
  const session = getLiveCharacterSession(characterID);
  if (!session || typeof session.sendNotification !== "function") {
    return false;
  }
...
function notifySkillQueueSaved(characterID, queueEntries: any[] = []) {
  const session = getLiveCharacterSession(characterID);
  if (!session || typeof session.sendNotification !== "function") {
    return;
  }
...
function notifySkillStateChanged(characterID, changedSkillRecords: any[] = [], options: Record<string, any> = {}) {
  recordRecentSkillPointChangesFromDiff(
    characterID,
    changedSkillRecords,
    options.previousSkillMap,
