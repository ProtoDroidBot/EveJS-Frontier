...
const { getCharacterRecord, updateCharacterRecord, } = require(path.join(__dirname, "../../character/characterState"));
const { buildSkillRecord, getCharacterBaseSkillMap, getCharacterBaseSkills, getSkillMutationVersion, getSkillTypeByID, replaceCharacterSkillRecords, } = require(path.join(__dirname, "../skillState"));
const { getRequiredSkillRequirements, } = require(path.join(__dirname, "../../fitting/liveFittingState"));
const { resolveCharacterAccountID, getTrainingSlotsForAccount, } = require(path.join(__dirname, "../../newEdenStore/storeState"));
const { buildFiletimeLong, normalizeBigInt, } = require(path.join(__dirname, "../../_shared/serviceHelpers"));
const { buildFiletimeString, cloneValue, FILETIME_TICKS_PER_SECOND, getEffectiveSkillPointsPerMinute, getEstimatedSkillPointsAtTime, getGlobalTrainingSpeedMultiplier, getNowFileTime, getSkillLevelForPoints, getSkillPointsForLevel, getTrainingDurationFiletimeTicks, } = require("./skillTrainingMath");
const { getAllQueueStates, getCharacterQueueState, getQueueMutationVersion, setCharacterQueueState, } = require("./skillQueueState");
const { notifyFreeSkillPointsChanged, notifyMultipleCharacterTrainingUpdated, notifySkillDogmaAttributesChangedAsMultiEvent, notifySkillDogmaBrainChanged, notifySkillQueuePaused, notifySkillQueueSaved, notifySkillStateChanged, } = require("./skillQueueNotifications");
const { ALPHA_MAX_TRAINING_SP, getAlphaTrainingCapBreakdown, getMaxTrainableLevelForClone, isSkillLevelRestrictedForClone, resolveCharacterCloneGrade, } = require("./skillCloneRestrictions");
const { recordRecentSkillPointChanges, } = require(path.join(__dirname, "../certificates/skillChangeTracker"));
...
function normalizeQueueInput(queueInfo) {
    ...
    const entries = [];
    const sourceEntries = queueInfo instanceof Map
        ? [...queueInfo.entries()]
    ...
    for (const [rawPosition, rawValue] of sourceEntries) {
        ...
        const typeID = toInt(unwrapValue(normalizedValue &&
            ...
            : Array.isArray(normalizedValue)
                ? normalizedValue[0]
                : null), 0);
        const toLevel = toInt(unwrapValue(normalizedValue &&
            typeof normalizedValue === "object" &&
            !Array.isArray(normalizedValue)
...
function getCharacterIDFromSession(session) {
    return toInt(session && (session.characterID || session.charid || session.userid), 0);
}
function getCharacterAccountID(characterID, session = null) {
    return (toInt(session && session.userid, 0) ||
...
function buildModifierCacheEntry(entry, typeKeys = ["typeID"]) {
    ...
    const typeID = toInt(typeKeys.map((key) => entry[key]).find((value) => toInt(value, 0) > 0), 0);
    return {
        typeID,
...
function sortModifierCacheEntries(left, right) {
    return (toInt(left.slot, 0) - toInt(right.slot, 0) ||
...
function persistSkillMap(characterID, skillMap) {
    return replaceCharacterSkillRecords(characterID, [...skillMap.values()].map((record) => cloneValue(record)));
}
function buildProjectedQueueEntries(characterID, characterRecord, queueState, skillMap, accountID) {
    ...
    if (!queueState.queue.length) {
        return {
            entries,
            queueEndTime: null,
            projectedSkillMap: new Map([...skillMap.entries()].map(([typeID, record]) => [typeID, cloneValue(record)])),
        };
    }
    const projectedSkillMap = new Map([...skillMap.entries()].map(([typeID, record]) => [typeID, cloneValue(record)]));
    ...
    for (let index = 0; index < queueState.queue.length; index += 1) {
        ...
        const trainingStartSP = toInt(skillRecord.trainedSkillLevel, 0) === entry.toLevel - 1
            ? Math.max(getSkillPointsForLevel(skillRecord.skillRank, entry.toLevel - 1), toInt(skillRecord.trainedSkillPoints, 0))
        ...
        const skillPointsPerMinute = getEffectiveSkillPointsPerMinute({
            ...characterRecord,
            characterID,
        }, entry.typeID, accountID);
        const durationTicks = queueActive
            ? getTrainingDurationFiletimeTicks(trainingDestinationSP - trainingStartSP, skillPointsPerMinute)
            : 0n;
        ...
        projectedSkillMap.set(entry.typeID, buildSatisfiedSkillRecord(characterID, skillRecord, entry.toLevel, trainingDestinationSP));
        if (queueActive) {
    ...
    return {
        entries,
        queueEndTime: entries.length > 0 ? entries[entries.length - 1].trainingEndTime : null,
        projectedSkillMap,
    };
}
function validateQueueEntries(characterID, characterRecord, queueEntries) {
    ...
    const projectedLevels = new Map([...skillMap.entries()].map(([typeID, record]) => [
        typeID,
        {
    ...
    const projectedQueue = buildProjectedQueueEntries(characterID, characterRecord, normalizedQueueState, skillMap, getCharacterAccountID(characterID));
    const queueStartTime = normalizeBigInt(normalizedQueueState.activeStartTime, 0n);
    if (projectedQueue.queueEndTime &&
        projectedQueue.queueEndTime - queueStartTime > SKILLQUEUE_TIME_LIMIT_TICKS) {
        throwWrappedUserError("QueueTooLong");
    }
    for (const entry of queueEntries) {
        const skillRecord = getSkillRecordForProjection(characterID, skillMap, entry.typeID);
        if (!skillRecord) {
        ...
        if (isSkillLevelRestrictedForClone(entry.typeID, entry.toLevel, {
            characterID,
        })) {
...
function settleCharacterTraining(characterID) {
    ...
    while (queueState.queue.length > 0) {
        const projectedQueue = buildProjectedQueueEntries(numericCharacterID, characterRecord, queueState, skillMap, accountID);
        const currentEntry = projectedQueue.entries[0];
        if (!currentEntry) {
            break;
        }
        const currentSkillRecord = getSkillRecordForProjection(numericCharacterID, skillMap, currentEntry.trainingTypeID);
        ...
        const alreadySatisfied = toInt(currentSkillRecord.trainedSkillLevel, 0) >= currentEntry.trainingToLevel ||
        ...
        const completedRecord = buildSatisfiedSkillRecord(numericCharacterID, currentSkillRecord, currentEntry.trainingToLevel, currentEntry.trainingDestinationSP);
        ...
        recentSkillPointChanges.push({
            ...
            pointChange: Math.max(0, toInt(currentEntry.trainingDestinationSP, 0)) -
        ...
        appendSkillHistoryEntry(numericCharacterID, queueState.queue.length > 1
            ? SKILL_EVENT_QUEUE_TRAINING_COMPLETED
            : SKILL_EVENT_TRAINING_COMPLETE, currentEntry.trainingTypeID, currentEntry.trainingDestinationSP, currentEntry.trainingToLevel);
        queueState.queue.shift();
        queueState.active = queueState.queue.length > 0;
    ...
    if (finishedSkills.length > 0) {
        ...
        const updatedProjection = buildProjectedQueueEntries(numericCharacterID, characterRecord, queueState, skillMap, accountID);
...
function buildSkillRecordProjection(characterID, snapshotEntry, baseSkillMap) {
    const skillRecord = getSkillRecordForProjection(characterID, baseSkillMap, snapshotEntry.trainingTypeID);
    if (!skillRecord) {
        return null;
    }
    const now = getNowFileTime();
    const estimatedPoints = snapshotEntry.trainingStartTime && snapshotEntry.trainingEndTime
        ? getEstimatedSkillPointsAtTime(snapshotEntry.trainingStartSP, snapshotEntry.trainingDestinationSP, snapshotEntry.trainingStartTime, now, snapshotEntry.skillPointsPerMinute)
...
function getQueueSnapshot(characterID, options = {}) {
    ...
    if (cached &&
        ...
        const projectedSkillMap = new Map((cached.projectedSkills || []).map((record) => [record.typeID, cloneValue(record)]));
        if (snapshot.currentEntry && snapshot.active) {
            const liveProjection = buildSkillRecordProjection(numericCharacterID, {
    ...
    const queueState = getCharacterQueueState(numericCharacterID);
    const skillMap = getCharacterBaseSkillMap(numericCharacterID);
    const projection = buildProjectedQueueEntries(numericCharacterID, characterRecord, queueState, skillMap, accountID);
    ...
    const projectedSkillMap = new Map(getCharacterBaseSkills(numericCharacterID).map((record) => [record.typeID, record]));
    const currentEntry = projection.entries[0] || null;
    if (currentEntry) {
        const projectedRecord = buildSkillRecordProjection(numericCharacterID, currentEntry, skillMap);
        if (projectedRecord) {
    ...
    const snapshot = {
        ...
        queuePayload: {
            type: "list",
            items: queueEntries.map((entry) => buildKeyVal([
                ["trainingStartSP", entry.trainingStartSP],
                ["queuePosition", entry.queuePosition],
                ["trainingTypeID", entry.trainingTypeID],
                ["trainingDestinationSP", entry.trainingDestinationSP],
        ...
        queueEndTime: queueEntries.length > 0 ? queueEntries[queueEntries.length - 1].trainingEndTime : null,
        currentEntry: currentEntry && queueEntries.length > 0
            ? {
                ...queueEntries[0],
...
function buildRebasedTrainingRecord(characterID, currentEntry, skillRecord) {
    ...
    const estimatedPoints = trainingStartTime > 0n && trainingEndTime > 0n
        ? getEstimatedSkillPointsAtTime(trainingStartSP, trainingDestinationSP, trainingStartTime, getNowFileTime(), currentEntry.skillPointsPerMinute)
        : trainingStartSP;
    const nextSkillPoints = Math.max(trainingStartSP, Math.min(trainingDestinationSP, toInt(estimatedPoints, trainingStartSP)));
    const nextSkillLevel = getSkillLevelForPoints(skillRecord.skillRank, nextSkillPoints);
    return {
        ...cloneValue(skillRecord),
        skillLevel: nextSkillLevel,
...
function cloneSkillMap(skillMap) {
    return new Map([...skillMap.entries()].map(([typeID, record]) => [typeID, cloneValue(record)]));
}
function skillProgressChanged(previousRecord, nextRecord) {
    if (!previousRecord || !nextRecord) {
        return Boolean(nextRecord);
    }
    return (toInt(previousRecord.trainedSkillPoints, 0) !==
...
function rebaseActiveTrainingProgress(characterID, options = {}) {
    ...
    const skillRecord = getSkillRecordForProjection(numericCharacterID, skillMap, currentEntry.trainingTypeID);
    if (!skillRecord) {
        clearSnapshotCache(numericCharacterID);
        return {
            success: false,
    ...
    const rebasedRecord = buildRebasedTrainingRecord(numericCharacterID, currentEntry, skillRecord);
    const changed = skillProgressChanged(previousSkillMap.get(currentEntry.trainingTypeID), rebasedRecord);
...
function saveQueue(characterID, queueEntries, options = {}) {
    ...
    const shouldRebaseCurrentSkill = currentState.active &&
    ...
    if (nextActive && cloneGrade !== undefined) {
        ...
        const capBreakdown = getAlphaTrainingCapBreakdown(numericCharacterID, nextQueue.map((entry) => {
            const skillRecord = currentSnapshot.projectedSkillMap.get(entry.typeID) ||
    ...
    const preserveActiveStart = currentState.active &&
        nextActive &&
        currentState.queue.length > 0 &&
        nextQueue.length > 0 &&
    ...
    if (nextActive && snapshot.currentEntry) {
        const startedFresh = !currentState.active ||
            !preserveActiveStart ||
        ...
        if (startedFresh) {
            appendSkillHistoryEntry(numericCharacterID, SKILL_EVENT_TRAINING_STARTED, snapshot.currentEntry.trainingTypeID, snapshot.currentEntry.trainingStartSP, snapshot.currentEntry.trainingToLevel);
        }
        scheduleQueueCompletion(numericCharacterID);
    }
    else if (!nextActive) {
...
function abortTraining(characterID, options = {}) {
    ...
    if (currentEntry) {
        ...
        appendSkillHistoryEntry(numericCharacterID, SKILL_EVENT_TRAINING_CANCELLED, currentEntry.trainingTypeID, cancelPoints, currentEntry.trainingToLevel);
    }
...
function previewFreeSkillPointsApplication(characterID, entries = []) {
    ...
    for (const entry of sourceEntries) {
        ...
        const skillRecord = snapshot.projectedSkillMap.get(typeID) ||
        ...
        const targetPoints = getSkillPointsForLevel(skillRecord.skillRank, Math.min(toInt(entry.toLevel, 0), maxLevel));
...
function buildPointsDict(pointsBySkillTypeID) {
    return buildDict([...pointsBySkillTypeID.entries()].map(([typeID, points]) => [typeID, points]));
}
function applyFreeSkillPointsInternal(characterID, entries = []) {
    let requestedPointsBySkillTypeID = null;
    let requestedEntries = Array.isArray(entries) ? entries : [];
    if (entries && !Array.isArray(entries) && typeof entries === "object") {
        ...
        if (entries.pointsBySkillTypeID instanceof Map) {
            ...
            requestedPointsBySkillTypeID = new Map(Object.entries(entries.pointsBySkillTypeID).map(([typeID, points]) => [
                toInt(typeID, 0),
                toInt(points, 0),
            ]));
        }
    ...
    const numericCharacterID = toInt(characterID, 0);
    const snapshot = getQueueSnapshot(numericCharacterID);
    const pointsBySkillTypeID = requestedPointsBySkillTypeID ||
    ...
    for (const [typeID, appliedPoints] of pointsBySkillTypeID.entries()) {
        ...
        const nextSkillPoints = Math.max(0, toInt(skillRecord.trainedSkillPoints, 0) + pointsUsed);
        const nextSkillLevel = getSkillLevelForPoints(skillRecord.skillRank, nextSkillPoints);
        const nextRecord = {
            ...cloneValue(skillRecord),
            skillLevel: nextSkillLevel,
        ...
        appendSkillHistoryEntry(numericCharacterID, SKILL_EVENT_FREE_SKILL_POINTS_USED, typeID, nextSkillPoints, nextSkillLevel);
    }
...
function buildTrainingSelectionInfo(characterID) {
    ...
    return {
        ...
        fromSP: getSkillPointsForLevel(snapshot.projectedSkillMap.get(currentEntry.trainingTypeID)?.skillRank || 1, Math.max(0, currentEntry.trainingToLevel - 1)),
        finishSP: currentEntry.trainingDestinationSP,
        trainedSP: getEstimatedSkillPointsAtTime(currentEntry.trainingStartSP, currentEntry.trainingDestinationSP, currentEntry.trainingStartTime, getNowFileTime(), currentEntry.skillPointsPerMinute),
        finishedSkills,
    };
}
...
//# sourceMappingURL=skillQueueRuntime.js.map