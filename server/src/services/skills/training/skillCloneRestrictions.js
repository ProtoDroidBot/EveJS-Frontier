...
const { resolveCharacterAccountID, resolveOmegaLicenseState, } = require(path.join(__dirname, "../../newEdenStore/storeState"));
const { getCharacterBaseSkills, } = require(path.join(__dirname, "../skillState"));
const { CLONE_STATE_ALPHA, CLONE_STATE_OMEGA, getSkillPointsForLevel, } = require("./skillTrainingMath");
...
function getAlphaCapsByTypeID() {
    ...
    const authorityCaps = alphaCapsAuthority &&
...
function resolveCharacterCloneGrade(characterID, accountID = 0) {
    const numericAccountID = toInt(accountID, 0) || resolveCharacterAccountID(toInt(characterID, 0));
...
function getMaxTrainableLevelForClone(skillTypeID, options = {}) {
    const cloneGrade = options.cloneGrade !== undefined && options.cloneGrade !== null
...
function buildCurrentPointsBySkillType(skillRecords = []) {
    ...
    for (const record of Array.isArray(skillRecords) ? skillRecords : []) {
        ...
        currentPointsBySkillType.set(typeID, Math.max(0, toInt(record.trainedSkillPoints ?? record.skillPoints, 0)));
    }
    return currentPointsBySkillType;
}
function getCharacterTotalSkillPoints(characterID, skillRecords = null) {
    const sourceSkillRecords = Array.isArray(skillRecords) && skillRecords.length > 0
        ? skillRecords
        : getCharacterBaseSkills(characterID);
    return sourceSkillRecords.reduce((sum, record) => sum + Math.max(0, toInt(record && (record.trainedSkillPoints ?? record.skillPoints), 0)), 0);
}
function getAlphaTrainingCapBreakdown(characterID, queueEntries = [], options = {}) {
    ...
    const sourceSkillRecords = Array.isArray(options.skillRecords) && options.skillRecords.length > 0
    ...
    for (const entry of Array.isArray(queueEntries) ? queueEntries : []) {
        ...
        const skillPointsAlreadyTrainedThisLevel = Math.max(0, currentSkillPoints - previousLevelPoints);
        const addedSkillPoints = Math.max(0, totalSkillPointsInThisLevel - skillPointsAlreadyTrainedThisLevel);
        accumulatedSkillPoints += addedSkillPoints;
        const overAlphaTrainingCap = cloneGrade === CLONE_STATE_ALPHA &&
...
//# sourceMappingURL=skillCloneRestrictions.js.map