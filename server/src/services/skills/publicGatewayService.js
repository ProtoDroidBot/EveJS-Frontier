...
class PublicGatewayService extends BaseService {
    ...
    callMethod(method, args, session, kwargs) {
        ...
        if (args) {
            log.info(`[PublicGateway] args: ${JSON.stringify(args, (k, v) => (typeof v === "bigint" ? v.toString() : Buffer.isBuffer(v) ? v.toString("utf8") : v))}`);
        }
        if (kwargs) {
            log.info(`[PublicGateway] kwargs: ${JSON.stringify(kwargs, (k, v) => (typeof v === "bigint" ? v.toString() : Buffer.isBuffer(v) ? v.toString("utf8") : v))}`);
        }
...
//# sourceMappingURL=publicGatewayService.js.map