...
const { getCharacterSkills, getSkillTypeByID, } = require(path.join(__dirname, "./skillState"));
const { abortTraining, applyFreeSkillPoints, applyFreeSkillPointsInternal, buildPointsDict, getCharacterIDFromSession, getQueueSnapshot, normalizeQueueInput, previewFreeSkillPointsApplication, readBooleanKwarg, saveQueue, } = require(path.join(__dirname, "./training/skillQueueRuntime"));
const { checkInjectionConstraints, combineSkillInjector, extractSkills, getDiminishedSpFromInjectors, injectSkillPoints, splitSkillInjector, } = require(path.join(__dirname, "./trading/skillTradingRuntime"));
const { getDirectPurchasePrice, injectSkillbookItems, isSkillAvailableForDirectPurchase, isSkillInjected, isSkillType, purchaseSkills, } = require(path.join(__dirname, "./skillbooks/skillbookRuntime"));
const { consumeRecentSkillPointChanges, } = require(path.join(__dirname, "./certificates/skillChangeTracker"));
const { buildCharacterSkillDict, buildCharacterSkillEntry, } = require(path.join(__dirname, "./skillTransport"));
const { buildCharacterBoostersDict, } = require(path.join(__dirname, "./boosters/boosterRuntime"));
const { getActiveImplants, } = require(path.join(__dirname, "../dogma/implants/activeImplantModifiers"));
const { applyClientTrainingSpeedScale, } = require(path.join(__dirname, "./training/skillTrainingSpeed"));
...
function normalizeSkillRequirements(skillTypeIDsAndLevels) {
    ...
    if (skillTypeIDsAndLevels.type === "dict" &&
...
class SkillMgrService extends BaseService {
    ...
    _getLiveSkillPointTotal(session) {
        return this._getSnapshot(session).projectedSkills.reduce((sum, skillRecord) => sum + Number(skillRecord.trainedSkillPoints || skillRecord.skillPoints || 0), 0);
    }
    ...
    _buildSkillsDict(session, snapshot = this._getSnapshot(session)) {
        ...
        return buildCharacterSkillDict(this._overlayActiveTrainingSkill(skills, snapshot), {
            includeMetadata: true,
            compatibilityProfile: session && session.compatibilityProfile,
        });
    }
    ...
    Handle_GetSkillHistory(args, session) {
        ...
        return buildList(history.map((entry) => buildKeyVal([
    ...
    Handle_CharStartTrainingSkillByTypeID(args, session) {
        ...
        const targetLevel = explicitToLevel > 0
            ? explicitToLevel
            : Math.max(1, Math.min(5, Number(existingSkill
                ? existingSkill.trainedSkillLevel || existingSkill.skillLevel || 0
                : 0) + 1));
        const nextQueue = [
            { typeID, toLevel: targetLevel },
            ...snapshot.queueEntries
                .filter((entry) => Number(entry.trainingTypeID || 0) !== typeID ||
    ...
    Handle_CharStartTrainingSkill(args, session) {
        ...
        return this.Handle_CharStartTrainingSkillByTypeID([item.typeID, args && args.length > 1 ? args[1] : null], session);
    }
    ...
    Handle_GetRespecInfo(args, session) {
        ...
        const charData = this._getCharacterData(session);
        const respecInfo = charData.respecInfo && typeof charData.respecInfo === "object"
            ? charData.respecInfo
            : {};
        return {
            type: "object",
    ...
    Handle_GetFreeSkillPointsAppliedToQueue(args, session) {
        ...
        return buildPointsDict(previewFreeSkillPointsApplication(this._getCharacterId(session)));
    }
    Handle_GetFreeSkillPointsAppliedToSkills(args, session) {
        log.debug("[SkillMgr] GetFreeSkillPointsAppliedToSkills called");
        return buildPointsDict(previewFreeSkillPointsApplication(this._getCharacterId(session), normalizeSkillRequirements(args && args[0])));
    }
    ...
    Handle_ApplyFreeSkillPointsToSkills(args, session) {
        ...
        return applyFreeSkillPointsInternal(this._getCharacterId(session), normalizeSkillRequirements(args && args[0])).newFreeSkillPoints;
    }
    Handle_ApplyFreeSkillPoints(args, session) {
        log.debug("[SkillMgr] ApplyFreeSkillPoints called");
        return applyFreeSkillPoints(this._getCharacterId(session), unwrapValue(args && args[0]), unwrapValue(args && args[1]));
    }
    Handle_SaveNewQueue(args, session, kwargs) {
        log.debug("[SkillMgr] SaveNewQueue called");
        saveQueue(this._getCharacterId(session), normalizeQueueInput(args && args[0]), {
    ...
    Handle_InjectSkillpoints(args, session) {
        ...
        return injectSkillPoints(this._getCharacterId(session), toInt(unwrapValue(args && args[0]), 0), toInt(unwrapValue(args && args[1]), 1), session);
    }
    Handle_CheckInjectionConstraints(args, session) {
        log.debug("[SkillMgr] CheckInjectionConstraints called");
        checkInjectionConstraints(this._getCharacterId(session), toInt(unwrapValue(args && args[0]), 0), toInt(unwrapValue(args && args[1]), 1), session);
        return null;
    }
    Handle_GetDiminishedSpFromInjectors(args, session) {
        log.debug("[SkillMgr] GetDiminishedSpFromInjectors called");
        return getDiminishedSpFromInjectors(this._getCharacterId(session), toInt(unwrapValue(args && args[0]), 0), toInt(unwrapValue(args && args[1]), 1), toInt(unwrapValue(args && args[2]), 0));
    }
    Handle_ExtractSkills(args, session) {
        log.debug("[SkillMgr] ExtractSkills called");
        return extractSkills(this._getCharacterId(session), args && args[0], toInt(unwrapValue(args && args[1]), 0), session);
    }
    Handle_SplitSkillInjector(args, session) {
        log.debug("[SkillMgr] SplitSkillInjector called");
        return splitSkillInjector(this._getCharacterId(session), toInt(unwrapValue(args && args[0]), 0), toInt(unwrapValue(args && args[1]), 1), session);
    }
    Handle_CombineSkillInjector(args, session) {
        log.debug("[SkillMgr] CombineSkillInjector called");
        return combineSkillInjector(this._getCharacterId(session), toInt(unwrapValue(args && args[0]), 0), toInt(unwrapValue(args && args[1]), 1), session);
    }
    ...
    Handle_MachoBindObject(args, session, kwargs) {
        ...
        log.debug(`[SkillMgr] MachoBindObject args.length=${args ? args.length : 0} bindParams=${JSON.stringify(bindParams, (k, v) => (typeof v === "bigint" ? v.toString() : v))} nestedCall=${JSON.stringify(nestedCall, (k, v) => (typeof v === "bigint" ? v.toString() : Buffer.isBuffer(v) ? v.toString("utf8") : v))}`);
...
//# sourceMappingURL=skillMgrService.js.map