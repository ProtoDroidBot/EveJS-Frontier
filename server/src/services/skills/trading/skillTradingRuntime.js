...
const { notifyFreeSkillPointsChanged, notifySkillStateChanged, } = require(path.join(__dirname, "../training/skillQueueNotifications"));
const { getQueueSnapshot, } = require(path.join(__dirname, "../training/skillQueueRuntime"));
const { getSkillLevelForPoints, getSkillPointsForLevel, CLONE_STATE_ALPHA, getNowFileTime, } = require(path.join(__dirname, "../training/skillTrainingMath"));
const { getMaxTrainableLevelForClone, resolveCharacterCloneGrade, } = require(path.join(__dirname, "../training/skillCloneRestrictions"));
const { replaceCharacterSkillRecords, } = require(path.join(__dirname, "../skillState"));
...
const { CLIENT_INVENTORY_STACK_LIMIT, findItemById, grantItemToCharacterLocation, isCapsuleTypeID, removeInventoryItem, updateInventoryItem, } = require(path.join(__dirname, "../../inventory/itemStore"));
const { getCharacterSkillTradingState, updateCharacterSkillTradingState, } = require("./skillTradingState");
const { SKILL_TRADING_BUCKET_SIZE, SKILL_TRADING_MINIMUM_SP_TO_EXTRACT, SKILL_TRADING_SMALL_INJECTOR_DIVISOR, TYPE_LARGE_SKILL_INJECTOR, TYPE_SMALL_SKILL_INJECTOR, TYPE_SKILL_EXTRACTOR, buildDiminishingInjectionPreview, getFixedInjectorMaxUsableQuantity, getFixedInjectorSkillPointAmount, getInjectorPreviewSkillPoints, getSkillInjectorSpec, isSkillInjectorType, isSkillExtractorType, resolveNextDowntimeFileTime, } = require("./skillTradingAuthority");
const {} = require(path.join(__dirname, "../../_shared/serviceHelpers"));
const { notifyNonDiminishingInjectionsUsed, } = require("./skillTradingNotifications");
...
function getActiveShipIDFromSession(session) {
    return toInt(session && (session.shipID || session.shipid || session.activeShipID), 0);
}
...
function consumeStackQuantity(item, quantity) {
    ...
    const availableQuantity = Math.max(0, toInt(currentItem.stacksize ?? currentItem.quantity, currentItem.singleton === 1 ? 1 : 0));
...
function getProjectedAllocatedSkillPoints(snapshot) {
    return (snapshot.projectedSkills || []).reduce((sum, skillRecord) => sum + Math.max(0, toInt(skillRecord && skillRecord.trainedSkillPoints, 0)), 0);
}
function getProjectedTotalSkillPoints(snapshot) {
    return (getProjectedAllocatedSkillPoints(snapshot) +
...
function getRecursiveRequiredSkills(typeID, cache = new Map()) {
    ...
    for (const requirement of getDirectRequiredSkills(numericTypeID)) {
        ...
        for (const [nestedSkillTypeID, nestedLevel] of nestedRequirements.entries()) {
            resolved.set(nestedSkillTypeID, Math.max(nestedLevel, resolved.get(nestedSkillTypeID) || 0));
        }
    }
...
function buildSkillDependencyRequirementMap(skillRecords = []) {
    ...
    for (const skillRecord of Array.isArray(skillRecords) ? skillRecords : []) {
        ...
        for (const requirement of getDirectRequiredSkills(skillRecord.typeID)) {
            ...
            dependencyMap.set(requiredSkillTypeID, Math.max(requiredLevel, dependencyMap.get(requiredSkillTypeID) || 0));
        }
    }
...
function buildImplantRestrictionMap(characterRecord = {}) {
    const implants = [
        ...
        ...((Array.isArray(characterRecord.jumpClones) ? characterRecord.jumpClones : [])
            .flatMap((jumpClone) => (Array.isArray(jumpClone && jumpClone.implants) ? jumpClone.implants : []))),
    ];
    ...
    for (const implant of implants) {
        ...
        for (const [skillTypeID, requiredLevel] of requirements.entries()) {
            restrictionMap.set(skillTypeID, Math.max(requiredLevel, restrictionMap.get(skillTypeID) || 0));
        }
    }
...
function buildPlanetRestrictionMap(characterRecord = {}) {
    ...
    if (colonies.length > 0) {
        restrictionMap.set(TYPE_INTERPLANETARY_CONSOLIDATION, Math.max(0, colonies.length - 1));
    }
    let maxCommandCenterLevel = 0;
    for (const colony of colonies) {
        maxCommandCenterLevel = Math.max(maxCommandCenterLevel, toInt(colony && (colony.commandCenterLevel ??
            colony.colonyLevel ??
            colony.commandCenterUpgradeLevel), 0));
    }
...
function validateExtractionSelections(characterID, session, snapshot, requestedSelections) {
    const { getCharacterRecord } = getCharacterState();
    const characterRecord = getCharacterRecord(characterID) || {};
    const cloneGrade = resolveCharacterCloneGrade(characterID);
    const projectedSkillMap = new Map((snapshot.projectedSkills || []).map((record) => [toInt(record.typeID, 0), cloneValue(record)]));
    const queueSkillTypeIDs = new Set((snapshot.queueEntries || [])
    ...
    const totalRequestedPoints = requestedSelections.reduce((sum, entry) => sum + Math.max(0, toInt(entry.skillPoints, 0)), 0);
    ...
    const nextSkillMap = new Map([...projectedSkillMap.entries()].map(([typeID, record]) => [typeID, cloneValue(record)]));
...
function buildInjectorConstraintContext(characterID, injectorSpec, quantity, totalSkillPoints) {
    ...
    return {
        ...
        fixedUsableQuantity: getFixedInjectorMaxUsableQuantity(injectorSpec, normalizedQuantity, totalSkillPoints),
        diminishingPreview: injectorSpec && injectorSpec.injectionMode === "diminishing"
            ? buildDiminishingInjectionPreview(injectorSpec, normalizedQuantity, totalSkillPoints, nonDiminishingRemaining)
            : null,
    };
}
function checkInjectionConstraints(characterID, itemID, quantity, session = null) {
    const injectorItem = resolveOwnedSkillTradingItem(session, characterID, itemID, (item) => isSkillInjectorType(item.typeID));
    ...
    const context = buildInjectorConstraintContext(characterID, injectorSpec, quantity, totalSkillPoints);
    const requestedQuantity = context.quantity;
    const availableQuantity = Math.max(0, toInt(injectorItem.stacksize ?? injectorItem.quantity, injectorItem.singleton === 1 ? 1 : 0));
...
function injectSkillPoints(characterID, itemID, quantity, session = null) {
    ...
    const { injectorItem, injectorSpec, skillPointsToInject, nonDiminishingUsed, } = constraintResult;
    const consumeQuantity = injectorSpec.oncePerDowntime === true ? 1 : normalizeOptionalQuantity(quantity, 1);
    ...
    if (injectorSpec.oncePerDowntime === true || nonDiminishingUsed > 0) {
        updateCharacterSkillTradingState(characterID, (state) => ({
            ...state,
            nextAlphaInjectionAt: injectorSpec.oncePerDowntime === true
                ? resolveNextDowntimeFileTime().toString()
                : state.nextAlphaInjectionAt,
            nonDiminishingInjectionsRemaining: nonDiminishingUsed > 0
                ? Math.max(0, toInt(state.nonDiminishingInjectionsRemaining, 0) - nonDiminishingUsed)
    ...
    notifyFreeSkillPointsChanged(characterID, Math.max(0, toInt((updateCharacterResult.data || {}).freeSkillPoints, 0)));
    return skillPointsToInject;
}
function getDiminishedSpFromInjectors(characterID, typeID, quantity, nonDiminishingInjectionsRemaining = 0) {
    const injectorSpec = getSkillInjectorSpec(typeID);
    if (!injectorSpec) {
        return 0;
    }
    const snapshot = getLiveSkillSnapshot(characterID);
    return getInjectorPreviewSkillPoints(injectorSpec, normalizeOptionalQuantity(quantity, 1), getProjectedTotalSkillPoints(snapshot), Math.max(0, toInt(nonDiminishingInjectionsRemaining, 0)));
}
function extractSkills(characterID, selection, itemID, session = null) {
    ...
    const extractorItem = resolveOwnedSkillTradingItem(session, characterID, itemID, (item) => isSkillExtractorType(item.typeID));
    const snapshot = getLiveSkillSnapshot(characterID);
    const nextSkillRecords = validateExtractionSelections(characterID, session, snapshot, requestedSelections);
    ...
    const grantResult = grantItemToCharacterLocation(characterID, getDockedLocationID(session), ITEM_FLAG_HANGAR, TYPE_LARGE_SKILL_INJECTOR, 1);
    if (!grantResult.success) {
        throwWrappedUserError("CustomNotify", {
            notify: "Failed to create the extracted skill injector.",
        });
    }
    const changedSkillTypeIDs = new Set(requestedSelections.map((entry) => toInt(entry.skillTypeID, 0)).filter(Boolean));
    const changedSkills = (replaceResult.data || []).filter((record) => changedSkillTypeIDs.has(toInt(record.typeID, 0)));
...
function splitSkillInjector(characterID, itemID, quantity, session = null) {
    const injectorItem = resolveOwnedSkillTradingItem(session, characterID, itemID, (item) => toInt(item.typeID, 0) === TYPE_LARGE_SKILL_INJECTOR);
    ...
    const grantResult = grantItemToCharacterLocation(characterID, toInt(injectorItem.locationID, 0), toInt(injectorItem.flagID, ITEM_FLAG_HANGAR), TYPE_SMALL_SKILL_INJECTOR, requestedQuantity * SKILL_TRADING_SMALL_INJECTOR_DIVISOR);
    if (!grantResult.success) {
        throwWrappedUserError("CustomNotify", {
...
function combineSkillInjector(characterID, itemID, quantity, session = null) {
    const injectorItem = resolveOwnedSkillTradingItem(session, characterID, itemID, (item) => toInt(item.typeID, 0) === TYPE_SMALL_SKILL_INJECTOR);
    const requestedQuantity = normalizeOptionalQuantity(quantity, 1);
    ...
    const largeInjectorCount = Math.floor(requestedQuantity / SKILL_TRADING_SMALL_INJECTOR_DIVISOR);
    const smallInjectorsToConsume = largeInjectorCount * SKILL_TRADING_SMALL_INJECTOR_DIVISOR;
    const consumeResult = consumeStackQuantity(injectorItem, smallInjectorsToConsume);
    if (!consumeResult.success) {
        throwWrappedUserError("NotEnoughQuantity", {});
    }
    const grantResult = grantItemToCharacterLocation(characterID, toInt(injectorItem.locationID, 0), toInt(injectorItem.flagID, ITEM_FLAG_HANGAR), TYPE_LARGE_SKILL_INJECTOR, largeInjectorCount);
    if (!grantResult.success) {
        throwWrappedUserError("CustomNotify", {
...
//# sourceMappingURL=skillTradingRuntime.js.map