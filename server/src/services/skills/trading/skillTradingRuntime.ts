...
function normalizeExtractorSelection(selection) {
  ...
  if (typeof selection === "object") {
    return Object.entries<any>(selection)
      .map(([skillTypeID, skillPoints]) => ({
        skillTypeID: toInt(skillTypeID, 0),
        skillPoints: Math.max(0, toInt(skillPoints, 0)),
      }))
...
function getCurrentCloneCount(characterRecord: Record<string, any> = {}) {
...
function getRecursiveRequiredSkills(typeID, cache = new Map()) {
  ...
  if (recursiveRequiredSkillCache.has(numericTypeID)) {
    return new Map<any, any>(recursiveRequiredSkillCache.get(numericTypeID));
  }
  if (cache.has(numericTypeID)) {
    return new Map<any, any>(cache.get(numericTypeID));
  }
...
function buildSkillDependencyRequirementMap(skillRecords: any[] = []) {
...
function buildImplantRestrictionMap(characterRecord: Record<string, any> = {}) {
  const implants = [
...
function buildCloneRestrictionMap(characterRecord: Record<string, any> = {}) {
...
function buildPlanetRestrictionMap(characterRecord: Record<string, any> = {}) {
...
function throwExtractorRestrictionError(message = "SkillExtractionRestricted", values: Record<string, any> = {}) {
...
function validateExtractionSelections(
  ...
  const queueSkillTypeIDs = new Set<any>(
  ...
  const nextSkillMap = new Map<any, any>(
...
function extractSkills(characterID, selection, itemID, session = null) {
  ...
  const changedSkillTypeIDs = new Set<any>(
