interface InjectorSeedSpec {
  injectionFamily: string;
  injectionMode: "diminishing" | "fixed";
  alphaOnly?: boolean;
  oncePerDowntime?: boolean;
  maxQuantityPerUse?: number;
  fullSkillPoints?: number;
  fixedSkillPoints?: number;
  diminishingBands?: readonly { upperBound: number; skillPoints: number }[];
}
...
const INJECTOR_SEED_SPECS: Readonly<Record<number, InjectorSeedSpec>> = Object.freeze({
...
function getDiminishedSkillPointsForTotalSp(injectorSpec, totalSkillPoints, options: Record<string, any> = {}) {
  if (
    !injectorSpec ||
