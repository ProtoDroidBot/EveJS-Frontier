...
const { getNowFileTime, } = require(path.join(__dirname, "../training/skillTrainingMath"));
...
const SKILL_TRADING_MINIMUM_SP_TO_EXTRACT = SKILL_TRADING_FREE_ZONE + SKILL_TRADING_BUCKET_SIZE;
...
const SMALL_DIMINISHING_BANDS = Object.freeze(LARGE_DIMINISHING_BANDS.map((band) => ({
...
function buildAuthorityCache() {
    ...
    const dogmaByTypeID = typeDogma && typeof typeDogma.typesByTypeID === "object"
    ...
    for (const [rawTypeID, seed] of Object.entries(INJECTOR_SEED_SPECS)) {
        ...
        const dogmaAttributes = dogmaRow && typeof dogmaRow.attributes === "object" ? dogmaRow.attributes : {};
        injectorSpecsByTypeID.set(typeID, Object.freeze({
            typeID,
            ...
            fullSkillPoints: normalizePositiveInteger(seed.fullSkillPoints ?? dogmaAttributes[String(ATTRIBUTE_SKILL_POINTS)], 0),
            fixedSkillPoints: normalizePositiveInteger(seed.fixedSkillPoints ?? dogmaAttributes[String(ATTRIBUTE_SKILL_POINTS)], 0),
            characterSkillPointLimit: normalizePositiveInteger(dogmaAttributes[String(ATTRIBUTE_CHARACTER_SKILL_POINT_LIMIT)], 0),
...
function buildDiminishingInjectionPreview(injectorSpec, quantity, totalSkillPoints, nonDiminishingInjectionsRemaining = 0) {
    ...
    for (let index = 0; index < normalizedQuantity; index += 1) {
        ...
        const skillPoints = getDiminishedSkillPointsForTotalSp(injectorSpec, runningTotal, { nonDiminishing: useNonDiminishing });
    ...
    return {
        ...
        perUsePoints,
        nonDiminishingUsed: Math.max(0, toInt(nonDiminishingInjectionsRemaining, 0)) - remainingNonDiminishing,
    };
}
function getFixedInjectorSkillPointAmount(injectorSpec) {
    return normalizePositiveInteger(injectorSpec && (injectorSpec.fixedSkillPoints || injectorSpec.fullSkillPoints), 0);
}
...
function getInjectorPreviewSkillPoints(injectorSpec, quantity, totalSkillPoints, nonDiminishingInjectionsRemaining = 0) {
    if (!injectorSpec) {
        return 0;
    }
    if (injectorSpec.injectionMode === "diminishing") {
        return buildDiminishingInjectionPreview(injectorSpec, quantity, totalSkillPoints, nonDiminishingInjectionsRemaining).totalSkillPoints;
    }
    const usableQuantity = getFixedInjectorMaxUsableQuantity(injectorSpec, quantity, totalSkillPoints);
...
function resolveNextDowntimeFileTime(afterFileTime = null) {
    ...
    const nextDowntimeMillis = Date.UTC(currentDate.getUTCFullYear(), currentDate.getUTCMonth(), currentDate.getUTCDate(), DAILY_DOWNTIME_HOUR_UTC, 0, 0, 0);
...
//# sourceMappingURL=skillTradingAuthority.js.map