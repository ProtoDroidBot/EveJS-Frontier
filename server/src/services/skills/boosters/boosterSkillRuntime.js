...
const { getCachedCharacterSkillMap, } = require(path.join(__dirname, "../skillState"));
...
function getSkillLevel(skill) {
    return Math.max(0, toInt(skill && (skill.effectiveSkillLevel ??
...
function buildSkillEffectiveAttributes(skill) {
    ...
    for (const effectEntry of getTypeEffects(typeEntry)) {
        ...
        for (const modifier of effectEntry.modifierInfo || []) {
            ...
            applyDogmaModifier(attributes, modifier.modifiedAttributeID, modifier.operation, attributes[toInt(modifier.modifyingAttributeID, 0)]);
        }
    }
    return attributes;
}
...
function applyBoosterSkillModifiersToAttributes(attributes, characterOrID) {
    ...
    for (const modifier of getCharacterBoosterSkillModifierEntries(characterOrID)) {
        applyDogmaModifier(output, modifier.modifiedAttributeID, modifier.operation, modifier.value);
    }
    return output;
}
module.exports = {
...
//# sourceMappingURL=boosterSkillRuntime.js.map