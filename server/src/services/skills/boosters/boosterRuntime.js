...
const { buildDict, buildKeyVal, buildList, buildFiletimeLong, } = require(path.join(__dirname, "../../_shared/serviceHelpers"));
...
const { getAttributeIDByNames, getTypeDogmaRecord, } = require(path.join(__dirname, "../../fitting/liveFittingState"));
const { applyBoosterSkillModifiersToAttributes, } = require(path.join(__dirname, "./boosterSkillRuntime"));
const { applyActiveImplantLocationModifiersToAttributes, } = require(path.join(__dirname, "../../dogma/implants/activeImplantModifiers"));
const BOOSTER_GROUP_ID = 303;
const BOOSTER_CATEGORY_ID = 20;
const WINDOWS_EPOCH_OFFSET = 116444736000000000n;
const ATTRIBUTE_BOOSTER_DURATION = getAttributeIDByNames("boosterDuration") || 330;
const ATTRIBUTE_BOOSTERNESS = getAttributeIDByNames("boosterness") || 1087;
const ATTRIBUTE_LAST_INJECTION = getAttributeIDByNames("boosterLastInjectionDatetime") || 2422;
const ATTRIBUTE_MAX_CHAR_AGE_HOURS = getAttributeIDByNames("boosterMaxCharAgeHours") || 1647;
...
function isBoosterType(typeID) {
    ...
    return (Number(itemType.groupID) === BOOSTER_GROUP_ID ||
...
function getBoosterTypeInfo(typeID) {
    ...
    const durationMs = toInt(getTypeAttribute(attributes, ATTRIBUTE_BOOSTER_DURATION, 0), 0);
    const boosterness = toInt(getTypeAttribute(attributes, ATTRIBUTE_BOOSTERNESS, 0), 0);
    const maxCharAgeHours = toNumber(getTypeAttribute(attributes, ATTRIBUTE_MAX_CHAR_AGE_HOURS, 0), 0);
    ...
    return {
        ...
        boosterness,
        maxCharAgeHours,
        lastInjectionLimitMs: parseLimitDate(getTypeAttribute(attributes, ATTRIBUTE_LAST_INJECTION, null)),
        attributes,
...
function getBoosterSideEffectIDs(typeInfo, options = {}) {
    ...
    for (const effectID of typeInfo.effects) {
        ...
        const chance = normalizeChance(getTypeAttribute(options.effectiveAttributes || typeInfo.attributes, chanceAttributeID, 0));
        if (chance <= 0) {
            continue;
        }
...
function parseLimitDate(value) {
    ...
    const dotDate = trimmed.match(/^(\d{4})\.(\d{2})\.(\d{2})(?:\s+(\d{2}):(\d{2})(?::(\d{2}))?)?$/);
    if (dotDate) {
        return Date.UTC(Number(dotDate[1]), Number(dotDate[2]) - 1, Number(dotDate[3]), Number(dotDate[4] || 0), Number(dotDate[5] || 0), Number(dotDate[6] || 0));
    }
...
function getCharacterCreatedAtMs(characterRecord) {
    ...
    return (fileTimeToMs(characterRecord.createDateTime) ??
...
function getBoosterAgeLimitRemainingMs(typeInfo, characterRecord, nowMs) {
    ...
    return Math.max(0, Math.trunc(createdAtMs + maxCharAgeHours * 60 * 60 * 1000 - nowMs));
}
function resolveBoosterDurationMs(typeInfo, characterRecord, nowMs, effectiveAttributes = null) {
    if (!typeInfo) {
        return 0;
    }
    const baseDurationMs = Math.max(0, toInt(getTypeAttribute(effectiveAttributes || typeInfo.attributes, ATTRIBUTE_BOOSTER_DURATION, typeInfo.durationMs), typeInfo.durationMs));
...
function buildCharacterBoostersDict(characterID, options = {}) {
    ...
    return buildDict(boosters.map((record) => [
...
function useBoosterItem(characterID, itemID, locationID = null, options = {}) {
    ...
    const skillModifiedAttributes = applyBoosterSkillModifiersToAttributes(validation.typeInfo.attributes, numericCharacterID);
    const effectiveAttributes = applyActiveImplantLocationModifiersToAttributes(skillModifiedAttributes, {
        typeID: validation.typeInfo.typeID,
        groupID: BOOSTER_GROUP_ID,
    }, numericCharacterID, { includeBoosters: false });
    const boosterDurationMs = resolveBoosterDurationMs(validation.typeInfo, validation.characterRecord || getCharacterRecord(numericCharacterID), nowMs, effectiveAttributes);
    if (boosterDurationMs <= 0) {
        return { success: false, errorMsg: "NOT_A_CONSUMABLE_BOOSTER" };
    }
    const existingBoosters = getActiveBoostersForCharacter(numericCharacterID, { nowFileTime });
    const occupiedSlot = existingBoosters.find((record) => (validation.typeInfo.boosterness > 0 &&
    ...
    return {
        ...
        data: {
            booster: record,
            inventoryChanges: (consumeResult.data && Array.isArray(consumeResult.data.changes)
...
//# sourceMappingURL=boosterRuntime.js.map