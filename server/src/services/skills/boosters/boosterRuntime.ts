...
function getRandomFraction(options: Record<string, any> = {}) {
...
function getBoosterSideEffectIDs(typeInfo, options: Record<string, any> = {}) {
  if (!typeInfo || !Array.isArray(typeInfo.effects)) {
    return [];
  }

  const sideEffectIDs: any[] = [];
...
function normalizeBoosterRecord(record, options: Record<string, any> = {}) {
...
function normalizeActiveBoosters(boosters, options: Record<string, any> = {}) {
  const active: any[] = [];
...
function getActiveBoostersForCharacter(characterID, options: Record<string, any> = {}) {
...
function buildCharacterBoostersDict(characterID, options: Record<string, any> = {}) {
...
function validateBoosterItem(characterID, itemID, locationID = null, options: Record<string, any> = {}) {
  ...
  if (
    ...
    return { success: false as const, errorMsg: "ITEM_LOCATION_MISMATCH" };
  }

  const typeInfo = getBoosterTypeInfo(item.typeID);
  if (!typeInfo) {
    return { success: false as const, errorMsg: "NOT_A_CONSUMABLE_BOOSTER" };
  }

  const nowMs = Number.isFinite(Number(options.nowMs)) ? Number(options.nowMs) : Date.now();
  if (typeInfo.lastInjectionLimitMs !== null && nowMs > typeInfo.lastInjectionLimitMs) {
    return { success: false as const, errorMsg: "BOOSTER_EXPIRED" };
  }
  ...
  if (maxCharAgeHours > 0) {
    ...
    const createdAtMs = getCharacterCreatedAtMs(characterRecord);
    if (createdAtMs === null) {
      return { success: false as const, errorMsg: "BOOSTER_CHARACTER_AGE_UNKNOWN" };
    }
    if (nowMs - createdAtMs >= maxCharAgeHours * 60 * 60 * 1000) {
      return { success: false as const, errorMsg: "BOOSTER_CHARACTER_TOO_OLD" };
    }
  }
...
function useBoosterItem(characterID, itemID, locationID = null, options: Record<string, any> = {}) {
