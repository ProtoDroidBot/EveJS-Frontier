...
function buildError(code, message, extra: Record<string, any> = {}) {
  const error: Error & Record<string, any> = new Error(message || code);
  error.code = code;
...
function getNormalizedRequirementList(requirements: any[] = []) {
...
function canonicalizeSkillRequirements(requirements: any[] = []) {
  ...
  const orderedRequirements: any[] = [];
  const emitted = new Set<any>();
  const visitingOrder = new Set<any>();
...
function normalizeMilestoneRecord(planID, milestoneID, rawMilestone: Record<string, any> = {}) {
...
function normalizePlanRecord(planID, rawPlan: Record<string, any> = {}) {
  ...
  const milestonesSource =
    rawPlan && rawPlan.milestones && typeof rawPlan.milestones === "object"
      ? rawPlan.milestones
      : {};
  const normalizedMilestones: Record<string, any> = {};
  for (const [milestoneID, milestone] of Object.entries<any>(milestonesSource)) {
...
function normalizeCharacterPlanState(rawState: Record<string, any> = {}) {
  const plansSource =
    rawState && rawState.plans && typeof rawState.plans === "object"
      ? rawState.plans
      : {};
  const normalizedPlans: Record<string, any> = {};
  for (const [planID, plan] of Object.entries<any>(plansSource)) {
...
function findSharedPersonalPlan(planID) {
  ...
  const table = readPlanTable();
  for (const [characterID, rawState] of Object.entries<any>(table)) {
...
function createPersonalPlan(characterID, attributes: Record<string, any> = {}) {
...
function updatePersonalPlan(characterID, planID, updates: Record<string, any> = {}) {
  const normalizedPlanID = normalizeUuidString(planID);
  const state = getCharacterPlanState(characterID);
...
function listPersonalMilestones(characterID, planID) {
  ...
  return Object.values<any>(plan.milestones || {}).map((milestone) => cloneValue(milestone));
}

function createPersonalMilestone(characterID, planID, attributes: Record<string, any> = {}) {
  const normalizedPlanID = normalizeUuidString(planID);
  const state = getCharacterPlanState(characterID);
