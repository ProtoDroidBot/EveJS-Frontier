...
const { getSkillTypeByID, } = require(path.join(__dirname, "../skillState"));
...
function canonicalizeSkillRequirements(requirements = []) {
    ...
    for (const requirement of normalizedRequirements) {
        ...
        targetLevelByTypeID.set(requirement.typeID, Math.max(targetLevelByTypeID.get(requirement.typeID) || 0, requirement.level));
    }
    ...
    if (orderedRequirements.length > MAX_NUM_SKILLS) {
        throw buildError("SKILL_PLAN_TOO_MANY_REQUIREMENTS", `Skill plan exceeds ${MAX_NUM_SKILLS} skill entries`, { maxEntries: MAX_NUM_SKILLS });
    }
...
function createPersonalPlan(characterID, attributes = {}) {
    ...
    if (Object.keys(state.plans).length >= MAX_PERSONAL_PLANS) {
        throw buildError("SKILL_PLAN_CAPACITY_REACHED", `Character already has ${MAX_PERSONAL_PLANS} personal skill plans`, { maxPlans: MAX_PERSONAL_PLANS });
    }
...
function createPersonalMilestone(characterID, planID, attributes = {}) {
    ...
    if (Object.keys(plan.milestones || {}).length >= MAX_NUM_MILESTONES) {
        throw buildError("SKILL_PLAN_TOO_MANY_MILESTONES", `Skill plan exceeds ${MAX_NUM_MILESTONES} milestones`, { maxMilestones: MAX_NUM_MILESTONES });
    }
...
//# sourceMappingURL=skillPlanState.js.map