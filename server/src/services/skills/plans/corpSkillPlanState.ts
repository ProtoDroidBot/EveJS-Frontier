...
function normalizeCorpPlanRecord(planID, rawPlan: Record<string, any> = {}) {
...
function normalizeCorporationPlanState(rawState: Record<string, any> = {}) {
  const plansSource =
...
function setCorporationPlanState(corporationID, nextState) {
  const numericCorporationID = toInt(corporationID, 0);
  if (numericCorporationID <= 0) {
    return { success: false as const, errorMsg: "INVALID_CORPORATION" };
  }
  const table = readCorpTable();
...
function findCorpPlanByID(planID) {
  ...
  const table = readCorpTable();
  for (const [corporationID, rawState] of Object.entries<any>(table)) {
...
function createCorpPlan(corporationID, attributes: Record<string, any> = {}) {
...
function updateCorpPlan(corporationID, planID, updates: Record<string, any> = {}) {
  const normalizedPlanID = normalizeUuidString(planID);
  const state = getCorporationPlanState(corporationID);
...
function createCorpMilestone(corporationID, planID, attributes: Record<string, any> = {}) {
  const normalizedPlanID = normalizeUuidString(planID);
  const state = getCorporationPlanState(corporationID);
