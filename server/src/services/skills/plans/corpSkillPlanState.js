...
const { MAX_LEN_DESC, MAX_NUM_MILESTONES, ZERO_UUID, assertPlanDescription, assertPlanName, canonicalizeSkillRequirements, createUuidString, normalizeMilestoneRecord, normalizeUuidString, } = require(path.join(__dirname, "./skillPlanState"));
...
function normalizeCorpPlanRecord(planID, rawPlan = {}) {
    ...
    const milestonesSource = rawPlan && rawPlan.milestones && typeof rawPlan.milestones === "object"
        ? rawPlan.milestones
        : {};
    const normalizedMilestones = {};
    for (const [milestoneID, milestone] of Object.entries(milestonesSource)) {
        const normalizedMilestone = normalizeMilestoneRecord(normalizedPlanID, milestoneID, milestone);
...
function normalizeCorporationPlanState(rawState = {}) {
    const plansSource = rawState && rawState.plans && typeof rawState.plans === "object"
        ? rawState.plans
        : {};
...
function listCorpPlans(corporationID) {
    return Object.values(getCorporationPlanState(corporationID).plans).map((plan) => cloneValue(plan));
}
function getCorpPlan(corporationID, planID) {
    ...
    return cloneValue(getCorporationPlanState(corporationID).plans[normalizedPlanID] || null);
}
...
function createCorpPlan(corporationID, attributes = {}) {
    ...
    if (Object.keys(state.plans).length >= MAX_CORP_PLANS) {
        throw buildError("CORP_SKILL_PLAN_CAPACITY_REACHED", `Corporation already has ${MAX_CORP_PLANS} skill plans`, { maxPlans: MAX_CORP_PLANS });
    }
...
function createCorpMilestone(corporationID, planID, attributes = {}) {
    ...
    if (Object.keys(plan.milestones || {}).length >= MAX_NUM_MILESTONES) {
        throw buildError("CORP_SKILL_PLAN_TOO_MANY_MILESTONES", `Corporation skill plan exceeds ${MAX_NUM_MILESTONES} milestones`, { maxMilestones: MAX_NUM_MILESTONES });
    }
...
function updateCorpMilestoneDescription(corporationID, milestoneID, description) {
    ...
    throw buildError("CORP_SKILL_PLAN_MILESTONE_NOT_FOUND", "Corporation skill plan milestone not found");
}
...
//# sourceMappingURL=corpSkillPlanState.js.map