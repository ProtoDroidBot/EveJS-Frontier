...
function mapsEqualQueueEntries(leftEntries: any[] = [], rightEntries: any[] = []) {
...
function buildSkillMap(skillRecords: any[] = []) {
...
function resetCharacterSkillsToStarterProfile(characterID, options: Record<string, any> = {}) {
  const numericCharacterID = toInt(characterID, 0);
  if (numericCharacterID <= 0) {
    return {
