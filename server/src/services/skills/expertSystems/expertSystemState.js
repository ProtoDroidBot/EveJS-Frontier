...
function normalizeInstallEntry(characterID, rawEntry, fallbackTypeID = 0) {
    ...
    const installedAtMs = Math.max(0, toFiniteNumber(rawEntry && rawEntry.installedAtMs, Date.now()));
    const expiresAtMs = Math.max(installedAtMs, toFiniteNumber(rawEntry && rawEntry.expiresAtMs, installedAtMs));
    return {
...
function getCharacterExpertSystemState(characterID, options = {}) {
    ...
    const normalized = normalizeCharacterState(numericCharacterID, table[String(numericCharacterID)] || {});
...
function getCharacterExpertSystemEntries(characterID, options = {}) {
    ...
    if (pruneExpired && activeEntries.length !== entries.length) {
        const nextState = Object.fromEntries(activeEntries.map((entry) => [String(entry.typeID), entry]));
...
function upsertCharacterExpertSystem(characterID, installEntry) {
    const numericCharacterID = toInt(characterID, 0);
    const normalizedEntry = normalizeInstallEntry(numericCharacterID, installEntry, installEntry && installEntry.typeID);
...
//# sourceMappingURL=expertSystemState.js.map