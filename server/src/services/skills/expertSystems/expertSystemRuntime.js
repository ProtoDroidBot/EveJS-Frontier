...
const { findItemById, removeInventoryItem, updateInventoryItem, } = require(path.join(__dirname, "../../inventory/itemStore"));
const { getCharacterSkillMap } = require(path.join(__dirname, "../skillState"));
const { getExpertSystemByTypeID, getExpertSystemConstants, isExpertSystemType, listExpertSystems, resolveExpertSystemQuery, } = require("./expertSystemCatalog");
const { clearCharacterExpertSystems, getCharacterExpertSystemEntries, getCharacterExpertSystemState, removeCharacterExpertSystem, upsertCharacterExpertSystem, } = require("./expertSystemState");
const { emitExpertSystemsUpdated, } = require("./expertSystemNotifications");
const { scheduleExpertSystemExpiry, } = require("./expertSystemExpiryScheduler");
...
function consumeStackQuantity(item, quantity = 1) {
    ...
    const currentQuantity = toInt(currentItem.singleton, 0) === 1
        ? 1
...
function syncInventoryChangesToSession(session, changes = []) {
    ...
    for (const change of Array.isArray(changes) ? changes : []) {
        ...
        if (change.item) {
            ...
            syncInventoryItemForSession(session, { ...change.previousData, locationID: 6 }, change.previousData);
        }
    }
}
function validateExpertSystemInstall(characterID, expertSystemTypeID, options = {}) {
    ...
    if (config.expertSystemsEnabled === false && options.force !== true) {
        return fail("EXPERT_SYSTEMS_DISABLED", "Expert Systems are disabled in server config.", options);
    }
    if ((expertSystem.hidden || expertSystem.retired) && options.force !== true) {
        return fail("EXPERT_SYSTEM_NOT_ACTIVATABLE", "That Expert System is hidden or retired and cannot be activated normally.", options);
    }
    ...
    const existingEntry = activeEntries.find((entry) => (toInt(entry.typeID, 0) === expertSystem.typeID)) || null;
    const isTopUp = Boolean(existingEntry);
    const maxInstallations = Math.max(1, toInt(constants.maxCharacterInstallations, 3));
    const topUpWindowMs = Math.max(1, toInt(constants.maxInstalledDurationToAllowTopUpDays, 30)) * DAY_MS;
    if (!isTopUp && activeEntries.length >= maxInstallations && options.force !== true) {
        return fail("EXPERT_SYSTEM_INSTALLATION_LIMIT", `You already have ${activeEntries.length}/${maxInstallations} Expert Systems installed.`, options);
    }
    if (isTopUp &&
        existingEntry.expiresAtMs - nowMs > topUpWindowMs &&
        options.force !== true) {
        return fail("EXPERT_SYSTEM_TOP_UP_TOO_EARLY", "That Expert System cannot be extended until it has 30 days or less remaining.", options);
    }
    return {
        success: true,
...
function installExpertSystemForCharacter(characterID, expertSystemTypeID, options = {}) {
    ...
    const previousSkillMap = options.emitNotifications === false
        ? null
        : getCharacterSkillMap(numericCharacterID);
    const durationDays = Math.max(1, toFiniteNumber(options.durationDays, expertSystem.durationDays));
...
function consumeExpertSystemItem(characterID, itemID, session = null, options = {}) {
    ...
    if (!item || toInt(item.ownerID, 0) !== numericCharacterID) {
        return fail("EXPERT_SYSTEM_ITEM_NOT_FOUND", "Expert System item was not found in your inventory.", options);
    }
    ...
    const previousSkillMap = options.emitNotifications === false
        ? null
        : getCharacterSkillMap(numericCharacterID);
    const installResult = installExpertSystemForCharacter(numericCharacterID, item.typeID, {
        ...options,
        session,
    ...
    if (!consumeResult.success) {
        ...
        return fail(consumeResult.errorMsg || "EXPERT_SYSTEM_ITEM_CONSUME_FAILED", "Failed to consume the Expert System item.", options);
    }
...
//# sourceMappingURL=expertSystemRuntime.js.map