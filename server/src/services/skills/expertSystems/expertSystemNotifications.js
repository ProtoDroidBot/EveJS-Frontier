...
const { emitSkillSessionState, } = require(path.join(__dirname, "../training/skillQueueNotifications"));
const { diffProjectedSkillMaps, } = require("./expertSystemProjection");
const { buildExpertSystemsPayload, } = require("./expertSystemSerializer");
...
function getLiveSession(characterID, fallbackSession = null) {
    return (sessionRegistry.findSessionByCharacterID(toInt(characterID, 0)) ||
        fallbackSession ||
        null);
}
function emitExpertSystemsUpdated(characterID, options = {}) {
    ...
    const previousSkillMap = options.previousSkillMap instanceof Map ? options.previousSkillMap : null;
    if (previousSkillMap) {
        ...
        if (diff.changedSkillRecords.length > 0 ||
            diff.removedSkillRecords.length > 0) {
            emitSkillSessionState(session, numericCharacterID, diff.changedSkillRecords, {
                previousSkillMap,
...
//# sourceMappingURL=expertSystemNotifications.js.map