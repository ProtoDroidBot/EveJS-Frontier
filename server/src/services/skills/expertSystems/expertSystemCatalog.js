...
function normalizeExpertSystem(rawEntry) {
    ...
    const associatedTypeIDs = [...new Set((Array.isArray(rawEntry.associatedTypeIDs) ? rawEntry.associatedTypeIDs : [])
            .map((typeIDToNormalize) => toInt(typeIDToNormalize, 0))
            .filter((typeIDToKeep) => typeIDToKeep > 0))].sort((left, right) => left - right);
    return Object.freeze({
        ...
        skillsGranted: Object.freeze([...skillGrantMap.values()].sort((left, right) => left.typeID - right.typeID)),
        associatedTypeIDs: Object.freeze(associatedTypeIDs),
        sourceRefs: Object.freeze((Array.isArray(rawEntry.sourceRefs) ? rawEntry.sourceRefs : [])
...
function buildCatalogCache() {
    ...
    return {
        constants: Object.freeze({
            maxCharacterInstallations: toInt(constants.maxCharacterInstallations, DEFAULT_MAX_INSTALLATIONS) ||
                DEFAULT_MAX_INSTALLATIONS,
            maxInstalledDurationToAllowTopUpDays: toInt(constants.maxInstalledDurationToAllowTopUpDays, DEFAULT_TOP_UP_DAYS) ||
                DEFAULT_TOP_UP_DAYS,
        }),
...
function filterVisibility(entry, options = {}) {
    return ((options.includeHidden || !entry.hidden) &&
...
//# sourceMappingURL=expertSystemCatalog.js.map