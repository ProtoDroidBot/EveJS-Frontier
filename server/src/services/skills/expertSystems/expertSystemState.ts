...
function normalizeInstallEntry(characterID, rawEntry, fallbackTypeID: number | string = 0) {
  const typeID = toInt(rawEntry && rawEntry.typeID, Number(fallbackTypeID));
  if (typeID <= 0) {
    return null;
  }
...
function normalizeCharacterState(characterID, rawState: Record<string, any> = {}) {
...
function getCharacterExpertSystemState(characterID, options: Record<string, any> = {}) {
...
function setCharacterExpertSystemState(characterID, nextState) {
  ...
  return { success: true as const, data: cloneValue(normalized) };
}

function getCharacterExpertSystemEntries(characterID, options: Record<string, any> = {}) {
  ...
  const entries = Object.values<any>(state)
...
function upsertCharacterExpertSystem(characterID, installEntry) {
  ...
  if (numericCharacterID <= 0 || !normalizedEntry) {
    return { success: false as const, errorMsg: "INVALID_EXPERT_SYSTEM_INSTALL" };
  }
...
function removeCharacterExpertSystem(characterID, typeID) {
  ...
  if (!removedEntry) {
    return { success: true as const, removed: false, data: null };
  }
...
function clearCharacterExpertSystems(characterID) {
  ...
  return {
    ...writeResult,
    previousEntries: Object.values<any>(previousState).map((entry) => cloneValue(entry)),
  };
}
