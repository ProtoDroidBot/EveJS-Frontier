...
const { getCharacterExpertSystemEntries, getExpertSystemMutationVersion, } = require("./expertSystemState");
...
function projectExistingSkillRecord(skillRecord, virtualLevel) {
    const currentTrainedLevel = Math.max(0, toInt(skillRecord.trainedSkillLevel ??
        skillRecord.skillLevel ??
        skillRecord.effectiveSkillLevel, 0));
    const currentEffectiveLevel = Math.max(currentTrainedLevel, toInt(skillRecord.effectiveSkillLevel, currentTrainedLevel));
...
function projectSkillRecordsForCharacter(characterID, skillRecords = [], options = {}) {
    ...
    const getSkillTypeByID = typeof options.getSkillTypeByID === "function" ? options.getSkillTypeByID : null;
...
//# sourceMappingURL=expertSystemProjection.js.map