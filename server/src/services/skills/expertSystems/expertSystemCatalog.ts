...
function normalizeExpertSystem(rawEntry) {
  ...
  const associatedTypeIDs = [...new Set<any>(
...
function getCatalogCache(options: Record<string, any> = {}) {
...
function filterVisibility(entry, options: Record<string, any> = {}) {
  return (
...
function listExpertSystems(options: Record<string, any> = {}) {
...
function getAssociatedExpertSystems(typeID, options: Record<string, any> = {}) {
  return (getCatalogCache().byAssociatedTypeID.get(toInt(typeID, 0)) || [])
    .filter((entry) => filterVisibility(entry, options))
    .map((entry) => cloneValue(entry));
}

function getExpertSystemsGrantingSkill(skillTypeID, options: Record<string, any> = {}) {
...
function resolveExpertSystemQuery(query, options: Record<string, any> = {}) {
  ...
  if (exact) {
    return { success: true as const, data: cloneValue(exact) };
  }
  ...
  if (
    ...
    return { success: true as const, data: cloneValue(candidates[0].entry) };
  }
