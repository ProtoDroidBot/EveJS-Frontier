...
const { TABLE, clearReferenceCache, readStaticTable, readStaticRows, } = require(path.join(__dirname, "../_shared/referenceData"));
const { getCharacterCreationRaces, } = require(path.join(__dirname, "../character/characterCreationData"));
...
function applyExpertSystemProjection(charId, skillRecords, options = {}) {
    ...
    try {
        const { projectSkillRecordsForCharacter, } = require(path.join(__dirname, "./expertSystems/expertSystemProjection"));
        ...
        log.warn(`[SkillState] Failed to project Expert System skill overlay for character ${toNumber(charId, 0)}: ${error.message}`);
        return skillRecords.map((record) => cloneValue(record));
    }
}
...
function calculateSkillPointsForLevel(skillRank, skillLevel = MAX_SKILL_LEVEL) {
    ...
    return Math.round(250 * normalizedRank * Math.pow(Math.sqrt(32), normalizedLevel - 1));
}
...
function loadSkillReference() {
    ...
    try {
        ...
        const dogmaByTypeID = (typeDogma && typeof typeDogma === "object" && typeDogma.typesByTypeID) || {};
        skillReferenceCache = skillTypes.map((skillType) => {
            ...
            const dogmaAttributes = dogmaRecord && typeof dogmaRecord === "object" && dogmaRecord.attributes
                ? dogmaRecord.attributes
                : {};
            const dogmaSkillRank = toFiniteNumber(dogmaAttributes[String(SKILL_TIME_CONSTANT_ATTRIBUTE_ID)], NaN);
            const skillRank = Number.isFinite(dogmaSkillRank) && dogmaSkillRank > 0
                ? dogmaSkillRank
...
function buildSkillRecord(charId, skillType, skillLevel = DEFAULT_MISSING_SKILL_LEVEL) {
...
function normalizeSkillRecord(charId, existingRecord, skillType, options = {}) {
    const defaultSkillLevel = clampSkillLevel(options.defaultSkillLevel, DEFAULT_MISSING_SKILL_LEVEL);
    ...
    const skillLevel = clampSkillLevel(hasExistingRecord ? existingRecord.skillLevel : undefined, defaultSkillLevel);
    const trainedSkillLevel = inTraining
        ? clampSkillLevel(hasExistingRecord ? existingRecord.trainedSkillLevel : undefined, skillLevel)
        : skillLevel;
    const effectiveSkillLevel = inTraining
        ? clampSkillLevel(hasExistingRecord ? existingRecord.effectiveSkillLevel : undefined, skillLevel)
        : skillLevel;
    ...
    const nextSkillLevelPoints = skillLevel >= MAX_SKILL_LEVEL
        ? maxSkillPoints
        : getSkillMaxPoints(skillType, skillLevel + 1) - 1;
    const nextTrainedSkillLevelPoints = trainedSkillLevel >= MAX_SKILL_LEVEL
        ? maxSkillPoints
        : getSkillMaxPoints(skillType, trainedSkillLevel + 1) - 1;
    const resolvedSkillPoints = inTraining
        ? normalizeSkillPoints(hasExistingRecord ? existingRecord.skillPoints : undefined, trainedSkillPoints, maxSkillPoints, skillPointsAtLevel)
        : normalizeSkillPoints(hasExistingRecord
            ? existingRecord.skillPoints ?? existingRecord.trainedSkillPoints
            : undefined, skillPointsAtLevel, nextSkillLevelPoints, skillPointsAtLevel);
    const resolvedTrainedSkillPoints = inTraining
        ? normalizeSkillPoints(hasExistingRecord ? existingRecord.trainedSkillPoints : undefined, trainedSkillPoints, resolvedSkillPoints, trainedSkillPoints)
        : normalizeSkillPoints(hasExistingRecord
            ? existingRecord.trainedSkillPoints ?? existingRecord.skillPoints
            : undefined, trainedSkillPoints, Math.min(nextTrainedSkillLevelPoints, resolvedSkillPoints), skillPointsAtLevel);
    const resolvedTrainingStartSP = inTraining
        ? normalizeSkillPoints(hasExistingRecord ? existingRecord.trainingStartSP : undefined, resolvedTrainedSkillPoints, maxSkillPoints, resolvedTrainedSkillPoints)
        : resolvedTrainedSkillPoints;
    const resolvedTrainingDestinationSP = inTraining
        ? normalizeSkillPoints(hasExistingRecord ? existingRecord.trainingDestinationSP : undefined, Math.max(resolvedSkillPoints, resolvedTrainingStartSP), maxSkillPoints, skillPointsAtLevel)
        : resolvedSkillPoints;
    return {
        ...baseRecord,
        ...
        trainedSkillLevel,
        effectiveSkillLevel,
        virtualSkillLevel: hasExistingRecord && Object.prototype.hasOwnProperty.call(existingRecord, "virtualSkillLevel")
...
function setCharacterSkillBootstrapSuppressed(charId, suppressed = true) {
    ...
    if (!writeResult || !writeResult.success) {
        log.warn(`[SkillState] Failed to persist bootstrap suppression for character ${numericCharId}`);
        return {
            success: false,
...
function normalizeGrantedSkillLevelEntries(skillLevels = []) {
    ...
    const entries = skillLevels instanceof Map
    ...
    for (const entry of entries) {
        ...
        grantedSkillLevelByTypeID.set(typeID, clampSkillLevel(entry.level, DEFAULT_MISSING_SKILL_LEVEL));
    }
...
function bootstrapCharacterSkillsIfMissing(charId) {
    ...
    const bootstrapSuppressed = Boolean(characterRecord[SKILL_BOOTSTRAP_SUPPRESSED_FIELD]);
    const existingSkills = readSkillsTable()[String(numericCharId)];
    if (existingSkills && typeof existingSkills === "object") {
        const normalizedExistingSkills = Object.values(existingSkills).map((record) => cloneValue(record));
...
function ensureCharacterSkills(charId, options = {}) {
    ...
    const grantedSkillTypeIDs = options.grantedSkillTypeIDs instanceof Set ? options.grantedSkillTypeIDs : null;
    const grantedSkillLevel = grantedSkillTypeIDs && options.grantedSkillLevel !== undefined
        ? clampSkillLevel(options.grantedSkillLevel, MAX_SKILL_LEVEL)
        : null;
    const grantedSkillLevelByTypeID = normalizeGrantedSkillLevelEntries(options.grantedSkillLevelByTypeID);
    ...
    for (const skillType of skillTypes) {
        ...
        const grantedSkillLevelForType = grantedSkillLevelByTypeID.get(toNumber(skillType.typeID, 0));
        const hasCustomGrantedSkillLevel = grantedSkillLevelByTypeID.has(toNumber(skillType.typeID, 0));
        const isGrantedSkill = grantedSkillTypeIDs && grantedSkillTypeIDs.has(toNumber(skillType.typeID, 0));
        let normalizedRecord = null;
        if (hasCustomGrantedSkillLevel) {
            normalizedRecord = buildSkillRecord(numericCharId, skillType, grantedSkillLevelForType);
        }
    ...
    const totalSkillPoints = skills.reduce((sum, skill) => sum + toNumber(skill.skillPoints, 0), 0);
    syncCharacterSkillPoints(numericCharId, totalSkillPoints);
    return skills;
}
function grantCharacterSkillTypes(charId, skillTypeIDs = [], skillLevel = MAX_SKILL_LEVEL) {
    const grantedSkillTypeIDs = new Set((Array.isArray(skillTypeIDs) ? skillTypeIDs : [...skillTypeIDs])
...
function removeCharacterSkillTypes(charId, skillTypeIDs = []) {
    ...
    const targetTypeIDs = new Set((Array.isArray(skillTypeIDs) ? skillTypeIDs : [...skillTypeIDs])
        .map((typeID) => toNumber(typeID, 0))
        .filter((typeID) => typeID > 0));
    ...
    const skillsTable = readSkillsTable();
    const existingSkills = skillsTable[characterKey] && typeof skillsTable[characterKey] === "object"
    ...
    if (dirty) {
        ...
        const writeResult = writeSkillsTable(skillsTable);
        if (!writeResult || !writeResult.success) {
            log.warn(`[SkillState] Failed to persist removed skills for character ${numericCharId}`);
            return [];
        }
    }
    const remainingSkills = Object.values(existingSkills).map((record) => cloneValue(record));
    const remainingSkillPoints = remainingSkills.reduce((sum, skill) => sum + toNumber(skill && skill.skillPoints, 0), 0);
...
function clearCharacterSkills(charId) {
    ...
    if (currentSkillMap.size === 0) {
        ...
        if (!skillsTable[characterKey]) {
            ...
            const writeResult = writeSkillsTable(skillsTable);
            if (!writeResult || !writeResult.success) {
                log.warn(`[SkillState] Failed to persist empty skill state for character ${numericCharId}`);
            }
        }
...
function seedCharacterAllSkills(charId, skillLevel = MAX_SKILL_LEVEL) {
...
function cachedSkillMapVersionKey() {
    ...
    const { getExpertSystemMutationVersion, } = require(path.join(__dirname, "./expertSystems/expertSystemState"));
...
function needsSkillLevelGrant(record, targetSkillLevel = MAX_SKILL_LEVEL) {
    ...
    const currentSkillLevel = clampSkillLevel(record.skillLevel, DEFAULT_MISSING_SKILL_LEVEL);
    const currentTrainedSkillLevel = clampSkillLevel(record.trainedSkillLevel, currentSkillLevel);
    const currentEffectiveSkillLevel = clampSkillLevel(record.effectiveSkillLevel, currentSkillLevel);
...
function ensureCharacterSkillTypes(charId, skillTypes = [], options = {}) {
    const targetSkillLevel = clampSkillLevel(options.skillLevel, MAX_SKILL_LEVEL);
    ...
    const targetSkillTypeIDs = skillTypes
        ...
        .filter((skillType) => needsSkillLevelGrant(existingSkillMap.get(toNumber(skillType.typeID, 0)), targetSkillLevel))
...
function seedCharacterPublishedSkills(charId, skillLevel = MAX_SKILL_LEVEL) {
    return grantCharacterSkillTypes(charId, getPublishedSkillTypes().map((skillType) => toNumber(skillType.typeID, 0)), skillLevel);
}
function seedCharacterStarterSkills(charId, raceId) {
    ...
    const raceProfile = raceProfiles.find((profile) => profile && profile.raceID === numericRaceId);
    if (!raceProfile || !Array.isArray(raceProfile.skills) || raceProfile.skills.length === 0) {
        log.warn(`[SkillState] No starter skill profile found for race=${numericRaceId} char=${toNumber(charId, 0)}`);
        return [];
    }
...
function replaceCharacterSkillRecords(charId, skillRecords = []) {
    ...
    for (const rawRecord of Array.isArray(skillRecords) ? skillRecords : []) {
        ...
        nextSkills[String(typeID)] = normalizeSkillRecord(numericCharId, rawRecord, skillType, {
    ...
    const writeResult = writeSkillsTable(skillsTable);
    if (!writeResult || !writeResult.success) {
        log.warn(`[SkillState] Failed to replace skill records for character ${numericCharId}`);
        return {
            success: false,
            errorMsg: writeResult && writeResult.errorMsg ? writeResult.errorMsg : "WRITE_ERROR",
        };
    }
    const totalSkillPoints = Object.values(nextSkills).reduce((sum, skillRecord) => sum + toNumber(skillRecord && skillRecord.skillPoints, 0), 0);
    syncCharacterSkillPoints(numericCharId, totalSkillPoints);
    return {
        success: true,
...
//# sourceMappingURL=skillState.js.map