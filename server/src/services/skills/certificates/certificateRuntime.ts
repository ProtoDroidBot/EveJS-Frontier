...
function normalizeCertificateState(rawState) {
  ...
  for (const [certificateID, entry] of Object.entries<any>(source)) {
...
function parseCertificateJsonlLine(line) {
  ...
  const requirementsBySkillTypeID: Record<string, any> = {};
...
function getCertificateLevelForSkillMap(skillMap, certificate) {
  ...
  for (let certificateLevel = 5; certificateLevel >= 1; certificateLevel -= 1) {
    ...
    for (const [skillTypeID, levelsByGrade] of Object.entries<any>(
...
function buildMaterializedCertificateStateForSkillMap(skillMap, currentState: Record<string, any> = {}) {
  const authority = loadCertificateAuthority();
...
function grantCertificates(characterID, certificateIDs: any[] = []) {
