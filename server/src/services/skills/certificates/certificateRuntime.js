...
const { buildFiletimeLong, buildKeyVal, buildList, buildRowset, currentFileTime, } = require(path.join(__dirname, "../../_shared/serviceHelpers"));
...
const { getCharacterSkills, } = require(path.join(__dirname, "../skillState"));
const CERTIFICATE_JSONL_PATHS = [
    process.env.EVEJS_CERTIFICATE_JSONL_PATH,
    path.join(__dirname, "../../../../../tools/DataSync/source_json/eve-online-static-data-3396210-jsonl/certificates.jsonl"),
    path.join(__dirname, "../../../../../tools/DataSync/source_json/eve-online-static-data-3393779-jsonl/certificates.jsonl"),
    path.join(__dirname, "../../../../../data/eve-online-static-data-3284752-jsonl/certificates.jsonl"),
].filter(Boolean);
...
function parseCertificateJsonlLine(line) {
    ...
    return {
        ...
        description: String(parsed && parsed.description && parsed.description.en ? parsed.description.en : ""),
        recommendedFor: [...new Set((Array.isArray(parsed.recommendedFor) ? parsed.recommendedFor : [])
...
function resolveCertificateJsonlPath() {
    ...
    throw new Error(`Certificate SDE JSONL not found. Checked: ${CERTIFICATE_JSONL_PATHS.join(", ")}`);
}
function loadCertificateAuthority() {
    ...
    for (const line of rawJsonl.split(/\r?\n/)) {
        ...
        certificateIDsByGroupID.get(normalizedCertificate.groupID).push(normalizedCertificate.certificateID);
...
function getCertificateLevelForSkillMap(skillMap, certificate) {
    ...
    for (let certificateLevel = 5; certificateLevel >= 1; certificateLevel -= 1) {
        ...
        for (const [skillTypeID, levelsByGrade] of Object.entries(certificate.requirementsBySkillTypeID || {})) {
            ...
            const effectiveSkillLevel = Math.max(0, toInt(skillRecord &&
...
function buildMyCertificatesRowsetForCharacter(characterID) {
    return buildRowset(["certificateID", "grantDate", "visibilityFlags"], materializeCurrentCertificates(characterID).map((row) => [
        row.certificateID,
...
function buildCharacterCertificatesRowsetForCharacter(characterID) {
    return buildRowset(["grantDate", "certificateID", "visibilityFlags"], materializeCurrentCertificates(characterID).map((row) => [
        buildFiletimeLong(row.grantDate),
        row.certificateID,
        row.visibilityFlags,
    ]));
...
function buildCertificateCategoriesPayload() {
    return buildList(listCertificateGroupSummaries().map((group) => buildKeyVal([
...
function buildCertificateClassesPayload() {
    return buildList(listCertificateDefinitions().map((certificate) => buildKeyVal([
        ["certificateID", certificate.certificateID],
        ["groupID", certificate.groupID],
        ["groupName", certificate.groupName],
        ["name", certificate.name],
...
function grantCertificates(characterID, certificateIDs = []) {
    const normalizedCertificateIDs = [...new Set((Array.isArray(certificateIDs) ? certificateIDs : [certificateIDs])
...
//# sourceMappingURL=certificateRuntime.js.map