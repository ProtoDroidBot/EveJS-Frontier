...
function recordRecentSkillPointChangesFromDiff(characterID, changedSkillRecords = [], previousSkillMap = null) {
    if (!previousSkillMap) {
        return [];
    }
    ...
    for (const changedSkillRecord of Array.isArray(changedSkillRecords)
        ...
        const currentPoints = Math.max(0, toInt(changedSkillRecord &&
            (changedSkillRecord.trainedSkillPoints ?? changedSkillRecord.skillPoints), 0));
        const previousPoints = Math.max(0, toInt(previousSkillRecord &&
...
//# sourceMappingURL=skillChangeTracker.js.map