...
function applyExpertSystemProjection(charId, skillRecords, options: Record<string, any> = {}) {
...
function getSkillTypes(options: Record<string, any> = {}) {
...
function getSkillTypeByID(typeID, options: Record<string, any> = {}) {
...
function getPublishedSkillTypes(options: Record<string, any> = {}) {
  return getSkillTypes(options)
    .filter((skillType) => skillType && skillType.published !== false)
    .map((skillType) => cloneValue(skillType));
}

function getUnpublishedSkillTypes(options: Record<string, any> = {}) {
  return getSkillTypes(options)
    .filter((skillType) => skillType && skillType.published === false)
    .map((skillType) => cloneValue(skillType));
}
...
function normalizeGrantedSkillLevelEntries(skillLevels: any[] = []) {
  ...
  const entries =
    skillLevels instanceof Map
      ...
      : Array.isArray(skillLevels)
        ? skillLevels
        : Object.entries<any>(skillLevels || {}).map(([typeID, level]) => ({
            typeID,
            level,
          }));
...
function bootstrapCharacterSkillsIfMissing(charId) {
  ...
  if (existingSkills && typeof existingSkills === "object") {
    const normalizedExistingSkills = Object.values<any>(existingSkills).map((record) =>
      cloneValue(record),
    );
...
function ensureCharacterSkills(charId, options: Record<string, any> = {}) {
  const numericCharId = toNumber(charId, 0);
  if (numericCharId <= 0) {
    return [];
  }
...
function grantCharacterSkillLevels(charId, skillLevels: any[] = []) {
...
function removeCharacterSkillTypes(charId, skillTypeIDs: Iterable<number> = []) {
  const numericCharId = toNumber(charId, 0);
  if (numericCharId <= 0) {
    return [];
  }
  ...
  const remainingSkills = Object.values<any>(existingSkills).map((record) => cloneValue(record));
...
function getCharacterBaseSkills(charId, options: Record<string, any> = {}) {
  return ensureCharacterSkills(charId, options)
    .sort((left, right) => left.typeID - right.typeID)
    .map((record) => cloneValue(record));
}

function getCharacterSkills(charId, options: Record<string, any> = {}) {
...
function getCharacterBaseSkillMap(charId, options: Record<string, any> = {}) {
  const entries = getCharacterBaseSkills(charId, options).map((record): [number, any] => [
    record.typeID,
    cloneValue(record),
  ]);
...
function getCharacterSkillMap(charId, options: Record<string, any> = {}) {
...
function ensureCharacterSkillTypes(charId, skillTypes: any[] = [], options: Record<string, any> = {}) {
...
function ensureCharacterPublishedSkills(charId, options: Record<string, any> = {}) {
...
function ensureCharacterUnpublishedSkills(charId, options: Record<string, any> = {}) {
...
function replaceCharacterSkillRecords(charId, skillRecords: any[] = []) {
  const numericCharId = toNumber(charId, 0);
  if (numericCharId <= 0) {
    return {
      success: false as const,
