...
const { buildObjectEx1, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const CHARACTER_SKILL_ENTRY_CLASS = "characterskills.common.character_skill_entry.CharacterSkillEntry";
...
function buildCharacterSkillEntry(skillRecord, options = {}) {
    ...
    const trainedSkillPoints = toOptionalInt(skillRecord.trainedSkillPoints ?? skillRecord.skillPoints);
    ...
    if (options.includeMetadata !== false) {
        stateEntries.push(["itemID", toOptionalInt(skillRecord.itemID)], ["ownerID", toOptionalInt(skillRecord.ownerID)], ["locationID", toOptionalInt(skillRecord.locationID)], ["flagID", toOptionalInt(skillRecord.flagID)], ["groupID", toOptionalInt(skillRecord.groupID)], ["categoryID", toOptionalInt(skillRecord.categoryID)], ["groupName", skillRecord.groupName || ""], ["published", Boolean(skillRecord.published)], ["inTraining", Boolean(skillRecord.inTraining)]);
    }
    ...
    return buildObjectEx1(CHARACTER_SKILL_ENTRY_CLASS, constructorArgs, stateEntries);
}
...
//# sourceMappingURL=skillTransport.js.map