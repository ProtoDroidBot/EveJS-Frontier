...
const { getCharacterCreationRace, resolveCharacterCreationBloodlineProfile, } = require(path.join(__dirname, "../character/characterCreationData"));
const { getCharacterBaseSkillMap, replaceCharacterSkillRecords, setCharacterSkillBootstrapSuppressed, } = require(path.join(__dirname, "./skillState"));
const { getQueueSnapshot, saveQueue, } = require(path.join(__dirname, "./training/skillQueueRuntime"));
...
function mapsEqualQueueEntries(leftEntries = [], rightEntries = []) {
    ...
    for (let index = 0; index < leftEntries.length; index += 1) {
        ...
        if (toInt(left && left.trainingTypeID, 0) !== toInt(right && right.typeID, 0) ||
...
function resolveStarterSkillProfile(characterID) {
    ...
    const bloodlineProfile = resolveCharacterCreationBloodlineProfile(characterRecord.bloodlineID, {
        raceID: characterRecord.raceID,
        corporationID: characterRecord.corporationID,
    });
    const raceProfile = getCharacterCreationRace(bloodlineProfile.raceID || characterRecord.raceID);
...
function reconcileQueueForSkillMutation(characterID, policy = "prune_satisfied") {
    ...
    if (policy === "clear") {
        ...
        const snapshot = saveQueue(numericCharacterID, [], {
            activate: false,
    ...
    const queueUnchanged = currentSnapshot.active === (filteredEntries.length > 0 && currentSnapshot.active) &&
    ...
    const snapshot = saveQueue(numericCharacterID, filteredEntries, {
...
function resetCharacterSkillsToStarterProfile(characterID, options = {}) {
    ...
    const replaceResult = replaceCharacterSkillRecords(numericCharacterID, starterSkillRecords);
    ...
    updateCharacterRecord(numericCharacterID, (record) => ({
        ...record,
        freeSkillPoints: options.resetFreeSkillPoints === false ? record.freeSkillPoints : 0,
        finishedSkills: [],
    }));
...
//# sourceMappingURL=skillAdminRuntime.js.map