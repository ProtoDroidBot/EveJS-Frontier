...
function buildDirectPurchaseCache() {
  ...
  const purchasableTypeIDs = new Set<any>();
...
function throwPurchaseError(errorName, values: Record<string, any> = {}) {
...
function injectSkillTypes(characterID, rawSkillTypeIDs, session = null, options: Record<string, any> = {}) {
  const skillTypeIDs = normalizeIDList(rawSkillTypeIDs);
  ...
  const changedTypeIDs = new Set<any>(
