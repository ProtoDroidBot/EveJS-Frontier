...
const { JOURNAL_ENTRY_TYPE, JOURNAL_CURRENCY, adjustCharacterBalance, buildNotEnoughMoneyUserErrorValues, getCharacterWallet, } = require(path.join(__dirname, "../../account/walletState"));
const { findItemById, removeInventoryItem, updateInventoryItem, } = require(path.join(__dirname, "../../inventory/itemStore"));
const { buildSkillRecord, getCharacterBaseSkillMap, getSkillTypeByID, grantCharacterSkillLevels, } = require(path.join(__dirname, "../skillState"));
const { notifySkillStateChanged, } = require(path.join(__dirname, "../training/skillQueueNotifications"));
...
function buildDirectPurchaseCache() {
    ...
    const { getSkillTypes, } = require(path.join(__dirname, "../skillState"));
    for (const rawSkillType of getSkillTypes()) {
        ...
        priceByTypeID.set(typeID, toMoney(toMoney(skillType.basePrice, 0) * (1 + DIRECT_PURCHASE_TAX_RATE), 0));
    }
    return {
        byTypeID,
        purchasableTypeIDs,
...
function validateSkillPurchase(characterID, rawSkillTypeIDs) {
    ...
    const totalCost = toMoney(skillTypeIDs.reduce((sum, typeID) => sum + getDirectPurchasePrice(typeID), 0), 0);
    const wallet = getCharacterWallet(characterID);
    const balance = toMoney(wallet && wallet.balance, 0);
    if (totalCost > balance) {
        throwPurchaseError("NotEnoughMoney", buildNotEnoughMoneyUserErrorValues(totalCost, balance));
    }
    return {
        skillTypeIDs,
        totalCost,
...
function injectSkillTypes(characterID, rawSkillTypeIDs, session = null, options = {}) {
    ...
    const previousSkillMap = options.previousSkillMap instanceof Map ? options.previousSkillMap : getBaseSkillMap(characterID);
    for (const typeID of skillTypeIDs) {
    ...
    const changedSkills = grantCharacterSkillLevels(characterID, skillTypeIDs.map((typeID) => ({
        typeID,
        level: 0,
    })));
    notifyInjectedSkills(characterID, changedSkills, previousSkillMap);
    const changedTypeIDs = new Set(changedSkills.map((skillRecord) => toInt(skillRecord.typeID, 0)).filter(Boolean));
...
function purchaseSkills(characterID, rawSkillTypeIDs, session = null) {
    ...
    if (!debitResult.success) {
        if (debitResult.errorMsg === "INSUFFICIENT_FUNDS") {
            const wallet = getCharacterWallet(characterID);
            throwPurchaseError("NotEnoughMoney", buildNotEnoughMoneyUserErrorValues(validation.totalCost, toMoney(wallet && wallet.balance, 0)));
        }
...
function syncInventoryChangesToSession(session, changes = []) {
    ...
    for (const change of Array.isArray(changes) ? changes : []) {
        ...
        if (change.item) {
            ...
            syncInventoryItemForSession(session, {
                ...change.previousData,
                locationID: 6,
            }, change.previousData);
        }
...
function validateSkillbookItem(characterID, itemID, previousSkillMap) {
    ...
    const stackQuantity = Math.max(0, toInt(item.stacksize ?? item.quantity, item.singleton === 1 ? 1 : 0));
    if (stackQuantity <= 0) {
        throwPurchaseError("ItemNotASkill", {
            skillName: typeID,
        });
...
function injectSkillbookItems(characterID, rawItemIDs, session = null) {
    ...
    const entries = itemIDs.map((itemID) => validateSkillbookItem(characterID, itemID, previousSkillMap));
    ...
    const injectedTypeIDs = injectSkillTypes(characterID, entries.map((entry) => entry.typeID), session, {
        previousSkillMap,
    });
...
//# sourceMappingURL=skillbookRuntime.js.map