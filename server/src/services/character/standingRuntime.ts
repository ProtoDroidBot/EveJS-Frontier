...
function clearStandingRuntimeCaches(options: Record<string, any> = {}) {
...
function readAllCharacterRecords() {
  ...
  const records =
    result.success && result.data && typeof result.data === "object"
      ? result.data
      : {};
  return Object.entries<any>(records).map(([characterID, record]) => ({
...
function listCorporationStandings(corporationID) {
  ...
  const rowsByOwnerID = new Map<any, any>(
    [],
  );

  const memberStandingOwnerIDs = new Set<any>();
  for (const characterID of getCharacterIDsInCorporation(normalizedCorporationID)) {
...
function getCharacterDivisionConnectionsLpMultiplier(characterID, missionKind) {
  ...
  const skillTypeID =
    DIVISION_CONNECTIONS_SKILL_BY_MISSION_KIND[
      String(missionKind || "").trim().toLowerCase()
    ] || 0;
  if (!skillTypeID) {
    return 1;
  }
  ...
  const skillLevel = getSkillLevel(
    getCachedCharacterSkillMap(normalizedCharacterID),
    skillTypeID,
  );
...
function buildStandingPreview(characterID, entries: any[] = [], options: Record<string, any> = {}) {
  const normalizedCharacterID = normalizePositiveInteger(characterID, 0);
  if (!normalizedCharacterID) {
    return {};
  }
...
function resolveBestStandingValue(characterID, targetOwnerIDs: any[] = []) {
  const normalizedCharacterID = normalizePositiveInteger(characterID, 0);
  if (!normalizedCharacterID) {
    return {
      standing: 0,
...
function applyStandingChanges(characterID, changes: any[] = [], options: Record<string, any> = {}) {
  const normalizedCharacterID = normalizePositiveInteger(characterID, 0);
...
function setCharacterStandings(characterID, entries: any[] = [], options: Record<string, any> = {}) {
  const normalizedCharacterID = normalizePositiveInteger(characterID, 0);
...
function setCharacterStanding(characterID, ownerID, standing, options: Record<string, any> = {}) {
...
function buildProtocolZeroPickleHexFromDict(payload: Record<string, any> = {}) {
...
function buildMissionStandingMessage(outcome, missionName, eventDefinition: Record<string, any> = {}) {
