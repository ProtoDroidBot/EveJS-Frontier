...
const { normalizeNumber, normalizeText, buildKeyVal, extractDictEntries, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
const { buildBookmarkText, buildGroupPayload, } = require(path.join(__dirname, "../bookmark/bookmarkPayloads"));
/**
...
class OwnerGroupManagerService extends BaseService {
    ...
    Handle_GetMembersForMultipleGroups(args, session) {
        ...
        log.debug(`[OwnerGroupManager] GetMembersForMultipleGroups char=${charID} groups=${JSON.stringify(groupIDs)}`);
    ...
    Handle_GetGroupsMany(args, session) {
        const charID = session && session.characterID;
        const groupIDs = args && Array.isArray(args[0]) ? args[0] : [];
        log.debug(`[OwnerGroupManager] GetGroupsMany char=${charID} groups=${JSON.stringify(groupIDs)}`);
    ...
    Handle_AddMembers(args, session) {
        ...
        log.info(`[OwnerGroupManager] AddMembers char=${charID} group=${groupID} members=${JSON.stringify(membershipDict)}`);
    ...
    Handle_DeleteMembers(args, session) {
        ...
        log.info(`[OwnerGroupManager] DeleteMembers char=${charID} group=${groupID} members=${JSON.stringify(memberIDs)}`);
    ...
    Handle_UpdateMembershipTypes(args, session) {
        ...
        log.info(`[OwnerGroupManager] UpdateMembershipTypes char=${charID} group=${groupID} members=${JSON.stringify(memberIDs)} type=${membershipType}`);
...
//# sourceMappingURL=ownerGroupManagerService.js.map