...
const { clearCharacterPortraits, } = require(path.join(__dirname, "./portraitImageStore"));
const { getCorporationRecord, setCharacterAffiliation, } = require(path.join(__dirname, "../corporation/corporationState"));
const { findItemById, listOwnedItems, removeInventoryItem, updateInventoryItem, } = require(path.join(__dirname, "../inventory/itemStore"));
const { clearCharacterSkills, } = require(path.join(__dirname, "../skills/skillState"));
const { clearCharacterExpertSystems, } = require(path.join(__dirname, "../skills/expertSystems/expertSystemState"));
const { clearCharacterQueueState, } = require(path.join(__dirname, "../skills/training/skillQueueState"));
const { clearCharacterPlanState, } = require(path.join(__dirname, "../skills/plans/skillPlanState"));
const { clearCharacterSkillTradingState, } = require(path.join(__dirname, "../skills/trading/skillTradingState"));
const missionRuntimeState = require(path.join(__dirname, "../agent/missionRuntimeState"));
const probeRuntimeState = require(path.join(__dirname, "../exploration/probes/probeRuntimeState"));
const bookmarkStore = require(path.join(__dirname, "../bookmark/bookmarkRuntimeStore"));
const { deleteManyFittings, getOwnerFittings, } = require(path.join(__dirname, "../../_secondary/fitting/fittingStore"));
...
function assertCharacterOffline(characterID) {
    let control;
    try {
        control = characterControlRuntime.getCharacterControlSnapshot(characterID);
    }
    catch {
        throwDeletionUserError("Character control is temporarily unavailable; deletion remains disabled.");
    }
...
function listCharacterOwnedRootItems(characterID) {
    const ownedItems = listOwnedItems(characterID).sort((left, right) => (toPositiveInt(left && left.itemID, 0) - toPositiveInt(right && right.itemID, 0)));
...
function transferOwnedItemTree(rootItemID, destinationOwnerID, destinationLocationID, destinationFlagID) {
...
function transferCharacterAssetsToCorporation(characterID, characterRecord, corporationID) {
    ...
    for (const rootItem of roots) {
        const transferResult = transferOwnedItemTree(rootItem.itemID, corporationID, resolveDeliveryLocationForRootItem(characterRecord, rootItem), CORPORATION_DELIVERIES_FLAG_ID);
        if (!transferResult.success) {
            throwDeletionUserError(`Failed to move assets into corporation deliveries for ${characterRecord.characterName || characterRecord.characterID}.`);
        }
    ...
    while (true) {
        ...
        for (const rootItem of roots) {
            ...
            if (!removeResult.success) {
                throwDeletionUserError(`Failed to destroy assets for character ${characterID}.`);
            }
...
function purgeBookmarkData(characterID) {
    ...
    for (const [bookmarkID, bookmark] of Object.entries(bookmarksRoot.records || {})) {
        if (deletedFolderIDs.has(toPositiveInt(bookmark && bookmark.folderID, 0)) ||
    ...
    for (const [ownerCharacterID, knownEntries] of Object.entries(knownRoot.recordsByCharacterID || {})) {
    ...
    for (const [groupID, group] of Object.entries(groupsRoot.records || {})) {
        ...
        if (toPositiveInt(group.creatorID, 0) === numericCharacterID &&
        ...
        const filteredMembers = Array.isArray(group.members)
            ...
            : [];
        if (filteredAdmins.length !== (Array.isArray(group.admins) ? group.admins.length : 0) ||
...
function purgeCalendarData(characterID) {
    ...
    for (const [eventID, event] of Object.entries(eventsRoot.events || {})) {
        ...
        if (toPositiveInt(event.ownerID, 0) === numericCharacterID &&
        ...
        if (Array.isArray(event.inviteeCharacterIDs)) {
            const filteredInvitees = event.inviteeCharacterIDs.filter((entry) => toPositiveInt(entry, 0) !== numericCharacterID);
    ...
    for (const [responseKey, response] of Object.entries(responsesRoot.responses || {})) {
        ...
        if (toPositiveInt(response.characterID, 0) === numericCharacterID ||
...
function deleteCharacter(characterID, accountID) {
    ...
    const characterRecord = assertCharacterOwnedByAccount(numericCharacterID, accountID);
    assertCharacterOffline(numericCharacterID);
    const deletePrepareDateTime = normalizeBigInt(getCharacterDeletePrepareDateTime(characterRecord), 0n);
    ...
    const assetSummary = originalCorporation && originalCorporation.isNPC !== true
        ? transferCharacterAssetsToCorporation(numericCharacterID, characterRecord, originalCorporationID)
    ...
    const updateResult = characterState.writeCharacterRecord(numericCharacterID, {
        ...
        tutorialEntryMode: characterRecord.tutorialEntryMode || "disabled",
        nesIntroState: 0,
    ...
    const affiliationResult = setCharacterAffiliation(numericCharacterID, DOOMHEIM_CORPORATION_ID, null);
    ...
    log.info(`[CharacterDeletion] Deleted char=${numericCharacterID} corp=${originalCorporationID || 0} ` +
...
//# sourceMappingURL=characterDeletionRuntime.js.map