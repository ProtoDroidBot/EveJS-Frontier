...
const { getAgentByID, listAgents, } = require(path.join(__dirname, "../agent/agentAuthority"));
const { clearCache: clearNpcStandingsAuthorityCache, getRelationsForOwner, listNpcStandings: listNpcStandingsFromAuthority, } = require(path.join(__dirname, "./npcStandingsAuthority"));
const { getCharacterIDsInCorporation, getCorporationRecord, getOwnerLookupRecord, } = require(path.join(__dirname, "../corporation/corporationState"));
const { getAllFactionRecords, getFactionIDForCorporation, isFactionID, } = require(path.join(__dirname, "../faction/factionState"));
...
// Division Connections skills: +10% mission loyalty point gain per level, but only for
// agents in the matching corporation division. Unlike the standing skills these carry no
// dogma attribute for the bonus — TQ applies it server-side — so the level is read directly.
const TYPE_MINING_CONNECTIONS = 3893;
const TYPE_DISTRIBUTION_CONNECTIONS = 3894;
const TYPE_SECURITY_CONNECTIONS = 3895;
const DIVISION_CONNECTIONS_LP_BONUS_PER_LEVEL = 0.1;
const DIVISION_CONNECTIONS_SKILL_BY_MISSION_KIND = Object.freeze({
    mining: TYPE_MINING_CONNECTIONS,
    courier: TYPE_DISTRIBUTION_CONNECTIONS,
    distribution: TYPE_DISTRIBUTION_CONNECTIONS,
    encounter: TYPE_SECURITY_CONNECTIONS,
    security: TYPE_SECURITY_CONNECTIONS,
});
...
function invalidateStandingCachesForCharacter(characterID, record = null) {
    ...
    clearStandingCompositionCacheForCorporation(normalizePositiveInteger(characterRecord && characterRecord.corporationID, 0));
}
...
function readAllCharacterRecords() {
    ...
    const records = result.success && result.data && typeof result.data === "object"
        ? result.data
        : {};
...
function getLegacyOrBucketStandingRows(record, bucketName) {
    const standingData = record && record.standingData && typeof record.standingData === "object"
        ? record.standingData
        : {};
...
function getCharacterRawStandingFromRecord(record, ownerID, characterID) {
    ...
    const buckets = buildCharacterStandingBuckets(record);
    const directMatch = buckets.charRows.find((entry) => entry.fromID === normalizedOwnerID &&
        entry.toID === normalizedCharacterID);
    if (directMatch) {
        return directMatch;
    }
    return buckets.npcRows.find((entry) => entry.fromID === normalizedOwnerID &&
        entry.toID === normalizedCharacterID) || null;
}
...
function listNpcStandings() {
    ...
    if (authorityRows.length) {
        return authorityRows.sort((left, right) => left.fromID - right.fromID ||
    ...
    for (const { record } of readAllCharacterRecords()) {
        for (const row of getLegacyOrBucketStandingRows(record, "npc")) {
            ...
            deduped.set(`${normalizedRow.fromID}:${normalizedRow.toID}`, normalizedRow);
        }
    }
    return [...deduped.values()].sort((left, right) => left.fromID - right.fromID ||
        left.toID - right.toID);
...
function getStandingCompositions(fromID, toID) {
    ...
    for (const characterID of getCharacterIDsInCorporation(normalizedCorporationID)) {
        ...
        const standingRow = getCharacterRawStandingFromRecord(characterRecord, normalizedFromID, characterID);
        if (!standingRow) {
            continue;
        }
        rows.push({
...
function listCorporationStandings(corporationID) {
    ...
    for (const ownerID of memberStandingOwnerIDs) {
        ...
        const average = compositions.reduce((sum, row) => sum + toNumber(row.standing, 0), 0) /
            compositions.length;
...
function getCharacterRawStanding(characterID, ownerID) {
    ...
    const row = getCharacterRawStandingFromRecord(characterRecord, normalizedOwnerID, normalizedCharacterID);
...
function getSkillLevel(skillMap, skillTypeID) {
    ...
    return Math.max(0, toInt(skillRecord.effectiveSkillLevel ??
...
function resolveCharacterSocialAttributes(characterID) {
    ...
    const attributes = {
        [ATTRIBUTE_NEGOTIATION_PERCENTAGE]: roundSocialAttributeValue(applyPercentPerLevel(1, NEGOTIATION_BONUS_PER_LEVEL, getSkillLevel(skillMap, TYPE_NEGOTIATION))),
        [ATTRIBUTE_DIPLOMACY_BONUS]: roundSocialAttributeValue(getSkillLevel(skillMap, TYPE_DIPLOMACY) * STANDING_BONUS_PER_LEVEL),
        [ATTRIBUTE_FAST_TALK_PERCENTAGE]: roundSocialAttributeValue(applyPercentPerLevel(1, FAST_TALK_BONUS_PER_LEVEL, getSkillLevel(skillMap, TYPE_FAST_TALK))),
        [ATTRIBUTE_CONNECTIONS_BONUS]: roundSocialAttributeValue(getSkillLevel(skillMap, TYPE_CONNECTIONS) * STANDING_BONUS_PER_LEVEL),
        [ATTRIBUTE_CRIMINAL_CONNECTIONS_BONUS]: roundSocialAttributeValue(getSkillLevel(skillMap, TYPE_CRIMINAL_CONNECTIONS) * STANDING_BONUS_PER_LEVEL),
        [ATTRIBUTE_SOCIAL_BONUS]: roundSocialAttributeValue(applyPercentPerLevel(1, SOCIAL_GAIN_BONUS_PER_LEVEL, getSkillLevel(skillMap, TYPE_SOCIAL))),
    };
    const { applyDogmaModifierGroups, getActiveImplantCharacterModifierEntries, } = getActiveImplantModifiersModule();
    applyDogmaModifierGroups(attributes, getActiveImplantCharacterModifierEntries(normalizedCharacterID).filter((entry) => SOCIAL_CHARACTER_ATTRIBUTE_IDS.includes(toInt(entry && entry.modifiedAttributeID, 0))));
    return attributes;
}
...
function getCharacterNegotiationRewardMultiplier(characterID) {
    const multiplier = getCharacterSocialAttribute(characterID, ATTRIBUTE_NEGOTIATION_PERCENTAGE, 1);
    return multiplier > 0 ? multiplier : 1;
}
function getCharacterDivisionConnectionsLpMultiplier(characterID, missionKind) {
    const normalizedCharacterID = normalizePositiveInteger(characterID, 0);
    if (!normalizedCharacterID) {
        return 1;
    }
    const skillTypeID = DIVISION_CONNECTIONS_SKILL_BY_MISSION_KIND[String(missionKind || "").trim().toLowerCase()] || 0;
    if (!skillTypeID) {
        return 1;
    }
    const skillLevel = getSkillLevel(getCachedCharacterSkillMap(normalizedCharacterID), skillTypeID);
    return 1 + Math.min(5, skillLevel) * DIVISION_CONNECTIONS_LP_BONUS_PER_LEVEL;
}
...
function applyStandingBonusToValue(rawStanding, bonus) {
    ...
    return roundStandingValue((1 - (1 - numericStanding / 10) * (1 - numericBonus / 10)) * 10);
}
function getCharacterEffectiveStanding(characterID, ownerID) {
    const rawStanding = getCharacterRawStanding(characterID, ownerID);
    const { bonus, skillTypeID } = getStandingBonusForCharacter(characterID, ownerID, rawStanding);
    return {
        rawStanding,
        bonus,
        skillTypeID,
...
function calculateStandingsByRawChange(currentStanding, rawChange) {
    ...
    if (change > 0) {
        if (standing < MAX_STANDING) {
            return roundStandingValue(Math.min(MAX_STANDING, 10 * (1 - (1 - standing / 10) * (1 - change))));
        }
        return standing;
    }
    if (change < 0) {
        if (standing > MIN_STANDING) {
            return roundStandingValue(Math.max(MIN_STANDING, 10 * (standing / 10 + (1 + standing / 10) * change)));
        }
        return standing;
    }
    return standing;
...
function calculateNewStandings(rawChange) {
    return roundStandingValue(Math.max(Math.min(10 * toNumber(rawChange, 0), MAX_STANDING), MIN_STANDING));
}
...
function calculateSocialAdjustedRawChange(characterID, rawChange) {
    ...
    return roundStandingValue(baseRawChange *
...
function buildStandingPreview(characterID, entries = [], options = {}) {
    ...
    for (const entry of Array.isArray(entries) ? entries : []) {
        ...
        const rawChange = options.applySocial === false
            ...
            : calculateSocialAdjustedRawChange(normalizedCharacterID, entry && entry.rawChange);
        if (rawChange === 0) {
            continue;
        }
...
function resolveBestStandingValue(characterID, targetOwnerIDs = []) {
    ...
    const targetSet = new Set((Array.isArray(targetOwnerIDs) ? targetOwnerIDs : [targetOwnerIDs])
    ...
    for (const [priorityFloor, rows] of [
        ...
        for (const entry of rows) {
            ...
            const priority = entry.fromID === normalizedCharacterID
                ? 3
                : priorityFloor;
...
function canCharacterUseAgent(characterID, agentRecord) {
    ...
    const factionStanding = getCharacterEffectiveStanding(normalizedCharacterID, agentRecord && agentRecord.factionID).standing;
    const corporationStanding = getCharacterEffectiveStanding(normalizedCharacterID, agentRecord && agentRecord.corporationID).standing;
    const agentStanding = getCharacterEffectiveStanding(normalizedCharacterID, agentRecord && agentRecord.agentID).standing;
    const threshold = (level - 1) * 2.0 - 1.0;
    if (Math.max(factionStanding, corporationStanding, agentStanding) >= threshold &&
...
function applyStandingChanges(characterID, changes = [], options = {}) {
    ...
    const writeResult = updateCharacterRecord(normalizedCharacterID, (record) => {
        ...
        for (const change of changes) {
            ...
            const currentRow = getCharacterRawStandingFromRecord(record, ownerID, normalizedCharacterID);
            ...
            upsertStandingRow(standingData.char, ownerID, normalizedCharacterID, nextStanding);
            ...
            if (!options.disableDerived) {
                for (const relation of getRelationsForOwner(ownerID)) {
                    ...
                    const relationValue = toNumber(relation && (Object.prototype.hasOwnProperty.call(relation, "propagationMultiplier")
            ...
            appendStandingTransaction(record, {
                fromID: ownerID,
                toID: normalizedCharacterID,
                eventTypeID: toInt(change && change.eventTypeID, EVENT_STANDING_AGENT_MISSION_COMPLETED),
                eventDateTime: nowFileTimeString(),
                modification: rawChange,
                msg: normalizeText(change && change.msg, ""),
                int_1: change && Object.prototype.hasOwnProperty.call(change, "int_1")
                    ? change.int_1
                    : null,
                int_2: change && Object.prototype.hasOwnProperty.call(change, "int_2")
                    ? change.int_2
                    : null,
                int_3: change && Object.prototype.hasOwnProperty.call(change, "int_3")
                    ? change.int_3
                    : null,
            });
        }
        for (const [targetOwnerID, derived] of derivedAccumulator.entries()) {
            ...
            const currentRow = getCharacterRawStandingFromRecord(record, targetOwnerID, normalizedCharacterID);
            const currentStanding = currentRow ? currentRow.standing : 0;
            const nextStanding = currentRow
                ? calculateStandingsByRawChange(currentStanding, derivedRawChange)
                : calculateNewStandings(derivedRawChange);
            upsertStandingRow(standingData.char, targetOwnerID, normalizedCharacterID, nextStanding);
            modifications.push([
                targetOwnerID,
            ...
            appendStandingTransaction(record, {
                fromID: targetOwnerID,
                toID: normalizedCharacterID,
                eventTypeID: derivedRawChange >= 0
...
function setCharacterStandings(characterID, entries = [], options = {}) {
    ...
    const writeResult = updateCharacterRecord(normalizedCharacterID, (record) => {
        ...
        for (const entry of entries) {
            ...
            upsertStandingRow(standingData.char, ownerID, normalizedCharacterID, nextStanding);
            const eventTypeID = Math.abs(nextStanding) <= ZERO_STANDING_EPSILON
    ...
    for (const entry of appliedChanges) {
        notifyStandingSet(normalizedCharacterID, entry.fromID, entry.toID, entry.standing);
    }
    return {
        success: true,
        data: {
...
function setCharacterStanding(characterID, ownerID, standing, options = {}) {
    return setCharacterStandings(characterID, [{ ownerID, standing }], options);
}
...
function normalizeMissionStandingOutcome(outcome = "completed") {
    ...
    return Object.prototype.hasOwnProperty.call(MISSION_STANDING_EVENT_TYPE_BY_OUTCOME, normalizedOutcome)
        ? normalizedOutcome
        : "completed";
}
...
function buildMissionStandingMessage(outcome, missionName, eventDefinition = {}) {
    ...
    const body = normalizeText(eventDefinition.messageBody, `${missionName} bonus standing adjustment`);
...
function buildMissionOutcomeStandingChanges(agentRecord, missionRecord, outcome = "completed") {
    const rewards = missionRecord && missionRecord.rewards && typeof missionRecord.rewards === "object"
        ...
        : {};
    const normalizedOutcome = normalizeMissionStandingOutcome(outcome);
    const eventDefinition = resolveMissionStandingEventDefinition(rewards, normalizedOutcome);
    const rawStandings = eventDefinition && eventDefinition.rawStandings && typeof eventDefinition.rawStandings === "object"
        ? eventDefinition.rawStandings
        : eventDefinition;
    const missionName = normalizeText(missionRecord && missionRecord.missionTitle, normalizeText(missionRecord && missionRecord.contentID, "Placeholder Mission"));
    const eventTypeID = toInt(eventDefinition && eventDefinition.eventTypeID, MISSION_STANDING_EVENT_TYPE_BY_OUTCOME[normalizedOutcome]);
    const applySocial = eventDefinition && Object.prototype.hasOwnProperty.call(eventDefinition, "applySocial")
        ? eventDefinition.applySocial === true
        : MISSION_STANDING_SOCIAL_BY_OUTCOME[normalizedOutcome] === true;
    const msg = buildMissionStandingMessage(normalizedOutcome, missionName, eventDefinition);
    return [
        {
            ...
            applySocial,
            msg,
            int_1: eventDefinition && Object.prototype.hasOwnProperty.call(eventDefinition, "int_1")
                ? eventDefinition.int_1
                : null,
            int_2: eventDefinition && Object.prototype.hasOwnProperty.call(eventDefinition, "int_2")
                ? eventDefinition.int_2
                : null,
            int_3: eventDefinition && Object.prototype.hasOwnProperty.call(eventDefinition, "int_3")
...
function buildMissionRewardStandingChanges(agentRecord, missionRecord) {
    return buildMissionOutcomeStandingChanges(agentRecord, missionRecord, "completed");
}
function applyMissionStandingChanges(characterID, agentRecord, missionRecord, outcome = "completed") {
    const changes = buildMissionOutcomeStandingChanges(agentRecord, missionRecord, outcome);
    if (!changes.length) {
        return {
            success: true,
            data: {
...
module.exports = {
    ...
    TYPE_DIPLOMACY,
    TYPE_DISTRIBUTION_CONNECTIONS,
    TYPE_FAST_TALK,
    TYPE_MINING_CONNECTIONS,
    TYPE_NEGOTIATION,
    TYPE_SECURITY_CONNECTIONS,
    TYPE_SOCIAL,
    ...
    getCharacterDivisionConnectionsLpMultiplier,
...
//# sourceMappingURL=standingRuntime.js.map