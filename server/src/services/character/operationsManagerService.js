...
const { getCharacterTutorialSnapshot, } = require(path.join(__dirname, "../npe/tutorialRuntime"));
...
function resolveOperationProgressState(args, session) {
    ...
    return ((session && (session.operationProgress || session.operationsProgress)) ||
        (characterRecord &&
...
class OperationsManagerService extends BaseService {
    ...
    Handle_get_character_progress(args, session) {
        ...
        return buildDict(normalizeProgressEntries(resolveOperationProgressState(args, session), GOLDEN_LOGIN_CHARACTER_PROGRESS_ENTRIES));
    }
...
//# sourceMappingURL=operationsManagerService.js.map