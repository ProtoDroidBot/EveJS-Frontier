...
function getAllGroups() {
  ...
  return Object.entries<any>(result.data)
...
function addMembers(groupID, membershipTypeByMemberID) {
  ...
  for (const [memberID, membershipType] of Object.entries<any>(membershipTypeByMemberID)) {
...
function deleteMembers(groupID, memberIDs) {
  ...
  const failed: any[] = [];
  const removeSet = new Set<any>(memberIDs.map((id) => Number(id)));
...
function updateMembershipTypes(groupID, memberIDs, newMembershipType) {
  ...
  if (!Array.isArray(group.members)) return [];
  const failed: any[] = [];
  const updateSet = new Set<any>(memberIDs.map((id) => Number(id)));
