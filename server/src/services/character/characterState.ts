...
function logMissileChargeDebug(event, details: Record<string, any> = {}) {
...
const invalidCharacterRowWarnings = new Set<any>();
const characterLocationReconcileWarnings = new Set<any>();
...
function resolveSystemIdentity(solarSystemID, fallback: Record<string, any> = {}) {
...
function writeCharacterRecord(charId, record, options?: any) {
...
function appendSessionChange(changes, key, oldValue, newValue, options: Record<string, any> = {}) {
...
function normalizeWorldSpaceID(record: Record<string, any> = {}) {
...
function resolveValidCharacterSchoolID(record: Record<string, any> = {}, fallback = 11) {
...
function buildDefaultEmploymentHistory(record: Record<string, any> = {}) {
  const createdAt =
...
function normalizeEmploymentHistory(record: Record<string, any> = {}) {
...
function buildDefaultStandingData(charId, record: Record<string, any> = {}) {
...
function normalizeStandingRows(rows: any[] = [], fallbackRows: any[] = []) {
...
function normalizeStandingData(charId, record: Record<string, any> = {}) {
...
function normalizeCharacterAttributes(record: Record<string, any> = {}) {
  const source =
...
function normalizeRespecInfo(record: Record<string, any> = {}) {
  const source =
...
function resolveHomeStationInfo(charData: Record<string, any> = {}, session = null) {
...
function reconcileCharacterLocationFromActiveShip(charId, record: Record<string, any> = {}, activeShip = null) {
...
function normalizeDogmaNumericAttributeMap(attributes: Record<string, any> = {}) {
...
function buildChargeDogmaPrimeAttributes(item, options: Record<string, any> = {}) {
  const normalizedChargeQuantity = Math.max(
    0,
    Number(item && (item.stacksize ?? item.quantity ?? 0)) || 0,
  );
  const attributes = Object.fromEntries(
    Object.entries<any>(getTypeDogmaAttributes(Number(item && item.typeID) || 0))
      .map(([attributeID, value]) => [Number(attributeID), Number(value)])
      .filter(
        ([attributeID, value]) =>
          Number.isInteger(attributeID) && Number.isFinite(value),
...
function buildChargeDogmaPrimeEntry(item, options: Record<string, any> = {}) {
  ...
  const attributes = normalizeInfoAttributesForProfile(
    {
      type: "dict",
      entries: Object.entries<any>(chargeAttributes).map(([attributeID, value]) => [
        Number(attributeID),
        Number(value),
      ]),
    },
...
function buildInventoryDogmaPrimeEntry(item, options: Record<string, any> = {}) {
  const now =
    typeof options.now === "bigint"
      ? options.now
      : currentFileTime();
...
function syncModuleAttributeChangesForSession(session, changes: any[] = []) {
  if (
    !session ||
    typeof session.sendNotification !== "function" ||
...
function buildItemChangePayload(item, previousState: Record<string, any> = {}) {
  const entries: any[] = [];
...
function buildChargeSublocationChangePayload(item, previousState: Record<string, any> = {}) {
  const entries: any[] = [];
...
function buildChargeSublocationRepairPreviousState({
  ...
  const repairPreviousState: Record<string, any> = {
    locationID: 0,
    flagID: 0,
  };
...
function syncDamageStateAttributesForSession(session, item, previousState: Record<string, any> = {}) {
...
function syncChargeSublocationTransitionForSession(
  ...
  if (nextTypeID > 0 && nextQuantity > 0) {
    ...
    const resolveProvidedNextChargeSync = () => {
      ...
      const providedQuantity = Math.max(
        0,
        Number(nextChargeItem.stacksize ?? nextChargeItem.quantity ?? 0) || 0,
      );
    ...
    const providedNextChargeSync =
      repairDelayMs <= 0 ? resolveProvidedNextChargeSync() : null;
    const resolveValidatedNextChargeSync = () =>
      providedNextChargeSync || resolveCurrentChargeTupleSyncItem({
        session,
...
function syncInventoryItemForSession(session, item, previousState: Record<string, any> = {}, options: Record<string, any> = {}) {
  if (
    !session ||
    typeof session.sendNotification !== "function" ||
    !item ||
...
function emitItemsChangedBatchForSession(session, changes: any[] = [], options: Record<string, any> = {}) {
...
function emitItemsChangedForSession(session, item, previousState: Record<string, any> = {}, options: Record<string, any> = {}) {
...
function sortAttributeChangesForStrip(changes, preferredOrder: any[] = []) {
...
function emitFittingTransactionForSession(session, shipID, changes, options: Record<string, any> = {}) {
  if (!session || typeof session.sendNotification !== "function") {
    return false;
  }
  const charID = Number(session.characterID || session.charid) || 0;
...
function syncModuleOnlineEffectForSession(session, item, options: Record<string, any> = {}) {
  if (
    !session ||
    typeof session.sendNotification !== "function" ||
    !item ||
...
function syncModuleOnlineBounceForSession(session, item, options: Record<string, any> = {}) {
  if (
    !session ||
    !item ||
    typeof item !== "object"
...
function syncOnlineModuleEffectsForSession(session, shipID = null, options: Record<string, any> = {}) {
  if (
    !session ||
    typeof session.sendNotification !== "function"
  ) {
...
function syncFittedModulesForSession(session, shipID = null, options: Record<string, any> = {}) {
  if (
    !session ||
    typeof session.sendNotification !== "function"
  ) {
  ...
  const loadedChargeItems = preferSyntheticHardpointOnlineBounce
    ...
    : [];
  const loadedChargeByFlag = new Map<any, any>(
...
function syncShipFittingStateForSession(session, shipID = null, options: Record<string, any> = {}) {
...
function completeDockedFittingBootstrap(session, options: Record<string, any> = {}) {
...
function flushDeferredDockedShipSessionChange(session, options: Record<string, any> = {}) {
  if (
    !session ||
...
function buildCharacterSessionNotificationPlan(session, options: Record<string, any> = {}) {
  if (!session) {
    return null;
  }
  ...
  const sessionChanges: Record<string, any> = {};
  appendSessionChange(
    sessionChanges,
    "charid",
...
function flushCharacterSessionNotificationPlan(session, plan, options: Record<string, any> = {}) {
  if (!session || !plan) {
    return false;
  }
...
function applyCharacterToSession(session, charId, options: Record<string, any> = {}) {
  if (!session) {
    return {
      success: false as const,
...
function activateShipForSession(session, shipId, options: Record<string, any> = {}) {
  ...
  let previousCapsuleRemovalChanges: any[] = [];
...
function clearCharacterFromSession(session, options: Record<string, any> = {}) {
  if (!session) {
    return {
      success: false as const,
      errorMsg: "SESSION_REQUIRED",
