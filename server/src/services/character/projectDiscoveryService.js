...
const SYNTHETIC_TASK_IMAGE_URL = "res:/UI/Texture/classes/ProjectDiscovery/covid/tutorial/onegate.png";
...
class ProjectDiscoveryService extends BaseService {
    ...
    Handle_increase_tutorial_level(args, session) {
        const state = getStateForSession(session);
        state.tutorialLevel = Math.min(TUTORIAL_LEVEL_COUNT, Math.max(0, Math.trunc(Number(state.tutorialLevel) || 0)) + 1);
    ...
    Handle_get_player_analysis_kredits(args, session) {
        return Math.max(0, Math.trunc(Number(getStateForSession(session).analysisKredits) || 0));
    }
