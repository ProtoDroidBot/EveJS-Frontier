...
const { buildDbRowset, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { listCharacterItems, getItemMetadata, } = require(path.join(__dirname, "../inventory/itemStore"));
const { buildDockableAssetLocationMetadata, isHiddenPersonalAssetLocation, } = require(path.join(__dirname, "../inventory/inventoryVisibilityRules"));
const { INVENTORY_ROW_DESCRIPTOR_COLUMNS, buildItemsInSystemsRowset, } = require(path.join(__dirname, "./charMgrGlobalAssetsRowsets"));
...
function isDockableAssetFlag(flagID) {
    ...
    return (numericFlagID === FLAG_WALLET ||
...
class CharMgrGlobalAssets {
    ...
    _getSessionCharacterId(session) {
        return toInteger(session && (session.characterID || session.charid || session.userid), 0);
    }
    ...
    _resolveRootLocation(item, itemById, session, caches = {}) {
        ...
        const rootLocationByItemID = caches.rootLocationByItemID instanceof Map ? caches.rootLocationByItemID : null;
        ...
        while (currentItem) {
            ...
            if (worldData.getStationByID(locationID) || worldData.getStructureByID(locationID)) {
                resolvedRootLocation = this._buildDockableLocation(locationID, session, caches.dockableLocationByContext);
                break;
            }
            ...
            if (!parentItem) {
                if (isDockableAssetFlag(currentItem.flagID) &&
    ...
    _buildAssetSnapshot(session) {
        ...
        const itemById = new Map(items.map((item) => [toInteger(item && item.itemID, 0), item]).filter(([itemID]) => itemID > 0));
        const allEntries = [];
        const caches = {
    ...
    _buildStationRows(topLevelEntries) {
        ...
        return [...rowsByStationID.values()]
            .sort((left, right) => toInteger(left.solarSystemID, 0) - toInteger(right.solarSystemID, 0) ||
    ...
    _calculateItemUnits(item) {
        ...
        return Math.max(0, toInteger(item.stacksize, toInteger(item.quantity, 0)));
    }
    ...
    Handle_MachoResolveObject(args, session, _kwargs) {
        ...
        log.debug(`[CharMgrGlobalAssets] MachoResolveObject char=${context.charID} container=${context.containerID}`);
    ...
    async Handle_MachoBindObject(args, session, kwargs, invokeNestedCall) {
        ...
        let callResult = null;
        if (typeof invokeNestedCall === "function" &&
            ...
            callResult = await invokeNestedCall(idString, methodName, normalizeCallArgs(callArgs), callKwargs);
        }
        log.debug(`[CharMgrGlobalAssets] MachoBindObject char=${context.charID} bound=${idString} nested=${Array.isArray(nestedCall) && nestedCall.length ? normalizeMethodName(nestedCall[0]) : "none"} kwargs=${kwargs ? "yes" : "no"}`);
        return [
            {
                type: "substruct",
                value: {
    ...
    Handle_ListStations(_args, session) {
        ...
        return buildDbRowset(STATION_DBROW_COLUMNS, this._buildStationRows(snapshot.topLevelEntries), "carbon.common.script.sys.crowset.CRowset");
    }
    ...
    Handle_ListStationItems(args, session) {
        ...
        const items = snapshot.topLevelEntries
            .filter((entry) => toInteger(entry.rootLocation && entry.rootLocation.stationID, 0) === locationID &&
...
//# sourceMappingURL=charMgrGlobalAssets.js.map