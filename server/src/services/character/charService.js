...
const characterControlRuntime = require("../online/characterControlRuntime");
...
const { ACCOUNT_KEY, JOURNAL_CURRENCY, JOURNAL_ENTRY_TYPE, } = require("../account/walletState");
const { PLEX_LOG_CATEGORY, getTransactionID, } = require("../account/plexVaultLogState");
const { clonePaperDollPayload, resolvePaperDollState, } = require("./paperDollPayloads");
const { normalizeCharacterGender, } = require("./characterIdentity");
const { VALIDATION_CODE, getValidRandomName, normalizeNameString, validateCharacterName, } = require("./characterNameRuntime");
const { spawnRookieShipForCharacter, } = require("../ship/rookieShipRuntime");
const { ensureCreationState, } = require("../frontier/creationRuntime");
const { SuiCharacterProvisioningError, prepareSuiCharacterIdentity, provisionSuiCharacter, } = require("../frontier/suiCharacterProvisioning");
const { SUI_GRPC_BASE_URL, SUI_GRPC_NETWORK, } = require("../frontier/suiGrpcClient");
const { listCharacterItems, removeInventoryItem, } = require("../inventory/itemStore");
const { broadcastStationGuestJoined, broadcastStructureGuestJoined, } = require("../_shared/guestLists");
const { getUnreadMailCount, sendWelcomeMailToCharacter, } = require("../mail/mailState");
const { getCorporationRecord, } = require("../corporation/corporationState");
const { getUnprocessedNotificationCount, } = require("../notifications/notificationState");
const { buildNewCharacterTutorialState, consumeCompletedAirNpeRevealOnFirstLogin, getCharacterTutorialSnapshot, markCharacterTutorialSkipped, } = require("../npe/tutorialRuntime");
const { getStationRecord, } = require("../_shared/stationStaticData");
const { NEW_CHARACTER_START_OVERRIDE, } = require("../_shared/newCharacterStartOverride");
const { cancelCharacterDeletePrepare, deleteCharacter, getCharacterDeletePrepareDateTime, isCharacterQueuedForDeletion, prepareCharacterForDelete, } = require("./characterDeletionRuntime");
const { reserveCharacterID, } = require("../_shared/identityAllocator");
const worldData = require("../../space/worldData");
const { ROLE_CONTENT, ROLE_GML, ROLE_PROGRAMMER, ROLE_QA, normalizeRoleValue, } = require("../account/accountRoleProfiles");
const CHARACTER_CREATION_VALIDATION_SESSION_KEY = "_characterCreationValidationEnabled";
const CHARACTER_CREATION_VALIDATION_ROLE_MASK = ROLE_CONTENT | ROLE_QA | ROLE_PROGRAMMER | ROLE_GML;
...
function readCreationIntArg(args, index, fallback, legacyIndex = null) {
    ...
    const rawLegacy = legacyIndex !== null && args && args.length > legacyIndex
        ? unwrapCreationArg(args[legacyIndex])
        : undefined;
    const candidate = rawPrimary !== undefined && rawPrimary !== null && rawPrimary !== ""
        ? rawPrimary
        : rawLegacy;
...
function isCreationPayloadArg(value) {
    ...
    return (Object.prototype.hasOwnProperty.call(value, "sculpts") ||
...
function readCreationPayloadArg(args, index, legacyIndex = null) {
    const candidate = args && args.length > index
        ? args[index]
...
function resolveCharacterRequestId(args, kwargs, fallback = 0) {
    const directArg = args && args.length > 0 ? Number(unwrapCreationArg(args[0])) : NaN;
...
function resolveStarterLocationContext(bloodlineProfile, schoolID) {
    ...
    const starterCorporation = resolveStarterCorporationRecord(bloodlineProfile, schoolProfile);
    const schoolStationID = schoolProfile.startingStations && schoolProfile.startingStations.length > 0
        ? Number(schoolProfile.startingStations[0]) || null
        : null;
    const starterStation = getStationRecord(null, NEW_CHARACTER_START_OVERRIDE.stationID ||
        schoolStationID ||
        (starterCorporation && starterCorporation.stationID
            ? starterCorporation.stationID
            : null)) || getStationRecord(null, schoolStationID ||
        (starterCorporation && starterCorporation.stationID
            ? starterCorporation.stationID
            : null));
    return {
        corporationID: Number(starterCorporation && starterCorporation.corporationID) ||
            Number(bloodlineProfile && bloodlineProfile.corporationID) ||
            1000009,
        factionID: Number(starterCorporation && starterCorporation.factionID) || null,
        ...
        constellationID: Number(starterStation && starterStation.constellationID) || 20000020,
        regionID: Number(starterStation && starterStation.regionID) || 10000002,
    };
}
function resolveFrontierStarterLocationContext(groupID) {
    if (String(config.clientCompatibilityProfile || "").toLowerCase() !==
        "frontier" ||
        Number(groupID) !== 1) {
        return null;
    }
    const station = [...worldData.ensureLoaded().stations].sort((left, right) => Number(left.stationID) - Number(right.stationID))[0];
    if (!station) {
        return null;
    }
    ...
    return {
        ...
        constellationID: Number(station.constellationID || system.constellationID) || 0,
...
function rollbackNewCharacterBootstrap(characterID) {
    ...
    const ownedItemIDs = new Set(ownedItems.map((item) => Number(item && item.itemID) || 0));
    const rootItems = ownedItems.filter((item) => !ownedItemIDs.has(Number(item && item.locationID) || 0));
...
function findPendingSuiCharacter(accountID, characterName) {
    const expectedAccountID = normalizeAccountID(accountID);
    const expectedName = normalizeNameString(characterName)
        .trim()
        .replace(/\s+/g, " ")
        .toLowerCase();
    if (expectedAccountID <= 0 || !expectedName) {
        return null;
    }
    const result = repo.read("characters", "/");
    const characters = result.success && result.data ? result.data : {};
    const pendingStatuses = new Set(["pending", "reconciliation-required"]);
    return (Object.entries(characters)
        .map(([characterID, record]) => ({
        characterID: Number(characterID),
        record,
    }))
        .filter(({ characterID, record }) => Number.isSafeInteger(characterID) &&
        characterID > 0 &&
        characterBelongsToAccount(record, expectedAccountID) &&
        pendingStatuses.has(String(record && record.suiProvisioningStatus)) &&
        normalizeNameString(record && record.characterName)
            .trim()
            .replace(/\s+/g, " ")
...
function resolveCharacterSelectionStationID(character) {
    ...
    const stationID = normalizePositiveLocationID(character.stationID) ||
    ...
    return (normalizePositiveLocationID(character.structureID) ||
...
function evictPriorSession(priorSession, options = {}) {
    if (!priorSession) {
        return;
    }
    try {
        const { disconnectCharacterSession, } = require("../_shared/sessionDisconnect");
        disconnectCharacterSession(priorSession, {
            broadcast: true,
            clearSession: true,
            controlTransition: options.controlTransition !== false,
            lifecycleReason: options.lifecycleReason || "retail_takeover",
        });
    }
    catch (error) {
...
class CharService extends BaseService {
    constructor(options = {}) {
        ...
        this.suiCharacterProvisioner =
            typeof options.suiCharacterProvisioner === "function"
                ? options.suiCharacterProvisioner
                : provisionSuiCharacter;
        this.suiCharacterProvisioningOptions =
            options.suiCharacterProvisioningOptions &&
                typeof options.suiCharacterProvisioningOptions === "object"
                ? { ...options.suiCharacterProvisioningOptions }
                : {};
        const envSetting = String(process.env.EVEJS_SUI_CHARACTER_PROVISIONING_ENABLED || "true").toLowerCase();
        this.suiCharacterProvisioningEnabled =
    ...
    _resumePendingSuiCharacter(pendingCharacter) {
        ...
        try {
            ...
            log.warn(`[CharService] Cannot resume pending Sui Character char=${characterID} ` +
                `code=${error && error.code ? error.code : "UNKNOWN"}`);
            throwWrappedUserError("CustomInfo", {
                info: "The pending Sui Character binding needs administrator reconciliation.",
            });
        }
        log.info(`[CharService] Resuming pending Sui Character provisioning char=${characterID}`);
        return Promise.resolve()
            ...
            const bindingWrite = updateCharacterRecord(characterID, (current) => ({
                ...current,
                suiNetwork: suiResult.network,
                suiBaseUrl: suiResult.baseUrl,
                suiWalletAddress: suiResult.walletAddress,
                suiCharacterObjectId: suiResult.characterObjectId,
                suiPlayerProfileObjectId: suiResult.playerProfileObjectId,
                suiTransactionDigest: suiResult.transactionDigest,
                suiWorldPackageId: suiResult.packageId,
                suiWorldObjectRegistryId: suiResult.objectRegistryId,
            ...
            if (!bindingWrite.success) {
                ...
                throwWrappedUserError("CustomInfo", {
                    info: "The Sui Character was recovered, but its local binding could not be saved.",
                });
            }
            const welcomeMailResult = sendWelcomeMailToCharacter(characterID, {
                characterName: record.characterName,
            });
            if (!welcomeMailResult.success) {
                log.warn(`[CharService] Failed to send welcome mail after Sui recovery char=${characterID}: ${welcomeMailResult.errorMsg}`);
            }
    ...
    Handle_GetCharactersToSelect(args, session) {
        ...
        for (const [charId, charData] of Object.entries(characters)) {
            if (characterBelongsToAccount(charData, userId)) {
                ...
                charList.push(buildKeyVal([
                    ["characterID", parseInt(String(charId), 10)],
        ...
        log.debug(`[CharService] Returning ${charList.length} characters for userId=${userId}`);
    ...
    Handle_GetCharacterSelectionData(args, session) {
        ...
        for (const [charId, rawCharacter] of Object.entries(characters)) {
            if (characterBelongsToAccount(rawCharacter, userId)) {
                ...
                const cid = parseInt(String(charId), 10);
                ...
                characterDetails.push(buildKeyVal([
                    ["characterID", cid],
                    ["characterName", character.characterName || "Unknown"],
                    [
                        "deletePrepareDateTime",
        ...
        log.debug(`[CharService] GetCharacterSelectionData: returning ${characterDetails.length} chars`);
        return [
            userDetails,
            trainingDetails,
    ...
    Handle_GetCharacterToSelect(args, session, kwargs) {
        ...
        if (character &&
            ...
            log.debug(`[CharService] Rejected GetCharacterToSelect(${charId}) for user=${requestingUserID}; ` +
        ...
        const skillPoints = getCharacterSkillPointTotal(charId) ||
        ...
        return buildKeyVal([
            ...
            [
                ...
                resolveUnprocessedNotificationCount(charId, character.unprocessedNotifications || 0),
            ],
            ["characterID", parseInt(String(charId), 10)],
    ...
    Handle_CreateCharacterWithDoll(args, session, kwargs, creationOptions = {}) {
        ...
        const raceID = createSignature === "latest-container"
            ? readCreationIntArg(args, 7, 1)
            : createSignature === "ccsvc-wrapper"
                ? readCreationIntArg(args, 1, 1)
                : 1;
        const bloodlineID = createSignature === "latest-container"
            ? readCreationIntArg(args, 1, 1)
            : createSignature === "ccsvc-wrapper"
                ? readCreationIntArg(args, 2, 1)
                : readCreationIntArg(args, 1, 1);
        const parsedGenderID = createSignature === "latest-container"
            ? readCreationIntArg(args, 2, 1)
            : createSignature === "ccsvc-wrapper"
                ? readCreationIntArg(args, 3, 1)
                : readCreationIntArg(args, 2, 1);
        const genderID = normalizeCreationGender(parsedGenderID, 1);
        const ancestryID = createSignature === "latest-container"
            ? readCreationIntArg(args, 3, 1)
            : createSignature === "ccsvc-wrapper"
                ? readCreationIntArg(args, 4, 1)
                : readCreationIntArg(args, 3, 1);
        const charInfo = createSignature === "latest-container"
            ? readCreationPayloadArg(args, 4)
            : createSignature === "ccsvc-wrapper"
                ? readCreationPayloadArg(args, 5)
                : readCreationPayloadArg(args, 4);
        const portraitInfo = createSignature === "latest-container"
            ? readCreationPayloadArg(args, 5)
            : createSignature === "ccsvc-wrapper"
                ? readCreationPayloadArg(args, 6)
                : readCreationPayloadArg(args, 5);
        const schoolID = createSignature === "latest-container"
        ...
        log.info(`[CharService] CreateCharacterWithDoll rawArgs=${JSON.stringify(summarizeCreationArg(args))}`);
        if (parsedGenderID !== genderID) {
            log.debug(`[CharService] CreateCharacterWithDoll clamped invalid gender=${parsedGenderID} -> ${genderID}`);
        }
        log.info(`[CharService] CreateCharacterWithDoll: name="${characterName}" race=${raceID} bloodline=${bloodlineID} gender=${genderID} ancestry=${ancestryID} school=${schoolID} signature=${createSignature}`);
        const accountID = normalizeAccountID(session && session.userid);
        if (accountID <= 0) {
            log.debug(`[CharService] Rejecting CreateCharacterWithDoll("${characterName}") without a valid authenticated account session`);
            throwWrappedUserError("CustomInfo", {
        ...
        if (nameValidationCode !== VALIDATION_CODE.VALID) {
            log.debug(`[CharService] Rejecting CreateCharacterWithDoll("${characterName}") invalidNameCode=${nameValidationCode}`);
            throwWrappedUserError("CharNameInvalid");
        }
        const characterResult = repo.read("characters", "/");
        const characters = characterResult.success ? characterResult.data : {};
        const newCharId = reserveCharacterID();
        const bloodlineProfile = resolveCharacterCreationBloodlineProfile(bloodlineID, {
            raceID: raceID || 1,
            typeID: 1373,
            corporationID: 1000009,
        });
        const resolvedSchoolID = resolveCharacterCreationSchoolIDForRace(schoolID, bloodlineProfile.raceID, 11);
        const raceProfile = getCharacterCreationRace(bloodlineProfile.raceID) || null;
        const explicitStarterShipTypeID = Number(creationOptions.starterShipTypeID) || 0;
        const starterShipTypeID = explicitStarterShipTypeID ||
            Number((raceProfile && raceProfile.shipTypeID) || 606) ||
            606;
        const starterShipName = String(creationOptions.starterShipName ||
            (raceProfile && raceProfile.shipName) ||
            "Velator").trim() || "Velator";
        const starterLocation = creationOptions.starterLocation ||
        ...
        characters[String(newCharId)] = {
            accountId: accountID,
            characterName: typeof characterName === "string" ? characterName : "New Character",
            gender: genderID,
            bloodlineID,
            ancestryID,
        ...
        repo.write("characters", `/${String(newCharId)}`, characters[String(newCharId)]);
        const rookieShipResult = spawnRookieShipForCharacter(newCharId, characters[String(newCharId)].stationID, {
        ...
        if (!rookieShipResult.success) {
            log.warn(`[CharService] CreateCharacterWithDoll failed to provision rookie ship for char=${newCharId} typeID=${starterShipTypeID} error=${rookieShipResult.errorMsg}`);
            if (creationOptions.initializeCreation === true) {
                const rollback = rollbackNewCharacterBootstrap(newCharId);
                log.warn(`[CharService] Rolled back failed Frontier character bootstrap char=${newCharId} success=${rollback.success}`);
                throwWrappedUserError("CustomInfo", {
                    info: "The starter Creation could not be initialized. Please try again.",
                });
            }
        }
        else if (creationOptions.initializeCreation === true &&
            rookieShipResult.data &&
            rookieShipResult.data.ship) {
            const creationResult = ensureCreationState(rookieShipResult.data.ship, newCharId);
            if (!creationResult.success) {
                log.warn(`[CharService] CreateCharacterWithDoll failed to initialize Creation ship=${rookieShipResult.data.ship.itemID} char=${newCharId} error=${creationResult.errorMsg}`);
                const rollback = rollbackNewCharacterBootstrap(newCharId);
                log.warn(`[CharService] Rolled back failed Frontier character bootstrap char=${newCharId} success=${rollback.success}`);
                throwWrappedUserError("CustomInfo", {
                    info: "The starter Creation could not be initialized. Please try again.",
        ...
        const finishCharacterCreation = () => {
            ...
            if (!welcomeMailResult.success) {
                log.warn(`[CharService] CreateCharacterWithDoll failed to send welcome mail for char=${newCharId}: ${welcomeMailResult.errorMsg}`);
            }
            ...
            log.success(`[CharService] Created character "${characterName}" with ID ${newCharId} ship=${createdCharacter ? createdCharacter.shipID : "unknown"}`);
            return newCharId;
        };
        if (creationOptions.provisionSuiCharacter !== true) {
            return finishCharacterCreation();
        }
        let suiIdentity;
        try {
            suiIdentity = prepareSuiCharacterIdentity({
                accountId: accountID,
            ...
            const rollback = rollbackNewCharacterBootstrap(newCharId);
            log.warn(`[CharService] Sui Character identity preparation failed char=${newCharId} ` +
                `code=${error && error.code ? error.code : "UNKNOWN"} rollback=${rollback.success}`);
            throwWrappedUserError("CustomInfo", {
                info: "The Sui player wallet could not be prepared. Please try again.",
            });
        }
        const pendingWrite = updateCharacterRecord(newCharId, (record) => ({
            ...record,
            suiNetwork: SUI_GRPC_NETWORK,
            suiBaseUrl: SUI_GRPC_BASE_URL,
            suiWalletAddress: suiIdentity.walletAddress,
            suiWalletDerivation: "frontier-client-sha512-v1",
        ...
        if (!pendingWrite.success || !pendingFlush.success) {
            const rollback = rollbackNewCharacterBootstrap(newCharId);
            log.warn(`[CharService] Failed to persist pending Sui Character identity char=${newCharId} rollback=${rollback.success}`);
            throwWrappedUserError("CustomInfo", {
        ...
        return Promise.resolve()
            ...
            if (!bindingWrite.success) {
                ...
                throwWrappedUserError("CustomInfo", {
                    info: "The Sui Character was created, but its local binding needs reconciliation.",
                });
            }
            log.success(`[CharService] Bound Frontier character ${newCharId} to Sui wallet=${suiResult.walletAddress} ` +
                `character=${suiResult.characterObjectId} profile=${suiResult.playerProfileObjectId}`);
            return finishCharacterCreation();
        }, (error) => {
            const isAmbiguous = (error instanceof SuiCharacterProvisioningError &&
                error.ambiguous === true) ||
                Boolean(error && error.ambiguous === true);
            const errorCode = error && typeof error.code === "string" ? error.code : "UNKNOWN";
            if (isAmbiguous) {
                updateCharacterRecord(newCharId, (record) => ({
                    ...record,
                    suiTransactionDigest: error && error.transactionDigest
                        ? error.transactionDigest
                ...
                throwWrappedUserError("CustomInfo", {
                    info: "The Sui Character transaction is still being reconciled. Please do not create a duplicate character.",
                });
            }
            const rollback = rollbackNewCharacterBootstrap(newCharId);
            log.warn(`[CharService] Sui Character provisioning failed char=${newCharId} code=${errorCode} rollback=${rollback.success}`);
            throwWrappedUserError("CustomInfo", {
                info: "The Sui Character could not be created on the local network. Please try again.",
            });
        });
    }
    Handle_CreateCharacterInSpace(args, session) {
        ...
        const starterLocation = resolveFrontierStarterLocationContext(starterGroupID);
        if (!starterLocation) {
            log.warn(`[CharService] CreateCharacterInSpace rejected starter group ${starterGroupID}`);
            throwWrappedUserError("CustomInfo", {
                info: "The selected Frontier starter group is unavailable.",
            });
        }
        if (this.suiCharacterProvisioningEnabled) {
            const pendingCharacter = findPendingSuiCharacter(session && session.userid, characterName);
            if (pendingCharacter) {
                return this._resumePendingSuiCharacter(pendingCharacter);
            }
        }
        log.info(`[CharService] CreateCharacterInSpace group=${starterGroupID} ` +
            `station=${starterLocation.stationID}`);
        return this.Handle_CreateCharacterWithDoll([characterName, 1, 1, 1, null, null, 0], session, null, {
            starterLocation,
            starterShipTypeID: FRONTIER_STARTER_SHIP_TYPE_ID,
            starterShipName: FRONTIER_STARTER_SHIP_NAME,
            initializeCreation: true,
            provisionSuiCharacter: this.suiCharacterProvisioningEnabled,
        });
    }
    Handle_GetNumCharacters(args, session) {
        ...
        const charactersResult = repo.read("characters", "/");
        const characters = charactersResult.success ? charactersResult.data : {};
        return Object.values(characters).filter((character) => characterBelongsToAccount(character, userId)).length;
    }
    Handle_UpdateCharacterGender(args, session) {
        ...
        log.info(`[CharService] UpdateCharacterGender(${charId}) gender=${genderID}`);
        const updateResult = updateCharacterRecord(charId, (record) => ({
            ...record,
            gender: genderID,
        }));
        if (!updateResult.success) {
            log.warn(`[CharService] UpdateCharacterGender failed for ${charId}: ${updateResult.errorMsg}`);
            return null;
        }
    ...
    Handle_UpdateCharacterBloodline(args, session) {
        ...
        log.info(`[CharService] UpdateCharacterBloodline(${charId}) bloodline=${bloodlineID} race=${bloodlineProfile.raceID}`);
        const updateResult = updateCharacterRecord(charId, (record) => ({
            ...record,
        ...
        if (!updateResult.success) {
            log.warn(`[CharService] UpdateCharacterBloodline failed for ${charId}: ${updateResult.errorMsg}`);
            return null;
        }
        if (session && Number(session.charid || session.characterID || 0) === Number(charId)) {
        ...
        return {
            ...
            entries: [
                [
                    ...
                    {
                        type: "list",
                        items: getCharacterCreationRaces().map((race) => buildKeyVal([
                            ["raceID", race.raceID],
                            ["raceName", race.name],
                ...
                [
                    ...
                    {
                        type: "list",
                        items: getCharacterCreationBloodlines().map((bloodline) => buildKeyVal([
    ...
    Handle_SelectCharacterID(args, session, kwargs) {
        ...
        if (characterRecord &&
            ...
            log.debug(`[CharService] Rejected foreign SelectCharacterID(${charId}) for user=${sessionUserID}; ` +
        ...
        if (characterRecord &&
            this.suiCharacterProvisioningEnabled &&
            ["pending", "reconciliation-required"].includes(String(characterRecord.suiProvisioningStatus || ""))) {
            return this._resumePendingSuiCharacter({
                characterID: charId,
                record: characterRecord,
            }).then(() => this.Handle_SelectCharacterID(args, session, kwargs));
        }
        let controlBeforeSelection = { controlState: "offline" };
        if (characterRecord) {
            try {
                controlBeforeSelection = characterControlRuntime.assertRetailControlAvailable(charId);
            }
            catch (error) {
                if (error && error.code === "CHARACTER_CONTROL_BROWSER_PILOT") {
                    throwWrappedUserError("CustomInfo", {
                        info: "That character is currently controlled by an active browser pilot. Release the browser lease before logging in with the game client.",
                    });
                }
                log.warn(`[CharService] Character-control preflight failed for ${charId}: ${error && error.message ? error.message : "unavailable"}`);
                throwWrappedUserError("CustomInfo", {
                    info: "Character control is temporarily unavailable. Please try again.",
                });
            }
        }
        const existingSession = sessionRegistry.findSessionByCharacterID(charId, {
            excludeSession: session,
            // A destroyed socket remains the retail controller until its canonical
            // close cleanup deindexes it. Including that closing session here lets a
            // configured retail takeover finish cleanup before the replacement is
            // applied, instead of racing a late stale-session disconnect.
            includeClosing: true,
        });
        let retailTakeover = false;
        if (existingSession) {
            const characterRecord = getCharacterRecord(charId);
            const characterLabel = (characterRecord && characterRecord.characterName) ||
                existingSession.characterName ||
                `Character ${charId}`;
            if (config.loginTakeoverEnabled === true) {
                retailTakeover = true;
                log.info(`[CharService] Login takeover for ${characterLabel}(${charId}); evicting prior session user=${existingSession.userName || "unknown"} client=${existingSession.clientID || 0}`);
                const evict = this._evictPriorSession || evictPriorSession;
                evict(existingSession, {
                    controlTransition: false,
                    lifecycleReason: "retail_takeover",
                });
                // fall through — allow the new login to take over the character
            }
            else {
                log.debug(`[CharService] Rejected duplicate login for ${characterLabel}(${charId}); already active on user=${existingSession.userName || "unknown"} client=${existingSession.clientID || 0}`);
                throwWrappedUserError("CustomInfo", {
        ...
        let applyResult;
        try {
            applyResult = applyCharacterToSession(session, charId, {
            ...
            if (retailTakeover) {
                characterControlRuntime.recordRetailSessionEnded(charId, {
                    session: existingSession,
                    reason: "retail_takeover_failed",
                });
            }
            throw error;
        }
        if (!applyResult.success) {
            ...
            characterControlRuntime.recordRetailSessionStarted(charId, {
                session,
                previousCharacterID,
                previousControlState: retailTakeover
                    ? "retail_client"
                    : controlBeforeSelection.controlState,
                reason: retailTakeover ? "retail_takeover" : "retail_session_started",
            });
            ...
            if (notificationFlushElapsedMs >= 250) {
                log.info(`[CharService] Session-change flush for ${session.characterName || charId} ` +
            ...
            if (guestBroadcastElapsedMs >= 250) {
                log.info(`[CharService] Guest-list broadcast for ${session.characterName || charId} ` +
    ...
    Handle_PrepareCharacterForDelete(args, session, kwargs) {
        const charId = resolveCharacterRequestId(args, kwargs, 0);
        const deletePrepareDateTime = prepareCharacterForDelete(charId, session && session.userid);
    ...
    Handle_ToggleValidation(args, session) {
        if (!canToggleCharacterCreationValidation(session)) {
            ...
            log.warn(`[CharService] ToggleValidation ignored for non-debug session userId=${session && session.userid}`);
            return true;
        }
        const nextValidationState = !getCharacterCreationValidationEnabled(session);
        session[CHARACTER_CREATION_VALIDATION_SESSION_KEY] = nextValidationState;
        log.debug(`[CharService] ToggleValidation -> ${nextValidationState ? "on" : "off"} ` +
    ...
    Handle_ValidateNameEx(args, session) {
        ...
        log.debug(`[CharService] ValidateNameEx("${normalizedName}") -> ${validationCode}`);
...
//# sourceMappingURL=charService.js.map