...
function normalizeSculptingRowHeader(header: any[] = []) {
...
function normalizePortraitDataForClient(value) {
  ...
  const portraitData: Record<string, number> = { ...DEFAULT_PORTRAIT_DATA };
...
function getStoredAppearanceInfo(record: Record<string, any> = {}) {
...
function getStoredPortraitInfo(record: Record<string, any> = {}) {
  if (!record || typeof record !== "object") {
    return null;
  }
...
function getStoredPortraitDataForClient(record: Record<string, any> = {}) {
  return normalizePortraitDataForClient(getStoredPortraitInfo(record));
}

function hasStoredAppearanceInfo(record: Record<string, any> = {}) {
  if (!record || typeof record !== "object") {
    return false;
  }
...
function resolvePaperDollState(record: Record<string, any> = {}, fallback = 0) {
