...
const { buildList, buildObjectEx2, currentFileTime, normalizeBigInt, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { getCorporationRecord, } = require(path.join(__dirname, "../corporation/corporationState"));
const { getCorporationOffices, } = require(path.join(__dirname, "../corporation/corporationRuntimeState"));
const { resolveCharacterCreationSchoolProfile, } = require(path.join(__dirname, "./characterCreationData"));
...
function resolveStation(session, args) {
    const charID = args && args.length > 0 ? Number(args[0] || 0) : Number(session && session.characterID);
...
function resolveDockedStationID(session, charData = {}) {
    return Number((session && (session.stationID || session.stationid)) ||
        charData.stationID ||
        0) || 0;
}
function resolveSchoolHQStationID(charData = {}, session = null) {
    const schoolID = Number(charData.schoolID ||
    ...
    const startingStationID = Number(schoolProfile.startingStations && schoolProfile.startingStations[0]) || 0;
...
function collectHomeStationCandidates(session) {
    ...
    const corporationID = Number(charData.corporationID ||
    ...
    const orderedStationIDs = [
        ...
        ...officeStationIDs,
        currentStationID,
    ].filter((stationID, index, array) => (stationID > 0 && array.indexOf(stationID) === index));
...
function buildHomeStationCandidatePayloads(session) {
    return collectHomeStationCandidates(session).map((candidate) => (buildStationCandidatePayload(candidate.station, {
...
function resolveAllowedHomeStationIDs(session) {
    return new Set(collectHomeStationCandidates(session).map((candidate) => (Number(candidate && candidate.station && candidate.station.stationID || 0) || 0)).filter((stationID) => stationID > 0));
}
...
function throwHomeStationValidation(errors = []) {
    throwWrappedObject(CHANGE_HOME_STATION_ERROR, [Array.isArray(errors) ? errors : []], {
...
function setHomeStation(session, stationID, allowRemote = false) {
    ...
    const cooldownFileTime = fileTimeFromMs(msFromFileTime(changeFileTime) + REMOTE_CHANGE_COOLDOWN_MS);
...
class HomeStationServiceBase extends BaseService {
    ...
    Handle_set_home_station(args, session) {
        return setHomeStation(session, args && args.length > 0 ? args[0] : null, args && args.length > 1 ? args[1] === true : false);
    }
...
//# sourceMappingURL=homeStationService.js.map