...
const characterEnergyState = require(path.join(__dirname, "characterEnergyState"));
const { buildFiletimeLong, buildObjectEx1, currentFileTime, normalizeBigInt, normalizeNumber, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const CHARACTER_ENERGY_STATE_CLASS = "characterenergy.characterEnergyState.CharacterEnergyState";
...
function resolveCharacterID(session) {
    const numericValue = Number(session && (session.charid || session.characterID || session.userid));
...
function normalizeCharacterEnergyState(rawState = {}) {
    ...
    const minEnergyLevel = normalizeNumber(source.minEnergyLevel ?? source._minEnergyLevel, DEFAULT_MIN_ENERGY_LEVEL);
    let quiescentEnergyLevel = normalizeNumber(source.quiescentEnergyLevel ?? source._quiescentEnergyLevel, DEFAULT_QUIESCENT_ENERGY_LEVEL);
    if (quiescentEnergyLevel <= minEnergyLevel) {
        quiescentEnergyLevel = minEnergyLevel + 1;
    }
    const energyIncreasePerRechargeTick = Math.max(Number.EPSILON, normalizeNumber(source.energyIncreasePerRechargeTick ??
        source._energyIncreasePerRechargeTick, DEFAULT_ENERGY_INCREASE_PER_RECHARGE_TICK));
    const rechargeTickPeriod = Math.max(Number.EPSILON, normalizeNumber(source.rechargeTickPeriod ?? source._rechargeTickPeriod, DEFAULT_RECHARGE_TICK_PERIOD_SECONDS));
    const fallbackEnergyLevel = quiescentEnergyLevel;
    const requestedEnergyLevel = normalizeNumber(source.energyLevel ?? source._energyLevel, fallbackEnergyLevel);
    const energyLevel = Math.min(quiescentEnergyLevel, Math.max(minEnergyLevel, requestedEnergyLevel));
    return {
        energyLevel,
        lastRechargeTickTimestamp: normalizeTimestamp(source.lastRechargeTickTimestamp ?? source._lastRechargeTickTimestamp),
...
function calculateCharacterEnergyStateAtFileTime(rawState = {}, filetime = currentFileTime()) {
    ...
    return {
        ...state,
        energyLevel: Math.min(state.quiescentEnergyLevel, state.energyLevel + energyIncrease),
...
function resolveRuntimeStateForCharacter(source, characterID) {
    ...
    if (Object.prototype.hasOwnProperty.call(source, "energyLevel") ||
...
function getCharacterEnergyStateNow(session, options = {}) {
    ...
    const rawState = resolveRuntimeStateForCharacter(source, characterID) ||
        characterEnergyState.getCharacterEnergyState(characterID) ||
        {};
    return calculateCharacterEnergyStateAtFileTime(rawState, options.filetime || currentFileTime());
}
function spendCharacterEnergy(session, rawAmount, options = {}) {
    ...
    const writeResult = characterEnergyState.writeCharacterEnergyState(characterID, nextState);
    ...
    log.debug(`[CharacterEnergyMgr] Spent ${amount} CEQ for char=${characterID}; energy=${nextState.energyLevel}/${nextState.quiescentEnergyLevel}`);
    return {
        success: true,
        data: {
            characterID,
...
class CharacterEnergyMgrService extends BaseService {
    ...
    getCharacterEnergyState(session) {
        const characterID = resolveCharacterID(session);
        return (resolveRuntimeStateForCharacter(this._stateByCharacterID, characterID) ||
    ...
    Handle_GetMyEnergyState(args, session) {
        ...
        log.debug(`[CharacterEnergyMgr] GetMyEnergyState char=${characterID || "unknown"} -> ${state ? "runtime" : "default"}`);
...
//# sourceMappingURL=characterEnergyMgrService.js.map