...
function resolveDockedStationID(session, charData: Record<string, any> = {}) {
  return Number(
...
function resolveSchoolHQStationID(charData: Record<string, any> = {}, session = null) {
...
function buildStationCandidatePayload(station, options: Record<string, any> = {}) {
...
function getRemoteChangeFileTime(charData: Record<string, any> = {}) {
  return normalizeBigInt(charData.nextRemoteHomeStationChangeTime, 0n);
}

function hasActiveRemoteChangeCooldown(charData: Record<string, any> = {}, nowFileTime = currentFileTime()) {
  return getRemoteChangeFileTime(charData) > normalizeBigInt(nowFileTime, currentFileTime());
}

function addCandidateValidationErrors(errors, stationID, context: Record<string, any> = {}) {
...
function isRemoteStationCandidate(stationID, context: Record<string, any> = {}) {
...
function isRemoteHomeStationChange(stationID, session, charData: Record<string, any> = {}) {
  const currentStationID = resolveDockedStationID(session, charData);
  const schoolHQStationID = resolveSchoolHQStationID(charData, session);
...
function throwHomeStationValidation(errors: any[] = []) {
  throwWrappedObject(
