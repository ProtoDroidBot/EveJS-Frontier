...
const { adjustCharacterBalance, } = require("../account/walletState");
const { readRuntimeAccount, writeRuntimeAccount, } = require("../newEdenStore/storeState");
...
function normalizeProgress(value) {
    ...
    return {
        onlineMilliseconds: Math.min(MILESTONE_DURATION_MS, Math.max(0, Math.trunc(Number(source.onlineMilliseconds) || 0))),
...
function currentProgress(accountID) {
    ...
    return {
        ...progress,
        onlineMilliseconds: Math.min(MILESTONE_DURATION_MS, progress.onlineMilliseconds + Math.max(0, nowMilliseconds() - active.startedAt)),
    };
}
...
function startMilestoneTimer(accountID, session) {
    ...
    if (priorActive && !progress.achieved && !progress.claimed) {
        progress = writeProgress(accountID, {
            ...progress,
            onlineMilliseconds: Math.min(MILESTONE_DURATION_MS, progress.onlineMilliseconds + Math.max(0, nowMilliseconds() - priorActive.startedAt)),
        });
    }
    if (progress.claimed) {
        return;
...
class MilestoneMgrService extends BaseService {
    ...
    Handle_ClaimRewards(args, session) {
        ...
        log.debug(`[MilestoneMgr] ClaimRewards milestone=${milestoneID} char=${characterID}`);
        ...
        if (!rewardResult.success) {
            ...
            log.warn(`[MilestoneMgr] ClaimRewards failed char=${characterID}: ${rewardResult.errorMsg}`);
            return null;
        }
        clearActiveMilestone(accountID);
        log.info(`[MilestoneMgr] ClaimRewards milestone=${MILESTONE_ID} char=${characterID} isk=${MILESTONE_REWARD_ISK}`);
        return null;
    }
}
...
//# sourceMappingURL=milestoneMgrService.js.map