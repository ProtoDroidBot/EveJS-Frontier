...
const { buildExpertSystemsPayload, } = require(path.join(__dirname, "../skills/expertSystems/expertSystemSerializer"));
const { consumeExpertSystemItem, removeExpertSystemFromCharacter, } = require(path.join(__dirname, "../skills/expertSystems/expertSystemRuntime"));
...
class ExpertSystemMgrService extends BaseService {
    ...
    Handle_GetMyExpertSystems(args, session) {
        ...
        log.debug(`[ExpertSystemMgr] GetMyExpertSystems(charID=${characterID}) -> ${payload && Array.isArray(payload.entries) ? payload.entries.length : 0}`);
        return payload;
    }
    Handle_ConsumeExpertSystem(args, session) {
        const characterID = resolveSessionCharacterID(session);
        const itemID = normalizePositiveInteger(args && args[0]);
        log.debug(`[ExpertSystemMgr] ConsumeExpertSystem(charID=${characterID}, itemID=${itemID})`);
    ...
    Handle_RemoveMyExpertSystem(args, session) {
        ...
        log.debug(`[ExpertSystemMgr] RemoveMyExpertSystem(charID=${characterID}, typeID=${expertSystemTypeID})`);
...
//# sourceMappingURL=expertSystemMgrService.js.map