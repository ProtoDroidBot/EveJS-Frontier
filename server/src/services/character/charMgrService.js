...
const { getCharacterRecord, resolveHomeStationInfo, updateCharacterRecord, } = require(path.join(__dirname, "./characterState"));
const { resolvePaperDollState, } = require(path.join(__dirname, "./paperDollPayloads"));
const { normalizeCharacterGender, } = require(path.join(__dirname, "./characterIdentity"));
const { buildSettingsInfoCode, } = require(path.join(__dirname, "./settingsInfoCompatibility"));
...
const { buildList, buildDict, extractList, buildFiletimeLong, buildKeyVal, buildRow, buildRowset, extractDictEntries, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
function sessionCharacterID(session) {
    return Number(session &&
        (session.characterID || session.charID || session.charid || 0)) || 0;
}
...
function normalizePersonalContacts(record = {}) {
    const source = record.personalContacts && typeof record.personalContacts === "object"
        ? record.personalContacts
        : {};
    const contacts = {};
    for (const [rawContactID, rawContact] of Object.entries(source)) {
        const contactID = normalizeInteger(rawContact && rawContact.contactID !== undefined
...
function normalizePersonalContactLabels(record = {}) {
    const source = record.personalContactLabels && typeof record.personalContactLabels === "object"
...
function normalizeBlockedOwners(record = {}) {
    const source = record.blockedOwners && typeof record.blockedOwners === "object"
        ? record.blockedOwners
        : {};
    const blocked = {};
    for (const [rawOwnerID, rawBlocked] of Object.entries(source)) {
        const ownerID = normalizeInteger(rawBlocked && rawBlocked.senderID !== undefined
            ? rawBlocked.senderID
...
function buildBlockedRow(blocked) {
    const ownerID = normalizeInteger(blocked && (blocked.senderID ?? blocked.contactID), 0);
    return [ownerID];
}
function upsertPersonalContact(characterID, contactID, options = {}) {
    ...
    return updateCharacterRecord(characterID, (record) => {
        ...
        contacts[String(normalizedContactID)] = {
            contactID: normalizedContactID,
            inWatchlist: options.inWatchlist !== undefined
                ? normalizeBoolean(options.inWatchlist, false)
                : normalizeBoolean(existing.inWatchlist, false),
            relationshipID: options.relationshipID !== undefined
...
function resolveCorporationChangeInfo(charId, charData, session) {
    const corporationID = Number(charData && charData.corporationID) ||
        Number(session && (session.corporationID || session.corpid)) ||
        0;
    const corporationMember = corporationID > 0 ? getCorporationMember(corporationID, charId) : null;
    const corporationDateTime = (corporationMember && corporationMember.startDate) ||
        (charData &&
            (charData.startDateTime ||
                (Array.isArray(charData.employmentHistory)
                    ? charData.employmentHistory.find((entry) => Number(entry && entry.corporationID) === Number(corporationID))?.startDate
                    : null) ||
...
function buildCloneEntries(entries = [], valueBuilder) {
    return buildDict(entries.map((entry, index) => [
...
class CharMgrService extends BaseService {
    ...
    async Handle_MachoBindObject(args, session, kwargs) {
        const result = await this._globalAssets.Handle_MachoBindObject(args, session, kwargs, async (boundObjectID, methodName, callArgs, callKwargs) => {
    ...
    Handle_GetRecentShipKillsAndLosses(args, session) {
        const charId = (session && Number(session.currentBoundObjectID || 0)) ||
    ...
    Handle_GetPrivateInfoOnCorpChange(args, session) {
        ...
        return buildCachedMethodCallResult(resolveCorporationChangeInfo(charId, charData, session), {
    ...
    Handle_GetCloneInfo(args, session) {
        ...
        return buildKeyVal([
            ...
            [
                "clones",
                buildCloneEntries(charData.jumpClones || [], (entry) => buildKeyVal([
            ...
            [
                "implants",
                buildCloneEntries(charData.implants || [], (entry) => buildKeyVal([
    ...
    Handle_GetCharacterSettings(args, session) {
        ...
        return buildDict(Object.entries(getCharacterSettings(characterID)).sort(([left], [right]) => (String(left).localeCompare(String(right)))));
    }
    ...
    Handle_GetContactList(args, session) {
        ...
        const characterID = sessionCharacterID(session);
        const character = getCharacterRecord(characterID) || {};
        const contacts = Object.values(normalizePersonalContacts(character)).sort((left, right) => Number(left.contactID) - Number(right.contactID));
        const blockedOwners = Object.values(normalizeBlockedOwners(character)).sort((left, right) => Number(left.senderID) - Number(right.senderID));
        return buildKeyVal([
            [
                "addresses",
                buildRowset(["contactID", "inWatchlist", "relationshipID", "labelMask"], contacts.map((contact) => buildContactRow(contact)), "eve.common.script.sys.rowset.Rowset"),
            ],
            [
                "blocked",
                buildRowset(["senderID"], blockedOwners.map((blocked) => buildBlockedRow(blocked)), "eve.common.script.sys.rowset.Rowset"),
            ],
        ]);
    }
    Handle_GetLabels(args, session) {
        ...
        return buildDict(Object.entries(labels).map(([labelID, label]) => [
    ...
    Handle_CreateLabel(args, session) {
        ...
        updateCharacterRecord(characterID, (record) => {
            ...
            const meta = record.personalContactLabelMeta &&
        ...
        updateCharacterRecord(characterID, (record) => {
            ...
            for (const rawContactID of contactIDs) {
                ...
                if (contacts[String(contactID)]) {
                    contacts[String(contactID)].labelMask = removeLabelMask(contacts[String(contactID)].labelMask, labelMask);
                }
            }
            record.personalContacts = contacts;
            return record;
    ...
    Handle_AddContact(args, session) {
        ...
        const relationshipID = args && args.length > 1 ? normalizeInteger(args[1], 0) : 0;
        const inWatchlist = args && args.length > 2 ? normalizeBoolean(args[2], false) : false;
    ...
    Handle_EditContactsRelationshipID(args, session) {
        ...
        log.debug(`[CharMgr] EditContactsRelationshipID(${characterID}, ${contactIDs.length}, ${relationshipID})`);
    ...
    Handle_SetActivityStatus(args, session, kwargs) {
        const characterID = sessionCharacterID(session);
        const status = normalizeInteger(extractKwarg(kwargs, "status") ?? (args && args[0]), 0);
        const extraInfo = unwrapMarshalValue(extractKwarg(kwargs, "extraInfo") ?? (args && args[1]) ?? null);
        log.debug(`[CharMgr] SetActivityStatus(${characterID}, ${status})`);
        updateCharacterRecord(characterID, (record) => ({
            ...record,
            activityStatus: status,
            activityStatusExtraInfo: extraInfo === undefined || extraInfo === null ? null : String(extraInfo),
        }));
        return null;
    }
    Handle_LogSettings(args, session) {
        ...
        updateCharacterRecord(characterID, (record) => ({
            ...record,
            lastSettingsTelemetry: settings && typeof settings === "object" ? settings : {},
        }));
        return null;
    }
    Handle_GetOwnerNoteLabels(args, session) {
        ...
        return buildRowset(["noteID", "label"], listOwnerNotes(characterID).map((entry) => [
...
//# sourceMappingURL=charMgrService.js.map