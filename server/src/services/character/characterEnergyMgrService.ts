...
function normalizeCharacterEnergyState(rawState: Record<string, any> = {}) {
...
function buildCharacterEnergyStatePayload(rawState: Record<string, any> = {}) {
...
function calculateCharacterEnergyStateAtFileTime(rawState: Record<string, any> = {}, filetime = currentFileTime()) {
  const state = normalizeCharacterEnergyState(rawState);
...
function getCharacterEnergyStateNow(session, options: Record<string, any> = {}) {
...
function spendCharacterEnergy(session, rawAmount, options: Record<string, any> = {}) {
  const characterID = resolveCharacterID(session);
...
class CharacterEnergyMgrService extends BaseService {
  declare _stateByCharacterID: any;
