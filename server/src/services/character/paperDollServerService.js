...
const { buildList, buildDict, buildPackedRow, extractList, unwrapMarshalValue, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
const { clonePaperDollPayload, getStoredAppearanceInfo, getStoredPortraitDataForClient, } = require(path.join(__dirname, "./paperDollPayloads"));
...
function buildPortraitPayloadTuple(record) {
    ...
    return [
        buildList(portraitInfo ? [buildPackedRow(PORTRAIT_DBROW_COLUMNS, portraitInfo)] : []),
        null,
    ];
}
...
class PaperDollServerService extends BaseService {
    ...
    Handle_GetPaperDollData(args, session) {
        const charId = normalizeNumber(args && args.length > 0 ? args[0] : session ? session.charid : 0, 0);
    ...
    Handle_GetPaperDollDataBatch(args) {
        ...
        return buildDict(charIds.map((charId) => [
            charId,
    ...
    Handle_UpdateExistingCharacterFull(args, session, kwargs) {
        ...
        if (!isOwnedCharacter(charId, session)) {
            log.warn(`[PaperDollServer] UpdateExistingCharacterFull rejected for char=${charId}`);
            return null;
        }
        ...
        log.info(`[PaperDollServer] UpdateExistingCharacterFull(${charId}) dollExists=${dollExists}`);
        const updateResult = persistPaperDollCharacter(charId, appearanceInfo, portraitInfo, dollExists);
        if (!updateResult.success) {
            log.warn(`[PaperDollServer] UpdateExistingCharacterFull failed for ${charId}: ${updateResult.errorMsg}`);
        }
        return null;
    }
    Handle_UpdateExistingCharacterLimited(args, session, kwargs) {
        const charId = normalizeNumber(args && args.length > 0 ? args[0] : 0, 0);
        if (!isOwnedCharacter(charId, session)) {
            log.warn(`[PaperDollServer] UpdateExistingCharacterLimited rejected for char=${charId}`);
            return null;
        }
        const appearanceInfo = clonePaperDollPayload(args && args.length > 1 ? args[1] : null);
        const portraitInfo = clonePaperDollPayload(args && args.length > 2 ? args[2] : null);
        const dollExists = normalizeBoolean(args && args.length > 3 ? args[3] : true, true);
        log.info(`[PaperDollServer] UpdateExistingCharacterLimited(${charId}) dollExists=${dollExists}`);
        const updateResult = persistPaperDollCharacter(charId, appearanceInfo, portraitInfo, dollExists);
        if (!updateResult.success) {
            log.warn(`[PaperDollServer] UpdateExistingCharacterLimited failed for ${charId}: ${updateResult.errorMsg}`);
        }
        return null;
    }
}
module.exports = PaperDollServerService;
//# sourceMappingURL=paperDollServerService.js.map