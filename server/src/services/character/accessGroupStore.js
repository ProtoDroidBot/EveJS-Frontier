...
function createGroup(creatorID, name, description) {
    ...
    writeGroup(group);
    log.info(`[AccessGroupStore] Created group ${groupID} "${name}" creator=${creatorID}`);
    return group;
}
/**
...
function addMembers(groupID, membershipTypeByMemberID) {
    ...
    writeGroup(group);
    log.info(`[AccessGroupStore] Added members to group=${groupID}: ${Object.keys(membershipTypeByMemberID).join(",")}`);
    return failed;
}
/**
...
function deleteMembers(groupID, memberIDs) {
    ...
    group.members = group.members.filter((m) => !removeSet.has(Number(m.memberID)));
    writeGroup(group);
    log.info(`[AccessGroupStore] Removed members from group=${groupID}: ${memberIDs.join(",")}`);
    return failed;
}
/**
...
//# sourceMappingURL=accessGroupStore.js.map