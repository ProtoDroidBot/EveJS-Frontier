...
const { buildWeaponDogmaAttributeOverrides, } = require(path.join(__dirname, "../../space/combat/weaponDogma"));
const { peekShipFittingSnapshot, refreshShipFittingSnapshot, listShipFittingAttributeChanges, } = require(path.join(__dirname, "../../_secondary/fitting/fittingRuntime"));
const { resolveModuleParityFamily, } = require(path.join(__dirname, "../../space/modules/moduleClientParityAuthority"));
const { resolveItemByTypeID, } = require(path.join(__dirname, "../inventory/itemTypeRegistry"));
const { getFactionIDForCorporation, } = require(path.join(__dirname, "../faction/factionState"));
const { getDockedLocationKind, isDockedSession, } = require(path.join(__dirname, "../structure/structureLocation"));
const { getSessionFleetState, } = require(path.join(__dirname, "../fleets/fleetHelpers"));
const { normalizeCharacterGender, } = require(path.join(__dirname, "./characterIdentity"));
const { getCharacterCreationSchool, resolveCharacterCreationSchoolIDForRace, resolveCharacterCreationSchoolProfile, } = require(path.join(__dirname, "./characterCreationData"));
...
function summarizeMissileDebugItem(item) {
    ...
    return {
        ...
        moduleState: item.moduleState && typeof item.moduleState === "object"
            ? {
...
function summarizeMissileDogmaSnapshot(snapshot) {
    ...
    const chargeAttributes = snapshot.chargeAttributes && typeof snapshot.chargeAttributes === "object"
        ? snapshot.chargeAttributes
        : {};
    const moduleAttributes = snapshot.moduleAttributes && typeof snapshot.moduleAttributes === "object"
    ...
    return {
        ...
        rawShotDamage: snapshot.rawShotDamage && typeof snapshot.rawShotDamage === "object"
            ? {
        ...
        relevantChargeAttributes: {
            explosionDelay: roundMissileTraceNumber(chargeAttributes[ATTRIBUTE_EXPLOSION_DELAY], 6),
            maxVelocity: roundMissileTraceNumber(chargeAttributes[ATTRIBUTE_MAX_VELOCITY], 6),
            detonationRange: roundMissileTraceNumber(chargeAttributes[ATTRIBUTE_DETONATION_RANGE], 6),
        },
        moduleAttributes,
        chargeAttributes,
        shipModifierAttributes: snapshot.shipModifierAttributes && typeof snapshot.shipModifierAttributes === "object"
            ? snapshot.shipModifierAttributes
            : {},
        characterAttributes: snapshot.characterAttributes && typeof snapshot.characterAttributes === "object"
...
function resolveSystemIdentity(solarSystemID, fallback = {}) {
    ...
    return {
        constellationID: Number((system && system.constellationID) || fallback.constellationID || 0) ||
            20000020,
        regionID: Number((system && system.regionID) || fallback.regionID || 0) ||
            10000002,
    };
}
...
function resolveValidCharacterSchoolID(record = {}, fallback = 11) {
    ...
    return resolveCharacterCreationSchoolIDForRace(requestedSchoolID, raceID, fallback);
}
function buildDefaultEmploymentHistory(record = {}) {
    const createdAt = String(record.startDateTime || record.createDateTime || "132000000000000000");
    const schoolProfile = resolveCharacterCreationSchoolProfile(record.schoolID, {
        raceID: record.raceID,
        corporationID: record.corporationID,
    });
    const schoolCorpID = Number(schoolProfile.corporationID || record.corporationID || 1000009) ||
        1000009;
...
function normalizeEmploymentHistory(record = {}) {
    ...
    const normalized = source
        ...
        startDate: String((entry && (entry.startDate || entry.startDateTime)) ||
...
function getCurrentCorporationStartDate(record = {}, employmentHistory = null) {
    ...
    const currentEntry = history
        .filter((entry) => (Number(entry && entry.corporationID) || 0) === currentCorporationID)
        .sort((left, right) => String(left.startDate).localeCompare(String(right.startDate)))
        .pop();
    return String((currentEntry && currentEntry.startDate) ||
        record.startDateTime ||
        record.createDateTime ||
        "132000000000000000");
}
function buildDefaultStandingData(charId, record = {}) {
    ...
    return {
        char: npcRows.filter((entry) => entry.fromID === characterID || entry.toID === characterID),
        corp: npcRows.filter((entry) => entry.fromID === corporationID || entry.toID === corporationID),
        npc: npcRows,
    };
}
...
function normalizeStandingRows(rows = [], fallbackRows = []) {
    ...
    return source
        .map((entry) => ({
        fromID: normalizeStandingOwnerID(entry && Object.prototype.hasOwnProperty.call(entry, "fromID")
            ? entry.fromID
            : null),
        toID: normalizeStandingOwnerID(entry && Object.prototype.hasOwnProperty.call(entry, "toID")
            ? entry.toID
            : null),
        standing: Number(entry && entry.standing) || 0.0,
    }))
        .filter((entry) => entry.fromID !== null &&
        entry.toID !== null &&
        entry.fromID !== entry.toID);
}
function normalizeStandingData(charId, record = {}) {
    const fallback = buildDefaultStandingData(charId, record);
    const source = record.standingData && typeof record.standingData === "object"
        ? record.standingData
        : {};
    return {
...
function normalizeCharacterAttributes(record = {}) {
    const source = record.characterAttributes && typeof record.characterAttributes === "object"
        ? record.characterAttributes
        : {};
    return {
        charisma: normalizeInteger(source.charisma ?? source[164], DEFAULT_CHARACTER_ATTRIBUTES.charisma),
        intelligence: normalizeInteger(source.intelligence ?? source[165], DEFAULT_CHARACTER_ATTRIBUTES.intelligence),
        memory: normalizeInteger(source.memory ?? source[166], DEFAULT_CHARACTER_ATTRIBUTES.memory),
        perception: normalizeInteger(source.perception ?? source[167], DEFAULT_CHARACTER_ATTRIBUTES.perception),
        willpower: normalizeInteger(source.willpower ?? source[168], DEFAULT_CHARACTER_ATTRIBUTES.willpower),
    };
}
function normalizeRespecInfo(record = {}) {
    const source = record.respecInfo && typeof record.respecInfo === "object"
        ? record.respecInfo
        : DEFAULT_RESPEC_INFO;
    return {
        freeRespecs: normalizeInteger(source.freeRespecs, DEFAULT_RESPEC_INFO.freeRespecs),
...
function resolveHomeStationInfo(charData = {}, session = null) {
    const authoritativeHomeStationID = Number(charData.homeStationID || charData.cloneStationID || 0) || 0;
    const fallbackHomeStationID = Number(charData.stationID ||
    ...
    return {
        homeStationID,
        cloneStationID: Number(charData.cloneStationID || authoritativeHomeStationID || homeStationID) ||
            homeStationID,
...
function reconcileCharacterLocationFromActiveShip(charId, record = {}, activeShip = null) {
    ...
    if (shipFlagID === ITEM_FLAGS.HANGAR && shipLocationID) {
        ...
        const inferredSolarSystemID = shipSpaceSystemID ||
    ...
    if (repairedStationID === currentStationID &&
    ...
    if (!characterLocationReconcileWarnings.has(warningKey)) {
        ...
        log.warn(`[CharacterState] Reconciled location from active ship for char=${charId} ship=${activeShip.itemID} station=${currentStationID}=>${nextRecord.stationID} structure=${currentStructureID}=>${nextRecord.structureID} system=${currentSolarSystemID}=>${nextRecord.solarSystemID}`);
    }
    return nextRecord;
}
function normalizeCharacterRecord(charId, record) {
    ...
    if (activeShip) {
        ...
        Object.assign(normalized, reconcileCharacterLocationFromActiveShip(charId, normalized, activeShip));
    }
    ...
    normalized.plexBalance = normalizeInteger(normalized.plexBalance, DEFAULT_PLEX_BALANCE);
    normalized.schoolID = resolveValidCharacterSchoolID(normalized, 11);
    normalized.securityStatus = Number(normalized.securityStatus ?? normalized.securityRating ?? 0);
    ...
    normalized.startDateTime = getCurrentCorporationStartDate(normalized, normalized.employmentHistory);
...
function updateCharacterRecord(charId, updater) {
    ...
    const updatedRecord = typeof updater === "function" ? updater(cloneValue(currentRecord)) : updater;
...
function buildInventoryItemRow(item) {
    ...
    const normalizedQuantity = item && item.quantity !== undefined && item.quantity !== null
        ? item.quantity
        : singleton === 2
            ? -2
            : singleton === 1
                ? -1
                : normalizedStacksize;
    const customInfo = item && Object.prototype.hasOwnProperty.call(item, "clientCustomInfo")
...
function buildChargeSublocationRow(item) {
    ...
    const normalizedChargeQuantity = Math.max(0, Number(normalizedItem && (normalizedItem.stacksize ?? normalizedItem.quantity ?? 0)) || 0);
...
function normalizeChargeSublocationItem(item) {
    ...
    const normalizedQuantity = Math.max(0, Number(item.stacksize ?? item.quantity ?? 0) || 0);
    ...
    return {
        ...
        customInfo: item.customInfo !== undefined && item.customInfo !== null
            ? String(item.customInfo)
            : "",
    };
}
...
function buildChargeDogmaPrimeAttributes(item, options = {}) {
    const normalizedChargeQuantity = Math.max(0, Number(item && (item.stacksize ?? item.quantity ?? 0)) || 0);
    const attributes = Object.fromEntries(Object.entries(getTypeDogmaAttributes(Number(item && item.typeID) || 0))
        .map(([attributeID, value]) => [Number(attributeID), Number(value)])
        .filter(([attributeID, value]) => Number.isInteger(attributeID) && Number.isFinite(value)));
    Object.assign(attributes, normalizeDogmaNumericAttributeMap(options.attributeOverrides));
...
function buildChargeDogmaPrimeEntry(item, options = {}) {
    ...
    const now = typeof options.now === "bigint"
        ? options.now
        : currentFileTime();
...
function buildInventoryDogmaPrimeEntry(item, options = {}) {
    const now = typeof options.now === "bigint"
        ? options.now
        : currentFileTime();
    const primeItem = Number(item && item.singleton) > 0
        ? {
            ...item,
            quantity: null,
        ...
        : item;
    const attributeOverrides = normalizeDogmaNumericAttributeMap(options.attributeOverrides || options.attributes || {});
    const attributes = {
        ...(options.includeTypeAttributes === true
            ? normalizeDogmaNumericAttributeMap(getTypeDogmaAttributes(Number(primeItem && primeItem.typeID) || 0))
            : {}),
        ...attributeOverrides,
    };
    if (options.includeTypeAttributes === true) {
        attributes[ATTRIBUTE_QUANTITY] = Math.max(1, Number(primeItem && (primeItem.stacksize ?? primeItem.quantity)) || 1);
    }
    return {
        type: "object",
        name: "util.KeyVal",
...
function syncChargeGodmaPrimeForSession(session, locationID, item, options = {}) {
    if (!session ||
...
function buildModuleAttributeChangePayload(session, itemID, attributeID, value, oldValue = null) {
    const when = session && session._space && typeof session._space.simFileTime === "bigint"
...
function syncShipFittingAttributeChangesForSession(session, shipID, previousSnapshot, nextSnapshot) {
    if (!session ||
        typeof session.sendNotification !== "function" ||
        !previousSnapshot ||
        !nextSnapshot) {
    ...
    const changes = listShipFittingAttributeChanges(previousSnapshot, nextSnapshot).map((change) => buildModuleAttributeChangePayload(session, numericShipID, change.attributeID, change.nextValue, change.previousValue));
...
function buildScannerProbeLauncherBootstrapChanges(session, moduleItem) {
    ...
    const shipItem = getActiveShipRecord(charID) ||
        ...
        null;
    const chargeItem = getLoadedChargeItems(charID, shipID).find((candidate) => Number(candidate && candidate.flagID) === Number(moduleItem.flagID)) || null;
    const spaceRuntime = require(path.join(__dirname, "../../space/runtime"));
    const runtimeAttributes = shipItem && spaceRuntime && typeof spaceRuntime.getGenericModuleRuntimeAttributes === "function"
        ? spaceRuntime.getGenericModuleRuntimeAttributes(charID, shipItem, moduleItem, chargeItem, null, {})
        : null;
    const moduleAttributes = buildEffectiveItemAttributeMap(moduleItem);
    const changes = [];
    const capacitorNeed = Number(runtimeAttributes && Number.isFinite(Number(runtimeAttributes.capNeed))
        ? runtimeAttributes.capNeed
        : moduleAttributes[MODULE_ATTRIBUTE_CAPACITOR_NEED]);
    if (Number.isFinite(capacitorNeed)) {
        changes.push(buildModuleAttributeChangePayload(session, Number(moduleItem.itemID) || 0, MODULE_ATTRIBUTE_CAPACITOR_NEED, capacitorNeed, 0));
    }
    const resolvedSpeed = Number(runtimeAttributes &&
    ...
    if (Number.isFinite(resolvedSpeed) && resolvedSpeed > 0) {
        changes.push(buildModuleAttributeChangePayload(session, Number(moduleItem.itemID) || 0, MODULE_ATTRIBUTE_SPEED, resolvedSpeed, 0));
    }
    const resolvedDuration = Number(runtimeAttributes && Number.isFinite(Number(runtimeAttributes.durationMs))
        ? runtimeAttributes.durationMs
        : moduleAttributes[MODULE_ATTRIBUTE_DURATION]);
    if (Number.isFinite(resolvedDuration) && resolvedDuration > 0) {
        changes.push(buildModuleAttributeChangePayload(session, Number(moduleItem.itemID) || 0, MODULE_ATTRIBUTE_DURATION, resolvedDuration, 0));
    }
    const reloadTime = Number(moduleAttributes[ATTRIBUTE_RELOAD_TIME]);
    if (Number.isFinite(reloadTime) && reloadTime > 0) {
        changes.push(buildModuleAttributeChangePayload(session, Number(moduleItem.itemID) || 0, ATTRIBUTE_RELOAD_TIME, reloadTime, 0));
    }
    return changes;
}
function buildLocationChangePayload(session, item) {
    const solarSystemID = Number((session && (session.solarsystemid2 || session.solarsystemid)) || 0) || null;
    const fallbackName = Number(item && item.categoryID) === 6
...
function buildItemChangePayload(item, previousState = {}) {
    ...
    const prefersStackSizeOnly = Number(item && item.singleton) !== 1 &&
...
function buildChargeSublocationChangePayload(item, previousState = {}) {
    const entries = [];
    const currentStackSize = Math.max(0, Number(item && (item.stacksize ?? item.quantity ?? 0)) || 0);
    const previousStackSize = Math.max(0, Number(previousState && (previousState.stacksize ?? previousState.quantity ?? 0)) || 0);
    if (previousState.locationID !== undefined &&
        previousState.locationID !== item.locationID) {
        entries.push([INV_UPDATE_LOCATION, previousState.locationID]);
    }
    ...
    if ((previousState.stacksize !== undefined || previousState.quantity !== undefined) &&
        Number.isFinite(previousStackSize) &&
        Number.isFinite(currentStackSize) &&
        previousStackSize >= 0 &&
        currentStackSize >= 0 &&
...
function buildChargeSublocationItem({ shipID, flagID, typeID, quantity, ownerID = null, groupID = null, categoryID = null, }) {
    const numericShipID = Number(shipID) || 0;
...
function buildChargeSublocationRepairPreviousState({ previousTypeID = 0, previousQuantity = 0, nextTypeID = 0, nextQuantity = 0, } = {}) {
    const normalizedPreviousTypeID = Number(previousTypeID) || 0;
    const normalizedNextTypeID = Number(nextTypeID) || 0;
    const normalizedPreviousQuantity = Math.max(0, Number(previousQuantity) || 0);
    ...
    // contract only.
    const previousStackSize = normalizedPreviousTypeID > 0 &&
...
function buildStableChargeSublocationQuantityPreviousState({ shipID = 0, flagID = 0, previousQuantity = 0, nextQuantity = 0, } = {}) {
    const numericShipID = Number(shipID) || 0;
    const numericFlagID = Number(flagID) || 0;
    const normalizedPreviousQuantity = Math.max(0, Number(previousQuantity) || 0);
    const normalizedNextQuantity = Math.max(0, Number(nextQuantity) || 0);
...
function syncChargeSublocationForSession(session, item, previousState = {}) {
    if (!session ||
        typeof session.sendNotification !== "function" ||
        !item ||
        typeof item !== "object") {
    ...
    const normalizedItem = normalizeChargeSublocationItem(item);
    session.sendNotification("OnItemChange", "clientID", buildChargeSublocationChangePayload(normalizedItem, previousState));
}
...
function buildDamageStateAttributeChangePayloads(session, item, previousState = {}) {
    if (!item || typeof item !== "object") {
        return [];
    }
    ...
    const shieldCapacity = Math.max(0, toFiniteNumber(getTypeAttributeValue(typeID, "shieldCapacity"), 0));
    const armorHP = Math.max(0, toFiniteNumber(getTypeAttributeValue(typeID, "armorHP"), 0));
    const structureHP = Math.max(0, toFiniteNumber(getTypeAttributeValue(typeID, "hp", "structureHP"), 0));
    const nextState = item.conditionState && typeof item.conditionState === "object"
        ? item.conditionState
        : item.moduleState && typeof item.moduleState === "object"
            ? item.moduleState
            : {};
    const previousConditionState = previousState && previousState.conditionState &&
        ...
        : previousState && previousState.moduleState &&
            ...
            : {};
    const nextShieldCharge = roundMissileTraceNumber(shieldCapacity * clamp01(nextState.shieldCharge, shieldCapacity > 0 ? 1 : 0));
    const previousShieldCharge = roundMissileTraceNumber(shieldCapacity *
        clamp01(previousConditionState.shieldCharge, shieldCapacity > 0 ? 1 : 0));
    const nextArmorDamage = roundMissileTraceNumber(armorHP * clamp01(nextState.armorDamage, 0));
    const previousArmorDamage = roundMissileTraceNumber(armorHP * clamp01(previousConditionState.armorDamage, 0));
    const nextHullDamage = roundMissileTraceNumber(structureHP * clamp01(nextState.damage, 0));
    const previousHullDamage = roundMissileTraceNumber(structureHP * clamp01(previousConditionState.damage, 0));
    const changes = [];
    if (nextShieldCharge !== previousShieldCharge) {
        changes.push(buildModuleAttributeChangePayload(session, itemID, ATTRIBUTE_SHIELD_CHARGE, nextShieldCharge, previousShieldCharge));
    }
    if (nextArmorDamage !== previousArmorDamage) {
        changes.push(buildModuleAttributeChangePayload(session, itemID, ATTRIBUTE_ARMOR_DAMAGE, nextArmorDamage, previousArmorDamage));
    }
    if (nextHullDamage !== previousHullDamage) {
        changes.push(buildModuleAttributeChangePayload(session, itemID, ATTRIBUTE_DAMAGE, nextHullDamage, previousHullDamage));
    }
    return changes;
}
function syncDamageStateAttributesForSession(session, item, previousState = {}) {
    return syncModuleAttributeChangesForSession(session, buildDamageStateAttributeChangePayloads(session, item, previousState));
}
function resolveCurrentChargeTupleSyncItem({ session = null, charID = 0, shipID = 0, flagID = 0, typeID = 0, quantity = null, ownerID = null, groupID = null, categoryID = null, } = {}) {
    ...
    const currentCharge = getLoadedChargeByFlag(numericCharID, numericShipID, numericFlagID);
    if (!currentCharge) {
        return null;
    }
    const currentTypeID = Number(currentCharge.typeID) || 0;
    const currentQuantity = Math.max(0, Number(currentCharge.stacksize ?? currentCharge.quantity ?? 0) || 0);
...
function syncChargeSublocationForSessionAfterDelay(session, item, previousState = {}, delayMs = 0, options = {}) {
    const afterSync = options && typeof options.afterSync === "function"
        ? options.afterSync
        : null;
    const validateBeforeSync = options && typeof options.validateBeforeSync === "function"
    ...
    const timerKey = buildChargeSublocationSyncTimerKey(item && item.locationID, item && item.flagID);
...
function syncChargeSublocationTransitionForSession(session, { shipID, flagID, ownerID = null, previousState = null, nextState = null, nextChargeItem = null, primeNextCharge = false, forceRepair = false, forcePrimeNextCharge = false, nextChargeRepairDelayMs = CHARGE_BOOTSTRAP_REPAIR_DELAY_MS, afterNextChargeSync = null, } = {}) {
    ...
    const afterNextChargeSyncCallback = typeof afterNextChargeSync === "function" ? afterNextChargeSync : null;
    const shouldForceRepair = forceRepair === true &&
        nextTypeID > 0 &&
        nextQuantity > 0;
    ...
    if (previousTypeID > 0 &&
        ...
        syncChargeSublocationForSession(session, nextCharge, buildStableChargeSublocationQuantityPreviousState({
            shipID: numericShipID,
            flagID: numericFlagID,
            previousQuantity,
            nextQuantity,
        ...
        log.debug(`[charge-transition] shipID=${numericShipID} ` +
            `flagID=${numericFlagID} typeID=${nextTypeID} quantity=${nextQuantity} ` +
            `itemID=${JSON.stringify(buildChargeTupleItemID(numericShipID, numericFlagID, nextTypeID))} mode=item-change-stable-quantity`);
        return;
    }
    ...
    if (nextTypeID > 0 && nextQuantity > 0) {
        ...
        const allowTupleGodmaPrime = !session ||
            isDockedSession(session) !== true;
        const shouldPrimeNextCharge = allowTupleGodmaPrime &&
        ...
        if (shouldPrimeNextCharge) {
            ...
            log.debug(`[charge-transition] shipID=${numericShipID} ` +
                `flagID=${numericFlagID} typeID=${nextTypeID} quantity=${nextQuantity} ` +
                `itemID=${JSON.stringify(buildChargeTupleItemID(numericShipID, numericFlagID, nextTypeID))} mode=godma-prime`);
        }
        const finalizeDelayMs = Math.max(0, Number(CHARGE_TRANSITION_FINALIZE_DELAY_MS) || 0);
        const shouldSendPostRepairQuantityBootstrap = shouldPrimeNextCharge &&
            previousTypeID !== nextTypeID &&
            nextTypeID > 0 &&
            nextQuantity > 0;
        ...
        const scheduled = syncChargeSublocationForSessionAfterDelay(session, nextCharge, shouldForceRepair
            ...
            afterSync: shouldSendPostRepairQuantityBootstrap || afterNextChargeSyncCallback
                ? () => {
                    ...
                    if (shouldSendPostRepairQuantityBootstrap) {
                        ...
                        syncModuleAttributeChangesForSession(session, [
                            buildModuleAttributeChangePayload(session, validatedNextChargeSync.chargeBootstrapItem.itemID, ATTRIBUTE_QUANTITY, validatedNextChargeSync.quantity, 0),
                        ]);
                        log.debug(`[charge-transition] shipID=${numericShipID} ` +
                            `flagID=${numericFlagID} typeID=${validatedNextChargeSync.typeID} quantity=${validatedNextChargeSync.quantity} ` +
                            `itemID=${JSON.stringify(buildChargeTupleItemID(numericShipID, numericFlagID, validatedNextChargeSync.typeID))} mode=post-item-change-quantity`);
                    }
                    if (afterNextChargeSyncCallback) {
                        afterNextChargeSyncCallback();
                    }
        ...
        log.debug(`[charge-transition] shipID=${numericShipID} ` +
            `flagID=${numericFlagID} typeID=${nextTypeID} quantity=${nextQuantity} ` +
            `itemID=${JSON.stringify(buildChargeTupleItemID(numericShipID, numericFlagID, nextTypeID))} mode=${shouldPrimeNextCharge
                ? scheduled
                    ? shouldForceRepair
                ...
                : shouldForceRepair
                    ? scheduled
                        ...
                        : "forced-item-change"
                    : "item-change"}${scheduled ? ` delayMs=${repairDelayMs}` : ""}`);
        if (!shouldPrimeNextCharge && previousTypeID !== nextTypeID) {
            const finalizeScheduled = syncChargeSublocationForSessionAfterDelay(session, nextCharge, buildChargeSublocationRepairPreviousState({
                previousTypeID,
                previousQuantity,
                nextTypeID,
                nextQuantity,
            ...
            if (finalizeScheduled) {
                log.debug(`[charge-transition] shipID=${numericShipID} ` +
                    `flagID=${numericFlagID} typeID=${nextTypeID} quantity=${nextQuantity} ` +
                    `itemID=${JSON.stringify(buildChargeTupleItemID(numericShipID, numericFlagID, nextTypeID))} mode=item-change-finalize-delayed ` +
...
function syncInventoryItemForSession(session, item, previousState = {}, options = {}) {
    ...
    if (isTrainedSkillInventoryRecord(item)) {
        log.debug(`[CharacterState] Suppressed trained-skill inventory sync for item ${Number(item.itemID) || 0} (${item.itemName || item.typeID})`);
        return;
    }
    session.sendNotification("OnItemChange", "clientID", buildItemChangePayload(item, previousState));
    ...
    log.info(`[CharacterState] Synced inventory item ${item.itemID} (${item.itemName || item.typeID}) to client inventory`);
}
...
function emitItemsChangedBatchForSession(session, changes = [], options = {}) {
    ...
    const locationContext = options && Object.prototype.hasOwnProperty.call(options, "locationContext")
        ? options.locationContext
        : null;
    const idType = options && typeof options.idType === "string" && options.idType.trim() !== ""
        ? options.idType
        : "charid";
...
function emitItemsChangedForSession(session, item, previousState = {}, options = {}) {
    return emitItemsChangedBatchForSession(session, [{ item, previousData: previousState || {} }], options);
}
function isFittingMoveChange(change, shipID) {
    ...
    const nowFitted = isShipFittingFlag(item.flagID) && Number(item.locationID) === shipID;
    const wasFitted = isShipFittingFlag(previous.flagID) && Number(previous.locationID) === shipID;
...
function getSnapshotTrackedAttributes(snapshot) {
    ...
    return (snapshot.trackedShipAttributes ||
...
function sortAttributeChangesForStrip(changes, preferredOrder = []) {
    const preferred = new Map(preferredOrder.map((attributeID, index) => [Number(attributeID), index]));
...
function pushShipAttributeChangePairs(pairs, charID, shipID, previousAttributes, nextAttributes, time, preferredOrder = [], excludeAttributeIDs = []) {
    const excluded = new Set((Array.isArray(excludeAttributeIDs) ? excludeAttributeIDs : [])
        .map((attributeID) => Number(attributeID) || 0)
        .filter((attributeID) => attributeID > 0));
    const changes = listShipFittingAttributeChanges({ trackedShipAttributes: previousAttributes || {} }, { trackedShipAttributes: nextAttributes || {} }).filter((change) => !excluded.has(Number(change.attributeID) || 0));
    for (const change of sortAttributeChangesForStrip(changes, preferredOrder)) {
        pairs.push({
            event: buildModuleAttributeChangeEvent(charID, shipID, change.attributeID, change.nextValue, change.previousValue, time),
            time,
        });
    }
}
...
function emitStripFittingDogmaMultiEventForSession(session, shipID, changes, options = {}) {
    if (!session || typeof session.sendNotification !== "function") {
        return false;
    }
    ...
    const baseTime = session._space && typeof session._space.simFileTime === "bigint"
        ? session._space.simFileTime
        : currentFileTime();
    const pairs = [];
    ...
    if (previousSnapshot &&
        ...
        movedModules.forEach((moduleItem, index) => {
            ...
            const nextRemainingFittedItems = remainingFittedItems.filter((item) => Number(item && item.itemID) !== moduleID);
            const nextResourceState = previousSnapshot.buildResourceStateForItems(nextRemainingFittedItems);
            const nextAttributes = getResourceStateTrackedAttributes(nextResourceState);
            const time = buildStepTime(baseTime, index * 50000);
            pushShipAttributeChangePairs(pairs, charID, numericShipID, currentAttributes, nextAttributes, time, [ATTRIBUTE_CPU_LOAD, ATTRIBUTE_POWER_LOAD], [ATTRIBUTE_SCAN_RESOLUTION]);
            if (isEffectivelyOnlineModule(moduleItem)) {
                pairs.push({
                    event: buildModuleAttributeChangeEvent(charID, moduleID, ATTRIBUTE_IS_ONLINE, 0, 1, time),
                    time,
                });
                pairs.push({
                    event: buildGodmaShipEffectEvent(moduleID, charID, numericShipID, EFFECT_ONLINE, time, { isStart: 0, shouldStart: 0 }),
                    time,
                });
            }
            pushShipAttributeChangePairs(pairs, charID, numericShipID, currentAttributes, nextAttributes, time, [ATTRIBUTE_SCAN_RESOLUTION], [ATTRIBUTE_CPU_LOAD, ATTRIBUTE_POWER_LOAD]);
            if (stripModuleHasExtraCloakingEffect(moduleItem)) {
                pairs.push({
                    event: buildGodmaShipEffectEvent(moduleID, charID, numericShipID, EFFECT_STRIP_CLOAKING_PASSIVE, time, {
                        isStart: 0,
                        shouldStart: 0,
    ...
    if (previousSnapshot) {
        ...
        pushShipAttributeChangePairs(pairs, charID, numericShipID, initialAttributes, finalAttributes, finalTime, [ATTRIBUTE_SCAN_RESOLUTION, ATTRIBUTE_CPU_LOAD, ATTRIBUTE_POWER_LOAD]);
    }
...
function emitFittingTransactionForSession(session, shipID, changes, options = {}) {
    ...
    const moves = (Array.isArray(changes) ? changes : []).filter((change) => change && change.item);
    if (moves.length === 0) {
        return false;
    }
    const fittingMoves = moves.filter((change) => isFittingMoveChange(change, numericShipID));
    const time = session._space && typeof session._space.simFileTime === "bigint"
        ? session._space.simFileTime
        : currentFileTime();
    // Pre-move snapshot must be read before refreshShipFittingSnapshot recomputes.
    const previousSnapshot = fittingMoves.length > 0
    ...
    if (fittingMoves.length > 0) {
        const shipRecord = options.shipItem ||
        ...
        if (previousSnapshot && nextSnapshot) {
            for (const attrChange of listShipFittingAttributeChanges(previousSnapshot, nextSnapshot)) {
                subEvents.push(buildModuleAttributeChangeEvent(charID, numericShipID, attrChange.attributeID, attrChange.nextValue, attrChange.previousValue, time));
            }
        }
        for (const change of fittingMoves) {
            const item = change.item;
            const previous = change.previousData || change.previousState || {};
            const nowFitted = isShipFittingFlag(item.flagID) && Number(item.locationID) === numericShipID;
            const wasFitted = isShipFittingFlag(previous.flagID) &&
                Number(previous.locationID) === numericShipID;
            const hasOnlineEffect = getTypeDogmaEffects(item.typeID || previous.typeID).has(EFFECT_ONLINE);
            if (nowFitted &&
                !wasFitted &&
                hasOnlineEffect &&
                isEffectivelyOnlineModule(item)) {
                subEvents.push(buildModuleAttributeChangeEvent(charID, item.itemID, ATTRIBUTE_IS_ONLINE, 1, 0, time));
                // Newly fitted + online → the module's online effect starts.
                subEvents.push(buildGodmaShipEffectEvent(item.itemID, charID, numericShipID, EFFECT_ONLINE, time, {
                    isStart: 1,
                    shouldStart: 1,
                }));
            }
                ...
                if (isEffectivelyOnlineModule(previous) || isEffectivelyOnlineModule(item)) {
                    subEvents.push(buildModuleAttributeChangeEvent(charID, item.itemID, ATTRIBUTE_IS_ONLINE, 0, 1, time));
                }
    ...
    for (const change of moves) {
        emitItemsChangedForSession(session, change.item, change.previousData || change.previousState || {});
    }
    return true;
}
function syncModuleOnlineEffectForSession(session, item, options = {}) {
    ...
    const active = options.active === undefined ? isEffectivelyOnlineModule(item) : Boolean(options.active);
    ...
    const now = typeof baseNow === "bigint"
    ...
    log.debug(`[module-effect-sync] shipID=${shipID} moduleID=${moduleID} ` +
...
function syncModuleOnlineBounceForSession(session, item, options = {}) {
    ...
    const started = syncModuleOnlineEffectForSession(session, item, {
        ...options,
        active: true,
        timeOffset: Number(options.startTimeOffset) ||
            (Number(options.stopTimeOffset) || 0) + 1,
    });
    if (stopped || started) {
        log.debug(`[module-effect-bounce] shipID=${Number(item.locationID) || 0} ` +
...
function syncOnlineModuleEffectsForSession(session, shipID = null, options = {}) {
    ...
    log.debug(`[module-effect-sync] online-effect shipID=${Number(resolvedShipID) || 0} ` +
...
function syncFittedModulesForSession(session, shipID = null, options = {}) {
    ...
    const allowInSpaceChargeInventoryRows = options.allowInSpaceChargeInventoryRows === true;
    const isInSpaceSession = Boolean(session &&
        session._space &&
        !isDockedSession(session));
    const emitChargeInventoryRows = options.emitChargeInventoryRows === undefined
        ? !isInSpaceSession
    ...
    const preferSyntheticHardpointOnlineBounce = emitOnlineEffects &&
        isInSpaceSession &&
        options.syntheticFitTransition === true;
    const restrictToItemIDs = (Array.isArray(options.restrictToItemIDs)
    ...
    const restrictedItemIDSet = restrictToItemIDs.length > 0 ? new Set(restrictToItemIDs) : null;
    ...
    const fittedItems = onlyCharges
        ...
        : (contextModuleItems || getFittedModuleItems(charId, resolvedShipID))
            ...
            .filter((item) => (onlyScannerProbeLaunchers !== true ||
    ...
    const loadedChargeItems = preferSyntheticHardpointOnlineBounce
        ...
        : [];
    const loadedChargeByFlag = new Map(loadedChargeItems.map((chargeItem) => [
    ...
    const restrictedChargeFlagSet = restrictedItemIDSet instanceof Set
        ? new Set(fittedItems
            .filter((item) => Number(item && item.categoryID) !== 8 &&
            restrictedItemIDSet.has(Number(item && item.itemID) || 0))
            .map((item) => Number(item && item.flagID) || 0)
            .filter((flagID) => flagID > 0))
        : null;
    const syncItems = restrictedItemIDSet instanceof Set
        ? fittedItems.filter((item) => {
            ...
            return Boolean(restrictedChargeFlagSet instanceof Set &&
    ...
    log.debug(`[fitting-sync] begin charID=${Number(charId) || 0} ` +
        ...
        `items=${JSON.stringify(syncItems.map((item) => summarizeMissileDebugItem(item)))}`);
    for (const moduleItem of syncItems) {
        ...
        const moduleFamily = preferSyntheticHardpointOnlineBounce
            ? resolveModuleParityFamily(moduleItem, loadedChargeByFlag.get(Number(moduleItem && moduleItem.flagID) || 0) || null)
            : null;
        const suppressSyntheticModuleInventorySync = options.forceSyntheticInventorySync !== true &&
            preferSyntheticHardpointOnlineBounce &&
            !isLoadedCharge &&
            moduleFamily &&
            moduleFamily.hardpointBound === true &&
            moduleFamily.requiresOnlineEffectBootstrap === true;
        const previousState = options.syntheticFitTransition === true
            ? {
        ...
        log.debug(`[fitting-sync] emit shipID=${Number(resolvedShipID) || 0} ` +
        ...
        if (!suppressSyntheticModuleInventorySync) {
            syncInventoryItemForSession(session, moduleItem, previousState, {
        ...
        if (isInSpaceSession) {
            syncModuleAttributeChangesForSession(session, buildScannerProbeLauncherBootstrapChanges(session, moduleItem));
        }
    }
...
function syncShipFittingStateForSession(session, shipID = null, options = {}) {
    ...
    const syncedItemCount = options.emitInventorySync === false
        ...
        : syncFittedModulesForSession(session, shipID, {
            ...
            allowInSpaceChargeInventoryRows: options.allowInSpaceChargeInventoryRows === true,
    ...
    if (options.emitOnlineEffects === true &&
    ...
    if (charId > 0 && resolvedShipID) {
        const refreshedShipRecord = findCharacterShip(charId, resolvedShipID) ||
            getActiveShipRecord(charId) ||
            null;
        ...
        syncShipFittingAttributeChangesForSession(session, resolvedShipID, previousSnapshot, nextSnapshot);
    }
    return syncedItemCount;
}
function queueDeferredDockedShipSessionChange(session, shipID, previousClientShipID = null, options = {}) {
    if (!session) {
        return;
    }
...
function clearDeferredDockedShipSessionChange(session) {
    ...
    clearDeferredDockedShipSessionChangeTimer(session._deferredDockedShipSessionChange);
    session._deferredDockedShipSessionChange = null;
}
...
function syncDockedFittingBootstrapForSession(session, shipID) {
    ...
    syncShipFittingAttributeChangesForSession(session, resolvedShipID, { trackedShipAttributes: {} }, nextSnapshot);
    return true;
}
function completeDockedFittingBootstrap(session, options = {}) {
    ...
    clearDockedFittingBootstrap(session);
    if (!isDockedSession(session)) {
        log.info(`[CharacterState] Dropped docked fitting bootstrap shipid=${pending.shipID} ` +
            `trigger=${trigger} reason=session-not-docked`);
        return false;
    }
    const didSync = syncDockedFittingBootstrapForSession(session, pending.shipID);
    log.info(`[CharacterState] Completed docked fitting bootstrap shipid=${pending.shipID} ` +
...
function shouldFlushDeferredDockedShipSessionChange(session, method) {
    ...
    return Boolean(pending.stationHangarSelfSeen || pending.stationHangarListCount >= 2);
}
function flushDeferredDockedShipSessionChange(session, options = {}) {
    ...
    session._deferredDockedShipSessionChange = null;
    log.info(`[CharacterState] Flushed deferred docked shipid=${shipID} trigger=${options.trigger || "unknown"}`);
    return true;
}
function buildCharacterSessionNotificationPlan(session, options = {}) {
    ...
    const isInitialCharacterSelection = options.isInitialCharacterSelection === true;
    const oldShipID = normalizeSessionShipValue(options.oldShipID);
    const newShipID = normalizeSessionShipValue(options.newShipID === undefined ? session.shipID : options.newShipID);
    const oldDockedLocationID = Number(options.oldStructureID || options.oldStationID || 0) || 0;
    const enteredStationFromNonStation = isDocked &&
        !oldDockedLocationID &&
        !isInitialCharacterSelection &&
        Boolean(options.oldLocationID ||
            options.oldSolarSystemID ||
            options.oldSolarSystemID2);
    const deferDockedShipSessionChange = options.deferDockedShipSessionChange !== false &&
        isDocked &&
        enteredStationFromNonStation;
    const sessionChanges = {};
    appendSessionChange(sessionChanges, "charid", options.oldCharID || null, options.charID);
    appendSessionChange(sessionChanges, "corpid", options.oldCorpID || null, session.corporationID);
    appendSessionChange(sessionChanges, "allianceid", options.oldAllianceID || null, session.allianceID || null);
    appendSessionChange(sessionChanges, "genderID", isInitialCharacterSelection ? null : options.oldGenderID, session.genderID);
    appendSessionChange(sessionChanges, "bloodlineID", isInitialCharacterSelection ? null : options.oldBloodlineID, session.bloodlineID);
    appendSessionChange(sessionChanges, "raceID", isInitialCharacterSelection ? null : options.oldRaceID, session.raceID);
    appendSessionChange(sessionChanges, "schoolID", options.oldSchoolID, session.schoolID);
    appendSessionChange(sessionChanges, "factionid", options.oldFactionID || null, session.factionid || session.factionID || null);
    appendSessionChange(sessionChanges, "stationid", options.oldStationID || null, session.stationid || null);
    appendSessionChange(sessionChanges, "stationid2", options.oldStationID2 || null, session.stationid2 || null);
    appendSessionChange(sessionChanges, "structureid", options.oldStructureID || null, session.structureid || null);
    appendSessionChange(sessionChanges, "solarsystemid", options.oldSolarSystemID || null, session.solarsystemid || null);
    appendSessionChange(sessionChanges, "solarsystemid2", options.oldSolarSystemID2 || null, session.solarsystemid2 || null);
    appendSessionChange(sessionChanges, "constellationid", options.oldConstellationID || null, session.constellationID);
    appendSessionChange(sessionChanges, "regionid", options.oldRegionID || null, session.regionID);
    appendSessionChange(sessionChanges, "shipid", oldShipID, deferDockedShipSessionChange ? null : newShipID);
    appendSessionChange(sessionChanges, "locationid", options.oldLocationID || null, session.locationid || null);
    appendSessionChange(sessionChanges, "worldspaceid", options.oldWorldspaceID || null, session.worldspaceid || null);
    appendSessionChange(sessionChanges, "fleetid", options.oldFleetID || null, session.fleetid || null);
    appendSessionChange(sessionChanges, "fleetrole", options.oldFleetRole || null, session.fleetrole || null);
    appendSessionChange(sessionChanges, "wingid", options.oldWingID || null, session.wingid || null);
    appendSessionChange(sessionChanges, "squadid", options.oldSquadID || null, session.squadid || null);
    appendSessionChange(sessionChanges, "warfactionid", options.oldWarFactionID || null, session.warfactionid || session.warFactionID || null);
    if (!(options.suppressCorpAccountKeyWithCorpChange === true &&
        Object.prototype.hasOwnProperty.call(sessionChanges, "corpid"))) {
        appendSessionChange(sessionChanges, "corpAccountKey", options.oldCorpAccountKey ?? null, session.corpAccountKey ?? session.corpaccountkey ?? null, {
            force: options.forceCorpAccountKeyChange === true ||
                Object.prototype.hasOwnProperty.call(sessionChanges, "corpid"),
        });
    }
    if (isCharacterSelection || options.includeRoleChanges === true) {
        appendSessionChange(sessionChanges, "hqID", options.oldHqID ?? null, session.hqID ?? null);
        appendSessionChange(sessionChanges, "baseID", options.oldBaseID ?? null, session.baseID ?? null);
        appendSessionChange(sessionChanges, "role", isInitialCharacterSelection
            ? null
            : composeSessionRoleMask(options.oldAccountRole ?? options.oldRole, options.oldChatRole ?? options.oldRole), composeSessionRoleMask(session.accountRole ?? session.role, session.chatRole ?? session.role));
        appendSessionChange(sessionChanges, "corprole", normalizeOptionalRoleMask(options.oldCorpRole), normalizeOptionalRoleMask(session.corprole));
        appendSessionChange(sessionChanges, "rolesAtAll", normalizeOptionalRoleMask(options.oldRolesAtAll), normalizeOptionalRoleMask(session.rolesAtAll));
        appendSessionChange(sessionChanges, "rolesAtBase", normalizeOptionalRoleMask(options.oldRolesAtBase), normalizeOptionalRoleMask(session.rolesAtBase));
        appendSessionChange(sessionChanges, "rolesAtHQ", normalizeOptionalRoleMask(options.oldRolesAtHQ), normalizeOptionalRoleMask(session.rolesAtHQ));
        appendSessionChange(sessionChanges, "rolesAtOther", normalizeOptionalRoleMask(options.oldRolesAtOther), normalizeOptionalRoleMask(session.rolesAtOther));
    }
    const initialSpaceCharacterSelection = isCharacterSelection &&
        isInitialCharacterSelection &&
        !isDocked;
    ...
    if (initialSpaceCharacterSelection) {
        reorderSessionChangesByKeys(sessionChanges, INITIAL_SPACE_CHARACTER_SESSION_CHANGE_KEYS);
    }
    return {
        ...
        fittingBootstrap: isDocked && !deferDockedShipSessionChange
            ? {
                shipID: Number(options.shipID) ||
                    Number(newShipID) ||
...
function flushCharacterSessionNotificationPlan(session, plan, options = {}) {
    ...
    if (plan.deferDockedShipSessionChange === true) {
        queueDeferredDockedShipSessionChange(session, plan.newShipID, plan.oldShipID, {
    ...
    if (options.includeFittingBootstrap !== false &&
        ...
        if (isDockedSession(session)) {
            ...
            syncShipFittingStateForSession(session, plan.fittingBootstrap.shipID, {
                includeOfflineModules: plan.fittingBootstrap.includeOfflineModules === true,
...
function applyCharacterToSession(session, charId, options = {}) {
    ...
    const oldShipID = normalizeSessionShipValue(session.shipID ?? session.shipid ?? null);
    ...
    const oldCorpAccountKey = session.corpAccountKey ?? session.corpaccountkey ?? null;
    ...
    const { getCorporationSessionRoleState, } = require(path.join(__dirname, "../corporation/corporationRuntimeState"));
    const corporationRoleState = getCorporationSessionRoleState(charData.corporationID || 1000009, charId);
    ...
    session.plexBalance = normalizeInteger(charData.plexBalance, DEFAULT_PLEX_BALANCE);
    ...
    if (options.deferCorpAccountKey === true) {
        if (options.clearCorpAccountKey === true ||
    ...
    session.role = composeSessionRoleMask(session.accountRole ?? session.role, session.chatRole ?? session.role);
    const isCharacterSelection = options.selectionEvent !== false &&
        (oldCharID === undefined || oldCharID === null || oldCharID !== charId);
    const isInitialCharacterSelection = isCharacterSelection &&
    ...
    if (options.prewarmSkillCache !== false && isCharacterSelection) {
        try {
            ...
            log.warn(`[CharState] Failed to prewarm skill cache for character ${charId}: ${error.message}`);
        }
    }
    ...
    if (options.logSelection !== false) {
        log.info(`[CharState] Applied ${session.characterName}(${charId}) ship=${session.shipName}(${session.shipTypeID}) activeShipID=${session.activeShipID} docked=${isDocked} station=${session.stationid} structure=${session.structureid} system=${solarSystemID}`);
    }
    return {
        success: true,
        data: charData,
...
function activateShipForSession(session, shipId, options = {}) {
    ...
    const shouldConsumePreviousCapsule = currentShip &&
    ...
    if (options.emitNotifications !== false &&
        ...
        syncInventoryItemForSession(session, targetShip, {
            locationID: 0,
            flagID: 0,
            quantity: 0,
            singleton: 0,
    ...
    if (shouldConsumePreviousCapsule) {
        ...
        if (!removeResult.success) {
            log.warn(`[CharState] Failed to consume docked capsule ${currentShip.itemID} for char=${charId}: ${removeResult.errorMsg}`);
        }
        else {
    ...
    if (applyResult.success &&
        ...
        for (const shipItem of refreshQueue) {
            ...
            syncInventoryItemForSession(session, shipItem, {
...
function clearCharacterFromSession(session, options = {}) {
    ...
    const wasRetailControlSession = Boolean(oldCharID &&
...
function giveItemToHangarForSession(session, itemType, quantity = 1) {
    ...
    const stationId = dockedLocationKind === "structure"
        ? (session.structureid || session.structureID || 60003760)
        : (session.stationid || session.stationID || 60003760);
    const grantResult = grantItemToCharacterStationHangar(charId, stationId, itemType, quantity);
...
function giveItemToActiveShipCargoForSession(session, itemType, quantity = 1, grantOptions = {}) {
    if (!session || !session.characterID) {
        return {
            success: false,
            errorMsg: "CHARACTER_NOT_SELECTED",
    ...
    const activeShip = getActiveShipRecord(charId);
    const shipId = Number(session.shipid || session.shipID || (activeShip && activeShip.itemID)) || 0;
    ...
    const solarSystemId = Number(session.solarsystemid2 || session.solarsystemid || session.locationid) || 0;
    if (Number(shipItem.flagID || 0) !== 0 ||
    ...
    const grantResult = grantItemToCharacterLocation(charId, shipItem.itemID, ITEM_FLAGS.CARGO_HOLD, itemType, quantity, grantOptions);
    if (!grantResult.success) {
        return grantResult;
    }
    for (const change of grantResult.data.changes || []) {
...
function spawnShipInHangarForSession(session, shipType) {
    ...
    const shipItem = (spawnResult.data.items && spawnResult.data.items[0]) || null;
    return {
        success: true,
        errorMsg: null,
        created: Boolean(spawnResult.data.changes &&
