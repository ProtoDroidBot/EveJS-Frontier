...
const { buildDict, buildFiletimeLong, buildKeyVal, buildList, currentFileTime, } = require(path.join(__dirname, "../_shared/serviceHelpers"));
...
function sendChallengeProgressUpdate(session, challengeID, progress) {
    ...
    const normalizedChallengeID = normalizeInteger(challengeID, DEFAULT_LOGGED_CHALLENGE_ID);
...
class SeasonManagerService extends BaseService {
    ...
    Handle_process_client_event(args, session, kwargs) {
        ...
        const progress = kwargs && Object.prototype.hasOwnProperty.call(kwargs, "amount")
            ? kwargs.amount
            : 0;
...
//# sourceMappingURL=seasonManagerService.js.map