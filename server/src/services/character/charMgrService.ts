...
function getNpcPublicCharacterData(characterID) {
  ...
  if (!isNpcCharacterID(id)) return null;
  const pilot = getNpcPilotIdentityStore().get(id);
  if (!pilot) return { npcPilot: true, characterName: `NPC ${id}` };
...
function normalizePersonalContacts(record: Record<string, any> = {}) {
  const source =
  ...
  for (const [rawContactID, rawContact] of Object.entries<any>(source)) {
...
function normalizePersonalContactLabels(record: Record<string, any> = {}) {
  const source =
  ...
  for (const [rawLabelID, rawLabel] of Object.entries<any>(source)) {
...
function normalizeBlockedOwners(record: Record<string, any> = {}) {
  const source =
  ...
  for (const [rawOwnerID, rawBlocked] of Object.entries<any>(source)) {
...
function upsertPersonalContact(characterID, contactID, options: Record<string, any> = {}) {
  const normalizedContactID = normalizeInteger(contactID, 0);
  if (!(normalizedContactID > 0)) {
    return { success: false as const, errorMsg: "CONTACT_REQUIRED" };
  }
...
function removePersonalContacts(characterID, contactIDs: any[] = []) {
  return updateCharacterRecord(characterID, (record) => {
    const contacts = normalizePersonalContacts(record);
...
function updateBlockedOwners(characterID, ownerIDs: any[] = [], shouldBlock = true) {
  return updateCharacterRecord(characterID, (record) => {
...
function buildHomeStationPayload(station, homeStationInfo: Record<string, any> = {}) {
  return buildKeyVal([
...
function buildCloneEntries(entries: any[] = [], valueBuilder) {
  return buildDict(
...
function buildPublicInfoEntries(charId, charData, session) {
  ...
  const corporationID =
    charData.corporationID || (viewerSession ? viewerSession.corporationID : 1000009);
  ...
  const solarSystemID =
    charData.solarSystemID || (viewerSession ? viewerSession.solarsystemid2
...
class CharMgrService extends BaseService {
  ...
  Handle_GetCharacterSettings(args, session) {
    ...
    return buildDict(
      Object.entries<any>(getCharacterSettings(characterID)).sort(([left], [right]) => (
