...
function buildRelationshipStandingsRowset(rows: any[] = []) {
  const rowDescriptor = {
    type: "list",
...
function buildFromStandingRowset(rows: any[] = [], targetID) {
...
function buildKeyValList(items: any[] = []) {
  return {
    type: "list",
    items,
  };
...
function buildStandingTransactionList(transactions: any[] = []) {
...
function buildStandingCompositionList(rows: any[] = []) {
  return buildKeyValList(
    rows.map((row) =>
      buildKeyVal([
...
function buildStandingCompositionRowset(rows: any[] = []) {
