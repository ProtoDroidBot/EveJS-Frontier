...
function buildStandingTransactionList(transactions = []) {
    return buildKeyValList(transactions.map((transaction) => buildKeyVal([
...
function buildStandingCompositionList(rows = []) {
    return buildKeyValList(rows.map((row) => buildKeyVal([
...
function buildStandingCompositionRowset(rows = []) {
    return buildDbRowset(STANDING_COMPOSITION_DBROW_COLUMNS, rows.map((row) => [
...
class StandingMgrService extends BaseService {
    ...
    Handle_GetCharStandings(args, session) {
        ...
        return buildFromStandingRowset(standingRuntime.listCharacterStandings(session && (session.characterID || session.charid)), session && (session.characterID || session.charid));
    }
    Handle_GetCorpStandings(args, session) {
        log.debug("[StandingMgr] GetCorpStandings called");
        return buildFromStandingRowset(standingRuntime.listCorporationStandings(session && (session.corporationID || session.corpid)), session && (session.corporationID || session.corpid));
    }
    Handle_GetStandingTransactions(args) {
        ...
        return buildStandingTransactionList(standingRuntime.getStandingTransactions(fromID, toID));
    }
    Handle_GetStandingCompositions(args) {
        const fromID = args && args.length > 0 ? args[0] : 0;
        const toID = args && args.length > 1 ? args[1] : 0;
        log.debug(`[StandingMgr] GetStandingCompositions(${fromID}, ${toID})`);
        return buildCachedMethodCallResult(buildStandingCompositionRowset(standingRuntime.getStandingCompositions(fromID, toID)), {
...
//# sourceMappingURL=standingMgrService.js.map