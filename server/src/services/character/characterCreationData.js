...
const { TABLE, clearReferenceCache, readStaticRows, } = require(path.join(__dirname, "../_shared/referenceData"));
...
function normalizeCharacterCreationSchool(entry) {
    ...
    return {
        ...
        isStarterSpaceSchool: entry.isStarterSpaceSchool === true ||
            entry.is_starter_space_school === true ||
            entry.is_starter_space_school === 1 ||
            entry.starterSystemID != null,
        solarSystemID: toNumber(entry.solarSystemID, 0) ||
            toNumber(entry.starterSystemID, 0) ||
            null,
        schoolMapSource: typeof entry.schoolMapSource === "string"
...
function getCharacterCreationRace(raceID, options = {}) {
    ...
    return (getCharacterCreationRaces(options).find((entry) => entry.raceID === numericRaceID) ||
        null);
}
...
function getCharacterCreationBloodline(bloodlineID, options = {}) {
    ...
    return (getCharacterCreationBloodlines(options).find((entry) => entry.bloodlineID === numericBloodlineID) || null);
}
...
function getCharacterCreationSchool(schoolID, options = {}) {
    ...
    return (getCharacterCreationSchools(options).find((entry) => entry.schoolID === numericSchoolID) || null);
}
...
function getCharacterCreationQAStarterSystemIDs(options = {}) {
    return [...new Set(getCharacterCreationSchools(options)
            .map((school) => toNumber(school && school.solarSystemID, 0))
            .filter((solarSystemID) => solarSystemID > 0))].sort((left, right) => left - right);
}
function resolveCharacterCreationSchoolProfile(schoolID, fallback = {}) {
    ...
    return {
        ...
        raceID: (school && school.raceID) ||
            toNumber(fallback.raceID, 1) ||
            1,
        corporationID: (school && school.corporationID) ||
            toNumber(fallback.corporationID, null),
        careerID: (school && school.careerID) ||
            toNumber(fallback.careerID, null),
        startingStations: school && Array.isArray(school.startingStations)
            ? cloneValue(school.startingStations)
            : [],
        isStarterSpaceSchool: Boolean(school && school.isStarterSpaceSchool),
        solarSystemID: (school && school.solarSystemID) ||
            toNumber(fallback.solarSystemID, null),
        schoolMapSource: (school && school.schoolMapSource) ||
            (typeof fallback.schoolMapSource === "string"
                ? fallback.schoolMapSource
                : null),
        careerAgents: school && Array.isArray(school.careerAgents)
...
function resolveCharacterCreationSchoolIDForRace(schoolID, raceID, fallback = 11) {
    ...
    const requestedSchool = schools.find((school) => school.schoolID === numericSchoolID);
    if (requestedSchool &&
        (numericRaceID <= 0 ||
    ...
    const legacyCorporationSchool = schools
        .filter((school) => school.corporationID === numericSchoolID &&
        (!numericRaceID || school.raceID === numericRaceID))
        .sort((left, right) => Number(right.isStarterSpaceSchool) - Number(left.isStarterSpaceSchool) ||
        left.schoolID - right.schoolID)[0];
...
function resolveCharacterCreationBloodlineProfile(bloodlineID, fallback = {}) {
    ...
    return {
        ...
        raceID: (bloodline && bloodline.raceID) ||
            toNumber(fallback.raceID, 1) ||
            1,
        corporationID: (bloodline && bloodline.corporationID) ||
            toNumber(fallback.corporationID, 1000009) ||
            1000009,
        typeID: BLOODLINE_CHARACTER_TYPE_ID[numericBloodlineID] ||
