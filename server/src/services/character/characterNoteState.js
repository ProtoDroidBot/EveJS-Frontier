...
function writeOwnerNote(characterKey, entry) {
    return database.write(TABLE_NAME, ownerNotePath(characterKey, entry.noteID), entry);
}
...
function editOwnerNote(characterID, noteID, label, noteText) {
    ...
    if (previousLabel === DEFAULT_FOLDERS_LABEL &&
...
function removeOwnerNote(characterID, noteID) {
    ...
    const removeResult = database.remove(TABLE_NAME, ownerNotePath(characterKey, numericNoteID));
    if (!removeResult ||
        (!removeResult.success && removeResult.errorMsg !== "ENTRY_NOT_FOUND")) {
        return false;
    }
    if (existingLabel === DEFAULT_FOLDERS_LABEL &&
        foldersNoteIDs(bucket.notes).length === 0) {
...
function setEntityNote(characterID, itemID, noteText) {
    ...
    if (normalizedText === "") {
        ...
        const removeResult = database.remove(TABLE_NAME, itemNotePath(characterKey, numericItemID));
    ...
    const writeResult = database.write(TABLE_NAME, itemNotePath(characterKey, numericItemID), entry);
    return Boolean(writeResult && writeResult.success);
}
module.exports = {
    listOwnerNotes,
...
//# sourceMappingURL=characterNoteState.js.map