...
function assertCharacterOffline(characterID) {
  ...
  try {
    ...
    throwDeletionUserError(
      "Character control is temporarily unavailable; deletion remains disabled.",
    );
  }
...
function listCharacterOwnedRootItems(characterID) {
  ...
  const ownedItemIDs = new Set<any>(ownedItems.map((item) => toPositiveInt(item && item.itemID, 0)));
...
function transferCharacterAssetsToCorporation(characterID, characterRecord, corporationID) {
  ...
  const transferredRootItemIDs: any[] = [];
...
function destroyCharacterAssets(characterID) {
  const destroyedRootItemIDs: any[] = [];
  const removedRootIDs = new Set<any>();
...
function purgeBookmarkData(characterID) {
  ...
  const deletedFolderIDs = new Set<any>();
  const deletedSubfolderIDs = new Set<any>();
  const deletedBookmarkIDs = new Set<any>();
  ...
  for (const [folderID, folder] of Object.entries<any>(foldersRoot.records || {})) {
    if (
      folder &&
  ...
  for (const [subfolderID, subfolder] of Object.entries<any>(subfoldersRoot.records || {})) {
  ...
  for (const [bookmarkID, bookmark] of Object.entries<any>(bookmarksRoot.records || {})) {
    if (
  ...
  for (const [ownerCharacterID, knownEntries] of Object.entries<any>(
  ...
  const referencedGroupIDs = new Set<any>();
  for (const folder of Object.values<any>(foldersRoot.records || {})) {
  ...
  for (const [groupID, group] of Object.entries<any>(groupsRoot.records || {})) {
    const numericGroupID = toPositiveInt(groupID, 0);
...
function purgeCalendarData(characterID) {
  ...
  const deletedEventIDs = new Set<any>();
  let eventsChanged = false;
  let responsesChanged = false;

  for (const [eventID, event] of Object.entries<any>(eventsRoot.events || {})) {
  ...
  for (const [responseKey, response] of Object.entries<any>(responsesRoot.responses || {})) {
