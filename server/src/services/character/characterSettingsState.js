...
function normalizeSettingsRecord(settings) {
    ...
    for (const [settingKey, settingValue] of Object.entries(source)) {
        ...
        if (Buffer.isBuffer(settingValue) ||
...
function setCharacterSetting(characterID, settingKey, value) {
    ...
    const writeResult = repo.write("characters", getSettingPath(numericCharacterID, normalizedKey), normalizedValue);
...
function deleteCharacterSetting(characterID, settingKey) {
    ...
    const removeResult = repo.remove("characters", getSettingPath(numericCharacterID, normalizedKey));
...
//# sourceMappingURL=characterSettingsState.js.map