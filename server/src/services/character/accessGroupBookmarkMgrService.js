...
const { buildList, buildDict, normalizeNumber, normalizeText, } = require("../_shared/serviceHelpers");
...
const { buildBookmarkPayload, buildBookmarkReplyTuple, buildFolderPayload, buildSubfolderPayload, } = require("../bookmark/bookmarkPayloads");
const { resolveLocationBookmarkTarget, } = require("../bookmark/bookmarkTargetResolver");
// -----------------------------------------------------------------------
// Shared helpers
// -----------------------------------------------------------------------
/**
...
class AccessGroupBookmarkMgrService extends BaseService {
    ...
    Handle_AddFolder(args, session) {
        ...
        log.info(`[AccessGroupBookmarkMgr] AddFolder char=${charID} personal=${isPersonal} name="${folderName}"`);
        try {
    ...
    Handle_UpdateFolder(args, session) {
        ...
        const useGroupID = args && args[5] != null ? normalizeNumber(args[5], null) : null;
        const viewGroupID = args && args[6] != null ? normalizeNumber(args[6], null) : null;
        log.info(`[AccessGroupBookmarkMgr] UpdateFolder char=${charID} folder=${folderID} name="${folderName}"`);
        try {
    ...
    Handle_DeleteFolder(args, session) {
        const charID = session && session.characterID;
        const folderID = normalizeNumber(args && args[0], 0);
        log.info(`[AccessGroupBookmarkMgr] DeleteFolder char=${charID} folder=${folderID}`);
        try {
    ...
    Handle_GetFolderInfo(args, session) {
        const charID = session && session.characterID;
        const folderID = normalizeNumber(args && args[0], 0);
        log.debug(`[AccessGroupBookmarkMgr] GetFolderInfo char=${charID} folder=${folderID}`);
        try {
    ...
    Handle_SearchFoldersWithAdminAccess(args, session) {
        const charID = session && session.characterID;
        log.debug(`[AccessGroupBookmarkMgr] SearchFoldersWithAdminAccess char=${charID}`);
        try {
    ...
    Handle_AddToKnownFolders(args, session) {
        ...
        log.info(`[AccessGroupBookmarkMgr] AddToKnownFolders char=${charID} folder=${folderID} active=${isActive}`);
        try {
    ...
    Handle_RemoveFromKnownFolders(args, session) {
        const charID = session && session.characterID;
        const folderID = normalizeNumber(args && args[0], 0);
        log.info(`[AccessGroupBookmarkMgr] RemoveFromKnownFolders char=${charID} folder=${folderID}`);
        try {
    ...
    Handle_UpdateKnownFolderState(args, session) {
        ...
        const folderID = normalizeNumber(args && args[0], 0);
        const isActive = args && args[1] !== undefined ? Boolean(args[1]) : true;
        log.info(`[AccessGroupBookmarkMgr] UpdateKnownFolderState char=${charID} folder=${folderID} active=${isActive}`);
        try {
    ...
    Handle_BookmarkStaticLocation(args, session) {
        ...
        log.info(`[AccessGroupBookmarkMgr] BookmarkStaticLocation char=${charID} item=${itemID} folder=${folderID} name="${name}"`);
        const target = runtime.resolveStaticBookmarkTarget(itemID, session);
        if (!target) {
            log.warn(`[AccessGroupBookmarkMgr] BookmarkStaticLocation: could not resolve geometry for item=${itemID}`);
    ...
    Handle_BookmarkLocation(args, session) {
        ...
        const expiryMode = normalizeNumber(args && args[4], 0);
        const subfolderID = getSubfolderArg(args && args[5]);
        log.info(`[AccessGroupBookmarkMgr] BookmarkLocation char=${charID} item=${itemID} folder=${folderID} name="${name}"`);
    ...
    Handle_UpdateBookmark(args, session) {
        ...
        log.info(`[AccessGroupBookmarkMgr] UpdateBookmark char=${charID} bm=${bookmarkID} folder=${newFolderID} name="${name}" expiryCancel=${expiryCancel}`);
        try {
            const result = runtime.updateBookmark(charID, bookmarkID, folderID, name, note, subfolderID, newFolderID, expiryCancel);
    ...
    Handle_DeleteBookmarks(args, session) {
        ...
        log.info(`[AccessGroupBookmarkMgr] DeleteBookmarks char=${charID} folder=${folderID} ids=${JSON.stringify(bookmarkIDs)}`);
        let deleted;
        try {
    ...
    Handle_MoveBookmarksToFolderAndSubfolder(args, session) {
        ...
        log.info(`[AccessGroupBookmarkMgr] MoveBookmarks char=${charID} from=${oldFolderID} to=${newFolderID} subfolder=${subfolderID} count=${bookmarkIDs.length}`);
        let result;
        try {
        ...
        if (result.movedBookmarks.length > 0) {
            bookmarkNotifications.notifyBookmarksMoved(result.oldFolderID, result.newFolderID, result.movedBookmarks, { excludeCharacterID: charID });
        }
        const rows = result.movedBookmarks.map((bookmark) => buildBookmarkPayload(bookmark));
    ...
    Handle_CopyBookmarksToFolderAndSubfolder(args, session) {
        ...
        const subfolderID = getSubfolderArg(args && args[2]);
        const bookmarkIDs = normalizeIDSetArg(args && args[3]);
        log.info(`[AccessGroupBookmarkMgr] CopyBookmarks char=${charID} from=${oldFolderID} to=${newFolderID} subfolder=${subfolderID} count=${bookmarkIDs.length}`);
        let createdBookmarks;
        try {
            createdBookmarks = runtime.copyBookmarks(charID, oldFolderID, newFolderID, subfolderID, bookmarkIDs);
        }
        catch (error) {
            return rethrowAsUserError(error);
        }
    ...
    Handle_CreateSubfolder(args, session) {
        ...
        log.info(`[AccessGroupBookmarkMgr] CreateSubfolder char=${charID} folder=${folderID} name="${subfolderName}"`);
        try {
    ...
    Handle_UpdateSubfolder(args, session) {
        ...
        log.info(`[AccessGroupBookmarkMgr] UpdateSubfolder char=${charID} folder=${folderID} subfolder=${subfolderID} name="${subfolderName}"`);
        try {
    ...
    Handle_DeleteSubfolder(args, session) {
        ...
        const folderID = normalizeNumber(args && args[0], 0);
        const subfolderID = normalizeNumber(args && args[1], 0);
        log.info(`[AccessGroupBookmarkMgr] DeleteSubfolder char=${charID} folder=${folderID} subfolder=${subfolderID}`);
        let deletedBmIDs;
        try {
...
//# sourceMappingURL=accessGroupBookmarkMgrService.js.map