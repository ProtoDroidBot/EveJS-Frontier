...
} = require("../frontier/suiCharacterProvisioning");
const {
  SUI_GRPC_BASE_URL,
  SUI_GRPC_NETWORK,
} = require("../frontier/suiGrpcClient");
const {
  listCharacterItems,
  removeInventoryItem,
...
const {
  ...
  ROLE_QA,
  normalizeRoleValue,
}: { ROLE_CONTENT: bigint; ROLE_GML: bigint; ROLE_PROGRAMMER: bigint; ROLE_QA: bigint; normalizeRoleValue: (value: unknown, fallback?: bigint) => bigint } = require("../account/accountRoleProfiles");
...
function readKeywordArg(kwargs, keys: any[] = []) {
  if (!kwargs) {
    return undefined;
  }
...
function readKeywordIntArg(kwargs, keys: any[] = [], fallback = 0) {
...
function summarizeCreationArg(value, depth = 0) {
  ...
  if (typeof value === "object") {
    ...
    for (const [key, entryValue] of Object.entries<any>(value).slice(0, 8)) {
...
function getFinishedSkillsCount(finishedSkills: any[] = []) {
...
function resolveStarterCorporationRecord(bloodlineProfile, schoolProfile: Record<string, any> = {}) {
...
function rollbackNewCharacterBootstrap(characterID) {
  ...
  const ownedItemIDs = new Set<any>(
...
function canToggleCharacterCreationValidation(session) {
  ...
  const roleMask: bigint = normalizeRoleValue(
...
function evictPriorSession(priorSession, options: Record<string, any> = {}) {
  if (!priorSession) {
    return;
  }
  try {
...
class CharService extends BaseService {
  declare suiCharacterProvisioner: (
    input: Record<string, any>,
    options?: Record<string, any>,
  ) => Promise<Record<string, any>>;
  declare suiCharacterProvisioningOptions: Record<string, any>;
  declare suiCharacterProvisioningEnabled: boolean;
  ...
  constructor(options: Record<string, any> = {}) {
    ...
    this.suiCharacterProvisioningOptions =
      ...
      typeof options.suiCharacterProvisioningOptions === "object"
        ? { ...options.suiCharacterProvisioningOptions }
        : {};
    const envSetting = String(
      process.env.EVEJS_SUI_CHARACTER_PROVISIONING_ENABLED || "true",
    ).toLowerCase();
  ...
  _buildSuiCharacterProvisioningOptions(characterID) {
    ...
    const existingPreparedHook =
      typeof baseOptions.onTransactionPrepared === "function"
    ...
    return {
      ...
      onTransactionPrepared: async (details) => {
        ...
        const transactionBytesBase64 = String(
          (details && details.transactionBytesBase64) || "",
        ).trim();
        const transactionSignature = String(
          (details && details.transactionSignature) || "",
        ).trim();
    ...
    try {
      ...
      if (
        ...
        throw new SuiCharacterProvisioningError(
          ...
          "Stored pending Sui identity does not match its deterministic identity",
          { ambiguous: true },
        );
      }
    } catch (error) {
      log.warn(
        `[CharService] Cannot resume pending Sui Character char=${characterID} ` +
          `code=${error && error.code ? error.code : "UNKNOWN"}`,
      );
      throwWrappedUserError("CustomInfo", {
        info:
          "The pending Sui Character binding needs administrator reconciliation.",
      });
    }

    log.info(
      `[CharService] Resuming pending Sui Character provisioning char=${characterID}`,
    );
        ...
        this.suiCharacterProvisioner(
          {
            ...
            transactionBytesBase64:
              record.suiPreparedTransactionBytesBase64,
      ...
      .then(
        (suiResult) => {
          ...
          if (!bindingWrite.success) {
            log.warn(
              `[CharService] Resumed Sui Character binding could not be saved char=${characterID}`,
            );
            throwWrappedUserError("CustomInfo", {
              info:
                "The Sui Character was recovered, but its local binding could not be saved.",
            });
          }
          ...
          if (!welcomeMailResult.success) {
            log.warn(
              `[CharService] Failed to send welcome mail after Sui recovery char=${characterID}: ${welcomeMailResult.errorMsg}`,
            );
          }
          ...
          log.success(
            `[CharService] Reconciled Frontier character ${characterID} with Sui character=${suiResult.characterObjectId}`,
          );
          return characterID;
        },
        (error) => {
          ...
          const currentPendingRecord =
            getCharacterRecord(characterID) || record;
          const storedDigest = String(
            currentPendingRecord.suiTransactionDigest || "",
          ).trim();
          const failedDigest = String(
            (error && error.transactionDigest) || "",
          ).trim();
          const isDigestConfirmedFailure =
            !isAmbiguous &&
          ...
          if (isDigestConfirmedFailure) {
            ...
            log.warn(
              `[CharService] Pending Sui Character failed definitively char=${characterID} ` +
                `code=${errorCode} rollback=${rollback.success}`,
            );
            throwWrappedUserError("CustomInfo", {
              info:
                "The pending Sui Character transaction failed and was rolled back. Please create the character again.",
            });
          }
          ...
          repo.flushTablesSync(["characters", "items"]);
          log.warn(
            `[CharService] Pending Sui Character remains unresolved char=${characterID} ` +
              `code=${errorCode}`,
          );
          throwWrappedUserError("CustomInfo", {
            info:
              "The Sui Character transaction is still being reconciled. Please try again shortly.",
          });
        },
      );
  }
  ...
  Handle_GetCharactersToSelect(args, session) {
    ...
    const charList: any[] = [];
    for (const [charId, charData] of Object.entries<any>(characters)) {
  ...
  Handle_GetCharacterSelectionData(args, session) {
    ...
    if (accounts) {
      for (const [username, acct] of Object.entries<any>(accounts)) {
    ...
    const characterDetails: any[] = [];
    for (const [charId, rawCharacter] of Object.entries<any>(characters)) {
   ...
   */
  Handle_GetCharacterToSelect(args, session, kwargs?: any) {
   ...
   */
  Handle_CreateCharacterWithDoll(args, session, kwargs, creationOptions: Record<string, any> = {}) {
    ...
    try {
      ...
      const rollback = rollbackNewCharacterBootstrap(newCharId);
      log.warn(
        `[CharService] Sui Character identity preparation failed char=${newCharId} ` +
    ...
    if (!pendingWrite.success || !pendingFlush.success) {
      const rollback = rollbackNewCharacterBootstrap(newCharId);
      log.warn(
        `[CharService] Failed to persist pending Sui Character identity char=${newCharId} rollback=${rollback.success}`,
      );
      throwWrappedUserError("CustomInfo", {
    ...
    const suiProvisioningOptions =
      this._buildSuiCharacterProvisioningOptions(newCharId);
    return Promise.resolve()
      .then(() =>
        this.suiCharacterProvisioner(
          {
      ...
      .then(
        (suiResult) => {
          ...
          if (!bindingWrite.success) {
            log.warn(
              `[CharService] Sui Character was created but its local binding could not be saved char=${newCharId}`,
            );
            throwWrappedUserError("CustomInfo", {
              info:
                "The Sui Character was created, but its local binding needs reconciliation.",
            });
          }
          log.success(
            `[CharService] Bound Frontier character ${newCharId} to Sui wallet=${suiResult.walletAddress} ` +
        ...
        (error) => {
          ...
          if (isAmbiguous) {
            ...
            repo.flushTablesSync(["characters", "items"]);
            log.warn(
              `[CharService] Sui Character provisioning requires reconciliation char=${newCharId} code=${errorCode}`,
            );
            throwWrappedUserError("CustomInfo", {
              info:
                "The Sui Character transaction is still being reconciled. Please do not create a duplicate character.",
            });
          }

          const rollback = rollbackNewCharacterBootstrap(newCharId);
          log.warn(
            `[CharService] Sui Character provisioning failed char=${newCharId} code=${errorCode} rollback=${rollback.success}`,
          );
          throwWrappedUserError("CustomInfo", {
            info:
              "The Sui Character could not be created on the local network. Please try again.",
          });
        },
      );
  }
  ...
  Handle_CreateCharacterInSpace(args, session) {
    ...
    if (this.suiCharacterProvisioningEnabled) {
      const pendingCharacter = findPendingSuiCharacter(
  ...
  Handle_GetNumCharacters(args, session) {
    ...
    const charactersResult = repo.read("characters", "/");
    const characters = charactersResult.success ? charactersResult.data : {};
    return Object.values<any>(characters).filter(
  ...
  async Handle_SelectCharacterID(args, session, kwargs) {
    ...
    if (
      characterRecord &&
      this.suiCharacterProvisioningEnabled &&
      ["pending", "reconciliation-required"].includes(
        String(characterRecord.suiProvisioningStatus || ""),
      )
    ) {
    ...
    if (characterRecord) {
      try {
        ...
        log.warn(
          `[CharService] Character-control preflight failed for ${charId}: ${error && error.message ? error.message : "unavailable"}`,
        );
        throwWrappedUserError("CustomInfo", {
