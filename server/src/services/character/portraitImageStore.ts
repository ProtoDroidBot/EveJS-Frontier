...
function storeCharacterPortrait(charId, bytes, options: Record<string, any> = {}) {
  const numericCharId = toNumber(charId, 0);
...
function storeCharacterPortraitAsync(charId, bytes, options: Record<string, any> = {}) {
  const numericCharId = toNumber(charId, 0);
  ...
  const write = previous.catch(() => {}).then(async () => {
    ...
    try {
      for (let index = 0; index < sizes.length; index++) {
        const finalPath = buildPortraitPath(
          portraitRoot,
          numericCharId,
          sizes[index],
          extension,
      ...
      await Promise.allSettled(
        tempPaths.map((tempPath) => fs.promises.rm(tempPath, { force: true })),
      );
      return {
        success: false as const,
...
async function findCharacterPortraitPathAsync(charId, size = null) {
  const numericCharId = toNumber(charId, 0);
  if (numericCharId <= 0) return null;
