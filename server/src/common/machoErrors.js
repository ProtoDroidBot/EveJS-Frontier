...
const WRONG_MACHO_NODE_CLASS = "carbon.common.script.net.machoNetExceptions.WrongMachoNode";
...
function throwWrappedObject(className = "", args = [], state = null) {
    throw new MachoWrappedException(buildWrappedObjectPayload(className, args, state));
}
function buildCacheOkPayload() {
    return buildWrappedObjectPayload("carbon.common.script.net.objectCaching.CacheOK", ["CacheOK"]);
}
...
function buildWrongMachoNodePayload(payload = 0) {
    return buildWrappedObjectPayload(WRONG_MACHO_NODE_CLASS, [], { payload });
}
...
function throwWrappedRaffleCreateError(reason = "UnknownError") {
    ...
    throwWrappedObject("raffles.CreateError", [normalizedReason], { msg: normalizedReason });
}
function throwWrappedRaffleError(className = "raffles.RafflesError", message = null) {
    ...
    const normalizedMessage = String(message || normalizedClassName.split(".").pop() || "UndhandledRaffleError");
    throwWrappedObject(normalizedClassName, [normalizedMessage], { msg: normalizedMessage });
}
...
//# sourceMappingURL=machoErrors.js.map