...
class PyDict extends PyRep {
    ...
    setItem(key, value) {
        ...
        for (const entry of this._entries) {
            if (entry.key instanceof PyString &&
                key instanceof PyString &&
                entry.key.value === key.value) {
                entry.value = value;
                return;
            }
            if (entry.key instanceof PyInt &&
    ...
    getItem(key) {
        const keyStr = key instanceof PyString
            ? key.value
    ...
    dump(indent = "") {
        const inner = this._entries
            .map((e) => `${indent}  ${e.key ? e.key.dump() : "null"} => ${e.value ? e.value.dump() : "null"}`)
            .join("\n");
...
class PyObjectEx extends PyRep {
    ...
    dump(indent = "") {
        return (`${indent}ObjectEx(type2=${this._isType2}):\n` +
...
class PySubStream extends PyRep {
    ...
    clone() {
        return new PySubStream(this._data ? this._data.clone() : null, this._decoded ? this._decoded.clone() : null);
    }
    dump(indent = "") {
        if (this._decoded) {
...
class PyChecksumedStream extends PyRep {
    ...
    clone() {
        return new PyChecksumedStream(this._data ? this._data.clone() : null, this._checksum);
    }
    dump(indent = "") {
        return (`${indent}ChecksumedStream(sum=0x${this._checksum.toString(16)}):\n` +
...
//# sourceMappingURL=pyTypes.js.map