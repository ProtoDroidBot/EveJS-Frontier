...
class MachoWrappedException extends Error {
  declare machoErrorResponse: any;
  declare name: any;
...
function buildWrappedObjectPayload(className = "", args: any[] = [], state = null) {
  const header: any[] = [
  ...
  if (state && typeof state === "object") {
    header.push({
      type: "dict",
      entries: Object.entries<any>(state),
    });
  }
...
function buildUserErrorPayload(message = "", values: Record<string, any> = {}) {
...
function throwWrappedObject(className = "", args: any[] = [], state = null) {
...
function throwWrappedUserError(message = "", values: Record<string, any> = {}) {
