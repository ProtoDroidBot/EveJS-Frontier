...
const { getTypeName, MACHONETMSG_TYPE } = require(path.join(__dirname, "./packetTypes"));
const { decodeAddress, encodeAddress, strVal, unwrapNamedObject, } = require(path.join(__dirname, "./machoAddress"));
...
function decodePacket(decoded) {
    ...
    if (!Array.isArray(tup) || tup.length < 6) {
        log.warn(`[PyPacket] Cannot decode: expected array of 6+, got ${Array.isArray(tup) ? `array(${tup.length})` : typeof tup}`);
        return null;
    }
...
function decodeCallRequest(payload) {
    ...
    try {
        ...
        log.debug(`[PyPacket] Call: method="${method}" args=${args.length} remoteObj=${remoteObject}`);
        return {
            method,
            args,
            kwargs: argsDict,
...
//# sourceMappingURL=pyPacket.js.map