        ...
        if (file.isFile() && file.name.endsWith(".js")) {
            try {
                ...
                if (service.enabled === true) {
                    ...
                    log.debug(`skipping service: ${service.serviceName} as it is not enabled`);
                }
                log.spacer();
            }
            catch (err) {
                log.err(`failed to start secondary service ${fullPath}: ${err.message}`);
                log.spacer();
            }
        }
    }
...
//# sourceMappingURL=secondaryServiceLoader.js.map