...
const { DEFAULT_CHARACTER_PORTRAIT_PATH, findCharacterPortraitPath, findCharacterPortraitPathAsync, } = require("../../services/character/portraitImageStore");
const { DEFAULT_FACTION_LOGO_PATH, findFactionLogoPath, findFactionLogoPathAsync, } = require("../../services/faction/factionImageStore");
const { getFactionIDForCorporation, getFactionRecord, } = require("../../services/faction/factionState");
const { findAllianceLogoPath, findAllianceLogoPathAsync, } = require("../../services/corporation/allianceImageStore");
const IMAGE_ROOT = __dirname;
const DEFAULT_CORPORATION_LOGO_PATH = path.join(IMAGE_ROOT, "images", "evejscorp.png");
const DEFAULT_ALLIANCE_LOGO_PATH = path.join(IMAGE_ROOT, "images", "alliance-default.png");
...
function getContentTypeForFilePath(filePath, fallbackExtension = "") {
    const fileExtension = path.extname(String(filePath || "")).replace(/^\./, "") || fallbackExtension;
...
function resolveLegacyImagePath(pathname) {
    const match = String(pathname || "").match(/^\/(Character|Corporation|Alliance|Faction)\/(\d+)(?:_(\d+))?\.(png|jpg|jpeg)$/i);
    if (!match) {
        return null;
    }
...
function resolveRestImagePath(url) {
    const match = String(url.pathname || "").match(/^\/(characters|corporations|alliances|factions)\/(\d+)\/(portrait|logo)$/i);
    if (!match) {
        return null;
    }
...
async function resolveCharacterImagePathAsync(characterID, size, extension) {
    ...
    return localPath || (String(extension || "").toLowerCase() === "png"
        ? DEFAULT_PNG_PATH
        : DEFAULT_CHARACTER_PORTRAIT_PATH);
}
...
async function resolveImageRequestAsync(requestUrl) {
    const url = buildRequestUrl(requestUrl);
    const legacy = String(url.pathname || "").match(/^\/(Character|Corporation|Alliance|Faction)\/(\d+)(?:_(\d+))?\.(png|jpg|jpeg)$/i);
    if (legacy) {
    ...
    const rest = String(url.pathname || "").match(/^\/(characters|corporations|alliances|factions)\/(\d+)\/(portrait|logo)$/i);
    if (rest) {
...
//# sourceMappingURL=imageRequestResolver.js.map