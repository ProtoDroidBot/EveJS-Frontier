...
const { scheduleSynchronizedTimeDilationForSystems, } = require(path.join(__dirname, "../../utils/synchronizedTimeDilation"));
const DEFAULT_PORT = 26400;
const DEFAULT_HOST = "127.0.0.1";
// ---------- Region / constellation name lookup ----------
// Loaded once on first use from the bundled EVE static data.
const STATIC_DATA_DIR = path.join(__dirname, "../../../../data/eve-online-static-data-3294658-jsonl");
...
function getSessionCountByScene() {
    ...
    try {
        for (const session of sessionRegistry.getSessions()) {
            const systemID = Number((session && session._space && session._space.systemID) ||
