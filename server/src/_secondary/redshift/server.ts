...
function loadMapNames() {
  ...
  const result: Record<string, any> = { constellations: new Map(), regions: new Map() };
  try {
...
function collectSystemSnapshot() {
  ...
  for (const [systemID, scene] of spaceRuntime.scenes.entries()) {
    ...
    const factorHistory = Array.isArray(scene._recentFactors)
      ...
      : [];
    const info: Partial<ReturnType<typeof resolveSystemInfo>> = resolveSystemInfo(systemID) || {};
    scenes.push({
