...
function dedupePositiveIntegers(values: any[] = []) {
  return [...new Set(
...
function getCorporationRoleMask(session): bigint {
  try {
...
function buildMergedLocalRecord(roomName, solarSystemID?: any) {
...
function cleanupExpiredEntries(record) {
  ...
  for (const [characterID, entry] of Object.entries<any>(nextRecord.mutedCharacters)) {
    if (Number(entry && entry.untilMs) > 0 && Number(entry.untilMs) <= nowMs) {
      delete nextRecord.mutedCharacters[characterID];
    }
  }
  for (const [characterID, entry] of Object.entries<any>(nextRecord.bannedCharacters)) {
    if (Number(entry && entry.untilMs) > 0 && Number(entry.untilMs) <= nowMs) {
...
function ensureChannel(roomName, options: Record<string, any> = {}) {
  const normalizedRoomName = normalizeString(roomName, "").trim();
...
function syncEnsuredChannel(roomName, options: Record<string, any> = {}) {
...
function getChannelAccessFailure(session, record, options: Record<string, any> = {}) {
  const characterID = getCharacterID(session);
...
function listChannelTemporaryRestrictions(roomName, category) {
  ...
  return Object.values<any>(restrictionMap)
    .map((entry) => ({
...
function assertChannelAccess(session, roomName, options: Record<string, any> = {}) {
  const record = ensureChannel(roomName);
  if (!record) {
    const error: Error & Record<string, any> = new Error("Channel not found");
  ...
  const error: Error & Record<string, any> = new Error(`Channel access denied: ${failure}`);
  error.code = failure;
  throw error;
}
...
function joinChannel(session, roomName, options: Record<string, any> = {}) {
...
function getChannelBacklog(roomName, limit = 50, options: Record<string, any> = {}) {
  return chatStore.listBacklogEntries(roomName, limit, options);
}

function sendChannelMessage(session, roomName, message, options: Record<string, any> = {}) {
  ...
  if (mutedEntry) {
    const error: Error & Record<string, any> = new Error("Muted");
    error.code = "muted";
    throw error;
  }
...
function ensureCorpChannel(corporationID, options: Record<string, any> = {}) {
...
function ensureFleetChannel(fleetID, options: Record<string, any> = {}) {
...
function ensureResourceWarsChannel(instanceID, options: Record<string, any> = {}) {
...
function ensurePlayerChannel(playerChannelID, options: Record<string, any> = {}) {
...
function createPlayerChannel(session, options: Record<string, any> = {}) {
...
function ensurePrivateChannelForCharacters(leftCharacterID, rightCharacterID, options: Record<string, any> = {}) {
...
function ensureIncursionChannel(taleID, options: Record<string, any> = {}) {
...
function ensureSpreadingIncursionChannel(taleID, options: Record<string, any> = {}) {
  const numericTaleID = normalizePositiveInteger(taleID, 0);
  if (!numericTaleID) {
    return null;
  }
...
function ensurePrivateChannelForInvite(session, targetCharacterID, options: Record<string, any> = {}) {
...
function setChannelMotd(roomName, motd, options: Record<string, any> = {}) {
...
function unmuteChannelCharacter(roomName, characterID) {
  return updateChannel(roomName, (record) => {
    const mutedCharacters: Record<string, any> = { ...(record.mutedCharacters || {}) };
...
function unbanChannelCharacter(roomName, characterID) {
  return updateChannel(roomName, (record) => {
    const bannedCharacters: Record<string, any> = { ...(record.bannedCharacters || {}) };
...
function getLocalSessionsForRoom(roomName, options: Record<string, any> = {}) {
  const parsed = parseLocalChatRoomName(roomName);
  if (!parsed) {
    return [];
  }
...
function getVisibleLocalSessions(roomName, options: Record<string, any> = {}) {
...
function getLocalGatewayMembershipPayload(session, options: Record<string, any> = {}) {
...
function forgetLocalSpeaker(session, options: Record<string, any> = {}) {
...
function leaveLocalLsc(session, options: Record<string, any> = {}) {
  const roomName = normalizeString(options.roomName, "").trim() || getLocalChatRoomNameForSession(session);
  const members = localLscMembership.get(roomName);
  if (!members) {
...
function publishLocalMembershipListForSession(session, options: Record<string, any> = {}) {
...
function deleteChannel(roomName, options: Record<string, any> = {}) {
  const record = ensureChannel(roomName);
  if (!record) {
    return false;
  }
...
function resetRuntimeState(options: Record<string, any> = {}) {
  roomActivity.clear();
...
function ensureAllianceChannel(allianceID, options: Record<string, any> = {}) {
...
function ensureFactionChannel(factionID, options: Record<string, any> = {}) {
