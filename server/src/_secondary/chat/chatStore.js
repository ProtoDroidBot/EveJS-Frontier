...
const { getXmppConferenceDomain, getXmppConferenceDomainPattern, } = require(path.join(__dirname, "../../services/chat/xmppConfig"));
const database = require(path.join(__dirname, "../../gameStore"));
const DATA_ROOT = path.resolve(process.env.EVEJS_CHAT_DATA_ROOT || path.join(__dirname, "../data/chat"));
...
function normalizeBacklogPayload(payload = {}) {
    ...
    const entriesByRoomName = payload &&
...
function loadBacklogFromStore() {
    const stored = normalizeBacklogPayload(readRuntimeTable(CHAT_BACKLOG_TABLE, buildDefaultBacklogState()));
...
function normalizeChannelRecord(record = {}) {
    ...
    return {
        ...
        operatorCharacterIDs: type === "player" && ownerCharacterID > 0
        ...
        allowedParticipantCharacterIDs: normalizeUniqueIntegerList(record.allowedParticipantCharacterIDs),
...
function normalizeStatePayload(parsed = {}) {
    ...
    nextState.nextPlayerChannelID = Math.max(1000000, normalizePositiveInteger(parsed && parsed.nextPlayerChannelID, 1000000));
    nextState.nextPrivateChannelID = Math.max(1, normalizePositiveInteger(parsed && parsed.nextPrivateChannelID, 1));
    ...
    const privateChannelByPair = parsed &&
...
function loadStateFromStore() {
    const stored = normalizeStatePayload(readRuntimeTable(CHAT_STATE_TABLE, buildDefaultState()));
...
function normalizeDiscoveryPayload(parsed = {}) {
    ...
    const observations = parsed && parsed.observations && typeof parsed.observations === "object"
        ? parsed.observations
        : {};
...
function loadDiscoveryFromStore() {
    const stored = normalizeDiscoveryPayload(readRuntimeTable(CHAT_DISCOVERY_TABLE, buildDefaultDiscovery()));
...
function appendBacklogEntry(roomName, entry, options = {}) {
    ...
    const trimmedEntries = limit > 0 ? nextEntries.slice(-limit) : nextEntries;
