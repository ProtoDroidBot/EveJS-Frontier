...
const { getStaticChannelContract, listDiscoverableStaticChannelContracts, listVerifiedStaticChannelContracts, } = require("./staticChannelContracts");
const sessionRegistry = require(path.join(__dirname, "../../services/chat/sessionRegistry"));
const { getCurrentSolarSystemID, getLocalChatRoomNameForSession, getLocalChatRoomNameForSolarSystemID, isDelayedLocalChatRoomName, isLocalChatRoomName, parseLocalChatRoomName, } = require(path.join(__dirname, "../../services/chat/channelRules"));
const { normalizeRoleValue, } = require(path.join(__dirname, "../../services/account/accountRoleProfiles"));
const { CORP_ROLE_CHAT_MANAGER, CORP_ROLE_DIRECTOR, } = require(path.join(__dirname, "../../services/corporation/corporationRuntimeState"));
...
function getCharacterID(session) {
    return normalizePositiveInteger(session && (session.characterID || session.charid || session.userid), 0);
}
...
function getSelectedCharacterID(session) {
    return normalizePositiveInteger(session && (session.characterID || session.charid), 0);
}
...
function getCorporationID(session) {
    return normalizePositiveInteger(session && (session.corporationID || session.corpid), 0);
}
function getAllianceID(session) {
    return normalizePositiveInteger(session && (session.allianceID || session.allianceid), 0);
}
function getWarFactionID(session) {
    return normalizePositiveInteger(session && (session.warFactionID || session.warfactionid), 0);
}
...
function buildCharacterSummary(session) {
    return {
        ...
        name: normalizeString(session && (session.characterName || session.userName), "Unknown"),
...
function buildLocalRecord(roomName, solarSystemID) {
    ...
    const resolvedSolarSystemID = normalizePositiveInteger(solarSystemID || (parsed && parsed.solarSystemID), 0);
    const resolvedRoomName = normalizeString(roomName || getLocalChatRoomNameForSolarSystemID(resolvedSolarSystemID), "").trim();
    return {
...
function parseKnownRoom(roomName) {
    ...
    if (staticContract) {
        return {
            ...
            contractBacked: true,
            metadata: staticContract.metadata && typeof staticContract.metadata === "object"
...
function ensureChannel(roomName, options = {}) {
    ...
    if (existing) {
        ...
        const normalizedExisting = (parsed &&
            parsed.static === true &&
    ...
    const created = chatStore.setChannelRecord({
        ...
        verifiedContract: options.verifiedContract !== undefined
        ...
        allowedParticipantCharacterIDs: dedupePositiveIntegers(options.allowedParticipantCharacterIDs),
...
function syncEnsuredChannel(roomName, options = {}) {
    ...
    const shouldPatch = hasOwnOption(options, "type") ||
    ...
    return updateChannel(record.roomName, (currentRecord) => ({
        ...
        ...(hasOwnOption(options, "type")
            ? {
                type: normalizeString(options.type, currentRecord.type)
                    .trim()
        ...
        ...(hasOwnOption(options, "scope")
            ? {
                scope: normalizeString(options.scope, currentRecord.scope)
                    .trim()
        ...
        ...(hasOwnOption(options, "ownerCharacterID")
            ? {
                ownerCharacterID: normalizePositiveInteger(options.ownerCharacterID, currentRecord.ownerCharacterID),
            }
            : {}),
        ...(hasOwnOption(options, "password")
            ? {
        ...
        ...(hasOwnOption(options, "contractSource")
            ? {
                contractSource: normalizeString(options.contractSource, currentRecord.contractSource),
            }
            : {}),
        ...(hasOwnOption(options, "memberless")
            ? {
        ...
        ...(hasOwnOption(options, "backlogLimit")
            ? {
                backlogLimit: Math.max(0, Number(options.backlogLimit) || currentRecord.backlogLimit || 0),
            }
            : {}),
        ...(hasOwnOption(options, "inviteToken")
            ? {
...
function isChannelAdmin(session, record) {
    ...
    if (record.type === "corp") {
        ...
        if ((corpRoles & CORP_ROLE_CHAT_MANAGER) !== 0n ||
...
function getChannelAccessFailure(session, record, options = {}) {
    ...
    if (record.type === "private") {
        const allowedParticipants = dedupePositiveIntegers(record.allowedParticipantCharacterIDs);
        if (allowedParticipants.length > 0 && !allowedParticipants.includes(characterID)) {
            return "private_mismatch";
        }
    }
    if (record.inviteOnly &&
        !channelAdmin &&
        !(record.invitedCharacters || []).includes(characterID)) {
        return "invite_required";
    }
    const allowListsConfigured = ((record.allowCharacterIDs || []).length > 0 ||
...
function listChannelTemporaryRestrictions(roomName, category) {
    ...
    const restrictionMap = normalizedCategory === "mute"
...
function sendChannelMessage(session, roomName, message, options = {}) {
    ...
    const entry = {
        ...
        characterID,
        characterName: normalizeString(session && (session.characterName || session.userName), "Unknown"),
...
function ensurePlayerChannel(playerChannelID, options = {}) {
    ...
    return syncEnsuredChannel(`player_${numericChannelID}`, {
        ...
        verifiedContract: true,
        contractSource: "client_verified",
        inviteToken: normalizeString(options.inviteToken, "").trim() ||
...
function createPlayerChannel(session, options = {}) {
    ...
    const record = ensureChannel(roomName, {
        ...
        password: normalizeString(options.password, ""),
        passwordRequired: Boolean(options.password),
        inviteToken: normalizeString(options.inviteToken, "").trim() || roomName,
        ownerCharacterID,
...
function ensurePrivateChannelForCharacters(leftCharacterID, rightCharacterID, options = {}) {
    ...
    const inviteToken = normalizeString(options.inviteToken, "").trim() ||
        `private_${privateConversationID}`;
    const record = syncEnsuredChannel(roomName, {
        ...options,
        displayName: "Private Chat",
        inviteOnly: options.inviteOnly !== undefined ? Boolean(options.inviteOnly) : true,
        static: false,
        verifiedContract: false,
        contractSource: "runtime",
        inviteToken,
...
function ensurePrivateChannelForInvite(session, targetCharacterID, options = {}) {
    return ensurePrivateChannelForCharacters(getCharacterID(session), targetCharacterID, options);
}
...
function revokeChannelAdmin(roomName, characterID) {
    return updateChannel(roomName, (record) => ({
        ...record,
        adminCharacterIDs: (record.adminCharacterIDs || []).filter((candidate) => candidate !== normalizePositiveInteger(characterID, 0)),
    }));
}
...
function getLiveSessions() {
    return sessionRegistry.getSessions().filter((session) => (session &&
...
function collectPreferredLiveSessionsByCharacter() {
    ...
    for (const session of getLiveSessions()) {
        ...
        if (typeof sessionRegistry.isPreferredCharacterSession !== "function" ||
...
function getLocalSessionsForRoom(roomName, options = {}) {
    ...
    return [...collectPreferredLiveSessionsByCharacter().values()].filter((session) => {
...
function forgetLocalSpeaker(session, options = {}) {
    ...
    if (removed && isDelayedLocalChatRoomName(roomName)) {
        runtimeEmitter.emit("local-leave", {
            roomName,
            solarSystemID: normalizePositiveInteger(options.solarSystemID, 0) ||
...
function joinLocalLsc(session) {
    ...
    return {
        roomName,
        joined,
        sessions: [...members].filter((candidate) => (candidate && candidate.socket && !candidate.socket.destroyed)),
    };
}
function leaveLocalLsc(session, options = {}) {
    ...
    return {
        ...
        left,
        sessions: members
            ? [...members].filter((candidate) => (candidate && candidate.socket && !candidate.socket.destroyed))
            : [],
    };
}
...
function muteLocalCharacter(session, characterID, durationMs, reason) {
    const roomName = getLocalChatRoomNameForSession(session);
    return muteChannelCharacter(roomName, characterID, durationMs, reason, getCharacterID(session));
}
...
function sortDiscoverableConferenceRecords(left, right) {
    ...
    if (!leftIsStatic && !rightIsStatic) {
        const updatedDelta = Math.max(0, Number(right.updatedAtMs) || 0) -
...
function findDiscoverablePlayerChannelsByDisplayName(session, displayName) {
    ...
    return listDiscoverablePlayerChannels(session)
        .filter((record) => {
        const candidateDisplayName = normalizeString(record.displayName, record.roomName).trim().toLowerCase();
        const candidateRoomName = normalizeString(record.roomName, "")
            .trim()
            .toLowerCase();
        return (candidateDisplayName === normalizedDisplayName ||
...
function getLocalLscSessions(roomName) {
    return [...(localLscMembership.get(roomName) || new Set())].filter((session) => (session && session.socket && !session.socket.destroyed));
}
...
//# sourceMappingURL=chatRuntime.js.map