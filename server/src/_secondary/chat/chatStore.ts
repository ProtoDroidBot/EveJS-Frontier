...
function readRuntimeTable(table, fallback: Record<string, any> = {}) {
...
function writeRuntimeTable(table, value, options: Record<string, any> = {}) {
  ensureRuntimeTable(table);
...
function normalizeBacklogPayload(payload: Record<string, any> = {}) {
  ...
  for (const [roomName, entries] of Object.entries<any>(entriesByRoomName)) {
...
function hasBacklogEntries(backlogState) {
  return Object.values<any>((backlogState && backlogState.entriesByRoomName) || {})
...
function writeBacklogEntries(roomName, entries: any[] = []) {
  const normalizedRoomName = normalizeString(roomName, "").trim();
  if (!normalizedRoomName) {
    return false;
  }
...
function normalizeUniqueIntegerList(values: any[] = []) {
  return [...new Set(
...
function normalizeModerationMap(entries: Record<string, any> = {}) {
...
function normalizeChannelRecord(record: Record<string, any> = {}) {
...
function normalizeStatePayload(parsed: Record<string, any> = {}) {
  ...
  const channels = parsed && parsed.channels && typeof parsed.channels === "object"
    ? parsed.channels
    : {};
  for (const [roomName, rawRecord] of Object.entries<any>(channels)) {
  ...
  const privateChannelByPair =
    ...
    typeof parsed.privateChannelByPair === "object"
      ? parsed.privateChannelByPair
      : {};
  for (const [pairKey, roomName] of Object.entries<any>(privateChannelByPair)) {
    const normalizedRoomName = normalizeString(roomName, "").trim();
    if (!normalizedRoomName) {
      continue;
    }
...
function normalizeDiscoveryPayload(parsed: Record<string, any> = {}) {
  ...
  const observations =
    parsed && parsed.observations && typeof parsed.observations === "object"
      ? parsed.observations
      : {};
  for (const [roomName, entries] of Object.entries<any>(observations)) {
    const normalizedRoomName = normalizeString(roomName, "").trim();
    if (!normalizedRoomName || !Array.isArray(entries)) {
      continue;
    }
...
function hasDiscoveryObservations(discovery) {
  return Object.values<any>((discovery && discovery.observations) || {})
    .some((entries) => Array.isArray(entries) && entries.length > 0);
}
...
function deleteChannelRecord(roomName) {
  ...
  for (const [pairKey, mappedRoomName] of Object.entries<any>(getState().privateChannelByPair)) {
...
function listChannelRecords() {
  return Object.values<any>(getState().channels).map((record) => cloneValue(record));
}
...
function appendBacklogEntry(roomName, entry, options: Record<string, any> = {}) {
  const normalizedRoomName = normalizeString(roomName, "").trim();
  if (!normalizedRoomName) {
    return false;
  }
...
function listBacklogEntries(roomName, limit = 50, options: Record<string, any> = {}) {
  const normalizedRoomName = normalizeString(roomName, "").trim();
  if (!normalizedRoomName) {
    return [];
  }
...
function recordStaticContractObservation(roomName, observation: Record<string, any> = {}) {
  const normalizedRoomName = normalizeString(roomName, "").trim();
  if (!normalizedRoomName) {
    return null;
  }
...
function resetAll(options: Record<string, any> = {}) {
