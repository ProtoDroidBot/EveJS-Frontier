...
interface GatewayRequestResult {
  error?: GatewayError;
  cursor?: Readonly<{ epoch: string; sequence: number }> | null;
  accountID?: number;
  characterID?: number;
  userid?: number;
...
function validateOwnedCharacter(runtime, accountValue, characterValue): GatewayRequestResult {
...
function parseEventUpgradeRequest(req): GatewayRequestResult {
  let requestUrl;
  try {
...
function parseEventCursorParams(requestUrl: URL): GatewayRequestResult {
...
function parseSessionEventUpgradeRequest(requestUrl: URL): GatewayRequestResult {
...
function mountEvejsWebGatewayUpgrades(server, runtimeContext = null, options: Record<string, any> = {}) {
  if (
    !server ||
  ...
  const activeSockets = new Set<GatewaySocket>();
  ...
  function onUpgrade(request, socket, head) {
    ...
    const runtime = getGatewayRuntime(runtimeContext);
    let ownership: GatewayRequestResult;
    try {
  ...
  function shutdown() {
    ...
    subscriptions.clear();
    const closingSockets: any[] = [...activeSockets];
...
function sendCharacterControlFailure(res, error) {
  const code = String(error && error.code || "");
  const knownStatuses: Record<string, any> = {
