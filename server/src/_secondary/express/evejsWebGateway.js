...
function authorizeGatewayRequest(req) {
    ...
    const suppliedToken = String(req.headers[GATEWAY_TOKEN_HEADER] || "").trim() || readBearerToken(req);
...
function authorizeGatewayUpgrade(req) {
    ...
    if (!configuredToken) {
        ...
        return (pathname === SESSION_EVENTS_PATH &&
...
function sendRuntimeNotReady(res) {
    sendGatewayError(res, 503, RUNTIME_NOT_READY_CODE, "Authoritative EveJS gateway runtime is not ready.");
}
...
function runtimeDependencies(runtimeContext) {
    ...
    const reported = runtime && runtime.dependencies && typeof runtime.dependencies === "object"
        ...
        : {};
    return Object.fromEntries(DEPENDENCY_NAMES.map((name) => [name, reported[name] === true]));
}
...
function parseEventCursorParams(requestUrl) {
    ...
    if (!/^[A-Za-z0-9_-]{1,128}$/.test(epoch) ||
...
function mountEvejsWebGatewayUpgrades(server, runtimeContext = null, options = {}) {
    ...
    const heartbeatIntervalMs = normalizePositiveLimit(options.heartbeatIntervalMs, DEFAULT_EVENT_HEARTBEAT_INTERVAL_MS);
    const maxInboundFrameBytes = normalizePositiveLimit(options.maxInboundFrameBytes, DEFAULT_EVENT_MAX_INBOUND_FRAME_BYTES);
    const maxOutboundFrameBytes = normalizePositiveLimit(options.maxOutboundFrameBytes, DEFAULT_EVENT_MAX_OUTBOUND_FRAME_BYTES);
    const maxBufferedOutputBytes = normalizePositiveLimit(options.maxBufferedOutputBytes, DEFAULT_EVENT_MAX_BUFFERED_OUTPUT_BYTES);
    const shutdownGraceMs = normalizePositiveLimit(options.shutdownGraceMs, DEFAULT_EVENT_SHUTDOWN_GRACE_MS);
    ...
    function openConnection(socket, request, eventRequest) {
        ...
        try {
            const runtime = getGatewayRuntime(runtimeContext);
            const unsubscribe = runtime.subscribeCharacterEvents(eventRequest.characterID, eventRequest.cursor, {
                onFrame(frame) {
    ...
    function openSessionConnection(socket, sessionRequest) {
        ...
        try {
            const runtime = getGatewayRuntime(runtimeContext);
            const unsubscribe = runtime.subscribeSessionEvents(sessionRequest.bridgeSessionID, sessionRequest.userid, sessionRequest.cursor, {
                onFrame(frame) {
                    return sendFrame(socket, frame);
                },
                onClose(details) {
    ...
    function onSessionUpgrade(request, socket, head, requestUrl) {
        ...
        if (stopped || !hasReadyGatewayRuntime(runtimeContext)) {
            rejectUpgrade(socket, 503, RUNTIME_NOT_READY_CODE, "Authoritative EveJS gateway runtime is not ready.");
            return;
        }
        ...
        const runtime = getGatewayRuntime(runtimeContext);
        try {
            runtime.authorizeSessionEvents(sessionRequest.bridgeSessionID, sessionRequest.userid);
        }
        catch (error) {
            const code = String(error && error.code || "");
            if (code === "SESSION_NOT_FOUND" || code === "CALL_INVALID") {
                rejectUpgrade(socket, 404, "SESSION_NOT_FOUND", "Unknown, expired, or released bridge session.");
                return;
            }
            rejectUpgrade(socket, 503, RUNTIME_NOT_READY_CODE, "Authoritative EveJS gateway runtime is not ready.");
            return;
        ...
        try {
            ...
            rejectUpgrade(socket, 400, "INVALID_EVENT_UPGRADE", "Event-stream WebSocket upgrade is invalid.");
        }
    }
    function onUpgrade(request, socket, head) {
        if (!authorizeGatewayUpgrade(request)) {
            rejectUpgrade(socket, 401, "UNAUTHORIZED", "Gateway event-stream request is not authorized.");
            return;
        }
        let requestUrl;
        try {
            ...
            rejectUpgrade(socket, 400, "INVALID_EVENT_REQUEST", "Event-stream request is invalid.");
            return;
        }
        ...
        let ownership;
        try {
            ownership = validateOwnedCharacter(runtime, eventRequest.accountID, eventRequest.characterID);
        }
        catch {
            rejectUpgrade(socket, 503, RUNTIME_NOT_READY_CODE, "Authoritative EveJS gateway runtime is not ready.");
            return;
...
function sendCharacterControlFailure(res, error) {
    ...
    if (knownStatuses[code]) {
        sendGatewayError(res, knownStatuses[code], code, error && error.message
            ? error.message
            : "Character-control operation failed.");
        return;
    }
    sendGatewayError(res, 503, "CHARACTER_CONTROL_UNAVAILABLE", "Authoritative character control is unavailable.");
}
function sendCharacterCommandFailure(res, error) {
    const code = String(error && error.code || "");
    const commandDetails = CHARACTER_COMMAND_ERROR_DETAILS[code];
    if (commandDetails) {
        sendGatewayError(res, commandDetails.status, code, commandDetails.message);
        return;
    }
    ...
    if (PUBLIC_SKILL_QUEUE_ERROR_CODES.has(code)) {
        sendGatewayError(res, 400, code, `Skill queue command was rejected: ${code}.`);
        return;
    }
    if (PUBLIC_PI_RESTART_ERROR_CODES.has(code)) {
        sendGatewayError(res, 400, code, `PI extractor restart command was rejected: ${code}.`);
        return;
    }
    const unavailable = CHARACTER_COMMAND_ERROR_DETAILS.CHARACTER_COMMAND_UNAVAILABLE;
    sendGatewayError(res, unavailable.status, "CHARACTER_COMMAND_UNAVAILABLE", unavailable.message);
}
function sendCallFailure(res, error) {
    const code = String(error && error.code || "");
    if (CALL_ERROR_STATUS_CODES[code]) {
        sendGatewayError(res, CALL_ERROR_STATUS_CODES[code], code, error && error.message ? error.message : "EveJS service call failed.");
        return;
    }
...
function mountEvejsWebGateway(app, runtimeContext = null) {
    ...
    app.get(`${GATEWAY_PREFIX}/snapshot`, requireAuthorizedRuntime((runtime, req, res) => {
        const ownership = validateOwnedCharacter(runtime, req.query.accountID, req.query.characterID);
        if (ownership.error) {
    ...
    app.post(`${GATEWAY_PREFIX}/character-control/claim`, requireAuthorizedRuntime((runtime, req, res) => {
        ...
        try {
            const result = runtime.claimBrowserControl(ownership.characterID, body.controllerID);
            res.status(200).json({
                ok: true,
                source: GATEWAY_SOURCE,
                apiVersion: GATEWAY_API_VERSION,
    ...
    app.post(`${GATEWAY_PREFIX}/character-control/renew`, requireAuthorizedRuntime((runtime, req, res) => {
        ...
        try {
            const result = runtime.renewBrowserControl(ownership.characterID, body.controllerID, body.leaseID, body.leaseSecret);
            res.status(200).json({
                ok: true,
                source: GATEWAY_SOURCE,
                apiVersion: GATEWAY_API_VERSION,
    ...
    app.post(`${GATEWAY_PREFIX}/character-control/release`, requireAuthorizedRuntime((runtime, req, res) => {
        ...
        try {
            const result = runtime.releaseBrowserControl(ownership.characterID, body.controllerID, body.leaseID, body.leaseSecret);
            res.status(200).json({
                ok: true,
                source: GATEWAY_SOURCE,
                apiVersion: GATEWAY_API_VERSION,
    ...
    app.post(`${GATEWAY_PREFIX}/skill-queue`, requireAuthorizedRuntime(async (runtime, req, res) => {
        ...
        try {
            const result = await runtime.submitSkillQueueSaveCommand(ownership.characterID, body.command);
            res.status(200).json({
                ok: true,
                source: GATEWAY_SOURCE,
                apiVersion: GATEWAY_API_VERSION,
            ...
            sendCharacterCommandFailure(res, error);
        }
    }));
    app.post(`${GATEWAY_PREFIX}/pi/restart-extractors`, requireAuthorizedRuntime(async (runtime, req, res) => {
        ...
        try {
            const result = await runtime.submitPiRestartExtractorsCommand(ownership.characterID, body.command);
            res.status(200).json({
                ok: true,
                source: GATEWAY_SOURCE,
                apiVersion: GATEWAY_API_VERSION,
    ...
    app.post(`${GATEWAY_PREFIX}/call`, requireAuthorizedRuntime(async (runtime, req, res) => {
        if (typeof runtime.callServiceMethod !== "function") {
            sendGatewayError(res, 503, "CALL_SERVICE_UNAVAILABLE", "Whitelisted service-call bridge is not available.");
            return;
        }
        const body = req.body || {};
        try {
    ...
    app.post(`${GATEWAY_PREFIX}/session/select`, requireAuthorizedRuntime(async (runtime, req, res) => {
        if (typeof runtime.selectCharacter !== "function") {
            sendGatewayError(res, 503, "CALL_SERVICE_UNAVAILABLE", "Persistent-session bridge is not available.");
            return;
        }
        const body = req.body || {};
        try {
    ...
    app.post(`${GATEWAY_PREFIX}/bound/bind`, requireAuthorizedRuntime(async (runtime, req, res) => {
        if (typeof runtime.bindBoundObject !== "function") {
            sendGatewayError(res, 503, "CALL_SERVICE_UNAVAILABLE", "Bound-object bridge is not available.");
            return;
        }
        const body = req.body || {};
        try {
    ...
    app.post(`${GATEWAY_PREFIX}/session/flight-status`, requireAuthorizedRuntime((runtime, req, res) => {
        if (typeof runtime.readFlightStatus !== "function") {
            sendGatewayError(res, 503, "CALL_SERVICE_UNAVAILABLE", "Flight-status read is not available.");
            return;
        }
        const body = req.body || {};
        try {
    ...
    app.post(`${GATEWAY_PREFIX}/space/snapshot`, requireAuthorizedRuntime((runtime, req, res) => {
        if (typeof runtime.readSpaceSnapshot !== "function") {
            sendGatewayError(res, 503, "CALL_SERVICE_UNAVAILABLE", "Space-snapshot read is not available.");
            return;
        }
        const body = req.body || {};
        try {
    ...
    app.post(`${GATEWAY_PREFIX}/chat/read`, requireAuthorizedRuntime((runtime, req, res) => {
        if (typeof runtime.readChat !== "function") {
            sendGatewayError(res, 503, "CALL_SERVICE_UNAVAILABLE", "Chat read is not available.");
            return;
        }
        const body = req.body || {};
        try {
    ...
    app.post(`${GATEWAY_PREFIX}/chat/send`, requireAuthorizedRuntime((runtime, req, res) => {
        if (typeof runtime.sendChat !== "function") {
            sendGatewayError(res, 503, "CALL_SERVICE_UNAVAILABLE", "Chat send is not available.");
            return;
        }
        const body = req.body || {};
        try {
    ...
    app.get(`${GATEWAY_PREFIX}/market/station-asks`, requireAuthorizedRuntime(async (runtime, req, res) => {
        ...
        try {
            ...
            sendGatewayError(res, 503, "MARKET_UNAVAILABLE", error && error.message ? error.message : "Market daemon unavailable.");
        }
    }));
    app.all(/^\/_evejs-web(?:\/|$)/, (req, res) => {
        ...
        sendGatewayError(res, 404, "GATEWAY_ROUTE_NOT_FOUND", "EveJS web gateway route was not found.");
    });
    app.use((error, req, res, next) => {
        ...
        const requestPath = String(req.originalUrl || req.url || "").split("?", 1)[0];
        if (requestPath !== `${GATEWAY_PREFIX}/health` &&
...
//# sourceMappingURL=evejsWebGateway.js.map