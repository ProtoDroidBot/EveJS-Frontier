...
function resolveGatewayCashCharacterID(requestEnvelope) {
  ...
  const liveCharacterIDs: any[] = [...new Set(
    sessionRegistry
      .getSessions()
      .map((session) =>
        toPositiveInteger(
...
function createNewEdenStoreGatewayService() {
  ...
  const types: Record<string, any> = {
    ...
    offerGetAllByTagResponse: protoRoot.lookupType(
      "eve_public.store.offer.api.GetAllByTagResponse",
    ),
    offerGetResponse: protoRoot.lookupType(
      "eve_public.store.offer.api.GetResponse",
    ),
  };
  ...
  return {
    ...
    handleRequest(requestTypeName, requestEnvelope) {
        ...
        return {
          ...
          responsePayloadBuffer: encodePayload(types.offerGetResponse, {
            attributes: {
              ...
              no_enddate: true,
              can_purchase: false,
              purchase_limited_reason:
                "Purchases are disabled on this local server.",
              no_schedule: true,
              offer_price: {
      ...
      if (requestTypeName === "eve_public.payment.token.api.GetQuickPayRequest") {
        const tokens = Object.values<any>(getQuickPayTokens())
          .map((token) => ({
