...
function sliceWithPage(items, page) {
    ...
    const slicedItems = items.slice(normalizedPage.offset, normalizedPage.offset + normalizedPage.size);
    const nextOffset = normalizedPage.offset + normalizedPage.size;
    return {
        items: slicedItems,
        nextPage: nextOffset < items.length
            ? {
...
function compareSemver(left, right) {
    ...
    for (const fieldName of ["major", "minor", "patch"]) {
        const delta = Number(leftValue[fieldName] || 0) - Number(rightValue[fieldName] || 0);
        if (delta !== 0) {
            return delta;
        }
    }
...
//# sourceMappingURL=gatewayServiceHelpers.js.map