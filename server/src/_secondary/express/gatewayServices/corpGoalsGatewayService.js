...
const { buildCorpGoalsProtoRoot, } = require("./corpGoalsProto");
const { ACTIVE_PROJECT_CAPACITY, GOAL_STATE, closeGoal, createGoal, deleteGoal, getCapacityInfo, getCharacterCorporationID, getContributorSummaryForGoal, getGoal, getGoalsForCorporation, goalToPayload, listActiveGoalIDsForCorporation, listContributorSummariesForCharacter, listContributorSummariesForGoal, listGoalIDsForCorporation, listInactiveGoalIDsForCorporation, listRewardGoalIDsForCharacter, redeemRewardsForGoal, setGoalCurrentProgress, } = require(path.join(__dirname, "../../../services/corporation/corpGoalsState"));
const { encodePayload, getActiveCharacterID, sliceWithPage, uuidStringToBuffer, } = require("./gatewayServiceHelpers");
...
const HANDLED_REQUEST_TYPE_ALIASES = Object.freeze(HANDLED_REQUEST_TYPES.map((requestType) => requestType.replace(".api.", ".api.requests_pb2.")));
...
function normalizeRequestTypeName(requestTypeName) {
    return String(requestTypeName || "").replace(".api.requests_pb2.", ".api.");
}
...
function buildContributorSummaryPayload(goalID, contributor) {
    ...
    const earnings = Number(contributor.rewardTotal || 0) > 0
        ? [
            {
                quantity: {
...
function createCorpGoalsGatewayService() {
    ...
    const types = {
        ...
        goalSetCurrentProgressRequest: protoRoot.lookupType("eve_public.goal.api.SetCurrentProgressRequest"),
        goalGetCapacityResponse: protoRoot.lookupType("eve_public.goal.api.GetCapacityResponse"),
        corpGoalGetAllResponse: protoRoot.lookupType("eve_public.corporationgoal.api.GetAllResponse"),
        corpGoalGetActiveRequest: protoRoot.lookupType("eve_public.corporationgoal.api.GetActiveRequest"),
        corpGoalGetActiveResponse: protoRoot.lookupType("eve_public.corporationgoal.api.GetActiveResponse"),
        corpGoalGetInactiveRequest: protoRoot.lookupType("eve_public.corporationgoal.api.GetInactiveRequest"),
        corpGoalGetInactiveResponse: protoRoot.lookupType("eve_public.corporationgoal.api.GetInactiveResponse"),
        corpGoalGetContributorSummariesForGoalRequest: protoRoot.lookupType("eve_public.corporationgoal.api.GetContributorSummariesForGoalRequest"),
        corpGoalGetContributorSummariesForGoalResponse: protoRoot.lookupType("eve_public.corporationgoal.api.GetContributorSummariesForGoalResponse"),
        corpGoalGetMyContributorSummaryForGoalRequest: protoRoot.lookupType("eve_public.corporationgoal.api.GetMyContributorSummaryForGoalRequest"),
        corpGoalGetMyContributorSummaryForGoalResponse: protoRoot.lookupType("eve_public.corporationgoal.api.GetMyContributorSummaryForGoalResponse"),
        corpGoalGetMyContributorSummariesRequest: protoRoot.lookupType("eve_public.corporationgoal.api.GetMyContributorSummariesRequest"),
        corpGoalGetMyContributorSummariesResponse: protoRoot.lookupType("eve_public.corporationgoal.api.GetMyContributorSummariesResponse"),
        corpGoalRedeemMyRewardsRequest: protoRoot.lookupType("eve_public.corporationgoal.api.RedeemMyRewardsRequest"),
        corpGoalGetMineWithRewardsRequest: protoRoot.lookupType("eve_public.corporationgoal.api.GetMineWithRewardsRequest"),
        corpGoalGetMineWithRewardsResponse: protoRoot.lookupType("eve_public.corporationgoal.api.GetMineWithRewardsResponse"),
    };
    return {
        ...
        handleRequest(requestTypeName, requestEnvelope) {
            ...
            if (requestTypeName === "eve_public.goal.api.GetAllRequest") {
                ...
                const showOnlyState = decoded && decoded.show_only_state !== undefined
                    ? Number(decoded.show_only_state || 0)
                    : null;
                const goals = getGoalsForCorporation(corporationID).filter((goal) => showOnlyState === null ? true : Number(goal.state || 0) === showOnlyState);
                return {
                    statusCode: 200,
                    statusMessage: "",
            ...
            if (requestTypeName === "eve_public.goal.api.DeleteRequest") {
                ...
                const result = deleteGoal(extractGoalID(decoded && decoded.goal), activeCharacterID);
                return {
                    statusCode: result.success ? 200 : 403,
                    statusMessage: result.success ? "" : result.errorMsg || "",
            ...
            if (requestTypeName === "eve_public.goal.api.SetCurrentProgressRequest") {
                const decoded = decodePayload(types.goalSetCurrentProgressRequest, requestEnvelope);
                const result = setGoalCurrentProgress(extractGoalID(decoded && decoded.goal), decoded && decoded.current_progress, decoded && decoded.new_progress, activeCharacterID);
                return {
                    statusCode: result.success ? 200 : 403,
                    statusMessage: result.success ? "" : result.errorMsg || "",
            ...
            if (requestTypeName === "eve_public.corporationgoal.api.GetInactiveRequest") {
                ...
                const paged = listInactiveGoalIDsForCorporation(corporationID, decoded && decoded.ended_timespan, decoded && decoded.page);
                return {
                    statusCode: 200,
                    statusMessage: "",
            ...
            if (requestTypeName === "eve_public.corporationgoal.api.GetContributorSummariesForGoalRequest") {
                const decoded = decodePayload(types.corpGoalGetContributorSummariesForGoalRequest, requestEnvelope);
                const goalID = extractGoalID(decoded && decoded.goal);
                const paged = listContributorSummariesForGoal(goalID, decoded && decoded.page);
                return {
                    statusCode: 200,
                    statusMessage: "",
                    responseTypeName: "eve_public.corporationgoal.api.GetContributorSummariesForGoalResponse",
                    responsePayloadBuffer: encodePayload(types.corpGoalGetContributorSummariesForGoalResponse, {
                        summaries: paged.items.map((summary) => buildContributorSummaryPayload(goalID, summary)),
            ...
            if (requestTypeName === "eve_public.corporationgoal.api.GetMyContributorSummaryForGoalRequest") {
                const decoded = decodePayload(types.corpGoalGetMyContributorSummaryForGoalRequest, requestEnvelope);
                const goalID = extractGoalID(decoded && decoded.goal);
                const summary = getContributorSummaryForGoal(goalID, activeCharacterID);
                return {
                    statusCode: 200,
                    statusMessage: "",
                    responseTypeName: "eve_public.corporationgoal.api.GetMyContributorSummaryForGoalResponse",
                    responsePayloadBuffer: encodePayload(types.corpGoalGetMyContributorSummaryForGoalResponse, summary
                        ? {
            ...
            if (requestTypeName === "eve_public.corporationgoal.api.GetMyContributorSummariesRequest") {
                const decoded = decodePayload(types.corpGoalGetMyContributorSummariesRequest, requestEnvelope);
                const paged = listContributorSummariesForCharacter(activeCharacterID, decoded && decoded.contributed_timespan, decoded && decoded.page);
                return {
                    statusCode: 200,
                    statusMessage: "",
                    responseTypeName: "eve_public.corporationgoal.api.GetMyContributorSummariesResponse",
                    responsePayloadBuffer: encodePayload(types.corpGoalGetMyContributorSummariesResponse, {
                        summaries: paged.items.map((summary) => buildContributorSummaryPayload(summary.goalID || "", summary)),
                        next_page: paged.nextPage || undefined,
                    }),
                };
            }
            if (requestTypeName === "eve_public.corporationgoal.api.RedeemMyRewardsRequest") {
                const decoded = decodePayload(types.corpGoalRedeemMyRewardsRequest, requestEnvelope);
                const result = redeemRewardsForGoal(extractGoalID(decoded && decoded.goal_id), activeCharacterID);
                return {
            ...
            const decoded = decodePayload(types.corpGoalGetMineWithRewardsRequest, requestEnvelope);
...
//# sourceMappingURL=corpGoalsGatewayService.js.map