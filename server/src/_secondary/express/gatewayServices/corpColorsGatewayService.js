...
const { canCharacterEditCorporationColorPalette, getCorporationColorPalette, getCorporationIDForCharacter, setCorporationColorPalette, } = require(path.join(__dirname, "../../../services/corporation/corpColorsState"));
...
function getActiveCharacterID(requestEnvelope) {
    ...
    const activeCharacter = requestEnvelope &&
        requestEnvelope.authoritative_context &&
...
function createCorpColorsGatewayService({ protoRoot, emptyPayload }) {
    const GetRequest = protoRoot.lookupType("eve_public.cosmetic.corporation.palette.api.GetRequest");
    const GetResponse = protoRoot.lookupType("eve_public.cosmetic.corporation.palette.api.GetResponse");
    const GetOwnResponse = protoRoot.lookupType("eve_public.cosmetic.corporation.palette.api.GetOwnResponse");
    const SetRequest = protoRoot.lookupType("eve_public.cosmetic.corporation.palette.api.SetRequest");
    const SetResponseTypeName = "eve_public.cosmetic.corporation.palette.api.SetResponse";
    const CanEditResponse = protoRoot.lookupType("eve_public.cosmetic.corporation.palette.api.CanEditResponse");
    return {
        ...
        handleRequest(requestTypeName, requestEnvelope) {
            ...
            if (requestTypeName === "eve_public.cosmetic.corporation.palette.api.GetRequest") {
                const decoded = GetRequest.decode(Buffer.from(requestEnvelope.payload && requestEnvelope.payload.value));
                const corporationID = Number(decoded &&
                    decoded.identifier &&
                    decoded.identifier.corporation &&
                    decoded.identifier.corporation.sequential) || 0;
            ...
            if (requestTypeName === "eve_public.cosmetic.corporation.palette.api.SetRequest") {
                ...
                const decoded = SetRequest.decode(Buffer.from(requestEnvelope.payload && requestEnvelope.payload.value));
...
//# sourceMappingURL=corpColorsGatewayService.js.map