...
const { bufferFromBytes, compareSemver, encodePayload, getActiveCharacterID, timestampFromMs, timestampToMs, } = require("./gatewayServiceHelpers");
const { getSovereigntyProtoTypes, } = require(path.join(__dirname, "../../../services/sovereignty/sovGatewayProto"));
const { POWER_STATE, addHubFuel, configureWorkforce, getActiveCharacterIdentity, getHubIDForSolarSystem, getHubFuel, getHubResources, getHubUpgrades, getSkyhook, getWorkforceConfiguration, getWorkforceState, listLocalSkyhooks, listNetworkableHubs, listSkyhooksByCorporation, listSolarSystemsWithTheftVulnerableSkyhooks, listTheftVulnerableSkyhooksInSolarSystem, listUpgradeDefinitions, modifySkyhookReagents, processHubUpgradeConfiguration, setSkyhookActivation, uninstallHubUpgrade, } = require(path.join(__dirname, "../../../services/sovereignty/sovModernState"));
const { getSovereigntyStaticSnapshot, } = require(path.join(__dirname, "../../../services/sovereignty/sovStaticData"));
...
function decodePayload(messageType, requestEnvelope) {
    return messageType.decode(bufferFromBytes(requestEnvelope && requestEnvelope.payload && requestEnvelope.payload.value));
}
...
function buildSkyhookPayload(skyhook) {
    return {
        ...
        reagent_simulations: (skyhook.reagentSimulations || []).map(buildSkyhookSimulationPayload),
        active: Boolean(skyhook.active),
        reagent_definitions: (skyhook.reagentDefinitions || []).map(buildSkyhookDefinitionPayload),
        ...
        planet_resources_definitions_version: buildSemanticVersion(skyhook.planetResourcesVersion),
    };
}
function buildSkyhookNoticePayload(skyhook) {
    return {
        ...
        reagent_definitions: (skyhook.reagentDefinitions || []).map(buildSkyhookDefinitionPayload),
        theft_vulnerability: buildSkyhookTheftPayload(skyhook.theftVulnerability),
        workforce_available: Number(skyhook.workforceAmount || 0) > 0
...
function createSovereigntyGatewayService({ publishGatewayNotice }) {
    ...
    function publishHubStateNotices(hubID) {
        ...
        publishNotice("eve_public.sovereignty.hub.api.ResourcesSimulatedNotice", types.hubResourcesSimulatedNotice, {
        ...
        publishNotice("eve_public.sovereignty.hub.upgrade.api.HubUpgradesSimulatedNotice", types.upgradeSimulatedNotice, {
    ...
    function publishSkyhookAllInSystemNotice(solarSystemID, identity) {
        ...
        publishNotice("eve_public.sovereignty.skyhook.api.AllInSolarSystemNotice", types.skyhookAllInSolarSystemNotice, {
    ...
    function publishSkyhookVulnerabilityNotice(skyhook, previousSkyhook = null) {
        ...
        if (!skyhook.active) {
            if (wasVulnerable) {
                publishNotice("eve_public.sovereignty.skyhook.api.TheftVulnerabilityWindowEndedNotice", types.skyhookTheftWindowEndedNotice, {
                    skyhook: buildSequentialIdentifier(skyhook.skyhookID),
        ...
        if (isVulnerable) {
            publishNotice("eve_public.sovereignty.skyhook.api.TheftVulnerabilityWindowStartedNotice", types.skyhookTheftWindowStartedNotice, {
                skyhook: buildSequentialIdentifier(skyhook.skyhookID),
        ...
        if (nextVulnerability &&
            ...
            publishNotice("eve_public.sovereignty.skyhook.api.TheftVulnerabilityWindowScheduledNotice", types.skyhookTheftWindowScheduledNotice, {
                skyhook: buildSequentialIdentifier(skyhook.skyhookID),
        ...
        handleRequest(requestTypeName, requestEnvelope) {
            ...
            if (requestTypeName === "eve_public.sovereignty.resource.planet.api.GetAllDefinitionsRequest") {
                ...
                const knownVersion = decoded && decoded.known_version
                    ...
                    : null;
                if (knownVersion &&
                    compareSemver(knownVersion, staticSnapshot.planetDefinitionsVersion) === 0) {
                ...
                return {
                    ...
                    responsePayloadBuffer: encodePayload(types.planetGetAllDefinitionsResponse, {
                        definitions: staticSnapshot.planetDefinitions.map(buildPlanetDefinitionPayload),
            ...
            if (requestTypeName === "eve_public.sovereignty.resource.planet.api.GetDefinitionsVersionRequest") {
                const decoded = decodePayload(types.planetGetDefinitionsVersionRequest, requestEnvelope);
                const staticSnapshot = getSovereigntyStaticSnapshot();
                const requestedVersion = buildSemanticVersion(decoded && decoded.version);
                if (compareSemver(requestedVersion, staticSnapshot.planetDefinitionsVersion) !==
                    0) {
                    return buildErrorResult(requestTypeName, 404, "NOT_FOUND");
                }
                return {
                    statusCode: 200,
                    statusMessage: "",
                    responseTypeName: "eve_public.sovereignty.resource.planet.api.GetDefinitionsVersionResponse",
                    responsePayloadBuffer: encodePayload(types.planetGetDefinitionsVersionResponse, {
                        definitions: staticSnapshot.planetDefinitions.map(buildPlanetDefinitionPayload),
                    }),
                };
            }
            if (requestTypeName === "eve_public.sovereignty.resource.star.api.GetAllConfigurationsRequest") {
                ...
                return {
                    ...
                    responsePayloadBuffer: encodePayload(types.starGetAllConfigurationsResponse, {
                        configurations: staticSnapshot.starConfigurations.map(buildStarConfigurationPayload),
                    }),
                };
            }
            ...
            if (requestTypeName === "eve_public.sovereignty.hub.upgrade.api.ProcessConfigurationRequest") {
                const decoded = decodePayload(types.upgradeProcessConfigurationRequest, requestEnvelope);
                const hubID = Number(decoded && decoded.hub && decoded.hub.sequential) || 0;
                const result = processHubUpgradeConfiguration(hubID, (decoded && decoded.new_upgrades || []).map((entry) => Number(entry && entry.sequential) || 0), (decoded && decoded.configuration || []).map((entry) => ({
                ...
                publishNotice("eve_public.sovereignty.hub.upgrade.api.HubUpgradesConfiguredNotice", types.upgradeConfiguredNotice, {
                    hub_upgrades: buildHubUpgradesPayload(hubUpgrades),
                ...
                return {
                    statusCode: 200,
                    statusMessage: "",
                    responseTypeName: "eve_public.sovereignty.hub.upgrade.api.ProcessConfigurationResponse",
            ...
            if (requestTypeName === "eve_public.sovereignty.hub.upgrade.api.UninstallRequest") {
                ...
                const hubID = Number(decoded && decoded.upgrade && decoded.upgrade.hub && decoded.upgrade.hub.sequential) || 0;
                const upgradeTypeID = Number(decoded &&
                    decoded.upgrade &&
                    decoded.upgrade.upgrade_type &&
                    decoded.upgrade.upgrade_type.sequential) || 0;
                const result = uninstallHubUpgrade(hubID, upgradeTypeID, identity);
                if (!result.ok) {
                    return buildErrorResult(requestTypeName, result.statusCode, result.errorCode);
                }
                publishNotice("eve_public.sovereignty.hub.upgrade.api.UninstalledNotice", types.upgradeUninstalledNotice, {
                    upgrade: {
            ...
            if (requestTypeName === "eve_public.sovereignty.hub.fuel.api.AddRequest") {
                ...
                const hubID = Number(decoded && decoded.hub && decoded.hub.sequential) || 0;
                const result = addHubFuel(hubID, Number(decoded && decoded.fuel_item && decoded.fuel_item.sequential) || 0, Number(decoded && decoded.amount) || 0, identity);
                if (!result.ok) {
                    return buildErrorResult(requestTypeName, result.statusCode, result.errorCode);
                }
                publishHubStateNotices(result.hubID);
            ...
            if (requestTypeName === "eve_public.sovereignty.hub.workforce.api.GetConfigurationRequest") {
                ...
                return {
                    statusCode: 200,
                    statusMessage: "",
                    responseTypeName: "eve_public.sovereignty.hub.workforce.api.GetConfigurationResponse",
            ...
            if (requestTypeName === "eve_public.sovereignty.hub.workforce.api.ConfigureRequest") {
                ...
                const nextConfiguration = configurationPayload && configurationPayload.import_settings
                    ? {
                        mode: "import",
                        sourceSystemIDs: (configurationPayload.import_settings.sources || []).map((entry) => Number(entry && entry.source && entry.source.sequential) || 0),
                    }
                    : configurationPayload && configurationPayload.export_settings
                        ? {
                            mode: "export",
                            destinationSystemID: Number(configurationPayload.export_settings.destination_system &&
                                configurationPayload.export_settings.destination_system.sequential) || null,
                ...
                publishNotice("eve_public.sovereignty.resource.transfer.workforce.api.ConfiguredNotice", types.workforceConfiguredNotice, {
                ...
                publishNotice("eve_public.sovereignty.resource.transfer.workforce.api.StateChangedNotice", types.workforceStateChangedNotice, {
                    solar_system: buildSequentialIdentifier(result.solarSystemID),
            ...
            if (requestTypeName === "eve_public.sovereignty.hub.workforce.api.GetNetworkableHubsRequest") {
                const decoded = decodePayload(types.workforceGetNetworkableHubsRequest, requestEnvelope);
                const hubID = Number(decoded && decoded.hub && decoded.hub.sequential) || 0;
                ...
                return {
                    statusCode: 200,
                    statusMessage: "",
                    responseTypeName: "eve_public.sovereignty.hub.workforce.api.GetNetworkableHubsResponse",
                    responsePayloadBuffer: encodePayload(types.workforceGetNetworkableHubsResponse, {
            ...
            if (requestTypeName === "eve_public.sovereignty.skyhook.api.GetAllByCorporationRequest") {
                ...
                const corporationID = Number(decoded && decoded.corporation && decoded.corporation.sequential) || 0;
                ...
                return {
                    statusCode: 200,
                    statusMessage: "",
                    responseTypeName: "eve_public.sovereignty.skyhook.api.GetAllByCorporationResponse",
            ...
            if (requestTypeName === "eve_public.sovereignty.skyhook.api.ActivateRequest" || requestTypeName === "eve_public.sovereignty.skyhook.api.DeactivateRequest") {
                const decoded = decodePayload(requestTypeName === "eve_public.sovereignty.skyhook.api.ActivateRequest"
                    ? types.skyhookActivateRequest
                    : types.skyhookDeactivateRequest, requestEnvelope);
                const skyhookID = Number(decoded && decoded.skyhook && decoded.skyhook.sequential) || 0;
                const previousSkyhookResult = getSkyhook(skyhookID, identity);
                const result = setSkyhookActivation(skyhookID, requestTypeName === "eve_public.sovereignty.skyhook.api.ActivateRequest", identity);
                if (!result.ok) {
                    return buildErrorResult(requestTypeName, result.statusCode, result.errorCode);
                }
                publishNotice("eve_public.sovereignty.skyhook.api.ActivationNotice", types.skyhookActivationNotice, {
                    solar_system: buildSequentialIdentifier(result.solarSystemID),
                ...
                publishSkyhookVulnerabilityNotice(result.skyhook, previousSkyhookResult && previousSkyhookResult.ok
                ...
                return {
                    ...
                    statusMessage: "",
                    responseTypeName: requestTypeName.replace(/Request$/, "Response"),
                    responsePayloadBuffer: encodePayload(requestTypeName === "eve_public.sovereignty.skyhook.api.ActivateRequest"
            ...
            if (requestTypeName === "eve_public.sovereignty.skyhook.api.GetSolarSystemsWithTheftVulnerableSkyhooksRequest") {
                return {
                    statusCode: 200,
                    statusMessage: "",
                    responseTypeName: "eve_public.sovereignty.skyhook.api.GetSolarSystemsWithTheftVulnerableSkyhooksResponse",
                    responsePayloadBuffer: encodePayload(types.skyhookGetSolarSystemsWithTheftVulnerableResponse, {
                        solar_systems: listSolarSystemsWithTheftVulnerableSkyhooks().map(buildSequentialIdentifier),
                    }),
                };
            }
            if (requestTypeName === "eve_public.sovereignty.skyhook.api.GetTheftVulnerableSkyhooksInSolarSystemRequest") {
                const decoded = decodePayload(types.skyhookGetTheftVulnerableInSolarSystemRequest, requestEnvelope);
                const solarSystemID = Number(decoded && decoded.solar_system && decoded.solar_system.sequential) || 0;
                const result = listTheftVulnerableSkyhooksInSolarSystem(solarSystemID);
                return {
                    statusCode: 200,
                    statusMessage: "",
                    responseTypeName: "eve_public.sovereignty.skyhook.api.GetTheftVulnerableSkyhooksInSolarSystemResponse",
                    responsePayloadBuffer: encodePayload(types.skyhookGetTheftVulnerableInSolarSystemResponse, {
            ...
            if (requestTypeName === "eve_public.sovereignty.skyhook.api.admin.ModifyReagentsRequest") {
                ...
                const skyhookID = Number(decoded && decoded.skyhook && decoded.skyhook.sequential) || 0;
                const result = modifySkyhookReagents(skyhookID, (decoded && decoded.reagents || []).map((entry) => ({
                    ...
                    timestampMs: entry && entry.timestamp
                ...
                publishNotice("eve_public.sovereignty.skyhook.api.ReagentSimulationsNotice", types.skyhookReagentSimulationsNotice, {
                    skyhook: buildSequentialIdentifier(result.skyhookID),
                    planet: buildSequentialIdentifier(result.planetID),
                    simulations: (result.skyhook.reagentSimulations || []).map(buildSkyhookSimulationPayload),
                }, { solar_system: result.solarSystemID });
                publishSkyhookAllInSystemNotice(result.solarSystemID, identity);
                return {
                    statusCode: 200,
                    statusMessage: "",
                    responseTypeName: "eve_public.sovereignty.skyhook.api.admin.ModifyReagentsResponse",
...
//# sourceMappingURL=sovereigntyGatewayService.js.map