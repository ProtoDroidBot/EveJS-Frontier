...
function buildErrorResult(responseTypeName, errorMsg, params) {
    ...
    if (errorMsg === "FUEL_CAPACITY_EXCEEDED" &&
        params &&
...
function createAssemblyNetworkNodeGatewayService(context) {
    ...
    function publishFuelChangedNotice(commitData) {
        ...
        const unitVolume = commitData.fuelTypeID > 0
            ? Number(networkNodeFuelRuntime.getNetworkNodeFuelStatus(commitData.characterID, commitData.networkNodeID)?.data?.unitVolume) || 0
            : 0;
        ...
        return publishGatewayNotice(FUEL_CHANGED_NOTICE, payload, commitData.solarSystemID > 0
    ...
    function handleGetFuel(characterID, requestEnvelope) {
        ...
        const networkNodeID = toNumber(request && request.network_node && request.network_node.sequential);
        const status = networkNodeFuelRuntime.getNetworkNodeFuelStatus(characterID, networkNodeID);
        if (!status.success) {
    ...
    function buildPrepareResult(responseTypeName, responseType, prepared) {
        return {
            ...
            responsePayloadBuffer: encodePayload(responseType, {
                ...
                prepared_transaction_bcs_data: Buffer.from(prepared.transactionData, "utf8"),
            }),
        };
    }
    function handlePrepareDeposit(characterID, requestEnvelope) {
        const request = decodeRequest(types.PrepareDepositFuelRequest, requestEnvelope);
        if (!request) {
        ...
        const result = networkNodeFuelRuntime.prepareNetworkNodeFuelDeposit({
            characterID,
            networkNodeID: toNumber(request.network_node && request.network_node.sequential),
            sourceItemID: toNumber(request.source && request.source.item && request.source.item.sequential),
        ...
        if (!result.success) {
            return buildErrorResult(PREPARE_DEPOSIT_FUEL_RESPONSE, result.errorMsg, result.params);
        }
        return buildPrepareResult(PREPARE_DEPOSIT_FUEL_RESPONSE, types.PrepareDepositFuelResponse, result.data);
    }
    function handlePrepareWithdraw(characterID, requestEnvelope) {
        const request = decodeRequest(types.PrepareWithdrawFuelRequest, requestEnvelope);
        if (!request) {
        ...
        const result = networkNodeFuelRuntime.prepareNetworkNodeFuelWithdraw({
            ...
            destinationItemID: toNumber(request.destination &&
        ...
        if (!result.success) {
            return buildErrorResult(PREPARE_WITHDRAW_FUEL_RESPONSE, result.errorMsg, result.params);
        }
        return buildPrepareResult(PREPARE_WITHDRAW_FUEL_RESPONSE, types.PrepareWithdrawFuelResponse, result.data);
    }
        ...
        handleRequest(requestTypeName, requestEnvelope) {
            ...
            switch (requestTypeName) {
                ...
                case EXECUTE_WITHDRAW_FUEL_REQUEST:
                    return handleExecute(characterID, requestEnvelope, "networknode-fuel-withdraw", types.ExecuteWithdrawFuelRequest, EXECUTE_WITHDRAW_FUEL_RESPONSE);
                default:
                    return null;
            }
        },
...
//# sourceMappingURL=assemblyNetworkNodeGatewayService.js.map