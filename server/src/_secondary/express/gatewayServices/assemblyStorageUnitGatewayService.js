...
const { getStorageUnitProtoTypes, } = require("./assemblyStorageUnitProto");
const smartStorageUnitRuntime = require(path.join(__dirname, "../../../services/frontier/smartStorageUnitRuntime"));
...
function decodeRequest(type, requestEnvelope) {
    try {
        return type.decode(bufferFromBytes(requestEnvelope && requestEnvelope.payload && requestEnvelope.payload.value));
    }
    catch (_) {
        return null;
    }
...
function buildErrorResult(responseTypeName, errorMsg, params) {
    ...
    if (errorMsg === "STORAGE_CAPACITY_EXCEEDED" &&
        params &&
        Number.isFinite(Number(params.freeVolume))) {
        statusMessage = `The Smart Storage Unit has ${Math.max(0, Math.floor(Number(params.freeVolume))).toLocaleString("en-US")} m³ free.`;
    }
    return {
...
function createAssemblyStorageUnitGatewayService(context) {
    ...
    const publishGatewayNotice = context && typeof context.publishGatewayNotice === "function"
        ...
        : null;
    const customAccessResolver = context && typeof context.resolveStorageUnitAccess === "function"
    ...
    function resolveLiveAccess(characterID, storageUnitID) {
        ...
        const activeShipID = toNumber((session._space && session._space.shipID) || session.shipid || session.shipID);
        const solarSystemID = toNumber(session.solarsystemid2 ||
    ...
    function publishInventoryNotices(typeName, messageType, commitData) {
        ...
        for (const item of commitData.noticeItems || []) {
            publishGatewayNotice(typeName, encodePayload(messageType, {
    ...
    async function handleGetInventory(characterID, requestEnvelope) {
        ...
        const result = await smartStorageUnitRuntime.getStorageInventory({
            ...
            resolveAccess: (candidateStorageUnitID) => resolveLiveAccess(characterID, candidateStorageUnitID),
            characterID,
    ...
    async function handlePrepareDeposit(characterID, requestEnvelope) {
        ...
        if (!result.success) {
            return buildErrorResult(PREPARE_DEPOSIT_ITEMS_RESPONSE, result.errorMsg, result.params);
        }
    ...
    async function handlePrepareWithdraw(characterID, requestEnvelope) {
        ...
        const result = await smartStorageUnitRuntime.prepareStorageWithdraw({
            ...
            destinationFlagID: request.generic_location && request.generic_location.flag
                ...
                : -1,
            destinationLocationID: toNumber(request.generic_location &&
                request.generic_location.item &&
                request.generic_location.item.sequential),
            destinationStorageUnitID: toNumber(request.another_assembly && request.another_assembly.sequential),
            stacks: (request.stacks || []).map((stack) => ({
                typeID: toNumber(stack && stack.item_type && stack.item_type.sequential),
                quantity: toNumber(stack && stack.quantity),
            })),
            storageUnitID,
        });
        if (!result.success) {
            return buildErrorResult(PREPARE_WITHDRAW_ITEMS_RESPONSE, result.errorMsg, result.params);
        }
    ...
    async function handleExecute(characterID, requestEnvelope, action, requestType, responseTypeName, noticeTypeName, noticeMessageType) {
        ...
        if (result.data.replayed !== true) {
            ...
            if (toNumber(result.data.destinationStorageUnitID) > 0) {
                publishInventoryNotices(INVENTORY_ITEM_DEPOSITED_NOTICE, types.InventoryItemDepositedNotice, {
                    ...result.data,
        ...
        handleRequest(requestTypeName, requestEnvelope) {
            ...
            switch (requestTypeName) {
                ...
                case EXECUTE_DEPOSIT_ITEMS_REQUEST:
                    return handleExecute(characterID, requestEnvelope, "storageunit-deposit", types.ExecuteDepositItemsRequest, EXECUTE_DEPOSIT_ITEMS_RESPONSE, INVENTORY_ITEM_DEPOSITED_NOTICE, types.InventoryItemDepositedNotice);
                case EXECUTE_WITHDRAW_ITEMS_REQUEST:
                    return handleExecute(characterID, requestEnvelope, "storageunit-withdraw", types.ExecuteWithdrawItemsRequest, EXECUTE_WITHDRAW_ITEMS_RESPONSE, INVENTORY_ITEM_WITHDRAWN_NOTICE, types.InventoryItemWithdrawnNotice);
                default:
                    return null;
            }
        },
