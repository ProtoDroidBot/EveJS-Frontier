...
const { bufferFromBytes, encodePayload, getActiveCharacterID, } = require("./gatewayServiceHelpers");
const { buildNewEdenStoreGatewayProtoRoot, } = require(path.join(__dirname, "../../../services/newEdenStore/storeGatewayProto"));
const { findPublicOffer, getFastCheckoutOffers, findQuickPayToken, getQuickPayTokens, getStoreConfig, resolveStoreCharacterID, } = require(path.join(__dirname, "../../../services/newEdenStore/storeState"));
const sessionRegistry = require(path.join(__dirname, "../../../services/chat/sessionRegistry"));
const HANDLED_REQUEST_TYPES = Object.freeze([
    "eve_public.store.offer.api.GetRequest",
    "eve_public.store.offer.api.GetAllByTagRequest",
...
function decodePayload(messageType, requestEnvelope) {
    return messageType.decode(bufferFromBytes(requestEnvelope &&
        requestEnvelope.payload &&
        requestEnvelope.payload.value));
}
function getCorrelationSuffix(requestEnvelope) {
    const correlationBytes = bufferFromBytes(requestEnvelope && requestEnvelope.correlation_uuid);
...
function resolveCashOfferForTokenPurchase(order) {
    ...
    if (explicitStoreOfferID) {
        const explicitOffer = findPublicOffer(explicitStoreOfferID) ||
        ...
        const fastCheckoutMatch = getFastCheckoutOffers().find((offer) => {
            const offerID = String(Math.trunc(Number(offer && (offer.id || offer.offerID) || 0)));
            return (offerID === explicitStoreOfferID ||
    ...
    const normalizedQuantity = Math.max(1, Math.trunc(Number(order && order.quantity) || 1));
    const normalizedCostInCents = Math.max(0, Math.trunc(Number(order &&
        order.cost &&
        order.cost.catalog_amount_in_cents) || 0));
    const normalizedCurrency = String(order && order.cost && order.cost.currency
        ? order.cost.currency
        : "USD")
        .trim()
        .toUpperCase();
    ...
    const fastCheckoutMatch = getFastCheckoutOffers().find((offer) => {
        const priceInCents = Math.max(0, Math.trunc(Number(offer && offer.price) * 100 || 0));
        const offerCurrency = String(offer && offer.currency ? offer.currency : "USD")
            .trim()
            .toUpperCase();
        return (priceInCents === normalizedCostInCents &&
...
function computeCostPayload(catalogAmountInCents, currencyCode) {
    ...
    const taxRatePoints = Math.max(0, Math.trunc(Number(storeConfig.defaultCashTaxRatePoints || 0)));
...
function resolvePlexVaultTargetCharacterID(activeCharacterID, decodedRequest) {
    const giftCharacterID = toPositiveInteger(decodedRequest &&
        decodedRequest.gift &&
        decodedRequest.gift.character &&
        decodedRequest.gift.character.sequential, 0);
    return giftCharacterID || activeCharacterID;
}
function resolveLiveGatewayUserID() {
    const liveUserIDs = [...new Set(sessionRegistry
            .getSessions()
...
function resolveGatewayCashCharacterID(requestEnvelope) {
    ...
    const liveCharacterIDs = [...new Set(sessionRegistry
            .getSessions()
            .map((session) => toPositiveInteger(session &&
            (session.characterID ||
                session.charid ||
                session.charID ||
...
function getStoreFulfillment() {
    return require(path.join(__dirname, "../../../services/newEdenStore/storeFulfillment"));
}
function createNewEdenStoreGatewayService() {
    ...
    const types = {
        ...
        tokenGetQuickPayResponse: protoRoot.lookupType("eve_public.payment.token.api.GetQuickPayResponse"),
        tokenDisableAllResponse: protoRoot.lookupType("eve_public.payment.token.api.DisableAllResponse"),
        purchaseCostRequest: protoRoot.lookupType("eve_public.payment.purchase.api.CostRequest"),
        purchaseCostResponse: protoRoot.lookupType("eve_public.payment.purchase.api.CostResponse"),
        purchaseTokenRequest: protoRoot.lookupType("eve_public.payment.purchase.api.TokenRequest"),
        purchaseTokenResponse: protoRoot.lookupType("eve_public.payment.purchase.api.TokenResponse"),
        plexVaultPurchaseRequest: protoRoot.lookupType("eve_public.plex.vault.api.PurchaseRequest"),
        plexVaultPurchaseResponse: protoRoot.lookupType("eve_public.plex.vault.api.PurchaseResponse"),
        offerGetAllByTagResponse: protoRoot.lookupType("eve_public.store.offer.api.GetAllByTagResponse"),
        offerGetResponse: protoRoot.lookupType("eve_public.store.offer.api.GetResponse"),
    };
    return {
        name: "new-eden-store",
        ...
        handleRequest(requestTypeName, requestEnvelope) {
            ...
            if (requestTypeName === "eve_public.store.offer.api.GetAllByTagRequest") {
                return {
                    statusCode: 200,
                    statusMessage: "",
                    responseTypeName: "eve_public.store.offer.api.GetAllByTagResponse",
                    responsePayloadBuffer: encodePayload(types.offerGetAllByTagResponse, {
                        offers: [{ store_offer: "local_store_disabled" }],
                    }),
                };
            }
            if (requestTypeName === "eve_public.store.offer.api.GetRequest") {
                return {
                    statusCode: 200,
                    statusMessage: "",
                    responseTypeName: "eve_public.store.offer.api.GetResponse",
                    responsePayloadBuffer: encodePayload(types.offerGetResponse, {
                        attributes: {
                            name: "Local Store Unavailable",
                            tags: ["UpsellStarterPack"],
                            single_purchase: false,
                            targeted: false,
                            no_enddate: true,
                            can_purchase: false,
                            purchase_limited_reason: "Purchases are disabled on this local server.",
                            no_schedule: true,
                            offer_price: {
                                currency_price: { code: "USD", amount_in_cents: 0 },
                                no_plex_price: true,
                            },
                            base_price: {
                                currency_price: { code: "USD", amount_in_cents: 0 },
            ...
            if (requestTypeName === "eve_public.payment.token.api.GetRequest") {
                ...
                const tokenID = toPositiveInteger(decoded && decoded.token && decoded.token.sequential, 0);
                ...
                return {
                    ...
                    responsePayloadBuffer: encodePayload(types.tokenGetResponse, {
                        token: {
                            credit_card: token.creditCard && typeof token.creditCard === "object"
                                ? {
                                    alias: String(token.creditCard.alias || ""),
                                    expiry: String(token.creditCard.expiry || ""),
                                }
                                : undefined,
                            paypal: token.payPal && typeof token.payPal === "object"
                                ? {
            ...
            if (requestTypeName === "eve_public.payment.purchase.api.CostRequest") {
                ...
                const cost = computeCostPayload(decoded && decoded.catalog_amount_in_cents, decoded && decoded.currency);
                return {
                    statusCode: 200,
                    statusMessage: "",
            ...
            if (requestTypeName === "eve_public.payment.purchase.api.TokenRequest") {
                ...
                const quantity = Math.max(1, Math.trunc(Number(decoded && decoded.order && decoded.order.quantity) || 1));
                const tokenID = toPositiveInteger(decoded && decoded.token && decoded.token.sequential, 0);
                ...
                const resolvedTokenID = tokenID && findQuickPayToken(tokenID) ? tokenID : 0;
            ...
            const activeCharacterID = getActiveCharacterID(requestEnvelope);
            const targetCharacterID = resolvePlexVaultTargetCharacterID(activeCharacterID, decoded);
            const storeOfferID = resolveStoreOfferID(decoded);
            const quantity = Math.max(1, Math.trunc(Number(decoded && decoded.quantity) || 1));
...
//# sourceMappingURL=newEdenStoreGatewayService.js.map