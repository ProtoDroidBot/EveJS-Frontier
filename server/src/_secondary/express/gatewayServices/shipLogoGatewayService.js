...
const { buildDict, } = require(path.join(__dirname, "../../../services/_shared/serviceHelpers"));
...
const { COSMETIC_TYPE_ALLIANCE_LOGO, COSMETIC_TYPE_CORPORATION_LOGO, } = require(path.join(__dirname, "../../../services/evermarks/evermarksConstants"));
const { clearDisplayedLogo, getEnabledCosmeticsEntries, setDisplayedLogo, } = require(path.join(__dirname, "../../../services/ship/shipLogoFittingState"));
const { buildShipLogoGatewayProtoRoot, } = require(path.join(__dirname, "../../../services/ship/shipLogoGatewayProto"));
...
function broadcastLiveShipSlimRefresh(characterID, shipID) {
    ...
    for (const session of sessionRegistry.getSessions()) {
        ...
        try {
            ...
            const entity = scene.dynamicEntities instanceof Map
...
function createShipLogoGatewayService() {
    ...
    const types = {
        displayRequest: protoRoot.lookupType("eve_public.cosmetic.ship.logo.DisplayRequest"),
        displayResponse: protoRoot.lookupType("eve_public.cosmetic.ship.logo.DisplayResponse"),
        clearRequest: protoRoot.lookupType("eve_public.cosmetic.ship.logo.ClearRequest"),
        clearResponse: protoRoot.lookupType("eve_public.cosmetic.ship.logo.ClearResponse"),
    };
    return {
        name: "ship-logo",
        ...
        handleRequest(requestTypeName, requestEnvelope) {
            ...
            if (requestTypeName === "eve_public.cosmetic.ship.logo.DisplayRequest") {
                ...
                const shipID = normalizePositiveInteger(decoded && decoded.id && decoded.id.ship && decoded.id.ship.sequential, 0);
                const backendSlot = normalizeNonNegativeInteger(decoded && decoded.id && decoded.id.index, -1);
                const cosmeticType = resolveDisplayCosmeticType(decoded);
                const result = setDisplayedLogo(shipID, backendSlot, cosmeticType, {
                    characterID: activeCharacterID,
                });
                if (!result.success) {
                    return buildErrorResult(result.errorMsg, "eve_public.cosmetic.ship.logo.DisplayResponse");
                }
                notifyLiveCharacterSessions(activeCharacterID, shipID, backendSlot, cosmeticType);
            ...
            const shipID = normalizePositiveInteger(decoded && decoded.logo && decoded.logo.ship && decoded.logo.ship.sequential, 0);
            const backendSlot = normalizeNonNegativeInteger(decoded && decoded.logo && decoded.logo.index, -1);
            const result = clearDisplayedLogo(shipID, backendSlot);
            if (!result.success) {
                return buildErrorResult(result.errorMsg, "eve_public.cosmetic.ship.logo.ClearResponse");
            }
...
//# sourceMappingURL=shipLogoGatewayService.js.map