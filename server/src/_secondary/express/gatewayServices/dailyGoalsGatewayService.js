...
const { buildDailyGoalsProtoRoot, } = require(path.join(__dirname, "../../../services/dailyGoals/dailyGoalsProto"));
const dailyGoalsState = require(path.join(__dirname, "../../../services/dailyGoals/dailyGoalsState"));
const { CATEGORY, rewardsForDailyTemplate, rewardsForBonusTemplate, paymentsForMilestone, } = require(path.join(__dirname, "../../../services/dailyGoals/dailyGoalsCatalog"));
const { encodePayload, getActiveCharacterID, sliceWithPage, timestampFromMs, uuidStringToBuffer, } = require("./gatewayServiceHelpers");
...
function createDailyGoalsGatewayService(context) {
    ...
    function publishGoalNotice(characterID, event) {
        ...
        if (event.kind === "progressed") {
            publishGatewayNotice("eve_public.dailygoal.api.ProgressedNotice", encodePayload(types.progressedNotice, {
                goal: id,
                current_progress: Number(event.currentProgress || 0),
            }), target);
        }
        else if (event.kind === "completed") {
            publishGatewayNotice("eve_public.dailygoal.api.CompletedNotice", encodePayload(types.completedNotice, { goal: id }), target);
        }
        else if (event.kind === "redeemed") {
            publishGatewayNotice("eve_public.dailygoal.api.RedeemedNotice", encodePayload(types.redeemedNotice, { goal: id }), target);
        }
    }
    ...
    return {
        ...
        handleRequest(requestTypeName, requestEnvelope) {
            ...
            if (normalized === `${REQUEST_PREFIX}GetAllCurrentRequest`) {
                ...
                return ok(`${REQUEST_PREFIX}GetAllCurrentResponse`, encodePayload(types.getAllCurrentResponse, {
                    goals: [
            ...
            if (normalized === `${REQUEST_PREFIX}GetAllWithRewardsRequest`) {
                ...
                return ok(`${REQUEST_PREFIX}GetAllWithRewardsResponse`, encodePayload(types.getAllWithRewardsResponse, {
                    ids: paged.items.map((descriptor) => buildIdentifier(descriptor.uuid)),
                    next_page: paged.nextPage || undefined,
                }));
            }
            if (normalized === `${REQUEST_PREFIX}GetAllCompletedWithEntitlementsRequest`) {
                return ok(`${REQUEST_PREFIX}GetAllCompletedWithEntitlementsResponse`, encodePayload(types.getAllCompletedResponse, { goals: [] }));
            }
            if (normalized === `${REQUEST_PREFIX}GetRequest`) {
                ...
                const dailyDescriptor = uuid
                    ...
                    : null;
                if (dailyDescriptor) {
                    return ok(`${REQUEST_PREFIX}GetResponse`, encodePayload(types.getResponse, {
...
//# sourceMappingURL=dailyGoalsGatewayService.js.map