...
function createCompatibilityGatewayService({ protoRoot, emptyPayload }) {
    const careerGoalGetDefinitionsResponse = protoRoot.lookupType("eve_public.career.goal.api.GetDefinitionsResponse");
    const skillPlanGetActiveResponse = protoRoot.lookupType("eve_public.character.skill.plan.GetActiveResponse");
    return {
        name: "compatibility",
        handledRequestTypes: [
        ...
        handleRequest(requestTypeName) {
            if (requestTypeName === "eve_public.career.goal.api.GetDefinitionsRequest") {
                return {
                    ...
                    responsePayloadBuffer: buildEncodedPayload(careerGoalGetDefinitionsResponse, {
            ...
            const emptySuccessResponseType = EMPTY_SUCCESS_RESPONSE_TYPES.get(requestTypeName);
...
//# sourceMappingURL=compatibilityGatewayService.js.map