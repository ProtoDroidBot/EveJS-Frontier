...
function getScoreEntriesFromSource(source, solarSystemID) {
  ...
  if (typeof rawScores === "object") {
    return Object.entries<any>(rawScores)
      .map(([factionID, score]) => normalizeScore({
        factionID,
        ...(score && typeof score === "object" ? score : { contribution: score }),
      }))
...
function buildSolarSystemScoresPayload(solarSystemID, scores: any[] = []) {
  return {
...
function createFwAdvantageGatewayService(context: Record<string, any> = {}) {
