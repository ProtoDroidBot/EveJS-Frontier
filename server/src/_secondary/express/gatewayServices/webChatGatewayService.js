...
const xmppStubServer = require(path.join(__dirname, "../../../services/chat/xmppStubServer"));
const sessionRegistry = require(path.join(__dirname, "../../../services/chat/sessionRegistry"));
const { getCurrentSolarSystemID, getLocalChatRoomNameForSession, } = require(path.join(__dirname, "../../../services/chat/channelRules"));
...
function normalizeChannel(channel) {
    ...
    if (!CHAT_CHANNELS.includes(normalized)) {
        const error = new Error(`chat channel must be one of ${CHAT_CHANNELS.join(", ")}.`);
...
function getCorpRoster(session) {
    return getCorpSessions(getCorporationID(session)).map((member) => chatRuntime.buildCharacterSummary(member));
}
...
function sendChannel(session, channel, message) {
    ...
    try {
        ...
        if (error && typeof error.code === "string") {
            throw chatRefusal(error.message || "Chat message refused.", error.code);
        }
        throw error;
    }
}
...
//# sourceMappingURL=webChatGatewayService.js.map