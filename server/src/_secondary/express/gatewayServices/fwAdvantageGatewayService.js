...
function normalizeScore(score) {
    ...
    const factionID = normalizeUint(score.factionID ?? score.factionId ?? score.faction_id ??
        (score.faction && score.faction.sequential), 0);
    if (factionID <= 0) {
        return null;
    }
    return {
...
function getScoreEntriesFromSource(source, solarSystemID) {
    ...
    const rawScores = source instanceof Map
...
function createFwAdvantageGatewayService(context = {}) {
    ...
    const getSolarSystemScoresRequest = protoRoot.lookupType("eve_public.faction.activity.GetSolarSystemScoresRequest");
    const getSolarSystemScoresResponse = protoRoot.lookupType("eve_public.faction.activity.GetSolarSystemScoresResponse");
    function getSystemID(requestEnvelope) {
        const request = getSolarSystemScoresRequest.decode(requestEnvelope.payload.value || Buffer.alloc(0));
    ...
    return {
        ...
        handleRequest(requestTypeName, requestEnvelope) {
            ...
            return {
                ...
                responsePayloadBuffer: buildEncodedPayload(getSolarSystemScoresResponse, buildSolarSystemScoresPayload(solarSystemID, getScoreEntriesFromSource(scoreSource, solarSystemID))),
            };
        },
    };
}
...
//# sourceMappingURL=fwAdvantageGatewayService.js.map