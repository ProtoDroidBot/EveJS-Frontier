...
const { buildSkillPlanProtoRoot, } = require("./skillPlanProto");
const { encodePayload, getActiveCharacterID, uuidStringToBuffer, uuidBufferToString, } = require("./gatewayServiceHelpers");
const { createPersonalMilestone, createPersonalPlan, deletePersonalMilestone, deletePersonalPlan, findSharedPersonalPlan, getActivePlanID, getPersonalPlan, listPersonalMilestones, listPersonalPlanIDs, setActivePlanID, ZERO_UUID, updatePersonalMilestoneDescription, updatePersonalPlan, } = require(path.join(__dirname, "../../../services/skills/plans/skillPlanState"));
...
function createSkillPlanGatewayService() {
    ...
    const types = {
        ...
        setDescriptionRequest: protoRoot.lookupType("eve_public.character.skill.plan.SetDescriptionRequest"),
        setRequirementsRequest: protoRoot.lookupType("eve_public.character.skill.plan.SetSkillRequirementsRequest"),
        getActiveResponse: protoRoot.lookupType("eve_public.character.skill.plan.GetActiveResponse"),
        setActiveRequest: protoRoot.lookupType("eve_public.character.skill.plan.SetActiveRequest"),
        milestoneGetAllRequest: protoRoot.lookupType("eve_public.character.skill.plan.milestone.GetAllRequest"),
        milestoneGetAllResponse: protoRoot.lookupType("eve_public.character.skill.plan.milestone.GetAllResponse"),
        milestoneCreateRequest: protoRoot.lookupType("eve_public.character.skill.plan.milestone.CreateRequest"),
        milestoneCreateResponse: protoRoot.lookupType("eve_public.character.skill.plan.milestone.CreateResponse"),
        milestoneDeleteRequest: protoRoot.lookupType("eve_public.character.skill.plan.milestone.DeleteRequest"),
        milestoneSetDescriptionRequest: protoRoot.lookupType("eve_public.character.skill.plan.milestone.SetDescriptionRequest"),
    };
    return {
        name: "skill-plans",
            ...
            try {
                ...
                if (requestTypeName === "eve_public.character.skill.plan.CreateRequest") {
                    ...
                    const created = createPersonalPlan(activeCharacterID, {
                        ...
                        description: decoded && decoded.skill_plan ? decoded.skill_plan.description : "",
                        requirements: decodePlanRequirements(decoded && decoded.skill_plan ? decoded.skill_plan.skill_requirements : []),
                    });
                    return {
                        statusCode: 200,
                        statusMessage: "",
                ...
                if (requestTypeName === "eve_public.character.skill.plan.GetActiveRequest") {
                    ...
                    return {
                        ...
                        responsePayloadBuffer: encodePayload(types.getActiveResponse, activePlanID
                            ? {
                ...
                if (requestTypeName === "eve_public.character.skill.plan.milestone.GetAllRequest") {
                    ...
                    const milestones = listPersonalMilestones(activeCharacterID, extractPlanID(decoded.skill_plan));
                    return {
                        statusCode: 200,
                        statusMessage: "",
                ...
                if (requestTypeName === "eve_public.character.skill.plan.milestone.CreateRequest") {
                    ...
                    const created = createPersonalMilestone(activeCharacterID, extractPlanID(milestoneData.skill_plan), {
                ...
                if (requestTypeName === "eve_public.character.skill.plan.milestone.DeleteRequest") {
                    ...
                    deletePersonalMilestone(activeCharacterID, decoded && decoded.milestone ? uuidBufferToString(decoded.milestone.uuid) : null);
                    return {
                        statusCode: 200,
                        statusMessage: "",
                ...
                updatePersonalMilestoneDescription(activeCharacterID, decoded && decoded.identifier ? uuidBufferToString(decoded.identifier.uuid) : null, decoded.description);
                return {
                    statusCode: 200,
                    statusMessage: "",
                ...
                return buildErrorResponse(requestTypeName.replace(/Request$/, "Response"), error);
            }
        },
    };
}
...
//# sourceMappingURL=skillPlanGatewayService.js.map