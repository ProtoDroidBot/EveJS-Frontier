...
const skillQueueRuntime = require(path.join(__dirname, "../../services/skills/training/skillQueueRuntime"));
...
const skillTrainingMath = require(path.join(__dirname, "../../services/skills/training/skillTrainingMath"));
const planetRuntime = require(path.join(__dirname, "../../services/planet/planetRuntimeStore"));
const onlineRuntime = require(path.join(__dirname, "../../services/online/onlineStatusRuntime"));
const characterControlRuntime = require(path.join(__dirname, "../../services/online/characterControlRuntime"));
const { AUTHORIZATION_POLICIES, createCharacterCommandRuntime, } = require(path.join(__dirname, "../../services/online/characterCommandRuntime"));
const { createCharacterEventRuntime, } = require(path.join(__dirname, "../../services/online/characterEventRuntime"));
const { marketDaemonClient } = require(path.join(__dirname, "../../services/market/marketDaemonClient"));
const { isDeferredCallResponse } = require(path.join(__dirname, "../../network/callResponseControl"));
...
const webChatGatewayService = require(path.join(__dirname, "gatewayServices/webChatGatewayService"));
...
const { createSessionEventRuntime, } = require(path.join(__dirname, "sessionEventStream"));
...
const { getLocalChatRoomNameForSession, } = require(path.join(__dirname, "../../services/chat/channelRules"));
...
const WEB_CALL_ALLOWLIST_KEYS = new Set(WEB_CALL_ALLOWLIST.map((pair) => `${pair.service} ${pair.method}`));
...
function normalizeSkillQueueCommandPayload(payload) {
    if (!hasExactKeys(payload, ["entries", "activate"]) ||
...
function normalizeWebCallSessionFields(sessionFields) {
    if (!isPlainObject(sessionFields)) {
        throw webCallError("CALL_INVALID", "Call session must be a JSON object of session fields.");
    }
    const normalized = {};
    for (const [key, value] of Object.entries(sessionFields)) {
        ...
        if (value !== null &&
            ...
            valueType !== "string" &&
            valueType !== "boolean") {
            throw webCallError("CALL_INVALID", `Call session field "${key}" must be a JSON scalar.`);
        }
        normalized[key] = value;
    }
    const userid = Number(normalized.userid);
    if (!Number.isSafeInteger(userid) || userid <= 0) {
        throw webCallError("CALL_INVALID", "Call session requires a positive integer userid.");
    }
...
function normalizeWebCallRequest(request) {
    ...
    if (!service || !method) {
        throw webCallError("CALL_INVALID", "Call requires non-empty service and method names.");
    }
    if (call.args !== undefined && !Array.isArray(call.args)) {
        throw webCallError("CALL_INVALID", "Call args must be an array.");
    }
    if (call.kwargs !== undefined &&
        call.kwargs !== null &&
        !isPlainObject(call.kwargs)) {
        throw webCallError("CALL_INVALID", "Call kwargs must be a JSON object or null.");
    }
    return {
        service,
        method,
        ...
        sessionFields: normalizeWebCallSessionFields(call.session === undefined ? {} : call.session),
    };
}
...
function encodeJsonSafeCallValue(value) {
    ...
    return JSON.parse(JSON.stringify(value, (key, fieldValue) => (typeof fieldValue === "bigint" ? fieldValue.toString() : fieldValue)));
}
...
function extractBoundObjectOIDs(value, output = [], seen = new Set(), depth = 0) {
    ...
    if (value.type === "substruct" && value.value) {
        const substream = value.value.type === "substream" ? value.value.value : value.value;
        if (Array.isArray(substream) &&
...
function materializePersistentBrowserSession(sessionFields, notifications, clientID, notificationSink = null) {
    const sink = notificationSink || { emit() { } };
    return {
        ...sessionFields,
        userName: (typeof sessionFields.userName === "string" && sessionFields.userName) ||
...
function readWrappedUserErrorRefusal(error) {
    ...
    const proseEntry = entries.find((entry) => Array.isArray(entry) && (entry[0] === "info" || entry[0] === "notify") && entry[1]);
    if (proseEntry) {
...
function readStructuredRefusalReasons(entries) {
    const errorsEntry = entries.find((entry) => Array.isArray(entry) && entry[0] === "errors" && entry[1]);
    const errorList = errorsEntry && errorsEntry[1];
    const items = errorList && Array.isArray(errorList.items) ? errorList.items : [];
    const names = [];
    for (const item of items) {
        ...
        const keyValEntries = keyVal && keyVal.args && Array.isArray(keyVal.args.entries)
            ? keyVal.args.entries
            : [];
        const nameEntry = keyValEntries.find((entry) => Array.isArray(entry) && entry[0] === "name" && entry[1]);
...
function normalizeChatChannel(channel) {
    try {
        ...
        throw webCallError("CALL_INVALID", error && error.message ? error.message : "Invalid chat channel.");
    }
}
function toWebChatError(error) {
    ...
    if (error && error.chatInvalid) {
        return webCallError("CALL_INVALID", error.message || "Invalid chat request.");
    }
    if (error && error.chatRefusal) {
        return webCallError("CALL_REFUSED", error.message || "Chat message refused.");
    }
    const detail = String(error && error.message ? error.message : error).slice(0, WEB_CALL_FAILURE_DETAIL_LIMIT);
...
function spaceHealthRatios(entity) {
    const condition = entity && entity.conditionState && typeof entity.conditionState === "object"
...
function refreshSpacePresentationFields(spaceRuntime, egoEntity, entities) {
    const testing = spaceRuntime && spaceRuntime._testing && typeof spaceRuntime._testing === "object"
...
function projectSpaceEntity(entity, egoItemID = 0, mineableStateFor = null) {
    ...
    const row = {
        ...
        categoryID: spaceNumber(entity.slimCategoryID, spaceNumber(entity.categoryID, 0)) || null,
...
function buildMineableStateLookup(scene) {
    ...
    try {
        miningRuntimeState = require(path.join(__dirname, "../../services/mining/miningRuntimeState"));
    }
    catch {
        return null;
    }
...
function projectActiveShipStatus(entity) {
    ...
    return {
        ...
        activeModuleIDs: entity.activeModuleEffects instanceof Map
...
async function driveDeferredCallResponse(deferred, context) {
    ...
    const adapterDispatcher = {
        ...
        _sendCallResponse(_packet, result, _session, _responseServiceName, _responsePerfMeta, responseOptions = {}) {
            captured.push({
                kind: "call",
                result,
    ...
    if (typeof deferred.start === "function") {
        await Promise.resolve(deferred.start({
...
function resolveDeferredCallResult(captured, serviceName, method) {
    const finalResponses = captured.filter((entry) => entry.kind === "call" && !isProvisionalDeferredResponse(entry));
    if (finalResponses.length > 0) {
        return finalResponses[finalResponses.length - 1].result;
    }
    throw webCallError("CALL_DEFERRED_UNSUPPORTED", `${serviceName}.${method} returned a deferred response that requires a ` +
...
async function invokeAfterCallResponse(serviceInstance, call, session, result) {
    ...
    try {
        await Promise.resolve(serviceInstance.afterCallResponse(call.method, session, {
            args: call.args,
            kwargs: call.kwargs,
            result,
        }));
        ...
        log.warn(`[EvejsWebGateway] afterCallResponse failed for ` +
...
function getAccountByUsername(username) {
    ...
    return normalizedUsername
        ? normalizeAccountRecord(normalizedUsername, readTableEntry("accounts", normalizedUsername))
        : null;
}
...
function getCharacter(characterID) {
    ...
    return numericCharacterID > 0
        ? normalizeCharacterRecord(numericCharacterID, readTableEntry("characters", numericCharacterID))
        : null;
}
...
function listItemsForCharacter(characterID) {
    ...
    for (let index = 0; index < queue.length && selected.size < SNAPSHOT_ITEM_LIMIT; index += 1) {
        const parent = byItemID.get(getNumericID(queue[index], ["locationID", "locationid"]));
    ...
    return Object.fromEntries([...selected.entries()].map(([itemID, item]) => [String(itemID), cloneValue(item)]));
}
function buildIndustryJobsForCharacter(characterID) {
    ...
    return {
        jobs: Object.fromEntries(Object.entries(jobs || {})
            .filter(([, job]) => (Number(job && job.ownerID) === characterID ||
...
function buildSkillSheet(accountID, characterID) {
    ...
    try {
        ...
        queue = {
            active: Boolean(snapshot && snapshot.active),
            entries: entries.map((entry, index) => skillSheetQueueEntry(entry, index === 0 ? currentRate : 0)),
        ...
        projectedByTypeID = new Map((Array.isArray(snapshot && snapshot.projectedSkills) ? snapshot.projectedSkills : [])
    ...
    for (const [key, record] of Object.entries(skillsTable)) {
        ...
        const rank = skillTrainingMath.normalizeSkillRank(record && record.skillRank);
        const levelSkillPoints = [];
        for (let level = 1; level <= SKILL_SHEET_MAX_LEVEL; level += 1) {
            levelSkillPoints.push(Number(skillTrainingMath.getSkillPointsForLevel(rank, level)) || 0);
        }
        skills.push({
            typeID,
            ...
            level: Number((projected && projected.trainedSkillLevel) ?? (record && record.trainedSkillLevel) ?? 0) || 0,
            rank,
            skillPoints: Number((projected && projected.skillPoints) ?? (record && record.skillPoints) ?? 0) || 0,
            levelSkillPoints,
...
function sanitizeCharacterControlSnapshot(snapshot, characterID = 0, stateVersion = "") {
    ...
    const normalizedStateVersion = typeof stateVersion === "string"
        ? stateVersion
        : "";
    if (!["offline", "retail_client", "browser_pilot"].includes(normalizedState) ||
    ...
    return {
        ...
        controlState,
        transport,
        leaseExpiresAt: controlState === "browser_pilot" && snapshot && snapshot.leaseExpiresAt
...
function buildSnapshot(accountID, characterID, stateVersion) {
    ...
    try {
        queueSnapshot = sanitizeQueueSnapshot(skillQueueRuntime.getQueueSnapshot(character.characterID));
    }
    catch (error) {
...
function getDefaultCharacterRuntimes() {
    ...
    const characterCommands = createCharacterCommandRuntime({
        ...
        isCommandIDRetained(characterID, commandID) {
            return Boolean(characterEvents &&
        ...
        commandDefinitions: {
            [CHARACTER_COMMAND_TYPES.SAVE_SKILL_QUEUE]: {
                ...
                handler({ characterID, payload }) {
                    const snapshot = skillQueueRuntime.saveQueue(characterID, payload.entries, {
                        activate: payload.activate,
                        emitNotifications: true,
                    });
                    requireSuccessfulFlush(gameStore.flushTablesSync(["skillQueues", "skills", "characters"]), "skill queue");
                    const sanitized = sanitizeQueueSnapshot(snapshot);
                    log.info(`[EvejsWebGateway] skill queue saved characterID=${characterID} ` +
            ...
            [CHARACTER_COMMAND_TYPES.RESTART_PI_EXTRACTORS]: {
                ...
                handler({ characterID, payload }) {
                    ...
                    requireSuccessfulFlush(gameStore.flushTablesSync(["planetRuntimeState"]), "PI extractor restart");
                    log.info(`[EvejsWebGateway] PI extractors restarted characterID=${characterID} ` +
...
function createEvejsWebGatewayRuntime({ serviceManager, characterCommandRuntime, characterEventRuntime, browserSessionIdleTtlMs, browserSessionSweepIntervalMs, sessionEventRuntime, now, } = {}) {
    if (Boolean(characterCommandRuntime) !== Boolean(characterEventRuntime)) {
        throw new TypeError("custom gateway command and event runtimes must be supplied together");
    }
    ...
    const dependencies = Object.freeze({
        ...
        gameStore: Boolean(gameStore &&
            typeof gameStore.read === "function" &&
            typeof gameStore.flushTablesSync === "function"),
        skillQueue: Boolean(typeof skillQueueRuntime.getQueueSnapshot === "function" &&
        ...
        skillSheet: Boolean(typeof skillTrainingMath.getSkillPointsForLevel === "function" &&
        ...
        characterControl: Boolean(characterControlRuntime &&
        ...
        characterEvents: Boolean(characterEvents &&
        ...
        market: Boolean(marketDaemonClient &&
    ...
    const idleTtlMs = Number.isSafeInteger(browserSessionIdleTtlMs) && browserSessionIdleTtlMs > 0
        ? browserSessionIdleTtlMs
        : DEFAULT_BROWSER_SESSION_IDLE_TTL_MS;
    const sweepIntervalMs = Number.isSafeInteger(browserSessionSweepIntervalMs) &&
    ...
    function isDefunctBrowserSession(entry) {
        return (entry.session._characterControlDisconnected === true ||
    ...
    function getBrowserSessionEntry(bridgeSessionID, userid) {
        ...
        if (!entry) {
            throw webCallError("SESSION_NOT_FOUND", "Unknown, expired, or released bridge session.");
        }
        if (isDefunctBrowserSession(entry)) {
            teardownBrowserSession(entry, "browser_session_defunct");
            throw webCallError("SESSION_NOT_FOUND", "Unknown, expired, or released bridge session.");
    ...
    function normalizeBridgeSessionID(value) {
        if (typeof value !== "string" || !value.trim()) {
            throw webCallError("CALL_INVALID", "bridgeSessionID must be a non-empty string.");
        }
        return value.trim();
    }
    function normalizeBoundHandle(value) {
        if (typeof value !== "string" || !value.trim()) {
            throw webCallError("CALL_INVALID", "boundHandle must be a non-empty string.");
        }
        return value.trim();
    }
    ...
    return Object.freeze({
        ...
        buildSnapshot(accountID, characterID) {
            return buildSnapshot(accountID, characterID, characterCommands.getStateVersion(characterID));
        },
        ...
        getCharacterControlStatus(characterID) {
            return sanitizeCharacterControlSnapshot(characterControlRuntime.getCharacterControlSnapshot(characterID), characterID, characterCommands.getStateVersion(characterID));
        },
        claimBrowserControl(characterID, controllerID) {
            const result = characterControlRuntime.claimBrowserControl(characterID, controllerID);
            return {
                control: sanitizeCharacterControlSnapshot(result.control, characterID, characterCommands.getStateVersion(characterID)),
                credentials: {
        ...
        renewBrowserControl(characterID, controllerID, leaseID, leaseSecret) {
            const result = characterControlRuntime.renewBrowserControl(characterID, controllerID, leaseID, leaseSecret);
            return {
                control: sanitizeCharacterControlSnapshot(result.control, characterID, characterCommands.getStateVersion(characterID)),
            };
        },
        releaseBrowserControl(characterID, controllerID, leaseID, leaseSecret) {
            const result = characterControlRuntime.releaseBrowserControl(characterID, controllerID, leaseID, leaseSecret);
            return {
                control: sanitizeCharacterControlSnapshot(result.control, characterID, characterCommands.getStateVersion(characterID)),
            };
        },
        ...
        subscribeSessionEvents(bridgeSessionID, userid, cursor, handlers) {
            ...
            const entry = getBrowserSessionEntry(normalizeBridgeSessionID(bridgeSessionID), userid);
        ...
        async callServiceMethod(request) {
            ...
            if (!isAllowlistedWebCall(call.service, call.method)) {
                throw webCallError("CALL_NOT_ALLOWED", `${call.service}.${call.method} is not on the web-call allowlist.`);
            }
            const serviceInstance = serviceManager.lookup(call.service);
            if (!serviceInstance || typeof serviceInstance.callMethod !== "function") {
                throw webCallError("CALL_SERVICE_UNAVAILABLE", `${call.service} is not available in this process.`);
            }
            ...
            const rawBridgeSessionID = request && typeof request === "object" ? request.bridgeSessionID : undefined;
            let entry = null;
            let session;
            let notifications;
            if (rawBridgeSessionID !== undefined && rawBridgeSessionID !== null) {
                entry = getBrowserSessionEntry(normalizeBridgeSessionID(rawBridgeSessionID), call.sessionFields.userid);
            ...
            try {
                result = await Promise.resolve(serviceInstance.callMethod(call.method, call.args, session, call.kwargs));
            ...
            // CALL_FAILED.
            if (deferredCaptured) {
                result = resolveDeferredCallResult(deferredCaptured, call.service, call.method);
            }
            let response;
            try {
                response = {
                    ...
                    notifications: encodeJsonSafeCallValue(entry ? notifications.splice(0) : notifications),
                };
            }
            catch {
                throw webCallError("CALL_FAILED", `${call.service}.${call.method} returned a result that is not JSON-serializable.`);
            }
        ...
        async selectCharacter(request) {
            ...
            const session = materializePersistentBrowserSession(call.sessionFields, notifications, BROWSER_SESSION_CLIENT_ID_BASE + browserSessionClientIDCounter, notificationSink);
            ...
            if (!(characterID > 0)) {
                ...
                discardMintedSession(session);
                throw webCallError("SESSION_SELECT_FAILED", `${call.service}.${call.method} completed without bringing a character online.`);
            }
            ...
            try {
                ...
                log.debug(`[EvejsWebGateway] chat presence sync on select skipped: ${error.message}`);
            }
            log.info(`[EvejsWebGateway] Browser session started characterID=${characterID} ` +
                `userid=${entry.userid}`);
            try {
                return {
                    bridgeSessionID,
                    ...
                    session: {
                        ...
                        solarSystemID: Number(session.solarsystemid2 || session.solarsystemid || 0) || null,
        ...
        releaseBrowserSession(request) {
            const bridgeSessionID = normalizeBridgeSessionID(request && typeof request === "object" ? request.bridgeSessionID : request);
            const sessionFields = request && typeof request === "object" && isPlainObject(request.session)
                ? request.session
                : null;
            const entry = getBrowserSessionEntry(bridgeSessionID, sessionFields && sessionFields.userid !== undefined
        ...
        readFlightStatus(request) {
            ...
            entry.lastUsedAtMs = nowMs();
            session.lastActivity = Date.now();
            const space = session._space && typeof session._space === "object" ? session._space : null;
            ...
            const solarSystemID = Number((space && space.systemID) ||
                session.solarsystemid2 ||
                session.solarsystemid ||
                0) || null;
            const shipID = Number((space && space.shipID) || session.shipid || session.shipID || 0) ||
                null;
            let shipMode = null;
                    ...
                    if (entity) {
                        shipMode =
                            (typeof entity.mode === "string" && entity.mode) ||
                                ...
                                null;
                        const fraction = Number(entity.speedFraction !== undefined
            ...
            if (inSpace) {
                try {
                    ...
                    const scene = typeof spaceRuntime.getSceneForSession === "function"
                        ...
                        : null;
                    if (scene) {
                        const egoEntity = typeof scene.getShipEntityForSession === "function"
        ...
        sendChat(request) {
            ...
            let sent;
            try {
                sent = webChatGatewayService.sendChannel(session, channel, request && request.message);
            }
            catch (error) {
                throw toWebChatError(error);
            }
        ...
        async bindBoundObject(request) {
            ...
            const rawBridgeSessionID = request && typeof request === "object" ? request.bridgeSessionID : undefined;
            if (rawBridgeSessionID === undefined || rawBridgeSessionID === null) {
                throw webCallError("CALL_INVALID", "A bound-object bind requires a bridgeSessionID.");
            }
            if (!isAllowlistedWebCall(call.service, call.method)) {
                throw webCallError("CALL_NOT_ALLOWED", `${call.service}.${call.method} is not on the web-call allowlist.`);
            }
            const entry = getBrowserSessionEntry(normalizeBridgeSessionID(rawBridgeSessionID), call.sessionFields.userid);
            const serviceInstance = serviceManager.lookup(call.service);
            if (!serviceInstance || typeof serviceInstance.callMethod !== "function") {
                throw webCallError("CALL_SERVICE_UNAVAILABLE", `${call.service} is not available in this process.`);
            }
            ...
            if (oids.length === 0) {
                throw webCallError("BOUND_NO_OBJECT", `${call.service}.${call.method} did not return a bound object.`);
            }
            for (const oid of oids) {
            ...
            try {
                ...
                throw webCallError("CALL_FAILED", `${call.service}.${call.method} bind returned an unserializable notification.`);
            }
        },
        ...
        async callBoundMethod(request) {
            ...
            const rawBridgeSessionID = request && typeof request === "object" ? request.bridgeSessionID : undefined;
            if (rawBridgeSessionID === undefined || rawBridgeSessionID === null) {
                throw webCallError("CALL_INVALID", "A bound-object call requires a bridgeSessionID.");
            }
            const boundHandle = normalizeBoundHandle(request && typeof request === "object" ? request.boundHandle : undefined);
            const entry = getBrowserSessionEntry(normalizeBridgeSessionID(rawBridgeSessionID), call.sessionFields.userid);
            const handleEntry = entry.boundHandles.get(boundHandle);
            if (!handleEntry) {
                throw webCallError("BOUND_HANDLE_NOT_FOUND", "Unknown bound-object handle for this session.");
            }
            ...
            if (call.service !== handleEntry.serviceName) {
                throw webCallError("BOUND_HANDLE_NOT_FOUND", "Bound-object handle does not belong to the requested service.");
            }
            // Deny by default on the bound method, before any service lookup.
            if (!isAllowlistedWebCall(handleEntry.serviceName, call.method)) {
                throw webCallError("CALL_NOT_ALLOWED", `${handleEntry.serviceName}.${call.method} is not on the web-call allowlist.`);
            }
            const serviceInstance = serviceManager.lookup(handleEntry.oid);
            if (!serviceInstance || typeof serviceInstance.callMethod !== "function") {
                // The OID was released or evicted out from under the handle.
                entry.boundHandles.delete(boundHandle);
                throw webCallError("BOUND_HANDLE_NOT_FOUND", "Bound-object handle is no longer resolvable.");
            }
            const session = entry.session;
            const notifications = entry.notifications;
            entry.lastUsedAtMs = nowMs();
            ...
            if (deferredCaptured) {
                result = resolveDeferredCallResult(deferredCaptured, handleEntry.serviceName, call.method);
            }
            let response;
            try {
                response = {
                ...
                throw webCallError("CALL_FAILED", `${handleEntry.serviceName}.${call.method} returned a result that is not JSON-serializable.`);
            }
            if (!deferredCaptured) {
                await invokeAfterCallResponse(serviceInstance, { ...call, service: handleEntry.serviceName }, session, result);
            }
            return response;
        },
...
//# sourceMappingURL=evejsWebGatewayRuntime.js.map