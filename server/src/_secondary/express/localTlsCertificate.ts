...
function ensureCertificateAuthority(options: Record<string, any> = {}) {
...
function certificateAttributesEqual(left: any[] = [], right: any[] = []) {
...
function ensureLocalLeafCertificate(options: Record<string, any> = {}) {
...
function ensureXmppTlsCertificate(options: Record<string, any> = {}) {
  ...
  const dnsAltNames: any[] = [];
  const ipAltNames: any[] = [];
  const pushHost = (host) => {
