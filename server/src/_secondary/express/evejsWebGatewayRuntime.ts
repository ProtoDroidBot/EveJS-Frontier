...
function normalizeWebCallSessionFields(sessionFields) {
  ...
  for (const [key, value] of Object.entries<any>(sessionFields)) {
...
function extractBoundObjectOIDs(value, output: any[] = [], seen = new Set(), depth = 0) {
...
async function driveDeferredCallResponse(deferred, context) {
  const captured: any[] = [];
  const adapterDispatcher: Record<string, any> = {
  ...
  const adapterPacket: Record<string, any> = {
    source: { callID: 0 },
    dest: {},
    oob: null,
...
function listAccounts() {
  return Object.entries<any>(readRootTable("accounts"))
...
function listCharacters(accountID) {
  ...
  return Object.entries<any>(readRootTable("characters"))
...
function listItemsForCharacter(characterID) {
  const allItems = Object.values<any>(readRootTable("items"))
  ...
  const queue: any[] = [...selected.values()];
...
function buildPlanetRuntimeForCharacter(characterID) {
  ...
  const coloniesByKey: Record<string, any> = {};
  const launchesByID: Record<string, any> = {};
  const resourcesByPlanetID: Record<string, any> = {};
  for (const [key, colony] of Object.entries<any>(state.coloniesByKey || {})) {
  ...
  for (const [launchID, launch] of Object.entries<any>(state.launchesByID || {})) {
...
function buildSkillSheet(accountID, characterID) {
  ...
  const skills: any[] = [];
  for (const [key, record] of Object.entries<any>(skillsTable)) {
...
function sanitizeCharacterControlSnapshot(snapshot, characterID = 0, stateVersion = "") {
  ...
  if (
    ...
    const error: Error & Record<string, any> = new Error("Authoritative character control is unavailable.");
  ...
  const characterCommands = createCharacterCommandRuntime({
    ...
    commandDefinitions: {
      [CHARACTER_COMMAND_TYPES.SAVE_SKILL_QUEUE]: {
        ...
        handler({ characterID, payload }: Record<string, any>) {
