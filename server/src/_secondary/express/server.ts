...
function shouldDenyUnhandledProxyHost(hostname, options: Record<string, any> = {}) {
  const normalizedHost = normalizeHostname(hostname);
  if (!normalizedHost) {
    return true;
  }
...
function shouldReturnLaunchDarklyEventStream(routePath, headers: Record<string, any> = {}) {
...
function isLaunchDarklyEventsRequest(routePath, headers: Record<string, any> = {}) {
  const authority = String(headers[":authority"] || headers.host || "").toLowerCase();
  const normalizedPath = String(routePath || "/").toLowerCase();
...
function pipeHttpRequest(req, res, targetUrl) {
  ...
  const upstreamReq = requestImpl.request(
    ...
    (upstreamRes) => {
      ...
      for (const [k, v] of Object.entries<any>(upstreamRes.headers)) {
...
function wireTunnel(clientSocket, upstreamSocket, head, label, options: Record<string, any> = {}) {
  let upBytes = 0;
  let downBytes = 0;
...
const ALLOWED_PROXY_HOSTS = (() => {
  ...
  const derived: any[] = ["clientresources.eveonline.com"];
