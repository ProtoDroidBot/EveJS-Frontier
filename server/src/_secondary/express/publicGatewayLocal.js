...
const { findShipItemById } = require(path.join(__dirname, "../../services/inventory/itemStore"));
const { getAppliedSkinRecord, getAppliedSkinRecordsForOwner, } = require(path.join(__dirname, "../../services/ship/shipCosmeticsState"));
const { resolveCharacterAccountID, resolveOmegaLicenseState, } = require(path.join(__dirname, "../../services/newEdenStore/storeState"));
const { createGatewayServiceRegistry, } = require(path.join(__dirname, "./gatewayServices"));
const { buildStructurePaintworkProtoRoot, } = require(path.join(__dirname, "./gatewayServices/structurePaintworkGatewayService"));
const sessionRegistry = require(path.join(__dirname, "../../services/chat/sessionRegistry"));
const spaceRuntime = require(path.join(__dirname, "../../space/runtime"));
const { getLicenseForStructure, getPaintworksForSolarSystem, normalizePaintwork, } = require(path.join(__dirname, "../../services/structure/structurePaintworkState"));
const { getStructureByID, } = require(path.join(__dirname, "../../services/structure/structureState"));
const GATEWAY_INSTANCE_UUID = Buffer.from(crypto.randomUUID().replace(/-/g, ""), "hex");
const ACTIVE_NOTICE_STREAMS = new Map();
const UNKNOWN_REQUEST_COUNTS = new Map();
const GRPC_TRAILERS_PROPERTY = Symbol.for("evejs.publicGateway.grpcTrailers");
                                    ...
                                    nested: {
                                        GetDefinitionsResponse: {
                                            fields: {
                                                goals: {
                                                    rule: "repeated",
                                                    type: "eve_public.career.goal.api.GetDefinitionsResponse.Goal",
                                                    id: 1,
                                                },
                                            },
                                            nested: {
...
const ShipStateGetRequest = PROTO_ROOT.lookupType("eve_public.cosmetic.ship.api.GetRequest");
const ShipStateGetResponse = PROTO_ROOT.lookupType("eve_public.cosmetic.ship.api.GetResponse");
const ShipStateGetAllInBubbleResponse = PROTO_ROOT.lookupType("eve_public.cosmetic.ship.api.GetAllInBubbleResponse");
const ShipStateSetNotice = PROTO_ROOT.lookupType("eve_public.cosmetic.ship.api.SetNotice");
const ShipStateSetAllInBubbleNotice = PROTO_ROOT.lookupType("eve_public.cosmetic.ship.api.SetAllInBubbleNotice");
const UserLicenseGetRequest = PROTO_ROOT.lookupType("eve_public.user.license.api.GetRequest");
const UserLicenseGetResponse = PROTO_ROOT.lookupType("eve_public.user.license.api.GetResponse");
const PlexVaultBalanceResponse = PROTO_ROOT.lookupType("eve_public.plex.vault.api.BalanceResponse");
const PlexVaultBalanceChangedNotice = PROTO_ROOT.lookupType("eve_public.plex.vault.api.BalanceChangedNotice");
const STRUCTURE_PAINTWORK_PROTO_ROOT = buildStructurePaintworkProtoRoot();
const StructurePaintworkSetNotice = STRUCTURE_PAINTWORK_PROTO_ROOT.lookupType("eve_public.cosmetic.structure.paintwork.api.SetNotice");
const StructurePaintworkSetAllInSolarSystemNotice = STRUCTURE_PAINTWORK_PROTO_ROOT.lookupType("eve_public.cosmetic.structure.paintwork.api.SetAllInSolarSystemNotice");
...
function removeNoticeStream(stream) {
    if (ACTIVE_NOTICE_STREAMS.delete(stream)) {
        log.info(`[PublicGatewayLocal] Notices.Consume disconnected active=${ACTIVE_NOTICE_STREAMS.size}`);
    }
}
...
function pushLocalMembershipSnapshotForCharacter(characterID) {
    ...
    try {
        ...
        log.debug(`[PublicGatewayLocal] Local membership push on notice identify failed char=${numericCharacterID}: ${error.message}`);
    }
}
function updateNoticeStreamState(stream, requestEnvelope) {
    ...
    state.applicationInstanceHex = bytesToHex(requestEnvelope && requestEnvelope.application_instance_uuid);
...
function resolveNoticeRoutingState(streamState) {
    ...
    if (session) {
        ...
        routingState.corporationID = Number(session.corporationID || session.corpid || 0) || 0;
        routingState.allianceID = Number(session.allianceID || session.allianceid || 0) || 0;
...
function shouldDeliverNoticeToState(routingState, noticeEnvelope) {
    const targetGroup = noticeEnvelope && noticeEnvelope.target_group
...
function correlationLabel(requestEnvelope) {
    const correlation = bufferFromBytes(requestEnvelope && requestEnvelope.correlation_uuid);
...
function createGrpcFrameParser(onFrame) {
    ...
    return function parseChunk(chunk) {
        ...
        while (pending.length >= 5) {
            ...
            if (compressed !== 0) {
                log.warn(`[PublicGatewayLocal] Ignoring compressed gRPC frame flag=${compressed}`);
                continue;
            }
...
function getActiveCharacterID(requestEnvelope) {
    const identityCharacter = requestEnvelope &&
        ...
        ? normalizeProtoNumber(requestEnvelope.authoritative_context.identity.character.sequential)
        : 0;
    ...
    return requestEnvelope &&
        ...
        ? normalizeProtoNumber(requestEnvelope.authoritative_context.active_character.sequential)
        : 0;
}
function getSessionCharacterID(session) {
    return normalizeProtoNumber(session &&
        (session.characterID ||
            session.charID ||
            session.charid ||
            session.characterId));
...
function buildShipStateObject(shipID, activeCharacterID) {
    ...
    const ownerID = normalizeProtoNumber(appliedRecord && appliedRecord.characterID ? appliedRecord.characterID : 0) ||
        normalizeProtoNumber(appliedRecord && appliedRecord.ownerID ? appliedRecord.ownerID : 0) ||
        normalizeProtoNumber(shipItem && shipItem.ownerID ? shipItem.ownerID : 0) ||
        normalizeProtoNumber(activeCharacterID);
    const skinID = normalizeProtoNumber(appliedRecord && appliedRecord.skinID ? appliedRecord.skinID : 0);
    const state = {
        character: {
            sequential: ownerID,
        },
...
function buildVisibleShipStatesForSession(session, activeCharacterID = 0) {
    ...
    states.sort((left, right) => {
        const leftShipID = normalizeProtoNumber(left && left.ship ? left.ship.sequential : 0);
        const rightShipID = normalizeProtoNumber(right && right.ship ? right.ship.sequential : 0);
...
function buildShipStateResponsePayload(requestEnvelope) {
    const decoded = ShipStateGetRequest.decode(bufferFromBytes(requestEnvelope.payload && requestEnvelope.payload.value));
    ...
    return Buffer.from(ShipStateGetResponse.encode(ShipStateGetResponse.create({
        state,
    })).finish());
}
function buildShipStatesInBubblePayload(requestEnvelope) {
    ...
    return Buffer.from(ShipStateGetAllInBubbleResponse.encode(ShipStateGetAllInBubbleResponse.create({
        states,
    })).finish());
}
...
function buildUserLicenseResponsePayload(requestEnvelope) {
    const decoded = UserLicenseGetRequest.decode(bufferFromBytes(requestEnvelope.payload && requestEnvelope.payload.value));
    const licenseType = String(decoded && decoded.license && decoded.license.license_type
        ? decoded.license.license_type
        : "");
    if (licenseType !== OMEGA_USER_LICENSE_TYPE) {
        return {
            payloadBuffer: Buffer.from(UserLicenseGetResponse.encode(UserLicenseGetResponse.create({})).finish()),
            hasLicense: false,
            licenseType,
        };
    }
    ...
    return {
        payloadBuffer: Buffer.from(UserLicenseGetResponse.encode(UserLicenseGetResponse.create({
            license: {
...
function buildPlexBalanceResponsePayload(requestEnvelope) {
    ...
    const plexBalance = Math.max(0, Math.trunc(Number(charData.plexBalance ?? getDefaultPlexBalance()) || 0));
    const totalInCents = plexBalance * PLEX_GATEWAY_CENTS_PER_PLEX;
    log.info(`[PublicGatewayLocal] PlexBalance.Get character=${activeCharacterID || 0} ` +
        `plex=${plexBalance} totalInCents=${totalInCents}`);
    return Buffer.from(PlexVaultBalanceResponse.encode(PlexVaultBalanceResponse.create({
        balance: {
            total_in_cents: totalInCents,
        },
    })).finish());
}
function publishPlexBalanceChangedNotice(activeCharacterID, resultingBalance, amountTransferred) {
    const numericCharacterID = normalizeProtoNumber(activeCharacterID);
    if (!numericCharacterID) {
        return false;
    }
    const resultingBalanceInCents = Math.max(0, Math.trunc(Number(resultingBalance || 0) * PLEX_GATEWAY_CENTS_PER_PLEX));
    const amountTransferredInCents = Math.trunc(Number(amountTransferred || 0) * PLEX_GATEWAY_CENTS_PER_PLEX);
    const noticePayload = Buffer.from(PlexVaultBalanceChangedNotice.encode(PlexVaultBalanceChangedNotice.create({
        identifier: {
            sequential: Date.now(),
        },
        attributes: {
    ...
    const noticeEnvelope = encodeNoticeEnvelope("eve_public.plex.vault.api.BalanceChangedNotice", noticePayload, {
        character: numericCharacterID,
    });
    log.info(`[PublicGatewayLocal] Notices.Publish eve_public.plex.vault.api.BalanceChangedNotice ` +
...
function broadcastNoticeEnvelope(noticeBuffer) {
    ...
    try {
        ...
        log.warn(`[PublicGatewayLocal] Failed to decode notice envelope for routing: ${error.message}`);
    }
    for (const [stream, streamState] of [...ACTIVE_NOTICE_STREAMS.entries()]) {
        ...
        try {
            if (noticeEnvelope &&
                !shouldDeliverNoticeToState(resolveNoticeRoutingState(streamState), noticeEnvelope)) {
                continue;
            }
            stream.write(grpcFrame);
        }
        catch (error) {
            log.warn(`[PublicGatewayLocal] Failed writing notice to stream: ${error.message}`);
            ACTIVE_NOTICE_STREAMS.delete(stream);
        }
    }
}
function publishGatewayNotice(noticeTypeName, noticePayloadBuffer, targetGroup) {
    const noticeEnvelope = encodeNoticeEnvelope(noticeTypeName, noticePayloadBuffer, targetGroup);
    broadcastNoticeEnvelope(noticeEnvelope);
    return true;
}
...
function resolveStructurePaintworkNoticeState(structureID, options = {}) {
    ...
    const solarSystemID = normalizeProtoNumber(options.solarSystemID ||
...
function publishStructurePaintworkSetNotice(structureID, options = {}) {
    ...
    if (!state.structureID || !state.solarSystemID) {
        log.warn(`[PublicGatewayLocal] Skipped SetNotice for structureID=${structureID}; solar system unavailable`);
        return false;
    }
    const noticePayload = Buffer.from(StructurePaintworkSetNotice.encode(StructurePaintworkSetNotice.create({
        structure: buildStructureIdentifier(state.structureID),
        paintwork: state.paintwork,
    })).finish());
    const noticeEnvelope = encodeNoticeEnvelope("eve_public.cosmetic.structure.paintwork.api.SetNotice", noticePayload, {
        solar_system: state.solarSystemID,
    });
    log.info(`[PublicGatewayLocal] Notices.Publish eve_public.cosmetic.structure.paintwork.api.SetNotice ` +
...
function publishStructurePaintworkSetAllInSolarSystemNotice(solarSystemID) {
    ...
    const paintworks = getPaintworksForSolarSystem(numericSolarSystemID).map((license) => ({
    ...
    const noticePayload = Buffer.from(StructurePaintworkSetAllInSolarSystemNotice.encode(StructurePaintworkSetAllInSolarSystemNotice.create({
        paintworks,
        solar_system: buildSolarSystemIdentifier(numericSolarSystemID),
    })).finish());
    const noticeEnvelope = encodeNoticeEnvelope("eve_public.cosmetic.structure.paintwork.api.SetAllInSolarSystemNotice", noticePayload, {
        solar_system: numericSolarSystemID,
    });
    log.info(`[PublicGatewayLocal] Notices.Publish eve_public.cosmetic.structure.paintwork.api.SetAllInSolarSystemNotice ` +
...
function publishShipStateSetNotice(shipID, activeCharacterID = 0) {
    ...
    if (!state) {
        log.warn(`[PublicGatewayLocal] Skipped SetNotice for shipID=${shipID}; state unavailable`);
        return false;
    }
    const fallbackTargetCharacterID = normalizeProtoNumber(state.character && state.character.sequential) ||
        normalizeProtoNumber(activeCharacterID);
    ...
    if (targetCharacterIDs.length === 0) {
        log.warn(`[PublicGatewayLocal] Skipped SetNotice for shipID=${shipID}; target character unavailable`);
        return false;
    }
    const noticePayload = Buffer.from(ShipStateSetNotice.encode(ShipStateSetNotice.create({
        state,
    })).finish());
    for (const targetCharacterID of targetCharacterIDs) {
        const noticeEnvelope = encodeNoticeEnvelope("eve_public.cosmetic.ship.api.SetNotice", noticePayload, {
            character: targetCharacterID,
        });
        broadcastNoticeEnvelope(noticeEnvelope);
    }
    log.info(`[PublicGatewayLocal] Notices.Publish eve_public.cosmetic.ship.api.SetNotice shipID=${normalizeProtoNumber(shipID)} skinID=${normalizeProtoNumber(state &&
        state.skin &&
        state.skin.firstparty &&
        state.skin.firstparty.identifier
        ? state.skin.firstparty.identifier.sequential
        : 0)} targets=${targetCharacterIDs.join(",") || "none"} streams=${ACTIVE_NOTICE_STREAMS.size}`);
    return true;
}
function publishShipStateSetAllInBubbleNotice(activeCharacterID) {
    ...
    const noticePayload = Buffer.from(ShipStateSetAllInBubbleNotice.encode(ShipStateSetAllInBubbleNotice.create({
        state: states,
    })).finish());
    const noticeEnvelope = encodeNoticeEnvelope("eve_public.cosmetic.ship.api.SetAllInBubbleNotice", noticePayload, {
        character: numericCharacterID,
    });
    log.info(`[PublicGatewayLocal] Notices.Publish eve_public.cosmetic.ship.api.SetAllInBubbleNotice targetCharacterID=${numericCharacterID} states=${states.length} streams=${ACTIVE_NOTICE_STREAMS.size}`);
    broadcastNoticeEnvelope(noticeEnvelope);
    return true;
}
...
function encodeResponseEnvelope(requestEnvelope, statusCode, statusMessage, responseTypeName, responsePayloadBuffer) {
    const responseEnvelope = ResponseEnvelope.create({
        dispatched: timestampNow(),
        correlation_uuid: bufferFromBytes(requestEnvelope.correlation_uuid),
        status_code: statusCode,
        status_message: statusMessage || "",
        payload: responseTypeName !== null
            ? buildAny(responseTypeName, responsePayloadBuffer || EMPTY_PAYLOAD)
            : null,
        internal_origin: PUBLIC_GATEWAY_ORIGIN,
        application_instance_uuid: bufferFromBytes(requestEnvelope.application_instance_uuid),
...
function logGatewaySummary(context, result) {
    const message = `[PublicGatewayLocal] Requests.Send type=${context.requestTypeName || "<unknown>"} ` +
...
function logUnknownRequest(context) {
    ...
    log.warn(`[PublicGatewayLocal] Unknown eve_public request type=${key} ` +
...
function buildGatewayResponseForRequest(frameBuffer) {
    ...
    const requestTypeName = normalizeGatewayRequestTypeName(extractTypeName(requestEnvelope &&
        requestEnvelope.payload &&
        requestEnvelope.payload.type_url
        ? requestEnvelope.payload.type_url
        : ""));
    const context = buildRequestContext(requestEnvelope, requestTypeName, frameBuffer);
    let result;
    ...
    if (!result && requestTypeName === "eve_public.user.license.api.GetRequest") {
        ...
        log.info(`[PublicGatewayLocal] UserLicense.Get license_type=${licenseResult.licenseType || "<unknown>"} ` +
    ...
    function recoverFailedRequest(error) {
        ...
        if (emptySuccessResponseType) {
            log.warn(`[PublicGatewayLocal] Degraded failed gateway request type=${requestTypeName || "<unknown>"} ` +
        ...
        log.warn(`[PublicGatewayLocal] Gateway service request failed type=${requestTypeName || "<unknown>"} ` +
            `correlation=${context.correlation}: ${error.message}`);
        return {
            statusCode: 500,
    ...
    function encodeResult(resolved) {
        ...
        const responseBuffer = encodeResponseEnvelope(requestEnvelope, resolved.statusCode, resolved.statusMessage, resolved.responseTypeName, resolved.responsePayloadBuffer);
...
function handleNoticesConsumeStream(stream) {
    ...
    log.info(`[PublicGatewayLocal] Notices.Consume connected active=${ACTIVE_NOTICE_STREAMS.size}`);
    const parseChunk = createGrpcFrameParser((payload) => {
        try {
            ...
            log.debug(`[PublicGatewayLocal] Notices.Consume ignored undecodable payload: ${error.message}`);
        }
    });
    stream.on("data", parseChunk);
    stream.on("end", () => {
        log.debug("[PublicGatewayLocal] Notices.Consume request completed; keeping stream open");
    });
    stream.on("close", () => {
...
//# sourceMappingURL=publicGatewayLocal.js.map