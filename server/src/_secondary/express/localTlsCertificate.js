...
function createCertificateAuthority(options) {
    ...
    fs.writeFileSync(options.caKeyPath, forge.pki.privateKeyToPem(keyPair.privateKey), "utf8");
}
...
function buildLocalLeafCertificate(options) {
    ...
    fs.writeFileSync(options.outCertPath, `${forge.pki.certificateToPem(cert).trim()}\n${caPem}\n`, "utf8");
    fs.writeFileSync(options.outKeyPath, forge.pki.privateKeyToPem(keyPair.privateKey), "utf8");
}
...
function isIssuedByCertificateAuthority(certPem, caCertPath = CA_CERT_PATH) {
    try {
        ...
        const caCert = forge.pki.certificateFromPem(fs.readFileSync(caCertPath, "utf8"));
        return (certificateAttributesEqual(cert.issuer.attributes, caCert.subject.attributes) &&
            caCert.verify(cert));
    }
    catch {
        return false;
...
function hasRequiredAltNames(certPem) {
    ...
    return (dnsNames.size === LOCAL_TLS_DNS_ALT_NAMES.length &&
        ipAddresses.size === LOCAL_TLS_IP_ALT_NAMES.length &&
        LOCAL_TLS_DNS_ALT_NAMES.every((name) => dnsNames.has(String(name).toLowerCase())) &&
...
function ensureLocalLeafCertificate(options = {}) {
    ...
    const outCertPath = options.outCertPath || path.join(certDir, "gateway-dev-cert.pem");
    const outKeyPath = options.outKeyPath || path.join(certDir, "gateway-dev-key.pem");
    const caCertPath = options.caCertPath || CA_CERT_PATH;
    const caKeyPath = options.caKeyPath || CA_KEY_PATH;
    ensureCertificateAuthority({ caCertPath, caKeyPath });
...
function ensureXmppTlsCertificate(options = {}) {
    ...
    fs.writeFileSync(outCertPath, `${forge.pki.certificateToPem(cert).trim()}\n${caPem}\n`, "utf8");
    fs.writeFileSync(outKeyPath, forge.pki.privateKeyToPem(keyPair.privateKey), "utf8");
...
//# sourceMappingURL=localTlsCertificate.js.map