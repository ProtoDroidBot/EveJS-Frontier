...
const { ensureLocalLeafCertificate, } = require("./localTlsCertificate");
...
function hostMatchesPattern(hostname, pattern) {
    ...
    if (normalizedPattern.startsWith("*.")) {
        ...
        return (normalizedHost === normalizedPattern.slice(2) ||
            normalizedHost.endsWith(suffix));
    }
    if (normalizedPattern.startsWith(".")) {
        return (normalizedHost === normalizedPattern.slice(1) ||
...
function hostExistsInCollection(hostname, collection) {
    ...
    if (Array.isArray(collection)) {
        return collection.some((entry) => String(entry || "").trim().toLowerCase() === normalizedHost);
    }
    return false;
}
...
function shouldBlockHost(hostname) {
    ...
    return BLOCKED_PROXY_HOSTS.some((pattern) => hostMatchesPattern(hostname, pattern));
}
...
function isLaunchDarklyEventsRequest(routePath, headers = {}) {
    const authority = String(headers[":authority"] || headers.host || "").toLowerCase();
    const normalizedPath = String(routePath || "/").toLowerCase();
    return (authority === "events.launchdarkly.com" ||
...
function createLoopbackCdnRequestHandler() {
    return (req, res) => {
        const routePath = String(req.url || "/");
        const binaryAsset = String(req.method || "").toUpperCase() === "GET"
...
function createLoopbackCdnResponder(activeHttpsPort, listenPort = 443, bindHost = "127.0.0.1") {
    if (activeHttpsPort === listenPort) {
        return null;
    }
    const { tlsOptions } = loadLocalTlsOptions();
    const server = https.createServer(tlsOptions, createLoopbackCdnRequestHandler());
    server.on("error", (err) => {
        log.http2Err(`loopback CDN responder failed on ${bindHost}:${listenPort}: ${err.message}`);
    });
    server.listen(listenPort, bindHost, () => {
        log.debug(`loopback CDN responder listening on ${bindHost}:${listenPort}`);
    });
    return server;
}
function createLocalSecureResponder(httpsPort, bindHost) {
    ...
    try {
        if (certPem) {
            ...
            log.debug(`[local https cert] subject=${x509.subject} issuer=${x509.issuer} validTo=${x509.validTo}`);
        }
    }
    catch (err) {
    ...
    secureServer.on("stream", (stream, headers) => {
        ...
        const binaryAsset = method === "GET" ? getGatewayBinaryAsset(routePath) : null;
        if (binaryAsset) {
            stream.respond({
                ":status": 200,
        ...
        if (contentType.includes("application/grpc")) {
            stream[GRPC_TRAILERS_PROPERTY] = {
                "grpc-status": "12",
                "grpc-message": encodeURIComponent(`EveJS Elysian local gateway has no handler for ${routePath}`),
            };
            stream.respond({
                ":status": 200,
...
const ENABLE_LOCAL_INTERCEPT = parseBooleanEnv(process.env.EVEJS_PROXY_LOCAL_INTERCEPT, shouldEnableLocalInterceptByDefault());
const EXPRESS_PROXY_ENABLED = parseBooleanEnv(process.env.EVEJS_EXPRESS_PROXY_ENABLED, true);
const LOOPBACK_CDN_LISTEN_PORT = parseNonNegativeIntegerEnv(process.env.EVEJS_PROXY_LOOPBACK_CDN_LISTEN_PORT, 443);
const PROXY_FORWARD_UPSTREAM_URL = parseOptionalUrl(process.env.EVEJS_PROXY_UPSTREAM_BASE_URL);
const PROXY_GATEWAY_MODE = String(process.env.EVEJS_PROXY_GATEWAY_MODE ||
    (PROXY_FORWARD_UPSTREAM_URL ? "forward" : "local"))
    .trim()
    .toLowerCase();
const DEFAULT_PROXY_TUNNEL_IDLE_TIMEOUT_MS = parseNonNegativeIntegerEnv(process.env.EVEJS_PROXY_TUNNEL_IDLE_TIMEOUT_MS, 30_000);
const INTERCEPT_PROXY_TUNNEL_IDLE_TIMEOUT_MS = parseNonNegativeIntegerEnv(process.env.EVEJS_PROXY_INTERCEPT_TUNNEL_IDLE_TIMEOUT_MS, 0);
const SOCKET_KEEPALIVE_INITIAL_DELAY_MS = parseNonNegativeIntegerEnv(process.env.EVEJS_PROXY_SOCKET_KEEPALIVE_INITIAL_DELAY_MS, 15_000);
...
const PROXY_UNHANDLED_HOST_POLICY = normalizeUnhandledProxyHostPolicy(config.proxyUnhandledHostPolicy);
...
function getGatewayUpstreamTarget(defaultPort) {
    ...
    const configuredHost = String(process.env.EVEJS_PROXY_GATEWAY_UPSTREAM_HOST || "").trim();
    const configuredPort = Number.parseInt(process.env.EVEJS_PROXY_GATEWAY_UPSTREAM_PORT || "", 10);
    return {
...
function startServer(runtimeContext) {
    ...
    app.use((req, res, next) => {
        ...
        const shouldForwardLoopbackImage = targetUrl &&
        ...
        if (targetUrl && ENABLE_LOCAL_INTERCEPT && shouldInterceptHost(targetUrl.hostname)) {
            ...
            if (shouldForwardInterceptToUpstream()) {
                const upstreamTargetUrl = buildForwardTargetUrl(PROXY_FORWARD_UPSTREAM_URL, `${targetUrl.pathname}${targetUrl.search}`);
    ...
    mountSmartIndustryEndpoints(app);
    app.all(/.*/, (req, res) => {
        if (PROXY_FORWARD_UPSTREAM_URL) {
    ...
    if (shouldHandleInterceptLocally()) {
        ...
        createLoopbackCdnResponder(httpsPort, LOOPBACK_CDN_LISTEN_PORT, bindHost);
    }
    const proxyServer = http.createServer(app);
    const { mountEvejsWebGatewayUpgrades, } = require("./evejsWebGateway");
    const gatewayEventServer = mountEvejsWebGatewayUpgrades(proxyServer, runtimeContext);
    ...
    proxyServer.on("connect", (req, clientSocket, head) => {
        ...
        const upstreamSocket = net.connect(connectPort, connectHost, () => {
            clientSocket.write("HTTP/1.1 200 Connection Established\r\n" +
                "Proxy-Agent: EveJS Elysian\r\n" +
                "\r\n");
            wireTunnel(clientSocket, upstreamSocket, head, `${targetRaw} via ${connectHost}:${connectPort}`, {
    ...
    log.debug(`express proxy mode: ${shouldForwardInterceptToUpstream()
