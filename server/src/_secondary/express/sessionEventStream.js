...
function createSessionEventRuntime(options = {}) {
    ...
    const historyLimit = Math.min(normalizePositiveInteger(options.historyLimit, DEFAULT_EVENT_HISTORY_LIMIT), MAX_EVENT_HISTORY_LIMIT);
    ...
    function subscribe(bridgeSessionIDValue, cursorValue, handlers = {}) {
        ...
        if (canReplay(stream, cursor)) {
            ...
            initialFrames = [
                buildSnapshotFrame(stream, cursor ? "cursor_not_replayable" : "no_cursor"),
            ];
        }
...
//# sourceMappingURL=sessionEventStream.js.map