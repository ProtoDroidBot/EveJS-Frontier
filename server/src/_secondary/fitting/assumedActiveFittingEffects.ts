...
function findLoadedChargeForModule(fittedItems: any[] = [], moduleItem) {
...
function buildAssumedActiveModuleContexts(fittedItems: any[] = []) {
  const contexts: any[] = [];
...
function filterOtherActiveModuleContexts(activeModuleContexts: any[] = [], moduleItem) {
...
function buildAssumedCommandBurstState({
  ...
  const burstState: Record<string, any> = {};
  let applied = false;
