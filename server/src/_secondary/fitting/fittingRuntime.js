...
function buildShipSnapshotCacheKey(characterID, shipReference, options = {}) {
    const shipID = toInt(shipReference && typeof shipReference === "object"
        ? shipReference.itemID
        : shipReference, toInt(options.shipID, toInt(options.shipItem && options.shipItem.itemID, 0)));
    if (shipID <= 0) {
        return null;
    }
    const fittingMode = options.assumeActiveShipModules !== undefined &&
...
function buildRuntimeSnapshotRecord(characterID, shipReference, options = {}) {
    ...
    const trackedShipAttributes = normalizeSnapshotAttributes(builtSnapshot.shipAttributes);
    ...
    return {
        ...
        buildOnlineCandidateResourceState: builtSnapshot.buildOnlineCandidateResourceState,
    };
}
...
function listShipFittingAttributeChanges(previousSnapshot, nextSnapshot) {
    ...
    const previousAttributes = previousSnapshot.trackedShipAttributes || previousSnapshot.shipAttributes || {};
    const nextAttributes = nextSnapshot.trackedShipAttributes || nextSnapshot.shipAttributes || {};
    ...
    return [...attributeIDs]
        ...
        .filter((change) => Math.abs(change.nextValue - change.previousValue) > 1e-6);
}
function buildShipFittingSnapshotDiagnostics(snapshot) {
    ...
    return {
        ...
        powerOutput: toFiniteNumber(snapshot.resourceState && snapshot.resourceState.powerOutput, 0),
        trackedAttributeCount: Object.keys(snapshot.trackedShipAttributes || snapshot.shipAttributes || {}).length,
    };
}
...
//# sourceMappingURL=fittingRuntime.js.map