...
const { getCharacterSkills, } = require(path.join(__dirname, "../../services/skills/skillState"));
const { buildSkillEffectiveAttributes, getAttributeIDByNames, getTypeEffectRecords, } = require(path.join(__dirname, "../../services/fitting/liveFittingState"));
...
function normalizeSkillLevel(skill) {
    return Math.max(0, Math.min(5, toInt(skill &&
...
function buildFittingBrainEffectDefinition(skill, skillAttributes, modifierInfo) {
    const funcInfo = BRAIN_MODIFIER_TYPE_BY_FUNC[String(modifierInfo && modifierInfo.func) || ""] ||
        null;
    if (!funcInfo) {
        return null;
    }
...
function buildRequiredSkillBrainEffect(skill, skillAttributes, targetAttributeID, sourceAttributeID, options = {}) {
    const skillTypeID = toInt(skill && skill.typeID, 0);
...
function buildNamedFittingBrainEffectDefinition(skill, skillAttributes, effectRecord) {
    ...
    if (normalizedEffectName === "dronedmgbonus") {
        return buildRequiredSkillBrainEffect(skill, skillAttributes, ATTRIBUTE_DAMAGE_MULTIPLIER, ATTRIBUTE_DAMAGE_MULTIPLIER_BONUS, {
            domain: "character",
            modifierType: "ORS",
        });
    }
    ...
    if (missileDamageAttributeID > 0) {
        return buildRequiredSkillBrainEffect(skill, skillAttributes, missileDamageAttributeID, ATTRIBUTE_DAMAGE_MULTIPLIER_BONUS);
    }
    if (normalizedEffectName === "selfrof") {
        return buildRequiredSkillBrainEffect(skill, skillAttributes, ATTRIBUTE_SPEED, ATTRIBUTE_ROF_BONUS);
    }
    return null;
}
...
function buildFittingBrainEffectDefinitions(characterID) {
    ...
    for (const skill of getCharacterSkills(numericCharacterID)) {
        ...
        for (const effectRecord of getTypeEffectRecords(skillTypeID)) {
            const namedDefinition = buildNamedFittingBrainEffectDefinition(skill, skillAttributes, effectRecord);
            if (namedDefinition) {
            ...
            for (const modifierInfo of effectRecord && effectRecord.modifierInfo || []) {
                const definition = buildFittingBrainEffectDefinition(skill, skillAttributes, modifierInfo);
                if (!definition) {
                    continue;
                }
...
//# sourceMappingURL=fittingBrainBuilder.js.map