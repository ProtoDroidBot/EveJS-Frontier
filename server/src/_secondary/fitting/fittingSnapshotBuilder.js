...
const { findCharacterShipItem, findItemById, findShipItemById, } = require(path.join(__dirname, "../../services/inventory/itemStore"));
const { resolveItemByTypeID, } = require(path.join(__dirname, "../../services/inventory/itemTypeRegistry"));
const { getAttributeIDByNames, getEffectiveModuleResourceLoad, isShipFittingFlag, listFittedItems, listFittedItemsForLocation, buildShipResourceState, } = require(path.join(__dirname, "../../services/fitting/liveFittingState"));
const { collectAssumedActiveFittingEffects, } = require(path.join(__dirname, "./assumedActiveFittingEffects"));
const { resolveStructureEffectiveHitpoints, } = require(path.join(__dirname, "../../services/structure/structureFullPowerDogma"));
...
function resolveShipReferenceID(shipReference, options = {}) {
    return toInt(shipReference && typeof shipReference === "object"
        ? shipReference.itemID
        : shipReference, toInt(options.shipID, toInt(options.shipItem && options.shipItem.itemID, 0)));
}
...
function getSnapshotFittedItems(characterID, shipID, options = {}) {
    ...
    if (toInt(options.shipItem && options.shipItem.categoryID, 0) === STRUCTURE_CATEGORY_ID ||
...
function isModuleInSnapshot(snapshot, moduleItem) {
    ...
    if (toInt(snapshot.shipItem && snapshot.shipItem.categoryID, 0) === STRUCTURE_CATEGORY_ID ||
        toInt(shipTypeRecord.categoryID, 0) === STRUCTURE_CATEGORY_ID) {
        return snapshot.fittedItems.some((item) => toInt(item && item.itemID, 0) === toInt(moduleItem.itemID, 0));
    }
...
function buildResourceStateForItems(snapshot, fittedItems) {
    ...
    return buildSnapshotResourceState(snapshot.characterID, snapshot.shipItem, Array.isArray(fittedItems) ? fittedItems : snapshot.fittedItems, snapshot.skillMap, {
...
function getSnapshotModuleResourceLoad(snapshot, moduleItem, fittedItems = null) {
    ...
    return getEffectiveModuleResourceLoad(snapshot.shipItem, moduleItem, snapshot.skillMap, effectiveFittedItems, {
        additionalLocationModifierSources: getActiveImplantLocationModifierSources(snapshot.characterID),
        additionalShipAttributeModifierEntries: getActiveImplantShipModifierEntries(snapshot.characterID),
        ...(Array.isArray(snapshot.hiddenModifierItems)
            ? { hiddenModifierItems: snapshot.hiddenModifierItems }
            : {}),
    });
...
function buildOnlineCandidateResourceState(snapshot, moduleItem) {
    ...
    const baselineFittedItems = snapshot.fittedItems.filter((item) => toInt(item && item.itemID, 0) !== moduleID);
    const baselineResourceState = buildResourceStateForItems(snapshot, baselineFittedItems);
    ...
    }, moduleItem, baselineFittedItems);
    return {
        baselineFittedItems,
        baselineResourceState,
        moduleResourceLoad,
        cpuAfter: toFiniteNumber(baselineResourceState.cpuLoad, 0) +
            toFiniteNumber(moduleResourceLoad.cpuLoad, 0),
        powerAfter: toFiniteNumber(baselineResourceState.powerLoad, 0) +
            toFiniteNumber(moduleResourceLoad.powerLoad, 0),
    };
}
function buildSnapshotResourceState(characterID, shipItem, fittedItems, skillMap = null, options = {}) {
    const assumeActiveShipModules = options.assumeActiveShipModules !== undefined
    ...
    const resolvedSkillMap = baseResourceState && baseResourceState.skillMap instanceof Map
    ...
    const assumedActiveEffects = collectAssumedActiveFittingEffects({
        characterID,
        shipItem,
        fittedItems: (baseResourceState && baseResourceState.fittedItems) || normalizedFittedItems,
        skillMap: resolvedSkillMap,
        ...(Array.isArray(options.hiddenModifierItems)
            ? { hiddenModifierItems: options.hiddenModifierItems }
            : {}),
    });
    const shipAttributeModifierEntries = Array.isArray(assumedActiveEffects && assumedActiveEffects.shipAttributeModifierEntries)
        ? assumedActiveEffects.shipAttributeModifierEntries
        : [];
    const assumedActiveModuleContexts = Array.isArray(assumedActiveEffects && assumedActiveEffects.activeModuleContexts)
...
function buildFittingSnapshot(characterID, shipReference, options = {}) {
    ...
    const shipItem = resolveSnapshotShipItem(numericCharacterID, shipReference, options);
    if (!shipItem) {
        return null;
    }
    const shipID = toInt(shipItem.itemID, resolveShipReferenceID(shipReference, options));
    const fittedItems = getSnapshotFittedItems(numericCharacterID, shipID, { ...options, shipItem });
    const snapshotResourceState = buildSnapshotResourceState(numericCharacterID, shipItem, fittedItems, null, options);
    const resourceState = applyStructureEffectiveHitpointsToSnapshot(snapshotResourceState.resourceState, shipItem);
    const snapshot = {
        characterID: numericCharacterID,
        shipID,
        shipItem,
        fittedItems: resourceState.fittedItems || fittedItems,
        assumeActiveShipModules: snapshotResourceState.assumeActiveShipModules !== false,
        assumedActiveModuleContexts: snapshotResourceState.assumedActiveModuleContexts || [],
        skillMap: resourceState.skillMap instanceof Map
    ...
    return {
        ...snapshot,
        buildResourceStateForItems: (nextFittedItems) => buildResourceStateForItems(snapshot, nextFittedItems),
        getModuleResourceLoad: (moduleItem, nextFittedItems = null) => getSnapshotModuleResourceLoad(snapshot, moduleItem, nextFittedItems),
        getModuleAttributeOverrides: (moduleItem) => buildSnapshotModuleAttributeOverrides(snapshot, moduleItem),
        buildOnlineCandidateResourceState: (moduleItem) => buildOnlineCandidateResourceState(snapshot, moduleItem),
    };
}
module.exports = {
...
//# sourceMappingURL=fittingSnapshotBuilder.js.map