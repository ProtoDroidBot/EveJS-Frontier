...
function buildFitDataPayload(fitData: any[] = []) {
  return buildList(
...
function normalizeStoredFittingRecord(rawRecord, options: Record<string, any> = {}) {
  const source =
...
function collectUsedFittingIDs() {
  ...
  const owners =
    storeRoot && storeRoot.owners && typeof storeRoot.owners === "object"
      ? storeRoot.owners
      : {};
  for (const ownerRecord of Object.values<any>(owners)) {
    const fittings =
  ...
  for (const [characterID, record] of Object.entries<any>(characters)) {
    ...
    const legacyFittings =
      record && record.savedFittings && typeof record.savedFittings === "object"
        ...
        : {};
    const legacyEntries = Object.entries<any>(legacyFittings);
...
function getOwnerFittings(ownerID, options: Record<string, any> = {}) {
...
function saveManyFittings(ownerID, rawFittings, ownerScope = null) {
  ...
  const ownerRecord = ensureOwnerRecord(numericOwnerID, ownerScope);
  const fittingEntries = Object.entries<any>(
  ...
  const nextFittings: Record<string, any> = { ...(ownerRecord.fittings || {}) };
...
function deleteManyFittings(ownerID, fittingIDs, ownerScope = null) {
  ...
  const ids = Array.isArray(unwrapMarshalValue(fittingIDs))
    ...
    : [];
  const ownerRecord = ensureOwnerRecord(numericOwnerID, ownerScope);
  const deletedFittingIDs: any[] = [];
  const uniqueIDs: any[] = [...new Set(ids.map((value) => toInt(value, 0)).filter((value) => value > 0))];
  const nextFittings: Record<string, any> = { ...((ownerRecord && ownerRecord.fittings) || {}) };
...
function throwFittingError(errorMsg = "", values: Record<string, any> = {}) {
