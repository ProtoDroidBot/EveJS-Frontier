...
const { getTypeDogmaEffects, getEffectTypeRecord, buildModuleStatusSnapshot, appendDirectModifierEntries, applyModifierGroups, } = require(path.join(__dirname, "../../services/fitting/liveFittingState"));
const { buildLiveModuleAttributeMap, } = require(path.join(__dirname, "../../space/modules/liveModuleAttributes"));
...
function shouldSkipAssumedActiveModule(moduleItem, effectRecord) {
    ...
    if (normalizedEffectName.includes("cloak") ||
...
function findLoadedChargeForModule(fittedItems = [], moduleItem) {
    ...
    return ((Array.isArray(fittedItems) ? fittedItems : []).find((item) => (item &&
        isChargeItem(item) &&
        toInt(item.flagID, 0) === moduleFlagID &&
        toInt(item.locationID, 0) === moduleLocationID)) || null);
}
...
function filterOtherActiveModuleContexts(activeModuleContexts = [], moduleItem) {
    ...
    return (Array.isArray(activeModuleContexts) ? activeModuleContexts : []).filter((context) => toInt(context && context.moduleItem && context.moduleItem.itemID, 0) !== moduleID);
}
function buildAssumedCommandBurstState({ characterID = 0, shipItem, fittedItems, skillMap, activeModuleContexts, nowMs, } = {}) {
    const burstState = {};
    let applied = false;
    for (const activeModuleContext of Array.isArray(activeModuleContexts)
        ...
        const activation = commandBurstRuntime.resolveCommandBurstActivation({
            ...
            skillMap,
            fittedItems,
            activeModuleContexts: filterOtherActiveModuleContexts(activeModuleContexts, activeModuleContext.moduleItem),
        });
        if (!activation || activation.success !== true || !activation.data) {
            continue;
        }
        const effectStatePatch = activation.data.effectStatePatch &&
        ...
        const collectionValues = effectStatePatch.commandBurstDbuffValues instanceof Map
        ...
        commandBurstRuntime.applyTimedCommandBurstToEntity(burstState, collectionValues, durationMs, nowMs);
        applied = true;
    }
...
function collectCommandBurstModifierEntries(commandBurstState, targetItem, nowMs) {
    ...
    return commandBurstRuntime.collectModifierEntriesForItem(commandBurstState, targetItem, nowMs);
}
function buildActiveModuleAttributeMap({ shipItem, moduleItem, chargeItem, skillMap, fittedItems, activeModuleContexts, commandBurstState, nowMs, hiddenModifierItems = null, } = {}) {
    const moduleAttributes = buildLiveModuleAttributeMap(shipItem, moduleItem, chargeItem, skillMap, fittedItems, filterOtherActiveModuleContexts(activeModuleContexts, moduleItem), Array.isArray(hiddenModifierItems) ? { hiddenModifierItems } : {}) || {};
    const commandBurstModifierEntries = collectCommandBurstModifierEntries(commandBurstState, moduleItem, nowMs);
    if (commandBurstModifierEntries.length > 0) {
        applyModifierGroups(moduleAttributes, commandBurstModifierEntries);
    }
    return moduleAttributes;
}
function collectAssumedActiveFittingEffects({ characterID = 0, shipItem, fittedItems = [], skillMap = new Map(), hiddenModifierItems = null, nowMs = Date.now(), } = {}) {
    if (!shipItem) {
        return {
            activeModuleContexts: [],
    ...
    const shipAttributeModifierEntries = collectCommandBurstModifierEntries(commandBurstState, shipItem, nowMs);
    for (const activeModuleContext of activeModuleContexts) {
        ...
        appendDirectModifierEntries(shipAttributeModifierEntries, moduleAttributes, [activeModuleContext.effectRecord], "fittedModule");
    }
    return {
        activeModuleContexts,
        shipAttributeModifierEntries,
...
//# sourceMappingURL=assumedActiveFittingEffects.js.map