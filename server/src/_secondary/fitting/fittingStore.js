...
const { buildDict, buildFiletimeLong, buildKeyVal, buildList, unwrapMarshalValue, currentFileTime, } = require(path.join(__dirname, "../../services/_shared/serviceHelpers"));
const { getCharacterRecord, } = require(path.join(__dirname, "../../services/character/characterState"));
const { getCorporationRecord, getAllianceRecord, } = require(path.join(__dirname, "../../services/corporation/corporationState"));
const { resolveItemByTypeID, } = require(path.join(__dirname, "../../services/inventory/itemTypeRegistry"));
const { isShipFittingFlag, } = require(path.join(__dirname, "../../services/fitting/liveFittingState"));
const { ITEM_FLAGS, } = require(path.join(__dirname, "../../services/inventory/itemStore"));
...
function buildFitDataPayload(fitData = []) {
    return buildList((Array.isArray(fitData) ? fitData : []).map(([typeID, flagID, quantity]) => ({
        type: "tuple",
...
function assertSessionCanMutateOwner(session, ownerID, ownerScope) {
    ...
    if (normalizedScope === OWNER_SCOPE.ALLIANCE) {
        ...
        const executorCorporationID = toInt(alliance && (alliance.executorCorporationID || alliance.executorCorpID), 0);
...
function isCommunityOwnerScope(ownerScope, ownerID) {
    return (normalizeOwnerScope(ownerScope) === OWNER_SCOPE.COMMUNITY ||
...
function ensureOwnerRecord(ownerID, ownerScope = null) {
    ...
    const owners = storeRoot && storeRoot.owners && typeof storeRoot.owners === "object"
        ? storeRoot.owners
        : {};
    const ownerKey = String(numericOwnerID);
    let ownerRecord = owners[ownerKey] && typeof owners[ownerKey] === "object"
        ? owners[ownerKey]
        : null;
    if (!ownerRecord) {
        ownerRecord = {
        ...
        const normalizedScope = normalizeOwnerScope(ownerRecord.scope, ownerScope || inferOwnerScope(numericOwnerID));
...
function normalizeStoredFittingRecord(rawRecord, options = {}) {
    ...
    const nameLabel = normalizeText(firstDefined(source.nameLabel, source.labelPath, source.localizationLabel), "").trim();
    const icon = normalizeText(firstDefined(source.icon, source.iconPath, source.texturePath), "").trim();
    const insurance = normalizeText(firstDefined(source.insurance, source.insuranceType, source.insuranceTypeName), "").trim();
...
function collectUsedFittingIDs() {
    ...
    for (const ownerRecord of Object.values(owners)) {
        const fittings = ownerRecord && ownerRecord.fittings && typeof ownerRecord.fittings === "object"
...
function allocateNextFittingID() {
    ensureStoreInitialized();
    const nextFittingID = Math.max(1, toInt(storeRoot && storeRoot._meta && storeRoot._meta.nextFittingID, 1));
    writeStoreMeta({
        ...(storeRoot._meta || {}),
...
function allocateNextFittingIDs(count = 1) {
    ...
    ensureStoreInitialized();
    const firstFittingID = Math.max(1, toInt(storeRoot && storeRoot._meta && storeRoot._meta.nextFittingID, 1));
...
function migrateLegacyCharacterFittings() {
    ...
    const characters = charactersResult.success && charactersResult.data && typeof charactersResult.data === "object"
        ? charactersResult.data
        : {};
    const usedIDs = collectUsedFittingIDs();
    let mutated = false;
    let highestFittingID = toInt(storeRoot && storeRoot._meta && storeRoot._meta.nextFittingID, 1) - 1;
    for (const [characterID, record] of Object.entries(characters)) {
        const numericCharacterID = toInt(characterID, 0);
        if (numericCharacterID <= 0) {
            continue;
        }
        const legacyFittings = record && record.savedFittings && typeof record.savedFittings === "object"
        ...
        for (const [legacyFittingID, legacyRecord] of legacyEntries) {
            ...
            if (ownerRecord.fittings[String(normalizedRecord.fittingID)] &&
                normalizeStoredFittingRecord(ownerRecord.fittings[String(normalizedRecord.fittingID)], {
    ...
    storeRoot._meta.nextFittingID = Math.max(toInt(storeRoot._meta.nextFittingID, 1), highestFittingID + 1);
...
function getOwnerFittings(ownerID, options = {}) {
    ...
    const fittings = ownerRecord && ownerRecord.fittings && typeof ownerRecord.fittings === "object"
        ? ownerRecord.fittings
        : {};
    return Object.fromEntries(Object.entries(fittings)
...
function getOwnerFittingsResponse(ownerID, ownerScope = null) {
    ...
    const fittings = ownerRecord && ownerRecord.fittings && typeof ownerRecord.fittings === "object"
        ? ownerRecord.fittings
        : {};
    const payload = buildDict(Object.values(fittings)
        .map((record) => normalizeStoredFittingRecord(record, {
...
function updateFitting(ownerID, fittingID, rawFitting, ownerScope = null) {
    ...
    const ownerRecord = ensureOwnerRecord(numericOwnerID, ownerScope);
    const existingRecord = ownerRecord && ownerRecord.fittings
        ? ownerRecord.fittings[String(numericFittingID)]
        : null;
    if (!existingRecord) {
        return { success: false, errorMsg: "FITTING_NOT_FOUND" };
...
function saveManyFittings(ownerID, rawFittings, ownerScope = null) {
    ...
    const ownerRecord = ensureOwnerRecord(numericOwnerID, ownerScope);
    const fittingEntries = Object.entries(normalizedPayload && typeof normalizedPayload === "object" ? normalizedPayload : {});
...
//# sourceMappingURL=fittingStore.js.map