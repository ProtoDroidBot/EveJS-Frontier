...
const disabledMissionTemplateIdentifiers = new Set(
  productionMissionPolicy.disabledMissions.flatMap(({ templateIDs }: Record<string, any>) =>
...
function isDisabledMissionIdentifier(value) {
  ...
  return productionMissionPolicy.disabledMissions.some(
    ({ missionID: disabledMissionID }: Record<string, any>) => missionID === disabledMissionID,
  );
}
