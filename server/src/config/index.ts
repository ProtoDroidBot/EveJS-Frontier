...
const CONFIG_ENTRY_DEFINITIONS: any[] = [
  {
  ...
  },
  {
    ...
    envType: "boolean",
    description:
      "Enables server-authoritative remote solar-system surveys. Network Nodes are the temporary dApp source; the scan contracts remain source-neutral.",
    validValues: "true or false.",
  },
  {
    ...
    maxValue: 25,
    description:
      "Maximum configurable remote scanning reach from a Network Node, measured along stargate connections in solar-system hops.",
...
function withDefinedEntries(values) {
  return Object.fromEntries(
    Object.entries<any>(values).filter(([, value]) => value !== undefined),
  );
}
...
function normalizePersistedConfig(rawConfig: Record<string, any> = {}) {
...
function buildPersistedConfigSnapshot(rawConfig: Record<string, any> = {}) {
...
function buildConfigPropertyLines(key, value, isLastEntry) {
  ...
  const lines: any[] = [`${prefix}${valueLines[0]}`];
...
function buildCommentedConfigText(nextConfig: Record<string, any> = {}) {
...
function syncConfigFileDefaults(filePath, rawConfig: Record<string, any> = {}, options: Record<string, any> = {}) {
...
function getConfigStateSnapshot() {
  const sharedRawConfig = normalizePersistedConfig(readJsonConfig(sharedConfigPath));
  const localRawConfig = normalizePersistedConfig(readJsonConfig(localConfigPath));
  const fileConfigSnapshot: Record<string, any> = {
    ...defaults,
    ...sharedRawConfig,
    ...localRawConfig,
  };
...
function buildValidatedConfigValues(rawValues: Record<string, any> = {}, options: Record<string, any> = {}) {
  if (
    rawValues === null ||
...
function validateFrontierSmartIndustryJobLaneCountByTypeID(value) {
  ...
  const compatibleTypeIDs = new Set(Object.keys(
    require("../services/frontier/industryStaticData.json").facilities || {},
  ));
  const normalized: Record<string, number> = {};
  for (const [rawTypeID, laneCount] of Object.entries<any>(value)) {
    const typeID = String(rawTypeID).trim();
    if (!/^[1-9][0-9]*$/.test(typeID) || !Number.isSafeInteger(Number(typeID))) {
      throw new Error(
        `frontierSmartIndustryJobLaneCountByTypeID key ${JSON.stringify(rawTypeID)} must be a positive integer type ID.`,
      );
    }
    if (!compatibleTypeIDs.has(typeID)) {
      throw new Error(
        `frontierSmartIndustryJobLaneCountByTypeID type ID ${typeID} is not a compatible Smart Industry facility.`,
      );
    }
    if (!Number.isInteger(laneCount) || laneCount < 1 || laneCount > 16) {
      throw new Error(
        `frontierSmartIndustryJobLaneCountByTypeID[${typeID}] must be an integer from 1 through 16.`,
      );
    }
...
function saveLocalConfig(rawValues: Record<string, any> = {}) {
