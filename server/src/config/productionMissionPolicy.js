...
function isValidMissionRewardPresentation(value) {
    ...
    const validReward = (reward, timed = false) => (reward &&
        ...
        reward.quantity > 0 &&
        (!timed || (Number.isInteger(reward.timeIntervalMinutes) &&
    ...
    return (hasNormal || hasBonus || hasLoyaltyPoints) &&
        ...
        (!hasLoyaltyPoints || (Number.isInteger(value.loyaltyPoints) && value.loyaltyPoints >= 0));
}
function isValidMissionSetup(value) {
    ...
    return validRoute && validPosition && (!Object.prototype.hasOwnProperty.call(value, "reward") ||
...
function validateProductionMissionPolicy(value) {
    ...
    const goldenSecurityMissions = value.goldenSecurityMissions.map((entry, index) => {
        ...
        if (mappedTemplateID && mappedTemplateID !== templateKey) {
            throw new TypeError(`golden dungeonID ${dungeonID} must map consistently to templateID ${mappedTemplateID}`);
        }
        const mappedDungeonID = dungeonIDByTemplateID.get(templateKey);
        if (mappedDungeonID && mappedDungeonID !== dungeonID) {
            throw new TypeError(`golden templateID ${templateID} must map consistently to dungeonID ${mappedDungeonID}`);
        }
    ...
    const disabledMissions = value.disabledMissions.map((entry, index) => {
        ...
        const templateIDs = entry.templateIDs.map((templateID, templateIndex) => {
            const normalized = nonEmptyText(templateID, `${prefix}.templateIDs[${templateIndex}]`);
    ...
    const minInclusive = positiveInteger(generatedMissionIDRange.minInclusive, "generatedMissionIDRange.minInclusive");
    const maxExclusive = positiveInteger(generatedMissionIDRange.maxExclusive, "generatedMissionIDRange.maxExclusive");
    ...
    for (const { missionID, templateID } of goldenSecurityMissions) {
        if (missionID >= minInclusive && missionID < maxExclusive) {
            throw new TypeError(`golden missionID ${missionID} cannot be inside generatedMissionIDRange`);
        }
        ...
        if (goldenTemplateAliases.some((alias) => disabledTemplateAliases.has(alias))) {
            throw new TypeError(`templateID ${templateID} cannot be both golden and disabled`);
        }
        ...
        if (retiredTemplatePrefixes.some((prefix) => normalizedTemplateID.startsWith(prefix))) {
            throw new TypeError(`golden templateID ${templateID} cannot use a retired template prefix`);
        }
    }
    return {
        version,
...
const productionMissionPolicy = deepFreeze(validateProductionMissionPolicy(rawPolicy));
const disabledMissionTemplateIdentifiers = new Set(productionMissionPolicy.disabledMissions.flatMap(({ templateIDs }) => templateIDs.flatMap(normalizedTemplateAliases)));
...
function isDisabledMissionIdentifier(value) {
    ...
    return productionMissionPolicy.disabledMissions.some(({ missionID: disabledMissionID }) => missionID === disabledMissionID);
}
...
function isRetiredMissionTemplateIdentifier(value) {
    ...
    return isDisabledMissionTemplateIdentifier(normalized) ||
        ...
        productionMissionPolicy.retiredTemplatePrefixes.some((prefix) => normalized.startsWith(prefix)) ||
...
//# sourceMappingURL=productionMissionPolicy.js.map