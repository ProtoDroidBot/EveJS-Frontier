...
function realpathExisting(filePath) {
    ...
    while (true) {
        try {
            return path.resolve(fs.realpathSync.native(existingCandidate), ...missingSegments);
        }
        catch (_) {
...
function verifyNodeTestStoreAttestation() {
    ...
    const attestationPath = path.resolve(process.env.EVEJS_TEST_STORE_ATTESTATION ||
    ...
    try {
        ...
        const attested = attestation.schemaVersion === 1 &&
        ...
        return !protectedGameStoreRoots(attestation).some((protectedRoot) => pathsOverlap(storeRealpath, protectedRoot) || isSubpath(dataRealpath, realpathExisting(protectedRoot)));
    }
    catch (_) {
        return false;
    }
...
function assertNodeTestIsolationBeforeOpen() {
    ...
    throw new Error("Refusing to import server/src/gameStore in an unisolated node:test process. " +
...
function ensureSqliteReady() {
    ...
    if (SQLITE_TABLES.size > 0 && sqliteRecoveryRequired) {
        ...
        if (recovered.length > 0) {
            dbWarn(`recovered ${recovered.length} unacknowledged persistence operation` +
...
function loadTable(table) {
    ...
    if (RECOVERABLE_EMPTY_TABLES.has(table) &&
...
function preloadAll() {
    ...
    dbLog(`${pc.green("cache ready")} — ${tables.length} tables, ` +
...
function handlePersistenceWorkerAcknowledged(operation) {
    ...
    if (!inFlight) {
        ...
        throw new Error(`unexpected persistence acknowledgment ${operation.operationId} for ${operation.table}`);
    }
    if (inFlight.operationId !== operation.operationId) {
        throw new Error(`stale persistence acknowledgment ${operation.operationId} cannot resolve ` +
...
function handlePersistenceWorkerRecovered(operations = []) {
    ...
    for (const operation of operations) {
        ...
        if (inFlight) {
            if (inFlight.operationId !== operation.operationId) {
                throw new Error(`stale recovery ${operation.operationId} cannot resolve ` +
                    `${inFlight.operationId} for ${operation.table}`);
            }
            advanceFlushBaseline(operation.table, inFlight.upserts, inFlight.deletes);
            inFlightFlushes.delete(operation.table);
        ...
        lastCompletedPersistenceOperationId.set(operation.table, Math.max(operation.operationId, lastCompletedPersistenceOperationId.get(operation.table) || 0));
...
function flushSqliteTable(table, options = {}) {
    ensureSqliteReady();
    if (persistenceWorker.isEnabled() &&
    ...
    const usePartial = DIRTY_ROWS_TRACKING &&
        trackedKeys &&
        !fullDirty.has(table) &&
    ...
    if (useWorker) {
        ...
        try {
            const reconciled = sqliteStore.reconcilePersistenceOperation(operation.operationId, table);
            if (!reconciled) {
                throw new Error(`persistence operation ${operation.operationId} disappeared before reconciliation`);
            }
        }
        catch (error) {
...
function handlePersistenceWorkerError(table, error, failure) {
    ...
    if (failure && table) {
        ...
        if (inFlight && inFlight.operationId !== failure.operationId) {
            dbWarn(`ignored failure ${failure.operationId} while ${inFlight.operationId} is in flight for ${table}`);
        }
    }
}
...
function reconcileInFlightFlush(table) {
    ...
    if (remaining && remaining.operationId === inFlight.operationId) {
        throw new Error(`persistence operation ${inFlight.operationId} remained unresolved after reconciliation`);
    }
    return true;
}
function flushTableSync(table, options = {}) {
    ...
    try {
        ...
        if (sqliteTable) {
            ...
            safeWriteFileSync(dataFilePath(table), JSON.stringify(buildFlushSnapshot(table), null, 2));
        }
        dirty.delete(table);
        if (options.log === true) {
...
function flushTablesSync(tables = []) {
    const uniqueTables = [...new Set((Array.isArray(tables) ? tables : [tables]).filter((table) => Boolean(table)))];
    const results = [];
    let success = true;
