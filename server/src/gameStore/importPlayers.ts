...
function main() {
  ...
  function nextNotificationID() {
    if (notifCounter === null) {
      ...
      for (const box of Object.values<any>(boxes)) {
  ...
  const summary: Record<string, any> = { accounts: 0, characters: 0, items: 0, skipped: [] };
...
function importCharacter(charEntry, newAccountID, ctx) {
  ...
  const itemMap = new Map<number, number>(oldItemIDs.map((id, i) => [id, newItemIDs[i]]));
  ...
  if (charEntry.skills && typeof charEntry.skills === "object") {
    ...
    const newSkillMap: Record<string, any> = {};
    for (const [typeID, sk] of Object.entries<any>(charEntry.skills)) {
  ...
  if (charEntry.mail) {
    ...
    if (charEntry.mail.mailbox) {
      ...
      if (box.statuses && typeof box.statuses === "object") {
        ...
        for (const [mid, st] of Object.entries<any>(box.statuses)) {
  ...
  if (charEntry.notifications && charEntry.notifications.box) {
    ...
    for (const [nid, n] of Object.entries<any>(box.byID || {})) {
