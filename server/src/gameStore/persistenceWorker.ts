...
import type { PersistenceOperation } from "./persistenceTypes";

type WorkerHandle = Pick<NodeWorker, "postMessage" | "on" | "once" | "removeListener" | "ref" | "unref" | "terminate">;
interface PersistenceWorkerState {
  worker: WorkerHandle | null;
  counters: Int32Array;
  dbPath: string;
  pendingOperationIds: Set<number>;
  ...
  shutdownPromise: Promise<unknown> | null;
}
...
function copyDeletes(deletes: unknown): string[] {
...
function copyOperation(operation: PersistenceOperation | null): PersistenceOperation | null {
  if (!operation) {
    return null;
  }
  return {
...
function createController(options: Record<string, any> = {}) {
  const createWorker =
  ...
  let state: PersistenceWorkerState | null = null;
  ...
  function rememberCallbackError(targetState: PersistenceWorkerState | null, error, fallback) {
  ...
  function invokeErrorCallback(targetState: PersistenceWorkerState | null, table, message, failure) {
    if (!onErrorCallback) {
      return;
    }
    try {
  ...
  function invokeAcknowledgedCallback(targetState: PersistenceWorkerState | null, operation) {
    ...
    try {
      ...
      const callbackError: Error & Record<string, any> = new Error(message);
  ...
  function invokeRecoveredCallback(targetState: PersistenceWorkerState | null, operations) {
  ...
  function findPendingOperation(targetState: PersistenceWorkerState | null, message) {
    if (
      !message ||
  ...
  function recordFailure(
    targetState: PersistenceWorkerState | null,
    operation,
    rawError,
    source,
    notify = true,
  ...
  function failPendingWrites(targetState: PersistenceWorkerState | null, rawError, source, notify = true) {
    const failures: any[] = [];
  ...
  function notifyWorkerReconciled(targetState: PersistenceWorkerState | null, operation) {
    ...
    try {
      ...
      invokeErrorCallback(targetState, operation.table, message, null);
      const noticeError: Error & Record<string, any> = new Error(message);
  ...
  function acknowledgeAppliedOperation(targetState: PersistenceWorkerState | null, operation) {
  ...
  function consumeQueuedFailureSignal(targetState: PersistenceWorkerState | null) {
    while (true) {
  ...
  function handleWorkerMessage(targetState: PersistenceWorkerState | null, message) {
  ...
  function attachWorkerListeners(targetState: PersistenceWorkerState | null) {
  ...
  function ensureWorker(dbPath) {
    if (state && !state.exited) {
      if (state.shuttingDown) {
        const error: Error & Record<string, any> = new Error("persistence worker shutdown is in progress");
    ...
    const nextState: PersistenceWorkerState = {
      worker: null,
      counters,
      dbPath,
  ...
  function registerPendingOperation(operation, dbPath) {
    ...
    const local: Record<string, any> = { ...durable, dbPath, workerState: null };
    pendingWrites.set(local.operationId, local);
    pendingByTable.set(local.table, local.operationId);
    return local;
  }

  function bindOperationToWorker(operation, targetState: PersistenceWorkerState | null) {
    operation.workerState = targetState;
  ...
  function makeDrainResult(targetState: PersistenceWorkerState | null, values: Record<string, any> = {}) {
   ...
   */
  function drain(timeoutMs = defaultDrainTimeoutMs, targetState: PersistenceWorkerState | null = state) {
    if (!targetState) {
   ...
   */
  function submitWrite(dbPath, table, upserts, deletes, submitOptions: Record<string, any> = {}) {
    assertControllerDatabase(dbPath);
    ...
    try {
      ...
      const postError: Error & Record<string, any> = new Error(message);
    ...
    if (submitOptions.sync === true) {
      ...
      if (!drainResult.drained || drainResult.timedOut) {
        const drainError: Error & Record<string, any> = new Error(
      ...
      if (pendingWrites.has(operation.operationId)) {
        ...
        try {
          ...
          if (!exactState) {
            if (!operation.acknowledgedOperation) {
              const missingError: Error & Record<string, any> = new Error(
            ...
            const writeError: Error & Record<string, any> = new Error(failure.error);
          ...
          const finalizationError: Error & Record<string, any> = new Error(message);
  ...
  function requestClose(targetState: PersistenceWorkerState | null, timeoutMs) {
  ...
  function waitForExit(targetState: PersistenceWorkerState | null, timeoutMs) {
    if (targetState.exited) {
  ...
  async function performShutdown(targetState: PersistenceWorkerState | null) {
    let drainResult;
    try {
      drainResult = drain(defaultDrainTimeoutMs, targetState);
    } catch (error) {
    ...
    let closeWait: Record<string, any> = { kind: "exit", postError: null };
    if (!targetState.exited) {
      try {
