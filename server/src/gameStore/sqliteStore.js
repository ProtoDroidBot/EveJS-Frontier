...
const ROW_GROUPS = {
    ...
    creationPresets: ["owners"],
    smartAssemblyConstructionTemplates: ["owners", "principals"],
    smartAssemblyDeploymentPlans: ["owners", "principals"],
        ...
        if (isGroup) {
            ...
            for (const entityKey of Object.keys(value)) {
                if (String(entityKey).includes(ROW_KEY_SEP)) {
                    throw new Error(`entity key contains reserved separator: ${table}.${key}.${entityKey}`);
                }
...
function assembleFromRows(table, rows = {}) {
    ...
    for (const rowKey of Object.keys(rows)) {
        ...
        if (sepIndex >= 0) {
            ...
            const blob = rows[rowKey] && typeof rows[rowKey] === "object" && !Array.isArray(rows[rowKey])
                ? rows[rowKey]
                : {};
...
function init(targetPath) {
    ...
    db.exec("CREATE TABLE IF NOT EXISTS _migrations (table_name TEXT PRIMARY KEY, migrated_at TEXT NOT NULL)");
...
function isMigrated(table) {
    ...
    return Boolean(requireDb().prepare("SELECT 1 FROM _migrations WHERE table_name = ?").get(table));
}
function markMigrated(table) {
    assertSafeTableName(table);
    requireDb()
        .prepare(`INSERT INTO _migrations(table_name, migrated_at) VALUES (?, ?)
...
function ensureSqlTable(table) {
    ...
    requireDb().exec(`CREATE TABLE IF NOT EXISTS "${table}" (key TEXT PRIMARY KEY, json TEXT NOT NULL)`);
    ensuredTables.add(table);
}
function getUpsertStatement(table) {
    if (!upsertStatements.has(table)) {
        upsertStatements.set(table, requireDb().prepare(`INSERT INTO "${table}"(key, json) VALUES (?, ?)
...
function getDeleteStatement(table) {
    if (!deleteStatements.has(table)) {
        deleteStatements.set(table, requireDb().prepare(`DELETE FROM "${table}" WHERE key = ?`));
    }
...
function normalizePersistenceOperationId(operationId) {
    if (!Number.isSafeInteger(operationId) || operationId <= 0) {
        throw new Error(`invalid persistence operation ID: ${JSON.stringify(operationId)}`);
    }
    return operationId;
}
...
function parsePersistencePayload(raw, label, operationId) {
    if (typeof raw !== "string") {
        throw new Error(`persistence operation ${operationId} has a non-string ${label} payload`);
    }
    try {
        return JSON.parse(raw);
    }
    catch (error) {
        throw new Error(`persistence operation ${operationId} has malformed ${label} JSON: ${error.message}`);
    }
}
function parsePersistenceOperationRow(row) {
    ...
    const table = normalizePersistenceTable(row.table_name, `persistence operation ${operationId} table`);
    const upserts = validatePersistenceUpserts(parsePersistencePayload(row.upserts_json, "upserts", operationId), `persistence operation ${operationId} upserts`);
    const deletes = validatePersistenceDeletes(parsePersistencePayload(row.deletes_json, "deletes", operationId), `persistence operation ${operationId} deletes`);
    if (row.state !== "pending" && row.state !== "applied") {
        throw new Error(`persistence operation ${operationId} has invalid state: ${JSON.stringify(row.state)}`);
    }
    ...
    if (row.state === "pending" && row.applied_at !== null) {
        throw new Error(`pending persistence operation ${operationId} unexpectedly has applied_at`);
    }
    if (row.state === "applied" &&
        (typeof row.applied_at !== "string" || row.applied_at.length === 0)) {
        throw new Error(`applied persistence operation ${operationId} is missing applied_at`);
    }
    return {
        operationId,
        table,
...
function assertExpectedPersistenceTable(operation, expectedTable) {
    const normalizedTable = normalizePersistenceTable(expectedTable, "expected persistence table");
    if (operation.table !== normalizedTable) {
        throw new Error(`persistence operation ${operation.operationId} targets ${operation.table}, not ${normalizedTable}`);
    }
...
function enqueuePersistenceOperation(table, upserts = [], deletes = []) {
    ...
    const result = requireDb()
        .prepare(`INSERT INTO _persistence_outbox(
         table_name, upserts_json, deletes_json, state, created_at, applied_at
       ) VALUES (?, ?, ?, 'pending', ?, NULL)`)
        .run(normalizedTable, JSON.stringify(normalizedUpserts), JSON.stringify(normalizedDeletes), createdAt);
...
function applyPersistenceOperation(operationId, expectedTable) {
    const normalizedId = normalizePersistenceOperationId(operationId);
    const normalizedExpectedTable = expectedTable === undefined
        ? undefined
...
function acknowledgePersistenceOperation(operationId, expectedTable) {
    ...
    const txn = requireDb().transaction(() => {
        ...
        if (operation.state !== "applied") {
            throw new Error(`persistence operation ${normalizedId} cannot be acknowledged from state ${operation.state}`);
        }
        const removed = requireDb()
            .prepare(`DELETE FROM _persistence_outbox
         WHERE operation_id = ? AND table_name = ? AND state = 'applied'`)
            .run(operation.operationId, operation.table);
        if (removed.changes !== 1) {
            throw new Error(`failed to acknowledge persistence operation ${operation.operationId}`);
        }
        return operation;
    });
    return txn.immediate();
...
function reconcilePersistenceOperation(operationId, expectedTable) {
    ...
    const txn = requireDb().transaction(() => {
        ...
        applyChangeStatements(operation.table, operation.upserts, operation.deletes);
        const removed = requireDb()
            .prepare("DELETE FROM _persistence_outbox WHERE operation_id = ? AND table_name = ?")
            .run(operation.operationId, operation.table);
        if (removed.changes !== 1) {
            throw new Error(`failed to reconcile persistence operation ${operation.operationId}`);
        }
        return {
            ...operation,
            state: "applied",
    ...
    const txn = requireDb().transaction(() => {
        ...
        for (const operation of operations) {
            ...
            const removed = requireDb()
                ...
                .run(operation.operationId, operation.table);
            if (removed.changes !== 1) {
                throw new Error(`failed to recover persistence operation ${operation.operationId}`);
            }
            recovered.push({
                ...operation,
                state: "applied",
...
function applyChanges(table, upserts = [], deletes = []) {
    ...
    ensureSqlTable(table);
    const txn = requireDb().transaction(() => applyChangeStatements(table, upserts, deletes));
    return txn();
}
/**
