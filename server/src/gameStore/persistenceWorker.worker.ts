import type { PersistenceOperation, PersistenceWriteMessage, PersistenceWorkerRequest, PersistenceWorkerData } from "./persistenceTypes";
...
const { parentPort, workerData } = require("worker_threads") as {
  parentPort: import("node:worker_threads").MessagePort;
  workerData: PersistenceWorkerData;
};
...
const blockedByTable = new Map<string, number>();
...
function assertExactMessage(operation: PersistenceOperation, message: PersistenceWriteMessage): void {
...
function reportWriteError(message: PersistenceWriteMessage, error): void {
...
function applyWrite(message: PersistenceWriteMessage): PersistenceOperation | null {
  if (
...
parentPort.on("message", (msg: PersistenceWorkerRequest) => {
