...
function main() {
    ...
    if (shared.toPositiveInt(bundle.bundleVersion, 0) !== shared.BUNDLE_VERSION) {
        console.error(`Bundle version ${bundle.bundleVersion} != expected ${shared.BUNDLE_VERSION}`);
        process.exitCode = 1;
        return;
    }
...
function importAccount(acctEntry, ctx) {
    ...
    if (opts.dryRun) {
        console.log(`  [dry-run] account "${username}" + ${acctEntry.characters.length} char(s)`);
...
function importCharacter(charEntry, newAccountID, ctx) {
    ...
    if (Array.isArray(rec.storedShips)) {
        for (const s of rec.storedShips) {
            if (s && s.itemID != null)
                s.itemID = remapItemRef(shared.toPositiveInt(s.itemID, 0));
        }
    }
    ...
    for (const item of charEntry.items || []) {
        ...
        if (item.shipID != null)
            item.shipID = remapItemRef(shared.toPositiveInt(item.shipID, 0));
        itemsObj[newID] = item;
        summary.items += 1;
    }
    ...
    for (const { table: tname, group } of shared.SIMPLE_CHAR_TABLES) {
        ...
        for (const f of shared.CHARID_FIELDS) {
            if (shared.toPositiveInt(value[f], 0) === oldCharID)
                value[f] = newCharID;
        }
    ...
    if (charEntry.mail) {
        ...
        if (!mailObj.messages || typeof mailObj.messages !== "object")
            mailObj.messages = {};
        if (!mailObj.mailboxes || typeof mailObj.mailboxes !== "object")
            mailObj.mailboxes = {};
    ...
    if (charEntry.notifications && charEntry.notifications.box) {
        ...
        if (!notifObj.boxes || typeof notifObj.boxes !== "object")
            notifObj.boxes = {};
    ...
    console.log(`      char "${charEntry.characterName}" ${oldCharID} -> ${newCharID} ` +
...
main();
//# sourceMappingURL=importPlayers.js.map