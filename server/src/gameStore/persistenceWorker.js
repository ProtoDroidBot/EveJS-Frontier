...
function normalizeOperationId(operationId) {
    if (!Number.isSafeInteger(operationId) || operationId <= 0) {
        const error = new TypeError("persistence operationId must be a positive safe integer");
...
function copyOperation(operation) {
    ...
    return {
        ...
        createdAt: operation.createdAt === undefined ? null : operation.createdAt,
        appliedAt: operation.appliedAt === undefined ? null : operation.appliedAt,
    };
}
...
function createController(options = {}) {
    ...
    function requireJournalMethod(name) {
        ...
        if (typeof method !== "function") {
            const error = new Error(`persistence journal does not implement ${name}()`);
    ...
    function getJournalOperation(dbPath, operationId) {
        ...
        const operation = requireJournalMethod("getPersistenceOperation")(normalizeOperationId(operationId));
    ...
    function invokeErrorCallback(targetState, table, message, failure) {
        ...
        try {
            ...
            rememberCallbackError(targetState, error, "persistence worker error callback failed");
        }
    }
    function invokeAcknowledgedCallback(targetState, operation) {
        ...
        try {
            ...
            const message = rememberCallbackError(targetState, error, "persistence acknowledgment callback failed");
    ...
    function invokeRecoveredCallback(targetState, operations) {
        ...
        try {
            ...
            const message = rememberCallbackError(targetState, error, "persistence recovery callback failed");
    ...
    function recordFailure(targetState, operation, rawError, source, notify = true) {
        const message = errorMessage(rawError, "persistence worker write failed without an error message");
    ...
    function failPendingWrites(targetState, rawError, source, notify = true) {
        ...
        for (const operationId of [...targetState.pendingOperationIds]) {
            ...
            if (operation) {
                failures.push(recordFailure(targetState, operation, rawError, source, notify));
            }
        }
        return failures;
    }
    function notifyWorkerReconciled(targetState, operation) {
        ...
        try {
            ...
            const message = "persistence worker reconciliation notice could not be posted: " +
    ...
    function finishAcknowledgedOperation(operation, acknowledgedOperation) {
        const completed = copyOperation(acknowledgedOperation || operation.acknowledgedOperation || operation);
    ...
    function acknowledgeAppliedOperation(targetState, operation) {
        const stored = getJournalOperation(targetState.dbPath, operation.operationId);
        if (!stored) {
            if (operation.acknowledgedOperation) {
                return finishAcknowledgedOperation(operation, operation.acknowledgedOperation);
            }
            const error = new Error(`persistence operation ${operation.operationId} disappeared before acknowledgment`);
            error.code = "PERSISTENCE_OPERATION_NOT_FOUND";
            throw error;
        }
        if (stored.table !== operation.table) {
            const error = new Error(`persistence operation ${operation.operationId} table mismatch`);
            error.code = "PERSISTENCE_OPERATION_MISMATCH";
            throw error;
        }
        if (stored.state !== "applied") {
            const error = new Error(`persistence operation ${operation.operationId} was acknowledged before durable apply`);
            error.code = "PERSISTENCE_OPERATION_NOT_APPLIED";
            throw error;
        }
        initializeJournal(targetState.dbPath);
        const acknowledged = copyOperation(requireJournalMethod("acknowledgePersistenceOperation")(operation.operationId, operation.table));
        return finishAcknowledgedOperation(operation, acknowledged);
    }
    function consumeQueuedFailureSignal(targetState) {
        while (true) {
            const count = Atomics.load(targetState.counters, FAILED_WRITE_INDEX);
            if (count <= 0) {
                return;
            }
            if (Atomics.compareExchange(targetState.counters, FAILED_WRITE_INDEX, count, count - 1) === count) {
                return;
            }
        }
    }
    function handleWorkerMessage(targetState, message) {
        ...
        if (message.type === "write-complete") {
            ...
            if (operation) {
                try {
                    ...
                    recordFailure(targetState, operation, "persistence worker acknowledgment failed: " +
                        errorMessage(error, "unknown journal acknowledgment error"), "acknowledgment-error", true);
                }
            }
            return;
        }
        if (message.type === "error") {
            ...
            const operation = findPendingOperation(targetState, message);
            const messageText = errorMessage(message.error, "persistence worker write failed without an error message");
            if (operation) {
                recordFailure(targetState, operation, messageText, "write-error", true);
            }
            else {
        ...
        if (message.type === "closed") {
            ...
            if (message.error !== undefined && message.error !== null) {
                targetState.closeError = errorMessage(message.error, "persistence worker close failed without an error message");
            }
        }
    }
    function attachWorkerListeners(targetState) {
        ...
        current.on("messageerror", (error) => {
            const message = errorMessage(error, "persistence worker message could not be deserialized");
            targetState.messageError = targetState.messageError || message;
            failPendingWrites(targetState, message, "message-error", false);
            invokeErrorCallback(targetState, null, message, null);
        });
        current.on("error", (error) => {
            const message = "persistence worker crashed: " +
        ...
        current.on("exit", (code) => {
            ...
            if (targetState.pendingOperationIds.size > 0) {
                failPendingWrites(targetState, "persistence worker exited before reporting write completion", "worker-exit", true);
            }
            if (code !== 0 &&
                !targetState.workerError &&
                !targetState.terminationRequested) {
                invokeErrorCallback(targetState, null, "persistence worker exited with code " + code, null);
            }
        });
    }
    function ensureWorker(dbPath) {
        if (state && !state.exited) {
            ...
            if (state.dbPath !== dbPath) {
                const error = new Error("persistence worker cannot switch databases while active");
                error.code = "PERSISTENCE_WORKER_DATABASE_MISMATCH";
                throw error;
            }
            return state;
        }
        const sharedBuffer = new SharedArrayBuffer(Int32Array.BYTES_PER_ELEMENT * SHARED_COUNTER_SLOTS);
    ...
    function registerPendingOperation(operation, dbPath) {
        ...
        if (existingById) {
            const error = new Error(`persistence operation ${durable.operationId} is already unresolved`);
        ...
        if (existingOperationId !== undefined) {
            const error = new Error(`persistence table ${durable.table} already has unresolved operation ${existingOperationId}`);
    ...
    function assertControllerDatabase(dbPath) {
        if (state && !state.exited && state.dbPath !== dbPath) {
            const error = new Error("persistence controller cannot switch databases while active");
            error.code = "PERSISTENCE_WORKER_DATABASE_MISMATCH";
            throw error;
        }
        for (const operation of pendingWrites.values()) {
            if (operation.dbPath !== dbPath) {
                const error = new Error(`unresolved persistence operation ${operation.operationId} belongs to a different database`);
                error.code = "PERSISTENCE_WORKER_DATABASE_MISMATCH";
                throw error;
            }
        }
        for (const recoveryPath of pendingRecoveryCallbacks.keys()) {
            if (recoveryPath !== dbPath) {
                const error = new Error("unreported persistence recovery belongs to a different database");
                error.code = "PERSISTENCE_WORKER_DATABASE_MISMATCH";
                throw error;
            }
        }
    ...
    function recover(dbPath) {
        ...
        for (const operation of [
            ...
            if (existing &&
                !operationMatchesBatch(existing, operation.table, operation.upserts, operation.deletes)) {
                const error = new Error(`conflicting recovered persistence operation ${operation.operationId}`);
                error.code = "PERSISTENCE_OPERATION_MISMATCH";
                throw error;
            }
            recoveredById.set(operation.operationId, operation);
        }
        const recovered = [...recoveredById.values()].sort((left, right) => left.operationId - right.operationId);
    ...
    function ensureRecovered(dbPath) {
        if (recoveredDbPaths.has(dbPath) &&
    ...
    function reconcileWrite(dbPath, operationId) {
        ...
        if (local && local.dbPath !== dbPath) {
            const error = new Error(`persistence operation ${id} belongs to a different database`);
            error.code = "PERSISTENCE_WORKER_DATABASE_MISMATCH";
            throw error;
        }
        ...
        if (!stored) {
            ...
            if (local.acknowledgedOperation) {
                notifyWorkerReconciled(local.workerState, local.acknowledgedOperation);
                return finishAcknowledgedOperation(local, local.acknowledgedOperation);
            }
            const error = new Error(`persistence operation ${id} disappeared before reconciliation`);
            error.code = "PERSISTENCE_OPERATION_NOT_FOUND";
            throw error;
        }
        const expectedTable = local ? local.table : stored.table;
        const reconciledValue = requireJournalMethod("reconcilePersistenceOperation")(id, expectedTable);
        if (!reconciledValue) {
            const error = new Error(`persistence operation ${id} disappeared during reconciliation`);
            error.code = "PERSISTENCE_OPERATION_NOT_FOUND";
            throw error;
        }
    ...
    function drain(timeoutMs = defaultDrainTimeoutMs, targetState = state) {
        ...
        while (true) {
            const pending = Atomics.load(targetState.counters, PENDING_WRITE_INDEX);
            if (pending <= 0) {
                return makeDrainResult(targetState);
            }
            if (targetState.exited) {
                return makeDrainResult(targetState, {
                    error: "persistence worker exited with " +
                        pending +
                        " write" +
                        (pending === 1 ? "" : "s") +
                        " still pending",
            ...
            if (remaining <= 0) {
                return makeDrainResult(targetState, {
                    timedOut: true,
                    error: "persistence worker drain timed out with " +
                        pending +
                        " write" +
                        (pending === 1 ? "" : "s") +
                        " still pending",
            ...
            Atomics.wait(targetState.counters, PENDING_WRITE_INDEX, pending, remaining);
        }
    }
    /**
    ...
    function submitWrite(dbPath, table, upserts, deletes, submitOptions = {}) {
        ...
        if (pendingByTable.has(normalizedTable)) {
            ...
            const error = new Error(`persistence table ${normalizedTable} already has unresolved operation ${existingOperationId}`);
            error.code = "PERSISTENCE_TABLE_WRITE_PENDING";
            error.operationId = existingOperationId;
        ...
        if (suppliedOperationId === undefined || suppliedOperationId === null) {
            durableOperation = copyOperation(requireJournalMethod("enqueuePersistenceOperation")(normalizedTable, normalizedUpserts, normalizedDeletes));
        }
        else {
            const operationId = normalizeOperationId(suppliedOperationId);
            durableOperation = getJournalOperation(dbPath, operationId);
            if (!durableOperation) {
                const error = new Error(`pre-journaled persistence operation ${operationId} does not exist`);
                error.code = "PERSISTENCE_OPERATION_NOT_FOUND";
                throw error;
            }
            recoveredDbPaths.add(dbPath);
        }
        if (!operationMatchesBatch(durableOperation, normalizedTable, normalizedUpserts, normalizedDeletes)) {
            const error = new Error(`persistence operation ${durableOperation.operationId} does not match the submitted batch`);
            error.code = "PERSISTENCE_OPERATION_MISMATCH";
            throw error;
        }
        ...
        try {
            ...
            const message = "persistence worker could not be started: " +
        ...
        try {
            ...
            const message = "persistence worker write could not be posted: " +
                errorMessage(error, "unknown postMessage error");
        ...
        if (submitOptions.sync === true) {
            ...
            if (!drainResult.drained || drainResult.timedOut) {
                const drainError = new Error(drainResult.error || "persistence worker drain failed");
            ...
            if (pendingWrites.has(operation.operationId)) {
                ...
                try {
                    ...
                    if (!exactState) {
                        if (!operation.acknowledgedOperation) {
                            const missingError = new Error(`persistence operation ${operation.operationId} disappeared before synchronous acknowledgment`);
                            missingError.code = "PERSISTENCE_OPERATION_NOT_FOUND";
                            throw missingError;
                        }
                        finishAcknowledgedOperation(operation, operation.acknowledgedOperation);
                    }
                        ...
                        consumeQueuedFailureSignal(targetState);
                        const failure = recordFailure(targetState, operation, "persistence worker drained but the exact journal operation remains pending", "sync-write-not-applied", true);
                    ...
                    const message = "persistence worker synchronous finalization failed: " +
                        errorMessage(error, "unknown journal finalization error");
                    recordFailure(targetState, operation, message, "sync-finalization-error", true);
    ...
    function requestClose(targetState, timeoutMs) {
        return new Promise((resolve) => {
            ...
            try {
                ...
                const message = "persistence worker close could not be posted: " +
                    errorMessage(error, "unknown postMessage error");
    ...
    function buildShutdownResult(targetState, drainResult, close, termination) {
        const writeErrors = getPendingFailures();
        const errors = [];
    ...
    async function performShutdown(targetState) {
        ...
        try {
            ...
            drainResult = makeDrainResult(targetState, {
                error: "persistence worker drain failed: " +
        ...
        if (!targetState.exited) {
            try {
                closeWait = await requestClose(targetState, defaultCloseTimeoutMs);
            }
            catch (error) {
                closeWait = {
                    kind: "post-error",
                    postError: "persistence worker close wait failed: " +
        ...
        const close = {
            ...
            lostAcknowledgment: closeWait.kind === "exit" && !targetState.closeAcknowledged,
            error: null,
        };
        ...
        if (targetState.pendingOperationIds.size > 0) {
            let pendingError = "persistence worker stopped before reporting write completion";
        ...
        return buildShutdownResult(targetState, drainResult, close, termination);
    }
    function shutdown() {
        if (!state) {
...
//# sourceMappingURL=persistenceWorker.js.map