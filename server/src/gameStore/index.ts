...
// ── Cache state ─────────────────────────────────────────────────────
const cache: Record<string, any> = {};            // table name → parsed JS object
const dirty = new Set();     // tables that need flushing
const flushTimers: Record<string, any> = {};      // table name → pending setTimeout id
const transientPaths: Record<string, any> = {};   // table name → Set of cache paths excluded from disk flush
const flushBaselines: Record<string, any> = {};   // sqlite table → Map(key → last-persisted JSON string)
...
const dirtyRowKeys: Record<string, any> = {};     // sqlite table → Set(rowKey) touched since last flush
const fullDirty = new Set(); // sqlite tables that must use the full diff next flush
const flushStats: Record<string, any> = { partial: 0, full: 0 }; // observability for tests/diagnostics
// ────────────────────────────────────────────────────────────────────
...
function advanceFlushBaseline(table, upserts: any[] = [], deletes: any[] = []) {
...
function flushSqliteTable(table, options: Record<string, any> = {}) {
  ensureSqliteReady();
  if (
...
function flushTableSync(table, options: Record<string, any> = {}) {
...
function flushTablesSync(tables: any[] = []) {
  const uniqueTables: any[] = [...new Set(
...
function flushAllSync() {
  const dirtyTables: any[] = [...new Set([...dirty, ...inFlightFlushes.keys()])];
  const results: any[] = [];
...
process.on("exit", () => {
  ...
  if (
    (!shutdownInProgress && shutdownHooks.size > 0) ||
    dirty.size > 0 ||
    inFlightFlushes.size > 0
  ) {
...
function write(table, pth, data, options: Record<string, any> = {}) {
  if (!ensureCached(table)) {
    log.warn(`[DATABASE] database table: '${table}' not found!`);
    return { success: false, errorMsg: "TABLE_NOT_FOUND" };
  }
