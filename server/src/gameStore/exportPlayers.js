...
function resolveSqlitePath(opts) {
    ...
    const dataDir = opts.sourceDataDir ||
...
function main() {
    ...
    const excluded = new Set((opts.exclude || shared.DEFAULT_EXCLUDED_USERNAMES).map((s) => s.toLowerCase()));
    ...
    for (const { key: username, value: account } of accounts) {
        if (includeSet) {
            if (!includeSet.has(username.toLowerCase()))
                continue;
        }
        ...
        for (const { characterID, record } of chars) {
            accountEntry.characters.push(exportCharacter(characterID, record, {
                playerCorpIDs: null,
        ...
        console.log(`  + account "${username}" (id ${accountId}) — ${chars.length} character(s)`);
    }
    db.close();
    const outPath = opts.out
        ? path.resolve(opts.out)
        : path.join(REPO_ROOT, "_local", "export", `players-${new Date().toISOString().replace(/[:.]/g, "-")}.json`);
...
function collectMail(charID, rowByKey, allRows) {
    ...
    for (const { key, value } of allRows("mail")) {
        ...
        if (group !== "messages" || !value)
            continue;
        const isRecipient = Array.isArray(value.toCharacterIDs) &&
        ...
        record.employmentHistory = [
            {
                ...
                startDate: (Array.isArray(record.employmentHistory) &&
...
function stripStandingsForCorp(record, corpID) {
    ...
    for (const bucket of ["char", "corp", "npc"]) {
        if (!Array.isArray(sd[bucket]))
            continue;
        sd[bucket] = sd[bucket].filter((entry) => shared.toPositiveInt(entry && entry.fromID, 0) !== corpID &&
            shared.toPositiveInt(entry && entry.toID, 0) !== corpID);
    }
}
main();
//# sourceMappingURL=exportPlayers.js.map