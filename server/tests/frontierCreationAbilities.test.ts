...
function findValidInteriorPlacement(state, template, itemID, typeID) {
  ...
  for (const [rawPartID, partReference] of Object.entries<any>(template.parts || {})) {
    const partID = Number(rawPartID);
    const part = getCreationPart(partReference.graphic_id);
    for (const rotationZ of [0, 90, 180, 270]) {
      const offsets = occupancy.map((cell) =>
        rotateCreationCellOffset(Number(cell.x), Number(cell.y), 0, rotationZ));
      ...
      for (const anchor of anchors) {
        ...
        const candidate = stateWithInteriorModule(
          state,
          itemID,
          typeID,
          placement,
...
function buildIffEffectRuntime(shipID) {
  ...
  return {
    ...
    runtime: {
      ...
      activateGenericModule(_session, moduleItem, effectName, options: Record<string, any> = {}) {
        const effectID = effectName === "iffBeacon" ? 12974 : 12972;
        const durationMs = effectName === "iffBeacon" ? 30000 : 5000;
        const effectState: Record<string, any> = {
      ...
      deactivateGenericModule(_session, moduleItemID, options: Record<string, any> = {}) {
      ...
      stopShipEntity(_entity, options: Record<string, any> = {}) {
...
test("Creation draft validation rejects invalid SDE geometry before inventory mutation", () => {
  ...
  const stateBefore = creationRuntime.readCreationState(
    itemStore.findItemById(ship.itemID),
  );
  ...
  assert.deepEqual(
    creationRuntime.readCreationState(itemStore.findItemById(ship.itemID)),
    stateBefore,
  );
});
...
test("Creation fitting splits one singleton and Dogma bridges online state", () => {
  ...
  const template = getCreationTemplate(ship.typeID);
  const beaconPlacement = findValidInteriorPlacement(
    initiallyEnsured.data.state,
    template,
    sourceStack.itemID,
    TYPE_BEACON,
  );
  ...
  assert.deepEqual(
    creationRuntime.readCreationState(persistedAfterUndock),
    stateBeforeUndock,
  );
  assert.equal(
    creationRuntime.isCreationModuleOnline(itemStore.findItemById(sourceStack.itemID)),
  ...
  const rehydrated = creationRuntime.ensureCreationState(
    persistedAfterUndock,
    OWNER_ID,
  );
...
test("Creation live entities enforce their derived capacitor and retain active modules", (t) => {
  ...
  const template = getCreationTemplate(ship.typeID);
  const transponderPlacement = findValidInteriorPlacement(
    ensured.data.state,
    template,
    sourceModule.itemID,
    TYPE_TRANSPONDER,
  );
  const stateWithTransponder = stateWithInteriorModule(
    ensured.data.state,
    sourceModule.itemID,
    TYPE_TRANSPONDER,
    transponderPlacement,
  );
  const blackstartPlacement = findValidInteriorPlacement(
    stateWithTransponder,
    template,
    sourceBlackstart.itemID,
    TYPE_BLACKSTART_CELL,
  );
  ...
  const startPosition: Record<string, any> = { ...entity.position };
  entity.mode = "GOTO";
...
test("GUI installation primes a Transponder for immediate activation", (t) => {
  ...
  const moduleGrant = itemStore.grantItemToCharacterLocation(
    ...
    TYPE_TRANSPONDER,
    1,
    { individualItems: true, singleton: 1, moduleState: { online: false } },
  );
  assert.equal(moduleGrant.success, true, moduleGrant.errorMsg);
  const moduleItem = moduleGrant.data.items[0];
  ...
  for (const [attributeID, valueAndTime] of primeFields.attributes.entries) {
    ...
    assert.deepEqual(
      valueAndTime,
      [valueAndTime[0], session._space.simFileTime],
      "Frontier Godma must receive a value/time pair for every primed attribute",
    );
...
test("transponder activation broadcasts explicitly and deactivation preserves its mode", () => {
  ...
  const invoke = (ability, kwargs: Record<string, any> = {}) =>
...
function buildViewer(overrides: Record<string, any> = {}) {
  return {
...
test("occluded scan contacts expose gravimetric and EM signatures only", () => {
  ...
  assert.deepEqual(
    scanningRuntime.getPresentedSignatureResults(signatures, false),
    signatures.slice(0, 2),
  );
  assert.deepEqual(
    scanningRuntime.getPresentedSignatureResults(signatures, true),
    signatures,
  );
});
...
test("scan contact resolution is delayed, stable across rescans, and session scoped", () => {
  ...
  const first = scanningRuntime.replaceResolvedScanningContacts(
    session,
    [4101],
    { nowMs: 1000, delayMs: 6000 },
  );
  ...
  const repeated = scanningRuntime.replaceResolvedScanningContacts(
    session,
    [4101],
    { nowMs: 3000, delayMs: 6000 },
  );
  assert.equal(repeated.delayMsByEntityID.get(4101), 4000);
  assert.equal(scanningRuntime.isScanningContactResolved(session, 4101, 7000), true);

  const removed = scanningRuntime.replaceResolvedScanningContacts(
    session,
    [],
    { nowMs: 8000, delayMs: 6000 },
  );
  ...
  scanningRuntime.replaceResolvedScanningContacts(
    session,
    [4102],
    { nowMs: 9000, delayMs: 0 },
  );
  session._space = {};
...
test("combat-resolved contacts are immediate and survive unrelated scans", () => {
  const session: Record<string, any> = { _space: {} };
  const contact = scanningRuntime.forceResolveScanningContact(
    session,
    4110,
    { nowMs: 1_000 },
  );
  ...
  assert.equal(
    scanningRuntime.isCombatResolvedScanningContact(session, 4110, 1_000),
    true,
  );

  const unrelatedScan = scanningRuntime.replaceResolvedScanningContacts(
    session,
    [4111],
    { nowMs: 2_000, delayMs: 6_000 },
  );
...
test("resolved contacts use the Frontier distance reveal curve per ball", () => {
  ...
  assert.equal(
    Math.round(scanningRuntime.calculateScanRevealDelayMs(50_000_000, 6000)),
    4731,
  );
  assert.equal(
    scanningRuntime.calculateScanRevealDelayMs(
  ...
  assert.equal(
    scanningRuntime.calculateScanRevealDelayMs(
      scanningRuntime.MAXIMUM_SCAN_DISTANCE_METERS + 1,
      6000,
    ),
    6000,
  );
  ...
  const resolution = scanningRuntime.replaceResolvedScanningContacts(
    session,
    [4201, 4202],
    { nowMs: 10_000, delayMs: 6000, delayMsByEntityID: delays },
  );
...
function runScan(candidates, overrides: Record<string, any> = {}) {
...
test("scanner cone includes in-cone and excludes out-of-cone targets", () => {
  ...
  const justInside: Record<string, any> = {
    itemID: 4004,
    typeID: TYPE_SIGNATURE_TARGET,
    position: {
...
test("scanner enforces the client-authored maximum range", () => {
  const withinRange: Record<string, any> = {
    itemID: 5001,
    typeID: TYPE_SIGNATURE_TARGET,
    position: { x: 0, y: 0, z: scanningRuntime.MAXIMUM_SCAN_DISTANCE_METERS - 1000 },
  };
  const beyondRange: Record<string, any> = {
    itemID: 5002,
    typeID: TYPE_SIGNATURE_TARGET,
    position: { x: 0, y: 0, z: scanningRuntime.MAXIMUM_SCAN_DISTANCE_METERS + 1000 },
  };
  const zeroDistance: Record<string, any> = { itemID: 5003, typeID: TYPE_SIGNATURE_TARGET, position: { x: 0, y: 0, z: 0 } };
...
test("contacts outside resolution range stay unresolved signatures", () => {
  ...
  assert.ok(scan.updatedScans[0].signature_results.every(
    ([, signature, noise]) =>
...
test("scan ids are stable and deltas report added/removed", () => {
  const target: Record<string, any> = { itemID: 6001, typeID: TYPE_SIGNATURE_TARGET, position: { x: 0, y: 0, z: 500000 } };
  const other: Record<string, any> = { itemID: 6002, typeID: TYPE_SIGNATURE_TARGET, position: { x: 0, y: 0, z: 600000 } };
...
test("a resolved ball replaces its prior unresolved signature with client fate", () => {
  ...
  assert.deepEqual(
    resolved.removedReasonsByScanId.get(6101),
    [scanningRuntime.UPDATE_RESOLVED_TO_ITEM, 6101],
  );
  assert.equal(
    Math.round(resolved.resolvedDelayMsById.get(6101)),
    4731,
  );
...
test("scan response uses KeyVal, timedelta duration, and resolved filetime deltas", () => {
  ...
  const entries = new Map<string, any>(response.args.entries);
...
test("module and weapon activity raise EM signature and decay to baseline", () => {
  ...
  assert.equal(
    scanningRuntime.resolveEntityEmSignatureMultiplier(entity, 1000),
    2,
  );
  assert.equal(
    scanningRuntime.resolveEntityEmSignatureMultiplier(entity, 6000),
    1.75,
  );
  ...
  assert.equal(
    scanningRuntime.resolveEntityEmSignatureMultiplier(entity, 21000),
    1,
  );
