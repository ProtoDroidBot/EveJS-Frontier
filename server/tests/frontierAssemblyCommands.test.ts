...
function buildDependencies(overrides: Record<string, any> = {}) {
  const calls: any[] = [];
...
test("Frontier Smart Assembly catalog includes published and hidden end types only", () => {
  ...
  const byID = new Map<number, { name: string; published: boolean }>(definitions.map((entry) => [entry.assemblyTypeID, entry]));
