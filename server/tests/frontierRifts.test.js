...
const { classifyCrudeMatterResource, createFrontierRiftAuthority, } = require("../src/space/frontierRiftAuthority");
const { RIFT_RESPAWN_DELAY_MS, RIFT_SITE_ID_BASE, createFrontierRiftSiteStore, } = require("../src/space/frontierRiftSites");
const { buildRiftEnvironmentPlan, buildRiftSiteEntity, handleResourceDepleted, materializeRiftSite, } = require("../src/space/frontierRiftSceneService");
const { DEFAULT_RIFT_SPAWN_DISTANCE_METERS, executeFrontierRiftCommand, } = require("../src/services/chat/frontierRiftCommands");
const { buildCrudeRiftInfo, } = require("../src/services/frontier/systemInfoService");
...
test("Frontier Rift authority preserves the authored room and initial trigger state", () => {
    ...
    assert.deepEqual(template.sceneObjects.map((object) => object.objectID), [1468972, 1466745]);
...
test("Crude Matter formations determine yield tier and mineable quantity", () => {
    ...
    assert.deepEqual(["Rupture", "Fissure", "Fault"].map((formation) => {
...
test("depleted Rift sites own a persisted 24-hour respawn lifecycle", () => {
    ...
    assert.deepEqual(store.reactivateDueSites(30021998, depleted.data.respawnAtMs - 1).data.sites, []);
...
test("mining the final Crude Matter prop depletes the Rift site, not the prop lifecycle", () => {
    ...
    const root = buildRiftSiteEntity(site);
    const resource = {
    ...
    const scene = {
        ...
        removeStaticEntity(entityID) {
            ...
            if (!entity)
                return { success: false, errorMsg: "STATIC_ENTITY_NOT_FOUND" };
...
test("Rift scene materialization emits a CRDungeon root and exact active room props", () => {
    ...
    const scene = {
        ...
        broadcastAddBalls(entities) {
            assert.ok(entities.every((entity) => (entity.dungeonMaterializedSiteContent !== true &&
    ...
    assert.ok(content.every((entity) => (entity.dungeonMaterializedSiteContent === undefined &&
        entity.dungeonSiteInstanceID === undefined)));
    assert.deepEqual(buildRiftEnvironmentPlan(authority.getTemplateByDungeonID(14001))
        .map((entry) => entry.typeID), [92394, 88430]);
    const crData = normalizeCrDataDictionaryForProfile(buildSlimItemDict(buildRiftSiteEntity(site)), root, "frontier");
...
test("Rift command spawns the preferred template through the live runtime", () => {
    ...
    const result = executeFrontierRiftCommand({ characterID: 140000005 }, "spawn ahead=5000", { authority, siteStore, spaceRuntime });
    assert.equal(result.success, true);
...
test("Rift command defaults to a system-map-visible placement", () => {
    ...
    const result = executeFrontierRiftCommand({ characterID: 140000005 }, "spawn", { authority, siteStore, spaceRuntime });
    assert.equal(result.success, true);
...
//# sourceMappingURL=frontierRifts.test.js.map