...
const { GET_ENERGY_CONFIG_REQUEST, GET_ENERGY_CONFIG_RESPONSE, createAssemblyGateGatewayService, } = require("../src/_secondary/express/gatewayServices/assemblyGateGatewayService");
...
test("server accepts only a chain table that exactly matches the synchronized build manifest", () => {
    ...
    assert.doesNotThrow(() => energyConfig.assertAssemblyEnergyConfigMatches([{ typeID: 77917, energyRequired: 500 }], manifest, components));
    assert.throws(() => energyConfig.assertAssemblyEnergyConfigMatches([], manifest, components), /77917: expected 500, chain absent/);
    assert.throws(() => energyConfig.assertAssemblyEnergyConfigMatches([{ typeID: 77917, energyRequired: 500 }, { typeID: 88086, energyRequired: 1 }], manifest, components), /88086: expected 0, chain 1/);
    assert.throws(() => energyConfig.assertAssemblyEnergyConfigMatches([{ typeID: 77917, energyRequired: 500 }, { typeID: 84556, energyRequired: 10 }], manifest, components), /84556: not in manifest/);
    assert.throws(() => energyConfig.assertAssemblyEnergyConfigMatches([{ typeID: 77917, energyRequired: 500 }], manifest.slice(1), components), /missing 77917/);
    assert.throws(() => energyConfig.assertAssemblyEnergyConfigMatches([{ typeID: 77917, energyRequired: 500 }], undefined, components), /run FrontierWorld\.ps1 sync/);
});
...
test("assembly energy protobuf matches the build-3502403 descriptor", () => {
    ...
    const decoded = types.getEnergyConfigResponse.toObject(types.getEnergyConfigResponse.decode(encodedResponse), { longs: Number });
...
test("assembly gateway returns the loaded nonzero energy configuration", (t) => {
    ...
    const response = service.handleRequest(GET_ENERGY_CONFIG_REQUEST, requestEnvelope(types.getEnergyConfigRequest.encode({}).finish()));
    const decoded = types.getEnergyConfigResponse.toObject(types.getEnergyConfigResponse.decode(response.responsePayloadBuffer), { longs: Number, arrays: true });
...
test("assembly energy configuration requires an active character", () => {
    const service = createAssemblyGateGatewayService();
    const denied = service.handleRequest(GET_ENERGY_CONFIG_REQUEST, requestEnvelope(Buffer.alloc(0), 0));
    assert.equal(denied.statusCode, 403);
    assert.equal(denied.statusMessage, "ACCESS_DENIED");
    assert.equal(denied.responseTypeName, GET_ENERGY_CONFIG_RESPONSE);
});
//# sourceMappingURL=frontierAssemblyEnergyConfig.test.js.map