...
const { WEB_CALL_ALLOWLIST, } = require("../src/_secondary/express/evejsWebGatewayRuntime");
const ShipCosmeticsMgrService = require("../src/services/ship/shipCosmeticsMgrService");
const ShipSkinMgrService = require("../src/services/ship/shipSkinMgrService");
const { SHIP_SKIN_MGR_ALLOWED_METHODS, } = ShipSkinMgrService;
const { buildCorporationMemberMapRows, } = MapService._testing;
const { buildLicensedSkinKeyVal, resolveAppliedSkinLicenseRecord, resolveOwnedShipItem, } = ShipCosmeticsMgrService._testing;
...
test("private corporation-member locations are not exposed by the web-call bridge", () => {
    assert.equal(WEB_CALL_ALLOWLIST.some(({ service, method }) => service === "map" && method === "GetMyExtraMapInfo"), false);
});
...
test("shipSkinMgr resolves an applied skin only for its licensee and hull", () => {
    ...
    const options = {
        ...
        getEffectiveLicenseRecord: (licenseeID, skinID) => licenseeID === 140000005 && skinID === 1234 ? licenseRecord : null,
    };
    assert.equal(resolveAppliedSkinLicenseRecord(140000005, 9988400001895, 95276, options), licenseRecord);
    assert.equal(resolveAppliedSkinLicenseRecord(140000006, 9988400001895, 95276, options), null);
    assert.equal(resolveAppliedSkinLicenseRecord(140000005, 9988400001895, 95277, options), null);
...
test("ship SKIN mutation accepts only a ship owned by the active character", () => {
    ...
    assert.equal(resolveOwnedShipItem(9988400001895, 140000005, options), ownedShip);
    assert.equal(resolveOwnedShipItem(9988400001895, 140000006, options), null);
    assert.equal(resolveOwnedShipItem(9988400001895, 0, options), null);
});
//# sourceMappingURL=frontierMapSkinCompatibility.test.js.map