...
} = require("../src/services/mining/frontierSalvageResources");
const {
  classifyMiningMaterialType,
} = require("../src/services/mining/miningInventory");
const {
...
test("landscape spawn config composes disjoint NPC families into site-specific encounters", () => {
  ...
  assert.ok(pulverizedAsteroidCluster.npcs.some(
    (entry) => entry.familyKey === "mooneater_entities",
  ));
  assert.ok(pulverizedAsteroidCluster.npcs.some(
    (entry) => entry.familyKey === "feral_support",
  ));
...
test("Frontier salvageable wreckage resolves its authored salvage output", () => {
  ...
  const options = {
    getTypeAttributeValue: (typeID, name) => (
      name === "asteroidOutputTypeID" ? wreckageOutputs.get(typeID) || null : null
    ),
  ...
  for (const [wreckageTypeID, outputTypeID] of wreckageOutputs.entries()) {
    ...
    assert.equal(
      resolveFrontierSalvageResourceProfile(wreckageObject, options).resourceQuantity,
      profile.resourceQuantity,
    );
...
test("Frontier landscape plans reserve authored wreckage outside the scenery cap", () => {
  ...
  const classifyResourceObject = (object) => (
    [95349, 95350].includes(Number(object && object.typeID))
      ? {
  ...
  const plan = buildLandscapeScenePlan(
    { itemID: 900_000_001, dungeonID: 100 },
    {
      entryDungeonID: 100,
...
test("Frontier landscape materialization promotes salvageable wreckage into mineable resources", () => {
  ...
  const resources = scene.staticEntities.filter(
    (entity) => entity.kind === "landscapeSalvageResource",
  );
