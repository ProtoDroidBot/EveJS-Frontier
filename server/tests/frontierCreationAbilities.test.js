...
const { buildNpcTransponderShipsForSystem, buildVerdictsForViewer, handleCreationIffStateChange, } = require("../src/services/frontier/iffAbilityHandlers");
...
test("GUI installation primes a Transponder for immediate activation", (t) => {
    ...
    const onlineEffectIndex = notifications.findIndex((entry) => entry.name === "OnGodmaShipEffect" &&
    ...
    assert.ok(creationChangedIndex > onlineEffectIndex, "the action-bar snapshot is published after Dogma can activate the module");
    ...
    assert.equal(creationRuntime.isCreationModuleOnline(installedModule), true, JSON.stringify({
        installedModule,
