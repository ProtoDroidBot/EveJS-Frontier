...
const { crcHqx, selectHandshakeSignedFunc, selectPlaceboChallengeResponseHash, selectVersionExchangeRelease, usesNoOpHandshakeSignedFunc, } = require("../src/network/tcp/handshakeCompatibility");
const { framePayload, readPacketLength, usesBigEndianPacketLengths, } = require("../src/network/tcp/packetFraming");
const { marshalDecode, marshalDecodeExact, marshalEncode, } = require("../src/network/tcp/utils/marshal");
const { decodePacket } = require("../src/common/pyPacket");
const { encodeAddress } = require("../src/common/machoAddress");
const { buildCachedMethodCallResult, getCachableObjectResponse, } = require("../src/services/cache/objectCacheRuntime");
const { normalizeActivationStateForProfile, normalizeInfoAttributesForProfile, normalizePackedInstanceRowForProfile, } = require("../src/services/dogma/dogmaInfoCompatibility");
const { normalizeCombatTimersForProfile, } = require("../src/services/security/crimewatchCompatibility");
const { buildDestinyConfigurationSettings, defersUndockBallparkStateUntilBeyonceBind, } = require("../src/services/ship/destinyCompatibility");
const { createDestinyWarpUpdateBuilders, } = require("../src/space/destiny/simulation/warpBuilders");
const { createDestinyMovementSimulator, } = require("../src/space/destiny/simulation/movement");
const { clearTrackingState, } = require("../src/space/destiny/simulation/motionState");
const { createMovementManualFlightCommands, } = require("../src/space/destiny/commands/manualFlightCommands");
...
const { normalizeCrDataDictionaryForProfile, normalizeSlimItemObjectForProfile, usesCrDataBallMetadata, usesCrDataSetState, usesFrontierStateStreamPreamble, usesWireSlimItemObjects, } = require("../src/space/destiny/stream/statePayloadCompatibility");
const { buildSlimItemPresentationUpdates, buildStructureLifecyclePresentationUpdates, } = require("../src/space/destiny/presentation/specialFxPayloads");
const { encodeEntityBall, } = require("../src/space/destiny/stream/ballEncoding");
const { buildAddBalls2Payload, buildSetStatePayload, } = require("../src/space/destiny/stream/statePayloads");
...
const { buildEmptyCrudeRiftInfo, buildEmptySystemInfo, } = require("../src/services/frontier/systemInfoService");
const { buildItemsInSystemsRowset, } = require("../src/services/character/charMgrGlobalAssetsRowsets");
const { buildSettingsInfoCode, } = require("../src/services/character/settingsInfoCompatibility");
const { buildCreationDiagnostic, buildCreationSnapshot, } = require("../src/services/frontier/creationCompatibility");
const { buildCreationShipAttributeModifierEntries, CREATION_FITTING_FLAG_ID, buildCreationSeedPlan, buildSeededCreationState, filterCreationModuleInventoryItems, getCreationModuleAbilities, normalizeCreationState, } = require("../src/services/frontier/creationRuntime");
const { applyModifierGroups, } = require("../src/services/fitting/liveFittingState");
const DeploymentMgrService = require("../src/services/frontier/deploymentMgrService");
const { ASSEMBLY_STATUS_OFFLINE, ASSEMBLY_STATUS_ONLINE, ASSEMBLY_STATUS_UNDER_CONSTRUCTION, hydrateConstructionEntityFromInventoryItem, _testing: deploymentContractTesting, } = require("../src/services/frontier/deploymentRuntime");
const { BERTHING_PHASE_APPROACHING, BERTHING_PHASE_BERTHED, BERTHING_PHASE_DEPARTING, berth, getContractForSession, undockBerth, _testing: berthingContractTesting, } = require("../src/services/frontier/berthingRuntime");
...
const { buildEmptyMemories, } = ExperienceService;
const StatusEffectMgrService = require("../src/services/frontier/statusEffectMgrService");
const ShellManagerService = require("../src/services/frontier/shellManagerService");
...
const BerthingSvcService = require("../src/services/frontier/berthingSvcService");
const { GET_METADATA_REQUEST, GET_ALL_OWNED_REQUEST, createAssemblyGateGatewayService, } = require("../src/_secondary/express/gatewayServices/assemblyGateGatewayService");
const { getAssemblyGateProtoTypes, } = require("../src/_secondary/express/gatewayServices/assemblyGateProto");
const { MarketDaemonClient, } = require("../src/services/market/marketDaemonClient");
const { shouldIncludeAgentRecord, } = require("../src/services/agent/agentCompatibility");
const { resolveDefaultStartupSystemIDs, } = require("../src/space/startupPreloadCompatibility");
const { skillLevelRecords, } = require("../../tools/DatabaseCreator/database-creator");
const { encodeCharacterBrainEffectLists, } = require("../src/services/dogma/brain/characterBrainCompatibility");
const { buildCharacterSkillEntry, } = require("../src/services/skills/skillTransport");
const { executeChatCommand, } = require("../src/services/chat/chatCommands");
const { getActiveShipRecord, } = require("../src/services/character/characterState");
const { ITEM_FLAGS, createSpaceItemForCharacter, findItemById, getItemMetadata, listCharacterItems, listContainerItems, removeInventoryItem, updateInventoryItem, } = require("../src/services/inventory/itemStore");
...
test("Frontier system view returns an iterable empty cairn collection", () => {
    for (const service of [new CairnService(), new IffMapService()]) {
        ...
        assert.doesNotThrow(() => marshalEncode(cairns, { compatibilityProfile: "frontier" }));
    }
...
test("Frontier system view returns an iterable empty IFF beacon collection", () => {
    ...
    assert.doesNotThrow(() => marshalEncode(beacons, { compatibilityProfile: "frontier" }));
    ...
    assert.equal(service.Handle_set_transponder([0, "tribe", null], { charid: 140000005 }), false);
});
test("Frontier system resource startup returns complete empty dictionaries", () => {
    ...
    assert.doesNotThrow(() => marshalEncode(buildEmptySystemInfo(), {
...
test("Frontier creation templates become populated client fitting models", () => {
    ...
    const state = buildSeededCreationState(template, 9988400001895, plan, [
    ...
    assert.deepEqual(Object.fromEntries(fields.hardpoints.items[0].entries).attached_item_id, 9988400002101);
    assert.doesNotThrow(() => marshalEncode(snapshot, {
        compatibilityProfile: "frontier",
    }));
});
test("Frontier creation module abilities follow static online effects", () => {
    ...
    assert.deepEqual(getCreationModuleAbilities(95320, options), ["online", "offline"]);
...
test("Frontier startup services return indexable empty/default contracts", () => {
    ...
    assert.deepEqual(experience.Handle_get_memories_from_shell(), buildEmptyMemories());
    assert.doesNotThrow(() => marshalEncode(experience.Handle_get_memories_from_shell(), { compatibilityProfile: "frontier" }));
    const statusEffectMgr = new StatusEffectMgrService();
    assert.deepEqual(statusEffectMgr.Handle_get_grace_state(), [
        0.0,
        false,
    ]);
    const statusEffectConfig = statusEffectMgr.callMethod("get_effect_config", [], null, { type: "dict", entries: [["effect_key", "heat"]] });
    ...
    assert.equal(shellManager.callMethod("has_raiment", [], null, null), false);
...
test("Frontier ship activation evaluates heat state without a missing clock symbol", () => {
    ...
    const frontierActivation = service._buildActivationResponse(activeShip, {
        characterID: 0,
        compatibilityProfile: "frontier",
    });
    ...
    const legacyActivation = service._buildActivationResponse(activeShip, {
        characterID: 0,
    ...
    assert.deepEqual(heatState.entries.map(([attributeID]) => attributeID), [1176, 1177, 1175]);
...
test("Frontier shell manager provisions one client-visible active shell", () => {
    ...
    const beforeShellIDs = new Set(listCharacterItems(characterID)
        .filter((item) => Number(item.categoryID) ===
    ...
    try {
        ...
        const second = Object.fromEntries(service.Handle_get_active_shell_db_data([], session).entries);
        assert.equal(second.shellID, shellID);
        assert.equal(listCharacterItems(characterID).filter((item) => Number(item.categoryID) ===
            ShellManagerService._testing.SHELL_CATEGORY_ID).length, Math.max(1, beforeShellIDs.size));
        const characterInventory = new InvBrokerService()
            ._getCharacterContainerItems(session, null);
        assert.equal(characterInventory.some((item) => Number(item.itemID) === shellID), true);
        if (!beforeShellIDs.has(shellID)) {
            assert.equal(notifications.some(([eventName]) => eventName === "OnItemChange"), true);
        }
...
test("Frontier industry details and assembly metadata resolve persisted facilities", () => {
    ...
    const createResult = createSpaceItemForCharacter(characterID, solarSystemID, getItemMetadata(87120), {
    ...
    try {
        const details = new IndustryService().Handle_get_facility_details([facilityID], session);
        ...
        const requestPayload = Buffer.from(types.getMetadataRequest.encode(types.getMetadataRequest.create({
            assembly: { sequential: facilityID },
        })).finish());
        const gatewayResult = createAssemblyGateGatewayService().handleRequest(GET_METADATA_REQUEST, {
            authoritative_context: {
                active_character: { sequential: characterID },
            },
            payload: { value: requestPayload },
        });
        const metadataResponse = types.getMetadataResponse.decode(gatewayResult.responsePayloadBuffer);
...
test("Frontier deployment contract maps depot and direct-placement assemblies", () => {
    ...
    assert.deepEqual(deploymentContractTesting.normalizeWorldVector([-411, 0, -1957]), { x: -411, y: 0, z: -1957 });
    assert.deepEqual(deploymentContractTesting.normalizeRotationDegrees([Math.PI, 0, -Math.PI / 2]), [180, 0, -90]);
    assert.deepEqual(deploymentContractTesting.resolveDeploymentPosition([0, 0, 765.4717407226562], { x: 988976934.328545, y: 201556508.345823, z: -194298032.529605 }), {
        buildAnchor: "ship",
    ...
    const heavyGatePlacement = deploymentContractTesting.resolveDeploymentPosition([-14253, -74, -20121], { x: 988990784.3285517, y: 201556508.34582326, z: -194297267.0578641 }, {
        networkNodeAnchors: [{
    ...
    const heavyGateWithoutNetworkNode = deploymentContractTesting.resolveDeploymentPosition([-14253, -74, -20121], { x: 988990784.3285517, y: 201556508.34582326, z: -194297267.0578641 });
    ...
    assert.equal(deploymentContractTesting.isCompletedNetworkNodeBuildAnchorState({
    ...
    assert.equal(deploymentContractTesting.readConstructionState(item).assemblyTypeID, 90184);
...
test("Frontier construction hydration publishes assembly activation state", () => {
    ...
    assert.equal(Object.hasOwn(constructionEntity, "activate_comp_durationSeconds"), false);
...
test("Frontier berthing service exposes the complete client contract", () => {
    ...
    assert.equal(Object.fromEntries(service.Handle_begin_berth([contract.hostAssemblyID], session).entries).phase, BERTHING_PHASE_APPROACHING);
    assert.equal(Object.fromEntries(service.Handle_berth([contract.hostAssemblyID], session).entries).phase, BERTHING_PHASE_BERTHED);
    assert.equal(Object.fromEntries(service.Handle_complete_berth([contract.hostAssemblyID], session).entries).phase, BERTHING_PHASE_BERTHED);
...
test("Frontier chain assemblies commit a one-use signed state transition", () => {
    ...
    const createResult = createSpaceItemForCharacter(characterID, solarSystemID, nodeMetadata, {
        customInfo: deploymentContractTesting.writeConstructionState(null, {
            assemblyStatus: ASSEMBLY_STATUS_OFFLINE,
            assemblyTypeID: 88092,
            completeAtMs: 0,
    ...
    try {
        ...
        assert.match(transactionData.gasData.payment[0].objectId, /^0x[0-9a-f]{64}$/u);
        assert.equal(service.Handle_set_online_signature([itemID, transaction.transaction_uuid, "not-a-signature"], session), false);
        assert.equal(deploymentContractTesting.readConstructionState(findItemById(itemID))
            .assemblyStatus, ASSEMBLY_STATUS_OFFLINE);
        const signatureEnvelope = Buffer.alloc(97, 1).toString("base64");
        assert.equal(service.Handle_set_online_signature([itemID, transaction.transaction_uuid, signatureEnvelope], session), true);
        assert.equal(deploymentContractTesting.readConstructionState(findItemById(itemID))
            .assemblyStatus, ASSEMBLY_STATUS_ONLINE);
        assert.equal(service.Handle_set_online_signature([itemID, transaction.transaction_uuid, signatureEnvelope], session), false);
    }
    finally {
...
test("Frontier Heavy Gates stay owner-managed while allowing visitor traversal", () => {
    ...
    const createGate = (solarSystemID, x) => createSpaceItemForCharacter(gateOwnerID, solarSystemID, gateMetadata, {
        customInfo: deploymentContractTesting.writeConstructionState(null, {
            assemblyStatus: ASSEMBLY_STATUS_OFFLINE,
            assemblyTypeID: 84955,
            completeAtMs: 0,
    ...
    try {
        ...
        const preparedLink = Object.fromEntries(service.Handle_link_gates([sourceGateID, destinationGateID], ownerSession).entries);
        assert.equal(service.Handle_link_gates_signature([sourceGateID, preparedLink.transaction_uuid, signatureEnvelope], ownerSession), true);
        const sourceState = deploymentContractTesting.readConstructionState(findItemById(sourceGateID));
        const destinationState = deploymentContractTesting.readConstructionState(findItemById(destinationGateID));
        ...
        const gatewayResult = createAssemblyGateGatewayService().handleRequest(GET_ALL_OWNED_REQUEST, {
            authoritative_context: {
                active_character: { sequential: gateOwnerID },
            },
        });
        const gatewayResponse = getAssemblyGateProtoTypes().getAllOwnedResponse.decode(gatewayResult.responsePayloadBuffer);
        const sourceEntry = gatewayResponse.gates.find((entry) => Number(entry.id.sequential) === sourceGateID);
        assert.equal(gatewayResult.statusCode, 200);
        assert.equal(Number(sourceEntry.attributes.location.solar_system.sequential), sourceSystemID);
        assert.equal(Number(sourceEntry.attributes.destination.sequential), destinationGateID);
        const visitorGatewayResult = createAssemblyGateGatewayService().handleRequest(GET_ALL_OWNED_REQUEST, {
            authoritative_context: {
        ...
        assert.equal(visitorGatewayResponse.gates.some((entry) => (Number(entry.id.sequential) === sourceGateID ||
            Number(entry.id.sequential) === destinationGateID)), false);
        const preparedOnline = Object.fromEntries(service.Handle_set_online([sourceGateID], ownerSession).entries);
        assert.equal(service.Handle_set_online_signature([sourceGateID, preparedOnline.transaction_uuid, signatureEnvelope], ownerSession), true);
        assert.equal(deploymentContractTesting.readConstructionState(findItemById(sourceGateID))
            .assemblyStatus, ASSEMBLY_STATUS_ONLINE);
        assert.throws(() => service.Handle_gate_jump([sourceGateID], ownerSession), (error) => {
            assert.equal(error && error.name, "MachoWrappedException");
            assert.match(JSON.stringify(error && error.machoErrorResponse), /destination Heavy Gate must be online/u);
            return true;
        });
        ...
        const preparedDestinationOnline = Object.fromEntries(service.Handle_set_online([destinationGateID], destinationSession).entries);
        assert.equal(service.Handle_set_online_signature([
            destinationGateID,
            preparedDestinationOnline.transaction_uuid,
            signatureEnvelope,
        ], destinationSession), true);
        assert.throws(() => service.Handle_set_offline([sourceGateID], visitorSession), (error) => {
            assert.equal(error && error.name, "MachoWrappedException");
            assert.match(JSON.stringify(error && error.machoErrorResponse), /do not own this assembly/u);
            return true;
        });
        assert.throws(() => service.Handle_unlink_gate([sourceGateID], visitorSession), (error) => {
            assert.equal(error && error.name, "MachoWrappedException");
            assert.match(JSON.stringify(error && error.machoErrorResponse), /do not own this assembly/u);
            return true;
        });
        const preparedJump = Object.fromEntries(service.Handle_gate_jump([sourceGateID], visitorSession).entries);
        ...
        transitions.jumpSessionToSolarSystem = (jumpSession, solarSystemID, options) => {
            capturedJump = { jumpSession, options, solarSystemID };
            return { success: true, data: { solarSystemID } };
        };
        try {
            assert.throws(() => service.Handle_gate_jump_signature([sourceGateID, preparedJump.transaction_uuid, signatureEnvelope], { ...visitorSession, characterID: gateOwnerID }), (error) => {
                assert.equal(error && error.name, "MachoWrappedException");
                assert.match(JSON.stringify(error && error.machoErrorResponse), /transaction no longer matches/u);
                return true;
            });
            assert.equal(service.Handle_gate_jump_signature([sourceGateID, preparedJump.transaction_uuid, signatureEnvelope], visitorSession), true);
        }
        finally {
        ...
        assert.equal(capturedJump.options.spawnStateOverride.anchorID, destinationGateID);
        assert.equal(capturedJump.options.spawnStateOverride.anchorType, "frontierSmartGate");
...
test("Frontier /create grants items to the active ship cargo while in space", () => {
    ...
    assert.equal(repositionResult.success, true);
    const quantityInCargo = () => listContainerItems(characterID, ship.itemID, ITEM_FLAGS.CARGO_HOLD)
        .filter((item) => Number(item.typeID) === 34)
        .reduce((total, item) => total + Math.max(0, Number(item.stacksize ?? item.quantity) || 0), 0);
    ...
    try {
        const result = executeChatCommand(session, "/create 34 7", null, { emitChatFeedback: false });
...
test("Frontier construction-site crdata omits completed-assembly activation fields", () => {
    ...
    }, { itemID: 9988400000900, kind: "deployable" }, "frontier");
...
test("Frontier completed deployable crdata preserves activation state", () => {
    ...
    }, { itemID: 9988400000999, kind: "deployable" }, "frontier");
    assert.deepEqual(normalized.entries, [
        ["itemID", 9988400000999],
        ["typeID", 88092],
        ["ownerID", 140000005],
...
test("Frontier agent authority excludes locations absent from its static data", () => {
    ...
    assert.equal(shouldIncludeAgentRecord({ stationID: 60000001 }, "frontier", stations, systems), false);
    assert.equal(shouldIncludeAgentRecord({ stationID: 64000001 }, "frontier", stations, systems), true);
    assert.equal(shouldIncludeAgentRecord({ stationID: null, solarSystemID: 30000004, isInSpace: true }, "frontier", stations, systems), true);
    assert.equal(shouldIncludeAgentRecord({ stationID: 60000001 }, "tranquility", stations, systems), true);
});
...
test("Frontier skill entries match the four-argument client constructor", () => {
    ...
    assert.doesNotThrow(() => marshalEncode(frontierEntry, { compatibilityProfile: "frontier" }));
});
...
test("Frontier global inventory returns an indexable typed rowset", () => {
    ...
    assert.equal(rowset.header[0][0].value, "carbon.common.script.sys.crowset.CRowset");
    ...
    assert.doesNotThrow(() => marshalEncode(rowset, { compatibilityProfile: "frontier" }));
});
test("Frontier handshake sends a no-op signed function", () => {
    const noOp = Buffer.from("no-op");
    const payload = selectHandshakeSignedFunc("frontier", noOp, () => Buffer.from("legacy"));
    ...
    assert.deepEqual(marshalDecode(encoded, { compatibilityProfile: "frontier" }), noOp);
});
test("Tranquility handshake preserves the existing TiDi signed function", () => {
    const legacy = Buffer.from("legacy");
    const payload = selectHandshakeSignedFunc("tranquility", Buffer.from("no-op"), () => legacy);
...
test("Frontier version exchange uses the client codename and region", () => {
    assert.deepEqual(selectVersionExchangeRelease("frontier", "V20.04@ccp", "cycle-6", "ccp"), {
...
test("Frontier compact Unicode characters consume one UTF-8 byte", () => {
    ...
    assert.deepEqual(marshalDecodeExact(encoded, { compatibilityProfile: "frontier" }), ["/", 1]);
});
...
test("Frontier nested dogma brain streams avoid the legacy PyObject tag", () => {
    ...
    const frontierBrain = encodeCharacterBrainEffectLists([keyVal], [], [], "frontier");
    const legacyBrain = encodeCharacterBrainEffectLists([keyVal], [], [], "tranquility");
...
test("Frontier Placebo challenge hash follows its Python 3 marshal dialect", () => {
    assert.equal(selectPlaceboChallengeResponseHash("frontier", "\0".repeat(64)), "39856");
...
test("Frontier binary payloads use the Python 3 bytes opcode", () => {
    ...
    assert.equal(encoded[5], 0x13);
    assert.deepEqual(marshalDecode(encoded, { compatibilityProfile: "frontier" }), payload);
});
...
test("Tranquility preserves the existing Placebo challenge hash", () => {
    assert.equal(selectPlaceboChallengeResponseHash("tranquility", Buffer.alloc(64)), "55087");
});
test("Tranquility version exchange preserves the legacy project version", () => {
    assert.equal(selectVersionExchangeRelease("tranquility", "V24.01@ccp", "EvEJS", "ccp"), "V24.01@ccp");
});
...
test("Frontier named objects use the observed ObjectEx2 wire shape", () => {
    ...
    assert.deepEqual(decoded.header[0], [{
            type: "token",
            value: "carbon.common.script.net.machoNetPacket.SessionInitialStateNotification",
        }]);
...
test("Frontier named objects normalize cached-call raw string descriptors", () => {
    const name = "carbon.common.script.net.objectCaching.CachedMethodCallResult";
    const encoded = marshalEncode({
        type: "object",
...
test("Frontier proxy cache stores its inner object with ObjectEx2 encoding", () => {
    ...
    const cachedObject = getCachableObjectResponse(true, cachedReference.args[0], cachedReference.args[2], cachedReference.args[1], "frontier");
    assert.equal(cachedObject.args[4].type, "frontier-bytes");
    const tqCachedObject = getCachableObjectResponse(true, cachedReference.args[0], cachedReference.args[2], cachedReference.args[1], "tq");
...
test("Frontier dogma info attributes carry value and timestamp pairs", () => {
    ...
    assert.deepEqual(normalizeInfoAttributesForProfile(attributes, timestamp, "frontier"), {
        type: "dict",
        entries: [
            [3, [0, timestamp]],
            [4, [6800000, timestamp]],
...
test("Tranquility dogma info attributes preserve scalar values", () => {
    ...
    assert.equal(normalizeInfoAttributesForProfile(attributes, 134299357435130000n, "tranquility"), attributes);
});
test("Frontier ship activation state omits the newer heat-state element", () => {
    ...
    assert.deepEqual(normalizeActivationStateForProfile(activationState, "frontier"), activationState.slice(0, 3));
    assert.equal(normalizeActivationStateForProfile(activationState, "tranquility"), activationState);
});
test("Frontier instance cache uses dogma attribute dictionaries", () => {
    ...
    assert.deepEqual(normalizePackedInstanceRowForProfile(packedRow, "frontier"), {
        type: "dict",
        entries: [
            [2, true],
            [3, 3],
    ...
    assert.equal(normalizePackedInstanceRowForProfile(packedRow, "tranquility"), packedRow);
});
test("Frontier combat timers omit the newer disapproval-timer element", () => {
    ...
    assert.deepEqual(normalizeCombatTimersForProfile(combatTimers, "frontier"), combatTimers.slice(0, 4));
    assert.equal(normalizeCombatTimersForProfile(combatTimers, "tranquility"), combatTimers);
});
test("Frontier space bootstrap receives its three destiny feature flags", () => {
    assert.deepEqual(buildDestinyConfigurationSettings("frontier"), [false, false, false]);
    assert.equal(buildDestinyConfigurationSettings("tranquility"), null);
});
test("Frontier sends an atomic native warp and FX packet", () => {
    ...
    const frontierPrepare = builders.buildWarpPrepareDispatch(entity, 100, warpState, {
        includePilotActivationVelocity: true,
    });
    assert.deepEqual(frontierPrepare.pilotUpdates.map((update) => update.payload[0]), [
        "SetBallWarpFactors",
        "SetMaxSpeed",
        "WarpTo",
        "SetBallVelocity",
    ...
    assert.equal(frontierPrepare.pilotUpdates[0].payload[1][2].value, 1 / 3000);
    ...
    const alignedVelocity = builders.buildWarpPrepareDispatch(movingEntity, 100, warpState, { includePilotActivationVelocity: true }).pilotUpdates[3].payload[1];
    ...
    const tqPrepare = legacyBuilders.buildWarpPrepareDispatch(entity, 100, warpState);
    assert.deepEqual(tqPrepare.pilotUpdates.map((update) => update.payload[0]), ["SetMaxSpeed", "WarpTo", "OnSpecialFX", "SetSpeedFraction"]);
...
test("Frontier deployable warp subjects use the entity warp path", () => {
    ...
    try {
        assert.equal(service.Handle_CmdWarpToStuff(["deployable", targetID], session, { minRange: 25_000 }), null);
    }
    finally {
...
test("Frontier undock defers authoritative ballpark state until beyonce bind", () => {
    assert.equal(defersUndockBallparkStateUntilBeyonceBind("frontier"), true);
    assert.equal(defersUndockBallparkStateUntilBeyonceBind("tranquility"), false);
...
test("Frontier free balls seed dynamical orientation from persisted heading", () => {
    ...
    assert.ok(Math.abs(Math.hypot(orientation.w, orientation.x, orientation.y, orientation.z) - 1) < 1e-12);
...
test("Frontier SetState uses the Python 3 crdata contract", () => {
    ...
    const buildPayload = (compatibilityProfile) => buildSetStatePayload(7, {}, 9001, [{ itemID: 9001, kind: "station" }], simFileTime, [], [], {
    ...
    const getStateEntry = (payload, name) => (payload[1][0].args.entries.find(([key]) => key === name)?.[1]);
    const frontierPayload = buildPayload("frontier");
    assert.deepEqual(getStateEntry(frontierPayload, "crdata").entries[0][0], 9001);
    assert.deepEqual(getStateEntry(frontierPayload, "crdata").entries[0][1].entries
        .map(([key]) => key), ["itemID", "typeID", "activityLevel"]);
...
test("Frontier omits wire slim items; legacy profiles keep them", () => {
    ...
    assert.equal(normalizeSlimItemObjectForProfile(legacySlim, "tranquility"), legacySlim);
...
test("Frontier presentation bundles drop the slim update but keep damage state", () => {
    ...
    assert.deepEqual(buildSlimItemPresentationUpdates({
        stamp: 100,
        entityID: 9988400001771,
        slimItem: legacySlim,
    }).map((update) => update.payload[0]), ["OnSlimItemChange"]);
});
test("Frontier stargate crdata uses a typed CRowset for jumps", () => {
    ...
    const normalized = normalizeCrDataDictionaryForProfile(crData, entity, "frontier");
    const frontierJumps = normalized.entries.find(([key]) => key === "jumps")[1];
    assert.equal(frontierJumps.type, "objectex2");
    assert.equal(frontierJumps.header[0][0].value, "carbon.common.script.sys.crowset.CRowset");
    assert.deepEqual(frontierJumps.list[0].columns, [
        ["toCelestialID", 0x14],
        ["locationID", 0x14],
    ]);
    assert.deepEqual(frontierJumps.list[0].values, [60000002, 30000005]);
    assert.equal(normalizeCrDataDictionaryForProfile(crData, entity, "tranquility"), crData);
});
...
test("Frontier AddBalls2 uses ball ID keyed crdata tuples", () => {
    ...
    const buildPayload = (compatibilityProfile) => buildAddBalls2Payload(9, [{ itemID: 9002, kind: "ship", forceDamageState: true }], 134141234567890001n, {
        compatibilityProfile,
        encodeEntityBall: () => Buffer.alloc(0),
    ...
    assert.deepEqual(frontierPayload[1][0][1].items[0][1].entries.map(([key]) => key), ["itemID", "typeID", "modules"]);
    assert.equal(frontierPayload[1][0][1].items[0][2], damageState);
    const tqPayload = buildPayload("tranquility");
    assert.equal(tqPayload[1][0][0].length, 5);
    assert.deepEqual(tqPayload[1][0][1].items[0], [slimItem, damageState]);
});
...
test("Frontier ObjectEx2 CallReq and MachoAddress decode as PyPacket", () => {
    ...
    const decoded = objectEx2("carbon.common.script.net.machoNetPacket.CallReq", [
        6,
        objectEx2("carbon.common.script.net.machoNetAddress.MachoAddress", [2, 0, 1, null]),
        objectEx2("carbon.common.script.net.machoNetAddress.MachoAddress", [1, 65450, "machoNet", null]),
        3,
        [],
        { type: "dict", entries: [] },
    ]);
...
//# sourceMappingURL=frontierHandshakeProfile.test.js.map