...
async function cloneSqliteDatabase(sourcePath, destinationPath) {
    if (fs.existsSync(destinationPath)) {
        throw new Error(`Refusing to replace an existing SQLite backup: ${destinationPath}`);
    }
    let Database;
    try {
        ...
        throw new Error("Cannot create a consistent SQLite test snapshot because " +
            "server/node_modules/better-sqlite3 is unavailable. Run " +
            "`npm --prefix server ci` under the active Node version.", { cause: error });
    }
    let sourceDatabase;
    try {
        ...
        throw new Error(`Failed to back up SQLite baseline ${sourcePath}: ${error.message}`, { cause: error });
    }
    finally {
    ...
    if (!fs.existsSync(destinationPath)) {
        throw new Error(`SQLite backup reported success without creating ${destinationPath}`);
    }
    return true;
}
function seedFrontierTestFixtures(storeEnvironment) {
    const seederPath = path.join(REPO_ROOT, "scripts", "Tests", "seed-frontier-test-fixture.js");
    ...
    if (result.status !== 0) {
        throw new Error(`Frontier test fixture seeder failed with exit code ${result.status}`);
    }
}
/**
...
async function createIsolatedGameStore(options = {}) {
    const baselineRoot = path.resolve(options.baselineRoot || resolveDefaultBaselineRoot());
    const storeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "evejs-test-store-"));
    ...
    try {
        ...
        if (fs.existsSync(baselineSqlite)) {
            await cloneSqliteDatabase(baselineSqlite, path.join(storeRoot, "gamestore.sqlite"));
        }
...
//# sourceMappingURL=isolatedGameStore.js.map