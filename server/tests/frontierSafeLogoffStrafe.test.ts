...
test("Safe Logoff persistence consumes the scoped token and keeps exact space state", () => {
  ...
  const liveSpaceState: Record<string, any> = {
    systemID: 30000004,
...
test("Creation hull damage and position survive disconnect/reconnect while Safe Logoff de-instances", (t) => {
  ...
  const scene = new spaceRuntime._testing.SolarSystemScene(
    PERSISTENCE_TEST_SYSTEM_ID,
  );
  spaceRuntime.scenes.set(PERSISTENCE_TEST_SYSTEM_ID, scene);
  const entity = spaceRuntime._testing.buildShipEntityForTesting(
    session,
    itemStore.findShipItemById(shipID),
    PERSISTENCE_TEST_SYSTEM_ID,
  );
  ...
  const resumedSession: Record<string, any> = {
    ...session,
    clientID: 91_005,
    socket: { destroyed: false },
    _space: null,
  };
  const resumed = scene.attachSessionToExistingEntity(
    resumedSession,
    persistedOfflineShip,
    offlineEntity,
    { broadcast: false },
  );
...
test("conventional hull damage and position also survive disconnect/reconnect", (t) => {
  ...
  const rebuiltScene = new spaceRuntime._testing.SolarSystemScene(
    PERSISTENCE_TEST_SYSTEM_ID,
  );
  ...
  const resumed = spaceRuntime.attachSession(
    resumedSession,
    persistedShip,
    {
      systemID: PERSISTENCE_TEST_SYSTEM_ID,
