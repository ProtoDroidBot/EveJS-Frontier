...
test("gateway deposit/withdraw round trip publishes exactly-once character notices", () => {
    ...
    const emptyResult = service.handleRequest(GET_INVENTORY_REQUEST, makeEnvelope(types.GetInventoryRequest, {
    ...
    const prepareResult = service.handleRequest(PREPARE_DEPOSIT_ITEMS_REQUEST, makeEnvelope(types.PrepareDepositItemsRequest, {
    ...
    const execute = service.handleRequest(EXECUTE_DEPOSIT_ITEMS_REQUEST, makeEnvelope(types.ExecuteDepositItemsRequest, executePayload));
    ...
    const replay = service.handleRequest(EXECUTE_DEPOSIT_ITEMS_REQUEST, makeEnvelope(types.ExecuteDepositItemsRequest, executePayload));
    assert.equal(replay.statusCode, 200);
    assert.equal(notices.length, 1, "retry must not publish a second notice");
    const inventoryResult = service.handleRequest(GET_INVENTORY_REQUEST, makeEnvelope(types.GetInventoryRequest, {
        inventory_owner: { sequential: OWNER_ID },
        storage_unit: { sequential: unit.itemID },
    }));
    ...
    const withdrawPrepareResult = service.handleRequest(PREPARE_WITHDRAW_ITEMS_REQUEST, makeEnvelope(types.PrepareWithdrawItemsRequest, {
        generic_location: {
            flag: { value: CARGO_FLAG },
            item: { sequential: ship.itemID },
        },
    ...
    const withdrawExecute = service.handleRequest(EXECUTE_WITHDRAW_ITEMS_REQUEST, makeEnvelope(types.ExecuteWithdrawItemsRequest, {
...
test("gateway rejects unauthenticated, spoofed owner, invalid signature, and stale UUID", () => {
    ...
    const spoofed = service.handleRequest(GET_INVENTORY_REQUEST, makeEnvelope(types.GetInventoryRequest, {
        inventory_owner: { sequential: VISITOR_ID },
        storage_unit: { sequential: unit.itemID },
    }));
    assert.equal(spoofed.statusCode, 403);
    const prepare = service.handleRequest(PREPARE_DEPOSIT_ITEMS_REQUEST, makeEnvelope(types.PrepareDepositItemsRequest, {
        destination_container: { sequential: unit.itemID },
        source_container: {
            flag: { value: CARGO_FLAG },
            item: { sequential: ship.itemID },
    ...
    const badSignature = service.handleRequest(EXECUTE_DEPOSIT_ITEMS_REQUEST, makeEnvelope(types.ExecuteDepositItemsRequest, {
        prepared_transaction: prepared.prepared_transaction,
        signature: "not-a-signature",
    }));
    assert.equal(badSignature.statusCode, 403);
    assert.equal(totalAt(OWNER_ID, unit.itemID, STORAGE_FLAG, MATERIAL_TYPE_ID), 0);
    const stale = service.handleRequest(EXECUTE_DEPOSIT_ITEMS_REQUEST, makeEnvelope(types.ExecuteDepositItemsRequest, {
