...
function chainStateFixture(t, initialStatus = 2) {
    const chain = { status: initialStatus, reads: 0, error: null, beforeRead: null };
...
test("gateway deposit uses online chain status over stale offline local state and commits once", async (t) => {
    ...
    const prepare = await service.handleRequest(PREPARE_DEPOSIT_ITEMS_REQUEST, makeEnvelope(types.PrepareDepositItemsRequest, {
...
test("storageunit proto mirrors nested prepared transaction and destination oneof", () => {
    const types = getStorageUnitProtoTypes();
    const deposit = types.PrepareDepositItemsRequest.decode(types.PrepareDepositItemsRequest.encode(types.PrepareDepositItemsRequest.create({
        source_container: {
    ...
    const withdraw = types.PrepareWithdrawItemsRequest.decode(types.PrepareWithdrawItemsRequest.encode(types.PrepareWithdrawItemsRequest.create({
    ...
    const response = types.PrepareDepositItemsResponse.decode(types.PrepareDepositItemsResponse.encode(types.PrepareDepositItemsResponse.create({
    ...
    assert.equal(response.prepared_transaction_attributes.bcs_data_b64_bytes, "{\"version\":2}");
});
...
test("deposit prepare is non-mutating; execute commits once and survives duplicate retry", () => {
    ...
    assert.equal(Object.prototype.hasOwnProperty.call(storedMaterial, "moduleState"), false, "ordinary stored material must never be treated as an online module");
...
test("dApp authorizations bind the wallet, unit, quantities and replay after prepare expiry", () => {
    ...
    assert.equal(transaction.gasData.payment[0].objectId, `0x${require("node:crypto").createHash("sha256").update(prepared.data.authorizationMessage).digest("hex")}`);
...
test("mutations reject offline, out-of-range, singleton, and changed source state", () => {
    ...
    const singleton = grantOne(OWNER_ID, ship.itemID, CARGO_FLAG, MATERIAL_TYPE_ID, 1, { individualItems: true, singleton: 1 });
    const specialSingleton = grantOne(OWNER_ID, ship.itemID, CARGO_FLAG, MATERIAL_TYPE_ID, 1, { individualItems: true, singleton: 2 });
    const base = {
        characterID: OWNER_ID,
        sourceFlagID: CARGO_FLAG,
        sourceLocationID: ship.itemID,
    ...
    assert.equal(smartStorageUnitRuntime.prepareStorageDeposit({
        ...base,
    ...
    assert.equal(smartStorageUnitRuntime.prepareStorageDeposit({
        ...
        storageUnitID: online.itemID,
    }).errorMsg, "SINGLETON_NOT_ACCEPTED");
    assert.equal(smartStorageUnitRuntime.prepareStorageDeposit({
        ...base,
        access: createAccess(ship),
...
test("withdraw rejects fitting flags without losing stored quantities", () => {
    ...
    assert.equal(smartStorageUnitRuntime._testing.getShipCargoCapacity(OWNER_ID, creation), 288, "the default Creation's eight cargo modules each contribute 36 m³");
...
test("gateway deposit/withdraw round trip publishes exactly-once character notices", async () => {
    ...
    const emptyResult = await service.handleRequest(GET_INVENTORY_REQUEST, makeEnvelope(types.GetInventoryRequest, {
    ...
    assert.equal(types.GetInventoryResponse.decode(emptyResult.responsePayloadBuffer).items.length, 0);
    const prepareResult = await service.handleRequest(PREPARE_DEPOSIT_ITEMS_REQUEST, makeEnvelope(types.PrepareDepositItemsRequest, {
        destination_container: { sequential: unit.itemID },
        source_container: {
            flag: { value: CARGO_FLAG },
    ...
    const prepared = types.PrepareDepositItemsResponse.decode(prepareResult.responsePayloadBuffer);
    const transactionUUID = uuidBufferToString(Buffer.from(prepared.prepared_transaction.uuid));
    assert.ok(transactionUUID);
    assert.match(prepared.prepared_transaction_attributes.bcs_data_b64_bytes, /"gasData"/u);
    assert.equal(notices.length, 0);
    const executePayload = {
        prepared_transaction: prepared.prepared_transaction,
        signature: VALID_SIGNATURE,
    };
    const execute = await service.handleRequest(EXECUTE_DEPOSIT_ITEMS_REQUEST, makeEnvelope(types.ExecuteDepositItemsRequest, executePayload));
    ...
    const depositNotice = types.InventoryItemDepositedNotice.decode(notices[0].payloadBuffer);
    assert.equal(Number(depositNotice.item.attributes.quantity), 25);
    assert.equal(Number(depositNotice.item.attributes.identifier.sequential), MATERIAL_TYPE_ID);
    const replay = await service.handleRequest(EXECUTE_DEPOSIT_ITEMS_REQUEST, makeEnvelope(types.ExecuteDepositItemsRequest, executePayload));
    assert.equal(replay.statusCode, 200);
    assert.equal(notices.length, 1, "retry must not publish a second notice");
    const inventoryResult = await service.handleRequest(GET_INVENTORY_REQUEST, makeEnvelope(types.GetInventoryRequest, {
        inventory_owner: { sequential: OWNER_ID },
        storage_unit: { sequential: unit.itemID },
    }));
    const inventory = types.GetInventoryResponse.decode(inventoryResult.responsePayloadBuffer);
    assert.equal(inventory.items.length, 1);
    assert.equal(Number(inventory.items[0].attributes.quantity), 25);
    const withdrawPrepareResult = await service.handleRequest(PREPARE_WITHDRAW_ITEMS_REQUEST, makeEnvelope(types.PrepareWithdrawItemsRequest, {
        generic_location: {
            flag: { value: CARGO_FLAG },
            item: { sequential: ship.itemID },
        },
    ...
    assert.equal(withdrawPrepareResult.statusCode, 200, withdrawPrepareResult.statusMessage);
    const withdrawPrepared = types.PrepareWithdrawItemsResponse.decode(withdrawPrepareResult.responsePayloadBuffer);
    const withdrawExecute = await service.handleRequest(EXECUTE_WITHDRAW_ITEMS_REQUEST, makeEnvelope(types.ExecuteWithdrawItemsRequest, {
    ...
    const withdrawNotice = types.InventoryItemWithdrawnNotice.decode(notices[1].payloadBuffer);
...
test("gateway rejects unauthenticated, spoofed owner, invalid signature, and stale UUID", async () => {
    ...
    const unauthenticated = await service.handleRequest(GET_INVENTORY_REQUEST, {
        authoritative_context: {},
        payload: {
            value: Buffer.from(types.GetInventoryRequest.encode(types.GetInventoryRequest.create({
                inventory_owner: { sequential: OWNER_ID },
                storage_unit: { sequential: unit.itemID },
            })).finish()),
        },
    ...
    const spoofed = await service.handleRequest(GET_INVENTORY_REQUEST, makeEnvelope(types.GetInventoryRequest, {
    ...
    const prepared = types.PrepareDepositItemsResponse.decode(prepare.responsePayloadBuffer);
    const badSignature = await service.handleRequest(EXECUTE_DEPOSIT_ITEMS_REQUEST, makeEnvelope(types.ExecuteDepositItemsRequest, {
        prepared_transaction: prepared.prepared_transaction,
        signature: "not-a-signature",
    }));
    assert.equal(badSignature.statusCode, 403);
    assert.equal(totalAt(OWNER_ID, unit.itemID, STORAGE_FLAG, MATERIAL_TYPE_ID), 0);
    const stale = await service.handleRequest(EXECUTE_DEPOSIT_ITEMS_REQUEST, makeEnvelope(types.ExecuteDepositItemsRequest, {
        prepared_transaction: { uuid: Buffer.alloc(16, 2) },
        signature: VALID_SIGNATURE,
    }));
    assert.equal(stale.statusCode, 404);
});
//# sourceMappingURL=frontierSmartStorageUnit.test.js.map