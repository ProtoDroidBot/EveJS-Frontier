...
test("Frontier creation templates become populated client fitting models", () => {
  ...
  assert.deepEqual(plan.map(({ kind, typeID }: Record<string, any>) => ({ kind, typeID })), [
...
test("Frontier creation passive modules derive ship fuel and capacitor capacity", () => {
  ...
  const shipAttributes: Record<string, any> = { 482: 0, 5633: 0 };
...
test("Frontier Creation refreshes derived attributes after Godma hydration", () => {
  ...
  assert.equal(service._queuePostGetAllInfoCreationAttributeRefresh(
    ...
    140000005,
    9988400001895,
    { 18: 200, 482: 200, 5633: 2250, 5635: 2250 },
    {
      shipAttributeModifierEntries: [
        { modifiedAttributeID: 482 },
        { modifiedAttributeID: 5633 },
...
test("Frontier Creation replays fuel charge after docked Godma hydration settles", () => {
  ...
  service._dispatchPostResponseAttributeChanges = (
    receivedSession,
    changes,
    options = {},
  ) => {
...
test("Frontier ship activation evaluates heat state without a missing clock symbol", () => {
  ...
  const activeShip: Record<string, any> = {
...
test("Frontier construction hydration publishes assembly activation state", () => {
  const offlineEntity: Record<string, any> = {};
  ...
  const constructionEntity: Record<string, any> = {
  ...
  const smartGateEntity: Record<string, any> = { kind: "deployable" };
...
test("Frontier Refuge accepts shuttles, corvettes, and current modular Creation hulls", () => {
  ...
  assert.equal(
    berthingContractTesting.smartHangarAcceptsShipGroup(partialRefuge, 31),
    true,
  );
  assert.equal(
    berthingContractTesting.smartHangarAcceptsShipGroup(partialRefuge, 237),
    true,
  );
});
...
test("Frontier Creation berths at Refuge center and departs outside its hull", () => {
  ...
  const creationState: Record<string, any> = {
    templateTypeID: 95276,
  ...
  const refugeItem: Record<string, any> = {
    itemID: refugeID,
    typeID: 87160,
    ownerID: characterID,
    locationID: solarSystemID,
  ...
  const refugeEntity: Record<string, any> = {
    itemID: refugeID,
    kind: "deployable",
    radius: 1,
  ...
  const spaceRuntime: Record<string, any> = {
    ...
    teleportSessionShipToPoint(_session, point, options) {
      const position: Record<string, any> = { ...point };
      const direction: Record<string, any> = { ...options.direction };
  ...
  try {
    ...
    assert.equal(
      frontierSpaceRuntime._testing.isShipMovementLockedByRuntimeForTesting(
        shipEntity,
      ),
      true,
    );
    ...
    assert.equal(
      Object.hasOwn(shipEntity, "frontierBerthingHostAssemblyID"),
      false,
    );
    assert.equal(
      frontierSpaceRuntime._testing.isShipMovementLockedByRuntimeForTesting(
...
test("Frontier berthing service exposes the complete client contract", () => {
  ...
  const session: Record<string, any> = { characterID: contract.characterID };
...
test("Frontier Heavy Gates stay owner-managed while allowing visitor traversal", () => {
  const networkNodeEnergyRuntime = require(
    "../src/services/frontier/networkNodeEnergyRuntime",
  );
  const originalValidateAssemblyOnline =
    networkNodeEnergyRuntime.validateAssemblyOnline;
  const originalReconcileNetworkNodeEnergy =
  ...
  const visitorSession: Record<string, any> = {
    ...ownerSession,
  ...
  try {
    ...
    const destinationSession: Record<string, any> = {
      ...ownerSession,
...
test("Frontier skill entries match the four-argument client constructor", () => {
  const skillRecord: Record<string, any> = {
    typeID: 3300,
    trainedSkillLevel: 4,
...
test("Frontier nested dogma brain streams avoid the legacy PyObject tag", () => {
  const keyVal: Record<string, any> = {
    type: "object",
    name: "util.KeyVal",
...
test("Frontier proxy cache stores its inner object with ObjectEx2 encoding", () => {
  const rowset: Record<string, any> = {
    type: "object",
...
test("Frontier instance cache uses dogma attribute dictionaries", () => {
  const packedRow: Record<string, any> = {
    type: "packedrow",
    fields: {
...
test("Frontier sends an atomic native warp and FX packet", () => {
  ...
  const cloneVector = (value, fallback: Record<string, any> = { x: 0, y: 0, z: 0 }) => ({
  ...
  const builderDeps: Record<string, any> = {
    cloneVector,
  ...
  const warpState: Record<string, any> = {
  ...
  const movingEntity: Record<string, any> = {
    ...entity,
...
test("Frontier deployable warp subjects use the entity warp path", () => {
  ...
  const session: Record<string, any> = { characterID: 140000005 };
...
test("Frontier free balls seed dynamical orientation from persisted heading", () => {
  const direction: Record<string, any> = {
  ...
  const rotatedUp: Record<string, any> = {
  ...
  const expectedUp: Record<string, any> = {
...
test("Frontier omits wire slim items; legacy profiles keep them", () => {
  const legacySlim: Record<string, any> = {
    type: "object",
    name: "foo.SlimItem",
...
test("Frontier presentation bundles drop the slim update but keep damage state", () => {
  const damageState: Record<string, any> = { type: "list", items: [] };
...
test("Frontier stargate crdata uses a typed CRowset for jumps", () => {
  ...
  const crData: Record<string, any> = {
    type: "dict",
    entries: [
      ["itemID", 60000001],
      ["typeID", 79787],
...
test("Frontier AddBalls2 uses ball ID keyed crdata tuples", () => {
  ...
  const damageState: Record<string, any> = { marker: "damage" };
