...
const { CUSTOM_SITE_ID_BASE, createCustomLandscapeSiteStore, } = require("../src/space/frontierLandscapeCustomSites");
const { executeFrontierLandscapeCommand, resolveSpawnPosition, } = require("../src/services/chat/frontierLandscapeCommands");
function buildReferenceData() {
    ...
    return {
        ...
        getLandscapeEcosystemByID: (ecosystemID) => (ecosystems.find((entry) => entry.ecosystemID === Number(ecosystemID)) || null),
        getLandscapeDungeonTemplateByID: (dungeonID) => (dungeons.get(Number(dungeonID)) || null),
...
test("landscape command spawns Supply Station through the live scene runtime", () => {
    ...
    const result = executeFrontierLandscapeCommand({ characterID: 140000005 }, "spawn 27 ahead=5000", {
        itemTypes: {
...
//# sourceMappingURL=frontierLandscapeCommands.test.js.map