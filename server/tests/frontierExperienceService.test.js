...
const shellEquipment = require("../src/services/frontier/shellEquipmentRuntime");
...
test("Frontier experience returns the complete zero-valued character contract", () => {
    ...
    assert.deepEqual(service.Handle_get_character([], { charid: CHARACTER_ID }), response);
...
test("Frontier experience progression exposes four attribute-bearing pathway rows", () => {
    ...
    assert.deepEqual(response.items.map((row) => {
    ...
    assert.doesNotThrow(() => marshalEncode(response, FRONTIER_MARSHAL_OPTIONS));
    assert.deepEqual(service.Handle_get_character_progression([], {}), response);
});
...
test("Frontier experience exposes bounded empty crown, totals, and ascension reads", () => {
    ...
    assert.deepEqual(service.Handle_get_memories_from_crown([12345], { charid: CHARACTER_ID }), emptyMemories);
    ...
    assert.doesNotThrow(() => marshalEncode(service.Handle_get_memory_point_totals(), FRONTIER_MARSHAL_OPTIONS));
});
test("semantically undefined experience mutations fail explicitly as UserError", () => {
    ...
    for (const method of mutationMethods) {
        ...
        assert.throws(() => service[`Handle_${method}`]([], { charid: CHARACTER_ID }), isWrappedUserError, method);
    }
});
test("shell rename is owner-scoped, client-bounded, persisted, and notified", () => {
    const grant = itemStore.grantItemToOwnerLocation(CHARACTER_ID, CHARACTER_ID, itemStore.ITEM_FLAGS.HANGAR, ShellManagerService._testing.DEFAULT_SHELL_TYPE_ID, 1, {
    ...
    try {
        ...
        assert.equal(service.Handle_set_shell_name([shell.itemID, "Pathfinder Shell"], session), true);
        assert.equal(itemStore.findItemById(shell.itemID).itemName, "Pathfinder Shell");
        assert.equal(notifications.some((entry) => entry[0] === "OnShellChangedName" &&
            entry[2][0] === shell.itemID &&
            entry[2][1] === "Pathfinder Shell"), true);
        assert.throws(() => service.Handle_set_shell_name([shell.itemID, "x".repeat(101)], session), isWrappedUserError);
        assert.throws(() => service.Handle_set_shell_name([shell.itemID, "Foreign Rename"], { charid: CHARACTER_ID + 1 }), isWrappedUserError);
        assert.equal(itemStore.findItemById(shell.itemID).itemName, "Pathfinder Shell");
    }
    finally {
...
test("shell implants and raiments persist per shell and are destroyed by lifecycle cleanup", () => {
    ...
    shellEquipment._testing.setEquipmentGroupOverride(IMPLANT_TYPE_ID, shellEquipment.SHELL_NEURAL_IMPLANT_GROUP_ID);
    shellEquipment._testing.setEquipmentGroupOverride(SECOND_IMPLANT_TYPE_ID, shellEquipment.SHELL_NEURAL_IMPLANT_GROUP_ID);
    shellEquipment._testing.setEquipmentGroupOverride(RAIMENT_TYPE_ID, shellEquipment.SHELL_RAIMENT_GROUP_ID);
    shellEquipment._testing.setEquipmentGroupOverride(CROWN_TYPE_ID, shellEquipment.SHELL_CROWN_GROUP_ID);
    const createdItems = [];
    const grantOwned = (typeID, itemName) => {
        const grant = itemStore.grantItemToOwnerLocation(CHARACTER_ID, CHARACTER_ID, itemStore.ITEM_FLAGS.HANGAR, typeID, 1, { individualItems: true, itemName, singleton: 1 });
    ...
    try {
        ...
        assert.equal(shellService.Handle_implant_implant([implant.itemID], session), true);
        assert.equal(experienceService.Handle_implant_raiment([raiment.itemID], session), true);
        ...
        assert.equal(itemStore.findItemById(implant.itemID).flagID, shellEquipment.SHELL_EQUIPMENT_FLAG_ID);
        ...
        assert.deepEqual(Object.fromEntries(shellService.Handle_get_active_shell_db_data([], session).entries).implants, { type: "list", items: [IMPLANT_TYPE_ID] });
        assert.throws(() => shellService.Handle_implant_implant([secondImplant.itemID], session), isWrappedUserError);
        ...
        assert.equal(shellService.Handle_implant_implant([secondImplant.itemID], session), true);
        // A crown can coexist in the same inventory flag, but is excluded until
        // its separate attribute semantics are implemented.
        assert.equal(itemStore.moveItemToLocation(crown.itemID, shell.itemID, shellEquipment.SHELL_EQUIPMENT_FLAG_ID, 1).success, true);
        const ascensionCleanup = shellEquipment.destroyActiveShellEquipmentForAscension(CHARACTER_ID);
        ...
        assert.equal(shellService.Handle_implant_implant([deathImplant.itemID], session), true);
        assert.equal(experienceService.Handle_implant_raiment([deathRaiment.itemID], session), true);
        const shipDestruction = require("../src/space/shipDestruction");
        const deathCleanup = shipDestruction._testing.destroySessionShellEquipmentForDeath(session);
...
//# sourceMappingURL=frontierExperienceService.test.js.map