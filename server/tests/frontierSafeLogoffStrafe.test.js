...
const { buildGlobalConfigEntries, } = require("../src/services/machoNet/globalConfig");
const { clearCharacterFromSession, getActiveShipRecord, } = require("../src/services/character/characterState");
const { persistCharacterLogoffState, } = require("../src/services/_shared/sessionDisconnect");
const { getEmergencyWarpReturnState, } = require("../src/services/_shared/emergencyWarpRuntime");
const { SAFE_LOGOFF_ABORT_LABELS, SAFE_LOGOFF_CONDITION_LABELS, beginSafeLogoff, cancelSafeLogoff, consumeSafeLogoffCompletion, evaluateSafeLogoffConditions, getSafeLogoffCompletion, getSafeLogoffState, markSafeLogoffCompletion, resetSafeLogoffRuntimeForTests, toFileTime, } = require("../src/services/ship/safeLogoffRuntime");
...
function createVectorHelpers() {
    ...
    const buildPerpendicular = (direction) => normalizeVector(crossProduct(normalizeVector(direction), { x: 0, y: 1, z: 0 }), { x: 0, y: 0, z: 1 });
    return {
        addVectors,
        buildPerpendicular,
        clamp: (value, minimum, maximum) => Math.max(minimum, Math.min(maximum, toFiniteNumber(value, 0))),
        cloneVector,
        crossProduct,
        magnitude,
        normalizeVector,
...
test("Frontier global config explicitly advertises Safe Logoff and strafe", () => {
    ...
    const frontier = new Map(buildGlobalConfigEntries({
        clientCompatibilityProfile: "frontier",
    }));
    assert.equal(frontier.get("sui_network"), "localnet");
    assert.equal(frontier.has("web3_api_gateway_url"), false);
});
test("Safe Logoff returns the live-client localization labels for unsafe state", () => {
    const { characterID, entity, options, scene, session } = createSafeLogoffFixture();
    const nowMs = 10_000;
    entity.mode = "WARP";
    entity.pendingWarp = {};
...
test("Safe Logoff activates then invokes canonical cleanup with scoped persistence identity", () => {
    ...
    const result = beginSafeLogoff(session, {
        ...
        disconnectCharacterSession(targetSession, cleanupOptions) {
            ...
            assert.equal(targetSession.notifications.at(-1).name, "OnSafeLogoffActivated");
            assert.deepEqual(getSafeLogoffCompletion(targetSession), {
                characterID,
                shipID: entity.itemID,
                completedAtMs: nowMs,
            });
            assert.deepEqual(consumeSafeLogoffCompletion(targetSession, {
                characterID,
                shipID: entity.itemID,
                nowMs,
...
test("Frontier strafe command clamps acceleration and emits native Destiny action", () => {
    ...
    assert.deepEqual(sent[0].payload[1].slice(1).map((value) => value.value), [10, -10, 0]);
...
//# sourceMappingURL=frontierSafeLogoffStrafe.test.js.map