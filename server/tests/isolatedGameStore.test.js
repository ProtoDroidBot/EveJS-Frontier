...
const { cloneSqliteDatabase, } = require("./helpers/isolatedGameStore");
...
test("SQLite backup refuses to overwrite a destination", async (t) => {
    ...
    await assert.rejects(cloneSqliteDatabase(sourcePath, destinationPath), /Refusing to replace an existing SQLite backup/);
    assert.equal(fs.readFileSync(destinationPath, "utf8"), "do not replace");
});
//# sourceMappingURL=isolatedGameStore.test.js.map