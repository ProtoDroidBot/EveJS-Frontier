...
const { createTypeListAuthority, expandTypeList, matchesTypeListInAuthority, isTradableInventoryItemInAuthority, } = require(path.join(__dirname, "../src/services/inventory/typeListAuthority"));
...
const mobileAnalysisBeaconRuntime = require(path.join(__dirname, "../src/services/ship/mobileAnalysisBeaconRuntime"));
const AutoMoonMiningService = require(path.join(__dirname, "../src/services/structure/autoMoonMiningService"));
const industryRuntimeState = require(path.join(__dirname, "../src/services/industry/industryRuntimeState"));
const miningIndustry = require(path.join(__dirname, "../src/services/mining/miningIndustry"));
const EssMgrService = require(path.join(__dirname, "../src/services/dynamic/essMgrService"));
const { resolveDungeonShipRestrictions } = require(path.join(__dirname, "../src/services/dungeon/dungeonShipRestrictionResolver"));
...
test("ESS link attempts without an in-space ship fail closed", () => {
    assert.equal(EssMgrService._testing.validateEssLinkEligibility({ characterID: 77 }, { essID: 501, mainBankLink: null, reserveLinkedCharacterIDs: new Set() }, "main"), "LINK_ERROR_NO_BALLPARK");
});
...
test("build 3502403 membership matches the installed client bytecode oracle", {
    ...
    for (const [listID, [expectedCount, expectedHash]] of Object.entries(fixture.membership)) {
    ...
    assert.equal(crypto.createHash("sha256").update(catalogDigestInput).digest("hex"), fixture.catalogSha256, "full Frontier typelist catalog matches the installed client");
});
//# sourceMappingURL=typeListAuthority.test.js.map