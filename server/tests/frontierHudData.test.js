...
const { buildSystemInfo, buildSystemInfoFromAuthoritativeRecord, } = require("../src/services/frontier/systemInfoService");
const { buildStaticSearchIndexFromRows, } = SearchService._testing;
...
test("Frontier static search indexes every constellation/region ID without invented names", () => {
    ...
    assert.deepEqual(constellationIndex.exactRawNameMap.get("c-20000005"), [20000005]);
    assert.equal(constellationIndex.entries.filter((entry) => entry.id === 20000004).length, 1);
    ...
    assert.equal(regionIndex.entries.some((entry) => entry.rawName === "region 10000005"), false);
});
...
test("Frontier SystemInfo stays exact-shaped when 3467658 lacks authored inputs", () => {
    const response = buildSystemInfo(30000004, {
        getSolarSystemByID: (systemID) => (systemID === 30000004
...
function buildCairnOptions(channel, code = null) {
    ...
    return {
        ...
        getCharacterCorporationID: (characterID) => (characterID === 42 || characterID === 43 ? 7 : 8),
...
test("Frontier Cairn markers honor matching code visibility", () => {
    const service = new CairnService("cairnService", buildCairnOptions("code", "shared-code"));
    const response = service.Handle_get_visible_cairns([], {
        characterID: 42,
        corporationID: 7,
        shipID: 500,
...
test("Frontier Cairn transponder mutation rejects owned non-Cairn items unchanged", () => {
    ...
    const service = new IffMapService({
        findItemById: (itemID) => (itemID === ownedShip.itemID ? ownedShip : null),
    ...
    assert.equal(service.Handle_set_transponder([ownedShip.itemID, "tribe", null], { characterID: 42, solarsystemid2: 30000004 }), false);
    assert.equal(writeCalls, 0);
    assert.equal(ownedShip.customInfo, originalCustomInfo);
});
//# sourceMappingURL=frontierHudData.test.js.map