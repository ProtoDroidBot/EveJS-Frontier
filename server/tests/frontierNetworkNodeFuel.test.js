...
function grantOne(ownerID, locationID, flagID, itemType, quantity, options) {
    const result = itemStore.grantItemsToCharacterLocation(ownerID, locationID, flagID, [{ itemType, quantity, options }]);
...
test("get fuel: empty node reads zero, errors are typed", () => {
    ...
    assert.equal(networkNodeFuelRuntime.getNetworkNodeFuelStatus(OTHER_OWNER_ID, node.itemID).errorMsg, "ASSEMBLY_NOT_OWNED");
    assert.equal(networkNodeFuelRuntime.getNetworkNodeFuelStatus(OWNER_ID, 424242).errorMsg, "ASSEMBLY_NOT_FOUND");
});
...
test("deposit validation failures are typed and non-destructive", () => {
    ...
    assert.equal(networkNodeFuelRuntime.prepareNetworkNodeFuelDeposit({
        characterID: OWNER_ID,
        networkNodeID: wrongOwnerNode.itemID,
        sourceItemID: container.itemID,
        sourceFlagID: CARGO_FLAG,
        items: [{ itemID: stack.itemID, quantity: 10 }],
    }).errorMsg, "ASSEMBLY_NOT_OWNED");
    ...
    for (const quantity of [0, -5]) {
        ...
        }).errorMsg, "INVALID_QUANTITY", `quantity=${quantity}`);
    }
    assert.equal(networkNodeFuelRuntime.prepareNetworkNodeFuelDeposit({
        characterID: OWNER_ID,
        networkNodeID: node.itemID,
    ...
    }).errorMsg, "INSUFFICIENT_SOURCE_FUEL");
    ...
    }).errorMsg, "UNSUPPORTED_FUEL_TYPE");
...
test("withdraw succeeds, validates, and remains atomic on grant failure", () => {
    ...
    assert.equal(networkNodeFuelRuntime.prepareNetworkNodeFuelWithdraw({
        characterID: OWNER_ID,
        networkNodeID: node.itemID,
        fuelTypeID: FUEL_D1,
        quantity: 100,
    ...
    assert.equal(networkNodeFuelRuntime.prepareNetworkNodeFuelWithdraw({
        ...
        destinationItemID: container.itemID,
        destinationFlagID: CARGO_FLAG,
    }).errorMsg, "INSUFFICIENT_STORED_FUEL");
    assert.equal(networkNodeFuelRuntime.prepareNetworkNodeFuelWithdraw({
        characterID: OWNER_ID,
        networkNodeID: node.itemID,
        fuelTypeID: FUEL_UNSTABLE,
    ...
    assert.equal(networkNodeFuelRuntime.readNetworkNodeFuelState(itemStore.findItemById(node.itemID)).quantity, 600);
...
test("gateway service: config, get, deposit round trip with one notice", () => {
    ...
    const node = createTestNetworkNode();
    const { container, stack } = createFuelSource();
    const configResult = service.handleRequest(GET_FUEL_CONFIG_REQUEST, makeEnvelope(types.GetFuelConfigRequest, {}));
    assert.equal(configResult.statusCode, 200);
    const configDecoded = types.GetFuelConfigResponse.decode(configResult.responsePayloadBuffer);
    ...
    const emptyFuel = service.handleRequest(GET_FUEL_REQUEST, makeEnvelope(types.GetFuelRequest, {
    ...
    const prepareResult = service.handleRequest(PREPARE_DEPOSIT_FUEL_REQUEST, makeEnvelope(types.PrepareDepositFuelRequest, {
        network_node: { sequential: node.itemID },
    ...
    const prepareDecoded = types.PrepareDepositFuelResponse.decode(prepareResult.responsePayloadBuffer);
    const transactionUUID = uuidBufferToString(Buffer.from(prepareDecoded.prepared_transaction_uuid));
    assert.ok(transactionUUID, "prepare must return a uuid");
    assert.ok(Buffer.from(prepareDecoded.prepared_transaction_bcs_data).toString("utf8")
        .includes("gasData"));
    assert.equal(notices.length, 0, "prepare must not publish a notice");
    const executeResult = service.handleRequest(EXECUTE_DEPOSIT_FUEL_REQUEST, makeEnvelope(types.ExecuteDepositFuelRequest, {
    ...
    const fuelResult = service.handleRequest(GET_FUEL_REQUEST, makeEnvelope(types.GetFuelRequest, {
        network_node: { sequential: node.itemID },
    }));
    ...
    const withdrawPrepare = service.handleRequest(PREPARE_WITHDRAW_FUEL_REQUEST, makeEnvelope(types.PrepareWithdrawFuelRequest, {
        network_node: { sequential: node.itemID },
    ...
    const withdrawDecoded = types.PrepareWithdrawFuelResponse.decode(withdrawPrepare.responsePayloadBuffer);
    const withdrawExecute = service.handleRequest(EXECUTE_WITHDRAW_FUEL_REQUEST, makeEnvelope(types.ExecuteWithdrawFuelRequest, {
    ...
    assert.equal(Number(types.FuelChangedNotice.decode(notices[1].payloadBuffer).fuel.quantity), 750);
});
test("gateway service: deposit and withdraw synchronize exact client inventory deltas", () => {
    ...
    const executeDeposit = (itemID, quantity) => {
        const prepared = service.handleRequest(PREPARE_DEPOSIT_FUEL_REQUEST, makeEnvelope(types.PrepareDepositFuelRequest, {
            network_node: { sequential: node.itemID },
            source: {
                item: { sequential: container.itemID },
                flag: { value: CARGO_FLAG },
        ...
        const decoded = types.PrepareDepositFuelResponse.decode(prepared.responsePayloadBuffer);
        return service.handleRequest(EXECUTE_DEPOSIT_FUEL_REQUEST, makeEnvelope(types.ExecuteDepositFuelRequest, {
            prepared_transaction_uuid: decoded.prepared_transaction_uuid,
            signature: VALID_SIGNATURE,
        }));
    };
    const executeWithdraw = (quantity) => {
        const prepared = service.handleRequest(PREPARE_WITHDRAW_FUEL_REQUEST, makeEnvelope(types.PrepareWithdrawFuelRequest, {
            network_node: { sequential: node.itemID },
            fuel_type: { sequential: FUEL_UNSTABLE },
            quantity,
            destination: {
        ...
        assert.equal(prepared.statusCode, 200, prepared.statusMessage);
        const decoded = types.PrepareWithdrawFuelResponse.decode(prepared.responsePayloadBuffer);
        return service.handleRequest(EXECUTE_WITHDRAW_FUEL_REQUEST, makeEnvelope(types.ExecuteWithdrawFuelRequest, {
            prepared_transaction_uuid: decoded.prepared_transaction_uuid,
            signature: VALID_SIGNATURE,
        }));
    };
    const findInventoryChange = (itemID) => {
        for (const notification of notifications) {
            if (notification[0] !== "OnItemsChanged")
                continue;
            const payload = notification[2];
            const row = payload?.[0]?.items?.find(entry => Number(entry?.fields?.itemID) === Number(itemID));
            if (row)
                return { row, changeDict: payload[1] };
        }
        return null;
    };
    try {
        ...
        assert.ok(removal.changeDict.entries.some(entry => Number(entry[0]) === 3 && Number(entry[1]) === container.itemID), "the removal must identify the stack's former container");
        notifications.length = 0;
        const withdrawn = executeWithdraw(250);
        assert.equal(withdrawn.statusCode, 200, withdrawn.statusMessage);
        const returnedStack = itemStore.listContainerItems(OWNER_ID, container.itemID, CARGO_FLAG)
        ...
        assert.ok(creation.changeDict.entries.some(entry => Number(entry[0]) === 3 && Number(entry[1]) === 0), "the new stack must be reported as entering the destination");
        notifications.length = 0;
        ...
        assert.ok(merge.changeDict.entries.some(entry => Number(entry[0]) === 9 && Number(entry[1]) === 250), "the stack update must include its exact previous quantity");
        notifications.length = 0;
        const partiallyDeposited = executeDeposit(returnedStack.itemID, 125);
        assert.equal(partiallyDeposited.statusCode, 200, partiallyDeposited.statusMessage);
        ...
        assert.ok(partialDeposit.changeDict.entries.some(entry => Number(entry[0]) === 9 && Number(entry[1]) === 500), "the partial deposit must include the exact pre-deposit quantity");
    }
    finally {
...
test("gateway service: unauthenticated and error mappings", () => {
    ...
    const noAuth = service.handleRequest(GET_FUEL_REQUEST, {
        payload: {
            value: Buffer.from(types.GetFuelRequest.encode(types.GetFuelRequest.create({
                network_node: { sequential: node.itemID },
            })).finish()),
        },
        authoritative_context: {},
    ...
    const missingNode = service.handleRequest(GET_FUEL_REQUEST, makeEnvelope(types.GetFuelRequest, {
    ...
    const staleExecute = service.handleRequest(EXECUTE_DEPOSIT_FUEL_REQUEST, makeEnvelope(types.ExecuteDepositFuelRequest, {
        prepared_transaction_uuid: uuidStringToBuffer("0f8fad5b-d9cb-469f-a165-70867728950e"),
        signature: VALID_SIGNATURE,
    }));
