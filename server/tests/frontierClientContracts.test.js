...
const { getFrontierCoreProtoTypes, } = require("../src/_secondary/express/gatewayServices/frontierCoreProto");
const { FRONTIER_TOKEN_DENOMINATION, createFrontierWalletGatewayService, } = require("../src/_secondary/express/gatewayServices/frontierWalletGatewayService");
const { createFrontierTokenExchangeGatewayService, } = require("../src/_secondary/express/gatewayServices/frontierTokenExchangeGatewayService");
...
test("Frontier legacy chat membership uses build 3450341 field names", () => {
    ...
    const decoded = types.chatGetMembershipResponse.toObject(types.chatGetMembershipResponse.decode(encoded), { longs: Number });
    assert.deepEqual(decoded.characters.map((entry) => entry.sequential), [140000005, 140000006]);
...
test("Frontier wallet gateway returns an EVE token amount", () => {
    ...
    const response = service.handleRequest("eve_public.wallet.api.GetBalanceRequest", requestEnvelope(140000005, request));
    const decoded = types.walletGetBalanceResponse.decode(response.responsePayloadBuffer);
...
test("Frontier token exchange returns a deterministic local rate", () => {
    ...
    const response = service.handleRequest("eve_public.token.exchange.api.GetExchangeRateRequest", requestEnvelope(140000005, request));
    const decoded = types.tokenGetExchangeRateResponse.toObject(types.tokenGetExchangeRateResponse.decode(response.responsePayloadBuffer), { longs: Number });
    assert.equal(response.statusCode, 200);
...
test("KISN exposes a stable nullable string to the character sheet", () => {
    ...
    assert.equal(service.Handle_get_serial_number([], { charid: 140000005 }), "140000005");
    assert.equal(service.Handle_get_serial_number([], {}), null);
});
//# sourceMappingURL=frontierClientContracts.test.js.map