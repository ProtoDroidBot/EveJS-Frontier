...
function runPendingWarpUntilReady(entity, maxTicks = 90) {
    ...
    for (let tick = 1; tick <= maxTicks; tick += 1) {
        ...
        const gate = evaluatePendingWarp(entity, entity.pendingWarp, requestedAtMs + (tick * 1000));
        gates.push(gate);
        if (gate.aligned) {
...
test("pending warp from rest accelerates through the speed gate over real time", () => {
    ...
    assert.ok(readyAtTick >= 3 && readyAtTick <= 30, `align completed at tick ${readyAtTick}, expected a physical duration`);
    assert.ok(magnitude(entity.velocity) > 0.75 * entity.maxVelocity, "gate requires genuine speed, not the commanded fraction");
});
test("pending warp from cruise turns the velocity vector gradually", () => {
    ...
    assert.ok(tickOneVelocity < 0.995, `tick 1 already aligned (${tickOneVelocity})`);
    assert.equal(gates[0].ready, false, "gate passed on the first tick");
    assert.notEqual(readyAtTick, null, "align gate never became ready");
    assert.ok(readyAtTick >= 2 && readyAtTick <= 60, `turn completed at tick ${readyAtTick}, expected a physical duration`);
    // Alignment progressed monotonically toward the warp vector.
    for (let index = 1; index < gates.length; index += 1) {
        assert.ok(gates[index].alignmentDot >= gates[index - 1].alignmentDot - 0.0001, `alignment regressed at tick ${index + 1}`);
    }
...
test("align gate rejects aligned-but-slow and fast-but-misaligned ships", () => {
    ...
    const slowGate = evaluatePendingWarp(alignedSlow, alignedSlow.pendingWarp, alignedSlow.pendingWarp.requestedAtMs + 1000);
    ...
    const misalignedGate = evaluatePendingWarp(fastMisaligned, fastMisaligned.pendingWarp, fastMisaligned.pendingWarp.requestedAtMs + 1000);
    ...
    const readyGate = evaluatePendingWarp(alignedFast, alignedFast.pendingWarp, alignedFast.pendingWarp.requestedAtMs + 1000);
...
test("align gate force-starts a stuck warp after the tick valve", () => {
    ...
    const forcedGate = evaluatePendingWarp(stuck, stuck.pendingWarp, stuck.pendingWarp.requestedAtMs + (181 * 1000));
    assert.equal(forcedGate.aligned, false);
    assert.equal(forcedGate.forced, true);
    assert.equal(forcedGate.ready, true);
});
//# sourceMappingURL=frontierWarpAlign.test.js.map