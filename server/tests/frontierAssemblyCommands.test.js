...
const { findItemById, getActiveShipItem, getItemMetadata, grantItemToCharacterLocation, removeInventoryItem, updateInventoryItem, } = require("../src/services/inventory/itemStore");
...
test("assembly help and catalog route through the shared slash dispatcher", () => {
    ...
    const help = executeChatCommand(buildSession(), "/assembly help", null, { emitChatFeedback: false, assemblyCommandOptions: dependencies.options });
    ...
    const hidden = executeFrontierAssemblyCommand(buildSession(), "types hidden turret", dependencies.options);
...
test("assembly mutations trust accountRole only and reject composed-role spoofing", () => {
    const dependencies = buildDependencies();
    const denied = executeFrontierAssemblyCommand(buildSession({
        accountRole: "0",
        role: MAX_ACCOUNT_ROLE | SESSION_BASE_ROLE_MASK,
    }), "spawn Heavy Storage ahead=250", dependencies.options);
    assert.equal(denied.success, false);
    assert.match(denied.message, /elevated GM account role/u);
    assert.equal(dependencies.calls.length, 0);
    assert.equal(deploymentRuntime.hasAssemblyAdminPrivileges({
        accountRole: "0",
...
test("assembly spawn resolves end-type names inside the Smart Assembly catalog", () => {
    const dependencies = buildDependencies();
    const result = executeFrontierAssemblyCommand(buildSession(), 'spawn "Heavy Storage" ahead=250 state=offline', dependencies.options);
    ...
    const hidden = executeFrontierAssemblyCommand(buildSession(), "spawn Heavy Turret here", dependencies.options);
    assert.equal(hidden.success, true);
...
test("assembly parser rejects unsafe IDs, unterminated names, and unconfirmed removal", () => {
    ...
    const unsafe = executeFrontierAssemblyCommand(buildSession(), "remove 9007199254740993 confirm", dependencies.options);
    assert.equal(unsafe.success, false);
    assert.equal(dependencies.calls.length, 0);
    const unconfirmed = executeFrontierAssemblyCommand(buildSession(), "remove 9988400004000", dependencies.options);
    assert.equal(unconfirmed.success, false);
    assert.equal(dependencies.calls.length, 0);
    const trailing = executeFrontierAssemblyCommand(buildSession(), "state 9988400004000 online trailing", dependencies.options);
    assert.equal(trailing.success, false);
    assert.equal(dependencies.calls.length, 0);
    const duplicateState = executeFrontierAssemblyCommand(buildSession(), "spawn Heavy Storage state=offline state=online", dependencies.options);
    assert.equal(duplicateState.success, false);
    assert.equal(dependencies.calls.length, 0);
    const trailingList = executeFrontierAssemblyCommand(buildSession(), "list all trailing", dependencies.options);
    assert.equal(trailingList.success, false);
    const trailingInfo = executeFrontierAssemblyCommand(buildSession(), "info 9988400004000 trailing", dependencies.options);
    assert.equal(trailingInfo.success, false);
    const confirmed = executeFrontierAssemblyCommand(buildSession(), "remove 9988400004000 confirm", dependencies.options);
...
test("admin runtime spawns hidden assemblies and refuses to delete their contents", () => {
    ...
    spaceRuntime.getEntity = (_session, itemID) => (Number(itemID) === Number(ship.itemID)
        ? {
            itemID: ship.itemID,
            direction: { x: 1, y: 0, z: 0 },
            position: { x: 100, y: 200, z: 300 },
    ...
    try {
        ...
        assert.equal(deploymentRuntime.adminCompleteConstruction(session, spawnedItemID).errorMsg, "CONSTRUCTION_ALREADY_COMPLETE");
        const online = deploymentRuntime.adminSetAssemblyState(buildSession({ characterID: 140000006 }), spawnedItemID, 2);
        ...
        assert.equal(deploymentRuntime.adminRemoveAssembly(session, spawnedItemID).errorMsg, "ASSEMBLY_MUST_BE_OFFLINE");
        assert.equal(deploymentRuntime.adminSetAssemblyState(session, spawnedItemID, 1).success, true);
        const shipBeforeBerthMarker = structuredClone(findItemById(ship.itemID));
        assert.equal(updateInventoryItem(ship.itemID, (current) => ({
            ...current,
        ...
        assert.equal(updateInventoryItem(ship.itemID, () => shipBeforeBerthMarker).success, true);
        const child = grantItemToCharacterLocation(session.characterID, spawnedItemID, 66, getItemMetadata(34), 10);
        ...
        childItemID = 0;
        assert.equal(deploymentRuntime.adminRemoveAssembly(session, spawnedItemID).success, true);
        assert.equal(findItemById(spawnedItemID), null);
        spawnedItemID = 0;
        const networkNode = deploymentRuntime.adminSpawnAssembly(session, 88092, { x: 700, y: 200, z: 300 }, { assemblyStatus: 1 });
        assert.equal(networkNode.success, true);
        networkNodeItemID = networkNode.data.item.itemID;
        assert.equal(networkNodeFuelRuntime.writeNetworkNodeFuelState(networkNodeItemID, {
            quantity: 25,
            typeID: 77818,
            updatedAtMs: Date.now(),
        }).success, true);
        const fueled = deploymentRuntime.adminRemoveAssembly(session, networkNodeItemID);
        ...
        assert.equal(deploymentRuntime.adminRemoveAssembly(session, networkNodeItemID).success, true);
...
test("admin Smart Gate link and unlink preserve reciprocal state", () => {
    ...
    const session = buildSession({ sendNotification() { } });
    try {
        const linked = deploymentRuntime.adminLinkSmartGates(session, sourceID, destinationID);
        assert.equal(linked.success, true);
        let sourceState = deploymentRuntime.readConstructionState(findItemById(sourceID));
        let destinationState = deploymentRuntime.readConstructionState(findItemById(destinationID));
        ...
        assert.equal(deploymentRuntime.adminRemoveAssembly(session, destinationID).errorMsg, "SMART_GATE_MUST_BE_UNLINKED");
        assert.equal(updateInventoryItem(destinationID, (current) => ({
            ...current,
            customInfo: deploymentRuntime._testing.writeConstructionState(current, {
                ...destinationState,
                destinationGateID: 0,
                targetSolarSystemID: 0,
            }),
        })).success, true);
        assert.equal(deploymentRuntime.adminRemoveAssembly(session, destinationID).errorMsg, "SMART_GATE_MUST_BE_UNLINKED", "an inbound one-sided link must still block removal");
        assert.equal(updateInventoryItem(destinationID, (current) => ({
            ...current,
            customInfo: deploymentRuntime._testing.writeConstructionState(current, destinationState),
        })).success, true);
        ...
        assert.equal(updateInventoryItem(sourceID, (current) => ({
            ...current,
            customInfo: deploymentRuntime._testing.writeConstructionState(current, {
                ...sourceState,
        ...
        assert.equal(deploymentRuntime.readConstructionState(findItemById(sourceID))
            .destinationGateID, 0);
    }
    finally {
...
//# sourceMappingURL=frontierAssemblyCommands.test.js.map