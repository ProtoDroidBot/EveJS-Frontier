...
const { installProcessLifecycleLogging, } = require(path.join(__dirname, "./src/utils/processLifecycle"));
const config = require(path.join(__dirname, "./src/config"));
const { installSharedOverviewMotd, } = require(path.join(__dirname, "./src/services/overview/overviewMotdBootstrap"));
const presenceReconciler = require(path.join(__dirname, "./src/services/_shared/presenceReconciler"));
// Service framework
const ServiceManager = require(path.join(__dirname, "./src/services/serviceManager"));
const { createRuntimeContext, } = require(path.join(__dirname, "./src/runtimeContext"));
const { createEvejsWebGatewayRuntime, } = require(path.join(__dirname, "./src/_secondary/express/evejsWebGatewayRuntime"));
const { loadSecondaryServices, } = require(path.join(__dirname, "./src/secondaryServiceLoader"));
// network
...
try {
    const { ensureCoreFixtures, } = require(path.join(__dirname, "./src/services/corporation/coreFixtureSeeder"));
    const fixtureSummary = ensureCoreFixtures();
    const seededCount = fixtureSummary.charactersCreated.length +
        (fixtureSummary.corporationCreated ? 1 : 0) +
        (fixtureSummary.allianceCreated ? 1 : 0);
    if (seededCount > 0) {
        log.info(`[CoreFixtures] Seeded missing fixtures: ${fixtureSummary.charactersCreated.length} character(s)` +
...
try {
    const calendarRuntime = require(path.join(__dirname, "./src/services/calendar/calendarRuntimeState"));
...
if (config.asteroidBeltStartupReset === true) {
    try {
        const miningRuntimeState = require(path.join(__dirname, "./src/services/mining/miningRuntimeState"));
        ...
        if (beltReset.entitiesCleared > 0) {
            log.info(`[BeltReset] Repopulated ${beltReset.entitiesCleared} mined belt asteroid(s) across ` +
...
if (config.wormholesEnabled === true) {
    try {
        ...
        const wormholeRuntime = require(path.join(__dirname, "./src/services/exploration/wormholes/wormholeRuntime"));
        ...
        if (seedResult && seedResult.success === true) {
            ...
            log.info(`[Wormholes] Startup ready: ${summary.activePairCount} pair(s) | static ${summary.staticPairCount} | random ${summary.randomPairCount} | systems ${summary.systemCount} | env ${summary.environmentSystemCount} | revealed ${summary.revealedExitCount} | hidden ${summary.hiddenExitCount} | ${seedDurationMs.toFixed(1)} ms`);
        }
        else {
            log.warn(`[Wormholes] Universe static seeding reported failure after ${seedDurationMs.toFixed(1)} ms`);
        }
        log.spacer();
    }
    catch (err) {
...
try {
    const { runStartupBillingMaintenance, } = require(path.join(__dirname, "./src/services/account/billingMaintenance"));
    ...
    if (Number(officeRental.processedCount) > 0 || officeRental.capped === true) {
        const actionSummary = Object.entries(actionCounts)
            ...
            .join(" ");
        const message = `[Billing] Startup office rental processing complete: ` +
