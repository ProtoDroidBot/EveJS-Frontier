...
function runPowerShell(command) {
    return spawnSync(POWERSHELL, ["-NoLogo", "-NoProfile", "-NonInteractive", "-Command", command], { encoding: "utf8" });
}
function createFrontierDiscoveryFixture(fixtureBase, build) {
    ...
    fs.writeFileSync(path.join(sourceRoot, "start.ini"), `[main]\nappname=FRONTIER\nbuild=${build}\nsync=${build}\nversion=20.04\n`);
...
function writeCopyStageMarker({ build, officialResFiles, sourceRoot, stageRoot, stagingBase, }) {
    const resFiles = path.join(stageRoot, "ResFiles");
    fs.mkdirSync(resFiles, { recursive: true });
    fs.writeFileSync(path.join(stageRoot, ".evejs-frontier-stage.json"), `${JSON.stringify({
...
test("launcher argument redaction never emits session credentials", {
    ...
    const modulePath = path.join(REPO_ROOT, "tools", "frontier-client", "FrontierWindows.Common.psm1");
    const secrets = [
        "/ssoToken=secret-sso",
...
test("stage ownership accepts only its exact build-numbered contained path", {
    ...
    const fixtureBase = fs.mkdtempSync(path.join(os.tmpdir(), "evejs frontier stage guards "));
...
test("patch dry run validates a marker without mutating it", {
    ...
    const fixtureBase = fs.mkdtempSync(path.join(os.tmpdir(), "evejs frontier patch dry run "));
    t.after(() => fs.rmSync(fixtureBase, { force: true, recursive: true }));
    const build = 9994409;
    const stageRoot = path.join(fixtureBase, String(build));
    fs.mkdirSync(stageRoot);
...
test("patch check rejects unsigned exefile before the Python transaction", {
    ...
    const fixtureBase = fs.mkdtempSync(path.join(os.tmpdir(), "evejs frontier trust preflight "));
    t.after(() => fs.rmSync(fixtureBase, { force: true, recursive: true }));
    const build = 9994411;
    const stageRoot = path.join(fixtureBase, String(build));
...
test("stage cleanup refuses a retargeted ResFiles junction", {
    ...
    const fixtureBase = fs.mkdtempSync(path.join(os.tmpdir(), "evejs frontier cleanup guard "));
    t.after(() => fs.rmSync(fixtureBase, { force: true, recursive: true }));
    const build = 9994412;
    const retailRoot = path.join(fixtureBase, "retail");
    const sourceRoot = path.join(retailRoot, "stillness");
...
test("stage clean can replace a verified stage from another source client", {
    ...
    const fixtureBase = fs.mkdtempSync(path.join(os.tmpdir(), "evejs frontier source switch "));
    t.after(() => fs.rmSync(fixtureBase, { force: true, recursive: true }));
    const build = 9994416;
    const sourceA = createFrontierDiscoveryFixture(path.join(fixtureBase, "source A"), build);
    const sourceB = createFrontierDiscoveryFixture(path.join(fixtureBase, "source B"), build);
    const stagingBase = path.join(fixtureBase, "staged-client");
    const stageRoot = path.join(stagingBase, String(build));
    ...
    const marker = JSON.parse(fs.readFileSync(path.join(stageRoot, ".evejs-frontier-stage.json"), "utf8"));
    assert.equal(path.resolve(marker.sourceRoot), path.resolve(sourceB.sourceRoot));
    assert.equal(marker.resFiles.mode, "junction");
    assert.equal(path.resolve(marker.resFiles.target), path.resolve(sourceB.officialResFiles));
    assert.equal(fs.realpathSync(path.join(stageRoot, "ResFiles")), fs.realpathSync(sourceB.officialResFiles));
});
test("stage preflight rejects a staging-base ancestor junction before writing", {
    skip: !canRunPowerShell,
}, (t) => {
    const fixtureBase = fs.mkdtempSync(path.join(os.tmpdir(), "evejs frontier base chain guard "));
    t.after(() => fs.rmSync(fixtureBase, { force: true, recursive: true }));
    const build = 9994413;
    ...
    assert.notEqual(result.status, 0);
    assert.match(`${result.stderr}\n${result.stdout}`, /Refusing a reparse-point staging path component/);
...
test("existing stage preflight rejects every nested reparse except ResFiles", {
    ...
    const fixtureBase = fs.mkdtempSync(path.join(os.tmpdir(), "evejs frontier nested reparse guard "));
    t.after(() => fs.rmSync(fixtureBase, { force: true, recursive: true }));
    const build = 9994414;
    const { officialResFiles, sourceRoot } = createFrontierDiscoveryFixture(fixtureBase, build);
    const stagingBase = path.join(fixtureBase, "staged-client");
    const stageRoot = path.join(stagingBase, String(build));
...
test("stage cleanup refuses a live exact exefile by PID marker and path scan", {
    ...
    const fixtureBase = fs.mkdtempSync(path.join(os.tmpdir(), "evejs frontier live cleanup guard "));
    ...
    const pidMarker = path.join(localAppData, "EveJS-Frontier", "windows", "logs", String(build), "client.pid.json");
    ...
    const ticks = runPowerShell(`(Get-Process -Id ${clientProcess.pid}).StartTime.ToUniversalTime().Ticks`);
    ...
    assert.match(`${withMarker.stderr}\n${withMarker.stdout}`, /PID marker reports the staged Frontier client is running/);
    ...
    assert.match(`${withoutMarker.stderr}\n${withoutMarker.stdout}`, /exact staged Frontier executable is running/);
    assert.equal(fs.readFileSync(sentinel, "utf8"), "live stage");
});
test("private launcher-session writer applies the SID-only ACL before publishing", {
    skip: !canRunPowerShell,
}, (t) => {
    const fixtureBase = fs.mkdtempSync(path.join(os.tmpdir(), "evejs frontier private session "));
    t.after(() => fs.rmSync(fixtureBase, { force: true, recursive: true }));
    ...
    assert.deepEqual(fs.readdirSync(fixtureBase).filter((name) => name.includes(".tmp-")), []);
});
test("StopFrontier accepts the explicit custom stage recorded by a client PID marker", {
    skip: !canRunPowerShell,
}, (t) => {
    const fixtureBase = fs.mkdtempSync(path.join(os.tmpdir(), "evejs frontier custom stop "));
    t.after(() => fs.rmSync(fixtureBase, { force: true, recursive: true }));
    const localAppData = path.join(fixtureBase, "local app data");
    const build = 9994410;
    const stageRoot = path.join(fixtureBase, "custom stages", String(build));
    const exefile = path.join(stageRoot, "bin64", "exefile.exe");
    const logRoot = path.join(localAppData, "EveJS-Frontier", "windows", "logs", String(build));
    fs.mkdirSync(path.dirname(exefile), { recursive: true });
...
//# sourceMappingURL=frontier-windows-client.test.js.map