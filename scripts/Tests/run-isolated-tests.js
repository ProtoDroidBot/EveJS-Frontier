...
if (testFiles.length === 0) {
    console.error("Usage: node scripts/Tests/run-isolated-tests.js server/tests/<file>.test.js [...]");
    process.exit(2);
}
async function main() {
    const store = await createIsolatedGameStore({
        seedFrontierFixtures: process.env.EVEJS_TEST_FRONTIER_FIXTURES === "1",
    });
    console.log(`[isolated-tests] store=${store.storeRoot} baseline=${store.baselineRoot}`);
    let exitCode = 1;
    try {
        ...
        try {
            ...
            console.warn(`[isolated-tests] failed to remove ${store.storeRoot}: ${error.message}`);
        }
    }
    return exitCode;
}
...
//# sourceMappingURL=run-isolated-tests.js.map