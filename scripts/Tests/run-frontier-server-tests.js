...
function resolveBuildInputs(build, repoRoot = REPO_ROOT) {
    const runtimeRoot = path.join(repoRoot, "_local", "frontier-runtime", build, "gameStore");
...
function main(argv = process.argv.slice(2)) {
    ...
    if (!fs.existsSync(path.join(runtimeRoot, "data"))) {
        throw new Error(`Frontier runtime is missing for build ${options.build}: ${runtimeRoot}`);
    }
    if (!fs.existsSync(staticRoot)) {
        throw new Error(`Frontier static snapshot is missing for build ${options.build}: ${staticRoot}`);
    }
    const testFiles = listFrontierServerTests(REPO_ROOT);
    if (testFiles.length === 0) {
        throw new Error("No server/tests/frontier*.test.js files were found");
    }
    console.log(`[frontier-server-tests] build=${options.build} files=${testFiles.length}`);
    const isolatedRunner = path.join(REPO_ROOT, "scripts", "Tests", "run-isolated-tests.js");
    const result = spawnSync(process.execPath, [isolatedRunner, "--test-concurrency=1", ...testFiles], {
        cwd: REPO_ROOT,
        stdio: "inherit",
        env: {
            ...process.env,
...
//# sourceMappingURL=run-frontier-server-tests.js.map