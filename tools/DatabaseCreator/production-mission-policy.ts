...
const GOLDEN_SECURITY_MISSIONS = productionMissionPolicy.goldenSecurityMissions.map(
  ({ missionID, dungeonID, agentLevel, templateID, requiresMissionSetup }) => ({
...
function rebuildMissionIndexes(payload) {
  ...
  for (const [missionID, record] of Object.entries<any>(payload.missionsByID || {})) {
  ...
  for (const [agentID, missionIDs] of Object.entries<any>(previousIndexes.agentIDToMissionIDs || {})) {
  ...
  for (const [name, missionID] of Object.entries<any>(previousIndexes.preferredMissionIDs || {})) {
...
function rebuildMissionCounts(payload) {
  const missions = Object.values<any>(payload.missionsByID || {});
  return {
    ...(payload.counts || {}),
...
function sanitizeMissionAuthority(payload) {
  ...
  for (const [missionID, record] of Object.entries<any>(missionsByID)) {
...
function filterDungeonIndex(index, retainedTemplateIDs) {
  ...
  for (const [key, value] of Object.entries<any>(index || {})) {
...
function sanitizeDungeonAuthority(payload) {
  ...
  for (const [indexName, index] of Object.entries<any>(payload.indexes || {})) {
  ...
  if (payload.coverage && payload.coverage.totalTemplatesByFamily) {
    const retainedMissionTemplateCount = Object.values<any>(templatesByID).filter((template) => (
  ...
  const sceneProfileCount = Object.values<any>(templatesByID).filter((template) => (
...
function sanitizeAgentAuthority(payload) {
  ...
  const missionPoolsByKindAndLevel: Record<string, any> = {};
  for (const [poolKey, templateIDs] of Object.entries<any>(payload.missionPoolsByKindAndLevel || {})) {
  ...
  for (const agent of Object.values<any>(payload.agentsByID || {})) {
  ...
  const retainedTemplateIDs = new Set([
    ...Object.values<any>(missionPoolsByKindAndLevel).flat(),
    ...Object.values<any>(payload.agentsByID || {}).flatMap((agent) => (
...
function policyViolationCount(tableName, payload) {
  if (tableName === "missionAuthority") {
    const prohibitedMissionCount = Object.entries<any>(payload.missionsByID || {}).filter(([missionID, record]) => (
  ...
  if (tableName === "dungeonAuthority") {
    const templatesByID = payload.templatesByID || {};
    const prohibitedTemplateCount = Object.entries<any>(templatesByID)
      .filter(([templateID, template]) => isEveSurvivalTemplateRecord(templateID, template)).length;
    const goldenTemplateIDs = new Set(
      GOLDEN_SECURITY_MISSIONS.map(({ templateID }) => templateID.toLowerCase()),
    );
    ...
    const malformedPresentationCount = Object.entries<any>(templatesByID)
      .filter(([templateID, template]) => (
        !goldenTemplateIDs.has(templateID.toLowerCase()) &&
    ...
    const brokenGoldenTemplateCount = GOLDEN_SECURITY_MISSIONS.filter(({
      ...
      const allowedMissionIDs = goldenMissionIDsByTemplateID.get(
      ...
      return !template || template.source !== "client" ||
        ...
        !isValidMissionPresentation(template.missionPresentation) ||
        !GOLDEN_DUNGEON_SOURCE_KEYS.has(normalizedText(
          template.populationHints && template.populationHints.source,
        ).toLowerCase()) ||
        !allowedMissionIDs.has(
          toInt(template.populationHints && template.populationHints.missionID, 0),
        ) ||
        (requiresMissionSetup && (
          !isValidMissionSetup(missionSetup) ||
  ...
  if (tableName === "agentAuthority") {
    const poolViolations = Object.values<any>(payload.missionPoolsByKindAndLevel || {}).flat()
      .filter(isRetiredMissionTemplateIdentifier).length;
    const agentViolations = Object.values<any>(payload.agentsByID || {}).reduce((total, agent) => (
