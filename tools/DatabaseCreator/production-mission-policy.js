...
const { isDisabledMissionIdentifier, isDisabledMissionSourceURL, isDisabledMissionTemplateIdentifier, isGeneratedMissionIdentifier, isRetiredMissionTemplateIdentifier, isValidMissionPresentation, isValidMissionRewardPresentation, isValidMissionSetup, productionMissionPolicy, } = require(path.join(__dirname, "..", "..", "server", "src", "config", "productionMissionPolicy"));
...
const GOLDEN_DUNGEON_SOURCE_KEYS = new Set([
    "golden_log_combat_mission",
    "tq_mission_log_decoder",
]);
...
const BANNED_MISSION_IDS = new Set(productionMissionPolicy.disabledMissions.map(({ missionID }) => String(missionID)));
...
const BANNED_SOURCE_MISSION_IDS = new Set(productionMissionPolicy.disabledMissions.flatMap(({ missionID, templateIDs }) => [
...
const GOLDEN_SECURITY_MISSIONS = productionMissionPolicy.goldenSecurityMissions.map(({ missionID, dungeonID, agentLevel, templateID, requiresMissionSetup }) => ({
    missionID: String(missionID),
    dungeonID,
    agentLevel,
    templateID,
    requiresMissionSetup,
}));
...
function isGeneratedEveSurvivalMission(missionID, record) {
    ...
    return (isEveSurvivalSourceName(record && record.generatedFromSource) ||
...
function isBannedMissionRecord(missionID, record) {
    return isDisabledMissionIdentifier(missionID) ||
        ...
        BANNED_SOURCE_MISSION_IDS.has(normalizedText(record && record.sourceMissionID).toLowerCase()) ||
        [
...
function retainedMissionIDList(values, retainedMissionIDs) {
    return [...new Set((Array.isArray(values) ? values : []).filter((missionID) => (retainedMissionIDs.has(String(missionID)))))].sort(compareMissionIDs);
}
function rebuildMissionIndexes(payload) {
    ...
    for (const [agentID, missionIDs] of Object.entries(previousIndexes.agentIDToMissionIDs || {})) {
        ...
        if (retained.length > 0)
            indexes.agentIDToMissionIDs[agentID] = retained;
    }
...
function rebuildMissionCounts(payload) {
    ...
    return {
        ...
        localizedMissionMessageCount: missions.reduce((total, record) => (total + Object.keys(record && record.localizedMessages || {}).length), 0),
        generatedEveSurvivalMissionCount: missions.filter((record) => (normalizedText(record && record.generatedFromSource).toLowerCase() ===
            GENERATED_EVE_SURVIVAL_SOURCE)).length,
    };
}
...
function filterDungeonIndex(index, retainedTemplateIDs) {
    ...
    for (const [key, value] of Object.entries(index || {})) {
        ...
        if (typeof value === "string") {
            if (retainedTemplateIDs.has(value))
                filtered[key] = value;
            else
...
function setCount(payload, countName, value) {
    ...
    if (payload.counts[countName] === value)
        return 0;
...
function sanitizeDungeonAuthority(payload) {
    ...
    for (const [templateID, template] of Object.entries(templatesByID)) {
        if (!isEveSurvivalTemplateRecord(templateID, template))
            continue;
        if (BANNED_DUNGEON_TEMPLATE_IDS.has(normalizedText(templateID).toLowerCase()) ||
            BANNED_DUNGEON_TEMPLATE_IDS.has(normalizedText(template && template.templateID).toLowerCase())) {
    ...
    if (payload.coverage && payload.coverage.totalTemplatesByFamily) {
        const retainedMissionTemplateCount = Object.values(templatesByID).filter((template) => (normalizedText(template && template.siteFamily) === "mission")).length;
        ...
        else if (Object.prototype.hasOwnProperty.call(payload.coverage.totalTemplatesByFamily, "mission")) {
            delete payload.coverage.totalTemplatesByFamily.mission;
            metadataChangeCount += 1;
        }
    }
    const sceneProfileCount = Object.values(templatesByID).filter((template) => (template && template.siteSceneProfile != null)).length;
...
function sanitizeAgentAuthority(payload) {
    ...
    for (const [poolKey, templateIDs] of Object.entries(payload.missionPoolsByKindAndLevel || {})) {
        const retained = (Array.isArray(templateIDs) ? templateIDs : []).filter((templateID) => (!isRetiredMissionTemplateIdentifier(templateID)));
        removedPoolReferenceCount += (Array.isArray(templateIDs) ? templateIDs.length : 0) - retained.length;
        if (retained.length > 0)
            missionPoolsByKindAndLevel[poolKey] = retained;
    }
    ...
    for (const agent of Object.values(payload.agentsByID || {})) {
        ...
        const retained = previous.filter((templateID) => !isRetiredMissionTemplateIdentifier(templateID));
    ...
    const retainedTemplateIDs = new Set([
        ...
        ...Object.values(payload.agentsByID || {}).flatMap((agent) => (Array.isArray(agent && agent.missionTemplateIDs) ? agent.missionTemplateIDs : [])),
    ]);
    let metadataChangeCount = 0;
    metadataChangeCount += setCount(payload, "missionPoolCount", Object.keys(missionPoolsByKindAndLevel).length);
...
function sanitizeAuthorityTable(tableName, payload) {
    if (tableName === "missionAuthority")
        return sanitizeMissionAuthority(payload);
    if (tableName === "dungeonAuthority")
        return sanitizeDungeonAuthority(payload);
    if (tableName === "agentAuthority")
        return sanitizeAgentAuthority(payload);
    return null;
}
function policyViolationCount(tableName, payload) {
    if (tableName === "missionAuthority") {
        const prohibitedMissionCount = Object.entries(payload.missionsByID || {}).filter(([missionID, record]) => (isBannedMissionRecord(missionID, record) ||
            isGeneratedEveSurvivalMission(missionID, record))).length;
    ...
    if (tableName === "dungeonAuthority") {
        const templatesByID = payload.templatesByID || {};
        const prohibitedTemplateCount = Object.entries(templatesByID)
            .filter(([templateID, template]) => isEveSurvivalTemplateRecord(templateID, template)).length;
        const goldenTemplateIDs = new Set(GOLDEN_SECURITY_MISSIONS.map(({ templateID }) => templateID.toLowerCase()));
        const goldenMissionIDsByTemplateID = new Map();
        for (const { missionID, templateID } of GOLDEN_SECURITY_MISSIONS) {
            const templateKey = templateID.toLowerCase();
            if (!goldenMissionIDsByTemplateID.has(templateKey)) {
                goldenMissionIDsByTemplateID.set(templateKey, new Set());
            }
            goldenMissionIDsByTemplateID.get(templateKey).add(toInt(missionID, 0));
        }
        const malformedPresentationCount = Object.entries(templatesByID)
            .filter(([templateID, template]) => (!goldenTemplateIDs.has(templateID.toLowerCase()) &&
            Object.prototype.hasOwnProperty.call(template || {}, "missionPresentation") &&
            !isValidMissionPresentation(template.missionPresentation))).length;
        const brokenGoldenTemplateCount = GOLDEN_SECURITY_MISSIONS.filter(({ missionID, dungeonID, templateID, requiresMissionSetup, }) => {
            const template = payload.templatesByID && payload.templatesByID[templateID];
            const allowedMissionIDs = goldenMissionIDsByTemplateID.get(templateID.toLowerCase()) || new Set();
            const missionSetup = template && template.missionSetupByMissionID &&
                template.missionSetupByMissionID[String(missionID)];
            return !template || template.source !== "client" ||
                toInt(template.sourceDungeonID, 0) !== dungeonID ||
                !isValidMissionPresentation(template.missionPresentation) ||
                !GOLDEN_DUNGEON_SOURCE_KEYS.has(normalizedText(template.populationHints && template.populationHints.source).toLowerCase()) ||
                !allowedMissionIDs.has(toInt(template.populationHints && template.populationHints.missionID, 0)) ||
                (requiresMissionSetup && (!isValidMissionSetup(missionSetup) ||
                    !isValidMissionRewardPresentation(missionSetup && missionSetup.reward)));
        }).length;
        return prohibitedTemplateCount + brokenGoldenTemplateCount +
            malformedPresentationCount;
    }
    if (tableName === "agentAuthority") {
        ...
        const agentViolations = Object.values(payload.agentsByID || {}).reduce((total, agent) => (total + (Array.isArray(agent && agent.missionTemplateIDs)
...
//# sourceMappingURL=production-mission-policy.js.map