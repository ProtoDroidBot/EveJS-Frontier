...
const { sanitizeAuthorityTable, policyViolationCount, } = require("./production-mission-policy");
...
function parseArgs(argv) {
    ...
    for (const argument of argv) {
        if (argument === "--apply")
            options.apply = true;
        else if (argument === "--check" || argument === "--dry-run")
            options.apply = false;
...
function reportChangeCount(tableName, report) {
    if (tableName === "missionAuthority") {
        return (report.removedGeneratedMissionCount +
    ...
    if (tableName === "dungeonAuthority") {
        return (report.removedEveSurvivalTemplateCount +
            report.removedIndexReferenceCount +
            report.metadataChangeCount);
    }
    return (report.removedPoolReferenceCount +
...
function run(options) {
    ...
    for (const root of AUTHORITY_ROOTS) {
        ...
        for (const tableName of TABLE_NAMES) {
            ...
            if (options.apply && changeCount > 0)
...
//# sourceMappingURL=enforce-production-mission-policy.js.map