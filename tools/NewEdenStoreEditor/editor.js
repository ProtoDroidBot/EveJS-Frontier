...
const { getEditorSnapshot, getStoreConfig, saveEditorAuthority, } = require(path.join(repoRoot, "server/src/services/newEdenStore/storeState"));
const app = express();
...
function buildStoreConfigPayload() {
    ...
    const definitions = config.getConfigDefinitions().filter((entry) => STORE_CONFIG_KEYS.includes(entry.key));
    return {
...
function sanitizeStoreConfigInput(rawValues = {}) {
    ...
    return Object.fromEntries(Object.entries(source).filter(([key]) => STORE_CONFIG_KEYS.includes(key)));
}
function buildBootstrapPayload() {
    ...
    return {
        ...
        summary: {
            ...
            fastCheckoutOffers: (((storeSnapshot.authority || {}).fastCheckout || {}).offers || []).length,
            quickPayTokens: Object.keys(((((storeSnapshot.authority || {}).fastCheckout || {}).tokensByID) || {})).length,
            runtimeAccounts: Object.keys((storeSnapshot.runtime && storeSnapshot.runtime.accounts) || {}).length,
            completedPurchases: Object.keys((storeSnapshot.runtime && storeSnapshot.runtime.completedPurchases) || {}).length,
        },
    };
}
...
app.post("/api/config", (req, res) => {
    try {
        const values = sanitizeStoreConfigInput(req.body && typeof req.body === "object" ? req.body.values : {});
...
app.listen(port, () => {
    console.log(`[NewEdenStoreEditor] listening on http://127.0.0.1:${port}`);
});
