...
function readJsonIfExists(filePath, fallback: Record<string, any> = {}) {
...
function buildSeedAuthority() {
  ...
  const itemTypesByName = new Map<string, SeedItemType>(
    itemTypes.map((entry) => [String((entry && entry.name) || ""), entry]),
  );
  const itemTypesByID = new Map<number, SeedItemType>(
    itemTypes
  ...
  const legacyOffers = [];
  const publicOffers: Record<string, any> = {};
