...
const authorityPath = path.join(repoRoot, "server/src/gameStore/data/newEdenStore/data.json");
const itemTypesPath = path.join(repoRoot, "server/src/gameStore/data/itemTypes/data.json");
const itemIconsPath = path.join(repoRoot, "server/src/gameStore/data/itemIcons/data.json");
...
const GENERIC_SHIP_IMAGE_URL = "res:/UI/Texture/Classes/Skills/skillGroups/spaceshipCmd.png";
...
function buildSeedAuthority() {
    ...
    const itemTypesByName = new Map(itemTypes.map((entry) => [String((entry && entry.name) || ""), entry]));
    const itemTypesByID = new Map(itemTypes
    ...
    function resolveImageUrlFromFulfillment(fulfillment) {
        ...
        if (kind === "bundle") {
            ...
            for (const grantKind of ["item", "grant_plex", "omega", "mct", "skill_points"]) {
                const grant = grants.find((entry) => entry && String(entry.kind || "").trim() === grantKind);
    ...
    function createLegacyOffer(definition) {
        const { storeOfferID, name, description, price, categoryID, fulfillment, imageUrl = null, label = null, tags = [], preview = null, } = definition;
    ...
    function createCashOffer(definition) {
        const { storeOfferID, name, description, usdCents, tags = [], previewTheme, fulfillment, fastCheckoutQuantity = null, label = null, sourceUrl, } = definition;
        const preview = buildPreview(previewTheme, name, description);
        const resolvedImageUrl = definition.imageUrl || resolveImageUrlFromFulfillment(fulfillment);
        registerPublicOffer({
            storeOfferID,
            name,
            description,
    ...
    return {
        ...
        config: {
            ...
            editorPort: existingAuthority.config && existingAuthority.config.editorPort
        ...
        fastCheckout: {
            ...
            tokensByID: existingAuthority.fastCheckout && existingAuthority.fastCheckout.tokensByID
...
function main() {
    ...
    const summary = {
        ...
        legacyOfferCount: ((((nextAuthority.stores || {})["4"] || {}).offers) || []).length,
        fastCheckoutOfferCount: (((nextAuthority.fastCheckout || {}).offers) || []).length,
        outputPath: authorityPath,
    };
    process.stdout.write(`Seeded New Eden Store authority with ${summary.publicOfferCount} public offers, ` +
...
//# sourceMappingURL=build_seed_authority.js.map