...
const state: { bootstrap: BootstrapPayload | null } = {
  bootstrap: null,
};

const summaryGrid = document.getElementById("summaryGrid");
const configForm = document.getElementById("configForm");
const authorityEditor = document.getElementById("authorityEditor") as HTMLTextAreaElement;
const runtimeEditor = document.getElementById("runtimeEditor") as HTMLTextAreaElement;
const statusBar = document.getElementById("statusBar");
const reloadButton = document.getElementById("reloadButton");
const saveAllButton = document.getElementById("saveAllButton");
const saveConfigButton = document.getElementById("saveConfigButton");

function setStatus(message: string, kind = "info") {
  statusBar.textContent = message;
  statusBar.dataset.kind = kind;
}

function buildSummaryCard(label: string, value: number) {
...
function renderSummary(summary: StoreSummary) {
...
function createConfigControl(entry: ConfigEntry) {
  ...
  if (entry.valueType === "boolean") {
    ...
    input.value = JSON.stringify(entry.currentValue, null, 2);
  } else {
    input = document.createElement("input");
    input.type = "text";
    input.value = String(entry.currentValue ?? "");
...
function renderConfig(configEntries: ConfigEntry[]) {
...
function renderEditors(storeSnapshot: StoreSnapshot) {
...
async function fetchJson(url: string, options: RequestInit = {}): Promise<BootstrapPayload> {
...
function readConfigValues() {
  ...
  for (const input of configForm.querySelectorAll<ConfigControl>("[data-key]")) {
