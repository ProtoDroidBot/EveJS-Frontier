...
function parseArgs(argv = process.argv.slice(2)) {
  const options: { snapshot: string | null; } & { help?: boolean } = { snapshot: null };
...
async function readJsonl(snapshot, fileName, onRecord: (record: Record<string, any>) => void = () => {}) {
...
async function validateSnapshot(snapshot) {
  ...
  async function load(fileName: string, onRecord?: (record: Record<string, any>) => void) {
  ...
  const creationTemplates = await load("creationTemplates.jsonl", (row) => {
    ...
    for (const [partID, part] of Object.entries<any>(row.parts)) {
      assert(
  ...
  for (const [fileName, metadata] of Object.entries<any>(manifest.outputs)) {
