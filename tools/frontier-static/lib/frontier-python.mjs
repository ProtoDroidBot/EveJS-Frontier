...
function windowsPythonCandidates() {
    const candidates = [
        ...
        {
            command: path.join(REPO_ROOT, "_local", "frontier-python312", "Scripts", "python.exe"),
            prefixArgs: [],
...
function probePortablePython(buildRoot, requiredImports) {
    const environment = {
        ...process.env,
...
function probeWinePython(buildRoot, requiredImports) {
...
function compileWindowsRunner(buildRoot, toolsRoot) {
    ...
    if (needsBuild) {
        ...
        if (!compiler) {
            throw new Error("No usable external Python 3.12 was found and the embedded-Python " +
...
function resolveFrontierPython(buildRoot, toolsRoot, { platform = process.platform, requiredImports = ["typesLoader"], allowPurePython = false, } = {}) {
    if (platform === "win32") {
