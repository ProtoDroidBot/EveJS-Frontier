...
function validateClientLayout(layout) {
    ...
    if (executableCandidates.length !== 1) {
        throw new Error(`Expected exactly one Frontier executable under ${path.join(buildRoot, "bin64")}`);
    }
    ...
    if (blueCandidates.length !== 1) {
        throw new Error(`Expected exactly one Frontier native blue binary (${blueNames.join(" or ")}) ` +
...
function enumerateLayoutCandidates(clientRoot) {
    ...
    add(path.join(root, "Contents", "Resources", "build"), null, path.basename(path.dirname(root)), root);
    ...
    if (isDirectory(sharedCacheRoot)) {
        for (const entry of fs.readdirSync(sharedCacheRoot, { withFileTypes: true })) {
            ...
            add(path.join(sharedCacheRoot, entry.name, "EVE.app", "Contents", "Resources", "build"), sharedCacheRoot, entry.name, path.basename(root).toLowerCase() === "sharedcache" ? path.dirname(root) : root);
            // Some launcher caches contain a direct Windows channel beside ResFiles.
            add(path.join(sharedCacheRoot, entry.name), sharedCacheRoot, entry.name, path.basename(root).toLowerCase() === "sharedcache" ? path.dirname(root) : root);
        }
    }
...
function likelyClientRoots({ platform = process.platform, env = process.env, homeDirectory = os.homedir(), } = {}) {
...
function selectBuild(candidates, requestedBuild = null) {
    ...
    const matches = candidates.filter((candidate) => Number(candidate.build) === Number(selectedBuild));
    if (matches.length === 0) {
        const available = [...new Set(candidates.map((candidate) => candidate.build))]
            .sort((left, right) => right - left)
            .join(", ");
        throw new Error(`Frontier build ${selectedBuild} is not installed. Available builds: ${available}`);
    }
    if (matches.length > 1) {
        throw new Error(`Frontier build ${selectedBuild} is ambiguous across equally ranked installs: ` +
...
export { DISCOVERY_FORMAT, REQUIRED_BUILD_FILES, discoverFrontierClients, enumerateLayoutCandidates, likelyClientRoots, publicClientRecord, selectBuild, validateClientLayout, };
//# sourceMappingURL=frontier-client-discovery.mjs.map