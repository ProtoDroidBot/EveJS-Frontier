...
export type FrontierClient = ReturnType<typeof validateClientLayout>;
...
function uniquePaths(candidates: (string | null | undefined)[]): string[] {
...
function parseNumericIniValue(value: unknown, label: string, { required = false }: { required?: boolean } = {}): number | null {
...
function findCacheRoot(buildRoot: string, preferredCacheRoot: string | null = null): string | null {
  const candidates = [];
...
function describeLayoutRoot(cacheRoot: string): string {
  return path.basename(cacheRoot).toLowerCase() === "sharedcache"
    ? path.dirname(cacheRoot)
    : cacheRoot;
}

function validateClientLayout(layout: ClientLayout) {
  ...
  const files: Record<string, string> = { startIni: startIniPath };
...
function enumerateLayoutCandidates(clientRoot: string): ClientLayout[] {
  const root = canonicalPath(clientRoot);
  if (!isDirectory(root)) {
    return [];
  }

  const layouts: ClientLayout[] = [];
  const seen = new Set<string>();
  const add = (buildRoot: string, cacheRoot: string | null, channel: string, logicalRoot = root) => {
...
function discoverFrontierClients({ clientRoot = null, roots = null }: ClientDiscoveryOptions = {}): FrontierClient[] {
  const searchRoots = clientRoot
    ? [path.resolve(clientRoot)]
    : (roots ? uniquePaths(roots) : likelyClientRoots());
  const candidates: FrontierClient[] = [];
  const errors: string[] = [];
  const seenBuildRoots = new Set<string>();
...
function selectBuild<T extends { build: number; buildRoot?: string }>(candidates: T[], requestedBuild: number | null = null): T {
...
function publicClientRecord(candidate: FrontierClient) {
  return {
    format: DISCOVERY_FORMAT,
