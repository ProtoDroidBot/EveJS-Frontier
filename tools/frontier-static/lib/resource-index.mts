...
export function parseResourceIndex(text: string): Map<string, ResourceIndexEntry> {
...
export function readResourceIndex(indexPath: string) {
  return parseResourceIndex(fs.readFileSync(indexPath, "utf8"));
}

export function resolveIndexedResource(entries: Map<string, ResourceIndexEntry>, logicalPath: string, resFilesRoot: string) {
