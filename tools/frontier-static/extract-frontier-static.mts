...
function parseArgs(argv = process.argv.slice(2)) {
  const options: { build: number | null; clientRoot: string | null; dryRun: boolean; force: boolean; outDir: string | null; } & { help?: boolean } = {
    build: null,
    clientRoot: null,
    dryRun: false,
    force: false,
...
function commandResult(command, args, options: Record<string, any> = {}) {
...
function outputMetadata(outDir, outputCounts) {
  ...
  for (const [fileName, count] of Object.entries<any>(outputCounts).sort()) {
