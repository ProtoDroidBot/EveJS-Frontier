...
import { discoverFrontierClients, publicClientRecord, selectBuild, } from "./lib/frontier-client-discovery.mjs";
function usage() {
    return [
        "Usage:",
...
function main(argv = process.argv.slice(2)) {
    ...
    if (options.json) {
        ...
        console.log(`[frontier-client] build=${record.metadata.build} sync=${record.metadata.sync ?? "unknown"} ` +
            `version=${record.metadata.version ?? "unknown"} channel=${record.channel}`);
        console.log(`[frontier-client] branch=${record.metadata.branch ?? "unknown"} ` +
        ...
        console.log(`[frontier-client] native-blue=${record.nativeBlueName} resfiles=${record.resFilesRoot}`);
    }
    return record;
}
if (process.argv[1] &&
...
//# sourceMappingURL=discover-frontier-client.mjs.map