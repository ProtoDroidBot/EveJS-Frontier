...
import { parseResourceIndex, resolveIndexedResource, } from "../lib/resource-index.mjs";
test("parseResourceIndex reads a Frontier index row", () => {
    const entries = parseResourceIndex("res:/staticdata/types.fsdbinary,3c/aabb_ccdd,source_hash,100,4\n");
...
test("parseResourceIndex rejects malformed and duplicate rows", () => {
    assert.throws(() => parseResourceIndex("res:/one,aa/cache,hash,1\n"), /expected 5 fields/);
    assert.throws(() => parseResourceIndex("res:/one,aa/aabb,hash,1,1\nres:/one,bb/ccdd,hash,1,1\n"), /Duplicate resource index entry/);
    assert.throws(() => parseResourceIndex("res:/one,../escape,hash,1,1\n"), /invalid cache path/);
});
...
//# sourceMappingURL=resource-index.test.mjs.map