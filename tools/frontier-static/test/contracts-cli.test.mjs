...
test("contract exporter recognizes public protobuf bytecode", () => {
    assert.equal(isPublicProtoMember("eveProto/generated/eve_public/chat/local_pb2.pyc"), true);
    assert.equal(isPublicProtoMember("eveProto/generated/eve/wallet/wallet_pb2.pyc"), false);
});
test("contract exporter inventories selected Frontier client modules", () => {
    assert.equal(isInventoryMember("frontier/landscape/common/resource_config.pyc"), true);
    assert.equal(isInventoryMember("frontier/smart_assemblies/client/proto_messenger.pyc"), true);
...
test("Linux static extraction still requires native client loaders", () => {
    ...
    try {
        assert.throws(() => resolveFrontierPython("/nonexistent/client", "/nonexistent/tools", {
            platform: "linux",
...
test("external Windows Python keeps its standard library ahead of client modules", () => {
    ...
    assert.equal(Object.keys(environment).some((key) => key.toUpperCase() === "PYTHONPATH"), false);
...
test("Wine resolver rejects a wrapper that exits successfully without Python", () => {
    ...
    try {
        ...
        assert.throws(() => resolveFrontierPython(fixture, fixture, { platform: "linux" }), /without the expected Python 3.12 success marker/);
    }
    finally {
        if (previousWine === undefined)
            delete process.env.EVEJS_FRONTIER_WINE;
        else
            process.env.EVEJS_FRONTIER_WINE = previousWine;
        if (previousPython === undefined)
            delete process.env.EVEJS_FRONTIER_PYTHON312;
        else
            process.env.EVEJS_FRONTIER_PYTHON312 = previousPython;
        fs.rmSync(fixture, { recursive: true, force: true });
    }
});
//# sourceMappingURL=contracts-cli.test.mjs.map