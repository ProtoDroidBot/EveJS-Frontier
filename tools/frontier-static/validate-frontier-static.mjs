...
import { inspectCollisionBundle, } from "./extract-frontier-static.mjs";
...
async function validateSnapshot(snapshot) {
    ...
    assert(collisionAsset.schema === "destiny.buffers.CollisionData", "Collision bundle declares an unexpected schema");
    ...
    assert(collisionRelativePath &&
        !collisionRelativePath.startsWith("..") &&
        !path.isAbsolute(collisionRelativePath), "Collision bundle path escapes the snapshot");
    assert(fs.existsSync(collisionPath), "Snapshot collision bundle is missing");
    assert(fs.statSync(collisionPath).size === collisionAsset.bytes, "Collision bundle byte size mismatch");
    assert(sha256File(collisionPath) === collisionAsset.sha256, "Collision bundle SHA-256 mismatch");
    assert(collisionAsset.sha256 === manifest.source.collisionBundle.sha256, "Collision bundle does not match the selected client source");
    assert(collisionAsset.items === manifest.source.collisionBundle.items, "Collision bundle item count does not match the selected client source");
    ...
    for (const fieldName of [
        ...
        assert(observedCollision[fieldName] === collisionAsset[fieldName], `Collision bundle ${fieldName} does not match its manifest`);
    }
    ...
    const creationHardpointTypeIDs = (await load("creationHardpointTypes.jsonl")).keys;
    const creationModuleIDs = (await load("creationModules.jsonl", (row) => {
        ...
        for (const hardpointType of placement.hardpoints || []) {
            assert(creationHardpointTypeIDs.has(String(hardpointType)), `Creation module ${typeID} references unknown hardpoint ${hardpointType}`);
        }
        for (const hardpointType of placement.compatible_hardpoints || []) {
            assert(creationHardpointTypeIDs.has(String(hardpointType)), `Creation module ${typeID} references unknown compatible hardpoint ${hardpointType}`);
        }
    })).keys;
    ...
    const creationTemplates = await load("creationTemplates.jsonl", (row) => {
        ...
        for (const [partID, part] of Object.entries(row.parts)) {
            assert(creationParts.keys.has(String(part && part.graphic_id)), `Creation template ${typeID} part ${partID} has unknown graphic ${part && part.graphic_id}`);
        ...
        for (const module of interiorModules) {
            assert(creationModuleIDs.has(String(module.type_id)), `Creation template ${typeID} references unknown module ${module.type_id}`);
            assert(templatePartIDs.has(String(module.part_id)), `Creation template ${typeID} module ${module.type_id} references unknown part ${module.part_id}`);
            ...
            for (const [hardpointIndex, hardpoint] of (module.hardpoints || []).entries()) {
                assert(templatePartIDs.has(String(hardpoint.part_id)), `Creation template ${typeID} module ${module.type_id} hardpoint ${hardpointIndex} references unknown part ${hardpoint.part_id}`);
                assert(!hardpoint.exterior_type_id || creationModuleIDs.has(String(hardpoint.exterior_type_id)), `Creation template ${typeID} module ${module.type_id} hardpoint ${hardpointIndex} references unknown exterior module ${hardpoint.exterior_type_id}`);
    ...
    await load("frontierDungeonTemplates.jsonl", (row) => {
        ...
        for (const room of row.rooms) {
            ...
            for (const object of room.objects) {
                ...
                assert(typeIDs.has(String(object.typeID)), `Frontier dungeon ${dungeonID} object ${object.objectID} has unknown type ${object.typeID}`);
            }
        }
    });
    ...
    await load("landscapeDungeonTemplates.jsonl", (row) => {
        ...
        for (const room of row.rooms) {
            ...
            for (const object of room.objects) {
                ...
                assert(typeIDs.has(String(object.typeID)), `Landscape dungeon ${dungeonID} object ${object.objectID} has unknown type ${object.typeID}`);
                assert(["entryLocator", "eventLocator", "poiLocator", "resourceLocator", "scenery"].includes(object.role), `Landscape dungeon ${dungeonID} object ${object.objectID} has invalid role ${object.role}`);
            }
        }
    });
    ...
    await load("landscapeEcosystems.jsonl", (row) => {
        ...
        assert(landscapeDungeonIDs.has(Number(row.entryDungeonID)), `Landscape ecosystem ${ecosystemID} has unknown entry dungeon ${row.entryDungeonID}`);
        for (const fieldName of ["naturalWorldPatterns", "brokenWorldPatterns"]) {
            ...
            for (const pattern of row[fieldName]) {
                assert(landscapeDungeonIDs.has(Number(pattern.dungeonID)), `Landscape ecosystem ${ecosystemID} has unknown pattern dungeon ${pattern.dungeonID}`);
            }
        }
    });
    ...
    assert(Number(relayComponents.smartDeployable?.constructionSite) === 91717, "Relay type 90184 does not map to construction site 91717");
    assert(relaySiteComponents?.assemblyConstruction, "Relay construction site 91717 has no assemblyConstruction component");
    ...
    const landscapeSites = await load("landscapeSites.jsonl", (row) => {
        ...
        assert(systemIDs.has(Number(row.solarSystemID)), `Landscape site ${siteID} has unknown system ${row.solarSystemID}`);
        assert(typeIDs.has(String(row.typeID)), `Landscape site ${siteID} has unknown type ${row.typeID}`);
        assert(landscapeEcosystemIDs.has(Number(row.ecosystemID)), `Landscape site ${siteID} has unknown ecosystem ${row.ecosystemID}`);
        assert(landscapeDungeonIDs.has(Number(row.dungeonID)), `Landscape site ${siteID} has unknown dungeon ${row.dungeonID}`);
        assert(row.featureKind === "asteroidBelt" || row.featureKind === "trojan", `Landscape site ${siteID} has invalid feature kind ${row.featureKind}`);
        assert(Number(row.dungeonID) > 0, `Landscape site ${siteID} has no dungeon`);
        assert(row.position && ["x", "y", "z"].every((axis) => Number.isFinite(Number(row.position[axis]))), `Landscape site ${siteID} has an invalid position`);
    });
    ...
    const stars = await load("mapStars.jsonl", (row) => {
        ...
        assert(systemStarIDs.get(Number(row.solarSystemID)) === Number(row._key), `System ${row.solarSystemID} does not point back to star ${row._key}`);
    });
    ...
    for (const fileName of [
        ...
        await load(fileName, (row) => {
            assert(systemIDs.has(Number(row.solarSystemID)), `${fileName} ${row._key} has unknown system ${row.solarSystemID}`);
        });
    }
    ...
    for (const [gateID, gate] of gatesByID) {
        ...
        assert(Number(destination.destination.stargateID) === gateID, `Stargate ${gateID} destination ${destinationID} is not reciprocal`);
        assert(Number(gate.destination.solarSystemID) === Number(destination.solarSystemID), `Stargate ${gateID} has an incorrect destination system`);
    }
...
async function main() {
    ...
    console.log(`[frontier-static] Valid build ${checked.build}: ` +
...
export { findLatestSnapshot, parseArgs, readJsonl, validateSnapshot, };
//# sourceMappingURL=validate-frontier-static.mjs.map