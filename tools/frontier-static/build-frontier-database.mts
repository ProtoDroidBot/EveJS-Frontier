...
function parseArgs(argv = process.argv.slice(2)) {
  const options: { force: boolean; outDir: string | null; snapshot: string | null; } & { help?: boolean } = { force: false, outDir: null, snapshot: null };
...
function validateDatabase(dataDir, snapshotManifest, databaseManifest) {
  ...
  const componentTypesByID = new Map<number, { smartDeployable?: { constructionSite?: number }; assemblyConstruction?: unknown }>(
