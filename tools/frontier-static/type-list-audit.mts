...
function readJsonlRows(snapshot: string, fileName: string) {
...
function auditTypeLists({ lists, categories, groups, types, runtimeBindings = RUNTIME_TYPE_LIST_BINDINGS }) {
  ...
  const emptyListIDs: number[] = [];
  const basicEmptyListIDs: number[] = [];
  for (const [listID, row] of byID) {
    ...
    if (ruleCount === 0) emptyListIDs.push(listID);
  ...
  const visitState = new Map<number, number>();
  function visit(listID: number, stack: number[]) {
    if (visitState.get(listID) === 2) return;
    ...
    for (const next of [...(row.includedTypeListIDs || []), ...(row.excludedTypeListIDs || [])]) {
      if (byID.has(Number(next))) visit(Number(next), [...stack, listID]);
    }
    visitState.set(listID, 2);
  }
  for (const listID of byID.keys()) visit(listID, []);
...
function auditTypeListSnapshot(snapshot: string, manifest: Record<string, any>) {
...
function compareTypeListCatalogs(previous: Record<string, any>[], current: Record<string, any>[]) {
  function canonical(row: Record<string, any>) {
...
function compareTypeListSnapshotToGenerated(snapshot: string, dataDir: string) {
  ...
  return compareTypeListCatalogs(
    Array.isArray(previous.typeLists) ? previous.typeLists : [],
    readJsonlRows(snapshot, "typeLists.jsonl"),
  );
}

export {
  RUNTIME_TYPE_LIST_BINDINGS,
  auditTypeLists,
  auditTypeListSnapshot,
  compareTypeListCatalogs,
