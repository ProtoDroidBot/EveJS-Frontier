...
function parseArgs(argv = process.argv.slice(2)) {
    const options = {
        ...
        gatewayLeaf: path.join(REPO_ROOT, "server", "src", "_secondary", "express", "certs", "gateway-dev-cert.pem"),
        bundles: [],
    };
...
function pemCertificateDers(text, filePath) {
    ...
    return blocks.map((block, index) => {
        try {
            ...
            throw new Error(`Invalid PEM certificate ${index + 1} in ${filePath}: ${error.message}`);
        }
    });
}
function countCaInBundle(bundlePath, caFingerprint) {
    ...
    const certificateDers = pemCertificateDers(fs.readFileSync(bundlePath, "ascii"), bundlePath);
    const count = certificateDers.filter((der) => crypto.createHash("sha256").update(der).digest("hex") === caFingerprint).length;
    if (count !== 1) {
        throw new Error(`Expected the EveJS CA exactly once in ${bundlePath}; found ${count}.`);
    }
...
function main() {
    ...
    const bundles = Object.fromEntries(options.bundles.map((bundlePath) => [
        bundlePath,
...
//# sourceMappingURL=verify-frontier-certificates.mjs.map