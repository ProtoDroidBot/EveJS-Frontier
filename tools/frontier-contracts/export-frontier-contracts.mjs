...
import { discoverFrontierClients, selectBuild, } from "../frontier-static/lib/frontier-client-discovery.mjs";
import { buildPythonInvocation, resolveFrontierPython, } from "../frontier-static/lib/frontier-python.mjs";
...
function prepareDestination(outDir, force) {
    ...
    if (!fs.existsSync(manifestPath)) {
        throw new Error(`Refusing to replace a directory not owned by this exporter: ${outDir}`);
    }
...
function outputMetadata(outDir, outputNames) {
    return Object.fromEntries([...outputNames].sort().map((fileName) => {
...
function main() {
    ...
    const selected = selectBuild(discoverFrontierClients({ clientRoot: options.clientRoot }), options.build);
    ...
    const outDir = options.outDir || path.join(REPO_ROOT, "_local", "frontier-contracts", String(selected.build));
    console.log(`[frontier-contracts] build=${selected.build} channel=${selected.channel}`);
    ...
    const invocation = buildPythonInvocation(runner, path.join(SCRIPT_DIR, "dump_frontier_contracts.py"), ["--request", requestPath, "--out", workDir]);
    ...
    fs.writeFileSync(path.join(workDir, "frontier-contract-manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
    fs.renameSync(workDir, outDir);
    console.log(`[frontier-contracts] Exported ${report.summary.descriptorFiles} protobuf files ` +
        `and indexed ${report.summary.clientModules} client modules.`);
    console.log(`[frontier-contracts] Manifest: ${path.join(outDir, "frontier-contract-manifest.json")}`);
}
if (process.argv[1] &&
...
export { CLIENT_MODULE_PREFIXES, isInventoryMember, isPublicProtoMember, parseArgs, };
//# sourceMappingURL=export-frontier-contracts.mjs.map