...
function loadDatabaseDriver() {
  ...
  throw new Error(
    `better-sqlite3 is not installed on this host (looked for ${serverCopy}). ` +
...
function readLivePlayerItems() {
  if (!fs.existsSync(databasePath)) {
    ...
    process.stderr.write(
      `Live SQLite player database was not found: ${databasePath}. Extracting catalog icons only.\n`,
    );
    return [];
  }
...
function resolveTargets() {
  ...
  try {
    ...
    throw new Error(
      `Could not parse the icon target list returned by the container: ${error.message}`,
    );
  }
