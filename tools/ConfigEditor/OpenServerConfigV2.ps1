...
$script:AllianceLogoSnapshot = $null
...
function Get-ControlValue {
  ...
  switch ($Controller.Kind) {
    ...
    "json" {
      $parsedJson = $Controller.Control.Text | ConvertFrom-Json
      if ($Controller.Control.Text.TrimStart().StartsWith("[")) {
        return ,@($parsedJson)
      }
      return $parsedJson
    }
...
function Set-ControlValue {
  ...
  switch ($Controller.Kind) {
    ...
    "json" { $Controller.Control.Text = ConvertTo-PrettyJson $Value }
...
function Render-OverviewFields {
  ...
  $script:DbGmCheckBox.IsChecked = [bool](Get-JsonPropertyValue -Object $script:DbWorkingPlayer.summary -Name "isGM")
...
function Sync-OverviewControlsToWorkingCopy {
  ...
  Set-ObjectProperty -Object $script:DbWorkingPlayer.account -Name "isGM" -Value ([bool]$script:DbGmCheckBox.IsChecked)
  ...
  Set-ObjectProperty -Object $script:DbWorkingPlayer.summary -Name "isGM" -Value ([bool]$script:DbGmCheckBox.IsChecked)
...
# Asset extraction is cosmetic - the editor is fully usable with its built-in
# Segoe glyphs. A failure here (an unreachable container, an unreadable client
# resource file) must not take the whole Config Editor down with it, which is
# what happened while extract-client-icons.js could not run on a Docker host.
function Invoke-OptionalAssetScript {
  param(
  ...
  try {
    Invoke-NodeScript -ScriptPath $ScriptPath
  } catch {
    $detail = @(($_.Exception.Message -split "`n") | Where-Object { $_.Trim() -ne "" } | Select-Object -First 4)
    Write-Host "  [WARN] $Label extraction failed; continuing with built-in glyphs."
    foreach ($line in $detail) { Write-Host "         $($line.TrimEnd())" }
  }
}
...
function Invoke-AssetExtraction {
  ...
  if ($needsClient) {
    ...
    Invoke-OptionalAssetScript -ScriptPath (Join-Path $PSScriptRoot "extract-client-icons.js") -Label "Item/ship icon"
  }

  if ($needsUi) {
    Write-Host "  Extracting UI icons..."
    Invoke-OptionalAssetScript -ScriptPath (Join-Path $PSScriptRoot "extract-ui-icons.js") -Label "UI icon"
  }
...
'@)
  ...
  foreach ($listBox in @($script:DbPlayerList, $script:DbSkillPreviewList, $script:DbItemPreviewList)) {
    ...
    # Keep the list transparent so the themed parent panel shows through and the
    # background re-themes on light/dark toggle (a directly-assigned brush here
    # would be captured once and go stale). Matches the Market Orders list.
...
function Set-AllianceLogoPreviewImage {
  param([string]$Path)
  if ([string]::IsNullOrWhiteSpace($Path) -or -not (Test-Path -LiteralPath $Path)) {
    $script:AllianceLogoPreview.Source = $null
    return
  }
  try {
    ...
    $bitmap.EndInit()
    $script:AllianceLogoPreview.Source = $bitmap
  } catch {
    $script:AllianceLogoPreview.Source = $null
  }
}

function Render-AllianceLogoList {
  if (-not $script:AllianceLogoSnapshot) { return }
  $search = ([string]$script:AllianceLogoSearchBox.Text).Trim().ToLower()
  $all = @($script:AllianceLogoSnapshot.alliances)
  $filtered = @($all | Where-Object {
    if ($search -eq "") { return $true }
    $hay = ("{0} {1} {2}" -f [string]$_.allianceName, [string]$_.shortName, [string]$_.allianceID).ToLower()
    $hay.Contains($search)
  })
  $displayItems = @($filtered | ForEach-Object {
    $name = if ([string]$_.allianceName) { [string]$_.allianceName } else { "(not in database)" }
    $sizeList = @($_.sizes)
    $logoText = if ([bool]$_.hasLogo) { "logo: {0} size(s)" -f $sizeList.Count } else { "no logo (default)" }
    [pscustomobject]@{
      label = ("{0}    |    ID {1}    |    {2}" -f $name, [string]$_.allianceID, $logoText)
      allianceID = [int64]$_.allianceID
      allianceName = [string]$_.allianceName
      shortName = [string]$_.shortName
      sizes = $sizeList
      hasLogo = [bool]$_.hasLogo
    }
  })
  $script:AllianceLogoList.DisplayMemberPath = "label"
  $script:AllianceLogoList.ItemsSource = $displayItems
  $withLogo = @($all | Where-Object { [bool]$_.hasLogo }).Count
  $script:AllianceLogoSummary.Text = ("{0} alliance(s)   |   {1} with a custom logo   |   showing {2}" -f $all.Count, $withLogo, $displayItems.Count)
}

function Update-AllianceLogoDetail {
  $selected = $script:AllianceLogoList.SelectedItem
  if (-not $selected) {
    $script:AllianceLogoDetailName.Text = "Select an alliance"
    $script:AllianceLogoDetailId.Text = ""
    $script:AllianceLogoDetailStatus.Text = ""
    $script:AllianceLogoPreview.Source = $null
    return
  }
  $name = if ([string]$selected.allianceName) { [string]$selected.allianceName } else { "(not in database)" }
  $script:AllianceLogoDetailName.Text = $name
  $script:AllianceLogoDetailId.Text = ("Alliance ID {0}" -f [string]$selected.allianceID)
  $script:AllianceLogoIdBox.Text = [string]$selected.allianceID
  $sizeList = @($selected.sizes)
  if ([bool]$selected.hasLogo -and $sizeList.Count -gt 0) {
    $root = [string]$script:AllianceLogoSnapshot.paths.allianceLogoRoot
    $maxSize = ($sizeList | Measure-Object -Maximum).Maximum
    $previewPath = Join-Path $root ("{0}_{1}.png" -f [string]$selected.allianceID, [string]$maxSize)
    Set-AllianceLogoPreviewImage -Path $previewPath
    $script:AllianceLogoDetailStatus.Text = ("Custom logo present at sizes: {0}." -f ((@($sizeList | Sort-Object) -join ", ")))
  } else {
    Set-AllianceLogoPreviewImage -Path ([string]$script:AllianceLogoSnapshot.defaultLogoPath)
    $script:AllianceLogoDetailStatus.Text = "No custom logo - the default alliance logo is served. Import a PNG to set one."
  }
}

function Get-AllianceLogoTargetId {
  $idText = ([string]$script:AllianceLogoIdBox.Text).Trim()
  if ($idText -match '^\d+$') { return [int64]$idText }
  $selected = $script:AllianceLogoList.SelectedItem
  if ($selected) { return [int64]$selected.allianceID }
  return [int64]0
}

function Initialize-AllianceLogos {
  try {
    $script:AllianceLogoSnapshot = Invoke-ProjectCli -Command "alliance-logo-export"
  } catch {
    $script:AllianceLogoSnapshot = $null
    $script:AllianceLogoSummary.Text = "Could not load alliances: $($_.Exception.Message)"
    return
  }
  Render-AllianceLogoList
  Update-AllianceLogoDetail
}

function Reload-AllianceLogos {
  param([int64]$SelectId = 0)
  try {
    $script:AllianceLogoSnapshot = Invoke-ProjectCli -Command "alliance-logo-export"
  } catch {
    Set-StatusUi -Message $_.Exception.Message -Tone "error"
  ...
  Render-AllianceLogoList
  if ($SelectId -gt 0) {
    $match = @(@($script:AllianceLogoList.ItemsSource) | Where-Object { [int64]$_.allianceID -eq $SelectId })
    if ($match.Count -gt 0) { $script:AllianceLogoList.SelectedItem = $match[0] }
  }
  Update-AllianceLogoDetail
}

function Import-AllianceLogo {
  $allianceId = Get-AllianceLogoTargetId
  if ($allianceId -le 0) {
    Set-StatusUi -Message "Select an alliance or enter an alliance ID before importing a logo." -Tone "info" -BadgeText "Alliance"
    return
  }
  $dialog = New-Object Microsoft.Win32.OpenFileDialog
  $dialog.Filter = "PNG images (*.png)|*.png"
  $dialog.Title = "Select a PNG logo for alliance $allianceId"
  if ($dialog.ShowDialog() -ne $true) { return }
  $sourcePath = $dialog.FileName
  try {
    $result = Invoke-ProjectCli -Command "alliance-logo-import" -InputObject ([pscustomobject]@{ allianceID = $allianceId; sourcePath = $sourcePath })
    Reload-AllianceLogos -SelectId $allianceId
    Set-StatusUi -Message ("Imported logo for alliance {0} at {1} size(s)." -f $allianceId, @($result.sizes).Count) -Tone "success" -BadgeText "Alliance"
  } catch {
    Set-StatusUi -Message $_.Exception.Message -Tone "error"
  }
}

function Remove-AllianceLogo {
  $allianceId = Get-AllianceLogoTargetId
  if ($allianceId -le 0) {
    Set-StatusUi -Message "Select an alliance (or enter an ID) whose logo you want to remove." -Tone "info" -BadgeText "Alliance"
    return
  }
  $confirm = [System.Windows.MessageBox]::Show(
    ("Remove the custom logo for alliance {0}?`n`nThe default alliance logo will be served instead." -f $allianceId),
    "EvEJS Alliance Logos",
    [System.Windows.MessageBoxButton]::YesNo,
    [System.Windows.MessageBoxImage]::Warning)
  if ($confirm -ne [System.Windows.MessageBoxResult]::Yes) { return }
  try {
    $result = Invoke-ProjectCli -Command "alliance-logo-delete" -InputObject ([pscustomobject]@{ allianceID = $allianceId })
    Reload-AllianceLogos -SelectId $allianceId
    if (@($result.removed).Count -gt 0) {
      Set-StatusUi -Message ("Removed the custom logo for alliance {0}." -f $allianceId) -Tone "success" -BadgeText "Alliance"
    } else {
      Set-StatusUi -Message ("Alliance {0} had no custom logo to remove." -f $allianceId) -Tone "info" -BadgeText "Alliance"
    }
  } catch {
    Set-StatusUi -Message $_.Exception.Message -Tone "error"
  }
...
function Open-NewAccountDialog {
  [xml]$acctXaml = @'
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml" Title="EvEJS New Account" Width="480" Height="410" WindowStartupLocation="CenterOwner" Background="{DynamicResource WindowBg}" FontFamily="Segoe UI">
  <Grid Margin="18">
    <Grid.RowDefinitions><RowDefinition Height="Auto" /><RowDefinition Height="*" /><RowDefinition Height="Auto" /></Grid.RowDefinitions>
    <Border Grid.Row="0" CornerRadius="14" Padding="16,12,16,12" Margin="0,0,0,12" Background="{DynamicResource HeroBg}" BorderBrush="{DynamicResource BorderSel}" BorderThickness="1">
      <StackPanel>
        <TextBlock Text="Create Account" FontSize="20" FontWeight="SemiBold" Foreground="{DynamicResource TextPrimary}" />
        <TextBlock Text="Creates account credentials only. Log in with the game client to make a character." Margin="0,4,0,0" FontSize="12" Foreground="{DynamicResource TextMuted}" TextWrapping="Wrap" />
      </StackPanel>
    </Border>
    <Border Grid.Row="1" CornerRadius="14" Background="{DynamicResource Panel}" BorderBrush="{DynamicResource Border}" BorderThickness="1" Padding="16">
      <StackPanel>
        <TextBlock Text="Username" FontSize="12" FontWeight="SemiBold" Foreground="{DynamicResource TextMuted}" Margin="0,0,0,6" />
        <TextBox x:Name="AcctUserBox" Height="36" Margin="0,0,0,14" VerticalContentAlignment="Center" />
        <TextBlock Text="Password" FontSize="12" FontWeight="SemiBold" Foreground="{DynamicResource TextMuted}" Margin="0,0,0,6" />
        <TextBox x:Name="AcctPassBox" Height="36" Margin="0,0,0,14" VerticalContentAlignment="Center" />
        <CheckBox x:Name="AcctGmCheck" Content="Game Master (GM)" IsChecked="True" FontSize="14" Foreground="{DynamicResource TextPrimary}" />
        <TextBlock Text="Password validation is skipped on this dev server, so any password logs this account in." Margin="0,12,0,0" FontSize="11" Foreground="{DynamicResource TextMuted}" TextWrapping="Wrap" />
      </StackPanel>
    </Border>
    <Border Grid.Row="2" Margin="0,12,0,0" CornerRadius="14" Background="{DynamicResource Panel}" BorderBrush="{DynamicResource Border}" BorderThickness="1" Padding="14,10,14,10">
      <StackPanel Orientation="Horizontal" HorizontalAlignment="Right">
        <Button x:Name="AcctCancelButton" Content="Cancel" Margin="0,0,10,0" Padding="14,8,14,8" />
        <Button x:Name="AcctCreateButton" Content="Create Account" Padding="16,8,16,8" />
      </StackPanel>
    </Border>
  </Grid>
</Window>
'@
  ...
  $acctWindow = [System.Windows.Markup.XamlReader]::Load([System.Xml.XmlNodeReader]::new($acctXaml))
  $acctWindow.Owner = $script:Window
  $acctWindow.Add_SourceInitialized({ param($s, $e) Apply-WindowThemeChrome -Window $s })

  $acctUserBox = $acctWindow.FindName("AcctUserBox")
  $acctPassBox = $acctWindow.FindName("AcctPassBox")
  $acctGmCheck = $acctWindow.FindName("AcctGmCheck")
  $acctCancelButton = $acctWindow.FindName("AcctCancelButton")
  $acctCreateButton = $acctWindow.FindName("AcctCreateButton")

  $acctUserBox.Style = $script:Window.FindResource("ConfigTextBoxStyle")
  $acctPassBox.Style = $script:Window.FindResource("ConfigTextBoxStyle")
  $acctCancelButton.Style = $script:Window.FindResource("SecondaryButtonStyle")
  $acctCreateButton.Style = $script:Window.FindResource("PrimaryButtonStyle")

  $script:NewAccountResult = $null

  $acctCancelButton.Add_Click({ $acctWindow.DialogResult = $false; $acctWindow.Close() })
  $acctCreateButton.Add_Click({
    try {
      $username = $acctUserBox.Text.Trim()
      if ($username -eq "") { throw "A username is required." }
      $payload = @{
        username = $username
        password = [string]$acctPassBox.Text
        isGM = [bool]$acctGmCheck.IsChecked
      }
      $script:NewAccountResult = Invoke-ProjectCli -Command "account-create" -InputObject $payload
      $acctWindow.DialogResult = $true
      $acctWindow.Close()
    } catch {
      [System.Windows.MessageBox]::Show($_.Exception.Message, "EvEJS New Account", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Error) | Out-Null
    }
  })

  if ($acctWindow.ShowDialog() -eq $true -and $script:NewAccountResult) {
    $created = $script:NewAccountResult
    Set-StatusUi -Message ("Created account '{0}' (GM: {1}). Log in with the game client to create a character." -f [string]$created.username, ([bool]$created.isGM)) -Tone "success" -BadgeText "Account"
  }
}
            ...
            <Border Grid.Column="0" CornerRadius="18" Background="{DynamicResource Panel}" BorderBrush="{DynamicResource Border}" BorderThickness="1" Padding="16">
              <DockPanel LastChildFill="True">
                <StackPanel DockPanel.Dock="Top">
                  ...
                  <Button x:Name="DbNewAccountButton" Content="New Account" Height="36" Margin="0,0,0,12" Style="{StaticResource SecondaryButtonStyle}" />
                </StackPanel>
                    ...
                    <Border CornerRadius="18" Background="{DynamicResource Panel}" BorderBrush="{DynamicResource Border}" BorderThickness="1" Padding="18" Margin="0,12,0,0">
                      <Grid>
                        ...
                        <StackPanel Grid.Row="0" Grid.Column="1" Margin="12,0,0,12">
                          <TextBlock Text="Account Access" Style="{StaticResource SectionLabelStyle}" />
                          <CheckBox x:Name="DbBannedCheckBox" Content="Account is banned" FontSize="14" Foreground="{DynamicResource TextPrimary}" />
                          <CheckBox x:Name="DbGmCheckBox" Content="Game Master (GM)" Margin="0,8,0,0" FontSize="14" Foreground="{DynamicResource TextPrimary}" />
                        </StackPanel>
      ...
      <TabItem Header="Alliance Logos">
        <Grid Margin="0,8,0,0">
          ...
          <Border Grid.Row="0" CornerRadius="14" Background="{DynamicResource Panel}" BorderBrush="{DynamicResource Border}" BorderThickness="1" Padding="14,10,14,10" Margin="0,0,0,10">
            <StackPanel>
              <Grid>
                ...
                <StackPanel VerticalAlignment="Center">
                  <TextBlock Text="Alliance Logos" FontSize="15" FontWeight="SemiBold" Foreground="{DynamicResource TextPrimary}" />
                  <TextBlock x:Name="AllianceLogoSubtitle" Margin="0,3,0,0" FontSize="12" Foreground="{DynamicResource TextMuted}" Text="Import a PNG per alliance; it is served by the image server. Alliances without a logo use the default." />
                </StackPanel>
                <StackPanel Grid.Column="1" Orientation="Horizontal" HorizontalAlignment="Right" VerticalAlignment="Center">
                  <Button x:Name="AllianceLogoRefreshButton" Content="Refresh" Padding="12,7,12,7" Style="{StaticResource SecondaryButtonStyle}" />
                </StackPanel>
              </Grid>
              <Grid Margin="0,12,0,0">
                <Grid.ColumnDefinitions>
                ...
                <TextBlock Grid.Column="0" Text="Search" VerticalAlignment="Center" Margin="0,0,10,0" FontSize="12" FontWeight="SemiBold" Foreground="{DynamicResource TextMuted}" />
                <TextBox x:Name="AllianceLogoSearchBox" Grid.Column="1" Height="34" VerticalContentAlignment="Center" Style="{StaticResource ConfigTextBoxStyle}" />
                <TextBlock Grid.Column="2" Text="Alliance ID" VerticalAlignment="Center" Margin="14,0,10,0" FontSize="12" FontWeight="SemiBold" Foreground="{DynamicResource TextMuted}" />
                <TextBox x:Name="AllianceLogoIdBox" Grid.Column="3" Height="34" VerticalContentAlignment="Center" Style="{StaticResource ConfigTextBoxStyle}" />
                <Button x:Name="AllianceLogoImportButton" Grid.Column="4" Content="Import PNG..." Margin="12,0,0,0" Padding="14,7,14,7" Style="{StaticResource PrimaryButtonStyle}" />
              </Grid>
            </StackPanel>
          </Border>
          ...
          <Grid Grid.Row="1">
            ...
            <Border Grid.Column="0" CornerRadius="14" Background="{DynamicResource Panel}" BorderBrush="{DynamicResource Border}" BorderThickness="1" Padding="8" Margin="0,0,10,0">
              <DockPanel LastChildFill="True">
                <TextBlock x:Name="AllianceLogoSummary" DockPanel.Dock="Top" Margin="6,4,6,10" FontSize="12" Foreground="{DynamicResource TextMuted}" Text="No alliances loaded." />
                <ListBox x:Name="AllianceLogoList" BorderThickness="0" Background="Transparent" />
              </DockPanel>
            </Border>

            <Border Grid.Column="1" CornerRadius="14" Background="{DynamicResource Panel}" BorderBrush="{DynamicResource Border}" BorderThickness="1" Padding="14">
              <StackPanel>
                <TextBlock Text="Selected alliance" FontSize="12" FontWeight="SemiBold" Foreground="{DynamicResource TextMuted}" />
                <TextBlock x:Name="AllianceLogoDetailName" Margin="0,4,0,0" FontSize="15" FontWeight="SemiBold" Foreground="{DynamicResource TextPrimary}" Text="Select an alliance" />
                <TextBlock x:Name="AllianceLogoDetailId" Margin="0,2,0,0" FontSize="12" Foreground="{DynamicResource TextMuted}" Text="" />
                <Border Margin="0,12,0,0" CornerRadius="10" Background="{DynamicResource PanelSubtle}" BorderBrush="{DynamicResource Border}" BorderThickness="1" Width="200" Height="200" HorizontalAlignment="Left">
                  <Image x:Name="AllianceLogoPreview" Stretch="Uniform" Margin="8" />
                </Border>
                <TextBlock x:Name="AllianceLogoDetailStatus" Margin="0,10,0,0" FontSize="12" Foreground="{DynamicResource TextMuted}" TextWrapping="Wrap" Text="" />
                <Button x:Name="AllianceLogoRemoveButton" Content="Remove Logo" Margin="0,14,0,0" Padding="12,7,12,7" HorizontalAlignment="Left" Style="{StaticResource SecondaryButtonStyle}" />
              </StackPanel>
            </Border>
          </Grid>
        </Grid>
...
$script:DbNewAccountButton = $script:Window.FindName("DbNewAccountButton")
...
$script:DbGmCheckBox = $script:Window.FindName("DbGmCheckBox")
...
$script:AllianceLogoRefreshButton = $script:Window.FindName("AllianceLogoRefreshButton")
$script:AllianceLogoSearchBox = $script:Window.FindName("AllianceLogoSearchBox")
$script:AllianceLogoIdBox = $script:Window.FindName("AllianceLogoIdBox")
$script:AllianceLogoImportButton = $script:Window.FindName("AllianceLogoImportButton")
$script:AllianceLogoList = $script:Window.FindName("AllianceLogoList")
$script:AllianceLogoSummary = $script:Window.FindName("AllianceLogoSummary")
$script:AllianceLogoDetailName = $script:Window.FindName("AllianceLogoDetailName")
$script:AllianceLogoDetailId = $script:Window.FindName("AllianceLogoDetailId")
$script:AllianceLogoPreview = $script:Window.FindName("AllianceLogoPreview")
$script:AllianceLogoDetailStatus = $script:Window.FindName("AllianceLogoDetailStatus")
$script:AllianceLogoRemoveButton = $script:Window.FindName("AllianceLogoRemoveButton")
...
$script:DbNewAccountButton.Add_Click({
  try {
...
$script:DbGmCheckBox.Add_Checked({
  if (-not $script:IsRenderingDatabase) { Set-DatabaseDirtyState -Dirty $true -Message "GM status change is staged in the form." }
})
$script:DbGmCheckBox.Add_Unchecked({
  if (-not $script:IsRenderingDatabase) { Set-DatabaseDirtyState -Dirty $true -Message "GM status change is staged in the form." }
})
...
$script:AllianceLogoRefreshButton.Add_Click({ Reload-AllianceLogos -SelectId (Get-AllianceLogoTargetId) })
$script:AllianceLogoImportButton.Add_Click({ Import-AllianceLogo })
$script:AllianceLogoRemoveButton.Add_Click({ Remove-AllianceLogo })
$script:AllianceLogoSearchBox.Add_TextChanged({ Render-AllianceLogoList })
$script:AllianceLogoList.Add_SelectionChanged({ Update-AllianceLogoDetail })
