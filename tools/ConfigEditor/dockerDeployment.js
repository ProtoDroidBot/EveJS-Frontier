...
function hasExplicitGameStoreOverride() {
    return Boolean(process.env.EVEJS_GAMESTORE_DATA_DIR ||
...
function throwIndeterminateDeployment(databasePath, detail) {
    throw new Error("Docker is installed but its state could not be determined" +
        (detail ? ` (${detail})` : "") +
...
function delegateNodeScript({ rootDir, composePrefix, scriptPath, argv = [], stdin = "inherit", failureLabel = "The containerized config CLI", }) {
    const result = spawnSync("docker", ["compose", ...composePrefix, "node", scriptPath, ...argv], {
        cwd: rootDir,
        encoding: "utf8",
        windowsHide: true,
    ...
    if (result.error) {
        throw new Error(`Could not reach the EvEJS container to read the live game database: ${result.error.message}`);
    }
    if (result.status !== 0) {
...
//# sourceMappingURL=dockerDeployment.js.map