...
const { containerScriptPath, delegateNodeScript, hasExplicitGameStoreOverride, isRunningInsideContainer, resolveDockerDelegation, } = require(path.join(__dirname, "dockerDeployment"));
// Self-delegation flag: the container computes the target list and prints it,
// the host does the extracting. Nothing but JSON may reach stdout in this mode.
const PRINT_TARGETS_FLAG = "--print-targets";
...
// Loaded lazily, and only where the database actually is. A Docker host has no
// server/node_modules at all - npm dependencies are installed into the image -
// so requiring this at module scope broke the extractor before it could decide
// to delegate. The bare specifier is tried first so a hoisted install works too.
function loadDatabaseDriver() {
    const serverCopy = path.join(repoRoot, "server", "node_modules", "better-sqlite3");
    for (const candidate of ["better-sqlite3", serverCopy]) {
        try {
            return require(candidate);
        }
        catch (error) {
            const missingCandidate = error &&
                error.code === "MODULE_NOT_FOUND" &&
                String(error.message).includes(candidate);
            if (!missingCandidate) {
                throw error;
            }
        }
    }
    throw new Error(`better-sqlite3 is not installed on this host (looked for ${serverCopy}). ` +
        "Run npm install in the server directory, or start the EvEJS container so " +
        "the live game database can be read from there.");
}
function readLivePlayerItems() {
    if (!fs.existsSync(databasePath)) {
        // Skills and ships come from the static catalogs, so an absent player
        // database costs only the icons for types someone owns. Not worth failing
        // the whole extraction over.
        process.stderr.write(`Live SQLite player database was not found: ${databasePath}. Extracting catalog icons only.\n`);
        return [];
    }
...
function buildTypeTargets() {
    const skillTypes = readJson(path.join(dataRoot, "skillTypes", "data.json")).skills || [];
    const shipTypes = readJson(path.join(dataRoot, "shipTypes", "data.json")).ships || [];
...
// The target list is the only part of this extractor that needs gamestore data:
// the static catalogs for skills and ships, plus the live player database for
// which item types anyone actually owns. Under the Docker deployment none of
// that exists on the host - the catalogs are generated into the image and the
// database lives in the evejs-data volume - so the list is computed inside the
// container. Extraction itself stays here, where the EVE client resource files
// and the editor's asset directory are.
function resolveTargets() {
    if (hasExplicitGameStoreOverride() || isRunningInsideContainer()) {
        return buildTypeTargets();
    }
    const composePrefix = resolveDockerDelegation({ rootDir: repoRoot, databasePath });
    if (!composePrefix) {
        return buildTypeTargets();
    }
    const relayed = delegateNodeScript({
        rootDir: repoRoot,
        composePrefix,
        scriptPath: containerScriptPath(repoRoot, __filename),
        argv: [PRINT_TARGETS_FLAG],
        stdin: "ignore",
        failureLabel: "The containerized icon target query",
    });
    let targets = null;
    try {
        targets = JSON.parse(relayed);
    }
    catch (error) {
        throw new Error(`Could not parse the icon target list returned by the container: ${error.message}`);
    }
    if (!Array.isArray(targets)) {
        throw new Error("The container returned an icon target list that was not an array.");
    }
    return targets;
}
...
function main() {
    // Delegated mode: we are the container-side half of resolveTargets().
    if (process.argv.includes(PRINT_TARGETS_FLAG)) {
        process.stdout.write(`${JSON.stringify(buildTypeTargets())}\n`);
        return;
    }
...
main();
//# sourceMappingURL=extract-client-icons.js.map