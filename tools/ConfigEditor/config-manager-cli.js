...
const Database = require(path.join(rootDir, "server", "node_modules", "better-sqlite3"));
const allianceImageStore = require(path.join(rootDir, "server", "src", "services", "corporation", "allianceImageStore"));
const accountRoleProfiles = require(path.join(rootDir, "server", "src", "services", "account", "accountRoleProfiles"));
const dataRoot = resolveDataRoot();
const databasePath = resolveDatabasePath();
// GM status is per-account, encoded in the role bitmask. Prefer the explicit
// isGM flag when present; otherwise infer it from any elevated role bit.
function deriveAccountIsGM(account) {
    if (!account) {
        return false;
    }
    if (typeof account.isGM === "boolean") {
        return account.isGM;
    }
    try {
        const role = BigInt(String(account.role ?? "0"));
        return (role & accountRoleProfiles.MAX_ACCOUNT_ROLE) !== 0n;
    }
    catch (error) {
        return false;
    }
...
// Docker deployment that is the evejs-data volume, which has no host path, so a
// host-side CLI reading _local/gameStore would silently serve a stale database.
// Rather than drag the live data onto the host, gamestore-backed commands are
// delegated into the container and their JSON is passed straight through.
// Detection and delegation are shared with the asset extractors; see
// dockerDeployment.js for the resolution order.
const { containerScriptPath, delegateNodeScript, hasExplicitGameStoreOverride, isRunningInsideContainer, resolveDockerDelegation, toHostRepoPath, } = require(path.join(__dirname, "dockerDeployment"));
// Commands whose data lives in the gamestore. alliance-logo-export is included
// because its alliance list comes from the database (listKnownAlliances); the
// logo files it also reports are on the bind-mounted logo directory, which the
// container sees at the same path. Deliberately excluded:
//   export / save        - evejs.config.local.json, a HOST file
//   market-*             - talk to the market daemon over published ports
//   alliance-logo-import - reads an arbitrary host path from the file picker,
//                          which a container cannot see
//   alliance-logo-delete - operates on the bind-mounted host logo directory
const GAMESTORE_COMMANDS = new Set([
    "database-export",
    "database-save",
    "database-type-search",
    "account-create",
    "alliance-logo-export",
]);
...
// repo files the GUI actually opens: Update-AllianceLogoDetail builds its
// preview path from paths.allianceLogoRoot and loads it with Test-Path, which
// fails silently on Windows and leaves the preview blank. The image is built by
// copying the repo to /app and the logo directory is bind-mounted, so anything
// under /app maps back to rootDir.
function rehostContainerRepoPaths(payloadText) {
    let payload = null;
    try {
    ...
    const toHostPath = (value) => toHostRepoPath(rootDir, value);
    if (payload && payload.paths && payload.paths.allianceLogoRoot) {
        payload.paths.allianceLogoRoot = toHostPath(payload.paths.allianceLogoRoot);
    }
    if (payload && payload.defaultLogoPath) {
        payload.defaultLogoPath = toHostPath(payload.defaultLogoPath);
    }
...
function inferSubGroup(key, metadata = {}) {
    if (SUBGROUP_BY_KEY[key])
        return SUBGROUP_BY_KEY[key];
    if (metadata.subGroup)
        return metadata.subGroup;
...
function buildConfigEntries(snapshot) {
    return config
        ...
        return {
            ...
            envOverrideValue: Object.prototype.hasOwnProperty.call(snapshot.envConfig, entry.key)
        ...
        const normalizedLeftGroupIndex = leftGroupIndex === -1 ? GROUP_ORDER.length : leftGroupIndex;
        const normalizedRightGroupIndex = rightGroupIndex === -1 ? GROUP_ORDER.length : rightGroupIndex;
...
function buildGroupOrder(entries = []) {
    const usedGroups = new Set(entries
...
function buildConfigExportPayload(snapshot = config.getConfigStateSnapshot()) {
    ...
    return {
        ...
        notes: environmentOverrides.length > 0
            ? [
...
function validateAllowedEditorValue(entry, value) {
    if (Array.isArray(entry && entry.allowedValues) &&
...
function coerceEditorConfigValue(entry, value) {
    switch (inferConfigEntryType(entry)) {
        ...
        case "number": {
            const parsedNumber = typeof value === "number" ? value : Number(String(value).trim());
        ...
        case "json": {
            if (typeof value !== "string") {
                return validateAllowedEditorValue(entry, value);
            }
            try {
                return validateAllowedEditorValue(entry, JSON.parse(value));
            }
            catch (error) {
...
function buildSnapshotAfterEditorSave(snapshot, definitions, nextLocalConfig, localConfigPath) {
    ...
    const sharedSource = snapshot.sharedConfigPath
        ...
        : "shared";
    const sources = Object.fromEntries(definitions.map((entry) => {
...
function saveConfigWithOptionalBlankFallback(values, blankKey) {
    ...
    if (!localConfigPath) {
        throw new Error(`${blankKey} is blank, but this EvEJS build does not expose a writable local config path.`);
    }
    ...
    const submittedBlankValue = values &&
...
function saveConfig() {
    ...
    const values = payload && typeof payload === "object" && !Array.isArray(payload) && payload.values
        ? payload.values
        : payload;
    let snapshot;
    try {
...
function readJsonFile(filePath) {
    try {
        ...
        if (error && error.code === "ENOENT") {
            throw new Error(`Required database file was not found: ${filePath}. Run tools\\DatabaseCreator\\CreateDatabase.bat if local database data has not been generated.`);
        }
        throw error;
    }
}
...
function openPlayerDatabase(readonly = true) {
    if (!fs.existsSync(databasePath)) {
        throw new Error(`The authoritative player database was not found: ${databasePath}`);
    }
...
function readSqliteTable(db, table) {
    ...
    for (const row of rows) {
        try {
            ...
            throw new Error(`Could not parse SQLite row ${table}.${row.key}: ${error.message}`);
        }
    }
    return records;
}
...
function getRecordMap(data) {
    ...
    return Object.fromEntries(Object.entries(data || {}).filter(([key, value]) => !["source", "_meta"].includes(key) && isPlainObject(value)));
}
...
function loadReferenceMaps() {
    const corporations = getRecordMap(readJsonFile(path.join(dataRoot, "corporations/data.json")));
    const stations = getRecordMap(readJsonFile(path.join(dataRoot, "stations/data.json")));
    const solarSystems = getRecordMap(readJsonFile(path.join(dataRoot, "solarSystems/data.json")));
    const shipTypes = getRecordMap(readJsonFile(path.join(dataRoot, "shipTypes/data.json")));
    return {
        corporationNames: Object.fromEntries(Object.entries(corporations).map(([key, value]) => [
            key,
            getReferenceName(value, ["corporationName", "name"]) || key,
        ])),
        stationNames: Object.fromEntries(Object.entries(stations).map(([key, value]) => [
            key,
            getReferenceName(value, ["stationName", "name"]) || key,
        ])),
        solarSystemNames: Object.fromEntries(Object.entries(solarSystems).map(([key, value]) => [
            key,
            getReferenceName(value, ["solarSystemName", "name"]) || key,
        ])),
        shipTypeNames: Object.fromEntries(Object.entries(shipTypes).map(([key, value]) => [
            key,
...
function buildPlayerSummary(characterId, character, accountKey, account, characterSkills, characterItems, references) {
    const corporationName = references.corporationNames[String(character?.corporationID ?? "")] ||
        (character?.corporationID ? String(character.corporationID) : "Unknown");
    const stationName = references.stationNames[String(character?.stationID ?? "")] ||
        (character?.stationID ? String(character.stationID) : "Unknown");
    const solarSystemName = references.solarSystemNames[String(character?.solarSystemID ?? "")] ||
        (character?.solarSystemID ? String(character.solarSystemID) : "Unknown");
    const shipName = character?.shipName ||
        references.shipTypeNames[String(character?.shipTypeID ?? "")] ||
        "Unknown ship";
    const itemCount = characterItems.length;
    const shipCount = characterItems.filter((item) => Number(item?.categoryID) === 6).length;
    ...
    return {
        ...
        isGM: deriveAccountIsGM(account),
        corporationName,
        stationName,
        solarSystemName,
        shipName,
...
function buildSkillCatalog(skillsTable, skillTypes = [], skillRanksByTypeID = {}) {
    ...
    for (const skillType of skillTypes || []) {
        ...
        catalog.set(normalizedSkillKey, {
            ...
            skillRank: skillType?.skillRank ?? skillRanksByTypeID[normalizedSkillKey] ?? 1,
    ...
    for (const skillMap of Object.values(skillsTable || {})) {
        for (const [skillKey, skill] of Object.entries(skillMap || {})) {
            ...
            catalog.set(normalizedSkillKey, {
                ...
                itemName: skill?.itemName || catalogEntry?.itemName || `Skill ${normalizedSkillKey}`,
...
function buildItemTypeSearchResults(itemTypes, queryText, limit = 80) {
    ...
    const publishedTypes = itemTypes.filter((type) => Boolean(type?.published) && Number(type?.categoryID ?? 0) !== 6);
...
function buildNextItemIdSeed(itemsTable, identityState = {}) {
    ...
    return Math.max(maxItemId + 1, Number.isFinite(allocatedNextItemId) ? allocatedNextItemId : 0);
}
function buildPlayerList(tables, references) {
    ...
    return Object.entries(characters || {})
        ...
        return buildPlayerSummary(characterId, character, accountKey, account, characterSkills, characterItems, references);
    })
        .sort((left, right) => {
...
function buildPlayerDetail(characterId, tables, references) {
    ...
    const skills = isPlainObject(tables.skills?.[normalizedCharacterId])
        ...
        : {};
    const items = Object.values(tables.items || {}).filter((item) => String(item?.ownerID ?? "") === normalizedCharacterId);
    const summary = buildPlayerSummary(normalizedCharacterId, character, accountKey, account, skills, items, references);
    return {
        summary,
...
function buildDatabasePayload(selectedCharacterId) {
    ...
    const skillCatalog = buildSkillCatalog(tables.skills, typeTables.skillTypes, skillRanksByTypeID);
    const shipCatalog = buildShipCatalog(typeTables.shipTypes);
    const resolvedCharacterId = selectedCharacterId && players.some((player) => player.characterId === String(selectedCharacterId))
...
function validateDatabaseSave(payload, tables) {
    ...
    const originalAccountKey = requireNonEmptyString(payload.originalAccountKey, "originalAccountKey");
    ...
    if (!Number.isFinite(characterSkillPoints) ||
        Math.round(characterSkillPoints) !== Math.round(skillPointTotal)) {
        throw new Error(`character.skillPoints must equal the skill total (${Math.round(skillPointTotal)}).`);
    }
    const ownedItemKeys = [];
...
function writeDatabasePlayer(db, payload) {
    const updateAccount = db.prepare('UPDATE "accounts" SET json = ? WHERE key = ?');
    const renameAccount = db.prepare('UPDATE "accounts" SET key = ?, json = ? WHERE key = ?');
    const updateCharacter = db.prepare('UPDATE "characters" SET json = ? WHERE key = ?');
    const upsertSkillMap = db.prepare('INSERT INTO "skills"(key, json) VALUES (?, ?) ' +
        'ON CONFLICT(key) DO UPDATE SET json = excluded.json');
    const deleteItem = db.prepare('DELETE FROM "items" WHERE key = ?');
    const upsertItem = db.prepare('INSERT INTO "items"(key, json) VALUES (?, ?) ' +
        'ON CONFLICT(key) DO UPDATE SET json = excluded.json');
    const upsertIdentity = db.prepare('INSERT INTO "identityState"(key, json) VALUES (?, ?) ' +
        'ON CONFLICT(key) DO UPDATE SET json = excluded.json');
    const transaction = db.transaction(() => {
        const tables = readPlayerTablesFromDatabase(db);
        const context = validateDatabaseSave(payload, tables);
        // Keep the persisted role/chatRole consistent with the isGM flag the GUI
        // sends, so a GM toggle takes effect immediately and the login path (which
        // re-runs the same normalization) does not fight it.
        const accountJson = JSON.stringify(accountRoleProfiles.buildPersistedAccountRoleRecord(payload.account));
        const accountResult = context.originalAccountKey === context.accountName
            ? updateAccount.run(accountJson, context.originalAccountKey)
            : renameAccount.run(context.accountName, accountJson, context.originalAccountKey);
        if (accountResult.changes !== 1) {
            throw new Error(`SQLite did not update account ${context.originalAccountKey}.`);
        }
        const characterResult = updateCharacter.run(JSON.stringify(payload.character), context.characterId);
...
function getMarketClientClass() {
    if (!_marketClientModule) {
        _marketClientModule = require(path.join(rootDir, "server/src/services/market/marketDaemonClient"));
    }
...
function listKnownAlliances() {
    ...
    try {
        let rows;
        try {
            rows = db.prepare("SELECT key, json FROM alliances").all();
        }
        catch (error) {
            return [];
        }
        ...
        for (const row of rows) {
            ...
            const allianceID = Number(parsed && parsed.allianceID);
            if (!Number.isFinite(allianceID) || allianceID <= 0) {
                continue;
            }
            if (!byId.has(allianceID)) {
                byId.set(allianceID, {
                    allianceID,
                    allianceName: String((parsed && parsed.allianceName) || ""),
                    shortName: String((parsed && parsed.shortName) || ""),
                });
            }
        }
        return [...byId.values()];
...
// Scans every logo root, so logos still sitting in the pre-migration location
// are reported as present rather than looking like a missing upload.
function scanAllianceLogoSizes(allianceID) {
    const present = new Set();
    for (const { size, filePath } of allianceImageStore.listAllianceLogoPaths(allianceID)) {
...
function scanAllianceLogoDirIds() {
    const ids = new Set();
    for (const dir of allianceImageStore.listAllianceLogoRoots()) {
        if (!fs.existsSync(dir)) {
            continue;
        }
        for (const entry of fs.readdirSync(dir)) {
            const match = /^(\d+)_\d+\.png$/i.exec(entry);
            if (match) {
...
function buildAllianceLogoPayload() {
    const alliances = listKnownAlliances().map((alliance) => {
        const sizes = scanAllianceLogoSizes(alliance.allianceID);
        return { ...alliance, sizes, hasLogo: sizes.length > 0 };
    });
    const known = new Set(alliances.map((alliance) => alliance.allianceID));
    // Surface any logos on disk whose alliance is not in the game database, so
    // the GUI can still list and manage (or clean up) them.
    for (const allianceID of scanAllianceLogoDirIds()) {
        if (known.has(allianceID)) {
            continue;
        }
        const sizes = scanAllianceLogoSizes(allianceID);
        alliances.push({
            allianceID,
            allianceName: "",
            shortName: "",
            sizes,
            hasLogo: sizes.length > 0,
        });
    }
    alliances.sort((left, right) => {
        const leftName = left.allianceName || String(left.allianceID);
        const rightName = right.allianceName || String(right.allianceID);
        return leftName.localeCompare(rightName);
    });
    return {
        paths: { allianceLogoRoot: allianceImageStore.getAllianceLogoRoot() },
        imageSizes: [...allianceImageStore.ALLIANCE_IMAGE_SIZES],
        defaultLogoPath: allianceImageStore.DEFAULT_ALLIANCE_LOGO_PATH,
        alliances,
    };
}
function importAllianceLogo() {
    const payload = readJsonFromStdin();
    const allianceID = Number(payload && payload.allianceID);
    if (!Number.isFinite(allianceID) || allianceID <= 0) {
        throw new Error("A valid alliance ID is required to import a logo.");
    }
    ...
    if (buffer.length < 8 || !buffer.subarray(0, 8).equals(PNG_MAGIC)) {
        throw new Error("The source image must be a PNG file (alliance logos are served to the client as PNG).");
    }
    const requestedSizes = Array.isArray(payload && payload.sizes)
        ? payload.sizes.map((size) => Number(size)).filter((size) => Number.isFinite(size) && size > 0)
        : [];
    const sizes = requestedSizes.length > 0
        ? requestedSizes
        : [...allianceImageStore.ALLIANCE_IMAGE_SIZES];
    allianceImageStore.ensureDirectory(allianceImageStore.getAllianceLogoRoot());
    const written = [];
    for (const size of sizes) {
        const filePath = allianceImageStore.getAllianceLogoFilePath(allianceID, size);
        fs.writeFileSync(filePath, buffer);
        written.push({ size, filePath });
    }
    return {
        ok: true,
        allianceID,
        sizes: written.map((entry) => entry.size),
        written,
        byteLength: buffer.length,
        allianceLogoRoot: allianceImageStore.getAllianceLogoRoot(),
    };
}
function deleteAllianceLogo() {
    const payload = readJsonFromStdin();
    const allianceID = Number(payload && payload.allianceID);
    if (!Number.isFinite(allianceID) || allianceID <= 0) {
        throw new Error("A valid alliance ID is required to remove a logo.");
    }
    const removed = [];
    for (const { size, filePath } of allianceImageStore.listAllianceLogoPaths(allianceID)) {
        if (fs.existsSync(filePath)) {
    ...
    return { ok: true, allianceID, removed };
}
function createAccount() {
    const payload = readJsonFromStdin();
    requirePlainObject(payload, "Account create payload");
    const username = String(payload.username ?? "").trim();
    if (!username) {
        throw new Error("An account username is required.");
    }
    // GM/admin is the default for new accounts; only an explicit false opts out.
    const isGM = payload.isGM !== false;
    const password = String(payload.password ?? "");
    const db = openPlayerDatabase(false);
    try {
        const existing = db.prepare('SELECT 1 FROM "accounts" WHERE key = ?').get(username);
        if (existing) {
        ...
        for (const row of db.prepare('SELECT json FROM "accounts"').all()) {
            try {
                const id = Number(JSON.parse(row.json).id);
                if (Number.isFinite(id) && id > maxId) {
                    maxId = id;
                }
            }
            catch (error) {
                // ignore malformed account rows when computing the next id
            }
        }
        const nextId = maxId + 1;
        // The EVE client hashes the password before sending it, so we cannot
        // reproduce the exact login hash here; store a stable placeholder. Login
        // succeeds via devSkipPasswordValidation on this dev server.
        const passwordhash = crypto
            .createHash("sha1")
            .update(password, "utf8")
            .digest("hex");
        const account = accountRoleProfiles.buildPersistedAccountRoleRecord({
            passwordhash,
            id: nextId,
            isGM,
            banned: false,
        });
        db.prepare('INSERT INTO "accounts"(key, json) VALUES (?, ?)').run(username, JSON.stringify(account));
        return {
            ok: true,
            username,
            accountId: nextId,
            isGM: account.isGM === true,
            note: "Password validation is skipped on this server (devSkipPasswordValidation), " +
                "so this account logs in with any password. It has no character yet — " +
                "create one from the game client.",
        };
    }
    finally {
        db.close();
...
async function main() {
    ...
    // Gamestore-backed commands must run against the database the server is
    // actually using. Under the Docker deployment that lives in a volume with no
    // host path, so run the command inside the container instead of reading a
    // stale host copy. Native deployments fall through unchanged.
    if (GAMESTORE_COMMANDS.has(command) &&
        !hasExplicitGameStoreOverride() &&
        !isRunningInsideContainer()) {
        const composePrefix = resolveDockerDelegation({ rootDir, databasePath });
        if (composePrefix) {
            const relayed = delegateNodeScript({
                rootDir,
                composePrefix,
                scriptPath: containerScriptPath(rootDir, __filename),
                argv: process.argv.slice(2),
            });
            process.stdout.write(command === "alliance-logo-export"
                ? rehostContainerRepoPaths(relayed)
                : relayed);
            return;
        }
    }
    switch (command) {
        case "export":
            process.stdout.write(`${JSON.stringify(buildConfigExportPayload(), null, 2)}\n`);
            return;
        case "save":
            process.stdout.write(`${JSON.stringify(saveConfig(), null, 2)}\n`);
            return;
        case "database-export":
            process.stdout.write(`${JSON.stringify(buildDatabasePayload(argument || null), null, 2)}\n`);
            return;
        case "database-save":
            process.stdout.write(`${JSON.stringify(await saveDatabasePlayer(), null, 2)}\n`);
            return;
        case "account-create":
            process.stdout.write(`${JSON.stringify(createAccount(), null, 2)}\n`);
            return;
        case "database-type-search":
            process.stdout.write(`${JSON.stringify(searchDatabaseTypes(argument || "item", secondaryArgument || ""), null, 2)}\n`);
            return;
        case "market-status":
            process.stdout.write(`${JSON.stringify(await getMarketStatus(), null, 2)}\n`);
            return;
        case "market-orders":
            process.stdout.write(`${JSON.stringify(await getMarketOrders(argument, secondaryArgument === "corp"), null, 2)}\n`);
            return;
        case "market-book":
            process.stdout.write(`${JSON.stringify(await getMarketBook(argument, secondaryArgument), null, 2)}\n`);
            return;
        case "market-place":
        ...
        case "market-adjust-seed":
            ...
            return;
        case "alliance-logo-export":
            process.stdout.write(`${JSON.stringify(buildAllianceLogoPayload(), null, 2)}\n`);
            return;
        case "alliance-logo-import":
            process.stdout.write(`${JSON.stringify(importAllianceLogo(), null, 2)}\n`);
            return;
        case "alliance-logo-delete":
            process.stdout.write(`${JSON.stringify(deleteAllianceLogo(), null, 2)}\n`);
            return;
        default:
            throw new Error(`Unknown command "${command}". Use "export", "save", "database-export", "database-save", "database-type-search", "market-status", "market-orders", "market-book", "market-place", "market-cancel", "market-modify", "alliance-logo-export", "alliance-logo-import", or "alliance-logo-delete".`);
    }
}
main().catch((error) => {
...
//# sourceMappingURL=config-manager-cli.js.map