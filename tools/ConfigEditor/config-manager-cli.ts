...
const allianceImageStore = require(path.join(
  rootDir,
  "server",
  "src",
  "services",
...
const accountRoleProfiles = require(path.join(
  rootDir,
  "server",
  "src",
  "services",
...
const {
  ...
  toHostRepoPath,
} = require(path.join(__dirname, "dockerDeployment"));
...
function inferSubGroup(key, metadata: Record<string, any> = {}) {
...
function inferGroup(entry, metadata: Record<string, any> = {}) {
  if (metadata.group) {
...
function buildControlOptions(entry, metadata: Record<string, any> = {}) {
...
function getRecordMap(data) {
  ...
  const arrayValue = Object.values<any>(data || {}).find((value) => Array.isArray(value));
  if (Array.isArray(arrayValue)) {
    return mapArrayRecords(arrayValue);
  }

  return Object.fromEntries(
    Object.entries<any>(data || {}).filter(
      ([key, value]) =>
...
function loadSkillRanksByTypeID() {
  ...
  const dogmaTypes = isPlainObject(typeDogmaPayload?.typesByTypeID)
    ...
    : {};
  const ranksByTypeID: Record<string, any> = {};

  for (const [typeID, dogmaType] of Object.entries<any>(dogmaTypes)) {
...
function buildAccountMaps(accounts) {
  const accountEntries = Object.entries<any>(accounts || {});
...
function buildSkillList(skillMap) {
  return Object.entries<any>(skillMap || {})
    .map(([skillKey, skill]) => ({
...
function buildSkillCatalog(skillsTable, skillTypes = [], skillRanksByTypeID: Record<string, any> = {}) {
  ...
  for (const skillMap of Object.values<any>(skillsTable || {})) {
    for (const [skillKey, skill] of Object.entries<any>(skillMap || {})) {
...
function buildNextItemIdSeed(itemsTable, identityState: Record<string, any> = {}) {
  let maxItemId = 990000000;
  for (const item of Object.values<any>(itemsTable || {})) {
...
function buildPlayerList(tables, references) {
  ...
  return Object.entries<any>(characters || {})
...
function buildPlayerDetail(characterId, tables, references) {
  ...
  const skills = isPlainObject(tables.skills?.[normalizedCharacterId])
    ...
    : {};
  const items = Object.values<any>(tables.items || {}).filter(
...
function validateDatabaseSave(payload, tables) {
  ...
  for (const [key, account] of Object.entries<any>(tables.accounts)) {
  ...
  for (const [skillKey, skill] of Object.entries<any>(payload.skills)) {
  ...
  const ownedItemKeys = [];
  for (const [itemKey, item] of Object.entries<any>(tables.items)) {
    if (String(item?.ownerID ?? "") === characterId) {
      ownedItemKeys.push(String(itemKey));
    }
  }

  for (const [itemKey, item] of Object.entries<any>(payload.items)) {
...
function writeDatabasePlayer(db, payload) {
  ...
  const transaction = db.transaction(() => {
    ...
    const accountJson = JSON.stringify(
      accountRoleProfiles.buildPersistedAccountRoleRecord(payload.account),
    );
...
async function saveDatabasePlayer() {
  ...
  const snapshot: ReturnType<typeof buildDatabasePayload> & { backupPath?: string } = buildDatabasePayload(payload.characterId);
...
async function adjustSeedStock() {
  ...
  const request: { station_id: number; type_id: number; delta_quantity: number; reason: string; new_price?: number } = {
    station_id: Number(payload.stationId ?? payload.station_id) || 0,
    type_id: Number(payload.typeId ?? payload.type_id) || 0,
...
function importAllianceLogo() {
  ...
  if (buffer.length < 8 || !buffer.subarray(0, 8).equals(PNG_MAGIC)) {
    throw new Error(
      "The source image must be a PNG file (alliance logos are served to the client as PNG).",
    );
  }
...
function createAccount() {
  ...
  try {
    ...
    db.prepare('INSERT INTO "accounts"(key, json) VALUES (?, ?)').run(
      username,
    ...
    return {
      ...
      note:
        "Password validation is skipped on this server (devSkipPasswordValidation), " +
...
async function main() {
  ...
  if (
    GAMESTORE_COMMANDS.has(command) &&
    !hasExplicitGameStoreOverride() &&
    !isRunningInsideContainer()
  ) {
  ...
  switch (command) {
    ...
    case "alliance-logo-export":
      process.stdout.write(
        `${JSON.stringify(buildAllianceLogoPayload(), null, 2)}\n`,
      );
      return;
    case "alliance-logo-import":
      process.stdout.write(
        `${JSON.stringify(importAllianceLogo(), null, 2)}\n`,
      );
      return;
    case "alliance-logo-delete":
      process.stdout.write(
        `${JSON.stringify(deleteAllianceLogo(), null, 2)}\n`,
      );
      return;
    default:
      throw new Error(
        `Unknown command "${command}". Use "export", "save", "database-export", "database-save", "database-type-search", "market-status", "market-orders", "market-book", "market-place", "market-cancel", "market-modify", "alliance-logo-export", "alliance-logo-import", or "alliance-logo-delete".`,
      );
  }
}
